...
void FX_DisruptorMainShot(vec3_t start, vec3_t end) {
	...
	trap->FX_AddLine(start, end, 0.1f, 6.0f, 0.0f, 1.0f, 0.0f, 0.0f, WHITE, WHITE, 0.0f, 150, trap->R_RegisterShader("gfx/effects/redLine"),
...
void FX_DisruptorAltShot(vec3_t start, vec3_t end, qboolean fullCharge) {
	trap->FX_AddLine(start, end, 0.1f, 10.0f, 0.0f, 1.0f, 0.0f, 0.0f, WHITE, WHITE, 0.0f, 175, trap->R_RegisterShader("gfx/effects/redLine"),
					 FX_SIZE_LINEAR | FX_ALPHA_LINEAR);
	...
	if (fullCharge) {
		...
		// add some beef
		trap->FX_AddLine(start, end, 0.1f, 7.0f, 0.0f, 1.0f, 0.0f, 0.0f, YELLER, YELLER, 0.0f, 150, trap->R_RegisterShader("gfx/misc/whiteline2"),
						 FX_SIZE_LINEAR | FX_ALPHA_LINEAR);
	}
}
...
void FX_DisruptorAltHit(vec3_t origin, vec3_t normal) { trap->FX_PlayEffectID(cgs.effects.disruptorAltHitEffect, origin, normal, -1, -1, qfalse); }
...
void FX_DisruptorHitWall(vec3_t origin, vec3_t normal) { trap->FX_PlayEffectID(cgs.effects.disruptorWallImpactEffect, origin, normal, -1, -1, qfalse); }
