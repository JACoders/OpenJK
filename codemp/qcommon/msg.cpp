...
void MSG_WriteDeltaUsercmdKey(msg_t *msg, int key, usercmd_t *from, usercmd_t *to) {
	...
	if (from->angles[0] == to->angles[0] && from->angles[1] == to->angles[1] && from->angles[2] == to->angles[2] && from->forwardmove == to->forwardmove &&
		...
		from->forcesel == to->forcesel && from->invensel == to->invensel && from->generic_cmd == to->generic_cmd) {
...
netField_t entityStateFields[] = {
	...
	{NETF(forceFrame),
	 16}, // if you have over 65536 frames, then this will explode. Of course if you have that many things then lots of things will probably explode.
	{NETF(generic1), 8},
...
netField_t playerStateFields[] = {
	...
	{PSF(fd.forcePowerLevel[FP_LEVITATION]),
	 2}, // unfortunately we need this for fall damage calculation (client needs to know the distance for the fall noise)
...
netField_t pilotPlayerStateFields[] = {
	...
	//===THE REST OF THESE SHOULD NOT BE RELEVANT, BUT, FOR SAFETY, INCLUDE THEM ANYWAY, JUST AT THE
	//BOTTOM===============================================================
	{PSF(velocity[0]), 0},
	{PSF(velocity[1]), 0},
	{PSF(velocity[2]), 0},
...
void MSG_CheckNETFPSFOverrides(qboolean psfOverrides) {
	...
	if (*bitStorage) { // if we have saved off the defaults before we want to stuff them all back in now
	...
	if (!f || len < 0) { // silently exit since this file is not needed to proceed.
		return;
	}
	...
	while (overrideFile[i]) {
		...
		if (overrideFile[i] != ';' && overrideFile[i] != '\n' && overrideFile[i] != '\r') { // on a valid char I guess, parse it
			...
			while (overrideFile[i] == ',' || overrideFile[i] == ' ') { // parse to the start of the value
				i++;
			}
			...
			if (bits[0]) {
				...
				while (j < numFields) {
					if (psfOverrides) {										 // check psf fields
						if (!strcmp(playerStateFields[j].name, entryName)) { // found it, set the bits
							playerStateFields[j].bits = ibits;
							break;
						}
					} else {												 // otherwise check netf fields
						if (!strcmp(entityStateFields[j].name, entryName)) { // found it, set the bits
...
//=====_OPTIMIZED_VEHICLE_NETWORKING=======================================================================
	...
	if (isVehiclePS) { // a vehicle playerstate
		...
		if (to->m_iVehicleNum && (to->eFlags & EF_NODRAW)) { // pilot riding *inside* a vehicle!
...
#ifdef _ONEBIT_COMBO
sendBitMask:
	if (numBitsInMask) {																		 // don't need to send at all if we didn't pass any 1bit values
		if (!bitComboDelta || bitComboMask != *bitComboDelta || numBitsInMask != *bitNumDelta) { // send the mask, it changed
			MSG_WriteBits(msg, 1, 1);
