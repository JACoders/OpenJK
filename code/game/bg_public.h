...
#define PMF_JUMPING                                                            \
  (1 << 2) // 4		// yes, I really am in a jump -- Mike, you may want to come
           // up with something better here since this is really a temp fix.
#define PMF_BACKWARDS_JUMP (1 << 3) // 8		// go into backwards
                                    // land
#define PMF_BACKWARDS_RUN                                                      \
  (1 << 4) // 16		// coast down to backwards run
#define PMF_TIME_LAND                                                          \
  (1 << 5) // 32		// pm_time is time before rejump
#define PMF_TIME_KNOCKBACK                                                     \
  (1 << 6) // 64		// pm_time is an air-accelerate only time
#define PMF_TIME_NOFRICTION                                                    \
  (1 << 7)                          // 128		// pm_time is a no-friction time
#define PMF_TIME_WATERJUMP (1 << 8) // 256		// pm_time is waterjump
#define PMF_RESPAWNED                                                          \
  (1 << 9) // 512		// clear after attack and jump buttons come up
...
#define PMF_TRIGGER_PUSHED                                                     \
  (1 << 12) // 4096	// pushed by a trigger_push or other such thing - cannot
            // force jump and will not take impact damage
#define PMF_STUCK_TO_WALL (1 << 13) // 8192	// grabbing a wall
#define PMF_SLOW_MO_FALL (1 << 14)  // 16384	// Fall slower until hit ground
#define PMF_ATTACK_HELD (1 << 15)   // 32768	// Holding down the attack
                                  // button
#define PMF_ALT_ATTACK_HELD                                                    \
  (1 << 16)                  // 65536	// Holding down the alt-attack button
#define PMF_BUMPED (1 << 17) // 131072	// Bumped into something
#define PMF_FORCE_FOCUS_HELD                                                   \
  (1 << 18) // 262144	// Holding down the saberthrow/kick button
#define PMF_FIX_MINS                                                           \
  (1 << 19) // 524288	// Mins raised for dual forward jump, fix them
...
void PM_SetAnim(pmove_t *pm, int setAnimParts, int anim, int setAnimFlags,
                int blendTime = SETANIM_BLEND_DEFAULT);
void PM_SetAnimFinal(int *torsoAnim, int *legsAnim, int type, int anim,
                     int priority, int *torsoAnimTimer, int *legsAnimTimer,
                     gentity_t *gent, int blendTime = SETANIM_BLEND_DEFAULT);
...
// cleared on respawn
//
//  NOTE!!! Even though this is an enum, the array that contains these uses
//  #define MAX_PERSISTANT 16 in q_shared.h,
//		so be careful how many you add since it'll just overflow without
//telling you -slc
//
typedef enum {
  ...
  PERS_SPAWN_COUNT, // incremented every respawn
                    //	PERS_REWARD_COUNT,				// incremented for each reward
                    //sound
...
#define EF_ALT_FIRING                                                          \
  0x00000200 // for alt-fires, mostly for lightning guns though
#define EF_VEH_BOARDING 0x00000400 // Whether a vehicle is being boarded or not.
#define EF_AUTO_SIZE                                                           \
  0x00000800 // CG_Ents will create the mins & max itself based on model bounds
...
#define EF_ANIM_ALLFAST                                                        \
  0x00004000 // automatically cycle through all frames at 10hz
...
#define EF_LOCKED_TO_WEAPON                                                    \
  0x00040000 // When we use an emplaced weapon, we turn this on to lock us to
             // that weapon

// rest not sent over net?

#define EF_PERMANENT                                                           \
  0x00080000 // this entity is permanent and is never updated (sent only in the
             // game state)
...
#define EF_LESS_ATTEN                                                          \
  0x04000000 // Use less sound attenuation (louder even when farther).
#define EF_JETPACK_ACTIVE 0x08000000 // Not used
#define EF_DISABLE_SHADER_ANIM                                                 \
  0x10000000 // Normally shader animation chugs along, but movers can force
             // shader animation to be on frame 1
#define EF_FORCE_GRIPPED 0x20000000 // Force gripped effect
#define EF_FORCE_DRAINED 0x40000000 // Force drained effect
#define EF_BLOCKED_MOVER                                                       \
  0x80000000 // for movers that are blocked - shared with previous
...
#define PW_REMOVE_AT_DEATH                                                     \
  ((1 << PW_QUAD) | (1 << PW_BATTLESUIT) | (1 << PW_HASTE) |                   \
   (1 << PW_CLOAKED) | (1 << PW_UNCLOAKING) | (1 << PW_UNCLOAKING) |           \
   (1 << PW_GALAK_SHIELD) | (1 << PW_DISINT_2) | (1 << PW_INVINCIBLE) |        \
   (1 << PW_SEEKER))
...
#define MASK_OPAQUE                                                            \
  (CONTENTS_OPAQUE | CONTENTS_SLIME |                                          \
   CONTENTS_LAVA) // was CONTENTS_SOLID, not CONTENTS_OPAQUE...?
/*
Ghoul2 Insert Start
*/
#define MASK_SHOT                                                              \
  (CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_CORPSE | CONTENTS_SHOTCLIP |      \
   CONTENTS_TERRAIN)
/*
Ghoul2 Insert End
*/
