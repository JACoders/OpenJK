...
static void Svcmd_ExitView_f(void) {
	...
	if (in_camera) {				// see if we need to exit an in-game cinematic
	...
	} else if (!G_ClearViewEntity(player)) { // didn't exit control of a droid or turret
		// okay, now try exiting emplaced guns or AT-ST's
		if (player->s.eFlags & EF_LOCKED_TO_WEAPON) { // get out of emplaced gun
			ExitEmplacedWeapon(player);
		} else if (player->client && player->client->NPC_class == CLASS_ATST) { // a player trying to get out of his ATST
...
gentity_t *G_GetSelfForPlayerCmd(void) {
	if (g_entities[0].client->ps.viewEntity > 0 && g_entities[0].client->ps.viewEntity < ENTITYNUM_WORLD &&
		g_entities[g_entities[0].client->ps.viewEntity].client &&
		g_entities[g_entities[0].client->ps.viewEntity].s.weapon == WP_SABER) { // you're controlling another NPC
...
SetForceCmd SetForceTable[] = {
	{"forceHeal", "setForceHeal", FORCE_LEVEL_3},		{"forceJump", "setForceJump", FORCE_LEVEL_3},
	{"forceSpeed", "setForceSpeed", FORCE_LEVEL_3},		{"forcePush", "setForcePush", FORCE_LEVEL_3},
	{"forcePull", "setForcePull", FORCE_LEVEL_3},		{"forceMindTrick", "setForceMindTrick", FORCE_LEVEL_4},
	{"forceGrip", "setForceGrip", FORCE_LEVEL_3},		{"forceLightning", "setForceLightning", FORCE_LEVEL_3},
	{"saberThrow", "setSaberThrow", FORCE_LEVEL_3},		{"saberDefense", "setSaberDefense", FORCE_LEVEL_3},
...
void Svcmd_SaberAttackCycle_f(void) {
	...
	if (self->client->ps.forcePowerLevel[FP_SABER_OFFENSE] == FORCE_LEVEL_1) {
		...
		if (saberAnimLevel > self->client->ps.forcePowerLevel[FP_SABER_OFFENSE]) { // wrap around
			saberAnimLevel = FORCE_LEVEL_1;
		}
	}
...
template <int32_t power> static void Svcmd_ForceSetLevel_f(void) { Svcmd_ForceSetLevel_f(power); }
...
static int svcmdcmp(const void *a, const void *b) { return Q_stricmp((const char *)a, ((svcmd_t *)b)->name); }
...
qboolean ConsoleCommand(void) {
	...
	if ((command->flags & CMD_CHEAT) && !g_cheats->integer) {
		gi.Printf("Cheats are not enabled on this server.\n");
		return qtrue;
	} else if ((command->flags & CMD_ALIVE) && (g_entities[0].health <= 0)) {
