  ...
  if (s_testsound->integer) {
    ...
    for (i = 0; i < count; i++)
      paintbuffer[i].left = paintbuffer[i].right =
          (int)(sin((s_paintedtime + i) * 0.1) * 20000 * 256);
  }
...
===============================================================================
*/
static void S_PaintChannelFrom16(channel_t *ch, const sfx_t *sfx, int count,
...
void S_PaintChannelFromMP3(channel_t *ch, const sfx_t *sc, int count,
                           int sampleOffset, int bufferOffset) {
  int data;
  ...
  MP3Stream_GetSamples(ch, sampleOffset, count, tempMP3Buffer,
                       qfalse); // qfalse = not stereo
...
//
void ChannelPaint(channel_t *ch, sfx_t *sc, int count, int sampleOffset,
                  int bufferOffset) {
  ...
  default:
    ...
    assert(0); // debug aid, ignored in release. FIXME: Should we ERR_DROP here
               // for badness-catch?
    break;
  }
}
    ...
    /* temprem
                    ...
                    for ( i = 0; i < numLoopChannels ; i++, ch++ ) {
...

                            {
                                    ...
                                    if (sc->soundData==NULL ||
       sc->soundLength==0) { continue;
                                    }
       ...
       sc->soundLength ) { count = sc->soundLength - sampleOffset;
                                            }

                                            if ( count > 0 )
                                            {
                                                    ChannelPaint(ch, sc, count,
       sampleOffset, ltime - s_paintedtime); ltime += count;
                                            }
