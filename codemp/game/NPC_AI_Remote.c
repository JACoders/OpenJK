  ...
  if (NPCS.NPC->enemy) {
    if (TIMER_Done(NPCS.NPC, "heightChange")) {
      ...
      dif = (NPCS.NPC->enemy->r.currentOrigin[2] +
             Q_irand(0, NPCS.NPC->enemy->r.maxs[2] + 8)) -
...
void Remote_Strafe(void) {
  ...
  // Close enough
  if (tr.fraction > 0.9f) {
    VectorMA(NPCS.NPC->client->ps.velocity, REMOTE_STRAFE_VEL * dir, right,
...
void Remote_Hunt(qboolean visible, qboolean advance, qboolean retreat) {
  ...
  speed = REMOTE_FORWARD_BASE_SPEED +
          REMOTE_FORWARD_MULTIPLIER * g_npcspskill.integer;
  if (retreat == qtrue) {
    speed *= -1;
  }
...
void Remote_Fire(void) {
  ...
  missile = CreateMissile(NPCS.NPC->r.currentOrigin, forward, 1000, 10000,
                          NPCS.NPC, qfalse);

  G_PlayEffectID(G_EffectIndex("bryar/muzzle_flash"), NPCS.NPC->r.currentOrigin,
                 forward);
