  ...
  void EvalLow(int index) {
    ...
    if (mFinalBones[index].touch != mCurrentTouch) {
      ...
      assert((mFinalBones[index].parent >= 0 &&
              mFinalBones[index].parent < mNumBones) ||
...
public:
  ...
  bool mUnsquash;
  float mSmoothFactor;
  //	int				mWraithID; // this is just used for debug prints, can
  //use it for any int of interest in JK2
  ...
  CBoneCache(const model_t *amod, const mdxaHeader_t *aheader)
    ...
    mFinalBones = (CTransformBone *)R_Malloc(sizeof(CTransformBone) * mNumBones,
                                             TAG_GHOUL2, qtrue);
    mSmoothBones = (CTransformBone *)R_Malloc(
        sizeof(CTransformBone) * mNumBones, TAG_GHOUL2, qtrue);
  ...
  const mdxaBone_t &Eval(int index) {
    ...
    *newM=&mFinalBones[index].boneMatrix.matrix[0][0]; for
    ...
    return mFinalBones[index].boneMatrix;
    */
    // all above is not necessary, smoothing is taken care of when we want to
    // use smoothlow (only when evalrender)
    assert(index >= 0 && index < mNumBones);
    if (mFinalBones[index].touch != mCurrentTouch) {
      EvalLow(index);
    }
...
void G2_GetBoneMatrixLow(CGhoul2Info &ghoul2, int boneNum, const vec3_t scale,
  ...
  skel = (mdxaSkel_t *)((byte *)boneCache.header + sizeof(mdxaHeader_t) +
                        offsets->offsets[boneNum]);
  Multiply_3x4Matrix(&bolt, &boneCache.Eval(boneNum),
...
void RemoveBoneCache(CBoneCache *boneCache) { delete boneCache; }
...
public:
  ...
  CRenderSurface(int initsurfaceNum, surfaceInfo_v &initrootSList,
                 const shader_t *initcust_shader, int initfogNum,
                 qboolean initpersonalModel, CBoneCache *initboneCache,
                 int initrenderfx, const skin_t *initskin,
                 const model_t *initcurrentModel, int initlod,
#ifdef _G2_GORE
...
static int R_GComputeFogNum(trRefEntity_t *ent) {
  ...
  for (i = 1; i < tr.world->numfogs; i++) {
    ...
    if (ent->e.origin[0] - ent->e.radius >= fog->bounds[0][0] &&
        ...
        ent->e.origin[2] + ent->e.radius <=
            fog->bounds[1][2]) { // totally inside it
      return i;
      break;
    }
    if ((ent->e.origin[0] - ent->e.radius >= fog->bounds[0][0] &&
        ...
        (ent->e.origin[0] + ent->e.radius >= fog->bounds[0][0] &&
         ent->e.origin[1] + ent->e.radius >= fog->bounds[0][1] &&
         ent->e.origin[2] + ent->e.radius >= fog->bounds[0][2] &&
         ent->e.origin[0] + ent->e.radius <= fog->bounds[1][0] &&
         ent->e.origin[1] + ent->e.radius <= fog->bounds[1][1] &&
         ent->e.origin[2] + ent->e.radius <=
             fog->bounds[1][2])) { // partially inside it
      if (tr.refdef.fogIndex == i ||
          R_FogParmsMatch(tr.refdef.fogIndex,
                          i)) { // take new one only if it's the same one that
                                // the viewpoint is in
        return i;
        break;
      } else if (!partialFog) { // first partialFog
        partialFog = i;
      }
    }
  }
...
void Multiply_3x4Matrix(mdxaBone_t *out, const mdxaBone_t *in2,
                        const mdxaBone_t *in) {
  // first row of out
...
// transform each individual bone's information - making sure to use any
// override information provided, both for angles and for animations, as well as
// multiplying each bone's matrix by it's parents matrix
void G2_TransformBone(int child, CBoneCache &BC) {
  ...
  if (boneListIndex != -1) {
    ...
    printTiming = true;
#endif
    if ((r_Ghoul2NoLerp->integer) ||
        ((boneList[boneListIndex].flags) & (BONE_ANIM_NO_LERP))) {
      TB.backlerp = 0.0f;
    }
  }
...
// start the recursive hirearchial bone transform and lerp process for this
// model
void G2_TransformGhoulBones(boneInfo_v &rootBoneList, mdxaBone_t &rootMatrix,
    ...
    if (ghoul2.mFlags & GHOUL2_RAG_STARTED) {
      for (size_t k = 0; k < rootBoneList.size(); k++) {
        ...
        if (bone.flags & BONE_ANGLES_RAGDOLL) {
          if (bone.firstCollisionTime && bone.firstCollisionTime > time - 250 &&
  ...
  ghoul2.mBoneCache->frameSize =
      0; // can be deleted in new G2 format	//(int)( &((mdxaFrame_t
...
void G2_GetBoltMatrixLow(CGhoul2Info &ghoul2, int boltNum, const vec3_t scale,
  ...
  if (boltList[boltNum].boneNumber >= 0) {
    ...
    Multiply_3x4Matrix(&retMatrix,
                       &boneCache.EvalUnsmooth(boltList[boltNum].boneNumber),
                       &skel->BasePoseMat);
    ...
    if (!surfInfo) {
      surface = (mdxmSurface_t *)G2_FindSurface(
          boneCache.mod, boltList[boltNum].surfaceNumber, 0);
    }
    if (!surface && surfInfo && surfInfo->surface < 10000) {
      surface =
          (mdxmSurface_t *)G2_FindSurface(boneCache.mod, surfInfo->surface, 0);
    }
...
void G2API_SetSurfaceOnOffFromSkin(CGhoul2Info *ghlInfo, qhandle_t renderSkin) {
  ...
  // FIXME:  using skin handles means we have to increase the numsurfs in a
  // skin, but reading directly would cause file hits, we need another way to
  // cache or just deal with the larger skin_t
  ...
  if (skin) {
    ...
    for (j = 0; j < skin->numSurfaces; j++) {
      uint32_t flags;
      int surfaceNum = G2_IsSurfaceLegal(ghlInfo->currentModel,
                                         skin->surfaces[j]->name, &flags);
      // the names have both been lowercased
      if (!(flags & G2SURFACEFLAG_OFF) &&
          !strcmp(skin->surfaces[j]->shader->name, "*off")) {
        G2_SetSurfaceOnOff(ghlInfo, skin->surfaces[j]->name, G2SURFACEFLAG_OFF);
      } else {
        // if ( strcmp(
        // &skin->surfaces[j]->name[strlen(skin->surfaces[j]->name)-4],"_off") )
        if ((surfaceNum != -1) &&
            (!(flags & G2SURFACEFLAG_OFF))) // only turn on if it's not an
                                            // "_off" surface
        {
...
void RenderSurfaces(CRenderSurface &RS) {
  ...
  if (!offFlags) {
    ...
    if (r_shadows->integer == 3
        //			&& RS.fogNum == 0
        && (RS.renderfx & RF_SHADOW_PLANE) && !(RS.renderfx & (RF_NOSHADOW)) &&
        shader->sort ==
            SS_OPAQUE) { // set the surface info to point at the where the
                         // transformed bone list is going to be for when the
                         // surface gets rendered out
...
void R_AddGhoulSurfaces(trRefEntity_t *ent) {
  ...
  assert(ent->e.ghoul2); // entity is foo if it has a glm model handle but no
                         // ghoul2 pointer!
  ...
  for (j = 0; j < modelCount; j++) {
    ...
    if (ghoul2[i].mValid && !(ghoul2[i].mFlags & GHOUL2_NOMODEL) &&
      ...
      if (ent->e.renderfx & RF_G2MINLOD) {
        ...
        whichLod =
            G2_ComputeLOD(ent, ghoul2[i].currentModel, ghoul2[i].mLodBias);
      }
      ...
      CRenderSurface RS(ghoul2[i].mSurfaceRoot, ghoul2[i].mSlist, cust_shader,
                        ...
                        ent->e.renderfx, skin, ghoul2[i].currentModel, whichLod,
...
qboolean R_LoadMDXM(model_t *mod, void *buffer, const char *mod_name,
  ...
  if (!bAlreadyFound) {
    ...
    //to do an FS_Freefile since 	we've hijacked that memory block...
    //
    // Aaaargh. Kill me now...
    //
  ...
  if (strcmp(mapname, "nomap")) {
    if (strrchr(mapname, '/')) // maps in subfolders use the root name, (
                               // presuming only one level deep!)
    {
    ...
    if (VALIDSTRING(strippedName)) {
      RE_RegisterModel(va("models/players/%s_%s/%s_%s.gla", strippedName,
                          mapname, strippedName, mapname));
    }
  }
  ...
  if (!mdxm->animIndex) {
    ...
    if (tr.models[mdxm->animIndex]->mdxa->numBones != mdxm->numBones) {
...
#ifdef _DEBUG
        Com_Error(ERR_DROP,
                  "R_LoadMDXM: %s has different bones than anim (%i != %i)\n",
      ...
      if (!isAnOldModelFile) { // hmm, load up the old JK2 ones anyway?
        return qfalse;
      }
    }
  }
  ...
  for (i = 0; i < mdxm->numSurfaces; i++) {
    ...
    if (surfInfo->shader[0] == '[') {
      surfInfo->shader[0] =
          0; // kill the stupid [nomaterial] since carcass doesn't
    }
    ...
    if (!sh) {
      surfInfo =
          (mdxmSurfHierarchy_t
               *)((byte *)surfInfo +
                  (intptr_t)(&((mdxmSurfHierarchy_t *)0)
...
qboolean R_LoadMDXA(model_t *mod, void *buffer, const char *mod_name,
  ...
  if (!bAlreadyFound) {
    ...
    bAlreadyCached = qtrue;
    assert(mdxa == buffer);
    //		memcpy( mdxa, buffer, size );	// and don't do this now, since it's
    //the same thing
