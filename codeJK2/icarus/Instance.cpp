...
int ICARUS_Instance::SaveSequenceIDTable(void) {
	...
	saved_game.write_chunk<int32_t>(INT_ID('S', 'Q', 'T', 'B'), idTable, numSequences);
...
int ICARUS_Instance::LoadSequences(void) {
	...
	saved_game.read_chunk<int32_t>(INT_ID('S', 'Q', 'T', 'B'), idTable, numSequences);
...
void ICARUS_Instance::Signal(const char *identifier) { m_signals[identifier] = 1; }
...
void ICARUS_Instance::ClearSignal(const char *identifier) { m_signals.erase(identifier); }
