...
qboolean CROFFSystem::IsROFF(unsigned char *data) {
	...
	if (LittleLong(hdr->mVersion) != ROFF_VERSION && LittleLong(hdr->mVersion) != ROFF_NEW_VERSION) { // bad version
		return qfalse;
	}

	if (LittleLong(hdr->mVersion) == ROFF_VERSION && LittleFloat(hdr->mCount) <= 0.0) { // bad count
		return qfalse;
	}

	if (LittleLong(hdr->mVersion) == ROFF_NEW_VERSION && LittleLong(hdr2->mCount) <= 0) { // bad count
		return qfalse;
	}
...
// Ideally we would fix the ROFF exporter, if that doesn't happen, this may be an adequate solution
	...
	for (index = 0; index < obj->mROFFEntries; index++) {
		for (t = 0; t < 3; t++) {
			if (obj->mMoveRotateList[index].mRotateOffset[t] > 180.0f) { // found a bad angle
			...
			} else if (obj->mMoveRotateList[index].mRotateOffset[t] < -180.0f) { // found a bad angle
				//	Com_Printf( S_COLOR_YELLOW"Fixing bad roff angle\n <%6.2f> changed to <%6.2f>.\n",
				//				roff_data[i].mRotateOffset[t], roff_data[i].mRotateOffset[t] + 360.0f );
				obj->mMoveRotateList[index].mRotateOffset[t] += 360.0f;
			}
...
int CROFFSystem::GetID(const char *file) {
	...
	for (itr = mROFFList.begin(); itr != mROFFList.end(); ++itr) {
		if (!strcmp(((CROFF *)((*itr).second))->mROFFFilePath, file)) { // return the ID to this roff
			return (*itr).first;
		}
	}
...
qboolean CROFFSystem::Unload(int id) {
	...
	if (itr != mROFFList.end()) { // requested item found in the list, free mem, then remove from list
		delete itr->second;
...
void CROFFSystem::UpdateEntities(qboolean isClient) {
	...
	for (itr = mROFFEntList.begin(); itr != mROFFEntList.end(); ++itr) {
		...
		if (itrRoff != mROFFList.end()) {											   // roff that baby!
			if (!ApplyROFF(((SROFFEntity *)(*itr)), ((CROFF *)((*itrRoff).second)))) { // done roffing, mark for death
...
qboolean CROFFSystem::ApplyROFF(SROFFEntity *roff_ent, CROFFSystem::CROFF *roff) {
	...
	if (sv.time < roff_ent->mNextROFFTime) { // Not time to roff yet
		return qtrue;
	}
	...
	if (roff_ent->mROFFFrame >= roff->mROFFEntries) { // we are done roffing, so stop moving and flag this ent to be removed
	...
	SetLerp(angleTrajectory, TR_LINEAR, angle, roff->mMoveRotateList[roff_ent->mROFFFrame].mRotateOffset, sv.time, roff->mLerp);
