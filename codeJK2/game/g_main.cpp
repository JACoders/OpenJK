...
void ClearAllInUse(void) { memset(g_entityInUseBits, 0, sizeof(g_entityInUseBits)); }
...
void InitGame(const char *mapname, const char *spawntarget, int checkSum, const char *entities, int levelTime, int randomSeed, int globalTime,
	...
	if (navCalculatePaths) {								// not loaded - need to calc paths
		...
		if (g_eSavedGameJustLoaded == eNO) { // clear all the failed edges unless we just loaded the game (which would include failed edges)
...
void PrintEntClassname(int gentNum) { Com_Printf("%d: %s in snapshot\n", gentNum, g_entities[gentNum].classname); }
