  ...
  private:
	...
	void LeaderIssueAndUpdateOrders(ETroopState NextState) {
		...
		if (NextState > TS_ATTACK) {
			...
			gi.trace(&trace, mActors[0]->currentOrigin, mActors[0]->mins, mActors[0]->maxs, mOrders[0].mPosition.v, mActors[0]->s.number, mActors[0]->clipmask,
					 (EG2_Collision)0, 0);
		...
		for (actorIndex = 1; actorIndex < actorCount; actorIndex++) {
			...
			//-----------------------------
				...
				if (NextState == TS_ATTACK_LINE || (NextState == TS_ATTACK_FLANK && actorIndex < 4)) {
					...
					Order.mCombatPoint = NPC_FindCombatPointRetry(mActors[actorIndex]->currentOrigin, mActors[actorIndex]->currentOrigin,
			...
			if (NextState >= TS_ATTACK) {
				...
				gi.trace(&trace, Order.mPosition.v, mActors[actorIndex]->mins, mActors[actorIndex]->maxs, OrderUp.v, mActors[actorIndex]->s.number,
						 CONTENTS_SOLID | CONTENTS_TERRAIN | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP, (EG2_Collision)0, 0);
...
void Trooper_UpdateTroop(gentity_t *actor) {
		...
		for (TTroopPool::iterator iTroop = mTroops.begin(); iTroop != mTroops.end(); ++iTroop) {
			if (iTroop->Team() == actor->client->playerTeam) {
				...
				if (curDist < MAX_TROOP_JOIN_DIST2 && (!closestDist || curDist < closestDist)) {
					...
					//---------------------------------------------
					gi.trace(&trace, actor->currentOrigin, actor->mins, actor->maxs, iTroop->TroopLeader()->currentOrigin, actor->s.number,
							 CONTENTS_SOLID | CONTENTS_TERRAIN | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP, (EG2_Collision)0, 0);

					if (!trace.allsolid && !trace.startsolid && (trace.fraction >= 1.0f || trace.entityNum == iTroop->TroopLeader()->s.number)) {
						closestDist = curDist;
						closestTroop = iTroop;
					}
				}
	...
	//----------------------------------------------------------------
		...
		for (TTroopPool::iterator iTroop = mTroops.begin(); iTroop != mTroops.end(); ++iTroop) {
			curDist = iTroop->DistanceSq(actor);
			if ((curDist < MAX_TROOP_MERGE_DIST2) && (!closestDist || curDist < closestDist) &&
...
//
////////////////////////////////////////////////////////////////////////////////////////
bool Trooper_Kneeling(gentity_t *actor) { return (actor->NPC->aiFlags & NPCAI_KNEEL || actor->client->ps.legsAnim == BOTH_STAND_TO_KNEEL); }
	...
	if (target) {
		...
		if (AtPos && (troop.TimeSinceSeenTarget() < 1500)) {
			...
			if (traceTgt == target->s.number) {
				...
				//------------------------------------------------
				else if (traceTgt != ENTITYNUM_NONE && traceTgt != ENTITYNUM_WORLD && g_entities[traceTgt].client && g_entities[traceTgt].NPC &&
						 g_entities[traceTgt].client->playerTeam == actor->client->playerTeam && NPC_IsTrooper(&g_entities[traceTgt]) &&
						 g_entities[traceTgt].resultspeed < 1.0f && !(g_entities[traceTgt].NPC->aiFlags & NPCAI_KNEEL)) {
...
////////////////////////////////////////////////////////////////////////////////////////
bool NPC_IsTrooper(gentity_t *actor) {
	return (actor && actor->NPC && actor->s.weapon && !!(actor->NPC->scriptFlags & SCF_NO_GROUPS) // &&
