...
// Filename:-	weapons.h
//
// Note that this is now included from both server and game modules, so don't
...
{ WP_NONE,
  ...
  WP_BOWCASTER, // NPC weapon - player can pick this up, but never starts with
                // them
  WP_REPEATER,  // NPC weapon - player can pick this up, but never starts with
                // them
  WP_DEMP2, // NPC weapon - player can pick this up, but never starts with them
  WP_FLECHETTE, // NPC weapon - player can pick this up, but never starts with
                // them
  WP_ROCKET_LAUNCHER, // NPC weapon - player can pick this up, but never starts
                      // with them
  WP_THERMAL,         // player and NPC weapon
  WP_TRIP_MINE,  // NPC weapon - player can pick this up, but never starts with
                 // them
  WP_DET_PACK,   // NPC weapon - player can pick this up, but never starts with
                 // them
  WP_CONCUSSION, // NPC weapon - player can pick this up, but never starts with
                 // them
  ...
  // NPC weapons
  WP_BRYAR_PISTOL, // NPC weapon - player can pick this up, but never starts
                   // with them
  ...
  //# #eol
  WP_NUM_WEAPONS } weapon_t;

#define FIRST_WEAPON                                                           \
  WP_SABER // this is the first weapon for next and prev weapon switching
#define MAX_PLAYER_WEAPONS                                                     \
  WP_STUN_BATON // this is the max you can switch to and get with the give all.
                // - FIXME: it's actually this one *minus* one... why?

// AMMO_NONE must be first and AMMO_MAX must be last, cause weapon load
// validates based off of these vals
typedef enum //# ammo_e
{ AMMO_NONE,
...
typedef struct weaponData_s {
  ...
  char chargeSnd[64]; // sound to start when the weapon initiates the charging
                      // sequence
  char altChargeSnd[64]; // alt sound to start when the weapon initiates the
                         // charging sequence
...
#define DISRUPTOR_CHARGE_UNIT                                                  \
  150.0f // distruptor charging gives us one more unit every 150ms--if you
         // change this, you'll have to do the same in bg_pmove
...
#define DEMP2_CHARGE_UNIT                                                      \
  500.0f // demp2 charging gives us one more unit every 500ms--if you change
         // this, you'll have to do the same in bg_pmove
...
#define TD_NPC_DAMAGE_CUT                                                      \
  0.6f // NPC thrown dets deliver only 60% of the damage that a player thrown
       // one does
...
#define TD_TEST_RAD                                                            \
  (TD_SPLASH_RAD * 0.8f) // no sense in auto-blowing up if exactly on the radius
                         // edge--it would hardly do any damage
