...
qboolean Grenadier_EvaluateShot(int hit) {
	...
	if (hit == NPC->enemy->s.number ||
		(&g_entities[hit] != NULL && (g_entities[hit].svFlags & SVF_GLASS_BRUSH))) { // can hit enemy or will hit glass, so shoot anyway
		return qtrue;
	}
	return qfalse;
}
...
void NPC_BSGrenadier_Attack(void) {
	...
	{												// enemy is close and not using saber
		if (NPC->client->ps.weapon == WP_THERMAL) { // grenadier
			...
			gi.trace(&trace, NPC->currentOrigin, NPC->enemy->mins, NPC->enemy->maxs, NPC->enemy->currentOrigin, NPC->s.number, NPC->enemy->clipmask,
					 (EG2_Collision)0, 0);
			if (!trace.allsolid && !trace.startsolid && (trace.fraction == 1.0 || trace.entityNum == NPC->enemy->s.number)) { // I can get right to him
	...
	{																										   // enemy is far or using saber
		if (NPC->client->ps.weapon == WP_MELEE && (NPC->client->ps.stats[STAT_WEAPONS] & (1 << WP_THERMAL))) { // fisticuffs, make switch to thermal if have it
			// reset fire-timing variables
	...
	if (NPC_ClearLOS(NPC->enemy)) {
		...
		} else if (InFOV(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 45, 90)) { // in front of me
	...
	if (enemyCS) {
		...
		if (NPC->client->ps.weapon == WP_THERMAL) { // don't chase and throw
			doMove = qfalse;
				   ...
				   enemyDist < (NPC->maxs[0] + NPC->enemy->maxs[0] + 16) * (NPC->maxs[0] + NPC->enemy->maxs[0] + 16)) { // close enough
			doMove = qfalse;
		}
