...
int CBlockMember::ReadMember(char **stream, long *streamPos, CIcarus *icarus) {
  ...
  *streamPos += sizeof(int);
  ...
  if (m_id == CIcarus::ID_RANDOM) { // special case, need to initialize this
                                    // member's data to Q3_INFINITE so we can
                                    // randomize the number only the first time
    ...
    *streamPos += sizeof(int);
    ...
    if (m_size == 4 && m_id != CIcarus::TK_STRING &&
        m_id != CIcarus::TK_IDENTIFIER && m_id != CIcarus::TK_CHAR)
...
const int IBI_HEADER_ID_LENGTH =
    4; // Length of s_IBI_HEADER_ID + 1 (for null terminating byte)
