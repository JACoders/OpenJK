...
template <class T, int TSize = HASH_SIZE,
          int (*TCompare)(const char *, const char *) = &strcmp>
...
public:
  ...
  ~CHash(void) {
#ifdef _DEBUG
//		Com_OPrintf("Shutting down %s hash table .....",
//typeid(T).name());
#endif
    clear();
#ifdef _DEBUG
//		Com_OPrintf(" done\n");
  ...
  bool remove(T *item) { return (remove(item->GetName())); }
