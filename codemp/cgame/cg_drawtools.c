...
void CG_DrawSides(float x, float y, float w, float h, float size) {
  ...
  trap->R_DrawStretchPic(x + w - size, y, size, h, 0, 0, 0, 0,
...
void CG_DrawTopBottom(float x, float y, float w, float h, float size) {
  ...
  trap->R_DrawStretchPic(x, y + h - size, w, size, 0, 0, 0, 0,
                         cgs.media.whiteShader);
}
...
-------------------------
*/
void CG_FillRect2(float x, float y, float width, float height,
                  const float *color) {
  trap->R_SetColor(color);
  trap->R_DrawStretchPic(x, y, width, height, 0, 0, 0, 0,
                         cgs.media.whiteShader);
  trap->R_SetColor(NULL);
}
...
void CG_DrawNumField(int x, int y, int width, int value, int charWidth,
    ...
    for (i = 0; i < (width - l); i++) {
      ...
      case NUM_FONT_SMALL:
        CG_DrawPic(x, y, charWidth, charHeight,
                   cgs.media.smallnumberShaders[0]);
        break;
      case NUM_FONT_CHUNKY:
        CG_DrawPic(x, y, charWidth, charHeight,
                   cgs.media.chunkyNumberShaders[0]);
        break;
      default:
      case NUM_FONT_BIG:
  ...
  while (*ptr && l) {
    ...
    case NUM_FONT_SMALL:
      CG_DrawPic(x, y, charWidth, charHeight,
                 cgs.media.smallnumberShaders[frame]);
      x++; // For a one line gap
      break;
    case NUM_FONT_CHUNKY:
      CG_DrawPic(x, y, charWidth, charHeight,
                 cgs.media.chunkyNumberShaders[frame]);
      break;
    default:
    case NUM_FONT_BIG:
...
void CG_DrawProportionalString(int x, int y, const char *str, int style,
                               vec4_t color) {
  // having all these different style defines (1 for UI, one for CG, and now one
  // for the re->font stuff)
  //	is dumb, but for now...
  //
  int iStyle = 0;
