...
int cgAnnouncerTime =
    0; // to prevent announce sounds from playing on top of each other
...
void CG_CheckLocalSounds(playerState_t *ps, playerState_t *ops) {
  ...
  if (ps->persistant[PERS_HITS] > ops->persistant[PERS_HITS]) {
    ...
    if (armor > health / 2) { // We also hit shields along the way, so consider
                              // them "pierced".
                              //			trap->S_StartLocalSound(
                              //cgs.media.shieldPierceSound, CHAN_LOCAL_SOUND );
    } else {                  // Shields didn't really stand in our way.
                              //			trap->S_StartLocalSound( cgs.media.hitSound,
                              //CHAN_LOCAL_SOUND );
    }
    ...
    /*
    if (armor > 50 ) {
            trap->S_StartLocalSound( cgs.media.hitSoundHighArmor,
    CHAN_LOCAL_SOUND ); } else if (armor || health > 100) {
            trap->S_StartLocalSound( cgs.media.hitSoundLowArmor,
    CHAN_LOCAL_SOUND ); } else { trap->S_StartLocalSound( cgs.media.hitSound,
    CHAN_LOCAL_SOUND );
    }
    */
  ...
  if (cg_oldPainSounds.integer) {
    if (ps->stats[STAT_HEALTH] < (ops->stats[STAT_HEALTH] - 3)) {
      if (ps->stats[STAT_HEALTH] > 0) {
        CG_PainEvent(&cg_entities[cg.predictedPlayerState.clientNum],
  ...
  if (ps->persistant[PERS_ASSIST_COUNT] != ops->persistant[PERS_ASSIST_COUNT]) {
    ...
    // ps->persistant[PERS_ASSIST_COUNT]); reward = qtrue;
    // Com_Printf("assist\n");
  }
      ...
      if (ps->persistant[PERS_RANK] != ops->persistant[PERS_RANK]) {
        if (cgs.gametype < GT_TEAM) {
          ...
          if (  ps->persistant[PERS_RANK] == 0 ) {
                  ...
                  //rww - only bother saying this if you have more than 1 kill
          already.
                  //joining the server and hearing "the force is not with you"
          is silly. if (ps->persistant[PERS_SCORE] > 0)
                  {
  ...
  // fraglimit warnings
  if (cgs.fraglimit > 0 && cgs.gametype < GT_CTF && cgs.gametype != GT_DUEL &&
      cgs.gametype != GT_POWERDUEL && cgs.gametype != GT_SIEGE &&
