...
class CWayNode {
  public:
	...
	enum EWayNodeFlags { WN_NONE = 0, WN_ISLAND, WN_FLOATING, WN_DROPTOFLOOR, WN_NOAUTOCONNECT, WN_MAX };
	...
	////////////////////////////////////////////////////////////////////////////////////
	virtual ESide LRTest(const CWayNode &A, const CWayNode &B) const { return (mPoint.LRTest(A.mPoint, B.mPoint)); }

	////////////////////////////////////////////////////////////////////////////////////
	// Point In Circle (For Triangulation)
	////////////////////////////////////////////////////////////////////////////////////
	virtual bool InCircle(const CWayNode &A, const CWayNode &B, const CWayNode &C) const { return (mPoint.PtInCircle(A.mPoint, B.mPoint, C.mPoint)); }
};
...
class CWayEdge {
  public:
	...
	// Size Function
	////////////////////////////////////////////////////////////////////////////////////
	inline int Size() const { return (mFlags.get_bit(WE_SIZE_MEDIUM) ? (WE_SIZE_MEDIUM) : (WE_SIZE_LARGE)); }
...
struct SNodeSort {
	...
	bool operator<(const SNodeSort &other) const { return (mDistance < other.mDistance); }
};
...
class CGraphUser : public TGraph::user {
  ...
  public:
	...
	//
	////////////////////////////////////////////////////////////////////////////////////
	virtual bool can_be_invalid(const CWayEdge &Edge) const { return (Edge.mFlags.get_bit(CWayEdge::WE_CANBEINVAL)); }
	...
	virtual bool is_valid(CWayEdge &Edge, int EndPoint = 0) const {
		...
		if (Edge.mEntityNum != ENTITYNUM_NONE) {
			...
			if (ent) {
				...
				//------------------------------------------------------
				if ((mActor) && (mActor->NPC) && (mActor->NPC->aiFlags & NPCAI_NAV_THROUGH_BREAKABLES) && (Edge.BlockingBreakable()) &&
		...
		} else if (Edge.BlockingBreakable()) {					   // we had a breakable in our way, now it's gone, see if there is anything else in the way
	...
	////////////////////////////////////////////////////////////////////////////////////
	virtual float cost(const CWayNode &A, const CWayNode &B) const { return (A.mPoint.Dist(B.mPoint)); }
	...
	//
	////////////////////////////////////////////////////////////////////////////////////
	virtual bool on_same_floor(const CWayNode &A, const CWayNode &B) const { return (fabsf(A.mPoint[2] - B.mPoint[2]) < 100.0f); }
...
//
////////////////////////////////////////////////////////////////////////////////////////
TAlertList &GetAlerts(gentity_t *actor) { return mEntityAlertList[actor->s.number]; }
...
////////////////////////////////////////////////////////////////////////////////////////
bool MoveTrace(const CVec3 &Start, const CVec3 &Stop, const CVec3 &Mins, const CVec3 &Maxs, int IgnoreEnt = 0, bool CheckForDoNotEnter = false,
			   bool RetryIfStartInDoNotEnter = true, bool IgnoreAllEnts = false, int OverrideContents = 0) {
...
bool NAV::TestEdge(TNodeHandle NodeA, TNodeHandle NodeB, qboolean IsDebugEdge) {
	...
	//-----------------------------------------------------------------------
	if ((a.mType == PT_WAYNODE && a.mRadius < radius) || (b.mType == PT_WAYNODE && b.mRadius < radius)) {
		if (IsDebugEdge) {
	...
	//-----------------------------------------------------------------------------------------
	if (!CanGo && !mMoveTrace.startsolid && EntHit != ENTITYNUM_WORLD && EntHit != ENTITYNUM_NONE && (&g_entities[EntHit]) != 0) {
		...
		} else if (!Q_stricmp("func_wall", ent->classname) || !Q_stricmp("func_static", ent->classname) || !Q_stricmp("func_usable", ent->classname)) {
			at.mFlags.set_bit(CWayEdge::WE_BLOCKING_WALL);
		} else if (!Q_stricmp("func_glass", ent->classname) || !Q_stricmp("func_breakable", ent->classname) ||
		...
		//-----------------------------------------------------
		else if (at.BlockingBreakable()) { // we'll do the trace again later if this ent gets broken
			CanGo = true;
		}
		...
		if (CanGo && !HitCharacter) {
			...
			if (at.BlockingDoor()) {
				...
				if (ent->targetname) {
					...
					if (owner && (!Q_stricmp("trigger_multiple", owner->classname) || !Q_stricmp("trigger_once", owner->classname))) {
...
bool NAV::LoadFromEntitiesAndSaveToFile(const char *filename, int checksum) {
	...
	*/
	...
	for (nodeIter = mGraph.nodes_begin(); nodeIter != mGraph.nodes_end(); nodeIter++) {
		...
		//---------------------------------------------------------------------------------
		if (!at->mFlags.get_bit(CWayNode::WN_NOAUTOCONNECT) && (at->mType == NAV::PT_COMBATNODE || at->mType == NAV::PT_GOALNODE)) {
	...
	for (edgeIter = mGraph.edges_begin(); edgeIter != mGraph.edges_end(); edgeIter++) {
		...
		//--------------------------------------------------------------
		bool CanGo = false;
		bool IsDebugEdge = (g_nav1->string[0] && g_nav2->string[0] && (!Q_stricmp(*(a.mName), g_nav1->string) || !Q_stricmp(*(b.mName), g_nav1->string)) &&
...
NAV::TNodeHandle NAV::GetNearestNode(gentity_t *ent, bool forceRecalcNow, NAV::TNodeHandle goal) {
	...
	if (ent->waypoint == WAYPOINT_NONE || forceRecalcNow || (level.time > ent->noWaypointTime)) {
		...
		ent->waypoint = GetNearestNode(ent->currentOrigin, ent->waypoint, goal, ent->s.number, (ent->client && ent->client->moveType == MT_FLYSWIM));
...
bool NAV::FindPath(gentity_t *actor, NAV::TNodeHandle target, float MaxDangerLevel) {
	...
	if (actor->enemy && actor->enemy->client) {
		...
		} else if (actor->enemy->client->NPC_class == CLASS_RANCOR || actor->enemy->client->NPC_class == CLASS_WAMPA) {
	...
	if (!(actor->NPC->scriptFlags & SCF_RUNNING) &&
		((actor->NPC->scriptFlags & SCF_WALKING) || (actor->NPC->aiFlags & NPCAI_WALKING) || (ucmd.buttons & BUTTON_WALKING))) {
	...
	for (int edge = 0; edge < numEdges; edge++) {
		...
		if (AtOnEdgeScale > 0.1f && AtOnEdgeScale < 0.9f) {
			if (AtDistToEdge < (radius) ||
				(AtDistToEdge < (radius * 20.0f) && MoveTrace(At, AtOnEdge, actor->mins, actor->maxs, actor->s.number, true, true, false))) {
...
//
////////////////////////////////////////////////////////////////////////////////////////
bool NAV::OnSamePoint(gentity_t *actor, gentity_t *target) { return (GetNearestNode(actor) == GetNearestNode(target)); }
...
bool NAV::OnNeighboringPoints(TNodeHandle A, TNodeHandle B) {
	...
	if (edgeNum && !mGraph.get_edge(edgeNum).mFlags.get_bit(CWayEdge::WE_JUMPING) && !mGraph.get_edge(edgeNum).mFlags.get_bit(CWayEdge::WE_FLYING) &&
...
bool NAV::InSafeRadius(CVec3 at, TNodeHandle atNode, TNodeHandle targetNode) {
	...
	if (targetNode > 0 && atNode != targetNode) {
		...
		if (atToTargetEdgeIndex != 0) {
			...
			// Is The Edge Valid?
			//--------------------
			if (!atToTargetEdge.mFlags.get_bit(CWayEdge::WE_FLYING) && !atToTargetEdge.mFlags.get_bit(CWayEdge::WE_JUMPING) &&
...
//
////////////////////////////////////////////////////////////////////////////////////////
bool NAV::FindPath(gentity_t *actor, const vec3_t &position, float MaxDangerLevel) { return FindPath(actor, GetNearestNode(position), MaxDangerLevel); }
...
float NAV::PathDangerLevel(gentity_t *actor) {
	...
	//-----------------------------------------------------
	if (actor->enemy && actor->enemy->client) {
		if (actor->enemy->client->ps.weapon == WP_SABER || actor->enemy->client->NPC_class == CLASS_RANCOR || actor->enemy->client->NPC_class == CLASS_WAMPA) {
...
void NAV::ShowStats() {
	...
	float totalBytes = (sizeof(mCells) + sizeof(mGraph) + sizeof(mRegion) + sizeof(mPathUsers) + sizeof(mPathUserIndex) + sizeof(mSteerUsers) +
						sizeof(mSteerUserIndex) + sizeof(mEntityAlertList));

	mGraph.ProfilePrint("TOTAL :  (KiloBytes): (%5.3f)  MeggaBytes(%3.3f)", ((float)(totalBytes) / 1024.0f), ((float)(totalBytes) / 1048576.0f));
	mGraph.ProfilePrint("");
...
void STEER::DeActivate(gentity_t *actor, usercmd_t *ucmd) {
	...
	if (!NPC_Jumping()) {
		...
		if (STEER::HasBeenBlockedFor(actor, 2000)) {
			...
			//---------------------------------
			if ((actor->NPC->scriptFlags & SCF_NAV_CAN_FLY) || (actor->NPC->scriptFlags & SCF_NAV_CAN_JUMP)) {
				// Ok, Well, How About Jumping To The Last Waypoint We Had That Was Valid
				//------------------------------------------------------------------------
				if ((STEER::HasBeenBlockedFor(actor, 8000)) &&
					(!actor->waypoint || Distance(NAV::GetNodePosition(actor->waypoint), actor->currentOrigin) > 150.0f) && (actor->lastWaypoint)) {
					if (player && (STEER::HasBeenBlockedFor(actor, 15000)) && !gi.inPVS(player->currentOrigin, NAV::GetNodePosition(actor->lastWaypoint)) &&
						!gi.inPVS(player->currentOrigin, actor->currentOrigin) && !G_CheckInSolidTeleport(NAV::GetNodePosition(actor->lastWaypoint), actor)) {
...
//
//////////////////////////////////////////////////////////////////////
bool STEER::Active(gentity_t *actor) { return (mSteerUserIndex[actor->s.number] != NULL_STEER_USER_INDEX); }
...
bool STEER::SafeToGoTo(gentity_t *actor, const vec3_t &targetPosition, int targetNode) {
	...
	if (actorToTargetDistance < 500.0f) {
		...
		//-------------------------------------------
		if (NAV::OnNeighboringPoints(actorNode, targetNode) && NAV::InSafeRadius(actor->currentOrigin, actorNode, targetNode) &&
...
bool STEER::HasBeenBlockedFor(gentity_t *actor, int duration) {
	return ((actor->NPC->aiFlags & NPCAI_BLOCKED) && (level.time - actor->NPC->blockedDebounceTime) > duration);
}
...
//
////////////////////////////////////////////////////////////////////////////////////
float STEER::Persue(gentity_t *actor, gentity_t *target, float slowingDistance, float offsetForward, float offsetRight, float offsetUp,
...
float STEER::FollowLeader(gentity_t *actor, gentity_t *leader, float dist) {
	...
	if (TimeRemaining < 0 || (LeaderSpeed > 0.0f && TimeRemaining > 1000)) {
		...
		leader->followPosRecalcTime =
			(level.time) + (Q_irand(50, 500)) + (SpeedScale * Q_irand(3000, 8000)) + ((!actor->enemy && !leader->enemy) ? (Q_irand(8000, 15000)) : (0));
	}
...
bool TestCollision(gentity_t *actor, SSteerUser &suser, const CVec3 &ProjectVelocity, float ProjectSpeed, ESide Side, float weight = 1.0f) {
	...
	if (mMoveTrace.entityNum != ENTITYNUM_NONE && mMoveTrace.entityNum != ENTITYNUM_WORLD) {
		...
		//-----------------------
		if (g_entities[mMoveTrace.entityNum].classname && Q_stricmp(g_entities[mMoveTrace.entityNum].classname, "func_door") == 0) {
			Safe = true;
		}

		// If It's Breakable And We Can Go Through It, Then That's Safe Too
		//------------------------------------------------------------------
		if ((actor->NPC->aiFlags & NPCAI_NAV_THROUGH_BREAKABLES) && G_EntIsBreakable(mMoveTrace.entityNum, actor)) {
			Safe = true;
		}
