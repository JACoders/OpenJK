...
#define fLINE_SECONDTOSCROLLUP                                                 \
  15.0f // how long one line takes to scroll up the screen
...
struct StringAndSize_t {
  ...
  int GetPixelLength(void) {
    if (iStrLenPixels == -1) {
      iStrLenPixels =
          cgi_R_Font_StrLenPixels(str.c_str(), ghFontHandle, gfFontScale);
    }
...
static const char *Capitalize(const char *psTest) {
  ...
  //	if (!cgi_Language_IsAsian())	// we don't have asian credits, so this
  //is ok to do now
  {
...
//
static bool CountsAsWhiteSpaceForCaps(
    unsigned /* avoid euro-char sign-extend assert within isspace()*/ char c) {
...
static const char *UpperCaseFirstLettersOnly(const char *psTest) {
  ...
  p = strstr(sTemp, " O'"); // eg "O'flaherty" should be "O'Flaherty" (this is
                            // probably done automatically now, but wtf.
  if (p && isalpha(p[3])) {
    p[3] = toupper(p[3]);
  }
...
// sort entries by their last name (starts at back of string and moves forward
// until start or just before whitespace)
// ...
...
void CG_Credits_Init(const char *psStripReference, vec4_t *pv4Color) {
  ...
  memcpy(gv4Color, pv4Color,
         sizeof(gv4Color)); // memcpy so we can poke into alpha channel
  ...
  //
  iStrLen =
      cgi_SP_GetStringTextString(psStripReference, psMallocText, iStrLen + 1);
  ...
  while (*psTextParse != '\0') {
    ...
    while (1) {
      ...
      int iAdvanceCount;
      unsigned int uiLetter = cgi_AnyLanguage_ReadCharFromString(
          psTextParse, &iAdvanceCount, &bIsTrailingPunctuation);
      ...
      if (uiLetter == '\n' || uiLetter == '\0') {
...
#ifndef FINAL_BUILD
              Com_Printf(S_COLOR_YELLOW "CG_Credits_Init(): No current support "
                                        "for cards after scroll!\n");
#endif
              eMode = eNothing;
            }
            break;
        ...
        //
        if (uiLetter > 255) {
          assert(0); // this means we're attempting to display asian credits,
                     // and we don't
                     //	support these now because the auto-capitalisation rules
                     //etc would have to 	be inhibited.
          Q_strcat(sLine, sizeof(sLine),
    ...
    if (bWasCommand) {
      ...
      case eDotEntry: {
        ...
        if (!CreditLine.strText.IsEmpty() && CreditLine.vstrText.size()) {
          ...
          //
          qsort(&CreditLine.vstrText[0], CreditLine.vstrText.size(),
                sizeof(CreditLine.vstrText[0]), SortBySurname);
      ...
      case eCard: {
        ...
        if (!CreditCard.strTitle.IsEmpty()) {
          ...
          //
          qsort(&CreditCard.vstrText[0], CreditCard.vstrText.size(),
                sizeof(CreditCard.vstrText[0]), SortBySurname);
...
qboolean CG_Credits_Draw(void) {
  if (CG_Credits_Running()) {
    const int iFontHeight =
        (int)(1.5f *
              (float)cgi_R_Font_HeightPixels(
                  ghFontHandle, gfFontScale)); // taiwanese & japanese need 1.5
                                               // fontheight spacing
    ...
    if (CreditData.CreditCards.size()) {
      ...
      if (fSecondsElapsed < fCARD_FADESECONDS) {
        ...
        //				OutputDebugString(va("fade up:
        //%f\n",gv4Color[3]));
      } else if (fSecondsElapsed > fCARD_FADESECONDS + fCARD_SUSTAINSECONDS) {
        // fading down...
        //
        const float fFadeDownSeconds =
            fSecondsElapsed - (fCARD_FADESECONDS + fCARD_SUSTAINSECONDS);
        gv4Color[3] = 1.0f - (fFadeDownSeconds / fCARD_FADESECONDS);
        //				OutputDebugString(va("fade dw:
        //%f\n",gv4Color[3]));
      } else {
        gv4Color[3] = 1.0f;
        //				OutputDebugString(va("normal:
        //%f\n",gv4Color[3]));
      }
      if (gv4Color[3] < 0.0f)
        gv4Color[3] = 0.0f; // ... otherwise numbers that have dipped slightly
                            // -ve flash up fullbright after fade down

      //
      // how many lines is it?
      //
      int iLines = CreditCard.vstrText.size() +
                   2; // +2 for title itself & one seperator line
      //
      ...
      cgi_R_Font_DrawString(iXpos, iYpos, CreditCard.strTitle.c_str(), gv4Color,
                            ghFontHandle, -1, gfFontScale);
      //
      ...
      for (size_t i = 0; i < CreditCard.vstrText.size(); i++) {
        ...
        cgi_R_Font_DrawString(iXpos, iYpos, StringAndSize.c_str(), gv4Color,
                              ghFontHandle, -1, gfFontScale);
        iYpos += iFontHeight;
      }
      ...
      //
      if (fSecondsElapsed >
          fCARD_FADESECONDS + fCARD_SUSTAINSECONDS + fCARD_FADESECONDS) {
        // yep, so erase the first entry (which will trigger the next one to be
        // initialised on re-entry)...
        //
      ...
      if (CreditData.CreditLines.size()) {
        ...
        for (CreditLines_t::iterator it = CreditData.CreditLines.begin();
             it != CreditData.CreditLines.end(); bEraseOccured ? it : ++it) {
          CreditLine_t &CreditLine = (*it);
          bEraseOccured = false;

          static const float fPixelsPerSecond =
              ((float)SCREEN_HEIGHT / fLINE_SECONDTOSCROLLUP);
          ...
          if (iYpos + (iTextLinesThisItem * iFontHeight) < 0) {
            ...
            cgi_R_Font_DrawString(iXpos, iYpos, CreditLine.strText.c_str(),
                                  gv4Color, ghFontHandle, -1, gfFontScale);
            ...
            for (size_t i = 0; i < CreditLine.vstrText.size(); i++) {
              ...
              cgi_R_Font_DrawString(iXpos, iYpos, StringAndSize.c_str(),
                                    gv4Color, ghFontHandle, -1, gfFontScale);
              iYpos += iFontHeight;
            }
          }
