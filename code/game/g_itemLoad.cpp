...
static void IT_SetDefaults() {
  ...
  bg_itemlist[itemParms.itemNum].pickup_sound =
      PICKUPSOUND; // give it a default sound
...
static void IT_Tag(const char **holdBuf) {
  ...
  if (!Q_stricmp(tokenStr, "WP_NONE"))
    ...
    tag = WP_BRYAR_PISTOL;
    // This error was slipping through too much, causing runaway exceptions and
    // shutting down, so now it's a real error when not in Final
#ifndef FINAL_BUILD
