...
void RB_CheckVBOandIBO(VBO_t *vbo, IBO_t *ibo) {
	...
	if (vbo != backEndData->currentFrame->dynamicVbo && ibo != backEndData->currentFrame->dynamicIbo) {
...
void RB_AddQuadStampExt(vec3_t origin, vec3_t left, vec3_t up, float color[4], float s1, float t1, float s2, float t2) {
	...
	tess.normal[ndx] = tess.normal[ndx + 1] = tess.normal[ndx + 2] = tess.normal[ndx + 3] = R_VboPackNormal(normal);
...
void RB_InstantTriangle() { qglDrawArrays(GL_TRIANGLES, 0, 3); }
...
static qboolean RB_SurfaceVbo(VBO_t *vbo, IBO_t *ibo, int numVerts, int numIndexes, int firstIndex, int minIndex, int maxIndex, int dlightBits, int pshadowBits,
...
static void RB_SurfaceBSPTriangles(srfBspSurface_t *srf) {
	if (RB_SurfaceVbo(srf->vbo, srf->ibo, srf->numVerts, srf->numIndexes, srf->firstIndex, srf->minIndex, srf->maxIndex, srf->dlightBits, srf->pshadowBits,
					  qtrue)) {
		return;
	}
...
//----------------------------------------------------------------------------
{
	VectorSet(sh1, 0.66f + Q_flrand(-1.0f, 1.0f) * 0.1f, // fwd
			  0.07f + Q_flrand(-1.0f, 1.0f) * 0.025f, 0.07f + Q_flrand(-1.0f, 1.0f) * 0.025f);
...
static void RB_SurfaceVBOMesh(srfBspSurface_t *srf) {
	RB_SurfaceVbo(srf->vbo, srf->ibo, srf->numVerts, srf->numIndexes, srf->firstIndex, srf->minIndex, srf->maxIndex, srf->dlightBits, srf->pshadowBits, qfalse);
}
...
static void RB_SurfaceSprites(srfSprites_t *surf) {
	...
	uniformDataWriter.SetUniformInt(UNIFORM_ALPHA_TEST_TYPE, surf->alphaTestType);
	...
	const UniformBlockBinding uniformBlockBindings[] = {{currentSpriteUbo, ss->spriteUboOffset, UNIFORM_BLOCK_SURFACESPRITE},
