...
static qboolean R_CullSurface(surfaceType_t *surface, shader_t *shader) {
	...
	if (r_cullRoofFaces->integer) {									 // Very slow, but this is only intended for taking shots for automap images.
		if (sface->plane.normal[2] > 0.0f && sface->numPoints > 0) { // it's facing up I guess
			static int i;
			static trace_t tr;
			...
			if (!tr.startsolid && !tr.allsolid &&
				(tr.fraction == 1.0f ||
				 (tr.surfaceFlags & SURF_NOIMPACT))) { // either hit nothing or sky, so this surface is near the top of the level I guess. Or the floor of a
													   // really tall room, but if that's the case we're just screwed.
				VectorSubtract(basePoint, tr.endpos, v);
				if (tr.fraction == 1.0f || VectorLength(v) < r_roofCullCeilDist->value) { // ignore it if it's not close to the top, unless it just hit nothing
					...
					while (i < 4096) {
						...
						if (!tr.startsolid && !tr.allsolid && tr.fraction == 1.0f) { // in the clear
							break;
						}
						i++;
					}
					if (i < 4096) { // Make sure we got into clearance
						...
						if (!tr.startsolid && !tr.allsolid &&
							(tr.fraction != 1.0f &&
							 !(tr.surfaceFlags & SURF_NOIMPACT))) { // if we hit nothing or a noimpact going down then this is probably "ground".
							VectorSubtract(basePoint, tr.endpos, endPoint);
							if (VectorLength(endPoint) > r_roofCullCeilDist->value) { // 128 (by default) is our maximum tolerance, above that will be removed
								return qtrue;
							}
						}
					}
	...
	if (tr_drawingAutoMap) {
		...
		if (*surf->data == SF_FACE) { // only do this if we need to
			...
			while (i < face->numIndices) {
				...
				if (alpha != 1.0f) { // this point is not entirely alpha'd out, so still draw the surface
...
float GetQuadArea(vec3_t v1, vec3_t v2, vec3_t v3, vec3_t v4) {
	...
	return (dis1[0] * dis1[0] + dis1[1] * dis1[1] + dis1[2] * dis1[2] + dis2[0] * dis2[0] + dis2[1] * dis2[1] + dis2[2] * dis2[2]);
}
...
void RE_GetBModelVerts(int bmodelIndex, vec3_t *verts, vec3_t normal) {
	...
	} else {			// Possibly only have one face, so may as well use the first face, which also should be the best one
...
static inline wireframeMapSurf_t *R_GetNewWireframeMapSurf(void) {
	...
	if (g_autoMapNextFree) { // save us the time of going through the entire linked list from root
...
static inline void R_RecursiveWireframeSurf(mnode_t *node) {
	...
	while (1) {
		if (!node || node->visframe != tr.visCount) { // not valid, stop this chain of recursion
			return;
		}
...
void R_AutomapElevationAdjustment(float newHeight) { g_playerHeight = newHeight; }
...
const void *R_DrawWireframeAutomap(const void *data) {
	...
	while (s) {
		...
		// need to draw it
		if (g_playerHeight != g_lastHeight || !g_lastHeightValid) { // only do this if we need to
			i = 0;
			...
			while (i < s->numPoints) {
				...
				if (s->points[i].alpha != 1.0f) { // this point is not entirely alpha'd out, so still draw the surface
