...
qboolean RE_RegisterModels_LevelLoadEnd(
  ...
  if (gbInsideRegisterModel) {
    ...
    for (CachedModels_t::iterator itModel = CachedModels->begin();
         itModel != CachedModels->end() && (bDeleteEverythingNotUsedThisLevel ||
...
void RE_RegisterModels_Info_f(void) {
  ...
  for (CachedModels_t::iterator itModel = CachedModels->begin();
    ...
    Com_Printf("%d/%d: \"%s\" (%d bytes)", iModel, iModels,
...
static qhandle_t RE_RegisterModel_Actual(const char *name) {
  ...
  */
  //	if (!tr.registered) {
  //		Com_Printf (S_COLOR_YELLOW  "RE_RegisterModel (%s) called before
  //ready!\n",name ); 		return 0;
  //	}
  //
...
#if 0 //#ifndef _M_IX86
      //
      // optimisation, we don't bother doing this for standard intel case since
      // our data's already in that format...
      //
