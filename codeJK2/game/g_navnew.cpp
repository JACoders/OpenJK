...
void NAVNEW_PushBlocker(
  ...
  if (!blocker->s.number) { // never push the player
    return;
  }
  ...
  gi.trace(&tr, blocker->currentOrigin, mins, blocker->maxs, end,
           blocker->s.number, blocker->clipmask | CONTENTS_BOTCLIP,
           G2_NOCOLLIDE, 0);
...
qboolean NAVNEW_SidestepBlocker(
  ...
  float avoidRadius =
      sqrt((blocker->maxs[0] * blocker->maxs[0]) +
           (blocker->maxs[1] * blocker->maxs[1])) +
  ...
  */
  ...
  if (self->NPC->sideStepHoldTime > level.time) {
    ...
    gi.trace(&tr, self->currentOrigin, mins, self->maxs, block_pos,
             self->s.number, self->clipmask | CONTENTS_BOTCLIP, G2_NOCOLLIDE,
             0);
  ...
  gi.trace(&tr, self->currentOrigin, mins, self->maxs, block_pos,
           self->s.number, self->clipmask | CONTENTS_BOTCLIP, G2_NOCOLLIDE, 0);
...
qboolean NAVNEW_TestNodeConnectionBlocked(
  ...
  gi.trace(&trace, pos1, mins, maxs, pos2, ignoreEntNum, clipmask, G2_NOCOLLIDE,
           0);
...
int NAVNEW_MoveToGoal(gentity_t *self, navInfo_t &info) {
  ...
  if (self->noWaypointTime > level.time &&
    ...
    bestNode = navigator.GetBestNodeAltRoute(
  ...
  // FIXME!!!!: this is making them wiggle back and forth between waypoints
                ...
                self, self->NPC->goalEntity, NF_CLEAR_PATH)) ==
  ...
  while (!foundClearPath) {
    ...
    /*
    bestNode = navigator.GetBestNodeAltRoute( self->waypoint,
    ...
    */
    ...
    {//we haven't already tried to go straight to goal or goal's wp
            ...
            {//our bestNode is the goal's wp
                    if ( NAV_HitNavGoal( self->currentOrigin, self->mins,
    self->maxs, origin, navigator.GetNodeRadius( bestNode ), FlyingCreature(
    self ) ) )
    ...
    macro_nav... VectorCopy( self->NPC->goalEntity->currentOrigin, origin );
                    }
            }
    }
    */
    if (!inGoalWP) {                    // not heading straight for goal
      if (bestNode == self->waypoint) { // we know it's clear or architecture
                                        // navigator.GetNodePosition(
                                        // self->waypoint, origin );
        /*
        if ( NAV_HitNavGoal( self->currentOrigin, self->mins, self->maxs,
        origin, navigator.GetNodeRadius( bestNode ), FlyingCreature( self ) ) )
    ...
    if (foundClearPath) { // clear!
      ...
      if (inGoalWP) { // we headed for our goal and failed and our goal's WP and
        ...
        if (d_altRoutes->integer) { // mark this edge failed and try our
                                    // waypoint
        ...
        if (d_altRoutes->integer) {
          ...
          // navigator.GetBestPathBetweenEnts( self, self->NPC->goalEntity,
