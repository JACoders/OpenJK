...
void MC_Compress(const float mat[3][4], unsigned char *_comp) {
  ...
  // byte (from 0), and therefore
  //	technically is writing beyond the 24th byte of the output array. This
  //*should* be harmless if the OR'd-in value 	doesn't change those bytes, but
  //BoundsChecker says that it's accessing undefined memory (which it does,
