...
void NPC_ChoosePainAnimation(gentity_t *self, gentity_t *other, const vec3_t point, int damage, int mod, int hitLoc, int voiceEvent = -1) {
	...
	if (self->s.weapon == WP_THERMAL && self->client->fireDelay > 0) { // don't interrupt thermal throwing anim
		...
		if (other && (other->s.weapon == WP_SABER || mod == MOD_ELECTROCUTE || mod == MOD_CRUSH /*FIXME:MOD_FORCE_GRIP*/)) {
			if (self->client->ps.weapon == WP_SABER && other->s.number < MAX_CLIENTS) { // hmm, shouldn't *always* react to damage from player if I have a saber
...
gentity_t *G_CheckControlledTurretEnemy(gentity_t *self, gentity_t *enemy, qboolean validate) {
	if (enemy->e_UseFunc == useF_emplaced_gun_use || enemy->e_UseFunc == useF_eweb_use) {
		if (enemy->activator && enemy->activator->client) { // return the controller of the eweb/emplaced gun
...
void NPC_Touch(gentity_t *self, gentity_t *other, trace_t *trace) {
	...
	if (NPC->client->NPC_class == CLASS_RANCOR) {										 // rancor
		if (NPCInfo->blockedEntity != other && TIMER_Done(NPC, "blockedEntityIgnore")) { // blocked
