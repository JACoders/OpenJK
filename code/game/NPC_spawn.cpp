...
void NPC_SetMiscDefaultData(gentity_t *ent) {
	...
	} else if (ent->client->NPC_class == CLASS_BOBAFETT) { // set some stuff, precache
	...
	} else if (ent->client->NPC_class == CLASS_ROCKETTROOPER) { // set some stuff, precache
		ent->client->ps.forcePowersKnown |= (1 << FP_LEVITATION);
		ent->client->ps.forcePowerLevel[FP_LEVITATION] = FORCE_LEVEL_3;
		ent->client->ps.forcePower = 100;
		ent->NPC->scriptFlags |= (SCF_NAV_CAN_FLY | SCF_FLY_WITH_JET | SCF_NAV_CAN_JUMP); // no groups, no combat points!
		if (Q_stricmp("rockettrooper2Officer", ent->NPC_type) == 0) {					  // start in the air, use spotlight
	...
	if (!Q_stricmp("emperor", ent->NPC_type) || !Q_stricmp("cultist_grip", ent->NPC_type) || !Q_stricmp("cultist_drain", ent->NPC_type) ||
		!Q_stricmp("cultist_lightning", ent->NPC_type)) { // FIXME: extern this into NPC.cfg?
	...
	if (ent->client->ps.saber[0].type != SABER_NONE &&
		(!(ent->NPC->aiFlags & NPCAI_MATCHPLAYERWEAPON) || !ent->weaponModel[0])) { // if I'm equipped with a saber, initialize it (them)
		ent->client->ps.SaberDeactivate();
		ent->client->ps.SetSaberLength(0);
		WP_SaberInitBladeData(ent);
		if (ent->client->ps.weapon == WP_SABER) { // this is our current weapon, add the models now
	...
	if (ent->client->NPC_class == CLASS_DESANN || ent->client->NPC_class == CLASS_TAVION || ent->client->NPC_class == CLASS_LUKE ||
		ent->client->NPC_class == CLASS_KYLE || Q_stricmp("tavion_scepter", ent->NPC_type) == 0 || Q_stricmp("alora_dual", ent->NPC_type) == 0) {
		ent->NPC->aiFlags |= NPCAI_BOSS_CHARACTER;
	...
	case TEAM_PLAYER:
		...
		if (ent->client->NPC_class == CLASS_SEEKER) {
			...
			return;
		} else if (ent->client->NPC_class == CLASS_JEDI || ent->client->NPC_class == CLASS_KYLE || ent->client->NPC_class == CLASS_LUKE) { // good jedi
	...
	case TEAM_ENEMY: {
		ent->NPC->defaultBehavior = BS_DEFAULT;
		if (ent->client->NPC_class == CLASS_SHADOWTROOPER && Q_stricmpn("shadowtrooper", ent->NPC_type, 13) == 0) { // FIXME: a spawnflag?
			Jedi_Cloak(ent);
		}
		if (ent->client->NPC_class == CLASS_TAVION || ent->client->NPC_class == CLASS_ALORA ||
			(ent->client->NPC_class == CLASS_REBORN && ent->client->ps.weapon == WP_SABER) || ent->client->NPC_class == CLASS_DESANN ||
			...
			case WP_BLASTER_PISTOL:
				...
				if (ent->client->NPC_class == CLASS_REBORN && ent->NPC->rank >= RANK_LT_COMM &&
					(!(ent->NPC->aiFlags & NPCAI_MATCHPLAYERWEAPON) || !ent->weaponModel[0])) // they do this themselves
	...
	//----------------------------------------------------------
	if (ent->client->NPC_class == CLASS_PROBE || ent->client->NPC_class == CLASS_REMOTE || ent->client->NPC_class == CLASS_SEEKER ||
		ent->client->NPC_class == CLASS_SENTRY || ent->client->NPC_class == CLASS_GLIDER || ent->client->NPC_class == CLASS_IMPWORKER ||
	...
	if (ent->client->ps.stats[STAT_WEAPONS] & (1 << WP_SCEPTER)) {
		if (!ent->weaponModel[1]) { // we have the scepter, so put it in our left hand if we don't already have a second weapon
...
int NPC_WeaponsForTeam(team_t team, int spawnflags, const char *NPC_type) {
	...
	switch (team) {
		...
		//	case TEAM_IMPERIAL:
	case TEAM_ENEMY:
		if (Q_stricmp("tavion", NPC_type) == 0 || Q_stricmpn("reborn", NPC_type, 6) == 0 || Q_stricmp("desann", NPC_type) == 0 ||
...
//--------------------------------------------------------------
void NPC_SetFX_SpawnStates(gentity_t *ent) { ent->client->ps.gravity = g_gravity->value; }
...
void NPC_Begin(gentity_t *ent) {
	...
	if (!(ent->spawnflags & SFB_NOTSOLID)) {			   // No NPCs should telefrag
		if (Q_stricmp(ent->NPC_type, "nullDriver") == 0) { // FIXME: should be a better way to check this
	...
	} else if (ent->NPC->stats.health) // Was health supplied in NPC.cfg?
	{
		...
		if (ent->client->NPC_class != CLASS_REBORN &&
			...
			if (!Q_stricmp("tavion_sith_sword", ent->NPC_type) || !Q_stricmp("tavion_scepter", ent->NPC_type) || !Q_stricmp("kyle_boss", ent->NPC_type) ||
				!Q_stricmp("alora_dual", ent->NPC_type) || !Q_stricmp("alora_boss", ent->NPC_type)) { // bosses are a bit different
				ent->NPC->stats.health = ceil((float)ent->NPC->stats.health * 0.75f +
											  ((float)ent->NPC->stats.health / 4.0f * g_spskill->value)); // 75% on easy, 100% on medium, 125% on hard
			} else {
	...
	*/
	...
	if (ent->client->playerTeam == TEAM_ENEMY || ent->client->playerTeam == TEAM_FREE) {	 // valid enemy spawned
...
gentity_t *NPC_Spawn_Do(gentity_t *ent, qboolean fullSpawnNow) {
	...
	*/
	...
	if (ent->NPC_type) {
		if (!Q_stricmp(ent->NPC_type, "player")) { // Or check NPC_type against player's NPC_type?
...
void SP_NPC_spawner(gentity_t *self) {
	...
	if (self->message) { // may drop a key, precache the key model and pickup sound
...
*/
void NPC_VehicleSpawnUse(gentity_t *self, gentity_t *other, gentity_t *activator) { G_VehicleSpawn(self); }
...
static void NPC_Spawn_f(void) {
	...
	if (isVehicle) { // must let NPC spawn func know this is a vehicle we're trying to spawn
	...
	if (!Q_stricmp("jedi_random", NPCspawner->NPC_type)) { // special case, for testing
