...
void CG_ToggleBinoculars(void) {
	...
	{
		if ((cg.snap->ps.saberActive && cg.snap->ps.saberInFlight) || cg.snap->ps.stats[STAT_HEALTH] <= 0) { // can't select binoculars when throwing saber
...
static const char *gcmds[] = {"entitylist",
							  "difficulty",
							  "force_distract",
							  "force_grip",
							  "force_heal",
