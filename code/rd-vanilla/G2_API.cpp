...
#define G2_MODEL_OK(g)                                                         \
  ((g) && (g)->mValid && (g)->aHeader && (g)->currentModel && (g)->animModel)
...
public:
  ErrorReporter(const std::string &name) : mName(name) {}
  ~ErrorReporter() {
    char mess[1000];
    int total = 0;
    ...
    for (i = 0; i < ghlInfo->mBlist.size(); i++) {
      if (ghlInfo->mBlist[i].boneNumber != -1) // slot used?
      {
        if (ghlInfo->mBlist[i].flags & BONE_ANIM_TOTAL) // anim on this?
        {
          ...
          if (loop == not_loop) {
            Error("Unusual animation flags, should have some sort of override, "
                  "but not both",
                  1, 0, line);
          }

          bool freeze =
              (ghlInfo->mBlist[i].flags & BONE_ANIM_OVERRIDE_FREEZE) ==
          ...
          int startFrame =
              ghlInfo->mBlist[i].startFrame; // start frame for animation
          int endFrame =
              ghlInfo->mBlist[i].endFrame; // end frame for animation NOTE anim
                                           // actually ends on endFrame+1
          int startTime =
              ghlInfo->mBlist[i].startTime; // time we started this animation
          int pauseTime =
              ghlInfo->mBlist[i]
                  .pauseTime; // time we paused this animation - 0 if not paused
          float animSpeed =
              ghlInfo->mBlist[i]
                  .animSpeed; // speed at which this anim runs. 1.0f means full
                              // speed of animation incoming - ie if anim is
                              // 20hrtz, we run at 20hrts. If 5hrts, we run at 5
                              // hrts
...
#define G2ERROR(exp, m)                                                        \
  (void)((exp) || (G2APIError().Error(m, 0, __FILE__, __LINE__), 0))
#define G2WARNING(exp, m)                                                      \
  (void)((exp) || (G2APIError().Error(m, 1, __FILE__, __LINE__), 0))
#define G2NOTE(exp, m)                                                         \
  (void)((exp) || (G2APIError().Error(m, 2, __FILE__, __LINE__), 0))
#define G2ANIM(ghlInfo, m)                                                     \
  (void)((G2APIError().AnimTest(ghlInfo, m, __FILE__, __LINE__), 0))
#else
...
int G2API_InitGhoul2Model(CGhoul2Info_v &ghoul2, const char *fileName, int,
                          qhandle_t customSkin, qhandle_t customShader,
  ...
  Q_strncpyz(ghoul2[model].mFileName, fileName,
             sizeof(ghoul2[model].mFileName));
...
qboolean G2API_SetSkin(CGhoul2Info *ghlInfo, qhandle_t customSkin,
  ...
  if (G2_SetupModelPointers(ghlInfo)) {
    ...
    if (renderSkin) { // this is going to set the surfs on/off matching the skin
                      // file
      extern void G2API_SetSurfaceOnOffFromSkin(
          CGhoul2Info * ghlInfo, qhandle_t renderSkin); // tr_ghoul2.cpp
...
qboolean G2API_SetSurfaceOnOff(CGhoul2Info *ghlInfo, const char *surfaceName,
                               const int flags) {
  if (G2_SetupModelPointers(ghlInfo)) {
    G2ERROR(!(flags & ~(G2SURFACEFLAG_OFF | G2SURFACEFLAG_NODESCENDANTS)),
            "G2API_SetSurfaceOnOff Illegal Flags");
...
qboolean G2API_SetRootSurface(CGhoul2Info_v &ghlInfo, const int modelIndex,
...
int G2API_GetSurfaceRenderStatus(CGhoul2Info *ghlInfo,
                                 const char *surfaceName) {
  G2ERROR(surfaceName, "Invalid surfaceName");
  if (G2_SetupModelPointers(ghlInfo)) {
...
qboolean G2API_RemoveGhoul2Model(CGhoul2Info_v &ghlInfo, const int modelIndex) {
  // sanity check
  if (!ghlInfo.size() || (ghlInfo.size() <= modelIndex) || modelIndex < 0 ||
      (ghlInfo[modelIndex].mModelindex < 0)) {
...
qboolean G2API_SetAnimIndex(CGhoul2Info *ghlInfo, const int index) {
  ...
  if (ghlInfo) {
    ...
    if (ghlInfo->animModelIndexOffset != index) {
      ...
      ghlInfo->currentAnimModelSize =
          0; // Clear anim size so SetupModelPointers recalcs
      ...
      for (size_t index = 0; index < ghlInfo->mBlist.size(); index++) {
        ...
        //				G2_Remove_Bone_Index(ghlInfo->mBlist,
        //index);
      }
    }
    return qtrue;
...
qboolean G2API_SetBoneAnimIndex(CGhoul2Info *ghlInfo, const int index,
                                ...
                                const int AcurrentTime, const float setFrame,
                                const int blendTime) {
  // rww - RAGDOLL_BEGIN
  ...
  if (G2_SetupModelPointers(ghlInfo)) {
    ...
    if (setFrame != -1.0f &&
        (setFrame < 0.0f || setFrame >= (float)ghlInfo->aHeader->numFrames)) {
      *(float *)&setFrame = 0.0f;
    }
    ...
    if (index >= 0 && index < (int)ghlInfo->mBlist.size()) {
      G2ERROR(ghlInfo->mBlist[index].boneNumber >= 0,
              va("Bone Index is not Active (%s)", ghlInfo->mFileName));
      int currentTime = G2API_GetTime(AcurrentTime);
			...
			*/
#endif
      ret = G2_Set_Bone_Anim_Index(ghlInfo->mBlist, index, startFrame, endFrame,
...
qboolean G2API_SetBoneAnim(CGhoul2Info *ghlInfo, const char *boneName,
                           const int startFrame, const int endFrame,
                           const int flags, const float animSpeed,
                           const int AcurrentTime, const float setFrame,
                           const int blendTime) {
  ...
  if (boneName && G2_SetupModelPointers(ghlInfo)) {
    ...
    ghlInfo->mSkelFrameNum = 0;
    int currentTime = G2API_GetTime(AcurrentTime);
    ret = G2_Set_Bone_Anim(ghlInfo, ghlInfo->mBlist, boneName, startFrame,
...
qboolean G2API_GetBoneAnim(CGhoul2Info *ghlInfo, const char *boneName,
                           const int AcurrentTime, float *currentFrame,
  ...
  if (G2_SetupModelPointers(ghlInfo)) {
    int currentTime = G2API_GetTime(AcurrentTime);
    ret =
        G2_Get_Bone_Anim(ghlInfo, ghlInfo->mBlist, boneName, currentTime,
...
qboolean G2API_GetBoneAnimIndex(CGhoul2Info *ghlInfo, const int iBoneIndex,
                                const int AcurrentTime, float *currentFrame,
                                int *startFrame, int *endFrame, int *flags,
                                float *animSpeed, int *) {
  qboolean ret = qfalse;
  if (G2_SetupModelPointers(ghlInfo)) {
    ...
    if (iBoneIndex >= 0 && iBoneIndex < (int)ghlInfo->mBlist.size()) {
      ...
      if ((ghlInfo->mBlist[iBoneIndex].flags &
        ...
        G2ERROR(sf < ghlInfo->aHeader->numFrames,
                "returning startframe>=numframes");
        G2ERROR(ef > 0, "returning endframe<=0");
        G2ERROR(ef <= ghlInfo->aHeader->numFrames,
                "returning endframe>numframes");
        if (currentFrame) {
          G2ERROR(*currentFrame >= 0.0f, "returning currentframe<0");
          G2ERROR(((int)(*currentFrame)) < ghlInfo->aHeader->numFrames,
                  "returning currentframe>=numframes");
        }
        if (endFrame) {
          *endFrame = ef;
        }
...
qboolean G2API_GetAnimRange(CGhoul2Info *ghlInfo, const char *boneName,
  ...
  G2ERROR(boneName, "NULL boneName");
  if (boneName && G2_SetupModelPointers(ghlInfo)) {
    ret = G2_Get_Bone_Anim_Range(ghlInfo, ghlInfo->mBlist, boneName, startFrame,
                                 endFrame);
...
qboolean G2API_GetAnimRangeIndex(CGhoul2Info *ghlInfo, const int boneIndex,
                                 int *startFrame, int *endFrame) {
  qboolean ret = qfalse;
  if (G2_SetupModelPointers(ghlInfo)) {
    G2ERROR(boneIndex >= 0 && boneIndex < (int)ghlInfo->mBlist.size(),
            "Bad Bone Index");
    if (boneIndex >= 0 && boneIndex < (int)ghlInfo->mBlist.size()) {
      ret = G2_Get_Bone_Anim_Range_Index(ghlInfo->mBlist, boneIndex, startFrame,
                                         endFrame);
      G2ANIM(ghlInfo, "G2API_GetAnimRange");
    }
  }
...
qboolean G2API_PauseBoneAnimIndex(CGhoul2Info *ghlInfo, const int boneIndex,
                                  const int AcurrentTime) {
  qboolean ret = qfalse;
  if (G2_SetupModelPointers(ghlInfo)) {
    int currentTime = G2API_GetTime(AcurrentTime);
...
qboolean G2API_SetBoneAnglesIndex(CGhoul2Info *ghlInfo, const int index,
                                  ...
                                  const Eorientations roll, qhandle_t *,
  ...
  if (G2_SetupModelPointers(ghlInfo)) {
    ...
    ghlInfo->mSkelFrameNum = 0;
    G2ERROR(index >= 0 && index < (int)ghlInfo->mBlist.size(),
            "G2API_SetBoneAnglesIndex:Invalid bone index");
    if (index >= 0 && index < (int)ghlInfo->mBlist.size()) {
      ret = G2_Set_Bone_Angles_Index(ghlInfo, ghlInfo->mBlist, index, angles,
                                     flags, yaw, pitch, roll, blendTime,
                                     currentTime);
    }
  }
...
qboolean G2API_SetBoneAngles(CGhoul2Info *ghlInfo, const char *boneName,
                             const vec3_t angles, const int flags,
                             const Eorientations up, const Eorientations left,
                             const Eorientations forward, qhandle_t *,
                             int blendTime, int AcurrentTime) {
  // rww - RAGDOLL_BEGIN
  if (ghlInfo && ghlInfo->mFlags & GHOUL2_RAG_STARTED) {
    return qfalse;
  ...
  if (boneName && G2_SetupModelPointers(ghlInfo)) {
    ...
    // ensure we flush the cache
    ghlInfo->mSkelFrameNum = 0;
    ret = G2_Set_Bone_Angles(ghlInfo, ghlInfo->mBlist, boneName, angles, flags,
                             up, left, forward, blendTime, currentTime);
  }
...
qboolean G2API_SetBoneAnglesMatrixIndex(CGhoul2Info *ghlInfo, const int index,
  ...
  if (G2_SetupModelPointers(ghlInfo)) {
    ...
    G2ERROR(index >= 0 && index < (int)ghlInfo->mBlist.size(),
            "Bad Bone Index");
    if (index >= 0 && index < (int)ghlInfo->mBlist.size()) {
      ret = G2_Set_Bone_Angles_Matrix_Index(ghlInfo->mBlist, index, matrix,
                                            flags, blendTime, currentTime);
    }
  }
...
qboolean G2API_SetBoneAnglesMatrix(CGhoul2Info *ghlInfo, const char *boneName,
  ...
  if (boneName && G2_SetupModelPointers(ghlInfo)) {
    ...
    // ensure we flush the cache
    ghlInfo->mSkelFrameNum = 0;
    ret = G2_Set_Bone_Angles_Matrix(ghlInfo, ghlInfo->mBlist, boneName, matrix,
                                    flags, blendTime, currentTime);
  }
...
void G2API_AnimateG2Models(CGhoul2Info_v &ghoul2, int AcurrentTime,
...
int G2API_AddBolt(CGhoul2Info *ghlInfo, const char *boneName) {
  ...
  if (boneName && G2_SetupModelPointers(ghlInfo)) {
    ...
    G2NOTE(ret >= 0,
           va("G2API_AddBolt Failed (%s:%s)", boneName, ghlInfo->mFileName));
  }
  return ret;
}
...
int G2API_AddBoltSurfNum(CGhoul2Info *ghlInfo, const int surfIndex) {
  int ret = -1;
  if (G2_SetupModelPointers(ghlInfo)) {
    ret = G2_Add_Bolt_Surf_Num(ghlInfo, ghlInfo->mBltlist, ghlInfo->mSlist,
                               surfIndex);
  }
...
qboolean G2API_AttachG2Model(CGhoul2Info *ghlInfo, CGhoul2Info *ghlInfoTo,
                             int toBoltIndex, int toModel) {
  qboolean ret = qfalse;
  if (G2_SetupModelPointers(ghlInfo) && G2_SetupModelPointers(ghlInfoTo)) {
    G2ERROR(toBoltIndex >= 0 && toBoltIndex < (int)ghlInfoTo->mBltlist.size(),
            "Invalid Bolt Index");
    G2ERROR(ghlInfoTo->mBltlist.size() > 0, "Empty Bolt List");
    assert(toBoltIndex >= 0);
    if (toBoltIndex >= 0 && ghlInfoTo->mBltlist.size()) {
      // make sure we have a model to attach, a model to attach to, and a bolt
      // on that model
      if (((ghlInfoTo->mBltlist[toBoltIndex].boneNumber != -1) ||
           (ghlInfoTo->mBltlist[toBoltIndex].surfaceNumber != -1))) {
        // encode the bolt address into the model bolt link
        toModel &= MODEL_AND;
        toBoltIndex &= BOLT_AND;
        ghlInfo->mModelBoltLink =
...
qboolean G2API_AttachEnt(int *boltInfo, CGhoul2Info *ghlInfoTo, int toBoltIndex,
                         int entNum, int toModelNum) {
  qboolean ret = qfalse;
...
qboolean G2API_GetBoltMatrix(CGhoul2Info_v &ghoul2, const int modelIndex,
                             ...
                             const int AframeNum, qhandle_t *modelList,
                             const vec3_t scale) {
...
// This is needed for some unfortunate jiggery-hackery to do with frameskipping
// & the animevents.cfg file
//
...
void G2API_CollisionDetect(CCollisionRecord *collRecMap, CGhoul2Info_v &ghoul2,
                           const vec3_t angles, const vec3_t position,
                           int AframeNumber, int entNum, vec3_t rayStart,
                           vec3_t rayEnd, vec3_t scale, CMiniHeap *,
  ...
  if (G2_SetupModelPointers(ghoul2) && collRecMap) {
...
#ifdef _G2_GORE
    G2_TransformModel(ghoul2, frameNumber, scale, ri.GetG2VertSpaceServer(),
                      useLod, false);
#else
    G2_TransformModel(ghoul2, frameNumber, scale, ri.GetG2VertSpaceServer(),
                      useLod);
    ...
    G2_TraceModels(ghoul2, transRayStart, transRayEnd, collRecMap, entNum,
                   eG2TraceType, useLod, fRadius, 0, 0, 0, 0, 0, qfalse);
#else
    G2_TraceModels(ghoul2, transRayStart, transRayEnd, collRecMap, entNum,
                   eG2TraceType, useLod, fRadius);
#endif

    ri.GetG2VertSpaceServer()->ResetHeap();
    // now sort the resulting array of collision records so they are distance
    // ordered
    qsort(collRecMap, MAX_G2_COLLISIONS, sizeof(CCollisionRecord),
          QsortDistance);
...
// copy a model from one ghoul2 instance to another, and reset the root surface
// on the new model if need be NOTE if modelIndex = -1 then copy all the models
void G2API_CopyGhoul2Instance(CGhoul2Info_v &ghoul2From,
                              CGhoul2Info_v &ghoul2To, int modelIndex) {
  // Ensiform: I'm commenting this out because modelIndex appears unused and
  // legitimately set in gamecode assert(modelIndex==-1); // copy individual
  // bolted parts is not used in jk2 and I didn't want to deal with it
  ...
  if (ghoul2From.IsValid()) {
    ...
    //(since we are sharing this gore set with the copied instance we will have
    //to increment the reference count - if the goreset is "removed" while the
    // refcount is > 0, the refcount is decremented to avoid giving other
    // instances an invalid pointer -rww)
...
char *G2API_GetSurfaceName(CGhoul2Info *ghlInfo, int surfNumber) {
  ...
  if (G2_SetupModelPointers(ghlInfo)) {
    ...
    surf =
        (mdxmSurface_t *)G2_FindSurface(ghlInfo->currentModel, surfNumber, 0);
    if (surf) {
      assert(G2_MODEL_OK(ghlInfo));
      mdxmHierarchyOffsets_t *surfIndexes =
          (mdxmHierarchyOffsets_t *)((byte *)ghlInfo->currentModel->mdxm +
                                     sizeof(mdxmHeader_t));
      surfInfo =
...
void G2API_AddSkinGore(CGhoul2Info_v &ghoul2, SSkinGoreData &gore) {
  ...
  for (lod = lodbias; lod < maxLod; lod++) {
    ...
    G2_TransformModel(ghoul2, gore.currentTime, gore.scale,
                      ri.GetG2VertSpaceServer(), lod, true, &gore);
    ...
    G2_TraceModels(ghoul2, transHitLocation, transRayDirection, 0, gore.entNum,
                   G2_NOCOLLIDE, lod, 1.0f, gore.SSize, gore.TSize, gore.theta,
    ...
    if (ghlInfo->currentModel) {
      if (ghlInfo->currentModel->mdxm) {
        ...
        if (ghlInfo->animModel) {
          ...
          if (!ghlInfo->aHeader) {
            Com_Error(ERR_DROP, "Ghoul2 Model has no mdxa (gla) %s",
                      ghlInfo->mFileName);
          }
