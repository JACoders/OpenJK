...
extern void G_ReflectMissile(gentity_t *ent, gentity_t *missile,
                             vec3_t forward);
extern int G_CheckLedgeDive(gentity_t *self, float checkDist, vec3_t checkVel,
...
int mindTrickTime[NUM_FORCE_POWER_LEVELS] = {0, // none
                                             5000, 10000, 15000};
...
void G_CreateG2AttachedWeaponModel(gentity_t *ent, const char *psWeaponModel) {
  ...
  if (char *spot = (char *)Q_stristr(weaponModel, ".md3")) {
    *spot = 0;
    spot = (char *)Q_stristr(
        weaponModel, "_w"); // i'm using the in view weapon array instead of
  ...
  ent->weaponModel = gi.G2API_InitGhoul2Model(ent->ghoul2, weaponModel,
                                              G_ModelIndex(weaponModel),
                                              NULL_HANDLE, NULL_HANDLE, 0, 0);
  if (ent->weaponModel != -1) {
    // attach it to the hand
    gi.G2API_AttachG2Model(&ent->ghoul2[ent->weaponModel],
                           &ent->ghoul2[ent->playerModel], ent->handRBolt,
                           ent->playerModel);
    // set up a bolt on the end so we can get where the sabre muzzle is - we can
    // assume this is always bolt 0
      ...
      if (ent->client->NPC_class == CLASS_DESANN) {
        ...
        ent->client->ps.saberAnimLevel = FORCE_LEVEL_1;
                 ...
                 (ent->NPC->rank == RANK_CREWMAN ||
                  ent->NPC->rank == RANK_ENSIGN)) { // acrobat & force-users
                                                    // always use medium attacks
    ...
    if (ent->client->ps.saberEntityNum <= 0 ||
      ...
      */
      ...
      saberent->s.modelindex =
          WP_SetSaberModel(ent->client, ent->client->NPC_class);
      gi.G2API_InitGhoul2Model(saberent->ghoul2, ent->client->ps.saberModel,
                               saberent->s.modelindex, NULL_HANDLE, NULL_HANDLE,
                               0, 0);
      ...
      if (dmgFraction[i] < saberHitFraction || brokenParry) {
        ...
        if (totalDmg[i] > 0) { // actually want to do *some* damage here
          ...
          if (totalDmg[i] > 0) {
            ...
            G_Damage(victim, ent, ent, dmgDir[i], dmgSpot[i], ceil(totalDmg[i]),
...
extern qboolean tri_tri_intersect(vec3_t V0, vec3_t V1, vec3_t V2, vec3_t U0,
                                  vec3_t U1, vec3_t U2);
qboolean WP_SabersIntersect(gentity_t *ent1, gentity_t *ent2,
                            qboolean checkDir) {
  ...
  */
  ...
  // if ( ent1->client->ps.saberInFlight )
  {
    ...
    VectorMA(saberBase1, ent1->client->ps.saberLength,
             ent1->client->renderInfo.muzzleDirOld, saberTip1);
    VectorMA(saberBaseNext1, ent1->client->ps.saberLength,
             ent1->client->renderInfo.muzzleDir, saberTipNext1);
  ...
  ent1->client->renderInfo.muzzleDir, saberTip1 ); VectorMA( saberBaseNext1,
  ent1->client->ps.saberLength, ent1->client->renderInfo.muzzleDirNext,
  saberTipNext1 );
  }
  */
  ...
  // if ( ent2->client->ps.saberInFlight )
  {
    ...
    VectorMA(saberBase2, ent2->client->ps.saberLength,
             ent2->client->renderInfo.muzzleDirOld, saberTip2);
    VectorMA(saberBaseNext2, ent2->client->ps.saberLength,
             ent2->client->renderInfo.muzzleDir, saberTipNext2);
  ...
  ent2->client->renderInfo.muzzleDir, saberTip2 ); VectorMA( saberBaseNext2,
  ent2->client->ps.saberLength, ent2->client->renderInfo.muzzleDirNext,
  saberTipNext2 );
  }
  */
  if (checkDir) { // check the direction of the two swings to make sure the
                  // sabers are swinging towards each other
    ...
    // now check orientation of sabers, make sure they're not parallel or close
    // to it
    float dot = DotProduct(ent1->client->renderInfo.muzzleDir,
                           ent2->client->renderInfo.muzzleDir);
    if (dot > 0.9f || dot < -0.9f) { // too parallel to really block
                                     // effectively?
      return qfalse;
    }
  }
...
float WP_SabersDistance(gentity_t *ent1, gentity_t *ent2) {
  ...
  */
  ...
  ent1->client->renderInfo.muzzleDirNext, saberTipNext1 );
  }
  */
  ...
  ent2->client->renderInfo.muzzleDirNext, saberTipNext2 );
  }
  */
...
qboolean WP_SabersIntersection(gentity_t *ent1, gentity_t *ent2,
  ...
  ShortestLineSegBewteen2LineSegs(saberBaseNext1, saberTipNext1, saberBaseNext2,
                                  saberTipNext2, saberPoint1, saberPoint2);
...
qboolean WP_SaberDamageEffects(trace_t *tr, const vec3_t start, float length,
                               float dmg, vec3_t dmgDir, vec3_t bladeDir,
                               int enemyTeam) {
...
farther than saberHitFraction %4.2f\n", (distFromStart/length), saberHitFraction
  ...
  for (i = 0; i < numHitEnts; i++) {
    ...
    if (hitEntNum[i] != ENTITYNUM_NONE) {
      ...
      if (doDmg > 0) {
        WP_SaberDamageAdd(1.0, hitEntNum[i], hitEntDir[i], hitEntPoint[i],
                          ceil(doDmg), hitEntStartFrac[i], trHitLoc,
                          trDismember, trDismemberLoc);
      }
    }
  }
...
qboolean WP_SaberDamageForTrace(int ignore, vec3_t start, vec3_t end, float dmg,
  ...
  if (d_saberCombat->integer > 1) {
    if (attacker != NULL && attacker->client != NULL) {
      G_DebugLine(start, end2, FRAMETIME,
                  WPDEBUG_SaberColor(attacker->client->ps.saberColor), qtrue);
    }
  }
#endif
  ...
  if (&g_entities[tr.entityNum]) {
    ...
    if (hitEnt->contents & CONTENTS_LIGHTSABER) {
      ...
      qboolean sabersIntersect =
          WP_SabersIntersect(attacker, owner, qfalse); // qtrue );
      float sabersDist;
      if (attacker && attacker->client && attacker->client->ps.saberInFlight &&
    ...
    if (hitEnt->takedamage) {
      ...
      // attacker->client->playerTeam) )
      {
        ...
        if (noGhoul ||
          ...
          WP_SaberDamageAdd(trFrac, tr.entityNum, dir, tr.endpos, dmg, dmgFrac,
                            HL_NONE, qfalse, HL_NONE);
          ...
          if (!WP_SaberDamageEffects(
                  &tr, start, len, dmg, dir, bladeDir,
...
qboolean WP_SabersCheckLock2(gentity_t *attacker, gentity_t *defender,
  ...
  if (ValidAnimFileIndex(attacker->client->clientInfo.animFileIndex)) {
    anim = &level.knownAnimFileSets[attacker->client->clientInfo.animFileIndex]
                .animations[attAnim];
  ...
  if (ValidAnimFileIndex(defender->client->clientInfo.animFileIndex)) {
    anim = &level.knownAnimFileSets[defender->client->clientInfo.animFileIndex]
                .animations[defAnim];
    PM_SetAnimFrame(defender, anim->firstFrame + advance, qtrue,
                    qtrue); // was anim->firstFrame + anim->numFrames - advance,
                            // but that's wrong since they are matched anims
#ifndef FINAL_BUILD
    if (d_saberCombat->integer) {
      Com_Printf("%s starting saber lock, anim = %s, %d frames to go!\n",
  ...
  attacker->client->ps.legsAnimTimer = attacker->client->ps.torsoAnimTimer =
      defender->client->ps.legsAnimTimer = defender->client->ps.torsoAnimTimer =
          SABER_LOCK_TIME;
  ...
  // move them closer?
  float defPitchAdd = 0,
        zDiff = ((attacker->currentOrigin[2] + attacker->client->standheight) -
  ...
  */
  ...
  gi.trace(&trace, attacker->currentOrigin, attacker->mins, attacker->maxs,
           newOrg, attacker->s.number, attacker->clipmask, G2_NOCOLLIDE, 0);
  ...
  gi.trace(&trace, defender->currentOrigin, defender->mins, defender->maxs,
           newOrg, defender->s.number, defender->clipmask, G2_NOCOLLIDE, 0);
  if (!trace.startsolid && !trace.allsolid) {
...
qboolean WP_SabersCheckLock(gentity_t *ent1, gentity_t *ent2) {
  ...
  if (ent1->client->ps.torsoAnim == BOTH_A1_BR_TL ||
      ...
      ent1->client->ps.torsoAnim ==
          BOTH_A5_BR_TL) { // ent1 is attacking in the opposite diagonal
  ...
  if (ent2->client->ps.torsoAnim == BOTH_A1_BR_TL ||
      ...
      ent2->client->ps.torsoAnim ==
          BOTH_A5_BR_TL) { // ent2 is attacking in the opposite diagonal
  ...
  if (ent1->client->ps.torsoAnim == BOTH_A1_BL_TR ||
      ...
      ent1->client->ps.torsoAnim ==
          BOTH_A5_BL_TR) { // ent1 is attacking in the opposite diagonal
  ...
  if (ent2->client->ps.torsoAnim == BOTH_A1_BL_TR ||
      ...
      ent2->client->ps.torsoAnim ==
          BOTH_A5_BL_TR) { // ent2 is attacking in the opposite diagonal
  ...
  if (ent1->client->ps.torsoAnim == BOTH_A1_TR_BL ||
      ...
      ent1->client->ps.torsoAnim == BOTH_A4_TR_BL ||
      ent1->client->ps.torsoAnim ==
          BOTH_A5_TR_BL) { // ent1 is attacking diagonally
  ...
  if (ent2->client->ps.torsoAnim == BOTH_A1_TR_BL ||
      ...
      ent2->client->ps.torsoAnim == BOTH_A4_TR_BL ||
      ent2->client->ps.torsoAnim ==
          BOTH_A5_TR_BL) { // ent2 is attacking diagonally
  ...
  if (ent1->client->ps.torsoAnim == BOTH_A1_TL_BR ||
      ...
      ent1->client->ps.torsoAnim == BOTH_A4_TL_BR ||
      ent1->client->ps.torsoAnim ==
          BOTH_A5_TL_BR) { // ent1 is attacking diagonally
  ...
  if (ent2->client->ps.torsoAnim == BOTH_A1_TL_BR ||
      ...
      ent2->client->ps.torsoAnim == BOTH_A4_TL_BR ||
      ent2->client->ps.torsoAnim ==
          BOTH_A5_TL_BR) { // ent2 is attacking diagonally
  ...
  if (ent1->client->ps.torsoAnim == BOTH_A1_T__B_ ||
      ...
      ent1->client->ps.torsoAnim ==
          BOTH_A5_T__B_) { // ent1 is attacking top-down
    /*
  ...
  if (ent2->client->ps.torsoAnim == BOTH_A1_T__B_ ||
      ...
      ent2->client->ps.torsoAnim ==
          BOTH_A5_T__B_) { // ent2 is attacking top-down
    /*
...
void WP_SaberDamageTrace(gentity_t *ent) {
  ...
  if (ent->client->ps.saberInFlight) { // flying sabers are much more deadly
    ...
    if (ent->health <= 0 &&
        !g_saberRealisticCombat
             ->integer) { // so enemies don't keep trying to block it
    ...
    // or unless returning
             ...
             SES_RETURNING) { // special case, since we're returning, chances
                              // are if we hit something
      ...
      if (!PM_SaberInSpecialAttack(ent->client->ps.torsoAnim) &&
        ...
        baseDamage = 0;
                 ...
                 (PM_SaberInAttack(ent->client->ps.saberMove) ||
                  ...
                  entPowerLevel >
                      FORCE_LEVEL_2)) { // normal attack swing swinging/spinning
                                        // (or if using strong set), do normal
                                        // damage
    ...
    if (ent->health > 0 && !ent->client->ps.saberLockTime) {
      ...
      {                       // a valid ent
        ...
        if (traceEnt && traceEnt->client && traceEnt->health > 0 &&
            ...
            !PM_InKnockDown(
                &traceEnt->client->ps)) { // enemy client, push them away
          if (!traceEnt->client->ps.saberLockTime &&
              !traceEnt->message) { // don't push people in saberlock or with
                                    // security keys
            vec3_t hitDir;
  ...
  if (VectorCompare2(baseOld, baseNew) && VectorCompare2(endOld, endNew)) {
    hit_wall =
        WP_SaberDamageForTrace(ent->s.number, mp2, endNew, baseDamage * 4, md2,
    ...
    hit_wall = WP_SaberDamageForTrace(ent->s.number, baseOld, baseNew,
                                      baseDamage, md2, qfalse, entPowerLevel);
    ...
    while (1) {
      ...
      for (step = stepsize; step < ent->client->ps.saberLength &&
                            step < ent->client->ps.saberLengthOld;
           step += 12) {
    ...
    aveLength =
        (ent->client->ps.saberLengthOld + ent->client->ps.saberLength) / 2;
    ...
    if (WP_SaberDamageForTrace(ent->s.number, endOld, endNew,
                               tipDmgMod * baseDamage, md2, qfalse,
  ...
  if ((saberHitFraction < 1.0f ||
      ...
      (ent->client->ps.weaponstate == WEAPON_FIRING ||
       ent->client->ps
           .saberInFlight)) { // The saber (in-hand) hit another saber, mano.
      ...
      if (entPowerLevel == FORCE_LEVEL_3 &&
        ...
        if (hitOwner->client->NPC_class == CLASS_DESANN ||
            ...
            hitOwner->client->NPC_class ==
                CLASS_LUKE) { // these masters can even block unblockables
                              // (stops cheap kills)
    ...
    if (ent->client->ps.saberInFlight && ent->client->ps.saberActive &&
        ent->client->ps.saberEntityNum != ENTITYNUM_NONE &&
    ...
    if (hitOwner && hitOwner->health > 0 && ent->health > 0 // both are alive
      ...
      if (baseDamage) { // there is damage involved, not just effects
        ...
        if (entAttacking && hitOwnerAttacking &&
        ...
        } else if (hitOwnerAttacking && entDefending && !Q_irand(0, 2) &&
                   (ent->client->ps.saberMove != LS_READY ||
                    (hitOwnerPowerLevel -
          ...
          if (!Q_irand(0, 2) && activeDefense &&
            ...
            if (entPowerLevel < FORCE_LEVEL_3
                ...
                (hitOwnerPowerLevel > FORCE_LEVEL_2 ||
                 (hitOwner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] >
                  ...
                  Q_irand(
                      0,
                      hitOwner->client->ps
                          .saberAnimLevel)))) { // knockaways can make
                                                // fast-attacker go into a broken
                                                // parry anim if the ent is using
                                                // fast or med (but not Tavion)
              // make me parry
              ...
              ent->client->ps.saberBlocked = BLOCKED_PARRY_BROKEN;
              if (ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE] <
                      FORCE_LEVEL_2 &&
                  (ent->s.number ||
...
#endif
            } else if (entPowerLevel > FORCE_LEVEL_2 || !activeDefense ||
                       (!deflected && Q_irand(0,
                  ...
                  Q_irand(0,
                          ent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] -
                              hitOwner->client->ps.forcePowerLevel
                                  [FP_SABER_DEFENSE])) { // chance of continuing
                                                         // with the attack (not
                                                         // bouncing back)
                ent->client->ps.saberEventFlags &= ~SEF_BLOCKED;
              ...
              if (PM_SaberInBounce(
                  ...
                  hitOwner->client->ps.saberAnimLevel != FORCE_LEVEL_5 &&
                  hitOwner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] >
                      FORCE_LEVEL_2) { // attacker bounced off, and defender has
                                       // ability to do knockaways, so do one
                                       // unless we're using fast attacks
                // turn the parry into a knockaway
                hitOwner->client->ps.saberBounceMove =
                    PM_KnockawayForParry(hitOwner->client->ps.saberBlocked);
...
(follow-through)!\n", ent->targetname );
          ...
          if (PM_SaberInAttack(hitOwner->client->ps.saberMove) ||
            ...
            */
            ...
            if (!PM_SaberInBrokenParry(
              ...
              if (!WP_SaberParry(
                      hitOwner,
                      ent)) { // FIXME: hitOwner can't parry, do some
                              // time-consuming saber-knocked-aside broken parry
                              // anim? hitOwner->client->ps.saberBlocked =
                              // BLOCKED_PARRY_BROKEN;
              }
            }
          }
    ...
    if (saberHitFraction < 1.0f ||
      ...
      if (ent->client->ps.saberLockTime < level.time) {
        if (inFlightSaberBlocked) { // FIXME: never hear this sound
          G_Sound(&g_entities[ent->client->ps.saberEntityNum],
                  G_SoundIndex(va("sound/weapons/saber/saberbounce%d.wav",
                                  Q_irand(1, 3))));
        } else {
          if (deflected) {
            G_Sound(ent,
            ...
            G_Sound(ent, G_SoundIndex(va("sound/weapons/saber/saberblock%d.wav",
                                         Q_irand(1, 9))));
          }
        }
        ...
        if (!Q_irand(0, knockAway) || // random
            (hitOwner && hitOwner->client &&
             (hitOwner->client->NPC_class == CLASS_DESANN ||
              hitOwner->client->NPC_class == CLASS_TAVION ||
              hitOwner->client->NPC_class ==
                  CLASS_LUKE)) // or if blocked by a Boss character FIXME: or
                               // base on defense level?
  ...
  if (WP_SaberApplyDamage(ent, baseDamage, baseDFlags,
    ...
    G_Sound(ent, G_SoundIndex(
                     va("sound/weapons/saber/saberhit%d.wav", Q_irand(1, 3))));
  }
        ...
        if (missile_list[x]->s.weapon == WP_SABER) { // just send it back
          if (missile_list[x]->owner && missile_list[x]->owner->client &&
            ...
            if (missile_list[x]->owner->client->ps.saberInFlight &&
              ...
              G_Sound(missile_list[x],
                      G_SoundIndex(va("sound/weapons/saber/saberblock%d.wav",
                                      Q_irand(1, 9))));
              g_saberFlashTime = level.time - 50;
...
qboolean
WP_SaberLaunch(gentity_t *self, gentity_t *saber,
...
void WP_SaberThrow(gentity_t *self, usercmd_t *ucmd) {
  ...
  if (!self->client->ps.saberInFlight) { // saber is not in flight right now
    ...
    if (saberent->s.pos.trType != TR_STATIONARY) { // saber is in flight, lerp
      ...
      if (self->health <= 0 &&
          level.time >
              saberent->s.time + 5000) { // make us free ourselves after a time
...
void WP_SaberStartMissileBlockCheck(gentity_t *self, usercmd_t *ucmd) {
  ...
  if (self->client->ps.forcePowersActive &
      (1 << FP_GRIP)) { // can't block while gripping (FIXME: or should it break
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (ent->s.eType != ET_MISSILE &&
      ...
      if (ent->owner->health <= 0 &&
          !g_saberRealisticCombat
               ->integer) { // it's not doing damage, so ignore it
        continue;
      }
    } else {
    ...
    if (ent->s.weapon == WP_THERMAL) { // thermal detonator!
      ...
      if (!self->s.number) { // players don't auto-handle these at all
        ...
        if (ent->s.pos.trType == TR_STATIONARY &&
            (ent->s.eFlags & EF_MISSILE_STICK)) { // a placed explosive like a
                                                  // tripmine or detpack
    ...
    if (dist < closestDist) {
      ...
      gi.trace(&trace, ent->currentOrigin, ent->mins, ent->maxs, traceTo,
               ent->s.number, ent->clipmask, G2_NOCOLLIDE, 0);
  ...
  if (incoming) {
    if (self->NPC && !G_ControlledByPlayer(self)) {
      ...
      if (Jedi_SaberBlockGo(self, &self->NPC->last_ucmd, NULL, NULL,
                            incoming) !=
...
void WP_SaberUpdate(gentity_t *self, usercmd_t *ucmd) {
      ...
      G_SetOrigin(saberent, self->currentOrigin);
               ...
               self->client->ps.saberBlocking ==
                   BLK_WIDE) { // FIXME: keep bbox in front of player, even when
                               // wide?
      vec3_t saberOrg;
      if (((self->s.number && !Jedi_SaberBusy(self) &&
            !g_saberRealisticCombat->integer) ||
           (self->s.number == 0 && self->client->ps.saberBlocking == BLK_WIDE &&
            (g_saberAutoBlocking->integer ||
             self->client->ps.saberBlockingTime > level.time))) &&
          self->client->ps.weaponTime <=
              0) { // full-size blocking for non-attacking player with
        ...
        vec3_t saberTip;
        VectorMA(self->client->renderInfo.muzzlePoint,
                 self->client->ps.saberLength,
                 self->client->renderInfo.muzzleDir, saberTip);
        VectorMA(self->client->renderInfo.muzzlePoint,
                 self->client->ps.saberLength * 0.5,
  ...
  if (d_saberCombat->integer > 2) {
    CG_CubeOutline(saberent->absmin, saberent->absmax, 50,
                   WPDEBUG_SaberColor(self->client->ps.saberColor), 1);
  }
#endif
}
...
void ForceThrow(gentity_t *self,
                qboolean pull) { // FIXME: pass in a target ent so we (an NPC)
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (ent->NPC && ent->NPC->scriptFlags & SCF_NO_FORCE) {
      if (ent->s.weapon ==
          WP_SABER) { // Hmm, should jedi do the resist behavior?  If this is
                      // on, perhaps it's because of a cinematic?
    ...
    if (ent->s.eType != ET_MISSILE) {
                                  ...
                                  //dropped items
      {
        ...
        if (!ent->client) {
          if (Q_stricmp("lightsaber", ent->classname) != 0) { // not a
                                                              // lightsaber
                ...
                if (Q_stricmp("func_static", ent->classname) != 0 ||
                  ...
                  if (Q_stricmp("limb", ent->classname)) { // not a limb
                    if (ent->s.weapon == WP_TURRET &&
                      ...
                      if (!self->s.number ||
                          self->enemy !=
                              ent) { // only NPCs who are actively mad at this
                                     // turret can push it over
                        continue;
                      }
                    } else {
                      continue;
          ...
          if (pull ||
            ...
            continue;
                   ...
                   ent->client->NPC_class ==
                       CLASS_ATST) { // can't push ATST or Galak
          continue;
          ...
          continue;
                   ...
                   self->enemy &&
                   self->enemy != ent) { // can't accidently push a teammate
                                         // while in combat
          continue;
        }
      } else if (ent->s.eType == ET_ITEM && ent->item &&
        ...
        if ((dot1 = DotProduct(dir, forward)) < cone - 0.3f)
          continue;
                 ...
                 ET_MISSILE) //&& ent->s.eType != ET_ITEM && ent->e_ThinkFunc !=
                             //thinkF_G_RunObject )
      { // missiles are easier to force-push, never require direct trace (FIXME:
        // maybe also items and general physics objects)
        if ((dot1 = DotProduct(dir, forward)) < cone - 0.3f)
          continue;
        ...
        continue;
               ...
               ET_MISSILE) { // a missile and we're at force level 1... just use
                             // a small cone, but not ridiculously small
    ...
    if (ent != forwardEnt) { // don't need to trace against forwardEnt again
      ...
      gi.trace(&tr, self->client->renderInfo.eyePoint, vec3_origin, vec3_origin,
               ent_org, self->s.number, MASK_OPAQUE | CONTENTS_SOLID,
               G2_NOCOLLIDE, 0); // was MASK_SHOT, but changed to match above
                                 // trace and crosshair trace
      if (tr.fraction < 1.0f &&
  ...
  if (ent_count) {
    for (int x = 0; x < ent_count; x++) {
      if (push_list[x]->client) {
        ...
        if (!push_list[x]->s.number) { // player
          if (push_list[x]->health > 0 // alive
              ...
              && push_list[x]->client->ps.torsoAnim !=
                     BOTH_FORCEGRIP_HOLD // BOTH_FORCEGRIP1//wasn't trying to
                                         // grip anyone
              ...
              && !PM_InKnockDown(
                     &push_list[x]->client->ps) // not knocked down already
              ...
              && (push_list[x]->client->ps.weapon == WP_SABER ||
                  push_list[x]->client->ps.weapon ==
                      WP_MELEE)) // using saber or fists
          {                      // trying to push or pull the player!
            if (push_list[x]->client->ps.powerups[PW_FORCE_PUSH] >
                ...
                ||
                (pull &&
                 Q_irand(0, (push_list[x]->client->ps.forcePowerLevel[FP_PULL] -
                             self->client->ps.forcePowerLevel[FP_PULL]) *
                                    2 +
                                1) > 0) // player's pull is high enough
                ||
                (!pull &&
                 Q_irand(0, (push_list[x]->client->ps.forcePowerLevel[FP_PUSH] -
              ...
              if (InFront(self->currentOrigin,
                ...
                push_list[x]->client->ps.saberMove =
                    push_list[x]->client->ps.saberBounceMove =
                        LS_READY; // don't finish whatever saber anim you may
                                  // have been in
        ...
        if (((self->s.number == 0 && Q_irand(0, 2)) || Q_irand(0, 2)) &&
          ...
          if (push_list[x]->s.number &&
              push_list[x]->message) { // an NPC who has a key
          ...
          if (pull) {
            ...
            if (self->client->ps.forcePowerLevel[FP_PULL] > FORCE_LEVEL_1 &&
                ...
                push_list[x]->s.weapon !=
                    WP_THERMAL) { // yank the weapon - NOTE: level 1 just knocks
                                  // them down, not take weapon
      ...
      } else if (push_list[x]->s.weapon == WP_SABER &&
                                   ...
                                   // are dealt with below
      {
        ...
        VectorMA(self->client->renderInfo.eyePoint, radius, forward, end);
        gi.trace(&tr, self->client->renderInfo.eyePoint, vec3_origin,
                 vec3_origin, end, self->s.number, MASK_SHOT, G2_NOCOLLIDE, 0);
        ...
        */
        ...
        if (pull) {
          ...
          // HMM, if I have an auto-enemy & he's in front of me, push it toward
          // him?
          VectorSubtract(push_list[x]->currentOrigin, self->currentOrigin,
                         pushDir);
          knockback -= VectorNormalize(pushDir);
      ...
      if (self->client->ps.forcePowerLevel[FP_PUSH] >
        ...
        if (actualCost > 50) {
          ...
          actualCost = cost;
                 ...
                 FORCE_LEVEL_1) { // at level 3, can push multiple, so costs
                                  // more
...
void ForceTelepathy(gentity_t *self) {
  ...
  gi.trace(&tr, self->client->renderInfo.eyePoint, vec3_origin, vec3_origin,
           end, self->s.number, MASK_OPAQUE | CONTENTS_BODY, G2_NOCOLLIDE, 0);
    ...
    if (G_ActivateBehavior(traceEnt,
      ...
      if ((traceEnt->NPC->scriptFlags & SCF_NO_MIND_TRICK)) {
        if (traceEnt->client->NPC_class == CLASS_GALAKMECH) {
          G_AddVoiceEvent(NPC, Q_irand(EV_CONFUSE1, EV_CONFUSE3),
                          Q_irand(3000, 5000));
        }
        ...
        traceEnt->NPC->controlledTime = level.time + 30000;
                 ...
                 WP_SABER) { // haha!  Jedi aren't easily confused!
...
void ForceGrip(gentity_t *self) { // FIXME: make enemy Jedi able to use this
  ...
  if (self->enemy &&
      (self->s.number ||
       InFront(self->enemy->currentOrigin, self->client->renderInfo.eyePoint,
               self->client->ps.viewangles,
               0.2f))) { // NPCs can always lift enemy since we assume they're
                         // looking at them, players need to be facing the enemy
  ...
  if (!traceEnt) { // okay, trace straight ahead and see what's there
    gi.trace(&tr, self->client->renderInfo.handLPoint, vec3_origin, vec3_origin,
             end, self->s.number, MASK_SHOT, G2_NOCOLLIDE, 0);
...
void ForceLightningDamage(gentity_t *self, gentity_t *traceEnt, vec3_t dir,
  ...
  if (traceEnt && traceEnt->takedamage) {
    if (!traceEnt->client ||
      ...
      if (traceEnt->client && traceEnt->health > 0 &&
          (traceEnt->client->NPC_class == CLASS_DESANN ||
           traceEnt->client->NPC_class ==
               CLASS_LUKE)) { // Luke and Desann can shield themselves from the
                              // attack
      ...
      if (traceEnt && traceEnt->client &&
          traceEnt->client->NPC_class == CLASS_GALAK) {
  ...
  if (self->client->ps.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_2) { // arc
    ...
    for (e = 0; e < numListedEntities; e++) {
      ...
      gi.trace(&tr, self->client->renderInfo.handLPoint, vec3_origin,
               vec3_origin, ent_org, self->s.number, MASK_SHOT, G2_NOCOLLIDE,
               0);
      if (tr.fraction < 1.0f &&
    ...
    while (traces <
      ...
      if (traceEnt && traceEnt->s.number &&
          traceEnt->s.weapon == WP_SABER) //&& traceEnt->NPC
...
qboolean WP_ForcePowerUsable(gentity_t *self, forcePowers_t forcePower,
  ...
  if (self->flags &
      FL_LOCK_PLAYER_WEAPONS) // yes this locked weapons check also includes
                              // force powers, if we need a separate check later
                              // I'll make one
  {
  ...
  */
  ...
  if (self->client->ps.vehicleModel !=
      0) { // Doh!  No force powers when flying a vehicle!
    return qfalse;
  }
  if (self->client->ps.viewEntity > 0 &&
      self->client->ps.viewEntity <
...
static void WP_ForcePowerRun(gentity_t *self, forcePowers_t forcePower,
  ...
  switch ((int)forcePower) {
  case FP_HEAL:
    if (self->client->ps.forceHealCount >= MAX_FORCE_HEAL ||
        self->health >=
            self->client->ps
  ...
  case FP_GRIP:
    if (!WP_ForcePowerAvailable(self, FP_GRIP, 0) ||
      ...
      { // either invalid ent, or dead ent
        ...
        WP_ForcePowerStop(self, FP_GRIP);
        return;
      } else if (gripEnt->s.weapon == WP_SABER && gripEnt->NPC &&
                 gripEnt->client &&
                 gripEnt->client->ps.forcePowersKnown & (1 << FP_PUSH) &&
                 !Q_irand(
                     0,
                     100 - (gripEnt->NPC->stats.evasion * 10) -
                         (g_spskill->integer *
                          10))) { // a jedi who broke free FIXME: maybe have
  ...
  if (ent->client->NPC_class == CLASS_TAVION ||
    ...
    if (ent->client->NPC_class == CLASS_DESANN) {
      ent->client->ps.forcePowersKnown =
          (1 << FP_LEVITATION) | (1 << FP_PUSH) | (1 << FP_PULL) |
      ...
      ent->client->ps.forcePowersKnown =
          (1 << FP_LEVITATION) | (1 << FP_PUSH) | (1 << FP_PULL) |
          (1 << FP_SABERTHROW) | (1 << FP_SPEED) | (1 << FP_SABER_DEFENSE) |
          (1 << FP_SABER_OFFENSE);
      ent->client->ps.forcePowerLevel[FP_LEVITATION] = FORCE_LEVEL_3;
      ent->client->ps.forcePowerLevel[FP_PUSH] = FORCE_LEVEL_3;
      ent->client->ps.forcePowerLevel[FP_PULL] = FORCE_LEVEL_3;
      ...
      if (ent->client->NPC_class == CLASS_TAVION) {
        ent->client->ps.forcePowersKnown |=
            (1 << FP_LIGHTNING) | (1 << FP_GRIP);
        ent->client->ps.forcePowerLevel[FP_LIGHTNING] = FORCE_LEVEL_2;
        ent->client->ps.forcePowerLevel[FP_GRIP] = FORCE_LEVEL_2;
      }
    } else if (ent->client->NPC_class == CLASS_SHADOWTROOPER) { // Shadow
                                                                // Trooper
      ent->client->ps.forcePowersKnown =
          (1 << FP_LEVITATION) | (1 << FP_PUSH) | (1 << FP_PULL) |
          (1 << FP_SABERTHROW) | (1 << FP_GRIP) | (1 << FP_LIGHTNING) |
          (1 << FP_SPEED) | (1 << FP_SABER_DEFENSE) | (1 << FP_SABER_OFFENSE);
      ...
      ent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] = FORCE_LEVEL_3;
               ...
               ent->client->NPC_class ==
                   CLASS_JEDI) { // Reborn Boss or ally Jedi
      ent->client->ps.forcePowersKnown =
          (1 << FP_LEVITATION) | (1 << FP_PUSH) | (1 << FP_PULL) |
          (1 << FP_SABERTHROW) | (1 << FP_GRIP) | (1 << FP_SPEED) |
      ...
      ent->client->ps.forcePowersKnown =
          ...
          (1 << FP_SABER_DEFENSE) | (1 << FP_SABER_OFFENSE);
      ent->client->ps.forcePowerLevel[FP_PUSH] =
          FORCE_LEVEL_2; // FIXME: maybe make him only use it in defense- to
                         // throw away grenades
      ent->client->ps.forcePowerLevel[FP_SABERTHROW] = FORCE_LEVEL_2;
      ...
      ent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] = FORCE_LEVEL_1;
      ent->client->ps.forcePowerLevel[FP_LEVITATION] = FORCE_LEVEL_2;
    } else if (ent->NPC->rank == RANK_CIVILIAN) { // Grunt (NOTE: grunt turns
                                                  // slower and has less health)
      ent->client->ps.forcePowersKnown =
          (1 << FP_SPEED) | (1 << FP_SABER_DEFENSE) | (1 << FP_SABER_OFFENSE);
      ent->client->ps.forcePowerLevel[FP_LEVITATION] = FORCE_LEVEL_1;
      ent->client->ps.forcePowerLevel[FP_SPEED] = FORCE_LEVEL_1;
    ...
    ent->client->ps.forcePowersKnown =
        ...
        (1 << FP_PUSH) | (1 << FP_PULL) | (1 << FP_TELEPATHY) | (1 << FP_GRIP) |
        (1 << FP_LIGHTNING) | (1 << FP_SABERTHROW) | (1 << FP_SABER_DEFENSE) |
        (1 << FP_SABER_OFFENSE);
    ent->client->ps.forcePower = ent->client->ps.forcePowerMax =
        FORCE_POWER_MAX;
    ent->client->ps.forcePowerRegenDebounceTime = 0;
