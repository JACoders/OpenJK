...
qboolean G_CheckSaberAllyAttackDelay(gentity_t *self, gentity_t *enemy) {
  ...
  if (self->NPC && self->client->leader == player && self->enemy &&
      self->enemy->s.weapon != WP_SABER &&
      self->s.weapon == WP_SABER) { // assisting the player and I'm using a
                                    // saber and my enemy is not
    ...
    int delay = 500;
    if (distance > 2048) { // the farther they are, the shorter the delay
      delay = 5000 - floor(distance); //(6-g_spskill->integer));
      if (delay < 500) {
        delay = 500;
      }
    } else {                       // the close they are, the shorter the delay
...
void G_SetEnemy(gentity_t *self, gentity_t *enemy) {
  ...
  if (self->enemy == NULL) {
    ...
    if (self->client && self->client->NPC_class == CLASS_SABOTEUR) {
      ...
      TIMER_Set(self, "decloakwait",
                3000); // Wait 3 sec before decloak and attack
    }

    // Special case- if player is being hunted by his own people, set the
    // player's team to team_free
    if (self->client->playerTeam == TEAM_PLAYER && enemy->s.number == 0 &&
        enemy->client &&
        enemy->client->playerTeam ==
            TEAM_PLAYER) { // make the player "evil" so that everyone goes after
                           // him
    ...
    if (G_ActivateBehavior(self, BSET_ANGER)) {
    } else if (self->client && self->client->NPC_class == CLASS_KYLE &&
               self->client->leader == player &&
               !TIMER_Done(
                   self, "kyleAngerSoundDebounce")) { // don't yell that you
                                                      // have an enemy more than
      ...
      if (self->forcePushTime < level.time) // not currently being pushed
      {
        if (!G_TeamEnemy(self) &&
            self->client->NPC_class !=
                CLASS_BOBAFETT) { // team did not have an enemy previously
          if (self->NPC && self->client->playerTeam == TEAM_PLAYER &&
            ...
            if (Q_irand(
                    0,
                    1)) { // hell, we're loading them, might as well use them!
      ...
      if (event) { // yell
        if (self->client && self->client->NPC_class == CLASS_KYLE &&
            self->client->leader ==
                player) { // don't yell that you have an enemy more than once
                          // every 4-8 seconds
    ...
    if (self->s.weapon == WP_BLASTER || self->s.weapon == WP_REPEATER ||
      ...
      if (self->client->playerTeam == TEAM_PLAYER) {
        ...
        G_AimSet(self, Q_irand(self->NPC->stats.aim -
                                   (maxErr * (3 - g_spskill->integer)),
                               self->NPC->stats.aim -
                                   (minErr * (3 - g_spskill->integer))));
      }
    }
    ...
    if (Q_stricmp("desperado", self->NPC_type) != 0 &&
      ...
      if (!(self->client->ps.eFlags &
            EF_FORCE_GRIPPED)) { // gripped people can't call for help
        G_AngerAlert(self);
      }
    }

    if (!G_CheckSaberAllyAttackDelay(self,
                                     enemy)) { // not a saber ally holding back
...
void ChangeWeapon(gentity_t *ent, int newWeapon) {
  ...
  case WP_CONCUSSION:
    ent->NPC->aiFlags &= ~NPCAI_BURST_WEAPON;
    if (ent->NPC->scriptFlags & SCF_ALT_FIRE) { // beam
  ...
  case WP_EMPLACED_GUN:
    ...
    if (ent->client && ent->client->NPC_class == CLASS_REELO) {
      ...
      if (ent->owner) // if we have an owner, it should be the chair at this
                      // point...so query the chair for its shot debounce times,
                      // etc.
      {
        if (g_spskill->integer == 0) {
...
void NPC_ChangeWeapon(int newWeapon) {
  ...
  if (changing && NPC->client->ps.weapon != WP_NONE) {
    if (NPC->client->ps.weapon == WP_SABER) {
      ...
      G_CreateG2AttachedWeaponModel(
          NPC, weaponData[NPC->client->ps.weapon].weaponMdl, NPC->handRBolt, 0);
    }
  }
}
/*
...
void NPC_ApplyWeaponFireDelay(void) {
  ...
  case WP_TUSKEN_STAFF:
    if (!PM_DroidMelee(
            client->NPC_class)) { // FIXME: should be unique per melee anim
  ...
  case WP_TUSKEN_RIFLE:
    if (!(NPCInfo->scriptFlags &
          SCF_ALT_FIRE)) { // FIXME: should be unique per melee anim
      client->fireDelay = 300;
    }
    break;
...
void ShootThink(void) {
  ...
  NPCInfo->currentAmmo =
      client->ps.ammo[weaponData[client->ps.weapon].ammoIndex]; // checkme
  ...
  if (NPCInfo->aiFlags & NPCAI_BURST_WEAPON) {
    ...
    if (!delay) {
      ...
      if (client->ps.weapon == WP_EMPLACED_GUN) {
        if (NPC->owner) // try and get the debounce values from the chair if we
                        // can
        {
          if (g_spskill->integer == 0) {
...
void WeaponThink(qboolean inCombat) {
  ...
  if (NPC->client && (NPC->client->ps.powerups[PW_CLOAKED] ||
  ...
  if (NPC->client->ps.ammo[weaponData[client->ps.weapon].ammoIndex] <
      weaponData[client->ps.weapon].energyPerShot) {
    Add_Ammo(NPC, client->ps.weapon,
             weaponData[client->ps.weapon].energyPerShot * 10);
  } else if (NPC->client->ps.ammo[weaponData[client->ps.weapon].ammoIndex] <
             weaponData[client->ps.weapon].altEnergyPerShot) {
    Add_Ammo(NPC, client->ps.weapon,
             weaponData[client->ps.weapon].altEnergyPerShot * 5);
  }
...
float NPC_MaxDistSquaredForWeapon(void) {
  ...
  case WP_SABER:
    if (NPC->client &&
        NPC->client->ps.SaberLength()) { // FIXME: account for whether enemy and
                                         // I are heading towards each other!
    ...
    if (newenemy->client && !(newenemy->flags & FL_NOTARGET) &&
      ...
      if (newenemy->health > 0) {
        ...
        { // FIXME:  check for range and FOV or vis?
          if (newenemy != NPC->lastEnemy) { // Make sure we're not just going
                                            // back and forth here
            if (gi.inPVS(newenemy->currentOrigin, NPC->currentOrigin)) {
              ...
              if (!failed) {
                VectorSubtract(closestTo->currentOrigin,
                               newenemy->currentOrigin, diff);
    ...
    if (newenemy != NPC &&
      ...
      if (newenemy->health > 0) {
        if ((newenemy->client && NPC_ValidEnemy(newenemy)) ||
            (!newenemy->client &&
             newenemy->noDamageTeam ==
                 enemyTeam)) { // FIXME:  check for range and FOV or vis?
          if (NPC->client->playerTeam == TEAM_PLAYER &&
              enemyTeam ==
                  TEAM_PLAYER) { // player allies turning on ourselves?  How?
            if (newenemy->s.number) { // only turn on the player, not other
                                      // player allies
              continue;
            }
          }
...
gentity_t *NPC_PickAlly(qboolean facingEachOther, float range,
  ...
  for (entNum = 0; entNum < globals.num_entities; entNum++) {
    ...
    if (ally->client) {
      if (ally->health > 0) {
        if (ally->client &&
            (ally->client->playerTeam == NPC->client->playerTeam ||
...
gentity_t *NPC_CheckEnemy(qboolean findNew, qboolean tooFarOk,
  ...
  if (NPC->enemy) {
    if (!NPC->enemy->inuse) //|| NPC->enemy == NPC )//wtf?  NPCs should never
  ...
  if (NPC->client->NPC_class == CLASS_KYLE && NPC->client->leader &&
  ...
  if (NPC->svFlags & SVF_LOCKEDENEMY) { // keep this enemy until dead
    if (NPC->enemy) {
      if ((!NPC->NPC && !(NPC->svFlags & SVF_NONNPC_ENEMY)) ||
          NPC->enemy->health >
              0) { // Enemy never had health (a train or info_not_null, etc) or
                   // did and is now dead (NPCs, turrets, etc)
        return NULL;
      }
    }
      ...
      } else if (!tooFarOk) // FIXME: don't need this extra bool any more
      {
        if (setEnemy) {
          G_ClearEnemy(NPC);
                         ...
                         NPC->enemy->currentOrigin)) { // FIXME: should this be
  ...
  if (!NPC->enemy || (NPC->enemy && NPC->enemy->health <= 0) ||
    ...
    if (NPC->client->enemyTeam != TEAM_NEUTRAL) {
      ...
      newEnemy = NPC_PickEnemy(
          closestTo, NPC->client->enemyTeam, qtrue, qfalse,
    ...
    // FIXME: if we can't find any at all, go into INdependant NPC AI, pursue
    // and kill
  }
...
qboolean NPC_ClearShot(gentity_t *ent) {
      ...
      NPC->s.weapon == WP_BLASTER_PISTOL) // any other guns to check for?
  {
    ...
    gi.trace(&tr, muzzle, mins, maxs, ent->currentOrigin, NPC->s.number,
             MASK_SHOT, (EG2_Collision)0, 0);
  } else {
    gi.trace(&tr, muzzle, NULL, NULL, ent->currentOrigin, NPC->s.number,
             MASK_SHOT, (EG2_Collision)0, 0);
  }
...
qboolean NPC_CheckCanAttack(float attack_scale, qboolean stationary) {
  ...
  enemyVisibility =
      NPC_CheckVisibility(NPC->enemy, CHECK_360 | CHECK_FOV); ////CHECK_PVS|
    ...
    if (attack_ok) {
      ...
      */
      ...
      {//no, so shoot for somewhere between the head and torso
              ...
              enemy_org[0] += 0.3*Q_flrand(NPC->enemy->mins[0],
      NPC->enemy->maxs[0]); enemy_org[1] += 0.3*Q_flrand(NPC->enemy->mins[1],
      NPC->enemy->maxs[1]); enemy_org[2] -=
      NPC->enemy->maxs[2]*Q_flrand(0.0f, 1.0f);

              attack_scale *= 0.75;
              gi.trace ( &tr, muzzle, NULL, NULL, enemy_org, NPC->s.number,
      ...
      if (!dead_on) { // We're not going to hit him directly, try a suppressing
        ...
        if (traceEnt && (traceEnt->health <= 30 ||
          ...
          if (traceEnt->e_DieFunc == dieF_ExplodeDeath_Wait &&
              traceEnt->splashDamage) { // going to explode, don't shoot if
                                        // close to self
...
/*QUAKED point_combat (0.7 0 0.7) (-20 -20 -24) (20 20 45) DUCK FLEE INVESTIGATE
...
void CP_FindCombatPointWaypoints(void) {
  for (int i = 0; i < level.numCombatPoints; i++) {
    level.combatPoints[i].waypoint =
        NAV::GetNearestNode(level.combatPoints[i].origin);
...
static int NPC_CollectCombatPoints(const vec3_t origin, const float radius,
                                   combatPoint_m &points, const int flags) {
...
int NPC_FindCombatPoint(const vec3_t position, const vec3_t avoidPosition,
                        vec3_t destPosition, const int flags, float avoidDist,
  ...
  bool useHorizDist =
      (NPC->s.weapon == WP_THERMAL) || (flags & CP_HORZ_DIST_COLL);
  ...
  if ((flags & CP_NO_PVS) ||
      (flags & CP_TRYFAR)) { // much larger radius since most will be dropped?
  ...
  for (cpi = points.begin(); cpi != points.end(); ++cpi) {
    ...
    distSqPointToEnemy =
        DistanceSquared(level.combatPoints[i].origin, enemyPosition);
    distSqPointToEnemyHoriz =
        DistanceHorizontalSquared(level.combatPoints[i].origin, enemyPosition);
    distSqPointToEnemyCheck =
        (useHorizDist) ? (distSqPointToEnemyHoriz) : (distSqPointToEnemy);

    distSqNPCToEnemy = DistanceSquared(NPC->currentOrigin, enemyPosition);
    distSqNPCToEnemyHoriz =
        DistanceHorizontalSquared(NPC->currentOrigin, enemyPosition);
    distSqNPCToEnemyCheck =
        (useHorizDist) ? (distSqNPCToEnemyHoriz) : (distSqNPCToEnemy);

    // Ignore points that are farther than currently located
    if ((flags & CP_APPROACH_ENEMY) &&
        (distSqPointToEnemyCheck > distSqNPCToEnemyCheck)) {
      continue;
    }
    ...
    // Avoid this position?
    if (avoidPosition && !(flags & CP_AVOID_ENEMY) && (flags & CP_AVOID) &&
        (DistanceSquared(level.combatPoints[i].origin, avoidPosition) <
         avoidDist)) {
      continue;
    }
    ...
    if ((flags & CP_HAS_ROUTE) &&
        !NAV::InSameRegion(NPC, level.combatPoints[i].origin)) {
      continue;
    }
    ...
    if (flags & CP_AVOID_ENEMY) {
      ...
      // otherwise, if currently safe and the path is not safe, ignore this
      // point
      if (distSqNPCToEnemy > (avoidDist) &&
          !NAV::SafePathExists(position, level.combatPoints[i].origin,
                               enemyPosition, avoidDist)) {
        continue;
      }
    }
    ...
    gi.trace(&tr, level.combatPoints[i].origin, NPC->mins, NPC->maxs,
             level.combatPoints[i].origin, NPC->s.number, NPC->clipmask,
             (EG2_Collision)0, 0);
    ...
    if (NPC->enemy) {
      ...
      if ((flags & CP_COVER) &&
          NPC_ClearLOS(level.combatPoints[i].origin, NPC->enemy) == qtrue) {
        continue;
      }
    }
...
int NPC_FindCombatPointRetry(const vec3_t position, const vec3_t avoidPosition,
                             vec3_t destPosition, int *cpFlags, float avoidDist,
                             const int ignorePoint) {
  int cp = -1;
  cp = NPC_FindCombatPoint(position, avoidPosition, destPosition, *cpFlags,
                           avoidDist, ignorePoint);
  while (cp == -1 && (*cpFlags & ~CP_HAS_ROUTE) !=
                         CP_ANY) { // start "OR"ing out certain flags to see if
    ...
    if (*cpFlags & CP_INVESTIGATE) { // don't need to investigate
      ...
      *cpFlags &= ~CP_FLANK;
               ...
               CP_SAFE) { // don't need one that hasn't been shot at recently
      *cpFlags &= ~CP_SAFE;
    ...
    //	}
      ...
      *cpFlags &= ~CP_AVOID;
    } else if (*cpFlags & CP_SHORTEST_PATH) { // okay, don't need the one with
                                              // the shortest path
      *cpFlags &= ~CP_SHORTEST_PATH;
    } else { // screw it, we give up!
      return -1;
      /*
      if ( *cpFlags & CP_HAS_ROUTE )
      {//NOTE: this is really an absolute worst case scenario - will go to the
      first cp on the map! *cpFlags &= ~CP_HAS_ROUTE;
      }
      else
      {//NOTE: this is really an absolute worst case scenario - will go to the
      first cp on the map! *cpFlags = CP_ANY;
      }
      */
    }
    // now try again
  ...
  for (i = 0; i < globals.num_entities; i++) {
    ...
    if (CheckItemCanBePickedUpByNPC(found, NPC)) {
      if (gi.inPVS(found->currentOrigin, NPC->currentOrigin)) {
        ...
        if (dist < bestDist) {
          if (NAV::InSameRegion(NPC, found)) { // can nav to it
            bestDist = dist;
            bestFound = found;
          }
        }
...
void NPC_CheckGetNewWeapon(void) {
  if (NPC->client &&
      !G_CanPickUpWeapons(NPC)) { // this NPC can't pick up weapons...
    return;
  }
  if (NPC->s.weapon == WP_NONE &&
      NPC->enemy) { // if running away because dropped weapon...
...
void NPC_AimAdjust(int change) {
  ...
  if (TIMER_Done(NPC, "aimDebounce")) {
    ...
    if (NPCInfo->currentAim >
        NPCInfo->stats.aim) { // can never be better than max aim
