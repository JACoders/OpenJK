...
void R2D2_PartsMove(void) {
	...
	if (TIMER_Done(NPCS.NPC, "eyeDelay")) {
		...
		if (NPC->genericBone1)
		{
			trap->G2API_SetBoneAnglesIndex( &NPC->ghoul2[NPC->playerModel], NPC->genericBone1, NPC->pos1, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
		NEGATIVE_Z, NULL );
		}
		*/
...
void Droid_Spin(void) {
	...
	if (NPCS.NPC->client->NPC_class == CLASS_R5D2 || NPCS.NPC->client->NPC_class == CLASS_R2D2) {
		// No head?
