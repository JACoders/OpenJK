...
void trap_LocateGameData(sharedEntity_t *gEnts, int numGEntities,
                         int sizeofGEntity_t, playerState_t *clients,
                         int sizeofGClient) {
  Q_syscall(G_LOCATE_GAME_DATA, gEnts, numGEntities, sizeofGEntity_t, clients,
            sizeofGClient);
}
...
void trap_SendServerCommand(int clientNum, const char *text) {
  if (strlen(text) > 1022) {
    G_SecurityLogPrintf(
        "trap_SendServerCommand( %d, ... ) length exceeds 1022.\n", clientNum);
...
void trap_Trace(trace_t *results, const vec3_t start, const vec3_t mins,
                ...
                int contentmask) {
  Q_syscall(G_TRACE, results, start, mins, maxs, end, passEntityNum,
            contentmask, 0, 10);
}
void trap_G2Trace(trace_t *results, const vec3_t start, const vec3_t mins,
                  const vec3_t maxs, const vec3_t end, int passEntityNum,
                  int contentmask, int g2TraceType, int traceLod) {
  Q_syscall(G_G2TRACE, results, start, mins, maxs, end, passEntityNum,
            contentmask, g2TraceType, traceLod);
}
...
void trap_TraceCapsule(trace_t *results, const vec3_t start, const vec3_t mins,
                       const vec3_t maxs, const vec3_t end, int passEntityNum,
                       int contentmask) {
  Q_syscall(G_TRACECAPSULE, results, start, mins, maxs, end, passEntityNum,
            contentmask, 0, 10);
}
...
int trap_SP_GetStringTextString(const char *text, char *buffer,
                                int bufferLength) {
...
void trap_ICARUS_SetVar(int taskID, int entID, const char *type_name,
                        const char *data) {
...
int trap_Nav_GetNearestNode(sharedEntity_t *ent, int lastID, int flags,
                            int targetID) {
...
qboolean trap_Nav_RouteBlocked(int startID, int testEdgeID, int endID,
                               int rejectRank) {
  return Q_syscall(G_NAV_ROUTEBLOCKED, startID, testEdgeID, endID, rejectRank);
}
int trap_Nav_GetBestNodeAltRoute(int startID, int endID, int *pathCost,
                                 int rejectID) {
  return Q_syscall(G_NAV_GETBESTNODEALTROUTE, startID, endID, pathCost,
                   rejectID);
}
...
int trap_Nav_GetBestPathBetweenEnts(sharedEntity_t *ent, sharedEntity_t *goal,
                                    int flags) {
...
int trap_AAS_AreaTravelTimeToGoalArea(int areanum, vec3_t origin,
  ...
  return Q_syscall(BOTLIB_AAS_AREA_TRAVEL_TIME_TO_GOAL_AREA, areanum, origin,
...
int trap_AAS_PredictRoute(void *route, int areanum, vec3_t origin,
  ...
  return Q_syscall(BOTLIB_AAS_PREDICT_ROUTE, route, areanum, origin,
                   goalareanum, travelflags, maxareas, maxtime, stopevent,
                   stopcontents, stoptfl, stopareanum);
}
int trap_AAS_AlternativeRouteGoals(vec3_t start, int startareanum, vec3_t goal,
                                   int goalareanum, int travelflags,
                                   void *altroutegoals, int maxaltroutegoals,
                                   int type) {
  return Q_syscall(BOTLIB_AAS_ALTERNATIVE_ROUTE_GOAL, start, startareanum, goal,
...
int trap_AAS_PredictClientMovement(void *move, int entnum, vec3_t origin,
  ...
  return Q_syscall(BOTLIB_AAS_PREDICT_CLIENT_MOVEMENT, move, entnum, origin,
...
float trap_Characteristic_BFloat(int character, int index, float min,
                                 float max) {
  byteAlias_t fi;
  fi.i = Q_syscall(BOTLIB_AI_CHARACTERISTIC_BFLOAT, character, index,
...
int trap_Characteristic_BInteger(int character, int index, int min, int max) {
  return Q_syscall(BOTLIB_AI_CHARACTERISTIC_BINTEGER, character, index, min,
                   max);
}
...
void trap_BotInitialChat(int chatstate, char *type, int mcontext, char *var0,
  ...
  Q_syscall(BOTLIB_AI_INITIAL_CHAT, chatstate, type, mcontext, var0, var1, var2,
...
int trap_BotReplyChat(int chatstate, char *message, int mcontext, int vcontext,
  ...
  return Q_syscall(BOTLIB_AI_REPLY_CHAT, chatstate, message, mcontext, vcontext,
...
int trap_BotChooseLTGItem(int goalstate, vec3_t origin, int *inventory,
                          int travelflags) {
  return Q_syscall(BOTLIB_AI_CHOOSE_LTG_ITEM, goalstate, origin, inventory,
                   travelflags);
}
int trap_BotChooseNBGItem(int goalstate, vec3_t origin, int *inventory,
                          int travelflags, void *ltg, float maxtime) {
  return Q_syscall(BOTLIB_AI_CHOOSE_NBG_ITEM, goalstate, origin, inventory,
...
int trap_BotItemGoalInVisButNotVisible(int viewer, vec3_t eye,
  ...
  return Q_syscall(BOTLIB_AI_ITEM_GOAL_IN_VIS_BUT_NOT_VISIBLE, viewer, eye,
                   viewangles, goal);
}
...
void trap_BotSetAvoidGoalTime(int goalstate, int number, float avoidtime) {
  Q_syscall(BOTLIB_AI_SET_AVOID_GOAL_TIME, goalstate, number,
            PASSFLOAT(avoidtime));
}
...
void trap_BotAddAvoidSpot(int movestate, vec3_t origin, float radius,
                          int type) {
  Q_syscall(BOTLIB_AI_ADD_AVOID_SPOT, movestate, origin, PASSFLOAT(radius),
            type);
}
void trap_BotMoveToGoal(void *result, int movestate, void *goal,
                        int travelflags) {
  Q_syscall(BOTLIB_AI_MOVE_TO_GOAL, result, movestate, goal, travelflags);
}
int trap_BotMoveInDirection(int movestate, vec3_t dir, float speed, int type) {
  return Q_syscall(BOTLIB_AI_MOVE_IN_DIRECTION, movestate, dir,
...
int trap_BotMovementViewTarget(int movestate, void *goal, int travelflags,
  ...
  return Q_syscall(BOTLIB_AI_MOVEMENT_VIEW_TARGET, movestate, goal, travelflags,
                   PASSFLOAT(lookahead), target);
}
int trap_BotPredictVisiblePosition(vec3_t origin, int areanum, void *goal,
                                   int travelflags, vec3_t target) {
  return Q_syscall(BOTLIB_AI_PREDICT_VISIBLE_POSITION, origin, areanum, goal,
                   travelflags, target);
}
...
int trap_GeneticParentsAndChildSelection(int numranks, float *ranks,
                                         ...
                                         int *child) {
  return Q_syscall(BOTLIB_AI_GENETIC_PARENTS_AND_CHILD_SELECTION, numranks,
...
qboolean trap_G2API_GetBoltMatrix(void *ghoul2, const int modelIndex,
                                  ...
                                  vec3_t scale) {
  return (qboolean)(Q_syscall(G_G2_GETBOLT, ghoul2, modelIndex, boltIndex,
...
qboolean trap_G2API_GetBoltMatrix_NoReconstruct(
  ...
  return (qboolean)(Q_syscall(G_G2_GETBOLT_NOREC, ghoul2, modelIndex, boltIndex,
                              matrix, angles, position, frameNum, modelList,
                              scale));
}
qboolean trap_G2API_GetBoltMatrix_NoRecNoRot(
    ...
    const vec3_t angles, const vec3_t position, const int frameNum,
    qhandle_t *modelList, vec3_t scale) {
  return (qboolean)(Q_syscall(G_G2_GETBOLT_NOREC_NOROT, ghoul2, modelIndex,
...
int trap_G2API_InitGhoul2Model(void **ghoul2Ptr, const char *fileName,
                               ...
                               int lodBias) {
  return Q_syscall(G_G2_INITGHOUL2MODEL, ghoul2Ptr, fileName, modelIndex,
...
qboolean trap_G2API_SetBoneAngles(void *ghoul2, int modelIndex,
                                  ...
                                  int currentTime) {
  return (Q_syscall(G_G2_ANGLEOVERRIDE, ghoul2, modelIndex, boneName, angles,
...
qboolean trap_G2API_SetBoneAnim(void *ghoul2, const int modelIndex,
  ...
  return Q_syscall(G_G2_PLAYANIM, ghoul2, modelIndex, boneName, startFrame,
...
qboolean trap_G2API_GetBoneAnim(void *ghoul2, const char *boneName,
  ...
  return Q_syscall(G_G2_GETBONEANIM, ghoul2, boneName, currentTime,
...
void trap_G2API_CollisionDetect(CollisionRecord_t *collRecMap, void *ghoul2,
  ...
  Q_syscall(G_G2_COLLISIONDETECT, collRecMap, ghoul2, angles, position,
...
void trap_G2API_CollisionDetectCache(CollisionRecord_t *collRecMap,
                                     ...
                                     float fRadius) {
  Q_syscall(G_G2_COLLISIONDETECTCACHE, collRecMap, ghoul2, angles, position,
            frameNumber, entNum, rayStart, rayEnd, scale, traceFlags, useLod,
            PASSFLOAT(fRadius));
}
...
int trap_G2API_GetSurfaceRenderStatus(void *ghoul2, const int modelIndex,
                                      const char *surfaceName) {
  return Q_syscall(G_G2_GETSURFACERENDERSTATUS, ghoul2, modelIndex,
                   surfaceName);
}
...
qboolean trap_G2API_RagPCJGradientSpeed(void *ghoul2, const char *boneName,
                                        const float speed) {
  return Q_syscall(G_G2_RAGPCJGRADIENTSPEED, ghoul2, boneName,
                   PASSFLOAT(speed));
}
...
qboolean trap_G2API_GetRagBonePos(void *ghoul2, const char *boneName,
                                  ...
                                  vec3_t entScale) {
  return Q_syscall(G_G2_GETRAGBONEPOS, ghoul2, boneName, pos, entAngles, entPos,
                   entScale);
}
...
qboolean trap_G2API_SetBoneIKState(void *ghoul2, int time, const char *boneName,
  ...
  return Q_syscall(G_G2_SETBONEIKSTATE, ghoul2, time, boneName, ikState,
                   params);
}
...
qboolean SVSyscall_EntityContact(const vec3_t mins, const vec3_t maxs,
...
void SVSyscall_Trace(trace_t *results, const vec3_t start, const vec3_t mins,
                     const vec3_t maxs, const vec3_t end, int passEntityNum,
                     int contentmask, int capsule, int traceFlags, int useLod) {
  if (capsule)
    trap_TraceCapsule(results, start, mins, maxs, end, passEntityNum,
                      contentmask);
  else if (traceFlags)
    trap_G2Trace(results, start, mins, maxs, end, passEntityNum, contentmask,
                 traceFlags, useLod);
  else
