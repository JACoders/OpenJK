...
class CMapBlock {
  ...
  public:
	...
	bool bOwnsNode(void *node) { return ((((SMapNode *)node) >= &mNodes[0]) && (((SMapNode *)node) < &mNodes[MAPBLOCK_SIZE_NODES])); }
};
...
class CPool {
  ...
  public:
	...
	CPool(void) : mNextStringId(1), mLastBlockNum(-1) { memset(gCharPtrs, 0, MAX_HSTRINGS * 4); }
