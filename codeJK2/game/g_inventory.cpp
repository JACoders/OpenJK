...
int INV_GoodieKeyCheck(gentity_t *target) {
  ...
  if (target->client->ps.inventory[INV_GOODIE_KEY]) { // found a key
...
qboolean INV_SecurityKeyGive(gentity_t *target, const char *keyname) {
  ...
  for (int i = 0; i <= 4; i++) {
    if (target->client->ps.security_key_message[i][0] ==
        '\0') { // fill in the first empty slot we find with this key
...
void INV_SecurityKeyTake(gentity_t *target, char *keyname) {
  ...
  for (int i = 0; i <= 4; i++) {
    ...
    /*
    //don't do this because we could have removed one that's between 2 valid
    ones else
    {
            break;
    }
...
qboolean INV_SecurityKeyCheck(gentity_t *target, char *keyname) {
  ...
  for (int i = 0; i <= 4; i++) {
    if (target->client->ps.inventory[INV_SECURITY_KEY] &&
        target->client->ps.security_key_message[i]) {
      if (!Q_stricmp(keyname, target->client->ps.security_key_message[i])) {
        return (qtrue);
      }
    }
