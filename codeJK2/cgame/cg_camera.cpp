...
extern void WP_SaberCatch(gentity_t *self, gentity_t *saber,
                          qboolean switchToSaber);
...
void CGCam_Enable(void) {
  ...
  if (&g_entities[0] && g_entities[0].client) {
    ...
    if (g_entities[0].client->ps.saberInFlight &&
        g_entities[0].client->ps.saberActive) { // saber is out
      gentity_t *saberent =
          &g_entities[g_entities[0].client->ps.saberEntityNum];
      if (saberent) {
        WP_SaberCatch(&g_entities[0], saberent, qfalse);
      }
    }

    for (int i = 0; i < NUM_FORCE_POWERS;
         i++) { // deactivate any active force powers
      g_entities[0].client->ps.forcePowerDuration[i] = 0;
      extern void WP_ForcePowerStop(gentity_t * self, forcePowers_t forcePower);
      if (g_entities[0].client->ps.forcePowerDuration[i] ||
          (g_entities[0].client->ps.forcePowersActive & (1 << i))) {
...
void CGCam_Disable(void) {
  ...
  if (cg_skippingcin.integer) { // We're skipping the cinematic and it's over
                                // now
...
void CGCam_Follow(const char *cameraGroup, float speed, float initLerp) {
  ...
  // NOTE: if this interrupts a pan before it's done, need to copy the
  // cg.refdef.viewAngles to the camera.angles!
  client_camera.info_state |= CAMERA_FOLLOWING;
  client_camera.info_state &= ~CAMERA_PANNING;

  // NULL terminate last char in case they type a name too long
  Q_strncpyz(client_camera.cameraGroup, cameraGroup,
             sizeof(client_camera.cameraGroup));
...
Q3_CameraAutoAim
  ...
  misc_camera_focus can be on a track that stays closest to it's subjects on
that path (like Q3_CameraAutoTrack) or is can simply always put itself between
it's subjects. misc_camera_focus can also set FOV/camera dist needed to keep the
subjects in frame
-------------------------
*/
...
void CGCam_Track(const char *trackName, float speed, float initLerp) {
  ...
  // NOTE: if this interrupts a move before it's done, need to copy the
  // cg.refdef.vieworg to the camera.origin! This will find a path_corner now,
  // not a misc_camera_track
  ...
  */
  ...
  // Set a moveDir
  VectorSubtract(trackEnt->currentOrigin, client_camera.origin,
                 client_camera.moveDir);
  ...
  if (!client_camera.trackInitLerp) { // want to snap to first position
    ...
    // Possible that track has no next point, in which case we won't be moving
    // anyway
    if (trackEnt->target && trackEnt->target[0]) {
      gentity_t *newTrackEnt = G_Find(NULL, FOFS(targetname), trackEnt->target);
      if (newTrackEnt) {
        VectorSubtract(newTrackEnt->currentOrigin, client_camera.origin,
                       client_camera.moveDir);
      }
    }
  }
...
Q3_CameraAutoTrack
  ...
  Keeps camera a certain distance from target entity but on the specified
CameraPath The distance can be set in a script or derived from a
misc_camera_focus. Dist will interpolate when changed, can also set
acceleration/deceleration values. FOV will also interpolate.

  CameraPath might be a MAX path or perhaps a series of path_corners on the map
itself
-------------------------
*/
...
void CGCam_FollowUpdate(void) {
  vec3_t center, dir, cameraAngles, vec,
      focus[MAX_CAMERA_GROUP_SUBJECTS]; // No more than 16 subjects in a
                                        // cameraGroup
  gentity_t *from = NULL;
  ...
  if (client_camera.cameraGroup[0]) {
    ...
    while (NULL != (from = G_Find(from, FOFS(cameraGroup),
      ...
      {//This is the misc_camera_focus, we'll be removing this ent altogether
      eventually continue;
      }
      */
      ...
      if (num_subjects >= MAX_CAMERA_GROUP_SUBJECTS) {
        gi.Printf(S_COLOR_RED
                  "ERROR: Too many subjects in shot composition %s\n",
                  client_camera.cameraGroup);
        break;
      }
      ...
      focused = qfalse;
      if (from->client && client_camera.cameraGroupTag[0] &&
          fromCent->gent->ghoul2.size()) {
        int newBolt =
            gi.G2API_AddBolt(&fromCent->gent->ghoul2[from->playerModel],
                             client_camera.cameraGroupTag);
        if (newBolt != -1) {
          mdxaBone_t boltMatrix;
          vec3_t fromAngles = {0, from->client->ps.legsYaw, 0};

          gi.G2API_GetBoltMatrix(fromCent->gent->ghoul2, from->playerModel,
                                 newBolt, &boltMatrix, fromAngles,
      ...
      if (!focused) {
        ...
        { // use interpolated origin?
          if (!VectorCompare(
                  vec3_origin,
                  fromCent
                      ->lerpOrigin)) { // hunh?  Somehow we've never seen this
                                       // gentity on the client, so there is no
                                       // lerpOrigin, so cheat over to the game
                                       // and use the currentOrigin
        ...
        // FIXME: make a list here of their s.numbers instead so we can do other
        // stuff with the list below
        if (from->client) { // Track to their eyes - FIXME: maybe go off a tag?
          // FIXME:
          // Based on FOV and distance to subject from camera, pick the point
          // that keeps eyes 3/4 up from bottom of screen... what about bars?
...
#ifndef FINAL_BUILD
      gi.Printf(S_COLOR_RED
                "ERROR: Camera Focus unable to locate cameragroup: %s\n",
                client_camera.cameraGroup);
#endif
      return;
    }
  ...
  */
  ...
  VectorSubtract(center, cg.refdef.vieworg,
                 dir); // can't use client_camera.origin because it's not
                       // updated until the end of the move.
  ...
  if (client_camera.followInitLerp) { // Lerping
    ...
    for (i = 0; i < 3; i++) {
      ...
      cameraAngles[i] = AngleNormalize180(
          client_camera.angles[i] +
          frac * AngleNormalize180(cameraAngles[i] - client_camera.angles[i]));
      cameraAngles[i] = AngleNormalize180(cameraAngles[i]);
    }
#if 0
		Com_Printf( "%s\n", vtos(cameraAngles) );
#endif
  } else { // Snapping, should do this first time if
           // follow_lerp_to_start_duration is zero
    // will lerp from this point on
    client_camera.followInitLerp = qtrue;
    for (i = 0; i < 3;
         i++) { // normalize so that when we start lerping, it doesn't freak out
      cameraAngles[i] = AngleNormalize180(cameraAngles[i]);
    }
    // So tracker doesn't move right away thinking the first angle change
    // is the subject moving... FIXME: shouldn't set this until lerp done OR
    // snapped?
...
void CGCam_TrackEntUpdate(void) { // FIXME: only do every 100 ms
  ...
  if (client_camera.trackEntNum >= 0 &&
      client_camera.trackEntNum <
          ENTITYNUM_WORLD) { // We're already heading to a path_corner
  ...
  if (trackEnt && reached) {
    ...
    if (trackEnt->target && trackEnt->target[0]) { // Find our next path_corner
      ...
      if (newTrackEnt) {
        if (newTrackEnt->radius <
            0) { // Don't bother trying to maintain a radius
...
void CGCam_TrackUpdate(void) {
  ...
  if (client_camera.distance && client_camera.info_state & CAMERA_FOLLOWING) {
    ...
    if (!client_camera.distanceInitLerp) {
      ...
      VectorMA(client_camera.subjectPos, client_camera.distance, vec,
               client_camera.origin);
      // Snap to first time only
      client_camera.distanceInitLerp = qtrue;
      return;
    } else if (client_camera.subjectSpeed >
               0.05f) { // Don't start moving until subject moves
      ...
      // desiredSpeed = aimCent->gent->speed + (adjust *
      // cg.frametime/100.0f);//cg.frameInterpolation);
      desiredSpeed =
          (adjust); // * cg.frametime/100.0f);//cg.frameInterpolation);
      ...
      } else if (client_camera.speed - desiredSpeed >
                 max_allowed_accel) { // new speed much slower, slow down at max
                                      // accel
        client_camera.speed -= max_allowed_accel;
      } else if (desiredSpeed - client_camera.speed >
                 max_allowed_accel) { // new speed much faster, speed up at max
                                      // accel
        client_camera.speed += max_allowed_accel;
  ...
  /*if(slowDown)
  {
          VectorMA( client_camera.origin, client_camera.speed * goalDist/100.0f
  * cg.frametime/100.0f, client_camera.moveDir, trackPos );
  }
  else*/
  {
    VectorMA(client_camera.origin, client_camera.speed * cg.frametime / 100.0f,
             client_camera.moveDir, trackPos);
  }

  // FIXME: Implement
  // Need to find point on camera's path that is closest to the desired distance
  // from subject OR: Need to intelligently pick this desired distance based on
  // framing...
...
void CGCam_UpdateBarFade(void) {
  if (client_camera.bar_time + BAR_DURATION < cg.time) {
    ...
    client_camera.bar_alpha =
        client_camera.bar_alpha_source +
        ((client_camera.bar_alpha_dest - client_camera.bar_alpha_source) /
         BAR_DURATION) *
            (cg.time - client_camera.bar_time);
    ;
    client_camera.bar_height =
        client_camera.bar_height_source +
        ((client_camera.bar_height_dest - client_camera.bar_height_source) /
         BAR_DURATION) *
            (cg.time - client_camera.bar_time);
    ;
  }
...
void CGCam_Update(void) {
  ...
  if (client_camera.info_state & CAMERA_ZOOMING) {
    ...
    if (client_camera.FOV_time + client_camera.FOV_duration < cg.time) {
      ...
      actualFOV_X =
          client_camera.FOV + (((client_camera.FOV2 - client_camera.FOV)) /
                               client_camera.FOV_duration) *
  ...
  if ((client_camera.info_state & CAMERA_ROFFING) &&
    ...
    for (i = 0; i < 3; i++) {
      cg.refdefViewAngles[i] =
          client_camera.angles[i] +
          (client_camera.angles2[i] / client_camera.pan_duration) *
              (cg.time - client_camera.pan_time);
    }
  } else if (client_camera.info_state & CAMERA_PANNING) {
    // Note: does not actually change the camera's angles until the pan time is
    // done!
    if (client_camera.pan_time + client_camera.pan_duration <
        cg.time) { // finished panning
      for (i = 0; i < 3; i++) {
        client_camera.angles[i] = AngleNormalize360(
            (client_camera.angles[i] + client_camera.angles2[i]));
      }
      ...
      for (i = 0; i < 3; i++) {
        // NOTE: does not store the resultant angle in client_camera.angles
        // until pan is done
        cg.refdefViewAngles[i] =
            client_camera.angles[i] +
            (client_camera.angles2[i] / client_camera.pan_duration) *
  ...
  if (client_camera.info_state & CAMERA_MOVING) {
    ...
    if (client_camera.move_time + client_camera.move_duration < cg.time) {
      ...
      for (i = 0; i < 3; i++) {
        cg.refdef.vieworg[i] =
            client_camera.origin[i] +
            (((client_camera.origin2[i] - client_camera.origin[i])) /
             client_camera.move_duration) *
  ...
  if (checkFollow) {
    if (client_camera.info_state &
        CAMERA_FOLLOWING) { // This needs to be done after camera movement
      CGCam_FollowUpdate();
    }
    VectorCopy(client_camera.angles, cg.refdefViewAngles);
  }
  ...
  if (checkTrack) {
    if (client_camera.info_state &
        CAMERA_TRACKING) { // This has to run AFTER Follow if the camera is
                           // following a cameraGroup
      CGCam_TrackUpdate();
    }
...
void CGCam_DrawWideScreen(void) {
  ...
  if (client_camera.bar_alpha) {
    ...
    CG_FillRect(cg.refdef.x, cg.refdef.y, 640, client_camera.bar_height,
                modulate);
    CG_FillRect(cg.refdef.x, cg.refdef.y + 480 - client_camera.bar_height, 640,
...
void CGCam_UpdateShake(vec3_t origin, vec3_t angles) {
  ...
  intensity_scale =
      1.0f - ((float)(cg.time - client_camera.shake_start) /
              (float)client_camera.shake_duration) *
...
void CGCam_Smooth(float intensity, int duration) {
  client_camera.smooth_active =
      false; // means smooth_origin and angles are valid
...
void CGCam_UpdateSmooth(vec3_t origin, vec3_t angles) {
  if (!(client_camera.info_state & CAMERA_SMOOTHING) ||
      cg.time > (client_camera.smooth_start + client_camera.smooth_duration)) {
    client_camera.info_state &= ~CAMERA_SMOOTHING;
    return;
  }
  ...
  if (client_camera.smooth_duration > 200 &&
      cg.time >
          (client_camera.smooth_start + client_camera.smooth_duration - 100)) {
    factor += (1.0f - client_camera.smooth_intensity) *
              (100.0f - (client_camera.smooth_start +
                         client_camera.smooth_duration - cg.time)) /
              100.0f;
  }
  int i;
  for (i = 0; i < 3; i++) {
...
void CGCam_StartRoff(char *roff) {
  ...
  // Set up the roff state info..we'll hijack the moving and panning code until
  // told otherwise
...
static void CGCam_Roff(void) {
  while (client_camera.next_roff_time <= cg.time) {
    ...
    if (roff->type == 2) {
      move_rotate2_t *data =
          &((move_rotate2_t *)roff->data)[client_camera.roff_frame];
      VectorCopy(data->origin_delta, org);
      VectorCopy(data->rotate_delta, ang);
    } else {
      move_rotate_t *data =
          &((move_rotate_t *)roff->data)[client_camera.roff_frame];
      VectorCopy(data->origin_delta, org);
      VectorCopy(data->rotate_delta, ang);
    }
    ...
    if (client_camera.roff_frame) {
      ...
      if (!(client_camera.info_state & CAMERA_FOLLOWING)) {
        VectorAdd(client_camera.angles, client_camera.angles2,
                  client_camera.angles);
      }
