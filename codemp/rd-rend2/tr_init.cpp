...
static void ConvertRGBtoBGR(byte *dst, const byte *src, int stride, int width, int height) {
	const byte *row = src;
...
static void R_SaveTGA(const char *filename, const byte *pixels, int width, int height, int stride) {
...
==================
*/
static void R_SaveScreenshotTGA(const screenshotReadback_t *screenshotReadback, byte *pixels) {
	R_SaveTGA(screenshotReadback->filename, pixels, screenshotReadback->width, screenshotReadback->height, screenshotReadback->strideInBytes);
}
...
==================
*/
static void R_SaveScreenshotPNG(const screenshotReadback_t *screenshotReadback, byte *pixels) {
	RE_SavePNG(screenshotReadback->filename, pixels, screenshotReadback->width, screenshotReadback->height, 3);
}
...
static void R_SaveScreenshotJPG(const screenshotReadback_t *screenshotReadback, byte *pixels) {
	RE_SaveJPG(screenshotReadback->filename, r_screenshotJpegQuality->integer, screenshotReadback->width, screenshotReadback->height, pixels,
...
void R_SaveScreenshot(screenshotReadback_t *screenshotReadback) {
	...
	byte *pixelBuffer = static_cast<byte *>(qglMapBuffer(GL_PIXEL_PACK_BUFFER, GL_READ_ONLY));

	if (pixelBuffer == nullptr) {
		ri.Printf(PRINT_ALL, S_COLOR_RED "Failed to read screenshot data from GPU\n");
	} else {
...
const void *RB_TakeScreenshotCmd(const void *data) {
	...
	qglBufferData(GL_PIXEL_PACK_BUFFER, strideInBytes * cmd->height, nullptr, GL_STATIC_COPY);
	qglReadPixels(cmd->x, cmd->y, cmd->width, cmd->height, GL_RGB, GL_UNSIGNED_BYTE, 0);
	...
	Q_strncpyz(screenshot->filename, cmd->fileName, sizeof(screenshot->filename));
...
void R_Register(void) {
	...
	r_cubeMappingBounces =
		ri.Cvar_Get("r_cubeMappingBounces", "0", CVAR_ARCHIVE | CVAR_LATCH, "Renders cubemaps multiple times to get reflections in reflections");
...
static void R_InitBackEndFrameData() {
	...
	for (int i = 0; i < MAX_FRAMES; i++) {
		...
		qglBufferData(GL_UNIFORM_BUFFER, FRAME_UNIFORM_BUFFER_SIZE, nullptr, GL_DYNAMIC_DRAW);

		frame->dynamicVbo = R_CreateVBO(nullptr, FRAME_VERTEX_BUFFER_SIZE, VBO_USAGE_DYNAMIC);
		...
		if (glRefConfig.immutableBuffers) {
			...
			frame->dynamicVboMemory = qglMapBufferRange(GL_ARRAY_BUFFER, 0, frame->dynamicVbo->vertexesSize, mapBits);

			R_BindIBO(frame->dynamicIbo);
			frame->dynamicIboMemory = qglMapBufferRange(GL_ELEMENT_ARRAY_BUFFER, 0, frame->dynamicIbo->indexesSize, mapBits);
		} else {
...
static void R_InitGoreVao() {
	tr.goreVBO = R_CreateVBO(nullptr, sizeof(g2GoreVert_t) * MAX_LODS * MAX_GORE_RECORDS * MAX_GORE_VERTS * MAX_FRAMES, VBO_USAGE_DYNAMIC);
	...
	tr.goreIBO = R_CreateIBO(nullptr, sizeof(glIndex_t) * MAX_LODS * MAX_GORE_RECORDS * MAX_GORE_INDECIES * MAX_FRAMES, VBO_USAGE_DYNAMIC);
...
static void R_InitStaticConstants() {
	...
	qglBufferData(GL_UNIFORM_BUFFER, STATIC_UNIFORM_BUFFER_SIZE, nullptr, GL_STATIC_DRAW);
	...
	qglBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(entity2DBlock), &entity2DBlock);
	alignedBlockSize += (sizeof(EntityBlock) + alignment) & ~alignment;

	// Setup static 2d camera data
	CameraBlock a2DCameraBlock = {};
	Matrix16Ortho(0.0f, 640.0f, 480.0f, 0.0f, 0.0f, 1.0f, a2DCameraBlock.viewProjectionMatrix);
	...
	qglBufferSubData(GL_UNIFORM_BUFFER, tr.entityFlareUboOffset, sizeof(entityFlareBlock), &entityFlareBlock);
	alignedBlockSize += (sizeof(EntityBlock) + alignment) & ~alignment;

	// Setup static flare camera data
	CameraBlock flareCameraBlock = {};
	Matrix16Ortho(0.0f, glConfig.vidWidth, glConfig.vidHeight, 0.0f, -99999.0f, 99999.0f, flareCameraBlock.viewProjectionMatrix);

	tr.cameraFlareUboOffset = alignedBlockSize;
	qglBufferSubData(GL_UNIFORM_BUFFER, tr.cameraFlareUboOffset, sizeof(flareCameraBlock), &flareCameraBlock);
	alignedBlockSize += (sizeof(CameraBlock) + alignment) & ~alignment;
	...
	qglBufferSubData(GL_UNIFORM_BUFFER, tr.defaultLightsUboOffset, sizeof(lightsBlock), &lightsBlock);
	...
	qglBufferSubData(GL_UNIFORM_BUFFER, tr.defaultSceneUboOffset, sizeof(sceneBlock), &sceneBlock);
	...
	qglBufferSubData(GL_UNIFORM_BUFFER, tr.defaultFogsUboOffset, sizeof(fogsBlock), &fogsBlock);
	...
	qglBufferSubData(GL_UNIFORM_BUFFER, tr.defaultShaderInstanceUboOffset, sizeof(shaderInstanceBlock), &shaderInstanceBlock);
...
void R_Init(void) {
	...
	ptr = (byte *)ri.Hunk_Alloc(
		sizeof(*backEndData) + sizeof(srfPoly_t) * max_polys + sizeof(polyVert_t) * max_polyverts + sizeof(Allocator) + PER_FRAME_MEMORY_BYTES, h_low);
...
int C_GetLevel(void) { return tr.currentLevel; }
