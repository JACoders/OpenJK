...
unsigned int cgi_AnyLanguage_ReadCharFromString(
    const char **ppText, qboolean *pbIsTrailingPunctuation /* = NULL */) {
  int dummy;
  return Q_syscall(CG_ANYLANGUAGE_READFROMSTRING2, ppText, &dummy,
...
unsigned int cgi_AnyLanguage_ReadCharFromString(
  ...
  return Q_syscall(CG_ANYLANGUAGE_READFROMSTRING, ppText, ppos,
                   pbIsTrailingPunctuation);
}
...
void cgi_R_DrawScreenShot(float x, float y, float w, float h) {
  Q_syscall(CG_R_DRAWSCREENSHOT, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w),
            PASSFLOAT(h));
}
