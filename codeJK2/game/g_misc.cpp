...
extern void SetMiscModelDefaults(gentity_t *ent, useFunc_t use_func,
                                 char *material, int solid_mask, int animFlag,
...
void camera_aim(gentity_t *self) {
  ...
  if (player && player->client &&
    ...
    if ((player->client->usercmd.buttons & BUTTON_BLOCKING) ||
        player->client->usercmd.forwardmove ||
...
void Use_Shooter(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  ...
  */
  ...
  /*
     ...
     ); VectorNormalize( dir ); } else { VectorCopy( ent->movedir, dir );
          }
...
Place in a map and point the angles at whatever surface you want it to attach
to.
...
x USETARGET #MODELNAME="models/items/psd_big.md3" Gives shield energy when used.
...
USETARGET #MODELNAME="models/items/power_converter.md3" Gives ammo energy when
used.
...
void CrystalCratePain(gentity_t *self, gentity_t *inflictor,
                      gentity_t *attacker, vec3_t point, int damage, int mod,
                      int hitLoc) {
  vec3_t pt;
...
void SP_misc_atst_drivable(gentity_t *ent) {
  ...
  gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel],
                           "head_hatchcover_off", 0);
