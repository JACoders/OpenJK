...
extern void CG_ExplosionEffects(vec3_t origin, float intensity, int radius,
                                int time);
...
//------------------------------------------------------
void SFxHelper::PlaySound(const vec3_t org, int entityNum, int entchannel,
                          int sfxHandle) {
...
int SFxHelper::RegisterShader(const gsl::cstring_view &shader) {
  // TODO: it would be nice to change the ABI here to allow for passing of
  // string views
  return cgi_R_RegisterShader(
      std::string(shader.begin(), shader.end()).c_str());
}
...
//------------------------------------------------------
void SFxHelper::AddLightToScene(vec3_t org, float radius, float red,
...
//------------------------------------------------------
void SFxHelper::CameraShake(vec3_t origin, float intensity, int radius,
                            int time) {
  CG_ExplosionEffects(origin, intensity, radius, time);
}

//------------------------------------------------------
int SFxHelper::GetOriginAxisFromBolt(const centity_t &cent, int modelNum,
                                     int boltNum, vec3_t /*out*/ origin,
                                     vec3_t /*out*/ axis[3]) {
  if ((cg.time - cent.snapShotTime) >
      200) { // you were added more than 200ms ago, so I say you are no longer
             // valid/in our snapshot.
    return 0;
  }

  int doesBoltExist;
  mdxaBone_t boltMatrix;
  vec3_t G2Angles = {cent.lerpAngles[0], cent.lerpAngles[1],
                     cent.lerpAngles[2]};
  if (cent.currentState.eType == ET_PLAYER) { // players use cent.renderAngles
    ...
    if (cent.gent                          // has a game entity
        ...
        && cent.gent->m_pVehicle->m_pVehicleInfo->type !=
               VH_FIGHTER // it's not a fighter
        && cent.gent->m_pVehicle->m_pVehicleInfo->type !=
               VH_SPEEDER // not a speeder
    ) {
      G2Angles[PITCH] = 0;
      G2Angles[ROLL] = 0;
    }
  ...
  doesBoltExist = gi.G2API_GetBoltMatrix(
      cent.gent->ghoul2, modelNum, boltNum, &boltMatrix, G2Angles,
