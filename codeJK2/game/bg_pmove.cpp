...
extern qboolean G_DoDismemberment(gentity_t *self, vec3_t point, int mod,
...
static void PM_SetWaterLevelAtPoint(vec3_t org, int *waterlevel,
                                    int *watertype);
...
const float pm_frictionModifier =
    3.0f; // Used for "careful" mode (when pressing use)
const float pm_airDecelRate =
    1.35f; // Used for air decelleration away from current movement velocity

int c_pmove = 0;

extern void PM_SetTorsoAnimTimer(gentity_t *ent, int *torsoAnimTimer, int time);
extern void PM_SetLegsAnimTimer(gentity_t *ent, int *legsAnimTimer, int time);
// extern void PM_SetAnim(pmove_t	*pm,int setAnimParts,int anim,int
// setAnimFlags);
...
qboolean PM_ControlledByPlayer(void) { return G_ControlledByPlayer(pm->gent); }
/*
===============
PM_AddEvent
...
===============
*/
void PM_AddEvent(int newEvent) { AddEventToPlayerstate(newEvent, 0, pm->ps); }
...
qboolean PM_ClientImpact(int otherEntityNum, qboolean damageSelf) {
  ...
  if ((VectorLength(pm->ps->velocity) * (pm->gent->mass / 10)) >= 100 &&
      pm->ps->lastOnGround + 100 <
          level
              .time) // was 300
                     // ||(other->material>=MAT_GLASS&&pm->gent->lastImpact+100<=level.time))
  {
  ...
  if (!traceEnt || !(traceEnt->contents &
                     pm->tracemask)) { // it's dead or not in my way anymore
    return qtrue;
  }
...
qboolean PM_ForceJumpingUp(gentity_t *gent) {
  ...
  if (gent->NPC) { // this is ONLY for the player
    if (player && player->client &&
        player->client->ps.viewEntity ==
            gent->s.number) { // okay to jump if an NPC controlled by the player
    } else {
      return qfalse;
    }
  }

  if (!(gent->client->ps.forcePowersActive & (1 << FP_LEVITATION)) &&
      gent->client->ps.forceJumpCharge) { // already jumped and let go
    return qfalse;
  }
  ...
  if (!gent->s.number &&
      in_camera) { // player can't use force powers in cinematic
    return qfalse;
  }
  if (gent->client->ps.groundEntityNum == ENTITYNUM_NONE // in air
      && (/*(gent->client->ps.waterHeightLevel==WHL_SHOULDERS&&gent->client->usercmd.upmove>0)
             ||*/
          ((gent->client->ps.pm_flags & PMF_JUMPING) &&
           gent->client->ps.velocity[2] >
               0)) // jumped & going up or at water surface
      && gent->client->ps.forcePowerLevel[FP_LEVITATION] >
             FORCE_LEVEL_0 // force-jump capable
      && !(gent->client->ps.pm_flags &
           PMF_TRIGGER_PUSHED)) // not pushed by a trigger
  {
    if (gent->flags &
...
static void PM_JumpForDir(void) {
  ...
  if (!PM_InDeathAnim()) {
    PM_SetAnim(pm, SETANIM_LEGS, anim, SETANIM_FLAG_OVERRIDE,
...
qboolean PM_GentCantJump(gentity_t *gent) { // FIXME: ugh, hacky, set a flag on
                                            // NPC or something, please...
  if (gent && gent->client &&
...
static qboolean PM_CheckJump(void) {
  ...
  */
...
#if METROID_JUMP
  if (pm->waterlevel <
      3) //|| (pm->ps->waterHeightLevel==WHL_SHOULDERS&&pm->cmd.upmove>0) )
  {
    if (pm->ps->gravity > 0) { // can't do this in zero-G
      // FIXME: still able to pogo-jump...
      if (PM_ForceJumpingUp(pm->gent) &&
          (pm->ps->pm_flags &
           PMF_JUMP_HELD)) //||pm->ps->waterHeightLevel==WHL_SHOULDERS) )
      {                    // force jumping && holding jump
        /*
        if ( !pm->ps->forceJumpZStart &&
        (pm->ps->waterHeightLevel==WHL_SHOULDERS&&pm->cmd.upmove>0) )
        {
        ...
        */
        ...
        if ((curHeight <=
                 forceJumpHeight[0] || // still below minimum jump height
             (pm->ps->forcePower &&
              pm->cmd.upmove >= 10)) && ////still have force power available and
                                        ///still trying to jump up
            curHeight <
                forceJumpHeight[pm->ps->forcePowerLevel
                                    [FP_LEVITATION]]) // still below maximum
                                                      // jump height
        { // can still go up
          // FIXME: after a certain amount of time of held jump, play force jump
          // sound and flip if a dir is being held
          // FIXME: if hit a wall... should we cut velocity or allow them to
          // slide up it?
          // FIXME: constantly drain force power at a rate by which the usage
          // for maximum height would use up the full cost of force jump
          if (curHeight > forceJumpHeight[0]) { // passed normal jump height *2?
            if (!(pm->ps->forcePowersActive &
                  (1 << FP_LEVITATION))) // haven't started forcejump yet
            {
              // start force jump
              pm->ps->forcePowersActive |= (1 << FP_LEVITATION);
              if (pm->gent) {
                G_SoundOnEnt(pm->gent, CHAN_BODY,
                             "sound/weapons/force/jump.wav");
              }
              // play flip
              // FIXME: do this only when they stop the jump (below) or when
              // they're just about to hit the peak of the jump
              if ((pm->cmd.forwardmove ||
                   pm->cmd.rightmove) && // pushing in a dir
                                         // pm->ps->legsAnim != BOTH_ARIAL_F1
                                         // &&//not already flipping
              ...
              { // FIXME: this could end up playing twice if the jump is very
                // long...
                ...
                if (pm->ps->weaponTime) { // FIXME: really only care if we're in
                                          // a saber attack anim...
                  parts = SETANIM_LEGS;
                }

                PM_SetAnim(pm, parts, anim,
                           SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
              } else if (pm->ps->forcePowerLevel[FP_LEVITATION] >
                         FORCE_LEVEL_1) { // FIXME: really want to know how far
                                          // off ground we are, probably...
                vec3_t facingFwd, facingRight,
                ...
                if (anim != -1) {
                  int parts = SETANIM_BOTH;
                  if (pm->ps->weaponTime) { // FIXME: really only care if we're
                                            // in a saber attack anim...
                    parts = SETANIM_LEGS;
                  }
              ...
              if (!pm->ps->legsAnimTimer) { // not in the middle of a legsAnim
          ...
          // need to scale this down, start with height velocity (based on max
          ...
          pm->ps->velocity[2] =
              ...
              forceJumpHeight[pm->ps->forcePowerLevel[FP_LEVITATION]] *
              forceJumpStrength
                  [pm->ps->forcePowerLevel[FP_LEVITATION]]; // JUMP_VELOCITY;
          pm->ps->velocity[2] /= 10;
          ...
          pm->ps->pm_flags |= PMF_JUMP_HELD;
                   ...
                   curHeight <
                       forceJumpHeight[pm->ps->forcePowerLevel[FP_LEVITATION]] -
                           forceJumpHeight[0]) { // still have some headroom,
                                                 // don't totally stop it
  ...
  if (pm->ps->gravity <=
      0) { // in low grav, you push in the dir you're facing as long as there is
           // something behind you to shove off of
    vec3_t forward, back;
    trace_t trace;

    AngleVectors(pm->ps->viewangles, forward, NULL, NULL);
    VectorMA(pm->ps->origin, -8, forward, back);
    pm->trace(&trace, pm->ps->origin, pm->mins, pm->maxs, back,
              pm->ps->clientNum, pm->tracemask, G2_NOCOLLIDE, 0);
    ...
    if (pm->ps->groundEntityNum ==
        ENTITYNUM_NONE) { // need to set some things and return
      // Jumping
      ...
      return qtrue;
             ...
             ((pm->ps->clientNum && !PM_ControlledByPlayer()) ||
              ((!pm->ps->clientNum || PM_ControlledByPlayer()) &&
               ...
               !(pm->gent->flags &
                 FL_LOCK_PLAYER_WEAPONS)))) // yes this locked weapons check
                                            // also includes force powers, if we
                                            // need a separate check later I'll
                                            // make one
  {
    if (pm->gent->NPC && pm->gent->NPC->rank != RANK_CREWMAN &&
        pm->gent->NPC->rank <= RANK_LT_JG) { // reborn who are not acrobats
                                             // can't do any of these acrobatics
    } else if (pm->ps->groundEntityNum != ENTITYNUM_NONE) { // on the ground
      // check for left-wall and right-wall special jumps
      int anim = -1;
      float vertPush = 0;
      if (pm->cmd.rightmove > 0 && pm->ps->forcePowerLevel[FP_LEVITATION] >
                                       FORCE_LEVEL_1) { // strafing right
        ...
        if (pm->cmd.forwardmove > 0) { // wall-run
          ...
          anim = BOTH_WALL_FLIP_LEFT;
                 ...
                 pm->ps->forcePowerLevel[FP_LEVITATION] >
                     FORCE_LEVEL_1) { // run up wall, flip backwards
        if (VectorLengthSquared(pm->ps->velocity) >
            40000 /*200*200*/) { // have to be moving... FIXME: make sure it's
                                 // opposite the wall... or at least forward?
          vertPush = forceJumpStrength[FORCE_LEVEL_2] / 2.25f;
          anim = BOTH_WALL_FLIP_BACK1;
        }
      } else if (pm->cmd.forwardmove < 0 &&
                 !(pm->cmd.buttons & BUTTON_ATTACK)) // pm->ps->clientNum &&
      {                                 // double-tap back-jump does backflip
        if (pm->ps->velocity[2] >= 0) { // must be going up already
          vertPush = JUMP_VELOCITY;
          anim = PM_PickAnim(pm->gent, BOTH_FLIP_BACK1, BOTH_FLIP_BACK3);
        }
      } else if (VectorLengthSquared(pm->ps->velocity) <
                 256 /*16 squared*/) { // not moving
        if (pm->ps->weapon == WP_SABER && (pm->cmd.buttons & BUTTON_ATTACK) &&
            pm->ps->saberAnimLevel == FORCE_LEVEL_2) {
          /*
          ...
          */
          if (pm->ps->clientNum &&
              !PM_ControlledByPlayer()) // NOTE: pretty much useless, so player
                                        // never does these
      ...
      if (anim != -1 && PM_HasAnimation(pm->gent, anim)) {
        vec3_t fwd, right, traceto, mins = {pm->mins[0], pm->mins[1], 0},
                                    maxs = {pm->maxs[0], pm->maxs[1], 24},
        ...
        if (doTrace) {
          ...
          pm->trace(&trace, pm->ps->origin, mins, maxs, traceto,
                    pm->ps->clientNum, contents, G2_NOCOLLIDE, 0);
          VectorSubtract(pm->ps->origin, traceto, idealNormal);
          VectorNormalize(idealNormal);
          if (anim == BOTH_WALL_FLIP_LEFT) { // sigh.. check for bottomless pit
                                             // to the right
            trace_t trace2;
            vec3_t start;
            VectorMA(pm->ps->origin, 128, right, traceto);
            pm->trace(&trace2, pm->ps->origin, mins, maxs, traceto,
                      pm->ps->clientNum, contents, G2_NOCOLLIDE, 0);
            if (!trace2.allsolid && !trace2.startsolid) {
              VectorCopy(trace2.endpos, traceto);
              VectorCopy(traceto, start);
              traceto[2] -= 384;
              pm->trace(&trace2, start, mins, maxs, traceto, pm->ps->clientNum,
                        contents, G2_NOCOLLIDE, 0);
              if (!trace2.allsolid && !trace2.startsolid &&
                  trace2.fraction >= 1.0f) { // bottomless pit!
                trace.fraction = 1.0f;       // way to stop it from doing the
                                       // side-flip
              }
            }
          } else if (anim ==
                     BOTH_WALL_FLIP_RIGHT) { // sigh.. check for bottomless pit
                                             // to the left
            trace_t trace2;
            vec3_t start;
            VectorMA(pm->ps->origin, -128, right, traceto);
            ...
            if (!trace2.allsolid && !trace2.startsolid) {
              ...
              if (!trace2.allsolid && !trace2.startsolid &&
                ...
                trace.fraction = 1.0f;       // way to stop it from doing the
                     ...
                     BOTH_WALL_FLIP_BACK1) { // sigh.. check for bottomless pit
                                             // to the rear
            trace_t trace2;
            vec3_t start;
        ...
        if (!doTrace || (trace.fraction < 1.0f &&
                         ((trace.entityNum < ENTITYNUM_WORLD && traceEnt &&
                           traceEnt->s.solid != SOLID_BMODEL) ||
                          DotProduct(trace.plane.normal, idealNormal) >
                              0.7))) { // there is a wall there
          if ((anim != BOTH_WALL_RUN_LEFT && anim != BOTH_WALL_RUN_RIGHT) ||
              trace.plane.normal[2] == 0.0f) { // wall-runs can only run on
                                               // perfectly flat walls, sorry.
            // move me to side
            ...
            if (doTrace && anim != BOTH_WALL_RUN_LEFT &&
                anim != BOTH_WALL_RUN_RIGHT) {
              if (pm->gent && trace.entityNum < ENTITYNUM_WORLD) {
                if (traceEnt && traceEnt->client && traceEnt->health > 0 &&
                    traceEnt->takedamage) { // push them away and do pain
                  vec3_t oppDir;
                  float strength = VectorNormalize2(pm->ps->velocity, oppDir);
                  VectorScale(oppDir, -1, oppDir);
                  // FIXME: need knockdown anim
                  G_Damage(traceEnt, pm->gent, pm->gent, oppDir,
                           traceEnt->currentOrigin, 10,
                           DAMAGE_NO_ARMOR | DAMAGE_NO_HIT_LOC |
                               DAMAGE_NO_KNOCKBACK,
                           MOD_MELEE);
                  ...
                  G_Sound(traceEnt,
                          G_SoundIndex(va("sound/weapons/melee/punch%d",
                                          Q_irand(1, 4))));
                }
              }
            }
            ...
            if (anim == BOTH_BUTTERFLY_LEFT || anim == BOTH_BUTTERFLY_RIGHT ||
                anim == BOTH_FJSS_TR_BL || anim == BOTH_FJSS_TL_BR) {
              parts = SETANIM_BOTH;
              pm->cmd.buttons &= ~BUTTON_ATTACK;
              pm->ps->saberMove = LS_NONE;
              pm->gent->client->saberTrail.inAction =
                  qtrue; // FIXME: reset this when done!
              pm->gent->client->saberTrail.duration =
                  300; // FIXME: reset this when done!
            ...
            if (anim == BOTH_BUTTERFLY_LEFT || anim == BOTH_BUTTERFLY_RIGHT ||
              ...
              pm->ps->weaponTime = pm->ps->torsoAnimTimer;
                       ...
                       anim == BOTH_WALL_FLIP_RIGHT ||
                       anim == BOTH_WALL_FLIP_BACK1) { // let us do some more
                                                       // moves after this
              pm->ps->saberAttackChainCount = 0;
            }
            pm->ps->forceJumpZStart =
                pm->ps->origin[2]; // so we don't take damage if we land at same
                                   // height
      ...
      if (legsAnim == BOTH_WALL_RUN_LEFT ||
          legsAnim == BOTH_WALL_RUN_RIGHT) { // running on a wall
        vec3_t right, traceto, mins = {pm->mins[0], pm->mins[0], 0},
                               maxs = {pm->maxs[0], pm->maxs[0], 24},
                               fwdAngles = {0, pm->ps->viewangles[YAW], 0};
        trace_t trace;
        int anim = -1;
        ...
        if (legsAnim == BOTH_WALL_RUN_LEFT) {
          if (pm->ps->legsAnimTimer > 400) { // not at the end of the anim
            float animLen = PM_AnimLength(
                pm->gent->client->clientInfo.animFileIndex, BOTH_WALL_RUN_LEFT);
            if (pm->ps->legsAnimTimer < animLen - 400) { // not at start of anim
              VectorMA(pm->ps->origin, -16, right, traceto);
          ...
          if (pm->ps->legsAnimTimer > 400) { // not at the end of the anim
            float animLen =
                PM_AnimLength(pm->gent->client->clientInfo.animFileIndex,
                              BOTH_WALL_RUN_RIGHT);
            if (pm->ps->legsAnimTimer < animLen - 400) { // not at start of anim
              VectorMA(pm->ps->origin, 16, right, traceto);
        ...
        if (anim != -1) {
          pm->trace(&trace, pm->ps->origin, mins, maxs, traceto,
                    pm->ps->clientNum, CONTENTS_SOLID | CONTENTS_BODY,
                    G2_NOCOLLIDE, 0);
        ...
        if (pm->cmd.upmove != 0) { // jump failed, so don't try to do normal
                                   // jump code, just return
          return qfalse;
        }
      }
      /*
      ...
      {//double-tap back-jump does backflip
              ...
              PM_SetAnim( pm, parts, PM_PickAnim( pm->gent, BOTH_FLIP_BACK1,
      BOTH_FLIP_BACK3 ), SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 );
              pm->ps->forceJumpZStart = pm->ps->origin[2];//so we don't take
      damage if we land at same height pm->ps->pm_flags |=
      PMF_JUMPING|PMF_SLOW_MO_FALL; pm->cmd.upmove = 0; G_SoundOnEnt( pm->gent,
      CHAN_BODY, "sound/weapons/force/jump.wav" ); WP_ForcePowerDrain( pm->gent,
      FP_LEVITATION, 0 );
      }
      */
               ...
               && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1 &&
               (level.time - pm->ps->lastOnGround) <= 500 // just jumped
               && (pm->ps->legsAnim == BOTH_JUMP1 ||
                   pm->ps->legsAnim ==
                       BOTH_INAIR1)) // not in a flip or spin or anything
      {                              // run up wall, flip backwards
        // FIXME: have to be moving... make sure it's opposite the wall... or at
        // least forward?
        if (PM_HasAnimation(pm->gent, BOTH_WALL_FLIP_BACK1)) {
          vec3_t fwd, traceto, mins = {pm->mins[0], pm->mins[1], 0},
                               maxs = {pm->maxs[0], pm->maxs[1], 24},
                               fwdAngles = {0, pm->ps->viewangles[YAW], 0};
          trace_t trace;
          vec3_t idealNormal;
          ...
          pm->trace(&trace, pm->ps->origin, mins, maxs, traceto,
                    pm->ps->clientNum, CONTENTS_SOLID, G2_NOCOLLIDE,
                    0); // FIXME: clip brushes too?
          VectorSubtract(pm->ps->origin, traceto, idealNormal);
          VectorNormalize(idealNormal);
          gentity_t *traceEnt = &g_entities[trace.entityNum];
          ...
          if (trace.fraction < 1.0f &&
            ...
            PM_SetAnim(pm, parts, BOTH_WALL_FLIP_BACK1,
                       SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
            pm->ps->forceJumpZStart =
                pm->ps->origin[2]; // so we don't take damage if we land at same
                                   // height
  ...
  if (pm->gent && pm->cmd.upmove > 0 && pm->ps->weapon == WP_SABER &&
      (pm->ps->weaponTime > 0 || pm->cmd.buttons & BUTTON_ATTACK) &&
      ((pm->ps->clientNum && !PM_ControlledByPlayer()) ||
       ((!pm->ps->clientNum || PM_ControlledByPlayer()) &&
        cg.renderingThirdPerson &&
        !cg.zoomMode))) { // okay, we just jumped and we're in an attack
    if (!PM_RollingAnim(pm->ps->legsAnim) && !PM_InKnockDown(pm->ps) &&
        !PM_InDeathAnim() && !PM_PainAnim(pm->ps->torsoAnim) &&
        ...
        PM_InAnimForSaberMove(
            pm->ps->torsoAnim,
            pm->ps->saberMove)) { // not in an anim we shouldn't interrupt
      // see if it's not too late to start a special jump-attack
      float animLength = PM_AnimLength(
          g_entities[pm->ps->clientNum].client->clientInfo.animFileIndex,
          (animNumber_t)pm->ps->torsoAnim);
      if (animLength - pm->ps->torsoAnimTimer <
          500) { // just started the saberMove
        // check for special-case jump attacks
        if (pm->ps->saberAnimLevel == FORCE_LEVEL_2 ||
            pm->ps->saberAnimLevel ==
                FORCE_LEVEL_5) // using medium attacks or Tavion)
        {                      // using medium attacks
          if (pm->gent->enemy &&
              pm->ps->forcePowerLevel[FP_LEVITATION] >
                  FORCE_LEVEL_1 // can force jump
              &&
              !(pm->gent->flags &
            ...
            if (((!pm->ps->clientNum || PM_ControlledByPlayer()) &&
                ...
                (pm->gent->NPC &&
                 (pm->gent->NPC->rank == RANK_CREWMAN ||
                  pm->gent->NPC->rank >=
                      RANK_LT))) { // only player or acrobat or boss and higher
                                   // can do this
              ...
              if (pm->gent->enemy->health > 0 &&
                  ...
                  InFront(pm->gent->enemy->currentOrigin,
                          pm->gent->currentOrigin, fwdAngles,
                          0.3f)) { // enemy must be alive, not low to ground,
                                   // close and in front
                PM_SetSaberMove(PM_SaberFlipOverAttackMove());
                   ...
                   (pm->gent->client->NPC_class == CLASS_DESANN &&
                    !Q_irand(0, 1))) { // using strong attacks
          if (pm->cmd.forwardmove > 0  // going forward
              ...
              && (pm->ps->legsAnim == BOTH_STAND2 ||
                  ...
                  level.time - pm->ps->lastStationary <=
                      500) // standing or just started moving
              && (pm->ps->groundEntityNum != ENTITYNUM_NONE ||
                  (pm->ps->clientNum && !PM_ControlledByPlayer() &&
                   level.time - pm->ps->lastOnGround <=
                       500))) // on ground or just jumped if non-player
...
static qboolean PM_CheckWaterJump(void) {
  ...
  if (pm->cmd.forwardmove <= 0 &&
      pm->cmd.upmove <= 0) { // they must not want to get out?
    return qfalse;
  }
  ...
  spot[2] += 16;
  cont = pm->pointcontents(spot, pm->ps->clientNum);
  if (cont & (CONTENTS_SOLID | CONTENTS_PLAYERCLIP | CONTENTS_WATER |
...
static void PM_WaterMove(void) {
  ...
  if (PM_CheckWaterJump()) {
    PM_WaterJumpMove();
    return;
  } else if (pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0 &&
             pm->waterlevel < 3) {
    if (PM_CheckJump()) {
      // jumped away
      return;
  ...
  if (!scale) {
    ...
    {
      float depth = (pm->ps->origin[2] + pm->gent->client->standheight) -
                    pm->ps->waterheight;
      if (depth >= 12) {   // too high!
        wishvel[2] -= 120; // sink towards bottom
        if (wishvel[2] > 0) {
          wishvel[2] = 0;
        }
      } else if (pm->ps->waterHeightLevel >=
                 WHL_UNDER) //! depth && pm->waterlevel == 3 )
      {
      } else if (depth < 12) { // still deep
        wishvel[2] -= 60;      // sink towards bottom
        if (wishvel[2] > 30) {
          wishvel[2] = 30;
        }
...
static void PM_FlyVehicleMove(void) {
  ...
  // normal slowdown
  if (pm->ps->gravity && pm->ps->velocity[2] < 0 &&
      pm->ps->groundEntityNum == ENTITYNUM_NONE) { // falling
...
static void PM_FlyMove(void) {
  ...
  if (pm->ps->gravity <= 0 &&
      (!pm->ps->clientNum || (pm->gent && pm->gent->NPC &&
                              pm->gent->NPC->stats.moveType == MT_RUNJUMP))) {
    PM_CheckJump();
    accel = 1.0f;
    pm->ps->velocity[2] -= pm->ps->gravity * pml.frametime;
    pm->ps->jumpZStart = pm->ps->origin[2]; // so we don't take a lot of damage
                                            // when the gravity comes back on
    lowGravMove = qtrue;
  } else {
...
static void PM_AirMove(void) {
  ...
  if ((pm->ps->pm_flags & PMF_SLOW_MO_FALL)) { // no air-control
    VectorClear(wishvel);
  } else {
    for (i = 0; i < 2; i++) {
  ...
  /*
  if ( pm->gent && pm->gent->client && pm->gent->client->playerTeam ==
  TEAM_STASIS )
  ...
  {
  */
  if ((DotProduct(pm->ps->velocity, wishdir)) <
      0.0f) { // Encourage deceleration away from the current velocity
  ...
  if (!pm->ps->clientNum &&
      ...
      pm->ps->forceJumpZStart &&
      pm->ps->velocity[2] >
          0) { // I am force jumping and I'm not holding the button anymore
    float curHeight = pm->ps->origin[2] - pm->ps->forceJumpZStart +
                      (pm->ps->velocity[2] * pml.frametime);
    float maxJumpHeight =
        forceJumpHeight[pm->ps->forcePowerLevel[FP_LEVITATION]];
    if (curHeight >= maxJumpHeight) { // reached top, cut velocity
      ...
      Com_Printf( S_COLOR_RED"capping force jump height %4.2f > %4.2f
...
static void PM_WalkMove(void) {
  ...
  if (pm->ps->groundEntityNum != ENTITYNUM_NONE && // on ground
      ...
      pm->ps->pm_flags &
          PMF_TIME_KNOCKBACK) // knockback fimter on (stops friction)
  {
    pm->ps->pm_flags &= ~PMF_TIME_KNOCKBACK;
  }

  qboolean slide = qfalse;
  if (pm->ps->pm_type == PM_DEAD) {                         // corpse
    if (g_entities[pm->ps->groundEntityNum].client) {       // on a client
      if (g_entities[pm->ps->groundEntityNum].health > 0) { // a living client
        // no friction
        slide = qtrue;
      }
    }
  ...
  if (pm->ps->clientNum && !VectorCompare(pm->ps->moveDir, vec3_origin)) { // NPC
    // FIXME: what if the ucmd was set directly.... sigh.... check movedir
    // timestamp?
  ...
  if ((pml.groundTrace.surfaceFlags & SURF_SLICK) ||
      pm->ps->pm_flags & PMF_TIME_KNOCKBACK ||
      (pm->ps->pm_flags & PMF_TIME_NOFRICTION)) {
    if (pm->ps->gravity >= 0 && pm->ps->groundEntityNum != ENTITYNUM_NONE &&
        !VectorLengthSquared(pm->ps->velocity) &&
        pml.groundTrace.plane.normal[2] ==
            1.0) { // on ground and not moving and on level ground, no reason to
                   // do stupid fucking gravity with the clipvelocity!!!!
    } else {
  ...
  if (pm->ps->gravity <=
      0) { // need to apply gravity since we're going to float up from ground
    PM_StepSlideMove(1);
  } else {
    PM_StepSlideMove(0);
  }
...
static void PM_NoclipMove(void) {
  ...
  if (pm->gent && pm->gent->client) {
    pm->ps->viewheight =
        pm->gent->client->standheight + STANDARD_VIEWHEIGHT_OFFSET;
    //		if ( !pm->gent->mins[0] || !pm->gent->mins[1] || !pm->gent->mins[2] ||
    //!pm->gent->maxs[0] || !pm->gent->maxs[1] || !pm->gent->maxs[2] )
    //		{
    //			assert(0);
    //		}
    ...
    pm->ps->viewheight =
        DEFAULT_MAXS_2 + STANDARD_VIEWHEIGHT_OFFSET; // DEFAULT_VIEWHEIGHT;

    if (!DEFAULT_MINS_0 || !DEFAULT_MINS_1 || !DEFAULT_MAXS_0 ||
        !DEFAULT_MAXS_1 || !DEFAULT_MINS_2 || !DEFAULT_MAXS_2) {
      assert(0);
    }
...
static float PM_DamageForDelta(int delta) {
  ...
  if (pm->gent->NPC) {
    if (pm->ps->weapon ==
        WP_SABER) { // FIXME: for now Jedi take no falling damage, but really
                    // they should if pushed off?
      damage = 0;
    }
...
static void PM_CrashLandDamage(int damage) {
  if (pm->gent) {
    ...
    if (damage && !(pm->gent->flags & FL_NO_IMPACT_DMG)) {
      ...
      G_Damage(pm->gent, NULL, NULL, NULL, NULL, damage, DAMAGE_NO_ARMOR,
               MOD_FALLING);
    }
  }
}
...
/*
static float PM_CrashLandDelta( vec3_t org, vec3_t prevOrg, vec3_t prev_vel,
float grav, int waterlevel )
{
        float		delta;
        float		dist;
        float		vel, acc;
...
int PM_GetLandingAnim(void) {
  ...
  case BOTH_WALL_RUN_LEFT:             //#
  case BOTH_WALL_RUN_RIGHT:            //#
    if (pm->ps->legsAnimTimer > 500) { // only land at end of anim
      return -1;
    }
...
static qboolean PM_TryRoll(void) {
  ...
  if (PM_SaberInAttack(pm->ps->saberMove) ||
      ...
      (!pm->ps->clientNum &&
       PM_SaberInStart(pm->ps->saberMove))) { // attacking or spinning (or, if
                                              // player, starting an attack)
    return qfalse;
  }
  if (!pm->ps->clientNum &&
      (!cg.renderingThirdPerson ||
       cg.zoomMode)) { // player can't do this in 1st person
    return qfalse;
  }
  if (!pm->gent) {
    return qfalse;
  ...
  if (pm->ps->clientNum && pm->ps->weapon != WP_SABER &&
      pm->ps->weapon != WP_NONE) { // only jedi
    return qfalse;
  }
  if (pm->gent && pm->gent->NPC) {
    if (pm->gent->NPC->rank != RANK_CREWMAN &&
        pm->gent->NPC->rank <
            RANK_LT_JG) { // reborn who are not acrobats or fencers can't do any
                          // of these acrobatics
      return qfalse;
    }
  ...
  if (anim != -1) {
    ...
    if (pm->ps->clientNum) {
      ...
      if (pm->gent && pm->gent->enemy &&
          pm->gent->enemy->health > 0) { // player can always roll in combat
        roll = qtrue;
      } else {
    ...
    if (!roll) {
      pm->trace(&trace, pm->ps->origin, mins, maxs, traceto, pm->ps->clientNum,
                clipmask, G2_NOCOLLIDE, 0);
      if (trace.fraction >= 1.0f) { // okay, clear, check for a bottomless drop
        vec3_t top;
        VectorCopy(traceto, top);
        traceto[2] -= 256;
        pm->trace(&trace, top, mins, maxs, traceto, pm->ps->clientNum,
                  CONTENTS_SOLID, G2_NOCOLLIDE, 0);
        if (trace.fraction < 1.0f) { // not a bottomless drop
          roll = qtrue;
        }
      } else {                   // hit an architectural obstruction
        if (pm->ps->clientNum) { // NPCs don't care about rolling into walls,
                                 // just off ledges
          if (!(trace.contents & CONTENTS_BOTCLIP)) {
            roll = qtrue;
          }
        } else if (G_EntIsDoor(trace.entityNum)) { // okay to roll into a door
          if (G_EntIsUnlockedDoor(trace.entityNum)) { // if it's an auto-door
            roll = qtrue;
          }
        } else { // check other conditions
          gentity_t *traceEnt = &g_entities[trace.entityNum];
          if (traceEnt && (traceEnt->svFlags &
                           SVF_GLASS_BRUSH)) { // okay to roll through glass
            roll = qtrue;
          }
        }
      }
    ...
    if (roll) {
      ...
      pm->ps->weaponTime =
          ...
          200; // just to make sure it's cleared when roll is done
      PM_AddEvent(EV_ROLL);
      pm->ps->saberMove = LS_NONE;
      return qtrue;
    }
...
static void PM_CrashLand(void) {
  ...
  if (pm->ps->pm_flags & PMF_TRIGGER_PUSHED) {
    ...
    if (pm->gent && pm->gent->NPC &&
        pm->gent->NPC->aiFlags &
            NPCAI_DIE_ON_IMPACT) { // have to do death on impact if we are
                                   // falling to our death, FIXME: should we
                                   // avoid any additional damage this func?
      PM_CrashLandDamage(1000);
    }

    if (pm->ps->jumpZStart &&
        (pm->ps->forcePowerLevel[FP_LEVITATION] >= FORCE_LEVEL_1 ||
         !pm->ps->clientNum)) {                      // we were force-jumping
      if (pm->ps->origin[2] >= pm->ps->jumpZStart) { // we landed at same height
                                                     // or higher than we landed
        if (pm->ps->forceJumpZStart) { // we were force-jumping
          forceLanding = qtrue;
        }
        delta = 0;
      } else { // take off some of it, at least
        delta = (pm->ps->jumpZStart - pm->ps->origin[2]);
        float dropAllow =
            forceJumpHeight[pm->ps->forcePowerLevel[FP_LEVITATION]];
        if (dropAllow < 128) { // always allow a drop from 128, at least
          dropAllow = 128;
        }
        if (delta >
            forceJumpHeight[FORCE_LEVEL_1]) { // will have to use force jump
                                              // ability to absorb some of it
          forceLanding = qtrue; // absorbed some - just to force the correct
                                // animation to play below
        }
  ...
  if ((pm->ps->pm_flags & PMF_DUCKED) &&
      (level.time - pm->ps->lastOnGround) >
          500) { // must be crouched and have been inthe air for half a second
                 // minimum
    if (!PM_InOnGroundAnim(pm->ps) && !PM_InKnockDown(pm->ps)) { // roll!
      if (PM_TryRoll()) { // absorb some impact
        delta /= 2;
      }
    }
  }
  ...
  if (!PM_InDeathAnim()) {
    if (pm->cmd.upmove >= 0 && !PM_InKnockDown(pm->ps) &&
        !PM_InRoll(pm->ps)) { // not crouching
      if (delta > 10 || pm->ps->pm_flags & PMF_BACKWARDS_JUMP ||
          (pm->ps->forcePowersActive & (1 << FP_LEVITATION)) ||
      ...
      {                 // decide which landing animation to use
        ...
        if (anim != -1) {
          if (PM_FlippingAnim(pm->ps->torsoAnim) ||
              PM_SpinningAnim(pm->ps->torsoAnim)) { // interrupt these if we're
                                                    // going to play a land
    ...
    if (pm->gent) {
      if ((!(pml.groundTrace.surfaceFlags & SURF_NODAMAGE)) &&
          ...
          //&&
          (!(pm->pointcontents(pm->ps->origin, pm->ps->clientNum) &
             CONTENTS_NODROP))) {
        if (pm->waterlevel <
            2) { // don't play fallsplat when impact in the water
          deadFallSound = qtrue;
          G_SoundOnEnt(pm->gent, CHAN_BODY, "sound/player/fallsplat.wav");
        }
        if (gi.VoiceVolume[pm->ps->clientNum] && pm->gent->NPC &&
            (pm->gent->NPC->aiFlags &
             NPCAI_DIE_ON_IMPACT)) { // I was talking, so cut it off... with a
                                     // jump sound?
    ...
    if (pm->ps->legsAnim == BOTH_FALLDEATH1 ||
        pm->ps->legsAnim ==
            BOTH_FALLDEATH1INAIR) { // FIXME: add a little bounce?
      // FIXME: cut voice channel?
      int old_pm_type = pm->ps->pm_type;
      pm->ps->pm_type = PM_NORMAL;
      // Hack because for some reason PM_SetAnim just returns if you're
      // dead...???
      PM_SetAnim(pm, SETANIM_BOTH, BOTH_FALLDEATH1LAND,
                 SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  ...
  if (pm->gent && pm->gent->client &&
      pm->gent->client->respawnTime >=
          level.time - 500) { // just spawned in, don't make a noise
    return;
  }
  ...
  if (delta >= 75) {
    ...
    if (pm->gent) {
      if (pm->gent->s.number == 0) {
        ...
        bottom[2] += pm->mins[2];
        //	if ( pm->gent->client && pm->gent->client->playerTeam !=
        //TEAM_DISGUISE )
        { AddSoundEvent(pm->gent, bottom, 256, AEL_SUSPICIOUS); }
      } else if (pm->ps->stats[STAT_HEALTH] <= 0 && pm->gent &&
                 pm->gent->enemy) {
    ...
    if (pm->ps->stats[STAT_HEALTH] > 0) {
      if (!deadFallSound) {
        PM_AddEvent(EV_FALL_MEDIUM); // damage is dealt in g_active,
                                     // ClientEvents
      }
      if (pm->gent) {
        if (!(pml.groundTrace.surfaceFlags & SURF_NODAMAGE)) {
    ...
    if (!deadFallSound) {
      if (forceLanding) { // we were force-jumping
        PM_AddEvent(EV_FALL_SHORT);
      } else {
  ...
  pm->ps->bobCycle = 0;
  if (pm->gent && pm->gent->client) { // stop the force push effect when you
                                      // land
...
static void PM_CorrectAllSolid(void) {
  if (pm->debugLevel) {
    Com_Printf("%i:allsolid\n", c_pmove); // NOTENOTE: If this ever happens, I'd
                                          // really like to see this print!
  }
...
static void PM_FallToDeath(void) {
  ...
  if (PM_HasAnimation(pm->gent, BOTH_FALLDEATH1)) {
    PM_SetAnim(pm, SETANIM_LEGS, BOTH_FALLDEATH1,
               SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  } else {
    PM_SetAnim(pm, SETANIM_LEGS, BOTH_DEATH1,
               SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  }
...
static void PM_GroundTraceMissed(void) {
  ...
  // FIXME: if in a contents_falldeath brush, play the falling death anim and
  // sound?
  if (pm->ps->clientNum != 0 && pm->gent && pm->gent->NPC && pm->gent->client &&
      pm->gent->client->NPC_class !=
          CLASS_DESANN) // desann never falls to his death
  {
    if (pm->ps->groundEntityNum == ENTITYNUM_NONE) {
      if (pm->ps->stats[STAT_HEALTH] > 0 &&
          !(pm->gent->NPC->aiFlags & NPCAI_DIE_ON_IMPACT) &&
          !(pm->gent->NPC->scriptFlags & SCF_NO_FALLTODEATH)) {
        if ((level.time - pm->gent->client->respawnTime >
             2000) // been in the world for at least 2 seconds
            && (!pm->gent->NPC->timeOfDeath ||
                level.time - pm->gent->NPC->timeOfDeath < 1000) &&
            pm->gent->e_ThinkFunc !=
                thinkF_NPC_RemoveBody // Have to do this now because timeOfDeath
                                      // is used by thinkF_NPC_RemoveBody to
                                      // debounce removal checks
            &&
          ...
          if (!FlyingCreature(pm->gent) && g_gravity->value > 0) {
            if (!(pm->gent->flags & FL_UNDYING) &&
              ...
              if (!(pm->ps->eFlags & EF_FORCE_GRIPPED) &&
                ...
                if (!pm->ps->forceJumpZStart ||
                    pm->ps->forceJumpZStart >
                        pm->ps->origin[2]) // && fabs(pm->ps->velocity[0])<10 &&
                                           // fabs(pm->ps->velocity[1])<10 &&
                                           // pm->ps->velocity[2]<0)//either not
                                           // force-jumping or force-jumped and
                                           // now fell below original jump start
                                           // height
                {
                  /*if ( pm->ps->legsAnim = BOTH_FALLDEATH1
                  ...
                  {
                    if (trace.fraction ==
                        1.0) { // didn't hit, we're probably going to die
                      if (pm->ps->velocity[2] < 0 &&
                          pm->ps->origin[2] - point[2] >
                              256) { // going down, into a bottomless pit,
                                     // apparently
                        PM_FallToDeath();
                        cliff_fall = qtrue;
                      }
                               ...
                               pm->ps->weapon !=
                                   WP_SABER) { // Jedi don't scream and die if
                                               // they're heading for a hard
                                               // impact
                      gentity_t *traceEnt = &g_entities[trace.entityNum];
                      if (trace.entityNum == ENTITYNUM_WORLD ||
                          (traceEnt && traceEnt->bmodel)) { // hit architecture,
                                                            // find impact force
                        float dmg;

                        VectorCopy(pm->ps->velocity, vel);
                        time = Distance(trace.endpos, pm->ps->origin) /
                               VectorLength(vel);
                        vel[2] -= 0.5 * time * pm->ps->gravity;

                        if (trace.plane.normal[2] > 0.5) { // use falling damage
                          int waterlevel, junk;
                          PM_SetWaterLevelAtPoint(trace.endpos, &waterlevel,
                                                  &junk);
                          dmg = PM_CrashLandDelta(vel, waterlevel);
                          if (dmg >= 30) { // there is a minimum fall threshhold
                            dmg = PM_DamageForDelta(dmg);
                          } else {
                            dmg = 0;
                          }
                        } else { // use impact damage
                          // guestimate
                          if (pm->gent->client &&
                              pm->gent->client->ps
                                  .forceJumpZStart) { // we were force-jumping
                            if (pm->gent->currentOrigin[2] >=
                                pm->gent->client->ps
                                    .forceJumpZStart) { // we landed at same
                                                        // height or higher than
                                                        // we landed
                              dmg = 0;
                            } else { // FIXME: take off some of it, at least?
                              dmg = (pm->gent->client->ps.forceJumpZStart -
                                     pm->gent->currentOrigin[2]) /
                                    3;
                            }
                          }
                  ...
                  /*
                  vec3_t	start;
                  //okay, kind of expensive temp hack here, but let's check to
                  see if we should scream
                  //FIXME: we should either do a better check (predict using
                  actual velocity) or we should wait until they've been over a
                  bottomless pit for a certain amount of time... VectorCopy(
                  pm->ps->origin, start ); if ( pm->ps->forceJumpZStart <
                  start[2] )
                  {//Jedi who are force-jumping should only do this from landing
                  point down? start[2] = pm->ps->forceJumpZStart;
                  }
                  VectorCopy( start, point );
                  point[2] -= 400;//320
                  pm->trace (&trace, pm->ps->origin, pm->mins, pm->maxs, point,
                  pm->ps->clientNum, pm->tracemask);
                  //FIXME: somehow, people can still get stuck on ledges and not
                  splat when hit...? if ( !trace.allsolid && !trace.startsolid
                  && trace.fraction == 1.0 )
                  {
                          PM_FallToDeath();
                          cliff_fall = qtrue;
  ...
  if (!cliff_fall) {
    if ((pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT ||
        ...
        !pm->ps->legsAnimTimer) { // if flip anim is done, okay to use inair
      PM_SetAnim(pm, SETANIM_LEGS, BOTH_FORCEINAIR1, SETANIM_FLAG_OVERRIDE,
                 350); // Only blend over 100ms
    } else if (!PM_InRoll(pm->ps) && !PM_SpinningAnim(pm->ps->legsAnim) &&
               !PM_FlippingAnim(pm->ps->legsAnim) &&
      ...
      if (pm->ps->groundEntityNum != ENTITYNUM_NONE) {
        ...
        pm->trace(&trace, pm->ps->origin, pm->mins, pm->maxs, point,
                  pm->ps->clientNum, pm->tracemask, G2_NOCOLLIDE, 0);
        if (trace.fraction == 1.0) { // FIXME: if velocity[2] < 0 and didn't
                                     // jump, use some falling anim
          if (pm->ps->velocity[2] <= 0 && !(pm->ps->pm_flags & PMF_JUMP_HELD)) {
            ...
            if (pm->cmd.forwardmove >= 0) {
              ...
              if (!PM_InDeathAnim()) {
                PM_SetAnim(pm, SETANIM_LEGS, BOTH_JUMPBACK1,
                           SETANIM_FLAG_OVERRIDE, 100); // Only blend over 100ms
              }
              pm->ps->pm_flags |= PMF_BACKWARDS_JUMP;
            }
          }
...
static void PM_GroundTrace(void) {
    ...
    */
    return;
             ...
             (pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT ||
              pm->ps->legsAnim == BOTH_WALL_RUN_LEFT)) { // wall-running forces
  ...
  if (((pm->ps->velocity[2] > 0 && (pm->ps->pm_flags & PMF_TIME_KNOCKBACK)) ||
       pm->ps->velocity[2] > 100) &&
      DotProduct(pm->ps->velocity, trace.plane.normal) >
          10) { // either thrown off ground (PMF_TIME_KNOCKBACK) or going off
                // the ground at a large velocity
    if (pm->debugLevel) {
      Com_Printf("%i:kickoff\n", c_pmove);
    }
    // go into jump animation
    if (PM_FlippingAnim(pm->ps->legsAnim)) {         // we're flipping
    } else if (PM_InSpecialJump(pm->ps->legsAnim)) { // special jumps
    } else if (PM_InKnockDown(pm->ps)) {             // in knockdown
    } else if (PM_InRoll(pm->ps)) {                  // in knockdown
    } else {
      PM_JumpForDir();
    }
  ...
  // FIXME: if the ground surface is a "cover surface (like tall grass), add a
  // "cover" flag to me
  pml.groundPlane = qtrue;
  pml.walking = qtrue;
  ...
  if (pm->ps->groundEntityNum == ENTITYNUM_NONE) {
    ...
    // if ( !PM_ClientImpact( trace.entityNum, qtrue ) )
    {
      ...
      if (!pm->cmd.forwardmove && !pm->cmd.rightmove) {
        pm->ps->velocity[2] = 0; // wouldn't normally want this because of
                                 // slopes, but we aren't tyring to move...
      }
    }
  }
  ...
  pm->ps->lastOnGround = level.time;
  if (!pm->ps->clientNum) { // if a player, clear the jumping "flag" so can't
                            // double-jump
...
void PM_SetWaterHeight(void) {
  ...
  gi.trace(&trace, top, pm->mins, pm->maxs, bottom, pm->ps->clientNum,
           (CONTENTS_WATER | CONTENTS_SLIME), G2_NOCOLLIDE, 0);

  if (trace.startsolid) { // under water
    pm->ps->waterheight = top[2] + 4;
  } else if (trace.fraction < 1.0f) { // partially in and partially out of water
    pm->ps->waterheight = trace.endpos[2] + pm->mins[2];
  } else if (trace.contents &
             (CONTENTS_WATER | CONTENTS_SLIME)) { // water is above me
    pm->ps->waterheight = top[2] + 4;
  } else { // water is below me
    pm->ps->waterheight = bottom[2] - 4;
  }
  float distFromEyes =
      (pm->ps->origin[2] + pm->gent->client->standheight) - pm->ps->waterheight;
  ...
  } else if (distFromEyes <
             pm->gent->client->standheight - 8) { // at least 8 above origin
...
static void PM_CheckDuck(void) {
  ...
  if (pm->gent && pm->gent->client) {
    ...
    if (!pm->ps->clientNum && pm->gent->client->NPC_class == CLASS_ATST &&
        !cg.renderingThirdPerson) {
  ...
  if (PM_InGetUp(pm->ps)) { // can't do any kind of crouching when getting up
    if (pm->ps->legsAnim == BOTH_GETUP_CROUCH_B1 ||
        pm->ps->legsAnim == BOTH_GETUP_CROUCH_F1) { // crouched still
  ...
  if (PM_InRoll(pm->ps)) {
    ...
    {
            pm->maxs[2] =
    pm->gent->client->renderInfo.eyePoint[2]-pm->ps->origin[2] + 4; if (
    crouchheight > pm->maxs[2] )
            {
                    pm->maxs[2] = crouchheight;
            }
    }
  ...
  if (PM_InKnockDown(pm->ps)) { // forced crouch
    if (pm->gent &&
        pm->gent->client) { // interrupted any potential delayed weapon fires
  ...
  if (pm->cmd.upmove < 0) { // trying to duck
    pm->maxs[2] = crouchheight;
    pm->ps->viewheight =
        crouchheight + STANDARD_VIEWHEIGHT_OFFSET; // CROUCH_VIEWHEIGHT;
    if (pm->ps->groundEntityNum == ENTITYNUM_NONE &&
        !PM_SwimmingAnim(pm->ps->legsAnim)) { // Not ducked already and trying
                                              // to duck in mid-air
      ...
      if (pm->gent) {
        pm->ps->origin[2] +=
            oldHeight -
            pm->maxs[2]; // diff will be zero if were already ducking
    ...
    if (pm->ps->pm_flags & PMF_DUCKED) { // Was ducking
      if (pm->ps->groundEntityNum ==
          ENTITYNUM_NONE) { // unducking whilst in air will try to drop feet
        ...
        // NOTE: this isn't the best way to check this, you may have room to
        // unduck while in air, but your feet are close to landing.  Probably
        // won't be a noticable shortcoming
      } else {
        // try to stand up
        pm->maxs[2] = standheight;
        pm->trace(&trace, pm->ps->origin, pm->mins, pm->maxs, pm->ps->origin,
    ...
    if (pm->ps->pm_flags & PMF_DUCKED) { // Still ducking
      pm->maxs[2] = crouchheight;
      pm->ps->viewheight =
          crouchheight + STANDARD_VIEWHEIGHT_OFFSET; // CROUCH_VIEWHEIGHT;
    } else {                                         // standing now
      ...
      pm->ps->viewheight =
          standheight + STANDARD_VIEWHEIGHT_OFFSET; // DEFAULT_VIEWHEIGHT;
    }
  }
}
...
qboolean PM_CrouchGetup(float crouchheight) {
  ...
  if (anim == -1) {              // WTF? stay down?
    pm->ps->legsAnimTimer = 100; // hold this anim for another 10th of a second
    return qfalse;
  } else { // get up into crouch anim
    PM_SetAnim(pm, SETANIM_BOTH, anim,
               SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD |
                   SETANIM_FLAG_HOLDLESS);
    pm->ps->saberMove = pm->ps->saberBounceMove =
        LS_READY; // don't finish whatever saber anim you may have been in
...
qboolean PM_GettingUpFromKnockDown(float standheight, float crouchheight) {
  int legsAnim = pm->ps->legsAnim;
  if (legsAnim == BOTH_KNOCKDOWN1 || legsAnim == BOTH_KNOCKDOWN2 ||
      legsAnim == BOTH_KNOCKDOWN3 || legsAnim == BOTH_KNOCKDOWN4 ||
      legsAnim == BOTH_KNOCKDOWN5) { // in a knockdown
    if (!pm->ps->legsAnimTimer) {    // done with the knockdown - FIXME: somehow
                                  // this is allowing an *instant* getup...???
      // FIXME: if trying to crouch (holding button?), just get up into a
      // crouch?
      if (pm->cmd.upmove < 0) {
        ...
        pm->trace(&trace, pm->ps->origin, pm->mins, pm->maxs, pm->ps->origin,
                  pm->ps->clientNum, pm->tracemask, G2_NOCOLLIDE, 0);
        if (!trace.allsolid) { // stand up
          qboolean forceGetUp = qfalse;
          int anim = BOTH_GETUP1;
          pm->maxs[2] = standheight;
          pm->ps->viewheight = standheight + STANDARD_VIEWHEIGHT_OFFSET;
          // NOTE: the force power checks will stop fencers and grunts from
          // getting up using force jump
          switch (pm->ps->legsAnim) {
          case BOTH_KNOCKDOWN1:
            if ((pm->ps->clientNum &&
                 pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0) ||
                (!pm->ps->clientNum && pm->cmd.upmove > 0 &&
                 pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1)) {
              anim =
                  Q_irand(BOTH_FORCE_GETUP_B1,
                          BOTH_FORCE_GETUP_B6); // NOTE: BOTH_FORCE_GETUP_B5
                                                // takes soe steps forward at end
              forceGetUp = qtrue;
            } else {
              anim = BOTH_GETUP1;
            }
          ...
          if (forceGetUp) {
            if (pm->gent && pm->gent->client &&
                pm->gent->client->playerTeam == TEAM_ENEMY && pm->gent->NPC &&
...
void PM_ResetAnkleAngles(void) {
  ...
  if (pm->gent->footLBone != -1) {
    gi.G2API_SetBoneAnglesIndex(&pm->gent->ghoul2[0], pm->gent->footLBone,
                                vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z,
                                NEGATIVE_Y, NEGATIVE_X, NULL, 0, 0);
  }
  if (pm->gent->footRBone != -1) {
    gi.G2API_SetBoneAnglesIndex(&pm->gent->ghoul2[0], pm->gent->footRBone,
                                vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z,
                                NEGATIVE_Y, NEGATIVE_X, NULL, 0, 0);
  }
}
...
void PM_FootSlopeTrace(float *pDiff, float *pInterval) {
  ...
  // FIXME: these really should have been gotten on the cgame, but I guess
  // sometimes they're not and we end up with qnan numbers!
  mdxaBone_t boltMatrix;
  vec3_t G2Angles = {0, pm->gent->client->ps.legsYaw, 0};
  // get the feet
  gi.G2API_GetBoltMatrix(pm->gent->ghoul2, pm->gent->playerModel,
                         pm->gent->footLBolt, &boltMatrix, G2Angles,
                         pm->ps->origin, (cg.time ? cg.time : level.time), NULL,
                         pm->gent->s.modelScale);
  gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN,
                                  pm->gent->client->renderInfo.footLPoint);

  gi.G2API_GetBoltMatrix(pm->gent->ghoul2, pm->gent->playerModel,
                         pm->gent->footRBolt, &boltMatrix, G2Angles,
                         pm->ps->origin, (cg.time ? cg.time : level.time), NULL,
                         pm->gent->s.modelScale);
  gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN,
                                  pm->gent->client->renderInfo.footRPoint);
#endif
  // NOTE: on AT-STs, rotating the foot moves this point, so it will wiggle...
  //		we have to do this extra work (more G2 transforms) to stop the
  //wiggle... is it worth it?
  /*
  if ( pm->gent->client->NPC_class == CLASS_ATST )
  {
          mdxaBone_t	boltMatrix;
          vec3_t		G2Angles = {0, pm->gent->client->ps.legsYaw, 0};
          //get the feet
          gi.G2API_SetBoneAnglesIndex( &pm->gent->ghoul2[0],
  pm->gent->footLBone, vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z,
  NEGATIVE_Y, NEGATIVE_X, NULL ); gi.G2API_GetBoltMatrix( pm->gent->ghoul2,
  pm->gent->playerModel, pm->gent->footLBolt, &boltMatrix, G2Angles,
  pm->ps->origin, (cg.time?cg.time:level.time), NULL, pm->gent->s.modelScale );
          gi.G2API_GiveMeVectorFromMatrix( boltMatrix, ORIGIN,
  pm->gent->client->renderInfo.footLPoint );

          gi.G2API_SetBoneAnglesIndex( &pm->gent->ghoul2[0],
  pm->gent->footRBone, vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z,
  NEGATIVE_Y, NEGATIVE_X, NULL ); gi.G2API_GetBoltMatrix( pm->gent->ghoul2,
  pm->gent->playerModel, pm->gent->footRBolt, &boltMatrix, G2Angles,
  pm->ps->origin, (cg.time?cg.time:level.time), NULL, pm->gent->s.modelScale );
          gi.G2API_GiveMeVectorFromMatrix( boltMatrix, ORIGIN,
  pm->gent->client->renderInfo.footRPoint );
  }
  */
  // get these on the cgame and store it, save ourselves a ghoul2 construct skel
  // call
  ...
  pm->trace(&trace, footLOrg, footMins, footMaxs, footLBot, pm->ps->clientNum,
            pm->tracemask, G2_NOCOLLIDE, 0);
  VectorCopy(trace.endpos, footLBot);
  VectorCopy(trace.plane.normal, footLSlope);

  pm->trace(&trace, footROrg, footMins, footMaxs, footRBot, pm->ps->clientNum,
            pm->tracemask, G2_NOCOLLIDE, 0);
  ...
  if (pm->gent->client->NPC_class == CLASS_ATST) {
    vec3_t footAngles;
    if (!VectorCompare(footLSlope, vec3_origin)) { // rotate the ATST's left
                                                   // foot pad to match the slope
      PM_AnglesForSlope(pm->gent->client->renderInfo.legsYaw, footLSlope,
                        footAngles);
      // Hmm... lerp this?
      gi.G2API_SetBoneAnglesIndex(&pm->gent->ghoul2[0], pm->gent->footLBone,
                                  footAngles, BONE_ANGLES_POSTMULT, POSITIVE_Z,
                                  NEGATIVE_Y, NEGATIVE_X, NULL, 0, 0);
    }
    if (!VectorCompare(footRSlope, vec3_origin)) { // rotate the ATST's right
                                                   // foot pad to match the slope
      PM_AnglesForSlope(pm->gent->client->renderInfo.legsYaw, footRSlope,
                        footAngles);
      // Hmm... lerp this?
      gi.G2API_SetBoneAnglesIndex(&pm->gent->ghoul2[0], pm->gent->footRBone,
                                  footAngles, BONE_ANGLES_POSTMULT, POSITIVE_Z,
...
qboolean PM_AdjustStandAnimForSlope(void) {
  ...
  if (pm->gent->client->NPC_class != CLASS_ATST &&
      (!pm->gent ||
       !G_StandardHumanoid(
           pm->gent->NPC_type))) { // only ATST and player does this
    return qfalse;
  }
  if (!pm->ps->clientNum && (!cg.renderingThirdPerson ||
                             cg.zoomMode)) { // first person doesn't do this
    return qfalse;
  }
  if (pm->gent->footLBolt == -1 ||
      pm->gent->footRBolt == -1) { // need these bolts!
    return qfalse;
  }
  ...
  // step 4: based on difference, choose one of the left/right slope-match
  // intervals
  int destAnim;
  if (diff >= interval * 5) {
  ...
  // step 5: based on the chosen interval and the current legsAnim, pick the
  // correct anim step 6: increment/decrement to the dest anim, not instant
  if ((legsAnim >= LEGS_LEFTUP1 && legsAnim <= LEGS_LEFTUP5) ||
      ...
      (legsAnim >= LEGS_S5_LUP1 &&
       legsAnim <= LEGS_S5_LUP5)) { // already in left-side up
    if (destAnim > legsAnim && pm->gent->client->slopeRecalcTime < level.time) {
      legsAnim++;
      pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
    } else if (destAnim < legsAnim &&
               pm->gent->client->slopeRecalcTime < level.time) {
      legsAnim--;
      pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
    } else {
      destAnim = legsAnim;
             ...
             (legsAnim >= LEGS_S5_RUP1 &&
              legsAnim <= LEGS_S5_RUP5)) { // already in right-side up
    if (destAnim > legsAnim && pm->gent->client->slopeRecalcTime < level.time) {
      legsAnim++;
      pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
    } else if (destAnim < legsAnim &&
    ...
    if (pm->gent->client->NPC_class == CLASS_ATST) {
      if (legsAnim == BOTH_STAND1 || legsAnim == BOTH_STAND2 ||
          legsAnim == BOTH_CROUCH1IDLE) {
        if (destAnim >= LEGS_LEFTUP1 &&
            destAnim <= LEGS_LEFTUP5) { // going into left side up
          destAnim = LEGS_LEFTUP1;
          pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
        } else if (destAnim >= LEGS_RIGHTUP1 &&
                   destAnim <= LEGS_RIGHTUP5) { // going into right side up
          destAnim = LEGS_RIGHTUP1;
          pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
      ...
      switch (legsAnim) {
      case BOTH_STAND1:
        if (destAnim >= LEGS_S1_LUP1 &&
            destAnim <= LEGS_S1_LUP5) { // going into left side up
          destAnim = LEGS_S1_LUP1;
          pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
        } else if (destAnim >= LEGS_S1_RUP1 &&
                   destAnim <= LEGS_S1_RUP5) { // going into right side up
      ...
      case BOTH_CROUCH1IDLE:
        ...
        break;
      case BOTH_STAND3:
        if (destAnim >= LEGS_S3_LUP1 &&
            destAnim <= LEGS_S3_LUP5) { // going into left side up
          destAnim = LEGS_S3_LUP1;
          pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
        } else if (destAnim >= LEGS_S3_RUP1 &&
                   destAnim <= LEGS_S3_RUP5) { // going into right side up
        ...
        break;
      case BOTH_STAND4:
        if (destAnim >= LEGS_S4_LUP1 &&
            destAnim <= LEGS_S4_LUP5) { // going into left side up
          destAnim = LEGS_S4_LUP1;
          pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
        } else if (destAnim >= LEGS_S4_RUP1 &&
                   destAnim <= LEGS_S4_RUP5) { // going into right side up
        ...
        break;
      case BOTH_STAND5:
        if (destAnim >= LEGS_S5_LUP1 &&
            destAnim <= LEGS_S5_LUP5) { // going into left side up
          destAnim = LEGS_S5_LUP1;
          pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
        } else if (destAnim >= LEGS_S5_RUP1 &&
                   destAnim <= LEGS_S5_RUP5) { // going into right side up
...
static void PM_Footsteps(void) {
  ...
  if (pm->ps->legsAnim == BOTH_FLIP_F || pm->ps->legsAnim == BOTH_FLIP_B ||
      pm->ps->legsAnim == BOTH_FLIP_L || pm->ps->legsAnim == BOTH_FLIP_R) {
    flipping = qtrue;
  }

  if (pm->ps->groundEntityNum == ENTITYNUM_NONE ||
      (pm->watertype & CONTENTS_LADDER) ||
      pm->ps->waterHeightLevel >=
          WHL_TORSO) { // in air or submerged in water or in ladder
    // airborne leaves position in cycle intact, but doesn't advance
    if (pm->waterlevel > 0) {
      if (pm->watertype & CONTENTS_LADDER) { // FIXME: check for watertype, save
                                             // waterlevel for whether to play
        // the get off ladder transition anim!
        if (pm->ps->velocity[2]) { // going up or down it
          int anim;
          if (pm->ps->velocity[2] > 0) {
        ...
        return;
                 ...
                 (!pm->ps->clientNum || pm->ps->weapon == WP_SABER ||
                  ...
                  pm->ps->weapon ==
                      WP_MELEE)) // pm->waterlevel > 1 )	//in deep water
      {
        if (!PM_ForceJumpingUp(pm->gent)) {
          if (pm->ps->groundEntityNum != ENTITYNUM_NONE &&
              (pm->ps->pm_flags & PMF_DUCKED)) {
            if (!flipping) { // you can crouch under water if feet are on ground
              if (pm->cmd.forwardmove || pm->cmd.rightmove) {
                if (pm->ps->pm_flags & PMF_BACKWARDS_RUN) {
                  PM_SetAnim(pm, SETANIM_LEGS, BOTH_CROUCH1WALKBACK,
                             setAnimFlags);
                } else {
          ...
          PM_SwimFloatAnim();
          if (pm->ps->legsAnim != BOTH_SWIM_IDLE1) { // moving
            old = pm->ps->bobCycle;
        ...
        return;
      } else { // hmm, in water, but not high enough to swim
        // fall through to walk/run/stand
        if (pm->ps->groundEntityNum == ENTITYNUM_NONE) { // unless in the air
          // NOTE: this is a dupe of the code just below... for when you are not
          // in the water at all
          if (pm->ps->pm_flags & PMF_DUCKED) {
            if (!flipping) {
              PM_SetAnim(pm, SETANIM_LEGS, BOTH_CROUCH1, SETANIM_FLAG_NORMAL);
  ...
  if (PM_SwimmingAnim(pm->ps->legsAnim) &&
      pm->waterlevel < 2) { // legs are in swim anim, and not swimming, be sure
                            // to override it
  ...
  if (!pm->cmd.forwardmove && !pm->cmd.rightmove) {
    if (pm->gent && pm->gent->client &&
      ...
      if (pm->ps->legsAnimTimer &&
          PM_LandingAnim(
              pm->ps->legsAnim)) { // still in a landing anim, let it play
        return;
      }
      ...
      if (pm->gent && pm->gent->client &&
          pm->gent->client->NPC_class ==
              CLASS_GALAKMECH) { // NOTE: stand1 is with the helmet retracted,
                                 // stand1to2 is the helmet going into place
      ...
      } else if ((pm->ps->weapon == WP_SABER && pm->ps->saberLength > 0 &&
                  !pm->ps->saberInFlight)) {
        if (!PM_AdjustStandAnimForSlope()) {
          int legsAnim;
      ...
      } else if ((validNPC && pm->ps->weapon > WP_SABER &&
                  pm->ps->weapon < WP_DET_PACK)) // Being careful or carrying a
                                                 // 2-handed weapon
      { // Squadmates use BOTH_STAND3
        ...
        if (oldAnim != BOTH_GUARD_LOOKAROUND1 && oldAnim != BOTH_GUARD_IDLE1 &&
            ...
            oldAnim != BOTH_STAND4TO2 &&
            oldAnim != BOTH_STAND4) { // Don't auto-override the guard idles
          if (!PM_AdjustStandAnimForSlope()) {
  ...
  if ((PM_InSaberAnim(pm->ps->legsAnim) &&
       !PM_SpinningSaberAnim(pm->ps->legsAnim)) ||
      pm->ps->legsAnim == BOTH_STAND1 || pm->ps->legsAnim == BOTH_STAND1TO2 ||
      pm->ps->legsAnim == BOTH_STAND2TO1 || pm->ps->legsAnim == BOTH_STAND2 ||
      pm->ps->legsAnim == BOTH_SABERFAST_STANCE ||
      pm->ps->legsAnim == BOTH_SABERSLOW_STANCE ||
      pm->ps->legsAnim == BOTH_BUTTON_HOLD ||
      pm->ps->legsAnim == BOTH_BUTTON_RELEASE ||
      PM_LandingAnim(pm->ps->legsAnim) || PM_PainAnim(pm->ps->legsAnim) ||
      PM_ForceAnim(pm->ps->legsAnim)) { // legs are in a saber anim, and not
                                        // spinning, be sure to override it
    setAnimFlags |= SETANIM_FLAG_OVERRIDE;
  }
  ...
  {                                  // atst
    if (pm->ps->legsAnim == BOTH_TURN_LEFT1 ||
        pm->ps->legsAnim == BOTH_TURN_RIGHT1) { // moving overrides turning
      setAnimFlags |= SETANIM_FLAG_OVERRIDE;
    }
  }
  ...
  if (pm->ps->pm_flags & PMF_DUCKED) {
    ...
    if (!PM_InOnGroundAnim(pm->ps)) {
      ...
      if (PM_RunningAnim(pm->ps->legsAnim) ||
          pm->ps->legsAnim == BOTH_FORCEHEAL_START) { // roll!
        rolled = PM_TryRoll();
      }
      if (!rolled) {
        if (pm->ps->pm_flags & PMF_BACKWARDS_RUN) {
  ...
  } else if (pm->ps->pm_flags & PMF_BACKWARDS_RUN) { // Moving backwards
    if (!(pm->cmd.buttons & BUTTON_WALKING)) {       // running backwards
    ...
    if (pm->gent && pm->gent->client &&
      ...
      if (pm->ps->weapon == WP_NONE) { // helmet retracted
        PM_SetAnim(pm, SETANIM_BOTH, BOTH_WALK1, SETANIM_FLAG_NORMAL);
      } else { // helmet in place
      ...
      if (pm->ps->weapon == WP_SABER && pm->ps->saberActive) {
        ...
        if (pm->gent && pm->gent->client &&
          ...
          if (pm->ps->legsAnim != BOTH_RUN1) {
            if (pm->ps->legsAnim !=
                BOTH_RUN1START) { // Hmm, he should really start slow and have
                                  // to accelerate... also need to do this for
                                  // stopping
              PM_SetAnim(pm, SETANIM_LEGS, BOTH_RUN1START,
                         setAnimFlags | SETANIM_FLAG_HOLD);
      ...
      if (pm->gent && pm->gent->NPC && pm->gent->client &&
...
DoFootSteps:
  ...
  if (((old + 64) ^ (pm->ps->bobCycle + 64)) & 128) {
    if (pm->watertype & CONTENTS_LADDER) {
      if (!pm->noFootsteps) {
        ...
        if (pm->gent && pm->gent->s.number == 0) {
          //					if ( pm->gent->client &&
          //pm->gent->client->playerTeam != TEAM_DISGUISE )
      ...
      if (pm->gent && pm->gent->s.number == 0) {
        ...
        //pm->gent->client->playerTeam != TEAM_DISGUISE )
        {
          AddSoundEvent(pm->gent, pm->ps->origin, 384,
                        AEL_SUSPICIOUS); // was bottom
    ...
    } else { // or no sound when completely underwater...?
      PM_AddEvent(EV_SWIM);
    }
  }
}
...
static void PM_WaterEvents(void) { // FIXME?
  ...
  if (impact_splash) {
    ...
    gi.trace(&tr, start, vec3_origin, vec3_origin, end, pm->gent->s.number,
             CONTENTS_WATER, G2_NOCOLLIDE, 0);
  ...
  if (pml.previous_waterlevel == 3 && pm->waterlevel != 3) {
    if (!pm->gent || !pm->gent->client ||
        pm->gent->client->airOutTime <
            level.time + 2000) { // only do this if we were drowning or about to
                                 // start drowning
...
static void PM_BeginWeaponChange(int weapon) {
  ...
  if (pm->gent && pm->gent->client &&
      pm->gent->client->pers.enterTime >=
          level.time - 500) { // just entered map
    if (weapon == WP_NONE &&
        pm->ps->weapon !=
            weapon) { // don't switch to weapon none if just entered map
      return;
    }
  }
  ...
  if (cg.time > 0) { // this way we don't get that annoying change weapon sound
                     // every time a map starts
  ...
  if (pm->gent && pm->gent->client &&
      pm->gent->client->NPC_class == CLASS_GALAKMECH) {
    if (pm->gent->alt_fire) { // FIXME: attack delay?
      PM_SetAnim(pm, SETANIM_TORSO, TORSO_DROPWEAP3,
                 SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    } else { // FIXME: attack delay?
      PM_SetAnim(pm, SETANIM_TORSO, TORSO_DROPWEAP1,
                 SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    }
  } else {
  ...
  } else if (weapon == WP_SABER) { // going to switch to lightsaber
  } else {
    if (pm->ps->weapon == WP_SABER) { // going to switch away from saber
      if (pm->gent) {
        G_SoundOnEnt(pm->gent, CHAN_WEAPON,
                     "sound/weapons/saber/saberoffquick.wav");
      }
      PM_SetSaberMove(LS_PUTAWAY);
    }
    // put this back in because saberActive isn't being set somewhere else
    // anymore
...
static void PM_FinishWeaponChange(void) {
  ...
  if (pm->gent && pm->gent->client &&
    ...
    if (pm->cmd.weapon == WP_NONE &&
        pm->ps->weapon !=
            pm->cmd.weapon) { // don't switch to weapon none if just entered map
      return;
    }
  }
  ...
  if (pm->gent && pm->gent->client &&
      pm->gent->client->NPC_class == CLASS_ATST) { // do nothing
  } else if (weapon == WP_SABER) {                 // turn on the lightsaber
    // FIXME: somehow sometimes I still end up with 2 weapons in hand... usually
    // if I 		cycle weapons fast enough that I end up in 1st person lightsaber,
    //then 		somehow throw the saber and switch to another weapon (all in 1st
    //person), 		making my saber drop to the ground... when I switch back to the
    //saber, it 		does not remove the current weapon model and then, when I pull
    //the saber 		back to my hand, I have 2 weaponModels active...?
    if (pm->gent) { // remove gun if we had it.
      if (pm->gent->weaponModel >= 0) {
        gi.G2API_RemoveGhoul2Model(pm->gent->ghoul2, pm->gent->weaponModel);
      }
    }

    if (!pm->ps->saberInFlight) { // if it's not in flight or lying around, turn
                                  // it on!
      // FIXME: AddSound/Sight Event
      // FIXME: don't do this if just loaded a game
      if (trueSwitch) { // actually did switch weapons, turn it on
    ...
    if (trueSwitch) { // actually did switch weapons, play anim
      PM_SetSaberMove(LS_DRAW);
    }
  } else { // switched away from saber
    if (pm->gent) {
      ...
      if (weaponData[weapon]
              .weaponMdl[0]) { // might be NONE, so check if it has a model
    ...
    if (pm->gent && pm->gent->client &&
        pm->gent->client->NPC_class == CLASS_GALAKMECH) {
      if (pm->gent->alt_fire) { // FIXME: attack delay?
        PM_SetAnim(pm, SETANIM_TORSO, TORSO_RAISEWEAP3,
                   SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      } else { // FIXME: attack delay?
        PM_SetAnim(pm, SETANIM_TORSO, TORSO_RAISEWEAP1,
                   SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      }
    } else {
...
void PM_SetSaberMove(short newMove) {
  ...
  if (newMove == LS_READY || newMove == LS_A_FLIP_STAB ||
      newMove == LS_A_FLIP_SLASH) { // finished with a kata (or in a special
                                    // move) reset attack counter
    pm->ps->saberAttackChainCount = 0;
  } else if (PM_SaberInAttack(
                 newMove)) { // continuing with a kata, increment attack counter
  ...
  if (newMove == LS_READY) {
    ...
    default:
      ...
      break;
             ...
             !PM_SaberInIdle(newMove) && !PM_SaberInParry(newMove) &&
             !PM_SaberInKnockaway(newMove) && !PM_SaberInBrokenParry(newMove) &&
             !PM_SaberInReflect(newMove) &&
             !PM_SaberInSpecial(newMove)) { // readies, parries and reflections
                                            // have only 1 level
    // increment the anim to the next level of saber anims
    anim += (pm->ps->saberAnimLevel - FORCE_LEVEL_1) * SABER_ANIM_GROUP_SIZE;
  }

  // If the move does the same animation as the last one, we need to force a
  // restart...
  if (saberMoveData[pm->ps->saberMove].animToUse == anim &&
      newMove > LS_PUTAWAY) {
    setflags |= SETANIM_FLAG_RESTART;
  }

  if (anim == BOTH_STAND2 || anim == BOTH_SABERFAST_STANCE ||
      anim == BOTH_SABERSLOW_STANCE) {
  ...
  if (newMove == LS_A_LUNGE || newMove == LS_A_JUMP_T__B_ ||
      newMove == LS_A_BACKSTAB || newMove == LS_A_BACK ||
      newMove == LS_A_BACK_CR || newMove == LS_A_FLIP_STAB ||
      newMove == LS_A_FLIP_SLASH) {
    parts = SETANIM_BOTH;
  } else if (PM_SpinningSaberAnim(anim)) { // spins must be played on entire
                                           // body
    parts = SETANIM_BOTH;
  } else if ((!pm->cmd.forwardmove && !pm->cmd.rightmove &&
              !pm->cmd.upmove)) { // not trying to run, duck or jump
    if (!PM_FlippingAnim(pm->ps->legsAnim) && !PM_InRoll(pm->ps) &&
        !PM_InKnockDown(pm->ps) && !PM_JumpingAnim(pm->ps->legsAnim) &&
        !PM_PainAnim(pm->ps->legsAnim) && !PM_InSpecialJump(pm->ps->legsAnim) &&
  ...
  if (pm->ps->torsoAnim == anim) { // successfully changed anims
    ...
    if (pm->gent && pm->ps->saberLength > 1) {
      if (PM_SaberInAttack(newMove) ||
          PM_SaberInSpecialAttack(anim)) {  // playing an attack
        if (pm->ps->saberMove != newMove) { // wasn't playing that attack before
          if (PM_SaberInSpecialAttack(anim)) {
            G_SoundOnEnt(
                pm->gent, CHAN_WEAPON,
                va("sound/weapons/saber/saberhup%d.wav", Q_irand(1, 3)));
          } else {
            switch (pm->ps->saberAnimLevel) {
            case FORCE_LEVEL_4:
            case FORCE_LEVEL_3:
              G_SoundOnEnt(
                  pm->gent, CHAN_WEAPON,
                  va("sound/weapons/saber/saberhup%d.wav", Q_irand(7, 9)));
              break;
            case FORCE_LEVEL_2:
              G_SoundOnEnt(
                  pm->gent, CHAN_WEAPON,
                  va("sound/weapons/saber/saberhup%d.wav", Q_irand(4, 6)));
              break;
            case FORCE_LEVEL_5:
            case FORCE_LEVEL_1:
              G_SoundOnEnt(
              ...
              break;
                 ...
                 pm->ps->saberAnimLevel == FORCE_LEVEL_3) {
        G_SoundOnEnt(pm->gent, CHAN_WEAPON,
                     va("sound/weapons/saber/saberhup%d.wav", Q_irand(1, 3)));
      }
    }
    ...
    if (pm->ps->clientNum == 0 || PM_ControlledByPlayer()) {
      if (pm->ps->saberBlocked >= BLOCKED_UPPER_RIGHT_PROJ &&
          ...
          newMove >= LS_REFLECT_UP &&
          newMove <=
              LS_REFLECT_LL) { // don't clear it when blocking projectiles
      } else {
        pm->ps->saberBlocked = BLOCKED_NONE;
      }
               ...
               !pm->ps->saberActive ||
               (newMove < LS_PARRY_UR ||
                newMove > LS_REFLECT_LL)) { // NPCs only clear blocked if not
                                            // blocking?
      pm->ps->saberBlocked = BLOCKED_NONE;
    }
      ...
      if (saberMoveData[newMove].trailLength > 0) {
        ...
        pm->gent->client->saberTrail.duration =
            saberMoveData[newMove]
                .trailLength; // saber trail lasts for 75ms...feel free to
                              // change this if you want it longer or shorter
      } else {
...
int PM_NPCSaberAttackFromQuad(int quad) {
  ...
  if (pm->gent &&
      ((pm->gent->NPC && pm->gent->NPC->rank != RANK_ENSIGN &&
        pm->gent->NPC->rank != RANK_CIVILIAN) ||
       (pm->gent->client && pm->gent->client->NPC_class == CLASS_TAVION))) {
    autoMove = PM_AttackForEnemyPos(qtrue);
  }
  if (autoMove != LS_NONE &&
      PM_SaberInSpecial(
          autoMove)) { // if have opportunity to do a special attack, do one
    return autoMove;
    ...
    case Q_B:
      if ((pm->gent && pm->gent->NPC &&
           pm->gent->NPC->rank >=
               RANK_LT_JG)) { // fencers and above can do bottom-up attack
        if (Q_irand(0, pm->gent->NPC->rank) >=
            RANK_LT_JG) { // but not overly likely
          newmove = LS_A_LUNGE;
        }
      }
      break;
  ...
  if (ucmd->rightmove > 0) {     // moving right
    ...
    if (ucmd->forwardmove > 0) { // forward= T2B slash
      return Q_T;
               ...
               0) { // backward= T2B slash	//or B2T uppercut?
      return Q_T;
...
void PM_SetAnimFrame(gentity_t *gent, int frame, qboolean torso,
                     qboolean legs) {
  ...
  if (torso && gent->lowerLumbarBone != -1) // gent->upperLumbarBone
  {
    gi.G2API_SetBoneAnimIndex(&gent->ghoul2[gent->playerModel],
                              gent->lowerLumbarBone, // gent->upperLumbarBone
                              frame, frame + 1,
                              BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, 1,
                              actualTime, frame, 150);
    if (gent->motionBone != -1) {
      gi.G2API_SetBoneAnimIndex(&gent->ghoul2[gent->playerModel],
                                gent->motionBone, // gent->upperLumbarBone
                                frame, frame + 1,
                                BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, 1,
                                actualTime, frame, 150);
    }
  ...
  if (legs && gent->rootBone != -1) {
    gi.G2API_SetBoneAnimIndex(
        &gent->ghoul2[gent->playerModel], gent->rootBone, frame, frame + 1,
        BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, 1, actualTime, frame, 150);
  }
}
...
int PM_SaberLockWinAnim(saberLockResult_t result) {
  ...
  switch (pm->ps->torsoAnim) {
    /*
            default:
    #ifndef FINAL_BUILD
                    Com_Printf( S_COLOR_RED"ERROR-PM_SaberLockBreak: %s not in
    saberlock anim, anim = (%d)%s\n", pm->gent->NPC_type, pm->ps->torsoAnim,
  ...
  if (winAnim != -1) {
    PM_SetAnim(pm, SETANIM_BOTH, winAnim,
               SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    pm->ps->weaponTime = pm->ps->torsoAnimTimer;
  }
  return winAnim;
...
int PM_SaberLockLoseAnim(gentity_t *genemy, saberLockResult_t result) {
  ...
  switch (genemy->client->ps.torsoAnim) {
    /*
            default:
                    ...
                    Com_Printf( S_COLOR_RED"ERROR-PM_SaberLockBreak: %s not in
    saberlock anim, anim = (%d)%s\n", genemy->NPC_type,
    genemy->client->ps.torsoAnim, animTable[genemy->client->ps.torsoAnim].name
    ); #endif
    */
  case BOTH_BF2LOCK:
    if (result == LOCK_DRAW) {
      ...
      if (result == LOCK_STALEMATE) { // no-one won
  ...
  case BOTH_CWCIRCLELOCK:
    if (result == LOCK_DRAW) {
      genemy->client->ps.saberMove = genemy->client->ps.saberBounceMove =
          LS_V1_BL;
      ...
      if (result == LOCK_STALEMATE) { // no-one won
        ...
        /*
        genemy->client->ps.saberMove = genemy->client->ps.saberBounceMove =
        LS_H1_BR; genemy->client->ps.saberBlocked = BLOCKED_PARRY_BROKEN;
  ...
  case BOTH_CCWCIRCLELOCK:
    if (result == LOCK_DRAW) {
      ...
      if (result == LOCK_STALEMATE) { // no-one won
        ...
        /*
        genemy->client->ps.saberMove = genemy->client->ps.saberBounceMove =
        LS_H1_BL; genemy->client->ps.saberBlocked = BLOCKED_PARRY_BROKEN;
  ...
  if (loseAnim != -1) {
    NPC_SetAnim(genemy, SETANIM_BOTH, loseAnim,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void PM_SaberLockBreak(gentity_t *gent, gentity_t *genemy,
                       saberLockResult_t result, int victoryStrength) {
  ...
  if (d_saberCombat->integer) {
    Com_Printf("%s won saber lock, anim = %s!\n", gent->NPC_type,
               animTable[winAnim].name);
    Com_Printf("%s lost saber lock, anim = %s!\n", genemy->NPC_type,
               animTable[loseAnim].name);
  }
  ...
  if (result == LOCK_STALEMATE) { // no-one won
    ...
    if (pm->ps->clientNum) { // an NPC
      pm->ps->saberEventFlags |=
          SEF_LOCK_WON; // tell the winner to press the advantage
    }
    // painDebounceTime will stop them from doing anything
    genemy->painDebounceTime =
        level.time + genemy->client->ps.torsoAnimTimer + 500;
    if (Q_irand(0, 1)) {
    ...
    if (result == LOCK_VICTORY &&
        winAnim != BOTH_CCWCIRCLEBREAK) { // one person won
      if (Q_irand(FORCE_LEVEL_1, FORCE_LEVEL_2) <
          pm->ps->forcePowerLevel[FP_SABER_OFFENSE]) {
        if ((!genemy->s.number && genemy->health <= 25) // player low on health
            || (genemy->s.number && genemy->client->NPC_class != CLASS_LUKE &&
                genemy->client->NPC_class != CLASS_TAVION &&
                genemy->client->NPC_class !=
                    CLASS_DESANN) // any NPC that's not a boss character
            || (genemy->s.number &&
                genemy->health <=
                    50)) // boss character with less than 50 health left
        {                // possibly knock saber out of hand OR cut hand off!
          ...
          if (Q_irand(0, 25) < victoryStrength &&
              ((!genemy->s.number && genemy->health <= 10) ||
               genemy->s.number)) {
            NPC_SetAnim(genemy, SETANIM_BOTH, BOTH_RIGHTHANDCHOPPEDOFF,
                        SETANIM_FLAG_OVERRIDE |
                            SETANIM_FLAG_HOLD); // force this
            genemy->client->dismembered = qfalse;
            G_DoDismemberment(genemy, genemy->client->renderInfo.handRPoint,
                              MOD_SABER, 1000, HL_HAND_RT, qtrue);
            G_Damage(genemy, gent, gent, throwDir,
                     genemy->client->renderInfo.handRPoint, genemy->health + 10,
...
qboolean PM_SaberLocked(void) {
  if (pm->ps->saberLockEnemy == ENTITYNUM_NONE) {
    if (pm->ps->torsoAnim == BOTH_BF2LOCK ||
        ...
        pm->ps->torsoAnim == BOTH_CCWCIRCLELOCK) { // wtf?  Maybe enemy died?
  ...
  if ((pm->ps->torsoAnim == BOTH_BF2LOCK || pm->ps->torsoAnim == BOTH_BF1LOCK ||
       pm->ps->torsoAnim == BOTH_CWCIRCLELOCK ||
    ...
    if (pm->ps->saberLockTime <= level.time + 500 &&
        pm->ps->saberLockEnemy != ENTITYNUM_NONE) { // lock just ended
      int strength = G_SaberLockStrength(gent);
      int eStrength = G_SaberLockStrength(genemy);
      if (strength > 1 && eStrength > 1 &&
          !Q_irand(0, fabs((double)strength - eStrength) +
                          1)) { // both knock each other down!
        PM_SaberLockBreak(gent, genemy, LOCK_DRAW, 0);
      } else { // both "win"
        PM_SaberLockBreak(gent, genemy, LOCK_STALEMATE, 0);
      }
      return qtrue;
    } else if (pm->ps->saberLockTime <
               level.time) { // done... tie breaker above should have handled
                             // this, but...?
      ...
      return qfalse;
    } else if (pm->cmd.buttons & BUTTON_ATTACK) {  // holding attack
      if (!(pm->ps->pm_flags & PMF_ATTACK_HELD)) { // tapping
        int remaining = 0;
        if (ValidAnimFileIndex(gent->client->clientInfo.animFileIndex)) {
          ...
          qboolean ret =
              ...
              gi.G2API_GetBoneAnimIndex(
                  &gent->ghoul2[gent->playerModel], gent->lowerLumbarBone,
                  (cg.time ? cg.time : level.time), &currentFrame, &junk, &junk,
                  &junk, &junk2, NULL);

          assert(ret); // this would be pretty bad, the below code seems to
                       // assume the call succeeds. -gil
          ...
          if (pm->ps->torsoAnim == BOTH_CCWCIRCLELOCK ||
            ...
            if (curFrame >=
                anim->firstFrame + anim->numFrames) { // I won!  Break out
              PM_SaberLockBreak(gent, genemy, LOCK_VICTORY, strength);
              return qtrue;
            } else {
              PM_SetAnimFrame(gent, curFrame, qtrue, qtrue);
          ...
          if (!Q_irand(0, 2)) {
            if (!pm->ps->clientNum) {
              ...
              if (gent->NPC &&
                  gent->NPC->blockedSpeechDebounceTime < level.time) {
                switch (Q_irand(0, 3)) {
                case 0:
                  PM_AddEvent(EV_JUMP);
                  break;
        ...
        if (ValidAnimFileIndex(genemy->client->clientInfo.animFileIndex)) {
          animation_t *anim;
          anim =
              &level.knownAnimFileSets[genemy->client->clientInfo.animFileIndex]
                   .animations[genemy->client->ps.torsoAnim];
          /*
          float		currentFrame, junk2;
          int			junk;

          gi.G2API_GetBoneAnimIndex( &genemy->ghoul2[genemy->playerModel],
          genemy->lowerLumbarBone, (cg.time?cg.time:level.time), &currentFrame,
          &junk, &junk, &junk, &junk2, NULL );
          */
          ...
          if (genemy->client->ps.torsoAnim == BOTH_CWCIRCLELOCK ||
            ...
            if (!Q_irand(0, 2)) {
              ...
              case 0:
                G_AddEvent(
                    genemy, EV_PAIN,
                    floor((float)genemy->health / genemy->max_health * 100.0f));
                break;
              case 1:
                G_AddVoiceEvent(genemy, Q_irand(EV_PUSHED1, EV_PUSHED3), 500);
                break;
            ...
            else
            {
                    if ( d_saberCombat->integer )
                    {
                            Com_Printf( "%s losing in saber lock, %d frames to
            ...
            */
            ...
            PM_SetAnimFrame(genemy,
                            anim->firstFrame + anim->numFrames - remaining,
                            qtrue, qtrue);
            /*
            ...
            else
            {
                    if ( d_saberCombat->integer )
                    {
                            Com_Printf( "%s losing in saber lock, %d frames to
            go!\n", genemy->NPC_type,
            anim->firstFrame+anim->numFrames-(curFrame+1) );
                    }
                    PM_SetAnimFrame( genemy, curFrame+1, qtrue, qtrue );
            }
            */
    ...
    } else { // FIXME: other ways out of a saberlock?
    ...
    if (gent->painDebounceTime > level.time &&
        genemy->painDebounceTime > level.time) {
      PM_SaberLockBreak(gent, genemy, LOCK_DRAW, 0);
...
While this is a little different than the Quake 3 code, there is no clean way of
using the Q3 code for this kind of thing.
=================
*/
// Ultimate goal is to set the sabermove to the proper next location
// Note that if the resultant animation is NONE, then the animation is
// essentially "idle", and is set in WP_TorsoAnim
void PM_WeaponLightsaber(void) {
  int addTime;
  qboolean delayed_fire = qfalse;
  int anim = -1, curmove, newmove = LS_NONE;

  if (!pm->ps->clientNum && cg.saberAnimLevelPending > FORCE_LEVEL_0 &&
      cg.saberAnimLevelPending != pm->ps->saberAnimLevel) {
    if (!PM_SaberInStart(pm->ps->saberMove) &&
        !PM_SaberInTransition(pm->ps->saberMove) &&
        !PM_SaberInAttack(
            pm->ps->saberMove)) { // don't allow changes when in the middle of
                                  // an attack set...(or delay the change until
                                  // it's done)
  ...
  */
  ...
  if (!PM_SaberInBrokenParry(pm->ps->saberMove) &&
      pm->ps->saberBlocked != BLOCKED_PARRY_BROKEN &&
      !PM_DodgeAnim(pm->ps->torsoAnim)) { // we're not stuck in a broken parry
    if (pm->ps->saberInFlight) {          // guiding saber
      if (pm->ps->saberEntityNum < ENTITYNUM_NONE &&
          pm->ps->saberEntityNum > 0) // player is 0
      {                               //
      ...
      } else if (pm->ps->weapon == WP_SABER) { // guiding saber
        if (saberInAir) {
  ...
  if (pm->gent && pm->gent->client &&
      pm->gent->client->fireDelay > 0) { // FIXME: this is going to fire off one
                                         // frame before you expect, actually
  ...
  if (!pm->ps->clientNum || PM_ControlledByPlayer()) { // player
    if (pm->ps->saberBlocked >= BLOCKED_UPPER_RIGHT_PROJ &&
        pm->ps->saberBlocked <= BLOCKED_TOP_PROJ) { // blocking a projectile
      if (pm->ps->forcePowerDebounce[FP_SABER_DEFENSE] <
          level.time) { // block is done or breaking out of it with an attack
        pm->ps->weaponTime = 0;
        pm->ps->saberBlocked = BLOCKED_NONE;
      } else if ((pm->cmd.buttons &
                  BUTTON_ATTACK)) { // block is done or breaking out of it with
                                    // an attack
        pm->ps->weaponTime = 0;
        pm->ps->saberBlocked = BLOCKED_NONE;
      }
  ...
  if (pm->ps->saberBlocked) {
    if (pm->ps->saberMove > LS_PUTAWAY && pm->ps->saberMove <= LS_A_BL2TR &&
        ...
        (pm->ps->saberBlocked < BLOCKED_UPPER_RIGHT_PROJ ||
         pm->ps->saberBlocked > BLOCKED_TOP_PROJ)) //&& Q_irand( 0, 2 )
    ...
    {//if hitting attack during a parry (not already a knockaway)
            ...
            {//have high saber defense, turn the parry into a knockaway?
                    //FIXME: this could actually be bad for us...?  Leaves us
    open pm->ps->saberBounceMove = PM_KnockawayForParry( pm->ps->saberBlocked );
            }
    }
    */

    if (pm->ps->saberBlocked !=
        BLOCKED_ATK_BOUNCE) { // can't attack for twice whatever your skill
                              // level's parry debounce time is
      if (pm->ps->clientNum == 0 || PM_ControlledByPlayer()) { // player
        if (pm->ps->forcePowerLevel[FP_SABER_DEFENSE] <= FORCE_LEVEL_1) {
          pm->ps->weaponTime =
              parryDebounce[pm->ps->forcePowerLevel[FP_SABER_DEFENSE]];
        }
      } else { // NPC
        // pm->ps->weaponTime =
        // parryDebounce[pm->ps->forcePowerLevel[FP_SABER_DEFENSE]] * 2;
        if (pm->gent) {
    ...
    case BLOCKED_PARRY_BROKEN:
      // whatever parry we were is in now broken, play the appropriate
      // knocked-away anim
      {
        int nextMove;
        if (PM_SaberInBrokenParry(
                pm->ps->saberBounceMove)) { // already have one...?
    ...
    case BLOCKED_ATK_BOUNCE:
      // If there is absolutely no blocked move in the chart, don't even mess
      // with the animation. OR if we are already in a block or parry.
      if (pm->ps->saberMove >= LS_T1_BR__R /*LS_BOUNCE_TOP*/) //||
                                                              //saberMoveData[pm->ps->saberMove].bounceMove
                                                              //== LS_NONE )
      { // an actual bounce?  Other bounces before this are actually
        // transitions?
        pm->ps->saberBlocked = BLOCKED_NONE;
      } else if (PM_SaberInBounce(pm->ps->saberMove) ||
                 !PM_SaberInAttack(
                     pm->ps->saberMove)) { // already in the bounce, go into an
                                           // attack or transition to ready..
                                           // should never get here since can't
                                           // be blocked in a bounce!
        int nextMove;
        ...
        if (pm->cmd.buttons & BUTTON_ATTACK) { // transition to a new attack
          if (pm->ps->clientNum && !PM_ControlledByPlayer()) { // NPC
            ...
            while (
                newQuad ==
                saberMoveData[pm->ps->saberMove]
                    .startQuad) { // player is still in same attack quad, don't
                                  // repeat that attack because it looks bad,
            ...
            nextMove =
                transitionMove[saberMoveData[pm->ps->saberMove].startQuad]
                              [newQuad];
          }
          ...
          if (pm->ps->clientNum && !PM_ControlledByPlayer()) { // NPC
            ...
            if (saberMoveData[pm->ps->saberMove].startQuad == Q_T) {
              ...
              nextMove = LS_R_TL2BR +
                         saberMoveData[pm->ps->saberMove].startQuad - Q_BR;
            } else // if ( saberMoveData[pm->ps->saberMove].startQuad > Q_T )
            {
              nextMove = LS_R_BR2TL +
                         saberMoveData[pm->ps->saberMove].startQuad - Q_TL;
            }
          }
        }
        PM_SetSaberMove(nextMove);
        pm->ps->weaponTime = pm->ps->torsoAnimTimer;
      } else { // start the bounce move
        int bounceMove;
    ...
    // Charging is like a lead-up before attacking again.  This is an
    // appropriate use, or we can create a new weaponstate for blocking
    pm->ps->saberBounceMove = LS_NONE;
  ...
  if (pm->ps->weaponstate == WEAPON_RAISING) { // Just selected the weapon
    ...
    if (pm->gent && pm->gent->s.number == 0) {
      ...
      if (pm->ps->weapon == WP_SABER && pm->ps->saberInFlight &&
        ...
        // SETANIM_FLAG_NORMAL);//TORSO_WEAPONIDLE1 Select the proper idle
        // Lightsaber attack move from the chart.
  ...
  if (!delayed_fire) {
    // Start with the current move, and cross index it with the current control
    // states.
    ...
    if (curmove == LS_A_JUMP_T__B_ ||
        pm->ps->torsoAnim == BOTH_FORCELEAP2_T__B_) { // must transition back to
                                                      // ready from this anim
      newmove = LS_R_T2B;
    }
    // check for fire
    else if (!(pm->cmd.buttons &
      ...
      if (pm->gent && pm->gent->client &&
          pm->gent->client->fireDelay > 0) { // Still firing
        pm->ps->weaponstate = WEAPON_FIRING;
      } else if (pm->ps->weaponstate != WEAPON_READY) {
        pm->ps->weaponstate = WEAPON_IDLE;
      }
      // Check for finishing an anim if necc.
      if (curmove >= LS_S_TL2BR &&
          curmove <= LS_S_T2B) { // started a swing, must continue from here
        newmove = LS_A_TL2BR + (curmove - LS_S_TL2BR);
      } else if (curmove >= LS_A_TL2BR &&
                 curmove <=
                     LS_A_T2B) { // finished an attack, must continue from here
        newmove = LS_R_TL2BR + (curmove - LS_A_TL2BR);
      } else if (PM_SaberInTransition(
                     curmove)) { // in a transition, must play sequential attack
        newmove = saberMoveData[curmove].chain_attack;
      } else if (PM_SaberInBounce(curmove)) { // in a bounce
        if (pm->ps->clientNum &&
            !PM_ControlledByPlayer()) { // NPCs must play sequential attack
          // going into another attack...
          // allow endless chaining in level 1 attacks, several in level 2 and
          // only one or a few in level 3
          if (PM_SaberKataDone(LS_NONE,
                               LS_NONE)) { // done with this kata, must return
                                           // to ready before attack again
            newmove = saberMoveData[curmove].chain_idle;
          } else { // okay to chain to another attack
            newmove = saberMoveData[curmove]
                          .chain_attack; // we assume they're attacking, even if
                                         // they're not
            pm->ps->saberAttackChainCount++;
          }
        } else { // player gets his by directional control
          newmove = saberMoveData[curmove]
                        .chain_idle; // oops, not attacking, so don't chain
        }
    ...
    if (pm->ps->weapon == WP_SABER && pm->ps->saberInFlight &&
        saberInAir) { // guiding saber
      if (!PM_ForceAnim(pm->ps->torsoAnim) ||
          pm->ps->torsoAnimTimer < 300) { // don't interrupt a force power anim
        PM_SetAnim(pm, SETANIM_TORSO, BOTH_SABERPULL,
                   SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      }
    } else if (pm->ps->weaponTime > 0) { // Last attack is not yet complete.
      pm->ps->weaponstate = WEAPON_FIRING;
      return;
    } else {
      int both = qfalse;
      ...
      if (curmove >= LS_PARRY_UP &&
          curmove <= LS_REFLECT_LL) { // from a parry or reflection, can go
                                      // directly into an attack
        if (pm->ps->clientNum && !PM_ControlledByPlayer()) { // NPCs
      ...
      if (newmove !=
          LS_NONE) { // have a valid, final LS_ move picked, so skip findingt he
                     // transition move and just get the anim
      ...
      if (anim == -1) {
        ...
        if (PM_SaberInTransition(
                curmove)) { // in a transition, must play sequential attack
          newmove = saberMoveData[curmove].chain_attack;
        } else if (curmove >= LS_S_TL2BR &&
                   curmove <=
                       LS_S_T2B) { // started a swing, must continue from here
          newmove = LS_A_TL2BR + (curmove - LS_S_TL2BR);
        } else if (PM_SaberInBrokenParry(
                       curmove)) { // broken parries must always return to ready
          newmove = LS_READY;
        } else // if ( pm->cmd.buttons&BUTTON_ATTACK &&
               // !(pm->ps->pm_flags&PMF_ATTACK_HELD) )//only do this if just
               // pressed attack button?
        { // get attack move from movement command
          /*
          if ( PM_SaberKataDone() )
          {//we came from a bounce and cannot chain to another attack because
          our kata is done newmove = saberMoveData[curmove].chain_idle;
          }
          else */
          if (pm->ps->clientNum && !PM_ControlledByPlayer() &&
              (Q_irand(0, pm->ps->saberAnimLevel - 1) ||
               (pm->ps->saberAnimLevel == FORCE_LEVEL_1 && pm->gent &&
                pm->gent->NPC && pm->gent->NPC->rank >= RANK_LT_JG &&
                Q_irand(0, 1)))) // minor change to make fast-attack users use
                                 // the special attacks more
          { // NPCs use more randomized attacks the more skilled they are
            newmove = PM_NPCSaberAttackFromQuad(saberMoveData[curmove].endQuad);
          } else {
            newmove = PM_SaberAttackForMovement(pm->cmd.forwardmove,
                                                pm->cmd.rightmove, curmove);
            if ((PM_SaberInBounce(curmove) || PM_SaberInBrokenParry(curmove)) &&
                saberMoveData[newmove].startQuad ==
                    saberMoveData[curmove]
                        .endQuad) { // this attack would be a repeat of the last
                                    // (which was blocked), so don't actually use
                                    // it, use the default chain attack for this
                                    // bounce
              newmove = saberMoveData[curmove].chain_attack;
            }
          }
        ...
        /*
        if ( newmove == LS_NONE )
        {//FIXME: should we allow this?  Are there some anims that you should
        never be able to chain into an attack?
                //only curmove that might get in here is LS_NONE, LS_DRAW,
        LS_PUTAWAY and the LS_R_ returns... all of which are in Q_R newmove =
        PM_AttackMoveForQuad( saberMoveData[curmove].endQuad );
        }
        */
        if (newmove != LS_NONE) {
      ...
      if (anim == -1) { // not side-stepping, pick neutral anim
        ...
        if (!pm->cmd.forwardmove && !pm->cmd.rightmove && pm->cmd.upmove >= 0 &&
            pm->ps->groundEntityNum !=
                ENTITYNUM_NONE) { // not moving at all, so set the anim on
                                  // entire body
          both = qtrue;
        }
      }
      ...
      if (!pm->ps->saberActive) { // turn on the saber if it's not on
        pm->ps->saberActive = qtrue;
      }
      ...
      if (pm->gent && pm->gent->client) {
        //				pm->gent->client->saberTrail.inAction =
        //qtrue; 				pm->gent->client->saberTrail.duration = 75; // saber trail
        //lasts for 75ms...feel free to change this if you want it longer or
        //shorter
      }
      // don't fire again until anim is done
      pm->ps->weaponTime = pm->ps->torsoAnimTimer;
      /*
      //FIXME: this may be making it so sometimes you can't swing again right
      away... if ( newmove == LS_READY )
      {
  ...
  if (pm->cmd.buttons & BUTTON_ALT_ATTACK) {
    ...
    if (!addTime) {
      ...
      if (g_timescale != NULL) {
        if (g_timescale->value < 1.0f) {
          if (!MatrixMode) { // Special test for Matrix Mode (tm)
            if (pm->ps->clientNum == 0 && !player_locked &&
                pm->ps->forcePowersActive &
                    (1 << FP_SPEED)) { // player always fires at normal speed
  ...
  if (pm->gent && pm->gent->NPC != NULL) { // NPCs have their own refire logic
    return;
  }
...
//---------------------------------------
{
  ...
  // FIXME: make jedi aware they're being aimed at with a charged-up weapon
  // (strafe and be evasive?)
  ...
  case WP_DISRUPTOR:
    ...
    // alt-fire charges the weapon...but due to zooming being controlled by the
    // alt-button, the main button actually charges...but only when zoomed.
    //	lovely, eh?
    if (!pm->ps->clientNum) {
      if (cg.zoomMode == 2) {
        if (pm->cmd.buttons & BUTTON_ATTACK) {
          charging = qtrue;
          altFire = qtrue; // believe it or not, it really is an alt-fire in
                           // this case!
        }
      }
  ...
  case WP_ROCKET_LAUNCHER:
    ...
    // Not really a charge weapon, but we still want to delay fire until the
    // button comes up so that we can
  ...
  //	Note that we ALWAYS return if charging is set ( meaning the buttons are
  //still down )
  if (charging) {
    if (altFire) {
      if (pm->ps->weaponstate != WEAPON_CHARGING_ALT &&
        ...
        if (cg_weapons[pm->ps->weapon].altChargeSound) {
          G_SoundOnEnt(pm->gent, CHAN_WEAPON,
                       weaponData[pm->ps->weapon].altChargeSnd);
        }
      }
    } else {
      if (pm->ps->weaponstate != WEAPON_CHARGING &&
        ...
        if (cg_weapons[pm->ps->weapon].chargeSound && pm->gent &&
            !pm->gent->NPC) // HACK: !NPC mostly for bowcaster and weequay
        {
          G_SoundOnEnt(pm->gent, CHAN_WEAPON,
                       weaponData[pm->ps->weapon].chargeSnd);
        }
      }
    }
  ...
  if (pm->ps->weaponstate == WEAPON_CHARGING) {
    ...
    // dumb, but since we shoot a charged weapon on button-up, we need to
    // repress this button for now
    pm->cmd.buttons |= BUTTON_ATTACK;
    pm->ps->eFlags |= EF_FIRING;
...
#define BOWCASTER_CHARGE_UNIT                                                  \
  200.0f // bowcaster charging gives us one more unit every 200ms--if you change
         // this, you'll have to do the same in g_weapon
#define BRYAR_CHARGE_UNIT                                                      \
  200.0f // bryar charging gives us one more unit every 200ms--if you change
         // this, you'll have to do the same in g_weapon
#define DEMP2_CHARGE_UNIT 500.0f     // ditto
#define DISRUPTOR_CHARGE_UNIT 150.0f // ditto

// Specific weapons can opt to modify the ammo usage based on charges, otherwise
// if no special case code
...
//---------------------------------------
{
  ...
  if (pm->ps->weapon == WP_BOWCASTER &&
      !(pm->cmd.buttons & BUTTON_ALT_ATTACK)) {
    // this code is duplicated ( I know, I know ) in G_weapon.cpp for the
    // bowcaster alt-fire
    ...
    if (pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] != -1) {
      int dif =
          pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] - *amount * count;

      // If we have enough ammo to do the full charged shot, we are ok
      if (dif < 0) {
        // we are not ok, so hack our chargetime and ammo usage, note that DIF
        ...
        // now get a real chargeTime so the duplicated code in g_weapon doesn't
        // get freaked
    ...
    *amount *= count;
             ...
             pm->cmd.buttons & BUTTON_ALT_ATTACK) {
    // this code is duplicated ( I know, I know ) in G_weapon.cpp for the bryar
    // alt-fire
    ...
    // now that count is cool, get the real ammo usage
    *amount *= count;
  } else if (pm->ps->weapon == WP_DEMP2 &&
             pm->cmd.buttons & BUTTON_ALT_ATTACK) {
    // this code is duplicated ( I know, I know ) in G_weapon.cpp for the demp2
    // alt-fire
    ...
    *amount *= count;
    ...
    // this is an after-thought.  should probably re-write the function to do
    // this naturally.
    if (*amount > pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex]) {
      *amount = pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex];
             ...
             pm->cmd.buttons &
                 BUTTON_ALT_ATTACK) // BUTTON_ATTACK will have been mapped to
                                    // BUTTON_ALT_ATTACK if we are zoomed
  {
    // this code is duplicated ( I know, I know ) in G_weapon.cpp for the
    // disruptor alt-fire
...
qboolean PM_DroidMelee(int npc_class) {
  if (npc_class == CLASS_PROBE || npc_class == CLASS_SEEKER ||
      npc_class == CLASS_INTERROGATOR || npc_class == CLASS_SENTRY ||
...
static void PM_Weapon(void) {
  ...
  { // Separate logic for lightsaber, but not for player when zoomed
    PM_WeaponLightsaber();
    if (pm->gent && pm->gent->client && pm->ps->saberActive &&
        pm->ps->saberInFlight) { // FIXME: put saberTrail in playerState
      if (pm->gent->client->ps.saberEntityState ==
          SES_RETURNING) { // turn off the saber trail
        pm->gent->client->saberTrail.inAction = qfalse;
        pm->gent->client->saberTrail.duration = 75;
      } else { // turn on the saber trail
        pm->gent->client->saberTrail.inAction = qtrue;
  ...
  if (!delayed_fire) {
    ...
    if (!(pm->cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK))) {
      ...
      if (pm->gent && pm->gent->client &&
        ...
        if (!pm->gent || !pm->gent->NPC ||
            pm->gent->attackDebounceTime < level.time) {
          pm->ps->weaponstate = WEAPON_IDLE;
        }
      }
    ...
    case WP_MELEE:
      ...
      // since there's no RACE_BOTS, I listed all the droids that have might
      // have melee attacks - dmv
      if (pm->gent && pm->gent->client) {
        if (PM_DroidMelee(pm->gent->client->NPC_class)) {
          if (rand() & 1)
            PM_SetAnim(pm, SETANIM_BOTH, BOTH_MELEE1,
                       SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
          else
            PM_SetAnim(pm, SETANIM_BOTH, BOTH_MELEE2,
                       SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        } else {
          int anim;
          if (!pm->ps->clientNum) {
    ...
    case WP_DISRUPTOR:
      if ((pm->ps->clientNum && pm->gent && pm->gent->NPC &&
           (pm->gent->NPC->scriptFlags & SCF_ALT_FIRE)) ||
          (!pm->ps->clientNum &&
           cg.zoomMode == 2)) { // NPC or player in alt-fire, sniper mode
        PM_SetAnim(pm, SETANIM_TORSO, BOTH_ATTACK4,
                   SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      } else { // in primary fire mode
        PM_SetAnim(pm, SETANIM_TORSO, BOTH_ATTACK3,
                   SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD |
                       SETANIM_FLAG_RESTART);
      }
    ...
    case WP_EMPLACED_GUN:
      // Guess we don't play an attack animation?  Maybe we should have a custom
      // one??
      break;
  ...
  if (pm->ps->weapon == WP_MELEE) {
    ...
    case WP_BOWCASTER:
      addTime *= ((trueCount < 3)
                      ? 0.35f
                      : 1.0f); // if you only did a small charge shot with the
                               // bowcaster, use less time between shots
      break;
    }
  }
  ...
  pm->ps->weaponTime += addTime;
  pm->ps->lastShotTime =
      level.time; // so we know when the last time we fired our gun is

  // HACK!!!!!
  if (pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] <= 0) {
    if (pm->ps->weapon == WP_THERMAL || pm->ps->weapon == WP_TRIP_MINE) {
      // because these weapons have the ammo attached to the hand, we should
      // switch weapons when the last one is thrown, otherwise it will look
      // silly
      //	NOTE: could also switch to an empty had version, but was told we
      //aren't getting any new models at this point
      CG_OutOfAmmoChange();
      PM_SetAnim(pm, SETANIM_TORSO, TORSO_DROPWEAP1 + 2,
                 SETANIM_FLAG_OVERRIDE |
                     SETANIM_FLAG_HOLD); // hack weapon down!
      pm->ps->weaponTime = 50;
    }
  }
}
...
//-------------------------------------------
{
  ...
  if (pm->ps->weapon == WP_SABER &&
      (!cg.zoomMode ||
       pm->ps->clientNum)) { // don't let the alt-attack be interpreted as an
                             // actual attack command
    // saber alt-attack does a normal swing, too
    pm->cmd.buttons &= ~BUTTON_ALT_ATTACK;
    if (pm->ps->saberInFlight) { // saber not in hand, can't swing it
      pm->cmd.buttons &= ~BUTTON_ATTACK;
    }
  }

  // disruptor alt-fire should toggle the zoom mode, but only bother doing this
  // for the player?
  if (pm->ps->weapon == WP_DISRUPTOR && pm->gent && pm->gent->s.number == 0 &&
    ...
    //	we either are ducking ( in which case we don't care if they are moving
    //)...or they are not ducking...and also not moving right/forward.
    if (!(pm->ps->eFlags & EF_ALT_FIRING) &&
        (pm->cmd.buttons & BUTTON_ALT_ATTACK) &&
        (pm->cmd.upmove < 0 || (!pm->cmd.forwardmove && !pm->cmd.rightmove))) {
      // We just pressed the alt-fire key
      if (cg.zoomMode == 0 || cg.zoomMode == 3) {
        G_SoundOnEnt(pm->gent, CHAN_AUTO,
                     "sound/weapons/disruptor/zoomstart.wav");
        // not already zooming, so do it now
        cg.zoomMode = 2;
        cg.zoomLocked = qfalse;
        cg_zoomFov = 80.0f; //(cg.overrides.active&CG_OVERRIDE_FOV) ?
                            //cg.overrides.fov : cg_fov.value;
    ...
    if (pm->cmd.buttons & BUTTON_ATTACK) {
      // If we are zoomed, we should switch the ammo usage to the alt-fire,
      // otherwise, we'll
  ...
  // set the firing flag for continuous beam weapons, phaser will fire even if
  // out of ammo
  if (((pm->cmd.buttons & BUTTON_ATTACK ||
        pm->cmd.buttons & BUTTON_ALT_ATTACK) &&
    ...
    if (pm->cmd.buttons & BUTTON_ALT_ATTACK) {
      pm->ps->eFlags |= EF_ALT_FIRING;
      if (!pm->ps->clientNum && pm->gent &&
          (pm->ps->eFlags & EF_IN_ATST)) { // switch ATST barrels
    ...
    // if I don't check the flags before stopping FX then it switches them off
    // too often, which tones down
    //	the stronger FFFX so you can hardly feel them. However, if you only do
    //iton these flags then the 	repeat-fire weapons like tetrion and dreadnought
    //don't switch off quick enough. So...
    //
    /* // Might need this for beam type weapons
                    if ( pm->ps->weapon == WP_DREADNOUGHT || (iFlags &
       (EF_FIRING|EF_ALT_FIRING) )
                    {
                            cgi_FF_StopAllFX();
                    }
                    */
  ...
  // disruptor should convert a main fire to an alt-fire if the gun is currently
  // zoomed
  if (pm->ps->weapon == WP_DISRUPTOR && pm->gent && pm->gent->s.number == 0) {
...
void Pmove(pmove_t *pmove) {
  ...
  // this counter lets us debug movement problems with a journal by setting a
  // conditional breakpoint fot the previous frame
  c_pmove++;
  ...
  // In certain situations, we may want to control which attack buttons are
  // pressed and what kind of functionality
  //	is attached to them
  ...
  if (pm->ps->pm_type >= PM_DEAD) {
    ...
    pm->cmd.rightmove = 0;
    pm->cmd.upmove = 0;
    if (pm->ps->viewheight > -12) { // slowly sink view to ground
      pm->ps->viewheight -= 1;
    }
  }
  ...
  if (pm->ps->pm_flags & PMF_SLOW_MO_FALL) { // half grav
    pm->ps->gravity *= 0.5;
  }
  ...
  PM_SetWaterHeight();
  if (!(pm->watertype &
        CONTENTS_LADDER)) { // Don't want to remember this for ladders, is only
                            // for waterlevel change events (sounds)
  ...
  if (!Flying && !(pm->watertype & CONTENTS_LADDER) &&
      pm->ps->pm_type !=
          PM_DEAD) { // NOTE: noclippers shouldn't jump or duck either, no?
    PM_CheckDuck();
  }
  ...
  if (pm->ps && pm->ps->eFlags & EF_LOCKED_TO_WEAPON) { // in an emplaced gun
    PM_NoclipMove();
  } else if (Flying == FLY_NORMAL) //|| pm->ps->gravity <= 0 )
  {
    ...
    PM_WaterJumpMove();
             ...
             && (!pm->ps->clientNum ||
                 !waterForceJump)) // player or NPC not force jumping
  {                                // force-jumping NPCs should
    ...
    float threshHold = 0.001f,
          movedDist = DistanceSquared(oldOrg, pm->ps->origin);
    if (PM_StandingAnim(pm->ps->legsAnim) || pm->ps->legsAnim == BOTH_CROUCH1) {
      threshHold = 0.005f;
    }

    if (movedDist < threshHold) { // didn't move, play no legs anim
