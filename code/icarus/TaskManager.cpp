...
CTaskGroup *CTaskManager::AddTaskGroup(const char *name, CIcarus *icarus) {
  ...
  if (group == NULL) {
    icarus->GetGame()->DebugPrint(IGameInterface::WL_ERROR,
                                  "Unable to allocate task group \"%s\"\n",
                                  name);
    return NULL;
  }
...
inline bool CTaskManager::Check(int targetID, CBlock *block,
                                int memberNum) const {
...
int CTaskManager::GetFloat(int entID, CBlock *block, int &memberNum,
  ...
  if (Check(CIcarus::ID_TAG, block, memberNum)) {
    icarus->GetGame()->DebugPrint(IGameInterface::WL_WARNING,
                                  "Invalid use of \"tag\" inline.  Not a valid "
...
int CTaskManager::GetVector(int entID, CBlock *block, int &memberNum,
  ...
  if (Check(CIcarus::ID_RANDOM, block, memberNum)) {
    ...
    for (i = 0; i < 3; i++) {
      value[i] = (float)icarus->GetGame()->Random(
          min,
          max); // FIXME: Just truncating it for now.. should be fine though
    }
  ...
  if (Check(CIcarus::ID_TAG, block, memberNum)) {
    ...
    if (icarus->GetGame()->GetTag(entID, tagName, (int)tagLookup, value) ==
        false) {
      icarus->GetGame()->DebugPrint(IGameInterface::WL_ERROR,
  ...
  if (type != CIcarus::TK_VECTOR) {
    //		icarus->GetGame()->DPrintf( WL_WARNING, "Unexpected value; expected
    //type VECTOR\n" );
    return false;
  }
...
int CTaskManager::GetID() { return m_id; }
...
int CTaskManager::Get(int entID, CBlock *block, int &memberNum, char **value,
  ...
  if (Check(CIcarus::ID_TAG, block, memberNum)) {
    ...
    if (icarus->GetGame()->GetTag(entID, tagName, (int)tagLookup, vector) ==
        false) {
      icarus->GetGame()->DebugPrint(IGameInterface::WL_ERROR,
                                    "Unable to find tag \"%s\"!\n", tagName);
      assert(0 && "Unable to find tag");
  ...
  if (bm->GetID() == CIcarus::TK_INT) {
    ...
    *value = (char *)tempBuffer;
    ...
    return true;
  } else if ((bm->GetID() == CIcarus::TK_STRING) ||
             (bm->GetID() == CIcarus::TK_IDENTIFIER)) {
...
int CTaskManager::CallbackCommand(CTask *task, int returnCode,
                                  CIcarus *icarus) {
  if (m_owner->Callback(this, task->GetBlock(), returnCode, icarus) ==
      CSequencer::SEQ_OK)
    return Go(icarus);
...
int CTaskManager::Wait(CTask *task, bool &completed, CIcarus *icarus) {
  ...
  {
    if (Check(CIcarus::ID_RANDOM, block,
              memberNum)) { // get it random only the first time
      float min, max;

      dwtime = *(float *)block->GetMemberData(memberNum++);
      if (dwtime == icarus->GetGame()
                        ->MaxFloat()) { // we have not evaluated this random yet
        min = *(float *)block->GetMemberData(memberNum++);
        max = *(float *)block->GetMemberData(memberNum++);
    ...
    if (task->GetTimeStamp() == icarus->GetGame()->GetTime()) {
      // Print out the debug info
      icarus->GetGame()->DebugPrint(IGameInterface::WL_DEBUG,
                                    "%4d wait( %d ); [%d]", m_ownerID,
                                    (int)dwtime, task->GetTimeStamp());
    }
...
int CTaskManager::Sound(CTask *task, CIcarus *icarus) {
  ...
  icarus->GetGame()->DebugPrint(IGameInterface::WL_DEBUG,
                                "%4d sound(\"%s\", \"%s\"); [%d]", m_ownerID,
                                sVal, sVal2, task->GetTimeStamp());

  // Only instantly complete if the user has requested it
  if (icarus->GetGame()->PlayIcarusSound(task->GetGUID(), m_ownerID, sVal2,
                                         sVal))
    Completed(task->GetGUID());
...
int CTaskManager::Rotate(CTask *task, CIcarus *icarus) {
  ...
  if (Check(CIcarus::ID_TAG, block, memberNum)) {
    ...
    if (icarus->GetGame()->GetTag(m_ownerID, tagName, (int)tagLookup, vector) ==
        false) {
      // TODO: Emit warning
      icarus->GetGame()->DebugPrint(IGameInterface::WL_ERROR,
                                    "Unable to find tag \"%s\"!\n", tagName);
  ...
  icarus->GetGame()->DebugPrint(
      IGameInterface::WL_DEBUG, "%4d rotate( <%f,%f,%f>, %d); [%d]", m_ownerID,
      vector[0], vector[1], vector[2], (int)duration, task->GetTimeStamp());
...
int CTaskManager::Camera(CTask *task, CIcarus *icarus) {
  ...
  case CIcarus::TYPE_PAN:
    ...
    icarus->GetGame()->DebugPrint(
        ...
        "%4d camera( PAN, <%f %f %f>, <%f %f %f>, %f); [%d]", m_ownerID,
        vector[0], vector[1], vector[2], vector2[0], vector2[1], vector2[2],
  ...
  case CIcarus::TYPE_ZOOM:
    ...
    icarus->GetGame()->DebugPrint(IGameInterface::WL_DEBUG,
                                  "%4d camera( ZOOM, %f, %f); [%d]", m_ownerID,
  ...
  case CIcarus::TYPE_MOVE:
    ...
    icarus->GetGame()->DebugPrint(
        IGameInterface::WL_DEBUG, "%4d camera( MOVE, <%f %f %f>, %f); [%d]",
        m_ownerID, vector[0], vector[1], vector[2], fVal, task->GetTimeStamp());
  ...
  case CIcarus::TYPE_ROLL:
    ...
    icarus->GetGame()->DebugPrint(IGameInterface::WL_DEBUG,
                                  "%4d camera( ROLL, %f, %f); [%d]", m_ownerID,
                                  fVal, fVal2, task->GetTimeStamp());
  ...
  case CIcarus::TYPE_FOLLOW:
    ...
    icarus->GetGame()->DebugPrint(
        IGameInterface::WL_DEBUG, "%4d camera( FOLLOW, \"%s\", %f, %f); [%d]",
  ...
  case CIcarus::TYPE_TRACK:
    ...
    icarus->GetGame()->DebugPrint(
        IGameInterface::WL_DEBUG, "%4d camera( TRACK, \"%s\", %f, %f); [%d]",
        m_ownerID, sVal, fVal, fVal2, task->GetTimeStamp());
  ...
  case CIcarus::TYPE_DISTANCE:
    ...
    icarus->GetGame()->DebugPrint(IGameInterface::WL_DEBUG,
                                  "%4d camera( DISTANCE, %f, %f); [%d]",
                                  m_ownerID, fVal, fVal2, task->GetTimeStamp());
  ...
  case CIcarus::TYPE_FADE:
    ...
    icarus->GetGame()->CameraFade(vector[0], vector[1], vector[2], fVal,
...
int CTaskManager::Move(CTask *task, CIcarus *icarus) {
  ...
  if (GetVector(m_ownerID, block, memberNum, vector2, icarus) == false) {
    ...
    icarus->GetGame()->DebugPrint(
        IGameInterface::WL_DEBUG, "%4d move( <%f %f %f>, %f ); [%d]", m_ownerID,
        vector[0], vector[1], vector[2], duration, task->GetTimeStamp());
    icarus->GetGame()->Lerp2Pos(task->GetGUID(), m_ownerID, vector, NULL,
                                duration);
  ...
  icarus->GetGame()->DebugPrint(
      IGameInterface::WL_DEBUG, "%4d move( <%f %f %f>, <%f %f %f>, %f ); [%d]",
      m_ownerID, vector[0], vector[1], vector[2], vector2[0], vector2[1],
      vector2[2], duration, task->GetTimeStamp());
  icarus->GetGame()->Lerp2Pos(task->GetGUID(), m_ownerID, vector, vector2,
                              duration);
...
int CTaskManager::Play(CTask *task, CIcarus *icarus) {
  ...
  icarus->GetGame()->DebugPrint(IGameInterface::WL_DEBUG,
                                "%4d play( \"%s\", \"%s\" ); [%d]", m_ownerID,
                                sVal, sVal2, task->GetTimeStamp());
  icarus->GetGame()->Play(task->GetGUID(), m_ownerID, (const char *)sVal,
                          (const char *)sVal2);
...
void CTaskManager::Load(CIcarus *icarus) {
  ...
  for (int i = 0; i < numTasks; i++) {
    ...
    for (int j = 0; j < numMembers; j++) {
      ...
      case CIcarus::ID_RANDOM:
        block->Write(CIcarus::ID_RANDOM, *(float *)bData,
                     icarus); // ID_RANDOM );
        break;
  ...
  // icarus->GetGame()->ReadSaveData( 'TG#G', &numTaskGroups, sizeof(
  // numTaskGroups ) );
  ...
  for (int i = 0; i < numTaskGroups; i++) {
    ...
    if (id != -1)
      taskGroup->m_parent =
          (GetTaskGroup(id, icarus) != NULL) ? GetTaskGroup(id, icarus) : NULL;
    ...
    pIcarus->BufferRead(&taskGroup->m_numCompleted,
                        sizeof(taskGroup->m_numCompleted));
  }
