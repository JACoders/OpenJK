...
bool Sys_FileOutOfDate(LPCSTR psFinalFileName /* dest */, LPCSTR psDataFileName /* src */) {
	...
	if (Sys_GetFileTime(psFinalFileName, ftFinalFile) && Sys_GetFileTime(psDataFileName, ftDataFile)) {
		...
		if ((fabs((double)(ftFinalFile.dwLowDateTime - ftDataFile.dwLowDateTime)) <= 20000000) && ftFinalFile.dwHighDateTime == ftDataFile.dwHighDateTime) {
				...
				do {
					...
					if (!FS_FilenameCompare(pakFile->name, filename)) {
						...
						if (!(pak->referenced & FS_GENERAL_REF)) {
							if (!FS_IsExt(filename, ".shader", l) && !FS_IsExt(filename, ".txt", l) && !FS_IsExt(filename, ".str", l) &&
								!FS_IsExt(filename, ".cfg", l) && !FS_IsExt(filename, ".config", l) && !FS_IsExt(filename, ".bot", l) &&
								!FS_IsExt(filename, ".arena", l) && !FS_IsExt(filename, ".menu", l) && !FS_IsExt(filename, ".fcf", l) &&
						...
						if (!(pak->referenced & FS_CGAME_REF)) {
							if (Q_stricmp(filename, "cgame.qvm") == 0 || Q_stricmp(filename, "cgamex86.dll") == 0) {
								pak->referenced |= FS_CGAME_REF;
							}
						}

						if (!(pak->referenced & FS_UI_REF)) {
							if (Q_stricmp(filename, "ui.qvm") == 0 || Q_stricmp(filename, "uix86.dll") == 0) {
