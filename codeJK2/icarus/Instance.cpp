...
int ICARUS_Instance::SaveSequenceIDTable(void) {
  ...
  ojk::SavedGameHelper saved_game(m_interface->saved_game);

  saved_game.write_chunk<int32_t>(INT_ID('#', 'S', 'E', 'Q'), numSequences);
  ...
  saved_game.write_chunk<int32_t>(INT_ID('S', 'Q', 'T', 'B'), idTable,
                                  numSequences);
...
int ICARUS_Instance::SaveSequencers(void) {
  ...
  saved_game.write_chunk<int32_t>(INT_ID('#', 'S', 'Q', 'R'), numSequencers);

  // The sequencers are then saved
  sequencer_l::iterator si;
  STL_ITERATE(si, m_sequencers) { (*si)->Save(); }
...
int ICARUS_Instance::SaveSignals(void) {
  ...
  saved_game.write_chunk<int32_t>(INT_ID('I', 'S', 'I', 'G'), numSignals);

  signal_m::iterator si;
  STL_ITERATE(si, m_signals) {
    // m_interface->I_WriteSaveData( INT_ID('I','S','I','G'), &numSignals,
    // sizeof( numSignals ) );
    ...
    saved_game.write_chunk<int32_t>(INT_ID('S', 'I', 'G', '#'), length);

    // Write out the string
    saved_game.write_chunk(INT_ID('S', 'I', 'G', 'N'), name, length);
  }
...
int ICARUS_Instance::Save(void) {
  ...
  saved_game.write_chunk<double>(INT_ID('I', 'E', 'N', 'D'), version);
...
int ICARUS_Instance::LoadSignals(void) {
  ...
  saved_game.read_chunk<int32_t>(INT_ID('I', 'S', 'I', 'G'), numSignals);
  ...
  for (int i = 0; i < numSignals; i++) {
    ...
    saved_game.read_chunk<int32_t>(INT_ID('S', 'I', 'G', '#'), length);

    assert(length < (int)sizeof(buffer));

    // Get the string
    saved_game.read_chunk(INT_ID('S', 'I', 'G', 'N'), buffer, length);
...
int ICARUS_Instance::LoadSequences(void) {
  ...
  saved_game.read_chunk<int32_t>(INT_ID('#', 'S', 'E', 'Q'), numSequences);
  ...
  saved_game.read_chunk<int32_t>(INT_ID('S', 'Q', 'T', 'B'), idTable,
                                 numSequences);
...
int ICARUS_Instance::LoadSequencers(void) {
  ...
  saved_game.read_chunk<int32_t>(INT_ID('#', 'S', 'Q', 'R'), numSequencers);
...
int ICARUS_Instance::Load(void) {
  ...
  if (LoadSignals() == false) {
    m_interface->I_DPrintf(WL_ERROR,
  ...
  saved_game.read_chunk<double>(INT_ID('I', 'E', 'N', 'D'), version);
