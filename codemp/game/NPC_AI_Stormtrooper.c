...
void NPC_ST_SayMovementSpeech(void) {
	...
	if (NPCS.NPCInfo->group && NPCS.NPCInfo->group->commander && NPCS.NPCInfo->group->commander->client &&
		NPCS.NPCInfo->group->commander->client->NPC_class == CLASS_IMPERIAL && !Q_irand(0, 3)) { // imperial (commander) gives the order
	...
	if (moved == qfalse) { // FIXME: if we're going to a combat point, need to pick a different one
		if (!trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC,
			...
			if (info.blocker && info.blocker->NPC && NPCS.NPCInfo->group != NULL &&
				info.blocker->NPC->group == NPCS.NPCInfo->group) //(NPCInfo->aiFlags&NPCAI_BLOCKED) && NPCInfo->group != NULL )
			{													 // dammit, something is in our way
				...
				for (j = 0; j < NPCS.NPCInfo->group->numGroup; j++) {
					if (NPCS.NPCInfo->group->member[j].number ==
						NPCS.NPCInfo->blockingEntNum) { // we're being blocked by one of our own, pass our goal onto them and I'll stand still
	...
	if (alertEvent >= 0) {
		...
		if (level.alertEvents[alertEvent].level == AEL_DISCOVERED && (NPCS.NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES)) {
			...
			for (i = 0; i < MAX_CLIENTS; i++) {
				ent = &g_entities[i];
				if (ent->inuse && ent->health > 0 && !(ent->client->ps.eFlags & EF_DEAD) && G_ClearLOS(NPCS.NPC, NPCS.NPC->s.origin, ent->s.origin)) {
...
qboolean NPC_CheckEnemyStealth(gentity_t *target) {
	...
	if (!(target->client->ps.pm_flags & PMF_DUCKED) && (NPCS.NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES) && (target_dist) < (minDist * minDist)) {
	...
	if (NPCS.NPCInfo->stats.visrange > maxViewDist) { // FIXME: should we always just set maxViewDist to this?
	...
	if (clearLOS) {
		...
		if (contents & CONTENTS_WATER) {
			...
			if (!(myContents & CONTENTS_WATER)) {						 // I'm not in water
				if (NPCS.NPC->client->NPC_class == CLASS_SWAMPTROOPER) { // these guys can see in in/through water pretty well
					vis_rating = 0.10f;									 // 10% bonus
				} else {
					vis_rating = 0.35f; // 35% bonus
				}
			} else {													 // else, if we're both in water
				if (NPCS.NPC->client->NPC_class == CLASS_SWAMPTROOPER) { // I can see him just fine
				} else {
		...
		if (NPCS.NPC->client->NPC_class == CLASS_SWAMPTROOPER) { // swamptroopers can see much better
		...
		if (target_rating > cautious && !(NPCS.NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) { // FIXME: ambushing guys should never talk
			if (TIMER_Done(NPCS.NPC, "enemyLastVisible")) {									// If we haven't already, start the counter
	...
	if (NPCS.NPCInfo->confusionTime < level.time) {
		if (level.alertEvents[eventID].level == AEL_DISCOVERED && (NPCS.NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES)) {
			...
			if (!level.alertEvents[eventID].owner || !level.alertEvents[eventID].owner->client || level.alertEvents[eventID].owner->health <= 0 ||
				level.alertEvents[eventID].owner->client->playerTeam != NPCS.NPC->client->enemyTeam) { // not an enemy
				return qfalse;
			}
	...
	if (level.alertEvents[eventID].level > AEL_MINOR && NPCS.NPCInfo->investigateCount > 1 && (NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) {
		...
		if (G_ExpandPointToBBox(NPCS.NPCInfo->investigateGoal, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, NPCS.NPC->s.number,
								((NPCS.NPC->clipmask & ~CONTENTS_BODY) |
								 CONTENTS_BOTCLIP))) { // we were able to move the investigateGoal to a point in which our bbox would fit
			...
			int id = NPC_FindCombatPoint(NPCS.NPCInfo->investigateGoal, NPCS.NPCInfo->investigateGoal, NPCS.NPCInfo->investigateGoal,
										 CP_INVESTIGATE | CP_HAS_ROUTE, 0, -1);
		...
		if (NPCS.NPCInfo->investigateDebounceTime + NPCS.NPCInfo->pauseTime > level.time) { // was already investigating
			if (NPCS.NPCInfo->group && NPCS.NPCInfo->group->commander && NPCS.NPCInfo->group->commander->client &&
				NPCS.NPCInfo->group->commander->client->NPC_class == CLASS_IMPERIAL && !Q_irand(0, 3)) {
...
void NPC_BSST_Investigate(void) {
	...
	if (NPCS.NPCInfo->localState == LSTATE_INVESTIGATE && (NPCS.NPCInfo->goalEntity != NULL)) {
		// See if we're there
		if (NAV_HitNavGoal(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, NPCS.NPCInfo->goalEntity->r.currentOrigin, 32,
...
void NPC_BSST_Patrol(void) { // FIXME: pick up on bodies of dead buddies?
	...
	{
		if (NPCS.NPC->client->NPC_class != CLASS_IMPERIAL && NPCS.NPC->client->NPC_class != CLASS_IMPWORKER) { // imperials do not look around
	...
	if (NPCS.NPC->client->NPC_class == CLASS_IMPERIAL || NPCS.NPC->client->NPC_class == CLASS_IMPWORKER) { // hack
		if (NPCS.ucmd.forwardmove || NPCS.ucmd.rightmove || NPCS.ucmd.upmove) {							   // moving

			if ((NPCS.NPC->client->ps.torsoTimer <= 0) || (NPCS.NPC->client->ps.torsoAnim == BOTH_STAND4)) {
				if ((NPCS.ucmd.buttons & BUTTON_WALKING) && !(NPCS.NPCInfo->scriptFlags & SCF_RUNNING)) { // not running, only set upper anim
...
static void ST_CheckMoveState(void) {
	...
	if (trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC, TID_MOVE_NAV)) { // moving toward a goal that a script is waiting on, so don't stop for anything!
		move = qtrue;
	}
	...
	// see if we're heading to some other combatPoint
		...
		// ucmd.buttons |= BUTTON_CAREFUL;
		if (!NPCS.NPCInfo->goalEntity) { // um, lost our goal?  Just stand and shoot, then
			NPCS.NPCInfo->squadState = SQUAD_STAND_AND_SHOOT;
		}
	}
	...
	else if (NPCS.NPCInfo->squadState == SQUAD_STAND_AND_SHOOT) { // from this squadState we can transition to others?
		move = qfalse;
		return;
	}
	// see if we're hiding
	...
	if ((NPCS.NPCInfo->goalEntity != NPCS.NPC->enemy) && (NPCS.NPCInfo->goalEntity != NULL)) {
		// Did we make it?
		if (NAV_HitNavGoal(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, NPCS.NPCInfo->goalEntity->r.currentOrigin, 16,
						   FlyingCreature(NPCS.NPC)) ||
			(!trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC, TID_MOVE_NAV) && NPCS.NPCInfo->squadState == SQUAD_SCOUT && enemyLOS &&
...
static void ST_CheckFireState(void) {
	...
	if (NPCS.NPCInfo->squadState == SQUAD_RETREAT || NPCS.NPCInfo->squadState == SQUAD_TRANSITION ||
		NPCS.NPCInfo->squadState == SQUAD_SCOUT) { // runners never try to fire at the last pos
		return;
	}

	if (!VectorCompare(NPCS.NPC->client->ps.velocity, vec3_origin)) { // if moving at all, don't do this
		return;
	}
			...
			NPCS.NPCInfo->group->numState[SQUAD_SCOUT] > 0)) // laying down covering fire
	{
		if (level.time - NPCS.NPCInfo->enemyLastSeenTime < 10000 && // we have seem the enemy in the last 10 seconds
			(!NPCS.NPCInfo->group ||
			 level.time - NPCS.NPCInfo->group->lastSeenEnemyTime < 10000)) // we are not in a group or the group has seen the enemy in the last 10 seconds
		{
			if (!Q_irand(0, 10)) {
...
int ST_GetCPFlags(void) {
	...
	if (NPCS.NPC && NPCS.NPCInfo->group) {
		if (NPCS.NPC == NPCS.NPCInfo->group->commander && NPCS.NPC->client->NPC_class == CLASS_IMPERIAL) { // imperials hang back and give orders
			if (NPCS.NPCInfo->group->numGroup > 1 && Q_irand(-3, NPCS.NPCInfo->group->numGroup) > 1) { // FIXME: make sure he;s giving orders with these lines
				if (Q_irand(0, 1)) {
		...
		} else if (NPCS.NPCInfo->group->morale < NPCS.NPCInfo->group->numGroup) { // morale is low for our size
...
void ST_Commander(void) {
	...
	if (group->lastSeenEnemyTime < level.time - 180000) { // dissolve the group
		...
		for (i = 0; i < group->numGroup; i++) {
			...
			if (trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC,
										   TID_MOVE_NAV)) { // running somewhere that a script requires us to go, don't break from that
				continue;
			}
			if (!(NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) { // not allowed to move on my own
				continue;
			}
	...
	*/
	...
	for (i = curMemberNum; i < lastMemberNum; i++) {
		...
		if (trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC, TID_MOVE_NAV)) { // running somewhere that a script requires us to go
			continue;
		}

		if (NPCS.NPC->s.weapon == WP_NONE && NPCS.NPCInfo->goalEntity && NPCS.NPCInfo->goalEntity == NPCS.NPCInfo->tempGoal &&
			NPCS.NPCInfo->goalEntity->enemy && NPCS.NPCInfo->goalEntity->enemy->s.eType == ET_ITEM) { // running to pick up a gun, don't do other logic
			continue;
		}
		...
		if (!(NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) { // not allowed to do combat-movement
			continue;
		}

		// check the local state
		if (NPCS.NPCInfo->squadState != SQUAD_RETREAT) {  // not already retreating
			if (NPCS.NPC->client->ps.weapon == WP_NONE) { // weaponless, should be hiding
				if (NPCS.NPCInfo->goalEntity == NULL || NPCS.NPCInfo->goalEntity->enemy == NULL ||
					NPCS.NPCInfo->goalEntity->enemy->s.eType != ET_ITEM) { // not running after a pickup
					if (TIMER_Done(NPCS.NPC, "hideTime") || (DistanceSquared(group->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin) < 65536 &&
															 NPC_ClearLOS4(NPCS.NPC->enemy))) { // done hiding or enemy near and can see us
			...
			if (TIMER_Done(NPCS.NPC, "roamTime") && TIMER_Done(NPCS.NPC, "hideTime") && NPCS.NPC->health > 10 &&
				!trap->InPVS(group->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin)) { // cant even see enemy
				// better go after him
				cpFlags |= (CP_CLEAR | CP_COVER);
			} else if (NPCS.NPCInfo->localState == LSTATE_UNDERFIRE) { // we've been shot
				...
				if (trap->InPVS(NPCS.NPC->r.currentOrigin, group->enemy->r.currentOrigin)) { // in the same room as enemy
					if (NPCS.NPC->client->ps.weapon == WP_ROCKET_LAUNCHER &&
						...
						NPCS.NPCInfo->squadState != SQUAD_TRANSITION) { // too close for me to fire my weapon and I'm not already on the move
		...
		if (!cpFlags) {										 // okay, we have no new enemy-driven reason to run... let's use tactics now
			if (runner && NPCS.NPCInfo->combatPoint != -1) { // someone is running and we have a combat point already
				if (NPCS.NPCInfo->squadState != SQUAD_SCOUT && NPCS.NPCInfo->squadState != SQUAD_TRANSITION &&
					NPCS.NPCInfo->squadState != SQUAD_RETREAT) { // it's not us
					if (TIMER_Done(NPCS.NPC, "verifyCP") &&
						DistanceSquared(NPCS.NPC->r.currentOrigin, level.combatPoints[NPCS.NPCInfo->combatPoint].origin) >
					...
					if (NPCS.NPCInfo->aiFlags & NPCAI_BLOCKED) { // dammit, something is in our way
						// see if it's one of ours
				...
				if (NPCS.NPCInfo->combatPoint != -1) { // we have a combat point we're supposed to be running to
					if (NPCS.NPCInfo->squadState != SQUAD_SCOUT && NPCS.NPCInfo->squadState != SQUAD_TRANSITION &&
						NPCS.NPCInfo->squadState != SQUAD_RETREAT) { // but we're not running
						if (TIMER_Done(NPCS.NPC, "verifyCP")) { // 1 - 3 seconds have passed since you chose a CP, see if you're there since, for some reason,
																// you've stopped running...
							if (DistanceSquared(NPCS.NPC->r.currentOrigin, level.combatPoints[NPCS.NPCInfo->combatPoint].origin) >
				...
				if (enemyLost) { // if no-one has seen the enemy for a while, send a scout
					...
					// closest one
					if (TIMER_Done(NPCS.NPC, "roamTime") && !Q_irand(0, group->numGroup)) { // only do this if we're ready to move again and we feel like it
					...
					{										   // we're ready to move
						if (NPCS.NPCInfo->combatPoint == -1) { // we're not on a combat point
							if (1)							   //! Q_irand( 0, 2 ) )
						...
						} else if (TIMER_Done(NPCS.NPC, "roamTime")) {						   // we are already on a combat point
					...
					if (!cpFlags) { // still not moving
						...
						*/
						...
						if (TIMER_Done(NPCS.NPC, "duck")) {		 // not ducking
							if (TIMER_Done(NPCS.NPC, "stand")) { // don't have to keep standing
								if (NPCS.NPCInfo->combatPoint == -1 || (level.combatPoints[NPCS.NPCInfo->combatPoint].flags & CPF_DUCK)) { // okay to duck here
									if (!Q_irand(0, 3)) {
		...
		if (cpFlags) { // we want to run to a combat point
			...
			*/
			...
			if (cp == -1) { // may have had sone set above
				cp = NPC_FindCombatPoint(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.currentOrigin, group->enemy->r.currentOrigin, cpFlags | CP_HAS_ROUTE, avoidDist,
			...
			} else if (NPCS.NPCInfo->squadState == SQUAD_SCOUT) { // we couldn't find a combatPoint by the player, so just go after him directly
...
void NPC_BSST_Attack(void) {
	...
	if (NPCS.NPCInfo->group) {				   // I belong to a squad of guys - we should *always* have a group
		if (!NPCS.NPCInfo->group->processed) { // I'm the first ent in my group, I'll make the command decisions
#if AI_TIMERS
	...
	if (!(NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) { // not supposed to chase my enemies
		if (NPCS.NPCInfo->goalEntity == NPCS.NPC->enemy) {	// goal is my entity, so don't move
			move = qfalse;
		}
	}
	...
	if (NPCS.NPC->enemy && NPCS.NPC->enemy->enemy) {
		if (NPCS.NPC->enemy->s.weapon == WP_SABER &&
			NPCS.NPC->enemy->enemy->s.weapon == WP_SABER) { // don't shoot at an enemy jedi who is fighting another jedi, for fear of injuring one or causing
															// rogue blaster deflections (a la Obi Wan/Vader duel at end of ANH)
			shoot = qfalse;
		}
	}
	...
	if (NPCS.NPC->client->ps.weaponTime > 0) {
		...
		if (TIMER_Done(NPCS.NPC, "attackDelay")) {
			...
			// NASTY
			if (NPCS.NPC->s.weapon == WP_ROCKET_LAUNCHER && (NPCS.ucmd.buttons & BUTTON_ATTACK) && !move && g_npcspskill.integer > 1 &&
