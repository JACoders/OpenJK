...
void G_WriteClientSessionData(gclient_t *client) {
  ...
  for (i = 0; i < MAX_OBJECTIVES; i++) {
    s2 = va("%s %i %i", s2, client->sess.mission_objectives[i].display,
  ...
  s2 = va("%i %i %i %i %i %i %i %i %i %i %i %i",
          ...
          client->sess.missionStats.shotsFired, client->sess.missionStats.hits,
...
void G_ReadSessionData(gclient_t *client) {
  ...
  var++;
  for (i = 0; i < MAX_OBJECTIVES; i++) {
    sscanf(var, "%i %i", &client->sess.mission_objectives[i].display,
           &client->sess.mission_objectives[i].status);
    var += 4;
  }
