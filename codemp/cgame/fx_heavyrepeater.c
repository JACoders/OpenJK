...
void FX_RepeaterProjectileThink(centity_t *cent,
  ...
  trap->FX_PlayEffectID(cgs.effects.repeaterProjectileEffect, cent->lerpOrigin,
                        forward, -1, -1, qfalse);
}
...
void FX_RepeaterHitWall(vec3_t origin, vec3_t normal) {
  trap->FX_PlayEffectID(cgs.effects.repeaterWallImpactEffect, origin, normal,
                        -1, -1, qfalse);
}
...
void FX_RepeaterHitPlayer(vec3_t origin, vec3_t normal, qboolean humanoid) {
  trap->FX_PlayEffectID(cgs.effects.repeaterFleshImpactEffect, origin, normal,
                        -1, -1, qfalse);
}
...
void FX_RepeaterAltProjectileThink(centity_t *cent,
  ...
  trap->FX_PlayEffectID(cgs.effects.repeaterAltProjectileEffect,
...
void FX_RepeaterAltHitWall(vec3_t origin, vec3_t normal) {
  trap->FX_PlayEffectID(cgs.effects.repeaterAltWallImpactEffect, origin, normal,
                        -1, -1, qfalse);
}
