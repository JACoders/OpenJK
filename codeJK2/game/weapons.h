...
{ WP_NONE,
  ...
  // Player weapons
  WP_SABER, // NOTE: lots of code assumes this is the first weapon (... which is
            // crap) so be careful -Ste.
  WP_BRYAR_PISTOL,
  WP_BLASTER,
  WP_DISRUPTOR,
  WP_BOWCASTER,
  ...
  // NOTE: player can only have up to 16 weapons, anything after that is enemy
  // only
  ...
  //	WP_CHAOTICA_GUARD_GUN,	//			- B/W version of scav rifle for
  //Chaotica's guards. 	WP_BOT_ROCKET,		// Hunter Seeker - Rocket
  //projectile 	WP_KLINGON_BLADE, 	WP_IMPERIAL_BLADE,
...
#define TD_TEST_RAD                                                            \
  (weaponData[WP_THERMAL].splashRadius *                                       \
   0.8f) // no sense in auto-blowing up if exactly on the radius edge--it would
