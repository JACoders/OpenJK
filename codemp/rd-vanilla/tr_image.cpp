...
//#define FILE_HASH_SIZE		1024	// actually the shader code still
//needs this (from another module, great), static	image_t*
// hashTable[FILE_HASH_SIZE];
...
static void Upload32(unsigned *data, GLenum format, qboolean mipmap,
  ...
  if (format == GL_RGBA) {
    ...
    *pUploadHeight = height;
    ...
    if (!mipmap) {
      qglTexImage2D(uiTarget, 0, *pformat, width, height, 0, GL_RGBA,
    ...
    if (mipmap) {
      ...
      while (width > 1 || height > 1) {
        ...
        qglTexImage2D(uiTarget, miplevel, *pformat, width, height, 0, GL_RGBA,
                      GL_UNSIGNED_BYTE, data);
      }
    }
  } else {
...
void R_Images_DeleteLightMaps(void) {
  for (AllocatedImages_t::iterator itImage = AllocatedImages.begin();
    ...
    if (pImage->imgName[0] == '*' &&
        strstr(pImage->imgName, "lightmap")) // loose check, but should be ok
    {
...
void RE_RegisterImages_Info_f(void) {
  ...
  while ((pImage = R_Images_GetNextIteration()) != NULL) {
    ...
    ri.Printf(PRINT_DEVELOPER, S_COLOR_RED ", levused %d",
              pImage->iLastLevelUsedOn);
    ri.Printf(PRINT_ALL, "\n");
  ...
  ri.Printf(PRINT_DEVELOPER, S_COLOR_RED "RE_RegisterMedia_GetLevel(): %d",
...
qboolean RE_RegisterImages_LevelLoadEnd(void) {
  ...
  //	int iNumImages = AllocatedImages.size();	// more for curiosity,
  //really.
  ...
  //	{
  //		ri.Printf( PRINT_ALL, S_COLOR_YELLOW  "Level uses %d images, old
  //limit was MAX_DRAWIMAGES (%d)\n", iNumImages, MAX_DRAWIMAGES);
  //	}

  ri.Printf(PRINT_DEVELOPER,
            S_COLOR_RED "RE_RegisterImages_LevelLoadEnd(): Ok\n");
...
image_t *R_CreateImage(const char *name, const byte *pic, int width, int height,
                       ...
                       qboolean allowTC, int glWrapClampMode, bool bRectangle) {
  image_t *image;
  ...
  qglBindTexture(uiTarget, 0); // jfm: i don't know why this is here, but it
                               // breaks lightmaps when there's only 1
...
image_t *R_FindImageFile(const char *name, qboolean mipmap,
  ...
  if (!name || ri.Cvar_VariableIntegerValue(
                   "dedicated")) // stop ghoul2 horribleness as regards image
                                 // loading from server
  {
    return NULL;
  }
  ...
  if ((width & (width - 1)) || (height & (height - 1))) {
    ri.Printf(PRINT_ALL,
              "Refusing to load non-power-2-dims(%d,%d) pic \"%s\"...\n", width,
              height, name);
    return NULL;
  }
...
static void R_CreateDefaultImage(void) {
  ...
  tr.defaultImage =
      R_CreateImage("*default", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE,
                    GL_RGBA, qtrue, qfalse, qfalse, GL_REPEAT);
}
...
void R_CreateBuiltinImages(void) {
  ...
  if (glConfigExt.doGammaCorrectionWithShaders) {
    ...
    qglTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA8, 64, 64, 64, 0, GL_RGBA,
  ...
  for (x = 0; x < NUM_SCRATCH_IMAGES; x++) {
    ...
    tr.scratchImage[x] =
        R_CreateImage(va("*scratch%d", x), (byte *)data, DEFAULT_SIZE,
                      DEFAULT_SIZE, GL_RGBA, qfalse, qtrue, qfalse, GL_CLAMP);
  }
...
void R_SetGammaCorrectionLUT() {
  if (glConfigExt.doGammaCorrectionWithShaders) {
    ...
    qglTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, 0, 64, 64, 64, GL_RGB,
                     GL_UNSIGNED_BYTE, lutTable);
