...
extern int G_PickPainAnim(gentity_t *self, const vec3_t point, int damage,
                          int hitLoc);
void NPC_ChoosePainAnimation(gentity_t *self, gentity_t *other,
                             const vec3_t point, int damage, int mod,
                             int hitLoc, int voiceEvent = -1) {
  ...
  if (self->s.weapon == WP_THERMAL &&
    ...
    if (other && (other->s.weapon == WP_SABER || mod == MOD_ELECTROCUTE ||
      ...
      if (self->client->ps.weapon == WP_SABER &&
          other->s.number <
              MAX_CLIENTS) { // hmm, shouldn't *always* react to damage from
                             // player if I have a saber
  ...
  if (Q_flrand(0.0f, 1.0f) < pain_chance) {
    ...
    if ((self->client->ps.eFlags & EF_FORCE_GRIPPED)) {
      ...
      // SIGH... because our choke sounds are inappropriately long, I have to
      // debounce them in code!
    ...
    } else { // not being force-gripped or force-drained
      if (G_CheckForStrongAttackMomentum(self) ||
        ...
        self->client->ps.saberAnimLevel =
            SS_FAST; // next attack must be a quick attack
...
gentity_t *G_CheckControlledTurretEnemy(gentity_t *self, gentity_t *enemy,
                                        qboolean validate) {
  if (enemy->e_UseFunc == useF_emplaced_gun_use ||
      enemy->e_UseFunc == useF_eweb_use) {
    if (enemy->activator &&
        enemy->activator
            ->client) { // return the controller of the eweb/emplaced gun
...
extern void Boba_Pain(gentity_t *self, gentity_t *inflictor, int damage,
                      int mod);
/*
===============
NPC_Pain
...
void NPC_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
  ...
  if (self->client->playerTeam && other->client &&
    ...
    if (other != self->enemy &&
      ...
      {                                            // dammit, stop that!
        if (self->NPC->charmedTime > level.time) { // mindtricked
          return;
  ...
  if (NPCInfo->ignorePain == qfalse) {
    ...
    if (NPC->ghoul2.size() && NPC->headBolt != -1) {
      G_StopEffect("force/confusion", NPC->playerModel, NPC->headBolt,
                   NPC->s.number);
    }
    if (damage != -1) { //-1 == don't play pain anim
      // Set our proper pain animation
...
void NPC_Touch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (NPC->client->NPC_class == CLASS_RANCOR) { // rancor
    if (NPCInfo->blockedEntity != other &&
        TIMER_Done(NPC, "blockedEntityIgnore")) { // blocked
  ...
  */
  ...
  case CLASS_JAWA:
    G_SoundOnEnt(self, CHAN_VOICE,
                 va("sound/chars/jawa/misc/chatter%d.wav", Q_irand(1, 6)));
    if (self->NPC) {
  ...
  if (event != -1) {
    ...
    qboolean addFlag =
        (qboolean)((self->NPC->scriptFlags & SCF_NO_COMBAT_TALK) != 0);
...
void NPC_Use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  if (self->client && self->NPC) {
    ...
    if (self->client->NPC_class == CLASS_VEHICLE) {
      ...
      if (pVeh && pVeh->m_pVehicleInfo && other && other->client) { // safety
    ...
    */
    ...
    //		}
             ...
             && activator->s.number == 0 && !gi.VoiceVolume[self->s.number] &&
             !(self->NPC->scriptFlags &
