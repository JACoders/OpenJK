...
gentity_t *TossClientItems(gentity_t *self) {
	...
	if (self->client->NPC_class == CLASS_SEEKER || self->client->NPC_class == CLASS_REMOTE || self->client->NPC_class == CLASS_SABER_DROID ||
		self->client->NPC_class == CLASS_VEHICLE || self->client->NPC_class == CLASS_ATST) {
	...
	if (weapon == WP_SABER) {
		...
		} else if (!(self->client->ps.saber[0].saberFlags & SFL_NOT_DISARMABLE) || g_saberPickuppableDroppedSabers->integer) { // okay to drop it
		...
		if (g_saberPickuppableDroppedSabers->integer) {																			// drop your left one, too
			if (self->weaponModel[1] >= 0) {																					// have one in left
				if (!(self->client->ps.saber[0].saberFlags & SFL_NOT_DISARMABLE) || g_saberPickuppableDroppedSabers->integer) { // okay to drop it
					// just drop an item
					if (self->client->ps.saber[1].name && self->client->ps.saber[1].name[0]) { // have a valid string to use for saberType
						// turn it into a pick-uppable item!
						if (G_DropSaberItem(self->client->ps.saber[1].name, self->client->ps.saber[1].blade[0].color, self->client->renderInfo.handLPoint,
											self->client->ps.velocity, self->currentAngles) != NULL) { // dropped it
	...
	} else if (weapon == WP_STUN_BATON || weapon == WP_MELEE) {	  // never drop these
...
qboolean OnSameTeam(gentity_t *ent1, gentity_t *ent2) {
	if (ent1->s.number < MAX_CLIENTS && ent1->client && ent1->client->playerTeam == TEAM_FREE) { // evil player *has* no allies
		return qfalse;
	}
	if (ent2->s.number < MAX_CLIENTS && ent2->client && ent2->client->playerTeam == TEAM_FREE) { // evil player *has* no allies
		return qfalse;
	}
...
void G_StartMatrixEffect(gentity_t *ent, int meFlags = 0, int length = 1000, float timeScale = 0.0f, int spinTime = 0) {
	...
	if (g_timescale->value != 1.0 || in_camera) { // already in some slow-mo mode or in_camera
		return;
	}
...
qboolean G_GetHitLocFromSurfName(gentity_t *ent, const char *surfName, int *hitLoc, vec3_t point, vec3_t dir, vec3_t bladeDir, int mod, saberType_t saberType) {
	...
	*hitLoc = HL_NONE;
	...
	} else if (mod == MOD_SABER && WP_BreakSaber(ent, surfName, saberType)) { // saber hit and broken
		*hitLoc = HL_HAND_RT;
	} else if (!Q_stricmpn("r_hand", surfName, 6) || !Q_stricmpn("w_", surfName, 2)) { // right hand or weapon
	...
	if (g_saberRealisticCombat->integer > 1 || debug_subdivision->integer) {
		...
		if (dir && (dir[0] || dir[1] || dir[2]) && bladeDir &&
			...
			if (g_dismemberProbabilities->value <= 0.0f || G_Dismemberable(ent, *hitLoc)) { // the probability let us continue
...
void LimbThink(gentity_t *ent) { // FIXME: just use object thinking?
	...
	gi.trace(&tr, ent->currentOrigin, ent->mins, ent->maxs, origin, ent->owner ? ent->owner->s.number : ENTITYNUM_NONE, ent->clipmask, (EG2_Collision)0, 0);
...
void G_RemoveWeaponsWithLimbs(gentity_t *ent, gentity_t *limb, int limbAnim) {
	...
	for (weaponModelNum = 0; weaponModelNum < MAX_INHAND_WEAPONS; weaponModelNum++) {
		if (ent->weaponModel[weaponModelNum] >= 0) { // have a weapon in this hand
			if (weaponModelNum == 0 &&
				ent->client->ps.saberInFlight) { // this is the right-hand weapon and it's a saber in-flight (i.e.: not in-hand, so no need to worry about it)
				continue;
			}
			...
			{																		// FIXME: is this first check needed with this lower one?
				if (!gi.G2API_GetSurfaceRenderStatus(&limb->ghoul2[0], handName)) { // only copy the weapon over if the hand is actually on this limb...
					// copy it to limb
					...
					if (!ent->client->ps.saberInFlight) { // saberent isn't flying through the air, it's in-hand and attached to player
						if (ent->client->ps.saberEntityNum != ENTITYNUM_NONE &&
							ent->client->ps.saberEntityNum > 0) { // remove the owner ent's saber model and entity
					...
					if (ent->weaponModel[weaponModelNum] > 0) { // still a weapon associated with it, remove it from the limb
				...
				if (ent->weaponModel[weaponModelNum] > 0) { // but a weapon was copied over to the limb, so remove it
					gi.G2API_RemoveGhoul2Model(limb->ghoul2, ent->weaponModel[weaponModelNum]);
					limb->weaponModel[weaponModelNum] = -1;
				}
			}
...
static qboolean G_Dismember(gentity_t *ent, vec3_t point, const char *limbBone, const char *rotateBone, const char *limbName, const char *limbCapName,
	...
	if (gi.G2API_GetSurfaceRenderStatus(&ent->ghoul2[ent->playerModel], limbName) == 0x00000100 /*G2SURFACEFLAG_NODESCENDANTS*/) { // already lost this limb
...
qboolean G_GetRootSurfNameWithVariant(gentity_t *ent, const char *rootSurfName, char *returnSurfName, int returnSize) {
	if (!gi.G2API_GetSurfaceRenderStatus(&ent->ghoul2[ent->playerModel], rootSurfName)) { // see if the basic name without variants is on
...
qboolean G_DoDismemberment(gentity_t *self, vec3_t point, int mod, int damage, int hitLoc, qboolean force = qfalse) {
	...
	{																						   // FIXME: don't do strcmps here
		if (G_StandardHumanoid(self) && (force || g_dismemberProbabilities->value > 0.0f ||
										 G_Dismemberable2(self, hitLoc))) { // either it's a forced dismemberment or we're using probabilities (which are
																			// checked before this) or we've done enough damage to this location
			// FIXME: check the hitLoc and hitDir against the cap tag for the place
			// where the split will be- if the hit dir is roughly perpendicular to
			// the direction of the cap, then the split is allowed, otherwise we
			// hit it at the wrong angle and should not dismember...
...
static int G_CheckSpecialDeathAnim(gentity_t *self, vec3_t point, int damage, int mod, int hitLoc) {
	...
	if (self->client->ps.legsAnim == BOTH_GETUP_BROLL_L || self->client->ps.legsAnim == BOTH_GETUP_BROLL_R) { // rolling away to the side on our back
		deathAnim = BOTH_DEATH_LYING_UP;
	} else if (self->client->ps.legsAnim == BOTH_GETUP_FROLL_L || self->client->ps.legsAnim == BOTH_GETUP_FROLL_R) { // rolling away to the side on our front
		deathAnim = BOTH_DEATH_LYING_DN;
	} else if (self->client->ps.legsAnim == BOTH_GETUP_BROLL_F && self->client->ps.legsAnimTimer > 350) { // kicking up
		deathAnim = BOTH_DEATH_FALLING_UP;
	} else if (self->client->ps.legsAnim == BOTH_GETUP_BROLL_B && self->client->ps.legsAnimTimer > 950) { // on back, rolling back to get up
		deathAnim = BOTH_DEATH_LYING_UP;
	...
	} else if (self->client->ps.legsAnim == BOTH_GETUP_FROLL_B && self->client->ps.legsAnimTimer <= 1100 &&
			   self->client->ps.legsAnimTimer > 250) { // flipping over backwards
		deathAnim = BOTH_FALLDEATH1LAND;
...
static int G_PickDeathAnim(gentity_t *self, vec3_t point, int damage, int mod,
	...
	if (deathAnim == -1 || !PM_HasAnimation(self, deathAnim)) {
		if (deathAnim == BOTH_DEADFLOP1 || deathAnim == BOTH_DEADFLOP2) { // if don't have deadflop, don't do anything
			deathAnim = -1;
		} else {
...
void player_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath, int dflags, int hitLoc) {
	...
	if (self->client && self->client->NPC_class != CLASS_VEHICLE && self->s.m_iVehicleNum != 0) {
		...
		if (pVeh) {
			if (pVeh->m_pOldPilot != self && pVeh->m_pPilot != self) { // whaaa?  I'm not on this bike?  er....
	...
	if (self->client->ps.saberEntityNum != ENTITYNUM_NONE && self->client->ps.saberEntityNum > 0) {
		if (self->client->ps.saberInFlight) { // just drop it
			...
			if (g_saberPickuppableDroppedSabers->integer) { // always drop your sabers
				...
				self->client->ps.weapon = self->s.weapon = WP_NONE;
			} else if (((hitLoc != HL_HAND_RT && hitLoc != HL_CHEST_RT && hitLoc != HL_ARM_RT && hitLoc != HL_BACK_LT) || self->client->dismembered ||
					   ...
					   (Q_irand(0, 1) || meansOfDeath == MOD_EXPLOSIVE || meansOfDeath == MOD_REPEATER_ALT || meansOfDeath == MOD_FLECHETTE_ALT ||
						meansOfDeath == MOD_ROCKET || meansOfDeath == MOD_ROCKET_ALT || meansOfDeath == MOD_CONC || meansOfDeath == MOD_CONC_ALT ||
						meansOfDeath == MOD_THERMAL || meansOfDeath == MOD_THERMAL_ALT || meansOfDeath == MOD_DETPACK || meansOfDeath == MOD_LASERTRIP ||
						meansOfDeath == MOD_LASERTRIP_ALT || meansOfDeath == MOD_MELEE || meansOfDeath == MOD_FORCE_GRIP || meansOfDeath == MOD_KNOCKOUT ||
						meansOfDeath == MOD_CRUSH || meansOfDeath == MOD_IMPACT || meansOfDeath == MOD_FALLING ||
	...
	if (attacker) {
		if (attacker->client && !attacker->s.number) {
			if (self->client) { // killed a client
				if (self->client->playerTeam == TEAM_ENEMY || self->client->playerTeam == TEAM_FREE ||
					(self->NPC && self->NPC->charmedTime > level.time)) { // killed an enemy
		...
		if (d_slowmodeath->integer) {
			if (!self->s.number) { // what the hell, always do slow-mo when player dies
				...
				if (!attacker->s.number && (holdingSaber || self->client->NPC_class == CLASS_WAMPA) && meansOfDeath == MOD_SABER && attacker->client &&
					attacker->client->ps.weapon == WP_SABER &&
					!attacker->client->ps
						 .saberInFlight // FIXME: if dualSabers, should still do slowmo if this killing blow was struck with the left-hand saber...
					&& (d_slowmodeath->integer > 2 || lastInGroup)) // either slow mo death level 3 (any jedi) or 2 and I was the last jedi in the room
				{													// Matrix!
					if (attacker->client->ps.torsoAnim == BOTH_A6_SABERPROTECT) { // don't override the range and vertbob
				...
				if (!attacker->s.number && meansOfDeath == MOD_SABER && attacker->client && attacker->client->ps.weapon == WP_SABER &&
					...
					(d_slowmodeath->integer > 4 || lastInGroup || holdingSaber ||
					 self->client->NPC_class ==
						 CLASS_WAMPA)) // either slow mo death level 5 (any enemy) or 4 and I was the last in my group or I'm a saber user
				{					   // Matrix!
					if (attacker->client->ps.torsoAnim == BOTH_A6_SABERPROTECT) { // don't override the range and vertbob
						G_StartMatrixEffect(self, (MEF_NO_RANGEVAR | MEF_NO_VERTBOB));
					} else {
	...
	} else if (self->s.weapon != WP_BRYAR_PISTOL) { // since player can't pick up bryar pistols, never drop those
		self->s.weapon = WP_NONE;
	...
	*/
	...
	if ((self->client->ps.eFlags & EF_HELD_BY_RANCOR) || (self->client->ps.eFlags & EF_HELD_BY_SAND_CREATURE) ||
		...
		if (anim == -1) {
			if (PM_InOnGroundAnim(&self->client->ps) && PM_HasAnimation(self, BOTH_LYINGDEATH1)) { // on ground, need different death anim
				anim = BOTH_LYINGDEATH1;
			} else if (meansOfDeath == MOD_TRIGGER_HURT && (self->s.powerups & (1 << PW_SHOCKED))) { // electrocuted
				anim = BOTH_DEATH17;
			} else if (meansOfDeath == MOD_WATER || meansOfDeath == MOD_GAS || meansOfDeath == MOD_FORCE_DRAIN) { // drowned
				anim = BOTH_DEATH17;
		...
		if (anim == -1) {
			if (meansOfDeath == MOD_ELECTROCUTE || (meansOfDeath == MOD_CRUSH && self->s.eFlags & EF_FORCE_GRIPPED) ||
				(meansOfDeath == MOD_FORCE_DRAIN && self->s.eFlags & EF_FORCE_DRAINED)) { // electrocuted or choked to death
				anim = BOTH_DEATH17;
			} else {
				anim = G_PickDeathAnim(self, self->pos1, damage, meansOfDeath, hitLoc);
			}
		...
		} else if (!PM_HasAnimation(self, anim)) {				 // crap, still missing an anim, so pick one that we do have
			anim = PM_PickAnim(self, BOTH_DEATH1, BOTH_DEATH25); // initialize to good data
		}
		...
		if (meansOfDeath == MOD_KNOCKOUT) {
			...
			if (hitLoc == HL_HEAD && !(dflags & DAMAGE_RADIUS) && meansOfDeath != MOD_REPEATER_ALT && meansOfDeath != MOD_FLECHETTE_ALT &&
				meansOfDeath != MOD_ROCKET && meansOfDeath != MOD_ROCKET_ALT && meansOfDeath != MOD_CONC && meansOfDeath != MOD_THERMAL &&
				meansOfDeath != MOD_THERMAL_ALT && meansOfDeath != MOD_DETPACK && meansOfDeath != MOD_LASERTRIP && meansOfDeath != MOD_LASERTRIP_ALT &&
				meansOfDeath != MOD_EXPLOSIVE && meansOfDeath != MOD_EXPLOSIVE_SPLASH) { // no sound when killed by headshot (explosions don't count)
				...
				if ((self->client->ps.eFlags & EF_FORCE_GRIPPED) || (self->client->ps.eFlags & EF_FORCE_DRAINED)) { // killed while gripped - no loud scream
					G_AlertTeam(self, attacker, 512, 32);
		...
		if (damage != -1 &&
			...
			if ((((mod == MOD_SABER || mod == MOD_MELEE) && self->client->damage_blood) || mod == MOD_CRUSH) &&
				(self->s.weapon == WP_SABER || self->s.weapon == WP_MELEE ||
				 cg.renderingThirdPerson)) // FIXME: not only if using saber, but if in third person at all?  But then 1st/third person functionality is
										   // different...
			{ // FIXME: only strong-level saber attacks should make me play pain anim?
				if (!G_CheckForStrongAttackMomentum(self) && !PM_SpinningSaberAnim(self->client->ps.legsAnim) &&
					...
					if ((parts == SETANIM_BOTH && damage > 30) ||
						(self->painDebounceTime > level.time && damage > 10)) { // took a lot of damage in 1 hit //or took 2 hits in quick succession
...
int CheckArmor(gentity_t *ent, int damage, int dflags, int mod) {
	...
	if (client->NPC_class == CLASS_ASSASSIN_DROID) {
		...
		//-----------------------------------------------------------
		if (mod == MOD_GAS || mod == MOD_IMPACT || mod == MOD_LAVA || mod == MOD_SLIME || mod == MOD_WATER || mod == MOD_FORCE_GRIP || mod == MOD_FORCE_DRAIN ||
			mod == MOD_SEEKER || mod == MOD_MELEE || mod == MOD_BOWCASTER || mod == MOD_BRYAR || mod == MOD_BRYAR_ALT || mod == MOD_BLASTER ||
			mod == MOD_BLASTER_ALT || mod == MOD_SNIPER || mod == MOD_BOWCASTER || mod == MOD_BOWCASTER_ALT || mod == MOD_REPEATER || mod == MOD_REPEATER_ALT) {
			return damage;
		}
...
void G_Knockdown(gentity_t *self, gentity_t *attacker, const vec3_t pushDir, float strength, qboolean breakSaberLock) {
	...
	if (Boba_StopKnockdown(self, attacker, pushDir)) {
		return;
	} else if (Jedi_StopKnockdown(self, attacker, pushDir)) { // They can sometimes backflip instead of be knocked down
		return;
...
void G_CheckKnockdown(gentity_t *targ, gentity_t *attacker, vec3_t newDir, int dflags, int mod) {
	...
	if (!(dflags & DAMAGE_RADIUS)) { // not inherently explosive damage, check mod
		if (mod != MOD_REPEATER_ALT && mod != MOD_FLECHETTE_ALT && mod != MOD_ROCKET && mod != MOD_ROCKET_ALT && mod != MOD_CONC && mod != MOD_CONC_ALT &&
			mod != MOD_THERMAL && mod != MOD_THERMAL_ALT && mod != MOD_DETPACK && mod != MOD_LASERTRIP && mod != MOD_LASERTRIP_ALT && mod != MOD_EXPLOSIVE &&
...
void G_ApplyKnockback(gentity_t *targ, vec3_t newDir, float knockback) {
	...
	if (targ && targ->client &&
		(targ->client->NPC_class == CLASS_ATST || targ->client->NPC_class == CLASS_RANCOR || targ->client->NPC_class == CLASS_SAND_CREATURE ||
		 targ->client->NPC_class == CLASS_WAMPA)) { // much to large to *ever* throw
		return;
	}
...
qboolean G_NonLocationSpecificDamage(int meansOfDeath) {
	if (meansOfDeath == MOD_EXPLOSIVE || meansOfDeath == MOD_REPEATER_ALT || meansOfDeath == MOD_FLECHETTE_ALT || meansOfDeath == MOD_ROCKET ||
		meansOfDeath == MOD_ROCKET_ALT || meansOfDeath == MOD_CONC || meansOfDeath == MOD_THERMAL || meansOfDeath == MOD_THERMAL_ALT ||
		meansOfDeath == MOD_DETPACK || meansOfDeath == MOD_LASERTRIP || meansOfDeath == MOD_LASERTRIP_ALT || meansOfDeath == MOD_MELEE ||
		meansOfDeath == MOD_FORCE_GRIP || meansOfDeath == MOD_KNOCKOUT || meansOfDeath == MOD_CRUSH || meansOfDeath == MOD_EXPLOSIVE_SPLASH) {
		return qtrue;
	}
	return qfalse;
}
...
qboolean G_ImmuneToGas(gentity_t *ent) {
	...
	if (ent->s.weapon == WP_NOGHRI_STICK // assumes user is immune
		|| ent->client->NPC_class == CLASS_HAZARD_TROOPER || ent->client->NPC_class == CLASS_ATST || ent->client->NPC_class == CLASS_GONK ||
		ent->client->NPC_class == CLASS_SAND_CREATURE || ent->client->NPC_class == CLASS_INTERROGATOR || ent->client->NPC_class == CLASS_MARK1 ||
		ent->client->NPC_class == CLASS_MARK2 || ent->client->NPC_class == CLASS_GALAKMECH || ent->client->NPC_class == CLASS_MOUSE ||
		...
		|| ent->client->NPC_class == CLASS_SENTRY || ent->client->NPC_class == CLASS_SWAMPTROOPER || ent->client->NPC_class == CLASS_TUSKEN ||
		ent->client->NPC_class == CLASS_BOBAFETT || ent->client->NPC_class == CLASS_ROCKETTROOPER || ent->client->NPC_class == CLASS_SABER_DROID ||
		ent->client->NPC_class == CLASS_ASSASSIN_DROID || ent->client->NPC_class == CLASS_HAZARD_TROOPER || ent->client->NPC_class == CLASS_VEHICLE) {
		return qtrue;
	}
	return qfalse;
}
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, const vec3_t dir, const vec3_t point, int damage, int dflags, int mod, int hitLoc) {
	...
	{																												// breakable brush or misc_model_breakable
		if (targ->NPC_targetname) {																					// only a certain attacker can destroy this
			if (!attacker || !attacker->targetname || Q_stricmp(targ->NPC_targetname, attacker->targetname) != 0) { // and it's not this one, do nothing
				return;
			}
		}
	}
	...
	} else if (mod == MOD_SABER) { // sabers do less damage to mark1's and atst's, and to hazard troopers and assassin droids
		if (targ->client) {
	...
	if ((client) && (mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT) &&
		(client->NPC_class == CLASS_SABER_DROID || client->NPC_class == CLASS_ASSASSIN_DROID || client->NPC_class == CLASS_GONK ||
		 client->NPC_class == CLASS_MOUSE || client->NPC_class == CLASS_PROBE || client->NPC_class == CLASS_PROTOCOL || client->NPC_class == CLASS_R2D2 ||
		 client->NPC_class == CLASS_R5D2 || client->NPC_class == CLASS_SEEKER || client->NPC_class == CLASS_INTERROGATOR)) {
		damage *= 7;
	}
	...
	if (client && client->NPC_class == CLASS_GALAKMECH && !(dflags & DAMAGE_NO_PROTECTION)) { // hit Galak
		if (client->ps.stats[STAT_ARMOR] > 0) {												  // shields are up
			...
			if ((dflags & DAMAGE_RADIUS) || mod == MOD_REPEATER_ALT || mod == MOD_FLECHETTE_ALT || mod == MOD_ROCKET || mod == MOD_ROCKET_ALT ||
				mod == MOD_CONC || mod == MOD_THERMAL || mod == MOD_THERMAL_ALT || mod == MOD_DETPACK || mod == MOD_LASERTRIP || mod == MOD_LASERTRIP_ALT ||
	...
	{									  // force protections
		...
		if ((targ->client->ps.forcePowersActive & (1 << FP_PROTECT))) {
			...
			*/
			...
			if (mod == MOD_FALLING && targ->NPC && (targ->NPC->aiFlags & NPCAI_DIE_ON_IMPACT)) { // if falling to your death, protect can't save you
			} else {
				qboolean doSound = qfalse;
				switch (mod) {
				case MOD_CRUSH:
					if (attacker && attacker->client) { // need to still be crushed by AT-ST
						break;
					}
				case MOD_REPEATER_ALT:
				case MOD_FLECHETTE_ALT:
	...
	if (targ->client && (targ->client->ps.forcePowersActive & (1 << FP_PROTECT)) &&
		targ->client->ps.forcePowerLevel[FP_PROTECT] == FORCE_LEVEL_3) { // pretend there was no damage?
		knockback = 0;
	...
	*/
	if (client) {
		...
		} else if (targ->client->NPC_class != CLASS_VEHICLE) { // vehicles don't have personal shields
		...
		if (mod == MOD_SLIME || mod == MOD_LAVA) {
			...
			if (mod == MOD_SLIME) {
				...
				// for (int i=0; i<numPuffs; i++)
				{
					...
					if (!testTrace.startsolid && !testTrace.allsolid && testTrace.entityNum == targ->s.number && testTrace.G2CollisionMap[0].mEntityNum != -1) {
		...
		if (targ->client->NPC_class == CLASS_VEHICLE) {
			...
			if ((damage > 0) &&											  // Actually took some damage
				...
				// DISMEMBER THE FRONT PART OF THE MODEL
				{
					...
					gi.G2API_SetBoneAnim(&limb->ghoul2[limb->playerModel], 0, animations[BOTH_A1_BL_TR].firstFrame,
										 animations[BOTH_A1_BL_TR].numFrames + animations[BOTH_A1_BL_TR].firstFrame, BONE_ANIM_OVERRIDE_FREEZE, 1, level.time,
										 -1, -1);
					...
					if (trace.startsolid) {
						...
						if (trace.startsolid) {
							limb->s.pos.trBase[2] += limb->mins[2];
							gi.trace(&trace, limb->s.pos.trBase, limb->mins, limb->maxs, limb->s.pos.trBase, limb->s.number, limb->clipmask, (EG2_Collision)0,
									 0);
						}
					}
			...
			if ((targ->m_pVehicle->m_iArmor <= 0) && targ->m_pVehicle->m_pVehicleInfo->type != VH_ANIMAL) { // vehicle all out of armor
				Vehicle_t *pVeh = targ->m_pVehicle;
				if (dflags & DAMAGE_IMPACT_DIE) {
					// kill it now
					pVeh->m_pVehicleInfo->StartDeathDelay(pVeh, -1 /* -1 causes instant death */);
				} else {
					if (pVeh->m_iDieTime == 0) { // just start the flaming effect and explosion delay, if it's not going already...
	...
	// Undying If:
	//--------------------------------------------------------------------------
	qboolean targUndying = (qboolean)(!alreadyDead && !(dflags & DAMAGE_NO_PROTECTION) &&
	...
	if (targ->client && targ->client->NPC_class == CLASS_WAMPA && targ->count &&
		take >= targ->health) { // wampa holding someone, don't die unless you can release them!
	...
	if (take || (dflags & DAMAGE_NO_DAMAGE)) {
		if (!targ->client || !attacker->client) {
			...
			{																						// on opposite team
				...
				if (targUndying) {
					if (targ->health < 1) {
						if (targ->NPC == NULL || !(targ->NPC->aiFlags & NPCAI_ROSH) ||
							!Rosh_TwinPresent(targ)) { // NOTE: Rosh won't run his deathscript until he doesn't have the twins to heal him
							G_ActivateBehavior(targ, BSET_DEATH);
						}
						targ->health = 1;
					}
				...
				if (takeDamage) {
					targ->health = targ->health - take;
					if (!alreadyDead && ((((targ->flags & FL_UNDYING) || targ->client->ps.forcePowersActive & (1 << FP_RAGE)) &&
					...
					} else if (!alreadyDead && ((((targ->flags & FL_UNDYING) || targ->client->ps.forcePowersActive & (1 << FP_RAGE)) &&
												 !(dflags & DAMAGE_NO_PROTECTION) && !attacker->s.number && !targ->s.number) ||
