...
void NPC_BSSaberDroid_Patrol(void) { // FIXME: pick up on bodies of dead buddies?
...
int SaberDroid_PowerLevelForSaberAnim(gentity_t *self) {
	...
	case BOTH_A2_TR_BL:
		...
		} else if (PM_AnimLength(self->client->clientInfo.animFileIndex, (animNumber_t)self->client->ps.legsAnim) - self->client->ps.torsoAnimTimer <
...
void NPC_SaberDroid_PickAttack(void) {
	...
	if (saberMoveData[NPC->client->ps.saberMove].trailLength > 0) {
		NPC->client->ps.SaberActivateTrail(
			saberMoveData[NPC->client->ps.saberMove].trailLength); // saber trail lasts for 75ms...feel free to change this if you want it longer or shorter
	} else {
...
void NPC_BSSaberDroid_Attack(void) {
	...
	*/
	...
	if (!TIMER_Done(NPC, "taunting")) {
		...
		shoot = qtrue;
		if (enemyDist < (NPC->maxs[0] + NPC->enemy->maxs[0] + 32) * (NPC->maxs[0] + NPC->enemy->maxs[0] + 32)) { // close enough
			doMove = qfalse;
		}
	...
	if (doMove) { // doMove toward goal
		...
		if (doMove) { // if we had to chase him, be sure to attack as soon as possible
