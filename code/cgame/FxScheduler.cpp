...
SLoopedEffect
    gLoopedEffectArray[MAX_LOOPED_FX]; // must be in sync with
                                       // CFxScheduler::mLoopedEffectArray
    ...
    for (size_t iFX = 0; iFX < g_vstrEffectsNeededPerSlot.size(); iFX++) {
      ...
      if (psFX_Filename[0]) {
        ...
        if (mLoopedEffectArray[iFX].mLoopStopTime) {
          mLoopedEffectArray[iFX].mLoopStopTime -=
              mLoopedEffectArray[iFX].mNextTime;
        }
        //
        // and finally reset the time to be the newly-zeroed game time...
        //
        mLoopedEffectArray[iFX].mNextTime =
            0; // otherwise it won't process until game time catches up
      } else {
...
void CFxScheduler::LoadSave_Read() {
  Clean(); // need to get rid of old pre-cache handles, or it thinks it has some
           // older effects when it doesn't
...
void CFxScheduler::LoadSave_Write() {
  ...
  //
  // then cope with the fact that the mID field in each struct of the array
  // we've just saved will not
  //	necessarily point at the same thing when reloading, so save out the
  //actual fx filename strings they 	need for re-registration...
  //
  // since this is only for savegames, and I've got < 2 hours to finish this and
  // test it I'm going to be lazy
  //	with the ondisk data... (besides, the RLE compression will kill most of
  //this anyway)
  //
  for (int iFX = 0; iFX < MAX_LOOPED_FX; iFX++) {
    char sFX_Filename[MAX_QPATH];
    memset(
        sFX_Filename, 0,
        sizeof(
            sFX_Filename)); // instead of "sFX_Filename[0]=0;" so RLE will
                            // squash whole array to nothing, not just stop at
                            // '\0' then have old crap after it to compress

    int &iID = mLoopedEffectArray[iFX].mId;
    if (iID) {
      // now we need to look up what string this represents, unfortunately the
      // existing
      //	lookup table is backwards (keywise) for our needs, so parse the
      //whole thing...
      //
      for (TEffectID::iterator it = mEffectIDs.begin(); it != mEffectIDs.end();
           ++it) {
        if ((*it).second == iID) {
...
int CFxScheduler::ScheduleLoopedEffect(int id, int boltInfo, bool isPortal,
...
#ifdef _DEBUG
//			theFxHelper.Print( "CFxScheduler::ScheduleLoopedEffect-
...
void CFxScheduler::AddLoopedEffects() {
  ...
  for (i = 0; i < MAX_LOOPED_FX; i++) {
    if (mLoopedEffectArray[i].mId &&
      ...
      if (cg_entities[entNum].gent->inuse) { // only play the looped effect when
                                             // the ent is still inUse....
        PlayEffect(mLoopedEffectArray[i].mId, cg_entities[entNum].lerpOrigin, 0,
                   mLoopedEffectArray[i].mBoltInfo, -1,
                   mLoopedEffectArray[i].mPortalEffect, false,
                   mLoopedEffectArray[i]
                       .mIsRelative); // very important to send FALSE looptime
                                      // to not recursively add me!
        mLoopedEffectArray[i].mNextTime =
            theFxHelper.mTime +
            mEffectTemplates[mLoopedEffectArray[i].mId].mRepeatDelay;
      } else {
        theFxHelper.Print("CFxScheduler::AddLoopedEffects- entity was removed "
                          "without stopping any looping fx it owned.");
        memset(&mLoopedEffectArray[i], 0, sizeof(mLoopedEffectArray[i]));
        continue;
      }
...
int CFxScheduler::ParseEffect(const char *file, const CGPGroup &base) {
  ...
  for (auto &property : base.GetProperties()) {
    if (Q::stricmp(property.GetName(), CSTRING_VIEW("repeatDelay")) ==
        Q::Ordering::EQ) {
...
void CFxScheduler::PlayEffect(const char *file, vec3_t origin, vec3_t axis[3],
  ...
  PlayEffect(mEffectIDs[sfile], origin, axis, boltInfo, entNum, isPortal,
...
bool gEffectsInPortal =
    false; // this is just because I don't want to have to add an mPortalEffect
...
void CFxScheduler::CreateEffect(CPrimitiveTemplate *fx, int clientID,
  ...
  case Line:
    ...
    FX_AddLine(clientID, org, org2, fx->mSizeStart.GetVal(),
  ...
  for (i = 0; i < fx->mPrimitiveCount; i++) {
    ...
    for (int t = 0; t < count; t++) {
      ...
      if (delay < 1 && !forceScheduling && !isPortal) {
        ...
        sfx->mEntNum = entityNum; // ent if bolted, else -1 for none, or -2 for
                                  // _Immersion client 0
      ...
      if (effect->mClientID >= 0) {
          ...
          if (cent.gent->ghoul2.IsValid()) {
            if (effect->mModelNum >= 0 &&
              ...
              if (cent.gent->ghoul2[effect->mModelNum].mModelindex >= 0) {
                doesBoltExist =
                    (qboolean)(theFxHelper.GetOriginAxisFromBolt(
                                   cent, effect->mModelNum, effect->mBoltNum,
                                   origin, axis) != 0);
              }
            }
          }
        ...
        if (doesBoltExist) {
          if (effect->mIsRelative) {
            CreateEffect(effect->mpTemplate, vec3_origin, axis, 0,
...
void CFxScheduler::CreateEffect(CPrimitiveTemplate *fx, const vec3_t origin,
                                vec3_t axis[3], int lateTime, int clientID,
  ...
  int flags = fx->mFlags;
  if (clientID >= 0 && modelNum >= 0 &&
      boltNum >=
          0) { // since you passed in these values, mark as relative to use them
    flags |= FX_RELATIVE;
  }
  ...
  case Electricity:
    ...
    FX_AddElectricity(
        clientID, org, org2, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(),
        fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(),
        fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB, eRGB,
        fx->mRGBParm.GetVal(), fx->mElasticity.GetVal(), fx->mLife.GetVal(),
        fx->mMediaHandles.GetHandle(), flags, modelNum, boltNum);
    break;
  ...
  case Cylinder:
    ...
    FX_AddCylinder(clientID, org, ax[0], fx->mSizeStart.GetVal(),
                   fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(),
                   fx->mSize2Start.GetVal(), fx->mSize2End.GetVal(),
                   fx->mSize2Parm.GetVal(), fx->mLengthStart.GetVal(),
  ...
  case Decal:
    ...
    if (fx->mFlags & FX_GHOUL2_DECALS) {
      ...
      theFxHelper.G2Trace(&tr, org, NULL, NULL, end, ENTITYNUM_NONE,
                          MASK_PLAYERSOLID);
        ...
        if (ent != NULL) {
          ...
          if (!(ent->s.eFlags & EF_NODRAW)) { // not drawn, no marks
            ...
            */
            ...
            CG_AddGhoul2Mark(fx->mMediaHandles.GetHandle(),
                             fx->mSizeStart.GetVal(), tr.endpos,
                             tr.plane.normal, tr.entityNum, entOrg, entYaw,
                             ent->ghoul2, ent->s.modelScale,
                             Q_irand(40000, 60000), firstModel);
          }
        }
      }
    }
  ...
  case OrientedParticle:
    ...
    FX_AddOrientedParticle(
        clientID, org, ax[0], vel, accel, fx->mSizeStart.GetVal(),
        fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(),
        fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB, eRGB,
        fx->mRGBParm.GetVal(), fx->mRotation.GetVal(),
        fx->mRotationDelta.GetVal(), fx->mMin, fx->mMax,
        fx->mElasticity.GetVal(), fx->mDeathFxHandles.GetHandle(),
