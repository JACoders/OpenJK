...
int CFxScheduler::ScheduleLoopedEffect(int id, int boltInfo, CGhoul2Info_v *ghoul2, bool isPortal, int iLoopTime, bool isRelative) {
	...
	{
		if (mLoopedEffectArray[i].mId == id && mLoopedEffectArray[i].mBoltInfo == boltInfo && mLoopedEffectArray[i].mPortalEffect == isPortal) {
#ifdef _DEBUG
...
void CFxScheduler::AddLoopedEffects() {
	...
	for (i = 0; i < MAX_LOOPED_FX; i++) {
		if (mLoopedEffectArray[i].mId && mLoopedEffectArray[i].mNextTime < theFxHelper.mTime) {
			...
			PlayEffect(mLoopedEffectArray[i].mId, data->mPoint, 0, mLoopedEffectArray[i].mBoltInfo, mLoopedEffectArray[i].mGhoul2, -1,
...
struct primitiveType_s {
	...
	{"particle", Particle}, {"line", Line},			{"tail", Tail},
	{"sound", Sound},		{"cylinder", Cylinder}, {"electricity", Electricity},
	{"emitter", Emitter},	{"decal", Decal},		{"orientedparticle", OrientedParticle},
	{"fxrunner", FxRunner}, {"light", Light},		{"cameraShake", CameraShake},
	{"flash", ScreenFlash},
};
...
//------------------------------------------------------
static void ReportPlayEffectError(int id) { theFxHelper.Print("CFxScheduler::PlayEffect called with invalid effect ID: %i\n", id); }
...
//	none
//------------------------------------------------------
void CFxScheduler::PlayEffect(const char *file, vec3_t origin, matrix3_t axis, const int boltInfo, CGhoul2Info_v *ghoul2, int fxParm /*-1*/, int vol, int rad,
...
//	none
//------------------------------------------------------
void CFxScheduler::PlayEffect(int id, vec3_t origin, matrix3_t axis, const int boltInfo, CGhoul2Info_v *ghoul2, int fxParm /*-1*/, int vol, int rad,
							  bool isPortal /*false*/, int iLoopTime /*0*/, bool isRelative) {
	SEffectTemplate *fx;
	CPrimitiveTemplate *prim;
	int i = 0;
	int count = 0, delay = 0;
...
void CFxScheduler::AddScheduledEffects(bool portal) {
	...
	for (itr = mFxSchedule.begin(); itr != mFxSchedule.end(); /* do nothing */) {
		...
		if (portal == effect->mPortalEffect && effect->mStartTime <= theFxHelper.mTime) { // only render portal fx on the skyportal pass and vice versa
			if (effect->mBoltNum == -1) {												  // ok, are we spawning a bolt on effect or a normal one?
				if (effect->mEntNum != ENTITYNUM_NONE) {
					...
					CGVM_GetLerpOrigin();
					CreateEffect(effect->mpTemplate, data->mPoint, effect->mAxis, theFxHelper.mTime - effect->mStartTime);
				} else {
				...
				if (doesBoltExist) {
					if (effect->mIsRelative) {
						CreateEffect(effect->mpTemplate, origin, axis, 0, -1, effect->ghoul2, effect->mEntNum, effect->mModelNum, effect->mBoltNum);
					} else {
...
//	none
//------------------------------------------------------
void CFxScheduler::CreateEffect(CPrimitiveTemplate *fx, const vec3_t origin, matrix3_t axis, int lateTime, int fxParm /*-1*/, CGhoul2Info_v *ghoul2, int entNum,
	...
	int flags = fx->mFlags;
	if (ghoul2 != NULL && modelNum >= 0 && boltNum >= 0) { // since you passed in these values, mark as relative to use them if it is supported
		switch (fx->mType) {
		case Particle:
		case Line:
		case Tail:
	...
	if (fx->mType == OrientedParticle) { // bolted oriented particles use origin2 as an angular rotation offset...
		if (flags & FX_RELATIVE) {
	...
	case Particle:
		...
		FX_AddParticle(org, vel, accel, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(),
					   fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB, eRGB, fx->mRGBParm.GetVal(), fx->mRotation.GetVal(), fx->mRotationDelta.GetVal(),
					   fx->mMin, fx->mMax, fx->mElasticity.GetVal(), fx->mDeathFxHandles.GetHandle(), fx->mImpactFxHandles.GetHandle(), fx->mLife.GetVal(),
	...
	case Line:
		...
		FX_AddLine(org, org2, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(), fx->mAlphaEnd.GetVal(),
				   fx->mAlphaParm.GetVal(), sRGB, eRGB, fx->mRGBParm.GetVal(), fx->mLife.GetVal(), fx->mMediaHandles.GetHandle(), flags, fx->mMatImpactFX,
	...
	case Tail:
		...
		FX_AddTail(org, vel, accel, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), fx->mLengthStart.GetVal(), fx->mLengthEnd.GetVal(),
				   fx->mLengthParm.GetVal(), fx->mAlphaStart.GetVal(), fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB, eRGB, fx->mRGBParm.GetVal(),
				   fx->mMin, fx->mMax, fx->mElasticity.GetVal(), fx->mDeathFxHandles.GetHandle(), fx->mImpactFxHandles.GetHandle(), fx->mLife.GetVal(),
				   fx->mMediaHandles.GetHandle(), flags, fx->mMatImpactFX, fxParm, ghoul2, entNum, modelNum, boltNum);
		break;
	...
	case Electricity:
		...
		FX_AddElectricity(org, org2, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(), fx->mAlphaEnd.GetVal(),
						  fx->mAlphaParm.GetVal(), sRGB, eRGB, fx->mRGBParm.GetVal(), fx->mElasticity.GetVal(), fx->mLife.GetVal(),
						  fx->mMediaHandles.GetHandle(), flags, fx->mMatImpactFX, fxParm, ghoul2, entNum, modelNum, boltNum);
		break;
	...
	case Cylinder:
		...
		FX_AddCylinder(org, ax[0], fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), fx->mSize2Start.GetVal(), fx->mSize2End.GetVal(),
					   fx->mSize2Parm.GetVal(), fx->mLengthStart.GetVal(), fx->mLengthEnd.GetVal(), fx->mLengthParm.GetVal(), fx->mAlphaStart.GetVal(),
					   fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB, eRGB, fx->mRGBParm.GetVal(), fx->mLife.GetVal(), fx->mMediaHandles.GetHandle(),
					   flags, fx->mMatImpactFX, fxParm, ghoul2, entNum, modelNum, boltNum, (qboolean)(fx->mSpawnFlags & FX_ORG2_FROM_TRACE));
		break;
	...
	case Emitter:
		...
		FX_AddEmitter(org, vel, accel, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(), fx->mAlphaEnd.GetVal(),
					  ...
					  fx->mVariance.GetVal(), fx->mLife.GetVal(), emitterModel, flags, fx->mMatImpactFX, fxParm);
		break;
	...
	case Decal:
		...
		theFxHelper.AddDecalToScene(fx->mMediaHandles.GetHandle(), org, ax[0], fx->mRotation.GetVal(), sRGB[0], sRGB[1], sRGB[2], fx->mAlphaStart.GetVal(),
	...
	case OrientedParticle:
		...
		FX_AddOrientedParticle(org, ax[0], vel, accel, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(),
							   fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB, eRGB, fx->mRGBParm.GetVal(), fx->mRotation.GetVal(),
							   fx->mRotationDelta.GetVal(), fx->mMin, fx->mMax, fx->mElasticity.GetVal(), fx->mDeathFxHandles.GetHandle(),
							   fx->mImpactFxHandles.GetHandle(), fx->mLife.GetVal(), fx->mMediaHandles.GetHandle(), flags, fx->mMatImpactFX, fxParm, ghoul2,
	...
	case Light:
		...
		FX_AddLight(org, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), sRGB, eRGB, fx->mRGBParm.GetVal(), fx->mLife.GetVal(), flags,
					fx->mMatImpactFX, fxParm, ghoul2, entNum, modelNum, boltNum);
		break;
	...
	case ScreenFlash:
		...
		FX_AddFlash(org, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(), fx->mAlphaEnd.GetVal(),
					fx->mAlphaParm.GetVal(), sRGB, eRGB, fx->mRGBParm.GetVal(), fx->mLife.GetVal(), fx->mMediaHandles.GetHandle(), flags, fx->mMatImpactFX,
					fxParm);
		break;
