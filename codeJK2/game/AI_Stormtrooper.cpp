...
void NPC_ST_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
...
static void ST_HoldPosition(void) {
  ...
  if (!Q3_TaskIDPending(
          NPC, TID_MOVE_NAV)) { // don't have a script waiting for me to get to
                                // my point, okay to stop trying and stand
...
void NPC_ST_SayMovementSpeech(void) {
  ...
  if (NPCInfo->group && NPCInfo->group->commander &&
    ...
    ST_Speech(NPCInfo->group->commander, NPCInfo->movementSpeech,
...
static qboolean ST_Move(void) {
  ...
  if (moved == qfalse) { // FIXME: if we're going to a combat point, need to
    ...
    if (!Q3_TaskIDPending(
            NPC, TID_MOVE_NAV)) { // can't transfer movegoal or stop when a
                                  // script we're running is waiting to complete
      if (info.blocker && info.blocker->NPC && NPCInfo->group != NULL &&
          info.blocker->NPC->group ==
              NPCInfo->group) //(NPCInfo->aiFlags&NPCAI_BLOCKED) &&
                              //NPCInfo->group != NULL )
      {                       // dammit, something is in our way
        ...
        for (int j = 0; j < NPCInfo->group->numGroup; j++) {
          if (NPCInfo->group->member[j].number ==
              NPCInfo->blockingEntNum) { // we're being blocked by one of our
                                         // own, pass our goal onto them and I'll
                                         // stand still
            ST_TransferMoveGoal(NPC,
                                &g_entities[NPCInfo->group->member[j].number]);
            break;
          }
        }
      }
...
void NPC_BSST_Sleep(void) {
  int alertEvent = NPC_CheckAlertEvents(
      qfalse, qtrue); // only check sounds since we're alseep!
...
qboolean NPC_CheckEnemyStealth(gentity_t *target) {
  ...
  if (target->client->ps.weapon == WP_SABER && target->client->ps.saberActive &&
      !target->client->ps
  ...
  if ((target_dist = DistanceSquared(
           target->currentOrigin, NPC->currentOrigin)) < (minDist * minDist) &&
      (NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES)) {
  ...
  qboolean clearLOS = (target->client->ps.leanofs)
  ...
  if (clearLOS) {
    ...
    vec3_t targ_org = {target->currentOrigin[0], target->currentOrigin[1],
                       target->currentOrigin[2] + target->maxs[2] - 4};
    float hAngle_perc = NPC_GetHFOVPercentage(
        targ_org, NPC->client->renderInfo.eyePoint,
    ...
    float turning_rating = AngleDelta(target->client->ps.viewangles[PITCH],
                                      target->lastAngles[PITCH]) /
                               180.0f +
                           AngleDelta(target->client->ps.viewangles[YAW],
                                      target->lastAngles[YAW]) /
                               180.0f;
    float light_level = (target->lightLevel / MAX_LIGHT_INTENSITY);
    float FOV_perc = 1.0f - (hAngle_perc + vAngle_perc) *
    ...
    // float difficulty_scale = 1.0f + (2.0f-g_spskill->value);//if playing on
    ...
    if (level.alertEvents[eventID].level == AEL_DISCOVERED &&
      ...
      if (!level.alertEvents[eventID].owner ||
          ...
          level.alertEvents[eventID].owner->client->playerTeam !=
              NPC->client->enemyTeam) { // not an enemy
        return qfalse;
      }
  ...
  */
  ...
  if (level.alertEvents[eventID].level > AEL_MINOR &&
    ...
    if (G_ExpandPointToBBox(
            NPCInfo->investigateGoal, NPC->mins, NPC->maxs, NPC->s.number,
      ...
      gi.trace(&trace, NPCInfo->investigateGoal, NPC->mins, NPC->maxs, end,
               ENTITYNUM_NONE,
      ...
      int id = NPC_FindCombatPoint(
          NPCInfo->investigateGoal, NPCInfo->investigateGoal,
    ...
    if (NPCInfo->investigateDebounceTime + NPCInfo->pauseTime >
        level.time) { // was already investigating
      if (NPCInfo->group && NPCInfo->group->commander &&
          NPCInfo->group->commander->client &&
          NPCInfo->group->commander->client->NPC_class == CLASS_IMPERIAL &&
          !Q_irand(0, 3)) {
        ST_Speech(NPCInfo->group->commander, SPEECH_LOOK,
                  0); // FIXME: "I'll go check it out" type sounds
      } else {
        ST_Speech(NPC, SPEECH_LOOK,
                  0); // FIXME: "I'll go check it out" type sounds
      }
    } else {
...
static void ST_LookAround(void) {
  vec3_t lookPos;
  float perc = (float)(level.time - NPCInfo->pauseTime) /
               (float)NPCInfo->investigateDebounceTime;
...
void NPC_BSST_Investigate(void) {
  ...
  if (NPCInfo->localState == LSTATE_INVESTIGATE &&
      (NPCInfo->goalEntity != NULL)) {
    // See if we're there
    if (NAV_HitNavGoal(NPC->currentOrigin, NPC->mins, NPC->maxs,
                       NPCInfo->goalEntity->currentOrigin, 32,
                       FlyingCreature(NPC)) == qfalse) {
...
void NPC_BSST_Patrol(void) { // FIXME: pick up on bodies of dead buddies?
  ...
  if (NPC->client->NPC_class == CLASS_IMPERIAL ||
      NPC->client->NPC_class == CLASS_IMPWORKER) {           // hack
...
static void ST_CheckMoveState(void) {
  if (Q3_TaskIDPending(
          NPC, TID_MOVE_NAV)) { // moving toward a goal that a script is waiting
                                // on, so don't stop for anything!
    AImove = qtrue;
  }
  ...
  // see if we're heading to some other combatPoint
    ...
    // ucmd.buttons |= BUTTON_CAREFUL;
    if (!NPCInfo->goalEntity) { // um, lost our goal?  Just stand and shoot,
                                // then
      NPCInfo->squadState = SQUAD_STAND_AND_SHOOT;
    }
  }
  ...
  if ((NPCInfo->goalEntity != NPC->enemy) && (NPCInfo->goalEntity != NULL)) {
    // Did we make it?
    if (NAV_HitNavGoal(NPC->currentOrigin, NPC->mins, NPC->maxs,
                       NPCInfo->goalEntity->currentOrigin, 16,
                       FlyingCreature(NPC)) ||
        (!Q3_TaskIDPending(NPC, TID_MOVE_NAV) &&
         NPCInfo->squadState == SQUAD_SCOUT && enemyLOS &&
...
static void ST_CheckFireState(void) {
  ...
  if (!VectorCompare(NPC->client->ps.velocity,
  ...
  if (!hitAlly // we're not going to hit an ally
      ...
      &&
      (NPCInfo->group->numState[SQUAD_RETREAT] > 0 ||
       NPCInfo->group->numState[SQUAD_TRANSITION] > 0 ||
       NPCInfo->group->numState[SQUAD_SCOUT] > 0)) // laying down covering fire
  {
    if (level.time - NPCInfo->enemyLastSeenTime <
            10000 && // we have seem the enemy in the last 10 seconds
        (!NPCInfo->group ||
         level.time - NPCInfo->group->lastSeenEnemyTime <
                     ...
                     // the last 10 seconds
    {
      if (!Q_irand(0, 10)) {
        ...
        if (VectorCompare(impactPos,
          ...
          gi.trace(&tr, muzzle, vec3_origin, vec3_origin, end, NPC->s.number,
...
void ST_HuntEnemy(gentity_t *self) {
  ...
  TIMER_Set(NPC, "scoutTime",
            TIMER_Get(NPC, "stick") - level.time + Q_irand(5000, 10000));
  // leave my combat point
...
void ST_TransferMoveGoal(gentity_t *self, gentity_t *other) {
  if (Q3_TaskIDPending(self,
                       TID_MOVE_NAV)) { // can't transfer movegoal when a script
  ...
  if (NPC && NPCInfo->group) {
    if (NPC == NPCInfo->group->commander &&
      ...
      if (NPCInfo->group->numGroup > 1 &&
          Q_irand(-3, NPCInfo->group->numGroup) >
    ...
    } else if (NPCInfo->group->morale <
               NPCInfo->group->numGroup) { // morale is low for our size
...
void ST_Commander(void) {
  ...
  if (group->lastSeenEnemyTime < level.time - 180000) { // dissolve the group
    ...
    for (i = 0; i < group->numGroup; i++) {
      ...
      if (Q3_TaskIDPending(
              NPC, TID_MOVE_NAV)) { // running somewhere that a script requires
                                    // us to go, don't break from that
        continue;
      }
  ...
  */
  ...
  for (i = curMemberNum; i < lastMemberNum; i++) {
    ...
    if (Q3_TaskIDPending(NPC, TID_MOVE_NAV)) { // running somewhere that a
                                               // script requires us to go
      continue;
    }

    if (NPC->s.weapon == WP_NONE && NPCInfo->goalEntity &&
        NPCInfo->goalEntity == NPCInfo->tempGoal &&
        NPCInfo->goalEntity->enemy &&
        NPCInfo->goalEntity->enemy->s.eType ==
    ...
    if (NPCInfo->squadState != SQUAD_RETREAT) { // not already retreating
      if (NPC->client->ps.weapon == WP_NONE) {  // weaponless, should be hiding
        if (NPCInfo->goalEntity == NULL || NPCInfo->goalEntity->enemy == NULL ||
          ...
          if (TIMER_Done(NPC, "hideTime") ||
              (DistanceSquared(group->enemy->currentOrigin,
                               NPC->currentOrigin) < 65536 &&
               NPC_ClearLOS(
                   NPC->enemy))) { // done hiding or enemy near and can see us
      ...
      if (TIMER_Done(NPC, "roamTime") && TIMER_Done(NPC, "hideTime") &&
          NPC->health > 10 &&
          !gi.inPVS(group->enemy->currentOrigin,
                    NPC->currentOrigin)) { // cant even see enemy
        // better go after him
        ...
        if (gi.inPVS(
                NPC->currentOrigin,
                group->enemy->currentOrigin)) { // in the same room as enemy
          if (NPC->client->ps.weapon == WP_ROCKET_LAUNCHER &&
              DistanceSquared(group->enemy->currentOrigin, NPC->currentOrigin) <
            ...
            case WP_SABER:
              if (group->enemy->client->ps.saberLength > 0) {
                if (DistanceSquared(group->enemy->currentOrigin,
                                    NPC->currentOrigin) < 65536) {
                  if (TIMER_Done(NPC, "hideTime")) {
          ...
          if (NPCInfo->aiFlags &
            ...
            for (j = 0; j < group->numGroup; j++) {
              if (group->member[j].number ==
                  NPCInfo->blockingEntNum) { // we're being blocked by one of
            ...
            if (TIMER_Done(NPC, "duck")) {    // not ducking
              if (TIMER_Done(NPC, "stand")) { // don't have to keep standing
                if (NPCInfo->combatPoint == -1 ||
                    (level.combatPoints[NPCInfo->combatPoint].flags &
    ...
    if (cpFlags) { // we want to run to a combat point
      ...
      */
      ...
      if (group->enemy->client->ps.weapon == WP_SABER &&
          group->enemy->client->ps.saberLength >
              0) { // we obviously want to avoid the enemy if he has a saber
      ...
      if (cp == -1) { // may have had sone set above
        cp = NPC_FindCombatPoint(
            NPC->currentOrigin, NPC->currentOrigin, group->enemy->currentOrigin,
            cpFlags | CP_HAS_ROUTE, avoidDist, NPCInfo->lastFailedCombatPoint);
      }
      while (cp == -1 &&
        ...
        // now try again
        cp = NPC_FindCombatPoint(NPC->currentOrigin, NPC->currentOrigin,
          ...
          if (cpFlags & CP_FLANK) {
            ...
            if (group->numGroup > 1) {
              ...
              if (!Q_irand(
                ...
                VectorSubtract(NPC->currentOrigin, group->enemy->currentOrigin,
                               eDir2Me);
                VectorNormalize(eDir2Me);

                VectorSubtract(level.combatPoints[NPCInfo->combatPoint].origin,
                               group->enemy->currentOrigin, eDir2CP);
...
void NPC_BSST_Attack(void) {
  ...
  if (NPCInfo->group) { // I belong to a squad of guys - we should *always* have
                        // a group
    if (!NPCInfo->group->processed) { // I'm the first ent in my group, I'll
                                      // make the command decisions
#if AI_TIMERS
  ...
  {
    if (NPC->client->ps.weapon == WP_DISRUPTOR) { // sniping... should be
                                                  // assumed
        ...
        if (hit == NPC->enemy->s.number ||
          ...
          if (hitEnt && hitEnt->client &&
              hitEnt->client->playerTeam ==
                  NPC->client->playerTeam) { // would hit an ally, don't fire!!!
            hitAlly = qtrue;
  ...
  if (!faceEnemy) { // we want to face in the dir we're running
    if (!AImove) {  // if we haven't moved, we should look in the direction we
                   // last looked?
    ...
    NPC_UpdateAngles(qtrue, qtrue);
    if (AImove) { // don't run away and shoot
      shoot = qfalse;
    }
  }
  ...
  if (NPC->client->fireDelay) {
    ...
    if (TIMER_Done(NPC, "attackDelay")) {
      if (!(NPCInfo->scriptFlags & SCF_FIRE_WEAPON)) // we've already fired, no
      ...
      // NASTY
      if (NPC->s.weapon == WP_ROCKET_LAUNCHER &&
          (ucmd.buttons & BUTTON_ATTACK) && !AImove && g_spskill->integer > 1 &&
