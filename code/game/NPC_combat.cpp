...
qboolean G_CheckSaberAllyAttackDelay(gentity_t *self, gentity_t *enemy) {
	...
	if (self->NPC && self->client->leader == player && self->enemy && self->enemy->s.weapon != WP_SABER &&
		self->s.weapon == WP_SABER) { // assisting the player and I'm using a saber and my enemy is not
...
void G_SetEnemy(gentity_t *self, gentity_t *enemy) {
	...
	if (self->enemy == NULL) {
		...
		if (self->client->playerTeam == TEAM_PLAYER && enemy->s.number == 0 && enemy->client &&
			enemy->client->playerTeam == TEAM_PLAYER) { // make the player "evil" so that everyone goes after him
		...
		if (G_ActivateBehavior(self, BSET_ANGER)) {
		} else if (self->client && self->client->NPC_class == CLASS_KYLE && self->client->leader == player &&
				   !TIMER_Done(self, "kyleAngerSoundDebounce")) { // don't yell that you have an enemy more than once every five seconds
			...
			{
				if (!G_TeamEnemy(self) && self->client->NPC_class != CLASS_BOBAFETT) { // team did not have an enemy previously
					if (self->NPC && self->client->playerTeam == TEAM_PLAYER && enemy->s.number < MAX_CLIENTS && self->client->clientInfo.customBasicSoundDir &&
						self->client->clientInfo.customBasicSoundDir[0] && Q_stricmp("jedi2", self->client->clientInfo.customBasicSoundDir) == 0) {
						switch (Q_irand(0, 2)) {
						case 0:
						...
						if (Q_irand(0, 1)) { // hell, we're loading them, might as well use them!
			...
			if (event) { // yell
				if (self->client && self->client->NPC_class == CLASS_KYLE &&
					self->client->leader == player) { // don't yell that you have an enemy more than once every 4-8 seconds
		...
		if (self->s.weapon == WP_BLASTER || self->s.weapon == WP_REPEATER || self->s.weapon == WP_THERMAL || self->s.weapon == WP_BLASTER_PISTOL ||
		...
		if (!G_CheckSaberAllyAttackDelay(self, enemy)) { // not a saber ally holding back
...
void NPC_ApplyWeaponFireDelay(void) {
	...
	case WP_TUSKEN_RIFLE:
		if (!(NPCInfo->scriptFlags & SCF_ALT_FIRE)) { // FIXME: should be unique per melee anim
			client->fireDelay = 300;
		}
		break;
...
void WeaponThink(qboolean inCombat) {
	...
	if (NPC->client && (NPC->client->ps.powerups[PW_CLOAKED] || (level.time < NPC->client->ps.powerups[PW_UNCLOAKING]))) {
		return;
	}
...
qboolean HaveWeapon(int weapon) { return (qboolean)(client->ps.stats[STAT_WEAPONS] & (1 << weapon)); }
...
float NPC_MaxDistSquaredForWeapon(void) {
	...
	case WP_TUSKEN_RIFLE:
		...
		*/
	case WP_SABER:
		if (NPC->client && NPC->client->ps.SaberLength()) { // FIXME: account for whether enemy and I are heading towards each other!
		...
		if (newenemy != NPC && (newenemy->client || newenemy->svFlags & SVF_NONNPC_ENEMY) && !(newenemy->flags & FL_NOTARGET) &&
			...
			if (newenemy->health > 0) {
				if ((newenemy->client && NPC_ValidEnemy(newenemy)) ||
					(!newenemy->client && newenemy->noDamageTeam == enemyTeam)) {			  // FIXME:  check for range and FOV or vis?
					if (NPC->client->playerTeam == TEAM_PLAYER && enemyTeam == TEAM_PLAYER) { // player allies turning on ourselves?  How?
...
gentity_t *NPC_CheckEnemy(qboolean findNew, qboolean tooFarOk, qboolean setEnemy) {
	...
	if (NPC->client->NPC_class == CLASS_KYLE && NPC->client->leader && Distance(NPC->client->leader->currentOrigin, NPC->currentOrigin) > 3000) {
		if (NPC->enemy) {
			G_ClearEnemy(NPC);
		}
		return NULL;
	...
	if (NPC->svFlags & SVF_LOCKEDENEMY) { // keep this enemy until dead
		if (NPC->enemy) {
			if ((!NPC->NPC && !(NPC->svFlags & SVF_NONNPC_ENEMY)) ||
				NPC->enemy->health > 0) { // Enemy never had health (a train or info_not_null, etc) or did and is now dead (NPCs, turrets, etc)
				return NULL;
			}
		}
	...
	if (NPC->enemy) {
		...
		} else if (!gi.inPVS(NPC->currentOrigin, NPC->enemy->currentOrigin)) { // FIXME: should this be a line-of site check?
	...
	if (NPC->enemy && NPC->enemy->client) {
		if (NPC->enemy->client->playerTeam && NPC->enemy->client->playerTeam != TEAM_FREE) {
			...
			if (NPC->client->playerTeam != NPC->enemy->client->playerTeam && NPC->client->enemyTeam != TEAM_FREE &&
		...
		if (attack_ok) {
			...
			if (!dead_on) { // We're not going to hit him directly, try a suppressing fire
				...
				if (traceEnt && (traceEnt->health <= 30 || EntIsGlass(traceEnt))) {				   // easy to kill - go for it
					if (traceEnt->e_DieFunc == dieF_ExplodeDeath_Wait && traceEnt->splashDamage) { // going to explode, don't shoot if close to self
...
int NPC_FindCombatPoint(const vec3_t position, const vec3_t avoidPosition, vec3_t destPosition, const int flags, float avoidDist, const int ignorePoint) {
	...
	if ((flags & CP_NO_PVS) || (flags & CP_TRYFAR)) { // much larger radius since most will be dropped?
	...
	for (cpi = points.begin(); cpi != points.end(); ++cpi) {
		...
		if (flags & CP_AVOID_ENEMY) {
			...
			if (distSqNPCToEnemy > (avoidDist) && !NAV::SafePathExists(position, level.combatPoints[i].origin, enemyPosition, avoidDist)) {
				continue;
			}
		}
...
int NPC_FindCombatPointRetry(const vec3_t position, const vec3_t avoidPosition, vec3_t destPosition, int *cpFlags, float avoidDist, const int ignorePoint) {
	int cp = -1;
	cp = NPC_FindCombatPoint(position, avoidPosition, destPosition, *cpFlags, avoidDist, ignorePoint);
	while (cp == -1 && (*cpFlags & ~CP_HAS_ROUTE) != CP_ANY) { // start "OR"ing out certain flags to see if we can find *any* point
		...
		//	}
			...
			*cpFlags &= ~CP_AVOID;
		} else if (*cpFlags & CP_SHORTEST_PATH) { // okay, don't need the one with the shortest path
...
void NPC_CheckGetNewWeapon(void) {
	if (NPC->client && !G_CanPickUpWeapons(NPC)) { // this NPC can't pick up weapons...
		return;
	}
