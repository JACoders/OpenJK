...
static void WP_FireConcussionAlt(gentity_t *ent) { // a rail-gun-like beam
  int damage = weaponData[WP_CONCUSSION].altDamage, skip,
  ...
  if (ent->s.number >= MAX_CLIENTS) {
    ...
    angles[PITCH] +=
        (Q_flrand(-1.0f, 1.0f) *
         (CONC_NPC_SPREAD + (6 - ent->NPC->currentAim) * 0.25f)); // was 0.5f
    angles[YAW] +=
        (Q_flrand(-1.0f, 1.0f) *
         (CONC_NPC_SPREAD + (6 - ent->NPC->currentAim) * 0.25f)); // was 0.5f
  ...
  VectorMA(ent->client->ps.velocity, -200, forwardVec,
  ...
  // The trace start will originate at the eye so we can ensure that it hits the
  // crosshair.
  if (ent->NPC) {
  ...
  for (int i = 0; i < traces; i++) {
    ...
    // the "G2_COLLIDE, 10" to see if we it the sitter gi.trace( &tr, start,
    // NULL, NULL, end, skip, MASK_SHOT, G2_COLLIDE, 10 );//G2_RETURNONHIT, 0 );
    gi.trace(&tr, start, shot_mins, shot_maxs, end, skip, MASK_SHOT, G2_COLLIDE,
             10); // G2_RETURNONHIT, 0 );
            ...
            (traceEnt->client &&
             (traceEnt->client->NPC_class == CLASS_BOBAFETT ||
              traceEnt->client->NPC_class ==
                  CLASS_REBORN)))) { // FIXME: need a more reliable way to know
                                     // we hit a jedi?
    ...
    if (!hitDodged) {
      if (render_impact) {
        if ((tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage) ||
          ...
          // world
          G_PlayEffect(G_EffectIndex("concussion/alt_hit"), tr.endpos,
                       tr.plane.normal);
          ...
          qboolean noKnockBack =
              (qboolean)((traceEnt->flags & FL_NO_KNOCKBACK) !=
                         0); // will be set if they die, I want to know if it
                             // was on *before* they died
          if (traceEnt && traceEnt->client &&
              traceEnt->client->NPC_class == CLASS_GALAKMECH) { // hehe
            G_Damage(traceEnt, ent, ent, forwardVec, tr.endpos, 10,
                     DAMAGE_NO_KNOCKBACK | DAMAGE_NO_HIT_LOC, MOD_CONC_ALT,
                     hitLoc);
            break;
          }
          G_Damage(traceEnt, ent, ent, forwardVec, tr.endpos, damage,
                   DAMAGE_NO_KNOCKBACK | DAMAGE_NO_HIT_LOC, MOD_CONC_ALT,
                   hitLoc);
          ...
          if (traceEnt->client) { // only if we hit a client
            ...
            // if ( traceEnt->NPC || Q_irand(0,g_spskill->integer+1) )
            {
              ...
              if (traceEnt->health > 0) {            // alive
                if (G_HasKnockdownAnims(traceEnt)) { // knock-downable
  ...
  // FIXME: if shoot *really* close to someone, the alert could be way out of
  // their FOV
  for (dist = 0; dist < shotDist; dist += 64) {
    // FIXME: on a really long shot, this could make a LOT of alerts in one
    // frame...
  ...
  // FIXME: spawn a temp ent that continuously spawns sight alerts here?  And 1
  // sound alert to draw their attention?
...
static void WP_FireConcussion(gentity_t *ent) { // a fast rocket-like projectile
  ...
  if (ent->s.number < MAX_CLIENTS  // player only
      && !cg.renderingThirdPerson) // gives an advantage to being in 3rd person,
                                   // but would look silly otherwise
  { // kick the view back
  ...
  gentity_t *missile =
      CreateMissile(start, forwardVec, vel, 10000, ent, qfalse);
