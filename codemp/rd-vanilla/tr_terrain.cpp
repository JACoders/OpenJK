...
// To do:
// Alter variance dependent on global distance from player (colour code this for
// cg_terrainCollisionDebug) Improve texture blending on edge conditions Link to
// neightbouring terrains or architecture (edge conditions) Post process
// generated light data to make sure there are no bands within a patch
  ...
  if ((corner[3] < 0) || (tess.registration != corner[4])) {
    ...
    *(ulong *)tess.vertexAlphas[tess.numVertexes] = corner[2];
    ...
    tess.texCoords[tess.numVertexes][0][0] =
        vert->tex[0]; // rwwRMG - reverse coords array from sof2
...
void CTRPatch::RecurseRender(int depth, ivec5_t left, ivec5_t right,
                             ivec5_t apex) {
...
void CTRPatch::Render(int Part) {
  ...
  VectorSet5M(BR, owner->GetTerxels(), owner->GetTerxels(), TEXTURE_ALPHA_BR,
              -1, 0);

  if ((Part & PI_TOP) && mTLShader) {
    /*		float		d;

                    d = DotProduct (backEnd.refdef.vieworg, mNormal[0]) -
       mDistance[0];
  ...
  if ((Part & PI_BOTTOM) && mBRShader) {
    /*		float		d;
                    ...
                    d = DotProduct (backEnd.refdef.vieworg, mNormal[1]) -
       mDistance[1];
...
//
// At this point the patch is visible and at least part of it is below water
// level
//
int CTRPatch::RenderWaterVert(int x, int y) {
  ...
  *(ulong *)tess.vertexColors[tess.numVertexes] = 0xffffffff;
  ...
  tess.texCoords[tess.numVertexes][0][0] =
      vert->tex[0]; // rwwRMG - reverse coords from sof2mp
  tess.texCoords[tess.numVertexes][0][1] = vert->tex[1];
...
const bool CTRPatch::HasWater(void) const {
  owner->SetRealWaterHeight(owner->GetBaseWaterHeight() +
                            r_terrainWaterOffset->integer);
  return (common->GetMins()[2] < owner->GetWaterHeight());
}

void CTRPatch::SetVisibility(bool visCheck) {
  if (visCheck) {
    if (DistanceSquared(mCenter, backEnd.refdef.vieworg) >
        TerrainDistanceCull) {
      misVisible = false;
    } else {
...
void CTRPatch::CalcNormal(void)
{
        ...
        VectorSet5(BR, owner->GetTerxels(), owner->GetTerxels(),
TEXTURE_ALPHA_BR, -1, 0);
...
void CTRLandScape::CalculateRegion(void) {
  ...
  mins[0] = backEnd.refdef.vieworg[0] - tr.distanceCull - (size[0] * 2.0f) -
            offset[0];
  mins[1] = backEnd.refdef.vieworg[1] - tr.distanceCull - (size[1] * 2.0f) -
            offset[1];

  maxs[0] = backEnd.refdef.vieworg[0] + tr.distanceCull + (size[0] * 2.0f) -
            offset[0];
  maxs[1] = backEnd.refdef.vieworg[1] + tr.distanceCull + (size[1] * 2.0f) -
            offset[1];
...
void CTRLandScape::CalculateNormals(void) {
  ...
  for (y = 0; y < GetHeight(); y++) {
    for (x = 0; x < GetWidth(); x++) {
      ...
      VectorSubtract(mRenderMap[offset].coords, mRenderMap[offset + 1].coords,
                     vcenter);
      VectorSubtract(mRenderMap[offset].coords,
                     mRenderMap[offset + GetRealWidth()].coords, vleft);
  ...
  for (x = 0; x < GetRealWidth(); x++) {
    VectorCopy(mRenderMap[offset - GetRealWidth() + x].normal,
...
void CTRLandScape::CalculateLighting(void) {
  ...
  // Work out the vertex normal (average of every attached face normal) and
  // apply to the direction of the light
  for (y = 0; y < GetHeight(); y++) {
    for (x = 0; x < GetWidth(); x++) {
      vec3_t ambient;
      ...
      if (!R_LightForPoint(mRenderMap[offset].coords, ambient, directed,
                           direction)) {
        mRenderMap[offset].tint[0] = mRenderMap[offset].tint[1] =
      ...
      mRenderMap[offset].tint[0] =
          (byte)Com_Clampi(0.0f, 255.0f, tint[0]) >> tr.overbrightBits;
      mRenderMap[offset].tint[1] =
          (byte)Com_Clampi(0.0f, 255.0f, tint[1]) >> tr.overbrightBits;
      mRenderMap[offset].tint[2] =
          (byte)Com_Clampi(0.0f, 255.0f, tint[2]) >> tr.overbrightBits;
  ...
  offset = GetHeight() * GetRealWidth();
  for (x = 0; x < GetRealWidth(); x++) {
    mRenderMap[offset + x].tint[0] =
        mRenderMap[offset - GetRealWidth() + x].tint[0];
    mRenderMap[offset + x].tint[1] =
        mRenderMap[offset - GetRealWidth() + x].tint[1];
    mRenderMap[offset + x].tint[2] =
        mRenderMap[offset - GetRealWidth() + x].tint[2];
  ...
  while (classes) {
    ...
    while (items) {
      ...
      if (!Q_stricmp(type, "altitudetexture")) {
        ...
        mWaterShader = R_GetShaderByHandle(
            RE_RegisterShader(items->FindPairValue("shader", "")));
...
qhandle_t CTRLandScape::GetBlendedShader(qhandle_t a, qhandle_t b, qhandle_t c,
                                         bool surfaceSprites) {
  qhandle_t blended;
...
void CTRLandScape::CalculateShaders(void) {
  ...
  for (y = 0; y < height + 1; y++) {
    ...
    for (x = 0; x < width + 1; x++, offset += common->GetTerxels()) {
      // Save the shader
      shaders[y * width + x] =
          GetHeightDetail(mRenderMap[offset].height)->GetShader();
    }
  }
    ...
    for (y = 1; y < height; y++) {
      for (x = 1; x < width; x++) {
        ...
        /*
                                        ...
                                        offset  = x * common->GetTerxels ( );
                                        offset += (y * common->GetTerxels ( ) *
           GetRealWidth());

                                        // Calculate the offsets around this
           particular shader location offsets[INDEX_TL] = offset - 1 -
           GetRealWidth(); offsets[INDEX_TR] = offsets[INDEX_TL] + 1;
                                        offsets[INDEX_BL] = offsets[INDEX_TL] +
           GetRealWidth(); offsets[INDEX_BR] = offsets[INDEX_BL] + 1;

                                        // If not equal to the top left one then
           skip if ( mRenderMap[offset].height !=
           mRenderMap[offsets[INDEX_TL]].height )
                                        {
                                                continue;
                                        }

                                        // If not equal to the top right one
           then skip if ( mRenderMap[offset].height !=
           mRenderMap[offsets[INDEX_TR]].height )
                                        {
                                                continue;
                                        }

                                        // If not equal to the bottom left one
           then skip if ( mRenderMap[offset].height !=
           mRenderMap[offsets[INDEX_BL]].height )
                                        {
                                                continue;
                                        }

                                        // If not equal to the bottom right one
           then skip if ( mRenderMap[offset].height !=
           mRenderMap[offsets[INDEX_BR]].height )
                                        {
                                                continue;
                                        }
                */
  ...
  for (y = 0; y < height; y++) {
    for (x = 0; x < width; x++, patch++) {
      ...
      patch->SetTLShader(GetBlendedShader(handles[INDEX_TR], handles[INDEX_BL],
                                          handles[INDEX_TL], surfaceSprites));
      current->mPatch = patch;
      current->mShader = patch->GetTLShader();
      current->mPart = PI_TOP;

      patch->SetBRShader(GetBlendedShader(handles[INDEX_TR], handles[INDEX_BL],
                                          handles[INDEX_BR], surfaceSprites));
  ...
  qsort(mSortedPatches, mSortedCount, sizeof(*mSortedPatches),
        (int(QDECL *)(const void *, const void *))ComparePatchInfo);
}
...
CTRLandScape::CTRLandScape(const char *configstring) {
  ...
  mTextureScale = (float)atof(Info_ValueForKey(configstring, "texturescale")) /
                  common->GetTerxels();
  ...
  mRenderMap = (CTerVert *)Z_Malloc(sizeof(CTerVert) * common->GetRealArea(),
                                    TAG_R_TERRAIN);
  ...
  mTRPatches = (CTRPatch *)Z_Malloc(sizeof(CTRPatch) * common->GetBlockCount(),
                                    TAG_R_TERRAIN);

  mSortedCount = 2 * common->GetBlockCount();
  mSortedPatches =
      (TPatchInfo *)Z_Malloc(sizeof(TPatchInfo) * mSortedCount, TAG_R_TERRAIN);
...
void R_AddTerrainSurfaces(void) {
  ...
  if (landscape) {
    R_AddDrawSurf((surfaceType_t *)(&tr.landScape), landscape->GetShader(), 0,
                  qfalse);
...
void R_TerrainShutdown(void) {
  ...
  //	ri.Printf( PRINT_ALL, "R_Terrain: Shutting down RENDERER
  //terrain.....\n");
