...
//
void CG_OwnerDraw(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, int ownerDrawFlags, int align, float special, float scale,
				  vec4_t color, qhandle_t shader, int textStyle, int font) {
...
void CG_MouseEvent(int x, int y) {
	...
	cg.predictedPlayerState.pm_type == PM_SPECTATOR) && cg.showScores == qfalse) { trap->Key_SetCatcher(0); return;
	}
	*/
