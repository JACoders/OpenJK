...
public:
  hstring() { mId = 0; }
  hstring(const char *str) { Init(str); }
  hstring(const string &str) { Init(str.c_str()); }
  hstring(const hstring &str) { mId = str.mId; }
  ...
  int compare(const hstring &str) const { return strcmp(c_str(), str.c_str()); }

  bool operator<(const hstring &str) const {
    return ((mId < str.mId) ? true : false);
  }
  int length() const { return strlen(c_str()); }
};
...
template <class T> class CMapPool {
  CMapPoolLow &mPool;

public:
  CMapPool() : mPool(GetMapPool()) {}
  template <class U> CMapPool(const U &) : mPool(GetMapPool()) {}
  ~CMapPool() {}
  ...
  template <class U> struct rebind { typedef CMapPool<U> other; };
  ...
  template <class U> void deallocate(U *p, size_type num) {
    assert(num == 1); // allocator not design for this
    mPool.Free(p);
  }
};
...
bool operator==(const CMapPool<T1> &, const CMapPool<T2> &) {
  return false;
}
template <class T1, class T2>
bool operator!=(const CMapPool<T1> &, const CMapPool<T2> &) {
...
template <class K> class hlist : public list<K, CMapPool<K>> {};
