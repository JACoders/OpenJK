...
static cvar_t *fs_dirbeforepak; // rww - when building search path, keep
                                // directories at top and insert pk3's under them
...
qboolean FS_Initialized(void) { return (qboolean)(fs_searchpaths != NULL); }
...
char *FS_BuildOSPath(const char *qpath) {
  ...
  int nextToggle =
      (toggle + 1) & 3; // allows four returns without clash (increased from 2
                        // during fs_copyfiles 2 enhancement)
  toggle = nextToggle;
  ...
  Com_sprintf(temp, sizeof(temp), "/base/%s",
              qpath); // FIXME SP used fs_gamedir here as well (not sure if this
                      // func is even used)
  FS_ReplaceSeparators(temp);
  Com_sprintf(ospath[toggle], sizeof(ospath[0]), "%s%s", fs_basepath->string,
              temp);
...
ERR_FATAL if trying to maniuplate a file with the platform library, or pk3
extension
=================
*/
...
=================
*/
// added extra param so behind-the-scenes copying in savegames doesn't clutter
// up the screen -slc
...
// The following functions with "UserGen" in them were added for savegame
// handling,
...
//
void FS_DeleteUserGenFile(const char *filename) { FS_HomeRemove(filename); }

// filenames are local (eg "saves/blah.sav")
//
// return: qtrue = OK
//
qboolean FS_MoveUserGenFile(const char *filename_src,
...
void FS_Rmdir(const char *osPath, qboolean recursive) {
  ...
  if (recursive) {
    ...
    for (i = 0; i < numfiles; i++) {
      ...
      Com_sprintf(fileOsPath, sizeof(fileOsPath), "%s/%s", osPath,
                  filesToRemove[i]);
      FS_Remove(fileOsPath);
    }
    FS_FreeFileList(filesToRemove);

    char **directoriesToRemove =
        Sys_ListFiles(osPath, "/", NULL, &numfiles, qfalse);
    for (i = 0; i < numfiles; i++) {
      ...
      Com_sprintf(directoryOsPath, sizeof(directoryOsPath), "%s/%s", osPath,
                  directoriesToRemove[i]);
...
void FS_HomeRmdir(const char *homePath, qboolean recursive) {
  ...
  FS_Rmdir(FS_BuildOSPath(fs_homepath->string, fs_gamedir, homePath),
           recursive);
}
...
qboolean FS_IsDemoExt(const char *filename, int namelen) {
  ...
  if (ext_test && !Q_stricmpn(ext_test + 1, DEMO_EXTENSION,
                              ARRAY_LEN(DEMO_EXTENSION) - 1)) {
...
bool Sys_GetFileTime(LPCSTR psFileName, FILETIME &ft) {
  ...
  hFile = CreateFile(
      ...
      NULL, // LPSECURITY_ATTRIBUTES lpSecurityAttributes,	// pointer to
            // security attributes
      OPEN_EXISTING,          // DWORD dwCreationDisposition,  // how to create
      FILE_FLAG_NO_BUFFERING, // DWORD dwFlagsAndAttributes,   // file
                              // attributes
...
bool Sys_FileOutOfDate(LPCSTR psFinalFileName /* dest */,
                       LPCSTR psDataFileName /* src */) {
  FILETIME ftFinalFile, ftDataFile;

  if (Sys_GetFileTime(psFinalFileName, ftFinalFile) &&
      Sys_GetFileTime(psDataFileName, ftDataFile)) {
    // timer res only accurate to within 2 seconds on FAT, so can't do exact
    // compare...
    //
    // LONG l = CompareFileTime( &ftFinalFile, &ftDataFile );
    if ((abs((double)(ftFinalFile.dwLowDateTime - ftDataFile.dwLowDateTime)) <=
         20000000) &&
        ftFinalFile.dwHighDateTime == ftDataFile.dwHighDateTime) {
      return false; // file not out of date, ie use it.
    }
    return true; // flag return code to copy over a replacement version of this
                 // file
  }

  // extra error check, report as suspicious if you find a file locally but not
  // out on the net.,.
  //
...
long FS_FOpenFileRead(const char *filename, fileHandle_t *file,
  ...
  isUserConfig = !Q_stricmp(filename, "autoexec_sp.cfg") ||
                 !Q_stricmp(filename, Q3CONFIG_NAME);
  ...
  *file = FS_HandleForFile();
  ...
  // this new bool is in for an optimisation, if you (eg) opened a BSP file
  // under fs_copyfiles==2,
  //	then it triggered a copy operation to update your local HD version, then
  //this will re-open the 	file handle on your local version, not the net build.
  //This uses a bit more CPU to re-do the loop 	logic, but should read faster
  //than accessing the net version a second time.
  //
  ...
  do {
    ...
    for (search = fs_searchpaths; search; search = search->next) {
      ...
      if (search->pack && search->pack->hashTable[hash]) {
        // autoexec_sp.cfg and openjk_sp.cfg can only be loaded outside of pk3
        // files.
        if (isUserConfig) {
          continue;
        }
...
#ifdef _WIN32
        // if running with fs_copyfiles 2, and search path == local, then we
        // need to fail to open
        //	if the time/date stamp != the network version (so it'll loop
        //round again and use the network path, 	which comes later in the search
        //order)
        //
        if (fs_copyfiles->integer == 2 && fs_cdpath->string[0] &&
            !Q_stricmp(dir->path, fs_basepath->string) &&
            FS_FileCacheable(filename)) {
          if (Sys_FileOutOfDate(
                  netpath,
                  FS_BuildOSPath(fs_cdpath->string, dir->gamedir, filename))) {
        ...
        if (fs_copyfiles->integer && !Q_stricmp(dir->path, fs_cdpath->string)) {
          ...
          copypath =
              FS_BuildOSPath(fs_basepath->string, dir->gamedir, filename);
          ...
          case 2: {
            ...
            if (FS_FileCacheable(filename)) {
              ...
              if (bOk) {
                // clear this handle and setup for re-opening of the new local
                // copy...
                //
...
long FS_ReadFile(const char *qpath, void **buffer) {
  ...
  buf[len] = '\0'; // because we're not calling Z_Malloc with optional trailing
                   // 'bZeroIt' bool
  *buffer = buf;
  ...
  if (com_sv_running && com_sv_running->integer &&
      cls.state >= CA_ACTIVE) { // com_cl_running
...
static pack_t *FS_LoadZipFile(const char *zipfile, const char *basename) {
  ...
  buildBuffer = (struct fileInPack_s *)Z_Malloc(
      (gi.number_entry * sizeof(fileInPack_t)) + len, TAG_FILESYS, qtrue);
  namePtr = ((char *)buildBuffer) + gi.number_entry * sizeof(fileInPack_t);
  fs_headerLongs =
      (int *)Z_Malloc(gi.number_entry * sizeof(int), TAG_FILESYS, qtrue);
  ...
  pack->checksum = Com_BlockChecksum(fs_headerLongs, sizeof(*fs_headerLongs) *
                                                         fs_numHeaderLongs);
...
static char **Sys_ConcatenateFileLists(char **list0, char **list1,
  ...
  /* Create new list. */
  dst = cat =
      (char **)Z_Malloc((totalLength + 1) * sizeof(char *), TAG_FILESYS, qtrue);
  ...
  for (search = fs_searchpaths; search; search = search->next) {
    if (search->pack) {
      ...
      if (filep) {
        ...
        char buf[MAX_OSPATH];
        Com_sprintf(buf, sizeof(buf), "%s%c%s", dir->path, PATH_SEP,
                    dir->gamedir);
...
static void FS_AddGameDirectory(const char *path, const char *dir) {
  ...
  //
  search =
      (struct searchpath_s *)Z_Malloc(sizeof(searchpath_t), TAG_FILESYS, qtrue);
  search->dir =
      (directory_t *)Z_Malloc(sizeof(*search->dir), TAG_FILESYS, qtrue);
...
qboolean FS_WriteToTemporaryFile(const void *data, size_t dataLength,
                                 char **tempFilePath) {
