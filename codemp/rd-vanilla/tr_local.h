...
typedef enum { DLIGHT_VERTICAL = 0, DLIGHT_PROJECTED } eDLightTypes;
...
typedef struct dlight_s {
  ...
  vec3_t
      mProjTransformed; // projected light's origin in local coordinate system
...
typedef struct image_s {
  ...
  word width, height; // after power of two and picmip but not including clamp
                      // to MAX_TEXTURE_SIZE
...
typedef struct viewParms_s {
  orientationr_t
      ori; // Can't use "or" as it is a reserved word with gcc DREWS 2/2/2002
...
typedef struct backEndState_s {
  ...
  orientationr_t
      ori; // Can't use or as it is a c++ reserved word DREWS 2/2/2002
...
extern cvar_t
    *r_cullRoofFaces; // attempted smart method of culling out upwards facing
...
void RE_AddDecalToScene(qhandle_t shader, const vec3_t origin, const vec3_t dir,
