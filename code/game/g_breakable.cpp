...
extern void CG_MiscModelExplosion(vec3_t mins, vec3_t maxs, int size,
                                  material_t chunkType);
extern void CG_Chunks(int owner, vec3_t origin, const vec3_t normal,
                      const vec3_t mins, const vec3_t maxs, float speed,
                      int numChunks, material_t chunkType, int customChunk,
                      float baseScale, int customSound = 0);
extern void G_SetEnemy(gentity_t *self, gentity_t *enemy);

extern gentity_t *G_CreateObject(gentity_t *owner, vec3_t origin, vec3_t angles,
                                 int modelIndex, int frame, trType_t trType,
                                 int effectID);
...
void funcBBrushDieGo(gentity_t *self) {
  ...
  for (int i = 0; i < MAX_GENTITIES; i++) {
    if (g_entities[i].s.groundEntityNum == self->s.number &&
        (g_entities[i].s.eFlags & EF_MISSILE_STICK)) {
      G_Damage(&g_entities[i], self, self, NULL, NULL, 99999, 0,
               MOD_CRUSH); //?? MOD?
    }
  }

  // NOTE: MUST do this BEFORE clearing contents, or you may not open the area
  // portal!!!
  ...
  // This formula really has no logical basis other than the fact that it seemed
  // to be the closest to yielding the results that I wanted. Volume is length *
  // width * height...then break that volume down based on how many chunks we
  // have
  ...
  if (self->radius > 0.0f) {
    // designer wants to scale number of chunks, helpful because the above scale
    // code is far from perfect
    //	I do this after the scale calculation because it seems that the chunk
    //size generally seems to be very close, it's just the number of chunks is a
    //bit weak
  ...
  if (self->splashDamage > 0 && self->splashRadius > 0) {
    ...
    AddSoundEvent(attacker, org, 128, AEL_DISCOVERED, qfalse,
                  qtrue); // FIXME: am I on ground or not?
    G_RadiusDamage(org, self, self->splashDamage, self->splashRadius, self,
                   MOD_UNKNOWN);
    ...
    AddSoundEvent(attacker, org, 64, AEL_SUSPICIOUS, qfalse,
                  qtrue); // FIXME: am I on ground or not?
  }

  // FIXME: base numChunks off size?
  CG_Chunks(self->s.number, org, dir, self->absmin, self->absmax, 300,
            numChunks, chunkType, 0, scale, self->noise_index);
...
void funcBBrushDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
...
void funcBBrushUse(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  if (self->spawnflags &
      64) { // Using it doesn't break it, makes it use it's targets
...
void funcBBrushPain(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
  ...
  if (self->material == MAT_DRK_STONE || self->material == MAT_LT_STONE ||
      self->material == MAT_GREY_STONE) {
    vec3_t org, dir;
    float scale;
    VectorSubtract(self->absmax, self->absmin, org); // size
    // This formula really has no logical basis other than the fact that it
    // seemed to be the closest to yielding the results that I wanted. Volume is
    // length * width * height...then break that volume down based on how many
    // chunks we have
    ...
    CG_Chunks(self->s.number, org, dir, self->absmin, self->absmax, 300,
              Q_irand(1, 3), self->material, 0, scale);
  }
...
/*QUAKED func_breakable (0 .8 .5) ? INVINCIBLE IMPACT CRUSHER THIN SABERONLY
HEAVY_WEAP USE_NOT_BREAK PLAYER_USE NO_EXPLOSION INVINCIBLE - can only be broken
by being used IMPACT - does damage on impact CRUSHER - won't reverse movement
when hit an obstacle THIN - can be broken by impact damage, like glass SABERONLY
- only takes damage from sabers HEAVY_WEAP - only takes damage by a heavy
weapon, like an emplaced gun or AT-ST gun. USE_NOT_BREAK - Using it doesn't make
it break, still can be destroyed by damage PLAYER_USE - Player can use it with
the use button NO_EXPLOSION - Does not play an explosion effect, though will
still create chunks if specified

When destroyed, fires it's trigger and chunks and plays sound "noise" or sound
for type if no noise specified
...
"delay"		When killed or used, how long (in seconds) to wait before
blowing up (none by default) "model2"	.md3 model to also draw "modelAngles"
md3 model's angles <pitch yaw roll> (in addition to any rotation on the part of
the brush entity itself) "target"	all entities with a matching targetname
will be used when this is destoryed "health"	default is 10 "radius"  Chunk
code tries to pick a good volume of chunks, but you can alter this to scale the
number of spawned chunks. (default 1)  (.5) is half as many chunks, (2) is twice
as many chunks "NPC_targetname" - Only the NPC with this name can damage this
"forcevisible" - When you turn on force sight (any level), you can see these
draw through the entire level... "redCrosshair" - crosshair turns red when you
look at this
...
void SP_func_breakable(gentity_t *self) {
  ...
  G_SpawnFloat(
      "radius", "1",
      &self->radius); // used to scale chunk code if desired by a designer
  ...
  if (forceVisible) { // can see these through walls with force sight, so must
                      // be broadcast
    if (VectorCompare(self->s.origin, vec3_origin)) { // no origin brush
  ...
  if (redCrosshair) { // can see these through walls with force sight, so must
                      // be broadcast
    self->flags |= FL_RED_CROSSHAIR;
  }
}

void misc_model_breakable_pain(gentity_t *self, gentity_t *inflictor,
                               gentity_t *other, const vec3_t point, int damage,
                               int mod, int hitLoc) {
  if (self->health > 0) {
...
void misc_model_breakable_die(gentity_t *self, gentity_t *inflictor,
  ...
  if (self->e_DieFunc ==
      dieF_NULL) // i was probably already killed since my die func was removed
  {
#ifndef FINAL_BUILD
    gi.Printf(S_COLOR_YELLOW "Recursive misc_model_breakable_die.  Use targets "
                             "probably pointing back at self.\n");
#endif
  ...
  CG_Chunks(self->s.number, dis, dir, self->absmin, self->absmax, 300,
            numChunks, self->material, self->s.modelindex3, scale);
  ...
  if (!(self->spawnflags & 4)) { // We don't want to stay solid
    self->s.solid = 0;
    self->contents = 0;
    self->clipmask = 0;
    if (self != 0) {
  ...
  if (!(self->spawnflags & 2048)) // NO_EXPLOSION
  {
    ...
    if (self->splashDamage > 0 && self->splashRadius > 0) { // explode
      vec3_t org;
      AddSightEvent(attacker, self->currentOrigin, 256, AEL_DISCOVERED, 100);
      AddSoundEvent(attacker, self->currentOrigin, 128, AEL_DISCOVERED, qfalse,
                    qtrue); // FIXME: am I on ground or not?
      // FIXME: specify type of explosion?  (barrel, electrical, etc.)  Also,
      // maybe just use the explosion effect below since it's 				a bit better?
      // up the origin a little for the damage check, because several models
      // have their origin on the ground, so they don't alwasy do damage, not
      // the optimal solution...
      VectorCopy(self->currentOrigin, org);
      if (self->mins[2] >
          -4) { // origin is going to be below it or very very low in the model
        // center the origin
      ...
      if (self->model &&
          (Q_stricmp("models/map_objects/ships/tie_fighter.md3", self->model) ==
               0 ||
           Q_stricmp("models/map_objects/ships/tie_bomber.md3", self->model) ==
               0)) { // TEMP HACK for Tie Fighters- they're HUGE
      ...
      AddSoundEvent(attacker, self->currentOrigin, 64, AEL_SUSPICIOUS, qfalse,
                    qtrue); // FIXME: am I on ground or not?
  ...
  if (self->s.modelindex2 != -1 &&
      !(self->spawnflags & 8)) { // FIXME: modelindex doesn't get set to -1 if
                                 // the damage model doesn't exist
...
void misc_model_throw_at_target4(gentity_t *self, gentity_t *activator) {
  ...
  if (!target) { // nothing to throw ourselves at...
    return;
  }
  ...
  // no trDuration?
  if (self->e_ThinkFunc != thinkF_G_RunObject) { // objects spin themselves?
    // spin it
...
void misc_model_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  if (self->target4) { // throw me at my target!
    misc_model_throw_at_target4(self, activator);
    return;
  }

  if (self->health <= 0 &&
      self->max_health > 0) { // used while broken fired target3
  ...
  if (self->spawnflags & 64) { // Usemodels toggling
...
void TieFighterThink(gentity_t *self) {
  ...
  if (player) {
    ...
    if (fighterSpeed) {
      ...
      // Magic number fun!  Speed is used for banking, so modulate the speed by
      // a sine wave
    ...
    if (dot > 0) { // heading toward the player
      if (playerDist < 1024) {
        if (DotProduct(playerDir, fwd) > 0.7) { // facing the player
          if (self->attackDebounceTime < level.time) {
            ...
            bolt->damage = 30;
            bolt->dflags =
                DAMAGE_NO_KNOCKBACK; // Don't push them around, or else we are
                                     // constantly re-aiming
            bolt->splashDamage = 0;
            bolt->splashRadius = 0;
            ...
            if (!Q_irand(0, 2)) {
              G_SoundOnEnt(bolt, CHAN_VOICE,
                           "sound/weapons/tie_fighter/tie_fire.wav");
            } else {
              G_SoundOnEnt(bolt, CHAN_VOICE,
                           va("sound/weapons/tie_fighter/tie_fire%d.wav",
                              Q_irand(2, 3)));
            }
    ...
    {                      // within range to start our sound
      if (dot > 0) {
        if (!self->fly_sound_debounce_time) { // start sound
          G_SoundOnEnt(
              self, CHAN_VOICE,
              va("sound/weapons/tie_fighter/tiepass%d.wav", Q_irand(1, 5)));
          self->fly_sound_debounce_time = 2000;
        } else { // sound already started
...
void TieFighterUse(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  bolt->owner = self;
  bolt->damage = 30;
  bolt->dflags = DAMAGE_NO_KNOCKBACK; // Don't push them around, or else we are
                                      // constantly re-aiming
  bolt->splashDamage = 0;
  bolt->splashRadius = 0;
  bolt->methodOfDeath = MOD_ENERGY; // ?
...
void TouchTieBomb(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  G_StopEffect(G_EffectIndex("ships/tiebomber_bomb_falling"), self->playerModel,
               gi.G2API_AddBolt(&self->ghoul2[0], "model_root"),
               self->s.number);

  self->e_ThinkFunc = thinkF_G_FreeEntity;
  self->nextthink = level.time + FRAMETIME;
  G_PlayEffect(G_EffectIndex("ships/tiebomber_explosion2"), self->currentOrigin,
               self->currentAngles);
  G_RadiusDamage(self->currentOrigin, self, 900, 500, self,
...
void TieBomberThink(gentity_t *self) {
  // NOTE: Lerp2Angles will think this thinkfunc if the model is a
  // misc_model_breakable. Watchout for that in a script (try to just use
  // ROFF's?).
  ...
  // Time to attack?
  if (player->health > 0 && playerDist < MIN_PLAYER_DIST &&
    ...
    gentity_t *bomb = G_CreateObject(self, self->s.pos.trBase,
                                     self->s.apos.trBase, 0, 0, TR_GRAVITY, 0);
    ...
    // Start the effect.
    G_PlayEffect(G_EffectIndex("ships/tiebomber_bomb_falling"),
                 bomb->playerModel,
                 gi.G2API_AddBolt(&bomb->ghoul2[0], "model_root"),
                 bomb->s.number, bomb->currentOrigin, 1000, qtrue);

    // Set the tie bomb to have a touch function, so when it hits the ground (or
    // whatever), there's a nice 'boom'.
...
void misc_model_breakable_gravity_init(gentity_t *ent, qboolean dropToFloor) {
  ...
  if (!ent->mass) { // not overridden by designer
  ...
  if (dropToFloor) {
    ...
    gi.trace(&tr, top, ent->mins, ent->maxs, bottom, ent->s.number,
             MASK_NPCSOLID, (EG2_Collision)0, 0);
...
START_OFF SOLID - Movement is blocked by it, if not set, can still be broken by
explosions and shots if it has health AUTOANIMATE - Will cycle it's anim
DEADSOLID - Stay solid even when destroyed (in case damage model is rather
large). NO_DMODEL - Makes it NOT display a damage model when destroyed, even if
one exists USE_MODEL - When used, will toggle to it's usemodel (model +
"_u1.md3")... this obviously does nothing if USE_NOT_BREAK is not checked
USE_NOT_BREAK - Using it, doesn't make it break, still can be destroyed by
damage PLAYER_USE - Player can use it with the use button NO_EXPLOSION - By
default, will explode when it dies...this is your override. START_OFF - Will
start off and will not appear until used.

"model"		arbitrary .md3 file to display
"modelscale"	"x" uniform scale
"modelscale_vec" "x y z" scale model in each axis - height, width and length -
bbox will scale with it "health"	how much health to have - default is
zero (not breakable)  If you don't set the SOLID flag, but give it health, it
can be shot but will not block NPCs or players from moving "targetname" when
used, dies and displays damagemodel (model + "_d1.md3"), if any (if not, removes
itself) "target" What to use when it dies "target2" What to use when it's
repaired "target3" What to use when it's used while it's broken "paintarget"
target to fire when hit (but not destroyed) "count"  the amount of
armor/health/ammo given (default 50) "radius"  Chunk code tries to pick a good
volume of chunks, but you can alter this to scale the number of spawned chunks.
(default 1)  (.5) is half as many chunks, (2) is twice as many chunks
...
"throwtarget" if set (along with gravity), this thing, when used, will throw
itself at the entity whose targetname matches this string
"mass"		if gravity is on, this determines how much damage this thing
does when it hits someone.  Default is the size of the object from one corner to
the other, that works very well.  Only override if this is an object whose mass
should be very high or low for it's size (density)
...
void SP_misc_model_breakable(gentity_t *ent) {
  ...
  // Chris F. requested default for misc_model_breakable to be NONE...so don't
  // arbitrarily change this.
  G_SpawnInt("material", "8", (int *)&ent->material);
  G_SpawnFloat(
      "radius", "1",
      &ent->radius); // used to scale chunk code if desired by a designer
  qboolean bHasScale =
      G_SpawnVector("modelscale_vec", "0 0 0", ent->s.modelScale);
  if (!bHasScale) {
    float temp;
  ...
  // HACK
  if (ent->model && Q_stricmp("models/map_objects/ships/x_wing_nogear.md3",
                              ent->model) == 0) {
  ...
  } else if (ent->model && Q_stricmp("models/map_objects/ships/tie_fighter.md3",
                                     ent->model) == 0) { // run a think
    ...
    if (ent->splashDamage > 0 && ent->splashRadius > 0) {
      ...
      // Yeah, I could have just made this value changable from the editor, but
  ...
  } else if (ent->model && Q_stricmp("models/map_objects/ships/tie_bomber.md3",
                                     ent->model) == 0) {
  ...
  if (grav) { // affected by gravity
    G_SetAngles(ent, ent->s.angles);
    G_SetOrigin(ent, ent->currentOrigin);
    G_SpawnString("throwtarget", NULL,
                  &ent->target4); // used to throw itself at something
...
extern void CG_DoGlass(vec3_t verts[4], vec3_t normal, vec3_t dmgPt,
                       vec3_t dmgDir, float dmgRadius);
extern cgs_t cgs;

//-----------------------------------------------------
void funcGlassDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                  int damage, int mod, int dFlags, int hitLoc) {
  vec3_t verts[4], normal;

  // if a missile is stuck to us, blow it up so we don't look dumb....we could,
  // alternately, just let the missile drop off??
  for (int i = 0; i < MAX_GENTITIES; i++) {
    if (g_entities[i].s.groundEntityNum == self->s.number &&
        (g_entities[i].s.eFlags & EF_MISSILE_STICK)) {
      G_Damage(&g_entities[i], self, self, NULL, NULL, 99999, 0,
...
When destroyed, fires it's target, breaks into glass chunks and plays glass
noise For now, instantly breaks on impact

INVINCIBLE - can only be broken by being used

"targetname" entities with matching target will fire it
"target"	all entities with a matching targetname will be used when this
is destroyed "health"	default is 1
*/
//-----------------------------------------------------
...
qboolean G_EntIsBreakable(
    int entityNum,
    gentity_t *breaker) { // breakable brush/model that can actually be broken
  ...
  if (ent->NPC_targetname) { // only a specific entity can break this!
    if (!breaker || !breaker->targetname ||
        Q_stricmp(ent->NPC_targetname, breaker->targetname) !=
            0) { // I'm not the one who can break it
      return qfalse;
    }
  }
