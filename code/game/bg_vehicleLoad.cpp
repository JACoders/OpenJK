...
static qboolean BG_ParseVehWeaponParm(vehWeaponInfo_t *vehWeapon, const char *parmName, char *pValue) {
	...
	case VF_STRING:									// string on disk, pointer in memory
		if (!*(char **)(b + vehWeaponField->ofs)) { // just use 1024 bytes in case we want to write over the string
#ifdef _JK2MP
...
vehField_t vehicleFields[] = {
	...
	{"strafePerc", VFOFS(strafePerc),
	 VF_FLOAT}, // multiplier on current speed for strafing.  If 1.0f, you can strafe at the same speed as you're going forward, 0.5 is half, 0 is no strafing
	...
	{"toughness", VFOFS(toughness),
	 VF_FLOAT}, // modifies incoming damage, 1.0 is normal, 0.5 is half, etc.  Simulates being made of tougher materials/construction
...
void BG_VehicleSetDefaults(vehicleInfo_t *vehicle) {
	...
	/*
		...
		VectorClear( vehicle->centerOfGravity );//offset from origin: {forward, right, up} as a modifier on that dimension (-1.0f is all the way back, 1.0f is
	all the way forward)
	...
	forward, 0.5 is half, 0 is no strafing
		...
		vehicle->toughness = VEH_DEFAULT_TOUGHNESS;	//modifies incoming damage, 1.0 is normal, 0.5 is half, etc.  Simulates being made of tougher
	materials/construction vehicle->malfunctionArmorLevel = 0;			//when armor drops to or below this point, start malfunctioning
		...
		vehicle->cameraHorzOffset = 0.0f;					//how far to left/right (negative/positive) of of the vehicle origin the camera should be - normal
	is 0 vehicle->cameraPitchOffset = 0.0f;					//a modifier on the camera's pitch (up/down angle) to the vehicle - normal is 0 vehicle->cameraFOV =
	0.0f;							//third person camera FOV, default is 80
...
static qboolean BG_ParseVehicleParm(vehicleInfo_t *vehicle, const char *parmName, char *pValue) {
	...
	case VF_FLOAT:
		*(float *)(b + vehField->ofs) = atof(value);
		break;
	case VF_STRING:							  // string on disk, pointer in memory
		if (!*(char **)(b + vehField->ofs)) { // just use 128 bytes in case we want to write over the string
#ifdef _JK2MP
...
int VEH_LoadVehicle(const char *vehicleName) { // load up specified vehicle and save in array: g_vehicleInfo
	...
	if (vehicle->skin && vehicle->skin[0]) {
		...
		for (ratl::string_vs<256>::tokenizer i = skins.begin("|"); i != skins.end(); i++) {
			...
			// this is for the server-side call, it will propgate down to cgame with configstrings and register it at the same time as all the other skins for
			// ghoul2 models
...
extern qboolean trap_G2API_GetBoltMatrix(void *ghoul2, const int modelIndex, const int boltIndex, mdxaBone_t *matrix, const vec3_t angles,
...
void AttachRidersGeneric(Vehicle_t *pVeh) {
	...
	if (pVeh->m_pPilot) {
		...
		// Get the driver tag.
		trap_G2API_GetBoltMatrix(parent->ghoul2, 0, crotchBolt, &boltMatrix, yawOnlyAngles, parent->playerState->origin, BG_GetTime(), NULL,
								 parent->modelScale);
