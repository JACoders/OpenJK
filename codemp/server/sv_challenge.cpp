...
static const size_t SECRET_KEY_LENGTH =
    MD5_DIGEST_SIZE; // Key length equal to digest length is adequate
...
static int SV_CreateChallenge(int timestamp, netadr_t from) {
  ...
  // Create an unforgeable, temporal challenge for this client using
  // HMAC(secretKey, clientParams + timestamp)
  ...
  // Use first 4 bytes of the HMAC digest as an int (client only deals with
  // numeric challenges) The most-significant bit stores whether the timestamp
  // is odd or even. This lets later verification code handle the case where the
  // engine timestamp has incremented between the time this challenge is sent
  ...
  if (com_developer->integer) {
    Com_Printf("Generated challenge %d (timestamp = %d) for %s\n", challenge,
               timestamp, NET_AdrToString(from));
  }
#endif
...
int SV_CreateChallenge(netadr_t from) {
  if (!challengerInitialized) {
    Com_Error(
        ERR_FATAL,
        "SV_CreateChallenge: The challenge subsystem has not been initialized");
  }

  // The current time gets 14 bits chopped off to create a challenge timestamp
  // that changes every 16.384 seconds This allows clients at least ~16 seconds
  // from now to reply to the challenge
...
qboolean SV_VerifyChallenge(int receivedChallenge, netadr_t from) {
  if (!challengerInitialized) {
    Com_Error(
        ERR_FATAL,
        "SV_VerifyChallenge: The challenge subsystem has not been initialized");
  }

  int currentTimestamp = svs.time >> 14;
  int currentPeriod = currentTimestamp & 0x1;

  // Use the current timestamp for verification if the current period matches
  // the client challenge's period. Otherwise, use the previous timestamp in
  // case the current timestamp incremented in the time between the client being
  // sent a challenge and the client's reply that's being verified now.
