...
void EWebPositionUser(gentity_t *owner, gentity_t *eweb) {
	...
	if (owner->s.number < MAX_CLIENTS) { // extra checks
		...
		if (tr.startsolid || tr.allsolid) { // crap, they're already in solid somehow, don't bother tracing over
			traceOver = qfalse;
		}
	}
	...
	// trace over
	gi.G2API_GetBoltMatrix(eweb->ghoul2, 0, eweb->headBolt, &boltMatrix, eweb->s.apos.trBase, eweb->currentOrigin, (cg.time ? cg.time : level.time), NULL,
						   eweb->s.modelScale);
	...
	if (!traceOver) {
		...
		if (owner->s.number < MAX_CLIENTS) { // extra checks
			...
			if (tr.startsolid || tr.allsolid) { // would be in solid there, so just trace over, I guess?
	...
	if (!tr.startsolid && !tr.allsolid) {
		...
		{									// all clear, we can move there
			...
			if (moveDist > 4.0f) { // moved past the threshold from last position
				...
				if (DotProduct(moveDir, oRight) > 0) { // moved to his right, play right strafe
...
qboolean eweb_can_be_used(gentity_t *self, gentity_t *other, gentity_t *activator) {
	...
	if (other && other->client && G_IsRidingVehicle(other)) { // can't use eweb when on a vehicle
		return qfalse;
	}

	if (activator && activator->client && G_IsRidingVehicle(activator)) { // can't use eweb when on a vehicle
		return qfalse;
	}

	if (activator && activator->client && (activator->client->ps.pm_flags & PMF_DUCKED)) { // stand up, ya cowardly varmint!
		return qfalse;
	}

	if (activator && activator->health <= 0) { // dead men ain't got no more use fer guns...
		return qfalse;
	}
	...
	if (activator->s.number < MAX_CLIENTS) { // player must be facing general direction of the turret head
...
void eweb_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
	...
	// Ugh, so much AI code seems to assume enemies are clients, maybe this shouldn't be on, but it's too late in the game to change it now without knowing what
	...
	if (!(self->spawnflags & EMPLACED_PLAYERUSE) || activator->s.number == 0) { // player-only usescript or any usescript
		// Run use script
...
void SP_emplaced_eweb(gentity_t *ent) {
	...
	gi.G2API_SetBoneAnglesIndex(&ent->ghoul2[ent->playerModel], ent->lowerLumbarBone, vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_X, NEGATIVE_Y,
								NULL, 0, 0);
	gi.G2API_SetBoneAnglesIndex(&ent->ghoul2[ent->playerModel], ent->upperLumbarBone, vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_X, NEGATIVE_Y,
								NULL, 0, 0);
...
void emplaced_gun_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
	...
	if (self->delay + 500 < level.time) {
		...
		// Since we move the activator inside of the gun, we reserve a solid spot where they were standing in order to be able to get back out without being in
		// solid
		...
		// Let the gun be considered an enemy
		// Ugh, so much AI code seems to assume enemies are clients, maybe this shouldn't be on, but it's too late in the game to change it now without knowing
		// what side-effects this will have
		self->svFlags |= SVF_NONNPC_ENEMY;
		self->noDamageTeam = activator->client->playerTeam;
...
void SP_emplaced_gun(gentity_t *ent) {
	...
	gi.G2API_SetBoneAnglesIndex(&ent->ghoul2[ent->playerModel], ent->lowerLumbarBone, vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X,
								NULL, 0, 0);
...
void ExitEmplacedWeapon(gentity_t *ent) {
	...
	if (ent->client) {
		...
		if (ent->health > 0) { // he's still alive, and we have a placeholder, so put him back
		...
		if (!ent->owner->bounceCount) { // not an EWeb - the overridden bone angles will remember the angle we left it at
	...
	if (ent->health <= 0) { // when die, don't set weapon back on when ejected from emplaced/eweb
		// empty hands
		...
		if (ent->NPC) { // BTW, if a saber-using NPC ever gets off of an emplaced gun/eweb, this will not work, look at NPC_ChangeWeapon for the proper way
