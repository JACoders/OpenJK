...
//#define DUAL_SPIN_PROTECT_POWER	50	//power required to do the dual
//spin attack
//#define SINGLE_SPECIAL_POWER	20	//power required to do the single saber
//special attacks
...
#define PLAYER_KNOCKDOWN_HOLD_EXTRA_TIME                                       \
  4000 // player stays down after a knockdown for 4 whole seconds before
       // automatically doing one of the slow get-ups
...
#define MEF_MULTI_SPIN 0x000040   // spin once every second, until the effect
                                // stops
...
#define FORCE_POWERS_ROSH_FROM_TWINS                                           \
  ((1 << FP_SPEED) | (1 << FP_GRIP) | (1 << FP_RAGE) | (1 << FP_SABERTHROW))
...
extern qboolean WP_SaberParseParms(const char *SaberName, saberInfo_t *saber,
                                   qboolean setColors = qtrue);
extern qboolean WP_BreakSaber(gentity_t *ent, const char *surfName,
...
extern qboolean G_GetHitLocFromSurfName(gentity_t *ent, const char *surfName,
                                        ...
                                        saberType_t saberType = SABER_NONE);
extern qboolean G_CheckEnemyPresence(gentity_t *ent, int dir, float radius,
                                     float tolerance = 0.75f);
extern void WP_SaberFreeStrings(saberInfo_t &saber);
extern qboolean G_EnoughPowerForSpecialMove(int forcePower, int cost,
                                            qboolean kataMove = qfalse);
extern void G_DrainPowerForSpecialMove(gentity_t *self, forcePowers_t fp,
                                       int cost, qboolean kataMove = qfalse);
extern int G_CostForSpecialMove(int cost, qboolean kataMove = qfalse);
extern gentity_t *G_DropSaberItem(const char *saberType,
                                  saber_colors_t saberColor, vec3_t saberPos,
                                  vec3_t saberVel, vec3_t saberAngles,
                                  gentity_t *copySaber = NULL);
...
typedef enum { SWING_FAST, SWING_MEDIUM, SWING_STRONG } swingType_t;
...
// sophisticated move-picker once the game gets out of prototype stage. <- HAHA!
