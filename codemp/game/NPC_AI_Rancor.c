...
extern void G_Dismember(gentity_t *ent, gentity_t *enemy, vec3_t point, int limbType, float limbRollBase, float limbPitchBase, int deathAnim,
						qboolean postDeath);
...
void Rancor_Swing(qboolean tryGrab) {
	...
	for (i = 0; i < numEnts; i++) {
		...
		if (DistanceSquared(radiusEnt->r.currentOrigin, boltOrg) <= radiusSquared) {
			if (tryGrab && NPCS.NPC->count != 1 // don't have one in hand or in mouth already - FIXME: allow one in hand and any number in mouth!
				...
				*/
				...
				if (radiusEnt->client->NPC_class != CLASS_RANCOR && radiusEnt->client->NPC_class != CLASS_ATST) {
					G_Damage(radiusEnt, NPCS.NPC, NPCS.NPC, vec3_origin, radiusEnt->r.currentOrigin, Q_irand(25, 40), DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK,
							 MOD_MELEE);
	...
	for (i = 0; i < numEnts; i++) {
		...
		if (distSq <= radiusSquared) {
			...
			if (distSq < halfRadSquared) { // close enough to do damage, too
				G_Damage(radiusEnt, NPCS.NPC, NPCS.NPC, vec3_origin, radiusEnt->r.currentOrigin, Q_irand(10, 25), DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK,
						 MOD_MELEE);
			}
	...
	if (TIMER_Done2(NPCS.NPC, "attack_dmg", qtrue)) {
		...
		case BOTH_ATTACK1:
			if (NPCS.NPC->count == 1 && NPCS.NPC->activator) {
				G_Damage(NPCS.NPC->activator, NPCS.NPC, NPCS.NPC, vec3_origin, NPCS.NPC->activator->r.currentOrigin, Q_irand(25, 40),
				...
				if (NPCS.NPC->activator->health <= 0) { // killed him
					...
					G_Dismember(NPCS.NPC->activator, NPCS.NPC, NPCS.NPC->activator->r.currentOrigin, G2_MODELPART_HEAD, 90, 0,
								NPCS.NPC->activator->client->ps.torsoAnim, qtrue);
		...
		case BOTH_ATTACK3:
			if (NPCS.NPC->count == 1 && NPCS.NPC->activator) {
				...
				if (NPCS.NPC->activator->client) {
					// NPC->activator->client->dismembered = qfalse;
					G_Dismember(NPCS.NPC->activator, NPCS.NPC, NPCS.NPC->activator->r.currentOrigin, G2_MODELPART_WAIST, 90, 0,
								NPCS.NPC->activator->client->ps.torsoAnim, qtrue);
					// G_DoDismemberment( NPC->activator, NPC->enemy->r.currentOrigin, MOD_SABER, 1000, HL_WAIST, qtrue );
				}
				// KILL
				G_Damage(NPCS.NPC->activator, NPCS.NPC, NPCS.NPC, vec3_origin, NPCS.NPC->activator->r.currentOrigin, NPCS.NPC->enemy->health + 10,
		...
		case BOTH_ATTACK3:
			if (NPCS.NPC->count == 1 && NPCS.NPC->activator) { // swallow victim
				...
				if (NPCS.NPC->activator->client) { //*sigh*, can't get tags right, just remove them?
...
void Rancor_Crush(void) {
	...
	if (!NPCS.NPC || !NPCS.NPC->client || NPCS.NPC->client->ps.groundEntityNum >= ENTITYNUM_WORLD) { // nothing to crush
		return;
	}
...
void NPC_BSRancor_Default(void) {
	...
	if (TIMER_Done2(NPCS.NPC, "clearGrabbed", qtrue)) {
		Rancor_DropVictim(NPCS.NPC);
	} else if (NPCS.NPC->client->ps.legsAnim == BOTH_PAIN2 && NPCS.NPC->count == 1 && NPCS.NPC->activator) {
		if (!Q_irand(0, 3)) {
			Rancor_CheckDropVictim();
		}
	}
	if (!TIMER_Done(NPCS.NPC, "rageTime")) {												// do nothing but roar first time we see an enemy
	...
	if (NPCS.NPC->enemy) {
		...
		if ( NPC->enemy->client //enemy is a client
			...
			&& (!NPC->enemy->enemy || !NPC->enemy->enemy->client || NPC->enemy->enemy->client->NPC_class!=CLASS_RANCOR) )//enemy's enemy is not a client or is
		not a rancor (which is as scary as me anyway)
		...
		*/
		...
		if (NPCS.NPC->count == 2 && NPCS.NPC->client->ps.legsAnim == BOTH_ATTACK3) { // we're still chewing our enemy up
			NPC_UpdateAngles(qtrue, qtrue);
			return;
		}
		// else, if he's in our hand, we eat, else if he's on the ground, we keep attacking his dead body for a while
		if (NPCS.NPC->enemy->client && NPCS.NPC->enemy->client->NPC_class == CLASS_RANCOR) { // got mad at another Rancor, look for a valid enemy
