...
#define BASE_COMPAT // some unused and leftover code has been stripped out, but
                    // this breaks compatibility
                    //	between base<->modbase clients and servers (mismatching
                    //events, powerups, etc)
...
// rww - a C-ified structure version of the class which fires off callbacks and
// gives arguments to update ragdoll status.
enum sharedERagPhase {
  ...
  RP_SET_PELVIS_OFFSET, // this actually does nothing but set the
                        // pelvisAnglesOffset, and pelvisPositionOffset
  RP_DISABLE_EFFECTORS // this removes effectors given by the effectorsToTurnOff
                       // member
};
...
typedef struct sharedRagDollParams_s {
  ...
  int effectorsToTurnOff; // set this to an | of the above flags for a
...
typedef struct sharedSetBoneIKStateParams_s {
  ...
  qboolean forceAnimOnBone; // normally if the bone has specified start/end
                            // frames already it will leave it alone.. if this is
                            // true, then the animation will be restarted on the
                            // bone with the specified frames anyway.
...
#define CVAR_ARCHIVE                                                           \
  (0x00000001u) // set to cause it to be saved to configuration file. used for
                // system variables,
...
#define CVAR_SYSTEMINFO                                                        \
  (0x00000008u) // these cvars will be duplicated on all clients
#define CVAR_INIT                                                              \
  (0x00000010u) // don't allow change from console at all, but can be set from
                // the command line
#define CVAR_LATCH                                                             \
  (0x00000020u) // will only change when C code next does a Cvar_Get(), so it
                // can't be changed
                //	without proper initialization. modified will be set,
                //even though the value 	hasn't changed yet
#define CVAR_ROM                                                               \
  (0x00000040u) // display only, cannot be set by user at all (can be set by
                // code)
#define CVAR_USER_CREATED (0x00000080u) // created by a set command
#define CVAR_TEMP                                                              \
  (0x00000100u) // can be set even when cheats are disabled, but is not archived
#define CVAR_CHEAT (0x00000200u) // can not be changed if cheats are disabled
#define CVAR_NORESTART                                                         \
  (0x00000400u) // do not clear when a cvar_restart is issued
#define CVAR_INTERNAL                                                          \
  (0x00000800u) // cvar won't be displayed, ever (for passwords and such)
#define CVAR_PARENTAL                                                          \
  (0x00001000u) // lets cvar system know that parental stuff needs to be updated
#define CVAR_SERVER_CREATED                                                    \
  (0x00002000u) // cvar was created by a server the client connected to.
#define CVAR_VM_CREATED                                                        \
  (0x00004000u) // cvar was created exclusively in one of the VMs.
#define CVAR_PROTECTED                                                         \
  (0x00008000u) // prevent modifying this var from VMs or the server
#define CVAR_NODEFAULT                                                         \
  (0x00010000u) // do not write to config if matching with default value
...
typedef CollisionRecord_t
    G2Trace_t[MAX_G2_COLLISIONS]; // map that describes all of the parts of
                                  // ghoul2 models that got hit
...
typedef struct trace_s {
  ...
  */
  ...
  //	CollisionRecord_t G2CollisionMap[MAX_G2_COLLISIONS];	// map that
  //describes all of the parts of ghoul2 models that got hit
  /*
  Ghoul2 Insert End
  */
} trace_t;
...
typedef enum {
  ...
  CHAN_MUSIC, //## %s !!"W:\game\base\!!sound\*.wav;*.mp3" #music played as a
              //looping sound - added by BTO (VV)
} soundChannel_t;
...
#define	GENTITYNUM_BITS		11
                                                        //rww - I am raising
this 1 bit. SP actually has room for 1024 ents - none - world - 1 client.
                                                        //Which means 1021
useable entities. However we have 32 clients.. so if we keep our limit
                                                        //at 1024 we are not
going to be able to load any SP levels at the edge of the ent limit. #define
MAX_GENTITIES	(1024+(MAX_CLIENTS-1))
                                                        //rww - we do have
enough room to send over 2048 ents now. However, I cannot live with the guilt of
                                                        //actually increasing
the entity limit to 2048 (as it would slow down countless things, and
                                                        //there are tons of ent
list traversals all over the place). So I am merely going to give enough
...
*/
#define MAX_G2BONES                                                            \
  64 // rww - changed from MAX_CHARSKINS to MAX_G2BONES. value still equal.
/*
Ghoul2 Insert End
*/

#define MAX_AMBIENT_SETS                                                       \
  256 // rww - ambient soundsets must be sent over in config strings.

#define MAX_CONFIGSTRINGS                                                      \
  1700 // this is getting pretty high. Try not to raise it anymore than it
       // already is.
...
typedef struct forcedata_s {
  ...
  float
      forceGripBeingGripped; // if > level.time then client is in someone's grip
  int forceGripCripple;      // if != 0 then make it so this client can't move
                        // quickly (he's being gripped)
  ...
  // This hurts me somewhat to do, but there's no other real way to allow
  // completely "dynamic" mindtricking.
  ...
  int killSoundEntIndex[TRACK_CHANNEL_MAX]; // this goes here so it doesn't get
                                            // wiped over respawn
...
#define MAX_AMMO_TRANSMIT                                                      \
  16 // This is needed because the ammo array is 19 but only 16 sized array is
     // networked
...
// Crazy optimization attempt to take all those 1 bit values and shove them into
// a single send. May help us not have to send so many 1/0 bits to acknowledge
// modified values. -rww

#define _OPTIMIZED_VEHICLE_NETWORKING
// Instead of sending 2 full playerStates for the pilot and the vehicle, send a
// smaller, specialized pilot playerState and vehicle playerState.  Also removes
// some vehicle fields from the normal playerState -mcg
...
typedef struct playerState_s {
  ...
  int gravity;
  float speed;
  int basespeed; // used in prediction to know base server g_speed value when
                 // modifying speed between updates
  int delta_angles[3]; // add to command angles to get view direction
                       // changed by spawns, rotating objects, and teleporters

  int slopeRecalcTime; // this is NOT sent across the net and is maintained
                       // seperately on game and cgame in pmove code.
  ...
  qboolean legsFlip; // set to opposite when the same anim needs restarting,
                     // sent over in only 1 bit. Cleaner and makes porting easier
                     // than having that god forsaken ANIM_TOGGLEBIT.
  qboolean torsoFlip;
  ...
  int eFlags2; // copied to entityState_t->eFlags2, EF2_??? used much less
               // frequently
  ...
  int painTime; // used for both game and client side to process the pain twitch
  ...
  int saberLockTime;
  int saberLockEnemy;
  int saberLockFrame; // since we don't actually have the ability to get the
                      // current anim frame
  int saberLockHits; // every x number of buttons hits, allow one push forward
                     // in a saber lock (server only)
  int saberLockHitCheckTime; // so we don't allow more than 1 push per server
                             // frame
  int saberLockHitIncrementTime; // so we don't add a hit per attack button
                                 // press more than once per server frame
  ...
  float holocronCantTouchTime; // for keeping track of the last holocron that
                               // just popped out of me (if any)
  int holocronBits;
  ...
  int heldByClient; // can only be a client index - this client should be
                    // holding onto my arm using IK stuff.
  ...
  // vehicle turnaround stuff (need this in ps so it doesn't jerk too much in
  // prediction)
...
// Here's an interesting bit.  The bots in TA used buttons to do additional
// gestures. I ripped them out because I didn't want too many buttons given the
// fact that I was already adding some for JK2. We can always add some back in
// if we want though.
/*
...
// rww - unsightly hack to allow us to make an FX call that takes a horrible
// amount of args
...
typedef struct entityState_s {
  ...
  // rww - these were originally because we shared g2 info client and server
  // side. Now they just get used as generic values everywhere.
  int bolt1;
  int bolt2;

  // rww - this is necessary for determining player visibility during a jedi
  // mindtrick
  ...
  qboolean loopIsSoundset; // qtrue if the loopSound index is actually a
                           // soundset index
  ...
  qboolean isPortalEnt; // this needs to be seperate for all entities I guess,
                        // which is why I couldn't reuse another value.
  ...
  int iModelScale; // rww - transfer a percentage of the normal scale in a
                   // single int instead of 3 x-y-z scale values

  int brokenLimbs;

  int boltToPlayer; // set to index of a real client+1 to bolt the ent to that
                    // client. Must be a real client, NOT an NPC.
  ...
  // I didn't want to do this, but I.. have no choice. However, we aren't
  // setting this for all ents or anything, only ones we want health knowledge
  // about on cgame (like siege objective breakables) -rww
  int health;
  ...
  int surfacesOn; // a bitflag of corresponding surfaces from a lookup table.
                  // These surfaces will be forced on.
...
typedef struct SSkinGoreData_s {
  ...
  qboolean fadeRGB; // specify fade method to modify RGB (by default, the alpha
                    // is set instead)
} SSkinGoreData;
