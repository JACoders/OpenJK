...
// This stuff looks a bit messy, but it's kept here as black box, and nothing
// appears in any .H files for other
...
void RE_RegisterModels_StoreShaderRequest(const char *psModelFileName,
  ...
  if (ModelBin.pModelDiskImage == NULL) {
    assert(0); // should never happen, means that we're being called on a model
               // that wasn't loaded
  } else {
    int iNameOffset = psShaderName - (char *)ModelBin.pModelDiskImage;
    int iPokeOffset =
        (char *)piShaderIndexPoke - (char *)ModelBin.pModelDiskImage;

    ModelBin.ShaderRegisterData.push_back(
        StringOffsetAndShaderIndexDest_t(iNameOffset, iPokeOffset));
  }
}

static const byte FakeGLAFile[] = {
    ,
    ...
    0x00, 0x00, 0x00, 0x00, 0xFD, 0xBF, 0xFE, 0x7F, 0xFE, 0x7F, 0xFE, 0x7F,
    0x00, 0x80, 0x00, 0x80, 0x00, 0x80};

void RE_LoadWorldMap_Actual(const char *name, world_t &worldData, int index);

// returns qtrue if loaded, and sets the supplied qbool to true if it was from
// cache (instead of disk)
//   (which we need to know to avoid LittleLong()ing everything again (well, the
//   Mac needs to know anyway)...
//
// don't use ri->xxx functions in case running on dedicated...
//
qboolean RE_RegisterModels_GetDiskFile(const char *psModelFileName,
                                       void **ppvBuffer,
                                       qboolean *pqbAlreadyCached) {
  char sModelName[MAX_QPATH];
  ...
  if (ModelBin.pModelDiskImage == NULL) {
    ...
    if (!strcmp(sDEFAULT_GLA_NAME ".gla", psModelFileName)) {
      ...
      *pqbAlreadyCached = qfalse; // faking it like this should mean that it
                                  // works fine on the Mac as well
      return qtrue;
    }
...
// don't use ri->xxx functions in case running on dedicated
//
void *RE_RegisterModels_Malloc(int iSize, void *pvDiskBufferIfJustLoaded,
                               const char *psModelFileName,
  ...
  if (ModelBin.pModelDiskImage == NULL) {
    // ... then this entry has only just been created, ie we need to load it
    // fully...
    //
    // new, instead of doing a Z_Malloc and assigning that we just morph the
    // disk buffer alloc
    //	then don't thrown it away on return - cuts down on mem overhead
    //
    // ... groan, but not if doing a limb hierarchy creation (some VV stuff?),
    ...
    if (ri.FS_FileIsInPAK(sModelName, &iCheckSum) == 1) {
      ModelBin.iPAKFileCheckSum =
          iCheckSum; // else ModelBin's constructor will leave it as -1
    }

    *pqbAlreadyFound = qfalse;
  } else {
    // if we already had this model entry, then re-register all the shaders it
    // wanted...
    //
    ...
    for (int i = 0; i < iEntries; i++) {
      ...
      char *psShaderName =
          &((char *)ModelBin.pModelDiskImage)[iShaderNameOffset];
      int *piShaderPokePtr =
          (int *)&((char *)ModelBin.pModelDiskImage)[iShaderPokeOffset];

      shader_t *sh =
          R_FindShader(psShaderName, lightmapsNone, stylesDefault, qtrue);
    ...
    *pqbAlreadyFound =
        qtrue; // tell caller not to re-Endian or re-Shader this binary
  }
...
// Unfortunately the dedicated server also hates shader loading. So we need an
// alternate of this func.
//
void *RE_RegisterServerModels_Malloc(int iSize, void *pvDiskBufferIfJustLoaded,
                                     const char *psModelFileName,
                                     qboolean *pqbAlreadyFound, memtag_t eTag) {
  char sModelName[MAX_QPATH];
  ...
  if (ModelBin.pModelDiskImage == NULL) {
    ...
    *pqbAlreadyFound = qfalse;
    ...
    for (int i=0; i<iEntries; i++)
    {
            ...
            char *psShaderName		=
    &((char*)ModelBin.pModelDiskImage)[iShaderNameOffset]; int  *piShaderPokePtr
    = (int *) &((char*)ModelBin.pModelDiskImage)[iShaderPokeOffset];

            shader_t *sh = R_FindShader( psShaderName, lightmapsNone,
    stylesDefault, qtrue );
...
// dump any models not being used by this level if we're running low on
// memory...
//
static int GetModelDataAllocSize(void) {
  return Z_MemSize(TAG_MODEL_MD3) + Z_MemSize(TAG_MODEL_GLM) +
         Z_MemSize(TAG_MODEL_GLA);
}
extern cvar_t *r_modelpoolmegs;
//
// return qtrue if at least one cached model was freed (which tells
// z_malloc()-fail recoveryt code to try again)
//
extern qboolean gbInsideRegisterModel;
qboolean RE_RegisterModels_LevelLoadEnd(
    qboolean bDeleteEverythingNotUsedThisLevel /* = qfalse */) {
  ...
  if (gbInsideRegisterModel) {
    ri.Printf(PRINT_DEVELOPER,
              "(Inside RE_RegisterModel (z_malloc recovery?), exiting...\n");
  } else {
    int iLoadedModelBytes = GetModelDataAllocSize();
    const int iMaxModelBytes = r_modelpoolmegs->integer * 1024 * 1024;

    for (CachedModels_t::iterator itModel = CachedModels->begin();
         itModel != CachedModels->end() &&
         (bDeleteEverythingNotUsedThisLevel ||
          iLoadedModelBytes > iMaxModelBytes);) {
      ...
      if (bDeleteEverythingNotUsedThisLevel) {
        bDeleteThis =
            (CachedModel.iLastLevelUsedOn != RE_RegisterMedia_GetLevel())
                ? qtrue
                : qfalse;
      } else {
        bDeleteThis =
      ...
      if (bDeleteThis) {
        ...
        if (CachedModel.pModelDiskImage) {
          ...
          // CachedModel.pModelDiskImage = NULL;	// REM for reference, erase()
          // call below negates the need for it.
  ...
  ri.Printf(PRINT_DEVELOPER,
            S_COLOR_RED "RE_RegisterModels_LevelLoadEnd(): Ok\n");

  return bAtLeastoneModelFreed;
}

// scan through all loaded models and see if their PAK checksums are still valid
// with the current pure PAK lists,
//	dump any that aren't (so people can't cheat by using models with huge
//spikes that show through walls etc)
//
...
static void RE_RegisterModels_DumpNonPure(void) {
  ...
  for (CachedModels_t::iterator itModel = CachedModels->begin();
    ...
    if (iInPak == -1 || iCheckSum != CachedModel.iPAKFileCheckSum) {
      if (Q_stricmp(sDEFAULT_GLA_NAME ".gla",
                    psModelName)) // don't dump "*default.gla", that's program
                                  // internal anyway
      {
        // either this is not from a PAK, or it's from a non-pure one, so ditch
        // it...
        //
        ri.Printf(PRINT_DEVELOPER, "Dumping none pure model \"%s\"",
                  psModelName);
...
void RE_RegisterModels_Info_f(void) {
  ...
  for (CachedModels_t::iterator itModel = CachedModels->begin();
       itModel != CachedModels->end(); ++itModel, iModel++) {
    CachedEndianedModelBinary_t &CachedModel = (*itModel).second;

    ri.Printf(PRINT_ALL, "%d/%d: \"%s\" (%d bytes)", iModel, iModels,
              (*itModel).first.c_str(), CachedModel.iAllocSize);
...
// do not use ri->xxx functions in here, the renderer may not be running (ie. if
...
void RE_RegisterMedia_LevelLoadBegin(const char *psMapName,
  ...
  // for development purposes we may want to ditch certain media just before
  // loading a map...
  //
  bool bDeleteModels =
      eForceReload == eForceReload_MODELS || eForceReload == eForceReload_ALL;
  //	bool bDeleteBSP		= eForceReload == eForceReload_BSP    ||
  ...
  //	{
  //		CM_DeleteCachedMap();
  R_Images_DeleteLightMaps(); // always do this now, makes no real load time
                              // difference, and lets designers work ok
                              //	}

  // at some stage I'll probably want to put some special logic here, like not
  // incrementing the level number
  //	when going into a map like "brig" or something, so returning to the
  //previous level doesn't require an 	asset reload etc, but for now...
  //
  // only bump level number if we're not on the same level.
  //	Note that this will hide uncached models, which is perhaps a bad
  //thing?...
  //
...
int RE_RegisterMedia_GetLevel(void) { return giRegisterMedia_CurrentLevel; }
...
// rww - Please forgive me for all of the below. Feel free to destroy it and
// replace it with something better. You obviously can't touch anything relating
// to shaders or ri-> functions here in case a dedicated server is running, which
// is the entire point of having these seperate functions. If anything major is
// changed in the non-server-only versions of these functions it would be wise to
// incorporate it here as well.
...
=================
*/
qboolean ServerLoadMDXA(model_t *mod, void *buffer, const char *mod_name,
                        qboolean &bAlreadyCached) {
  ...
  mdxa = mod->mdxa = (mdxaHeader_t *) // Hunk_Alloc( size );
      RE_RegisterServerModels_Malloc(size, buffer, mod_name, &bAlreadyFound,
                                     TAG_MODEL_GLA);
...
=================
*/
qboolean ServerLoadMDXM(model_t *mod, void *buffer, const char *mod_name,
                        qboolean &bAlreadyCached) {
  int i, l, j;
...
Same as RE_RegisterModel, except used by the server to handle ghoul2 instance
models.
====================
*/
qhandle_t RE_RegisterServerModel(const char *name) {
  ...
  */
  ...
  if (!r_noServerGhoul2) { // keep it from choking when it gets to these checks
                           // in the g2 code. Registering all r_ cvars for the
                           // server would be a Bad Thing though.
  ...
  int iLODStart = 0;
  if (strstr(name, ".md3")) {
    iLODStart = MD3_MAX_LODS -
                1; // this loads the md3s in reverse so they can be biased
  }
  mod->numLods = 0;
  ...
  for (lod = iLODStart; lod >= 0; lod--) {
    ...
    if (!RE_RegisterModels_GetDiskFile(filename, (void **)&buf,
                                       &bAlreadyCached)) {
      continue;
    }
    ...
    // important that from now on we pass 'filename' instead of 'name' to all
    // model load functions,
    //	because 'filename' accounts for any LOD mangling etc so guarantees
    //unique lookups for yet more 	internal caching...
    //
    ident = *(unsigned *)buf;
    if (!bAlreadyCached) {
      LL(ident);
    ...
    switch (ident) { // if you're trying to register anything else as a model
                     // type on the server, you are out of luck
...
static qhandle_t RE_RegisterModel_Actual(const char *name) {
  ...
  */
  //	if (!tr.registered) {
  //		ri.Printf( PRINT_ALL, S_COLOR_YELLOW  "RE_RegisterModel (%s) called
  //before ready!\n",name ); 		return 0;
  //	}
  //
  ...
  if (name[0] == '#') {
    ...
    RE_LoadWorldMap_Actual(va("maps/%s.bsp", name + 1),
                           tr.bspModels[tr.numBSPModels - 1], tr.numBSPModels);
  ...
  if (name[0] == '*') { // don't create a bad model for a bsp model
  ...
  */
  ...
  for (lod = iLODStart; lod >= 0; lod--) {
    ...
    default:
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW "RE_RegisterModel: unknown fileid for %s\n",
                filename);
      goto fail;
    }
...
#ifdef _DEBUG
    if (r_noPrecacheGLA && r_noPrecacheGLA->integer &&
        ident == MDXA_IDENT) { // I expect this will cause leaks, but I don't
                               // care because it's a debugging utility.
      return mod->index;
    }
#endif
...
// wrapper function needed to avoid problems with mid-function returns so I can
// safely use this bool to tell the
//	z_malloc-fail recovery code whether it's safe to ditch any model
//caches...
//
...
static qboolean R_LoadMD3(model_t *mod, int lod, void *buffer,
                          const char *mod_name, qboolean &bAlreadyCached) {
  int i, j;
  ...
  if (!bAlreadyFound) {
    ...
    //		memcpy( mod->md3[lod], buffer, size );	// and don't do this now,
  ...
  for (i = 0; i < mod->md3[lod]->numSurfaces; i++) {
    ...
    for (j = 0; j < surf->numShaders; j++, shader++) {
      ...
      RE_RegisterModels_StoreShaderRequest(mod_name, &shader->name[0],
                                           &shader->shaderIndex);
    }
...
void R_SVModelInit() { R_ModelInit(); }
...
void RE_HunkClearCrap(
    void) { // get your dirty sticky assets off me, you damn dirty hunk!
