...
static void misc_lightstyle_set(gentity_t *ent) {
  ...
  if (!ent->alt_fire) { // turn off
    ...
    {
      char lightstyle[32];
      trap->GetConfigstring(CS_LIGHT_STYLES + (mLightOffStyle * 3) + 0,
                            lightstyle, 32);
      trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle * 3) + 0,
                            lightstyle);

      trap->GetConfigstring(CS_LIGHT_STYLES + (mLightOffStyle * 3) + 1,
                            lightstyle, 32);
      trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle * 3) + 1,
                            lightstyle);

      trap->GetConfigstring(CS_LIGHT_STYLES + (mLightOffStyle * 3) + 2,
                            lightstyle, 32);
      trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle * 3) + 2,
                            lightstyle);
    } else {
    ...
    {
      char lightstyle[32];
      trap->GetConfigstring(CS_LIGHT_STYLES + (mLightSwitchStyle * 3) + 0,
                            lightstyle, 32);
      trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle * 3) + 0,
                            lightstyle);

      trap->GetConfigstring(CS_LIGHT_STYLES + (mLightSwitchStyle * 3) + 1,
                            lightstyle, 32);
      trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle * 3) + 1,
                            lightstyle);

      trap->GetConfigstring(CS_LIGHT_STYLES + (mLightSwitchStyle * 3) + 2,
                            lightstyle, 32);
      trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle * 3) + 2,
                            lightstyle);
    } else {
...
/*QUAKED misc_model_breakable (1 0 0) (-16 -16 -16) (16 16 16) SOLID AUTOANIMATE
DEADSOLID NO_DMODEL NO_SMOKE USE_MODEL USE_NOT_BREAK PLAYER_USE NO_EXPLOSION
SOLID - Movement is blocked by it, if not set, can still be broken by explosions
and shots if it has health AUTOANIMATE - Will cycle it's anim DEADSOLID - Stay
solid even when destroyed (in case damage model is rather large). NO_DMODEL -
Makes it NOT display a damage model when destroyed, even if one exists USE_MODEL
- When used, will toggle to it's usemodel (model name + "_u1.md3")... this
obviously does nothing if USE_NOT_BREAK is not checked USE_NOT_BREAK - Using it,
doesn't make it break, still can be destroyed by damage PLAYER_USE - Player can
use it with the use button NO_EXPLOSION - By default, will explode when it
dies...this is your override.

"model"		arbitrary .md3 file to display
"health"	how much health to have - default is zero (not breakable)  If
you don't set the SOLID flag, but give it health, it can be shot but will not
block NPCs or players from moving "targetname" when used, dies and displays
damagemodel, if any (if not, removes itself) "target" What to use when it dies
...
"radius"  Chunk code tries to pick a good volume of chunks, but you can alter
this to scale the number of spawned chunks. (default 1)  (.5) is half as many
chunks, (2) is twice as many chunks
...
void SP_misc_model_breakable(gentity_t *ent) {
  ...
  G_SpawnFloat(
      "radius", "1",
      &ent->radius); // used to scale chunk code if desired by a designer
...
void misc_model_breakable_gravity_init(gentity_t *ent, qboolean dropToFloor) {
  ...
  ent->physicsBounce = ent->mass =
      VectorLength(ent->r.maxs) + VectorLength(ent->r.mins);
  ...
  if (dropToFloor) {
    ...
    trap->Trace(&tr, top, ent->r.mins, ent->r.maxs, bottom, ent->s.number,
                MASK_NPCSOLID, qfalse, 0, 0);
  ...
  if (VectorCompare(ent->s.pos.trDelta, vec3_origin)) { // not moving
  ...
  if (VectorCompare(ent->s.apos.trDelta, vec3_origin)) { // not moving
...
void misc_model_breakable_init(gentity_t *ent) {
  ...
  if (ent->spawnflags & 1) { // Blocks movement
    ent->r.contents = CONTENTS_SOLID | CONTENTS_OPAQUE | CONTENTS_BODY |
                      CONTENTS_MONSTERCLIP |
                      CONTENTS_BOTCLIP; // Was CONTENTS_SOLID, but only
                                        // architecture should be this
  } else if (ent->health) { // Can only be shot
...
void SP_misc_bsp(gentity_t *ent) {
  ...
  trap->SetBrushModel((sharedEntity_t *)ent,
                      temp); // SV_SetBrushModel -- sets mins and maxs
  G_BSPIndex(temp);
  ...
  */
  ...
  {
          G_SpawnDebugCylinder ( ent->s.origin, ent->s.time2, &g_entities[0],
  2000, COLOR_WHITE );

          if ( ent->s.time )
          {
                  G_SpawnDebugCylinder ( ent->s.origin, ent->s.time,
  &g_entities[0], 2000, COLOR_RED );
          }
  }
  */
}
...
image to use, located in heightmaps/*.png. (must be PNG format) terrainDef -
defines how the game textures the terrain (file is base/ext_data/rmg/*.terrain -
...
base/ext_data/rmg/*.miscents) densityMap - how dense the client models are
packed
...
void G_PortalifyEntities(gentity_t *ent) {
  ...
  while (i < MAX_GENTITIES) {
    ...
    if (scan && scan->inuse && scan->s.number != ent->s.number &&
        trap->InPVS(ent->s.origin, scan->r.currentOrigin)) {
      trace_t tr;

      trap->Trace(&tr, ent->s.origin, vec3_origin, vec3_origin,
                  scan->r.currentOrigin, ent->s.number, CONTENTS_SOLID, qfalse,
                  0, 0);
      ...
      if (tr.fraction == 1.0 ||
        ...
        if (!scan->client ||
            scan->s.eType ==
                ET_NPC) { // making a client a portal entity would be bad.
  ...
  ent->think = G_FreeEntity; // the portal entity is no longer needed because
                             // its information is stored in a config string.
...
"onlyfoghere" if non-0 allows you to set a global fog, but will only use that
fog within this sky portal.
...
void HolocronTouch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (hasall) { // once we pick up this holocron we'll have all of them, so give
                // us super special best prize! trap->Print("You deserve a pat on
                // the back.\n");
  }

  if (!(other->client->ps.fd.forcePowersActive &
        (1 << other->client->ps.fd
                  .forcePowerSelected))) { // If the player isn't using his
                                           // currently selected force power,
                                           // select this one
    if (self->count != FP_SABER_OFFENSE && self->count != FP_SABER_DEFENSE &&
        self->count != FP_SABERTHROW && self->count != FP_LEVITATION) {
  ...
  if (g_maxHolocronCarry.integer &&
      othercarrying >=
          g_maxHolocronCarry
              .integer) { // make the oldest holocron carried by the player pop
    ...
    */
    // NOTE: No longer valid as we are now always giving a force level 1 saber
    // attack level in holocron
  }
...
void HolocronThink(gentity_t *ent) {
  ...
  if (ent->enemy && ent->enemy->client) {
    ...
    if (!ent->enemy->inuse ||
        (ent->enemy->client && ent->enemy->client->ps.fallingToDeath)) {
  ...
  if (ent->pos2[0] &&
      ent->pos2[1] < level.time) { // isn't in original place and has been there
                                   // for (HOLOCRON_RESPAWN_TIME) seconds without
                                   // being picked up, so respawn
    VectorCopy(ent->s.origin2, ent->s.pos.trBase);
    VectorCopy(ent->s.origin2, ent->s.origin);
    VectorCopy(ent->s.origin2, ent->r.currentOrigin);
...
void SP_misc_holocron(gentity_t *ent) {
  ...
  if (HasSetSaberOnly()) {
    if (ent->count == FP_SABER_OFFENSE || ent->count == FP_SABER_DEFENSE ||
        ent->count == FP_SABERTHROW) { // having saber holocrons in saber only
                                       // mode is pointless
      G_FreeEntity(ent);
      return;
    }
  ...
  trap->Trace(&tr, ent->s.origin, ent->r.mins, ent->r.maxs, dest, ent->s.number,
  ...
  */
  // No longer doing this, causing too many complaints about accidentally
  // setting no force powers at all and starting a holocron game (making it
  // basically just FFA)
  ...
  ent->s.modelindex =
      (ent->count - 128); // G_ModelIndex(holocronTypeModels[ent->count]);
...
void check_recharge(gentity_t *ent) {
  if (ent->fly_sound_debounce_time < level.time || !ent->activator ||
...
void shield_power_converter_use(gentity_t *self, gentity_t *other,
  ...
  if (level.gametype == GT_SIEGE && other && other->client &&
...
void ammo_generic_power_converter_use(gentity_t *self, gentity_t *other,
                                      gentity_t *activator) {
  int /*dif,*/ add;
  // int ammoType;
  int stop = 1;
  ...
  if (self->setTime < level.time) {
    ...
    while (i < 3)
    {
            ...
            {
                    self->s.loopSound =
    G_SoundIndex("sound/interface/ammocon_run"); self->s.loopIsSoundset =
    qfalse;
            }
            self->setTime = level.time + 100;

            //dif = activator->client->ps.stats[STAT_MAX_HEALTH] -
    activator->client->ps.stats[STAT_ARMOR]; switch (i) { //don't give rockets I
    guess case 0: ammoType = AMMO_BLASTER; break; case 1: ammoType =
    AMMO_POWERCELL; break; case 2: ammoType = AMMO_METAL_BOLTS; break; default:
                    ammoType = -1;
                    break;
            }
            ...
            if (ammoType != -1)
            {
                    dif = ammoData[ammoType].max -
    activator->client->ps.ammo[ammoType];
            }
            else
            {
                    dif = 0;
    ...
    */
    ...
    while (i < AMMO_MAX) {
      ...
      if (((activator->client->ps.eFlags & EF_DOUBLE_AMMO) &&
        ...
        gaveSome = qtrue;
        if (level.gametype == GT_SIEGE && i == AMMO_ROCKETS &&
            activator->client->ps.ammo[i] >=
                10) { // this stuff is already a freaking mess, so..
          gaveSome = qfalse;
        }
        activator->client->ps.ammo[i] += add;
        if (level.gametype == GT_SIEGE && i == AMMO_ROCKETS &&
            activator->client->ps.ammo[i] >=
                10) { // fixme - this should SERIOUSLY be externed.
...
void SP_misc_ammo_floor_unit(gentity_t *ent) {
  ...
  if (tr.startsolid) {
    trap->Print(
        "SP_misc_ammo_floor_unit: misc_ammo_floor_unit startsolid at %s\n",
        vtos(ent->s.origin));
    G_FreeEntity(ent);
    return;
  }
  ...
  if (level.gametype == GT_SIEGE) { // show on radar from everywhere
    ...
    ent->s.genericenemyindex =
        G_IconIndex("gfx/mp/siegeicons/desert/weapon_recharge");
  }
}
...
void SP_misc_shield_floor_unit(gentity_t *ent) {
  ...
  if (tr.startsolid) {
    trap->Print(
        "SP_misc_shield_floor_unit: misc_shield_floor_unit startsolid at %s\n",
        vtos(ent->s.origin));
    G_FreeEntity(ent);
    return;
  }
  ...
  if (level.gametype == GT_SIEGE) { // show on radar from everywhere
    ...
    ent->s.eFlags |= EF_RADAROBJECT;
    ent->s.genericenemyindex =
        G_IconIndex("gfx/mp/siegeicons/desert/shield_recharge");
  }
}
...
void ammo_power_converter_use(gentity_t *self, gentity_t *other,
  ...
  if (self->setTime < level.time) {
    ...
    if (self->count) // Has it got any power left?
    {
      ...
      difBlaster = activator->client->ps.ammo[AMMO_BLASTER] -
      ammoData[AMMO_BLASTER].max; difPowerCell =
      activator->client->ps.ammo[AMMO_POWERCELL] - ammoData[AMMO_POWERCELL].max;
      difMetalBolts = activator->client->ps.ammo[AMMO_METAL_BOLTS] -
      ammoData[AMMO_METAL_BOLTS].max;
...
================
*/
void health_power_converter_use(gentity_t *self, gentity_t *other,
                                gentity_t *activator) {
  int dif, add;
  int stop = 1;
...
effect
        ...
        "delay"  - how often to call the effect, don't over-do this ( default
200 ) "random" - random amount of time to add to delay, ( default 0, 200 = 0ms
to 200ms ) "splashRadius" - only works when damage is checked ( default 16 )
        "splashDamage" - only works when damage is checked ( default 5 )
        "soundset"	- bmodel set to use, plays start sound when toggled on,
loop sound while on ( doesn't play on a oneshot), and a stop sound when turned
off
*/
...
void fx_runner_think(gentity_t *ent) {
  ...
  {
    G_RadiusDamage(ent->r.currentOrigin, ent, ent->splashDamage,
                   ent->splashRadius, ent, ent, MOD_UNKNOWN);
  }
  ...
  if (!(ent->spawnflags & 2) &&
      !ent->s.loopSound) // NOT ONESHOT...this is an assy thing to do
  {
...
void fx_runner_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  if (self->s.isPortalEnt) { // rww - mark it as broadcast upon first use if
                             // it's within the area of a skyportal
    self->r.svFlags |= SVF_BROADCAST;
  }

  if (self->spawnflags & 2) // ONESHOT
  {
    // call the effect with the desired position and orientation, as a safety
    // thing,
    ...
    // toggle our state
    if (self->nextthink == -1) {
      // NOTE: we fire the effect immediately on use, the fx_runner_think func
      // will set
...
void fx_runner_link(gentity_t *ent) {
  ...
  // don't really do anything with this right now other than do a check to warn
  // the designers if the target2 is bogus
  if (ent->target2 && ent->target2[0]) {
    gentity_t *target = NULL;
...
void SP_fx_runner(gentity_t *ent) {
  ...
  if (!fxFile || !fxFile[0]) {
    Com_Printf(S_COLOR_RED
               "ERROR: fx_runner %s at %s has no fxFile specified\n",
               ent->targetname, vtos(ent->s.origin));
    G_FreeEntity(ent);
    return;
  }

  // Try and associate an effect file, unfortunately we won't know if this
  // worked or not
  ...
  // Give us a bit of time to spawn in the other entities, since we may have to
  // target one of 'em
...
/*QUAKED fx_wind (0 .5 .8) (-16 -16 -16) (16 16 16) NORMAL CONSTANT GUSTING
SWIRLING x  FOG LIGHT_FOG Generates global wind forces
...
void SP_CreateWind(gentity_t *ent) {
  ...
  if (ent->spawnflags & 2) {
    ...
    Com_sprintf(temp, sizeof(temp), "*constantwind ( %f %f %f )", windDir[0],
                windDir[1], windDir[2]);
    G_EffectIndex(temp);
  }
...
/*QUAKED fx_rain (1 0 0) (-16 -16 -16) (16 16 16) LIGHT MEDIUM HEAVY ACID x
MISTY_FOG This world effect will spawn rain globally into the level.
...
void Use_Target_Screenshake(gentity_t *ent, gentity_t *other,
                            gentity_t *activator) {
...
void Use_Target_Escapetrig(gentity_t *ent, gentity_t *other,
                           gentity_t *activator) {
  if (!ent->genericValue6) {
    gEscaping = qtrue;
    ...
    if (activator && activator->inuse &&
        activator->client) { // the one who escaped gets 500
...
void maglock_link(gentity_t *self) {
  ...
  trap->Trace(&trace, start, vec3_origin, vec3_origin, end, self->s.number,
...
void misc_faller_create(gentity_t *ent, gentity_t *other,
                        gentity_t *activator) {
...
void misc_faller_think(gentity_t *ent) {
  ...
  ent->nextthink =
      level.time + ent->genericValue1 + Q_irand(0, ent->genericValue2);
}
...
fudgefactor	- milliseconds between 0 and this number randomly added to
interval
*/
...
// As is this means 16*256 sizeof(reference_tag_t)'s in addition to
// name+inuse*16.
#define MAX_TAGS 256
#define MAX_TAG_OWNERS 16

// Maybe I should use my trap->TrueMalloc/trap->TrueFree stuff with this.
// But I am not yet confident that it can be used without exploding at some
// point.
...
reference_tag_t *TAG_Add(const char *name, const char *owner, vec3_t origin,
  ...
  // This is actually reverse order of how SP does it because of the way we're
  // storing/allocating. Now that we have the owner, we want to get the first
  // free reftag on the owner itself.
  ...
  Q_strlwr((char *)tagOwner
               ->name); // NOTENOTE: For case insensitive searches on a map

  // Copy the name
  Q_strncpyz((char *)tag->name, name, MAX_REFNAME);
  Q_strlwr(
      (char *)tag->name); // NOTENOTE: For case insensitive searches on a map
...
void ref_link(gentity_t *ent) {
  ...
  // NOTE: this means if you wanted to link them in a chain for, say, a path,
  // you can't
  G_FreeEntity(ent);
}
...
gclient_t *G_ClientForShooter(void) {
  ...
  if (!g_shooterClientInit) { // in theory it should be initialized to 0 on the
                              // stack, but just in case.
...
void misc_weapon_shooter_use(gentity_t *self, gentity_t *other,
                             gentity_t *activator) {
...
void SP_misc_weapon_shooter(gentity_t *self) {
  ...
  self->client =
      G_ClientForShooter(); //(gclient_s *)trap->Malloc(sizeof(gclient_s),
                            //TAG_G_ALLOC, qtrue);
...
Determines a region to check for weather contents - will significantly reduce
load time
*/
