...
typedef struct {
  ...
  void(QDECL *Error)(int errorLevel, const char *fmt, ...) NORETURN_PTR
      __attribute__((format(printf, 2, 3)));

  // milliseconds should only be used for profiling, never for anything game
  // related. Get time from the refdef
  ...
  void (*WIN_SetGamma)(glconfig_t *glConfig, byte red[256], byte green[256],
                       byte blue[256]);
...
typedef struct {
  ...
  int (*GetAnimationCFG)(const char *psCFGFilename, char *psDest,
                         int iDestSize);
  ...
  //
  void (*RegisterMedia_LevelLoadBegin)(const char *psMapName,
  ...
  qboolean (*RegisterModels_LevelLoadEnd)(
      qboolean bDeleteEverythingNotUsedThisLevel);
  ...
  qboolean (*GetLighting)(const vec3_t org, vec3_t ambientLight,
  ...
  void (*UploadCinematic)(int cols, int rows, const byte *data, int client,
                          qboolean dirty);
  ...
  size_t (*SaveJPGToBuffer)(byte *buffer, size_t bufSize, int quality,
                            ...
                            bool flip_vertical);
  void (*LoadJPGFromBuffer)(byte *inputBuffer, size_t len, byte **pic,
                            int *width, int *height);
#endif

  // this is so you can get access to raw pixels from a graphics format
  // (TGA/JPG/BMP etc),
  //	currently only the save game uses it (to make raw shots for the
  //autosaves)
  //
  byte *(*TempRawImage_ReadFromFile)(const char *psLocalFilename, int *piWidth,
  ...
  // model stuff
  void (*LerpTag)(orientation_t *tag, qhandle_t model, int startFrame,
  ...
  void (*Font_DrawString)(int x, int y, const char *s, const float *rgba,
  ...
  unsigned int (*AnyLanguage_ReadCharFromString)(
      char *psText, int *piAdvanceCount,
      qboolean *pbIsTrailingPunctuation /* = NULL */);
  unsigned int (*AnyLanguage_ReadCharFromString2)(
      char **psText, qboolean *pbIsTrailingPunctuation /* = NULL */);
  ...
  void (*G2API_SetGhoul2ModelIndexes)(CGhoul2Info_v &ghoul2,
                                      qhandle_t *modelList,
                                      qhandle_t *skinList);
