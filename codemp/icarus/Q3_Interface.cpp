...
static void Q3_CenterPrint(const char *format, ...) {
  ...
  // FIXME: added '!' so you can print something that's hasn't been precached,
  // '@' searches only for precache text this is just a TEMPORARY placeholder
  // until objectives are in!!!  -- dmv 11/26/01
...
void Q3_TaskIDComplete(sharedEntity_t *ent, taskID_t taskType) {
  ...
  if (gTaskManagers[ent->s.number] &&
      Q3_TaskIDPending(ent, taskType)) { // Complete it
    gTaskManagers[ent->s.number]->Completed(ent->taskID[taskType]);

    // See if any other tasks have the name number and clear them so we don't
...
void Q3_TaskIDSet(sharedEntity_t *ent, taskID_t taskType, int taskID) {
  ...
  // Might be stomping an old task, so complete and clear previous task if there
  // was one
...
=============
*/
static unsigned int Q3_GetTime(void) { return svs.time; }
...
//get the last slash before the wav if (start != NULL) { if (!Q_strncmp( start,
"/mr_", 4) ) { if (qDoBoth) {	//we want to change mr to ms start[2] = 's';
//change mr to ms return qtrue; } else {	//IF qDoBoth return qfalse;
//don't want this one
                                                }
                                        }
                                }	//IF found slash
                        ...
                        start = strrchr( string, '/' );		//get the last
slash before the wav if (start != NULL) { if (!Q_strncmp( start, "/ms_", 4) ) {
...
=============
*/
static int Q3_PlaySound(int taskID, int entID, const char *name,
                        const char *channel) {
...
============
*/
static void Q3_Set(int taskID, int entID, const char *type_name,
                   const char *data) {
...
============
*/
static int Q3_Evaluate(int p1Type, const char *p1, int p2Type, const char *p2,
                       int operatorType) {
  float f1 = 0, f2 = 0;
  vec3_t v1, v2;
  char *c1 = 0, *c2 = 0;
  ...
  case TK_EQUALS:
    ...
    case TK_STRING:
    case TK_IDENTIFIER:
      return (int)!Q_stricmp(c1,
                             c2); // NOTENOTE: The script uses proper string
                                  // comparison logic (ex. ( a == a ) == true )
      break;
  ...
  case TK_GREATER_THAN:
    ...
    case TK_VECTOR:
      Q3_DebugPrint(WL_ERROR, "Q3_Evaluate vector comparisons of type GREATER "
                              "THAN cannot be performed!");
      return false;
      break;

    case TK_STRING:
    case TK_IDENTIFIER:
      Q3_DebugPrint(WL_ERROR, "Q3_Evaluate string comparisons of type GREATER "
                              "THAN cannot be performed!");
      return false;
      break;
  ...
  case TK_LESS_THAN:
    ...
    case TK_VECTOR:
      Q3_DebugPrint(WL_ERROR, "Q3_Evaluate vector comparisons of type LESS "
                              "THAN cannot be performed!");
      return false;
      break;
    ...
    case TK_STRING:
    case TK_IDENTIFIER:
      Q3_DebugPrint(WL_ERROR, "Q3_Evaluate string comparisons of type LESS "
                              "THAN cannot be performed!");
      return false;
      break;
...
-------------------------
*/
static void Q3_CameraFade(float sr, float sg, float sb, float sa, float dr,
                          float dg, float db, float da, float duration) {
...
// These are useless for MP. Just taking it for now since I don't want to remove
// all calls to this in ICARUS.
int AppendToSaveGame(unsigned long chid, const void *data, int length) {
  return 1;
}

// Changed by BTO (VV) - Visual C++ 7.1 doesn't allow default args on funcion
// pointers
int ReadFromSaveGame(unsigned long chid, void *pvAddress,
                     int iLength /* , void **ppvAddressPtr = NULL */) {
  return 1;
}

void CGCam_Enable(void) { CGCam_Anything(); }

void CGCam_Disable(void) { CGCam_Anything(); }

void CGCam_Zoom(float FOV, float duration) { CGCam_Anything(); }

void CGCam_Pan(vec3_t dest, vec3_t panDirection, float duration) {
  CGCam_Anything();
}

void CGCam_Move(vec3_t dest, float duration) { CGCam_Anything(); }

void CGCam_Shake(float intensity, int duration) { CGCam_Anything(); }
...
void CGCam_Distance(float distance, float initLerp) { CGCam_Anything(); }

void CGCam_Roll(float dest, float duration) { CGCam_Anything(); }

int ICARUS_LinkEntity(int entID, CSequencer *sequencer,
                      CTaskManager *taskManager);

static unsigned int Q3_GetTimeScale(void) { return com_timescale->value; }

static void Q3_Lerp2Pos(int taskID, int entID, vec3_t origin, vec3_t angles,
                        float duration) {
  ...
  GVM_ICARUS_Lerp2Pos();
  // We do this in case the values are modified in the game. It would be
  // expected by icarus that the values passed in here are modified equally.
...
static void Q3_Lerp2Origin(int taskID, int entID, vec3_t origin,
                           float duration) {
  T_G_ICARUS_LERP2ORIGIN *sharedMem =
      (T_G_ICARUS_LERP2ORIGIN *)sv.mSharedMemory;
...
static void Q3_Lerp2Angles(int taskID, int entID, vec3_t angles,
                           float duration) {
  T_G_ICARUS_LERP2ANGLES *sharedMem =
      (T_G_ICARUS_LERP2ANGLES *)sv.mSharedMemory;
...
static int Q3_GetString(int entID, int type, const char *name, char **value) {
  ...
  // rww - careful with this, next time shared memory is altered this will get
  // stomped
