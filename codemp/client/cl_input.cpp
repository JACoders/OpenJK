...
void CL_MouseMove(usercmd_t *cmd) {
	...
	if (cl_mouseAccel->value != 0.0f) {
		if (cl_mouseAccelStyle->integer == 0) {
			...
			if (cl_mYawOverride || cl_mPitchOverride) { // FIXME: different people have different speed mouses,
				if (cl_mSensitivityOverride) {
...
void CL_CmdButtons(usercmd_t *cmd) {
	...
	if (cmd->buttons & BUTTON_FORCEPOWER) {						  // check for transferring a use force to a use inventory...
		if ((cmd->buttons & BUTTON_USE) || CL_NoUseableForce()) { // it's pushed, remap it!
...
void CL_WritePacket(void) {
	...
	if (count >= 1) {
		...
		if (cl.gcmdSentValue) { // hmm, just clear here, I guess.. hoping it will resolve issues with gencmd values sometimes not going through.
			cl.gcmdSendValue = qfalse;
...
static const cmdList_t inputCmds[] = {{"centerview", "Centers view on screen", IN_CenterView, NULL},
