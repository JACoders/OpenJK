...
void ResampleSfx(sfx_t *sfx, int iInRate, int iInWidth, byte *pData) {
  ...
  int i;
  int iSample;
  unsigned int uiSampleFrac,
      uiFracStep; // uiSampleFrac MUST be unsigned, or large samples (eg music
                  // tracks) crash

  fStepScale = (float)iInRate / dma.speed; // this is usually 0.5, 1, or 2

  // When stepscale is > 1 (we're downsampling), we really ought to run a low
  // pass filter on the samples
...
// maybe I'm re-inventing the wheel, here, but I can't see any functions that
// already do this, so...
//
...
void R_CheckMP3s(const char *psDir) {
  ...
  for (i = 0; i < numSysFiles; i++) {
    ...
    if (pbData) {
      ...
      //
      qboolean qbTagNeedsUpdating =
          (/* qbForceRescan || */ !MP3_ReadSpecialTagInfo(pbData, iSize, &pTAG))
              ? qtrue
              : qfalse;
      ...
      if (pTAG == NULL || qbTagNeedsUpdating || qbForceRescan) {
        ...
        //
        // For EF1 I used a temp sfx_t struct, but I can't do that now with this
        // new alloc scheme,
        //	I have to ask for it legally, so I'll keep re-using one, and
        //restoring it's name after use. 	(slightly dodgy, but works ok if no-one
        //else changes stuff)
        //
        // sfx_t SFX = {0};
        ...
        const char sReservedSFXEntrynameForMP3[] =
            "reserved_for_mp3"; // ( strlen() < MAX_QPATH )

        if (pSFX == NULL) // once only
        {
          pSFX = S_FindName(
              sReservedSFXEntrynameForMP3); // always returns, else ERR_FATAL
        }

        if (MP3_IsValid(sFilename, pbData, iSize, qbForceStereo)) {
          wavinfo_t info;

          int iRawPCMDataSize = MP3_GetUnpackedSize(sFilename, pbData, iSize,
                                                    qtrue, qbForceStereo);

          if (iRawPCMDataSize) // should always be true, unless file is fucked,
                               // in which case, stop this conversion process
          {
            float fMaxVol = 128; // any old default
            int iActualUnpackedSize =
                iRawPCMDataSize; // default, override later if not doing music

            if (!qbForceStereo) // no point for stereo files, which are for
                                // music and therefore no lip-sync
            {
              byte *pbUnpackBuffer =
                  (byte *)Z_Malloc(iRawPCMDataSize + 10, TAG_TEMP_WORKSPACE,
                                   qfalse); // won't return if fails

              iActualUnpackedSize =
                  MP3_UnpackRawPCM(sFilename, pbData, iSize, pbUnpackBuffer);
              if (iActualUnpackedSize != iRawPCMDataSize) {
                Com_Error(ERR_DROP,
                          "******* Whoah! MP3 %s unpacked to %d bytes, but "
                          "size calc said %d!\n",
                          sFilename, iActualUnpackedSize, iRawPCMDataSize);
              }

              // fake up a WAV structure so I can use the other post-load sound
              // code such as volume calc for lip-synching
              //
              MP3_FakeUpWAVInfo(sFilename, pbData, iSize, iActualUnpackedSize,
                                // these params are all references...
                                info.format, info.rate, info.width,
                                info.channels, info.samples, info.dataofs);

              S_LoadSound_Finalize(
                  &info, pSFX,
                  pbUnpackBuffer); // all this just for lipsynch. Oh well.
...
#define INT_MIN (-2147483647 - 1) /* minimum (signed) int value */
#endif
                //
                pSFX->iLastTimeUsed = INT_MIN; // force this to be oldest sound
                                               // file, therefore disposable...
                pSFX->bInMemory = true;
                SND_FreeOldestSound(); // ... and do the disposal

                // now set our temp SFX struct back to default name so nothing
                // else accidentally uses it...
                //
                strcpy(pSFX->sSoundName, sReservedSFXEntrynameForMP3);
                pSFX->bDefaultSound = false;
              }

              //							OutputDebugString(va("File:
            ...
            if (f) {
              // write the file back out, but omitting the tag if there was
              // one...
              //
              int iWritten =
              ...
              if (iWritten) {
                // make up a new tag if we didn't find one in the original
                // file...
                //
                id3v1_1 TAG;
                if (!pTAG) {
                ...
                strncpy(pTAG->comment, va("%s %g", sKEY_MAXVOL, fMaxVol),
                        sizeof(pTAG->comment));
                strncpy(pTAG->album,
              ...
              iErrors++;
              strErrors +=
                  va("Failed to re-open for write: \"%s\"\n", sFilename);
            }
          } else {
...
static qboolean S_LoadSound_FileLoadAndNameAdjuster(char *psFilename,
                                                    byte **pData, int *piSize,
                                                    int iNameStrlen) {
  ...
  if (psVoice) {
    ...
    if (s_language && Q_stricmp("DEUTSCH", s_language->string) == 0) {
      ...
      psVoice = NULL; // use this ptr as a flag as to whether or not we
                      // substituted with a foreign version
    }
  }
  ...
  *piSize = FS_ReadFile(psFilename, (void **)pData); // try WAV
  if (!*pData) {
    ...
    *piSize = FS_ReadFile(psFilename, (void **)pData); // try MP3
    ...
    if (!*pData) {
      // hmmm, not found, ok, maybe we were trying a foreign noise
      // ("arghhhhh.mp3" that doesn't matter?) but it
      // was missing?   Can't tell really, since both types are now in
      // sound/chars. Oh well, fall back to English for now...
      ...
      if (psVoice) // were we trying to load foreign?
      {
        ...
        Com_Printf(S_COLOR_YELLOW
                   "Foreign file missing: \"%s\"! (using English...)\n",
                   psFilename);
#endif
...
// returns qtrue if this dir is allowed to keep loaded MP3s, else qfalse if they
// should be WAV'd instead...
//
// note that this is passed the original, un-language'd name
//
// (I was going to remove this, but on kejim_post I hit an assert because
// someone had got an ambient sound when the
//	perimter fence goes online that was an MP3, then it tried to get added
//as looping. Presumably it sounded ok or 	they'd have noticed, but we therefore
//need to stop other levels using those. "sound/ambience" I can check for, 	but
//doors etc could be anything. Sigh...)
//
...
of a forced fallback of a player specific sound	(or of a wav/mp3 substitution
now -Ste)
==============
*/
...
static qboolean S_LoadSound_Actual(sfx_t *sfx) {
  ...
  Q_strlwr(sLoadName);
  //
  // Ensure name has an extension (which it must have, but you never know), and
  // get ptr to it...
  //
  psExt = &sLoadName[strlen(sLoadName) - 4];
  if (*psExt != '.') {
    // Com_Printf( "WARNING: soundname '%s' does not have 3-letter
    // extension\n",sLoadName);
    COM_DefaultExtension(sLoadName, sizeof(sLoadName),
                         ".wav"); // so psExt below is always valid
    psExt = &sLoadName[strlen(sLoadName) - 4];
  ...
  if (Q_stricmpn(psExt, ".mp3", 4) == 0) {
    ...
    if (MP3_IsValid(sLoadName, data, size, qfalse)) {
      int iRawPCMDataSize =
          MP3_GetUnpackedSize(sLoadName, data, size, qfalse, qfalse);

      if (S_LoadSound_DirIsAllowedToKeepMP3s(
              sfx->sSoundName) // NOT sLoadName, this uses original un-languaged
                               // name
          &&
          MP3Stream_InitFromFile(
              sfx, data, size, sLoadName,
              iRawPCMDataSize + 2304 /* + 1 MP3 frame size, jic */, qfalse)) {
        //				Com_DPrintf("(Keeping file \"%s\" as
        //MP3)\n",sLoadName);

#ifdef USE_OPENAL
        if (s_UseOpenAL) {
          // Create space for lipsync data (4 lip sync values per streaming AL
          // buffer)
        ...
        // small file, not worth keeping as MP3 since it would increase in size
        // (with MP3 header etc)...
        //
        ...
        //
        {
          byte *pbUnpackBuffer = (byte *)Z_Malloc(
              iRawPCMDataSize + 10 + 2304 /* <g> */, TAG_TEMP_WORKSPACE,
              qfalse); // won't return if fails

          {
            int iResultBytes =
                MP3_UnpackRawPCM(sLoadName, data, size, pbUnpackBuffer, qfalse);

            if (iResultBytes != iRawPCMDataSize) {
              Com_Printf(S_COLOR_YELLOW "**** MP3 %s final unpack size %d "
                                        "different to previous value %d\n",
            ...
            // code such as volume calc for lip-synching
            //
            // (this is a bit crap really, but it lets me drop through into
            // existing code)...
            //
            MP3_FakeUpWAVInfo(sLoadName, data, size, iResultBytes,
                              // these params are all references...
                              info.format, info.rate, info.width, info.channels,
                              info.samples, info.dataofs, qfalse);

            S_LoadSound_Finalize(&info, sfx, pbUnpackBuffer);

#ifdef Q3_BIG_ENDIAN
            // the MP3 decoder returns the samples in the correct endianness,
            // but ResampleSfx byteswaps them, so we have to swap them again...
            sfx->fVolRange = 0;
            ...
            for (int i = 0; i < sfx->iSoundLengthInSamples; i++) {
              ...
              // because double >> int is not defined. Force interpretation as
              // int
              if (sfx->fVolRange <
                  (abs(static_cast<int>(sfx->pSoundData[i])) >> 8)) {
            ...
            if (s_UseOpenAL) {
              if (strstr(sfx->sSoundName, "chars")) {
                sfx->lipSyncData =
                    (char *)Z_Malloc((sfx->iSoundLengthInSamples / 1000) + 1,
                                     TAG_SND_RAWDATA, qfalse);
              ...
              if (alGetError() == AL_NO_ERROR) {
                ...
                alBufferData(Buffer, AL_FORMAT_MONO16, sfx->pSoundData,
                             sfx->iSoundLengthInSamples * 2, 22050);
                if (alGetError() == AL_NO_ERROR) {
                  sfx->Buffer = Buffer;
    ...
    /*		if ( info.width == 1 ) {
                            Com_Printf(S_COLOR_YELLOW "WARNING: %s is a 8 bit
       wav file\n", sLoadName);
                    }

                    if ( info.rate != 22050 ) {
                            Com_Printf(S_COLOR_YELLOW "WARNING: %s is not a
       22kHz wav file\n", sLoadName);
                    }
    */
    samples = (short *)Z_Malloc(info.samples * sizeof(short) * 2,
...
// wrapper function for above so I can guarantee that we don't attempt any
// audio-dumping during this call because
  ...
  for (i = 0; i < sfx->iSoundLengthInSamples; i += 100) {
    ...
    if (((i + 100) % 1000) == 0) {
      ...
      if (sampleTotal < sfx->fVolRange * s_lip_threshold_1->value) {
        // tell the scripts that are relying on this that we are still going,
        // but actually silent right now.
        sample = -1;
