...
void Mark2_FireBlaster(qboolean advance) {
	...
	gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->playerModel, NPC->genericBolt1, &boltMatrix, NPC->currentAngles, NPC->currentOrigin,
...
void Mark2_AttackDecision(void) {
	...
	if (NPCInfo->localState == LSTATE_RISINGUP) {
		...
		if ((NPC->client->ps.legsAnimTimer == 0) && NPC->client->ps.torsoAnim == BOTH_RUN1START) {
