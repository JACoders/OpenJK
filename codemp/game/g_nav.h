...
// If you modify it without recompiling the exe with new code, there could be
// issues.
...
#define NAVGOAL_USE_RADIUS                                                     \
  16384 // Used to force the waypoint_navgoals with a manually set radius to
        // actually do a DistanceSquared check, not just bounds overlap
...
extern int NAV_TestBestNode(gentity_t *self, int startID, int endID,
                            qboolean failEdge);
