...
=====================
*/
static inline qboolean CM_PlaneFromPoints(vec4_t plane, vec3_t a, vec3_t b,
                                          vec3_t c) {
  vec3_t d1, d2;
...
static inline int CM_FindPlane2(float plane[4], int *flipped) {
  ...
  if (numPlanes == MAX_PATCH_PLANES) {
    Com_Error(ERR_DROP, "CM_FindPlane2: MAX_PATCH_PLANES (%d)",
              MAX_PATCH_PLANES);
  }
...
static inline int CM_FindPlane(float *p1, float *p2, float *p3) {
  ...
  // add a new plane
  if (numPlanes == MAX_PATCH_PLANES) {
    Com_Error(ERR_DROP, "CM_FindPlane: MAX_PATCH_PLANES (%d)",
              MAX_PATCH_PLANES);
  }
...
static inline int CM_GridPlane(int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2],
                               int i, int j, int tri) {
  int p;
    ...
    for (axis = 0; axis < 3; axis++) {
      for (dir = -1; dir <= 1; dir += 2) {
        ...
        if (i == facet->numBorders) {
          ...
          //
          w2 = CopyWinding(w);
          VectorCopy4(planes[facet->borderPlanes[facet->numBorders]].plane,
                      newplane);
...
static inline void CM_PatchCollideFromGrid(cGrid_t *grid, patchCollide_t *pf) {
  ...
  int numFacets;
  facets = (facet_t *)Z_Malloc(MAX_FACETS * sizeof(facet_t), TAG_TEMP_WORKSPACE,
                               qfalse, 4);
  ...
  pf->planes =
      (patchPlane_t *)Hunk_Alloc(numPlanes * sizeof(*pf->planes), h_high);
...
*/
static inline void
CM_TracePointThroughPatchCollide(traceWork_t *tw, trace_t &trace,
...
====================
*/
static inline int CM_CheckFacetPlane(float *plane, vec3_t start, vec3_t end,
                                     float *enterFrac, float *leaveFrac,
                                     int *hit) {
  float d1, d2, f;
...
====================
*/
void CM_TraceThroughPatchCollide(traceWork_t *tw, trace_t &trace,
                                 const struct patchCollide_s *pc) {
  int i, j, hit, hitnum;
