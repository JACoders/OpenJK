...
qboolean NPC_CheckEnemyStealth(gentity_t *target) {
	...
	if (clearLOS) {
		...
		float turning_rating = AngleDelta(target->client->ps.viewangles[PITCH], target->lastAngles[PITCH]) / 180.0f +
...
static void ST_CheckMoveState(void) {
	...
	if ((NPCInfo->goalEntity != NPC->enemy) && (NPCInfo->goalEntity != NULL)) {
		...
		if (NAV_HitNavGoal(NPC->currentOrigin, NPC->mins, NPC->maxs, NPCInfo->goalEntity->currentOrigin, 16, FlyingCreature(NPC)) ||
			(!Q3_TaskIDPending(NPC, TID_MOVE_NAV) && NPCInfo->squadState == SQUAD_SCOUT && enemyLOS &&
...
void ST_TransferMoveGoal(gentity_t *self, gentity_t *other) {
	if (Q3_TaskIDPending(self, TID_MOVE_NAV)) { // can't transfer movegoal when a script we're running is waiting to complete
		return;
	}
	if (self->NPC->combatPoint != -1) { // I've got a combatPoint I'm going to, give it to him
		...
		if (self->NPC->goalEntity == self->NPC->tempGoal) {
			NPC_SetMoveGoal(other, self->NPC->tempGoal->currentOrigin, self->NPC->goalRadius, ((self->NPC->tempGoal->svFlags & SVF_NAVGOAL) ? qtrue : qfalse));
		} else {
...
void ST_Commander(void) {
	...
	if (group->lastSeenEnemyTime < level.time - 180000) { // dissolve the group
		...
		for (i = 0; i < group->numGroup; i++) {
			...
			if (Q3_TaskIDPending(NPC, TID_MOVE_NAV)) { // running somewhere that a script requires us to go, don't break from that
				continue;
			}
	...
	*/
	...
	for (i = curMemberNum; i < lastMemberNum; i++) {
		...
		if (Q3_TaskIDPending(NPC, TID_MOVE_NAV)) { // running somewhere that a script requires us to go
			continue;
		}
		...
		if (NPCInfo->squadState != SQUAD_RETREAT) {	 // not already retreating
			if (NPC->client->ps.weapon == WP_NONE) { // weaponless, should be hiding
				if (NPCInfo->goalEntity == NULL || NPCInfo->goalEntity->enemy == NULL ||
					...
					if (TIMER_Done(NPC, "hideTime") || (DistanceSquared(group->enemy->currentOrigin, NPC->currentOrigin) < 65536 &&
														NPC_ClearLOS(NPC->enemy))) { // done hiding or enemy near and can see us
			...
			if (TIMER_Done(NPC, "roamTime") && TIMER_Done(NPC, "hideTime") && NPC->health > 10 &&
				!gi.inPVS(group->enemy->currentOrigin, NPC->currentOrigin)) { // cant even see enemy
				// better go after him
				...
				if (gi.inPVS(NPC->currentOrigin, group->enemy->currentOrigin)) { // in the same room as enemy
			...
			if (runner && NPCInfo->combatPoint != -1) { // someone is running and we have a combat point already
				if (NPCInfo->squadState != SQUAD_SCOUT && NPCInfo->squadState != SQUAD_TRANSITION && NPCInfo->squadState != SQUAD_RETREAT) { // it's not us
					if (TIMER_Done(NPC, "verifyCP") &&
						DistanceSquared(NPC->currentOrigin, level.combatPoints[NPCInfo->combatPoint].origin) >
				...
				if (NPCInfo->combatPoint != -1) { // we have a combat point we're supposed to be running to
					if (NPCInfo->squadState != SQUAD_SCOUT && NPCInfo->squadState != SQUAD_TRANSITION &&
						...
						if (TIMER_Done(NPC, "verifyCP")) { // 1 - 3 seconds have passed since you chose a CP, see if you're there since, for some reason, you've
														   // stopped running...
							if (DistanceSquared(NPC->currentOrigin, level.combatPoints[NPCInfo->combatPoint].origin) >
		...
		if (cpFlags) { // we want to run to a combat point
			...
			*/
			...
			if (group->enemy->client->ps.weapon == WP_SABER &&
				group->enemy->client->ps.saberLength > 0) { // we obviously want to avoid the enemy if he has a saber
...
void NPC_BSST_Attack(void) {
	...
	if (NPC->client->fireDelay) {
		...
		if (TIMER_Done(NPC, "attackDelay")) {
			...
			// NASTY
			if (NPC->s.weapon == WP_ROCKET_LAUNCHER && (ucmd.buttons & BUTTON_ATTACK) && !AImove && g_spskill->integer > 1 &&
