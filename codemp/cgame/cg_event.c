...
typedef enum { TAUNT_TAUNT = 0, TAUNT_BOW, TAUNT_MEDITATE, TAUNT_FLOURISH, TAUNT_GLOAT } tauntTypes_t;
/*
===================
CG_PlaceString
...
const char *CG_GetStringForVoiceSound(const char *s) {
	...
	while (i < MAX_CUSTOM_SIEGE_SOUNDS) {
		if (bg_customSiegeSoundNames[i] && !Q_stricmp(bg_customSiegeSoundNames[i], s)) { // get the matching reference name
...
void CG_EntityEvent(centity_t *cent, vec3_t position) {
	...
	case EV_FOOTSTEP_METAL:
		...
		if (cg_footsteps.integer) {
			trap->S_StartSound(NULL, es->number, CHAN_BODY, cgs.media.footsteps[FOOTSTEP_METALWALK][rand() & 3]);
		}
		break;
	case EV_FOOTSPLASH:
		DEBUGNAME("EV_FOOTSPLASH");
		if (cg_footsteps.integer) {
			trap->S_StartSound(NULL, es->number, CHAN_BODY, cgs.media.footsteps[FOOTSTEP_SPLASH][rand() & 3]);
		}
		break;
	case EV_FOOTWADE:
	...
	case EV_STEP_16: // smooth out step up transitions
		DEBUGNAME("EV_STEP");
		{
			...
			if (cg.demoPlayback || (cg.snap->ps.pm_flags & PMF_FOLLOW) || cg_noPredict.integer || g_synchronousClients.integer) {
				break;
			}
	...
	case EV_TAUNT:
		DEBUGNAME("EV_TAUNT");
		{
			...
			if (cgs.gametype != GT_DUEL && cgs.gametype != GT_POWERDUEL && es->eventParm == TAUNT_TAUNT) { // normal taunt
	...
	case EV_ITEM_PICKUP:
		DEBUGNAME("EV_ITEM_PICKUP");
		{
			...
			if (index < 1 && cg_entities[es->eventParm].currentState.isJediMaster) { // a holocron most likely
				...
				if (index != FP_SABER_OFFENSE && index != FP_SABER_DEFENSE && index != FP_SABERTHROW && index != FP_LEVITATION &&
	...
	case EV_FIRE_WEAPON:
		...
		if (cent->currentState.number >= MAX_CLIENTS && cent->currentState.eType != ET_NPC) { // special case for turret firing
			...
			if (cent->currentState.eventParm) {
				trap->G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt2, &matrix, cent->currentState.angles, cent->currentState.origin, cg.time, cgs.gameModels,
										  cent->modelScale);
			} else {
				trap->G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt1, &matrix, cent->currentState.angles, cent->currentState.origin, cg.time, cgs.gameModels,
										  cent->modelScale);
			}
	...
	case EV_DISRUPTOR_MAIN_SHOT:
		...
		if (cent->currentState.eventParm != cg.snap->ps.clientNum || cg.renderingThirdPerson) { // h4q3ry
			...
			if (cg.lastFPFlashPoint[0] || cg.lastFPFlashPoint[1] ||
				cg.lastFPFlashPoint[2]) { // get the position of the muzzle flash for the first person weapon model from the last frame
	...
	case EV_MISSILE_HIT:
		...
		if (cg_ghoul2Marks.integer && es->trickedentindex) { // flag to place a ghoul2 mark
			CG_G2MarkEvent(es);
		}
		break;
	...
	case EV_VOICECMD_SOUND:
		...
		if (es->groundEntityNum < MAX_CLIENTS && es->groundEntityNum >= 0) {
			...
			if (sfx) {
				if (clientNum != cg.predictedPlayerState.clientNum) {				 // play on the head as well to simulate hearing in radio and in world
					if (ci->team == cg.predictedPlayerState.persistant[PERS_TEAM]) { // don't hear it if this person is on the other team, but they can still
						// hear it in the world spot.
						trap->S_StartSound(NULL, cg.snap->ps.clientNum, CHAN_MENU1, sfx);
					}
				}
				if (ci->team == cg.predictedPlayerState.persistant[PERS_TEAM]) { // add to the chat box
					// hear it in the world spot.
	...
	case EV_DEBRIS:
		DEBUGNAME("EV_DEBRIS");
		CG_Chunks(es->owner, es->origin, es->angles, es->origin2, es->angles2, es->speed, es->eventParm, es->trickedentindex, es->modelindex,
				  es->apos.trBase[0]);
		break;
