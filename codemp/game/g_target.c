...
void Use_Target_Give( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	...
	while ( (t = G_Find (t, FOFS(targetname), ent->target)) != NULL ) {
		...
		t->nextthink = 0;
		gi.UnlinkEntity( (sharedEntity_t *)t );
	}
}
...
void Use_Target_Print (gentity_t *ent, gentity_t *other, gentity_t *activator)
{
	...
	{//private, to one client only
		...
		{//make sure there's a valid client ent to send it to
			...
			{
				gi.SendServerCommand( activator-g_entities, va("cps \"%s\"", ent->message ));
			}
			else
			{
				gi.SendServerCommand( activator-g_entities, va("cp \"%s\"", ent->message ));
			}
		}
	...
	if (ent->message[0] == '@' && ent->message[1] != '@')
	{
		gi.SendServerCommand( -1, va("cps \"%s\"", ent->message ));
	}
	else
	{
		gi.SendServerCommand( -1, va("cp \"%s\"", ent->message ));
	}
}
...
void SP_target_speaker( gentity_t *ent ) {
	...
	if ( !G_SpawnString( "noise", "NOSOUND", &s ) ) {
		gi.Error( ERR_DROP, "target_speaker without a noise key at %s", vtos( ent->s.origin ) );
	}
...
void target_laser_think (gentity_t *self) {
	...
	gi.Trace( &tr, self->s.origin, NULL, NULL, end, self->s.number, CONTENTS_SOLID|CONTENTS_BODY|CONTENTS_CORPSE, qfalse, 0, 0);
...
{
	gi.UnlinkEntity( (sharedEntity_t *)self );
	self->nextthink = 0;
}
...
void target_laser_start (gentity_t *self)
{
	...
	if (self->target) {
		...
		if (!ent) {
			gi.Print ("%s at %s: %s is a bad target\n", self->classname, vtos(self->s.origin), self->target);
		}
		self->enemy = ent;
	} else {
...
void target_teleporter_use( gentity_t *self, gentity_t *other, gentity_t *activator ) {
	...
	if (!dest) {
		gi.Print ("Couldn't find teleporter destination\n");
		return;
	}
...
static void target_location_linkup(gentity_t *ent)
{
	...
	gi.SetConfigstring( CS_LOCATIONS, "unknown" );
	...
	for (i = 0, ent = g_entities, n = 1;
		...
		if (ent->classname && !Q_stricmp(ent->classname, "target_location")) {
			...
			gi.SetConfigstring( CS_LOCATIONS + n, ent->message );
			n++;
	...
	*/
	...
	if (self->behaviorSet[BSET_USE])
	{
		if ( self->spawnflags & 1 )
		{
			...
			if (!gi.ICARUS_IsInitialized(self->s.number))
			{//Need to be initialized through ICARUS
				...
				if ( gi.ICARUS_ValidEnt( (sharedEntity_t *)self->activator ) )
				{
					gi.ICARUS_InitEnt( (sharedEntity_t *)self->activator );
				}
				else
				{
					if (developer.integer)
			...
			gi.ICARUS_RunScript( (sharedEntity_t *)self->activator, va( "%s/%s", Q3_SCRIPT_DIR, self->behaviorSet[BSET_USE] ) );
		}
		else
		{
...
void target_level_change_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	gi.SendConsoleCommand(EXEC_NOW, va("map %s", self->message));
}
...
void SP_target_level_change( gentity_t *self )
{
	...
	{
		gi.Error( ERR_DROP, "target_level_change with no mapname!\n");
		return;
	}
...
void SP_target_play_music( gentity_t *self )
{
	...
	{
		gi.Error( ERR_DROP, "target_play_music without a music key at %s", vtos( self->s.origin ) );
	}
