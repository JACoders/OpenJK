...
void Item_Text_Paint(itemDef_t *item) {
	...
	if (item->text2) // Is there a second line of text?
	{
		...
		Item_TextColor(item, &color);
		DC->drawText(item->textRect.x + item->text2alignx, item->textRect.y + item->text2aligny, item->textscale, color, textPtr, 0, 0, item->textStyle,
					 item->iMenuFont);
	}
}
...
void Item_TextField_Paint(itemDef_t *item) {
	...
	if (item->window.flags & WINDOW_HASFOCUS && g_editingField) {
		...
		DC->drawTextWithCursor(item->textRect.x + item->textRect.w + offset, item->textRect.y, item->textscale, newColor, buff + editPtr->paintOffset,
							   item->cursorPos - editPtr->paintOffset, cursor, item->window.rect.w, item->textStyle, item->iMenuFont);
	} else {
		DC->drawText(item->textRect.x + item->textRect.w + offset, item->textRect.y, item->textscale, newColor, buff + editPtr->paintOffset, 0,
...
void Item_YesNo_Paint(itemDef_t *item) {
	...
	/* JLF ORIGINAL CODE
		if (item->text) {
			Item_Text_Paint(item);
	//		DC->drawText(item->textRect.x + item->textRect.w + 8, item->textRect.y, item->textscale, newColor, (value != 0) ? sYES : sNO, 0, 0,
	item->textStyle); DC->drawText(item->textRect.x + item->textRect.w + 8, item->textRect.y, item->textscale, newColor, (value != 0) ? sYES : sNO, 0, 0,
...
void Item_ListBox_Paint(itemDef_t *item) {
	...
	if (item->window.flags & WINDOW_HORIZONTAL) {
		...
		if (text) {
			DC->drawText(item->window.rect.x, item->window.rect.y + item->window.rect.h, item->textscale, item->window.foreColor, text, 0, 0, item->textStyle,
						 item->iMenuFont);
		}
#endif
					...
					for (j = 0; j < listPtr->numColumns; j++) {
						...
						*/
						if (text) {
							...
							// WAS LAST						DC->drawText(x + 4 + listPtr->columnInfo[j].pos, y, item->textscale, item->window.foreColor, text, 0,
							// listPtr->columnInfo[j].maxChars, item->textStyle, item->iMenuFont);
							DC->drawText(x + 4 + listPtr->columnInfo[j].pos, y + listPtr->elementHeight + textyOffset + item->textaligny, item->textscale,
										 item->window.foreColor, text, 0, listPtr->columnInfo[j].maxChars, item->textStyle, item->iMenuFont);
						...
						if (optionalImage1 >= 0) {
							DC->drawHandlePic(imageStartX - listPtr->elementHeight, y + listPtr->elementHeight + 2, listPtr->elementHeight,
...
keywordHash_t itemParseKeywords[] = {{"action", ItemParse_action, NULL},
