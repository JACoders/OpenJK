...
// extern void CG_SurfaceExplosion( vec3_t origin, vec3_t normal, float radius,
// float shake_speed, qboolean smoke );
...
void funcBBrushDieGo(gentity_t *self) {
  ...
  CG_Chunks(self->s.number, org, dir, self->mins, self->maxs, 300, numChunks,
            chunkType, 0, scale);
...
void funcBBrushPain(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
  ...
  if (self->material == MAT_DRK_STONE || self->material == MAT_LT_STONE ||
    ...
    CG_Chunks(self->s.number, org, dir, self->mins, self->maxs, 300,
...
blowing up (none by default) "model2"	.md3 model to also draw "target"
all entities with a matching targetname will be used when this is destoryed
...
void misc_model_breakable_pain(gentity_t *self, gentity_t *inflictor,
                               gentity_t *other, vec3_t point, int damage,
                               int mod, int hitLoc) {
  if (self->health > 0) {
...
void misc_model_breakable_init(gentity_t *ent) {
  ...
  if (!ent->model) {
    G_Error("no model set on %s at (%.1f %.1f %.1f)", ent->classname,
...
void misc_model_breakable_gravity_init(gentity_t *ent, qboolean dropToFloor) {
  ...
  ent->physicsBounce = ent->mass =
      VectorLength(ent->maxs) + VectorLength(ent->mins);
  // drop to floor
  trace_t tr;
  vec3_t top, bottom;
...
void SP_misc_model_breakable(gentity_t *ent) {
  ...
  // HACK
  if (ent->model && Q_stricmp("models/map_objects/ships/tie_fighter.md3",
