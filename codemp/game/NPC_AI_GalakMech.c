...
static void GM_CheckMoveState(void) {
	...
	if ((NPCS.NPCInfo->goalEntity != NPCS.NPC->enemy) && (NPCS.NPCInfo->goalEntity != NULL)) {
		...
		if (NAV_HitNavGoal(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, NPCS.NPCInfo->goalEntity->r.currentOrigin, 16, qfalse) ||
			(!trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC, TID_MOVE_NAV) && enemyLOS4 &&
		...
		if (level.time - NPCS.NPCInfo->enemyLastSeenTime < 10000) {
			if (!Q_irand(0, 10)) {
				...
				if (dist < distThreshold) { // impact would be too close to me
					tooClose = qtrue;
				} else if (level.time - NPCS.NPCInfo->enemyLastSeenTime > 5000) { // we've haven't seen them in the last 5 seconds
...
void NPC_BSGM_Attack(void) {
	...
	if (0) {
		shoot4 = qfalse;
		if (TIMER_Done(NPCS.NPC, "smackTime") && !NPCS.NPCInfo->blockedDebounceTime) { // time to smack
			...
			if (enemyDist4 < MELEE_DIST_SQUARED &&
				...
				G_Damage(NPCS.NPC->enemy, NPCS.NPC, NPCS.NPC, smackDir, NPCS.NPC->r.currentOrigin, (g_npcspskill.integer + 1) * Q_irand(5, 10),
				...
				if (NPCS.NPC->client->ps.torsoAnim == BOTH_ATTACK4) { // smackdown
					...
					if (BG_CrouchAnim(NPCS.NPC->enemy->client->ps.legsAnim)) { // knockdown from crouch
	...
	{								// sometimes use the laser beam attack, but only after he's taken down our generator
		...
		if (NPCS.NPC->lockCount == 1) {				 // charging up
			if (TIMER_Done(NPCS.NPC, "beamDelay")) { // time to start the beam
				...
				*/
				...
				if (!NPCS.NPCInfo->coverTarg) { // for moving looping sound at end of trace
		...
		if (  // !NPC->client->ps.powerups[PW_GALAK_SHIELD]
			...
			&& enemyDist4 < MELEE_DIST_SQUARED && InFront(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles, 0.3f) &&
		...
		{										  // our shield is down, and enemy within 80, if very close, use melee attack to slap away
			if (TIMER_Done(NPCS.NPC, "attackDelay")) {
				...
				// FIXME: say something?
		...
		} else if (!NPCS.NPC->lockCount && NPCS.NPC->locationDamage[HL_GENERIC1] > GENERATOR_HEALTH && TIMER_Done(NPCS.NPC, "attackDelay") &&
				   InFront(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles, 0.3f) &&
				   ...
				   NPCS.NPC->enemy->s.weapon != WP_TURRET) { // sometimes use the laser beam attack, but only after he's taken down our generator
			shoot4 = qfalse;
			NPC_GM_StartLaser();
		} else if (enemyDist4 < MIN_LOB_DIST_SQUARED && (NPCS.NPC->enemy->s.weapon != WP_TURRET || Q_stricmp("PAS", NPCS.NPC->enemy->classname)) &&
		...
		{										  // enemy more than 448 away and we are ready to try lob fire again
			if ((NPCS.NPC->client->ps.weapon == WP_REPEATER) && !(NPCS.NPCInfo->scriptFlags & SCF_ALT_FIRE)) { // enemy far enough away to use lobby explosives
				NPCS.NPCInfo->scriptFlags |= SCF_ALT_FIRE;
	...
	if (enemyLOS4) {
		...
		if (NPCS.NPCInfo->goalEntity == NPCS.NPC->enemy) { // for now, always chase the enemy
			move4 = qtrue;
		}
	}
	if (enemyCS4) {
	...
	if (NPCS.NPC->client->ps.weapon == WP_REPEATER && (NPCS.NPCInfo->scriptFlags & SCF_ALT_FIRE) && shoot4 && TIMER_Done(NPCS.NPC, "attackDelay")) {
		...
		clearshot = WP_LobFire(NPCS.NPC, muzzle, target, mins, maxs, MASK_SHOT | CONTENTS_LIGHTSABER, velocity, qtrue, NPCS.NPC->s.number,
							   NPCS.NPC->enemy->s.number, 300, 1100, 1500, qtrue);
		if (VectorCompare(vec3_origin, velocity) ||
			...
			if (enemyLOS4 && enemyCS4 && TIMER_Done(NPCS.NPC, "noRapid")) { // have a clear straight shot, so switch to primary
				NPCS.NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;
				NPCS.NPC->alt_fire = qfalse;
				NPC_ChangeWeapon(WP_REPEATER);
	...
	// also:
	if (NPCS.NPC->enemy->s.weapon == WP_TURRET && !Q_stricmp("PAS", NPCS.NPC->enemy->classname)) { // crush turrets
		if (G_BoundsOverlap(
				NPCS.NPC->r.absmin, NPCS.NPC->r.absmax, NPCS.NPC->enemy->r.absmin,
				NPCS.NPC->enemy->r.absmax)) { // have to do this test because placed turrets are not solid to NPCs (so they don't obstruct navigation)
	...
	} else if (NPCS.NPCInfo->touchedByPlayer != NULL && NPCS.NPCInfo->touchedByPlayer == NPCS.NPC->enemy) { // touched enemy
		// if ( NPC->client->ps.powerups[PW_GALAK_SHIELD] > 0 )
		if (0) { // zap him!
			vec3_t smackDir;
...
// FIXME: some shield effect?
			...
			smackDir[2] += 30;
			VectorNormalize(smackDir);
			G_Damage(NPCS.NPC->enemy, NPCS.NPC, NPCS.NPC, smackDir, NPCS.NPC->r.currentOrigin, (g_npcspskill.integer + 1) * Q_irand(5, 10), DAMAGE_NO_KNOCKBACK,
					 MOD_UNKNOWN);
			// throw them
			G_Throw(NPCS.NPC->enemy, smackDir, 100);
...
void NPC_BSGM_Default(void) {
	...
	if (NPCS.NPC->client->ps.stats[STAT_ARMOR] <= 0) { // armor gone
		...
		if (0) { // start regenerating the armor
			...
			if (NPCS.NPC->locationDamage[HL_GENERIC1] < GENERATOR_HEALTH) { // still have the generator bolt-on
		...
		} else if (NPCS.NPCInfo->investigateDebounceTime < level.time) { // armor regenerated, turn shield back on
			// do a trace and make sure we can turn this back on?
			trace_t tr;
			trap->Trace(&tr, NPCS.NPC->r.currentOrigin, shieldMins, shieldMaxs, NPCS.NPC->r.currentOrigin, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0,
						0);
			if (!tr.startsolid) {
