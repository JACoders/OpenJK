...
cvar_t *Cvar_Get(const char *var_name, const char *var_value, int flags) {
  ...
  if (var) {
    ...
    // Make sure the game code cannot mark engine-added variables as gamecode
    // vars
...
void Cvar_Print(cvar_t *v) {
  ...
  if (!(v->flags & CVAR_ROM)) {
    if (!Q_stricmp(v->string, v->resetString))
      ...
      Com_Printf(", " S_COLOR_WHITE "default = " S_COLOR_GREY "\"" S_COLOR_WHITE
  ...
  if (v->latchedString)
    Com_Printf("     latched = " S_COLOR_GREY "\"" S_COLOR_WHITE
               "%s" S_COLOR_GREY "\"\n",
               v->latchedString);
}
    ...
    if (var->flags & CVAR_ARCHIVE) {
      ...
      if (var->latchedString) {
        ...
        if ((var->flags & CVAR_NODEFAULT) &&
            !strcmp(var->latchedString, var->resetString)) {
          continue;
        }
        ...
        if ((var->flags & CVAR_NODEFAULT) &&
            !strcmp(var->string, var->resetString)) {
          continue;
        }
        Com_sprintf(buffer, sizeof(buffer), "seta %s \"%s\"\n", var->name,
                    var->string);
...
void Cvar_List_f(void) {
  ...
  for (var = cvar_vars, i = 0; var; var = var->next, i++) {
    ...
    Com_Printf(S_COLOR_WHITE " %s = " S_COLOR_GREY "\"" S_COLOR_WHITE
                             "%s" S_COLOR_GREY "\"" S_COLOR_WHITE,
               var->name, var->string);
    if (var->latchedString)
      Com_Printf(", latched = " S_COLOR_GREY "\"" S_COLOR_WHITE
...
void Cvar_ListModified_f(void) {
  ...
  for (var = cvar_vars; var; var = var->next) {
    ...
    if (!var->name || !var->modificationCount ||
        !strcmp(value, var->resetString))
      continue;

    Com_Printf(S_COLOR_GREY
               "Cvar " S_COLOR_WHITE "%s = " S_COLOR_GREY "\"" S_COLOR_WHITE
               "%s" S_COLOR_GREY "\"" S_COLOR_WHITE ", " S_COLOR_WHITE
               "default = " S_COLOR_GREY "\"" S_COLOR_WHITE "%s" S_COLOR_GREY
               "\"" S_COLOR_WHITE "\n",
...
void Cvar_ListUserCreated_f(void) {
  ...
  if (count > 0)
    Com_Printf(S_COLOR_GREY "Showing " S_COLOR_WHITE "%u" S_COLOR_GREY
                            " user created cvars" S_COLOR_WHITE "\n",
               count);
  else
...
void Cvar_UnsetUserCreated_f(void) {
  ...
  if (count > 0)
    Com_Printf(S_COLOR_GREY "Removed " S_COLOR_WHITE "%u" S_COLOR_GREY
                            " user created cvars" S_COLOR_WHITE "\n",
               count);
  else
...
void Cvar_Update(vmCvar_t *vmCvar) {
  ...
  if ((unsigned)vmCvar->handle >= (unsigned)cvar_numIndexes) {
    Com_Error(ERR_DROP, "Cvar_Update: handle %u out of range",
