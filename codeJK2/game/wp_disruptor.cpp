  ...
  if (render_impact) {
    if (tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage) {
      ...
      if (traceEnt && traceEnt->client &&
        ...
        G_Damage(traceEnt, ent, ent, wpFwd, tr.endpos, 3,
                 DAMAGE_DEATH_KNOCKBACK, MOD_DISRUPTOR, hitLoc);
      } else {
        G_Damage(traceEnt, ent, ent, wpFwd, tr.endpos, damage,
                 DAMAGE_DEATH_KNOCKBACK, MOD_DISRUPTOR, hitLoc);
      }
    } else {
...
//---------------------------------------------------------
{
  int damage = weaponData[WP_DISRUPTOR].altDamage, skip,
  ...
  if (ent->NPC) {
    ...
    int count = (level.time - ent->client->ps.weaponChargeTime - 50) /
    ...
    damage = damage * count + weaponData[WP_DISRUPTOR].damage *
                                  0.5f; // give a boost to low charge shots
  }
  ...
  for (int i = 0; i < traces; i++) {
    ...
    // the "G2_COLLIDE, 10" to see if we it the sitter
    gi.trace(&tr, start, NULL, NULL, end, skip, MASK_SHOT, G2_COLLIDE,
             10); // G2_RETURNONHIT, 0 );
    ...
    if (!hitDodged) {
      if (render_impact) {
        if ((tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage) ||
          ...
          // world
          G_PlayEffect(G_EffectIndex("disruptor/alt_hit"), tr.endpos,
                       tr.plane.normal);
          ...
          if (traceEnt && traceEnt->client &&
              traceEnt->client->NPC_class == CLASS_GALAKMECH) { // hehe
            G_Damage(traceEnt, ent, ent, wpFwd, tr.endpos, 10,
