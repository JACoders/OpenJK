...
float weaponSpeed[WP_NUM_WEAPONS][2] = {
    {0, 0}, // WP_NONE,
    {0, 0}, // WP_SABER,				 // NOTE: lots of code assumes this is the first
    ...
    {BRYAR_PISTOL_VEL, BRYAR_PISTOL_VEL}, // WP_BOT_LASER,		// Probe droid
                                          // - Laser blast
...
//-----------------------------------------------------------------------------
void WP_TraceSetStart(const gentity_t *ent, vec3_t start, const vec3_t mins,
                      const vec3_t maxs)
//-----------------------------------------------------------------------------
{
  ...
  newstart[2] = start[2]; // force newstart to be on the same plane as the
                          // muzzle ( start )

  gi.trace(&tr, newstart, entMins, entMaxs, start, ent->s.number,
           MASK_SOLID | CONTENTS_SHOTCLIP, (EG2_Collision)0, 0);
...
//-----------------------------------------------------------------------------
{
  ...
  missile->s.pos.trTime =
      level.time; // - 10;	// move a bit on the very first frame
  ...
  if (pVeh) {
    ...
    VectorMA(missile->s.pos.trDelta, 2.0f,
             pVeh->m_pParentEntity->client->ps.velocity,
...
//-----------------------------------------------------------------------------
{
  ...
  if (self->splashDamage > 0 && self->splashRadius > 0) {
    G_RadiusDamage(self->currentOrigin, attacker, self->splashDamage,
                   self->splashRadius, 0 /*don't ignore attacker*/,
...
// We need to have a dieFunc, otherwise G_Damage won't actually make us die.  I
// could modify G_Damage, but that entails too many changes
//-----------------------------------------------------------------------------
    ...
    if ((coll.mFlags & G2_FRONTFACE)) {
      G_GetHitLocFromSurfName(
          ...
          gi.G2API_GetSurfaceName(
              ...
              coll.mSurfaceIndex),
          &hitLoc, coll.mCollisionPosition, NULL, NULL, mod);
...
//---------------------------------------------------------
{
  // FIXME: this is hacky and doesn't need to be here.  Was only put here to
  // make up for the times a crouch anim would be used but not actually
  // crouching. When we start calcing eyepos (SPOT_HEAD) from the tag_eyes, we
  // won't need this (or viewheight at all?)
  if (!ent)
    return;
  ...
  if (ent->client->ps.legsAnim == BOTH_CROUCH1IDLE ||
      ent->client->ps.legsAnim == BOTH_CROUCH1 ||
      ent->client->ps.legsAnim == BOTH_CROUCH1WALK) {
    if (ent->client->ps.viewheight !=
        ent->client->crouchheight + STANDARD_VIEWHEIGHT_OFFSET)
      ent->client->ps.viewheight =
          ent->client->crouchheight + STANDARD_VIEWHEIGHT_OFFSET;
  } else {
    if (ent->client->ps.viewheight !=
        ent->client->standheight + STANDARD_VIEWHEIGHT_OFFSET)
      ent->client->ps.viewheight =
          ent->client->standheight + STANDARD_VIEWHEIGHT_OFFSET;
  }
}
...
//---------------------------------------------------------
void CalcMuzzlePoint(gentity_t *const ent, vec3_t forwardVec, vec3_t right,
...
//---------------------------------------------------------
{
  ...
  {             // Not players or melee
    if (ent->client) {
      if (ent->client->renderInfo.mPCalcTime >=
          level.time -
              FRAMETIME *
                  2) { // Our muzz point was calced no more than 2 frames ago
  ...
  case WP_BLASTER:
    ...
    muzzlePoint[2] -= 1;
    if (ent->s.number == 0)
      VectorMA(muzzlePoint, 12, forwardVec,
               muzzlePoint); // player, don't set this any lower otherwise the
                             // projectile will impact immediately when your
                             // back is to a wall
    else
      VectorMA(muzzlePoint, 2, forwardVec,
               muzzlePoint); // NPC, don't set too far forwardVec otherwise the
                             // projectile can go through doors
...
void WP_RocketLock(gentity_t *ent, float lockDist) {
  ...
  gi.trace(&tr, muzzlePoint, NULL, NULL, ang, ent->client->ps.clientNum,
           MASK_PLAYERSOLID, (EG2_Collision)0, 0);

  if (tr.fraction != 1 && tr.entityNum < ENTITYNUM_NONE &&
      tr.entityNum != ent->client->ps.clientNum) {
    ...
    if (bgEnt && (bgEnt->s.powerups & PW_CLOAKED)) {
      ...
      if (ent->client->rocketLockIndex == ENTITYNUM_NONE) {
        ...
        ent->client->rocketLockTime = level.time;
                 ...
                 ent->client->rocketTargetTime < level.time) {
        ent->client->rocketLockIndex = tr.entityNum;
        ent->client->rocketLockTime = level.time;
      } else if (ent->client->rocketLockIndex == tr.entityNum) {
...
void WP_FireVehicleWeapon(gentity_t *ent, vec3_t start, vec3_t dir,
  ...
  if (!vehWeapon) { // invalid vehicle weapon
    ...
    if (ent->owner && ent->owner->s.number == 0) {
      ...
      if (ent->m_pVehicle->m_pVehicleInfo &&
          ent->m_pVehicle->m_pVehicleInfo->type == VH_SPEEDER) {
    ...
    */
    ...
    if (vehWeapon->fWidth ||
      ...
      missile->s.weapon = WP_BLASTER; // does this really matter?
      missile->methodOfDeath =
          MOD_EMPLACED; // MOD_TURBLAST; //count as a heavy weap
      ...
      if (ent->client && ent->client->rocketLockIndex != ENTITYNUM_NONE) {
        ...
        if (dif >= 10 && rTime != -1) {
          ...
          if (missile->enemy && missile->enemy->client &&
            ...
            missile->spawnflags |= 1; // just to let it know it should be
                                      // faster... FIXME: EXTERNALIZE
            missile->speed = vehWeapon->fSpeed;
            missile->angle = vehWeapon->fHoming;
            if (vehWeapon->iLifeTime) { // expire after a time
              missile->disconnectDebounceTime =
                  level.time + vehWeapon->iLifeTime;
...
//---------------------------------------------------------
{
  ...
  if (ent->owner && ent->owner->client &&
      ent->owner->client->ps.weapon != WP_NONE) {
    return;
  }
      ...
      if (pVeh->m_pPilot &&
        ...
        for (i = 0; i < MAX_VEHICLE_MUZZLES; i++) {
          ...
          if (pVeh->m_iMuzzleTag[i] != -1 &&
              pVeh->m_Muzzles[i].m_iMuzzleWait <
        ...
        if (pVeh->m_iMuzzleTag[i] != -1 &&
          ...
          if (pVeh->weaponStatus[weaponNum].ammo <
            ...
            if (WP_VehCheckTraceFromCamPos(
              ...
              gi.trace(&trace, ent->currentOrigin, vec3_origin, vec3_origin,
            ...
            if (vehWeapon->iMuzzleFX) {
              G_PlayEffect(vehWeapon->iMuzzleFX,
                           pVeh->m_Muzzles[i].m_vMuzzlePos,
          ...
          pVeh->m_Muzzles[pVeh->weaponStatus[weaponNum].nextMuzzle]
              .m_iMuzzleWait = level.time + delay;
          // take away the ammo
...
void WP_FireScepter(gentity_t *ent, qboolean alt_fire) { // just a straight beam
  int damage = 1;
  vec3_t start, end;
  trace_t tr;
  ...
  if (render_impact) {
    if (tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage) {
      ...
      G_PlayEffect(G_EffectIndex("disruptor/flesh_impact"), tr.endpos,
                   tr.plane.normal);

      int hitLoc = G_GetHitLocFromTrace(&tr, MOD_DISRUPTOR);
      G_Damage(traceEnt, ent, ent, forwardVec, tr.endpos, damage,
               DAMAGE_EXTRA_KNOCKBACK, MOD_DISRUPTOR, hitLoc);
    } else {
      G_PlayEffect(G_EffectIndex("disruptor/wall_impact"), tr.endpos,
                   tr.plane.normal);
    }
  }
  ...
  frame... VectorMA( start, dist, forwardVec, spot ); AddSightEvent( ent, spot,
  256, AEL_DISCOVERED, 50 );
  }
...
//---------------------------------------------------------
{
  ...
  if (ent->s.weapon == WP_DISRUPTOR && alt_fire) {
    ...
    if (!ent->s.number) { // player driving an AT-ST
      // SIGH... because we can't anticipate alt-fire, must calc muzzle here and
      // now
      mdxaBone_t boltMatrix;
      int bolt;

      if (ent->client->ps.weapon ==
          WP_ATST_MAIN) { // FIXME: alt_fire should fire both barrels, but
                          // slower?
        if (ent->alt_fire) {
          bolt = ent->handRBolt;
        } else {
        ...
        if (ent->alt_fire) {
          if (gi.G2API_GetSurfaceRenderStatus(
                  ...
                  "head_light_blaster_cann")) { // don't have it!
            return;
          }
          ...
          if (gi.G2API_GetSurfaceRenderStatus(
                  &ent->ghoul2[ent->playerModel],
                  "head_concussion_charger")) { // don't have it!
            return;
          }
      ...
      gi.G2API_GetBoltMatrix(ent->ghoul2, ent->playerModel, bolt, &boltMatrix,
                             yawOnlyAngles, ent->currentOrigin,
                             (cg.time ? cg.time : level.time), NULL,
                             ent->s.modelScale);

      // work the matrix axis stuff into the original axis and origins used.
      gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN,
                                      ent->client->renderInfo.muzzlePoint);
      gi.G2API_GiveMeVectorFromMatrix(boltMatrix, NEGATIVE_Y,
                                      ent->client->renderInfo.muzzleDir);
    ...
    } else if (!ent->enemy) { // an NPC with no enemy to auto-aim at
      VectorCopy(ent->client->renderInfo.muzzleDir, forwardVec);
    } else { // NPC, auto-aim at enemy
    ...
    { // use our muzzleDir, can't use viewangles or vehicle m_vOrientation
      // because we may be animated to shoot left or right...
      if ((ent->s.eFlags & EF_NODRAW)) // we're inside it
      {
        ...
        // Aiming Left
        //-------------
        if (ent->client->ps.torsoAnim == BOTH_VT_ATL_G ||
            ent->client->ps.torsoAnim == BOTH_VS_ATL_G) {
          VectorScale(actorRight, -1.0f, forwardVec);
        }

        // Aiming Right
        //--------------
        else if (ent->client->ps.torsoAnim == BOTH_VT_ATR_G ||
                 ent->client->ps.torsoAnim == BOTH_VS_ATR_G) {
        ...
        if (ent->enemy) {
          ...
          if (DotProduct(toEnemy, forwardVec) > 0.75f &&
              ((ent->s.number == 0 &&
                !Q_irand(0, 2)) || // the player has a 1 in 3 chance
               (ent->s.number != 0 &&
                !Q_irand(0, 5)))) // other guys have a 1 in 6 chance
          {
  ...
  case WP_EMPLACED_GUN:
    // doesn't care about whether it's alt-fire or not.  We can do an alt-fire
    // if needed
    WP_EmplacedFire(ent);
    break;
  ...
  case WP_ATST_SIDE:
    ...
    if (alt_fire) {
      ...
      /*	if ( ent->s.number == 0
                      ...
                      && vehicleData[((CVehicleNPC
         *)ent->NPC)->m_iVehicleTypeID].type == VH_FIGHTER )
              {
                      WP_ATSTMainFire( ent );
              }
              else*/
  ...
  if (!ent->s.number) {
    if (ent->s.weapon == WP_FLECHETTE ||
        (ent->s.weapon == WP_BOWCASTER &&
         !alt_fire)) { // these can fire multiple shots, count them individually
                       // within the firing functions
    } else if (W_AccuracyLoggableWeapon(ent->s.weapon, alt_fire, MOD_UNKNOWN)) {
      ent->client->sess.missionStats.shotsFired++;
    }
  }
  // We should probably just use this as a default behavior, in special cases,
  // just set alert to false.
  if (ent->s.number == 0 && alert > 0) {
    if (ent->client->ps.groundEntityNum ==
            ENTITYNUM_WORLD // FIXME: check for sand contents type?
        && ent->s.weapon != WP_STUN_BATON && ent->s.weapon != WP_MELEE &&
        ent->s.weapon != WP_TUSKEN_STAFF && ent->s.weapon != WP_THERMAL &&
        ent->s.weapon != WP_TRIP_MINE &&
        ent->s.weapon != WP_DET_PACK) { // the vibration of the shot carries
...
void misc_weapon_shooter_fire(gentity_t *self) {
  ...
  if ((self->spawnflags & 2)) { // repeat
    ...
    if (self->random) {
      self->nextthink =
          level.time + self->wait + (int)(Q_flrand(0.0f, 1.0f) * self->random);
    } else {
...
void misc_weapon_shooter_use(gentity_t *self, gentity_t *other,
                             gentity_t *activator) {
  if (self->e_ThinkFunc ==
      thinkF_misc_weapon_shooter_fire) { // repeating fire, stop
...
void misc_weapon_shooter_aim(gentity_t *self) {
  ...
  if (self->target) {
    ...
    if (targ) {
      self->enemy = targ;
      VectorSubtract(targ->currentOrigin, self->currentOrigin,
                     self->client->renderInfo.muzzleDir);
      VectorCopy(targ->currentOrigin, self->pos1);
      vectoangles(self->client->renderInfo.muzzleDir,
...
void SP_misc_weapon_shooter(gentity_t *self) {
  ...
  if (self->paintarget) { // use a different weapon
    self->s.weapon = self->client->ps.weapon =
        GetIDForString(WPTable, self->paintarget);
  }
  ...
  if (self->target) {
    ...
    AngleVectors(self->s.angles, self->client->renderInfo.muzzleDir, NULL,
                 NULL);
  }
