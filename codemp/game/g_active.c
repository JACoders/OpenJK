...
#define MIN_NPC_SPEED 16.0f
      ...
      ent->s.NPC_class != CLASS_VEHICLE) // vehicles manage their own speed
  {
    ...
    if (ucmd->buttons &
        BUTTON_WALKING) { // sort of a hack I guess since MP handles walking
                          // differently from SP (has some proxy cheat prevention
                          // methods)
      /*
  ...
  if (ent->client->ps.duelInProgress) {
    ...
    // engage in duels in case this player wants to engage again right after he's
    // done fighting and someone else is waiting.
