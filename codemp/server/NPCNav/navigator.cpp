...
void CNavigator::SetEdgeCost(int ID1, int ID2, int cost) {
	if ((ID1 == -1) || (ID2 == -1)) { // not valid nodes, must have come from the ClearAllFailedEdges initization-type calls
		return;
	}
		...
		for (edgeNum = 0; edgeNum < (*ni)->GetNumEdges(); edgeNum++) {
			...
			if ((flags & EFLAG_BLOCKED)) {
				...
				if (trace.entityNum < ENTITYNUM_WORLD &&
					(trace.fraction < 1.0f || trace.startsolid == qtrue || trace.allsolid == qtrue)) { // could be assumed, since failed before
					if (GVM_NAV_EntIsDoor(trace.entityNum)) {										   // door
						if (!GVM_NAV_EntIsUnlockedDoor(trace.entityNum)) {							   // locked door
							failed = qtrue;
						}
					} else {
						if (GVM_NAV_EntIsBreakable(trace.entityNum)) { // do same for breakable brushes/models/glass?
							failed = qtrue;
						} else if (GVM_NAV_EntIsRemovableUsable(trace.entityNum)) {
							failed = qtrue;
						} else if (trace.allsolid ||
								   trace
									   .startsolid) { // FIXME: the entitynum would be none here, so how do we know if this is stuck inside an ent or the world?
...
int CNavigator::TestNodePath(sharedEntity_t *ent, int okToHitEntNum, vec3_t position, qboolean includeEnts) { // rwwFIXMEFIXME: Share clipmask?
...
int CNavigator::TestNodeLOS(sharedEntity_t *ent, vec3_t position) { return GVM_NPC_ClearLOS2(ent->s.number, position); }
...
#define MAX_Z_DELTA 18
	...
	STL_ITERATE(nci, nodeChain) {
		...
		if (CheckedNode(nodeNum, ent->s.number) == CHECKED_FAILED) { // already checked this node against ent and it failed
			continue;
		}
		if (CheckedNode(nodeNum, ent->s.number) == CHECKED_PASSED) { // already checked this node against ent and it passed
			...
			// okay, since we only have to do this once, let's check to see if this node is even usable (could help us short-circuit a whole loop of the dest
			// nodes)
			...
			if ((int)(*nci).distance >= (radius * radius) || (fabs(position[2] - ent->r.currentOrigin[2]) >= MAX_Z_DELTA)) {
				...
				{													 // need a clear path or LOS
					if (!SV_inPVS(ent->r.currentOrigin, position)) { // not even potentially clear
						SetCheckedNode(nodeNum, ent->s.number, CHECKED_FAILED);
						continue;
					}
				}
		...
		STL_ITERATE(nci2, nodeChain2) {
			...
			if (CheckedNode(nodeNum2, goal->s.number) == CHECKED_FAILED) { // already checked this node against goal and it failed
				continue;
			}
			if (CheckedNode(nodeNum2, goal->s.number) == CHECKED_PASSED) { // already checked this node against goal and it passed
				...
				if ((int)(*nci2).distance >= (radius * radius) || (fabs(position2[2] - goal->r.currentOrigin[2]) >= MAX_Z_DELTA)) {
					...
					if (flags & NF_CLEAR_PATH)							   //|| flags & NF_CLEAR_LOS )
					{													   // need a clear path or LOS
						if (!SV_inPVS(goal->r.currentOrigin, position2)) { // not even potentially clear
							SetCheckedNode(nodeNum2, goal->s.number, CHECKED_FAILED);
							continue;
						}
					}
	...
	if (!d_altRoutes->integer) {										 // bestNode would not have been set by GetBestNodeAltRoute above, so get it here
		if (ent->waypoint != NODE_NONE && goal->waypoint != NODE_NONE) { // have 2 valid waypoints which means a valid path
...
#define MAX_Z_DELTA 18
	...
	STL_ITERATE(nci, nodeChain) {
		...
		if (targetID != WAYPOINT_NONE) { // we want to find the one with the shortest route here
	...
	if (ent->failedWaypointCheckTime && ent->failedWaypointCheckTime < svs.time) {
		...
		for (j = 0; j < MAX_FAILED_NODES; j++) {
			if (ent->failedWaypoints[j] != 0) {
				...
				if (!GVM_NAV_ClearPathToPoint(ent->s.number, ent->r.mins, ent->r.maxs, nodePos, (CONTENTS_SOLID | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP),
											  ENTITYNUM_NONE)) { // no path clear of architecture, so clear this since we can't check against entities
				...
				// have clear architectural path, now check against ents only
												  ...
												  ENTITYNUM_NONE)) { // clear of ents, too, so all clear, clear this one out
					ent->failedWaypoints[j] = 0;
					failed--;
				}
			}
	...
	for (j = 0; j < MAX_FAILED_NODES; j++) {
		if (ent->failedWaypoints[j] == 0) {
			ent->failedWaypoints[j] =
				nodeID + 1; //+1 because 0 is the default value and that's a valid node, so we take the +1 out when we check the node above
			if (!ent->failedWaypointCheckTime) {
				ent->failedWaypointCheckTime = svs.time + CHECK_FAILED_EDGE_INTITIAL;
			}
			return;
		}
		if (ent->failedWaypoints[j] == nodeID + 1) { // already have this one marked as failed
			return;
		}
	}
...
qboolean CNavigator::NodesAreNeighbors(int startID, int endID) { // See if these 2 are neighbors
	if (startID == endID) {
		return qfalse;
	}
...
qboolean CNavigator::CheckFailedEdge(failedEdge_t *failedEdge) {
	...
	if (failedEdge->checkTime < svs.time) {
		if (failedEdge->startID != WAYPOINT_NONE) {
			...
			SV_Trace(&trace, start, mins, maxs, end, ignore, clipmask | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP, qfalse, 0,
					 10); // NOTE: should we really always include monsterclip (physically blocks NPCs) and botclip (do not enter)?
			...
			} else if (hitEntNum == failedEdge->entID) { // don't hit the person who initially marked the edge failed
				hitEntNum = ENTITYNUM_NONE;
			}
...
class NodeTotalGreater {
  public:
	bool operator()(CEdge *first, CEdge *second) const { return (first->m_cost > second->m_cost); }
};
...
void CPriorityQueue::Update(CEdge *edge) {
	for (std::vector<CEdge *>::iterator i = mHeap.begin(); i != mHeap.end(); ++i) {
		if ((*i)->m_first == edge->m_first) { // Found node - resort from this position in the mHeap
