...
typedef struct boltOn_s { // a tagged object
  char name[32]; // If there is a name, it is considered active (look up in
                 // boltOns.cfg)
  modelInfo_t model;
  targetModel_t targetModel; //	name of object to attach to
  char targetTag[20];        //	name of tag on target object to attach to
  vec3_t angleOffsets;       //	angle offsets to apply when attaching to target
                             //object's tag
  vec3_t originOffsets;      //	angle offsets to apply when attaching to target
                             //object's tag
} boltOn_t;
