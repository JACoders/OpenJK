...
void SandCreature_MoveEffect(void) {
	...
	if (level.time - NPC->client->ps.lastStationary > 2000) { // first time moving for at least 2 seconds
		// clear speakingtime
		TIMER_Set(NPC, "speaking", -level.time);
	}

	if (TIMER_Done(NPC, "breaching") && TIMER_Done(NPC, "breachDebounce") && TIMER_Done(NPC, "pain") && TIMER_Done(NPC, "attacking") &&
		!Q_irand(0, 10)) { // Breach!
...
qboolean SandCreature_Move(void) {
	...
	if (SandCreature_CheckAhead(dest)) { // use our temp move straight to goal check
	...
	if (moved && NPC->radius) {
		...
		if (newTurfRange > NPC->radius && newTurfRange > curTurfRange) { // would leave our range
			// stop
			...
			if (g_spskill->integer > 0) {
				...
				if (enemyDist < 200 && NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE) {
					...
					if (g_spskill->integer > 1) {									   // knock them down, too
						if (NPC->enemy->health > 0 && Q_flrand(50, 150) > enemyDist) { // knock them down
							G_Knockdown(NPC->enemy, NPC, dir2Enemy, 300, qtrue);
							if (NPC->enemy->s.number < MAX_CLIENTS) { // make the player look up at me
								vec3_t vAng;
...
void SandCreature_CheckMovingEnts(void) {
	...
	for (i = 0; i < numEnts; i++) {
		...
		if (radiusEnts[i]->client == NULL) {													 // must be a client
			if (radiusEnts[i]->s.eType != ET_MISSILE || radiusEnts[i]->s.weapon != WP_THERMAL) { // not a thermal detonator
				continue;
			}
		} else {
			...
			if ((radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_WAMPA)) { // can't be one being held
				continue;
			}

			if ((radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_SAND_CREATURE)) { // can't be one being held
				continue;
			}
			...
			if (radiusEnts[i]->client->ps.groundEntityNum != ENTITYNUM_WORLD) { // not on the ground
				continue;
			}
...
void SandCreature_Chase(void) {
	...
	if (NPC->enemy->client) {
		if ((NPC->enemy->client->ps.eFlags & EF_HELD_BY_SAND_CREATURE) || (NPC->enemy->client->ps.eFlags & EF_HELD_BY_RANCOR) ||
			(NPC->enemy->client->ps.eFlags & EF_HELD_BY_WAMPA)) { // was picked up by another monster, forget about him
			NPC->enemy = NULL;
	...
	// chase the enemy
	if (NPC->enemy->client && NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_WORLD &&
		...
		if (enemyScore < MIN_SCORE && !(NPC->svFlags & SVF_LOCKEDENEMY)) { // too slow or too far away
		} else {
			float moveSpeed;
			if (NPC->enemy->client) {
	...
	if ((level.time - NPCInfo->enemyLastSeenTime) > 5000 &&
		!(NPC->svFlags & SVF_LOCKEDENEMY)) { // enemy hasn't moved in about 5 seconds, see if there's anything else of interest
	...
	if (enemyDistSq >= MIN_ATTACK_DIST_SQ						// NPCInfo->goalEntity &&
		&& (level.time - NPCInfo->enemyLastSeenTime) <= 3000) { // sensed enemy (or something) less than 3 seconds ago
		ucmd.buttons &= ~BUTTON_WALKING;
		if (SandCreature_Move()) {
			SandCreature_MoveEffect();
		}
	} else if ((level.time - NPCInfo->enemyLastSeenTime) <= 5000 &&
			   !(NPC->svFlags & SVF_LOCKEDENEMY)) { // NOTE: this leaves a 2-second dead zone in which they'll just sit there unless their enemy moves
		// If there is an event we might be interested in if we weren't still interested in our enemy
		if (NPC_CheckAlertEvents(qfalse, qtrue, NPCInfo->lastAlertID, qfalse, AEL_MINOR, qtrue) >= 0) { // just stir
			SandCreature_MoveEffect();
		}
	}
	...
	} else if (enemyDistSq < MAX_MISS_DIST_SQ && enemyDistSq > MIN_MISS_DIST_SQ && NPC->enemy->client && TIMER_Done(NPC, "breaching") &&
			   TIMER_Done(NPC, "missDebounce") && !VectorCompare(NPC->pos1, NPC->currentOrigin) // so we don't come up again in the same spot
			   && !Q_irand(0, 10)) {
		if (!(NPC->svFlags & SVF_LOCKEDENEMY)) {
			// miss them
...
void NPC_BSSandCreature_Default(void) {
	...
	if (NPC->health > 0 && TIMER_Done(NPC, "breaching")) { // go back to non-solid mode
		if (NPC->contents) {
			NPC->contents = 0;
		}
	...
	if (NPC->client->ps.legsAnim == BOTH_ATTACK1 ||
		NPC->client->ps.legsAnim == BOTH_ATTACK2) { // FIXME: get start and end frame numbers for this effect for each of these anims
		vec3_t up = {0, 0, 1};
		vec3_t org;
