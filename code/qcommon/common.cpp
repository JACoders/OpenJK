...
void NORETURN QDECL Com_Error(int code, const char *fmt, ...) {
  ...
  if (code != ERR_DISCONNECT) {
    Cvar_Get("com_errorMessage", "",
             CVAR_ROM); // give com_errorMessage a default so it won't come back
                        // to life after a resetDefaults
...
// I'm leaving this in just in case we ever need to remember where's a good
// place to hook something like this in.
//
...
void SG_WipeSavegame(
    const char *name); // pretty sucky, but that's how SoF did it...<g>
...
void Com_Init(char *commandLine) {
  ...
  try {
    ...
    // re.R_InitWorldEffects();   // this doesn't do much but I want to be sure
    // certain variables are intialized.
    ...
    Sys_Init(); // this also detects CPU type, so I can now do this CPU check
                // below...
    ...
    // if ( (Cvar_VariableIntegerValue("ui_iscensored") == 1) 	//if this was on
    // before, set it again so it gets its flags
    //	)
    //{
    //	Cvar_Get( "ui_iscensored",   "1",
    //CVAR_ARCHIVE|CVAR_ROM|CVAR_INIT|CVAR_CHEAT|CVAR_NORESTART); 	Cvar_Set(
    //"ui_iscensored",   "1");	//just in case it was archived
    //	// NOTE : I also create this in UI_Init()
    //	Cvar_Get( "g_dismemberment", "0",
    //CVAR_ARCHIVE|CVAR_ROM|CVAR_INIT|CVAR_CHEAT); 	Cvar_Set( "g_dismemberment",
    //"0");	//just in case it was archived
    //}
  } catch (int code) {
    Com_CatchError(code);
...
void Com_WriteConfig_f(void) {
  ...
  if (!COM_CompareExtension(filename, ".cfg")) {
    Com_Printf("Com_WriteConfig_f: Only the \".cfg\" extension is supported by "
               "this command!\n");
    return;
  }

  if (!FS_FilenameCompare(filename, "mpdefault.cfg") ||
      !FS_FilenameCompare(filename, "default.cfg")) {
    Com_Printf(S_COLOR_YELLOW "Com_WriteConfig_f: The filename \"%s\" is "
...
void Com_Frame(void) {
  try {
    int timeBeforeFirstEvents = 0, timeBeforeServer = 0, timeBeforeEvents = 0,
        timeBeforeClient = 0, timeAfter = 0;
    int msec, minMsec;
    int timeVal;
    ...
    if (com_speeds->integer) {
      ...
      Com_Printf("fr:%i all:%3i sv:%3i ev:%3i cl:%3i gm:%3i tr:%3i pvs:%3i "
                 "rf:%3i bk:%3i\n",
                 com_frameNumber, all, sv, ev, cl, time_game, timeInTrace,
                 timeInPVSCheck, time_frontend, time_backend);
      ...
      if (com_speedslog && com_speedslog->integer) {
        ...
        if (speedslog) {
          ...
          Com_sprintf(msg, sizeof(msg), "%8.4f,%8.4f,%8.4f,%8.4f,%8.4f,%8.4f,",
                      corg[0], corg[1], corg[2], cangles[0], cangles[1],
                      cangles[2]);
    ...
    if (com_showtrace->integer) {
      ...
      c_traces - numTraces, numTraces, timeInTrace,
      (float)timeInTrace/(float)numTraces ); timeInTrace = numTraces = 0;
      c_traces = 0;
      */
...
static void PrintCvarMatches(const char *s) {
  ...
  if (!Q_stricmpn(s, shortestMatch, (int)strlen(shortestMatch))) {
    ...
    Com_Printf(S_COLOR_GREY "Cvar " S_COLOR_WHITE "%s = " S_COLOR_GREY
                            "\"" S_COLOR_WHITE "%s" S_COLOR_GREY
                            "\"" S_COLOR_WHITE "\n",
               s, value);
  }
}
