...
// Generic group flags, used by parser, then get converted to the appropriate
// specific flags
#define FX_PARM_MASK                                                           \
  0xC // use this to mask off any transition types that use a parm
...
// Shared flag--these flags, at first glance would appear to be shared, but are
// safe.  I'd rather not do this, but as you can see, features flags are
// currently all accounted for
#define FX_PAPER_PHYSICS                                                       \
  0x00010000 // emitters ONLY.  shared with FX_SIZE_2_LINEAR
#define FX_LOCALIZED_FLASH                                                     \
  0x00010000 // full screen flashes ONLY.  shared with FX_SIZE_2_LINEAR
#define FX_PLAYER_VIEW                                                         \
  0x00010000 // player view effects ONLY.  shared with FX_SIZE_2_LINEAR
...
// rww - g2-related flags (these can slow things down significantly, use
// sparingly) These should be used only with particles/decals as they steal flags
// used by cylinders.
#define FX_GHOUL2_TRACE                                                        \
  0x00020000 // use in conjunction with particles - actually do full ghoul2
             // traces for physics collision against entities with a ghoul2
             // instance shared FX_SIZE2_RAND (used only with cylinders)
#define FX_GHOUL2_DECALS                                                       \
  0x00040000 // use in conjunction with decals - can project decal as a ghoul2
             // gore skin object onto ghoul2 models shared FX_SIZE2_NONLINEAR
             // (used only with cylinders)

#define FX_ATTACHED_MODEL 0x01000000

#define FX_APPLY_PHYSICS 0x02000000
#define FX_USE_BBOX                                                            \
  0x04000000 // can make physics more accurate at the expense of speed

#define FX_USE_ALPHA                                                           \
  0x08000000 // the FX system actually uses RGB to do fades, but this will
             // override that
             //	and cause it to fill in the alpha.

#define FX_EMIT_FX                                                             \
  0x10000000 // emitters technically don't have to emit stuff, but when they do
             //	this flag needs to be set
#define FX_DEATH_RUNS_FX                                                       \
  0x20000000 // Normal death triggers effect, but not kill_on_impact
#define FX_KILL_ON_IMPACT                                                      \
  0x40000000 // works just like it says, but only when physics are on.
#define FX_IMPACT_RUNS_FX                                                      \
  0x80000000 // an effect can call another effect when it hits something.

// Lightning flags, duplicates of existing flags, but lightning doesn't use
// those flags in that context...and nothing will ever use these in this
// context..so we are safe.
...
#define FX_GROW                                                                \
  0x04000000 // lightning grows from start point to end point over the course of
             // its life

// stuff that can occur when an effect is flagged with "materialImpact" and it
// hits something
enum EMatImpactEffect { MATIMPACTFX_NONE = 0, MATIMPACTFX_SHELLSOUND };
...
public:
  ...
  inline void SetSound(int vol, int rad) {
    mSoundRadius = rad;
    mSoundVolume = vol;
  }
  inline void SetMin(vec3_t min) {
    if (min) {
      VectorCopy(min, mMin);
    } else {
      VectorClear(mMin);
  ...
  inline void SetOrigin1(vec3_t org) {
    if (org) {
  ...
  inline void SetTimeStart(int time) {
    ...
    if (mFlags & FX_SET_SHADER_TIME) {
      mRefEnt.shaderTime = time * 0.001f;
    }
  }
...
public:
  typedef struct {
    ...
    // this is a very specific case thing...allow interpolating the st coords so
    // we can map the texture
...
public:
  ...
  inline void SetBoltinfo(CGhoul2Info_v *ghoul2, int entNum, int modelNum = -1,
                          int boltNum = -1) {
    mGhoul2 = ghoul2;
    mEntNum = entNum;
    mModelNum = modelNum;
...
public:
  ...
  inline void SetAccel(vec3_t ac) {
    if (ac) {
  ...
  inline void SetSizeStart(float sz) {
    mSizeStart = sz;
    mRefEnt.radius = sz;
  }
  inline void SetSizeEnd(float sz) { mSizeEnd = sz; }
...
public:
  CFlash() : mScreenX(0), mScreenY(0), mRadiusModifier(1) {}
...
protected:
  // kef -- mScreenX and mScreenY are used for flashes that are
  // FX_LOCALIZED_FLASH
  float mScreenX;
  float mScreenY;
...
public:
  ...
  void DrawSegment(vec3_t start, vec3_t end, float texcoord1, float texcoord2,
                   float segPercent, float lastSegPercent);

  inline void SetControlPoints(vec3_t ctrl1, vec3_t ctrl2) {
    VectorCopy(ctrl1, mControl1);
    VectorCopy(ctrl2, mControl2);
  }
  inline void SetControlVel(vec3_t ctrl1v, vec3_t ctrl2v) {
...
//------------------------------
// Emitters are derived from particles because, although they don't draw, any
// effect called
//	from them can borrow an initial or ending value from the emitters
...
protected:
  ...
  vec3_t mAngleDelta; //	as opposed to an end angle is probably much
                      //easier
...
public:
  ...
  inline void SetAngles(vec3_t ang) {
    if (ang) {
  ...
  inline void SetAngleDelta(vec3_t ang) {
    if (ang) {
  ...
  inline void SetLastOrg(vec3_t org) {
    if (org) {
  ...
  inline void SetLastVel(vec3_t vel) {
    if (vel) {
...
// We're getting pretty low level here, not the kind of thing to abuse
// considering how much overhead this
...
public:
  ...
  inline void SetMotionTimeStamp(int t) {
    mTimeStamp = theFxHelper.GetTime() + t;
  }
