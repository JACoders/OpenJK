...
extern void cgi_S_StartSound(vec3_t origin, int entityNum, int entchannel,
                             sfxHandle_t sfx);
...
static void NPC_CheckAttacker(gentity_t *other, int mod) {
  // FIXME: I don't see anything in here that would stop teammates from taking a
  // teammate 			as an enemy.  Ideally, there would be code before this to prevent
  ...
  if (NPC->svFlags & SVF_LOCKEDENEMY) { // IF LOCKED, CANNOT CHANGE ENEMY!!!!!
    return;
  }
  ...
  if (NPC->client->ps.weapon == WP_SABER) { // I'm a jedi
    if (mod == MOD_SABER) { // I was hit by a saber  FIXME: what if this was a
                            // thrown saber?
...
void NPC_SetPainEvent(gentity_t *self) {
  if (!self->NPC || !(self->NPC->aiFlags & NPCAI_DIE_ON_IMPACT)) {
    if (!Q3_TaskIDPending(self, TID_CHAN_VOICE)) {
      G_AddEvent(self, EV_PAIN,
                 floor((float)self->health / self->max_health * 100.0f));
    }
  }
}
...
float NPC_GetPainChance(gentity_t *self, int damage) {
  ...
  float pain_chance =
      (float)(self->max_health - self->health) / (self->max_health * 2.0f) +
...
extern int G_PickPainAnim(gentity_t *self, vec3_t point, int damage,
                          int hitLoc);
void NPC_ChoosePainAnimation(gentity_t *self, gentity_t *other, vec3_t point,
                             int damage, int mod, int hitLoc,
                             int voiceEvent = -1) {
  // If we've already taken pain, then don't take it again
  if (level.time < self->painDebounceTime && mod != MOD_ELECTROCUTE &&
      mod != MOD_MELEE) { // FIXME: if hit while recoving from losing a saber
                          // lock, we should still play a pain anim?
    return;
  }
  ...
  if (self->s.weapon == WP_THERMAL &&
      self->client->fireDelay > 0) { // don't interrupt thermal throwing anim
    return;
    ...
    } else if (self->client->ps.powerups[PW_GALAK_SHIELD]) { // shield up
      return;
    } else if (self->health > 200 && damage < 100) { // have a *lot* of health
      pain_chance = 0.05f;
    } else { // the lower my health and greater the damage, the more likely I am
             // to play a pain anim
      pain_chance = (200.0f - self->health) / 100.0f + damage / 50.0f;
    }
  } else if (self->client && self->client->playerTeam == TEAM_PLAYER && other &&
             !other->s.number) { // ally shot by player always complains
    pain_chance = 1.1f;
  } else {
    if (other && (other->s.weapon == WP_SABER || mod == MOD_ELECTROCUTE ||
                  mod == MOD_CRUSH /*FIXME:MOD_FORCE_GRIP*/)) {
      pain_chance = 1.0f;          // always take pain from saber
    } else if (mod == MOD_MELEE) { // higher in rank (skill) we are, less likely
                                   // we are to be fazed by a punch
      pain_chance =
          1.0f - ((RANK_CAPTAIN - self->NPC->rank) / (float)RANK_CAPTAIN);
  ...
  if (Q_flrand(0.0f, 1.0f) < pain_chance) {
    ...
    if (!(self->client->ps.eFlags &
          EF_FORCE_GRIPPED)) { // not being force-gripped
      if (G_CheckForStrongAttackMomentum(self) ||
          ...
          (PM_FlippingAnim(self->client->ps.legsAnim) &&
           !PM_InCartwheel(
               self->client->ps
                   .legsAnim))) { // strong attacks, rolls, knockdowns, flips
                                  // and spins cannot be interrupted by pain
        return;
      } else {                                            // play an anim
        if (self->client->NPC_class == CLASS_GALAKMECH) { // only has 1 for now
          ...
          pain_anim = PM_PickAnim(self, BOTH_PAIN2, BOTH_PAIN3);
                   ...
                   WP_SABER) { // temp HACK: these are the only 2 pain anims
                               // that look good when holding a saber
          pain_anim = PM_PickAnim(self, BOTH_PAIN2, BOTH_PAIN3);
        ...
        self->client->ps.saberAnimLevel =
            FORCE_LEVEL_1; // next attack must be a quick attack
        self->client->ps.saberMove =
            LS_READY; // don't finish whatever saber move you may have been in
        int parts = SETANIM_BOTH;
        if (PM_CrouchAnim(self->client->ps.legsAnim) ||
            PM_InCartwheel(self->client->ps.legsAnim)) {
          parts = SETANIM_LEGS;
        }
    ...
    self->painDebounceTime =
        level.time + PM_AnimLength(self->client->clientInfo.animFileIndex,
                                   (animNumber_t)pain_anim);
...
===============
*/
void NPC_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
  ...
  if (self->client->playerTeam && other->client &&
    ...
    if (other != self->enemy &&
      ...
      if (self->enemy || other->enemy ||
          ...
          /*|| (!other->s.number&&Q_irand( 0, 3 ))*/) { // if one of us actually
                                                        // has an enemy already,
                                                        // it's okay, just an
                                                        // accident OR wasn't hit
                                                        // by player or someone
                                                        // controlled by player
                                                        // OR player hit ally and
                                                        // didn't get 25% chance
                                                        // of getting mad
                                                        // (FIXME:accumulate
                                                        // anger+base on diff?)
        // FIXME: player should have to do a certain amount of damage to ally or
        // hit them several times to make them mad Still run pain and flee
        // scripts
      ...
      {                                         // dammit, stop that!
        if (self->NPC->charmedTime) {           // mindtricked
          return;
        } else if (self->NPC->ffireCount <
                   3 + ((2 - g_spskill->integer) * 2)) { // not mad enough yet
          // Com_Printf( "chck: %d < %d\n", self->NPC->ffireCount,
          ...
          return;
        } else if (G_ActivateBehavior(
                       self, BSET_FFIRE)) { // we have a specific script to run,
                                            // so do that instead
          return;
        } else { // okay, we're going to turn on our ally, we need to set and
                 // lock our enemy and put ourselves in a bstate that lets us
                 // attack him (and clear any flags that would stop us)
          self->NPC->blockedSpeechDebounceTime = 0;
          voiceEvent = EV_FFTURN;
          self->NPC->behaviorState = self->NPC->tempBehavior =
              self->NPC->defaultBehavior = BS_DEFAULT;
          other->flags &= ~FL_NOTARGET;
          self->svFlags &=
              ~(SVF_IGNORE_ENEMIES | SVF_ICARUS_FREEZE | SVF_NO_COMBAT_SOUNDS);
          G_SetEnemy(self, other);
          self->svFlags |= SVF_LOCKEDENEMY;
          self->NPC->scriptFlags &=
              ~(SCF_DONT_FIRE | SCF_CROUCHED | SCF_WALKING |
                SCF_NO_COMBAT_TALK | SCF_FORCED_MARCH);
...
void NPC_Touch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (self->message && self->health <= 0) { // I am dead and carrying a key
    if (other && player && player->health > 0 &&
        other == player) { // player touched me
      char *text;
      qboolean keyTaken;
      // give him my key
      if (Q_stricmp("goodie", self->message) == 0) { // a goodie key
        if ((keyTaken = INV_GoodieKeyGive(other)) == qtrue) {
          text = "cp @INGAME_TOOK_IMPERIAL_GOODIE_KEY";
          G_AddEvent(other, EV_ITEM_PICKUP,
                     (FindItemForInventory(INV_GOODIE_KEY) - bg_itemlist));
        } else {
          text = "cp @INGAME_CANT_CARRY_GOODIE_KEY";
        }
      } else { // a named security key
        if ((keyTaken = INV_SecurityKeyGive(player, self->message)) == qtrue) {
          text = "cp @INGAME_TOOK_IMPERIAL_SECURITY_KEY";
          G_AddEvent(other, EV_ITEM_PICKUP,
                     (FindItemForInventory(INV_SECURITY_KEY) - bg_itemlist));
        } else {
          text = "cp @INGAME_CANT_CARRY_SECURITY_KEY";
        }
      }
      if (keyTaken) { // remove my key
        gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel], "l_arm_key",
                                 0x00000002);
        self->message = NULL;
  ...
  if (other->client) { // FIXME:  if pushing against another bot, both
                       // ucmd.rightmove = 127???
    ...
    if (!(self->svFlags & SVF_LOCKEDENEMY) &&
        !(self->svFlags & SVF_IGNORE_ENEMIES) &&
        !(other->flags & FL_NOTARGET)) {
      if (self->client->enemyTeam) { // See if we bumped into an enemy
        if (other->client->playerTeam ==
            self->client->enemyTeam) { // bumped into an enemy
          if (NPCInfo->behaviorState != BS_HUNT_AND_KILL &&
              !NPCInfo->tempBehavior) { // MCG - Begin: checking specific BS
                                        // mode here, this is bad, a HACK
            // FIXME: not medics?
            if (NPC->enemy != other) { // not already mad at them
              G_SetEnemy(NPC, other);
            }
            //				NPCInfo->tempBehavior =
            //BS_HUNT_AND_KILL;
          }
        }
      }
    ...
    /*
    if ( other->s.number == 0 && self->client->playerTeam ==
    other->client->playerTeam )
    {
            VectorAdd( self->client->pushVec, other->client->ps.velocity,
    self->client->pushVec );
    }
    */
...
void NPC_TempLookTarget(gentity_t *self, int lookEntNum, int minLookTime,
                        int maxLookTime) {
  if (!self->client) {
    return;
  }
  ...
  if (!NPC_CheckLookTarget(self)) { // Not already looking at something else
    ...
    NPC_SetLookTarget(self, lookEntNum,
                      level.time + Q_irand(minLookTime, maxLookTime));
  }
}
...
void NPC_Respond(gentity_t *self, int userNum) {
  ...
  */
  ...
  case CLASS_R2D2: // droid
    G_Sound(self, G_SoundIndex(va("sound/chars/r2d2/misc/r2d2talk0%d.wav",
                                  Q_irand(1, 3))));
    break;
  case CLASS_R5D2: // droid
    G_Sound(self, G_SoundIndex(
                      va("sound/chars/r5d2/misc/r5talk%d.wav", Q_irand(1, 4))));
    break;
  case CLASS_MOUSE: // droid
    G_Sound(self, G_SoundIndex(va("sound/chars/mouse/misc/mousego%d.wav",
                                  Q_irand(1, 3))));
    break;
  case CLASS_GONK: // droid
    G_Sound(self, G_SoundIndex(va("sound/chars/gonk/misc/gonktalk%d.wav",
                                  Q_irand(1, 2))));
    break;
  default:
    break;
...
void NPC_UseResponse(gentity_t *self, gentity_t *user, qboolean useWhenDone) {
  ...
  if (user->s.number != 0) { // not used by the player
    if (useWhenDone) {
  ...
  if (user->client && self->client->playerTeam != user->client->playerTeam &&
      self->client->playerTeam !=
          TEAM_NEUTRAL) { // only those on the same team react
    if (useWhenDone) {
      G_ActivateBehavior(self, BSET_USE);
    }
    return;
  ...
  if (self->NPC->blockedSpeechDebounceTime >
      level.time) { // I'm not responding right now
    return;
  }

  if (gi.VoiceVolume[self->s.number]) { // I'm talking already
    if (!useWhenDone) {                 // you're not trying to use me
      return;
    }
  }
...
void NPC_Use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  if (self->client->ps.pm_type ==
      PM_DEAD) { // or just remove ->pain in player_die?
    return;
  }
  ...
  if (self->client && self->NPC) {
    ...
    if (activator && activator->s.number == 0 &&
      ...
      // NOTE: this will steal up to MAX_BATTERIES for the activator, leaving
      // the residual on the gonk for potential later use.
    ...
    /*
       ...
       activator->client )
                    {//Heal me NOW, dammit!
                            ...
                            else if ( !self->enemy && activator->s.number == 0
       && !gi.VoiceVolume[self->s.number] &&
       !(self->NPC->scriptFlags&SCF_NO_RESPONSE) )
                            {//I don't have an enemy and I'm not talking and I
       was used by the player NPC_UseResponse( self, other, qfalse );
                            }
                    }
    */
    ...
    //		}
             ...
             !gi.VoiceVolume[self->s.number] &&
             !(self->NPC->scriptFlags &
               SCF_NO_RESPONSE)) { // I don't have an enemy and I'm not talking
                                   // and I was used by the player
...
void NPC_CheckPlayerAim(void) {
  ...
  if ( player && player->client && player->client->ps.weapon > (int)(WP_NONE) &&
  player->client->ps.weapon < (int)(WP_TRICORDER) )
  {//player has a weapon ready
          if ( g_crosshairEntNum == NPC->s.number && level.time -
  g_crosshairEntTime < 200
...
void NPC_CheckAllClear(void) {
  // FIXME: need to make this happen only once after losing enemies, not over
  // and over again
  /*
  if ( NPC->client && !NPC->enemy && level.time -
  teamLastEnemyTime[NPC->client->playerTeam] > 10000 )
