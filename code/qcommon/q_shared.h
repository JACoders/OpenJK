...
//#pragma warning(disable : 4032)		//formal parameter 'number' has
//different type when promoted #pragma warning(disable : 4051)		//type
//conversion; possible loss of data
//#pragma warning(disable : 4057)		// slightly different base types
#pragma warning(disable : 4100) // unreferenced formal parameter
//#pragma warning(disable : 4115)		//'type' : named type definition in
//parentheses
#pragma warning(                                                               \
    disable : 4125) // decimal digit terminates octal escape sequence
#pragma warning(disable : 4127) // conditional expression is constant
//#pragma warning(disable : 4136)		//conversion between different
//floating-point types
//#pragma warning(disable : 4201)		//nonstandard extension used :
//nameless struct/union #pragma warning(disable : 4214)
////nonstandard extension used : bit field types other than int #pragma
//warning(disable : 4220)		// varargs matches remaining parameters
...
#define BASE_SAVE_COMPAT // this is defined to disable/fix some changes that
...
typedef int32_t qhandle_t, thandle_t, fxHandle_t, sfxHandle_t, fileHandle_t,
    clipHandle_t;
...
#define CVAR_TEMP                                                              \
  0 // can be set even when cheats are disabled, but is not archived
#define CVAR_ARCHIVE                                                           \
...
#define CVAR_NODEFAULT                                                         \
  16384 // do not write to config if matching with default value
...
typedef struct {
  ...
  int entityNum;       // entity the contacted sirface is a part of
                       ...
                       */
  CCollisionRecord
      G2CollisionMap[MAX_G2_COLLISIONS]; // map that describes all of the parts
                                         // of ghoul2 models that got hit
                                         /*
                                         Ghoul2 Insert End
                                         */
...
#define CS_RESERVED1                                                           \
  (CS_SOUNDS +                                                                 \
   MAX_SOUNDS) // reserved field for base compat from immersion removal
...
#else
#define CS_EFFECTS                                                             \
  (CS_BSP_MODELS + MAX_SUB_BSP) //(CS_LIGHT_STYLES + (MAX_LIGHT_STYLES*3))
#endif                          // JK2_MODE
...
typedef enum {
  ...
  FP_RAGE, // duration - speed, invincibility and extra damage for short period,
           // drains your health and leaves you weak and slow afterwards.
  FP_PROTECT, // duration - protect against physical/energy (level 1 stops
              // blaster/energy bolts, level 2 stops projectiles, level 3
              // protects against explosions)
  FP_ABSORB, // duration - protect against dark force powers (grip, lightning,
             // drain - maybe push/pull, too?)
...
// NOTE!!! be careful about altering this because although it's used to define
// an array size, the entry indexes come from
//	the typedef'd enum "persEnum_t" in bg_public.h, and there's no
//compile-tie between the 2 -slc
//
...
#define MAX_PS_EVENTS                                                          \
  2 // this must be a power of 2 unless you change some &'s to %'s -ste
...
typedef struct {
  ...
  int forceRestrictions; // force powers that cannot be used while this saber is
                         // on (bitfield) - FIXME: maybe make this a limit on the
                         // max level, per force power, that can be used with
                         // this type?
  ...
  int disarmBonus; // added to disarm chance when win saberlock or have a good
                   // parry (knockaway)
  ...
  char *brokenSaber1; // if saber is actually hit by another saber, it can be
                      // cut in half/broken and will be replaced with this saber
                      // in your right hand
  char *brokenSaber2; // if saber is actually hit by another saber, it can be
                      // cut in half/broken and will be replaced with this saber
                      // in your left hand
  //===NEW========================================================================================
  ...
  qhandle_t spinSound; // none - if set, plays this sound as it spins when
                       // thrown
  qhandle_t
      swingSound[3]; // none - if set, plays one of these 3 sounds when swung
                     // during an attack - NOTE: must provide all 3!!!
  qhandle_t
      fallSound[3]; // none - if set, plays one of these 3 sounds when weapon
                    // drops to the ground - NOTE: must provide all 3!!!
  ...
  //===PRIMARY BLADES=====================
  // done in cgame (client-side code)
  int trailStyle; // 0 - default (0) is normal, 1 is a motion blur and 2 is no
                  // trail at all (good for real-sword type mods)
  char g2MarksShader[MAX_QPATH]; // none - if set, the game will use this shader
                                 // for marks on enemies instead of the default
                                 // "gfx/damage/saberglowmark"
  char g2WeaponMarkShader[MAX_QPATH]; // none - if set, the game will ry to
                                      // project this shader onto the weapon when
                                      // it damages a person (good for a blood
                                      // splatter on the weapon)
  // int		bladeShader;				//none - if set, overrides the shader used
  // for the saber blade? int		trailShader;
  // //none - if set, overrides the shader used for the saber trail?
  qhandle_t hitSound[3]; // none - if set, plays one of these 3 sounds when
                         // saber hits a person - NOTE: must provide all 3!!!
  qhandle_t blockSound[3]; // none - if set, plays one of these 3 sounds when
                           // saber/sword hits another saber/sword - NOTE: must
                           // provide all 3!!!
  qhandle_t bounceSound[3]; // none - if set, plays one of these 3 sounds when
                            // saber/sword hits a wall and bounces off (must set
                            // bounceOnWall to 1 to use these sounds) - NOTE:
                            // must provide all 3!!!
  int blockEffect; // none - if set, plays this effect when the saber/sword hits
                   // another saber/sword (instead of "saber/saber_block.efx")
  int hitPersonEffect; // none - if set, plays this effect when the saber/sword
                       // hits a person (instead of "saber/blood_sparks_mp.efx")
  int hitOtherEffect; // none - if set, plays this effect when the saber/sword
                      // hits something else damagable (instead of
                      // "saber/saber_cut.efx")
  int bladeEffect; // none - if set, plays this effect at the blade tag

  // done in game (server-side code)
  float knockbackScale; // 0 - if non-zero, uses damage done to calculate an
  ...
  int splashDamage;   // 0 - amount of splashDamage, 100% at a distance of 0, 0%
                    // at a distance = splashRadius
  float splashKnockback; // 0 - amount of splashKnockback, 100% at a distance of
                         // 0, 0% at a distance = splashRadius

  //===SECONDARY BLADES===================
  // done in cgame (client-side code)
  int trailStyle2; // 0 - default (0) is normal, 1 is a motion blur and 2 is no
                   // trail at all (good for real-sword type mods)
  char g2MarksShader2[MAX_QPATH]; // none - if set, the game will use this
                                  // shader for marks on enemies instead of the
                                  // default "gfx/damage/saberglowmark"
  char g2WeaponMarkShader2[MAX_QPATH]; // none - if set, the game will ry to
                                       // project this shader onto the weapon
                                       // when it damages a person (good for a
                                       // blood splatter on the weapon)
  // int		bladeShader2;				//none - if set, overrides the shader
  // used for the saber blade?
  // int		trailShader2;				//none - if set, overrides the shader
  // used for the saber trail?
  qhandle_t hit2Sound[3]; // none - if set, plays one of these 3 sounds when
                          // saber hits a person - NOTE: must provide all 3!!!
  qhandle_t block2Sound[3]; // none - if set, plays one of these 3 sounds when
                            // saber/sword hits another saber/sword - NOTE: must
                            // provide all 3!!!
  qhandle_t bounce2Sound[3]; // none - if set, plays one of these 3 sounds when
                             // saber/sword hits a wall and bounces off (must set
                             // bounceOnWall to 1 to use these sounds) - NOTE:
                             // must provide all 3!!!
  int blockEffect2; // none - if set, plays this effect when the saber/sword
                    // hits another saber/sword (instead of
                    // "saber/saber_block.efx")
  int hitPersonEffect2; // none - if set, plays this effect when the saber/sword
                        // hits a person (instead of "saber/blood_sparks_mp.efx")
  int hitOtherEffect2; // none - if set, plays this effect when the saber/sword
                       // hits something else damagable (instead of
                       // "saber/saber_cut.efx")
  int bladeEffect2; // none - if set, plays this effect at the blade tag

  // done in game (server-side code)
  float knockbackScale2; // 0 - if non-zero, uses damage done to calculate an
                         // appropriate amount of knockback
  float damageScale2;  // 1 - scale up or down the damage done by the saber
  float splashRadius2; // 0 - radius of splashDamage
  int splashDamage2; // 0 - amount of splashDamage, 100% at a distance of 0, 0%
                     // at a distance = splashRadius
  float splashKnockback2; // 0 - amount of splashKnockback, 100% at a distance
                          // of 0, 0% at a distance = splashRadius
  //=========================================================================================================================================
  void Activate(void) {
  ...
  //	[in]		bool bActive	Whether to activate it (default true), or
  //deactivate it (false). 	[return]	void
    ...
    for (int i = 0; i < numBlades; i++) {
      if (bladeStyle2Start > 0) {
        if (i >= bladeStyle2Start) {
          if ((saberFlags2 &
               SFL2_NO_MANUAL_DEACTIVATE2)) { // don't count this blade
            continue;
          }
        } else if ((saberFlags2 &
                    SFL2_NO_MANUAL_DEACTIVATE)) { // don't count this blade
          continue;
        }
      } else if ((saberFlags2 & SFL2_NO_MANUAL_DEACTIVATE)) { // don't count any
                                                              // of these blades!
        continue;
  ...
  float Length() { // return largest length
    float len1 = 0;
    for (int i = 0; i < numBlades; i++) {
...
// NOTE: Below is the *retail* version of the saberInfo_t structure - it is ONLY
// used for loading retail-version savegames (we load the savegame into this
// smaller structure, then copy each field into the appropriate field in the new
// structure - see SG_ConvertRetailSaberinfoToNewSaberinfo()
...
public:
  ...
  qboolean throwable; // whether or not this saber can be thrown - FIXME: maybe
                      // make this a max level of force saber throw that can be
                      // used with this saber?
  qboolean disarmable;     // whether or not this saber can be dropped
  qboolean activeBlocking; // whether or not to try to block incoming shots with
                           // this saber
  ...
  saber_styles_t singleBladeStyle; // makes it so that you use a different style
                                   // if you only have the first blade active
  qboolean singleBladeThrowable; // makes it so that you can throw this saber if
                                 // only the first blade is on
  char *brokenSaber1; // if saber is actually hit by another saber, it can be
                      // cut in half/broken and will be replaced with this saber
                      // in your right hand
  char *brokenSaber2; // if saber is actually hit by another saber, it can be
                      // cut in half/broken and will be replaced with this saber
                      // in your left hand
  qboolean returnDamage; // when returning from a saber throw, it keeps spinning
                         // and doing damage
  void Activate(void) {
    for (int i = 0; i < numBlades; i++) {
      blade[i].active = qtrue;
  ...
  void sg_export(saberInfo_t &dst) const;
}; // saberInfoRetail_t

#define MAX_SABERS                                                             \
  2 // if this ever changes then update the table "static const save_field_t
    // savefields_gClient[]"!!!!!!!!!!!!
...
template <typename TSaberInfo> class PlayerStateBase {
public:
  ...
  int legsAnim;        //
  int legsAnimTimer;  // don't change low priority animations on legs until this
                      // runs out
  int torsoAnim;      //
  int torsoAnimTimer; // don't change low priority animations on torso until
                      // this runs out
  ...
  char security_key_message[MAX_SECURITY_KEYS]
                           [MAX_SECURITY_KEY_MESSSAGE]; // Security key types
  ...
  // NEW vehicle stuff
  // This has been localized to the vehicle stuff (NOTE: We can still use it
  // later, I'm just commenting it to root out all the calls. We can store the
  // data in vehicles and update by copying it here).
  // int			vehicleIndex;	// Index into vehicleData table
  // vec3_t		vehicleAngles;	// current angles of your vehicle
  // int			vehicleArmor;	// current armor of your vehicle (explodes if
  // drops to 0)

  // !!
  // not communicated over the net at all
  // !!
  // int			vehicleLastFXTime;				//timer for all
  // cgame-FX...? int			vehicleExplodeTime;
  // //when it will go BOOM!
  ...
  //			or descend them as classes - so not every client has all this
  //info
  TSaberInfo saber[MAX_SABERS];
  qboolean dualSabers;
  qboolean SaberStaff(void) {
    return (qboolean)(saber[0].type == SABER_STAFF ||
  ...
  float SaberLength() { // return largest length
  ...
  //1)). 	[in]	bool bActive	Whether to make it active (default true) or
  //inactive (false). 	[return]	void
  void SaberBladeActivate(int iSaber, int iBlade, qboolean bActive = qtrue) {
    // Validate saber (if it's greater than or equal to zero, OR it above the
    // first saber but we are not doing duel Sabers, leave, something is not
    // right.
  ...
  int SaberDisarmBonus(int bladeNum) {
    ...
    if (dualSabers && saber[1].Active()) { // bonus for having 2 sabers
  ...
  int forcePowerDebounce[NUM_FORCE_POWERS]; // for effects that must have an
                                            // interval
  int forcePower;
  int forcePowerMax;
  ...
  int forcePowerLevel[NUM_FORCE_POWERS]; // so we know the max forceJump power
                                         // you have
  float forceJumpZStart; // So when you land, you don't get hurt as much
  float forceJumpCharge; // you're current forceJump charge-up level, increases
                         // the longer you hold the force jump button down
  ...
  int forcePowersForced; // client is being forced to use these powers (FIXME:
                         // and only these?)
  ...
  float waterheight; // exactly what the z org of the water is (will be +4 above
                     // if under water, -4 below if not in water)
  ...
  int heldByBolt; // for IK, what bolt I'm attached to on the holdersomeone is
                  // grabbing me by
...
#define BUTTON_WALKING                                                         \
  16 // walking can't just be infered from MOVE_RUN because a key pressed late
     // in the frame will only generate a small move value for that frame
...
typedef struct entityState_s { // !!!!!!!!!!! LOADSAVE-affecting struct
  ...
  // FIXME: why did IMMERSION dupe these 2 fields here?  There's no reason for
  // this!!!
  qboolean saberInFlight;
  qboolean saberActive;
...
#ifndef JK2_MODE
  // int		vehicleIndex;		// What kind of vehicle you're
  // driving
  ...
  */
  ...
  int radius; // used for culling all the ghoul models attached to this ent NOTE
              // - this is automatically scaled by Ghoul2 if/when you scale the
              // model. This is a 100% size value
  int boltInfo; // info used for bolting entities to Ghoul2 models - NOT used
                // for bolting ghoul2 models to themselves, more for stuff like
                // bolting effects to ghoul2 models
  /*
  Ghoul2 Insert End
  */
...
typedef struct SSkinGoreData_s {
  ...
  bool frontFaces;
  bool backFaces;
  bool fadeRGB; // specify fade method to modify RGB (by default, the alpha is
                // set instead)

  // growing stuff
  int growDuration; // time over which we want this to scale up, set to -1 for
                    // no scaling
  float goreScaleStartFraction; // fraction of the final size at which we want
                                // the gore to initially appear
  ...
  int fadeOutTime; // specify the duration of fading, from the lifeTime (e.g.
                   // 3000 will start fading 3 seconds before removal and be
                   // faded entirely by removal)
  // int				shrinkOutTime;			//
  // unimplemented float			alphaModulate;			//
  // unimplemented
  // vec3_t			tint;					//
  // unimplemented float			impactStrength;			//
  // unimplemented
...
typedef struct {
  ...
  vec3_t desiredOrigin; // world coordinate that this bone should be attempting
                        // to reach
  vec3_t origin; // world coordinate of the entity who owns the g2 instance that
                 // owns the bone
...
#define sCVARNAME_PLAYERSAVE                                                   \
  "playersave" // used for level-transition, accessed by game and server modules
...
typedef struct parseData_s {
  ...
  int com_tokenline;
  const char
      *bufferStart; // Start address of buffer holding data that was read in
...
// stuff to help out during development process, force reloading/uncacheing of
// certain filetypes...
//
typedef enum {
