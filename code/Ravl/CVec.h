...
// Vector Library
// --------------
// The base implimention of the Raven Vector object attempts to solve a number
// of high level problems as efficiently as possible.  Where ever feasible,
// functions have been included in the .h file so the compiler can inline them.
//
...
//
// As necessary, some projects may #define special faster versions of these
// routines to make better use of native hardware / software implimentations.
//
//
//
//
// NOTES:
// 05/29/02 - CREATED
// 05/30/02 - RotatePoint() is currently unimplimented.  Waiting for Matrix
// Library
//
//
////////////////////////////////////////////////////////////////////////////////////////
...
#define RAVL_VEC_DEGTORAD(a)                                                   \
  ((a)*RAVL_VEC_DEGTORADCONST) // Quick Macro For Degrees -> Radians
#define RAVL_VEC_RADTODEG(a)                                                   \
  ((a)*RAVL_VEC_RADTODEGCONST) // Quick Macro For Radians -> Degrees
...
public:
  ...
  inline CVec4 operator+(const CVec4 &t) const {
    return CVec4(v[0] + t.v[0], v[1] + t.v[1], v[2] + t.v[2], v[3] + t.v[3]);
  }
  inline CVec4 operator-(const CVec4 &t) const {
    return CVec4(v[0] - t.v[0], v[1] - t.v[1], v[2] - t.v[2], v[3] - t.v[3]);
  }
  ...
  float Dist2(const CVec4 &t) const {
    return (
        (t.v[0] - v[0]) * (t.v[0] - v[0]) + (t.v[1] - v[1]) * (t.v[1] - v[1]) +
        (t.v[2] - v[2]) * (t.v[2] - v[2]) + (t.v[3] - v[3]) * (t.v[3] - v[3]));
  }
  ...
  float MaxElement() const { return v[MaxElementIndex()]; }
  ...
  // Project
  //
  // Standard projection function.  Take the (this) and project it onto the
  // vector (U).  Imagine drawing a line perpendicular to U from the endpoint of
  // the (this) Vector.  That then becomes the new vector.
  //
  // The value returned is the scale of the new vector with respect to the one
  // passed to the function.  If the scale is less than (1.0) then the new
  // vector is shorter than (U).  If the scale is negative, then the vector is
  // going in the opposite direction of (U).
  //
  //               _  (U)
  //               /|
  //             /                                        _ (this)
  ...
  float Project(const CVec4 &U) {
    ...
    (*this) *=
        Scale; // Use the previously calculated scale to get the right length.
    return Scale;
  }
  ...
  // Project To Line
  //
  // This function takes two other points in space as the start and end of a
  // line segment and projects the (this) point onto the line defined by
  // (Start)->(Stop)
  //
  // RETURN VALUES:
  //   (-INF, 0.0)  : (this) landed on the line before (Start)
  //   (0.0, 1.0)   : (this) landed in the line segment between (Start) and
  //   (Stop) (1.0, INF)   : (this) landed on the line beyond (End)
  //
  //             (Stop)
  //               /
  //             /
  ...
  //
  // Given two more points, this function calculates the area of the parallel
  // pipid formed.
  //
  // Note: This function CAN return a negative "area" if (this) is above or
  // right of (A) and (B)...  We do not take the abs because the sign of the
  // "area" is needed for the left right test (see below)
  //
  //
  //               ___---( ... )
  //        (A)---/        /
  ...
  float AreaParallelPipid(const CVec4 &A, const CVec4 &B) const {
    return ((A.v[0] * B.v[1] - A.v[1] * B.v[0]) +
            (B.v[0] * v[1] - v[0] * B.v[1]) + (v[0] * A.v[1] - A.v[0] * v[1]));
  }

  ////////////////////////////////////////////////////////////////////////////////////
  // Area Of The Triangle (2D)
  //
  // Given two more points, this function calculates the area of the triangle
  // formed.
  //
  //        (A)
  //        /  \__
  ...
  //
  // Given a line segment (Start->End) and a tolerance for *right on*, this
  // function evaluates which side the point is of the line.  (Side_Left in this
  // example)
  //
  //
  //
  ...
  //
  ////////////////////////////////////////////////////////////////////////////////////
  ESide LRTest(const CVec4 &Start, const CVec4 &End,
               float Tolerance = 0.0) const {
...
public:
  ...
  bool operator==(const CVec3 &t) const {
    return (v[0] == t.v[0] && v[1] == t.v[1] && v[2] == t.v[2]);
  }
  bool operator!=(const CVec3 &t) const {
    return !(v[0] == t.v[0] && v[1] == t.v[1] && v[2] == t.v[2]);
  ...
  inline CVec3 operator+(const CVec3 &t) const {
    return CVec3(v[0] + t.v[0], v[1] + t.v[1], v[2] + t.v[2]);
  }
  inline CVec3 operator-(const CVec3 &t) const {
    return CVec3(v[0] - t.v[0], v[1] - t.v[1], v[2] - t.v[2]);
  }
  ...
  //
  ////////////////////////////////////////////////////////////////////////////////////
  ESide LRTest(const CVec3 &Start, const CVec3 &End,
               float Tolerance = 0.0) const {
    float Area = AreaParallelPipid(Start, End);
    if (Area > Tolerance) {
      return Side_Left;
