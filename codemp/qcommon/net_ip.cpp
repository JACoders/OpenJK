...
*/
#ifdef MACOS_X
// Don't do a forward mapping from the hostname of the machine to the IP.  The reason is that we might have obtained an IP address from DHCP and there might not
// be any name registered for the machine.  On Mac OS X, the machine name defaults to 'localhost' and NetInfo has 127.0.0.1 listed for this name.  Instead, we
// want to get a list of all the IP network interfaces on the machine. This code adapted from OmniNetworking.

#ifdef _SIZEOF_ADDR_IFREQ
// tjw: OSX 10.4 does not have sa_len
#define IFR_NEXT(ifr) ((struct ifreq *)((char *)ifr + _SIZEOF_ADDR_IFREQ(*ifr)))
#else
// tjw: assume that once upon a time some version did have sa_len
#define IFR_NEXT(ifr) ((struct ifreq *)((char *)(ifr) + sizeof(*(ifr)) + Q_max(0, (int)(ifr)->ifr_addr.sa_len - (int)sizeof((ifr)->ifr_addr))))
#endif
