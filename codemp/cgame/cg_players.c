...
char *cg_customSoundNames[MAX_CUSTOM_SOUNDS] = {
    "*death1", "*death2",  "*death3",   "*jump1",  "*pain25", "*pain50",
    "*pain75", "*pain100", "*falling1", "*choke1", "*choke2", "*choke3",
    "*gasp",   "*land1",   "*taunt",    NULL};
...
const char *cg_customCombatSoundNames[MAX_CUSTOM_COMBAT_SOUNDS] = {
    ...
    "*pushed2",  "*pushed3",  "*choke1", "*choke2",
    "*choke3",   "*ffwarn",   "*ffturn", NULL};

// Used as a supplement to the basic set for stormtroopers
// (keep numbers in ascending order in order for variant-capping to work)
const char *cg_customExtraSoundNames[MAX_CUSTOM_EXTRA_SOUNDS] = {"*chase1",
                                                                 "*chase2",
                                                                 "*chase3",
                                                                 "*cover1",
                                                                 "*cover2",
...
const char *cg_customJediSoundNames[MAX_CUSTOM_JEDI_SOUNDS] = {
    "*combat1",    "*combat2",  "*combat3",  "*jdetected1", "*jdetected2",
    "*jdetected3", "*taunt1",   "*taunt2",   "*taunt3",     "*jchase1",
    "*jchase2",    "*jchase3",  "*jlost1",   "*jlost2",     "*jlost3",
    "*deflect1",   "*deflect2", "*deflect3", "*gloat1",     "*gloat2",
    "*gloat3",     "*pushfail", NULL};
...
sfxHandle_t CG_CustomSound(int clientNum, const char *soundName) {
  ...
  if (clientNum >= MAX_CLIENTS) { // these are only for npc's
    for (i = 0; i < MAX_CUSTOM_SOUNDS; i++) {
  ...
  if (cgs.gametype >= GT_TEAM || com_buildScript.integer) { // siege only
  ...
  if (cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL ||
      com_buildScript.integer) { // Duel only
    for (i = 0; i < MAX_CUSTOM_SOUNDS; i++) {
  ...
  for (i = 0; i < MAX_CUSTOM_SOUNDS; i++) {
    if (i < numCSounds && !strcmp(lSoundName, cg_customSoundNames[i])) {
      return ci->sounds[i];
    } else if ((cgs.gametype >= GT_TEAM || com_buildScript.integer) &&
               i < numCSiegeSounds &&
               !strcmp(lSoundName, bg_customSiegeSoundNames[i])) { // siege only
      return ci->siegeSounds[i];
    } else if ((cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL ||
                com_buildScript.integer) &&
               i < numCDuelSounds &&
               !strcmp(lSoundName, cg_customDuelSoundNames[i])) { // siege only
      return ci->duelSounds[i];
    } else if (clientNum >= MAX_CLIENTS && i < numCComSounds &&
               !strcmp(lSoundName, cg_customCombatSoundNames[i])) { // npc only
      return ci->combatSounds[i];
    } else if (clientNum >= MAX_CLIENTS && i < numCExSounds &&
               !strcmp(lSoundName, cg_customExtraSoundNames[i])) { // npc only
      return ci->extraSounds[i];
    } else if (clientNum >= MAX_CLIENTS && i < numCJediSounds &&
               !strcmp(lSoundName, cg_customJediSoundNames[i])) { // npc only
...
qboolean CG_ParseSurfsFile(const char *modelName, const char *skinName,
                           char *surfOff, char *surfOn) {
  const char *text_p;
  int len;
  const char *token;
  const char *value;
  ...
  while (skinName && skinName[i]) {
    if (skinName[i] == '|') { // this is a multi-part skin, said skins do not
                              // support .surf files
      return qfalse;
    }
  ...
  Com_sprintf(sfilename, sizeof(sfilename), "models/players/%s/model_%s.surf",
              modelName, skinName);
...
retryModel:
  if (badModel) {
    if (modelName && modelName[0]) {
      Com_Printf("WARNING: Attempted to load an unsupported multiplayer model "
                 "%s! (bad or missing bone, or missing animation sequence)\n",
                 modelName);
    }
  ...
  if (cgs.gametype >= GT_TEAM && !cgs.jediVmerc &&
      cgs.gametype != GT_SIEGE) { // We won't force colors for siege.
    BG_ValidateSkinForTeam(ci->modelName, ci->skinName, ci->team,
                           ci->colorOverride);
  ...
  if (strchr(skinName, '|') && strstr(skinName, "head") &&
      strstr(skinName, "torso") &&
      strstr(skinName, "lower")) { // three part skin
  ...
  if (checkSkin) {
    ...
    ci->torsoSkin = trap->R_RegisterSkin(
        va("models/players/%s/model_default.skin", modelName, skinName));
  }
  Com_sprintf(afilename, sizeof(afilename), "models/players/%s/model.glm",
              modelName);
  handle = trap->G2API_InitGhoul2Model(&ci->ghoul2Model, afilename, 0,
                                       ci->torsoSkin, 0, 0, 0);
  ...
  if (!BGPAFtextLoaded) {
    ...
    else { // Didn't find any slashes, this is a raw filename right in base
           // (whish isn't a good thing)
      return qfalse;
    }
    ...
    if (Q_stricmp(afilename, "models/players/_humanoid/animation.cfg") /*&&
      ...
      Com_Printf("Failed to load animation file "
                 "models/players/_humanoid/animation.cfg\n");
      return qfalse;
    }

    BG_ParseAnimationEvtFile("models/players/_humanoid/", 0,
                             -1); // get the sounds for the humanoid anims
    //		if (cgs.gametype == GT_SIEGE)
    //		{
    //			BG_ParseAnimationEvtFile( "models/players/rockettrooper/", 1, 1 );
    ////parse rockettrooper too
    //		}
    // For the time being, we're going to have all real players use the generic
    // humanoid soundset and that's it. Only npc's will use model-specific
    // soundsets.

    //	BG_ParseAnimationSndFile(va("models/players/%s/", modelName), 0, -1);
  } else if (!bgAllEvents[0]
                  .eventsParsed) { // make sure the player anim sounds are
                                   // loaded even if the anims already are
  ...
  if (CG_ParseSurfsFile(modelName, skinName, surfOff,
                        surfOn)) { // turn on/off any surfs
    const char *token;
    const char *p;
    ...
    if (surfOff[0]) {
      ...
      while (1) {
        ...
        // turn off this surf
        trap->G2API_SetSurfaceOnOff(ci->ghoul2Model, token,
  ...
  if (!trap->G2API_SetBoneAnim(ci->ghoul2Model, 0, "model_root", 0, 12,
                               BONE_ANIM_OVERRIDE_LOOP, 1.0f, cg.time, -1,
                               -1)) {
    badModel = qtrue;
  }

  if (!trap->G2API_SetBoneAngles(ci->ghoul2Model, 0, "upper_lumbar", tempVec,
                                 BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                 NEGATIVE_Z, NULL, 0, cg.time)) {
    badModel = qtrue;
  }

  if (!trap->G2API_SetBoneAngles(ci->ghoul2Model, 0, "cranium", tempVec,
  ...
  if (ci->bolt_rhand == -1 || ci->bolt_lhand == -1 || ci->bolt_head == -1 ||
      ci->bolt_motion == -1 || ci->bolt_llumbar == -1) {
    badModel = qtrue;
  }
  ...
  if (!Q_stricmp(modelName,
                 "boba_fett")) { // special case, turn off the jetpack surfs
  ...
  if (clientNum != -1) {
    ...
    trap->G2_HaveWeGhoul2Models(cg_entities[clientNum].ghoul2))
    {
  ...
  ci->modelIcon = trap->R_RegisterShaderNoMip(
      va("models/players/%s/icon_%s", modelName, skinName));
  if (!ci->modelIcon) {
    int i = 0;
    int j;
    char iconName[1024];
    ...
    iconName[j] = 0;
    if (skinName[i] == '|') { // looks like it actually may be a custom model
                              // skin, let's try getting the icon...
      ci->modelIcon = trap->R_RegisterShaderNoMip(
          va("models/players/%s/%s", modelName, iconName));
    }
  }
  return qtrue;
}
...
#define DEFAULT_FEMALE_SOUNDPATH                                               \
  "chars/mp_generic_female/misc" //"chars/tavion/misc"
...
void CG_LoadCISounds(clientInfo_t *ci, qboolean modelloaded) {
  ...
  if (!ci->skinName[0] ||
      !Q_stricmp("default", ci->skinName)) { // try default sounds.cfg first
    fLen = trap->FS_Open(va("models/players/%s/sounds.cfg", dir), &f, FS_READ);
    if (!f) { // no?  Look for _default sounds.cfg
      fLen = trap->FS_Open(va("models/players/%s/sounds_default.cfg", dir), &f,
                           FS_READ);
    }
  } else { // use the .skin associated with this skin
    fLen = trap->FS_Open(
        va("models/players/%s/sounds_%s.cfg", dir, ci->skinName), &f, FS_READ);
    if (!f) { // fall back to default sounds
      fLen =
          trap->FS_Open(va("models/players/%s/sounds.cfg", dir), &f, FS_READ);
    }
  }
  ...
  for (i = 0; i < MAX_CUSTOM_SOUNDS; i++) {
    ...
    if (soundpath[0]) {
      ci->sounds[i] = trap->S_RegisterSound(
          va("sound/chars/%s/misc/%s", soundpath, soundName));
    } else {
      if (modelloaded) {
        ci->sounds[i] =
            trap->S_RegisterSound(va("sound/chars/%s/misc/%s", dir, soundName));
      }
    }

    if (!ci->sounds[i]) { // failed the load, try one out of the generic path
      if (isFemale) {
        ci->sounds[i] = trap->S_RegisterSound(
            va("sound/%s/%s", DEFAULT_FEMALE_SOUNDPATH, soundName));
      } else {
        ci->sounds[i] = trap->S_RegisterSound(
            va("sound/%s/%s", DEFAULT_MALE_SOUNDPATH, soundName));
      }
    }
  }

  if (cgs.gametype >= GT_TEAM ||
      com_buildScript.integer) { // load the siege sounds then
    for (i = 0; i < MAX_CUSTOM_SIEGE_SOUNDS; i++) {
      ...
      if (soundpath[0]) {
        ...
        if (!ci->siegeSounds[i])
          ci->siegeSounds[i] =
              trap->S_RegisterSound(va("sound/%s/%s", soundpath, soundName));
      } else {
        if (modelloaded) {
          ci->siegeSounds[i] = trap->S_RegisterSound(
              va("sound/chars/%s/misc/%s", dir, soundName));
        }
      }

      if (!ci->siegeSounds[i]) { // failed the load, try one out of the generic
                                 // path
        if (isFemale) {
          ci->siegeSounds[i] = trap->S_RegisterSound(
              va("sound/%s/%s", DEFAULT_FEMALE_SOUNDPATH, soundName));
  ...
  if (cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL ||
      com_buildScript.integer) { // load the Duel sounds then
    for (i = 0; i < MAX_CUSTOM_DUEL_SOUNDS; i++) {
      ...
      if (!ci->duelSounds[i]) { // failed the load, try one out of the generic
                                // path
        if (isFemale) {
          ci->duelSounds[i] = trap->S_RegisterSound(
              va("sound/%s/%s", DEFAULT_FEMALE_SOUNDPATH, soundName));
  ...
  */
  ...
  if (cgs.gametype >= GT_TEAM) {
    if (ci->team == TEAM_BLUE) {
      Q_strncpyz(teamname, DEFAULT_BLUETEAM_NAME /*cg_blueTeamName.string*/,
                 sizeof(teamname));
    } else {
      Q_strncpyz(teamname, DEFAULT_REDTEAM_NAME /*cg_redTeamName.string*/,
                 sizeof(teamname));
    }
  }
  if (teamname[0]) {
  ...
  if (cgs.gametype == GT_SIEGE &&
      (ci->team == TEAM_SPECTATOR ||
       ci->siegeIndex ==
           -1)) { // yeah.. kind of a hack I guess. Don't care until they are
                  // actually ingame with a valid class.
    if (!CG_RegisterClientModelname(ci, fallbackModel, "default", teamname,
                                    -1)) {
      trap->Error(ERR_DROP, "DEFAULT_MODEL (%s) failed to register",
                  fallbackModel);
    }
  } else {
    if (!CG_RegisterClientModelname(ci, ci->modelName, ci->skinName, teamname,
                                    clientNum)) {
      // trap->Error( ERR_DROP, "CG_RegisterClientModelname( %s, %s, %s, %s %s )
      // failed", ci->modelName, ci->skinName, ci->headModelName,
      // ci->headSkinName, teamname ); rww - DO NOT error out here! Someone could
      // just type in a nonsense model name and crash everyone's client. Give it
      // a chance to load default model for this client instead.
      ...
      if (cgs.gametype >= GT_TEAM) {
        ...
        if (!CG_RegisterClientModelname(ci, fallbackModel, ci->skinName,
                                        teamname, -1)) {
          trap->Error(ERR_DROP,
                      "DEFAULT_MODEL / skin (%s/%s) failed to register",
                      fallbackModel, ci->skinName);
        }
      } else {
        if (!CG_RegisterClientModelname(ci, fallbackModel, "default", teamname,
                                        -1)) {
  ...
  if (clientNum != -1 && ci->ghoul2Model &&
    ...
    trap->G2API_AttachInstanceToEntNum(cg_entities[clientNum].ghoul2, clientNum,
                                       qfalse);

    if (trap->G2API_AddBolt(cg_entities[clientNum].ghoul2, 0, "face") ==
    ...
    cg_entities[clientNum].localAnimIndex =
        CG_G2SkelForModel(cg_entities[clientNum].ghoul2);
    cg_entities[clientNum].eventAnimIndex = CG_G2EvIndexForModel(
        cg_entities[clientNum].ghoul2, cg_entities[clientNum].localAnimIndex);
  }
  ...
  if (cgs.gametype == GT_SIEGE &&
      (ci->team == TEAM_SPECTATOR ||
       ci->siegeIndex == -1)) { // don't need to load sounds
  } else {
...
// Take care of initializing all the ghoul2 saber stuff based on clientinfo
// data. -rww
static void CG_InitG2SaberData(int saberNum, clientInfo_t *ci) {
  trap->G2API_InitGhoul2Model(&ci->ghoul2Weapons[saberNum],
  ...
  if (ci->ghoul2Weapons[saberNum]) {
    ...
    if (ci->saber[saberNum].skin) {
      trap->G2API_SetSkin(ci->ghoul2Weapons[saberNum], 0,
                          ci->saber[saberNum].skin, ci->saber[saberNum].skin);
    }
    ...
    while (k < ci->saber[saberNum].numBlades) {
      ...
      if (tagBolt == -1) {
        if (k == 0) { // guess this is an 0ldsk3wl saber
          tagBolt =
              trap->G2API_AddBolt(ci->ghoul2Weapons[saberNum], 0, "*flash");
...
static void CG_CopyClientInfoModel(clientInfo_t *from, clientInfo_t *to) {
  ...
  // rww - Trying to use the same ghoul2 pointer for two seperate clients ==
  // DISASTER
  ...
  // Don't do this, I guess. Just leave the saber info in the original, so it
  ...
  while (i < MAX_SABERS)
  {
          if (to->ghoul2Weapons[i] &&
  trap->G2_HaveWeGhoul2Models(to->ghoul2Weapons[i]))
          {
...
static qboolean CG_ScanForExistingClientInfo(clientInfo_t *ci, int clientNum) {
  ...
  for (i = 0; i < cgs.maxclients; i++) {
    ...
    if (!Q_stricmp(ci->modelName, match->modelName) &&
        ...
        ci->siegeIndex == match->siegeIndex && match->ghoul2Model &&
        match->bolt_head) // if the bolts haven't been initialized, this "match"
                          // is useless to us
    {
      ...
      // rww - Filthy hack. If this is actually the info already belonging to
      // us, just reassign the pointer. Switching instances when not necessary
      // produces small animation glitches. Actually, before, were we even
      // freeing the instance attached to the old clientinfo before copying this
      // new clientinfo over it? Could be a nasty leak possibility. (though this
      // should remedy it in theory)
      if (clientNum == i) {
        if (match->ghoul2Model &&
            trap->G2_HaveWeGhoul2Models(
                match->ghoul2Model)) { // The match has a valid instance (if it
                                       // didn't, we'd probably already be fudged
                                       // (^_^) at this state)
          if (ci->ghoul2Model &&
              trap->G2_HaveWeGhoul2Models(
                  ci->ghoul2Model)) { // First kill the copy we have if we have
                                      // one. (but it should be null)
            trap->G2API_CleanGhoul2Models(&ci->ghoul2Model);
          }
          ...
          // We can share this pointer, because it already belongs to this
          // client. The pointer itself and the ghoul2 instance is never actually
          // changed, just passed between clientinfo structures.
          ci->ghoul2Model = match->ghoul2Model;

          // Don't need to do this I guess, whenever this function is called the
          // saber stuff should already be taken care of in the new info.
          /*
          while (k < MAX_SABERS)
          {
                  if (match->ghoul2Weapons[k] && match->ghoul2Weapons[k] !=
ci->ghoul2Weapons[k])
                  {
...
static void CG_SetDeferredClientInfo(clientInfo_t *ci) {
  ...
  for (i = 0; i < cgs.maxclients; i++) {
    ...
    */
    ...
    if (match->deferred) { // no deferring off of deferred info. Because I said
                           // so.
      continue;
    }
...
void CG_NewClientInfo(int clientNum, qboolean entitiesInitialized) {
  ...
  if (!configstring[0]) {
    if (ci->ghoul2Model && trap->G2_HaveWeGhoul2Models(
                               ci->ghoul2Model)) { // clean this stuff up first
      trap->G2API_CleanGhoul2Models(&ci->ghoul2Model);
    }
    k = 0;
    while (k < MAX_SABERS) {
      if (ci->ghoul2Weapons[k] &&
          trap->G2_HaveWeGhoul2Models(ci->ghoul2Weapons[k])) {
        trap->G2API_CleanGhoul2Models(&ci->ghoul2Weapons[k]);
      }
      k++;
    }
  ...
  // players have -1 skill so you can determine the bots from the scoreboard
  // code
  if (v && v[0])
  ...
  if (cgs.gametype == GT_SIEGE) { // entries only sent in siege mode
    // siege desired team
    ...
    if (v) {
      ...
      if (siegeClass) { // See if this class forces a model, if so, then use it.
                        // Same for skin.
        newInfo.siegeIndex = BG_SiegeFindClassIndexByName(v);

        if (siegeClass->forcedModel[0]) {
          Q_strncpyz(newInfo.modelName, siegeClass->forcedModel,
                     sizeof(newInfo.modelName));
        }

        if (siegeClass->forcedSkin[0]) {
          Q_strncpyz(newInfo.skinName, siegeClass->forcedSkin,
    ...
    while (j < MAX_SABERS) {
      if (saberUpdate[j]) {
        if (newInfo.saber[j].model[0]) {
          if (oldG2Weapons[j]) { // free the old instance(s)
  ...
  if (cgs.gametype >= GT_TEAM && !cgs.jediVmerc &&
    ...
    BG_ValidateSkinForTeam(newInfo.modelName, newInfo.skinName, newInfo.team,
                           newInfo.colorOverride);
  } else {
    newInfo.colorOverride[0] = newInfo.colorOverride[1] =
  ...
  if (!CG_ScanForExistingClientInfo(&newInfo, clientNum)) {
    ...
    if (cg.snap &&
        cg.snap->ps.clientNum ==
            clientNum) { // rww - don't defer your own client info ever
      CG_LoadClientInfo(&newInfo);
    } else if (cg_deferPlayers.integer && cgs.gametype != GT_SIEGE &&
               !com_buildScript.integer && !cg.loading) {
  ...
  if (ci->ghoul2Model && ci->ghoul2Model != newInfo.ghoul2Model &&
      trap->G2_HaveWeGhoul2Models(
          ci->ghoul2Model)) { // We must kill this instance before we remove our
                              // only pointer to it from the cgame. Otherwise we
                              // will end up with extra instances all over the
                              // place, I think.
    trap->G2API_CleanGhoul2Models(&ci->ghoul2Model);
  }
  *ci = newInfo;
  ...
  if (clientNum != -1) { // don't want it using an invalid pointer to share
    trap->G2API_ClearAttachedInstance(clientNum);
  }
  ...
  if (entitiesInitialized && ci->ghoul2Model &&
      (oldGhoul2 !=
       ci->ghoul2Model)) { // Copy the new ghoul2 model to the centity.
    animation_t *anim;
    centity_t *cent = &cg_entities[clientNum];

    anim =
        &bgHumanoidAnimations[(cg_entities[clientNum].currentState.legsAnim)];
    ...
    if (anim) {
      ...
      // model change
      trap->G2API_SetBoneAnim(ci->ghoul2Model, 0, "model_root", firstFrame,
                              anim->firstFrame + anim->numFrames, flags,
                              animSpeed, cg.time, setFrame, 150);

      cg_entities[clientNum].currentState.legsAnim = 0;
    }

    anim =
        &bgHumanoidAnimations[(cg_entities[clientNum].currentState.torsoAnim)];
    ...
    if (anim) {
      ...
      // rww - Set the animation again because it just got reset due to the
      // model change
      trap->G2API_SetBoneAnim(ci->ghoul2Model, 0, "lower_lumbar", firstFrame,
                              anim->firstFrame + anim->numFrames, flags,
                              animSpeed, cg.time, setFrame, 150);
    ...
    if (clientNum != -1) {
      // Attach the instance to this entity num so we can make use of
      // client-server shared operations if possible.
      trap->G2API_AttachInstanceToEntNum(cg_entities[clientNum].ghoul2,
                                         clientNum, qfalse);
    }
    ...
    if (cg_entities[clientNum].currentState.number !=
      ...
      if (cg_entities[clientNum].ghoul2 && ci->ghoul2Model) {
        CG_CopyG2WeaponInstance(&cg_entities[clientNum],
                                cg_entities[clientNum].currentState.weapon,
                                cg_entities[clientNum].ghoul2);
        cg_entities[clientNum].ghoul2weapon =
            CG_G2WeaponInstance(&cg_entities[clientNum],
                                cg_entities[clientNum].currentState.weapon);
      }
      if (!cg_entities[clientNum]
               .currentState
               .saberHolstered) { // if not holstered set length and desired
                                  // length for both blades to full right now.
        int j;
...
static void _PlayerFootStep(const vec3_t origin, const float orientation,
                            const float radius, centity_t *const cent,
  ...
  if (soundType < FOOTSTEP_TOTAL) {
    trap->S_StartSound(NULL, cent->currentState.clientNum, CHAN_BODY,
                       cgs.media.footsteps[soundType][rand() & 3]);
  }
  ...
  if (effectID != -1) {
    trap->FX_PlayEffectID(effectID, trace.endpos, trace.plane.normal, -1, -1,
                          qfalse);
  }

  if (cg_footsteps.integer < 4) { // debugging - 4 always does footstep effect
    if (!bMark ||
        cg_footsteps.integer < 3) // 1 for sounds, 2 for effects, 3 for marks
    {
      return;
    }
  }
  ...
  if (trace.plane.normal[0] || trace.plane.normal[1] || trace.plane.normal[2]) {
    CG_ImpactMark(footMarkShader, trace.endpos, trace.plane.normal, orientation,
                  1, 1, 1, 1.0f, qfalse, radius, qfalse);
  }
}
...
static void CG_PlayerFootsteps(centity_t *cent, footstepType_t footStepType) {
  ...
  // FIXME: make this a feature of NPCs in the NPCs.cfg? Specify a footstep
  // shader, if any?
  if (cent->currentState.NPC_class != CLASS_ATST &&
    ...
    case FOOTSTEP_R:
    case FOOTSTEP_HEAVY_R:
      footBolt = trap->G2API_AddBolt(cent->ghoul2, 0,
                                     "*r_leg_foot"); // cent->gent->footRBolt;
      break;
    case FOOTSTEP_L:
    case FOOTSTEP_HEAVY_L:
    default:
      footBolt = trap->G2API_AddBolt(cent->ghoul2, 0,
                                     "*l_leg_foot"); // cent->gent->footLBolt;
      break;
    }

    // FIXME: get yaw orientation of the foot and use on decal
    trap->G2API_GetBoltMatrix(cent->ghoul2, 0, footBolt, &boltMatrix,
                              tempAngles, cent->lerpOrigin, cg.time,
...
void CG_PlayerAnimEventDo(centity_t *cent, animevent_t *animEvent) {
  ...
  case AEV_SOUND: { // are there variations on the sound?
    const int holdSnd =
        animEvent
            ->eventData[AED_SOUNDINDEX_START +
                        Q_irand(0,
                                animEvent->eventData[AED_SOUND_NUMRANDOMSNDS])];
    if (holdSnd > 0) {
  ...
  case AEV_SABER_SWING:
    ...
    if (client && client->infoValid &&
        client->saber[animEvent->eventData[AED_SABER_SWING_SABERNUM]]
      ...
      swingSound = trap->S_RegisterSound(
          va("sound/weapons/saber/saberhup%i.wav", randomSwing));
    }
    trap->S_StartSound(cent->currentState.pos.trBase, cent->currentState.number,
                       CHAN_AUTO, swingSound);
    break;
  case AEV_SABER_SPIN:
    if (cent->currentState.eType == ET_NPC) {
      client = cent->npcClient;
    ...
    if (client && client->infoValid &&
        client->saber[AED_SABER_SPIN_SABERNUM].spinSound) { // use override
      spinSound = client->saber[AED_SABER_SPIN_SABERNUM].spinSound;
    } else {
      switch (animEvent->eventData[AED_SABER_SPIN_TYPE]) {
      case 0: // saberspinoff
        spinSound =
            trap->S_RegisterSound("sound/weapons/saber/saberspinoff.wav");
        break;
      case 1: // saberspin
      ...
      default: // random saberspin1-3
        spinSound = trap->S_RegisterSound(
            va("sound/weapons/saber/saberspin%d.wav", Q_irand(1, 3)));
        break;
      }
    }
    if (spinSound) {
      trap->S_StartSound(NULL, cent->currentState.clientNum, CHAN_AUTO,
                         spinSound);
    }
    break;
  case AEV_FOOTSTEP:
    CG_PlayerFootsteps(cent,
                       (footstepType_t)animEvent->eventData[AED_FOOTSTEP_TYPE]);
    break;
  case AEV_EFFECT:
#if 0 // SP method
...
#else // my method
    if (animEvent->stringData && animEvent->stringData[0] && cent &&
        cent->ghoul2) {
      animEvent->eventData[AED_MODELINDEX] = 0;
      if ((Q_stricmpn("*blade", animEvent->stringData, 6) == 0 ||
           Q_stricmp("*flash", animEvent->stringData) ==
               0)) { // must be a weapon, try weapon 0?
        animEvent->eventData[AED_BOLTINDEX] =
            trap->G2API_AddBolt(cent->ghoul2, 1, animEvent->stringData);
        if (animEvent->eventData[AED_BOLTINDEX] != -1) { // found it!
          ...
          animEvent->eventData[AED_BOLTINDEX] =
              trap->G2API_AddBolt(cent->ghoul2, 0, animEvent->stringData);
        }
      } else {
        animEvent->eventData[AED_BOLTINDEX] =
            trap->G2API_AddBolt(cent->ghoul2, 0, animEvent->stringData);
    ...
    if (animEvent->eventData[AED_BOLTINDEX] != -1) {
      ...
      trap->FX_PlayEffectID(animEvent->eventData[AED_EFFECTINDEX], bPoint,
                            bAngle, -1, -1, qfalse);
    } else {
      vec3_t bAngle;

      VectorSet(bAngle, 0, 1, 0);
      trap->FX_PlayEffectID(animEvent->eventData[AED_EFFECTINDEX],
                            cent->lerpOrigin, bAngle, -1, -1, qfalse);
    }
#endif
    break;
...
case AEV_MOVE:
    ...
    if ( cent && cent->gent && cent->gent->client )
    {
            ...
            {//on something
                    vec3_t	fwd, rt, up, angles = {0,
cent->gent->client->ps.viewangles[YAW], 0}; AngleVectors( angles, fwd, rt, up );
                    //FIXME: set or add to velocity?
                    VectorScale( fwd, animEvent->eventData[AED_MOVE_FWD],
cent->gent->client->ps.velocity ); VectorMA( cent->gent->client->ps.velocity,
animEvent->eventData[AED_MOVE_RT], rt, cent->gent->client->ps.velocity );
                    VectorMA( cent->gent->client->ps.velocity,
animEvent->eventData[AED_MOVE_UP], up, cent->gent->client->ps.velocity );
                    ...
                    {//a jump
                            ...
                            //FIXME: if have force jump, do this?  or specify
sound in the event data?
                            //cent->gent->client->ps.forceJumpZStart =
cent->gent->client->ps.origin[2];//so we don't take damage if we land at same
height
                            //G_SoundOnEnt( cent->gent, CHAN_BODY,
...
/*
void CG_PlayerAnimEvents( int animFileIndex, int eventFileIndex, qboolean torso,
int oldFrame, int frame, const vec3_t org, int entNum )

play any keyframed sounds - only when start a new frame
This func is called once for legs and once for torso
*/
void CG_PlayerAnimEvents(int animFileIndex, int eventFileIndex, qboolean torso,
                         int oldFrame, int frame, int entNum) {
  int i;
  int firstFrame = 0, lastFrame = 0;
  qboolean doEvent = qfalse, inSameAnim = qfalse, loopAnim = qfalse,
           match = qfalse, animBackward = qfalse;
  ...
  if (fabs((float)(oldFrame - frame)) >
      1) { // given a range, see if keyFrame falls in that range
    int oldAnim, anim;
    if (torso) {
      /*
    ...
    if (anim != oldAnim) { // not in same anim
      inSameAnim = qfalse;
      // FIXME: we *could* see if the oldFrame was *just about* to play the
      // keyframed sound...
  ...
  for (i = 0; i < MAX_ANIM_EVENTS; ++i) {
    ...
    if (animEvents[i].keyFrame == frame) { // exact match
      match = qtrue;
    } else if (fabs((float)(oldFrame - frame)) >
               1 /*&& cg_reliableAnimSounds.integer*/) { // given a range, see
                                                         // if keyFrame falls in
                                                         // that range
      if (inSameAnim) { // if changed anims altogether, sorry, the sound is lost
        if (fabs((float)(oldFrame - animEvents[i].keyFrame)) <= 3 ||
            fabs((float)(frame - animEvents[i].keyFrame)) <=
                3) {          // must be at least close to the keyframe
          if (animBackward) { // animation plays backwards
            if (oldFrame > animEvents[i].keyFrame &&
                frame < animEvents[i]
                            .keyFrame) { // old to new passed through keyframe
              match = qtrue;
            } else if (loopAnim) { // hmm, didn't pass through it linearally,
                                   // see if we looped
              if (animEvents[i].keyFrame >= firstFrame &&
                  animEvents[i].keyFrame <
                      lastFrame) { // keyframe is in this anim
                if (oldFrame > animEvents[i].keyFrame &&
                    frame > oldFrame) { // old to new passed through keyframe
                  match = qtrue;
                }
              }
            }
    ...
    if (match) {
      ...
      case AEV_SOUNDCHAN:
        ...
        {
          doEvent = qtrue;
        } else if (animEvents[i].eventData[AED_SOUND_PROBABILITY] >
                   Q_irand(0, 99)) {
          doEvent = qtrue;
        }
        break;
      case AEV_SABER_SWING:
        ...
        {
          doEvent = qtrue;
        } else if (animEvents[i].eventData[AED_SABER_SWING_PROBABILITY] >
                   Q_irand(0, 99)) {
          doEvent = qtrue;
        }
        break;
      case AEV_SABER_SPIN:
        ...
        {
          doEvent = qtrue;
        } else if (animEvents[i].eventData[AED_SABER_SPIN_PROBABILITY] >
                   Q_irand(0, 99)) {
          doEvent = qtrue;
        }
        break;
      case AEV_FOOTSTEP:
        ...
        {
          doEvent = qtrue;
        } else if (animEvents[i].eventData[AED_FOOTSTEP_PROBABILITY] >
                   Q_irand(0, 99)) {
          doEvent = qtrue;
        }
        break;
      case AEV_EFFECT:
        ...
        {
          doEvent = qtrue;
        } else if (animEvents[i].eventData[AED_EFFECT_PROBABILITY] >
                   Q_irand(0, 99)) {
          doEvent = qtrue;
        }
        break;
      case AEV_FIRE:
        ...
        {
          doEvent = qtrue;
        } else if (animEvents[i].eventData[AED_FIRE_PROBABILITY] >
                   Q_irand(0, 99)) {
          doEvent = qtrue;
        }
        break;
...
void CG_TriggerAnimSounds(
    centity_t *cent) { // this also sets the lerp frames, so I suggest you keep
                       // calling it regardless of if you want anim sounds.
  int curFrame = 0;
  ...
  if (trap->G2API_GetBoneFrame(cent->ghoul2, "model_root", cg.time,
                               &currentFrame, cgs.gameModels, 0)) {
    // the above may have failed, not sure what to do about it, current frame
    // will be zero in that case
    curFrame = floor(currentFrame);
  }
  if (curFrame != cent->pe.legs.frame) {
    CG_PlayerAnimEvents(cent->localAnimIndex, sFileIndex, qfalse,
  ...
  if (trap->G2API_GetBoneFrame(cent->ghoul2, "lower_lumbar", cg.time,
                               &currentFrame, cgs.gameModels, 0)) {
    curFrame = floor(currentFrame);
  }
  if (curFrame != cent->pe.torso.frame) {
    CG_PlayerAnimEvents(cent->localAnimIndex, sFileIndex, qtrue,
...
static qboolean CG_FirstAnimFrame(lerpFrame_t *lf, qboolean torsoOnly,
                                  float speedScale);
...
static void CG_SetLerpFrameAnimation(centity_t *cent, clientInfo_t *ci,
                                     ...
                                     float animSpeedMult, qboolean torsoOnly,
                                     qboolean flipState) {
  animation_t *anim;
  float animSpeed;
  int flags = BONE_ANIM_OVERRIDE_FREEZE;
  ...
  if (cent->localAnimIndex > 0) { // rockettroopers can't have broken arms, nor
                                  // can anything else but humanoids
  ...
  if (cent->localAnimIndex > 1 && anim->firstFrame == 0 &&
      anim->numFrames == 0) { // We'll allow this for non-humanoids.
    return;
  }

  if (cg_debugAnim.integer &&
      (cg_debugAnim.integer < 0 ||
       cg_debugAnim.integer == cent->currentState.clientNum)) {
    if (lf == &cent->pe.legs) {
      trap->Print("%d: %d TORSO Anim: %i, '%s'\n", cg.time,
                  cent->currentState.clientNum, newAnimation,
  ...
  if (cent->ghoul2) {
    ...
    BG_SaberStartTransAnim(cent->currentState.number,
                           cent->currentState.fireflag,
    ...
    if (torsoOnly) {
      if (lf->animationTorsoSpeed != animSpeedMult && newAnimation == oldAnim &&
          flipState ==
              lf->lastFlip) { // same animation, but changing speed, so we will
                              // want to resume off the frame we're on.
        resumeFrame = qtrue;
      }
    ...
    if (cent->currentState.NPC_class == CLASS_VEHICLE) {
      trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", firstFrame,
                              lastFrame, flags, animSpeed, cg.time, beginFrame,
                              blendTime);
      return;
    }

    if (torsoOnly &&
        !cent->noLumbar) { // rww - The guesswork based on the lerp frame
                           // figures is usually BS, so I've resorted to a call
                           // to get the frame of the bone directly.
      float GBAcFrame = 0;
      if (resumeFrame) { // we already checked, and this is the same anim, same
                         // flip state, but different speed, so we want to resume
                         // with the new speed off of the same frame.
        trap->G2API_GetBoneFrame(cent->ghoul2, "lower_lumbar", cg.time,
                                 &GBAcFrame, NULL, 0);
        beginFrame = GBAcFrame;
      }

      // even if resuming, also be sure to check if we are running the same
      // frame on the legs. If so, we want to use their frame no matter what.
      trap->G2API_GetBoneFrame(cent->ghoul2, "model_root", cg.time, &GBAcFrame,
                               NULL, 0);

      if ((cent->currentState.torsoAnim) == (cent->currentState.legsAnim) &&
          GBAcFrame >= anim->firstFrame &&
          GBAcFrame <=
              (anim->firstFrame +
               anim->numFrames)) { // if the legs are already running this anim,
                                   // pick up on the exact same frame to avoid
                                   // the "wobbly spine" problem.
        beginFrame = GBAcFrame;
      }

      if (firstFrame > lastFrame ||
          ci->torsoAnim == newAnimation) { // don't resume on backwards playing
                                           // animations.. I guess.
        beginFrame = -1;
      }

      trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lower_lumbar", firstFrame,
                              lastFrame, flags, animSpeed, cg.time, beginFrame,
                              blendTime);
      ...
      if (resumeFrame) { // we already checked, and this is the same anim, same
                         ...
                         // with the new speed off of the same frame.
        float GBAcFrame = 0;
        trap->G2API_GetBoneFrame(cent->ghoul2, "model_root", cg.time,
                                 &GBAcFrame, NULL, 0);
        beginFrame = GBAcFrame;
      }

      if ((beginFrame < firstFrame) ||
          (beginFrame > lastFrame)) { // out of range, don't use it then.
        beginFrame = -1;
      }
      ...
      if (cent->currentState.torsoAnim == cent->currentState.legsAnim &&
          (ci->legsAnim != newAnimation ||
           oldSpeed !=
               animSpeed)) { // alright, we are starting an anim on the legs,
                             // and that same anim is already playing on the
                             // toro, so pick up the frame.
        float GBAcFrame = 0;
    ...
    if (cent->localAnimIndex <= 1 &&
        ...
        !cent->noLumbar) { // make sure we're humanoid before we access the
                           // motion bone
      trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Motion", firstFrame, lastFrame,
                              flags, animSpeed, cg.time, beginFrame, blendTime);
    }
...
===============
*/
static void CG_RunLerpFrame(centity_t *cent, clientInfo_t *ci, lerpFrame_t *lf,
                            qboolean flipState, int newAnimation,
  ...
  if (cent->currentState.forceFrame) {
    if (lf->lastForcedFrame != cent->currentState.forceFrame) {
      ...
      trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lower_lumbar",
                              cent->currentState.forceFrame,
                              cent->currentState.forceFrame + 1, flags,
    ...
    if ((newAnimation != lf->animationNumber ||
         ...
         lf->lastFlip != flipState || !lf->animation) ||
        (CG_FirstAnimFrame(lf, torsoOnly, speedScale))) {
      CG_SetLerpFrameAnimation(cent, ci, lf, newAnimation, speedScale,
                               torsoOnly, flipState);
    }
  }
  ...
  if (lf->frameTime) { // calculate current lerp value
    if (lf->frameTime == lf->oldFrameTime)
      lf->backlerp = 0.0f;
    else
      lf->backlerp = 1.0f - (float)(cg.time - lf->oldFrameTime) /
...
static void CG_ClearLerpFrame(centity_t *cent, clientInfo_t *ci,
  ...
  if (lf->animation->frameLerp < 0) { // Plays backwards
    lf->oldFrame = lf->frame =
        (lf->animation->firstFrame + lf->animation->numFrames);
  } else {
...
static void CG_PlayerAnimation(centity_t *cent, int *legsOld, int *legs,
  ...
  if (!PM_RunningAnim(cent->currentState.legsAnim) &&
      !PM_WalkingAnim(
          cent->currentState
              .legsAnim)) { // if legs are not in a walking/running anim then
  ...
  CG_RunLerpFrame(cent, ci, &cent->pe.legs, cent->currentState.legsFlip,
                  cent->currentState.legsAnim, speedScale, qfalse);

  if (!(cent->currentState.forcePowersActive &
        (1 << FP_RAGE))) { // don't affect torso anim speed unless raged
    speedScale = 1.0f;
  } else {
    speedScale = 1.7f;
  }
  ...
  *legsBackLerp = cent->pe.legs.backlerp;
  ...
  // If this is not a vehicle, you may lerm the frame (since vehicles never have
  // a torso anim). -AReis
  if (cent->currentState.NPC_class != CLASS_VEHICLE) {
    CG_RunLerpFrame(cent, ci, &cent->pe.torso, cent->currentState.torsoFlip,
                    cent->currentState.torsoAnim, speedScale, qtrue);
...
void CG_G2SetBoneAngles(
    void *ghoul2, int modelIndex, const char *boneName, const vec3_t angles,
    const int flags, const int up, const int right, const int forward,
    qhandle_t *modelList, int blendTime,
    int currentTime) { // we want to hold off on setting the bone angles until
                       // the end of the frame, because every time we set them
                       // the entire skeleton has to be reconstructed.
#if 0
...
#endif We don't
                       // want to go with the delayed approach, we want out bolt
                       // points and everything to be updated in realtime.
  // We'll just take the reconstructs and live with them.
  trap->G2API_SetBoneAngles(ghoul2, modelIndex, boneName, angles, flags, up,
                            right, forward, modelList, blendTime, currentTime);
}
...
Variant on CG_Trace. Doesn't trace for ents because ragdoll engine trace code
has no entity trace access. Maybe correct this sometime, so bmodel col. at least
works with ragdoll. But I don't want to slow it down..
================
*/
void CG_Rag_Trace(trace_t *result, const vec3_t start, const vec3_t mins,
                  const vec3_t maxs, const vec3_t end, int skipNumber,
                  int mask) {
  trap->CM_Trace(result, start, end, mins, maxs, 0, mask, 0);
  result->entityNum =
      result->fraction != 1.0 ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
}
...
void CG_TempTestFunction(centity_t *cent, vec3_t forcedAngles) {
  ...
  trap->G2API_GetBoltMatrix(cent->ghoul2, 1, 0, &boltMatrix, tAngles,
...
static int CG_RagAnimForPositioning(centity_t *cent) {
  ...
  trap->G2API_GetBoltMatrix(cent->ghoul2, 0, bolt, &matrix, cent->turAngles,
                            cent->lerpOrigin, cg.time, cgs.gameModels,
                            cent->modelScale);
  BG_GiveMeVectorFromMatrix(&matrix, NEGATIVE_Z, dir);

  if (dir[2] > 0.0f) { // facing up
...
qboolean CG_RagDoll(centity_t *cent, vec3_t forcedAngles) {
  ...
  if (cent->localAnimIndex) { // don't rag non-humanoids
    return qfalse;
  }

  VectorCopy(cent->lerpOrigin, usedOrg);

  if (!cent->isRagging) { // If we're not in a ragdoll state, perform the
                          // checks.
    if (cent->currentState.eFlags &
        EF_RAG) { // want to go into it no matter what then
      inSomething = qtrue;
    ...
    if (cent->currentState.eType ==
        ET_BODY) { // just rag bodies immediately if their own was ragging on
                   // respawn
    ...
    if (!inSomething) {
      ...
      int dur =
          (bgAllAnims[cent->localAnimIndex].anims[anim].numFrames - 1) *
          fabs((float)(bgAllAnims[cent->localAnimIndex].anims[anim].frameLerp));
      int i = 0;
      int boltChecks[5];
      vec3_t boltPoints[5];
      ...
      VectorSet(tAng, cent->turAngles[PITCH], cent->turAngles[YAW],
                cent->turAngles[ROLL]);

      if (cent->pe.legs.animationTime > 50 &&
          (cg.time - cent->pe.legs.animationTime) >
              dur) { // Looks like the death anim is done playing
        deathDone = qtrue;
      }

      if (deathDone) { // only trace from the hands if the death anim is already
                       // done.
      ...
      // This may seem bad, but since we have a bone cache now it should manage
      // to not be too disgustingly slow. Do the head first, because the hands
      // reference it anyway.
      trap->G2API_GetBoltMatrix(cent->ghoul2, 0, boltChecks[2], &boltMatrix,
                                tAng, cent->lerpOrigin, cg.time, cgs.gameModels,
                                cent->modelScale);
      BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, boltPoints[2]);

      while (i < 5) {
        if (i < 2) { // when doing hands, trace to the head instead of origin
          trap->G2API_GetBoltMatrix(cent->ghoul2, 0, boltChecks[i], &boltMatrix,
                                    tAng, cent->lerpOrigin, cg.time,
                                    cgs.gameModels, cent->modelScale);
          ...
          if (i > 2) { // 2 is the head, which already has the bolt point.
            trap->G2API_GetBoltMatrix(
                cent->ghoul2, 0, boltChecks[i], &boltMatrix, tAng,
                cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
            BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, boltPoints[i]);
          }
          VectorCopy(boltPoints[i], trStart);
        ...
        // desire.
        CG_Rag_Trace(&tr, trStart, NULL, NULL, trEnd, cent->currentState.number,
                     MASK_SOLID);
  ...
  if (cent->isRagging) { // We're in a ragdoll state, so make the call to keep
                         // our positions updated and whatnot.
    sharedRagDollParams_t tParms;
    sharedRagDollUpdateParams_t tuParms;

    ragAnim = CG_RagAnimForPositioning(cent);

    if (cent->ikStatus) { // ik must be reset before ragdoll is started, or
    ...
    tParms.startFrame =
        bgAllAnims[cent->localAnimIndex]
            .anims[ragAnim]
            .firstFrame; // +
                         // bgAllAnims[cent->localAnimIndex].anims[ragAnim].numFrames;
    tParms.endFrame =
        bgAllAnims[cent->localAnimIndex].anims[ragAnim].firstFrame +
        bgAllAnims[cent->localAnimIndex].anims[ragAnim].numFrames;
#if 0
		{
			float animSpeed = 0;
...
#elif 1 // with my new method of doing things I want it to continue the anim
    {
      ...
      if (trap->G2API_GetBoneAnim(cent->ghoul2, "model_root", cg.time,
                                  &currentFrame, &startFrame, &endFrame, &flags,
                                  &animSpeed, cgs.gameModels,
                                  0)) { // lock the anim on the current frame.
        int blendTime = 500;
        animation_t *curAnim = &bgAllAnims[cent->localAnimIndex]
                                    .anims[cent->currentState.legsAnim];

        if (currentFrame >=
            (curAnim->firstFrame + curAnim->numFrames -
             1)) { // this is sort of silly but it works for now.
          currentFrame = (curAnim->firstFrame + curAnim->numFrames - 2);
        }

        trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lower_lumbar", currentFrame,
                                currentFrame + 1, flags, animSpeed, cg.time,
                                currentFrame, blendTime);
        trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", currentFrame,
                                currentFrame + 1, flags, animSpeed, cg.time,
                                currentFrame, blendTime);
        trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Motion", currentFrame,
                                currentFrame + 1, flags, animSpeed, cg.time,
                                currentFrame, blendTime);
      }
    }
    ...
    cent->turAngles[YAW] = cent->lerpAngles[YAW] = cent->pe.torso.yawAngle =
    ...
    if (cent->currentState.ragAttach &&
      ...
      if (cent->currentState.ragAttach ==
          ENTITYNUM_NONE) { // switch cl 0 and entitynum_none, so we can operate
                            // on the "if non-0" concept
      ...
      if (grabEnt->ghoul2) {
        ...
        trap->G2API_GetBoltMatrix(grabEnt->ghoul2, 0, 0, &matrix,
                                  grabEnt->turAngles, grabEnt->lerpOrigin,
                                  cg.time, cgs.gameModels, grabEnt->modelScale);
        BG_GiveMeVectorFromMatrix(&matrix, ORIGIN, bOrg);

        // Get our hand's location
        trap->G2API_GetBoltMatrix(cent->ghoul2, 0, 0, &matrix, cent->turAngles,
                                  cent->lerpOrigin, cg.time, cgs.gameModels,
                                  cent->modelScale);
        ...
        trap->G2API_GetBoltMatrix(cent->ghoul2, 0, thorBolt, &matrix,
        ...
        if (cent->ragLastOriginTime >= cg.time &&
          ...
          if (difLen > 0.0f) { // if we're being dragged, then kick all the
            ...
            while (cg_effectorStringTable[i]) {
              ...
              VectorSet(rVel, flrand(-0.1f, 0.1f), flrand(-0.1f, 0.1f),
                        flrand(0.1f, 0.5));
              ...
              trap->G2API_RagEffectorKick(cent->ghoul2,
                                          cg_effectorStringTable[i], dVel);
    ...
    } else if (cent->overridingBones) { // reset things to their normal rag
                                        // state
      vec3_t pcjMin, pcjMax;
      vec3_t dVel;
...
void CG_G2ServerBoneAngles(centity_t *cent) {
  ...
  while (i < 4) { // cycle through the 4 bone index values on the entstate
    if (bone) {   // if it's non-0 then it could have something in it.
      const char *boneName = CG_ConfigString(CS_G2BONES + bone);

      if (boneName &&
          boneName[0]) { // got the bone, now set the angles from the
                         // corresponding entitystate boneangles value.
        ...
        trap->G2API_SetBoneAngles(cent->ghoul2, 0, boneName, boneAngles, flags,
                                  up, right, forward, cgs.gameModels, 100,
                                  cg.time);
      }
    }
...
static void CG_G2SetHeadAnim(centity_t *cent, int anim) {
  ...
  //	int startFrame, endFrame;
  //	const qboolean animatingHead =
  //gi.G2API_GetAnimRangeIndex(&gent->ghoul2[gent->playerModel],
  //cent->gent->faceBone, &startFrame, &endFrame);
  ...
  //only set the anim if we aren't going to do the same animation again
  {
    ...
    //cent->gent->faceBone, 		firstFrame, lastFrame, animFlags, animSpeed,
    //cg.time, -1, blendTime);
    trap->G2API_SetBoneAnim(cent->ghoul2, 0, "face", firstFrame, lastFrame,
                            animFlags, animSpeed, cg.time, -1, blendTime);
  }
}
...
qboolean CG_G2PlayerHeadAnims(centity_t *cent) {
  ...
  if (cent->localAnimIndex > 1) { // only do this for humanoids
    return qfalse;
  }

  if (cent->noFace) { // i don't have a face
    return qfalse;
  }
  ...
  if (cent->currentState.eFlags &
      EF_DEAD) { // Dead people close their eyes and don't make faces!
    anim = FACE_DEAD;
    ci->facial_blink = -1;
  } else {
    if (!ci->facial_blink) { // set the timers
    ...
    if (ci->facial_blink < 0) { // yes, check if we are we done blinking ?
      if (-(ci->facial_blink) < cg.time) { // yes, so reset blink timer
        ci->facial_blink = cg.time + flrand(4000.0, 8000.0);
    ...
    } else // no we aren't blinking
    {
      if (ci->facial_blink < cg.time) // but should we start ?
      {
        ...
        if (ci->facial_blink ==
            1) { // requested to stay shut by SET_FACEEYESCLOSED
    ...
    {                            // not talking
      ...
      {                          // yes
        ...
        } else { // not yet, so choose aux
          anim = FACE_ALERT;
        }
      } else                            // no we aren't auxing
      {                                 // but should we start ?
        if (ci->facial_aux < cg.time) { // yes
          anim = FACE_ALERT;
          // set aux timer
      ...
      if (anim !=
          -1) // we we are auxing, see if we should override with a frown
      {
...
static void CG_G2PlayerAngles(centity_t *cent, matrix3_t legs,
                              vec3_t legsAngles) {
  clientInfo_t *ci;
  ...
  if ((cent->currentState.eFlags & EF_DEAD) ||
    ...
    if (CG_RagDoll(
            cent,
            forcedAngles)) { // if we managed to go into the rag state, give our
                             // ent axis the forced angles and return.
  ...
  if (cent->localAnimIndex <= 1) { // don't do these things on non-humanoids
    vec3_t lookAngles;
    entityState_t *emplaced = NULL;

    if (cent->currentState.hasLookTarget) {
      VectorSubtract(cg_entities[cent->currentState.lookTarget].lerpOrigin,
                     cent->lerpOrigin, lookAngles);
    ...
    BG_G2PlayerAngles(cent->ghoul2, ci->bolt_motion, &cent->currentState,
                      cg.time, cent->lerpOrigin, cent->lerpAngles, legs,
                      legsAngles, &cent->pe.torso.yawing,
                      &cent->pe.torso.pitching, &cent->pe.legs.yawing,
                      &cent->pe.torso.yawAngle, &cent->pe.torso.pitchAngle,
                      &cent->pe.legs.yawAngle, cg.frametime, cent->turAngles,
                      cent->modelScale, ci->legsAnim, ci->torsoAnim,
                      &ci->corrTime, lookAngles, ci->lastHeadAngles,
                      ci->lookTime, emplaced, &ci->superSmoothTime);
    ...
    if (cent->currentState.heldByClient &&
      ...
      if (other && other->ghoul2 && ci->bolt_lhand) {
        ...
        trap->G2API_GetBoltMatrix(other->ghoul2, 0, ci->bolt_lhand, &boltMatrix,
                                  other->turAngles, other->lerpOrigin, cg.time,
                                  cgs.gameModels, other->modelScale);
        BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, boltOrg);

        BG_IK_MoveArm(cent->ghoul2, ci->bolt_lhand, cg.time,
                      &cent->currentState,
                      cent->currentState.torsoAnim /*BOTH_DEAD1*/, boltOrg,
                      &cent->ikStatus, cent->lerpOrigin, cent->lerpAngles,
                      cent->modelScale, 500, qfalse);
      }
    } else if (cent->ikStatus) { // make sure we aren't IKing if we don't have
                                 // anyone to hold onto us.
      BG_IK_MoveArm(cent->ghoul2, ci->bolt_lhand, cg.time, &cent->currentState,
                    cent->currentState.torsoAnim /*BOTH_DEAD1*/, vec3_origin,
                    &cent->ikStatus, cent->lerpOrigin, cent->lerpAngles,
                    cent->modelScale, 500, qtrue);
    }
  } else if (cent->m_pVehicle &&
             cent->m_pVehicle->m_pVehicleInfo->type == VH_WALKER) {
    vec3_t lookAngles;
    ...
    if (cent->currentState.eType == ET_NPC &&
        cent->currentState.NPC_class == CLASS_VEHICLE && cent->m_pVehicle &&
        cent->m_pVehicle->m_pVehicleInfo->type ==
      ...
      AnglesToAxis(legsAngles, legs);
               ...
               cent->currentState.NPC_class !=
                   CLASS_VEHICLE) { // an NPC bolted to a vehicle should use the
                                    // full angles
      VectorCopy(cent->lerpAngles, legsAngles);
      AnglesToAxis(legsAngles, legs);
    } else {
...
static void CG_PlayerFlag(centity_t *cent, qhandle_t hModel) {
  ...
  trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_llumbar, &boltMatrix,
                            tAng, cent->lerpOrigin, cg.time, cgs.gameModels,
                            cent->modelScale);
  BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, boltOrg);
  ...
  /*
  if (cent->currentState.number == cg.snap->ps.clientNum)
  { //If we're the current client (in third person), render the flag on our back
  transparently ent.renderfx |= RF_FORCE_ENT_ALPHA; ent.shaderRGBA[3] = 100;
  }
  */
...
static void CG_PlayerPowerups(centity_t *cent, refEntity_t *torso) {
  ...
  if (powerups & (1 << PW_QUAD))
    trap->R_AddLightToScene(cent->lerpOrigin, 200 + (rand() & 31), 0.2f, 0.2f,
                            1);
#endif // BASE_COMPAT
  ...
  if (powerups & (1 << PW_REDFLAG)) {
    ...
    trap->R_AddLightToScene(cent->lerpOrigin, 200 + (rand() & 31), 1.0, 0.2f,
                            0.2f);
  }
  ...
  if (powerups & (1 << PW_NEUTRALFLAG)) {
    trap->R_AddLightToScene(cent->lerpOrigin, 200 + (rand() & 31), 1.0, 1.0,
                            1.0);
  }
...
static void CG_PlayerSprites(centity_t *cent) {
  ...
  if (cg.snap && CG_IsMindTricked(cent->currentState.trickedentindex,
                                  ...
                                  cg.snap->ps.clientNum)) {
    return; // this entity is mind-tricking the current client, so don't render
            // it
  }
  ...
  } else if (cent->currentState.eType !=
                 ET_NPC && // don't draw talk balloons on NPCs
...
static qboolean CG_PlayerShadow(centity_t *cent, float *shadowPlane) {
  ...
  *shadowPlane = 0;
  ...
  if (CG_IsMindTricked(cent->currentState.trickedentindex,
                       ...
                       cent->currentState.trickedentindex4,
                       cg.snap->ps.clientNum)) {
    return qfalse; // this entity is mind-tricking the current client, so don't
                   // render it
  }

  if (cg_shadows.integer == 1) { // dropshadow
    if (cent->currentState.m_iVehicleNum &&
        cent->currentState.NPC_class !=
            CLASS_VEHICLE) { // riding a vehicle, no dropshadow
      return qfalse;
    }
  }
  // send a trace down from the player to the ground
  VectorCopy(cent->lerpOrigin, end);
  if (cg_shadows.integer == 2) { // stencil
    end[2] -= 4096.0f;

    trap->CM_Trace(&trace, cent->lerpOrigin, end, mins, maxs, 0,
                   MASK_PLAYERSOLID, 0);
  ...
  if (cg_shadows.integer == 2) { // stencil shadows need plane to be on ground
  ...
  CG_ImpactMark(cgs.media.shadowMarkShader, trace.endpos, trace.plane.normal,
                cent->pe.legs.yawAngle, alpha, alpha, alpha, 1, qfalse, radius,
                qtrue);
...
static void CG_PlayerSplash(centity_t *cent) {
  ...
  trap->CM_Trace(&trace, start, end, NULL, NULL, 0,
...
static void CG_ForcePushBlur(vec3_t org, centity_t *cent) {
  if (!cent || !cg_renderToTextureFX.integer) {
    ...
    ex->refEntity.customShader =
        trap->R_RegisterShader("gfx/effects/forcePush");
  } else { // superkewl "refraction" (well sort of) effect -rww
    refEntity_t ent;
    vec3_t ang;
    float scale;
    float vLen;
    ...
    if (!cent->bodyFadeTime) { // the duration for the expansion and fade
    ...
    if ((REFRACT_EFFECT_DURATION - tDif) <
        200) { // stop following the hand after a little and stay in a fixed
               // spot
...
static const char *cg_pushBoneNames[] = {"cranium", "lower_lumbar", "rhand",
                                         "lhand",   "ltibia",       "rtibia",
                                         "lradius", "rradius",      NULL};
...
static void CG_ForcePushBodyBlur(centity_t *cent) {
  ...
  if (cent->localAnimIndex > 1) { // Sorry, the humanoid IS IN ANOTHER CASTLE.
    return;
  }
  ...
  for (i = 0; cg_pushBoneNames[i];
       i++) { // go through all the bones we want to put a blur effect on
    ...
    trap->G2API_GetBoltMatrix(cent->ghoul2, 0, bolt, &boltMatrix,
                              cent->turAngles, cent->lerpOrigin, cg.time,
                              cgs.gameModels, cent->modelScale);
...
static void CG_ForceGripEffect(vec3_t org) {
  ...
  */
  ...
  ex->color[2] = 255;
  ex->refEntity.customShader =
      cgs.media.redSaberGlowShader; // trap->R_RegisterShader(
...
void CG_AddRefEntityWithPowerups(refEntity_t *ent, entityState_t *state,
  ...
  if (CG_IsMindTricked(state->trickedentindex, state->trickedentindex2,
                       state->trickedentindex3, state->trickedentindex4,
                       cg.snap->ps.clientNum)) {
    return; // this entity is mind-tricking the current client, so don't render
            // it
  }
...
void CG_DrawPlayerShield(centity_t *cent, vec3_t origin) {
  ...
  alpha = 255.0 * ((cent->damageTime - cg.time) / MIN_SHIELD_TIME) +
          Q_flrand(0.0f, 1.0f) * 16;
  if (alpha > 255)
    alpha = 255;
...
void CG_PlayerHitFX(centity_t *cent) {
  // only do the below fx if the cent in question is...uh...me, and it's first
  // person.
...
static void CG_DoSaberLight(saberInfo_t *saber) {
  ...
  float lengths[MAX_BLADES * 2] = {0}, totallength = 0, numpositions = 0, dist,
                             diameter = 0;
  int i, j;

  // RGB combine all the colors of the sabers you're using into one averaged
  // color!
  if (!saber) {
    return;
  }
  ...
  for (i = 0; i < saber->numBlades; i++) {
    if (saber->blade[i].length >= 0.5f) {
      ...
      VectorMA(saber->blade[i].muzzlePoint, saber->blade[i].length,
               saber->blade[i].muzzleDir, positions[i]);
      if (!numpositions) { // first blade, store middle of that as midpoint
        VectorMA(saber->blade[i].muzzlePoint, saber->blade[i].length * 0.5,
                 saber->blade[i].muzzleDir, mid);
  ...
  if (totallength) { // actually have something to do
    if (numpositions ==
        1) { // only 1 blade, midpoint is already set (halfway between the start
             // and end of that blade), rgb is already set, so it diameter
    } else { // multiple blades, calc averages
      VectorClear(mid);
      VectorClear(rgb);
      // now go through all the data and get the average RGB and middle position
      // and the radius
      ...
      // find the farthest distance between the blade tips, this will be our
      // diameter
      for (i = 0; i < MAX_BLADES * 2; i++) {
        if (lengths[i]) {
    ...
    trap->R_AddLightToScene(mid, diameter + (Q_flrand(0.0f, 1.0f) * 8.0f),
                            rgb[0], rgb[1], rgb[2]);
  }
}

void CG_DoSaber(vec3_t origin, vec3_t dir, float length, float lengthMax,
  ...
  if (doLight) { // always add a light because sabers cast a nice glow before
                 // they slice you in half!!  or something...
    vec3_t rgb = {1, 1, 1};
    CG_RGBForSaberColor(color, rgb);
    trap->R_AddLightToScene(mid,
                            (length * 1.4f) + (Q_flrand(0.0f, 1.0f) * 3.0f),
                            rgb[0], rgb[1], rgb[2]);
  }
  ...
  if (cg_saberTrail.integer == 2 && cg_shadows.integer != 2 &&
      cgs.glconfig.stencilBits >= 4) { // draw the blade as a post-render so it
                                       // doesn't get in the cap...
    rfx |= RF_FORCEPOST;
  }
...
void CG_CreateSaberMarks(vec3_t start, vec3_t end, vec3_t normal) {
  ...
  for (i = 0; i < 3;
       i++) { // stretch a bit more in the direction that we are traveling in...
  ...
  numFragments = trap->R_MarkFragments(
      4, (const float(*)[3])originalPoints, projection, MAX_MARK_POINTS,
  ...
  for (i = 0, mf = markFragments; i < numFragments; i++, mf++) {
    ...
    for (j = 0, v = verts; j < mf->numPoints; j++, v++) {
      ...
      v->st[0] = 0.5 + DotProduct(delta, axis[1]) *
                           (0.05f + Q_flrand(0.0f, 1.0f) * 0.03f);
      v->st[1] = 0.5 + DotProduct(delta, axis[2]) *
                           (0.15f + Q_flrand(0.0f, 1.0f) * 0.05f);
    }
    ...
    if (cg_saberDynamicMarks.integer) {
      ...
      // When using addpoly, having a situation like this tends to cause bad
      // results. (I assume it doesn't like trying to draw a polygon over two
      //planes and extends the vertex out to some odd value)
      ...
      // by moving the start time back, we can hack it to fade out way before
      // the burn does
      mark = CG_AllocMark();
...
qboolean CG_G2TraceCollide(trace_t *tr, vec3_t const mins, vec3_t const maxs,
  ...
  if (g2Hit && g2Hit->ghoul2) {
    ...
    if (com_optvehtrace.integer && g2Hit->currentState.eType == ET_NPC &&
      ...
      trap->G2API_CollisionDetectCache(
          G2Trace, g2Hit->ghoul2, angles, g2Hit->lerpOrigin, cg.time,
          g2Hit->currentState.number, (float *)lastValidStart,
          (float *)lastValidEnd, g2Hit->modelScale, 0, cg_g2TraceLod.integer,
          fRadius);
    } else {
...
void CG_G2SaberEffects(vec3_t start, vec3_t end, centity_t *owner) {
  ...
  while (!doneWithTraces) {
    ...
    CG_Trace(&trace, startTr, NULL, NULL, endTr, owner->currentState.number,
             MASK_PLAYERSOLID);

    if (trace.entityNum < MAX_CLIENTS) { // hit a client..
      CG_G2TraceCollide(&trace, NULL, NULL, startTr, endTr);

      if (trace.entityNum !=
          ENTITYNUM_NONE) { // it succeeded with the ghoul2 trace
        trap->FX_PlayEffectID(cgs.effects.mSaberBloodSparks, trace.endpos,
                              trace.plane.normal, -1, -1, qfalse);
        trap->S_StartSound(
            trace.endpos, trace.entityNum, CHAN_AUTO,
            trap->S_RegisterSound(
                va("sound/weapons/saber/saberhit%i.wav", Q_irand(1, 3))));
      }
    }
...
#define CG_MAX_SABER_COMP_TIME                                                 \
  400 // last registered saber entity hit must match within this many ms for the
      // client effect to take place.
...
void CG_AddGhoul2Mark(int shader, float size, vec3_t start, vec3_t end,
  ...
  if (trap->G2API_GetNumGoreMarks(ghoul2, 0) >=
      cg_ghoul2Marks.integer) { // you've got too many marks already
    return;
  }
  ...
  if (lifeTime)
  {
          goreSkin.fadeOutTime = lifeTime*0.1; //default fade duration is
  relative to lifetime.
  }
  goreSkin.fadeRGB = qtrue; //fade on RGB instead of alpha (this depends on the
  shader really, modify if needed)
  */
  // rwwFIXMEFIXME: fade has sorting issues with other non-fading decals,
...
void CG_SaberCompWork(vec3_t start, vec3_t end, centity_t *owner, int saberNum,
  ...
  if (cg.time ==
      owner->serverSaberHitTime) { // don't want to do it the same frame as the
                                   // server hit, to avoid burst effect
                                   // concentrations every x ms.
    return;
  }
  ...
  while (!doneWithTraces) {
    ...
    if (trace.entityNum ==
        owner->serverSaberHitIndex) { // this is the guy the server says we last
                                      // hit, so continue.
      if (cg_entities[trace.entityNum]
              .ghoul2) { // If it has a g2 instance, do the proper ghoul2 checks
        CG_G2TraceCollide(&trace, NULL, NULL, startTr, endTr);
          ...
          if (cg_ghoul2Marks.integer) {
            ...
            if (trEnt->ghoul2) {
              if (trEnt->currentState.eType != ET_NPC ||
                  ...
                  !trEnt->m_pVehicle ||
                  trEnt->m_pVehicle->m_pVehicleInfo->type !=
                      VH_FIGHTER) { // don't do on fighters cause they have
                                    // crazy full axial angles
                int weaponMarkShader = 0,
                    markShader = cgs.media.bdecal_saberglow;
                ...
                if (client && client->infoValid) {
                  if (WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum],
                                                       bladeNum)) {
                    if (client->saber[saberNum]
                            .g2MarksShader2) { // we have a shader to use
                                               // instead of the standard mark
                                               // shader
                      markShader = client->saber[saberNum].g2MarksShader2;
                    }
                    if (client->saber[saberNum]
                            .g2WeaponMarkShader2) { // we have a shader to use
                                                    // as a splashback onto the
                                                    // weapon model
                      weaponMarkShader =
                          client->saber[saberNum].g2WeaponMarkShader2;
                    }
                  } else {
                    if (client->saber[saberNum]
                            .g2MarksShader) { // we have a shader to use instead
                                              // of the standard mark shader
                      markShader = client->saber[saberNum].g2MarksShader;
                    }
                    if (client->saber[saberNum]
                            .g2WeaponMarkShader) { // we have a shader to use as
                                                   // a splashback onto the
                                                   // weapon model
                      weaponMarkShader =
                          client->saber[saberNum].g2WeaponMarkShader;
                    }
                  }
                }
                CG_AddGhoul2Mark(markShader, flrand(3.0f, 4.0f), trace.endpos,
                                 ePos, trace.entityNum, trEnt->lerpOrigin,
                                 trEnt->lerpAngles[YAW], trEnt->ghoul2,
                                 trEnt->modelScale, Q_irand(5000, 10000));
                if (weaponMarkShader) {
                  vec3_t splashBackDir;
                  VectorScale(ePos, -1, splashBackDir);
                  CG_AddGhoul2Mark(
                      weaponMarkShader, flrand(0.5f, 2.0f), trace.endpos,
                      splashBackDir, owner->currentState.clientNum,
                      owner->lerpOrigin, owner->lerpAngles[YAW], owner->ghoul2,
                      owner->modelScale, Q_irand(5000, 10000));
                }
              }
            }
          }
        }
      } else { // otherwise, we're all set.
        doEffect = qtrue;
      }
      ...
      if (doEffect) {
        ...
        if (client && client->infoValid) {
          if (WP_SaberBladeUseSecondBladeStyle(
                  &client->saber[saberNum],
                  bladeNum)) { // use second blade style values
            if (client->saber[saberNum].hitPersonEffect2) {
              hitPersonFxID = client->saber[saberNum].hitPersonEffect2;
            }
            if (client->saber[saberNum]
                    .hitOtherEffect2) { // custom hit other effect
            ...
            if (client->saber[saberNum]
                    .hitOtherEffect) { // custom hit other effect
              hitOtherFxID = client->saber[saberNum].hitOtherEffect;
            }
          }
        }
        if (!trace.plane.normal[0] && !trace.plane.normal[1] &&
            !trace.plane.normal[2]) { // who cares, just shoot it somewhere.
          trace.plane.normal[1] = 1;
        }

        if (owner->serverSaberFleshImpact) { // do standard player/live ent hit
                                             // sparks
          trap->FX_PlayEffectID(hitPersonFxID, trace.endpos, trace.plane.normal,
                                -1, -1, qfalse);
          // trap->S_StartSound(trace.endpos, trace.entityNum, CHAN_AUTO,
          // trap->S_RegisterSound(va("sound/weapons/saber/saberhit%i.wav",
          // Q_irand(1, 3))));
        } else { // do the cut effect
          trap->FX_PlayEffectID(hitOtherFxID, trace.endpos, trace.plane.normal,
                                -1, -1, qfalse);
        }
        doEffect = qfalse;
      }
    ...
    */
    doneWithTraces = qtrue; // disabling backwards tr for now, sometimes it just
                            // makes too many effects.
  }
}
...
void CG_AddSaberBlade(centity_t *cent, centity_t *scent, refEntity_t *saber,
  ...
  if (saberLen <= 0 && !dontDraw) { // don't bother then.
    return;
  }
  ...
  // Assume bladeNum is equal to the bolt index because bolts should be added in
  // order of the blades. if there is an effect on this blade, play it
  if (!WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum], bladeNum) &&
    ...
    trap->FX_PlayBoltedEffectID(
        client->saber[saberNum].bladeEffect, scent->lerpOrigin, scent->ghoul2,
        bladeNum, scent->currentState.number, useModelIndex, -1, qfalse);
  } else if (WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum],
                                              bladeNum) &&
             client->saber[saberNum].bladeEffect2) {
    trap->FX_PlayBoltedEffectID(
        client->saber[saberNum].bladeEffect2, scent->lerpOrigin, scent->ghoul2,
        bladeNum, scent->currentState.number, useModelIndex, -1, qfalse);
  }
  // get the boltMatrix
  trap->G2API_GetBoltMatrix(scent->ghoul2, useModelIndex, bladeNum, &boltMatrix,
                            futureAngles, origin, cg.time, cgs.gameModels,
                            scent->modelScale);
  ...
  if (cgs.gametype >= GT_TEAM && cgs.gametype != GT_SIEGE && !cgs.jediVmerc &&
      cent->currentState.eType != ET_NPC) {
  ...
  if (!cg_saberContact
           .integer) { // if we don't have saber contact enabled, just add the
                       // blade and don't care what it's touching
    goto CheckTrail;
  }
  ...
  if (!dontDraw) {
    if (cg_saberModelTraceEffect.integer) {
      ...
      if (trace.fraction !=
          1) { // nudge the endpos a very small amount from the beginning to the
               // end, so the comp trace hits at the end.
        // I'm only bothering with this because I want to do a backwards trace
        // too in the comp trace, so if the blade is sticking through a player or
        // something the standard trace doesn't it, it will make sparks on each
        // side.
        vec3_t seDif;
      ...
      if (client->saber[saberNum].blade[bladeNum].storageTime <
          cg.time) { // debounce it in case our framerate is absurdly high.
                     // Using storageTime since it's not used for anything else
                     // in the client.
    ...
    for (i = 0; i < 1;
         i++) // was 2 because it would go through architecture and leave saber
              // trails on either side of the brush - but still looks bad if we
              // hit a corner, blade is still 8 longer than hit
    {
      ...
      if (trace.fraction < 1.0f) {
        ...
        if ((client->saber[saberNum].saberFlags2 &
             SFL2_NO_WALL_MARKS)) { // don't actually draw the marks/impact
                                    // effects
        } else {
          if (!(trace.surfaceFlags & SURF_NOIMPACT)) // never spark on sky
          {
            trap->FX_PlayEffectID(cgs.effects.mSparks, trace.endpos, trDir, -1,
                                  -1, qfalse);
          }
        }

        // Stop saber? (it wouldn't look right if it was stuck through a thin
        // wall and unable to hurt players on the other side)
        ...
        if ((client->saber[saberNum].saberFlags2 &
             SFL2_NO_WALL_MARKS)) { // don't actually draw the marks
        } else {                    // draw marks if we hit a wall
          // All I need is a bool to mark whether I have a previous point to
          // work with.
          ...
          if (client->saber[saberNum].blade[bladeNum].trail.haveOldPos[i]) {
            if (trace.entityNum == ENTITYNUM_WORLD ||
                ...
                (cg_entities[trace.entityNum].currentState.eFlags &
                 EF_PERMANENT)) { // only put marks on architecture
              // Let's do some cool burn/glowing mark bits!!!
              CG_CreateSaberMarks(
                  client->saber[saberNum].blade[bladeNum].trail.oldPos[i],
                  trace.endpos, trace.plane.normal);

              // make a sound
              if (cg.time - client->saber[saberNum]
                                .blade[bladeNum]
                                .hitWallDebounceTime >=
                  100) { // ugh, need to have a real sound debouncer... or do
                         // this game-side
                client->saber[saberNum].blade[bladeNum].hitWallDebounceTime =
                    cg.time;
                trap->S_StartSound(
                    trace.endpos, -1, CHAN_WEAPON,
            ...
            //				CG_ImpactMark( cgs.media.rivetMarkShader,
            //client->saber[saberNum].blade[bladeNum].trail.oldPos[i],
            //client->saber[saberNum].blade[bladeNum].trail.oldNormal[i],
        ...
        if (client->saber[saberNum].blade[bladeNum].trail.haveOldPos[i]) {
          ...
          // Let's put the mark there, we should use an endcap mark to close the
          // line, but we
...
CheckTrail:
  ...
  if (!cg_saberTrail.integer) { // don't do the trail in this case
    goto JustDoIt;
  }
  ...
  if ((!WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum], bladeNum) &&
      ...
      (WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum], bladeNum) &&
       client->saber[saberNum].trailStyle2 >
           1)) { // don't actually draw the trail at all
    goto JustDoIt;
  }
  ...
  saberTrail->duration =
      saberMoveData[cent->currentState.saberMove].trailLength;

  trailDur = (saberTrail->duration / 5.0f);
  if (!trailDur) { // hmm.. ok, default
  ...
  // if we happen to be timescaled or running in a high framerate situation, we
  // don't want to flood
  //	the system with very small trail slices...but perhaps doing it by
  //distance would yield better results?
  if (cg.time > saberTrail->lastTime + 2 || cg_saberTrail.integer == 2) // 2ms
  {
    if (!dontDraw) {
      if ((BG_SuperBreakWinAnim(cent->currentState.torsoAnim) ||
           saberMoveData[cent->currentState.saberMove].trailLength > 0 ||
           ((cent->currentState.powerups & (1 << PW_SPEED) &&
             cg_speedTrail.integer)) ||
           (cent->currentState.saberInFlight && saberNum == 0)) &&
          cg.time <
              saberTrail->lastTime + 2000) // if we have a stale segment, don't
                                           // draw until we have a fresh one
      {
#if 0
...
#endif
        {
          ...
          // Here we will use the happy process of filling a struct in with
          // arguments and passing it to a trap function so that we can take the
          // struct and fill in an actual CTrail type using the data within it
          // once we get it into the effects area

          // Go from new muzzle to new end...then to old end...back down to old
          // muzzle...finally
          ...
          // This prevents the trail from showing at all in low framerate
          // situations. if ( diff <= SABER_TRAIL_TIME * 2 )
          if (diff <=
              10000) { // don't draw it if the last time is way out of date
            float oldAlpha = 1.0f - (diff / trailDur);

            if (cg_saberTrail.integer == 2 && cg_shadows.integer != 2 &&
                cgs.glconfig.stencilBits >= 4) { // does other stuff below
            } else {
              if ((!WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum],
                                                     bladeNum) &&
                   client->saber[saberNum].trailStyle == 1) ||
                  (WP_SaberBladeUseSecondBladeStyle(&client->saber[saberNum],
                                                    bladeNum) &&
                   client->saber[saberNum].trailStyle2 == 1)) { // motion trail
                fx.mShader = cgs.media.swordTrailShader;
                VectorSet(rgb1, 32.0f, 32.0f,
                          32.0f); // make the sith sword trail pretty faint
            ...
            fx.mVerts[2].ST[0] =
                1.0f -
                oldAlpha; // NOTE: this just happens to contain the value I want
            ...
            if (cg_saberTrail.integer == 2 && cg_shadows.integer != 2 &&
              ...
              trap->R_SetRefractionProperties(
                  1.0f, 0.0f, qtrue,
                  qtrue); // don't need to do this every frame.. but..

              if (BG_SaberInAttack(cent->currentState.saberMove) ||
                  BG_SuperBreakWinAnim(
                      cent->currentState
                          .torsoAnim)) { // in attack, strong trail
                fx.mKillTime = 300;
              } else { // faded trail
                fx.mKillTime = 40;
              }
    ...
    // we must always do this, even if we aren't active..otherwise we won't know
...
JustDoIt:
  ...
  if ((client->saber[saberNum].saberFlags2 &
       SFL2_NO_BLADE)) { // don't actually draw the blade at all
    if (client->saber[saberNum].numBlades < 3 &&
        !(client->saber[saberNum].saberFlags2 &
          SFL2_NO_DLIGHT)) { // hmm, but still add the dlight
      CG_DoSaberLight(&client->saber[saberNum]);
    }
    return;
  }
  // Pass in the renderfx flags attached to the saber weapon model...this is
  // done so that saber glows
  //	will get rendered properly in a mirror...not sure if this is necessary??
  // CG_DoSaber( org_, axis_[0], saberLen,
  // client->saber[saberNum].blade[bladeNum].lengthMax,
  // client->saber[saberNum].blade[bladeNum].radius, 	scolor, renderfx,
  //(qboolean)(saberNum==0&&bladeNum==0) );
  CG_DoSaber(
      org_, axis_[0], saberLen,
      client->saber[saberNum].blade[bladeNum].lengthMax,
      client->saber[saberNum].blade[bladeNum].radius, scolor, renderfx,
      (qboolean)(client->saber[saberNum].numBlades < 3 &&
...
void CG_DrawPlayerSphere(centity_t *cent, vec3_t origin, float scale,
                         int shader) {
  refEntity_t ent;
  vec3_t ang;
  float vLen;
  ...
  if (!cg.renderingThirdPerson &&
      cent->currentState.number ==
          cg.predictedPlayerState.clientNum) { // don't do the rest then
    return;
  }
  ...
  if (shader ==
      cgs.media
          .invulnerabilityShader) { // ok, ok, this is a little hacky. sorry!
    ent.shaderRGBA[0] = 0;
    ent.shaderRGBA[1] = 255;
    ent.shaderRGBA[2] = 0;
...
void CG_AddLightningBeam(vec3_t start, vec3_t end) {
  ...
  VectorSet(chaos, len * 0.01f * s1, len * 0.02f * s2,
            len * 0.04f * (s1 + s2 + s3));

  VectorAdd(c1, chaos, c1);
  VectorScale(chaos, 4.0f, v1);

  VectorSet(chaos, -len * 0.02f * s3, len * 0.01f * (s1 * s2),
            -len * 0.02f * (s1 + s2 * s3));
...
// Checks to see if the model string has a * appended with a custom skin name
// after. If so, it terminates the model string correctly, parses the skin name
// out, and returns the handle of the registered skin.
int CG_HandleAppendedSkin(char *modelName) {
  ...
  if (p) { // found a *, we should have a model name before it and a skin name
           // after it.
    *p = 0; // terminate the modelName string at this point, then go ahead and
            // parse to the next 0 for the skin.
    p++;
...
// Create a temporary ghoul2 instance and get the gla name so we can try loading
// animation data and sounds.
...
void CG_CacheG2AnimInfo(char *modelName) {
  ...
  if (modelName[0] ==
    ...
    if (useSkin[0]) { // use a custom skin
      trap->R_RegisterSkin(
          va("models/players/%s/model_%s.skin", useModel, useSkin));
    } else {
      trap->R_RegisterSkin(
          va("models/players/%s/model_default.skin", useModel));
    }
    Q_strncpyz(useModel, va("models/players/%s/model.glm", useModel),
               sizeof(useModel));
  }
...
static void CG_RegisterVehicleAssets(Vehicle_t *pVeh) {
  ...
  {
          pVeh->m_pVehicleInfo->iExhaustFX = trap->FX_RegisterEffect(
  pVeh->m_pVehicleInfo->exhaustFX );
  }
  if ( pVeh->m_pVehicleInfo->trailFX )
  {
          pVeh->m_pVehicleInfo->iTrailFX = trap->FX_RegisterEffect(
  pVeh->m_pVehicleInfo->trailFX );
  }
  if ( pVeh->m_pVehicleInfo->impactFX )
  {
          pVeh->m_pVehicleInfo->iImpactFX = trap->FX_RegisterEffect(
  pVeh->m_pVehicleInfo->impactFX );
  }
  if ( pVeh->m_pVehicleInfo->explodeFX )
  {
          pVeh->m_pVehicleInfo->iExplodeFX = trap->FX_RegisterEffect(
  pVeh->m_pVehicleInfo->explodeFX );
  }
  if ( pVeh->m_pVehicleInfo->wakeFX )
  {
          pVeh->m_pVehicleInfo->iWakeFX = trap->FX_RegisterEffect(
  pVeh->m_pVehicleInfo->wakeFX );
  }

  if ( pVeh->m_pVehicleInfo->dmgFX )
  {
          pVeh->m_pVehicleInfo->iDmgFX = trap->FX_RegisterEffect(
  pVeh->m_pVehicleInfo->dmgFX );
  }
  if ( pVeh->m_pVehicleInfo->wpn1FX )
  {
          pVeh->m_pVehicleInfo->iWpn1FX = trap->FX_RegisterEffect(
  pVeh->m_pVehicleInfo->wpn1FX );
  }
  if ( pVeh->m_pVehicleInfo->wpn2FX )
  {
          pVeh->m_pVehicleInfo->iWpn2FX = trap->FX_RegisterEffect(
  pVeh->m_pVehicleInfo->wpn2FX );
  }
  if ( pVeh->m_pVehicleInfo->wpn1FireFX )
  {
          pVeh->m_pVehicleInfo->iWpn1FireFX = trap->FX_RegisterEffect(
  pVeh->m_pVehicleInfo->wpn1FireFX );
  }
  if ( pVeh->m_pVehicleInfo->wpn2FireFX )
  {
          pVeh->m_pVehicleInfo->iWpn2FireFX = trap->FX_RegisterEffect(
  pVeh->m_pVehicleInfo->wpn2FireFX );
  }
  */
}
...
void CG_G2AnimEntModelLoad(centity_t *cent) {
  ...
  if (!cent->npcClient) { // have not init'd client yet
    return;
  }
  ...
  if (cModelName && cModelName[0]) {
    ...
    if (cent->currentState.NPC_class == CLASS_VEHICLE &&
        modelName[0] ==
            '$') { // vehicles pass their veh names over as model names, then we
                   // get the model name from the veh type
      ...
      cent->m_pVehicle->m_vOrientation =
          &cgSendPS[cent->currentState.number]->vehOrientation[0];
      ...
      if (cent->m_pVehicle->m_pVehicleInfo->skin &&
          cent->m_pVehicle->m_pVehicleInfo->skin[0]) { // use a custom skin
        skinID = trap->R_RegisterSkin(
            va("models/players/%s/model_%s.skin", modelName,
               cent->m_pVehicle->m_pVehicleInfo->skin));
      } else {
        skinID = trap->R_RegisterSkin(
            va("models/players/%s/model_default.skin", modelName));
      }
      ...
      skinID = CG_HandleAppendedSkin(modelName); // get the skin if there is
                                                 // one.
    }

    if (cent->ghoul2) { // clean it first!
    ...
    if (cent->ghoul2) {
      ...
      if (cent->currentState.NPC_class == CLASS_VEHICLE &&
          cent->m_pVehicle) { // do special vehicle stuff
        char strTemp[128];
        int i;
        ...
        cent->m_pVehicle->m_iDroidUnitTag =
            trap->G2API_AddBolt(cent->ghoul2, 0, "*droidunit");

        // Setup the Exhausts.
        for (i = 0; i < MAX_VEHICLE_EXHAUSTS; i++) {
          Com_sprintf(strTemp, 128, "*exhaust%i", i + 1);
          cent->m_pVehicle->m_iExhaustTag[i] =
              trap->G2API_AddBolt(cent->ghoul2, 0, strTemp);
        }

        // Setup the Muzzles.
        for (i = 0; i < MAX_VEHICLE_MUZZLES; i++) {
          Com_sprintf(strTemp, 128, "*muzzle%i", i + 1);
          cent->m_pVehicle->m_iMuzzleTag[i] =
              trap->G2API_AddBolt(cent->ghoul2, 0, strTemp);
      ...
      if (cent->currentState.npcSaber1) {
        saber =
            (char *)CG_ConfigString(CS_MODELS + cent->currentState.npcSaber1);
        assert(!saber || !saber[0] || saber[0] == '@');
        // valid saber names should always start with '@' for NPCs

        if (saber && saber[0]) {
          saber++; // skip over the @
          WP_SetSaber(cent->currentState.number, cent->npcClient->saber, 0,
                      saber);
        }
      }
      if (cent->currentState.npcSaber2) {
        saber =
            (char *)CG_ConfigString(CS_MODELS + cent->currentState.npcSaber2);
        assert(!saber || !saber[0] || saber[0] == '@');
        // valid saber names should always start with '@' for NPCs

        if (saber && saber[0]) {
          saber++; // skip over the @
          WP_SetSaber(cent->currentState.number, cent->npcClient->saber, 1,
                      saber);
        }
      }
      ...
      if (cent->currentState.NPC_class != CLASS_VEHICLE) {
        while (j < MAX_SABERS) {
          if (cent->npcClient->saber[j].model[0]) {
            if (cent->npcClient->ghoul2Weapons[j]) { // free the old instance(s)
      ...
      if (GLAName[0] &&
        ...
        // rhand must always be first bolt. lhand always second. Whichever you
        // want the jetpack bolted to must always be third.
      ...
      if (cent->currentState.NPC_class != CLASS_VEHICLE) {
        if (trap->G2API_AddBolt(cent->ghoul2, 0, "lower_lumbar") ==
            -1) { // check now to see if we have this bone for setting anims and
                  // such
          cent->noLumbar = qtrue;
        }

        if (trap->G2API_AddBolt(cent->ghoul2, 0, "face") ==
            -1) { // check now to see if we have this bone for setting anims and
                  // such
          cent->noFace = qtrue;
        }
      ...
      if (cent->localAnimIndex != -1) {
        ...
        cent->eventAnimIndex = BG_ParseAnimationEvtFile(
            originalModelName, cent->localAnimIndex, bgNumAnimEvents);
      }
    }
  }
...
// because I am lazy.
static void CG_CreateSurfaceDebris(centity_t *cent, int surfNum, int fxID,
                                   qboolean throwPart) {
  int lostPartFX = 0;
  int b;
  vec3_t v, d;
  ...
  if (!cent->ghoul2) { // oh no
    return;
  }

  // let's add the surface as a bolt so we can get the base point of it
  if (bgToggleableSurfaceDebris[surfNum] == 3) { // right wing flame
  ...
  } else if (bgToggleableSurfaceDebris[surfNum] == 4) { // left wing flame
    b = trap->G2API_AddBolt(cent->ghoul2, 0, "*l_wingdamage");
    if (throwPart && cent->m_pVehicle && cent->m_pVehicle->m_pVehicleInfo) {
      lostPartFX = cent->m_pVehicle->m_pVehicleInfo->iLWingFX;
    }
  } else if (bgToggleableSurfaceDebris[surfNum] == 5) { // right wing flame 2
    b = trap->G2API_AddBolt(cent->ghoul2, 0, "*r_wingdamage");
    if (throwPart && cent->m_pVehicle && cent->m_pVehicle->m_pVehicleInfo) {
      lostPartFX = cent->m_pVehicle->m_pVehicleInfo->iRWingFX;
    }
  } else if (bgToggleableSurfaceDebris[surfNum] == 6) { // left wing flame 2
    b = trap->G2API_AddBolt(cent->ghoul2, 0, "*l_wingdamage");
    if (throwPart && cent->m_pVehicle && cent->m_pVehicle->m_pVehicleInfo) {
      lostPartFX = cent->m_pVehicle->m_pVehicleInfo->iLWingFX;
    }
  } else if (bgToggleableSurfaceDebris[surfNum] == 7) { // nose flame
  ...
  if (b == -1) { // couldn't find this surface apparently
    return;
  }

  // now let's get the position and direction of this surface and make a big
  // explosion
  trap->G2API_GetBoltMatrix(cent->ghoul2, 0, b, &boltMatrix, cent->lerpAngles,
                            cent->lerpOrigin, cg.time, cgs.gameModels,
                            cent->modelScale);
  ...
  if (throwPart && lostPartFX) { // throw off a ship part, too
    vec3_t fxFwd;
...
static void CG_CreateSurfaceSmoke(centity_t *cent, int shipSurf, int fxID) {
  ...
  // let's add the surface as a bolt so we can get the base point of it
  if (shipSurf == SHIPSURF_FRONT) { // front flame/smoke
    surfName = "*nosedamage";
  } else if (shipSurf == SHIPSURF_BACK) { // back flame/smoke
    surfName = "*exhaust1"; // FIXME: random?  Some point in-between?
  } else if (shipSurf == SHIPSURF_RIGHT) { // right wing flame/smoke
    surfName = "*r_wingdamage";
  } else if (shipSurf == SHIPSURF_LEFT) { // left wing flame/smoke
...
qboolean CG_VehicleShouldDrawShields(centity_t *vehCent) {
  if (vehCent->damageTime > cg.time // ship shields currently taking damage
      ...
      vehCent->m_pVehicle && vehCent->m_pVehicle->m_pVehicleInfo) {
    return qtrue;
  }
  return qfalse;
}
...
extern void BG_VehicleAdjustBBoxForOrientation( Vehicle_t *veh, vec3_t origin,
vec3_t mins, vec3_t maxs, int clientNum, int tracemask, void
(*localTrace)(trace_t *results, const vec3_t start, const vec3_t mins, const
...
qboolean CG_VehicleAttachDroidUnit(centity_t *droidCent, refEntity_t *legs) {
  if (droidCent && droidCent->currentState.owner &&
      droidCent->currentState.clientNum >=
          MAX_CLIENTS) { // the only NPCs that can ride a vehicle are
                         // droids...???
    centity_t *vehCent = &cg_entities[droidCent->currentState.owner];
    if (vehCent && vehCent->m_pVehicle && vehCent->ghoul2 &&
        vehCent->m_pVehicle->m_iDroidUnitTag != -1) {
      mdxaBone_t boltMatrix;
      vec3_t fwd, rt, tempAng;

      trap->G2API_GetBoltMatrix(
          vehCent->ghoul2, 0, vehCent->m_pVehicle->m_iDroidUnitTag, &boltMatrix,
          vehCent->lerpAngles, vehCent->lerpOrigin, cg.time, cgs.gameModels,
          vehCent->modelScale);
...
void CG_G2Animated(centity_t *cent) {
  ...
  if (!cent->ghoul2) { // Initialize this g2 anim ent, then return (will start
  ...
  if (cent->npcLocalSurfOff != cent->currentState.surfacesOff ||
      cent->npcLocalSurfOn !=
          cent->currentState
              .surfacesOn) { // looks like it's time for an update.
    int i = 0;

    while (i < BG_NUM_TOGGLEABLE_SURFACES && bgToggleableSurfaces[i]) {
      if (!(cent->npcLocalSurfOff & (1 << i)) &&
          (cent->currentState.surfacesOff &
           (1 << i))) { // it wasn't off before but it's off now, so reflect
                        // this change in the g2 instance.
        if (bgToggleableSurfaceDebris[i] >
            0) { // make some local debris of this thing?
          // FIXME: throw off the proper model effect, too
          CG_CreateSurfaceDebris(cent, i, cgs.effects.mShipDestDestroyed,
                                 qtrue);
        }

        trap->G2API_SetSurfaceOnOff(cent->ghoul2, bgToggleableSurfaces[i],
                                    TURN_OFF);
      }

      if (!(cent->npcLocalSurfOn & (1 << i)) &&
          (cent->currentState.surfacesOn &
           (1 << i))) { // same as above, but on instead of off.
        trap->G2API_SetSurfaceOnOff(cent->ghoul2, bgToggleableSurfaces[i],
                                    TURN_ON);
      }
  ...
  { //if the server says we have a weapon and we haven't copied one onto
  ourselves yet, then do so.
          trap->G2API_CopySpecificGhoul2Model(g2WeaponInstances[cent->currentState.weapon],
  0, cent->ghoul2, 1);

          if (cent->currentState.weapon == WP_SABER)
          {
                  trap->S_StartSound(cent->lerpOrigin,
  cent->currentState.number, CHAN_AUTO, trap->S_RegisterSound(
  "sound/weapons/saber/saberon.wav" ));
          }
  }
  */

  if (cent->torsoBolt && !(cent->currentState.eFlags &
                           EF_DEAD)) { // he's alive and has a limb missing
                                       // still, reattach it and reset the weapon
  ...
  if ((cent->lerpAngles[YAW] > 0 && cent->smoothYaw < 0) ||
      (cent->lerpAngles[YAW] < 0 &&
       cent->smoothYaw > 0)) { // keep it from snapping around on the threshold
    cent->smoothYaw = -cent->smoothYaw;
  }
  cent->lerpAngles[YAW] =
      cent->smoothYaw +
      (cent->lerpAngles[YAW] - cent->smoothYaw) * angSmoothFactor;
  ...
  cg.predictedVehicleState.clientNum )
          {//not the predicted vehicle
                  ...
                  BG_VehicleAdjustBBoxForOrientation( cent->m_pVehicle,
  cent->lerpOrigin, bmins, bmaxs, cent->currentState.number, MASK_PLAYERSOLID,
  NULL ); cent->m_pVehicle->m_vOrientation = old;
...
// Disabled for now, I'm too lazy to keep it working with all the stuff changing
// around.
#if 0
int cgFPLSState = 0;
...
static void CG_ForceElectrocution(centity_t *cent, const vec3_t origin,
                                  vec3_t tempAngles, qhandle_t shader,
                                  qboolean alwaysDo) {
  // Undoing for now, at least this code should compile if I ( or anyone else )
  // decides to work on this effect
  ...
  if (cent->localAnimIndex <= 1) { // humanoid
  ...
  } else if (cent->currentState.NPC_class ==
             CLASS_PROTOCOL) { // any others that can use these bolts too?
    torsoBolt = trap->G2API_AddBolt(cent->ghoul2, 0, "lower_lumbar");
  ...
  if (bolt >= 0) {
    found = trap->G2API_GetBoltMatrix(cent->ghoul2, 0, bolt, &boltMatrix,
                                      tempAngles, origin, cg.time,
                                      cgs.gameModels, cent->modelScale);
  }

  // Make sure that it's safe to even try and get these values out of the
  // Matrix, otherwise the values could be garbage
  if (found) {
    BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, fxOrg);
    ...
    VectorSet(dir, Q_flrand(-1.0f, 1.0f), Q_flrand(-1.0f, 1.0f),
              Q_flrand(-1.0f, 1.0f)); // Not normalized, but who cares.
  ...
  if (tr.fraction < 1.0f || Q_flrand(0.0f, 1.0f) > 0.94f || alwaysDo) {
    ...
    FX_AddElectricity( fxOrg, tr.endpos,
            ...
            1.0f, 0.5f, 0.0f,
            rgb, rgb, 0.0f,
            5.5f, Q_flrand(0.0f, 1.0f) * 50 + 100, shader, FX_ALPHA_LINEAR |
    FX_SIZE_LINEAR | FX_BRANCH | FX_GROW | FX_TAPER );
    */
  }
}
...
void CG_InitJetpackGhoul2(void) {
  ...
  trap->G2API_InitGhoul2Model(&cg_g2JetpackInstance, JETPACK_MODEL, 0, 0, 0, 0,
                              0);
...
static QINLINE void CG_VehicleEffects(centity_t *cent) {
  ...
  if (cent->currentState.clientNum ==
          cg.predictedPlayerState.m_iVehicleNum         // my vehicle
  ...
  {                                                     // in hyperspace!
    if (cg.predictedVehicleState.hyperSpaceTime &&
      ...
      if (!cg_lastHyperSpaceEffectTime ||
          (cg.time - cg_lastHyperSpaceEffectTime) >
              HYPERSPACE_TIME + 500) { // can't be from the last time we were in
                                       // hyperspace, so play the effect!
        trap->FX_PlayBoltedEffectID(cgs.effects.mHyperspaceStars,
                                    cent->lerpOrigin, cent->ghoul2, 0,
  ...
  if (cent->currentState.clientNum != cg.predictedPlayerState.m_iVehicleNum &&
      (pVehNPC->m_pVehicleInfo->soundFlyBy ||
       pVehNPC->m_pVehicleInfo->soundFlyBy2)) { // not my vehicle
    if (cent->currentState.speed &&
        cg.predictedPlayerState.speed + cent->currentState.speed >
            500) { // he's moving and between the two of us, we're moving fast
      vec3_t diff;
      ...
      if (VectorLength(diff) < 2048) { // close
        ...
        if (lastFlyBySound[cent->currentState.clientNum] + FLYBYSOUNDTIME <
            cg.time) { // okay to do a flyby sound on this vehicle
          if (DotProduct(myFwd, theirFwd) < 500) {
            int flyBySound = 0;
            if (pVehNPC->m_pVehicleInfo->soundFlyBy &&
                pVehNPC->m_pVehicleInfo->soundFlyBy2) {
              flyBySound = Q_irand(0, 1) ? pVehNPC->m_pVehicleInfo->soundFlyBy
                                         : pVehNPC->m_pVehicleInfo->soundFlyBy2;
            ...
            trap->S_StartSound(NULL, cent->currentState.clientNum,
                               CHAN_LESS_ATTEN, flyBySound);
  ...
  if (!cent->currentState.speed    // was stopped
      ...
      && cent->m_pVehicle->m_pVehicleInfo
             ->soundEngineStart) { // engines rev up for the first time
  ...
  if (pVehNPC->m_pVehicleInfo->type != VH_ANIMAL) {
    if (pVehNPC->m_pVehicleInfo->surfDestruction &&
        cent->ghoul2) { // see if anything has been blown off
      int i = 0;
      qboolean surfDmg = qfalse;

      while (i < BG_NUM_TOGGLEABLE_SURFACES) {
        if (bgToggleableSurfaceDebris[i] >
            1) { // this is decidedly a destroyable surface, let's check its
                 // status
          int surfTest = trap->G2API_GetSurfaceRenderStatus(
              cent->ghoul2, 0, bgToggleableSurfaces[i]);

          if (surfTest != -1 &&
              (surfTest & TURN_OFF)) { // it exists, but it's off...
            surfDmg = qtrue;

            // create some flames
            CG_CreateSurfaceDebris(cent, i, cgs.effects.mShipDestBurning,
                                   qfalse);
          }
        }
      ...
      if (surfDmg) { // if any surface are damaged, neglect exhaust etc effects
                     // (so we don't have exhaust trails coming out of invisible
                     // surfaces)
        return;
      }
    }

    if (pVehNPC->m_iLastFXTime <=
        cg.time) { // until we attach it, we need to debounce this
      vec3_t fwd, rt, up;
      vec3_t flat;
      ...
      if (cent->currentState.speed >
          0) { // FIXME: only do this when accelerator is being pressed! (must
               // have a driver?)
        vec3_t org;
        ...
        if (0) { // hurt
          trap->FX_PlayEffectID(cgs.effects.mBlackSmoke, org, fwd, -1, -1,
                                qfalse);
        } else if (pVehNPC->m_pVehicleInfo->iTrailFX) { // okay, do normal trail
          trap->FX_PlayEffectID(pVehNPC->m_pVehicleInfo->iTrailFX, org, fwd, -1,
                                -1, qfalse);
        }
        //=====================================================================
        // EXHAUST FX
        ...
        // do exhaust
        if ((cent->currentState.eFlags &
             EF_JETPACK_ACTIVE)) { // cheap way of telling us the vehicle is in
                                   // "turbo" mode
        ...
        if (doExhaust && cent->ghoul2) {
          ...
          for (i = 0; i < MAX_VEHICLE_EXHAUSTS; i++) {
            // We hit an invalid tag, we quit (they should be created in order
            // so tough luck if not).
            if (pVehNPC->m_iExhaustTag[i] == -1) {
              break;
            }

            if ((cent->currentState.brokenLimbs &
                 (1 << SHIPSURF_DAMAGE_BACK_HEAVY))) { // engine has taken heavy
                                                       // damage
              if (!Q_irand(0, 1)) { // 50% chance of not drawing this engine
                                    // glow this frame
                continue;
              }
            } else if ((cent->currentState.brokenLimbs &
                        (1
                         << SHIPSURF_DAMAGE_BACK_LIGHT))) { // engine has taken
                                                            // light damage
              if (!Q_irand(0, 4)) { // 20% chance of not drawing this engine
                                    // glow this frame
                continue;
              }
            }
            ...
            if ((cent->currentState.eFlags &
                 EF_JETPACK_ACTIVE) // cheap way of telling us the vehicle is in
                                    // "turbo" mode
                && pVehNPC->m_pVehicleInfo->iTurboFX) // they have a valid turbo
                                                      // exhaust effect to play
            {
              fx = pVehNPC->m_pVehicleInfo->iTurboFX;
            } else { // play the normal one
              fx = pVehNPC->m_pVehicleInfo->iExhaustFX;
            }

            if (pVehNPC->m_pVehicleInfo->type == VH_FIGHTER) {
              trap->FX_PlayBoltedEffectID(
                  fx, cent->lerpOrigin, cent->ghoul2, pVehNPC->m_iExhaustTag[i],
                  cent->currentState.number, 0, 0, qtrue);
            } else { // fixme: bolt these too
              mdxaBone_t boltMatrix;
              vec3_t boltOrg, boltDir;

              trap->G2API_GetBoltMatrix(
                  cent->ghoul2, 0, pVehNPC->m_iExhaustTag[i], &boltMatrix, flat,
                  cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
        ...
        if (pVehNPC->m_pVehicleInfo->iTrailFX != 0 && cent->ghoul2) {
          ...
          if (pVehNPC->m_pVehicleInfo->type !=
              VH_FIGHTER) { // only fighters use pitch/roll in refent axis
            getBoltAngles[PITCH] = getBoltAngles[ROLL] = 0.0f;
          }

          for (i = 1; i < 5; i++) {
            int trailBolt =
                trap->G2API_AddBolt(cent->ghoul2, 0, va("*trail%d", i));
            // We hit an invalid tag, we quit (they should be created in order
            // so tough luck if not).
            if (trailBolt == -1) {
              break;
            ...
            trap->G2API_GetBoltMatrix(cent->ghoul2, 0, trailBolt, &boltMatrix,
                                      getBoltAngles, cent->lerpOrigin, cg.time,
                                      cgs.gameModels, cent->modelScale);

            BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, boltOrg);
            VectorCopy(fwd, boltDir); // fixme?

            trap->FX_PlayEffectID(pVehNPC->m_pVehicleInfo->iTrailFX, boltOrg,
                                  boltDir, -1, -1, qfalse);
          }
        }
      }
      ...
      {
        if ((cent->currentState.eFlags &
             EF_DEAD)) { // just plain dead, use flames
          vec3_t up = {0, 0, 1};
          vec3_t boltOrg;
          ...
          //pVehNPC->m_iDriverTag, &boltMatrix, getBoltAngles, 		cent->lerpOrigin,
          //cg.time, cgs.gameModels, cent->modelScale);

          //	BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, boltOrg);
          //}
          trap->FX_PlayEffectID(cgs.effects.mShipDestBurning, boltOrg, up, -1,
                                -1, qfalse);
        }
      }
      if (cent->currentState.brokenLimbs) {
        ...
        if (!Q_irand(0, 5)) {
          for (i = SHIPSURF_FRONT; i <= SHIPSURF_LEFT; i++) {
            if ((cent->currentState.brokenLimbs &
                 (1 << ((i - SHIPSURF_FRONT) +
                        SHIPSURF_DAMAGE_FRONT_HEAVY)))) { // heavy damage, do
                                                          // both effects
              if (pVehNPC->m_pVehicleInfo->iInjureFX) {
                CG_CreateSurfaceSmoke(cent, i,
                                      pVehNPC->m_pVehicleInfo->iInjureFX);
              }
              if (pVehNPC->m_pVehicleInfo->iDmgFX) {
                CG_CreateSurfaceSmoke(cent, i, pVehNPC->m_pVehicleInfo->iDmgFX);
                        ...
                        (1 << ((i - SHIPSURF_FRONT) +
                               SHIPSURF_DAMAGE_FRONT_LIGHT)))) { // only light
                                                                 // damage
              if (pVehNPC->m_pVehicleInfo->iInjureFX) {
                CG_CreateSurfaceSmoke(cent, i,
                                      pVehNPC->m_pVehicleInfo->iInjureFX);
      ...
      {//FIXME: should use something attached.. but want it to build up over
      time, so... if ( flrand( 0, cg.time - pVehNPC->m_iDieTime ) < 1000 )
              {//flaming!
                      VectorMA( cent->lerpOrigin, flrand(-64, 64), fwd, org );
                      VectorScale( fwd, -1, fwd );
                      trap->FX_PlayEffectID(
      trap->FX_RegisterEffect("ships/fire"), org, fwd, -1, -1, qfalse );
                      nextFXDelay = 50;
              }
      }
      */
...
float CG_RadiusForCent(centity_t *cent) {
  if (cent->currentState.eType == ET_NPC) {
    if (cent->currentState.NPC_class == CLASS_VEHICLE && cent->m_pVehicle &&
        cent->m_pVehicle->m_pVehicleInfo->g2radius) { // has override
...
void CG_CheckThirdPersonAlpha(centity_t *cent, refEntity_t *legs) {
  ...
  if (cent->m_pVehicle) { // a vehicle
    if (cg.predictedPlayerState.m_iVehicleNum !=
            cent->currentState.clientNum // not mine
        && cent->m_pVehicle->m_pVehicleInfo &&
  ...
  if (cg.predictedPlayerState.m_iVehicleNum) { // in a vehicle
    if (cg.predictedPlayerState.m_iVehicleNum ==
        cent->currentState.clientNum) { // this is my vehicle
      if (cent->m_pVehicle && cent->m_pVehicle->m_pVehicleInfo &&
          cent->m_pVehicle->m_pVehicleInfo->cameraOverride &&
          cent->m_pVehicle->m_pVehicleInfo
              ->cameraAlpha) { // vehicle has auto third-person alpha on
        trace_t trace;
        vec3_t dir2Crosshair, end;
        VectorSubtract(cg_crosshairPos, cameraCurLoc, dir2Crosshair);
        VectorNormalize(dir2Crosshair);
        VectorMA(cameraCurLoc,
                 cent->m_pVehicle->m_pVehicleInfo->cameraRange * 2.0f,
                 dir2Crosshair, end);
        CG_G2Trace(&trace, cameraCurLoc, vec3_origin, vec3_origin, end,
                   ENTITYNUM_NONE, CONTENTS_BODY);
        if (trace.entityNum == cent->currentState.clientNum ||
            trace.entityNum == cg.predictedPlayerState
                                   .clientNum) { // hit me or the vehicle I'm in
          cg_vehThirdPersonAlpha -= 0.1f * cg.frametime / 50.0f;
          if (cg_vehThirdPersonAlpha <
              cent->m_pVehicle->m_pVehicleInfo->cameraAlpha) {
            cg_vehThirdPersonAlpha =
                cent->m_pVehicle->m_pVehicleInfo->cameraAlpha;
          }
        } else {
          cg_vehThirdPersonAlpha += 0.1f * cg.frametime / 50.0f;
  ...
  } else if (cg.predictedPlayerState.clientNum ==
             cent->currentState.clientNum) { // it's me
    // reset this
    cg_vehThirdPersonAlpha = 1.0f;
    // use the cvar
...
void CG_Player(centity_t *cent) {
  ...
  // first if we are not an npc and we are using an emplaced gun then make sure
  // our angles are visually capped to the constraints (otherwise it's possible
  // to lerp a little outside and look kind of twitchy)
  if (cent->currentState.weapon == WP_EMPLACED_GUN &&
    ...
    if (BG_EmplacedView(
            cent->lerpAngles,
            cg_entities[cent->currentState.otherEntityNum2].currentState.angles,
            &empYaw,
            cg_entities[cent->currentState.otherEntityNum2]
                .currentState.origin2[0])) {
  ...
  if ((cg_smoothClients.integer || cent->currentState.heldByClient) &&
      ...
      cg.predictedPlayerState.m_iVehicleNum !=
          cent->currentState.number) { // always smooth when being thrown
    vec3_t posDif;
    float smoothFactor;
    int k = 0;
    ...
    if (cent->currentState
            .heldByClient) { // smooth the origin more when in this state,
                             // because movement is origin-based on server.
      smoothFactor = 0.2f;
    } else if ((cent->currentState.powerups & (1 << PW_SPEED)) ||
               (cent->currentState.forcePowersActive &
                (1 << FP_RAGE))) { // we're moving fast so don't smooth as much
      smoothFactor = 0.6f;
    } else if (cent->currentState.eType == ET_NPC &&
               cent->currentState.NPC_class == CLASS_VEHICLE &&
               cent->m_pVehicle &&
               cent->m_pVehicle->m_pVehicleInfo->type ==
                   VH_FIGHTER) { // greater smoothing for flying vehicles, since
                                 // they move so fast
    ...
    if (DistanceSquared(cent->beamEnd, cent->lerpOrigin) >
        smoothFactor * fTolerance) // 10000
    {
  ...
  if (cent->currentState.m_iVehicleNum &&
      cent->currentState.NPC_class !=
          CLASS_VEHICLE) { // this player is riding a vehicle
    ...
    if (veh->m_pVehicle && cent->playerState && veh->playerState &&
        cent->ghoul2 && veh->ghoul2) {
      if (veh->currentState.owner !=
          cent->currentState
              .clientNum) { // FIXME: what about visible passengers?
      ...
      // switching to someone else, often happens on siege
               ...
               (cent->playerState->clientNum !=
                cg.snap->ps.clientNum)) { // has a pilot...???
        vec3_t oldPSOrg;

        // make sure it has its pilot and parent set
        veh->m_pVehicle->m_pPilot =
            (bgEntity_t *)&cg_entities[veh->currentState.owner];
        ...
        // copy the "playerstate origin" to the lerpOrigin since that's what we
        // use to display
  ...
  if (cent->currentState.eType != ET_NPC) {
    ...
    if (cent->npcClient->ghoul2Model != cent->ghoul2 && cent->ghoul2) {
      ...
      if (cent->localAnimIndex <= 1) {
        cent->npcClient->bolt_rhand =
            trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "*r_hand");
        cent->npcClient->bolt_lhand =
            trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "*l_hand");
        ...
        // claw bolts
        trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0,
                            "*r_hand_cap_r_arm");
        trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0,
                            "*l_hand_cap_l_arm");

        cent->npcClient->bolt_head =
            trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "*head_top");
        if (cent->npcClient->bolt_head == -1) {
          cent->npcClient->bolt_head =
              trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "ceyebrow");
        }
        cent->npcClient->bolt_motion =
            trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "Motion");
        cent->npcClient->bolt_llumbar = trap->G2API_AddBolt(
            cent->npcClient->ghoul2Model, 0, "lower_lumbar");
      } else {
  ...
  if (cent->currentState.eType == ET_NPC &&
      cent->currentState.NPC_class == CLASS_VEHICLE) { // add vehicles
    CG_AddRadarEnt(cent);
    if (CG_InFighter()) { // this is a vehicle, bracket it
      if (cg.predictedPlayerState.m_iVehicleNum !=
          cent->currentState.clientNum) { // don't add the vehicle I'm in... :)
        CG_AddBracketedEnt(cent);
      }
    }
  }

  if (!cent->ghoul2) { // not ready yet?
#ifdef _DEBUG
    ...
    if (ci->ghoul2Model && trap->G2_HaveWeGhoul2Models(ci->ghoul2Model)) {
      ...
      // Attach the instance to this entity num so we can make use of
      // client-server shared operations if possible.
      trap->G2API_AttachInstanceToEntNum(cent->ghoul2,
      ...
      cent->eventAnimIndex =
          CG_G2EvIndexForModel(cent->ghoul2, cent->localAnimIndex);
    }
    return;
  }

  if (ci->superSmoothTime) {             // do crazy smoothing
    if (ci->superSmoothTime > cg.time) { // do it
  ...
  if (cg.predictedPlayerState.pm_type ==
      PM_INTERMISSION) { // don't show all this shit during intermission
    if (cent->currentState.eType == ET_NPC &&
        cent->currentState.NPC_class != CLASS_VEHICLE) { // NPC in intermission
    } else { // don't render players or vehicles in intermissions, allow other
             // NPCs for scripts
      return;
    }
  }
  ...
  if ((cent->currentState.eFlags & EF_JETPACK) &&
      !(cent->currentState.eFlags & EF_DEAD) &&
      cg_g2JetpackInstance) { // should have a jetpack attached
    // 1 is rhand weap, 2 is lhand weap (akimbo sabs), 3 is jetpack
    if (!trap->G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), 3)) {
      trap->G2API_CopySpecificGhoul2Model(cg_g2JetpackInstance, 0, cent->ghoul2,
                                          3);
    }
    ...
    if (cent->currentState.eFlags & EF_JETPACK_ACTIVE) {
      ...
      while (n < 2) {
        // Get the position/dir of the flame bolt on the jetpack model bolted to
        // the player
        trap->G2API_GetBoltMatrix(cent->ghoul2, 3, n, &mat, cent->turAngles,
                                  cent->lerpOrigin, cg.time, cgs.gameModels,
                                  cent->modelScale);
        ...
        if (cent->currentState.eFlags & EF_JETPACK_FLAMING) { // create effects
          // FIXME: Just one big effect
          // Play the effect
          trap->FX_PlayEffectID(cgs.effects.mBobaJet, flamePos, flameDir, -1,
                                -1, qfalse);
          trap->FX_PlayEffectID(cgs.effects.mBobaJet, flamePos, flameDir, -1,
                                -1, qfalse);
      ...
      trap->S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                 ...
                 &(cent->ghoul2), 3)) { // fixme: would be good if this could be
                                        // done not every frame
    trap->G2API_RemoveGhoul2Model(&(cent->ghoul2), 3);
  }

  g2HasWeapon = trap->G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), 1);

  if (!g2HasWeapon) { // force a redup of the weapon instance onto the client
                      // instance
  ...
  if (cent->isRagging && !(cent->currentState.eFlags & EF_DEAD) &&
      !(cent->currentState.eFlags &
        EF_RAG)) { // make sure we don't ragdoll ever while alive unless
                   // directly told to with eFlags
    cent->isRagging = qfalse;
    trap->G2API_SetRagDoll(
        cent->ghoul2, NULL); // calling with null parms resets to no ragdoll.
  }

  if (cent->ghoul2 && cent->torsoBolt &&
      ((cent->torsoBolt & RARMBIT) || (cent->torsoBolt & RHANDBIT) ||
       (cent->torsoBolt & WAISTBIT)) &&
      g2HasWeapon) { // kill the weapon if the limb holding it is no longer on
                     // the model
    trap->G2API_RemoveGhoul2Model(&(cent->ghoul2), 1);
    g2HasWeapon = qfalse;
  }

  if (!cent->trickAlphaTime ||
      (cg.time - cent->trickAlphaTime) >
          1000) { // things got out of sync, perhaps a new client is trying to
                  // fill in this slot
    cent->trickAlpha = 255;
    cent->trickAlphaTime = cg.time;
  }

  if (cent->currentState.eFlags & EF_NODRAW) { // If nodraw, return here
    return;
  } else if (cent->currentState.eFlags2 &
             EF2_SHIP_DEATH) { // died in ship, don't draw, we were
                               // "obliterated"
    return;
  }
  ...
  // Explanation:  The entitystate has a weapond defined on it.  The
  // cliententity does as well. The cliententity's weapon tells us what the
  // ghoul2 instance on the cliententity has bolted to it. If the entitystate
  // and cliententity weapons differ, then the state's needs to be copied to the
  // client. Save the old weapon, to verify that it is or is not the same as the
  // new weapon. rww - Make sure weapons don't get set BEFORE cent->ghoul2 is
  // initialized or else we'll have no weapon bolted on
  ...
  if (cent->ghoul2 &&
      (cent->currentState.eType != ET_NPC ||
       (cent->currentState.NPC_class != CLASS_VEHICLE &&
        cent->currentState.NPC_class != CLASS_REMOTE &&
        cent->currentState.NPC_class !=
            CLASS_SEEKER)) && // don't add weapon models to NPCs that have no
                              // bolt for them!
      cent->ghoul2weapon !=
          CG_G2WeaponInstance(cent, cent->currentState.weapon) &&
    ...
    if (ci->team == TEAM_SPECTATOR) {
      ...
      if (cent->currentState.eType != ET_NPC) {
        if (cent->weapon == WP_SABER &&
            ...
            !cent->currentState
                 .saberHolstered) { // switching away from the saber
          ...
          if (ci->saber[0].soundOn) {
            trap->S_StartSound(cent->lerpOrigin, cent->currentState.number,
                               CHAN_AUTO, ci->saber[0].soundOn);
          }

          if (ci->saber[1].soundOn) {
            trap->S_StartSound(cent->lerpOrigin, cent->currentState.number,
                               CHAN_AUTO, ci->saber[1].soundOn);
          }
    ...
    cent->ghoul2weapon =
        NULL; // be sure to update after respawning/getting limb regrown
  }
  ...
  if (ci->colorOverride[0] != 0.0f || ci->colorOverride[1] != 0.0f ||
  ...
  if (cgs.gametype >= GT_TEAM && cg_drawFriend.integer &&
      ...
      cent->currentState.eType !=
          ET_NPC) { // If the view is either a spectator or on the same team as
                    // this character, show a symbol above their head.
    if ((cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR ||
         cg.snap->ps.persistant[PERS_TEAM] == team) &&
        !(cent->currentState.eFlags & EF_DEAD)) {
      if (cgs.gametype == GT_SIEGE) { // check for per-map team shaders
        if (team == SIEGETEAM_TEAM1) {
          if (cgSiegeTeam1PlShader) {
            CG_PlayerFloatSprite(cent, cgSiegeTeam1PlShader);
          } else { // if there isn't one fallback to default
    ...
    if (cg.predictedPlayerState.persistant[PERS_TEAM] != TEAM_SPECTATOR &&
        ...
        !(cent->currentState.eFlags & EF_DEAD) && ci &&
        cgs.clientinfo[cg.snap->ps.clientNum].duelTeam ==
            ci->duelTeam) { // ally in powerduel, so draw the icon
  ...
  if (cgs.gametype == GT_JEDIMASTER && cg_drawFriend.integer &&
      cent->currentState.number !=
          cg.snap->ps.clientNum) // Don't show a sprite above a player's own
                                 // head in 3rd person.
  { // If the view is either a spectator or on the same team as this character,
    // show a symbol above their head.
    if ((cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR ||
         cg.snap->ps.persistant[PERS_TEAM] == team) &&
        !(cent->currentState.eFlags & EF_DEAD)) {
  ...
  if (((cent->currentState.eFlags & EF_SEEKERDRONE) ||
       cent->currentState.genericenemyindex != -1) &&
      cent->currentState.eType != ET_NPC) {
    refEntity_t seeker;
    ...
    if (cent->currentState.genericenemyindex > MAX_GENTITIES) {
      ...
      dir[2] = cos(angle) * 5;
      VectorAdd(elevated, dir, seeker.origin);
    } else if (cent->currentState.genericenemyindex != ENTITYNUM_NONE &&
               cent->currentState.genericenemyindex != -1) {
  ...
  if (cg_shadows.integer == 2 &&
      (renderfx & RF_THIRD_PERSON)) { // can see own shadow
                          ...
                          // presumes that client 0 is NOT a Rancor...
  {
    ...
    if (rancor) {
      BG_AttachToRancor(rancor->ghoul2, // ghoul2 info
                        rancor->lerpAngles[YAW], rancor->lerpOrigin, cg.time,
                        cgs.gameModels, rancor->modelScale,
                        (rancor->currentState.eFlags2 & EF2_GENERIC_NPC_FLAG),
                        legs.origin, legs.angles, NULL);

      if (cent->isRagging) { // hack, ragdoll has you way at bottom of bounding
                             // box
      ...
      VectorCopy(
          cent->lerpAngles,
          rootAngles); //??? tempAngles );//tempAngles is needed a lot below
      VectorCopy(cent->lerpAngles, cent->turAngles);
      VectorCopy(legs.origin, cent->lerpOrigin);
    }
  }
  // This call is mainly just to reconstruct the skeleton. But we'll get the
  // left hand matrix while we're at it. If we don't reconstruct the skeleton
  // after setting the bone angles, we will get bad bolt points on the model
  //(e.g. the weapon model bolt will look "lagged") if there's no other
  //GetBoltMatrix call for the rest of the frame. Yes, this is stupid and needs
  // to be fixed properly. The current solution is to force it not to reconstruct
  // the skeleton for the first GBM call in G2PlayerAngles. It works and we end
  // up only reconstructing it once, but it doesn't seem like the best solution.
  trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_lhand, &lHandMatrix,
                            cent->turAngles, cent->lerpOrigin, cg.time,
                            cgs.gameModels, cent->modelScale);
  ...
  if (cent->currentState.eFlags & EF_DEAD) {
    // rww - since our angles are fixed when we're dead this shouldn't be an
    // issue anyway we need to render the dying/dead player because we are now
    // spawning the body on respawn instead of death return;
  }
  ...
  if (!(cent->currentState.powerups & (1 << PW_SPEED)) || doAlpha ||
      !cg_speedTrail.integer) {
  ...
  if (cent->frame_minus1_refreshed || cent->frame_minus2_refreshed) {
    ...
    if (cent->frame_minus1_refreshed) {
      ...
      // rww - if the client gets a bad framerate we will only receive frame
      // positions once per frame anyway, so we might end up with speed trails
      // very spread out. in order to avoid that, we'll get the direction of the
      // last trail from the player and place the trail refent a set distance
      // from the player location this frame
    ...
    if (cent->frame_minus2_refreshed) {
      ...
      // Same as above but do it between trail points instead of the player and
      // first trail entry
  ...
  if (cent->currentState.eFlags &
      EF_DEAD) { // keep track of death anim frame for when we copy off the
                 // bodyqueue
  ...
  if (cent->currentState.activeForcePass > FORCE_LEVEL_3 &&
    ...
    if (cent->currentState.torsoAnim == BOTH_FORCE_2HANDEDLIGHTNING_HOLD &&
        Q_irand(0, 1)) { // alternate back and forth between left and right
      mdxaBone_t rHandMatrix;
      trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_rhand, &rHandMatrix,
                                cent->turAngles, cent->lerpOrigin, cg.time,
                                cgs.gameModels, cent->modelScale);
      ...
      // trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_lhand, &boltMatrix,
    ...
    if (realForceLev > FORCE_LEVEL_2) { // arc
      // trap->FX_PlayEffectID( cgs.effects.forceLightningWide, efOrg, fxDir );
      // trap->FX_PlayEntityEffectID(cgs.effects.forceDrainWide, efOrg, axis,
      // cent->boltInfo, cent->currentState.number, -1, -1);
      trap->FX_PlayEntityEffectID(cgs.effects.forceDrainWide, efOrg, axis, -1,
                                  -1, -1, -1);
    } else { // line
      // trap->FX_PlayEffectID( cgs.effects.forceLightning, efOrg, fxDir );
      // trap->FX_PlayEntityEffectID(cgs.effects.forceDrain, efOrg, axis,
      // cent->boltInfo, cent->currentState.number, -1, -1);
      trap->FX_PlayEntityEffectID(cgs.effects.forceDrain, efOrg, axis, -1, -1,
                                  -1, -1);
    }
    ...
    */
             ...
             cent->currentState.NPC_class !=
                 CLASS_VEHICLE) { // doing the electrocuting
    matrix3_t axis;
    vec3_t tAng, fAng, fxDir;
    vec3_t efOrg;
    ...
    if (cent->currentState.activeForcePass > FORCE_LEVEL_2) { // arc
      // trap->FX_PlayEffectID( cgs.effects.forceLightningWide, efOrg, fxDir );
      // trap->FX_PlayEntityEffectID(cgs.effects.forceLightningWide, efOrg,
      // axis, cent->boltInfo, cent->currentState.number, -1, -1);
      trap->FX_PlayEntityEffectID(cgs.effects.forceLightningWide, efOrg, axis,
                                  -1, -1, -1, -1);
    } else { // line
      // trap->FX_PlayEffectID( cgs.effects.forceLightning, efOrg, fxDir );
      // trap->FX_PlayEntityEffectID(cgs.effects.forceLightning, efOrg, axis,
      // cent->boltInfo, cent->currentState.number, -1, -1);
      trap->FX_PlayEntityEffectID(cgs.effects.forceLightning, efOrg, axis, -1,
                                  -1, -1, -1);
    }
    ...
    if (cent->bolt4 < cg.time)
    {
            cent->bolt4 = cg.time + 100;
            trap->S_StartSound(NULL, cent->currentState.number, CHAN_AUTO,
    trap->S_RegisterSound("sound/weapons/force/lightning.wav") );
    }
    */
  }
  ...
  if (cent->currentState.powerups & (1 << PW_DISINT_4)) {
    ...
    if ((cent->currentState.forcePowersActive & (1 << FP_GRIP)) &&
      ...
      //Render a scaled version of the model's hand with a n337 looking shader
      {
              ...
              trap->G2API_DuplicateGhoul2Instance(cent->ghoul2,
      &cent->grip_arm);
              ...
              trap->G2API_SetNewOrigin(cent->grip_arm,
      trap->G2API_AddBolt(cent->grip_arm, 0, rotateBone));
              ...
              regrip_arm.customShader = trap->R_RegisterShader(
      "gfx/misc/red_portashield" );
              ...
              regrip_arm.shaderRGBA[1] = regrip_arm.shaderRGBA[2] =
      regrip_arm.shaderRGBA[0];
  ...
  } else if (cent->bodyFadeTime) { // reset the counter for keeping track of
                                   // push refraction effect state
    cent->bodyFadeTime = 0;
  }
  ...
  if (cent->currentState.weapon == WP_STUN_BATON &&
      cent->currentState.number == cg.snap->ps.clientNum) {
    trap->S_AddLoopingSound(
        cent->currentState.number, cg.refdef.vieworg, vec3_origin,
        trap->S_RegisterSound("sound/weapons/baton/idle.wav"));
  }

  // NOTE: All effects that should be visible during mindtrick should go above
  // here
  ...
  if (iwantout) {
    ...
    if (legs.shaderRGBA[3] < 1) { // don't cancel it out even if it's < 1
      legs.shaderRGBA[3] = 1;
    }
  }

  if (cent->teamPowerEffectTime > cg.time) {
    if (cent->teamPowerType ==
        3) { // absorb is a somewhat different effect entirely
             // Guess I'll take care of it where it's always been, just checking
      ...
      if (cent->teamPowerType == 1) { // heal
        legs.shaderRGBA[0] = 0;
        legs.shaderRGBA[1] = 255;
        legs.shaderRGBA[2] = 0;
      } else if (cent->teamPowerType == 0) { // regen
        legs.shaderRGBA[0] = 0;
        legs.shaderRGBA[1] = 0;
  ...
  if (CG_IsMindTricked(cg.snap->ps.fd.forceMindtrickTargetIndex,
    ...
    if (cent->ghoul2) {
      ...
      trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_head, &boltMatrix,
                                tAng, cent->lerpOrigin, cg.time, cgs.gameModels,
                                cent->modelScale);
      ...
      // trap->FX_PlayEntityEffectID(trap->FX_RegisterEffect("force/confusion.efx"),
      // efOrg, axis, cent->boltInfo, cent->currentState.number);
      trap->FX_PlayEntityEffectID(cgs.effects.mForceConfustionOld, efOrg, axis,
                                  -1, -1, -1, -1);
    }
  }
  ...
  if ((cent->currentState.powerups & (1 << PW_YSALAMIRI)) ||
    ...
    if (cgs.gametype == GT_CTY &&
        (cent->currentState.powerups & (1 << PW_REDFLAG))) {
      CG_DrawPlayerSphere(cent, cent->lerpOrigin, 1.4f,
                          cgs.media.ysaliredShader);
    } else if (cgs.gametype == GT_CTY &&
               (cent->currentState.powerups & (1 << PW_BLUEFLAG))) {
      CG_DrawPlayerSphere(cent, cent->lerpOrigin, 1.4f,
                          cgs.media.ysaliblueShader);
    } else {
      CG_DrawPlayerSphere(cent, cent->lerpOrigin, 1.4f,
                          cgs.media.ysalimariShader);
    }
  }
  ...
  if (cent->currentState.powerups & (1 << PW_FORCE_ENLIGHTENED_DARK)) {
    CG_DrawPlayerSphere(cent, cent->lerpOrigin, 2.0f,
                        cgs.media.endarkenmentShader);
  } else if (cent->currentState.powerups & (1 << PW_FORCE_ENLIGHTENED_LIGHT)) {
    CG_DrawPlayerSphere(cent, cent->lerpOrigin, 2.0f,
                        cgs.media.enlightenmentShader);
  }

  if (cent->currentState.eFlags & EF_INVULNERABLE) {
    CG_DrawPlayerSphere(cent, cent->lerpOrigin, 1.0f,
                        cgs.media.invulnerabilityShader);
  }
stillDoSaber:
  if ((cent->currentState.eFlags & EF_DEAD) &&
      cent->currentState.weapon == WP_SABER) {
        ...
        && !(cent->currentState.eFlags & EF_DEAD)) // still alive
    {
      ...
      if (cg.snap->ps.clientNum == cent->currentState.number) {
        ...
        // cg.refdef.vieworg, vec3_origin, 	trap->S_RegisterSound(
        //"sound/weapons/saber/saberhum1.wav" ) );
        VectorCopy(cg.refdef.vieworg, soundSpot);
      } else {
        // trap->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin,
        // vec3_origin, 	trap->S_RegisterSound(
        //"sound/weapons/saber/saberhum1.wav" ) );
        VectorCopy(cent->lerpOrigin, soundSpot);
      }

      if (ci->saber[0].model[0] && ci->saber[0].soundLoop &&
        ...
        if (hasLen) {
          trap->S_AddLoopingSound(cent->currentState.number, soundSpot,
                                  vec3_origin, ci->saber[0].soundLoop);
          didFirstSound = qtrue;
        }
      }
      if (ci->saber[1].model[0] && ci->saber[1].soundLoop &&
          (!didFirstSound ||
        ...
        if (hasLen) {
          trap->S_AddLoopingSound(cent->currentState.number, soundSpot,
                                  vec3_origin, ci->saber[1].soundLoop);
        }
      }
    }
    ...
    if (iwantout && !cent->currentState.saberInFlight) {
      if (cent->currentState.eFlags & EF_DEAD) {
        if (cent->ghoul2 && cent->currentState.saberInFlight &&
            g2HasWeapon) { // special case, kill the saber on a freshly dead
                           // player if another source says to.
          trap->G2API_RemoveGhoul2Model(&(cent->ghoul2), 1);
          g2HasWeapon = qfalse;
        }
      }
    ...
    if (g2HasWeapon &&
        cent->currentState
            .saberInFlight) { // keep this set, so we don't re-unholster the
                              // thing when we get it back, even if it's knocked
                              // away.
    ...
    if (cent->currentState.saberInFlight && cent->currentState.saberEntityNum) {
      ...
      if (/*!cent->bolt4 &&*/ g2HasWeapon || !cent->bolt3 ||
          saberEnt->serverSaberHitIndex !=
              saberEnt->currentState
                  .modelindex /*|| !cent->saberLength*/) { // saber is in
                                                           // flight, do not have
                                                           // it as a standard
                                                           // weapon model
        ...
        if (g2HasWeapon) {
          ...
          trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_rhand, &boltMat,
                                    cent->turAngles, cent->lerpOrigin, cg.time,
                                    cgs.gameModels, cent->modelScale);
          BG_GiveMeVectorFromMatrix(&boltMat, ORIGIN,
                                    saberEnt->currentState.pos.trBase);
        ...
        if (saberEnt->ghoul2 && saberEnt->serverSaberHitIndex ==
                                    saberEnt->currentState.modelindex) {
          // now set up the gun bolt on it
          addBolts = qtrue;
        } else {
          const char *saberModel =
              CG_ConfigString(CS_MODELS + saberEnt->currentState.modelindex);

          saberEnt->serverSaberHitIndex = saberEnt->currentState.modelindex;

          if (saberEnt->ghoul2) { // clean if we already have one (because
                                  // server changed model string index)
            trap->G2API_CleanGhoul2Models(&(saberEnt->ghoul2));
            saberEnt->ghoul2 = 0;
          }

          if (saberModel && saberModel[0]) {
            trap->G2API_InitGhoul2Model(&saberEnt->ghoul2, saberModel, 0, 0, 0,
                                        0, 0);
          } else if (ci->saber[0].model[0]) {
            trap->G2API_InitGhoul2Model(&saberEnt->ghoul2, ci->saber[0].model,
                                        0, 0, 0, 0, 0);
          } else {
            trap->G2API_InitGhoul2Model(&saberEnt->ghoul2,
                                        "models/weapons2/saber/saber_w.glm", 0,
                                        0, 0, 0, 0);
          }
          // trap->G2API_DuplicateGhoul2Instance(cent->ghoul2,
          // &saberEnt->ghoul2);
          ...
          if (saberEnt->ghoul2) {
            ...
            VectorCopy(saberEnt->currentState.pos.trBase, saberEnt->lerpOrigin);
            VectorCopy(saberEnt->currentState.apos.trBase,
                       saberEnt->lerpAngles);
            saberEnt->currentState.pos.trTime = cg.time;
            saberEnt->currentState.apos.trTime = cg.time;
          }
        ...
        if (addBolts) {
          ...
          while (m < ci->saber[0].numBlades) {
            ...
            if (tagBolt == -1) {
              if (m == 0) { // guess this is an 0ldsk3wl saber
      ...
      if (saberEnt && saberEnt->ghoul2 /*&& cent->bolt4 == 2*/) {
        ...
        if (!saberEnt->currentState.saberInFlight &&
            saberEnt->currentState.bolt2 != 123) { // owner is pulling is back
        ...
        // We don't actually want to rely entirely on server updates to render
        // the position of the saber, because we actually know generally where
        // it's going to be before the first position update even gets here, and
        // it needs to start getting rendered the instant the saber model is
        // removed from the player hand. So we'll just render it manually and
        // let normal rendering for the entity be ignored.
        if (!saberEnt->currentState.saberInFlight &&
            saberEnt->currentState.bolt2 !=
                123) { // tell it that we're a saber and to render the glow
                       // around our handle because we're being pulled back
          saberEnt->bolt3 = 999;
        }
        ...
        while (l < 1) {
          ...
          while (k < ci->saber[l].numBlades) {
            if ( // cent->currentState.fireflag == SS_STAFF&& //in saberstaff
                 // style
                l == 0 // first saber
                && cent->currentState.saberHolstered ==
                       1  // extra blades should be off
                && k > 0) // this is an extra blade
            {             // extra blades off
              // don't draw them
              CG_AddSaberBlade(cent, saberEnt, NULL, 0, 0, l, k,
                               saberEnt->lerpOrigin, bladeAngles, qtrue, qtrue);
            } else {
              CG_AddSaberBlade(cent, saberEnt, NULL, 0, 0, l, k,
                               saberEnt->lerpOrigin, bladeAngles, qtrue,
                               qfalse);
            }

            k++;
          }
          if (ci->saber[l].numBlades >
              2) { // add a single glow for the saber based on all the blade
                   // colors combined
        ...
        trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_rhand, &boltMatrix,
                                  tAng, cent->lerpOrigin, cg.time,
                                  cgs.gameModels, cent->modelScale);
        ...
        // trap->FX_AddSprite( NULL, efOrg, NULL, NULL, 8.0f, 8.0f, wv, wv,
        // 0.0f, 0.0f, 1.0f, cgs.media.yellowSaberGlowShader, 0x08000000 );
  ...
  if (drawPlayerSaber) {
    ...
    if (!cent->currentState.saberEntityNum) {
      l = 1; // The "primary" saber is missing or in flight or something, so
             // only try to draw in the second one
    } else if (!cent->currentState.saberInFlight) {
      saberEnt = &cg_entities[cent->currentState.saberEntityNum];

      if (/*cent->bolt4 && */ !g2HasWeapon) {
        trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_SABER),
                                            0, cent->ghoul2, 1);
    ...
    while (l < MAX_SABERS) {
      ...
      while (k < ci->saber[l].numBlades) {
        ...
        {                                          // extra blades off
          // don't draw them
          CG_AddSaberBlade(cent, cent, NULL, 0, 0, l, k, legs.origin,
                           rootAngles, qfalse, qtrue);
        ...
        { // second saber is turned off and this blade is done with turning off
          ...
          CG_AddSaberBlade(cent, cent, NULL, 0, 0, l, k, legs.origin,
                           rootAngles, qfalse, qfalse);
        }

        k++;
      }
      if (ci->saber[l].numBlades > 2) { // add a single glow for the saber based
                                        // on all the blade colors combined
        CG_DoSaberLight(&ci->saber[l]);
      }
  ...
  if (cent->currentState.saberInFlight &&
      !cent->currentState
           .saberEntityNum) { // reset the length if the saber is knocked away
    BG_SI_SetDesiredLength(&ci->saber[0], 0, -1);
    BG_SI_SetDesiredLength(&ci->saber[1], 0, -1);

    if (g2HasWeapon) { // and remember to kill the bolton model in case we
                       // didn't get a thrown saber update first
      trap->G2API_RemoveGhoul2Model(&(cent->ghoul2), 1);
      g2HasWeapon = qfalse;
    }
    cent->bolt3 = 0;
  ...
  if (cent->currentState.eFlags & EF_DEAD) {
    if (cent->ghoul2 && cent->currentState.saberInFlight &&
        g2HasWeapon) { // special case, kill the saber on a freshly dead player
                       // if another source says to.
      trap->G2API_RemoveGhoul2Model(&(cent->ghoul2), 1);
      g2HasWeapon = qfalse;
    }
  }
  ...
  if (cg.snap->ps.duelInProgress /*&& cent->currentState.number != cg.snap->ps.clientNum*/)
    ...
    if (cent->currentState.number != cg.snap->ps.duelIndex &&
        cent->currentState.number !=
            cg.snap->ps.clientNum) { // everyone not involved in the duel is
                                     // drawn very dark
      legs.shaderRGBA[0] /= 5.0f;
      legs.shaderRGBA[1] /= 5.0f;
      legs.shaderRGBA[2] /= 5.0f;
      legs.renderfx |= RF_RGB_TINT;
    } else { // adjust the glow by how far away you are from your dueling
             // partner
      centity_t *duelEnt;
    ...
    if (cent->currentState.bolt1 && !(cent->currentState.eFlags & EF_DEAD) &&
        cent->currentState.number != cg.snap->ps.clientNum &&
        (!cg.snap->ps.duelInProgress ||
         cg.snap->ps.duelIndex != cent->currentState.number)) {
  ...
  if (cent->currentState.eFlags & EF_DISINTEGRATION) {
    ...
    if ((cg.time - cent->dustTrailTime) >
        1500) { // avoid rendering the entity after disintegration has finished
                // anyway
      // goto endOfCall;
      return;
    }

    trap->G2API_SetBoneAnim(legs.ghoul2, 0, "model_root", cent->miscTime,
                            cent->miscTime, BONE_ANIM_OVERRIDE_FREEZE, 1.0f,
                            cg.time, cent->miscTime, -1);

    if (!cent->noLumbar) {
      trap->G2API_SetBoneAnim(legs.ghoul2, 0, "lower_lumbar", cent->miscTime,
                              cent->miscTime, BONE_ANIM_OVERRIDE_FREEZE, 1.0f,
                              cg.time, cent->miscTime, -1);

      if (cent->localAnimIndex <= 1) {
        trap->G2API_SetBoneAnim(legs.ghoul2, 0, "Motion", cent->miscTime,
                                cent->miscTime, BONE_ANIM_OVERRIDE_FREEZE, 1.0f,
                                cg.time, cent->miscTime, -1);
      }
    }
  ...
  if (cent->uncloaking > cg.time) { // in the middle of cloaking
    if ((cg.snap->ps.fd.forcePowersActive & (1 << FP_SEE)) &&
        cg.snap->ps.clientNum != cent->currentState.number) { // just draw him
      trap->R_AddRefEntityToScene(&legs);
    } else {
      float perc = (float)(cent->uncloaking - cg.time) / 2000.0f;
      if ((cent->currentState.powerups &
           (1 << PW_CLOAKED))) { // actually cloaking, so reverse it
        perc = 1.0f - perc;
      }
  ...
  } else if ((cent->currentState.powerups &
              (1 << PW_CLOAKED))) { // fully cloaked
    if ((cg.snap->ps.fd.forcePowersActive & (1 << FP_SEE)) &&
        cg.snap->ps.clientNum != cent->currentState.number) { // just draw him
      trap->R_AddRefEntityToScene(&legs);
    } else {
      if (cg.renderingThirdPerson ||
        ...
        legs.shaderRGBA[0] = legs.shaderRGBA[1] = legs.shaderRGBA[2] =
        legs.shaderRGBA[3] = 255; legs.customShader = cgs.media.cloakedShader;
        ...
        */
        ...
        if (cg_shadows.integer != 2 && cgs.glconfig.stencilBits >= 4 &&
            cg_renderToTextureFX.integer) {
          trap->R_SetRefractionProperties(
              1.0f, 0.0f, qfalse,
              qfalse);           // don't need to do this every frame.. but..
  ...
  if (!(cent->currentState.powerups &
        (1 << PW_CLOAKED))) { // don't add the normal model if cloaked
  ...
  if (!cent->frame_hold_refreshed &&
      (cent->currentState.powerups & (1 << PW_SPEEDBURST))) {
  ...
  if (cent->frame_hold_time >= cg.time) {
    ...
    if (!cent->frame_hold_refreshed) { // We're taking the ghoul2 instance from
                                       // the original refent and duplicating it
                                       // onto our refent alias so that we can
                                       // then freeze the frame and fade it for
                                       // the effect
      ...
      // trap->G2API_SetBoneAnim(cent->frame_hold.ghoul2, 0, "model_root",
      // cent->frame_hold.frame, cent->frame_hold.frame,
      // BONE_ANIM_OVERRIDE_FREEZE, 1.0f, cg.time, cent->frame_hold.frame, -1);
      trap->G2API_SetBoneAnim(cent->frame_hold, 0, "model_root", legs.frame,
                              legs.frame, 0, 1.0f, cg.time, legs.frame, -1);
    } else {
      reframe_hold = legs;
  ...
  if (CG_VehicleShouldDrawShields(cent) // vehicle
      ||
      (checkDroidShields &&
       CG_VehicleShouldDrawShields(
           &cg_entities[cent->currentState.m_iVehicleNum]))) // droid in vehicle
  { // Vehicles have form-fitting shields
    ...
    legs.shaderRGBA[2] = 255;
    legs.shaderRGBA[3] =
        10.0f + (sin((float)(cg.time / 4)) *
                 128.0f); // 112.0 * ((cent->damageTime - cg.time) /
                          // MIN_SHIELD_TIME) + Q_flrand(0.0f, 1.0f)*16;
    ...
    if (pVeh && pVeh->m_pVehicleInfo &&
        pVeh->m_pVehicleInfo
            ->shieldShaderHandle) { // use the vehicle-specific shader
  ...
  // For now, these two are using the old shield shader. This is just so that
  // you can tell it apart from the JM/duel shaders, but it's still very obvious.
  if (cent->currentState.forcePowersActive &
      (1 << FP_PROTECT)) { // aborb is represented by green..
    refEntity_t prot;
  ...
  // Showing only when the power has been active (absorbed something) recently
  // now, instead of always. AND always show if it is you with the absorb on
  if ((cent->currentState.number == cg.predictedPlayerState.clientNum &&
       (cg.predictedPlayerState.fd.forcePowersActive & (1 << FP_ABSORB))) ||
      (cent->teamPowerEffectTime > cg.time &&
       cent->teamPowerType == 3)) { // aborb is represented by blue..
    legs.shaderRGBA[0] = 0;
    legs.shaderRGBA[1] = 0;
    legs.shaderRGBA[2] = 255;
  ...
  if ((cg.snap->ps.fd.forcePowersActive & (1 << FP_SEE)) &&
    ...
    if (cgs.gametype == GT_SIEGE) {                              // A team game
      if (ci->team == TEAM_SPECTATOR || ci->team == TEAM_FREE) { // yellow
        legs.shaderRGBA[0] = 255;
        legs.shaderRGBA[1] = 255;
        legs.shaderRGBA[2] = 0;
      } else if (ci->team != cgs.clientinfo[cg.snap->ps.clientNum].team) { // red
        legs.shaderRGBA[0] = 255;
        legs.shaderRGBA[1] = 50;
        legs.shaderRGBA[2] = 50;
      } else { // green
    ...
    } else if (cgs.gametype >= GT_TEAM) { // A team game
      switch (ci->team) {
      case TEAM_RED:
        legs.shaderRGBA[0] = 255;
        legs.shaderRGBA[1] = 50;
    ...
    */
    { // See through walls.
      ...
      if (cg.snap->ps.fd.forcePowerLevel[FP_SEE] <
          FORCE_LEVEL_2) { // only level 2+ can see players through walls
        legs.renderfx &= ~RF_NODEPTH;
      }
    }
  ...
  if (cent->currentState.emplacedOwner > cg.time) {
    ...
    CG_ForceElectrocution(cent, legs.origin, tempAngles, cgs.media.boltShader,
                          qfalse);
  }

  if (cent->currentState.powerups & (1 << PW_SHIELDHIT)) {
    /*
    legs.shaderRGBA[0] = legs.shaderRGBA[1] = legs.shaderRGBA[2] = 255.0f *
    0.5f;//t; legs.shaderRGBA[3] = 255; legs.renderfx &= ~RF_ALPHA_FADE;
    legs.renderfx |= RF_RGB_TINT;
    */
...
void CG_ResetPlayerEntity(centity_t *cent) {
  ...
  if (cent->currentState.eType == ET_NPC) {
    if (cent->currentState.NPC_class == CLASS_VEHICLE && cent->m_pVehicle &&
        ...
        cg.predictedPlayerState.m_iVehicleNum &&
        cent->currentState.number ==
            cg.predictedPlayerState.m_iVehicleNum) { // holy hackery, batman!
  ...
  if (cent->currentState.eType != ET_NPC ||
      !(cent->currentState.eFlags & EF_DEAD)) {
    CG_ClearLerpFrame(cent, ci, &cent->pe.legs, cent->currentState.legsAnim,
                      qfalse);
    CG_ClearLerpFrame(cent, ci, &cent->pe.torso, cent->currentState.torsoAnim,
                      qtrue);
    ...
    if (cent->currentState.eType == ET_NPC) { // just start them off at 0 pitch
  ...
  // do this to prevent us from making a saber unholster sound the first time we
  // enter the pvs
  if (cent->currentState.number != cg.predictedPlayerState.clientNum &&
      cent->currentState.weapon == WP_SABER &&
      cent->weapon != cent->currentState.weapon) {
