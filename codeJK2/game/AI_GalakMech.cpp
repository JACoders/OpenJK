...
extern qboolean WP_LobFire(gentity_t *self, vec3_t start, vec3_t target,
                           vec3_t mins, vec3_t maxs, int clipmask,
                           vec3_t velocity, qboolean tracePath,
                           int ignoreEntNum, int enemyNum, float minSpeed,
                           float maxSpeed, float idealSpeed, qboolean mustHit);
...
void NPC_GalakMech_Init(gentity_t *ent) {
  if (ent->NPC->behaviorState != BS_CINEMATIC) {
    ...
    gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel], "torso_shield_off",
                             TURN_ON);
    gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel],
                             "torso_galakface_off", TURN_OFF);
    gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel],
                             "torso_galakhead_off", TURN_OFF);
    gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel],
                             "torso_eyes_mouth_off", TURN_OFF);
    gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel], "torso_collar_off",
                             TURN_OFF);
    gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel],
                             "torso_galaktorso_off", TURN_OFF);
  } else {
    //		gi.G2API_SetSurfaceOnOff( &ent->ghoul2[ent->playerModel], "helmet",
    //TURN_OFF );
    gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel], "torso_shield_off",
                             TURN_OFF);
    gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel],
                             "torso_galakface_off", TURN_ON);
    gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel],
                             "torso_galakhead_off", TURN_ON);
    gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel],
                             "torso_eyes_mouth_off", TURN_ON);
    gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel], "torso_collar_off",
                             TURN_ON);
    gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel],
                             "torso_galaktorso_off", TURN_ON);
  }
}

//-----------------------------------------------------------------
static void GM_CreateExplosion(gentity_t *self, const int boltID,
                               qboolean doSmall = qfalse) {
  if (boltID >= 0) {
    mdxaBone_t boltMatrix;
    vec3_t org, dir;

    gi.G2API_GetBoltMatrix(self->ghoul2, self->playerModel, boltID, &boltMatrix,
...
void GM_Dying(gentity_t *self) {
  if (level.time - self->s.time < 4000) { // FIXME: need a real effect
    ...
    if (TIMER_Done(self, "dyingExplosion")) {
      ...
      case 1:
        if (!gi.G2API_GetSurfaceRenderStatus(&self->ghoul2[self->playerModel],
                                             "r_hand")) { // r_hand still there
          GM_CreateExplosion(self, self->handRBolt, qtrue);
          gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel], "r_hand",
                                   TURN_OFF);
        } else if (!gi.G2API_GetSurfaceRenderStatus(
                       &self->ghoul2[self->playerModel],
                       "r_arm_middle")) { // r_arm_middle still there
          newBolt = gi.G2API_AddBolt(&self->ghoul2[self->playerModel],
                                     "*r_arm_elbow");
          gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel],
                                   "r_arm_middle", TURN_OFF);
        }
        break;
      case 2:
        // FIXME: do only once?
        if (!gi.G2API_GetSurfaceRenderStatus(&self->ghoul2[self->playerModel],
                                             "l_hand")) { // l_hand still there
          GM_CreateExplosion(self, self->handLBolt);
          gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel], "l_hand",
                                   TURN_OFF);
        } else if (!gi.G2API_GetSurfaceRenderStatus(
                       &self->ghoul2[self->playerModel],
                       "l_arm_wrist")) { // l_arm_wrist still there
          newBolt = gi.G2API_AddBolt(&self->ghoul2[self->playerModel],
                                     "*l_arm_cap_l_hand");
          gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel],
                                   "l_arm_wrist", TURN_OFF);
        } else if (!gi.G2API_GetSurfaceRenderStatus(
                       &self->ghoul2[self->playerModel],
                       "l_arm_middle")) { // l_arm_middle still there
          newBolt = gi.G2API_AddBolt(&self->ghoul2[self->playerModel],
                                     "*l_arm_cap_l_hand");
          gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel],
                                   "l_arm_middle", TURN_OFF);
        } else if (!gi.G2API_GetSurfaceRenderStatus(
                       &self->ghoul2[self->playerModel],
                       "l_arm_augment")) { // l_arm_augment still there
          newBolt = gi.G2API_AddBolt(&self->ghoul2[self->playerModel],
                                     "*l_arm_elbow");
          gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel],
                                   "l_arm_augment", TURN_OFF);
        }
        break;
      case 3:
      case 4:
      ...
      case 6:
        newBolt =
            gi.G2API_AddBolt(&self->ghoul2[self->playerModel], "*shldr_l");
        GM_CreateExplosion(self, newBolt);
        break;
      case 7:
      case 8:
        newBolt =
            gi.G2API_AddBolt(&self->ghoul2[self->playerModel], "*uchest_r");
        GM_CreateExplosion(self, newBolt);
        break;
      case 9:
      case 10:
      ...
      case 11:
        newBolt =
            gi.G2API_AddBolt(&self->ghoul2[self->playerModel], "*l_leg_knee");
        GM_CreateExplosion(self, newBolt, qtrue);
        break;
      case 12:
        newBolt =
            gi.G2API_AddBolt(&self->ghoul2[self->playerModel], "*r_leg_knee");
        GM_CreateExplosion(self, newBolt, qtrue);
        break;
      case 13:
        newBolt =
            gi.G2API_AddBolt(&self->ghoul2[self->playerModel], "*l_leg_foot");
        GM_CreateExplosion(self, newBolt, qtrue);
        break;
      case 14:
        newBolt =
    ...
    //		G_PlayEffect( "env/exp_trail_comp", self->currentOrigin,
    //self->currentAngles );
...
void NPC_GM_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                 vec3_t point, int damage, int mod, int hitLoc) {
  if (self->client->ps.powerups[PW_GALAK_SHIELD] ==
      0) { // shield is currently down
    // FIXME: allow for radius damage?
    if ((hitLoc == HL_GENERIC1) &&
        (self->locationDamage[HL_GENERIC1] > GENERATOR_HEALTH)) {
      int newBolt =
          gi.G2API_AddBolt(&self->ghoul2[self->playerModel], "*antenna_base");
      if (newBolt != -1) {
        GM_CreateExplosion(self, newBolt);
      }
      ...
      gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel],
                               "torso_antenna", TURN_OFF);
      gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel],
                               "torso_antenna_base_cap_off", TURN_ON);
  ...
  if (!self->lockCount &&
      !self->client->ps.torsoAnimTimer) { // don't interrupt laser sweep attack
                                          // or other special attacks/moves
  ...
  if (inflictor && inflictor->lastEnemy ==
                       self) { // He force-pushed my own lobfires back at me
...
static void GM_CheckMoveState(void) {
  ...
  if ((NPCInfo->goalEntity != NPC->enemy) && (NPCInfo->goalEntity != NULL)) {
    ...
    if (NAV_HitNavGoal(NPC->currentOrigin, NPC->mins, NPC->maxs,
                       NPCInfo->goalEntity->currentOrigin, 16, qfalse) ||
        (!Q3_TaskIDPending(NPC, TID_MOVE_NAV) && enemyLOS &&
    ...
    if (level.time - NPCInfo->enemyLastSeenTime < 10000) {
      if (!Q_irand(0, 10)) {
        ...
        if (dist < distThreshold) { // impact would be too close to me
          tooClose = qtrue;
        } else if (level.time - NPCInfo->enemyLastSeenTime >
                   5000) { // we've haven't seen them in the last 5 seconds
...
void NPC_GM_StartLaser(void) {
  if (!NPC->lockCount) { // haven't already started a laser attack
    ...
    NPC_SetAnim(NPC, SETANIM_TORSO, TORSO_RAISEWEAP2,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void GM_StartGloat(void) {
  NPC->wait = 0;
  gi.G2API_SetSurfaceOnOff(&NPC->ghoul2[NPC->playerModel],
                           "torso_galakface_off", TURN_ON);
  gi.G2API_SetSurfaceOnOff(&NPC->ghoul2[NPC->playerModel],
                           "torso_galakhead_off", TURN_ON);
  gi.G2API_SetSurfaceOnOff(&NPC->ghoul2[NPC->playerModel],
                           "torso_eyes_mouth_off", TURN_ON);
  gi.G2API_SetSurfaceOnOff(&NPC->ghoul2[NPC->playerModel], "torso_collar_off",
                           TURN_ON);
  gi.G2API_SetSurfaceOnOff(&NPC->ghoul2[NPC->playerModel],
                           "torso_galaktorso_off", TURN_ON);
  NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_STAND2TO1,
              SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void NPC_BSGM_Attack(void) {
  ...
  if (NPC->enemy && NPC->enemy->health <= 0 &&
      !NPC->enemy->s.number) { // my enemy is dead
    if (NPC->client->ps.torsoAnim == BOTH_STAND2TO1) {
      if (NPC->client->ps.torsoAnimTimer <= 500) {
        ...
        NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_TRIUMPHANT1START,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        NPC->client->ps.legsAnimTimer += 500;
        NPC->client->ps.torsoAnimTimer += 500;
      }
      ...
      if (NPC->client->ps.torsoAnimTimer <= 500) {
        NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_TRIUMPHANT1STARTGESTURE,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        NPC->client->ps.legsAnimTimer += 500;
        NPC->client->ps.torsoAnimTimer += 500;
      }
      ...
      if (NPC->client->ps.torsoAnimTimer <= 500) {
        NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_TRIUMPHANT1STOP,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        NPC->client->ps.legsAnimTimer += 500;
        NPC->client->ps.torsoAnimTimer += 500;
      }
      ...
      if (NPC->client->ps.torsoAnimTimer <= 500) {
        NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_STAND1,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      ...
      if (TIMER_Done(NPC, "gloatTime")) {
        GM_StartGloat();
      } else if (DistanceHorizontalSquared(NPC->client->renderInfo.eyePoint,
  ...
  if (NPC->client->ps.torsoAnim == BOTH_ATTACK4 ||
    ...
    shoot = qfalse;
    if (TIMER_Done(NPC, "smackTime") &&
        !NPCInfo->blockedDebounceTime) { // time to smack
      ...
      if (enemyDist < MELEE_DIST_SQUARED &&
        ...
        G_Damage(NPC->enemy, NPC, NPC, smackDir, NPC->currentOrigin,
                 (g_spskill->integer + 1) * Q_irand(5, 10),
        ...
        if (NPC->client->ps.torsoAnim == BOTH_ATTACK4) { // smackdown
          ...
          if (PM_CrouchAnim(
                  NPC->enemy->client->ps.legsAnim)) { // knockdown from crouch
          ...
          NPC_SetAnim(NPC->enemy, SETANIM_BOTH, knockAnim,
                      SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        } else { // uppercut
          // throw them
          G_Throw(NPC->enemy, smackDir, 100);
          // make them backflip
          NPC_SetAnim(NPC->enemy, SETANIM_BOTH, BOTH_KNOCKDOWN5,
                      SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        }
  ...
  { // sometimes use the laser beam attack, but only after he's taken down our
    // generator
    shoot = qfalse;
    if (NPC->lockCount == 1) {            // charging up
      if (TIMER_Done(NPC, "beamDelay")) { // time to start the beam
        ...
        NPC_SetAnim(NPC, SETANIM_BOTH, laserAnim,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        TIMER_Set(NPC, "attackDelay",
                  NPC->client->ps.torsoAnimTimer + Q_irand(1000, 3000));
        // turn on beam effect
        NPC->lockCount = 2;
      ...
      if (!NPC->client->ps.torsoAnimTimer) { // attack done!
        NPC->lockCount = 0;
        G_FreeEntity(NPCInfo->coverTarg);
        NPC->s.loopSound = 0;
        NPC_SetAnim(NPC, SETANIM_TORSO, TORSO_DROPWEAP2,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        ...
        VectorMA(NPC->client->renderInfo.muzzlePoint, 1024,
                 NPC->client->renderInfo.muzzleDir, end);
        gi.trace(&trace, NPC->client->renderInfo.muzzlePoint, mins, maxs, end,
                 NPC->s.number, MASK_SHOT, G2_NOCOLLIDE, 0);
        if (trace.allsolid || trace.startsolid) { // oops, in a wall
          ...
          if (trace.fraction < 1.0f) { // hit something
            ...
            if (traceEnt && traceEnt->takedamage) { // damage it
              G_SoundAtSpot(
                  trace.endpos,
                  G_SoundIndex("sound/weapons/galak/laserdamage.wav"));
              G_Damage(traceEnt, NPC, NPC, NPC->client->renderInfo.muzzleDir,
                       trace.endpos, 10, 0, MOD_ENERGY);
            }
          }
          if (NPCInfo->coverTarg) {
          ...
          if (!Q_irand(0, 5)) {
            G_SoundAtSpot(trace.endpos,
                          G_SoundIndex("sound/weapons/galak/laserdamage.wav"));
          }
        }
      }
    }
  } else { // Okay, we're not in a special attack, see if we should switch
           // weapons or start a special attack
    /*
    if ( NPC->s.weapon == WP_REPEATER
    ...
    (Q_irand( 0, 1 )||enemyDist < MAX_LOB_DIST_SQUARED) )
    ...
    { // our shield is down, and enemy within 80, if very close, use melee
      // attack to slap away
      if (TIMER_Done(NPC, "attackDelay")) {
        // animate me
        int swingAnim;
        if (NPC->locationDamage[HL_GENERIC1] >
            GENERATOR_HEALTH) { // generator down, use random melee
          swingAnim =
              Q_irand(BOTH_ATTACK4, BOTH_ATTACK5); // smackdown or uppercut
        } else {                                   // always knock-away
          swingAnim = BOTH_ATTACK5;                // uppercut
        }
        // FIXME: swing sound
        NPC_SetAnim(NPC, SETANIM_BOTH, swingAnim,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        TIMER_Set(NPC, "attackDelay",
                  NPC->client->ps.torsoAnimTimer + Q_irand(1000, 3000));
    ...
    } else if (!NPC->lockCount &&
               NPC->locationDamage[HL_GENERIC1] > GENERATOR_HEALTH &&
               TIMER_Done(NPC, "attackDelay") &&
               InFront(NPC->enemy->currentOrigin, NPC->currentOrigin,
                       NPC->client->ps.viewangles, 0.3f) &&
               ((!Q_irand(0, 10 * (2 - g_spskill->integer)) &&
                 enemyDist > MIN_LOB_DIST_SQUARED &&
                 enemyDist < MAX_LOB_DIST_SQUARED) ||
                (!TIMER_Done(NPC, "noLob") && !TIMER_Done(NPC, "noRapid"))) &&
               NPC->enemy->s.weapon !=
                   WP_TURRET) { // sometimes use the laser beam attack, but only
                                // after he's taken down our generator
      shoot = qfalse;
      NPC_GM_StartLaser();
    ...
    {                                      // enemy within 256
      if ((NPC->client->ps.weapon == WP_REPEATER) &&
          (NPCInfo->scriptFlags &
           SCF_ALT_FIRE)) { // shooting an explosive, but enemy too close,
                            // switch to primary fire
        NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;
        NPC->alt_fire = qfalse;
        // FIXME: use weap raise & lower anims
        NPC_ChangeWeapon(WP_REPEATER);
      }
    } else if ((enemyDist > MAX_LOB_DIST_SQUARED ||
                (NPC->enemy->s.weapon == WP_TURRET &&
                 !Q_stricmp("PAS", NPC->enemy->classname))) &&
    ...
    { // enemy more than 448 away and we are ready to try lob fire again
      if ((NPC->client->ps.weapon == WP_REPEATER) &&
          !(NPCInfo->scriptFlags &
            SCF_ALT_FIRE)) { // enemy far enough away to use lobby explosives
        NPCInfo->scriptFlags |= SCF_ALT_FIRE;
        NPC->alt_fire = qtrue;
        // FIXME: use weap raise & lower anims
        NPC_ChangeWeapon(WP_REPEATER);
  ...
  if (NPC_ClearLOS(NPC->enemy)) {
    NPCInfo->enemyLastSeenTime =
        level.time; // used here for aim debouncing, not always a clear LOS
    enemyLOS = qtrue;
    ...
    if (NPC->client->ps.weapon == WP_NONE) {
      ...
      if (((NPC->client->ps.weapon == WP_REPEATER &&
            (NPCInfo->scriptFlags & SCF_ALT_FIRE))) &&
          enemyDist < MIN_LOB_DIST_SQUARED) // 256
      {
        enemyCS = qfalse; // not true, but should stop us from firing
        hitAlly = qtrue;  // us!
        ...
        if (hit == NPC->enemy->s.number ||
            ...
            (hitEnt &&
             hitEnt->takedamage)) { // can hit enemy or will hit glass or other
                                    // breakable, so shoot anyway
          enemyCS = qtrue;
          NPC_AimAdjust(
    ...
    if (hit == NPC->enemy->s.number ||
        ...
        (hitEnt && hitEnt->takedamage)) { // can hit enemy or will hit glass or
                                          // other breakable, so shoot anyway
      enemyCS = qtrue;
    } else {
      faceEnemy = qtrue;
  ...
  if (enemyCS) {
    shoot = qtrue;
    // NPCInfo->enemyCheckDebounceTime = level.time;//actually used here as a
    // last actual LOS
  } else {
    if (!NPCInfo->goalEntity) {
      NPCInfo->goalEntity = NPC->enemy;
  ...
  if (NPC->client->ps.weapon == WP_REPEATER &&
      (NPCInfo->scriptFlags & SCF_ALT_FIRE) && shoot &&
      TIMER_Done(NPC, "attackDelay")) {
    vec3_t muzzle;
    vec3_t angles;
    vec3_t target;
    vec3_t velocity = {0, 0, 0};
    vec3_t mins = {-REPEATER_ALT_SIZE, -REPEATER_ALT_SIZE, -REPEATER_ALT_SIZE},
           maxs = {REPEATER_ALT_SIZE, REPEATER_ALT_SIZE, REPEATER_ALT_SIZE};

    CalcEntitySpot(NPC, SPOT_WEAPON, muzzle);

    VectorCopy(NPC->enemy->currentOrigin, target);

    target[0] += Q_flrand(-5, 5) +
                 (Q_flrand(-1.0f, 1.0f) * (6 - NPCInfo->currentAim) * 2);
    target[1] += Q_flrand(-5, 5) +
                 (Q_flrand(-1.0f, 1.0f) * (6 - NPCInfo->currentAim) * 2);
    target[2] += Q_flrand(-5, 5) +
                 (Q_flrand(-1.0f, 1.0f) * (6 - NPCInfo->currentAim) * 2);

    // Find the desired angles
    qboolean clearshot =
        WP_LobFire(NPC, muzzle, target, mins, maxs,
                   MASK_SHOT | CONTENTS_LIGHTSABER, velocity, qtrue,
                   NPC->s.number, NPC->enemy->s.number, 300, 1100, 1500, qtrue);
    if (VectorCompare(vec3_origin, velocity) ||
        (!clearshot && enemyLOS &&
         enemyCS)) { // no clear lob shot and no lob shot that will hit
                     // something breakable
      if (enemyLOS && enemyCS &&
          TIMER_Done(
              NPC,
              "noRapid")) { // have a clear straight shot, so switch to primary
        NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;
        NPC->alt_fire = qfalse;
        NPC_ChangeWeapon(WP_REPEATER);
  ...
  } else if (faceEnemy) { // face the enemy
    NPC_FaceEnemy(qtrue);
  }
  ...
  if (AImove && !NPC->lockCount) { // move toward goal
    if (NPCInfo->goalEntity && NPC->client->ps.legsAnim != BOTH_ALERT1 &&
  ...
  if (NPC->enemy->s.weapon == WP_TURRET &&
      !Q_stricmp("PAS", NPC->enemy->classname)) { // crush turrets
    if (G_BoundsOverlap(
            NPC->absmin, NPC->absmax, NPC->enemy->absmin,
            NPC->enemy->absmax)) { // have to do this test because placed
                                   // turrets are not solid to NPCs (so they
                                   // don't obstruct navigation)
      if (NPC->client->ps.powerups[PW_GALAK_SHIELD] > 0) {
        NPC->client->ps.powerups[PW_BATTLESUIT] =
            level.time + ARMOR_EFFECT_TIME;
        G_Damage(NPC->enemy, NPC, NPC, NULL, NPC->currentOrigin, 100,
                 DAMAGE_NO_KNOCKBACK, MOD_ELECTROCUTE);
      } else {
        G_Damage(NPC->enemy, NPC, NPC, NULL, NPC->currentOrigin, 100,
                 DAMAGE_NO_KNOCKBACK, MOD_CRUSH);
      }
    }
  } else if (NPCInfo->touchedByPlayer != NULL &&
             NPCInfo->touchedByPlayer == NPC->enemy) {   // touched enemy
    if (NPC->client->ps.powerups[PW_GALAK_SHIELD] > 0) { // zap him!
      // animate me
      NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_ATTACK6,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      TIMER_Set(NPC, "attackDelay", NPC->client->ps.torsoAnimTimer);
      ...
      VectorNormalize(smackDir);
      G_Damage(NPC->enemy, NPC, NPC, smackDir, NPC->currentOrigin,
               (g_spskill->integer + 1) * Q_irand(5, 10), DAMAGE_NO_KNOCKBACK,
               MOD_ELECTROCUTE);
      // throw them
      G_Throw(NPC->enemy, smackDir, 100);
...
void NPC_BSGM_Default(void) {
  ...
  if (NPC->client->ps.stats[STAT_ARMOR] <= 0) { // armor gone
    if (!NPCInfo->investigateDebounceTime) {    // start regenerating the armor
      gi.G2API_SetSurfaceOnOff(&NPC->ghoul2[NPC->playerModel],
                               "torso_shield_off", TURN_OFF);
      ...
      if (NPC->locationDamage[HL_GENERIC1] <
          GENERATOR_HEALTH) { // still have the generator bolt-on
        if (NPCInfo->investigateCount < 12) {
          NPCInfo->investigateCount++;
        }
        NPCInfo->investigateDebounceTime =
            level.time + (NPCInfo->investigateCount * 5000);
      }
    } else if (NPCInfo->investigateDebounceTime <
               level.time) { // armor regenerated, turn shield back on
      // do a trace and make sure we can turn this back on?
      trace_t tr;
      gi.trace(&tr, NPC->currentOrigin, shieldMins, shieldMaxs,
               NPC->currentOrigin, NPC->s.number, NPC->clipmask, G2_NOCOLLIDE,
               0);
      if (!tr.startsolid) {
        ...
        gi.G2API_SetSurfaceOnOff(&NPC->ghoul2[NPC->playerModel],
                                 "torso_shield_off", TURN_ON);
      }
    }
  }
  if (NPC->client->ps.stats[STAT_ARMOR] > 0) {               // armor present
    NPC->client->ps.powerups[PW_GALAK_SHIELD] = Q3_INFINITE; // temp, for effect
    gi.G2API_SetSurfaceOnOff(&NPC->ghoul2[NPC->playerModel], "torso_shield_off",
                             TURN_ON);
  } else {
    gi.G2API_SetSurfaceOnOff(&NPC->ghoul2[NPC->playerModel], "torso_shield_off",
                             TURN_OFF);
