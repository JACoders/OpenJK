...
// extern void CG_ShimmeryThing_Spawner( vec3_t start, vec3_t end, float radius,
// qboolean taper, int duration );
    ...
    if (ent->client->NPC_class == CLASS_SEEKER) {
      ...
      if (ent->client->ps.weapon != WP_NONE) {
        G_CreateG2AttachedWeaponModel(
            ent, weaponData[ent->client->ps.weapon].weaponMdl);
      }
  ...
  case TEAM_ENEMY: {
    ...
    if (ent->client->NPC_class == CLASS_TAVION ||
      ...
      case WP_DISRUPTOR:
        // Sniper
        ent->NPC->scriptFlags |=
            SCF_ALT_FIRE; // FIXME: use primary fire sometimes?  Up close?
                          // Different class of NPC?
        break;
      case WP_BOWCASTER:
        break;
      case WP_REPEATER:
...
void NPC_SetWeapons(gentity_t *ent) {
  ...
  // these teams are gone now anyway, plus all team stuff should be read in from
  // the config file
  /*
          switch ( ent->client->playerTeam )
          {
...
void NPC_SpawnEffect(gentity_t *ent) {
  ...
  // NOTE:  Make sure any effects called here have their models, tga's and
sounds precached in
...
void NPC_SetFX_SpawnStates(gentity_t *ent) {
  ...
  // stasis, 8472, etc no longer exist. However if certain NPC's need customized
  // spawn effects, use NPC_class instead of TEAM_
  /*
          // -Etherians -------
          if ( ent->client->playerTeam == TEAM_STASIS )
          {
                  ...
                  // The spawn effect can happen, so it's ok to do this extra
     setup stuff for the effect. ent->fx_time = level.time; ent->s.eFlags |=
     EF_SCALE_UP; ent->client->ps.eFlags |= EF_SCALE_UP;
...
void NPC_Begin(gentity_t *ent) {
  ...
  if (ent->client->playerTeam == TEAM_ENEMY) { // valid enemy spawned
  ...
  */
  ...
  if (ent->spawnflags & NSF_DROP_TO_FLOOR) {
    ...
    gi.trace(&tr, ent->currentOrigin, ent->mins, ent->maxs, bottom,
             ent->s.number, MASK_NPCSOLID, G2_NOCOLLIDE, 0);
...
/*QUAK-ED NPC_spawner (1 0 0) (-16 -16 -24) (16 16 32) x x x x DROPTOFLOOR
...
// STAR WARS
// NPCs============================================================================
/*QUAKED NPC_***** (1 0 0) (-16 -16 -24) (16 16 32) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY
...
/*QUAKED NPC_Jedi(1 0 0) (-16 -16 -24) (16 16 40) TRAINER x x x CEILING
CINEMATIC NOTSOLID STARTINSOLID SHY TRAINER - Special Jedi- instructor CEILING -
...
static void NPC_Spawn_f(void) {
  ...
  gi.trace(&trace, trace.endpos, NULL, NULL, end, 0, MASK_SOLID, G2_NOCOLLIDE,
           0);
  VectorCopy(trace.endpos, end);
  end[2] += 24;
