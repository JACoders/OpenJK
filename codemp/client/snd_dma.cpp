...
void S_Init(void) {
  ...
  s_volumeVoice = Cvar_Get("s_volumeVoice", "1.0", CVAR_ARCHIVE,
                           "Volume for voice channels");
  s_musicVolume =
      Cvar_Get("s_musicvolume", "0.25", CVAR_ARCHIVE, "Music Volume");
...
=================
*/
void S_SpatializeOrigin(const vec3_t origin, float master_vol, int *left_vol,
                        int *right_vol, int channel) {
  float dot;
  float dist;
...
====================
*/
void S_StartSound(const vec3_t origin, int entityNum, int entchannel,
...
void S_AddLoopingSound(int entityNum, const vec3_t origin,
  ...
  if (s_doppler->integer && VectorLengthSquared(velocity) > 0.0) {
    ...
    VectorAdd(loopSounds[numLoopSounds].origin,
              loopSounds[numLoopSounds].velocity, out);
  ...
  for (i = 0; i < numLoopSounds; i++) {
    ...
    for (j = i; j < numLoopSounds; j++) {
      ...
      S_SpatializeOrigin(loop2->origin, loop2->volume, &left, &right,
                         CHAN_AUTO); // FIXME: Allow for volume change!!
...
static int S_CheckAmplitude(channel_t *ch,
                            const unsigned int s_oldpaintedtime) {
...
qboolean S_ScanChannelStarts(void) {
  ...
  for (i = 0; i < MAX_CHANNELS; i++, ch++) {
    ...
    if ((int)(ch->startSample + ch->thesfx->iSoundLengthInSamples) <=
        s_paintedtime) {
      ch->thesfx = NULL;
      continue;
    }
...
void S_GetSoundtime(void) {
  ...
  if (CL_VideoRecording()) {
    ...
    float frameDuration =
        Q_max(dma.speed / fps, 1.0f) + clc.aviSoundFrameRemainder;
...
#ifdef _DEBUG
            Com_OPrintf("[%d] Restarting playback of single-shot streaming MP3 "
  ...
  for (i = 1; i < s_numChannels; i++, ch++) {
    ...
    if (ch->entchannel == CHAN_VOICE || ch->entchannel == CHAN_VOICE_ATTEN ||
      ...
      if (ch->thesfx->eSoundCompressionMethod == ct_16) {
        ...
        if ((ch->thesfx->lipSyncData) &&
            ((int)samples < ch->thesfx->iSoundLengthInSamples)) {
              ...
              for (k = 1; k < s_numChannels; k++, ch++) {
                ...
                if (lSourceID == i) {
                  if (s_eaxSet(&EAXPROPERTYID_EAX40_Source,
                               ...
                               sizeof(GUID)) != AL_NO_ERROR) {
                    Com_OPrintf("Failed to set Source ActiveFXSlotID to new "
                                "environment\n");
                  }
