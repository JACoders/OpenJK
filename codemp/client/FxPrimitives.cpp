...
bool CParticle::UpdateOrigin(void)
{
	...
	if (((mFlags & FX_APPLY_PHYSICS) || (fx_physics && fx_physics->integer > 1)) && !(mFlags & FX_PLAYER_VIEW))//JAPRO ENGINE
	{
		bool solid;

		if (mFlags & FX_EXPENSIVE_PHYSICS || (fx_physics && fx_physics->integer))//JAPRO ENGINE
		{
