...
void InitTrigger(gentity_t *self) {
  ...
  self->r.contents =
      CONTENTS_TRIGGER; // replaces the -1 from trap->SetBrushModel
...
void multi_trigger_run(gentity_t *ent) {
  ...
  if (ent->genericValue4) { // we want to activate target3 for team1 or target4
                            // for team2
    if (ent->genericValue4 == SIEGETEAM_TEAM1 && ent->target3 &&
        ent->target3[0]) {
      G_UseTargets2(ent, ent->activator, ent->target3);
    } else if (ent->genericValue4 == SIEGETEAM_TEAM2 && ent->target4 &&
               ent->target4[0]) {
  ...
  if (ent->target2 && ent->target2[0] && ent->wait >= 0) {
    ...
    ent->r.contents &= ~CONTENTS_TRIGGER; // so the EntityContact trace doesn't
  ...
  if (ent->activator &&
      ent->activator
          ->client) { // mark the trigger as being touched by the player
...
void multi_trigger(gentity_t *ent, gentity_t *activator) {
  ...
  if (ent->think ==
      multi_trigger_run) { // already triggered, just waiting to run
    return;
  }
  ...
  if (level.gametype == GT_SIEGE && activator && activator->client &&
      ent->alliedTeam &&
      activator->client->sess.sessionTeam !=
          ent->alliedTeam) { // this team can't activate this trigger.
    return;
  }

  if (level.gametype == GT_SIEGE && ent->idealclass &&
      ent->idealclass[0]) { // only certain classes can activate it
    ...
    if (!G_NameInTriggerClassList(
            bgSiegeClasses[activator->client->siegeClass].name,
            ent->idealclass)) { // wasn't in the list
      return;
    }
  }
  ...
  if (level.gametype == GT_SIEGE && ent->genericValue1) {
    ...
    if (activator && activator->client &&
        activator->client->holdingObjectiveItem && ent->targetname &&
        ent->targetname[0]) {
      ...
      if (objItem && objItem->inuse) {
        if (objItem->goaltarget && objItem->goaltarget[0] &&
          ...
          if (objItem->genericValue7 !=
              activator->client->sess
                  .sessionTeam) { // The carrier of the item is not on the team
                                  // which disallows objective scoring for it
            if (objItem->target3 &&
                objItem->target3[0]) { // if it has a target3, fire it off
                                       // instead of using the trigger
              G_UseTargets2(objItem, objItem, objItem->target3);

              // 3-24-03 - want to fire off the target too I guess, if we have
              // one.
  ...
  } else if (ent->genericValue1) { // Never activate in non-siege gametype I
                                   // guess.
    return;
  }
  ...
  if (ent->genericValue2) { // has "teambalance" property
    ...
    if (!activator->client ||
        (activator->client->sess.sessionTeam != SIEGETEAM_TEAM1 &&
         activator->client->sess.sessionTeam !=
             SIEGETEAM_TEAM2)) { // activator must be a valid client to begin
                                 // with
      return;
    }

    // Count up the number of clients standing within the bounds of the trigger
    // and the number of them on each team
    numEnts = trap->EntitiesInBox(ent->r.absmin, ent->r.absmax, entityList,
                                  MAX_GENTITIES);
    while (i < numEnts) {
    ...
    if (!team1ClNum &&
        !team2ClNum) { // no one in the box? How did we get activated? Oh well.
      return;
    }

    if (team1ClNum == team2ClNum) { // if equal numbers the ownership will
    ...
    if (owningTeam ==
        newOwningTeam) { // it's the same one it already was, don't care then.
      return;
    }

    // Set the new owner and set the variable which will tell us to activate a
    // team-specific target
    ent->genericValue3 = newOwningTeam;
    ent->genericValue4 = newOwningTeam;
  }

  if (haltTrigger) { // This is an objective trigger and the activator is not
                     // carrying an objective item that matches the targetname.
    return;
  }
...
void Touch_Multi(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (self->spawnflags & 4) { // USE_BUTTON
    if (!(other->client->pers.cmd.buttons &
    ...
    if ((other->client->ps.weaponTime > 0 &&
        ...
        other->health < 1 || (other->client->ps.pm_flags & PMF_FOLLOW) ||
        other->client->sess.sessionTeam == TEAM_SPECTATOR ||
        other->client->ps.forceHandExtend !=
            HANDEXTEND_NONE) { // player has to be free of other things to use.
      return;
    }

    if (self->genericValue7) { // we have to be holding the use key in this
                               // trigger for x milliseconds before firing
      if (level.gametype == GT_SIEGE && self->idealclass &&
          self->idealclass[0]) { // only certain classes can activate it
        ...
        if (!G_NameInTriggerClassList(
                bgSiegeClasses[other->client->siegeClass].name,
                self->idealclass)) { // wasn't in the list
          return;
        }
      }

      if (!G_PointInBounds(other->client->ps.origin, self->r.absmin,
                           self->r.absmax)) {
        return;
      } else if (other->client->isHacking != self->s.number &&
                 other->s.number < MAX_CLIENTS) { // start the hack
        ...
        if (other->client->ps.hackingBaseTime >
            60000) { // don't allow a bit overflow
          other->client->ps.hackingTime = level.time + 60000;
          other->client->ps.hackingBaseTime = 60000;
        }
        return;
      } else if (other->client->ps.hackingTime <
                 level.time) { // finished with the hack, reset the hacking
                               // values and let it fall through
  ...
  if (self->spawnflags & 8) { // FIRE_BUTTON
    if (!(other->client->pers.cmd.buttons & BUTTON_ATTACK) &&
        !(other->client->pers.cmd.buttons &
          BUTTON_ALT_ATTACK)) { // not pressing fire button or altfire button
      return;
    }
  }
  ...
  if (self->radius) {
    ...
    if (G_PointInBounds(eyeSpot, self->r.absmin, self->r.absmax)) {
      if (!(other->client->pers.cmd.buttons & BUTTON_ATTACK) &&
          !(other->client->pers.cmd.buttons &
            BUTTON_ALT_ATTACK)) { // not attacking, so hiding bonus
        /*
  ...
  if (self->spawnflags & 4) { // USE_BUTTON
    if (other->client->ps.torsoAnim != BOTH_BUTTON_HOLD &&
        other->client->ps.torsoAnim != BOTH_CONSOLE1) {
      G_SetAnim(other, NULL, SETANIM_TORSO, BOTH_BUTTON_HOLD,
  ...
  if (self->think ==
      trigger_cleared_fire) { // We're waiting to fire our target2 first
    self->nextthink = level.time + self->speed;
    return;
  }
...
/*QUAKED trigger_multiple (.1 .5 .1) ? CLIENTONLY FACING USE_BUTTON FIRE_BUTTON
NPCONLY x x INACTIVE MULTIPLE CLIENTONLY - only a player can trigger this by
...
play when the trigger fires (plays at activator's origin) "NPC_targetname"  Only
the NPC with this NPC_targetname fires this trigger
...
flag, will require a client to hold the use key for x amount of ms before
firing.

Applicable only during Siege gametype:
teamuser	-	if 1, team 2 can't use this. If 2, team 1 can't use
this. siegetrig	-	if non-0, can only be activated by players carrying a
misc_siege_item which is associated with this trigger by the item's goaltarget
value.
teambalance	-	if non-0, is "owned" by the last team that activated.
Can only be activated by the other team if the number of players on the other
team inside	the trigger outnumber the number of players on the owning team
inside the trigger. target3		-	fire when activated by team1
target4		-	fire when activated by team2

idealclass	-	Can only be used by this class/these classes. You can
specify use by multiple classes with the use of |, e.g.: "Imperial
Medic|Imperial Assassin|Imperial Demolitionist"
*/
...
/*QUAKED trigger_once (.5 1 .5) ? CLIENTONLY FACING USE_BUTTON FIRE_BUTTON x x x
INACTIVE MULTIPLE CLIENTONLY - only a player can trigger this by touch FACING -
Won't fire unless triggering ent's view angles are within 45 degrees of
trigger's angles (in addition to any other conditions) USE_BUTTON - Won't fire
unless player is in it and pressing use button (in addition to any other
conditions) FIRE_BUTTON - Won't fire unless player/NPC is in it and pressing
fire button (in addition to any other conditions) INACTIVE - Start off, has to
be activated to be touchable/usable MULTIPLE - multiple entities can touch this
trigger in a single frame *and* if needed, the trigger can have a wait of > 0
...
origin) "NPC_targetname"  Only the NPC with this NPC_targetname fires this
trigger
...
void Do_Strike(gentity_t *ent) {
  ...
  trap->Trace(&localTrace, strikeFrom, NULL, NULL, strikePoint, ent->s.number,
              MASK_PLAYERSOLID, qfalse, 0, 0);
  VectorCopy(localTrace.endpos, strikePoint);

  if (localTrace.startsolid ||
      localTrace.allsolid) { // got a bad spot, think again next frame to try
                             // another strike
  ...
  if (ent->radius) { // do a radius damage at the end pos
    G_RadiusDamage(strikePoint, ent, ent->damage, ent->radius, ent, NULL,
                   MOD_SUICIDE);
    ...
    if (trHit->inuse && trHit->takedamage) { // damage it then
      G_Damage(trHit, ent, ent, NULL, trHit->r.currentOrigin, ent->damage, 0,
               MOD_SUICIDE);
    }
  }
...
"radius"		if non-0, does a radius damage at the lightning strike
                                impact point (using this value as the radius).
otherwise will only do line trace damage. default 0.
...
void trigger_push_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  {
    if (self->spawnflags & PUSH_MULTIPLE) // MULTIPLE - allow multiple entities
                                          // to touch this trigger in one frame
    {
...
towards it.  Must set a "speed" (see below) RELATIVE - instead of pushing you in
a direction that is always from the center of the trigger to the
target_position, it pushes *you* toward the target position, relative to your
current location (can use with "speed"... if don't set a speed, it will use the
distance from you to the target_position) INACTIVE - not active until targeted
by a target_activate MULTIPLE - multiple entities can touch this trigger in a
single frame *and* if needed, the trigger can have a wait of > 0
...
/*QUAKED trigger_hurt (.5 .5 .5) ? START_OFF CAN_TARGET SILENT NO_PROTECTION
SLOW Any entity that touches this will be hurt. It does dmg points of damage
each server frame Targeting the trigger will toggle its on / off state.
...
"team"			team (1 or 2) to allow hurting (if none then hurt
anyone) only applicable for siege "dmg"			default 5 (whole numbers
only) If dmg is set to -1 this brush will use the fade-kill method
...
void hurt_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (self->damage == -1 && other && other->client &&
      other->client->ps.fallingToDeath) {
    return;
  }
  ...
  */
  ...
  if (self->damage == -1 && other && other->client) {
    if (other->client->ps.otherKillerTime >
        level.time) { // we're as good as dead, so if someone pushed us into
                      // this then remember them
    ...
    // rag on the way down, this flag will automatically be cleared for us on
    // respawn
    ...
    if (other->NPC) { // kill it now
      ...
      G_Damage(other, other, other, vDir, other->client->ps.origin, Q3_INFINITE,
               0, MOD_FALLING);
    } else {
    ...
    if (self->activator && self->activator->inuse && self->activator->client) {
      G_Damage(other, self->activator, self->activator, NULL, NULL, dmg,
               dflags | DAMAGE_NO_PROTECTION, MOD_TRIGGER_HURT);
    } else {
      G_Damage(other, self, self, NULL, NULL, dmg,
               dflags | DAMAGE_NO_PROTECTION, MOD_TRIGGER_HURT);
    }
  }
}
...
void space_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (!G_PointInBounds(
          other->client->ps.origin, self->r.absmin,
          self->r.absmax)) { // his origin must be inside the trigger
    return;
  }
...
void shipboundary_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (other->client->ps.hyperSpaceTime &&
      level.time - other->client->ps.hyperSpaceTime <
          HYPERSPACE_TIME) { // don't interfere with hyperspacing ships
    return;
  }
...
void shipboundary_think(gentity_t *ent) {
  ...
  if (ent->genericValue7 < level.time) { // don't need to be doing this check,
                                         // no one has touched recently
    return;
  }

  numListedEntities = trap->EntitiesInBox(ent->r.absmin, ent->r.absmax,
                                          iEntityList, MAX_GENTITIES);
  while (i < numListedEntities) {
    listedEnt = &g_entities[iEntityList[i]];
    if (listedEnt->inuse && listedEnt->client &&
        listedEnt->client->ps.m_iVehicleNum) {
...
void hyperspace_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (other->client->ps.hyperSpaceTime &&
      level.time - other->client->ps.hyperSpaceTime <
          HYPERSPACE_TIME) { // already hyperspacing, just keep us moving
    if ((other->client->ps.eFlags2 &
         EF2_HYPERSPACE)) { // they've started the hyperspace but haven't been
                            // teleported yet
      float timeFrac =
          ((float)(level.time - other->client->ps.hyperSpaceTime)) /
          HYPERSPACE_TIME;
      if (timeFrac >= HYPERSPACE_TELEPORT_FRAC) { // half-way, now teleport
                                                  // them!
        ...
        // vtos(other->client->ps.origin), vtos(newOrg) ); now put them in the
        // offset position, facing the angles that position wants them to be
        // facing
        TeleportPlayer(other, newOrg, ent->s.angles);
        if (other->m_pVehicle &&
            other->m_pVehicle->m_pPilot) { // teleport the pilot, too
          TeleportPlayer((gentity_t *)other->m_pVehicle->m_pPilot, newOrg,
                         ent->s.angles);
    ...
    if (!other->client->ps.m_iVehicleNum ||
        other->m_pVehicle
            ->m_iRemovedSurfaces) { // if a vehicle touches a boundary without a
                                    // pilot in it or with parts missing, just
                                    // blow the thing up
      G_Damage(other, other, other, NULL, other->client->ps.origin, 99999,
               DAMAGE_NO_PROTECTION, MOD_SUICIDE);
      return;
...
void trigger_hyperspace_find_targets( gentity_t *self )
{
        ...
        { //this is bad
                trap->Error( ERR_DROP, "trigger_hyperspace has invalid target
'%s'\n", self->target ); return;
        }
        targEnt->r.svFlags |= SVF_BROADCAST;//crap, need to tell the cgame about
the target_position targEnt = G_Find (NULL, FOFS(targetname), self->target2); if
(!targEnt || !targEnt->inuse) { //this is bad trap->Error( ERR_DROP,
"trigger_hyperspace has invalid target2 '%s'\n", self->target2 ); return;
        }
        targEnt->r.svFlags |= SVF_BROADCAST;//crap, need to tell the cgame about
the target_position
}
...
playing the hyperspace effect, then pop out at a relative point around the
target

"target"		whatever position the ship teleports from in relation to
the target_position specified here, that's the relative position the ship will
spawn at around the target2 target_position "target2"		name of
target_position to teleport the ship to (will be relative to it's origin)
*/
...
extern void Q3_Lerp2Origin(int taskID, int entID, vec3_t origin,
                           float duration);
      ...
      if (copyAsteroid) {
        ...
        for (axis = 0; axis < 3; axis++) {
          if (axis == capAxis) {
            ...
            startSpot[axis] =
                self->r.mins[axis] +
                (flrand(0, 1.0f) * (self->r.maxs[axis] - self->r.mins[axis]));
            endSpot[axis] =
                self->r.mins[axis] +
                (flrand(0, 1.0f) * (self->r.maxs[axis] - self->r.mins[axis]));
          }
        }
        // FIXME: maybe trace from start to end to make sure nothing is in the
        // way?  How big of a trace?
