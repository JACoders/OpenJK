...
// mapPoolBlockCount is defined differently in the executable (sv_main.cpp) and
// the game dll (g_main.cpp) cuz
...
public:
  CMapBlock(int id, vector<void *> &freeList) : mLastNode(0) {
    ...
    mNodes = (SMapNode *)((((unsigned long)mRaw) + MAP_NODE_SIZE) &
                          ~(unsigned long)0x1f);
  ...
  bool bOwnsNode(void *node) {
    return ((((SMapNode *)node) >= &mNodes[0]) &&
            (((SMapNode *)node) < &mNodes[MAPBLOCK_SIZE_NODES]));
  }
};

CMapPoolLow::CMapPoolLow() { mLastBlockNum = -1; }
...
public:
  ...
  CPool(void) : mNextStringId(1), mLastBlockNum(-1) {
...
public:
  ...
  ~CPoolChecker() {
    ...
    sprintf(mess, "[MEM][GAME]  String Pool %d unique strings, %dK\n",
            ThePool().mNextStringId,
            (ThePool().mLastBlockNum + 1) * BLOCK_SIZE / 1024);
#elif _CGAME
