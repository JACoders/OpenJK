...
qboolean G_BoundsOverlap(const vec3_t mins1, const vec3_t maxs1, const vec3_t mins2, const vec3_t maxs2) { // NOTE: flush up against counts as overlapping
	if (mins1[0] > maxs2[0])
		return qfalse;
...
qboolean ReachedGoal(gentity_t *goal) {
	...
	*/
	return NAV_HitNavGoal(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, goal->r.currentOrigin, NPCS.NPCInfo->goalRadius,
...
gentity_t *UpdateGoal(void) {
	...
	if (!NPCS.NPCInfo->goalEntity->inuse) { // Somehow freed it, but didn't clear it
		NPC_ClearGoal();
		return NULL;
	}
