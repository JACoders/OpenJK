...
typedef char* (*PFNCURLVERSION)(void);

typedef CURL* (*PFNCURLEASYINIT)(void);
typedef CURLcode (*PFNCURLEASYSETOPT)(CURL *curl, CURLoption option, ...);
typedef CURLcode (*PFNCURLEASYPERFORM)(CURL *curl);
typedef void (*PFNCURLEASYCLEANUP)(CURL *curl);
typedef CURLcode (*PFNCURLEASYGETINFO)(CURL *curl, CURLINFO info, ...);
//typedef CURL* (*PFNCURLEASYDUPHANDLE)(CURL *curl);
typedef void (*PFNCURLEASYRESET)(CURL *curl);
typedef const char* (*PFNCURLEASYSTRERROR)(CURLcode code);

typedef CURLM* (*PFNCURLMULTIINIT)(void);
typedef CURLMcode (*PFNCURLMULTIADDHANDLE)(CURLM *multi_handle, CURL *curl_handle);
typedef CURLMcode (*PFNCURLMULTIREMHANDLE)(CURLM *multi_handle, CURL *curl_handle);
typedef CURLMcode (*PFNCURLMULTIPERFORM)(CURLM *multi_handle, int *running_handles);
typedef CURLMcode (*PFNCURLMULTICLEANUP)(CURLM *multi_handle);
typedef CURLMsg* (*PFNCURLMULTIINFOREAD)(CURLM *multi_handle, int *msgs_in_queue);
typedef const char* (*PFNCURLMULTISTRERROR)(CURLMcode code);
...
qboolean CL_cURL_Init()
{
	...
	qcurl_version = (PFNCURLVERSION)GPA("curl_version");

	qcurl_easy_init = (PFNCURLEASYINIT)GPA("curl_easy_init");
	qcurl_easy_setopt = (PFNCURLEASYSETOPT)GPA("curl_easy_setopt");
	qcurl_easy_perform = (PFNCURLEASYPERFORM)GPA("curl_easy_perform");
	qcurl_easy_cleanup = (PFNCURLEASYCLEANUP)GPA("curl_easy_cleanup");
	qcurl_easy_strerror = (PFNCURLEASYSTRERROR)GPA("curl_easy_strerror");
	
	qcurl_multi_init = (PFNCURLMULTIINIT)GPA("curl_multi_init");
	qcurl_multi_add_handle = (PFNCURLMULTIADDHANDLE)GPA("curl_multi_add_handle");
	qcurl_multi_remove_handle = (PFNCURLMULTIREMHANDLE)GPA("curl_multi_remove_handle");
	qcurl_multi_perform = (PFNCURLMULTIPERFORM)GPA("curl_multi_perform");
	qcurl_multi_cleanup = (PFNCURLMULTICLEANUP)GPA("curl_multi_cleanup");
	qcurl_multi_info_read = (PFNCURLMULTIINFOREAD)GPA("curl_multi_info_read");
	qcurl_multi_strerror = (PFNCURLMULTISTRERROR)GPA("curl_multi_strerror");
...
static size_t CL_cURL_CallbackWrite(void *buffer, size_t size, size_t nmemb, void *stream)
{
	...
	return FS_Write( buffer, size * nmemb, file );
...
template< typename T >
static CURLcode qcurl_easy_setopt_warn( CURL *curl, CURLoption option, T arg )
{
	CURLcode result;
	result = qcurl_easy_setopt( curl, option, arg );
...
void CL_cURL_BeginDownload( const char *localName, const char *remoteURL )
{
	...
	qcurl_easy_setopt_warn(clc.downloadCURL, CURLOPT_REFERER, va("OJK://%s", NET_AdrToString(clc.serverAddress)));
	qcurl_easy_setopt_warn(clc.downloadCURL, CURLOPT_USERAGENT, va("%s %s", JK_VERSION, qcurl_version()));
	qcurl_easy_setopt_warn(clc.downloadCURL, CURLOPT_WRITEFUNCTION, CL_cURL_CallbackWrite);
	qcurl_easy_setopt_warn(clc.downloadCURL, CURLOPT_WRITEDATA, (void *)(intptr_t)clc.download);
...
void CL_cURL_PerformDownload(void)
{
	...
	if ( (res = qcurl_multi_perform( clc.downloadCURLM, &c )) == CURLM_CALL_MULTI_PERFORM && c )
		return;

	while ( (msg = qcurl_multi_info_read( clc.downloadCURLM, &c )) && msg->easy_handle != clc.downloadCURL )
		;
	...
	{
		err = qcurl_easy_strerror( msg->data.result );
	}

	CL_cURL_Cleanup();
	FS_FCloseFile( clc.download );
	clc.download = NULL_FILE;

	if ( err != NULL )
	{
		Com_Error( ERR_DROP, "Download Error: %s [%d] URL: \"%s\"", err, (int)msg->data.result, clc.downloadURL );
		return;
	}
