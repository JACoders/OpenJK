...
================
*/
void TransposeMatrix(/*const*/ matrix3_t matrix,
                     matrix3_t transpose) { // bk: FIXME
  int i, j;
  for (i = 0; i < 3; i++) {
    for (j = 0; j < 3; j++) {
...
================
*/
void CM_TestInLeaf(traceWork_t *tw, trace_t &trace, cLeaf_t *leaf,
                   clipMap_t *local) {
  int k;
  int brushnum;
  cbrush_t *b;
...
==================
*/
void CM_TestBoundingBoxInCapsule(traceWork_t *tw, trace_t &trace,
                                 clipHandle_t model) {
...
================
*/
void CM_TraceThroughBrush(traceWork_t *tw, trace_t &trace, cbrush_t *brush,
                          bool infoOnly) {
  int i;
  cbrushside_t *side;
...
================
*/
void CM_TraceThroughLeaf(traceWork_t *tw, trace_t &trace, clipMap_t *local,
                         cLeaf_t *leaf) {
  int k;
  int brushnum;
  cbrush_t *b;
...
================
*/
void CM_TraceThroughSphere(traceWork_t *tw, trace_t &trace, vec3_t origin,
                           float radius, vec3_t start, vec3_t end) {
...
================
*/
void CM_TraceThroughVerticalCylinder(traceWork_t *tw, trace_t &trace,
                                     vec3_t origin, float radius,
...
void CM_TraceCapsuleThroughCapsule(traceWork_t *tw, trace_t &trace,
  ...
  if (tw->start[0] != tw->end[0] || tw->start[1] != tw->end[1]) {
    ...
    if (h > 0) {
      ...
      CM_TraceThroughVerticalCylinder(tw, trace, offset, radius, h, tw->start,
                                      tw->end);
    }
  }
...
================
*/
void CM_TraceBoundingBoxThroughCapsule(traceWork_t *tw, trace_t &trace,
                                       clipHandle_t model) {
  vec3_t mins, maxs, offset, size[2];
  clipHandle_t h;
  cmodel_t *cmod;
...
================
*/
void CM_TraceToLeaf(traceWork_t *tw, trace_t &trace, cLeaf_t *leaf,
                    clipMap_t *local) {
  int k;
  int brushnum;
  cbrush_t *b;
...
==================
*/
void CM_TraceThroughTree(traceWork_t *tw, trace_t &trace, clipMap_t *local,
                         int num, float p1f, float p2f, vec3_t p1, vec3_t p2) {
  cNode_t *node;
  cplane_t *plane;
  float t1, t2, offset;
  float frac, frac2;
  ...
  CM_TraceThroughTree(tw, trace, local, node->children[side], p1f, midf, p1,
                      mid);
  ...
  CM_TraceThroughTree(tw, trace, local, node->children[side ^ 1], midf, p2f,
                      mid, p2);
}
...
void CM_Trace(trace_t *trace, const vec3_t start, const vec3_t end,
  ...
  if (start[0] == end[0] && start[1] == end[1] && start[2] == end[2] &&
    ...
    if (model && cmod->firstNode == -1) {
      ...
      CM_TraceThroughTree(&tw, *trace, local, cmod->firstNode, 0, 1, tw.start,
                          tw.end);
    }
  } else {
    //
  ...
  assert(trace->allsolid || trace->fraction == 1.0 ||
...
==================
*/
void CM_TransformedBoxTrace(trace_t *trace, const vec3_t start,
...
bool CM_CullBox(const cplane_t *frustum, const vec3_t transformed[8]) {
  ...
  for (i = 0, frust = frustum; i < 4; i++, frust++) {
    for (j = 0; j < 8; j++) {
      if (DotProduct(transformed[j], frust->normal) >
          frust->dist) { // a point is in front
        break;
      }
    }
