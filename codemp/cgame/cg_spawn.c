...
qboolean CG_SpawnBoolean(const char *key, const char *defaultString,
  ...
  present = CG_SpawnString(key, defaultString, &s);
  if (!Q_stricmp(s, "qfalse") || !Q_stricmp(s, "false") ||
      !Q_stricmp(s, "no") || !Q_stricmp(s, "0")) {
    *out = qfalse;
  } else if (!Q_stricmp(s, "qtrue") || !Q_stricmp(s, "true") ||
             !Q_stricmp(s, "yes") || !Q_stricmp(s, "1")) {
    *out = qtrue;
  } else {
    *out = qfalse;
  }
...
void CG_ParseEntityFromSpawnVars(void) {
  ...
  if (CG_SpawnString("classname", "", &classname)) {
    s = (spawn_t *)Q_LinearSearch(classname, spawns, ARRAY_LEN(spawns),
...
qboolean CG_ParseSpawnVars(void) {
  ...
  if (com_token[0] != '{') {
    trap->Error(ERR_DROP, "CG_ParseSpawnVars: found %s when expecting {",
                com_token);
  }
