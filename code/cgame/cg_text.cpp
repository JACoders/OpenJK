...
const char *CG_DisplayBoxedText(int iBoxX, int iBoxY, int iBoxWidth,
  ...
  const int iFontHeightAdvance =
      (int)(((gfAdvanceHack == 0.0f) ? 1.5f : gfAdvanceHack) *
            (float)iFontHeight);
...
void CG_CaptionText(const char *str, int sound) {
  ...
  // kor...
  //	s="Wp:澗顫歜檜棻 詩萼. 斜菟檜 蜓и渠煎 啻陛 澀й雖 晦渠ж啊棻.澗顫歜檜棻
  //詩萼. 斜菟檜 蜓и渠煎 啻陛 澀й雖 晦渠ж啊棻.";
  holds = s;
  ...
  while (*s) {
    ...
    int iAdvanceCount;
    unsigned int uiLetter = cgi_AnyLanguage_ReadCharFromString(
        s, &iAdvanceCount, &bIsTrailingPunctuation);
    s += iAdvanceCount;
  ...
  while (1) {
    ...
    for (l = 0; l < sizeof(linebuffer) - 1; l++) {
      ...
      unsigned int uiLetter =
          cgi_AnyLanguage_ReadCharFromString(psString, &iAdvanceCount);
