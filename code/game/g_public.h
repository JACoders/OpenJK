...
#define SVF_USE_CURRENT_ORIGIN                                                 \
  0x00000080 // entity->currentOrigin instead of entity->s.origin
             // for link position (missiles and movers)
#define SVF_TRIMODEL                                                           \
  0x00000100 // Use a three piece model make up like a player does
#define SVF_OBJECTIVE 0x00000200 // Draw it's name if crosshair comes across it
#define SVF_ANIMATING                                                          \
  0x00000400 // Currently animating from startFrame to endFrame
#define SVF_NPC_PRECACHE                                                       \
  0x00000800 // Tell cgame to precache this spawner's NPC stuff
#define SVF_KILLED_SELF                                                        \
  0x00001000 // ent killed itself in a script, so don't do ICARUS_FreeEnt on
             // it... or else!
#define SVF_NAVGOAL 0x00002000 // Navgoal
#define SVF_NOPUSH 0x00004000  // Can't be pushed around
#define SVF_ICARUS_FREEZE                                                      \
  0x00008000 // NPCs are frozen, ents don't execute ICARUS commands
...
#define SVF_NONNPC_ENEMY                                                       \
  0x02000000 // Not a client/NPC, but can still be considered a hostile lifeform
#define SVF_SELF_ANIMATING                                                     \
  0x04000000 // Ent will control it's animation itself in it's think func
...
#define SVF_MOVER_ADJ_AREA_PORTALS                                             \
  0x80000000 // For scripted movers only- must *explicitly instruct* them to
             // affect area portals
//===============================================================
...
// the server needs to know enough information to handle collision and snapshot
// generation
...
typedef struct {
  ...
  void (*unlinkentity)(
      gentity_t *ent); // call before removing an interactive entity

  // EntitiesInBox will return brush models based on their bounding box,
  // so exact determination must still be done with EntityContact
  int (*EntitiesInBox)(const vec3_t mins, const vec3_t maxs, gentity_t **list,
                       int maxcount);

  // perform an exact check against inline brush models of non-square shape
  qboolean (*EntityContact)(const vec3_t mins, const vec3_t maxs,
                            const gentity_t *ent);
  ...
  void *(*Malloc)(int iSize, memtag_t eTag,
                  qboolean bZeroIt); // see qcommon/tags.h for choices
  int (*Free)(void *buf);
  qboolean (*bIsFromZone)(const void *buf,
                          memtag_t eTag); // see qcommon/tags.h for choices
  ...
  */
  ...
  int (*G2API_InitGhoul2Model)(CGhoul2Info_v &ghoul2, const char *fileName,
  ...
  qboolean (*G2API_SetBoneAnglesIndex)(CGhoul2Info *ghlInfo, const int index,
                                       ...
                                       qhandle_t *modelList, int blendTime,
                                       int currentTime);
  qboolean (*G2API_SetBoneAnglesMatrix)(CGhoul2Info *ghlInfo,
                                        const char *boneName,
  ...
  qboolean (*G2API_RemoveGhoul2Model)(CGhoul2Info_v &ghlInfo,
                                      const int modelIndex);
  qboolean (*G2API_SetSurfaceOnOff)(CGhoul2Info *ghlInfo,
                                    const char *surfaceName, const int flags);
  qboolean (*G2API_SetRootSurface)(CGhoul2Info_v &ghlInfo, const int modelIndex,
                                   const char *surfaceName);
  qboolean (*G2API_RemoveSurface)(CGhoul2Info *ghlInfo, const int index);
  int (*G2API_AddSurface)(CGhoul2Info *ghlInfo, int surfaceNumber,
  ...
  qboolean (*G2API_PauseBoneAnim)(CGhoul2Info *ghlInfo, const char *boneName,
                                  const int currentTime);
  qboolean (*G2API_PauseBoneAnimIndex)(CGhoul2Info *ghlInfo,
                                       const int boneIndex,
                                       const int currentTime);
  ...
  qboolean (*G2API_AttachEnt)(int *boltInfo, CGhoul2Info *ghlInfoTo,
  ...
  qboolean (*G2API_RagPCJConstraint)(CGhoul2Info_v &ghoul2,
                                     const char *boneName, vec3_t min,
                                     vec3_t max);
  qboolean (*G2API_RagPCJGradientSpeed)(CGhoul2Info_v &ghoul2,
                                        const char *boneName,
                                        const float speed);
  ...
  int (*RE_GetAnimationCFG)(const char *psCFGFilename, char *psDest,
                            int iDestSize);
...
typedef struct {
  ...
  void (*Init)(const char *mapname, const char *spawntarget, int checkSum,
               const char *entstring, int levelTime, int randomSeed,
