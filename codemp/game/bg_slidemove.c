...
void PM_VehicleImpact(bgEntity_t *pEnt, trace_t *trace) {
	...
	if (trace->entityNum < ENTITYNUM_WORLD && hitEnt->s.eType == ET_MOVER && hitEnt->s.apos.trType != TR_STATIONARY // rotating
	...
	} else if ((fabs(pm->ps->velocity[0]) + fabs(pm->ps->velocity[1])) < 100.0f && pm->ps->velocity[2] > -100.0f)
#elif defined(_CGAME)
	if ((fabs(pm->ps->velocity[0]) + fabs(pm->ps->velocity[1])) < 100.0f && pm->ps->velocity[2] > -100.0f)
#endif
	/*
if ( (pSelfVeh->m_ulFlags&VEH_GEARSOPEN)
	...
	if (pSelfVeh &&
		...
		if (pEnt->m_pVehicle->m_iHitDebounce < pm->cmd.serverTime || forceSurfDestruction) { // a bit of a hack, may conflict with getting shot, but...
				...
				{																			 // bounce off in the opposite direction of the impact
					if (pSelfVeh->m_pVehicleInfo->type == VH_SPEEDER) {
						...
						VectorCopy(trace->plane.normal, bounceDir);
							   ...
							   && pm->ps->speed <= MIN_LANDING_SPEED) {	   // could land here, don't bounce off, in fact, return altogether!
						return;
					} else {
			...
			if (hitEnt && (hitEnt->s.eType != ET_TERRAIN || !(hitEnt->spawnflags & 1) ||
						   pSelfVeh->m_pVehicleInfo->type == VH_FIGHTER)) { // don't damage the vehicle from terrain that doesn't want to damage vehicles
			...
			if (hitEnt && hitEnt->inuse && hitEnt->takedamage) { // damage this guy because we hit him
				float pmult = 1.0f;
				int finalD;
				gentity_t *attackEnt;
				...
				if ((hitEnt->s.eType == ET_PLAYER && hitEnt->s.number < MAX_CLIENTS) ||
					...
					if (hitEnt->client && BG_KnockDownable(&hitEnt->client->ps) && G_CanBeEnemy((gentity_t *)pEnt, hitEnt)) { // smash!
...
qboolean PM_ClientImpact(trace_t *trace) {
	...
	if (!traceEnt || !(traceEnt->r.contents & pm->tracemask)) { // it's dead or not in my way anymore, don't clip against it
		return qtrue;
	}
...
qboolean PM_SlideMove(qboolean gravity) {
	...
	for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
		...
		if (pm->ps->clientNum >= MAX_CLIENTS) {
			...
			if (pEnt && pEnt->s.eType == ET_NPC && pEnt->s.NPC_class == CLASS_VEHICLE && pEnt->m_pVehicle) { // do vehicle impact stuff then
		...
		// non-axial planes
		//
		if (!(pm->ps->pm_flags & PMF_STUCK_TO_WALL)) { // no sliding if stuck to wall!
...
void PM_StepSlideMove(qboolean gravity) {
	...
	if (pm->ps->clientNum >= MAX_CLIENTS) {
		if (pEnt && pEnt->s.NPC_class == CLASS_VEHICLE && pEnt->m_pVehicle && pEnt->m_pVehicle->m_pVehicleInfo->hoverHeight > 0) {
			return;
		}
	}
	...
	if (pm->stepSlideFix) {
		if (pm->ps->clientNum < MAX_CLIENTS && trace.plane.normal[2] < MIN_WALK_NORMAL) { // normal players cannot step up slopes that are too steep to walk on!
			vec3_t stepVec;
	...
	if (!trace.allsolid && !skipStep) // normal players cannot step up slopes that are too steep to walk on!
	{
		if (pm->ps->clientNum >= MAX_CLIENTS															// NPC
			&& isGiant && trace.entityNum < MAX_CLIENTS && pEnt && pEnt->s.NPC_class == CLASS_RANCOR) { // Rancor don't step on clients
			if (pm->stepSlideFix) {
