...
qboolean Jedi_StopKnockdown(gentity_t *self, gentity_t *pusher, const vec3_t pushDir) {
	if (self->s.number < MAX_CLIENTS || !self->NPC) { // only NPCs
		return qfalse;
	}

	if (self->client->ps.forcePowerLevel[FP_LEVITATION] < FORCE_LEVEL_1) { // only force-users
		return qfalse;
	}

	if (self->client->moveType == MT_FLYSWIM) { // can't knock me down when I'm flying
		return qtrue;
	}

	if (self->NPC && (self->NPC->aiFlags & NPCAI_BOSS_CHARACTER)) { // bosses always get out of a knockdown
	} else if (Q_irand(0, RANK_CAPTAIN + 5) > self->NPC->rank) {	// lower their rank, the more likely they are fall down
		return qfalse;
	}
...
void Tavion_ScepterDamage(void) {
	...
	if (NPC->genericBolt1 != -1) {
		...
		for (int time = curTime - 25; time <= curTime + 25 && !hit; time += 25) {
			...
			gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->weaponModel[1], NPC->genericBolt1, &boltMatrix, angles, NPC->currentOrigin, time, NULL, NPC->s.modelScale);
			...
			if (trace.fraction < 1.0f) { // hit something
				...
				if (traceEnt->takedamage && trace.entityNum != lastHit &&
					(!traceEnt->client || traceEnt == NPC->enemy || traceEnt->client->NPC_class != NPC->client->NPC_class)) { // smack
...
void Tavion_ScepterSlam(void) {
	...
	if (boltIndex != -1) {
		...
		gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->weaponModel[1], boltIndex, &boltMatrix, angles, NPC->currentOrigin, (cg.time ? cg.time : level.time), NULL,
							   NPC->s.modelScale);
		...
		for (i = 0; i < numEnts; i++) {
			...
			if ((radiusEnts[i]->flags & FL_NO_KNOCKBACK)) { // don't throw them back
				continue;
			}

			if (radiusEnts[i] == NPC) { // Skip myself
				continue;
			}

			if (radiusEnts[i]->client == NULL) {					  // must be a client
				if (G_EntIsBreakable(radiusEnts[i]->s.number, NPC)) { // damage breakables within range, but not as much
			...
			if (dist <= radius) {
				if (dist < halfRad) { // close enough to do damage, too
				...
				if (radiusEnts[i]->client && radiusEnts[i]->client->NPC_class != CLASS_RANCOR && radiusEnts[i]->client->NPC_class != CLASS_ATST) {
					...
					if (radiusEnts[i]->health > 0) {
						if (dist < halfRad || radiusEnts[i]->client->ps.groundEntityNum !=
...
void Tavion_SithSwordRecharge(void) {
	if (NPC->client->ps.torsoAnim != BOTH_TAVION_SWORDPOWER && NPC->count && TIMER_Done(NPC, "rechargeDebounce") && NPC->weaponModel[0] != -1) {
		...
		G_PlayEffect(G_EffectIndex("scepter/recharge.efx"), NPC->weaponModel[0], boltIndex, NPC->s.number, NPC->currentOrigin, NPC->client->ps.torsoAnimTimer,
					 qtrue);
		NPC->painDebounceTime = level.time + NPC->client->ps.torsoAnimTimer;
		NPC->client->ps.pm_time = NPC->client->ps.torsoAnimTimer;
		NPC->client->ps.pm_flags |= PMF_TIME_KNOCKBACK;
		...
		G_PlayEffect(G_EffectIndex("scepter/invincibility.efx"), NPC->playerModel, 0, NPC->s.number, NPC->currentOrigin, NPC->client->ps.torsoAnimTimer + 10000,
					 qfalse);
	...
	if (TIMER_Done(NPC, "chatter") && !Q_irand(0, 3) && NPCInfo->blockedSpeechDebounceTime < level.time &&
		...
		if (NPC->enemy && NPC->enemy->client &&
			(NPC->enemy->client->NPC_class == CLASS_RANCOR || NPC->enemy->client->NPC_class == CLASS_WAMPA ||
			 NPC->enemy->client->NPC_class == CLASS_SAND_CREATURE)) { // never taunt these mindless creatures
...
static void Jedi_AdjustSaberAnimLevel(gentity_t *self, int newLevel) {
	...
	if (self->client->playerTeam == TEAM_ENEMY) {
		...
		if (!Q_stricmp("cultist_saber_all", self->NPC_type) || !Q_stricmp("cultist_saber_all_throw", self->NPC_type)) { // use any, regardless of rank, etc.
		} else if (!Q_stricmp("cultist_saber", self->NPC_type) || !Q_stricmp("cultist_saber_throw", self->NPC_type)) {	// fast only
			self->client->ps.saberAnimLevel = SS_FAST;
		} else if (!Q_stricmp("cultist_saber_med", self->NPC_type) || !Q_stricmp("cultist_saber_med_throw", self->NPC_type)) { // med only
			self->client->ps.saberAnimLevel = SS_MEDIUM;
		} else if (!Q_stricmp("cultist_saber_strong", self->NPC_type) || !Q_stricmp("cultist_saber_strong_throw", self->NPC_type)) { // strong only
	...
	if (!(self->client->ps.saberStylesKnown & (1 << newLevel))) { // don't know that style, sorry
		return;
...
static void Jedi_CheckDecreaseSaberAnimLevel(void) {
	if (!NPC->client->ps.weaponTime && !(ucmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK | BUTTON_FORCE_FOCUS))) { // not attacking
...
static qboolean Jedi_DecideKick(void) {
	...
	if (!NPC->enemy || (NPC->enemy->s.number < MAX_CLIENTS && NPC->enemy->health <= 0)) { // have no enemy or enemy is a dead player
		return qfalse;
	}
	// FIXME: check FP_SABER_OFFENSE?
	// FIXME: check for saber staff style only?
	// FIXME: g_spskill?
	if (Q_irand(0, RANK_CAPTAIN + 5) > NPCInfo->rank) { // low chance, based on rank
		return qfalse;
	}
	if (Q_irand(0, 10) > NPCInfo->stats.aggression) { // the madder the better
		return qfalse;
	}
	if (!TIMER_Done(NPC, "kickDebounce")) { // just did one
		return qfalse;
	}
...
qboolean Kyle_CanDoGrab(void) {
	if (NPC->client->NPC_class == CLASS_KYLE && (NPC->spawnflags & 1)) { // Boss Kyle
		if (NPC->enemy && NPC->enemy->client) {							 // have a valid enemy
			if (TIMER_Done(NPC, "grabEnemyDebounce")) {					 // okay to grab again
				if (NPC->client->ps.groundEntityNum != ENTITYNUM_NONE &&
					NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE) { // me and enemy are on ground
					if (!PM_InOnGroundAnim(&NPC->enemy->client->ps)) {
						if ((NPC->client->ps.weaponTime <= 200 || NPC->client->ps.torsoAnim == BOTH_KYLE_GRAB) && !NPC->client->ps.saberInFlight) {
							if (fabs(NPC->enemy->currentOrigin[2] - NPC->currentOrigin[2]) <= 8.0f) { // close to same level of ground
...
static void Jedi_CombatDistance(int enemy_dist) { // FIXME: for many of these checks, what we really want is horizontal distance to enemy
	if (Jedi_CultistDestroyer(NPC)) {			  // destroyer
		Jedi_Advance();
	...
	if (enemy_dist < 128 && NPC->enemy && NPC->enemy->client &&
		(NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK6 || NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK7)) { // whoa, back off!!!
	...
	if (NPC->client->ps.forcePowersActive & (1 << FP_DRAIN) && NPC->client->ps.forcePowerLevel[FP_DRAIN] > FORCE_LEVEL_1) { // when draining, don't move
		return;
	...
	if (NPC->client->NPC_class == CLASS_BOBAFETT) {
	...
	} else if (NPC->client->ps.legsAnim == BOTH_ALORA_SPIN_THROW) { // don't move at all
																	// FIXME: sabers need trails
	} else if (NPC->client->ps.torsoAnim == BOTH_KYLE_GRAB) { // see if we grabbed enemy
		if (NPC->client->ps.torsoAnimTimer <= 200) {
			if (Kyle_CanDoGrab() && NPC_EnemyRangeFromBolt(NPC->handRBolt) <= 72.0f) { // grab him!
				Kyle_GrabEnemy();
				return;
			} else {
			...
			if (((NPCInfo->scriptFlags & SCF_DONT_FIRE) || NPC->max_health - NPC->health > NPC->max_health * 0.25f) // lost over 1/4 of our health or not firing
				...
				if (Jedi_DecideKick()) { // let's try a kick
					if (G_PickAutoMultiKick(NPC, qfalse, qtrue) != LS_NONE ||
						(G_CanKickEntity(NPC, NPC->enemy) && G_PickAutoKick(NPC, NPC->enemy, qtrue) != LS_NONE)) { // kicked!
		...
		if (NPCInfo->stats.aggression < Q_irand(0, 20) && NPC->health < NPC->max_health * 0.75f && !Q_irand(0, 2)) {
			if (NPC->enemy && NPC->enemy->s.number < MAX_CLIENTS && NPC->client->NPC_class != CLASS_KYLE &&
				((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) || NPC->client->NPC_class == CLASS_SHADOWTROOPER) &&
				Q_irand(0, 3 - g_spskill->integer)) { // hmm, bosses should do this less against the player
			...
			} else if ((NPC->client->ps.forcePowersKnown & (1 << FP_PROTECT)) != 0 && (NPC->client->ps.forcePowersActive & (1 << FP_PROTECT)) == 0 &&
					   Q_irand(0, 1)) {
				ForceProtect(NPC);
				usedForce = qtrue;
			} else if ((NPC->client->ps.forcePowersKnown & (1 << FP_ABSORB)) != 0 && (NPC->client->ps.forcePowersActive & (1 << FP_ABSORB)) == 0 &&
					   Q_irand(0, 1)) {
				ForceAbsorb(NPC);
				usedForce = qtrue;
		...
		if (NPCInfo->stats.aggression > 0) { // approach enemy
			if (!usedForce) {
				if (NPC->enemy && NPC->enemy->client &&
					(NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK6 || NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK7)) { // stay put!
				} else {
					Jedi_Advance();
				}
			}
	...
	*/
	...
	{						  // we're out of striking range and we are allowed to attack
		...
		if (NPC->enemy && NPC->enemy->client && (NPC->enemy->client->ps.eFlags & EF_FORCE_GRIPPED)) { // They're being gripped, rush them!
			...
			if ((NPCInfo->rank >= RANK_LT_JG || WP_ForcePowerUsable(NPC, FP_SABERTHROW, 0)) && !Q_irand(0, 5) &&
				!(NPC->client->ps.forcePowersActive & (1 << FP_SPEED)) && !(NPC->client->ps.saberEventFlags & SEF_INWATER)) // saber not in water
			{																												// throw saber
				ucmd.buttons |= BUTTON_ALT_ATTACK;
			}
				   ...
				   && InFOV(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 20, 30)) { // They're throwing their saber, grip them!
			// taunt
			...
			if (NPC->client->NPC_class == CLASS_KYLE && (NPC->spawnflags & 1) && (NPC->enemy && NPC->enemy->client && !NPC->enemy->client->ps.saberInFlight) &&
				TIMER_Done(NPC, "kyleTakesSaber") && !Q_irand(0, 20)) {
				...
				if (NPC->client->NPC_class == CLASS_KYLE && (NPC->spawnflags & 1)) {
					chanceScale = 4;
				} else if (NPC->enemy && NPC->enemy->s.number < MAX_CLIENTS &&
						   ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) ||
							NPC->client->NPC_class == CLASS_SHADOWTROOPER)) { // hmm, bosses do this less against player
				...
				if (chanceScale &&
					...
					if ((NPCInfo->rank == RANK_ENSIGN  // old reborn crap
						...
						if (WP_ForcePowerUsable(NPC, FP_PULL, 0) && !Q_irand(0, 2)) {
						...
						} else if (NPC->health < NPC->max_health * 0.75f && Q_irand(FORCE_LEVEL_0, NPC->client->ps.forcePowerLevel[FP_DRAIN]) > FORCE_LEVEL_1 &&
								   WP_ForcePowerUsable(NPC, FP_DRAIN, 0) &&
							...
							TIMER_Set(NPC, "attackDelay", NPC->client->ps.weaponTime);
						} else if (WP_ForcePowerUsable(NPC, FP_GRIP, 0) && NPC->enemy &&
							...
							if (WP_ForcePowerUsable(NPC, FP_SABERTHROW, 0) && !(NPC->client->ps.forcePowersActive & (1 << FP_SPEED)) &&
...
static qboolean Jedi_Strafe(int strafeTimeMin, int strafeTimeMax, int nextStrafeTimeMin, int nextStrafeTimeMax, qboolean walking) {
	if (Jedi_CultistDestroyer(NPC)) { // never strafe
		return qfalse;
	}
...
qboolean Jedi_DodgeEvasion(gentity_t *self, gentity_t *shooter, trace_t *tr, int hitLoc) {
	...
	if (self->s.number) { // if an NPC, check game skill setting
		...
		*/
		...
		if (!(self->client->ps.forcePowersActive & (1 << FP_SPEED))) { // not already in speed
	...
	case HL_WAIST:
		if (!self->s.number) { // don't force the player to jump
			...
			if (self->client && (self->client->ps.forceRageRecoveryTime > level.time ||
								 (self->client->ps.forcePowersActive & (1 << FP_RAGE)))) { // no fancy dodges when raging or recovering
				return qfalse;
			}
	...
	if (dodgeAnim != -1) {
		...
		*/
		if (self->s.number < MAX_CLIENTS) {																		 // player
			if ((self->client->ps.forcePowersActive & (1 << FP_SPEED))) {										 // in speed
				if (PM_DodgeAnim(self->client->ps.torsoAnim) && !PM_DodgeHoldAnim(self->client->ps.torsoAnim)) { // already in a dodge
...
evasionType_t Jedi_CheckFlipEvasions(gentity_t *self, float rightdot, float zdiff) {
	...
	if (self->client) {
		if (self->client->NPC_class == CLASS_BOBAFETT) { // boba can't flip
			return EVASION_NONE;
		}
		if (self->client->ps.forceRageRecoveryTime > level.time || (self->client->ps.forcePowersActive & (1 << FP_RAGE))) { // no fancy dodges when raging
			return EVASION_NONE;
		}
	}
	...
	if (self->client->ps.legsAnim == BOTH_WALL_RUN_LEFT || self->client->ps.legsAnim == BOTH_WALL_RUN_RIGHT) { // already running on a wall
		...
		vec3_t fwd, right, traceto, mins = {self->mins[0], self->mins[1], self->mins[2] + STEPSIZE}, maxs = {self->maxs[0], self->maxs[1], 24},
		...
		if (self->client->ps.weapon == WP_SABER) {
			...
			} else if (self->client->ps.dualSabers && (self->client->ps.saber[1].saberFlags & SFL_NO_CARTWHEELS)) {
				allowCartWheels = qfalse;
			}
		}
		...
		gi.trace(&trace, self->currentOrigin, mins, maxs, traceto, self->s.number, CONTENTS_SOLID | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP, (EG2_Collision)0,
				 0);
			...
			if ((trace.entityNum < ENTITYNUM_WORLD && traceEnt && traceEnt->s.solid != SOLID_BMODEL) ||
					...
					if ((trace.fraction * checkDist) <= 32) { // wall on that side is close enough to wall-flip off of or wall-run on
						...
						if (trace.fraction >= 1.0f) { // it's clear, let's do it
							...
							if (self->client->ps.weapon == WP_SABER) {
								...
								} else if (self->client->ps.dualSabers && (self->client->ps.saber[1].saberFlags & SFL_NO_WALL_FLIPS)) {
									allowWallFlips = qfalse;
								}
							}
				...
				if (bestCheckDist) { // one of the walls was close enough to wall-run on
					...
					if (self->client->ps.weapon == WP_SABER) {
						...
						} else if (self->client->ps.dualSabers && (self->client->ps.saber[1].saberFlags & SFL_NO_WALL_RUNS)) {
							allowWallRuns = qfalse;
						}
					}
		...
		*/
		{
			...
			if (evasionType == EVASION_DODGE) {
				...
				*/
				...
				if (self->client->NPC_class == CLASS_ALORA || self->client->NPC_class == CLASS_SHADOWTROOPER ||
					self->client->NPC_class == CLASS_TAVION) { // Tavion & Alora are faster
...
qboolean Jedi_QuickReactions(gentity_t *self) {
	if ((self->client->NPC_class == CLASS_JEDI && NPCInfo->rank == RANK_COMMANDER) || self->client->NPC_class == CLASS_SHADOWTROOPER ||
		self->client->NPC_class == CLASS_ALORA || self->client->NPC_class == CLASS_TAVION ||
...
qboolean Jedi_SaberBusy(gentity_t *self) {
	if (self->client->ps.torsoAnimTimer > 300 &&
		((PM_SaberInAttack(self->client->ps.saberMove) && self->client->ps.saberAnimLevel == SS_STRONG) || PM_SpinningSaberAnim(self->client->ps.torsoAnim) ||
...
qboolean Jedi_InNoAIAnim(gentity_t *self) {
	if (!self || !self->client) { // wtf???
		return qtrue;
	}

	if (NPCInfo->rank >= RANK_COMMANDER) { // boss-level guys can multitask, the rest need to chill out during special moves
		return qfalse;
	}

	if (PM_KickingAnim(NPC->client->ps.legsAnim) || PM_StabDownAnim(NPC->client->ps.legsAnim) || PM_InAirKickingAnim(NPC->client->ps.legsAnim) ||
		PM_InRollIgnoreTimer(&NPC->client->ps) || PM_SaberInKata((saberMoveName_t)NPC->client->ps.saberMove) ||
		PM_SuperBreakWinAnim(NPC->client->ps.torsoAnim) || PM_SuperBreakLoseAnim(NPC->client->ps.torsoAnim)) {
		return qtrue;
	}
...
void Jedi_CheckJumpEvasionSafety(gentity_t *self, usercmd_t *cmd, evasionType_t evasionType) {
	if (evasionType != EVASION_OTHER																 // not a FlipEvasion, which does it's own safety checks
		&& NPC->client->ps.groundEntityNum != ENTITYNUM_NONE) {										 // on terra firma right now
		if (NPC->client->ps.velocity[2] > 0 || NPC->client->ps.forceJumpCharge || cmd->upmove > 0) { // going to jump
			if (!NAV_MoveDirSafe(NPC, cmd, NPC->client->ps.speed * 10.0f)) {						 // we can't jump in the dir we're pushing in
				// cancel the evasion
			...
			} else if (NPC->client->ps.velocity[0] || NPC->client->ps.velocity[1]) { // sliding
				vec3_t jumpDir;
				float jumpDist = VectorNormalize2(NPC->client->ps.velocity, jumpDir);
				if (!NAV_DirSafe(NPC, jumpDir, jumpDist)) { // this jump combined with our momentum would send us into a do not enter brush, so cancel it
					// cancel the evasion
					NPC->client->ps.velocity[2] = NPC->client->ps.forceJumpCharge = 0;
					cmd->upmove = 0;
					if (d_JediAI->integer) {
...
evasionType_t Jedi_SaberBlockGo(gentity_t *self, usercmd_t *cmd, vec3_t pHitloc, vec3_t phitDir, gentity_t *incoming, float dist = 0.0f) {
	...
	if (PM_LockedAnim(self->client->ps.torsoAnim) && self->client->ps.torsoAnimTimer) { // Never interrupt these...
		return EVASION_NONE;
	}
	...
	if (self->client->NPC_class == CLASS_BOBAFETT) {
		...
		if ((dist > 16 && (Q_irand(0, 2) || saberBusy)) || self->client->ps.saberInFlight || !self->client->ps.SaberActive() ||
			(self->client->NPC_class == CLASS_REBORN &&
			 self->s.weapon != WP_SABER)) { // either it will miss by a bit (and 25% chance) OR our saber is not in-hand OR saber is off
			if (self->NPC && (self->NPC->rank == RANK_CREWMAN || self->NPC->rank >= RANK_LT_JG)) {			  // acrobat or fencer or above
				if (self->client->ps.groundEntityNum != ENTITYNUM_NONE &&									  // on the ground
					...
					&& (self->client->ps.saberInFlight || (self->client->NPC_class == CLASS_REBORN && self->s.weapon != WP_SABER) ||
		...
		{																   // jump!
			if (self->client->ps.groundEntityNum == ENTITYNUM_NONE) {	   // already in air, duck to pull up legs
				...
				if (self->NPC && (self->NPC->rank == RANK_CREWMAN || self->NPC->rank > RANK_LT_JG) &&
					...
					if (self->NPC && !(self->NPC->scriptFlags & SCF_NO_ACROBATICS) && self->client->ps.forceRageRecoveryTime < level.time &&
						!(self->client->ps.forcePowersActive & (1 << FP_RAGE)) && !PM_InKnockDown(&self->client->ps)) {
						self->client->ps.forceJumpCharge = 320; // FIXME: calc this intelligently
					...
					if (self->client->NPC_class == CLASS_ALORA || self->client->NPC_class == CLASS_SHADOWTROOPER || self->client->NPC_class == CLASS_TAVION) {
						if (!incoming && self->client->ps.groundEntityNum < ENTITYNUM_NONE && !Q_irand(0, 2)) {
							if (!PM_SaberInAttack(self->client->ps.saberMove) && !PM_SaberInStart(self->client->ps.saberMove) &&
								!PM_InRoll(&self->client->ps) && !PM_InKnockDown(&self->client->ps) &&
								!PM_SaberInSpecialAttack(self->client->ps.torsoAnim)) { // do the butterfly!
								int butterflyAnim;
...
static evasionType_t Jedi_CheckEvadeSpecialAttacks(void) {
	...
	if (!NPC->enemy || NPC->enemy->health <= 0 || !NPC->enemy->client) { // don't keep blocking him once he's dead (or if not a client)
		return EVASION_NONE;
	}

	if (NPC->enemy->s.number >= MAX_CLIENTS) { // only do these against player
		return EVASION_NONE;
	}

	if (!TIMER_Done(NPC, "specialEvasion")) { // still evading from last time
		return EVASION_NONE;
	}

	if (NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK6 || NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK7) { // back away from these
		if ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) || NPC->client->NPC_class == CLASS_SHADOWTROOPER || NPC->client->NPC_class == CLASS_ALORA ||
			Q_irand(0, NPCInfo->rank) > RANK_LT_JG) {												 // see if we should back off
			if (InFront(NPC->currentOrigin, NPC->enemy->currentOrigin, NPC->enemy->currentAngles)) { // facing me
				...
				if (DistanceSquared(NPC->currentOrigin, NPC->enemy->currentOrigin) < minSafeDistSq) { // back off!
					Jedi_StartBackOff();
					return EVASION_OTHER;
				}
			}
		}
	} else { // check some other attacks?
		// check roll-stab
		if (NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_STAB ||
			(NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_F &&
			 ((NPC->enemy->client->pers.lastCommand.buttons & BUTTON_ATTACK) ||
			  (NPC->enemy->client->ps.pm_flags & PMF_ATTACK_HELD)))) { // either already in a roll-stab or may go into one
			if ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) || NPC->client->NPC_class == CLASS_SHADOWTROOPER || NPC->client->NPC_class == CLASS_ALORA ||
				Q_irand(-3, NPCInfo->rank) > RANK_LT_JG) { // see if we should evade
				vec3_t yawOnlyAngles = {0, NPC->enemy->currentAngles[YAW], 0};
				if (InFront(NPC->currentOrigin, NPC->enemy->currentOrigin, yawOnlyAngles, 0.25f)) { // facing me
					float minSafeDistSq = (NPC->maxs[0] * 1.5f + NPC->enemy->maxs[0] * 1.5f + NPC->enemy->client->ps.SaberLength() + 24.0f);
					minSafeDistSq *= minSafeDistSq;
					float distSq = DistanceSquared(NPC->currentOrigin, NPC->enemy->currentOrigin);
					if (distSq < minSafeDistSq) {																			  // evade!
						qboolean doJump = (qboolean)(NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_STAB || distSq < 3000.0f); // not much time left, just jump!
						if ((NPCInfo->scriptFlags & SCF_NO_ACROBATICS) || !doJump) {										  // roll?
							...
							if (dot > 0) {								  // I'm to his right
								if (!NPC_MoveDirClear(0, -127, qfalse)) { // fuck, jump instead
									...
									if (NPC->client->ps.groundEntityNum != ENTITYNUM_NONE) { // fuck it, just force it
						...
						// didn't roll, do jump
						if (NPC->s.weapon != WP_SABER || (NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) || NPC->client->NPC_class == CLASS_SHADOWTROOPER ||
							NPC->client->NPC_class == CLASS_ALORA || Q_irand(-3, NPCInfo->rank) > RANK_CREWMAN) { // superjump
							NPC->client->ps.forceJumpCharge = 320;												  // FIXME: calc this intelligently
							if (Q_irand(0, 2)) {																  // make it a backflip
								ucmd.forwardmove = -127;
								TIMER_Set(NPC, "roamTime", -level.time);
								TIMER_Set(NPC, "strafeLeft", -level.time);
								TIMER_Set(NPC, "strafeRight", -level.time);
...
static qboolean Jedi_SaberBlock(void) {
	...
	*/
	...
	for (saberNum = 0; saberNum < MAX_SABERS; saberNum++) {
		for (bladeNum = 0; bladeNum < NPC->enemy->client->ps.saber[saberNum].numBlades; bladeNum++) {
			if (NPC->enemy->client->ps.saber[saberNum].type != SABER_NONE &&
				NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].length > 0) { // valid saber and this blade is on
				VectorMA(NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].muzzlePointOld, NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].length,
						 NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].muzzleDirOld, saberTipOld);
				VectorMA(NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint, NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].length,
				...
				dist = ShortestLineSegBewteen2LineSegs(NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint, saberTip, bottom, top, saberPoint,
													   axisPoint);
				if (dist < bestDist) {
					bestDist = dist;
	...
	dist = ShortestLineSegBewteen2LineSegs(NPC->enemy->client->ps.saber[closestSaberNum].blade[closestBladeNum].muzzlePoint, saberTip, bottom, top, saberPoint,
										   axisPoint);
	...
	if ((evasionType = Jedi_SaberBlockGo(NPC, &ucmd, hitloc, dir, NULL, dist)) != EVASION_NONE) { // did some sort of evasion
		if (evasionType != EVASION_DODGE) {														  //(not dodge)
	...
	if (evasionType != EVASION_DUCK_PARRY && evasionType != EVASION_JUMP_PARRY && evasionType != EVASION_JUMP && evasionType != EVASION_DUCK &&
		evasionType != EVASION_FJUMP) {
		if (Jedi_CheckEvadeSpecialAttacks() != EVASION_NONE) { // got a new evasion!
			// see if it's okay to jump
...
static void Jedi_EvasionSaber(vec3_t enemy_movedir, float enemy_dist, vec3_t enemy_dir) {
	...
	if (!NPC->enemy->client) {
		...
		return;
	} else if ((NPC->client->ps.saberEventFlags & SEF_LOCK_WON) && NPC->enemy->painDebounceTime > level.time) { // pressing the advantage of winning a saber
																												// lock
		return;
	}
	...
	if (Q_irand(0, 100) < evasionChance) { // check to see if he's coming at me
		...
		if (Q_flrand(0.25, 1) < facingAmt) { // coming at/facing me!
			...
			if (NPC->client->ps.weaponTime || NPC->client->ps.saberInFlight || NPC->client->NPC_class == CLASS_BOBAFETT ||
				...
				NPC->client->NPC_class == CLASS_ROCKETTROOPER) { // I'm attacking or recovering from a parry, can only try to strafe/jump right now
			...
			case 3:
				...
				if (Jedi_DecideKick() // let's try a kick
					&& (G_PickAutoMultiKick(NPC, qfalse, qtrue) != LS_NONE ||
						(G_CanKickEntity(NPC, NPC->enemy) && G_PickAutoKick(NPC, NPC->enemy, qtrue) != LS_NONE))) { // kicked
					TIMER_Set(NPC, "kickDebounce", Q_irand(3000, 10000));
				} else if ((NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) &&
			...
			default:
				...
				if (!Q_irand(0, 5) || !Jedi_Strafe(300, 1000, 0, 1000, qfalse)) { // certain chance they will pick an alternative evasion
					...
					if (Jedi_DecideKick() && G_CanKickEntity(NPC, NPC->enemy) && G_PickAutoKick(NPC, NPC->enemy, qtrue) != LS_NONE) { // kicked!
						TIMER_Set(NPC, "kickDebounce", Q_irand(3000, 10000));
						...
						if (shooting_lightning || (!Q_irand(0, 2) && NPCInfo->stats.aggression < 4 && TIMER_Done(NPC, "parryTime"))) {
							if ((NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) && !shooting_lightning &&
								...
								ForceThrow(NPC, qfalse);
									   ...
									   NPC->client->ps.forceRageRecoveryTime < level.time && !(NPC->client->ps.forcePowersActive & (1 << FP_RAGE)) &&
					...
					if (!(NPCInfo->scriptFlags & SCF_NO_ACROBATICS) && NPC->client->ps.forceRageRecoveryTime < level.time &&
						!(NPC->client->ps.forcePowersActive & (1 << FP_RAGE)) && (NPCInfo->rank == RANK_CREWMAN || NPCInfo->rank > RANK_LT_JG) &&
						!PM_InKnockDown(&NPC->client->ps) && !Q_irand(0, 5)) { // FIXME: make this a function call?
						// FIXME: check for clearance, safety of landing spot?
						if (NPC->client->NPC_class == CLASS_BOBAFETT || (NPC->client->NPC_class == CLASS_REBORN && NPC->s.weapon != WP_SABER) ||
...
static void Jedi_SetEnemyInfo(vec3_t enemy_dest, vec3_t enemy_dir, float *enemy_dist, vec3_t enemy_movedir, float *enemy_movespeed, int prediction) {
	...
	} else {						// if he's too far away, see if he's at least facing us or coming towards us
		if (*enemy_dist <= 32.0f) { // has to be facing us
			vec3_t eAngles = {0, NPC->currentAngles[YAW], 0};
			if (InFOV(NPC->currentOrigin, NPC->enemy->currentOrigin, eAngles, 30, 90)) { // in striking range
				enemy_in_striking_range = qtrue;
			}
		}
		if (*enemy_dist >= 64.0f) { // we have to be approaching each other
			float vDot = 1.0f;
			if (!VectorCompare(NPC->client->ps.velocity, vec3_origin)) { // I am moving, see if I'm moving toward the enemy
				vec3_t eDir;
				...
				vDot = DotProduct(eDir, NPC->client->ps.velocity);
					   ...
					   !VectorCompare(NPC->enemy->client->ps.velocity, vec3_origin)) { // I'm not moving, but the enemy is, see if he's moving towards me
				vec3_t meDir;
			...
			} else { // neither of us is moving, below check will fail, so just return
				return;
			}
			if (vDot >= *enemy_dist) { // moving towards each other
				enemy_in_striking_range = qtrue;
			}
		}
	}
...
static void Jedi_FaceEnemy(qboolean doPitch) {
	...
	if (NPC->client->NPC_class == CLASS_BOBAFETT && TIMER_Done(NPC, "flameTime") && NPC->s.weapon != WP_NONE && NPC->s.weapon != WP_DISRUPTOR &&
		...
		NPC->s.weapon != WP_DET_PACK && NPC->s.weapon != WP_STUN_BATON && NPC->s.weapon != WP_MELEE) { // boba leads his enemy
		if (NPC->health < NPC->max_health * 0.5f) {													   // lead
	...
	if (!NPC->client->ps.saberInFlight && (NPC->client->ps.legsAnim == BOTH_A2_STABBACK1 || NPC->client->ps.legsAnim == BOTH_CROUCHATTACKBACK1 ||
										   NPC->client->ps.legsAnim == BOTH_ATTACK_BACK || NPC->client->ps.legsAnim == BOTH_A7_KICK_B)) { // point *away*
		GetAnglesForDirection(enemy_eyes, eyes, angles);
	} else if (NPC->client->ps.legsAnim == BOTH_A7_KICK_R) { // keep enemy to right
	} else if (NPC->client->ps.legsAnim == BOTH_A7_KICK_L) { // keep enemy to left
	...
	if (ucmd.forwardmove > 0) {
		if (!TIMER_Done(NPC, "moveback") || !TIMER_Done(NPC, "movenone")) {
			...
			if (TIMER_Done(NPC, "lastmoveforward")) {
				...
				// so we don't keep doing this over and over again - new nav stuff makes them coast to a stop, so they could be just slowing down from the last
				// "moveback" timer's ending...
				TIMER_Set(NPC, "lastmoveforward", holdDirTime + Q_irand(1000, 2000));
			}
		} else { // NOTE: edge checking should stop me if this is bad... but what if it sends us colliding into the enemy?
		...
		} else { // NOTE: edge checking should stop me if this is bad...
...
static void Jedi_CombatIdle(int enemy_dist) {
	...
	if (NPC->client->ps.forcePowersActive & (1 << FP_RAGE) ||
		NPC->client->ps.forceRageRecoveryTime > level.time) { // never taunt while raging or recovering from rage
		return;
	}
	if (NPC->client->ps.stats[STAT_WEAPONS] & (1 << WP_SCEPTER)) { // never taunt when holding scepter
		return;
	}
	if (NPC->client->ps.saber[0].type == SABER_SITH_SWORD) { // never taunt when holding sith sword
		return;
	}
	...
	if (enemy_dist >= 64) { // FIXME: only do this if standing still?
		...
		if (Q_irand(2, chance) < NPCInfo->stats.aggression) {
			if (TIMER_Done(NPC, "chatter")) { // FIXME: add more taunt behaviors
				...
				if (enemy_dist > 200 && NPC->client->NPC_class != CLASS_BOBAFETT && (NPC->client->NPC_class != CLASS_REBORN || NPC->s.weapon == WP_SABER) &&
					NPC->client->NPC_class != CLASS_ROCKETTROOPER && NPC->client->ps.SaberActive() && !Q_irand(0, 5)) { // taunt even more, turn off the saber
					// FIXME: don't do this if health low?
					if (NPC->client->ps.saberAnimLevel != SS_STAFF && NPC->client->ps.saberAnimLevel != SS_DUAL) { // those taunts leave saber on
						WP_DeactivateSaber(NPC);
					}
...
static qboolean Jedi_AttackDecide(int enemy_dist) {
	...
	if (NPC->client->ps.saberEventFlags & SEF_LOCK_WON) { // we won a saber lock, press the advantage with an attack!
		int chance = 0;
		if ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER)) { // desann and luke
			chance = 20;
		} else if (NPC->client->NPC_class == CLASS_TAVION || NPC->client->NPC_class == CLASS_ALORA) { // tavion
			chance = 10;
		} else if (NPC->client->NPC_class == CLASS_SHADOWTROOPER) { // shadowtrooper
			chance = 5;
	...
	if (NPC->client->NPC_class == CLASS_TAVION || NPC->client->NPC_class == CLASS_ALORA || NPC->client->NPC_class == CLASS_SHADOWTROOPER ||
	...
	if (!(ucmd.buttons & BUTTON_ATTACK) && !(ucmd.buttons & BUTTON_ALT_ATTACK) && !(ucmd.buttons & BUTTON_FORCE_FOCUS)) { // not already attacking
		// Try to attack
		WeaponThink(qtrue);
	}
	...
	if (ucmd.buttons & BUTTON_ATTACK && !NPC_Jumping()) { // attacking
		/*
...
static void Jedi_CheckEnemyMovement(float enemy_dist) {
	...
	if (!(NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER)) {
		if (PM_KickingAnim(NPC->enemy->client->ps.legsAnim) &&
			...
			&& (NPC->enemy->client->ps.legsAnim == BOTH_A7_KICK_RL || NPC->enemy->client->ps.legsAnim == BOTH_A7_KICK_BF ||
				NPC->enemy->client->ps.legsAnim == BOTH_A7_KICK_S || (NPC->enemy->enemy && NPC->enemy->enemy == NPC))) { // run into the kick!
			ucmd.forwardmove = ucmd.rightmove = ucmd.upmove = 0;
			VectorClear(NPC->client->ps.moveDir);
			Jedi_Advance();
		} else if (NPC->enemy->client->ps.torsoAnim == BOTH_A7_HILT && NPC->client->ps.groundEntityNum != ENTITYNUM_NONE) { // run into the hilt bash
			// FIXME : only if in front!
			ucmd.forwardmove = ucmd.rightmove = ucmd.upmove = 0;
			VectorClear(NPC->client->ps.moveDir);
			Jedi_Advance();
		} else if ((NPC->enemy->client->ps.torsoAnim == BOTH_A6_FB || NPC->enemy->client->ps.torsoAnim == BOTH_A6_LR) &&
				   NPC->client->ps.groundEntityNum != ENTITYNUM_NONE) { // run into the attack
			// FIXME : only if on R/L or F/B?
			ucmd.forwardmove = ucmd.rightmove = ucmd.upmove = 0;
			VectorClear(NPC->client->ps.moveDir);
			Jedi_Advance();
		} else if (NPC->enemy->enemy && NPC->enemy->enemy == NPC) { // enemy is mad at *me*
			if (NPC->enemy->client->ps.legsAnim == BOTH_JUMPFLIPSLASHDOWN1 || NPC->enemy->client->ps.legsAnim == BOTH_JUMPFLIPSTABDOWN ||
				NPC->enemy->client->ps.legsAnim == BOTH_FLIP_ATTACK7) { // enemy is flipping over me
...
void RT_CheckJump(void) {
	...
	if (!NPCInfo->goalEntity) {
		if (NPC->enemy) {
			// FIXME: debounce this?
			if (TIMER_Done(NPC, "roamTime") && Q_irand(0, 9)) { // okay to try to find another spot to be
				...
				if (enemyDistSq > (2048 * 2048)) { // hmm, close in?
					cpFlags |= CP_APPROACH_ENEMY;
				} else if (enemyDistSq < (256 * 256)) { // back off!
					cpFlags |= CP_RETREAT;
				}
				int sendFlags = cpFlags;
				int cp = NPC_FindCombatPointRetry(NPC->currentOrigin, NPC->currentOrigin, NPC->currentOrigin, &sendFlags, 256, NPCInfo->lastFailedCombatPoint);
				if (cp == -1) { // try again, no route needed since we can rocket-jump to it!
					cpFlags &= ~CP_HAS_ROUTE;
					cp = NPC_FindCombatPointRetry(NPC->currentOrigin, NPC->currentOrigin, NPC->currentOrigin, &cpFlags, 256, NPCInfo->lastFailedCombatPoint);
				}
				if (cp != -1) {
					NPC_SetMoveGoal(NPC, level.combatPoints[cp].origin, 8, qtrue, cp);
				} else { // FIXME: okay to do this if have good close-range weapon...
	...
	if (fabs(vec2Goal[2]) < 32) { // not a big height diff, see how far it is
		vec2Goal[2] = 0;
		if (VectorLengthSquared(vec2Goal) < (256 * 256)) { // too close!  Don't rocket-jump to it...
			return;
		}
	}
	// If we can't get straight at him
	if (!Jedi_ClearPathToSpot(jumpPos, jumpEntNum)) { // hunt him down
		...
		{														// can macro-navigate to him
			...
			if (STEER::HasBeenBlockedFor(NPC, 2000)) {			 // try to jump to the blockedTargetPosition
...
static void Jedi_Combat(void) {
	...
	if (NPC_Jumping()) { // I'm in the middle of a jump, so just see if I should attack
...
void NPC_Jedi_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other, const vec3_t point, int damage, int mod, int hitLoc) {
	...
	if (other && other->client && !OnSameTeam(self, other)) { // hit by a client
		// FIXME: delay this until *after* the pain anim?
		if (mod == MOD_FORCE_GRIP || mod == MOD_FORCE_LIGHTNING || mod == MOD_FORCE_DRAIN) { // see if we should turn on absorb
			if ((self->client->ps.forcePowersKnown & (1 << FP_ABSORB)) != 0 &&
				(self->client->ps.forcePowersActive & (1 << FP_ABSORB)) == 0) { // know absorb and not already using it
		...
		} else if (damage > Q_irand(5, 20)) { // respectable amount of normal damage
			if ((self->client->ps.forcePowersKnown & (1 << FP_PROTECT)) != 0 &&
				(self->client->ps.forcePowersActive & (1 << FP_PROTECT)) == 0) { // know protect and not already using it
				if (other->s.number >= MAX_CLIENTS								 // enemy is an NPC
					|| Q_irand(0, g_spskill->integer + 1))						 // enemy is player
				{
					if (Q_irand(0, self->NPC->rank) > RANK_ENSIGN) {
						if (!Q_irand(0, 1)) {
							if (other->s.number < MAX_CLIENTS &&
								((self->NPC->aiFlags & NPCAI_BOSS_CHARACTER) || self->client->NPC_class == CLASS_SHADOWTROOPER) &&
		...
		if (!NPC->enemy) { // still not mad
			if (!best_enemy) {
				...
				if (NPC_ClearLOS(best_enemy)) {						// we have a clear (of architecture) LOS to him
					if ((NPCInfo->aiFlags & NPCAI_NO_JEDI_DELAY)) { // just get mad right away
					...
					} else if (best_enemy->s.number) { // just attack
						G_SetEnemy(NPC, best_enemy);
						NPCInfo->stats.aggression = 3;
...
qboolean Jedi_CanPullBackSaber(gentity_t *self) {
	...
	if (self->client->NPC_class == CLASS_SHADOWTROOPER || self->client->NPC_class == CLASS_ALORA ||
...
void NPC_BSJedi_FollowLeader(void) {
	...
	if (!NPC->enemy && NPC->health < NPC->max_health && (NPC->client->ps.forcePowersKnown & (1 << FP_HEAL)) != 0 &&
		(NPC->client->ps.forcePowersActive & (1 << FP_HEAL)) == 0 && TIMER_Done(NPC, "FollowHealDebouncer")) {
		if (Q_irand(0, 3) == 0) {
...
qboolean Jedi_CheckKataAttack(void) {
	if (NPCInfo->rank >= RANK_LT_COMM) {	  // only top-level guys and bosses do this
		if ((ucmd.buttons & BUTTON_ATTACK)) { // attacking
			if ((g_saberNewControlScheme->integer && !(ucmd.buttons & BUTTON_FORCE_FOCUS)) ||
				(!g_saberNewControlScheme->integer && !(ucmd.buttons & BUTTON_ALT_ATTACK))) { // not already going to do a kata move somehow
				if (NPC->client->ps.groundEntityNum != ENTITYNUM_NONE) {					  // on the ground
					if (ucmd.upmove <= 0 && NPC->client->ps.forceJumpCharge <= 0) {			  // not going to try to jump
						/*
						if ( (NPCInfo->scriptFlags&SCF_NO_ACROBATICS) )
...
static void Jedi_Attack(void) {
	...
	if (NPC->painDebounceTime > level.time) {
		...
		NPC_UpdateAngles(qtrue, qtrue);
		if (NPC->client->ps.torsoAnim == BOTH_KYLE_GRAB) { // see if we grabbed enemy
			if (NPC->client->ps.torsoAnimTimer <= 200) {
				if (Kyle_CanDoGrab() && NPC_EnemyRangeFromBolt(NPC->handRBolt) < 88.0f) { // grab him!
					Kyle_GrabEnemy();
					return;
				} else {
					NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_KYLE_MISS, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
	...
	if (NPC->client->ps.saberLockTime > level.time) {
		...
		// based on my skill, hit attack button every other to every several frames in order to push enemy back
			...
			} else if (NPC->client->NPC_class == CLASS_TAVION || NPC->client->NPC_class == CLASS_SHADOWTROOPER || NPC->client->NPC_class == CLASS_ALORA ||
	...
	} else if (NPC->enemy && NPC->enemy->NPC && NPC->enemy->NPC->charmedTime > level.time) { // my enemy was charmed
		if (OnSameTeam(NPC, NPC->enemy)) {													 // has been charmed to be on my team
			G_ClearEnemy(NPC);
		}
	}
	if (NPC->client->playerTeam == TEAM_ENEMY && NPC->client->enemyTeam == TEAM_PLAYER && NPC->enemy && NPC->enemy->client &&
		NPC->enemy->client->playerTeam != NPC->client->enemyTeam && OnSameTeam(NPC, NPC->enemy) &&
		!(NPC->svFlags & SVF_LOCKEDENEMY)) { // an evil jedi somehow got another evil NPC as an enemy, they were probably charmed and it's run out now
	...
	if (!(NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) || ((NPC->client->ps.forcePowersActive & (1 << FP_HEAL)) &&
														NPC->client->ps.forcePowerLevel[FP_HEAL] < FORCE_LEVEL_2)) { // this is really stupid, but okay...
		ucmd.forwardmove = 0;
		ucmd.rightmove = 0;
		if (ucmd.upmove > 0) {
			ucmd.upmove = 0;
	...
	if (Jedi_CheckKataAttack()) { // doing a kata attack
	} else {					  // check other special combat behavior
		if (NPC->client->NPC_class != CLASS_BOBAFETT && (NPC->client->NPC_class != CLASS_REBORN || NPC->s.weapon == WP_SABER) &&
			NPC->client->NPC_class != CLASS_ROCKETTROOPER) {
			if (NPC->client->NPC_class == CLASS_TAVION || NPC->client->NPC_class == CLASS_SHADOWTROOPER || NPC->client->NPC_class == CLASS_ALORA ||
		...
		if (NPC->client->NPC_class == CLASS_ALORA) {
			if ((ucmd.buttons & BUTTON_ALT_ATTACK)) { // chance of doing a special dual saber throw
			...
			} else if (NPC->enemy && ucmd.forwardmove > 0 && fabs((float)ucmd.rightmove) < 32 && !(ucmd.buttons & BUTTON_WALKING) &&
					   !(ucmd.buttons & BUTTON_ATTACK) && NPC->client->ps.saberMove == LS_READY &&
					   NPC->client->ps.legsAnim == BOTH_RUN_DUAL) { // running at us, not attacking
				if (Distance(NPC->enemy->currentOrigin, NPC->currentOrigin) > 80) {
					if (NPC->client->ps.legsAnim == BOTH_FLIP_F || NPC->client->ps.legsAnim == BOTH_ALORA_FLIP_1 ||
						NPC->client->ps.legsAnim == BOTH_ALORA_FLIP_2 || NPC->client->ps.legsAnim == BOTH_ALORA_FLIP_3) {
						if (NPC->client->ps.legsAnimTimer <= 200 && Q_irand(0, 2)) { // go ahead and start anotther
	...
	if (VectorCompare(NPC->client->ps.moveDir, vec3_origin) && (ucmd.forwardmove || ucmd.rightmove)) { // using ucmds to move this turn, not NAV
		if ((ucmd.buttons & BUTTON_WALKING)) { // FIXME: NAV system screws with speed directly, so now I have to re-set it myself!
...
qboolean Rosh_BeingHealed(gentity_t *self) {
	if (self && self->NPC && self->client && (self->NPC->aiFlags & NPCAI_ROSH) && (self->flags & FL_UNDYING) &&
	...
	if (NPC->client && NPC->client->leader && NPC->client->leader->client) {
		if (DistanceSquared(NPC->client->leader->currentOrigin, NPC->currentOrigin) <= (256 * 256) &&
			...
			if (NPC->ghoul2.size()) {
				...
				gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->playerModel, (Q_irand(0, 1) ? NPC->handLBolt : NPC->handRBolt), &boltMatrix, angles,
									   NPC->currentOrigin, (cg.time ? cg.time : level.time), NULL, NPC->s.modelScale);
			...
			if (NPC->client->leader->client) {
				if (NPC->client->leader->client->ps.legsAnim == BOTH_FORCEHEAL_START &&
					NPC->client->leader->health >= NPC->client->leader->max_health) { // let him get up now
					NPC_SetAnim(NPC->client->leader, SETANIM_BOTH, BOTH_FORCEHEAL_STOP, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					// FIXME: temp effect
					G_PlayEffect(G_EffectIndex("force/kothos_recharge.efx"), NPC->client->leader->playerModel, 0, NPC->client->leader->s.number,
								 NPC->client->leader->currentOrigin, NPC->client->leader->client->ps.torsoAnimTimer, qfalse);
					...
					G_PlayEffect(G_EffectIndex("force/kothos_recharge.efx"), NPC->client->leader->playerModel, 0, NPC->client->leader->s.number,
								 NPC->client->leader->currentOrigin, 500, qfalse);
			...
			// now protect me, too
			if (g_spskill->integer) { // not on easy
...
void Kothos_PowerRosh(void) {
	if (NPC->client && NPC->client->leader) {
		if (Distance(NPC->client->leader->currentOrigin, NPC->currentOrigin) <= 512.0f &&
			...
			if (NPC->client->leader->client) { // hmm, give him some force?
...
qboolean Jedi_InSpecialMove(void) {
	if (NPC->client->ps.torsoAnim == BOTH_KYLE_PA_1 || NPC->client->ps.torsoAnim == BOTH_KYLE_PA_2 || NPC->client->ps.torsoAnim == BOTH_KYLE_PA_3 ||
		NPC->client->ps.torsoAnim == BOTH_PLAYER_PA_1 || NPC->client->ps.torsoAnim == BOTH_PLAYER_PA_2 || NPC->client->ps.torsoAnim == BOTH_PLAYER_PA_3 ||
		NPC->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_END || NPC->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRABBED) {
		NPC_UpdateAngles(qtrue, qtrue);
		return qtrue;
	}

	if (Jedi_InNoAIAnim(NPC)) { // in special anims, don't do force powers or attacks, just face the enemy
		if (NPC->enemy) {
			NPC_FaceEnemy(qtrue);
		} else {
			NPC_UpdateAngles(qtrue, qtrue);
	...
	*/
	...
	if (NPC->client->ps.torsoAnim == BOTH_SCEPTER_START) {
		if (NPC->client->ps.torsoAnimTimer <= 100) { // go into the hold
	...
	if (Jedi_CultistDestroyer(NPC)) {
		if (!NPC->takedamage) { // ready to explode
	...
	if (NPC->client->NPC_class == CLASS_REBORN) {
		if ((NPCInfo->aiFlags & NPCAI_HEAL_ROSH)) {
			if (!NPC->client->leader) { // find Rosh
			...
			if (NPC->client->leader) {
				...
				*/
				...
				if (helpingRosh) {
					...
					if (NPC->enemy && Kothos_Retreat()) {
						...
						if (TIMER_Done(NPC, "attackDelay")) {
							if (NPC->painDebounceTime > level.time || (NPC->health < 100 && Q_irand(-20, (g_spskill->integer + 1) * 10) > 0) ||
				...
				} else if (!G_ClearLOS(NPC, NPC->client->leader) || DistanceSquared(NPC->currentOrigin, NPC->client->leader->currentOrigin) >
																		(512 * 512)) { // can't see Rosh or too far away, catch up with him
					if (!TIMER_Done(NPC, "attackDelay")) {
						NPC->flags &= ~FL_LOCK_PLAYER_WEAPONS;
					}
					NPC_BSJedi_FollowLeader();
			...
			if ((NPC->flags & FL_UNDYING)) {								 // Vil and/or Dasariah still around to heal me
				if (NPC->health == 1										 // need healing
					|| NPC->client->ps.powerups[PW_INVINCIBLE] > level.time) // being healed
				{															 // FIXME: custom anims
					if (Rosh_TwinPresent(NPC)) {
						if (!NPC->client->ps.weaponTime) { // not attacking
							if (NPC->client->ps.legsAnim != BOTH_FORCEHEAL_START &&
								NPC->client->ps.legsAnim != BOTH_FORCEHEAL_STOP) { // get down and wait for Vil or Dasariah to help us
								// FIXME: sound?
								NPC->client->ps.legsAnimTimer = NPC->client->ps.torsoAnimTimer = 0;
...
void NPC_BSJedi_Default(void) {
	...
	{								   // have an enemy
		...
		if (Jedi_CultistDestroyer(NPC) && !NPCInfo->charmedTime) { // destroyer
			// permanent effect
