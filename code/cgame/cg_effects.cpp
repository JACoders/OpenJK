...
// localEntity_t *CG_MakeExplosion( vec3_t origin, vec3_t dir,
//								qhandle_t hModel, int
//numFrames, qhandle_t shader, 								int msec, qboolean isSprite, float scale ) {
//	return CG_MakeExplosion( origin, dir, hModel, numFrames, shader, msec,
//isSprite, scale, 0 );
//}
...
shake_speed, qboolean smoke )
{
        ...
        for ( i = 0; i < numSparks; i++ )
        {
                ...
                particle = FX_AddTrail( origin,
                                                                ...
                                                                rand() &
FXF_BOUNCE); if ( particle == NULL ) return;

                FXE_Spray( normal, 500, 150, 1.0f, 768 + (rand() & 255),
(FXPrimitive *) particle );
        }
        ...
        for ( i = 0; i < 4; i++ )
        {
                VectorSet( temp_org, new_org[0] + (Q_flrand(-1.0f, 1.0f)
* 16.0f), new_org[1] + (Q_flrand(-1.0f, 1.0f) * 16.0f), new_org[2] +
(Q_flrand(0.0f, 1.0f) * 4.0f) ); VectorSet( temp_vel, velocity[0] +
(Q_flrand(-1.0f, 1.0f) * 8.0f), velocity[1] + (Q_flrand(-1.0f, 1.0f) * 8.0f),
velocity[2] + (Q_flrand(-1.0f, 1.0f) * 8.0f) );

                FX_AddSprite(	temp_org,
                                                temp_vel,
                                                NULL,
                                                64.0f + (Q_flrand(0.0f, 1.0f)
* 32.0f), 16.0f, 1.0f, 0.0f, 20.0f + (Q_flrand(-1.0f, 1.0f) * 90.0f), 0.5f,
                                                1500.0f,
...
FXF_USE_ALPHA_CHAN );
        ...
        le = CG_MakeExplosion( origin, direction, cgs.media.explosionModel, 6,
cgs.media.surfaceExplosionShader, 500, qfalse, radius * 0.02f +
(Q_flrand(0.0f, 1.0f) * 0.3f) ); le->light = 150; VectorSet( le->lightColor,
0.9f, 0.8f, 0.5f );

        for ( i = 0; i < NUM_EXPLOSIONS-1; i ++)
        {
                VectorSet( new_org, (origin[0] + (16 + (Q_flrand(-1.0f, 1.0f) *
8))*Q_flrand(-1.0f, 1.0f)), (origin[1] + (16 + (Q_flrand(-1.0f, 1.0f) *
8))*Q_flrand(-1.0f, 1.0f)), (origin[2] + (16 + (Q_flrand(-1.0f, 1.0f) *
8))*Q_flrand(-1.0f, 1.0f)) ); le = CG_MakeExplosion( new_org, direction,
cgs.media.explosionModel, 6, cgs.media.surfaceExplosionShader, 300 + (rand() &
99), qfalse, radius * 0.05f + (Q_flrand(-1.0f, 1.0f) *0.3f) );
        }

        //Shake the camera
        CG_ExplosionEffects( origin, shake_speed, 350, 750 );

        // The level designers wanted to be able to turn the smoke spawners off.
The rationale is that they
        //	want to blow up catwalks and such that fall down...when that
happens, it shouldn't really leave a mark
        //	and a smoke spewer at the explosion point...
        if ( smoke )
        {
                VectorMA( origin, -8, normal, temp_org );
//		FX_AddSpawner( temp_org, normal, NULL, NULL, 100,
Q_flrand(0.0f, 1.0f)*25.0f, 5000.0f, (void *) CG_SmokeSpawn );

                //Impact mark
                //FIXME: Replace mark
                //CG_ImpactMark( cgs.media.burnMarkShader, origin, normal,
Q_flrand(0.0f, 1.0f)*360, 1,1,1,1, qfalse, 8, qfalse );
        }
}
*/
/*
...
void CG_MiscModelExplosion(vec3_t mins, vec3_t maxs, int size,
  ...
  // FIXME: real precache .. VERify that these need to be here...don't think
  // they would because the effects should be registered in g_breakable
...
void CG_Chunks(int owner, vec3_t origin, const vec3_t normal, const vec3_t mins,
  ...
  // Set up our chunk sound info...breaking sounds are done here so they are
  // done once on breaking..some return instantly because the chunks are done
  // with effects instead of models
  switch (chunkType) {
  case MAT_GLASS:
    if (!customSound) {
  ...
  case MAT_LT_STONE:
  case MAT_GREY_STONE:
  case MAT_WHITE_METAL: // not quite sure what this stuff is supposed to
                        // be...it's for Stu
    if (!customSound) {
  ...
  case MAT_GLASS_METAL:
    if (!customSound) {
      cgi_S_StartSound(
          NULL, owner, CHAN_BODY,
          cgs.media
              .glassChunkSound); // FIXME: should probably have a custom sound
      bounce = LEBS_METAL;
    }
    break;
  case MAT_CRATE1:
  case MAT_CRATE2:
    if (!customSound) {
      cgi_S_StartSound(NULL, owner, CHAN_BODY,
                       cgs.media.crateBreakSound[Q_irand(0, 1)]);
    }
    break;
  case MAT_METAL:
  case MAT_METAL2:
  ...
  case MAT_ROPE:
    ...
    if ( !customSound )
    {
            cgi_S_StartSound( NULL, owner, CHAN_BODY, cgi_S_RegisterSound( ""
    ));  FIXME:  needs a sound
    }
    */
    return;
  ...
  for (i = 0; i < numChunks; i++) {
    ...
    if (!chunk) {
      // No custom chunk.  Pick a random chunk type at run-time so we don't get
      // the same chunks
      switch (chunkType) {
    ...
    // It wouldn't look good to throw a bunch of RGB axis models...so make sure
    ...
    if (chunkModel) {
      ...
      // spawn chunk roughly in the bbox of the thing...bias towards center in
      // case thing blowing up doesn't complete fill its bbox.
      for (j = 0; j < 3; j++) {
        r = Q_flrand(0.0f, 1.0f) * 0.8f + 0.1f;
      ...
      // Move out from center of thing, otherwise you can end up things moving
      // across the brush in an undesirable direction.  Visually looks wrong
      VectorSubtract(re->origin, origin, dir);
      VectorNormalize(dir);
      VectorScale(dir, Q_flrand(speed * 0.5f, speed * 1.25f) * speedMod,
                  le->pos.trDelta);

      // Angular Velocity
      VectorSet(le->angles.trBase, Q_flrand(0.0f, 1.0f) * 360,
                Q_flrand(0.0f, 1.0f) * 360, Q_flrand(0.0f, 1.0f) * 360);

      le->angles.trDelta[0] = Q_flrand(-1.0f, 1.0f);
      le->angles.trDelta[1] = Q_flrand(-1.0f, 1.0f);
      le->angles.trDelta[2] = 0; // don't do roll

      VectorScale(le->angles.trDelta, Q_flrand(0.0f, 1.0f) * 600.0f + 200.0f,
                  le->angles.trDelta);
      ...
      AxisCopy(axisDefault, re->axis); // could do an angles to axis, but this
                                       // is cheaper and works ok
      for (k = 0; k < 3; k++) {
...
void CG_TestLine(vec3_t start, vec3_t end, int time, unsigned int color,
  ...
  re->customShader =
      cgs.media
          .whiteShader; // trap_R_RegisterShaderNoMip("textures/colombia/canvas_doublesided");

  re->shaderTexCoord[0] = re->shaderTexCoord[1] = 1.0f;

  if (color == 0) {
    re->shaderRGBA[0] = re->shaderRGBA[1] = re->shaderRGBA[2] =
...
// Since we have shared verts when we tesselate the glass sheet, it helps to
// have a
//	random offset table set up up front...so that we can have more random
//looking breaks.

static float offX[20][20], offZ[20][20];

static void CG_DoGlassQuad(vec3_t p[4], vec2_t uv[4], bool stick, int time,
                           vec3_t dmgDir) {
  float bounce;
  vec3_t rotDelta;
  vec3_t vel, accel;
  ...
  // Set up acceleration due to gravity, 800 is standard QuakeIII gravity, so
  // let's use something close
  ...
  // Set up our random rotate, we only do PITCH and YAW, not ROLL.  This is
  // something like degrees per second
  VectorSet(rotDelta, Q_flrand(-1.0f, 1.0f) * 40.0f,
            Q_flrand(-1.0f, 1.0f) * 40.0f, 0.0f);

  CPoly *pol = FX_AddPoly(
      p, uv, 4,           // verts, ST, vertCount
      vel, accel,         // motion
      0.15f, 0.0f, 85.0f, // alpha start, alpha end, alpha parm ( begin alpha
                          // fade when 85% of life is complete )
      rgb1, rgb1, 0.0f,   // rgb start, rgb end, rgb parm ( not used )
      rotDelta, bounce, time, // rotation amount, bounce, and time to delay
                              // motion for ( zero if no delay );
...
// bilinear
// f(p',q') = (1 - y) × {[(1 - x) × f(p,q)] + [x × f(p,q+1)]} + y × {[(1 - x) ×
// f(p+1,q)] + [x × f(p+1,q+1)]}.
...
void CG_DoGlass(vec3_t verts[4], vec3_t normal, vec3_t dmgPt, vec3_t dmgDir,
                float dmgRadius) {
  int i, t;
  ...
  // To do a smarter tesselation, we should figure out the relative height and
  // width of the brush face,
  //	then use this to pick a lod value from 1-3 in each axis.  This will give
  //us 1-9 lod levels, which will 	hopefully be sufficient.
  CG_CalcHeightWidth(verts, &height, &width);

  cgi_S_StartSound(dmgPt, -1, CHAN_AUTO,
                   cgi_S_RegisterSound("sound/effects/glassbreak1.wav"));
  ...
  /*	else if ( height > 220 ) // was originally mxHeight = 20....but removing
     this whole section because it causes huge number of chunks...which is bad
          {
                  stepHeight = 0.075f;
                  mxHeight = 15;
  ...
  /*	else if ( width > 220 ) // don't do this because it causes too much chug
     with large glass panes...especially when more than one pane can be broken
     at a time
          {
                  stepWidth = 0.075f;
                  mxWidth = 15;
  ...
  for (z = 0.0f, i = 0; z < 1.0f; z += stepHeight, i++) {
    for (x = 0.0f, t = 0; x < 1.0f; x += stepWidth, t++) {
      // This is nasty..we do this because we don't want to add a random offset
      // on the edge of the glass brush
      //	...but we do in the center, otherwise the breaking scheme looks
      //way too orderly
      if (t > 0 && t < mxWidth) {
        xx = x - offX[i][t];
      } else {
      ...
      float dif = DistanceSquared(subVerts[0], dmgPt) * timeDecay -
                  Q_flrand(0.0f, 1.0f) * 32;

      // If we decrease dif, we are increasing the impact area, making it more
      // likely to blow out large holes
...
/*void CG_Seeker( centity_t *cent )
{
        ...
        // must match cg_effects ( CG_Seeker ) & g_weapon ( SeekerAcquiresTarget
) & cg_weapons ( CG_FireSeeker ) angle = cg.time * 0.004f; c = cos( angle );

        seekerOrg[0] = cent->lerpOrigin[0] + 18 * c;
        seekerOrg[1] = cent->lerpOrigin[1] + 18 * sin(angle);
        seekerOrg[2] = cent->lerpOrigin[2] +
cg.predicted_player_state.viewheight + 8 + (3 * cos(cg.time * 0.001));
        ...
        VectorCopy( cent->lerpAngles, viewAng ); // so the seeker faces the same
direction the player is viewAng[PITCH] = -90; // but, we don't want the seeker
facing up or down, always horizontal viewAng[YAW] += c * 15.f;
...
void CG_PlayEffectBolted(const char *fxName, const int modelIndex,
  ...
  vec3_t axis[3]; // FIXME: shouldn't this be initialized to something?  It
                  // isn't in the EV_PLAY_EFFECT call... irrelevant?
  int boltInfo;

  // pack the data into boltInfo as if we were going to send it over the network
  gi.G2API_AttachEnt(&boltInfo, &g_entities[entNum].ghoul2[modelIndex],
                     boltIndex, entNum, modelIndex);
  // send direcly to FX scheduler
  theFxScheduler.PlayEffect(
      fxName, origin, axis, boltInfo, -1, false, iLoopTime,
...
void CG_PlayEffectIDBolted(const int fxID, const int modelIndex,
  ...
  CG_PlayEffectBolted(fxName, modelIndex, boltIndex, entNum, origin, iLoopTime,
                      isRelative);
}
