...
//#define USE_LAST_SAVE_FROM_THIS_MAP	// enable this if you want to use the
//last explicity-loaded savegame from this map 	when respawning after dying, else
//it'll just load "auto" regardless 	(EF1 behaviour). I should maybe time/date
//check them though?
...
#ifdef JK2_MODE
  const char *psReference = bOpen ? "MENUS3_FAILED_TO_OPEN_SAVEGAME"
                                  : "MENUS3_FAILED_TO_CREATE_SAVEGAME";
#else
  const char *psReference = bOpen ? "MENUS_FAILED_TO_OPEN_SAVEGAME"
                                  : "MENUS3_FAILED_TO_CREATE_SAVEGAME";
#endif
  Q_strncpyz(sTemp + strlen(sTemp), va(SE_GetString(psReference), psFilename),
             sizeof(sTemp));
  strcat(sTemp, "\n");
  return sTemp;
}
...
void SG_WipeSavegame(const char *psPathlessBaseName) {
  ojk::SavedGame::remove(psPathlessBaseName);
}

// called from the ERR_DROP stuff just in case the error occured during loading
// of a saved game, because if
...
void SG_Shutdown() {
  ...
  // important to do this if we ERR_DROP during loading, else next map you load
  // after a bad save-file you'll arrive at dead :-)

  // and this bit stops people messing up the laoder by repeatedly stabbing at
  // the load key during loads...
  //
...
void SV_WipeGame_f(void) {
  ...
  //	Com_Printf("%s has been wiped\n", Cmd_Argv(1));	// wurde gelöscht in
  //german, but we've only got one string 	Com_Printf("Ok\n"); // no localization
  //of this
}
...
qboolean SV_TryLoadTransition(const char *mapname) {
  ...
  if (!SG_ReadSavegame(psFilename)) { // couldn't load a savegame
    return qfalse;
  }
#ifdef JK2_MODE
...
void SV_LoadGame_f(void) {
  ...
  if (strstr(psFilename, "..") || strstr(psFilename, "/") ||
      strstr(psFilename, "\\")) {
  ...
  // special case, if doing a respawn then check that the available auto-save
  // (if any) is from the same map
  //	as we're currently on (if in a map at all), if so, load that "auto",
  //else re-load the last-loaded file...
  //
  if (!Q_stricmp(psFilename, "*respawn")) {
    psFilename = "auto"; // default to standard respawn behaviour

    // see if there's a last-loaded file to even check against as regards
    // loading...
    //
    if (sLastSaveFileLoaded[0]) {
      ...
      if (!Q_stricmp(psMapName,
                     "_brig")) { // if you're in the brig and there is no
                                 // autosave, load the last loaded savegame
        if (!psMapNameOfAutoSave) {
          psFilename = sLastSaveFileLoaded;
        }
      } else {
#ifdef USE_LAST_SAVE_FROM_THIS_MAP
        // if the map name within the name of the last save file we explicitly
        // loaded is the same
        //	as the current map, then use that...
        //
        char *psMapNameOfLastSaveFileLoaded =
            SG_GetSaveGameMapName(sLastSaveFileLoaded);
...
#endif
            if (!(psMapName && psMapNameOfAutoSave &&
                  !Q_stricmp(psMapName, psMapNameOfAutoSave))) {
          // either there's no auto file, or it's from a different map to the
          // one we've just died on...
          //
          psFilename = sLastSaveFileLoaded;
        }
      }
  ...
  if (!SG_ReadSavegame(psFilename)) {
    gbAlreadyDoingLoad = qfalse; //	do NOT do this here now, need to wait
                                 //until client spawn, unless the load failed.
  } else {
#ifdef JK2_MODE
    Com_Printf(S_COLOR_CYAN "Done.\n");
#else
...
void SV_SaveGame_f(void) {
  ...
  if (svs.clients[0]
          .frames[svs.clients[0].netchan.outgoingSequence & PACKET_MASK]
  ...
  // this check catches deaths even the instant you die, like during a slo-mo
  // death!
  gentity_t *svent;
  ...
  if (!Q_stricmp(filename, "quik*") || !Q_stricmp(filename, "auto*")) {
    ...
    SG_StoreSaveGameComment(""); // clear previous comment/description, which
                                 // will force time/date comment.
  }
#else
...
static void WriteGame(qboolean autosave) {
  ...
  saved_game.write_chunk<int32_t>(INT_ID('G', 'A', 'M', 'E'), autosave);

  if (autosave) {
    // write out player ammo level, health, etc...
    //
    extern void SV_Player_EndOfLevelSave(void);
    SV_Player_EndOfLevelSave(); // this sets up the various cvars needed, so we
                                // can then write them to disk
    //
    ...
    saved_game.write_chunk(INT_ID('C', 'V', 'S', 'V'), s);
    ...
    saved_game.write_chunk(INT_ID('A', 'M', 'M', 'O'), s);
    ...
    saved_game.write_chunk(INT_ID('I', 'V', 'T', 'Y'), s);
    ...
    saved_game.write_chunk(INT_ID('F', 'P', 'L', 'V'), s);
  }
}
...
static qboolean ReadGame(void) {
  ...
  saved_game.read_chunk<int32_t>(INT_ID('G', 'A', 'M', 'E'), qbAutoSave);
  ...
  if (qbAutoSave) {
    ...
    saved_game.read_chunk(INT_ID('C', 'V', 'S', 'V'), s);
    ...
    saved_game.read_chunk(INT_ID('A', 'M', 'M', 'O'), s);
    ...
    saved_game.read_chunk(INT_ID('I', 'V', 'T', 'Y'), s);
    ...
    saved_game.read_chunk(INT_ID('F', 'P', 'L', 'V'), s);
...
//
extern cvar_t
    *cvar_vars; // I know this is really unpleasant, but I need access for
                // scanning/writing latched cvars during save games
...
void SG_WriteCvars(void) {
  ...
  // store count...
  //
  saved_game.write_chunk<int32_t>(INT_ID('C', 'V', 'C', 'N'), iCount);
  ...
  for (var = cvar_vars; var; var = var->next) {
    ...
    saved_game.write_chunk(INT_ID('C', 'V', 'A', 'R'), var->name,
                           static_cast<int>(strlen(var->name) + 1));

    saved_game.write_chunk(INT_ID('V', 'A', 'L', 'U'), var->string,
...
void SG_ReadCvars() {
  ...
  saved_game.read_chunk<int32_t>(INT_ID('C', 'V', 'C', 'N'), iCount);

  for (int i = 0; i < iCount; ++i) {
    saved_game.read_chunk(INT_ID('C', 'V', 'A', 'R'));

    psName = reinterpret_cast<const char *>(saved_game.get_buffer_data());

    saved_game.read_chunk(INT_ID('V', 'A', 'L', 'U'));

    psValue = reinterpret_cast<const char *>(saved_game.get_buffer_data());
...
void SG_WriteServerConfigStrings() {
  ...
  int iCount = 0;
  int i; // not in FOR statement in case compiler goes weird by reg-optimising
         // it then failing to get the address later
  ...
  saved_game.write_chunk<int32_t>(INT_ID('C', 'S', 'C', 'N'), iCount);
  ...
  for (i = 0; i < MAX_CONFIGSTRINGS; i++) {
    if (i != CS_SYSTEMINFO) {
      if (sv.configstrings[i] && strlen(sv.configstrings[i])) {
        saved_game.write_chunk<int32_t>(INT_ID('C', 'S', 'I', 'N'), i);

        saved_game.write_chunk(
            INT_ID('C', 'S', 'D', 'A'), ::sv.configstrings[i],
...
void SG_ReadServerConfigStrings(void) {
  ...
  saved_game.read_chunk<int32_t>(INT_ID('C', 'S', 'C', 'N'), iCount);
  ...
  for (int i = 0; i < iCount; i++) {
    ...
    saved_game.read_chunk<int32_t>(INT_ID('C', 'S', 'I', 'N'), iIndex);

    saved_game.read_chunk(INT_ID('C', 'S', 'D', 'A'));
...
static void SG_WriteComment(qboolean qbAutosave, const char *psMapName) {
  ...
  saved_game.write_chunk(INT_ID('C', 'O', 'M', 'M'), sComment);

  // Add Date/Time/Map stamp
  unsigned int timestamp = SG_UnixTimestamp(time(NULL));

  saved_game.write_chunk<uint32_t>(INT_ID('C', 'M', 'T', 'M'), timestamp);
...
//
int SG_GetSaveGameComment(const char *psPathlessBaseName, char *sComment,
                          char *sMapName) {
  int ret = 0;

  ojk::SavedGame &saved_game = ojk::SavedGame::get_instance();

  ojk::SavedGameHelper sgh(&ojk::SavedGame::get_instance());
  ...
  // Read description
  //
  is_succeed = sgh.try_read_chunk(INT_ID('C', 'O', 'M', 'M'));

  if (is_succeed) {
    if (sComment) {
      if (sgh.get_buffer_size() == iSG_COMMENT_SIZE) {
        std::uninitialized_copy_n(
            static_cast<const char *>(sgh.get_buffer_data()), iSG_COMMENT_SIZE,
            sComment);
      } else {
        sComment[0] = '\0';
      }
  ...
  if (is_succeed) {
    ...
    is_succeed =
        sgh.try_read_chunk<uint32_t>(INT_ID('C', 'M', 'T', 'M'), fileTime);
  ...
  if (is_succeed) {
    ...
    is_succeed = sgh.try_read_chunk<uint32_t>(INT_ID('S', 'H', 'L', 'N'),
                                              iScreenShotLength);
  }

  if (is_succeed) {
    is_succeed = sgh.try_read_chunk(INT_ID('S', 'H', 'O', 'T'));
  }
#endif
  ...
  //
  if (is_succeed) {
    is_succeed = sgh.try_read_chunk(INT_ID('M', 'P', 'C', 'M'));

    if (is_succeed) {
      if (sMapName) {
        if (sgh.get_buffer_size() == iSG_MAPCMD_SIZE) {
          std::uninitialized_copy_n(
              static_cast<const char *>(sgh.get_buffer_data()), iSG_MAPCMD_SIZE,
              sMapName);
        } else {
          sMapName[0] = '\0';
        }
...
//
#ifdef JK2_MODE
static bool SG_ReadScreenshot(bool set_as_loading_screen,
                              void *screenshot_ptr) {
  bool is_succeed = true;
  ...
  is_succeed = saved_game.try_read_chunk<uint32_t>(INT_ID('S', 'H', 'L', 'N'),
                                                   screenshot_length);

  //
  // alloc enough space plus extra 4K for sloppy JPG-decode reader to not do
  // memory access violation...
  //
  byte *jpeg_data = nullptr;

  if (is_succeed) {
    jpeg_data = static_cast<byte *>(
        ::Z_Malloc(static_cast<int>(screenshot_length + 4096),
  ...
  if (is_succeed) {
    is_succeed =
        saved_game.try_read_chunk(INT_ID('S', 'H', 'O', 'T'), jpeg_data,
  ...
  if (is_succeed) {
    ::re.LoadJPGFromBuffer(jpeg_data, screenshot_length, &image, &width,
                           &height);

    //
    // if the loaded image is the same size as the game is expecting, then copy
    // it to supplied arg (if present)...
    //
    if (width == SG_SCR_WIDTH && height == SG_SCR_HEIGHT) {
      if (screenshot_ptr) {
        ::memcpy(screenshot_ptr, image, SG_SCR_WIDTH * SG_SCR_HEIGHT * 4);
      }

      if (set_as_loading_screen) {
        ::SCR_SetScreenshot(image, SG_SCR_WIDTH, SG_SCR_HEIGHT);
      }
    } else {
      is_succeed = false;
    }
...
//
qboolean SG_GetSaveImage(const char *base_name, void *image_ptr) {
  if (!base_name) {
    return qfalse;
  }
  ...
  ojk::SavedGameHelper sgh(&saved_game);

  is_succeed = sgh.try_read_chunk(INT_ID('C', 'O', 'M', 'M'));

  if (is_succeed) {
    is_succeed = sgh.try_read_chunk(INT_ID('C', 'M', 'T', 'M'));
  }

  if (is_succeed) {
    is_succeed = SG_ReadScreenshot(false, image_ptr);
  }
...
static void SG_WriteScreenshot(qboolean qbAutosave, const char *psMapName) {
  ...
  if (qbAutosave) {
    // try to read a levelshot (any valid TGA/JPG etc named the same as the
    // map)...
    //
    ...
    pbRawScreenShot = SCR_TempRawImage_ReadFromFile(
        va("levelshots/%s.tga", psMapName), &iWidth, &iHeight, byBlank,
        qtrue); // qtrue = vert flip
  ...
  iJPGDataSize =
      re.SaveJPGToBuffer(pJPGData, bufSize, JPEG_IMAGE_QUALITY, SG_SCR_WIDTH,
                         SG_SCR_HEIGHT, pbRawScreenShot, 0, flip_vertical);
  if (qbAutosave)
    delete[] byBlank;

  saved_game.write_chunk<uint32_t>(INT_ID('S', 'H', 'L', 'N'), iJPGDataSize);

  saved_game.write_chunk(INT_ID('S', 'H', 'O', 'T'), pJPGData,
...
qboolean SG_GameAllowedToSaveHere(qboolean inCamera) {
  if (!inCamera) {
    ...
    if (sv.state != SS_GAME) {
      return qfalse; //		Com_Printf (S_COLOR_RED "You must be in a game to
                     //save.\n");
    }

    // No savegames from "_" maps
    if (!sv_mapname ||
        (sv_mapname->string != NULL && sv_mapname->string[0] == '_')) {
      return qfalse; //		Com_Printf (S_COLOR_RED "Cannot save on holodeck or
                     //brig.\n");
    }
...
qboolean SG_WriteSavegame(const char *psPathlessBaseName, qboolean qbAutosave) {
  if (!qbAutosave && !SG_GameAllowedToSaveHere(qfalse)) // full check
    return qfalse; // this prevents people saving via quick-save now during
                   // cinematics
...
// JLF
  ...
  if (!saved_game.create("current")) {
    Com_Printf(GetString_FailedToOpenSaveGame(
        "current", qfalse)); // S_COLOR_RED "Failed to create savegame\n");
  ...
  Q_strncpyz(sMapCmd, psMapName,
             sizeof(sMapCmd)); // need as array rather than ptr because const
                               // strlen needed for MPCM chunk
  ...
  sgh.write_chunk(INT_ID('M', 'P', 'C', 'M'), sMapCmd);
  ...
  //
  if (!qbAutosave) {
    sgh.write_chunk<int32_t>(INT_ID('T', 'I', 'M', 'E'), ::sv.time);

    sgh.write_chunk<int32_t>(INT_ID('T', 'I', 'M', 'R'), ::sv.timeResidual);

    CM_WritePortalState();
    SG_WriteServerConfigStrings();
  }
  ge->WriteLevel(
      qbAutosave); // always done now, but ent saver only does player if auto
  ...
  if (is_write_failed) {
    Com_Printf(GetString_FailedToOpenSaveGame(
        "current", qfalse)); // S_COLOR_RED "Failed to write savegame!\n");
    SG_WipeSavegame("current");
    sv_testsave->integer = iPrevTestSave;
    return qfalse;
  }

  ojk::SavedGame::rename("current", psPathlessBaseName);
...
qboolean SG_ReadSavegame(const char *psPathlessBaseName) {
  ...
  ojk::ScopeGuard scope_guard([&]() { ::sv_testsave->integer = 0; },
  ...
  // this check isn't really necessary, but it reminds me that these two strings
  // may actually be the same physical one.
  //
  if (psPathlessBaseName != sLastSaveFileLoaded) {
    ::Q_strncpyz(::sLastSaveFileLoaded, psPathlessBaseName,
                 sizeof(sLastSaveFileLoaded));
  }

  // Read in all the server data...
  //
  sgh.read_chunk(INT_ID('C', 'O', 'M', 'M'), sComment);

  ::Com_DPrintf("Reading: %s\n", sComment);

  sgh.read_chunk(INT_ID('C', 'M', 'T', 'M'));

#ifdef JK2_MODE
  ::SG_ReadScreenshot(true, nullptr);
#endif

  sgh.read_chunk(INT_ID('M', 'P', 'C', 'M'), sMapCmd);
  ...
  //
  if (!qbAutosave) {
    sgh.read_chunk<int32_t>(INT_ID('T', 'I', 'M', 'E'), ::sv.time);

    sgh.read_chunk<int32_t>(INT_ID('T', 'I', 'M', 'R'), ::sv.timeResidual);
