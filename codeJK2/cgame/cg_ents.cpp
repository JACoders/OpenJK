  ...
  if (cent->gent->ghoul2.size()) {
    if (s1->weapon == WP_SABER && cent->gent && cent->gent->owner &&
      ...
      if (cent->gent->classname &&
        ...
        if (cent->gent->owner->client) {
          if (cent->gent->owner->client->ps.saberLength > 0) {
            CG_AddSaberBlade(&cg_entities[cent->gent->owner->s.number],
                             ...
                             ent.renderfx, cent->gent->weaponModel,
            ...
            CG_CheckSaberInWater(&cg_entities[cent->gent->owner->s.number],
                                 &cg_entities[cent->gent->s.number],
                                 cent->gent->weaponModel, cent->lerpOrigin,
                                 cent->lerpAngles);
          }
        }
      } else { // thrown saber
        // light?  sound?
        if (cent->gent->owner->client &&
            cg_entities[cent->currentState.otherEntityNum]
                .currentState.saberActive) { // saber is in-flight and active,
                                             // play a sound on it
        ...
        if (cent->gent->owner->client) {
          if (cent->gent->owner->client->ps.saberLength >
  ...
  if (cent->gent && cent->gent->owner && cent->gent->owner->ghoul2.size()) {
    ...
    if (cent->gent->aimDebounceTime) { // done with dismemberment, just waiting
      ...
      {                       // add smoke to cap surf, spawn effect
        int newBolt = gi.G2API_AddBolt(&owner->ghoul2[owner->playerModel],
                                       cent->gent->target);
        if (newBolt != -1) {
          G_PlayEffect("blaster/smoke_bolton", owner->playerModel, newBolt,
                       owner->s.number);
        }
      }
      ...
      if (owner->weaponModel >= 0) { // the corpse hasn't dropped their weapon
...
void CG_MatrixEffect(centity_t *cent) {
  ...
  if (totalElapsedTime > cent->currentState.eventParm // MATRIX_EFFECT_TIME
      ...
      cg.missionStatusShow) { // time is up or this is a falling spin and they
                              // hit the ground or mission end screen is up
    cg.overrides.active &=
  ...
  if (!cent->currentState
           .weapon) { // go ahead and do all the slowdown and vert bob stuff
    // slowdown
  ...
  } else { // FIXME: if they're on the ground, stop spinning and stop timescale
