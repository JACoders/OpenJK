...
void CG_ItemPickup(int itemNum, qboolean bHadItem) {
  ...
  if (bg_itemlist[itemNum].classname && bg_itemlist[itemNum].classname[0]) {
    ...
    if (cgi_SP_GetStringTextString("INGAME_PICKUPLINE", text, sizeof(text))) {
      if (cgi_SP_GetStringTextString(
              va("INGAME_%s", bg_itemlist[itemNum].classname), data,
              sizeof(data))) {
        Com_Printf("%s %s\n", text, data);
      }
    }
  ...
  if (bg_itemlist[itemNum].giType == IT_WEAPON) {
    ...
    //
    // NOTE: automatically switching to any weapon you pick up is stupid and
    // annoying and we won't do it.
    //
    ...
    if (nNewWpn == WP_SABER) { // always switch to saber
      ...
      // safe switching
      if ((nNewWpn > nCurWpn) && !(nNewWpn == WP_DET_PACK) &&
          !(nNewWpn == WP_TRIP_MINE) && !(nNewWpn == WP_THERMAL) &&
...
static void CG_UseItem(centity_t *cent) {
  ...
  if (es->number == cg.snap->ps.clientNum) {
    if (!itemNum) {
      //			CG_CenterPrint( "No item to use", SCREEN_HEIGHT
      //* 0.30, BIGCHAR_WIDTH );
    } else {
...
void CG_EntityEvent(centity_t *cent, vec3_t position) {
  ...
  case EV_FOOTSTEP:
    ...
    if (cg_footsteps.integer) {
      if (cent->gent && cent->gent->s.number == 0 &&
  ...
  case EV_FALL_MEDIUM:
    ...
    if (g_entities[es->number].health <= 0) { // dead
      cgi_S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.landSound);
    } else if (g_entities[es->number].s.weapon == WP_SABER) { // jedi
      CG_TryPlayCustomSound(NULL, es->number, CHAN_BODY, "*land1.wav",
                            CS_BASIC);
    } else { // still alive
      CG_TryPlayCustomSound(NULL, es->number, CHAN_BODY, "*pain100.wav",
                            CS_BASIC);
    }
    if (clientNum == cg.predicted_player_state.clientNum) {
      // smooth landing z changes
  ...
  case EV_JUMP:
    DEBUGNAME("EV_JUMP");
    CG_TryPlayCustomSound(NULL, es->number, CHAN_AUTO, "*jump1.wav",
                          CS_BASIC); // CHAN_VOICE
    break;
  ...
  case EV_ROLL:
    ...
    CG_TryPlayCustomSound(NULL, es->number, CHAN_AUTO, "*jump1.wav",
                          CS_BASIC); // CHAN_VOICE
    cgi_S_StartSound(NULL, es->number, CHAN_BODY,
                     cgs.media.rollSound); // CHAN_AUTO
    // FIXME: need some sort of body impact on ground sound and maybe kick up
    // some dust?
    break;
  ...
  case EV_WATER_GURP2:
    ...
    CG_TryPlayCustomSound(NULL, es->number, CHAN_AUTO,
                          va("*gurp%d.wav", event - EV_WATER_GURP1 + 1),
                          CS_BASIC);
    break;
  ...
  case EV_ITEM_PICKUP:
    DEBUGNAME("EV_ITEM_PICKUP");
    {
      ...
      cgi_S_StartSound(NULL, es->number, CHAN_AUTO,
  ...
  case EV_CHANGE_WEAPON:
    ...
    if (es->weapon == WP_SABER) {
      /*
      if ( !cent->gent || !cent->gent->client ||
      (cent->currentState.saberInFlight == qfalse &&
      cent->currentState.saberActive == qtrue) )
      {
              cgi_S_StartSound (NULL, es->number, CHAN_AUTO,
    ...
    // FIXME: if it happens that you don't want the saber to play the switch
    // sounds, feel free to modify this bit.
  ...
  case EV_DISRUPTOR_SNIPER_SHOT:
    ...
    FX_DisruptorAltShot(cent->currentState.origin2, cent->lerpOrigin,
                        cent->gent->alt_fire);
    break;
  ...
  case EV_POWERUP_BATTLESUIT:
    ...
    // cgi_S_StartSound (NULL, es->number, CHAN_ITEM,
    // cgs.media.invulnoProtectSound );
    break;
  ...
  case EV_BATTERIES_CHARGED:
    ...
    cgi_S_StartSound((float *)vec3_origin, es->number, CHAN_AUTO,
      ...
      if (cent->gent->owner->client) {
        if (disintSound1 && disintSound2) { // play an extra sound
          ...
          if (npc_class != CLASS_ATST && npc_class != CLASS_GONK &&
            ...
            cgi_S_StartSound(NULL, cent->gent->owner->s.number, CHAN_VOICE,
                             disintSound1);
          }
          ...
          //)
          //	{
          //		cgi_S_StartSound ( NULL, cent->gent->s.number, CHAN_VOICE,
          //disintSound3 );
          //	}
          else {
            cgi_S_StartSound(NULL, cent->gent->s.number, CHAN_AUTO,
                             disintSound2);
          }
        }
        cent->gent->owner->s.powerups |= (1 << disintPW);
        cent->gent->owner->client->ps.powerups[disintPW] =
            cg.time + disintLength;

        // Things that are being disintegrated should probably not be solid...
        if (makeNotSolid &&
            cent->gent->owner->client->playerTeam != TEAM_NEUTRAL) {
  ...
  case EV_MISSILE_HIT:
    ...
    CG_MissileHitPlayer(cent, es->weapon, position, cent->gent->pos1,
                        cent->gent->alt_fire);
    break;
  ...
  case EV_GENERAL_SOUND:
    ...
    if (cgs.sound_precache[es->eventParm]) {
      cgi_S_StartSound(NULL, es->number, CHAN_AUTO,
                       cgs.sound_precache[es->eventParm]);
    } else {
  ...
  case EV_GLOBAL_SOUND: // play from the player's head so it never diminishes
    ...
    if (cgs.sound_precache[es->eventParm]) {
      ...
      s = CG_ConfigString(CS_SOUNDS + es->eventParm);
      CG_TryPlayCustomSound(NULL, cg.snap->ps.clientNum, CHAN_AUTO, s,
                            CS_BASIC);
    }
    break;
  ...
  case EV_DRUGGED:
    ...
    if (cent->gent && cent->gent->owner && cent->gent->owner->s.number == 0) {
      // Only allow setting up the wonky vision on the player..do it for 10
      // seconds...must be synchronized with calcs done in cg_view.  Just search
      // for cg.wonkyTime to find 'em.
  ...
  case EV_DEATH3:
    DEBUGNAME("EV_DEATHx");
    /*
    if ( cent->gent && cent->gent->NPC && (cent->gent->NPC->aiFlags &
    NPCAI_DIE_ON_IMPACT) )
    {
            return;
    }
    */
    CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE,
                          va("*death%i.wav", event - EV_DEATH1 + 1), CS_BASIC);
    break;
  ...
  case EV_PLAY_EFFECT:
    ...
    if (es->boltInfo != 0) {
      ...
      if (es->otherEntityNum) {
        theFxScheduler.PlayEffect(s, cent->lerpOrigin, axis, -1,
                                  es->otherEntityNum);
      } else {
  ...
  case EV_TARGET_BEAM_DRAW:
    ...
    if (cent->gent) {
      ...
      if (s && s[0]) {
        ...
        CG_DrawTargetBeam(cent->lerpOrigin, cent->gent->s.origin2,
                          cent->gent->pos1, s, s2);
      }
      /*			else
                              {
                                      int gack = 0; // this is bad if it get's
         here
                              }
      */
    }
  ...
  case EV_DEBUG_LINE:
    ...
    CG_TestLine(position, es->origin2, es->time, (unsigned int)(es->time2),
                es->weapon);
    break;
