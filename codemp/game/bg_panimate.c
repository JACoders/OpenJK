...
qboolean BG_InDeathAnim(int anim) { // Purposely does not cover stumbledeath and falldeath...
	switch (anim) {
...
qboolean BG_InKnockDownOnGround(playerState_t *ps) {
	...
	case BOTH_FORCE_GETUP_B6:
		if (BG_AnimLength(0, (animNumber_t)ps->legsAnim) - ps->legsTimer < 500) { // at beginning of getup anim
			return qtrue;
		}
		break;
	case BOTH_GETUP_BROLL_B:
...
#ifdef _CGAME // none of this is actually needed serverside. Could just be moved to cgame code but it's here since it used to tie in a lot with the anim loading
			  // stuff.
...
void ParseAnimationEvtBlock(const char *aeb_filename, animevent_t *animEvents, animation_t *animations, int *i, const char **text_p) {
	...
	while (1) {
		...
		if (eventType == AEV_NONE ||
			eventType == (animEventType_t)-1) { // Unrecognized ANIM EVENT TYOE, or we're skipping this line, keep going till you get a good one
			// Com_Printf(S_COLOR_YELLOW"WARNING: Unknown token %s in animEvent file %s\n", token, aeb_filename );
			continue;
		}
...
void PM_SetAnimFinal(int setAnimParts, int anim, int setAnimFlags) { BG_SetAnimFinal(pm->ps, pm->animations, setAnimParts, anim, setAnimFlags); }
...
void PM_SetAnim(int setAnimParts, int anim, int setAnimFlags) { BG_SetAnim(pm->ps, pm->animations, setAnimParts, anim, setAnimFlags); }
