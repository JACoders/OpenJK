...
saberMoveData_t saberMoveData[LS_MOVE_MAX] = {
	...
	{"L2TR Trans", BOTH_T1__L_TR, Q_L, Q_TR, AFLAG_ACTIVE, 100, BLK_NO, LS_R_BL2TR, LS_A_TR2BL,
	 150},																							  //# Fast arc left to top right			(use: BOTH_T1_TR__L)
	...
	{"L2BL Trans", BOTH_T1__L_BL, Q_L, Q_BL, AFLAG_ACTIVE, 100, BLK_NO, LS_R_TR2BL, LS_A_BL2TR,
	 150},																								//# Fast arc left to bottom left			(use: BOTH_T1_BL__L)
...
saberMoveName_t transitionMove[Q_NUM_QUADS][Q_NUM_QUADS] = {
	...
	},
	{
		...
		LS_T1_BR__R, LS_T1_BR_TR, LS_T1_BR_T_, LS_T1_BR_TL, LS_T1_BR__L, LS_T1_BR_BL,
		LS_NONE // No transitions to bottom, and no anims start there, so shouldn't need any
	}};
...
int PM_PowerLevelForSaberAnim(playerState_t *ps, int saberNum) {
	...
	if ((anim >= BOTH_P1_S1_T_ && anim <= BOTH_P1_S1_BR) || (anim >= BOTH_P6_S6_T_ && anim <= BOTH_P6_S6_BR) ||
	...
	if ((anim >= BOTH_K1_S1_T_ && anim <= BOTH_K1_S1_BR) || (anim >= BOTH_K6_S6_T_ && anim <= BOTH_K6_S6_BR) ||
		(anim >= BOTH_K7_S7_T_ && anim <= BOTH_K7_S7_BR)) { // knockaways
		return FORCE_LEVEL_3;
	}
	if ((anim >= BOTH_V1_BR_S1 && anim <= BOTH_V1_B__S1) || (anim >= BOTH_V6_BR_S6 && anim <= BOTH_V6_B__S6) ||
		(anim >= BOTH_V7_BR_S7 && anim <= BOTH_V7_B__S7)) { // knocked-away attacks
		return FORCE_LEVEL_1;
	}
	if ((anim >= BOTH_H1_S1_T_ && anim <= BOTH_H1_S1_BR) || (anim >= BOTH_H6_S6_T_ && anim <= BOTH_H6_S6_BR) ||
	...
	case BOTH_JUMPATTACK6:
		...
		*/
		if ((ps->torsoAnimTimer >= 1450 && animTimeElapsed >= 400) || (ps->torsoAnimTimer >= 400 && animTimeElapsed >= 1100)) { // pretty much sideways
			return FORCE_LEVEL_3;
		}
		return FORCE_LEVEL_0;
		break;
...
qboolean PM_InAnimForSaberMove(int anim, int saberMove) {
	...
	if (animLevel <= 0) { // NOTE: this will always return false for the ready poses and putaway/draw...
		return qfalse;
	}
...
qboolean PM_SaberKataDone(int curmove = LS_NONE, int newmove = LS_NONE) {
	if (pm->ps->forceRageRecoveryTime > level.time) { // rage recovery, only 1 swing at a time (tired)
		if (pm->ps->saberAttackChainCount > 0) {	  // swung once
			return qtrue;
		} else { // allow one attack
			return qfalse;
		}
	} else if ((pm->ps->forcePowersActive & (1 << FP_RAGE))) { // infinite chaining when raged
		return qfalse;
	...
	if (pm->ps->saberAnimLevel == SS_DESANN || pm->ps->saberAnimLevel == SS_TAVION) { // desann and tavion can link up as many attacks as they want
		return qfalse;
	}
...
qboolean PM_CheckEnemyInBack(float backCheckDist) {
	...
	if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && !g_saberAutoAim->integer && pm->cmd.forwardmove >= 0) { // don't auto-backstab
		return qfalse;
	}
	...
	if (trace.fraction < 1.0f && trace.entityNum < ENTITYNUM_WORLD) {
		...
		if (traceEnt && traceEnt->health > 0 && traceEnt->client && traceEnt->client->playerTeam == pm->gent->client->enemyTeam &&
...
saberMoveName_t PM_PickBackStab(void) {
	...
	} else if (pm->ps->saberAnimLevel == FORCE_LEVEL_2 || pm->ps->saberAnimLevel == SS_DUAL) { // using medium attacks or dual sabers
		if (pm->ps->pm_flags & PMF_DUCKED) {
			return LS_A_BACK_CR;
		} else {
			return LS_A_BACK;
...
saberMoveName_t PM_CheckStabDown(void) {
	...
	if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())) {						   // player
		...
		{																					   // sorry must be on ground (or have just jumped)
			if (level.time - pm->ps->lastOnGround <= 50 && (pm->ps->pm_flags & PMF_JUMPING)) { // just jumped, it's okay
			} else {
				return LS_NONE;
			}
		}
		...
		*/
		pm->ps->velocity[2] = 0;
		pm->cmd.upmove = 0;
	} else if ((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer())) {				// NPC
		if (pm->ps->groundEntityNum == ENTITYNUM_NONE)											// in air
		{																						// sorry must be on ground (or have just jumped)
			if (level.time - pm->ps->lastOnGround <= 250 && (pm->ps->pm_flags & PMF_JUMPING)) { // just jumped, it's okay
			} else {
				return LS_NONE;
			}
		}
		...
		if (Q_irand(0, RANK_CAPTAIN) > pm->gent->NPC->rank) { // lower ranks do this less often
			return LS_NONE;
		}
	}
	...
	if ( //(pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())
		...
		if (pm->gent->enemy->s.number >= MAX_CLIENTS || !G_ControlledByPlayer(pm->gent->enemy)) { // don't get up while I'm doing this
...
saberMoveName_t PM_AttackForEnemyPos(qboolean allowFB, qboolean allowStabDown) {
	...
	if (pm->gent->enemy->client) {
		...
		if (pm->gent->enemy->bmodel && VectorCompare(vec3_origin, pm->gent->enemy->currentOrigin)) { // a brush model without an origin brush
			vec3_t size;
			VectorSubtract(pm->gent->enemy->absmax, pm->gent->enemy->absmin, size);
			VectorMA(pm->gent->enemy->absmin, 0.5, size, enemy_org);
		} else {
	...
	if (dot > 0) {			 // enemy is in front
		...
		if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && dot > 0.65f && enemyDist <= 64 && pm->gent->enemy->client &&
			(enemyZDiff <= 20 || PM_InKnockDownOnGround(&pm->gent->enemy->client->ps) ||
			 PM_CrouchAnim(pm->gent->enemy->client->ps.legsAnim))) { // swing down at them
			return LS_A_T2B;
		}
		if (allowFB) { // directly in front anim allowed
			if (!(pm->ps->saber[0].saberFlags & SFL_NO_BACK_ATTACK) &&
				(!pm->ps->dualSabers || !(pm->ps->saber[1].saberFlags & SFL_NO_BACK_ATTACK))) { // okay to do backstabs with this saber
				if (enemyDist > 200 || pm->gent->enemy->health <= 0) {							// hmm, look in back for an enemy
					if (pm->ps->clientNum && !PM_ControlledByPlayer()) {						// player should never do this automatically
						if (pm->ps->groundEntityNum != ENTITYNUM_NONE) {						// only when on ground
							if (pm->gent && pm->gent->client && pm->gent->NPC && pm->gent->NPC->rank >= RANK_LT_JG &&
								Q_irand(0, pm->gent->NPC->rank) > RANK_ENSIGN) { // only fencers and higher can do this, higher rank does it more
			...
			if ((pm->ps->clientNum && !PM_ControlledByPlayer()) ||
				((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && cg.renderingThirdPerson && !cg.zoomMode)) { // NPC or player not in 1st person
				if (PM_CheckFlipOverAttackMove(qtrue)) {																	 // enemy must be close and in front
		...
		if (!(pm->ps->saber[0].saberFlags & SFL_NO_BACK_ATTACK) &&
			...
			if (pm->ps->groundEntityNum != ENTITYNUM_NONE) {								// only when on ground
				if (!pm->gent->enemy->client ||
					pm->gent->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE) { // enemy not a client or is a client and on ground
					if (dot < -0.75f && enemyDist < 128 &&
						(pm->ps->saberAnimLevel == SS_FAST || pm->ps->saberAnimLevel == SS_STAFF ||
						 (pm->gent->client && (pm->gent->client->NPC_class == CLASS_TAVION || pm->gent->client->NPC_class == CLASS_ALORA) &&
						  Q_irand(0, 2)))) {										   // fast back-stab
						if (!(pm->ps->pm_flags & PMF_DUCKED) && pm->cmd.upmove >= 0) { // can't do it while ducked?
							if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) ||
								(pm->gent->NPC && pm->gent->NPC->rank >= RANK_LT_JG)) { // only fencers and above can do this
								autoMove = LS_A_BACKSTAB;
							}
						}
					} else if (pm->ps->saberAnimLevel != SS_FAST && pm->ps->saberAnimLevel != SS_STAFF) { // higher level back spin-attacks
						if ((pm->ps->clientNum && !PM_ControlledByPlayer()) ||
...
qboolean PM_InSecondaryStyle(void) {
	if (pm->ps->saber[0].numBlades > 1 && pm->ps->saber[0].singleBladeStyle && (pm->ps->saber[0].stylesForbidden & (1 << pm->ps->saber[0].singleBladeStyle)) &&
...
saberMoveName_t PM_SaberLungeAttackMove(qboolean fallbackToNormalLunge) {
	...
	// do normal checks
	if (pm->gent->client->NPC_class == CLASS_ALORA && !Q_irand(0, 3)) { // alora NPC
...
qboolean PM_CheckLungeAttackMove(void) {
	...
	if (pm->ps->saber[0].lungeAtkMove == LS_NONE) {
		if (pm->ps->dualSabers) {
			if (pm->ps->saber[1].lungeAtkMove == LS_NONE || pm->ps->saber[1].lungeAtkMove == LS_INVALID) {
				return qfalse;
			}
		} else {
			return qfalse;
	...
	if (pm->ps->dualSabers) {
		if (pm->ps->saber[1].lungeAtkMove == LS_NONE) {
			if (pm->ps->saber[0].lungeAtkMove == LS_NONE || pm->ps->saber[0].lungeAtkMove == LS_INVALID) {
				return qfalse;
			}
		}
	}
	...
	if (pm->ps->saberAnimLevel == SS_FAST									// fast
		...
		|| pm->ps->saberAnimLevel == SS_DESANN || pm->ps->dualSabers) {		// alt+back+attack using fast, dual or staff attacks
		if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) { // NPC
			if (pm->cmd.upmove < 0 || (pm->ps->pm_flags & PMF_DUCKED)) {	// ducking
				if (pm->ps->legsAnim == BOTH_STAND2 || pm->ps->legsAnim == BOTH_SABERFAST_STANCE || pm->ps->legsAnim == BOTH_SABERSLOW_STANCE ||
					pm->ps->legsAnim == BOTH_SABERSTAFF_STANCE || pm->ps->legsAnim == BOTH_SABERDUAL_STANCE ||
					(level.time - pm->ps->lastStationary) <= 500) { // standing or just stopped standing
					if (pm->gent && pm->gent->NPC					// NPC
						&& pm->gent->NPC->rank >= RANK_LT_JG		// high rank
						&& (pm->gent->NPC->rank == RANK_LT_JG ||
							Q_irand(-3, pm->gent->NPC->rank) >= RANK_LT_JG) // Q_irand( 0, pm->gent->NPC->rank ) >= RANK_LT_JG )
						&& !Q_irand(0, 3 - g_spskill->integer)) {			// only fencer and higher can do this
...
qboolean PM_CheckJumpForwardAttackMove(void) {
	...
	if (pm->ps->saber[0].jumpAtkFwdMove == LS_NONE) {
		if (pm->ps->dualSabers) {
			if (pm->ps->saber[1].jumpAtkFwdMove == LS_NONE || pm->ps->saber[1].jumpAtkFwdMove == LS_INVALID) {
				return qfalse;
			}
		} else {
			return qfalse;
	...
	if (pm->ps->dualSabers) {
		if (pm->ps->saber[1].jumpAtkFwdMove == LS_NONE) {
			if (pm->ps->saber[0].jumpAtkFwdMove == LS_NONE || pm->ps->saber[0].jumpAtkFwdMove == LS_INVALID) {
				return qfalse;
			}
		}
	}
	...
	if (pm->cmd.forwardmove > 0									  // going forward
		...
		if (pm->ps->saberAnimLevel == SS_DUAL || pm->ps->saberAnimLevel == SS_STAFF) { // dual and staff
				 ...
				 || pm->ps->saberAnimLevel == SS_DESANN) // desann
		{
			if ( //&& !PM_InKnockDown( pm->ps )
				!pm->ps->dualSabers
				//&& (pm->ps->legsAnim == BOTH_STAND2||pm->ps->legsAnim == BOTH_SABERFAST_STANCE||pm->ps->legsAnim ==
				//BOTH_SABERSLOW_STANCE||level.time-pm->ps->lastStationary<=500)//standing or just started moving
				...
				if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) { // NPC
					if (pm->cmd.upmove > 0 || (pm->ps->pm_flags & PMF_JUMPING))		// NPC jumping
					{
						if (pm->gent && pm->gent->NPC &&
							(pm->gent->NPC->rank == RANK_CREWMAN || pm->gent->NPC->rank >= RANK_LT)) { // only acrobat or boss and higher can do this
							if (pm->ps->legsAnim == BOTH_STAND2 || pm->ps->legsAnim == BOTH_SABERFAST_STANCE || pm->ps->legsAnim == BOTH_SABERSLOW_STANCE ||
								level.time - pm->ps->lastStationary <= 250) { // standing or just started moving
...
saberMoveName_t PM_SaberFlipOverAttackMove(void) {
	...
	if (pm->gent && pm->gent->enemy) { // go higher for taller enemies
		...
		if (zDiff <= 0 && pm->ps->velocity[2] < 200) { // if we're on same level, don't let me jump so low, I clip into the ground
	...
	if ((pm->ps->saberAnimLevel == SS_MEDIUM					  // medium
		...
		if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) { // NPC
			...
			{																// flip over-forward down-attack
				if ((pm->gent->NPC && (pm->gent->NPC->rank == RANK_CREWMAN || pm->gent->NPC->rank >= RANK_LT) &&
			...
			{
				if (!pm->cmd.rightmove) {
					if (pm->ps->legsAnim == BOTH_JUMP1 || pm->ps->legsAnim == BOTH_FORCEJUMP1 || pm->ps->legsAnim == BOTH_INAIR1 ||
						pm->ps->legsAnim == BOTH_FORCEINAIR1) { // in a non-flip forward jump
						tryMove = qtrue;
					}
				}
			}
			...
			if (!checkEnemy) { // based just on command input
				...
				if (pm->gent->enemy) // have an enemy
				{
					...
					if (pm->gent->enemy->health > 0 && pm->ps->forceRageRecoveryTime < pm->cmd.serverTime // not in a force Rage recovery period
						&& pm->gent->enemy->maxs[2] > 12 && (!pm->gent->enemy->client || !PM_InKnockDownOnGround(&pm->gent->enemy->client->ps)) &&
...
qboolean PM_CheckBackflipAttackMove(void) {
	...
	if (pm->ps->saber[0].jumpAtkBackMove == LS_NONE) {
		if (pm->ps->dualSabers) {
			if (pm->ps->saber[1].jumpAtkBackMove == LS_NONE || pm->ps->saber[1].jumpAtkBackMove == LS_INVALID) {
				return qfalse;
			}
		} else {
			return qfalse;
	...
	if (pm->ps->dualSabers) {
		if (pm->ps->saber[1].jumpAtkBackMove == LS_NONE) {
			if (pm->ps->saber[0].jumpAtkBackMove == LS_NONE || pm->ps->saber[0].jumpAtkBackMove == LS_INVALID) {
				return qfalse;
			}
		}
	}
		...
		&& (pm->ps->groundEntityNum != ENTITYNUM_NONE || level.time - pm->ps->lastOnGround <= 250)) // on ground or just jumped (if not player)
	{
		...
		{																			// jumping backwards and using staff
			...
			{																		// not already attacking
				if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) { // NPC
					if (pm->gent && pm->gent->NPC &&
						(pm->gent->NPC->rank == RANK_CREWMAN || pm->gent->NPC->rank >= RANK_LT)) { // acrobat or boss and higher can do this
						return qtrue;
					}
				} else { // player
					return qtrue;
...
saberMoveName_t PM_CheckDualSpinProtect(void) {
	...
	if (pm->ps->saberMove == LS_READY // ready
		...
		&& G_EnoughPowerForSpecialMove(pm->ps->forcePower, SABER_ALT_ATTACK_POWER,
									   qtrue) // pm->ps->forcePower >= SABER_ALT_ATTACK_POWER//DUAL_SPIN_PROTECT_POWER//force push 3
...
saberMoveName_t PM_CheckPullAttack(void) {
	...
	if (pm->ps->dualSabers && (pm->ps->saber[1].saberFlags & SFL_NO_PULL_ATTACK)) {
		return LS_NONE;
	}
		...
		if (g_saberNewControlScheme->integer || g_crosshairEntNum < ENTITYNUM_WORLD) // in old control scheme, there has to be someone there
		{
			...
			if (g_crosshairEntNum < ENTITYNUM_WORLD && pm->gent && pm->gent->client) {
				...
				if (targEnt->client &&
					...
					&& !PM_InOnGroundAnim(&targEnt->client->ps) && !PM_LockedAnim(targEnt->client->ps.legsAnim) &&
					!PM_SuperBreakLoseAnim(targEnt->client->ps.legsAnim) && !PM_SuperBreakWinAnim(targEnt->client->ps.legsAnim) &&
					targEnt->client->ps.saberLockTime <= 0 && WP_ForceThrowable(targEnt, targEnt, pm->gent, qtrue, 1.0f, 0.0f, NULL)) {
					if (!g_saberNewControlScheme->integer) { // in old control scheme, make sure they're close or far enough away for the move we'll be doing
						float targDist = Distance(targEnt->currentOrigin, pm->ps->origin);
						if (pullAttackMove == LS_PULL_ATTACK_STAB) { // must be closer than 512
							if (targDist > 384.0f) {
								return LS_NONE;
							}
					...
					targEnt->client->ps.legsAnimTimer = targEnt->client->ps.torsoAnimTimer =
						PM_AnimLength(pm->gent->client->clientInfo.animFileIndex, (animNumber_t)saberMoveData[pullAttackMove].animToUse);
					// set pullAttackTime
...
saberMoveName_t PM_SaberAttackForMovement(int forwardmove, int rightmove, int curmove) {
	...
	if (pm->ps->saber[0].jumpAtkRightMove != LS_INVALID) {
		...
		} else if (pm->ps->dualSabers && pm->ps->saber[1].jumpAtkRightMove > LS_NONE) { // would be cancelling it, but check the second saber, too
	...
	} else if (pm->ps->dualSabers && pm->ps->saber[1].jumpAtkRightMove != LS_INVALID) { // first saber not overridden, check second
		overrideJumpRightAttackMove = (saberMoveName_t)pm->ps->saber[1].jumpAtkRightMove;
	}
	...
	if (pm->ps->saber[0].jumpAtkLeftMove != LS_INVALID) {
		...
		} else if (pm->ps->dualSabers && pm->ps->saber[1].jumpAtkLeftMove > LS_NONE) { // would be cancelling it, but check the second saber, too
			overrideJumpLeftAttackMove = (saberMoveName_t)pm->ps->saber[1].jumpAtkLeftMove;
		} else { // nope, just cancel it
			overrideJumpLeftAttackMove = LS_NONE;
		}
	} else if (pm->ps->dualSabers && pm->ps->saber[1].jumpAtkLeftMove != LS_INVALID) { // first saber not overridden, check second
		overrideJumpLeftAttackMove = (saberMoveName_t)pm->ps->saber[1].jumpAtkLeftMove;
	}
	if (rightmove > 0) { // moving right
		if (!noSpecials && overrideJumpRightAttackMove != LS_NONE &&
			...
			&& (((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) && pm->cmd.upmove > 0) // jumping NPC
				|| ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
					G_TryingCartwheel(pm->gent, &pm->cmd) /*(pm->cmd.buttons&BUTTON_FORCE_FOCUS)*/))) // focus-holding player
		{																							  // cartwheel right
			...
			if (overrideJumpRightAttackMove != LS_INVALID) { // overridden with another move
				...
				if (!(pm->ps->saber[0].saberFlags & SFL_NO_CARTWHEELS) &&
					(!pm->ps->dualSabers || !(pm->ps->saber[1].saberFlags & SFL_NO_CARTWHEELS))) { // okay to do cartwheels with this saber
					/*
		...
		} else if (pm->ps->legsAnim != BOTH_CARTWHEEL_RIGHT && pm->ps->legsAnim != BOTH_ARIAL_RIGHT) { // not in a cartwheel/arial
			if (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())							   // PLAYER ONLY
			{																						   // player
		...
		} else if (pm->ps->legsAnim != BOTH_CARTWHEEL_LEFT && pm->ps->legsAnim != BOTH_ARIAL_LEFT) { // not in a left cartwheel/arial
			if (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())							 // PLAYER ONLY
			{																						 // player
				if (G_TryingSpecial(pm->gent, &pm->cmd) /*(pm->cmd.buttons&BUTTON_FORCE_FOCUS)*/)	 // Holding focus
				{																					 // if no special worked, do nothing
		...
		if (forwardmove > 0) { // forward= T2B slash
			...
			if (((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && cg.renderingThirdPerson &&
				 !cg.zoomMode)) // player in third person, not zoomed in
			{					// player in thirdperson, not zoomed in
				// flip-over attack logic
				if (!noSpecials && PM_CheckFlipOverAttackMove(qfalse)) { // flip over-forward down-attack
					return PM_SaberFlipOverAttackMove();
				}
				// lunge attack logic
			...
			if (pm->gent && pm->gent->enemy && pm->gent->enemy->client) {		  // I have an active enemy
				if (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) { // a player who is running at an enemy
			...
			{												// player lunge attack logic
				...
				{																						   // alt+back+attack using fast, dual or staff attacks
					PM_SaberLungeAttackMove(qfalse);
					   ...
					   || ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && cg.renderingThirdPerson &&
			...
			{							  // NPC or player in third person, not zoomed
				...
				if (!(pm->ps->saber[0].saberFlags & SFL_NO_BACK_ATTACK) &&
					(!pm->ps->dualSabers || !(pm->ps->saber[1].saberFlags & SFL_NO_BACK_ATTACK))) { // okay to do backstabs with this saber
					if (pm->ps->groundEntityNum != ENTITYNUM_NONE) {								// only when on ground
						if (pm->gent && pm->gent->enemy) { // FIXME: or just trace for a valid enemy standing behind me?  And no enemy in front?
							vec3_t enemyDir, faceFwd, facingAngles = {0, pm->ps->viewangles[YAW], 0};
							AngleVectors(facingAngles, faceFwd, NULL, NULL);
							VectorSubtract(pm->gent->enemy->currentOrigin, pm->ps->origin, enemyDir);
							float dot = DotProduct(enemyDir, faceFwd);
							if (dot < 0) {																							 // enemy is behind me
								...
								} else if (pm->ps->saberAnimLevel != SS_FAST && pm->ps->saberAnimLevel != SS_STAFF) { // medium and higher attacks
									if ((pm->ps->pm_flags & PMF_DUCKED) || pm->cmd.upmove < 0) {
										return LS_A_BACK_CR;
									} else {
										return LS_A_BACK;
								...
								if (((pm->ps->saberAnimLevel == FORCE_LEVEL_1 || pm->ps->saberAnimLevel == SS_STAFF ||
								...
								{								  // my enemy is pretty far in front of me and I'm using fast attacks
									if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) ||
										(pm->gent && pm->gent->client && pm->gent->NPC && pm->gent->NPC->rank >= RANK_LT_JG &&
										 Q_irand(0, pm->gent->NPC->rank) > RANK_ENSIGN)) { // only fencers and higher can do this, higher rank does it more
										if (PM_CheckEnemyInBack(128)) {
											return PM_PickBackStab();
										}
									}
								} else if (((pm->ps->saberAnimLevel >= FORCE_LEVEL_2 || pm->gent->client->NPC_class == CLASS_DESANN) && enemyDistSq > 40000) ||
							...
							if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && pm->gent && pm->gent->client) { // only player
								if (PM_CheckEnemyInBack(128)) {
									return PM_PickBackStab();
								}
							}
			...
			if (PM_SaberInBounce(curmove)) { // bounces should go to their default attack if you don't specify a direction but are attacking
				if (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) // PLAYER ONLY
				{																// player
					if (G_TryingSpecial(pm->gent, &pm->cmd))					//(pm->cmd.buttons&BUTTON_FORCE_FOCUS) )//Holding focus
					{															// if no special worked, do nothing
				...
				if (pm->ps->clientNum && !PM_ControlledByPlayer() && Q_irand(0, 3)) { // use NPC random
			...
			} else if (PM_SaberInKnockaway(curmove)) { // bounces should go to their default attack if you don't specify a direction but are attacking
				if (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) // PLAYER ONLY
				{																// player
					if (G_TryingSpecial(pm->gent, &pm->cmd))					//(pm->cmd.buttons&BUTTON_FORCE_FOCUS) )//Holding focus
					{															// if no special worked, do nothing
				...
				if (pm->ps->clientNum && !PM_ControlledByPlayer() && Q_irand(0, 3)) { // use NPC random
					...
					if (pm->ps->saberAnimLevel == SS_FAST ||
						pm->ps->saberAnimLevel == SS_TAVION) { // player is in fast attacks, so come right back down from the same spot
						newmove = PM_AttackMoveForQuad(saberMoveData[curmove].endQuad);
					} else { // use a transition to wrap to another attack from a different dir
						newmove = saberMoveData[curmove].chain_attack;
					}
				}
				if (PM_SaberKataDone(curmove, newmove)) {
			...
			} else if (curmove == LS_READY || curmove == LS_A_FLIP_STAB || curmove == LS_A_FLIP_SLASH ||
					   (curmove >= LS_PARRY_UP && curmove <= LS_REFLECT_LL)) { // Not moving at all, not too busy to attack
				...
				if (pm->ps->clientNum || g_saberAutoAim->integer) { // auto-aim
					if (pm->gent && pm->gent->enemy) {				// based on enemy position, pick a proper attack
						saberMoveName_t autoMove = PM_AttackForEnemyPos(qtrue, (qboolean)(pm->ps->clientNum >= MAX_CLIENTS));
						if (autoMove != LS_INVALID) {
							return autoMove;
						}
					} else if (fabs(pm->ps->viewangles[0]) >
							   30) { // looking far up or far down uses the top to bottom attack, presuming you want a vertical attack
						return LS_A_T2B;
					}
...
saberMoveName_t PM_SaberAnimTransitionMove(saberMoveName_t curmove, saberMoveName_t newmove) {
	...
	if (curmove == LS_READY) { // just standing there
		...
		case LS_A_TR2BL:
		case LS_A_T2B:
			if (newmove == curmove) { // FIXME: need a spin or something or go to next level, but for now, just play the return
...
int PM_LegsAnimForFrame(gentity_t *ent, int legsFrame) {
	...
	for (int animation = 0; animation < BOTH_CIN_1; animation++) // first anim after last legs
	{
		...
		if (animations[animation].firstFrame > legsFrame) { // This anim starts after this frame
			continue;
		}

		if (animations[animation].firstFrame + animations[animation].numFrames < legsFrame) { // This anim ends before this frame
			continue;
		}
...
int PM_ValidateAnimRange(const int startFrame, const int endFrame,
						 const float animSpeed) { // given a startframe and endframe, see if that lines up with any known animation
	...
	for (int anim = 0; anim < MAX_ANIMATIONS; anim++) {
		if (animSpeed < 0) { // playing backwards
			...
			if (animations[anim].firstFrame == startFrame) {								// This anim starts on this frame
				if (animations[anim].firstFrame + animations[anim].numFrames == endFrame) { // This anim ends on this frame
...
int PM_TorsoAnimForFrame(gentity_t *ent, int torsoFrame) {
	...
	for (int animation = 0; animation < LEGS_TURN1; animation++) // first legs only anim
	{
		...
		if (animations[animation].firstFrame > torsoFrame) { // This anim starts after this frame
			continue;
		}

		if (animations[animation].firstFrame + animations[animation].numFrames < torsoFrame) { // This anim ends before this frame
			continue;
		}
		// else, must be in this anim!
		return animation;
...
qboolean PM_FinishedCurrentLegsAnim(gentity_t *self) {
	...
	gi.G2API_GetBoneAnimIndex(&self->ghoul2[self->playerModel], self->rootBone, (cg.time ? cg.time : level.time), &currentFrame, &junk, &junk, &junk,
							  &animSpeed, NULL);
...
void PM_SetLegsAnimTimer(gentity_t *ent, int *legsAnimTimer, int time) {
	*legsAnimTimer = time;
	...
	if (*legsAnimTimer < 0 && time != -1) { // Cap timer to 0 if was counting down, but let it be -1 if that was intentional
		*legsAnimTimer = 0;
	}

	if (!*legsAnimTimer && ent && Q3_TaskIDPending(ent, TID_ANIM_LOWER)) { // Waiting for legsAnimTimer to complete, and it just got set to zero
		if (!Q3_TaskIDPending(ent, TID_ANIM_BOTH)) {					   // Not waiting for top
			...
			if (!Q3_TaskIDPending(ent, TID_ANIM_UPPER)) { // top is done and we're done
...
void PM_SetTorsoAnimTimer(gentity_t *ent, int *torsoAnimTimer, int time) {
	*torsoAnimTimer = time;
	...
	if (*torsoAnimTimer < 0 && time != -1) { // Cap timer to 0 if was counting down, but let it be -1 if that was intentional
		*torsoAnimTimer = 0;
	}

	if (!*torsoAnimTimer && ent && Q3_TaskIDPending(ent, TID_ANIM_UPPER)) { // Waiting for torsoAnimTimer to complete, and it just got set to zero
		if (!Q3_TaskIDPending(ent, TID_ANIM_BOTH)) {						// Not waiting for bottom
			...
			if (!Q3_TaskIDPending(ent, TID_ANIM_LOWER)) { // lower is done and we're done
				Q3_TaskIDComplete(ent, TID_ANIM_BOTH);
			}
		}
	}
...
void PM_SaberStartTransAnim(int saberAnimLevel, int anim, float *animSpeed, gentity_t *gent) {
	...
	if (gent && gent->client && gent->client->ps.stats[STAT_WEAPONS] & (1 << WP_SCEPTER) && gent->client->ps.dualSabers && saberAnimLevel == SS_DUAL &&
		gent->weaponModel[1]) { // using a scepter and dual style, slow down anims
		if (anim >= BOTH_A1_T__B_ && anim <= BOTH_H7_S7_BR) {
			*animSpeed *= 0.75;
		}
	}
	if (gent && gent->client && gent->client->ps.forceRageRecoveryTime > level.time) { // rage recovery
		if (anim >= BOTH_A1_T__B_ && anim <= BOTH_H1_S1_BR) {						   // animate slower
			*animSpeed *= 0.75;
		}
	} else if (gent && gent->NPC && gent->NPC->rank == RANK_CIVILIAN) { // grunt reborn
		if (anim >= BOTH_A1_T__B_ && anim <= BOTH_R1_TR_S1) {			// his fast attacks are slower
		...
		if (gent->client->ps.saber[0].type == SABER_LANCE ||
			gent->client->ps.saber[0].type == SABER_TRIDENT) {	  // FIXME: hack for now - these use the fast anims, but slowed down.  Should have own style
			if (anim >= BOTH_A1_T__B_ && anim <= BOTH_R1_TR_S1) { // his fast attacks are slower
				if (!PM_SpinningSaberAnim(anim)) {
					*animSpeed *= 0.75;
				}
	...
	if ((anim >= BOTH_T1_BR__R && anim <= BOTH_T1_BL_TL) || (anim >= BOTH_T3_BR__R && anim <= BOTH_T3_BL_TL) ||
		(anim >= BOTH_T5_BR__R && anim <= BOTH_T5_BL_TL)) {
		if (saberAnimLevel == FORCE_LEVEL_1 || saberAnimLevel == FORCE_LEVEL_5) { // FIXME: should not be necc for FORCE_LEVEL_1's
			*animSpeed *= 1.5;
...
float PM_GetTimeScaleMod(gentity_t *gent) {
	if (g_timescale->value) {
		if (!MatrixMode && gent->client->ps.legsAnim != BOTH_FORCELONGLEAP_START && gent->client->ps.legsAnim != BOTH_FORCELONGLEAP_ATTACK &&
					 ...
					 int blendTime) // default blendTime=350
{
	...
	if (g_noFootSlide->integer && animFootMove &&
		...
		&& gent->client->NPC_class != CLASS_HOWLER && gent->client->NPC_class != CLASS_WAMPA && gent->client->NPC_class != CLASS_GONK &&
		gent->client->NPC_class != CLASS_HOWLER && gent->client->NPC_class != CLASS_MOUSE && gent->client->NPC_class != CLASS_PROBE &&
	...
	bool bodyAnimating =
		!!gi.G2API_GetBoneAnimIndex(&gent->ghoul2[gent->playerModel], bodyBone, actualTime, &bodyCurrent, &bodyStart, &bodyEnd, &bodyFlags, &bodySpeed, NULL);
	...
	bool torsAnimating =
		!!gi.G2API_GetBoneAnimIndex(&gent->ghoul2[gent->playerModel], torsBone, actualTime, &torsCurrent, &torsStart, &torsEnd, &torsFlags, &torsSpeed, NULL);
	...
	if ((cg_debugAnim.integer == 3) ||												// 3 = do everyone
		...
		if (bodyPlay || torsPlay) {
			...
			// Print It!
			//-----------
			Com_Printf("[%10d] ent[%3d-%18s] %s anim[%3d] - %s\n", actualTime, gent->s.number, entName, location, anim, animTable[anim].name);
		}
	}
#endif
	...
	if (torsPlay) {
		*torsoAnim = anim;
		...
		gi.G2API_SetBoneAnimIndex(&gent->ghoul2[gent->playerModel], torsBone, animStart, animEnd,
								  (torsOnAnimNow && !animRestart) ? (animFlags & ~BONE_ANIM_BLEND) : (animFlags), animSpeed, actualTime, animCurrent,
								  blendTime);

		if (gent->motionBone != -1) {
			gi.G2API_SetBoneAnimIndex(&gent->ghoul2[gent->playerModel], gent->motionBone, animStart, animEnd,
									  (torsOnAnimNow && !animRestart) ? (animFlags & ~BONE_ANIM_BLEND) : (animFlags), animSpeed, actualTime, animCurrent,
									  blendTime);
		}
	...
	if (bodyPlay) {
		*legsAnim = anim;
		...
		gi.G2API_SetAnimIndex(&gent->ghoul2[gent->playerModel], curAnim.glaIndex);
		gi.G2API_SetBoneAnimIndex(&gent->ghoul2[gent->playerModel], bodyBone, animStart, animEnd,
								  (bodyOnAnimNow && !animRestart) ? (animFlags & ~BONE_ANIM_BLEND) : (animFlags), animSpeed, actualTime, animCurrent,
								  blendTime);
...
void PM_SetAnim(pmove_t *pm, int setAnimParts, int anim, int setAnimFlags,
				int blendTime) { // FIXME : once torsoAnim and legsAnim are in the same structure for NPC and Players
	// rename PM_SetAnimFinal to PM_SetAnim and have both NPC and Players call PM_SetAnim

	if (pm->ps->pm_type >= PM_DEAD) { // FIXME: sometimes we'll want to set anims when your dead... twitches, impacts, etc.
		return;
	}
	...
	if (!pm->gent || pm->gent->health > 0) {													  // don't lock anims if the guy is dead
		if (pm->ps->torsoAnimTimer && PM_LockedAnim(pm->ps->torsoAnim) && !PM_LockedAnim(anim)) { // nothing can override these special anims
			setAnimParts &= ~SETANIM_TORSO;
		}

		if (pm->ps->legsAnimTimer && PM_LockedAnim(pm->ps->legsAnim) && !PM_LockedAnim(anim)) { // nothing can override these special anims
	...
	PM_SetAnimFinal(&pm->ps->torsoAnim, &pm->ps->legsAnim, setAnimParts, anim, setAnimFlags, &pm->ps->torsoAnimTimer, &pm->ps->legsAnimTimer,
...
bool TorsoAgainstWindTest(gentity_t *ent) {
	if (ent &&																			 // valid ent
		...
		(ent->s.number < MAX_CLIENTS || G_ControlledByPlayer(ent)) && gi.WE_GetWindGusting(ent->currentOrigin) && gi.WE_IsOutside(ent->currentOrigin)) {
...
void PM_TorsoAnimLightsaber() {
	...
	// WEAPON_READY
	// *********************************************************
	if (pm->ps->forcePowersActive & (1 << FP_GRIP) && pm->ps->forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1) { // holding an enemy aloft with force-grip
		return;
	}

	if (pm->ps->forcePowersActive & (1 << FP_LIGHTNING) && pm->ps->forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_1) { // lightning
		return;
	}
	...
	if (pm->ps->saber[0].blade[0].active && pm->ps->saber[0].blade[0].length < 3 && !(pm->ps->saberEventFlags & SEF_HITWALL) &&
	...
	if (pm->ps->weaponTime > 0) {												  // weapon is already busy.
		if (pm->ps->torsoAnim == BOTH_TOSS1 || pm->ps->torsoAnim == BOTH_TOSS2) { // in toss
	...
	if (pm->ps->weaponstate == WEAPON_READY || pm->ps->weaponstate == WEAPON_CHARGING || pm->ps->weaponstate == WEAPON_CHARGING_ALT) { // ready
		if (pm->ps->weapon == WP_SABER && (pm->ps->SaberLength())) {																   // saber is on
			...
			if (pm->ps->saberMove > LS_READY && pm->ps->saberMove < LS_MOVE_MAX) {
				...
				if (PM_JumpingAnim(pm->ps->legsAnim) || PM_LandingAnim(pm->ps->legsAnim) || PM_InCartwheel(pm->ps->legsAnim) ||
					PM_FlippingAnim(pm->ps->legsAnim)) {
					PM_SetAnim(pm, SETANIM_TORSO, pm->ps->legsAnim, SETANIM_FLAG_NORMAL);
				} else {
					if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && pm->ps->torsoAnim == BOTH_BUTTON_HOLD) { // using something
						...
						if ((PM_RunningAnim(pm->ps->legsAnim) || pm->ps->legsAnim == BOTH_WALK_STAFF || pm->ps->legsAnim == BOTH_WALK_DUAL ||
							 pm->ps->legsAnim == BOTH_WALKBACK_STAFF || pm->ps->legsAnim == BOTH_WALKBACK_DUAL) &&
							pm->ps->saberBlockingTime < cg.time) { // running w/1-handed weapon uses full-body anim
							PM_SetAnim(pm, SETANIM_TORSO, pm->ps->legsAnim, SETANIM_FLAG_NORMAL);
						} else {
	...
	// *********************************************************
		...
		if (TorsoAgainstWindTest(pm->gent)) {
			...
			pm->ps->saberMove = LS_READY;
		} else if (pm->ps->legsAnim == BOTH_STAND1IDLE1 || pm->ps->legsAnim == BOTH_STAND2IDLE1 || pm->ps->legsAnim == BOTH_STAND2IDLE2 ||
				   pm->ps->legsAnim == BOTH_STAND3IDLE1 || pm->ps->legsAnim == BOTH_STAND5IDLE1) {
			PM_SetAnim(pm, SETANIM_TORSO, pm->ps->legsAnim, SETANIM_FLAG_NORMAL);
			pm->ps->saberMove = LS_READY;
...
void PM_TorsoAnimation(void) { // FIXME: Write a much smarter and more appropriate anim picking routine logic...
	//	int	oldAnim;
	...
	if ((pm->ps->forcePowersActive & (1 << FP_DRAIN)) && pm->ps->forceDrainEntityNum < ENTITYNUM_WORLD) { // draining
	...
	/*	else if ( pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_VEHICLE && pm->ps->clientNum < MAX_CLIENTS &&
	   (m_pVehicleInfo[((CVehicleNPC *)pm->gent->NPC)->m_iVehicleTypeID].numHands == 2 || g_speederControlScheme->value == 2) )
		{//can't look around
		...
		if (pm->ps->SaberLength() && !pm->ps->saberInFlight) {
			...
			if (PM_SaberInBrokenParry(pm->ps->saberMove) || pm->ps->saberBlocked == BLOCKED_PARRY_BROKEN ||
				...
				if (pm->ps->saberInFlight && saberInAir &&
					...
					if (PM_InSlopeAnim(pm->ps->legsAnim)) { // HMM... this probably breaks the saber putaway and select anims
	...
	if (PM_ForceAnim(pm->ps->torsoAnim) && pm->ps->torsoAnimTimer > 0) { // in a force anim, don't do a stand anim
		return;
	}
	...
	if (pm->ps->weapon == WP_NONE) {
		...
		return;
	} else if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && cg.zoomTime > cg.time - 5000) { // if we used binoculars recently, aim weapon
		weaponBusy = qtrue;
		pm->ps->weaponstate = WEAPON_IDLE;
	} else if (pm->ps->pm_flags & PMF_DUCKED) { // ducking is considered on alert... plus looks stupid to have arms hanging down when crouched
		weaponBusy = qtrue;
	}

	if (pm->ps->weapon == WP_NONE || pm->ps->weaponstate == WEAPON_READY || pm->ps->weaponstate == WEAPON_CHARGING ||
		...
		} else if (pm->ps->legsAnim == BOTH_CROUCH1IDLE && pm->ps->clientNum != 0) // player falls through
		{
			...
			*/
			...
			if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && pm->ps->torsoAnim == BOTH_BUTTON_HOLD) { // using something
			...
			} else if (pm->gent != NULL && (pm->gent->s.number < MAX_CLIENTS || G_ControlledByPlayer(pm->gent)) && pm->ps->weaponstate != WEAPON_CHARGING &&
					   pm->ps->weaponstate != WEAPON_CHARGING_ALT) {							   // PLayer- temp hack for weapon frame
				if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_RANCOR) { // ignore
				} else if (pm->ps->weapon == WP_MELEE) {										   // hehe
					PM_SetAnim(pm, SETANIM_TORSO, BOTH_STAND6, SETANIM_FLAG_NORMAL);
				} else {
					PM_SetAnim(pm, SETANIM_TORSO, BOTH_STAND1, SETANIM_FLAG_NORMAL);
				}
			} else if (PM_InSpecialJump(pm->ps->legsAnim)) { // use legs anim
				...
				case WP_BRYAR_PISTOL:
					...
					} else if (PM_RunningAnim(pm->ps->legsAnim) || PM_WalkingAnim(pm->ps->legsAnim) || PM_JumpingAnim(pm->ps->legsAnim) ||
							   PM_SwimmingAnim(pm->ps->legsAnim)) { // running w/1-handed weapon uses full-body anim
						PM_SetAnim(pm, SETANIM_TORSO, pm->ps->legsAnim, SETANIM_FLAG_NORMAL);
					} else {
						PM_SetAnim(pm, SETANIM_TORSO, TORSO_WEAPONREADY2, SETANIM_FLAG_NORMAL);
					}
				...
				case WP_NONE:
					...
					break;
				case WP_MELEE:
					if (PM_RunningAnim(pm->ps->legsAnim) || PM_WalkingAnim(pm->ps->legsAnim) || PM_JumpingAnim(pm->ps->legsAnim) ||
						PM_SwimmingAnim(pm->ps->legsAnim)) { // running w/1-handed weapon uses full-body anim
						PM_SetAnim(pm, SETANIM_TORSO, pm->ps->legsAnim, SETANIM_FLAG_NORMAL);
					} else {
						if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_RANCOR) { // ignore
				...
				case WP_TUSKEN_RIFLE:
					if ((pm->ps->weaponstate != WEAPON_FIRING && pm->ps->weaponstate != WEAPON_CHARGING && pm->ps->weaponstate != WEAPON_CHARGING_ALT) ||
						PM_RunningAnim(pm->ps->legsAnim) || PM_WalkingAnim(pm->ps->legsAnim) || PM_JumpingAnim(pm->ps->legsAnim) ||
						PM_SwimmingAnim(pm->ps->legsAnim)) { // running sniper weapon uses normal ready
						if (pm->ps->clientNum) {
							PM_SetAnim(pm, SETANIM_TORSO, TORSO_WEAPONREADY3, SETANIM_FLAG_NORMAL);
						} else {
							PM_SetAnim(pm, SETANIM_TORSO, TORSO_WEAPONREADY3, SETANIM_FLAG_NORMAL);
						...
						if (pm->ps->clientNum) {
							PM_SetAnim(pm, SETANIM_TORSO, TORSO_WEAPONREADY4,
									   SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD); // TORSO_WEAPONREADY4//SETANIM_FLAG_RESTART|
						} else {
				...
				case WP_THERMAL:
					if (pm->ps->weaponstate != WEAPON_FIRING && pm->ps->weaponstate != WEAPON_CHARGING && pm->ps->weaponstate != WEAPON_CHARGING_ALT &&
						...
						if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
							(pm->ps->weaponstate == WEAPON_CHARGING || pm->ps->weaponstate == WEAPON_CHARGING_ALT)) { // player pulling back to throw
							if (PM_StandingAnim(pm->ps->legsAnim)) {
								PM_SetAnim(pm, SETANIM_LEGS, BOTH_THERMAL_READY, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
							} else if (pm->ps->legsAnim == BOTH_THERMAL_READY) { // sigh... hold it so pm_footsteps doesn't override
		...
		if (pm->ps->legsAnim == BOTH_GUARD_LOOKAROUND1) {
			...
			PM_SetAnim(pm, SETANIM_TORSO, BOTH_SWIMFORWARD, SETANIM_FLAG_NORMAL);
														 ...
														 // FIXME: or just use whatever's currently playing?
														 // PM_SetAnim( pm, SETANIM_TORSO, pm->ps->legsAnim, SETANIM_FLAG_NORMAL );
		} else if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && pm->ps->torsoAnim == BOTH_BUTTON_HOLD) { // using something
			if (!pm->ps->useTime) {																							// stopped holding it, release
				PM_SetAnim(pm, SETANIM_TORSO, BOTH_BUTTON_RELEASE, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			} // else still holding, leave it as it is
		} else {
			if (!weaponBusy && pm->ps->weapon != WP_BOWCASTER && pm->ps->weapon != WP_REPEATER && pm->ps->weapon != WP_FLECHETTE &&
				pm->ps->weapon != WP_ROCKET_LAUNCHER && pm->ps->weapon != WP_CONCUSSION &&
				(PM_RunningAnim(pm->ps->legsAnim) || (PM_WalkingAnim(pm->ps->legsAnim) && (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())) ||
				 ...
				 PM_SwimmingAnim(
					 pm->ps->legsAnim))) { // running w/1-handed or light 2-handed weapon uses full-body anim if you're not using the weapon right now
				PM_SetAnim(pm, SETANIM_TORSO, pm->ps->legsAnim, SETANIM_FLAG_NORMAL);
			} else {
				switch (pm->ps->weapon) {
				// ********************************************************
...
qboolean PM_InOnGroundAnim(playerState_t *ps) {
	...
	case BOTH_FORCE_GETUP_B6:
		if (ps->legsAnimTimer > PM_AnimLength(g_entities[ps->clientNum].client->clientInfo.animFileIndex, (animNumber_t)ps->legsAnim) -
...
qboolean PM_InDeathAnim(void) { // Purposely does not cover stumbledeath and falldeath...
	switch (pm->ps->legsAnim) {
...
qboolean PM_StandingAnim(int anim) { // NOTE: does not check idles or special (cinematic) stands
	switch (anim) {
	case BOTH_STAND1:
	case BOTH_STAND2:
	case BOTH_STAND3:
...
qboolean PM_ForceUsingSaberAnim(int anim) { // saber/acrobatic anims that should prevent you from recharging force power while you're in them...
	switch (anim) {
	case BOTH_JUMPFLIPSLASHDOWN1:
	case BOTH_JUMPFLIPSTABDOWN:
	case BOTH_FORCELEAP2_T__B_:
...
qboolean G_HasKnockdownAnims(gentity_t *ent) {
	if (PM_HasAnimation(ent, BOTH_KNOCKDOWN1) && PM_HasAnimation(ent, BOTH_KNOCKDOWN2) && PM_HasAnimation(ent, BOTH_KNOCKDOWN3) &&
		PM_HasAnimation(ent, BOTH_KNOCKDOWN4) && PM_HasAnimation(ent, BOTH_KNOCKDOWN5)) {
		return qtrue;
	}
	return qfalse;
}
...
qboolean PM_LockedAnim(int anim) { // anims that can *NEVER* be overridden, regardless
	switch (anim) {
	case BOTH_KYLE_PA_1:
	case BOTH_KYLE_PA_2:
	case BOTH_KYLE_PA_3:
