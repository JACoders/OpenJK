...
static void NPC_CheckAttacker(gentity_t *other, int mod) {
  ...
  if (NPCS.NPC->client->ps.weapon == WP_SABER) { // I'm a jedi
...
void NPC_SetPainEvent(gentity_t *self) {
  if (!self->NPC || !(self->NPC->aiFlags & NPCAI_DIE_ON_IMPACT)) {
    ...
    if (!trap->ICARUS_TaskIDPending((sharedEntity_t *)self, TID_CHAN_VOICE) &&
      ...
      G_AddEvent(self, EV_PAIN,
                 floor((float)self->health /
                       self->client->ps.stats[STAT_MAX_HEALTH] * 100.0f));
...
float NPC_GetPainChance(gentity_t *self, int damage) {
  ...
  pain_chance =
      (float)(self->client->ps.stats[STAT_MAX_HEALTH] - self->health) /
          (self->client->ps.stats[STAT_MAX_HEALTH] * 2.0f) +
...
void NPC_ChoosePainAnimation(gentity_t *self, gentity_t *other, vec3_t point,
                             int damage, int mod, int hitLoc, int voiceEvent) {
  int pain_anim = -1;
  float pain_chance;

  // If we've already taken pain, then don't take it again
  if (level.time < self->painDebounceTime &&
      /*mod != MOD_ELECTROCUTE &&*/ mod !=
          MOD_MELEE) // rwwFIXMEFIXME: MOD_ELECTROCUTE
  { // FIXME: if hit while recoving from losing a saber lock, we should still
    // play a pain anim?
    return;
  }
  ...
  if (self->s.weapon == WP_THERMAL &&
      self->client->ps.weaponTime >
          0) { // don't interrupt thermal throwing anim
    return;
  } else if (self->client->NPC_class == CLASS_GALAKMECH) {
    if (hitLoc == HL_GENERIC1) { // hit the antenna!
      pain_chance = 1.0f;
      //	self->s.powerups |= ( 1 << PW_SHOCKED );
      //	self->client->ps.powerups[PW_SHOCKED] = level.time + Q_irand(
      //500, 2500 ); rwwFIXMEFIXME: support for this
    }
  ...
  } else if (self->client && self->client->playerTeam == NPCTEAM_PLAYER &&
             other &&
             !other->s.number) { // ally shot by player always complains
    pain_chance = 1.1f;
  } else {
    if (other && (other->s.weapon == WP_SABER ||
                  /*mod == MOD_ELECTROCUTE ||*/ mod ==
                      MOD_CRUSH /*FIXME:MOD_FORCE_GRIP*/)) {
  ...
  if (Q_flrand(0.0f, 1.0f) < pain_chance) {
    ...
    if (self->client->ps.fd.forceGripBeingGripped <
        level.time) { // not being force-gripped or force-drained
      if (/*G_CheckForStrongAttackMomentum( self ) //rwwFIXMEFIXME: Is this
             needed?
             ||*/
        ...
        */
        ...
        if (pain_anim == -1) {
          pain_anim =
              BG_PickAnim(self->localAnimIndex, BOTH_PAIN1, BOTH_PAIN18);
        }
...
void NPC_Pain(gentity_t *self, gentity_t *attacker, int damage) {
      ...
      {                                         // dammit, stop that!
        if (self->NPC->charmedTime) {           // mindtricked
          return;
                   ...
                   3 + ((2 - g_npcspskill.integer) * 2)) { // not mad enough yet
          // Com_Printf( "chck: %d < %d\n", self->NPC->ffireCount,
          ...
          // self->svFlags |= SVF_LOCKEDENEMY; //rwwFIXMEFIXME: proper support
          // for these flags.
  ...
  if (NPCS.NPCInfo->ignorePain == qfalse) {
    ...
    if (NPCS.NPC->enemy != other &&
        NPCS.NPC != other) { // not already mad at them
...
void NPC_Touch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (self->message && self->health <= 0) { // I am dead and carrying a key
    ...
    if (other && other->client &&
        other->s.number < MAX_CLIENTS) { // player touched me
      /*
      char *text;
      qboolean	keyTaken;
      //give him my key
      ...
      {//a goodie key
              if ( (keyTaken = INV_GoodieKeyGive( other )) == qtrue )
              {
                      ...
                      G_AddEvent( other, EV_ITEM_PICKUP, (FindItemForInventory(
      INV_GOODIE_KEY )-bg_itemlist) );
              }
              else
              {
      ...
      {//a named security key
              if ( (keyTaken = INV_SecurityKeyGive( player, self->message )) ==
      qtrue )
              {
                      text = "cp @SP_INGAME_TOOK_IMPERIAL_SECURITY_KEY";
                      G_AddEvent( other, EV_ITEM_PICKUP, (FindItemForInventory(
      INV_SECURITY_KEY )-bg_itemlist) );
              }
              else
              {
      ...
      */
      ...
      {//remove my key
              ...
              //FIXME: need some event to pass to cgame for
      sound/graphic/message?
      }
      //FIXME: temp message
  ...
  if (other->client) { // FIXME:  if pushing against another bot, both
    ...
    if (/*!(self->svFlags&SVF_LOCKEDENEMY) &&
      ...
      if (self->client->enemyTeam) { // See if we bumped into an enemy
        if (other->client->playerTeam ==
          ...
          if (NPCS.NPCInfo->behaviorState != BS_HUNT_AND_KILL &&
              !NPCS.NPCInfo->tempBehavior) { // MCG - Begin: checking specific
                                             // BS mode here, this is bad, a HACK
            // FIXME: not medics?
            if (NPCS.NPC->enemy != other) { // not already mad at them
              G_SetEnemy(NPCS.NPC, other);
            }
    ...
    */
    ...
    if (other->health > 0) {
      ...
      if (0) // rwwFIXMEFIXME: Can probably just check if num < MAX_CLIENTS for
             // non-npc enemy stuff
      {
        NPCS.NPCInfo->touchedByPlayer = other;
      }
...
void NPC_Respond(gentity_t *self, int userNum) {
  ...
  */
  ...
  case CLASS_R5D2: // droid
    G_Sound(
        self, CHAN_AUTO,
        G_SoundIndex(va("sound/chars/r5d2/misc/r5talk%d.wav", Q_irand(1, 4))));
    break;
...
void NPC_UseResponse(gentity_t *self, gentity_t *user, qboolean useWhenDone) {
  ...
  if (user->client && self->client->playerTeam != user->client->playerTeam &&
      self->client->playerTeam !=
          NPCTEAM_NEUTRAL) { // only those on the same team react
    if (useWhenDone) {
      G_ActivateBehavior(self, BSET_USE);
    }
    return;
...
void NPC_Use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  if (self->client && self->NPC) {
    // If this is a vehicle, let the other guy board it. Added 12/14/02 by
    // AReis.
    ...
    if (activator && activator->s.number >= 0 &&
      ...
      //			Add_Batteries( activator,
      //&self->client->ps.batteryCharge ); rwwFIXMEFIXME: support for this?
    }
    ...
    /*
       ...
       && !trap->VoiceVolume[self->s.number] &&
    ...
    */
    ...
    //			NPC_TakePatient( activator );
    //		}
    else if (activator && !self->enemy && activator->s.number >= 0 &&
             activator->s.number < MAX_CLIENTS &&
             !(self->NPC->scriptFlags & SCF_NO_RESPONSE))
