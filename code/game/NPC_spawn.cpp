...
painFunc_t NPC_PainFunc(gentity_t *ent) {
  ...
  if (ent->client->ps.weapon == WP_SABER) {
    ...
    // team no longer indicates race/species, use NPC_class to determine
    // different npc types
    /*
...
touchFunc_t NPC_TouchFunc(gentity_t *ent) { return touchF_NPC_Touch; }
...
void NPC_SetMiscDefaultData(gentity_t *ent) {
  if (ent->spawnflags &
      SFB_CINEMATIC) { // if a cinematic guy, default us to wait bState
    ent->NPC->behaviorState = BS_CINEMATIC;
  }
  if (ent->client->NPC_class == CLASS_RANCOR) {
    if (Q_stricmp("mutant_rancor", ent->NPC_type) == 0) {
      ent->spawnflags |=
          1; // just so I know it's a mutant rancor as opposed to a normal one
    ...
    ent->clipmask =
        CONTENTS_SOLID | CONTENTS_MONSTERCLIP; // it can go through others
    ent->contents = 0;                         // can't be hit?
    ent->takedamage = qfalse;                  // can't be killed
    ent->flags |= FL_NO_KNOCKBACK;
    SandCreature_ClearTimers(ent);
  } else if (ent->client->NPC_class ==
             CLASS_BOBAFETT) { // set some stuff, precache
    ...
    ent->NPC->scriptFlags |=
        (SCF_NAV_CAN_FLY | SCF_FLY_WITH_JET | SCF_NAV_CAN_JUMP);
    NPC->flags |= FL_UNDYING; // Can't Kill Boba
  } else if (ent->client->NPC_class ==
             CLASS_ROCKETTROOPER) { // set some stuff, precache
    ent->client->ps.forcePowersKnown |= (1 << FP_LEVITATION);
    ent->client->ps.forcePowerLevel[FP_LEVITATION] = FORCE_LEVEL_3;
    ent->client->ps.forcePower = 100;
    ent->NPC->scriptFlags |= (SCF_NAV_CAN_FLY | SCF_FLY_WITH_JET |
                              SCF_NAV_CAN_JUMP); // no groups, no combat points!
    if (Q_stricmp("rockettrooper2Officer", ent->NPC_type) ==
        0) { // start in the air, use spotlight
  ...
  if (ent->spawnflags & 4096) {
    ent->NPC->scriptFlags |=
        SCF_NO_GROUPS; // don't use combat points or group AI
  }
  ...
  if (Q_stricmpn(ent->NPC_type, "hazardtrooper", 13) == 0) { // hazard trooper
    ent->NPC->scriptFlags |=
        SCF_NO_GROUPS; // don't use combat points or group AI
    ent->flags |= (FL_SHIELDED |
                   FL_NO_KNOCKBACK); // low-level shots bounce off, no knockback
  }
  ...
  if (!Q_stricmp("emperor", ent->NPC_type) ||
      ...
      !Q_stricmp("cultist_lightning",
                 ent->NPC_type)) {          // FIXME: extern this into NPC.cfg?
  ...
  if (ent->client->ps.saber[0].type != SABER_NONE &&
      (!(ent->NPC->aiFlags & NPCAI_MATCHPLAYERWEAPON) ||
       !ent->weaponModel[0])) { // if I'm equipped with a saber, initialize it
                                // (them)
    ent->client->ps.SaberDeactivate();
    ent->client->ps.SetSaberLength(0);
    WP_SaberInitBladeData(ent);
    if (ent->client->ps.weapon ==
        WP_SABER) { // this is our current weapon, add the models now
  ...
  if (ent->client->ps.forcePowersKnown != 0) {
    ...
    if (ent->client->ps.forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0) {
      ent->NPC->scriptFlags |=
          SCF_NAV_CAN_JUMP; // anyone who has any force jump can jump
    }
  }
  ...
  if (ent->client->NPC_class == CLASS_TUSKEN) {
    if (g_spskill->integer > 1) { // on hard, tusken raiders are faster than you
      ent->NPC->stats.runSpeed = 280;
      ent->NPC->stats.walkSpeed = 65;
    }
  }
  //***I'm not sure whether I should leave this as a TEAM_ switch, I think
  //NPC_class may be more appropriate - dmv
  switch (ent->client->playerTeam) {
  case TEAM_PLAYER:
    // ent->flags |= FL_NO_KNOCKBACK;
    if (ent->client->NPC_class == CLASS_SEEKER) {
      ...
      return;
               ...
               ent->client->NPC_class == CLASS_KYLE ||
               ent->client->NPC_class == CLASS_LUKE) { // good jedi
      ...
      if (ent->client->ps.weapon != WP_NONE &&
          ...
          && (!(ent->NPC->aiFlags & NPCAI_MATCHPLAYERWEAPON) ||
              !ent->weaponModel[0])) // they do this themselves
      {
        G_CreateG2AttachedWeaponModel(
            ent, weaponData[ent->client->ps.weapon].weaponMdl, ent->handRBolt,
            0);
      }
      ...
      case WP_THERMAL:
      case WP_BLASTER:
        // FIXME: health in NPCs.cfg, and not all blaster users are
        // stormtroopers ent->health = 25;
        // FIXME: not necc. a ST
        ST_ClearTimers(ent);
        if (ent->NPC->rank >= RANK_LT ||
            ent->client->ps.weapon ==
                WP_THERMAL) { // officers, grenade-throwers use alt-fire
                              // ent->health = 50;
                              // ent->NPC->scriptFlags |= SCF_ALT_FIRE;
        }
        break;
  ...
  case TEAM_NEUTRAL:
    ...
    if (Q_stricmp(ent->NPC_type, "gonk") == 0) {
      ...
      // Not even sure if we want to give different levels of batteries?  ...Or
      // even that these are the values we'd want to use.
    ...
    if (ent->client->NPC_class == CLASS_TAVION ||
      ...
      case WP_BLASTER_PISTOL:
        ...
        { // dual blaster pistols, so add the left-hand one, too
          G_CreateG2AttachedWeaponModel(
              ent, weaponData[ent->client->ps.weapon].weaponMdl, ent->handLBolt,
              1);
        }
        break;
      case WP_DISRUPTOR:
        // Sniper
        // ent->NPC->scriptFlags |= SCF_ALT_FIRE;//FIXME: use primary fire
        // sometimes?  Up close?  Different class of NPC?
        break;
      case WP_BOWCASTER:
        NPCInfo->scriptFlags |= SCF_PILOT;
        break;
      ...
      case WP_BLASTER:
        ...
        ST_ClearTimers(ent);
        if (ent->NPC->rank >=
            RANK_COMMANDER) { // commanders use alt-fire
                              // ent->NPC->scriptFlags |= SCF_ALT_FIRE;
        }
  ...
  default:
    ...
    {
      G_CreateG2AttachedWeaponModel(
          ent, weaponData[ent->client->ps.weapon].weaponMdl, ent->handRBolt, 0);
    }
    break;
  }

  if (ent->client->NPC_class == CLASS_ATST ||
      ent->client->NPC_class == CLASS_MARK1) // chris/steve/kevin requested that
                                             // the mark1 be shielded also
  {
  ...
  if (ent->client->ps.stats[STAT_WEAPONS] & (1 << WP_SCEPTER)) {
    if (!ent->weaponModel[1]) { // we have the scepter, so put it in our left
                                // hand if we don't already have a second weapon
      G_CreateG2AttachedWeaponModel(ent, weaponData[WP_SCEPTER].weaponMdl,
                                    ent->handLBolt, 1);
    }
    ent->genericBolt1 =
        gi.G2API_AddBolt(&ent->ghoul2[ent->weaponModel[1]], "*flash");
  }

  if (ent->client->ps.saber[0].type == SABER_SITH_SWORD) {
    ent->genericBolt1 =
        gi.G2API_AddBolt(&ent->ghoul2[ent->weaponModel[0]], "*flash");
    G_PlayEffect(G_EffectIndex("scepter/sword.efx"), ent->weaponModel[0],
                 ent->genericBolt1, ent->s.number, ent->currentOrigin, qtrue,
                 qtrue);
...
int NPC_WeaponsForTeam(team_t team, int spawnflags, const char *NPC_type) {
  //*** not sure how to handle this, should I pass in class instead of team and
  //go from there? - dmv
  switch (team) {
    // no longer exists
  ...
  case TEAM_ENEMY:
    ...
    if ((Q_stricmp("interrogator", NPC_type) == 0) ||
        (Q_stricmp("sentry", NPC_type) == 0) ||
        (Q_stricmpn("protocol", NPC_type, 8) == 0)) {
      return WP_NONE;
    }
    ...
    if ((Q_stricmp("probe", NPC_type) == 0) ||
        (Q_stricmp("seeker", NPC_type) == 0)) {
  ...
  case TEAM_PLAYER:
    ...
    if (Q_stricmpn("jedi", NPC_type, 4) == 0 ||
...
void NPC_SetWeapons(gentity_t *ent) {
  ...
  int weapons = NPC_WeaponsForTeam(ent->client->playerTeam, ent->spawnflags,
                                   ent->NPC_type);
  ...
  for (int curWeap = WP_SABER; curWeap < WP_NUM_WEAPONS; curWeap++) {
    if ((weapons & (1 << curWeap))) {
      ...
      RegisterItem(
          FindItemForWeapon((weapon_t)(curWeap))); // precache the weapon
      ent->NPC->currentAmmo =
          ent->client->ps.ammo[weaponData[curWeap].ammoIndex] =
              100; // FIXME: max ammo
      ...
      if (curWeap == WP_MELEE) {
        if (bestWeap == WP_NONE) { // We'll only consider giving Melee since we
                                   // haven't found anything better yet.
          bestWeap = curWeap;
        }
      } else if (curWeap > bestWeap || bestWeap == WP_MELEE) {
        // This will never override saber as best weap.  Also will override
        // WP_MELEE if something better comes later in the list
        bestWeap = curWeap;
      }
    }
  }
...
NPC_SpawnEffect
  ...
  NOTE:  Make sure any effects called here have their models, tga's and sounds
precached in CG_RegisterNPCEffects in cg_player.cpp
-------------------------
*/
...
void NPC_Begin(gentity_t *ent) {
  ...
  if (!(ent->spawnflags & SFB_NOTSOLID)) { // No NPCs should telefrag
    if (Q_stricmp(ent->NPC_type, "nullDriver") ==
        0) { // FIXME: should be a better way to check this
    } else if (SpotWouldTelefrag(ent,
                                 TEAM_FREE)) //(team_t)(ent->client->playerTeam)
    {
      if (ent->wait < 0) { // remove yourself
        Quake3Game()->DebugPrint(
        ...
        Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                                 "NPC %s at (%5.0f %5.0f %5.0f) couldn't "
                                 "spawn, waiting %4.2f secs to try again\n",
                                 ent->targetname, ent->s.origin[0],
  ...
  } else if (ent->NPC->stats.health) // Was health supplied in NPC.cfg?
  {
    ...
    if (ent->client->NPC_class != CLASS_REBORN &&
      ...
      if (!Q_stricmp("tavion_sith_sword", ent->NPC_type) ||
          ...
          !Q_stricmp("alora_boss",
                     ent->NPC_type)) { // bosses are a bit different
        ent->NPC->stats.health = ceil(
            (float)ent->NPC->stats.health * 0.75f +
            ((float)ent->NPC->stats.health / 4.0f *
             g_spskill->value)); // 75% on easy, 100% on medium, 125% on hard
      } else {
        ent->NPC->stats.health +=
            ent->NPC->stats.health / 4 *
            g_spskill->integer; // 100% on easy, 125% on medium, 150% on hard
      }
    }

    ent->max_health = client->pers.maxHealth =
        client->ps.stats[STAT_MAX_HEALTH] = ent->NPC->stats.health;
  } else {
    ent->max_health = client->pers.maxHealth =
  ...
  if (!Q_stricmp("rodian", ent->NPC_type)) { // sniper
    ...
    case 2:
      ...
      break;
             ...
             ent->client->NPC_class == CLASS_IMPWORKER ||
             !Q_stricmp("rodian2", ent->NPC_type)) { // tweak yawspeed for these
                                                     // NPCs based on difficulty
    switch (g_spskill->integer) {
    case 0:
  ...
  */
  ...
  // CRITICAL NOTE! This was already done somewhere else and it was overwriting
  // the previous value!!!
  ...
  //		ent->contents =
  //CONTENTS_PLAYERCLIP|CONTENTS_MONSTERCLIP|CONTENTS_ITEM;//contents_corspe to
  //make them show up in ID and use traces 		ent->clipmask = MASK_NPCSOLID; 	} else
  ...
  //	ent->s.skinNum = ent - g_entities - 1;	// used as index to get custom
  //models
  ...
  // ICARUS include
  // NOTE! Does this need to be called here? It seems to be getting called
  // twice... Once before the level starts, then once again when the entity is
  // spawned!
  ...
  if (G_ActivateBehavior(ent, BSET_SPAWN)) {
    if (ent->m_iIcarusID !=
            IIcarusInterface::ICARUS_INVALID /*ent->taskManager*/
        && !stop_icarus) {
  ...
  if (ent->client->playerTeam == TEAM_ENEMY ||
      ent->client->playerTeam == TEAM_FREE) { // valid enemy spawned
    if (!(ent->spawnflags & SFB_CINEMATIC) &&
        ent->NPC->behaviorState != BS_CINEMATIC) { // not a cinematic enemy
...
qboolean NPC_StasisSpawn_Go( gentity_t *ent )
{
        ...
        trace_t	tr;
        gi.trace( &tr, ent->currentOrigin, ent->mins, ent->maxs,
ent->currentOrigin, ent->s.number, MASK_NPCSOLID );
...
bool NPC_SafeSpawn(gentity_t *ent, float safeRadius) {
  ...
  numEnts =
      gi.EntitiesInBox(safeMins, safeMaxs, radiusEnts, MAX_SAFESPAWN_ENTS);
  ...
  for (i = 0; i < numEnts; i++) {
    ...
    distance =
        DistanceSquared(ent->currentOrigin, radiusEnts[i]->currentOrigin);
...
gentity_t *NPC_Spawn_Do(gentity_t *ent, qboolean fullSpawnNow) {
  ...
  */
  ...
  if (ent->spawnflags & NSF_DROP_TO_FLOOR) {
    ...
    gi.trace(&tr, ent->currentOrigin, ent->mins, ent->maxs, bottom,
             ent->s.number, MASK_NPCSOLID, (EG2_Collision)0, 0);
  ...
  newent->client =
      (gclient_t *)gi.Malloc(sizeof(gclient_t), TAG_G_ALLOC, qtrue);
  ...
  if (ent->NPC_type == NULL) {
    ...
    newent->NPC_type = Q_strlwr(G_NewString(
        ent->NPC_type)); // get my own copy so i can free it when i die
  }
  ...
  // If this is a vehicle we need to see what kind it is so we properlly
  // allocate it.
  if (Q_stricmp(ent->classname, "NPC_Vehicle") == 0) {
    ...
    if (iVehIndex == -1) {
      Com_Printf(S_COLOR_RED
                 "ERROR: Attempting to Spawn an unrecognized Vehicle! - %s\n",
                 newent->NPC_type);
      G_FreeEntity(newent);
      if (ent->spawnflags & NSF_DROP_TO_FLOOR) {
        G_SetOrigin(ent, saveOrg);
    ...
    newent->soundSet = G_NewString(
        ent->soundSet); // get my own copy so i can free it when i die

    // NOTE: If you change/add any of these, update NPC_Spawn_f for the new
    // vehicle you want to be able to spawn in manually.
    ...
    default:
      Com_Printf(
          S_COLOR_RED
          "ERROR: Attempting to Spawn an unrecognized Vehicle Type! - %s\n",
          newent->NPC_type);
      G_FreeEntity(newent);
      if (ent->spawnflags & NSF_DROP_TO_FLOOR) {
        G_SetOrigin(ent, saveOrg);
    ...
    if ((ent->spawnflags & 1)) { // wants to explode when not in vis of player
    ...
    if (g_vehicleInfo[iVehIndex].type == VH_FIGHTER) { // FIXME: EXTERN!!!
      newent->flags |=
          (FL_NO_KNOCKBACK |
           FL_SHIELDED); // don't get pushed around, blasters bounce off
    }
    ...
    newent->m_pVehicle->m_vOrientation[PITCH] =
        newent->m_pVehicle->m_vOrientation[ROLL] = 0.0f;
    ...
    // There was no initial anim, it would just open the gear even though it's
    // already on the ground (fixed now, made an initial anim)

    // For SUSPEND spawnflag, the amount of time to drop like a rock after
    // SUSPEND turns off
  ...
  // NOTE: on vehicles, anything in the .npc file will STOMP data on the NPC
  // that's set by the vehicle
  ...
  if (ent->NPC_type) {
    if (!Q_stricmp(ent->NPC_type,
                   "player")) { // Or check NPC_type against player's NPC_type?
...
void NPC_Spawn_Go(gentity_t *ent) { NPC_Spawn_Do(ent, qfalse); }
...
void NPC_ShySpawn(gentity_t *ent) {
  ...
  if (DistanceSquared(g_entities[0].currentOrigin, ent->currentOrigin) <=
...
/*QUAKED NPC_spawner (1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY SAFE

DROPTOFLOOR - NPC can be in air, but will spawn on the closest floor surface
below it CINEMATIC - Will spawn with no default AI (BS_CINEMATIC) NOTSOLID -
Starts not solid STARTINSOLID - Don't try to fix if spawn in solid SHY - Spawner
is shy SAFE - Won't spawn if an entity is within 64 units

NPC_type - name of NPC (in npcs.cfg) to spawn in

targetname - name this NPC goes by for targetting
target - NPC will fire this when it spawns it's last NPC (should this be when
the last NPC it spawned dies?) target2 - Fired by stasis spawners when they try
to spawn while their spawner model is broken target3 - Fired by spawner if they
try to spawn and are blocked and have a wait < 0 (removes them)
...
wait - if trying to spawn and blocked, how many seconds to wait before trying
again (default = 0.5, < 0 = never try again and fire target2)
...
death, etc) "noCombatSounds" - set to 1 to prevent loading and usage of combat
sounds (anger, victory, etc.) "noExtraSounds" - set to 1 to prevent loading and
usage of "extra" sounds (chasing the enemy - detecting them, flanking them...
also jedi combat sounds)
...
fleescript - default script to run when hit and below 50% health (none by
default) deathscript - default script to run when killed (none by default) These
...
first and so no scripts should be names with these names:
    default - 0: whatever
        ...
        snipe - 13: Stay hidden, shoot enemy only when have perfect shot and
back turned combat - 14: Attack, evade, use cover, move about, etc.  Full combat
AI - id NPC code medic - 15: Go for lowest health buddy, hide and heal him.
...
*/
extern qboolean spawning; // the G_Spawn*() functions are valid  (only turned on
                          // during one function)
...
void SP_NPC_spawner(gentity_t *self) {
  ...
  // We have to load the animation.cfg now because spawnscripts are going to
  // want to set anims and we need to know their length and if they're valid
  ...
  if (self->targetname) { // Wait for triggering
    ...
    if (spawning) { // in entity spawn stage - map starting up
  ...
  // FIXME: store cameraGroup somewhere else and apply to spawned NPCs'
  // cameraGroup Or just don't include NPC_spawners in cameraGroupings

  if (self->message) { // may drop a key, precache the key model and pickup
                       // sound
...
// VEHICLES
//=============================================================================================
/*QUAKED NPC_Vehicle (1 0 0) (-16 -16 -24) (16 16 32) NO_VIS_DIE x x x
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY SAFE set NPC_type to vehicle
name in vehicles.dat

NO_VIS_DIE - die after certain amount of time of not having a LOS to the Player
(default time is 10 seconds, change by setting "noVisTime") CINEMATIC - Will
spawn with no default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID
- Don't try to fix if spawn in solid SHY - Spawner is shy SAFE - Won't spawn if
an entity is within 64 units

"noVisTime" - how long to wait after spawning/last being seen by the player
beforw blowing ourselves up (default is 10 seconds) "skin" - which skin to set
"red" for example - If no skin it is random
*/
void NPC_VehicleSpawnUse(gentity_t *self, gentity_t *other,
                         gentity_t *activator) {
  G_VehicleSpawn(self);
}
...
void SP_NPC_Vehicle(gentity_t *self) {
  ...
  // grab this from the spawner
  if ((self->spawnflags & 1)) { // wants to explode when not in vis of player
    if (!self->endFrame) {
  ...
  if (self->targetname) {
    self->svFlags |=
        SVF_NPC_PRECACHE; // Precache The Bike when all other npcs are precached
...
/*QUAKED NPC_Player (1 0 0) (-16 -16 -24) (16 16 32) x RIFLEMAN PHASER TRICORDER
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
*/
...
/*QUAKED NPC_Kyle (1 0 0) (-16 -16 -24) (16 16 32) BOSS x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY BOSS - Uses Boss AI CINEMATIC - Will spawn
with no default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID -
Don't try to fix if spawn in solid SHY - Spawner is shy
*/
...
/*QUAKED NPC_Lando(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR CINEMATIC
NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will spawn on the
closest floor surface below it CINEMATIC - Will spawn with no default AI
(BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try to fix if
spawn in solid SHY - Spawner is shy
*/
...
/*QUAKED NPC_Jan(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR CINEMATIC
NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will spawn on the
closest floor surface below it CINEMATIC - Will spawn with no default AI
(BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try to fix if
spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Luke(1 0 0) (-16 -16 -24) (16 16 40) x x x x CEILING CINEMATIC
NOTSOLID STARTINSOLID SHY CEILING - Sticks to the ceiling until he sees an enemy
or takes pain CINEMATIC - Will spawn with no default AI (BS_CINEMATIC) NOTSOLID
- Starts not solid STARTINSOLID - Don't try to fix if spawn in solid SHY -
Spawner is shy
*/
...
/*QUAKED NPC_MonMothma(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
*/
...
/*QUAKED NPC_Rosh_Penin (1 0 0) (-16 -16 -24) (16 16 32) DARKSIDE NOFORCE x x
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY Good Rosh DARKSIDE - Evil Rosh
...
/*QUAKED NPC_Tavion (1 0 0) (-16 -16 -24) (16 16 32) x x x x CEILING CINEMATIC
NOTSOLID STARTINSOLID SHY CEILING - Sticks to the ceiling until he sees an enemy
or takes pain CINEMATIC - Will spawn with no default AI (BS_CINEMATIC) NOTSOLID
- Starts not solid STARTINSOLID - Don't try to fix if spawn in solid SHY -
Spawner is shy
...
/*QUAKED NPC_Tavion_New (1 0 0) (-16 -16 -24) (16 16 32) SCEPTER SITH_SWORD x x
CEILING CINEMATIC NOTSOLID STARTINSOLID SHY Has a red lightsaber and force
powers, uses her saber style from JK2

SCEPTER - Has a red lightsaber and force powers, Ragnos' Scepter in left hand,
uses dual saber style and occasionally attacks with Scepter SITH_SWORD - Has
Ragnos' Sith Sword in right hand and force powers, uses strong style CEILING -
Sticks to the ceiling until he sees an enemy or takes pain CINEMATIC - Will
spawn with no default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID
- Don't try to fix if spawn in solid SHY - Spawner is shy
*/
...
/*QUAKED NPC_Alora (1 0 0) (-16 -16 -24) (16 16 32) DUAL x x x CEILING CINEMATIC
NOTSOLID STARTINSOLID SHY Lightsaber and level 2 force powers, 300 health
...
/*QUAKED NPC_Reelo(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR CINEMATIC
NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will spawn on the
closest floor surface below it CINEMATIC - Will spawn with no default AI
(BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try to fix if
spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Galak(1 0 0) (-16 -16 -24) (16 16 40) MECH x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY MECH - will be the armored Galak
...
/*QUAKED NPC_Desann(1 0 0) (-16 -16 -24) (16 16 40) x x x x CEILING CINEMATIC
NOTSOLID STARTINSOLID SHY CEILING - Sticks to the ceiling until he sees an enemy
or takes pain CINEMATIC - Will spawn with no default AI (BS_CINEMATIC) NOTSOLID
- Starts not solid STARTINSOLID - Don't try to fix if spawn in solid SHY -
Spawner is shy
...
/*QUAKED NPC_Rax(1 0 0) (-16 -16 -24) (16 16 40) FUN x x x DROPTOFLOOR CINEMATIC
NOTSOLID STARTINSOLID SHY FUN - Makes him magically the funnest thing ever in
any game ever made. (actually does nothing, it'll just be fun by the power of
suggestion). DROPTOFLOOR - NPC can be in air, but will spawn on the closest
floor surface below it CINEMATIC - Will spawn with no default AI (BS_CINEMATIC)
NOTSOLID - Starts not solid
STARTINSOLID - Don't try to fix if spawn in solid
SHY - Spawner is shy
*/
...
/*QUAKED NPC_BobaFett(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Ragnos(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Lannik_Racto(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Kothos(1 0 0) (-16 -16 -24) (16 16 40) VIL x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY VIL - spawns Vil Kothos instead of Dasariah
(can anyone tell them apart anyway...?)

Force only... will (eventually) be set up to re-inforce their leader (use
SET_LEADER) by healing them, recharging them, keeping the player away, etc.
...
/*QUAKED NPC_Chewbacca(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Bartender(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_MorganKatarn(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Jedi(1 0 0) (-16 -16 -24) (16 16 40) TRAINER MASTER RANDOM x
CEILING CINEMATIC NOTSOLID STARTINSOLID SHY TRAINER - Special Jedi- instructor
MASTER - Special Jedi- master
RANDOM - creates a random Jedi student using the available player models/skins
(excludes the current model of the player) CEILING - Sticks to the ceiling until
he sees an enemy or takes pain CINEMATIC - Will spawn with no default AI
(BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try to fix if
spawn in solid SHY - Spawner is shy
  ...
  if (!self->NPC_type) {
    if (self->spawnflags & 4) { // random!
      ...
      while (sanityCheck--) {
        ...
        if (strstr(self->NPC_type, g_char_model->string) !=
            NULL) { // bah, we're using this one, try again
          continue;
        }
...
/*QUAKED NPC_Prisoner(1 0 0) (-16 -16 -24) (16 16 40) ELDER x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Merchant(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Rebel(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR CINEMATIC
NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will spawn on the
closest floor surface below it CINEMATIC - Will spawn with no default AI
(BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try to fix if
spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Human_Merc(1 0 0) (-16 -16 -24) (16 16 40) BOWCASTER REPEATER
FLECHETTE CONCUSSION DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY 100 health,
blaster rifle
...
If you want them to start with any other kind of weapon, make a spawnscript for
them that sets their weapon.

"message" - turns on his key surface.  This is the name of the key you get when
you walk over his body.  This must match the "message" field of the
func_security_panel you want this key to open.  Set to "goodie" to have him
carrying a goodie key that player can use to operate doors with "GOODIE"
spawnflag.  NOTE: this overrides all the weapon spawnflags
...
/*QUAKED NPC_Stormtrooper(1 0 0) (-16 -16 -24) (16 16 40) OFFICER COMMANDER
ALTOFFICER ROCKET DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY COMMANDO 30
health, blaster
...
/*QUAKED NPC_Snowtrooper(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY 30 health, blaster
...
/*QUAKED NPC_Tie_Pilot(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY 30 health, blaster
...
/*QUAKED NPC_RocketTrooper(1 0 0) (-16 -16 -24) (16 16 40) OFFICER SPOTLIGHT x x
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY 200 health, flies, rockets
...
/*QUAKED NPC_HazardTrooper(1 0 0) (-16 -16 -24) (16 16 40) OFFICER CONCUSSION x
x DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY 250 health, repeater
...
/*QUAKED NPC_Ugnaught(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Jawa(1 0 0) (-16 -16 -24) (16 16 40) ARMED x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY ARMED - starts with the Jawa gun in-hand
...
/*QUAKED NPC_Gran(1 0 0) (-16 -16 -24) (16 16 40) SHOOTER BOXER x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY Uses grenade
...
/*QUAKED NPC_Rodian(1 0 0) (-16 -16 -24) (16 16 40) BLASTER NO_HIDE x x
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY BLASTER uses a blaster instead
of sniper rifle, different skin NO_HIDE (only applicable with snipers) does not
duck and hide between shots DROPTOFLOOR - NPC can be in air, but will spawn on
the closest floor surface below it CINEMATIC - Will spawn with no default AI
(BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try to fix if
spawn in solid SHY - Spawner is shy
*/
...
/*QUAKED NPC_Weequay(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Trandoshan(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Tusken(1 0 0) (-16 -16 -24) (16 16 40) SNIPER x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Noghri(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_SwampTrooper(1 0 0) (-16 -16 -24) (16 16 40) REPEATER x x x
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY REPEATER - Swaptrooper who uses
the repeater DROPTOFLOOR - NPC can be in air, but will spawn on the closest
floor surface below it CINEMATIC - Will spawn with no default AI (BS_CINEMATIC)
NOTSOLID - Starts not solid
STARTINSOLID - Don't try to fix if spawn in solid
SHY - Spawner is shy
...
/*QUAKED NPC_Imperial(1 0 0) (-16 -16 -24) (16 16 40) OFFICER COMMANDER x x
...
"message" - if a COMMANDER, turns on his key surface.  This is the name of the
key you get when you walk over his body.  This must match the "message" field of
the func_security_panel you want this key to open.  Set to "goodie" to have him
carrying a goodie key that player can use to operate doors with "GOODIE"
spawnflag.
...
/*QUAKED NPC_ImpWorker(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_BespinCop(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Reborn(1 0 0) (-16 -16 -24) (16 16 40) FORCE FENCER ACROBAT BOSS
CEILING CINEMATIC NOTSOLID STARTINSOLID SHY

Default Reborn is A poor lightsaber fighter, acrobatic and uses no force powers.
40 health.

FORCE - Uses force powers but is not the best lightsaber fighter and not
acrobatic.  75 health. FENCER - A good lightsaber fighter, but not acrobatic and
uses no force powers.  100 health. ACROBAT - quite acrobatic, but not the best
lightsaber fighter and uses no force powers.  100 health. BOSS - quite
acrobatic, good lightsaber fighter and uses force powers.  150 health.
...
nodelay - set to "1" to make the Reborn not stand around and taunt the player
before attacking
*/
...
/*QUAKED NPC_Reborn_New(1 0 0) (-16 -16 -24) (16 16 40) DUAL STAFF WEAK MASTER
CEILING CINEMATIC NOTSOLID STARTINSOLID SHY

Reborn is an excellent lightsaber fighter, acrobatic and uses force powers.
Full-length red saber, 200 health.
...
void SP_NPC_Reborn_New(gentity_t *self) {
  if (!self->NPC_type) {
    if ((self->spawnflags & 8)) { // tougher guys
      if ((self->spawnflags & 1)) {
    ...
    } else if ((self->spawnflags & 4)) { // weaker guys
      if ((self->spawnflags & 1)) {
...
/*QUAKED NPC_Cultist_Saber(1 0 0) (-16 -16 -24) (16 16 40) MED STRONG ALL THROW
CEILING CINEMATIC NOTSOLID STARTINSOLID SHY Uses a saber and no force powers.
100 health.
...
STRONG - Uses strong style, slower than others, but can do a lot of damage with
one blow.  Weak defense. ALL - Knows all 3 styles, switches between them, good
defense. THROW - can throw their saber (level 2) - reduces their defense some
(can use this spawnflag alone or in combination with any *one* of the previous 3
spawnflags)
...
nodelay - set to "1" to make the Cultist not stand around and taunt the player
before attacking
*/
...
/*QUAKED NPC_Cultist_Saber_Powers(1 0 0) (-16 -16 -24) (16 16 40) MED STRONG ALL
THROW CEILING CINEMATIC NOTSOLID STARTINSOLID SHY Uses a saber and has a couple
low-level powers.  150 health.
...
/*QUAKED NPC_Cultist(1 0 0) (-16 -16 -24) (16 16 40) SABER GRIP LIGHTNING DRAIN
CEILING CINEMATIC NOTSOLID STARTINSOLID SHY
...
/*QUAKED NPC_Cultist_Commando(1 0 0) (-16 -16 -24) (16 16 40) x x x x CEILING
...
/*QUAKED NPC_Cultist_Destroyer(1 0 0) (-16 -16 -24) (16 16 40) x x x x CEILING
CINEMATIC NOTSOLID STARTINSOLID SHY

Cultist has no weapons, runs up to you chanting & building up a Force
Destruction blast - when gets to you, screams & explodes
...
/*QUAKED NPC_ShadowTrooper(1 0 0) (-16 -16 -24) (16 16 40) x x x x CEILING
CINEMATIC NOTSOLID STARTINSOLID SHY CEILING - Sticks to the ceiling until he
sees an enemy or takes pain CINEMATIC - Will spawn with no default AI
(BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try to fix if
spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Saboteur(1 0 0) (-16 -16 -24) (16 16 40) SNIPER PISTOL x x CLOAKED
CINEMATIC NOTSOLID STARTINSOLID SHY Has a blaster rifle, can cloak and roll
...
/*QUAKED NPC_Monster_Murjj (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Monster_Swamp (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Monster_Howler (1 0 0) (-16 -16 -24) (16 16 8) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Monster_Rancor (1 0 0) (-30 -30 -24) (30 30 136) MUTANT FASTKILL x
x DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY 2000 health, picks people up
and eats them
...
/*QUAKED NPC_Monster_Mutant_Rancor (1 0 0) (-60 -60 -24) (60 60 360) x FASTKILL
x x DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY Bigger, meaner, nastier,
Frencher.  Breath attack, pound attack.
...
/*QUAKED NPC_Monster_Wampa (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_MineMonster (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Monster_Claw (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Monster_Glider (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Monster_Flier2 (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Monster_Lizard (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Monster_Fish (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Monster_Sand_Creature (1 0 0) (-24 -24 -24) (24 24 0) FAST x x x
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air,
but will spawn on the closest floor surface below it CINEMATIC - Will spawn with
no default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't
try to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Droid_Interrogator (1 0 0) (-12 -12 -24) (12 12 0) x x x x
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air,
but will spawn on the closest floor surface below it CINEMATIC - Will spawn with
no default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't
try to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Droid_Probe (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy

Imperial Probe Droid - the multilegged floating droid that Han and Chewie shot
...
/*QUAKED NPC_Droid_Mark1 (1 0 0) (-36 -36 -24) (36 36 80) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Droid_Mark2 (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Droid_ATST (1 0 0) (-40 -40 -24) (40 40 248) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Droid_Remote (1 0 0) (-4 -4 -24) (4 4 8) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Droid_Seeker (1 0 0) (-4 -4 -24) (4 4 8) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Droid_Sentry (1 0 0) (-24 -24 -24) (24 24 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Droid_Gonk (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy

Gonk Droid - the droid that looks like a walking ice machine. Was in the Jawa
land crawler, walking around talking to itself.
...
/*QUAKED NPC_Droid_Mouse (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy

Mouse Droid - small, box shaped droid, first seen on the Death Star. Chewie
yelled at it and it backed up and ran away.
...
/*QUAKED NPC_Droid_R2D2 (1 0 0) (-12 -12 -24) (12 12 40) IMPERIAL x x x
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air,
but will spawn on the closest floor surface below it CINEMATIC - Will spawn with
no default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't
try to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Droid_R5D2 (1 0 0) (-12 -12 -24) (12 12 40) IMPERIAL ALWAYSDIE x x
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air,
but will spawn on the closest floor surface below it ALWAYSDIE - won't go into
spinning zombie AI when at low health. CINEMATIC - Will spawn with no default AI
(BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try to fix if
spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Droid_Protocol (1 0 0) (-12 -12 -24) (12 12 40) IMPERIAL x x x
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air,
but will spawn on the closest floor surface below it CINEMATIC - Will spawn with
no default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't
try to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Droid_Assassin (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Droid_Saber (1 0 0) (-12 -12 -24) (12 12 40) TRAINING x x x
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air,
but will spawn on the closest floor surface below it CINEMATIC - Will spawn with
no default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't
try to fix if spawn in solid SHY - Spawner is shy
...
static void NPC_Spawn_f(void) {
  ...
  if (!Q_stricmp("vehicle", npc_type)) { // spawning a vehicle
    isVehicle = qtrue;
    npc_type = gi.argv(3);
    if (!npc_type || !npc_type[0]) {
      gi.Printf(
  ...
  if (isVehicle) { // must let NPC spawn func know this is a vehicle we're
                   // trying to spawn
    NPCspawner->classname = "NPC_Vehicle";
  }

  NPC_PrecacheByClassName(NPCspawner->NPC_type);
  if (!Q_stricmp("kyle_boss", NPCspawner->NPC_type)) { // bah
    NPCspawner->spawnflags |= 1;
  }

  if (!Q_stricmp("key", NPCspawner->NPC_type)) { // bah
    NPCspawner->message = "key";
    NPCspawner->NPC_type = "imperial";
  }
  if (!Q_stricmp("jedi_random",
                 NPCspawner->NPC_type)) { // special case, for testing
...
void NPC_Kill_f(void) {
  ...
  if (!*name || !name[0]) {
    ...
    gi.Printf(S_COLOR_RED "or\n");
    gi.Printf(S_COLOR_RED "NPC team '[teamname]' - kills all NPCs of a certain "
                          "team ('nonally' is all but your allies)\n");
    return;
  }
  ...
  for (n = 1; n < ENTITYNUM_MAX_NORMAL; n++) {
    ...
    if (killNonSF) {
      if (player) {
        if (player->client) {
          if (player->client->playerTeam != TEAM_PLAYER) {
            ...
            {
                    G_Damage( player, NULL, NULL, NULL, NULL,
            player->health+10000, 0, MOD_UNKNOWN );
            }
            else
            */
            {
              player->health = 0;
              GEntity_DieFunc(player, player, player, player->max_health,
                              MOD_UNKNOWN);
            }
          }
        } else if (player->NPC_type && player->classname &&
                   player->classname[0] &&
                   Q_stricmp("NPC_starfleet", player->classname) !=
                       0) { // A spawner, remove it
          gi.Printf(S_COLOR_GREEN "Removing NPC spawner %s with NPC named %s\n",
                    player->NPC_type, player->NPC_targetname);
          G_FreeEntity(player);
          // FIXME: G_UseTargets2(player, player, player->NPC_target &
          // player->target);?
        }
      }
      ...
      if (killTeam != TEAM_FREE) {
      ...
      } else if ((player->targetname &&
                  Q_stricmp(name, player->targetname) == 0) ||
        ...
        if ( (player->flags&FL_UNDYING) )
        {
                G_Damage( player, NULL, NULL, NULL, NULL, player->health+10000,
        0, MOD_UNKNOWN );
        }
        else
        */
    ...
    } else if (player && (player->svFlags & SVF_NPC_PRECACHE)) { // a spawner
...
void Svcmd_NPC_f(void) {
  ...
  if (!*cmd) {
    ...
    gi.Printf(" spawn vehicle [NPC type (from *.npc files, only for NPCs that "
              "are CLASS_VEHICLE and have a .veh file)]\n");
    gi.Printf(" kill [NPC targetname] or [all(kills all NPCs)] or 'team "
              "[teamname]'\n");
    ...
    NPC_Kill_f();
  } else if (Q_stricmp(cmd, "showbounds") == 0) { // Toggle on and off
    showBBoxes = showBBoxes ? qfalse : qtrue;
  } else if (Q_stricmp(cmd, "score") == 0) {
    char *cmd2 = gi.argv(2);
    gentity_t *ent = NULL;

    if (!cmd2 || !cmd2[0]) { // Show the score for all NPCs
