...
bool BG_FighterUpdate(Vehicle_t *pVeh, const usercmd_t *pUcmd, vec3_t trMins, vec3_t trMaxs, float gravity,
					  void (*traceFunc)(trace_t *results, const vec3_t start, const vec3_t lmins, const vec3_t lmaxs, const vec3_t end, int passEntityNum,
										int contentMask)) {
	vec3_t bottom;
...
static bool Update(Vehicle_t *pVeh, const usercmd_t *pUcmd) {
	...
	if (!BG_FighterUpdate(pVeh, pUcmd, ((gentity_t *)pVeh->m_pParentEntity)->mins, ((gentity_t *)pVeh->m_pParentEntity)->maxs,
#ifdef _JK2MP
						  g_gravity.value,
#else
						  g_gravity->value,
...
// a predicted event or only doing it game-side. -rich
...
#ifdef _JK2MP
			if (parentPS->velocity[2] <= 0 && pVeh->m_pVehicleInfo->soundTakeOff) { // taking off for the first time
#ifdef QAGAME																		// MP GAME-side
...
#ifndef _JK2MP // SP
					G_PlayEffect(pVeh->m_pVehicleInfo->iTurboStartFX, pVeh->m_pParentEntity->playerModel, pVeh->m_iExhaustTag[i],
								 pVeh->m_pParentEntity->s.number, pVeh->m_pParentEntity->currentOrigin);
#else
					// TODO: MP Play Effect?
#endif
				}
	...
	*/
	...
	if (pVeh->m_iRemovedSurfaces || parentPS->electrifyTime >= curTime) { // go out of control
...
#endif
	else if ((parentPS->speed || parentPS->groundEntityNum == ENTITYNUM_NONE || pVeh->m_ucmd.forwardmove || pVeh->m_ucmd.upmove > 0) &&
		...
		else if (pVeh->m_pVehicleInfo->throttleSticks) {	// we're using a throttle that sticks at current speed
		...
		if (!pVeh->m_pVehicleInfo->strafePerc ||
			(!g_speederControlScheme->value &&
			 !pVeh->m_pParentEntity->s.number)) { // if in a strafe-capable vehicle, clear strafing unless using alternate control scheme
	...
	if (pVeh->m_pVehicleInfo->strafePerc
		...
		if (pVeh->m_ucmd.rightmove > 0) { // strafe right
			...
			if (pVeh->m_fStrafeTime > -MAX_STRAFE_TIME) { // can strafe right for 2 seconds
			...
			if (pVeh->m_fStrafeTime < MAX_STRAFE_TIME) { // can strafe left for 2 seconds
				float curStrafeSpeed = DotProduct(parentPS->velocity, vRight);
...
// STRAFING==============================================================================
	...
	if (pVeh->m_iRemovedSurfaces || parentPS->electrifyTime >= curTime) { // going down
		...
		parentPS->gravity = 0;
	} else if ((!parentPS->speed || parentPS->speed < speedIdle) && pVeh->m_ucmd.upmove <= 0) { // slowing down or stopped and not trying to take off
...
static void ProcessOrientCommands(Vehicle_t *pVeh) {
		...
		&& !pVeh->m_iRemovedSurfaces && parentPS->electrifyTime < curTime) // not spiraling out of control
	{
		...
		if (pVeh->m_LandTrace.fraction > 0.1f ||
			pVeh->m_LandTrace.plane.normal[2] < MIN_LANDING_SLOPE) { // off the ground, at least (or not on a valid landing surf)
		...
		if (BG_UnrestrainedPitchRoll(riderPS, pVeh)) {
			...
			*/
			...
			// if ( ( pVeh->m_LandTrace.fraction >= 1.0f || pVeh->m_ucmd.forwardmove != 0 ) && pVeh->m_LandTrace.fraction >= 0.0f )
			{
				...
				fYawDelta =
					AngleSubtract(pVeh->m_vOrientation[YAW], pVeh->m_vPrevOrientation[YAW]); // pVeh->m_vOrientation[YAW] - pVeh->m_vPrevOrientation[YAW];
				if (fYawDelta > 8.0f) {
					fYawDelta = 8.0f;
	...
	*/
	...
	if (!pVeh->m_fStrafeTime) { // use that roll
		...
		if (pVeh->m_vOrientation[ROLL]) {																 // continually adjust the yaw based on the roll..
...
#endif
	{
		...
		// otherwise we're below launch height and still taking off.
			...
			if ((pVeh->m_ucmd.forwardmove < 0 || pVeh->m_ucmd.upmove < 0 || isLanded) && pVeh->m_LandTrace.fraction <= 0.4f &&
				pVeh->m_LandTrace.plane.normal[2] >= MIN_LANDING_SLOPE) { // already landed or trying to land and close to ground
				// Open gears.
