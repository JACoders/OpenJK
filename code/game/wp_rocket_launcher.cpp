...
//---------------------------------------------------------
{
	...
	if (ent->disconnectDebounceTime && ent->disconnectDebounceTime < level.time) { // time's up, we're done, remove us
	...
	if (alt_fire) {
		...
		if ((lockEntNum > 0 || (ent->NPC && lockEntNum >= 0)) && lockEntNum < ENTITYNUM_WORLD && lockTime > 0) {
			...
			if (dif == 8 || Q_flrand(0.0f, 1.0f) * dif > 2 || Q_flrand(0.0f, 1.0f) > 0.97f) {
				...
				if (missile->enemy &&
					missile->enemy->inuse) //&& DistanceSquared( missile->currentOrigin, missile->enemy->currentOrigin ) < 262144 && InFOV(
										   //missile->currentOrigin, missile->enemy->currentOrigin, missile->enemy->client->ps.viewangles, 45, 45 ) )
				{
					if (missile->enemy->client && (missile->enemy->client->ps.forcePowersKnown & (1 << FP_PUSH)) &&
						missile->enemy->client->ps.forcePowerLevel[FP_PUSH] > FORCE_LEVEL_0) { // have force push, don't flee from homing rockets
					} else {
						vec3_t dir, dir2;
