...
#define MAX_CUSTOM_SOUNDS                                                      \
  40 // rww - Note that for now these must all be the same, because of the way I
     // am cycling through them and comparing for custom sounds.
...
typedef struct clientInfo_s {
  ...
  float facial_blink; // time before next blink. If a minus value, we are in
                      // blink mode
  float facial_frown; // time before next frown. If a minus value, we are in
                      // frown mode
  float
      facial_aux; // time before next aux. If a minus value, we are in aux mode
...
typedef struct centity_s {
  // This comment below is correct, but now m_pVehicle is the first thing in bg
  // shared entity, so it goes first. - AReis
  ...
  //interpolation 	vec3_t			rawOrigin;
  vec3_t rawAngles;
  ...
  qboolean noLumbar; // if true only do anims and things on model_root instead
                     // of lower_lumbar, this will be the case for some NPCs.
  qboolean noFace;

  // For keeping track of the current surface status in relation to the
  // entitystate surface fields.
  int npcLocalSurfOn;
  int npcLocalSurfOff;

  int eventAnimIndex;

  clientInfo_t *npcClient; // dynamically allocated - always free it, and never
                           // stomp over it.

  int weapon;

  void *ghoul2weapon; // rww - pointer to ghoul2 instance of the current 3rd
                      // person weapon

  float radius;
  int boltInfo;

  // sometimes used as a bolt index, but these values are also used as generic
  // values for clientside entities at times
  int bolt1;
  int bolt2;
  int bolt3;
  int bolt4;
...
typedef struct localEntity_s {
  ...
  union {
    ...
    struct {
      ...
      vec3_t
          dir; // magnitude is 1, but this is oldpos - newpos right before the
               // particle is sent to the renderer
      // may want to add something like particle::localEntity_s *le (for the
      // particle's think fn)
    } particle;
    struct {
      qboolean dontDie;
...
void CG_DrawProportionalString(int x, int y, const char *str, int style,
                               vec4_t color);
void CG_DrawScaledProportionalString(int x, int y, const char *str, int style,
...
qboolean CG_G2TraceCollide(trace_t *tr, const vec3_t mins, const vec3_t maxs,
                           const vec3_t lastValidStart,
                           const vec3_t lastValidEnd);
void CG_AddGhoul2Mark(int shader, float size, vec3_t start, vec3_t end,
                      int entnum, vec3_t entposition, float entangle,
                      void *ghoul2, vec3_t scale, int lifeTime);
...
void CG_G2Trace(trace_t *result, const vec3_t start, const vec3_t mins,
                const vec3_t maxs, const vec3_t end, int skipNumber, int mask);
...
void CG_S_AddLoopingSound(int entityNum, const vec3_t origin,
                          const vec3_t velocity, sfxHandle_t sfx);
void CG_S_AddRealLoopingSound(int entityNum, const vec3_t origin,
                              const vec3_t velocity, sfxHandle_t sfx);
...
void CG_GlassShatter(int entnum, vec3_t dmgPt, vec3_t dmgDir, float dmgRadius,
                     int maxShards);
void CG_ScorePlum(int client, vec3_t org, int score);

void CG_Chunks(int owner, vec3_t origin, const vec3_t normal, const vec3_t mins,
               const vec3_t maxs, float speed, int numChunks,
               material_t chunkType, int customChunk, float baseScale);
void CG_MiscModelExplosion(vec3_t mins, vec3_t maxs, int size,
                           material_t chunkType);

localEntity_t *CG_MakeExplosion(vec3_t origin, vec3_t dir, qhandle_t hModel,
                                int numframes, qhandle_t shader, int msec,
                                qboolean isSprite, float scale,
                                int flags); // Overloaded in single player

void CG_SurfaceExplosion(vec3_t origin, vec3_t normal, float radius,
...
qboolean CG_SpawnBoolean(const char *key, const char *defaultString,
                         qboolean *out);
...
// cg_playerstate.c
//
int CG_IsMindTricked(int trickIndex1, int trickIndex2, int trickIndex3,
