...
typedef struct vehWeaponInfo_s {
  ...
  float fHoming; // 0.0 = not homing, 0.5 = half vel to targ, half cur vel, 1.0
                 // = all vel to targ
  float
      fHomingFOV; // missile will lose lock on if DotProduct of missile
                  // direction and direction to target ever drops below this (-1
                  // to 1, -1 = never lose target, 0 = lose if ship gets behind
                  // missile, 1 = pretty much will lose it's target right away)
...
typedef struct vehicleInfo_s {
  ...
  int dmgIndicFrameHandle; // what image to use for the frame of the damage
                           // indicator
  int dmgIndicShieldHandle; // what image to use for the shield of the damage
                            // indicator
  int dmgIndicBackgroundHandle; // what image to use for the background of the
                                // damage indicator
  int iconFrontHandle; // what image to use for the front of the ship on the
                       // damage indicator
  int iconBackHandle; // what image to use for the back of the ship on the
                      // damage indicator
  int iconRightHandle; // what image to use for the right of the ship on the
                       // damage indicator
  int iconLeftHandle; // what image to use for the left of the ship on the
                      // damage indicator
  int crosshairShaderHandle; // what image to use for the left of the ship on
  ...
  //otherwise.
  qboolean (*UpdateRider)(Vehicle_t *pVeh, bgEntity_t *pRider,
                          usercmd_t *pUcmd);
...
#endif
{
  ...
  // Yeah, since we use the SP code for vehicles, I want to use this value, but
  // I'm going to make it a pointer to a vec3_t in the playerstate for
  // prediction's sake. -rww
