...
static void R_LoadLightmaps(lump_t *l, const char *psMapName,
  ...
  for (i = 0; i < tr.numLightmaps; i++) {
    ...
    tr.lightmaps[i] =
        R_CreateImage(va("*%s/lightmap%d", sMapName, i), image, LIGHTMAP_SIZE,
                      LIGHTMAP_SIZE, GL_RGBA, qfalse, qfalse,
...
===============
*/
static shader_t *ShaderForShaderNum(int shaderNum, const int *lightmapNum,
                                    const byte *lightmapStyles,
                                    const byte *vertexStyles,
                                    world_t &worldData) {
  shader_t *shader;
  dshader_t *dsh;
  const byte *styles;
...
===============
*/
static void ParseFace(dsurface_t *ds, mapVert_t *verts, msurface_t *surf,
                      int *indexes, world_t &worldData, int index) {
  int i, j, k;
  srfSurfaceFace_t *cv;
  ...
  if (numPoints > MAX_FACE_POINTS) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "WARNING: MAX_FACE_POINTS exceeded: %i\n",
              numPoints);
  ...
  for (i = 0; i < numPoints; i++) {
    ...
    for (j = 0; j < 2; j++) {
      ...
      for (k = 0; k < MAXLIGHTMAPS; k++) {
        cv->points[i][VERTEX_LM + j + (k * 2)] =
            LittleFloat(verts[i].lightmap[k][j]);
      }
    }
    for (k = 0; k < MAXLIGHTMAPS; k++) {
      R_ColorShiftLightingBytes(verts[i].color[k],
                                (byte *)&cv->points[i][VERTEX_COLOR + k]);
    }
  }
...
===============
*/
static void ParseTriSurf(dsurface_t *ds, mapVert_t *verts, msurface_t *surf,
                         int *indexes, world_t &worldData, int index) {
  srfTriangles_t *tri;
  int i, j, k;
  ...
  // get shader
  surf->shader =
      ShaderForShaderNum(ds->shaderNum, lightmapsVertex, ds->lightmapStyles,
                         ds->vertexStyles, worldData);
  if (r_singleShader->integer && !surf->shader->sky) {
    surf->shader = tr.defaultShader;
  }
  ...
  if (numVerts >= SHADER_MAX_VERTEXES) {
    Com_Error(ERR_DROP, "ParseTriSurf: verts > MAX (%d > %d) on misc_model %s",
              numVerts, SHADER_MAX_VERTEXES, surf->shader->name);
  }
  if (numIndexes >= SHADER_MAX_INDEXES) {
    Com_Error(ERR_DROP,
              "ParseTriSurf: indices > MAX (%d > %d) on misc_model %s",
              numIndexes, SHADER_MAX_INDEXES, surf->shader->name);
  }

  tri = (srfTriangles_t *)Hunk_Alloc(sizeof(*tri) +
                                         numVerts * sizeof(tri->verts[0]) +
...
===============
*/
static void ParseFlare(dsurface_t *ds, mapVert_t *verts, msurface_t *surf,
                       int *indexes, world_t &worldData, int index) {
  srfFlare_t *flare;
  int i;
  ...
  // get shader
  surf->shader =
      ShaderForShaderNum(ds->shaderNum, lightmaps, ds->lightmapStyles,
                         ds->vertexStyles, worldData);
  if (r_singleShader->integer && !surf->shader->sky) {
    surf->shader = tr.defaultShader;
  }
...
static void R_LoadNodesAndLeafs(lump_t *nodeLump, lump_t *leafLump,
  ...
  out =
      (struct mnode_s *)Hunk_Alloc((numNodes + numLeafs) * sizeof(*out), h_low);
  ...
  for (i = 0; i < numLeafs; i++, inLeaf++, out++) {
    ...
    out->firstmarksurface =
        worldData.marksurfaces + LittleLong(inLeaf->firstLeafSurface);
...
static void R_LoadFogs(lump_t *l, lump_t *brushesLump, lump_t *sidesLump,
  ...
  for (i = 0; i < count; i++, fogs++) {
    ...
    if (sideNum == -1) {
      ...
      VectorSubtract(vec3_origin, worldData.planes[planeNum].normal,
                     out->surface);
...
void R_LoadLightGridArray(lump_t *l, world_t &worldData) {
  ...
  if ((unsigned)l->filelen !=
      w->numGridArrayElements * sizeof(*w->lightGridArray)) {
...
void R_LoadEntities(lump_t *l, world_t &worldData) {
  ...
  while (1) {
    ...
    if (!Q_strncmp(keyname, s, strlen(s))) {
      ...
      if (!s) {
        ri.Printf(PRINT_ALL,
                  S_COLOR_YELLOW "WARNING: no semi colon in shaderremap '%s'\n",
                  value);
        break;
      }
      *s++ = 0;
    ...
    if (!Q_stricmp(keyname, "_color")) {
      sscanf(value, "%f %f %f", &tr.sunAmbient[0], &tr.sunAmbient[1],
             &tr.sunAmbient[2]);
      continue;
    }
...
void RE_LoadWorldMap_Actual(const char *name, world_t &worldData, int index) {
  ...
  R_LoadFogs(&header->lumps[LUMP_FOGS], &header->lumps[LUMP_BRUSHES],
             &header->lumps[LUMP_BRUSHSIDES], worldData, index);
  R_LoadSurfaces(&header->lumps[LUMP_SURFACES], &header->lumps[LUMP_DRAWVERTS],
                 &header->lumps[LUMP_DRAWINDEXES], worldData, index);
...
// new wrapper used for convenience to tell z_malloc()-fail recovery code
// whether it's safe to dump the cached-bsp or not.
//
void RE_LoadWorldMap(const char *name) {
