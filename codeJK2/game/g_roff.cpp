...
void G_RoffNotetrackCallback(gentity_t *cent, const char *notetrack) {
  int i = 0, r = 0, r2 = 0, objectID = 0, anglesGathered = 0,
      posoffsetGathered = 0;
  char type[256];
  char argument[512];
  char addlArg[512];
  ...
  defaultoffsetposition:
    ...
    if (r2 && strstr(teststr, "effects")) { // get rid of the leading "effects"
                                            // since it's auto-inserted
      r++;
      r2 = 0;
...
qboolean G_ValidRoff(roff_hdr2_t *header) {
  if (!strncmp(header->mHeader, "ROFF", 4)) {
    if (LittleLong(header->mVersion) == ROFF_VERSION2 &&
        LittleLong(header->mCount) > 0) {
      return qtrue;
    } else if (LittleLong(header->mVersion) == ROFF_VERSION &&
               LittleFloat(((roff_hdr_t *)header)->mCount) >
                   0.0f) { // version 1 defines the count as a float, so we best
                           // do the count check as a float or we'll get bogus
                           // results
      return qtrue;
    }
  }
...
qboolean G_InitRoff(char *file, unsigned char *data) {
  ...
  if (LittleLong(header->mVersion) == ROFF_VERSION) {
    ...
    roffs[num_roffs].mFrameTime =
        100; // old school ones have a hard-coded frame time
    roffs[num_roffs].mLerp = 10;

    if (mem) {
      // The allocation worked, so stash this stuff off so we can reference the
      ...
      for (i = 0; i < count; i++, roff_data++, mem++) {
        // Copy just the delta position and orientation which can be applied to
        // anything at a later point
#ifdef Q3_BIG_ENDIAN
    ...
    if (mem) {
      ...
      roffs[num_roffs].type =
          2; // rww - any reason this wasn't being set already?
      ...
      if (LittleLong(hdr->mNumNotes)) {
        ...
        roffs[num_roffs].mNoteTrackIndexes =
            new char *[LittleLong(hdr->mNumNotes)];
...
int G_LoadRoff(const char *fileName) {
  ...
  // Before even bothering with all of this, make sure we have a place to store
  // it.
  if (num_roffs >= MAX_ROFFS) {
    Com_Printf(S_COLOR_RED
               "MAX_ROFFS count exceeded.  Skipping load of .ROF '%s'\n",
               fileName);
    return roff_id;
  }
  ...
  if (!G_ValidRoff(header)) {
    ...
    // Done loading this roff, so save off an id to it..increment first to avoid
    // zero index
    roff_id = ++num_roffs;
  }
...
void G_Roff(gentity_t *ent) {
  ...
  if (ent->next_roff_time > level.time) { // either I don't think or it's just
                                          // not time to have me think yet
    return;
  }
  ...
  if (roff->type == 2) {
    ...
    if (data->mStartNote != -1 || data->mNumNotes) {
      G_RoffNotetrackCallback(
          ent, roffs[roff_id - 1].mNoteTrackIndexes[data->mStartNote]);
    }
  } else {
  ...
  if (g_developer->integer) {
    Com_Printf(S_COLOR_GREEN
               "ROFF dat: num: %d o:<%.2f %.2f %.2f> a:<%.2f %.2f %.2f>\n",
  ...
  if (ent->client) {
    ...
    // make it true linear... FIXME: sticks around after ROFF is done, but do we
    // really care?
    ent->alt_fire = qtrue;

    if (!ent->e_ThinkFunc && ent->s.eType != ET_MISSILE &&
        ent->s.eType != ET_ITEM &&
        ent->s.eType !=
            ET_MOVER) { // will never set currentAngles & currentOrigin itself
...
void G_SaveCachedRoffs() {
  ...
  // Now dump out the cached ROFF file names in order so they can be loaded on
  // the other end
  for (i = 0; i < num_roffs; i++) {
    // Dump out the string length to make things a bit easier on the other
    // end...heh heh.
    len = strlen(roffs[i].fileName) + 1;

    saved_game.write_chunk<int32_t>(INT_ID('S', 'L', 'E', 'N'), len);

    saved_game.write_chunk(INT_ID('R', 'S', 'T', 'R'), ::roffs[i].fileName,
                           len);
  }
}
