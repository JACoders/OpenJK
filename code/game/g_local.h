...
#define FL_DMG_BY_HEAVY_WEAP_ONLY                                              \
  0x00000002 // protected from all damage except heavy weap class missiles
#define FL_DMG_BY_SABER_ONLY                                                   \
  0x00000004 // protected from all damage except saber damage
...
//#define FL_OVERCHARGED			0x00008000	// weapon shot is an
//overcharged version....probably a lame place to be putting this flag...
#define FL_LOCK_PLAYER_WEAPONS                                                 \
  0x00010000 // player can't switch weapons... ask james if there's a better
             // spot for this
#define FL_DISINTEGRATED                                                       \
  0x00020000 // marks that the corpse has already been disintegrated
...
#define FL_NO_ANGLES                                                           \
  0x00200000 // No bone angle overrides, no pitch or roll in full angles
...
// =====================================
//
// NOTE!!!!!!   The only things beyond this point in the structure should be the
// ones you do NOT wish to be
//					affected by loading saved-games. Since loading a
//game first starts the map and then loads 					over things like entities etc then
//these fields are usually the ones setup by the map loader. 					If they ever get
//modified in-game let me know and I'll include them in the save. -Ste
//
#define LEVEL_LOCALS_T_SAVESTOP                                                \
  logFile // name of whichever field is next below this in the source
  ...
  int numCombatPoints;
  char spawntarget[MAX_QPATH]; // the targetname of the spawnpoint you want the
                               // player to start at
...
void G_SetBoltSurfaceRemoval(const int entNum, const int modelIndex,
                             const int boltIndex, const int surfaceIndex = -1,
                             float duration = 5000);
/*
Ghoul2 Insert End
*/
...
void G_PlayEffect(int fxID, const int modelIndex, const int boltIndex,
                  const int entNum, const vec3_t origin, int iLoopTime = qfalse,
                  qboolean isRelative = qfalse); // iLoopTime 0 = not looping, 1
                                                 // for infinite, else duration
...
void G_RadiusDamage(const vec3_t origin, gentity_t *attacker, float damage,
...
#define DAMAGE_HEAVY_WEAP_CLASS                                                \
  0x00001000 // doing heavy weapon type damage, certain objects may only take
             // damage by missiles containing this flag
#define DAMAGE_CUSTOM_HUD 0x00002000 // really dumb, but....
#define DAMAGE_IMPACT_DIE                                                      \
  0x00004000 // if a vehicle hits a wall it should instantly die
#define DAMAGE_DIE_ON_IMPACT 0x00008000 // ignores force-power based protection
#define DAMAGE_SABER_KNOCKBACK1                                                \
  0x00010000 // scale knockback based on saber1's knockbackScale value
#define DAMAGE_SABER_KNOCKBACK2                                                \
  0x00020000 // scale knockback based on saber2's knockbackScale value
#define DAMAGE_SABER_KNOCKBACK1_B2                                             \
  0x00040000 // scale knockback based on saber1's knockbackScale2 value
#define DAMAGE_SABER_KNOCKBACK2_B2                                             \
  0x00080000 // scale knockback based on saber2's knockbackScale2 value
//
// g_missile.c
//
...
extern void G_SoundIndexOnEnt(gentity_t *ent, soundChannel_t channel,
                              int index);
...
extern void AddSightEvent(gentity_t *owner, vec3_t position, float radius,
                          alertEventLevel_e alertLevel, float addLight = 0.0f);
extern void AddSoundEvent(gentity_t *owner, vec3_t position, float radius,
                          alertEventLevel_e alertLevel,
                          qboolean needLOS = qfalse,
                          qboolean onGround = qfalse);
extern qboolean G_CheckForDanger(gentity_t *self, int alertEvent);
extern int G_CheckAlertEvents(gentity_t *self, qboolean checkSight,
                              qboolean checkSound, float maxSeeDist,
                              float maxHearDist, int ignoreAlert = -1,
                              qboolean mustHaveOwner = qfalse,
...
extern reference_tag_t *TAG_Add(const char *name, const char *owner,
                                vec3_t origin, vec3_t angles, int radius,
                                int flags);
...
qboolean TIMER_Done2(gentity_t *ent, const char *identifier,
