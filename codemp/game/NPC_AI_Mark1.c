...
void NPC_Mark1_Part_Explode(gentity_t *self, int bolt) {
  if (bolt >= 0) {
    ...
    trap->G2API_GetBoltMatrix(self->ghoul2, 0, bolt, &boltMatrix,
  ...
  // G_PlayEffectID( G_EffectIndex("blaster/smoke_bolton"), self->playerModel,
  // bolt, self->s.number );
}
...
void Mark1Dead_FireRocket(void) {
  ...
  trap->G2API_GetBoltMatrix(
      NPCS.NPC->ghoul2, 0, bolt, &boltMatrix, NPCS.NPC->r.currentAngles,
  ...
  missile = CreateMissile(muzzle1, muzzle_dir, BOWCASTER_VELOCITY, 10000,
                          NPCS.NPC, qfalse);
...
void Mark1_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
  ...
  (self->client->ps.torsoTimer<=0)) {	// This is because self->health keeps
  getting zeroed out. HL_NONE acts as health in this case.
  ...
  */
  ...
  G_Sound(
      self, CHAN_AUTO,
      G_SoundIndex(va("sound/chars/mark1/misc/death%d.wav", Q_irand(1, 2))));
  ...
  if (self->client->ps.torsoTimer > 0) {
    if (TIMER_Done(self, "dyingExplosion")) {
      ...
      if (num == 1) {
        ...
        num = Q_irand(1, 6);
        newBolt =
            trap->G2API_AddBolt(self->ghoul2, 0, va("*torso_tube%d", num));
        NPC_Mark1_Part_Explode(self, newBolt);
    ...
    if (!trap->G2API_GetSurfaceRenderStatus(
            self->ghoul2, 0, "l_arm")) // Is the blaster still on the model?
    {
      if (Q_irand(1, 5) == 1) {
        SaveNPCGlobals();
        SetNPCGlobals(self);
    ...
    if (!trap->G2API_GetSurfaceRenderStatus(
            self->ghoul2, 0, "r_arm")) // Is the rocket still on the model?
    {
      if (Q_irand(1, 10) == 1) {
        SaveNPCGlobals();
        SetNPCGlobals(self);
    ...
    for (i = 0; i < 6; i++) {
           ...
           AMMO_POD_HEALTH)) // Blow it up?
      {
        if (self->locationDamage[hitLoc] >= AMMO_POD_HEALTH) {
          newBolt = trap->G2API_AddBolt(self->ghoul2, 0,
                                        va("*torso_tube%d", (i + 1)));
          if (newBolt != -1) {
            NPC_Mark1_Part_Explode(self, newBolt);
          }
...
void Mark1_FireBlaster(void) {
  ...
  if ((NPCS.NPCInfo->localState <= LSTATE_FIRED0) ||
...
void Mark1_BlasterAttack(qboolean advance) {
  ...
  if (TIMER_Done(NPCS.NPC, "attackDelay")) // Attack?
  {
    ...
    } else if (NPCS.NPCInfo->burstCount >
               12) // Too many shots fired this burst?
    {
    ...
    if (chance == 1) {
      ...
      if (TIMER_Done(NPCS.NPC,
                     "attackDelay2")) // Can't be shooting every frame.
      {
        TIMER_Set(NPCS.NPC, "attackDelay2", Q_irand(50, 50));
        Mark1_FireBlaster();
        NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_ATTACK1,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      }
      return;
    }
...
void Mark1_FireRocket(void) {
  ...
  missile = CreateMissile(muzzle1, forward, BOWCASTER_VELOCITY, 10000, NPCS.NPC,
                          qfalse);
...
void Mark1_RocketAttack(qboolean advance) {
  ...
  {
    TIMER_Set(NPCS.NPC, "attackDelay", Q_irand(1000, 3000));
    NPC_SetAnim(NPCS.NPC, SETANIM_TORSO, BOTH_ATTACK2,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    Mark1_FireRocket();
  } else if (advance) {
    Mark1_Hunt();
...
void Mark1_AttackDecision(void) {
  ...
  blasterTest =
      trap->G2API_GetSurfaceRenderStatus(NPCS.NPC->ghoul2, 0, "l_arm");
