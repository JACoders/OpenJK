...
#define OVERRIDE_MOUSE_SENSITIVITY                                             \
  5.0f // 20.0f = 180 degree turn in one mouse swipe across keyboard
#else  // VEH_CONTROL_SCHEME_4
#define OVERRIDE_MOUSE_SENSITIVITY                                             \
  10.0f // 20.0f = 180 degree turn in one mouse swipe across keyboard
...
void IN_AutoMapButton(void) { g_clAutoMapMode = !g_clAutoMapMode; }
...
void IN_VoiceChatButton(void) {
  if (!cls.uiStarted) { // ui not loaded so this command is useless
    return;
  }
...
void CL_AdjustAngles(void) {
  ...
  if (!in_strafe.active) {
    if (cl_mYawOverride) {
      if (cl_mSensitivityOverride) {
        cl.viewangles[YAW] -= cl_mYawOverride * cl_mSensitivityOverride *
        ...
        cl.viewangles[YAW] -= cl_mYawOverride * OVERRIDE_MOUSE_SENSITIVITY *
                              speed * cl_yawspeed->value *
                              CL_KeyState(&in_right);
        cl.viewangles[YAW] += cl_mYawOverride * OVERRIDE_MOUSE_SENSITIVITY *
                              speed * cl_yawspeed->value *
  ...
  if (cl_mPitchOverride) {
    if (cl_mSensitivityOverride) {
      cl.viewangles[PITCH] -= cl_mPitchOverride * cl_mSensitivityOverride *
...
void CL_MouseEvent(int dx, int dy, int time) {
  if (g_clAutoMapMode && cls.cgameStarted) { // automap input
    autoMapInput_t *data = (autoMapInput_t *)cl.mSharedMemory;
...
void CL_JoystickMove(usercmd_t *cmd) {
  ...
  if (!in_strafe.active) {
    if (cl_mYawOverride) {
      if (cl_mSensitivityOverride) {
        cl.viewangles[YAW] += cl_mYawOverride * cl_mSensitivityOverride *
                              cl.joystickAxis[AXIS_SIDE] / 2.0f;
      } else {
        cl.viewangles[YAW] += cl_mYawOverride * OVERRIDE_MOUSE_SENSITIVITY *
                              cl.joystickAxis[AXIS_SIDE] / 2.0f;
      }
    } else {
      cl.viewangles[YAW] += anglespeed * (cl_yawspeed->value / 100.0f) *
  ...
  if (in_mlooking || cl_freelook->integer) {
    if (cl_mPitchOverride) {
      if (cl_mSensitivityOverride) {
        cl.viewangles[PITCH] += cl_mPitchOverride * cl_mSensitivityOverride *
                                cl.joystickAxis[AXIS_FORWARD] / 2.0f;
      } else {
        cl.viewangles[PITCH] += cl_mPitchOverride * OVERRIDE_MOUSE_SENSITIVITY *
                                cl.joystickAxis[AXIS_FORWARD] / 2.0f;
      }
    } else {
      cl.viewangles[PITCH] += anglespeed * (cl_pitchspeed->value / 100.0f) *
...
void CL_MouseMove(usercmd_t *cmd) {
  ...
  if (cl_mouseAccel->value != 0.0f) {
    if (cl_mouseAccelStyle->integer == 0) {
      ...
      if (cl_mYawOverride || cl_mPitchOverride) { // FIXME: different people
                                                  // have different speed mouses,
        if (cl_mSensitivityOverride) {
      ...
      // doubled the non accelerated amplification NOTE: decouple the config
      // cvars for independent acceleration setup along X and Y?
      ...
      if (cl_mYawOverride || cl_mPitchOverride) { // FIXME: different people
        ...
        if (cl_mSensitivityOverride) {
          ...
          mx = cl_mSensitivityOverride *
               (mx +
                ((mx < 0) ? -power[0] : power[0]) * cl_mouseAccelOffset->value);
          my = cl_mSensitivityOverride *
               (my +
                ((my < 0) ? -power[1] : power[1]) * cl_mouseAccelOffset->value);
        } else {
    ...
    if (cl_mYawOverride || cl_mPitchOverride) { // FIXME: different people have
                                                // different speed mouses,
      if (cl_mSensitivityOverride) {
        // this will fuck things up for them, need to clamp
        // max input?
        mx *= cl_mSensitivityOverride;
  ...
  if ((in_mlooking || cl_freelook->integer) && !in_strafe.active) {
    ...
    const float pitch =
        cl_bUseFighterPitch ? m_pitchVeh->value : m_pitch->value;
    if (cl_mPitchOverride) {
      if (pitch > 0)
...
qboolean CL_NoUseableForce(void) {
  if (!cls.cgameStarted) { // ahh, no cgame loaded
    return qfalse;
  }
...
void CL_CmdButtons(usercmd_t *cmd) {
  ...
  if (cmd->buttons & BUTTON_FORCEPOWER) { // check for transferring a use force
                                          // to a use inventory...
    if ((cmd->buttons & BUTTON_USE) ||
        CL_NoUseableForce()) { // it's pushed, remap it!
...
void CL_WritePacket(void) {
  ...
  if (count >= 1) {
    ...
    if (cl.gcmdSentValue) { // hmm, just clear here, I guess.. hoping it will
                            // resolve issues with gencmd values sometimes not
                            // going through.
      cl.gcmdSendValue = qfalse;
...
============
*/
void CL_ShutdownInput(void) { Cmd_RemoveCommandList(inputCmds); }
