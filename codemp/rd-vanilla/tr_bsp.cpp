...
static void R_LoadLightmaps(lump_t *l, const char *psMapName, world_t &worldData) {
	...
	for (i = 0; i < tr.numLightmaps; i++) {
		...
		tr.lightmaps[i] = R_CreateImage(va("*%s/lightmap%d", sMapName, i), image, LIGHTMAP_SIZE, LIGHTMAP_SIZE, GL_RGBA, qfalse, qfalse,
...
static void R_LoadFogs(lump_t *l, lump_t *brushesLump, lump_t *sidesLump, world_t &worldData, int index) {
	...
	for (i = 0; i < count; i++, fogs++) {
		...
		out->colorInt =
			ColorBytes4(out->parms.color[0] * tr.identityLight, out->parms.color[1] * tr.identityLight, out->parms.color[2] * tr.identityLight, 1.0);
