...
void target_counter_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
	...
	if (activator) {
		Q3_DebugPrint(WL_VERBOSE, "target_counter %s used by %s (%d/%d)\n", self->targetname, activator->targetname, (self->max_health - self->count),
					  self->max_health);
	}
	...
	*/
	...
	if (self->behaviorSet[BSET_USE]) {
		if (self->spawnflags & 1) {
			...
			if (!self->activator->sequencer || !self->activator->taskManager) { // Need to be initialized through ICARUS
