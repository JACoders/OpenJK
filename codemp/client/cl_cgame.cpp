...
void CL_ConfigstringModified(void) {
  ...
  if (cl_autolodscale && cl_autolodscale->integer) {
    if (index >= CS_PLAYERS &&
        index < CS_G2BONES) { // this means that a client was updated in some
                              // way. Go through and count the clients.
      int clientCount = 0;
      i = CS_PLAYERS;
      ...
      if (str[i + 1] == '@' && (i + 2) < strLen) {
        if (str[i + 2] == '@' &&
          ...
          Q_strcat(buf, MAX_STRINGED_SV_STRING,
                   SE_GetString("MP_SVGAME", stringRef));
          b = strlen(buf);
        }
      }
    }
...
void CL_InitCGame(void) {
  ...
  CGVM_Init(clc.serverMessageSequence, clc.lastExecutedServerCommand,
            clc.clientNum);

  int clRate = Cvar_VariableIntegerValue("rate");
  if (clRate == 4000) {
    Com_Printf(S_COLOR_YELLOW
               "WARNING: Old default /rate value detected (4000). Suggest "
               "typing /rate 25000 into console for a smoother connection!\n");
  }
...
void CL_CGameRendering(stereoFrame_t stereo) {
  // rww - RAGDOLL_BEGIN
  if (!com_sv_running->integer) { // set the server time to match the client
                                  // time, if we don't have a server going.
