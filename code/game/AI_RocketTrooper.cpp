...
void RT_Precache(void) {
  ...
  G_EffectIndex(
      "rockettrooper/light_cone"); // extern this?  At least use a different one
}
      ...
      if (NPC->client->ps.weapon == WP_NONE) {
            ...
            enemyDist < MIN_ROCKET_DIST_SQUARED) // 128*128
        {
          ...
          if (hit == NPC->enemy->s.number ||
            ...
            // at enemy
            VectorCopy(NPC->enemy->currentOrigin,
    ...
    if (NPC->client->fireDelay) {
      if (NPC->s.weapon == WP_ROCKET_LAUNCHER ||
        ...
        if (!enemyLOS || !enemyCS) { // cancel it
          ...
          TIMER_Set(NPC, "nextAttackDelay",
                    Q_irand(1000, 3000)); // FIXME: base on g_spskill
        }
      }
      ...
      if (TIMER_Done(NPC, "nextAttackDelay")) {
        ...
        if (NPC->s.weapon == WP_ROCKET_LAUNCHER) {
          if ((ucmd.buttons & BUTTON_ATTACK) &&
              !Q_irand(
                  0, altChance)) { // every now and then, shoot a homing rocket
            ucmd.buttons &= ~BUTTON_ATTACK;
            ucmd.buttons |= BUTTON_ALT_ATTACK;
            NPC->client->fireDelay =
                Q_irand(1000, 3000); // FIXME: base on g_spskill
          }
        } else if (NPC->s.weapon == WP_CONCUSSION) {
          if ((ucmd.buttons & BUTTON_ATTACK) &&
              Q_irand(0, altChance * 5)) { // fire the beam shot
            ucmd.buttons &= ~BUTTON_ATTACK;
            ucmd.buttons |= BUTTON_ALT_ATTACK;
            TIMER_Set(NPC, "nextAttackDelay",
                      Q_irand(1500, 2500)); // FIXME: base on g_spskill
          } else {                          // fire the rocket-like shot
            TIMER_Set(NPC, "nextAttackDelay",
                      Q_irand(3000, 5000)); // FIXME: base on g_spskill
          }
        }
      }
    }
...
void RT_FlyStart(gentity_t *self) { // switch to seeker AI for a while
  if (TIMER_Done(self, "jetRecharge") && !RT_Flying(self)) {
    ...
    if (self->genericBolt1 != -1) {
      G_PlayEffect(G_EffectIndex("rockettrooper/flameNEW"), self->playerModel,
                   self->genericBolt1, self->s.number, self->currentOrigin,
                   qtrue, qtrue);
    }
    if (self->genericBolt2 != -1) {
      G_PlayEffect(G_EffectIndex("rockettrooper/flameNEW"), self->playerModel,
                   self->genericBolt2, self->s.number, self->currentOrigin,
                   qtrue, qtrue);
    }
...
void RT_FlyStop(gentity_t *self) {
  ...
  if (self->genericBolt1 != -1) {
    G_StopEffect("rockettrooper/flameNEW", self->playerModel,
                 self->genericBolt1, self->s.number);
  }
  if (self->genericBolt2 != -1) {
    G_StopEffect("rockettrooper/flameNEW", self->playerModel,
                 self->genericBolt2, self->s.number);
  }
  // stop jet loop sound
  self->s.loopSound = 0;
...
void RT_JetPackEffect(int duration) {
  if (NPC->genericBolt1 != -1) {
    G_PlayEffect(G_EffectIndex("rockettrooper/flameNEW"), NPC->playerModel,
                 NPC->genericBolt1, NPC->s.number, NPC->currentOrigin, duration,
                 qtrue);
  }
  if (NPC->genericBolt2 != -1) {
    G_PlayEffect(G_EffectIndex("rockettrooper/flameNEW"), NPC->playerModel,
                 NPC->genericBolt2, NPC->s.number, NPC->currentOrigin, duration,
                 qtrue);
  }
...
void RT_Flying_MaintainHeight(void) {
  ...
  if (NPC->forcePushTime >
      level.time) { // if being pushed, we don't have control over our movement
    return;
  }

  if ((NPC->client->ps.pm_flags &
       PMF_TIME_KNOCKBACK)) { // don't slow down for a bit
  ...
  */
  // If we have an enemy, we should try to hover at or a little below enemy eye
  // level
  if (NPC->enemy &&
    ...
    if (TIMER_Done(NPC, "heightChange")) {
      ...
      if (NPC->enemy->client &&
          ...
          (NPC->enemy->client->ps.forcePowersActive &
           (1 << FP_LEVITATION))) { // so we don't go up when they force jump up
                                    // at us
      ...
      dif = (enemyZHeight +
             Q_flrand(NPC->enemy->maxs[2] / 2, NPC->enemy->maxs[2] + 8)) -
            NPC->currentOrigin[2];
    ...
    } else { // don't get too far away from height of enemy...
      float enemyZHeight = NPC->enemy->currentOrigin[2];
      if (NPC->enemy->client &&
          NPC->enemy->client->ps.groundEntityNum == ENTITYNUM_NONE &&
          (NPC->enemy->client->ps.forcePowersActive &
      ...
      float maxHeight = 200;
      float hDist =
          DistanceHorizontal(NPC->enemy->currentOrigin, NPC->currentOrigin);
      if (hDist < 512) {
        maxHeight *= hDist / 512;
      }
      if (dif > maxHeight) {
        if (NPC->client->ps.velocity[2] >
            0) // FIXME: or: we can't see him anymore
        {      // slow down
          if (NPC->client->ps.velocity[2]) {
          ...
          NPC->client->ps.velocity[2] -= 4;
                 ...
                 NPC->client->ps.velocity[2] < 0) // we're way below him
      {
        if (NPC->client->ps.velocity[2] <
            0) // FIXME: or: we can't see him anymore
        {      // slow down
    ...
    } else if (VectorCompare(NPC->pos1,
                             vec3_origin)) { // have a starting position as a
                                             // reference point
...
void RT_Flying_Strafe(void) {
  ...
  if (Q_flrand(0.0f, 1.0f) > 0.7f || !NPC->enemy || !NPC->enemy->client) {
    ...
    // Pick a random strafe direction, then check to see if doing a strafe would
    // be
    //	reasonably valid
    side = (rand() & 1) ? -1 : 1;
    VectorMA(NPC->currentOrigin, RT_FLYING_STRAFE_DIS * side, right, end);

    gi.trace(&tr, NPC->currentOrigin, NULL, NULL, end, NPC->s.number,
             MASK_SOLID, (EG2_Collision)0, 0);

    // Close enough
    if (tr.fraction > 0.9f) {
      float vel = RT_FLYING_STRAFE_VEL + Q_flrand(-20, 20);
      VectorMA(NPC->client->ps.velocity, vel * side, right,
...
void RT_Flying_Hunt(qboolean visible, qboolean advance) {
  ...
  speed = RT_FLYING_FORWARD_BASE_SPEED +
          RT_FLYING_FORWARD_MULTIPLIER * g_spskill->integer;
  if (advance && distance < Q_flrand(256, 3096)) {
    NPC->delay = 0;
    VectorMA(NPC->client->ps.velocity, speed, forward,
             NPC->client->ps.velocity);
  } else if (distance < Q_flrand(0, 128)) {
    if (NPC->health <= 50) { // always back off
      NPC->delay = 0;
    } else if (!TIMER_Done(
                   NPC,
                   "backoffTime")) { // still backing off from end of last delay
      NPC->delay = 0;
    } else if (!NPC->delay) { // start a new delay
      NPC->delay = Q_irand(0, 10 + (20 * (2 - g_spskill->integer)));
    } else { // continue the current delay
      NPC->delay--;
    }
    if (!NPC->delay) { // delay done, now back off for a few seconds!
      TIMER_Set(NPC, "backoffTime", Q_irand(2000, 5000));
      VectorMA(NPC->client->ps.velocity, speed * -2, forward,
               NPC->client->ps.velocity);
    }
  } else {
    NPC->delay = 0;
...
void RT_Flying_Think(void) {
  if (Q3_TaskIDPending(NPC, TID_MOVE_NAV) &&
      UpdateGoal()) { // being scripted to go to a certain spot, don't maintain
                      // height
    if (NPC_MoveToGoal(qtrue)) { // we could macro-nav to our goal
      if (NPC->enemy && NPC->enemy->health && NPC->enemy->inuse) {
        NPC_FaceEnemy(qtrue);
        RT_FireDecide();
      }
    } else { // frick, no where to nav to, keep us in the air!
      RT_Flying_MaintainHeight();
    }
    return;
  }
  ...
  if (NPC->random == 0.0f) {
    // used to offset seekers around a circle so they don't occupy the same
    // spot.  This is not a fool-proof method.
...
void NPC_BSRT_Default(void) {
  ...
  if (NPC->client->ps.groundEntityNum != ENTITYNUM_NONE) {
    ...
    {                             // officers always stay in the air
      ...
      NPC->NPC->aiFlags |=
          NPCAI_FLY; // fixme also, Inform NPC_HandleAIFlags we want to fly
    }
  }

  if (RT_Flying(NPC)) { // FIXME: only officers need do this, right?
    RT_Flying_Think();
  } else if (NPC->enemy != NULL) { // rocketrooper on ground with enemy
    UpdateGoal();
    RT_RunStormtrooperAI();
    RT_CheckJump();
    // NPC_BSST_Default();//FIXME: add missile avoidance
    // RT_Hunt();//NPC_BehaviorSet_Jedi( bState );
  } else { // shouldn't have gotten in here
    RT_RunStormtrooperAI();
    // NPC_BSST_Patrol();
  }
}