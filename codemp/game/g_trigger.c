...
void TimerStop(gentity_t *trigger, gentity_t *player, trace_t *trace) {//JAPRO Timers
	...
	if (player->client->pers.stats.startTime) {
		...
		if (!valid) {
			PrintRaceTime(NULL, playerName, trigger->message, styleStr, (int)floorf(player->client->pers.stats.topSpeed + 0.5f), average, timeStr, player->client->ps.clientNum, qfalse, qfalse, qfalse, qfalse, qfalse, 0, 0);
		}
		else {
			...
			if (player->client->pers.userName[0]) { //omg
				...
				PrintRaceTime(NULL, playerName, trigger->message, styleStr, (int)floorf(player->client->pers.stats.topSpeed + 0.5f), average, timeStr, player->client->ps.clientNum, qfalse, qfalse, qfalse, qfalse, qtrue, 0, 0);
			}
		}
