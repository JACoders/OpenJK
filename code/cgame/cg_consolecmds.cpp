...
void CG_WriteCam_f(void) {
  ...
  Com_sprintf(text, sizeof(text),
              ...
              "\"ref_tag\"\n\"targetname\"	\"%s\"\n\"origin\" \"%i %i "
              "%i\"\n\"angles\" \"%i %i %i\"\n\"fov\" \"%i\"\n}\n",
              numCams, targetname, (int)cg.refdef.vieworg[0],
              (int)cg.refdef.vieworg[1], (int)cg.refdef.vieworg[2],
              (int)cg.refdefViewAngles[0], (int)cg.refdefViewAngles[1],
              (int)cg.refdefViewAngles[2], cg_fov.integer);
  gi.WriteCam(text);
}
...
void CG_ToggleBinoculars(void) {
  ...
  if (cg.zoomMode == 0 ||
      cg.zoomMode >=
          2) // not zoomed or currently zoomed with the disruptor or LA goggles
  {
    if ((cg.snap->ps.saber[0].Active() && cg.snap->ps.saberInFlight) ||
        cg.snap->ps.stats[STAT_HEALTH] <=
            0) { // can't select binoculars when throwing saber
      // FIXME: indicate this to the player
      return;
    }

    if (cg.snap->ps.viewEntity ||
        (cg_entities[cg.snap->ps.clientNum].currentState.eFlags &
         (EF_LOCKED_TO_WEAPON | EF_IN_ATST))) {
      // can't zoom when you have a viewEntity or driving an atst or in an
      // emplaced gun
      return;
    }
    ...
    if (cg.weaponSelect == WP_NONE &&
        cg.snap->ps.stats[STAT_WEAPONS] & (1 << WP_SABER)) {
...
void CG_ToggleLAGoggles(void) {
  ...
  if (cg.zoomMode == 0 ||
      cg.zoomMode < 3) // not zoomed or currently zoomed with the disruptor or
                       // regular binoculars
  {
    if ((cg.snap->ps.saber[0].Active() && cg.snap->ps.saberInFlight) ||
        cg.snap->ps.stats[STAT_HEALTH] <=
...
static consoleCommand_t commands[] = {
    ...
    {"cam_enable",
     CGCam_Enable}, // gets into camera mode for precise camera placement
...
qboolean CG_ConsoleCommand(void) {
  ...
  command = (consoleCommand_t *)Q_LinearSearch(
      CG_Argv(0), commands, numCommands, sizeof(commands[0]), cmdcmp);
