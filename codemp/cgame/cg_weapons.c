...
void CG_RegisterItemVisuals(int itemNum) {
  ...
  if (item->giType == IT_TEAM &&
      ...
      cgs.gametype == GT_CTY) { // in CTY the flag model is different
  ...
  Ghoul2 Insert Start
  */
  if (!Q_stricmp(&item->world_model[0][strlen(item->world_model[0]) - 4],
                 ".glm")) {
    handle = trap->G2API_InitGhoul2Model(&itemInfo->g2Models[0],
                                         item->world_model[0], 0, 0, 0, 0, 0);
    if (handle < 0) {
  ...
  */
  if (item->icon) {
    if (item->giType ==
        IT_HEALTH) { // medpack gets nomip'd by the ui or something I guess.
...
static int CG_MapTorsoToWeaponFrame(clientInfo_t *ci, int frame, int animNum) {
  ...
  if (cg.snap->ps.forceHandExtend != HANDEXTEND_NONE ||
      cgWeapFrameTime > cg.time) {
    // the reason for the after delay is so that it doesn't snap the weapon
    // frame to the "idle" (0) frame for a very quick moment
    if (cgWeapFrame < 6) {
      cgWeapFrame = 6;
    ...
    else if (cg.snap->ps.forceHandExtend != HANDEXTEND_NONE &&
             cgWeapFrame == 10)
  ...
  case TORSO_DROPWEAP1:
    if (frame >= animations[animNum].firstFrame &&
        frame < animations[animNum].firstFrame + 5) {
  ...
  case TORSO_RAISEWEAP1:
    if (frame >= animations[animNum].firstFrame &&
        frame < animations[animNum].firstFrame + 4) {
  ...
  case BOTH_THERMAL_THROW:
    if (frame >= animations[animNum].firstFrame &&
        frame < animations[animNum].firstFrame + 6) {
...
static void CG_LightningBolt(centity_t *cent, vec3_t origin) {
  ...
  if (cent->currentState.weapon == WP_DEMP2 &&
      cent->currentState.eFlags & EF_ALT_FIRING) { /*nothing*/
  } else {
    return;
  }
  ...
  && (cg_trueLightning.value != 0)) { vec3_t angle; int i;

                  for (i = 0; i < 3; i++) {
                          float a = cent->lerpAngles[i] -
  cg.refdef.viewangles[i]; if (a > 180) { a -= 360;
                          }
                          if (a < -180) {
                                  a += 360;
                          }

                          angle[i] = cg.refdef.viewangles[i] + a * (1.0 -
  cg_trueLightning.value); if (angle[i] < 0) { angle[i] += 360;
                          }
                          if (angle[i] > 360) {
                                  angle[i] -= 360;
                          }
...
static void CG_AddWeaponWithPowerups(refEntity_t *gun, int powerups) {
  ...
  if (cg.predictedPlayerState.electrifyTime >
      cg.time) { // add electrocution shell
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent,
                        int team, vec3_t newAngles, qboolean thirdPerson) {
  refEntity_t gun;
  refEntity_t barrel;
  vec3_t angles;
  weapon_t weaponNum;
  ...
  if (cg.predictedPlayerState.pm_type == PM_SPECTATOR &&
      cent->currentState.number ==
          cg.predictedPlayerState
              .clientNum) { // spectator mode, don't draw it...
    return;
  }
  ...
  */
  ...
  // only do this if we are in first person, since world weapons are now handled
  // on the server by Ghoul2
  if (!thirdPerson) {
    ...
    if (ps) { // this player, in first person view
    ...
    if (!ps) {
      ...
      if ((cent->currentState.eFlags & EF_FIRING) && weapon->firingSound) {
        // lightning gun and gauntlet make a different sound when fire is held
        // down
        trap->S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
    ...
    if (!CG_IsMindTricked(cent->currentState.trickedentindex,
      ...
      CG_AddWeaponWithPowerups(
          &gun,
          cent->currentState
              .powerups); // don't draw the weapon if the player is invisible
      /*
      ...
      while (i < 3) {
        ...
        if (i == 0) {
          CG_PositionRotatedEntityOnTag(
              &barrel, parent /*&gun*/,
              /*weapon->weaponModel*/ weapon->handsModel, "tag_barrel");
        } else if (i == 1) {
          CG_PositionRotatedEntityOnTag(
              &barrel, parent /*&gun*/,
              /*weapon->weaponModel*/ weapon->handsModel, "tag_barrel2");
        } else {
          CG_PositionRotatedEntityOnTag(
              &barrel, parent /*&gun*/,
              /*weapon->weaponModel*/ weapon->handsModel, "tag_barrel3");
        }
  ...
  */
  ...
  if ((ps || cg.renderingThirdPerson ||
      ...
      ((cent->currentState.modelindex2 == WEAPON_CHARGING_ALT &&
    ...
    if (!thirdPerson) {
      ...
      if (!trap->G2API_HasGhoul2ModelOnIndex(
              &(cent->ghoul2), 1)) { // it's quite possible that we may have
                                     // have no weapon model and be in a valid
                                     // state, so return here if this is the case
        return;
      }

      // go away and get me the bolt position for this frame please
      if (!(trap->G2API_GetBoltMatrix(
              cent->ghoul2, 1, 0, &boltMatrix, newAngles, cent->lerpOrigin,
              cg.time, cgs.gameModels,
              cent->modelScale))) { // Couldn't find bolt point.
        return;
      }
    ...
    // FX_AddSprite( flash.origin, NULL, NULL, 3.0f * val, 0.0f, 0.7f, 0.7f,
  ...
  if (ps || cg.renderingThirdPerson ||
      cent->currentState.number !=
          cg.predictedPlayerState
              .clientNum) { // Make sure we don't do the thirdperson model
                            // effects for the local player if we're in first
                            // person
    vec3_t flashorigin, flashdir;
    refEntity_t flash;
    ...
    if (cg.time - cent->muzzleFlashTime <=
        MUZZLE_FLASH_TIME + 10) { // Handle muzzle flashes
      if (cent->currentState.eFlags &
          EF_ALT_FIRING) { // Check the alt firing first.
        if (weapon->altMuzzleEffect) {
          if (!thirdPerson) {
            trap->FX_PlayEntityEffectID(weapon->altMuzzleEffect, flashorigin,
                                        flash.axis, -1, -1, -1, -1);
          } else {
            trap->FX_PlayEffectID(weapon->altMuzzleEffect, flashorigin,
                                  flashdir, -1, -1, qfalse);
          }
        }
      } else { // Regular firing
        if (weapon->muzzleEffect) {
          if (!thirdPerson) {
            trap->FX_PlayEntityEffectID(weapon->muzzleEffect, flashorigin,
                                        flash.axis, -1, -1, -1, -1);
          } else {
            trap->FX_PlayEffectID(weapon->muzzleEffect, flashorigin, flashdir,
                                  -1, -1, qfalse);
          }
        }
      }
    ...
    if (weapon->flashDlightColor[0] || weapon->flashDlightColor[1] ||
      ...
      trap->R_AddLightToScene(
          flashorigin, 300 + (rand() & 31), weapon->flashDlightColor[0],
...
void CG_AddViewWeapon(playerState_t *ps) {
  ...
  VectorMA(hand.origin, (cg_gunZ.value + fovOffset), cg.refdef.viewaxis[2],
           hand.origin);
  ...
  if (cg_fovViewmodel.integer) {
    ...
    float fracWeapFOV =
        (1.0f / fracDistFOV) * tanf(cgFov * (M_PI / 180) * 0.5f);
  ...
  if (cg_debugGun.integer) {
    ...
    trap->G2API_GetBoneFrame(cent->ghoul2, "lower_lumbar", cg.time,
                             &currentFrame, cgs.gameModels, 0);
    hand.frame =
        CG_MapTorsoToWeaponFrame(ci, ceil(currentFrame), ps->torsoAnim);
    hand.oldframe =
        CG_MapTorsoToWeaponFrame(ci, floor(currentFrame), ps->torsoAnim);
  ...
  CG_AddPlayerWeapon(&hand, ps, &cg_entities[cg.predictedPlayerState.clientNum],
                     ps->persistant[PERS_TEAM], angles, qfalse);
}
...
void CG_DrawIconBackground(void) {
  ...
  //	int				drawType =
  //cgs.media.weaponIconBackground; 	int				yOffset = 0;
  ...
  if ((cg.iconSelectTime + WEAPON_SELECT_TIME) <
      cg.time) // Time is up for the HUD to display
  {
    if (cg.iconHUDActive) // The time is up, but we still need to move the
                          // prongs back to their original position
    {
      ...
      // CG_DrawPic( x2+60, y2+30+yOffset, 460, -height, drawType);	// Top
      // half
      // CG_DrawPic( x2+60, y2+30-2+yOffset, 460, height, drawType);	//
      // Bottom half
  ...
  // CG_DrawPic( x2+60, y2+30-2+yOffset, 460, height, drawType);	// Bottom
  // half
...
qboolean CG_WeaponCheck(int weap) {
  if (cg.snap->ps.ammo[weaponData[weap].ammoIndex] <
          weaponData[weap].energyPerShot &&
      cg.snap->ps.ammo[weaponData[weap].ammoIndex] <
          weaponData[weap].altEnergyPerShot) {
    return qfalse;
  }
...
static qboolean CG_WeaponSelectable(int i) {
  ...
  if (cg.predictedPlayerState.ammo[weaponData[i].ammoIndex] <
          weaponData[i].energyPerShot &&
      cg.predictedPlayerState.ammo[weaponData[i].ammoIndex] <
          weaponData[i].altEnergyPerShot) {
    return qfalse;
  }

  if (i == WP_DET_PACK &&
      cg.predictedPlayerState.ammo[weaponData[i].ammoIndex] < 1 &&
...
void CG_DrawWeaponSelect(void) {
  ...
  if (cg.predictedPlayerState.emplacedIndex) { // can't cycle when on a weapon
  ...
  if (!CG_WeaponSelectable(cg.weaponSelect) &&
      (cg.weaponSelect == WP_THERMAL ||
       cg.weaponSelect ==
           WP_TRIP_MINE)) { // display this weapon that we don't actually "have"
                            // as unhighlighted until it's deselected since it's
                            // selected we must increase the count to display the
                            // proper number of valid selectable weapons
    count++;
  }
  ...
  for (iconCnt = 1; iconCnt < (sideLeftIconCnt + 1); i--) {
    if (i == WP_CONCUSSION) {
      i--;
    } else if (i == WP_FLECHETTE && !drewConc &&
    ...
    if (!CG_WeaponSelectable(i) &&
        (i == WP_THERMAL || i == WP_TRIP_MINE)) { // Don't show thermal and
                                                  // tripmine when out of them
      continue;
    }
    ...
    if (cgs.media.weaponIcons[i]) {
      ...
      if (!CG_WeaponCheck(i)) {
        CG_DrawPic(
            holdX, y + 10 + yOffset, smallIconSize, smallIconSize,
            /*weaponInfo->weaponIconNoAmmo*/ cgs.media.weaponIcons_NA[i]);
      } else {
        CG_DrawPic(holdX, y + 10 + yOffset, smallIconSize, smallIconSize,
                   /*weaponInfo->weaponIcon*/ cgs.media.weaponIcons[i]);
      }
  ...
  for (iconCnt = 1; iconCnt < (sideRightIconCnt + 1); i++) {
    if (i == WP_CONCUSSION) {
      i++;
    } else if (i == WP_ROCKET_LAUNCHER && !drewConc &&
               cg.weaponSelect != WP_CONCUSSION) {
      i = WP_CONCUSSION;
    }
    if (i > LAST_USEABLE_WEAPON) {
  ...
  if (cg_weapons[cg.weaponSelect].item) {
    ...
    if (trap->SE_GetStringTextString(va("SP_INGAME_%s", Q_strupr(upperKey)),
                                     text, sizeof(text))) {
      CG_DrawProportionalString(320, y + 45 + yOffset, text,
                                UI_CENTER | UI_SMALLFONT, textColor);
    } else {
      CG_DrawProportionalString(320, y + 45 + yOffset,
...
void CG_Weapon_f(void) {
  ...
  if (num > WP_STUN_BATON) {
    // num++;
    num += 2; // I suppose this is getting kind of crazy, what with the wp_melee
              // in there too now.
  } else {
  ...
  if (num > LAST_USEABLE_WEAPON + 1) { // other weapons are off limits due to
                                       // not actually being weapon weapons
    return;
  }
  ...
  if (!(cg.snap->ps.stats[STAT_WEAPONS] & (1 << num))) {
    if (num == WP_SABER) { // don't have saber, try melee on the same slot
      num = WP_MELEE;
...
void CG_OutOfAmmoChange(int oldWeapon) {
  ...
  for (i = LAST_USEABLE_WEAPON; i > 0;
       i--) // We don't want the emplaced or turret
  {
    if (CG_WeaponSelectable(i)) {
      /*
      if ( 1 == cg_autoswitch.integer &&
              ( i == WP_TRIP_MINE || i == WP_DET_PACK || i == WP_THERMAL || i ==
      WP_ROCKET_LAUNCHER) ) // safe weapon switch
      */
      // rww - Don't we want to make sure i != one of these if autoswitch is 1
      // (safe)?
      if (cg_autoSwitch.integer != 1 ||
          (i != WP_TRIP_MINE && i != WP_DET_PACK && i != WP_THERMAL &&
           i != WP_ROCKET_LAUNCHER)) {
        if (i != oldWeapon) { // don't even do anything if we're just selecting
                              // the weapon we already have/had
          cg.weaponSelect = i;
          break;
        }
      }
...
void CG_GetClientWeaponMuzzleBoltPoint(int clIndex, vec3_t to) {
  ...
  trap->G2API_GetBoltMatrix(cent->ghoul2, 1, 0, &boltMatrix, cent->turAngles,
                            cent->lerpOrigin, cg.time, cgs.gameModels,
                            cent->modelScale);
...
void CG_FireWeapon(centity_t *cent, qboolean altFire) {
  ...
  if (cg.predictedPlayerState.clientNum == cent->currentState.number) {
    if ((ent->weapon == WP_BRYAR_PISTOL && altFire) ||
      ...
      if (ent->weapon == WP_CONCUSSION) {
        if (!cg.renderingThirdPerson) // gives an advantage to being in 3rd
  ...
  if (cent->currentState.powerups & (1 << PW_QUAD)) {
    // trap->S_StartSound (NULL, cent->currentState.number, CHAN_ITEM,
    // cgs.media.quadSound );
  }
#endif // BASE_COMPAT
  ...
  if (altFire) {
    ...
    if (c > 0) {
      c = rand() % c;
      if (weap->altFlashSound[c]) {
        trap->S_StartSound(NULL, ent->number, CHAN_WEAPON,
                           weap->altFlashSound[c]);
      }
    }
    //		if ( weap->altFlashSnd )
    //		{
...
qboolean CG_VehicleWeaponImpact(
    centity_t
        *cent) { // see if this is a missile entity that's owned by a vehicle
                 // and should do a special, overridden impact effect
  if ((cent->currentState.eFlags &
       EF_JETPACK_ACTIVE) // hack so we know we're a vehicle Weapon shot
      && cent->currentState.otherEntityNum2 &&
      g_vehWeaponInfo[cent->currentState.otherEntityNum2]
          .iImpactFX) { // missile is from a special vehWeapon
    vec3_t normal;
    ...
    trap->FX_PlayEffectID(
        ...
        cent->lerpOrigin, normal, -1, -1, qfalse);
    return qtrue;
  }
  return qfalse;
}
...
void CG_MissileHitWall(int weapon, int clientNum, vec3_t origin, vec3_t dir,
                       impactSound_t soundType, qboolean altFire, int charge) {
  int parm;
  vec3_t up = {0, 0, 1};
  ...
  case WP_DEMP2:
    if (altFire) {
      trap->FX_PlayEffectID(cgs.effects.mAltDetonate, origin, dir, -1, -1,
                            qfalse);
    } else {
  ...
  case WP_THERMAL:
    trap->FX_PlayEffectID(cgs.effects.thermalExplosionEffect, origin, dir, -1,
                          -1, qfalse);
    trap->FX_PlayEffectID(cgs.effects.thermalShockwaveEffect, origin, up, -1,
                          -1, qfalse);
    break;
...
qboolean CG_CalcMuzzlePoint(int entityNum, vec3_t muzzle) {
  ...
  if (entityNum ==
      cg.snap->ps.clientNum) { // I'm not exactly sure why we'd be rendering
                               // someone else's crosshair, but hey.
    ...
    if (weapontype == WP_DISRUPTOR || weapontype == WP_STUN_BATON ||
        weapontype == WP_MELEE || weapontype == WP_SABER) {
...
// create one instance of all the weapons we are going to use so we can just
// copy this info into each clients gun ghoul2 object in fast way
...
void CG_InitG2Weapons(void) {
  ...
  for (item = bg_itemlist + 1; item->classname; item++) {
    if (item->giType == IT_WEAPON) {
      ...
      // initialise model
      trap->G2API_InitGhoul2Model(&g2WeaponInstances[/*i*/ item->giTag],
                                  item->world_model[0], 0, 0, 0, 0, 0);
      //			trap->G2API_InitGhoul2Model(&g2WeaponInstances[i],
      //item->world_model[0],G_ModelIndex( item->world_model[0] ) , 0, 0, 0, 0);
      if (g2WeaponInstances[/*i*/ item->giTag]) {
        ...
        if (item->giTag == WP_SABER) {
          trap->G2API_AddBolt(g2WeaponInstances[/*i*/ item->giTag], 0,
                              "*blade1");
        } else {
          trap->G2API_AddBolt(g2WeaponInstances[/*i*/ item->giTag], 0,
                              "*flash");
    ...
    if (weaponNum == WP_SABER) {
      ...
      if (!ci) {
        trap->G2API_CopySpecificGhoul2Model(
            CG_G2WeaponInstance(cent, weaponNum /*-1*/), 0, toGhoul2, 1);
      } else { // Try both the left hand saber and the right hand saber
        int i = 0;

        while (i < MAX_SABERS) {
          if (ci->saber[i].model[0] && ci->ghoul2Weapons[i]) {
            trap->G2API_CopySpecificGhoul2Model(ci->ghoul2Weapons[i], 0,
                                                toGhoul2, i + 1);
          } else if (ci->ghoul2Weapons[i]) { // if the second saber has been
                                             // removed, then be sure to remove
                                             // it and free the instance.
            qboolean g2HasSecondSaber =
                trap->G2API_HasGhoul2ModelOnIndex(&(toGhoul2), 2);

            if (g2HasSecondSaber) { // remove it now since we're switching away
                                    // from sabers
      ...
      if (g2HasSecondSaber) { // remove it now since we're switching away from
                              // sabers
        trap->G2API_RemoveGhoul2Model(&(toGhoul2), 2);
      }

      if (weaponNum == WP_EMPLACED_GUN) { // a bit of a hack to remove gun model
                                          // when using an emplaced weap
        if (trap->G2API_HasGhoul2ModelOnIndex(&(toGhoul2), 1)) {
          trap->G2API_RemoveGhoul2Model(&(toGhoul2), 1);
        }
      } else if (weaponNum ==
                 WP_MELEE) { // don't want a weapon on the model for this one
        if (trap->G2API_HasGhoul2ModelOnIndex(&(toGhoul2), 1)) {
          trap->G2API_RemoveGhoul2Model(&(toGhoul2), 1);
        }
      } else {
...
void CG_CheckPlayerG2Weapons(playerState_t *ps, centity_t *cent) {
  ...
  if (cent->currentState.eFlags & EF_DEAD) { // no updating weapons when dead
    cent->ghoul2weapon = NULL;
    return;
  }

  if (cent->torsoBolt) { // got our limb cut off, no updating weapons until it's
                         // restored
    cent->ghoul2weapon = NULL;
    return;
  }
  ...
  if (cent->ghoul2 &&
      ...
      ps->clientNum ==
          cent->currentState.number) // don't want spectator mode forcing one
                                     // client's weapon instance over another's
  {
    CG_CopyG2WeaponInstance(cent, ps->weapon, cent->ghoul2);
    cent->ghoul2weapon = CG_G2WeaponInstance(cent, ps->weapon);
    if (cent->weapon == WP_SABER && cent->weapon != ps->weapon &&
        !ps->saberHolstered) { // switching away from the saber
      // trap->S_StartSound(cent->lerpOrigin, cent->currentState.number,
      // CHAN_AUTO, trap->S_RegisterSound(
      ...
      if (cgs.clientinfo[ps->clientNum].saber[1].soundOff &&
        ...
        trap->S_StartSound(cent->lerpOrigin, cent->currentState.number,
               ...
               !cent->saberWasInFlight) { // switching to the saber
      // trap->S_StartSound(cent->lerpOrigin, cent->currentState.number,
      // CHAN_AUTO, trap->S_RegisterSound( "sound/weapons/saber/saberon.wav" ));
