...
#define REFENTITYNUM_BITS                                                      \
  11 // can't be increased without changing drawsurf bit packing
...
#define RF_THIRD_PERSON                                                        \
  0x00002 // don't draw through eyes, only mirrors (player bodies, chat sprites)
#define RF_FIRST_PERSON                                                        \
  0x00004 // only draw through eyes (view weapon, damage blood blob)
...
#define RF_LIGHTING_ORIGIN                                                     \
  0x00080 // use refEntity->lightingOrigin instead of refEntity->origin
          // for lighting.  This allows entities to sink into the floor
          // with their origin going solid, and allows all parts of a
          // player to get the same lighting
#define RF_SHADOW_PLANE 0x00100 // use refEntity->shadowPlane
#define RF_WRAP_FRAMES                                                         \
  0x00200 // mod the model frames by the maxframes to allow continuous
...
#define RF_SHADOW_ONLY                                                         \
  0x01000 // add surfs for shadowing but don't draw them -rww
...
#define RF_DISINTEGRATE2                                                       \
  0x40000 // does a procedural hole-ripping thing with scaling at the ripping
          // point

#define RF_SETANIMINDEX                                                        \
  0x80000 // use backEnd.currentEntity->e.skinNum for R_BindAnimatedImage
...
typedef struct refEntity_s {
  ...
  // texturing
  union {
    //		int			skinNum;		// inline skin
    //index 		ivec3_t		terxelCoords;	// coords of patch for
    //RT_TERXELS
    struct {
      int miniStart;
      int miniCount;
  ...
  union {
    ...
    struct // that whole
           // put-the-opening-brace-on-the-same-line-as-the-beginning-of-the-definition
  ...
  */
  ...
  vec3_t modelScale; // axis scale for models
                     //	CGhoul2Info_v	*ghoul2;  		// has to be at the end of the
                     //ref-ent in order for it to be created properly
  void *ghoul2; // has to be at the end of the ref-ent in order for it to be
                // created properly
  /*
  Ghoul2 Insert End
  */
...
typedef struct model_s {
  ...
  md3Header_t *md3[MD3_MAX_LODS]; // only if type == MOD_MESH
                                  ...
                                  Ghoul2 Insert Start
                                  */
  mdxmHeader_t *mdxm; // only if type == MOD_GL2M which is a GHOUL II Mesh file
                      // NOT a GHOUL II animation file
  mdxaHeader_t
      *mdxa; // only if type == MOD_GL2A which is a GHOUL II Animation file
             /*
             Ghoul2 Insert End
             */
  int numLods;
