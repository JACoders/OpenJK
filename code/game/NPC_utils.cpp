...
void CalcEntitySpot(const gentity_t *ent, const spot_t spot, vec3_t point) {
  ...
  switch (spot) {
  case SPOT_ORIGIN:
    if (VectorCompare(ent->currentOrigin, vec3_origin)) { // brush
    ...
    if (ent->client && VectorLengthSquared(ent->client->renderInfo.eyePoint) &&
        (ent->client->ps.viewEntity <= 0 ||
         ent->client->ps.viewEntity >=
             ENTITYNUM_WORLD)) { // Actual tag_head eyespot!
      ...
      if (ent->NPC) { // always aim from the center of my bbox, so we don't
                      // wiggle when we lean forward or backwards
  ...
  case SPOT_WEAPON:
    if (ent->NPC && !VectorCompare(ent->NPC->shootAngles, vec3_origin) &&
        !VectorCompare(ent->NPC->shootAngles, ent->client->ps.viewangles)) {
  ...
  case SPOT_GROUND:
    ...
    gi.trace(&tr, start, ent->mins, ent->maxs, end, ent->s.number,
...
qboolean NPC_UpdateAngles(qboolean doPitch, qboolean doYaw) {
  ...
  // aimTime isn't even set anymore... so this code was never reached, but I
  // need a way to lock NPC's yaw, so instead of making a new SCF_ flag, just
  // use the existing render flag... - dmv
  if (!NPC->enemy && ((level.time < NPCInfo->aimTime) ||
    ...
    // we're changing the lockedDesired Pitch/Yaw below so it's lost it's
    // original meaning, get rid of the lock flag
    NPC->client->renderInfo.renderFlags &= ~RF_LOCKEDANGLE;
  ...
  if (NPC->s.weapon == WP_EMPLACED_GUN) {
    ...
    if (NPC->client->NPC_class == CLASS_ROCKETTROOPER &&
        !NPC->enemy) { // just slowly lookin' around
      yawSpeed = 1;
    } else {
  ...
  if (!NPC_IsTrooper(NPC) && NPC->enemy && !G_IsRidingVehicle(NPC) &&
      NPC->client->NPC_class != CLASS_VEHICLE) {
    if (NPC->s.weapon == WP_BLASTER_PISTOL || NPC->s.weapon == WP_BLASTER ||
        NPC->s.weapon == WP_BOWCASTER || NPC->s.weapon == WP_REPEATER ||
        NPC->s.weapon == WP_FLECHETTE || NPC->s.weapon == WP_BRYAR_PISTOL ||
  ...
  ucmd.angles[ROLL] = ANGLE2SHORT(NPC->client->ps.viewangles[ROLL]) -
                      client->ps.delta_angles[ROLL];
  ...
  if (doYaw) {
    ...
    if (fabs(error) > MIN_ANGLE_ERROR) {
      ...
      if(NPC->client->playerTeam == TEAM_BORG&&
              NPCInfo->behaviorState != BS_FACE&&NPCInfo->tempBehavior!=
      BS_FACE)
  ...
  if(doSound)
  {
          G_Sound(NPC, G_SoundIndex(va("sound/enemies/borg/borgservo%d.wav",
  Q_irand(1, 8))));
  }
  */
...
void NPC_AimWiggle(vec3_t enemy_org) {
  ...
  if (NPCInfo->aimErrorDebounceTime < level.time) {
    NPCInfo->aimOfs[0] =
        0.3 * Q_flrand(NPC->enemy->mins[0], NPC->enemy->maxs[0]);
    NPCInfo->aimOfs[1] =
        0.3 * Q_flrand(NPC->enemy->mins[1], NPC->enemy->maxs[1]);
...
qboolean NPC_UpdateFiringAngles(qboolean doPitch, qboolean doYaw) {
  ...
  if (NPCInfo->aimErrorDebounceTime < level.time) {
    if (Q_irand(0, 1)) {
      NPCInfo->lastAimErrorYaw =
          ((float)(6 - NPCInfo->stats.aim)) * Q_flrand(-1, 1);
    }
    if (Q_irand(0, 1)) {
  ...
  if (doPitch) {
    ...
    ucmd.angles[PITCH] = ANGLE2SHORT(targetPitch + diff + error) -
                         client->ps.delta_angles[PITCH];
  }
...
/*
static void NPC_UpdateShootAngles (vec3_t angles, qboolean doPitch, qboolean
doYaw )

Does update angles on shootAngles
*/

void NPC_UpdateShootAngles(
    vec3_t angles, qboolean doPitch,
    qboolean doYaw) { // FIXME: shoot angles either not set right or not used!
  float error;
  float decay;
  float targetPitch = 0;
  float targetYaw = 0;
...
void SetTeamNumbers(void) {
  ...
  for (i = 0; i < TEAM_NUM_TEAMS; i++) { // Get the average health
    teamStrength[i] =
        floor(((float)(teamStrength[i])) / ((float)(teamNumbers[i])));
  }
}
...
qboolean G_ActivateBehavior(gentity_t *self, int bset) {
  ...
  if (bSID != (bState_t)-1) {
    ...
    Quake3Game()->DebugPrint(
        IGameInterface::WL_VERBOSE, "%s attempting to run bSet %s (%s)\n",
...
qboolean G_ValidEnemy(gentity_t *self, gentity_t *enemy) {
  ...
  if (enemy->client == NULL) {
    if (enemy->svFlags & SVF_NONNPC_ENEMY) { // still potentially valid
      if (self->client) {
  ...
  if (enemy->client->playerTeam == TEAM_FREE &&
      enemy->s.number < MAX_CLIENTS) { // An evil player, everyone attacks him
    return qtrue;
  }
  ...
  // if ( NPCInfo->enemyLastSeenTime != 0 && level.time -
  // NPCInfo->enemyLastSeenTime > 7000 )//FIXME: make a stat? 	return qfalse;

  if (enemy->client->playerTeam ==
          self->client->enemyTeam // simplest case: they're on my enemy team
      || (self->client->enemyTeam == TEAM_FREE &&
          enemy->client->NPC_class !=
              self->client->NPC_class) // I get mad at anyone and this guy isn't
                                       // the same class as me
      || (enemy->client->NPC_class == CLASS_WAMPA &&
          enemy->enemy) // a rampaging wampa
      || (enemy->client->NPC_class == CLASS_RANCOR &&
          enemy->enemy) // a rampaging rancor
      || (enemy->client->playerTeam == TEAM_FREE &&
          enemy->client->enemyTeam == TEAM_FREE && enemy->enemy &&
          enemy->enemy->client &&
          (enemy->enemy->client->playerTeam == self->client->playerTeam ||
           (enemy->enemy->client->playerTeam != TEAM_ENEMY &&
            self->client->playerTeam ==
                TEAM_PLAYER))) // enemy is a rampaging non-aligned creature who
                               // is attacking someone on our team or a non-enemy
                               // (this last condition is used only if we're a
...
qboolean NPC_ValidEnemy(gentity_t *ent) { return G_ValidEnemy(NPC, ent); }
...
qboolean NPC_TargetVisible(gentity_t *ent) {
  ...
  if (DistanceSquared(ent->currentOrigin, NPC->currentOrigin) >
...
static int NPC_GetCheckDelta( void )
{
        ...
        {
                int distance = DistanceSquared( NPC->currentOrigin,
g_entities[0].currentOrigin );
...
extern gentity_t *G_CheckControlledTurretEnemy(gentity_t *self,
                                               gentity_t *enemy,
                                               qboolean validate);
...
static qboolean NPC_CheckPlayerDistance(void) {
  ...
  if (InFOV(&g_entities[0], NPC, NPCInfo->stats.hfov, NPCInfo->stats.vfov) ==
      qfalse)
    return qfalse;

  float distance =
      DistanceSquared(NPC->currentOrigin, NPC->enemy->currentOrigin);

  if (distance >
      DistanceSquared(NPC->currentOrigin, g_entities[0].currentOrigin)) {
...
qboolean NPC_FindEnemy(qboolean checkAlerts = qfalse) {
  ...
  if (NPC->client->NPC_class != CLASS_RANCOR &&
      ...
      NPC_CheckPlayerDistance()) { // rancors, wampas & sand creatures don't
                                   // care if player is closer, they always go
                                   // with closest
    return qtrue;
  }
...
qboolean NPC_FacePosition(vec3_t position, qboolean doPitch) {
  ...
  // Get the positions
  if (NPC->client && (NPC->client->NPC_class == CLASS_RANCOR ||
                      NPC->client->NPC_class == CLASS_WAMPA ||
                      NPC->client->NPC_class == CLASS_SAND_CREATURE)) {
    ...
    if (NPC->client->NPC_class ==
        CLASS_ROCKETTROOPER) { //*sigh*, look down more
      position[2] -= 32;
    }
  }
  ...
  if (NPC->enemy && NPC->enemy->client &&
    ...
    // FIXME: this is kind of dumb, but it was the easiest way to get it to look
    // sort of ok
  ...
  float yawDelta = AngleNormalize360(
      NPCInfo->desiredYaw -
      (SHORT2ANGLE(ucmd.angles[YAW] + client->ps.delta_angles[YAW])));
...
qboolean NPC_CheckLookTarget(gentity_t *self) {
  if (self->client) {
    if (self->client->renderInfo.lookTarget >= 0 &&
        self->client->renderInfo.lookTarget <
            ENTITYNUM_WORLD) { // within valid range
      if ((&g_entities[self->client->renderInfo.lookTarget] == NULL) ||
          !g_entities[self->client->renderInfo.lookTarget]
               .inuse) { // lookTarget not inuse or not valid anymore
        NPC_ClearLookTarget(self);
      } else if (self->client->renderInfo.lookTargetClearTime &&
                 self->client->renderInfo.lookTargetClearTime <
                     level.time) { // Time to clear lookTarget
        NPC_ClearLookTarget(self);
      } else if (g_entities[self->client->renderInfo.lookTarget].client &&
                 self->enemy &&
                 (&g_entities[self->client->renderInfo.lookTarget] !=
                  self->enemy)) { // should always look at current enemy if
                                  // engaged in battle... FIXME: this could
                                  // override certain scripted lookTargets...???
        NPC_ClearLookTarget(self);
      } else {
        return qtrue;
      }
...
void G_CheckCharmed(gentity_t *self) {
  if (self && self->client && self->client->playerTeam == TEAM_PLAYER &&
      self->NPC && self->NPC->charmedTime &&
      (self->NPC->charmedTime < level.time ||
       self->health <= 0)) { // we were charmed, set us back!
...
void G_GetBoltPosition(gentity_t *self, int boltIndex, vec3_t pos,
                       int modelIndex = 0) {
  ...
  gi.G2API_GetBoltMatrix(self->ghoul2, modelIndex, boltIndex, &boltMatrix,
                         angles, self->currentOrigin,
...
int G_GetEntsNearBolt(gentity_t *self, gentity_t **radiusEnts, float radius,
...
int NPC_GetEntsNearBolt(gentity_t **radiusEnts, float radius, int boltIndex,
                        vec3_t boltOrg) {
