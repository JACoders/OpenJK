...
void NPC_SandCreature_Die(gentity_t *self, gentity_t *inflictor,
                          gentity_t *other, const vec3_t point, int damage,
                          int mod, int hitLoc) {
  // FIXME: somehow make him solid when he dies?
}

void NPC_SandCreature_Pain(gentity_t *self, gentity_t *inflictor,
                           gentity_t *other, const vec3_t point, int damage,
                           int mod, int hitLoc) {
  if (TIMER_Done(self, "pain")) {
    ...
    TIMER_Set(self, "pain",
              self->client->ps.legsAnimTimer + Q_irand(500, 2000));
...
void SandCreature_MoveEffect(void) {
  vec3_t up = {0, 0, 1};
  vec3_t org = {NPC->currentOrigin[0], NPC->currentOrigin[1],
                NPC->absmin[2] + 2};
  ...
  if (level.time - NPC->client->ps.lastStationary >
      2000) { // first time moving for at least 2 seconds
    // clear speakingtime
    TIMER_Set(NPC, "speaking", -level.time);
  }

  if (TIMER_Done(NPC, "breaching") && TIMER_Done(NPC, "breachDebounce") &&
      TIMER_Done(NPC, "pain") && TIMER_Done(NPC, "attacking") &&
      !Q_irand(0, 10)) { // Breach!
    // FIXME: only do this while moving forward?
    trace_t trace;
    // make him solid here so he can be hit/gets blocked on stuff. Check clear
    // first.
    gi.trace(&trace, NPC->currentOrigin, NPC->mins, NPC->maxs,
             NPC->currentOrigin, NPC->s.number, MASK_NPCSOLID, (EG2_Collision)0,
             0);
    if (!trace.allsolid && !trace.startsolid) {
      ...
      TIMER_Set(NPC, "breachDebounce",
                NPC->client->ps.legsAnimTimer + Q_irand(0, 10000));
    }
  }
  if (!TIMER_Done(NPC, "breaching")) { // different effect when breaching
    // FIXME: make effect
...
qboolean SandCreature_Move(void) {
  ...
  // Sand Creatures look silly using waypoints when they can go straight to the
  // goal
  if (SandCreature_CheckAhead(
          dest)) { // use our temp move straight to goal check
  ...
  if (moved && NPC->radius) {
    ...
    VectorMA(NPC->currentOrigin, NPC->client->ps.speed / 100.0f,
             NPC->client->ps.moveDir, newPos);
    newTurfRange = DistanceHorizontal(newPos, NPC->s.origin);
    if (newTurfRange > NPC->radius &&
        newTurfRange > curTurfRange) { // would leave our range
      // stop
...
void SandCreature_Attack(qboolean miss) {
  ...
  // FIXME: shootable during this anim?
  if (!NPC->enemy->client) {
    NPC_SetAnim(NPC, SETANIM_LEGS, BOTH_ATTACK1,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD |
                    SETANIM_FLAG_RESTART);
  } else {
  ...
  if (miss) { // purposely missed him, chance of knocking him down
    // FIXME: if, during the attack anim, I do end up catching him close to my
    // mouth, then snatch him anyway...
    if (NPC->enemy && NPC->enemy->client) {
      ...
      if (g_spskill->integer > 0) {
        ...
        if (enemyDist < 200 &&
          ...
          if (g_spskill->integer > 1) { // knock them down, too
            if (NPC->enemy->health > 0 &&
                Q_flrand(50, 150) > enemyDist) { // knock them down
              G_Knockdown(NPC->enemy, NPC, dir2Enemy, 300, qtrue);
              if (NPC->enemy->s.number <
                  MAX_CLIENTS) { // make the player look up at me
                vec3_t vAng;
    ...
    if (NPC->activator->client) {
      ...
      if (NPC->activator->health > 0 && NPC->activator->client) {
        ...
        NPC_SetAnim(NPC->activator, SETANIM_LEGS, BOTH_SWIM_IDLE1,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      ...
      {
              cg.overrides.active |=
      (CG_OVERRIDE_3RD_PERSON_CDP|CG_OVERRIDE_3RD_PERSON_RNG);
  ...
  for (i = 0; i < numEnts; i++) {
    ...
    if (radiusEnts[i]->client == NULL) { // must be a client
      if (radiusEnts[i]->s.eType != ET_MISSILE ||
          radiusEnts[i]->s.weapon != WP_THERMAL) { // not a thermal detonator
        continue;
      }
    } else {
      ...
      if (radiusEnts[i]->client->ps.groundEntityNum !=
          ENTITYNUM_WORLD) { // not on the ground
        continue;
      }
    ...
    /*
    if ( radiusEnts[i]->client && (radiusEnts[i]->client->NPC_class ==
    CLASS_RANCOR || radiusEnts[i]->client->NPC_class == CLASS_ATST ) )
...
void SandCreature_CheckAlerts(void) {
  if (!(NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
    int alertEvent = NPC_CheckAlertEvents(qfalse, qtrue, NPCInfo->lastAlertID,
                                          qfalse, AEL_MINOR, qtrue);
...
float SandCreature_DistSqToGoal(qboolean goalIsEnemy) {
  ...
  if (NPCInfo->goalEntity->client) {
    goalDistSq =
        DistanceSquared(NPC->currentOrigin, NPCInfo->goalEntity->currentOrigin);
  } else {
    vec3_t gOrg;
    VectorCopy(NPCInfo->goalEntity->currentOrigin, gOrg);
    gOrg[2] -= (NPC->mins[2] -
                NPCInfo->goalEntity
                    ->mins[2]); // moves the gOrg up/down to make it's origin
                                // seem at the proper height as if it had my mins
...
void SandCreature_Chase(void) {
  if (!NPC->enemy->inuse) { // freed
    NPC->enemy = NULL;
    return;
  }

  if ((NPC->svFlags & SVF_LOCKEDENEMY)) { // always know where he is
    NPCInfo->enemyLastSeenTime = level.time;
  }
  ...
  if (NPC->enemy->client) {
    if ((NPC->enemy->client->ps.eFlags & EF_HELD_BY_SAND_CREATURE) ||
        ...
        (NPC->enemy->client->ps.eFlags &
         EF_HELD_BY_WAMPA)) { // was picked up by another monster, forget about
                              // him
      NPC->enemy = NULL;
  ...
  if (NPC->enemy->client &&
      ...
      !(NPC->svFlags & SVF_LOCKEDENEMY)) { // off the ground!
                                           // FIXME: keep moving in the dir we
                                           // were moving for a little bit...
  } else {
    float enemyScore = SandCreature_EntScore(NPC->enemy);
    if (enemyScore < MIN_SCORE &&
        !(NPC->svFlags & SVF_LOCKEDENEMY)) { // too slow or too far away
    } else {
      float moveSpeed;
      if (NPC->enemy->client) {
      ...
      if (moveSpeed) { // he's still moving, update my goalEntity's origin
  ...
  if ((level.time - NPCInfo->enemyLastSeenTime) > 5000 &&
      !(NPC->svFlags &
        SVF_LOCKEDENEMY)) { // enemy hasn't moved in about 5 seconds, see if
  ...
  if (enemyDistSq >= MIN_ATTACK_DIST_SQ // NPCInfo->goalEntity &&
      && (level.time - NPCInfo->enemyLastSeenTime) <=
             3000) { // sensed enemy (or something) less than 3 seconds ago
    ...
    if (SandCreature_Move()) {
      SandCreature_MoveEffect();
             ...
             !(NPC->svFlags &
               SVF_LOCKEDENEMY)) { // NOTE: this leaves a 2-second dead zone in
                                   // which they'll just sit there unless their
                                   // enemy moves
    // If there is an event we might be interested in if we weren't still
    // interested in our enemy
    if (NPC_CheckAlertEvents(qfalse, qtrue, NPCInfo->lastAlertID, qfalse,
                             AEL_MINOR, qtrue) >= 0) { // just stir
      SandCreature_MoveEffect();
    }
  }
  ...
  } else if (enemyDistSq < MAX_MISS_DIST_SQ && enemyDistSq > MIN_MISS_DIST_SQ &&
             NPC->enemy->client && TIMER_Done(NPC, "breaching") &&
             TIMER_Done(NPC, "missDebounce") &&
             !VectorCompare(NPC->pos1,
                            NPC->currentOrigin) // so we don't come up again in
                                                // the same spot
             && !Q_irand(0, 10)) {
    if (!(NPC->svFlags & SVF_LOCKEDENEMY)) {
      // miss them
...
void SandCreature_Sleep(void) {
  ...
  {
          ucmd.buttons |= BUTTON_WALKING;
          //FIXME: Sand Creatures look silly using waypoints when they can go
  straight to the goal if ( SandCreature_Move() )
          {
                  SandCreature_MoveEffect();
          }
  }
...
void NPC_BSSandCreature_Default(void) {
  ...
  if (NPC->health > 0 &&
      TIMER_Done(NPC, "breaching")) { // go back to non-solid mode
    if (NPC->contents) {
      NPC->contents = 0;
    }
    ...
    if (TIMER_Done(NPC, "speaking")) {
      G_SoundOnEnt(NPC, CHAN_VOICE,
                   va("sound/chars/sand_creature/voice%d.mp3", Q_irand(1, 3)));
      TIMER_Set(NPC, "speaking", Q_irand(3000, 10000));
    }
  } else { // still in breaching anim
    visible = qtrue;
    // FIXME: maybe push things up/away and maybe knock people down when doing
    // this?
  ...
  // FIXME: when in start and end of attack/pain anims, need ground disturbance
  // effect around him
  // NOTENOTE: someone stubbed this code in, so I figured I'd use it.  The
  // timers are all weird, ie, magic numbers that sort of work,
  ...
  if (NPC->client->ps.legsAnim == BOTH_ATTACK1 ||
      NPC->client->ps.legsAnim ==
          BOTH_ATTACK2) { // FIXME: get start and end frame numbers for this
                          // effect for each of these anims
    vec3_t up = {0, 0, 1};
    vec3_t org;
    VectorCopy(NPC->currentOrigin, org);
    org[2] -= 40;
    if (NPC->client->ps.legsAnimTimer > 3700) {
      //			G_PlayEffect( G_EffectIndex( "env/sand_dive"  ),
      //NPC->currentOrigin, up );
      G_PlayEffect(G_EffectIndex("env/sand_spray"), org, up);
    } else if (NPC->client->ps.legsAnimTimer > 1600 &&
               NPC->client->ps.legsAnimTimer < 1900) {
      G_PlayEffect(G_EffectIndex("env/sand_spray"), org, up);
    }
  ...
  if (!TIMER_Done(NPC, "pain")) {
    ...
    if (NPC->activator) { // kill and remove the guy we ate
      ...
      } else { // can't remove the player, just make him invisible
    ...
    if (NPC->enemy) {
      SandCreature_Chase();
               ...
               5000) // FIXME: should make this able to be variable
    { // we were alerted recently, move towards there and look for footsteps,
      // etc.
      SandCreature_Hunt();
    } else { // no alerts, sleep and wake up only by alerts
      // FIXME: keeps chasing goalEntity even when it's already reached it!
      SandCreature_Sleep();
    }
  }
...
// FIXME: need pain behavior of sticking up through ground, writhing and
// screaming
// FIXME: need death anim like pain, but flopping aside and staying above
// ground...