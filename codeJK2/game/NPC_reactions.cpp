...
void NPC_ChoosePainAnimation(gentity_t *self, gentity_t *other, vec3_t point, int damage, int mod, int hitLoc, int voiceEvent = -1) {
	...
	if (level.time < self->painDebounceTime && mod != MOD_ELECTROCUTE &&
		mod != MOD_MELEE) { // FIXME: if hit while recoving from losing a saber lock, we should still play a pain anim?
		return;
	}
	...
	if (self->s.weapon == WP_THERMAL && self->client->fireDelay > 0) { // don't interrupt thermal throwing anim
		return;
	...
	} else if (self->client && self->client->playerTeam == TEAM_PLAYER && other && !other->s.number) { // ally shot by player always complains
		pain_chance = 1.1f;
	} else {
	...
	if (Q_flrand(0.0f, 1.0f) < pain_chance) {
		...
		if (!(self->client->ps.eFlags & EF_FORCE_GRIPPED)) { // not being force-gripped
			if (G_CheckForStrongAttackMomentum(self) || PM_SpinningAnim(self->client->ps.legsAnim) || PM_SaberInSpecialAttack(self->client->ps.torsoAnim) ||
...
void NPC_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other, vec3_t point, int damage, int mod, int hitLoc) {
	...
	if (self->client->playerTeam && other->client && otherTeam == self->client->playerTeam &&
		(!player->client->ps.viewEntity || other->s.number != player->client->ps.viewEntity)) { // hit by a teammate
...
void NPC_Touch(gentity_t *self, gentity_t *other, trace_t *trace) {
	...
	if (self->message && self->health <= 0) {							// I am dead and carrying a key
		if (other && player && player->health > 0 && other == player) { // player touched me
			char *text;
			qboolean keyTaken;
			// give him my key
...
void NPC_Respond(gentity_t *self, int userNum) {
	...
	*/
	...
	case CLASS_JEDI:
		if (!self->enemy) {
			if (!(self->svFlags & SVF_IGNORE_ENEMIES) && (self->NPC->scriptFlags & SCF_LOOK_FOR_ENEMIES) && self->client->enemyTeam == TEAM_ENEMY) {
...
void NPC_UseResponse(gentity_t *self, gentity_t *user, qboolean useWhenDone) {
	...
	if (user->client && self->client->playerTeam != user->client->playerTeam && self->client->playerTeam != TEAM_NEUTRAL) { // only those on the same team react
		if (useWhenDone) {
			G_ActivateBehavior(self, BSET_USE);
		}
		return;
...
void NPC_Use(gentity_t *self, gentity_t *other, gentity_t *activator) {
	...
	if (self->client && self->NPC) {
		...
		*/
		...
		//			NPC_TakePatient( activator );
		//		}
		else if (!self->enemy && activator->s.number == 0 && !gi.VoiceVolume[self->s.number] &&
				 !(self->NPC->scriptFlags & SCF_NO_RESPONSE)) { // I don't have an enemy and I'm not talking and I was used by the player
			NPC_UseResponse(self, other, qfalse);
		}
	}
