...
void SV_LinkEntity(sharedEntity_t *gEnt) {
  ...
  if (gEnt->r.bmodel) {
    ...
    if (gEnt->s.solid ==
        SOLID_BMODEL) { // yikes, this would make everything explode violently.
  ...
  for (i = 0; i < num; i++) {
    ...
    if (clip->passEntityNum != ENTITYNUM_NONE) {
      ...
      if (touch->r.ownerNum == clip->passEntityNum) {
        if (touch->r.svFlags & SVF_OWNERNOTSHARED) {
          if (clip->contentmask != (MASK_SHOT | CONTENTS_LIGHTSABER) &&
              clip->contentmask !=
                  (MASK_SHOT)) { // it's not a laser hitting the other
                                 // "missile", don't care then
            continue;
          }
        } else {
      ...
      if (touch->r.ownerNum == passOwnerNum &&
          !(touch->r.svFlags & SVF_OWNERNOTSHARED) && thisOwnerShared) {
        continue; // don't clip against other missiles from our owner
      }

      if (touch->s.eType == ET_MISSILE &&
          !(touch->r.svFlags & SVF_OWNERNOTSHARED) &&
          touch->r.ownerNum == passOwnerNum) { // blah, hack
        continue;
      }
    }
    ...
    if (clip->traceFlags &
        G2TRFLAG_DOGHOULTRACE) { // keep these older variables around for a bit,
                                 // incase we need to replace them in the Ghoul2
                                 // Collision check
      oldTrace = clip->trace;
    }
    ...
    if (trace.allsolid) {
      ...
      // rww - added this because we want to get the number of an ent even if
      // our trace starts inside it.
    ...
    if (trace.fraction < clip->trace.fraction) {
      ...
      clip->trace.startsolid =
          (qboolean)((unsigned)clip->trace.startsolid | (unsigned)oldStart);
    }
/*
Ghoul2 Insert Start
*/
...
#else
    // rww - since this is multiplayer and we don't have the luxury of violating
    // networking rules in horrible ways, this must be done somewhat differently.
    if ((clip->traceFlags & G2TRFLAG_DOGHOULTRACE) &&
        trace.entityNum == touch->s.number && touch->ghoul2 &&
        ((clip->traceFlags & G2TRFLAG_HITCORPSES) ||
         !(touch->s.eFlags &
           EF_DEAD))) { // standard behavior will be to ignore g2 col on dead
                        // ents, but if traceFlags is set to allow, then we'll
                        // try g2 col on EF_DEAD people too.
      ...
      if (clip->traceFlags &
          G2TRFLAG_THICK) { // if using this flag, make sure it's at least 1.0f
        if (fRadius < 1.0f) {
          fRadius = 1.0f;
        }
      }
      ...
      // I would think that you could trace from trace.endpos instead of
      // clip->start, but that causes it to miss sometimes.. Not sure what it's
      // off, but if it could be done like that, it would probably be faster.
#ifndef FINAL_BUILD
      ...
      if (com_optvehtrace && com_optvehtrace->integer &&
          touch->s.eType == ET_NPC && touch->s.NPC_class == CLASS_VEHICLE &&
          touch->m_pVehicle) { // for vehicles cache the transform data.
        re->G2API_CollisionDetectCache(
            G2Trace, *((CGhoul2Info_v *)touch->ghoul2), angles,
            touch->r.currentOrigin, sv.time, touch->s.number, clip->start,
            clip->end, touch->modelScale, G2VertSpaceServer, 0, clip->useLod,
            fRadius);
      } else {
        re->G2API_CollisionDetect(G2Trace, *((CGhoul2Info_v *)touch->ghoul2),
                                  angles, touch->r.currentOrigin, sv.time,
                                  touch->s.number, clip->start, clip->end,
                                  touch->modelScale, G2VertSpaceServer, 0,
                                  clip->useLod, fRadius);
      }
      ...
      while (tN < MAX_G2_COLLISIONS) {
        if (G2Trace[tN].mEntityNum == touch->s.number) { // ok, valid
          bestTr = tN;
          break;
        } else if (G2Trace[tN].mEntityNum ==
                   -1) { // there should not be any after the first -1
          break;
        }
        tN++;
      }
      ...
      if (bestTr == -1) { // Well then, put the trace back to the old one.
        clip->trace = oldTrace;
      } else { // Otherwise, set the endpos/normal/etc. to the model location
               // hit instead of leaving it out in space.
        ...
        if (clip->traceFlags &
            G2TRFLAG_GETSURFINDEX) { // we have requested that surfaceFlags be
                                     // stomped over with the g2 hit surface
                                     // index.
