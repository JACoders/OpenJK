...
void NPC_Blocked(gentity_t *self, gentity_t *blocker) {
  ...
  if ((blocker->s.number == 0) &&
    ...
    // NOTE: only imperials, misc crewmen and hazard team have these wav files
    // now G_AddVoiceEvent( self, Q_irand(EV_BLOCKED1, EV_BLOCKED3), 0 );
  }
...
void NPC_SetMoveGoal(gentity_t *ent, vec3_t point, int radius,
                     qboolean isNavGoal, int combatPoint,
                     gentity_t *targetEnt) {
  // Must be an NPC
  if (ent->NPC == NULL) {
    return;
  ...
  // Copy the origin
  // VectorCopy( point, ent->NPC->goalPoint );	//FIXME: Make it use this, and
  // this alone!
...
qboolean NAV_HitNavGoal(vec3_t point, vec3_t mins, vec3_t maxs, vec3_t dest,
  ...
  if (radius & NAVGOAL_USE_RADIUS) {
    ...
    //			a radius manually set! We can't do the smaller navgoals
    //against 			walls to get around this because player-sized traces to them 			from
    //angles will not work... - MCG
    ...
    // There is probably a better way to do this, either by preserving the
    // original 		mins and maxs of the navgoal and doing this check ONLY if the
    //radius 		is non-zero (like the original implementation) or some boolean to
    //		tell us to do this check rather than the fake bbox overlap
    //check...
  } else {
...
qboolean NAV_ClearPathToPoint(gentity_t *self, vec3_t pmins, vec3_t pmaxs,
  ...
  if (self->client || (self->svFlags & SVF_NAVGOAL)) {
    // Clients can step up things, or if this is a navgoal check, a client will
    // be using this info
    mins[2] += STEPSIZE;
  ...
  if (self->svFlags & SVF_NAVGOAL) {
    ...
    gi.trace(
        &trace, point, mins, maxs, self->currentOrigin, self->owner->s.number,
    ...
    if (trace.startsolid &&
      ...
      gi.trace(
          &trace, point, mins, maxs, self->currentOrigin, self->owner->s.number,
          (clipmask | CONTENTS_MONSTERCLIP) & ~CONTENTS_BODY, G2_NOCOLLIDE, 0);
    }
    ...
    if (NAV_HitNavGoal(self->currentOrigin, self->owner->mins,
                       self->owner->maxs, trace.endpos, NPCInfo->goalRadius,
                       FlyingCreature(self->owner))) {
      return qtrue;
    } else {
      if (NAVDEBUG_showCollision) {
        if (trace.entityNum < ENTITYNUM_WORLD &&
            (&g_entities[trace.entityNum] != NULL) &&
            !g_entities[trace.entityNum].bmodel) {
          vec3_t p1, p2;
          CG_DrawEdge(point, trace.endpos, EDGE_PATH);
          VectorAdd(g_entities[trace.entityNum].mins,
                    g_entities[trace.entityNum].currentOrigin, p1);
          VectorAdd(g_entities[trace.entityNum].maxs,
                    g_entities[trace.entityNum].currentOrigin, p2);
    ...
    gi.trace(&trace, self->currentOrigin, mins, maxs, point, self->s.number,
    ...
    if (trace.startsolid &&
      ...
      clipmask &= ~CONTENTS_BOTCLIP;
      gi.trace(&trace, self->currentOrigin, mins, maxs, point, self->s.number,
               clipmask | CONTENTS_MONSTERCLIP, G2_NOCOLLIDE, 0);
    }

    if (((trace.startsolid == qfalse) && (trace.allsolid == qfalse)) &&
        (trace.fraction == 1.0f)) { // FIXME: check for drops
      return qtrue;
    }
...
int NAV_FindClosestWaypointForPoint(gentity_t *ent, vec3_t point) {
  ...
  bestWP = navigator.GetNearestNode(marker, marker->waypoint, NF_CLEAR_PATH,
                                    WAYPOINT_NONE);
...
qboolean NAV_CheckAhead(gentity_t *self, vec3_t end, trace_t &trace,
  ...
  float radius =
      (self->maxs[0] > self->maxs[1]) ? self->maxs[0] : self->maxs[1];
  ...
  if (trace.entityNum < ENTITYNUM_WORLD) {
    ...
    if
      VALIDSTRING(blocker->classname) {
        if (G_EntIsUnlockedDoor(blocker->s.number))
        // if ( Q_stricmp( blocker->classname, "func_door" ) == 0 )
        {
          // We're too close, try and avoid the door (most likely stuck on a
          // lip)
          if (DistanceSquared(self->currentOrigin, trace.endpos) <
...
static qboolean NAV_TestBypass(gentity_t *self, float yaw, float blocked_dist,
  ...
  if (NAV_CheckAhead(self, block_pos, tr,
                     (self->clipmask & ~CONTENTS_BODY) | CONTENTS_BOTCLIP)) {
...
qboolean NAV_Bypass(gentity_t *self, gentity_t *blocker, vec3_t blocked_dir,
  ...
  // FIXME: Although the below code will cause the NPC to take the "better"
  // route, it can cause NPCs to become stuck on 		 one another in certain
  //situations where both decide to take the same direction.
  ...
  if (NAV_TestBypass(self, AngleNormalize360(yaw + arcAngle), blocked_dist,
                     movedir))
    return qtrue;

  // Try a smaller arc
  if (NAV_TestBypass(self, AngleNormalize360(yaw + (arcAngle * 0.5f)),
                     blocked_dist, movedir))
    return qtrue;

  // Try the other direction
  if (NAV_TestBypass(self, AngleNormalize360(yaw + (arcAngle * -1)),
                     blocked_dist, movedir))
    return qtrue;

  // Try the other direction more precisely
  if (NAV_TestBypass(self, AngleNormalize360(yaw + ((arcAngle * -1) * 0.5f)),
                     blocked_dist, movedir))
    return qtrue;
...
qboolean NAV_MoveBlocker(gentity_t *self, vec3_t shove_dir) {
  ...
  // FIXME: This will, of course, push blockers off of cliffs, into walls and
  // all over the place
...
qboolean NAV_ResolveBlock(gentity_t *self, gentity_t *blocker,
                          vec3_t blocked_dir) {
  // Stop double waiting
...
qboolean NAV_StackedCanyon(gentity_t *self, gentity_t *blocker,
                           vec3_t pathDir) {
  ...
  trace_t tr;
  gi.trace(&tr, test, self->mins, self->maxs, test, self->s.number,
           self->clipmask | extraClip, G2_NOCOLLIDE, 0);
  if (tr.startsolid &&
      (tr.contents &
       CONTENTS_BOTCLIP)) { // started inside do not enter, so ignore them
...
qboolean NAV_ResolveEntityCollision(gentity_t *self, gentity_t *blocker,
  ...
  // if ( Q_stricmp( blocker->classname, "func_door" ) == 0 )
  {
    if (DistanceSquared(self->currentOrigin, blocker->currentOrigin) >
        MIN_DOOR_BLOCK_DIST_SQR)
      return qtrue;
  }
  ...
  //	if ( NAV_PredictCollision( self, blocker, movedir, blocked_dir ) ==
  //qfalse ) 		return qtrue;
...
qboolean NAV_TestForBlocked(gentity_t *self, gentity_t *goal,
  ...
  if (NAV_HitNavGoal(blocker->currentOrigin, blocker->mins, blocker->maxs,
                     goal->currentOrigin, 12, qfalse)) {
    flags |= NIF_BLOCKED;
...
qboolean NAV_AvoidCollision(gentity_t *self, gentity_t *goal, navInfo_t &info) {
  ...
  if (NAV_CheckAhead(self, movepos, info.trace, CONTENTS_BODY) == qfalse) {
    ...
    // if ( NAV_TrueCollision( self, info.blocker, movedir, info.direction ) ==
    // qfalse ) 	return qtrue;
    ...
    if (NAV_ResolveEntityCollision(self, info.blocker, movedir,
                                   info.pathDirection) == qfalse)
      return qfalse;
...
int NAV_TestBestNode(gentity_t *self, int startID, int endID,
                     qboolean failEdge) { // check only against architectrure
  vec3_t end;
  trace_t trace;
  vec3_t mins;
  ...
  // Do a simple check
  if ((trace.allsolid == qfalse) && (trace.startsolid == qfalse) &&
      (trace.fraction == 1.0f)) { // it's clear
    return endID;
  }

  // See if we're too far above
  if (self->s.weapon != WP_SABER &&
      fabs(self->currentOrigin[2] - end[2]) > 48) {
  } else {
    // This is a work around
    float radius =
        (self->maxs[0] > self->maxs[1]) ? self->maxs[0] : self->maxs[1];
  ...
  if (trace.entityNum < ENTITYNUM_WORLD) {
    ...
    if
      VALIDSTRING(blocker->classname) { // special case: doors are architecture,
                                        // but are dynamic, like entitites
        if (G_EntIsUnlockedDoor(blocker->s.number))
        // if ( Q_stricmp( blocker->classname, "func_door" ) == 0 )
        { // it's unlocked, go for it
          ...
          if (self->s.weapon != WP_SABER &&
              fabs(self->currentOrigin[2] - end[2]) > 48) { // too far above
          } else {
            return endID;
          }
        } else if (G_EntIsDoor(blocker->s.number)) { // a locked door!
          // path is blocked by a locked door, mark it as such if instructed to
          // do so
          if (failEdge) {
            navigator.AddFailedEdge(self->s.number, startID, endID);
          }
        } else if (G_EntIsBreakable(
                       blocker->s.number)) { // do same for breakable
                                             // brushes/models/glass?
          // path is blocked by a breakable, mark it as such if instructed to do
          // so
          if (failEdge) {
            navigator.AddFailedEdge(self->s.number, startID, endID);
          }
        } else if (G_EntIsRemovableUsable(
                       blocker->s.number)) { // and removable usables
          // path is blocked by a removable usable, mark it as such if
          // instructed to do so
          if (failEdge) {
            navigator.AddFailedEdge(self->s.number, startID, endID);
          }
        } else if (blocker->targetname && blocker->s.solid == SOLID_BMODEL &&
                   ((blocker->contents & CONTENTS_MONSTERCLIP) ||
                    (blocker->contents &
                     CONTENTS_BOTCLIP))) { // some other kind of do not enter
                                           // entity brush that will probably be
                                           // removed
          // path is blocked by a removable brushent, mark it as such if
          // instructed to do so
          if (failEdge) {
            navigator.AddFailedEdge(self->s.number, startID, endID);
          }
...
int NAV_MoveToGoal(gentity_t *self, navInfo_t &info) {
  ...
  if (self->NPC->goalEntity->s.number == 0) {
    ...
    // NOTENOTE: Otherwise trust this waypoint for the whole frame (reduce all
    // unnecessary calculations)
  } else {
    // Find the target's waypoint
    if ((self->NPC->goalEntity->waypoint = NAV_GetNearestNode(
             self->NPC->goalEntity, self->NPC->goalEntity->waypoint)) ==
        WAYPOINT_NONE)
      return WAYPOINT_NONE;
  }

  // Find our waypoint
  if ((self->waypoint = NAV_GetNearestNode(self, self->lastWaypoint)) ==
      WAYPOINT_NONE)
    return WAYPOINT_NONE;

  int bestNode =
      navigator.GetBestNode(self->waypoint, self->NPC->goalEntity->waypoint);
  ...
  // Check this node
  bestNode =
      NAV_TestBestNode(self, bestNode, self->NPC->goalEntity->waypoint, qfalse);
  ...
  if (NAV_CheckAhead(self, origin, info.trace,
                     (self->clipmask & ~CONTENTS_BODY) | CONTENTS_BOTCLIP) ==
      qfalse) {
    // First attempt to move to the closest point on the line between the
    // waypoints
...
unsigned int waypoint_getRadius(gentity_t *ent) {
  ...
  for (int i = 0; i < YAW_ITERATIONS; i++) {
    dist = waypoint_testDirection(ent->currentOrigin,
                                  ((360.0f / YAW_ITERATIONS) * i), minDist);
...
SOLID_OK - only use if placing inside solid is unavoidable in map, but may be
clear in-game (ie: at the bottom of a tall, solid lift that starts at the top
position)
...
void SP_waypoint(gentity_t *ent) {
  if (navCalculatePaths) {
    ...
    if (!(ent->spawnflags & 1) &&
        G_CheckInSolid(ent, qtrue)) { // if not SOLID_OK, and in solid
      ...
      if (G_CheckInSolid(ent, qtrue)) {
        ...
        if (!g_entities[ENTITYNUM_WORLD].s.radius) { // not a region
          G_Error("Waypoint %s at %s in solid!", ent->targetname,
    ...
    ent->health =
        navigator.AddRawPoint(ent->currentOrigin, ent->spawnflags, radius);
...
#ifndef FINAL_BUILD
        if (!g_entities[ENTITYNUM_WORLD].s.radius) { // not a region
          G_Error("Waypoint_small %s at %s in solid!", ent->targetname,
                  vtos(ent->currentOrigin));
        }
#endif
        G_FreeEntity(ent);
...
targetname - name you would use in script when setting a navgoal (like so:)
  ...
  For example: if you give this waypoint a targetname of "console", make an NPC
go to it in a script like so:
...
#ifndef FINAL_BUILD
    if (!g_entities[ENTITYNUM_WORLD].s.radius) { // not a region
      G_Error("Waypoint_navgoal %s at %s in solid!", ent->targetname,
              vtos(ent->currentOrigin));
    }
#endif
  }
  TAG_Add(ent->targetname, NULL, ent->s.origin, ent->s.angles, radius,
          RTF_NAVGOAL);

  ent->classname = "navgoal";
  G_FreeEntity(ent); // can't do this, they need to be found later by some
                     // functions, though those could be fixed, maybe?
}
...
#ifndef FINAL_BUILD
    if (!g_entities[ENTITYNUM_WORLD].s.radius) { // not a region
      G_Error("Waypoint_navgoal_8 %s at %s in solid!", ent->targetname,
              vtos(ent->currentOrigin));
    }
#endif
    assert(0);
...
#ifndef FINAL_BUILD
    if (!g_entities[ENTITYNUM_WORLD].s.radius) { // not a region
      G_Error("Waypoint_navgoal_4 %s at %s in solid!", ent->targetname,
              vtos(ent->currentOrigin));
    }
#endif
    assert(0);
...
#ifndef FINAL_BUILD
    if (!g_entities[ENTITYNUM_WORLD].s.radius) { // not a region
      G_Error("Waypoint_navgoal_2 %s at %s in solid!", ent->targetname,
              vtos(ent->currentOrigin));
    }
#endif
    assert(0);
...
#ifndef FINAL_BUILD
    if (!g_entities[ENTITYNUM_WORLD].s.radius) { // not a region
      G_Error("Waypoint_navgoal_1 %s at %s in solid!", ent->targetname,
              vtos(ent->currentOrigin));
    }
#endif
    assert(0);
...
void Svcmd_Nav_f(void) {
  ...
  if (Q_stricmp(cmd, "show") == 0) {
    ...
    if (Q_stricmp(cmd, "testgoal") == 0) {
      NAVDEBUG_curGoal = navigator.GetNearestNode(
...
qboolean NAV_WaypointsTooFar(gentity_t *wp1, gentity_t *wp2) {
  if (Distance(wp1->currentOrigin, wp2->currentOrigin) > 1024) {
    ...
    len = strlen(temp);
    if ((fatalErrorPointer - fatalErrorString) + len >=
        (int)sizeof(fatalErrorString)) {
...
void NAV_StoreWaypoint(gentity_t *ent) {
  if (!tempWaypointList) {
    tempWaypointList = (waypointData_t *)gi.Malloc(sizeof(waypointData_t) *
                                                       MAX_STORED_WAYPOINTS,
                                                   TAG_TEMP_WORKSPACE, qtrue);
  }

  if (numStoredWaypoints >= MAX_STORED_WAYPOINTS) {
    G_Error("Too many waypoints!  (%d > %d)", numStoredWaypoints,
            MAX_STORED_WAYPOINTS);
    return;
  }
  if (ent->targetname) {
    Q_strncpyz(tempWaypointList[numStoredWaypoints].targetname, ent->targetname,
               MAX_QPATH);
  }
  if (ent->target) {
    Q_strncpyz(tempWaypointList[numStoredWaypoints].target, ent->target,
               MAX_QPATH);
  }
  if (ent->target2) {
    Q_strncpyz(tempWaypointList[numStoredWaypoints].target2, ent->target2,
               MAX_QPATH);
  }
  if (ent->target3) {
    Q_strncpyz(tempWaypointList[numStoredWaypoints].target3, ent->target3,
               MAX_QPATH);
  }
  if (ent->target4) {
...
#endif
      {
        navigator.HardConnect(tempWaypointList[i].nodeID,
                              tempWaypointList[target].nodeID);
      }
    }
...
//			if ( !NAV_WaypointsTooFar( ent, target ) )
  ...
  {
          Com_Printf("Unable to save navigations data for map \"%s\"
  (checksum:%d)\n", filename, checksum );
  }
  */
#ifndef FINAL_BUILD
...
void NAV_Shutdown(void) { navigator.Free(); }
...
void NAV_ShowDebugInfo(void) {
  ...
  if (NAVDEBUG_showTestPath) {
    ...
    int nearestNode = navigator.GetNearestNode(
        &g_entities[0], g_entities[0].waypoint, NF_ANY, WAYPOINT_NONE);
    int testNode = navigator.GetBestNode(nearestNode, NAVDEBUG_curGoal);

    nearestNode =
        NAV_TestBestNode(&g_entities[0], nearestNode, testNode, qfalse);
...
void NAV_FindPlayerWaypoint(void) {
  g_entities[0].waypoint = navigator.GetNearestNode(
      &g_entities[0], g_entities[0].lastWaypoint, NF_CLEAR_PATH, WAYPOINT_NONE);
}
