...
static void CG_DrawClientScore(int y, score_t *score, float *color, float fade, qboolean largeFormat) {
	...
	else if (cgs.gametype == GT_SIEGE) { // try to draw the shader for this class on the scoreboard
		if (ci->siegeIndex != -1) {
...
qboolean CG_DrawOldScoreboard(void) {
	...
	if ((cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL) && cgs.duelWinner != -1 && cg.predictedPlayerState.pm_type == PM_INTERMISSION) {
		...
		*/
		...
		if (cgs.gametype == GT_POWERDUEL && cgs.duelist3 != -1) {
			s = va("%s^7 %s %s^7 %s %s", cgs.clientinfo[cgs.duelist1].name, CG_GetStringEdString("MP_INGAME", "SPECHUD_VERSUS"),
	...
	if (cgs.gametype == GT_POWERDUEL) { // do nothing?
		...
		if (cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR) {
			...
			s = va("%s %s (%s %i) %s %i", CG_PlaceString(cg.snap->ps.persistant[PERS_RANK] + 1), sPlace, sOf, cg.numScores, sWith,
