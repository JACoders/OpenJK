...
// Filename:-	sstring.h
//
// Gil's string template, used to replace Microsoft's <string> vrsion which
// doesn't compile under certain stl map<>
//	conditions...
...
template <int MaxSize> class sstring {
  struct SStorage {
    char data[MaxSize];
  };
  SStorage mStorage;
...
public:
  ...
  */
  ...
  sstring(const gsl::cstring_view &v) {
    if (v.size() + 1 > sizeof(mStorage.data)) {
      Com_Error(ERR_FATAL,
                "String dest buffer too small (%d) to hold string of length %d",
                sizeof(mStorage.data), v.size());
    }
    std::copy(v.begin(), v.end(), mStorage.data);
    mStorage.data[v.size()] = '\0';
  }
  sstring() { mStorage.data[0] = 0; }
  /* don't figure we need this
          template<int oMaxSize>
  ...
  */
  ...
  char *c_str() { return mStorage.data; }
  const char *c_str() const { return mStorage.data; }
  int capacity() const {
    return MaxSize; // not sure if this should be MaxSize-1? depends if talking
                    // bytes or strlen space I guess
  }
  int length() const { return strlen(mStorage.data); }
