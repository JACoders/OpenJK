...
// FIXME: The following tables should be passed in to the interpreter for
// flexibility
...
keywordArray_t CInterpreter::m_IDKeywords[] = {
    ...
    "LOOP",   ID_LOOP,   "PRINT",   ID_PRINT,   "TAG",        ID_TAG,
    "USE",    ID_USE,    "FLUSH",   ID_FLUSH,   "RUN",        ID_RUN,
    "KILL",   ID_KILL,   "REMOVE",  ID_REMOVE,  "CAMERA",     ID_CAMERA,
    "GET",    ID_GET,    "RANDOM",  ID_RANDOM,  "IF",         ID_IF,
    "ELSE",   ID_ELSE,   "REM",     ID_REM,     "FLOAT",      TK_FLOAT,
    "VECTOR", TK_VECTOR, "STRING",  TK_STRING,  "TASK",       ID_TASK,
    "DO",     ID_DO,     "DECLARE", ID_DECLARE, "FREE",       ID_FREE,
    "DOWAIT", ID_DOWAIT, "SIGNAL",  ID_SIGNAL,  "WAITSIGNAL", ID_WAITSIGNAL,
    "PLAY",   ID_PLAY,
...
int CInterpreter::Error(char *format, ...) {
  va_list argptr;
  char *error_file, error_msg[1024] = "", work_dir[1024] = "",
                    out_msg[1024] = "";
  ...
  // A bit of kludge code that takes care of the case where there's some garbage
  // at the beginning of the file
  //	before any blocks are read as valid. This is needed because ints are
  //incapable of containing 0
  //
  // This'll mean that technically it's saying block 1 is wrong, rather than the
  // one in between them, but I can
  //	live with that.
  //
...
===================================================================================================
  ...
  Looks ahead to the next token to try and match it to the passed token,
...
int CInterpreter::GetString(CBlock *block) {
  ...
  if ((type != TK_STRING) && (type != TK_CHAR)) {
    return Error("syntax error : expected string; found %s",
                 GetTokenName(type));
  }
...
int CInterpreter::GetIdentifier(CBlock *block) {
  ...
  if (type != TK_IDENTIFIER) {
    return Error("syntax error : expected indentifier; found %s",
                 GetTokenName(type));
  }
...
int CInterpreter::GetEvaluator(CBlock *block) {
  ...
  default:
    return Error("syntax error : expected operator type, found %s",
                 GetTokenName(type));
  }
...
int CInterpreter::GetType(char *get) {
  ...
  if (type != TK_IDENTIFIER) {
    return Error("syntax error : expected identifier; found %s",
                 GetTokenName(type));
  }
...
int CInterpreter::GetID(char *id_name) {
  ...
  // FIXME: Function pointers would be awfully nice.. but not inside a class!
  // Weee!!
  ...
  case ID_ELSE:
    ...
    return GetElse(); // FIXME: Protect this call so that floating else's aren't
                      // allowed
    break;
  ...
  case ID_PLAY:
    GetPlay(); // Bad eighties slang joke...  yeah, it's not really funny, I
               // know...
    break;
...
// this is just so people can put comments in scripts in BehavEd and not have
// them lost as normal comments would be.
//
...
// NOTENOTE: The tag's information is included as block members, not as a
// separate block.
...
int CInterpreter::GetTag(CBlock *block) {
  ...
  if ((typeID != TYPE_ORIGIN) && (typeID != TYPE_ANGLES)) {
    return Error("syntax error : 'tag' : %s is not a valid look up identifier",
                 typeName);
  }
...
// note new return type, this now returns the bad block number, else 0 for
// success.
//
//  I also return -ve block numbers for errors between blocks. Eg if you read 3
//  good blocks, then find an unexpected
//		float in the script between blocks 3 & 4 then I return -3 to
//indicate the error is after that, but not block 4
//
int CInterpreter::Interpret(CTokenizer *Tokenizer, CBlockStream *BlockStream,
                            char *filename) {
  CBlock block;
  CToken *token;
  int type, blockLevel = 0, parenthesisLevel = 0;

  m_sCurrentFile = filename; // used during error reporting because you can't
                             // ask tokenizer for pushed streams
