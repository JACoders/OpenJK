...
static void R_SetupEntityLightingGrid(trRefEntity_t *ent) {
	...
	for (i = 0; i < 8; i++) {
		...
		if (gridPos >= tr.world->lightGridArray + tr.world->numGridArrayElements) { // we've gone off the array somehow
			continue;
		}
...
void R_SetupEntityLighting(const trRefdef_t *refdef, trRefEntity_t *ent) {
	...
	if (ent->e.renderfx & RF_MINLIGHT) { // the minlight flag is now for items rotating on their holo thing
