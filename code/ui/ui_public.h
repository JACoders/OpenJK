...
typedef struct {
  ...
  qhandle_t (*R_RegisterShaderNoMip)(
      const char *name);                         // returns white if not found
  qhandle_t (*R_RegisterFont)(const char *name); // returns 0 for bad font

  int (*R_Font_StrLenPixels)(const char *text, const int setIndex,
                             const float scale);
  ...
  void (*R_ScissorPic)(float x, float y, float w, float h, float s1, float t1,
  ...
  void (*R_LerpTag)(orientation_t *tag, clipHandle_t mod, int startFrame,
