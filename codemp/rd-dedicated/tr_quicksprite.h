...
private:
  ...
  unsigned int mIndexes[SHADER_MAX_VERTEXES]; // Ideally this would be static,
                                              // cause it never changes
  vec2_t mTextureCoords[SHADER_MAX_VERTEXES]; // Ideally this would be static,
                                              // cause it never changes
...
public:
  ...
  void StartGroup(textureBundle_t *bundle, unsigned long glbits,
                  int fogIndex = -1);
  void EndGroup(void);
