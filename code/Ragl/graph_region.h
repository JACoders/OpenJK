...
template <class TNODE, int MAXNODES, class TEDGE, int MAXEDGES,
          int NUM_EDGES_PER_NODE, int MAXREGIONS, int MAXREGIONEDGES>
class graph_region : public ratl::ratl_base {
public:
  ////////////////////////////////////////////////////////////////////////////////////
  // Capacity Enum
  ////////////////////////////////////////////////////////////////////////////////////
  enum { NULL_REGION = -1, NULL_EDGE = -1, CAPACITY = MAXREGIONS };

  ////////////////////////////////////////////////////////////////////////////////////
  // Some Public Type Defines
  ////////////////////////////////////////////////////////////////////////////////////
  typedef ragl::graph_vs<TNODE, MAXNODES, TEDGE, MAXEDGES, NUM_EDGES_PER_NODE>
      TGraph;
  typedef ratl::vector_vs<int, MAXNODES> TRegions;
  typedef ratl::vector_vs<short, MAXREGIONS>
      TRegionEdge; // List Of All Edges Which Connect RegionA<->RegionB
  typedef ratl::pool_vs<TRegionEdge, MAXREGIONEDGES>
      TEdges; // Pool Of All RegionEdges
  typedef ratl::grid2_vs<short, MAXREGIONS, MAXREGIONS>
      TLinks; // Graph Of Links From Region To Region, Each Points To A
              // RegionEdge
  ...
  // Constructor
  ////////////////////////////////////////////////////////////////////////////////////
  graph_region(TGraph &Graph) : mGraph(Graph) { clear(); }
  ~graph_region() {}
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  int size() { return mRegionCount; }

  ////////////////////////////////////////////////////////////////////////////////////
  // Get The Region For A Given Node
  ////////////////////////////////////////////////////////////////////////////////////
  int get_node_region(int Node) { return mRegions[mGraph.node_index(Node)]; }

  ////////////////////////////////////////////////////////////////////////////////////
  // Call this function to find out if it is at all possible to get from nodeA
  // to nodeB.  If there is no possible connection, or there is one, but the
  // connection is not valid at the current time, this routine will return
  // false.  Use it as a quick cull routine before a search.
  //
  // In order to use this function, you must have an EdgeQuery class (use the
  // default above, or derive your own for more specialized behavior).
  ////////////////////////////////////////////////////////////////////////////////////
  ...
  int reserve() {
    ...
    if (mRegionCount >=
        (MAXREGIONS - 1)) { // stop adding points, we're full, you MUST increase
                            // MAXREGIONS for this to work
      return NULL_REGION;
    }
  ...
  // Define Regions
  //
  // Scan through all the nodes (calling the depth first recursive traversal
  // below), and mark regions of nodes which can traverse to one another without
  // needing to check for valid edges.
  //
  ////////////////////////////////////////////////////////////////////////////////////
  bool find_regions(const typename TGraph::user &user) {
    int CurNodeIndex;
    for (typename TGraph::TNodes::iterator i = mGraph.nodes_begin();
         i != mGraph.nodes_end(); i++) {
      CurNodeIndex = i.index();
      if (mRegions[CurNodeIndex] == NULL_REGION) {
        assert(mRegionCount < (MAXREGIONS - 1));
        if (mRegionCount >=
            (MAXREGIONS - 1)) { // stop adding points, we're full, you MUST
                                // increase MAXREGIONS for this to work
          return false;
        }
  ...
  //
  // Once called, this class will have reference data for how to get from one
  // region to another.
  ////////////////////////////////////////////////////////////////////////////////////
    ...
    for (int indexA = 0; indexA < MAXNODES; indexA++) {
      ...
      if (RegionA != NULL_REGION) {
        for (int indexB = 0; indexB < MAXNODES; indexB++) {
          ...
          ReservedRegionLink = (RegionA <= mReservedRegionCount ||
                                RegionB <= mReservedRegionCount);
          if (RegionB != NULL_REGION && RegionB != RegionA &&
              mGraph.get_edge_across(indexA, indexB)) {
            ...
            if (RegionLink == -1) {
              if (ReservedRegionLink) {
                mLinks.get(RegionA, RegionB) =
                    -2; // Special Flag For Reserved Regions - they have no
                        // edges
...
private:
  ...
  //
  // It will visit all neighbors for each node which have not already been
  // visited and assigned to a region.  Neighbors must always be valid.
  ////////////////////////////////////////////////////////////////////////////////////
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  bool has_valid_region_edge(int CurRegion, int TargetRegion,
                             const typename TGraph::user &user) {
    ...
    for (int NextRegion = 0; NextRegion < mRegionCount; NextRegion++) {
      // Check If The Link Exists And We Have Not Already Visited The Next
      // Region
      //--------------------------------------------------------------------------
      ...
      if (CurRegionEdge != NULL_EDGE && !mClosed.get_bit(NextRegion)) {
        if (CurRegion <= mReservedRegionCount) {
          ...
          if (has_valid_region_edge(NextRegion, TargetRegion, user)) {
            return true; // HEY!  Somehow, Going To Next Region Got Us To The
                         // Target Region!
          }
        } else {
          ...
          for (int j = 0; j < mEdges[CurRegionEdge].size(); j++) {
            if (user.is_valid(mGraph.get_edge(mEdges[CurRegionEdge][j]),
                              (NextRegion == TargetRegion) ? (-1) : (0))) {
              // Great, So We Have Found A Valid Neighboring Region, Search
              // There
              //------------------------------------------------------------------
              if (has_valid_region_edge(NextRegion, TargetRegion, user)) {
                return true; // HEY!  Somehow, Going To Next Region Got Us To
                             // The Target Region!
              }

              // Ok, The Target Region Turned Out To Be A Dead End, We Can Stop
              // Trying To Get There
              //------------------------------------------------------------------------------------
              break;
            }
    ...
    // Nope, We Failed To Find Any Valid Region Edges Which Lead To The Target
    // Region
    //--------------------------------------------------------------------------------
    return false;
  }
...
public:
  ...
  void ProfileSpew() {
    ...
    ProfilePrint("--------------------------------------------------------");
    ProfilePrint("");
    ProfilePrint("REGION SIZE (Bytes): (%d)  (KiloBytes): (%5.3f)",
                 sizeof(*this), ((float)(sizeof(*this)) / 1024.0f));
    ProfilePrint("REGION COUNT: (%d) Regions  (%d) Edges", mRegionCount,
                 mEdges.size());
    if (mRegionCount) {
      int RegionEdges = 0;
      for (typename TEdges::iterator it = mEdges.begin(); it != mEdges.end();
           ++it) {
        RegionEdges += (*it).size();
      }
      ProfilePrint("REGION COUNT: (%f) Ave Edges Size",
                   (float)RegionEdges / (float)mRegionCount);
    }
    ProfilePrint("");
  };
#endif
