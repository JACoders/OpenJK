...
void G_Give(gentity_t *ent, const char *name, const char *args, int argc) {
  ...
  if (give_all || !Q_stricmp(name, "weapons")) {
    ent->client->ps.stats[STAT_WEAPONS] =
        (1 << (MAX_PLAYER_WEAPONS + 1)) - (1 << WP_NONE);
    if (!give_all)
      return;
  }
  ...
  if (!give_all && !Q_stricmp(name, "eweaps")) // for developing, gives you all
                                               // the weapons, including enemy
  {
    ent->client->ps.stats[STAT_WEAPONS] =
        (unsigned)(1 << WP_NUM_WEAPONS) -
        (1 << WP_NONE); // NOTE: this wasn't giving the last weapon in the list
    return;
  }
  ...
  if (give_all || !Q_stricmp(name, "inventory")) {
    // Huh?  Was doing a INV_MAX+1 which was wrong because then you'd actually
    // have every inventory item including INV_MAX
    ent->client->ps.stats[STAT_ITEMS] =
        (1 << (INV_MAX)) - (1 << INV_ELECTROBINOCULARS);
...
void Cmd_Fx(gentity_t *ent) {
    ...
    while ((fx_ent = G_Find(fx_ent, FOFS(classname), "cmd_fx")) != NULL) {
      if (gi.argc() == 5) {
        ...
        gi.Printf(S_COLOR_GREEN "FX: current origin is: <%6.2f %6.2f %6.2f>\n",
                  fx_ent->currentOrigin[0], fx_ent->currentOrigin[1],
                  fx_ent->currentOrigin[2]);
      }
    ...
    while ((fx_ent = G_Find(fx_ent, FOFS(classname), "cmd_fx")) != NULL) {
      if (gi.argc() == 5) {
        ...
        gi.Printf(S_COLOR_GREEN "FX: current dir is: <%6.2f %6.2f %6.2f>\n",
                  fx_ent->s.angles[0], fx_ent->s.angles[1],
                  fx_ent->s.angles[2]);
      }
  ...
  gi.Printf(S_COLOR_CYAN
            "----------------------------------------------------------\n");
  gi.Printf(S_COLOR_CYAN
            "fx play <filename>     fx play sparks, fx play env/fire\n");
...
void UserSpawn(gentity_t *ent, const char *name) {
  ...
  ent2 = G_Spawn();
  ent2->classname =
      G_NewString(name); // FIXME: This will leave floating memory...
  ...
  VectorMA(ent->s.pos.trBase, 96, vf,
           origin); // FIXME: Find the radius size of the object, and push out
                    // 32 + radius
...
void Cmd_SetObjective_f(gentity_t *ent) {
  ...
  if (gi.argc() != 4) {
    gi.SendServerCommand(ent - g_entities,
                         va("print \"usage: setobjective <objective #>  "
...
void Cmd_ViewObjective_f(gentity_t *ent) {
  ...
  if (gi.argc() != 2) {
    gi.SendServerCommand(ent - g_entities,
                         va("print \"usage: viewobjective <objective #>\n\""));
    return;
  }
  ...
  gi.SendServerCommand(
      ent - g_entities,
      va("print \"Objective %d   Display Status(1=show): %d  Status:%d\n\"",
...
//----------------------------------------------------------------------------------
qboolean PickSeekerSpawnPoint(vec3_t org, vec3_t fwd, vec3_t right, int skip,
                              vec3_t spot) {
...
void Cmd_UseSeeker_f(gentity_t *ent) {
  ...
  // don't use them if we don't have any...also don't use them if one is already
  // going
  if (ent->client && ent->client->ps.inventory[INV_SEEKER] > 0 &&
    ...
    if (tent) {
      ...
      VectorCopy(ent->currentOrigin,
                 spot); // does nothing really, just initialize the goods...

      if (PickSeekerSpawnPoint(ent->currentOrigin, fwd, right, ent->s.number,
                               spot)) {
        ...
        ent->client->ps.powerups[PW_SEEKER] =
            level.time +
            1000; // can only drop one every second..maybe this is annoying?
      }
    }
  }
}
...
void Cmd_UseSentry_f(gentity_t *ent) {
  ...
  if (place_portable_assault_sentry(ent, ent->currentOrigin,
...
void G_Victory(gentity_t *ent) {
  if (ent->health > 0) { // say something and put away saber
...
void ClientCommand(int clientNum) {
  ...
  if (Q_stricmp(cmd, "give") == 0)
    ...
    G_Victory(ent);
  //	else if (Q_stricmp (cmd, "use_shield") == 0)	// sounds like the
  //design doc states that the shields will be a pickup and so the player never
  //decides whether to use them or not. 		G_ActivatePersonalShield( ent );
  ////	If you want shields (armor),  type "give all" or "give armor" or "give
  //armor #amt#"
