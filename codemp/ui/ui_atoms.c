...
static consoleCommand_t commands[] = {
    {"ui_cache", UI_Cache_f},       {"ui_load", UI_Load},
    {"ui_openmenu", UI_OpenMenu_f}, {"ui_opensiegemenu", UI_OpenSiegeMenu_f},
    {"ui_report", UI_Report},
};
...
qboolean UI_ConsoleCommand(int realTime) {
  ...
  command = (consoleCommand_t *)Q_LinearSearch(
      UI_Argv(0), commands, numCommands, sizeof(commands[0]), cmdcmp);
...
void UI_DrawSides(float x, float y, float w, float h) {
  ...
  trap->R_DrawStretchPic(x + w - 1, y, 1, h, 0, 0, 0, 0,
                         uiInfo.uiDC.whiteShader);
}

void UI_DrawTopBottom(float x, float y, float w, float h) {
  trap->R_DrawStretchPic(x, y, w, 1, 0, 0, 0, 0, uiInfo.uiDC.whiteShader);
  trap->R_DrawStretchPic(x, y + h - 1, w, 1, 0, 0, 0, 0,
                         uiInfo.uiDC.whiteShader);
}
/*
================
