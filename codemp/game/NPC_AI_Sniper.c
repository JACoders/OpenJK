...
void NPC_Sniper_PlayConfusionSound(
    gentity_t *self) { // FIXME: make this a custom sound in sound set
  if (self->health > 0) {
...
static qboolean Sniper_Move(void) {
  ...
  if (moved == qfalse) { // couldn't get to enemy
    if ((NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) &&
        ...
        NPCS.NPCInfo->goalEntity ==
            NPCS.NPC->enemy) { // we were running after enemy
      ...
      cp = NPC_FindCombatPoint(NPCS.NPC->r.currentOrigin,
                               ...
                               NPCS.NPC->r.currentOrigin, cpFlags, 32, -1);
      if (cp == -1 && !(NPCS.NPCInfo->scriptFlags &
                        SCF_USE_CP_NEAREST)) { // okay, try one by the enemy
      ...
      // NOTE: there may be a perfectly valid one, just not one within
      // CP_COLLECT_RADIUS of either me or him...
...
void NPC_BSSniper_Patrol(void) { // FIXME: pick up on bodies of dead buddies?
  ...
  if (NPCS.NPCInfo->confusionTime < level.time) {
    ...
    if (!(NPCS.NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
      ...
      int alertEvent =
          NPC_CheckAlertEvents(qtrue, qtrue, -1, qfalse, AEL_SUSPICIOUS);
      if (NPC_CheckForDanger(alertEvent)) {
        ...
        if (alertEvent >= 0 &&
            level.alertEvents[alertEvent].ID != NPCS.NPCInfo->lastAlertID) {
          ...
          if (level.alertEvents[alertEvent].level == AEL_DISCOVERED) {
            if (level.alertEvents[alertEvent].owner &&
                ...
                level.alertEvents[alertEvent].owner->client->playerTeam ==
                    NPCS.NPC->client->enemyTeam) { // an enemy
              G_SetEnemy(NPCS.NPC, level.alertEvents[alertEvent].owner);
              // NPCInfo->enemyLastSeenTime = level.time;
              TIMER_Set(NPCS.NPC, "attackDelay",
                        Q_irand((6 - NPCS.NPCInfo->stats.aim) * 100,
                                (6 - NPCS.NPCInfo->stats.aim) * 500));
            }
          } else { // FIXME: get more suspicious over time?
            // Save the position for movement (if necessary)
            // FIXME: sound?
            VectorCopy(level.alertEvents[alertEvent].position,
                       NPCS.NPCInfo->investigateGoal);
            NPCS.NPCInfo->investigateDebounceTime =
                level.time + Q_irand(500, 1000);
            if (level.alertEvents[alertEvent].level ==
                AEL_SUSPICIOUS) { // suspicious looks longer
      ...
      if (NPCS.NPCInfo->investigateDebounceTime >
          level.time) { // FIXME: walk over to it, maybe?  Not if not chase
                        // enemies flag
        // NOTE: stops walking or doing anything else below
        vec3_t dir, angles;
        float o_yaw, o_pitch;

        VectorSubtract(NPCS.NPCInfo->investigateGoal,
                       NPCS.NPC->client->renderInfo.eyePoint, dir);
...
static void Sniper_CheckMoveState(void) {
  ...
  if (!(NPCS.NPCInfo->scriptFlags &
        SCF_CHASE_ENEMIES)) // NPCInfo->behaviorState == BS_STAND_AND_SHOOT )
  {
  ...
  if ((NPCS.NPCInfo->goalEntity != NPCS.NPC->enemy) &&
    ...
    if (NAV_HitNavGoal(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins,
        ...
        (NPCS.NPCInfo->squadState == SQUAD_SCOUT && enemyLOS2 &&
         enemyDist2 <= 10000)) {
      ...
      TIMER_Set(NPCS.NPC, "attackDelay",
                Q_irand((6 - NPCS.NPCInfo->stats.aim) * 50,
                        (6 - NPCS.NPCInfo->stats.aim) *
                            100)); // FIXME: Slant for difficulty levels, too?
...
static void Sniper_ResolveBlockedShot(void) {
  if (TIMER_Done(NPCS.NPC, "duck")) {       // we're not ducking
    if (TIMER_Done(NPCS.NPC, "roamTime")) { // not roaming
...
static void Sniper_CheckFireState(void) {
  ...
  if (!Q_irand(0, 1) && NPCS.NPCInfo->enemyLastSeenTime &&
      level.time - NPCS.NPCInfo->enemyLastSeenTime <
          ((5 - NPCS.NPCInfo->stats.aim) *
           1000)) // FIXME: incorporate skill too?
  {
    ...
    return;
  } else if (level.time - NPCS.NPCInfo->enemyLastSeenTime >
             10000) { // next time we see him, we'll miss few times first
    NPCS.NPC->count = 0;
  }
}
...
void Sniper_FaceEnemy(void) {
  // FIXME: the ones behind kill holes are facing some arbitrary direction and
  // not firing
  // FIXME: If actually trying to hit enemy, don't fire unless enemy is at least
  // in front of me?
  ...
  if (NPCS.NPC->enemy) {
    ...
    if (enemyDist2 > 65536 &&
        NPCS.NPCInfo->stats.aim < 5) // is 256 squared, was 16384 (128*128)
    {
      if (NPCS.NPC->count <
          (5 - NPCS.NPCInfo->stats.aim)) { // miss a few times first
        if (shoot2 && TIMER_Done(NPCS.NPC, "attackDelay") &&
            level.time >= NPCS.NPCInfo->shotTime) { // ready to fire again
          ...
          while (hit && tryMissCount < 10) {
            ...
            if (!Q_irand(0, 1)) {
              aimError = qtrue;
              if (!Q_irand(0, 1)) {
                VectorMA(target, NPCS.NPC->enemy->r.maxs[2] * flrand(1.5, 4),
                         right, target);
              } else {
                VectorMA(target, NPCS.NPC->enemy->r.mins[2] * flrand(1.5, 4),
                         right, target);
              }
            }
            ...
            trap->Trace(&trace, muzzle, vec3_origin, vec3_origin, target,
      ...
      } else { // based on distance, aim value, difficulty and enemy movement,
               // miss
...
void Sniper_UpdateEnemyPos(void) {
  ...
  for (i = MAX_ENEMY_POS_LAG - ENEMY_POS_LAG_INTERVAL; i >= 0;
       i -= ENEMY_POS_LAG_INTERVAL) {
    index = i / ENEMY_POS_LAG_INTERVAL;
    if (!index) {
      CalcEntitySpot(NPCS.NPC->enemy, SPOT_HEAD_LEAN,
                     NPCS.NPCInfo->enemyLaggedPos[index]);
      NPCS.NPCInfo->enemyLaggedPos[index][2] -= flrand(2, 16);
    } else {
      VectorCopy(NPCS.NPCInfo->enemyLaggedPos[index - 1],
                 NPCS.NPCInfo->enemyLaggedPos[index]);
    }
  }
}
...
void NPC_BSSniper_Attack(void) {
  ...
  if (TIMER_Done(NPCS.NPC, "flee") &&
      NPC_CheckForDanger(NPC_CheckAlertEvents(qtrue, qtrue, -1, qfalse,
                                              AEL_DANGER))) { // going to run
    NPC_UpdateAngles(qtrue, qtrue);
    return;
  }
  ...
  faceEnemy2 = qfalse;
  shoot2 = qfalse;
  enemyDist2 = DistanceSquared(NPCS.NPC->r.currentOrigin,
                               NPCS.NPC->enemy->r.currentOrigin);
  ...
  {                       // too close, so switch to primary fire
    if (NPCS.NPC->client->ps.weapon ==
      ...
      if (NPCS.NPCInfo->scriptFlags & SCF_ALT_FIRE) { // use primary fire
        trace_t trace;
        trap->Trace(&trace, NPCS.NPC->enemy->r.currentOrigin,
                    NPCS.NPC->enemy->r.mins, NPCS.NPC->enemy->r.maxs,
                    NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->s.number,
                    NPCS.NPC->enemy->clipmask, qfalse, 0, 0);
        if (!trace.allsolid && !trace.startsolid &&
            (trace.fraction == 1.0 ||
             trace.entityNum == NPCS.NPC->s.number)) { // he can get right to me
  ...
  if (NPC_ClearLOS4(
          NPCS.NPC->enemy)) //|| (NPCInfo->stats.aim >= 5 && trap->inPVS(
                            ...
                            //NPC->enemy->currentOrigin )) )
  {
    ...
    if (enemyDist2 < maxShootDist) {
      ...
      trap->Trace(&tr, muzzle, NULL, NULL, end, NPCS.NPC->s.number, MASK_SHOT,
                  qfalse, 0, 0);
  ...
  */
  ...
  if (enemyLOS2) { // FIXME: no need to face enemy if we're moving to some other
  ...
  if (enemyCS2) {
    shoot2 = qtrue;
             ...
             3000) { // Hmm, have to get around this bastard... FIXME: this
                     // NPCInfo->enemyLastSeenTime builds up when ducked seems to
                     // make them want to run when they uncrouch
  ...
  if (!move2) {
    if (!TIMER_Done(NPCS.NPC, "duck")) {
      if (TIMER_Done(NPCS.NPC, "watch")) { // not while watching
        NPCS.ucmd.upmove = -127;
      }
    }
    // FIXME: what about leaning?
    // FIXME: also, when stop ducking, start looking, if enemy can see me,
    // chance of ducking back down again
  } else { // stop ducking!
    TIMER_Set(NPCS.NPC, "duck", -1);
  }

  if (TIMER_Done(NPCS.NPC, "duck") && TIMER_Done(NPCS.NPC, "watch") &&
  ...
  if (shoot2) { // try to shoot if it's time
    if (TIMER_Done(NPCS.NPC, "attackDelay")) {
      ...
      if (!(NPCS.NPC->spawnflags & SPF_NO_HIDE) && !Q_irand(0, 1)) {
        // FIXME: do this if in combat point and combat point has duck-type
        // cover... also handle lean-type cover
        Sniper_StartHide();
      } else {
