...
static char *GetToken(char **text, bool allowLineBreaks,
                      bool readUntilEOL = false) {
  char *pointer = *text;
  int length = 0;
  int c = 0;
  bool foundLineBreak;
  ...
  while (1) {
    ...
    } else { // found the start of a token
      break;
    }
  }

  if (c == '\"') { // handle a string
    pointer++;
    while (1) {
      c = *pointer++;
      if (c == '\"') {
  ...
  if (token[0] == '\"') { // remove start quote
    length--;
    memmove(token, token + 1, length);

    if (length && token[length - 1] == '\"') { // remove end quote
      length--;
    }
  }
...
CGPObject::CGPObject(const char *initName)
    : mName(initName), mNext(0), mInOrderNext(0), mInOrderPrevious(0) {}
...
CGPValue *CGPValue::Duplicate(CTextPool **textPool) {
  ...
  while (iterator) {
    if (textPool) {
      name =
          (*textPool)->AllocText((char *)iterator->GetName(), true, textPool);
    } else {
...
bool CGPValue::Parse(char **dataPtr, CTextPool **textPool) {
  ...
  while (1) {
    ...
    if (!token[0]) { // end of data - error!
      return false;
    } else if (Q_stricmp(token, "]") == 0) { // ending brace for this list
      break;
    }
...
CGPGroup::CGPGroup(const char *initName, CGPGroup *initParent)
    : CGPObject(initName), mPairs(0), mInOrderPairs(0), mCurrentPair(0),
      mSubGroups(0), mInOrderSubGroups(0), mCurrentSubGroup(0),
      mParent(initParent), mWriteable(false) {}

CGPGroup::~CGPGroup(void) { Clean(); }
...
void CGPGroup::SortObject(CGPObject *object, CGPObject **unsortedList,
                          CGPObject **sortedList, CGPObject **lastObject) {
  CGPObject *test, *last;
...
void CGPGroup::AddGroup(CGPGroup *NewGroup) {
  SortObject(NewGroup, (CGPObject **)&mSubGroups,
             (CGPObject **)&mInOrderSubGroups, (CGPObject **)&mCurrentSubGroup);
}
...
bool CGPGroup::Parse(char **dataPtr, CTextPool **textPool) {
  ...
  while (1) {
    ...
    } else if (Q_stricmp(token, "}") == 0) { // ending brace for this group
      break;
    }
 ...
 *Multiple keys may be searched if you specify "||" inbetween each key name in
 *the string.  The first key to be found (from left to right) will be returned.
 *
 * Input
...
CGenericParser2::CGenericParser2(void) : mTextPool(0), mWriteable(false) {}

CGenericParser2::~CGenericParser2(void) { Clean(); }
...
// C++ users should just use the objects as normally and not call these routines
// below
//
...
const char *GPG_FindPairValue(TGPGroup GPG, const char *key,
...
bool GPG_FindPairValue(TGPGroup GPG, const char *key, const char *defaultVal,
                       char *Value) {
