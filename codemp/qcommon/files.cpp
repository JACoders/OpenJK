...
bool Sys_FileOutOfDate(LPCSTR psFinalFileName /* dest */,
  ...
  if (Sys_GetFileTime(psFinalFileName, ftFinalFile) &&
    ...
    if ((fabs((double)(ftFinalFile.dwLowDateTime - ftDataFile.dwLowDateTime)) <=
         20000000) &&
...
long FS_FOpenFileRead(const char *filename, fileHandle_t *file,
  ...
  isUserConfig = !Q_stricmp(filename, "autoexec.cfg") ||
      ...
      if (search->pack && search->pack->hashTable[hash]) {
        ...
        do {
          ...
          if (!FS_FilenameCompare(pakFile->name, filename)) {
            ...
            // mark the pak as having been referenced and mark specifics on
            // cgame and ui shaders, txt, arena files  by themselves do not
            // count as a reference as these are loaded from all pk3s from every
            // pk3 file..

            // The x86.dll suffixes are needed in order for sv_pure to continue
            // to work on non-x86/windows systems...
...
// This is a bit of a hack but it is used for other OS'/arch to still be
// acceptable with pure servers. Intentionally looking for x86.dll because this
// is all that exists in pk3s.
...
static pack_t *FS_LoadZipFile(const char *zipfile, const char *basename) {
  ...
  fs_headerLongs =
      (int *)Z_Malloc((gi.number_entry + 1) * sizeof(int), TAG_FILESYS, qtrue);
...
char **FS_ListFilteredFiles(const char *path, const char *extension,
  ...
  for (search = fs_searchpaths; search; search = search->next) {
    ...
    if (search->pack) {
      ...
      if (fs_numServerPaks && (!extension || Q_stricmp(extension, "fcf"))) {
...
static void FS_AddGameDirectory(const char *path, const char *dir) {
  ...
  for (sp = fs_searchpaths; sp; sp = sp->next) {
    if (sp->dir && Sys_PathCmp(sp->dir->path, path) &&
...
/**
        ...
        @param gameName Name of the default folder (i.e. always BASEGAME =
   "base" in OpenJK)
*/
void FS_Startup(const char *gameName) {
  ...
  fs_gamedirvar =
      Cvar_Get("fs_game", "", CVAR_INIT | CVAR_SYSTEMINFO, "Mod directory");
...
bool FS_LoadMachOBundle(const char *name) {
  ...
  Com_sprintf(dllName, sizeof(dllName), "%s.bundle/Contents/MacOS/%s", name,
              name);
...
qboolean FS_WriteToTemporaryFile(const void *data, size_t dataLength,
  ...
  if (tempPathResult) {
    ...
    if (tempFileNameResult) {
      HANDLE file = CreateFile(tempFileName, GENERIC_WRITE, 0, NULL,
        ...
        if (WriteFile(file, data, dataLength, &bytesWritten, NULL)) {
          ...
          while (deletesRemaining > 0 &&
            ...
            Com_DPrintf("FS_WriteToTemporaryFile failed for '%s'. "
                        ...
                        fs_temporaryFileNames[fs_temporaryFileWriteIdx], error);
            ...
            fs_temporaryFileWriteIdx = (fs_temporaryFileWriteIdx + 1) %
                                       ARRAY_LEN(fs_temporaryFileNames);
            deletesRemaining--;
          }

          // If this happened, then all slots are used and we some how have 8
          // DLLs loaded at once?!
      ...
      error = GetLastError();
      Com_DPrintf(
          "FS_WriteToTemporaryFile failed to generate temporary file name. "
          "Win32 error code: 0x%08x\n",
          error);
    }
  } else {
