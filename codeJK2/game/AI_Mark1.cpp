...
void NPC_Mark1_Part_Explode(gentity_t *self, int bolt) {
	if (bolt >= 0) {
		...
		gi.G2API_GetBoltMatrix(self->ghoul2, self->playerModel, bolt, &boltMatrix, self->currentAngles, self->currentOrigin, (cg.time ? cg.time : level.time),
...
void Mark1Dead_FireRocket(void) {
	...
	gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->playerModel, NPC->genericBolt5, &boltMatrix, NPC->currentAngles, NPC->currentOrigin,
...
void Mark1Dead_FireBlaster(void) {
	...
	gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->playerModel, bolt, &boltMatrix, NPC->currentAngles, NPC->currentOrigin, (cg.time ? cg.time : level.time), NULL,
						   NPC->s.modelScale);
