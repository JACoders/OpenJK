...
#define MIN_TURN_AROUND_DIST_SQ                                                \
  (10000) //(100 squared) don't stop running backwards if your goal is less than
          //100 away
...
void Saboteur_Decloak(gentity_t *self, int uncloakTime) {
  if (self && self->client) {
    if (self->client->ps.powerups[PW_CLOAKED] &&
        TIMER_Done(self, "decloakwait")) { // Uncloak
...
void Saboteur_Cloak(gentity_t *self) {
  if (self && self->client &&
      self->NPC) { // FIXME: need to have this timer set once first?
    if (TIMER_Done(self,
                   "nocloak")) { // not sitting around waiting to cloak again
      if (!(self->NPC->aiFlags &
            NPCAI_SHIELDS)) { // not allowed to cloak, actually
        Saboteur_Decloak(self);
      } else if (!self->client->ps.powerups[PW_CLOAKED]) { // cloak
...
qboolean NPC_CheckEnemiesInSpotlight(void) {
  ...
  for (i = 0; i < numListedEntities; i++) {
    ...
    if (enemy && enemy->client && NPC_ValidEnemy(enemy) &&
        enemy->client->playerTeam ==
            NPC->client->enemyTeam) { // valid ent & client, valid enemy, on the
                                      // target team
      // check to see if they're in my FOV
      if (InFOV(enemy->currentOrigin, NPC->client->renderInfo.eyePoint,
                NPC->client->renderInfo.eyeAngles, NPCInfo->stats.hfov,
                NPCInfo->stats.vfov)) { // in my cone
        // check to see that they're close enough
        if (DistanceSquared(NPC->client->renderInfo.eyePoint,
                            enemy->currentOrigin) -
                256                       /*fudge factor: 16 squared*/
            <= NPC->speed * NPC->speed) { // within range
          // check to see if we have a clear trace to them
          if (G_ClearLOS(NPC, enemy)) { // clear LOS
            ...
            // FIXME: have the graduated noticing like other NPCs? (based on
            // distance, FOV dot, etc...)
      ...
      if (InFOV(enemy->currentOrigin, NPC->client->renderInfo.eyePoint,
                NPC->client->renderInfo.eyeAngles, 90,
                NPCInfo->stats.vfov *
                    3)) {             // one to look at if we don't get an enemy
        if (G_ClearLOS(NPC, enemy)) { // clear LOS
          if (suspect == NULL ||
              DistanceSquared(NPC->client->renderInfo.eyePoint,
                              enemy->currentOrigin) <
                  DistanceSquared(NPC->client->renderInfo.eyePoint,
                                  suspect->currentOrigin)) { // remember him
            suspect = enemy;
          }
        }
      }
  ...
  if (suspect &&
      Q_flrand(0, NPCInfo->stats.visrange * NPCInfo->stats.visrange) >
          DistanceSquared(NPC->client->renderInfo.eyePoint,
                          suspect->currentOrigin)) { // hey!  who's that?
    if (TIMER_Done(
            NPC,
            "enemyLastVisible")) { // If we haven't already, start the counter
      int lookTime = Q_irand(4500, 8500);
                ...
                SCF_LOOK_FOR_ENEMIES)) // FIXME: Is this reliable?
    {
      if (!Q_irand(0, 2)) {
        ...
        NPC_FacePosition(suspect->currentOrigin, qtrue);
        // FIXME: they still need some sort of eye/head tag/bone that can turn?
        // NPC_TempLookTarget( NPC, suspect->s.number, interrogateTime,
        // interrogateTime );
      }
    }
  }
  ...
  if (level.alertEvents[eventID].level > AEL_MINOR &&
    ...
    if (G_ExpandPointToBBox(
            ...
            ((NPC->clipmask & ~CONTENTS_BODY) |
             CONTENTS_BOTCLIP))) { // we were able to doMove the investigateGoal
...
void NPC_BSST_Investigate(void) {
  ...
  if (NPCInfo->localState == LSTATE_INVESTIGATE &&
      (NPCInfo->goalEntity != NULL)) {
    // See if we're there
    if (!STEER::Reached(NPC, NPCInfo->goalEntity, 32,
...
void NPC_BSST_Patrol(void) { // FIXME: pick up on bodies of dead buddies?
  ...
  if (NPC->client->NPC_class == CLASS_ROCKETTROOPER &&
      (NPC->client->ps.eFlags & EF_SPOTLIGHT)) { // using spotlight search mode
    vec3_t eyeFwd, end, mins = {-2, -2, -2}, maxs = {2, 2, 2};
    trace_t trace;
    AngleVectors(NPC->client->renderInfo.eyeAngles, eyeFwd, NULL, NULL);
    VectorMA(NPC->client->renderInfo.eyePoint, NPCInfo->stats.visrange, eyeFwd,
             end);
    // get server-side trace impact point
    gi.trace(&trace, NPC->client->renderInfo.eyePoint, mins, maxs, end,
             NPC->s.number, MASK_OPAQUE | CONTENTS_BODY | CONTENTS_CORPSE,
             (EG2_Collision)0, 0);
  ...
  if (!(NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
    ...
    // There is an event to look at
    if (alertEvent >= 0) {
      if (NPC_CheckForDanger(alertEvent)) { // going to run?
        ...
        // FIXME: what if can't actually see enemy, don't know where he is...
        // should we make them just become very alert and start looking for him?
        // Or just let combat AI handle this... (act as if you lost him)
        // ST_Speech( NPC, SPEECH_CHARGE, 0 );
        G_SetEnemy(NPC, level.alertEvents[alertEvent].owner);
        NPCInfo->enemyLastSeenTime = level.time;
        TIMER_Set(NPC, "attackDelay", Q_irand(500, 2500));
        return;
      } else if (NPC_ST_InvestigateEvent(
                     alertEvent, qfalse)) { // actually going to investigate it
        NPC_UpdateAngles(qtrue, qtrue);
        return;
      }
    }
  ...
  if (NPC->client->NPC_class == CLASS_IMPERIAL ||
    ...
    if (NPC->client->ps.weapon != WP_CONCUSSION) {             // not Rax
...
static void ST_CheckMoveState(void) {
  ...
  if ((NPCInfo->goalEntity != NPC->enemy) && (NPCInfo->goalEntity != NULL)) {
    ...
    if (STEER::Reached(NPC, NPCInfo->goalEntity, 16, !!FlyingCreature(NPC)) ||
        (enemyLOS && (NPCInfo->aiFlags & NPCAI_STOP_AT_LOS) &&
         !Q3_TaskIDPending(
             NPC,
             TID_MOVE_NAV))) { // either hit our navgoal or our navgoal was not
                               // a crucial (scripted) one (maybe a combat point)
                               // and we're scouting and found our enemy
      ...
      TIMER_Set(NPC, "roamTime",
                Q_irand(8000, 15000)); // Q_irand( 1000, 4000 ) );
      if (Q_irand(0, 3) == 0) {
        TIMER_Set(
            NPC, "duck",
            Q_irand(5000,
                    10000)); // just reached our goal, chance of ducking now
      }
      return;
    }
    ...
    if (AI_GroupContainsEntNum(NPCInfo->group, hit)) {
      ...
      if (TIMER_Done(member, "duck")) {    // they aren't ducking
        if (TIMER_Done(member, "stand")) { // they're not being forced to stand
          ...
          TIMER_Set(
              member, "duck",
              stuckTime); // tell my friend to duck so I can shoot over his head
          return;
        }
      }
    }
  ...
  if (self->NPC->combatPoint !=
    ...
    if (self->NPC->goalEntity == self->NPC->tempGoal) {
      NPC_SetMoveGoal(
          ...
          (qboolean)((self->NPC->tempGoal->svFlags & SVF_NAVGOAL) != 0));
    } else {
  ...
  if (NPC && NPCInfo->group) {
    ...
    /*		else if ( NPCInfo->group->morale < NPCInfo->group->numGroup )
                    {//morale is low for our size
                            int moraleDrop = NPCInfo->group->numGroup -
       NPCInfo->group->morale; if ( moraleDrop < -6 )
                            {//flee (no clear shot needed)
                                    cpFlags =
       (CP_FLEE|CP_RETREAT|CP_COVER|CP_AVOID|CP_SAFE);
                            }
                            else if ( moraleDrop < -3 )
                            {//retreat (no clear shot needed)
                                    cpFlags =
       (CP_RETREAT|CP_COVER|CP_AVOID|CP_SAFE);
                            }
...
void ST_Commander(void) {
  ...
  if (group->lastSeenEnemyTime < level.time - 180000) { // dissolve the group
    ...
    for (i = 0; i < group->numGroup; i++) {
      ...
      if (!(NPCInfo->scriptFlags &
            SCF_CHASE_ENEMIES)) { // not allowed to doMove on my own
        continue;
      }
  ...
  if (group->lastSeenEnemyTime <
      level.time - 7000) { // no-one has seen the enemy for at least 10 seconds!
                           // Should send a scout
    enemyLost = qtrue;
  }
  ...
  for (i = curMemberNum; i < lastMemberNum; i++) {
    ...
    if (TIMER_Done(NPC, "checkGrenadeTooCloseDebouncer")) {
      ...
      for (int e = 0; e < numListedEntities; e++) {
        ...
        if (ent->s.eType == ET_MISSILE) {
          if (ent->s.weapon == WP_THERMAL) { // a thermal
            if (ent->has_bounced &&
                (!ent->owner ||
                 !OnSameTeam(ent->owner,
                             NPC))) {          // bounced and an enemy thermal
              ST_Speech(NPC, SPEECH_COVER, 0); // FIXME: flee sound?
              NPC_StartFlee(NPC->enemy, ent->currentOrigin, AEL_DANGER_GREAT,
                            1000, 2000);
              fled = true;
              //							cpFlags
              //|= (CP_CLEAR|CP_COVER);	// NOPE, Can't See The Enemy, So Find A
              //New Combat Point
              TIMER_Set(NPC, "checkGrenadeTooCloseDebouncer",
                        Q_irand(2000, 4000));
              break;
            }
          }
    ...
    if (TIMER_Done(NPC, "checkEnemyVisDebouncer")) {
      ...
      if (!NPC_ClearLOS(NPC->enemy)) {
        cpFlags |=
            (CP_CLEAR |
             CP_COVER); // NOPE, Can't See The Enemy, So Find A New Combat Point
      }
    }
    ...
    if (cpFlags) { // we want to run to a combat point
      // always avoid enemy when picking combat points, and we always want to be
      // able to get there
      ...
      if (cp == -1) { // may have had sone set above
        cp = NPC_FindCombatPointRetry(NPC->currentOrigin, NPC->currentOrigin,
                                      NPC->currentOrigin, &cpFlags, avoidDist,
      ...
      if (cp != -1) { // found a combat point
        ...
        // If Successfully
        if ((cpFlags & CP_FLANK) ||
            ((cpFlags & CP_COVER) && (cpFlags & CP_CLEAR))) {
        ...
        if ((cpFlags & CP_FLANK)) {
          if (group->numGroup > 1) {
            NPC_ST_StoreMovementSpeech(SPEECH_OUTFLANK, -1);
                   ...
                   !(cpFlags & CP_CLEAR)) { // going into hiding
          NPC_ST_StoreMovementSpeech(SPEECH_COVER, -1);
        } else {
          if (!Q_irand(0, 20)) { // hell, we're loading the sounds, use them
                                 // every now and then!
            if (Q_irand(0, 1)) {
              NPC_ST_StoreMovementSpeech(SPEECH_OUTFLANK, -1);
            } else {
...
void Noghri_StickTrace(void) {
  ...
  if (boltIndex != -1) {
    ...
    for (int time = curTime - 25; time <= curTime + 25 && !hit; time += 25) {
      ...
      gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->weaponModel[0], boltIndex,
...
#endif
      gi.trace(&trace, base, mins, maxs, tip, NPC->s.number, MASK_SHOT,
               G2_RETURNONHIT, 10);
      if (trace.fraction < 1.0f && trace.entityNum != lastHit) { // hit
                                                                 // something
        ...
        if (traceEnt->takedamage &&
          ...
          // FIXME: debounce?
          G_Sound(traceEnt,
                  G_SoundIndex(va("sound/weapons/tusken_staff/stickhit%d.wav",
                                  Q_irand(1, 4))));
          G_Damage(traceEnt, NPC, NPC, vec3_origin, trace.endpos, dmg,
                   DAMAGE_NO_KNOCKBACK, MOD_MELEE);
          if (traceEnt->health > 0 && dmg > 17) { // do pain on enemy
...
void NPC_BSST_Attack(void) {
  ...
  {
    if (NPC->client->ps.weapon == WP_DISRUPTOR) {   // sniping...
  ...
  if (!(NPCInfo->scriptFlags &
    ...
    if (NPCInfo->goalEntity ==
        NPC->enemy) { // goal is my entity, so don't doMove
      doMove = qfalse;
    }
  } else if (NPC->NPC->scriptFlags & SCF_NO_GROUPS) {
    //	NPCInfo->goalEntity = UpdateGoal();
  ...
  if (!ucmd.rightmove) { // only if not already strafing for some strange
                         // reason...?
    // NOTE: these are never set here, but can be set in AI_Jedi.cpp for those
    // NPCs who are sort of Stormtrooper/Jedi hybrids NOTE: this stomps
    // navigation movement entirely!
  ...
  if (NPC->client->ps.legsAnim ==
      BOTH_GUARD_LOOKAROUND1) { // don't doMove when doing silly look around
                                // thing
    doMove = qfalse;
  }
  if (doMove) {              // doMove toward goal
                             ...
                             //enemyDist > 10000 ) )//100 squared
    {
      ...
      if ((NPC->client->NPC_class != CLASS_ROCKETTROOPER ||
           ...
           enemyDist <
               MIN_ROCKET_DIST_SQUARED) // rockettroopers who use rocket
                                        // launchers turn around and run if you
                                        // get too close (closer than 128)
          && ucmd.forwardmove <= -32) { // moving backwards at least 45 degrees
        if (NPCInfo->goalEntity &&
            DistanceSquared(NPCInfo->goalEntity->currentOrigin,
                            NPC->currentOrigin) >
                MIN_TURN_AROUND_DIST_SQ) { // don't stop running backwards if
                                           // your goal is less than 100 away
          if (TIMER_Done(NPC,
                         "runBackwardsDebounce")) { // not already waiting for
                                                    // next run backwards
            if (!TIMER_Exists(NPC,
                              "runningBackwards")) { // start running backwards
              TIMER_Set(NPC, "runningBackwards",
                        Q_irand(500, 1000)); // Q_irand( 2000, 3500 ) );
            } else if (TIMER_Done2(NPC, "runningBackwards",
                                   qtrue)) { // done running backwards
              TIMER_Set(NPC, "runBackwardsDebounce", Q_irand(3000, 5000));
            }
          }
        }
      } else { // not running backwards
  ...
  if ( //! TIMER_Done( NPC, "flee" ) ||
      (doMove && !TIMER_Done(NPC, "runBackwardsDebounce"))) { // running away
    faceEnemy = qfalse;
  }
  ...
  if (!faceEnemy) { // we want to face in the dir we're running
    if (!doMove) {  // if we haven't moved, we should look in the direction we
                   // last looked?
  ...
  if (NPC->client->fireDelay) {
    ...
    if (TIMER_Done(NPC, "attackDelay")) {
      ...
      // NASTY
      if (NPC->s.weapon == WP_ROCKET_LAUNCHER) {
        if ((ucmd.buttons & BUTTON_ATTACK) && !doMove &&
            g_spskill->integer > 1 &&
        ...
        && (!NPC->enemy->enemy || !NPC->enemy->enemy->client ||
            (NPC->enemy->enemy->client->NPC_class != CLASS_RANCOR &&
             NPC->enemy->enemy->client->NPC_class !=
                 CLASS_WAMPA))) // enemy's enemy is not a client or is not a
                                // wampa or rancor (which is scarier than me)
