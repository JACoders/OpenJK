...
void CG_AddFragment(localEntity_t *le) {
  vec3_t newOrigin;
  trace_t trace;
  // used to sink into the ground, but it looks better to maybe just fade them
  // out
  int t;
  ...
  CG_Trace(&trace, le->refEntity.origin, NULL, NULL, newOrigin,
           le->ownerGentNum, CONTENTS_SOLID);
...
static void CG_AddLocalLight(localEntity_t *le) {
  ...
  if (le->light) {
    ...
    cgi_R_AddLightToScene(le->refEntity.origin, light, le->lightColor[0],
...
static void CG_AddFadeModel(localEntity_t *le) {
  ...
  float frac = 1.0f - ((float)(cg.time - le->startTime) /
...
// create a quad that doesn't use a refEnt.  Currently only for use with the
// DebugNav drawing so it doesn't have to use fx
//------------------------------------------
...
// create a sprite that doesn't use a refEnt.  Currently only for use with the
// DebugNav drawing so it doesn't have to use fx
//------------------------------------------
