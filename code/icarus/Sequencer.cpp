...
// using a chunks for EVERYTHING is vastly inefficient and wasteful. Thus, the
// saving is now primary focused on saving large chunks with *expected* data
// read from there.
...
int CSequencer::Flush(CSequence *owner, CIcarus *icarus) {
  ...
  /*	sequenceID_m::iterator iterSeq = NULL;
          ...
          {
                  if ( ( (*iterSeq).second == owner ) || ( owner->HasChild(
     (*iterSeq).second ) ) || ( (*iterSeq).second->HasFlag(
     CSequence::SQ_PENDING ) ) || ( (*iterSeq).second->HasFlag(
     CSequence::SQ_TASK ) ) )
                  {
                          iterSeq++;
                          continue;
  ...
  for (sli = m_sequences.begin(); sli != m_sequences.end();) {
    if (((*sli) == owner) || (owner->HasChild((*sli))) ||
        ((*sli)->HasFlag(CSequence::SQ_PENDING)) ||
        ((*sli)->HasFlag(CSequence::SQ_TASK))) {
      ++sli;
      continue;
    }
...
int CSequencer::Run(char *buffer, long size, CIcarus *icarus) {
  ...
  CSequence *sequence =
      AddSequence(NULL, m_curSequence, CSequence::SQ_COMMON, icarus);
...
int CSequencer::ParseRun(CBlock *block, CIcarus *icarus) {
  ...
  new_sequence =
      AddSequence(m_curSequence, m_curSequence,
                  (CSequence::SQ_RUN | CSequence::SQ_PENDING), icarus);
...
int CSequencer::ParseIf(CBlock *block, bstream_t *bstream, CIcarus *icarus) {
  ...
  sequence = AddSequence(m_curSequence, m_curSequence,
                         CSequence::SQ_CONDITIONAL, icarus);

  assert(sequence);
  if (sequence == NULL) {
    game->DebugPrint(IGameInterface::WL_ERROR,
                     "ParseIf: failed to allocate container sequence");
    block->Free(icarus);
    delete block;
    block = NULL;
    return SEQ_FAILED;
...
int CSequencer::ParseLoop(CBlock *block, bstream_t *bstream, CIcarus *icarus) {
  ...
  // Set the parent
  sequence = AddSequence(m_curSequence, m_curSequence,
                         (CSequence::SQ_LOOP | CSequence::SQ_RETAIN), icarus);
...
int CSequencer::AddAffect(bstream_t *bstream, int retain, int *id,
  ...
  if
    S_FAILED(Route(sequence, &new_stream, icarus)) { return SEQ_FAILED; }
...
int CSequencer::ParseAffect(CBlock *block, bstream_t *bstream,
  ...
  if (ent < 0) // if there wasn't a valid entname in the affect, we need to
               // check if it's a get command
  {
    ...
    if (ent < 0) { // a valid entity name was not returned from the get command
      game->DebugPrint(IGameInterface::WL_WARNING,
                       "'%s' : invalid affect() target\n");
    }
  ...
  if
    S_FAILED(stream_sequencer->AddAffect(
        bstream, (int)m_curSequence->HasFlag(CSequence::SQ_RETAIN), &ret,
        icarus)) {
      block->Free(icarus);
      delete block;
      block = NULL;
...
int CSequencer::ParseTask(CBlock *block, bstream_t *bstream, CIcarus *icarus) {
  ...
  sequence = AddSequence(m_curSequence, m_curSequence,
                         CSequence::SQ_TASK | CSequence::SQ_RETAIN, icarus);
  m_curSequence->AddChild(sequence);
  ...
  if (group == NULL) {
    game->DebugPrint(IGameInterface::WL_ERROR,
                     "error : unable to allocate a new task group");
    block->Free(icarus);
    delete block;
    block = NULL;
    return SEQ_FAILED;
  ...
  while (stream->BlockAvailable()) {
    ...
    case CIcarus::ID_BLOCK_END:
      ...
      if (m_curSequence->HasFlag(CSequence::SQ_RUN) ||
          m_curSequence->HasFlag(CSequence::SQ_AFFECT)) {
    ...
    case CIcarus::ID_AFFECT:
      ...
      if
        S_FAILED(ParseAffect(block, bstream, icarus))
      return SEQ_FAILED;
    ...
    case CIcarus::ID_RUN:
      ...
      if
        S_FAILED(ParseRun(block, icarus))
      return SEQ_FAILED;
    ...
    case CIcarus::ID_LOOP:
      ...
      if
        S_FAILED(ParseLoop(block, bstream, icarus))
      return SEQ_FAILED;
    ...
    case CIcarus::ID_IF:
      ...
      if
        S_FAILED(ParseIf(block, bstream, icarus))
      return SEQ_FAILED;
    ...
    case CIcarus::ID_ELSE:
      ...
      if
        S_FAILED(ParseElse(block, bstream, icarus))
      return SEQ_FAILED;
    ...
    case CIcarus::ID_TASK:
      ...
      if
        S_FAILED(ParseTask(block, bstream, icarus))
      return SEQ_FAILED;
    ...
    default:
      ...
      game->DebugPrint(IGameInterface::WL_ERROR, "'%d' : invalid block ID",
                       block->GetBlockID());
...
void CSequencer::CheckRun(CBlock **command, CIcarus *icarus) {
  ...
  if (block->GetBlockID() == CIcarus::ID_RUN) {
    ...
    game->DebugPrint(IGameInterface::WL_DEBUG, "%4d run( \"%s\" ); [%d]",
                     m_ownerID, (char *)block->GetMemberData(0),
                     game->GetTime());
  ...
  if ((block->GetBlockID() == CIcarus::ID_BLOCK_END) &&
      (m_curSequence->HasFlag(CSequence::SQ_RUN))) {
    if (m_curSequence->HasFlag(CSequence::SQ_RETAIN)) {
      PushCommand(block, CSequence::PUSH_FRONT);
    } else {
      block->Free(icarus);
...
void CSequencer::CheckIf(CBlock **command, CIcarus *icarus) {
  ...
  if ((block->GetBlockID() == CIcarus::ID_BLOCK_END) &&
      (m_curSequence->HasFlag(CSequence::SQ_CONDITIONAL))) {
...
void CSequencer::CheckLoop(CBlock **command, CIcarus *icarus) {
  ...
  if ((block->GetBlockID() == CIcarus::ID_BLOCK_END) &&
      (m_curSequence->HasFlag(CSequence::SQ_LOOP))) {
...
void CSequencer::CheckAffect(CBlock **command, CIcarus *icarus) {
  ...
  if (block->GetBlockID() == CIcarus::ID_AFFECT) {
                 ...
                 // check if it's a get command
    {
      ...
      if (ent <
          0) { // a valid entity name was not returned from the get command
        game->DebugPrint(IGameInterface::WL_WARNING,
                         "'%s' : invalid affect() target\n");
      }
    ...
    *command = PopCommand(CSequence::POP_BACK);
    Prep(command, icarus);
    if (ent >= 0) { // ents need to update upon being affected
      int sequencerID = game->CreateIcarus(ent);
...
void CSequencer::CheckDo(CBlock **command, CIcarus *icarus) {
  ...
  if (block->GetBlockID() == CIcarus::ID_DO) {
    ...
    if (group == NULL) {
      ...
      game->DebugPrint(IGameInterface::WL_ERROR,
                       "ICARUS Unable to find task group \"%s\"!\n", groupName);
      *command = NULL;
      return;
    }
  ...
  if ((block->GetBlockID() == CIcarus::ID_BLOCK_END) &&
      (m_curSequence->HasFlag(CSequence::SQ_TASK))) {
    if (m_curSequence->HasFlag(CSequence::SQ_RETAIN)) {
      PushCommand(block, CSequence::PUSH_FRONT);
    } else {
      block->Free(icarus);
...
int CSequencer::Callback(CTaskManager *taskManager, CBlock *block,
  ...
  if (returnCode == TASK_RETURN_COMPLETE) {
    ...
    if (m_curSequence->HasFlag(
            CSequence::SQ_RETAIN)) // This isn't true for affect sequences...?
    {
      PushCommand(block, CSequence::PUSH_FRONT);
    } else {
      block->Free(icarus);
  ...
  game->DebugPrint(IGameInterface::WL_ERROR,
                   "command could not be called back\n");
  assert(0);
...
int CSequencer::Load(CIcarus *icarus, IGameInterface *game) {
  ...
  m_curGroup = (curGroupID == -1)
                   ? NULL
                   : m_taskManager->GetTaskGroup(curGroupID, icarus);
  ...
  m_curSequence =
      (seqID != -1) ? (CSequence *)icarus->GetSequence(seqID) : NULL;
