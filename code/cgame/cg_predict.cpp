  ...
  for (i = 0; i < cg_numpermanents; i++) {
    ...
    if (cent->currentState.eType == ET_TERRAIN ||
        ((difference[0] * difference[0]) + (difference[1] * difference[1]) +
         (difference[2] * difference[2])) <= dsquared) {
...
void CG_ClipMoveToEntities(const vec3_t start, const vec3_t mins,
  ...
  for (i = 0; i < cg_numSolidEntities; i++) {
    ...
    if (ent->solid == SOLID_BMODEL) {
      ...
      // Hmm... this would cause traces against brush movers to snap at 20fps
      // (as with the third person camera)... Let's use the lerpOrigin for now
      // and see if it breaks anything... EvaluateTrajectory(
      // &cent->currentState.pos, cg.snap->serverTime, origin );
    ...
    cgi_CM_TransformedBoxTrace(&trace, start, end, mins, maxs, cmodel, mask,
                               origin, angles);
...
void CG_Trace(trace_t *result, const vec3_t start, const vec3_t mins,
              ...
              const int mask,
              const EG2_Collision eG2TraceType /*=G2_NOCOLLIDE*/,
              const int useLod /*=0*/) {
  trace_t t;
...
int CG_PointContents(const vec3_t point, int passEntityNum) {
  ...
  for (i = 0; i < cg_numSolidEntities; i++) {
    ...
    contents |= cgi_CM_TransformedPointContents(point, cmodel, ent->origin,
                                                ent->angles);
  }
...
void CG_SetClientViewAngles(vec3_t angles, qboolean overrideViewEnt) {
  if (cg.snap->ps.viewEntity <= 0 ||
      ...
      overrideViewEnt) { // don't clamp angles when looking through a viewEntity
    for (int i = 0; i < 3; i++) {
...
qboolean CG_CheckModifyUCmd(usercmd_t *cmd, vec3_t viewangles) {
  ...
  if (cg.snap->ps.viewEntity > 0 &&
      cg.snap->ps.viewEntity < ENTITYNUM_WORLD) { // controlling something else
    memset(cmd, 0, sizeof(usercmd_t));
    /*
    //to keep pointing in same dir, need to set cmd.angles
    cmd->angles[PITCH] = ANGLE2SHORT( cg.snap->ps.viewangles[PITCH] ) -
    cg.snap->ps.delta_angles[PITCH]; cmd->angles[YAW] = ANGLE2SHORT(
    cg.snap->ps.viewangles[YAW] ) - cg.snap->ps.delta_angles[YAW];
    cmd->angles[ROLL] = 0;
    */
    VectorCopy(g_entities[0].pos4, viewangles);
    overridAngles = qtrue;
    // CG_SetClientViewAngles(
    // g_entities[cg.snap->ps.viewEntity].client->ps.viewangles, qtrue );
  } else if (G_IsRidingVehicle(&g_entities[0])) {
    overridAngles = qtrue;
    /*
    int vehIndex = g_entities[0].owner->client->ps.vehicleIndex;
    if ( vehIndex != VEHICLE_NONE
            && (vehicleData[vehIndex].type == VH_FIGHTER ||
    (vehicleData[vehIndex].type == VH_SPEEDER )) )
    {//in vehicle flight mode
            ...
            if ( !speed || cg.snap->ps.groundEntityNum != ENTITYNUM_NONE )
            {
                    cmd->rightmove = 0;
                    cmd->angles[PITCH] = 0;
                    cmd->angles[YAW] = ANGLE2SHORT( cg.snap->ps.viewangles[YAW]
    ) - cg.snap->ps.delta_angles[YAW]; CG_SetClientViewAngles(
    cg.snap->ps.viewangles, qfalse );
            }
    }
    */
  }
    ...
    if (es->eType == ET_MOVER) { // on a mover
      if (es->pos.trType != TR_STATIONARY) {
        if (es->pos.trType != TR_LINEAR_STOP &&
            es->pos.trType != TR_NONLINEAR_STOP) { // a constant mover
...
void CG_InterpolatePlayerState(qboolean grabAngles) {
  ...
  *out = cg.snap->ps;
  ...
  if (grabAngles) {
    ...
    if (!skip) {
      // NULL so that it doesn't execute a block of code that must be run from
      // game
  ...
  if (cg.validPPS && cg_smoothPlayerPos.value > 0.0f &&
      cg_smoothPlayerPos.value < 1.0f && !onPlat) {
    // 0 = no smoothing, 1 = no movement
    for (i = 0; i < 3; i++) {
      out->origin[i] = cg_smoothPlayerPos.value * (oldOrg[i] - out->origin[i]) +
                       out->origin[i];
    }
  } else if (onPlat && cg_smoothPlayerPlat.value > 0.0f &&
    ...
    if (cg.validPPS && cg_smoothPlayerPlat.value > 0.0f &&
        cg_smoothPlayerPlat.value < 1.0f) {
      // 0 = no smoothing, 1 = no movement
      for (i = 0; i < 3; i++) {
        out->origin[i] =
            cg_smoothPlayerPlat.value * (oldOrg[i] - out->origin[i]) +
            out->origin[i];
      }
    }
    //		}
...
void CG_TouchItem(centity_t *cent) {
  ...
  if (!BG_PlayerTouchesItem(&cg.predicted_player_state, &cent->currentState,
                            cg.time)) {
    return;
  }
  ...
  // grab it
  AddEventToPlayerstate(EV_ITEM_PICKUP, cent->currentState.modelindex,
...
void CG_TouchTriggerPrediction(void) {
  ...
  for (i = 0; i < cg.snap->numEntities; i++) {
    ...
    cgi_CM_BoxTrace(&trace, cg.predicted_player_state.origin,
                    cg.predicted_player_state.origin, cg_pmove.mins,
...
void CG_PredictPlayerState(void) {
  ...
  do {
    ...
    gentity_t *ent = &g_entities[0]; // cheating and dirty, I know, but this is
                                     // a SP game so prediction can cheat
    if (player_locked ||
        (ent && !ent->s.number && ent->aimDebounceTime > level.time) ||
        (ent && ent->client && ent->client->ps.pm_time &&
         (ent->client->ps.pm_flags & PMF_TIME_KNOCKBACK)) ||
        (ent && ent->forcePushTime >
                    level.time)) { // lock out player control unless dead
  ...
  CG_AdjustPositionForMover(cg.predicted_player_state.origin,
                            cg.predicted_player_state.groundEntityNum, cg.time,
