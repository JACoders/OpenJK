...
//
// (filenames are used purely for error reporting, all files should already be
// loaded before you get here)
//
...
int MP3_UnpackRawPCM(const char *psLocalFilename, void *pvData, int iDataLen,
                     byte *pbUnpackBuffer, qboolean bStereoDesired = qfalse);
qboolean MP3Stream_InitPlayingTimeFields(LP_MP3STREAM lpMP3Stream,
                                         const char *psLocalFilename,
                                         void *pvData, int iDataLen,
                                         qboolean bStereoDesired = qfalse);
...
///////////////////////////////////////
//
// the real worker code deep down in the MP3 C code...  (now externalised here
// so the music streamer can access one)
//
#ifdef __cplusplus
extern "C" {
#endif
...
char *C_MP3_GetUnpackedSize(void *pvData, int iDataLen, int *piUnpackedSize,
                            int bStereoDesired);
char *C_MP3_UnpackRawPCM(void *pvData, int iDataLen, int *piUnpackedSize,
                         void *pbUnpackBuffer, int bStereoDesired);
