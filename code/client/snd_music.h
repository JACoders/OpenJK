...
typedef enum {
  ...
  eBGRNDTRACK_EXPLORETRANS3, // "
                             //
  //(	eBGRNDTRACK_DATAEND ),			// tracks from this point on are for
  //logic or copies, do NOT free them.
  //
  eBGRNDTRACK_NONDYNAMIC, // used for when music is just streaming, not part of
                          // dynamic stuff (used to be defined as same as
                          // explore entry, but this allows playing music in
                          // between 2 invokations of the same dynamic music
                          // without midleve reload, and also faster level
                          // transitioning if two consecutive dynamic sections
                          // use same DMS.DAT entries. Are you still reading
                          // this far?
  eBGRNDTRACK_SILENCE, // silence (more of a logic thing than an actual track at
                       // the moment)
...
#define iMAX_ACTION_TRANSITIONS                                                \
  4 // these can be increased easily enough, I just need to know about them
#define iMAX_EXPLORE_TRANSITIONS 4 //

#define eBGRNDTRACK_DATABEGIN                                                  \
  eBGRNDTRACK_EXPLORE // label for FOR() loops (not in enum, else debugger shows
                      // in instead of the explore one unless I declare them
                      // backwards, which is gay)
#define eBGRNDTRACK_DATAEND                                                    \
  eBGRNDTRACK_NONDYNAMIC // tracks from this point on are for logic or copies,
                         // do NOT free them.

#define eBGRNDTRACK_FIRSTTRANSITION                                            \
  eBGRNDTRACK_ACTIONTRANS0 // used for "are we in transition mode" check
