...
int ICARUS_RunScript(gentity_t *ent, const char *name) {
  ...
  if
    S_FAILED(ent->sequencer->Run(buf, len))
  return false;
...
bool ICARUS_RegisterScript(const char *name,
                           bool bCalledDuringInterrogate /* = false */) {
  bufferlist_t::iterator ei;
  pscript_t *pscript;
...
void ICARUS_InterrogateScript(const char *filename) {
  ...
  if (ICARUS_RegisterScript(sFilename, true) ==
...
void Svcmd_ICARUS_f(void) {
  ...
  if (Q_stricmp(cmd, "log") == 0) {
    ...
    if (VALIDSTRING(gi.argv(2))) {
      ...
      ICARUS_entFilter =
          (ent->s.number == ICARUS_entFilter) ? -1 : ent->s.number;
