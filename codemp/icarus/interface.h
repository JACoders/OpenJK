...
typedef struct interface_export_s {
  ...
  sharedEntity_t *(*I_GetEntityByName)(
      const char *name); // Polls the engine for the sequencer of the entity
                         // matching the name passed
  unsigned int (*I_GetTime)(void); // Gets the current time
  unsigned int (*I_GetTimeScale)(void);
  int (*I_PlaySound)(int taskID, int entID, const char *name,
                     const char *channel);
  void (*I_Lerp2Pos)(int taskID, int entID, vec3_t origin, vec3_t angles,
                     float duration);
  ...
  void (*I_CameraFade)(float sr, float sg, float sb, float sa, float dr,
  ...
  int (*I_Evaluate)(int p1Type, const char *p1, int p2Type, const char *p2,
                    int operatorType);
  ...
  // Below changed by BTO (VV). Visual C++ 7.1 compiler no longer allows default
  // args on function pointers. Ack.
  int (*I_ReadSaveData)(unsigned long chid, void *address,
                        int length /* , void **addressptr = NULL */);
  int (*I_LinkEntity)(int entID, CSequencer *sequencer,
