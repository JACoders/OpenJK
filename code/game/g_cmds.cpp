...
enum { TAUNT_TAUNT = 0, TAUNT_BOW, TAUNT_MEDITATE, TAUNT_FLOURISH, TAUNT_GLOAT };
...
void G_SetTauntAnim(gentity_t *ent, int taunt) {
	...
	if (!ent->client->ps.torsoAnimTimer && !ent->client->ps.legsAnimTimer && !ent->client->ps.weaponTime && ent->client->ps.saberLockTime < level.time) {
		...
		case TAUNT_TAUNT:
			...
			} else if (ent->client->ps.dualSabers && ent->client->ps.saber[1].tauntAnim != -1) {
		...
		case TAUNT_BOW:
			...
			} else if (ent->client->ps.dualSabers && ent->client->ps.saber[1].bowAnim != -1) {
		...
		case TAUNT_GLOAT:
			if (ent->client->ps.weapon == WP_SABER) {
				...
				} else if (ent->client->ps.dualSabers && ent->client->ps.saber[1].gloatAnim != -1) {
...
void Cmd_SaberDrop_f(gentity_t *ent, int saberNum) {
	...
	if (ent->client->ps.saberMove != LS_READY && ent->client->ps.saberMove != LS_PUTAWAY && ent->client->ps.saberMove != LS_DRAW &&
	...
	if (!ent->client->ps.saber[saberNum].name || !ent->client->ps.saber[saberNum].name[0]) {
		return;
	}
	...
	if (G_DropSaberItem(ent->client->ps.saber[saberNum].name, ent->client->ps.saber[saberNum].blade[0].color,
						(saberNum == 0 ? ent->client->renderInfo.handRPoint : ent->client->renderInfo.handLPoint), ent->client->ps.velocity,
