...
void SetGoal(gentity_t *goal, float rating) {
  ...
  if (goal) {
    //		Debug_NPCPrintf( NPC, d_npcai, DEBUG_LEVEL_INFO, "NPC_SetGoal: %s @ %s
...
void NPC_SetGoal(gentity_t *goal, float rating) {
  ...
  if (!goal) {
    //		Debug_NPCPrintf( NPC, d_npcai, DEBUG_LEVEL_ERROR, "NPC_SetGoal: NULL
    //goal\n" );
    return;
  }

  if (goal->client) {
    //		Debug_NPCPrintf( NPC, d_npcai, DEBUG_LEVEL_ERROR, "NPC_SetGoal: goal
    //is a client\n" );
    return;
  }

  if (NPCS.NPCInfo->goalEntity) {
    //		Debug_NPCPrintf( NPC, d_npcai, DEBUG_LEVEL_INFO, "NPC_SetGoal: push
...
void NPC_ClearGoal(void) {
  ...
  if (goal->inuse && !(goal->s.eFlags & EF_NODRAW)) {
    //		Debug_NPCPrintf( NPC, d_npcai, DEBUG_LEVEL_INFO, "NPC_ClearGoal: pop
    //%s\n", goal->classname );
...
qboolean G_BoundsOverlap(
    const vec3_t mins1, const vec3_t maxs1, const vec3_t mins2,
    const vec3_t maxs2) { // NOTE: flush up against counts as overlapping
  if (mins1[0] > maxs2[0])
    return qfalse;
...
void NPC_ReachedGoal(void) {
  //	Debug_NPCPrintf( NPC, d_npcai, DEBUG_LEVEL_INFO, "UpdateGoal: reached
  //goal entity\n" );
  NPC_ClearGoal();
  NPCS.NPCInfo->goalTime = level.time;
...
*/
// qboolean NAV_HitNavGoal( vec3_t point, vec3_t mins, vec3_t maxs, gentity_t
// *goal, qboolean flying );
qboolean ReachedGoal(gentity_t *goal) {
  ...
  /*
          ...
          {//waypoint_navgoal
                  return NAV_HitNavGoal( NPC->currentOrigin, NPC->mins,
     NPC->maxs, goal, FlyingCreature( NPC ) );
          }
          ...
          {//MUST touch waypoints, even if moving to it
                  ...
                  //surface and the tempGoal in in the FOV - does NOT check
     distance!
                  // are we on same surface?

                  //FIXME: NPC->waypoint reset every frame, need to find it
     first
                  //Should we do that here?  (Still will do it only once per
     frame) if ( NPC->waypoint >= 0 && NPC->waypoint < num_waypoints )
                  {
                  ...
                  VectorSubtract ( NPCInfo->tempGoal->currentOrigin,
     NPC->currentOrigin, vec);
                  //Who cares if it's in our FOV?!
                  /*
                  // is it in our FOV
                  vectoangles ( vec, angles );
                  delta = AngleDelta ( NPC->client->ps.viewangles[YAW],
     angles[YAW] ); if ( fabs ( delta ) > NPCInfo->stats.hfov )
                  {
                          return qfalse;
                  }
                  */
  ...
  */
  ...
  /*
          ...
          {//trying to get to dead guy
                  ...
                  if ( NPC->waypoint == goalWpNum )
                  {
                          VectorSubtract(NPC->currentOrigin,
     goal->currentOrigin, vec); vec[2] = 0; delta = VectorLengthSquared(vec);
                          if(delta <= 800)
  ...
  */
  return NAV_HitNavGoal(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins,
                        NPCS.NPC->r.maxs, goal->r.currentOrigin,
...
Id removed a lot of shit here... doesn't seem to handle waypoints independently
of goalentity

In fact, doesn't seem to be any waypoint info on entities at all any more?

MCG - Since goal is ALWAYS goalEntity, took out a lot of sending goal entity
pointers around for no reason
*/
...
gentity_t *UpdateGoal(void) {
  ...
  if (!NPCS.NPCInfo->goalEntity
           ->inuse) { // Somehow freed it, but didn't clear it
    NPC_ClearGoal();
    return NULL;
  }
