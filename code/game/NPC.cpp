...
void CorpsePhysics(gentity_t *self) {
	...
	if (self->client->ps.groundEntityNum != ENTITYNUM_NONE && !(self->flags & FL_DISINTEGRATED)) { // on the ground
	...
	if (level.time - self->s.time > 3000) {									   // been dead for 3 seconds
		if (!debug_subdivision->integer && !g_saberRealisticCombat->integer) { // can't be dismembered once dead
	...
	if (level.time - self->s.time > 500) { // don't turn "nonsolid" until about 1 second after actual death
...
qboolean G_OkayToRemoveCorpse(gentity_t *self) {
	...
	if (self->client && self->client->NPC_class != CLASS_VEHICLE && self->s.m_iVehicleNum != 0) {
		...
		if (pVeh) {
			if (!pVeh->m_pVehicleInfo->Eject(pVeh, self, qtrue)) { // dammit, still can't get off the vehicle...
				return qfalse;
			}
		} else {
			assert(0);
	...
	if (IIcarusInterface::GetIcarus()->IsRunning(self->m_iIcarusID)) { // still running a script
		return qfalse;
	}

	if (self->activator && self->activator->client &&
		((self->activator->client->ps.eFlags & EF_HELD_BY_RANCOR) || (self->activator->client->ps.eFlags & EF_HELD_BY_SAND_CREATURE) ||
		 (self->activator->client->ps.eFlags & EF_HELD_BY_WAMPA))) { // still holding a victim?
		return qfalse;
	}

	if (self->client && ((self->client->ps.eFlags & EF_HELD_BY_RANCOR) || (self->client->ps.eFlags & EF_HELD_BY_SAND_CREATURE) ||
						 (self->client->ps.eFlags & EF_HELD_BY_WAMPA))) { // being held by a creature
		return qfalse;
	}
...
void NPC_RemoveBody(gentity_t *self) {
	...
	if (self->NPC->nextBStateThink <= level.time) { // run logic at 10 fps
		...
		if (self->client->NPC_class == CLASS_REMOTE || self->client->NPC_class == CLASS_SENTRY || self->client->NPC_class == CLASS_PROBE ||
			self->client->NPC_class == CLASS_INTERROGATOR || self->client->NPC_class == CLASS_PROBE || self->client->NPC_class == CLASS_MARK2) {
			G_FreeEntity(self);
			return;
		}
		...
		if ((self->NPC->aiFlags & NPCAI_HEAL_ROSH)) { // kothos twins' bodies are never removed
			return;
		}
...
void pitch_roll_for_slope(gentity_t *forwhom, vec3_t pass_slope, vec3_t storeAngles, qboolean keepPitch) {
	...
	float oldPitch = 0;
	if (forwhom->client && forwhom->client->NPC_class == CLASS_VEHICLE) { // special code for vehicles
...
static void DeadThink(void) {
	...
	if (NPC->maxs[2] > oldMaxs2) { // inflating maxs, make sure we're not inflating into solid
...
qboolean G_JediInNormalAI(gentity_t *ent) { // NOTE: should match above func's switch!
	// check our bState
...
void NPC_RunBehavior(int team, int bState) {
	...
	if (bState == BS_CINEMATIC) {
	...
	} else if (NPC->client->NPC_class == CLASS_REBORN && NPC->client->ps.weapon == WP_MELEE) { // force-only reborn
		NPC_BehaviorSet_Jedi(bState);
		...
		case TEAM_ENEMY:
			...
			if (NPC->enemy && NPC->client->ps.weapon == WP_NONE && bState != BS_HUNT_AND_KILL &&
				!Q3_TaskIDPending(NPC, TID_MOVE_NAV)) { // if in battle and have no weapon, run away, fixme: when in BS_HUNT_AND_KILL, they just stand there
				if (bState != BS_FLEE) {
			...
			if (NPC->client->ps.weapon == WP_DISRUPTOR && (NPCInfo->scriptFlags & SCF_ALT_FIRE)) { // a sniper
				NPC_BehaviorSet_Sniper(bState);
				return;
			}
		...
		case TEAM_NEUTRAL:
			...
			if (NPC->client->NPC_class == CLASS_PROTOCOL) {
				NPC_BehaviorSet_Default(bState);
			} else if (NPC->client->NPC_class == CLASS_UGNAUGHT || NPC->client->NPC_class == CLASS_JAWA) { // others, too?
...
static bState_t G_CurrentBState(gNPC_t *gNPC) {
	if (gNPC->tempBehavior != BS_DEFAULT) { // Overrides normal behavior until cleared
...
void NPC_Think(gentity_t *self) //, int msec )
{
	...
	if (self->client && self->client->NPC_class == CLASS_VEHICLE && self->NPC_type &&
		(!self->m_pVehicle->m_pVehicleInfo->Inhabited(self->m_pVehicle))) { // empty swoop logic
		if (self->owner) { // still have attached owner, check and see if can forget him (so he can use me later)
			vec3_t dir2owner;
			...
			if (VectorLengthSquared(dir2owner) > 128 * 128 || !(self->clipmask & oldOwner->clipmask) ||
				(DotProduct(self->client->ps.velocity, oldOwner->client->ps.velocity) < -200.0f &&
				 !G_BoundsOverlap(self->absmin, self->absmin, oldOwner->absmin, oldOwner->absmax))) { // all clear, become solid to our owner now
				gi.linkentity(self);
	...
	if (player->client->ps.viewEntity == self->s.number) { // being controlled by player
	...
	if (NPCInfo->nextBStateThink <= level.time) {
		...
		if (NPC->client && NPC->client->NPC_class == CLASS_ROCKETTROOPER && (NPC->client->ps.eFlags & EF_FORCE_GRIPPED) &&
			NPC->client->moveType == MT_FLYSWIM && NPC->client->ps.groundEntityNum == ENTITYNUM_NONE) { // reduce velocity
...
void NPC_SetAnim(gentity_t *ent, int setAnimParts, int anim, int setAnimFlags,
				 int iBlend) { // FIXME : once torsoAnim and legsAnim are in the same structure for NCP and Players
	...
	if (ent->health > 0) {																						  // don't lock anims if the guy is dead
		if (ent->client->ps.torsoAnimTimer && PM_LockedAnim(ent->client->ps.torsoAnim) && !PM_LockedAnim(anim)) { // nothing can override these special anims
			setAnimParts &= ~SETANIM_TORSO;
		}

		if (ent->client->ps.legsAnimTimer && PM_LockedAnim(ent->client->ps.legsAnim) && !PM_LockedAnim(anim)) { // nothing can override these special anims
	...
	if (ent->client) { // Players, NPCs
		...
		PM_SetAnimFinal(&ent->client->ps.torsoAnim, &ent->client->ps.legsAnim, setAnimParts, anim, setAnimFlags, &ent->client->ps.torsoAnimTimer,
		...
		PM_SetAnimFinal(&ent->s.torsoAnim, &ent->s.legsAnim, setAnimParts, anim, setAnimFlags, &ent->s.torsoAnimTimer, &ent->s.legsAnimTimer, ent);
	}
}
