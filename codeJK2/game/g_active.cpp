  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (ent->health <= 0) {
      if ((ucmd->buttons & BUTTON_ATTACK) ||
          (ucmd->buttons &
           BUTTON_ALT_ATTACK)) { // if attacking, don't consider dead enemies
        continue;
      }
  ...
  if (!self->client || self->client->ps.lastOnGround + 300 < level.time ||
    ...
    if (other->material == MAT_GLASS || other->material == MAT_GLASS_METAL ||
        ...
        ((other->svFlags & SVF_BBRUSH) &&
         (other->spawnflags & 4 /*THIN*/))) //(other->absmax[0]-other->absmin[0]<=32||other->absmax[1]-other->absmin[1]<=32||other->absmax[2]-other->absmin[2]<=32))
                                            //)
      ...
      */
      ...
      if ((force >= 1 && other->s.number != 0) || force >= 10) {
        ...
        */
        ...
        if (self->forcePushTime >
                level.time - 1000 // was force pushed/pulled in the last 1600
                                  // milliseconds
            && self->forcePuller ==
                   other->s.number) // hit the person who pushed/pulled me
        {                           // ignore the impact
        } else if (other->takedamage) {
          if (!self->client || !other->s.number ||
...
void G_NPCMunroMatchPlayerWeapon(gentity_t *ent) {
  ...
  if (!in_camera) {
    if (ent && ent->client && ent->NPC &&
        (ent->NPC->aiFlags & NPCAI_MATCHPLAYERWEAPON)) { // we're a Kyle NPC
...
qboolean ClientInactivityTimer(gclient_t *client) {
  ...
  } else if (client->usercmd.forwardmove || client->usercmd.rightmove ||
  ...
  } else { // FIXME: here is where we can decide to play an idle animation
  }
  return qtrue;
}
...
void G_UpdateEmplacedWeaponData(gentity_t *ent) {
  if (ent && ent->owner && ent->health > 0) {
    ...
    gi.G2API_GetBoltMatrix(chair->ghoul2, chair->playerModel, chair->headBolt,
                           &boltMatrix, chairAng, chair->currentOrigin,
...
void ExitEmplacedWeapon(gentity_t *ent) {
  ...
  // We'll leave the gun pointed in the direction it was last facing, though
  // we'll cut out the pitch
  if (ent->client) {
    ...
    // if we are the player we will have put down a brush that blocks NPCs so
    // that we have a clear spot to get back out.
    // gentity_t *place = G_Find( NULL, FOFS(classname), "emp_placeholder" );

    if (ent->health > 0 &&
        ent->owner->nextTrain) { // he's still alive, and we have a placeholder,
                                 // so put him back
  ...
  if (!ent->NPC) {
    // by keeping the owner, a dead npc can be pushed out of the chair without
    // colliding with it
    ent->owner = NULL;
  }
}
...
void RunEmplacedWeapon(gentity_t *ent, usercmd_t **ucmd) {
  if (((*ucmd)->buttons & BUTTON_USE || (*ucmd)->forwardmove < 0 ||
       (*ucmd)->upmove > 0) &&
      ent->owner && ent->owner->delay + 500 < level.time) {
    ...
    /*		if ( ent->owner )
                    {
                            if ( !VectorCompare( ent->owner->pos3,
       ent->owner->movedir ))
                            {
                                    ent->owner->s.loopSound = G_SoundIndex(
       "sound/weapons/emplaced/emplaced_move_lp.wav" );
                                    ent->owner->fly_sound_debounce_time =
       level.time;
                            }
                            else
                            {
                                    if ( ent->owner->fly_sound_debounce_time +
       100 <= level.time )
                                    {
                                            ent->owner->s.loopSound = 0;
                                    }
...
void G_StartCinematicSkip(void) {
  ...
  if (cinematicSkipScript[0]) {
    ICARUS_RunScript(&g_entities[0],
                     va("%s/%s", Q3_SCRIPT_DIR, cinematicSkipScript));
...
void ClientThink_real(gentity_t *ent, usercmd_t *ucmd) {
  ...
  if (ent->s.number == 0) {
    ...
    if (in_camera) {
      ...
      if (speed && ent->client->ps.groundEntityNum == ENTITYNUM_NONE) {
        int diff = AngleNormalize180(
            SHORT2ANGLE(ucmd->angles[YAW] + ent->client->ps.delta_angles[YAW]) -
            floor(ent->client->ps.viewangles[YAW]));
        int slide = floor(((float)(diff)) / 120.0f * -127.0f);

        if ((slide > 0 && ucmd->rightmove >= 0) ||
            ((slide < 0 &&
              ucmd->rightmove <=
                  0))) { // note: don't want these to conflict right now because
                         // that seems to feel really weird
  ...
  if (ent->client) {
    if (ent->client->NPC_class == CLASS_GONK ||
    ...
    } else if (ent->client->NPC_class == CLASS_ATST) { // no jumping in atst
      if (ent->client->ps.pm_type != PM_NOCLIP) {
        ucmd->upmove = 0;
      }
      if (ent->client->ps.groundEntityNum !=
          ENTITYNUM_NONE) { // ATST crushed anything underneath it
      ...
      if (groundEnt && groundEnt->client &&
          ...
          !inSpinFlipAttack) { // landed on a live client who is on the ground,
                               // jump off them and knock them down
        if (ent->health > 0) {
          if (!PM_InRoll(&ent->client->ps) &&
            ...
            } else if (!ucmd->upmove) { // if not ducking (which should cause a
                                        // roll), then jump
              ucmd->upmove = 127;
            }
            if (!ucmd->forwardmove &&
  ...
  // set speed
  if (ent->NPC !=
      NULL) { // we don't actually scale the ucmd, we use actual speeds
    if (ent->NPC->combatMove == qfalse) {
      if (!(ucmd->buttons & BUTTON_USE)) { // Not leaning
        qboolean Flying =
            (qboolean)(ucmd->upmove && ent->NPC->stats.moveType == MT_FLYSWIM);
        qboolean Climbing =
    ...
    if (ent->client->ps.vehicleModel != 0 && ent->health > 0) {
      if (client->ps.speed || client->ps.groundEntityNum == ENTITYNUM_NONE ||
          ucmd->forwardmove > 0 || ucmd->upmove > 0) {
        if (ucmd->forwardmove > 0) {
          client->ps.speed += 20;
        } else if (ucmd->forwardmove < 0) {
        ...
        } else { // accelerate to cruising speed only, otherwise, just coast
          if (client->ps.speed < 800) {
            client->ps.speed += 10;
            if (client->ps.speed > 800) {
              client->ps.speed = 800;
      ...
      if (PM_SaberInAttack(ent->client->ps.saberMove) &&
          ucmd->forwardmove <
              0) { // if running backwards while attacking, don't run as fast.
  ...
  if (ucmd->forwardmove < 0 && !(ucmd->buttons & BUTTON_WALKING) &&
      client->ps.groundEntityNum !=
          ENTITYNUM_NONE) { // running backwards is slower than running forwards
  ...
  if ((ucmd->buttons & BUTTON_BLOCKING) &&
      !g_saberAutoBlocking->integer) { // blocking with saber
  ...
  if (client->ps.stats[STAT_HEALTH] <= 0) {
    ...
    if (ent->NPC == NULL && level.time > client->respawnTime) {
      ...
      */
      ...
      if (ucmd->buttons & (BUTTON_ATTACK)) {
        respawn(ent);
        //				gi.SendConsoleCommand(
        //va("disconnect;wait;wait;wait;wait;wait;wait;devmap
        //%s\n",level.mapname) );
      }
    }
...
void ClientThink(int clientNum, usercmd_t *ucmd) {
  ...
  if (!ent->s.number) {
    if (ent->client->ps.viewEntity > 0 &&
      ...
      } else { // others use the blocking key, button3
        if ((ucmd->buttons &
             BUTTON_BLOCKING)) { // jumping gets you out of it FIXME: check some
                                 // other button instead... like ESCAPE... so you
                                 // could even have total control over an NPC?
          G_ClearViewEntity(ent);
          ucmd->buttons = 0;
