...
#define PMF_JUMPING                                                            \
  4 // yes, I really am in a jump -- Mike, you may want to come up with
    // something better here since this is really a temp fix.
#define PMF_BACKWARDS_JUMP 8    // go into backwards land
#define PMF_BACKWARDS_RUN 16    // coast down to backwards run
#define PMF_TIME_LAND 32        // pm_time is time before rejump
#define PMF_TIME_KNOCKBACK 64   // pm_time is an air-accelerate only time
#define PMF_TIME_NOFRICTION 128 // pm_time is a no-friction time
//#define	PMF_BLOCKED			128		// Bumped into
//something you can't step up - NOTHING WAS EVER CHECKING THIS
...
#define PMF_TRIGGER_PUSHED                                                     \
  4096 // pushed by a trigger_push or other such thing - cannot force jump and
       // will not take impact damage
//#define PMF_FOLLOW			4096	// spectate following another
//player
#define PMF_SCOREBOARD 8192 // spectate as a scoreboard
//#define	PMF_BUMPED			16384	// Bumped into something
//and slid to the side - NOTHING WAS EVER CHECKING THIS
...
typedef struct {
  ...
  void (*trace)(trace_t *results, const vec3_t start, const vec3_t mins,
                ...
                const EG2_Collision eG2TraceType /* = (EG2_Collision)0*/,
                const int useLod /* = 0 */);
...
void PM_SetAnimFinal(int *torsoAnim, int *legsAnim, int type, int anim,
                     ...
                     gentity_t *gent, int blendTime = 100);
...
#define EF_NO_TED                                                              \
  0x00000400 // won't show up on TED unless actively scan with tricorder
#define EF_AUTO_SIZE                                                           \
...
typedef struct animsounds_s {
  ...
  int soundIndex[MAX_RANDOM_ANIMSOUNDS]; // sound file to play - FIXME: should
                                         // be an index, handle random some other
                                         // way?
  int numRandomAnimSounds; // string variable min for va("...%d.wav",
                           // Q_irand(lowestVa, highestVa))
  int probability; // chance sound will play, zero value will not run this test
                   // (0 = 100% basically)
