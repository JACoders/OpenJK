...
qboolean CG_SpawnString( const char *key, const char *defaultString, char **out ) {
	...
	if( !cg.spawning ) {
		*out = (char*)defaultString;
		// cgi.Error( ERR_DROP, "CG_SpawnString() called while not spawning" );
	}
...
void SP_misc_model_static( void ) {
	...
	if( cgs.numMiscStaticModels >= MAX_STATIC_MODELS ) {
		cgi.Error( ERR_DROP, "MAX_STATIC_MODELS(%i) hit", MAX_STATIC_MODELS );
	}

	CG_SpawnString( "model", "", &model );

	if( !model || !model[0] ) {
		cgi.Error( ERR_DROP, "misc_model_static with no model." );
	}
	...
	modelIndex = cgi.R_RegisterModel( model );
	if( modelIndex == 0 ) {
		cgi.Error( ERR_DROP, "misc_model_static failed to load model '%s'", model );
		return;
	}
...
void SP_misc_skyportal_orient( void ) {
	...
	if( cg_skyOri ) {
		cgi.Error( ERR_DROP, "ERROR: multiple misc_skyportal_orients found" );
	}
...
void SP_misc_weather_zone( void ) {
	...
	if( !model || !model[0] ) {
		cgi.Error( ERR_DROP, "misc_weather_zone with invalid brush model data." );
		return;
	}

	cgi.R_ModelBounds( cgi.R_RegisterModel( model ), mins, maxs );

	cgi.WE_AddWeatherZone( mins, maxs );
}
typedef struct {
	char *name;
	void ( *spawn )( void );
...
char *CG_AddSpawnVarToken( const char *string ) {
	...
	if( cg.numSpawnVarChars + l + 1 > MAX_SPAWN_VARS_CHARS ) {
		cgi.Error( ERR_DROP, "CG_AddSpawnVarToken: MAX_SPAWN_VARS_CHARS" );
	}
...
qboolean CG_ParseSpawnVars( void ) {
	...
	if( !cgi.R_GetEntityToken( com_token, sizeof( com_token ) ) ) {
		// end of spawn string
		return qfalse;
	}

	if( com_token[0] != '{' ) {
		cgi.Error( ERR_DROP, "CG_ParseSpawnVars: found %s when expecting {", com_token );
	}

	// go through all the key / value pairs
	while( 1 ) {
		// parse key
		if( !cgi.R_GetEntityToken( keyname, sizeof( keyname ) ) ) {
			cgi.Error( ERR_DROP, "CG_ParseSpawnVars: EOF without closing brace" );
		}
		...
		if( com_token[0] == '}' ) {
			cgi.Error( ERR_DROP, "CG_ParseSpawnVars: closing brace without data" );
		}

		if( cg.numSpawnVars == MAX_SPAWN_VARS ) {
			cgi.Error( ERR_DROP, "CG_ParseSpawnVars: MAX_SPAWN_VARS" );
		}
...
void SP_worldspawn( void ) {
	...
	if( Q_stricmp( s, "worldspawn" ) ) {
		cgi.Error( ERR_DROP, "SP_worldspawn: The first entity isn't 'worldspawn'" );
	}
...
void CG_ParseEntitiesFromString( void ) {
	...
	if( !CG_ParseSpawnVars() ) {
		cgi.Error( ERR_DROP, "ParseEntities: no entities" );
	}
