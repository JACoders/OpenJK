...
//-----------------------------------------------------------------------------
/*
static long RllDecodeMonoToMono(unsigned char *from,short *to,unsigned int
size,char signedOutput ,unsigned short flag)
{
        unsigned int z;
        int prev;
...
// Returns:		Number of samples placed in output buffer
//-----------------------------------------------------------------------------
static long RllDecodeMonoToStereo(unsigned char *from, short *to,
                                  unsigned int size, char signedOutput,
                                  unsigned short flag) {
  unsigned int z;
  int prev;
...
// Returns:		Number of samples placed in output buffer
//-----------------------------------------------------------------------------
static long RllDecodeStereoToStereo(unsigned char *from, short *to,
                                    unsigned int size, char signedOutput,
                                    unsigned short flag) {
  unsigned int z;
...
//-----------------------------------------------------------------------------
/*
static long RllDecodeStereoToMono(unsigned char *from,short *to,unsigned int
size,char signedOutput, unsigned short flag)
{
        unsigned int z;
        int prevL,prevR;
...
static void setupQuad(long xOff, long yOff) {
  ...
  if (xOff == cin.oldXOff && yOff == cin.oldYOff &&
      cinTable[currentHandle].ysize == (unsigned)cin.oldysize &&
  ...
  /*	Enisform: Not in q3 source
     ...
     (16); numQuadCels += numQuadCels/4 + numQuadCels/16; numQuadCels += 64;
     // for overflow
  */
...
e_status CIN_RunCinematic(int handle) {
  ...
  if (cinTable[currentHandle].shader &&
      (abs(thisTime - (double)cinTable[currentHandle].lastTime)) > 100) {
...
int CIN_PlayCinematic(const char *arg, int x, int y, int w, int h,
  ...
  cinTable[currentHandle].alterGameState =
      (qboolean)((systemBits & CIN_system) != 0);
