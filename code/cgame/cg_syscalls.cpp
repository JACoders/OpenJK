...
int cgi_R_Font_HeightPixels(const int iFontIndex,
                            const float scale /*= 1.0f*/) {
...
unsigned int cgi_AnyLanguage_ReadCharFromString(
  ...
  return Q_syscall(CG_ANYLANGUAGE_READFROMSTRING, psText, piAdvanceCount,
...
void cgi_R_Font_DrawString(int ox, int oy, const char *text, const float *rgba,
                           ...
                           const float scale /*= 1.0f*/) {
  Q_syscall(CG_R_FONTDRAWSTRING, ox, oy, text, rgba, setIndex, iMaxPixelWidth,
            PASSFLOAT(scale));
}

// set some properties for the draw layer for my refractive effect (here
// primarily for mod authors) -rww
...
void cgi_R_LerpTag(orientation_t *tag, qhandle_t mod, int startFrame,
  ...
  Q_syscall(CG_R_LERPTAG, tag, mod, startFrame, endFrame, PASSFLOAT(frac),
            tagName);
}
...
void cgi_R_Scissor(float x, float y, float w, float h) {
  Q_syscall(CG_R_SCISSOR, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w),
            PASSFLOAT(h));
}
...
void cgi_SetUserCmdAngles(float pitchOverride, float yawOverride,
                          float rollOverride) {
  Q_syscall(CG_SETUSERCMDANGLES, PASSFLOAT(pitchOverride),
            PASSFLOAT(yawOverride), PASSFLOAT(rollOverride));
}
/*
Ghoul2 Insert Start
*/
...
int cgi_UI_GetMenuItemInfo(const char *menuFile, const char *itemName, int *x,
  ...
  return (int)Q_syscall(CG_UI_GETITEMINFO, menuFile, itemName, x, y, w, h,
                        color, background);
}
...
int cgi_SP_GetStringTextString(const char *text, char *buffer,
                               int bufferLength) {
...
/*
OpenJK Add
Since the modules are incompatible, might as well break base compat even further
amirite?
*/
