...
extern void CG_Chunks(int owner, vec3_t origin, const vec3_t normal, const vec3_t mins, const vec3_t maxs, float speed, int numChunks, material_t chunkType,
...
void funcBBrushDieGo(gentity_t *self) {
	...
	if (self->radius > 0.0f) {
		...
		//	I do this after the scale calculation because it seems that the chunk size generally seems to be very close, it's just the number of chunks is a bit
		//weak
...
"radius"  Chunk code tries to pick a good volume of chunks, but you can alter this to scale the number of spawned chunks. (default 1)  (.5) is half as many
chunks, (2) is twice as many chunks "NPC_targetname" - Only the NPC with this name can damage this "forcevisible" - When you turn on force sight (any level),
you can see these draw through the entire level... "redCrosshair" - crosshair turns red when you look at this
...
void SP_func_breakable(gentity_t *self) {
	...
	if (redCrosshair) { // can see these through walls with force sight, so must be broadcast
...
void misc_model_breakable_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath, int dFlags, int hitLoc) {
	...
	if (!(self->spawnflags & 2048)) // NO_EXPLOSION
	{
		...
		if (self->splashDamage > 0 && self->splashRadius > 0) { // explode
			...
			//				a bit better?
			//  up the origin a little for the damage check, because several models have their origin on the ground, so they don't alwasy do damage, not the
			//  optimal solution...
			VectorCopy(self->currentOrigin, org);
			if (self->mins[2] > -4) { // origin is going to be below it or very very low in the model
				// center the origin
			...
			if (self->model && (Q_stricmp("models/map_objects/ships/tie_fighter.md3", self->model) == 0 ||
								Q_stricmp("models/map_objects/ships/tie_bomber.md3", self->model) == 0)) { // TEMP HACK for Tie Fighters- they're HUGE
	...
	if (self->s.modelindex2 != -1 && !(self->spawnflags & 8)) { // FIXME: modelindex doesn't get set to -1 if the damage model doesn't exist
...
void misc_model_throw_at_target4(gentity_t *self, gentity_t *activator) {
	...
	if (self->s.pos.trType != TR_INTERPOLATE) { // don't do this to rolling missiles
...
void misc_model_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
	...
	if (self->health <= 0 && self->max_health > 0) { // used while broken fired target3
...
void misc_model_breakable_init(gentity_t *ent) {
	...
	if (ent->spawnflags & 1) { // Blocks movement
		ent->contents = CONTENTS_SOLID | CONTENTS_OPAQUE | CONTENTS_BODY | CONTENTS_MONSTERCLIP |
						CONTENTS_BOTCLIP; // Was CONTENTS_SOLID, but only architecture should be this
...
void TieBomberThink(gentity_t *self) {
	...
	if (player->health > 0 && playerDist < MIN_PLAYER_DIST && self->attackDebounceTime < level.time) {
		...
		// Start the effect.
		G_PlayEffect(G_EffectIndex("ships/tiebomber_bomb_falling"), bomb->playerModel, gi.G2API_AddBolt(&bomb->ghoul2[0], "model_root"), bomb->s.number,
					 bomb->currentOrigin, 1000, qtrue);
...
START_OFF SOLID - Movement is blocked by it, if not set, can still be broken by explosions and shots if it has health AUTOANIMATE - Will cycle it's anim
...
"health"	how much health to have - default is zero (not breakable)  If you don't set the SOLID flag, but give it health, it can be shot but will not block
NPCs or players from moving "targetname" when used, dies and displays damagemodel (model + "_d1.md3"), if any (if not, removes itself) "target" What to use when
it dies "target2" What to use when it's repaired "target3" What to use when it's used while it's broken "paintarget" target to fire when hit (but not destroyed)
...
"mass"		if gravity is on, this determines how much damage this thing does when it hits someone.  Default is the size of the object from one corner to the
other, that works very well.  Only override if this is an object whose mass should be very high or low for it's size (density)
...
void SP_misc_model_breakable(gentity_t *ent) {
	...
	} else if (ent->model && Q_stricmp("models/map_objects/ships/tie_fighter.md3", ent->model) == 0) { // run a think
...
qboolean G_EntIsBreakable(int entityNum, gentity_t *breaker) { // breakable brush/model that can actually be broken
	...
	if (ent->NPC_targetname) {																				// only a specific entity can break this!
		if (!breaker || !breaker->targetname || Q_stricmp(ent->NPC_targetname, breaker->targetname) != 0) { // I'm not the one who can break it
			return qfalse;
		}
	}
