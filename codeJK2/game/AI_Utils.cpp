...
int AI_GetGroupSize(gentity_t *ent, int radius) {
  ...
  return AI_GetGroupSize(ent->currentOrigin, radius, ent->client->playerTeam,
                         ent);
}
...
void AI_SetClosestBuddy(AIGroupInfo_t *group) {
  ...
  for (i = 0; i < group->numGroup; i++) {
    ...
    for (j = 0; j < group->numGroup; j++) {
      dist = DistanceSquared(g_entities[group->member[i].number].currentOrigin,
    ...
    if (group->enemyWP == WAYPOINT_NONE) { // FIXME: just use member->waypoint?
      ...
      if (group->member[i].waypoint != WAYPOINT_NONE) {
        group->member[i].pathCostToEnemy =
            navigator.GetPathCost(group->member[i].waypoint, group->enemyWP);
      ...
      for (j = 0; j < group->numGroup; j++) {
        if (bestMembers[j].number != ENTITYNUM_NONE) { // slot occupied
          if (group->member[i].pathCostToEnemy <
              bestMembers[j]
                  .pathCostToEnemy) { // this guy has a shorter path than the
                                      // one currenly in this spot, bump him and
                                      // put myself in here
            for (k = group->numGroup; k > j; k++) {
              memcpy(&bestMembers[k], &bestMembers[k - 1],
                     sizeof(bestMembers[k]));
            }
...
qboolean AI_FindSelfInPreviousGroup(
    gentity_t *self) { // go through other groups made this frame and see if any
                       // of those contain me already
  int i, j;
...
void AI_InsertGroupMember(AIGroupInfo_t *group, gentity_t *member) {
  // okay, you know what?  Check this damn group and make sure we're not already
  // in here!
  int i;
  for (i = 0; i < group->numGroup; i++) {
  ...
  if (!group->commander ||
      (member->NPC->rank >
       group->commander->NPC->rank)) { // keep track of highest rank
...
qboolean AI_TryJoinPreviousGroup(
    gentity_t *self) { // go through other groups made this frame and see if any
                       // of those have the same enemy as me... if so, add me in!
  int i;
  for (i = 0; i < MAX_FRAME_GROUPS; i++) {
    if (level.groups[i].numGroup &&
        level.groups[i].numGroup < (MAX_GROUP_MEMBERS - 1)
        //&& level.groups[i].enemy != NULL
        && level.groups[i].enemy ==
               self->enemy) { // has members, not full and has my enemy
      if (AI_ValidateGroupMember(&level.groups[i],
                                 self)) { // I am a valid member for this group
...
qboolean AI_GetNextEmptyGroup(gentity_t *self) {
  if (AI_FindSelfInPreviousGroup(
          self)) { // already in one, no need to make a new one
    return qfalse;
  }

  if (AI_TryJoinPreviousGroup(
          self)) { // try to just put us in one that already exists
    return qfalse;
  }
...
qboolean AI_ValidateNoEnemyGroupMember(AIGroupInfo_t *group,
                                       gentity_t *member) {
  if (!group) {
    return qfalse;
  }
  ...
  if (group->commander) {
    ...
    if (group->member[0].number < 0 ||
        group->member[0].number >= ENTITYNUM_WORLD) {
      return qfalse;
    }
    VectorCopy(g_entities[group->member[0].number].currentOrigin, center);
  }
  // FIXME: maybe it should be based on the center of the mass of the group, not
  // the commander?
  if (DistanceSquared(center, member->currentOrigin) > 147456 /*384*384*/) {
    return qfalse;
  }
  if (!gi.inPVS(member->currentOrigin,
                center)) { // not within PVS of the group enemy
    return qfalse;
  }
  return qtrue;
}
...
qboolean AI_ValidateGroupMember(AIGroupInfo_t *group, gentity_t *member) {
  ...
  if (member->NPC->group != NULL &&
      member->NPC->group != group) { // FIXME: if that group's enemy is mine,
                                     // why not absorb that group into mine?
    return qfalse;
  }
  ...
  if (member->client->ps.weapon == WP_SABER || //!= self->s.weapon )
      ...
      member->client->ps.weapon ==
          WP_TIE_FIGHTER) { // not really a squad-type guy
    return qfalse;
  }
  ...
  if (member->client->NPC_class == CLASS_ATST ||
      ...
      member->client->NPC_class ==
          CLASS_MARK2) { // these kinds of enemies don't actually use this group
                         // AI
    return qfalse;
  }
  ...
  if (member->enemy != group->enemy) {
    ...
    if (!gi.inPVS(
            ...
            group->enemy->currentOrigin)) { // not within PVS of the group enemy
      return qfalse;
    }
  } else if (group->enemy == NULL) { // if the group is a patrol group, only
                                     // take those within the room and radius
...
void AI_GetGroup(gentity_t *self) {
  ...
  if (self->enemy && (!self->enemy->client ||
                      (level.time - self->NPC->enemyLastSeenTime > 7000))) {
    self->NPC->group = NULL;
    return;
  }

  if (!AI_GetNextEmptyGroup(self)) { // either no more groups left or we're
                                     // already in a group built earlier
    return;
  }
  ...
  if (self->NPC->group->enemy) {
    ...
    VectorCopy(self->NPC->group->enemy->currentOrigin,
               self->NPC->group->enemyLastSeenPos);
  }

  //	for ( i = 0, member = &g_entities[0]; i < globals.num_entities ; i++,
  //member++)
  for (i = 0; i < globals.num_entities; i++) {
    ...
    if (!AI_ValidateGroupMember(
            self->NPC->group,
            member)) { // FIXME: keep track of those who aren't angry yet and
                       // see if we should wake them after we assemble the core
                       // group
      continue;
    }
...
void AI_SetNewGroupCommander(AIGroupInfo_t *group) {
  ...
  for (int i = 0; i < group->numGroup; i++) {
    ...
    if (!group->commander ||
        (member && member->NPC && group->commander->NPC &&
         member->NPC->rank >
             group->commander->NPC->rank)) { // keep track of highest rank
      group->commander = member;
    }
...
void AI_DeleteSelfFromGroup(gentity_t *self) {
  // FIXME: if killed, keep track of how many in group killed?  To affect
  // morale?
...
extern void ST_StartFlee(gentity_t *self, gentity_t *enemy, vec3_t dangerPoint,
...
void AI_GroupMemberKilled(gentity_t *self) {
  ...
  if (!self || !self->NPC ||
      self->NPC->rank <
          RANK_ENSIGN) { // I'm not an officer, let's not really care for now
    return;
  }
  ...
  for (int i = 0; i < group->numGroup; i++) {
    ...
    if (member->NPC->rank > RANK_ENSIGN) { // officers do not panic
      ...
      member->NPC->currentAim -=
          Q_irand(0, 10); // Q_irand( 0, 2);//drop their aim accuracy
    }
  }
  // okay, if I'm the group commander, make everyone else flee
  if (group->commander != self) { // I'm not the commander... hmm, should maybe
                                  // a couple flee... maybe those near me?
    return;
  }
  ...
  if (!noflee) {
    ...
    for (int i = 0; i < group->numGroup; i++) {
      ...
      if (member->NPC->rank < RANK_ENSIGN) { // grunt
        if (group->enemy &&
            DistanceSquared(member->currentOrigin,
                            group->enemy->currentOrigin) <
                65536 /*256*256*/) { // those close to enemy run away!
          ST_StartFlee(member, group->enemy, member->currentOrigin,
                       AEL_DANGER_GREAT, 3000, 5000);
        } else if (DistanceSquared(member->currentOrigin, self->currentOrigin) <
                   65536 /*256*256*/) { // those close to me run away!
          ST_StartFlee(member, group->enemy, member->currentOrigin,
                       AEL_DANGER_GREAT, 3000, 5000);
        } else { // else, maybe just a random chance
          if (Q_irand(0, self->NPC->rank) >
              member->NPC->rank) { // lower rank they are, higher rank I am,
                                   // more likely they are to flee
            ST_StartFlee(member, group->enemy, member->currentOrigin,
                         AEL_DANGER_GREAT, 3000, 5000);
          } else {
        ...
        member->NPC->currentAim -= Q_irand(
            1, 15); // Q_irand( 1, 3 );//drop their aim accuracy even more
      }
      member->NPC->currentAim -=
          Q_irand(1, 15); // Q_irand( 1, 3 );//drop their aim accuracy even more
    }
  }
}
...
void AI_GroupUpdateSquadstates(AIGroupInfo_t *group, gentity_t *member,
                               int newSquadState) {
  if (!group) {
...
qboolean AI_RefreshGroup(AIGroupInfo_t *group) {
    ...
    if (&level.groups[i] == group) {
      ...
      if (level.groups[i].enemy == group->enemy) { // 2 groups with same enemy
        if (level.groups[i].numGroup + group->numGroup <
            (MAX_GROUP_MEMBERS -
             1)) { // combining the members would fit in one group
          ...
          for (int j = 0; j < group->numGroup; j++) {
            ...
            if (level.groups[i].enemy ==
                NULL) { // special case for groups without enemies, must be in
                        // range
            ...
            // keep marker at same place since we deleted this guy and shifted
            // everyone up one
            j--;
  ...
  for (i = 0; i < group->numGroup; i++) {
    ...
    if ( j < group->numGroup )
    {//found a dupe!
            gi.Printf( S_COLOR_RED"ERROR: member %s(%d) a duplicate group
    member!!!\n", g_entities[group->member[i].number].targetname,
    group->member[i].number ); AI_DeleteGroupMember( group, i ); i--; continue;
    }
    */
    member = &g_entities[group->member[i].number];
    ...
    if (member->health <= 0) {
      ...
      i--;
               ...
               !AI_ValidateGroupMember(group, member)) {
  ...
  /*
  for ( i = 0, member = &g_entities[0]; i < globals.num_entities &&
  group->numGroup < (MAX_GROUP_MEMBERS - 1); i++, member++)
  {
          if ( !AI_ValidateGroupMember( group, member ) )
          {//FIXME: keep track of those who aren't angry yet and see if we
  should wake them after we assemble the core group continue;
          }
  ...
  */
  ...
  for (i = 0; i < group->numGroup; i++) {
    ...
    if (group->commander && debugNPCAI->integer) {
      G_DebugLine(group->commander->currentOrigin, member->currentOrigin,
  ...
  if (group->moraleDebounce <
      level.time) { // slowly degrade whatever moraleAdjusters we may have
...
void AI_UpdateGroups(void) {
  ...
  for (int i = 0; i < MAX_FRAME_GROUPS; i++) {
    if (!level.groups[i].numGroup ||
        AI_RefreshGroup(&level.groups[i]) ==
            qfalse) // level.groups[i].enemy == NULL ||
    {
...
temp ); VectorNormalize( temp );	//FIXME: Needed? vectoangles( temp,
angles );
        }
        else
        {
        ...
        AI_GetGroup( group, ent->currentOrigin, ent->currentAngles,
DEFAULT_RADIUS, radius, ent->client->playerTeam, ent, ent->enemy );
}
*/
/*
-------------------------
...
gentity_t *AI_DistributeAttack(gentity_t *attacker, gentity_t *enemy,
                               team_t team, int threshold) {
  // Don't take new targets
  if (NPC->svFlags & SVF_LOCKEDENEMY)
    return enemy;

  int numSurrounding =
      AI_GetGroupSize(enemy->currentOrigin, 48, team, attacker);

  // First, see if we should look for the player
  if (enemy != &g_entities[0]) {
    int aroundPlayer =
        AI_GetGroupSize(g_entities[0].currentOrigin, 48, team, attacker);
  ...
  for (int j = 0; j < numEnts; j++) {
    ...
    if (AI_GetGroupSize(radiusEnts[j]->currentOrigin, 48, team, attacker) >
        threshold)
      continue;
