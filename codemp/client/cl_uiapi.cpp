...
static void CL_Cvar_Get(const char *var_name, const char *value, uint32_t flags) { Cvar_Register(NULL, var_name, value, flags); }
...
static void CL_SE_GetLanguageName(const int languageIndex, char *buffer) { Q_strncpyz(buffer, SE_GetLanguageName(languageIndex), 128); }
...
static void UIVM_Cvar_Set(const char *var_name, const char *value) { Cvar_VM_Set(var_name, value, VM_UI); }

static void UIVM_Cvar_SetValue(const char *var_name, float value) { Cvar_VM_SetValue(var_name, value, VM_UI); }

static void CL_AddUICommand(const char *cmdName) { Cmd_AddCommand(cmdName, NULL); }

static void UIVM_Cmd_RemoveCommand(const char *cmd_name) { Cmd_VM_RemoveCommand(cmd_name, VM_UI); }
...
intptr_t CL_UISystemCalls(intptr_t *args) {
	...
	case UI_G2_ANGLEOVERRIDE:
		return CL_G2API_SetBoneAngles(VMA(1), args[2], (const char *)VMA(3), (float *)VMA(4), args[5], (const Eorientations)args[6],
