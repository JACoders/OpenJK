...
void CGVM_DrawActiveFrame(int serverTime, stereoFrame_t stereoView,
...
void CGVM_ROFF_NotetrackCallback(int entID, const char *notetrack) {
  if (cgvm->isLegacy) {
    VM_Call(cgvm, CG_ROFF_NOTETRACK_CALLBACK, entID,
...
static void CL_CM_LoadMap(const char *mapname, qboolean subBSP) {
  if (subBSP)
    CM_LoadSubBSP(va("maps/%s.bsp", mapname + 1), qfalse);
  else
...
static void RegisterSharedMemory(char *memory) { cl.mSharedMemory = memory; }
...
static int CL_CM_RegisterTerrain(const char *config) { return 0; }

extern int s_entityWavVol[MAX_GENTITIES];
static int CL_S_GetVoiceVolume(int entID) { return s_entityWavVol[entID]; }

static void CL_S_Shutup(qboolean shutup) { s_shutUp = shutup; }

static int CL_GetCurrentCmdNumber(void) { return cl.cmdNumber; }

static void _CL_SetUserCmdValue(int stateValue, float sensitivityScale,
                                float mPitchOverride, float mYawOverride,
                                float mSensitivityOverride, int fpSel,
                                int invenSel, qboolean fighterControls) {
  cl_bUseFighterPitch = fighterControls;
  CL_SetUserCmdValue(stateValue, sensitivityScale, mPitchOverride, mYawOverride,
                     mSensitivityOverride, fpSel, invenSel);
}
...
static void CGFX_AddLine(vec3_t start, vec3_t end, float size1, float size2,
                         float sizeParm, float alpha1, float alpha2,
                         float alphaParm, vec3_t sRGB, vec3_t eRGB,
                         float rgbParm, int killTime, qhandle_t shader,
                         int flags) {
  FX_AddLine(start, end, size1, size2, sizeParm, alpha1, alpha2, alphaParm,
...
static void CGFX_AddPoly(addpolyArgStruct_t *p) {
  FX_AddPoly(p->p, p->ev, p->numVerts, p->vel, p->accel, p->alpha1, p->alpha2,
             p->alphaParm, p->rgb1, p->rgb2, p->rgbParm, p->rotationDelta,
             p->bounce, p->motionDelay, p->killTime, p->shader, p->flags);
}

static void CGFX_AddBezier(addbezierArgStruct_t *b) {
  FX_AddBezier(b->start, b->end, b->control1, b->control1Vel, b->control2,
               b->control2Vel, b->size1, b->size2, b->sizeParm, b->alpha1,
               b->alpha2, b->alphaParm, b->sRGB, b->eRGB, b->rgbParm,
               b->killTime, b->shader, b->flags);
}

static void CGFX_AddPrimitive(effectTrailArgStruct_t *e) { FX_FeedTrail(e); }

static void CGFX_AddSprite(addspriteArgStruct_t *s) {
  vec3_t rgb = {1.0f, 1.0f, 1.0f};
  FX_AddParticle(s->origin, s->vel, s->accel, s->scale, s->dscale, 0, s->sAlpha,
                 s->eAlpha, 0, rgb, rgb, 0, s->rotation, 0, vec3_origin,
                 vec3_origin, s->bounce, 0, 0, s->life, s->shader, s->flags);
}

static void CGFX_AddElectricity(addElectricityArgStruct_t *p) {
  FX_AddElectricity(p->start, p->end, p->size1, p->size2, p->sizeParm,
                    p->alpha1, p->alpha2, p->alphaParm, p->sRGB, p->eRGB,
                    p->rgbParm, p->chaos, p->killTime, p->shader, p->flags);
}

static qboolean CL_ROFF_Clean(void) { return theROFFSystem.Clean(qtrue); }
...
static void CL_PrecisionTimerStart(void **p) {
  ...
  *p = newTimer; // assign the pointer within the pointer to point at the mem
                 // addr of our new timer
  newTimer->Start(); // start the timer
}

static int CL_PrecisionTimerEnd(void *p) {
  int r = 0;
  timing_c *timer = (timing_c *)p; // this is the pointer we assigned in start,
                                   // so we can directly cast it back
...
static qboolean CGFX_PlayBoltedEffectID(int id, vec3_t org, void *ghoul2,
...
static qboolean CL_SE_GetStringTextString(const char *text, char *buffer,
                                          int bufferLength) {
  const char *str;
...
static void CL_G2API_SetGhoul2ModelIndexes(void *ghoul2, qhandle_t *modelList,
                                           qhandle_t *skinList) {
  if (!ghoul2)
    return;
...
static qboolean CL_G2API_GetBoltMatrix(void *ghoul2, const int modelIndex,
...
static qboolean CL_G2API_GetBoltMatrix_NoReconstruct(
...
static qboolean CL_G2API_GetBoltMatrix_NoRecNoRot(
    void *ghoul2, const int modelIndex, const int boltIndex, mdxaBone_t *matrix,
    const vec3_t angles, const vec3_t position, const int frameNum,
    qhandle_t *modelList, vec3_t scale) {
  if (!ghoul2)
    return qfalse;
  // Intentionally not setting bolt matrix reconstruction state per original
  // code comments
...
static int CL_G2API_InitGhoul2Model(void **ghoul2Ptr, const char *fileName,
...
static qboolean CL_G2API_SetSkin(void *ghoul2, int modelIndex,
...
static void CL_G2API_CollisionDetect(CollisionRecord_t *collRecMap,
  ...
  re->G2API_CollisionDetect(collRecMap, *((CGhoul2Info_v *)ghoul2), angles,
                            ...
                            scale, G2VertSpaceClient, traceFlags, useLod,
                            fRadius);
}
...
static void CL_G2API_CollisionDetectCache(
  ...
  re->G2API_CollisionDetectCache(collRecMap, *((CGhoul2Info_v *)ghoul2), angles,
                                 ...
                                 rayEnd, scale, G2VertSpaceClient, traceFlags,
                                 useLod, fRadius);
}
...
static qboolean CL_G2API_SetBoneAngles(void *ghoul2, int modelIndex,
                                       const char *boneName,
...
static qboolean
CL_G2API_SetBoneAnim(void *ghoul2, const int modelIndex, const char *boneName,
...
static qboolean CL_G2API_GetBoneAnim(void *ghoul2, const char *boneName,
...
static qboolean CL_G2API_GetBoneFrame(void *ghoul2, const char *boneName,
                                      const int currentTime,
                                      float *currentFrame, int *modelList,
                                      const int modelIndex) {
  if (!ghoul2)
    return qfalse;
  CGhoul2Info_v &g2 = *((CGhoul2Info_v *)ghoul2);
  ...
  return re->G2API_GetBoneAnim(g2, modelIndex, boneName, currentTime,
                               currentFrame, &iDontCare1, &iDontCare2,
...
static int CL_G2API_CopyGhoul2Instance(void *g2From, void *g2To,
                                       int modelIndex) {
  if (!g2From || !g2To)
    return 0;
...
static void CL_G2API_CopySpecificGhoul2Model(void *g2From, int modelFrom,
...
static int CL_G2API_AddBolt(void *ghoul2, int modelIndex,
                            const char *boneName) {
  if (!ghoul2)
    return -1;
...
static qboolean CL_G2API_AttachEnt(int *boltInfo, void *ghlInfoTo,
                                   int toBoltIndex, int entNum,
                                   int toModelNum) {
  if (!ghlInfoTo)
    return qfalse;
...
static qboolean CL_G2API_SetRootSurface(void *ghoul2, const int modelIndex,
...
static qboolean CL_G2API_SetSurfaceOnOff(void *ghoul2, const char *surfaceName,
                                         const int flags) {
  if (!ghoul2)
    return qfalse;
...
static qboolean CL_G2API_DoesBoneExist(void *ghoul2, int modelIndex,
                                       const char *boneName) {
  if (!ghoul2)
    return qfalse;
  CGhoul2Info_v &g2 = *((CGhoul2Info_v *)ghoul2);
...
static int CL_G2API_GetSurfaceRenderStatus(void *ghoul2, const int modelIndex,
                                           const char *surfaceName) {
  if (!ghoul2)
    return -1;
  CGhoul2Info_v &g2 = *((CGhoul2Info_v *)ghoul2);
...
static int CL_G2API_GetTime(void) { return re->G2API_GetTime(0); }
...
static void CL_G2API_AnimateG2Models(void *ghoul2, int time,
...
static qboolean CL_G2API_RagPCJConstraint(void *ghoul2, const char *boneName,
...
static qboolean CL_G2API_RagPCJGradientSpeed(void *ghoul2, const char *boneName,
                                             const float speed) {
  if (!ghoul2)
    return qfalse;
...
static qboolean CL_G2API_RagEffectorGoal(void *ghoul2, const char *boneName,
                                         vec3_t pos) {
  if (!ghoul2)
    return qfalse;
...
static qboolean CL_G2API_GetRagBonePos(void *ghoul2, const char *boneName,
...
static qboolean CL_G2API_RagEffectorKick(void *ghoul2, const char *boneName,
                                         vec3_t velocity) {
  if (!ghoul2)
    return qfalse;
...
static qboolean CL_G2API_IKMove(void *ghoul2, int time,
...
static qboolean CL_G2API_RemoveBone(void *ghoul2, const char *boneName,
                                    int modelIndex) {
  if (!ghoul2)
    return qfalse;
  CGhoul2Info_v &g2 = *((CGhoul2Info_v *)ghoul2);
...
static void CL_G2API_AttachInstanceToEntNum(void *ghoul2, int entityNum,
                                            qboolean server) {
  if (!ghoul2)
    return;
...
static void CL_G2API_GetSurfaceName(void *ghoul2, int surfNumber,
...
intptr_t CL_CgameSystemCalls(intptr_t *args) {
  ...
  case CG_MILLISECONDS:
    ...
    // supplied ptr, or you'll get a nasty memory leak. not that you should be
    // using these outside of debug anyway.. because you shouldn't be. So don't.
  ...
  case CG_CM_TRANSFORMEDPOINTCONTENTS:
    return CM_TransformedPointContents((const float *)VMA(1), args[2],
                                       (const float *)VMA(3),
                                       (const float *)VMA(4));
  ...
  case CG_CM_TRANSFORMEDBOXTRACE:
    CM_TransformedBoxTrace(
        (trace_t *)VMA(1), (const float *)VMA(2), (const float *)VMA(3),
        (const float *)VMA(4), (const float *)VMA(5), args[6], args[7],
        (const float *)VMA(8), (const float *)VMA(9), /*int capsule*/ qfalse);
    return 0;
  ...
  case CG_CM_TRANSFORMEDCAPSULETRACE:
    CM_TransformedBoxTrace(
        (trace_t *)VMA(1), (const float *)VMA(2), (const float *)VMA(3),
        (const float *)VMA(4), (const float *)VMA(5), args[6], args[7],
        (const float *)VMA(8), (const float *)VMA(9), /*int capsule*/ qtrue);
    return 0;

  case CG_CM_MARKFRAGMENTS:
    return re->MarkFragments(args[1], (const vec3_t *)VMA(2),
                             (const float *)VMA(3), args[4], (float *)VMA(5),
                             args[6], (markFragment_t *)VMA(7));
  ...
  case CG_S_ADDLOOPINGSOUND:
    S_AddLoopingSound(args[1], (const float *)VMA(2), (const float *)VMA(3),
                      args[4]);
    return 0;

  case CG_S_ADDREALLOOPINGSOUND:
    /*S_AddRealLoopingSound*/ S_AddLoopingSound(args[1], (const float *)VMA(2),
                                                (const float *)VMA(3), args[4]);
    return 0;
  ...
  case CG_S_ADDLOCALSET:
    return S_AddLocalSet((const char *)VMA(1), (float *)VMA(2), (float *)VMA(3),
                         args[4], args[5]);
  ...
  case CG_R_FONT_DRAWSTRING:
    re->Font_DrawString(args[1], args[2], (const char *)VMA(3),
                        (const float *)VMA(4), args[5], args[6], VMF(7));
    return 0;
  ...
  case CG_ANYLANGUAGE_READCHARFROMSTRING:
    return re->AnyLanguage_ReadCharFromString(
        (const char *)VMA(1), (int *)VMA(2), (qboolean *)VMA(3));
  ...
  case CG_R_ADDDECALTOSCENE:
    re->AddDecalToScene((qhandle_t)args[1], (const float *)VMA(2),
                        (const float *)VMA(3), VMF(4), VMF(5), VMF(6), VMF(7),
                        VMF(8), (qboolean)args[9], VMF(10), (qboolean)args[11]);
    return 0;

  case CG_R_LIGHTFORPOINT:
    return re->LightForPoint((float *)VMA(1), (float *)VMA(2), (float *)VMA(3),
                             (float *)VMA(4));
  ...
  case CG_R_ADDADDITIVELIGHTTOSCENE:
    re->AddAdditiveLightToScene((const float *)VMA(1), VMF(2), VMF(3), VMF(4),
                                VMF(5));
    return 0;
  ...
  case CG_R_LERPTAG:
    return re->LerpTag((orientation_t *)VMA(1), args[2], args[3], args[4],
                       VMF(5), (const char *)VMA(6));

  case CG_R_DRAWROTATEPIC:
    re->DrawRotatePic(VMF(1), VMF(2), VMF(3), VMF(4), VMF(5), VMF(6), VMF(7),
                      VMF(8), VMF(9), args[10]);
    return 0;

  case CG_R_DRAWROTATEPIC2:
    re->DrawRotatePic2(VMF(1), VMF(2), VMF(3), VMF(4), VMF(5), VMF(6), VMF(7),
                       VMF(8), VMF(9), args[10]);
    return 0;
  ...
  case CG_R_SETREFRACTIONPROP:
    re->SetRefractionProperties(VMF(1), VMF(2), (qboolean)args[3],
                                (qboolean)args[4]);
    return 0;
  ...
  case CG_SETUSERCMDVALUE:
    _CL_SetUserCmdValue(args[1], VMF(2), VMF(3), VMF(4), VMF(5), args[6],
                        args[7], (qboolean)args[8]);
    return 0;
  ...
  case CG_CIN_PLAYCINEMATIC:
    return CIN_PlayCinematic((const char *)VMA(1), args[2], args[3], args[4],
                             args[5], args[6]);
  ...
  case CG_R_REMAP_SHADER:
    re->RemapShader((const char *)VMA(1), (const char *)VMA(2),
                    (const char *)VMA(3));
    return 0;
  ...
  case CG_FX_ADDLINE:
    CGFX_AddLine((float *)VMA(1), (float *)VMA(2), VMF(3), VMF(4), VMF(5),
                 VMF(6), VMF(7), VMF(8), (float *)VMA(9), (float *)VMA(10),
                 VMF(11), args[12], args[13], args[14]);
    return 0;
  ...
  case CG_FX_PLAY_EFFECT_ID:
    FX_PlayEffectID(args[1], (float *)VMA(2), (float *)VMA(3), args[4],
                    args[5]);
    return 0;
  ...
  case CG_FX_PLAY_ENTITY_EFFECT_ID:
    FX_PlayEntityEffectID(args[1], (float *)VMA(2), (vec3_t *)VMA(3), args[4],
                          args[5], args[6], args[7]);
    return 0;

  case CG_FX_PLAY_BOLTED_EFFECT_ID:
    return CGFX_PlayBoltedEffectID(args[1], (float *)VMA(2), (void *)args[3],
  ...
  case CG_G2_SETMODELS:
    CL_G2API_SetGhoul2ModelIndexes(VMA(1), (qhandle_t *)VMA(2),
                                   (qhandle_t *)VMA(3));
    return 0;

  case CG_G2_GETBOLT:
    return CL_G2API_GetBoltMatrix(
  ...
  case CG_G2_GETBOLT_NOREC:
    return CL_G2API_GetBoltMatrix_NoReconstruct(
        VMA(1), args[2], args[3], (mdxaBone_t *)VMA(4), (const float *)VMA(5),
        (const float *)VMA(6), args[7], (qhandle_t *)VMA(8), (float *)VMA(9));

  case CG_G2_GETBOLT_NOREC_NOROT:
    return CL_G2API_GetBoltMatrix_NoRecNoRot(
        VMA(1), args[2], args[3], (mdxaBone_t *)VMA(4), (const float *)VMA(5),
        (const float *)VMA(6), args[7], (qhandle_t *)VMA(8), (float *)VMA(9));

  case CG_G2_INITGHOUL2MODEL:
    return CL_G2API_InitGhoul2Model((void **)VMA(1), (const char *)VMA(2),
  ...
  case CG_G2_COLLISIONDETECT:
    CL_G2API_CollisionDetect((CollisionRecord_t *)VMA(1), VMA(2),
  ...
  case CG_G2_ANGLEOVERRIDE:
    return CL_G2API_SetBoneAngles(
        VMA(1), args[2], (const char *)VMA(3), (float *)VMA(4), args[5],
        args[6], args[7], args[8], (qhandle_t *)VMA(9), args[10], args[11]);
  ...
  case CG_G2_PLAYANIM:
    return CL_G2API_SetBoneAnim(VMA(1), args[2], (const char *)VMA(3), args[4],
                                args[5], args[6], VMF(7), args[8], VMF(9),
                                args[10]);

  case CG_G2_GETBONEANIM:
    return CL_G2API_GetBoneAnim(
  ...
  case CG_G2_GETBONEFRAME:
    return CL_G2API_GetBoneFrame(VMA(1), (const char *)VMA(2), args[3],
                                 (float *)VMA(4), (int *)VMA(5), args[6]);
  ...
  case CG_G2_GETSURFACERENDERSTATUS:
    return CL_G2API_GetSurfaceRenderStatus(VMA(1), args[2],
                                           (const char *)VMA(3));
  ...
  case CG_G2_ANIMATEG2MODELS:
    CL_G2API_AnimateG2Models(VMA(1), args[2],
                             (sharedRagDollUpdateParams_t *)VMA(3));
    return 0;

    // additional ragdoll options -rww
  case CG_G2_RAGPCJCONSTRAINT:
    return CL_G2API_RagPCJConstraint(VMA(1), (const char *)VMA(2),
  ...
  case CG_G2_RAGEFFECTORGOAL:
    return CL_G2API_RagEffectorGoal(VMA(1), (const char *)VMA(2),
                                    (float *)VMA(3));

  case CG_G2_GETRAGBONEPOS:
    return CL_G2API_GetRagBonePos(VMA(1), (const char *)VMA(2), (float *)VMA(3),
                                  (float *)VMA(4), (float *)VMA(5),
                                  (float *)VMA(6));

  case CG_G2_RAGEFFECTORKICK:
    return CL_G2API_RagEffectorKick(VMA(1), (const char *)VMA(2),
                                    (float *)VMA(3));
  ...
  case CG_G2_SETBONEIKSTATE:
    return CL_G2API_SetBoneIKState(VMA(1), args[2], (const char *)VMA(3),
                                   args[4],
  ...
  case CG_SP_GETSTRINGTEXTSTRING:
    return CL_SE_GetStringTextString((const char *)VMA(1), (char *)VMA(2),
                                     args[3]);
...
void CL_BindCGame(void) {
  ...
  if (cgvm && !cgvm->isLegacy) {
    ...
    cgi.G2API_GetBoltMatrix_NoReconstruct =
        CL_G2API_GetBoltMatrix_NoReconstruct;
