...
static void CG_ParseTeamInfo(void) {
  ...
  if (numSortedTeamPlayers < 0 || numSortedTeamPlayers > TEAM_MAXOVERLAY) {
    trap->Error(ERR_DROP,
                "CG_ParseTeamInfo: numSortedTeamPlayers out of range (%d)",
...
void CG_ParseServerinfo(void) {
  ...
  cgs.debugMelee = atoi(Info_ValueForKey(
      info, "g_debugMelee")); // trap->Cvar_GetHiddenVarValue("g_iknowkungfu");
  ...
  if (cgs.wDisable != value) {
    ...
    for (i = 1, item = bg_itemlist, itemInfo = cg_items; i < bg_numItems;
         i++, item++, itemInfo++) { // register all weapons that aren't disabled
  ...
  cgs.maxclients =
      Com_Clampi(0, MAX_CLIENTS, atoi(Info_ValueForKey(info, "sv_maxclients")));
  mapname = Info_ValueForKey(info, "mapname");

  // rww - You must do this one here, Info_ValueForKey always uses the same
  // memory pointer.
  ...
  //sizeof(cgs.redTeam) ); 	trap->Cvar_Set("g_redTeam", cgs.redTeam); 	Q_strncpyz(
  //cgs.blueTeam, Info_ValueForKey( info, "g_blueTeam" ), sizeof(cgs.blueTeam)
  ...
  trap->Cvar_Set("ui_about_botminplayers",
...
void SetCustomSoundForType(clientInfo_t *ci, int setType, int index,
                           sfxHandle_t sfx) {
  switch (setType) {
  case 1:
    ci->sounds[index] = sfx;
...
static void CG_RegisterCustomSounds(clientInfo_t *ci, int setType,
                                    const char *psDir) {
  int iTableEntries = 0;
  int i;
        ...
        if (atoi(testNumber)) {
          *p = 0;
          ...
          hSFX = trap->S_RegisterSound(
              va("sound/chars/%s/misc/%s", psDir, modifiedSound));
        }
      }
    }
...
void CG_PrecacheNPCSounds(const char *str) {
  ...
  while (i < 4) // 4 types
  { // It would be better if we knew what type this actually was (extra, combat,
    // jedi, etc). But that would require extra configstring indexing and that is
    // a bad thing.

    while (j < MAX_CUSTOM_SOUNDS) {
      const char *s = GetCustomSoundForType(i + 1, j);

      if (s && s[0]) { // whatever it is, try registering it under this folder.
        k = 1;
        while (s[k]) {
          sEnd[k - 1] = s[k];
          k++;
...
void CG_HandleNPCSounds(centity_t *cent) {
  ...
  if (cent->currentState.csSounds_Std) {
    const char *s =
        CG_ConfigString(CS_SOUNDS + cent->currentState.csSounds_Std);
    ...
    if (s && s[0]) {
      ...
      // Parse past the initial "*" which indicates this is a custom sound, and
      // the $ which indicates it is an NPC custom sound dir.
      while (s[i]) {
        sEnd[j] = s[i];
        j++;
        i++;
  ...
  if (cent->currentState.csSounds_Combat) {
    const char *s =
        CG_ConfigString(CS_SOUNDS + cent->currentState.csSounds_Combat);
    ...
    memset(&cent->npcClient->combatSounds, 0,
           sizeof(cent->npcClient->combatSounds));
  }

  // extra
  if (cent->currentState.csSounds_Extra) {
    const char *s =
        CG_ConfigString(CS_SOUNDS + cent->currentState.csSounds_Extra);
    ...
    memset(&cent->npcClient->extraSounds, 0,
           sizeof(cent->npcClient->extraSounds));
  }

  // jedi
  if (cent->currentState.csSounds_Jedi) {
    const char *s =
        CG_ConfigString(CS_SOUNDS + cent->currentState.csSounds_Jedi);
    ...
    memset(&cent->npcClient->jediSounds, 0,
           sizeof(cent->npcClient->jediSounds));
  }
}
...
// nmckenzie: DUEL_HEALTH - fixme - we could really clean this up immensely with
...
static void CG_ConfigStringModified(void) {
  ...
  if (num == CS_MUSIC) {
    ...
    if (strstr(modelName, ".glm") ||
        modelName[0] == '$') { // Check to see if it has a custom skin attached.
    ...
    */
    ...
    } else if (str[1] == '$') { // an NPC soundset
      CG_PrecacheNPCSounds(str);
    }
  } else if (num >= CS_EFFECTS && num < CS_EFFECTS + MAX_FX) {
    if (str[0] == '*') { // it's a special global weather effect
    ...
    CG_ShaderStateChanged();
             ...
             num < CS_LIGHT_STYLES + (MAX_LIGHT_STYLES * 3)) {
...
void CG_KillCEntityG2(int entNum) {
  ...
  if (ci) {
    if (ci == cent->npcClient) { // never going to be != cent->ghoul2, unless
                                 // cent->ghoul2 has already been removed (and
                                 // then this ptr is not valid)
      ci->ghoul2Model = NULL;
    ...
    while (j < MAX_SABERS) {
      if (ci->ghoul2Weapons[j] &&
          trap->G2_HaveWeGhoul2Models(ci->ghoul2Weapons[j])) {
  ...
  while (i < MAX_GENTITIES) {
    ...
    if (i >= MAX_CLIENTS &&
        cent->currentState.number ==
            i) { // do not clear G2 instances on client ents, they are constant
      CG_KillCEntityG2(i);
    }
...
static void CG_MapRestart(void) {
  ...
  if (cg.warmup == 0 && cgs.gametype != GT_SIEGE &&
      cgs.gametype != GT_POWERDUEL /* && cgs.gametype == GT_DUEL */) {
    trap->S_StartLocalSound(cgs.media.countFightSound, CHAN_ANNOUNCER);
    CG_CenterPrint(CG_GetStringEdString("MP_SVGAME", "BEGIN_DUEL"), 120,
                   GIANTCHAR_WIDTH * 2);
  }
  /*
  ...
  *cg_recordSPDemoName.string) { trap->SendConsoleCommand(va("set
  g_synchronousclients 1 ; record %s \n", cg_recordSPDemoName.string));
          }
  }
  */
...
#define MAX_STRINGED_SV_STRING                                                 \
  1024 // this is an quake-engine limit, not a StringEd limit

void CG_CheckSVStringEdRef(
    char *buf,
    const char *str) { // I don't really like doing this. But it utilizes the
  ...
  while (i < strLen && str[i]) {
    ...
    if (str[i] == '@' && (i + 1) < strLen) {
      if (str[i + 1] == '@' && (i + 2) < strLen) {
        if (str[i + 2] == '@' &&
            (i + 3) <
                strLen) { //@@@ should mean to insert a StringEd reference here,
                          //so insert it into buf at the current place
          ...
          while (i < strLen && str[i] && str[i] != ' ' && str[i] != ':' &&
                 str[i] != '.' && str[i] != '\n') {
            stringRef[r] = str[i];
            r++;
            i++;
          ...
          buf[b] = 0;
          Q_strcat(buf, MAX_STRINGED_SV_STRING,
                   CG_GetStringEdString("MP_SVGAME", stringRef));
          b = strlen(buf);
        }
      }
    }
...
static void CG_BodyQueueCopy(centity_t *cent, int clientNum, int knownWeapon) {
  ...
  cent->isRagging = qfalse; // reset in case it's still set from another body
                            // that was in this cent slot.
  cent->ownerRagging =
      source->isRagging; // if the owner was in ragdoll state, then we want to
                         // go into it too right away.
  ...
  if (source->isRagging) { // just reset it now.
    source->isRagging = qfalse;
    trap->G2API_SetRagDoll(
        source->ghoul2, NULL); // calling with null parms resets to no ragdoll.
  }

  // either force the weapon from when we died or remove it if it was a dropped
  // weapon
  if (knownWeapon > WP_BRYAR_PISTOL &&
    ...
    trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, knownWeapon),
                                        0, cent->ghoul2, 1);
  }
  ...
  if (!cent->ownerRagging) {
    ...
    // anim = &bgAllAnims[cent->localAnimIndex].anims[
    // cent->currentState.torsoAnim ];
    if (!BG_InDeathAnim(
            source->currentState
                .torsoAnim)) { // then just snap the corpse into a default
      anim = &bgAllAnims[source->localAnimIndex].anims[BOTH_DEAD1];
      fallBack = qtrue;
    } else {
      anim = &bgAllAnims[source->localAnimIndex]
                  .anims[source->currentState.torsoAnim];
    }
    ...
    // if (!cgs.clientinfo[source->currentState.number].frame ||
    // (cent->currentState.torsoAnim) != (source->currentState.torsoAnim) )
    //{
    //	aNum = (anim->firstFrame+anim->numFrames)-1;
    //}

    trap->G2API_SetBoneAnim(cent->ghoul2, 0, "upper_lumbar", aNum, eFrame,
                            flags, animSpeed, cg.time, -1, 150);
    trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", aNum, eFrame, flags,
                            animSpeed, cg.time, -1, 150);
    trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Motion", aNum, eFrame, flags,
                            animSpeed, cg.time, -1, 150);
  }
...
static void CG_KillGhoul2_f(void) {
  ...
  // If it has been occupied since this message was sent somehow, the worst that
  // can (should) happen is the instance will have to reinit with its current
  // info.
  int indexNum = 0;
  ...
  for (i = 1; i < argNum; i++) {
    ...
    if (cg_entities[indexNum].ghoul2 &&
        trap->G2_HaveWeGhoul2Models(cg_entities[indexNum].ghoul2)) {
      if (indexNum < MAX_CLIENTS) { // You try to do very bad thing!
#ifdef _DEBUG
        Com_Printf("WARNING: Tried to kill a client ghoul2 instance with a kg2 "
                   "command!\n");
#endif
        return;
      }
...
static void CG_RestoreClientGhoul_f(void) {
  // rcg - Restore Client Ghoul (make sure limbs are reattached and ragdoll
  // state is reset - this must be done reliably)
  int indexNum = 0;
  int argNum = trap->Cmd_Argc();
  centity_t *clent;
  ...
  if (clent->isRagging) {
    ...
    trap->G2API_SetRagDoll(
        clent->ghoul2, NULL); // calling with null parms resets to no ragdoll.
  }
...
static void CG_Chat_f(void) {
  ...
  if (!strcmp(cmd, "chat")) {
    ...
    if (!cg_teamChatsOnly.integer) {
      char name[MAX_NETNAME] = {0}, loc[MAX_STRING_CHARS] = {0}, color[8] = {0},
           message[MAX_STRING_CHARS] = {0};
      ...
      if (cg_chatBeep.integer)
        trap->S_StartLocalSound(cgs.media.talkSound, CHAN_LOCAL_SOUND);
      Com_sprintf(text, sizeof(text), "%s^7<%s> ^%s%s", name, loc, color,
                  message);
      CG_RemoveChatEscapeChar(text);
      CG_ChatBox_AddString(text);
      trap->Print("*%s\n", text);
...
static serverCommand_t commands[] = {
    ...
    {"loaddefered",
     CG_LoadDeferredPlayers}, // FIXME: spelled wrong, but not changing for demo
    {"ltchat", CG_Chat_f},
...
static void CG_ServerCommand(void) {
  ...
  command = (serverCommand_t *)Q_LinearSearch(cmd, commands, numCommands,
