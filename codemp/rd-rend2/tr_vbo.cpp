...
uint32_t R_VboPackTangent(vec4_t v) {
	return (((uint32_t)(v[3] * 1.5f + 2.0f)) << 30) | (((uint32_t)(v[2] * 511.5f + 512.0f)) << 20) | (((uint32_t)(v[1] * 511.5f + 512.0f)) << 10) |
		   (((uint32_t)(v[0] * 511.5f + 512.0f)));
}

uint32_t R_VboPackNormal(vec3_t v) {
	return (((uint32_t)(v[2] * 511.5f + 512.0f)) << 20) | (((uint32_t)(v[1] * 511.5f + 512.0f)) << 10) | (((uint32_t)(v[0] * 511.5f + 512.0f)));
}
...
void R_InitGPUBuffers(void) {
	...
	qglBufferData(GL_UNIFORM_BUFFER, MAX_SHADERS * alignedBlockSize, nullptr, GL_STATIC_DRAW);
...
void AddVertexArray(VertexArraysProperties *properties, int attributeIndex, size_t size, int stride, int offset, void *stream, int streamStride) {
...
void CalculateVertexArraysProperties(uint32_t attributes, VertexArraysProperties *properties) {
	...
	if (!attributes) {
		attributes = ATTR_POSITION | ATTR_TEXCOORD0 | ATTR_TEXCOORD1 | ATTR_NORMAL | ATTR_TANGENT | ATTR_COLOR | ATTR_LIGHTDIRECTION;
	}

	if (attributes & ATTR_BITS) {
		if (attributes & ATTR_POSITION)
			AddVertexArray(properties, ATTR_INDEX_POSITION, sizeof(tess.xyz[0]), 0, properties->vertexDataSize, tess.xyz, sizeof(tess.xyz[0]));

		if (attributes & ATTR_TEXCOORD0)
			AddVertexArray(properties, ATTR_INDEX_TEXCOORD0, sizeof(tess.texCoords[0][0]), 0, properties->vertexDataSize, tess.texCoords[0][0],
						   sizeof(tess.texCoords[0][0]) * NUM_TESS_TEXCOORDS);

		if (attributes & ATTR_TEXCOORD1)
			AddVertexArray(properties, ATTR_INDEX_TEXCOORD1, sizeof(tess.texCoords[0][1]), 0, properties->vertexDataSize, tess.texCoords[0][1],
						   sizeof(tess.texCoords[0][0]) * NUM_TESS_TEXCOORDS);

		if (attributes & ATTR_TEXCOORD2)
			AddVertexArray(properties, ATTR_INDEX_TEXCOORD2, sizeof(tess.texCoords[0][2]) * 2, 0, properties->vertexDataSize, tess.texCoords[0][2],
						   sizeof(tess.texCoords[0][0]) * NUM_TESS_TEXCOORDS);
		;

		if (attributes & ATTR_TEXCOORD3)
			AddVertexArray(properties, ATTR_INDEX_TEXCOORD3, sizeof(tess.texCoords[0][3]) * 2, 0, properties->vertexDataSize, tess.texCoords[0][3],
						   sizeof(tess.texCoords[0][0]) * NUM_TESS_TEXCOORDS);

		if (attributes & ATTR_TEXCOORD4)
			AddVertexArray(properties, ATTR_INDEX_TEXCOORD4, sizeof(tess.texCoords[0][4]) * 2, 0, properties->vertexDataSize, tess.texCoords[0][4],
						   sizeof(tess.texCoords[0][0]) * NUM_TESS_TEXCOORDS);

		if (attributes & ATTR_NORMAL)
			AddVertexArray(properties, ATTR_INDEX_NORMAL, sizeof(tess.normal[0]), 0, properties->vertexDataSize, tess.normal, sizeof(tess.normal[0]));

		if (attributes & ATTR_TANGENT)
			AddVertexArray(properties, ATTR_INDEX_TANGENT, sizeof(tess.tangent[0]), 0, properties->vertexDataSize, tess.tangent, sizeof(tess.tangent[0]));
...
void CalculateVertexArraysFromVBO(uint32_t attributes, const VBO_t *vbo, VertexArraysProperties *properties) {
	...
	for (int i = 0, j = 1; i < ATTR_INDEX_MAX; i++, j <<= 1) {
		if (vbo->sizes[i] == 0)
			continue;
		if (attributes & j)
			AddVertexArray(properties, i, vbo->sizes[i], vbo->strides[i], vbo->offsets[i], NULL, 0);
	}
}
...
void RB_UpdateVBOs(unsigned int attribBits) {
	...
	if (tess.numVertexes > 0 && tess.numVertexes <= SHADER_MAX_VERTEXES) {
		...
		if (glRefConfig.immutableBuffers) {
			...
			dstPtr = qglMapBufferRange(GL_ARRAY_BUFFER, currentFrame->dynamicVboWriteOffset, totalVertexDataSize, mapFlags);
		}
	...
	if (tess.numIndexes > 0 && tess.numIndexes <= SHADER_MAX_INDEXES) {
		...
		if (glRefConfig.immutableBuffers) {
			...
			dst = qglMapBufferRange(GL_ELEMENT_ARRAY_BUFFER, currentFrame->dynamicIboWriteOffset, totalIndexDataSize, mapFlags);
		}
...
void RB_UpdateGoreVBO(srfG2GoreSurface_t *goreSurface) {
	...
	qglBufferSubData(GL_ARRAY_BUFFER, sizeof(g2GoreVert_t) * tr.goreVBOCurrentIndex, sizeof(g2GoreVert_t) * goreSurface->numVerts, goreSurface->verts);
	...
	qglBufferSubData(GL_ELEMENT_ARRAY_BUFFER, sizeof(glIndex_t) * tr.goreIBOCurrentIndex, sizeof(glIndex_t) * goreSurface->numIndexes, goreSurface->indexes);
...
void RB_BindUniformBlock(GLuint ubo, uniformBlock_t block, int offset) {
	...
	if (currentBinding->buffer != ubo || currentBinding->offset != offset || currentBinding->size != blockInfo->size) {
		qglBindBufferRange(GL_UNIFORM_BUFFER, blockInfo->slot, ubo, offset, blockInfo->size);
...
int RB_BindAndUpdateFrameUniformBlock(uniformBlock_t block, void *data) {
	...
	qglBufferSubData(GL_UNIFORM_BUFFER, thisFrame->uboWriteOffset, blockInfo->size, data);
...
int RB_AddShaderInstanceBlock(void *data) {
	...
	qglBufferSubData(GL_UNIFORM_BUFFER, tr.shaderInstanceUboWriteOffset, sizeof(ShaderInstanceBlock), data);
...
void RB_BeginConstantsUpdate(gpuFrame_t *frame) {
	...
	const GLbitfield mapFlags = GL_MAP_WRITE_BIT | GL_MAP_UNSYNCHRONIZED_BIT | GL_MAP_FLUSH_EXPLICIT_BIT;

	frame->uboMapBase = frame->uboWriteOffset;
	frame->uboMemory = qglMapBufferRange(GL_UNIFORM_BUFFER, frame->uboWriteOffset, frame->uboSize - frame->uboWriteOffset, mapFlags);
}

int RB_AppendConstantsData(gpuFrame_t *frame, const void *data, size_t dataSize) {
...
void RB_EndConstantsUpdate(const gpuFrame_t *frame) {
	qglFlushMappedBufferRange(GL_UNIFORM_BUFFER, frame->uboMapBase, frame->uboWriteOffset - frame->uboMapBase);
