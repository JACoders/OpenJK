...
void SG_WipeSavegame(const char *psPathlessBaseName) { ojk::SavedGame::remove(psPathlessBaseName); }
...
// actually saved
*/
void SG_StoreSaveGameComment(const char *sComment) { memmove(saveGameComment, sComment, iSG_COMMENT_SIZE); }
...
void SV_LoadGame_f(void) {
	...
	if (!Q_stricmp(psFilename, "*respawn")) {
		...
		if (sLastSaveFileLoaded[0]) {
			...
			if (!Q_stricmp(psMapName, "_brig")) { // if you're in the brig and there is no autosave, load the last loaded savegame
...
void SG_WriteCvars(void) {
	...
	for (var = cvar_vars; var; var = var->next) {
		...
		saved_game.write_chunk(INT_ID('C', 'V', 'A', 'R'), var->name, static_cast<int>(strlen(var->name) + 1));

		saved_game.write_chunk(INT_ID('V', 'A', 'L', 'U'), var->string, static_cast<int>(strlen(var->string) + 1));
	}
}
	...
	for (i = 0; i < MAX_CONFIGSTRINGS; i++) {
		if (i != CS_SYSTEMINFO) {
			if (sv.configstrings[i] && strlen(sv.configstrings[i])) {
				...
				saved_game.write_chunk(INT_ID('C', 'S', 'D', 'A'), ::sv.configstrings[i], static_cast<int>(strlen(::sv.configstrings[i]) + 1));
			}
		}
	}
}
...
static unsigned int SG_UnixTimestamp(const time_t &t) { return static_cast<unsigned int>(t); }
...
static time_t SG_GetTime(unsigned int timestamp) { return static_cast<time_t>(timestamp); }
...
int SG_GetSaveGameComment(const char *psPathlessBaseName, char *sComment, char *sMapName) {
	...
	if (is_succeed) {
		if (sComment) {
			if (sgh.get_buffer_size() == iSG_COMMENT_SIZE) {
				std::uninitialized_copy_n(static_cast<const char *>(sgh.get_buffer_data()), iSG_COMMENT_SIZE, sComment);
			} else {
				sComment[0] = '\0';
			}
		}
	...
	if (is_succeed) {
		...
		is_succeed = sgh.try_read_chunk<uint32_t>(INT_ID('C', 'M', 'T', 'M'), fileTime);
	...
	if (is_succeed) {
		...
		is_succeed = sgh.try_read_chunk<uint32_t>(INT_ID('S', 'H', 'L', 'N'), iScreenShotLength);
	}
		...
		if (is_succeed) {
			if (sMapName) {
				if (sgh.get_buffer_size() == iSG_MAPCMD_SIZE) {
					std::uninitialized_copy_n(static_cast<const char *>(sgh.get_buffer_data()), iSG_MAPCMD_SIZE, sMapName);
				} else {
					sMapName[0] = '\0';
				}
			}
...
//
#ifdef JK2_MODE
static bool SG_ReadScreenshot(bool set_as_loading_screen, void *screenshot_ptr) {
	bool is_succeed = true;
	...
	is_succeed = saved_game.try_read_chunk<uint32_t>(INT_ID('S', 'H', 'L', 'N'), screenshot_length);
	...
	if (is_succeed) {
		jpeg_data = static_cast<byte *>(::Z_Malloc(static_cast<int>(screenshot_length + 4096), TAG_TEMP_WORKSPACE, qfalse));
	}
	...
	//
	if (is_succeed) {
		is_succeed = saved_game.try_read_chunk(INT_ID('S', 'H', 'O', 'T'), jpeg_data, static_cast<int>(screenshot_length));
	}
	...
	if (is_succeed) {
		::re.LoadJPGFromBuffer(jpeg_data, screenshot_length, &image, &width, &height);
...
static void SG_WriteScreenshot(qboolean qbAutosave, const char *psMapName) {
	...
	saved_game.write_chunk(INT_ID('S', 'H', 'O', 'T'), pJPGData, static_cast<int>(iJPGDataSize));
...
qboolean SG_ReadSavegame(const char *psPathlessBaseName) {
	...
	::SV_SpawnServer(sMapCmd, eForceReload_NOTHING, (::eSavedGameJustLoaded != eFULL ? qtrue : qfalse));
