...
extern gentity_t *WP_FireVehicleWeapon(gentity_t *ent, vec3_t start, vec3_t dir,
                                       vehWeaponInfo_t *vehWeapon,
                                       qboolean alt_fire,
                                       qboolean isTurretWeap);

extern void G_VehMuzzleFireFX(gentity_t *ent, gentity_t *broadcaster,
                              int muzzlesFired);
//-----------------------------------------------------
void VEH_TurretCheckFire(Vehicle_t *pVeh, gentity_t *parent,
                         // gentity_t *turretEnemy,
                         turretStats_t *turretStats, vehWeaponInfo_t *vehWeapon,
  ...
  if (pVeh->turretStatus[turretNum].ammo <
      vehWeapon->iAmmoPerShot) { // no ammo, can't fire
    return;
  }
  ...
  // if ( turretEnemy )
  {
    ...
    missile = WP_FireVehicleWeapon(parent, pVeh->m_vMuzzlePos[curMuzzle],
                                   pVeh->m_vMuzzleDir[curMuzzle], vehWeapon,
                                   (turretNum != 0), qtrue);
    ...
    nextMuzzle =
        ((curMuzzle + 1) == pVeh->m_pVehicleInfo->turret[turretNum].iMuzzle[0])
    ...
    if (nextMuzzle) { // a valid muzzle to toggle to
      pVeh->turretStatus[turretNum].nextMuzzle =
          nextMuzzle - 1; //-1 because you type muzzles 1-10 in the .veh file
    }
    // add delay to the next muzzle so it doesn't fire right away on the next
    // frame
    pVeh->m_iMuzzleWait[pVeh->turretStatus[turretNum].nextMuzzle] =
        level.time + turretStats->iDelay;
  }
}

void VEH_TurretAnglesToEnemy(Vehicle_t *pVeh, int curMuzzle, float fSpeed,
                             gentity_t *turretEnemy, qboolean bAILead,
  ...
  // FIXME: this isn't quite right, it's aiming from the muzzle, not the center
  // of the turret...
...
//-----------------------------------------------------
qboolean VEH_TurretAim(Vehicle_t *pVeh, gentity_t *parent,
                       gentity_t *turretEnemy, turretStats_t *turretStats,
                       vehWeaponInfo_t *vehWeapon, int turretNum, int curMuzzle,
                       vec3_t desiredAngles)
//-----------------------------------------------------
{
  ...
  if (turretEnemy) {
    ...
    VEH_TurretAnglesToEnemy(pVeh, curMuzzle, vehWeapon->fSpeed, turretEnemy,
                            turretStats->bAILead, desiredAngles);
  }
  ...
  if (pVeh->m_pVehicleInfo->turret[turretNum].pitchClampDown &&
      desiredAngles[PITCH] >
          pVeh->m_pVehicleInfo->turret[turretNum].pitchClampDown) {
    aimCorrect = qfalse;
    desiredAngles[PITCH] =
        pVeh->m_pVehicleInfo->turret[turretNum].pitchClampDown;
  }
  if (pVeh->m_pVehicleInfo->turret[turretNum].pitchClampUp &&
      desiredAngles[PITCH] <
          pVeh->m_pVehicleInfo->turret[turretNum].pitchClampUp) {
    aimCorrect = qfalse;
...
static qboolean VEH_TurretFindEnemies(Vehicle_t *pVeh, gentity_t *parent,
                                      turretStats_t *turretStats, int turretNum,
                                      int curMuzzle)
//-----------------------------------------------------
{
  ...
  for (i = 0; i < count; i++) {
    ...
    if (target == parent || !target->takedamage || target->health <= 0 ||
    ...
    if (!target->client) {             // only attack clients
      if (!(target->flags & FL_BBRUSH) // not a breakable brush
          ...
          || (target->NPC_targetname && parent->targetname &&
              Q_stricmp(target->NPC_targetname, parent->targetname) !=
                  0)) // not in invicible bbrush, but can only be broken by an
                      // NPC that is not me
      {
        if (target->s.weapon == WP_TURRET && target->classname &&
            Q_strncmp("misc_turret", target->classname, 11) ==
                0) { // these guys we want to shoot at
        } else {
          continue;
        }
      }
    ...
    if (target == ((gentity_t *)pVeh->m_pPilot) ||
        target->r.ownerNum ==
            parent->s.number) { // don't get angry at my pilot or passengers?
      continue;
    }
      ...
      if (target->client) {
        if (target->client->sess.sessionTeam ==
          ...
          continue;
                 ...
                 parent->client->sess
                     .sessionTeam) { // some other entity that's allied with us
        continue;
      }
    }
    ...
    trap->Trace(&tr, org2, NULL, NULL, org, parent->s.number, MASK_SHOT, qfalse,
                0, 0);
...
void VEH_TurretObeyPassengerControl(Vehicle_t *pVeh, gentity_t *parent,
                                    int turretNum) {
  turretStats_t *turretStats = &pVeh->m_pVehicleInfo->turret[turretNum];
  gentity_t *passenger =
      (gentity_t *)pVeh->m_ppPassengers[turretStats->passengerNum - 1];

  if (passenger && passenger->client &&
      passenger->health > 0) { // a valid, living passenger client
    ...
    VEH_TurretAim(pVeh, parent, NULL, turretStats, vehWeapon, turretNum,
                  curMuzzle, aimAngles);
    if ((passenger->client->pers.cmd.buttons &
         (BUTTON_ATTACK |
          BUTTON_ALT_ATTACK))) { // he's pressing an attack button, so fire!
      VEH_TurretCheckFire(pVeh, parent, turretStats, vehWeapon, turretNum,
                          curMuzzle);
    }
  }
}
...
//-----------------------------------------------------
{
  ...
  if (turretStats->passengerNum &&
      pVeh->m_iNumPassengers >=
          turretStats->passengerNum) { // the passenger that has control of this
                                       // turret is on the ship
  ...
  if (pVeh->turretStatus[turretNum].enemyEntNum < ENTITYNUM_WORLD) {
    ...
    if (turretEnemy->health < 0 || !turretEnemy->inuse ||
        turretEnemy ==
            ((gentity_t *)pVeh->m_pPilot) // enemy became my pilot///?
        || turretEnemy == parent ||
        turretEnemy->r.ownerNum == parent->s.number // a passenger?
        || (turretEnemy->client &&
            turretEnemy->client->sess.sessionTeam == TEAM_SPECTATOR) ||
        (turretEnemy->client &&
         turretEnemy->client->tempSpectate >=
             level.time)) { // don't keep going after spectators, pilot, self,
                            // dead people, etc.
      turretEnemy = NULL;
  ...
  if (pVeh->turretStatus[turretNum].enemyHoldTime < level.time) {
    if (VEH_TurretFindEnemies(pVeh, parent, turretStats, turretNum,
                              curMuzzle)) {
      turretEnemy = &g_entities[pVeh->turretStatus[turretNum].enemyEntNum];
      doAim = qtrue;
  ...
  if (turretEnemy != NULL) {
    if (turretEnemy->health > 0) {
      // enemy is alive
      WP_CalcVehMuzzle(parent, curMuzzle);
      VectorSubtract(turretEnemy->r.currentOrigin,
                     pVeh->m_vMuzzlePos[curMuzzle], enemyDir);
      enemyDist = VectorLengthSquared(enemyDir);

      if (enemyDist < rangeSq) {
        // was in valid radius
        if (trap->InPVS(pVeh->m_vMuzzlePos[curMuzzle],
          ...
          trap->Trace(&tr, start, NULL, NULL, end, parent->s.number, MASK_SHOT,
                      qfalse, 0, 0);
  ...
  if (doAim) {
    vec3_t aimAngles;
    if (VEH_TurretAim(pVeh, parent, turretEnemy, turretStats, vehWeapon,
                      turretNum, curMuzzle, aimAngles)) {
      VEH_TurretCheckFire(pVeh, parent, /*turretEnemy,*/ turretStats, vehWeapon,
                          turretNum, curMuzzle);
    }
  }
}
