...
void G_VehicleTrace(trace_t *results, const vec3_t start, const vec3_t tMins,
                    ...
                    int contentmask) {
  trap->Trace(results, start, tMins, tMaxs, end, passEntityNum, contentmask,
              qfalse, 0, 0);
}
...
void G_AttachToVehicle(gentity_t *pEnt, usercmd_t **ucmd) {
  ...
  trap->G2API_GetBoltMatrix(vehEnt->ghoul2, 0, crotchBolt, &boltMatrix,
                            ...
                            vehEnt->r.currentOrigin, level.time, NULL,
                            vehEnt->modelScale);
...
void FighterStorePilotViewAngles(Vehicle_t *pVeh, bgEntity_t *parent) {
  ...
  pVeh->m_vPrevRiderViewAngles[YAW] =
      AngleNormalize180(riderPS->viewangles[YAW]);
}
...
qboolean Board(Vehicle_t *pVeh, bgEntity_t *pEnt) {
  ...
  if (!ent ||
      ...
      || (pVeh->m_iBoarding > 0) || (ent->client->ps.m_iVehicleNum))
    return qfalse;
...
qboolean VEH_TryEject(Vehicle_t *pVeh, gentity_t *parent, gentity_t *ent,
  ...
  fVehDiag = sqrtf((parent->r.maxs[0] * parent->r.maxs[0]) +
  ...
  G_VehicleTrace(&m_ExitTrace, ent->r.currentOrigin, vEntMins, vEntMaxs,
                 vExitPos, ent->s.number, ent->clipmask);
...
getItOutOfMe:
  ...
  if ((gentity_t *)pVeh->m_pPilot == ent) {
    ...
    // Didn't find him, can't eject because they aren't in the vehicle
    // (hopefully)!
    ...
    for (i = 0; i < pVeh->m_pVehicleInfo->maxPassengers; i++) {
      if (pVeh->m_ppPassengers[i]) {
        ...
        if (pVeh->m_pVehicleInfo->killRiderOnDeath && rider) { // Kill them, too
          // FIXME: proper origin, MOD and attacker (for credit/death message)?
          // Get from vehicle?
          G_MuteSound(rider->s.number, CHAN_VOICE);
          G_Damage(rider, NULL, NULL, NULL, rider->s.origin, 10000, 0,
                   MOD_SUICIDE); // FIXME: proper MOD?  Get from vehicle?
        }
#endif
      }
  ...
  if (level.time >= pVeh->m_iDieTime) {
    ...
    if (!pVeh->m_pVehicleInfo->Inhabited(
      ...
      if (pVeh->m_pVehicleInfo->iExplodeFX) {
        ...
        G_PlayEffectID(pVeh->m_pVehicleInfo->iExplodeFX,
                       parent->r.currentOrigin, fxAng);
        // trace down and place mark
        VectorCopy(parent->r.currentOrigin, bottom);
        bottom[2] -= 80;
        G_VehicleTrace(&trace, parent->r.currentOrigin, vec3_origin,
                       vec3_origin, bottom, parent->s.number, CONTENTS_SOLID);
      ...
      if (pVeh->m_pVehicleInfo->explosionRadius > 0 &&
        ...
        G_VehicleTrace(&trace, parent->r.currentOrigin, lMins, lMaxs, bottom,
...
static qboolean Update(Vehicle_t *pVeh, const usercmd_t *pUmcd) {
  ...
  if (pVeh->m_iDieTime !=
      0) { // NOTE!!!: This HAS to be consistent with cgame!!!
  ...
  if (pVeh->m_iNumPassengers > 0) {
    ...
    for (i = 0; i < pVeh->m_pVehicleInfo->maxPassengers; i++) {
      ...
      if (psngr && (!psngr->inuse || !psngr->client || psngr->health <= 0 ||
  ...
  */
  ...
  for (i = 0; i < MAX_VEHICLE_WEAPONS;
    ...
    } else if ((pVeh->m_ucmd.buttons &
                BUTTON_USE_HOLDABLE)) { // pilot pressed the "weapon link"
                                        // toggle button
  ...
  */
  ...
  if (pVeh->m_pVehicleInfo->surfDestruction) {
    if (pVeh->m_iRemovedSurfaces) {
      ...
      dmg = (float)parent->client->ps.stats[STAT_MAX_HEALTH] *
            pVeh->m_fTimeModifier / 180.0f;
      G_Damage(parent, killer, killer, NULL, parent->client->ps.origin, dmg,
               DAMAGE_NO_SELF_PROTECTION | DAMAGE_NO_HIT_LOC |
...
// vehicle).
static qboolean UpdateRider(Vehicle_t *pVeh, bgEntity_t *pRider,
                            usercmd_t *pUmcd) {
  gentity_t *parent;
  gentity_t *rider;
  ...
  if (pVeh->m_pVehicleInfo->type != VH_FIGHTER &&
    ...
    if (pUmcd->upmove > 0) {
      ...
      if (pVeh->m_pVehicleInfo->Eject(pVeh, pRider, qfalse)) {
        ...
        if (!trap->ICARUS_TaskIDPending((sharedEntity_t *)rider,
                                        TID_CHAN_VOICE)) {
  ...
  if (pVeh->m_pDroidUnit && pVeh->m_iDroidUnitTag != -1) {
    ...
    if (droid->client) {
      ...
      trap->G2API_GetBoltMatrix(
          ...
          parent->r.currentOrigin, level.time, NULL, parent->modelScale);
...
static void Ghost(Vehicle_t *pVeh, bgEntity_t *pEnt) {
  ...
  // This was introduced to prevent one extra entity from being sent to the
  // clients
...
void G_VehicleDamageBoxSizing(Vehicle_t *pVeh) {
  ...
  trap->Trace(&trace, parent->client->ps.origin, back, nose,
...
int G_FlyVehicleImpactDir(gentity_t *veh, trace_t *trace) {
  ...
  trap->Trace(&localTrace, veh->client->ps.origin, testMins, testMaxs, fPos,
              veh->s.number, veh->clipmask, qfalse, 0, 0);
  ...
  if (noseClear) { // if nose is clear check for tearing the wings off
    ...
    if (!(pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_E) ||
      ...
      trap->Trace(&localTrace, rWing, testMins, testMaxs, fPos, veh->s.number,
                  veh->clipmask, qfalse, 0, 0);
      if (localTrace.startsolid || localTrace.allsolid ||
    ...
    if (!(pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_C) ||
      ...
      trap->Trace(&localTrace, lWing, testMins, testMaxs, fPos, veh->s.number,
                  veh->clipmask, qfalse, 0, 0);
      if (localTrace.startsolid || localTrace.allsolid ||
          localTrace.fraction != 1.0f) { // impact
