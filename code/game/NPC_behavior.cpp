...
/*
FIXME - MCG:
These all need to make use of the snapshots.  Write something that can look for
only specific things in a snapshot or just go through the snapshot every frame
and save the info in case we need it...
*/
...
void NPC_BSAdvanceFight(void) { // FIXME: IMPLEMENT
  ...
  if (NPCInfo->captureGoal) { // FIXME: if no captureGoal, what do we do?
    // VectorCopy( NPCInfo->captureGoal->currentOrigin,
    // NPCInfo->tempGoal->currentOrigin ); NPCInfo->goalEntity =
    // NPCInfo->tempGoal;
  ...
  if (NPC->enemy) { // See if we can shoot him
    ...
    if (attack_ok) {
      ...
      enemyVisibility =
          NPC_CheckVisibility(NPC->enemy, CHECK_FOV); // CHECK_360|//CHECK_PVS|
      ...
      if (enemyVisibility == VIS_FOV) { // He's in our FOV
        ...
        if (attack_ok) {
          ...
          if (traceEnt != NPC->enemy &&
              (!traceEnt || !traceEnt->client || !NPC->client->enemyTeam ||
               NPC->client->enemyTeam !=
                   traceEnt->client->playerTeam)) { // no, so shoot for the head
            attack_scale *= 0.75;
            gi.trace(&tr, muzzle, NULL, NULL, enemy_head, NPC->s.number,
          ...
          if (traceEnt == NPC->enemy ||
              (traceEnt->client && NPC->client->enemyTeam &&
        ...
        if (attack_ok) {
          ...
          if (!dead_on) { // We're not going to hit him directly, try a
                          // suppressing fire
            ...
            if (aim_off >
                Q_flrand(0.0f, 1.0f) *
                    max_aim_off) // FIXME: use aim value to allow poor aim?
            {
              attack_scale *= 0.75;
    ...
    if (attack_ok) {
      if (NPC_CheckAttack(
              attack_scale)) { // check aggression to decide if we should shoot
  ...
  if (!ucmd.forwardmove && !ucmd.rightmove) { // We reached our captureGoal
    if (NPC->m_iIcarusID !=
        IIcarusInterface::ICARUS_INVALID /*NPC->taskManager*/) {
...
void NPC_BSInvestigate(void) {
  /*
          //FIXME: maybe allow this to be set as a tempBState in a script?  Just
     specify the
          //investigateGoal, investigateDebounceTime and investigateCount?
     (Needs a macro) vec3_t		invDir, invAngles, spot; gentity_t
     *saveGoal;
          //BS_INVESTIGATE would turn toward goal, maybe take a couple steps
     towards it,
          //look for enemies, then turn away after your investigate counter was
     down-
     ...
     (NPCInfo->stats.vigilance * 1000); NPC_CheckEnemy(qtrue, qfalse);
                  if(NPC->enemy)
          ...
          NPC_SetAnim( NPC, SETANIM_TORSO, TORSO_WEAPONREADY3,
     SETANIM_FLAG_NORMAL );

          if(NPCInfo->stats.vigilance <= 1.0 && NPCInfo->eventOwner)
          {
                  VectorCopy(NPCInfo->eventOwner->currentOrigin,
     NPCInfo->investigateGoal);
          ...
          if(	level.time > NPCInfo->walkDebounceTime )
          {
                  ...
                  VectorSubtract(NPCInfo->investigateGoal, NPC->currentOrigin,
     vec); vec[2] = 0; if(VectorLength(vec) > 64)
                  {
                          if(Q_irand(0, 100) < NPCInfo->investigateCount)
                          {//take a full step
                                  //NPCInfo->walkDebounceTime = level.time +
     1400;
                                  //actually finds length of my BOTH_WALK anim
                                  NPCInfo->walkDebounceTime = PM_AnimLength(
     NPC->client->clientInfo.animFileIndex, BOTH_WALK1 );
                          }
                  }
          }
          ...
          {//walk toward investigateGoal
                  ...
                  VectorCopy(NPCInfo->investigateGoal,
  ...
  /*		NPC_SetMoveGoal( NPC, NPCInfo->investigateGoal, 16, qtrue );
          ...
          else
          {
                  ...
                  NPCInfo->desiredYaw = AngleNormalize360(invAngles[YAW] +
     NPCInfo->hlookCount); NPCInfo->desiredPitch =
     AngleNormalize360(invAngles[PITCH] + NPCInfo->hlookCount);
          }
...
qboolean NPC_CheckInvestigate(int alertEventNum) {
  ...
  // if(!gi.inPVSIgnorePortals(ent->currentOrigin, NPC->currentOrigin))//should
  // we be able to hear through areaportals?
  if (!gi.inPVS(soundPos, NPC->currentOrigin)) { // can hear through doors?
    return qfalse;
  }

  if (owner->client && owner->client->playerTeam && NPC->client->playerTeam &&
      owner->client->playerTeam != NPC->client->playerTeam) {
    if ((float)NPCInfo->investigateCount >= (NPCInfo->stats.vigilance * 200) &&
        owner) { // If investigateCount == 10, just take it as enemy and go
      if (NPC_ValidEnemy(owner)) { // FIXME: run angerscript
...
void NPC_BSSleep(void) {
  ...
  if ( level.time > NPCInfo->enemyCheckDebounceTime )
  {
          ...
          {//only 1 alert per second per 0.1 of vigilance
                  NPCInfo->enemyCheckDebounceTime = level.time +
  (NPCInfo->stats.vigilance * 10000); G_ActivateBehavior(NPC, BSET_AWAKE);
          }
  }
  */
}

extern qboolean NPC_MoveDirClear(int forwardmove, int rightmove,
                                 qboolean reset);
...
bool NPC_BSFollowLeader_UpdateLeader(void) {
  if (NPC->client->leader                            // have a leader
      ...
      && !NPC->client->leader->client->pers
              .enterTime) // player has not finished spawning in yet
...
void NPC_BSFollowLeader_UpdateEnemy(void) {
  if (!NPC->enemy) { // no enemy, find one
    NPC_CheckEnemy((qboolean)(NPCInfo->confusionTime < level.time),
    ...
    if (NPC->enemy) {       // just found one
      ...
      if (!(NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
        ...
        if (eventID > -1 &&
            level.alertEvents[eventID].level >= AEL_SUSPICIOUS &&
          ...
          if (!level.alertEvents[eventID].owner ||
            ...
            // FIXME: what if can't actually see enemy, don't know where he
            // is... should we make them just become very alert and start looking
            // for him?  Or just let combat AI handle this... (act as if you lost
            // him)
    ...
    if (!NPC->enemy) {
      if (NPC->client->leader && NPC->client->leader->enemy &&
          NPC->client->leader->enemy != NPC &&
          ((NPC->client->leader->enemy->client &&
            NPC->client->leader->enemy->client->playerTeam ==
                NPC->client->enemyTeam) ||
           (NPC->client->leader->enemy->svFlags & SVF_NONNPC_ENEMY &&
...
bool NPC_BSFollowLeader_AttackEnemy(void) {
  if (NPC->client->ps.weapon ==
      WP_SABER) //|| NPCInfo->confusionTime>level.time )
  {             // lightsaber user or charmed enemy
    if (NPCInfo->tempBehavior !=
        BS_FOLLOW_LEADER) { // not already in a temp bState
      // go after the guy
  ...
  enemyVisibility = NPC_CheckVisibility(
      NPC->enemy, CHECK_FOV | CHECK_SHOOT); // CHECK_360|CHECK_PVS|
  if (enemyVisibility > VIS_PVS) {          // face
    ...
    if (enemyVisibility >= VIS_SHOOT) { // shoot
      NPC_AimAdjust(2);
      if (NPC_GetHFOVPercentage(NPC->enemy->currentOrigin, NPC->currentOrigin,
                                NPC->client->ps.viewangles,
                                NPCInfo->stats.hfov) > 0.6f &&
          NPC_GetHFOVPercentage(NPC->enemy->currentOrigin, NPC->currentOrigin,
                                NPC->client->ps.viewangles,
                                NPCInfo->stats.vfov) >
...
bool NPC_BSFollowLeader_CanAttack(void) {
  return (NPC->enemy && NPC->client->ps.weapon &&
          !(NPCInfo->aiFlags &
            NPCAI_HEAL_ROSH) // Kothos twins never go after their enemy
  );
}
...
void NPC_BSJump(void) {
  ...
  if (NPCInfo->jumpState != JS_JUMPING && NPCInfo->jumpState != JS_LANDING) {
    ...
    NPCInfo->desiredPitch = NPCInfo->lockedDesiredPitch =
        AngleNormalize360(angles[PITCH]);
  ...
  case JS_FACING:
    if (yawError < MIN_ANGLE_ERROR) { // Facing it, Start crouching
      NPC_SetAnim(NPC, SETANIM_LEGS, BOTH_CROUCH1,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      NPCInfo->jumpState = JS_CROUCHING;
    }
    break;
  case JS_CROUCHING:
    if (NPC->client->ps.legsAnimTimer > 0) { // Still playing crouching anim
      return;
    }
    ...
    */
    ...
    //NPC->targetname, vtos(NPC->client->ps.velocity),
    //NPC->client->ps.gravity/8.0f );
  ...
  case JS_JUMPING:
    ...
    if (NPC->s.groundEntityNum != ENTITYNUM_NONE) { // Landed, start landing
                                                    // anim
      // FIXME: if the
      VectorClear(NPC->client->ps.velocity);
      NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_LAND1,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      NPCInfo->jumpState = JS_LANDING;
      // FIXME: landsound?
    } else if (NPC->client->ps.legsAnimTimer > 0) { // Still playing jumping
                                                    // anim
      // FIXME: apply jump velocity here, a couple frames after start, not right
      // away
      return;
    ...
    break;
  case JS_LANDING:
    if (NPC->client->ps.legsAnimTimer > 0) { // Still playing landing anim
      return;
    } else {
...
void NPC_BSRemove(void) {
  ...
  if (!gi.inPVS(NPC->currentOrigin,
                g_entities[0].currentOrigin)) // FIXME: use cg.vieworg?
  {
...
void NPC_BSSearch(void) {
  ...
  if ((NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES) &&
      NPC->client->enemyTeam != TEAM_NEUTRAL) { // look for enemies
    NPC_CheckEnemy(qtrue, qfalse);
    if (NPC->enemy) { // found one
      if (NPCInfo->tempBehavior ==
          BS_SEARCH) { // if tempbehavior, set tempbehavior to default
        NPCInfo->tempBehavior = BS_DEFAULT;
  ...
  if (!NPCInfo->investigateDebounceTime) { // On our way to a tempGoal
    ...
    if (NPCInfo->tempGoal->waypoint != WAYPOINT_NONE) {
      ...
      float	wpRadSq = waypoints[NPCInfo->tempGoal->waypoint].radius *
      waypoints[NPCInfo->tempGoal->waypoint].radius; if (
    ...
    if (VectorLengthSquared(vec) < minGoalReachedDistSquared) {
      ...
      if (NPC->waypoint == NPCInfo->homeWp) {
        // Just Reached our homeWp, if this is the first time, run your
        // lostenemyscript
    ...
    if (NPCInfo->investigateDebounceTime > level.time) {
      ...
      if (NPCInfo->tempGoal->waypoint != WAYPOINT_NONE) {
        if (!Q_irand(0, 30)) {
          ...
          NPCInfo->desiredYaw =
              AngleNormalize360(vectoyaw(lookDir) + Q_flrand(-45, 45));
        }
      }
      // gi.Printf(".");
...
void NPC_BSWander(void) { // FIXME: don't actually go all the way to the next
                          // waypoint, just move in fits and jerks...?
  ...
  if ((NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES) &&
    ...
    if (NPC->enemy) { // found one
      if (NPCInfo->tempBehavior ==
          BS_WANDER) { // if tempbehavior, set tempbehavior to default
        NPCInfo->tempBehavior = BS_DEFAULT;
      } else { // if bState, change to run and shoot
        NPCInfo->behaviorState = BS_DEFAULT; // BS_HUNT_AND_KILL;
        // NPC_BSRunAndShoot();
    ...
    if (NPCInfo->investigateDebounceTime < level.time ||
      ...
      //-----------------
        ...
        NPC_SetAnim(NPC, SETANIM_BOTH,
                    ((Q_irand(0, 1) == 0) ? (BOTH_GUARD_LOOKAROUND1)
                                          : (BOTH_GUARD_IDLE1)),
                    SETANIM_FLAG_NORMAL);
      }
    }

    // Ok, So We Don't Have A Path, And Debounce Time Is Still Active, So We Are
    // Either Wandering Or Looking Around
    //--------------------------------------------------------------------------------------------------------------
    else {
...
void NPC_Surrender(
    void) { // FIXME: say "don't shoot!" if we weren't already surrendering
  ...
  if (NPCInfo->surrenderTime <
      level.time - 5000) { // haven't surrendered for at least 6 seconds, tell
  ...
  if (NPCInfo->surrenderTime > level.time) {
    if (NPC->client->ps.torsoAnim == BOTH_COWER1_START &&
      ...
      NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_COWER1,
    ...
    if (NPC->client->ps.torsoAnim == BOTH_COWER1 &&
        NPC->client->ps.torsoAnimTimer <= 100) {
      NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_COWER1_STOP,
                  SETANIM_FLAG_HOLD | SETANIM_FLAG_OVERRIDE);
      NPCInfo->surrenderTime = level.time + NPC->client->ps.torsoAnimTimer;
    }
  }
  ...
  //----------------------------------------------
    ...
    if (NPC->client->NPC_class == CLASS_JAWA &&
        NPC->client->ps.weapon == WP_NONE) { // an unarmed Jawa is very scared
      NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_COWER1,
                  SETANIM_FLAG_HOLD | SETANIM_FLAG_OVERRIDE);
      // FIXME: stop doing this if decide to take off and run
    } else {
      // A Big Monster?  OR: Being Tracked By A Homing Rocket?  So Do The Cower
      // Sequence
      //------------------------------------------
      if ((NPC->enemy && NPC->enemy->client &&
           NPC->enemy->client->NPC_class == CLASS_RANCOR) ||
          !TIMER_Done(NPC, "rocketChasing")) {
        NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_COWER1_START,
                    SETANIM_FLAG_HOLD | SETANIM_FLAG_OVERRIDE);
      }
      ...
      //----------------------------------------------------------
        ...
        NPC_SetAnim(NPC, SETANIM_TORSO, TORSO_SURRENDER_START,
                    SETANIM_FLAG_HOLD | SETANIM_FLAG_OVERRIDE);
        NPC->client->ps.torsoAnimTimer =
            Q_irand(3000, 8000); // Pretend the anim lasts longer
      }
    }
...
qboolean NPC_CheckSurrender(void) {
  if (!g_AIsurrender->integer && NPC->client->NPC_class != CLASS_UGNAUGHT &&
      NPC->client->NPC_class != CLASS_JAWA) { // not enabled
    return qfalse;
  }
  if (!Q3_TaskIDPending(NPC, TID_MOVE_NAV) // not scripted to go somewhere
      ...
      !PM_InKnockDown(&NPC->client->ps) // not firing and not on the ground
      && NPC->enemy && NPC->enemy->client && NPC->enemy->enemy == NPC &&
      NPC->enemy->s.weapon != WP_NONE &&
      (NPC->enemy->s.weapon != WP_MELEE ||
       (NPC->enemy->client->NPC_class == CLASS_RANCOR ||
        NPC->enemy->client->NPC_class ==
            CLASS_WAMPA)) // enemy is using a weapon or is a Rancor or Wampa
      && NPC->enemy->health > 20 &&
      NPC->enemy->painDebounceTime < level.time - 3000 &&
      NPC->enemy->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] <
          level.time -
              1000) { // don't surrender if scripted to run somewhere or if
                      // we're in the air or if we're busy or if we don't have an
                      // enemy or if the enemy is not mad at me or is hurt or not
                      // a threat or busy being attacked
    // FIXME: even if not in a group, don't surrender if there are other enemies
    // in the PVS and within a certain range?
    if (NPC->s.weapon != WP_ROCKET_LAUNCHER && NPC->s.weapon != WP_CONCUSSION &&
        NPC->s.weapon != WP_REPEATER && NPC->s.weapon != WP_FLECHETTE &&
        NPC->s.weapon !=
            WP_SABER) { // jedi and heavy weapons guys never surrender
      // FIXME: rework all this logic into some orderly fashion!!!
      if (NPC->s.weapon != WP_NONE) { // they have a weapon so they'd have to
                                      // drop it to surrender
        // don't give up unless low on health
        if (NPC->health > 25 || NPC->health >= NPC->max_health) {
          return qfalse;
        }
        if (g_crosshairEntNum == NPC->s.number &&
            NPC->painDebounceTime > level.time) { // if he just shot me, always
                                                  // give up fall through
        } else { // don't give up unless facing enemy and he's very close
          if (!InFOV(player, NPC, 60, 30)) { // I'm not looking at them
            return qfalse;
          } else if (DistanceSquared(NPC->currentOrigin,
                                     player->currentOrigin) <
                     65536 /*256*256*/) { // they're not close
            return qfalse;
          } else if (!gi.inPVS(NPC->currentOrigin,
                               player->currentOrigin)) { // they're not in the
                                                         // same room
            return qfalse;
          }
        }
      ...
      if (!NPCInfo->group ||
          (NPCInfo->group &&
           NPCInfo->group->numGroup <=
               1)) { // I'm alone but I was in a group//FIXME: surrender anyway
                     // if just melee or no weap?
        if (NPC->s.weapon == WP_NONE
            // NPC has a weapon
            || NPC->enemy == player ||
            (NPC->enemy->s.weapon == WP_SABER && NPC->enemy->client &&
             NPC->enemy->client->ps.SaberActive()) ||
            (NPC->enemy->NPC && NPC->enemy->NPC->group &&
             NPC->enemy->NPC->group->numGroup >
                 2)) { // surrender only if have no weapon or fighting a player
                       // or jedi or if we are outnumbered at least 3 to 1
          if (NPC->enemy == player) { // player is the guy I'm running from
            if (g_crosshairEntNum ==
                NPC->s.number) { // give up if player is aiming at me
              NPC_Surrender();
              NPC_UpdateAngles(qtrue, qtrue);
              return qtrue;
            } else if (player->s.weapon == WP_SABER) { // player is using saber
              if (InFOV(NPC, player, 60, 30)) {        // they're looking at me
                if (DistanceSquared(NPC->currentOrigin, player->currentOrigin) <
                    16384 /*128*128*/) { // they're close
                  if (gi.inPVS(
                          NPC->currentOrigin,
                          player->currentOrigin)) { // they're in the same room
                    NPC_Surrender();
                    NPC_UpdateAngles(qtrue, qtrue);
                    return qtrue;
                  }
            ...
            if (InFOV(NPC, NPC->enemy, 30, 30)) { // they're looking at me
              float maxDist =
                  (64 + (NPC->maxs[0] * 1.5) + (NPC->enemy->maxs[0] * 1.5));
              maxDist *= maxDist;
              if (DistanceSquared(NPC->currentOrigin,
                                  NPC->enemy->currentOrigin) <
                  maxDist) { // they're close
                if (gi.inPVS(NPC->currentOrigin,
                             NPC->enemy
                                 ->currentOrigin)) { // they're in the same room
                  // FIXME: should player-team NPCs not fire on surrendered
                  // NPCs?
                  NPC_Surrender();
                  NPC_UpdateAngles(qtrue, qtrue);
                  return qtrue;
...
void NPC_JawaFleeSound(void) {
  if (NPC && NPC->client && NPC->client->NPC_class == CLASS_JAWA &&
      !Q_irand(0, 3) && NPCInfo->blockedSpeechDebounceTime < level.time &&
      !Q3_TaskIDPending(NPC, TID_CHAN_VOICE)) { // ooteenee!!!!
    // Com_Printf( "ooteenee!!!!\n" );
...
qboolean NPC_BSFlee(void) {
  ...
  enemyRecentlySeen =
      (NPC->enemy && (level.time - NPCInfo->enemyLastSeenTime) < 3000);
  ...
  if (enemyRecentlySeen && // Is There An Enemy Near?
      NPC->client->NPC_class !=
          CLASS_PRISONER &&            // Prisoners can't pickup weapons
      NPCInfo->rank > RANK_CIVILIAN && // Neither can civilians
      TIMER_Done(
          NPC,
          "panic") && // Panic causes him to run for a bit, don't pickup weapons
    ...
    //------------------------------------------
    if (foundWeap &&
        NAV::SafePathExists(NPC->currentOrigin, foundWeap->currentOrigin,
  ...
  if (NPCInfo->goalEntity && !Q3_TaskIDPending(NPC, TID_MOVE_NAV) &&
      NPC->enemy &&
      Distance(NPCInfo->goalEntity->currentOrigin, NPC->enemy->currentOrigin) <
          enemyTooCloseDist) {
  ...
  STEER::Activate(NPC);
  {
    ...
    //---------------------------------------------------------
    if (enemyRecentlySeen && Distance(NPC->enemy->currentOrigin,
                                      NPC->currentOrigin) < enemyTooCloseDist) {
    ...
    //-------------------------------------------------------------
      ...
      if (!moveSuccess && enemyRecentlySeen) {
        ...
        //--------------------------------------------------
        NAV::TNodeHandle Nbr =
            NAV::ChooseFarthestNeighbor(NPC, NPC->enemy->currentOrigin, 0.25f);
        if (Nbr > 0) {
  ...
  if (enemyRecentlySeen) {
    ...
    if (TIMER_Done(NPC, "panic")) {
      // done panicking, time to realize we're dogmeat, if we haven't been able
      // to flee for a few seconds
      if ((level.time - NPC->lastMoveTime) > 3000 &&
          (level.time - NPCInfo->surrenderTime) >
              3000) // and haven't just finished surrendering
      {
        NPC_FaceEnemy();
        NPC_Surrender();
      }
    ...
    if ((!hasEscapePoint || reachedEscapePoint) &&
        TIMER_Done(NPC, "FindNewEscapePointDebounce")) {
      TIMER_Set(NPC, "FindNewEscapePointDebounce", 2500);

      int escapePoint = NPC_FindCombatPoint(
          NPC->currentOrigin, NPC->enemy->currentOrigin, NPC->currentOrigin,
          CP_COVER | CP_AVOID_ENEMY | CP_HAS_ROUTE, 128);
      if (escapePoint != -1) {
        NPC_JawaFleeSound();
        NPC_SetCombatPoint(escapePoint);
        NPC_SetMoveGoal(NPC, level.combatPoints[escapePoint].origin, 8, qtrue,
                        escapePoint);
      }
    }
  }
  ...
  //---------------------------------------------------------------------
  if (NPCInfo->tempBehavior == BS_FLEE && TIMER_Done(NPC, "flee") &&
      NPC->s.weapon != WP_NONE && NPC->s.weapon != WP_MELEE) {
    NPCInfo->tempBehavior = BS_DEFAULT;
  }
...
void NPC_StartFlee(gentity_t *enemy, vec3_t dangerPoint, int dangerLevel,
                   int fleeTimeMin, int fleeTimeMax) {
  if (Q3_TaskIDPending(
          NPC, TID_MOVE_NAV)) { // running somewhere that a script requires us
                                // to go, don't interrupt that!
    return;
  }
  ...
  // FIXME: if don't have a weapon, find nearest one we have a route to and run
  // for it?
  int cp = -1;
  if (dangerLevel > AEL_DANGER || NPC->s.weapon == WP_NONE ||
      ((!NPCInfo->group || NPCInfo->group->numGroup <= 1) &&
       NPC->health <=
           10)) { // IF either great danger OR I have no weapon OR I'm alone and
                  // low on health, THEN try to find a combat point out of PVS
    cp = NPC_FindCombatPoint(
        NPC->currentOrigin, dangerPoint, NPC->currentOrigin,
        CP_COVER | CP_AVOID | CP_HAS_ROUTE | CP_NO_PVS, 128);
  }
  // FIXME: still happens too often...
  if (cp == -1) { // okay give up on the no PVS thing
    cp =
        NPC_FindCombatPoint(NPC->currentOrigin, dangerPoint, NPC->currentOrigin,
                            CP_COVER | CP_AVOID | CP_HAS_ROUTE, 128);
    if (cp == -1) { // okay give up on the avoid
      cp =
          NPC_FindCombatPoint(NPC->currentOrigin, dangerPoint,
                              NPC->currentOrigin, CP_COVER | CP_HAS_ROUTE, 128);
      if (cp == -1) { // okay give up on the cover
        cp = NPC_FindCombatPoint(NPC->currentOrigin, dangerPoint,
                                 NPC->currentOrigin, CP_HAS_ROUTE, 128);
      }
    }
  }
  ...
  } else { // couldn't find a place to hide
    // FIXME: re-implement the old BS_FLEE behavior of following any the
    // waypoint edge 			that leads away from the danger point.
  ...
  if (dangerLevel > AEL_DANGER // geat danger always makes people turn and run
      || NPC->s.weapon == WP_NONE // melee/unarmed guys turn and run, others
                                  // keep facing you and shooting
  ...
  TIMER_Set(
      ...
      Q_irand(
          1000,
          4000)); // how long to wait before trying to nav to a dropped weapon
  TIMER_Set(NPC, "duck", 0);
}

void G_StartFlee(gentity_t *self, gentity_t *enemy, vec3_t dangerPoint,
                 int dangerLevel, int fleeTimeMin, int fleeTimeMax) {
  if (!self->NPC) { // player
    return;
  }
  SaveNPCGlobals();
...
void NPC_BSEmplaced(void) {
  ...
  if (NPC_ClearLOS(NPC->enemy)) {
    ...
    if (hit == NPC->enemy->s.number ||
        (hitEnt && hitEnt->takedamage)) { // can hit enemy or will hit glass or
                                          // other minor breakable (or in
                                          // emplaced gun), so shoot anyway
      enemyCS = qtrue;
  ...
  /*
          else if ( gi.inPVS( NPC->enemy->currentOrigin, NPC->currentOrigin ) )
          {
                  NPCInfo->enemyLastSeenTime = level.time;
                  faceEnemy = qtrue;
                  NPC_AimAdjust( -1 );//adjust aim worse longer we cannot see
     enemy
          }
  */

  if (enemyLOS) { // FIXME: no need to face enemy if we're moving to some other
                  // goal and he's too far away to shoot?
    faceEnemy = qtrue;
  }
  if (enemyCS) {
    shoot = qtrue;
  ...
  if (shoot) { // try to shoot if it's time
    if (!(NPCInfo->scriptFlags &
          SCF_FIRE_WEAPON)) // we've already fired, no need to do it again here
    {
      WeaponThink(qtrue);
    }
  }
