...
void RE_ClearScene(void) {
  ...
  tr.refdef.rdflags &= ~(RDF_doLAGoggles |
                         RDF_doFullbright); // probably not needed since it gets
                                            // copied over in RE_RenderScene
...
void RE_AddPolyToScene(qhandle_t hShader, int numVerts,
  ...
  if (r_numpolyverts + numVerts >= MAX_POLYVERTS || r_numpolys >= MAX_POLYS) {
    ...
    */
    ri.Printf(
        PRINT_DEVELOPER, S_COLOR_YELLOW
  ...
  if (!tr.world || tr.world->numfogs == 1) {
    ...
    for (int fI = 1; fI < tr.world->numfogs; fI++) {
      ...
      if (bounds[0][0] >= fog->bounds[0][0] &&
          ...
          bounds[1][2] <= fog->bounds[1][2]) { // completely in this one
        fogIndex = fI;
        break;
      } else if ((bounds[0][0] >= fog->bounds[0][0] &&
                  bounds[0][1] >= fog->bounds[0][1] &&
                  bounds[0][2] >= fog->bounds[0][2] &&
                  bounds[0][0] <= fog->bounds[1][0] &&
                  bounds[0][1] <= fog->bounds[1][1] &&
                  bounds[0][2] <= fog->bounds[1][2]) ||
                 (bounds[1][0] >= fog->bounds[0][0] &&
                  bounds[1][1] >= fog->bounds[0][1] &&
                  bounds[1][2] >= fog->bounds[0][2] &&
                  bounds[1][0] <= fog->bounds[1][0] &&
                  bounds[1][1] <= fog->bounds[1][1] &&
                  bounds[1][2] <= fog->bounds[1][2])) { // partially in this one
        if (tr.refdef.fogIndex == fI ||
            R_FogParmsMatch(tr.refdef.fogIndex,
                            fI)) { // take new one only if it's the same one
                                   // that the viewpoint is in
          fogIndex = fI;
          break;
        } else if (!fogIndex) { // didn't find one yet, so use this one
          fogIndex = fI;
        }
      }
    }
...
void RE_AddRefEntityToScene(const refEntity_t *ent) {
  ...
  if (r_numentities >= MAX_REFENTITIES) {
    ...
    ri.Printf(PRINT_WARNING,
              "WARNING: RE_AddRefEntityToScene: too many entities\n");
#endif
    return;
  }
...
void RE_RenderScene(const refdef_t *fd) {
  ...
  // Ignore my last there. This actually breaks the rest of the game as well,
  // causing massive issues. We need to figure out what's going on in
  // fd->rdflags first :S .. I'm half-tempted to just revert back to JK2 and use
  // a qbool for it --eez
