...
void BG_IK_MoveLimb(CGhoul2Info_v &ghoul2, int boltIndex, char *animBone, char *firstBone, char *secondBone, int time, entityState_t *ent, int animFileIndex,
					int basePose, vec3_t desiredPos, qboolean *ikInProgress, vec3_t origin, vec3_t angles, vec3_t scale, int blendTime, qboolean forceHalt) {
			...
			if (gi.G2API_SetBoneIKState(ghoul2, time, "upper_lumbar", IKS_DYNAMIC, &ikP)) {
				...
				if (gi.G2API_SetBoneIKState(ghoul2, time, "thoracic", IKS_DYNAMIC, &ikP)) {
					...
					if (gi.G2API_SetBoneIKState(ghoul2, time, secondBone, IKS_DYNAMIC, &ikP)) {
						...
						if (gi.G2API_SetBoneIKState(ghoul2, time, firstBone, IKS_DYNAMIC, &ikP)) { // everything went alright.
							*ikInProgress = qtrue;
						}
					}
				}
...
void PM_IKUpdate(gentity_t *ent) {
	...
	if (ent->client->ps.heldByClient <= ENTITYNUM_WORLD) { // then put our arm in this client's hand
		...
		if (ent->client->ps.heldByBolt) {
			...
			gi.G2API_GetBoltMatrix(holder->ghoul2, 0, ent->client->ps.heldByBolt, &boltMatrix, tAngles, holder->client->ps.origin, level.time, 0,
								   holder->s.modelScale);
			...
			if (grabbedByBolt) {
				// point the limb
				BG_IK_MoveLimb(ent->ghoul2, grabbedByBolt, animBone, firstBone, secondBone, level.time, &ent->s, ent->client->clientInfo.animFileIndex,
							   ent->client->ps.torsoAnim /*BOTH_DEAD1*/, boltOrg, &ent->client->ps.ikStatus, ent->client->ps.origin, ent->client->ps.viewangles,
							   ent->s.modelScale, 500, qfalse);
				...
				if (VectorLength(grabDiff) > 128.0f) { // too far, release me
					...
					gi.trace(&trace, ent->currentOrigin, ent->mins, ent->maxs, destOrg, ent->s.number, (ent->clipmask & ~holder->contents), (EG2_Collision)0,
							 0);
	...
	} else if (ent->client->ps.ikStatus) { // make sure we aren't IKing if we don't have anyone to hold onto us.
		...
		if (ent->client->ps.heldByBolt) {
			BG_IK_MoveLimb(ent->ghoul2, ent->client->ps.heldByBolt, animBone, firstBone, secondBone, level.time, &ent->s, ent->client->clientInfo.animFileIndex,
						   ent->client->ps.torsoAnim /*BOTH_DEAD1*/, (float *)vec3_origin, &ent->client->ps.ikStatus, ent->client->ps.origin,
						   ent->client->ps.viewangles, ent->s.modelScale, 500, qtrue);
		}
	}
}

void BG_G2SetBoneAngles(centity_t *cent, gentity_t *gent, int boneIndex, const vec3_t angles, const int flags, const Eorientations up,
						const Eorientations right, const Eorientations forward, qhandle_t *modelList) {
	if (boneIndex != -1) {
...
qboolean PM_LockAngles(gentity_t *ent, usercmd_t *ucmd) {
	if (ent->client->ps.viewEntity <= 0 || ent->client->ps.viewEntity >= ENTITYNUM_WORLD) { // don't clamp angles when looking through a viewEntity
...
qboolean
PM_AdjustAnglesToGripper(gentity_t *ent,
						 usercmd_t *ucmd) { // FIXME: make this more generic and have it actually *tell* the client what cmd angles it should be locked at?
...
qboolean
PM_AdjustAnglesToPuller(gentity_t *ent, gentity_t *puller, usercmd_t *ucmd,
						qboolean faceAway) { // FIXME: make this more generic and have it actually *tell* the client what cmd angles it should be locked at?
	vec3_t dir, angles;
...
qboolean PM_AdjustAngleForWallRun(gentity_t *ent, usercmd_t *ucmd, qboolean doMove) {
	if ((ent->client->ps.legsAnim == BOTH_WALL_RUN_RIGHT || ent->client->ps.legsAnim == BOTH_WALL_RUN_LEFT) &&
		ent->client->ps.legsAnimTimer > 500) { // wall-running and not at end of anim
		// stick to wall, if there is one
		vec3_t fwd, rt, traceTo, mins = {ent->mins[0], ent->mins[1], 0}, maxs = {ent->maxs[0], ent->maxs[1], 24},
		...
		{																							   // still a vertical wall there
			...
			if ((ent->s.number && !G_ControlledByPlayer(ent)) || !player_locked) {
				if (doMove) {
					...
					if (ent->client->ps.legsAnimTimer > 500) { // not at end of anim yet, pushing forward
						// FIXME: or MA?
						float speed = 175;
...
int G_MinGetUpTime(gentity_t *ent) {
	if (ent && ent->client &&
		(ent->client->ps.legsAnim == BOTH_PLAYER_PA_3_FLY || ent->client->ps.legsAnim == BOTH_LK_DL_ST_T_SB_1_L ||
		 ent->client->ps.legsAnim == BOTH_RELEASED)) { // special cases
		return 200;
	} else if (ent && ent->client && ent->client->NPC_class == CLASS_ALORA) { // alora springs up very quickly from knockdowns!
		return 1000;
	} else if ((ent->s.clientNum < MAX_CLIENTS || G_ControlledByPlayer(ent))) { // player can get up faster based on his/her force jump skill
...
qboolean PM_AdjustAnglesForKnockdown(gentity_t *ent, usercmd_t *ucmd, qboolean angleClampOnly) {
	if (PM_InKnockDown(&ent->client->ps)) { // being knocked down or getting up, can't do anything!
		if (!angleClampOnly) {
			if (ent->client->ps.legsAnimTimer > G_MinGetUpTime(ent) || (ent->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(ent))) { // can't get up yet
				ucmd->forwardmove = 0;
				ucmd->rightmove = 0;
			}
			if (ent->NPC) {
			...
			if (ent->health > 0) { // can only attack if you've started a force-getup and are using the saber
				ucmd->buttons = 0;
			}
		}
		if (!PM_InForceGetUp(&ent->client->ps)) {													// can't turn unless in a force getup
			if (ent->client->ps.viewEntity <= 0 || ent->client->ps.viewEntity >= ENTITYNUM_WORLD) { // don't clamp angles when looking through a viewEntity
				SetClientViewAngle(ent, ent->client->ps.viewangles);
			}
			ucmd->angles[PITCH] = ANGLE2SHORT(ent->client->ps.viewangles[PITCH]) - ent->client->ps.delta_angles[PITCH];
...
qboolean PM_AdjustAngleForWallRunUp(gentity_t *ent, usercmd_t *ucmd, qboolean doMove) {
	if (ent->client->ps.legsAnim == BOTH_FORCEWALLRUNFLIP_START) { // wall-running up
		// stick to wall, if there is one
		vec3_t fwd, traceTo, mins = {ent->mins[0], ent->mins[1], 0}, maxs = {ent->maxs[0], ent->maxs[1], 24},
							 fwdAngles = {0, ent->client->ps.viewangles[YAW], 0};
		trace_t trace;
		float dist = 128;
		...
		if ( // ucmd->upmove <= 0 &&
			ent->client->ps.legsAnimTimer > 0 && ucmd->forwardmove > 0 && trace.fraction < 1.0f &&
			(trace.plane.normal[2] >= 0.0f && trace.plane.normal[2] <= MAX_WALL_RUN_Z_NORMAL)) { // still a vertical wall there
...
float G_ForceWallJumpStrength(void) { return (forceJumpStrength[FORCE_LEVEL_3] / 2.5f); }

qboolean PM_AdjustAngleForWallJump(gentity_t *ent, usercmd_t *ucmd, qboolean doMove) {
	if (PM_InReboundJump(ent->client->ps.legsAnim) || PM_InReboundHold(ent->client->ps.legsAnim)) { // hugging wall, getting ready to jump off
		// stick to wall, if there is one
		vec3_t checkDir, traceTo, mins = {ent->mins[0], ent->mins[1], 0}, maxs = {ent->maxs[0], ent->maxs[1], 24},
								  fwdAngles = {0, ent->client->ps.viewangles[YAW], 0};
		trace_t trace;
			...
			if (ucmd->upmove > 0) {								  // hold on until you let go manually
				if (PM_InReboundHold(ent->client->ps.legsAnim)) { // keep holding
					...
					if (ent->client->ps.legsAnimTimer <= 300) {
						...
						NPC_SetAnim(ent, SETANIM_BOTH, BOTH_FORCEWALLRELEASE_FORWARD + (ent->client->ps.legsAnim - BOTH_FORCEWALLHOLD_FORWARD),
		...
		gi.trace(&trace, ent->currentOrigin, mins, maxs, traceTo, ent->s.number, ent->clipmask, (EG2_Collision)0, 0);
		if (																													  // ucmd->upmove <= 0 &&
			ent->client->ps.legsAnimTimer > 100 && trace.fraction < 1.0f && fabs(trace.plane.normal[2]) <= MAX_WALL_GRAB_SLOPE) { // still a vertical wall there
			// FIXME: don't pull around 90 turns
			/*
			if ( (ent->s.number>=MAX_CLIENTS&&!G_ControlledByPlayer(ent)) || !player_locked )
			{
			...
			*/
			...
			return qtrue;
		} else if (doMove && (ent->client->ps.pm_flags & PMF_STUCK_TO_WALL)) { // jump off
			// push off of it!
			ent->client->ps.pm_flags &= ~PMF_STUCK_TO_WALL;
			...
			if (PM_InReboundHold(ent->client->ps.legsAnim)) { // if was in hold pose, release now
				NPC_SetAnim(ent, SETANIM_BOTH, BOTH_FORCEWALLRELEASE_FORWARD + (ent->client->ps.legsAnim - BOTH_FORCEWALLHOLD_FORWARD),
							SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
			}
...
qboolean PM_AdjustAnglesForBFKick(gentity_t *self, usercmd_t *ucmd, vec3_t fwdAngs, qboolean aimFront) {
	...
	for (e = 0; e < numListedEntities; e++) {
		...
		// dead?
		if (ent->health <= 0) {
			if (level.time - ent->s.time > 2000) { // died more than 2 seconds ago, forget him
				continue;
			}
		}
		// too far?
		...
		if (dot > bestDot ||
			((bestDot - dot < 0.25f) && distToEnt - bestDist > 8.0f)) { // more in front... OR: still relatively close to in front and significantly closer
			bestDot = dot;
			bestDist = distToEnt;
	...
	if (bestYaw != Q3_INFINITE && bestYaw != fwdAngs[YAW]) { // aim us at them
		...
		if (self->client->ps.viewEntity <= 0 || self->client->ps.viewEntity >= ENTITYNUM_WORLD) { // don't clamp angles when looking through a viewEntity
...
qboolean PM_AdjustAnglesForStabDown(gentity_t *ent, usercmd_t *ucmd) {
	if (PM_StabDownAnim(ent->client->ps.torsoAnim) && ent->client->ps.torsoAnimTimer) { // lock our angles
...
qboolean PM_AdjustAnglesForSpinProtect(gentity_t *ent, usercmd_t *ucmd) {
	if (ent->client->ps.torsoAnim == BOTH_A6_SABERPROTECT) { // in the dual spin thing
		if (ent->client->ps.torsoAnimTimer) {				 // flatten and lock our angles, pull back camera
			// FIXME: lerp this
...
qboolean G_OkayToLean(playerState_t *ps, usercmd_t *cmd, qboolean interruptOkay) {
	if ((ps->clientNum < MAX_CLIENTS || G_ControlledByPlayer(&g_entities[ps->clientNum])) // player
		...
		!(cmd->buttons &
		  (BUTTON_ATTACK | BUTTON_ALT_ATTACK | BUTTON_FORCE_LIGHTNING | BUTTON_USE_FORCE | BUTTON_FORCE_DRAIN | BUTTON_FORCEGRIP)) // not trying to attack
...
void PM_UpdateViewAngles(playerState_t *ps, usercmd_t *cmd, gentity_t *gent) {
	...
	if (gent && gent->client && gent->client->NPC_class == CLASS_VEHICLE && gent->m_pVehicle) {
		...
		// If we're riding a vehicle...
			...
			if (pVeh->m_pVehicleInfo->type != VH_ANIMAL) { // animals just turn normally, no clamping
		...
		if ((cmd->buttons & BUTTON_USE)) {																			 // check leaning
			if (cg.renderingThirdPerson) {																			 // third person lean
				if (G_OkayToLean(ps, cmd, qtrue) && (cmd->rightmove || (cmd->forwardmove && g_debugMelee->integer))) // pushing a direction
				{
					...
					if (anim != -1) {
						int extraHoldTime = 0;
						if (PM_DodgeAnim(ps->torsoAnim) && !PM_DodgeHoldAnim(ps->torsoAnim)) { // already in a dodge
			...
			} else if ((!cg.renderingThirdPerson || cg.zoomMode) && cmd->rightmove != 0 && !cmd->forwardmove &&
					   cmd->upmove <= 0) { // Only lean if holding use button, strafing and not moving forward or back and not jumping
				int leanofs = 0;
				vec3_t viewangles;
