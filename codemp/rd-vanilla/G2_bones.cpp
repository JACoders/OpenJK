...
qboolean G2_Set_Bone_Angles_Index(boneInfo_v &blist, const int index,
                                  const float *angles, const int flags,
                                  const Eorientations yaw,
                                  const Eorientations pitch,
                                  const Eorientations roll,
                                  qhandle_t *modelList, const int modelIndex,
  ...
  if (flags & (BONE_ANGLES_PREMULT | BONE_ANGLES_POSTMULT)) {
    // you CANNOT call this with an index with these kinds of bone overrides -
    // we need the model details for these kinds of bone angle overrides
    assert(0);
    return qfalse;
  }
  ...
  Com_OPrintf("PCJ %2d %6d   (%6.2f,%6.2f,%6.2f) %d %d %d %d\n", index,
              currentTime, angles[0], angles[1], angles[2], yaw, pitch, roll,
              flags);
#endif
...
qboolean G2_Set_Bone_Angles(CGhoul2Info *ghlInfo, boneInfo_v &blist,
                            ...
                            const int modelIndex, const int blendTime,
  ...
  if (index != -1) {
    ...
    Com_OPrintf("%2d %6d   (%6.2f,%6.2f,%6.2f) %d %d %d %d\n", index,
                currentTime, angles[0], angles[1], angles[2], up, left, forward,
                flags);
#endif
  ...
  //	assert(0);
  // Jeese, we don't need an assert here too. There's already a warning in
  // G2_Add_Bone if it fails.
...
qboolean G2_Set_Bone_Angles_Matrix_Index(boneInfo_v &blist, const int index,
                                         ...
                                         const int flags, qhandle_t *modelList,
...
// Given a model handle, and a bone name, we want to set angles specifically for
// overriding - using a matrix directly
qboolean G2_Set_Bone_Angles_Matrix(const char *fileName, boneInfo_v &blist,
                                   const char *boneName,
...
qboolean G2_Set_Bone_Anim_Index(boneInfo_v &blist, const int index,
                                ...
                                const int blendTime, const int numFrames) {
  int modFlags = flags;
  ...
  if (setFrame != -1) {
    blist[index].lastTime = blist[index].startTime =
...
void G2_TimingModel(boneInfo_t &bone, int currentTime, int numFramesInFile,
                    int &currentFrame, int &newFrame, float &lerp);
...
qboolean G2_Get_Bone_Anim_Index(boneInfo_v &blist, const int index,
                                ...
                                float *retAnimSpeed, qhandle_t *modelList,
                                int numFrames) {
  ...
  if ((index >= 0) &&
    ...
    if (blist[index].flags & (BONE_ANIM_OVERRIDE_LOOP | BONE_ANIM_OVERRIDE)) {
      ...
      float lerp;
      G2_TimingModel(blist[index], currentTime, numFrames, lcurrentFrame,
                     newFrame, lerp);
...
qboolean G2_Get_Bone_Anim(CGhoul2Info *ghlInfo, boneInfo_v &blist,
                          ...
                          int *flags, float *retAnimSpeed, qhandle_t *modelList,
                          int modelIndex) {
  model_t *mod_a = (model_t *)ghlInfo->animModel;
...
qboolean G2_Pause_Bone_Anim(CGhoul2Info *ghlInfo, boneInfo_v &blist,
  ...
  if (index != -1) {
    ...
    if (blist[index].pauseTime) {
      ...
      G2_Get_Bone_Anim(ghlInfo, blist, boneName, blist[index].pauseTime,
                       &currentFrame, &startFrame, &endFrame, &flags,
                       &animSpeed, NULL, 0);
      // reset start time so we are actually on this frame right now
      G2_Set_Bone_Anim(ghlInfo, blist, boneName, startFrame, endFrame, flags,
...
qboolean G2_IsPaused(const char *fileName, boneInfo_v &blist,
                     const char *boneName) {
...
// given a model, bonelist and bonename, lets stop an anim if it's playing.
qboolean G2_Stop_Bone_Anim(const char *fileName, boneInfo_v &blist,
                           const char *boneName) {
  model_t *mod_m = R_GetModelByHandle(RE_RegisterModel(fileName));
  model_t *mod_a = R_GetModelByHandle(mod_m->mdxm->animIndex);
  int index = G2_Find_Bone(mod_a, blist, boneName);
...
// given a model, bonelist and bonename, lets stop an anim if it's playing.
qboolean G2_Stop_Bone_Angles(const char *fileName, boneInfo_v &blist,
                             const char *boneName) {
  model_t *mod_m = R_GetModelByHandle(RE_RegisterModel(fileName));
  model_t *mod_a = R_GetModelByHandle(mod_m->mdxm->animIndex);
  int index = G2_Find_Bone(mod_a, blist, boneName);
...
// actually walk the bone list and update each and every bone if we have ended
        ...
        if (animSize) {
          ...
          if (((animSpeed > 0.0f) && (newFrame_g > endFrame - 1)) ||
            ...
            if (blist[i].flags & BONE_ANIM_OVERRIDE_LOOP) {
              // get our new animation frame back within the bounds of the
              // animation set
              if (animSpeed < 0.0f) {
                if (newFrame_g <= endFrame + 1) {
                  newFrame_g = endFrame +
                               fmod(newFrame_g - endFrame, animSize) - animSize;
                }
              } else {
              ...
              blist[i].startTime =
                  currentTime - (int)((frameTime / animSpeed) * 50.0f);
              ...
              if ((blist[i].flags & BONE_ANIM_OVERRIDE_FREEZE) !=
...
static std::vector<boneInfo_t *>
    rag; // once we get the dependents precomputed this can be local
...
void G2_ResetRagDoll(CGhoul2Info_v &ghoul2V) {
  ...
  // probably aren't directly reversible, so just reset it all.
...
if (index>=0)
{
  ...
  G2_Set_Bone_Angles_Rag(ghoul2, mod_a, blist, "lower_lumbar", pcjflags, 10.0f,
                         pcjMin, pcjMax, 500);
  G2_Set_Bone_Angles_Rag(ghoul2, mod_a, blist, "upper_lumbar", pcjflags, 10.0f,
                         pcjMin, pcjMax, 500);
  VectorSet(pcjMin, -45.0f, -45.0f, -45.0f);
  VectorSet(pcjMax, 45.0f, 45.0f, 45.0f);
  G2_Set_Bone_Angles_Rag(ghoul2, mod_a, blist, "thoracic", pcjflags, 10.0f,
                         pcjMin, pcjMax, 500);

  VectorSet(pcjMin, -10.0f, -10.0f, -90.0f);
  VectorSet(pcjMax, 10.0f, 10.0f, 90.0f);
  G2_Set_Bone_Angles_Rag(ghoul2, mod_a, blist, "cranium",
                         pcjflags | RAG_BONE_LIGHTWEIGHT, 6.0f, pcjMin, pcjMax,
                         500);
  ...
  G2_Set_Bone_Angles_Rag(ghoul2, mod_a, blist, "rhumerus",
                         pcjflags | RAG_BONE_LIGHTWEIGHT, 4.0f, pcjMin, pcjMax,
                         500);
  ...
  G2_Set_Bone_Angles_Rag(ghoul2, mod_a, blist, "rradius",
                         pcjflags | RAG_BONE_LIGHTWEIGHT, 3.0f, pcjMin, pcjMax,
                         500);
  VectorSet(pcjMin, -120.0f, -20.0f, -20.0f);
  ...
  G2_Set_Bone_Angles_Rag(ghoul2, mod_a, blist, "rtibia",
                         pcjflags | RAG_EFFECTOR | RAG_BONE_LIGHTWEIGHT, 4.0f,
                         pcjMin, pcjMax, 500);
  VectorSet(pcjMin, 20.0f, -15.0f, -15.0f);
  VectorSet(pcjMax, 120.0f, 15.0f, 15.0f);
  G2_Set_Bone_Angles_Rag(ghoul2, mod_a, blist, "ltibia",
  ...
  //	G2_Set_Bone_Angles_Rag(ghoul2,
  //mod_a,blist,"rtibia",RAG_EFFECTOR|RAG_BONE_LIGHTWEIGHT,(4.0f*sRadELeg)*fRadScale);
  //	G2_Set_Bone_Angles_Rag(ghoul2,
  //mod_a,blist,"ltibia",RAG_EFFECTOR|RAG_BONE_LIGHTWEIGHT,(4.0f*sRadELeg)*fRadScale);
    ...
    if (bone.boneNumber >= 0) {
      ...
      if ((bone.flags & BONE_ANGLES_RAGDOLL) || (bone.flags & BONE_ANGLES_IK)) {
        ...
        if (anyRendered && (bone.RagFlags & RAG_WAS_EVER_RENDERED) &&
          ...
          //					bone.flags &=
          //~BONE_ANGLES_RAGDOLL; 					bone.RagFlags = 0; Com_OPrintf("Deleted
          // Effector %d\n",i); 					continue;
        }
...
static void G2_RagDoll(CGhoul2Info_v &ghoul2V, int g2Index,
  ...
  if (ragOriginChangeDir[2] < -100.0f) {
    // kicked=true;
    // iters*=8;
    iters *= 2; // rww - changed to this.. it was getting up to around 600
                // traces at times before (which is insane)
  }
  if (iters) {
...
#endif
  if (ri.CGVMLoaded()) {
    ragCallbackTraceLine_t *callData =
        (ragCallbackTraceLine_t *)ri.GetSharedMemory();
    ...
    *results = callData->tr;
    ...
    // SV_Trace(results, start, mins, maxs, end, passEntityNum, contentmask,
    // eG2TraceType, useLod);
    ri.CM_BoxTrace(results, start, end, mins, maxs, 0, contentmask, 0);
    results->entityNum =
        results->fraction != 1.0 ? ENTITYNUM_WORLD : ENTITYNUM_NONE;
  }
...
static inline bool G2_ApplyRealBonePhysics(
    boneInfo_t &bone, SRagEffector &e, CRagDollUpdateParams *params,
  ...
  if (gravity) { // factor it in before we do anything.
    ...
    Rag_Trace(&tr, e.currentOrigin, testMins, testMaxs, ground, params->me,
    ...
    boneOnGround =
        G2_BoneOnGround(e.currentOrigin, testMins, testMaxs, params->me);
  }
  ...
  Rag_Trace(&tr, e.currentOrigin, testMins, testMaxs, projectedOrigin,
            params->me, RAG_MASK, G2_NOCOLLIDE, 0);
...
static inline void G2_RagDebugBox(vec3_t mins, vec3_t maxs, int duration) {
  ...
  ragCallbackDebugBox_t *callData =
      (ragCallbackDebugBox_t *)ri.GetSharedMemory();
...
static inline void G2_RagDebugLine(vec3_t start, vec3_t end, int time,
  ...
  ragCallbackDebugLine_t *callData =
      (ragCallbackDebugLine_t *)ri.GetSharedMemory();
      ...
      if (tr.startsolid) {
        ...
        if (e.currentOrigin[2] > groundSpot[2]) {
          if (params) {
            ...
            if (ri.CGVMLoaded()) { // make a callback and see if the cgame wants
                                   // to help us out
              ragCallbackBoneInSolid_t *callData =
                  (ragCallbackBoneInSolid_t *)ri.GetSharedMemory();
        ...
        if (horzontalTraceFraction < 0.9f) {
          ...
          // gotta run the trace backwards to get a plane
          {
            ...
            if (!tr.startsolid && tr.fraction < 0.7f) {
              ...
              // args.useTracePlane=true;
              // params->EffectorCollision(args);
              if (ri.CGVMLoaded()) { // make a callback and see if the cgame
...
void G2_GetRagBoneMatrixLow(CGhoul2Info &ghoul2, int boneNum,
                            mdxaBone_t &retMatrix);
void G2_GetBoltMatrixLow(CGhoul2Info &ghoul2, int boltNum, const vec3_t scale,
                         mdxaBone_t &retMatrix);
  ...
  for (i = 0; i < numRags; i++) {
    ...
    if (bone.boneNumber) {
      ...
      if (bone.parentBoneIndex == -1) {
        ...
        while (bParentIndex > 0) { // go upward through hierarchy searching for
          ...
          bParentIndex = skel->parent;
          bParentListIndex =
              G2_Find_Bone(ghoul2V[0].animModel, ghoul2V[0].mBlist, skel->name);
    ...
    if (broadsword_ragtobase && broadsword_ragtobase->integer) {
      ...
      if (broadsword_ragtobase->integer > 1) {
        ...
        if (d > 16.0f || d < -16.0f) { // don't update unless x degrees away
                                       // from the ideal to avoid moving goal
                                       // spots too much if pelvis rotates
    ...
    if (tr.startsolid || tr.allsolid ||
      ...
      if (!G2_ApplyRealBonePhysics(
              bone, e, params, goalSpot, testMins, testMaxs, gravity, mass,
...
static inline void G2_BoneSnap(CGhoul2Info_v &ghoul2V, boneInfo_t &bone,
  ...
  ragCallbackBoneSnap_t *callData =
      (ragCallbackBoneSnap_t *)ri.GetSharedMemory();

  callData->entNum = params->me;
  strcpy(callData->boneName,
         G2_Get_Bone_Name(&ghoul2V[0], ghoul2V[0].mBlist, bone.boneNumber));
...
#if 1 // constraint breaks?
        if (limitAngles && (allSolidCount < 32 ||
                            (bone.RagFlags &
                             RAG_UNSNAPPABLE))) // 32 tries and still in solid?
...
qboolean G2_SetBoneIKState(CGhoul2Info_v &ghoul2, int time,
  ...
  if (bone.flags &
      BONE_ANGLES_RAGDOLL) { // otherwise if the bone is already flagged as rag,
                             // then we can't set it again. (non-active ik bones
                             // will be BONE_ANGLES_IK, active are considered
                             // rag)
    return qfalse;
  }
  ...
  if (bone.startFrame != startFrame || bone.endFrame != endFrame ||
      params->forceAnimOnBone) { // if it's already on this anim leave it alone,
                                 // to allow smooth transitions into IK on the
                                 // current anim if it is so desired.
    G2_Set_Bone_Anim_No_BS(g2, rmod_a, blist, boneName, startFrame,
                           endFrame - 1,
                           BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, 1.0f,
                           curTime, float(startFrame), 150, 0, true);
  }
...
void G2_RemoveRedundantBoneOverrides(boneInfo_v &blist, int *activeBones) {
  // walk the surface list, removing surface overrides or generated surfaces
  // that are pointing at surfaces that aren't active anymore
  for (size_t i = 0; i < blist.size(); i++) {
    if (blist[i].boneNumber != -1) {
