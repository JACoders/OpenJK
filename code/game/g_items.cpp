...
int Add_Ammo2(gentity_t *ent, int ammoType, int count) {
	...
	if (ammoType != AMMO_FORCE) {
		...
		if (ent->client->ps.forcePower >= ammoData[ammoType].max) { // if have full force, just get 25 extra per crystal
			...
			if (ent->client->ps.forcePower >= ammoData[ammoType].max + 25) { // cap at max + 25
				ent->client->ps.forcePower = ammoData[ammoType].max + 25;
			}
		}

		if (ent->client->ps.forcePower >= ammoData[ammoType].max * 2) { // always cap at twice a full charge
...
gentity_t *G_DropSaberItem(const char *saberType, saber_colors_t saberColor, vec3_t saberPos, vec3_t saberVel, vec3_t saberAngles,
...
qboolean Pickup_Saber(gentity_t *self, qboolean hadSaber, gentity_t *pickUpSaber) {
	...
	if (Q_stricmp("player", pickUpSaber->NPC_type) == 0) { //"player" means use cvar info
		...
		if (self->client->ps.weapon == WP_SABER &&
			self->client->ps.weaponTime > 0) { // can't pick up a new saber while the old one is busy (also helps to work as a debouncer so you don't swap out
											   // sabers rapidly when touching more than one at a time)
			return qfalse;
		}
		...
		if (WP_SaberParseParms(pickUpSaber->NPC_type, &newSaber)) { // successfully found a saber .sab entry to use
			int saberNum = 0;
			...
			} else if (!hadSaber) { // don't have a saber at all yet, put it in our right hand
				saberNum = 0;
				// just in case...
				removeLeftSaber = qtrue;
			...
			{ // replace the old right-hand saber and remove the left hand one
				saberNum = 0;
				removeLeftSaber = qtrue;
			} else {							   // have, at least, a saber in our right hand and the new one could go in either left or right hand
			...
			if (saberNum == 0) {									  // want to reach out with right hand
				if (self->client->ps.torsoAnim == BOTH_BUTTON_HOLD) { // but only if already playing the pickup with left hand anim...
				...
				if (swapSabers) { // drop first one where the one we're picking up is
					G_DropSaberItem(self->client->ps.saber[saberNum].name, self->client->ps.saber[saberNum].blade[0].color, pickUpSaber->currentOrigin,
									(float *)vec3_origin, pickUpSaber->currentAngles, pickUpSaber);
					if (removeLeftSaber) { // drop other one at my origin
						G_DropSaberItem(self->client->ps.saber[1].name, self->client->ps.saber[1].blade[0].color, self->currentOrigin, (float *)vec3_origin,
			...
			if (self->client->ps.torsoAnim == BOTH_BUTTON_HOLD ||
				self->client->ps.torsoAnim == BOTH_SABERPULL) { // don't let them attack right away, force them to finish the anim
...
int Pickup_Weapon(gentity_t *ent, gentity_t *other) {
	...
	*/
	...
	if (ent->item->giTag == WP_SABER && (!hadWeapon || ent->NPC_type != NULL)) { // didn't have a saber or it is specifying a certain kind of saber to use
	...
	if (other->s.number) {												  // NPC
		if (other->s.weapon == WP_NONE || ent->item->giTag == WP_SABER) { // NPC with no weapon picked up a weapon, change to this weapon
	...
	if (ent->item->giTag == WP_SABER) {	   // picked up a saber
		if (other->s.weapon != WP_SABER) { // player picking up saber
			...
			other->client->ps.weaponstate = WEAPON_RAISING;
			if (other->s.number < MAX_CLIENTS) { // make sure the cgame-side knows this
...
void Touch_Item(gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	if (ent->noDamageTeam != TEAM_FREE && other->client->playerTeam != ent->noDamageTeam) { // only one team can pick it up
		return;
	}
	...
	if (CheckItemCanBePickedUpByNPC(ent, other)) {
		if (other->NPC && other->NPC->goalEntity && other->NPC->goalEntity == ent) { // they were running to pick me up, they did, so clear goal
	...
	if (other->client) {
		if ((other->client->ps.eFlags & EF_FORCE_GRIPPED) || (other->client->ps.eFlags & EF_FORCE_DRAINED)) { // can't pick up anything while being gripped
			return;
		}
		if (PM_InKnockDown(&other->client->ps) && !PM_InGetUp(&other->client->ps)) { // can't pick up while in a knockdown
			return;
		}
	}
	...
	if (ent->item->giType == IT_WEAPON && ent->item->giTag == WP_SABER) { // a saber
		if (ent->delay > level.time) {									  // just picked it up, don't pick up again right away
			return;
		}
	}

	if (other->s.number < MAX_CLIENTS && (ent->spawnflags & ITMSF_USEPICKUP)) { // only if player is holing use button
	...
	case IT_WEAPON:
		if (other->NPC && other->s.weapon == WP_NONE) { // Make them duck and sit here for a few seconds
	...
	if (!other->s.number && g_timescale->value < 1.0f) { // SIGH... with timescale on, you lose events left and right
	...
	if (ent->item->giType == IT_WEAPON && ent->item->giTag == WP_SABER) { // a saber that was picked up
...
gentity_t *LaunchItem(gitem_t *item, const vec3_t origin, const vec3_t velocity, char *target) {
	...
	if ((!dropped->mins[0] && !dropped->mins[1] && !dropped->mins[2]) && (!dropped->maxs[0] && !dropped->maxs[1] && !dropped->maxs[2])) {
...
void Use_Item(gentity_t *ent, gentity_t *other, gentity_t *activator) {
	if ((ent->svFlags & SVF_PLAYER_USABLE) && other && !other->s.number) { // used directly by the player, pick me up
		if ((ent->spawnflags & ITMSF_USEPICKUP)) {						   // player has to be touching me and hit use to pick it up, so don't allow this
			if (!G_BoundsOverlap(ent->absmin, ent->absmax, other->absmin, other->absmax)) { // not touching
				return;
			}
		}
...
void FinishSpawningItem(gentity_t *ent) {
	...
	if ((!ent->mins[0] && !ent->mins[1] && !ent->mins[2]) && (!ent->maxs[0] && !ent->maxs[1] && !ent->maxs[2])) {
	...
	if (ent->item->giType == IT_WEAPON && ent->item->giTag == WP_SABER && ent->NPC_type && ent->NPC_type[0]) {
		saberInfo_t itemSaber;
		if (Q_stricmp("player", ent->NPC_type) == 0 && g_saber->string && g_saber->string[0] && Q_stricmp("none", g_saber->string) &&
...
void G_SpawnItem(gentity_t *ent, gitem_t *item) {
	...
	if (ent->item && ent->item->giType == IT_WEAPON && ent->item->giTag == WP_SABER) { // weapon_saber item
...
void G_BounceItem(gentity_t *ent, trace_t *trace) {
	...
	if (ent->item && ent->item->giType == IT_WEAPON && ent->item->giTag == WP_SABER && (ent->flags & FL_DROPPED_ITEM)) {
		droppedSaber = qtrue;
	}
...
void G_RunItem(gentity_t *ent) {
	...
	if (ent->s.pos.trType == TR_STATIONARY) {
		...
		if (!g_gravity->value) {
		...
		} else if ((ent->flags & FL_DROPPED_ITEM) && ent->item && ent->item->giType == IT_WEAPON &&
				   ent->item->giTag == WP_SABER) { // a dropped saber item, check below, just in case
			...
			if (!tr.allsolid && !tr.startsolid && tr.fraction > 0.001f) { // wha?  fall....
				ent->s.pos.trType = TR_GRAVITY;
				ent->s.pos.trTime = level.time;
			}
		}
