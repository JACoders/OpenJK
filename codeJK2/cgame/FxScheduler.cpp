...
//------------------------------------------------------
int CFxScheduler::RegisterEffect(const char *path,
  ...
  // FIXME: this could maybe be a cstring_view, if mEffectIDs were to use a
  // transparent comparator, but those were only added in C++14, which we don't
  // support yet (sigh)
  char filenameNoExt[MAX_QPATH];

  // Get an extension stripped version of the file
  if (bHasCorrectPath) {
    // FIXME: this is basically COM_SkipPath, except it also accepts '\\'
    // instead of '/'
  ...
  if (bHasCorrectPath) {
    ...
    Com_sprintf(correctFilenameBuffer, sizeof(correctFilenameBuffer),
                "%s/%s.efx", FX_FILE_PATH, filenameNoExt);
...
int CFxScheduler::ParseEffect(const char *file, const CGPGroup &base) {
  ...
  for (const auto &primitiveGroup : base.GetSubGroups()) {
    static std::map<gsl::cstring_view, EPrimType, Q::CStringViewILess>
        primitiveTypes{{CSTRING_VIEW("particle"), Particle},
...
SEffectTemplate *CFxScheduler::GetEffectCopy(int fxHandle, int *newHandle) {
  if (fxHandle < 1 || fxHandle >= FX_MAX_EFFECTS ||
      !mEffectTemplates[fxHandle].mInUse) {
...
//	none
//------------------------------------------------------
void CFxScheduler::PlayEffect(const char *file, vec3_t origin, vec3_t axis[3],
                              const int boltInfo, const int entNum) {
  char sfile[MAX_QPATH];

  // Get an extenstion stripped version of the file
  COM_StripExtension(file, sfile, sizeof(sfile));

  // This is a horribly dumb thing to have to do, but QuakeIII might not have
  // calc'd the lerpOrigin
  //	for the entity we may be trying to bolt onto.  We like having the
  //correct origin, so we are 	forced to call this function....
  if (entNum != -1) {
...
//	none
//------------------------------------------------------
void CFxScheduler::CreateEffect(CPrimitiveTemplate *fx, int clientID,
                                int delay) {
  vec3_t sRGB, eRGB;
  vec3_t vel, accel;
  vec3_t org;
  ...
  if (fx->mSpawnFlags & FX_RGB_COMPONENT_INTERP) {
    ...
    VectorSet(sRGB, fx->mRedStart.GetVal(perc), fx->mGreenStart.GetVal(perc),
              fx->mBlueStart.GetVal(perc));
    VectorSet(eRGB, fx->mRedEnd.GetVal(perc), fx->mGreenEnd.GetVal(perc),
              fx->mBlueEnd.GetVal(perc));
  } else {
    VectorSet(sRGB, fx->mRedStart.GetVal(), fx->mGreenStart.GetVal(),
              fx->mBlueStart.GetVal());
    VectorSet(eRGB, fx->mRedEnd.GetVal(), fx->mGreenEnd.GetVal(),
              fx->mBlueEnd.GetVal());
  }
  ...
  // If depth hack ISN'T already on, then turn it on.  Otherwise, we treat a
  // pre-existing depth_hack flag as NOT being depth_hack.
  //	This is done because muzzle flash fx files are shared amongst all
  //shooters, but for the player we need to do depth hack in first person....
  ...
  case Particle:
    ...
    FX_AddParticle(clientID, org, vel, accel, fx->mGravity.GetVal(),
                   fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(),
                   ...
                   fx->mRotationDelta.GetVal(), fx->mLife.GetVal(),
                   fx->mMediaHandles.GetHandle(), flags);
    break;
  ...
  case Line:
    ...
    FX_AddLine(clientID, org, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(),
               fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(),
               fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB, eRGB,
               fx->mRGBParm.GetVal(), fx->mLife.GetVal(),
               fx->mMediaHandles.GetHandle(), fx->mImpactFxHandles.GetHandle(),
               flags);
    break;
  ...
  case Tail:
    ...
    FX_AddTail(clientID, org, vel, accel, fx->mSizeStart.GetVal(),
               ...
               fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB, eRGB,
               fx->mRGBParm.GetVal(), fx->mMin, fx->mMax,
  ...
  case Sound:
    ...
    theFxHelper.PlaySound(NULL, clientID, CHAN_WEAPON,
  ...
  case Light:
    ...
    if (clientID >= 0 && clientID < ENTITYNUM_WORLD) {
      ...
      if (cent && cent->gent && cent->gent->client) {
        FX_AddLight(cent->gent->client->renderInfo.muzzlePoint,
                    fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(),
  ...
  case CameraShake:
    ...
    if (clientID >= 0 && clientID < ENTITYNUM_WORLD) {
      ...
      if (cent && cent->gent && cent->gent->client) {
        theFxHelper.CameraShake(cent->gent->currentOrigin,
                                fx->mElasticity.GetVal(), fx->mRadius.GetVal(),
                                fx->mLife.GetVal());
      }
    }
    break;
...
//	none
//------------------------------------------------------
void CFxScheduler::PlayEffect(int id, vec3_t origin, vec3_t axis[3],
                              const int boltInfo, const int entNum) {
  SEffectTemplate *fx;
  CPrimitiveTemplate *prim;
  int i = 0;
  ...
  for (i = 0; i < fx->mPrimitiveCount; i++) {
    ...
    if (prim->mCullRange) {
      if (DistanceSquared(origin, cg.refdef.vieworg) >
          prim->mCullRange) // cull range has already been squared
      {
  ...
  for (itr = mFxSchedule.begin(); itr != mFxSchedule.end(); /* do nothing */) {
    ...
    if (effect->mStartTime <= theFxHelper.mTime) {
      if (effect->mClientID >= 0) {
        ...
        // time lets try and do it only once
        if ((effect->mModelNum != oldModelNum) ||
          ...
          if (cg_entities[effect->mEntNum].gent->ghoul2.IsValid()) {
            if (effect->mModelNum >= 0 &&
                effect->mModelNum <
                    cg_entities[effect->mEntNum].gent->ghoul2.size()) {
              if (cg_entities[effect->mEntNum]
                      .gent->ghoul2[effect->mModelNum]
                      .mModelindex >= 0) {

                // go away and get me the bolt position for this frame please
                doesBoltExist = gi.G2API_GetBoltMatrix(
                    cg_entities[effect->mEntNum].gent->ghoul2,
                    effect->mModelNum, effect->mBoltNum, &boltMatrix,
                    cg_entities[effect->mEntNum].lerpAngles,
                    cg_entities[effect->mEntNum].lerpOrigin, cg.time,
                    cgs.model_draw,
                    cg_entities[effect->mEntNum].currentState.modelScale);
                // set up the axis and origin we need for the actual effect
                // spawning
...
void CFxScheduler::CreateEffect(CPrimitiveTemplate *fx, const vec3_t origin,
  ...
  if (fx->mSpawnFlags & FX_RAND_ROT_AROUND_FWD) {
    RotatePointAroundVector(ax[1], ax[0], axis[1],
                            Q_flrand(0.0f, 1.0f) * 360.0f);
    CrossProduct(ax[0], ax[1], ax[2]);
  }

  // Origin calculations
  //-------------------------------------
  if (fx->mSpawnFlags & FX_CHEAP_ORG_CALC) { // let's take the easy way out
    VectorSet(org, fx->mOrigin1X.GetVal(), fx->mOrigin1Y.GetVal(),
              fx->mOrigin1Z.GetVal());
  ...
  if (fx->mSpawnFlags & FX_ORG_ON_SPHERE) {
    ...
    VectorMA(pt, Q_flrand(-1.0f, 1.0f) * 0.5f * fx->mHeight.GetVal(), ax[0],
             pt);
  ...
  if (fx->mType == Line || fx->mType == Electricity) {
    ...
    if (fx->mSpawnFlags & FX_ORG2_FROM_TRACE) {
      ...
      theFxHelper.Trace(&tr, org, NULL, NULL, temp, -1,
                        CONTENTS_SOLID | CONTENTS_SHOTCLIP); // MASK_SHOT );
  ...
  case Particle:
    ...
    FX_AddParticle(org, vel, accel, fx->mSizeStart.GetVal(),
                   ...
                   fx->mImpactFxHandles.GetHandle(), fx->mLife.GetVal(),
                   fx->mMediaHandles.GetHandle(), fx->mFlags);
    break;
  ...
  case Electricity:
    ...
    FX_AddElectricity(org, org2, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(),
                      ...
                      fx->mLife.GetVal(), fx->mMediaHandles.GetHandle(),
                      fx->mFlags);
    break;
  ...
  case Decal:
    ...
    // I'm calling this function ( at least for now ) because it handles
    // projecting
    //	the decal mark onto the surfaces properly.  This is especially important
    //for large marks.
    // The downside is that it's much less flexible....
    CG_ImpactMark(fx->mMediaHandles.GetHandle(), org, ax[0],
                  fx->mRotation.GetVal(), sRGB[0], sRGB[1], sRGB[2],
                  fx->mAlphaStart.GetVal(), qtrue, fx->mSizeStart.GetVal(),
                  qfalse);
    break;
  ...
  case Sound:
    ...
    if (fx->mSpawnFlags & FX_SND_LESS_ATTENUATION) {
      theFxHelper.PlaySound(org, ENTITYNUM_NONE, CHAN_LESS_ATTEN,
                            fx->mMediaHandles.GetHandle());
    } else {
  ...
  case ScreenFlash:
    ...
    FX_AddFlash(org, sRGB, eRGB, fx->mRGBParm.GetVal(), fx->mLife.GetVal(),
                fx->mMediaHandles.GetHandle(), fx->mFlags);
    break;
