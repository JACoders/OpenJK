...
qboolean NAV_ClearPathToPoint(gentity_t *self, vec3_t pmins, vec3_t pmaxs, vec3_t point, int clipmask, int okToHitEntNum) {
	...
	if (self->flags & FL_NAVGOAL) {
		...
		if (trace.startsolid && (trace.contents & CONTENTS_BOTCLIP)) { // started inside do not enter, so ignore them
			...
			trap->Trace(&trace, point, mins, maxs, self->r.currentOrigin, self->parent->s.number, (clipmask | CONTENTS_MONSTERCLIP) & ~CONTENTS_BODY, qfalse, 0,
						0);
		}
		...
		if (NAV_HitNavGoal(self->r.currentOrigin, self->parent->r.mins, self->parent->r.maxs, trace.endpos, NPCS.NPCInfo->goalRadius,
	...
	if (trace.entityNum < ENTITYNUM_WORLD) {
		...
		if VALIDSTRING (blocker->classname) { // special case: doors are architecture, but are dynamic, like entitites
			...
			{ // it's unlocked, go for it
				...
				if (self->s.weapon != WP_SABER && fabs(self->r.currentOrigin[2] - end[2]) > 48) { // too far above
				} else {
					return endID;
				}
