...
// display text in a supplied box, start at top left and going down by however
// many pixels I feel like internally,
//	return value is NULL if all fitted, else char * of next char to continue
//from that didn't fit.
//
// (coords are in the usual 640x480 virtual space)...
//
// ( if you get the same char * returned as what you passed in, then none of it
// fitted at all (box too small) )
//
const char *CG_DisplayBoxedText(int iBoxX, int iBoxY, int iBoxWidth,
  ...
  // Setup a reasonable vertical spacing (taiwanese & japanese need 1.5
  // fontheight, so use that for all)...
  //
  ...
  //	psText =
  //"ÁÉ¨ô­ôº¸³Õ¤h¤w¸g¦w¥þ¤F¡A§Ú¤]§â©Ò¦³µo²{³ø§iµ¹¡u°Ó©±¡v¡C«Ü¤£©¯¦a¡A·ç¤hÄµ§½¦³¨Ç¥ÕÃ¨µo²{¤F¤@¨Çª¬ªp¡A·Ç³Æ¦b¾÷³õ¶e®·¨È¾ú¦è¡E¯Ç§J·ç±o¡C¥L°°¸Ë¦¨¥~¥æ¨Ï¸`¡A¬ï¹L¤F¼h¼h¨¾³Æ¡C²{¦b¥L´¤¦³¤H½è¡A¨Ã¥B«Â¯Ù­n´²¼½¯f¬r¡C®Ú¾Ú³Ì·psCurrentTextReadPosªº³ø§i¡A¯Ç§J·ç±o¥H¤Î¥LªºÄÒ¦Ð¤w¸g§¹¥þ¦û¾Ú¤F¾÷³õ¡C§Ú¨ü©R¨Ó°l®·¯Ç§J·ç±o¥H¤Î±Ï¥X©Ò¦³¤H½è¡C³o¨Ã¤£®e©ö¡C";
  ...
  while (*psCurrentTextReadPos &&
         (iYpos + iFontHeight < (iBoxY + iBoxHeight))) {
    ...
    while (*psCurrentTextReadPos) {
      ...
      unsigned int uiLetter = cgi_AnyLanguage_ReadCharFromString(
          &psCurrentTextReadPos, &bIsTrailingPunctuation);
      ...
      if (uiLetter == '\n') {
        ...
        if (uiLetter > 255 && bIsTrailingPunctuation &&
          ...
          if (psBestLineBreakSrcPos == psReadPosAtLineStart) {
            ...
            //happen 		normally).  On the other hand I suppose
            //it'psCurrentTextReadPos entirely possible that some taiwanese
            //string 		might have no punctuation at all, so...
            //
      ...
      if (bIsTrailingPunctuation || uiLetter == ' ' ||
          (uiLetter > 255 && !cgi_Language_UsesSpaces())) {
        psBestLineBreakSrcPos = psCurrentTextReadPos;
      }
    }
    ...
    // ... and print it...
    //
    cgi_R_Font_DrawString(iBoxX, iYpos, sLineForDisplay, v4Color, iFontHandle,
                          -1, fScale);
...
//
static int cg_SP_GetStringTextStringWithRetry(const char *psReference,
                                              char *psDest, int iSizeofDest) {
  int iReturn;

  if (psReference[0] == '#') {
    // then we know the striped package name is already built in, so do NOT try
    // prepending anything else...
    //
    return cgi_SP_GetStringTextString(va("%s", psReference + 1), psDest,
                                      iSizeofDest);
  }
  ...
  for (int i = 0; i < STRIPED_LEVELNAME_VARIATIONS; i++) {
    if (cgs.stripLevelName[i][0]) // entry present?
    {
      iReturn = cgi_SP_GetStringTextString(
          va("%s_%s", cgs.stripLevelName[i], psReference), psDest, iSizeofDest);
      if (iReturn) {
        return iReturn;
      }
    }
...
// slightly confusingly, the char arg for this function is an audio filename of
// the form "path/path/filename",
//	the "filename" part of which should be the same as the StripEd reference
//we're looking for in the current 	level's string package...
//
void CG_CaptionText(const char *str, int sound) {
...
#ifndef FINAL_BUILD
    Com_Printf(
        "WARNING: CG_CaptionText given audio filename with no '/':'%s'\n", str);
#endif
    return;
  }
  ...
  if (!i) {
    ...
    if (!Q_strncmp(str, "sound/chars/",
                   12)) // whichever language it is, it'll be pathed as english
                        // at this point
    {
      Com_Printf("WARNING: CG_CaptionText given invalid text key :'%s'\n", str);
    } else {
      // anything else is probably stuff we don't care about. It certainly
      // shouldn't be speech, anyway
    }
#endif
    return;
  }

  const int fontHeight = (int)((cgi_Language_IsAsian() ? 1.4f : 1.0f) *
                               (float)cgi_R_Font_HeightPixels(
                                   cgs.media.qhFontMedium,
                                   fFontScale)); // taiwanese & japanese
                                                 // need 1.5 fontheight spacing

  cg.captionTextTime = cg.time;
  if (in_camera) {
    cg.captionTextY =
        SCREEN_HEIGHT - (client_camera.bar_height_dest /
                         2); // ths is now a centre'd Y, not a start Y
  } else {                   // get above the hud
    cg.captionTextY =
        (int)(0.88f * ((float)SCREEN_HEIGHT -
                       (float)fontHeight *
                           1.5f)); // do NOT move this, it has to fit in between
                                   // the weapon HUD and the datapad update.
  }
  ...
  i = 0; // this could be completely removed and replace by
         // "cg.scrollTextLines-1", but wtf?
  ...
  // kor...
  //	s="Wp:¼îÅ¸ÀÓÀÌ´Ù ¸Ö¸°. ±×µéÀÌ ¸»ÇÑ´ë·Î ³×°¡ ÀßÇÒÁö ±â´ëÇÏ°Ú´Ù.¼îÅ¸ÀÓÀÌ´Ù
  //¸Ö¸°. ±×µéÀÌ ¸»ÇÑ´ë·Î ³×°¡ ÀßÇÒÁö ±â´ëÇÏ°Ú´Ù.";
  holds = s;

  int iPlayingTimeMS = cgi_S_GetSampleLength(sound);
  int iLengthInChars =
      strlen(s); // cgi_R_Font_StrLenChars(s);	// strlen is also good for MBCS
                 // in this instance, since it's for timing
  if (iLengthInChars == 0) {
    iLengthInChars = 1;
  }
  ...
  while (*s) {
    ...
    unsigned int uiLetter =
        cgi_AnyLanguage_ReadCharFromString(&s, &bIsTrailingPunctuation);
    ...
    if (uiLetter > 255) {
      Q_strcat(cg.captionText[i], sizeof(cg.captionText[i]),
               va("%c%c", uiLetter >> 8, uiLetter & 0xFF));
    } else {
      Q_strcat(cg.captionText[i], sizeof(cg.captionText[i]),
               va("%c", uiLetter & 0xFF));
    ...
    if (uiLetter == '\n') {
      ...
      cg.scrollTextLines++;
    } else if (cgi_R_Font_StrLenPixels(cg.captionText[i],
                                       cgs.media.qhFontMedium,
                                       fFontScale) >= SCREEN_WIDTH) {
      // reached screen edge, so cap off string at bytepos after last good
      // position...
      //
      if (uiLetter > 255 && bIsTrailingPunctuation &&
        ...
        if (psBestLineBreakSrcPos == holds) {
          //  aarrrggh!!!!!   we'll only get here is someone has fed in a
          //  (probably) garbage string,
          //		since it doesn't have a single space or punctuation mark
          //right the way across one line 		of the screen.  So far, this has only
          //happened in testing when I hardwired a taiwanese 		string into this
          //function while the game was running in english (which should NEVER
  ...
  // calc the length of time to hold each 2 lines of text on the screen....
  // presumably this works?
  //
  holdTime = strlen(cg.captionText[0]);
  if (cg.scrollTextLines > 1) {
    holdTime +=
        strlen(cg.captionText[1]); // strlen is also good for MBCS in this
                                   // instance, since it's for timing
  }
  ...
  if (cg_developer.integer) {
    Com_Printf("%s\n", cg.captionText[0]); // ste:  was [i], but surely sentence
                                           // 0 is more useful than last?
  }
}
...
void CG_DrawCaptionText(void) {
  ...
  if (cg.captionNextTextTime < cg.time) {
    ...
    if (cg.captionTextCurrentLine >= cg.scrollTextLines) {
      ...
      if (cg.scrollTextLines >= cg.captionTextCurrentLine) {
        // ( strlen is also good for MBCS in this instance, since it's for
        // timing -ste)
        //
        holdTime += strlen(cg.captionText[cg.captionTextCurrentLine + 1]);
      }

      cg.captionNextTextTime =
          cg.time + (holdTime * cg.captionLetterTime); // 50);
    }
  }
  ...
  const int fontHeight =
      (int)((cgi_Language_IsAsian() ? 1.4f : 1.0f) *
            (float)cgi_R_Font_HeightPixels(cgs.media.qhFontMedium, fFontScale));
  const bool bPrinting2Lines =
      !!(cg.captionText[cg.captionTextCurrentLine + 1][0]);
  y = cg.captionTextY -
      ((float)fontHeight *
       (bPrinting2Lines
            ? 1
            : 0.5f)); // captionTextY was a centered Y pos, not a top one
  y -= cgi_Language_IsAsian() ? 0 : 4;

  for (i = cg.captionTextCurrentLine; i < cg.captionTextCurrentLine + 2; ++i) {
    w = cgi_R_Font_StrLenPixels(cg.captionText[i], cgs.media.qhFontMedium,
                                fFontScale);
    if (w) {
      x = (SCREEN_WIDTH - w) / 2;
      cgi_R_Font_DrawString(x, y, cg.captionText[i], textcolor_caption,
                            cgs.media.qhFontMedium, -1, fFontScale);
      y += fontHeight;
    }
  }
...
void CG_ScrollText(const char *str, int iPixelWidth) {
  ...
  while (*s) {
    ...
    if (uiLetter > 255) {
      Q_strcat(cg.printText[i], sizeof(cg.printText[i]),
               va("%c%c", uiLetter >> 8, uiLetter & 0xFF));
    } else {
      Q_strcat(cg.printText[i], sizeof(cg.printText[i]),
               va("%c", uiLetter & 0xFF));
    ...
    if (uiLetter == '\n') {
      ...
      holds = s;
      cg.scrollTextLines++;
    } else if (cgi_R_Font_StrLenPixels(cg.printText[i], cgs.media.qhFontMedium,
                                       1.0f) >= iPixelWidth) {
      // reached screen edge, so cap off string at bytepos after last good
      // position...
      //
      if (psBestLineBreakSrcPos == holds) {
        ...
        //happened in testing when I hardwired a taiwanese 		string into this
        //function while the game was running in english (which should NEVER
        //happen 		normally).  On the other hand I suppose it's entirely possible
        //that some taiwanese string 		might have no punctuation at all, so...
        //
        psBestLineBreakSrcPos =
            psLastGood_s; // force a break after last good letter
      }
...
void CG_DrawScrollText(void) {
  ...
  const int fontHeight =
      (int)(1.5f *
            (float)cgi_R_Font_HeightPixels(
                cgs.media.qhFontMedium,
                1.0f)); // taiwanese & japanese need 1.5 fontheight spacing
  ...
  //	cgi_R_Font_DrawString(320, 200, va("Scrolltext printing @ %d",y),
  ...
  for (i = 0; i < cg.scrollTextLines; ++i) {
    ...
    //		w = cgi_R_Font_StrLenPixels(cg.printText[i],
    //cgs.media.qhFontMedium, 1.0f); 		if (w)
    {
      x = (SCREEN_WIDTH - giScrollTextPixelWidth) / 2;
      cgi_R_Font_DrawString(x, y, cg.printText[i], textcolor_scroll,
...
void CG_CenterPrint(const char *str, int y) {
  ...
  if (*str == '@') {
    ...
    i = cgi_SP_GetStringTextString(str + 1, cg.centerPrint,
...
void CG_DrawCenterString(void) {
  ...
  while (1) {
    ...
    cgi_R_Font_DrawString(x, y, linebuffer, textcolor_center,
                          cgs.media.qhFontMedium, -1, 1.0f);
