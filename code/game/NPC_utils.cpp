...
void CalcEntitySpot(const gentity_t *ent, const spot_t spot, vec3_t point) {
	...
	case SPOT_HEAD:
		if (ent->client && VectorLengthSquared(ent->client->renderInfo.eyePoint) &&
			(ent->client->ps.viewEntity <= 0 || ent->client->ps.viewEntity >= ENTITYNUM_WORLD)) { // Actual tag_head eyespot!
...
qboolean NPC_UpdateAngles(qboolean doPitch, qboolean doYaw) {
	...
	if (NPC->s.weapon == WP_EMPLACED_GUN) {
		...
		if (NPC->client->NPC_class == CLASS_ROCKETTROOPER && !NPC->enemy) { // just slowly lookin' around
			yawSpeed = 1;
		} else {
	...
	if (!NPC_IsTrooper(NPC) && NPC->enemy && !G_IsRidingVehicle(NPC) && NPC->client->NPC_class != CLASS_VEHICLE) {
		if (NPC->s.weapon == WP_BLASTER_PISTOL || NPC->s.weapon == WP_BLASTER || NPC->s.weapon == WP_BOWCASTER || NPC->s.weapon == WP_REPEATER ||
			NPC->s.weapon == WP_FLECHETTE || NPC->s.weapon == WP_BRYAR_PISTOL || NPC->s.weapon == WP_NOGHRI_STICK) {
			yawSpeed *= 10.0f;
		}
	}
...
void NPC_UpdateShootAngles(vec3_t angles, qboolean doPitch, qboolean doYaw) { // FIXME: shoot angles either not set right or not used!
	float error;
	float decay;
	float targetPitch = 0;
	float targetYaw = 0;
...
qboolean G_ValidEnemy(gentity_t *self, gentity_t *enemy) {
	...
	if (enemy->client->playerTeam == TEAM_FREE && enemy->s.number < MAX_CLIENTS) { // An evil player, everyone attacks him
		return qtrue;
	}
	...
	if (enemy->client->playerTeam == self->client->enemyTeam // simplest case: they're on my enemy team
		|| (self->client->enemyTeam == TEAM_FREE &&
			enemy->client->NPC_class != self->client->NPC_class)	  // I get mad at anyone and this guy isn't the same class as me
		...
		|| (enemy->client->playerTeam == TEAM_FREE && enemy->client->enemyTeam == TEAM_FREE && enemy->enemy && enemy->enemy->client &&
			(enemy->enemy->client->playerTeam == self->client->playerTeam ||
			 (enemy->enemy->client->playerTeam != TEAM_ENEMY &&
			  self->client->playerTeam == TEAM_PLAYER))) // enemy is a rampaging non-aligned creature who is attacking someone on our team or a non-enemy (this
														 // last condition is used only if we're a good guy - in effect, we protect the innocent)
	) {
		return qtrue;
	}
...
qboolean NPC_FindEnemy(qboolean checkAlerts = qfalse) {
	...
	if (NPC->client->NPC_class != CLASS_RANCOR && NPC->client->NPC_class != CLASS_WAMPA && NPC->client->NPC_class != CLASS_SAND_CREATURE &&
...
qboolean NPC_FacePosition(vec3_t position, qboolean doPitch) {
	...
	if (NPC->client && (NPC->client->NPC_class == CLASS_RANCOR || NPC->client->NPC_class == CLASS_WAMPA || NPC->client->NPC_class == CLASS_SAND_CREATURE)) {
		...
		if (NPC->client->NPC_class == CLASS_ROCKETTROOPER) { //*sigh*, look down more
			position[2] -= 32;
		}
	}
...
void G_CheckCharmed(gentity_t *self) {
	if (self && self->client && self->client->playerTeam == TEAM_PLAYER && self->NPC && self->NPC->charmedTime &&
		(self->NPC->charmedTime < level.time || self->health <= 0)) { // we were charmed, set us back!
...
void G_GetBoltPosition(gentity_t *self, int boltIndex, vec3_t pos, int modelIndex = 0) {
	...
	gi.G2API_GetBoltMatrix(self->ghoul2, modelIndex, boltIndex, &boltMatrix, angles, self->currentOrigin, (cg.time ? cg.time : level.time), NULL,
						   self->s.modelScale);
	if (pos) {
