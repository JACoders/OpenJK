...
int CTaskManager::Wait(CTask *task, bool &completed, CIcarus *icarus) {
	...
	{
		if (Check(CIcarus::ID_RANDOM, block, memberNum)) { // get it random only the first time
			float min, max;

			dwtime = *(float *)block->GetMemberData(memberNum++);
			if (dwtime == icarus->GetGame()->MaxFloat()) { // we have not evaluated this random yet
				min = *(float *)block->GetMemberData(memberNum++);
				max = *(float *)block->GetMemberData(memberNum++);
		...
		if ((task->GetTimeStamp() + dwtime) < (icarus->GetGame()->GetTime())) {
			completed = true;
			memberNum = 0;
			if (Check(CIcarus::ID_RANDOM, block, memberNum)) { // set the data back to 0 so it will be re-randomized next time
...
int CTaskManager::Rotate(CTask *task, CIcarus *icarus) {
	...
	icarus->GetGame()->DebugPrint(IGameInterface::WL_DEBUG, "%4d rotate( <%f,%f,%f>, %d); [%d]", m_ownerID, vector[0], vector[1], vector[2], (int)duration,
								  task->GetTimeStamp());
...
int CTaskManager::Camera(CTask *task, CIcarus *icarus) {
	...
	case CIcarus::TYPE_PAN:
		...
		icarus->GetGame()->DebugPrint(IGameInterface::WL_DEBUG, "%4d camera( PAN, <%f %f %f>, <%f %f %f>, %f); [%d]", m_ownerID, vector[0], vector[1],
									  vector[2], vector2[0], vector2[1], vector2[2], fVal, task->GetTimeStamp());
	...
	case CIcarus::TYPE_MOVE:
		...
		icarus->GetGame()->DebugPrint(IGameInterface::WL_DEBUG, "%4d camera( MOVE, <%f %f %f>, %f); [%d]", m_ownerID, vector[0], vector[1], vector[2], fVal,
									  task->GetTimeStamp());
	...
	case CIcarus::TYPE_FOLLOW:
		...
		icarus->GetGame()->DebugPrint(IGameInterface::WL_DEBUG, "%4d camera( FOLLOW, \"%s\", %f, %f); [%d]", m_ownerID, sVal, fVal, fVal2,
									  task->GetTimeStamp());
	...
	case CIcarus::TYPE_FADE:
		...
		icarus->GetGame()->DebugPrint(IGameInterface::WL_DEBUG, "%4d camera( FADE, <%f %f %f>, %f, <%f %f %f>, %f, %f); [%d]", m_ownerID, vector[0], vector[1],
									  vector[2], fVal, vector2[0], vector2[1], vector2[2], fVal2, fVal3, task->GetTimeStamp());
...
int CTaskManager::Move(CTask *task, CIcarus *icarus) {
	...
	if (GetVector(m_ownerID, block, memberNum, vector2, icarus) == false) {
		...
		icarus->GetGame()->DebugPrint(IGameInterface::WL_DEBUG, "%4d move( <%f %f %f>, %f ); [%d]", m_ownerID, vector[0], vector[1], vector[2], duration,
									  task->GetTimeStamp());
	...
	icarus->GetGame()->DebugPrint(IGameInterface::WL_DEBUG, "%4d move( <%f %f %f>, <%f %f %f>, %f ); [%d]", m_ownerID, vector[0], vector[1], vector[2],
								  vector2[0], vector2[1], vector2[2], duration, task->GetTimeStamp());
