...
#define BYTESREMAINING_ACCOUNT_FOR_REAR_TAG(_pvData, _iBytesRemaining)         \
  ...
  gpTAG = (id3v1_1 *)(((byte *)_pvData + _iBytesRemaining) -                   \
...
#define PCM_BUFBYTES                                                           \
  60000U // more than enough to cover the largest that one packet will ever
         // expand to
char PCM_Buffer[PCM_BUFBYTES]; // better off being declared, so we don't do
                               // mallocs in this module (MAC reasons)

typedef struct {
  int (*decode_init)(MPEG_HEAD *h, int framebytes_arg, int reduction_code,
...
static const AUDIO audio = {audio_decode_init, audio_decode_info,
                            audio_decode}; // audio_table[0][0];
...
const int freq_limit =
    24000; // no idea what this is about, but it's always this value so...
...
char *C_MP3_IsValid(void *pvData, int iDataLen, int bStereoDesired) {
  ...
  //#ifdef _DEBUG
  //	int iIgnoreThisForNowIJustNeedItToBreakpointOnToReadAValue =
  //sizeof(MP3STREAM); #endif

  memset(pMP3Stream, 0, sizeof(*pMP3Stream));

  iFrameBytes =
      head_info3(pvData, iDataLen / 2, &head, &iBitRate, &iRealDataStart);
  if (iFrameBytes == 0) {
    return "MP3ERR: Bad or unsupported file!";
  }

  // check for files with bad frame unpack sizes (that would crash the game), or
  // stereo files.
  //
  // although the decoder can convert stereo to mono (apparently), we want to
  // know about stereo files
  //	because they're a waste of source space... (all FX are mono, and moved
  //via panning)
  //
  ...
  if (audio.decode_init(&head, iFrameBytes, reduction_code, iRealDataStart,
                        bStereoDesired ? convert_code_stereo
                                       : convert_code_mono,
                        freq_limit)) {
    if (bStereoDesired) {
      if (pMP3Stream->outbytes > 4608) {
        return "MP3ERR: Source file has output packet size > 2304 (*2 for "
               "stereo) bytes!";
      }
    } else {
    ...
    if (decinfo.bits != 16) {
      return "MP3ERR: Source file is not 16bit!"; // will this ever happen? oh
                                                  // well...
    }
    ...
    if (bStereoDesired && decinfo.channels != 2) {
      return "MP3ERR: Source file is not stereo!"; // sod it, I'm going to count
                                                   // this as an error now
    }
  } else {
...
// char *return is NZ for any errors (no trailing CR!)
//
char *C_MP3_GetHeaderData(void *pvData, int iDataLen, int *piRate, int *piWidth,
                          int *piChannels, int bStereoDesired) {
  unsigned int iRealDataStart;
  MPEG_HEAD head;
  DEC_INFO decinfo;
...
// this duplicates work done in C_MP3_IsValid(), but it avoids global structs,
// and means that you can call this anytime
//	if you just want info for some reason
//
// ( size is now workd out just by decompressing each packet header, not the
// whole stream. MUCH faster :-)
//
// char *return is NZ for any errors (no trailing CR!)
//
char *C_MP3_GetUnpackedSize(void *pvData, int iSourceBytesRemaining,
                            int *piUnpackedSize, int bStereoDesired) {
  int iReadLimit;
  unsigned int iRealDataStart;
  MPEG_HEAD head;
  int iBitRate;
...
#define iSourceReadIndex iRealDataStart
  ...
  iFrameBytes = head_info3(pvData, iSourceBytesRemaining / 2, &head, &iBitRate,
                           &iRealDataStart);
    ...
    // if (pPCM_Buffer)
    {
      ...
      if (audio.decode_init(&head, iFrameBytes, reduction_code, iRealDataStart,
        ...
        for (iFrameCounter = 0;; iFrameCounter++) {
          ...
          x = audio.decode((unsigned char *)pvData + iSourceReadIndex,
                           (short *)((char *)pPCM_Buffer
                                     //+ iDestWriteIndex		// keep decoding over
                                     //the same spot since we're only counting
...
char *C_MP3_UnpackRawPCM(void *pvData, int iSourceBytesRemaining,
                         int *piUnpackedSize, void *pbUnpackBuffer,
                         int bStereoDesired) {
  int iReadLimit;
  unsigned int iRealDataStart;
  MPEG_HEAD head;
  ...
  if (iFrameBytes) {
    //		if (1)////////////////////////pPCM_Buffer)
    {
      ...
      if (audio.decode_init(&head, iFrameBytes, reduction_code, iRealDataStart,
        ...
        //				printf("\n output samprate =
        //%6ld",decinfo.samprate); 				printf("\n output channels = %6d",
        //decinfo.channels); 				printf("\n output bits     = %6d", decinfo.bits);
        //				printf("\n output type     = %6d",
        //decinfo.type);
...
// called once, after we've decided to keep something as MP3. This just sets up
// the decoder for subsequent stream-calls.
//
// (the struct pSFX_MP3Stream is cleared internally, so pass as args anything
// you want stored in it)
//
// char * return is NULL for ok, else error string
//
// NEW CODE NOTE: Now that this function is being called for raw data that's
// going to be runtime-stored in a link-list
//	chunk format for SOF2 instead of just one alloc then you must re-init
//pMP3Stream->pbSourceData after you've called 	this, or it'll be pointing at the
//deallocated original raw data, not the first chunk of the link list
//
char *C_MP3Stream_DecodeInit(LP_MP3STREAM pSFX_MP3Stream, void *pvSourceData,
  ...
  pMP3Stream->pbSourceData =
      (byte *)pvSourceData; // this MUST be re-initialised to link-mem outside
                            // here for SOF2, since raw data is now link-listed
  pMP3Stream->iSourceBytesRemaining = iSourceBytesRemaining;
  pMP3Stream->iSourceFrameBytes =
      head_info3((byte *)pvSourceData, iSourceBytesRemaining / 2, &head,
                 &iBitRate, (unsigned int *)&pMP3Stream->iSourceReadIndex);

  // hack, do NOT do this for stereo, since music files are now streamed and
  // therefore the data isn't actually fully
  //	loaded at this point, only about 4k or so for the header is actually in
  //memory!!!...
  //
  if (!bStereoDesired) {
    BYTESREMAINING_ACCOUNT_FOR_REAR_TAG(pvSourceData,
                                        pMP3Stream->iSourceBytesRemaining);
  ...
  if (pMP3Stream->iSourceFrameBytes) {
    if (audio.decode_init(&head, pMP3Stream->iSourceFrameBytes, reduction_code,
                          pMP3Stream->iSourceReadIndex,
                          bStereoDesired ? convert_code_stereo
                                         : convert_code_mono,
                          freq_limit)) {
      pMP3Stream->iRewind_FinalReductionCode =
          reduction_code; // default = 0 (no reduction), 1=half, 2 = quarter
      ...
      // only now can we ask what kind of properties this file has, and then
      // adjust to fit what the game wants...
      //
      audio.decode_info(&decinfo);
      ...
      if (iGameAudioSampleBits ==
          decinfo.bits >> 1) // if game wants 8 bit sounds, then setup for that
        pMP3Stream->iRewind_FinalConvertCode |= 8;

      if (audio.decode_init(&head, pMP3Stream->iSourceFrameBytes,
                            pMP3Stream->iRewind_FinalReductionCode,
                            pMP3Stream->iSourceReadIndex,
                            pMP3Stream->iRewind_FinalConvertCode, freq_limit)) {
        audio.decode_info(&decinfo);
#ifdef _DEBUG
        ...
        //
        if (iGameAudioSampleRate != decinfo.samprate ||
            iGameAudioSampleBits != decinfo.bits) {
          psReturn = "MP3ERR: Decoder unable to convert to current game audio "
                     "settings";
        }
      } else {
        psReturn =
            "MP3ERR: Decoder failed to initialise for pass 2 sample adjust";
      }
    } else {
      psReturn = "MP3ERR: Decoder failed to initialise";
    }
    ...
    psReturn =
        "MP3ERR: Errr.... something's broken with this MP3 file"; // should
                                                                  // never
                                                                  // happen by
                                                                  // this point
  }

  // restore global stream ptr before returning to normal functions (so the rest
  // of the MP3 code still works)...
  //
...
// return value is decoded bytes for this packet, which is effectively a BOOL,
// NZ for not finished decoding yet...
//
unsigned int C_MP3Stream_Decode(LP_MP3STREAM pSFX_MP3Stream) {
  ...
  if (pSFX_MP3Stream->iSourceBytesRemaining ==
      0) // || pSFX_MP3Stream->iSourceBytesRemaining <
         // pSFX_MP3Stream->iSourceFrameBytes)
  {
  ...
  x = audio.decode(
      pSFX_MP3Stream->pbSourceData + pSFX_MP3Stream->iSourceReadIndex,
      (short *)(pSFX_MP3Stream->bDecodeBuffer),
      pSFX_MP3Stream->pbSourceData + pSFX_MP3Stream->iRewind_SourceReadIndex +
          pSFX_MP3Stream->iRewind_SourceBytesRemaining);
...
char *C_MP3Stream_Rewind(LP_MP3STREAM pSFX_MP3Stream) {
  ...
  pMP3Stream->iSourceBytesRemaining =
      pMP3Stream->iRewind_SourceBytesRemaining; // already adjusted for tags etc

  // I'm not sure that this is needed, but where else does decode_init get
  // passed useful data ptrs?...
  //
  if (pMP3Stream->iSourceFrameBytes ==
      head_info3(pMP3Stream->pbSourceData,
                 pMP3Stream->iSourceBytesRemaining / 2, &head, &iBitRate,
                 (unsigned int *)&iNULL)) {
    if (audio.decode_init(&head, pMP3Stream->iSourceFrameBytes,
                          pMP3Stream->iRewind_FinalReductionCode,
                          pMP3Stream->iSourceReadIndex,
