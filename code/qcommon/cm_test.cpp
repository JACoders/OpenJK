...
{
public:
        bool operator()(const CPoint& _A,const CPoint& _B) const
...
// I think it is a patholoically bad idea to do a 64 item linear search for a
// cache, so I reduced this to something more manageable. hopefully getting rid
// of water checks on maps with no water will leave us less reliant on this
// cache. -gwg
...
int CM_PointContents(const vec3_t p, clipHandle_t model) {
  ...
  if (model) {
    ...
    /*		map<CPoint,int,CPointComparator>::iterator
       l=pointToLeaf.find(pt); if(l!=pointToLeaf.end())
                    {
                            leafnum=(*l).second;
                    }
                    else
    ...
    if (l == sizePointToLeaf) {                     // Didn't find it
      if (sizePointToLeaf < MAX_POINTS_TO_LEAVES) { // We're adding a new one,
                                                    // rather than replacing
        sizePointToLeaf++;
      } else { // Put it in the "oldest" slot
        l = oldestPointToLeaf++;
