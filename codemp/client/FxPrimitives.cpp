...
//
//--------------------------
CEffect::CEffect(void) : mFlags(0), mMatImpactFX(MATIMPACTFX_NONE), mMatImpactParm(-1), mSoundRadius(-1), mSoundVolume(-1) {
...
bool CParticle::Update(void) {
	...
	if (mFlags & FX_RELATIVE) {
		if (!re->G2API_IsGhoul2InfovValid(*mGhoul2)) { // the thing we are bolted to is no longer valid, so we may as well just die.
			return false;
		}
		...
		if (!theFxHelper.GetOriginAxisFromBolt(mGhoul2, mEntNum, mModelNum, mBoltNum, org, ax)) { // could not get bolt
			return false;
		}
...
//
//--------------------------------
COrientedParticle::COrientedParticle(void) { mRefEnt.reType = RT_ORIENTED_QUAD; }
...
bool CLine::Update(void) {
	...
	if (mFlags & FX_RELATIVE) {
		...
		matrix3_t ax;
		// Get our current position and direction
		if (!theFxHelper.GetOriginAxisFromBolt(mGhoul2, mEntNum, mModelNum, mBoltNum, mOrigin1, ax)) { // could not get bolt
			return false;
		}
