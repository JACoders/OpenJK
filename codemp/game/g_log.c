...
char *weaponNameFromIndex[WP_NUM_WEAPONS] = {"No Weapon", "Stun Baton", "Saber",		   "Bryar Pistol", "Blaster",  "Disruptor", "Bowcaster",	"Repeater",
											 "Demp2",	  "Flechette",	"Rocket Launcher", "Thermal",	   "Tripmine", "Detpack",	"Emplaced gun", "Turret"};
...
qboolean CalculateUntouchable(gentity_t *ent) {
	...
	if (level.gametype == GT_JEDIMASTER && ent->client->ps.isJediMaster) { // Jedi Master can only be killed once anyway
		return qfalse;
	}
...
qboolean CalculateTactician(gentity_t *ent, int *kills) {
	...
	if (level.gametype == GT_JEDIMASTER && ent->client->ps.isJediMaster) { // Jedi Master has only 1 weapon
		return qfalse;
	}
	//------------------------------------------------------ MUST HAVE ACHIEVED 2 KILLS PER MINUTE
	if (playTime < 0.3)
