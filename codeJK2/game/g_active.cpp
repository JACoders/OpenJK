...
void G_ChooseLookEnemy(gentity_t *self, usercmd_t *ucmd) {
	...
	for (e = 0; e < numListedEntities; e++) {
		...
		if (ent->health <= 0) {
			if ((ucmd->buttons & BUTTON_ATTACK) || (ucmd->buttons & BUTTON_ALT_ATTACK)) { // if attacking, don't consider dead enemies
				continue;
			}
	...
	if (!self->client || self->client->ps.lastOnGround + 300 < level.time || (self->client->ps.lastOnGround + 100 < level.time)) {
		...
		if (other->material == MAT_GLASS || other->material == MAT_GLASS_METAL || other->material == MAT_GRATE1 ||
			((other->svFlags & SVF_BBRUSH) &&
			 (other->spawnflags &
			  4 /*THIN*/))) //(other->absmax[0]-other->absmin[0]<=32||other->absmax[1]-other->absmin[1]<=32||other->absmax[2]-other->absmin[2]<=32)) )
		...
		if (damageSelf && self->takedamage && !(self->flags & FL_NO_IMPACT_DMG)) {
			...
			if (self->client && self->client->ps.forceJumpZStart) {				  // we were force-jumping
			...
			if ((magnitude >= 100 + self->health && self->s.number != 0 && self->s.weapon != WP_SABER) ||
				(magnitude >= 700)) //&& self.safe_time < level.time ))//health here is used to simulate structural integrity
			{
				if ((self->s.weapon == WP_SABER || self->s.number == 0) && self->client && self->client->ps.groundEntityNum < ENTITYNUM_NONE &&
...
void G_NPCMunroMatchPlayerWeapon(gentity_t *ent) {
	...
	if (!in_camera) {
		if (ent && ent->client && ent->NPC && (ent->NPC->aiFlags & NPCAI_MATCHPLAYERWEAPON)) { // we're a Kyle NPC
...
qboolean ClientInactivityTimer(gclient_t *client) {
	...
	} else if (client->usercmd.forwardmove || client->usercmd.rightmove || client->usercmd.upmove || (client->usercmd.buttons & BUTTON_ATTACK) ||
...
void G_UpdateEmplacedWeaponData(gentity_t *ent) {
	if (ent && ent->owner && ent->health > 0) {
		...
		gi.G2API_GetBoltMatrix(chair->ghoul2, chair->playerModel, chair->headBolt, &boltMatrix, chairAng, chair->currentOrigin,
							   (cg.time ? cg.time : level.time), NULL, chair->s.modelScale);
...
void ExitEmplacedWeapon(gentity_t *ent) {
	...
	if (ent->client) {
		...
		if (ent->health > 0 && ent->owner->nextTrain) { // he's still alive, and we have a placeholder, so put him back
...
void ClientThink_real(gentity_t *ent, usercmd_t *ucmd) {
		...
		if (in_camera) {
			...
			if (speed && ent->client->ps.groundEntityNum == ENTITYNUM_NONE) {
				...
				if ((slide > 0 && ucmd->rightmove >= 0) ||
					((slide < 0 && ucmd->rightmove <= 0))) { // note: don't want these to conflict right now because that seems to feel really weird
	...
	if (ent->client) {
		if (ent->client->NPC_class == CLASS_GONK || ent->client->NPC_class == CLASS_MOUSE || ent->client->NPC_class == CLASS_R2D2 ||
			...
			if (ent->client->ps.groundEntityNum != ENTITYNUM_NONE) { // ATST crushed anything underneath it
			...
			if (groundEnt && groundEnt->client && groundEnt->client->ps.groundEntityNum != ENTITYNUM_NONE && groundEnt->health > 0 &&
				!PM_InRoll(&groundEnt->client->ps) && !(groundEnt->client->ps.eFlags & EF_LOCKED_TO_WEAPON) &&
				!inSpinFlipAttack) { // landed on a live client who is on the ground, jump off them and knock them down
				if (ent->health > 0) {
				...
				if (groundEnt->client->NPC_class == CLASS_DESANN && ent->client->NPC_class != CLASS_LUKE) { // can't knock down desann unless you're luke
																											// FIXME: should he smack you away like Galak Mech?
				} else if ((((groundEnt->s.number &&
							  (groundEnt->s.weapon != WP_SABER || !groundEnt->NPC ||
							   groundEnt->NPC->rank <
								   Q_irand(RANK_CIVILIAN, RANK_CAPTAIN + 1))) // an NPC who is either not a saber user or passed the rank-based probability test
	...
	// set speed
	if (ent->NPC != NULL) { // we don't actually scale the ucmd, we use actual speeds
		...
		if (ent->client->ps.vehicleModel != 0 && ent->health > 0) {
			...
			if (PM_SaberInAttack(ent->client->ps.saberMove) && ucmd->forwardmove < 0) { // if running backwards while attacking, don't run as fast.
	...
	if ((ucmd->buttons & BUTTON_BLOCKING) && !g_saberAutoBlocking->integer) { // blocking with saber
	...
	if (!ent->s.number) {
		if (ent->client->ps.viewEntity > 0 && ent->client->ps.viewEntity < ENTITYNUM_WORLD) { // you're controlling another NPC
			...
			{															 // these types use jump to get out
				...
				if ((ucmd->buttons & BUTTON_BLOCKING)) { // jumping gets you out of it FIXME: check some other button instead... like ESCAPE... so you could
														 // even have total control over an NPC?
					G_ClearViewEntity(ent);
					ucmd->buttons = 0;
					freed = qtrue;
				}
