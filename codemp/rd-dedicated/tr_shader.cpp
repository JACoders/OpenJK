...
static void ParseSurfaceSpritesOptional(const char *param, const char *_text,
  ...
  if (!Q_stricmp(param, "ssFademax")) {
    ...
    if (value <= stage->ss->fadeDist) {
      Com_Printf(S_COLOR_YELLOW "WARNING: invalid surfacesprite fademax (%.2f "
  ...
  if (!Q_stricmp(param, "ssFXWeather")) {
    if (stage->ss->surfaceSpriteType != SURFSPRITE_EFFECT) {
      Com_Printf(S_COLOR_YELLOW "WARNING: weather applied to non-effect "
...
static qboolean ParseStage(shaderStage_t *stage, const char **text) {
  ...
  while (1) {
    ...
    //
      ...
      token = COM_ParseExt(text, qfalse);
      if (!token[0]) {
        Com_Printf(S_COLOR_YELLOW "WARNING: missing parameter for 'clampmap' "
                                  "keyword in shader '%s'\n",
                   shader.name);
        return qfalse;
      }
...
#define MAX_IMAGE_ANIMATIONS 32
      ...
      token = COM_ParseExt(text, qfalse);
      if (!token[0]) {
        Com_Printf(S_COLOR_YELLOW "WARNING: missing parameter for 'videoMap' "
                                  "keyword in shader '%s'\n",
                   shader.name);
        return qfalse;
      }
    ...
    //
      ...
      token = COM_ParseExt(text, qfalse);
      if (!token[0]) {
        Com_Printf(S_COLOR_YELLOW "WARNING: missing parameter for 'alphaFunc' "
                                  "keyword in shader '%s'\n",
                   shader.name);
        return qfalse;
      }
    ...
    //
      ...
      if (!token[0]) {
        Com_Printf(S_COLOR_YELLOW "WARNING: missing parameter for 'depthfunc' "
                                  "keyword in shader '%s'\n",
                   shader.name);
        return qfalse;
      }
    ...
    //
      ...
      if (!Q_stricmp(token, "wave")) {
        ...
        if (shader.lightmapIndex[0] != LIGHTMAP_NONE) {
          Com_Printf(S_COLOR_RED "ERROR: rgbGen lightingDiffuseEntity used on "
                                 "a misc_model! in shader '%s'\n",
                     shader.name);
        }
...
static void ParseDeform(const char **text) {
  ...
  if (!Q_stricmp(token, "wave")) {
    ...
    if (atof(token) != 0) {
      ...
      Com_Printf(S_COLOR_YELLOW "WARNING: illegal div value of 0 in "
...
=================
*/
static qboolean CollapseMultitexture(void) { return qfalse; }
...
static shader_t *GeneratePermanentShader(void) {
  ...
  if (tr.numShaders == MAX_SHADERS) {
    // Com_Printf (S_COLOR_YELLOW  "WARNING: GeneratePermanentShader -
    // MAX_SHADERS hit\n");
