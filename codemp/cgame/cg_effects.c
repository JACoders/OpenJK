...
void CG_TestLine(vec3_t start, vec3_t end, int time, unsigned int color,
  ...
  re->customShader =
      cgs.media
          .whiteShader; // trap->R_RegisterShaderNoMip("textures/colombia/canvas_doublesided");
...
static void CG_DoGlassQuad(vec3_t p[4], vec2_t uv[4], qboolean stick, int time,
                           vec3_t dmgDir) {
  float bounce;
  vec3_t rotDelta;
  vec3_t vel, accel;
  ...
  // We are using an additive shader, so let's set the RGB low so we look more
  ...
  CPoly *pol = FX_AddPoly(p, uv, 4,			// verts, ST, vertCount
                  vel, accel,				// motion
                  0.15f, 0.0f, 85.0f,		// alpha start, alpha end, alpha
  parm ( begin alpha fade when 85% of life is complete ) rgb1, rgb1, 0.0f,
  // rgb start, rgb end, rgb parm ( not used ) rotDelta, bounce, time,	//
...
void CG_DoGlass(vec3_t verts[4], vec3_t normal, vec3_t dmgPt, vec3_t dmgDir,
  ...
  trap->S_StartSound(dmgPt, -1, CHAN_AUTO,
  ...
  */
  ...
  for (z = 0.0f, i = 0; z < 1.0f; z += stepHeight, i++) {
    for (x = 0.0f, t = 0; x < 1.0f; x += stepWidth, t++) {
      ...
      dif = DistanceSquared(subVerts[0], dmgPt) * timeDecay -
...
void CG_GlassShatter(int entnum, vec3_t dmgPt, vec3_t dmgDir, float dmgRadius,
  ...
  if (cgs.inlineDrawModel[cg_entities[entnum].currentState.modelindex]) {
    trap->R_GetBModelVerts(
        cgs.inlineDrawModel[cg_entities[entnum].currentState.modelindex], verts,
        normal);
...
void CG_MiscModelExplosion(vec3_t mins, vec3_t maxs, int size,
  ...
  // rww - No they don't.. indexed effects gameside get precached on load
  // clientside, as server objects are setup before client asset load time.
...
void CG_Chunks(int owner, vec3_t origin, const vec3_t normal, const vec3_t mins,
  ...
  case MAT_ELEC_METAL: // FIXME: maybe have its own sound?
    ...
    break;
  case MAT_ROPE:
    //		trap->S_StartSound( NULL, owner, CHAN_BODY, cgi_S_RegisterSound( ""
    //));  FIXME:  needs a sound
    return;
    break;
  }
  ...
  for (i = 0; i < numChunks; i++) {
    ...
    if (chunkModel) {
      ...
      VectorNormalize(dir);
      VectorScale(dir, flrand(speed * 0.5f, speed * 1.25f) * speedMod,
                  le->pos.trDelta);
...
localEntity_t *CG_MakeExplosion(vec3_t origin, vec3_t dir, qhandle_t hModel,
                                ...
                                qboolean isSprite, float scale, int flags) {
  float ang = 0;
  localEntity_t *ex;
  int offset;
...
void CG_SurfaceExplosion(vec3_t origin, vec3_t normal, float radius,
  ...
  for (i = 0; i < 4; i++) {
    VectorSet(temp_org, new_org[0] + (Q_flrand(-1.0f, 1.0f) * 16.0f),
              new_org[1] + (Q_flrand(-1.0f, 1.0f) * 16.0f),
              new_org[2] + (Q_flrand(0.0f, 1.0f) * 4.0f));
    VectorSet(temp_vel, velocity[0] + (Q_flrand(-1.0f, 1.0f) * 8.0f),
    ...
    /*		FX_AddSprite(	temp_org,
                                                    ...
                                                    NULL,
                                                    64.0f +
       (Q_flrand(0.0f, 1.0f) * 32.0f), 16.0f, 1.0f, 0.0f, 20.0f +
       (Q_flrand(-1.0f, 1.0f) * 90.0f), 0.5f, 1500.0f, cgs.media.smokeShader,
       FXF_USE_ALPHA_CHAN );*/
  }
  ...
  for (i = 0; i < NUM_EXPLOSIONS - 1; i++) {
    VectorSet(new_org,
              (origin[0] +
               (16 + (Q_flrand(-1.0f, 1.0f) * 8)) * Q_flrand(-1.0f, 1.0f)),
              (origin[1] +
               (16 + (Q_flrand(-1.0f, 1.0f) * 8)) * Q_flrand(-1.0f, 1.0f)),
              (origin[2] +
               (16 + (Q_flrand(-1.0f, 1.0f) * 8)) * Q_flrand(-1.0f, 1.0f)));
    le = CG_MakeExplosion(new_org, direction, cgs.media.explosionModel, 6,
                          cgs.media.surfaceExplosionShader, 300 + (rand() & 99),
                          qfalse,
                          radius * 0.05f + (Q_flrand(-1.0f, 1.0f) * 0.3f), 0);
  }

  // Shake the camera
  CG_ExplosionEffects(origin, shake_speed, 350, 750);

  // The level designers wanted to be able to turn the smoke spawners off.  The
  // rationale is that they
  //	want to blow up catwalks and such that fall down...when that happens, it
  //shouldn't really leave a mark 	and a smoke spewer at the explosion point...
  if (smoke) {
