...
int G_SkinIndex(const char *name) { return G_FindConfigstringIndex(name, CS_CHARSKINS, MAX_CHARSKINS, qtrue); }
...
int G_BSPIndex(char *name) { return G_FindConfigstringIndex(name, CS_BSP_MODELS, MAX_SUB_BSP, qtrue); }
...
//-----------------------------
void G_PlayEffect(int fxID, const int modelIndex, const int boltIndex, const int entNum, const vec3_t origin, int iLoopTime,
				  qboolean isRelative) // iLoopTime 0 = not looping, 1 for infinite, else duration
{
	gentity_t *tent;
...
//-----------------------------
void G_PlayEffect(const char *name, const vec3_t origin, const vec3_t fwd) { G_PlayEffect(G_EffectIndex(name), origin, fwd); }

//-----------------------------
void G_PlayEffect(const char *name, const vec3_t origin, const vec3_t axis[3]) { G_PlayEffect(G_EffectIndex(name), origin, axis); }
...
void G_SpeechEvent(gentity_t *self, int event) {
	if (in_camera && g_skippingcin && g_skippingcin->integer) { // Skipping a cinematic, so skip NPC voice sounds...
		return;
	}
...
void G_KillBox(gentity_t *ent) {
	...
	for (i = 0; i < num; i++) {
		...
		// nail it
		G_Damage(hit, ent, ent, NULL, NULL, 100000, DAMAGE_NO_PROTECTION, MOD_UNKNOWN);
	}
}
...
qboolean ValidUseTarget(gentity_t *ent) {
	...
	if (!(ent->svFlags & SVF_PLAYER_USABLE)) { // Check for flag that denotes BUTTON_USE useability
		return qfalse;
	}
...
static void DebugTraceForNPC(gentity_t *ent) {
	...
	gi.trace(&trace, src, vec3_origin, vec3_origin, dest, ent->s.number,
			 MASK_OPAQUE | CONTENTS_SOLID | CONTENTS_TERRAIN | CONTENTS_BODY | CONTENTS_ITEM | CONTENTS_CORPSE, (EG2_Collision)0, 0);
...
static qboolean G_IsTriggerUsable(gentity_t *self, gentity_t *other) {
	...
	if ((!G_ValidActivateBehavior(self, BSET_USE) && !self->target) ||
		(self->target && (Q_stricmp(self->target, "n") == 0 || (Q_stricmp(self->target, "neveropen") == 0 || (Q_stricmp(self->target, "run_gran_drop") == 0) ||
																(Q_stricmp(self->target, "speaker") == 0) || (Q_stricmp(self->target, "locked") == 0))))) {
		return qfalse;
	}
...
qboolean CanUseInfrontOf(gentity_t *ent) {
	...
	if (ent->s.number && ent->client->NPC_class == CLASS_ATST) { // a player trying to get out of his ATST
	...
	// Trace ahead to find a valid target
	gi.trace(&trace, src, vec3_origin, vec3_origin, dest, ent->s.number,
			 MASK_OPAQUE | CONTENTS_SOLID | CONTENTS_TERRAIN | CONTENTS_BODY | CONTENTS_ITEM | CONTENTS_CORPSE, G2_NOCOLLIDE, 10);
	...
	if (ValidUseTarget(target)) {
		if (target->s.eType == ET_ITEM) {						  // item, see if we could actually pick it up
			if ((target->spawnflags & 128 /*ITMSF_USEPICKUP*/)) { // player has to be touching me and hit use to pick it up, so don't allow this
				if (!G_BoundsOverlap(target->absmin, target->absmax, ent->absmin, ent->absmax)) { // not touching
					return qfalse;
				}
			}
			if (!BG_CanItemBeGrabbed(&target->s, &ent->client->ps)) { // nope, so don't indicate that we can use it
				return qfalse;
			}
		} else if (target->e_UseFunc == useF_misc_atst_use) {		   // drivable AT-ST from JK2
			if (ent->client->ps.groundEntityNum != target->s.number) { // must be standing on it to use it
				return qfalse;
			}
	...
	if (target->client && target->client->ps.pm_type < PM_DEAD && target->NPC != NULL && target->client->playerTeam &&
		(target->client->playerTeam == ent->client->playerTeam || target->client->playerTeam == TEAM_NEUTRAL) &&
		!(target->NPC->scriptFlags & SCF_NO_RESPONSE) && G_ValidActivateBehavior(target, BSET_USE)) {
		return qtrue;
	}
...
void TryUse(gentity_t *ent) {
	...
	if (ent->s.number == 0 && ent->client->NPC_class == CLASS_ATST) { // a player trying to get out of his ATST
		GEntity_UseFunc(ent->activator, ent, ent);
		return;
	}
	...
	if (ValidUseTarget(target)) {
		...
		*/
		...
		return;
	} else if (target->client && target->client->ps.pm_type < PM_DEAD && target->NPC != NULL && target->client->playerTeam &&
			   (target->client->playerTeam == ent->client->playerTeam || target->client->playerTeam == TEAM_NEUTRAL) &&
...
void G_ChangeMap(const char *mapname, const char *spawntarget, qboolean hub) {
	...
	if (g_entities[0].client->ps.pm_type == PM_DEAD)
		return;
	if (killPlayerTimer) { // can't go to next map if your allies have turned on you
		return;
	}
...
void removeBoltSurface(gentity_t *ent) {
	...
	if ((hitEnt->ghoul2.size() > ent->damage) && (hitEnt->ghoul2[ent->damage].mModelindex != -1) &&
