...
int Pickup_Weapon(gentity_t *ent, gentity_t *other) {
  ...
  */
  ...
  if (other->s.number) {              // NPC
    if (other->s.weapon == WP_NONE) { // NPC with no weapon picked up a weapon,
      ...
      if (ent->item->giTag == WP_SABER) {
        ...
        G_CreateG2AttachedWeaponModel(other,
                                      weaponData[ent->item->giTag].weaponMdl);
      }
    }
  }
...
void Touch_Item(gentity_t *ent, gentity_t *other, trace_t *trace) {
  ...
  if ((ent->spawnflags & ITMSF_MONSTER) &&
      (other->client->playerTeam == TEAM_PLAYER)) {
    return;
  }

  // Only starfleet can pick it up
  if ((ent->spawnflags & ITMSF_TEAM) &&
      (other->client->playerTeam != TEAM_PLAYER)) {
    return;
  }
  ...
  } else if (!(ent->spawnflags & ITMSF_TEAM) &&
             !(ent->spawnflags & ITMSF_MONSTER)) { // Only player can pick it up
  ...
  if (other->client) {
    if (other->client->ps.eFlags &
        EF_FORCE_GRIPPED) { // can't pick up anything while being gripped
      return;
    }
...
void FinishSpawningItem(gentity_t *ent) {
  ...
  //	if ( item->giType == IT_WEAPON ) // NOTE: james thought it was ok to
  //just always do this?
  {
    ent->s.radius = 20;
  ...
  */
  ...
  if (ent->spawnflags & ITMSF_SUSPEND) {
    ...
    gi.trace(&tr, ent->s.origin, ent->mins, ent->maxs, dest, ent->s.number,
             MASK_SOLID | CONTENTS_PLAYERCLIP, G2_NOCOLLIDE, 0);
    if (tr.startsolid) {
...
void ClearRegisteredItems(void) {
  ...
  RegisterItem(FindItemForWeapon(
      WP_BRYAR_PISTOL)); // these are given in g_client, ClientSpawn(), but MUST
                         // be registered HERE, BEFORE cgame starts.
  RegisterItem(FindItemForWeapon(
      WP_STUN_BATON)); // these are given in g_client, ClientSpawn(), but MUST
                       // be registered HERE, BEFORE cgame starts.
