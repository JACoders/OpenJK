...
void CMod_LoadShaders(lump_t *l, clipMap_t &cm) {
  ...
  cm.shaders = (CCMShader *)Z_Malloc((1 + count) * sizeof(*cm.shaders), TAG_BSP,
                                     qtrue); //+1 for the BOX_SIDES to point at
  cm.numShaders = count;
...
void CMod_LoadSubmodels(lump_t *l, clipMap_t &cm) {
  ...
  // FIXME: note that MAX_SUBMODELS - 1 is used for BOX_MODEL_HANDLE, if that
  // slot gets used, that would be bad, no?
  if (count > MAX_SUBMODELS) {
    Com_Error(ERR_DROP, "MAX_SUBMODELS (%d) exceeded by %d", MAX_SUBMODELS,
              count - MAX_SUBMODELS);
  }

  cm.cmodels =
      (struct cmodel_s *)Z_Malloc(count * sizeof(*cm.cmodels), TAG_BSP, qtrue);
  ...
  for (i = 0; i < count; i++, in++, out++) {
    ...
    // rww - I changed this to do the &cm == &cmg check. sof2 does not have to
    // do this, but I think they have a different tracing system that allows it
    // to catch subbsp stuff without extra leaf data. The reason we have to do
    // this for subbsp instances is that they often are compiled in a sort of
    // "prefab" form, so the first model isn't necessarily the world model.
    if (i == 0 && &cm == &cmg) {
...
void CMod_LoadBrushes(lump_t *l, clipMap_t &cm) {
  ...
  cm.brushes = (cbrush_t *)Z_Malloc((BOX_BRUSHES + count) * sizeof(*cm.brushes),
                                    TAG_BSP, qfalse);
  cm.numBrushes = count;
...
#ifdef JK2_MODE
    // JK2 HACK: for water that cuts vis but is not solid!!! (used on yavin
    // swamp)
...
void CMod_LoadLeafs(lump_t *l, clipMap_t &cm) {
  ...
  cm.leafs = (cLeaf_t *)Z_Malloc((BOX_LEAFS + count) * sizeof(*cm.leafs),
                                 TAG_BSP, qfalse);
  cm.numLeafs = count;
  out = cm.leafs;
  ...
  cm.areas =
      (cArea_t *)Z_Malloc(cm.numAreas * sizeof(*cm.areas), TAG_BSP, qtrue);
  cm.areaPortals = (int *)Z_Malloc(
      cm.numAreas * cm.numAreas * sizeof(*cm.areaPortals), TAG_BSP, qtrue);
}
...
void CMod_LoadPlanes(lump_t *l, clipMap_t &cm) {
  ...
  cm.planes = (struct cplane_s *)Z_Malloc(
      (BOX_PLANES + count) * sizeof(*cm.planes), TAG_BSP, qfalse);
  cm.numPlanes = count;
...
void CMod_LoadLeafBrushes(lump_t *l, clipMap_t &cm) {
  ...
  cm.leafbrushes = (int *)Z_Malloc(
      (BOX_BRUSHES + count) * sizeof(*cm.leafbrushes), TAG_BSP, qfalse);
...
void CMod_LoadLeafSurfaces(lump_t *l, clipMap_t &cm) {
  ...
  cm.leafsurfaces =
      (int *)Z_Malloc(count * sizeof(*cm.leafsurfaces), TAG_BSP, qfalse);
...
void CMod_LoadBrushSides(lump_t *l, clipMap_t &cm) {
  ...
  cm.brushsides = (cbrushside_t *)Z_Malloc(
      (BOX_SIDES + count) * sizeof(*cm.brushsides), TAG_BSP, qfalse);
...
void CMod_LoadPatches(lump_t *surfs, lump_t *verts, clipMap_t &cm) {
  ...
  cm.surfaces = (cPatch_t **)Z_Malloc(cm.numSurfaces * sizeof(cm.surfaces[0]),
                                      TAG_BSP, qtrue);
  ...
  for (i = 0; i < count; i++, in++) {
    ...
    cm.surfaces[i] = patch =
        (cPatch_t *)Z_Malloc(sizeof(*patch), TAG_BSP, qtrue);
...
qboolean gbUsingCachedMapDataRightNow =
    qfalse; // if true, signifies that you can't delete this at the moment!!
            // (used during z_malloc()-fail recovery attempt)

// called in response to a "devmapbsp blah" or "devmapall blah" command, do NOT
// use inside CM_Load unless you pass in qtrue
//
// new bool return used to see if anything was freed, used during z_malloc
// failure re-try
//
qboolean CM_DeleteCachedMap(qboolean bGuaranteedOkToDelete) {
  ...
  if (bGuaranteedOkToDelete || !gbUsingCachedMapDataRightNow) {
    ...
    // force map loader to ignore cached internal BSP structures for next level
    // CM_LoadMap() call...
    //
    cmg.name[0] = '\0';
  }
...
static void CM_LoadMap_Actual(const char *name, qboolean clientload,
                              int *checksum, clipMap_t &cm) {
  const int *buf;
  size_t i;
  dheader_t header;
  ...
  cm_playerCurveClip =
      Cvar_Get("cm_playerCurveClip", "1", CVAR_ARCHIVE_ND | CVAR_CHEAT);
#endif
  ...
  if (&cm == &cmg) {
    // if there was a cached disk image but the name was empty (ie ERR_DROP
    // happened) or just doesn't match
    //	the current name, then ditch it...
    //
    if (gpvCachedMapDiskImage && (gsCachedMapDiskImage[0] == '\0' ||
                                  strcmp(gsCachedMapDiskImage, name))) {
      Z_Free(gpvCachedMapDiskImage);
      gpvCachedMapDiskImage = NULL;
      gsCachedMapDiskImage[0] = '\0';
  ...
  // if there's a valid map name, and it's the same as last time (respawn?), and
  ...
  if (name[0] && !strcmp(cm.name, name) && !clientload && &cm == &cmg) {
    ...
    memset(cm.areaPortals, 0,
           cm.numAreas * cm.numAreas * sizeof(*cm.areaPortals));
  } else {
    // ... else load map from scratch...
    //
    if (&cm == &cmg) {
      assert(!clientload); // logic check. I'm assuming that a client load
      ...
      if (!name[0]) {
        ...
        cm.numAreas = 1;
        cm.cmodels =
            (struct cmodel_s *)Z_Malloc(sizeof(*cm.cmodels), TAG_BSP, qtrue);
        *checksum = 0;
        return;
      }
    }

    // load the file into a buffer that we either discard as usual at the
    // bottom, or if we've got enough memory
    //	then keep it long enough to save the renderer re-loading it, then
    ...
    if (&cm == &cmg) {
      if (gpvCachedMapDiskImage &&
          gsCachedMapDiskImage
              [0]) { // didn't get cleared elsewhere so free it before we
                     // allocate the pointer again Maps with terrain will allow
                     // this to happen because they want everything to be cleared
                     // out (going between terrain and no-terrain is messy)
        Z_Free(gpvCachedMapDiskImage);
      }
      gsCachedMapDiskImage[0] =
          '\0'; // flag that map isn't valid, until name is filled in
      gpvCachedMapDiskImage = Z_Malloc(iBSPLen, TAG_BSP_DISKIMAGE, qfalse);
      FS_Read(gpvCachedMapDiskImage, iBSPLen, h);
      FS_FCloseFile(h);

      buf = (int *)
          gpvCachedMapDiskImage; // so the rest of the code works as normal
    } else {                     // otherwise, read straight in..
    ...
    CMod_LoadPatches(&header.lumps[LUMP_SURFACES],
                     &header.lumps[LUMP_DRAWVERTS], cm);

    TotalSubModels += cm.numSubModels;

    // we are NOT freeing the file, because it is cached for the ref
    // actually we do because the new hunk sys won't allow it
    // actually we DON'T now <g>, if we've got enough ram to keep it for the
    // renderer's disk-load...
    //
    extern qboolean Sys_LowPhysicalMemory();
    if (Sys_LowPhysicalMemory() //|| com_dedicated->integer	// no need to
                                //check for dedicated in single-player codebase
    ) {
      Z_Free(gpvCachedMapDiskImage);
      gpvCachedMapDiskImage = NULL;
    } else {
    ...
    if (&cm == &cmg) {
      ...
      Q_strncpyz(gsCachedMapDiskImage, name,
                 sizeof(gsCachedMapDiskImage)); // so the renderer can check it
    }
  }
...
// need a wrapper function around this because of multiple returns, need to
...
clipHandle_t CM_InlineModel(int index) {
  if (index < 0 || index >= TotalSubModels) {
    Com_Error(ERR_DROP,
              "CM_InlineModel: bad number: %d >= %d (may need to re-BSP map?)",
              index, TotalSubModels);
  }
  return index;
}

int CM_NumInlineModels(void) { return cmg.numSubModels; }

char *CM_EntityString(void) { return cmg.entityString; }

char *CM_SubBSPEntityString(int index) { return SubBSP[index].entityString; }
...
void CM_InitBoxHull(void) {
  ...
  for (i = 0; i < 6; i++) {
    ...
    s->shaderNum = cmg.numShaders; // not storing flags directly anymore, so be
                                   // sure to point @ a valid shader
...
clipHandle_t CM_TempBoxModel(const vec3_t mins,
  ...
  // FIXME: this is the "correct" way, but not the way JK2 was designed
  // around... fix for further projects box_brush->contents = contents;
...
===================
*/
void CM_ModelBounds(clipMap_t &cm, clipHandle_t model, vec3_t mins,
                    vec3_t maxs) {
  cmodel_t *cmod;
...
int CM_FindSubBSP(int modelIndex) {
  ...
  count = cmg.numSubModels;
  if (modelIndex < count) { // belongs to the main bsp
    return -1;
  }
...
int CM_ModelContents_Actual(clipHandle_t model, clipMap_t *cm) {
  ...
  for (int i = 0; i < cmod->leaf.numLeafSurfaces; i++) {
    ...
    if (cm->surfaces[surfaceNum]) {
      // HERNH?  How could we have a null surf within our
      // cmod->leaf.numLeafSurfaces?
...
int SG_Read(unsigned int chid, void *pvAddress, int iLength,
            void **ppvAddressPtr = NULL);

void CM_WritePortalState() {
  ojk::SavedGameHelper saved_game(&ojk::SavedGame::get_instance());

  saved_game.write_chunk<int32_t>(INT_ID('P', 'R', 'T', 'S'), ::cmg.areaPortals,
...
void CM_ReadPortalState() {
  ...
  saved_game.read_chunk<int32_t>(INT_ID('P', 'R', 'T', 'S'), ::cmg.areaPortals,
                                 ::cmg.numAreas * ::cmg.numAreas);
