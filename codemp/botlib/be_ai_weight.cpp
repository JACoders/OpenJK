	...
	if (inventory[fs->index] < fs->value) {
		...
		if (inventory[fs->index] < fs->next->value) {
			...
			if (fs->child)
				...
				w1 = fs->minweight + Q_flrand(0.0f, 1.0f) * (fs->maxweight - fs->minweight);
			// second weight
			if (fs->next->child)
				w2 = FuzzyWeight_r(inventory, fs->next->child);
			else
				w2 = fs->next->minweight + Q_flrand(0.0f, 1.0f) * (fs->next->maxweight - fs->next->minweight);
			// the scale factor
			if (fs->next->value == MAX_INVENTORYVALUE) // is fs->next the default case?
				return w2;							   // can't interpolate, return default weight
			else
	...
	while (1) {
		if (inventory[s->index] < s->value) {
			if (s->child)
				...
				return s->minweight + Q_flrand(0.0f, 1.0f) * (s->maxweight - s->minweight);
		} // end if
		else {
			if (s->next)
				s = s->next;
...
void EvolveFuzzySeperator_r(fuzzyseperator_t *fs) {
	if (fs->child) {
		...
		if (Q_flrand(0.0f, 1.0f) < 0.01)
			fs->weight += Q_flrand(-1.0f, 1.0f) * (fs->maxweight - fs->minweight);
		else
			fs->weight += Q_flrand(-1.0f, 1.0f) * (fs->maxweight - fs->minweight) * 0.5;
