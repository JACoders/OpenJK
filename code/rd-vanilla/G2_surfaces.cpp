 ...
 *
 ************************************************************************************************/
const mdxmSurface_t *G2_FindSurface(CGhoul2Info *ghlInfo, surfaceInfo_v &slist, const char *surfaceName, int *surfIndex /*NULL*/) {
	int i = 0;
...
qboolean G2_SetSurfaceOnOff(CGhoul2Info *ghlInfo, const char *surfaceName, const int offFlags) {
	...
	if (surf) {
		...
		if (surfaceNum != -1) {
			...
			if (newflags != flags) { // insert here then because it changed, no need to add an override otherwise
