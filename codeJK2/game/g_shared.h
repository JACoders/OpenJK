...
{ TID_CHAN_VOICE = 0, // Waiting for a voice sound to complete
  ...
  TID_ANIM_BOTH,  // Waiting to finish lower and upper anim holdtimes or normal
                  // md3 animating
  ...
  //	TID_MISSIONSTATUS,	// Waiting for player to finish reading MISSION
  //STATUS SCREEN
...
#define MAX_CUSTOM_SOUNDS                                                      \
  (MAX_CUSTOM_JEDI_SOUNDS + MAX_CUSTOM_EXTRA_SOUNDS +                          \
   MAX_CUSTOM_COMBAT_SOUNDS + MAX_CUSTOM_BASIC_SOUNDS)
...
public:
  ...
  vec3_t muzzleDirOld;
  // vec3_t		muzzlePointNext;	// Muzzle point one server frame in the
  // future! vec3_t		muzzleDirNext;
...
public:
  ...
  int otherAttacksCnt; // # of times anything else on a monster was hit with
                       // saber
...
public:
  int missionObjectivesShown; // Number of times mission objectives have been
                              // updated
  team_t sessionTeam;
...
typedef enum {
  BLK_NO,
  BLK_TIGHT, // Block only attacks and shots around the saber itself, a bbox of
             // around 12x12x12
  BLK_WIDE   // Block all attacks in an area around the player in a rough arc of
             // 180 degrees
} saberBlockType_t;
...
public:
  ...
  // Client info - updated when ClientInfoChanged is called, instead of using
  // configstrings
  clientInfo_t clientInfo;
  signed char forced_forwardmove;
  signed char forced_rightmove;
  int fireDelay; // msec to delay calling G_FireWeapon after EV_FIREWEAPON event
                 // is called
  ...
  char dismemberProbLegs;  // probability of the legs being dismembered (located
                           // in NPC.cfg, 0 = never, 100 = always)
  char dismemberProbHead;  // probability of the head being dismembered (located
                           // in NPC.cfg, 0 = never, 100 = always)
  char dismemberProbArms;  // probability of the arms being dismembered (located
                           // in NPC.cfg, 0 = never, 100 = always)
  char dismemberProbHands; // probability of the hands being dismembered
                           // (located in NPC.cfg, 0 = never, 100 = always)
  char dismemberProbWaist; // probability of the waist being dismembered
                           // (located in NPC.cfg, 0 = never, 100 = always)
...
#define MAX_PARMS 16
#define MAX_PARM_STRING_LENGTH                                                 \
  MAX_QPATH // was 16, had to lengthen it so they could take a valid file path
...
struct gentity_s {
  ...
  gclient_t *client; // NULL if not a player (unless it's NPC ( if (this->NPC !=
                     // NULL)  )  <sigh>... -slc)
  qboolean inuse;
  ...
  gentity_t *owner; // objects never interact with their owners, to
                    ...
                    Ghoul2 Insert Start
                    */
  // this marker thing of Jake's is used for memcpy() length calcs, so don't put
  // any ordinary fields (like above)
  //	below this point or they won't work, and will mess up all sorts of
  //stuff.
  //
  ...
  */
  ...
  // note: all the char* fields from here on should be left as ptrs, not
  // declared, because of the way that ent-parsing
  //	works by forcing field offset ptrs as char* and using G_NewString()!!
  //(see G_ParseField() in gmae/g_spawn.cpp -slc
  //
  ...
  char *target;
  char *target2; // For multiple targets, not used for firing/triggering/using,
  ...
  union {
    ...
    qboolean has_bounced; // for thermal Det.  we force at least one bounce to
  ...
  // Force effects
  int forcePushTime;
  int forcePuller; // who force-pulled me (so we don't damage them if we hit
                   // them)
...
centity_t	cg_entities[MAX_GENTITIES]
{
        ...
        {
                gentity_t	owner, nextTrain, prevTrain, chain, enemy,
activator, teamchain, teammaster, team_leader, leader, follower,
formationGoal, lastEnemy entityState_t	s
                {
                        trajectory_t	pos, apos
                }
                gclient_t		client
                {
                        ...
                        clientPersistant_t	pers;
                        {
                                playerTeamState_t teamState;	// status in
teamplay games
                        }
                        clientSession_t		sess
                        usercmd_t	usercmd
