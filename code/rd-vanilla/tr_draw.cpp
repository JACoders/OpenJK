	...
	if (Dissolve.iStartTime) {
		...
		if (iDissolvePercentage <= 100) {
			...
			case eDISSOLVE_LT_TO_RT: {
				float fXboundary =
					(((float)(Dissolve.iWidth + (2 * Dissolve.pDissolve->width)) * (float)iDissolvePercentage) / 100.0f) - Dissolve.pDissolve->width;
			...
			case eDISSOLVE_TP_TO_BT: {
				float fYboundary =
					(((float)(Dissolve.iHeight + (2 * Dissolve.pDissolve->width)) * (float)iDissolvePercentage) / 100.0f) - Dissolve.pDissolve->width;
...
qboolean RE_InitDissolve(qboolean bForceCircularExtroWipe) {
	...
	if ( // Dissolve.iStartTime == 0	// no point in interruping an existing one
		...
		if (pBuffer) {
			...
			//
			byte *pbReSampleBuffer = (iPow2VidWidth == Dissolve.iUploadWidth && iPow2VidHeight == Dissolve.iUploadHeight)
										 ? NULL
