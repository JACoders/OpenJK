...
gentity_t *NPC_AnimalUpdateLeader(void) {
  ...
  if (NPC->client->leader) {
    ...
    if (NPC->client->leader != NPC) {
      ...
      //------------------------------------------------------
      else if (Distance(NPC->client->leader->currentOrigin,
                        NPC->currentOrigin) > LEAVE_PACK_DISTANCE) {
        NPC->client->leader = 0;
      }
    }
...
void NPC_BSAnimal_Default(void) {
  ...
  int alertEvent =
      NPC_CheckAlertEvents(qtrue, qtrue, -1, qfalse, AEL_MINOR, qfalse);
  if (alertEvent >= 0) {
    alertEvent_t *event = &level.alertEvents[alertEvent];
    if (event->owner != NPC &&
        Distance(event->position, CurrentLocation.v) < event->radius) {
  ...
  //--------------------------------------------------------------------------
  /*	if (false && !EvadeThreat && PlayerSpawned &&
     (DistToThreat<FRIGHTEN_DISTANCE))
          {
                  ...
                  {
                          EvadeThreat = true;
                          NPCInfo->investigateSoundDebounceTime = level.time +
     Q_irand(0, 1000); VectorCopy(ThreatLocation.v, NPCInfo->investigateGoal);
                  }
          }*/
  ...
  STEER::Activate(NPC);
  {
    ...
    //-----------------
      ...
      if (leader && leader != NPC) {
        ...
        bool onNbrPoints = (NAV::OnNeighboringPoints(
            NAV::GetNearestNode(NPC), leader->followPosWaypoint));
        ...
        if (inSeekRange || onNbrPoints) {
          ...
          //----------------------------------------------
            ...
            STEER::Seek(NPC, leader->followPos,
                        fabsf(followDist) / 2.0f /*slowing distance*/,
                        1.0f /*wight*/, leader->resultspeed);
          }
        }
        ...
        if (!HasPath) {
          ...
          if (NPCInfo->investigateDebounceTime < level.time) {
            ...
            if (RandomPathNode) {
              ...
              NAV::FindPath(NPC, NAV::ChooseRandomNeighbor(NAV::GetNearestNode(
                                     NPC))); //, mHome.v, WANDER_RANGE));
            }
            ...
            //-----------------
              ...
              // NPC_SetAnim(NPC, SETANIM_BOTH, ((Q_irand(0,
              // 1)==0)?(BOTH_GUARD_LOOKAROUND1):(BOTH_GUARD_IDLE1)),
              // SETANIM_FLAG_NORMAL);
            }
          }

          // Ok, So We Don't Have A Path, And Debounce Time Is Still Active, So
          // We Are Either Wandering Or Looking Around
          //--------------------------------------------------------------------------------------------------------------
          else {
