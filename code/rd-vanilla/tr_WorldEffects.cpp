...
#define COUTSIDE_STRUCT_VERSION 1 // you MUST increase this any time you change any binary (fields) inside this class, or cahced files will fuck up
  ...
  public:
	...
	const char *GenCachedWeatherFilename(void) { return va("maps/%s.weather", sv_mapname->string); }
...
void R_AddWeatherZone(vec3_t mins, vec3_t maxs) { mOutside.AddWeatherZone(mins, maxs); }

bool R_IsOutside(vec3_t pos) { return mOutside.PointOutside(pos); }

bool R_IsShaking(vec3_t pos) { return (mOutside.mOutsideShake && mOutside.PointOutside(pos)); }

float R_IsOutsideCausingPain(vec3_t pos) { return (mOutside.mOutsidePain && mOutside.PointOutside(pos)); }
	...
	if (tr.world && tr.world->globalFog != -1) {
		...
		if (color[0] || color[1] || color[2]) {
			...
			tr.world->fogs[tr.world->globalFog].colorInt =
				ColorBytes4(color[0] * tr.identityLight, color[1] * tr.identityLight, color[2] * tr.identityLight, 1.0);
		}
	...
	void Render() {
		...
		for (particleNum = 0; particleNum < mParticleCount; particleNum++) {
			...
			if (mVertexCount == 3) {
				...
				qglVertex3f(part->mPosition[0] + mCameraLeft[0], part->mPosition[1] + mCameraLeft[1], part->mPosition[2] + mCameraLeft[2]);

				qglTexCoord2f(0.0, 0.0);
				qglVertex3f(part->mPosition[0] + mCameraLeftPlusUp[0], part->mPosition[1] + mCameraLeftPlusUp[1], part->mPosition[2] + mCameraLeftPlusUp[2]);
			}
			...
			//---------------
				...
				// Left bottom.
				qglTexCoord2f(0.0, 0.0);
				qglVertex3f(part->mPosition[0] - mCameraLeftMinusUp[0], part->mPosition[1] - mCameraLeftMinusUp[1], part->mPosition[2] - mCameraLeftMinusUp[2]);
...
void RB_RenderWorldEffects(void) {
	if (!tr.world || (tr.refdef.rdflags & RDF_NOWORLDMODEL) || (backEnd.refdef.rdflags & RDF_SKYBOXPORTAL) || !mParticleClouds.size() ||
		ri.CL_IsRunningInGameCinematic()) { //  no world rendering or no world or no particle clouds
		return;
	}
