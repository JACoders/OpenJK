...
normally.  Note that you cannot use the door again after this. PLAYER_USE	Player can use it with the use button INACTIVE	must be used by a target_activate
...
normal), max is 1000 (ten times normal). "color"		constantLight color "light"		constantLight radius "dmg"		how much damage to do when it crushes
(use with CRUSHER spawnflag) "linear" set to 1 and it will move linearly rather than with acceleration (default is 0)
*/
...
"model2"		.md3 model to also draw
"model2scale"	percent of normal scale (on all x y and z axii) to scale the model2 if there is one. 100 is normal scale, min is 1 (100 times smaller than
normal), max is 1000 (ten times normal). "speed"			determines how fast it moves; default value is 100. "dmg"			damage to inflict when blocked
(2 default) "color"			constantLight color "light"			constantLight radius "spinangles"	instead of using "speed", you can set use this to set
rotation on all 3 axes (pitch yaw and roll)
...
void G_Chunks(int owner, vec3_t origin, const vec3_t normal, const vec3_t mins, const vec3_t maxs, float speed, int numChunks, material_t chunkType,
...
"numchunks" Multiplies the number of chunks spawned.  Chunk code tries to pick a good volume of chunks, but you can alter this to scale the number of spawned
chunks. (default 1)  (.5) is half as many chunks, (2) is twice as many chunks "chunksize"	scales up/down the chunk size by this number (default is 1) "playfx"
path of effect to play on death
