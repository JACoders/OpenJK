...
gentity_t *G_TestEntityPosition(gentity_t *ent) {
	...
	if ((ent->client && ent->health <= 0) || !ent->clipmask) { // corpse or something with no clipmask
		mask = MASK_SOLID;
	} else {
		mask = ent->clipmask;
	}
	if (ent->client) {
		...
		if (ent->s.eFlags & EF_MISSILE_STICK) // Arggh, this is dumb...but when it used the bbox, it was pretty much always in solid when it is riding
											  // something..which is wrong..so I changed it to basically be a point contents check
		{
	...
	for (e = 0; e < listedEntities; e++) {
		...
		if (check->s.eType != ET_ITEM) {
			...
			*/
				...
				if (G_BoundsOverlap(checkMins, checkMaxs, pusherMins, pusherMaxs)) { // They're inside me already, no push - FIXME: we're testing a moves spot,
																					 // aren't we, so we could have just moved inside them?
					continue;
				}
			}
		}

		if (check->maxs[0] - check->mins[0] <= 0 && check->maxs[1] - check->mins[1] <= 0 && check->maxs[2] - check->mins[2] <= 0) { // no size, don't push
			continue;
		}
		...
		if (check->s.groundEntityNum != pusher->s.number) {
			...
			if (check->absmin[0] >= maxs[0] || check->absmin[1] >= maxs[1] || check->absmin[2] >= maxs[2] || check->absmax[0] <= mins[0] ||
		...
		if (((pusher->spawnflags & 2) && !Q_stricmp("func_breakable", pusher->classname)) ||
			((pusher->spawnflags & 16) && !Q_stricmp("func_static", pusher->classname))) { // ugh, avoid stricmp with a unique flag
			// Damage on impact
			if (pusher->damage) { // Do damage
				G_Damage(check, pusher, pusher->activator, move, check->currentOrigin, pusher->damage, 0, MOD_CRUSH);
				if (pusher->health >= 0 && pusher->takedamage && !(pusher->spawnflags & 1)) { // do some damage to me, too
...
void G_MoverTeam(gentity_t *ent) {
	...
	if (part) {
		...
		if (ent->e_BlockedFunc != blockedF_NULL) { // this check no longer necessary, done internally below, but it's here for reference
...
void Use_BinaryMover(gentity_t *ent, gentity_t *other, gentity_t *activator) {
	...
	if (ent->e_UseFunc == useF_NULL) { // I cannot be used anymore, must be a door with a wait of -1 that's opened.
		return;
	}
...
void Touch_DoorTrigger(gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	if (ent->owner->spawnflags & MOVER_LOCKED) { // don't even try to use the door if it's locked
		return;
	}
...
qboolean G_EntIsUnlockedDoor(int entityNum) {
	...
	if (G_EntIsDoor(entityNum)) {
		...
		if (ent->targetname) { // find out what is targetting it
			...
			if (owner && (owner->svFlags & SVF_INACTIVE)) { // owning auto-created trigger is inactive
				return qfalse;
			}
		}
...
LOCKED		Starts locked, with the shader animmap at the first frame and inactive.  Once used, the animmap changes to the second frame and the door operates
normally.  Note that you cannot use the door again after this. GOODIE		Door will not work unless activator has a "goodie key" in his inventory PLAYER_USE
Player can use it with the use button INACTIVE	must be used by a target_activate before it can be used
...
void Reached_Train(gentity_t *ent) {
	...
	*/
	...
	// This is for the tie fighter shooting gallery on doom detention, you could see them waiting under the bay, but the architecture couldn't easily be
	// changed..
...
void TrainUse(gentity_t *ent, gentity_t *other, gentity_t *activator) { Reached_Train(ent); }
...
void SP_func_train(gentity_t *self) {
	...
	if (self->playerModel >= 0 &&
		self->spawnflags & 32) // loop...dunno, could support it on other things, but for now I need it for the glider...so...kill the flag
	{
		self->spawnflags &= ~32; // once only

		gi.G2API_SetBoneAnim(&self->ghoul2[self->playerModel], "model_root", self->startFrame, self->endFrame, BONE_ANIM_OVERRIDE_LOOP,
