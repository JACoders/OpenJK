...
static size_t GLSL_GetShaderHeader(GLenum shaderType, const GLcharARB *extra, int firstLineNumber, char *dest, size_t size) {
	...
	Q_strcat(dest, size,
			 va("#ifndef deformGen_t\n"
				...
				"#endif\n",
				DEFORM_NONE, DEFORM_WAVE, DEFORM_NORMALS, DEFORM_BULGE, DEFORM_BULGE_UNIFORM, DEFORM_MOVE, DEFORM_PROJECTION_SHADOW, DEFORM_DISINTEGRATION,
				GF_NONE, GF_SIN, GF_SQUARE, GF_TRIANGLE, GF_SAWTOOTH, GF_INVERSE_SAWTOOTH));
	...
	Q_strcat(dest, size,
			 va("#ifndef tcGen_t\n"
				...
				"#endif\n",
				TCGEN_LIGHTMAP, TCGEN_LIGHTMAP1, TCGEN_LIGHTMAP2, TCGEN_LIGHTMAP3, TCGEN_TEXTURE, TCGEN_ENVIRONMENT_MAPPED, TCGEN_FOG, TCGEN_VECTOR));
	...
	Q_strcat(dest, size,
			 va("#ifndef colorGen_t\n"
				...
				"#endif\n",
				CGEN_LIGHTING_DIFFUSE, CGEN_DISINTEGRATION_1, CGEN_DISINTEGRATION_2));
	...
	Q_strcat(dest, size,
			 va("#define ALPHA_TEST_GT0 %d\n"
				...
				ALPHA_TEST_GT0, ALPHA_TEST_LT128, ALPHA_TEST_GE128, ALPHA_TEST_GE192));
...
static GLuint GLSL_CompileGPUShader(GLuint program, const GLchar *buffer, int size, GLenum shaderType) {
...
static size_t GLSL_LoadGPUShaderSource(const char *name, const char *fallback, GLenum shaderType, char *dest, int destSize) {
...
static void GLSL_BindShaderInterface(shaderProgram_t *program) {
	...
	if (xfbVars != 0) {
		...
		qglTransformFeedbackVaryings(program->program, activeXfbVarsCount, activeXfbVarNames, GL_INTERLEAVED_ATTRIBS);
	}
}
...
class ShaderProgramBuilder {
  public:
	...
	void Start(const char *name, const uint32_t attribs, const uint32_t xfbVariables);
...
ShaderProgramBuilder::ShaderProgramBuilder()
	: name(nullptr), attribs(0), program(0), shaderNames(), numShaderNames(0), shaderSource(MAX_SHADER_SOURCE_LEN, '\0') {}
...
void ShaderProgramBuilder::Start(const char *name, const uint32_t attribs, const uint32_t xfbVariables) {
...
bool ShaderProgramBuilder::AddShader(const GPUShaderDesc &shaderDesc, const char *extra) {
	...
	while (sourceLen == 0 && attempts < MAX_ATTEMPTS) {
		headerLen = GLSL_GetShaderHeader(apiShader, extra, shaderDesc.firstLineNumber, &shaderSource[0], shaderSource.size());

		sourceLen = GLSL_LoadGPUShaderSource(name, shaderDesc.source, apiShader, &shaderSource[headerLen], shaderSource.size() - headerLen);
	...
	const GLuint shader = GLSL_CompileGPUShader(program, shaderSource.c_str(), sourceLen + headerLen, apiShader);
	if (shader == 0) {
		ri.Printf(PRINT_ALL, "ShaderProgramBuilder::AddShader: Unable to load \"%s\"\n", name);
		return false;
	}
...
static bool GLSL_LoadGPUShader(ShaderProgramBuilder &builder, shaderProgram_t *program, const char *name, const uint32_t attribs, const uint32_t xfbVariables,
							   const GLcharARB *extra, const GPUProgramDesc &programDesc) {
...
void GLSL_InitUniforms(shaderProgram_t *program) {
	...
	for (int i = 0; i < UNIFORM_BLOCK_COUNT; ++i) {
		const GLuint blockIndex = qglGetUniformBlockIndex(program->program, uniformBlocksInfo[i].name);
		if (blockIndex == GL_INVALID_INDEX)
			continue;
		ri.Printf(PRINT_DEVELOPER, "Binding block %d (name '%s', size %zu bytes) to slot %d\n", blockIndex, uniformBlocksInfo[i].name,
				  uniformBlocksInfo[i].size, uniformBlocksInfo[i].slot);
		qglUniformBlockBinding(program->program, blockIndex, uniformBlocksInfo[i].slot);
	...
	for (int i = 0; i < numActiveUniformBlocks; ++i) {
		char blockName[512];
		qglGetActiveUniformBlockName(program->program, i, sizeof(blockName), nullptr, blockName);

		GLint blockSize = 0;
		qglGetActiveUniformBlockiv(program->program, i, GL_UNIFORM_BLOCK_DATA_SIZE, &blockSize);

		ri.Printf(PRINT_DEVELOPER, "..block %d: %s (%d bytes)\n", i, blockName, blockSize);
		GLint numMembers = 0;
		qglGetActiveUniformBlockiv(program->program, i, GL_UNIFORM_BLOCK_ACTIVE_UNIFORMS, &numMembers);

		if (numMembers > 0) {
			GLuint memberIndices[128];
			qglGetActiveUniformBlockiv(program->program, i, GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES, (GLint *)memberIndices);

			GLint memberOffsets[128];
			qglGetActiveUniformsiv(program->program, numMembers, memberIndices, GL_UNIFORM_OFFSET, memberOffsets);

			GLint memberTypes[128];
			qglGetActiveUniformsiv(program->program, numMembers, memberIndices, GL_UNIFORM_TYPE, memberTypes);

			for (int j = 0; j < numMembers; ++j) {
				char memberName[512];
				qglGetActiveUniformName(program->program, memberIndices[j], sizeof(memberName), nullptr, memberName);
...
#ifdef REND2_SP
	if ((shaderCaps & GENERICDEF_USE_VERTEX_ANIMATION) && (shaderCaps & GENERICDEF_USE_SKELETAL_ANIMATION))
		return false;
#endif // REND2_SP
	return true;
}
...
#ifdef REND2_SP
	if ((shaderCaps & LIGHTDEF_USE_SKELETAL_ANIMATION) && (shaderCaps & LIGHTDEF_USE_VERTEX_ANIMATION))
		return false;
#endif // REND2_SP
	return true;
}
...
static const GPUProgramDesc *LoadProgramSource(const char *programName, Allocator &allocator, const GPUProgramDesc &fallback) {
...
static int GLSL_LoadGPUProgramGeneric(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	int numPrograms = 0;
	Allocator allocator(scratchAlloc.Base(), scratchAlloc.GetSize());

	char extradefines[1200];
	const GPUProgramDesc *programDesc = LoadProgramSource("generic", allocator, fallback_genericProgram);
...
static int GLSL_LoadGPUProgramFogPass(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	int numPrograms = 0;
	Allocator allocator(scratchAlloc.Base(), scratchAlloc.GetSize());

	char extradefines[1200];
	const GPUProgramDesc *programDesc = LoadProgramSource("fogpass", allocator, fallback_fogpassProgram);
...
static int GLSL_LoadGPUProgramRefraction(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	int numPrograms = 0;
	Allocator allocator(scratchAlloc.Base(), scratchAlloc.GetSize());

	char extradefines[1200];
	const GPUProgramDesc *programDesc = LoadProgramSource("refraction", allocator, fallback_refractionProgram);
...
static int GLSL_LoadGPUProgramLightAll(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	int numPrograms = 0;
	Allocator allocator(scratchAlloc.Base(), scratchAlloc.GetSize());

	char extradefines[1200];
	const GPUProgramDesc *programDesc = LoadProgramSource("lightall", allocator, fallback_lightallProgram);
	const bool useFastLight = (!r_normalMapping->integer && !r_specularMapping->integer);
	for (int i = 0; i < LIGHTDEF_COUNT; i++) {
		...
		if (!GLSL_LoadGPUShader(builder, &tr.lightallShader[i], "lightall", attribs, NO_XFB_VARS, extradefines, *programDesc)) {
...
static int GLSL_LoadGPUProgramBasicWithDefinitions(ShaderProgramBuilder &builder, Allocator &scratchAlloc, shaderProgram_t *shaderProgram,
												   const char *programName, const GPUProgramDesc &programFallback, const char *extraDefines,
												   const uint32_t attribs = ATTR_POSITION | ATTR_TEXCOORD0, const uint32_t xfbVariables = NO_XFB_VARS) {
	Allocator allocator(scratchAlloc.Base(), scratchAlloc.GetSize());

	const GPUProgramDesc *programDesc = LoadProgramSource(programName, allocator, programFallback);
	if (!GLSL_LoadGPUShader(builder, shaderProgram, programName, attribs, xfbVariables, extraDefines, *programDesc)) {
...
static int GLSL_LoadGPUProgramBasic(ShaderProgramBuilder &builder, Allocator &scratchAlloc, shaderProgram_t *shaderProgram, const char *programName,
									const GPUProgramDesc &programFallback, const uint32_t attribs = ATTR_POSITION | ATTR_TEXCOORD0,
									const uint32_t xfbVariables = NO_XFB_VARS) {
	return GLSL_LoadGPUProgramBasicWithDefinitions(builder, scratchAlloc, shaderProgram, programName, programFallback, nullptr, attribs, xfbVariables);
}

static int GLSL_LoadGPUProgramTextureColor(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	GLSL_LoadGPUProgramBasic(builder, scratchAlloc, &tr.textureColorShader, "texturecolor", fallback_texturecolorProgram);
...
static int GLSL_LoadGPUProgramPShadow(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	const char *extradefines = "#define USE_PCF\n#define USE_DISCARD\n";

	GLSL_LoadGPUProgramBasicWithDefinitions(builder, scratchAlloc, &tr.pshadowShader, "pshadow", fallback_pshadowProgram, extradefines,
...
static int GLSL_LoadGPUProgramVShadow(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	Allocator allocator(scratchAlloc.Base(), scratchAlloc.GetSize());

	char extradefines[1200];
	const GPUProgramDesc *programDesc = LoadProgramSource("shadowvolume", allocator, fallback_shadowvolumeProgram);
...
static int GLSL_LoadGPUProgramDownscale4x(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	GLSL_LoadGPUProgramBasic(builder, scratchAlloc, &tr.down4xShader, "down4x", fallback_down4xProgram);
...
static int GLSL_LoadGPUProgramBokeh(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	GLSL_LoadGPUProgramBasic(builder, scratchAlloc, &tr.bokehShader, "bokeh", fallback_bokehProgram);
...
static int GLSL_LoadGPUProgramTonemap(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	Allocator allocator(scratchAlloc.Base(), scratchAlloc.GetSize());
	char extradefines[1200];
	const GPUProgramDesc *programDesc = LoadProgramSource("tonemap", allocator, fallback_tonemapProgram);
...
static int GLSL_LoadGPUProgramCalcLuminanceLevel(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	int numPrograms = 0;
	Allocator allocator(scratchAlloc.Base(), scratchAlloc.GetSize());

	char extradefines[1200];
	const GPUProgramDesc *programDesc = LoadProgramSource("calclevels4x", allocator, fallback_calclevels4xProgram);
	for (int i = 0; i < 2; i++) {
		const uint32_t attribs = ATTR_POSITION | ATTR_TEXCOORD0;
		extradefines[0] = '\0';
...
static int GLSL_LoadGPUProgramSSAO(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	GLSL_LoadGPUProgramBasic(builder, scratchAlloc, &tr.ssaoShader, "ssao", fallback_ssaoProgram);
...
static int GLSL_LoadGPUProgramPrefilterEnvMap(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	GLSL_LoadGPUProgramBasic(builder, scratchAlloc, &tr.prefilterEnvMapShader, "prefilterEnvMap", fallback_prefilterEnvMapProgram);
...
static int GLSL_LoadGPUProgramDepthBlur(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	int numPrograms = 0;
	Allocator allocator(scratchAlloc.Base(), scratchAlloc.GetSize());

	char extradefines[1200];
	const GPUProgramDesc *programDesc = LoadProgramSource("depthBlur", allocator, fallback_depthblurProgram);
	for (int i = 0; i < 2; i++) {
		const uint32_t attribs = ATTR_POSITION | ATTR_TEXCOORD0;
		extradefines[0] = '\0';
...
static int GLSL_LoadGPUProgramGaussianBlur(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	Allocator allocator(scratchAlloc.Base(), scratchAlloc.GetSize());

	char extradefines[1200];
	const GPUProgramDesc *programDesc = LoadProgramSource("gaussian_blur", allocator, fallback_gaussian_blurProgram);
	...
	if (!GLSL_LoadGPUShader(builder, &tr.gaussianBlurShader[1], "gaussian_blur", attribs, NO_XFB_VARS, nullptr, *programDesc)) {
...
static int GLSL_LoadGPUProgramDynamicGlowUpsample(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	GLSL_LoadGPUProgramBasic(builder, scratchAlloc, &tr.dglowUpsample, "dglow_upsample", fallback_dglow_upsampleProgram, 0);
...
static int GLSL_LoadGPUProgramDynamicGlowDownsample(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	GLSL_LoadGPUProgramBasic(builder, scratchAlloc, &tr.dglowDownsample, "dglow_downsample", fallback_dglow_downsampleProgram, 0);
...
static int GLSL_LoadGPUProgramSurfaceSprites(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	int numPrograms = 0;
	Allocator allocator(scratchAlloc.Base(), scratchAlloc.GetSize());

	char extradefines[1200];
	const GPUProgramDesc *programDesc = LoadProgramSource("surface_sprites", allocator, fallback_surface_spritesProgram);
	...
	for (int i = 0; i < SSDEF_COUNT; ++i) {
		...
		if (i & SSDEF_FACE_CAMERA)
			Q_strcat(extradefines, sizeof(extradefines), "#define FACE_CAMERA\n");
		else if (i & SSDEF_FACE_UP)
			Q_strcat(extradefines, sizeof(extradefines), "#define FACE_UP\n");
		...
		if (i & SSDEF_ALPHA_TEST)
			Q_strcat(extradefines, sizeof(extradefines), "#define ALPHA_TEST\n");
...
static int GLSL_LoadGPUProgramWeather(ShaderProgramBuilder &builder, Allocator &scratchAlloc) {
	GLSL_LoadGPUProgramBasic(builder, scratchAlloc, &tr.weatherShader, "weather", fallback_weatherProgram, ATTR_POSITION | ATTR_COLOR);
	...
	GLSL_LoadGPUProgramBasic(builder, scratchAlloc, &tr.weatherUpdateShader, "weatherUpdate", fallback_weatherUpdateProgram, ATTR_POSITION | ATTR_COLOR,
...
void GLSL_LoadGPUShaders() {
	...
	ri.Printf(PRINT_ALL, "loaded %i GLSL shaders (%i gen %i light %i etc) in %5.2f seconds\n", numGenShaders + numLightShaders + numEtcShaders, numGenShaders,
			  numLightShaders, numEtcShaders, (ri.Milliseconds() - startTime) / 1000.0);
}
...
shaderProgram_t *GLSL_GetGenericShaderProgram(int stage) {
	...
	if (tess.fogNum && pStage->adjustColorsForFog != ACFF_NONE && r_drawfog->integer)
