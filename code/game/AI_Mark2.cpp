...
void NPC_Mark2_Part_Explode(gentity_t *self, int bolt) {
  if (bolt >= 0) {
    ...
    G_PlayEffect(G_EffectIndex("blaster/smoke_bolton"), self->playerModel, bolt,
                 self->s.number, org);
  }
...
-------------------------
*/
void NPC_Mark2_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
  ...
  for (i = 0; i < 3; i++) {
    if ((hitLoc == HL_GENERIC1 + i) && (self->locationDamage[HL_GENERIC1 + i] >
  ...
  if (self->count > 0) {
    G_Damage(self, NULL, NULL, NULL, NULL, self->health, DAMAGE_NO_PROTECTION,
             MOD_UNKNOWN);
  }
}
...
void Mark2_FireBlaster(qboolean advance) {
  ...
  gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->playerModel, NPC->genericBolt1,
...
void Mark2_AttackDecision(void) {
  ...
  if (NPCInfo->localState == LSTATE_RISINGUP) {
    ...
    NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_RUN1START,
  ...
  if ((!visible) || (!NPC_FaceEnemy(qtrue))) {
    ...
    if ((NPCInfo->localState == LSTATE_DOWN) ||
        (NPCInfo->localState == LSTATE_DROPPINGDOWN)) {
      if (TIMER_Done(NPC, "downTime")) // Down being down?? (The delay is so he
                                       // doesn't pop up and down when the
                                       // player goes in and out of range)
      {
        NPCInfo->localState = LSTATE_RISINGUP;
        NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_RUN1STOP,
                    SETANIM_FLAG_HOLD | SETANIM_FLAG_OVERRIDE);
        TIMER_Set(NPC, "runTime",
                  Q_irand(3000, 8000)); // So he runs for a while before testing
                                        // to see if he should drop down.
      }
    } else {
      Mark2_Hunt();
    }
  ...
  if ((advance) && (TIMER_Done(NPC, "downTime")) &&
      (NPCInfo->localState == LSTATE_DOWN)) {
    NPCInfo->localState = LSTATE_RISINGUP;
    NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_RUN1STOP,
                SETANIM_FLAG_HOLD | SETANIM_FLAG_OVERRIDE);
    TIMER_Set(NPC, "runTime",
              Q_irand(3000, 8000)); // So he runs for a while before testing to
                                    // see if he should drop down.
  }
  ...
  // He's down and shooting
    ...
    //		NPC->flags |= FL_SHIELDED;//only damagable by lightsabers and
    //missiles

    Mark2_BlasterAttack(qfalse);
  } else if (TIMER_Done(NPC, "runTime")) // Lowering down to attack. But only if
                                         // he's done running at you.
  {
    NPCInfo->localState = LSTATE_DROPPINGDOWN;
  } else if (advance) {
