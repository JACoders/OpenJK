...
// The Troop
//
// Troopers primarly derive their behavior from cooperation as a collective
// group of individuals.  They join Troops, each of which has a leader
// responsible for direcing the movement of the rest of the group.
//
////////////////////////////////////////////////////////////////////////////////////////
class CTroop {
  ////////////////////////////////////////////////////////////////////////////////////
...
public:
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  // DistanceSq - Quick Operation to see how far an ent is from the rest of the
  // troop
  ////////////////////////////////////////////////////////////////////////////////////
...
private:
  ...
  // The Actors
  //
  // Actors are all the troopers who belong to the group, their positions in
  // this vector affect their positions in the troop, whith the first actor as
  // the leader
  ////////////////////////////////////////////////////////////////////////////////////
...
public:
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  // RemoveActor - Removes an actor from the troop & automatically promote
  // leader
  ////////////////////////////////////////////////////////////////////////////////////
...
private:
  ...
  // Enemy
  //
  // The troop has a collective enemy that it knows about, which is updated by
  ...
  bool TargetLastKnownPositionVisited() {
    if (!mTargetLastKnownPositionVisited) {
      float dist = DistanceSquared(mTargetLastKnownPosition.v,
  ...
  float TargetVisibility(gentity_t *target) {
    float Scale = 0.8f;
    if (target->client && target->client->ps.weapon == WP_SABER &&
  ...
  void ScanForTarget(int scannerIndex) {
    ...
    for (; targetIndex < targetStop; targetIndex++) {
      ...
      if (targetDistance < scannerMaxHearDist) {
        ...
        targetNoiseLevel *=
            (1.0f - (targetDistance / scannerMaxHearDist)); // scale by distance
...
private:
  ...
  // Troop State
  //
  // The troop as a whole can be acting under a number of different "behavior
  // states"
  ////////////////////////////////////////////////////////////////////////////////////
  enum ETroopState {
    ...
    TS_ADVANCE_COVER,   // One at a time moves forward, goes off path, provides
                        // cover
    ...
    TS_ATTACK_FLANK,    // Same As Line, except scouting group attemts to get
                        // around other side of target
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  // TroopInFormation - A quick check to see if the troop is currently in
  // formation
  ////////////////////////////////////////////////////////////////////////////////////
  ...
  void LeaderIssueAndUpdateOrders(ETroopState NextState) {
    ...
    for (int orderIndex = 1; orderIndex < actorCount; orderIndex++) {
      ...
      if (mOrders[orderIndex].mCombatPoint == -1) {
        ...
        float closestActorDistance =
            DistanceSquared(mOrders[orderIndex].mPosition.v,
                            mActors[orderIndex]->currentOrigin);
        float currentDistance = closestActorDistance;
        for (actorIndex = orderIndex + 1; actorIndex < actorCount;
             actorIndex++) {
          currentDistance = DistanceSquared(mOrders[orderIndex].mPosition.v,
    ...
    mFormFwd = (NAV::HasPath(leader)) ? (NAV::NextPosition(leader))
                                      : (mTargetLastKnownPosition);
    mFormFwd -= mFormHead;
    mFormFwd[2] = 0;
    ...
    if (NextState > TS_ATTACK) {
      ...
      gi.trace(&trace, mActors[0]->currentOrigin, mActors[0]->mins,
               mActors[0]->maxs, mOrders[0].mPosition.v, mActors[0]->s.number,
               mActors[0]->clipmask, (EG2_Collision)0, 0);
    ...
    //		float		WidestAngle = FORMATION_SURROUND_FAN *
    //(NumPerSide+1);
    ...
    for (actorIndex = 1; actorIndex < actorCount; actorIndex++) {
      ...
      //-----------------------------
        ...
        if (NextState == TS_ATTACK_LINE ||
            (NextState == TS_ATTACK_FLANK && actorIndex < 4)) {
          Order.mPosition.ScaleAdd(mFormFwd, FwdScale);
          Order.mPosition.ScaleAdd(mFormRight, SideScale);
        } else if (NextState == TS_ATTACK_FLANK && actorIndex >= 4) {
          int cpFlags = (CP_HAS_ROUTE | CP_AVOID_ENEMY | CP_CLEAR | CP_COVER |
                         CP_FLANK | CP_APPROACH_ENEMY);
          float avoidDist = 128.0f;

          Order.mCombatPoint = NPC_FindCombatPointRetry(
              mActors[actorIndex]->currentOrigin,
              mActors[actorIndex]->currentOrigin,
              mActors[actorIndex]->currentOrigin, &cpFlags, avoidDist, 0);
          ...
          /*					CVec3	FanAngles =
             BaseAngleToHead; FanAngles[YAW] += (SideScale *
             (WidestAngle-(FwdScale*FORMATION_SURROUND_FAN)));
                                                  FanAngles.AngToVec();

                                                  Order.mPosition =
             mTargetLastKnownPosition; Order.mPosition.ScaleAdd(FanAngles,
             FormTgtToHeadDist);
          */
      ...
      if (NextState >= TS_ATTACK) {
        ...
        gi.trace(&trace, Order.mPosition.v, mActors[actorIndex]->mins,
                 mActors[actorIndex]->maxs, OrderUp.v,
                 mActors[actorIndex]->s.number,
                 CONTENTS_SOLID | CONTENTS_TERRAIN | CONTENTS_MONSTERCLIP |
                     CONTENTS_BOTCLIP,
                 (EG2_Collision)0, 0);
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  // SufficientCoverNearby - Look at nearby combat points, see if there is
  // enough
  ////////////////////////////////////////////////////////////////////////////////////
...
public:
  ...
  void Update() {
    ...
    if (mTarget) {
      ...
      //	int			TimeVisable			=
      //(mTargetVisableStopTime - mTargetVisableStartTime);
      ...
      if (Attack) {
        ...
        if (mState < TS_ATTACK /*|| TODO: Timer To Pick New Tactic */) {
          if (TroopInFormation()) {
            ...
            NextState = (SufficientCoverNearby()) ? (TS_ATTACK_COVER)
                                                  : (TS_ATTACK_SURROUND);
          }
        }
      } else {
        if (!TroopInFormation()) {
          ...
          if (TargetLastKnownPositionVisited()) {
            ...
            NextState = (TimeSinceLastSeen < 10000) ? (TS_ADVANCE_COVER)
  ...
  //
  ////////////////////////////////////////////////////////////////////////////////////
  gentity_t *TrackingTarget() { return mTarget; }

  ////////////////////////////////////////////////////////////////////////////////////
  //
  ////////////////////////////////////////////////////////////////////////////////////
  gentity_t *TroopLeader() { return mActors[0]; }

  ////////////////////////////////////////////////////////////////////////////////////
  //
  ////////////////////////////////////////////////////////////////////////////////////
  int TimeSinceSeenTarget() { return (level.time - mTargetVisableStopTime); }

  ////////////////////////////////////////////////////////////////////////////////////
  //
  ////////////////////////////////////////////////////////////////////////////////////
  CVec3 &TargetVisablePosition() { return mTargetVisablePosition; }

  ////////////////////////////////////////////////////////////////////////////////////
  //
  ////////////////////////////////////////////////////////////////////////////////////
  float FormSpacingFwd() { return mFormSpacingFwd; }
  ...
  gentity_t *TooCloseToTroopMember(gentity_t *actor) {
    for (int i = 0; i < mActors.size(); i++) {
      ...
      if (i == 0) {
        if (Distance(actor->currentOrigin, mActors[i]->currentOrigin) <
            (mFormSpacingFwd * 0.5f)) {
          return mActors[i];
        }
      } else {
        if (Distance(actor->currentOrigin, mActors[i]->currentOrigin) <
...
void Trooper_UpdateTroop(gentity_t *actor) {
  ...
  if (!actor->NPC->troop) {
    ...
    for (TTroopPool::iterator iTroop = mTroops.begin(); iTroop != mTroops.end();
         ++iTroop) {
      if (iTroop->Team() == actor->client->playerTeam) {
        curDist = iTroop->DistanceSq(actor);
        if (curDist < MAX_TROOP_JOIN_DIST2 &&
            (!closestDist || curDist < closestDist)) {
          // Only Join A Troop If You Can See The Leader
          //---------------------------------------------
          gi.trace(&trace, actor->currentOrigin, actor->mins, actor->maxs,
                   iTroop->TroopLeader()->currentOrigin, actor->s.number,
                   CONTENTS_SOLID | CONTENTS_TERRAIN | CONTENTS_MONSTERCLIP |
                       CONTENTS_BOTCLIP,
                   (EG2_Collision)0, 0);
          ...
          if (!trace.allsolid && !trace.startsolid &&
              (trace.fraction >= 1.0f ||
               trace.entityNum == iTroop->TroopLeader()->s.number)) {
            closestDist = curDist;
            closestTroop = iTroop;
          }
        }
...
void Trooper_SmackAway(gentity_t *actor, gentity_t *target) {
  ...
  if (actor->client->ps.legsAnim != BOTH_MELEE1) {
    NPC_SetAnim(actor, SETANIM_BOTH, BOTH_MELEE1,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    TIMER_Set(actor, "Trooper_SmackAway",
              actor->client->ps.torsoAnimTimer / 4.0f);
  }
}
...
bool Trooper_Kneeling(gentity_t *actor) {
  return (actor->NPC->aiFlags & NPCAI_KNEEL ||
          actor->client->ps.legsAnim == BOTH_STAND_TO_KNEEL);
}
...
void Trooper_KneelDown(gentity_t *actor) {
  ...
  if (!Trooper_Kneeling(actor) && level.time > actor->NPC->kneelTime) {
    NPC_SetAnim(actor, SETANIM_BOTH, BOTH_STAND_TO_KNEEL,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void Trooper_StandUp(gentity_t *actor, bool always = false) {
  ...
  if (Trooper_Kneeling(actor) &&
      (always || level.time > actor->NPC->kneelTime)) {
    actor->NPC->aiFlags &= ~NPCAI_KNEEL;
    NPC_SetAnim(actor, SETANIM_BOTH, BOTH_KNEEL_TO_STAND,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    actor->NPC->kneelTime = level.time + Q_irand(3000, 6000);
  }
}
...
//
////////////////////////////////////////////////////////////////////////////////////////
int Trooper_CanHitTarget(gentity_t *actor, gentity_t *target, CTroop &troop,
                         float &MuzzleToTargetDistance, CVec3 &MuzzleToTarget) {
  trace_t tr;
  ...
  if (MuzzleDirection.Dot(MuzzleToTarget) > 0.95) {
    ...
    //--------------------------------
    gi.trace(&tr, MuzzlePoint.v, NULL, NULL, troop.TargetVisablePosition().v,
...
void Trooper_Think(gentity_t *actor) {
  gentity_t *target =
      (actor->NPC->troop) ? (mTroops[actor->NPC->troop].TrackingTarget()) : (0);
  if (target) {
    ...
    if (actor->NPC->combatPoint != -1) {
      traceTgt = Trooper_CanHitTarget(actor, target, troop,
                                      MuzzleToTargetDistance, MuzzleToTarget);
      traced = true;
    ...
    if (AtPos && (troop.TimeSinceSeenTarget() < 1500)) {
      ...
      if (traceTgt == target->s.number) {
        ...
        //------------------------------------------------
        else if (traceTgt != ENTITYNUM_NONE && traceTgt != ENTITYNUM_WORLD &&
                 g_entities[traceTgt].client && g_entities[traceTgt].NPC &&
                 g_entities[traceTgt].client->playerTeam ==
                     actor->client->playerTeam &&
...
////////////////////////////////////////////////////////////////////////////////////////
bool NPC_IsTrooper(gentity_t *actor) {
  return (actor && actor->NPC && actor->s.weapon &&
