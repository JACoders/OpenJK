...
// If a cylinder has a tapered end that has a very small radius, the engine converts it to a cone.  Not a huge savings, but the texture mapping is slightly
// better
...
//-------------------------------------
{
	...
	for (i = 0; i < segments; i++) {
		...
		tess.texCoords[tess.numVertexes][0][0] =
			detail * i + detail * 0.5f; // set the texture coordinates to the point half-way between the untapered ends....but on the other end of the texture
...
//----------------------------------------------------------------------------
{
	VectorSet(sh1, 0.66f, // + Q_flrand(-1.0f, 1.0f) * 0.1f,	// fwd
			  0.08f + Q_flrand(-1.0f, 1.0f) * 0.02f, 0.08f + Q_flrand(-1.0f, 1.0f) * 0.02f);
...
static void RB_SurfaceClouds() {
	...
	for (i = 0; i < ct - 1; i++) {
		...
		for (t = latheStep; t <= 360; t += latheStep) {
			...
			tess.vertexColors[tess.numVertexes][0] = tess.vertexColors[tess.numVertexes][1] = tess.vertexColors[tess.numVertexes][2] =
...
static bool RB_TestZFlare(vec3_t point) {
	...
	R_TransformModelToClip(point, backEnd.ori.modelMatrix, backEnd.viewParms.projectionMatrix, eye, clip);
