...
void CG_ParseServerinfo(void) {
	...
	if (cgs.wDisable != value) {
		...
		for (i = 1, item = bg_itemlist, itemInfo = cg_items; i < bg_numItems; i++, item++, itemInfo++) { // register all weapons that aren't disabled
...
static char ctfFlagStatusRemap[] = {FLAG_ATBASE,
									FLAG_TAKEN, // CTF
...
static void CG_ConfigStringModified(void) {
	...
	if (num == CS_MUSIC) {
		...
		if (strstr(modelName, ".glm") || modelName[0] == '$') { // Check to see if it has a custom skin attached.
			CG_HandleAppendedSkin(modelName);
			CG_CacheG2AnimInfo(modelName);
		}

		if (modelName[0] != '$' && modelName[0] != '@') { // don't register vehicle names and saber names as models.
...
void CG_KillCEntityG2(int entNum) {
	...
	if (ci) {
		if (ci == cent->npcClient) { // never going to be != cent->ghoul2, unless cent->ghoul2 has already been removed (and then this ptr is not valid)
			ci->ghoul2Model = NULL;
...
void CG_KillCEntityInstances(void) {
	...
	while (i < MAX_GENTITIES) {
		...
		if (i >= MAX_CLIENTS && cent->currentState.number == i) { // do not clear G2 instances on client ents, they are constant
			CG_KillCEntityG2(i);
		}
	...
	while (i < strLen && str[i]) {
		...
		if (str[i] == '@' && (i + 1) < strLen) {
			if (str[i + 1] == '@' && (i + 2) < strLen) {
				if (str[i + 2] == '@' && (i + 3) < strLen) { //@@@ should mean to insert a StringEd reference here, so insert it into buf at the current place
...
static void CG_BodyQueueCopy(centity_t *cent, int clientNum, int knownWeapon) {
	...
	if (!cent->ownerRagging) {
		...
		if (!BG_InDeathAnim(source->currentState.torsoAnim)) { // then just snap the corpse into a default
...
static void CG_SiegeBriefingDisplay_f(void) { CG_SiegeBriefingDisplay(atoi(CG_Argv(1)), 0); }
...
static void CG_Chat_f(void) {
	...
	if (!strcmp(cmd, "chat")) {
		...
		if (!cg_teamChatsOnly.integer) {
			char name[MAX_NETNAME] = {0}, loc[MAX_STRING_CHARS] = {0}, color[8] = {0}, message[MAX_STRING_CHARS] = {0};
...
int svcmdcmp(const void *a, const void *b) { return Q_stricmp((const char *)a, ((serverCommand_t *)b)->cmd); }
