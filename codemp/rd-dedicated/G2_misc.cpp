...
static std::map<std::pair<int, int>, int>
    GoreTagsTemp; // this is a surface index to gore tag map used only
                  // temporarily during the generation phase so we reuse gore
                  // tags per LOD
int goreModelIndex;
...
// TODO: This needs to be set via a scalability cvar with some reasonable
// minimum value if pgore is used at all
...
public:
  ...
  CTraceSurface(int initsurfaceNum, surfaceInfo_v &initrootSList,
                model_t *initcurrentModel, int initlod, vec3_t initrayStart,
                vec3_t initrayEnd, CollisionRecord_t *initcollRecMap,
                int initentNum, int initmodelIndex, skin_t *initskin,
                shader_t *initcust_shader, size_t *initTransformedVertsArray,
                int inittraceFlags,
#ifdef _G2_GORE
                float fRadius, float initssize, float inittsize,
                float inittheta, int initgoreShader,
                CGhoul2Info *initghoul2info, SSkinGoreData *initgore)
      :
#else
                float fRadius)
      :
        ...
        currentModel(initcurrentModel), lod(initlod),
        collRecMap(initcollRecMap), entNum(initentNum),
        modelIndex(initmodelIndex), skin(initskin),
        cust_shader(initcust_shader),
        TransformedVertsArray(initTransformedVertsArray),
        traceFlags(inittraceFlags),
#ifdef _G2_GORE
        m_fRadius(fRadius), ssize(initssize), tsize(inittsize),
        theta(inittheta), goreShader(initgoreShader),
        ghoul2info(initghoul2info), gore(initgore)
#else
        m_fRadius(fRadius)
#endif
  {
...
void G2_List_Model_Surfaces(const char *fileName) {
  ...
  surf = (mdxmSurfHierarchy_t *)((byte *)mod_m->mdxm +
                                 mod_m->mdxm->ofsSurfHierarchy);
  mdxmSurface_t *surface =
      (mdxmSurface_t *)((byte *)mod_m->mdxm + mod_m->mdxm->ofsLODs +
                        sizeof(mdxmLOD_t));
  ...
  for (x = 0; x < mod_m->mdxm->numSurfaces; x++) {
    ...
    surf = (mdxmSurfHierarchy_t *)((byte *)surf +
                                   (size_t)(
                                       &((mdxmSurfHierarchy_t *)0)
...
void G2_List_Model_Bones(const char *fileName, int frame) {
  ...
  //	aframe = (mdxaFrame_t *)((byte *)header + header->ofsFrames + (frame *
  //frameSize));
  ...
  for (x = 0; x < mod_a->mdxa->numBones; x++) {
    ...
    Com_Printf("X pos %f, Y pos %f, Z pos %f\n", skel->BasePoseMat.matrix[0][3],
               skel->BasePoseMat.matrix[1][3], skel->BasePoseMat.matrix[2][3]);
...
int G2_DecideTraceLod(CGhoul2Info &ghoul2, int useLod) {
  ...
  // if we are overriding the LOD at top level, then we can afford to only check
  // this level of model
...
void R_TransformEachSurface(const mdxmSurface_t *surface, vec3_t scale,
  ...
  TransformedVerts =
      (float *)G2VertSpace->MiniHeapAlloc(surface->numVerts * 5 * 4);
  TransformedVertsArray[surface->thisSurfaceIndex] = (size_t)TransformedVerts;
  if (!TransformedVerts) {
    Com_Error(ERR_DROP, "Ran out of transform space for Ghoul2 Models. Adjust "
                        "MiniHeapSize in SV_SpawnServer.\n");
  }
    ...
    for (j = 0; j < numVerts; j++) {
      ...
      for (k = 0; k < iNumWeights; k++) {
        ...
        const mdxaBone_t &bone =
            EvalBoneCache(piBoneReferences[iBoneIndex], boneCache);
...
void G2_TransformSurfaces(int surfaceNum, surfaceInfo_v &rootSList,
                          CBoneCache *boneCache, const model_t *currentModel,
                          int lod, vec3_t scale, IHeapAllocator *G2VertSpace,
                          size_t *TransformedVertArray, bool secondTimeAround) {
  int i;
  assert(currentModel);
  assert(currentModel->mdxm);
  // back track and get the surfinfo struct for this surface
  const mdxmSurface_t *surface =
      (mdxmSurface_t *)G2_FindSurface((void *)currentModel, surfaceNum, lod);
  ...
  const surfaceInfo_t *surfOverride =
      G2_FindOverrideSurface(surfaceNum, rootSList);
  ...
  if (!offFlags) {
    ...
    R_TransformEachSurface(surface, scale, G2VertSpace, TransformedVertArray,
                           boneCache);
  }
  ...
  for (i = 0; i < surfInfo->numChildren; i++) {
    G2_TransformSurfaces(surfInfo->childIndexes[i], rootSList, boneCache,
                         currentModel, lod, scale, G2VertSpace,
                         TransformedVertArray, secondTimeAround);
  }
}

// main calling point for the model transform for collision detection. At this
// point all of the skeleton has been transformed.
#ifdef _G2_GORE
void G2_TransformModel(CGhoul2Info_v &ghoul2, const int frameNum, vec3_t scale,
                       IHeapAllocator *G2VertSpace, int useLod, bool ApplyGore)
#else
void G2_TransformModel(CGhoul2Info_v &ghoul2, const int frameNum, vec3_t scale,
                       IHeapAllocator *G2VertSpace, int useLod)
#endif
{
  int i, lod;
  vec3_t correctScale;
  ...
  for (i = 0; i < ghoul2.size(); i++) {
    ...
    if (!(g.mFlags &
          GHOUL2_ZONETRANSALLOC)) { // do not stomp if we're using zone space
      g.mTransformedVertsArray = (size_t *)G2VertSpace->MiniHeapAlloc(
          g.currentModel->mdxm->numSurfaces * sizeof(size_t));
      if (!g.mTransformedVertsArray) {
        Com_Error(ERR_DROP, "Ran out of transform space for Ghoul2 Models. "
                            "Adjust MiniHeapSize in SV_SpawnServer.\n");
      }
    }

    memset(g.mTransformedVertsArray, 0,
           g.currentModel->mdxm->numSurfaces * sizeof(size_t));

    G2_FindOverrideSurface(
        -1, g.mSlist); // reset the quick surface override lookup;
    // recursively call the model surface transform

    G2_TransformSurfaces(g.mSurfaceRoot, g.mSlist, g.mBoneCache, g.currentModel,
                         lod, correctScale, G2VertSpace,
...
qboolean G2_SegmentTriangleTest(const vec3_t start, const vec3_t end,
                                ...
                                qboolean backFaces, qboolean frontFaces,
                                vec3_t returnedPoint, vec3_t returnedNormal,
                                float *denom) {
...
// now we at poly level, check each model space transformed poly against the
// model world transfomed ray
void G2_GorePolys(const mdxmSurface_t *surface, CTraceSurface &TS,
                  const mdxmSurfHierarchy_t *surfInfo) {
  int j;
  vec3_t basis1;
  vec3_t basis2;
  ...
  for (j = 0; j < numTris; j++) {
    ...
    flags = 15 & GoreVerts[tris[j].indexes[0]].flags &
    ...
    if (!TS.gore->frontFaces || !TS.gore->backFaces) {
      ...
      VectorSubtract(&verts[tris[j].indexes[1] * 5],
                     &verts[tris[j].indexes[0] * 5], e1);
      VectorSubtract(&verts[tris[j].indexes[2] * 5],
                     &verts[tris[j].indexes[0] * 5], e2);
    ...
    for (k = 0; k < 3; k++) {
      if (GoreVerts[tris[j].indexes[k]].touch == GoreTouch) {
        GoreIndecies[newNumTris * 3 + k] =
            GoreVerts[tris[j].indexes[k]].newindex;
      } else {
  ...
  int newTag;
  std::map<std::pair<int, int>, int>::iterator f =
      GoreTagsTemp.find(std::make_pair(goreModelIndex, TS.surfaceNum));
  if (f == GoreTagsTemp.end()) // need to generate a record
  {
    ...
    add.mGoreGrowFactor =
        (1.0f - TS.gore->goreScaleStartFraction) /
        (float)(TS.gore
                    ->growDuration); // curscale =
                                     // (curtime-mGoreGrowStartTime)*mGoreGrowFactor;
  ...
  if (gore) {
    ...
    *data++ = newNumTris;
    ...
    TransformPoint(TS.rayStart, shotOriginInCurrentSpace,
                   (mdxaBone_t *)fdata); // dest middle arg
    // this will insure the shot origin in our unknown space is now the shot
    // origin, making it a known space
    fdata[3] -= shotOriginInCurrentSpace[0];
    fdata[7] -= shotOriginInCurrentSpace[1];
    fdata[11] -= shotOriginInCurrentSpace[2];
    Inverse_Matrix((mdxaBone_t *)fdata,
                   (mdxaBone_t *)(fdata + 12)); // dest 2nd arg
    data += 24;
...
// now we're at poly level, check each model space transformed poly against the
// model world transfomed ray
static bool G2_TracePolys(const mdxmSurface_t *surface,
                          const mdxmSurfHierarchy_t *surfInfo,
                          CTraceSurface &TS) {
  int j, numTris;

  // whip through and actually transform each vertex
  const mdxmTriangle_t *tris =
      (mdxmTriangle_t *)((byte *)surface + surface->ofsTriangles);
  const float *verts =
  ...
  for (j = 0; j < numTris; j++) {
    ...
    // did we hit it?
    int i;
    if (G2_SegmentTriangleTest(
            TS.rayStart, TS.rayEnd, point1, point2, point3, qtrue, qtrue,
            hitPoint, normal,
            &face)) { // find space in the collision records for this record
      for (i = 0; i < MAX_G2_COLLISIONS; i++) {
        if (TS.collRecMap[i].mEntityNum == -1) {
          ...
          TransformAndTranslatePoint(hitPoint, newCol.mCollisionPosition,
                                     &worldMatrix);
          ...
          // Determine our location within the texture, and barycentric
          // coordinates
          G2_BuildHitPointST(point1, point1[3], point1[4], point2, point2[3],
                             point2[4], point3, point3[3], point3[4], hitPoint,
                             &x_pos, &y_pos, newCol.mBarycentricI,
                             newCol.mBarycentricJ);

          /*
                                                  const shader_t
             *shader = 0;
                                                  // now, we know what surface
             this hit belongs to, we need to go get the shader handle so we can
             get the correct hit location and hit material info if ( cust_shader
             )
                                                  {
                                                          shader = cust_shader;
                                                  }
                                                  else if ( skin )
                                                  {
                                                          ...
                                                          // match the surface
             name to something in the skin file shader = tr.defaultShader; for (
             j = 0 ; j < skin->numSurfaces ; j++ )
                                                          {
                                                                  // the names
             have both been lowercased if ( !strcmp( skin->surfaces[j]->name,
             surfInfo->name ) )
                                                                  {
                                                                          shader
             = skin->surfaces[j]->shader; break;
                                                                  }
                                                          }
                                                  }
                                                  else
                                                  {
                                                          shader =
             R_GetShaderByHandle( surfInfo->shaderIndex );
                                                  }

                                                  // do we even care to decide
             what the hit or location area's are? If we don't have them in the
             shader there is little point if ((shader->hitLocation) ||
             (shader->hitMaterial))
                                                  {
                                                          // ok, we have a
             floating point position. - determine location in data we need to
             look at if (shader->hitLocation)
                                                          {
                                                                  newCol.mLocation
             = *(hitMatReg[shader->hitLocation].loc +
             ...
             (shader->hitMaterial)
                                                          {
                                                                  newCol.mMaterial
             = *(hitMatReg[shader->hitMaterial].loc +
      ...
      if (i == MAX_G2_COLLISIONS) {
        // assert(i!=MAX_G2_COLLISIONS);		// run out of collision record
        // space - will probalbly never happen It happens. And the assert is
        // bugging me.
        TS.hitOne = true; // force stop recursion
        return true; // return true to avoid wasting further time, but no hit
                     // will result without a record
      }
    }
  }
  return false;
...
// now we're at poly level, check each model space transformed poly against the
// model world transfomed ray
static bool G2_RadiusTracePolys(const mdxmSurface_t *surface,
                                CTraceSurface &TS) {
  int j;
  vec3_t basis1;
  vec3_t basis2;
  ...
  if (flags) {
    return false; // completely off the gore splotch  (so presumably hit
                  // nothing? -Ste)
  }
  const int numTris = surface->numTriangles;
  const mdxmTriangle_t *const tris =
      (mdxmTriangle_t *)((byte *)surface + surface->ofsTriangles);
  ...
  for (j = 0; j < numTris; j++) {
    ...
    assert(tris[j].indexes[1] >= 0 && tris[j].indexes[1] < numVerts);
    assert(tris[j].indexes[2] >= 0 && tris[j].indexes[2] < numVerts);
    flags = 63 & GoreVerts[tris[j].indexes[0]].flags &
            GoreVerts[tris[j].indexes[1]].flags &
            GoreVerts[tris[j].indexes[2]].flags;
    int i;
    if (flags) {
...
#else
          // yeah, I want the collision point. Let's work out the impact point
          // on the triangle. -rww
          vec3_t hitPoint;
          float side, side2;
          float dist;
          float third = -(A[0] * (B[1] * C[2] - C[1] * B[2]) +
                          B[0] * (C[1] * A[2] - A[1] * C[2]) +
                          C[0] * (A[1] * B[2] - B[1] * A[2]));

          VectorSubtract(TS.rayEnd, TS.rayStart, distVect);
          side = normal[0] * TS.rayStart[0] + normal[1] * TS.rayStart[1] +
                 normal[2] * TS.rayStart[2] + third;
          side2 = normal[0] * distVect[0] + normal[1] * distVect[1] +
                  normal[2] * distVect[2];
          dist = side / side2;
...
static void G2_TraceSurfaces(CTraceSurface &TS) {
  ...
  const mdxmSurface_t *surface =
      (mdxmSurface_t *)G2_FindSurface(TS.currentModel, TS.surfaceNum, TS.lod);
  const mdxmHierarchyOffsets_t *surfIndexes =
      (mdxmHierarchyOffsets_t *)((byte *)TS.currentModel->mdxm +
                                 sizeof(mdxmHeader_t));
  const mdxmSurfHierarchy_t *surfInfo =
      (mdxmSurfHierarchy_t *)((byte *)surfIndexes +
                              surfIndexes->offsets[surface->thisSurfaceIndex]);
  ...
  // see if we have an override surface in the surface list
  const surfaceInfo_t *surfOverride =
      G2_FindOverrideSurface(TS.surfaceNum, TS.rootSList);
    ...
    if (TS.collRecMap) {
      ...
      if (!(fabs(TS.m_fRadius) < 0.1)) // if not a point-trace
      {
        ...
        if (G2_TracePolys(surface, surfInfo, TS) &&
            (TS.traceFlags == G2_RETURNONHIT)) {
          // ok, we hit one, *and* we want to return instantly because the
          // returnOnHit is set so indicate we've hit one, so other surfaces
          // don't get hit and return
          TS.hitOne = true;
          return;
        }
      }
...
#ifdef _G2_GORE
void G2_TraceModels(CGhoul2Info_v &ghoul2, vec3_t rayStart, vec3_t rayEnd,
                    CollisionRecord_t *collRecMap, int entNum, int eG2TraceType,
                    int useLod, float fRadius, float ssize, float tsize,
                    float theta, int shader, SSkinGoreData *gore,
                    qboolean skipIfLODNotMatch)
#else
void G2_TraceModels(CGhoul2Info_v &ghoul2, vec3_t rayStart, vec3_t rayEnd,
                    CollisionRecord_t *collRecMap, int entNum, int eG2TraceType,
...
#endif
{
  ...
  for (i = 0; i < ghoul2.size(); i++) {
    ...
    if (ghoul2[i].mCustomShader &&
        ghoul2[i].mCustomShader != -20) // rww - -20 is a server instance (hack)
    {
    ...
    if (skipIfLODNotMatch) { // we only want to hit this SPECIFIC LOD...
      if (lod != useLod) {   // doesn't match, skip this model
        continue;
      }
    }
...
#ifdef _G2_GORE
    CTraceSurface TS(ghoul2[i].mSurfaceRoot, ghoul2[i].mSlist,
                     (model_t *)ghoul2[i].currentModel, lod, rayStart, rayEnd,
                     collRecMap, entNum, i, skin, cust_shader,
                     ghoul2[i].mTransformedVertsArray, eG2TraceType, fRadius,
                     ssize, tsize, theta, shader, &ghoul2[i], gore);
#else
    CTraceSurface TS(ghoul2[i].mSurfaceRoot, ghoul2[i].mSlist,
                     (model_t *)ghoul2[i].currentModel, lod, rayStart, rayEnd,
                     collRecMap, entNum, i, skin, cust_shader,
...
void TransformPoint(const vec3_t in, vec3_t out, mdxaBone_t *mat) {
  for (int i = 0; i < 3; i++) {
    out[i] = in[0] * mat->matrix[i][0] + in[1] * mat->matrix[i][1] +
             in[2] * mat->matrix[i][2];
  }
}
...
void TransformAndTranslatePoint(const vec3_t in, vec3_t out, mdxaBone_t *mat) {
  ...
  for (int i = 0; i < 3; i++) {
    out[i] = in[0] * mat->matrix[i][0] + in[1] * mat->matrix[i][1] +
             in[2] * mat->matrix[i][2] + mat->matrix[i][3];
  }
}
...
// generate the world matrix for a given set of angles and origin - called from
// lots of places
...
// go away and determine what the pointer for a specific surface definition
// within the model definition is
...
qboolean G2_SaveGhoul2Models(CGhoul2Info_v &ghoul2, char **buffer, int *size) {
  ...
  *size = 0;
  ...
  // this one isn't a define since I couldn't work out how to figure it out at
  // compile time
  int ghoul2BlockSize = (size_t)&ghoul2[0].mTransformedVertsArray -
                        (size_t)&ghoul2[0].mModelindex;
  ...
  *(int *)tempBuffer = ghoul2.size();
  ...
  for (i = 0; i < ghoul2.size(); i++) {
    ...
    //		Com_OPrintf("G2_SaveGhoul2Models(): ghoul2[%d].mModelindex =
...
// have to free space malloced in the save system here because the game DLL
// can't.
void G2_FreeSaveBuffer(char *buffer) { Z_Free(buffer); }
...
void G2_LoadGhoul2Model(CGhoul2Info_v &ghoul2, char *buffer) {
  ...
  // now we have enough instances, lets go through each one and load up the
  // relevant details
  for (int i = 0; i < ghoul2.size(); i++) {
    ...
    //		Com_OPrintf("G2_LoadGhoul2Model(): ghoul2[%d].mModelindex =
    //%d\n",i,ghoul2[i].mModelindex);
...
void G2_LerpAngles(CGhoul2Info_v &ghoul2, CGhoul2Info_v &nextGhoul2,
                   float interpolation) {
  // loop each model
  for (int i = 0; i < ghoul2.size(); i++) {
    if (ghoul2[i].mModelindex != -1) {
      ...
      for (size_t x = 0; x < ghoul2[i].mBlist.size(); x++) {
        ...
        if ((nextGhoul2.size() > i) && (nextGhoul2[i].mModelindex != -1) &&
            (nextGhoul2[i].mBlist.size() > x) &&
            (nextGhoul2[i].mBlist[x].boneNumber != -1)) {
          boneInfo_t &nextBone = nextGhoul2[i].mBlist[x];
          // does this bone override actually have anything in it, and if it
          // does, is it a bone angles override?
          if ((bone.boneNumber != -1) && ((bone.flags) & (BONE_ANGLES_TOTAL))) {
            ...
            for (int z = 0; z < 12; z++) {
              newMatrix[z] =
                  nowMatrix[z] + interpolation * (nextMatrix[z] - nowMatrix[z]);
            }
          }
        } else {
          memcpy(&ghoul2[i].mBlist[x].newMatrix, &ghoul2[i].mBlist[x].matrix,
                 sizeof(mdxaBone_t));
        }
      }
    }
