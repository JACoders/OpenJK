...
qboolean NPC_UpdateAngles(qboolean doPitch, qboolean doYaw) {
  ...
  if (doYaw) {
    ...
    NPCS.ucmd.angles[YAW] =
        ANGLE2SHORT(targetYaw + error) - NPCS.client->ps.delta_angles[YAW];
  }
  ...
  if (doPitch) {
    ...
    NPCS.ucmd.angles[PITCH] =
        ANGLE2SHORT(targetPitch + error) - NPCS.client->ps.delta_angles[PITCH];
  }

  NPCS.ucmd.angles[ROLL] = ANGLE2SHORT(NPCS.NPC->client->ps.viewangles[ROLL]) -
                           NPCS.client->ps.delta_angles[ROLL];

  if (exact &&
      trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC, TID_ANGLE_FACE)) {
...
void NPC_AimWiggle(vec3_t enemy_org) {
  ...
  if (NPCS.NPCInfo->aimErrorDebounceTime < level.time) {
    NPCS.NPCInfo->aimOfs[0] =
        0.3 * flrand(NPCS.NPC->enemy->r.mins[0], NPCS.NPC->enemy->r.maxs[0]);
    NPCS.NPCInfo->aimOfs[1] =
        0.3 * flrand(NPCS.NPC->enemy->r.mins[1], NPCS.NPC->enemy->r.maxs[1]);
    if (NPCS.NPC->enemy->r.maxs[2] > 0) {
      NPCS.NPCInfo->aimOfs[2] =
          NPCS.NPC->enemy->r.maxs[2] * flrand(0.0f, -1.0f);
    }
  }
...
qboolean NPC_UpdateFiringAngles(qboolean doPitch, qboolean doYaw) {
  ...
  if (NPCS.NPCInfo->aimErrorDebounceTime < level.time) {
    if (Q_irand(0, 1)) {
      NPCS.NPCInfo->lastAimErrorYaw =
          ((float)(6 - NPCS.NPCInfo->stats.aim)) * flrand(-1, 1);
    }
    if (Q_irand(0, 1)) {
      NPCS.NPCInfo->lastAimErrorPitch =
          ((float)(6 - NPCS.NPCInfo->stats.aim)) * flrand(-1, 1);
    }
  ...
  if (doYaw) {
    ...
    */
    ...
    NPCS.ucmd.angles[YAW] = ANGLE2SHORT(targetYaw + diff + error) -
  ...
  if (doPitch) {
    ...
    NPCS.ucmd.angles[PITCH] = ANGLE2SHORT(targetPitch + diff + error) -
...
qboolean G_ActivateBehavior(gentity_t *self, int bset) {
  ...
  if (bSID != (bState_t)-1) {
    ...
    */
    ...
    // FIXME: between here and actually getting into the ICARUS_RunScript
    // function, the stack gets blown! if ( ( ICARUS_entFilter == -1 ) || (
    // ICARUS_entFilter == self->s.number ) )
    if (0) {
      G_DebugPrint(WL_VERBOSE, "%s attempting to run bSet %s (%s)\n",
                   self->targetname, GetStringForID(BSETTable, bset), bs_name);
    }
    trap->ICARUS_RunScript((sharedEntity_t *)self,
                           va("%s/%s", Q3_SCRIPT_DIR, bs_name));
  }
  return qtrue;
}
...
// rww - and another method of automatically managing surface status for the
// client and server at once
#define TURN_ON 0x00000000
#define TURN_OFF 0x00000100

void NPC_SetSurfaceOnOff(gentity_t *ent, const char *surfaceName,
                         int surfaceFlags) {
  int i = 0;
  ...
  if (!foundIt) {
    Com_Printf("WARNING: Tried to toggle NPC surface that isn't in toggleable "
               "surface list (%s)\n",
               surfaceName);
    return;
  }

  if (surfaceFlags == TURN_ON) { // Make sure the entitystate values reflect
                                 // this surface as on now.
...
// rww - cheap check to see if an armed client is looking in our general
// direction
qboolean NPC_SomeoneLookingAtMe(gentity_t *ent) {
  ...
  while (i < MAX_CLIENTS) {
    ...
    if (pEnt && pEnt->inuse && pEnt->client &&
        ...
        pEnt->client->tempSpectate < level.time &&
        !(pEnt->client->ps.pm_flags & PMF_FOLLOW) &&
        pEnt->s.weapon != WP_NONE) {
      if (trap->InPVS(ent->r.currentOrigin, pEnt->r.currentOrigin)) {
        if (InFOV(ent, pEnt, 30, 30)) { // I'm in a 30 fov or so cone from this
                                        // player.. that's enough I guess.
          return qtrue;
        }
      }
    }
...
qboolean NPC_ClearLOS5(const vec3_t end) { return G_ClearLOS5(NPCS.NPC, end); }
qboolean NPC_ClearLOS4(gentity_t *ent) { return G_ClearLOS4(NPCS.NPC, ent); }
...
qboolean NPC_ValidEnemy(gentity_t *ent) {
  ...
  } else if (ent->client && ent->client->sess.sessionTeam ==
                                TEAM_SPECTATOR) { // don't go after spectators
    return qfalse;
  } else if (ent->client && ent->client->tempSpectate >=
  ...
  if (entTeam ==
          NPCS.NPC->client->enemyTeam // simplest case: they're on my enemy team
      || (NPCS.NPC->client->enemyTeam == NPCTEAM_FREE &&
          ent->client->NPC_class !=
              NPCS.NPC->client->NPC_class) // I get mad at anyone and this guy
                                           // isn't the same class as me
      ||
      (ent->client->NPC_class == CLASS_WAMPA && ent->enemy) // a rampaging wampa
      || (ent->client->NPC_class == CLASS_RANCOR &&
          ent->enemy) // a rampaging rancor
      ||
      (entTeam == NPCTEAM_FREE && ent->client->enemyTeam == NPCTEAM_FREE &&
       ...
       (ent->enemy->client->playerTeam == NPCS.NPC->client->playerTeam ||
        (ent->enemy->client->playerTeam != NPCTEAM_ENEMY &&
         NPCS.NPC->client->playerTeam ==
             NPCTEAM_PLAYER))) // enemy is a rampaging non-aligned creature who
...
qboolean NPC_TargetVisible(gentity_t *ent) {
  ...
  if (DistanceSquared(ent->r.currentOrigin, NPCS.NPC->r.currentOrigin) >
      (NPCS.NPCInfo->stats.visrange * NPCS.NPCInfo->stats.visrange))
    return qfalse;

  // Check our FOV
  if (InFOV(ent, NPCS.NPC, NPCS.NPCInfo->stats.hfov,
...
static int NPC_GetCheckDelta( void )
{
        ...
        {
                int distance = DistanceSquared( NPC->r.currentOrigin,
    ...
    if (alertEvent >= 0) {
      ...
      if (event->level >= AEL_DISCOVERED) {
        ...
        if ((event->owner->client) &&
            (event->owner->client->playerTeam == NPCS.NPC->client->playerTeam))
...
static qboolean NPC_CheckPlayerDistance(void) {
  ...
  if ( distance > DistanceSquared( NPC->r.currentOrigin,
  g_entities[0].r.currentOrigin ) ) { //rwwFIXMEFIXME: care about all clients
  not just client 0 G_SetEnemy( NPC, &g_entities[0] ); return qtrue;
  }
...
qboolean NPC_FacePosition(vec3_t position, qboolean doPitch) {
  ...
  // Get the positions
  if (NPCS.NPC->client &&
      (NPCS.NPC->client->NPC_class == CLASS_RANCOR ||
       NPCS.NPC->client->NPC_class ==
           CLASS_WAMPA)) // || NPC->client->NPC_class == CLASS_SAND_CREATURE) )
  {
    CalcEntitySpot(NPCS.NPC, SPOT_ORIGIN, muzzle);
    muzzle[2] += NPCS.NPC->r.maxs[2] * 0.75f;
  } else if (NPCS.NPC->client &&
             NPCS.NPC->client->NPC_class == CLASS_GALAKMECH) {
  ...
  if (doPitch) {
    // Find the delta between our goal and our current facing
    float currentAngles = (SHORT2ANGLE(NPCS.ucmd.angles[PITCH] +
                                       NPCS.client->ps.delta_angles[PITCH]));
...
void NPC_ClearLookTarget(gentity_t *self) {
  ...
  if ((self->client->ps.eFlags2 &
       EF2_HELD_BY_MONSTER)) { // lookTarget is set by and to the monster that's
                               // holding you, no other operations can change
                               // that
    return;
  }
...
void NPC_CheckCharmed(void) {
  ...
  if (NPCS.NPCInfo->charmedTime && NPCS.NPCInfo->charmedTime < level.time &&
      NPCS.NPC->client) { // we were charmed, set us back!
...
void G_GetBoltPosition(gentity_t *self, int boltIndex, vec3_t pos,
  ...
  if (self->client) { // clients don't actually even keep r.currentAngles
                      // maintained
  ...
  trap->G2API_GetBoltMatrix(self->ghoul2, modelIndex, boltIndex, &boltMatrix,
                            angles, self->r.currentOrigin, level.time, NULL,
                            self->modelScale);
  if (pos) {
...
int NPC_GetEntsNearBolt(int *radiusEnts, float radius, int boltIndex,
                        vec3_t boltOrg) {
  vec3_t mins, maxs;
  int i;
