...
public:
  ...
  int WriteMember(
      FILE *); // Writes the member's data, in block format, to FILE *
  int ReadMember(
      char **, long *,
      CIcarus *icarus); // Reads the member's data, in block format, from FILE *
...
public:
  ...
  // Overloaded delete operator.
  inline void operator delete(void *pRawData) { // Validate data.
    if (pRawData == 0)
      return;
...
public:
  ...
  // Overloaded new operator.
  static void *operator new(size_t size) { // Allocate the memory.
    return IGameInterface::GetGame()->Malloc(size);
  }

  // Overloaded delete operator.
  static void operator delete(void *pRawData) { // Free the Memory.
    IGameInterface::GetGame()->Free(pRawData);
  }
