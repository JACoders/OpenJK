...
// objectives with some fluff just in case we add more in the future.
...
template <typename TSaberInfo> class GClientBase {
public:
  ...
  float facial_blink; // time before next blink. If a minus value, we are in
                      // blink mode
  float facial_timer; // time before next alert, frown or smile. If a minus
                      // value, we are in anim mode
  ...
  qboolean isRagging;
  int overridingBones; // dragging body or doing something else to override one
                       // or more ragdoll effector's/pcj's

  vec3_t ragLastOrigin; // keeping track of positions between rags while
                        // dragging corpses
  ...
  // Rocket locking vars for non-player clients (only Vehicles use these right
  // now...)
  int rocketLockIndex;
...
struct gentity_s {
  ...
  */
  ...
  // Navigation
  int spawnContents; // store contents of ents on spawn so nav system can
                     // restore them
  int waypoint;      // Set once per frame, if you've moved, and if someone asks
  int wayedge; // Used by doors and breakable things to know what edge goes
               // through them
  ...
  union {
    vec4_t finalRGBA;
    vec3_t pos4;
    vec3_t modelAngles; // for brush entities with an attached md3 model, as an
                        // offset to the brush's angles
  };
