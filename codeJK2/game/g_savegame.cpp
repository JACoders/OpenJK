...
static const field_t savefields_gEntity[] = {{strFOFS(client), F_GCLIENT},
...
static const field_t savefields_LevelLocals[] = {
	{strLLOFS(locationHead), F_GENTITY}, {strLLOFS(alertEvents), F_ALERTEVENT}, {strLLOFS(groups), F_AIGROUPS}, {NULL, 0, F_IGNORE}};
...
//
static const field_t savefields_gClient[] = {{strCLOFS(ps.saberModel), F_STRING},
...
template <typename T> static void EnumerateFields(const field_t *pFields, T *src_instance, unsigned int ulChid) {
	strList.clear();
...
template <typename T> static void EvaluateFields(const field_t *pFields, T *pbData, T *pbOriginalRefData, unsigned int ulChid) {
	ojk::SavedGameHelper saved_game(::gi.saved_game);
