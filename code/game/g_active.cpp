...
int G_FindLookItem(gentity_t *self) {
	...
	for (e = 0; e < numListedEntities; e++) {
		...
		if (ent->item->giType == IT_WEAPON && ent->item->giTag == WP_SABER) { // a weapon_saber pickup
		...
		if (ent->item->giType == IT_HOLDABLE && ent->item->giTag == INV_SECURITY_KEY) { // security keys are of the highest importance
			rating *= 2.0f;
		}
...
qboolean G_ControlledByPlayer(gentity_t *self) {
	if (self && self->NPC && self->NPC->controlledTime > level.time) { // being controlled
		gentity_t *controller = &g_entities[0];
		if (controller->client && controller->client->ps.viewEntity == self->s.number) { // we're the player's viewEntity
			return qtrue;
		}
	}
	return qfalse;
...
qboolean G_ValidateLookEnemy(gentity_t *self, gentity_t *enemy) {
	...
	if (!enemy->client || !enemy->NPC) { // not valid
		if ((enemy->svFlags & SVF_NONNPC_ENEMY) && enemy->s.weapon == WP_TURRET && enemy->noDamageTeam != self->client->playerTeam &&
		...
		if (enemy->health <= 0 &&
			((level.time - enemy->s.time) > 3000 || !InFront(enemy->currentOrigin, self->currentOrigin, self->client->ps.viewangles, 0.2f) ||
	...
	if ((!InFront(enemy->currentOrigin, self->currentOrigin, self->client->ps.viewangles, 0.0f) ||
		...
		(DistanceHorizontalSquared(enemy->currentOrigin, self->currentOrigin) > 65536 ||
		 fabs(enemy->currentOrigin[2] - self->currentOrigin[2]) > 384)) { //(not in front or not clear LOS) & greater than 256 away
		return qfalse;
	}
	...
	for (e = 0; e < numListedEntities; e++) {
		...
		if (ent->health <= 0) {
			if ((ucmd->buttons & BUTTON_ATTACK) || (ucmd->buttons & BUTTON_ALT_ATTACK) ||
				(ucmd->buttons & BUTTON_FORCE_FOCUS)) { // if attacking, don't consider dead enemies
				continue;
			}
		...
		if (ent->enemy == self) { // he's mad at me, he's more important
			rating *= 2.0f;
		} else if (ent->NPC && ent->NPC->blockedSpeechDebounceTime > level.time - 6000) { // he's detected me, he's more important
...
void P_WorldEffects(gentity_t *ent) {
	...
	if ((mouthContents & (CONTENTS_WATER | CONTENTS_SLIME))) {
		...
		if (ent->client->NPC_class == CLASS_SWAMPTROOPER) { // they have air tanks
			ent->client->airOutTime = level.time + 12000;	// don't need air
			ent->damage = 2;
		} else if (ent->client->airOutTime < level.time) { // if out of air, start drowning
			// drown!
	...
	if ((ent->health > 0) && (ent->painDebounceTime < level.time) && gi.WE_IsOutsideCausingPain(ent->currentOrigin) && TIMER_Done(ent, "AcidPainDebounce")) {
	...
	if (ent->client->inSpaceIndex && ent->client->inSpaceIndex != ENTITYNUM_NONE) { // we're in space, check for suffocating and for exiting
		...
		if (!spacetrigger->inuse || !G_PointInBounds(ent->client->ps.origin, spacetrigger->absmin, spacetrigger->absmax)) { // no longer in space then I suppose
...
void DoImpact(gentity_t *self, gentity_t *other, qboolean damageSelf, trace_t *trace) {
	...
	//-----------------------------------------------
	if (pSelfVeh && pSelfVeh->m_pVehicleInfo->type != VH_ANIMAL && pOtherVeh && pSelfVeh->m_pVehicleInfo == pOtherVeh->m_pVehicleInfo) {
	...
	if (!self->client || self->client->ps.lastOnGround + 300 < level.time || (self->client->ps.lastOnGround + 100 < level.time)) {
		...
		if (other->material == MAT_GLASS || other->material == MAT_GLASS_METAL || other->material == MAT_GRATE1 ||
			((other->svFlags & SVF_BBRUSH) &&
			 (other->spawnflags &
			  8 /*THIN*/))) //(other->absmax[0]-other->absmin[0]<=32||other->absmax[1]-other->absmin[1]<=32||other->absmax[2]-other->absmin[2]<=32)) )
		...
		if (pSelfVeh && pSelfVeh->m_pVehicleInfo->type != VH_ANIMAL) {
			if ((magnitude >= 80) && (self->painDebounceTime < level.time)) {
				...
				if (!other || !other->client) {
					...
					if (vehHitPercent > 0.9f && (level.time - self->lastImpact) > 2000 && vehSpeed > 300.0f) {
						...
						//-----------------------------------------------
						if (vehPilotedByPlayer) {
							if ((vehHitPercent > 0.99f && vehSpeed > 1000.0f && !Q_irand(0, 30)) || (vehHitPercent > 0.999f && vehInTurbo)) {
								vehDFlags |= DAMAGE_IMPACT_DIE;
							}
						} else if (player && G_IsRidingVehicle(player) && (Distance(self->currentOrigin, player->currentOrigin) < 800.0f) &&
					...
					if (vehDFlags & DAMAGE_IMPACT_DIE) {
						...
						if (player && G_IsRidingVehicle(player) && self->owner && Distance(self->currentOrigin, player->currentOrigin) < 500.0f) {
			...
			if (self->owner == other || self->activator == other) {										  // hit owner/activator
				if (self->m_pVehicle && !self->m_pVehicle->m_pVehicleInfo->Inhabited(self->m_pVehicle)) { // empty swoop
					if (self->client->respawnTime - level.time < 1000) {								  // just spawned in a second ago
		...
		} else if (self->client && (PM_InKnockDown(&self->client->ps) || (self->client->ps.eFlags & EF_FORCE_GRIPPED)) &&
		...
		if (magnitude >= 100 && other->s.number < ENTITYNUM_WORLD) {
			...
			if (VectorCompare(other->currentOrigin, vec3_origin)) { // a brush with no origin
			...
			if (self->NPC && other->s.number == ENTITYNUM_WORLD) { // NPCs take less damage
				force *= 0.5f;
			}

			if (self->s.number >= MAX_CLIENTS && self->client &&
				(PM_InKnockDown(&self->client->ps) ||
				 self->client->ps.eFlags & EF_FORCE_GRIPPED)) { // NPC: I was knocked down or being gripped, impact should be harder
			...
			if (pOtherVeh) { // if hit another vehicle, take their toughness into account, too
			...
			if (((force >= 1 || pSelfVeh) && other->s.number >= MAX_CLIENTS) || force >= 10) {
				...
				*/
				...
				if (pSelfVeh) {				// if in a vehicle when land on someone, always knockdown, throw, damage
				...
				{													// ignore the impact
					...
					if (!self->client || other->s.number < MAX_CLIENTS ||
						!other->client) {	// aw, fuck it, clients no longer take impact damage from other clients, unless you're the player
		...
		if (damageSelf && self->takedamage && !(self->flags & FL_NO_IMPACT_DMG)) {
			...
			if (pSelfVeh && self->client->ps.forceJumpZStart) {					  // we were force-jumping
				if (self->currentOrigin[2] >= self->client->ps.forceJumpZStart) { // we landed at same height or higher than we landed
					magnitude = 0;
			...
			if ((magnitude >= 100 + self->health && self->s.number >= MAX_CLIENTS && self->s.weapon != WP_SABER) || self->client->NPC_class == CLASS_VEHICLE ||
				(magnitude >= 700)) // health here is used to simulate structural integrity
			{
				if ((self->s.weapon == WP_SABER || self->s.number < MAX_CLIENTS ||
					...
					self->client && self->client->ps.groundEntityNum < ENTITYNUM_NONE && magnitude < 1000) { // players and jedi take less impact damage
				...
				if (other->bmodel &&
					other->material != MAT_GLASS) { // take off only a little because we broke architecture (not including glass), that should hurt
				...
				if (pSelfVeh) {
					...
					if (other->bmodel && other->material != MAT_GLASS) { // broke through some architecture, take a good amount of damage
				...
				if (magnitude >= 1) {
					...
					/*
									...
									dprint("\n");
					*/
					if (self->NPC && self->s.weapon == WP_SABER) { // FIXME: for now Jedi take no falling damage, but really they should if pushed off?
						magnitude = 0;
					}
		...
		for (i = 0; i < num; i++) {
			...
			if (ent->client->ps.stats[STAT_HEALTH] <= 0) {
				if (Q_stricmp("trigger_teleport", hit->classname) ||
					!(hit->spawnflags & 16 /*TTSF_DEAD_OK*/)) { // dead clients can only touch tiogger_teleports that are marked as touchable
					continue;
				}
			}
...
void G_NPCMunroMatchPlayerWeapon(gentity_t *ent) {
	...
	if (!in_camera) {
		if (ent && ent->client && ent->NPC && (ent->NPC->aiFlags & NPCAI_MATCHPLAYERWEAPON)) { // we're a Player NPC
...
void ClientTimerActions(gentity_t *ent, int msec) {
	...
	while (client->timeResidual >= 1000) {
		...
		if ((ent->flags & FL_OVERCHARGED_HEALTH)) {						// need to gradually reduce health back to max
			if (ent->health > ent->client->ps.stats[STAT_MAX_HEALTH]) { // decrement it
				ent->health--;
...
void ClientEvents(gentity_t *ent, int oldEventSequence) {
	...
	if (client->ps.weapon == WP_SABER) {
		if (g_timescale->value >= 1.0f || !(client->ps.forcePowersActive & (1 << FP_SPEED))) {
			...
			// sanity check
			if (client->ps.saberDamageDebounceTime - level.time > wait) { // when you unpause the game with force speed on, the time gets *really* wiggy...
...
gentity_t *G_KickTrace(gentity_t *ent, vec3_t kickDir, float kickDist, vec3_t kickEnd, int kickDamage, float kickPush, qboolean doSoundOnWalls) {
	...
	if (trace.fraction < 1.0f && !trace.startsolid && !trace.allsolid && trace.entityNum < ENTITYNUM_NONE) {
		...
		if (hitEnt) { // we hit an entity
			if (hitEnt->client) {
				...
				{										  // FIXME: this should not always work
					if (PM_InKnockDown(&hitEnt->client->ps) &&
						!PM_InGetUp(
							&hitEnt->client->ps)) { // don't hit people who are knocked down or being knocked down (okay to hit people getting up, though)
						return NULL;
					}
					if (PM_InRoll(&hitEnt->client->ps)) { // can't hit people who are rolling
						return NULL;
					}
					...
					if (ent->client->ps.torsoAnim == BOTH_A7_HILT) {			  // hit in head
						if (hitEnt->health > 0) {								  // knock down
							if (kickPush >= 150.0f /*75.0f*/ && !Q_irand(0, 1)) { // knock them down
					...
					} else if (ent->client->ps.legsAnim == BOTH_GETUP_BROLL_B || ent->client->ps.legsAnim == BOTH_GETUP_BROLL_F ||
							   ent->client->ps.legsAnim == BOTH_GETUP_FROLL_B || ent->client->ps.legsAnim == BOTH_GETUP_FROLL_F) {
						if (hitEnt->health > 0) {										// knock down
							if (hitEnt->client->ps.groundEntityNum == ENTITYNUM_NONE) { // he's in the air?  Send him flying back
								if (!(hitEnt->flags & FL_NO_KNOCKBACK)) {
									G_Throw(hitEnt, kickDir, kickPush);
								}
							} else {
			...
			} else { // FIXME: don't do this in repeated frames... only allow 1 frame in kick to hit wall?  The most extended one?  Pass in a bool on that
					 // frame.
...
qboolean G_CheckRollSafety(gentity_t *self, int anim, float testDist) {
	...
	if (PM_InAttackRoll(self->client->ps.legsAnim)) { // we don't care if people are in the way, we'll knock them down!
	...
	if (trace.fraction < 1.0f || trace.allsolid || trace.startsolid) { // inside something or will hit something
		return qfalse;
	}
	return qtrue;
}
...
void G_CamPullBackForLegsAnim(gentity_t *ent, qboolean useTorso = qfalse) {
	if ((ent->s.number < MAX_CLIENTS || G_ControlledByPlayer(ent))) {
		float animLength =
			PM_AnimLength(ent->client->clientInfo.animFileIndex, (useTorso ? (animNumber_t)ent->client->ps.torsoAnim : (animNumber_t)ent->client->ps.legsAnim));
...
qboolean G_GrabClient(gentity_t *ent, usercmd_t *ucmd) {
	...
	for (i = 0; i < numEnts; i++) {
		...
		if ((radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_RANCOR) || (radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_WAMPA) ||
			(radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_SAND_CREATURE)) { // can't be one being held
			continue;
		}

		if (PM_LockedAnim(radiusEnts[i]->client->ps.torsoAnim) || PM_LockedAnim(radiusEnts[i]->client->ps.legsAnim)) { // don't interrupt
			continue;
		}
		if (radiusEnts[i]->client->ps.groundEntityNum == ENTITYNUM_NONE) { // must be on ground
			continue;
		}
		...
		if (fabs(radiusEnts[i]->currentOrigin[2] - ent->currentOrigin[2]) > 8.0f) { // have to be close in Z
			continue;
		}

		if (!PM_HasAnimation(radiusEnts[i], BOTH_PLAYER_PA_1)) { // doesn't have matching anims
			continue;
		}
...
qboolean G_PullAttack(gentity_t *ent, usercmd_t *ucmd) {
	...
	if (ent->client->ps.torsoAnim == BOTH_PULL_IMPALE_STAB || ent->client->ps.torsoAnim == BOTH_PULL_IMPALE_SWING) { // pulling
		if (ent->NPC) {
			VectorClear(ent->client->ps.moveDir);
		}
		overridAngles = (PM_LockAngles(ent, ucmd) ? qtrue : overridAngles);
		ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
	} else if (ent->client->ps.torsoAnim == BOTH_PULLED_INAIR_B || ent->client->ps.torsoAnim == BOTH_PULLED_INAIR_F) { // being pulled
		gentity_t *puller = &g_entities[ent->client->ps.pullAttackEntNum];
		if (puller && puller->inuse && puller->client &&
			(puller->client->ps.torsoAnim == BOTH_PULL_IMPALE_STAB || puller->client->ps.torsoAnim == BOTH_PULL_IMPALE_SWING)) {
			vec3_t pullDir;
			vec3_t pullPos;
		...
		if (trace.fraction >= 1.0f) { // all clear
			...
			if (!trace.allsolid && !trace.startsolid) {
				if (trace.fraction >= 1.0f) { // all clear
					...
					if (ent->client->ps.legsAnimTimer <= 200) { // at the end of the anim, and we can't leave ourselves like this
...
qboolean G_CheckClampUcmd(gentity_t *ent, usercmd_t *ucmd) {
	...
	if (ent->client->NPC_class == CLASS_PROTOCOL || ent->client->NPC_class == CLASS_R2D2 || ent->client->NPC_class == CLASS_R5D2 ||
		ent->client->NPC_class == CLASS_GONK || ent->client->NPC_class == CLASS_MOUSE) { // these droids *cannot* strafe (looks bad)
		if (ucmd->rightmove) {
			// clear the strafe
			ucmd->rightmove = 0;
	...
	if ((ent->s.number < MAX_CLIENTS || G_ControlledByPlayer(ent)) && ent->s.weapon == WP_MELEE &&
	...
	if ((!ent->s.number && ent->aimDebounceTime > level.time) || (ent->client->ps.pm_time && (ent->client->ps.pm_flags & PMF_TIME_KNOCKBACK)) ||
		ent->forcePushTime > level.time) { // being knocked back, can't do anything!
		ucmd->buttons = 0;
		ucmd->forwardmove = 0;
		ucmd->rightmove = 0;
		ucmd->upmove = 0;
	...
	if (ent->client->ps.saberLockTime > level.time) { // in saberlock
		ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
		if (ent->client->ps.saberLockTime - level.time > SABER_LOCK_DELAYED_TIME) { // 2 second delay before either can push
	...
	if (ent->client->ps.saberMove == LS_A_JUMP_T__B_) {												  // can't move during leap
		if (ent->client->ps.groundEntityNum != ENTITYNUM_NONE || (!ent->s.number && player_locked)) { // hit the ground
			ucmd->forwardmove = 0;
		}
		ucmd->rightmove = ucmd->upmove = 0;
		if (ent->NPC) { // invalid now
	...
	if (ent->client->ps.saberMove == LS_A_BACKFLIP_ATK && ent->client->ps.legsAnim == BOTH_JUMPATTACK7) {					  // backflip attack
		...
		{																													  // middle of anim
			if (ent->client->ps.groundEntityNum != ENTITYNUM_NONE) {														  // still on ground?
	...
	if (ent->client->ps.legsAnim == BOTH_JUMPATTACK6 && ent->client->ps.legsAnimTimer > 0) {
		...
		if ((ent->client->ps.legsAnimTimer >= 1450 &&
			...
			(ent->client->ps.legsAnimTimer >= 400 && PM_AnimLength(ent->client->clientInfo.animFileIndex, BOTH_JUMPATTACK6) - ent->client->ps.legsAnimTimer >=
														 1100)) { // in a part of the anim that we're pretty much sideways in, raise up the mins
			ent->mins[2] = 0;
			ent->client->ps.pm_flags |= PMF_FIX_MINS;
		} else if ((ent->client->ps.pm_flags & PMF_FIX_MINS)) { // drop the mins back down
			G_FixMins(ent);
		}
		if (ent->NPC) { // invalid now
			VectorClear(ent->client->ps.moveDir);
	...
	if ((ent->client->ps.legsAnim == BOTH_BUTTERFLY_FL1 && ent->client->ps.saberMove == LS_JUMPATTACK_STAFF_LEFT) ||
		(ent->client->ps.legsAnim == BOTH_BUTTERFLY_FR1 && ent->client->ps.saberMove == LS_JUMPATTACK_STAFF_RIGHT) ||
		(ent->client->ps.legsAnim == BOTH_BUTTERFLY_RIGHT && ent->client->ps.saberMove == LS_BUTTERFLY_RIGHT) ||
		(ent->client->ps.legsAnim == BOTH_BUTTERFLY_LEFT && ent->client->ps.saberMove == LS_BUTTERFLY_LEFT)) { // forward flip/spin attack
		ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
			...
			|| ent->client->ps.legsAnim == BOTH_BUTTERFLY RIGHT )*/
		{
			...
			if (ent->client->ps.legsAnim != BOTH_BUTTERFLY_LEFT && ent->client->ps.legsAnim != BOTH_BUTTERFLY_RIGHT) {
				if (ent->client->ps.legsAnimTimer >= 100 // not at end
					&& PM_AnimLength(ent->client->clientInfo.animFileIndex, (animNumber_t)ent->client->ps.legsAnim) - ent->client->ps.legsAnimTimer >=
			...
			if (ent->client->ps.legsAnimTimer >= 1700 && ent->client->ps.legsAnimTimer < 1800) { // one of the two jumps
				if (ent->client->ps.groundEntityNum != ENTITYNUM_NONE) {						 // still on ground?
					// jump!
					ent->client->ps.velocity[2] = 250;
					ent->client->ps.forceJumpZStart = ent->client->ps.origin[2]; // so we don't take damage if we land at same height
	...
	if (ent->client->ps.legsAnim == BOTH_A7_SOULCAL && ent->client->ps.saberMove == LS_STAFF_SOULCAL) { // forward spinning staff attack
		ucmd->upmove = 0;
		...
		if (ent->client->ps.legsAnimTimer >= 2650 && ent->client->ps.legsAnimTimer < 2850) { // the jump
			if (ent->client->ps.groundEntityNum != ENTITYNUM_NONE) {						 // still on ground?
				// jump!
				ent->client->ps.velocity[2] = 250;
				ent->client->ps.forceJumpZStart = ent->client->ps.origin[2]; // so we don't take damage if we land at same height
	...
	if (ent->client->ps.torsoAnim == BOTH_A6_FB || ent->client->ps.torsoAnim == BOTH_A6_LR) { // can't turn or move during dual attack
		if (ent->NPC) {
			VectorClear(ent->client->ps.moveDir);
		}
		overridAngles = (PM_LockAngles(ent, ucmd) ? qtrue : overridAngles);
	...
	} else if (PM_SuperBreakLoseAnim(ent->client->ps.torsoAnim)) { // can't turn during Kyle's grappling
		if (ent->NPC) {
			VectorClear(ent->client->ps.moveDir);
		}
		overridAngles = (PM_LockAngles(ent, ucmd) ? qtrue : overridAngles);
	...
	} else if (ent->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_START || ent->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_HOLD ||
			   ent->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_END || ent->client->ps.legsAnim == BOTH_FORCE_DRAIN_GRABBED) { // can't turn or move
		if (ent->s.number < MAX_CLIENTS || G_ControlledByPlayer(ent)) {															 // player
			float forceDrainAngle = 90.0f;
			if (ent->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_START) { // starting drain
				...
				cg.overrides.active |= CG_OVERRIDE_3RD_PERSON_ANG;
				cg.overrides.thirdPersonAngle = cg_thirdPersonAngle.value + angle;
			} else if (ent->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_HOLD) { // draining
				cg.overrides.active |= CG_OVERRIDE_3RD_PERSON_ANG;
				cg.overrides.thirdPersonAngle = cg_thirdPersonAngle.value + forceDrainAngle;
			} else if (ent->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_END) { // ending drain
				float animLength = PM_AnimLength(ent->client->clientInfo.animFileIndex, (animNumber_t)ent->client->ps.torsoAnim);
				float elapsedTime = (float)(animLength - ent->client->ps.legsAnimTimer);
		...
		ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
		overridAngles = PM_LockAngles(ent, ucmd) ? qtrue : overridAngles;
	} else if (ent->client->ps.legsAnim == BOTH_PLAYER_PA_1 || ent->client->ps.legsAnim == BOTH_PLAYER_PA_2 || ent->client->ps.legsAnim == BOTH_PLAYER_PA_3 ||
			   ent->client->ps.legsAnim == BOTH_PLAYER_PA_3_FLY || ent->client->ps.legsAnim == BOTH_KYLE_PA_1 || ent->client->ps.legsAnim == BOTH_KYLE_PA_2 ||
			   ent->client->ps.legsAnim == BOTH_KYLE_PA_3) { // can't turn during Kyle's grappling
		if (ent->NPC) {
			VectorClear(ent->client->ps.moveDir);
		}
		...
		{ // actually do some damage during sequence
			...
			if (TIMER_Done(ent, "grappleDamageDebounce")) {
				...
				case BOTH_PLAYER_PA_1:
					...
					if (ent->client->ps.legsAnimTimer <= 1150 && ent->client->ps.legsAnimTimer > 1050) {
						TIMER_Set(ent, "grappleDamageDebounce", 150);
						if (ent->s.number < MAX_CLIENTS) { // special case
							damage = Q_irand(3, 5);
						} else {
		...
		if (ent->s.number < MAX_CLIENTS &&
			ent->client->ps.legsAnim == BOTH_PLAYER_PA_3_FLY) { // player holds extra long so you have more time to decide to do the quick getup
		...
		if (ent->client->ps.legsAnimTimer <= endOfAnimTime) { // pretty much done with the anim, so get up now
			if (ent->client->ps.legsAnim == BOTH_PLAYER_PA_3) {
				...
				*/
				...
				ent->client->ps.weaponTime = ent->client->ps.legsAnimTimer;
				if (ent->s.number < MAX_CLIENTS) { // player holds extra long so you have more time to decide to do the quick getup
					ent->client->ps.legsAnimTimer += PLAYER_KNOCKDOWN_HOLD_EXTRA_TIME;
					ent->client->ps.torsoAnimTimer += PLAYER_KNOCKDOWN_HOLD_EXTRA_TIME;
				}
			}
		...
		if (d_slowmodeath->integer <= 3 && ent->s.number < MAX_CLIENTS) { // no matrix effect, so slide the camera back and to the side
			G_CamPullBackForLegsAnim(ent);
			G_CamCircleForLegsAnim(ent);
		}
	} else if (ent->client->ps.legsAnim == BOTH_FORCELONGLEAP_START || ent->client->ps.legsAnim == BOTH_FORCELONGLEAP_ATTACK ||
			   ent->client->ps.legsAnim == BOTH_FORCELONGLEAP_LAND) { // can't turn during force leap
		if (ent->NPC) {
			VectorClear(ent->client->ps.moveDir);
		}
		overridAngles = PM_AdjustAnglesForLongJump(ent, ucmd) ? qtrue : overridAngles;
	} else if (PM_KickingAnim(ent->client->ps.legsAnim) || ent->client->ps.legsAnim == BOTH_ARIAL_F1 || ent->client->ps.legsAnim == BOTH_ARIAL_LEFT ||
			   ent->client->ps.legsAnim == BOTH_ARIAL_RIGHT || ent->client->ps.legsAnim == BOTH_CARTWHEEL_LEFT ||
			   ent->client->ps.legsAnim == BOTH_CARTWHEEL_RIGHT || ent->client->ps.legsAnim == BOTH_JUMPATTACK7 ||
			   ent->client->ps.legsAnim == BOTH_BUTTERFLY_FL1 || ent->client->ps.legsAnim == BOTH_BUTTERFLY_FR1 ||
			   ent->client->ps.legsAnim == BOTH_BUTTERFLY_RIGHT || ent->client->ps.legsAnim == BOTH_BUTTERFLY_LEFT ||
			   ent->client->ps.legsAnim == BOTH_A7_SOULCAL) { // can't move, check for damage frame
		...
		if (ent->client->ps.torsoAnim == BOTH_A7_HILT) {
			...
			case BOTH_BUTTERFLY_RIGHT:
				if ((elapsedTime >= 100 && elapsedTime <= 450) || (elapsedTime >= 1100 && elapsedTime <= 1350)) {		// lead leg
			...
			case BOTH_GETUP_FROLL_F:
				...
				if (ent->client->ps.legsAnim == BOTH_GETUP_BROLL_B || ent->client->ps.legsAnim == BOTH_GETUP_FROLL_B) { // rolling back, pull back the view
					G_CamPullBackForLegsAnim(ent);
				}
				break;
			...
			case BOTH_A7_KICK_BF:
				...
				if (ent->footRBolt != -1) {																			// actually trace to a bolt
					if ((elapsedTime >= 750 && elapsedTime < 850) || (elapsedTime >= 1400 && elapsedTime < 1500)) { // right, though either would do
						doKick = qtrue;
						G_GetBoltPosition(ent, ent->footRBolt, kickEnd);
						VectorSubtract(kickEnd, ent->currentOrigin, kickDir);
						kickDir[2] = 0; // ah, flatten it, I guess...
		...
		G_CamPullBackForLegsAnim(ent);
			   ...
			   ent->client->ps.saberMove == LS_A_BACKSTAB) { // can't move or turn during back attacks
		ucmd->forwardmove = ucmd->rightmove = 0;
		if (ent->NPC) {
			VectorClear(ent->client->ps.moveDir);
		}
	...
	} else if (ent->client->ps.torsoAnim == BOTH_WALL_FLIP_BACK1 || ent->client->ps.torsoAnim == BOTH_WALL_FLIP_BACK2 ||
			   ent->client->ps.legsAnim == BOTH_FORCEWALLRUNFLIP_END || ent->client->ps.legsAnim == BOTH_FORCEWALLREBOUND_BACK) {
		// pull back the view
		G_CamPullBackForLegsAnim(ent);
		...
		if (ent->client->ps.legsAnimTimer > 200.0f) {
			float animLength = PM_AnimLength(ent->client->clientInfo.animFileIndex, (animNumber_t)ent->client->ps.torsoAnim);
			float elapsedTime = (float)(animLength - ent->client->ps.torsoAnimTimer);
			if (elapsedTime < 750 || (elapsedTime >= 1650 && elapsedTime < 2400)) { // push forward
				ucmd->forwardmove = 64;
				ent->client->ps.speed = 200.0f;
			}
		}
		// FIXME: pull back camera?
			 ...
			 (ucmd->forwardmove || ucmd->rightmove ||
			  (VectorCompare(ent->client->ps.moveDir, vec3_origin) && ent->client->ps.speed > 0))) { // moving during full-body fast special
	...
	} else if (ent->client->ps.legsAnim == BOTH_FLIP_LAND) { // moving during full-body fast special
		float animLength = PM_AnimLength(ent->client->clientInfo.animFileIndex, (animNumber_t)ent->client->ps.torsoAnim);
		float elapsedTime = (float)(animLength - ent->client->ps.torsoAnimTimer);
		ucmd->upmove = ucmd->rightmove = ucmd->forwardmove = 0;
		if (elapsedTime > 600 && elapsedTime < 800 && ent->client->ps.groundEntityNum != ENTITYNUM_NONE) { // jump - FIXME: how do we stop double-jumps?
	...
	} else if ((PM_SuperBreakWinAnim(ent->client->ps.torsoAnim) || PM_SuperBreakLoseAnim(ent->client->ps.torsoAnim)) &&
		...
		if ((ucmd->buttons & BUTTON_ATTACK) || (ucmd->buttons & BUTTON_ALT_ATTACK) || (ucmd->buttons & BUTTON_USE_FORCE) ||
			(ucmd->buttons & BUTTON_FORCEGRIP) || (ucmd->buttons & BUTTON_FORCE_LIGHTNING) || (ucmd->buttons & BUTTON_FORCE_DRAIN) ||
			ucmd->upmove) { // stop the anim
	...
	if (PM_InRoll(&ent->client->ps)) {
		if (ent->s.number >= MAX_CLIENTS || !player_locked) {
			...
			PM_CmdForRoll(&ent->client->ps, ucmd);
			if (ent->s.number >= MAX_CLIENTS) {								 // make sure it doesn't roll me off a ledge
				if (!G_CheckRollSafety(ent, ent->client->ps.legsAnim, 24)) { // crap!  I guess all we can do is stop...  UGH
	...
	if (PM_InCartwheel(ent->client->ps.legsAnim)) { // can't keep moving in cartwheel
		if (ent->client->ps.legsAnimTimer > 100) {	// still have time left in the anim
			ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
			if (ent->NPC) { // invalid now
				VectorClear(ent->client->ps.moveDir);
			}
...
void G_HeldByMonster(gentity_t *ent, usercmd_t **ucmd) {
	if (ent && ent->activator && ent->activator->inuse && ent->activator->health > 0) {
		...
		if (monster->count == 1) {		  // being held in hand rather than the mouth, so use *that* bolt
			boltIndex = monster->handRBolt;
		}
		vec3_t monAngles = {0};
		monAngles[YAW] = monster->currentAngles[YAW]; // only use YAW when passing angles to G2
		gi.G2API_GetBoltMatrix(monster->ghoul2, monster->playerModel, boltIndex, &boltMatrix, monAngles, monster->currentOrigin,
							   (cg.time ? cg.time : level.time), NULL, monster->s.modelScale);
...
void G_CheckClientIdle(gentity_t *ent, usercmd_t *ucmd) {
	...
	if (!VectorCompare(vec3_origin, ent->client->ps.velocity) || ucmd->buttons || ucmd->forwardmove || ucmd->rightmove || ucmd->upmove ||
		!PM_StandingAnim(ent->client->ps.legsAnim) || ent->enemy || ent->client->ps.legsAnimTimer ||
...
void ClientAlterSpeed(gentity_t *ent, usercmd_t *ucmd, qboolean controlledByPlayer, float vehicleFrameTimeModifier) {
	...
	if (ent->NPC != NULL && ent->client &&
		(ent->s.m_iVehicleNum != 0) /*&& ent->client->NPC_class == CLASS_VEHICLE*/) { // we don't actually scale the ucmd, we use actual speeds
			...
			if (!(ucmd->buttons & BUTTON_USE)) { // Not leaning
				...
				if (ent->NPC->currentSpeed <= 24 && ent->NPC->desiredSpeed < ent->NPC->currentSpeed) { // No-one walks this slow
					...
					if (player && player->client && player->client->ps.viewEntity == ent->s.number) {
						...
						float turndelta = 0;
						// if the NPC is locked into a Yaw, we want to check the lockedDesiredYaw...otherwise the NPC can't walk backwards, because it always
						// thinks it trying to turn according to desiredYaw
						...
						if (turndelta < 0.75f) {
							client->ps.speed = 0;
						} else if (ent->NPC->distToGoal < 100 && turndelta < 1.0) { // Turn is greater than 45 degrees or closer than 100 to goal
	...
	} else { // Client sets ucmds and such for speed alterations
		{
			...
			} else if (PM_SaberInAttack(ent->client->ps.saberMove) && ucmd->forwardmove < 0) { // if running backwards while attacking, don't run as fast.
			...
			} else if (PM_LeapingSaberAnim(client->ps.legsAnim)) { // no mod on speed when leaping
																   // FIXME: maybe jump?
			...
			} else if (client->ps.weapon == WP_SABER && (ucmd->buttons & BUTTON_ATTACK)) { // if attacking with saber while running, drop your speed
	...
	if (client->NPC_class == CLASS_ATST && client->ps.legsAnim == BOTH_RUN1START) { // HACK: when starting to move as atst, ramp up speed
	...
	if (!pVeh) {
		if (ucmd->forwardmove < 0 && !(ucmd->buttons & BUTTON_WALKING) &&
			client->ps.groundEntityNum != ENTITYNUM_NONE) { // running backwards is slower than running forwards
...
void ClientThink_real(gentity_t *ent, usercmd_t *ucmd) {
	...
	if (ent->s.number == 0) {
		...
		{																	// NOTE: doesn't keep updating to nearest enemy once you're dead
			...
			if (ent->enemy) { // target
				...
				} else { // okay, no interesting things and no enemies, so look for items
		...
		// If he's riding the vehicle...
			...
			if (g_skippingcin->integer) { // We're skipping the cinematic and it's over now
				gi.cvar_set("timescale", "1");
				gi.cvar_set("skippingCinematic", "0");
			}
			if (ent->client->ps.pm_type == PM_DEAD &&
				cg.missionStatusDeadTime < level.time) { // mission status screen is up because player is dead, stop all scripts
				stop_icarus = qtrue;
			}
		}
		...
		if ((player_locked || (ent->client->ps.eFlags & EF_FORCE_GRIPPED) || (ent->client->ps.eFlags & EF_FORCE_DRAINED) ||
			 (ent->client->ps.legsAnim == BOTH_PLAYER_PA_1) || (ent->client->ps.legsAnim == BOTH_PLAYER_PA_2) ||
		...
		if (ent->client->ps.leanofs) { // no shooting while leaning
			...
			if (ent->client->ps.weapon != WP_DISRUPTOR) { // can still zoom around corners
	...
	} else if (ent->client) { // this is any client that is not a vehicle (OR: is a vehicle and it not being ridden, is not being boarded, or is a TaunTaun...!
		if (ent->client->NPC_class == CLASS_GONK || ent->client->NPC_class == CLASS_MOUSE || ent->client->NPC_class == CLASS_R2D2 ||
			ent->client->NPC_class == CLASS_R5D2) { // no jumping or strafing in these guys
			ucmd->upmove = ucmd->rightmove = 0;
		} else if (ent->client->NPC_class == CLASS_ATST || ent->client->NPC_class == CLASS_RANCOR) { // no jumping in atst
			...
			if (ent->client->ps.groundEntityNum != ENTITYNUM_NONE) { // ATST crushes anything underneath it
				...
				if (ent->client->NPC_class != CLASS_RANCOR && !Q_irand(0, 10)) { // not so often...
				   ...
				   !ent->client->ps.forceJumpCharge) { // standing on an entity and not currently force jumping
			...
			if (groundEnt && groundEnt->client) {
				...
				if (groundEnt->client && groundEnt->client->NPC_class == CLASS_VEHICLE) {
					if (ent->client->NPC_class != CLASS_VEHICLE) { // um... vehicles shouldn't ride other vehicles, mmkay?
						if ((groundEnt->m_pVehicle->m_pVehicleInfo->type == VH_ANIMAL && PM_HasAnimation(ent, BOTH_VT_IDLE)) ||
						   ...
						   G_HasKnockdownAnims(ent)) { // step on a sand creature = *you* fall down
					G_Knockdown(ent, groundEnt, vec3_origin, 300, qtrue);
				} else if (groundEnt->client && groundEnt->client->NPC_class == CLASS_RANCOR) { // step on a Rancor, it bucks & throws you off
					if (groundEnt->client->ps.legsAnim != BOTH_ATTACK3 && groundEnt->client->ps.legsAnim != BOTH_ATTACK4 &&
						groundEnt->client->ps.legsAnim != BOTH_BUCK_RIDER) { // don't interrupt special anims
				...
				} else if (groundEnt->client->ps.groundEntityNum != ENTITYNUM_NONE && groundEnt->health > 0 && !PM_InRoll(&groundEnt->client->ps) &&
						   !(groundEnt->client->ps.eFlags & EF_LOCKED_TO_WEAPON) && !(groundEnt->client->ps.eFlags & EF_HELD_BY_RANCOR) &&
						   !(groundEnt->client->ps.eFlags & EF_HELD_BY_WAMPA) && !(groundEnt->client->ps.eFlags & EF_HELD_BY_SAND_CREATURE) &&
						   !inSpinFlipAttack)
				...
				{ // landed on a live client who is on the ground, jump off them and knock them down
					...
					{																										// force-jumper landed on someone
						...
						if (groundEnt->client->playerTeam != ent->client->playerTeam) { // don't knock down own guys
							forceKnockdown = (qboolean)(Q_irand(0, RANK_CAPTAIN + 4) < ent->NPC->rank);
						}
						// now what... push the groundEnt out of the way?
						if (!ent->client->ps.velocity[0] && !ent->client->ps.velocity[1]) { // not moving, shove us a little
							vec3_t slideFwd;
						...
						if (!PM_InRoll(&ent->client->ps) && !PM_FlippingAnim(ent->client->ps.legsAnim)) {
							...
							} else if (!ucmd->upmove) {				   // if not ducking (which should cause a roll), then jump
								ucmd->upmove = 127;
							}
							if (!ucmd->forwardmove && !ucmd->rightmove) { //  If not moving, don't want to jump straight up
							...
							if (!ent->s.number && ucmd->upmove < 0) { // player who should roll- force it
					...
					if (!forceKnockdown && groundEnt->client->NPC_class == CLASS_DESANN &&
						ent->client->NPC_class != CLASS_LUKE) { // can't knock down desann unless you're luke
							   ...
							   || (((groundEnt->s.number &&
									 (groundEnt->s.weapon != WP_SABER || !groundEnt->NPC ||
									  groundEnt->NPC->rank < Q_irand(RANK_CIVILIAN, RANK_CAPTAIN + 1))) // an NPC who is either not a saber user or passed the
																										// rank-based probability test
									|| ((!ent->s.number || G_ControlledByPlayer(groundEnt)) && !Q_irand(0, 3) && cg.renderingThirdPerson &&
										!cg.zoomMode))										   // or a player in third person, 25% of the time
								   ...
								   && ent->client->ps.legsAnim != BOTH_JUMPATTACK6))		   // not in the sideways-spinning jump attack
					{
						...
						if (PM_CrouchAnim(groundEnt->client->ps.legsAnim)) { // knockdown from crouch
							...
							if (DotProduct(ent->client->ps.velocity, gEFwd) > 50) { // pushing him forward
	...
	if (ent->client && ent->s.m_iVehicleNum != 0 && !ent->s.number && !MatrixMode) { // FIXME: extern and read from g_vehicleInfo?
	...
	if (client->ps.stats[STAT_HEALTH] <= 0) {
		...
		if (ent && !ent->s.number && ent->enemy && ent->enemy != ent && ent->enemy->s.number < ENTITYNUM_WORLD && ent->enemy->inuse &&
			!(cg.overrides.active & CG_OVERRIDE_3RD_PERSON_ANG)) { // keep facing enemy
			vec3_t deadDir;
			float deadYaw;
...
void ClientThink(int clientNum, usercmd_t *ucmd) {
	...
	if (ent->s.number < MAX_CLIENTS) {
		if (ent->client->ps.viewEntity > 0 && ent->client->ps.viewEntity < ENTITYNUM_WORLD) { // you're controlling another NPC
			...
			if (controlled->NPC && controlled->NPC->controlledTime &&
				ent->client->ps.forcePowerLevel[FP_TELEPATHY] > FORCE_LEVEL_3) { // An NPC I'm controlling with mind trick
				if (controlled->NPC->controlledTime < level.time) {				 // time's up!
					G_ClearViewEntity(ent);
					freed = qtrue;
				} else if (ucmd->upmove > 0) { // jumping gets you out of it FIXME: check some other button instead... like ESCAPE... so you could even have
											   // total control over an NPC?
					G_ClearViewEntity(ent);
			...
			{														  // these types use jump to get out
				if (ucmd->upmove > 0) { // jumping gets you out of it FIXME: check some other button instead... like ESCAPE... so you could even have total
										// control over an NPC?
					G_ClearViewEntity(ent);
					ucmd->upmove = 0; // ucmd->buttons = 0;
					// stop player from doing anything for a half second after
			...
			if (!freed) { // still controlling, save off my ucmd and clear it for my actual run through pmove
				restore_ucmd = qtrue;
	...
	if (restore_ucmd && ent->client) { // restore ucmd for later so NPC you're controlling can refer to them
