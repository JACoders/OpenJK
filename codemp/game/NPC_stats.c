...
stringID_table_t TeamTable[] = {
    ENUM2STRING(NPCTEAM_FREE), // caution, some code checks a team_t via "if
                               // (!team_t_varname)" so I guess this should stay
                               // as entry 0, great or what? -slc
    ENUM2STRING(NPCTEAM_PLAYER),
    ENUM2STRING(NPCTEAM_ENEMY),
    ENUM2STRING(
        NPCTEAM_NEUTRAL), // most droids are team_neutral, there are some
...
char *TeamNames[TEAM_NUM_TEAMS] = {"", "player", "enemy", "neutral"};

// this list was made using the model directories, this MUST be in the same
// order as the CLASS_ enum in teams.h
char *ClassNames[CLASS_NUM_CLASSES] = {
    "", // class none
    "atst",       "bartender",    "bespin_cop",    "claw",
    "commando",   "desann",       "fish",          "flier2",
    "galak",      "glider",       "gonk",          "gran",
    "howler",     "imperial",     "impworker",     "interrogator",
    "jan",        "jedi",         "kyle",          "lando",
    "lizard",     "luke",         "mark1",         "mark2",
    "galak_mech", "minemonster",  "monmotha",      "morgankatarn",
    "mouse",      "murjj",        "prisoner",      "probe",
    "protocol",   "r2d2",         "r5d2",          "rebel",
    "reborn",     "reelo",        "remote",        "rodian",
    "seeker",     "sentry",       "shadowtrooper", "stormtrooper",
    "swamp",      "swamptrooper", "tavion",        "trandoshan",
    "ugnaught",   "weequay",      "bobafett",      "vehicle",
    "rancor",     "wampa",
};
...
int NPC_ReactionTime(void) { return 200 * (6 - NPCS.NPCInfo->stats.reactions); }
...
qboolean G_ParseAnimFileSet(const char *filename, const char *animCFG,
                            int *animFileIndex) {
  *animFileIndex = BG_ParseAnimationFile(filename, NULL, qfalse);
  // if it's humanoid we should have it cached and return it, if it is not it
  // will be loaded (unless it's also cached already)
...
void NPC_Precache(gentity_t *spawner) {
  ...
  p = NPCParms;
  Com_sprintf(sessionName, sizeof(sessionName), "NPC_Precache(%s)",
              spawner->NPC_type);
  ...
  if (!spawner->client || spawner->client->NPC_class != CLASS_VEHICLE) {
    ...
    } else { // if we have a model/skin then index them so they'll be registered
             // immediately
      // when the client gets a configstring update.
      char modelName[MAX_QPATH];

      Com_sprintf(modelName, sizeof(modelName), "models/players/%s/model.glm",
                  playerModel);
  ...
  NPC_PrecacheWeapons((team_t)playerTeam, spawner->spawnflags,
                      spawner->NPC_type);
...
extern void SetupGameGhoul2Model(gentity_t *ent, char *modelname,
                                 char *skinName);
qboolean NPC_ParseParms(const char *NPCName, gentity_t *NPC) {
  ...
  */
  ...
  if (!Q_stricmp("random", NPCName)) { // Randomly assemble a starfleet guy
    ...
    p = NPCParms;
    Com_sprintf(sessionName, sizeof(sessionName), "NPC_ParseParms(%s)",
                NPCName);
    COM_BeginParseSession(sessionName);
    ...
    while (1) {
      ...
      if (!Q_stricmp(token, "legsmodel")) {
        ...
        G_ParseAnimFileSet( ri->legsModelName, ri->legsModelName,
        &ci->animFileIndex );
        */
        continue;
      }
      ...
      if (!Q_stricmp(token, "scale")) {
        ...
        if (n != 100) {
          ...
          NPC->modelScale[0] = NPC->modelScale[1] = NPC->modelScale[2] =
              n / 100.0f;
        }
        continue;
      }
      ...
      if (!Q_stricmp(token, "playerTeam")) {
        ...
        Com_sprintf(tk, sizeof(tk), "NPC%s", token);
        NPC->client->playerTeam = NPC->s.teamowner =
            (team_t)GetIDForString(TeamTable, tk); // TranslateTeamName(value);
        continue;
      }
      ...
      if (!Q_stricmp(token, "enemyTeam")) {
        ...
        NPC->client->enemyTeam =
            GetIDForString(TeamTable, tk); // TranslateTeamName(value);
        continue;
      }
      ...
      if (!Q_stricmp(token, "class")) {
        ...
        NPC->s.NPC_class =
            NPC->client->NPC_class; // we actually only need this value now, but
                                    // at the moment I don't feel like changing
                                    // the 200+ references to client->NPC_class.
        ...
        if (NPC->client->NPC_class == CLASS_VEHICLE && NPC->m_pVehicle &&
            ...
            NPC->m_pVehicle->m_pVehicleInfo->type ==
                VH_FIGHTER) { // a flying vehicle's origin must be centered in
                              // bbox and it should spawn on the ground
          // trace_t		tr;
          // vec3_t		bottom;
          // float		adjust = 32.0f;
          ...
          bottom[2] -= adjust;
          trap->Trace( &tr, NPC->s.origin, NPC->r.mins, NPC->r.maxs, bottom,
          NPC->s.number, MASK_NPCSOLID ); if ( !tr.allsolid && !tr.startsolid )
          {
      ...
      if (!Q_stricmp(token, "weapon")) {
        ...
        if (weap >= WP_NONE &&
            weap <= WP_NUM_WEAPONS) ///*WP_BLASTER_PISTOL*/WP_SABER ) //?!
        {
          NPC->client->ps.weapon = weap;
          NPC->client->ps.stats[STAT_WEAPONS] |= (1 << NPC->client->ps.weapon);
          if (weap > WP_NONE) {
            //	RegisterItem( FindItemForWeapon(
            //(weapon_t)(NPC->client->ps.weapon) ) );	//precache the weapon
            NPC->client->ps.ammo[weaponData[NPC->client->ps.weapon].ammoIndex] =
                100; // FIXME: max ammo!
          }
        }
        continue;
      ...
      if (!Q_stricmp(token, "saber2")) {
        ...
        if (!(NPC->client->saber[0].saberFlags &
          ...
          if ((NPC->client->saber[1].saberFlags &
  ...
  if (!md3Model) {
    ...
    if (NPC->client &&
        NPC->client->NPC_class ==
            CLASS_VEHICLE) { // vehicles want their names fed in as models
    ...
    if (!NPC->NPC_type) { // just do this for now so NPC_Precache can see the
                          // name.
    ...
    NPC_Precache(
        NPC); // this will just soundindex some values for sounds on the client,
  ...
  */
  /*
  if(	NPCsPrecached )
  {//Spawning in after initial precache, our models are precached, we just need
  to set our clientInfo CG_RegisterClientModels( NPC->s.number );
          CG_RegisterNPCCustomSounds( ci );
...
void NPC_LoadParms(void) {
  ...
  *marker = 0;
  ...
  fileCnt = trap->FS_GetFileList("ext_data/NPCs", ".npc", npcExtensionListBuf,
  ...
  for (i = 0; i < fileCnt; i++, holdChar += npcExtFNLen + 1) {
    ...
    if (len == -1) {
      ...
      totallen += len;
      marker = NPCParms + totallen;
      //*marker = 0; //rww - make sure this is null or strcat will not append to
      //the correct place rww  12/19/02-actually the probelm was npcParseBuffer
      // not being nul-term'd, which could cause issues in the strcat too
    }
  }
}
