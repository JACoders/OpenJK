...
static void CG_DrawForcePower(centity_t *cent, int x, int y) {
	...
	for (i = MAX_TICS - 1; i >= 0; i--) {
		...
		CG_DrawPic(x + forceTicPos[i].x, y + forceTicPos[i].y, forceTicPos[i].width, forceTicPos[i].height, forceTicPos[i].tic);
...
static void CG_DrawAmmo(centity_t *cent, int x, int y) {
	...
	for (i = MAX_TICS - 1; i >= 0; i--) {
		...
		cgi_R_SetColor(calcColor);
		CG_DrawPic(x + ammoTicPos[i].x, y + ammoTicPos[i].y, ammoTicPos[i].width, ammoTicPos[i].height, ammoTicPos[i].tic);
...
static void CG_ScanForCrosshairEntity(qboolean scanAll) {
	...
	{														// try to check for force-affectable stuff first
		...
		gi.trace(&trace, start, vec3_origin, vec3_origin, end, ignoreEnt, MASK_OPAQUE | CONTENTS_SHOTCLIP | CONTENTS_BODY | CONTENTS_ITEM, G2_NOCOLLIDE,
				 10); // ); took out CONTENTS_SOLID| so you can target people through glass.... took out CONTENTS_CORPSE so disintegrated guys aren't shown,
					  // could just remove their body earlier too...
	...
	if (!cg_forceCrosshair) {
		if (cg_dynamicCrosshair.integer) { // 100% accurate
			vec3_t d_f, d_rt, d_up;
			if (cg.snap->ps.weapon == WP_NONE || cg.snap->ps.weapon == WP_SABER || cg.snap->ps.weapon == WP_STUN_BATON) {
