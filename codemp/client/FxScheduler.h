...
public:
  ...
  inline float GetVal(float fraction) const {
    if (mMin != mMax) {
      return mMin + fraction * (mMax - mMin);
    } else {
      return mMin;
    }
  }
...
#define MAX_LOOPED_FX 32
  ...
  int ScheduleLoopedEffect(int id, int boltInfo, CGhoul2Info_v *ghoul2,
  ...
  public:
    CScheduled2DEffect()
        : mScreenX(0), mScreenY(0), mWidth(0), mHeight(0), mShaderHandle(0) {
  ...
  void CreateEffect(CPrimitiveTemplate *fx, const vec3_t origin, matrix3_t axis,
                    int lateTime, int fxParm = -1, CGhoul2Info_v *ghoul2 = NULL,
...
public:
  ...
  // rww - maybe this should be done differently.. it's more than a bit
  // confusing. Remind me when I don't have 50 files checked out.
  void
  PlayEffect(int id, vec3_t org, vec3_t fwd, int vol = -1, int rad = -1,
             bool isPortal = false); // builds arbitrary perp. right vector,
                                     // does a cross product to define up
  void PlayEffect(int id, vec3_t origin, matrix3_t axis,
                  const int boltInfo = -1, CGhoul2Info_v *ghoul2 = NULL,
                  int fxParm = -1, int vol = -1, int rad = -1,
                  bool isPortal = false, int iLoopTime = false,
                  bool isRelative = false);
  void PlayEffect(const char *file, vec3_t org, int vol = -1,
                  int rad = -1); // uses a default up axis
  void PlayEffect(const char *file, vec3_t org, vec3_t fwd, int vol = -1,
                  int rad = -1); // builds arbitrary perp. right vector, does a
                                 // cross product to define up
  void PlayEffect(const char *file, vec3_t origin, matrix3_t axis,
                  const int boltInfo = -1, CGhoul2Info_v *ghoul2 = NULL,
                  int fxParm = -1, int vol = -1, int rad = -1,
  ...
  bool Add2DEffect(float x, float y, float w, float h, vec4_t color,
  ...
  int GetHighWatermark() const {
    return mScheduledEffectsPool.GetHighWatermark();
  }
