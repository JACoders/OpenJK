...
static void CG_RegisterCustomSounds(clientInfo_t *ci, int iSoundEntryBase, int iTableEntries, const char *ppsTable[], const char *psDir) {
...
void CG_ParseAnimationSndFile(const char *as_filename, int animFileIndex) {
	...
	if (level.knownAnimFileSets[animFileIndex].soundsCached) { // already cached this one
		return;
	}
...
===============
*/
void CG_PlayerAnimation(centity_t *cent, int *legsOld, int *legs, float *legsBackLerp, int *torsoOld, int *torso, float *torsoBackLerp) {
	clientInfo_t *ci;
	int legsAnim;
	int legsTurnAnim = -1;
...
void CG_PlayerAnimSounds(int animFileIndex, qboolean torso, int oldFrame, int frame, int entNum) {
	...
	if (fabs((double)oldFrame - frame) > 1 && cg_reliableAnimSounds.integer) { // given a range, see if keyFrame falls in that range
		int oldAnim, anim;
		if (torso) {
	...
	for (i = 0; i < MAX_ANIM_SOUNDS; ++i) {
		...
		if (animSounds[i].keyFrame == frame) { // exact match
			match = qtrue;
		} else if (fabs((double)oldFrame - frame) > 1 && cg_reliableAnimSounds.integer) { // given a range, see if keyFrame falls in that range
			if (inSameAnim) {															  // if changed anims altogether, sorry, the sound is lost
				if (fabs((double)oldFrame - animSounds[i].keyFrame) <= 3 ||
					fabs((double)frame - animSounds[i].keyFrame) <= 3) {						   // must be at least close to the keyframe
					if (animBackward) {															   // animation plays backwards
						if (oldFrame > animSounds[i].keyFrame && frame < animSounds[i].keyFrame) { // old to new passed through keyframe
							match = qtrue;
						} else if (loopAnim) { // hmm, didn't pass through it linearally, see if we looped
							if (animSounds[i].keyFrame >= firstFrame && animSounds[i].keyFrame < lastFrame) { // keyframe is in this anim
								if (oldFrame > animSounds[i].keyFrame && frame > oldFrame) {				  // old to new passed through keyframe
									match = qtrue;
								}
							}
						}
...
void CGG2_AnimSounds(centity_t *cent) {
	...
	if (ValidAnimFileIndex(cent->gent->client->clientInfo.animFileIndex)) {
		...
		if (cent->gent->rootBone >= 0 && gi.G2API_GetBoneAnimIndex(&cent->gent->ghoul2[cent->gent->playerModel], cent->gent->rootBone, cg.time, &currentFrame,
		...
		if (curFrame != cent->gent->client->renderInfo.legsFrame) {
			CG_PlayerAnimSounds(cent->gent->client->clientInfo.animFileIndex, qfalse, cent->gent->client->renderInfo.legsFrame, curFrame,
								cent->currentState.clientNum);
		}
		cent->gent->client->renderInfo.legsFrame = curFrame;
		cent->pe.legs.frame = curFrame;

		if (cent->gent->lowerLumbarBone >= 0 && gi.G2API_GetBoneAnimIndex(&cent->gent->ghoul2[cent->gent->playerModel], cent->gent->lowerLumbarBone, cg.time,
																		  &currentFrame, &junk, &junk, &junk, &animSpeed, cgs.model_draw)) {
			curFrame = floor(currentFrame);
		}
		if (curFrame != cent->gent->client->renderInfo.torsoFrame) {
			CG_PlayerAnimSounds(cent->gent->client->clientInfo.animFileIndex, qtrue, cent->gent->client->renderInfo.torsoFrame, curFrame,
								cent->currentState.clientNum);
		}
...
==================
*/
void CG_SwingAngles(float destAngle, float swingTolMin, float swingTolMax, float clampMin, float clampMax, float angleSpeed, float *curAngle,
					qboolean *turning) {
	float swing;
	float move;
	float scale;
...
qboolean CG_PlayerLegsYawFromMovement(centity_t *cent, const vec3_t velocity, float *yaw, float fwdAngle, float swingTolMin, float swingTolMax,
									  qboolean alwaysFace) {
...
void CG_G2ClientNeckAngles(centity_t *cent, const vec3_t lookAngles, vec3_t headAngles, vec3_t neckAngles, vec3_t thoracicAngles, vec3_t headClampMinAngles,
...
void CG_UpdateLookAngles(centity_t *cent, vec3_t lookAngles, float lookSpeed, float minPitch, float maxPitch, float minYaw, float maxYaw, float minRoll,
						 float maxRoll) {
	if (!cent || !cent->gent || !cent->gent->client) {
		return;
	}
	...
	if (cent->gent && cent->gent->client && (cent->gent->client->NPC_class != CLASS_GONK) && (cent->gent->client->NPC_class != CLASS_INTERROGATOR) &&
		...
		if (cent->gent->client) {
			if (cent->gent->client->NPC_class != CLASS_ATST) {
				// FIXME: use actual swing/clamp tolerances?
				if (cent->gent->client->ps.groundEntityNum != ENTITYNUM_NONE && !PM_InRoll(&cent->gent->client->ps)) { // on the ground
		...
		if (cent->gent && cent->gent->client && cent->gent->client->NPC_class == CLASS_ATST) {
			...
			if (cg_turnAnims.integer && !in_camera && cent->gent->hipsBone >= 0) {
				...
				const qboolean animatingHips =
					gi.G2API_GetAnimRangeIndex(&cent->gent->ghoul2[cent->gent->playerModel], cent->gent->hipsBone, &startFrame, &endFrame);
				...
				if (angles[YAW] == cent->pe.legs.yawAngle) {
					...
					if (turnAnim != -1 && cent->gent->health > 0) {
							...
							(animations[turnAnim].firstFrame != startFrame)) // only set the anim if we aren't going to do the same animation again
						{
							...
							gi.G2API_SetBoneAnimIndex(&cent->gent->ghoul2[cent->gent->playerModel], cent->gent->hipsBone, animations[turnAnim].firstFrame,
...
void CG_PlayerAngles(centity_t *cent, vec3_t legs[3], vec3_t torso[3], vec3_t head[3]) {
	...
	if (cent->gent->client->renderInfo.renderFlags & RF_LOCKEDANGLE) {
		...
		if (cent->gent && cent->gent->health > 0) {
			...
			CG_SwingAngles(legsAngles[YAW], legsYawSwingTolMin, legsYawSwingTolMax, torsoYawClampMin, torsoYawClampMax, maxYawSpeed, &cent->pe.legs.yawAngle,
						   &cent->pe.legs.yawing);
			legsAngles[YAW] = cent->pe.legs.yawAngle;
			if (cent->gent->client) {
				cent->gent->client->renderInfo.legsYaw = legsAngles[YAW];
	...
	*/
	...
	CG_SwingAngles(headAngles[YAW], torsoYawSwingTolMin, torsoYawSwingTolMax, headYawClampMin, headYawClampMax, yawSpeed, &cent->pe.torso.yawAngle,
	...
	CG_SwingAngles(dest, torsoPitchSwingTolMin, torsoPitchSwingTolMax, torsoPitchClampMin, torsoPitchClampMax, 0.1f, &cent->pe.torso.pitchAngle,
...
void _PlayerSplash(const vec3_t origin, const vec3_t velocity, const float radius, const int maxUp) {
	...
	FX_AddOrientedParticle(end, trace.plane.normal, NULL, NULL, 6.0f, radius + Q_flrand(0.0f, 1.0f) * 48.0f, 0, alpha, 0.0f, 0.0f, WHITE, WHITE, 0.0f,
...
static void CG_ForceElectrocution(centity_t *cent, const vec3_t origin, vec3_t tempAngles) {
	...
	if (tr.fraction < 1.0f || Q_flrand(0.0f, 1.0f) > 0.94f) {
		FX_AddElectricity(fxOrg, tr.endpos, 1.5f, 4.0f, 0.0f, 1.0f, 0.5f, 0.0f, rgb, rgb, 0.0f, 5.5f, Q_flrand(0.0f, 1.0f) * 50 + 100, cgs.media.boltShader,
...
void CG_AddRefEntityWithPowerups(refEntity_t *ent, int powerups, centity_t *cent) {
	...
	//------------------------------------------------------
	if (gent->client->ps.forcePowersActive & (1 << FP_SPEED) && (gent->s.number || cg.renderingThirdPerson)) // looks dumb doing this with first peron mode on
	{
		localEntity_t *ex;
	...
	if (powerups & (1 << PW_GALAK_SHIELD)) {
		...
		gi.G2API_GetBoltMatrix(cent->gent->ghoul2, gent->playerModel, gent->genericBolt1, &boltMatrix, angles, cent->lerpOrigin, cg.time, cgs.model_draw,
							   cent->currentState.modelScale);
		gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, tent.origin); // pass in the emitter origin here

		tent.endTime =
			gent->fx_time + 1000; // if you want the shell to build around the guy, pass in a time that is 1000ms after the start of the turn-on-effect
...
int CG_PlayerHeadExtension(centity_t *cent, refEntity_t *head) {
	...
	if (ci->extensions && (gi.VoiceVolume[cent->gent->s.clientNum] > 0)) { // FIXME: When talking, look at talkTarget, if any
		// ALSO: When talking, add a head bob/movement on syllables - when gi.VoiceVolume[] changes drastically

		if (cent->gent->health <= 0) { // Dead people close their eyes and don't make faces!  They also tell no tales...  BUM BUM BAHHHHHHH!
		...
		if (ci->extensions) {
			...
			if (!add_in) {
				if (gi.VoiceVolume[cent->gent->s.clientNum] == -1) { // then we're talking and don't want to use blinking normal frames, force open eyes.
					add_in = 0;
					// reset blink timer
					cent->gent->client->facial_blink = cg.time + Q_flrand(3000.0, 5000.0);
				}
...
void CG_StopWeaponSounds(centity_t *cent) {
	...
	if (cent->currentState.weapon == WP_SABER) {
		...
		cgi_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin, vec3_origin, CG_SaberHumSoundForEnt(&g_entities[cent->currentState.clientNum]));
		return;
	}
...
void CG_CheckSaberInWater(centity_t *cent, centity_t *scent, int modelIndex, vec3_t origin, vec3_t angles) {
	...
	if ((contents & CONTENTS_WATER) || (contents & CONTENTS_SLIME)) { // still in water
									...
									// if we hit a corner, blade is still 8 longer than hit
		{
			...
			if (trace.fraction < 1.0f) {
				if ((trace.contents & CONTENTS_WATER) || (trace.contents & CONTENTS_SLIME)) {
					...
					*/
					...
					if (client->saberTrail.haveOldPos[i]) {
						if (trace.entityNum == ENTITYNUM_WORLD) { // only put marks on architecture
				...
				if (!i) {
					...
					if (cent->gent->client->ps.saberLength <= 0.1f) { // SIGH... hack so it doesn't play the saber turn-on sound that plays when you first turn
																	  // the saber on (assumed when saber is active but length is zero)
...
void CG_Player(centity_t *cent) {
	...
	if (cent->currentState.vehicleModel != 0) { // Using an alternate (md3 for now) model
		refEntity_t ent;
	...
	*/
	...
	//	if (1)
	{
		...
		if (!cg.renderingThirdPerson && (cg.snap->ps.weapon == WP_SABER || cg.snap->ps.weapon == WP_MELEE) &&
			!cent->gent->s.number) {													 // Yeah um, this needs to not do this quite this way
		...
		if (cent->gent && cent->gent->client && (cent->currentState.weapon == WP_SABER || cent->currentState.saberInFlight) &&
			...
			if (/*!cent->currentState.saberActive ||*/ !cent->gent->client->ps.saberActive ||
				cent->gent->client->ps.saberLength > cent->gent->client->ps.saberLengthMax) // hack around network lag for now
			{																				// saber is off
			...
			} else if (cent->gent->client->ps.saberLength < cent->gent->client->ps.saberLengthMax) { // saber is on
				if (!cent->gent->client->ps.saberLength) {
					...
					if (!cent->gent->client->ps.weaponTime) { // make us play the turn on anim
		...
		if (cent->currentState.number != 0 || cg.renderingThirdPerson || cg.snap->ps.stats[STAT_HEALTH] <= 0 ||
			...
			*/
			...
			if (!calcedMp) {
				...
				} else if ((cent->gent->weaponModel != -1) && (cent->gent->ghoul2.size() > cent->gent->weaponModel) &&
						   (cent->gent->ghoul2[cent->gent->weaponModel].mModelindex != -1)) {
					mdxaBone_t boltMatrix;
					// figure out where the actual model muzzle is
					gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->weaponModel, 0, &boltMatrix, tempAngles, ent.origin, cg.time, cgs.model_draw,
										   cent->currentState.modelScale);
					// work the matrix axis stuff into the original axis and origins used.
					gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, cent->gent->client->renderInfo.muzzlePoint);
					gi.G2API_GiveMeVectorFromMatrix(boltMatrix, NEGATIVE_Y, cent->gent->client->renderInfo.muzzleDir);
			...
			if (cent->gent->NPC && (cent->gent->NPC->confusionTime > cg.time || cent->gent->NPC->charmedTime > cg.time ||
									cent->gent->NPC->controlledTime > cg.time)) { // we are currently confused, so play an effect at the headBolt position
			...
			if (cent->gent->client->ps.powerups[PW_FORCE_PUSH] > cg.time ||
				(cent->gent->client->ps.forcePowersActive & (1 << FP_GRIP))) { // doing the pushing/gripping
			...
			if (cent->gent->client->ps.forcePowersActive & (1 << FP_LIGHTNING)) { // doing the electrocuting
				vec3_t tAng, fxDir;
		...
		if (torso.hModel) {
			...
			if (gun.hModel) {
				...
				if (cent->gent && cent->gent->client && (cent->currentState.weapon == WP_SABER || cent->currentState.saberInFlight)) {
					...
					{										 // saber is off
						...
						if (!cent->gent->client->ps.saberLength) {
							if (cent->currentState.saberInFlight) { // play it on the saber
								...
								cgi_S_StartSound(NULL, cent->gent->client->ps.saberEntityNum, CHAN_AUTO,
					...
					if (cent->currentState.saberInFlight) { // not holding the saber in-hand
						drawGun = qfalse;
					}
					if (cent->gent->client->ps.saberLength > 0) {
						if (!cent->currentState.saberInFlight) { // holding the saber in-hand
				...
				if (drawGun) {
					CG_AddRefEntityWithPowerups(&gun, (cent->currentState.powerups & ((1 << PW_CLOAKED) | (1 << PW_BATTLESUIT))), cent);
				}
