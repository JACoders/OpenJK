...
extern qboolean WP_SaberBladeUseSecondBladeStyle(saberInfo_t *saber,
                                                 int bladeNum);
extern qboolean WP_SaberBladeDoTransitionDamage(saberInfo_t *saber,
                                                int bladeNum);
...
extern void CG_SaberDoWeaponHitMarks(gclient_t *client, gentity_t *saberEnt,
                                     gentity_t *hitEnt, int saberNum,
                                     int bladeNum, vec3_t hitPos, vec3_t hitDir,
                                     vec3_t uaxis, vec3_t splashBackDir,
                                     float sizeTimeScale);
extern void G_AngerAlert(gentity_t *self);
extern void G_ReflectMissile(gentity_t *ent, gentity_t *missile,
                             vec3_t forward);
extern int G_CheckLedgeDive(gentity_t *self, float checkDist,
                            const vec3_t checkVel, qboolean tryOpposite,
                            qboolean tryPerp);
...
extern void G_MissileImpacted(gentity_t *ent, gentity_t *other,
                              vec3_t impactPos, vec3_t normal,
                              int hitLoc = HL_NONE);
extern evasionType_t Jedi_SaberBlockGo(gentity_t *self, usercmd_t *cmd,
                                       vec3_t pHitloc, vec3_t phitDir,
...
extern qboolean Jedi_DodgeEvasion(gentity_t *self, gentity_t *shooter,
                                  trace_t *tr, int hitLoc);
...
extern void G_KnockOffVehicle(gentity_t *pRider, gentity_t *self,
                              qboolean bPull);
extern qboolean PM_LockedAnim(int anim);
extern qboolean Rosh_BeingHealed(gentity_t *self);
extern qboolean G_OkayToLean(playerState_t *ps, usercmd_t *cmd,
...
qboolean WP_ForcePowerAvailable(gentity_t *self, forcePowers_t forcePower,
                                int overrideAmt);
void WP_ForcePowerDrain(gentity_t *self, forcePowers_t forcePower,
                        int overrideAmt);
...
int forcePowerNeeded[NUM_FORCE_POWERS] = {
    ...
    0,  // FP_SABER_OFFENSE,
    // new Jedi Academy powers
    50, // FP_RAGE,//duration - speed, invincibility and extra damage for short
        // period, drains your health and leaves you weak and slow afterwards.
    30, // FP_PROTECT,//duration - protect against physical/energy (level 1
        // stops blaster/energy bolts, level 2 stops projectiles, level 3
        // protects against explosions)
    30, // FP_ABSORB,//duration - protect against dark force powers (grip,
        // lightning, drain)
...
float forcePushCone[NUM_FORCE_POWER_LEVELS] = {1.0f, // none
                                               1.0f, 0.8f, 0.6f};

float forcePullCone[NUM_FORCE_POWER_LEVELS] = {1.0f, // none
                                               1.0f, 1.0f, 0.8f};

float forceSpeedValue[NUM_FORCE_POWER_LEVELS] = {1.0f, // none
                                                 0.75f, 0.5f, 0.25f};

float forceSpeedRangeMod[NUM_FORCE_POWER_LEVELS] = {0.0f, // none
                                                    30.0f, 45.0f, 60.0f};

float forceSpeedFOVMod[NUM_FORCE_POWER_LEVELS] = {0.0f, // none
                                                  20.0f, 30.0f, 40.0f};

int forceGripDamage[NUM_FORCE_POWER_LEVELS] = {0, // none
                                               0, 6, 9};
...
int saberThrowDist[NUM_FORCE_POWER_LEVELS] = {0, // none
                                              256, 400, 400};

// NOTE: keep in synch with table above!!!
int saberThrowDistSquared[NUM_FORCE_POWER_LEVELS] = {0, // none
                                                     65536, 160000, 160000};
...
stringID_table_t SaberStyleTable[] = {
    {"NULL", SS_NONE},     ENUM2STRING(SS_FAST),
    {"fast", SS_FAST},     ENUM2STRING(SS_MEDIUM),
    {"medium", SS_MEDIUM}, ENUM2STRING(SS_STRONG),
    {"strong", SS_STRONG}, ENUM2STRING(SS_DESANN),
    {"desann", SS_DESANN}, ENUM2STRING(SS_TAVION),
    {"tavion", SS_TAVION}, ENUM2STRING(SS_DUAL),
    {"dual", SS_DUAL},     ENUM2STRING(SS_STAFF),
    {"staff", SS_STAFF},   {"", 0},
};
...
void G_CreateG2AttachedWeaponModel(gentity_t *ent, const char *psWeaponModel,
  ...
  if (ent && ent->client &&
      ent->client->NPC_class ==
          CLASS_GALAKMECH) { // hack for galakmech, no weaponmodel
  ...
  if (wModelIndex) {
    ent->weaponModel[weaponNum] = gi.G2API_InitGhoul2Model(
        ent->ghoul2, weaponModel, wModelIndex, NULL_HANDLE, NULL_HANDLE, 0, 0);
    if (ent->weaponModel[weaponNum] != -1) {
      // attach it to the hand
      gi.G2API_AttachG2Model(&ent->ghoul2[ent->weaponModel[weaponNum]],
                             &ent->ghoul2[ent->playerModel], boltNum,
                             ent->playerModel);
      // set up a bolt on the end so we can get where the sabre muzzle is - we
      // can assume this is always bolt 0
...
void WP_SaberAddG2SaberModels(gentity_t *ent, int specificSaberNum) {
  ...
  for (; saberNum <= maxSaber; saberNum++) {
    if (ent->weaponModel[saberNum] >
        0) { // we already have a weapon model in this slot
      // remove it
    ...
    if (saberNum > 0) { // second saber
      if (!ent->client->ps.dualSabers ||
          G_IsRidingVehicle(ent)) { // only have one saber or riding a vehicle
                                    // and can only use one saber
        return;
      }
    } else if (saberNum == 0) { // first saber
      if (ent->client->ps
              .saberInFlight) { // it's still out there somewhere, don't add it
        // FIXME: call it back?
        continue;
      }
    }
    int handBolt = ((saberNum == 0) ? ent->handRBolt : ent->handLBolt);
    if ((ent->client->ps.saber[saberNum].saberFlags &
         SFL_BOLT_TO_WRIST)) { // special case, bolt to forearm
      if (saberNum == 0) {
        handBolt = gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel],
                                    "*r_hand_cap_r_arm");
      } else {
        handBolt = gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel],
                                    "*l_hand_cap_l_arm");
    ...
    G_CreateG2AttachedWeaponModel(ent, ent->client->ps.saber[saberNum].model,
                                  handBolt, saberNum);

    if (ent->client->ps.saber[saberNum].skin !=
        NULL) { // if this saber has a customSkin, use it
      ...
      if (saberSkin) {
        ...
        gi.G2API_SetSkin(&ent->ghoul2[ent->weaponModel[saberNum]],
                         G_SkinIndex(ent->client->ps.saber[saberNum].skin),
                         saberSkin);
      }
    }
  }
...
//----------------------------------------------------------
{
  ...
  if (targ && targ->client &&
      (targ->client->NPC_class == CLASS_ATST ||
       ...
       targ->client->NPC_class ==
           CLASS_SAND_CREATURE)) { // much to large to *ever* throw
    return;
  }
  ...
  if (g_gravity->value > 0) {
    ...
    if (!targ->client || targ->client->ps.groundEntityNum !=
                             ENTITYNUM_NONE) { // give them some z lift to get
                                               // them off the ground
...
int WP_SetSaberModel(gclient_t *client,
                     class_t npcClass) { // FIXME: read from NPCs.cfg
  if (client) {
    switch (npcClass) {
...
void WP_SetSaberEntModelSkin(gentity_t *ent, gentity_t *saberent) {
  ...
  if (saberModel && saberent->s.modelindex != saberModel) {
    if (saberent->playerModel >= 0) { // remove the old one, if there is one
      gi.G2API_RemoveGhoul2Model(saberent->ghoul2, saberent->playerModel);
    }
    // add the new one
    saberent->playerModel = gi.G2API_InitGhoul2Model(
        saberent->ghoul2, ent->client->ps.saber[0].model, saberModel,
  ...
  } else { // if this saber has a customSkin, use it
    // lets see if it's out there
    int saberSkin = gi.RE_RegisterSkin(ent->client->ps.saber[0].skin);
    if (saberSkin && (newModel || saberent->s.modelindex2 != saberSkin)) {
      // put it in the config strings
      // and set the ghoul2 model to use it
      gi.G2API_SetSkin(&saberent->ghoul2[0],
                       G_SkinIndex(ent->client->ps.saber[0].skin), saberSkin);
...
void WP_SaberFallSound(gentity_t *owner, gentity_t *saber) {
  ...
  if (owner &&
      owner->client) { // have an owner, use their data (assume saberNum is 0
                       // because only the 0 saber can be thrown)
    if (owner->client->ps.saber[0].fallSound[0]) { // have an override
      G_Sound(saber, owner->client->ps.saber[0].fallSound[Q_irand(0, 2)]);
    } else if (owner->client->ps.saber[0].type ==
               SABER_SITH_SWORD) { // is a sith sword
      G_Sound(saber, G_SoundIndex(
                         va("sound/weapons/sword/fall%d.wav", Q_irand(1, 7))));
    } else { // normal saber
      G_Sound(saber, G_SoundIndex(va("sound/weapons/saber/bounce%d.wav",
                                     Q_irand(1, 3))));
    }
  } else if (saber->NPC_type &&
             saber->NPC_type[0]) { // have a saber name to look up
    saberInfo_t saberInfo;
    if (WP_SaberParseParms(saber->NPC_type, &saberInfo)) { // found it
      if (saberInfo.fallSound[0]) { // have an override sound
        G_Sound(saber, saberInfo.fallSound[Q_irand(0, 2)]);
      } else if (saberInfo.type == SABER_SITH_SWORD) { // is a sith sword
        G_Sound(saber, G_SoundIndex(va("sound/weapons/sword/fall%d.wav",
                                       Q_irand(1, 7))));
      } else { // normal saber
        G_Sound(saber, G_SoundIndex(va("sound/weapons/saber/bounce%d.wav",
                                       Q_irand(1, 3))));
  ...
  } else { // no saber name specified
    G_Sound(saber, G_SoundIndex(
                       va("sound/weapons/saber/bounce%d.wav", Q_irand(1, 3))));
  }
}
...
void WP_SaberSwingSound(gentity_t *ent, int saberNum, swingType_t swingType) {
  ...
  if (ent->client->ps.saber[saberNum].swingSound[0]) {
    G_SoundIndexOnEnt(
        ent, CHAN_WEAPON,
        ent->client->ps.saber[saberNum].swingSound[Q_irand(0, 2)]);
  } else if (ent->client->ps.saber[saberNum].type == SABER_SITH_SWORD) {
    G_SoundOnEnt(ent, CHAN_WEAPON,
                 va("sound/weapons/sword/swing%d.wav", Q_irand(1, 4)));
  } else {
    G_SoundOnEnt(ent, CHAN_WEAPON,
                 va("sound/weapons/saber/saberhup%d.wav", index));
  }
}
...
void WP_SaberBounceSound(gentity_t *ent, gentity_t *hitEnt,
                         gentity_t *playOnEnt, int saberNum, int bladeNum,
                         qboolean doForce) {
  int index = 1;
  if (!ent || !ent->client) {
    return;
  ...
  // NOTE: we don't allow overriding of the saberbounce sound, but since it's
  // just a variant on the saberblock sound, we use that as the override
  if (!WP_SaberBladeUseSecondBladeStyle(&ent->client->ps.saber[saberNum],
                                        bladeNum) &&
      ent->client->ps.saber[saberNum].blockSound[0]) {
    G_Sound(playOnEnt,
            ent->client->ps.saber[saberNum].blockSound[Q_irand(0, 2)]);
  } else if (WP_SaberBladeUseSecondBladeStyle(&ent->client->ps.saber[saberNum],
                                              bladeNum) &&
             ent->client->ps.saber[saberNum].block2Sound[0]) {
    G_Sound(playOnEnt,
            ent->client->ps.saber[saberNum].block2Sound[Q_irand(0, 2)]);
  } else {
    G_Sound(playOnEnt,
            G_SoundIndex(va("sound/weapons/saber/saberbounce%d.wav", index)));
  }
}
  ...
  if (1) {
    ...
    for (int saberNum = 0; saberNum < MAX_SABERS; saberNum++) {
      for (int bladeNum = 0; bladeNum < MAX_BLADES; bladeNum++) {
        VectorClear(
            ent->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint);
        VectorClear(
        ...
        ent->client->ps.saber[saberNum].blade[bladeNum].lengthOld =
            ent->client->ps.saber[saberNum].blade[bladeNum].length = 0;
    ...
    if (ent->s.number) {
      if (!ent->client->ps.saberAnimLevel) {
        ...
        // FIXME: CLASS_CULTIST instead of this Q_stricmpn?
        else if (!Q_stricmpn("cultist", ent->NPC_type,
                             7)) { // should already be set in the .npc file
          ent->client->ps.saberAnimLevel = Q_irand(SS_FAST, SS_STRONG);
                   ...
                   (ent->NPC->rank == RANK_CIVILIAN ||
                    ent->NPC->rank == RANK_LT_JG)) { // grunt and fencer always
                                                     // uses quick attacks
          ent->client->ps.saberAnimLevel = SS_FAST;
        } else if (ent->NPC && ent->client->playerTeam == TEAM_ENEMY &&
                   (ent->NPC->rank == RANK_CREWMAN ||
                    ent->NPC->rank ==
                        RANK_ENSIGN)) { // acrobat & force-users always use
                                        // medium attacks
          ent->client->ps.saberAnimLevel = SS_MEDIUM;
        } else if (ent->client->playerTeam == TEAM_ENEMY &&
                   ent->client->NPC_class ==
                       CLASS_SHADOWTROOPER) { // shadowtroopers
          ent->client->ps.saberAnimLevel = Q_irand(SS_FAST, SS_STRONG);
        } else if (ent->NPC && ent->client->playerTeam == TEAM_ENEMY &&
                   ent->NPC->rank ==
                       RANK_LT) { // boss always starts with strong attacks
      ...
      if (ent->client->sess.missionStats.weaponUsed[WP_SABER] <=
          0) { // let missionStats know that we actually do have the saber, even
               // if we never use it
    ...
    if (ent->client->ps.saberEntityNum <= 0 ||
        ent->client->ps.saberEntityNum >=
            ENTITYNUM_WORLD) { // FIXME: if you do have a saber already, be sure
                               // to re-set the model if it's changed (say, via a
                               // script).
      ...
      */
      ...
      if (ent->client->ps.dualSabers) {
        ...
        // gi.G2API_InitGhoul2Model( saberent->ghoul2,
        // ent->client->ps.saber[1].model, saber2 );
        // set up a bolt on the end so we can get where the sabre muzzle is - we
        // can assume this is always bolt 0
        // gi.G2API_AddBolt( &saberent->ghoul2[0], "*flash" );
        // gi.G2API_SetLodBias( &saberent->ghoul2[0], 0 );
      ...
      */
      ...
      // FIXME: need a think function to create alerts when turned on or is on,
      // etc.
    } else { // already have one, might just be changing sabers, register the
             // model and skin and use them if different from what we're using
             // now.
...
void WP_SaberUpdateOldBladeData(gentity_t *ent) {
  if (ent->client) {
    ...
    for (int saberNum = 0; saberNum < 2; saberNum++) {
      for (int bladeNum = 0;
           bladeNum < ent->client->ps.saber[saberNum].numBlades; bladeNum++) {
        VectorCopy(
            ent->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint,
            ent->client->ps.saber[saberNum].blade[bladeNum].muzzlePointOld);
        VectorCopy(
            ent->client->ps.saber[saberNum].blade[bladeNum].muzzleDir,
            ent->client->ps.saber[saberNum].blade[bladeNum].muzzleDirOld);
        if (!didEvent) {
          if (ent->client->ps.saber[saberNum].blade[bladeNum].lengthOld <= 0 &&
              ent->client->ps.saber[saberNum].blade[bladeNum].length >
                  0) { // just turned on
            // do sound event
            vec3_t saberOrg;
            ...
            if ((!ent->client->ps.saberInFlight &&
                 ent->client->ps.groundEntityNum ==
                     ENTITYNUM_WORLD) // holding saber and on ground
                || g_entities[ent->client->ps.saberEntityNum].s.pos.trType ==
                       TR_STATIONARY) // saber out there somewhere and on ground
            {                         // a ground alert
              AddSoundEvent(ent, saberOrg, 256, AEL_SUSPICIOUS, qfalse, qtrue);
            } else { // an in-air alert
    ...
    VectorCopy(ent->client->renderInfo.muzzlePoint,
               ent->client->renderInfo.muzzlePointOld);
    VectorCopy(ent->client->renderInfo.muzzleDir,
               ent->client->renderInfo.muzzleDirOld);
  }
}
...
qboolean WP_GetSaberDeflectionAngle(gentity_t *attacker, gentity_t *defender) {
  vec3_t temp, att_SaberBase, att_StartPos, saberMidNext, att_HitDir,
      att_HitPos, def_BladeDir;
  ...
  VectorMA(attacker->client->renderInfo.muzzlePointOld, saberHitFraction, temp,
           att_SaberBase);

  // get the position along the length of the blade where the hit occured
  att_SaberHitLength = Distance(saberHitLocation, att_SaberBase) /
                       attacker->client->ps.SaberLength();

  // now get the start of that midpoint in the swing and the actual impact point
  // in the swing (shouldn't the latter just be saberHitLocation?)
  ...
  VectorMA(defender->client->renderInfo.muzzleDirOld, saberHitFraction, temp,
           def_BladeDir);
  ...
  if (hitDot < 0.25f &&
    ...
    // normalize the result to determine what direction our saber should bounce
    // back toward
    ...
    if (swingRDot > 0.25f) {   // deflect to right
      ...
      } else { // deflect horizontally?  Well, no such thing as straight back in
               // my face, so use top
        if (swingRDot > 0) {
          attacker->client->ps.saberBounceMove = LS_D1_TR;
        } else if (swingRDot < 0) {
    ...
    if (d_saberCombat->integer) {
      gi.Printf(S_COLOR_BLUE "%s deflected from %s to %s\n",
                attacker->targetname,
                saberMoveData[attacker->client->ps.saberMove].name,
                saberMoveData[attacker->client->ps.saberBounceMove].name);
    }
#endif
    return qtrue;
  }
...
void WP_SaberClearDamageForEntNum(gentity_t *attacker, int entityNum,
  ...
  // FIXME: if hit their saber in WP_SaberDamageForTrace, need to still do
  // knockback on them...
  float knockBackScale = 0.0f;
  if (attacker && attacker->client) {
    if (!WP_SaberBladeUseSecondBladeStyle(&attacker->client->ps.saber[saberNum],
                                          bladeNum) &&
    ...
    if (victimEntityNum[i] == entityNum) {
      ...
      if (knockBackScale) {
        ...
        if (victim && victim->client) {
          ...
          float knockDownThreshHold,
              knockback = knockBackScale * totalDmg[i] * 0.5f;
          ...
          if (victim->client->ps.groundEntityNum != ENTITYNUM_NONE &&
              dirToCenter[2] <=
                  0) { // hit downward on someone who is standing on firm
                       // ground, so more likely to knock them down
...
qboolean WP_SaberApplyDamage(gentity_t *ent, float baseDamage, int baseDFlags,
                             qboolean brokenParry, int saberNum, int bladeNum,
                             qboolean thrownSaber) {
  ...
  for (int i = 0; i < numVictims; i++) {
    ...
    if (victimEntityNum[i] != ENTITYNUM_NONE &&
        &g_entities[victimEntityNum[i]] !=
            NULL) { // Don't bother with this damage if the fraction is higher
                    // than the saber's fraction
      if (dmgFraction[i] < saberHitFraction || brokenParry) {
        ...
        if (victim->e_DieFunc ==
            dieF_maglock_die) { //*sigh*, special check for maglocks
          vec3_t testFrom;
          if (ent->client->ps.saberInFlight) {
            VectorCopy(g_entities[ent->client->ps.saberEntityNum].currentOrigin,
                       testFrom);
          ...
          trace_t testTrace;
          gi.trace(&testTrace, testFrom, vec3_origin, vec3_origin,
                   victim->currentOrigin, ent->s.number, MASK_SHOT,
                   (EG2_Collision)0, 0);
          if (testTrace.entityNum !=
              victim->s.number) { // can only damage maglocks if have a clear
                                  // trace to the thing's origin
            continue;
          }
        }
        if (totalDmg[i] > 0) { // actually want to do *some* damage here
          if (victim->client && victim->client->NPC_class == CLASS_WAMPA &&
              victim->activator == ent) {
          } else if (PM_SuperBreakWinAnim(ent->client->ps.torsoAnim) ||
                     PM_StabDownAnim(
                         ent->client->ps
                             .torsoAnim)) { // never cap the superbreak wins
          } else {
            if (victim->client &&
                ...
                !g_saberRealisticCombat
                     ->integer) { // dmg vs other saber fighters is modded by
                                  // hitloc and capped
              ...
              if (hitLoc[i] == HL_NONE) {
                ...
                maxDmg =
                    50 * hitLocHealthPercentage[hitLoc[i]] *
            ...
            // clamp the dmg
            if (victim->s.weapon !=
                WP_SABER) { // clamp the dmg between 25 and maxhealth
              /*
              ...
              if (!victim->s.number &&
                  totalDmg[i] > 50) { // never do more than half full health
                                      // damage to player
          ...
          if (totalDmg[i] > 0) {
            ...
            if (baseDamage <= 0.1f) { // just get their attention?
            ...
            if (victim->client) {
              if (victim->client->ps.pm_time > 0 &&
                  ...
                  victim->client->ps.velocity[2] >
                      0) { // already being knocked around
                dFlags |= DAMAGE_NO_KNOCKBACK;
              }
              if (!WP_SaberBladeUseSecondBladeStyle(
                      &ent->client->ps.saber[saberNum], bladeNum) &&
                  (ent->client->ps.saber[saberNum].saberFlags2 &
                   SFL2_NO_DISMEMBERMENT)) { // no dismemberment!
                                             // (blunt/stabbing weapon?)
              } else if (WP_SaberBladeUseSecondBladeStyle(
                             &ent->client->ps.saber[saberNum], bladeNum) &&
                         (ent->client->ps.saber[saberNum].saberFlags2 &
                          SFL2_NO_DISMEMBERMENT2)) { // no dismemberment!
                                                     // (blunt/stabbing weapon?)
              } else {
                if (debug_subdivision->integer ||
              ...
              if (baseDamage <= 1.0f) { // very mild damage
                if (victim->s.number == 0 ||
                    ...
                    victim->client->NPC_class ==
                        CLASS_GALAKMECH) { // if it's the player or a
                                           // saber-user, don't kill them with
                                           // this blow
            ...
            if (!PM_SuperBreakWinAnim(ent->client->ps.torsoAnim) &&
                ...
                if (victim->client->NPC_class == CLASS_SHADOWTROOPER ||
                  ...
                  if (totalDmg[i] > maxDmg) {
                    totalDmg[i] = maxDmg;
                           ...
                           victim->client->NPC_class == CLASS_REBORN ||
                           victim->client->NPC_class ==
                               CLASS_JEDI) { // hit a non-boss saber-user
              ...
              if (victim->s.number < MAX_CLIENTS && ent->NPC) {
                if ((ent->NPC->aiFlags & NPCAI_BOSS_CHARACTER) ||
                    ...
                    ent->client->NPC_class ==
                        CLASS_SHADOWTROOPER) { // player hit by a boss character
            ...
            dFlags |= DAMAGE_NO_KNOCKBACK; // okay, let's try no knockback
                                           // whatsoever...
            ...
            if (saberType == SABER_SITH_SWORD) { // do knockback
            ...
            if (!WP_SaberBladeUseSecondBladeStyle(
              ...
              damage = ceil(totalDmg[i] *
                            ent->client->ps.saber[saberNum].damageScale);
            } else if (WP_SaberBladeUseSecondBladeStyle(
                           &ent->client->ps.saber[saberNum], bladeNum) &&
                       ent->client->ps.saber[saberNum].damageScale2 != 1.0f) {
              damage = ceil(totalDmg[i] *
                            ent->client->ps.saber[saberNum].damageScale2);
            } else {
              damage = ceil(totalDmg[i]);
            }
            G_Damage(victim, inflictor, ent, dmgDir[i], dmgSpot[i], damage,
                     dFlags, MOD_SABER, hitDismemberLoc[i]);
            if (damage > 0 && cg.time) {
              float sizeTimeScale = 1.0f;
              if ((vicWasAlive && victim->health <= 0) ||
                  (!vicWasDismembered && victim->client->dismembered &&
                   hitDismemberLoc[i] != HL_NONE && hitDismember[i])) {
                sizeTimeScale = 3.0f;
              }
              // FIXME: if not hitting the first model on the enemy, don't do
              // this!
              CG_SaberDoWeaponHitMarks(
                  ...
                  (ent->client->ps.saberInFlight
                       ...
                       : NULL),
                  victim, saberNum, bladeNum, dmgSpot[i], dmgDir[i],
                  dmgBladeVec[i], dmgNormal[i], sizeTimeScale);
            }
#ifndef FINAL_BUILD
            if (d_saberCombat->integer) {
            ...
            // G_PlayEffect( G_EffectIndex( "blood_sparks" ), dmgSpot[i],
            // splashBackDir );
            if (ent->s.number == 0) {
            ...
            if (ent->client) {
              if (ent->enemy &&
                  ent->enemy ==
                      victim) { // just so Jedi knows that he hit his enemy
...
void WP_SaberDamageAdd(float trDmg, int trVictimEntityNum, vec3_t trDmgDir,
                       vec3_t trDmgBladeVec, vec3_t trDmgNormal,
                       vec3_t trDmgSpot, float dmg, float fraction,
                       int trHitLoc, qboolean trDismember, int trDismemberLoc) {
  int curVictim = 0;
  int i;
  ...
  if (trDmg * dmg < 10.0f) { // too piddly an amount of damage to really count?
                             // FIXME: but already did the effect, didn't we...
                             // sigh... return;
  }
  if (trDmg) { // did some damage to something
    ...
    if (trHitLoc != HL_NONE &&
        (hitLoc[curVictim] == HL_NONE ||
         hitLocHealthPercentage[trHitLoc] >
             hitLocHealthPercentage
                 [hitLoc[curVictim]])) { // this hitLoc is more critical than
                                         // the previous one this frame
    ...
    // Well, if the saber hits something that stops it, the damage isn't done
    // past that point.
    dmgFraction[curVictim] = fraction;
    if ((trDismemberLoc != HL_NONE && hitDismemberLoc[curVictim] == HL_NONE) ||
        (!hitDismember[curVictim] &&
         trDismember)) { // either this is the first dismember loc we got or we
                         // got a loc before, but it wasn't a dismember loc, so
                         // take the new one
...
qboolean WP_SabersIntersect(gentity_t *ent1, int ent1SaberNum, int ent1BladeNum,
  ...
  */
  ...
  if (ent1->client->ps.SaberLength() <= 0 ||
      ent2->client->ps.SaberLength() <= 0) {
    return qfalse;
  }

  for (ent2SaberNum = 0; ent2SaberNum < MAX_SABERS; ent2SaberNum++) {
    for (ent2BladeNum = 0;
         ent2BladeNum < ent2->client->ps.saber[ent2SaberNum].numBlades;
         ent2BladeNum++) {
      if (ent2->client->ps.saber[ent2SaberNum].type != SABER_NONE &&
          ent2->client->ps.saber[ent2SaberNum].blade[ent2BladeNum].length >
              0) { // valid saber and this blade is on
        // if ( ent1->client->ps.saberInFlight )
        {
          VectorCopy(ent1->client->ps.saber[ent1SaberNum]
                         .blade[ent1BladeNum]
                         .muzzlePointOld,
                     saberBase1);
          VectorCopy(ent1->client->ps.saber[ent1SaberNum]
          ...
          VectorSubtract(ent1->client->ps.saber[ent1SaberNum]
                             .blade[ent1BladeNum]
                             .muzzlePoint,
                         ent1->client->ps.saber[ent1SaberNum]
                             .blade[ent1BladeNum]
                             .muzzlePointOld,
                         dir);
          VectorNormalize(dir);
          ...
          VectorMA(
              saberBase1,
              ent1->client->ps.saber[ent1SaberNum].blade[ent1BladeNum].length,
              ent1->client->ps.saber[ent1SaberNum]
                  .blade[ent1BladeNum]
                  .muzzleDirOld,
              saberTip1);
        ...
        {
                VectorCopy(
        ent1->client->ps.saber[ent1SaberNum].blade[ent1BladeNum].muzzlePoint,
        saberBase1 ); VectorCopy(
        ent1->client->ps.saber[ent1SaberNum].blade[ent1BladeNum].muzzlePointNext,
        saberBaseNext1 ); VectorMA( saberBase1,
        ent1->client->ps.saber[ent1SaberNum].blade[ent1BladeNum].length,
        ent1->client->ps.saber[ent1SaberNum].blade[ent1BladeNum].muzzleDir,
        saberTip1 ); VectorMA( saberBaseNext1,
        ent1->client->ps.saber[ent1SaberNum].blade[ent1BladeNum].length,
        ent1->client->ps.saber[ent1SaberNum].blade[ent1BladeNum].muzzleDirNext,
        saberTipNext1 );
        }
        */
        ...
        {
          VectorCopy(ent2->client->ps.saber[ent2SaberNum]
                         .blade[ent2BladeNum]
                         .muzzlePointOld,
                     saberBase2);
          VectorCopy(ent2->client->ps.saber[ent2SaberNum]
          ...
          VectorSubtract(ent2->client->ps.saber[ent2SaberNum]
                             .blade[ent2BladeNum]
                             .muzzlePoint,
                         ent2->client->ps.saber[ent2SaberNum]
                             .blade[ent2BladeNum]
                             .muzzlePointOld,
                         dir);
          VectorNormalize(dir);
        ...
        {
                VectorCopy(
        ent2->client->ps.saber[ent2SaberNum].blade[ent2BladeNum].muzzlePoint,
        saberBase2 ); VectorCopy(
        ent2->client->ps.saber[ent2SaberNum].blade[ent2BladeNum].muzzlePointNext,
        saberBaseNext2 ); VectorMA( saberBase2,
        ent2->client->ps.saber[ent2SaberNum].blade[ent2BladeNum].length,
        ent2->client->ps.saber[ent2SaberNum].blade[ent2BladeNum].muzzleDir,
        saberTip2 ); VectorMA( saberBaseNext2,
        ent2->client->ps.saber[ent2SaberNum].blade[ent2BladeNum].length,
        ent2->client->ps.saber[ent2SaberNum].blade[ent2BladeNum].muzzleDirNext,
        saberTipNext2 );
        }
        */
        if (checkDir) { // check the direction of the two swings to make sure
          ...
          // now check orientation of sabers, make sure they're not parallel or
          // close to it
          float dot = DotProduct(ent1->client->ps.saber[ent1SaberNum]
                                     .blade[ent1BladeNum]
                                     .muzzleDir,
                                 ent2->client->ps.saber[ent2SaberNum]
                                     .blade[ent2BladeNum]
        ...
        if (tri_tri_intersect(saberBase1, saberTip1, saberBaseNext1, saberBase2,
                              saberTip2, saberBaseNext2)) {
          return qtrue;
        }
        if (tri_tri_intersect(saberBase1, saberTip1, saberBaseNext1, saberBase2,
        ...
        if (tri_tri_intersect(saberBase1, saberTip1, saberTipNext1, saberBase2,
                              saberTip2, saberBaseNext2)) {
          return qtrue;
        }
        if (tri_tri_intersect(saberBase1, saberTip1, saberTipNext1, saberBase2,
...
float WP_SabersDistance(gentity_t *ent1, gentity_t *ent2) {
  ...
  */
  ...
  // if ( ent1->client->ps.saberInFlight )
  {
    ...
    VectorMA(saberBaseNext1, ent1->client->ps.saber[0].blade[0].length,
             ent1->client->ps.saber[0].blade[0].muzzleDir, saberTipNext1);
  }
  /*
  else
  {
          VectorCopy( ent1->client->ps.saber[0].blade[0].muzzlePointNext,
  saberBaseNext1 ); VectorMA( saberBaseNext1,
  ent1->client->ps.saber[0].blade[0].length,
  ent1->client->ps.saber[0].blade[0].muzzleDirNext, saberTipNext1 );
  }
  */
  ...
  // if ( ent2->client->ps.saberInFlight )
  {
    ...
    VectorMA(saberBaseNext2, ent2->client->ps.saber[0].blade[0].length,
             ent2->client->ps.saber[0].blade[0].muzzleDir, saberTipNext2);
  }
  /*
  else
  {
          VectorCopy( ent2->client->ps.saber[0].blade[0].muzzlePointNext,
  saberBaseNext2 ); VectorMA( saberBaseNext2,
  ent2->client->ps.saber[0].blade[0].length,
  ent2->client->ps.saber[0].blade[0].muzzleDirNext, saberTipNext2 );
  }
  */

  float sabersDist = ShortestLineSegBewteen2LineSegs(
      saberBaseNext1, saberTipNext1, saberBaseNext2, saberTipNext2, saberPoint1,
      saberPoint2);

  // okay, this is a super hack, but makes saber collisions look better from the
...
qboolean WP_SabersIntersection(gentity_t *ent1, gentity_t *ent2,
                               vec3_t intersect) {
  vec3_t saberBaseNext1, saberTipNext1, saberPoint1;
  vec3_t saberBaseNext2, saberTipNext2, saberPoint2;
  ...
  for (saberNum1 = 0; saberNum1 < MAX_SABERS; saberNum1++) {
    for (bladeNum1 = 0; bladeNum1 < ent1->client->ps.saber[saberNum1].numBlades;
         bladeNum1++) {
      if (ent1->client->ps.saber[saberNum1].type != SABER_NONE &&
          ent1->client->ps.saber[saberNum1].blade[bladeNum1].length >
              0) { // valid saber and this blade is on
        for (saberNum2 = 0; saberNum2 < MAX_SABERS; saberNum2++) {
          for (bladeNum2 = 0;
               bladeNum2 < ent2->client->ps.saber[saberNum2].numBlades;
               bladeNum2++) {
            if (ent2->client->ps.saber[saberNum2].type != SABER_NONE &&
                ent2->client->ps.saber[saberNum2].blade[bladeNum2].length >
                    0) { // valid saber and this blade is on
              VectorCopy(ent1->client->ps.saber[saberNum1]
                             .blade[bladeNum1]
                             .muzzlePoint,
                         saberBaseNext1);
              VectorMA(
                  saberBaseNext1,
                  ent1->client->ps.saber[saberNum1].blade[bladeNum1].length,
                  ent1->client->ps.saber[saberNum1].blade[bladeNum1].muzzleDir,
                  saberTipNext1);

              VectorCopy(ent2->client->ps.saber[saberNum2]
                             .blade[bladeNum2]
                             .muzzlePoint,
                         saberBaseNext2);
              VectorMA(
                  saberBaseNext2,
                  ent2->client->ps.saber[saberNum2].blade[bladeNum2].length,
                  ent2->client->ps.saber[saberNum2].blade[bladeNum2].muzzleDir,
                  saberTipNext2);

              lineSegLength = ShortestLineSegBewteen2LineSegs(
                  saberBaseNext1, saberTipNext1, saberBaseNext2, saberTipNext2,
...
const char *hit_blood_sparks =
    "sparks/blood_sparks2"; // could have changed this effect directly, but this
                            // is just safer in case anyone anywhere else is
                            // using the old one for something?
const char *hit_sparks = "saber/saber_cut";

qboolean WP_SaberDamageEffects(trace_t *tr, const vec3_t start, float length,
                               float dmg, vec3_t dmgDir, vec3_t bladeVec,
                               int enemyTeam, saberType_t saberType,
                               saberInfo_t *saber, int bladeNum) {
  ...
  for (z = 0; z < MAX_G2_COLLISIONS; z++) {
    if (tr->G2CollisionMap[z].mEntityNum ==
        -1) { // actually, completely break out of this for loop since nothing
              // after this in the aray should ever be valid either
      continue; // break;//
    }
    ...
    /*
    //FIXME: (distFromStart/length) is not guaranteed to be from 0 to 1...
*sigh*... if ( length && saberHitFraction < 1.0f && (distFromStart/length)
< 1.0f && (distFromStart/length) > saberHitFraction )
    {//a saber was hit before this point, don't count it
#ifndef FINAL_BUILD
            if ( d_saberCombat->integer )
            {
                    gi.Printf( S_COLOR_MAGENTA"rejecting G2 collision- %4.2f
farther than saberHitFraction %4.2f\n", (distFromStart/length), saberHitFraction
);
            }
#endif
            continue;
    ...
    */
    ...
    for (i = 0; i < numHitEnts; i++) {
      if (hitEntNum[i] == coll.mEntityNum) { // we hit this ent before
    ...
    if (i == numHitEnts) {                   // first time we hit this ent
      ...
      if (!coll.mFlags) { // hmm, we came out first, so we must have started
                          // inside
      ...
      if (hitEnt && hitEnt->client &&
          coll.mModelIndex >
              0) { // hit a submodel on the enemy, not their actual body!
      ...
      if (hitEnt != NULL) {
        if (hitEnt->client) {
          ...
          // So sue me, this is the easiest way to check to see if this is the
          // turbo laser from t2_wedge, in which case I don't want the saber
          // effects goin off on it.
          if ((hitEnt->flags & FL_DMG_BY_HEAVY_WEAP_ONLY) &&
              hitEnt->takedamage == qfalse &&
              Q_stricmp(hitEnt->classname, "misc_turret") == 0) {
            continue;
          } else {
            if (dmg) { // only do these effects if actually trying to damage the
                       // thing...
              if ((hitEnt->svFlags & SVF_BBRUSH) // a breakable brush
                  &&
                  ((hitEnt->spawnflags & 1) // INVINCIBLE
                   || (hitEnt->flags &
                       FL_DMG_BY_HEAVY_WEAP_ONLY)) // HEAVY weapon damage only
      ...
      if (!g_saberNoEffects) {
        if (hitEffect != 0) {
          // FIXME: when you have multiple blades hitting someone for many
          // sequential server frames, this can get a bit chuggy!
          G_PlayEffect(hitEffect, coll.mCollisionPosition,
                       coll.mCollisionNormal);
        }
      }

      // Get the hit location based on surface name
      if ((hitLoc[numHitEnts] == HL_NONE && trHitLoc[numHitEnts] == HL_NONE) ||
          (hitDismemberLoc[numHitEnts] == HL_NONE &&
           trDismemberLoc[numHitEnts] == HL_NONE) ||
          (!hitDismember[numHitEnts] &&
           !trDismember[numHitEnts])) { // no hit loc set for this ent this
                                        // damage cycle yet
        // FIXME: find closest impact surf *first* (per ent), then call
        // G_GetHitLocFromSurfName? FIXED: if hit multiple ents in this collision
        // record, these trSurfName, trDismember and trDismemberLoc will get
        // stomped/confused over the multiple ents I hit
        trSurfName = gi.G2API_GetSurfaceName(
            &g_entities[coll.mEntityNum].ghoul2[coll.mModelIndex],
            coll.mSurfaceIndex);
        trDismember[numHitEnts] = G_GetHitLocFromSurfName(
            &g_entities[coll.mEntityNum], trSurfName, &trHitLoc[numHitEnts],
            coll.mCollisionPosition, dmgDir, bladeDir, MOD_SABER, saberType);
  ...
  for (i = 0; i < numHitEnts; i++) {
    ...
    if (hitEntNum[i] != ENTITYNUM_NONE) {
          ...
          if (!hitEnt->client ||
              (hitEnt->client->ps.weapon != WP_SABER &&
               ...
               hitEnt->client->playerTeam ==
                   enemyTeam)) { // did *not* hit a jedi and did *not* hit the
                                 // player
      ...
      if (doDmg > 0) {
        WP_SaberDamageAdd(1.0, hitEntNum[i], dmgDir, bladeVec, hitEntNormal[i],
                          hitEntPoint[i], ceil(doDmg), hitEntStartFrac[i],
                          trHitLoc[i], trDismember[i], trDismemberLoc[i]);
      }
    }
  }
...
void WP_SaberBlockEffect(gentity_t *attacker, int saberNum, int bladeNum,
                         vec3_t position, vec3_t normal, qboolean cutNotBlock) {
  ...
  if (saber && !WP_SaberBladeUseSecondBladeStyle(saber, bladeNum) &&
      saber->blockEffect) {
    if (normal) {
  ...
  } else if (saber && WP_SaberBladeUseSecondBladeStyle(saber, bladeNum) &&
             saber->blockEffect2) {
    if (normal) {
...
void WP_SaberKnockaway(gentity_t *attacker, trace_t *tr) {
  ...
  // G_Sound( &g_entities[attacker->client->ps.saberEntityNum], G_SoundIndex(
  ...
  // FIXME: make hitEnt play an attack anim or some other special anim when this
  // happens gentity_t *hitEnt = &g_entities[tr->entityNum]; NPC_SetAnim( hitEnt,
  // SETANIM_BOTH, BOTH_KNOCKSABER, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
}

qboolean G_InCinematicSaberAnim(gentity_t *self) {
  if (self->NPC && self->NPC->behaviorState == BS_CINEMATIC &&
      (self->client->ps.torsoAnim == BOTH_CIN_16 ||
       self->client->ps.torsoAnim == BOTH_CIN_17)) {
    return qtrue;
  }
  return qfalse;
}
...
qboolean WP_SaberDamageForTrace(int ignore, vec3_t start, vec3_t end, float dmg,
                                vec3_t bladeDir, qboolean noGhoul,
                                int attackStrength, saberType_t saberType,
                                qboolean extrapolate, int saberNum,
                                int bladeNum) {
  trace_t tr;
  vec3_t dir;
  ...
  VectorCopy(end, end2);
  if (extrapolate) {
    // NOTE: since we can no longer use the predicted point, extrapolate the
    // trace some. 		this may allow saber hits that aren't actually hits, but it
  ...
  if (!noGhoul) {
    ...
    if (attacker &&
        attacker->client) { // see if we're not drawing the blade, if so, do a
                            // trace based on radius of blade (because the radius
                            // is being used to simulate a larger/smaller piece
                            // of a solid weapon)...
      if (!WP_SaberBladeUseSecondBladeStyle(
              &attacker->client->ps.saber[saberNum], bladeNum) &&
          (attacker->client->ps.saber[saberNum].saberFlags2 &
           SFL2_NO_BLADE)) { // not drawing blade
        useRadiusForDamage =
            attacker->client->ps.saber[saberNum].blade[bladeNum].radius;
      } else if (WP_SaberBladeUseSecondBladeStyle(
                     &attacker->client->ps.saber[saberNum], bladeNum) &&
                 (attacker->client->ps.saber[saberNum].saberFlags2 &
                  SFL2_NO_BLADE2)) { // not drawing blade
        useRadiusForDamage =
            attacker->client->ps.saber[saberNum].blade[bladeNum].radius;
      }
    }
    if (!useRadiusForDamage) { // do normal check for larger-size saber traces
      if (!attacker->s.number ||
          (attacker->client &&
           (attacker->client->playerTeam == TEAM_PLAYER ||
            ...
            (attacker->NPC &&
             (attacker->NPC->aiFlags &
              NPCAI_BOSS_CHARACTER))))) //&&attackStrength==FORCE_LEVEL_3)
      {
    ...
    { // player,. player allies, shadowtroopers, tavion and desann use larger
      // traces
      vec3_t traceMins = {-useRadiusForDamage, -useRadiusForDamage,
                          -useRadiusForDamage},
             traceMaxs = {useRadiusForDamage, useRadiusForDamage,
                          useRadiusForDamage};
    ...
    else if ( !attacker->s.number )
    {
            ...
            gi.trace( &tr, start, traceMins, traceMaxs, end2, ignore, mask,
    G2_COLLIDE, 10 );//G2_SUPERSIZEDBBOX
    }
    */
    else { // reborn use smaller traces
      gi.trace(&tr, start, NULL, NULL, end2, ignore, mask, G2_COLLIDE,
               10); // G2_SUPERSIZEDBBOX
    }
  } else {
  ...
  if (tr.entityNum == ENTITYNUM_WORLD) {
    if (attacker && attacker->client &&
        (attacker->client->ps.saber[saberNum].saberFlags &
         SFL_BOUNCE_ON_WALLS)) {
      VectorCopy(tr.endpos, saberHitLocation);
  ...
  if (&g_entities[tr.entityNum]) {
    ...
    if (hitEnt->contents & CONTENTS_LIGHTSABER) {
      if (attacker && attacker->client &&
        ...
        if (owner && owner->s.number && owner->client && owner->NPC &&
            owner->health > 0) {
          if (owner->client->NPC_class ==
              CLASS_ALORA) { // alora takes less damage
            dmg *= 0.25f;
          }
      ...
      // FIXME: take target FP_SABER_DEFENSE and attacker FP_SABER_OFFENSE into
      // account here somehow?
      qboolean sabersIntersect = WP_SabersIntersect(
          attacker, saberNum, bladeNum, owner, qfalse); // qtrue );
      float sabersDist;
      if (attacker && attacker->client && attacker->client->ps.saberInFlight &&
          owner && owner->s.number == 0 &&
      ...
      { // players have g_saberAutoBlocking, do the more generous check against
        // flying sabers
      ...
      if (owner && owner->client && (attacker != NULL) &&
          (sabersDist > collisionDist) //|| !InFront( attacker->currentOrigin,
                                       //owner->currentOrigin,
                                       //owner->client->ps.viewangles, 0.35f ))
      ...
      { // swing came from behind and/or was not stopped by a lightsaber
        ...
        gi.trace(&tr, start, NULL, NULL, end2, ignore,
                 mask & ~CONTENTS_LIGHTSABER, G2_NOCOLLIDE, 10);
        if (tr.entityNum == ENTITYNUM_WORLD) {
          return qtrue;
        }
        if (tr.entityNum == ENTITYNUM_NONE ||
          ...
          if ( attacker
                  && attacker->client
                  && (PM_SaberInAttack( attacker->client->ps.saberMove ) ||
          PM_SaberInStart( attacker->client->ps.saberMove ))
                  && DistanceSquared( tr.endpos, owner->currentOrigin ) < 10000
          )
          {
                  if ( owner->NPC
                          && !owner->client->ps.saberInFlight
                          && owner->client->ps.saberBlocked !=
          BLOCKED_PARRY_BROKEN
                          && !Jedi_SaberBusy( owner ) )
                  {//owner parried, just make sure they're saber is in the right
          spot - only does this if they're not already doing something with
          saber if ( g_spskill->integer && (g_spskill->integer > 1 || Q_irand(
          0, 1 )))
                          {//if on easy, they don't cheat like this, if on
          medium, they cheat 50% of the time, if on hard, they always cheat
                                  //FIXME: also take into account the owner's
          FP_DEFENSE? if ( Q_irand( 0, owner->NPC->rank ) >= RANK_CIVILIAN )
                                  {//lower-rank Jedi aren't as good blockers
                                          vec3_t attDir;
                                          VectorSubtract( end2, start, attDir );
                                          VectorNormalize( attDir );
                                          Jedi_SaberBlockGo( owner,
          owner->NPC->last_ucmd, start, attDir, NULL );
                                  }
                          }
                  }
          }
        ...
        if (d_saberCombat->integer > 1) {
          if (!attacker->s.number) {
            gi.Printf(S_COLOR_MAGENTA
                      "%d saber hit owner through saber %4.2f, dist = %4.2f\n",
        ...
        if ((tr.fraction < saberHitFraction || tr.startsolid) &&
            ...
            && (sabersIntersect ||
                sabersDist < (4.0f + g_spskill->value) * 2.0f)) // 32.0f) )
        { // This saber hit closer than the last one.
          if ((tr.allsolid || tr.startsolid) && owner &&
              owner->client) { // tr.fraction will be 0, unreliable... so
                               // calculate actual
            float dist = Distance(start, end2);
            if (dist) {
              float hitFrac = WP_SabersDistance(attacker, owner) / dist;
              if (hitFrac > 1.0f) { // umm... minimum distance between sabers
                                    // was longer than trace...?
                hitFrac = 1.0f;
              }
            ...
            if (d_saberCombat->integer > 1) {
              if (!attacker->s.number) {
                gi.Printf(S_COLOR_GREEN
                          "%d saber hit saber dist %4.2f allsolid %4.2f\n",
            ...
            if (d_saberCombat->integer > 1) {
              if (!attacker->s.number) {
                gi.Printf(S_COLOR_BLUE
                          "%d saber hit saber dist %4.2f, frac %4.2f\n",
                          level.time, sabersDist, saberHitFraction);
              }
              saberHitFraction = tr.fraction;
            }
...
#ifndef FINAL_BUILD
          if (d_saberCombat->integer) {
            gi.Printf(S_COLOR_MAGENTA "hit saber: saberHitFraction %4.2f, "
        ...
        BLOCKED_PARRY_BROKEN
                        && !Jedi_SaberBusy( owner ) )
                {//owner parried, just make sure they're saber is in the right
        spot - only does this if they're not already doing something with saber
                        if ( g_spskill->integer && (g_spskill->integer > 1 ||
        Q_irand( 0, 1 )))
                        {//if on easy, they don't cheat like this, if on medium,
        they cheat 50% of the time, if on hard, they always cheat
                                //FIXME: also take into account the owner's
        FP_DEFENSE? if ( Q_irand( 0, owner->NPC->rank ) >= RANK_CIVILIAN )
                                {//lower-rank Jedi aren't as good blockers
                                        vec3_t attDir;
    ...
    if (attacker && attacker->client &&
      ...
      if ((hitEnt && hitEnt->client && hitEnt->health > 0 &&
        ...
        // FIXME: control the direction of the thrown saber... if hit Galak's
        // shield, bounce directly away from his origin?
        WP_SaberKnockaway(attacker, &tr);
        if (hitEnt->client) {
    ...
    if (hitEnt->takedamage) {
      // no team damage: if ( !hitEnt->client || attacker == NULL ||
      // !attacker->client || (hitEnt->client->playerTeam !=
      // attacker->client->playerTeam) )
      {
        ...
        if (attacker && attacker->client) {
          VectorScale(
              bladeDir,
              attacker->client->ps.saber[saberNum].blade[bladeNum].length,
              bladeVec);
        }
        ...
        if (noGhoul ||
            !hitEnt->ghoul2.size()) { // we weren't doing a ghoul trace
          int hitEffect = 0;
          ...
          if (tr.allsolid) { // totally inside them
            ...
            // we don't know how much was inside, we know it's less than all, so
            // use half?
            trFrac = 0.5;
            dmgFrac = 0.0;
          } else { // started outside them and hit them
            // yeah. this doesn't account for coming out the other wide, but we
            // can worry about that later (use ghoul2)
            trFrac = (1.0f - tr.fraction);
            dmgFrac = tr.fraction;
          }
          vec3_t backdir;
          VectorScale(dir, -1, backdir);
          WP_SaberDamageAdd(trFrac, tr.entityNum, dir, bladeVec, backdir,
                            tr.endpos, dmg, dmgFrac, HL_NONE, qfalse, HL_NONE);
          ...
          if (hitEnt != NULL) {
            if (hitEnt->client) {
              ...
              if (npc_class == CLASS_SEEKER || npc_class == CLASS_PROBE ||
                  ...
                  npc_class == CLASS_REMOTE || npc_class == CLASS_PROTOCOL ||
                  npc_class == CLASS_MARK1 || npc_class == CLASS_MARK2 ||
                  npc_class == CLASS_INTERROGATOR || npc_class == CLASS_ATST ||
                  npc_class == CLASS_SENTRY) {
                if (!WP_SaberBladeUseSecondBladeStyle(
                  ...
                  hitEffect =
                      attacker->client->ps.saber[saberNum].hitOtherEffect;
                               ...
                               &attacker->client->ps.saber[saberNum],
                               bladeNum) &&
                           attacker->client->ps.saber[saberNum]
                               .hitOtherEffect2) {
                  hitEffect =
              ...
              if (dmg) { // only do these effects if actually trying to damage
                         // the thing...
                if ((hitEnt->svFlags & SVF_BBRUSH) // a breakable brush
                    &&
                    ((hitEnt->spawnflags & 1) // INVINCIBLE
                     || (hitEnt->flags &
                         FL_DMG_BY_HEAVY_WEAP_ONLY) // HEAVY weapon damage only
                     ||
                     (hitEnt->NPC_targetname && attacker &&
                      attacker->targetname &&
                      Q_stricmp(attacker->targetname,
                                hitEnt->NPC_targetname)))) // only breakable by
                                                           // an entity who is
                                                           // not the attacker
                { // no hit effect (besides regular client-side one)
                } else {
          ...
          if (!attacker || !attacker->client ||
            ...
            if (!WP_SaberDamageEffects(&tr, start, len, dmg, dir, bladeVec,
                                       attacker->client->enemyTeam, saberType,
                                       &attacker->client->ps.saber[saberNum],
                                       bladeNum)) { // didn't hit a ghoul ent
              /*
...
#define LOCK_IDEAL_DIST_JKA                                                    \
  46.0f // all of the new saberlocks are 46.08 from each other because Richard
        // Lico is da MAN
extern void PM_SetAnimFrame(gentity_t *gent, int frame, qboolean torso,
                            qboolean legs);
...
qboolean G_CheckIncrementLockAnim(int anim, int winOrLose) {
  ...
  // RULE: if you are the first style in the lock anim, you advance from LOSING
  // position to WINNING position 		if you are the second style in the lock anim,
  //you advance from WINNING position to LOSING position
  switch (anim) {
  // increment to win:
  case BOTH_LK_DL_DL_S_L_1: // lock if I'm using dual vs. dual and I initiated
  case BOTH_LK_DL_DL_S_L_2: // lock if I'm using dual vs. dual and other
                            // initiated
  case BOTH_LK_DL_DL_T_L_1: // lock if I'm using dual vs. dual and I initiated
  case BOTH_LK_DL_DL_T_L_2: // lock if I'm using dual vs. dual and other
                            // initiated
  ...
  case BOTH_LK_S_S_S_L_1:   // lock if I'm using single vs. a single and I
                          // initiated
  case BOTH_LK_S_S_T_L_2: // lock if I'm using single vs. a single and other
                          // initiated
  ...
  case BOTH_LK_ST_ST_T_L_1: // lock if I'm using staff vs. a staff and I
                            // initiated
  case BOTH_LK_ST_ST_T_L_2: // lock if I'm using staff vs. a staff and other
                            // initiated
    if (winOrLose == SABERLOCK_WIN) {
      increment = qtrue;
    } else {
  ...
  case BOTH_LK_S_S_S_L_2:  // lock if I'm using single vs. a single and other
                          // intitiated
  ...
  case BOTH_LK_ST_ST_S_L_1: // lock if I'm using staff vs. a staff and I
                            // initiated
  case BOTH_LK_ST_ST_S_L_2: // lock if I'm using staff vs. a staff and other
                            // initiated
    if (winOrLose == SABERLOCK_WIN) {
      increment = qfalse;
    } else {
...
qboolean WP_SabersCheckLock2(gentity_t *attacker, gentity_t *defender,
  ...
  // FIXME: this distances need to be modified by the lengths of the sabers
  // involved... MATCH ANIMS
  if (lockMode == LOCK_KYLE_GRAB1 || lockMode == LOCK_KYLE_GRAB2 ||
    ...
    case LOCK_KYLE_GRAB3:
      ...
      defender->forcePushTime =
          level.time + PM_AnimLength(defender->client->clientInfo.animFileIndex,
                                     BOTH_PLAYER_PA_2);
      numSpins = 3.0f;
      break;
    }
    ...
    if (d_slowmodeath->integer > 3 && (defender->s.number < MAX_CLIENTS ||
                                       attacker->s.number < MAX_CLIENTS)) {
      if (ValidAnimFileIndex(attacker->client->clientInfo.animFileIndex)) {
        int effectTime = PM_AnimLength(
            attacker->client->clientInfo.animFileIndex, (animNumber_t)attAnim);
    ...
    if (lockMode == LOCK_RANDOM) {
      lockMode =
          (sabersLockMode_t)Q_irand((int)LOCK_FIRST, (int)(LOCK_RANDOM)-1);
    }
    // FIXME: attStart% and idealDist will change per saber lock anim pairing...
    // do we need a big table like in bg_panimate.cpp?
    if (attacker->client->ps.saberAnimLevel >= SS_FAST &&
      ...
      idealDist =
          LOCK_IDEAL_DIST_JKA; // all of the new saberlocks are 46.08 from each
                               // other because Richard Lico is da MAN
      if (lockMode == LOCK_TOP) { // top lock
        ...
        defAnim =
            G_SaberLockAnim(defender->client->ps.saberAnimLevel,
                            attacker->client->ps.saberAnimLevel, SABERLOCK_TOP,
                            SABERLOCK_LOCK, SABERLOCK_LOSE);
        attStart = defStart = 0.5f;
      } else { // side lock
        switch (lockMode) {
        case LOCK_DIAG_TR:
          attAnim =
              G_SaberLockAnim(attacker->client->ps.saberAnimLevel,
                              defender->client->ps.saberAnimLevel,
  ...
  NPC_SetAnim(attacker, SETANIM_BOTH, attAnim,
              SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  NPC_SetAnim(defender, SETANIM_BOTH, defAnim,
              SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  // don't let them store a kick for the whole saberlock....
  attacker->client->ps.saberMoveNext = defender->client->ps.saberMoveNext =
      LS_NONE;
  //
  if (attStart > 0.0f) {
    if (ValidAnimFileIndex(attacker->client->clientInfo.animFileIndex)) {
      anim =
          &level.knownAnimFileSets[attacker->client->clientInfo.animFileIndex]
               .animations[attAnim];
      ...
      if (d_saberCombat->integer) {
        Com_Printf("%s starting saber lock, anim = %s, %d frames to go!\n",
                   attacker->NPC_type, animTable[attAnim].name,
                   anim->numFrames - advance);
      }
#endif
    }
  }
  if (defStart > 0.0f) {
    if (ValidAnimFileIndex(defender->client->clientInfo.animFileIndex)) {
      anim =
          &level.knownAnimFileSets[defender->client->clientInfo.animFileIndex]
               .animations[defAnim];
      advance = ceil(anim->numFrames * defStart);
      PM_SetAnimFrame(
          defender, anim->firstFrame + advance, qtrue,
          qtrue); // was anim->firstFrame + anim->numFrames - advance, but
                  // that's wrong since they are matched anims
#ifndef FINAL_BUILD
      if (d_saberCombat->integer) {
        Com_Printf("%s starting saber lock, anim = %s, %d frames to go!\n",
  ...
  if (lockMode == LOCK_KYLE_GRAB1 || lockMode == LOCK_KYLE_GRAB2 ||
      lockMode == LOCK_KYLE_GRAB3 ||
      lockMode ==
          LOCK_FORCE_DRAIN) { // not a real lock, just freeze them both in place
    // can't move or attack
    attacker->client->ps.pm_time = attacker->client->ps.weaponTime =
        attacker->client->ps.legsAnimTimer;
    ...
    defender->client->ps.pm_time = defender->client->ps.weaponTime =
        defender->client->ps.legsAnimTimer;
    ...
    attacker->client->ps.legsAnimTimer = attacker->client->ps.torsoAnimTimer =
        defender->client->ps.legsAnimTimer =
  ...
  // MATCH ANGLES
  if (lockMode == LOCK_KYLE_GRAB1 || lockMode == LOCK_KYLE_GRAB2 ||
      lockMode == LOCK_KYLE_GRAB3) { // not a real lock, just set pitch to 0
    attacker->client->ps.viewangles[PITCH] =
        defender->client->ps.viewangles[PITCH] = 0;
  } else {
    // FIXME: if zDiff in elevation, make lower look up and upper look down and
    // move them closer?
    float defPitchAdd = 0,
          zDiff =
              ((attacker->currentOrigin[2] + attacker->client->standheight) -
    ...
    } else { // if a player is involved, clamp player's pitch and match NPC's to
             // player
      if (!attacker->s.number) {
        // clamp to defPitch
        ...
        defender->client->ps.viewangles[PITCH] =
            attacker->client->ps.viewangles[PITCH] * -1;
        ...
        defPitchAdd = defender->client->ps.viewangles[PITCH];
        attacker->client->ps.viewangles[PITCH] =
            defender->client->ps.viewangles[PITCH] * -1;
      }
    }
  }
  ...
  */
  ...
  float diff = VectorNormalize(defDir) -
  ...
  trace_t trace;
  gi.trace(&trace, attacker->currentOrigin, attacker->mins, attacker->maxs,
           newOrg, attacker->s.number, attacker->clipmask, (EG2_Collision)0, 0);
  ...
  gi.trace(&trace, defender->currentOrigin, defender->mins, defender->maxs,
           newOrg, defender->s.number, defender->clipmask, (EG2_Collision)0, 0);
  if (!trace.startsolid && !trace.allsolid) {
...
qboolean WP_SabersCheckLock(gentity_t *ent1, gentity_t *ent2) {
  ...
  if (ent1->client->NPC_class == CLASS_SABER_DROID ||
      ent2->client->NPC_class ==
          CLASS_SABER_DROID) { // they don't have saberlock anims
    return qfalse;
  }
  ...
  if ((ent1->client->ps.saber[0].saberFlags & SFL_NOT_LOCKABLE) ||
      (ent2->client->ps.saber[0].saberFlags &
       SFL_NOT_LOCKABLE)) { // one of these sabers cannot lock (like a lance)
    return qfalse;
  }
  if (ent1->client->ps.dualSabers && ent1->client->ps.saber[1].Active() &&
      (ent1->client->ps.saber[1].saberFlags &
       SFL_NOT_LOCKABLE)) { // one of these sabers cannot lock (like a lance)
    return qfalse;
  }
  if (ent2->client->ps.dualSabers && ent2->client->ps.saber[1].Active() &&
      (ent2->client->ps.saber[1].saberFlags &
       SFL_NOT_LOCKABLE)) { // one of these sabers cannot lock (like a lance)
    return qfalse;
  }
  if (ent1->painDebounceTime > level.time - 1000 ||
      ent2->painDebounceTime >
          level.time - 1000) { // can't saberlock if you're not ready
    return qfalse;
  }
  ...
  // FIXME: there should probably be a whole *list* of these, but I'll put them
  // in as they come up
  if (ent1->client->ps.torsoAnim == BOTH_A2_STABBACK1 &&
      ent1->client->ps.torsoAnimTimer >
          300) { // can't lock when saber is behind you
    return qfalse;
  }
  if (ent2->client->ps.torsoAnim == BOTH_A2_STABBACK1 &&
      ent2->client->ps.torsoAnimTimer >
          300) { // can't lock when saber is behind you
    return qfalse;
  }
  if (PM_LockedAnim(ent1->client->ps.torsoAnim) ||
      PM_LockedAnim(ent2->client->ps.torsoAnim)) { // stuck doing something else
    return qfalse;
  }
  if (PM_SaberLockBreakAnim(ent1->client->ps.torsoAnim) ||
      PM_SaberLockBreakAnim(
          ent2->client->ps.torsoAnim)) { // still finishing the last lock break!
    return qfalse;
  }
  // BR to TL lock
  if (ent1->client->ps.torsoAnim == BOTH_A1_BR_TL ||
      ...
      ent1->client->ps.torsoAnim ==
          BOTH_A7_BR_TL) { // ent1 is attacking in the opposite diagonal
  ...
  if (ent2->client->ps.torsoAnim == BOTH_A1_BR_TL ||
      ...
      ent2->client->ps.torsoAnim ==
          BOTH_A7_BR_TL) { // ent2 is attacking in the opposite diagonal
  ...
  if (ent1->client->ps.torsoAnim == BOTH_A1_BL_TR ||
      ...
      ent1->client->ps.torsoAnim ==
          BOTH_A7_BL_TR) { // ent1 is attacking in the opposite diagonal
  ...
  if (ent2->client->ps.torsoAnim == BOTH_A1_BL_TR ||
      ...
      ent2->client->ps.torsoAnim ==
          BOTH_A7_BL_TR) { // ent2 is attacking in the opposite diagonal
...
qboolean WP_SaberParry(gentity_t *victim, gentity_t *attacker, int saberNum,
                       int bladeNum) {
  ...
  if (victim->s.number || g_saberAutoBlocking->integer ||
      victim->client->ps.saberBlockingTime >
    ...
    if (!PM_SaberInTransitionAny(victim->client->ps.saberMove) &&
        ...
        !PM_SaberInKnockaway(
            victim->client->ps
                .saberMove)) { // I'm not attacking, in transition or in a
                               // bounce or knockaway, so play a parry
    ...
    WP_SaberClearDamageForEntNum(attacker, victim->s.number, saberNum,
                                 bladeNum);

    // tell the victim to get mad at me
    if (victim->enemy != attacker &&
        victim->client->playerTeam !=
            attacker->client->playerTeam) { // they're not mad at me and they're
                                            // not on my team
      G_ClearEnemy(victim);
...
qboolean WP_BrokenParryKnockDown(gentity_t *victim) {
  ...
  if (victim->client->ps.saberMove == LS_PARRY_UP ||
    ...
    NPC_SetAnim(victim, SETANIM_BOTH, knockAnim,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
qboolean G_TryingKataAttack(gentity_t *self, usercmd_t *cmd) {
  if (g_saberNewControlScheme
          ->integer) { // use the new control scheme: force focus button
  ...
  {                                           // use the old control scheme
    if ((cmd->buttons & BUTTON_ALT_ATTACK)) { // pressing alt-attack
      // if ( !(self->client->ps.pm_flags&PMF_ALT_ATTACK_HELD) )
      {                                       // haven't been holding alt-attack
        if ((cmd->buttons & BUTTON_ATTACK)) { // pressing attack
          return qtrue;
        }
      }
    }
...
qboolean G_TryingPullAttack(gentity_t *self, usercmd_t *cmd,
                            qboolean amPulling) {
  if (g_saberNewControlScheme
          ->integer) { // use the new control scheme: force focus button
    if ((cmd->buttons & BUTTON_FORCE_FOCUS)) {
      if (self && self->client) {
        if (self->client->ps.forcePowerLevel[FP_PULL] >=
            FORCE_LEVEL_3) { // force pull 3
          if (amPulling ||
              (self->client->ps.forcePowersActive & (1 << FP_PULL)) ||
              self->client->ps.forcePowerDebounce[FP_PULL] >
                  level.time) // force-pulling
          {                   // pulling
            return qtrue;
          }
        }
  ...
  } else {                                // use the old control scheme
    if ((cmd->buttons & BUTTON_ATTACK)) { // pressing attack
      if (self && self->client) {
        if (self->client->ps.forcePowerLevel[FP_PULL] >=
            FORCE_LEVEL_3) { // force pull 3
        ...
        if (self && self->client) {
          if (cmd->upmove > 0 //)
              && self->client->ps.groundEntityNum !=
                     ENTITYNUM_NONE) { // on ground, pressing jump
            return qtrue;
          } else { // just jumped?
...
qboolean G_TryingJumpAttack(gentity_t *self, usercmd_t *cmd) {
  if (g_saberNewControlScheme
    ...
    if ((cmd->buttons & BUTTON_ATTACK)) { // pressing attack
      if (cmd->upmove > 0) {              // pressing jump
        return qtrue;
      } else if (self && self->client) { // just jumped?
        if (self->client->ps.groundEntityNum == ENTITYNUM_NONE &&
...
qboolean G_TryingJumpForwardAttack(gentity_t *self, usercmd_t *cmd) {
  if (g_saberNewControlScheme
    ...
    if ((cmd->buttons & BUTTON_ATTACK)) { // pressing attack
      if (cmd->forwardmove > 0) {         // moving forward
        if (self && self->client) {
          if (cmd->upmove > 0 && self->client->ps.groundEntityNum !=
                                     ENTITYNUM_NONE) { // pressing jump
            return qtrue;
          } else { // no slop on forward jumps - must be precise!
            if (self->client->ps.groundEntityNum == ENTITYNUM_NONE &&
...
qboolean G_TryingLungeAttack(gentity_t *self, usercmd_t *cmd) {
  if (g_saberNewControlScheme
    ...
    if ((cmd->buttons & BUTTON_ATTACK)) { // pressing attack
      if (cmd->upmove < 0) {              // pressing crouch
        return qtrue;
      } else if (self && self->client) {                // just unducked?
        if ((self->client->ps.pm_flags & PMF_DUCKED)) { // just unducking
          return qtrue;
        }
      }
    }
...
// FIXME: for these below funcs, maybe in the old control scheme some moves
// should still cost power... if so, pass in the saberMove and use a switch
// statement
qboolean G_EnoughPowerForSpecialMove(int forcePower, int cost,
                                     qboolean kataMove) {
  if (g_saberNewControlScheme->integer || kataMove) { // special moves cost
                                                      // power
    if (forcePower >= cost) {
      return qtrue;
    } else {
  ...
  } else { // old control scheme: uses no power, so just do it
    return qtrue;
  }
}

void G_DrainPowerForSpecialMove(gentity_t *self, forcePowers_t fp, int cost,
                                qboolean kataMove) {
...
void WP_SaberRadiusDamage(gentity_t *ent, vec3_t point, float radius,
  ...
  if (!ent || !ent->client) {
    ...
    for (i = 0; i < numEnts; i++) {
      ...
      if (radiusEnts[i] == ent) { // Skip myself
        continue;
      }

      if (radiusEnts[i]->client == NULL) { // must be a client
        if (G_EntIsBreakable(
                radiusEnts[i]->s.number,
                ent)) { // damage breakables within range, but not as much
          G_Damage(radiusEnts[i], ent, ent, vec3_origin,
                   radiusEnts[i]->currentOrigin, 10, 0, MOD_EXPLOSIVE_SPLASH);
        }
        continue;
      }
      ...
      if ((radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_RANCOR) ||
          (radiusEnts[i]->client->ps.eFlags &
           EF_HELD_BY_WAMPA)) { // can't be one being held
        continue;
      }
      ...
      if (dist <= radius) { // in range
        if (damage > 0) {   // do damage
          ...
          G_Damage(radiusEnts[i], ent, ent, vec3_origin,
                   radiusEnts[i]->currentOrigin, points, DAMAGE_NO_KNOCKBACK,
...
void WP_SaberDamageTrace( gentity_t *ent, int saberNum, int bladeNum )
  ...
  Constantly trace from the old blade pos to new, down the saber beam and do
damage

  FIXME: if the dot product of the old muzzle dir and the new muzzle dir is <
0.75, subdivide it and do multiple traces so we don't flatten out the arc!
---------------------------------------------------------
*/
#define MAX_SABER_SWING_INC 0.33f
void WP_SaberDamageTrace(gentity_t *ent, int saberNum, int bladeNum) {
  vec3_t mp1, mp2, md1, md2, baseOld, baseNew, baseDiff, endOld, endNew,
      bladePointOld, bladePointNew;
  ...
  saberHitFraction =
      1.0; // Closest saber hit.  The saber can do no damage past this point.
  ...
  if (ent->client->ps.saber[saberNum].blade[bladeNum].length <=
      1) // cen get down to 1 when in a wall
  {      // saber is not on
    return;
  }
  ...
  int saberContents = 0;
  if (!(ent->client->ps.saber[saberNum].saberFlags &
        SFL_ON_IN_WATER)) { // saber can't stay on underwater
    saberContents = gi.pointcontents(ent->client->renderInfo.muzzlePoint,
                                     ent->client->ps.saberEntityNum);
  }
  if ((saberContents & CONTENTS_WATER) || (saberContents & CONTENTS_SLIME) ||
      (saberContents &
       CONTENTS_LAVA)) { // um... turn off?  Or just set length to 1?
    // FIXME: short-out effect/sound?
    ent->client->ps.saber[saberNum].blade[bladeNum].active = qfalse;
    return;
  } else if (!g_saberNoEffects &&
             gi.WE_IsOutside(ent->client->renderInfo.muzzlePoint)) {
  ...
  // FIXMEFIXMEFIXME: When in force speed (esp. lvl 3), need to interpolate this
  // because 		we animate so much faster that the arc is pretty much flat...
  ...
  if (ent->client->ps.saberInFlight) { // flying sabers are much more deadly
    ...
    if (ent->health <= 0 && g_saberRealisticCombat->integer <
                                2) { // so enemies don't keep trying to block it
      // FIXME: still do damage, just not to humanoid clients who should try to
      // avoid it baseDamage = 0.0f;
      return;
    }
    // or unless returning
    else if (ent->client->ps.saberEntityState == SES_RETURNING &&
             !(ent->client->ps.saber[0].saberFlags &
               SFL_RETURN_DAMAGE)) // type != SABER_STAR )
    ...
    VectorCopy(ent->client->ps.saber[saberNum].blade[bladeNum].muzzlePointOld,
               mp1);
    VectorCopy(ent->client->ps.saber[saberNum].blade[bladeNum].muzzleDirOld,
               md1);
    ...
    if (ent->client->ps.torsoAnim == BOTH_A7_HILT) { // no effects, no damage
      return;
      ...
      baseDamage = 0.1f;
               ...
               !PM_SaberInSpecialAttack(
                   ent->client->ps.torsoAnim)) { // just do effects
      ...
      if ((!WP_SaberBladeUseSecondBladeStyle(&ent->client->ps.saber[saberNum],
           ...
           (ent->client->ps.saber[saberNum].saberFlags2 &
            SFL2_NO_IDLE_EFFECT)) ||
          (WP_SaberBladeUseSecondBladeStyle(&ent->client->ps.saber[saberNum],
                                            bladeNum) &&
           (ent->client->ps.saber[saberNum].saberFlags2 &
            SFL2_NO_IDLE_EFFECT2))) { // do nothing at all when idle
        return;
      }
      baseDamage = 0;
      ...
      baseDamage = 0;
               ...
               ent->client->ps.saber[saberNum].knockbackScale <=
                   0.0f) { // this blade does no damage and no knockback (only
                           // for blocking?)
      baseDamage = 0;
    } else if (WP_SaberBladeUseSecondBladeStyle(
                   &ent->client->ps.saber[saberNum], bladeNum) &&
               ent->client->ps.saber[saberNum].damageScale2 <= 0.0f &&
               ent->client->ps.saber[saberNum].knockbackScale2 <=
                   0.0f) { // this blade does no damage and no knockback (only
                           // for blocking?)
      baseDamage = 0;
               ...
               (!PM_SaberInAttack(ent->client->ps.saberMove) &&
                ...
                !PM_SaberInTransitionAny(
                    ent->client->ps
                        .saberMove))) { // don't do damage if
                                        // parrying/reflecting/bouncing/deflecting
                                        // or not actually attacking or in a
                                        // transition to/from/between attacks
      if ((!WP_SaberBladeUseSecondBladeStyle(&ent->client->ps.saber[saberNum],
                                             bladeNum) &&
           (ent->client->ps.saber[saberNum].saberFlags2 &
            SFL2_NO_IDLE_EFFECT)) ||
      ...
      if (!PM_SaberInSpecialAttack(ent->client->ps.torsoAnim) &&
          !PM_InAnimForSaberMove(
              ...
              ent->client->ps.saberMove)) { // forced into some other animation
                                            // somehow, like a pain or death?
        if ((!WP_SaberBladeUseSecondBladeStyle(&ent->client->ps.saber[saberNum],
                                               bladeNum) &&
             (ent->client->ps.saber[saberNum].saberFlags2 &
              SFL2_NO_IDLE_EFFECT)) ||
        ...
        baseDamage = 0;
                 ...
                 (PM_SaberInAttack(ent->client->ps.saberMove) ||
                  ...
                  (WP_SaberBladeDoTransitionDamage(
                   ...
                   PM_SaberInTransitionAny(
                       ent->client->ps
                           .saberMove)))) // || ent->client->ps.saberAnimLevel
                                          // == SS_STAFF ) )
      { // normal attack swing swinging/spinning (or if using strong set), do
        // normal damage //FIXME: or if using staff?
      ...
      } else { // saber is transitioning, defending or idle, don't do as much
               // damage
        // FIXME: strong attacks and returns should do damage and be unblockable
        if (g_timescale->value < 1.0) { // in slow mo or force speed, we need to
                                        // do damage during the transitions
          if (g_saberRealisticCombat->integer) {
            switch (entPowerLevel) {
            case FORCE_LEVEL_3:
              baseDamage = 10.0f;
        ...
        { // I have to do *some* damage in transitions or else you feel like a
          // total gimp
          baseDamage = 0.1f;
        }
        /*
    ...
    //		arm to pass through them without any damage... so check the
    //radius 		and push them away (do pain & knockback)
    // FIXME: if going into/coming from a parry/reflection or going into a
    // deflection, don't use old mp & dir?  Otherwise, deflections will cut
    // through? VectorCopy( ent->client->renderInfo.muzzlePoint, mp1 );
    ...
    if (ent->client->ps.saberDamageDebounceTime >
        level.time) { // really only used when a saber attack start anim starts,
                      // not actually for stopping damage
    ...
    // NOTE: this is a test, may not be necc, as I can still swing right through
    // someone without hitting them, somehow... see if anyone is so close that
    // they're within the dist from my origin to the start of the saber
    if (ent->health > 0 && !ent->client->ps.saberLockTime && saberNum == 0 &&
        bladeNum == 0 &&
        !G_InCinematicSaberAnim(ent)) { // only do once - for first blade
      trace_t trace;
      gi.trace(&trace, ent->currentOrigin, vec3_origin, vec3_origin, mp1,
               ent->s.number, (MASK_SHOT & ~(CONTENTS_CORPSE | CONTENTS_ITEM)),
               (EG2_Collision)0, 0);
      ...
      {                       // a valid ent
        ...
        if (traceEnt && traceEnt->client &&
            ...
            !G_InCinematicSaberAnim(traceEnt)) { // enemy client, push them away
          if (!traceEnt->client->ps.saberLockTime && !traceEnt->message &&
              !(traceEnt->flags & FL_NO_KNOCKBACK) &&
              (!traceEnt->NPC ||
               traceEnt->NPC->jumpState !=
                   JS_JUMPING)) { // don't push people in saberlock or with
                                  // security keys or who are in BS_JUMP
            vec3_t hitDir;
            ...
            hitDir[2] = 0;
            // FIXME: do we need to call G_Throw?  Seems unfair to put actual
            // knockback on them, stops the attack G_Throw( traceEnt, hitDir,
            // knockback );
  ...
  baseDamage *= ent->client->ps.saber[saberNum].blade[bladeNum].radius /
  ...
  if (g_saberRealisticCombat->integer > 1) { // always do damage, and lots of it
    if (g_saberRealisticCombat->integer >
      ...
      baseDamage = 25.0f;
    } else if (baseDamage > 0.1f) { // only do super damage if we would have
                                    // done damage according to normal rules
      baseDamage = 25.0f;
    }
  } else if (((!ent->s.number &&
               ent->client->ps.forcePowersActive & (1 << FP_SPEED)) ||
  ...
  if (baseDamage > 0.1f) {
    if ((ent->client->ps.forcePowersActive &
         (1 << FP_RAGE))) { // add some damage if raged
      baseDamage += ent->client->ps.forcePowerLevel[FP_RAGE] * 5.0f;
    } else if (ent->client->ps.forceRageRecoveryTime) { // halve it if
                                                        // recovering
      baseDamage *= 0.5f;
    }
  }
  ...
  VectorMA(baseOld, ent->client->ps.saber[saberNum].blade[bladeNum].length, md1,
           endOld);
  // Get the future state of the blade
  VectorCopy(mp2, baseNew);
  VectorMA(baseNew, ent->client->ps.saber[saberNum].blade[bladeNum].length, md2,
           endNew);

  sabersCrossed = -1;
  if (VectorCompare2(baseOld, baseNew) && VectorCompare2(endOld, endNew)) {
    hit_wall = WP_SaberDamageForTrace(
        ent->s.number, mp2, endNew, baseDamage * 4, md2, qfalse, entPowerLevel,
    ...
    hit_wall = WP_SaberDamageForTrace(
        ent->s.number, baseOld, baseNew, baseDamage, md2, qfalse, entPowerLevel,
    ...
    if (saberHitFraction < 1.0) {
      ...
      VectorMA(baseNew, ent->client->ps.saber[saberNum].blade[bladeNum].length,
               md2, endNew);
    }
    ...
    while (1) {
      ...
      for (step = stepsize;
           step < ent->client->ps.saber[saberNum].blade[bladeNum].length &&
           step < ent->client->ps.saber[saberNum].blade[bladeNum].lengthOld;
           step += 12) {
        VectorMA(curBase1, step, curMD1, bladePointOld);
        VectorMA(curBase2, step, curMD2, bladePointNew);
        if (WP_SaberDamageForTrace(ent->s.number, bladePointOld, bladePointNew,
                                   baseDamage, curMD2, qfalse, entPowerLevel,
                                   ent->client->ps.saber[saberNum].type, qtrue,
                                   saberNum, bladeNum)) {
          hit_wall = qtrue;
        }
    ...
    if (WP_SaberDamageForTrace(
            ent->s.number, endOld, endNew, tipDmgMod * baseDamage, md2, qfalse,
            entPowerLevel, ent->client->ps.saber[saberNum].type, qfalse,
            saberNum, bladeNum)) {
      hit_wall = qtrue;
    }
  }
  ...
  if ((saberHitFraction < 1.0f ||
       (sabersCrossed >= 0 && sabersCrossed <= 32.0f)) &&
      (ent->client->ps.weaponstate == WEAPON_FIRING ||
       ent->client->ps.saberInFlight ||
       G_InCinematicSaberAnim(
           ent))) { // The saber (in-hand) hit another saber, mano.
    ...
    // FIXME: check for certain anims, facing, etc, to make them lock into a
    // sabers-locked pose SEF_LOCKED

    if (ent->client->ps.saberInFlight && saberNum == 0 &&
        ent->client->ps.saber[saberNum].blade[bladeNum].active &&
        ent->client->ps.saberEntityNum != ENTITYNUM_NONE &&
        ent->client->ps.saberEntityState !=
            SES_RETURNING) { // saber was blocked, return it
      inFlightSaberBlocked = qtrue;
    }

    // FIXME: based on strength, position and angle of attack & defense, decide
    // if: 	defender and attacker lock sabers 	*defender's parry should hold and
    //attack bounces (or deflects, based on angle of sabers) 	*defender's parry
    //is somewhat broken and both bounce (or deflect) 	*defender's parry is
    //broken and they bounce while attacker's attack deflects or carries through
    //(especially if they're dead) 	defender is knocked down and attack goes
    //through

    // Check deflections and broken parries
    if (hitOwner && hitOwner->health > 0 && ent->health > 0 // both are alive
        && !inFlightSaberBlocked && hitOwner->client &&
        ...
        hitOwner->client->ps.saberLockTime <
            level.time) { // 2 in-hand sabers hit
      // FIXME: defender should not parry or block at all if not in a saber
      // anim... like, if in a roll or knockdown...
      if (baseDamage) { // there is damage involved, not just effects
        ...
        if ((ent->client->NPC_class == CLASS_KYLE && (ent->spawnflags & 1) &&
             hitOwner->s.number < MAX_CLIENTS) ||
            (hitOwner->client->NPC_class == CLASS_KYLE &&
             (hitOwner->spawnflags & 1) &&
             ent->s.number <
                 MAX_CLIENTS)) { // Player vs. Kyle Boss == lots of saberlocks
          if (!Q_irand(0, 2)) {
            forceLock = qtrue;
          }
        }
        ...
        if (PM_SaberInAttack(ent->client->ps.saberMove) ||
            PM_SaberInSpecialAttack(ent->client->ps.torsoAnim)) {
          entAttacking = qtrue;
        } else if (entPowerLevel >
                   FORCE_LEVEL_2) { // stronger styles count as attacking even
                                    // if in a transition
        ...
        if (entAttacking) { // add twoHanded bonus and breakParryBonus to
                            // entPowerLevel here
        ...
        } else if (entDefending) { // add twoHanded bonus and dualSaber bonus
                                   // and parryBonus to entPowerLevel here
          if ((ent->client->ps.saber[saberNum].saberFlags & SFL_TWO_HANDED) ||
              (ent->client->ps.dualSabers &&
               ent->client->ps.saber[1].Active())) {
            entPowerLevel++;
          }
        ...
        if (hitOwnerAttacking) { // add twoHanded bonus and breakParryBonus to
                                 // entPowerLevel here
          if ((hitOwner->client->ps.saber[0].saberFlags & SFL_TWO_HANDED)) {
            hitOwnerPowerLevel++;
          }
          hitOwnerPowerLevel += hitOwner->client->ps.saber[0].breakParryBonus;
          if (hitOwner->client->ps.dualSabers &&
              Q_irand(0, 1)) { // FIXME: assumes both sabers are hitting at same
                               // time...?
            hitOwnerPowerLevel +=
                1 + hitOwner->client->ps.saber[1].breakParryBonus;
          }
        } else if (hitOwnerDefending) { // add twoHanded bonus and dualSaber
                                        // bonus and parryBonus to entPowerLevel
                                        // here
          if ((hitOwner->client->ps.saber[0].saberFlags & SFL_TWO_HANDED) ||
              (hitOwner->client->ps.dualSabers &&
               hitOwner->client->ps.saber[1].Active())) {
            hitOwnerPowerLevel++;
          }
          hitOwnerPowerLevel += hitOwner->client->ps.saber[0].parryBonus;
          if (hitOwner->client->ps.dualSabers &&
              Q_irand(0, 1)) { // FIXME: assumes both sabers are defending at
                               // same time...?
        ...
        if (PM_SuperBreakLoseAnim(ent->client->ps.torsoAnim) ||
            ...
            PM_SuperBreakWinAnim(
                hitOwner->client->ps.torsoAnim)) { // don't mess with this
          ...
          collisionResolved = qtrue;
                   ...
                   hitOwnerDefending) { // I'm attacking hit, they're parrying
          qboolean activeDefense =
              (qboolean)(hitOwner->s.number || g_saberAutoBlocking->integer ||
          ...
          if (!Q_irand(0, g_saberLockRandomNess->integer * 3) &&
            ...
            if (entPowerLevel < FORCE_LEVEL_3 &&
              ...
              // based on angle of attack & angle of defensive saber, see if I
              // should deflect off in another dir rather than bounce back
            ...
            // base parry breaks on animation (saber attack level), not
            // FP_SABER_OFFENSE
            if (entPowerLevel < FORCE_LEVEL_3
                //&& ent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] <
                //FORCE_LEVEL_3//if you have high saber offense, you cannot have
                //your attack knocked away, regardless of what style you're
                //using?
                ...
                (hitOwnerPowerLevel > FORCE_LEVEL_2 ||
                 (hitOwner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] >
                      FORCE_LEVEL_2 &&
                  Q_irand(0,
                          hitOwner->client->ps.forcePowerLevel
                              [FP_SABER_OFFENSE])))) { // knockaways can make
                                                       // fast-attacker go into a
                                                       // broken parry anim if
                                                       // the ent is using fast
                                                       // or med (but not Tavion)
              // make me parry
              WP_SaberParry(hitOwner, ent, saberNum, bladeNum);
              // turn the parry into a knockaway
              hitOwner->client->ps.saberBounceMove =
                  PM_KnockawayForParry(hitOwner->client->ps.saberBlocked);
              // make them go into a broken parry
              ent->client->ps.saberBounceMove =
                  PM_BrokenParryForAttack(ent->client->ps.saberMove);
              ent->client->ps.saberBlocked = BLOCKED_PARRY_BROKEN;
              if (saberNum ==
                  0) { // FIXME: can only lose right-hand saber for now
                if (!(ent->client->ps.saber[saberNum].saberFlags &
                      SFL_NOT_DISARMABLE) &&
                    ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE] <
                        FORCE_LEVEL_2
                    ...
                    (hitOwner->s.number || g_saberAutoBlocking->integer ||
                     !Q_irand(0, 2))) // if player defending and autoblocking is
                                      // on, this is less likely to happen, so
                                      // don't do the random check
                { // knocked the saber right out of his hand! (never happens to
                  // player)
                  // Get a good velocity to send the saber in based on my parry
                  // move
                  vec3_t throwDir;
                  if (!PM_VelocityForBlockedMove(&hitOwner->client->ps,
                                                 throwDir)) {
              ...
              if (d_saberCombat->integer) {
                gi.Printf(
                    S_COLOR_RED
                    "%s knockaway %s's attack, new move = %s, anim = %s\n",
                    hitOwner->NPC_type, ent->NPC_type,
                    saberMoveData[ent->client->ps.saberBounceMove].name,
                    animTable[saberMoveData[ent->client->ps.saberBounceMove]
                                  .animToUse]
                        .name);
              }
#endif
                       ...
                       || (entPowerLevel > FORCE_LEVEL_2 // I hit hard
                           && hitOwnerPowerLevel <
                                  entPowerLevel) // they are defending, but
                                                 // their defense strength is
                                                 // lower than my attack...
                       || (!deflected &&
                           Q_irand(0, Q_max(
                                          0, PM_PowerLevelForSaberAnim(
                                                 &ent->client->ps, saberNum) -
                                                 hitOwner->client->ps
                                                     .forcePowerLevel
                                                         [FP_SABER_DEFENSE]) /*PM_PowerLevelForSaberAnim( &hitOwner->client->ps )*/) >
                               0)) { // broke their parry altogether
              if (entPowerLevel > FORCE_LEVEL_2 ||
                  Q_irand(
                      0,
                      Q_max(0,
                            ent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] -
                                hitOwner->client->ps.forcePowerLevel
                                    [FP_SABER_DEFENSE]))) { // chance of
                                                            // continuing with
                                                            // the attack (not
                                                            // bouncing back)
                ent->client->ps.saberEventFlags &= ~SEF_BLOCKED;
              ...
              if (!(hitOwner->client->ps.saber[0].saberFlags &
                    SFL_NOT_DISARMABLE) &&
                  hitOwner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] <
                      FORCE_LEVEL_2
                  //&& (ent->s.number||g_saberRealisticCombat->integer)
                  &&
                  Q_irand(0, 2 - ent->client->ps.SaberDisarmBonus(bladeNum)) <=
                      0) { // knocked the saber right out of his hand!
...
#endif
            } else { // just a parry, possibly the hitOwner can knockaway the
                     // ent
              WP_SaberParry(hitOwner, ent, saberNum, bladeNum);
              if (PM_SaberInBounce(
                      ent->client->ps
                          .saberMove) // FIXME: saberMove not set until pmove!
                  && activeDefense &&
                  hitOwner->client->ps.saberAnimLevel !=
                      SS_FAST //&& hitOwner->client->ps.saberAnimLevel !=
                              //FORCE_LEVEL_5
                  && hitOwner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] >
                         FORCE_LEVEL_2) { // attacker bounced off, and defender
                                          // has ability to do knockaways, so do
                                          // one unless we're using fast attacks
                // turn the parry into a knockaway
                hitOwner->client->ps.saberBounceMove =
                    PM_KnockawayForParry(hitOwner->client->ps.saberBlocked);
              } else if ((ent->client->ps.saberAnimLevel == SS_STRONG &&
                          !Q_irand(0, 6)) ||
                         (ent->client->ps.saberAnimLevel == SS_DESANN &&
                          !Q_irand(0, 3))) { // a strong attack can sometimes do
                                             // a knockdown
        ...
        {//I'm parrying, they're attacking
                ...
                {//strong attacks cannot be deflected
                        //based on angle of attack & angle of defensive saber,
see if I should deflect off in another dir rather than bounce back deflected =
WP_GetSaberDeflectionAngle( hitOwner, ent );
                        //just so Jedi knows that he was blocked
                        hitOwner->client->ps.saberEventFlags |= SEF_BLOCKED;
                }
                //FIXME: base parry breaks on animation (saber attack level),
not FP_SABER_OFFENSE if ( hitOwnerPowerLevel > FORCE_LEVEL_2 || (!deflected &&
Q_irand( 0, PM_PowerLevelForSaberAnim( &hitOwner->client->ps ) -
ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE] ) > 0 ) )
                {//broke my parry altogether
                        if ( hitOwnerPowerLevel > FORCE_LEVEL_2 || Q_irand( 0,
hitOwner->client->ps.forcePowerLevel[FP_SABER_OFFENSE] -
ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE] ) )
                        {//chance of continuing with the attack (not bouncing
back) hitOwner->client->ps.saberEventFlags &= ~SEF_BLOCKED;
                                hitOwner->client->ps.saberBounceMove = LS_NONE;
                        }
                        //do some time-consuming saber-knocked-aside broken
parry anim ent->client->ps.saberBlocked = BLOCKED_PARRY_BROKEN; #ifndef
FINAL_BUILD if ( d_saberCombat->integer )
                        {
                                if (
hitOwner->client->ps.saberEventFlags&SEF_BLOCKED )
                                {
                                        gi.Printf( S_COLOR_RED"%s parry broken
(bounce/deflect)!\n", ent->targetname );
                                }
                                else
                                {
...
(follow-through)!\n", ent->targetname );
    ...
    if (saberHitFraction < 1.0f) {
      if (!collisionResolved &&
          baseDamage) { // some other kind of in-hand saber collision
        // handle my reaction
        if (!ent->client->ps.saberInFlight &&
          ...
          if (ent->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN) {
            if (PM_SaberInAttack(ent->client->ps.saberMove) ||
                ...
                (entPowerLevel > FORCE_LEVEL_2 &&
                 ...
                 !PM_SaberInReflect(
                     ent->client->ps
                         .saberMove))) { // in the middle of attacking
              if (entPowerLevel < FORCE_LEVEL_3 &&
                  hitOwner->health > 0) { // don't deflect/bounce in strong
                                          // attack or when enemy is dead
                WP_GetSaberDeflectionAngle(ent, hitOwner);
                ent->client->ps.saberEventFlags |= SEF_BLOCKED;
                // since it was blocked/deflected, take away any damage done
                // FIXME: what if the damage was done before the parry?
                WP_SaberClearDamageForEntNum(ent, hitOwner->s.number, saberNum,
                                             bladeNum);
              }
              ...
              WP_SaberClearDamageForEntNum(ent, hitOwner->s.number, saberNum,
                                           bladeNum);
              /*
              if ( ent->s.number || g_saberAutoBlocking->integer ||
              ...
              && !PM_SaberInBounce( ent->client->ps.saberMove ) )
                      {//I'm not attacking, in transition or in a bounce, so
              play a parry
                              //just so Jedi knows that he parried something
                              WP_SaberBlockNonRandom( ent, saberHitLocation,
              qfalse );
                      }
        ...
        } else { // nothing happens to *me* when my inFlight saber hits
                 // something
        }
        // handle their reaction
        if (hitOwner && hitOwner->health > 0 && hitOwner->client &&
            !hitOwner->client->ps.saberInFlight &&
            hitOwner->client->ps.saberLockTime <
                level.time) { // their saber is in hand
          if (PM_SaberInAttack(hitOwner->client->ps.saberMove) ||
              PM_SaberInSpecialAttack(hitOwner->client->ps.torsoAnim) ||
              (hitOwner->client->ps.saberAnimLevel > SS_MEDIUM &&
            ...
            */
            ...
            if (!PM_SaberInBrokenParry(
                    hitOwner->client->ps
                        .saberMove)) { // not currently in a broken parry
              if (!WP_SaberParry(
                      hitOwner, ent, saberNum,
                      bladeNum)) { // FIXME: hitOwner can't parry, do some
                                   // time-consuming saber-knocked-aside broken
                                   // parry anim?
        ...
        } else { // nothing happens to *hitOwner* when their inFlight saber hits
                 // something
        }
      }

      // collision must have been handled by now
      // Set the blocked attack bounce value in saberBlocked so we actually play
      // our saberBounceMove anim
      if (ent->client->ps.saberEventFlags & SEF_BLOCKED) {
        if (ent->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN) {
    ...
    if (saberHitFraction < 1.0f ||
        collisionResolved) { // either actually hit or locked
      if (ent->client->ps.saberLockTime < level.time) {
        if (inFlightSaberBlocked) { // FIXME: never hear this sound
          WP_SaberBounceSound(ent, hitOwner,
                              &g_entities[ent->client->ps.saberEntityNum], 0, 0,
                              qfalse);
        } else {
          if (deflected) {
        ...
        if (!g_saberNoEffects) {
          WP_SaberBlockEffect(ent, saberNum, bladeNum, saberHitLocation,
                              saberHitNormal, qfalse);
        }
      }
    ...
    if (saberHitFraction < 1.0f) {
      if (inFlightSaberBlocked) { // we threw a saber and it was blocked, do any
                                  // effects, etc.
        int knockAway = 5;
        if (hitEnt && hitOwner && hitOwner->client &&
            (PM_SaberInAttack(hitOwner->client->ps.saberMove) ||
             PM_SaberInSpecialAttack(hitOwner->client->ps.torsoAnim) ||
             PM_SpinningSaberAnim(
                 hitOwner->client->ps
                     .torsoAnim))) { // if hit someone who was in an attack or
                                     // spin anim, more likely to have in-flight
                                     // saber knocked away
          if (hitOwnerPowerLevel >
              FORCE_LEVEL_2) { // strong attacks almost always knock it aside!
            knockAway = 1;
          } else { // 33% chance
            knockAway = 2;
          }
        ...
        if (Q_irand(0, knockAway) <= 0 || // random
            (hitOwner && hitOwner->client && hitOwner->NPC &&
             (hitOwner->NPC->aiFlags &
              NPCAI_BOSS_CHARACTER)) // or if blocked by a Boss character FIXME:
                                     // or base on defense level?
            ) // FIXME: player should not auto-block a flying saber, let him
              // override the parry with an attack to knock the saber from the
              // air, rather than this random chance
        { // knock it aside and turn it off
          if (!g_saberNoEffects) {
            if (!WP_SaberBladeUseSecondBladeStyle(
                    &ent->client->ps.saber[saberNum], bladeNum) &&
                ent->client->ps.saber[saberNum].hitOtherEffect) {
              G_PlayEffect(ent->client->ps.saber[saberNum].hitOtherEffect,
                           saberHitLocation, saberHitNormal);
            } else if (WP_SaberBladeUseSecondBladeStyle(
                           &ent->client->ps.saber[saberNum], bladeNum) &&
          ...
          if (hitEnt) {
            ...
            VectorSubtract(
                g_entities[ent->client->ps.saberEntityNum].currentOrigin,
                hitEnt->currentOrigin, newDir);
            VectorNormalize(newDir);
            G_ReflectMissile(ent, &g_entities[ent->client->ps.saberEntityNum],
                             newDir);
          }
  ...
  if (ent->client->ps.saberLockTime > level.time) {
    if (ent->s.number < ent->client->ps.saberLockEnemy &&
      ...
      if (WP_SabersIntersection(ent,
                                &g_entities[ent->client->ps.saberLockEnemy],
                                g_saberFlashPos)) {
        if (Q_irand(0, 10)) {
          if (!g_saberNoEffects) {
            WP_SaberBlockEffect(ent, saberNum, bladeNum, g_saberFlashPos,
                                hitNorm, qfalse);
          }
        } else {
          if (!g_noClashFlare) {
        ...
        WP_SaberBlockSound(ent, &g_entities[ent->client->ps.saberLockEnemy], 0,
                           0);
      }
    }
  } else {
    if (hit_wall &&
        ...
        (PM_SaberInAttackPure(
             ent->client->ps.saberMove) // only in a normal attack anim
      ...
      ent->client->ps.saberBounceMove =
          LS_D1_BR +
          (saberMoveData[ent->client->ps.saberMove].startQuad - Q_BR);
      ...
      if (!WP_SaberBladeUseSecondBladeStyle(&ent->client->ps.saber[saberNum],
                                            bladeNum)) {
        WP_SaberRadiusDamage(ent, saberHitLocation,
                             ent->client->ps.saber[saberNum].splashRadius,
                             ent->client->ps.saber[saberNum].splashDamage,
                             ent->client->ps.saber[saberNum].splashKnockback);
      } else {
        WP_SaberRadiusDamage(ent, saberHitLocation,
                             ent->client->ps.saber[saberNum].splashRadius2,
  ...
  if (WP_SaberApplyDamage(
          ent, baseDamage, baseDFlags, brokenParry, saberNum, bladeNum,
          (qboolean)(saberNum == 0 &&
                     ent->client->ps
                         .saberInFlight))) { // actually did damage to something
#ifndef FINAL_BUILD
    if (d_saberCombat->integer) {
  ...
  if (hit_wall) {
    ...
    if (ent->s.number == 0) {
      AddSoundEvent(ent, ent->currentOrigin, 128, AEL_DISCOVERED, qfalse,
                    qtrue); // FIXME: is this impact on ground or not?
...
void WP_SabersDamageTrace(gentity_t *ent, qboolean noEffects) {
  ...
  for (int i = 0; i < ent->client->ps.saber[0].numBlades; i++) {
    // If the Blade is not active and the length is 0, don't trace it, try the
    // next blade...
    if (!ent->client->ps.saber[0].blade[i].active &&
        ent->client->ps.saber[0].blade[i].length == 0)
      continue;

    if (i != 0) { // not first blade
    ...
    if ((!WP_SaberBladeUseSecondBladeStyle(&ent->client->ps.saber[0], i) &&
         (ent->client->ps.saber[0].saberFlags2 & SFL2_NO_CLASH_FLARE)) ||
        (WP_SaberBladeUseSecondBladeStyle(&ent->client->ps.saber[0], i) &&
         (ent->client->ps.saber[0].saberFlags2 & SFL2_NO_CLASH_FLARE2))) {
  ...
  if (ent->client->ps.dualSabers) {
    for (int i = 0; i < ent->client->ps.saber[1].numBlades; i++) {
      // If the Blade is not active and the length is 0, don't trace it, try the
      // next blade...
      if (!ent->client->ps.saber[1].blade[i].active &&
          ent->client->ps.saber[1].blade[i].length == 0)
        continue;
      ...
      g_noClashFlare = qfalse;
      if ((!WP_SaberBladeUseSecondBladeStyle(&ent->client->ps.saber[1], i) &&
           (ent->client->ps.saber[1].saberFlags2 & SFL2_NO_CLASH_FLARE)) ||
          (WP_SaberBladeUseSecondBladeStyle(&ent->client->ps.saber[1], i) &&
           (ent->client->ps.saber[1].saberFlags2 & SFL2_NO_CLASH_FLARE2))) {
        g_noClashFlare = qtrue;
      }
...
void WP_SaberImpact(gentity_t *owner, gentity_t *saber, trace_t *trace) {
  ...
  if (other->takedamage &&
      (other->svFlags & SVF_BBRUSH)) { // a breakable brush?  break it!
    ...
    {                                               // can't actually break it
      // no hit effect (besides regular client-side one)
               ...
               (!owner || !owner->targetname ||
                Q_stricmp(
                    owner->targetname,
                    other->NPC_targetname))) { // only breakable by an entity
                                               // who is not the attacker no hit
                                               // effect (besides regular
                                               // client-side one)
    } else {
      vec3_t dir;
      ...
      if (other->health > 50 && dmg > 20 &&
          !(other->svFlags & SVF_GLASS_BRUSH)) {
        dmg = 20;
      }
      G_Damage(other, saber, owner, dir, trace->endpos, dmg, 0, MOD_SABER);
      if (owner && owner->client && owner->client->ps.saber[0].hitOtherEffect) {
        G_PlayEffect(owner->client->ps.saber[0].hitOtherEffect, trace->endpos,
                     dir);
      } else {
  ...
  if (owner && owner->s.number == 0 && owner->client) {
    ...
    if (owner->client->ps.SaberLength() > 0) { // saber is on, very suspicious
      if ((!owner->client->ps.saberInFlight &&
           owner->client->ps.groundEntityNum ==
               ENTITYNUM_WORLD) // holding saber and on ground
          || saber->s.pos.trType ==
                 TR_STATIONARY) // saber out there somewhere and on ground
      {                         // an on-ground alert
        AddSoundEvent(owner, saber->currentOrigin, 128, AEL_DISCOVERED, qfalse,
                      qtrue);
      } else { // an in-air alert
  ...
  if (!other->takedamage && (saber->s.eFlags & (EF_BOUNCE | EF_BOUNCE_HALF))) {
      ...
      if (--saber->bounceCount <= 0) {
        ...
        if (saber->s.pos.trType == TR_LINEAR && owner && owner->client &&
          ...
          if (!gi.inPVS(saber->currentOrigin,
                        owner->client->renderInfo
                            .handRPoint)) { // not in the PVS of my master
    ...
    // FIXME: make Luke/Desann/Tavion play an attack anim or some other special
    // anim when this happens
    Jedi_PlayDeflectSound(other);
  }
}

extern float G_PointDistFromLineSegment(const vec3_t start, const vec3_t end,
                                        const vec3_t from);
void WP_SaberInFlightReflectCheck(gentity_t *self, usercmd_t *ucmd) {
  gentity_t *ent;
  ...
  if (self->NPC &&
      (self->NPC->scriptFlags &
       SCF_IGNORE_ALERTS)) { // don't react to things flying at me...
    return;
  }
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (ent->s.eType != ET_MISSILE) {
      if (ent->client || ent->s.weapon != WP_SABER) { // FIXME: wake up bad
                                                      // guys?
        continue;
      }
      ...
      if (ent->s.pos.trType ==
          TR_STATIONARY) { // nothing you can do with a stationary missile
        continue;
      }
      if (ent->splashDamage ||
          ent->splashRadius) { // can't deflect exploding missiles
    ...
    for (int saberNum = 0; saberNum < numSabers; saberNum++) {
      for (int bladeNum = 0;
           bladeNum < self->client->ps.saber[saberNum].numBlades; bladeNum++) {
        VectorMA(self->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint,
                 self->client->ps.saber[saberNum].blade[bladeNum].length,
                 self->client->ps.saber[saberNum].blade[bladeNum].muzzleDir,
                 tip);

        if (G_PointDistFromLineSegment(
                self->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint,
                tip, ent->currentOrigin) <= 32) {
          willHit = qtrue;
          break;
        }
      }
      ...
      for (int x = 0; x < ent_count; x++) {
        if (missile_list[x]->s.weapon == WP_SABER) { // just send it back
          if (missile_list[x]->owner && missile_list[x]->owner->client &&
              missile_list[x]->owner->client->ps.saber[0].Active() &&
            ...
            WP_SaberBlockEffect(self, 0, 0, missile_list[x]->currentOrigin,
                                fx_dir, qfalse);
            if (missile_list[x]->owner->client->ps.saberInFlight &&
              ...
              // G_Sound( missile_list[x], G_SoundIndex( va(
              // "sound/weapons/saber/saberblock%d.wav", Q_irand(1, 9) ) ) );
              qboolean noFlare = qfalse;
              if ((missile_list[x]->owner->client->ps.saber[0].saberFlags2 &
                   SFL2_NO_CLASH_FLARE) &&
                  (self->client->ps.saber[0].saberFlags2 &
                   SFL2_NO_CLASH_FLARE)) {
                noFlare = qtrue;
              }
              if (!noFlare) {
                ...
                vec3_t org;
                VectorSubtract(missile_list[x]->currentOrigin,
                               saber->currentOrigin, org);
          ...
          // do an effect
          VectorNormalize2(missile_list[x]->s.pos.trDelta, fx_dir);
          G_PlayEffect("blaster/deflect", missile_list[x]->currentOrigin,
                       fx_dir);
        }
      }
    }
...
qboolean WP_SaberValidateEnemy(gentity_t *self, gentity_t *enemy) {
  ...
  */
  if (enemy->s.number >= MAX_CLIENTS) { // NPCs can cheat and use the homing
                                        // saber throw 3 on the player
    if (enemy->client->ps.forcePowersKnown) { // not other jedi?
      return qfalse;
    }
  }
  ...
  if (DistanceSquared(self->client->renderInfo.handRPoint,
                      enemy->currentOrigin) >
      saberThrowDistSquared[self->client->ps
                                .forcePowerLevel[FP_SABERTHROW]]) { // too far
    return qfalse;
  }
...
float WP_SaberRateEnemy(gentity_t *enemy, vec3_t center, vec3_t forward,
                        float radius) {
  float rating;
  vec3_t dir;
...
gentity_t *WP_SaberFindEnemy(gentity_t *self, gentity_t *saber) {
  ...
  if (WP_SaberValidateEnemy(self, saber->enemy)) {
    if (gi.inPVS(self->currentOrigin,
                 saber->enemy->currentOrigin)) { // potentially visible
      if (G_ClearLOS(self, self->client->renderInfo.eyePoint,
                     saber->enemy)) { // can see him
        bestEnt = saber->enemy;
  ...
  if (WP_SaberValidateEnemy(self, self->enemy)) {
    float myEnemyRating =
        WP_SaberRateEnemy(self->enemy, center, forward, radius);
    if (myEnemyRating > bestRating) {
      if (gi.inPVS(self->currentOrigin,
                   self->enemy->currentOrigin)) { // potentially visible
        if (G_ClearLOS(self, self->client->renderInfo.eyePoint,
                       self->enemy)) { // can see him
          bestEnt = self->enemy;
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (!WP_SaberValidateEnemy(
            self, ent)) { // doesn't meet criteria of valid look enemy (don't
                          // check current since we would have done that before
                          // this func's call
      continue;
    }
    if (!gi.inPVS(self->currentOrigin,
...
void WP_RunSaber(gentity_t *self, gentity_t *saber) {
  ...
  if (!self || !self->client ||
      self->client->ps.SaberLength() <=
          0) { // don't keep hitting other sabers when turned off
  ...
  if (self->client->ps.SaberActive()) {
    if (self->client->ps.saberInFlight ||
        (self->client->ps.weaponTime &&
         !Q_irand(0, 100))) { // make enemies run from a lit saber in flight or
                              // from me when I'm attacking
      if (!Q_irand(0, 10)) { // not so often...
        AddSightEvent(self, saber->currentOrigin,
                      self->client->ps.SaberLength() * 3, AEL_DANGER, 100);
      }
    }
  }
  ...
  if (saber->s.pos.trType == TR_LINEAR) { // home
    ...
    if (self->client->ps.forcePowerLevel[FP_SABERTHROW] > FORCE_LEVEL_1 ||
      ...
      if (self->client->ps.saberEntityDist <
          100) { // make the saber head to my hand- the bolt it was attached to
        VectorCopy(self->client->renderInfo.handRPoint, saberHome);
      } else { // aim saber from eyes
      ...
      if (self->client->ps.forcePowerLevel[FP_SABERTHROW] > FORCE_LEVEL_2 &&
        ...
        if (self->enemy &&
            !WP_SaberValidateEnemy(self,
                                   self->enemy)) { // if my enemy isn't valid to
                                                   // auto-aim at, don't autoaim
        } else {
          // pick an enemy
          ...
          if (enemy) { // home in on enemy
            ...
            saberDest[2] +=
                enemy->maxs[2] / 2.0f; // FIXME: when in a knockdown anim, the
                                       // saber float above them... do we care?
            self->client->ps.saberEntityDist = enemyDist;
            // once you pick an enemy, stay with it!
            saber->enemy = enemy;
            // FIXME: lock onto that enemy for a minimum amount of time (unless
            // they become invalid?)
          }
        }
      }
      ...
      } else if (self->client->ps.saberEntityState == SES_LEAVING &&
                 dist < 50) {
        saberSpeed = dist * 2 + 30;
        if ((enemy && dist > enemy->maxs[0]) ||
            (!enemy &&
             dist > 24)) {        // auto-tracking an enemy and we can't hit him
...
qboolean WP_SaberLaunch(
    gentity_t *self, gentity_t *saber, qboolean thrown,
    qboolean noFail = qfalse) { // FIXME: probably need a debounce time
  vec3_t saberMins = {-3.0f, -3.0f, -3.0f};
  vec3_t saberMaxs = {3.0f, 3.0f, 3.0f};
  trace_t trace;

  if (self->client->NPC_class ==
      CLASS_SABER_DROID) { // saber droids can't drop their saber
    return qfalse;
  }
  if (!noFail) {
    if (thrown) { // this is a regular throw, so see if it's legal
      if (self->client->ps.forcePowerLevel[FP_SABERTHROW] > FORCE_LEVEL_2) {
    ...
    if (!self->s.number &&
        (cg.zoomMode ||
         in_camera)) { // can't saber throw when zoomed in or in cinematic
      return qfalse;
    }
    // make sure it won't start in solid
    gi.trace(&trace, self->client->renderInfo.handRPoint, saberMins, saberMaxs,
             self->client->renderInfo.handRPoint, saber->s.number, MASK_SOLID,
             (EG2_Collision)0, 0);
    if (trace.startsolid || trace.allsolid) {
      return qfalse;
    }
    // make sure I'm not throwing it on the other side of a door or wall or
    // whatever
    ...
    if (thrown) { // this is a regular throw, so take force power
  ...
  // place it
  VectorCopy(self->client->renderInfo.handRPoint,
             saber->currentOrigin); // muzzlePoint
  VectorCopy(saber->currentOrigin, saber->s.pos.trBase);
  saber->s.pos.trTime = level.time;
  saber->s.pos.trType = TR_LINEAR;
  ...
  switch (
      self->client->ps.forcePowerLevel[FP_SABERTHROW]) { // FIXME: make a table?
  default:
  case FORCE_LEVEL_1:
  ...
  self->client->ps.saberEntityDist =
      saberThrowDist[self->client->ps.forcePowerLevel[FP_SABERTHROW]];
  ...
  if (thrown) { // this is a regular throw, so turn the saber on
    // turn saber on
    if ((self->client->ps.saber[0].saberFlags &
         SFL_SINGLE_BLADE_THROWABLE)) // SaberStaff() )
    {                                 // only first blade can be on
      if (!self->client->ps.saber[0].blade[0].active) { // turn on first one
        self->client->ps.SaberBladeActivate(0, 0);
      }
      for (int i = 1; i < self->client->ps.saber[0].numBlades;
           i++) { // turn off all others
        if (self->client->ps.saber[0].blade[i].active) {
          self->client->ps.SaberBladeActivate(0, i, qfalse);
        }
      }
    } else { // turn the sabers, all blades...?
...
qboolean WP_SaberLose(gentity_t *self, vec3_t throwDir) {
  ...
  if (self->client->ps.saber[0].Active()) { // on
    // drop it instantly
...
void WP_SetSaberOrigin(gentity_t *self, vec3_t newOrg) {
  ...
  if (self->client->ps.saberEntityNum <= 0 ||
      self->client->ps.saberEntityNum >=
          ENTITYNUM_WORLD) { // no saber ent to reposition
    return;
  }
  if (self->client->NPC_class ==
      CLASS_SABER_DROID) { // saber droids can't drop their saber
  ...
  gentity_t *dropped = &g_entities[self->client->ps.saberEntityNum];
  if (!self->client->ps.saberInFlight) { // not already in air
    qboolean noForceThrow = qfalse;
    // make it so we can throw it
    self->client->ps.forcePowersKnown |= (1 << FP_SABERTHROW);
    if (self->client->ps.forcePowerLevel[FP_SABERTHROW] < FORCE_LEVEL_1) {
    ...
    // throw it
    if (!WP_SaberLaunch(self, dropped, qfalse, qtrue)) { // couldn't throw it
      return;
    }
    if (noForceThrow) {
      self->client->ps.forcePowerLevel[FP_SABERTHROW] = FORCE_LEVEL_0;
...
void WP_SaberCatch(
    gentity_t *self, gentity_t *saber,
  ...
  if (self->health > 0 && !PM_SaberInBrokenParry(self->client->ps.saberMove) &&
    ...
    if (self->client->ps.weapon ==
        WP_SABER) { // only do the first saber since we only throw the first one
    ...
    if (switchToSaber) {
      if (self->client->ps.weapon != WP_SABER) {
        ...
        if ((self->client->ps.saber[0].saberFlags &
             SFL_SINGLE_BLADE_THROWABLE)) // SaberStaff() )
        {                                 // only first blade can be on
          if (!self->client->ps.saber[0]
                   .blade[0]
                   .active) { // only turn it on if first blade is off,
                              // otherwise, leave as-is
            self->client->ps.saber[0].Activate();
          }
        } else { // turn all blades on
          self->client->ps.saber[0].Activate();
        }
      }
    }
...
void WP_SaberReturn(gentity_t *self, gentity_t *saber) {
  ...
  if (self && self->client) { // still alive and stuff
    // FIXME: when it's returning, flies butt first, but seems to do a lot of
    // damage when going through people... hmm...
    self->client->ps.saberEntityState = SES_RETURNING;
...
void WP_SaberThrow(gentity_t *self, usercmd_t *ucmd) {
  ...
  if (self->client->ps.saberEntityNum <= 0 ||
      self->client->ps.saberEntityNum >=
          ENTITYNUM_WORLD) { // WTF?!!  We lost it?
    return;
  }

  if (self->client->ps.torsoAnim ==
      BOTH_LOSE_SABER) { // can't catch it while it's being yanked from your
                         // hand!
    return;
  }

  if (!g_saberNewControlScheme->integer) {
    if (PM_SaberInKata((saberMoveName_t)self->client->ps
                           .saberMove)) { // don't throw saber when in special
                                          // attack (alt+attack)
      return;
    }
    if ((ucmd->buttons & BUTTON_ATTACK) &&
        ...
        !self->client->ps
             .saberInFlight) { // trying to do special attack, don't throw it
      return;
    } else if (self->client->ps.torsoAnim == BOTH_A1_SPECIAL ||
               self->client->ps.torsoAnim == BOTH_A2_SPECIAL ||
               self->client->ps.torsoAnim ==
                   BOTH_A3_SPECIAL) { // don't throw in these anims!
      return;
    }
  }
  ...
  if (!self->client->ps.saberInFlight) { // saber is not in flight right now
    if (self->client->ps.weapon != WP_SABER) { // don't even have it out
      return;
               ...
               !(self->client->ps.pm_flags &
                 PMF_ALT_ATTACK_HELD)) { // still holding it, not still holding
                                         // attack from a previous throw, so
                                         // throw it.
      if (!(self->client->ps.saberEventFlags & SEF_INWATER) &&
        ...
        VectorSubtract(self->client->renderInfo.handRPoint,
                       saberent->currentOrigin, saberDiff);
    ...
    if (self->client->ps.saberEntityState ==
      ...
      if (VectorLengthSquared(saberDiff) <=
          256) // 16 squared//G_BoundsOverlap( self->absmin, self->absmax,
               // saberent->absmin, saberent->absmax ) )//
      { // caught it
        vec3_t axisPoint;
        trace_t trace;
        ...
        gi.trace(&trace, axisPoint, vec3_origin, vec3_origin,
                 self->client->renderInfo.handRPoint, self->s.number,
                 MASK_SOLID, (EG2_Collision)0, 0);
        if (!trace.startsolid &&
            trace.fraction >= 1.0f) { // our hand isn't through a wall
          WP_SaberCatch(self, saberent, qtrue);
          // NPC_SetAnim( self, SETANIM_TORSO, TORSO_HANDRETRACT1,
    ...
    if (saberent->s.pos.trType != TR_STATIONARY) { // saber is in flight, lerp
                                                   // it
      ...
      {                      // make us free ourselves after a time
        if (g_saberPickuppableDroppedSabers->integer &&
            G_DropSaberItem(self->client->ps.saber[0].name,
                            self->client->ps.saber[0].blade[0].color,
                            saberent->currentOrigin, saberent->s.pos.trDelta,
                            saberent->currentAngles) != NULL) { // dropped it
          // free it
      ...
      if (self->health <= 0) //&& level.time > saberent->s.time + 5000 )
      {                      // make us free ourselves after a time
        if (g_saberPickuppableDroppedSabers->integer) { // spawn an item
          G_DropSaberItem(self->client->ps.saber[0].name,
                          self->client->ps.saber[0].blade[0].color,
                          saberent->currentOrigin, saberent->s.pos.trDelta,
                          saberent->currentAngles);
        }
        // free it
        G_FreeEntity(saberent);
        // forget it
      ...
      if ((!self->s.number && level.time - saberent->aimDebounceTime > 15000) ||
          (self->s.number && level.time - saberent->aimDebounceTime >
                                 5000)) { //(only for player) been missing for
                                          //15 seconds, automagicially return
  ...
  if (self->client->ps.weapon != WP_SABER) { // switched away
    if (!self->client->ps.saberInFlight) {   // wasn't throwing saber
      return;
               ...
               TR_LINEAR) { // switched away while controlling it, just drop the
                            // saber
      WP_SaberDrop(self, saberent);
      return;
    } else { // it's on the ground, see if it's inside us (touching)
      if (G_PointInBounds(
              saberent->currentOrigin, self->absmin,
              self->absmax)) { // it's in us, pick it up automatically
        WP_SaberPull(self, saberent);
      }
    }
  } else if (saberent->s.pos.trType !=
             TR_LINEAR) {                 // weapon is saber and not flying
    if (self->client->ps.saberInFlight) { // we dropped it
      if (ucmd->buttons &
          BUTTON_ATTACK) //|| self->client->ps.weaponstate == WEAPON_RAISING
                         //)//ucmd->buttons & BUTTON_ALT_ATTACK ||
      { // we actively want to pick it up or we just switched to it, so pull it
        // back
        ...
        if (G_PointInBounds(
          ...
          WP_SaberPull(self, saberent);
             ...
             self->client->ps.saberInFlight) { // we died, drop it
    WP_SaberDrop(self, saberent);
    return;
  } else if (!self->client->ps.saber[0].Active() &&
             self->client->ps.saberEntityState !=
                 SES_RETURNING) { // we turned it off, drop it
    WP_SaberDrop(self, saberent);
    return;
  }
  ...
  if (self->client->ps.saberEntityState ==
      SES_LEAVING) { // saber still flying forward
    if (self->client->ps.forcePowerLevel[FP_SABERTHROW] >
        FORCE_LEVEL_2) { // still holding it out
      if (!(ucmd->buttons & BUTTON_ALT_ATTACK) &&
          self->client->ps.forcePowerDebounce[FP_SABERTHROW] <
              level.time) {                       // done throwing, return to me
        if (self->client->ps.saber[0].Active()) { // still on
      ...
      if (!(ucmd->buttons & BUTTON_ALT_ATTACK) &&
          self->client->ps.forcePowerDebounce[FP_SABERTHROW] <
              level.time) { // not holding button and has been out at least 1
                            // second, return to me
        if (self->client->ps.saber[0].Active()) { // still on
          WP_SaberReturn(self, saberent);
        }
      } else if (level.time - self->client->ps.saberThrowTime > 3000 ||
                 (self->client->ps.forcePowerLevel[FP_SABERTHROW] ==
                      FORCE_LEVEL_1 &&
                  saberDist >= self->client->ps
                                   .saberEntityDist)) { // been out too long, or
                                                        // saber throw 1 went too
                                                        // far, return to me
        if (self->client->ps.saber[0].Active()) { // still on
          WP_SaberReturn(self, saberent);
        }
  ...
  if (self->client->ps.saberEntityState == SES_RETURNING) {
    ...
    if (self->client->ps.saberEntityDist < 0) {
      self->client->ps.saberEntityDist = 0;
               ...
               self->client->ps.saberEntityDist) { // if it's coming back to me,
                                                   // never push it away
...
void WP_SaberBlockNonRandom(gentity_t *self, vec3_t hitloc,
  ...
  if (!missileBlock && self->s.number != 0 &&
  ...
  } else if (zdiff > -22) //-20 )//20 )
  {
    ...
    { // hmm, pretty low, but not low enough to use the low block, so we need to
      // duck NPC should duck, but NPC should never get here
    }
    if (rightdot > 0.1) {
      self->client->ps.saberBlocked = BLOCKED_UPPER_RIGHT;
  ...
  if (d_saberCombat->integer) {
    if (!self->s.number) {
      gi.Printf("EyeZ: %4.2f  HitZ: %4.2f  zdiff: %4.2f  rdot: %4.2f\n",
                self->client->renderInfo.eyePoint[2], hitloc[2], zdiff,
                rightdot);
...
void WP_SaberStartMissileBlockCheck(gentity_t *self, usercmd_t *ucmd) {
  ...
  if (PM_SuperBreakLoseAnim(self->client->ps.torsoAnim) ||
      PM_SuperBreakWinAnim(
          self->client->ps.torsoAnim)) { // can't block while in break anim
    return;
  }
  ...
  if (self->client->NPC_class == CLASS_ROCKETTROOPER) {       // rockettrooper
    if (self->client->ps.groundEntityNum != ENTITYNUM_NONE) { // must be in air
      return;
    }
    if (Q_irand(0, 4 - (g_spskill->integer *
                        2))) { // easier level guys do this less
      return;
    }
    if (Q_irand(
            0, 3)) { // base level: 25% chance of looking for something to dodge
      if (Q_irand(0,
                  1)) { // dodge sabers twice as frequently as other projectiles
  ...
  if (self->client->NPC_class ==
      CLASS_BOBAFETT) { // Boba doesn't dodge quite as much
    if (Q_irand(0, 2 - g_spskill->integer)) { // easier level guys do this less
      return;
    }
  }
  ...
  if (self->client->NPC_class != CLASS_BOBAFETT &&
      ...
      (self->client->NPC_class != CLASS_ROCKETTROOPER || !self->NPC ||
       self->NPC->rank < RANK_LT) // if a rockettrooper, but not an officer, do
                                  // these normal checks
  ) {
    if (g_debugMelee->integer && (ucmd->buttons & BUTTON_USE) &&
      ...
      if (self->s.number < MAX_CLIENTS) {
        if (!self->client->ps.SaberLength()) { // player doesn't auto-activate
          return;
        }
      ...
      if ((self->client->ps.saber[0].saberFlags &
           SFL_NOT_ACTIVE_BLOCKING)) { // can't actively block with this saber
                                       // type
        return;
      }
    }
    ...
    if (self->client->ps.forcePowerLevel[FP_SABER_DEFENSE] <
        FORCE_LEVEL_1) { // you have not the SKILLZ
      return;
    }

    if (self->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] >
        level.time) { // can't block while already blocking
      return;
    }

    if (self->client->ps.forcePowersActive &
    ...
    if (self->client->ps.forcePowersActive &
        (1
         << FP_GRIP)) { // can't block while gripping (FIXME: or should it break
                        // the grip?  Pain should break the grip, I think...)
      return;
    }
  }
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (!(ent->inuse))
      continue;
    if (dodgeOnlySabers) { // only care about thrown sabers
      if (ent->client || ent->s.weapon != WP_SABER || !ent->classname ||
          !ent->classname[0] ||
          Q_stricmp("lightsaber",
                    ent->classname)) { // not a lightsaber, ignore it
        continue;
      }
    }
    if (ent->s.eType != ET_MISSILE &&
      ...
      if (Q_stricmp("lightsaber", ent->classname) != 0) { // not a lightsaber
        // FIXME: what about general objects that are small in size- like rocks,
        // etc...
        continue;
      }
      ...
      if (ent->owner->health <= 0 &&
          g_saberRealisticCombat->integer <
              2) { // it's not doing damage, so ignore it
        continue;
      }
    } else {
      if (ent->s.pos.trType == TR_STATIONARY &&
          !self->s.number) { // nothing you can do with a stationary missile if
                             // you're the player
        continue;
      }
    }
    ...
    if (ent->s.weapon == WP_THERMAL) { // thermal detonator!
      if (self->NPC && dist < ent->splashRadius) {
        if (dist < ent->splashRadius && ent->nextthink < level.time + 600 &&
            ...
            (ent->s.pos.trType == TR_STATIONARY ||
             ...
             !WP_ForcePowerUsable(
                 self, FP_PUSH,
                 0))) { // TD is close enough to hurt me, I'm on the ground and
                        // the thing is at rest or behind me and about to blow
                        // up, or I don't have force-push so force-jump!
          ...
          self->client->ps.forceJumpCharge = 480;
            ...
            self->client->NPC_class !=
                CLASS_ROCKETTROOPER) { // FIXME: check
      ...
      if (!self->s.number) { // players don't auto-handle these at all
        ...
        if (self->client->NPC_class == CLASS_BOBAFETT ||
          ...
          */
          // HMM... let's see what happens if these guys try to avoid tripmines
          // and detpacks, too...?
        } else { // normal Jedi
          if (ent->s.pos.trType == TR_STATIONARY &&
              (ent->s.eFlags & EF_MISSILE_STICK) &&
              (self->client->NPC_class != CLASS_REBORN ||
               self->s.weapon ==
                   WP_SABER)) { // a placed explosive like a tripmine or detpack
                ...
                ent->nextthink = level.time + Q_irand(500, 3000);
                     ...
                     (DotProduct(dir, forward) < SABER_REFLECT_MISSILE_CONE ||
                      !WP_ForcePowerUsable(self, FP_PUSH,
                                           0))) { // NPCs try to evade it
            self->client->ps.forceJumpCharge = 480;
          } else if ((self->client->NPC_class != CLASS_REBORN ||
                      self->s.weapon ==
                          WP_SABER)) { // else, try to force-throw it away
            if (!ent->owner || !OnSameTeam(self, ent->owner)) {
              // FIXME: check
              // forcePushRadius[NPC->client->ps.forcePowerLevel[FP_PUSH]]
              ForceThrow(self, qfalse);
    ...
    if (dist < closestDist) {
      ...
      gi.trace(&trace, ent->currentOrigin, ent->mins, ent->maxs, traceTo,
      ...
      if (self->s.number != 0) { // An NPC
        if (self->NPC && !self->enemy && ent->owner) {
          if (ent->owner->health >= 0 &&
              (!ent->owner->client ||
               ent->owner->client->playerTeam != self->client->playerTeam)) {
  ...
  if (incoming) {
    if (self->NPC && !G_ControlledByPlayer(self)) {
      ...
      if ((self->client->NPC_class == CLASS_BOBAFETT ||
          ...
          incoming->methodOfDeath !=
              MOD_ROCKET_ALT) { // a hovering Boba Fett, not a tracking rocket
        ...
        if (!Q_irand(0, 1)) { // go up/down
          ...
          self->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] =
              level.time + Q_irand(1000, 2000);
                 ...
                 Jedi_SaberBlockGo(self, &self->NPC->last_ucmd, NULL, NULL,
                                   incoming) !=
                     EVASION_NONE) { // make sure to turn on your saber if it's
                                     // not on
        if (self->client->NPC_class != CLASS_BOBAFETT &&
            (self->client->NPC_class != CLASS_REBORN ||
             self->s.weapon == WP_SABER)) {
    ...
    {
      if (!(ucmd->buttons & BUTTON_USE)) // self->s.weapon == WP_SABER &&
...
extern void CG_CubeOutline(vec3_t mins, vec3_t maxs, int time,
...
void WP_SaberUpdate(gentity_t *self, usercmd_t *ucmd) {
  ...
  // Check if we are throwing it, launch it if needed, update position if
  // needed.
  ...
  // FIXME: Based on difficulty level/jedi saber combat skill, make this
  // bounding box fatter/smaller
  if (self->client->ps.saberBlocked !=
      BLOCKED_NONE) { // we're blocking, increase min size
    minsize = 32;
  }

  if (G_InCinematicSaberAnim(self)) { // fake some blocking
  ...
  // is our saber in flight?
  if (!self->client->ps.saberInFlight) { // It isn't, which means we can update
                                         // its position as we will.
    ...
    if (!self->client->ps.SaberActive() || !self->client->ps.saberBlocking ||
        PM_InKnockDown(&self->client->ps) ||
        PM_SuperBreakLoseAnim(self->client->ps.torsoAnim) ||
        (pVeh && pVeh->m_pVehicleInfo &&
         pVeh->m_pVehicleInfo->type != VH_ANIMAL &&
         pVeh->m_pVehicleInfo->type !=
    ...
    {                   // can't block if saber isn't on
      ...
      for (i = 0; i < MAX_SABERS; i++) {
        ...
        if (i > 0 &&
            !self->client->ps.dualSabers) { // not using a second saber, leave
                                            // it not blocking
        } else {
          ...
          if (self->client->ps.saber[i].bladeStyle2Start > 0) {
            for (j = self->client->ps.saber[i].bladeStyle2Start;
                 j < self->client->ps.saber[i].numBlades; j++) {
              if ((self->client->ps.saber[i].saberFlags2 &
                   SFL2_ALWAYS_BLOCK2)) {
                alwaysBlock[i][j] = qtrue;
                forceBlock = qtrue;
              } else {
      ...
      if (!forceBlock) {
        noBlocking = qtrue;
      } else if (!self->client->ps.saberBlocking) { // turn blocking on!
        self->client->ps.saberBlocking = BLK_TIGHT;
      }
    }
    if (noBlocking) {
      ...
      G_SetOrigin(saberent, self->currentOrigin);
               ...
               self->client->ps.saberBlocking ==
                   BLK_WIDE) { // FIXME: keep bbox in front of player, even when
                               // wide?
      vec3_t saberOrg;
      if (!forceBlock &&
          ...
          !G_InCinematicSaberAnim(
              self)) { // full-size blocking for non-attacking player with
                       // g_saberAutoBlocking on
        vec3_t saberang = {0, 0, 0}, fwd, sabermins = {-8, -8, -8},
               sabermaxs = {8, 8, 8};
        ...
        for (int saberNum = 0; saberNum < numSabers; saberNum++) {
          for (int bladeNum = 0;
               bladeNum < self->client->ps.saber[saberNum].numBlades;
               bladeNum++) {
            if (self->client->ps.saber[saberNum].blade[bladeNum].length <=
                0.0f) { // don't include blades that are not on...
              continue;
            }
            if (forceBlock) { // doing blade-specific bbox-sizing only, see if
                              // this blade should be counted
              if (!alwaysBlock[saberNum][bladeNum]) { // this blade doesn't
                                                      // count right now
                continue;
              }
            }
            ...
            for (int i = 0; i < 3; i++) {
              /*
              if ( saberTip[i] >
              self->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint[i] )
              {
                      saberent->maxs[i] = saberTip[i] - saberOrg[i] +
              8;//self->client->renderInfo.muzzlePoint[i]; saberent->mins[i] =
              self->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint[i] -
              saberOrg[i] - 8;
              }
              ...
              )
              {
                      saberent->maxs[i] =
              self->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint[i] -
              saberOrg[i] + 8; saberent->mins[i] = saberTip[i] - saberOrg[i] -
              8;//self->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint[i];
              }
              */
        ...
        if (!forceBlock) { // not doing special "alwaysBlock" bbox
          if (self->client->ps.weaponTime > 0 || self->s.number ||
              g_saberAutoBlocking->integer ||
              self->client->ps.saberBlockingTime >
                  level.time) { // if attacking or blocking (or an NPC), inflate
                                // to a minimum size
            for (int i = 0; i < 3; i++) {
    ...
    {	// Assuming that we are not swinging, the saber's bounding box should be
    around the player. vec3_t saberang={0,0,0}, fwd;
    ...
    {	// If the player is swinging, the bbox is around just the saber
            ...
            VectorMA(sabermins, self->client->ps.saberLength,
    self->client->renderInfo.muzzleDir, sabermaxs);
            ...
            // Finally, turn the mins and maxs, which are absolute, into
    relative mins and maxs. VectorSubtract(sabermins, saberloc, saberent->mins);
...
#ifndef FINAL_BUILD
  if (d_saberCombat->integer > 2) {
    CG_CubeOutline(saberent->absmin, saberent->absmax, 50,
...
qboolean G_CheckEnemyPresence(gentity_t *ent, int dir, float radius,
                              float tolerance) {
  ...
  // Get all ents in range, see if they're living clients and enemies, then
  // check dot to them...
  ...
  for (i = 0; i < numEnts; i++) {
    ...
    VectorSubtract(radiusEnts[i]->currentOrigin, ent->currentOrigin,
                   dir2checkEnt);
    dist = VectorNormalize(dir2checkEnt);
    if (dist <= radius && DotProduct(dir2checkEnt, checkDir) >= tolerance) {
...
void WP_DropWeapon(gentity_t *dropper, vec3_t velocity) {
  ...
  if (oldWeap == WP_THERMAL &&
      dropper->NPC) { // Hmm, maybe all NPCs should go into melee?  Not too
                      // many, though, or they mob you and look silly
    replaceWeap = WP_MELEE;
  }
  if (dropper->ghoul2.IsValid()) {
    if (dropper->weaponModel[0] >
        0) { // NOTE: guess you never drop the left-hand weapon, eh?
  ...
  if (!dropper->s.number) {
    if (oldWeap == WP_THERMAL) {
      dropper->client->ps.ammo[weaponData[oldWeap].ammoIndex] -=
          weaponData[oldWeap].energyPerShot;
    } else {
  ...
  if (weapon != NULL && velocity &&
      !VectorCompare(
          velocity,
          vec3_origin)) { // weapon should have a direction to it's throw
    VectorScale(
        velocity, 3,
        weapon->s.pos.trDelta); // NOTE: Presumes it is moving already...?
    if (weapon->s.pos.trDelta[2] <
        150) { // this is presuming you don't want them to drop the weapon down
               // on you...
      weapon->s.pos.trDelta[2] = 150;
    }
    // FIXME: gets stuck inside it's former owner...
    weapon->forcePushTime =
...
void WP_ForceThrowHazardTrooper(gentity_t *self, gentity_t *trooper,
                                qboolean pull) {
  if (!self || !self->client) {
    return;
  }
  ...
  trooper->forcePushTime =
      level.time + 600; // let the push effect last for 600 ms

  if ((pull && self->client->ps.forcePowerLevel[FP_PULL] > FORCE_LEVEL_1) ||
      (!pull && self->client->ps.forcePowerLevel[FP_PUSH] >
                    FORCE_LEVEL_1)) { // level 2: they stop for a couple seconds
                                      // and make a sound
    trooper->painDebounceTime = level.time + Q_irand(1500, 2500);
    G_AddVoiceEvent(trooper, Q_irand(EV_PUSHED1, EV_PUSHED3),
                    Q_irand(1000, 3000));
    ...
    if ((pull && self->client->ps.forcePowerLevel[FP_PULL] > FORCE_LEVEL_2) ||
        (!pull && self->client->ps.forcePowerLevel[FP_PUSH] >
                      FORCE_LEVEL_2)) { // level 3: they actually play a pushed
                                        // anim and stumble a bit
      vec3_t hazAngles = {0, trooper->currentAngles[YAW], 0};
      int anim = -1;
      if (InFront(self->currentOrigin, trooper->currentOrigin,
                  hazAngles)) { // I'm on front of him
        if (pull) {
          anim = BOTH_PAIN4;
        } else {
          anim = BOTH_PAIN1;
      ...
      if (anim != -1) {
        if (anim == BOTH_PAIN1) { // make them take a couple steps back
          AngleVectors(hazAngles, trooper->client->ps.velocity, NULL, NULL);
          VectorScale(trooper->client->ps.velocity, -40.0f,
                      trooper->client->ps.velocity);
          trooper->client->ps.pm_flags |= PMF_TIME_NOFRICTION;
        } else if (anim == BOTH_PAIN4) { // make them stumble forward
          AngleVectors(hazAngles, trooper->client->ps.velocity, NULL, NULL);
          VectorScale(trooper->client->ps.velocity, 80.0f,
                      trooper->client->ps.velocity);
          trooper->client->ps.pm_flags |= PMF_TIME_NOFRICTION;
        }
        NPC_SetAnim(trooper, SETANIM_BOTH, anim,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  ...
  } else { // level 1: no pain reaction, but they should still notice
    if (trooper->enemy == NULL // not mad at anyone
        && trooper->client->playerTeam !=
               self->client->playerTeam             // not on our team
...
void WP_ResistForcePush(gentity_t *self, gentity_t *pusher,
  ...
  if (!PM_SaberCanInterruptMove(
          self->client->ps.saberMove,
          self->client->ps
              .torsoAnim)) { // can't interrupt my current torso anim/sabermove
                             // with this, so ignore it entirely!
    return;
  }
  ...
  if (!noPenalty) {
    if (!runningResist) {
      ...
      if (self->client->ps.forcePowersActive & (1 << FP_SPEED)) {
        self->client->ps.weaponTime =
            floor(self->client->ps.weaponTime * g_timescale->value);
      }
  ...
  // self->client->ps.powerups[PW_FORCE_PUSH] = level.time +
  // self->client->ps.torsoAnimTimer + 500;
  ...
  if (!pusher                  //???
      ...
      ||
      (pusher->client &&
       pusher->client->playerTeam !=
           self->client->playerTeam)) // someone not on my team tried to push me
  {
    Jedi_PlayBlockedPushSound(self);
  }
}

extern qboolean Boba_StopKnockdown(gentity_t *self, gentity_t *pusher,
                                   const vec3_t pushDir,
                                   qboolean forceKnockdown);
extern qboolean Jedi_StopKnockdown(gentity_t *self, gentity_t *pusher,
                                   const vec3_t pushDir);
void WP_ForceKnockdown(gentity_t *self, gentity_t *pusher, qboolean pull,
                       qboolean strongKnockdown, qboolean breakSaberLock) {
  ...
  if (self->client->NPC_class == CLASS_ROCKETTROOPER) {
    return;
  } else if (PM_LockedAnim(
                 self->client->ps.legsAnim)) { // stuck doing something else
    return;
  ...
  if (self->health > 0) {
    ...
    if (Boba_StopKnockdown(
            self, pusher, pushDir,
            qtrue)) { // He can backflip instead of be knocked down
      return;
    } else if (Jedi_StopKnockdown(
                   self, pusher,
                   pushDir)) { // They can backflip instead of be knocked down
      return;
    }
    ...
    if (!PM_SpinningSaberAnim(self->client->ps.legsAnim) &&
      ...
      if (pusher->client->NPC_class == CLASS_DESANN &&
          self->client->NPC_class !=
              CLASS_LUKE) { // desann always knocks down, unless you're Luke
        strongKnockdown = qtrue;
      }
      if (!self->s.number && !strongKnockdown &&
          ((!pull &&
            (self->client->ps.forcePowerLevel[FP_PUSH] > FORCE_LEVEL_1 ||
             !g_spskill->integer)) ||
           (pull &&
            (self->client->ps.forcePowerLevel[FP_PULL] > FORCE_LEVEL_1 ||
             !g_spskill
                  ->integer)))) { // player only knocked down if pushed *hard*
        if (self->s.weapon ==
            WP_SABER) { // temp HACK: these are the only 2 pain anims that look
                        // good when holding a saber
      ...
      NPC_SetAnim(self, SETANIM_BOTH, knockAnim,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      if (self->s.number >=
          MAX_CLIENTS) { // randomize getup times - but not for boba
        int addTime;
        if (self->client->NPC_class == CLASS_BOBAFETT) {
      ...
      } else { // player holds extra long so you have more time to decide to do
               // the quick getup
      ...
      //
      if (pusher->NPC && pusher->enemy == self) { // pushed pushed down his
                                                  // enemy
...
qboolean WP_ForceThrowable(gentity_t *ent, gentity_t *forwardEnt,
                           gentity_t *self, qboolean pull, float cone,
                           float radius, vec3_t forward) {
  if (ent == self)
    return qfalse;
  if (ent->owner == self &&
      ent->s.weapon != WP_THERMAL) // can push your own thermals
    return qfalse;
  if (!(ent->inuse))
    return qfalse;
  if (ent->NPC && ent->NPC->scriptFlags & SCF_NO_FORCE) {
    if (ent->s.weapon ==
        WP_SABER) { // Hmm, should jedi do the resist behavior?  If this is on,
                    // perhaps it's because of a cinematic?
      WP_ResistForcePush(ent, self, qtrue);
    }
    return qfalse;
  }
  if ((ent->flags & FL_FORCE_PULLABLE_ONLY) &&
      !pull) { // simple HACK: cannot force-push ammo rack items (because they
               // may start in solid)
    return qfalse;
  }
  ...
  if (ent->s.eType != ET_MISSILE) {
    ...
    if (cone >= 1.0f) {        // must be pointing right at them
      if (ent != forwardEnt) { // must be the person I'm looking right at
        if (ent->client && !pull &&
            ...
            (self->s.eFlags &
             EF_FORCE_GRIPPED)) { // this is the guy that's force-gripping me,
                                  // use a wider cone regardless of force power
                                  // level
        } else {
          if (ent->client && !pull &&
              ent->client->ps.forceDrainEntityNum == self->s.number &&
              (self->s.eFlags &
               EF_FORCE_DRAINED)) { // this is the guy that's force-draining me,
                                    // use a wider cone regardless of force power
                                    // level
          } else {
            return qfalse;
    ...
    if (ent->s.eType != ET_ITEM &&
        ent->e_ThinkFunc !=
            thinkF_G_RunObject) //|| !(ent->flags&FL_DROPPED_ITEM) )//was only
                                //dropped items
    {
      ...
      if (!ent->client) {
          ...
          if (!(ent->svFlags & SVF_GLASS_BRUSH)) {          // and not glass
            if (Q_stricmp("func_door", ent->classname) != 0 ||
              ...
              if (Q_stricmp("func_static", ent->classname) != 0 ||
                  ...
                  (ent->spawnflags &
                   32 /*SOLITARY*/)) { // not a force-usable func_static or, it
                                       // is one, but it's solitary, so you only
                                       // press it when looking right at it
                if (Q_stricmp("limb", ent->classname)) { // not a limb
                  if (ent->s.weapon == WP_TURRET &&
                    ...
                    if (!self->s.number ||
                        self->enemy != ent) { // only NPCs who are actively mad
                                              // at this turret can push it over
                      return qfalse;
                    }
                  } else {
                    return qfalse;
        ...
        // return qfalse;
                 ...
                 CLASS_MARK1) { // can't push Mark1 unless push 3
        if (pull || self->client->ps.forcePowerLevel[FP_PUSH] < FORCE_LEVEL_3) {
          return qfalse;
                 ...
                 ent->client->NPC_class ==
                     CLASS_SAND_CREATURE) { // can't push ATST or Galak or
                                            // Rancor or Wampa
        return qfalse;
      } else if (ent->s.weapon ==
                 WP_EMPLACED_GUN) { // FIXME: maybe can pull them out?
        return qfalse;
      } else if (ent->client->playerTeam == self->client->playerTeam &&
                 self->enemy &&
                 self->enemy !=
                     ent) { // can't accidently push a teammate while in combat
        return qfalse;
      } else if (G_IsRidingVehicle(ent) &&
                 (ent->s.eFlags & EF_NODRAW)) { // can't push/pull anyone riding
                                                // *inside* vehicle
        return qfalse;
      }
      ...
      { // dropped security keys can't be pushed?  But placed ones can...?  does
        // this make any sense?
        if (!pull ||
            self->s.number) { // can't push, NPC's can't do anything to it
          return qfalse;
        } else {
          if (g_crosshairEntNum !=
            ...
            } else if (forward) { // do a forwardEnt trace
              trace_t tr;
              vec3_t end;
              VectorMA(self->client->renderInfo.eyePoint, radius, forward, end);
              gi.trace(&tr, self->client->renderInfo.eyePoint, vec3_origin,
                       vec3_origin, end, self->s.number,
    ...
    switch (ent->s.weapon) { // only missiles with mass are force-pushable
    case WP_SABER:
    case WP_FLECHETTE:
    ...
    case WP_REPEATER:
      if (ent->methodOfDeath != MOD_REPEATER_ALT) { // not an alt-fire missile
        return qfalse;
      }
      break;
    // everything else cannot be pushed
    case WP_ATST_SIDE:
      if (ent->methodOfDeath != MOD_EXPLOSIVE) { // not a rocket
        return qfalse;
      }
      break;
    default:
    ...
    if (ent->s.pos.trType == TR_STATIONARY &&
        ent->s.weapon !=
            WP_THERMAL) { // only thermal detonators can be pushed once stopped
      return qfalse;
    }
  }
  return qtrue;
...
static qboolean ShouldPlayerResistForceThrow(gentity_t *player,
                                             gentity_t *attacker,
                                             qboolean pull) {
  if (player->health <= 0) {
    return qfalse;
  ...
  if ((attacker->client->NPC_class == CLASS_DESANN ||
       Q_stricmp("Yoda", attacker->NPC_type) == 0) &&
      Q_irand(0, 2) > 0) {
    return qfalse;
  }
  ...
  if (player->client->ps.powerups[PW_FORCE_PUSH] > level.time ||
      Q_irand(0, Q_max(0, defendingForceLevel - attackingForceLevel) * 2 + 1) >
          0) {
    // player was pushing, or player's force push/pull is high enough to try to
    // stop me
    if (InFront(attacker->currentOrigin, player->client->renderInfo.eyePoint,
                player->client->ps.viewangles, 0.3f)) {
...
void ForceThrow(gentity_t *self, qboolean pull,
                qboolean fake) { // FIXME: pass in a target ent so we (an NPC)
                                 // can push/pull just one targeted ent.
  ...
  if (self->client->ps.forcePowerDebounce[FP_PUSH] >
      level.time) { // already pushing- now you can't haul someone across the
                    // room, sorry
    return;
  }
  if (self->client->ps.forcePowerDebounce[FP_PULL] >
      level.time) { // already pulling- now you can't haul someone across the
                    // room, sorry
    return;
  }
  if (self->client->ps.pullAttackTime > level.time) { // already pull-attacking
    return;
  }
  if (!self->s.number &&
      (cg.zoomMode ||
       in_camera)) { // can't force throw/pull when zoomed in or in cinematic
    return;
  }
  if (self->client->ps.saberLockTime > level.time) {
  ...
  if (self->client->ps.legsAnim == BOTH_KNOCKDOWN3 ||
      (self->client->ps.torsoAnim == BOTH_FORCE_GETUP_F1 &&
       self->client->ps.torsoAnimTimer > 400) ||
      (self->client->ps.torsoAnim == BOTH_FORCE_GETUP_F2 &&
       self->client->ps.torsoAnimTimer > 900) ||
      (self->client->ps.torsoAnim == BOTH_GETUP3 &&
       self->client->ps.torsoAnimTimer > 500) ||
      (self->client->ps.torsoAnim == BOTH_GETUP4 &&
       self->client->ps.torsoAnimTimer > 300) ||
      (self->client->ps.torsoAnim == BOTH_GETUP5 &&
       self->client->ps.torsoAnimTimer >
           500)) { // we're face-down, so we'd only be force-push/pulling the
                   // floor
    return;
  }
  if (pull) {
  ...
  if (pull) {
    ...
    // make sure this plays and that you cannot press fire for about 200ms after
    // this
    anim = BOTH_FORCEPULL;
  ...
  self->client->ps.powerups[PW_FORCE_PUSH] =
  ...
  if ((!pull && self->client->ps.forcePowersForced & (1 << FP_PUSH)) ||
      ...
      (pull && self->client->NPC_class == CLASS_KYLE &&
       (self->spawnflags & 1) && TIMER_Done(self, "kyleTakesSaber"))) {
    noResist = qtrue;
  }
  ...
  { // must be pointing right at them
    ...
    gi.trace(&tr, self->client->renderInfo.eyePoint, vec3_origin, vec3_origin,
             ...
             0); // was MASK_SHOT, changed to match crosshair trace
    if (tr.entityNum <
        ENTITYNUM_WORLD) { // found something right in front of self,
      forwardEnt = &g_entities[tr.entityNum];
      if (!forwardEnt->client &&
          !Q_stricmp("func_static", forwardEnt->classname)) {
        if ((forwardEnt->spawnflags & 1 /*F_PUSH*/) ||
            (forwardEnt->spawnflags & 2 /*F_PULL*/)) { // push/pullable
          if ((forwardEnt->spawnflags &
               32 /*SOLITARY*/)) { // can only push/pull ME, ignore all others
            if (forwardEnt->NPC_targetname == NULL ||
                (self->targetname &&
                 Q_stricmp(forwardEnt->NPC_targetname, self->targetname) ==
                     0)) { // anyone can push it or only 1 person can push it
                           // and it's me
  ...
  if (forwardEnt) {
    if (G_TryingPullAttack(self, &self->client->usercmd,
                           qtrue)) { // we're going to try to do a pull attack
                                     // on our forwardEnt
      if (WP_ForceThrowable(forwardEnt, forwardEnt, self, pull, cone, radius,
                            forward)) { // we will actually pull-attack him, so
                                        // don't pull him or anything else here
        // activate the power, here, though, so the later check that actually
        // does the pull attack knows we tried to pull
        self->client->ps.forcePowersActive |= (1 << FP_PULL);
        self->client->ps.forcePowerDebounce[FP_PULL] =
            level.time + 100; // force-pulling
        return;
      }
    }
  }
  ...
  if (!numListedEntities) {
    ...
    for (e = 0; e < numListedEntities; e++) {
      ...
      if (!WP_ForceThrowable(ent, forwardEnt, self, pull, cone, radius,
                             forward)) {
        continue;
      }
        ...
        if (ent->client && !pull &&
          ...
          if ((dot1 = DotProduct(dir, forward)) < cone - 0.3f)
            continue;
                   ...
                   ent->client->ps.forceDrainEntityNum == self->s.number &&
                   (self->s.eFlags &
                    EF_FORCE_DRAINED)) { // this is the guy that's
                                         // force-draining me, use a wider cone
                                         // regardless of force power level
          if ((dot1 = DotProduct(dir, forward)) < cone - 0.3f)
            continue;
        } else if (ent->s.eType ==
                   ET_MISSILE) //&& ent->s.eType != ET_ITEM && ent->e_ThinkFunc
                               //!= thinkF_G_RunObject )
        { // missiles are easier to force-push, never require direct trace
          // (FIXME: maybe also items and general physics objects)
          if ((dot1 = DotProduct(dir, forward)) < cone - 0.3f)
            continue;
          ...
          continue;
                 ...
                 ET_MISSILE) { // a missile and we're at force level 1... just
                               // use a small cone, but not ridiculously small
        if ((dot1 = DotProduct(dir, forward)) < 0.75f) {
          continue;
        }
      } // else is an NPC or brush entity that our forward trace would have to
        // hit
      ...
      if (!ent->bmodel &&
          !gi.inPVS(ent_org,
                    self->client->renderInfo.eyePoint)) { // must be in PVS
        continue;
      }
      ...
      if (ent != forwardEnt) { // don't need to trace against forwardEnt again
        ...
        gi.trace(&tr, self->client->renderInfo.eyePoint, vec3_origin,
                 vec3_origin, ent_org, self->s.number, MASK_FORCE_PUSH,
                 (EG2_Collision)0, 0); // was MASK_SHOT, but changed to match
                                       // above trace and crosshair trace
        if (tr.fraction < 1.0f &&
  ...
  if (ent_count) {
    for (int x = 0; x < ent_count; x++) {
      if (push_list[x]->client) {
        ...
        if (push_list[x]->s.number >= MAX_CLIENTS &&
          ...
          if ((push_list[x]->client->ps.forcePowersActive & (1 << FP_GRIP))
              //&& push_list[x]->client->ps.forcePowerDebounce[FP_GRIP] <
              //level.time
              &&
          ...
          if ((push_list[x]->client->ps.forcePowersActive & (1 << FP_DRAIN))
              //&& push_list[x]->client->ps.forcePowerDebounce[FP_DRAIN] <
              //level.time
              &&
        ...
        if (push_list[x]->client->NPC_class == CLASS_HAZARD_TROOPER &&
            push_list[x]->health >
                0) { // living hazard troopers resist push/pull
        ...
        // FIXMEFIXMEFIXMEFIXMEFIXME: extern a lot of this common code when I
        // have the time!!!
        ...
        int modPowerLevel = WP_AbsorbConversion(
            push_list[x], push_list[x]->client->ps.forcePowerLevel[FP_ABSORB],
        ...
        // First, if this is the player we're push/pulling, see if he can
        // counter it
        if (modPowerLevel != -1 && !noResist &&
            InFront(self->currentOrigin,
                    push_list[x]->client->renderInfo.eyePoint,
                    push_list[x]->client->ps.viewangles,
                    0.3f)) { // absorbed and I'm in front of them
          // counter it
          if (push_list[x]->client->ps.forcePowerLevel[FP_ABSORB] >
              FORCE_LEVEL_2) { // no reaction at all
          } else {
            WP_ResistForcePush(push_list[x], self, qfalse);
            push_list[x]->client->ps.saberMove =
                push_list[x]->client->ps.saberBounceMove =
                    LS_READY; // don't finish whatever saber anim you may have
                              // been in
            push_list[x]->client->ps.saberBlocked = BLOCKED_NONE;
          }
          continue;
        } else if (!push_list[x]->s.number) { // player
          if (!noResist &&
              ShouldPlayerResistForceThrow(push_list[x], self, pull)) {
            WP_ResistForcePush(push_list[x], self, qfalse);
            push_list[x]->client->ps.saberMove =
                push_list[x]->client->ps.saberBounceMove =
                    LS_READY; // don't finish whatever saber anim you may have
        ...
        if (!pull &&
            push_list[x]->client->ps.forceDrainEntityNum == self->s.number &&
            (self->s.eFlags &
             EF_FORCE_DRAINED)) { // stop them from draining me now, dammit!
          WP_ForcePowerStop(push_list[x], FP_DRAIN);
        }
        ...
        if (((self->s.number == 0 && Q_irand(0, 2)) || Q_irand(0, 2)) &&
            ...
            && push_list[x]->client->ps.forceRageRecoveryTime <
                   level.time // not recobering from rage
            && ((self->client->NPC_class != CLASS_DESANN &&
                 Q_stricmp("Yoda", self->NPC_type)) ||
                !Q_irand(0, 2)) // only 30% chance of resisting a Desann push
            && push_list[x]->client->ps.groundEntityNum !=
                   ENTITYNUM_NONE // on the ground
            && InFront(self->currentOrigin, push_list[x]->currentOrigin,
                       push_list[x]->client->ps.viewangles,
                       0.3f) // I'm in front of him
            && (push_list[x]->client->ps.powerups[PW_FORCE_PUSH] >
                    level.time || // he's pushing too
                (push_list[x]->s.number != 0 &&
                 push_list[x]->client->ps.weaponTime <
                     level.time) // not the player and not attacking (NPC jedi
                                 // auto-defend against pushes)
                )) { // Jedi don't get pushed, they resist as long as they
                     // aren't already attacking and are on the ground
          if (push_list[x]->client->ps.saberLockTime >
              level.time) { // they're in a lock
            if (push_list[x]->client->ps.saberLockEnemy !=
                self->s.number) { // they're not in a lock with me
              continue;
            } else if (pull ||
                       self->client->ps.forcePowerLevel[FP_PUSH] <
                           FORCE_LEVEL_3 ||
                       push_list[x]->client->ps.forcePowerLevel[FP_PUSH] >
                           FORCE_LEVEL_2) { // they're in a lock with me, but my
                                            // push is too weak
              continue;
          ...
          if (push_list[x]->s.number >= MAX_CLIENTS) { // NPC
            if (g_spskill->integer == 1) { // stupid tweak for graham
              resistChance = Q_irand(0, 3);
            }
          }
          if (noResist ||
              (!pull && modPowerLevel == -1 &&
               self->client->ps.forcePowerLevel[FP_PUSH] > FORCE_LEVEL_2 &&
               !resistChance &&
               push_list[x]->client->ps.forcePowerLevel[FP_PUSH] <
                   FORCE_LEVEL_3)) { // a level 3 push can even knock down a
                                     // jedi
            if (PM_InKnockDown(
          ...
          // UGH: FIXME: for enemy jedi, they should probably always do force
          // pull 3, and not your weapon (if player?)! shove them
          if (push_list[x]->NPC &&
              push_list[x]->NPC->jumpState ==
                  JS_JUMPING) { // don't interrupt a scripted jump
            // WP_ResistForcePush( push_list[x], self, qfalse );
          ...
          if (pull) {
            ...
            if (self->client->ps.forcePowerLevel[FP_PULL] >= FORCE_LEVEL_3 &&
              ...
              push_list[x]->client->ps.pm_time =
                  push_list[x]->client->ps.weaponTime =
                      push_list[x]->client->ps.torsoAnimTimer;
              push_list[x]->client->ps.pm_flags |= PMF_TIME_KNOCKBACK;
              push_list[x]->client->ps.saberMove = LS_NONE;
              push_list[x]->aimDebounceTime =
                  level.time + push_list[x]->client->ps.torsoAnimTimer;
              ...
              TIMER_Set(
                  self, "kyleTakesSaber",
                  Q_irand(60000, 180000)); // don't do this again for a while
              G_AddVoiceEvent(self, Q_irand(EV_TAUNT1, EV_TAUNT3),
                              Q_irand(4000, 6000));
              VectorClear(self->client->ps.velocity);
              VectorClear(self->client->ps.moveDir);
              continue;
            } else if (push_list[x]->NPC &&
                       (push_list[x]->NPC->scriptFlags &
                        SCF_DONT_FLEE)) { //*SIGH*... if an NPC can't flee, they
                                          //can't run after and pick up their
                                          //weapon, do don't drop it
            } else if (self->client->ps.forcePowerLevel[FP_PULL] >
                           FORCE_LEVEL_1 &&
                       push_list[x]->client->NPC_class !=
                           CLASS_ROCKETTROOPER // rockettroopers never drop
                                               // their weapon
                       ...
                       push_list[x]->s.weapon !=
                           WP_CONCUSSION // so rax can't drop his
            ) { // yank the weapon - NOTE: level 1 just knocks them down, not
                // take weapon
            ...
            VectorSubtract(push_list[x]->currentOrigin, self->currentOrigin,
                           pushDir);
            knockback -= VectorNormalize(pushDir);
            if (knockback < 100) {
              knockback = 100;
            ...
            if (self->client->ps.forcePowerLevel[FP_PUSH] <
              ...
              knockback /= 3;
                       ...
                       FORCE_LEVEL_2) { // super-hard push
                                        // Hmm, maybe in this case can even
                                        // nudge/knockdown a jedi?  Especially if
                                        // close? knockback *= 5;
            }
          }
          ...
          if (push_list[x]->client->ps.groundEntityNum !=
              ENTITYNUM_NONE) { // if on the ground, make sure they get shoved
                                // up some
          ...
          if (push_list[x]->health > 0) { // target is still alive
            if ((push_list[x]->s.number ||
                 (cg.renderingThirdPerson &&
                  !cg.zoomMode)) // NPC or 3rd person player
                &&
                ((!pull &&
                  self->client->ps.forcePowerLevel[FP_PUSH] < FORCE_LEVEL_2 &&
                  push_list[x]->client->ps.forcePowerLevel[FP_PUSH] <
                 ...
                 ||
                 (pull &&
                  self->client->ps.forcePowerLevel[FP_PULL] < FORCE_LEVEL_2 &&
                  push_list[x]->client->ps.forcePowerLevel[FP_PULL] <
                      FORCE_LEVEL_1))) // level 1 pull
            { // NPC or third person player (without force push/pull skill), and
              // force push/pull level is at 1
              WP_ForceKnockdown(push_list[x], self, pull,
                                (qboolean)(!pull && knockback > 150), qfalse);
            } else if (!push_list[x]->s.number) { // player, have to force an
                                                  // anim on him
              WP_ForceKnockdown(push_list[x], self, pull,
                                (qboolean)(!pull && knockback > 150), qfalse);
      ...
      } else if (!fake) { // not a fake push/pull
        if (push_list[x]->s.weapon == WP_SABER &&
            (push_list[x]->contents &
          ...
          if (push_list[x]->owner && push_list[x]->owner->client &&
            ...
            // FIXME: prevent it from damaging me?
            if (self->s.number == 0 ||
                Q_irand(0, 2)) { // certain chance of throwing it aside and
                                 // turning it off?
            ...
            // different effect?
                   ...
                   (push_list[x]->s.pos.trType != TR_INTERPOLATE ||
                    push_list[x]->s.weapon !=
                        WP_THERMAL)) // rolling and stationary thermal
                                     // detonators are dealt with below
        {
          vec3_t dir2Me;
          VectorSubtract(self->currentOrigin, push_list[x]->currentOrigin,
                         dir2Me);
          ...
          if (pull) { // deflect rather than reflect?
            ...
            if (push_list[x]->s.eFlags &
                EF_MISSILE_STICK) { // caught a sticky in-air
            ...
            // deflect sound
            // G_Sound( push_list[x], G_SoundIndex(
            // va("sound/weapons/blaster/reflect%d.wav", Q_irand( 1, 3 ) ) ) );
            // push_list[x]->forcePushTime = level.time + 600; // let the push
            // effect last for 600 ms
          }
          if (push_list[x]->s.eType == ET_MISSILE &&
              push_list[x]->s.weapon == WP_ROCKET_LAUNCHER &&
              push_list[x]->damage < 60) { // pushing away a rocket raises it's
                                           // damage to the max for NPCs
          ...
          VectorMA(self->client->renderInfo.eyePoint, radius, forward, end);
          gi.trace(&tr, self->client->renderInfo.eyePoint, vec3_origin,
                   vec3_origin, end, self->s.number, MASK_SHOT,
                   (EG2_Collision)0, 0);
          ...
          if (pull) {
            VectorSubtract(self->client->renderInfo.eyePoint, tr.endpos,
                           pushDir);
          } else {
            VectorSubtract(tr.endpos, self->client->renderInfo.eyePoint,
                           pushDir);
          }
          /*
          ...
          if ( pull )
          {
                  VectorSubtract( self->client->renderInfo.eyePoint, center,
          pushDir );
          }
          else
          {
                  VectorSubtract( center, self->client->renderInfo.eyePoint,
          pushDir );
          }
          */
          ...
          G_Damage(push_list[x], self, self, pushDir, tr.endpos, damage, 0,
                   MOD_UNKNOWN);
        } else if (!Q_stricmp(
                       "func_static",
          ...
          if (!pull && (push_list[x]->spawnflags & 1 /*F_PUSH*/)) {
            if (push_list[x]->NPC_targetname == NULL ||
                (self->targetname &&
                 Q_stricmp(push_list[x]->NPC_targetname, self->targetname) ==
                     0)) { // anyone can pull it or only 1 person can push it
                           // and it's me
              GEntity_UseFunc(push_list[x], self, self);
            }
          } else if (pull && (push_list[x]->spawnflags & 2 /*F_PULL*/)) {
            if (push_list[x]->NPC_targetname == NULL ||
                (self->targetname && Q_stricmp(push_list[x]->NPC_targetname,
                                               self->NPC_targetname) ==
                                         0)) { // anyone can push it or only 1
                                               // person can push it and it's me
              GEntity_UseFunc(push_list[x], self, self);
            }
          }
        } else if (!Q_stricmp("func_door", push_list[x]->classname) &&
                   (push_list[x]->spawnflags &
                    2 /*MOVER_FORCE_ACTIVATE*/)) { // push/pull the door
          vec3_t pos1, pos2;
          ...
          if (VectorCompare(
                  vec3_origin,
                  push_list[x]->s.origin)) { // does not have an origin brush,
                                             // so pos1 & pos2 are relative to
                                             // world origin, need to calc center
            VectorSubtract(push_list[x]->absmax, push_list[x]->absmin, size);
            VectorMA(push_list[x]->absmin, 0.5, size, center);
            if ((push_list[x]->spawnflags & 1) &&
                push_list[x]->moverState ==
                    MOVER_POS1) { // if at pos1 and started open, make sure we
                                  // get the center where it *started* because
                                  // we're going to add back in the relative
              ...
              VectorSubtract(center, push_list[x]->pos1, center);
                       ...
                       push_list[x]->moverState ==
                           MOVER_POS2) { // if at pos2, make sure we get the
                                         // center where it *started* because
                                         // we're going to add back in the
                                         // relative values pos1 and pos2
          ...
          if (Distance(pos1, self->client->renderInfo.eyePoint) <
              Distance(pos2,
                       self->client->renderInfo.eyePoint)) { // pos1 is closer
            if (push_list[x]->moverState == MOVER_POS1) { // at the closest pos
              if (pull) { // trying to pull, but already at closest point, so
                          // screw it
                continue;
              }
            } else if (push_list[x]->moverState ==
                       MOVER_POS2) { // at farthest pos
          ...
          GEntity_UseFunc(push_list[x], self, self);
        } else if (push_list[x]->s.eType ==
                       ET_MISSILE /*thermal resting on ground*/
                   || push_list[x]->s.eType == ET_ITEM ||
                   push_list[x]->e_ThinkFunc == thinkF_G_RunObject ||
                   Q_stricmp("limb", push_list[x]->classname) ==
                       0) { // general object, toss it
          vec3_t pushDir, kvel;
          float knockback = pull ? 0 : 200;
          float mass = 200;

          if (pull) {
            if (push_list[x]->s.eType ==
                ET_ITEM) { // pull it to a little higher point
              vec3_t adjustedOrg;
              ...
              VectorSubtract(adjustedOrg, push_list[x]->currentOrigin, pushDir);
                                   ...
                                   //&& push_list[x]->s.eType != ET_ITEM //not
                                   //an item
                       && self->client->ps.forcePowerLevel[FP_PUSH] >
                              FORCE_LEVEL_2 // have push 3 or greater
                       && InFront(push_list[x]->currentOrigin,
                                  self->currentOrigin, self->currentAngles,
                                  0.25f) // object is generally in front of me
                       &&
                       InFront(
                           self->enemy->currentOrigin, self->currentOrigin,
                           self->currentAngles,
                           0.75f) // enemy is pretty much right in front of me
                       && !InFront(push_list[x]->currentOrigin,
                                   self->enemy->currentOrigin,
                                   self->enemy->currentAngles,
                                   -0.25f) // object is generally behind enemy
                       // FIXME: check dist to enemy and clear LOS to enemy and
                       // clear Path between object and enemy?
                       &&
                       ((self->NPC &&
                         (noResist ||
                          Q_irand(0, RANK_CAPTAIN) <
                              self->NPC->rank)) // NPC with enough skill
                        ||
                        (self->s.number <
                         MAX_CLIENTS))) { // if I have an auto-enemy & he's in
                                          // front of me, push it toward him!
              /*
              if ( targetedObjectMassTotal + push_list[x]->mass >
              TARGETED_OBJECT_PUSH_MASS_MAX )
              ...
              */
              VectorSubtract(self->enemy->currentOrigin,
                             push_list[x]->currentOrigin, pushDir);
            } else {
              VectorSubtract(self->currentOrigin, push_list[x]->currentOrigin,
                             pushDir);
            }
            ...
            if (push_list[x]->s.eType == ET_ITEM && push_list[x]->item &&
                push_list[x]->item->giType == IT_HOLDABLE &&
                push_list[x]->item->giTag ==
                    INV_SECURITY_KEY) { // security keys are pulled with less
                                        // enthusiasm
              if (knockback > 100) {
                knockback = 100;
              }
            ...
            if (self->enemy                         // I have an enemy
                ...
                && self->client->ps.forcePowerLevel[FP_PUSH] >
                       FORCE_LEVEL_2 // have push 3 or greater
                && InFront(push_list[x]->currentOrigin, self->currentOrigin,
                           self->currentAngles,
                           0.25f) // object is generally in front of me
                && InFront(self->enemy->currentOrigin, self->currentOrigin,
                           self->currentAngles,
                           0.75f) // enemy is pretty much right in front of me
                &&
                InFront(push_list[x]->currentOrigin, self->enemy->currentOrigin,
                        self->enemy->currentAngles,
                        0.25f) // object is generally in front of enemy
                // FIXME: check dist to enemy and clear LOS to enemy and clear
                // Path between object and enemy?
                && ((self->NPC &&
                     (noResist || Q_irand(0, RANK_CAPTAIN) <
                                      self->NPC->rank)) // NPC with enough skill
                    || (self->s.number <
                        MAX_CLIENTS))) { // if I have an auto-enemy & he's in
                                         // front of me, push it toward him!
          ...
          // FIXME: if pull a FL_FORCE_PULLABLE_ONLY, clear the flag, assuming
          // it's no longer in solid?  or check?
          VectorCopy(push_list[x]->currentOrigin, push_list[x]->s.pos.trBase);
          push_list[x]->s.pos.trTime =
              level.time; // move a bit on the very first frame
          ...
          if (push_list[x]->e_ThinkFunc == thinkF_G_RunObject &&
              push_list[x]
                  ->physicsBounce) { // it's a pushable misc_model_breakable,
                                     // use it's mass instead of our
                                     // one-size-fits-all mass
            mass = push_list[x]
                       ->physicsBounce; // same as push_list[x]->mass, right?
          }
          if (mass < 50) { //???
            mass = 50;
          }
          if (g_gravity->value > 0) {
            VectorScale(pushDir, g_knockback->value * knockback / mass * 0.8,
                        kvel);
            kvel[2] = pushDir[2] * g_knockback->value * knockback / mass * 1.5;
          } else {
            VectorScale(pushDir, g_knockback->value * knockback / mass, kvel);
          }
          VectorAdd(push_list[x]->s.pos.trDelta, kvel,
                    push_list[x]->s.pos.trDelta);
          if (g_gravity->value > 0) {
          ...
          // no trDuration?
          if (push_list[x]->e_ThinkFunc !=
              thinkF_G_RunObject) { // objects spin themselves?
            // spin it
          ...
          push_list[x]->forcePushTime =
              level.time + 600; // let the push effect last for 600 ms
          push_list[x]->forcePuller = self->s.number; // remember this
                                                      // regardless
          if (push_list[x]->item &&
            ...
            AddSightEvent(
                player, push_list[x]->currentOrigin, 128,
                AEL_SUSPICIOUS); // hmm... or should this always be discovered?
                   ...
                   push_list[x]->s.apos.trType ==
                       TR_STATIONARY) { // a portable turret
      ...
      if (self->client->ps.forcePowerLevel[FP_PUSH] >
        ...
        if (actualCost > 50) {
          ...
          actualCost = cost;
                 ...
                 FORCE_LEVEL_1) { // at level 2, can push multiple, so costs
                                  // more
...
void WP_DebounceForceDeactivateTime(gentity_t *self) {
  // FIXME: if these are interruptable, should they also drain power at a
  // constant rate 		rather than just taking one lump sum of force power upfront?
  if (self && self->client) {
    if (self->client->ps.forcePowersActive & (1 << FP_SPEED) ||
        ...
        self->client->ps.forcePowersActive &
            (1 << FP_SEE)) { // already running another power that can be
                             // manually, stopped don't debounce so long
      self->client->ps.forceAllowDeactivateTime = level.time + 500;
    } else { // not running one of the interruptable powers
      // FIXME: this should be shorter for force speed and rage (because of
      // timescaling)
...
void ForceSpeed(gentity_t *self, int duration) {
  ...
  if (self->client->ps.forceAllowDeactivateTime < level.time &&
      (self->client->ps.forcePowersActive & (1 << FP_SPEED))) { // stop using it
  ...
  if (self->client->ps.saberLockTime >
      level.time) { // FIXME: can this be a way to break out?
    return;
  }
...
void WP_StartForceHealEffects(gentity_t *self) {
  if (self->ghoul2.size()) {
    if (self->chestBolt != -1) {
      G_PlayEffect(G_EffectIndex("force/heal2"), self->playerModel,
                   self->chestBolt, self->s.number, self->currentOrigin, 3000,
                   qtrue);
    }
    /*
    if ( self->headBolt != -1 )
    {
            G_PlayEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->headBolt, self->s.number, self->currentOrigin,
    3000, qtrue );
    }
    if ( self->cervicalBolt != -1 )
    {
            G_PlayEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->cervicalBolt, self->s.number, self->currentOrigin,
    3000, qtrue );
    }
    if ( self->chestBolt != -1 )
    {
            G_PlayEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->chestBolt, self->s.number, self->currentOrigin,
    3000, qtrue );
    }
    if ( self->gutBolt != -1 )
    {
            G_PlayEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->gutBolt, self->s.number, self->currentOrigin, 3000,
    qtrue );
    }
    if ( self->kneeLBolt != -1 )
    {
            G_PlayEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->kneeLBolt, self->s.number, self->currentOrigin,
    3000, qtrue );
    }
    if ( self->kneeRBolt != -1 )
    {
            G_PlayEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->kneeRBolt, self->s.number, self->currentOrigin,
    3000, qtrue );
    }
    if ( self->elbowLBolt != -1 )
    {
            G_PlayEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->elbowLBolt, self->s.number, self->currentOrigin,
    3000, qtrue );
    }
    if ( self->elbowRBolt != -1 )
    {
            G_PlayEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->elbowRBolt, self->s.number, self->currentOrigin,
    3000, qtrue );
    }
    */
  }
...
void WP_StopForceHealEffects(gentity_t *self) {
  if (self->ghoul2.size()) {
    if (self->chestBolt != -1) {
      G_StopEffect(G_EffectIndex("force/heal2"), self->playerModel,
                   self->chestBolt, self->s.number);
    }
    /*
    if ( self->headBolt != -1 )
    {
            G_StopEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->headBolt, self->s.number );
    }
    if ( self->cervicalBolt != -1 )
    {
            G_StopEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->cervicalBolt, self->s.number );
    }
    if ( self->chestBolt != -1 )
    {
            G_StopEffect( G_EffectIndex( "force/heal_joint" ),
    ...
    {
            G_StopEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->gutBolt, self->s.number );
    }
    if ( self->kneeLBolt != -1 )
    {
            G_StopEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->kneeLBolt, self->s.number );
    }
    if ( self->kneeRBolt != -1 )
    {
            G_StopEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->kneeRBolt, self->s.number );
    }
    if ( self->elbowLBolt != -1 )
    {
            G_StopEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->elbowLBolt, self->s.number );
    }
    if ( self->elbowRBolt != -1 )
    {
            G_StopEffect( G_EffectIndex( "force/heal_joint" ),
    self->playerModel, self->elbowRBolt, self->s.number );
    }
    */
  }
}
...
int FP_ForceHealInterval(gentity_t *self) {
  return (self->client->ps.forcePowerLevel[FP_HEAL] > FORCE_LEVEL_2)
             ? 50
             : FORCE_HEAL_INTERVAL;
}
...
void ForceHeal(gentity_t *self) {
  ...
  {//instant heal
          ...
          G_SoundOnEnt( self, CHAN_VOICE, va( "sound/weapons/force/heal%d.mp3",
  Q_irand( 1, 4 ) ) );
  }
  else
  */
  {
    ...
    if (self->client->ps.forcePowerLevel[FP_HEAL] <
      ...
      self->client->ps.torsoAnimTimer = self->client->ps.legsAnimTimer =
          FP_ForceHealInterval(self) * FP_MaxForceHeal(self) + 2000; //???
      WP_DeactivateSaber(self); // turn off saber when meditating
    } else {                    // just a quick gesture
      /*
      //Can't get an anim that looks good...
      NPC_SetAnim( self, SETANIM_TORSO, BOTH_FORCEHEAL_QUICK,
      SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD ); self->client->ps.saberMove =
      self->client->ps.saberBounceMove = LS_READY;//don't finish whatever saber
      anim you may have been in self->client->ps.saberBlocked = BLOCKED_NONE;
      */
    }
  }
...
qboolean WP_CheckBreakControl(gentity_t *self) {
  ...
  if (!self->s.number) { // player
    if (self->client && self->client->ps.forcePowerLevel[FP_TELEPATHY] >
                            FORCE_LEVEL_3) { // control-level
      if (self->client->ps.viewEntity > 0 &&
          self->client->ps.viewEntity <
              ENTITYNUM_WORLD) { // we are in a viewentity
        gentity_t *controlled = &g_entities[self->client->ps.viewEntity];
        if (controlled->NPC && controlled->NPC->controlledTime >
                                   level.time) { // it is an NPC we controlled
          // clear it and return
...
void ForceTelepathy(gentity_t *self) {
  ...
  gi.trace(&tr, self->client->renderInfo.eyePoint, vec3_origin, vec3_origin,
           end, self->s.number, MASK_OPAQUE | CONTENTS_BODY, (EG2_Collision)0,
           0);
  ...
  if (targetLive && traceEnt->NPC &&
      traceEnt->health > 0) { // hit an organic non-player
        ...
        if (self->client->ps.forcePowerLevel[FP_TELEPATHY] > FORCE_LEVEL_2 &&
            traceEnt->s.weapon !=
                WP_NONE // don't charm people who aren't capable of fighting...
                        // like ugnaughts and droids, just confuse them
            && traceEnt->client->NPC_class !=
                   CLASS_TUSKEN // don't charm them, just confuse them
            && traceEnt->client->NPC_class !=
                   CLASS_NOGHRI // don't charm them, just confuse them
            && !Pilot_AnyVehiclesRegistered() // also, don't charm guys when
                                              // bikes are near
          ...
          // assume that "confusion" means they should switch to team_enemy when
          // done?
          ...
          if (traceEnt->ghoul2.size() &&
              traceEnt->headBolt !=
                  -1) { // FIXME: what if already playing effect?
            G_PlayEffect(
                G_EffectIndex("force/confusion"), traceEnt->playerModel,
                traceEnt->headBolt, traceEnt->s.number, traceEnt->currentOrigin,
                mindTrickTime[self->client->ps.forcePowerLevel[FP_TELEPATHY]],
                qtrue);
          }
    ...
    G_PlayEffect("force/force_touch", traceEnt->client->renderInfo.eyePoint,
                 eyeDir);
    ...
    if (self->client->ps.forcePowerLevel[FP_TELEPATHY] > FORCE_LEVEL_1 &&
        tr.fraction * 2048 > 64) { // don't create a diversion less than 64 from
                                   // you of if at power level 1
      // use distraction anim instead
      G_PlayEffect(G_EffectIndex("force/force_touch"), tr.endpos,
                   tr.plane.normal);
...
void ForceGrip(gentity_t *self) { // FIXME: make enemy Jedi able to use this
  ...
  if (!self->s.number &&
      (cg.zoomMode ||
       in_camera)) { // can't force grip when zoomed in or in cinematic
    return;
  }
  ...
  if (self->client->ps.forceGripEntityNum <=
      ENTITYNUM_WORLD) { // already gripping
  ...
  if (self->enemy) { // I have an enemy
    if (!self->enemy->message &&
        !(self->flags & FL_NO_KNOCKBACK)) { // don't auto-pickup guys with keys
      if (DistanceSquared(self->enemy->currentOrigin, self->currentOrigin) <
          FORCE_GRIP_DIST_SQUARED) { // close enough to grab
        float minDot = 0.5f;
        if (self->s.number <
            MAX_CLIENTS) { // player needs to be facing more directly
          minDot = 0.2f;
        }
        if (InFront(self->enemy->currentOrigin,
                    self->client->renderInfo.eyePoint,
                    self->client->ps.viewangles,
                    minDot)) // self->s.number || //NPCs can always lift enemy
                             // since we assume they're looking at them...?
        { // need to be facing the enemy
          if (gi.inPVS(self->enemy->currentOrigin,
                       self->client->renderInfo.eyePoint)) { // must be in PVS
            gi.trace(&tr, self->client->renderInfo.eyePoint, vec3_origin,
                     vec3_origin, self->enemy->currentOrigin, self->s.number,
                     MASK_SHOT, (EG2_Collision)0, 0);
            if (tr.fraction == 1.0f ||
                tr.entityNum == self->enemy->s.number) { // must have clear LOS
              traceEnt = self->enemy;
            }
          }
        }
  ...
  if (!traceEnt) { // okay, trace straight ahead and see what's there
    ...
    if (tr.entityNum >= ENTITYNUM_WORLD || tr.fraction == 1.0 || tr.allsolid ||
        tr.startsolid) {
      return;
    }
...
// rww - RAGDOLL_BEGIN
  ...
  if (traceEnt->m_pVehicle != NULL) { // is it a vehicle
    // grab pilot if there is one
    if (traceEnt->m_pVehicle->m_pPilot != NULL &&
        traceEnt->m_pVehicle->m_pPilot->client != NULL) { // grip the pilot
      traceEnt = traceEnt->m_pVehicle->m_pPilot;
    } else { // can't grip a vehicle
      return;
    }
  }
  if (traceEnt->client) {
    if (traceEnt->client->ps
            .forceJumpZStart) { // can't catch them in mid force jump - FIXME:
                                // maybe base it on velocity?
      return;
    }
    if (traceEnt->client->ps.pullAttackTime >
        level.time) { // can't grip someone who is being pull-attacked or is
                      // pull-attacking
      return;
    }
    ...
    if (G_IsRidingVehicle(traceEnt) &&
        (traceEnt->s.eFlags & EF_NODRAW)) { // riding *inside* vehicle
      return;
    }

    switch (traceEnt->client->NPC_class) {
    case CLASS_GALAKMECH: // cant grip him, he's in armor
      G_AddVoiceEvent(traceEnt, Q_irand(EV_PUSHED1, EV_PUSHED3),
                      Q_irand(3000, 5000));
      return;
      break;
    ...
    case CLASS_INTERROGATOR:
      ...
      return;
      break;
    case CLASS_DESANN: // Desann cannot be gripped, he just pushes you back
                       // instantly
    case CLASS_KYLE:
    case CLASS_TAVION:
    case CLASS_LUKE:
    ...
    if (self->enemy && traceEnt != self->enemy &&
        traceEnt->client->playerTeam ==
            self->client
                ->playerTeam) { // can't accidently grip your teammate in combat
      return;
    }
    //=CHECKABSORB===
    if (-1 !=
        WP_AbsorbConversion(
            traceEnt, traceEnt->client->ps.forcePowerLevel[FP_ABSORB], self,
    ...
    // if ( Q_stricmp( "misc_model_breakable", traceEnt->classname ) ||
    // !(traceEnt->s.eFlags&EF_BOUNCE_HALF) || !traceEnt->physicsBounce )
    { return; }
  }
  ...
  if (traceEnt->client) {
    Vehicle_t *pVeh;
    if ((pVeh = G_IsRidingVehicle(traceEnt)) !=
        NULL) { // riding vehicle? pull him off!
    ...
    if (self->client->ps.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_2 ||
        traceEnt->s.weapon ==
            WP_SABER) { // if we pick up & carry, drop their weap
      if (traceEnt->s.weapon &&
        ...
        if (traceEnt->client->NPC_class ==
            CLASS_BOBAFETT) { // he doesn't drop them, just puts it away
          ChangeWeapon(traceEnt, WP_MELEE);
        } else if (traceEnt->s.weapon ==
                   WP_MELEE) { // they can't take that away from me, oh no...
        } else if (traceEnt->NPC &&
                   (traceEnt->NPC->scriptFlags &
                    SCF_DONT_FLEE)) { //*SIGH*... if an NPC can't flee, they
                                      //can't run after and pick up their
                                      //weapon, do don't drop it
        } else if (traceEnt->s.weapon != WP_SABER) {
    ...
    // else FIXME: need a one-armed choke if we're not on a high enough level to
    // make them drop their gun
    VectorCopy(traceEnt->client->renderInfo.headPoint,
               self->client->ps.forceGripOrg);
  } else {
  ...
  if (self->client->ps.forcePowerLevel[FP_GRIP] <
      FORCE_LEVEL_2) { // just a duration
    self->client->ps.forcePowerDebounce[FP_GRIP] = level.time + 250;
    self->client->ps.forcePowerDuration[FP_GRIP] = level.time + 5000;

    if (self->m_pVehicle &&
        self->m_pVehicle->m_pVehicleInfo->Inhabited(
            self->m_pVehicle)) { // empty vehicles don't make gripped noise
...
void ForceLightningAnim(gentity_t *self) {
  ...
  if (self->client->ps.forcePowerLevel[FP_LIGHTNING] >= FORCE_LEVEL_3 &&
      ForceLightningCheck2Handed(self)) { // empty handed lightning 3
...
void ForceLightning(gentity_t *self) {
  ...
  if (!self->s.number &&
      (cg.zoomMode ||
       in_camera)) { // can't force lightning when zoomed in or in cinematic
    return;
  }
  ...
  if (self->client->ps.forcePowerLevel[FP_LIGHTNING] < FORCE_LEVEL_2) {
    ...
    {//empty handed lightning 3
            NPC_SetAnim( self, SETANIM_TORSO, BOTH_FORCE_2HANDEDLIGHTNING_START,
    SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
    }
    else
  ...
  if (self->client->ps.forcePowerLevel[FP_LIGHTNING] <
      FORCE_LEVEL_2) { // short burst
                       // G_SoundOnEnt( self, CHAN_BODY,
                       // "sound/weapons/force/lightning.wav" );
  } else { // holding it
...
void ForceLightningDamage(gentity_t *self, gentity_t *traceEnt, vec3_t dir,
                          float dist, float dot, vec3_t impactPoint) {
  if (traceEnt->NPC && traceEnt->NPC->scriptFlags & SCF_NO_FORCE) {
    return;
  }
  ...
  if (traceEnt && traceEnt->takedamage) {
    if (!traceEnt->client ||
        ...
        self->enemy == traceEnt ||
        traceEnt->enemy == self) { // an enemy or object
      int dmg;
      ...
      if (self->client->ps.forcePowerLevel[FP_LIGHTNING] >
        ...
        if (self->client->NPC_class == CLASS_REBORN &&
            self->client->ps.weapon ==
                WP_NONE) { // Cultist: looks fancy, but does less damage
        } else {
          if (dist < 100) {
            dmg += 2;
          } else if (dist < 200) {
        ...
        if (self->client->ps.torsoAnim == BOTH_FORCE_2HANDEDLIGHTNING ||
            ...
            self->client->ps.torsoAnim ==
                BOTH_FORCE_2HANDEDLIGHTNING_RELEASE) { // jackin' 'em up,
                                                       // Palpatine-style
          dmg *= 2;
        }
      } else {
      ...
      if (traceEnt->client && traceEnt->health > 0 && traceEnt->NPC &&
          (traceEnt->NPC->aiFlags &
           NPCAI_BOSS_CHARACTER)) { // Luke, Desann Tavion and Kyle can shield
        ...
        if (traceEnt->client->ps.groundEntityNum != ENTITYNUM_NONE &&
            ...
            !PM_RollingAnim(
                traceEnt->client->ps
                    .legsAnim)) { // if on a surface and not in a spin or flip,
                                  // play full body resist
          parts = SETANIM_BOTH;
        } else { // play resist just in torso
          parts = SETANIM_TORSO;
        }
        ...
        Jedi_PlayDeflectSound(traceEnt);
        dmg = Q_irand(0, 1);
      } else if (traceEnt->s.weapon == WP_SABER) { // saber can block lightning
        if (traceEnt->client                       // a client
            && !traceEnt->client->ps.saberInFlight // saber in hand
            && (traceEnt->client->ps.saberMove == LS_READY ||
                PM_SaberInParry(traceEnt->client->ps.saberMove) ||
                PM_SaberInReturn(
                    traceEnt->client->ps.saberMove)) // not attacking with saber
            && InFOV(self->currentOrigin, traceEnt->currentOrigin,
                     traceEnt->client->ps.viewangles, 20,
                     35) // I'm in front of them
            &&
            ...
            (!traceEnt->s.number ||
             (traceEnt->NPC &&
              traceEnt->NPC->rank >=
                  RANK_LT_COMM))) // the player or a tough jedi/reborn
        {
          if (Q_irand(0,
                      traceEnt->client->ps.forcePowerLevel[FP_SABER_DEFENSE] *
                          3) >
              0) // more of a chance of defending if saber defense is high
          {
            dmg = 0;
          }
          if ((traceEnt->client->ps.forcePowersActive & (1 << FP_ABSORB)) &&
              traceEnt->client->ps.forcePowerLevel[FP_ABSORB] >
                  FORCE_LEVEL_2) { // no parry, just absorb
          } else {
            ...
            if (traceEnt->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] <
                level.time + parryReCalcTime) {
              traceEnt->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] =
                  level.time + parryReCalcTime;
            }
            traceEnt->client->ps.weaponTime =
                Q_irand(100, 300); // hold this move - can't attack! - FIXME:
                                   // unless dual sabers?
          }
        } else if (Q_irand(0, 1)) { // jedi less likely to be damaged
          dmg = 0;
        } else {
          dmg = 1;
        }
      ...
      // FIXME: if ForceDrain, sap force power and add health to self, use
      ...
      if (traceEnt->client) {
        if (!Q_irand(0, 2)) {
          G_Sound(traceEnt,
                  G_SoundIndex(va("sound/weapons/force/lightninghit%d.wav",
                                  Q_irand(1, 3))));
        }
...
void ForceShootLightning(gentity_t *self) {
  ...
  // FIXME: if lightning hits water, do water-only-flagged radius damage from
  // that point
  if (self->client->ps.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_2) { // arc
    ...
    for (e = 0; e < numListedEntities; e++) {
      ...
      */
      ...
      if (!traceEnt->bmodel &&
          !gi.inPVS(ent_org,
                    self->client->renderInfo.handLPoint)) { // must be in PVS
        continue;
      }

      // Now check and see if we can actually hit it
      gi.trace(&tr, self->client->renderInfo.handLPoint, vec3_origin,
               vec3_origin, ent_org, self->s.number, MASK_SHOT,
               (EG2_Collision)0, 0);
      if (tr.fraction < 1.0f &&
    ...
    while (traces <
      ...
      gi.trace(&tr, start, vec3_origin, vec3_origin, end, ignore, MASK_SHOT,
               G2_RETURNONHIT, 10);
      if (tr.entityNum == ENTITYNUM_NONE || tr.fraction == 1.0 || tr.allsolid ||
          tr.startsolid) {
        return;
      ...
      if (traceEnt && traceEnt->s.number >= MAX_CLIENTS && traceEnt->client &&
          traceEnt->client->ps.forcePowerLevel[FP_LEVITATION] >
              FORCE_LEVEL_0) //&& traceEnt->NPC
...
void ForceDrainGrabStart(gentity_t *self) {
  NPC_SetAnim(self, SETANIM_BOTH, BOTH_FORCE_DRAIN_GRAB_START,
              SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  self->client->ps.saberMove = self->client->ps.saberBounceMove =
      LS_READY; // don't finish whatever saber anim you may have been in
  self->client->ps.saberBlocked = BLOCKED_NONE;
...
qboolean
ForceDrain2(gentity_t *self) { // FIXME: make enemy Jedi able to use this
  trace_t tr;
  vec3_t end, forward;
  gentity_t *traceEnt = NULL;
  ...
  if (self->client->ps.leanofs) { // can't force-drain while leaning
    return qtrue;
  }
  ...
  */
  ...
  if (self->client->ps.forceDrainEntityNum <=
      ENTITYNUM_WORLD) { // already draining
    // keep my saber off!
    WP_DeactivateSaber(self, qtrue);
  ...
  if (self->client->ps.forcePowerDebounce[FP_DRAIN] >
      level.time) { // stops it while using it and also after using it, up to 3
                    // second delay
    return qtrue;
  }

  if (self->client->ps.weaponTime > 0) { // busy
    return qtrue;
  }

  if (self->client->ps.forcePower < 25 ||
      !WP_ForcePowerUsable(self, FP_DRAIN, 0)) {
    return qtrue;
  }

  if (self->client->ps.saberLockTime > level.time) { // in saberlock
    return qtrue;
  }

  // NOTE: from here on, if it fails, it's okay to try a normal drain, so return
  // qfalse
  if (self->client->ps.groundEntityNum == ENTITYNUM_NONE) { // in air
    return qfalse;
  }
  ...
  if (traceEnt->client) {
    ...
    if (traceEnt->client->ps.groundEntityNum ==
        ENTITYNUM_NONE) { // can't catch them in mid air
      return qfalse;
    }
    if (!Q_stricmp("Yoda", traceEnt->NPC_type)) {
      Jedi_PlayDeflectSound(traceEnt);
    ...
    if (traceEnt != self->enemy &&
        OnSameTeam(self,
                   traceEnt)) { // can't accidently grip-drain your teammate
      return qfalse;
    }
    //=CHECKABSORB===
    /*
    if ( -1 != WP_AbsorbConversion( traceEnt,
    traceEnt->client->ps.forcePowerLevel[FP_ABSORB], self, FP_DRAIN,
  ...
  if (/*self->client->ps.forcePowerLevel[FP_DRAIN] > FORCE_LEVEL_2 ||*/ traceEnt
          ->s.weapon == WP_SABER) { // if we pick up, turn off their weapon
  ...
  */
  ...
  //	NPC_SetAnim( traceEnt, SETANIM_BOTH, BOTH_HUGGEE1,
  //SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
  NPC_SetAnim(traceEnt, SETANIM_BOTH, BOTH_FORCE_DRAIN_GRABBED,
              SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void ForceDrainDamage(gentity_t *self, gentity_t *traceEnt, vec3_t dir,
                      vec3_t impactPoint) {
  if (traceEnt && traceEnt->health > 0 && traceEnt->takedamage &&
      FP_ForceDrainableEnt(traceEnt)) {
    if (traceEnt->client &&
        (!OnSameTeam(self, traceEnt) ||
         self->enemy == traceEnt) // don't drain an ally unless that is actually
                                  // my current enemy
        && self->client->ps.forceDrainTime < level.time) { // an enemy or object
      int modPowerLevel = -1;
      int dmg = self->client->ps.forcePowerLevel[FP_DRAIN] + 1;
      int dflags = (DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK |
                    DAMAGE_NO_HIT_LOC); //|DAMAGE_NO_KILL);
      if (traceEnt->s.number ==
          self->client->ps.forceDrainEntityNum) { // grabbing hold of them does
                                                  // more damage/drains more, and
      ...
      if (traceEnt->client) {
        ...
        // Since this is drain, don't absorb any power, but nullify the affect
        // it has
      }
      ...
      if (dmg) {
        ...
        */
        ...
        if (self->client->ps.forcePowerLevel[FP_DRAIN] >
            FORCE_LEVEL_2) { // overcharge health
          maxHealth =
              floor((float)self->client->ps.stats[STAT_MAX_HEALTH] * 1.25f);
        }
        ...
        if (dmg) { // do damage, too
          G_Damage(traceEnt, self, self, dir, impactPoint, dmg, dflags,
                   MOD_FORCE_DRAIN);
        } else if (drain) {
          /*
          ...
          */
          ...
          {
                  GEntity_PainFunc( traceEnt, self, self, impactPoint, 0,
          MOD_FORCE_DRAIN );
          }
          */
        }
        ...
        traceEnt->client->ps.forcePowerRegenDebounceTime =
...
qboolean WP_CheckForceDraineeStopMe(gentity_t *self, gentity_t *drainee) {
  if (drainee->NPC && drainee->client &&
      (drainee->client->ps.forcePowersKnown & (1 << FP_PUSH)) &&
      level.time - (self->client->ps.forcePowerDebounce[FP_DRAIN] >
                    self->client->ps.forcePowerLevel[FP_DRAIN] *
                        500) // at level 1, I always get at least 500ms of
                             // drain, at level 3 I get 1500ms
      && !Q_irand(0, 100 - (drainee->NPC->stats.evasion * 10) -
                         (g_spskill->integer * 12))) { // a jedi who broke free
...
void ForceShootDrain(gentity_t *self) {
  ...
  if (self->client->ps.forcePowerDebounce[FP_DRAIN] <= level.time) {
    ...
    if (self->client->ps.forcePowerLevel[FP_DRAIN] > FORCE_LEVEL_2) { // arc
      vec3_t center, mins, maxs, dir, ent_org, size, v;
      ...
      for (e = 0; e < numListedEntities; e++) {
        ...
        if ( !traceEnt->client->ps.forcePower )
                continue;
        */
        //			if (traceEnt->client->ps.forceSide ==
        //FORCE_DARKSIDE)	// We no longer care if the victim is dark or
        //light 				continue;
        ...
        {
          if (!Q_irand(0, 4) &&
              !Jedi_DodgeEvasion(traceEnt, self, &tr,
                                 HL_NONE)) { // act like we didn't even hit him
            continue;
          }
        }
      ...
      while (
        ...
        gi.trace(&tr, start, vec3_origin, vec3_origin, end, ignore, MASK_SHOT,
                 G2_RETURNONHIT, 10);
        if (tr.entityNum == ENTITYNUM_NONE || tr.fraction == 1.0 ||
    ...
    self->client->ps.forcePowerDebounce[FP_DRAIN] =
        level.time + 200; // so we don't drain so damn fast!
  }
  self->client->ps.forcePowerRegenDebounceTime = level.time + 500;

  if (!numDrained) { // always take 1 force point per frame that we're shooting
                     // this
    WP_ForcePowerDrain(self, FP_DRAIN, 1);
  } else {
  ...
  if (self->client->ps.forcePowerDebounce[FP_DRAIN] <= level.time) {
    ...
    if (drainEnt->s.number) {
      if (self->client->ps.forcePowerLevel[FP_DRAIN] >
        ...
        self->client->ps.forcePowerDebounce[FP_DRAIN] =
            level.time + Q_irand(100, 500);
      } else {
        self->client->ps.forcePowerDebounce[FP_DRAIN] =
            level.time + Q_irand(200, 800);
      }
...
void ForceProtect(gentity_t *self) {
  ...
  if (self->client->ps.saberLockTime < level.time) {
    if (self->client->ps.forcePowerLevel[FP_PROTECT] <
        FORCE_LEVEL_3) { // animate
      int parts = SETANIM_BOTH;
      int anim = BOTH_FORCE_PROTECT;
      if (self->client->ps.forcePowerLevel[FP_PROTECT] >
          FORCE_LEVEL_1) { // level 2 only does it on torso (can keep running)
        parts = SETANIM_TORSO;
...
void ForceAbsorb(gentity_t *self) {
  ...
  if (self->client->ps.saberLockTime < level.time) {
    if (self->client->ps.forcePowerLevel[FP_ABSORB] <
        FORCE_LEVEL_3) { // must animate
      int parts = SETANIM_BOTH;
      if (self->client->ps.forcePowerLevel[FP_ABSORB] >
          FORCE_LEVEL_1) { // level 2 only does it on torso (can keep running)
        parts = SETANIM_TORSO;
      ...
      */
      NPC_SetAnim(self, parts, BOTH_FORCE_ABSORB,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      self->client->ps.weaponTime = self->client->ps.torsoAnimTimer;
      if (parts == SETANIM_BOTH) { // can't move
        self->client->ps.pm_flags |= PMF_TIME_KNOCKBACK;
        self->client->ps.pm_time = self->client->ps.legsAnimTimer =
            self->client->ps
                .torsoAnimTimer; // =
                                 // self->client->ps.forcePowerDuration[FP_ABSORB];
        if (self->s.number) { // NPC
          self->painDebounceTime =
              level.time +
              self->client->ps
                  .pm_time; // self->client->ps.forcePowerDuration[FP_ABSORB];
        } else {            // player
...
void ForceRage(gentity_t *self) {
  ...
  if (self->s.number < MAX_CLIENTS &&
      self->health < 25) { // have to have at least 25 health to start it
    return;
  }
  ...
  if (self->client->ps.saberLockTime < level.time) {
    if (self->client->ps.forcePowerLevel[FP_RAGE] <
        FORCE_LEVEL_3) { // must animate
      if (self->client->ps.forcePowerLevel[FP_RAGE] <
          FORCE_LEVEL_2) { // have to stand still for whole length of anim
        // FIXME: if in air, only do on torso?
        NPC_SetAnim(self, SETANIM_BOTH, BOTH_FORCE_RAGE,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        ...
        NPC_SetAnim(self, SETANIM_TORSO, BOTH_FORCE_RAGE,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        // don't attack during this anim
        self->client->ps.weaponTime = self->client->ps.torsoAnimTimer;
      }
...
void ForceJumpCharge(gentity_t *self, usercmd_t *ucmd) {
  ...
  // need to play sound
  if (!self->client->ps.forceJumpCharge) { // FIXME: this should last only as
                                           // long as the actual charge-up
  ...
  if (self->client->ps.forceJumpCharge / forceJumpChargeInterval /
    ...
    self->client->ps.forceJumpCharge = self->client->ps.forcePower *
...
void ForceJump(gentity_t *self, usercmd_t *ucmd) {
  ...
  if (!self->s.number &&
      (cg.zoomMode ||
       in_camera)) { // can't force jump when zoomed in or in cinematic
    return;
  }
  if (self->client->ps.saberLockTime >
      level.time) { // FIXME: can this be a way to break out?
  ...
  case FJ_FORWARD:
    if (((self->client->NPC_class == CLASS_BOBAFETT ||
        ...
        (self->client->ps.dualSabers &&
         (self->client->ps.saber[1].saberFlags & SFL_NO_FLIPS)) ||
        (self->NPC && self->NPC->rank != RANK_CREWMAN &&
  ...
  int parts = SETANIM_BOTH;
  if (self->client->ps.weaponTime) { // FIXME: really only care if we're in a
                                     // saber attack anim.. maybe trail length?
    parts = SETANIM_LEGS;
  }
  ...
  self->client->ps.forceJumpZStart =
      self->currentOrigin[2]; // remember this for when we land
  ...
  WP_ForcePowerStart(self, FP_LEVITATION,
                     self->client->ps.forceJumpCharge /
                         ...
                         (FORCE_JUMP_CHARGE_TIME / FRAMETIME) *
                         forcePowerNeeded[FP_LEVITATION]);
  // self->client->ps.forcePowerDuration[FP_LEVITATION] = level.time +
  // self->client->ps.weaponTime;
...
int WP_AbsorbConversion(gentity_t *attacked, int atdAbsLevel,
  ...
  if (!(attacked->client->ps.forcePowersActive &
...
void WP_ForcePowerStart(gentity_t *self, forcePowers_t forcePower,
  ...
  case FP_SPEED:
    ...
    duration = ceil(
        ...
        forceSpeedValue[self->client->ps.forcePowerLevel
                            [FP_SPEED]]); // FIXME: because the timescale scales
                                          // down (not instant), this doesn't end
                                          // up being exactly right...
    self->client->ps.forcePowersActive |= (1 << forcePower);
    self->s.loopSound = G_SoundIndex("sound/weapons/force/speedloop.wav");
    if (self->client->ps.forcePowerLevel[FP_SPEED] >
        FORCE_LEVEL_2) { // HACK: just using this as a timestamp for when the
                         // power started, setting debounce to current time
                         // shouldn't adversely affect anything else
  ...
  case FP_GRIP:
    duration = 1000;
    self->client->ps.forcePowersActive |= (1 << forcePower);
    // HACK: just using this as a timestamp for when the power started, setting
    // debounce to current time shouldn't adversely affect anything else
  ...
  case FP_RAGE:
    // duration is always 5 seconds, player time
    duration = ceil(FORCE_RAGE_DURATION *
                    forceSpeedValue[self->client->ps.forcePowerLevel[FP_RAGE] -
                                    1]); // FIXME: because the timescale scales
                                         // down (not instant), this doesn't end
                                         // up being exactly right...
    self->client->ps.forcePowersActive |= (1 << forcePower);
    ...
    if (self->chestBolt != -1) {
      G_PlayEffect(G_EffectIndex("force/rage2"), self->playerModel,
                   self->chestBolt, self->s.number, self->currentOrigin,
                   duration, qtrue);
    }
    break;
  case FP_DRAIN:
    if (self->client->ps.forcePowerLevel[forcePower] > FORCE_LEVEL_1 &&
      ...
      duration = overrideAmt;
      overrideAmt = 0;
      // HACK: just using this as a timestamp for when the power started,
      // setting debounce to current time shouldn't adversely affect anything
      // else
      self->client->ps.forcePowerDebounce[forcePower] = level.time;
    } else {
      duration = 1000;
...
qboolean WP_ForcePowerUsable(gentity_t *self, forcePowers_t forcePower,
                             int overrideAmt) {
  if (!(self->client->ps.forcePowersKnown &
        (1 << forcePower))) { // don't know this power
    return qfalse;
  } else if (self->NPC && (self->NPC->aiFlags & NPCAI_ROSH)) {
    if (((1 << forcePower) &
         FORCE_POWERS_ROSH_FROM_TWINS)) { // this is a force power we can only
                                          // use when a twin is near us
  ...
  if (self->client->ps.forcePowerLevel[forcePower] <=
      0) { // can't use this power
    return qfalse;
  }
  ...
  if (in_camera &&
      self->s.number <
          MAX_CLIENTS) { // player can't turn on force powers duing cinematics
    return qfalse;
  }

  if (PM_LockedAnim(self->client->ps.torsoAnim) &&
      self->client->ps
          .torsoAnimTimer) { // no force powers during these special anims
    return qfalse;
  }
  if (PM_SuperBreakLoseAnim(self->client->ps.torsoAnim) ||
      PM_SuperBreakWinAnim(self->client->ps.torsoAnim)) {
  ...
  */
  if (self->client->NPC_class ==
      CLASS_ATST) { // Doh!  No force powers in an AT-ST!
    return qfalse;
  }
  Vehicle_t *pVeh = NULL;
  if ((pVeh = G_IsRidingVehicle(self)) !=
      NULL) { // Doh!  No force powers when flying a vehicle!
    if (pVeh->m_pVehicleInfo->numHands > 1) { // if in a two-handed vehicle
      return qfalse;
    }
  }
  if (self->client->ps.viewEntity > 0 &&
  ...
  if ((self->client->ps.saber[0].saberFlags &
       SFL_SINGLE_BLADE_THROWABLE)    // SaberStaff() //using staff
  ...
  {                                                  // allow power
    ...
    if (forcePower == FP_SABERTHROW &&
        (self->client->ps.saber[0].saberFlags &
         SFL_NOT_THROWABLE)) { // cannot throw this kind of saber
      return qfalse;
    }
    ...
    if (self->client->ps.saber[0].Active()) {
      ...
      if ((self->client->ps.saber[0].saberFlags & SFL_TWO_HANDED) ||
          (self->client->ps.dualSabers &&
           self->client->ps.saber[1]
               .Active())) { // this saber requires the use of two hands OR our
                             // other hand is using an active saber too
        if ((self->client->ps.saber[0].forceRestrictions &
             (1
              << forcePower))) { // this power is verboten when using this saber
          return qfalse;
        }
      }
    }
    if (self->client->ps.dualSabers && self->client->ps.saber[1].Active()) {
      ...
      if ((self->client->ps.saber[1].forceRestrictions &
...
void WP_ForcePowerStop(gentity_t *self, forcePowers_t forcePower) {
  ...
  if (!(self->client->ps.forcePowersActive &
        (1 << forcePower))) { // umm, wasn't doing it, so...
    return;
  }
      ...
      if (g_timescale->value != 1.0) {
        if (!(self->client->ps.forcePowersActive & (1 << FP_RAGE)) ||
            self->client->ps.forcePowerLevel[FP_RAGE] <
                FORCE_LEVEL_2) { // not slowed down because of force rage
          gi.cvar_set("timescale", "1");
        }
      }
    }
    // FIXME: reset my current anim, keeping current frame, but with proper anim
    // speed 		otherwise, the anim will continue playing at high speed
    self->s.loopSound = 0;
    break;
  case FP_PUSH:
    break;
    ...
    if (self->client->ps.forceGripEntityNum < ENTITYNUM_WORLD) {
      ...
      if (gripEnt) {
        ...
        if (gripEnt->client) {
          ...
          // FIXME: they probably dropped their weapon, should we make them
          // flee?  Or should AI handle no-weapon behavior?
// rww - RAGDOLL_BEGIN
...
// rww - RAGDOLL_BEGIN
          ...
          // rww - RAGDOLL_END
          {
            ...
            if (gripEnt->client->ps.legsAnim == BOTH_CHOKE1 ||
              ...
              if (holdTime) {
                ...
                gripEnt->client->ps.pm_time =
                    gripEnt->client->ps.torsoAnimTimer;
          ...
          if (gripEnt->s.eType == ET_MISSILE) { // continue normal movement
            if (gripEnt->s.weapon == WP_THERMAL) {
              ...
              gripEnt->s.pos.trType =
                  TR_LINEAR; // FIXME: what about gravity-effected projectiles?
            }
    ...
    if (self->client->ps.torsoAnim == BOTH_FORCEGRIP_HOLD) {
      NPC_SetAnim(self, SETANIM_BOTH, BOTH_FORCEGRIP_RELEASE,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    }
    break;
  case FP_LIGHTNING:
    ...
    break;
  case FP_RAGE:
    self->client->ps.forceRageRecoveryTime =
        level.time + 10000; // recover for 10 seconds
    if (self->client->ps.forcePowerDuration[FP_RAGE] >
        level.time) { // still had time left, we cut it short
      self->client->ps.forceRageRecoveryTime -=
          (self->client->ps.forcePowerDuration[FP_RAGE] -
           level.time); // minus however much time you had left when you cut it
                        // short
    }
    if (!self->s.number) { // player using force speed
      if (g_timescale->value != 1.0) {
        if (!(self->client->ps.forcePowersActive &
              (1 << FP_SPEED))) { // not slowed down because of force speed
          gi.cvar_set("timescale", "1");
        }
      }
    }
    ...
    if (self->chestBolt != -1) {
      G_StopEffect("force/rage2", self->playerModel, self->chestBolt,
                   self->s.number);
    }
    break;
  case FP_DRAIN:
      ...
      if (drainEnt) {
        if (drainEnt->client) {
          ...
          if (drainEnt->health > 0) {
            if (drainEnt->client->ps.forcePowerDebounce[FP_PUSH] >
                level.time) { // they probably pushed out of it
            } else {
              // NPC_SetAnim( drainEnt, SETANIM_BOTH, BOTH_HUGGEESTOP1,
              // SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
              if (drainEnt->client->ps.torsoAnim !=
                  BOTH_FORCEPUSH) { // don't stop the push
                drainEnt->client->ps.torsoAnimTimer = 0;
              }
              drainEnt->client->ps.legsAnimTimer = 0;
            }
            if (drainEnt->NPC) { // if still alive after stopped draining, let
                                 // them wake others up
              G_AngerAlert(drainEnt);
            }
          } else { // leave the effect playing on them for a few seconds
            // drainEnt->client->ps.eFlags |= EF_FORCE_DRAINED;
            drainEnt->s.powerups |= (1 << PW_DRAINED);
            drainEnt->client->ps.powerups[PW_DRAINED] =
                level.time + Q_irand(1000, 4000);
          }
        }
      }
      self->client->ps.forceDrainEntityNum = ENTITYNUM_NONE;
    }
    if (self->client->ps.torsoAnim == BOTH_HUGGER1) { // old anim
      NPC_SetAnim(self, SETANIM_BOTH, BOTH_HUGGERSTOP1,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    } else if (self->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_START ||
               self->client->ps.torsoAnim ==
                   BOTH_FORCE_DRAIN_GRAB_HOLD) { // new anim
      NPC_SetAnim(self, SETANIM_BOTH, BOTH_FORCE_DRAIN_GRAB_END,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  ...
  case FP_ABSORB:
    ...
    if (self->client->ps.torsoAnim == BOTH_FORCE_ABSORB_START) {
      NPC_SetAnim(self, SETANIM_TORSO, BOTH_FORCE_ABSORB_END,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    }
    if (self->client->ps.forcePowerLevel[FP_ABSORB] <
        FORCE_LEVEL_2) { // was stuck, free us in case we interrupted it or
                         // something
      self->client->ps.weaponTime = 0;
      self->client->ps.pm_flags &= ~PMF_TIME_KNOCKBACK;
...
void WP_ForceForceThrow(gentity_t *thrower) {
  ...
  if (!(thrower->client->ps.forcePowersKnown &
        (1 << FP_PUSH))) { // give them push just for this specific purpose
    thrower->client->ps.forcePowersKnown |= (1 << FP_PUSH);
    thrower->client->ps.forcePowerLevel[FP_PUSH] = FORCE_LEVEL_1;
  }

  if (thrower->NPC && (thrower->NPC->aiFlags & NPCAI_HEAL_ROSH) &&
...
static void WP_ForcePowerRun(gentity_t *self, forcePowers_t forcePower,
  ...
  switch ((int)forcePower) {
  case FP_HEAL:
    if (self->client->ps.forceHealCount >= FP_MaxForceHeal(self) ||
        self->health >=
            self->client->ps
                .stats[STAT_MAX_HEALTH]) { // fully healed or used up all 25
      if (!Q3_TaskIDPending(self, TID_CHAN_VOICE)) {
        ...
        if (self->s.number < MAX_CLIENTS) {
          ...
          if (self->NPC->blockedSpeechDebounceTime <=
              level.time) { // enough time has passed since our last speech
            if (Q3_TaskIDPending(
                    self, TID_CHAN_VOICE)) { // not playing a scripted line
              // say "Ahhh...."
              if (self->NPC->stats.sex == SEX_MALE ||
                  self->NPC->stats.sex == SEX_NEUTRAL) {
                G_SoundOnEnt(self, CHAN_VOICE,
                             va("sound/weapons/force/heal%d_m.mp3", index));
              } else // all other sexes use female sounds
              {
                G_SoundOnEnt(self, CHAN_VOICE,
                             va("sound/weapons/force/heal%d_f.mp3", index));
              }
            }
          }
        }
      ...
      WP_ForcePowerStop(self, forcePower);
               ...
               (cmd->rightmove || cmd->forwardmove ||
                cmd->upmove > 0)) { // moved while healing... FIXME: also, in
                                    // WP_ForcePowerStart, stop healing if any
                                    // other force power is used
      // stop healing
      WP_ForcePowerStop(self, forcePower);
      ...
      if (WP_ForcePowerAvailable(self, forcePower, 4)) { // have available power
        ...
        if (self->s.number < MAX_CLIENTS) {
          if (g_spskill->integer == 1) { // medium, heal twice as fast
            healAmount *= 2;
          } else if (g_spskill->integer == 0) { // easy, heal 3 times as fast...
            healAmount *= 3;
          }
        }
        if (self->health + healAmount >
      ...
      if (PM_ForceJumpingUp(self)) { // holding jump in air
        if (cmd->upmove > 10) {      // still trying to go up
          if (WP_ForcePowerAvailable(self, FP_LEVITATION, 1)) {
            if (self->client->ps.forcePowerDebounce[FP_LEVITATION] <
                level.time) {
            ...
            self->client->ps.forceJumpCharge =
                1; // just used as a flag for the player, cleared when he lands
  ...
  case FP_SPEED:
    ...
    if (!self->s.number) { // player using force speed
      if (!(self->client->ps.forcePowersActive & (1 << FP_RAGE)) ||
          self->client->ps.forcePowerLevel[FP_SPEED] >=
              self->client->ps
                  .forcePowerLevel[FP_RAGE]) { // either not using rage or rage
                                               // is at a lower level than speed
  ...
  case FP_GRIP:
    if (!WP_ForcePowerAvailable(self, FP_GRIP, 0) ||
        (self->client->ps.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1 &&
         !self->s.number && !(cmd->buttons & BUTTON_FORCEGRIP))) {
      WP_ForcePowerStop(self, FP_GRIP);
      return;
      ...
      if (!gripEnt || !gripEnt->inuse) { // invalid or freed ent
        WP_ForcePowerStop(self, FP_GRIP);
        return;
      } else
// rww - RAGDOLL_BEGIN
          ...
          // rww - RAGDOLL_END
          if (gripEnt->health <= 0 &&
              gripEnt->takedamage) // FIXME: what about things that never had
                                   // health or lose takedamage when they die?
...
// rww - RAGDOLL_BEGIN
        ...
        return;
                 ...
                 VectorLengthSquared(gripEnt->client->ps.velocity) >
                     (300 * 300)) { // flying creature broke free
        WP_ForcePowerStop(self, FP_GRIP);
        return;
      } else if (gripEnt->client && gripEnt->health > 0 // dead dudes don't fly
                 && (gripEnt->client->NPC_class == CLASS_BOBAFETT ||
                     gripEnt->client->NPC_class == CLASS_ROCKETTROOPER) &&
                 self->client->ps.forcePowerDebounce[FP_GRIP] < level.time &&
                 !Q_irand(0, 3)) { // boba fett - fly away!
        gripEnt->client->ps.forceJumpCharge =
            0; // so we don't play the force flip anim
        gripEnt->client->ps.velocity[2] = 250;
        gripEnt->client->ps.forceJumpZStart =
            gripEnt->currentOrigin[2]; // so we don't take damage if we land at
                                       // same height
        ...
        WP_ForcePowerStop(self, FP_GRIP);
        return;
      } else if (gripEnt->NPC && gripEnt->client &&
                 gripEnt->client->ps.forcePowersKnown &&
                 (gripEnt->client->NPC_class == CLASS_REBORN ||
                  gripEnt->client->ps.weapon == WP_SABER) &&
                 !Jedi_CultistDestroyer(gripEnt) &&
                 !Q_irand(
                     0,
                     100 - (gripEnt->NPC->stats.evasion * 8) -
                         (g_spskill->integer *
                          20))) { // a jedi who broke free FIXME: maybe have
                                  // some minimum grip length- a reaction time?
        ...
        return;
                 ...
                 PM_SaberInStart(
                     self->client->ps.saberMove)) { // started an attack
        WP_ForcePowerStop(self, FP_GRIP);
        return;
      } else {
        ...
        if (gripEnt->client) {
          gripLevel = WP_AbsorbConversion(
              gripEnt, gripEnt->client->ps.forcePowerLevel[FP_ABSORB], self,
              FP_GRIP, self->client->ps.forcePowerLevel[FP_GRIP],
              forcePowerNeeded[gripLevel]);
        }
        if (!gripLevel) {
          WP_ForcePowerStop(self, forcePower);
          return;
        ...
        if (self->client->ps.forcePowerLevel[FP_GRIP] >
            FORCE_LEVEL_1) { // holding it
          NPC_SetAnim(self, SETANIM_TORSO, BOTH_FORCEGRIP_HOLD,
                      SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
          if (self->client->ps.torsoAnimTimer <
              100) { // we were already playing this anim, we didn't want to
                     // restart it, but we want to hold it for at least 100ms,
                     // sooo....
        ...
        if (self->client->ps.forcePowerLevel[FP_GRIP] == FORCE_LEVEL_2 &&
            (!InFront(gripEntOrg, self->client->renderInfo.handLPoint,
                      self->client->ps.viewangles, 0.3f) ||
        ...
        if (self->client->ps.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_2 &&
            (!gripEnt->client ||
             (!gripEnt->message &&
              !(gripEnt->flags & FL_NO_KNOCKBACK)))) { // carry
          // cap dist
        ...
        if (self->client->ps.forcePowerLevel[FP_GRIP] >
            FORCE_LEVEL_1) { // if holding him, make sure there's a clear LOS
                             // between my hand and him
          trace_t gripTrace;
          gi.trace(&gripTrace, self->client->renderInfo.handLPoint, NULL, NULL,
                   gripEntOrg, ENTITYNUM_NONE, MASK_FORCE_PUSH,
                   (EG2_Collision)0, 0);
          if (gripTrace.startsolid || gripTrace.allsolid ||
              gripTrace.fraction < 1.0f) { // no clear trace, drop them
            WP_ForcePowerStop(self, FP_GRIP);
            return;
          }
        }
        ...
        if (gripEnt->client) {
          if (self->client->ps.forcePowerLevel[FP_GRIP] >
            ...
            if (self->client->ps.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_2 &&
                (!gripEnt->client ||
                 (!gripEnt->message &&
                  !(gripEnt->flags &
                    FL_NO_KNOCKBACK)))) { // level 2 just lifts them
              float gripDist =
              ...
              if (gripDist < 20.0f) {
                if (gripDist < 2.0f) {
                  ...
                  VectorScale(gripEnt->client->ps.velocity,
                              (gripDist * gripDist),
                              gripEnt->client->ps.velocity);
                }
              } else {
          ...
          if (gripEnt->NPC) {
            ...
            if (self->client->ps.forcePowerLevel[FP_GRIP] >
              ...
              RestoreNPCGlobals();
              // FIXME: why does he turn back to his original angles once he
              // dies or is let go?
            }
          } else if (!gripEnt->s.number) {
            // vectoangles( dir, angles );
            // gripEnt->client->ps.viewangles[0] = -angles[0];
            // gripEnt->client->ps.viewangles[1] =
            // AngleNormalize180(angles[YAW]+180);
            gripEnt->enemy = self;
        ...
        if (self->client->ps.forcePowerDebounce[FP_GRIP] < level.time) {
          ...
          if (!gripEnt->client || gripEnt->client->NPC_class != CLASS_VEHICLE ||
              (gripEnt->m_pVehicle && gripEnt->m_pVehicle->m_pVehicleInfo &&
               gripEnt->m_pVehicle->m_pVehicleInfo->type ==
                   VH_ANIMAL)) { // we don't damage the empty vehicle
            ...
            G_Damage(gripEnt, self, self, dir, gripOrg, gripDmg,
          ...
          if (self->client->NPC_class == CLASS_KYLE &&
              (self->spawnflags & 1)) { //"Boss" Kyle
            if (gripEnt->client) {
              if (!Q_irand(0, 2)) { // toss him aside!
                vec3_t vRt;
                ...
                WP_ForcePowerStop(self, FP_GRIP);
                // now toss him
                if (Q_irand(0, 1)) { // throw him to my left
                ...
                self->client->ps.weaponTime =
                    self->client->ps.torsoAnimTimer + 2000;
                self->painDebounceTime =
          ...
          if (!gripEnt->enemy) {
            if (gripEnt->client && gripEnt->client->playerTeam == TEAM_PLAYER &&
                self->s.number < MAX_CLIENTS && self->client &&
                self->client->playerTeam ==
                    TEAM_PLAYER) { // this shouldn't make allies instantly turn
                                   // on you, let the damage->pain routine
                                   // determine how allies should react to this
            } else {
  ...
  case FP_RAGE:
    ...
    if (self->health < 1) {
      ...
      if (!self->s.number) { // player using force rage
        if (!(self->client->ps.forcePowersActive & (1 << FP_SPEED)) ||
            self->client->ps.forcePowerLevel[FP_RAGE] >
                self->client->ps.forcePowerLevel[FP_SPEED] +
                    1) { // either not using speed or speed is at a lower level
                         // than rage
          gi.cvar_set("timescale", va("%4.2f", speed));
          if (g_timescale->value > speed) {
            newSpeed = g_timescale->value - 0.05;
  ...
  case FP_DRAIN:
    ...
    if (!WP_ForcePowerAvailable(self, forcePower,
      ...
      if (!WP_ForcePowerAvailable(self, FP_DRAIN, 0) ||
        ...
        if (!drainEnt) { // invalid ent
          WP_ForcePowerStop(self, FP_DRAIN);
          return;
        } else if ((drainEnt->health <= 0 &&
                    drainEnt->takedamage)) // FIXME: what about things that
                                           // never had health or lose takedamage
                                           // when they die?
        { // dead ent
          WP_ForcePowerStop(self, FP_DRAIN);
          return;
          ...
          return;
                   ...
                   && (drainEnt->client->NPC_class == CLASS_BOBAFETT ||
                       drainEnt->client->NPC_class == CLASS_ROCKETTROOPER) &&
                   self->client->ps.forcePowerDebounce[FP_DRAIN] < level.time &&
                   !Q_irand(0, 10)) { // boba fett - fly away!
          drainEnt->client->ps.forceJumpCharge =
              0; // so we don't play the force flip anim
          drainEnt->client->ps.velocity[2] = 250;
          drainEnt->client->ps.forceJumpZStart =
              drainEnt->currentOrigin[2]; // so we don't take damage if we land
                                          // at same height
          ...
          WP_ForcePowerStop(self, FP_DRAIN);
          return;
        } else if (drainEnt->NPC && drainEnt->client &&
                   drainEnt->client->ps.forcePowersKnown &&
                   (drainEnt->client->NPC_class == CLASS_REBORN ||
                    drainEnt->client->ps.weapon == WP_SABER) &&
                   ...
                   !Q_irand(
                       0,
                       100 - (drainEnt->NPC->stats.evasion * 8) -
                           (g_spskill->integer *
                            15))) { // a jedi who broke free FIXME: maybe have
                                    // some minimum grip length- a reaction time?
          ...
          /*
          int drainLevel = WP_AbsorbConversion( drainEnt,
          drainEnt->client->ps.forcePowerLevel[FP_ABSORB], self, FP_DRAIN,
          self->client->ps.forcePowerLevel[FP_DRAIN],
          forcePowerNeeded[self->client->ps.forcePowerLevel[FP_DRAIN]] ); if (
          !drainLevel )
          {
                  WP_ForcePowerStop( self, forcePower );
                  return;
          ...
          */
          ...
          // NPC_SetAnim( self, SETANIM_BOTH, BOTH_HUGGER1,
          // SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
          if (self->client->ps.torsoAnim != BOTH_FORCE_DRAIN_GRAB_START ||
              !self->client->ps.torsoAnimTimer) {
            NPC_SetAnim(self, SETANIM_BOTH, BOTH_FORCE_DRAIN_GRAB_HOLD,
                        SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
          }
          if (self->handLBolt != -1) {
            G_PlayEffect(G_EffectIndex("force/drain_hand"), self->playerModel,
                         self->handLBolt, self->s.number, self->currentOrigin,
                         200, qtrue);
          }
          if (self->handRBolt != -1) {
            G_PlayEffect(G_EffectIndex("force/drain_hand"), self->playerModel,
                         self->handRBolt, self->s.number, self->currentOrigin,
                         200, qtrue);
          }

          // how far are they
          dist = Distance(self->client->renderInfo.eyePoint,
                          drainEnt->currentOrigin);
          if (DistanceSquared(drainEnt->currentOrigin, self->currentOrigin) >
              FORCE_DRAIN_DIST_SQUARED) { // must be close, got away somehow!
            WP_ForcePowerStop(self, FP_DRAIN);
            return;
          }
          ...
          if (drainEnt->client) {
            ...
            AngleVectors(angles, dir, NULL, NULL);
            /*
            VectorMA( self->currentOrigin, self->maxs[0], dir,
            drainEnt->client->ps.forceDrainOrg ); trace_t	trace; gi.trace(
            &trace, drainEnt->currentOrigin, drainEnt->mins, drainEnt->maxs,
            drainEnt->client->ps.forceDrainOrg, drainEnt->s.number,
            drainEnt->clipmask ); if ( !trace.startsolid && !trace.allsolid )
            {
                    G_SetOrigin( drainEnt, trace.endpos );
                    gi.linkentity( drainEnt );
                    VectorClear( drainEnt->client->ps.velocity );
            }
            VectorMA( self->currentOrigin, self->maxs[0]*0.5f, dir,
            drainEnt->client->ps.forceDrainOrg );
            */
            // stop them from thinking
            drainEnt->client->ps.pm_time = 2000;
            drainEnt->client->ps.pm_flags |= PMF_TIME_KNOCKBACK;
            if (drainEnt->NPC) {
              if (!(drainEnt->NPC->aiFlags &
                    NPCAI_DIE_ON_IMPACT)) { // not falling to their death
          ...
          if (self->client->ps.forcePowerDebounce[FP_DRAIN] < level.time) {
            int drainLevel = WP_AbsorbConversion(
                drainEnt, drainEnt->client->ps.forcePowerLevel[FP_ABSORB], self,
                FP_DRAIN, self->client->ps.forcePowerLevel[FP_DRAIN],
                forcePowerNeeded[self->client->ps.forcePowerLevel[FP_DRAIN]]);
            if ((drainLevel && drainLevel == -1) ||
                Q_irand(drainLevel, 3) < 3) { // the drain is being absorbed
          ...
          if (drainEnt->health > 0) { // still alive
            // NPC_SetAnim( drainEnt, SETANIM_BOTH, BOTH_HUGGEE1,
            // SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
    ...
    } else if (self->client->ps.forcePowerLevel[forcePower] >
               FORCE_LEVEL_1) {                // regular distance-drain
      if (cmd->buttons & BUTTON_FORCE_DRAIN) { // holding it keeps it going
        self->client->ps.forcePowerDuration[FP_DRAIN] = level.time + 500;
...
void WP_CheckForcedPowers(gentity_t *self, usercmd_t *ucmd) {
  for (int forcePower = FP_FIRST; forcePower < NUM_FORCE_POWERS; forcePower++) {
    if ((self->client->ps.forcePowersForced & (1 << forcePower))) {
      ...
      case FP_GRIP:
        ucmd->buttons &=
            ~(BUTTON_ATTACK | BUTTON_ALT_ATTACK | BUTTON_FORCE_FOCUS |
              BUTTON_FORCE_DRAIN | BUTTON_FORCE_LIGHTNING);
      ...
      case FP_LIGHTNING:
        ucmd->buttons &=
            ~(BUTTON_ATTACK | BUTTON_ALT_ATTACK | BUTTON_FORCE_FOCUS |
              BUTTON_FORCEGRIP | BUTTON_FORCE_DRAIN);
        ucmd->buttons |= BUTTON_FORCE_LIGHTNING;
        // holds until cleared
        break;
      case FP_SABERTHROW:
        ucmd->buttons &=
            ~(BUTTON_ATTACK | BUTTON_FORCE_FOCUS | BUTTON_FORCEGRIP |
              BUTTON_FORCE_DRAIN | BUTTON_FORCE_LIGHTNING);
        ucmd->buttons |= BUTTON_ALT_ATTACK;
        // holds until cleared?
        break;
      ...
      case FP_DRAIN:
        ucmd->buttons &=
            ~(BUTTON_ATTACK | BUTTON_ALT_ATTACK | BUTTON_FORCE_FOCUS |
              BUTTON_FORCEGRIP | BUTTON_FORCE_LIGHTNING);
...
void WP_ForcePowersUpdate(gentity_t *self, usercmd_t *ucmd) {
  ...
  if (!self->s.number) { // player uses different kind of force-jump
    ...
    if (self->client->ps
            .forceJumpCharge) { // let go of charge button, have charge
      // if leave the ground by some other means, cancel the force jump so we
      // don't suddenly jump when we land.
      if (self->client->ps.groundEntityNum == ENTITYNUM_NONE &&
          !PM_SwimmingAnim(
              self->client->ps
                  .legsAnim)) { // FIXME: stop sound?
                                // self->client->ps.forceJumpCharge = 0;
                                // FIXME: actually, we want this to still be
                                // cleared... don't clear it if the button isn't
                                // being pressed, but clear it if not holding
                                // button and not on ground.
  ...
  if (!self->s.number &&
      self->client->NPC_class == CLASS_BOBAFETT) { // Boba Fett
    if (ucmd->buttons & BUTTON_FORCE_LIGHTNING) {  // start flamethrower
  ...
  if (ucmd->buttons & BUTTON_FORCE_DRAIN) {
    if (!ForceDrain2(self)) { // can't drain-grip someone right in front
      if (self->client->ps.forcePowerLevel[FP_DRAIN] >
          FORCE_LEVEL_1) { // try ranged
  ...
  if (self->client->ps
          .saberInFlight) { // don't regen force power while throwing saber
    if (self->client->ps.saberEntityNum < ENTITYNUM_NONE &&
    ...
    {                                        //
      if (&g_entities[self->client->ps.saberEntityNum] != NULL &&
          g_entities[self->client->ps.saberEntityNum].s.pos.trType ==
              TR_LINEAR) { // fell to the ground and we're trying to pull it
                           // back
        usingForce = qtrue;
      }
    }
  ...
  if (!usingForce) { // when not using the force, regenerate at 10 points per
                     // second
    if (self->client->ps.forcePowerRegenDebounceTime < level.time) {
      ...
      if (self->client->ps.forceRageRecoveryTime >=
          level.time) { // regen half as fast
        self->client->ps.forcePowerRegenDebounceTime +=
...
void WP_InitForcePowers(gentity_t *ent) {
  ...
  ent->client->ps.forceGripEntityNum = ent->client->ps.forceDrainEntityNum =
      ent->client->ps.pullAttackEntNum = ENTITYNUM_NONE;
  ...
  if (ent->s.number < MAX_CLIENTS) { // player
    if (!g_cheats->integer)          // devmaps give you all the FP
    {
      ...
      ent->client->ps.forcePowersKnown =
          (1 << FP_HEAL) | (1 << FP_LEVITATION) | (1 << FP_SPEED) |
          (1 << FP_PUSH) | (1 << FP_PULL) | (1 << FP_TELEPATHY) |
          (1 << FP_GRIP) | (1 << FP_LIGHTNING) | (1 << FP_SABERTHROW) |
          (1 << FP_SABER_DEFENSE) | (1 << FP_SABER_OFFENSE) | (1 << FP_RAGE) |
          (1 << FP_DRAIN) | (1 << FP_PROTECT) | (1 << FP_ABSORB) |
          (1 << FP_SEE);
