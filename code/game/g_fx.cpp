...
void fx_runner_think(gentity_t *ent) {
  ...
  MakeNormalVectors(ent->pos3, ent->pos4,
                    temp); // there IS a reason this is done...it's so that it
                           // doesn't break every effect in the game...

  ent->nextthink = level.time + ent->delay + Q_flrand(0.0f, 1.0f) * ent->random;

  if (ent->spawnflags & 4) // damage
  {
    G_RadiusDamage(ent->currentOrigin, ent, ent->splashDamage,
                   ent->splashRadius, ent, MOD_UNKNOWN);
  }
...
void SP_CreateWind(gentity_t *ent) {
  ...
  if (ent->spawnflags & 2) {
    ...
    sprintf(temp, "constantwind ( %f %f %f )", windDir[0], windDir[1],
            windDir[2]);
...
void SP_CreateWindZone(gentity_t *ent) {
  ...
  char temp[256];
  sprintf(temp, "windzone ( %f %f %f ) ( %f %f %f ) ( %f %f %f )", ent->mins[0],
          ent->mins[1], ent->mins[2], ent->maxs[0], ent->maxs[1], ent->maxs[2],
          windDir[0], windDir[1], windDir[2]);
  G_FindConfigstringIndex(temp, CS_WORLD_FX, MAX_WORLD_FX, qtrue);
}

/*QUAKED fx_rain (1 0 0) (-16 -16 -16) (16 16 16) LIGHT MEDIUM HEAVY ACID
OUTSIDE_SHAKE MISTY_FOG This world effect will spawn rain globally into the
level.
...
void fx_rain_think(gentity_t *ent) {
  if (player) {
    if (ent->count != 0) {
      ...
      bool PlaySound =
          (PlayEffect || PlayFlicker || Q_irand(1, ent->pushDebounceTime) == 1);
      ...
      if (PlaySound && !PlayEffect) {
        ...
        G_SoundAtSpot(
            effectPos,
            G_SoundIndex(va("sound/ambience/thunder%d", Q_irand(1, 4))), qtrue);
      }
      ...
      if (PlayEffect) {
        ...
        if (PlaySound) {
          G_Sound(player, G_SoundIndex(va("sound/ambience/thunder_close%d",
                                          Q_irand(1, 2))));
        }
...
wind ( windOriginX windOriginY windOriginZ ) ( windVelocityX windVelocityY
windVelocityZ ) ( sizeX sizeY sizeZ  )
...
void SP_CreatePuffSystem(gentity_t *ent) {
  ...
  // Initialize the puff system to either 1000 particles or whatever they
  // choose.
  ...
  if (r_weatherScale->value > 0.0f) {
    sprintf(temp, "puff%i init %i", iPuffSystem,
            (int)(ent->count * r_weatherScale->value));
  ...
  // Go through all the fields and assign the values to the created puff system
  // now.
  for (int i = 0; i < 20; i++) {
    char *key = NULL;
    char *value = NULL;
    ...
    if (Q_stricmp(key, "origin") == 0 || Q_stricmp(key, "classname") == 0 ||
        ...
        Q_stricmp(key, "sabersparks") == 0 ||
        Q_stricmp(key, "whichsystem") == 0)
      continue;
...
void fx_explosion_trail_think(gentity_t *ent) {
  ...
  if (tr.fraction < 1.0f) {
    ...
    if (!(tr.surfaceFlags & SURF_NOIMPACT)) {
      if (ent->splashDamage && ent->splashRadius) {
        G_RadiusDamage(tr.endpos, ent, ent->splashDamage, ent->splashRadius,
                       ent, MOD_EXPLOSIVE_SPLASH);
      }
    }
    ...
    if (VALIDSTRING(ent->soundSet) == true) {
      G_AddEvent(ent, EV_BMODEL_SOUND,
                 CAS_GetBModelSound(ent->soundSet, BMS_END));
    }
  ...
  G_RadiusDamage(origin, ent, ent->damage, ent->radius, ent,
...
//----------------------------------------------------------
void fx_explosion_trail_use(gentity_t *self, gentity_t *other,
                            gentity_t *activator) {
  gentity_t *missile = G_Spawn();

  // We aren't a missile in the truest sense, rather we just move through the
  // world and spawn effects
  if (missile) {
    ...
    if (VALIDSTRING(self->soundSet) == true) {
      ...
      missile->soundSet = G_NewString(
          self->soundSet); // get my own copy so i can free it when i die
  ...
  if (ent->target) {
    ...
    if (!target) {
      gi.Printf(S_COLOR_RED
                "ERROR: fx_explosion_trail %s could not find target %s\n",
    ...
    // we are assuming that we have angles, but there are no checks to verify
    // this
...
Creates an explosion type trail using the specified effect file, damaging things
as it moves through the environment Can also be used for something like a
meteor, just add an impact effect ( fxFile2 ) and a splashDamage and
splashRadius

  GRAVITY - object uses gravity instead of linear motion

  "fxFile" - name of the effect to play for the trail ( default
"env/exp_trail_comp" ) "fxFile2" - effect file to play on impact
  ...
  "radius" - damage radius around trail as it travels through the world (
default 128 ) "damage" - radius damage ( default 128 ) "splashDamage" - damage
when thing impacts ( default 0 ) "splashRadius" - damage radius on impact (
default 0 ) "soundset" - soundset to use, start sound plays when explosion trail
starts, loop sound plays on explosion trail, end sound plays when it impacts
...
void SP_fx_explosion_trail(gentity_t *ent) {
  ...
  if (!ent->targetname) {
    gi.Printf(S_COLOR_RED
              "ERROR: fx_explosion_trail at %s has no targetname specified\n",
              vtos(ent->s.origin));
    G_FreeEntity(ent);
    return;
  }
  ...
  // Try to associate an effect file, unfortunately we won't know if this worked
  // or not until the CGAME trys to register it...
  ent->fxID = G_EffectIndex(ent->fxFile);
...
void fx_target_beam_set_debounce(gentity_t *self) {
  if (self->wait >= FRAMETIME) {
    self->attackDebounceTime =
        level.time + self->wait + Q_irand(-self->random, self->random);
  } else if (self->wait < 0) {
    self->e_UseFunc = useF_NULL;
  } else {
    self->attackDebounceTime =
        level.time + FRAMETIME + Q_irand(-self->random, self->random);
  }
}
...
void fx_target_beam_fire(gentity_t *ent) {
  ...
  if (!ent->enemy || !ent->enemy->inuse) { // info_null most likely
    // ignore = ent->s.number;
    ent->enemy = NULL;
  ...
  gi.trace(&trace, ent->s.origin, NULL, NULL, org, ENTITYNUM_NONE, MASK_SHOT,
           (EG2_Collision)0, 0); // ignore
  if (ent->spawnflags & 2) {
    open = qtrue;
    VectorCopy(org, end);
  } else {
  ...
  if (trace.fraction < 1.0) {
    if (trace.entityNum < ENTITYNUM_WORLD) {
      ...
      if (victim && victim->takedamage) {
        ...
        {
          G_Damage(victim, ent, ent->activator, dir, trace.endpos, ent->damage,
                   DAMAGE_NO_KNOCKBACK, MOD_UNKNOWN);
        } else {
          G_Damage(victim, ent, ent->activator, dir, trace.endpos, ent->damage,
                   0, MOD_UNKNOWN);
        }
...
//------------------------------------------
void fx_target_beam_use(gentity_t *self, gentity_t *other,
                        gentity_t *activator) {
...
void fx_target_beam_link(gentity_t *ent) {
  ...
  if (!target->classname ||
      Q_stricmp("info_null",
                target->classname)) { // don't want to set enemy to something
                                      // that's going to free itself... actually,
                                      // this could be bad in other ways, too...
                                      // ent pointer could be freed up and
                                      // re-used by the time we check it next
    G_SetEnemy(ent, target);
  }
  VectorSubtract(target->s.origin, ent->s.origin,
                 dir);             // er, does it ever use dir?
...
/*QUAKED fx_target_beam (1 0.5 0.5) (-8 -8 -8) (8 8 8) STARTOFF OPEN
NO_KNOCKBACK ONE_SHOT NO_IMPACT Emits specified effect file, doing damage if
required
...
NO_KNOCKBACK - beam damage does no knockback
 ...
 "fxFile2" - Effect used for the beam impact effect, ( default
"env/targ_beam_impact" ) "targetname" - Fires only when used "duration" - How
many seconds each burst lasts, -1 will make it stay on forever "wait" - If
always on, how long to wait between blasts, in MILLISECONDS - default/min is 100
(1 frame at 10 fps), -1 means it will never fire again "random" - random amount
of seconds added to/subtracted from "wait" each firing "damage" - How much
damage to inflict PER FRAME (so probably want it kind of low), default is none
 "target" - ent to point at- you MUST have this.  This can be anything you want,
including a moving ent - for static beams, just use info_null
*/
//------------------------------------------
...
/*QUAKED fx_cloudlayer (1 0.3 0.5) (-8 -8 -8) (8 8 8) TUBE ALT
  ...
  Creates a scalable scrolling cloud layer, mostly for bespin undercity but
could be used other places

  TUBE - creates cloud layer with tube opening in the middle, must an INNER
radius also ALT - uses slightly different shader, good if using two layers sort
of close together

"radius" - outer radius of cloud layer, (default 2048)
"random" - inner radius of cloud layer, (default 128) only works for TUBE type
"wait" - adds curvature as it moves out to the edge of the layer.  ( default 0
), 1 = small up, 3 = up more, -1 = small down, -3 = down more, etc.

*/

void SP_fx_cloudlayer(gentity_t *ent) {
  // HACK: this effect is never played, rather it just caches the shaders I need
  // cgame side
