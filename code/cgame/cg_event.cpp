...
void CG_ItemPickup(int itemNum, qboolean bHadItem) {
  ...
  if (bg_itemlist[itemNum].classname && bg_itemlist[itemNum].classname[0]) {
    ...
    if (cgi_SP_GetStringTextString("SP_INGAME_PICKUPLINE", text,
                                   sizeof(text))) {
      if (cgi_SP_GetStringTextString(
              va("SP_INGAME_%s", bg_itemlist[itemNum].classname), data,
              sizeof(data))) {
        //				Com_Printf("%s %s\n", text, data );
...
void CG_EntityEvent(centity_t *cent, vec3_t position) {
  ...
  case EV_FALL_MEDIUM:
    ...
    if (g_entities[es->number].health <= 0) { // dead
      cgi_S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.landSound);
               ...
               (g_entities[es->number].client &&
                (g_entities[es->number].client->ps.forcePowersKnown &
                 (1 << FP_LEVITATION)))) { // jedi or someone who has force jump
                                           // (so probably took no damage)
  ...
  case EV_KOTHOS_BEAM:
    ...
    if (Q_irand(0, 1)) {
      FX_KothosBeam(cg_entities[cent->currentState.otherEntityNum]
                        .gent->client->renderInfo.handRPoint,
                    cg_entities[cent->currentState.otherEntityNum2].lerpOrigin);
    } else {
      FX_KothosBeam(cg_entities[cent->currentState.otherEntityNum]
                        .gent->client->renderInfo.handLPoint,
                    cg_entities[cent->currentState.otherEntityNum2].lerpOrigin);
    }
    break;
  //=================================================================
  ...
  case EV_BATTERIES_CHARGED:
    ...
    cgi_S_StartSound(vec3_origin, es->number, CHAN_AUTO,
  ...
  case EV_PLAY_EFFECT:
    ...
    {
      const bool portalEnt =
          !!es->isPortalEnt; // the fxrunner spawning this effect is within a
                             // skyportal, so only render this effect within that
                             // portal.
      ...
      if (es->boltInfo != 0) {
        ...
        theFxScheduler.PlayEffect(s, cent->lerpOrigin, axis, es->boltInfo, -1,
                                  portalEnt, es->loopSound,
                                  isRelative); // loopSound 0 = not looping, 1
  ...
  case EV_STOP_EFFECT:
    DEBUGNAME("EV_STOP_EFFECT");
    {
      ...
      if (es->isPortalEnt) { // the fxrunner spawning this effect is within a
                             // skyportal, so only render this effect within that
                             // portal.
        portalEnt = true;
      }
