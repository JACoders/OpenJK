...
#else
bool VEH_StartStrafeRam(Vehicle_t *pVeh, bool Right, int Duration) { return false; }
#endif
...
static void ProcessMoveCommands(Vehicle_t *pVeh) {
				...
				if (pVeh->m_pVehicleInfo->iTurboStartFX) {
					...
					for (i = 0; (i < MAX_VEHICLE_EXHAUSTS && pVeh->m_iExhaustTag[i] != -1); i++) {
						...
						if (pVeh->m_pVehicleInfo->iTurboFX) {
							G_PlayEffect(pVeh->m_pVehicleInfo->iTurboFX, pVeh->m_pParentEntity->playerModel, pVeh->m_iExhaustTag[i],
										 pVeh->m_pParentEntity->s.number, pVeh->m_pParentEntity->currentOrigin, pVeh->m_pVehicleInfo->turboDuration, qtrue);
						}
					...
					if (pVeh->m_pParentEntity && pVeh->m_pParentEntity->ghoul2 &&
						...
						trap_G2API_GetBoltMatrix(pVeh->m_pParentEntity->ghoul2, 0, pVeh->m_iExhaustTag[i], &boltMatrix, boltDir,
	...
	if (pVeh->m_ulFlags & VEH_SLIDEBREAKING) {
		...
		parentPS->speed = 0;
	} else if ((curTime > pVeh->m_iTurboTime) && !(pVeh->m_ulFlags & VEH_FLYING) && pVeh->m_ucmd.forwardmove < 0 && fabs(pVeh->m_vOrientation[ROLL]) > 25.0f) {
		pVeh->m_ulFlags |= VEH_SLIDEBREAKING;
	}
...
void ProcessOrientCommands(Vehicle_t *pVeh) {
	...
	} else if ((pVeh->m_ulFlags & VEH_SLIDEBREAKING) || // No Angles Control While Out Of Control
			   (pVeh->m_ulFlags & VEH_OUTOFCONTROL)		// No Angles Control While Out Of Control
	) {
		// Any ability to change orientation?
	} else if ((pVeh->m_ulFlags & VEH_STRAFERAM) // No Angles Control While Strafe Ramming
	) {
	...
	*/
	{
		...
		//--------------------------------------
#ifndef _JK2MP
		if (HasWeapon && (pVeh->m_pPilot->s.number >= MAX_CLIENTS || (cg.weaponSelectTime + 500) < cg.time) &&
		...
		if (pilotPS->torsoAnim >= BOTH_VS_ATL_S && pilotPS->torsoAnim <= BOTH_VS_ATF_G) {
			...
			int bodyEnd = 0;
			if (!!gi.G2API_GetBoneAnimIndex(&pVeh->m_pPilot->ghoul2[pVeh->m_pPilot->playerModel], pVeh->m_pPilot->rootBone, level.time, &bodyCurrent, NULL,
