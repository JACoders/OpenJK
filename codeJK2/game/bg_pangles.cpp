...
void BG_G2SetBoneAngles(centity_t *cent, gentity_t *gent, int boneIndex,
  ...
  if (boneIndex != -1) {
    gi.G2API_SetBoneAnglesIndex(&cent->gent->ghoul2[0], boneIndex, angles,
                                flags, up, left, forward, modelList, 0, 0);
  }
}
...
void PM_ScaleUcmd(playerState_t *ps, usercmd_t *cmd, gentity_t *gent) {
  if (ps->vehicleModel != 0) { // driving a vehicle
    // clamp the turn rate
    int maxPitchSpeed =
...
qboolean PM_AdjustAngleForWallRun(gentity_t *ent, usercmd_t *ucmd,
  ...
  if ((ent->client->ps.legsAnim == BOTH_WALL_RUN_RIGHT ||
    ...
    vec3_t rt, traceTo, mins = {ent->mins[0], ent->mins[1], 0},
    ...
    if (trace.fraction < 1.0f &&
        trace.plane.normal[2] == 0.0f) { // still a vertical wall there
      ...
      if (ent->s.number || !player_locked) {
        if (doMove) {
          ...
          if (ent->client->ps.legsAnimTimer > 500) { // not at end of anim yet
          ...
          // VectorMA( ent->client->ps.velocity, dist, rt,
          // ent->client->ps.velocity );
        }
      }
      ucmd->forwardmove = 0;
...
void PM_UpdateViewAngles(playerState_t *ps, usercmd_t *cmd, gentity_t *gent) {
  short temp;
  float pitchMin = -75, pitchMax = 75, yawMin = 0,
        yawMax = 0; // just to shut up warnings
  int i;
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (i == YAW && lockedYaw) {
      ...
      if (AngleSubtract(ps->viewangles[i], gent->client->renderInfo.lockYaw) >
          yawMax) {
        ps->viewangles[i] = yawMax;
      } else if (AngleSubtract(ps->viewangles[i],
                               gent->client->renderInfo.lockYaw) < yawMin) {
  ...
  if ((!cg.renderingThirdPerson || cg.zoomMode) &&
      (cmd->buttons & BUTTON_USE) && cmd->rightmove != 0 && !cmd->forwardmove &&
      cmd->upmove <= 0) { // Only lean if holding use button, strafing and not
    ...
    if (gent) {
      ...
      gi.trace(&trace, start, tmins, tmaxs, end, gent->s.number,
               MASK_PLAYERSOLID, G2_NOCOLLIDE, 0);
