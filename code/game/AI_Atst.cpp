...
void G_ATSTCheckPain(gentity_t *self, gentity_t *other, const vec3_t point,
  ...
  if ((hitLoc == HL_ARM_LT) &&
    ...
    if (self->locationDamage[hitLoc] >= LEFT_ARM_HEALTH) // Blow it up?
    {
      ...
      if (newBolt != -1) {
        ...
        G_PlayEffect(G_EffectIndex("blaster/smoke_bolton"), self->playerModel,
                     newBolt, self->s.number, point);
      }

      gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel],
                               "head_light_blaster_cann", TURN_OFF);
    }
                                       ...
                                       RIGHT_ARM_HEALTH)) // Blow it up?
  {
    if (self->locationDamage[hitLoc] >= RIGHT_ARM_HEALTH) {
      ...
      gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel],
                               "head_concussion_charger", TURN_OFF);
    }
  }
}
/*
...
-------------------------
*/
void NPC_ATST_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
...
void ATST_Attack(void) {
  ...
  case DIST_LONG:
    ...
    blasterTest = gi.G2API_GetSurfaceRenderStatus(
        &NPC->ghoul2[NPC->playerModel], "head_light_blaster_cann");
    chargerTest = gi.G2API_GetSurfaceRenderStatus(
        &NPC->ghoul2[NPC->playerModel], "head_concussion_charger");
