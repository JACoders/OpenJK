...
void Boba_DustFallNear(const vec3_t origin, int dustcount) {
	...
	for (int i = 0; i < dustcount; i++) {
		...
		if (!testTrace.startsolid && !testTrace.allsolid && testTrace.fraction > 0.1f && testTrace.fraction < 0.9f) {
...
void Boba_DoSniper(gentity_t *self) {
	if (TIMER_Done(NPC, "PickNewSniperPoint")) {
		...
		int SniperPoint =
			NPC_FindCombatPoint(NPC->currentOrigin, 0, NPC->currentOrigin, CP_SNIPE | CP_CLEAR | CP_HAS_ROUTE | CP_TRYFAR | CP_HORZ_DIST_COLL, 0, -1);
		if (SniperPoint != -1) {
	...
	//--------------------------
	if (NPCInfo->aiFlags & NPCAI_BLOCKED && NPC->client->moveType != MT_FLYSWIM && ((level.time - NPCInfo->blockedDebounceTime) > 3000)) {
...
void Boba_Update() {
	...
	if (NPC->enemy) {
		if (!(NPC->svFlags & SVF_NOCLIENT)) {
			...
			if (!testTrace.startsolid && !testTrace.allsolid && testTrace.entityNum == NPC->enemy->s.number) {
	...
	//---------------------------------------------
	if (NPC->client->ps.groundEntityNum == ENTITYNUM_NONE && NPC->client->ps.forceJumpZStart && !Q_irand(0, 10)) { // take off
		Boba_FlyStart(NPC);
	}
...
bool Boba_Flee() {
	...
	bool IsOnAPath = !!NPC_MoveToGoal(qtrue);
	if (!ReachedEscapePoint && NPCInfo->aiFlags & NPCAI_BLOCKED && NPC->client->moveType != MT_FLYSWIM &&
