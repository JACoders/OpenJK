...
void NPC_Seeker_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
  ...
  if (!(self->svFlags &
        SVF_CUSTOM_GRAVITY)) { // void G_Damage( gentity_t *targ, gentity_t
                               // *inflictor, gentity_t *attacker, vec3_t dir,
...
void Seeker_Strafe(void) {
  ...
  if (Q_flrand(0.0f, 1.0f) > 0.7f || !NPC->enemy || !NPC->enemy->client) {
    ...
    // Close enough
    if (tr.fraction > 0.9f) {
      VectorMA(NPC->client->ps.velocity, SEEKER_STRAFE_VEL * side, right,
