...
#define WINDOW_PLAYERCOLOR                                                     \
  0x01000000 // hack the forecolor to match ui_char_color_*
...
enum { SSF_JPEG = 0, SSF_TGA, SSF_PNG };
...
typedef struct textScrollDef_s {
  ...
  //
  int iLineCount;
  const char *pLines[MAX_TEXTSCROLL_LINES]; // can contain NULL ptrs that you
                                            // should skip over during paint.
...
typedef struct itemDef_s {
  ...
  int iMenuFont;     // FONT_SMALL,FONT_MEDIUM,FONT_LARGE	// changed from 'font'
                     // so I could see what didn't compile, and differentiate
                     // between font handles returned from RegisterFont -ste
  qboolean disabled; // Does this item ignore mouse and keyboard focus
  int invertYesNo;
  int xoffset;

  qboolean disabledHidden; // hide the item when 'disabled' is true (for generic
                           // image items)
} itemDef_t;
...
typedef struct displayContextDef_s {
  ...
  int (*Font_StrLenPixels)(const char *text, const int iFontIndex,
                           const float scale);
  int (*Font_StrLenChars)(const char *text);
  int (*Font_HeightPixels)(const int iFontIndex, const float scale);
  void (*Font_DrawString)(int ox, int oy, const char *text, const float *rgba,
  ...
  void (*ownerDrawItem)(float x, float y, float w, float h, float text_x,
                        ...
                        qhandle_t shader, int textStyle, int iMenuFont);
  ...
  void (*drawTextWithCursor)(float x, float y, float scale, vec4_t color,
                             ...
                             int limit, int style, int iFontIndex);
  ...
  const char *(*feederItemText)(float feederID, int index, int column,
                                qhandle_t *handle1, qhandle_t *handle2,
                                qhandle_t *handle3);
  ...
  struct {
    float (*Font_StrLenPixels)(const char *text, const int iFontIndex,
                               const float scale);
  } ext;
} displayContextDef_t;
