...
void cgi_S_StartSound(vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfx) { Q_syscall(CG_S_STARTSOUND, origin, entityNum, entchannel, sfx); }
...
qboolean cgi_Language_UsesSpaces(void) { return (qboolean)Q_syscall(CG_LANGUAGE_USESSPACES); }
...
void cgi_R_DrawScreenShot(float x, float y, float w, float h) { Q_syscall(CG_R_DRAWSCREENSHOT, PASSFLOAT(x), PASSFLOAT(y), PASSFLOAT(w), PASSFLOAT(h)); }
...
qboolean cgi_GetSnapshot(int snapshotNumber, snapshot_t *snapshot) { return (qboolean)Q_syscall(CG_GETSNAPSHOT, snapshotNumber, snapshot); }

qboolean cgi_GetServerCommand(int serverCommandNumber) { return (qboolean)Q_syscall(CG_GETSERVERCOMMAND, serverCommandNumber); }

int cgi_GetCurrentCmdNumber(void) { return Q_syscall(CG_GETCURRENTCMDNUMBER); }

qboolean cgi_GetUserCmd(int cmdNumber, usercmd_t *ucmd) { return (qboolean)Q_syscall(CG_GETUSERCMD, cmdNumber, ucmd); }
...
int cgi_UI_GetMenuInfo(char *menuFile, int *x, int *y) { return (int)Q_syscall(CG_UI_GETMENUINFO, menuFile, x, y); }
...
int cgi_SP_Register(const char *text, qboolean persist) { return Q_syscall(CG_SP_REGISTER, text, persist); }
...
int cgi_SP_GetStringText(int ID, char *buffer, int bufferLength) { return Q_syscall(CG_SP_GETSTRINGTEXT, ID, buffer, bufferLength); }
