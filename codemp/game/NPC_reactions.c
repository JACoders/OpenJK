...
void NPC_Pain(gentity_t *self, gentity_t *attacker, int damage) {
		...
		if (other != self->enemy && self != other->enemy) { // we weren't already enemies
			...
			{										  // dammit, stop that!
				if (self->NPC->charmedTime) {		  // mindtricked
					return;
				} else if (self->NPC->ffireCount < 3 + ((2 - g_npcspskill.integer) * 2)) { // not mad enough yet
	...
	if (NPCS.NPCInfo->ignorePain == qfalse) {
		...
		if (NPCS.NPC->enemy != other && NPCS.NPC != other) { // not already mad at them
			...
			if (self->client->enemyTeam) {									// See if we bumped into an enemy
				if (other->client->playerTeam == self->client->enemyTeam) { // bumped into an enemy
					if (NPCS.NPCInfo->behaviorState != BS_HUNT_AND_KILL &&
						!NPCS.NPCInfo->tempBehavior) { // MCG - Begin: checking specific BS mode here, this is bad, a HACK
						// FIXME: not medics?
...
void NPC_Use(gentity_t *self, gentity_t *other, gentity_t *activator) {
	...
	if (self->client && self->NPC) {
		...
		*/
		...
		//			NPC_TakePatient( activator );
		//		}
		else if (activator && !self->enemy && activator->s.number >= 0 && activator->s.number < MAX_CLIENTS && !(self->NPC->scriptFlags & SCF_NO_RESPONSE))
