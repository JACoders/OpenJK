...
qboolean PM_ClientImpact(int otherEntityNum, qboolean damageSelf) {
	...
	if ((VectorLength(pm->ps->velocity) * (pm->gent->mass / 10)) >= 100 &&
		pm->ps->lastOnGround + 100 < level.time) // was 300 ||(other->material>=MAT_GLASS&&pm->gent->lastImpact+100<=level.time))
	{
	...
	if (!traceEnt || !(traceEnt->contents & pm->tracemask)) { // it's dead or not in my way anymore
		return qtrue;
	}
...
static float PM_CmdScale(usercmd_t *cmd) {
	...
	total = sqrt((double)((cmd->forwardmove * cmd->forwardmove) + (cmd->rightmove * cmd->rightmove) + (cmd->upmove * cmd->upmove)));
...
qboolean PM_ForceJumpingUp(gentity_t *gent) {
	...
	if (gent->NPC) {																	   // this is ONLY for the player
		if (player && player->client && player->client->ps.viewEntity == gent->s.number) { // okay to jump if an NPC controlled by the player
		} else {
			return qfalse;
		}
	}

	if (!(gent->client->ps.forcePowersActive & (1 << FP_LEVITATION)) && gent->client->ps.forceJumpCharge) { // already jumped and let go
		return qfalse;
	}
	...
	if (!gent->s.number && in_camera) { // player can't use force powers in cinematic
		return qfalse;
	}
	if (gent->client->ps.groundEntityNum == ENTITYNUM_NONE // in air
		&& (/*(gent->client->ps.waterHeightLevel==WHL_SHOULDERS&&gent->client->usercmd.upmove>0) ||*/ (
			   (gent->client->ps.pm_flags & PMF_JUMPING) && gent->client->ps.velocity[2] > 0)) // jumped & going up or at water surface
...
qboolean PM_GentCantJump(gentity_t *gent) { // FIXME: ugh, hacky, set a flag on NPC or something, please...
	if (gent && gent->client &&
		(gent->client->NPC_class == CLASS_ATST || gent->client->NPC_class == CLASS_GONK || gent->client->NPC_class == CLASS_MARK1 ||
		 gent->client->NPC_class == CLASS_MARK2 || gent->client->NPC_class == CLASS_MOUSE || gent->client->NPC_class == CLASS_PROBE ||
		 gent->client->NPC_class == CLASS_PROTOCOL || gent->client->NPC_class == CLASS_R2D2 || gent->client->NPC_class == CLASS_R5D2 ||
		 gent->client->NPC_class == CLASS_SEEKER || gent->client->NPC_class == CLASS_REMOTE || gent->client->NPC_class == CLASS_SENTRY)) {
		return qtrue;
	}
	return qfalse;
}
...
static qboolean PM_CheckJump(void) {
	...
	*/
				...
				*/
				...
				{																		 // can still go up
					...
					if (curHeight > forceJumpHeight[0]) {						 // passed normal jump height  *2?
						if (!(pm->ps->forcePowersActive & (1 << FP_LEVITATION))) // haven't started forcejump yet
						{
							...
							if ((pm->cmd.forwardmove || pm->cmd.rightmove) && // pushing in a dir
								...
								pm->ps->legsAnim != BOTH_FLIP_F && pm->ps->legsAnim != BOTH_FLIP_B && pm->ps->legsAnim != BOTH_FLIP_R &&
								pm->ps->legsAnim != BOTH_FLIP_L && cg.renderingThirdPerson // third person only
							...
							{															   // FIXME: this could end up playing twice if the jump is very long...
								...
								PM_SetAnim(pm, parts, anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
									   ...
									   FORCE_LEVEL_1) { // FIXME: really want to know how far off ground we are, probably...
					...
					pm->ps->velocity[2] = (forceJumpHeight[pm->ps->forcePowerLevel[FP_LEVITATION]] - curHeight) /
										  ...
										  forceJumpStrength[pm->ps->forcePowerLevel[FP_LEVITATION]]; // JUMP_VELOCITY;
					pm->ps->velocity[2] /= 10;
					pm->ps->velocity[2] += JUMP_VELOCITY;
					pm->ps->pm_flags |= PMF_JUMP_HELD;
				} else if (curHeight > forceJumpHeight[0] && curHeight < forceJumpHeight[pm->ps->forcePowerLevel[FP_LEVITATION]] -
	...
	if (pm->ps->gravity <= 0) { // in low grav, you push in the dir you're facing as long as there is something behind you to shove off of
		...
		if (pm->ps->groundEntityNum == ENTITYNUM_NONE) { // need to set some things and return
			// Jumping
	...
	} else if (pm->cmd.upmove > 0 && pm->waterlevel < 2 && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0 &&
			   !(pm->ps->pm_flags & PMF_JUMP_HELD)
			   //&& !PM_InKnockDown( pm->ps )
			   && pm->gent && WP_ForcePowerAvailable(pm->gent, FP_LEVITATION, 0) &&
			   ((pm->ps->clientNum && !PM_ControlledByPlayer()) ||
				((!pm->ps->clientNum || PM_ControlledByPlayer()) && cg.renderingThirdPerson && !cg.zoomMode &&
				 !(pm->gent->flags &
				   FL_LOCK_PLAYER_WEAPONS)))) // yes this locked weapons check also includes force powers, if we need a separate check later I'll make one
	{
		if (pm->gent->NPC && pm->gent->NPC->rank != RANK_CREWMAN &&
			pm->gent->NPC->rank <= RANK_LT_JG) {				// reborn who are not acrobats can't do any of these acrobatics
		} else if (pm->ps->groundEntityNum != ENTITYNUM_NONE) { // on the ground
			// check for left-wall and right-wall special jumps
			int anim = -1;
			float vertPush = 0;
			if (pm->cmd.rightmove > 0 && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1) { // strafing right
			...
			} else if (pm->cmd.rightmove < 0 && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1) { // strafing left
				if (pm->cmd.forwardmove > 0) {															  // wall-run
					vertPush = forceJumpStrength[FORCE_LEVEL_2] / 2.0f;
			...
			} else if (pm->ps->clientNum && !PM_ControlledByPlayer() && pm->cmd.forwardmove > 0 &&
					   pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1) { // run up wall, flip backwards
				if (VectorLengthSquared(pm->ps->velocity) >
					40000 /*200*200*/) { // have to be moving... FIXME: make sure it's opposite the wall... or at least forward?
					vertPush = forceJumpStrength[FORCE_LEVEL_2] / 2.25f;
			...
			} else if (VectorLengthSquared(pm->ps->velocity) < 256 /*16 squared*/) { // not moving
			...
			if (anim != -1 && PM_HasAnimation(pm->gent, anim)) {
				...
				if (doTrace) {
					...
					if (anim == BOTH_WALL_FLIP_LEFT) { // sigh.. check for bottomless pit to the right
						trace_t trace2;
						vec3_t start;
						...
						if (!trace2.allsolid && !trace2.startsolid) {
							...
							if (!trace2.allsolid && !trace2.startsolid && trace2.fraction >= 1.0f) { // bottomless pit!
								trace.fraction = 1.0f;												 // way to stop it from doing the side-flip
							}
						}
					} else if (anim == BOTH_WALL_FLIP_RIGHT) { // sigh.. check for bottomless pit to the left
						trace_t trace2;
						vec3_t start;
						VectorMA(pm->ps->origin, -128, right, traceto);
						pm->trace(&trace2, pm->ps->origin, mins, maxs, traceto, pm->ps->clientNum, contents, G2_NOCOLLIDE, 0);
					...
					} else if (anim == BOTH_WALL_FLIP_BACK1) { // sigh.. check for bottomless pit to the rear
						trace_t trace2;
						vec3_t start;
				...
				if (!doTrace || (trace.fraction < 1.0f && ((trace.entityNum < ENTITYNUM_WORLD && traceEnt && traceEnt->s.solid != SOLID_BMODEL) ||
														   DotProduct(trace.plane.normal, idealNormal) > 0.7))) { // there is a wall there
					if ((anim != BOTH_WALL_RUN_LEFT && anim != BOTH_WALL_RUN_RIGHT) ||
						trace.plane.normal[2] == 0.0f) { // wall-runs can only run on perfectly flat walls, sorry.
						// move me to side
						...
						if (doTrace && anim != BOTH_WALL_RUN_LEFT && anim != BOTH_WALL_RUN_RIGHT) {
							if (pm->gent && trace.entityNum < ENTITYNUM_WORLD) {
								if (traceEnt && traceEnt->client && traceEnt->health > 0 && traceEnt->takedamage) { // push them away and do pain
									vec3_t oppDir;
						...
						if (anim == BOTH_BUTTERFLY_LEFT || anim == BOTH_BUTTERFLY_RIGHT || anim == BOTH_FJSS_TR_BL || anim == BOTH_FJSS_TL_BR) {
							parts = SETANIM_BOTH;
						...
						} else if (anim == BOTH_WALL_FLIP_LEFT || anim == BOTH_WALL_FLIP_RIGHT ||
								   anim == BOTH_WALL_FLIP_BACK1) { // let us do some more moves after this
			...
			*/
					 ...
					 && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1 && (level.time - pm->ps->lastOnGround) <= 500 // just jumped
			...
			{																				 // run up wall, flip backwards
				...
				if (PM_HasAnimation(pm->gent, BOTH_WALL_FLIP_BACK1)) {
					vec3_t fwd, traceto, mins = {pm->mins[0], pm->mins[1], 0}, maxs = {pm->maxs[0], pm->maxs[1], 24},
										 fwdAngles = {0, pm->ps->viewangles[YAW], 0};
					trace_t trace;
					vec3_t idealNormal;
	...
	if (pm->gent && pm->cmd.upmove > 0 && pm->ps->weapon == WP_SABER && (pm->ps->weaponTime > 0 || pm->cmd.buttons & BUTTON_ATTACK) &&
		((pm->ps->clientNum && !PM_ControlledByPlayer()) ||
		 ((!pm->ps->clientNum || PM_ControlledByPlayer()) && cg.renderingThirdPerson && !cg.zoomMode))) { // okay, we just jumped and we're in an attack
		if (!PM_RollingAnim(pm->ps->legsAnim) && !PM_InKnockDown(pm->ps) && !PM_InDeathAnim() && !PM_PainAnim(pm->ps->torsoAnim) &&
			!PM_FlippingAnim(pm->ps->legsAnim) && !PM_SpinningAnim(pm->ps->legsAnim) && !PM_SaberInSpecialAttack(pm->ps->torsoAnim) &&
			(PM_SaberInTransitionAny(pm->ps->saberMove) || PM_SaberInAttack(pm->ps->saberMove)) &&
			PM_InAnimForSaberMove(pm->ps->torsoAnim, pm->ps->saberMove)) { // not in an anim we shouldn't interrupt
			...
			if (animLength - pm->ps->torsoAnimTimer < 500) { // just started the saberMove
				...
				{																						// using medium attacks
					...
					{																								// flip over-forward down-attack
						if (((!pm->ps->clientNum || PM_ControlledByPlayer()) && pm->cmd.forwardmove >= 0 && !pm->cmd.rightmove) ||
							(pm->gent->NPC && (pm->gent->NPC->rank == RANK_CREWMAN ||
											   pm->gent->NPC->rank >= RANK_LT))) { // only player or acrobat or boss and higher can do this
							...
							if (pm->gent->enemy->health > 0 && pm->gent->enemy->maxs[2] > 12 &&
								...
								InFront(pm->gent->enemy->currentOrigin, pm->gent->currentOrigin, fwdAngles,
										0.3f)) { // enemy must be alive, not low to ground, close and in front
								PM_SetSaberMove(PM_SaberFlipOverAttackMove());
							}
						}
					}
				} else if (pm->ps->saberAnimLevel == FORCE_LEVEL_3 || (pm->gent->client->NPC_class == CLASS_DESANN && !Q_irand(0, 1))) { // using strong attacks
					if (pm->cmd.forwardmove > 0																							 // going forward
						&& pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1														 // can force jump
						&&
						!(pm->gent->flags &
						  FL_LOCK_PLAYER_WEAPONS) // yes this locked weapons check also includes force powers, if we need a separate check later I'll make one
						&& (pm->ps->legsAnim == BOTH_STAND2 || pm->ps->legsAnim == BOTH_SABERFAST_STANCE || pm->ps->legsAnim == BOTH_SABERSLOW_STANCE ||
							level.time - pm->ps->lastStationary <= 500) // standing or just started moving
						&& (pm->ps->groundEntityNum != ENTITYNUM_NONE || (pm->ps->clientNum && !PM_ControlledByPlayer() &&
																		  level.time - pm->ps->lastOnGround <= 500))) // on ground or just jumped if non-player
...
static qboolean PM_CheckWaterJump(void) {
	...
	if (pm->cmd.forwardmove <= 0 && pm->cmd.upmove <= 0) { // they must not want to get out?
		return qfalse;
	}
...
static void PM_FlyVehicleMove(void) {
	...
	// normal slowdown
	if (pm->ps->gravity && pm->ps->velocity[2] < 0 && pm->ps->groundEntityNum == ENTITYNUM_NONE) { // falling
...
static void PM_AirMove(void) {
	...
	{
	*/
	if ((DotProduct(pm->ps->velocity, wishdir)) < 0.0f) { // Encourage deceleration away from the current velocity
	...
	if (!pm->ps->clientNum && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0 && pm->ps->forceJumpZStart &&
...
static void PM_WalkMove(void) {
	...
	if ((pml.groundTrace.surfaceFlags & SURF_SLICK) || pm->ps->pm_flags & PMF_TIME_KNOCKBACK || (pm->ps->pm_flags & PMF_TIME_NOFRICTION)) {
		if (pm->ps->gravity >= 0 && pm->ps->groundEntityNum != ENTITYNUM_NONE && !VectorLengthSquared(pm->ps->velocity) &&
			pml.groundTrace.plane.normal[2] ==
				1.0) { // on ground and not moving and on level ground, no reason to do stupid fucking gravity with the clipvelocity!!!!
		} else {
	...
	if (pm->ps->gravity <= 0) { // need to apply gravity since we're going to float up from ground
		PM_StepSlideMove(1);
	} else {
		PM_StepSlideMove(0);
	}
...
static float PM_DamageForDelta(int delta) {
	float damage = delta;
	if (pm->gent->NPC) {
		if (pm->ps->weapon == WP_SABER) { // FIXME: for now Jedi take no falling damage, but really they should if pushed off?
			damage = 0;
		}
...
static qboolean PM_TryRoll(void) {
	...
	if (PM_SaberInAttack(pm->ps->saberMove) || PM_SaberInSpecialAttack(pm->ps->torsoAnim) || PM_SpinningSaberAnim(pm->ps->legsAnim) ||
		(!pm->ps->clientNum && PM_SaberInStart(pm->ps->saberMove))) { // attacking or spinning (or, if player, starting an attack)
		return qfalse;
	}
	if (!pm->ps->clientNum && (!cg.renderingThirdPerson || cg.zoomMode)) { // player can't do this in 1st person
		return qfalse;
	}
	if (!pm->gent) {
		return qfalse;
	}
	if (pm->ps->clientNum && pm->ps->weapon != WP_SABER && pm->ps->weapon != WP_NONE) { // only jedi
		return qfalse;
	}
	if (pm->gent && pm->gent->NPC) {
		if (pm->gent->NPC->rank != RANK_CREWMAN && pm->gent->NPC->rank < RANK_LT_JG) { // reborn who are not acrobats or fencers can't do any of these
																					   // acrobatics
			return qfalse;
		}
	...
	if (anim != -1) {
		...
		if (pm->ps->clientNum) {
			...
			if (pm->gent && pm->gent->enemy && pm->gent->enemy->health > 0) { // player can always roll in combat
				roll = qtrue;
			} else {
		...
		if (!roll) {
			...
			if (trace.fraction >= 1.0f) { // okay, clear, check for a bottomless drop
				...
				if (pm->ps->clientNum) { // NPCs don't care about rolling into walls, just off ledges
					...
					gentity_t *traceEnt = &g_entities[trace.entityNum];
					if (traceEnt && (traceEnt->svFlags & SVF_GLASS_BRUSH)) { // okay to roll through glass
						roll = qtrue;
					}
				}
			}
...
static void PM_CrashLand(void) {
	...
	if (pm->ps->pm_flags & PMF_TRIGGER_PUSHED) {
		...
		if (pm->gent && pm->gent->NPC &&
			pm->gent->NPC->aiFlags &
				NPCAI_DIE_ON_IMPACT) { // have to do death on impact if we are falling to our death, FIXME: should we avoid any additional damage this func?
			PM_CrashLandDamage(1000);
		}

		if (pm->ps->jumpZStart && (pm->ps->forcePowerLevel[FP_LEVITATION] >= FORCE_LEVEL_1 || !pm->ps->clientNum)) { // we were force-jumping
			if (pm->ps->origin[2] >= pm->ps->jumpZStart) { // we landed at same height or higher than we landed
				...
				if (delta > forceJumpHeight[FORCE_LEVEL_1]) { // will have to use force jump ability to absorb some of it
	...
	if ((pm->ps->pm_flags & PMF_DUCKED) && (level.time - pm->ps->lastOnGround) > 500) { // must be crouched and have been inthe air for half a second minimum
	...
	if (!PM_InDeathAnim()) {
		if (pm->cmd.upmove >= 0 && !PM_InKnockDown(pm->ps) && !PM_InRoll(pm->ps)) { // not crouching
			if (delta > 10 || pm->ps->pm_flags & PMF_BACKWARDS_JUMP || (pm->ps->forcePowersActive & (1 << FP_LEVITATION)) ||
				forceLanding) // EV_FALL_SHORT or jumping back or force-land
			{				  // decide which landing animation to use
				int anim = PM_GetLandingAnim();
				if (anim != -1) {
					if (PM_FlippingAnim(pm->ps->torsoAnim) || PM_SpinningAnim(pm->ps->torsoAnim)) { // interrupt these if we're going to play a land
		...
		if (pm->gent) {
			if ((!(pml.groundTrace.surfaceFlags & SURF_NODAMAGE)) &&
				...
				if (pm->waterlevel < 2) { // don't play fallsplat when impact in the water
					deadFallSound = qtrue;
					G_SoundOnEnt(pm->gent, CHAN_BODY, "sound/player/fallsplat.wav");
				}
				if (gi.VoiceVolume[pm->ps->clientNum] && pm->gent->NPC &&
					(pm->gent->NPC->aiFlags & NPCAI_DIE_ON_IMPACT)) { // I was talking, so cut it off... with a jump sound?
					G_SoundOnEnt(pm->gent, CHAN_VOICE_ATTEN, "*pain100.wav");
				}
			}
		}
		if (pm->ps->legsAnim == BOTH_FALLDEATH1 || pm->ps->legsAnim == BOTH_FALLDEATH1INAIR) { // FIXME: add a little bounce?
	...
	if (pm->gent && pm->gent->client && pm->gent->client->respawnTime >= level.time - 500) { // just spawned in, don't make a noise
		return;
	}
	...
	pm->ps->bobCycle = 0;
	if (pm->gent && pm->gent->client) { // stop the force push effect when you land
...
static void PM_GroundTraceMissed(void) {
	...
	if (pm->ps->clientNum != 0 && pm->gent && pm->gent->NPC && pm->gent->client &&
		pm->gent->client->NPC_class != CLASS_DESANN) // desann never falls to his death
	{
		if (pm->ps->groundEntityNum == ENTITYNUM_NONE) {
			if (pm->ps->stats[STAT_HEALTH] > 0 && !(pm->gent->NPC->aiFlags & NPCAI_DIE_ON_IMPACT) && !(pm->gent->NPC->scriptFlags & SCF_NO_FALLTODEATH)) {
				if ((level.time - pm->gent->client->respawnTime > 2000) // been in the world for at least 2 seconds
					&& (!pm->gent->NPC->timeOfDeath || level.time - pm->gent->NPC->timeOfDeath < 1000) &&
					pm->gent->e_ThinkFunc !=
						thinkF_NPC_RemoveBody // Have to do this now because timeOfDeath is used by thinkF_NPC_RemoveBody to debounce removal checks
					...
					if (!FlyingCreature(pm->gent) && g_gravity->value > 0) {
						if (!(pm->gent->flags & FL_UNDYING) && !(pm->gent->flags & FL_GODMODE)) {
							if (!(pm->ps->eFlags & EF_FORCE_GRIPPED) && !(pm->ps->pm_flags & PMF_TRIGGER_PUSHED)) {
								if (!pm->ps->forceJumpZStart ||
									pm->ps->forceJumpZStart >
										pm->ps->origin[2]) // && fabs(pm->ps->velocity[0])<10 && fabs(pm->ps->velocity[1])<10 && pm->ps->velocity[2]<0)//either
														   // not force-jumping or force-jumped and now fell below original jump start height
								{
									...
									{
										if (trace.fraction == 1.0) {											 // didn't hit, we're probably going to die
											if (pm->ps->velocity[2] < 0 && pm->ps->origin[2] - point[2] > 256) { // going down, into a bottomless pit,
																												 // apparently
												PM_FallToDeath();
												cliff_fall = qtrue;
											}
										} else if (trace.entityNum < ENTITYNUM_NONE &&
												   pm->ps->weapon != WP_SABER) { // Jedi don't scream and die if they're heading for a hard impact
											gentity_t *traceEnt = &g_entities[trace.entityNum];
											if (trace.entityNum == ENTITYNUM_WORLD || (traceEnt && traceEnt->bmodel)) { // hit architecture, find impact force
												float dmg;
												...
												if (trace.plane.normal[2] > 0.5) { // use falling damage
													...
													// guestimate
													if (pm->gent->client && pm->gent->client->ps.forceJumpZStart) { // we were force-jumping
														if (pm->gent->currentOrigin[2] >=
															pm->gent->client->ps.forceJumpZStart) { // we landed at same height or higher than we landed
															dmg = 0;
									...
									//FIXME: we should either do a better check (predict using actual velocity) or we should wait until they've been over a
									bottomless pit for a certain amount of time... VectorCopy( pm->ps->origin, start ); if ( pm->ps->forceJumpZStart < start[2]
									)
	...
	if (!cliff_fall) {
		if ((pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT || pm->ps->legsAnim == BOTH_WALL_RUN_LEFT || pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT_STOP ||
			 pm->ps->legsAnim == BOTH_WALL_RUN_LEFT_STOP || pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT_FLIP || pm->ps->legsAnim == BOTH_WALL_RUN_LEFT_FLIP ||
			 pm->ps->legsAnim == BOTH_WALL_FLIP_RIGHT || pm->ps->legsAnim == BOTH_WALL_FLIP_LEFT || pm->ps->legsAnim == BOTH_CEILING_DROP) &&
			...
			if (pm->ps->groundEntityNum != ENTITYNUM_NONE) {
				...
				pm->trace(&trace, pm->ps->origin, pm->mins, pm->maxs, point, pm->ps->clientNum, pm->tracemask, G2_NOCOLLIDE, 0);
				if (trace.fraction == 1.0) { // FIXME: if velocity[2] < 0 and didn't jump, use some falling anim
...
static void PM_GroundTrace(void) {
	...
	if (pm->ps->eFlags & EF_LOCKED_TO_WEAPON) {
		...
		*/
		return;
			   ...
			   (pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT || pm->ps->legsAnim == BOTH_WALL_RUN_LEFT)) { // wall-running forces you to be in the air
		pml.groundPlane = qfalse;
		pml.walking = qfalse;
		pm->ps->groundEntityNum = ENTITYNUM_NONE;
		return;
	...
	if (((pm->ps->velocity[2] > 0 && (pm->ps->pm_flags & PMF_TIME_KNOCKBACK)) || pm->ps->velocity[2] > 100) &&
		DotProduct(pm->ps->velocity, trace.plane.normal) > 10) { // either thrown off ground (PMF_TIME_KNOCKBACK) or going off the ground at a large velocity
		if (pm->debugLevel) {
	...
	pm->ps->lastOnGround = level.time;
	if (!pm->ps->clientNum) { // if a player, clear the jumping "flag" so can't double-jump
...
void PM_SetWaterHeight(void) {
	...
	} else if (trace.contents & (CONTENTS_WATER | CONTENTS_SLIME)) { // water is above me
		pm->ps->waterheight = top[2] + 4;
	...
	} else if (distFromEyes < pm->gent->client->standheight - 8) { // at least 8 above origin
...
static void PM_CheckDuck(void) {
	...
	if (PM_InGetUp(pm->ps)) {																		// can't do any kind of crouching when getting up
		if (pm->ps->legsAnim == BOTH_GETUP_CROUCH_B1 || pm->ps->legsAnim == BOTH_GETUP_CROUCH_F1) { // crouched still
	...
	if (PM_InKnockDown(pm->ps)) {			// forced crouch
		if (pm->gent && pm->gent->client) { // interrupted any potential delayed weapon fires
	...
	if (pm->cmd.upmove < 0) { // trying to duck
		...
		if (pm->ps->groundEntityNum == ENTITYNUM_NONE && !PM_SwimmingAnim(pm->ps->legsAnim)) { // Not ducked already and trying to duck in mid-air
		...
		if (pm->ps->pm_flags & PMF_DUCKED) {				 // Was ducking
			if (pm->ps->groundEntityNum == ENTITYNUM_NONE) { // unducking whilst in air will try to drop feet
...
qboolean PM_InKnockDownOnGround(playerState_t *ps) {
	...
	case BOTH_FORCE_GETUP_B5:
	case BOTH_FORCE_GETUP_B6:
		if (PM_AnimLength(g_entities[ps->clientNum].client->clientInfo.animFileIndex, (animNumber_t)ps->legsAnim) - ps->legsAnimTimer <
...
qboolean PM_GettingUpFromKnockDown(float standheight, float crouchheight) {
	int legsAnim = pm->ps->legsAnim;
	if (legsAnim == BOTH_KNOCKDOWN1 || legsAnim == BOTH_KNOCKDOWN2 || legsAnim == BOTH_KNOCKDOWN3 || legsAnim == BOTH_KNOCKDOWN4 ||
		legsAnim == BOTH_KNOCKDOWN5) { // in a knockdown
		if (!pm->ps->legsAnimTimer) {  // done with the knockdown - FIXME: somehow this is allowing an *instant* getup...???
				...
				if (!trace.allsolid) { // stand up
					...
					case BOTH_KNOCKDOWN1:
						if ((pm->ps->clientNum && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0) ||
							(!pm->ps->clientNum && pm->cmd.upmove > 0 && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1)) {
					...
					if (forceGetUp) {
						if (pm->gent && pm->gent->client && pm->gent->client->playerTeam == TEAM_ENEMY && pm->gent->NPC &&
							pm->gent->NPC->blockedSpeechDebounceTime < level.time && !Q_irand(0, 1)) {
...
void PM_ResetAnkleAngles(void) {
	...
	if (pm->gent->footLBone != -1) {
		gi.G2API_SetBoneAnglesIndex(&pm->gent->ghoul2[0], pm->gent->footLBone, vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, NEGATIVE_X, NULL, 0,
									0);
	}
	if (pm->gent->footRBone != -1) {
		gi.G2API_SetBoneAnglesIndex(&pm->gent->ghoul2[0], pm->gent->footRBone, vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, NEGATIVE_X, NULL, 0,
									0);
	}
}
...
void PM_FootSlopeTrace(float *pDiff, float *pInterval) {
...
#if 1
	for (int i = 0; i < 3; i++) {
		if (Q_isnan(pm->gent->client->renderInfo.footLPoint[i]) || Q_isnan(pm->gent->client->renderInfo.footRPoint[i])) {
			if (pDiff != NULL) {
				*pDiff = 0;
			}
			if (pInterval != NULL) {
	...
	// get the feet
	gi.G2API_GetBoltMatrix(pm->gent->ghoul2, pm->gent->playerModel, pm->gent->footLBolt, &boltMatrix, G2Angles, pm->ps->origin,
						   (cg.time ? cg.time : level.time), NULL, pm->gent->s.modelScale);
	gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, pm->gent->client->renderInfo.footLPoint);

	gi.G2API_GetBoltMatrix(pm->gent->ghoul2, pm->gent->playerModel, pm->gent->footRBolt, &boltMatrix, G2Angles, pm->ps->origin,
						   (cg.time ? cg.time : level.time), NULL, pm->gent->s.modelScale);
	...
	*/
	...
	if (pm->gent->client->NPC_class == CLASS_ATST) {
		vec3_t footAngles;
		if (!VectorCompare(footLSlope, vec3_origin)) { // rotate the ATST's left foot pad to match the slope
		...
		if (!VectorCompare(footRSlope, vec3_origin)) { // rotate the ATST's right foot pad to match the slope
...
qboolean PM_AdjustStandAnimForSlope(void) {
	...
	if (pm->gent->client->NPC_class != CLASS_ATST && (!pm->gent || !G_StandardHumanoid(pm->gent->NPC_type))) { // only ATST and player does this
		return qfalse;
	}
	if (!pm->ps->clientNum && (!cg.renderingThirdPerson || cg.zoomMode)) { // first person doesn't do this
		return qfalse;
	}
	if (pm->gent->footLBolt == -1 || pm->gent->footRBolt == -1) { // need these bolts!
		return qfalse;
	}
...
void PM_SwimFloatAnim(void) {
	...
	if (pm->cmd.forwardmove || pm->cmd.rightmove || pm->cmd.upmove) {
		...
		if (legsAnim == BOTH_SWIMFORWARD) { // I was swimming
			...
			if (!(pm->ps->pm_flags & PMF_DUCKED) && pm->cmd.upmove >= 0) { // not crouching
				PM_SetAnim(pm, SETANIM_LEGS, BOTH_SWIM_IDLE1, SETANIM_FLAG_NORMAL);
			}
		}
	}
...
static void PM_Footsteps(void) {
	...
	if (PM_SpinningSaberAnim(pm->ps->legsAnim) && pm->ps->legsAnimTimer) { // spinning
		return;
	}
	if (PM_InKnockDown(pm->ps) || PM_InRoll(pm->ps)) { // in knockdown
		return;
	...
	if (pm->ps->legsAnim == BOTH_FLIP_F || pm->ps->legsAnim == BOTH_FLIP_B || pm->ps->legsAnim == BOTH_FLIP_L || pm->ps->legsAnim == BOTH_FLIP_R) {
		flipping = qtrue;
	}

	if (pm->ps->groundEntityNum == ENTITYNUM_NONE || (pm->watertype & CONTENTS_LADDER) ||
		pm->ps->waterHeightLevel >= WHL_TORSO) { // in air or submerged in water or in ladder
		...
		if (pm->waterlevel > 0) {
			if (pm->watertype & CONTENTS_LADDER) { // FIXME: check for watertype, save waterlevel for whether to play
				...
				return;
			} else if (pm->ps->waterHeightLevel >= WHL_TORSO && (!pm->ps->clientNum || pm->ps->weapon == WP_SABER || pm->ps->weapon == WP_NONE ||
																 pm->ps->weapon == WP_MELEE)) // pm->waterlevel > 1 )	//in deep water
			{
	...
	if (PM_SwimmingAnim(pm->ps->legsAnim) && pm->waterlevel < 2) { // legs are in swim anim, and not swimming, be sure to override it
	...
	if (!pm->cmd.forwardmove && !pm->cmd.rightmove) {
		if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_ATST) {
			...
			if (pm->ps->legsAnimTimer && PM_LandingAnim(pm->ps->legsAnim)) { // still in a landing anim, let it play
				return;
			}
			...
			if (pm->ps->saberInFlight) { // guiding saber
				...
				{																		   //
					if (&g_entities[pm->ps->saberEntityNum] != NULL &&
						g_entities[pm->ps->saberEntityNum].s.pos.trType == TR_STATIONARY) { // fell to the ground and we're not trying to pull it back
						saberInAir = qfalse;
					}
				}
			}
			if (pm->gent && pm->gent->client &&
				pm->gent->client->NPC_class == CLASS_GALAKMECH) { // NOTE: stand1 is with the helmet retracted, stand1to2 is the helmet going into place
			...
			{																					// Squadmates use BOTH_STAND3
				...
				if (oldAnim != BOTH_GUARD_LOOKAROUND1 && oldAnim != BOTH_GUARD_IDLE1 && oldAnim != BOTH_STAND3IDLE1 && oldAnim != BOTH_STAND2TO4 &&
					oldAnim != BOTH_STAND4TO2 && oldAnim != BOTH_STAND4) { // Don't auto-override the guard idles
					if (!PM_AdjustStandAnimForSlope()) {
	...
	if ((PM_InSaberAnim(pm->ps->legsAnim) && !PM_SpinningSaberAnim(pm->ps->legsAnim)) || pm->ps->legsAnim == BOTH_STAND1 ||
		...
		pm->ps->legsAnim == BOTH_BUTTON_RELEASE || PM_LandingAnim(pm->ps->legsAnim) || PM_PainAnim(pm->ps->legsAnim) ||
		PM_ForceAnim(pm->ps->legsAnim)) { // legs are in a saber anim, and not spinning, be sure to override it
		setAnimFlags |= SETANIM_FLAG_OVERRIDE;
	}

	if ((pm->ps->eFlags & EF_IN_ATST))													   // does this catch NPCs, too?
	{																					   // atst
		if (pm->ps->legsAnim == BOTH_TURN_LEFT1 || pm->ps->legsAnim == BOTH_TURN_RIGHT1) { // moving overrides turning
			setAnimFlags |= SETANIM_FLAG_OVERRIDE;
		}
	}
	...
	if (pm->ps->pm_flags & PMF_DUCKED) {
		...
		if (!PM_InOnGroundAnim(pm->ps)) {
			...
			if (PM_RunningAnim(pm->ps->legsAnim) || pm->ps->legsAnim == BOTH_FORCEHEAL_START) { // roll!
				rolled = PM_TryRoll();
			}
			if (!rolled) {
				if (pm->ps->pm_flags & PMF_BACKWARDS_RUN) {
			...
			if (pm->ps->weapon == WP_SABER && pm->ps->saberActive) {
				...
				if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_ATST) {
					if (pm->ps->legsAnim != BOTH_RUN1) {
						if (pm->ps->legsAnim != BOTH_RUN1START) { // Hmm, he should really start slow and have to accelerate... also need to do this for
																  // stopping
...
static void PM_WaterEvents(void) { // FIXME?
	...
	//
	if (pml.previous_waterlevel == 3 && pm->waterlevel != 3) {
		if (!pm->gent || !pm->gent->client || pm->gent->client->airOutTime < level.time + 2000) { // only do this if we were drowning or about to start drowning
...
static void PM_BeginWeaponChange(int weapon) {
	...
	if (pm->gent && pm->gent->client && pm->gent->client->pers.enterTime >= level.time - 500) { // just entered map
		if (weapon == WP_NONE && pm->ps->weapon != weapon) {									// don't switch to weapon none if just entered map
			return;
		}
	}
	...
	if (cg.time > 0) { // this way we don't get that annoying change weapon sound every time a map starts
...
static void PM_FinishWeaponChange(void) {
	...
	if (pm->gent && pm->gent->client && pm->gent->client->pers.enterTime >= level.time - 500) { // just entered map
		if (pm->cmd.weapon == WP_NONE && pm->ps->weapon != pm->cmd.weapon) {					// don't switch to weapon none if just entered map
			return;
		}
	}
	...
	if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_ATST) { // do nothing
		...
		if (!pm->ps->saberInFlight) { // if it's not in flight or lying around, turn it on!
...
void PM_SetSaberMove(short newMove) {
	...
	if (newMove == LS_READY || newMove == LS_A_FLIP_STAB || newMove == LS_A_FLIP_SLASH) { // finished with a kata (or in a special move) reset attack counter
		pm->ps->saberAttackChainCount = 0;
	} else if (PM_SaberInAttack(newMove)) { // continuing with a kata, increment attack counter
	...
	} else if (pm->ps->saberAnimLevel > FORCE_LEVEL_1 && !PM_SaberInIdle(newMove) && !PM_SaberInParry(newMove) && !PM_SaberInKnockaway(newMove) &&
			   !PM_SaberInBrokenParry(newMove) && !PM_SaberInReflect(newMove) &&
			   !PM_SaberInSpecial(newMove)) { // readies, parries and reflections have only 1 level
	...
	if (anim == BOTH_STAND2 || anim == BOTH_SABERFAST_STANCE || anim == BOTH_SABERSLOW_STANCE) {
	...
	if (newMove == LS_A_LUNGE || newMove == LS_A_JUMP_T__B_ || newMove == LS_A_BACKSTAB || newMove == LS_A_BACK || newMove == LS_A_BACK_CR ||
		newMove == LS_A_FLIP_STAB || newMove == LS_A_FLIP_SLASH) {
		parts = SETANIM_BOTH;
	} else if (PM_SpinningSaberAnim(anim)) { // spins must be played on entire body
		parts = SETANIM_BOTH;
	} else if ((!pm->cmd.forwardmove && !pm->cmd.rightmove && !pm->cmd.upmove)) { // not trying to run, duck or jump
		if (!PM_FlippingAnim(pm->ps->legsAnim) && !PM_InRoll(pm->ps) && !PM_InKnockDown(pm->ps) && !PM_JumpingAnim(pm->ps->legsAnim) &&
			!PM_PainAnim(pm->ps->legsAnim) && !PM_InSpecialJump(pm->ps->legsAnim) && !PM_InSlopeAnim(pm->ps->legsAnim) &&
	...
	if (pm->ps->torsoAnim == anim) { // successfully changed anims
		...
		if (pm->gent && pm->ps->saberLength > 1) {
			if (PM_SaberInAttack(newMove) || PM_SaberInSpecialAttack(anim)) { // playing an attack
		...
		if (pm->ps->clientNum == 0 || PM_ControlledByPlayer()) {
			if (pm->ps->saberBlocked >= BLOCKED_UPPER_RIGHT_PROJ && pm->ps->saberBlocked <= BLOCKED_TOP_PROJ && newMove >= LS_REFLECT_UP &&
				newMove <= LS_REFLECT_LL) { // don't clear it when blocking projectiles
			} else {
				pm->ps->saberBlocked = BLOCKED_NONE;
			}
		} else if (pm->ps->saberBlocked <= BLOCKED_ATK_BOUNCE || !pm->ps->saberActive ||
				   (newMove < LS_PARRY_UR || newMove > LS_REFLECT_LL)) { // NPCs only clear blocked if not blocking?
			pm->ps->saberBlocked = BLOCKED_NONE;
		}
		...
		if (pm->gent && pm->gent->client) {
			if (saberMoveData[newMove].trailLength > 0) {
				...
				pm->gent->client->saberTrail.duration =
					saberMoveData[newMove].trailLength; // saber trail lasts for 75ms...feel free to change this if you want it longer or shorter
			} else {
...
int PM_NPCSaberAttackFromQuad(int quad) {
	...
	if (autoMove != LS_NONE && PM_SaberInSpecial(autoMove)) { // if have opportunity to do a special attack, do one
		return autoMove;
		...
		case Q_R:
			...
			break;
		case Q_B:
			if ((pm->gent && pm->gent->NPC && pm->gent->NPC->rank >= RANK_LT_JG)) { // fencers and above can do bottom-up attack
				if (Q_irand(0, pm->gent->NPC->rank) >= RANK_LT_JG) {				// but not overly likely
					newmove = LS_A_LUNGE;
				}
			}
			break;
...
void PM_SetAnimFrame(gentity_t *gent, int frame, qboolean torso, qboolean legs) {
	...
	if (legs && gent->rootBone != -1) {
		gi.G2API_SetBoneAnimIndex(&gent->ghoul2[gent->playerModel], gent->rootBone, frame, frame + 1, BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, 1,
...
int PM_SaberLockLoseAnim(gentity_t *genemy, saberLockResult_t result) {
	...
	switch (genemy->client->ps.torsoAnim) {
		/*
			default:
		#ifndef FINAL_BUILD
				Com_Printf( S_COLOR_RED"ERROR-PM_SaberLockBreak: %s not in saberlock anim, anim = (%d)%s\n", genemy->NPC_type, genemy->client->ps.torsoAnim,
		animTable[genemy->client->ps.torsoAnim].name ); #endif
		*/
	case BOTH_BF2LOCK:
		if (result == LOCK_DRAW) {
...
void PM_SaberLockBreak(gentity_t *gent, gentity_t *genemy, saberLockResult_t result, int victoryStrength) {
	...
	if (result == LOCK_STALEMATE) { // no-one won
		...
		if (result == LOCK_VICTORY && winAnim != BOTH_CCWCIRCLEBREAK) { // one person won
			if (Q_irand(FORCE_LEVEL_1, FORCE_LEVEL_2) < pm->ps->forcePowerLevel[FP_SABER_OFFENSE]) {
				if ((!genemy->s.number && genemy->health <= 25) // player low on health
					|| (genemy->s.number && genemy->client->NPC_class != CLASS_LUKE && genemy->client->NPC_class != CLASS_TAVION &&
						genemy->client->NPC_class != CLASS_DESANN) // any NPC that's not a boss character
				...
				{												   // possibly knock saber out of hand OR cut hand off!
					...
					if (Q_irand(0, 25) < victoryStrength && ((!genemy->s.number && genemy->health <= 10) || genemy->s.number)) {
						...
						G_Damage(genemy, gent, gent, throwDir, genemy->client->renderInfo.handRPoint, genemy->health + 10,
								 DAMAGE_NO_PROTECTION | DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK | DAMAGE_NO_HIT_LOC, MOD_SABER, HL_NONE);
...
qboolean PM_SaberLocked(void) {
	if (pm->ps->saberLockEnemy == ENTITYNUM_NONE) {
		if (pm->ps->torsoAnim == BOTH_BF2LOCK || pm->ps->torsoAnim == BOTH_BF1LOCK || pm->ps->torsoAnim == BOTH_CWCIRCLELOCK ||
	...
	if ((pm->ps->torsoAnim == BOTH_BF2LOCK || pm->ps->torsoAnim == BOTH_BF1LOCK || pm->ps->torsoAnim == BOTH_CWCIRCLELOCK ||
		...
		(genemy->client->ps.torsoAnim == BOTH_BF2LOCK || genemy->client->ps.torsoAnim == BOTH_BF1LOCK || genemy->client->ps.torsoAnim == BOTH_CWCIRCLELOCK ||
		 genemy->client->ps.torsoAnim == BOTH_CCWCIRCLELOCK)) {
		if (pm->ps->saberLockTime <= level.time + 500 && pm->ps->saberLockEnemy != ENTITYNUM_NONE) { // lock just ended
			int strength = G_SaberLockStrength(gent);
			int eStrength = G_SaberLockStrength(genemy);
			if (strength > 1 && eStrength > 1 && !Q_irand(0, fabs((double)strength - eStrength) + 1)) { // both knock each other down!
				PM_SaberLockBreak(gent, genemy, LOCK_DRAW, 0);
			} else { // both "win"
				PM_SaberLockBreak(gent, genemy, LOCK_STALEMATE, 0);
			}
			return qtrue;
		} else if (pm->ps->saberLockTime < level.time) { // done... tie breaker above should have handled this, but...?
			...
			if (!(pm->ps->pm_flags & PMF_ATTACK_HELD)) { // tapping
				...
				if (ValidAnimFileIndex(gent->client->clientInfo.animFileIndex)) {
					...
					qboolean ret =
#endif // _DEBUG
						gi.G2API_GetBoneAnimIndex(&gent->ghoul2[gent->playerModel], gent->lowerLumbarBone, (cg.time ? cg.time : level.time), &currentFrame,
				...
				if (ValidAnimFileIndex(genemy->client->clientInfo.animFileIndex)) {
					...
					gi.G2API_GetBoneAnimIndex( &genemy->ghoul2[genemy->playerModel], genemy->lowerLumbarBone, (cg.time?cg.time:level.time), &currentFrame,
					&junk, &junk, &junk, &junk2, NULL );
					*/

					if (genemy->client->ps.torsoAnim == BOTH_CWCIRCLELOCK || genemy->client->ps.torsoAnim == BOTH_BF1LOCK) {
						if (!Q_irand(0, 2)) {
							switch (Q_irand(0, 3)) {
							case 0:
...
void PM_WeaponLightsaber(void) {
	...
	if (!pm->ps->clientNum && cg.saberAnimLevelPending > FORCE_LEVEL_0 && cg.saberAnimLevelPending != pm->ps->saberAnimLevel) {
		if (!PM_SaberInStart(pm->ps->saberMove) && !PM_SaberInTransition(pm->ps->saberMove) &&
			!PM_SaberInAttack(pm->ps->saberMove)) { // don't allow changes when in the middle of an attack set...(or delay the change until it's done)
	...
	*/
	...
	if (!PM_SaberInBrokenParry(pm->ps->saberMove) && pm->ps->saberBlocked != BLOCKED_PARRY_BROKEN &&
		...
		if (pm->ps->saberInFlight) {												   // guiding saber
			...
			if (pm->ps->weaponTime <= 0 || pm->ps->weaponstate != WEAPON_FIRING) {
				...
				if (saberInAir) {
					if (!PM_ForceAnim(pm->ps->torsoAnim) || pm->ps->torsoAnimTimer < 300) { // don't interrupt a force power anim
	...
	if (pm->gent && pm->gent->client && pm->gent->client->fireDelay > 0) { // FIXME: this is going to fire off one frame before you expect, actually
	...
	if (!pm->ps->clientNum || PM_ControlledByPlayer()) {													// player
		if (pm->ps->saberBlocked >= BLOCKED_UPPER_RIGHT_PROJ && pm->ps->saberBlocked <= BLOCKED_TOP_PROJ) { // blocking a projectile
			if (pm->ps->forcePowerDebounce[FP_SABER_DEFENSE] < level.time) { // block is done or breaking out of it with an attack
				pm->ps->weaponTime = 0;
				pm->ps->saberBlocked = BLOCKED_NONE;
			} else if ((pm->cmd.buttons & BUTTON_ATTACK)) { // block is done or breaking out of it with an attack
				pm->ps->weaponTime = 0;
				pm->ps->saberBlocked = BLOCKED_NONE;
			}
		}
	...
	if (pm->ps->saberBlocked) {
		...
		*/
		...
		if (pm->ps->saberBlocked != BLOCKED_ATK_BOUNCE) {			 // can't attack for twice whatever your skill level's parry debounce time is
		...
		case BLOCKED_PARRY_BROKEN:
			...
			{
				int nextMove;
				if (PM_SaberInBrokenParry(pm->ps->saberBounceMove)) { // already have one...?
		...
		case BLOCKED_ATK_BOUNCE:
			...
			{														// an actual bounce?  Other bounces before this are actually transitions?
				pm->ps->saberBlocked = BLOCKED_NONE;
					   ...
					   !PM_SaberInAttack(pm->ps->saberMove)) { // already in the bounce, go into an attack or transition to ready.. should never get here since
															   // can't be blocked in a bounce!
				int nextMove;
	...
	if (pm->ps->weaponstate == WEAPON_RAISING) { // Just selected the weapon
		...
		if (pm->gent && pm->gent->s.number == 0) {
			...
			if (pm->ps->weapon == WP_SABER && pm->ps->saberInFlight && saberInAir) {	// guiding saber
				if (!PM_ForceAnim(pm->ps->torsoAnim) || pm->ps->torsoAnimTimer < 300) { // don't interrupt a force power anim
					PM_SetAnim(pm, SETANIM_TORSO, BOTH_SABERPULL, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
				}
			} else {
	...
	if (!delayed_fire) {
		...
		// check for fire
			...
			if (curmove >= LS_S_TL2BR && curmove <= LS_S_T2B) { // started a swing, must continue from here
				...
				if (pm->ps->clientNum && !PM_ControlledByPlayer()) { // NPCs must play sequential attack
					// going into another attack...
					// allow endless chaining in level 1 attacks, several in level 2 and only one or a few in level 3
					if (PM_SaberKataDone(LS_NONE, LS_NONE)) { // done with this kata, must return to ready before attack again
		...
		if (pm->ps->weapon == WP_SABER && pm->ps->saberInFlight && saberInAir) {	// guiding saber
				...
				{	   // get attack move from movement command
					...
					if (pm->ps->clientNum && !PM_ControlledByPlayer() &&
						(Q_irand(0, pm->ps->saberAnimLevel - 1) ||
						 (pm->ps->saberAnimLevel == FORCE_LEVEL_1 && pm->gent && pm->gent->NPC && pm->gent->NPC->rank >= RANK_LT_JG &&
					...
					{					   // NPCs use more randomized attacks the more skilled they are
						...
						if ((PM_SaberInBounce(curmove) || PM_SaberInBrokenParry(curmove)) &&
							saberMoveData[newmove].startQuad ==
								saberMoveData[curmove].endQuad) { // this attack would be a repeat of the last (which was blocked), so don't actually use it,
			...
			if (pm->gent && pm->gent->client) {
				//				pm->gent->client->saberTrail.inAction = qtrue;
				//				pm->gent->client->saberTrail.duration = 75; // saber trail lasts for 75ms...feel free to change this if you want it longer or
				//shorter
			}
			...
			if (g_timescale != NULL) {
				if (g_timescale->value < 1.0f) {
					if (!MatrixMode) {																				   // Special test for Matrix Mode (tm)
						if (pm->ps->clientNum == 0 && !player_locked && pm->ps->forcePowersActive & (1 << FP_SPEED)) { // player always fires at normal speed
		...
		*amount *= count;
		...
		if (*amount > pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex]) {
			*amount = pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex];
			   ...
			   pm->cmd.buttons & BUTTON_ALT_ATTACK) // BUTTON_ATTACK will have been mapped to BUTTON_ALT_ATTACK if we are zoomed
	{
...
static void PM_Weapon(void) {
	...
	{																						   // Separate logic for lightsaber, but not for player when zoomed
		PM_WeaponLightsaber();
		if (pm->gent && pm->gent->client && pm->ps->saberActive && pm->ps->saberInFlight) { // FIXME: put saberTrail in playerState
			if (pm->gent->client->ps.saberEntityState == SES_RETURNING) {					// turn off the saber trail
				pm->gent->client->saberTrail.inAction = qfalse;
	...
	if (!delayed_fire) {
		...
		case WP_DISRUPTOR:
			if ((pm->ps->clientNum && pm->gent && pm->gent->NPC && (pm->gent->NPC->scriptFlags & SCF_ALT_FIRE)) ||
				(!pm->ps->clientNum && cg.zoomMode == 2)) { // NPC or player in alt-fire, sniper mode
...
//-------------------------------------------
{
	...
	if (pm->ps->weapon == WP_SABER && (!cg.zoomMode || pm->ps->clientNum)) { // don't let the alt-attack be interpreted as an actual attack command
	...
	if (((pm->cmd.buttons & BUTTON_ATTACK || pm->cmd.buttons & BUTTON_ALT_ATTACK) && (amount >= 0 || pm->ps->weapon == WP_SABER))) {
		if (pm->cmd.buttons & BUTTON_ALT_ATTACK) {
			pm->ps->eFlags |= EF_ALT_FIRING;
			if (!pm->ps->clientNum && pm->gent && (pm->ps->eFlags & EF_IN_ATST)) { // switch ATST barrels
...
void Pmove(pmove_t *pmove) {
	...
	PM_SetWaterHeight();
	if (!(pm->watertype & CONTENTS_LADDER)) { // Don't want to remember this for ladders, is only for waterlevel change events (sounds)
	...
	if (!Flying && !(pm->watertype & CONTENTS_LADDER) && pm->ps->pm_type != PM_DEAD) { // NOTE: noclippers shouldn't jump or duck either, no?
		PM_CheckDuck();
	}
