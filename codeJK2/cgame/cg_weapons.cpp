...
void CG_RegisterWeapon(int weaponNum) {
  ...
  case WP_SABER:
    ...
    cgs.effects.forceInvincibility =
        theFxScheduler.RegisterEffect("force/invin");
    ...
    cgs.media.HUDSaberStyleFast =
        cgi_R_RegisterShader("gfx/hud/saber_stylesFast");
    cgs.media.HUDSaberStyleMed =
        cgi_R_RegisterShader("gfx/hud/saber_stylesMed");
    cgs.media.HUDSaberStyleStrong =
        cgi_R_RegisterShader("gfx/hud/saber_stylesStrong");
  ...
  case WP_BOWCASTER:
    ...
    cgs.effects.bowcasterBounceEffect =
        theFxScheduler.RegisterEffect("bowcaster/bounce");
  ...
  case WP_EMPLACED_GUN:
    ...
    cgs.media.emplacedHealthBarShader =
        cgi_R_RegisterShaderNoMip("gfx/hud/atst_health_frame");
    cgs.media.ladyLuckHealthShader =
        cgi_R_RegisterShaderNoMip("gfx/hud/ladyluck_health_frame");
...
void CG_AddViewWeapon(playerState_t *ps) {
  ...
  float cgFov =
      (cg_fovViewmodel.integer) ? cg_fovViewmodel.integer : cg_fov.integer;
  int i;
          ...
          .forcePowerDuration[FP_SPEED]) // cg.renderingThirdPerson &&
  {
    ...
    actualFOV = (cg_fovViewmodel.integer)
                    ? actualFOV + (cg_fovViewmodel.integer - cg_fov.integer)
                    : actualFOV;
  } else {
    actualFOV = (cg.overrides.active & CG_OVERRIDE_FOV) ? cg.overrides.fov
                                                        : cg_fov.value;
  }
  ...
  VectorMA(hand.origin, (cg_gun_y.value + leanOffset), cg.refdef.viewaxis[1],
           hand.origin);
  ...
  //---------
  // OK, we are making an assumption here that if we have the phaser that it is
  // always on....
  ...
  if (cent->gent && cent->gent->client &&
    ...
    //		FX_Saber( org_, axis_[0], cent->gent->client->ps.saberLength, 2.0 +
    //Q_flrand(-1.0f, 1.0f) * 0.2f, cent->gent->client->ps.saberColor );
...
char *weaponDesc[13] = {
    "SABER_DESC",         "BLASTER_PISTOL_DESC",
...
void CG_DrawDataPadWeaponSelect(void) {
  ...
  cgi_SP_GetStringTextString(
      va("INGAME_%s", weaponDesc[cg.DataPadWeaponSelect - 1]), text,
      sizeof(text));
...
void CG_DrawDataPadIconBackground(int backgroundType) {
  ...
  /*	if ((cg.iconDataPadSelectTime+WEAPON_SELECT_TIME)<cg.time)	// Time is
     up for the HUD to display
          {
                  if (cg.iconDataPadHUDActive)		// The time is up, but
     we still need to move the prongs back to their original position
                  {
                          t =  cg.time -
     (cg.iconDataPadSelectTime+WEAPON_SELECT_TIME); cg.iconDataPadHUDPercent =
     t/ 130.0f; cg.iconDataPadHUDPercent = 1 - cg.iconDataPadHUDPercent;
                          ...
                          CG_DrawPic( x2+60, y2+30, 460, -height, background);
     ...
     Bottom half
                  ...
                  cgi_R_SetColor( colorTable[CT_WHITE] );
                  CG_DrawPic( prongLeftX+xAdd, y2-10, 40, 80,
     cgs.media.weaponProngsOff); CG_DrawPic( prongRightX-xAdd, y2-10, -40, 80,
...
void CG_DrawWeaponSelect(void) {
  ...
  if (item && item->classname && item->classname[0]) {
    ...
    if (cgi_SP_GetStringTextString(va("INGAME_%s", item->classname), text,
                                   sizeof(text))) {
      int w = cgi_R_Font_StrLenPixels(text, cgs.media.qhFontSmall, 1.0f);
      int x = (SCREEN_WIDTH - w) / 2;
      cgi_R_Font_DrawString(x, (SCREEN_HEIGHT - 24), text, textColor,
...
qboolean CG_WeaponSelectable(int i, int original, qboolean dpMode) {
  ...
  if (original == WP_SABER &&
      cg.weaponSelectTime + 500 >
          cg.time) { // when sqitch to lightsaber, have to stay there for at
                     // least half a second!
    return qfalse;
  }
...
void CG_NextWeapon_f(void) {
  ...
  */
  ...
  for (i = 0; i <= MAX_PLAYER_WEAPONS; i++) {
    ...
    if (cg.weaponSelect < FIRST_WEAPON ||
  ...
  if (num == WP_SABER) { // lightsaber
    if (!(cg.snap->ps.stats[STAT_WEAPONS] &
      ...
      if (!in_camera) { // player can't activate/deactivate saber when in a
        ...
        if (cg.predicted_player_state.stats[STAT_HEALTH] > 0 &&
            (!cg_entities[0].currentState.saberInFlight ||
          ...
          if (cg_entities[0].currentState.saberActive) {
            ...
            if (cg_entities[0]
                    .currentState.saberInFlight) { // play it on the saber
