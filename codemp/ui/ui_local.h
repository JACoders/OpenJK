...
typedef struct uiInfo_s {
  ...
  qboolean forceConfigSide[MAX_FORCE_CONFIGS]; // true if it's a light side
                                               // config, false if dark side
  int forceConfigDarkIndexBegin;  // mark the index number dark configs start at
  int forceConfigLightIndexBegin; // mark the index number light configs start
                                  // at
