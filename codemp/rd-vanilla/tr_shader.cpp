...
// Hash value (generated using the generateHashValueForText function) for the
// original retail JKA shader for gfx/2d/wedge.
#define RETAIL_ROCKET_WEDGE_SHADER_HASH (1217042)

// Hash value (generated using the generateHashValueForText function) for the
// original retail JKA shader for gfx/menus/radar/arrow_w.
...
const int lightmapsNone[MAXLIGHTMAPS] = {LIGHTMAP_NONE, LIGHTMAP_NONE,
                                         LIGHTMAP_NONE, LIGHTMAP_NONE};

const int lightmaps2d[MAXLIGHTMAPS] = {LIGHTMAP_2D, LIGHTMAP_2D, LIGHTMAP_2D,
                                       LIGHTMAP_2D};

const int lightmapsVertex[MAXLIGHTMAPS] = {
    LIGHTMAP_BY_VERTEX, LIGHTMAP_BY_VERTEX, LIGHTMAP_BY_VERTEX,
    LIGHTMAP_BY_VERTEX};

const int lightmapsFullBright[MAXLIGHTMAPS] = {
    LIGHTMAP_WHITEIMAGE, LIGHTMAP_WHITEIMAGE, LIGHTMAP_WHITEIMAGE,
    LIGHTMAP_WHITEIMAGE};

const byte stylesDefault[MAXLIGHTMAPS] = {LS_NORMAL, LS_LSNONE, LS_LSNONE,
                                          LS_LSNONE};
...
void R_RemapShader(const char *shaderName, const char *newShaderName,
  ...
  if (sh == NULL || sh == tr.defaultShader) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "WARNING: R_RemapShader: shader %s not found\n",
              shaderName);
    return;
  }
...
qboolean ParseVector(const char **text, int count, float *v) {
  ...
  if (strcmp(token, "(")) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "WARNING: missing parenthesis in shader '%s'\n",
              shader.name);
    return qfalse;
  }
...
static void ParseWaveForm(const char **text, waveForm_t *wave) {
  ...
  if (token[0] == 0) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "WARNING: missing waveform parm in shader '%s'\n",
              shader.name);
    return;
  }
...
static void ParseTexMod(const char *_text, shaderStage_t *stage) {
  ...
  if (!Q_stricmp(token, "turb")) {
    ...
    if (token[0] == 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW "WARNING: missing tcMod turb in shader '%s'\n",
                shader.name);
      return;
    }
  ...
  //
    ...
    if (token[0] == 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW "WARNING: missing scale parms in shader '%s'\n",
                shader.name);
      return;
    }
  ...
  //
    ...
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "WARNING: unknown tcMod '%s' in shader '%s'\n",
              token, shader.name);
  }
}
...
//
// NOTE:  This parsing function used to be 12 pages long and very complex.  The
// new version of surfacesprites utilizes optional parameters parsed in
// ParseSurfaceSpriteOptional.
static void ParseSurfaceSprites(const char *_text, shaderStage_t *stage) {
  ...
  if (token[0] == 0) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW
              "WARNING: missing surfaceSprites params in shader '%s'\n",
              shader.name);
    return;
  }
  ...
  if (fadedist < 32) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW
              "WARNING: invalid fadedist (%f < 32) in shader '%s'\n",
...
// ssFXWeather
//
// Optional parameters that will override the defaults set in the surfacesprites
// command above.
//
static void ParseSurfaceSpritesOptional(const char *param, const char *_text,
                                        shaderStage_t *stage) {
  const char *token;
  const char **text = &_text;
  float value;
  ...
  if (!Q_stricmp(param, "ssFademax")) {
    ...
    if (token[0] == 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: missing surfacesprite fademax in shader '%s'\n",
                shader.name);
      return;
    }
    value = atof(token);
    if (value <= stage->ss->fadeDist) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW "WARNING: invalid surfacesprite fademax (%.2f "
                               "<= fadeDist(%.2f)) in shader '%s'\n",
  ...
  if (!Q_stricmp(param, "ssFadescale")) {
    ...
    if (token[0] == 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: missing surfacesprite fadescale in shader '%s'\n",
                shader.name);
      return;
    }
    value = atof(token);
  ...
  if (!Q_stricmp(param, "ssVariance")) {
    ...
    if (token[0] == 0) {
      ri.Printf(
          PRINT_ALL,
          S_COLOR_YELLOW
          "WARNING: missing surfacesprite variance width in shader '%s'\n",
          shader.name);
      return;
    }
    value = atof(token);
    if (value < 0) {
      ri.Printf(
          PRINT_ALL,
          S_COLOR_YELLOW
          "WARNING: invalid surfacesprite variance width in shader '%s'\n",
          shader.name);
      return;
    }
    ...
    if (token[0] == 0) {
      ri.Printf(
          PRINT_ALL,
          S_COLOR_YELLOW
          "WARNING: missing surfacesprite variance height in shader '%s'\n",
          shader.name);
      return;
    }
    value = atof(token);
    if (value < 0) {
      ri.Printf(
          PRINT_ALL,
          S_COLOR_YELLOW
          "WARNING: invalid surfacesprite variance height in shader '%s'\n",
          shader.name);
      return;
    }
  ...
  if (!Q_stricmp(param, "ssHangdown")) {
    if (stage->ss->facing != SURFSPRITE_FACING_NORMAL) {
      ri.Printf(
          PRINT_ALL,
          S_COLOR_YELLOW
          "WARNING: Hangdown facing overrides previous facing in shader '%s'\n",
          shader.name);
      return;
    }
  ...
  if (!Q_stricmp(param, "ssAnyangle")) {
    if (stage->ss->facing != SURFSPRITE_FACING_NORMAL) {
      ri.Printf(
          PRINT_ALL,
          S_COLOR_YELLOW
          "WARNING: Anyangle facing overrides previous facing in shader '%s'\n",
          shader.name);
      return;
    }
  ...
  if (!Q_stricmp(param, "ssFaceup")) {
    if (stage->ss->facing != SURFSPRITE_FACING_NORMAL) {
      ri.Printf(
          PRINT_ALL,
          S_COLOR_YELLOW
          "WARNING: Faceup facing overrides previous facing in shader '%s'\n",
          shader.name);
      return;
    }
  ...
  if (!Q_stricmp(param, "ssWind")) {
    ...
    if (token[0] == 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: missing surfacesprite wind in shader '%s'\n",
                shader.name);
      return;
    }
    value = atof(token);
    if (value < 0.0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: invalid surfacesprite wind in shader '%s'\n",
                shader.name);
      return;
    }
    stage->ss->wind = value;
    if (stage->ss->windIdle <=
        0) { // Also override the windidle, it usually is the same as wind
  ...
  if (!Q_stricmp(param, "ssWindidle")) {
    ...
    if (token[0] == 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: missing surfacesprite windidle in shader '%s'\n",
                shader.name);
      return;
    }
    value = atof(token);
    if (value < 0.0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: invalid surfacesprite windidle in shader '%s'\n",
                shader.name);
      return;
    }
    stage->ss->windIdle = value;
  ...
  if (!Q_stricmp(param, "ssVertskew")) {
    ...
    if (token[0] == 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: missing surfacesprite vertskew in shader '%s'\n",
                shader.name);
      return;
    }
    value = atof(token);
    if (value < 0.0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: invalid surfacesprite vertskew in shader '%s'\n",
                shader.name);
      return;
    }
  ...
  if (!Q_stricmp(param, "ssFXDuration")) {
    ...
    if (token[0] == 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: missing surfacesprite duration in shader '%s'\n",
                shader.name);
      return;
    }
    value = atof(token);
    if (value <= 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: invalid surfacesprite duration in shader '%s'\n",
                shader.name);
      return;
    }
  ...
  if (!Q_stricmp(param, "ssFXGrow")) {
    ...
    if (token[0] == 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: missing surfacesprite grow width in shader '%s'\n",
                shader.name);
      return;
    }
    value = atof(token);
    if (value < 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: invalid surfacesprite grow width in shader '%s'\n",
                shader.name);
      return;
    }
    ...
    if (token[0] == 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: missing surfacesprite grow height in shader '%s'\n",
                shader.name);
      return;
    }
    value = atof(token);
    if (value < 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: invalid surfacesprite grow height in shader '%s'\n",
                shader.name);
      return;
    }
  ...
  if (!Q_stricmp(param, "ssFXAlphaRange")) {
    ...
    if (token[0] == 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: missing surfacesprite fxalpha start in shader '%s'\n",
                shader.name);
      return;
    }
    value = atof(token);
    if (value < 0 || value > 1.0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: invalid surfacesprite fxalpha start in shader '%s'\n",
                shader.name);
      return;
    }
    ...
    if (token[0] == 0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: missing surfacesprite fxalpha end in shader '%s'\n",
                shader.name);
      return;
    }
    value = atof(token);
    if (value < 0 || value > 1.0) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW
                "WARNING: invalid surfacesprite fxalpha end in shader '%s'\n",
                shader.name);
      return;
    }
  ...
  if (!Q_stricmp(param, "ssFXWeather")) {
    if (stage->ss->surfaceSpriteType != SURFSPRITE_EFFECT) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW "WARNING: weather applied to non-effect "
                               "surfacesprite in shader '%s'\n",
                shader.name);
      return;
    }
  ...
  ri.Printf(
      PRINT_ALL,
      S_COLOR_YELLOW
      "WARNING: invalid optional surfacesprite param '%s' in shader '%s'\n",
      param, shader.name);
  return;
}
...
static qboolean ParseStage(shaderStage_t *stage, const char **text) {
  ...
  while (1) {
    ...
    //
      ...
      if (!Q_stricmp(token, "$whiteimage")) {
        ...
        if (shader.lightmapIndex[0] < 0 ||
            shader.lightmapIndex[0] >= tr.numLightmaps) {
#ifndef FINAL_BUILD
          ri.Printf(PRINT_ALL,
                    S_COLOR_RED
                    "Lightmap requested but none available for shader %s\n",
                    shader.name);
#endif
          stage->bundle[0].image = tr.whiteImage;
        } else {
        ...
        stage->bundle[0].image = R_FindImageFile(
            token, (qboolean)!shader.noMipMaps, (qboolean)!shader.noPicMip,
            (qboolean)!shader.noTC, GL_REPEAT);
    ...
    //
      ...
      if (!token[0]) {
        ri.Printf(PRINT_ALL,
                  S_COLOR_YELLOW "WARNING: missing parameter for 'clampmap' "
                                 "keyword in shader '%s'\n",
                  shader.name);
        return qfalse;
      }
...
#define MAX_IMAGE_ANIMATIONS 32
      ...
      if (!token[0]) {
        ri.Printf(
            PRINT_ALL,
            S_COLOR_YELLOW
            "WARNING: missing parameter for '%s' keyword in shader '%s'\n",
      ...
      while (1) {
        ...
        if (num < MAX_IMAGE_ANIMATIONS) {
          images[num] = R_FindImageFile(
              token, (qboolean)!shader.noMipMaps, (qboolean)!shader.noPicMip,
              (qboolean)!shader.noTC, bClamp ? GL_CLAMP : GL_REPEAT);
          if (!images[num]) {
            ri.Printf(
                PRINT_ALL,
                S_COLOR_YELLOW
      ...
      stage->bundle[0].image = (image_t *)Hunk_Alloc(
          stage->bundle[0].numImageAnimations * sizeof(image_t *), h_low);
      memcpy(stage->bundle[0].image, images,
             stage->bundle[0].numImageAnimations * sizeof(image_t *));
    } else if (!Q_stricmp(token, "videoMap")) {
      token = COM_ParseExt(text, qfalse);
      if (!token[0]) {
        ri.Printf(PRINT_ALL,
                  S_COLOR_YELLOW "WARNING: missing parameter for 'videoMap' "
                                 "keyword in shader '%s'\n",
                  shader.name);
        return qfalse;
      }
    ...
    //
      ...
      if (!token[0]) {
        ri.Printf(PRINT_ALL,
                  S_COLOR_YELLOW "WARNING: missing parameter for 'alphaFunc' "
                                 "keyword in shader '%s'\n",
                  shader.name);
        return qfalse;
      }
    ...
    //
      ...
      if (!token[0]) {
        ri.Printf(PRINT_ALL,
                  S_COLOR_YELLOW "WARNING: missing parameter for 'depthfunc' "
                                 "keyword in shader '%s'\n",
                  shader.name);
        return qfalse;
      }
    ...
    //
      ...
      if (!Q_stricmp(token, "wave")) {
        ...
        if (shader.lightmapIndex[0] != LIGHTMAP_NONE) {
          ri.Printf(PRINT_ALL,
                    S_COLOR_RED "ERROR: rgbGen lightingDiffuseEntity used on a "
                                "misc_model! in shader '%s'\n",
                    shader.name);
        }
    ...
    //
      ...
      if (!Q_stricmp(token, "environment")) {
        ...
        stage->bundle[0].tcGenVectors =
            (vec3_t *)Hunk_Alloc(2 * sizeof(vec3_t), h_low);
    ...
    // ssWeather
    //
    else if (!Q_stricmpn(token, "ss",
                         2)) // <--- NOTE ONLY COMPARING FIRST TWO LETTERS
    {
      char buffer[1024] = "";
      char param[128];
      strcpy(param, token);
...
static void ParseDeform(const char **text) {
  ...
  if (token[0] == 0) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "WARNING: missing deform parm in shader '%s'\n",
              shader.name);
    return;
  }
  ...
  shader.deforms[shader.numDeforms] =
      (deformStage_t *)Hunk_Alloc(sizeof(deformStage_t), h_low);
  ...
  if (!Q_stricmp(token, "wave")) {
    ...
    if (atof(token) != 0) {
      ...
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW "WARNING: illegal div value of 0 in "
                               "deformVertexes command for shader '%s'\n",
                shader.name);
    }
...
static void ParseSkyParms(const char **text) {
  ...
  if (strcmp(token, "-")) {
    for (i = 0; i < 6; i++) {
      ...
      shader.sky->outerbox[i] = R_FindImageFile(
          (char *)pathname, qtrue, qtrue, (qboolean)!shader.noTC, GL_CLAMP);
      if (!shader.sky->outerbox[i]) {
        if (i) {
          shader.sky->outerbox[i] =
              shader.sky->outerbox[i - 1]; // not found, so let's use the
                                           // previous image
        } else {
  ...
  if (token[0] == 0) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW
              "WARNING: 'skyParms' missing cloudheight in shader '%s'\n",
              shader.name);
    return;
  }
  ...
  if (strcmp(token, "-")) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW
              "WARNING: in shader '%s' 'skyParms', innerbox is not supported!",
              shader.name);
  }
}
...
static void ParseSort(const char **text) {
  ...
  if (token[0] == 0) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "WARNING: missing sort parameter in shader '%s'\n",
              shader.name);
    return;
  }
...
void ParseMaterial(const char **text) {
  ...
  if (token[0] == 0) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "WARNING: missing material in shader '%s'\n",
              shader.name);
    return;
  }
...
infoParm_t infoParms[] = {
    // Game content Flags
    {"nonsolid", ~CONTENTS_SOLID, SURF_NONE,
     CONTENTS_NONE}, // special hack to clear solid flag
    {"nonopaque", ~CONTENTS_OPAQUE, SURF_NONE,
     CONTENTS_NONE}, // special hack to clear opaque flag
    ...
    {"fog", ~CONTENTS_SOLID, SURF_NONE,
     CONTENTS_FOG}, // carves surfaces entering
    {"shotclip", ~CONTENTS_SOLID, SURF_NONE,
     CONTENTS_SHOTCLIP}, // block shots, but not people
    {"playerclip", ~(CONTENTS_SOLID | CONTENTS_OPAQUE), SURF_NONE,
     CONTENTS_PLAYERCLIP}, // block only the player
    {"monsterclip", ~(CONTENTS_SOLID | CONTENTS_OPAQUE), SURF_NONE,
     CONTENTS_MONSTERCLIP}, //
    {"botclip", ~(CONTENTS_SOLID | CONTENTS_OPAQUE), SURF_NONE,
     CONTENTS_BOTCLIP}, // for bots
    {"trigger", ~(CONTENTS_SOLID | CONTENTS_OPAQUE), SURF_NONE,
     CONTENTS_TRIGGER}, //
    {"nodrop", ~(CONTENTS_SOLID | CONTENTS_OPAQUE), SURF_NONE,
     CONTENTS_NODROP}, // don't drop items or leave bodies (death fog, lava,
                       // etc)
    {"terrain", ~(CONTENTS_SOLID | CONTENTS_OPAQUE), SURF_NONE,
     CONTENTS_TERRAIN}, // use special terrain collsion
    {"ladder", ~(CONTENTS_SOLID | CONTENTS_OPAQUE), SURF_NONE,
     CONTENTS_LADDER}, // climb up in it like water
    {"abseil", ~(CONTENTS_SOLID | CONTENTS_OPAQUE), SURF_NONE,
     CONTENTS_ABSEIL}, // can abseil down this brush
    {"outside", ~(CONTENTS_SOLID | CONTENTS_OPAQUE), SURF_NONE,
     CONTENTS_OUTSIDE}, // volume is considered to be in the outside (i.e. not
                        // indoors)
    {"inside", ~(CONTENTS_SOLID | CONTENTS_OPAQUE), SURF_NONE,
     CONTENTS_INSIDE}, // volume is considered to be inside (i.e. indoors)

    {"detail", CONTENTS_ALL, SURF_NONE,
     CONTENTS_DETAIL}, // don't include in structural bsp
    {"trans", CONTENTS_ALL, SURF_NONE,
     CONTENTS_TRANSLUCENT}, // surface has an alpha component

    /* Game surface flags */
    {"sky", CONTENTS_ALL, SURF_SKY,
     CONTENTS_NONE}, // emit light from an environment map
    {"slick", CONTENTS_ALL, SURF_SLICK, CONTENTS_NONE}, //

    {"nodamage", CONTENTS_ALL, SURF_NODAMAGE, CONTENTS_NONE}, //
    {"noimpact", CONTENTS_ALL, SURF_NOIMPACT,
     CONTENTS_NONE}, // don't make impact explosions or marks
    {"nomarks", CONTENTS_ALL, SURF_NOMARKS,
     CONTENTS_NONE}, // don't make impact marks, but still explode
    {"nodraw", CONTENTS_ALL, SURF_NODRAW,
     CONTENTS_NONE}, // don't generate a drawsurface (or a lightmap)
    {"nosteps", CONTENTS_ALL, SURF_NOSTEPS, CONTENTS_NONE}, //
    {"nodlight", CONTENTS_ALL, SURF_NODLIGHT,
     CONTENTS_NONE}, // don't ever add dynamic lights
    {"metalsteps", CONTENTS_ALL, SURF_METALSTEPS, CONTENTS_NONE}, //
    {"nomiscents", CONTENTS_ALL, SURF_NOMISCENTS,
     CONTENTS_NONE}, // No misc ents on this surface
    {"forcefield", CONTENTS_ALL, SURF_FORCEFIELD, CONTENTS_NONE}, //
    {"forcesight", CONTENTS_ALL, SURF_FORCESIGHT,
     CONTENTS_NONE}, // only visible with force sight
};
...
static qboolean ParseShader(const char **text) {
  ...
  while (1) {
    ...
    if (!token[0]) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW "WARNING: no concluding '}' in shader %s\n",
                shader.name);
      return qfalse;
    }
    ...
    // cull <face>
      ...
      if (token[0] == 0) {
        ri.Printf(PRINT_ALL,
                  S_COLOR_YELLOW "WARNING: missing cull parms in shader '%s'\n",
                  shader.name);
        continue;
      }
...
static shader_t *GeneratePermanentShader(void) {
  ...
  if (tr.numShaders == MAX_SHADERS) {
    // ri.Printf( PRINT_ALL, S_COLOR_YELLOW  "WARNING: GeneratePermanentShader -
    // MAX_SHADERS hit\n");
    ri.Printf(PRINT_ALL,
  ...
  *newShader = shader;
  ...
  size = newShader->numUnfoggedPasses
             ? newShader->numUnfoggedPasses * sizeof(stages[0])
             : sizeof(stages[0]);
    ...
    for (b = 0; b < NUM_TEXTURE_BUNDLES; b++) {
      if (newShader->stages[i].bundle[b].numTexMods) {
        ...
        newShader->stages[i].bundle[b].texMods =
            (texModInfo_t *)Hunk_Alloc(size, h_low);
...
what it is supposed to look like.
  ...
  OUTPUT:  Number of stages after the collapse (in the case of surfacesprites
this isn't one).
=================
*/
...
static shader_t *FinishShader(void) {
  ...
  if (lmStage < MAX_SHADER_STAGES) {
    if (shader.lightmapIndex[0] == LIGHTMAP_BY_VERTEX) {
      ...
      // change anything that was moved down (or the *white if LM is first) to
      // use vertex color
    ...
    if (numStyles > 0) {
      ...
      for (i = 0; i < numStyles; i++) {
        ...
        if (shader.lightmapIndex[i + 1] == LIGHTMAP_BY_VERTEX) {
          ...
          Com_Error(ERR_DROP,
                    "FinishShader: light style with no light map or vertex "
                    "color for shader %s",
                    shader.name);
        } else {
          stages[lmStage + i + 1].bundle[0].image =
              tr.lightmaps[shader.lightmapIndex[i + 1]];
          stages[lmStage + i + 1].bundle[0].tcGen =
              (texCoordGen_t)(TCGEN_LIGHTMAP + i + 1);
        }
        stages[lmStage + i + 1].rgbGen = CGEN_LIGHTMAPSTYLE;
        stages[lmStage + i + 1].stateBits &=
  ...
  for (stage = 0; stage < MAX_SHADER_STAGES;) {
    ...
    if (!pStage->bundle[0].image) {
      ri.Printf(PRINT_ALL,
                S_COLOR_YELLOW "Shader %s has a stage with no image\n",
                shader.name);
    ...
    if ((pStage->stateBits & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) &&
      ...
      if (!shader.sort) {
        ...
        if (pStage->stateBits & GLS_DEPTHMASK_TRUE) {
          ...
          /*
          if (( blendSrcBits == GLS_SRCBLEND_ONE ) && ( blendDstBits ==
          GLS_DSTBLEND_ONE ))
          {
                  // GL_ONE GL_ONE needs to come a bit later
                  shader.sort = SS_BLEND2;
          }
          else if (( blendSrcBits == GLS_SRCBLEND_SRC_ALPHA ) && ( blendDstBits
          == GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA )) { //rww - Pushed SS_BLEND1 up
          to SS_BLEND2, inserting this so that saber glow will render above
          water and things.
            //Unfortunately it still affects other shaders with the same blend
          settings, but it seems more or less alright. shader.sort = SS_BLEND1;
          }
          else
          {
    ...
    if ((pStage->stateBits & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) ==
    ...
    } else if ((pStage->stateBits & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) ==
      ...
      pStage->mGLFogColorOverride = GLFOGOVERRIDE_WHITE;
                   ...
                   0 &&
               pStage->bundle[0].isLightmap && stage < MAX_SHADER_STAGES - 1 &&
               stages[stage + 1]
                   .bundle[0]
                   .isLightmap) { // multiple light map blending
      pStage->mGLFogColorOverride = GLFOGOVERRIDE_WHITE;
    } else if ((pStage->stateBits & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) ==
                   (GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ZERO) &&
               pStage->bundle[0].isLightmap) { // I don't know, it works. -rww
      pStage->mGLFogColorOverride = GLFOGOVERRIDE_WHITE;
    } else if ((pStage->stateBits & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) ==
               (GLS_SRCBLEND_DST_COLOR |
                GLS_DSTBLEND_ZERO)) { // I don't know, it works. -rww
      pStage->mGLFogColorOverride = GLFOGOVERRIDE_BLACK;
    } else if ((pStage->stateBits & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) ==
               (GLS_SRCBLEND_ONE |
                GLS_DSTBLEND_ONE_MINUS_SRC_COLOR)) { // I don't know, it works.
                                                     // -rww
      pStage->mGLFogColorOverride = GLFOGOVERRIDE_BLACK;
    } else {
  ...
  if (stage > 1 && (r_vertexLight->integer && !r_uiFullScreen->integer)) {
    ...
    // rww - since this does bad things, I am commenting it out for now. If you
    // want to attempt a fix, feel free.
    hasLightmapStage = qfalse;
  }
  ...
  if (shader.lightmapIndex[0] >= 0 && !hasLightmapStage) {
    if (vertexLightmap) {
      //			ri.DPrintf( "WARNING: shader '%s' has VERTEX
      //forced lightmap!\n", shader.name );
    } else {
...
inline qboolean IsShader(shader_t *sh, const char *name,
                         const int *lightmapIndex, const byte *styles) {
  int i;
...
static inline const int *R_FindLightmap(const int *lightmapIndex) {
  ...
  Com_sprintf(fileName, sizeof(fileName), "%s/" EXTERNAL_LIGHTMAP, tr.worldDir,
              *lightmapIndex);
  image =
      R_FindImageFile(fileName, qfalse, qfalse,
...
===============
*/
shader_t *R_FindShader(const char *name, const int *lightmapIndex,
                       const byte *styles, qboolean mipRawImage) {
  char strippedName[MAX_QPATH];
  char fileName[MAX_QPATH];
  int hash;
  ...
  image = R_FindImageFile(fileName, mipRawImage, mipRawImage, qtrue,
                          mipRawImage ? GL_REPEAT : GL_CLAMP);
  if (!image) {
    ri.Printf(PRINT_DEVELOPER,
              S_COLOR_RED "Couldn't find image for shader %s\n", name);
    shader.defaultShader = true;
    return FinishShader();
  }
  //
...
shader_t *R_FindServerShader(const char *name, const int *lightmapIndex,
                             const byte *styles, qboolean mipRawImage) {
  char strippedName[MAX_QPATH];
  int hash;
  shader_t *sh;
...
====================
*/
qhandle_t RE_RegisterShaderLightMap(const char *name, const int *lightmapIndex,
                                    const byte *styles) {
  shader_t *sh;
...
shader_t *R_GetShaderByHandle(qhandle_t hShader) {
  if (hShader < 0) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "R_GetShaderByHandle: out of range hShader '%d'\n",
              hShader);
    return tr.defaultShader;
  }
...
static void ScanAndLoadShaderFiles(void) {
  ...
  for (i = 0; i < numShaderFiles; i++) {
    ...
    // Do a simple check on the shader structure in that file to make sure one
    // bad shader file cannot fuck up all other shaders.
    p = buffers[i];
    ...
    while (1) {
      ...
      if (token[0] == '#') {
        ri.Printf(PRINT_WARNING,
                  "WARNING: Deprecated shader comment \"%s\" on line %d in "
                  "file %s.  Ignoring line.\n",
      ...
      if (token[0] != '{' || token[1] != '\0') {
        ri.Printf(PRINT_WARNING,
                  "WARNING: Ignoring shader file %s. Shader \"%s\" on line %d "
                  "missing opening brace",
                  filename, shaderName, shaderLine);
        if (token[0]) {
          ri.Printf(PRINT_WARNING, " (found \"%s\" on line %d)", token,
...
static void CreateExternalShaders(void) {
  tr.projectionShadowShader =
      R_FindShader("projectionShadow", lightmapsNone, stylesDefault, qtrue);
