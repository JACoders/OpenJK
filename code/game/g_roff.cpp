...
static void G_RoffNotetrackCallback(gentity_t *ent, const char *notetrack) {
	...
	defaultoffsetposition:
		...
		if (strcmp(argument, "rof") == 0) {
			...
			if (r2 && strstr(teststr, "kill")) { // kill the looping sound
			...
			} else if (r2 && strstr(teststr, "sound")) { // OK... we should have a relative sound path
				...
				if (ent->s.eType == ET_MOVER) {
					...
					if (objectID) {
						...
						ent->s.loopSound = 0;
						sprintf(errMsg, "cgi_S_RegisterSound(%s) failed to return a valid sfxHandle_t for additional argument. Setting 'loopSound' to 0.",
								addlArg);
						goto functionend;
					}
				} else {
...
void G_Roff(gentity_t *ent) {
	...
	if (ent->client) {
		...
		if (ent->e_ThinkFunc == thinkF_TieFighterThink || ent->e_ThinkFunc == thinkF_TieBomberThink ||
			(!ent->e_ThinkFunc && ent->s.eType != ET_MISSILE && ent->s.eType != ET_ITEM &&
			 ent->s.eType !=
				 ET_MOVER)) { // will never set currentAngles & currentOrigin itself ( why do we limit which one's get set?, just set all the time? )
