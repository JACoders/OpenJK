...
void Cmd_ViewObjective_f(gentity_t *ent) {
	...
	gi.SendServerCommand(ent - g_entities,
						 va("print \"Objective %d   Display Status(1=show): %d  Status:%d\n\"", objectiveI,
							ent->client->sess.mission_objectives[objectiveI].display, ent->client->sess.mission_objectives[objectiveI].status));
}
...
void ClientCommand(int clientNum) {
	...
	if (Q_stricmp(cmd, "give") == 0)
		...
		G_Victory(ent);
	//	else if (Q_stricmp (cmd, "use_shield") == 0)	// sounds like the design doc states that the shields will be a pickup and so the player never decides
	//whether to use them or not. 		G_ActivatePersonalShield( ent );			//	If you want shields (armor),  type "give all" or "give armor" or "give armor
	//#amt#"
