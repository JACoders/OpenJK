...
float FuzzyWeightUndecided_r(int *inventory, fuzzyseperator_t *fs) {
  ...
  if (inventory[fs->index] < fs->value) {
    if (fs->child)
      ...
      return fs->minweight +
             Q_flrand(0.0f, 1.0f) * (fs->maxweight - fs->minweight);
  } // end if
  else if (fs->next) {
    if (inventory[fs->index] < fs->next->value) {
      // first weight
      ...
      if (fs->next->child)
        ...
        w2 = fs->next->minweight +
             Q_flrand(0.0f, 1.0f) * (fs->next->maxweight - fs->next->minweight);
      // the scale factor
      if (fs->next->value ==
          MAX_INVENTORYVALUE) // is fs->next the default case?
        return w2;            // can't interpolate, return default weight
  ...
  while (1) {
    if (inventory[s->index] < s->value) {
      if (s->child)
        ...
        return s->minweight +
               Q_flrand(0.0f, 1.0f) * (s->maxweight - s->minweight);
    } // end if
    else {
      if (s->next)
        s = s->next;
...
void EvolveFuzzySeperator_r(fuzzyseperator_t *fs) {
  if (fs->child) {
    ...
    if (Q_flrand(0.0f, 1.0f) < 0.01)
      ...
      fs->weight +=
          Q_flrand(-1.0f, 1.0f) * (fs->maxweight - fs->minweight) * 0.5;
