...
void CG_Chunks(int owner, vec3_t origin, const vec3_t normal, const vec3_t min,
               const vec3_t maxs, float speed, int numChunks,
...
void cgi_S_StartSound(vec3_t origin, int entityNum, int entchannel,
                      sfxHandle_t sfx);
...
unsigned int
cgi_AnyLanguage_ReadCharFromString(const char **ppText,
                                   qboolean *pbIsTrailingPunctuation = NULL);
unsigned int cgi_AnyLanguage_ReadCharFromString(
    const char **ppText, int *ppos,
    qboolean *pbIsTrailingPunctuation /* = NULL */);
