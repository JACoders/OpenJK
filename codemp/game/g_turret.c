...
//------------------------------------------------------------------------------------------------------------
void auto_turret_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                     int damage, int meansOfDeath)
//------------------------------------------------------------------------------------------------------------
{
...
//------------------------------------------------------------------------------------------------------------
void bottom_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                int damage, int meansOfDeath)
//------------------------------------------------------------------------------------------------------------
{
  if (self->target_ent && self->target_ent->health > 0) {
    ...
    auto_turret_die(self->target_ent, inflictor, attacker, damage,
                    meansOfDeath);
  }
}
...
//-----------------------------------------------------
{
  ...
  if (self->enemy && self->setTime < level.time &&
    ...
    self->r.currentOrigin, (cg.time?cg.time:level.time), NULL,
    self->s.modelScale );
...
//-----------------------------------------------------
{
  ...
  if (self->painDebounceTime > level.time) {
    ...
    diffPitch =
        AngleSubtract(desiredAngles[PITCH], top->r.currentAngles[PITCH]);
    ...
    if (self->enemy->s.eType == ET_NPC &&
        self->enemy->s.NPC_class == CLASS_VEHICLE && self->enemy->m_pVehicle &&
    ...
    trap->G2API_GetBoltMatrix( self->ghoul2, self->playerModel,
                            self->torsoBolt,
                            &boltMatrix, self->r.currentAngles, self->s.origin,
    ...
    if (!tr.allsolid && !tr.startsolid &&
      ...
      if (enemyDist < bestDist // all things equal, keep current
          ...
          (!Q_stricmp("atst_vehicle", target->NPC_type) && bestTarget &&
           Q_stricmp("atst_vehicle",
                     bestTarget->NPC_type))) // target AT-STs over non-AT-STs...
                                             // FIXME: must be a better, easier
                                             // way to tell this, no?
      {
        if (self->attackDebounceTime < level.time) {
          // We haven't fired or acquired an enemy in the last 2
          // seconds-start-up sound
          // G_Sound( self, CHAN_BODY, G_SoundIndex(
          // "sound/chars/turret/startup.wav" ));
...
//-----------------------------------------------------
{
  ...
  if (!self->enemy) {
    ...
    self->enemy = NULL;
  } else { // FIXME: remain single-minded or look for a new enemy every now and
           // then?
...
//-----------------------------------------------------
{
  ...
  base->s.modelindex2 =
      G_ModelIndex("models/map_objects/hoth/turret_bottom.md3");
  base->s.modelindex = G_ModelIndex("models/map_objects/hoth/turret_base.md3");
  // base->playerModel = trap->G2API_InitGhoul2Model( base->ghoul2,
  // "models/map_objects/imp_mine/turret_canon.glm", base->s.modelindex );
  // base->s.radius = 80.0f;

  // trap->G2API_SetBoneAngles( &base->ghoul2[base->playerModel], "Bone_body",
  // vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X, NULL
  // ); base->torsoBolt = trap->G2API_AddBolt( &base->ghoul2[base->playerModel],
  // "*flash03" );
...
qboolean turret_base_spawn_top(gentity_t *base) {
  ...
  top->s.modelindex =
      G_ModelIndex("models/map_objects/hoth/turret_top_new.md3");
  ...
  // even if we don't want to show health, let's at least light the crosshair up
  // properly over ourself
  if (!top->s.teamowner) {
