...
float enemyDist = 0;
extern qboolean NAV_CheckAhead(gentity_t *self, vec3_t end, trace_t &trace,
                               int clipmask);
...
void Wampa_Move(qboolean visible) {
  if (NPCInfo->localState != LSTATE_WAITING) {
    ...
    NPCInfo->goalRadius =
        MIN_DISTANCE; // MAX_DISTANCE;	// just get us within combat range
    ...
    if (NPC->enemy) { // pick correct movement speed and anim
      ...
      if (!TIMER_Done(NPC, "runfar") ||
          !TIMER_Done(
              NPC,
              "runclose")) { // keep running with this anim & speed for a bit
      } else if (!TIMER_Done(NPC, "walk")) { // keep walking for a bit
        ucmd.buttons |= BUTTON_WALKING;
      } else if (visible && enemyDist > 350 &&
      ...
      {                                          // fast run, all fours
        ...
        if (NPC->client->ps.legsAnim == BOTH_RUN2) {
          NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_RUN2TORUN1, SETANIM_FLAG_HOLD);
                 ...
                 NPCInfo->stats.runSpeed == 300) { // slow run, upright
        // BOTH_RUN2
        NPCInfo->stats.runSpeed = 200; // 180;
...
extern int NPC_GetEntsNearBolt(gentity_t **radiusEnts, float radius,
                               int boltIndex, vec3_t boltOrg);
...
void Wampa_Slash(int boltIndex, qboolean backhand) {
  ...
  for (i = 0; i < numEnts; i++) {
    ...
    if (radiusEnts[i] == NPC) { // Skip the wampa ent
      continue;
    }
    ...
    if (DistanceSquared(radiusEnts[i]->currentOrigin, boltOrg) <=
        radiusSquared) {
      // smack
      G_Damage(radiusEnts[i], NPC, NPC, vec3_origin,
               radiusEnts[i]->currentOrigin, damage,
      ...
      if (backhand) {
        ...
        if (radiusEnts[i]->client->NPC_class != CLASS_WAMPA &&
          ...
          if (radiusEnts[i]->health > 0 && Q_irand(0, 1)) { // do pain on enemy
            G_Knockdown(radiusEnts[i], NPC, pushDir, 300, qtrue);
          }
        }
      } else if (radiusEnts[i]->health <= 0 &&
                 radiusEnts[i]->client) { // killed them, chance of dismembering
        if (!Q_irand(0, 1)) {             // bite something off
          ...
          G_DoDismemberment(radiusEnts[i], radiusEnts[i]->currentOrigin,
                            MOD_SABER, 1000, hitLoc, qtrue);
        }
      } else if (!Q_irand(0, 3) &&
                 radiusEnts[i]->health > 0) { // one out of every 4 normal hits
                                              // does a knockdown, too
        vec3_t pushDir;
        vec3_t angs;
        VectorCopy(NPC->client->ps.viewangles, angs);
        angs[YAW] += Q_flrand(25, 50);
...
void Wampa_Attack(float distance, qboolean doCharge) {
  if (!TIMER_Exists(NPC, "attacking")) {
    if (!Q_irand(0, 3) && !doCharge) { // double slash
      NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_ATTACK1,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      TIMER_Set(NPC, "attack_dmg", 750);
    } else if (doCharge ||
               (distance > 270 && distance < 430 && !Q_irand(0, 1))) { // leap
      NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_ATTACK2,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    ...
    {                          // grab
      NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_HOLD_START,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      NPC->client->ps.legsAnimTimer += 200;
      TIMER_Set(NPC, "attack_dmg", 250);
    } else { // backhand
      NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_ATTACK3,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      TIMER_Set(NPC, "attack_dmg", 250);
    }
  ...
  // Need to do delayed damage since the attack animations encapsulate multiple
  // mini-attacks
  ...
  if (NPC->client->ps.legsAnim == BOTH_ATTACK1 &&
      distance > (NPC->maxs[0] + MIN_DISTANCE)) { // okay to keep moving
    ucmd.buttons |= BUTTON_WALKING;
    Wampa_Move(qtrue);
  }
}
...
void Wampa_Combat(void) {
  ...
  else if ( UpdateGoal() )
  {
          NPCInfo->combatMove = qtrue;
          NPCInfo->goalEntity = NPC->enemy;
          NPCInfo->goalRadius = MIN_DISTANCE;//MAX_DISTANCE;	// just get us
  within combat range
  ...
  // Sometimes I have problems with facing the enemy I'm attacking, so force the
  // issue so I don't look dumb
  // FIXME: always seems to face off to the left or right?!!!!
  NPC_FaceEnemy(qtrue);

  float distance = enemyDist =
      Distance(NPC->currentOrigin, NPC->enemy->currentOrigin);

  qboolean advance =
      (qboolean)(distance > (NPC->maxs[0] + MIN_DISTANCE) ? qtrue : qfalse);
  ...
  if (advance) { // have to get closer
    ...
    if (NPC->enemy->health > 0        // enemy still alive
        ...
        && InFOV(NPC->enemy->currentOrigin, NPC->currentOrigin, yawOnlyAngles,
                 20, 20))   // enemy generally in front
    {                       // 10% chance of doing charge anim
      if (!Q_irand(0, 6)) { // go for the charge
        doCharge = qtrue;
        advance = qfalse;
      }
    }
  ...
  if ((advance || NPCInfo->localState == LSTATE_WAITING) &&
      TIMER_Done(NPC, "attacking")) // waiting monsters can't attack
  {
    ...
    if (!Q_irand(
            0,
            15)) { // FIXME: only do this if we just damaged them or vice-versa?
      if (Wampa_CheckRoar(NPC)) {
        return;
      }
    }
...
-------------------------
*/
void NPC_Wampa_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
                    const vec3_t point, int damage, int mod, int hitLoc) {
  qboolean hitByWampa = qfalse;
  if (self->count) { // FIXME: need pain anim
  ...
  if (other && other->inuse && other != self->enemy &&
      !(other->flags & FL_NOTARGET)) {
    if ((!other->s.number && !Q_irand(0, 3)) || !self->enemy ||
        self->enemy->health == 0 ||
        (self->enemy->client &&
         self->enemy->client->NPC_class == CLASS_WAMPA) ||
        (!Q_irand(0, 4) &&
         DistanceSquared(other->currentOrigin, self->currentOrigin) <
             DistanceSquared(
                 self->enemy->currentOrigin,
                 self->currentOrigin))) { // if my enemy is dead (or attacked by
                                          // player) and I'm not still
                                          // holding/eating someone, turn on the
                                          // attacker
      // FIXME: if can't nav to my enemy, take this guy if I can nav to him
      self->lastEnemy = other;
      G_SetEnemy(self, other);
      if (self->enemy !=
          self->lastEnemy) { // clear this so that we only sniff the player the
                             // first time we pick them up
        self->useDebounceTime = 0;
      }
      TIMER_Set(self, "lookForNewEnemy", Q_irand(5000, 15000));
      if (hitByWampa) { // stay mad at this Wampa for 2-5 secs before looking
                        // for other enemies
  ...
  if ((hitByWampa ||
       Q_irand(0, 100) < damage) // hit by wampa, hit while holding live victim,
                                 // or took a lot of damage
    ...
    if (!Wampa_CheckRoar(self)) {
      if (self->client->ps.legsAnim != BOTH_ATTACK1 &&
          ...
          self->client->ps.legsAnim !=
              BOTH_ATTACK3) { // cant interrupt one of the big attack anims
...
void Wampa_DropVictim(gentity_t *self) {
  ...
  if (self->activator) {
    ...
    NPC_SetAnim(self->activator, SETANIM_BOTH, BOTH_RELEASED,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    self->activator->client->ps.legsAnimTimer += 500;
    self->activator->client->ps.weaponTime =
        self->activator->client->ps.torsoAnimTimer =
            self->activator->client->ps.legsAnimTimer;
    if (self->activator->health > 0) {
      ...
      if (self->activator->client && self->activator->s.number < MAX_CLIENTS) {
        vec3_t vicAngles = {
            30, AngleNormalize180(self->client->ps.viewangles[YAW] + 180), 0};
...
qboolean Wampa_CheckDropVictim(gentity_t *self, qboolean excludeMe) {
  ...
  vec3_t start = {self->activator->currentOrigin[0],
                  self->activator->currentOrigin[1],
                  self->activator->absmin[2]};
  vec3_t end = {self->activator->currentOrigin[0],
                self->activator->currentOrigin[1],
                self->activator->absmax[2] - 1};
  trace_t trace;
  if (excludeMe) {
    gi.unlinkentity(self);
  }
  gi.trace(&trace, start, mins, maxs, end, self->activator->s.number,
           self->activator->clipmask, (EG2_Collision)0, 0);
  if (excludeMe) {
    gi.linkentity(self);
  }
  ...
  if (excludeMe) {   // victim stuck in wall
    if (self->NPC) { // turn
...
qboolean Wampa_TryGrab(void) {
  ...
  if (enemyDist <= radius && !NPC->count // don't have one in hand already
      ...
      NPC->enemy->client->NPC_class != CLASS_VEHICLE) { // grab
    NPC->enemy = NPC->enemy; // make him my new best friend
    NPC->enemy->client->ps.eFlags |= EF_HELD_BY_WAMPA;
    // FIXME: this makes it so that the victim can't hit us with shots!  Just
    // use activator or something
    NPC->enemy->activator = NPC; // kind of dumb, but when we are locked to the
                                 // Rancor, we are owned by it.
    NPC->activator = NPC->enemy; // remember him
    NPC->count = 1;              // in my hand
    // wait to attack
    TIMER_Set(NPC, "attacking",
              NPC->client->ps.legsAnimTimer + Q_irand(500, 2500));
    NPC_SetAnim(NPC->enemy, SETANIM_BOTH, BOTH_GRABBED,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_HOLD_END,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    TIMER_Set(NPC, "takingPain", -level.time);
    return qtrue;
  } else if (enemyDist < radius * 2.0f) { // smack
    ...
    if (NPC->enemy->client->NPC_class != CLASS_RANCOR &&
      ...
      if (NPC->enemy->health > 0) { // do pain on enemy
...
void NPC_BSWampa_Default(void) {
  ...
  if (NPC->client->ps.legsAnim == BOTH_HOLD_START) {
    NPC_FaceEnemy(qtrue);
    if (NPC->client->ps.legsAnimTimer < 200) { // see if he's there to grab
      if (!Wampa_TryGrab()) {
        NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_HOLD_MISS,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      }
    }
    return;
  ...
  if (NPC->count) {
    if (!NPC->activator || !NPC->activator->client) { // wtf?
      NPC->count = 0;
      NPC->activator = NULL;
    } else {
      if (NPC->client->ps.legsAnim == BOTH_HOLD_DROP) {
        if (NPC->client->ps.legsAnimTimer <
            PM_AnimLength(NPC->client->clientInfo.animFileIndex,
                          (animNumber_t)NPC->client->ps.legsAnim) -
                500) { // at least half a second into the anim
          if (Wampa_CheckDropVictim(NPC, qfalse)) {
            TIMER_Set(NPC, "attacking",
                      1000 + (Q_irand(500, 1000) * (3 - g_spskill->integer)));
          }
        }
        ...
        if (TIMER_Done(NPC, "sniffCorpse")) {
          Wampa_CheckDropVictim(NPC, qfalse);
                 ...
                 NPC->activator) { // just sniffing the guy
        if (NPC->useDebounceTime <= level.time + 100 &&
            NPC->client->ps.legsAnim !=
                BOTH_HOLD_DROP) { // just about done, drop him
          NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_HOLD_DROP,
                      SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
          TIMER_Set(NPC, "attacking", NPC->client->ps.legsAnimTimer + 500);
        }
      } else {
        if (!NPC->useDebounceTime && NPC->activator &&
            NPC->activator->s.number <
                MAX_CLIENTS) { // first time I pick the player, just sniff them
          if (TIMER_Done(NPC, "attacking")) { // ready to attack
            NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_HOLD_SNIFF,
                        SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
            NPC->useDebounceTime =
                level.time + NPC->client->ps.legsAnimTimer + Q_irand(500, 2000);
          }
        } else {
          if (TIMER_Done(NPC, "attacking")) { // ready to attack
            ...
            TIMER_Set(NPC, "attacking",
                      NPC->client->ps.legsAnimTimer + Q_irand(3000, 10000));
          }
            ...
            if (NPC->client->ps.legsAnimTimer) {
              if (TIMER_Done2(NPC, "grabAttackDamage", qtrue)) {
                ...
                G_Damage(NPC->activator, NPC, NPC, vec3_origin,
                         NPC->activator->currentOrigin, Q_irand(25, 40),
                         (DAMAGE_NO_KNOCKBACK | DAMAGE_NO_ARMOR), MOD_MELEE);
                if (NPC->activator->health <=
                    0) { // killed them, chance of dismembering
                  int hitLoc = HL_WAIST;
                  if (g_dismemberment->integer < 4) {
                    hitLoc = Q_irand(HL_BACK_RT, HL_HAND_LT);
                  } else {
                  ...
                  G_DoDismemberment(NPC->activator,
                                    NPC->activator->currentOrigin, MOD_SABER,
                                    1000, hitLoc, qtrue);
              ...
              NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_HOLD_IDLE /*BOTH_ATTACK4*/,
                          SETANIM_FLAG_NORMAL);
            }
  ...
  if (NPCInfo->localState == LSTATE_WAITING &&
      TIMER_Done2(NPC, "takingPain",
                  qtrue)) { // was not doing anything because we were taking
                            // pain, but pain is done now, so clear it...
    NPCInfo->localState = LSTATE_CLEAR;
  }

  if (!TIMER_Done(
          NPC, "rageTime")) { // do nothing but roar first time we see an enemy
    NPC_FaceEnemy(qtrue);
    return;
  }
  if (NPC->enemy) {
    if (NPC->enemy->client // enemy is a client
        && (NPC->enemy->client->NPC_class == CLASS_UGNAUGHT ||
            NPC->enemy->client->NPC_class ==
                CLASS_JAWA)         // enemy is a lowly jawa or ugnaught
        && NPC->enemy->enemy != NPC // enemy's enemy is not me
        && (!NPC->enemy->enemy || !NPC->enemy->enemy->client ||
            NPC->enemy->enemy->client->NPC_class !=
                CLASS_RANCOR)) // enemy's enemy is not a client or is not a
                               // rancor (which is scarier than me)
    ...
    if (!TIMER_Done(NPC, "attacking")) { // in middle of attack
      ...
      if (TIMER_Done(NPC, "angrynoise")) {
        G_SoundOnEnt(NPC, CHAN_AUTO,
                     va("sound/chars/wampa/misc/anger%d.wav", Q_irand(1, 2)));

        TIMER_Set(NPC, "angrynoise", Q_irand(5000, 10000));
      }
      // else, if he's in our hand, we eat, else if he's on the ground, we keep
      // attacking his dead body for a while
      if (NPC->enemy->client &&
          NPC->enemy->client->NPC_class ==
              CLASS_WAMPA) { // got mad at another Wampa, look for a valid enemy
        if (TIMER_Done(NPC, "wampaInfight")) {
          NPC_CheckEnemyExt(qtrue);
        }
      } else {
        if (NPC_ValidEnemy(NPC->enemy) == qfalse) {
          TIMER_Remove(NPC, "lookForNewEnemy"); // make them look again right
                                                // now
          if (!NPC->enemy->inuse ||
              level.time - NPC->enemy->s.time >
                  Q_irand(
                      10000,
                      15000)) { // it's been a while since the enemy died, or
                                // enemy is completely gone, get bored with him
            NPC->enemy = NULL;
            Wampa_Patrol();
            NPC_UpdateAngles(qtrue, qtrue);
            return;
        ...
        if (TIMER_Done(NPC, "lookForNewEnemy")) {
          gentity_t *sav_enemy = NPC->enemy; // FIXME: what about
                                             // NPC->lastEnemy?
          NPC->enemy = NULL;
          ...
          if (newEnemy && newEnemy != sav_enemy) { // picked up a new enemy!
            ...
            if (NPC->enemy !=
                NPC->lastEnemy) { // clear this so that we only sniff the player
                                  // the first time we pick them up
