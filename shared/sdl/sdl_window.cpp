...
===============
*/
void GLimp_Minimize(void) { SDL_MinimizeWindow(screen); }
...
static rserr_t GLimp_SetMode(glconfig_t *glConfig, const windowDesc_t *windowDesc, const char *windowTitle, int mode, qboolean fullscreen, qboolean noborder) {
	...
	icon = SDL_CreateRGBSurfaceFrom((void *)CLIENT_WINDOW_ICON.pixel_data, CLIENT_WINDOW_ICON.width, CLIENT_WINDOW_ICON.height,
...
window_t WIN_Init(const windowDesc_t *windowDesc, glconfig_t *glConfig) {
	...
	if (!GLimp_StartDriverAndSetMode(glConfig, windowDesc, r_mode->integer, (qboolean)r_fullscreen->integer, (qboolean)r_noborder->integer)) {
...
void *WIN_GL_GetProcAddress(const char *proc) { return SDL_GL_GetProcAddress(proc); }

qboolean WIN_GL_ExtensionSupported(const char *extension) { return SDL_GL_ExtensionSupported(extension) == SDL_TRUE ? qtrue : qfalse; }
