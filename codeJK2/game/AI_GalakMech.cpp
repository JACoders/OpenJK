...
extern qboolean WP_LobFire(gentity_t *self, vec3_t start, vec3_t target, vec3_t mins, vec3_t maxs, int clipmask, vec3_t velocity, qboolean tracePath,
...
void GM_Dying(gentity_t *self) {
	if (level.time - self->s.time < 4000) { // FIXME: need a real effect
		...
		if (TIMER_Done(self, "dyingExplosion")) {
			...
			case 1:
				if (!gi.G2API_GetSurfaceRenderStatus(&self->ghoul2[self->playerModel], "r_hand")) { // r_hand still there
					GM_CreateExplosion(self, self->handRBolt, qtrue);
					gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel], "r_hand", TURN_OFF);
				} else if (!gi.G2API_GetSurfaceRenderStatus(&self->ghoul2[self->playerModel], "r_arm_middle")) { // r_arm_middle still there
			...
			case 2:
				// FIXME: do only once?
				if (!gi.G2API_GetSurfaceRenderStatus(&self->ghoul2[self->playerModel], "l_hand")) { // l_hand still there
					GM_CreateExplosion(self, self->handLBolt);
					gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel], "l_hand", TURN_OFF);
				} else if (!gi.G2API_GetSurfaceRenderStatus(&self->ghoul2[self->playerModel], "l_arm_wrist")) { // l_arm_wrist still there
				...
				} else if (!gi.G2API_GetSurfaceRenderStatus(&self->ghoul2[self->playerModel], "l_arm_middle")) { // l_arm_middle still there
					newBolt = gi.G2API_AddBolt(&self->ghoul2[self->playerModel], "*l_arm_cap_l_hand");
					gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel], "l_arm_middle", TURN_OFF);
				} else if (!gi.G2API_GetSurfaceRenderStatus(&self->ghoul2[self->playerModel], "l_arm_augment")) { // l_arm_augment still there
...
void NPC_GM_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, vec3_t point, int damage, int mod, int hitLoc) {
	if (self->client->ps.powerups[PW_GALAK_SHIELD] == 0) { // shield is currently down
	...
	if (!self->lockCount && !self->client->ps.torsoAnimTimer) { // don't interrupt laser sweep attack or other special attacks/moves
...
static void GM_HoldPosition(void) {
	...
	if (!Q3_TaskIDPending(NPC, TID_MOVE_NAV)) { // don't have a script waiting for me to get to my point, okay to stop trying and stand
...
static qboolean GM_Move(void) {
	...
	if (moved == qfalse) {							// FIXME: if we're going to a combat point, need to pick a different one
		if (!Q3_TaskIDPending(NPC, TID_MOVE_NAV)) { // can't transfer movegoal or stop when a script we're running is waiting to complete
			GM_HoldPosition();
		}
	}
...
static void GM_CheckMoveState(void) {
	if (Q3_TaskIDPending(NPC, TID_MOVE_NAV)) { // moving toward a goal that a script is waiting on, so don't stop for anything!
		AImove = qtrue;
	}
	...
	if ((NPCInfo->goalEntity != NPC->enemy) && (NPCInfo->goalEntity != NULL)) {
		...
		if (NAV_HitNavGoal(NPC->currentOrigin, NPC->mins, NPC->maxs, NPCInfo->goalEntity->currentOrigin, 16, qfalse) ||
			(!Q3_TaskIDPending(NPC, TID_MOVE_NAV) && enemyLOS && enemyDist <= 10000)) { // either hit our navgoal or our navgoal was not a crucial (scripted)
																						// one (maybe a combat point) and we're scouting and found our enemy
			NPC_ReachedGoal();
...
void NPC_BSGM_Attack(void) {
	...
	if (NPC->enemy && NPC->enemy->health <= 0 && !NPC->enemy->s.number) { // my enemy is dead
	...
	if (NPC->client->ps.torsoAnim == BOTH_ATTACK4 || NPC->client->ps.torsoAnim == BOTH_ATTACK5) {
		...
		if (TIMER_Done(NPC, "smackTime") && !NPCInfo->blockedDebounceTime) { // time to smack
			...
			if (enemyDist < MELEE_DIST_SQUARED && InFront(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, 0.3f)) {
				...
				G_Damage(NPC->enemy, NPC, NPC, smackDir, NPC->currentOrigin, (g_spskill->integer + 1) * Q_irand(5, 10), DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK,
						 MOD_CRUSH);
				if (NPC->client->ps.torsoAnim == BOTH_ATTACK4) { // smackdown
					int knockAnim = BOTH_KNOCKDOWN1;
					if (PM_CrouchAnim(NPC->enemy->client->ps.legsAnim)) { // knockdown from crouch
	...
	{						   // sometimes use the laser beam attack, but only after he's taken down our generator
		...
		if (!NPC->client->ps.powerups[PW_GALAK_SHIELD] && enemyDist < MELEE_DIST_SQUARED &&
		...
		{											  // our shield is down, and enemy within 80, if very close, use melee attack to slap away
			if (TIMER_Done(NPC, "attackDelay")) {
				// animate me
				int swingAnim;
				if (NPC->locationDamage[HL_GENERIC1] > GENERATOR_HEALTH) { // generator down, use random melee
			...
			shoot = qfalse;
			NPC_GM_StartLaser();
		} else if (enemyDist < MIN_LOB_DIST_SQUARED && (NPC->enemy->s.weapon != WP_TURRET || Q_stricmp("PAS", NPC->enemy->classname)) &&
	...
	if (NPC->client->ps.weapon == WP_REPEATER && (NPCInfo->scriptFlags & SCF_ALT_FIRE) && shoot && TIMER_Done(NPC, "attackDelay")) {
		...
		qboolean clearshot = WP_LobFire(NPC, muzzle, target, mins, maxs, MASK_SHOT | CONTENTS_LIGHTSABER, velocity, qtrue, NPC->s.number, NPC->enemy->s.number,
										300, 1100, 1500, qtrue);
		if (VectorCompare(vec3_origin, velocity) || (!clearshot && enemyLOS && enemyCS)) { // no clear lob shot and no lob shot that will hit something
																						   // breakable
			if (enemyLOS && enemyCS && TIMER_Done(NPC, "noRapid")) { // have a clear straight shot, so switch to primary
				NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;
				NPC->alt_fire = qfalse;
				NPC_ChangeWeapon(WP_REPEATER);
	...
	if (AImove && !NPC->lockCount) { // move toward goal
		if (NPCInfo->goalEntity && NPC->client->ps.legsAnim != BOTH_ALERT1 && NPC->client->ps.legsAnim != BOTH_ATTACK2 &&
			NPC->client->ps.legsAnim != BOTH_ATTACK4 && NPC->client->ps.legsAnim != BOTH_ATTACK5 && NPC->client->ps.legsAnim != BOTH_ATTACK7) {
			AImove = GM_Move();
		} else {
			AImove = qfalse;
		}
	...
	if (!faceEnemy) {  // we want to face in the dir we're running
		if (!AImove) { // if we haven't moved, we should look in the direction we last looked?
	...
	// also:
	if (NPC->enemy->s.weapon == WP_TURRET && !Q_stricmp("PAS", NPC->enemy->classname)) { // crush turrets
		if (G_BoundsOverlap(NPC->absmin, NPC->absmax, NPC->enemy->absmin,
							NPC->enemy->absmax)) { // have to do this test because placed turrets are not solid to NPCs (so they don't obstruct navigation)
