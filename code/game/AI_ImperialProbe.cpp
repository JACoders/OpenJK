...
void ImperialProbe_MaintainHeight(void) {
  ...
  if (NPC->enemy) {
    ...
    /*		VectorSet( endPos, NPC->currentOrigin[0], NPC->currentOrigin[1],
       NPC->currentOrigin[2] - 512 ); gi.trace( &trace, NPC->currentOrigin,
       NULL, NULL, endPos, NPC->s.number, MASK_SOLID );
...
void ImperialProbe_Strafe(void) {
  ...
  // Close enough
  if (tr.fraction > 0.9f) {
    VectorMA(NPC->client->ps.velocity, HUNTER_STRAFE_VEL * dir, right,
...
void ImperialProbe_Hunt(qboolean visible, qboolean advance) {
  ...
  NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_RUN1,
  ...
  speed = HUNTER_FORWARD_BASE_SPEED +
          HUNTER_FORWARD_MULTIPLIER * g_spskill->integer;
...
void ImperialProbe_AttackDecision(void) {
  ...
  if (TIMER_Done(NPC, "patrolNoise")) {
    if (TIMER_Done(NPC, "angerNoise")) {
      G_SoundOnEnt(NPC, CHAN_AUTO,
                   va("sound/chars/probe/misc/probetalk%d", Q_irand(1, 3)));
  ...
  //	distance_e	distRate	= ( distance > MIN_MELEE_RANGE_SQR ) ?
  //DIST_LONG : DIST_MELEE;
...
-------------------------
*/
void NPC_Probe_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
                    const vec3_t point, int damage, int mod, int hitLoc) {
  float pain_chance;

  VectorCopy(self->NPC->lastPathAngles, self->s.angles);

  if (self->health < 30 || mod == MOD_DEMP2 ||
      mod == MOD_DEMP2_ALT) // demp2 always messes them up real good
  {
    vec3_t endPos;
    trace_t trace;

    VectorSet(endPos, self->currentOrigin[0], self->currentOrigin[1],
              self->currentOrigin[2] - 128);
    gi.trace(&trace, self->currentOrigin, NULL, NULL, endPos, self->s.number,
    ...
    if (trace.fraction == 1.0f || mod == MOD_DEMP2) // demp2 always does this
    {
      ...
      if ((mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT) && other) {
        ...
        VectorMA(self->client->ps.velocity, 550, dir,
...
void ImperialProbe_Wait(void) {
  if (NPCInfo->localState == LSTATE_DROP) {
    ...
    VectorSet(endPos, NPC->currentOrigin[0], NPC->currentOrigin[1],
              NPC->currentOrigin[2] - 32);
    gi.trace(&trace, NPC->currentOrigin, NULL, NULL, endPos, NPC->s.number,
             MASK_SOLID, (EG2_Collision)0, 0);
