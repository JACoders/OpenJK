	...
	if (s1->boltInfo) {
			...
			cent->gent->activator->owner->s.number == cent->currentState.number) // gun number must be same as current entities number
		{
			...
			if (cc->muzzleFlashTime > 0 && wData) {
				...
				if (cc->currentState.eFlags & EF_FIRING || cc->currentState.eFlags & EF_ALT_FIRING) {
					gi.G2API_SetBoneAnimIndex(&cent->gent->ghoul2[cent->gent->playerModel], cent->gent->rootBone, 0, 3, BONE_ANIM_OVERRIDE_FREEZE, 0.6f,
											  cg.time, -1, -1);
			...
			if (cent->gent->classname && !Q_stricmp("limb", cent->gent->classname)) { // limb, just add blade
				if (cent->gent->owner->client) {
					if (cent->gent->owner->client->ps.saberLength > 0) {
						...
						CG_CheckSaberInWater(&cg_entities[cent->gent->owner->s.number], &cg_entities[cent->gent->s.number], cent->gent->weaponModel,
				...
				if (cent->gent->owner->client &&
					...
					if (cent->gent->owner->client->ps.saberEntityState == SES_RETURNING) {
						cgi_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin, vec3_origin, CG_SaberHumSoundForEnt(cent->gent->owner));
					} else {
						int spinSound;
				...
				if (cent->gent->owner->client) {
					if (cent->gent->owner->client->ps.saberLength > 0) { // only add the blade if it's on
						CG_AddSaberBlade(&cg_entities[cent->gent->owner->s.number], &cg_entities[cent->gent->s.number], NULL, ent.renderfx, 0, cent->lerpOrigin,
										 cent->lerpAngles);
					} else if (cent->gent->owner->client->ps.saberEventFlags & SEF_INWATER) {
						CG_CheckSaberInWater(&cg_entities[cent->gent->owner->s.number], &cg_entities[cent->gent->s.number], 0, cent->lerpOrigin,
											 cent->lerpAngles);
					}
				}
		...
		if (cent->gent->aimDebounceTime) {				 // done with dismemberment, just waiting to mark owner dismemberable again
			...
			if (owner->weaponModel >= 0) { // the corpse hasn't dropped their weapon
				if (cent->gent->count == BOTH_DISMEMBER_RARM ||
					cent->gent->count == BOTH_DISMEMBER_TORSO1) //&& ent->s.weapon == WP_SABER && ent->weaponModel != -1 )
...
void CG_MatrixEffect(centity_t *cent) {
	...
	if (totalElapsedTime > cent->currentState.eventParm // MATRIX_EFFECT_TIME
		|| (cent->currentState.weapon && g_entities[cent->currentState.otherEntityNum].client &&
			g_entities[cent->currentState.otherEntityNum].client->ps.groundEntityNum != ENTITYNUM_NONE) ||
		cg.missionStatusShow) { // time is up or this is a falling spin and they hit the ground or mission end screen is up
	...
	if (!cent->currentState.weapon) { // go ahead and do all the slowdown and vert bob stuff
		// slowdown
