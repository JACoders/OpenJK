...
float AAS_ClosestEdgePoints(vec3_t v1, vec3_t v2, vec3_t v3, vec3_t v4, aas_plane_t *plane1, aas_plane_t *plane2, vec3_t beststart1, vec3_t bestend1,
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
		...
		if (!AAS_AreaTeleporter(area2num) && !AAS_AreaJumpPad(area2num)) {
			...
			else*/
			{
				...
				VectorClear(cmdmove);
				AAS_PredictClientMovement(&move, -1, destorigin, PRESENCE_NORMAL, qfalse, velocity, cmdmove, 0, 30, 0.1f,
			...
			for (i = 0; i < area2->numfaces; i++) {
				...
				if (ret && speed < 150) {
					...
					// if (hordist < 1.6 * facecenter[2] - areastart[2])
					{
						...
						//
						AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, qfalse, velocity, cmdmove, 30, 30, 0.1f,
...
void AAS_SetWeaponJumpAreaFlags(void) {
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
		if (!AAS_ValueForBSPEpairKey(ent, "classname", classname, MAX_EPAIRKEY))
			continue;
		if (!strcmp(classname, "item_armor_body") || !strcmp(classname, "item_health") || !strcmp(classname, "weapon_disruptor") ||
			!strcmp(classname, "weapon_repeater") || !strcmp(classname, "weapon_demp2") || !strcmp(classname, "weapon_flechette") ||
		...
		for (n = 0; n < 1 /*2*/; n++) {
			...
			if (ret && speed < 300) {
				...
				// if (hordist < 1.6 * (facecenter[2] - areastart[2]))
				{
					...
					*/
					//
					AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, qtrue, velocity, cmdmove, 30, 30, 0.1f,
