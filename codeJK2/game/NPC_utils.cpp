...
qboolean NPC_ValidEnemy(gentity_t *ent) {
  ...
  // Must be an NPC
  if (ent->client == NULL) {
    if (ent->svFlags & SVF_NONNPC_ENEMY) { // still potentially valid
...
void NPC_CheckCharmed(void) {
  if (NPC->client->playerTeam == TEAM_PLAYER && NPCInfo->charmedTime &&
      NPCInfo->charmedTime < level.time &&
      NPC->client) { // we were charmed, set us back!
