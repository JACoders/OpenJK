...
static const save_field_t savefields_gVHIC[] = {
    ...
    {strVHICOFS(m_pVehicleInfo),
     F_VEHINFO}, //! another ptr! store name field instead and re-hook on load?

    {NULL, 0, F_IGNORE}};

static const save_field_t savefields_gClient[] = {
    // sabers are stomped over by specific code elsewhere, it seems, but the
    // first two fields MUST be saved
...
static int GetStringNum(const char *psString) {
  ...
  return strlen(psString) +
         1; // this gives us the chunk length for the reader later
}
...
static char *GetStringPtr(int iStrlen, char *psOriginal /*may be NULL*/) {
  if (iStrlen != -1) {
    ...
    if (psOriginal && gi.bIsFromZone(psOriginal, TAG_G_ALLOC)) {
      if (!strcmp(psOriginal,
                  sString)) { // it's a legal ptr and they're the same so let's
                              // just reuse it instead of free/alloc
        return psOriginal;
      }
      gi.Free(psOriginal);
    }
...
static intptr_t GetGEntityNum(gentity_t *ent) {
  ...
  // note that I now validate the return value (to avoid triggering asserts on
  // re-load) because of the
  //	way that the level_locals_t alertEvents struct contains a count of which
  //ones are valid, so I'm guessing 	that some of them aren't (valid)...
  //
...
static intptr_t GetGroupNumber(AIGroupInfo_t *pGroup) {
  ...
  if (iReturnIndex < 0 ||
      iReturnIndex >= (int)(sizeof(level.groups) / sizeof(level.groups[0]))) {
    iReturnIndex = -1; // will get a NULL ptr on reload
  }
  return iReturnIndex;
}
...
static intptr_t GetGClientNum(gclient_t *c, gentity_t *ent) {
  // unfortunately, I now need to see if this is a INT_ID('r','e','a','l')
  // client (and therefore resolve to an enum), or
  ...
  if (ent->s.number < MAX_CLIENTS) { // regular client...
    return (c - level.clients);
  } else { // this must be an NPC or weapon_shooter, so mark it as special...
    return -2; // yeuch, but distinguishes it from a valid 0 index, or -1 for
               // client==NULL
  }
}
...
static gclient_t *GetGClientPtr(intptr_t c) {
  ...
  if (c == -2) {
    return (gclient_t *)-2; // preserve this value so that I know to load in one
                            // of Mike's private NPC clients later
  }
...
static void EnumerateField(const save_field_t *pField, const byte *pbBase) {
            ...
            case F_BODYQUEUE:
                    {
                            ...
                            {
                                    pv = &p[i];	// since you can't ++ a void ptr
                                    *(int *)pv = GetGEntityNum(*(gentity_t
       **)pv);
                            }
                    }
                    break;
  ...
  case F_ALERTEVENT: // convert all gentity_t ptrs in an alertEvent array into
                     // indexes...
  {
  ...
  case F_ANIMFILESETS: {
    ...
    for (int i = 0; i < MAX_ANIM_FILES; i++) {
      for (int j = 0; j < MAX_ANIM_EVENTS; j++) {
        byteAlias_t *baTorso =
                        (byteAlias_t *)&p[i].torsoAnimEvents[j].stringData,
...
static void EnumerateFields(const save_field_t *pFields, const T *src_instance,
  ...
  const byte *pbData = reinterpret_cast<const byte *>(src_instance);
  ...
  src_instance->sg_export(saved_game);

  saved_game.write_chunk(ulChid);

  // save out any associated strings..
  //
  for (const auto &it : strList) {
    saved_game.write_chunk(INT_ID('S', 'T', 'R', 'G'), it.c_str(),
                           static_cast<int>(it.length() + 1));
  }
}

static void EvaluateField(const save_field_t *pField, byte *pbBase,
                          byte *pbOriginalRefData /* may be NULL*/) {
  void *pv = (void *)(pbBase + pField->iOffset);
  void *pvOriginal = (void *)(pbOriginalRefData + pField->iOffset);

  switch (pField->eFieldType) {
  case F_STRING:
    *(char **)pv = GetStringPtr(
        *(int *)pv, pbOriginalRefData ? *(char **)pvOriginal : NULL);
    break;
  ...
  case F_ANIMFILESETS: {
    ...
    for (int i = 0; i < MAX_ANIM_FILES; i++) {
      for (int j = 0; j < MAX_ANIM_EVENTS; j++) {
        pO = pbOriginalRefData
                 ? level.knownAnimFileSets[i].torsoAnimEvents[j].stringData
                 : NULL;
        p[i].torsoAnimEvents[j].stringData =
            GetStringPtr((intptr_t)p[i].torsoAnimEvents[j].stringData, pO);
        pO = pbOriginalRefData
                 ? level.knownAnimFileSets[i].legsAnimEvents[j].stringData
                 : NULL;
        p[i].legsAnimEvents[j].stringData =
            GetStringPtr((intptr_t)p[i].legsAnimEvents[j].stringData, pO);
      }
    }
  } break;
...
void saberInfoRetail_t::sg_export(saberInfo_t &dst) const {
  ::WP_SaberSetDefaults(&dst, qfalse);

  if (!activeBlocking) {
    dst.saberFlags |= SFL_NOT_ACTIVE_BLOCKING;
  }

  ::memcpy(dst.blade, blade, sizeof(blade));
  ...
  if (style != SS_NONE && style < SS_NUM_SABER_STYLES) {
    ...
    for (int styleNum = SS_NONE + 1; styleNum < SS_NUM_SABER_STYLES;
         ++styleNum) {
      if (styleNum != style) {
...
static void copy_retail_gclient_to_current(const RetailGClient &src,
                                           gclient_t &dst) {
  ...
  ::memcpy(reinterpret_cast<char *>(&dst), reinterpret_cast<const char *>(&src),
           src_pre_size);

  for (int i = 0; i < MAX_SABERS; ++i) {
    src.ps.saber[i].sg_export(dst.ps.saber[i]);
  }

  ::memcpy(reinterpret_cast<char *>(&dst) + src_post_offset,
           reinterpret_cast<const char *>(&src) + dst_post_offset,
           src_post_size);
}

template <typename T>
static void EvaluateFields(const save_field_t *pFields, T *pbData,
                           byte *pbOriginalRefData, unsigned int ulChid) {
  T &instance = *pbData;

  ojk::SavedGameHelper saved_game(::gi.saved_game);

  if (ulChid != INT_ID('G', 'C', 'L', 'I')) {
    saved_game.read_chunk(ulChid, instance);
  } else {
    if (!saved_game.try_read_chunk(ulChid, instance)) {
      RetailGClient retail_client;

      saved_game.reset_buffer_offset();

      if (saved_game.try_read(retail_client)) {
        copy_retail_gclient_to_current(retail_client,
...
static void WriteLevelLocals() {
  level_locals_t *temp = (level_locals_t *)gi.Malloc(
      sizeof(level_locals_t), TAG_TEMP_WORKSPACE, qfalse);
...
static void ReadLevelLocals() {
  // preserve client ptr either side of the load, because clients are already
...
static void WriteGEntities(qboolean qbAutosave) {
  ...
  saved_game.write_chunk<int32_t>(INT_ID('N', 'M', 'E', 'D'), iCount);

  for (i = 0; i < (qbAutosave ? 1 : globals.num_entities); i++) {
    gentity_t *ent = &g_entities[i];

    if (ent->inuse) {
      saved_game.write_chunk<int32_t>(INT_ID('E', 'D', 'N', 'M'), i);
      ...
      // now for any fiddly bits that would be rather awkward to build into the
      // enumerator...
      //
      if (tempEnt.NPC) {
      ...
      if (tempEnt.parms) {
        saved_game.write_chunk(INT_ID('P', 'A', 'R', 'M'), *ent->parms);
      }
      ...
      tempEnt.ghoul2.kill(); // this handle was shallow copied from an ent. We
  ...
  if (!qbAutosave) {
    ...
    // this marker needs to be here, it lets me know if Icarus doesn't load
    // everything back later,
    //	which has happened, and doesn't always show up onscreen until certain
    //game situations. 	This saves time debugging, and makes things easier to
    //track.
    //
    static int iBlah = 1234;

    saved_game.write_chunk<int32_t>(INT_ID('I', 'C', 'O', 'K'), iBlah);
  }
  if (!qbAutosave) // really shouldn't need to write these bits at all, just
                   // restore them from the ents...
  {
    WriteInUseBits();
  }
}
...
static void ReadGEntities(qboolean qbAutosave) {
  ...
  saved_game.read_chunk<int32_t>(INT_ID('N', 'M', 'E', 'D'), iCount);

  int iPreviousEntRead = -1;
  for (i = 0; i < iCount; i++) {
    int iEntIndex = 0;

    saved_game.read_chunk<int32_t>(INT_ID('E', 'D', 'N', 'M'), iEntIndex);
    ...
    *pEntOriginal = *pEnt; // struct copy, so we can refer to original
    ...
    //
    // sneaky:  destroy the ghoul2 object within this struct before
    // binary-loading over the top of it...
    //
    ...
    if (pEnt->NPC) // will be qtrue/qfalse
    {
      ...
      EvaluateFields(savefields_gNPC, &tempNPC, (byte *)pEntOriginal->NPC,
                     INT_ID('G', 'N', 'P', 'C'));
      ...
      *pEnt->NPC = tempNPC; // struct copy
      ...
      {//not player
              ...
              G_SetSkin( *pEnt, *pEnt->NPC_type, NULL );//  it probably wasn't
      the default skin, do we need this at all?
      }
      */
    }
    ...
    if (pEnt->client == (gclient_t *)-2) // one of Mike G's NPC clients?
    {
      ...
      EvaluateFields(savefields_gClient, &tempGClient,
                     (byte *)pEntOriginal->client, INT_ID('G', 'C', 'L', 'I'));

      // can we pinch the original's client handle or do we have to alloc a new
      // one?...
      //
    ...
    if (pEnt->parms) // will be qtrue/qfalse
    {
      ...
      saved_game.read_chunk(INT_ID('P', 'A', 'R', 'M'), tempParms);
    ...
    if (pEnt->m_pVehicle) // will be qtrue/qfalse
    {
      ...
      if (pEntOriginal->m_pVehicle) {
        ...
        //
        pEnt->m_pVehicle =
            (Vehicle_t *)gi.Malloc(sizeof(Vehicle_t), TAG_G_ALLOC, qfalse);
      }
    ...
    //
    {
      saved_game.read_chunk(INT_ID('G', 'H', 'L', '2'));
    ...
    // because the sytem stores sfx_t handles directly instead of the set, we
    // have to reget the set's sfx_t...
    //
  ...
  if (!qbAutosave) {
    // now zap any g_ents that were inuse when the level was loaded, but are no
    // longer in use in the saved version
    ...
    // check that Icarus has loaded everything it saved out by having a marker
    // chunk after it...
    //
    static int iBlah = 1234;

    saved_game.read_chunk<int32_t>(INT_ID('I', 'C', 'O', 'K'), iBlah);
  }
  if (!qbAutosave) {
    ReadInUseBits(); // really shouldn't need to read these bits in at all, just
                     // restore them from the ents...
  }
}
...
void WriteLevel(qboolean qbAutosave) {
  if (!qbAutosave) //-always save the client
  {
    ...
    //
    assert(level.maxclients ==
           1); // I'll need to know if this changes, otherwise I'll need to
               // change the way ReadGame works
  ...
  //
  // put out an end-marker so that the load code can check everything was read
  // in...
  //
  ...
  saved_game.write_chunk<int32_t>(INT_ID('D', 'O', 'N', 'E'), iDONE);
}
...
void ReadLevel(qboolean qbAutosave, qboolean qbLoadTransition) {
  ...
  if (qbLoadTransition) {
    // I STRONGLY SUSPECT THAT THIS WILL JUST ERR_DROP BECAUSE OF THE LOAD
    // SWAPPING OF THE CHUNK-ORDER
    //	BELOW BETWEEN OBJECTIVES AND LEVEL_LOCALS, SO I'M GUESSING THIS IS SOME
    //OLD EF1 JUNK?
    // IN ANY CASE, LET'S MAKE SURE...   // -ste (no idea who wrote the comment
    // stuff below, did it ever work?)
    //
    assert(0);
    //
    // loadtransitions do not need to read the objectives and client data from
    // the level they're going to In a loadtransition, client data is carried
    // over on the server and will be stomped later anyway. The objective info
    // (in client->sess data), however, is read in from G_ReadSessionData which
    // is called before this func, we do NOT want to stomp that session data when
    // doing a load transition

    // However, we should still save this info out because these savegames may
    // need to be loaded normally later- perhaps if you die and need to respawn,
    // perhaps as some kind of emergency savegame for resuming, etc.

    // SO: We read it in, but throw it away.

    // Read & throw away gclient info
    gclient_t junkClient;
    EvaluateFields(savefields_gClient, &junkClient, (byte *)&level.clients[0],
                   INT_ID('G', 'C', 'L', 'I'));
    ...
    if (!qbAutosave) // always load the client unless it's an autosave
    {
      ...
      gclient_t GClient;
      EvaluateFields(savefields_gClient, &GClient, (byte *)&level.clients[0],
                     INT_ID('G', 'C', 'L', 'I'));
  ...
  //
  // check that the whole file content was loaded by specifically requesting an
  // end-marker...
  //
  static int iDONE = 1234;
