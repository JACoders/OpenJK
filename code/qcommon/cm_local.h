...
public:
  ...
  class CCMShader *GetNext(void) const {
    return (mNext);
  }
...
typedef struct traceWork_s {
  ...
  vec3pair_t localBounds; // enclosing box of start and end surrounding by size
                          // for a segment
  ...
  float baseEnterFrac; // global enter fraction (before processing subsections
                       // of the brush)
  float baseLeaveFrac; // global leave fraction (before processing subsections
                       // of the brush)
...
// cm_trace.cpp
void CM_CalcExtents(const vec3_t start, const vec3_t end, const traceWork_t *tw,
                    vec3pair_t bounds);
