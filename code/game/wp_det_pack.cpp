...
//---------------------------------------------------------
{
  ...
  gentity_t *missile =
      CreateMissile(start, forwardVec, 300, 10000, self, qfalse);

  missile->fxID =
      G_EffectIndex("detpack/explosion"); // if we set an explosion effect,
                                          // explode death can use that instead
    ...
    if (ent->client->ps.eFlags & EF_PLANTED_CHARGE) {
      ...
      while ((found = G_Find(found, FOFS(classname), "detpack")) != NULL) {
        if (found->activator == ent) {
          ...
          AddSoundEvent(NULL, found->currentOrigin, found->splashRadius * 2,
                        AEL_DANGER, qfalse,
                        qtrue); // FIXME: are we on ground or not?
          AddSightEvent(NULL, found->currentOrigin, found->splashRadius * 2,
                        AEL_DISCOVERED, 100);
        }
      }
