...
class SavedGame : public ISavedGame {
public:
  SavedGame();
  ...
  bool create(const std::string &base_file_name);

  // Opens an existing saved game file for reading.
  bool open(const std::string &base_file_name);

  // Closes the current saved game file.
  void close();

  // Reads a chunk from the file into the internal buffer.
  bool read_chunk(const uint32_t chunk_id) override;
  ...
  bool write_chunk(const uint32_t chunk_id) override;

  // Reads a raw data from the internal buffer.
  // Returns true on success or false otherwise.
  bool read(void *dst_data, int dst_size) override;

  // Writes a raw data into the internal buffer.
  // Returns true on success or false otherwise.
  bool write(const void *src_data, int src_size) override;
  ...
  static void rename(const std::string &old_base_file_name,
                     const std::string &new_base_file_name);

  // Remove a saved game file.
  static void remove(const std::string &base_file_name);
...
private:
  ...
  // Compresses data.
  static void compress(const Buffer &src_buffer, Buffer &dst_buffer);

  // Decompresses data.
  static void decompress(const Buffer &src_buffer, Buffer &dst_buffer);

  static std::string generate_path(const std::string &base_file_name);

  // Returns a string representation of a chunk id.
  static std::string get_chunk_id_string(uint32_t chunk_id);
