...
typedef struct shader_s {
  ...
  int lightmapIndex[MAXLIGHTMAPS]; // for a shader to match, both name and
                                   // lightmapIndex must match
  ...
  bool noTC; // for images that don't want to be texture compressed (namely
             // skies)
  ...
  //	struct shader_s		*remappedShader;                  // current
  //shader this one is remapped too
...
Ghoul2 Insert Start
*/
// bogus little registration system for hit and location based damage files in
// hunk memory
/*
typedef struct
{
...
typedef struct {
  ...
  world_t *world;
  char worldDir[MAX_QPATH]; // ie: maps/tim_dm2 (copy of world_t::name sans
                            // extension but still includes the path)
  ...
  unsigned shiftedEntityNum; // currentEntityNum << QSORT_REFENTITYNUM_SHIFT
                             // (possible with high bit set for RF_ALPHA_FADE)
  ...
  vec3_t sunAmbient;   // from the sky shader	(only used for John's terrain
                       // system)
...
// rjr - removed for hacking extern cvar_t	*r_dlightBacks;			//
...
extern cvar_t *r_ext_compressed_lightmaps; // turns on compression of lightmaps,
                                           // off by default
...
image_t *R_CreateImage(const char *name, const byte *pic, int width, int height,
                       ...
                       qboolean allowTC, int wrapClampMode);
...
qboolean RE_GetLighting(const vec3_t origin, vec3_t ambientLight,
                        vec3_t directedLight, vec3_t lightDir);

// Only returns a four sided face and normal of the best face to break ( this is
// for glass right now )
...
public:
  ...
  int ident;
#else
  const int ident; // ident of this surface - required so the materials renderer
                   // knows what sort of surface this refers to
#endif
  ...
  float scale;
  float fade;
  float impactTime; // this is a number between 0 and 1 that dictates the
                    // progression of the bullet impact
#endif
  ...
  CRenderableSurface()
      : ident(SF_MDX), boneCache(0),
#ifdef _G2_GORE
        surfaceData(0), alternateTex(0), goreChain(0)
#else
        surfaceData(0)
#endif
  {
