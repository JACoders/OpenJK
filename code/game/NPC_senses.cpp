...
/*
qboolean G_ClearLineOfSight(const vec3_t point1, const vec3_t point2, int
ignore, int clipmask)

returns true if can see from point 1 to 2, even through glass (1 pane)- doesn't
work with portals
*/
qboolean G_ClearLineOfSight(const vec3_t point1, const vec3_t point2,
                            int ignore, int clipmask) {
  trace_t tr;

  gi.trace(&tr, point1, NULL, NULL, point2, ignore, clipmask, (EG2_Collision)0,
           0);
  if (tr.fraction == 1.0) {
    return qtrue;
  }
...
or take any AI related factors (for example, the NPC's reaction time) into
account
...
qboolean InFront(vec3_t spot, vec3_t from, vec3_t fromAngles,
...
IDEA: further off to side of FOV range, higher chance of failing even if
technically in FOV, keep core of 50% to sides as always succeeding
*/

// Position compares

qboolean InFOV(vec3_t spot, vec3_t from, vec3_t fromAngles, int hFOV,
               int vFOV) {
...
qboolean InFOV(gentity_t *ent, gentity_t *from, int hFOV, int vFOV) {
  ...
  if (from->client) {
    if (from->client->NPC_class != CLASS_RANCOR &&
        ...
        !VectorCompare(from->client->renderInfo.eyeAngles,
                       vec3_origin)) { // Actual facing of tag_head!
...
qboolean InVisrange(gentity_t *ent) { // FIXME: make a calculate visibility for
                                      // ents that takes into account
  // lighting, movement, turning, crouch/stand up, other anims, hide brushes,
  // etc.
  vec3_t eyes;
  vec3_t spot;
  vec3_t deltaVector;
  ...
  ent->client->ps.velocity[2])
          ...
          {//FIXME: shouldn't they need to have line of sight to you to detect
  this? avel = VectorLength(ent->avelocity); if(avel > 15)
                  {
...
-------------------------
*/
static int G_CheckSoundEvents(gentity_t *self, float maxHearDist,
                              int ignoreAlert, qboolean mustHaveOwner,
  ...
  for (int i = 0; i < level.numAlertEvents; i++) {
    ...
    if (self->client &&
        self->client->NPC_class !=
            CLASS_SAND_CREATURE) { // sand creatures hear all in within their
                                   // earshot, regardless of quietness and alert
                                   // sound radius!
      radius = level.alertEvents[i].radius * level.alertEvents[i].radius;
      if (dist > radius)
        continue;

      if (level.alertEvents[i]
              .addLight) { // a quiet sound, must have LOS to hear it
        if (G_ClearLOS(self, level.alertEvents[i].position) ==
            qfalse) { // no LOS, didn't hear it
          continue;
        }
      }
    }
    ...
    if (level.alertEvents[i].level >= bestAlert // higher alert level
        || (level.alertEvents[i].level == bestAlert &&
            level.alertEvents[i].timestamp >=
                bestTime)) // same alert level, but this one is newer
...
float G_GetLightLevel(vec3_t pos, vec3_t fromDir) {
  ...
  lightLevel = VectorLength(ambient) +
               (VectorLength(directed) * DotProduct(lightDir, fromDir));
...
-------------------------
*/
static int G_CheckSightEvents(gentity_t *self, int hFOV, int vFOV,
                              float maxSeeDist, int ignoreAlert,
...
qboolean G_RememberAlertEvent(gentity_t *self, int alertIndex) {
  if (!self || !self->NPC) { // not a valid ent
    return qfalse;
  }

  if (alertIndex == -1) { // not a valid event
    return qfalse;
  }
  ...
  if (at.ID == self->NPC->lastAlertID) { // already know this one
    return qfalse;
  }

  if (at.owner == self) { // don't care about events that I made
    return qfalse;
  }

  self->NPC->lastAlertID = at.ID;

  // Now, If It Is Dangerous Enough, We Want To Register This With The
  // Pathfinding System
  //--------------------------------------------------------------------------------------
  bool IsDangerous = (at.level >= AEL_DANGER);
  bool IsFromNPC = (at.owner && at.owner->client);
  bool IsFromEnemy =
      (IsFromNPC && at.owner->client->playerTeam != self->client->playerTeam);
...
int G_CheckAlertEvents(gentity_t *self, qboolean checkSight,
                       qboolean checkSound, float maxSeeDist, float maxHearDist,
                       int ignoreAlert, qboolean mustHaveOwner,
                       int minAlertLevel, qboolean onGroundOnly) {
  ...
  if (checkSound) {
    // get sound event
    bestSoundEvent =
        G_CheckSoundEvents(self, maxHearDist, ignoreAlert, mustHaveOwner,
                           minAlertLevel, onGroundOnly);
  ...
  if (checkSight) {
    ...
    if (self->NPC) {
      bestSightEvent = G_CheckSightEvents(
          self, self->NPC->stats.hfov, self->NPC->stats.vfov, maxSeeDist,
          ignoreAlert, mustHaveOwner, minAlertLevel);
    } else {
      bestSightEvent = G_CheckSightEvents(
          self, 80, 80, maxSeeDist, ignoreAlert, mustHaveOwner,
          minAlertLevel); // FIXME: look at cg_view to get more accurate
                          // numbers?
    }
    ...
    if (bestSightEvent >= 0 &&
        bestSightAlert > bestSoundAlert) { // valid best sight event, more
                                           // important than the sound event
      ...
      VectorSubtract(level.alertEvents[bestSightEvent].position, eyePoint,
                     sightDir);
...
int NPC_CheckAlertEvents(qboolean checkSight, qboolean checkSound,
                         int ignoreAlert, qboolean mustHaveOwner,
                         int minAlertLevel, qboolean onGroundOnly) {
  return G_CheckAlertEvents(NPC, checkSight, checkSound,
                            NPCInfo->stats.visrange, NPCInfo->stats.earshot,
                            ignoreAlert, mustHaveOwner, minAlertLevel,
                            onGroundOnly);
}
...
qboolean
G_CheckForDanger(gentity_t *self,
                 int alertEvent) { // FIXME: more bStates need to call this?
  if (alertEvent == -1) {
    return qfalse;
  }
  ...
  if (level.alertEvents[alertEvent].level >= AEL_DANGER) { // run away!
    if (!level.alertEvents[alertEvent].owner ||
        ...
        (level.alertEvents[alertEvent].owner != self &&
      ...
      if (self->NPC) {
        if (self->NPC->scriptFlags & SCF_DONT_FLEE) { // can't flee
          ...
          if (level.alertEvents[alertEvent].level >= AEL_DANGER_GREAT ||
              self->s.weapon == WP_NONE ||
              self->s.weapon == WP_MELEE) { // flee for a longer period of time
            NPC_StartFlee(level.alertEvents[alertEvent].owner,
                          level.alertEvents[alertEvent].position,
                          level.alertEvents[alertEvent].level, 3000, 6000);
          } else if (!Q_irand(0, 10)) // FIXME: base on rank?  aggression?
          { // just normal danger and I have a weapon, so just a 25% chance of
            // fleeing only for a few seconds
            // FIXME: used to just find a better combat point, need that
            // functionality back
            NPC_StartFlee(level.alertEvents[alertEvent].owner,
                          level.alertEvents[alertEvent].position,
...
qboolean
NPC_CheckForDanger(int alertEvent) { // FIXME: more bStates need to call this?
...
void AddSoundEvent(gentity_t *owner, vec3_t position, float radius,
                   alertEventLevel_e alertLevel, qboolean needLOS,
                   qboolean onGround) {
  ...
  // FIXME: if owner is not a player or player ally, and there are no player
  // allies present, 			perhaps we don't need to store the alert... unless we want
  //the player to 			react to enemy alert events in some way?

#ifdef _DEBUG
  assert(!Q_isnan(position[0]) && !Q_isnan(position[1]) &&
         !Q_isnan(position[2]));
#endif
  ...
  if (needLOS) { // a very low-level sound, when check this sound event, check
                 // for LOS
    level.alertEvents[level.numAlertEvents].addLight =
        1; // will force an LOS trace on this sound
  } else {
    level.alertEvents[level.numAlertEvents].addLight =
        0; // will force an LOS trace on this sound
  }
...
void AddSightEvent(gentity_t *owner, vec3_t position, float radius,
                   alertEventLevel_e alertLevel, float addLight) {
  // FIXME: Handle this in another manner?
  if (level.numAlertEvents >= MAX_ALERT_EVENTS) {
    if (!RemoveOldestAlert()) { // how could that fail?
      return;
  ...
  level.alertEvents[level.numAlertEvents].owner = owner;
  level.alertEvents[level.numAlertEvents].addLight =
      addLight; // will get added to actual light at that point when it's
                // checked
  level.alertEvents[level.numAlertEvents].ID = level.curAlertID++;
  level.alertEvents[level.numAlertEvents].timestamp = level.time;
...
void ClearPlayerAlertEvents(void) {
  ...
  for (int i = 0; i < curNumAlerts; i++) {
    ...
    if (level.alertEvents[i].timestamp &&
        level.alertEvents[i].timestamp + ALERT_CLEAR_TIME <
            level.time) { // this event has timed out
      // drop the count
      level.numAlertEvents--;
      // shift the rest down
  ...
  // make sure this never drops below zero... if it does, something very very
  // bad happened
  assert(level.numAlertEvents >= 0);

  if (eventClearTime <
      level.time) { // this is just a 200ms debouncer so things that generate
                    // constant alerts (like corpses and missiles) add an alert
                    // every 200 ms
...
qboolean G_ClearLOS(gentity_t *self, const vec3_t start, const vec3_t end) {
  ...
  gi.trace(&tr, start, NULL, NULL, end, ENTITYNUM_NONE,
           CONTENTS_OPAQUE /*CONTENTS_SOLID*/
           /*(CONTENTS_SOLID|CONTENTS_MONSTERCLIP)*/,
           (EG2_Collision)0, 0);
  while (tr.fraction < 1.0 &&
         traceCount < 3) { // can see through 3 panes of glass
    if (tr.entityNum < ENTITYNUM_WORLD) {
      if (&g_entities[tr.entityNum] != NULL &&
          (g_entities[tr.entityNum].svFlags &
           SVF_GLASS_BRUSH)) { // can see through glass, trace again, ignoring
                               // me
  ...
  for (i = 0; i < level.numInterestPoints; i++) {
    ...
    if (gi.inPVS(level.interestPoints[i].origin, eyes)) {
      ...
      if ((fabs(diffVec[0]) + fabs(diffVec[1])) / 2 < 48 &&
          fabs(diffVec[2]) > (fabs(diffVec[0]) + fabs(diffVec[1])) /
                                 2) { // Too close to look so far up or down
        continue;
      }
      dist = VectorLengthSquared(diffVec);
      // Some priority to more interesting points
      // dist -= ((int)level.interestPoints[i].lookMode * 5) *
      // ((int)level.interestPoints[i].lookMode * 5);
      if (dist < MAX_INTEREST_DIST && dist < bestDist) {
        if (G_ClearLineOfSight(eyes, level.interestPoints[i].origin,
                               self->s.number, MASK_OPAQUE)) {
          bestDist = dist;
          bestPoint = i;
        }
      }
...
void SP_target_interest(gentity_t *self) { // FIXME: rename point_interest
  ...
  VectorCopy(self->currentOrigin,
             level.interestPoints[level.numInterestPoints].origin);

  if (self->target && self->target[0]) {
    level.interestPoints[level.numInterestPoints].target =
        G_NewString(self->target);
  }
