...
//---------------------------------------------------------
{
  ...
  if (ent->disconnectDebounceTime &&
      ent->disconnectDebounceTime <
          level.time) {   // time's up, we're done, remove us
    if (ent->lockCount) { // explode when die
  ...
  if (ent->enemy && ent->enemy->inuse) {
    ...
    if (dot < 0.0f) {
      ...
      // slowing down coupled with fairly tight turns can lead us to orbit an
      // enemy..looks bad so don't do it!
      //			vel *= 0.5f;
    } else if (dot < 0.70f) {
...
//---------------------------------------------------------
{
  ...
  gentity_t *missile =
      CreateMissile(start, forwardVec, vel, 10000, ent, alt_fire);
  ...
  if (alt_fire) {
    ...
    if ((lockEntNum > 0 || (ent->NPC && lockEntNum >= 0)) &&
        lockEntNum < ENTITYNUM_WORLD && lockTime > 0) {
      // take our current lock time and divide that by 8 wedge slices to get the
      // current lock amount
      ...
      //	Also give a slight advantage to higher, but not quite full
      //charges. 	Finally, just give any amount of charge a very slight random
      //chance of locking.
      if (dif == 8 || Q_flrand(0.0f, 1.0f) * dif > 2 ||
          Q_flrand(0.0f, 1.0f) > 0.97f) {
        missile->enemy = &g_entities[lockEntNum];

        if (missile->enemy &&
            missile->enemy
                ->inuse) //&& DistanceSquared( missile->currentOrigin,
                         //missile->enemy->currentOrigin ) < 262144 && InFOV(
                         //missile->currentOrigin,
                         //missile->enemy->currentOrigin,
                         //missile->enemy->client->ps.viewangles, 45, 45 ) )
        {
          if (missile->enemy->client &&
              (missile->enemy->client->ps.forcePowersKnown & (1 << FP_PUSH)) &&
              missile->enemy->client->ps.forcePowerLevel[FP_PUSH] >
                  FORCE_LEVEL_0) { // have force push, don't flee from homing
                                   // rockets
          } else {
            vec3_t dir, dir2;
            ...
            if (DotProduct(dir, dir2) < 0.0f) {
              G_StartFlee(missile->enemy, ent, missile->enemy->currentOrigin,
