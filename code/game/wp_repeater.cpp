...
//---------------------------------------------------------
{
  ...
  if (ent->client && ent->client->NPC_class == CLASS_GALAKMECH) {
    missile = CreateMissile(start, ent->client->hiddenDir,
                            ent->client->hiddenDist, 10000, ent, qtrue);
  } else {
    WP_MissileTargetHint(ent, start, forwardVec);
    missile = CreateMissile(start, forwardVec, REPEATER_ALT_VELOCITY, 10000,
                            ent, qtrue);
  }
  ...
  VectorSet(missile->maxs, REPEATER_ALT_SIZE, REPEATER_ALT_SIZE,
            REPEATER_ALT_SIZE);
...
//---------------------------------------------------------
{
  ...
  if (alt_fire) {
    ...
    if (!(ent->client->ps.forcePowersActive & (1 << FP_SEE)) ||
      ...
      // so check for all classes of stormtroopers and anyone else that has aim
      // error
      if (ent->client && ent->NPC &&
        ...
        angs[PITCH] +=
            (Q_flrand(-1.0f, 1.0f) *
             (REPEATER_NPC_SPREAD + (6 - ent->NPC->currentAim) * 0.25f));
        angs[YAW] +=
            (Q_flrand(-1.0f, 1.0f) *
             (REPEATER_NPC_SPREAD + (6 - ent->NPC->currentAim) * 0.25f));
      } else {
