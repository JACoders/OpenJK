...
static const short objectiveStartingYpos =
    75; // Y starting position for objective text
static const short objectiveStartingXpos =
    60; // X starting position for objective text
...
const char *showLoadPowersName[] = {
    "SP_INGAME_HEAL2",          "SP_INGAME_JUMP2",
    "SP_INGAME_SPEED2",         "SP_INGAME_PUSH2",
    "SP_INGAME_PULL2",          "SP_INGAME_MINDTRICK2",
    "SP_INGAME_GRIP2",          "SP_INGAME_LIGHTNING2",
    "SP_INGAME_SABER_THROW2",   "SP_INGAME_SABER_OFFENSE2",
    "SP_INGAME_SABER_DEFENSE2", NULL,
};
...
====================
*/
static void ObjectivePrint_Line(const int color, const int objectIndex,
                                int &missionYcnt) {
  char *str, *strBegin;
  ...
  cgi_SP_GetStringTextString(
      va("OBJECTIVES_%s", objectiveTable[objectIndex].name), finalText,
      sizeof(finalText));
  ...
  if (objectIndex == T2_RANCOR_OBJ5) {
    ...
    gi.Cvar_VariableStringBuffer("ui_prisonerobj_currtotal", value,
                                 sizeof(value));
    currTotal = atoi(value);
    gi.Cvar_VariableStringBuffer("ui_prisonerobj_maxtotal", value,
                                 sizeof(value));
    minTotal = atoi(value);

    Q_strncpyz(finalText, va(finalText, currTotal, minTotal),
               sizeof(finalText));
  }
  ...
  if (cgi_Language_IsAsian()) {
    ...
    //
    extern const char *CG_DisplayBoxedText(
        int iBoxX, int iBoxY, int iBoxWidth, int iBoxHeight, const char *psText,
        int iFontHandle, float fScale, const vec4_t v4Color);
    ...
    CG_DisplayBoxedText(objectiveStartingXpos, y, objectiveTextBoxWidth,
                        objectiveTextBoxHeight,
                        finalText, // int iBoxX, int iBoxY, int iBoxWidth, int
                                   // iBoxHeight, const char *psText
    ...
    // western...
    //
    if (pixelLen < objectiveTextBoxWidth) // One shot - small enough to print
                                          // entirely on one line
    {
      y = objectiveStartingYpos + (iYPixelsPerLine * (missionYcnt));

      cgi_R_Font_DrawString(objectiveStartingXpos, y, str, colorTable[color],
    ...
    // Text is too long, break into lines.
      ...
      while (*str) {
        holdText2[0] = *str;
        pixelLen +=
            cgi_R_Font_StrLenPixels(holdText2, cgs.media.qhFontMedium, 1.0f);

        pixelLen += 2; // For kerning
        ++charLen;

        if (pixelLen >
            objectiveTextBoxWidth) { // Reached max length of this line
          ...
          CG_DrawProportionalString(objectiveStartingXpos, y, holdText,
                                    CG_SMALLFONT, colorTable[color]);
...
void CG_DrawDataPadObjectives(const centity_t *cent) {
  ...
  const short titleXPos =
      objectiveStartingXpos - 22; // X starting position for title text
  const short titleYPos =
      objectiveStartingYpos - 23; // Y starting position for title text
  const short graphic_size =
      16; // Size (width and height) of graphic used to show status of objective
  const short graphicXpos =
      objectiveStartingXpos - graphic_size -
      8; // Amount of X to backup from text starting position
  const short graphicYOffset =
      (iYPixelsPerLine - graphic_size) /
      2; // Amount of Y to raise graphic so it's in the center of the text line
  ...
  cgi_R_Font_DrawString(titleXPos, titleYPos, text, colorTable[CT_TITLE],
                        cgs.media.qhFontMedium, -1, 1.0f);
  ...
  for (i = 0; i < MAX_OBJECTIVES; i++) {
    ...
    if (cent->gent->client->sess.mission_objectives[i].display) {
      ...
      totalY = objectiveStartingYpos + (iYPixelsPerLine * (missionYcnt)) +
               (iYPixelsPerLine / 2);

      //	Draw graphics that show if mission has been accomplished or not
      cgi_R_SetColor(colorTable[CT_BLUE3]);
      CG_DrawPic((graphicXpos), (totalY - graphicYOffset), graphic_size,
                 graphic_size, cgs.media.messageObjCircle); // Circle in front
      if (cent->gent->client->sess.mission_objectives[i].status ==
          OBJECTIVE_STAT_SUCCEEDED) {
        CG_DrawPic((graphicXpos), (totalY - graphicYOffset), graphic_size,
                   graphic_size, cgs.media.messageLitOn); // Center Dot
      }
  ...
  // No mission text?
  if (!missionYcnt) {
    // Set the message a quarter of the way down and in the center of the text
    // box
    ...
    int messageXPosition =
        objectiveStartingXpos + (objectiveTextBoxWidth / 2) -
        (cgi_R_Font_StrLenPixels(text, cgs.media.qhFontMedium, 1.0f) / 2);

    cgi_R_Font_DrawString(messageXPosition, messageYPosition, text,
                          colorTable[CT_WHITE], cgs.media.qhFontMedium, -1,
                          1.0f);
  }
}
...
//-------------------------------------------------------
static void CG_DrawForceCount( const int force, int x, float *y, const int
pad,qboolean *hasForcePowers )
{
        ...
        // Draw title
        cgi_SP_GetStringTextString( showLoadPowersName[force], text,
sizeof(text) ); CG_DrawProportionalString( x, *y, text, CG_BIGFONT,
colorTable[textColor] );
        ...
        if ( val >= 0 )
        {
                ...
                for ( int i = 0; i < val; i++ )
                {
                        CG_DrawPic( x - iconSize - i * (iconSize + 10) , *y,
iconSize, iconSize, force_icons[force] );
                }
        }
...
static void CG_LoadScreen_PersonalInfo(void)
{
        ...
        if (hasForcePowers)
        {
                cgi_SP_GetStringTextString( "SP_INGAME_CURRENTFORCEPOWERS",
text, sizeof(text) ); CG_DrawProportionalString( 200, 65, text, CG_CENTER |
CG_BIGFONT, colorTable[CT_WHITE] );
        }
        else
        {	//you are only totally empty on the very first map?
//		cgi_SP_GetStringTextString( "SP_INGAME_NONE", text, sizeof(text)
);
//		CG_DrawProportionalString( 320, y+30, text, CG_CENTER |
CG_BIGFONT, colorTable[CT_ICON_BLUE] ); cgi_SP_GetStringTextString(
"SP_INGAME_ALONGTIME", text, sizeof(text) ); int w =
cgi_R_Font_StrLenPixels(text,cgs.media.qhFontMedium, 1.5f);
                cgi_R_Font_DrawString((320)-(w/2), y+40, text,
colorTable[CT_ICON_BLUE], cgs.media.qhFontMedium, -1, 1.5f);
        }
...
static void CG_LoadBar(void) {
  ...
  const int barwidth = numticks * tickwidth + tickpadx * 2 + capwidth * 2,
            barleft = ((640 - barwidth) / 2);
  const int barheight = tickheight + tickpady * 2, bartop = 475 - barheight;
  const int capleft = barleft + tickpadx, tickleft = capleft + capwidth,
            ticktop = bartop + tickpady;
  ...
  // Draw right cap
  CG_DrawPic(tickleft + tickwidth * cg.loadLCARSStage, ticktop, capwidth,
             tickheight, cgs.media.loadTickCap);
}
...
static int CG_DrawLoadWeaponsPrintRow(const char *itemName, int weaponsBits,
                                      int rowIconCnt, int startIndex) {
  ...
  if (!cgi_UI_GetMenuItemInfo("loadScreen", itemName, &x, &y, &width, &height,
                              color, &background)) {
    return (0);
  }
  ...
  holdX =
      x + (width - ((iconSize * rowIconCnt) + (pad * (rowIconCnt - 1)))) / 2;
  ...
  for (i = startIndex; i < MAXLOADWEAPONS; i++) {
    ...
    if (weaponData[i].weaponIcon[0]) {
      ...
      //		if (!CG_WeaponCheck(i))
      //		{
      //			CG_DrawPic( holdX, y+yOffset, iconSize,
      //iconSize, weaponInfo->weaponIconNoAmmo );
      //		}
      //		else
      {
        CG_DrawPic(holdX, y + yOffset, iconSize, iconSize,
...
static void CG_DrawLoadWeapons(int weaponBits) {
  ...
  // Two lines of icons
    ...
    // Print top row
    endIndex = CG_DrawLoadWeaponsPrintRow("weaponicons_row1", weaponBits,
                                          MAXLOADICONSPERROW, 0);

    // Print second row
    rowIconCnt = iconCnt - MAXLOADICONSPERROW;
    CG_DrawLoadWeaponsPrintRow("weaponicons_row2", weaponBits, rowIconCnt,
                               endIndex + 1);
  }
...
static int CG_DrawLoadForcePrintRow(const char *itemName, int forceBits,
                                    int rowIconCnt, int startIndex) {
  int i, endIndex = 0, printedIconCnt = 0;
  int holdX, x, y;
  int yOffset = 0;
  ...
  // calculate placement of weapon icons
  holdX = x + (width - ((MAXLOAD_FORCEICONSIZE * rowIconCnt) +
                        (MAXLOAD_FORCEICONPAD * (rowIconCnt - 1)))) /
                  2;
  ...
  for (i = startIndex; i < MAX_SHOWPOWERS; i++) {
    ...
    if (force_icons[showPowers[i]]) {
      ...
      CG_DrawPic(holdX, y + yOffset, MAXLOAD_FORCEICONSIZE,
                 MAXLOAD_FORCEICONSIZE, force_icons[showPowers[i]]);
...
static void CG_DrawLoadForcePowers(int forceBits) {
  ...
  // Two lines of icons
    ...
    // Print top row
    endIndex = CG_DrawLoadForcePrintRow("forceicons_row1", forceBits,
                                        MAXLOADICONSPERROW, 0);

    // Print second row
    rowIconCnt = iconCnt - MAXLOADICONSPERROW;
    CG_DrawLoadForcePrintRow("forceicons_row2", forceBits, rowIconCnt,
                             endIndex + 1);
  }
...
static void CG_DrawLoadingScreen(qhandle_t levelshot, const char *mapName) {
  ...
  // Print background
  if (cgi_UI_GetMenuItemInfo("loadScreen", "background", &xPos, &yPos, &width,
  ...
  // Print level pic
  if (cgi_UI_GetMenuItemInfo("loadScreen", "mappic", &xPos, &yPos, &width,
                             &height, color, &background)) {
    // if (!levelshot)
    //{// No level shot so use screenshot.
    //		CG_DrawPic( xPos, yPos, 1, 1, 0);	//force the tess to
    //flush 	cgi_R_DrawScreenShot( xPos, yPos+height, width, -height );
    //}
    // else
    {
      cgi_R_SetColor(color);
...
void CG_DrawInformation(void) {
  ...
  extern SavedGameJustLoaded_e
      g_eSavedGameJustLoaded; // hack! (hey, it's the last week of coding, ok?
                              //	if ( g_eSavedGameJustLoaded == eFULL )
                              //	{
  //		levelshot = 0;	//use the loaded thumbnail instead of the
  //levelshot
  //	}
  //	else
  {
      ...
      !strcmp(s, "yavin1")) // special case for first map!
  {
    ...
    cgi_R_Font_DrawString((320) - (w / 2), 140, text, colorTable[CT_ICON_BLUE],
                          cgs.media.qhFontMedium, -1, 1.0f);
  } else {
...
#ifdef _DEBUG
    cgi_R_Font_DrawString(40, 416, va("LOADING ... %s", cg.infoScreenText),
                          colorTable[CT_LTGOLD1], cgs.media.qhFontSmall, -1,
                          1.0f);
#endif
  }
  ...
  if (s[0]) {
    if (s[0] == '@') {
      ...
      cgi_R_Font_DrawString(15, y, va("\"%s\"", text), colorTable[CT_WHITE],
                            cgs.media.qhFontMedium, -1, 1.0f);
    } else {
      cgi_R_Font_DrawString(15, y, va("\"%s\"", s), colorTable[CT_WHITE],
                            cgs.media.qhFontMedium, -1, 1.0f);
    }
    y += 20;
  }
