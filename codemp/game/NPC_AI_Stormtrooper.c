...
extern void AI_GroupUpdateSquadstates(AIGroupInfo_t *group, gentity_t *member,
                                      int newSquadState);
...
#define DISTANCE_SCALE                                                         \
  0.35f // These first three get your base detection rating, ideally add up to 1
...
int groupSpeechDebounceTime[TEAM_NUM_TEAMS]; // used to stop several group AI
...
static void ST_Speech(gentity_t *self, int speechType, float failChance) {
  ...
  if (failChance >= 0) {    // a negative failChance makes it always talk
    if (self->NPC->group) { // group AI speech debounce timer
      ...
      {
              if ( groupSpeechDebounceTime[self->client->playerTeam] >
      level.time )
              {
                      return;
              }
      ...
      */
      ...
      return;
    } else if (groupSpeechDebounceTime[self->client->playerTeam] >
               level.time) { // for those not in group AI
      // FIXME: let certain speech types interrupt others?  Let closer NPCs
      // interrupt farther away ones?
      return;
    }
  }

  if (self->NPC->group) { // So they don't all speak at once...
    // FIXME: if they're not yet mad, they have no group, so distracting a group
    // of them makes them all speak!
    self->NPC->group->speechDebounceTime = level.time + Q_irand(2000, 4000);
  } else {
    TIMER_Set(self, "chatter", Q_irand(2000, 4000));
  }
  groupSpeechDebounceTime[self->client->playerTeam] =
...
void ST_StartFlee(gentity_t *self, gentity_t *enemy, vec3_t dangerPoint,
                  int dangerLevel, int minTime, int maxTime) {
  if (!self || !self->NPC) {
    return;
  }
...
static void ST_HoldPosition(void) {
  ...
  TIMER_Set(
      NPCS.NPC, "verifyCP",
      Q_irand(1000, 3000)); // don't look for another one for a few seconds
  NPC_FreeCombatPoint(NPCS.NPCInfo->combatPoint, qtrue);
  // NPCInfo->combatPoint = -1;//???
  if (!trap->ICARUS_TaskIDPending(
          (sharedEntity_t *)NPCS.NPC,
          TID_MOVE_NAV)) { // don't have a script waiting for me to get to my
                           // point, okay to stop trying and stand
    AI_GroupUpdateSquadstates(NPCS.NPCInfo->group, NPCS.NPC,
...
void NPC_ST_SayMovementSpeech(void) {
  ...
  if (NPCS.NPCInfo->group && NPCS.NPCInfo->group->commander &&
    ...
    ST_Speech(NPCS.NPCInfo->group->commander, NPCS.NPCInfo->movementSpeech,
              NPCS.NPCInfo->movementSpeechChance);
  } else { // really don't want to say this unless we can actually get there...
    ST_Speech(NPCS.NPC, NPCS.NPCInfo->movementSpeech,
              NPCS.NPCInfo->movementSpeechChance);
  }
...
static qboolean ST_Move(void) {
  ...
  // just stop! If we hit our target, then stop and fire!
  ...
  if (moved == qfalse) { // FIXME: if we're going to a combat point, need to
                         // pick a different one
    if (!trap->ICARUS_TaskIDPending(
            (sharedEntity_t *)NPCS.NPC,
            TID_MOVE_NAV)) { // can't transfer movegoal or stop when a script
                             // we're running is waiting to complete
      if (info.blocker && info.blocker->NPC && NPCS.NPCInfo->group != NULL &&
          info.blocker->NPC->group ==
              NPCS.NPCInfo->group) //(NPCInfo->aiFlags&NPCAI_BLOCKED) &&
                                   //NPCInfo->group != NULL )
      {                            // dammit, something is in our way
        ...
        for (j = 0; j < NPCS.NPCInfo->group->numGroup; j++) {
          if (NPCS.NPCInfo->group->member[j].number ==
              NPCS.NPCInfo->blockingEntNum) { // we're being blocked by one of
                                              // our own, pass our goal onto them
                                              // and I'll stand still
            ST_TransferMoveGoal(
                NPCS.NPC, &g_entities[NPCS.NPCInfo->group->member[j].number]);
            break;
          }
        }
      }
...
static void NPC_ST_SleepShuffle(void) {
  ...
  if (TIMER_Done(NPCS.NPC, "shuffleTime")) {
    ...
    case 0:
            G_Sound( NPC,
    G_SoundIndex("sound/chars/imperialsleeper1/scav4/hunh.mp3") ); break;

    case 1:
            G_Sound( NPC,
    G_SoundIndex("sound/chars/imperialsleeper3/scav4/tryingtosleep.wav") );
            break;
    }
    */
...
void NPC_BSST_Sleep(void) {
  int alertEvent =
      NPC_CheckAlertEvents(qfalse, qtrue, -1, qfalse,
                           AEL_MINOR); // only check sounds since we're alseep!
...
qboolean NPC_CheckEnemyStealth(gentity_t *target) {
  float target_dist, minDist = 40; // any closer than 40 and we definitely
                                   // notice
  float maxViewDist;
  qboolean clearLOS;
  ...
  if (target->client->ps.weapon == WP_SABER &&
      !target->client->ps.saberHolstered &&
      !target->client->ps
           .saberInFlight) { // if target has saber in hand and activated, we
                             // wake up even sooner even if not facing him
    minDist = 100;
  }

  target_dist =
      DistanceSquared(target->r.currentOrigin, NPCS.NPC->r.currentOrigin);
  ...
  if (NPCS.NPCInfo->stats.visrange >
      maxViewDist) { // FIXME: should we always just set maxViewDist to this?
  ...
  // Check FOV first
  if (InFOV(target, NPCS.NPC, NPCS.NPCInfo->stats.hfov,
            NPCS.NPCInfo->stats.vfov) == qfalse)
    return qfalse;

  // clearLOS = ( target->client->ps.leanofs ) ? NPC_ClearLOS5(
  // target->client->renderInfo.eyePoint ) : NPC_ClearLOS4( target );
  ...
  if (clearLOS) {
    ...
    hAngle_perc = NPC_GetHFOVPercentage(
        targ_org, NPCS.NPC->client->renderInfo.eyePoint,
        NPCS.NPC->client->renderInfo.eyeAngles, NPCS.NPCInfo->stats.hfov);
    vAngle_perc = NPC_GetVFOVPercentage(
        targ_org, NPCS.NPC->client->renderInfo.eyePoint,
    ...
    turning_rating = 5.0f; // AngleDelta( target->client->ps.viewangles[PITCH],
                           // target->lastAngles[PITCH] )/180.0f + AngleDelta(
                           // target->client->ps.viewangles[YAW],
                           // target->lastAngles[YAW] )/180.0f;
    light_level =
        (255 /
         MAX_LIGHT_INTENSITY); //( target->lightLevel / MAX_LIGHT_INTENSITY );
    FOV_perc = 1.0f - (hAngle_perc + vAngle_perc) *
                          0.5f; // FIXME: Dunno about the average...
    vis_rating = 0.0f;
    ...
    //...As the percentage out of the FOV increases, straight perception suffers
    ...
    if (contents & CONTENTS_WATER) {
      int myContents = trap->PointContents(
          NPCS.NPC->client->renderInfo.eyePoint, NPCS.NPC->s.number);
      if (!(myContents & CONTENTS_WATER)) { // I'm not in water
        if (NPCS.NPC->client->NPC_class ==
            CLASS_SWAMPTROOPER) { // these guys can see in in/through water
                                  // pretty well
        ...
        if (NPCS.NPC->client->NPC_class ==
            CLASS_SWAMPTROOPER) { // I can see him just fine
        } else {
    ...
    // FIXME: check to see if they're animating, too?  But can we do something
    // as simple as frame != oldframe?
    ...
    // float difficulty_scale = 1.0f + (2.0f-g_npcspskill.value);//if playing on
    // easy, 20% harder to be seen...?
    if (NPCS.NPC->client->NPC_class ==
        CLASS_SWAMPTROOPER) { // swamptroopers can see much better
    ...
    // If he's above the caution threshold, then realize him in a few seconds
    // unless he moves to cover
    if (target_rating > cautious &&
        !(NPCS.NPCInfo->scriptFlags &
          SCF_IGNORE_ALERTS)) { // FIXME: ambushing guys should never talk
      if (TIMER_Done(
              NPCS.NPC,
              "enemyLastVisible")) { // If we haven't already, start the counter
                  ...
                  SCF_LOOK_FOR_ENEMIES)) // FIXME: Is this reliable?
      {
        if (NPCS.NPCInfo->rank < RANK_LT && !Q_irand(0, 2)) {
          ...
          G_SetEnemy(NPCS.NPC, target);
          NPCS.NPCInfo->enemyLastSeenTime = level.time;
          // FIXME: ambush guys (like those popping out of water) shouldn't
          // delay...
          TIMER_Set(NPCS.NPC, "attackDelay", Q_irand(500, 2500));
...
qboolean NPC_CheckPlayerTeamStealth(void) {
  ...
  NPC_CheckEnemyStealth( &g_entities[0] );	//Change this pointer to assess
  other entities
  */
  gentity_t *enemy;
  int i;
  ...
  for (i = 0; i < ENTITYNUM_WORLD; i++) {
    ...
    if (enemy && enemy->client && NPC_ValidEnemy(enemy) &&
      ...
      if (NPC_CheckEnemyStealth(
              enemy)) // Change this pointer to assess other entities
      {
        return qtrue;
      }
    }
...
static qboolean NPC_ST_InvestigateEvent(int eventID, qboolean extraSuspicious) {
  ...
  if (NPCS.NPCInfo->confusionTime < level.time) {
    if (level.alertEvents[eventID].level == AEL_DISCOVERED &&
      ...
      if (!level.alertEvents[eventID].owner ||
          ...
          level.alertEvents[eventID].owner->client->playerTeam !=
              NPCS.NPC->client->enemyTeam) { // not an enemy
        return qfalse;
      }
      // FIXME: what if can't actually see enemy, don't know where he is...
      // should we make them just become very alert and start looking for him? Or
      // just let combat AI handle this... (act as if you lost him) ST_Speech(
      // NPC, SPEECH_CHARGE, 0 );
      G_SetEnemy(NPCS.NPC, level.alertEvents[eventID].owner);
      NPCS.NPCInfo->enemyLastSeenTime = level.time;
      TIMER_Set(NPCS.NPC, "attackDelay", Q_irand(500, 2500));
      if (level.alertEvents[eventID].type ==
          AET_SOUND) { // heard him, didn't see him, stick for a bit
  ...
  */
  ...
  if (level.alertEvents[eventID].type ==
      AET_SIGHT) { // sight alert, check the light level
    if (level.alertEvents[eventID].light <
        Q_irand(ST_MIN_LIGHT_THRESHOLD,
                ST_MAX_LIGHT_THRESHOLD)) { // below my threshhold of potentially
                                           // seeing
      return qfalse;
    }
  }
  ...
  VectorCopy(level.alertEvents[eventID].position,
  ...
  if (level.alertEvents[eventID].level > AEL_MINOR &&
      NPCS.NPCInfo->investigateCount > 1 &&
      (NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) {
    // make it so they can walk right to this point and look at it rather than
    // having to use combatPoints
    if (G_ExpandPointToBBox(
            NPCS.NPCInfo->investigateGoal, NPCS.NPC->r.mins, NPCS.NPC->r.maxs,
            NPCS.NPC->s.number,
            ((NPCS.NPC->clipmask & ~CONTENTS_BODY) |
             CONTENTS_BOTCLIP))) { // we were able to move the investigateGoal
                                   // to a point in which our bbox would fit
      // drop the goal to the ground so we can get at it
      vec3_t end;
      trace_t trace;
      VectorCopy(NPCS.NPCInfo->investigateGoal, end);
      end[2] -=
          512; // FIXME: not always right?  What if it's even higher, somehow?
      trap->Trace(&trace, NPCS.NPCInfo->investigateGoal, NPCS.NPC->r.mins,
                  NPCS.NPC->r.maxs, end, ENTITYNUM_NONE,
      ...
      if (trace.fraction >= 1.0f) { // too high to even bother
        ...
        NPC_SetMoveGoal(NPCS.NPC, NPCS.NPCInfo->investigateGoal, 16, qtrue, -1,
                        NULL);
        NPCS.NPCInfo->localState = LSTATE_INVESTIGATE;
      }
    } else {
      int id = NPC_FindCombatPoint(
          NPCS.NPCInfo->investigateGoal, NPCS.NPCInfo->investigateGoal,
          NPCS.NPCInfo->investigateGoal, CP_INVESTIGATE | CP_HAS_ROUTE, 0, -1);

      if (id != -1) {
        NPC_SetMoveGoal(NPCS.NPC, level.combatPoints[id].origin, 16, qtrue, id,
                        NULL);
        NPCS.NPCInfo->localState = LSTATE_INVESTIGATE;
      }
    }
    ...
    if (NPCS.NPCInfo->investigateDebounceTime + NPCS.NPCInfo->pauseTime >
        level.time) { // was already investigating
      if (NPCS.NPCInfo->group && NPCS.NPCInfo->group->commander &&
          NPCS.NPCInfo->group->commander->client &&
          NPCS.NPCInfo->group->commander->client->NPC_class == CLASS_IMPERIAL &&
          !Q_irand(0, 3)) {
        ST_Speech(NPCS.NPCInfo->group->commander, SPEECH_LOOK,
                  0); // FIXME: "I'll go check it out" type sounds
      } else {
        ST_Speech(NPCS.NPC, SPEECH_LOOK,
                  0); // FIXME: "I'll go check it out" type sounds
      }
    } else {
    ...
    NPCS.NPCInfo->investigateDebounceTime =
        NPCS.NPCInfo->investigateCount * 5000;
    ...
    // Setup the debounce info
    NPCS.NPCInfo->investigateDebounceTime =
        NPCS.NPCInfo->investigateCount * 1000;
...
static void ST_OffsetLook(float offset, vec3_t out) {
  ...
  GetAnglesForDirection(NPCS.NPC->r.currentOrigin,
                        NPCS.NPCInfo->investigateGoal, angles);
  angles[YAW] += offset;
...
static void ST_LookAround(void) {
  vec3_t lookPos;
  float perc = (float)(level.time - NPCS.NPCInfo->pauseTime) /
               (float)NPCS.NPCInfo->investigateDebounceTime;
...
void NPC_BSST_Investigate(void) {
  ...
  if (!(NPCS.NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
    int alertEvent = NPC_CheckAlertEvents(
        qtrue, qtrue, NPCS.NPCInfo->lastAlertID, qfalse, AEL_MINOR);
  ...
  if (NPCS.NPCInfo->localState == LSTATE_INVESTIGATE &&
      (NPCS.NPCInfo->goalEntity != NULL)) {
    // See if we're there
    if (NAV_HitNavGoal(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins,
                       NPCS.NPC->r.maxs,
                       NPCS.NPCInfo->goalEntity->r.currentOrigin, 32,
                       FlyingCreature(NPCS.NPC)) == qfalse) {
...
void NPC_BSST_Patrol(void) { // FIXME: pick up on bodies of dead buddies?
  ...
  if (!(NPCS.NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
    ...
    // There is an event to look at
    if (alertEvent >= 0) {
      if (NPC_ST_InvestigateEvent(alertEvent,
                                  qfalse)) { // actually going to investigate it
        NPC_UpdateAngles(qtrue, qtrue);
        return;
      }
    }
  ...
  } else // if ( !(NPCInfo->scriptFlags&SCF_IGNORE_ALERTS) )
  {
    if (NPCS.NPC->client->NPC_class != CLASS_IMPERIAL &&
        NPCS.NPC->client->NPC_class !=
            CLASS_IMPWORKER) { // imperials do not look around
      if (TIMER_Done(NPCS.NPC,
                     "enemyLastVisible")) { // nothing suspicious, look around
        if (!Q_irand(0, 30)) {
  ...
  if (NPCS.NPC->client->NPC_class == CLASS_IMPERIAL ||
      NPCS.NPC->client->NPC_class == CLASS_IMPWORKER) { // hack
    if (NPCS.ucmd.forwardmove || NPCS.ucmd.rightmove ||
        NPCS.ucmd.upmove) { // moving

      if ((NPCS.NPC->client->ps.torsoTimer <= 0) ||
          (NPCS.NPC->client->ps.torsoAnim == BOTH_STAND4)) {
        if ((NPCS.ucmd.buttons & BUTTON_WALKING) &&
            !(NPCS.NPCInfo->scriptFlags &
              SCF_RUNNING)) { // not running, only set upper anim
          //  No longer overrides scripted anims
          NPC_SetAnim(NPCS.NPC, SETANIM_TORSO, BOTH_STAND4,
      ...
      if ((NPCS.NPC->client->ps.torsoTimer <= 0 ||
          ...
          (NPCS.NPC->client->ps.legsTimer <= 0 ||
           (NPCS.NPC->client->ps.legsAnim == BOTH_STAND4))) {
        NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_STAND4,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        NPCS.NPC->client->ps.torsoTimer = NPCS.NPC->client->ps.legsTimer = 200;
      }
    }
    // FIXME: this is a disgusting hack that is supposed to make the Imperials
    // start with their weapon holstered- need a better way
...
static void ST_CheckMoveState(void) {
  ...
  if (trap->ICARUS_TaskIDPending(
          (sharedEntity_t *)NPCS.NPC,
          TID_MOVE_NAV)) { // moving toward a goal that a script is waiting on,
                           // so don't stop for anything!
    move = qtrue;
  }
  // See if we're a scout
    ...
    {//we can't scout to him, someone else give it a try
            AI_GroupUpdateSquadstates( NPCInfo->group, NPC,
    SQUAD_STAND_AND_SHOOT ); TIMER_Set( NPC, "roamTime", Q_irand( 1000, 2000 )
    ); move = qfalse; return;
    }
    */
  ...
  // see if we're heading to some other combatPoint
    ...
    // ucmd.buttons |= BUTTON_CAREFUL;
    if (!NPCS.NPCInfo
             ->goalEntity) { // um, lost our goal?  Just stand and shoot, then
      NPCS.NPCInfo->squadState = SQUAD_STAND_AND_SHOOT;
    }
  }
  ...
  else if (NPCS.NPCInfo->squadState ==
           SQUAD_STAND_AND_SHOOT) { // from this squadState we can transition to
                                    // others?
    move = qfalse;
    return;
  }
  ...
  if ((NPCS.NPCInfo->goalEntity != NPCS.NPC->enemy) &&
    ...
    if (NAV_HitNavGoal(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins,
                       NPCS.NPC->r.maxs,
                       NPCS.NPCInfo->goalEntity->r.currentOrigin, 16,
                       FlyingCreature(NPCS.NPC)) ||
        (!trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC,
                                     TID_MOVE_NAV) &&
         NPCS.NPCInfo->squadState == SQUAD_SCOUT && enemyLOS &&
         enemyDist <= 10000)) { // either hit our navgoal or our navgoal was not
                                // a crucial (scripted) one (maybe a combat
                                // point) and we're scouting and found our enemy
      ...
      case SQUAD_RETREAT: // was running away
        ...
        TIMER_Set(NPCS.NPC, "duck",
                  (NPCS.NPC->client->pers.maxHealth - NPCS.NPC->health) * 100);
      ...
      TIMER_Set(NPCS.NPC, "attackDelay",
                Q_irand(250, 500)); // FIXME: Slant for difficulty levels
...
void ST_ResolveBlockedShot(int hit) {
  ...
  if (TIMER_Done(NPCS.NPC, "duck")) { // we're not ducking
    ...
    if (TIMER_Done(NPCS.NPC, "stand")) { // stand for as long as we'll be here
...
static void ST_CheckFireState(void) {
  ...
  if (NPCS.NPCInfo->squadState == SQUAD_RETREAT ||
      ...
      NPCS.NPCInfo->squadState ==
          SQUAD_SCOUT) { // runners never try to fire at the last pos
    return;
  }

  if (!VectorCompare(NPCS.NPC->client->ps.velocity,
                     vec3_origin)) { // if moving at all, don't do this
    return;
  }
  ...
  if (!hitAlly // we're not going to hit an ally
      ...
      && (NPCS.NPCInfo->group->numState[SQUAD_RETREAT] > 0 ||
          NPCS.NPCInfo->group->numState[SQUAD_TRANSITION] > 0 ||
          NPCS.NPCInfo->group->numState[SQUAD_SCOUT] >
              0)) // laying down covering fire
  {
    if (level.time - NPCS.NPCInfo->enemyLastSeenTime <
        ...
        (!NPCS.NPCInfo->group ||
         level.time - NPCS.NPCInfo->group->lastSeenEnemyTime <
             10000)) // we are not in a group or the group has seen the enemy in
                     // the last 10 seconds
    {
      if (!Q_irand(0, 10)) {
        ...
        if (dist < distThreshold) { // impact would be too close to me
          ...
          dist =
              DistanceSquared(impactPos, NPCS.NPCInfo->enemyLastSeenLocation);
...
void ST_TrackEnemy(gentity_t *self, vec3_t enemyPos) {
  ...
  TIMER_Set(self, "scoutTime",
            TIMER_Get(self, "stick") - level.time + Q_irand(5000, 10000));
...
void ST_HuntEnemy(gentity_t *self) {
  // TIMER_Set( NPC, "attackDelay", Q_irand( 250, 500 ) );//Disabled this for
  // now, guys who couldn't hunt would never attack TIMER_Set( NPC, "duck", -1 );
...
void ST_TransferMoveGoal(gentity_t *self, gentity_t *other) {
  if (trap->ICARUS_TaskIDPending(
          (sharedEntity_t *)self,
          TID_MOVE_NAV)) { // can't transfer movegoal when a script we're
                           // running is waiting to complete
    return;
  }
  if (self->NPC->combatPoint !=
      -1) { // I've got a combatPoint I'm going to, give it to him
    self->NPC->lastFailedCombatPoint = other->NPC->combatPoint =
        self->NPC->combatPoint;
    self->NPC->combatPoint = -1;
  } else { // I must be going for a goal, give that to him instead
    if (self->NPC->goalEntity == self->NPC->tempGoal) {
      NPC_SetMoveGoal(
          other, self->NPC->tempGoal->r.currentOrigin, self->NPC->goalRadius,
          ((self->NPC->tempGoal->flags & FL_NAVGOAL) ? qtrue : qfalse), -1,
          NULL);
    } else {
...
int ST_GetCPFlags(void) {
  ...
  if (NPCS.NPC && NPCS.NPCInfo->group) {
    if (NPCS.NPC == NPCS.NPCInfo->group->commander &&
        NPCS.NPC->client->NPC_class ==
            CLASS_IMPERIAL) { // imperials hang back and give orders
      if (NPCS.NPCInfo->group->numGroup > 1 &&
          Q_irand(-3, NPCS.NPCInfo->group->numGroup) >
              1) { // FIXME: make sure he;s giving orders with these lines
        if (Q_irand(0, 1)) {
    ...
    } else if (NPCS.NPCInfo->group->morale < 0) { // hide
      cpFlags = (CP_COVER | CP_AVOID | CP_SAFE | CP_RETREAT);
    } else if (NPCS.NPCInfo->group->morale <
               NPCS.NPCInfo->group->numGroup) { // morale is low for our size
      int moraleDrop =
          NPCS.NPCInfo->group->numGroup - NPCS.NPCInfo->group->morale;
      ...
      int moraleBoost =
          NPCS.NPCInfo->group->morale - NPCS.NPCInfo->group->numGroup;
      if (moraleBoost > 20) { // charge to any one and outflank (no cover
                              // needed)
...
void ST_Commander(void) {
  ...
  if (group->enemy == NULL ||
      group->enemy->client == NULL) { // hmm, no enemy...?!
    return;
  }

  // FIXME: have this group commander check the enemy group (if any) and see if
  // they have 		superior numbers.  If they do, fall back rather than advance.  If
  //you have 		superior numbers, advance on them.
  // FIXME: find the group commander and have him occasionally give orders when
  // there is speech
  // FIXME: start fleeing when only a couple of you vs. a lightsaber, possibly
  // give up if the only one left
  ...
  if (group->lastSeenEnemyTime < level.time - 180000) { // dissolve the group
    ...
    for (i = 0; i < group->numGroup; i++) {
      ...
      if (trap->ICARUS_TaskIDPending(
              (sharedEntity_t *)NPCS.NPC,
              TID_MOVE_NAV)) { // running somewhere that a script requires us to
                               // go, don't break from that
        continue;
      }
      if (!(NPCS.NPCInfo->scriptFlags &
            SCF_CHASE_ENEMIES)) { // not allowed to move on my own
        continue;
      }
      // Lost enemy for three minutes?  go into search mode?
      G_ClearEnemy(NPCS.NPC);
      NPCS.NPC->waypoint =
          NAV_FindClosestWaypointForEnt(NPCS.NPC, group->enemy->waypoint);
      if (NPCS.NPC->waypoint == WAYPOINT_NONE) {
        NPCS.NPCInfo->behaviorState = BS_DEFAULT; // BS_PATROL;
                 ...
                 (trap->Nav_GetPathCost(NPCS.NPC->waypoint,
                                        group->enemy->waypoint) >=
                  Q3_INFINITE)) {
  ...
  */
  ...
  if (/*!runner &&*/ group->lastSeenEnemyTime > level.time - 32000 &&
      group->lastSeenEnemyTime <
          level.time - 30000) { // no-one has seen the enemy for 30 seconds//
                                // and no-one is running after him
  ...
  if (group->lastSeenEnemyTime <
      level.time - 10000) { // no-one has seen the enemy for at least 10
                            // seconds!  Should send a scout
    enemyLost = qtrue;
  }

  if (group->lastClearShotTime <
      level.time - 5000) { // no-one has had a clear shot for 5 seconds!
  ...
  for (i = curMemberNum; i < lastMemberNum; i++) {
    ...
    if (!TIMER_Done(NPCS.NPC, "flee")) { // running away
      continue;
    }

    if (trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC,
                                   TID_MOVE_NAV)) { // running somewhere that a
                                                    // script requires us to go
      continue;
    }

    if (NPCS.NPC->s.weapon == WP_NONE && NPCS.NPCInfo->goalEntity &&
        NPCS.NPCInfo->goalEntity == NPCS.NPCInfo->tempGoal &&
        NPCS.NPCInfo->goalEntity->enemy &&
        NPCS.NPCInfo->goalEntity->enemy->s.eType ==
            ET_ITEM) { // running to pick up a gun, don't do other logic
      continue;
    }

    // see if this member should start running (only if have no officer...
    // FIXME: should always run from AEL_DANGER_GREAT?)
    if (!group->commander || group->commander->NPC->rank < RANK_ENSIGN) {
      if (NPC_CheckForDanger(NPC_CheckAlertEvents(
              qtrue, qtrue, -1, qfalse, AEL_DANGER))) { // going to run
    ...
    if (!(NPCS.NPCInfo->scriptFlags &
          SCF_CHASE_ENEMIES)) { // not allowed to do combat-movement
      continue;
    }

    // check the local state
    if (NPCS.NPCInfo->squadState != SQUAD_RETREAT) { // not already retreating
      if (NPCS.NPC->client->ps.weapon ==
          WP_NONE) { // weaponless, should be hiding
        if (NPCS.NPCInfo->goalEntity == NULL ||
            NPCS.NPCInfo->goalEntity->enemy == NULL ||
            NPCS.NPCInfo->goalEntity->enemy->s.eType !=
                ET_ITEM) { // not running after a pickup
          if (TIMER_Done(NPCS.NPC, "hideTime") ||
              (DistanceSquared(group->enemy->r.currentOrigin,
                               NPCS.NPC->r.currentOrigin) < 65536 &&
               NPC_ClearLOS4(NPCS.NPC->enemy))) { // done hiding or enemy near
                                                  // and can see us
            // er, start another flee I guess?
            NPC_StartFlee(NPCS.NPC->enemy, NPCS.NPC->enemy->r.currentOrigin,
      ...
      if (TIMER_Done(NPCS.NPC, "roamTime") &&
          TIMER_Done(NPCS.NPC, "hideTime") && NPCS.NPC->health > 10 &&
          !trap->InPVS(group->enemy->r.currentOrigin,
                       NPCS.NPC->r.currentOrigin)) { // cant even see enemy
        // better go after him
        cpFlags |= (CP_CLEAR | CP_COVER);
      } else if (NPCS.NPCInfo->localState ==
                 LSTATE_UNDERFIRE) { // we've been shot
        switch (group->enemy->client->ps.weapon) {
        case WP_SABER:
          if (DistanceSquared(group->enemy->r.currentOrigin,
                              NPCS.NPC->r.currentOrigin) < 65536) // 256 squared
          {
        ...
        if (trap->InPVS(
                ...
                group->enemy->r.currentOrigin)) { // in the same room as enemy
          if (NPCS.NPC->client->ps.weapon == WP_ROCKET_LAUNCHER &&
              DistanceSquared(group->enemy->r.currentOrigin,
                              NPCS.NPC->r.currentOrigin) <
                  MIN_ROCKET_DIST_SQUARED &&
              NPCS.NPCInfo->squadState !=
                  SQUAD_TRANSITION) { // too close for me to fire my weapon and
                                      // I'm not already on the move
            ...
            case WP_SABER:
              ...
              if (!group->enemy->client->ps.saberHolstered) {
                if (DistanceSquared(group->enemy->r.currentOrigin,
                                    NPCS.NPC->r.currentOrigin) < 65536) {
                  if (TIMER_Done(NPCS.NPC, "hideTime")) {
                    if (NPCS.NPCInfo->squadState !=
                        SQUAD_TRANSITION) { // not already moving: FIXME: we
                                            // need to see if where we're going
                                            // is good now?
                      cpFlags |= (CP_AVOID_ENEMY | CP_CLEAR | CP_AVOID);
                      avoidDist = 256;
                    }
    ...
    if (!cpFlags) { // okay, we have no new enemy-driven reason to run... let's
                    // use tactics now
      if (runner &&
          NPCS.NPCInfo->combatPoint !=
              -1) { // someone is running and we have a combat point already
        if (NPCS.NPCInfo->squadState != SQUAD_SCOUT &&
            NPCS.NPCInfo->squadState != SQUAD_TRANSITION &&
            NPCS.NPCInfo->squadState != SQUAD_RETREAT) { // it's not us
          if (TIMER_Done(NPCS.NPC, "verifyCP") &&
              DistanceSquared(
                  NPCS.NPC->r.currentOrigin,
                  level.combatPoints[NPCS.NPCInfo->combatPoint].origin) >
                  64 * 64) { // 1 - 3 seconds have passed since you chose a CP,
          ...
          if (NPCS.NPCInfo->aiFlags &
              NPCAI_BLOCKED) { // dammit, something is in our way
            // see if it's one of ours
            for (j = 0; j < group->numGroup; j++) {
              if (group->member[j].number ==
                  NPCS.NPCInfo
                      ->blockingEntNum) { // we're being blocked by one of our
                                          // own, pass our goal onto them and
                                          // I'll stand still
                ST_TransferMoveGoal(NPCS.NPC,
                                    &g_entities[group->member[j].number]);
                break;
              }
            }
          }
        ...
        if (NPCS.NPCInfo->combatPoint !=
            -1) { // we have a combat point we're supposed to be running to
          if (NPCS.NPCInfo->squadState != SQUAD_SCOUT &&
              NPCS.NPCInfo->squadState != SQUAD_TRANSITION &&
              NPCS.NPCInfo->squadState !=
                  SQUAD_RETREAT) { // but we're not running
            if (TIMER_Done(
                    NPCS.NPC,
                    "verifyCP")) { // 1 - 3 seconds have passed since you chose
                                   // a CP, see if you're there since, for some
                                   // reason, you've stopped running...
              if (DistanceSquared(
                      NPCS.NPC->r.currentOrigin,
                      level.combatPoints[NPCS.NPCInfo->combatPoint].origin) >
        ...
        if (enemyLost) { // if no-one has seen the enemy for a while, send a
                         // scout
          // ask where he went
          if (group->numState[SQUAD_SCOUT] <= 0) {
            //	scouting = qtrue;
          ...
          // Since no-one else has done this, I should be the closest one, so go
          // after him...
          ...
          runner = qtrue;
        } else if (enemyProtected) { // if no-one has a clear shot at the enemy,
                                     // someone should go after him
          // FIXME: if I'm in an area where no safe combat points have a clear
          // shot at me, they don't come after me... they should anyway, though
          // after some extra hesitation. ALSO: seem to give up when behind an
          // area portal? since no-one else here has done this, I should be the
          // closest one
          if (TIMER_Done(NPCS.NPC, "roamTime") &&
              !Q_irand(0, group->numGroup)) { // only do this if we're ready to
                                              // move again and we feel like it
          ...
          {                                        // we're ready to move
            if (NPCS.NPCInfo->combatPoint == -1) { // we're not on a combat
                                                   // point
              if (1) //! Q_irand( 0, 2 ) )
              {      // we should go for a combat point
                ...
                TIMER_Set(NPCS.NPC, "roamTime", Q_irand(1000, 3000));
                           ...
                           NPCS.NPC,
                           "roamTime")) { // we are already on a combat point
              if (i == 0) {               // we're the closest
                if ((group->morale - group->numGroup > 0) &&
                    !Q_irand(0, 4)) { // try to outflank him
                  cpFlags |=
                ...
                if ((group->morale - group->numGroup <
                     0)) { // low morale, just hang here
                  TIMER_Set(NPCS.NPC, "roamTime", Q_irand(2000, 5000));
                  TIMER_Set(NPCS.NPC, "stick", Q_irand(2000, 5000));
                } else if ((group->morale - group->numGroup >
                            0)) { // try to move in on the enemy
                  cpFlags |= ST_ApproachEnemy(NPCS.NPC);
                  // set me into scout mode
                  AI_GroupUpdateSquadstates(group, NPCS.NPC, SQUAD_SCOUT);
          ...
          if (!cpFlags) { // still not moving
            ...
            */
            ...
            // toy with ducking
            if (TIMER_Done(NPCS.NPC, "duck")) {    // not ducking
              if (TIMER_Done(NPCS.NPC, "stand")) { // don't have to keep
                                                   // standing
                if (NPCS.NPCInfo->combatPoint == -1 ||
                    (level.combatPoints[NPCS.NPCInfo->combatPoint].flags &
                     CPF_DUCK)) { // okay to duck here
                  if (!Q_irand(0, 3)) {
    ...
    if (cpFlags) { // we want to run to a combat point
      ...
      {//if we're on a combat point, we obviously don't want the one we're
      closest to cpFlags |= CP_AVOID;
      }
      */

      if (group->enemy->client->ps.weapon == WP_SABER &&
          /*group->enemy->client->ps.SaberLength() > 0*/ !group->enemy->client
              ->ps.saberHolstered) { // we obviously want to avoid the enemy if
                                     // he has a saber
      ...
      if (cp == -1) { // may have had sone set above
        cp = NPC_FindCombatPoint(
            NPCS.NPC->r.currentOrigin, NPCS.NPC->r.currentOrigin,
            group->enemy->r.currentOrigin, cpFlags | CP_HAS_ROUTE, avoidDist,
            NPCS.NPCInfo->lastFailedCombatPoint);
      }
      while (cp == -1 &&
             cpFlags != CP_ANY) { // start "OR"ing out certain flags to see if
                                  // we can find *any* point
        if (cpFlags & CP_INVESTIGATE) { // don't need to investigate
          ...
          cpFlags &= ~CP_FLANK;
        } else if (cpFlags & CP_SAFE) { // don't need one that hasn't been shot
                                        // at recently
          cpFlags &= ~CP_SAFE;
        ...
        } else if (cpFlags & CP_APPROACH_ENEMY) { // don't need to approach
                                                  // enemy
          cpFlags &= ~CP_APPROACH_ENEMY;
        ...
        cp = NPC_FindCombatPoint(NPCS.NPC->r.currentOrigin,
                                 ...
                                 cpFlags | CP_HAS_ROUTE, avoidDist, -1);
      }
      ...
      if (cp != -1) { // found a combat point
        ...
        TIMER_Set(
            ...
            Q_irand(
                1000,
                3000)); // don't make sure you're in your CP for 1 - 3 seconds
        NPC_SetCombatPoint(cp);
        NPC_SetMoveGoal(NPCS.NPC, level.combatPoints[cp].origin, 8, qtrue, cp,
                        NULL);
        // okay, try a move right now to see if we can even get there

        // if ( ST_Move() )
        { // we actually can get to it, so okay to say you're going there.
          // FIXME: Hmm... any way we can store this move info so we don't have
          // to do it again 		when our turn to think comes up?
          ...
          if (cpFlags & CP_FLANK) {
            ...
            if (group->numGroup > 1) {
              ...
              if (!Q_irand(
                      0,
                      3)) { // 25% of the time, see if we're flanking the enemy
                vec3_t eDir2Me, eDir2CP;

                VectorSubtract(NPCS.NPC->r.currentOrigin,
                               group->enemy->r.currentOrigin, eDir2Me);
              ...
              if (dot < 0.4) { // flanking!
                ...
                NPC_ST_StoreMovementSpeech(SPEECH_YELL,
                                           0.2f); // was SPEECH_COVER
              }
            }
          }
          /*
          ...
          */
        } // else: nothing, a failed move should clear the combatPoint and you
          // can try again next frame
      } else if (NPCS.NPCInfo->squadState ==
                 SQUAD_SCOUT) { // we couldn't find a combatPoint by the player,
                                // so just go after him directly
...
void NPC_BSST_Attack(void) {
  ...
  if (NPCS.NPCInfo->group) { // I belong to a squad of guys - we should *always*
                             // have a group
    if (!NPCS.NPCInfo->group->processed) { // I'm the first ent in my group,
                                           // I'll make the command decisions
#if AI_TIMERS
  ...
  } else if (TIMER_Done(NPCS.NPC, "flee") &&
             NPC_CheckForDanger(NPC_CheckAlertEvents(
                 qtrue, qtrue, -1, qfalse,
                 AEL_DANGER))) { // not already fleeing, and going to run
    ST_Speech(NPCS.NPC, SPEECH_COVER, 0);
    NPC_UpdateAngles(qtrue, qtrue);
    return;
  }

  if (!NPCS.NPC->enemy) { // WTF?  somehow we lost our enemy?
    NPC_BSST_Patrol();    // FIXME: or patrol?
    return;
  }
  ...
  if (NPC_ClearLOS4(NPCS.NPC->enemy)) {
    AI_GroupUpdateEnemyLastSeen(NPCS.NPCInfo->group,
                                NPCS.NPC->enemy->r.currentOrigin);
    NPCS.NPCInfo->enemyLastSeenTime = level.time;
    enemyLOS = qtrue;
             ...
             ((hitEnt->r.svFlags & SVF_GLASS_BRUSH) || hitEnt->health < 40 ||
              NPCS.NPC->s.weapon ==
                  WP_EMPLACED_GUN))) { // can hit enemy or enemy ally or will
                                       // hit glass or other minor breakable (or
                                       // in emplaced gun), so shoot anyway
          AI_GroupUpdateClearShotTime(NPCS.NPCInfo->group);
          enemyCS = qtrue;
          NPC_AimAdjust(
              2); // adjust aim better longer we have clear shot at enemy
  ...
  if (NPCS.NPC->client->ps.weapon == WP_NONE) {
    ...
    if (enemyLOS) { // FIXME: no need to face enemy if we're moving to some
                    // other goal and he's too far away to shoot?
      faceEnemy = qtrue;
    }
    if (enemyCS) {
      shoot = qtrue;
  ...
  if (!(NPCS.NPCInfo->scriptFlags &
        SCF_CHASE_ENEMIES)) { // not supposed to chase my enemies
    if (NPCS.NPCInfo->goalEntity ==
        NPCS.NPC->enemy) { // goal is my entity, so don't move
      move = qfalse;
    }
  }
  ...
  if (move) {                     // move toward goal
    if (NPCS.NPCInfo->goalEntity) //&& ( NPCInfo->goalEntity != NPC->enemy ||
                                  //enemyDist > 10000 ) )//100 squared
    {
      move = ST_Move();
    } else {
      move = qfalse;
  ...
  if (!faceEnemy) { // we want to face in the dir we're running
    if (!move) { // if we haven't moved, we should look in the direction we last
                 // looked?
  ...
  if (NPCS.NPC->enemy && NPCS.NPC->enemy->enemy) {
    if (NPCS.NPC->enemy->s.weapon == WP_SABER &&
        NPCS.NPC->enemy->enemy->s.weapon ==
            WP_SABER) { // don't shoot at an enemy jedi who is fighting another
                        // jedi, for fear of injuring one or causing rogue
                        // blaster deflections (a la Obi Wan/Vader duel at end of
                        // ANH)
      shoot = qfalse;
    }
  }
  ...
  if (NPCS.NPC->client->ps.weaponTime > 0) {
    ...
    if (TIMER_Done(NPCS.NPC, "attackDelay")) {
      ...
      // NASTY
      if (NPCS.NPC->s.weapon == WP_ROCKET_LAUNCHER &&
          (NPCS.ucmd.buttons & BUTTON_ATTACK) && !move &&
