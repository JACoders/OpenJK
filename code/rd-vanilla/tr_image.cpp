...
#define FILE_HASH_SIZE                                                         \
  1024 // actually, the shader code needs this (from another module, great).
...
// makeup a nice clean, consistant name to query for and file under, for map<>
// usage...
//
...
void GL_TextureMode(const char *string) {
  ...
  if (r_ext_texture_filter_anisotropic->value >
    ...
    ri.Cvar_SetValue("r_ext_texture_filter_anisotropic",
                     glConfig.maxTextureFilterAnisotropy);
  }
...
float R_SumOfUsedImages(qboolean bUseFormat) {
  ...
  while ((pImage = R_Images_GetNextIteration()) != NULL) {
    if (pImage->frameUsed ==
        tr.frameCount -
            1) { // it has already been advanced for the next frame, so...
      if (bUseFormat) {
...
void R_ImageList_f(void) {
  ...
  while ((image = R_Images_GetNextIteration()) != NULL) {
    ...
    texBytes +=
        image->width * image->height * R_BytesPerTex(image->internalFormat);
    //		totalFileSizeK += (image->imgfileSize+1023)/1024;
    // ri.Printf (PRINT_ALL,  "%4i: %4i %4i %5i  %s ",
    //	i, image->width, image->height,(image->fileSize+1023)/1024,
    //yesno[image->mipmap] );
    ri.Printf(PRINT_ALL, "%4i: %4i %4i  %s ", i, image->width, image->height,
              yesno[image->mipmap]);
...
================
*/
static void R_LightScaleTexture(unsigned *in, int inwidth, int inheight,
                                qboolean only_gamma) {
  if (only_gamma) {
...
static void R_MipMap2(unsigned *in, int inWidth, int inHeight) {
  ...
  temp = (unsigned int *)R_Malloc(outWidth * outHeight * 4, TAG_TEMP_WORKSPACE,
                                  qfalse);
...
===============
*/
static void Upload32(unsigned *data, GLenum format, qboolean mipmap,
                     qboolean picmip, qboolean isLightmap, qboolean allowTC,
  ...
  if (format == GL_RGBA) {
    ...
    //
    while (width > glConfig.maxTextureSize ||
    ...
    if (samples == 3) {
      if (glConfig.textureCompression == TC_S3TC && allowTC) {
        *pformat = GL_RGB4_S3TC;
                 ...
                 allowTC) { // Compress purely color - no alpha
        if (r_texturebits->integer == 16) {
          *pformat =
              GL_COMPRESSED_RGB_S3TC_DXT1_EXT; // this format cuts to 16 bit
      ...
      if (glConfig.textureCompression == TC_S3TC_DXT &&
          allowTC) { // Compress both alpha and color
        *pformat = GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;
      } else if (r_texturebits->integer == 16) {
        *pformat = GL_RGBA4;
      } else if (r_texturebits->integer == 32) {
    ...
    *pUploadHeight = height;
    ...
    if (!mipmap) {
      qglTexImage2D(GL_TEXTURE_2D, 0, *pformat, width, height, 0, GL_RGBA,
    ...
    if (mipmap) {
      ...
      while (width > 1 || height > 1) {
        ...
        qglTexImage2D(GL_TEXTURE_2D, miplevel, *pformat, width, height, 0,
...
// clean up anything to do with an image_t struct, but caller will have to clear
// the internal to an image_t struct ready for either struct free() or
// overwrite...
//
...
void R_Images_DeleteLightMaps(void) {
  for (AllocatedImages_t::iterator itImage = AllocatedImages.begin();
       itImage != AllocatedImages.end();
       /* empty */) {
    image_t *pImage = (*itImage).second;

    if (pImage->imgName[0] ==
        '$' /*&& strstr(pImage->imgName,"lightmap")*/) // loose check, but
                                                       // should be ok
    {
...
void R_Images_DeleteImage(image_t *pImage) {
  // Even though we supply the image handle, we need to get the corresponding
  // iterator entry...
  //
...
void RE_RegisterImages_Info_f(void) {
  ...
  int iNumImages = R_Images_StartIteration();
  while ((pImage = R_Images_GetNextIteration()) != NULL) {
    ri.Printf(PRINT_ALL, "%d: (%4dx%4dy) \"%s\"", iImage, pImage->width,
              pImage->height, pImage->imgName);
  ...
  ri.Printf(PRINT_ALL,
            "%d Images. %d (%.2fMB) texels total, (not including mipmaps)\n",
            iNumImages, iTexels, (float)iTexels / 1024.0f / 1024.0f);
  ri.Printf(PRINT_DEVELOPER, "RE_RegisterMedia_GetLevel(): %d",
...
qboolean RE_RegisterImages_LevelLoadEnd(void) {
  ...
  for (AllocatedImages_t::iterator itImage = AllocatedImages.begin();
    ...
    // don't un-register system shaders (*fog, *dlight, *white, *default), but
    // DO de-register lightmaps ("$<mapname>/lightmap%d")
    if (pImage->imgName[0] != '*') {
      // image used on this level?
      //
      if (pImage->iLastLevelUsedOn !=
          RE_RegisterMedia_GetLevel()) { // nope, so dump it...
        // ri.Printf( PRINT_DEVELOPER, "Dumping image
        // \"%s\"\n",pImage->imgName);
        R_Images_DeleteImageContents(pImage);
...
// returns image_t struct if we already have this, else NULL. No disk-open
// performed
//	(important for creating default images).
//
// This is called by both R_FindImageFile and anything that creates default
// images...
//
static image_t *R_FindImageFile_NoLoad(const char *name, qboolean mipmap,
                                       qboolean allowPicmip, qboolean allowTC,
                                       int glWrapClampMode) {
  if (!name) {
    return NULL;
  }
...
image_t *R_CreateImage(const char *name, const byte *pic, int width, int height,
                       GLenum format, qboolean mipmap, qboolean allowPicmip,
  ...
  image = R_FindImageFile_NoLoad(name, mipmap, allowPicmip, allowTC,
                                 glWrapClampMode);
  if (image) {
    return image;
  }
  ...
  image->texnum =
      1024 + giTextureBindNum++; // ++ is of course staggeringly important...
  ...
  Upload32((unsigned *)pic, format, (qboolean)image->mipmap, allowPicmip,
           isLightmap, allowTC, &image->internalFormat, &image->width,
           &image->height);

  qglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, glWrapClampMode);
  qglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, glWrapClampMode);

  qglBindTexture(GL_TEXTURE_2D, 0); // jfm: i don't know why this is here, but
                                    // it breaks lightmaps when there's only 1
...
image_t *R_FindImageFile(const char *name, qboolean mipmap,
  ...
  // R_FindImageFile_NoLoad() may complain about
  ...
  image = R_CreateImage((char *)name, pic, width, height, GL_RGBA, mipmap,
                        allowPicmip, allowTC, glWrapClampMode);
  R_Free(pic);
  return image;
}
...
static void R_CreateDlightImage(void) {
  ...
  // The old code claims to have made a centered inverse-square falloff blob for
  // dynamic lighting
  //	and it looked nasty, so, just doing something simpler that seems to have
  //a much softer result
  for (x = 0; x < DLIGHT_SIZE; x++) {
    for (y = 0; y < DLIGHT_SIZE; y++) {
      xs = (DLIGHT_SIZE * 0.5f - x);
      ys = (DLIGHT_SIZE * 0.5f - y);

      b = 255 - sqrt((double)xs * xs + ys * ys) *
                    9.0f; // try and generate numbers in the range of 255-0
  ...
  tr.dlightImage =
      R_CreateImage("*dlight", (byte *)data, DLIGHT_SIZE, DLIGHT_SIZE, GL_RGBA,
                    qfalse, qfalse, qfalse, GL_CLAMP);
#else
  int width, height;
  byte *pic;
  ...
  if (pic) {
    tr.dlightImage = R_CreateImage("*dlight", pic, width, height, GL_RGBA,
                                   qfalse, qfalse, qfalse, GL_CLAMP);
    R_Free(pic);
    ...
    tr.dlightImage =
        R_CreateImage("*dlight", (byte *)data, DLIGHT_SIZE, DLIGHT_SIZE,
                      GL_RGBA, qfalse, qfalse, qfalse, GL_CLAMP);
  }
#endif
}
...
static void R_CreateFogImage(void) {
  ...
  // what we want.
  tr.fogImage = R_CreateImage("*fog", (byte *)data, FOG_S, FOG_T, GL_RGBA,
                              qfalse, qfalse, qfalse, GL_CLAMP);
  R_Free(data);
...
static void R_CreateDefaultImage(void) {
  ...
  tr.defaultImage =
      R_CreateImage("*default", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE,
                    GL_RGBA, qtrue, qfalse, qtrue, GL_REPEAT);
}
...
void R_CreateBuiltinImages(void) {
  ...
  tr.whiteImage = R_CreateImage("*white", (byte *)data, 8, 8, GL_RGBA, qfalse,
                                qfalse, qtrue, GL_REPEAT);

  tr.screenImage = R_CreateImage("*screen", (byte *)data, 8, 8, GL_RGBA, qfalse,
  ...
  qglTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA16, glConfig.vidWidth,
                glConfig.vidHeight, 0, GL_RGB, GL_FLOAT, 0);
  qglTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
  qglTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  qglTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_S,
                   GL_CLAMP_TO_EDGE);
  qglTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_T,
                   GL_CLAMP_TO_EDGE);
  ...
  qglTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA16,
                r_DynamicGlowWidth->integer, r_DynamicGlowHeight->integer, 0,
                GL_RGB, GL_FLOAT, 0);
  qglTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
  qglTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  qglTexParameteri(GL_TEXTURE_RECTANGLE_ARB, GL_TEXTURE_WRAP_S,
  ...
  tr.identityLightImage =
      R_CreateImage("*identityLight", (byte *)data, 8, 8, GL_RGBA, qfalse,
                    qfalse, qtrue, GL_REPEAT);
  ...
  for (x = 0; x < NUM_SCRATCH_IMAGES; x++) {
    // scratchimage is usually used for cinematic drawing
    tr.scratchImage[x] =
        R_CreateImage(va("*scratch%d", x), (byte *)data, DEFAULT_SIZE,
                      DEFAULT_SIZE, GL_RGBA, qfalse, qfalse, qfalse, GL_CLAMP);
  }
...
void R_InitImages(void) {
  // memset(hashTable, 0, sizeof(hashTable));	// DO NOT DO THIS NOW (because
  // of image cacheing)	-ste.
