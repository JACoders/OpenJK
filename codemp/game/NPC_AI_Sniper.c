...
static qboolean Sniper_Move(void) {
	...
	if (moved == qfalse) { // couldn't get to enemy
		if ((NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) && NPCS.NPCInfo->goalEntity &&
			NPCS.NPCInfo->goalEntity == NPCS.NPC->enemy) { // we were running after enemy
			...
			if (cp == -1 && !(NPCS.NPCInfo->scriptFlags & SCF_USE_CP_NEAREST)) { // okay, try one by the enemy
				cp = NPC_FindCombatPoint(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin,
...
void NPC_BSSniper_Patrol(void) { // FIXME: pick up on bodies of dead buddies?
	...
	if (NPCS.NPCInfo->confusionTime < level.time) {
		...
		if (!(NPCS.NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
				...
				if (alertEvent >= 0 && level.alertEvents[alertEvent].ID != NPCS.NPCInfo->lastAlertID) {
					...
					if (level.alertEvents[alertEvent].level == AEL_DISCOVERED) {
						if (level.alertEvents[alertEvent].owner && level.alertEvents[alertEvent].owner->client &&
							...
							level.alertEvents[alertEvent].owner->client->playerTeam == NPCS.NPC->client->enemyTeam) { // an enemy
			...
			if (NPCS.NPCInfo->investigateDebounceTime > level.time) { // FIXME: walk over to it, maybe?  Not if not chase enemies flag
	...
	if ((NPCS.NPCInfo->goalEntity != NPCS.NPC->enemy) && (NPCS.NPCInfo->goalEntity != NULL)) {
		...
		if (NAV_HitNavGoal(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, NPCS.NPCInfo->goalEntity->r.currentOrigin, 16,
			...
			TIMER_Set(NPCS.NPC, "attackDelay",
					  Q_irand((6 - NPCS.NPCInfo->stats.aim) * 50, (6 - NPCS.NPCInfo->stats.aim) * 100)); // FIXME: Slant for difficulty levels, too?
	...
	if (TIMER_Done(NPCS.NPC, "duck")) {			// we're not ducking
		if (TIMER_Done(NPCS.NPC, "roamTime")) { // not roaming
			...
			if ((NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) &&
				(!NPCS.NPCInfo->goalEntity || NPCS.NPCInfo->goalEntity == NPCS.NPC->enemy)) { // we were running after enemy
				// Try to find a combat point that can hit the enemy
				int cpFlags = (CP_CLEAR | CP_HAS_ROUTE);
				int cp;
...
static void Sniper_CheckFireState(void) {
	...
	if (!Q_irand(0, 1) && NPCS.NPCInfo->enemyLastSeenTime &&
		level.time - NPCS.NPCInfo->enemyLastSeenTime < ((5 - NPCS.NPCInfo->stats.aim) * 1000)) // FIXME: incorporate skill too?
	{
		...
		return;
	} else if (level.time - NPCS.NPCInfo->enemyLastSeenTime > 10000) { // next time we see him, we'll miss few times first
		NPCS.NPC->count = 0;
	}
}
...
void Sniper_FaceEnemy(void) {
	...
	if (NPCS.NPC->enemy) {
		...
		{
			if (NPCS.NPC->count < (5 - NPCS.NPCInfo->stats.aim)) {											 // miss a few times first
				if (shoot2 && TIMER_Done(NPCS.NPC, "attackDelay") && level.time >= NPCS.NPCInfo->shotTime) { // ready to fire again
...
void NPC_BSSniper_Attack(void) {
	...
	if (TIMER_Done(NPCS.NPC, "flee") && NPC_CheckForDanger(NPC_CheckAlertEvents(qtrue, qtrue, -1, qfalse, AEL_DANGER))) { // going to run
		NPC_UpdateAngles(qtrue, qtrue);
		return;
	}
	...
	{														// too close, so switch to primary fire
		if (NPCS.NPC->client->ps.weapon == WP_DISRUPTOR) {	// sniping... should be assumed
			if (NPCS.NPCInfo->scriptFlags & SCF_ALT_FIRE) { // use primary fire
				trace_t trace;
				trap->Trace(&trace, NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->enemy->r.mins, NPCS.NPC->enemy->r.maxs, NPCS.NPC->r.currentOrigin,
							NPCS.NPC->enemy->s.number, NPCS.NPC->enemy->clipmask, qfalse, 0, 0);
				if (!trace.allsolid && !trace.startsolid && (trace.fraction == 1.0 || trace.entityNum == NPCS.NPC->s.number)) { // he can get right to me
	...
	*/
	...
	if (enemyCS2) {
		shoot2 = qtrue;
	} else if (level.time - NPCS.NPCInfo->enemyLastSeenTime > 3000) { // Hmm, have to get around this bastard... FIXME: this NPCInfo->enemyLastSeenTime builds
																	  // up when ducked seems to make them want to run when they uncrouch
	...
	if (TIMER_Done(NPCS.NPC, "duck") && TIMER_Done(NPCS.NPC, "watch") && (TIMER_Get(NPCS.NPC, "attackDelay") - level.time) > 1000 &&
