...
static void G_RoffNotetrackCallback(gentity_t *ent, const char *notetrack) {
  ...
  // Note: <> denote required argument, [] denote optional argument, | denotes
  // argument choices
  //
  // Examples:
  // effect notetrack format:		effect <relative_effectfilepath.efx>
  // [originOffset] [rotationOffset] 					notetrack = "effect effects/explosion1.efx
  //0+0+64 0-0-1"; 'effect' notes: 		(1) the '+' and '-' are delimiters and not
  //positive/negative signs; e.g., negative origin offset would be: -10+-20+-10
  //		(2) angles are expected to be from 0 to 360, i.e., no negative
  //angles. 		(3) optional additional argument for rotationOffset requires the
  //originOffset preceding it.
  //
  //
  // sound notetrack format:		sound <relative_soundfilepath.ext>
  //					notetrack =	"sound
  //sound/vehicles/tie/flyby2.mp3"; 'sound' notes: 		(1) supported sound file
  //formats are: .mp3, .wav
  //
  //
  // USE notetrack format:			USE
  // <relative_scriptfilepath_noExt> 					notetrack = "USE shuttlemap/shuttletakeoff";
  //
  //
  // loop notetrack format:		loop <rof> < absolute | relative >
  //								loop <sfx> < relative_soundfilepath.ext |
  //kill > 					notetrack = "loop rof absolute"; 					notetrack = "loop rof relative";
  //					notetrack = "loop sfx
  //sound/vehicles/tie/loop.wav"; 					notetrack = "loop sfx kill"; 'loop rof' notes:
  //		(1) absolute ==> reset rof to original delta position/rotation world
  //location before looping. 		(2) relative ==> reset rof to original delta
  //position/rotation at current location before looping. 'loop sfx' notes: 		(1)
  //adds a sound to be looped which gets assigned to the entitystate's loopSound
  //parameter. 		(2) addlArg 'kill' -- kills the looping sound by setting
  //entitystate's loopSound equal to zero.
  //
  //////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  defaultoffsetposition:
    ...
    if (strcmp(argument, "rof") == 0) {
      if (strcmp(addlArg, "absolute") == 0) {
        ...
        sprintf(errMsg, "Invalid additional argument <%s> for type 'loop rof'",
                addlArg);
        goto functionend;
      }
      ...
      if (r2 && strstr(teststr, "kill")) { // kill the looping sound
        ent->s.loopSound = 0;
        if (g_developer->integer) {
          Com_Printf(S_COLOR_GREEN "NoteTrack:  \"%s\"\n", notetrack);
        }
                 ...
                 strstr(
                     teststr,
                     "sound")) { // OK... we should have a relative sound path
        // try to register the sound and add it to the entitystate loopSound
        // parameter
        if (ent->s.eType == ET_MOVER) {
          ...
          if (objectID) {
            ...
            sprintf(
                errMsg,
                "cgi_S_RegisterSound(%s) failed to return a valid sfxHandle_t "
                "for additional argument. Setting 'loopSound' to 0.",
                addlArg);
            goto functionend;
          }
        } else {
        ...
        sprintf(errMsg, "Invalid additional argument <%s> for type 'loop sfx'",
                addlArg);
        goto functionend;
      }
    } else {
      sprintf(errMsg, "Invalid argument <%s> for type 'loop' notetrack.",
              argument);
      goto functionend;
    }
  }
  // else if ...
    ...
    if (type[0]) {
      Com_Printf(S_COLOR_YELLOW
                 "Warning: \"%s\" is an invalid ROFF NoteTrack function\n",
                 type);
    } else {
      Com_Printf(S_COLOR_YELLOW
                 "Warning: NoteTrack is missing function and/or arguments\n");
    }
  }
    ...
    if (mem) {
      ...
      if (roffs[num_roffs].mFrameTime < 50) {
        Com_Printf(S_COLOR_RED
                   "Error: \"%s\" has an invalid ROFF framerate (%d < 50)\n",
                   file, roffs[num_roffs].mFrameTime);
      }
      assert(roffs[num_roffs].mFrameTime >=
             50); // HAS to be at least 50 to be reliable
...
void G_Roff(gentity_t *ent) {
    ...
    if (data->mStartNote != -1) {
      for (int n = 0; n < data->mNumNotes; n++) {
        G_RoffNotetrackCallback(
            ent, roffs[roff_id - 1].mNoteTrackIndexes[data->mStartNote + n]);
      }
    }
  } else {
  ...
  if (ent->client) {
    ...
    if (ent->e_ThinkFunc == thinkF_TieFighterThink ||
        ent->e_ThinkFunc == thinkF_TieBomberThink ||
        (!ent->e_ThinkFunc && ent->s.eType != ET_MISSILE &&
         ent->s.eType != ET_ITEM &&
         ent->s.eType !=
             ET_MOVER)) { // will never set currentAngles & currentOrigin itself
                          // ( why do we limit which one's get set?, just set all
                          // the time? )
