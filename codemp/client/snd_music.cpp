...
#define MUSIC_PARSE_ERROR(_string)                                                                                                                             \
	Music_Parse_Error(                                                                                                                                         \
		_string) // only use during parse, not run-time use, and bear in mid that data is zapped after error message, so exit any loops immediately
...
static qboolean Music_ParseMusic(CGenericParser2 &Parser, MusicData_t *MusicData, CGPGroup *pgMusicFiles, const char *psMusicName, const char *psMusicNameKey,
			...
			if (!strcmp(psGroupName, sKEY_ENTRY)) {
				...
				for (CGPValue *pValue = pGroup->GetPairs(); pValue; pValue = pValue->GetNext()) {
					...
					if (!strcmp(psKey, sKEY_NEXTFILE)) {
						...
						for (MusicEntryTimes_t::iterator itEntryTimes = MusicFile.MusicEntryTimes.begin(); itEntryTimes != MusicFile.MusicEntryTimes.end();
							 ++itEntryTimes) {
...
qboolean Music_DynamicDataAvailable(const char *psDynamicMusicLabel) {
	...
	Q_strncpyz(sLevelName,
			   COM_SkipPath(const_cast<char *>((psDynamicMusicLabel && psDynamicMusicLabel[0]) ? psDynamicMusicLabel : gsLevelNameFromServer.c_str())),
			   sizeof(sLevelName));
