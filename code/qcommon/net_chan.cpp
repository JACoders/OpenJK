...
#define MAX_PACKETLEN                                                          \
  (MAX_MSGLEN) //(1400)		// max size of a network packet
...
void Netchan_Init(int port) {
  if (!loopbacks) {
    loopbacks =
        (loopback_t *)Z_Malloc(sizeof(loopback_t) * 2, TAG_NEWDEL, qtrue);
  }
...
qboolean NET_GetLoopPacket(netsrc_t sock, netadr_t *net_from,
  ...
  if (i + length <= MAX_LOOPDATA) {
    ...
    memcpy((char *)net_message->data + copyToEnd, loop->loopData,
           length - copyToEnd);
    net_message->cursize = length;
