...
void Pilot_Update(void) {
  ...
  for (int i = 0; i < ENTITYNUM_WORLD; i++) {
    if (g_entities[i].inuse && g_entities[i].client && g_entities[i].NPC &&
        g_entities[i].NPC->greetEnt &&
        g_entities[i].NPC->greetEnt->owner == (&g_entities[i])) {
      mActivePilotCount++;
    }
    if (g_entities[i].inuse && g_entities[i].client &&
        g_entities[i].m_pVehicle && !g_entities[i].owner &&
  ...
  if (player && player->inuse &&
    ...
    if (pVeh &&
      ...
      VectorMA(player->currentOrigin, 1.2f,
               pVeh->m_pParentEntity->client->ps.velocity, projectedPosition);
      VectorMA(projectedPosition, Q_flrand(-200.0f, 200.0f), projectedRight,
               projectedPosition);

      gi.trace(&mPilotViewTrace, player->currentOrigin, 0, 0, projectedPosition,
               player->s.number, MASK_SHOT, (EG2_Collision)0, 0);
      ...
      if ((mPilotViewTrace.allsolid == qfalse) &&
          ...
          (DotProduct(projectedDirection, mPilotViewTrace.plane.normal) <
           -0.5f)) {
        //	CG_DrawEdge(player->currentOrigin, mPilotViewTrace.endpos,
        //EDGE_IMPACT_POSSIBLE);
        TIMER_Set(player, "FlybySoundArchitectureDebounce",
                  Q_irand(1000, 2000));

        int soundFlyBy = pVeh->m_pVehicleInfo->soundFlyBy;
        if (pVeh->m_pVehicleInfo->soundFlyBy2 &&
...
// Vehicle_Find
//
// Will look through all registered vehicles and choose the closest one that the
// given entity can get to.
//
////////////////////////////////////////////////////////////////////////////////////////
...
//
// Will return true if the character is either driving a vehicle or on his way
// to get onto one.
////////////////////////////////////////////////////////////////////////////////////////
bool Pilot_MasterUpdate() {
  ...
  //------------------------------------------------------------------
  if (NPCInfo->greetEnt) {
    if (!NPCInfo->greetEnt->inuse || !NPCInfo->greetEnt->m_pVehicle ||
...
void Pilot_Update_Enemy() {
  ...
  if (TIMER_Done(NPC, "NextPilotCheckEnemyTime")) {
    ...
    if (NPC->enemy &&
        Distance(NPC->currentOrigin, NPC->enemy->currentOrigin) > 1000.0f) {
      mPilotViewTraceCount++;
      gi.trace(&mPilotViewTrace, NPC->currentOrigin, 0, 0,
               NPC->enemy->currentOrigin, NPC->s.number, MASK_SHOT,
               (EG2_Collision)0, 0);
...
void Pilot_Steer_Vehicle() {
  ...
  // If my vehicle is spinning out of control, just hold on, we're going to
  // die!!!!!
  //---------------------------------------------------------------------------------
  ...
  EnemyAngles[2] = 0;
  Vehicle_t *EnemyVeh =
      (NPC->enemy->s.m_iVehicleNum)
          ? (g_entities[NPC->enemy->s.m_iVehicleNum].m_pVehicle)
          : (0);
  bool EnemyInTurbo = (EnemyVeh && EnemyVeh->m_iTurboTime > level.time);
  float EnemySpeed = (EnemyVeh) ? (EnemyVeh->m_pParentEntity->client->ps.speed)
                                : (NPC->enemy->resultspeed);
  bool EnemySlideBreak =
      (EnemyVeh && (EnemyVeh->m_ulFlags & VEH_SLIDEBREAKING ||
                    EnemyVeh->m_ulFlags & VEH_STRAFERAM));
  bool EnemyDead = (NPC->enemy->health <= 0);

  bool ActorFlank = (NPCInfo->lastAvoidSteerSideDebouncer > level.time &&
  ...
  EnemyFlankPos.ScaleAdd(EnemyRight, (EnemySide == Side_Right)
                                         ? (FUTURE_SIDE_DIST)
                                         : (-FUTURE_SIDE_DIST));
  ...
  // Fly By Sounds
  //---------------
  if ((ActorVeh->m_pVehicleInfo->soundFlyBy ||
       ActorVeh->m_pVehicleInfo->soundFlyBy2) &&
  ...
  //===================
    ...
    ucmd.buttons |=
        BUTTON_VEH_SPEED; // Tells PMove to use the ps.speed we calculate here,
                          // not the one from g_vehicles.c

    // For Flanking, We Calculate The Speed By Hand, Rather Than Using Pure
    // Accelerate / No Accelerate Functionality
    //---------------------------------------------------------------------------------------------------------------
    NPC->client->ps.speed = ActorVeh->m_pVehicleInfo->speedMax *
                            ((ActorInTurbo) ? (1.35f) : (1.15f));

    // If In Slowing Distance, Scale Down The Speed As We Approach Our Move
    // Target
    //-----------------------------------------------------------------------------
  ...
  //=======================
    ...
    if (!EnemyVeh && AimAccuracy > 0.99f && MoveDistance < 500 && !EnemyDead) {
      ...
      ActorAccelerate =
          ((MoveDistance > 500 && EnemySpeed > 20.0f) || MoveDistance > 1000);
    ...
    if (ActorYawOffset != 0) {
      ...
      if (AimAccuracy < ATTACK_SIDE && AimAccuracy > -ATTACK_SIDE) {
        ...
        /*		if (TIMER_Done(NPC, "RiderAltAttackCheck"))
                        {
                                TIMER_Set(NPC, "RiderAltAttackCheck",
           Q_irand(1000, 3000)); if (Q_irand(0, 2)==0)
                                {
  ...
  if (ActorAimAtTarget) {
    ...
    NPCInfo->desiredYaw =
        AngleNormalize360(MoveDirection[YAW] + ActorYawOffset);
  }
  NPC_UpdateAngles(qtrue, qtrue);
}
