...
const char *CG_PlaceString(int rank) {
  ...
  // note that the rules are different for french, but by changing the required
  // strip strings they seem to work
  char sST[10];
  char sND[10];
  char sRD[10];
  char sTH[10];
  ...
  trap->SE_GetStringTextString("MP_INGAME_TIED_FOR", sTiedFor,
                               sizeof(sTiedFor));
  ...
  if (rank == 1) {
    s = va("1%s",
           sST); // S_COLOR_BLUE "1st" S_COLOR_WHITE;		// draw in blue
  } else if (rank == 2) {
    s = va("2%s",
           sND); // S_COLOR_RED "2nd" S_COLOR_WHITE;		// draw in red
  } else if (rank == 3) {
    s = va("3%s", sRD); // S_COLOR_YELLOW "3rd" S_COLOR_WHITE;		// draw
                        // in yellow
  } else if (rank == 11) {
    s = va("11%s", sTH);
  } else if (rank == 12) {
...
clientkilled:
  ...
  if (attacker == cg.snap->ps.clientNum) {
    ...
    if (cgs.gametype < GT_TEAM && cgs.gametype != GT_DUEL &&
      ...
      if (cgs.gametype == GT_JEDIMASTER && attacker < MAX_CLIENTS &&
          !ent->isJediMaster && !cg.snap->ps.isJediMaster &&
          CG_ThereIsAMaster()) {
        char part1[512];
        char part2[512];
        trap->SE_GetStringTextString("MP_INGAME_KILLED_MESSAGE", part1,
                                     sizeof(part1));
        trap->SE_GetStringTextString("MP_INGAME_JMKILLED_NOTJM", part2,
                                     sizeof(part2));
        s = va("%s %s\n%s\n", part1, targetName, part2);
      } else if (cgs.gametype == GT_JEDIMASTER && attacker < MAX_CLIENTS &&
                 !ent->isJediMaster &&
        ...
        */
        ...
        char sPlaceWith[256];
        char sKilledStr[256];
        trap->SE_GetStringTextString("MP_INGAME_PLACE_WITH", sPlaceWith,
                                     sizeof(sPlaceWith));
        trap->SE_GetStringTextString("MP_INGAME_KILLED_MESSAGE", sKilledStr,
                                     sizeof(sKilledStr));

        s = va("%s %s.\n%s %s %i.", sKilledStr, targetName,
               CG_PlaceString(cg.snap->ps.persistant[PERS_RANK] + 1),
               sPlaceWith, cg.snap->ps.persistant[PERS_SCORE]);
      }
    } else {
      char sKilledStr[256];
      trap->SE_GetStringTextString("MP_INGAME_KILLED_MESSAGE", sKilledStr,
  ...
  trap->Print("%s %s\n", targetName,
              (char *)CG_GetStringEdString("MP_INGAME", "DIED_GENERIC"));
}
...
static void CG_UseItem(centity_t *cent) {
  ...
  if (cg.snap && cg.snap->ps.clientNum == cent->currentState.number &&
      itemNum != HI_BINOCULARS && itemNum != HI_JETPACK &&
      itemNum != HI_HEALTHDISP && itemNum != HI_AMMODISP &&
      itemNum != HI_CLOAK &&
      itemNum != HI_EWEB) { // if not using binoculars/jetpack/dispensers/cloak,
                            // we just used that item up, so switch
...
static void CG_ItemPickup(int itemNum) {
  ...
  if (cg.snap && bg_itemlist[itemNum].giType == IT_WEAPON) {
    ...
    if (0 == cg_autoSwitch.integer) {
      // don't switch
    } else if (cg_autoSwitch.integer ==
               1) { // only autoselect if not explosive ("safe")
  ...
  if (bg_itemlist[itemNum].classname && bg_itemlist[itemNum].classname[0] &&
      (bg_itemlist[itemNum].giType != IT_TEAM ||
       (bg_itemlist[itemNum].giTag != PW_REDFLAG &&
        bg_itemlist[itemNum].giTag !=
            PW_BLUEFLAG))) { // don't print messages for flags, they have their
                             // own pickup event broadcasts
    char text[1024];
    char upperKey[1024];
    ...
    if (trap->SE_GetStringTextString(va("SP_INGAME_%s", Q_strupr(upperKey)),
                                     text, sizeof(text))) {
      Com_Printf("%s %s\n", CG_GetStringEdString("MP_INGAME", "PICKUPLINE"),
                 text);
    } else {
      Com_Printf("%s %s\n", CG_GetStringEdString("MP_INGAME", "PICKUPLINE"),
                 bg_itemlist[itemNum].classname);
    }
  }
}
...
void CG_ReattachLimb(centity_t *source) {
  ...
  while (i <= G2_MODELPART_RLEG)
  {
          if (source->torsoBolt & (1 << (i-10)))
          {
                  ...
                  trap->G2API_SetSurfaceOnOff(source->ghoul2, stubCapName,
  0x00000100);
          }
          i++;
  }
...
void CG_PrintCTFMessage(clientInfo_t *ci, const char *teamName,
                        int ctfMessage) {
  char printMsg[1024];
  char *refName = NULL;
  ...
  if (ci) {
    Com_sprintf(printMsg, sizeof(printMsg), "%s^7 %s", ci->name,
                psStringEDString);
  } else {
...
void DoFall(centity_t *cent, entityState_t *es, int clientNum) {
  ...
  if (cent->currentState.eFlags & EF_DEAD) { // corpses crack into the ground
                                             // ^_^
    if (delta > 25) {
...
void CG_TryPlayCustomSound(vec3_t origin, int entityNum, int channel,
...
void CG_G2MarkEvent(entityState_t *es) {
  ...
  if (es->eventParm) {
    ...
    CG_G2Trace(&tr, es->origin, NULL, NULL, es->origin2, ignore,
               MASK_PLAYERSOLID);

    if (tr.entityNum != es->otherEntityNum) { // try again if we hit an ent but
      ...
      if (tr.entityNum < ENTITYNUM_WORLD) {
        ...
        CG_G2Trace(&tr, es->origin, NULL, NULL, es->origin2, ignore,
                   MASK_PLAYERSOLID);
        if (tr.entityNum !=
            es->otherEntityNum) { // try extending the trace a bit.. or not
          /*
          vec3_t v;
          ...
          CG_G2Trace(&tr, es->origin, NULL, NULL, es->origin2, ignore,
          MASK_PLAYERSOLID); if (tr.entityNum != es->otherEntityNum)
          {
                  return;
          }
          */
          // didn't manage to collide with the desired person. No mark will be
          // placed then.
          return;
        }
      }
  ...
  if ((es->eFlags &
       EF_JETPACK_ACTIVE)) { // a vehicle weapon, make it a larger size mark
    // OR base this on the size of the thing you hit?
    if (g_vehWeaponInfo[es->otherEntityNum2].fG2MarkSize) {
      size =
          flrand(0.6f, 1.4f) * g_vehWeaponInfo[es->otherEntityNum2].fG2MarkSize;
    } else {
      size = flrand(32.0f, 72.0f);
    }
    // specify mark shader in vehWeapon file
    if (g_vehWeaponInfo[es->otherEntityNum2]
            .iG2MarkShaderHandle) { // have one we want to use instead of
                                    // defaults
  ...
  case WP_TURRET:
    ...
    CG_AddGhoul2Mark(shader, size, startPoint, es->origin2, es->owner,
                     pOwner->lerpOrigin, pOwner->lerpAngles[YAW],
                     pOwner->ghoul2, pOwner->modelScale, Q_irand(10000, 20000));
    break;
...
void CG_CalcVehMuzzle(Vehicle_t *pVeh, centity_t *ent, int muzzleNum) {
  ...
  if (pVeh->m_iMuzzleTime[muzzleNum] ==
      cg.time) { // already done for this frame, don't need to do it again
    return;
  }
...
void CG_VehMuzzleFireFX(centity_t *veh, entityState_t *broadcaster) {
  ...
  for (curMuz = 0; curMuz < MAX_VEHICLE_MUZZLES;
    ...
    {                                                      // this muzzle fired
      muzFX = 0;
      if (pVeh->m_pVehicleInfo->weapMuzzle[curMuz] ==
          0) { // no weaopon for this muzzle?  check turrets
        int i, j;
        for (i = 0; i < MAX_VEHICLE_TURRETS; i++) {
          for (j = 0; j < MAX_VEHICLE_TURRETS; j++) {
            if (pVeh->m_pVehicleInfo->turret[i].iMuzzle[j] - 1 ==
                curMuz) { // this muzzle belongs to this turret
              muzFX = g_vehWeaponInfo[pVeh->m_pVehicleInfo->turret[i].iWeapon]
                          .iMuzzleFX;
              break;
            }
          }
        ...
        muzFX =
            g_vehWeaponInfo[pVeh->m_pVehicleInfo->weapMuzzle[curMuz]].iMuzzleFX;
      }
      if (muzFX) {
        // CG_CalcVehMuzzle(pVeh, veh, curMuz);
        // trap->FX_PlayEffectID(muzFX, pVeh->m_vMuzzlePos[curMuz],
        // pVeh->m_vMuzzleDir[curMuz], -1, -1, qfalse);
        trap->FX_PlayBoltedEffectID(muzFX, veh->currentState.origin,
                                    veh->ghoul2, pVeh->m_iMuzzleTag[curMuz],
                                    veh->currentState.number, 0, 0, qtrue);
      }
    }
  }
}
...
const char *CG_GetStringForVoiceSound(const char *s) {
  ...
  while (i < MAX_CUSTOM_SIEGE_SOUNDS) {
    if (bg_customSiegeSoundNames[i] &&
        !Q_stricmp(bg_customSiegeSoundNames[i],
                   s)) { // get the matching reference name
      assert(cg_stringEdVoiceChatTable[i]);
      return CG_GetStringEdString("MENUS",
                                  (char *)cg_stringEdVoiceChatTable[i]);
    }
    i++;
  }
...
void CG_EntityEvent(centity_t *cent, vec3_t position) {
  ...
  case EV_TAUNT:
    DEBUGNAME("EV_TAUNT");
    {
      ...
      if (cgs.gametype != GT_DUEL && cgs.gametype != GT_POWERDUEL &&
          es->eventParm == TAUNT_TAUNT) { // normal taunt
        ...
        default:
          if (Q_irand(0, 1)) {
            soundIndex =
                CG_CustomSound(es->number, va("*anger%d.wav", Q_irand(1, 3)));
          } else {
            soundIndex =
                CG_CustomSound(es->number, va("*taunt%d.wav", Q_irand(1, 3)));
            if (!soundIndex) {
              soundIndex =
                  CG_CustomSound(es->number, va("*anger%d.wav", Q_irand(1, 3)));
            }
          ...
          break;
        case TAUNT_BOW:
          // soundIndex = CG_CustomSound( es->number, va("*respect%d.wav",
          // Q_irand(1,3)) );
          break;
        case TAUNT_MEDITATE:
          // soundIndex = CG_CustomSound( es->number, va("*meditate%d.wav",
          // Q_irand(1,3)) );
          break;
        case TAUNT_FLOURISH:
          if (Q_irand(0, 1)) {
            soundIndex =
                CG_CustomSound(es->number, va("*deflect%d.wav", Q_irand(1, 3)));
            if (!soundIndex) {
              soundIndex =
                  CG_CustomSound(es->number, va("*gloat%d.wav", Q_irand(1, 3)));
              if (!soundIndex) {
                soundIndex = CG_CustomSound(es->number,
                                            va("*anger%d.wav", Q_irand(1, 3)));
              }
            }
          } else {
            soundIndex =
        ...
        case TAUNT_GLOAT:
          soundIndex =
              CG_CustomSound(es->number, va("*victory%d.wav", Q_irand(1, 3)));
          break;
        }
      }
      if (!soundIndex) {
  ...
  case EV_ANGER3:
    ...
    CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE,
                          va("*anger%i.wav", event - EV_ANGER1 + 1));
    break;
  ...
  case EV_CHOKE3:
    DEBUGNAME("EV_CHOKEx");
    CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE,
                          va("*choke%i.wav", event - EV_CHOKE1 + 1));
    break;
  ...
  case EV_COVER5:
    DEBUGNAME("EV_COVERx");
    CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE,
                          va("*cover%i.wav", event - EV_COVER1 + 1));
    break;
  case EV_DETECTED1:
  case EV_DETECTED2:
  case EV_DETECTED3:
  ...
  case EV_LOOK2:
    DEBUGNAME("EV_LOOKx");
    CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE,
                          va("*look%i.wav", event - EV_LOOK1 + 1));
    break;
  case EV_LOST1:
    DEBUGNAME("EV_LOST1");
  ...
  case EV_SIGHT3:
    DEBUGNAME("EV_SIGHTx");
    CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE,
                          va("*sight%i.wav", event - EV_SIGHT1 + 1));
    break;
  case EV_SOUND1:
  case EV_SOUND2:
  case EV_SOUND3:
    DEBUGNAME("EV_SOUNDx");
    CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE,
                          va("*sound%i.wav", event - EV_SOUND1 + 1));
    break;
  case EV_SUSPICIOUS1:
  case EV_SUSPICIOUS2:
  case EV_SUSPICIOUS3:
  ...
  case EV_COMBAT3:
    ...
    CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE,
                          va("*combat%i.wav", event - EV_COMBAT1 + 1));
    break;
  case EV_JDETECTED1:
  case EV_JDETECTED2:
  case EV_JDETECTED3:
  ...
  case EV_TAUNT3:
    DEBUGNAME("EV_TAUNTx");
    CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE,
                          va("*taunt%i.wav", event - EV_TAUNT1 + 1));
    break;
  case EV_JCHASE1:
  case EV_JCHASE2:
  case EV_JCHASE3:
    ...
    CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE,
                          va("*jchase%i.wav", event - EV_JCHASE1 + 1));
    break;
  case EV_JLOST1:
  case EV_JLOST2:
  case EV_JLOST3:
    DEBUGNAME("EV_JLOSTx");
    CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE,
                          va("*jlost%i.wav", event - EV_JLOST1 + 1));
    break;
  case EV_DEFLECT1:
  case EV_DEFLECT2:
  case EV_DEFLECT3:
  ...
  case EV_GLOAT3:
    DEBUGNAME("EV_GLOATx");
    CG_TryPlayCustomSound(NULL, es->number, CHAN_VOICE,
                          va("*gloat%i.wav", event - EV_GLOAT1 + 1));
    break;
  case EV_PUSHFAIL:
  ...
  case EV_ITEM_PICKUP:
    DEBUGNAME("EV_ITEM_PICKUP");
    {
      ...
      index = cg_entities[es->eventParm]
                  .currentState.modelindex; // player predicted

      if (index < 1 &&
          cg_entities[es->eventParm]
              .currentState.isJediMaster) { // a holocron most likely
        index = cg_entities[es->eventParm].currentState.trickedentindex4;
        trap->S_StartSound(NULL, es->number, CHAN_AUTO,
                           cgs.media.holocronPickup);
        ...
        if (es->number == cg.snap->ps.clientNum && showPowersName[index]) {
          ...
          CG_CenterPrint(
              va("%s %s\n", strText,
                 CG_GetStringEdString("SP_INGAME", showPowersName[index])),
              SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH);
        }

        // Show the player their force selection bar in case picking the
        // holocron up changed the current selection
        if (index != FP_SABER_OFFENSE && index != FP_SABER_DEFENSE &&
            index != FP_SABERTHROW && index != FP_LEVITATION &&
            es->number == cg.snap->ps.clientNum &&
            (index == cg.snap->ps.fd.forcePowerSelected ||
      ...
      if (cg_entities[es->eventParm].weapon >=
          cg.time) { // rww - an unfortunately necessary hack to prevent double
                     // item pickups
        break;
      }

      // Hopefully even if this entity is somehow removed and replaced with,
      // say, another item, this time will have expired by the time that item
      // needs to be picked up. Of course, it's quite possible this will fail
      // miserably, so if you've got a better solution then please do use it.
  ...
  case EV_NOAMMO:
    ...
    //);
    if (es->number == cg.snap->ps.clientNum) {
      if (CG_InFighter() || CG_InATST() ||
          cg.snap->ps.weapon ==
              WP_NONE) { // just letting us know our vehicle is out of ammo
        // FIXME: flash something on HUD or give some message so we know we have
        // no ammo
        centity_t *localCent = &cg_entities[cg.snap->ps.clientNum];
        if (localCent->m_pVehicle && localCent->m_pVehicle->m_pVehicleInfo &&
            localCent->m_pVehicle->m_pVehicleInfo->weapon[es->eventParm]
                .soundNoAmmo) { // play the "no Ammo" sound for this weapon
          trap->S_StartSound(
        ...
        // flash the HUD so they associate the sound with the visual indicator
        // that they don't have enough ammo
        if (cg_vehicleAmmoWarningTime < cg.time ||
            cg_vehicleAmmoWarning !=
                es->eventParm) { // if there's already one going, don't
                                 // interrupt it (unless they tried to fire
                                 // another weapon that's out of ammo)
  ...
  case EV_CHANGE_WEAPON:
    DEBUGNAME("EV_CHANGE_WEAPON");
    {
      ...
      } else if (weapon != WP_SABER) { // not sure what SP is doing for this but
                                       // I don't want a select sound for saber
                                       // (it has the saber-turn-on)
  ...
  case EV_FIRE_WEAPON:
    ...
    if (cent->currentState.number >= MAX_CLIENTS &&
        cent->currentState.eType != ET_NPC) { // special case for turret firing
      ...
      if (cent->currentState.eventParm) {
        trap->G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt2, &matrix,
                                  ...
                                  cent->currentState.origin, cg.time,
      ...
      trap->FX_PlayEffectID(cgs.effects.mEmplacedMuzzleFlash, gunpoint,
                            gunangle, -1, -1, qfalse);
    } else if (cent->currentState.weapon != WP_EMPLACED_GUN ||
               cent->currentState.eType == ET_NPC) {
      if (cent->currentState.eType == ET_NPC &&
          cent->currentState.NPC_class == CLASS_VEHICLE &&
          cent->m_pVehicle) { // vehicles do nothing for clientside weapon fire
  ...
  case EV_ALT_FIRE:
    ...
    if (cent->currentState.weapon ==
        WP_EMPLACED_GUN) { // don't do anything for emplaced stuff
      break;
    }
  ...
  case EV_SABER_ATTACK:
    ...
    {
      qhandle_t swingSound = trap->S_RegisterSound(
          va("sound/weapons/saber/saberhup%i.wav", Q_irand(1, 8)));
      ...
      if (client && client->infoValid &&
          client->saber[0].swingSound[0]) { // custom swing sound
  ...
  case EV_SABER_HIT:
    DEBUGNAME("EV_SABER_HIT");
    {
      ...
      int hitSound = trap->S_RegisterSound(
          va("sound/weapons/saber/saberhit%i.wav", Q_irand(1, 3)));

      if (es->otherEntityNum2 >= 0 &&
          es->otherEntityNum2 <
              ENTITYNUM_NONE) { // we have a specific person who is causing this
                                // effect, see if we should override it with any
                                // custom saber effects/sounds
        clientInfo_t *client = NULL;
        ...
        if (client && client->infoValid) {
          ...
          if (WP_SaberBladeUseSecondBladeStyle(
            ...
            if (client->saber[saberNum]
                    .hitPersonEffect2) { // custom hit person effect
              hitPersonFxID = hitPersonSmallFxID = hitPersonMidFxID =
            ...
            if (client->saber[saberNum]
                    .hitPersonEffect) { // custom hit person effect
              hitPersonFxID = hitPersonSmallFxID = hitPersonMidFxID =
      ...
      if (es->eventParm ==
          16) { // Make lots of sparks, something special happened
        vec3_t fxDir;
        ...
        if (es->eventParm == 3) { // moderate or big hits.
          trap->FX_PlayEffectID(hitPersonSmallFxID, es->origin, fxDir, -1, -1,
                                qfalse);
        } else if (es->eventParm == 2) { // this is for really big hits.
          trap->FX_PlayEffectID(hitPersonMidFxID, es->origin, fxDir, -1, -1,
                                qfalse);
        } else { // this should really just be done in the effect itself, no?
          trap->FX_PlayEffectID(hitPersonFxID, es->origin, fxDir, -1, -1,
                                qfalse);
          trap->FX_PlayEffectID(hitPersonFxID, es->origin, fxDir, -1, -1,
                                qfalse);
          trap->FX_PlayEffectID(hitPersonFxID, es->origin, fxDir, -1, -1,
        ...
        // old jk2mp method
        /*
        trap->S_StartSound(es->origin, es->number, CHAN_AUTO,
        trap->S_RegisterSound("sound/weapons/saber/saberhit.wav"));
        trap->FX_PlayEffectID( trap->FX_RegisterEffect("saber/spark.efx"),
        es->origin, fxDir, -1, -1, qfalse );
        */

        trap->FX_PlayEffectID(hitOtherFxID, es->origin, fxDir, -1, -1, qfalse);
      }

      // rww - this means we have the number of the ent being hit and the ent
      // that owns the saber doing the hit. This being the case, we can store
      // these indecies and the current time in order to do some visual tricks on
      // the client between frames to make it look like we're actually continuing
  ...
  case EV_SABER_BLOCK:
    DEBUGNAME("EV_SABER_BLOCK");
    {
      if (es->eventParm) { // saber block
        ...
        qhandle_t blockSound = trap->S_RegisterSound(
            va("sound/weapons/saber/saberblock%d.wav", Q_irand(1, 9)));
        qboolean noFlare = qfalse;

        if (es->otherEntityNum2 >= 0 &&
            es->otherEntityNum2 <
                ENTITYNUM_NONE) { // we have a specific person who is causing
                                  // this effect, see if we should override it
                                  // with any custom saber effects/sounds
          clientInfo_t *client = NULL;
          if (cg_entities[es->otherEntityNum2].currentState.eType == ET_NPC) {
            client = cg_entities[es->otherEntityNum2].npcClient;
          } else if (es->otherEntityNum2 < MAX_CLIENTS) {
          ...
          if (client && client->infoValid) {
            ...
            if (WP_SaberBladeUseSecondBladeStyle(
                    ...
                    bladeNum)) { // use second blade style values
              if (client->saber[saberNum]
                      .blockEffect2) { // custom saber block effect
              ...
              if (client->saber[saberNum]
                      .blockEffect) { // custom saber block effect
        ...
        trap->FX_PlayEffectID(cgs.effects.mBlasterDeflect, es->origin, fxDir,
                              -1, -1, qfalse);
      }
    }
    break;
  ...
  case EV_SABER_CLASHFLARE:
    DEBUGNAME("EV_SABER_CLASHFLARE");
    {
      ...
      VectorCopy(es->origin, cg_saberFlashPos);
      trap->S_StartSound(
          es->origin, -1, CHAN_WEAPON,
          trap->S_RegisterSound(
              va("sound/weapons/saber/saberhitwall%i", Q_irand(1, 3))));
    }
    break;
  ...
  case EV_BECOME_JEDIMASTER:
    DEBUGNAME("EV_SABER_UNHOLSTER");
    {
      ...
      CG_Trace(&tr, position, playerMins, playerMaxs, dpos, es->number,
               MASK_SOLID);
  ...
  case EV_DISRUPTOR_MAIN_SHOT:
    ...
    if (cent->currentState.eventParm != cg.snap->ps.clientNum ||
      ...
      CG_GetClientWeaponMuzzleBoltPoint(cent->currentState.eventParm,
                                        cent->currentState.origin2);
    } else {
      if (cg.lastFPFlashPoint[0] || cg.lastFPFlashPoint[1] ||
          cg.lastFPFlashPoint[2]) { // get the position of the muzzle flash for
                                    // the first person weapon model from the
                                    // last frame
      ...
      while (clnum < MAX_CLIENTS) {
        if (CG_InClientBitflags(es, clnum)) {
          if (es->eventParm == 1) { // eventParm 1 is heal
            ...
            trap->S_StartSound(NULL, clnum, CHAN_AUTO,
  ...
  case EV_SCREENSHAKE:
    ...
    if (!es->modelindex ||
        cg.predictedPlayerState.clientNum == es->modelindex - 1) {
  ...
  case EV_ITEMUSEFAIL:
    ...
    if (cg.snap->ps.clientNum == es->number) {
      ...
      case SENTRY_NOROOM:
        psStringEDRef =
            (char *)CG_GetStringEdString("MP_INGAME", "SENTRY_NOROOM");
        break;
      case SENTRY_ALREADYPLACED:
        psStringEDRef =
            (char *)CG_GetStringEdString("MP_INGAME", "SENTRY_ALREADYPLACED");
        break;
      case SHIELD_NOROOM:
        psStringEDRef =
            (char *)CG_GetStringEdString("MP_INGAME", "SHIELD_NOROOM");
        break;
      case SEEKER_ALREADYDEPLOYED:
        psStringEDRef =
            (char *)CG_GetStringEdString("MP_INGAME", "SEEKER_ALREADYDEPLOYED");
        break;
      default:
        break;
      }
  ...
  case EV_BODYFADE:
    ...
    if (cent->ghoul2 && trap->G2_HaveWeGhoul2Models(cent->ghoul2)) {
      // turn the inside of the face off, to avoid showing the mouth when we
      // start alpha fading the corpse
      trap->G2API_SetSurfaceOnOff(cent->ghoul2, "head_eyes_mouth",
  ...
  case EV_SIEGE_ROUNDOVER:
    ...
    CG_SiegeRoundOver(&cg_entities[cent->currentState.weapon],
                      cent->currentState.eventParm);
    break;
  case EV_SIEGE_OBJECTIVECOMPLETE:
    DEBUGNAME("EV_SIEGE_OBJECTIVECOMPLETE");
    CG_SiegeObjectiveCompleted(&cg_entities[cent->currentState.weapon],
                               cent->currentState.eventParm,
  ...
  case EV_DESTROY_GHOUL2_INSTANCE:
    ...
    if (cg_entities[es->eventParm].ghoul2 &&
        trap->G2_HaveWeGhoul2Models(cg_entities[es->eventParm].ghoul2)) {
      if (es->eventParm < MAX_CLIENTS) { // You try to do very bad thing!
#ifdef _DEBUG
        Com_Printf("WARNING: Tried to kill a client ghoul2 instance with a "
                   "server event!\n");
#endif
        break;
      }
  ...
  case EV_DESTROY_WEAPON_MODEL:
    ...
    if (cg_entities[es->eventParm].ghoul2 &&
        trap->G2_HaveWeGhoul2Models(cg_entities[es->eventParm].ghoul2) &&
        trap->G2API_HasGhoul2ModelOnIndex(&(cg_entities[es->eventParm].ghoul2),
                                          1)) {
  ...
  case EV_CONC_ALT_IMPACT:
    DEBUGNAME("EV_CONC_ALT_IMPACT");
    {
      ...
      for (dist = 0.0f; dist < shotDist;
           dist += 64.0f) { // one effect would be.. a whole lot better
        VectorMA(es->origin2, dist, es->angles, spot);
        trap->FX_PlayEffectID(cgs.effects.mConcussionAltRing, spot, es->angles2,
                              -1, -1, qfalse);
      }

      ByteToDir(es->eventParm, dir);
      CG_MissileHitWall(WP_CONCUSSION, es->owner, position, dir,
                        IMPACTSOUND_DEFAULT, qtrue, 0);
  ...
  case EV_MISSILE_HIT:
    ...
    if (es->emplacedOwner) { // hack: this is an index to a custom effect to use
      trap->FX_PlayEffectID(cgs.gameEffects[es->emplacedOwner], position, dir,
                            -1, -1, qfalse);
    } else if (CG_VehicleWeaponImpact(cent)) { // a vehicle missile that uses an
                                               // overridden impact effect...
  ...
  case EV_MISSILE_MISS:
    ...
    if (es->emplacedOwner) { // hack: this is an index to a custom effect to use
      trap->FX_PlayEffectID(cgs.gameEffects[es->emplacedOwner], position, dir,
                            -1, -1, qfalse);
    } else if (CG_VehicleWeaponImpact(cent)) { // a vehicle missile that used an
                                               // overridden impact effect...
    } else if (cent->currentState.eFlags & EF_ALT_FIRING) {
  ...
  case EV_MISSILE_MISS_METAL:
    ...
    if (es->emplacedOwner) { // hack: this is an index to a custom effect to use
      ...
      CG_MissileHitWall(es->weapon, 0, position, dir, IMPACTSOUND_METAL, qtrue,
                        es->generic1);
    } else {
  ...
  case EV_PLAY_PORTAL_EFFECT_ID:
    DEBUGNAME("EV_PLAY_EFFECT_ID");
    {
      ...
      if (event == EV_PLAY_PORTAL_EFFECT_ID) { // This effect should only be
                                               // played inside sky portals.
        portalEffect = qtrue;
      }
  ...
  case EV_VOICECMD_SOUND:
    ...
    if (es->groundEntityNum < MAX_CLIENTS && es->groundEntityNum >= 0) {
      ...
      Q_strncpyz(
          descr,
          CG_GetStringForVoiceSound(CG_ConfigString(CS_SOUNDS + es->eventParm)),
          sizeof(descr));
      ...
      if (sfx) {
        if (clientNum !=
            cg.predictedPlayerState
                .clientNum) { // play on the head as well to simulate hearing in
                              // radio and in world
          if (ci->team ==
              cg.predictedPlayerState
                  .persistant[PERS_TEAM]) { // don't hear it if this person is
                                            // on the other team, but they can
                                            // still
            // hear it in the world spot.
            trap->S_StartSound(NULL, cg.snap->ps.clientNum, CHAN_MENU1, sfx);
          }
        }
        if (ci->team == cg.predictedPlayerState
                            .persistant[PERS_TEAM]) { // add to the chat box
          // hear it in the world spot.
  ...
  case EV_GENERAL_SOUND:
    ...
    if (es->saberEntityNum == TRACK_CHANNEL_2 ||
        es->saberEntityNum == TRACK_CHANNEL_3 ||
        es->saberEntityNum == TRACK_CHANNEL_5) { // channels 2 and 3 are for
                                                 // speed and rage, 5 for sight
      if (cgs.gameSounds[es->eventParm]) {
        CG_S_AddRealLoopingSound(es->number, es->pos.trBase, vec3_origin,
  ...
  case EV_GLOBAL_SOUND: // play from the player's head so it never diminishes
    ...
    if (cgs.gameSounds[es->eventParm]) {
      trap->S_StartSound(NULL, cg.snap->ps.clientNum, CHAN_MENU1,
                         cgs.gameSounds[es->eventParm]);
    } else {
      s = CG_ConfigString(CS_SOUNDS + es->eventParm);
      trap->S_StartSound(NULL, cg.snap->ps.clientNum, CHAN_MENU1,
  ...
  case EV_ENTITY_SOUND:
    ...
    // somewhat of a hack - weapon is the caller entity's index, trickedentindex
    // is the proper sound channel
    if (cgs.gameSounds[es->eventParm]) {
  ...
  case EV_GLASS_SHATTER:
    ...
    CG_GlassShatter(es->genericenemyindex, es->origin, es->angles,
                    es->trickedentindex, es->pos.trTime);
    break;

  case EV_DEBRIS:
    DEBUGNAME("EV_DEBRIS");
    CG_Chunks(es->owner, es->origin, es->angles, es->origin2, es->angles2,
              es->speed, es->eventParm, es->trickedentindex, es->modelindex,
              es->apos.trBase[0]);
    break;
  ...
  case EV_WEAPON_CHARGE:
    ...
    if (cg_weapons[es->eventParm].chargeSound) {
      trap->S_StartSound(NULL, es->number, CHAN_WEAPON,
                         cg_weapons[es->eventParm].chargeSound);
  ...
  case EV_WEAPON_CHARGE_ALT:
    ...
    if (cg_weapons[es->eventParm].altChargeSound) {
      trap->S_StartSound(NULL, es->number, CHAN_WEAPON,
                         cg_weapons[es->eventParm].altChargeSound);
    }
    break;
