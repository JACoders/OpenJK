...
void SV_Startup(void) {
  ...
  svs.clients =
      (struct client_s *)Z_Malloc(sizeof(client_t) * 1, TAG_CLIENTS, qtrue);
...
void SV_SpawnServer(const char *server, ForceReload_e eForceReload,
  ...
  if (!CM_SameMap(server)) { // rww - only clear if not loading the same map
    CM_ClearMap();
  }
  ...
  /*
                  This is useful for debugging memory fragmentation.  Please
     don't remove it.
  */
  ...
  svs.snapshotEntities = (entityState_t *)Z_Malloc(
      sizeof(entityState_t) * svs.numSnapshotEntities, TAG_CLIENTS, qtrue);
  ...
  for (i = 0; i < 1; i++) {
    ...
    if (svs.clients[i].state >= CS_CONNECTED) {
      ...
      denied = ge->ClientConnect(
          i, qfalse, eNO /*qfalse*/); // firstTime = qfalse, qbFromSavedGame
      if (denied) {
