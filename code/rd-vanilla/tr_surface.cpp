...
static void RB_SurfaceOrientedQuad(void) {
  ...
  if (backEnd.currentEntity->e.rotation == 0) {
    ...
    VectorMA(tempUp, s * radius, left,
             up); // no need to use the temp anymore, so copy into the dest
                  // vector ( up )

    // This was copied for safekeeping, we're done, so we can move it back to
    // left
...
static void DoLine(const vec3_t start, const vec3_t end, const vec3_t up,
                   float spanWidth) {
  float spanWidth2;
  int vbase;
  ...
  tess.vertexColors[tess.numVertexes][0] =
      backEnd.currentEntity->e.shaderRGBA[0]; // * 0.25;//wtf??not sure why the
                                              // code would be doing this
  ...
  tess.vertexColors[tess.numVertexes][3] =
      backEnd.currentEntity->e.shaderRGBA[3]; // * 0.25;
  tess.numVertexes++;

  VectorMA(start, spanWidth2, up, tess.xyz[tess.numVertexes]);
  tess.texCoords[tess.numVertexes][0][0] =
      1; // backEnd.currentEntity->e.shaderTexCoord[0];
  tess.texCoords[tess.numVertexes][0][1] = 0;
  tess.vertexColors[tess.numVertexes][0] =
  ...
  tess.texCoords[tess.numVertexes][0][1] =
      1; // backEnd.currentEntity->e.shaderTexCoord[1];
  tess.vertexColors[tess.numVertexes][0] =
      backEnd.currentEntity->e.shaderRGBA[0];
  tess.vertexColors[tess.numVertexes][1] =
      backEnd.currentEntity->e.shaderRGBA[1];
...
static void DoLine2(const vec3_t start, const vec3_t end, const vec3_t up,
                    float spanWidth, float spanWidth2, const float tcStart,
                    const float tcEnd) {
  int vbase;
  ...
  tess.texCoords[tess.numVertexes][0][0] = 0;
  tess.texCoords[tess.numVertexes][0][1] =
      tcEnd; // backEnd.currentEntity->e.shaderTexCoord[1];
  tess.vertexColors[tess.numVertexes][0] =
      backEnd.currentEntity->e.shaderRGBA[0];
  tess.vertexColors[tess.numVertexes][1] =
      backEnd.currentEntity->e.shaderRGBA[1];
...
// If a cylinder has a tapered end that has a very small radius, the engine
// converts it to a cone.  Not a huge savings, but the texture mapping is
// slightly better
...
//-------------------------------------
{
  ...
  // this doesn't need to be perfect....just a rough compensation for zoom level
  // is enough
  ...
  // we only need to rotate around the larger radius, the smaller radius get's
  // welded
  ...
  // Calculate the texture coords so the texture can wrap around the whole
  // cylinder
  detail = 1.0f / (float)segments;

  RB_CHECKOVERFLOW(2 * (segments + 1),
                   3 * segments); // this isn't 100% accurate
  ...
  for (i = 0; i < segments; i++) {
    ...
    // We could add this vert once, but using the given texture mapping method,
    // we need to generate different texture coordinates
    VectorCopy(tapered, tess.xyz[tess.numVertexes]);
    tess.texCoords[tess.numVertexes][0][0] =
        detail * i +
        detail *
            0.5f; // set the texture coordinates to the point half-way between
                  // the untapered ends....but on the other end of the texture
  ...
  // last point has the same verts as the first, but does not share the same tex
  // coords, so we have to duplicate it
...
//-------------------------------------
{
  static vec3_t lower_points[NUM_CYLINDER_SEGMENTS],
      upper_points[NUM_CYLINDER_SEGMENTS];
  ...
  // check for tapering
  if (!(e->radius < 0.3f && e->backlerp < 0.3f) &&
      (e->radius < 0.3f || e->backlerp < 0.3f)) {
  ...
  RB_CHECKOVERFLOW(2 * (segments + 1),
                   6 * segments); // this isn't 100% accurate
...
// Up front, we create a random "shape", then apply that to each line
// segment...and then again to each of those segments...kind of like a fractal
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
{
  ...
  // it seems to look best to have a point on one side of the ideal line, then
  // the other point on the other side.
  VectorSet(sh2, 0.33f, // + Q_flrand(-1.0f, 1.0f) * 0.1f,	// fwd
            -sh1[1] + Q_flrand(-1.0f, 1.0f) *
                          0.02f, // forcing point to be on the opposite side of
                                 // the line -- right
            -sh1[2] + Q_flrand(-1.0f, 1.0f) * 0.02f); // up
}

//----------------------------------------------------------------------------
static void ApplyShape(vec3_t start, vec3_t end, vec3_t right, float sradius,
...
//----------------------------------------------------------------------------
{
  ...
  // recursion
  ApplyShape(start, point1, right, sradius, rads1, count - 1, startPerc,
...
//----------------------------------------------------------------------------
{
  ...
  for (i = 16; i <= dis; i += 16) {
    // because of our large step size, we may not actually draw to the end.  In
    // this case, fudge our percent so that we are basically complete
    if (i + 16 > dis) {
      perc = 1.0f;
    } else {
    ...
    VectorScale(
        fwd, Q_crandom(&e->frame) * 3.0f,
        temp); // move less in fwd direction, chaos also does not affect this
    VectorMA(temp, Q_crandom(&e->frame) * 7.0f * e->angles[0], rt,
             temp); // move more in direction perpendicular to line, angles is
                    // really the chaos
    VectorMA(temp, Q_crandom(&e->frame) * 7.0f * e->angles[0], up,
             temp); // move more in direction perpendicular to line

    // track our total level of offset from the ideal line
    VectorAdd(off, temp, off);

    // Move from start to end, always adding our current level of offset from
    // the ideal line
    //	Even though we are adding a random offset.....by nature, we always move
    //from exactly start....to end
    VectorAdd(start, off, cur);
    VectorScale(cur, 1.0f - perc, cur);
    VectorMA(cur, perc, end, cur);

    if (e->renderfx & RF_TAPERED) {
      // This does pretty close to perfect tapering since apply shape
      // interpolates the old and new as it goes along.
      //	by using one minus the square, the radius stays fairly constant,
      //then drops off quickly at the very point of the bolt
      oldRadius = radius * (1.0f - oldPerc * oldPerc);
      newRadius = radius * (1.0f - perc * perc);
    }

    // Apply the random shape to our line seg to give it some
    // micro-detail-jaggy-coolness.
    ApplyShape(cur, old, right, newRadius, oldRadius, 2 - r_lodbias->integer, 0,
               1);

    // randomly split off to create little tendrils, but don't do it too close
    // to the end and especially if we are not even of the forked variety
    if ((e->renderfx & RF_FORKED) && f_count > 0 &&
        Q_random(&e->frame) > 0.93f && (1.0f - perc) > 0.8f) {
      vec3_t newDest;
      ...
      // we could branch off using OLD and NEWDEST, but that would allow
      // multiple forks...whereas, we just want simpler brancing
...
=============
*/
/* // we could try to do something similar to this to get better normals into
the tess for these types of surfs.  As it stands, any shader pass that
//	requires a normal ( env map ) will not work properly since the normals
seem to essentially be random garbage. void RB_SurfacePolychain( srfPoly_t *p )
{ int		i; int		numv; vec3_t	a,b,normal={1,0,0};
        ...
        for ( i = 0; i < p->numVerts; i++ ) {
                ...
                *(int *)&tess.vertexColors[numv] = *(int *)p->verts[ i
].modulate;
...
inline static uint32_t ComputeFinalVertexColor(const byte *colors) {
  ...
  // an optimization could be added here to compute the style[0] (which is
  // always the world normal light)
  r = g = b = 0;
...
void RB_SurfaceTriangles(srfTriangles_t *srf) {
  ...
  for (i = 0; i < srf->numVerts; i++, dv++) {
    ...
    for (k = 0; k < MAXLIGHTMAPS; k++) {
      ...
      } else { // can't have an empty slot in the middle, so we are done
        break;
      }
    }
...
static void RB_SurfaceSaberGlow() {
  ...
  // Big hilt sprite
  // Please don't kill me Pat...I liked the hilt glow blob, but wanted a subtle
  // pulse.:)  Feel free to ditch it if you don't like it.  --Jeff Please don't
  // kill me Jeff...  The pulse is good, but now I want the halo bigger if the
  // saber is shorter...  --Pat
  DoSprite(e->origin, 5.5f + Q_flrand(0.0f, 1.0f) * 0.25f,
           0.0f); // Q_flrand(0.0f, 1.0f) * 360.0f );
}
...
static void RB_SurfaceLathe() {
  ...
  for (mu = 0.0f; mu <= 1.01f * d; mu += bezierStep) {
    ...
    for (i = 0; i < 2; i++) {
      l_oldpt2[i] = mum13 * e->axis[0][i] + group1 * e->axis[1][i] +
                    group2 * e->axis[2][i] + mu3 * e->oldorigin[i];
    }
    ...
    for (t = latheStep; t <= 360; t += latheStep) {
      ...
      VectorAdd(e->origin, tess.normal[tess.numVertexes],
      ...
      tess.texCoords[tess.numVertexes][0][1] =
          mu - bezierStep + cos(i + backEnd.refdef.floatTime) * pain;
      tess.vertexColors[tess.numVertexes][0] = e->shaderRGBA[0];
      tess.vertexColors[tess.numVertexes][1] = e->shaderRGBA[1];
      tess.vertexColors[tess.numVertexes][2] = e->shaderRGBA[2];
      tess.vertexColors[tess.numVertexes][3] = e->shaderRGBA[3];
      ...
      VectorSet(tess.normal[tess.numVertexes], oldpt2[0], oldpt2[1],
                l_oldpt2[1]);
      VectorAdd(e->origin, tess.normal[tess.numVertexes],
                tess.xyz[tess.numVertexes]);
      VectorNormalize(tess.normal[tess.numVertexes]);
      ...
      tess.texCoords[tess.numVertexes][0][0] = (t - latheStep) / 360.0f;
      tess.texCoords[tess.numVertexes][0][1] =
          mu + cos(i + backEnd.refdef.floatTime) * pain;
      tess.vertexColors[tess.numVertexes][0] = e->shaderRGBA[0];
      tess.vertexColors[tess.numVertexes][1] = e->shaderRGBA[1];
      tess.vertexColors[tess.numVertexes][2] = e->shaderRGBA[2];
      tess.vertexColors[tess.numVertexes][3] = e->shaderRGBA[3];
...
static void RB_SurfaceClouds() {
  // Disk definition
  float diskStripDef[DISK_DEF] = {0.0f, 0.4f, 0.7f, 1.0f};

  float diskAlphaDef[DISK_DEF] = {1.0f, 1.0f, 0.4f, 0.0f};

  float diskCurveDef[DISK_DEF] = {0.0f, 0.0f, 0.008f, 0.02f};

  // tube definition
  float tubeStripDef[TUBE_DEF] = {0.0f, 0.05f, 0.1f, 0.5f, 0.7f, 1.0f};

  float tubeAlphaDef[TUBE_DEF] = {0.0f, 0.45f, 1.0f, 1.0f, 0.45f, 0.0f};

  float tubeCurveDef[TUBE_DEF] = {0.0f, 0.004f, 0.006f, 0.01f, 0.006f, 0.0f};
  ...
  for (i = 0; i < ct - 1; i++) {
    VectorSet(oldpt, (stripDef[i] * (e->radius - e->rotation)) + e->rotation, 0,
              curveDef[i] * e->radius * e->backlerp);
    VectorSet(oldpt2,
              (stripDef[i + 1] * (e->radius - e->rotation)) + e->rotation, 0,
              curveDef[i + 1] * e->radius * e->backlerp);
    ...
    for (t = latheStep; t <= 360; t += latheStep) {
      ...
      if (t < 360.0f) {
        ...
        VectorSet(pt, (stripDef[i] * (e->radius - e->rotation)) + e->rotation,
                  0, curveDef[i] * e->radius * e->backlerp);
        VectorSet(pt2,
                  (stripDef[i + 1] * (e->radius - e->rotation)) + e->rotation,
                  0, curveDef[i + 1] * e->radius * e->backlerp);
      }
      ...
      tess.texCoords[tess.numVertexes][0][0] =
          tess.xyz[tess.numVertexes][0] * 0.1f;
      tess.texCoords[tess.numVertexes][0][1] =
          tess.xyz[tess.numVertexes][1] * 0.1f;
      tess.vertexColors[tess.numVertexes][0] =
          tess.vertexColors[tess.numVertexes][1] =
              tess.vertexColors[tess.numVertexes][2] =
                  e->shaderRGBA[0] * alphaDef[i];
      tess.vertexColors[tess.numVertexes][3] = e->shaderRGBA[3];
      tess.numVertexes++;
...
static bool RB_TestZFlare(vec3_t point) {
  ...
  if (window[0] < 0 || window[0] >= backEnd.viewParms.viewportWidth ||
    ...
    return qfalse; // shouldn't happen, since we check the clip[] above, except
                   // for FP rounding
  }
  ...
  qglReadPixels(backEnd.viewParms.viewportX + window[0],
                backEnd.viewParms.viewportY + window[1], 1, 1,
