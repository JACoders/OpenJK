...
static void UI_AddWeaponSelection(const int weaponIndex, const int ammoIndex, const int ammoAmount, const char *iconItemName, const char *litIconItemName,
								  const char *hexBackground, const char *soundfile);
static void UI_AddThrowWeaponSelection(const int weaponIndex, const int ammoIndex, const int ammoAmount, const char *iconItemName, const char *litIconItemName,
									   const char *hexBackground, const char *soundfile);
...
static void UI_OwnerDraw(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, int ownerDrawFlags, int align, float special,
						 float scale, vec4_t color, qhandle_t shader, int textStyle, int iFontIndex);
...
enum { MD_ACROBATICS = 0, MD_SINGLE_FAST, MD_SINGLE_MEDIUM, MD_SINGLE_STRONG, MD_DUAL_SABERS, MD_SABER_STAFF, MD_MOVE_TITLE_MAX };
...
static cvarTable_t cvarTable[] = {{&ui_menuFiles, "ui_menuFiles", "ui/menus.txt", NULL, CVAR_ARCHIVE},
#ifdef JK2_MODE
...
void _UI_Refresh(int realtime) {
	...
	if (Menus_AnyFullScreenVisible()) { // if not in full screen, don't mess with ghoul2
...
================
*/
extern "C" Q_EXPORT intptr_t QDECL vmMain(int command, int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7, int arg8, int arg9,
										  int arg10, int arg11) {
	return 0;
}
...
void Text_PaintWithCursor(float x, float y, float scale, vec4_t color, const char *text, int cursorPos, char cursor, int iMaxPixelWidth, int style,
						  int iFontIndex) {
	...
	*handle = -1;
	...
	if (feederID == FEEDER_SAVEGAMES) {
		...
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHeadCount) {
			*handle = ui.R_RegisterShaderNoMip(va("models/players/%s/icon_%s.jpg", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name,
...
qhandle_t UI_FeederItemImage(float feederID, int index) {
	if (feederID == FEEDER_PLAYER_SKIN_HEAD) {
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHeadCount) {
			...
			return ui.R_RegisterShaderNoMip(va("models/players/%s/icon_%s.jpg", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name,
											   uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHead[index].name));
		}
	} else if (feederID == FEEDER_PLAYER_SKIN_TORSO) {
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinTorsoCount) {
			...
			if (menu) {
				...
				if (item) {
					...
					if (modelPtr) {
						...
						Com_sprintf(skin, sizeof(skin), "models/players/%s/|%s|%s|%s", Cvar_VariableString("g_char_model"),
...
// Force Warnings
enum { FW_VERY_LIGHT = 0, FW_SEMI_LIGHT, FW_NEUTRAL, FW_SEMI_DARK, FW_VERY_DARK };
...
static void UI_CalcForceStatus(void) {
	...
	lightSide =
		pState->forcePowerLevel[FP_HEAL] + pState->forcePowerLevel[FP_TELEPATHY] + pState->forcePowerLevel[FP_PROTECT] + pState->forcePowerLevel[FP_ABSORB];

	darkSide = pState->forcePowerLevel[FP_GRIP] + pState->forcePowerLevel[FP_LIGHTNING] + pState->forcePowerLevel[FP_RAGE] + pState->forcePowerLevel[FP_DRAIN];
...
qboolean UI_ParseAnimFileSet(const char *animCFG, int *animFileIndex) { // Not going to bother parsing the sound config here.
	...
	for (i = 0; i < numdirs; i++, dirptr += dirlen + 1) {
		...
		if (f) {
			...
			for (j = 0; j < numfiles; j++, fileptr += filelen + 1) {
				...
				if (IsImageFile(dirptr, skinname, (qboolean)(building != 0))) { // if it exists
...
#define MAX_LINES 64
	...
	y += 15;
	Text_Paint(rect->x, y, scale, color, va("GL_VERSION: %s: %s", uiInfo.uiDC.glconfig.version_string, uiInfo.uiDC.glconfig.renderer_string), rect->w,
			   textStyle, iFontIndex);
	y += 15;
	Text_Paint(rect->x, y, scale, color, "GL_PIXELFORMAT:", rect->w, textStyle, iFontIndex);
	y += 15;
	Text_Paint(
		rect->x, y, scale, color,
		va("Color(%d-bits) Z(%d-bits) stencil(%d-bits)", uiInfo.uiDC.glconfig.colorBits, uiInfo.uiDC.glconfig.depthBits, uiInfo.uiDC.glconfig.stencilBits),
...
static int UI_CountForcePowers(void) {
	...
	if (cl && cl->gentity) {
		...
		return ps->forcePowerLevel[FP_HEAL] + ps->forcePowerLevel[FP_TELEPATHY] + ps->forcePowerLevel[FP_PROTECT] + ps->forcePowerLevel[FP_ABSORB] +
			   ps->forcePowerLevel[FP_GRIP] + ps->forcePowerLevel[FP_LIGHTNING] + ps->forcePowerLevel[FP_RAGE] + ps->forcePowerLevel[FP_DRAIN];
	} else {
		return uiInfo.forcePowerLevel[FP_HEAL] + uiInfo.forcePowerLevel[FP_TELEPATHY] + uiInfo.forcePowerLevel[FP_PROTECT] + uiInfo.forcePowerLevel[FP_ABSORB] +
			   uiInfo.forcePowerLevel[FP_GRIP] + uiInfo.forcePowerLevel[FP_LIGHTNING] + uiInfo.forcePowerLevel[FP_RAGE] + uiInfo.forcePowerLevel[FP_DRAIN];
	}
}
#endif
...
static void UI_DemoSetForceLevels(void) {
	...
	if (Q_stricmp(buffer, "t1_sour") == 0) { // NOTE : always set the uiInfo powers
	...
	if (pState) { // i am carrying over from a previous level, so get the increased power! (non-core only)
...
static void UI_ShutdownForceHelp(void) {
	...
	// In upgrade mode so just turn the deallocate button on
		...
		Com_sprintf(itemName, sizeof(itemName), "%s_level%ddesc", powerEnums[uiInfo.forcePowerUpdated].title,
...
static void UI_ShowForceLevelDesc(const char *forceName) {
	...
	// Update level description
	Com_sprintf(itemName, sizeof(itemName), "%s_level%ddesc", powerEnums[forcePowerI].title, pState->forcePowerLevel[powerEnums[forcePowerI].powerEnum]);
...
static void UI_WeaponsSelectionsComplete(void) {
	...
	if ((uiInfo.selectedWeapon1 != NOWEAPON) && (uiInfo.selectedWeapon2 != NOWEAPON) && (uiInfo.selectedThrowWeapon != NOWEAPON)) {
...
static void UI_RemoveWeaponSelection(const int weaponSelectionIndex) {
	...
	if (cl) // No client, get out
	{
		...
		if (cl->gentity && cl->gentity->client) {
			...
			// Remove ammo too
			if ((ammoIndex > 0) && (ammoIndex < AMMO_MAX)) { // But don't take it away if the other weapon is using that ammo
...
static void UI_UpdateCharacterSkin(void) {
	...
	Com_sprintf(skin, sizeof(skin), "models/players/%s/|%s|%s|%s", Cvar_VariableString("ui_char_model"), Cvar_VariableString("ui_char_skin_head"),
				Cvar_VariableString("ui_char_skin_torso"), Cvar_VariableString("ui_char_skin_legs"));
...
=================
*/
void Menus_SaveGoToMenu(const char *menuTo) { memcpy(GoToMenu, menuTo, sizeof(GoToMenu)); }
