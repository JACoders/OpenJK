...
void Interrogator_PartsMove(void) {
	...
	if (TIMER_Done(NPCS.NPC, "scalpelDelay")) {
		...
		//	trap->G2API_SetBoneAnglesIndex( &NPC->ghoul2[NPC->playerModel], NPC->genericBone2, NPC->pos2, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
		//NEGATIVE_Z, NULL );
	...
	// trap->G2API_SetBoneAnglesIndex( &NPC->ghoul2[NPC->playerModel], NPC->genericBone3, NPC->pos3, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
	// NULL );
...
void Interrogator_Melee(qboolean visible, qboolean advance) {
	if (TIMER_Done(NPCS.NPC, "attackDelay")) // Attack?
	{
		...
		if (NPCS.NPC->r.currentOrigin[2] >= NPCS.NPC->enemy->r.currentOrigin[2] + NPCS.NPC->enemy->r.mins[2] &&
			NPCS.NPC->r.currentOrigin[2] + NPCS.NPC->r.mins[2] + 8 < NPCS.NPC->enemy->r.currentOrigin[2] + NPCS.NPC->enemy->r.maxs[2]) {
			// gentity_t *tent;
