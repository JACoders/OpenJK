...
void UI_CacheSaberGlowGraphics(void) { // FIXME: these get fucked by
                                       // vid_restarts
...
qboolean UI_SaberParseParm(const char *saberName, const char *parmname,
                           char *saberData) {
  const char *token;
  const char *value;
  const char *p;
...
qboolean UI_SaberProperNameForSaber(const char *saberName,
                                    char *saberProperName) {
...
void UI_SaberLoadParms(void) {
  ...
  fileCnt = ui.FS_GetFileList("ext_data/sabers", ".sab", saberExtensionListBuf,
  ...
  for (i = 0; i < fileCnt; i++, holdChar += saberExtFNLen + 1) {
    ...
    if (len == -1) {
      ...
      if (totallen &&
          *(marker - 1) == '}') { // don't let it end on a } because that should
                                  // be a stand-alone token
        strcat(marker, " ");
        totallen++;
        marker++;
      }
      ...
      if (totallen + len >= MAX_SABER_DATA_SIZE) {
        Com_Error(ERR_FATAL,
                  ...
                  "must make the .npc files smaller)",
                  holdChar);
      }
...
void UI_DoSaber(vec3_t origin, vec3_t dir, float length, float lengthMax,
                float radius, saber_colors_t color) {
  ...
  // always add a light because sabers cast a nice glow before they slice you in
  // half!!  or something...
  /*
  if ( doLight )
  {//FIXME: RGB combine all the colors of the sabers you're using into one
  averaged color! cgi_R_AddLightToScene( mid, (length*2.0f) +
  (Q_flrand(0.0f, 1.0f)*8.0f), rgb[0], rgb[1], rgb[2] );
  }
  */

  memset(&saber, 0, sizeof(refEntity_t));

  // Saber glow is it's own ref type because it uses a ton of sprites, otherwise
  // it would eat up too many
  //	refEnts to do each glow blob individually
  saber.saberLength = length;

  // Jeff, I did this because I foolishly wished to have a bright halo as the
  // saber is unleashed. It's not quite what I'd hoped tho.  If you have any
  // ideas, go for it!  --Pat
  if (length < lengthMax) {
    radiusmult =
        1.0 +
        (2.0 /
         length); // Note this creates a curve, and length cannot be < 0.5.
  } else {
    radiusmult = 1.0;
  }
  ...
  saber.radius =
      (radiusStart + Q_flrand(-1.0f, 1.0f) * radiusRange) * radiusmult;
  ...
  saber.shaderRGBA[0] = saber.shaderRGBA[1] = saber.shaderRGBA[2] =
...
void UI_SaberDrawBlade(itemDef_t *item, char *saberName, int saberModel,
                       saberType_t saberType, vec3_t origin, float curYaw,
                       int bladeNum) {
  char bladeColorString[MAX_QPATH];
  vec3_t angles = {0};

  if (item->flags & (ITF_ISANYSABER) &&
      item->flags & (ITF_ISCHARACTER)) { // it's bolted to a dude!
    angles[YAW] = curYaw;
  } else {
    angles[PITCH] = curYaw;
    angles[ROLL] = 90;
  ...
  if (saberModel >= item->ghoul2.size()) { // uhh... invalid index!
    return;
  }

  if ((item->flags & ITF_ISSABER) && saberModel < 2) {
    DC->getCVarString("ui_saber_color", bladeColorString,
                      sizeof(bladeColorString));
  } else // if ( item->flags&ITF_ISSABER2 ) - presumed
  {
    DC->getCVarString("ui_saber2_color", bladeColorString,
                      sizeof(bladeColorString));
  }
  ...
  if (bolt == -1) {
    tagHack = qtrue;
    // hmm, just fall back to the most basic tag (this will also make it work
    // with pre-JKA saber models
  ...
  DC->g2_GetBoltMatrix(item->ghoul2, saberModel, bolt, &boltMatrix, angles,
                       origin, uiInfo.uiDC.realTime, NULL,
                       vec3_origin); // NULL was cgs.model_draw

  // work the matrix axis stuff into the original axis and origins used.
  DC->g2_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, bladeOrigin);
  DC->g2_GiveMeVectorFromMatrix(
      boltMatrix, NEGATIVE_X,
      axis[0]); // front (was NEGATIVE_Y, but the md3->glm exporter screws up
                // this tag somethin' awful)
  ...
  if (saberType == SABER_SITH_SWORD) { // draw no blade
    return;
  }

  UI_DoSaber(bladeOrigin, axis[0], bladeLength, bladeLength, bladeRadius,
             bladeColor);
}

extern qboolean ItemParse_asset_model_go(itemDef_t *item, const char *name);
extern qboolean ItemParse_model_g2skin_go(itemDef_t *item,
                                          const char *skinName);
...
void UI_SaberDrawBlades(itemDef_t *item, vec3_t origin, float curYaw) {
  ...
  if ((item->flags &
       ITF_ISCHARACTER) // hacked sabermoves sabers in character's hand
      ...
      if (numBlades) { // okay, here we go, time to draw each blade...
        ...
        for (int curBlade = 0; curBlade < numBlades; curBlade++) {
          if (UI_SaberShouldDrawBlade(saber, curBlade)) {
            UI_SaberDrawBlade(item, saber, saberModel, saberType, origin,
                              curYaw, curBlade);
          }
        }
      }
...
void UI_SaberAttachToChar(itemDef_t *item) {
  ...
  if (item->ghoul2.size() > 2 &&
      item->ghoul2[2].mModelindex >= 0) { // remove any extra models
    DC->g2_RemoveGhoul2Model(item->ghoul2, 2);
  }
  if (item->ghoul2.size() > 1 &&
      item->ghoul2[1].mModelindex >= 0) { // remove any extra models
  ...
  for (saberNum = 0; saberNum < numSabers; saberNum++) {
    ...
    if (UI_SaberModelForSaber(saber, modelPath)) { // successfully found a model
      int g2Saber = DC->g2_InitGhoul2Model(item->ghoul2, modelPath, 0, 0, 0, 0,
                                           0); // add the model
      if (g2Saber) {
        // get the customSkin, if any
        if (UI_SaberSkinForSaber(saber, skinPath)) {
          int g2skin = DC->registerSkin(skinPath);
          DC->g2_SetSkin(&item->ghoul2[g2Saber], 0,
                         g2skin); // this is going to set the surfs on/off
        ...
        re.G2API_AttachG2Model(&item->ghoul2[g2Saber], &item->ghoul2[0],
                               boltNum, 0);
      }
    }
  }
