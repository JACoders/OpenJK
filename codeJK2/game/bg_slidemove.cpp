...
qboolean PM_SlideMove(float gravMod) {
	...
	if (gravMod) {
		...
		if (pml.groundPlane) {
			if (PM_GroundSlideOkay(pml.groundTrace.plane.normal[2])) { // slide along the ground plane
	...
	for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
		...
		if (trace.surfaceFlags & SURF_NODAMAGE) {
			damageSelf = qfalse;
				   ...
				   trace.plane.normal[2] > 0.5f) { // if we land on the ground, let falling damage do it's thing itself, otherwise do impact damage
			damageSelf = qfalse;
		} else {
			damageSelf = qtrue;
		}
...
void PM_StepSlideMove(float gravMod) {
	...
	} else if (pm->maxs[2] <= 0) { // short little guys can't go up steps... FIXME: just make this a flag for certain NPCs- especially ones that roll?
		stepSize = 4;
	}
	...
	if (!pm->ps->velocity[0] && !pm->ps->velocity[1]) { // All our velocity was cancelled sliding
		return;
	}
	...
	if (fabs(stepUpMove[0]) < 0.1 && fabs(stepUpMove[1]) < 0.1 && VectorLengthSquared(slideMove) > VectorLengthSquared(stepUpMove)) {
		...
		if (!trace.allsolid) {
			if (pm->ps->clientNum && isATST && g_entities[trace.entityNum].client &&
				g_entities[trace.entityNum].client->playerTeam == pm->gent->client->playerTeam) { // AT-ST's don't step up on allies
			} else {
				VectorCopy(trace.endpos, pm->ps->origin);
			}
		}
