...
extern void G_TeamRetaliation(gentity_t *targ, gentity_t *attacker,
                              qboolean stopIcarus);
...
extern void WP_SaberSetColor(gentity_t *ent, int saberNum, int bladeNum,
                             char *colorName);
...
stringID_table_t BSETTable[] = {
    ...
    ENUM2STRING(BSET_BLOCKED), //# script to run when blocked by a friendly NPC
                               //or player
    ENUM2STRING(BSET_BUMPED),  //# script to run when bumped into a friendly NPC
                               //or player (can set bumpRadius)
...
stringID_table_t WPTable[] = {
    ...
    ENUM2STRING(WP_NONE),
    // Player weapons
    ENUM2STRING(WP_SABER), // NOTE: lots of code assumes this is the first
                           // weapon (... which is crap) so be careful -Ste.
    ENUM2STRING(
    ...
    // NOTE: player can only have up to 16 weapons), anything after that is
    // enemy only
...
stringID_table_t INVTable[] = {ENUM2STRING(INV_ELECTROBINOCULARS),
...
stringID_table_t HLTable[] = {{"NULL", -1},
...
stringID_table_t setTable[] = {ENUM2STRING(SET_SPAWNSCRIPT), // 0
...
qboolean Q3_TaskIDPending(gentity_t *ent, taskID_t taskType) {
  if (ent->m_iIcarusID ==
      IIcarusInterface::
          ICARUS_INVALID /*!ent->sequencer || !ent->taskManager*/) {
    return qfalse;
  }
...
void Q3_TaskIDComplete(gentity_t *ent, taskID_t taskType) {
  ...
  if (ent->m_iIcarusID != IIcarusInterface::ICARUS_INVALID /*ent->taskManager*/
      && Q3_TaskIDPending(ent, taskType)) {                // Complete it
    IIcarusInterface::GetIcarus()->Completed(ent->m_iIcarusID,
                                             ent->taskID[taskType]);
...
static char *Q3_GetAnimBoth(gentity_t *ent) {
  ...
  if (Q_stricmp(lowerName, upperName)) {
    ...
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_GetAnimBoth: legs and torso animations did "
...
static void Q3_SetObjective(const char *ObjEnum, int status) {
  ...
  if (objectiveID == LIGHTSIDE_OBJ &&
      status == SET_OBJ_FAILED) { // don't do this unless we just set that
                                  // specific objective (just turned to the dark
                                  // side) with this SET_OBJECTIVE command
...
static void Q3_SetMissionFailed(const char *TextEnum) {
  ...
  if (ent->client) { // hold this screen up for at least 2 seconds
...
static void Q3_ObjectiveClearAll(void) {
  client = &level.clients[0];
  memset(client->sess.mission_objectives, 0,
         sizeof(client->sess.mission_objectives));
}
...
Take any string, look for "jaden_male/" replace with "jaden_fmle/" based on
"sex" And: Take any string, look for "/mr_" replace with "/ms_" based on "sex"
...
static void Q3_SetCopyOrigin(int entID, const char *name) {
  ...
  if (found) {
    ...
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetCopyOrigin: ent %s not found!\n", name);
  }
}
...
static void Q3_SetVelocity(int entID, int axis, float speed) {
  ...
  if (!found) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetVelocity invalid entID %d\n", entID);
    return;
  }

  if (!found->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetVelocity: not a client %d\n", entID);
    return;
  }
...
static void Q3_SetAdjustAreaPortals(int entID, qboolean adjust) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetAdjustAreaPortals: invalid entID %d\n",
                             entID);
    return;
  }

  ent->svFlags = (adjust) ? (ent->svFlags | SVF_MOVER_ADJ_AREA_PORTALS)
...
static void Q3_SetDmgByHeavyWeapOnly(int entID, qboolean dmg) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetDmgByHeavyWeapOnly: invalid entID %d\n",
                             entID);
    return;
  }

  ent->flags = (dmg) ? (ent->flags | FL_DMG_BY_HEAVY_WEAP_ONLY)
...
static void Q3_SetShielded(int entID, qboolean dmg) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetShielded: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_SetNoGroups(int entID, qboolean noGroups) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetNoGroups: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetNoGroups: ent %s is not an NPC!\n",
                             ent->targetname);
    return;
  }

  ent->NPC->scriptFlags = noGroups ? (ent->NPC->scriptFlags | SCF_NO_GROUPS)
...
=============
*/
extern void misc_model_breakable_gravity_init(gentity_t *ent,
...
void moverCallback(gentity_t *ent) {
  ...
  if (!Q_stricmp("misc_model_breakable", ent->classname) &&
      ent->physicsBounce) { // a gravity-affected model
...
void anglerCallback(gentity_t *ent) {
  ...
  VectorMA(ent->s.apos.trBase, (ent->s.apos.trDuration * 0.001f),
           ent->s.apos.trDelta, ent->currentAngles);
...
entID %d\n", entID); return;
        ...
        if ( ent->client || ent->NPC || Q_stricmp(ent->classname,
"target_scriptrunner") == 0 )
        {
                Quake3Game()->DebugPrint( WL_ERROR, "Q3_Lerp2Start: ent %d is
NOT a mover!\n", entID); return;
        }
        ...
        ent->s.eType = ET_MOVER;
        ent->e_ReachedFunc = reachedF_moverCallback;		//Callsback the
the completion of the move if ( ent->damage )
        {
...
/*static void Q3_Lerp2End( int entID, int taskID, float duration )
{
        ...
        if(!ent)
        {
                Quake3Game()->DebugPrint( WL_WARNING, "Q3_Lerp2End: invalid
entID %d\n", entID); return;
        }
...
"target_scriptrunner") == 0 )
        {
                Quake3Game()->DebugPrint( WL_ERROR, "Q3_Lerp2End: ent %d is NOT
a mover!\n", entID); return;
        }
...
"target_scriptrunner") == 0 )
        {
                Quake3Game()->DebugPrint( WL_ERROR, "Q3_Lerp2Pos: ent %d is NOT
a mover!\n", entID); return;
        }
...
static void Q3_Lerp2Origin(int taskID, int entID, vec3_t origin,
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_Lerp2Origin: invalid entID %d\n", entID);
    return;
  }

  if (ent->client || ent->NPC ||
      Q_stricmp(ent->classname, "target_scriptrunner") == 0) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_Lerp2Origin: ent %d is NOT a mover!\n", entID);
    return;
  }
  ...
  if (moverState == MOVER_POS1 || moverState == MOVER_2TO1) {
    ...
    if (moverState == MOVER_POS1) { // open the portal
      if (ent->svFlags & SVF_MOVER_ADJ_AREA_PORTALS) {
        gi.AdjustAreaPortalState(ent, qtrue);
      }
    }
...
static void Q3_SetOriginOffset(int entID, int axis, float offset) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetOriginOffset: invalid entID %d\n", entID);
    return;
  }
  ...
  if (ent->client || ent->NPC ||
      Q_stricmp(ent->classname, "target_scriptrunner") == 0) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetOriginOffset: ent %d is NOT a mover!\n",
                             entID);
    return;
  }
...
=============
*/
/*static void Q3_Lerp2Angles( int taskID, int entID, vec3_t angles, float
duration )
{
        gentity_t	*ent = &g_entities[entID];
        vec3_t		ang;
        ...
        if(!ent)
        {
                Quake3Game()->DebugPrint( WL_WARNING, "Q3_Lerp2Angles: invalid
entID %d\n", entID); return;
        }
...
"target_scriptrunner") == 0 )
        {
                Quake3Game()->DebugPrint( WL_ERROR, "Q3_Lerp2Angles: ent %d is
NOT a mover!\n", entID); return;
        }
        ...
        for ( i = 0; i < 3; i++ )
        {
                ...
                ent->s.apos.trDelta[i] = ( ang[i] / ( ent->s.apos.trDuration *
0.001f ) );
        }
...
static qboolean Q3_SetNavGoal(int entID, const char *name) {
  ...
  if (!ent->NPC->tempGoal) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetNavGoal: tried to set a navgoal (\"%s\") "
                             "on a dead NPC: \"%s\"\n",
                             name, ent->script_targetname);
    return qfalse;
  }
  ...
  if (Q_stricmp("null", name) == 0 || Q_stricmp("NULL", name) == 0) {
    ...
    if (TAG_GetOrigin2(NULL, name, goalPos) == false) {
      ...
      if (!targ) {
        Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                                 "Q3_SetNavGoal: can't find NAVGOAL \"%s\"\n",
                                 name);
        return qfalse;
      } else {
        ent->NPC->goalEntity = targ;
        ent->NPC->goalRadius = sqrt(ent->maxs[0] + ent->maxs[0]) +
                               sqrt(targ->maxs[0] + targ->maxs[0]);
...
static void SetLowerAnim(int entID, int animID) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "SetLowerAnim: invalid entID %d\n", entID);
    return;
  }

  if (!ent->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "SetLowerAnim: ent %d is NOT a player or NPC!\n",
                             entID);
    return;
  }

  NPC_SetAnim(ent, SETANIM_LEGS, animID,
...
static void Q3_SetAnimHoldTime(int entID, int int_data, qboolean lower) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetAnimHoldTime: invalid entID %d\n", entID);
    return;
  }
  ...
  if (!ent->client) {
    Quake3Game()->DebugPrint(
        IGameInterface::WL_ERROR,
        "Q3_SetAnimHoldTime: ent %d is NOT a player or NPC!\n", entID);
    return;
  }
...
static void Q3_SetEnemy(int entID, const char *name) {
  ...
  if (!Q_stricmp("NONE", name) || !Q_stricmp("NULL", name)) {
    ...
    if (enemy == NULL) {
      Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                               "Q3_SetEnemy: no such enemy: '%s'\n", name);
      return;
    }
    /*else if(enemy->health <= 0)
    {
            //Quake3Game()->DebugPrint( WL_ERROR, "Q3_SetEnemy: ERROR - desired
    enemy has health %d\n", enemy->health ); return;
    }*/
    else {
      if (ent->NPC) {
...
static void Q3_SetLeader(int entID, const char *name) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetLeader: invalid entID %d\n", entID);
    return;
  }

  if (!ent->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetLeader: ent %d is NOT a player or NPC!\n",
                             entID);
    return;
  }
  ...
  if (!Q_stricmp("NONE", name) || !Q_stricmp("NULL", name)) {
    ...
    if (leader == NULL) {
      // Quake3Game()->DebugPrint( WL_ERROR,"Q3_SetEnemy: unable to locate
      // enemy: '%s'\n", name );
      return;
    } else if (leader->health <= 0) {
      // Quake3Game()->DebugPrint( WL_ERROR,"Q3_SetEnemy: ERROR - desired enemy
      // has health %d\n", enemy->health );
      return;
    } else {
...
stringID_table_t teamTable[] = {
    ENUM2STRING(TEAM_FREE),  ENUM2STRING(TEAM_PLAYER),
    ENUM2STRING(TEAM_ENEMY), ENUM2STRING(TEAM_NEUTRAL),
    {"", TEAM_FREE},
};
...
static void Q3_SetPlayerTeam(int entID, const char *teamName) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetPlayerTeam: invalid entID %d\n", entID);
    return;
  }
  ...
  if (!ent->client) {
    Quake3Game()->DebugPrint(
        IGameInterface::WL_ERROR,
        "Q3_SetPlayerTeam: ent %d is NOT a player or NPC!\n", entID);
    return;
  }
...
static void Q3_SetEnemyTeam(int entID, const char *teamName) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetEnemyTeam: invalid entID %d\n", entID);
    return;
  }
  ...
  if (!ent->client) {
    Quake3Game()->DebugPrint(
        IGameInterface::WL_ERROR,
        "Q3_SetEnemyTeam: ent %d is NOT a player or NPC!\n", entID);
    return;
  }
...
static void Q3_SetHealth(int entID, int data) {
  ...
  if (ent->health <= 0) {
    Quake3Game()->DebugPrint(
        IGameInterface::WL_WARNING,
        "Q3_SetHealth: trying to set health on a dead guy! %d\n", entID);
  }
...
static void Q3_SetArmor(int entID, int data) {
  ...
  if (ent->s.number == 0) { // clamp armor to max_health
...
static qboolean Q3_SetBState(int entID, const char *bs_name) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetBState: invalid entID %d\n", entID);
    return qtrue;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetBState: '%s' is not an NPC\n",
                             ent->targetname);
...
static qboolean Q3_SetTempBState(int entID, const char *bs_name) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetTempBState: invalid entID %d\n", entID);
    return qtrue;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetTempBState: '%s' is not an NPC\n",
                             ent->targetname);
    return qtrue; // ok to complete
  }
...
static void Q3_SetDefaultBState(int entID, const char *bs_name) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetDefaultBState: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetDefaultBState: '%s' is not an NPC\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetDPitch(int entID, float data) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetDPitch: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC || !ent->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetDPitch: '%s' is not an NPC\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetDYaw(int entID, float data) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetDYaw: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetDYaw: '%s' is not an NPC\n",
                             ent->targetname);
    return;
  }
  ...
  if (!ent->enemy) { // don't mess with this if they're aiming at someone
    ...
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Could not set DYAW: '%s' has an enemy (%s)!\n",
...
static void Q3_SetShootDist(int entID, float data) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetShootDist: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetShootDist: '%s' is not an NPC\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetVisrange(int entID, float data) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetVisrange: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetVisrange: '%s' is not an NPC\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetEarshot(int entID, float data) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetEarshot: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetEarshot: '%s' is not an NPC\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetVigilance(int entID, float data) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetVigilance: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetVigilance: '%s' is not an NPC\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetVFOV(int entID, int data) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetVFOV: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetVFOV: '%s' is not an NPC\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetHFOV(int entID, int data) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetHFOV: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetHFOV: '%s' is not an NPC\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetWidth(int entID, int data) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetWidth: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetWidth: '%s' is not an NPC\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetTimeScale(int entID, const char *data) {
  // if we're skipping the script DO NOT allow timescale to be set (skipping
  // needs it at 100)
...
static void Q3_SetInvisible(int entID, qboolean invisible) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetInvisible: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_SetVampire(int entID, qboolean vampire) {
  ...
  if (!self || !self->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetVampire: entID %d not a client\n", entID);
    return;
  }
...
static void Q3_SetGreetAllies(int entID, qboolean greet) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetGreetAllies: invalid entID %d\n", entID);
    return;
  }

  if (!self->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetGreetAllies: ent %s is not an NPC!\n",
                             self->targetname);
    return;
  }
...
static void Q3_SetViewTarget(int entID, const char *name) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetViewTarget: invalid entID %d\n", entID);
    return;
  }

  if (!self->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetViewTarget: '%s' is not a player/NPC!\n",
                             self->targetname);
    return;
  }
  ...
  if (viewtarget == NULL) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetViewTarget: can't find ViewTarget: '%s'\n",
                             name);
    return;
  }
...
static void Q3_SetWatchTarget(int entID, const char *name) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetWatchTarget: invalid entID %d\n", entID);
    return;
  }

  if (!self->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetWatchTarget: '%s' is not an NPC!\n",
                             self->targetname);
    return;
  }
...
void Q3_SetLoopSound(int entID, const char *name) {
  ...
  if (index) {
    ...
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetLoopSound: can't find sound file: '%s'\n",
                             name);
  }
}
...
void Q3_SetICARUSFreeze(int entID, const char *name, qboolean freeze) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetICARUSFreeze: invalid ent %s\n", name);
    return;
  }
...
void Q3_SetViewEntity(int entID, const char *name) {
  ...
  if (entID != 0) { // only valid on player
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetViewEntity: only valid on player\n", entID);
    return;
  }

  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetViewEntity: invalid entID %d\n", entID);
    return;
  }

  if (!self->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetViewEntity: '%s' is not a player!\n",
                             self->targetname);
    return;
  }
  ...
  if (viewtarget == NULL) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetViewEntity: can't find ViewEntity: '%s'\n",
                             name);
    return;
  }
...
void G_SetWeapon(gentity_t *self, int wp) {
  ...
  if (!self->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetWeapon: '%s' is not a player/NPC!\n",
                             self->targetname);
    return;
  }

  if (self->NPC) { // since a script sets a weapon, we presume we don't want to
                   // auto-match the player's weapon anymore
    self->NPC->aiFlags &= ~NPCAI_MATCHPLAYERWEAPON;
  }

  if (wp == WP_NONE) { // no weapon
    self->client->ps.weapon = WP_NONE;
    G_RemoveWeaponModels(self);
    if (self->s.number < MAX_CLIENTS) { // make sure the cgame-side knows this
      CG_ChangeWeapon(wp);
    }
    return;
  }
  ...
  if (self->NPC) { // Should NPCs have only 1 weapon at a time?
    ...
    G_AddEvent(self, EV_GENERAL_SOUND,
               G_SoundIndex("sound/weapons/change.wav"));
  } else {
    self->client->ps.stats[STAT_WEAPONS] |= (1 << wp);
    self->client->ps.ammo[weaponData[wp].ammoIndex] =
        ammoData[weaponData[wp].ammoIndex].max;
...
static void Q3_SetWeapon(int entID, const char *wp_name) {
  ...
  if (!Q_stricmp("drop", wp_name)) { // no weapon, drop it
...
static void Q3_SetItem(int entID, const char *item_name) {
  ...
  RegisterItem(
      item); // make sure the item is cached in case this runs at startup

  //		G_AddEvent( self, EV_ITEM_PICKUP, (item - bg_itemlist) );
  //		G_AddEvent( self, EV_GENERAL_SOUND, G_SoundIndex(
...
static void Q3_SetWalkSpeed(int entID, int int_data) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetWalkSpeed: invalid entID %d\n", entID);
    return;
  }

  if (!self->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetWalkSpeed: '%s' is not an NPC!\n",
                             self->targetname);
    return;
  }
...
static void Q3_SetRunSpeed(int entID, int int_data) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetRunSpeed: invalid entID %d\n", entID);
    return;
  }

  if (!self->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetRunSpeed: '%s' is not an NPC!\n",
                             self->targetname);
    return;
  }
...
static void Q3_SetYawSpeed(int entID, float float_data) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetYawSpeed: invalid entID %d\n", entID);
    return;
  }

  if (!self->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetYawSpeed: '%s' is not an NPC!\n",
                             self->targetname);
    return;
  }
...
static void Q3_SetAggression(int entID, int int_data) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetAggression: invalid entID %d\n", entID);
    return;
  }

  if (!self->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetAggression: '%s' is not an NPC!\n",
                             self->targetname);
    return;
  }
...
static void Q3_SetAim(int entID, int int_data) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetAim: invalid entID %d\n", entID);
    return;
  }

  if (!self->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetAim: '%s' is not an NPC!\n",
                             self->targetname);
    return;
  }
...
static void Q3_SetFriction(int entID, int int_data) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetFriction: invalid entID %d\n", entID);
    return;
  }

  if (!self->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetFriction: '%s' is not an NPC/player!\n",
                             self->targetname);
    return;
  }
...
static void Q3_SetGravity(int entID, float float_data) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetGravity: invalid entID %d\n", entID);
    return;
  }

  if (!self->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetGravity: '%s' is not an NPC/player!\n",
                             self->targetname);
    return;
  }
...
static void Q3_SetShotSpacing(int entID, int int_data) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetShotSpacing: invalid entID %d\n", entID);
    return;
  }

  if (!self->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetShotSpacing: '%s' is not an NPC!\n",
                             self->targetname);
    return;
  }
...
static void Q3_SetFollowDist(int entID, float float_data) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetFollowDist: invalid entID %d\n", entID);
    return;
  }

  if (!self->client || !self->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetFollowDist: '%s' is not an NPC!\n",
                             self->targetname);
    return;
  }
...
Q3_SetRenderCullRadius
  Description	: allows NPCs to be drawn even when their origin is very far
away from their model Return type	: static void Argument		: int
entID Argument		: float float_data (the new radius for render culling)
============
*/
static void Q3_SetRenderCullRadius(int entID, float float_data) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetRenderCullRadius: invalid entID %d\n",
                             entID);
    return;
  }
...
static void Q3_SetSquadName (int entID, const char *squadname)
{
        ...
        {
                Quake3Game()->DebugPrint( IGameInterface::WL_WARNING,
"Q3_SetSquadName: invalid entID %d\n", entID); return;
        }

        if ( !self->client )
        {
                Quake3Game()->DebugPrint( IGameInterface::WL_ERROR,
"Q3_SetSquadName: '%s' is not an NPC/player!\n", self->targetname ); return;
        }
...
static void Q3_SetTargetName(int entID, const char *targetname) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetTargetName: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_SetRemoveTarget(int entID, const char *target) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetRemoveTarget: invalid entID %d\n", entID);
    return;
  }

  if (!self->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetRemoveTarget: '%s' is not an NPC!\n",
                             self->targetname);
    return;
  }
...
static void Q3_SetPainTarget(int entID, const char *targetname) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetPainTarget: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_SetForcePowerLevel(int entID, int forcePower, int forceLevel) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetForcePowerLevel: invalid entID %d\n",
                             entID);
    return;
  }
...
static void Q3_GiveSecurityKey(int entID, char *keyname) {
  ...
  if (!other) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_GiveSecurityKey: invalid entID %d\n", entID);
    return;
  }
  ...
  if (!other->client) {
    Quake3Game()->DebugPrint(
        IGameInterface::WL_ERROR,
        "Q3_GiveSecurityKey: ent %s is not a player or NPC\n",
        other->targetname);
    return;
  }

  if (!keyname || !keyname[0] || !Q_stricmp("none", keyname) ||
      !Q_stricmp("null", keyname)) { // remove the key
    if (other->message) {            // remove it
...
void Q3_SetParm(int entID, int parmNum, const char *parmValue) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetParm: invalid entID %d\n", entID);
    return;
  }

  if (parmNum < 0 || parmNum >= MAX_PARMS) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "SET_PARM: parmNum %d out of range!\n", parmNum);
    return;
  }
...
static void Q3_SetCaptureGoal(int entID, const char *name) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetCaptureGoal: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetCaptureGoal: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetEvent(int entID, const char *event_name) {
  ...
  case EV_BAD:
  default:
    // Quake3Game()->DebugPrint( IGameInterface::WL_ERROR,"Q3_SetEvent: Invalid
    // Event %d\n", event );
    return;
    break;
  }
...
/*static void Q3_Use( int entID, const char *target )
{
        ...
        if ( !ent )
        {
                Quake3Game()->DebugPrint( IGameInterface::WL_WARNING, "Q3_Use:
invalid entID %d\n", entID); return;
        }
...
static qboolean Q3_SetBehaviorSet(int entID, int toSet,
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetBehaviorSet: invalid entID %d\n", entID);
    return qfalse;
  }
...
static void Q3_SetDelayScriptTime(int entID, int delayTime) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetDelayScriptTime: invalid entID %d\n",
                             entID);
    return;
  }
...
static void Q3_SetIgnorePain(int entID, qboolean data) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetIgnorePain: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetIgnorePain: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetIgnoreEnemies(int entID, qboolean data) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetIgnoreEnemies: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetIgnoreEnemies: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetIgnoreAlerts(int entID, qboolean data) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetIgnoreAlerts: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetIgnoreAlerts: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetNoTarget(int entID, qboolean data) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetNoTarget: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_SetDontShoot(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetDontShoot: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_SetDontFire(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetDontFire: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetDontFire: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetFireWeapon(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_FireWeapon: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetFireWeapon: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetFireWeaponNoAnim(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_FireWeaponNoAnim: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetFireWeaponNoAnim: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetSafeRemove(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetSafeRemove: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetSafeRemove: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetBobaJetPack(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetBobaJetPack: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetBobaJetPack: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
  ...
  if (ent->client && ent->client->NPC_class != CLASS_BOBAFETT) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetBobaJetPack: '%s' is not Boba Fett!\n",
                             ent->targetname);
    return;
  }
  ...
  if (add) {
    if (ent->genericBolt1 != -1) {
      G_PlayEffect(G_EffectIndex("boba/jetSP"), ent->playerModel,
                   ent->genericBolt1, ent->s.number, ent->currentOrigin, qtrue,
                   qtrue);
    }
    if (ent->genericBolt2 != -1) {
      G_PlayEffect(G_EffectIndex("boba/jetSP"), ent->playerModel,
                   ent->genericBolt2, ent->s.number, ent->currentOrigin, qtrue,
                   qtrue);
    }
    // take-off sound
    ...
    if (ent->genericBolt1 != -1) {
      G_StopEffect("boba/jetSP", ent->playerModel, ent->genericBolt1,
                   ent->s.number);
    }
    if (ent->genericBolt2 != -1) {
      G_StopEffect("boba/jetSP", ent->playerModel, ent->genericBolt2,
                   ent->s.number);
    }
    // stop jet loop sound
    ent->s.loopSound = 0;
...
static void Q3_SetInactive(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetInactive: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_SetFuncUsableVisible(int entID, qboolean visible) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetFuncUsableVisible: invalid entID %d\n",
                             entID);
    return;
  }
...
static void Q3_SetLockedEnemy(int entID, qboolean locked) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetLockedEnemy: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetLockedEnemy: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetCinematicSkipScript(char *scriptname) {
  ...
  if (Q_stricmp("none", scriptname) == 0 ||
      Q_stricmp("NULL", scriptname) == 0) {
...
static void Q3_SetNoMindTrick(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetNoMindTrick: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetNoMindTrick: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
  ...
  if (add) {
    ...
    if (ent->ghoul2.size() && ent->headBolt != -1) {
      G_StopEffect("force/confusion", ent->playerModel, ent->headBolt,
                   ent->s.number);
    }
  } else {
    ent->NPC->scriptFlags &= ~SCF_NO_MIND_TRICK;
...
static void Q3_SetCrouched(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetCrouched: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetCrouched: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetWalking(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetWalking: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetWalking: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetRunning(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetRunning: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetRunning: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetForcedMarch(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetForcedMarch: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetForcedMarch: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetChaseEnemies(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetChaseEnemies: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetChaseEnemies: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetLookForEnemies(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetLookForEnemies: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetLookForEnemies: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetFaceMoveDir(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetFaceMoveDir: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetFaceMoveDir: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetAltFire(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetAltFire: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetAltFire: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetDontFlee(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetDontFlee: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetDontFlee: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetNoResponse(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetNoResponse: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetNoResponse: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetCombatTalk(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetCombatTalk: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetCombatTalk: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetAlertTalk(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetAlertTalk: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetAlertTalk: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetUseCpNearest(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetUseCpNearest: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetUseCpNearest: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetNoForce(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetNoForce: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetNoForce: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetNoAcrobatics(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetNoAcrobatics: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetNoAcrobatics: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetUseSubtitles(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetUseSubtitles: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetUseSubtitles: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetNoFallToDeath(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetNoFallToDeath: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetNoFallToDeath: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetDismemberable(int entID, qboolean dismemberable) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetDismemberable: invalid entID %d\n", entID);
    return;
  }

  if (!ent->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetDismemberable: '%s' is not an client!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetMoreLight(int entID, qboolean add) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetMoreLight: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetMoreLight: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetInvincible(int entID, qboolean invincible) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetInvincible: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_SetForceInvincible(int entID, qboolean forceInv) {
  ...
  if (!self || !self->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetForceInvincible: entID %d not a client\n",
                             entID);
    return;
  }
...
static void Q3_SetNoAvoid(int entID, qboolean noAvoid) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetNoAvoid: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetNoAvoid: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetLean(int entID, int lean) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetLean: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetLean: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetForwardMove(int entID, int fmoveVal) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetForwardMove: invalid entID %d\n", entID);
    return;
  }

  if (!ent->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetForwardMove: '%s' is not an NPC/player!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetRightMove(int entID, int rmoveVal) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetRightMove: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_SetLockAngle(int entID, const char *lockAngle) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetLockAngle: invalid entID %d\n", entID);
    return;
  }
  ...
  if (Q_stricmp("off", lockAngle) == 0) { // free it
    ent->client->renderInfo.renderFlags &= ~RF_LOCKEDANGLE;
  } else {
    ent->client->renderInfo.renderFlags |= RF_LOCKEDANGLE;

    if (Q_stricmp("auto", lockAngle) == 0) { // use current yaw
      if (ent->NPC) // I need this to work on NPCs, so their locked value
      {
        ent->NPC->lockedDesiredYaw =
            NPC->client->ps.viewangles[YAW]; // could also set s.angles[1] and
                                             // desiredYaw to this value...
      } else {
      ...
      {
        ent->NPC->lockedDesiredYaw =
            atof((char *)lockAngle); // could also set s.angles[1] and
                                     // desiredYaw to this value...
      } else {
...
static void Q3_CameraGroup(int entID, char *camG) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_CameraGroup: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_CameraGroupTag(char *camGTag) {
  Q_strncpyz(client_camera.cameraGroupTag, camGTag,
             sizeof(client_camera.cameraGroupTag));
}
...
static void Q3_AddRHandModel(int entID, char *addModel) {
  ...
  ent->cinematicModel = gi.G2API_InitGhoul2Model(
      ent->ghoul2, addModel, G_ModelIndex(addModel), 0, 0, 0, 0);
  if (ent->cinematicModel != -1) {
    // attach it to the hand
    gi.G2API_AttachG2Model(&ent->ghoul2[ent->cinematicModel],
...
static void Q3_AddLHandModel(int entID, char *addModel) {
  ...
  if (ent->cinematicModel != -1) {
    // attach it to the hand
    gi.G2API_AttachG2Model(&ent->ghoul2[ent->cinematicModel],
                           &ent->ghoul2[ent->playerModel], ent->handLBolt,
                           ent->playerModel);
  }
}
...
static void Q3_LookTarget(int entID, char *targetName) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_LookTarget: invalid entID %d\n", entID);
    return;
  }

  if (!ent->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_LookTarget: '%s' is not an NPC/player!\n",
                             ent->targetname);
    return;
  }
  ...
  if (!targ) {
    ...
    if (!targ) {
      ...
      if (!targ) {
        Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                                 "Q3_LookTarget: Can't find ent %s\n",
                                 targetName);
        return;
      }
    }
...
static void Q3_Face(int entID, int expression, float holdtime) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_Face: invalid entID %d\n", entID);
    return;
  }

  if (!ent->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_Face: '%s' is not an NPC/player!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetPlayerUsable(int entID, qboolean usable) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetPlayerUsable: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_SetDisableShaderAnims(int entID, int disabled) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetDisableShaderAnims: invalid entID %d\n",
                             entID);
    return;
  }
...
static void Q3_SetShaderAnim(int entID, int disabled) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetShaderAnim: invalid entID %d\n", entID);
    return;
  }
...
void Q3_SetForcePower(int entID, int forcePower, qboolean powerOn) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetForcePower: invalid entID %d\n", entID);
    return;
  }
  if (!ent->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetForcePower: ent # %d not a client!\n",
                             entID);
    return;
  }
  if (powerOn) {
...
static void Q3_SetStartFrame(int entID, int startFrame) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetStartFrame: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_SetEndFrame(int entID, int endFrame) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetEndFrame: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_SetAnimFrame(int entID, int animFrame) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetAnimFrame: invalid entID %d\n", entID);
    return;
  }
  ...
  if (animFrame >= ent->endFrame) {
    ...
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetAnimFrame: value must be valid number "
...
void InflateOwner(gentity_t *self) {
  ...
  gi.trace(&trace, self->currentOrigin, self->mins, self->maxs,
           self->currentOrigin, self->owner->s.number,
           self->owner->clipmask & ~(CONTENTS_SOLID | CONTENTS_MONSTERCLIP),
           (EG2_Collision)0, 0);
...
static void Q3_SetLoopAnim(int entID, qboolean loopAnim) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetLoopAnim: invalid entID %d\n", entID);
    return;
  }
...
static void Q3_SetShields(int entID, qboolean shields) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetShields: invalid entID %d\n", entID);
    return;
  }

  if (!ent->NPC) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetShields: '%s' is not an NPC!\n",
                             ent->targetname);
    return;
  }
...
static void Q3_SetSaberActive(int entID, qboolean active) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetSaberActive: invalid entID %d\n", entID);
    return;
  }

  if (!ent->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_SetSaberActive: '%s' is not an player/NPC!\n",
                             ent->targetname);
    return;
  }
  ...
  if (ent->client->ps.weapon != WP_SABER) {
    if ((ent->client->ps.stats[STAT_WEAPONS] &
         (1 << WP_SABER))) { // change to it right now
      if (ent->NPC) {
        ChangeWeapon(ent, WP_SABER);
      } else {
        gitem_t *item = FindItemForWeapon(WP_SABER);
        RegisterItem(item); // make sure the weapon is cached in case this runs
                            // at startup
      ...
      Quake3Game()->DebugPrint(
          IGameInterface::WL_ERROR,
          "Q3_SetSaberActive: '%s' is not using a saber!\n", ent->targetname);
      return;
    }
  }
...
============
        ...
        Description: Make a specific blade of a specific Saber active or
inactive. Created: 10/02/02 by Aurelio Reis, Modified: 10/02/02 by Aurelio Reis
        [in]	int entID		The ID of the Entity to modify.
        [in]	int iSaber		Which Saber to modify.
        [in]	int iBlade		Which blade to modify (0 - (NUM_BLADES -
1)).
        [in]	bool bActive	Whether to make it active (default true) or
inactive (false). [return]	void
============
*/
static void Q3_SetSaberBladeActive(int entID, int iSaber, int iBlade,
                                   qboolean bActive = qtrue) {
  gentity_t *ent = &g_entities[entID];

  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetSaberBladeActive: invalid entID %d\n",
                             entID);
    return;
  }
  ...
  if (!ent->client) {
    Quake3Game()->DebugPrint(
        IGameInterface::WL_ERROR,
        "Q3_SetSaberBladeActive: '%s' is not an player/NPC!\n",
        ent->targetname);
    return;
  }
...
static void Q3_SetNoKnockback(int entID, qboolean noKnockback) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetNoKnockback: invalid entID %d\n", entID);
    return;
  }
  ...
  for (int i = 0; i < ENTITYNUM_WORLD; i++) {
    ...
    if (ent) {
      if (!ent->client &&
        ...
        // check for a client, otherwise we could remove someone holding this
        // weapon
        G_FreeEntity(ent);
      } else if (ent->s.weapon == WP_TURRET && ent->activator &&
                 ent->activator->s.number == 0 &&
        ...
        // they blow up when they run out of ammo, so this may as well just do
        // the same.
...
static void Q3_SetPlayerLocked(int entID, qboolean locked) {
  ...
  if (ent && ent->client) { // stop him too
    VectorClear(ent->client->ps.velocity);
  }
}
...
static void Q3_SetNoImpactDamage(int entID, qboolean noImp) {
  ...
  if (!ent) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_SetNoImpactDamage: invalid entID %d\n", entID);
    return;
  }
...
/*void SetVar( int taskID, int entID, const char *type_name, const char *data )
{
        ...
        Quake3Game()->DebugPrint( IGameInterface::WL_ERROR, "%s variable or
field not found!\n", type_name );
}*/
...
// NOTE! RemoveOwner() is a function used within the entity (a pointer to the
// function is contained). This leads to a "funny" predicament: why is
// RemoveOwner() here??? NOTE NOTE! This is also true of Q3_Remove, which should
...
void RemoveOwner(gentity_t *self) {
  if (self->owner &&
      self->owner->inuse) { // I have an owner and they heavn't been freed yet
...
void Q3_DismemberLimb(int entID, char *hitLocName) {
  ...
  if (!self) {
    Quake3Game()->DebugPrint(IGameInterface::WL_WARNING,
                             "Q3_DismemberLimb: invalid entID %d\n", entID);
    return;
  }

  if (!self->client) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_DismemberLimb: '%s' is not a player/NPC!\n",
                             self->targetname);
    return;
  }
  ...
  if (!self->ghoul2.size()) {
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "Q3_DismemberLimb: '%s' is not a ghoul model!\n",
                             self->targetname);
    return;
  }
  ...
  if (hitLoc <= HL_NONE || hitLoc >= HL_MAX) {
    Quake3Game()->DebugPrint(
        IGameInterface::WL_ERROR,
        "Q3_DismemberLimb: '%s' is not a valid hit location!\n", hitLocName);
    return;
  }
...
void CQuake3GameInterface::SetVar(int taskID, int entID, const char *type_name,
                                  const char *data) {
...
int CQuake3GameInterface::GetStringVariable(const char *name,
                                            const char **value) {
  // Check the strings
  varString_m::iterator vsi = m_varStrings.find(name);
...
int CQuake3GameInterface::SetStringVariable(const char *name,
                                            const char *value) {
  // Check the strings
  varString_m::iterator vsi = m_varStrings.find(name);
...
int CQuake3GameInterface::SetVectorVariable(const char *name,
                                            const char *value) {
  // Check the strings
  varString_m::iterator vvi = m_varVectors.find(name);
...
void CQuake3GameInterface::VariableSaveFloats(varFloat_m &fmap) {
  ...
  saved_game.write_chunk<int32_t>(INT_ID('F', 'V', 'A', 'R'), numFloats);
  ...
  STL_ITERATE(vfi, fmap) {
    ...
    saved_game.write_chunk<int32_t>(INT_ID('F', 'I', 'D', 'L'), idSize);

    saved_game.write_chunk(INT_ID('F', 'I', 'D', 'S'), ((*vfi).first).c_str(),
                           idSize);

    // Save out the float value
    saved_game.write_chunk<float>(INT_ID('F', 'V', 'A', 'L'), (*vfi).second);
  }
}
...
void CQuake3GameInterface::VariableSaveStrings(varString_m &smap) {
  ...
  saved_game.write_chunk<int32_t>(INT_ID('S', 'V', 'A', 'R'), numStrings);
  ...
  STL_ITERATE(vsi, smap) {
    ...
    // Save out the real data
    saved_game.write_chunk<int32_t>(INT_ID('S', 'I', 'D', 'L'), idSize);

    saved_game.write_chunk(INT_ID('S', 'I', 'D', 'S'), ((*vsi).first).c_str(),
                           idSize);

    // Save out the string value
    idSize = strlen(((*vsi).second).c_str());

    saved_game.write_chunk<int32_t>(INT_ID('S', 'V', 'S', 'Z'), idSize);

    saved_game.write_chunk(INT_ID('S', 'V', 'A', 'L'), ((*vsi).second).c_str(),
                           idSize);
  }
}
...
void CQuake3GameInterface::VariableLoadFloats(varFloat_m &fmap) {
  ...
  saved_game.read_chunk<int32_t>(INT_ID('F', 'V', 'A', 'R'), numFloats);

  for (int i = 0; i < numFloats; i++) {
    int idSize = 0;

    saved_game.read_chunk<int32_t>(INT_ID('F', 'I', 'D', 'L'), idSize);
    ...
    saved_game.read_chunk(INT_ID('F', 'I', 'D', 'S'), tempBuffer, idSize);

    tempBuffer[idSize] = 0;

    float val = 0.0F;

    saved_game.read_chunk<float>(INT_ID('F', 'V', 'A', 'L'), val);
...
void CQuake3GameInterface::VariableLoadStrings(int type, varString_m &fmap) {
  ...
  saved_game.read_chunk<int32_t>(INT_ID('S', 'V', 'A', 'R'), numFloats);

  for (int i = 0; i < numFloats; i++) {
    int idSize = 0;

    saved_game.read_chunk<int32_t>(INT_ID('S', 'I', 'D', 'L'), idSize);
    ...
    saved_game.read_chunk(INT_ID('S', 'I', 'D', 'S'), tempBuffer, idSize);

    tempBuffer[idSize] = 0;

    saved_game.read_chunk<int32_t>(INT_ID('S', 'V', 'S', 'Z'), idSize);
    ...
    saved_game.read_chunk(INT_ID('S', 'V', 'A', 'L'), tempBuffer2, idSize);
...
CQuake3GameInterface::~CQuake3GameInterface() {
  ...
  for (iterScript = m_ScriptList.begin(); iterScript != m_ScriptList.end();
       ++iterScript) {
...
bool CQuake3GameInterface::InitEntity(gentity_t *pEntity) {
  ...
  // Get an Icarus ID.
  pEntity->m_iIcarusID =
      IIcarusInterface::GetIcarus()->GetIcarusID(pEntity->s.number);

  // Initialize all taskIDs to -1
  memset(&pEntity->taskID, -1, sizeof(pEntity->taskID));

  // Add this entity to a map of valid associated entity's for quick retrieval
  // later.
...
void CQuake3GameInterface::FreeEntity(gentity_t *pEntity) {
  ...
  // Remove the Entity from the Entity map so that when their g_entity index is
  // reused, ICARUS doesn't try to affect the new (incorrect) pEntity.
  if
    VALIDSTRING(pEntity->script_targetname) {
      char temp[1024];
...
bool CQuake3GameInterface::ValidEntity(gentity_t *pEntity) {
  ...
  for (i = 0; i < NUM_BSETS; i++) {
    if
      VALIDSTRING(pEntity->behaviorSet[i]) {
...
int CQuake3GameInterface::MakeValidScriptName(char **strScriptName) {
  if (!Q_stricmp(*strScriptName, "NULL") ||
      !Q_stricmp(*strScriptName, "default"))
    return 0;

  // ensure "scripts" (Q3_SCRIPT_DIR), which will be missing if this was called
  // recursively... MAX_FILENAME_LENGTH should really be MAX_QPATH (and 64 bytes
  // instead of 1024), but this fits the rest of the code
  ...
  if (!Q_stricmpn(*strScriptName, Q3_SCRIPT_DIR, strlen(Q3_SCRIPT_DIR))) {
    ...
    Q_strncpyz(sFilename, va("%s/%s", Q3_SCRIPT_DIR, *strScriptName),
               sizeof(sFilename));
  }
...
int CQuake3GameInterface::RegisterScript(const char *strFileName, void **ppBuf,
                                         int &iLength) {
  // If the file name is invalid, leave.
  if (!strFileName || strFileName[0] == '\0' ||
      !Q_stricmp(strFileName, "NULL") || !Q_stricmp(strFileName, "default"))
  ...
  if (!Q_stricmpn(strFileName, Q3_SCRIPT_DIR, strlen(Q3_SCRIPT_DIR))) {
    ...
    Q_strncpyz(sFilename, va("%s/%s", Q3_SCRIPT_DIR, strFileName),
               sizeof(sFilename));
  }
  ...
  /*#if 0//#ifndef FINAL_BUILD
          // small update here, if called during interrogate, don't let
  gi.FS_ReadFile() complain because it can't
          //	find stuff like BS_RUN_AND_SHOOT as scriptname...   During
  FINALBUILD the message won't appear anyway, hence
          //	the ifndef, this just cuts down on internal error reports while
  testing release mode...
          ...
          if (bCalledDuringInterrogate)
          {
                  ...
                  if ( file == NULL )
                  {
                          qbIgnoreFileRead = qtrue;	// warn disk code
  further down not to try FS_ReadFile()
                  }
                  else
                  {
...
int CQuake3GameInterface::PrecacheEntity(gentity_t *pEntity) {
  ...
  for (i = 0; i < NUM_BSETS; i++) {
    ...
    if (GetIDForString(BSTable, pEntity->behaviorSet[i]) ==
        -1) { // not a behavior set
      char *pBuf = NULL;
      int iLength = 0;

      // Try to register this script.
      if (RegisterScript(pEntity->behaviorSet[i], (void **)&pBuf, iLength) !=
...
// Run the script.
void CQuake3GameInterface::RunScript(const gentity_t *pEntity,
  ...
  case SCRIPT_COULDNOTREGISTER:
    DebugPrint(
        WL_WARNING,
        "RunScript: Script was not found and could not be loaded!!! %s\n",
        strScriptName);
    return;
  ...
  case SCRIPT_ALREADYREGISTERED:
    ...
    if (IIcarusInterface::GetIcarus()->Run(pEntity->m_iIcarusID, pBuf,
      ...
      DebugPrint(WL_VERBOSE, "%d Script %s executed by %s %s\n", level.time,
                 (char *)strScriptName, pEntity->classname,
                 pEntity->targetname);
    return;
  }
}
...
int CQuake3GameInterface::GetFlavor() { return 0; }
...
void CQuake3GameInterface::DebugPrint(e_DebugPrintLevel level,
                                      const char *format, ...) {
...
unsigned int CQuake3GameInterface::GetTime(void) { return level.time; }
...
int CQuake3GameInterface::PlayIcarusSound(int taskID, int entID,
                                          const char *name,
                                          const char *channel) {
  gentity_t *ent = &g_entities[entID];
  ...
  if (!in_camera ||
      (!g_skippingcin ||
       !g_skippingcin->integer)) // paranoia towards project end <g>
  {
    // Text on
    // certain NPC's we always want to use subtitles regardless of subtitle
    // setting
    if (g_subtitles->integer == 1 ||
        (ent->NPC &&
         (ent->NPC->scriptFlags & SCF_USE_SUBTITLES))) // Show all text
    {
      ...
      } else // if (precacheWav[i].speaker==SP_NONE)	//  lower screen text
      {
        ...
        // the numbers in here were either the original ones Bob entered (350),
        // or one arrived at from checking the distance Chell stands at in
        // stasis2 by the computer core that was submitted as a bug report...
        //
        if (bBroadcast ||
            (DistanceSquared(ent->currentOrigin, ent2->currentOrigin) <
             ((voice_chan == CHAN_VOICE_ATTEN) ? (350 * 350)
                                               : (1200 * 1200)))) {
          gi.SendServerCommand(0, "ct \"%s\" %i", finalName, soundHandle);
        }
      }
    ...
    // Cinematic only
    else if (g_subtitles->integer ==
             2) // Show only talking head text and CINEMATIC
    {
  ...
  if (type_voice) {
    if (g_timescale->value > 1.0f) { // Skip the damn sound!
      return qtrue;
    } else {
...
// Lerps the origin and angles of an entity to the destination values.
void CQuake3GameInterface::Lerp2Pos(int taskID, int entID, vec3_t origin,
                                    vec3_t angles, float duration) {
  gentity_t *ent = &g_entities[entID];
  vec3_t ang;
  int i;
...
//	 void	CQuake3GameInterface::Lerp2Origin( int taskID, int entID, vec3_t
//origin, float duration ) {}

// Lerps the angles to the destination value.
void CQuake3GameInterface::Lerp2Angles(int taskID, int entID, vec3_t angles,
                                       float duration) {
  gentity_t *ent = &g_entities[entID];
  vec3_t ang;
  int i;
...
// Gets the value of a tag by the give name.
int CQuake3GameInterface::GetTag(int entID, const char *name, int lookup,
                                 vec3_t info) {
  gentity_t *ent = &g_entities[entID];
...
//	 void	CQuake3GameInterface::Lerp2Start( int taskID, int entID, float
//duration ) {} 	 void	CQuake3GameInterface::Lerp2End( int taskID, int entID,
//float duration ) {}

void CQuake3GameInterface::Set(int taskID, int entID, const char *type_name,
                               const char *data) {
  gentity_t *ent = &g_entities[entID];
  float float_data;
  int int_data, toSet;
  ...
  if (!Q_stricmpn(type_name, "cvar_", 5) && strlen(type_name) > 5) {
  ...
  case SET_ORIGIN:
    ...
    if (ent->client) { // clear jump start positions so we don't take damage
                       // when we land...
      ent->client->ps.jumpZStart = ent->client->ps.forceJumpZStart =
          vector_data[2];
    }
    gi.linkentity(ent);
    break;
  ...
  // NOTE: Preconditions for entering a vehicle still exist. This is not assured
  // to work. -Areis
  case SET_VEHICLE:
    Use(entID, (char *)data);
  ...
  case SET_SABER1_COLOR2:
    WP_SaberSetColor(&g_entities[entID], 0, toSet - SET_SABER1_COLOR1,
                     (char *)data);
    break;
  case SET_SABER2_COLOR1:
  case SET_SABER2_COLOR2:
    WP_SaberSetColor(&g_entities[entID], 1, toSet - SET_SABER2_COLOR1,
                     (char *)data);
    break;
  ...
  case SET_CLOAK: // Created: 01/08/03 by AReis.
    ...
    break;
  case SET_FORCE_HEAL:
    Q3_SetForcePower(entID, FP_HEAL,
                     (qboolean)(Q_stricmp("true", (char *)data) == 0));
    break;
  case SET_FORCE_SPEED:
    Q3_SetForcePower(entID, FP_SPEED,
                     (qboolean)(Q_stricmp("true", (char *)data) == 0));
    break;
  case SET_FORCE_PUSH:
    Q3_SetForcePower(entID, FP_PUSH,
                     (qboolean)(Q_stricmp("true", (char *)data) == 0));
    break;
  case SET_FORCE_PUSH_FAKE:
    ...
    break;
  case SET_FORCE_PULL:
    Q3_SetForcePower(entID, FP_PULL,
                     (qboolean)(Q_stricmp("true", (char *)data) == 0));
    break;
  case SET_FORCE_MIND_TRICK:
    Q3_SetForcePower(entID, FP_TELEPATHY,
                     (qboolean)(Q_stricmp("true", (char *)data) == 0));
    break;
  case SET_FORCE_GRIP:
    Q3_SetForcePower(entID, FP_GRIP,
                     (qboolean)(Q_stricmp("true", (char *)data) == 0));
    break;
  case SET_FORCE_LIGHTNING:
    Q3_SetForcePower(entID, FP_LIGHTNING,
                     (qboolean)(Q_stricmp("true", (char *)data) == 0));
    break;
  case SET_FORCE_SABERTHROW:
    Q3_SetForcePower(entID, FP_SABERTHROW,
                     (qboolean)(Q_stricmp("true", (char *)data) == 0));
    break;
  case SET_FORCE_RAGE:
    Q3_SetForcePower(entID, FP_RAGE,
                     (qboolean)(Q_stricmp("true", (char *)data) == 0));
    break;
  case SET_FORCE_PROTECT:
    Q3_SetForcePower(entID, FP_PROTECT,
                     (qboolean)(Q_stricmp("true", (char *)data) == 0));
    break;
  case SET_FORCE_ABSORB:
    Q3_SetForcePower(entID, FP_ABSORB,
                     (qboolean)(Q_stricmp("true", (char *)data) == 0));
    break;
  case SET_FORCE_DRAIN:
    Q3_SetForcePower(entID, FP_DRAIN,
                     (qboolean)(Q_stricmp("true", (char *)data) == 0));
    break;
  ...
  case SET_WINTER_GEAR: // Created: 03/26/03 by AReis.
  {
    ...
    if (entID == 0 ||
        (ent->NPC_type && Q_stricmp(ent->NPC_type, "player") == 0)) {
      char strSkin[MAX_QPATH];
      // Set the Winter Gear Skin if true, otherwise set back to normal
      // configuration.
      if (Q_stricmp("true", ((char *)data)) == 0) {
        Com_sprintf(strSkin, sizeof(strSkin), "models/players/%s/|%s|%s|%s",
        ...
        Com_sprintf(strSkin, sizeof(strSkin),
                    "models/players/%s/model_default.skin",
                    g_char_model->string);
      } else {
        Com_sprintf(strSkin, sizeof(strSkin), "models/players/%s/|%s|%s|%s",
      ...
      if (iSkinID) {
        gi.G2API_SetSkin(&ent->ghoul2[ent->playerModel], G_SkinIndex(strSkin),
                         iSkinID);
      }
    }
    break;
  ...
  case SET_SKIN:
    // If this is a (fake) Player NPC or this IS the Player...
    { // just blindly sets whatever skin you set!  include full path after
      // "base/"... eg: "models/players/tavion_new/model_possessed.skin"
      gentity_t *ent = &g_entities[entID];
      if (ent && ent->inuse && ent->ghoul2.size()) {
        int iSkinID = gi.RE_RegisterSkin((char *)data);
        if (iSkinID) {
          gi.G2API_SetSkin(&ent->ghoul2[ent->playerModel],
                           G_SkinIndex((char *)data), iSkinID);
        }
      }
    }
    break;
...
void CQuake3GameInterface::PrisonerObjCheck(const char *name,
                                            const char *data) {
  float float_data = 0.0f;
  int holdData;
...
void CQuake3GameInterface::Use(int entID, const char *name) {
  ...
  /*
  if ( ent->client )
  {//put the button on him, too, since some other things (like getting out of a
  vehicle?) check for the use button directly... ent->client->usercmd.buttons |=
  BUTTON_USE;
  }
  */
...
void CQuake3GameInterface::Kill(int entID, const char *name) {
  ...
  if ( victim == ent )
  {
          DebugPrint( WL_ERROR, "Kill: entity %s trying to kill self - not
  allowed!\n", name); return;
  }
  */
  if (victim ==
      ent) { // don't ICARUS_FreeEnt me, I'm in the middle of a script!  (FIXME:
             // shouldn't ICARUS handle this internally?)
...
void CQuake3GameInterface::Play(int taskID, int entID, const char *type,
                                const char *name) {
  gentity_t *ent = &g_entities[entID];
...
// Camera functions
void CQuake3GameInterface::CameraPan(vec3_t angles, vec3_t dir,
                                     float duration) {
...
void CQuake3GameInterface::CameraFollow(const char *name, float speed,
                                        float initLerp) {
  CGCam_Follow(name, speed, initLerp);
}

void CQuake3GameInterface::CameraTrack(const char *name, float speed,
                                       float initLerp) {
...
void CQuake3GameInterface::CameraFade(float sr, float sg, float sb, float sa,
...
void CQuake3GameInterface::CameraEnable(void) { CGCam_Enable(); }

void CQuake3GameInterface::CameraDisable(void) { CGCam_Disable(); }
...
int CQuake3GameInterface::GetFloat(int entID, const char *name, float *value) {
  ...
  int toGet = GetIDForString(
      setTable, name); // FIXME: May want to make a "getTable" as well
  ...
  case SET_PARM16:
    if (ent->parms == NULL) {
      ...
      return false; // would prefer qfalse, but I'm fitting in with what's here
                    // <sigh>
    }
  ...
  case SET_FACEFROWN: //## %f="0.0" # Set face to Frown expression for number of
                      //seconds
  case SET_FACESMILE: //## %f="0.0" # Set face to Smile expression for number of
                      //seconds
  case SET_FACEGLAD:  //## %f="0.0" # Set face to Glad expression for number of
                      //seconds
  case SET_FACEHAPPY: //## %f="0.0" # Set face to Happy expression for number of
                      //seconds
  case SET_FACESHOCKED: //## %f="0.0" # Set face to Shocked expression for
                        //number of seconds
  ...
  case SET_ANIM_HOLDTIME_LOWER: //## %d="0" # Hold lower anim for number of
    ...
    if (ent->client == NULL) {
      DebugPrint(WL_WARNING,
                 "GetFloat: SET_ANIM_HOLDTIME_LOWER, %s not a client\n",
                 ent->targetname);
      return false;
    }
  ...
  case SET_ANIM_HOLDTIME_UPPER: //## %d="0" # Hold upper anim for number of
    ...
    if (ent->client == NULL) {
      DebugPrint(WL_WARNING,
                 "GetFloat: SET_ANIM_HOLDTIME_UPPER, %s not a client\n",
                 ent->targetname);
      return false;
    }
  ...
  case SET_ANIM_HOLDTIME_BOTH: //## %d="0" # Hold lower and upper anims for
    ...
    DebugPrint(WL_WARNING,
               "GetFloat: SET_ANIM_HOLDTIME_BOTH not implemented\n");
    return false;
    break;
  ...
  // kef 4/16/03 -- just trying to put together some scripted meta-AI for swoop
  // riders
...
int CQuake3GameInterface::GetVector(int entID, const char *name, vec3_t value) {
  ...
  case SET_PARM15:
  case SET_PARM16:
    sscanf(ent->parms->parm[toGet - SET_PARM1], "%f %f %f", &value[0],
           &value[1], &value[2]);
    break;
...
int CQuake3GameInterface::Evaluate(int p1Type, const char *p1, int p2Type,
                                   const char *p2, int operatorType) {
  float f1 = 0, f2 = 0;
  vec3_t v1, v2;
  char *c1 = 0, *c2 = 0;
  int i1 = 0, i2 = 0;
  ...
  case TK_GREATER_THAN:
    ...
    case TK_VECTOR:
      DebugPrint(WL_ERROR, "Evaluate vector comparisons of type GREATER THAN "
                           "cannot be performed!");
      return false;
      break;
    ...
    case TK_STRING:
    case TK_IDENTIFIER:
      DebugPrint(WL_ERROR, "Evaluate string comparisons of type GREATER THAN "
                           "cannot be performed!");
      return false;
      break;
  ...
  case TK_LESS_THAN:
    ...
    case TK_VECTOR:
      DebugPrint(
          WL_ERROR,
          "Evaluate vector comparisons of type LESS THAN cannot be performed!");
      return false;
      break;
    ...
    case TK_IDENTIFIER:
      DebugPrint(
          WL_ERROR,
          "Evaluate string comparisons of type LESS THAN cannot be performed!");
      return false;
      break;
  ...
  case TK_GE:
    ...
    case TK_VECTOR:
      DebugPrint(WL_ERROR, "Evaluate vector comparisons of type GREATER THAN "
                           "OR EQUAL TO cannot be performed!");
      return false;
      break;
  ...
  case TK_LE:
    ...
    case TK_VECTOR:
      DebugPrint(WL_ERROR, "Evaluate vector comparisons of type LESS THAN OR "
                           "EQUAL TO cannot be performed!");
      return false;
      break;

    case TK_STRING:
    case TK_IDENTIFIER:
      DebugPrint(WL_ERROR, "Evaluate string comparisons of type LESS THAN OR "
                           "EQUAL TO cannot be performed!");
      return false;
      break;
...
void CQuake3GameInterface::DeclareVariable(int type, const char *name) {
  ...
  default:
    DebugPrint(WL_ERROR,
               "unknown INT_ID('t','y','p','e') for declare() function!\n");
    return;
    break;
  }
...
void CQuake3GameInterface::Free(void *data) { gi.Free(data); }
...
void CQuake3GameInterface::PrecacheRoff(const char *name) { G_LoadRoff(name); }
...
void CQuake3GameInterface::PrecacheScript(const char *name) {
  ...
  case SCRIPT_COULDNOTREGISTER:
    if (!Q_stricmp(newname, "NULL") ||
        !Q_stricmp(newname,
                   "default")) { // these are not real errors, suppress warning
      return;
    }
    Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                             "PrecacheScript: Failed to load %s!\n", newname);
...
void CQuake3GameInterface::PrecacheSound(const char *name) {
  ...
  Q_strncpyz(finalName, name, MAX_QPATH);
  Q_strlwr(finalName);
  if (com_buildScript->integer) { // get the male sound first
...
void CQuake3GameInterface::PrecacheFromSet(const char *setname,
                                           const char *filename) {
  // JW NOTENOTE: This will not catch special case get() inlines! (There's not
