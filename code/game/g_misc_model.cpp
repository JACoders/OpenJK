...
//
//------------------------------------------------------------
void SetMiscModelModels(char *modelNameString, gentity_t *ent,
...
//------------------------------------------------------------
void SetMiscModelDefaults(gentity_t *ent, useFunc_t use_func,
                          const char *material, int solid_mask, int animFlag,
                          qboolean take_damage,
                          qboolean damage_model = qfalse) {
...
void set_MiscAnim(gentity_t *ent) {
  ...
  if (ent->playerModel & 1) {
    ...
    // yes, its the same animation, so work out where we are in the leg anim,
    // and blend us
    gi.G2API_SetBoneAnim(
        &ent->ghoul2[0], "model_root", animations[anim].firstFrame,
        (animations[anim].numFrames - 1) + animations[anim].firstFrame,
        BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, animSpeed,
        (cg.time ? cg.time : level.time), -1, 350);
  } else {
    int anim = BOTH_PAIN3;
    float animSpeed = 50.0f / animations[anim].frameLerp;
    gi.G2API_SetBoneAnim(
...
void SP_misc_model_ghoul(gentity_t *ent) {
  ...
  qboolean bHasScale =
      G_SpawnVector("modelscale_vec", "1 1 1", ent->s.modelScale);
  if (!bHasScale) {
    float temp;
  ...
  if (temp_animFileIndex < 0) {
    Com_Printf(S_COLOR_RED "Failed to load animation file set "
                           "models/players/jedi/animation.cfg\n");
  }
  ...
  //"models/weapons2/blaster_r/g2blaster_w.glm" );
  //	gi.G2API_InitGhoul2Model(ent->ghoul2,
  //"models/weapons2/blaster_r/g2blaster_w.glm", ent->s.modelindex);
  ...
  //	gi.G2API_SetNewOrigin(&ent->ghoul2[0], gi.G2API_AddBolt(&ent->ghoul2[0],
  //"rhang_tag_bone")); 	ent->s.apos.trDelta[1] = 10; 	ent->s.apos.trType =
  //TR_LINEAR;
...
/*QUAKED misc_model_gun_rack (1 0 0.25) (-14 -14 -4) (14 14 30) BLASTER REPEATER
ROCKET model="models/map_objects/kejim/weaponsrack.md3"

NOTE: can mix and match these spawnflags to get multi-weapon racks.  If only one
type is checked the rack will be full of those weapons BLASTER - Puts one or
more blaster guns on the rack. REPEATER - Puts one or more repeater guns on the
rack. ROCKET - Puts one or more rocket launchers on the rack.
*/

void GunRackAddItem(gitem_t *gun, vec3_t org, vec3_t angs, float ffwd,
                    float fright, float fup) {
  vec3_t fwd, right;
  ...
  if (it_ent && gun) {
    // FIXME: scaling the ammo will probably need to be tweaked to a reasonable
    // amount...adjust as needed Set base ammo per type
    if (gun->giType == IT_WEAPON) {
      ...
      // must deliberately make it small, or else the objects will spawn inside
      // of each other.
    ...
    it_ent->classname =
        G_NewString(gun->classname); // copy it so it can be freed safely
    G_SpawnItem(it_ent, gun);

    // FinishSpawningItem handles everything, so clear the thinkFunc that was
    // set in G_SpawnItem
    FinishSpawningItem(it_ent);

    if (gun->giType == IT_AMMO) {
      if (gun->giTag ==
          AMMO_BLASTER) // I guess this just has to use different logic??
      {
        if (g_spskill->integer >= 2) {
          it_ent->count += 10; // give more on higher difficulty because there
                               // will be more/harder enemies?
        }
      } else {
    ...
    // by doing this, we can force the amount of ammo we desire onto the weapon
    // for when it gets picked-up
    ...
    for (int t = 0; t < 3; t++) {
      if (rotate) {
        if (t == YAW) {
          it_ent->s.angles[t] = AngleNormalize180(it_ent->s.angles[t] + 180 +
                                                  Q_flrand(-1.0f, 1.0f) * 14);
        } else {
          it_ent->s.angles[t] = AngleNormalize180(it_ent->s.angles[t] +
                                                  Q_flrand(-1.0f, 1.0f) * 4);
        }
      } else {
        if (t == YAW) {
          it_ent->s.angles[t] = AngleNormalize180(it_ent->s.angles[t] + 90 +
                                                  Q_flrand(-1.0f, 1.0f) * 4);
        }
      }
    }
...
void SP_misc_model_gun_rack(gentity_t *ent) {
  ...
  if ((ent->spawnflags & RACK_BLASTER) ||
      !(ent->spawnflags & (RACK_BLASTER | RACK_REPEATER | RACK_ROCKET))) {
  ...
  if (ct) //..should always have at least one item on their, but just being safe
  {
    for (; ct < 3; ct++) {
      ofz[ct] = ofz[0];
      itemList[ct] =
          itemList[0]; // first weapon ALWAYS propagates to fill up the shelf
    }
  }

  // now actually add the items to the shelf...validate that we have a list to
  // add
  if (ct) {
    for (int i = 0; i < ct; i++) {
      GunRackAddItem(itemList[i], ent->s.origin, ent->s.angles,
...
/*QUAKED misc_model_ammo_rack (1 0 0.25) (-14 -14 -4) (14 14 30) BLASTER
METAL_BOLTS ROCKETS WEAPON HEALTH PWR_CELL NO_FILL
model="models/map_objects/kejim/weaponsrung.md3"

NOTE: can mix and match these spawnflags to get multi-ammo racks.  If only one
type is checked the rack will be full of that ammo.  Only three ammo packs max
can be displayed.


BLASTER - Adds one or more ammo packs that are compatible with Blasters and the
Bryar pistol. METAL_BOLTS - Adds one or more metal bolt ammo packs that are
compatible with the heavy repeater and the flechette gun ROCKETS - Puts one or
more rocket packs on a rack. WEAPON - adds a weapon matching a selected ammo
type to the rack. HEALTH - adds a health pack to the top shelf of the ammo rack
PWR_CELL - Adds one or more power cell packs that are compatible with the
Disuptor, bowcaster, and demp2 NO_FILL - Only puts selected ammo on the rack, it
never fills up all three slots if only one or two items were checked
*/
...
void SP_misc_model_ammo_rack(gentity_t *ent) {
  ...
  if ((ent->spawnflags & RACK_BLASTER) ||
      !(ent->spawnflags &
        (RACK_BLASTER | RACK_METAL_BOLTS | RACK_ROCKETS | RACK_PWR_CELL))) {
  ...
  ent->contents =
      CONTENTS_SHOTCLIP | CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP |
      CONTENTS_BOTCLIP; // CONTENTS_SOLID;//so use traces can go through them
...
void spawn_rack_goods(gentity_t *ent) {
  ...
  gitem_t *am_blaster = NULL, *am_metal_bolts = NULL, *am_rockets = NULL,
          *am_pwr_cell = NULL;
  gitem_t *health = NULL;
  int pos = 0, ct = 0;
  gitem_t
      *itemList[4]; // allocating 4, but we only use 3.  done so I don't have to
                    // validate that the array isn't full before I add another
  ...
  if (!(ent->spawnflags & RACK_NO_FILL) &&
      ct) // double negative..should always have at least one item on there, but
          // just being safe
  {
    for (; ct < 3; ct++) {
      itemList[ct] =
          itemList[0]; // first item ALWAYS propagates to fill up the shelf
    }
  }
  ...
  if (ct) {
    for (int i = 0; i < ct; i++) {
      GunRackAddItem(itemList[i], ent->s.origin, ent->s.angles,
                     Q_flrand(-1.0f, 1.0f) * 0.5f, (i - 1) * 8, 7.0f);
    }
  }
  ...
  if (ent->spawnflags & RACK_WEAPONS) {
    if (!(ent->spawnflags &
      ...
      // if weapon is checked...and so are one or more ammo types, then pick a
      // random weapon to display..always give weaker weapons first
      if (blaster) {
        it = blaster;
        v_off = 25.5f;
      } else if (metal_bolts) {
    ...
    if (it) {
      // since we may have to put up a health pack on the shelf, we should know
      // where we randomly put
      //	the gun so we don't put the pack on the same spot..so pick
      //either the left or right side
      pos = (Q_flrand(0.0f, 1.0f) > .5) ? -1 : 1;

      GunRackAddItem(it, ent->s.origin, ent->s.angles,
                     Q_flrand(-1.0f, 1.0f) * 2,
  ...
  if ((ent->spawnflags & RACK_HEALTH) && health) {
    ...
    GunRackAddItem(health, ent->s.origin, ent->s.angles,
                   Q_flrand(-1.0f, 1.0f) * 0.5f,
...
/*QUAKED misc_model_cargo_small (1 0 0.25) (-14 -14 -4) (14 14 30) MEDPACK
SHIELDS BACTA BATTERIES model="models/map_objects/kejim/cargo_small.md3"

  Cargo crate that can only be destroyed by heavy class weapons ( turrets,
emplaced guns, at-st )  Can spawn useful things when it breaks
...
*/
extern gentity_t *LaunchItem(gitem_t *item, const vec3_t origin,
                             const vec3_t velocity, char *target);

void misc_model_cargo_die(gentity_t *self, gentity_t *inflictor,
  ...
  // we already had spawn flags, but we don't care what they were...we just need
  // to set up the flags we want for misc_model_breakable_die
  ...
  // now that the model is broken, we can safely spawn these in it's place
  // without them being in solid
  temp[2] = org[2] + 16;

  // annoying, but spawn each thing in its own little quadrant so that they
  // don't end up on top of each other
...
void SP_misc_model_cargo_small(gentity_t *ent) {
  ...
  SetMiscModelDefaults(ent, useF_NULL, "11",
                       CONTENTS_SOLID | CONTENTS_OPAQUE | CONTENTS_BODY |
                           CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP,
                       0, qtrue, qfalse);
  ent->s.modelindex2 = G_ModelIndex(
      "/models/map_objects/kejim/cargo_small.md3"); // Precache model
