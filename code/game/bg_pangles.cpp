...
void BG_IK_MoveLimb(CGhoul2Info_v &ghoul2, int boltIndex, char *animBone,
                    char *firstBone, char *secondBone, int time,
                    entityState_t *ent, int animFileIndex, int basePose,
                    vec3_t desiredPos, qboolean *ikInProgress, vec3_t origin,
                    vec3_t angles, vec3_t scale, int blendTime,
                    qboolean forceHalt) {
  ...
  if (!*ikInProgress && !forceHalt) {
    ...
    // ikP.forceAnimOnBone = qfalse; //let it use existing anim if it's the same
    // as this one.
    ...
    if (gi.G2API_SetBoneIKState(ghoul2, time, "lower_lumbar", IKS_DYNAMIC,
                                &ikP)) {
      // restrict the elbow joint
      VectorSet(ikP.pcjMins, -90.0f, -20.0f, -20.0f);
      VectorSet(ikP.pcjMaxs, 30.0f, 20.0f, -20.0f);
      if (gi.G2API_SetBoneIKState(ghoul2, time, "upper_lumbar", IKS_DYNAMIC,
                                  &ikP)) {
        // restrict the elbow joint
        VectorSet(ikP.pcjMins, -90.0f, -20.0f, -20.0f);
        VectorSet(ikP.pcjMaxs, 30.0f, 20.0f, -20.0f);
        if (gi.G2API_SetBoneIKState(ghoul2, time, "thoracic", IKS_DYNAMIC,
                                    &ikP)) {
          // restrict the elbow joint
          VectorSet(ikP.pcjMins, -90.0f, -20.0f, -20.0f);
          VectorSet(ikP.pcjMaxs, 30.0f, 20.0f, -20.0f);
          if (gi.G2API_SetBoneIKState(ghoul2, time, secondBone, IKS_DYNAMIC,
                                      &ikP)) {
            // restrict the elbow joint
            VectorSet(ikP.pcjMins, -90.0f, -20.0f, -20.0f);
            VectorSet(ikP.pcjMaxs, 30.0f, 20.0f, -20.0f);

            if (gi.G2API_SetBoneIKState(ghoul2, time, firstBone, IKS_DYNAMIC,
                                        &ikP)) { // everything went alright.
              *ikInProgress = qtrue;
            }
          }
        }
  ...
  if (*ikInProgress && !forceHalt) { // actively update our ik state.
    ...
    VectorCopy(
        desiredPos,
        ikM.desiredOrigin); // we want the bone to move here.. if possible

    VectorCopy(angles, tAngles);
    tAngles[PITCH] = tAngles[ROLL] = 0;

    gi.G2API_GetBoltMatrix(ghoul2, 0, boltIndex, &holdPointMatrix, tAngles,
                           origin, time, 0, scale);
    ...
    // if we're far away we don't want to be too fast or we'll start twitching
    // all over.
    if (distToDest <
        2) { // however if we're this close we want very precise movement
    ...
    gi.G2API_SetBoneAngles(&ghoul2[0], "lower_lumbar", vec3_origin,
                           BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                           NEGATIVE_Z, NULL, 0, time);
    gi.G2API_SetBoneAngles(&ghoul2[0], "upper_lumbar", vec3_origin,
                           BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                           NEGATIVE_Z, NULL, 0, time);
    gi.G2API_SetBoneAngles(&ghoul2[0], "thoracic", vec3_origin,
                           BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                           NEGATIVE_Z, NULL, 0, time);
    gi.G2API_SetBoneAngles(&ghoul2[0], secondBone, vec3_origin,
                           BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                           NEGATIVE_Z, NULL, 0, time);
    gi.G2API_SetBoneAngles(&ghoul2[0], firstBone, vec3_origin,
                           BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                           NEGATIVE_Z, NULL, 0, time);

    // Get the anim/frames that the pelvis is on exactly, and match the left arm
    // back up with them again.
    gi.G2API_GetBoneAnim(&ghoul2[0], animBone, (const int)time, &cFrame,
                         &sFrame, &eFrame, &flags, &animSpeed, 0);
    gi.G2API_SetBoneAnim(&ghoul2[0], "lower_lumbar", sFrame, eFrame, flags,
                         animSpeed, time, sFrame, 300);
    gi.G2API_SetBoneAnim(&ghoul2[0], "upper_lumbar", sFrame, eFrame, flags,
                         animSpeed, time, sFrame, 300);
    gi.G2API_SetBoneAnim(&ghoul2[0], "thoracic", sFrame, eFrame, flags,
                         animSpeed, time, sFrame, 300);
    gi.G2API_SetBoneAnim(&ghoul2[0], secondBone, sFrame, eFrame, flags,
                         animSpeed, time, sFrame, 300);
    gi.G2API_SetBoneAnim(&ghoul2[0], firstBone, sFrame, eFrame, flags,
                         animSpeed, time, sFrame, 300);

    // And finally, get rid of all the ik state effector data by calling with
    // null bone name (similar to how we init it).
...
void PM_IKUpdate(gentity_t *ent) {
  ...
  if (ent->client->ps.heldByClient <=
      ENTITYNUM_WORLD) { // then put our arm in this client's hand
    gentity_t *holder = &g_entities[ent->client->ps.heldByClient];

    if (holder && holder->inuse && holder->client && holder->ghoul2.size()) {
      if (!ent->client->ps.heldByBolt) { // bolt wan't set
        ent->client->ps.heldByBolt =
            gi.G2API_AddBolt(&holder->ghoul2[0], defaultBoltName);
      }
    } else { // they're gone, stop holding me
    ...
    if (ent->client->ps.heldByBolt) {
      ...
      gi.G2API_GetBoltMatrix(holder->ghoul2, 0, ent->client->ps.heldByBolt,
                             &boltMatrix, tAngles, holder->client->ps.origin,
                             level.time, 0, holder->s.modelScale);
      ...
      if (grabbedByBolt) {
        // point the limb
        BG_IK_MoveLimb(
            ent->ghoul2, grabbedByBolt, animBone, firstBone, secondBone,
            ...
            ent->client->ps.viewangles, ent->s.modelScale, 500, qfalse);
        ...
        gi.G2API_GetBoltMatrix(ent->ghoul2, 0, grabbedByBolt, &boltMatrix,
                               tAngles, ent->client->ps.origin, level.time, 0,
                               ent->s.modelScale);
        ...
        if (VectorLength(grabDiff) > 128.0f) { // too far, release me
          ent->client->ps.heldByClient = holder->client->ps.heldClient =
              ENTITYNUM_NONE;
        } else if (1) { // pull me along
          trace_t trace;
          vec3_t destOrg;
          VectorAdd(ent->currentOrigin, grabDiff, destOrg);
          gi.trace(&trace, ent->currentOrigin, ent->mins, ent->maxs, destOrg,
                   ent->s.number, (ent->clipmask & ~holder->contents),
                   (EG2_Collision)0, 0);
  ...
  } else if (ent->client->ps.ikStatus) { // make sure we aren't IKing if we
                                         // don't have anyone to hold onto us.
    if (ent && ent->inuse && ent->client && ent->ghoul2.size()) {
      if (!ent->client->ps.heldByBolt) {
        ent->client->ps.heldByBolt =
            gi.G2API_AddBolt(&ent->ghoul2[0], defaultBoltName);
      }
    ...
    if (ent->client->ps.heldByBolt) {
      BG_IK_MoveLimb(ent->ghoul2, ent->client->ps.heldByBolt, animBone,
                     firstBone, secondBone, level.time, &ent->s,
                     ent->client->clientInfo.animFileIndex,
                     ent->client->ps.torsoAnim /*BOTH_DEAD1*/,
                     (float *)vec3_origin, &ent->client->ps.ikStatus,
                     ent->client->ps.origin, ent->client->ps.viewangles,
                     ent->s.modelScale, 500, qtrue);
    }
  }
}

void BG_G2SetBoneAngles(centity_t *cent, gentity_t *gent, int boneIndex,
  ...
  if (boneIndex != -1) {
    gi.G2API_SetBoneAnglesIndex(&cent->gent->ghoul2[0], boneIndex, angles,
                                flags, up, right, forward, modelList, 0, 0);
  }
}
...
void PM_ScaleUcmd(playerState_t *ps, usercmd_t *cmd, gentity_t *gent) {
  if (G_IsRidingVehicle(gent)) { // driving a vehicle
    // clamp the turn rate
    int maxPitchSpeed =
        MAX_PITCHSPEED_X_WING; // switch, eventually?  Or read from file?
    int diff = AngleNormalize180(SHORT2ANGLE(
                   (cmd->angles[PITCH] + ps->delta_angles[PITCH]))) -
               floor(ps->viewangles[PITCH]);

    if (diff > maxPitchSpeed) {
      cmd->angles[PITCH] = ANGLE2SHORT(ps->viewangles[PITCH] + maxPitchSpeed) -
    ...
    // Um, WTF?  When I turn in a certain direction, I start going backwards? Or
    // strafing?
    int maxYawSpeed =
        MAX_YAWSPEED_X_WING; // switch, eventually?  Or read from file?
    diff = AngleNormalize180(
        SHORT2ANGLE(cmd->angles[YAW] + ps->delta_angles[YAW]) -
        floor(ps->viewangles[YAW]));

    // clamp the turn rate
    if (diff > maxYawSpeed) {
      cmd->angles[YAW] = ANGLE2SHORT(ps->viewangles[YAW] + maxYawSpeed) -
                         ps->delta_angles[YAW];
...
qboolean PM_LockAngles(gentity_t *ent, usercmd_t *ucmd) {
  if (ent->client->ps.viewEntity <= 0 ||
      ent->client->ps.viewEntity >=
          ENTITYNUM_WORLD) { // don't clamp angles when looking through a
                             // viewEntity
...
qboolean PM_AdjustAnglesToGripper(
    gentity_t *ent,
    usercmd_t
        *ucmd) { // FIXME: make this more generic and have it actually *tell*
                 // the client what cmd angles it should be locked at?
  if (((ent->client->ps.eFlags & EF_FORCE_GRIPPED) ||
    ...
    ucmd->angles[PITCH] =
        ANGLE2SHORT(angles[PITCH]) - ent->client->ps.delta_angles[PITCH];
    ucmd->angles[YAW] =
        ANGLE2SHORT(angles[YAW]) - ent->client->ps.delta_angles[YAW];
    return qtrue;
  }
  return qfalse;
}

qboolean PM_AdjustAnglesToPuller(
    gentity_t *ent, gentity_t *puller, usercmd_t *ucmd,
    qboolean
        faceAway) { // FIXME: make this more generic and have it actually *tell*
                    // the client what cmd angles it should be locked at?
  vec3_t dir, angles;
...
qboolean PM_AdjustAngleForWallRun(gentity_t *ent, usercmd_t *ucmd,
                                  qboolean doMove) {
  if ((ent->client->ps.legsAnim == BOTH_WALL_RUN_RIGHT ||
       ent->client->ps.legsAnim == BOTH_WALL_RUN_LEFT) &&
      ent->client->ps.legsAnimTimer >
          500) { // wall-running and not at end of anim
    // stick to wall, if there is one
    vec3_t fwd, rt, traceTo,
        mins = {ent->mins[0], ent->mins[1], 0},
        maxs = {ent->maxs[0], ent->maxs[1], 24},
        fwdAngles = {0, ent->client->ps.viewangles[YAW], 0};
    trace_t trace;
    ...
    gi.trace(&trace, ent->currentOrigin, mins, maxs, traceTo, ent->s.number,
             ent->clipmask, (EG2_Collision)0, 0);
    if (trace.fraction < 1.0f &&
        (trace.plane.normal[2] >= 0.0f &&
         trace.plane.normal[2] <=
             0.4f)) //&& ent->client->ps.groundEntityNum == ENTITYNUM_NONE )
    {
      trace_t trace2;
      vec3_t traceTo2;
      ...
      gi.trace(&trace2, ent->currentOrigin, mins, maxs, traceTo2, ent->s.number,
               ent->clipmask, (EG2_Collision)0, 0);
      if (trace2.fraction < 1.0f &&
          DotProduct(trace2.plane.normal, wallRunFwd) <=
              -0.999f) { // wall we can't run on in front of us
        trace.fraction =
            1.0f; // just a way to get it to kick us off the wall below
      }
    }
    if (trace.fraction < 1.0f &&
        (trace.plane.normal[2] >= 0.0f &&
    ...
    {               // still a vertical wall there
      ...
      ent->client->ps.viewangles[YAW] =
          vectoyaw(trace.plane.normal) + yawAdjust;
      if (ent->client->ps.viewEntity <= 0 ||
          ent->client->ps.viewEntity >=
              ENTITYNUM_WORLD) { // don't clamp angles when looking through a
                                 // viewEntity
      ...
      if ((ent->s.number && !G_ControlledByPlayer(ent)) || !player_locked) {
        if (doMove) {
          ...
          if (ent->client->ps.legsAnimTimer >
              500) { // not at end of anim yet, pushing forward
            // FIXME: or MA?
            float speed = 175;
            ...
            VectorMA(ent->client->ps.velocity, speed, fwd,
                     ent->client->ps.velocity);
          }
          ent->client->ps.velocity[2] = zVel; // preserve z velocity
          // VectorMA( ent->client->ps.velocity, -128, trace.plane.normal,
          // ent->client->ps.velocity ); pull me toward the wall, too VectorMA(
          // ent->client->ps.velocity, dist, rt, ent->client->ps.velocity );
        }
      }
      ucmd->forwardmove = 0;
      return qtrue;
...
qboolean PM_AdjustAnglesForSpinningFlip(gentity_t *ent, usercmd_t *ucmd,
                                        qboolean anglesOnly) {
  vec3_t newAngles;
  ...
  if (elapsedTime >= spinStart && elapsedTime <= spinEnd) {
    ...
    newAngles[YAW] =
        ent->angle + (spinAmt * (elapsedTime - spinStart) / spinLength);
    if (ent->client->ps.viewEntity <= 0 ||
        ent->client->ps.viewEntity >=
            ENTITYNUM_WORLD) { // don't clamp angles when looking through a
                               // viewEntity
  ...
  // push me
  if (ent->client->ps.legsAnimTimer >
      300) //&& ent->client->ps.groundEntityNum == ENTITYNUM_NONE )
  {        // haven't landed or reached end of anim yet
    if ((ent->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(ent)) ||
      ...
      if (DotProduct(ent->client->ps.velocity, pushDir) < 100) {
        VectorMA(ent->client->ps.velocity, 10, pushDir,
                 ent->client->ps.velocity);
      }
    }
  }
  ...
  if ((ent->s.number < MAX_CLIENTS || G_ControlledByPlayer(ent))) {
    ...
    cg.overrides.thirdPersonVertOffset =
        cg_thirdPersonVertOffset.value + viewDip;
...
qboolean PM_AdjustAnglesForBackAttack(gentity_t *ent, usercmd_t *ucmd) {
  ...
  if ((ent->client->ps.saberMove == LS_A_BACK ||
    ...
    if (ent->client->ps.saberMove != LS_A_BACKSTAB || !ent->enemy ||
      ...
      ucmd->angles[YAW] = ANGLE2SHORT(ent->client->ps.viewangles[YAW]) -
                          ent->client->ps.delta_angles[YAW];
    } else { // keep player facing away from their enemy
      vec3_t enemyBehindDir;
      VectorSubtract(ent->currentOrigin, ent->enemy->currentOrigin,
                     enemyBehindDir);
      float enemyBehindYaw = AngleNormalize180(vectoyaw(enemyBehindDir));
      float yawError = AngleNormalize180(
          enemyBehindYaw - AngleNormalize180(ent->client->ps.viewangles[YAW]));
      if (yawError > 1) {
        yawError = 1;
      } else if (yawError < -1) {
        yawError = -1;
      }
      ucmd->angles[YAW] = ANGLE2SHORT(AngleNormalize180(
                              ent->client->ps.viewangles[YAW] + yawError)) -
                          ent->client->ps.delta_angles[YAW];
      ucmd->angles[PITCH] = ANGLE2SHORT(ent->client->ps.viewangles[PITCH]) -
                            ent->client->ps.delta_angles[PITCH];
    }
...
int G_MinGetUpTime(gentity_t *ent) {
  if (ent && ent->client &&
      (ent->client->ps.legsAnim == BOTH_PLAYER_PA_3_FLY ||
       ...
       ent->client->ps.legsAnim == BOTH_RELEASED)) { // special cases
    return 200;
  } else if (ent && ent->client &&
             ent->client->NPC_class == CLASS_ALORA) { // alora springs up very
                                                      // quickly from knockdowns!
    return 1000;
  } else if ((ent->s.clientNum < MAX_CLIENTS ||
              G_ControlledByPlayer(ent))) { // player can get up faster based on
                                            // his/her force jump skill
    ...
    } else if (ent->client->ps.forcePowerLevel[FP_LEVITATION] ==
               FORCE_LEVEL_2) {
...
qboolean PM_AdjustAnglesForKnockdown(gentity_t *ent, usercmd_t *ucmd,
                                     qboolean angleClampOnly) {
  if (PM_InKnockDown(&ent->client->ps)) { // being knocked down or getting up,
                                          // can't do anything!
    if (!angleClampOnly) {
      if (ent->client->ps.legsAnimTimer > G_MinGetUpTime(ent) ||
          (ent->s.number >= MAX_CLIENTS &&
           !G_ControlledByPlayer(ent))) { // can't get up yet
        ucmd->forwardmove = 0;
        ucmd->rightmove = 0;
      }
      if (ent->NPC) {
      ...
      // you can jump up out of a knockdown and you get get up into a crouch
      // from a knockdown ucmd->upmove = 0; if ( !PM_InForceGetUp(
      // &ent->client->ps ) || ent->client->ps.torsoAnimTimer > 800 ||
      // ent->s.weapon != WP_SABER )
      if (ent->health > 0) { // can only attack if you've started a force-getup
                             // and are using the saber
        ucmd->buttons = 0;
      }
    }
    if (!PM_InForceGetUp(
            &ent->client->ps)) { // can't turn unless in a force getup
      if (ent->client->ps.viewEntity <= 0 ||
          ent->client->ps.viewEntity >=
              ENTITYNUM_WORLD) { // don't clamp angles when looking through a
                                 // viewEntity
...
qboolean PM_AdjustAngleForWallRunUp(gentity_t *ent, usercmd_t *ucmd,
                                    qboolean doMove) {
  if (ent->client->ps.legsAnim ==
      BOTH_FORCEWALLRUNFLIP_START) { // wall-running up
    // stick to wall, if there is one
    vec3_t fwd, traceTo, mins = {ent->mins[0], ent->mins[1], 0},
                         maxs = {ent->maxs[0], ent->maxs[1], 24},
                         fwdAngles = {0, ent->client->ps.viewangles[YAW], 0};
    trace_t trace;
    float dist = 128;
    ...
    gi.trace(&trace, ent->currentOrigin, mins, maxs, traceTo, ent->s.number,
             ent->clipmask, (EG2_Collision)0, 0);
    if (trace.fraction >
        0.5f) { // hmm, some room, see if there's a floor right here
      trace_t trace2;
      vec3_t top, bottom;
      ...
      gi.trace(&trace2, top, ent->mins, ent->maxs, bottom, ent->s.number,
               ent->clipmask, (EG2_Collision)0, 0);
      ...
      { // cool, do the alt-flip and land on whetever it is we just scaled up
        ...
        NPC_SetAnim(ent, SETANIM_BOTH, BOTH_FORCEWALLRUNFLIP_ALT,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    ...
    if ( // ucmd->upmove <= 0 &&
        ent->client->ps.legsAnimTimer > 0 && ucmd->forwardmove > 0 &&
        trace.fraction < 1.0f &&
        (trace.plane.normal[2] >= 0.0f &&
         trace.plane.normal[2] <=
             MAX_WALL_RUN_Z_NORMAL)) { // still a vertical wall there
      // make sure there's not a ceiling above us!
      trace_t trace2;
      VectorCopy(ent->currentOrigin, traceTo);
      traceTo[2] += 64;
      gi.trace(&trace2, ent->currentOrigin, mins, maxs, traceTo, ent->s.number,
               ent->clipmask, (EG2_Collision)0, 0);
      if (trace2.fraction <
          1.0f) { // will hit a ceiling, so force jump-off right now
                  // NOTE: hits any entity or clip brush in the way, too, not
                  // just architecture!
        ...
        if ((ent->s.number >= MAX_CLIENTS && !G_ControlledByPlayer(ent)) ||
          ...
          if (doMove) {
            ...
            VectorScale(trace.plane.normal, -128, ent->client->ps.velocity);
            // push me up
            if (ent->client->ps.legsAnimTimer > 200) { // not at end of anim yet
              float speed = 300;
              /*
    ...
    if (doMove) { // stop it
      ...
      ent->client->ps.velocity[2] += 200;
      NPC_SetAnim(ent, SETANIM_BOTH, BOTH_FORCEWALLRUNFLIP_END,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      ent->client->ps.pm_flags |= PMF_JUMP_HELD;
      ent->client->ps.pm_flags |= PMF_JUMPING | PMF_SLOW_MO_FALL;
      ent->client->ps.forcePowersActive |= (1 << FP_LEVITATION);
...
qboolean PM_AdjustAngleForWallJump(gentity_t *ent, usercmd_t *ucmd,
                                   qboolean doMove) {
  if (PM_InReboundJump(ent->client->ps.legsAnim) ||
      PM_InReboundHold(ent->client->ps.legsAnim)) { // hugging wall, getting
                                                    // ready to jump off
    // stick to wall, if there is one
    ...
    if (g_debugMelee->integer) { // uber-skillz
      if (ucmd->upmove > 0) {    // hold on until you let go manually
        if (PM_InReboundHold(ent->client->ps.legsAnim)) { // keep holding
          ...
          if (ent->client->ps.legsAnimTimer <= 300) {
            ...
            NPC_SetAnim(
                ...
                BOTH_FORCEWALLRELEASE_FORWARD +
                    (ent->client->ps.legsAnim - BOTH_FORCEWALLHOLD_FORWARD),
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    ...
    gi.trace(&trace, ent->currentOrigin, mins, maxs, traceTo, ent->s.number,
             ent->clipmask, (EG2_Collision)0, 0);
    if ( // ucmd->upmove <= 0 &&
        ent->client->ps.legsAnimTimer > 100 && trace.fraction < 1.0f &&
        fabs(trace.plane.normal[2]) <=
            MAX_WALL_GRAB_SLOPE) { // still a vertical wall there
      // FIXME: don't pull around 90 turns
      /*
      if ( (ent->s.number>=MAX_CLIENTS&&!G_ControlledByPlayer(ent)) ||
      !player_locked )
      ...
      */
      ...
      return qtrue;
               ...
               (ent->client->ps.pm_flags & PMF_STUCK_TO_WALL)) { // jump off
      // push off of it!
      ent->client->ps.pm_flags &= ~PMF_STUCK_TO_WALL;
      ...
      if (PM_InReboundHold(
              ent->client->ps.legsAnim)) { // if was in hold pose, release now
...
qboolean PM_AdjustAnglesForBFKick(gentity_t *self, usercmd_t *ucmd,
  ...
  vec3_t mins, maxs;
  int i, e;
  int radius = ((self->maxs[0] * 1.5f) + (self->maxs[0] * 1.5f) +
                STAFF_KICK_RANGE + 24.0f); // a little wide on purpose
  ...
  int numListedEntities =
      gi.EntitiesInBox(mins, maxs, entityList, MAX_GENTITIES);
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (ent->health <= 0) {
      if (level.time - ent->s.time >
          2000) { // died more than 2 seconds ago, forget him
        continue;
      }
    }
    // too far?
    ...
    if (!aimFront) { // aim away from them
      VectorScale(vec2Ent, -1, vec2Ent);
    }
    dot = DotProduct(vec2Ent, v_fwd);
    if (dot < 0.0f) { // never turn all the way around
      continue;
    }
    if (dot > bestDot ||
        ((bestDot - dot < 0.25f) &&
         distToEnt - bestDist >
             8.0f)) { // more in front... OR: still relatively close to in front
                      // and significantly closer
      bestDot = dot;
      bestDist = distToEnt;
      bestYaw = vectoyaw(vec2Ent);
    }
  }
  if (bestYaw != Q3_INFINITE && bestYaw != fwdAngs[YAW]) { // aim us at them
    ...
    } else if (angDiff > 0.0f) { // more than 3 degrees higher
      self->client->ps.viewangles[YAW] += 3.0f;
    } else { // must be more than 3 less than
      self->client->ps.viewangles[YAW] -= 3.0f;
    }
    if (self->client->ps.viewEntity <= 0 ||
        self->client->ps.viewEntity >=
            ENTITYNUM_WORLD) { // don't clamp angles when looking through a
                               // viewEntity
      SetClientViewAngle(self, self->client->ps.viewangles);
    }
    ucmd->angles[YAW] = ANGLE2SHORT(self->client->ps.viewangles[YAW]) -
                        self->client->ps.delta_angles[YAW];
    return qtrue;
  } else { // lock these angles
    if (self->client->ps.viewEntity <= 0 ||
        self->client->ps.viewEntity >=
            ENTITYNUM_WORLD) { // don't clamp angles when looking through a
                               // viewEntity
...
qboolean PM_AdjustAnglesForStabDown(gentity_t *ent, usercmd_t *ucmd) {
  if (PM_StabDownAnim(ent->client->ps.torsoAnim) &&
      ent->client->ps.torsoAnimTimer) { // lock our angles
    ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
    float elapsedTime = PM_AnimLength(ent->client->clientInfo.animFileIndex,
                                      (animNumber_t)ent->client->ps.torsoAnim) -
                        ent->client->ps.torsoAnimTimer;
    // FIXME: scale forwardmove by dist from enemy - none if right next to him
    // (so we don't slide off!)
    if (ent->enemy) {
      float dist2Enemy =
          DistanceHorizontal(ent->enemy->currentOrigin, ent->currentOrigin);
    ...
    switch (ent->client->ps.legsAnim) {
    case BOTH_STABDOWN:
      if (elapsedTime >= 300 && elapsedTime < 900) { // push forward?
        // FIXME: speed!
        ucmd->forwardmove = 127;
      }
      break;
    case BOTH_STABDOWN_STAFF:
      if (elapsedTime > 400 && elapsedTime < 950) { // push forward?
        // FIXME: speed!
        ucmd->forwardmove = 127;
      }
      break;
    ...
    {                                                // aim at the enemy
      ...
      float yawError = AngleNormalize180(
          enemyYaw - AngleNormalize180(ent->client->ps.viewangles[YAW]));
      if (yawError > 1) {
        yawError = 1;
      } else if (yawError < -1) {
        yawError = -1;
...
qboolean PM_AdjustAnglesForSpinProtect(gentity_t *ent, usercmd_t *ucmd) {
  if (ent->client->ps.torsoAnim ==
      BOTH_A6_SABERPROTECT) { // in the dual spin thing
    if (ent->client->ps
            .torsoAnimTimer) { // flatten and lock our angles, pull back camera
      // FIXME: lerp this
...
qboolean PM_AdjustAnglesForHeldByMonster(gentity_t *ent, gentity_t *monster,
                                         usercmd_t *ucmd) {
  ...
  ucmd->angles[PITCH] =
      ANGLE2SHORT(newViewAngles[PITCH]) - ent->client->ps.delta_angles[PITCH];
  ucmd->angles[YAW] =
      ANGLE2SHORT(newViewAngles[YAW]) - ent->client->ps.delta_angles[YAW];
  return qtrue;
}

qboolean G_OkayToLean(playerState_t *ps, usercmd_t *cmd,
                      qboolean interruptOkay) {
  if ((ps->clientNum < MAX_CLIENTS ||
       G_ControlledByPlayer(&g_entities[ps->clientNum])) // player
      ...
      !(cmd->buttons &
        (BUTTON_ATTACK | BUTTON_ALT_ATTACK | BUTTON_FORCE_LIGHTNING |
         BUTTON_USE_FORCE | BUTTON_FORCE_DRAIN |
         BUTTON_FORCEGRIP)) // not trying to attack
...
void PM_UpdateViewAngles(playerState_t *ps, usercmd_t *cmd, gentity_t *gent) {
  short temp;
  float rootPitch = 0, pitchMin = -75, pitchMax = 75, yawMin = 0, yawMax = 0,
        lockedYawValue = 0; // just to shut up warnings
  int i;
  ...
  // TEMP
#if 0 // rww 12/23/02 - I'm disabling this for now, I'm going to try to make it
      // work with my new rag stuff
	if ( gent != NULL )
	{
		PM_IKUpdate( gent );
	}
  ...
  if (ps->clientNum != 0 && gent != NULL && gent->client != NULL) {
    if (gent->client->renderInfo.renderFlags & RF_LOCKEDANGLE) {
      pitchMin = 0 - gent->client->renderInfo.headPitchRangeUp -
                 gent->client->renderInfo.torsoPitchRangeUp;
      pitchMax = gent->client->renderInfo.headPitchRangeDown +
                 gent->client->renderInfo.torsoPitchRangeDown;

      yawMin = 0 - gent->client->renderInfo.headYawRangeLeft -
               gent->client->renderInfo.torsoYawRangeLeft;
      yawMax = gent->client->renderInfo.headYawRangeRight +
               gent->client->renderInfo.torsoYawRangeRight;
  ...
  if (gent && gent->client && gent->client->NPC_class == CLASS_VEHICLE &&
      gent->m_pVehicle) {
    pVeh = gent->m_pVehicle;
    // If we're a vehicle...
    if (pVeh->m_pVehicleInfo->Inhabited(pVeh) || pVeh->m_iBoarding != 0 ||
        pVeh->m_pVehicleInfo->type != VH_ANIMAL) {
      lockedYawValue = pVeh->m_vOrientation[YAW];
      yawMax = yawMin = 0;
      rootPitch = pVeh->m_vOrientation[PITCH]; //???  what if goes over 90 when
                                               //add the min/max?
    ...
    // If we're riding a vehicle...
      ...
      if (pVeh->m_pVehicleInfo->type !=
          VH_ANIMAL) {      // animals just turn normally, no clamping
        ...
        if (pVeh->m_pVehicleInfo->type == VH_FIGHTER) {
          rootPitch = pVeh->m_vOrientation[PITCH]; // gent->owner->client->ps.vehicleAngles[PITCH];//???
                                                   // what if goes over 90 when
                                                   // add the min/max?
          pitchMax = pVeh->m_pVehicleInfo->pitchLimit;
          pitchMin = -pitchMax;
        } else {
          rootPitch = 0; // gent->owner->client->ps.vehicleAngles[PITCH];//???
                         // what if goes over 90 when add the min/max?
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (i == PITCH) {
      ...
      if (temp > pitchClampMax) {
        ps->delta_angles[i] =
            (pitchClampMax - cmd->angles[i]) & 0xffff; //& clamp to short
        temp = pitchClampMax;
        // clamped = qtrue;
      } else if (temp < pitchClampMin) {
        ps->delta_angles[i] =
            (pitchClampMin - cmd->angles[i]) & 0xffff; //& clamp to short
        temp = pitchClampMin;
        // clamped = qtrue;
      }
    }
    ...
    0xffff; temp = pitchClampMax;
            }
            else if ( temp < pitchClampMin )
            {
                    ps->delta_angles[i] = (pitchClampMin - cmd->angles[i]) &
    0xffff; temp = pitchClampMin;
            }
    }
    */
    ...
    if ( i == YAW && lockedYaw )
    {
            ...
            // don't let the player look left or right more than the clamp, if
    any if ( yawDiff > yawMax )
            {
                    //clamped = qtrue;
                    ps->viewangles[i] = AngleNormalize180(
    lockedYawValue+((yawMax-2)*multiplier) );
            }
            else if ( yawDiff < yawMin )
            {
                    //clamped = qtrue;
                    ps->viewangles[i] = AngleNormalize180(
    lockedYawValue+((yawMin+2)*multiplier) );
            }
            else
            {
    ...
    */
    if (i == YAW && lockedYaw) {
      ...
      if (temp > yawClampMax) {
        ps->delta_angles[i] =
            (yawClampMax - cmd->angles[i]) & 0xffff; //& clamp to short
        temp = yawClampMax;
        // clamped = qtrue;
      } else if (temp < yawClampMin) {
        ps->delta_angles[i] =
            (yawClampMin - cmd->angles[i]) & 0xffff; //& clamp to short
        temp = yawClampMin;
        // clamped = qtrue;
      }
  ...
  if (gent) {                          // only in the real pmove
    if ((cmd->buttons & BUTTON_USE)) { // check leaning
      if (cg.renderingThirdPerson) {   // third person lean
        if (G_OkayToLean(ps, cmd, qtrue) &&
            (cmd->rightmove || (cmd->forwardmove &&
                                g_debugMelee->integer))) // pushing a direction
        {
          int anim = -1;
          if (cmd->rightmove > 0) {     // lean right
            if (cmd->forwardmove > 0) { // lean forward right
              if (ps->torsoAnim == BOTH_DODGE_HOLD_FR) {
                anim = ps->torsoAnim;
              } else {
                anim = BOTH_DODGE_FR;
              }
            } else if (cmd->forwardmove < 0) { // lean backward right
          ...
          } else if (cmd->rightmove < 0) { // lean left
            if (cmd->forwardmove > 0) {    // lean forward left
              if (ps->torsoAnim == BOTH_DODGE_HOLD_FL) {
                anim = ps->torsoAnim;
              } else {
                anim = BOTH_DODGE_FL;
              }
            } else if (cmd->forwardmove < 0) { // lean backward left
          ...
          } else {                      // not pressing either side
            if (cmd->forwardmove > 0) { // lean forward
            ...
            } else if (cmd->forwardmove < 0) { // lean backward
              if (PM_DodgeAnim(ps->torsoAnim)) {
                anim = ps->torsoAnim;
              } else if (Q_irand(0, 1)) {
                anim = BOTH_DODGE_BL;
          ...
          if (anim != -1) {
            int extraHoldTime = 0;
            if (PM_DodgeAnim(ps->torsoAnim) &&
                !PM_DodgeHoldAnim(ps->torsoAnim)) { // already in a dodge
            ...
            if (anim == pm->ps->torsoAnim) {
              ...
              NPC_SetAnim(gent, SETANIM_TORSO, anim,
                          SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
            }
            ...
            if (ps->groundEntityNum != ENTITYNUM_NONE && !cmd->upmove) {
              NPC_SetAnim(gent, SETANIM_LEGS, anim,
                          SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
            ...
            ps->leanStopDebounceTime =
                ceil((float)ps->torsoAnimTimer / 50.0f); // 20;
          }
        }
      } else if ((!cg.renderingThirdPerson || cg.zoomMode) &&
                 cmd->rightmove != 0 && !cmd->forwardmove &&
                 cmd->upmove <=
                     0) { // Only lean if holding use button, strafing and not
                          // moving forward or back and not jumping
        int leanofs = 0;
        vec3_t viewangles;
        ...
        if (cmd->rightmove > 0) {
          ...
          {
          PM_SetAnim(pm, SETANIM_LEGS, LEGS_LEAN_RIGHT1,
          SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
          }
          pm->ps->legsAnimTimer = 500;//Force it to hold the anim for at least
          half a sec
          */
          ...
          {
          PM_SetAnim(pm, SETANIM_LEGS, LEGS_LEAN_LEFT1,
          SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
          }
          pm->ps->legsAnimTimer = 500;//Force it to hold the anim for at least
          half a sec
        ...
        gi.trace(&trace, start, tmins, tmaxs, end, gent->s.number,
