...
// The surfacesprites are a simple system.  When a polygon with this shader
// stage on it is drawn, there are randomly distributed images (defined by the
// shader stage) placed on the surface. these are capable of doing effects,
// grass, or simple oriented sprites. They usually stick vertically off the
// surface, hence the term vertigons.

// The vertigons are applied as part of the renderer backend.  That is, they
// access OpenGL calls directly.
...
static void R_SurfaceSpriteFrameUpdate(void) {
  float dtime,
      dampfactor; // Time since last update and damping time for wind changes
  float ratio;
  ...
  if (backEnd.refdef.time <
      lastSSUpdateTime) { // Time is BEFORE the last update time, so reset
                          // everything.
    curWindGust = 5;
  ...
  // Adjust for an FOV.  If things look twice as wide on the screen, pretend the
  // shaders have twice the range. ASSUMPTION HERE IS THAT "standard" fov is the
  // first one rendered.

  if (!standardfovinitialized) { // This isn't initialized yet.
    if (backEnd.refdef.fov_x > 50 &&
        backEnd.refdef.fov_x < 135) // I don't consider anything below 50 or
                                    // above 135 to be "normal".
    {
  ...
  } else if (standardfovx ==
             backEnd.refdef.fov_x) { // This is the standard FOV (or higher),
                                     // don't multiply the shader range.
    rangescalefactor = 1.0;
  } else { // We are using a non-standard FOV.  We need to multiply the range of
           // the shader by a scale factor.
    if (backEnd.refdef.fov_x > 135) {
      rangescalefactor = standardscalex / tan(135.0f * 0.5f * (M_PI / 180.0f));
    } else {
      rangescalefactor =
          standardscalex / tan(backEnd.refdef.fov_x * 0.5 * (M_PI / 180.0f));
    }
  }

  // Create a set of four right vectors so that vertical sprites aren't always
  // facing the same way. First generate a HORIZONTAL forward vector
  // (important).
  ...
  // Update the wind.
  // If it is raining, get the windspeed from the rain system rather than the
  // cvar
  ...
  if (R_GetWindSpeed(
          targetspeed,
          NULL)) { // We successfully got a speed from the rain system.
  ...
  if (targetspeed > 0 && curWindGust) {
    if (gustLeft > 0) { // We are gusting
      // Add an amount to the target wind speed
      targetspeed *= 1.0 + gustLeft;

      gustLeft -=
          (float)(backEnd.refdef.time - lastSSUpdateTime) * WIND_GUST_DECAY;
      if (gustLeft <= 0) {
        nextGustTime =
            backEnd.refdef.time + (curWindGust * 1000) * Q_flrand(1.0f, 4.0f);
      }
    } else if (backEnd.refdef.time >=
               nextGustTime) { // See if there is another right now
      // Gust next time, mano
  ...
  } else { // Calculate the target wind vector based off cvars
  ...
  if (targetspeed > 0) {
    //		ang[YAW] +=
    //cos(tr.refdef.time*0.01+flrand(-1.0,1.0))*targetspeed*0.5; 		ang[PITCH] +=
    //sin(tr.refdef.time*0.01+flrand(-1.0,1.0))*targetspeed*0.5;
  }

  // Get the grass wind vector first
  AngleVectors(ang, targetWindGrassDir, NULL, NULL);
  targetWindGrassDir[2] -= 1.0;
  //		VectorScale(targetWindGrassDir, targetspeed,
  //targetWindGrassDir);
  ...
  // Start calculating a smoothing factor so wind doesn't change abruptly
  // between speeds.
  dampfactor =
      1.0 - r_windDampFactor->value; // We must exponent the amount LEFT rather
                                     // than the amount bled off
  dtime = (float)(backEnd.refdef.time - lastSSUpdateTime) *
          (1.0 / (float)WIND_DAMP_INTERVAL); // Our dampfactor is geared towards
                                             // a time interval equal to "1".

  // Note that since there are a finite number of "practical" delta millisecond
  // values possible, the ratio should be initialized into a chart ultimately.
  ...
  if (fabsf(r_windPointForce->value - curWindPointForce) >
      ratio) { // Make sure not to get infinitly small number here
    curWindPointForce = r_windPointForce->value -
                        (ratio * (r_windPointForce->value - curWindPointForce));
  }
...
static void RB_VerticalSurfaceSprite(vec3_t loc, float width, float height,
                                     byte light, byte alpha, float wind,
                                     float windidle, vec2_t fog, int hangdown,
                                     vec2_t skew, bool flattened) {
  vec3_t loc2, right;
  float angle;
  float windsway;
  float points[16];
...
static void RB_VerticalSurfaceSpriteWindPoint(
    vec3_t loc, float width, float height, byte light, byte alpha, float wind,
    float windidle, vec2_t fog, int hangdown, vec2_t skew, vec2_t winddiff,
    float windforce, bool flattened) {
  vec3_t loc2, right;
  float angle;
  float windsway;
  float points[16];
  ...
  loc2[2] -= height * windforce *
             (0.75 + 0.15 * sin((tr.refdef.time + 500 * windforce) * 0.01));
...
static void RB_DrawVerticalSurfaceSprites(shaderStage_t *stage,
  ...
  vec2_t winddiffv;
  float windforce = 0;
  qboolean usewindpoint =
      (qboolean) !!(curWindPointActive && stage->ss->wind > 0);

  float cutdist = stage->ss->fadeMax * rangescalefactor,
        cutdist2 = cutdist * cutdist;
  float fadedist = stage->ss->fadeDist * rangescalefactor,
        fadedist2 = fadedist * fadedist;

  assert(cutdist2 != fadedist2);
  float inv_fadediff = 1.0 / (cutdist2 - fadedist2);

  // The faderange is the fraction amount it takes for these sprites to fade
  // out, assuming an ideal fade range of 250
  float faderange = FADE_RANGE / (cutdist - fadedist);

  if (faderange > 1.0) { // Don't want to force a new fade_rand
    faderange = 1.0;
  }
  ...
  for (curvert = 0; curvert < input->numVertexes; curvert++) {
    ...
    SSVertAlpha[curvert] =
        1.0 - (VectorLengthSquared(dist) - fadedist2) * inv_fadediff;
  }

  // Wind only needs initialization once per tess.
  if (usewindpoint && !tess.SSInitializedWind) {
    for (curvert = 0; curvert < input->numVertexes;
         curvert++) { // Calc wind at each point
      ...
      if (step >= (float)(WINDPOINT_RADIUS * WINDPOINT_RADIUS)) { // No wind
        SSVertWindDir[curvert][0] = 0;
        SSVertWindDir[curvert][1] = 0;
        SSVertWindForce[curvert] = 0; // Should be < 1
      } else {
        if (step < 1) { // Don't want to divide by zero
          SSVertWindDir[curvert][0] = 0;
          SSVertWindDir[curvert][1] = 0;
          ...
          step =
              1.0 -
              (1.0 /
               (step * WINDPOINT_RADIUS)); // 1- (dist/maxradius) = a scale from
                                           // 0 to 1 linearly dropping off
          SSVertWindForce[curvert] =
              curWindPointForce * stage->ss->wind *
              step; // *step means divide by the distance.
        }
      }
    }
  ...
  for (curindex = 0; curindex < input->numIndexes - 2; curindex += 3) {
    ...
    if (triarea <= 1.0) { // Insanely small abhorrent triangle.
      continue;
    }
    ...
    randominterval =
        (byte)(v1[0] + v2[1] + v3[2]) |
        0x03; // Make sure the interval is at least 3, and always odd
    rightvectorcount = 0;
    ...
    for (posi = 0; posi < 1.0; posi += step) {
      for (posj = 0; posj < (1.0 - posi); posj += step) {
        ...
        // Note that the alpha at this point is a value from 1.0 to 0.0, but
        // represents when to START fading
        thisspritesfadestart =
            faderange + (1.0 - faderange) * randomchart[randomindex];
        randomindex += randominterval;

        // Find where the alpha is relative to the fadestart, and calc the real
        // alpha to draw at.
        ...
        if (alpha > 0.0) {
          ...
          if (usewindpoint) {
            winddiffv[0] =
                winddiff1[0] * fa + winddiff2[0] * fb + winddiff3[0] * fc;
            winddiffv[1] =
                winddiff1[1] * fa + winddiff2[1] * fb + winddiff3[1] * fc;
          ...
          if (SSAdditiveTransparency) { // Additive transparency, scale light
                                        // value
                                        //						light *= alpha;
          ...
          width = stage->ss->width *
                  (1.0 + (stage->ss->variance[0] * randomchart[randomindex2]));
          height =
              stage->ss->height *
              (1.0 + (stage->ss->variance[1] * randomchart[randomindex2++]));
          ...
          if (stage->ss->vertSkew != 0) { // flrand(-vertskew, vertskew)
            skew[0] =
                height *
                ((stage->ss->vertSkew * 2.0f * randomchart[randomindex2++]) -
                 stage->ss->vertSkew);
            skew[1] =
                height *
                ((stage->ss->vertSkew * 2.0f * randomchart[randomindex2++]) -
          ...
          if (usewindpoint && windforce > 0 && stage->ss->wind > 0.0) {
            if (SSUsingFog) {
              RB_VerticalSurfaceSpriteWindPoint(
                  curpoint, width, height, (byte)light, (byte)(alpha * 255.0),
                  stage->ss->wind, stage->ss->windIdle, fogv, stage->ss->facing,
                  skew, winddiffv, windforce,
                  SURFSPRITE_FLATTENED == stage->ss->surfaceSpriteType);
            } else {
              RB_VerticalSurfaceSpriteWindPoint(
                  curpoint, width, height, (byte)light, (byte)(alpha * 255.0),
                  stage->ss->wind, stage->ss->windIdle, NULL, stage->ss->facing,
                  skew, winddiffv, windforce,
                  SURFSPRITE_FLATTENED == stage->ss->surfaceSpriteType);
            }
          } else {
            if (SSUsingFog) {
              RB_VerticalSurfaceSprite(
                  curpoint, width, height, (byte)light, (byte)(alpha * 255.0),
                  stage->ss->wind, stage->ss->windIdle, fogv, stage->ss->facing,
                  skew, SURFSPRITE_FLATTENED == stage->ss->surfaceSpriteType);
            } else {
              RB_VerticalSurfaceSprite(
                  curpoint, width, height, (byte)light, (byte)(alpha * 255.0),
                  stage->ss->wind, stage->ss->windIdle, NULL, stage->ss->facing,
...
static void RB_OrientedSurfaceSprite(vec3_t loc, float width, float height,
                                     byte light, byte alpha, vec2_t fog,
                                     int faceup) {
  vec3_t loc2, right;
  float points[16];
  color4ub_t color;
...
static void RB_DrawOrientedSurfaceSprites(shaderStage_t *stage,
                                          shaderCommands_t *input) {
  int curindex, curvert;
  vec3_t dist;
  ...
  if (stage->ss->facing) { // Faceup sprite.
    minnormal = 0.99f;
  } else { // Normal oriented sprite
    minnormal = 0.5f;
  }
    ...
    for (posi = 0; posi < 1.0; posi += step) {
      for (posj = 0; posj < (1.0 - posi); posj += step) {
        ...
        if (alpha > 0.0) {
          ...
          if (SSUsingFog) {
            RB_OrientedSurfaceSprite(curpoint, width, height, (byte)light,
                                     (byte)(alpha * 255.0), fogv,
                                     stage->ss->facing);
          } else {
            RB_OrientedSurfaceSprite(curpoint, width, height, (byte)light,
                                     (byte)(alpha * 255.0), NULL,
                                     stage->ss->facing);
          }
...
static void RB_EffectSurfaceSprite(vec3_t loc, float width, float height,
                                   byte light, byte alpha, float life,
                                   int faceup) {
  vec3_t loc2, right;
  float points[16];
  color4ub_t color;
...
static void RB_DrawEffectSurfaceSprites(shaderStage_t *stage,
                                        shaderCommands_t *input) {
  int curindex, curvert;
  vec3_t dist;
  float triarea, minnormal;
  ...
  // out, assuming an ideal fade range of 250
  float faderange = FADE_RANGE / (cutdist - fadedist);
  if (faderange > 1.0f) { // Don't want to force a new fade_rand
    faderange = 1.0f;
  }
  ...
  if (stage->ss->fxAlphaEnd < 0.05 && stage->ss->height >= 0.1 &&
      stage->ss->width >= 0.1) { // The sprite fades out, and it doesn't start
                                 // at a pinpoint.  Let's fade it in.
    fadeinout = qtrue;
  }

  if (stage->ss->surfaceSpriteType ==
      SURFSPRITE_WEATHERFX) { // This effect is affected by weather settings.
    if (curWeatherAmount < 0.01) { // Don't show these effects
      return;
    } else {
  ...
  for (curvert = 0; curvert < input->numVertexes; curvert++) {
    ...
    VectorSubtract(ssViewOrigin, input->xyz[curvert], dist);
    SSVertAlpha[curvert] =
        1.0f - (VectorLengthSquared(dist) - fadedist2) * inv_fadediff;

    // Note this is the proper equation, but isn't used right now because it
    // would be just a tad slower. Formula for alpha is 1.0f -
    // ((len-fade)/(cut-fade)) Which is equal to (1.0+fade/(cut-fade)) -
    // (len/(cut-fade)) So mult=1/(cut-fade), and base=(1+fade*mult).
  ...
  for (curindex = 0; curindex < input->numIndexes - 2; curindex += 3) {
    ...
    triarea = vec1to3[0] * vec1to2[1] - vec1to3[1] * vec1to2[0];
    triarea = fabs(triarea);
    if (triarea <= 1.0f) { // Insanely small abhorrent triangle.
      continue;
    }
    ...
    for (posi = 0; posi < 1.0f; posi += step) {
      for (posj = 0; posj < (1.0 - posi); posj += step) {
        effecttime = (tr.refdef.time + 10000.0 * randomchart[randomindex]) /
                     stage->ss->fxDuration;
        ...
        // Note that the alpha at this point is a value from 1.0f to 0.0, but
        // represents when to START fading
        thisspritesfadestart =
            faderange + (1.0 - faderange) * randomchart[randomindex2];
        ...
        if (alpha > 0.0f) {
          ...
          randomindex2 = randomindex;
          width = stage->ss->width *
                  (1.0f + (stage->ss->variance[0] * randomchart[randomindex2]));
          height =
              stage->ss->height *
              (1.0f + (stage->ss->variance[1] * randomchart[randomindex2++]));

          width = width + (effectpos * stage->ss->fxGrow[0] * width);
          height = height + (effectpos * stage->ss->fxGrow[1] * height);

          // If we want to fade in and out, that's different than a straight
          // fade.
          if (fadeinout) {
            if (effectpos > 0.5) { // Fade out
              alpha = alpha * (stage->ss->fxAlphaStart +
                               (fxalpha * (effectpos - 0.5) * 2.0));
            } else { // Fade in
              alpha = alpha * (stage->ss->fxAlphaStart +
                               (fxalpha * (0.5 - effectpos) * 2.0));
            }
          } else { // Normal fade
          ...
          if (stage->ss->wind > 0.0f && curWindSpeed > 0.001) {
            ...
            VectorMA(curpoint, effectpos * stage->ss->wind, curWindBlowVect,
                     drawpoint);
            RB_EffectSurfaceSprite(drawpoint, width, height, (byte)light,
                                   (byte)(alpha * 255.0f),
                                   stage->ss->fxDuration, stage->ss->facing);
          } else {
            RB_EffectSurfaceSprite(curpoint, width, height, (byte)light,
                                   (byte)(alpha * 255.0f),
                                   stage->ss->fxDuration, stage->ss->facing);
          }
...
void RB_DrawSurfaceSprites(shaderStage_t *stage, shaderCommands_t *input) {
  ...
  if ((glbits & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) ==
      (GLS_SRCBLEND_ONE |
       GLS_DSTBLEND_ONE)) { // Additive transparency, scale light value
  ...
  if (backEnd.currentEntity != ssLastEntityDrawn) {
    if (backEnd.currentEntity ==
        &tr.worldEntity) { // Drawing the world, so our job is dead-easy, in the
                           // viewparms
    ...
    } else { // Drawing an entity, so we need to transform the viewparms to the
             // model's coordinate system
             //			R_WorldPointToEntity
             //(backEnd.viewParms.ori.origin, ssViewOrigin);
      ...
      //			R_WorldToLocal(backEnd.viewParms.ori.axis[1],
      //ssViewRight); 			R_WorldToLocal(backEnd.viewParms.ori.axis[2], ssViewUp);
    }
