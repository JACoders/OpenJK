...
void RB_UpdateWindObject(windObject_t *wo) {
	if (wo->targetVelocityTimeRemaining == 0) {
		if (Q_flrand(0.f, 1.f) < wo->chanceOfDeadTime) {
			...
			VectorSet(wo->targetVelocity, Q_flrand(wo->minVelocity[0], wo->maxVelocity[0]), Q_flrand(wo->minVelocity[1], wo->maxVelocity[1]),
...
void GenerateRainModel(weatherObject_t &ws, const int maxParticleCount) {
	...
	ws.lastVBO = R_CreateVBO(nullptr, sizeof(rainVertex_t) * rainVertices.size(), VBO_USAGE_XFB);
	ws.vbo = R_CreateVBO((byte *)rainVertices.data(), sizeof(rainVertex_t) * rainVertices.size(), VBO_USAGE_XFB);
...
void GenerateDepthMap() {
	...
	vec3_t mapSize;
	vec3_t halfMapSize;
	VectorSubtract(tr.world->bmodels[0].bounds[0], tr.world->bmodels[0].bounds[1], mapSize);
	...
	const vec3_t viewBounds[2] = {{0.0f, -halfMapSize[1], -halfMapSize[0]}, {halfMapSize[2] * 2.0f, halfMapSize[1], halfMapSize[0]}};

	R_SetupViewParmsForOrthoRendering(tr.weatherDepthFbo->width, tr.weatherDepthFbo->height, tr.weatherDepthFbo,
									  VPF_DEPTHCLAMP | VPF_DEPTHSHADOW | VPF_ORTHOGRAPHIC | VPF_NOVIEWMODEL, orientation, viewBounds);
	Matrix16Multiply(tr.viewParms.projectionMatrix, tr.viewParms.world.modelViewMatrix, tr.weatherSystem->weatherMVP);
	...
	if (tr.weatherSystem->numWeatherBrushes > 0) {
		...
		vec3_t up = {stepSize[0] * 0.5f, 0.0f, 0.0f};
		vec3_t left = {0.0f, stepSize[1] * 0.5f, 0.0f};
		vec3_t traceVec = {0.0f, 0.0f, -1.0f};
		...
		for (int i = 0; i < tr.weatherSystem->numWeatherBrushes; i++) {
			...
			ivec2_t numSteps = {int((maxs[0] - mins[0]) / stepSize[0]) + 2, int((maxs[1] - mins[1]) / stepSize[1]) + 2};

			vec2_t rayOrigin = {tr.world->bmodels[0].bounds[0][0] + (floorf((mins[0] - tr.world->bmodels[0].bounds[0][0]) / stepSize[0]) + 0.5f) * stepSize[0],
			...
			for (int y = 0; y < (int)numSteps[1]; y++) {
				for (int x = 0; x < (int)numSteps[0]; x++) {
					vec3_t rayPos = {rayOrigin[0] + x * stepSize[0], rayOrigin[1] + y * stepSize[1], tr.world->bmodels[0].bounds[1][2]};
					...
					for (int j = 0; j < currentWeatherBrush->numPlanes; j++) {
						...
						vec3_t normal = {currentWeatherBrush->planes[j][0], currentWeatherBrush->planes[j][1], currentWeatherBrush->planes[j][2]};
					...
					if (tess.numVertexes + 4 >= SHADER_MAX_VERTEXES || tess.numIndexes + 6 >= SHADER_MAX_INDEXES) {
						...
						GLSL_SetUniformMatrix4x4(&tr.textureColorShader, UNIFORM_MODELVIEWPROJECTIONMATRIX, tr.weatherSystem->weatherMVP);
...
void RB_SimulateWeather(weatherObject_t *ws, vec2_t *zoneOffsets, int zoneIndex) {
	if (ws->vboLastUpdateFrame == backEndData->realFrameNumber || tr.weatherSystem->frozen) {
	...
	item.attributes = ojkAllocArray<vertexAttribute_t>(*backEndData->perFrameMemory, numAttribs);
	memcpy(item.attributes, ws->attribsTemplate, sizeof(*item.attributes) * numAttribs);
	...
	const vec2_t mapZExtents = {tr.world->bmodels[0].bounds[0][2], tr.world->bmodels[0].bounds[1][2]};
	const float frictionInverse = 0.7f;
	vec3_t envForce = {tr.weatherSystem->windDirection[0] * frictionInverse, tr.weatherSystem->windDirection[1] * frictionInverse,
					   -ws->gravity * frictionInverse};
	vec4_t randomOffset = {Q_flrand(-4.0f, 4.0f), Q_flrand(-4.0f, 4.0f), Q_flrand(0.0f, 1.0f),
	...
	const UniformBlockBinding uniformBlockBindings[] = {{currentFrameUbo, tr.sceneUboOffset, UNIFORM_BLOCK_SCENE}};
...
void R_ShutdownWeatherSystem() {
	if (tr.weatherSystem != nullptr) {
		...
		ri.Printf(PRINT_DEVELOPER, "Weather system shutdown requested, but it is already shut down.\n");
	}
}
...
void RE_WorldEffectCommand(const char *command) {
	...
	//---------------
		...
		if (!WE_ParseVector(&command, 3, parsedWind))
			VectorAdd(tr.weatherSystem->constWindDirection, defaultWind, tr.weatherSystem->constWindDirection);
		else
			VectorMA(tr.weatherSystem->constWindDirection, 0.001f, parsedWind, tr.weatherSystem->constWindDirection);
	}
	...
	//---------------------
		...
		VectorScale(tr.weatherSystem->weatherSlots[WEATHER_RAIN].color, 0.5f, tr.weatherSystem->weatherSlots[WEATHER_RAIN].color);
	}
	...
	//---------------------
		...
		VectorScale(tr.weatherSystem->weatherSlots[WEATHER_RAIN].color, 0.7f, tr.weatherSystem->weatherSlots[WEATHER_RAIN].color);
	}
	...
	//---------------------
		...
		VectorScale(tr.weatherSystem->weatherSlots[WEATHER_SPACEDUST].color, 0.75f, tr.weatherSystem->weatherSlots[WEATHER_SPACEDUST].color);
	}
...
void R_AddWeatherSurfaces() {
	...
	if (tr.weatherSystem->activeWeatherTypes == 0 && r_debugWeather->integer == 0)
		return;

	R_AddDrawSurf((surfaceType_t *)&tr.weatherSystem->weatherSurface, REFENTITYNUM_WORLD, tr.weatherInternalShader, 0, /* fogIndex */
				  qfalse,																							   /* dlightMap */
				  qfalse,																							   /* postRender */
				  0																									   /* cubemapIndex */
	);
...
void RB_SurfaceWeather(srfWeather_t *surf) {
	...
	float centerZoneOffsetX = std::floor((viewOrigin[0] / CHUNK_EXTENDS) + 0.5f);
	float centerZoneOffsetY = std::floor((viewOrigin[1] / CHUNK_EXTENDS) + 0.5f);
	...
	int centerZoneIndex;
	{
		...
		for (int y = -1; y <= 1; ++y) {
			for (int x = -1; x <= 1; ++x, ++currentIndex) {
				...
				VectorSet2(zoneOffsets[chunkIndex], x, y);
	...
	for (int weatherType = 0; weatherType < NUM_WEATHER_TYPES; weatherType++) {
		...
		if (weatherObject->vbo == nullptr)
			GenerateRainModel(tr.weatherSystem->weatherSlots[weatherType], maxWeatherTypeParticles[weatherType]);

		RB_SimulateWeather(weatherObject, &zoneOffsets[0], centerZoneIndex);

		vec4_t viewInfo = {weatherObject->size[0], weatherObject->size[1], weatherObject->velocityOrientationScale, weatherObject->fadeDistance};

		int stateBits = weatherType == WEATHER_SAND ? GLS_DEPTHFUNC_LESS | GLS_SRCBLEND_SRC_ALPHA | GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA
		...
		item.numAttributes = numAttribs;
		item.attributes = ojkAllocArray<vertexAttribute_t>(*backEndData->perFrameMemory, numAttribs);
		memcpy(item.attributes, weatherObject->attribsTemplate, sizeof(*item.attributes) * numAttribs);
		...
		for (int y = -1; y <= 1; ++y) {
			for (int x = -1; x <= 1; ++x, ++currentIndex) {
				const GLuint currentFrameUbo = backEndData->currentFrame->ubo;
				const UniformBlockBinding uniformBlockBindings[] = {{currentFrameUbo, tr.cameraUboOffsets[tr.viewParms.currentViewParm], UNIFORM_BLOCK_CAMERA}};
				DrawItemSetUniformBlockBindings(item, uniformBlockBindings, frameAllocator);

				UniformDataWriter uniformDataWriter;
				uniformDataWriter.Start(&tr.weatherShader);
				uniformDataWriter.SetUniformVec2(UNIFORM_ZONEOFFSET, (centerZoneOffsetX + x) * CHUNK_EXTENDS, (centerZoneOffsetY + y) * CHUNK_EXTENDS);
				...
				item.samplerBindings = samplerBindingsWriter.Finish(frameAllocator, &item.numSamplerBindings);
