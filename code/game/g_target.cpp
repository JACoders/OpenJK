...
"sounds" va() min max, so if your sound string is borgtalk%d.wav, and you set a "sounds" value of 4, it will randomly play borgtalk1.wav - borgtalk4.wav when
...
void target_laser_think(gentity_t *self) {
	...
	if (tr.entityNum) {
		// hurt it if we can
		G_Damage(&g_entities[tr.entityNum], self, self->activator, self->movedir, tr.endpos, self->damage, DAMAGE_NO_KNOCKBACK, MOD_ENERGY);
	}
...
void target_relay_use_go(gentity_t *self) {
	...
	if (self->spawnflags & 4) {
		...
		if (ent && (ent->e_UseFunc != useF_NULL)) { // e_UseFunc check can be omitted
...
void target_relay_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
	if ((self->spawnflags & 1) && activator->client) { //&& activator->client->ps.persistant[PERS_TEAM] != TEAM_RED ) {
		return;
	}

	if ((self->spawnflags & 2) && activator->client) { //&& activator->client->ps.persistant[PERS_TEAM] != TEAM_BLUE ) {
		return;
	}
...
void SP_target_random(gentity_t *self) { self->e_UseFunc = useF_target_random_use; }
	...
	*/
	...
	if (self->behaviorSet[BSET_USE]) {
		if (self->spawnflags & 1) {
			...
			if (self->activator->m_iIcarusID == IIcarusInterface::ICARUS_INVALID) { // Need to be initialized through ICARUS
...
void target_friction_change_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
	...
	if (self->spawnflags & 1) { // FIXME - make a global?
...
void set_mission_stats_cvars(void) {
	...
	if (cg_entities[0].gent->client->sess.missionStats.totalSecrets) {
		...
		gi.cvar_set("ui_stats_secretsfound", va("%d %s %d", cg_entities[0].gent->client->sess.missionStats.secretsFound, text,
