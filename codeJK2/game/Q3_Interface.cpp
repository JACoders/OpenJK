...
============
*/
static void Q3_SetTimeScale(int entID, const char *data) { gi.cvar_set("timescale", data); }
...
void InflateOwner(gentity_t *self) {
	...
	gi.trace(&trace, self->currentOrigin, self->mins, self->maxs, self->currentOrigin, self->owner->s.number,
			 self->owner->clipmask & ~(CONTENTS_SOLID | CONTENTS_MONSTERCLIP), G2_NOCOLLIDE, 0);
...
-------------------------
*/
static void Q3_CameraPath(const char *name) { CGCam_StartRoff(G_NewString(name)); }
