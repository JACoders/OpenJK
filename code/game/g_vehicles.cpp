...
float G_CanJumpToEnemyVeh(Vehicle_t *pVeh, const usercmd_t *pUcmd) {
	...
	//------------------------------------------------------
	if (rider && rider->enemy && pUcmd->rightmove && fabsf(rider->enemy->currentOrigin[2] - rider->currentOrigin[2]) < 50.0f) {
		...
		if (enemyVeh) {
			...
			if (toEnemyDistance < 70.0f && pVeh->m_pParentEntity->resultspeed > 100.0f &&
...
void G_AttachToVehicle(gentity_t *pEnt, usercmd_t **ucmd) {
	...
	// Get the driver tag.
	trap_G2API_GetBoltMatrix(vehEnt->ghoul2, 0, crotchBolt, &boltMatrix, vehEnt->m_pVehicle->m_vOrientation, vehEnt->currentOrigin, level.time, NULL,
							 vehEnt->modelScale);
...
#else
	// Get the driver tag.
	gi.G2API_GetBoltMatrix(vehEnt->ghoul2, vehEnt->playerModel, vehEnt->crotchBolt, &boltMatrix, vehEnt->m_pVehicle->m_vOrientation, vehEnt->currentOrigin,
...
void G_DriveATST(gentity_t *pEnt, gentity_t *atst) {
	if (pEnt->NPC_type && pEnt->client && (pEnt->client->NPC_class == CLASS_ATST)) { // already an atst, switch back
		// open hatch
...
bool Board(Vehicle_t *pVeh, bgEntity_t *pEnt) {
	...
	if (pVeh->m_pVehicleInfo->numHands == 2) { // switch to vehicle weapon
		...
		if ((ent->client->ps.weapon != WP_SABER && ent->client->ps.weapon != WP_BLASTER) ||
			!(pVeh->m_pVehicleInfo->type == VH_ANIMAL || pVeh->m_pVehicleInfo->type == VH_SPEEDER)) { // switch to weapon none?
			if (ent->s.number < MAX_CLIENTS) {
				CG_ChangeWeapon(WP_NONE);
			}
...
getItOutOfMe:
	...
	if (ent->client->ps.weapon == WP_NONE) { // FIXME: check against this vehicle's gun from the g_vehicleInfo table
											 ...
											 // ent->client->ps.ammo[weaponData[WP_EMPLACED_GUN].ammoIndex] = 0;//maybe store this ammo on the vehicle before
											 // clearing it? switch back to a normal weapon we're carrying
			...
			if (pVeh->m_pVehicleInfo->Inhabited(pVeh)) { // if we've still got people in us, just kill the bastards
				if (pVeh->m_pPilot) {
					...
					G_Damage((gentity_t *)pVeh->m_pPilot, (gentity_t *)pVeh->m_pParentEntity, (gentity_t *)pVeh->m_pParentEntity, NULL,
							 pVeh->m_pParentEntity->playerState->origin, 999, DAMAGE_NO_PROTECTION, MOD_EXPLOSIVE);
				}
...
#else
				G_RadiusDamage(trace.endpos, player, pVeh->m_pVehicleInfo->explosionDamage, pVeh->m_pVehicleInfo->explosionRadius, NULL,
							   MOD_EXPLOSIVE); // FIXME: extern damage and radius or base on fuel
#endif
			}
...
static bool Update(Vehicle_t *pVeh, const usercmd_t *pUmcd) {
...
#endif
#else
	if (parent->spawnflags & 1) {			   // NOTE: in SP, this actually just checks LOS to the Player
		...
		if (pVeh->m_iPilotTime && pVeh->m_iPilotTime < level.time) { // die
			// FIXME: does this give proper credit to the enemy who shot you down?
	...
	// always knock guys around now...
	{
		...
		if (force > 30.0f) {
			...
			if (tr.fraction < 1.0f && !tr.allsolid && !tr.startsolid && tr.entityNum != ENTITYNUM_NONE && tr.entityNum != ENTITYNUM_WORLD &&
				...
				if (other && other->client && !other->s.m_iVehicleNum) {
					...
					G_Damage(other, player, player, parent->client->ps.velocity, parent->currentOrigin, force, DAMAGE_NO_ARMOR | DAMAGE_EXTRA_KNOCKBACK,
							 MOD_IMPACT);
				}
			}
		}
...
static void AttachRiders(Vehicle_t *pVeh) {
	...
	while (i < pVeh->m_iNumPassengers) {
		if (pVeh->m_ppPassengers[i]) {
			...
			// Get the driver tag.
			trap_G2API_GetBoltMatrix(parent->ghoul2, 0, crotchBolt, &boltMatrix, yawOnlyAngles, parent->client->ps.origin, level.time, NULL,
									 parent->modelScale);
	...
	if (pVeh->m_pDroidUnit && pVeh->m_iDroidUnitTag != -1) {
		...
		if (droid->client) {
			...
			// Get the droid tag.
			trap_G2API_GetBoltMatrix(parent->ghoul2, 0, pVeh->m_iDroidUnitTag, &boltMatrix, yawOnlyAngles, parent->currentOrigin, level.time, NULL,
									 parent->modelScale);
	...
	if (pVeh->m_pPilot) {
		...
		// Get the driver tag.
		gi.G2API_GetBoltMatrix(parent->ghoul2, parent->playerModel, parent->crotchBolt, &boltMatrix, pVeh->m_vOrientation, parent->currentOrigin,
