...
static void ObjectivePrint_Line(const int color, const int objectIndex, int &missionYcnt) {
	...
	if (cgi_Language_IsAsian()) {
		...
		CG_DisplayBoxedText(objectiveStartingXpos, y, objectiveTextBoxWidth, objectiveTextBoxHeight,
		...
		if (pixelLen < objectiveTextBoxWidth) // One shot - small enough to print entirely on one line
		{
			...
			cgi_R_Font_DrawString(objectiveStartingXpos, y, str, colorTable[color], cgs.media.qhFontMedium, -1, 1.0f);
		...
		// Text is too long, break into lines.
			...
			while (*str) {
				...
				if (pixelLen > objectiveTextBoxWidth) { // Reached max length of this line
					...
					CG_DrawProportionalString(objectiveStartingXpos, y, holdText, CG_SMALLFONT, colorTable[color]);
...
*/
qboolean CG_ForcePower_Valid(int forceKnownBits, int index) {
	if ((forceKnownBits & (1 << showPowers[index])) && loadForcePowerLevel[showPowers[index]]) // Does he have the force power?
	{
		return qtrue;
	}
