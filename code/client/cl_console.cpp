...
void Con_Dump_f(void) {
  ...
  if (!COM_CompareExtension(filename, ".txt")) {
    Com_Printf("Con_Dump_f: Only the \".txt\" extension is supported by this "
               "command!\n");
    return;
  }
...
void Con_DrawNotify(void) {
  ...
  for (i = con.current - NUM_CON_TIMES + 1; i <= con.current; i++) {
    ...
    if (re.Language_IsAsian()) {
      ...
      const int iPixelHeightToAdvance =
          2 + (1.3 / con.yadjust) *
                  re.Font_HeightPixels(
                      iFontIndex, fFontScale); // for asian spacing, since we
                                               // don't want glyphs to touch.
      ...
      //
      re.Font_DrawString(
          con.xadjust * (con.xadjust + (1 * SMALLCHAR_WIDTH /*aesthetics*/)),
          con.yadjust * (v), sTemp, g_color_table[currentColor], iFontIndex, -1,
          fFontScale);
      ...
      for (x = 0; x < con.linewidth; x++) {
        ...
        SCR_DrawSmallChar(con.xadjust + (x + 1) * SMALLCHAR_WIDTH, v,
                          text[x] & 0xff);
      }
      v += SMALLCHAR_HEIGHT;
    }
...
void Con_DrawSolidConsole(float frac) {
  ...
  if (y < 1) {
    ...
    if (frac < 1.0f) {
      vec4_t con_color;
      MAKERGBA(con_color, 1.0f, 1.0f, 1.0f,
               Com_Clamp(0.0f, 1.0f, con_opacity->value));
  ...
  int iFontIndexForAsian =
      0; // kinda tacky, this just gets the first registered font, since Asian
         // stuff ignores the contents anyway
  const float fFontScaleForAsian = 0.75f * con.yadjust;
  int iPixelHeightToAdvance = SMALLCHAR_HEIGHT;
  if (re.Language_IsAsian()) {
    if (!iFontIndexForAsian) {
      iFontIndexForAsian =
          re.RegisterFont("ocr_a"); // must be a font that's used elsewhere
    }
    iPixelHeightToAdvance =
        (1.3 / con.yadjust) *
        re.Font_HeightPixels(iFontIndexForAsian,
                             fFontScaleForAsian); // for asian spacing, since we
                                                  // don't want glyphs to touch.
  }
  ...
  for (i = 0; i < rows; i++, y -= iPixelHeightToAdvance, row--) {
    ...
    if (re.Language_IsAsian()) {
      ...
      re.Font_DrawString(
          con.xadjust * (con.xadjust + (1 * SMALLCHAR_WIDTH /*(aesthetics)*/)),
          con.yadjust * (y), sTemp, g_color_table[currentColor],
          iFontIndexForAsian, -1, fFontScaleForAsian);
    } else {
      for (x = 0; x < con.linewidth; x++) {
        if ((text[x] & 0xff) == ' ') {
          continue;
