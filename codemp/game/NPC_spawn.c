...
extern void misc_model_breakable_pain(gentity_t *self, gentity_t *attacker,
                                      int damage);
...
extern void NPC_MineMonster_Pain(gentity_t *self, gentity_t *attacker,
                                 int damage);
...
void NPC_SetMiscDefaultData(gentity_t *ent) {
  ...
  // if ( !Q_stricmp( "atst_vehicle", ent->NPC_type ) )//FIXME: has to be a
  // better, easier way to tell this, no?
  if (ent->s.NPC_class == CLASS_VEHICLE && ent->m_pVehicle) {
    ent->s.g2radius =
        255; // MAX for this value, was (ent->r.maxs[2]-ent->r.mins[2]), which
             // is 272 or something
  ...
  if (ent->client->ps.weapon ==
      WP_SABER) // rwwFIXMEFIXME: is this going to work?
  {             // if I'm equipped with a saber, initialize it (them)
    //	ent->client->ps.SaberDeactivate();
    //	ent->client->ps.SetSaberLength( 0 );
    WP_SaberInitBladeData(ent);
    ent->client->ps.saberHolstered = 2;
    //	G_CreateG2AttachedWeaponModel( ent, ent->client->ps.saber[0].model,
    //ent->handRBolt, 0 ); 	if ( ent->client->ps.dualSabers )
    //	{
    //		G_CreateG2AttachedWeaponModel( ent, ent->client->ps.saber[1].model,
    //ent->handLBolt, 1 );
    //	}
  ...
  case NPCTEAM_PLAYER:
    ...
    if (ent->client->NPC_class == CLASS_JEDI ||
      ...
      weaponData[ent->client->ps.weapon].weaponMdl, ent->handRBolt, 0 );
      }
      */
...
int NPC_WeaponsForTeam(team_t team, int spawnflags, const char *NPC_type) {
  ...
  case NPCTEAM_ENEMY:
    ...
    if ((Q_stricmp("interrogator", NPC_type) == 0) ||
        ...
        (Q_strncmp("protocol", NPC_type, 8) == 0)) {
      return WP_NONE;
    }
...
void NPC_SetWeapons(gentity_t *ent) {
  ...
  int curWeap;
  int weapons = NPC_WeaponsForTeam((team_t)ent->client->playerTeam,
                                   ent->spawnflags, ent->NPC_type);

  ent->client->ps.stats[STAT_WEAPONS] = 0;
  for (curWeap = WP_SABER; curWeap < WP_NUM_WEAPONS; curWeap++) {
    if ((weapons & (1 << curWeap))) {
      ent->client->ps.stats[STAT_WEAPONS] |= (1 << curWeap);
      //			RegisterItem( FindItemForWeapon(
      //(weapon_t)(curWeap) ) );	//precache the weapon rwwFIXMEFIXME:
      // Precache
      ent->NPC->currentAmmo =
      ...
      if (curWeap == WP_STUN_BATON) {
        ...
        // WP_STUN_BATON if something better comes later in the list
        bestWeap = curWeap;
      }
    }
  }
...
qboolean NPC_SpotWouldTelefrag(gentity_t *npc) {
  ...
  for (i = 0; i < num; i++) {
    ...
    if (hit->inuse && hit->client && hit->s.number != npc->s.number &&
        (hit->r.contents & MASK_NPCSOLID) && hit->s.number != npc->r.ownerNum &&
...
void NPC_Begin(gentity_t *ent) {
  ...
  if (!(ent->spawnflags & SFB_NOTSOLID)) { // No NPCs should telefrag
    if (NPC_SpotWouldTelefrag(ent)) {
      if (ent->wait < 0) { // remove yourself
        ...
        G_DebugPrint(WL_DEBUG,
                     "NPC %s could not spawn, waiting %4.2 secs to try again\n",
  ...
  } else if (ent->NPC->stats.health) // Was health supplied in NPC.cfg?
  {
    ...
    if (ent->client->NPC_class != CLASS_REBORN &&
      ...
      ent->NPC->stats.health +=
          ...
          g_npcspskill.integer; // 100% on easy, 125% on medium, 150% on hard
    }
  ...
  */
  ...
  if (ent->s.m_iVehicleNum) { // I'm an NPC in a vehicle (or a vehicle), I
                              // already have owner set
  } else if (client->NPC_class == CLASS_SEEKER &&
             ent->activator != NULL) { // somebody else "owns" me
  ...
  if (ent->client->playerTeam == NPCTEAM_ENEMY) { // valid enemy spawned
    if (!(ent->spawnflags & SFB_CINEMATIC) &&
      ...
      if (g_entities[0].client) {
        ...
        // to be storing this sort of data.
      }
    }
  }
...
qboolean NPC_StasisSpawn_Go( gentity_t *ent )
{
        ...
        trace_t	tr;
        trap->Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs,
ent->r.currentOrigin, ent->s.number, MASK_NPCSOLID );
...
gentity_t *NPC_Spawn_Do(gentity_t *ent) {
  ...
  */
  ...
  if (ent->spawnflags & NSF_DROP_TO_FLOOR) {
    ...
    trap->Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, bottom,
                ent->s.number, MASK_NPCSOLID, qfalse, 0, 0);
  ...
  if (ent->count != -1) {
    ...
    if (ent->count <= 0) {
      ...
      // FIXME: why not remove me...?  Because of all the string pointers?  Just
      // do G_NewStrings?
    }
  }
  ...
  if (Q_stricmp(ent->classname, "NPC_Vehicle") == 0) {
    ...
    assert(newent->m_pVehicle && newent->m_pVehicle->m_pVehicleInfo &&
    ...
    if (g_vehicleInfo[iVehIndex].type == VH_FIGHTER) { // FIXME: EXTERN!!!
      newent->flags |= (FL_NO_KNOCKBACK | FL_SHIELDED |
                        FL_DMG_BY_HEAVY_WEAP_ONLY); // don't get pushed around,
                                                    // blasters bounce off, only
                                                    // damage from heavy weaps
    }
  ...
  if (ent->NPC_type) {
    if (!Q_stricmp(ent->NPC_type,
                   "kyle")) { // FIXME: "player", not Kyle?  Or check NPC_type
                              // against player's NPC_type?
      ...
      for (n = 0; n < 1; n++) {
        if (g_entities[n].s.eType != ET_NPC && g_entities[n].client) {
          ...
          newent->client->playerTeam = newent->s.teamowner =
              g_entities[n].client->playerTeam;
          break;
        }
      }
  ...
  if (ent->paintarget) { // safe to point at owner's string since memory is
                         // never freed during game
  ...
  if (!ent->use) {
    ...
    ent->targetname = NULL;
    // why not remove me...?  Because of all the string pointers?  Just do
    // G_NewStrings?
...
void NPC_ShySpawn(gentity_t *ent) {
  ...
  if (DistanceSquared(g_entities[0].r.currentOrigin, ent->r.currentOrigin) <=
...
teamnodmg - team that NPC does not take damage from (turrets and other
auto-defenses that have "alliedTeam" set to this team won't target this NPC) 0 -
none 1 - red 2 - blue
...
void NPC_PrecacheType(char *NPC_type) {
  ...
  if (fakespawner) {
    ...
    // NOTE: does the spawner have to stay around to send any precached info to
    // the clients...?
...
void SP_NPC_spawner(gentity_t *self) {
  ...
  */
  ...
  if (self->targetname) { // Wait for triggering
    self->use = NPC_Spawn;
    //	self->r.svFlags |= SVF_NPC_PRECACHE;//FIXME: precache my weapons
    //somehow?
...
/*QUAKED NPC_Vehicle (1 0 0) (-16 -16 -24) (16 16 32) NO_PILOT_DIE SUSPENDED x x
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY NO_PILOT_DIE - die after certain
amount of time of not having a pilot SUSPENDED - Fighters: Don't drop until
someone gets in it (this only works as long as no-nw has *ever* ridden the
vehicle, to simulate ships that are suspended-docked) - note: ships inside
trigger_spaces do not drop when unoccupied CINEMATIC - Will spawn with no
...
this number of seconds before flying forward "dmg" use with NO_PILOT_DIE - delay
in milliseconds for ship to explode if no pilot (default 10000) "speed" use with
NO_PILOT_DIE - distance for pilot to get away from ship after dismounting before
it starts counting down the death timer "model2" - if the vehicle can have a
droid (has "*droidunit" tag), this NPC will be spawned and placed there - note:
game will automatically use the one specified in the .veh file (if any) or,
absent that, it will use an R2D2 or R5D2 NPC)
...
qboolean NPC_VehiclePrecache(gentity_t *spawner) {
  ...
  G_ModelIndex(
      va("$%s", spawner->NPC_type)); // make sure the thing is frickin precached
  ...
  if (pVehInfo->model && pVehInfo->model[0]) {
    ...
    if (pVehInfo->skin && pVehInfo->skin[0]) {
      skin = trap->R_RegisterSkin(va("models/players/%s/model_%s.skin",
                                     pVehInfo->model, pVehInfo->skin));
    }
    trap->G2API_InitGhoul2Model(
        &tempG2, va("models/players/%s/model.glm", pVehInfo->model), 0, skin, 0,
        0, 0);
  ...
  if (droidNPCType) {
    if (Q_stricmp("random", droidNPCType) == 0 ||
        Q_stricmp("default", droidNPCType) ==
            0) { // precache both r2 and r5, as defaults
...
// STAR WARS
// NPCs============================================================================
/*QUAKED NPC_spawner (1 0 0) (-16 -16 -24) (16 16 32) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY
...
painscript - default script to run when hit (none by default)
fleescript - default script to run when hit and below 50% health (none by
default) deathscript - default script to run when killed (none by default)

These strings can be used to activate behaviors instead of scripts - these are
checked first and so no scripts should be names with these names: default - 0:
whatever idle - 1: Stand around, do abolutely nothing roam - 2: Roam around,
collect stuff walk - 3: Crouch-Walk toward their goals run - 4: Run toward their
goals standshoot - 5: Stay in one spot and shoot- duck when neccesary standguard
- 6: Wait around for an enemy patrol - 7: Follow a path, looking for enemies
        huntkill - 8: Track down enemies and kill them
        evade - 9: Run from enemies
        evadeshoot - 10: Run from enemies, shoot them if they hit you
        runshoot - 11: Run to your goal and shoot enemy when possible
...
/*QUAKED NPC_Kyle (1 0 0) (-16 -16 -24) (16 16 32) x RIFLEMAN PHASER TRICORDER
...
/*QUAKED NPC_Reborn_New(1 0 0) (-16 -16 -24) (16 16 40) DUAL STAFF WEAK x
...
/*QUAKED NPC_Prisoner(1 0 0) (-16 -16 -24) (16 16 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
ALTOFFICER ROCKET DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY 30 health,
blaster
...
acrobatic.  Purple saber, 75 health. FENCER - A good lightsaber fighter, but not
acrobatic and uses no force powers.  Yellow saber, 100 health. ACROBAT - quite
acrobatic, but not the best lightsaber fighter and uses no force powers.  Red
saber, 100 health. BOSS - quite acrobatic, good lightsaber fighter and uses
force powers.  Orange saber, 150 health.
...
/*QUAKED NPC_Monster_Howler (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Monster_Wampa (1 0 0) (-12 -12 -24) (12 12 40) WANDER SEARCH x x
DROPTOFLOOR CINEMATIC NOTSOLID STARTINSOLID SHY WANDER - When I don't have an
enemy, I'll just wander the waypoint network aimlessly SEARCH - When I don't
have an enemy, I'll go back and forth between the nearest waypoints, looking for
enemies DROPTOFLOOR - NPC can be in air, but will spawn on the closest floor
surface below it CINEMATIC - Will spawn with no default AI (BS_CINEMATIC)
NOTSOLID - Starts not solid
STARTINSOLID - Don't try to fix if spawn in solid
SHY - Spawner is shy
*/
...
/*QUAKED NPC_Monster_Rancor (1 0 0) (-30 -30 -24) (30 30 104) x x x x
...
/*QUAKED NPC_Droid_Remote (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
/*QUAKED NPC_Droid_Seeker (1 0 0) (-12 -12 -24) (12 12 40) x x x x DROPTOFLOOR
CINEMATIC NOTSOLID STARTINSOLID SHY DROPTOFLOOR - NPC can be in air, but will
spawn on the closest floor surface below it CINEMATIC - Will spawn with no
default AI (BS_CINEMATIC) NOTSOLID - Starts not solid STARTINSOLID - Don't try
to fix if spawn in solid SHY - Spawner is shy
...
gentity_t *NPC_SpawnType(gentity_t *ent, char *npc_type, char *targetname,
  ...
  if (!npc_type[0]) {
    Com_Printf(S_COLOR_RED
               ...
               "[VEH type (from ext_data/vehicles)]\n");
    return NULL;
  }
  ...
  // rwwFIXMEFIXME: Care about who is issuing this command/other clients besides
  // 0? Spawn it at spot of first player
  // FIXME: will gib them!
  AngleVectors(ent->client->ps.viewangles, forward, NULL, NULL);
  VectorNormalize(forward);
  VectorMA(ent->r.currentOrigin, 64, forward, end);
  trap->Trace(&trace, ent->r.currentOrigin, NULL, NULL, end, 0, MASK_SOLID,
              qfalse, 0, 0);
  VectorCopy(trace.endpos, end);
  end[2] -= 24;
  trap->Trace(&trace, trace.endpos, NULL, NULL, end, 0, MASK_SOLID, qfalse, 0,
              0);
  VectorCopy(trace.endpos, end);
  end[2] += 24;
...
void NPC_Kill_f(void) {
  ...
  if (!name[0]) {
    ...
    Com_Printf(S_COLOR_RED "or\n");
    Com_Printf(S_COLOR_RED "NPC team '[teamname]' - kills all NPCs of a "
                           "certain team ('nonally' is all but your allies)\n");
    return;
  }
  ...
  for (n = 1; n < ENTITYNUM_MAX_NORMAL; n++) {
    ...
    if (killNonSF) {
        ...
        if (player->client) {
          if (player->client->playerTeam != NPCTEAM_PLAYER) {
            ...
            if (player->die && player->client) {
              player->die(player, player, player,
                          player->client->pers.maxHealth, MOD_UNKNOWN);
            }
          }
          ...
          Com_Printf(S_COLOR_GREEN
                     "Removing NPC spawner %s with NPC named %s\n",
      ...
      if (killTeam != NPCTEAM_FREE) {
        if (player->client->playerTeam == killTeam) {
          ...
          player->health = 0;
          if (player->die) {
            player->die(player, player, player, player->client->pers.maxHealth,
                        MOD_UNKNOWN);
          }
        }
    ...
    player->NPC_type, player->targetname ); G_FreeEntity( player );
    }
    */
...
void Cmd_NPC_f(gentity_t *ent) {
  ...
  if (!cmd[0]) {
    ...
    Com_Printf(" kill [NPC targetname] or [all(kills all NPCs)] or 'team "
               "[teamname]'\n");
    ...
    if (!cmd2[0]) { // Show the score for all NPCs
      int i;
      ...
      if ((thisent = G_Find(NULL, FOFS(targetname), cmd2)) != NULL &&
          thisent->client) {
        NPC_PrintScore(thisent);
      } else {
