...
qboolean InFOV(gentity_t *ent, gentity_t *from, int hFOV, int vFOV) {
	...
	if (from->client) {
		if (from->client->NPC_class != CLASS_RANCOR && from->client->NPC_class != CLASS_WAMPA &&
			!VectorCompare(from->client->renderInfo.eyeAngles, vec3_origin)) { // Actual facing of tag_head!
...
static int G_CheckSoundEvents(gentity_t *self, float maxHearDist, int ignoreAlert, qboolean mustHaveOwner, int minAlertLevel, qboolean onGroundOnly) {
	...
	for (int i = 0; i < level.numAlertEvents; i++) {
		...
		if (self->client && self->client->NPC_class !=
								CLASS_SAND_CREATURE) { // sand creatures hear all in within their earshot, regardless of quietness and alert sound radius!
...
int G_CheckAlertEvents(gentity_t *self, qboolean checkSight, qboolean checkSound, float maxSeeDist, float maxHearDist, int ignoreAlert, qboolean mustHaveOwner,
	...
	if (checkSight) {
		...
		if (self->NPC) {
			...
			bestSightEvent =
				G_CheckSightEvents(self, 80, 80, maxSeeDist, ignoreAlert, mustHaveOwner, minAlertLevel); // FIXME: look at cg_view to get more accurate numbers?
		}
		...
		if (!level.alertEvents[alertEvent].owner || !level.alertEvents[alertEvent].owner->client ||
			...
			if (self->NPC) {
				if (self->NPC->scriptFlags & SCF_DONT_FLEE) { // can't flee
					...
					if (level.alertEvents[alertEvent].level >= AEL_DANGER_GREAT || self->s.weapon == WP_NONE ||
					...
					{							// just normal danger and I have a weapon, so just a 25% chance of fleeing only for a few seconds
						...
						NPC_StartFlee(level.alertEvents[alertEvent].owner, level.alertEvents[alertEvent].position, level.alertEvents[alertEvent].level, 1000,
									  3000);
					} else {						  // didn't flee
...
qboolean G_ClearLOS(gentity_t *self, const vec3_t start, const vec3_t end) {
	...
	while (tr.fraction < 1.0 && traceCount < 3) { // can see through 3 panes of glass
		if (tr.entityNum < ENTITYNUM_WORLD) {
			if (&g_entities[tr.entityNum] != NULL && (g_entities[tr.entityNum].svFlags & SVF_GLASS_BRUSH)) { // can see through glass, trace again, ignoring me
	...
	for (i = 0; i < level.numInterestPoints; i++) {
		...
		if (gi.inPVS(level.interestPoints[i].origin, eyes)) {
			...
			if ((fabs(diffVec[0]) + fabs(diffVec[1])) / 2 < 48 &&
				fabs(diffVec[2]) > (fabs(diffVec[0]) + fabs(diffVec[1])) / 2) { // Too close to look so far up or down
				continue;
			}
