...
char *showPowersName[] = {"HEAL2",           // FP_HEAL
                          "JUMP2",           // FP_LEVITATION
                          "SPEED2",          // FP_SPEED
                          "PUSH2",           // FP_PUSH
                          "PULL2",           // FP_PULL
...
// Called from UI shared code. For now we'll just redirect to the normal anim
// load function.

int UI_ParseAnimationFile(const char *filename, animation_t *animset,
                          qboolean isHumanoid) {
...
int MenuFontToHandle(int iMenuFont) {
  switch (iMenuFont) {
  case FONT_SMALL:
    return cgDC.Assets.qhSmallFont;
  case FONT_SMALL2:
    return cgDC.Assets.qhSmall2Font;
  case FONT_MEDIUM:
    return cgDC.Assets.qhMediumFont;
  case FONT_LARGE:
    return cgDC.Assets.qhMediumFont; // cgDC.Assets.qhBigFont;
...
static void CG_DrawZoomMask(void) {
  ...
  //	int ammo =
  //cg_entities[0].gent->client->ps.ammo[weaponData[cent->currentState.weapon].ammoIndex];
  float cx, cy;
  //	int val[5];
  float max, fi;

  // Check for Binocular specific zooming since we'll want to render different
  // bits in each case
  if (cg.predictedPlayerState.zoomMode == 2) {
    ...
    // draw blue tinted distortion mask, trying to make it as small as is
    // necessary to fill in the viewable area
    ...
    // Draw scrolling numbers, use intervals 10 units apart--sorry, this section
    // of code is just kind of hacked
    ...
    for (i = -10; i < 30; i += 10) {
      ...
      // we only want to draw the very far left one some of the time, if it's
      // too far to the left it will
      //	poke outside the mask.
      if ((off > 3.0f && i == -10) || i > -10) {
        // draw the value, but add 200 just to bump the range up...arbitrary, so
        // change it if you like
        CG_DrawNumField(155 + i * 10 + off * 10, 374, 3, val + 200, 24, 14,
                        NUM_FONT_CHUNKY, qtrue);
        CG_DrawPic(245 + (i - 1) * 10 + off * 10, 376, 6, 6,
    ...
    // disruptor zoom mode
    level =
        (float)(50.0f - zoomFov) / 50.0f; //(float)(80.0f - zoomFov) / 80.0f;
    ...
    // Using a magic number to convert the zoom level to a rotation amount that
    // correlates more or less with the zoom artwork.
    level *= 103.0f;
    ...
    // weirdness.....converting ammo to a base five number scale just to be
    // geeky.
    /*		val[0] = ammo % 5;
                    val[1] = (ammo / 5) % 5;
                    val[2] = (ammo / 25) % 5;
                    ...
                    for ( int t = 0; t < 5; t++ )
                    {
                            ...
                            CG_DrawRotatePic2( cx, cy, 24, 38, 45 - t * 10,
       trap->R_RegisterShader( va("gfx/2d/char%d",val[4-t] )));
                    }
    */
    // max = ( cg_entities[0].gent->health / 100.0f );

    if ((cg.snap->ps.eFlags & EF_DOUBLE_AMMO)) {
      max = cg.snap->ps.ammo[weaponData[WP_DISRUPTOR].ammoIndex] /
            ((float)ammoData[weaponData[WP_DISRUPTOR].ammoIndex].max * 2.0f);
    } else {
      max = cg.snap->ps.ammo[weaponData[WP_DISRUPTOR].ammoIndex] /
            (float)ammoData[weaponData[WP_DISRUPTOR].ammoIndex].max;
    }
    if (max > 1.0f) {
      max = 1.0f;
    }
    ...
    for (fi = 18.5f; fi <= 18.5f + max;
         fi += 3) // going from 15 to 45 degrees, with 5 degree increments
    {
    ...
    if (cg.predictedPlayerState.weaponstate == WEAPON_CHARGING_ALT) {
      ...
      max = (cg.time - cg.predictedPlayerState.weaponChargeTime) /
            (50.0f * 30.0f); // bad hardcodedness 50 is disruptor charge unit
                             // and 30 is max charge units allowed.

      if (max > 1.0f) {
        max = 1.0f;
      }

      trap->R_DrawStretchPic(257, 435, 134 * max, 34, 0, 0, max, 1,
                             cgs.media.disruptorChargeShader);
    }
    //		trap->R_SetColor( colorTable[CT_WHITE] );
    //		CG_DrawPic( 0, 0, 640, 480, cgs.media.disruptorMask );
  }
...
void CG_Draw3DModel(float x, float y, float w, float h, qhandle_t model,
                    void *ghoul2, int g2radius, qhandle_t skin, vec3_t origin,
                    vec3_t angles) {
  refdef_t refdef;
  refEntity_t ent;
...
void CG_DrawHealth(menuDef_t *menuHUD) {
  ...
  for (i = (MAX_HUD_TICS - 1); i >= 0; i--) {
    ...
    CG_DrawPic(focusItem->window.rect.x, focusItem->window.rect.y,
               focusItem->window.rect.w, focusItem->window.rect.h,
  ...
  if (focusItem) {
    ...
    CG_DrawNumField(focusItem->window.rect.x, focusItem->window.rect.y, 3,
                    ps->stats[STAT_HEALTH], focusItem->window.rect.w,
...
void CG_DrawArmor(menuDef_t *menuHUD) {
  ...
  if (focusItem) {
    ...
    CG_DrawNumField(focusItem->window.rect.x, focusItem->window.rect.y, 3,
                    ps->stats[STAT_ARMOR], focusItem->window.rect.w,
                    focusItem->window.rect.h, NUM_FONT_SMALL, qfalse);
  }
  ...
  if (ps->stats[STAT_ARMOR]) // Is there armor? Draw the HUD Armor TIC
  {
    ...
    if (ps->stats[STAT_ARMOR] <
        quarterArmor) // Do whatever the flash timer says
    {
...
static void CG_DrawAmmo(centity_t *cent, menuDef_t *menuHUD) {
  ...
  if (weaponData[cent->currentState.weapon].energyPerShot == 0 &&
    ...
    if (focusItem) {
      CG_DrawProportionalString(focusItem->window.rect.x,
                                focusItem->window.rect.y, "--", NUM_FONT_SMALL,
    ...
    if (focusItem) {
      ...
      if ((cent->currentState.eFlags & EF_DOUBLE_AMMO)) {
        inc = (float)(ammoData[weaponData[cent->currentState.weapon].ammoIndex]
                          .max *
                      2.0f) /
              MAX_HUD_TICS;
      } else {
      ...
      CG_DrawNumField(focusItem->window.rect.x, focusItem->window.rect.y, 3,
                      value, focusItem->window.rect.w, focusItem->window.rect.h,
...
void CG_DrawForcePower(menuDef_t *menuHUD) {
  ...
  if (focusItem) {
    ...
    CG_DrawNumField(focusItem->window.rect.x, focusItem->window.rect.y, 3,
                    cg.snap->ps.fd.forcePower, focusItem->window.rect.w,
                    focusItem->window.rect.h, NUM_FONT_SMALL, qfalse);
  }
}
...
static void CG_DrawSimpleSaberStyle(const centity_t *cent) {
  ...
  CG_DrawProportionalString(
      SCREEN_WIDTH - (weapX + 16 + 32), (SCREEN_HEIGHT - 80) + 40, num,
...
static void CG_DrawSimpleAmmo(const centity_t *cent) {
  ...
  // No ammo
  if (currValue < 0 ||
      (weaponData[cent->currentState.weapon].energyPerShot == 0 &&
       weaponData[cent->currentState.weapon].altEnergyPerShot == 0)) {
    CG_DrawProportionalString(
        SCREEN_WIDTH - (16 + 32), (SCREEN_HEIGHT - 80) + 40, "--",
  ...
  CG_DrawProportionalString(SCREEN_WIDTH - (16 + 32), (SCREEN_HEIGHT - 80) + 40,
                            num, UI_SMALLFONT | UI_DROPSHADOW,
...
static void CG_DrawSimpleForcePower(const centity_t *cent) {
  ...
  CG_DrawProportionalString(
      SCREEN_WIDTH - (18 + 14 + 32), (SCREEN_HEIGHT - 80) + 40 + 14, num,
      UI_SMALLFONT | UI_DROPSHADOW, colorTable[calcColor]);
}
...
void CG_DrawHUD(centity_t *cent) {
  ...
  if (cg_hudFiles.integer) {
    ...
    if (cg.predictedPlayerState.pm_type != PM_SPECTATOR) {
      CG_DrawProportionalString(
          x + 16, y + 40, va("%i", cg.snap->ps.stats[STAT_HEALTH]),
          UI_SMALLFONT | UI_DROPSHADOW, colorTable[CT_HUD_RED]);

      CG_DrawProportionalString(
          x + 18 + 14, y + 40 + 14, va("%i", cg.snap->ps.stats[STAT_ARMOR]),
  ...
  if (cg.predictedPlayerState.pm_type != PM_SPECTATOR) {
    ...
    if (menuHUD) {
      ...
      // trap->Error( ERR_DROP, "CG_ChatBox_ArrayInsert: unable to locate HUD
      // menu file ");
    }

    // scoreStr = va("Score: %i", cgs.clientinfo[cg.snap->ps.clientNum].score);
    if (cgs.gametype == GT_DUEL) { // A duel that requires more than one kill to
                                   // knock the current enemy back to the queue
      // show current kills out of how many needed
      scoreStr = va("%s: %i/%i", CG_GetStringEdString("MP_INGAME", "SCORE"),
                    cg.snap->ps.persistant[PERS_SCORE], cgs.fraglimit);
    } else if (0 &&
               cgs.gametype <
                   GT_TEAM) { // This is a teamless mode, draw the score bias.
      ...
      if (scoreBias == 0) {     // We are the leader!
        if (cgs.scores2 <= 0) { // Nobody to be ahead of yet.
          ...
          if (scoreBias == 0) {
            ...
            Com_sprintf(scoreBiasStr, sizeof(scoreBiasStr), " (+%d)",
                        scoreBias);
          }
        }
      } else // if (scoreBias < 0)
      ...
      scoreStr = va("%s: %i%s", CG_GetStringEdString("MP_INGAME", "SCORE"),
                    cg.snap->ps.persistant[PERS_SCORE], scoreBiasStr);
    } else { // Don't draw a bias.
      scoreStr = va("%s: %i", CG_GetStringEdString("MP_INGAME", "SCORE"),
      ...
      if (cgs.gametype != GT_POWERDUEL) {
        ...
        if (focusItem) {
          CG_DrawScaledProportionalString(
              focusItem->window.rect.x, focusItem->window.rect.y, scoreStr,
              UI_RIGHT | UI_DROPSHADOW, focusItem->window.foreColor, 0.7f);
        }
      }
...
void CG_DrawForceSelect(void) {
  ...
  if ((cg.forceSelectTime + WEAPON_SELECT_TIME) <
      cg.time) // Time is up for the HUD to display
  {
  ...
  if (ForcePower_Valid(cg.forceSelect)) {
    ...
    if (cgs.media.forcePowerIcons[cg.forceSelect]) {
      CG_DrawPic(x - (bigIconSize / 2),
                 (y - ((bigIconSize - smallIconSize) / 2)) + yOffset,
                 bigIconSize, bigIconSize,
                 cgs.media.forcePowerIcons[cg.forceSelect]); // only cache the
                                                             // icon for display
    }
  }
  ...
  for (iconCnt = 1; iconCnt < (sideRightIconCnt + 1); i++) {
    ...
    if (cgs.media.forcePowerIcons[forcePowerSorted[i]]) {
      CG_DrawPic(
          holdX, y + yOffset, smallIconSize, smallIconSize,
          cgs.media.forcePowerIcons[forcePowerSorted[i]]); // only cache the
                                                           // icon for display
      holdX += (smallIconSize + pad);
    }
  }
...
void CG_DrawInvenSelect(void) {
  ...
  if ((cg.invenSelectTime + WEAPON_SELECT_TIME) <
      cg.time) // Time is up for the HUD to display
  {
    return;
  }
  ...
  if (!cg.snap->ps.stats[STAT_HOLDABLE_ITEM] ||
      !cg.snap->ps.stats[STAT_HOLDABLE_ITEMS]) {
    return;
  }
  ...
  if (!count) {
    y2 = 0; // err?
    CG_DrawProportionalString(320, y2 + 22, "EMPTY INVENTORY",
  ...
  for (iconCnt = 0; iconCnt < sideLeftIconCnt; i--) {
    ...
    if (cgs.media.invenIcons[i]) {
      ...
      CG_DrawPic(holdX, y + 10, smallIconSize, smallIconSize,
                 cgs.media.invenIcons[i]);

      trap->R_SetColor(colorTable[CT_ICON_BLUE]);
      /*CG_DrawNumField (holdX + addX, y + smallIconSize, 2,
         cg.snap->ps.inventory[i], 6, 12, NUM_FONT_SMALL,qfalse);
              */
  ...
  if (cgs.media.invenIcons[cg.itemSelect] &&
    ...
    trap->R_SetColor(NULL);
    CG_DrawPic(x - (bigIconSize / 2),
               (y - ((bigIconSize - smallIconSize) / 2)) + 10, bigIconSize,
               bigIconSize, cgs.media.invenIcons[cg.itemSelect]);
    //	addX = (float) bigIconSize * .75;
    trap->R_SetColor(colorTable[CT_ICON_BLUE]);
    /*CG_DrawNumField ((x-(bigIconSize/2)) + addX, y, 2,
       cg.snap->ps.inventory[cg.inventorySelect], 6, 12,
            NUM_FONT_SMALL,qfalse);*/
    ...
    if (bg_itemlist[itemNdex].classname) {
      ...
      if (trap->SE_GetStringTextString(va("SP_INGAME_%s", Q_strupr(upperKey)),
                                       text, sizeof(text))) {
        CG_DrawProportionalString(320, y + 45, text, UI_CENTER | UI_SMALLFONT,
                                  textColor);
      } else {
        CG_DrawProportionalString(320, y + 45, bg_itemlist[itemNdex].classname,
  ...
  *alpha = 1.0f;
  ...
  if (cg.predictedPlayerState.rocketLockIndex < ENTITYNUM_WORLD) {
    targetNum = cg.predictedPlayerState.rocketLockIndex;
             ...
             cg.time - cg.crosshairVehTime <
                 3000) { // crosshair was on a vehicle in the last 3 seconds
  ...
  if (targetNum < MAX_CLIENTS) { // real client
    if (cg_entities[targetNum].currentState.m_iVehicleNum >=
        MAX_CLIENTS) { // in a vehicle
  ...
  if (targetNum < ENTITYNUM_WORLD && targetNum >= MAX_CLIENTS) {
    ...
    if (targetVeh->currentState.NPC_class == CLASS_VEHICLE &&
        targetVeh->m_pVehicle && targetVeh->m_pVehicle->m_pVehicleInfo &&
        targetVeh->m_pVehicle->m_pVehicleInfo->type ==
            VH_FIGHTER) { // it's a vehicle
...
float CG_DrawVehicleShields(const menuDef_t *menuHUD, const centity_t *veh) {
  ...
  if (item) {
    ...
    CG_DrawPic(item->window.rect.x, item->window.rect.y, item->window.rect.w,
  ...
  // Look at the amount of health left and show only as much of the graphic as
  // there is health. Use alpha to fade out partial section of health
...
// The HUD.menu file has the graphic print with a negative height, so it will
...
void CG_DrawVehicleWeaponsLinked(const menuDef_t *menuHUD,
  ...
  if (veh->m_pVehicle && veh->m_pVehicle->m_pVehicleInfo &&
      (veh->m_pVehicle->m_pVehicleInfo->weapon[0].linkable == 2 ||
       veh->m_pVehicle->m_pVehicleInfo->weapon[1].linkable ==
           2)) { // weapon is always linked
    drawLink = qtrue;
  } else {
    // MP way:
  ...
  if (cg_drawLink != drawLink) { // state changed, play sound
    ...
    trap->S_StartSound(
        NULL, cg.predictedPlayerState.clientNum, CHAN_LOCAL,
  ...
  if (drawLink) {
    ...
    if (item) {
      ...
      CG_DrawPic(item->window.rect.x, item->window.rect.y, item->window.rect.w,
                 item->window.rect.h, cgs.media.whiteShader);
    }
  }
}
...
void CG_DrawVehicleDamage(const centity_t *veh, int brokenLimbs,
                          const menuDef_t *menuHUD, float alpha, int index) {
  itemDef_t *item;
  int colorI;
  vec4_t color;
  int graphicHandle = 0;
  ...
  item =
      Menu_FindItemByName((menuDef_t *)menuHUD, vehDamageData[index].itemName);
  if (item) {
    ...
    if (graphicHandle) {
      CG_DrawPic(item->window.rect.x, item->window.rect.y, item->window.rect.w,
                 item->window.rect.h, graphicHandle);
    }
  }
}

// Used on both damage indicators :  player vehicle and the vehicle the player
// is locked on
void CG_DrawVehicleDamageHUD(const centity_t *veh, int brokenLimbs,
                             float percShields, char *menuName, float alpha) {
  menuDef_t *menuHUD;
  itemDef_t *item;
  vec4_t color;
  ...
  if (item) {
    if (veh->m_pVehicle->m_pVehicleInfo->dmgIndicBackgroundHandle) {
      if (veh->damageTime > cg.time) { // ship shields currently taking damage
        // NOTE: cent->damageAngle can be accessed to get the direction from the
        // ship origin to the impact point (in 3-D space)
        float perc =
            1.0f - ((veh->damageTime - cg.time) / 2000.0f /*MIN_SHIELD_TIME*/);
        if (perc < 0.0f) {
          perc = 0.0f;
        } else if (perc > 1.0f) {
          perc = 1.0f;
        ...
        color[1] = item->window.foreColor[1] *
                   perc; // fade other colors back in over time
  ...
  // TODO: if we check nextState.brokenLimbs & prevState.brokenLimbs, we can
  // tell when a damage flag has been added and flash that part of the ship
...
qboolean CG_DrawVehicleHud(const centity_t *cent) {
  ...
  if (veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID &&
      !veh->m_pVehicle->m_pVehicleInfo->weapon[1].ID) {
  ...
  if (veh->m_pVehicle->m_pVehicleInfo->hideRider) {
    CG_DrawVehicleDamageHUD(veh, cg.predictedVehicleState.brokenLimbs,
                            shieldPerc, "vehicledamagehud", 1.0f);

    // Has he targeted an enemy?
    if (CG_CheckTargetVehicle(&veh, &alpha)) {
      CG_DrawVehicleDamageHUD(
          veh, veh->currentState.brokenLimbs,
          ((float)veh->currentState.activeForcePass / 10.0f),
          "enemyvehicledamagehud", alpha);
    }
...
static float CG_DrawMiniScoreboard(float y) {
  ...
  if (cgs.gametype >= GT_TEAM) {
    Q_strncpyz(temp, va("%s: ", CG_GetStringEdString("MP_INGAME", "RED")),
               sizeof(temp));
    Q_strcat(temp, sizeof(temp),
             cgs.scores1 == SCORE_NOT_PRESENT ? "-" : (va("%i", cgs.scores1)));
    Q_strcat(temp, sizeof(temp),
             va(" %s: ", CG_GetStringEdString("MP_INGAME", "BLUE")));
    Q_strcat(temp, sizeof(temp),
             cgs.scores2 == SCORE_NOT_PRESENT ? "-" : (va("%i", cgs.scores2)));

    CG_Text_Paint(630 - CG_Text_Width(temp, 0.7f, FONT_MEDIUM) + xOffset, y,
                  0.7f, colorWhite, temp, 0, 0, ITEM_TEXTSTYLE_SHADOWEDMORE,
                  FONT_MEDIUM);
    y += 15;
  } else {
    /*
    strcpy ( temp, "1st: " );
    Q_strcat ( temp, MAX_QPATH,
    cgs.scores1==SCORE_NOT_PRESENT?"-":(va("%i",cgs.scores1)) );
    ...
    CG_Text_Paint( 630 - CG_Text_Width ( temp, 0.7f, FONT_SMALL ), y, 0.7f,
    colorWhite, temp, 0, 0, ITEM_TEXTSTYLE_SHADOWEDMORE, FONT_MEDIUM ); y += 15;
    */
    // rww - no longer doing this. Since the attacker now shows who is first, we
    // print the score there.
  }
...
static float CG_DrawEnemyInfo(float y) {
  ...
  if (cgs.gametype == GT_JEDIMASTER) {
    ...
    if (clientNum < 0) {
      ...
      CG_DrawPic(640 - size - 12 + xOffset, y, size, size,
                 cgs.media.weaponIcons[WP_SABER]);

      y += size;

      /*
      CG_Text_Paint( 630 - CG_Text_Width ( ci->name, 0.7f, FONT_MEDIUM ), y,
      0.7f, colorWhite, ci->name, 0, 0, 0, FONT_MEDIUM ); y += 15;
      */

      CG_Text_Paint(630 - CG_Text_Width(title, 0.7f, FONT_MEDIUM) + xOffset, y,
                    0.7f, colorWhite, title, 0, 0, 0, FONT_MEDIUM);
    ...
    if (cg.snap->ps.clientNum == cgs.duelist1) {
      clientNum = cgs.duelist2; // if power duel, should actually draw both
                                // duelists 2 and 3 I guess
    ...
    */
    // As of current, we don't want to draw the attacker. Instead, draw whoever
    // is in first place.
    ...
    title =
        va("%s: %i", CG_GetStringEdString("MP_INGAME", "LEADER"), cgs.scores1);
  ...
  //	CG_Text_Paint( 630 - CG_Text_Width ( ci->name, 0.7f, FONT_MEDIUM ) +
  //xOffset, y, 0.7f, colorWhite, ci->name, 0, 0, 0, FONT_MEDIUM );
  CG_Text_Paint(630 - CG_Text_Width(ci->name, 1.0f, FONT_SMALL2) + xOffset, y,
                1.0f, colorWhite, ci->name, 0, 0, 0, FONT_SMALL2);

  y += 15;
  //	CG_Text_Paint( 630 - CG_Text_Width ( title, 0.7f, FONT_MEDIUM ) +
  //xOffset, y, 0.7f, colorWhite, title, 0, 0, 0, FONT_MEDIUM );
  CG_Text_Paint(630 - CG_Text_Width(title, 1.0f, FONT_SMALL2) + xOffset, y,
                1.0f, colorWhite, title, 0, 0, 0, FONT_SMALL2);

  if ((cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL) &&
      cgs.clientinfo[cg.snap->ps.clientNum].team !=
          TEAM_SPECTATOR) { // also print their score
    char text[1024];
    y += 15;
    Com_sprintf(text, sizeof(text), "%i/%i", cgs.clientinfo[clientNum].score,
                cgs.fraglimit);
    CG_Text_Paint(630 - CG_Text_Width(text, 0.7f, FONT_MEDIUM) + xOffset, y,
                  0.7f, colorWhite, text, 0, 0, 0, FONT_MEDIUM);
  }
...
void CG_DrawHealthBarRough(float x, float y, int width, int height, float ratio,
  ...
  CG_DrawRect(x + 1, y + height / 2 - 1, midpoint, 1, height / 4 + 1,
              color1); // creme-y filling.
  CG_DrawRect(x + midpoint, y + height / 2 - 1, remainder, 1, height / 4 + 1,
              color3);                         // used-up-ness.
...
void CG_DrawDuelistHealth(float x, float y, float w, float h, int duelist) {
  ...
  CG_DrawHealthBarRough(x, y, w, h, ratio, duelHealthColor,
                        colorTable[CT_WHITE]); // new art for this?  I'm not
                                               // crazy about how this looks.
}
...
float CG_DrawRadar(float y) {
  ...
  color[3] = 0.6f;
  trap->R_SetColor(color);
  CG_DrawPic(RADAR_X + xOffset, y, RADAR_RADIUS * 2, RADAR_RADIUS * 2,
             cgs.media.radarShader);

  // Always green for your own team.
  VectorCopy(g_color_table[ColorIndex(COLOR_GREEN)], teamColor);
  teamColor[3] = 1.0f;

  // Draw all of the radar entities.  Draw them backwards so players are drawn
  // last
  for (i = cg.radarEntityCount - 1; i >= 0; i--) {
    ...
    if (distance > cg_radarRange * 0.8f) {
      if ((cent->currentState.eFlags &
           EF_RADAROBJECT) // still want to draw the direction
          || (cent->currentState.eType ==
                  ET_NPC // FIXME: draw last, with players...
    ...
    default: {
      ...
      // we want to scale the thing up/down based on the relative Z (up/down)
      // positioning
      if (cent->lerpOrigin[2] >
          cg.predictedPlayerState
              .origin[2]) { // higher, scale up (between 16 and 24)
        ...
        zScale += dif;
      } else if (cent->lerpOrigin[2] <
                 cg.predictedPlayerState
                     .origin[2]) { // lower, scale down (between 16 and 8)
      ...
      if (cent->currentState
              .brokenLimbs) { // slightly misleading to use this value, but
                              // don't want to add more to entstate.
        ...
        trap->Cvar_VariableStringBuffer(va("team%i_objective%i",
        ...
        if (!complete) {
          ...
          if (cent->currentState.genericenemyindex &&
              cent->currentState.genericenemyindex < MAX_ICONS) {
            color[0] = color[1] = color[2] = color[3] = 1.0f;
            shader = cgs.gameIcons[cent->currentState.genericenemyindex];
          } else {
            if (cg.snap && cent->currentState.brokenLimbs ==
      ...
      if (shader) {
        // Pulse the alpha if time2 is set.  time2 gets set when the entity
        // takes pain
        if ((cent->currentState.time2 &&
             cg.time - cent->currentState.time2 < 5000) ||
        ...
        trap->R_SetColor(color);
        CG_DrawPic(x - 4 + xOffset, ly - 4, arrowBaseScale, arrowBaseScale,
                   shader);
      }
    } break;
    ...
    case ET_NPC: // FIXME: draw last, with players...
      if (cent->currentState.NPC_class == CLASS_VEHICLE &&
        ...
        if (cent->m_pVehicle &&
          ...
          x = (float)RADAR_X + (float)RADAR_RADIUS +
          ...
          // we want to scale the thing up/down based on the relative Z
          // (up/down) positioning
          if (cent->lerpOrigin[2] >
              cg.predictedPlayerState
                  .origin[2]) { // higher, scale up (between 16 and 24)
            float dif =
                (cent->lerpOrigin[2] - cg.predictedPlayerState.origin[2]);
            ...
            zScale += dif;
                     ...
                     cg.predictedPlayerState
                         .origin[2]) { // lower, scale down (between 16 and 8)
            float dif =
                (cg.predictedPlayerState.origin[2] - cent->lerpOrigin[2]);
          ...
          if (cent->currentState.m_iVehicleNum // vehicle has a driver
            ...
            if (cgs.clientinfo[cent->currentState.m_iVehicleNum - 1].team ==
                local->team) {
    ...
    case ET_MOVER:
      if (cent->currentState.speed // the mover's size, actually
          ...
          cg.predictedPlayerState.m_iVehicleNum) { // a mover that's close to me
        ...
        if (surfaceDist < RADAR_MIN_ASTEROID_SURF_WARN_DIST) { // always warn!
          ...
          float newDist;
          for (predictTime = timeStep; predictTime < 5000;
               predictTime += timeStep) {
            // asteroid dir, speed, size, + my dir & speed...
            BG_EvaluateTrajectory(&cent->currentState.pos,
                                  cg.time + predictTime, asteroidPos);
            // FIXME: I don't think it's calcing "myPos" correctly
            AngleVectors(cg.predictedVehicleState.viewangles, moveDir, NULL,
                         NULL);
            VectorMA(cg.predictedVehicleState.origin,
                     cg.predictedVehicleState.speed * predictTime / 1000.0f,
                     moveDir, myPos);
            newDist = Distance(myPos, asteroidPos);
            if ((newDist - cent->currentState.speed) <=
                RADAR_MIN_ASTEROID_SURF_WARN_DIST) // 200.0f )
        ...
        if (mayImpact) { // possible collision
          ...
          float ly;
          float asteroidScale =
              (cent->currentState.speed / 2000.0f); // average asteroid radius?
          ...
          if (impactSoundDebounceTime < cg.time) {
            ...
            VectorMA(cg.refdef.vieworg,
                     -500.0f * (surfaceDist / RADAR_ASTEROID_RANGE), dirPlayer,
                     soundOrg);
            trap->S_StartSound(
                soundOrg, ENTITYNUM_WORLD, CHAN_AUTO,
          ...
          if ((cg.time - impactSoundDebounceTime) > 100) {
            asteroidColor[3] =
                (float)((cg.time - impactSoundDebounceTime) - 100) / 900.0f;
          }
    ...
    case ET_MISSILE:
      if ( // cent->currentState.weapon == WP_ROCKET_LAUNCHER &&//a rocket
          ...
          && cg_entities[cent->currentState.owner].currentState.NPC_class ==
                 CLASS_VEHICLE) { // a rocket belonging to an NPC, FIXME: only
                                  // tracking rockets!
        float x;
        float ly;
        ...
        if (cg.predictedPlayerState.m_iVehicleNum) { // I'm in a vehicle
          ...
          if (cent->currentState.otherEntityNum ==
            ...
            if (radarLockSoundDebounceTime < cg.time) {
              ...
              VectorMA(cg.refdef.vieworg,
                       -500.0f * (actualDist / RADAR_MISSILE_RANGE), dirPlayer,
                       soundOrg);
        ...
        if (cent->currentState.owner >= MAX_CLIENTS // missile owned by an NPC
            && cg_entities[cent->currentState.owner].currentState.NPC_class ==
                   CLASS_VEHICLE // NPC is a vehicle
            &&
            cg_entities[cent->currentState.owner].currentState.m_iVehicleNum <=
                MAX_CLIENTS // Vehicle has a player driver
            && cgs.clientinfo[cg_entities[cent->currentState.owner]
                                  .currentState.m_iVehicleNum -
                              1]
                   .infoValid) // player driver is valid
        {
          cl = &cgs.clientinfo[cg_entities[cent->currentState.owner]
                                   .currentState.m_iVehicleNum -
                               1];
    ...
    case ET_PLAYER: {
      ...
      if (cent->lerpOrigin[2] >
          cg.predictedPlayerState
              .origin[2]) { // higher, scale up (between 16 and 32)
        float dif = (cent->lerpOrigin[2] - cg.predictedPlayerState.origin[2]);
      ...
      CG_DrawRotatePic2(
          RADAR_X + RADAR_RADIUS + sin(angle) * distance + xOffset,
          y + RADAR_RADIUS + cos(angle) * distance, arrow_w, arrow_h,
          (360 - cent->lerpAngles[YAW]) +
  ...
  CG_DrawRotatePic2(RADAR_X + RADAR_RADIUS + xOffset, y + RADAR_RADIUS, arrow_w,
                    arrow_h, 0, cgs.media.mAutomapPlayerIcon);
...
static float CG_DrawTeamOverlay(float y, qboolean right, qboolean upper) {
  ...
  // TODO: On basejka servers, we won't have valid teaminfo if we're spectating
  // someone. 		Find a way to detect invalid info and return early?
  ...
  for (i = 0; i < count; i++) {
    ...
    if (ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM]) {
      ...
      CG_DrawStringExt(xx + xOffset, y, ci->name, hcolor, qfalse, qfalse,
      ...
      if (lwidth) {
        ...
        CG_DrawStringExt(xx + xOffset, y, p, hcolor, qfalse, qfalse,
                         TINYCHAR_WIDTH, TINYCHAR_HEIGHT,
      ...
      CG_DrawStringExt(xx + xOffset, y, st, hcolor, qfalse, qfalse,
  ...
  for (j = 0; j < PW_NUM_POWERUPS; j++) {
    if (cg.snap->ps.powerups[j] > cg.time) {
      ...
      if (item) {
        ...
        CG_DrawPic((640 - (ico_size * 1.1)) + xOffset, y, ico_size, ico_size,
                   icoShader);
        ...
        if (j != PW_REDFLAG && j != PW_BLUEFLAG && secondsleft < 999) {
          CG_DrawProportionalString(
              (640 - (ico_size * 1.1)) + (ico_size / 2) + xOffset, y - 8,
...
static void CG_DrawUpperRight(void) {
  ...
  if ((cgs.gametype >= GT_TEAM || cg.predictedPlayerState.m_iVehicleNum) &&
      cg_drawRadar.integer) { // draw Radar in Siege mode or when in a vehicle
                              // of any kind
    y = CG_DrawRadar(y);
  }
...
static void CG_DrawDisconnect(void) {
  ...
  if (cg.mMapChange) {
    s = CG_GetStringEdString(
        "MP_INGAME", "SERVER_CHANGING_MAPS"); // s = "Server Changing Maps";
    w = CG_DrawStrlen(s) * BIGCHAR_WIDTH;
    CG_DrawBigString(320 - w / 2, 100, s, 1.0F);

    s = CG_GetStringEdString("MP_INGAME",
                             "PLEASE_WAIT"); // s = "Please wait...";
    w = CG_DrawStrlen(s) * BIGCHAR_WIDTH;
  ...
  s = CG_GetStringEdString(
      "MP_INGAME", "CONNECTION_INTERRUPTED"); // s = "Connection Interrupted";
                                              // // bk 010215 - FIXME
  w = CG_DrawStrlen(s) * BIGCHAR_WIDTH;
  CG_DrawBigString(320 - w / 2, 100, s, 1.0F);
...
static void CG_DrawLagometer(void) {
  ...
  for (a = 0; a < aw; a++) {
    ...
    if (v > 0) {
      ...
      trap->R_DrawStretchPic(ax + aw - a, mid - v, 1, v, 0, 0, 0, 0,
                             cgs.media.whiteShader);
    } else if (v < 0) {
      if (color != 2) {
        color = 2;
      ...
      trap->R_DrawStretchPic(ax + aw - a, mid, 1, v, 0, 0, 0, 0,
                             cgs.media.whiteShader);
    }
  }
  ...
  for (a = 0; a < aw; a++) {
    ...
    if (v > 0) {
      ...
      trap->R_DrawStretchPic(ax + aw - a, ay + ah - v, 1, v, 0, 0, 0, 0,
                             cgs.media.whiteShader);
    } else if (v < 0) {
      if (color != 4) {
      ...
      trap->R_DrawStretchPic(ax + aw - a, ay + ah - range, 1, range, 0, 0, 0, 0,
                             cgs.media.whiteShader);
    }
  }
...
void CG_CenterPrint(const char *str, int y, int charWidth) {
  ...
  while (*s) {
    //[BugFix19]
    i++;
    if (i >= 50) { // maxed out a line of text, this will make the line spill
...
===================
*/
qboolean BG_IsWhiteSpace(char c) { // this function simply checks to see if the
                                   // given character is whitespace.
  if (c == ' ' || c == '\n' || c == '\0')
    return qtrue;
...
static void CG_DrawCenterString(void) {
  ...
  while (1) {
    ...
    //[BugFix19]
    if (!BG_IsWhiteSpace(start[l]) &&
        !BG_IsWhiteSpace(
            linebuffer[l - 1])) { // we might have cut a word off, attempt to
                                  // find a spot where we won't cut words off at.
      int savedL = l;
      int counter = l - 2;

      for (; counter >= 0; counter--) {
        if (BG_IsWhiteSpace(start[counter])) { // this location is whitespace,
                                               // line break from this position
          linebuffer[counter] = 0;
          l = counter + 1;
          break;
        }
      }
      if (counter < 0) { // couldn't find a break in the text, just go ahead and
                         // cut off the word mid-word.
        l = savedL;
      }
    }
    //[/BugFix19]
    ...
    //[BugFix19]
    // this method of advancing to new line from the start of the array was
    // causing long lines without new lines to be totally truncated.
    if (start[l] && start[l] == '\n') { // next char is a newline, advance past
      l++;
    }

    if (!start[l]) { // end of string, we're done.
      break;
    }
...
void CG_DrawSiegeInfo(centity_t *cent, float chX, float chY, float chW,
                      float chH) {
  ...
  if ((cg.time - se->lastUpdated) >
      10000) { // if you haven't received a status update on this guy in 10
               // seconds, forget about it
    return;
  }

  if (cent->currentState.eFlags &
      EF_DEAD) { // he's dead, don't display info on him
    return;
  }

  if (cent->currentState.weapon !=
      se->weapon) { // data is invalidated until it syncs back again
    return;
  }

  ci = &cgs.clientinfo[cent->currentState.number];
  if (ci->team !=
      cg.predictedPlayerState.persistant[PERS_TEAM]) { // not on the same team
    return;
  }

  configstring =
      CG_ConfigString(cg.predictedPlayerState.clientNum + CS_PLAYERS);
  ...
  if (!(siegeClass->classflags &
        (1 << CFL_STATVIEWER))) { // doesn't really have the ability to see
                                  // others' stats
    return;
  }
  ...
  CG_FillRect(x + percent, y + 1.0f, HEALTH_WIDTH - percent - 1.0f,
              HEALTH_HEIGHT - 1.0f, cColor);
  ...
  if (!weaponData[cent->currentState.weapon].energyPerShot &&
      !weaponData[cent->currentState.weapon]
           .altEnergyPerShot) { // a weapon that takes no ammo, so show full
...
void CG_DrawHealthBar(centity_t *cent, float chX, float chY, float chW,
                      float chH) {
  vec4_t aColor;
  vec4_t cColor;
  ...
  float percent =
      ((float)cent->currentState.health / (float)cent->currentState.maxhealth) *
      HEALTH_WIDTH;
  if (percent <= 0) {
    return;
  }
  ...
  if (!cent->currentState.teamowner ||
      cgs.gametype < GT_TEAM) { // not owned by a team or teamplay
    aColor[0] = 1.0f;
    aColor[1] = 1.0f;
    aColor[2] = 0.0f;
    aColor[3] = 0.4f;
             ...
             cg.predictedPlayerState
                 .persistant[PERS_TEAM]) { // owned by my team
    aColor[0] = 0.0f;
    aColor[1] = 1.0f;
    aColor[2] = 0.0f;
    aColor[3] = 0.4f;
...
void CG_DrawHaqrBar(float chX, float chY, float chW, float chH) {
  ...
  float y = (chY + chH) + 8.0f;
  float percent =
      (((float)cg.predictedPlayerState.hackingTime - (float)cg.time) /
       (float)cg.predictedPlayerState.hackingBaseTime) *
      HEALTH_WIDTH;
  ...
  CG_DrawPic(x, y - HEALTH_WIDTH, HEALTH_WIDTH, HEALTH_WIDTH,
...
void CG_DrawGenericTimerBar(void) {
  ...
  float percent =
      ((float)(cg_genericTimerBar - cg.time) / (float)cg_genericTimerDur) *
      CGTIMERBAR_H;
  ...
  // now draw the part to show how much health there is in the color specified
  CG_FillRect(x + 1.0f, y + 1.0f + (CGTIMERBAR_H - percent),
              CGTIMERBAR_W - 2.0f,
              CGTIMERBAR_H - 1.0f - (CGTIMERBAR_H - percent), aColor);

  // then draw the other part greyed out
  CG_FillRect(x + 1.0f, y + 1.0f, CGTIMERBAR_W - 2.0f, CGTIMERBAR_H - percent,
              cColor);
}
...
void CG_LerpCrosshairPos(float *x, float *y) {
  if (cg_crosshairPrevPosX) { // blend from old pos
  ...
  if (cg_crosshairPrevPosY) { // blend from old pos
...
static void CG_DrawCrosshair(vec3_t worldPoint, int chEntValid) {
  ...
  if (cg.predictedPlayerState.zoomMode != 0) { // not while scoped
    return;
  }
  ...
  if (cg_crosshairHealth.integer) {
    ...
    // rwwFIXMEFIXME: Write this a different way, it's getting a bit too sloppy
    // looking
    else if (chEntValid &&
             (cg_entities[cg.crosshairClientNum].currentState.number <
                  MAX_CLIENTS ||
              cg_entities[cg.crosshairClientNum].currentState.eType == ET_NPC ||
              cg_entities[cg.crosshairClientNum].currentState.shouldtarget ||
              cg_entities[cg.crosshairClientNum]
                  .currentState.health || // always show ents with health data
                                          // under crosshair
              (cg_entities[cg.crosshairClientNum].currentState.eType ==
                   ET_MOVER &&
               cg_entities[cg.crosshairClientNum].currentState.bolt1 &&
               cg.predictedPlayerState.weapon == WP_SABER) ||
              (cg_entities[cg.crosshairClientNum].currentState.eType ==
                   ET_MOVER &&
               cg_entities[cg.crosshairClientNum].currentState.teamowner))) {
      crossEnt = &cg_entities[cg.crosshairClientNum];

      if (crossEnt->currentState.powerups &
          (1 << PW_CLOAKED)) { // don't show up for cloaked guys
        ecolor[0] = 1.0;       // R
        ecolor[1] = 1.0;       // G
        ecolor[2] = 1.0;       // B
      } else if (crossEnt->currentState.number < MAX_CLIENTS) {
        if (cgs.gametype >= GT_TEAM &&
            cgs.clientinfo[crossEnt->currentState.number].team ==
                cgs.clientinfo[cg.snap->ps.clientNum].team) {
          // Allies are green
          ecolor[0] = 0.0; // R
          ecolor[1] = 1.0; // G
          ...
          if (cgs.gametype == GT_POWERDUEL &&
              cgs.clientinfo[crossEnt->currentState.number].duelTeam ==
                  cgs.clientinfo[cg.snap->ps.clientNum]
                      .duelTeam) { // on the same duel team in powerduel, so
                                   // he's a friend
            ecolor[0] = 0.0; // R
            ecolor[1] = 1.0; // G
            ecolor[2] = 0.0; // B
        ...
        if (cg.snap->ps.duelInProgress) {
          if (crossEnt->currentState.number !=
              cg.snap->ps.duelIndex) { // grey out crosshair for everyone but
                                       // your foe if you're in a duel
            ecolor[0] = 0.4f;
            ecolor[1] = 0.4f;
            ecolor[2] = 0.4f;
          }
        } else if (crossEnt->currentState
                       .bolt1) { // this fellow is in a duel. We just checked if
                                 // we were in a duel above, so this means we
                                 // aren't and he is. Which of course means our
                                 // crosshair greys out over him.
          ecolor[0] = 0.4f;
          ecolor[1] = 0.4f;
          ecolor[2] = 0.4f;
        }
        ...
        if (crossEnt->currentState.eType == ET_NPC) {
          ...
          if (crossEnt->currentState.powerups & (1 << PW_CLOAKED)) {
            ...
            ecolor[1] = 1.0f;                             // G
            ecolor[2] = 1.0f;                             // B
          } else if (!crossEnt->currentState.teamowner) { // not on a team
            if (!crossEnt->currentState.teamowner ||
                crossEnt->currentState.NPC_class == CLASS_VEHICLE) { // neutral
              if (crossEnt->currentState.owner <
                  MAX_CLIENTS) { // base color on who is pilotting this thing
                clientInfo_t *ci =
                    &cgs.clientinfo[crossEnt->currentState.owner];

                if (cgs.gametype >= GT_TEAM &&
                    ci->team == cg.predictedPlayerState
                                    .persistant[PERS_TEAM]) { // friendly
                  ecolor[0] = 0.0f;                           // R
                  ecolor[1] = 1.0f;                           // G
                  ecolor[2] = 0.0f;                           // B
                } else {                                      // hostile
          ...
          if (cgs.gametype < GT_TEAM) { // not teamplay, just neutral then
            ecolor[0] = 1.0f;           // R
            ecolor[1] = 1.0f;           // G
            ecolor[2] = 0.0f;           // B
          } else if (crossEnt->currentState.teamowner !=
                     cgs.clientinfo[cg.snap->ps.clientNum]
                         .team) { // on the enemy team
            ecolor[0] = 1.0f;     // R
            ecolor[1] = 0.0f;     // G
            ecolor[2] = 0.0f;     // B
          } else {                // on my team
      ...
      } else if (crossEnt->currentState.eType == ET_MOVER &&
                 crossEnt->currentState.bolt1 &&
                 cg.predictedPlayerState.weapon ==
                     WP_SABER) { // can push/pull this mover. Only show it if
                                 // we're using the saber.
        ecolor[0] = 0.2f;
        ecolor[1] = 0.5f;
        ecolor[2] = 1.0f;
        ...
        corona = qtrue;
                 ...
                 crossEnt->currentState
                     .teamowner) { // a team owns this - if it's my team green,
                                   // if not red, if not teamplay then yellow
        if (cgs.gametype < GT_TEAM) {
          ecolor[0] = 1.0f; // R
          ecolor[1] = 1.0f; // G
          ecolor[2] = 0.0f; // B
        } else if (cg.predictedPlayerState.persistant[PERS_TEAM] !=
                   crossEnt->currentState.teamowner) { // not my team
          ecolor[0] = 1.0f;                            // R
          ecolor[1] = 0.0f;                            // G
          ecolor[2] = 0.0f;                            // B
        } else {                                       // my team
  ...
  if (worldPoint && VectorLength(worldPoint)) {
    if (!CG_WorldCoordToScreenCoordFloat(worldPoint, &x,
                                         &y)) { // off screen, don't draw it
      return;
    }
  ...
  // draw a health bar directly under the crosshair if we're looking at
  ...
  if (crossEnt && crossEnt->currentState.maxhealth) {
    ...
    if (cg.crosshairVehNum &&
        cg.time == cg.crosshairVehTime) { // it was in the crosshair this frame
      ...
      if (hisVeh->currentState.eType == ET_NPC &&
          ...
          hisVeh->m_pVehicle) { // draw the health for this vehicle
  ...
  if (cg.predictedPlayerState.hackingTime) { // hacking something
    CG_DrawHaqrBar(chX, chY, w, h);
  }

  if (cg_genericTimerBar >
      cg.time) { // draw generic timing bar, can be used for whatever
  ...
  if (corona) // drawing extra bits
  {
    ...
    ecolor[0] = ecolor[1] = ecolor[2] =
        (1 - ecolor[3]) *
        (sin(cg.time * 0.001f) * 0.08f + 0.35f); // don't draw full color
    ecolor[3] = 1.0f;
    ...
    trap->R_DrawStretchPic(x + cg.refdef.x + 0.5 * (640 - w),
                           y + cg.refdef.y + 0.5 * (480 - h), w, h, 0, 0, 1, 1,
...
qboolean CG_WorldCoordToScreenCoordFloat(vec3_t worldCoord, float *x,
                                         float *y) {
  vec3_t trans;
  float xc, yc;
  float px, py;
...
void CG_SaberClashFlare(void) {
  ...
  CG_Trace(&tr, cg.refdef.vieworg, NULL, NULL, cg_saberFlashPos, -1,
           CONTENTS_SOLID);
  ...
  */
  ...
  CG_DrawPic(x - (v * 300), y - (v * 300), v * 600, v * 600,
             trap->R_RegisterShader("gfx/effects/saberFlare"));
}

void CG_DottedLine(float x1, float y1, float x2, float y2, float dotSize,
                   int numDots, vec4_t color, float alpha) {
...
void CG_BracketEntity(centity_t *cent, float radius) {
  ...
  if (cg.crosshairClientNum != cent->currentState.clientNum &&
      (!cg.snap ||
       cg.snap->ps.rocketLockIndex !=
           cent->currentState
               .clientNum)) { // if they're the entity you're locking onto or
                              // under your crosshair, always draw bracket
  ...
  if (!CG_WorldCoordToScreenCoordFloat(cent->lerpOrigin, &x,
                                       &y)) { // off-screen, don't draw it
    return;
  }
  ...
  if (cent->currentState.m_iVehicleNum // vehicle has a driver
    ...
    if (cgs.gametype < GT_TEAM) { // ffa?
      isEnemy = qtrue;
      trap->R_SetColor(g_color_table[ColorIndex(COLOR_RED)]);
    } else if (cgs.clientinfo[cent->currentState.m_iVehicleNum - 1].team ==
               local->team) {
      trap->R_SetColor(g_color_table[ColorIndex(COLOR_GREEN)]);
    } else {
      isEnemy = qtrue;
    ...
    if (cgs.gametype < GT_TEAM) { // ffa?
      ...
      trap->R_SetColor(g_color_table[ColorIndex(COLOR_RED)]);
               ...
               cg.predictedPlayerState.persistant[PERS_TEAM]) { // on enemy team
      isEnemy = qtrue;
      trap->R_SetColor(g_color_table[ColorIndex(COLOR_RED)]);
    } else { // a friend
      trap->R_SetColor(g_color_table[ColorIndex(COLOR_GREEN)]);
    }
  } else { // FIXME: if we want to ever bracket anything besides vehicles (like
           // siege objectives we want to blow up), we should handle the coloring
           // here
    trap->R_SetColor(NULL);
  }
  ...
  */
  { // brackets would be drawn on the screen, so draw them
    ...
    // upper right corner
    // horz
    CG_DrawPic(x + size - lineLength, y, lineLength, lineWidth,
               cgs.media.whiteShader);
    // vert
    CG_DrawPic(x + size - lineWidth, y, lineWidth, lineLength,
               cgs.media.whiteShader);
    // lower left corner
    // horz
    CG_DrawPic(x, y + size - lineWidth, lineLength, lineWidth,
               cgs.media.whiteShader);
    // vert
    CG_DrawPic(x, y + size - lineLength, lineWidth, lineLength,
               cgs.media.whiteShader);
    // lower right corner
    // horz
    CG_DrawPic(x + size - lineLength, y + size - lineWidth, lineLength,
               lineWidth, cgs.media.whiteShader);
    // vert
    CG_DrawPic(x + size - lineWidth, y + size - lineLength, lineWidth,
               lineLength, cgs.media.whiteShader);
  }
  // Lead Indicator...
  if (cg_drawVehLeadIndicator.integer) { // draw the lead indicator
    if (isEnemy) {                       // an enemy object
      if (cent->currentState.NPC_class == CLASS_VEHICLE) { // enemy vehicle
        if (!VectorCompare(cent->currentState.pos.trDelta,
                           vec3_origin)) { // enemy vehicle is moving
          if (cg.predictedPlayerState.m_iVehicleNum) { // I'm in a vehicle
            centity_t *veh =
                &cg_entities[cg.predictedPlayerState.m_iVehicleNum];
            if (veh                                // vehicle cent
                && veh->m_pVehicle                 // vehicle
                && veh->m_pVehicle->m_pVehicleInfo // vehicle stats
                && veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID >
                       VEH_WEAPON_BASE) // valid vehicle weapon
            {
              vehWeaponInfo_t *vehWeapon =
                  &g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[0]
                                       .ID];
              if (vehWeapon &&
                  vehWeapon
                      ->bIsProjectile // primary weapon's shot is a projectile
                  && !vehWeapon->bHasGravity // primary weapon's shot is not
                                             // affected by gravity
              ...
              { // our primary weapon's projectile has a speed
                ...
                VectorSubtract(cent->lerpOrigin,
                               cg.predictedVehicleState.origin, vehDiff);
                vehDist = VectorNormalize(vehDiff);
                eta = (vehDist /
                       vehWeapon->fSpeed); // how many seconds it would take for
                                           // my primary weapon's projectile to
                                           // get from my ship to theirs
                // now extrapolate their position that number of seconds into
                // the future based on their velocity
                VectorMA(cent->lerpOrigin, eta, cent->currentState.pos.trDelta,
                         vehLeadPos);
                // now we have where we should be aiming at, project that onto
                // the screen at a 2D co-ord
                if (!CG_WorldCoordToScreenCoordFloat(
                        cent->lerpOrigin, &x,
                        &y)) { // off-screen, don't draw it
                  return;
                }
                if (!CG_WorldCoordToScreenCoordFloat(
                        vehLeadPos, &leadX,
                        &leadY)) { // off-screen, don't draw it
                  // just draw the line
                  CG_DottedLine(x, y, leadX, leadY, 1, 10,
                                g_color_table[ColorIndex(COLOR_RED)], 0.5f);
                  return;
                }
...
qboolean CG_InFighter(void) {
  if (cg.predictedPlayerState.m_iVehicleNum) { // I'm in a vehicle
    centity_t *vehCent = &cg_entities[cg.predictedPlayerState.m_iVehicleNum];
    if (vehCent && vehCent->m_pVehicle && vehCent->m_pVehicle->m_pVehicleInfo &&
        vehCent->m_pVehicle->m_pVehicleInfo->type ==
            VH_FIGHTER) { // I'm in a fighter
      return qtrue;
    }
  }
  return qfalse;
...
//--------------------------------------------------------------
{
  ...
  while (i < NUM_FORCE_POWERS) {
    if (cg.snap->ps.holocronBits & (1 << forcePowerSorted[i])) {
      CG_DrawPic(startx, starty, endx, endy,
                 cgs.media.forcePowerIcons[forcePowerSorted[i]]);
...
static qboolean CG_IsDurationPower(int power) {
  if (power == FP_HEAL || power == FP_SPEED || power == FP_TELEPATHY ||
      power == FP_RAGE || power == FP_PROTECT || power == FP_ABSORB ||
      power == FP_SEE) {
    return qtrue;
  }
...
//--------------------------------------------------------------
{
  ...
  float lockTimeInterval =
      ((cgs.gametype == GT_SIEGE) ? 2400.0f : 1200.0f) / 16.0f;
    ...
    if (veh->m_pVehicle) {
      ...
      if (cg.predictedVehicleState.weaponstate == WEAPON_CHARGING_ALT) {
        if (veh->m_pVehicle->m_pVehicleInfo->weapon[1].ID > VEH_WEAPON_BASE &&
          ...
          vehWeapon =
              &g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[1].ID];
        }
      } else {
        if (veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID > VEH_WEAPON_BASE &&
            veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID < MAX_VEH_WEAPONS) {
          vehWeapon =
              &g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID];
        }
      }
      if (vehWeapon !=
          NULL) { // we are trying to lock on with a valid vehicle weapon, so
                  // use *its* locktime, not the hard-coded one
  ...
  // We can't check to see in pmove if players are on the same team, so we
  // resort to just not drawing the lock if a teammate is the locked on ent
    ...
    if (ci) {
      if (ci->team == cgs.clientinfo[cg.snap->ps.clientNum].team) {
        ...
        if (hitEnt->currentState.eType == ET_NPC &&
            ...
            hitEnt->currentState.owner <
                ENTITYNUM_WORLD) { // this is a vehicle, if it has a pilot and
                                   // that pilot is on my team, then...
  ...
  if (CG_WorldCoordToScreenCoord(org, &cx, &cy)) {
    ...
    for (i = 0; i < dif; i++) {
      ...
      CG_DrawRotatePic(cx - sz, cy - sz, sz, sz, i * 45.0f,
    ...
    if (dif == 8) {
      ...
      CG_DrawPic(cx - sz, cy - sz * 2, sz * 2, sz * 2,
...
qboolean CG_CalcVehicleMuzzlePoint(int entityNum, vec3_t start, vec3_t d_f,
                                   vec3_t d_rt, vec3_t d_up) {
  centity_t *vehCent = &cg_entities[entityNum];
  if (vehCent->m_pVehicle && vehCent->m_pVehicle->m_pVehicleInfo->type ==
                                 VH_WALKER) { // draw from barrels
    VectorCopy(vehCent->lerpOrigin, start);
    start[2] +=
        vehCent->m_pVehicle->m_pVehicleInfo->height - DEFAULT_MINS_2 - 48;
    ...
    {                                     // passenger in a vehicle
      if (vehCent->m_pVehicle && vehCent->m_pVehicle->m_pVehicleInfo &&
          vehCent->m_pVehicle->m_pVehicleInfo
              ->maxPassengers) { // a vehicle capable of carrying passengers
        int turretNum;
        for (turretNum = 0; turretNum < MAX_VEHICLE_TURRETS; turretNum++) {
          if (vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum]
                  .iAmmoMax) { // valid turret
            if (vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum]
                    .passengerNum ==
                cg.predictedPlayerState.generic1) { // I control this turret
              // Go through all muzzles, average their positions and directions
              // and use the result for crosshair trace
              ...
              for (i = 0; i < MAX_VEHICLE_TURRET_MUZZLES; i++) {
                vehMuzzle =
                    vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum]
                        .iMuzzle[i];
                if (vehMuzzle) {
                  vehMuzzle -= 1;
                  CG_CalcVehMuzzle(vehCent->m_pVehicle, vehCent, vehMuzzle);
                  VectorAdd(muzzlesAvgPos,
                            vehCent->m_pVehicle->m_vMuzzlePos[vehMuzzle],
                            muzzlesAvgPos);
                  VectorAdd(muzzlesAvgDir,
                            vehCent->m_pVehicle->m_vMuzzleDir[vehMuzzle],
                            muzzlesAvgDir);
                  numMuzzles++;
                }
                if (numMuzzles) {
...
void CG_CalcEWebMuzzlePoint(centity_t *cent, vec3_t start, vec3_t d_f,
                            vec3_t d_rt, vec3_t d_up) {
  ...
  if (bolt != -1) {
    ...
    trap->G2API_GetBoltMatrix_NoRecNoRot(cent->ghoul2, 0, bolt, &boltMatrix,
                                         cent->lerpAngles, cent->lerpOrigin,
                                         cg.time, NULL, cent->modelScale);
    BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, start);
    BG_GiveMeVectorFromMatrix(&boltMatrix, NEGATIVE_X, d_f);

    // these things start the shot a little inside the bbox to assure not
    // starting in something solid
...
static void CG_ScanForCrosshairEntity(void) {
    ...
    */
    ...
    if (cg.predictedPlayerState.m_iVehicleNum &&
        (cg.predictedPlayerState.eFlags &
         EF_NODRAW)) { // we're *inside* a vehicle
      ...
      { // the pilot
        ...
        gunner = CG_CalcVehicleMuzzlePoint(
            cg.predictedPlayerState.m_iVehicleNum, start, d_f, d_rt, d_up);
      }
      /*
      else
      { //a passenger
      ...
      */
      if (veh->m_pVehicle && veh->m_pVehicle->m_pVehicleInfo &&
        ...
        //		to see if we hit anything closer and auto-aim at it if
        //so
        bVehCheckTraceFromCamPos = qtrue;
      }
    } else if (cg.snap && cg.snap->ps.weapon == WP_EMPLACED_GUN &&
               cg.snap->ps.emplacedIndex &&
               cg_entities[cg.snap->ps.emplacedIndex].ghoul2 &&
               cg_entities[cg.snap->ps.emplacedIndex].currentState.weapon ==
                   WP_NONE) { // locked into our e-web, calc the muzzle from it
      CG_CalcEWebMuzzlePoint(&cg_entities[cg.snap->ps.emplacedIndex], start,
                             d_f, d_rt, d_up);
    } else {
      if (cg.snap && cg.snap->ps.weapon == WP_EMPLACED_GUN &&
          cg.snap->ps.emplacedIndex) {
  ...
  if (cg_dynamicCrosshair.integer &&
      cg_dynamicCrosshairPrecision
          .integer) { // then do a trace with ghoul2 models in mind
    CG_G2Trace(&trace, start, vec3_origin, vec3_origin, end, ignore,
               CONTENTS_SOLID | CONTENTS_BODY);
    if (bVehCheckTraceFromCamPos) {
      // NOTE: this MUST stay up to date with the method used in
      // WP_VehCheckTraceFromCamPos
      centity_t *veh = &cg_entities[cg.predictedPlayerState.m_iVehicleNum];
      trace_t extraTrace;
      vec3_t viewDir2End, extraEnd;
      float minAutoAimDist = Distance(veh->lerpOrigin, cg.refdef.vieworg) +
                             (veh->m_pVehicle->m_pVehicleInfo->length / 2.0f) +
                             200.0f;

      VectorSubtract(end, cg.refdef.vieworg, viewDir2End);
      VectorNormalize(viewDir2End);
      VectorMA(cg.refdef.vieworg, MAX_XHAIR_DIST_ACCURACY, viewDir2End,
               extraEnd);
      CG_G2Trace(&extraTrace, cg.refdef.vieworg, vec3_origin, vec3_origin,
                 extraEnd, ignore, CONTENTS_SOLID | CONTENTS_BODY);
        ...
        if (extraTrace.fraction < 1.0f) {
          if ((extraTrace.fraction * MAX_XHAIR_DIST_ACCURACY) >
            ...
            if (((extraTrace.fraction * MAX_XHAIR_DIST_ACCURACY) -
                ...
                (trace.fraction *
                 cg.distanceCull)) { // this trace hit *something* that's closer
                                     // than the thing the main trace hit, so use
                                     // this result instead
    ...
    CG_Trace(&trace, start, vec3_origin, vec3_origin, end, ignore,
             CONTENTS_SOLID | CONTENTS_BODY);
  }

  if (trace.entityNum < MAX_CLIENTS) {
    if (CG_IsMindTricked(
            cg_entities[trace.entityNum].currentState.trickedentindex,
      ...
      return; // this entity is mind-tricking the current client, so don't
              // render it
    }
  }
  ...
  if (cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR) {
    if (trace.entityNum < /*MAX_CLIENTS*/ ENTITYNUM_WORLD) {
      ...
      if (veh->currentState.eType == ET_NPC &&
          ...
          veh->currentState.owner <
              MAX_CLIENTS) { // draw the name of the pilot then
...
static void CG_DrawCrosshairNames(void) {
  ...
  if (cgs.gametype >= GT_TEAM) {
    // if (cgs.gametype == GT_SIEGE)
    if (1) { // instead of team-based we'll make it oriented based on which team
             // we're on
    ...
    if (cgs.gametype == GT_POWERDUEL &&
        cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR &&
        cgs.clientinfo[cg.crosshairClientNum].duelTeam ==
            cgs.clientinfo[cg.predictedPlayerState.clientNum]
                .duelTeam) { // on the same duel team in powerduel, so he's a
                             // friend
      baseColor = CT_GREEN;
    } else {
      baseColor = CT_RED; // just make it red in nonteam modes since everyone is
                          // hostile and crosshair will be red on them too
    }
  }

  if (cg.snap->ps.duelInProgress) {
    if (cg.crosshairClientNum !=
        cg.snap->ps.duelIndex) { // grey out crosshair for everyone but your foe
                                 // if you're in a duel
      baseColor = CT_BLACK;
    }
  } else if (cg_entities[cg.crosshairClientNum]
                 .currentState
                 .bolt1) { // this fellow is in a duel. We just checked if we
                           // were in a duel above, so this means we aren't and
                           // he is. Which of course means our crosshair greys
                           // out over him.
    baseColor = CT_BLACK;
  }
...
static void CG_DrawSpectator(void) {
  ...
  if ((cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL) &&
      cgs.duelist1 != -1 && cgs.duelist2 != -1) {
    char text[1024];
    int size = 64;

    if (cgs.gametype == GT_POWERDUEL && cgs.duelist3 != -1) {
      Com_sprintf(text, sizeof(text), "%s^7 %s %s^7 %s %s",
                  cgs.clientinfo[cgs.duelist1].name,
                  CG_GetStringEdString("MP_INGAME", "SPECHUD_VERSUS"),
                  cgs.clientinfo[cgs.duelist2].name,
                  CG_GetStringEdString("MP_INGAME", "AND"),
                  cgs.clientinfo[cgs.duelist3].name);
    } else {
      Com_sprintf(text, sizeof(text), "%s^7 %s %s",
                  cgs.clientinfo[cgs.duelist1].name,
                  CG_GetStringEdString("MP_INGAME", "SPECHUD_VERSUS"),
                  cgs.clientinfo[cgs.duelist2].name);
    }
    CG_Text_Paint(320 - CG_Text_Width(text, 1.0f, 3) / 2, 420, 1.0f, colorWhite,
                  text, 0, 0, 0, 3);

    trap->R_SetColor(colorTable[CT_WHITE]);
    if (cgs.clientinfo[cgs.duelist1].modelIcon) {
      CG_DrawPic(10, SCREEN_HEIGHT - (size * 1.5), size, size,
                 cgs.clientinfo[cgs.duelist1].modelIcon);
    }
    if (cgs.clientinfo[cgs.duelist2].modelIcon) {
      CG_DrawPic(SCREEN_WIDTH - size - 10, SCREEN_HEIGHT - (size * 1.5), size,
                 size, cgs.clientinfo[cgs.duelist2].modelIcon);
    }

    // nmckenzie: DUEL_HEALTH
    if (cgs.gametype == GT_DUEL) {
      if (cgs.showDuelHealths >=
          1) { // draw the healths on the two guys - how does this interact with
               // power duel, though?
        CG_DrawDuelistHealth(10, SCREEN_HEIGHT - (size * 1.5) - 12, 64, 8, 1);
        CG_DrawDuelistHealth(SCREEN_WIDTH - size - 10,
                             SCREEN_HEIGHT - (size * 1.5) - 12, 64, 8, 2);
      }
    }

    if (cgs.gametype != GT_POWERDUEL) {
      Com_sprintf(text, sizeof(text), "%i/%i",
                  cgs.clientinfo[cgs.duelist1].score, cgs.fraglimit);
      CG_Text_Paint(42 - CG_Text_Width(text, 1.0f, 2) / 2,
                    SCREEN_HEIGHT - (size * 1.5) + 64, 1.0f, colorWhite, text,
                    0, 0, 0, 2);

      Com_sprintf(text, sizeof(text), "%i/%i",
                  cgs.clientinfo[cgs.duelist2].score, cgs.fraglimit);
      CG_Text_Paint(SCREEN_WIDTH - size + 22 - CG_Text_Width(text, 1.0f, 2) / 2,
                    SCREEN_HEIGHT - (size * 1.5) + 64, 1.0f, colorWhite, text,
                    0, 0, 0, 2);
    }
    ...
    if (cgs.gametype == GT_POWERDUEL && cgs.duelist3 != -1) {
      if (cgs.clientinfo[cgs.duelist3].modelIcon) {
        CG_DrawPic(SCREEN_WIDTH - size - 10, SCREEN_HEIGHT - (size * 2.8), size,
                   size, cgs.clientinfo[cgs.duelist3].modelIcon);
      }
    }
  } else {
    CG_Text_Paint(320 - CG_Text_Width(s, 1.0f, 3) / 2, 420, 1.0f, colorWhite, s,
                  0, 0, 0, 3);
  }
  ...
  if (cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL) {
    ...
    CG_Text_Paint(320 - CG_Text_Width(s, 1.0f, 3) / 2, 440, 1.0f, colorWhite, s,
                  0, 0, 0, 3);
  } else // if ( cgs.gametype >= GT_TEAM )
  {
...
static void CG_DrawVote(void) {
  ...
  if (!Q_strncmp(cgs.voteString, "map_restart", 11))
    ...
    if (!Q_stricmp("Free For All", cgs.voteString + 11))
      sParm = CG_GetStringEdString("MENUS", "FREE_FOR_ALL");
    else if (!Q_stricmp("Duel", cgs.voteString + 11))
      sParm = CG_GetStringEdString("MENUS", "DUEL");
    else if (!Q_stricmp("Holocron FFA", cgs.voteString + 11))
      sParm = CG_GetStringEdString("MENUS", "HOLOCRON_FFA");
    else if (!Q_stricmp("Power Duel", cgs.voteString + 11))
      sParm = CG_GetStringEdString("MENUS", "POWERDUEL");
    else if (!Q_stricmp("Team FFA", cgs.voteString + 11))
      sParm = CG_GetStringEdString("MENUS", "TEAM_FFA");
    else if (!Q_stricmp("Siege", cgs.voteString + 11))
  ...
  } else { // custom votes like ampoll, cointoss, etc
  ...
  if (sParm && sParm[0])
    s = va("%s(%i):<%s %s> %s:%i %s:%i", sVote, sec, sCmd, sParm, sYes,
           cgs.voteYes, sNo, cgs.voteNo);
  else
    s = va("%s(%i):<%s> %s:%i %s:%i", sVote, sec, sCmd, sYes, cgs.voteYes, sNo,
           cgs.voteNo);
  CG_DrawSmallString(4, 58, s, 1.0F);
  if (cgs.clientinfo[cg.clientNum].team != TEAM_SPECTATOR) {
    s = CG_GetStringEdString(
        "MP_INGAME", "OR_PRESS_ESC_THEN_CLICK_VOTE"); //	s = "or press
                                                      //ESC then click Vote";
...
static void CG_DrawTeamVote(void) {
  ...
  if (strstr(cgs.teamVoteString[cs_offset], "leader")) {
    ...
    while (cgs.teamVoteString[cs_offset][i] &&
           cgs.teamVoteString[cs_offset][i] != ' ') {
      i++;
    }
    ...
    if (cgs.teamVoteString[cs_offset][i] == ' ') {
      ...
      s = va("TEAMVOTE(%i):(Make %s the new team leader) yes:%i no:%i", sec,
             cgs.clientinfo[voteIndex].name, cgs.teamVoteYes[cs_offset],
...
static qboolean CG_DrawScoreboard() { return CG_DrawOldScoreboard(); }
...
static qboolean CG_DrawFollow(void) {
  ...
  CG_Text_Paint(320 - CG_Text_Width(s, 1.0f, FONT_MEDIUM) / 2, 60, 1.0f,
                colorWhite, s, 0, 0, 0, FONT_MEDIUM);

  s = cgs.clientinfo[cg.snap->ps.clientNum].name;
  CG_Text_Paint(320 - CG_Text_Width(s, 2.0f, FONT_MEDIUM) / 2, 80, 2.0f,
                colorWhite, s, 0, 0, 0, FONT_MEDIUM);
...
static void CG_DrawWarmup(void) {
  ...
  if (cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL) {
    ...
    if (cgs.gametype == GT_FFA)
      ...
      s = CG_GetStringEdString("MENUS",
                               "CAPTURE_THE_FLAG"); //"Capture the Flag";
    else if (cgs.gametype == GT_CTY)
      s = CG_GetStringEdString(
          "MENUS", "CAPTURE_THE_YSALIMARI"); //"Capture the Ysalamiri";
    else if (cgs.gametype == GT_SINGLE_PLAYER)
      s = "Cooperative";
    else
      s = "";
    w = CG_Text_Width(s, 1.5f, FONT_MEDIUM);
    CG_Text_Paint(320 - w / 2, 90, 1.5f, colorWhite, s, 0, 0,
                  ITEM_TEXTSTYLE_SHADOWEDMORE, FONT_MEDIUM);
  }
...
void CG_DrawJetpackFuel(void) {
  ...
  // now draw the part to show how much health there is in the color specified
  CG_FillRect(x + 1.0f, y + 1.0f + (JPFUELBAR_H - percent), JPFUELBAR_W - 1.0f,
              JPFUELBAR_H - 1.0f - (JPFUELBAR_H - percent), aColor);

  // then draw the other part greyed out
  CG_FillRect(x + 1.0f, y + 1.0f, JPFUELBAR_W - 1.0f, JPFUELBAR_H - percent,
              cColor);
}
...
void CG_DrawEWebHealth(void) {
  ...
  float percent =
      ((float)eweb->currentState.health / eweb->currentState.maxhealth) *
      EWEBHEALTH_H;
  ...
  // now draw the part to show how much health there is in the color specified
  CG_FillRect(x + 1.0f, y + 1.0f + (EWEBHEALTH_H - percent),
              EWEBHEALTH_W - 1.0f,
              EWEBHEALTH_H - 1.0f - (EWEBHEALTH_H - percent), aColor);

  // then draw the other part greyed out
  CG_FillRect(x + 1.0f, y + 1.0f, EWEBHEALTH_W - 1.0f, EWEBHEALTH_H - percent,
              cColor);
}
...
void CG_DrawCloakFuel(void) {
  ...
  if (cg.snap->ps.jetpackFuel <
      100) { // if drawing jetpack fuel bar too, then move this over...?
    x -= (JPFUELBAR_W + 8.0f);
  }
  ...
  CG_FillRect(x + 1.0f, y + 1.0f + (CLFUELBAR_H - percent), CLFUELBAR_W - 1.0f,
              CLFUELBAR_H - 1.0f - (CLFUELBAR_H - percent), aColor);

  // then draw the other part greyed out
  CG_FillRect(x + 1.0f, y + 1.0f, CLFUELBAR_W - 1.0f, CLFUELBAR_H - percent,
              cColor);
}
...
static void
CG_DrawSiegeTimer(int timeRemaining,
                  qboolean isMyTeam) { // rwwFIXMEFIXME: Make someone make
                                       // assets and use them. this function is
                                       // pretty much totally placeholder.
  //	int x = 0;
  ...
  //	CG_DrawProportionalString( x+16, y+40, timeStr,
  //UI_SMALLFONT|UI_DROPSHADOW, colorTable[fColor] );

  item = Menu_FindItemByName(menuHUD, "timer");
  if (item) {
    CG_DrawProportionalString(item->window.rect.x, item->window.rect.y, timeStr,
                              UI_SMALLFONT | UI_DROPSHADOW, colorTable[fColor]);
  }
}
...
static void CG_DrawSiegeHUDItem(void) {
  ...
  CG_Draw3DModel(8, 8, 64, 64, handle, g2, cent->currentState.g2radius, 0,
                 origin, angles);
...
void CG_ChatBox_AddString(char *chatStr) {
  ...
  if (cg_chatBox.integer <= 0) { // don't bother then.
    return;
  }
  ...
  if (strlen(chatStr) >
      sizeof(chat->string)) { // too long, terminate at proper len.
...
void CG_ChatBox_ArrayInsert(chatBoxItem_t **array, int insPoint, int maxNum,
                            chatBoxItem_t *item) {
  ...
  // now that we have moved anything that would be in this slot up, insert what
  // we want into the slot
  array[insPoint] = item;
}
...
static QINLINE void CG_ChatBox_DrawStrings(void) {
  ...
  while (i < MAX_CHATBOX_ITEMS) {
    if (cg.chatItems[i].time >= cg.time) {
      ...
      while (check >= 0) {
        if (drawThese[check] &&
            cg.chatItems[i].time < drawThese[check]->time) { // insert here
          insertionPoint = check;
        }
        check--;
      }
      CG_ChatBox_ArrayInsert(drawThese, insertionPoint, MAX_CHATBOX_ITEMS,
                             &cg.chatItems[i]);
      numToDraw++;
  ...
  i = 0;
  while (i < numToDraw) {
    CG_Text_Paint(x, y, fontScale, colorWhite, drawThese[i]->string, 0, 0,
...
static void CG_Draw2DScreenTints(void) {
  ...
  if (cgs.clientinfo[cg.snap->ps.clientNum].team != TEAM_SPECTATOR) {
    ...
    if (cg.snap->ps.rocketLockIndex != ENTITYNUM_NONE &&
        (cg.time - cg.snap->ps.rocketLockTime) > 0) {
      CG_DrawRocketLocking(cg.snap->ps.rocketLockIndex,
                           cg.snap->ps.rocketLockTime);
    }
  ...
  if ((cg.refdef.viewContents & CONTENTS_LAVA)) { // tint screen red
    ...
    CG_FillRect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, hcolor);
  } else if ((cg.refdef.viewContents &
              CONTENTS_WATER)) { // tint screen light blue -- FIXME: don't do
                                 // this if CONTENTS_FOG? (in case someone *does*
                                 // make a water shader with fog in it?)
...
static void CG_Draw2D(void) {
  ...
  if (cg.snap->ps.fd.forcePowersActive ||
      cg.snap->ps.fd.forceRageRecoveryTime > cg.time) {
    CG_DrawActivePowers();
  }
  ...
  if (cg.predictedPlayerState.emplacedIndex > 0) {
    ...
    if (eweb->currentState.weapon ==
        WP_NONE) { // using an e-web, draw its health
      CG_DrawEWebHealth();
    }
  }
  ...
  */
  ...
  if (cgSiegeRoundState) {
    ...
    case 1:
      CG_CenterPrint(CG_GetStringEdString("MP_INGAME", "WAITING_FOR_PLAYERS"),
    ...
    case 2:
      ...
      Q_strncpyz(pStr,
                 va("%s %i...",
                    CG_GetStringEdString("MP_INGAME", "ROUNDBEGINSIN"), rTime),
                 sizeof(pStr));
    ...
    cgSiegeEntityRender = 0;
  } else if (cgSiegeRoundBeganTime) { // Draw how much time is left in the round
                                      // based on local info.
    int timedTeam = TEAM_FREE;
    int timedValue = 0;

    if (cgSiegeEntityRender) { // render the objective item model since this
                               // client has it
      CG_DrawSiegeHUDItem();
    }
    ...
    if (timedTeam != TEAM_FREE) { // one of the teams has a timer
      ...
      if (cgs.siegeTeamSwitch &&
          !cg_beatingSiegeTime) { // in switchy mode but not beating a time, so
                                  // count up.
      ...
      if (cg.predictedPlayerState.persistant[PERS_TEAM] ==
          timedTeam) { // the team that's timed is the one this client is on
        isMyTeam = qtrue;
      }
...
void CG_DrawMiscStaticModels(void) {
  ...
  for (i = 0; i < cgs.numMiscStaticModels; i++) {
    ...
    if (VectorLength(diff) - (cgs.miscStaticModels[i].radius) <=
        cg.distanceCull) {
      trap->R_AddRefEntityToScene(&ent);
    }
  }
