...
void G_MissileBounceEffect(gentity_t *ent, vec3_t org, vec3_t dir) {
  // FIXME: have an EV_BOUNCE_MISSILE event that checks the s.weapon and does
...
static void G_MissileStick(gentity_t *missile, gentity_t *other, trace_t *tr) {
  if (other->NPC || !Q_stricmp(other->classname, "misc_model_breakable")) {
    // we bounce off of NPC's and misc model breakables because sticking to them
    // requires too much effort
    vec3_t velocity;

    int hitTime =
        level.previousTime + (level.time - level.previousTime) * tr->fraction;
    ...
    VectorMA(missile->s.pos.trDelta, 10, tr->plane.normal,
             missile->s.pos.trDelta);
    missile->s.pos.trTime = level.time - 10; // move a bit on the first frame

    // check for stop
    if (tr->entityNum >= 0 && tr->entityNum < ENTITYNUM_WORLD &&
        tr->plane.normal[2] > 0.7 &&
        missile->s.pos.trDelta[2] <
            40) // this can happen even on very slightly sloped walls, so
  ...
  if (other->s.eType == ET_MOVER || other->e_DieFunc == dieF_funcBBrushDie ||
    ...
    // movers and breakable brushes need extra info...so sticky missiles can
    // ride lifts and blow up when the thing they are attached to goes away.
...
================
*/
extern gentity_t *Jedi_FindEnemyInCone(gentity_t *self, gentity_t *fallback,
                                       float minDot);
void G_ReflectMissile(gentity_t *ent, gentity_t *missile, vec3_t forward) {
  ...
  if (ent && owner && owner->client && !owner->client->ps.saberInFlight &&
      (owner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_2 ||
       (owner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_1 &&
        !Q_irand(0,
                 3)))) { // if high enough defense skill and saber in-hand (100%
                         // at level 3, 25% at level 2, 0% at level 1),
      ...
      if (!PM_SaberInParry(owner->client->ps.saberMove) &&
        ...
        if (PM_SaberInAttack(owner->client->ps.saberMove) ||
            ...
            PM_SaberInSpecialAttack(
                owner->client->ps.torsoAnim)) { // moderately more wild
          for (i = 0; i < 3; i++) {
  ...
  if (!reflected) {
    if (missile->owner &&
        missile->s.weapon != WP_SABER) { // bounce back at them if you can
      VectorSubtract(missile->owner->currentOrigin, missile->currentOrigin,
                     bounce_dir);
      VectorNormalize(bounce_dir);
    } else {
      vec3_t missile_dir;
    ...
    if (owner->s.weapon == WP_SABER && owner->client) { // saber
      if (owner->client->ps
              .saberInFlight) { // reflecting off a thrown saber is totally wild
        for (i = 0; i < 3; i++) {
      ...
      if (!PM_SaberInParry(owner->client->ps.saberMove) &&
        ...
        if (PM_SaberInAttack(owner->client->ps.saberMove) ||
            PM_SaberInTransitionAny(owner->client->ps.saberMove) ||
            PM_SaberInSpecialAttack(
                owner->client->ps.torsoAnim)) { // really wild
          for (i = 0; i < 3; i++) {
...
void G_BounceMissile(gentity_t *ent, trace_t *trace) {
  ...
  if (hitTime >= level.time) { // trace fraction must have been 1
    ...
    ent->s.pos.trTime =
        hitTime - 10; // this is kinda dumb hacking, but it pushes the missile
                      // away from the impact plane a bit
  }
#endif
  ...
  if (ent->s.weapon != WP_SABER && ent->s.weapon != WP_THERMAL &&
      ...
      ent->e_ThinkFunc !=
          thinkF_LimbThink) { // not a saber, bouncing thermal or limb
...
extern void WP_SaberBlock(gentity_t *saber, vec3_t hitloc,
                          qboolean missleBlock);
extern void laserTrapStick(gentity_t *ent, vec3_t endpos, vec3_t normal);
extern qboolean W_AccuracyLoggableWeapon(int weapon, qboolean alt_fire,
                                         int mod);
void G_MissileImpacted(gentity_t *ent, gentity_t *other, vec3_t impactPos,
                       vec3_t normal, int hitLoc = HL_NONE) {
  // impact damage
  if (other->takedamage) {
    ...
    if (ent->damage) {
      ...
      if (other->client) {
        ...
        // If we are a robot and we aren't currently doing the full body
        // electricity...
        if (npc_class == CLASS_SEEKER || npc_class == CLASS_PROBE ||
            ...
            npc_class == CLASS_REMOTE || npc_class == CLASS_MARK1 ||
            npc_class == CLASS_MARK2 || // npc_class == CLASS_PROTOCOL ||//no
                                        // protocol, looks odd
      ...
      G_Damage(other, ent, ent->owner, velocity, impactPos, damage, ent->dflags,
               ent->methodOfDeath, hitLoc);
    }
  }
...
void G_MissileImpact(gentity_t *ent, trace_t *trace, int hitLoc = HL_NONE) {
  ...
  if (trace->plane.normal[0] == 0.0f && trace->plane.normal[1] == 0.0f &&
      trace->plane.normal[2] ==
          0.0f) { // model moved into missile in flight probably...
  ...
  if (ent->owner && (other->takedamage || other->client)) {
    if (!ent->lastEnemy ||
        ent->lastEnemy ==
            ent->owner) { // a missile that was not reflected or, if so, still
                          // is owned by original owner
      if (LogAccuracyHit(other, ent->owner)) {
        ent->owner->client->ps.persistant[PERS_ACCURACY_HITS]++;
      }
      if (ent->owner->client && !ent->owner->s.number) {
        if (W_AccuracyLoggableWeapon(ent->s.weapon, qfalse,
                                     ent->methodOfDeath)) {
  ...
  // check for bounce
  // OR: if the surfaceParm is has a reflect property (magnetic shielding) and
  // the missile isn't an exploding missile
  qboolean bounce = (qboolean)(
      (!other->takedamage && (ent->s.eFlags & (EF_BOUNCE | EF_BOUNCE_HALF))) ||
      (((trace->surfaceFlags & SURF_FORCEFIELD) ||
  ...
  if (other->flags & (FL_DMG_BY_HEAVY_WEAP_ONLY | FL_SHIELDED)) {
    ...
    if ((!strcmp("misc_ion_cannon", other->classname)) &&
        (other->flags & FL_SHIELDED)) {
      // Anything will bounce off of us.
      bounce = qtrue;

      // Not exactly the debounce time, but rather the impact time for the
      // shield effect...play effect for 1 second
  ...
  if (bounce) {
    ...
    if (other->NPC) {
      G_Damage(other, ent, ent->owner, ent->currentOrigin, ent->s.pos.trDelta,
               0, DAMAGE_NO_DAMAGE, MOD_UNKNOWN);
    }
  ...
  // I would glom onto the EF_BOUNCE code section above, but don't feel like
  // risking breaking something else
  if ((!other->takedamage && (ent->s.eFlags & (EF_BOUNCE_SHRAPNEL))) ||
      ((trace->surfaceFlags & SURF_FORCEFIELD) && !ent->splashDamage &&
       !ent->splashRadius)) {
    if (!(other->contents & CONTENTS_LIGHTSABER) ||
        ...
        || (g_spskill->integer == 1 && ent->s.weapon != WP_FLECHETTE &&
            ent->s.weapon !=
                WP_DEMP2) // on medium it won't reflect flechette or demp shots
        || (g_spskill->integer >= 2 && ent->s.weapon != WP_FLECHETTE &&
            ent->s.weapon != WP_DEMP2 && ent->s.weapon != WP_BOWCASTER &&
            ent->s.weapon != WP_REPEATER) // on hard it won't reflect flechette,
                                          // demp, repeater or bowcaster shots
    ) {
      G_BounceMissile(ent, trace);
  ...
  if ((!other->takedamage || (other->client && other->health <= 0)) &&
      ...
      !ent->alt_fire) { // rolling thermal det - FIXME: make this an eFlag like
                        // bounce & stick!!!
  ...
  if (ent->s.eFlags & EF_MISSILE_STICK) {
    if (ent->owner) //&& ent->owner->s.number == 0 )
    {
      ...
      if (ent->s.weapon == WP_TRIP_MINE) {
        ...
        ent->splashRadius*2, AEL_DANGER, 60 );
        */
      } else {
        AddSoundEvent(ent->owner, ent->currentOrigin, 128, AEL_DISCOVERED);
  ...
  if (other->contents & CONTENTS_LIGHTSABER) {
    ...
    if ((g_spskill->integer <= 0 // on easy, it reflects all shots
        ...
        (!ent->splashDamage ||
         !ent->splashRadius)) // this would be cool, though, to "bat" the
                              // thermal det away...
    {
      ...
      { // Jedi cannot block shots from behind!
        if ((other->owner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] >
                 FORCE_LEVEL_1 &&
             Q_irand(0, 3)) ||
            (other->owner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] >
                 FORCE_LEVEL_0 &&
             Q_irand(0, 1))) { // level 1 reflects 50% of the time, level 2
                               // reflects 75% of the time
          VectorSubtract(ent->currentOrigin, other->currentOrigin, diff);
          VectorNormalize(diff);
          // FIXME: take other's owner's FP_SABER_DEFENSE into account here
          // somehow?
...
void G_ExplodeMissile(gentity_t *ent) {
  ...
  if (ent->splashDamage) {
    G_RadiusDamage(ent->currentOrigin, ent->owner, ent->splashDamage,
                   ent->splashRadius, NULL, ent->splashMethodOfDeath);
  }
    ...
    if (ent->s.groundEntityNum >= 0 &&
      ...
      if ((!VectorCompare(vec3_origin, other->s.pos.trDelta) &&
          ...
          (!VectorCompare(vec3_origin, other->s.apos.trDelta) &&
           other->s.apos.trType !=
               TR_STATIONARY)) { // thing I stuck to is moving or rotating now,
                                 // kill me
...
int G_GroundTrace(gentity_t *ent, pml_t *pPml) {
  ...
  gi.trace(&trace, ent->currentOrigin, ent->mins, ent->maxs, point,
  ...
  if (ent->s.pos.trDelta[2] > 0 &&
      DotProduct(ent->s.pos.trDelta, trace.plane.normal) > 10) {
    pPml->groundPlane = qfalse;
    pPml->walking = qfalse;
    return ENTITYNUM_NONE;
  }
...
void G_RollMissile(gentity_t *ent) {
  ...
  if (objPML.groundPlane) { // FIXME: never happens!
    ...
    PM_ClipVelocity(ent->s.pos.trDelta, objPML.groundTrace.plane.normal,
                    ent->s.pos.trDelta, BUMPCLIP);
  ...
  */
  ...
  for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
    ...
    // TEMP HACK:
    // had to move this up above the trace.allsolid check now that for some
    // reason ghoul2 impacts tell me I'm allsolid?! this needs to be fixed,
    // really
    if (trace.entityNum < ENTITYNUM_WORLD) { // hit another ent
      hitEnt = &g_entities[trace.entityNum];
      if (hitEnt &&
          (hitEnt->takedamage || (hitEnt->contents & CONTENTS_LIGHTSABER))) {
    ...
    */
    ...
    if (&g_entities[trace.entityNum] != NULL &&
        g_entities[trace.entityNum]
            .client) { // hit a person, bounce off much less
      bounceAmt = OVERCLIP;
    } else {
      bounceAmt = BUMPCLIP;
    }
...
void G_RunMissile(gentity_t *ent) {
  ...
  if (ent->s.pos.trType == TR_INTERPOLATE) { // rolling missile?
    ...
    if (ent->s.eType != ET_GENERAL) { // didn't explode
      VectorCopy(ent->currentOrigin, ent->s.pos.trBase);
      gi.trace(&tr, oldOrg, ent->mins, ent->maxs, ent->currentOrigin,
               ent->s.number, ent->clipmask, G2_RETURNONHIT, 10);
      if (VectorCompare(ent->s.pos.trDelta, vec3_origin)) {
        ...
        // This is because when roll gets to 90, pitch becomes yaw, and
        // vice-versa Maybe need to just set the angles directly?
    ...
    */
    gi.trace(&tr, ent->currentOrigin, ent->mins, ent->maxs, origin,
             ...
             G2_COLLIDE, 10);
    /*
    if ( !VectorCompare( ent->mins, vec3_origin ) || !VectorCompare( ent->maxs,
    vec3_origin ) )
    {//don't do ghoul trace if ent has size because g2 just ignores that anyway
            gi.trace( &tr, ent->currentOrigin, ent->mins, ent->maxs, origin,
                    ent->owner ? ent->owner->s.number : ENTITYNUM_NONE,
    ent->clipmask, G2_NOCOLLIDE, 10 );
    }
    else
    //Now we always do ghoul trace, regardless of bbox size of missile, this is
    presuming that non-point ghoul traces will be possible...?
    {
    ...
    */
    /*
    if ( tr.fraction == 0.0f && tr.plane.normal[2] == 1.0f && origin[2] <
    ent->currentOrigin[2] )
    {
            if ( ent->s.pos.trType == TR_GRAVITY && !(ent->s.eFlags&EF_BOUNCE)
    && !(ent->s.eFlags&EF_BOUNCE_HALF) && ent->s.weapon == WP_THERMAL )//FIXME:
    EF_ROLLING
            {
    ...
    origin, ent->owner ? ent->owner->s.number : ENTITYNUM_NONE, ent->clipmask |
    CONTENTS_GHOUL2 ); if ( tr.fraction == 1.0f )
                    {
                            VectorCopy( tr.endpos, ent->s.pos.trBase );
                            VectorScale( ent->s.pos.trDelta, 0.975f,
    ent->s.pos.trDelta ); ent->s.pos.trTime = level.time;
                    }
                    ent->s.pos.trType = TR_GRAVITY;
            }
    }
    ...
    if (tr.entityNum != ENTITYNUM_NONE && &g_entities[tr.entityNum] != NULL) {
      ...
      if (other->contents & CONTENTS_LIGHTSABER) { // hit a lightsaber bbox
        if (other->owner && other->owner->client &&
            !other->owner->client->ps.saberInFlight &&
            !InFront(
                ent->currentOrigin, other->owner->currentOrigin,
                other->owner->client->ps.viewangles,
                SABER_REFLECT_MISSILE_CONE)) // other->owner->s.number == 0 &&
        { // Jedi cannot block shots from behind!
  ...
  // get current angles
  VectorMA(ent->s.apos.trBase, (level.time - ent->s.apos.trTime) * 0.001,
           ent->s.apos.trDelta, ent->s.apos.trBase);
  ...
  //?	if ( tr.fraction != 1 )
  {
    ...
    for (int i = 0; i < MAX_G2_COLLISIONS; i++) {
      ...
      #if _DEBUG
                              vec3_t delta;
                              VectorSubtract(origin, coll.mCollisionPosition,
      delta); VectorNormalize(delta); VectorScale(delta, 30, delta);

                              if (coll.mFlags & G2_BACKFACE)
                              {
                                      VectorAdd(delta, coll.mCollisionPosition,
      delta); G_DebugLine(coll.mCollisionPosition, delta, 10000, 0x00ff0000,
      qtrue);
                              }
                              else
                              {
                                      VectorSubtract(coll.mCollisionPosition,
      delta, delta); G_DebugLine(coll.mCollisionPosition, delta, 10000,
      0x0000ff00, qtrue);
                              }
      ...
      */
      ...
      // make sure we only do this once, not for all the entrance wounds we
      // might generate
      if ((coll.mFlags & G2_FRONTFACE) /* && !(hitModel)*/ && hitEnt->health) {
        // create a new surface using the details of what poly/surface/model we
        // hit
        //				int newSurface =
        //gi.G2API_AddSurface(&hitEnt->ghoul2[coll.mModelIndex],
        //coll.mSurfaceIndex, coll.mPolyIndex, coll.mBarycentricI,
        //coll.mBarycentricJ, 10); 				surfaceInfo_t	*newSuf =
        //&hitEnt->ghoul2[coll.mModelIndex].mSlist[newSurface];
        // attach a bolt to this surface
        //				int newBolt =
        //gi.G2API_AddBoltSurfNum(&hitEnt->ghoul2[coll.mModelIndex],
        //newSurface);
        // now attach an effect to this new bolt

        //	Bolting on this effect just looks dumb and adds lots of
        //unnecessary effects to the scene
        //
        //				G_PlayEffect( G_EffectIndex(
        //"blaster/smoke_bolton") , coll.mModelIndex, newBolt,
        //hitEnt->s.number);
        //
        //

        //				G_SetBoltSurfaceRemoval(coll.mEntityNum,
        //coll.mModelIndex, newBolt, newSurface, 10000); 				hitModel = qtrue;
        ...
        if (trHitLoc == HL_NONE) {
          G_GetHitLocFromSurfName(
              ...
              gi.G2API_GetSurfaceName(
                  ...
                  coll.mSurfaceIndex),
              &trHitLoc, coll.mCollisionPosition, NULL, NULL,
              ent->methodOfDeath);
        }

        break; // NOTE: the way this whole section was working, it would only
               // get inside of this IF once anyway, might as well break out now
      }
    }
  }
  /////////////////////////////////////////////////////////
  ...
  if (ent->s.pos.trType == TR_STATIONARY &&
      (ent->s.eFlags &
       EF_MISSILE_STICK)) { // stuck missiles should check some special stuff
  ...
  */
  ...
  if (!Q_irand(0, 10)) { // not so often...
    if (ent->splashDamage &&
        ent->splashRadius) { // I'm an exploder, let people around me know
                             // danger is coming
      if (ent->s.weapon == WP_TRIP_MINE) { //???
      } else {
        if (ent->s.weapon == WP_ROCKET_LAUNCHER &&
            ent->e_ThinkFunc ==
                thinkF_rocketThink) { // homing rocket- run like hell!
        ...
        AddSoundEvent(ent->owner, ent->currentOrigin, ent->splashRadius,
                      AEL_DANGER);
      }
