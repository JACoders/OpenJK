...
void CG_FragmentBounceMark(localEntity_t *le, trace_t *trace) {
  ...
  if (le->leMarkType == LEMT_BLOOD) {
    ...
    //trace->plane.normal, Q_flrand(0.0f, 1.0f)*360, 1,1,1,1, qtrue, radius,
    //qfalse );
  ...
  // don't allow a fragment to make multiple marks, or they pile up while
  // settling
...
void CG_FragmentBounceSound(localEntity_t *le, trace_t *trace) {
  ...
  if (rand() & 1) {
    ...
    case LEBS_ROCK:
      ...
      break;
    case LEBS_METAL:
      s = cgs.media.metalBounceSound[Q_irand(
          0, 1)]; // FIXME: make sure that this sound is registered
                  // properly...might still be rock bounce sound....
      break;
    default:
      return;
    }
    ...
    // we may end up bouncing again, but each bounce reduces the chance of
    // playing the sound again or they may make a lot of noise when they settle
...
void CG_AddFragment(localEntity_t *le) {
  ...
  if (!trace.startsolid) {
    ...
    if (le->bounceSound) { // specified bounce sound (debris)
      trap->S_StartSound(le->pos.trBase, ENTITYNUM_WORLD, CHAN_AUTO,
                         le->bounceSound);
    }
...
static void CG_AddFadeScaleModel(localEntity_t *le) {
  ...
  float frac =
      (cg.time - le->startTime) / ((float)(le->endTime - le->startTime));
...
static void CG_AddExplosion(localEntity_t *ex) {
  ...
  if (ex->light) {
    ...
    trap->R_AddLightToScene(ent->origin, light, ex->lightColor[0],
...
static void CG_AddSpriteExplosion(localEntity_t *le) {
  ...
  if (le->light) {
    ...
    trap->R_AddLightToScene(re.origin, light, le->lightColor[0],
...
void CG_AddOLine(localEntity_t *le) {
  ...
  re->shaderRGBA[3] = 0xff * alpha; // Yes, we could apply c to this too, but
                                    // fading the color is better for lines.
