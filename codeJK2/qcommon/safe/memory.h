...
*/
struct Deleter {
  void operator()(void *memory) const NOEXCEPT { detail::Free(memory); }
};

/**
Zone Allocator - for use with the STL
*/
template <typename T, memtag_t tag> struct Allocator {
  Allocator() = default;
  ...
  void deallocate(T *mem, std::size_t n) const NOEXCEPT { Deleter{}(mem); }
  ...
  // I think the standard guarantees automatic generation of this, but MSVC
  // can't handle it.
  template <typename U> struct rebind { using other = Allocator<U, tag>; };
};
...
inline UniquePtr<T> make_unique(Args &&... args) {
  ...
  // placement new. may throw, in which case the unique_ptr will take care of
  // freeing the memory.
