...
void Sentry_Fire(void) {
	...
	trap->G2API_GetBoltMatrix(NPCS.NPC->ghoul2, 0, bolt, &boltMatrix, NPCS.NPC->r.currentAngles, NPCS.NPC->r.currentOrigin, level.time, NULL,
							  NPCS.NPC->modelScale);
...
void Sentry_RangedAttack(qboolean visible, qboolean advance) {
	if (TIMER_Done(NPCS.NPC, "attackDelay") && NPCS.NPC->attackDebounceTime < level.time && visible) // Attack?
	{
		if (NPCS.NPCInfo->burstCount > 6) {
			if (!NPCS.NPC->fly_sound_debounce_time) { // delay closing down to give the player an opening
