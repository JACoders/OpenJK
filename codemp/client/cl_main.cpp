...
// ja_guid is a persistent "cookie" that allows servers to track players across
// game sessions
...
void CL_DemoCompleted(void) {
  ...
  // rww - The above code seems to just stick you in a no-menu state and you
  // can't do anything there. I'm not sure why it ever worked in TA, but
  // whatever. This code will bring us back to the main menu after a demo is
...
void CL_ShutdownAll(qboolean shutdownRef) {
  ...
  if (clc.demorecording)
    CL_StopRecord_f();
      ...
      // find a crash bug. so it doesn't barf on shutdown saying refentities
...
void CL_RequestMotd(void) {
  ...
  if (!i) {
    Com_Printf(
        "CL_RequestMotd: Error: could not resolve address of motd server %s\n",
        motdaddress);
    return;
  }
  to.type = NA_IP;
  ...
  info[0] = 0;
  // NOTE TTimo xoring against Com_Milliseconds, otherwise we may not have a
  // true randomization only srand I could catch before here is tr_noise.c l:26
  // srand(1001) https://zerowing.idsoftware.com/bugzilla/show_bug.cgi?id=382
  ...
  if (*clc.downloadList) {
    ...
    *s++ = 0;
    ...
    if (!cl_allowDownload->integer) {
      Com_Error(
          ERR_DROP,
          "UDP Downloads are disabled on your client. (cl_allowDownload is %d)",
...
void CL_CheckForResend(void) {
  ...
  case CA_CONNECTING:
    ...
    // malicious server can hijack this connection.
...
void CL_ServersResponsePacket(const netadr_t *from, msg_t *msg) {
  ...
  while (buffptr + 1 < buffend) {
    ...
    if (*buffptr == '\\') {
      ...
      if (buffend - buffptr < (int)(sizeof(addresses[numservers].ip) +
...
#endif
static void CL_CheckSVStringEdRef(
    char *buf,
      ...
      if (str[i + 1] == '@' && (i + 2) < strLen) {
        if (str[i + 2] == '@' &&
          ...
          Q_strcat(buf, MAX_STRINGED_SV_STRING,
                   SE_GetString(va("MP_SVGAME_%s", stripRef)));
          b = strlen(buf);
        }
      }
    }
...
void CL_Frame(int msec) {
  ...
  SE_CheckForLanguageUpdates(); // will take zero time to execute unless
                                // language changes, then will reload strings.
                                //	of course this still doesn't work for
                                //menus...
  ...
  if (CL_VideoRecording() && cl_aviFrameRate->integer && msec) {
    if (cls.state == CA_ACTIVE || cl_forceavidemo->integer) {
      ...
      float frameDuration =
          Q_max(1000.0f / fps, 1.0f) + clc.aviVideoFrameRemainder;
...
void CL_InitRef(void) {
  ...
  cl_renderer = Cvar_Get("cl_renderer", DEFAULT_RENDER_LIBRARY,
                         CVAR_ARCHIVE | CVAR_LATCH | CVAR_PROTECTED,
                         "Which renderer library to use");

  Com_sprintf(dllName, sizeof(dllName), "%s_" ARCH_STRING DLL_EXT,
              cl_renderer->string);
  ...
  if (!(rendererLib = Sys_LoadDll(dllName, qfalse)) &&
    ...
    Com_sprintf(dllName, sizeof(dllName),
                DEFAULT_RENDER_LIBRARY "_" ARCH_STRING DLL_EXT);
...
void CL_SetModel_f(void) {
  ...
  if (arg[0]) {
    /*
    //If you wanted to be foolproof you would put this on the server I guess.
    But that
    //tends to put things out of sync regarding cvar status. And I sort of doubt
    someone
    //is going to write a client and figure out the protocol so that they can
    annoy people
    ...
    (MODEL_CHANGE_DELAY/1000)); return;
    }

    gCLModelDelay = curTime + MODEL_CHANGE_DELAY;
    */
    // rwwFIXMEFIXME: This is currently broken and doesn't seem to work for
    // connecting clients
    Cvar_Set("model", arg);
  } else {
...
void CL_SetForcePowers_f(void) { return; }
...
static void CL_AddFavorite_f(void) {
  ...
  const char *server =
      (argc == 2) ? Cmd_Argv(1) : NET_AdrToString(clc.serverAddress);
...
void CL_Init(void) {
  ...
  cl_motd = Cvar_Get("cl_motd", "1", CVAR_ARCHIVE_ND,
                     "Display welcome message from master server on the bottom "
  ...
  for (int index = 2; index < MAX_MASTER_SERVERS; index++)
    cl_motdServer[index] =
        Cvar_Get(va("cl_motdServer%d", index + 1), "", CVAR_ARCHIVE_ND);
  ...
  cl_sensitivity =
      Cvar_Get("sensitivity", "5", CVAR_ARCHIVE, "Mouse sensitivity value");
  ...
  cl_mouseAccelStyle =
      Cvar_Get("cl_mouseAccelStyle", "0", CVAR_ARCHIVE_ND,
  ...
  cl_allowAltEnter =
      Cvar_Get("cl_allowAltEnter", "1", CVAR_ARCHIVE_ND,
  ...
  cl_enableGuid = Cvar_Get("cl_enableGuid", "1", CVAR_ARCHIVE_ND,
  ...
  cl_consoleKeys =
      Cvar_Get("cl_consoleKeys", "~ ` 0x7e 0x60 0xb2", CVAR_ARCHIVE,
  ...
  Cvar_Get("model", DEFAULT_MODEL "/default", CVAR_USERINFO | CVAR_ARCHIVE,
           "Player model");
  Cvar_Get("forcepowers", "7-1-032330000000001333",
           CVAR_USERINFO | CVAR_ARCHIVE, "Player forcepowers");
  //	Cvar_Get ("g_redTeam", DEFAULT_REDTEAM_NAME, CVAR_SERVERINFO |
  //CVAR_ARCHIVE); 	Cvar_Get ("g_blueTeam", DEFAULT_BLUETEAM_NAME,
...
void CL_Shutdown(void) {
  ...
  // RJ: added the shutdown all to close down the cgame (to free up some memory,
  // such as in the fx system)
...
qboolean CL_ConnectedToRemoteServer(void) {
  return (qboolean)(com_sv_running && !com_sv_running->integer &&
                    cls.state >= CA_CONNECTED && !clc.demoplaying);
}
...
static void CL_SetServerInfo(serverInfo_t *server, const char *info, int ping) {
  if (server) {
    if (info) {
      ...
      //			server->pure =
      //(qboolean)atoi(Info_ValueForKey(info, "pure" ));
    }
    server->ping = ping;
  }
}
...
single master server. OpenJK 2013; added support for requesting five separate
master servers using 0-4. OpenJK July 2017; made master 0 fetch all master
servers and 1-5 request a single master server.
...
void CL_GlobalServers_f(void) {
  ...
  if (!*masteraddress) {
    Com_Printf(
        "CL_GlobalServers_f: Error: No master server address given for %s.\n",
        command);
    return;
  }
