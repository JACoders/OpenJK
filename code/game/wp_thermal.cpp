...
//---------------------------------------------------------
{
  if ((ent->s.eFlags & EF_HELD_BY_SAND_CREATURE)) {
    ...
    G_Damage(ent->activator, ent, ent->owner, vec3_origin, ent->currentOrigin,
             weaponData[WP_THERMAL].altDamage, 0, MOD_EXPLOSIVE);
...
//---------------------------------------------------------
{
  ...
  if ((ent->s.eFlags &
       EF_HELD_BY_SAND_CREATURE)) { // blow once creature is underground (done
                                    // with anim)
    ...
    if (!ent->activator || !ent->activator->client ||
        !ent->activator->client->ps
             .legsAnimTimer) { // either something happened to the sand creature
                               // or it's done with it's attack anim
      // blow!
      ...
      for (int i = 0; i < count; i++) {
        if (ent_list[i]->s.number == 0) {
          ...
          break;
                   ...
                   ent_list[i]->client->NPC_class !=
                       CLASS_SAND_CREATURE // ignore sand creatures
                   && ent_list[i]->health > 0) {
          // FIXME! sometimes the ent_list order changes, so we should make sure
          // that the player isn't anywhere in this list
          blow = qtrue;
        }
      }
    }
...
//---------------------------------------------------------
{
  ...
  const qboolean thisIsAShooter =
      (qboolean)!Q_stricmp("misc_weapon_shooter", ent->classname);
  ...
  if (ent->health > 0) {
    ...
    if ((ent->NPC || (ent->s.number && thisIsAShooter)) &&
        ent->enemy) { // NPC or misc_weapon_shooter
      ...
      WP_LobFire(ent, start, target, bolt->mins, bolt->maxs, bolt->clipmask,
                 ...
                 ent->enemy->s.number);
    } else if (thisIsAShooter && ent->target &&
               !VectorCompare(
                   ent->pos1,
                   vec3_origin)) { // misc_weapon_shooter firing at a position
      WP_LobFire(ent, start, ent->pos1, bolt->mins, bolt->maxs, bolt->clipmask,
                 bolt->s.pos.trDelta, qtrue, ent->s.number,
                 ent->enemy->s.number);
    }
  }
