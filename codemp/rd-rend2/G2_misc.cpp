...
void G2_List_Model_Surfaces(const char *fileName)
{
	...
	mdxmHeader_t *mdxm = (mdxmHeader_t *)mod_m->modelData;
...
//	int				frameSize;
	mdxaHeader_t	*header = (mdxaHeader_t *)mod_a->modelData;
...
//	assert(G2_MODEL_OK(&ghoul2));
	...
	mdxmHeader_t *mdxm = (mdxmHeader_t *)ghoul2.currentModel->modelData;
					...
					CBoneCache *boneCache, const model_t *currentModel, int lod, vec3_t scale, CMiniHeap *G2VertSpace, size_t *TransformedVertArray, bool secondTimeAround)
{
	int	i;
	assert(currentModel);
	assert(currentModel->modelData);
	// back track and get the surfinfo struct for this surface
	const mdxmSurface_t			*surface = (mdxmSurface_t *)G2_FindSurface((void*)currentModel, surfaceNum, lod);
	const mdxmHierarchyOffsets_t	*surfIndexes = (mdxmHierarchyOffsets_t *)((byte *)currentModel->modelData + sizeof(mdxmHeader_t));
...
//		assert(G2_MODEL_OK(&g));
		...
		mdxmHeader_t *mdxm = (mdxmHeader_t *)g.currentModel->modelData;
...
static void G2_TraceSurfaces(CTraceSurface &TS)
{
	...
	assert(TS.currentModel->modelData);
	const mdxmSurface_t		*surface = (mdxmSurface_t *)G2_FindSurface(TS.currentModel, TS.surfaceNum, TS.lod);
	const mdxmHierarchyOffsets_t	*surfIndexes = (mdxmHierarchyOffsets_t *)((byte *)TS.currentModel->modelData + sizeof(mdxmHeader_t));
	const mdxmSurfHierarchy_t		*surfInfo = (mdxmSurfHierarchy_t *)((byte *)surfIndexes + surfIndexes->offsets[surface->thisSurfaceIndex]);
