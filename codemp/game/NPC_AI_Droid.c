...
void R2D2_PartsMove(void) {
  ...
  if (TIMER_Done(NPCS.NPC, "eyeDelay")) {
    ...
    if (NPC->genericBone1)
    {
            trap->G2API_SetBoneAnglesIndex( &NPC->ghoul2[NPC->playerModel],
    NPC->genericBone1, NPC->pos1, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
    NEGATIVE_Z, NULL );
    }
    */
...
void R2D2_TurnAnims(void) {
  ...
  turndelta =
      AngleDelta(NPCS.NPC->r.currentAngles[YAW], NPCS.NPCInfo->desiredYaw);

  if ((fabs(turndelta) > 20) && ((NPCS.NPC->client->NPC_class == CLASS_R2D2) ||
                                 (NPCS.NPC->client->NPC_class == CLASS_R5D2))) {
    anim = NPCS.NPC->client->ps.legsAnim;
    if (turndelta < 0) {
      if (anim != BOTH_TURN_LEFT1) {
        NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_TURN_LEFT1,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      }
    } else {
      if (anim != BOTH_TURN_RIGHT1) {
        NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_TURN_RIGHT1,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      }
    }
  } else {
    NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_RUN1,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  }
}
...
void Droid_Patrol(void) {
  ...
  if (NPCS.NPC->client && NPCS.NPC->client->NPC_class != CLASS_GONK) {
    if (NPCS.NPC->client->NPC_class != CLASS_R5D2) { // he doesn't have an eye.
  ...
  if (UpdateGoal()) {
    ...
    if (NPCS.NPC->client && NPCS.NPC->client->NPC_class == CLASS_MOUSE) {
      NPCS.NPCInfo->desiredYaw +=
          sin(level.time * .5) * 25; // Weaves side to side a little
  ...
  if (NPCS.NPCInfo->localState == LSTATE_BACKINGUP) {
    ...
    if (UpdateGoal()) {
      if (NPC_MoveToGoal(qfalse)) {
        NPCS.NPCInfo->desiredYaw +=
            sin(level.time * .5) * 5; // Weaves side to side a little
      }
    }
  }
...
void Droid_Spin(void) {
  ...
  if (NPCS.NPC->client->NPC_class == CLASS_R5D2 ||
    ...
    if (trap->G2API_GetSurfaceRenderStatus(NPCS.NPC->ghoul2, 0, "head") > 0) {
      if (TIMER_Done(NPCS.NPC, "smoke") &&
          !TIMER_Done(NPCS.NPC, "droidsmoketotal")) {
        TIMER_Set(NPCS.NPC, "smoke", 100);
        G_PlayEffectID(G_EffectIndex("volumetric/droid_smoke"),
                       NPCS.NPC->r.currentOrigin, dir);
      }

      if (TIMER_Done(NPCS.NPC, "droidspark")) {
        TIMER_Set(NPCS.NPC, "droidspark", Q_irand(100, 500));
        G_PlayEffectID(G_EffectIndex("sparks/spark"), NPCS.NPC->r.currentOrigin,
                       dir);
      }
      ...
      if (TIMER_Done(NPCS.NPC, "roam")) {
        ...
        NPCS.NPCInfo->desiredYaw =
            AngleNormalize360(NPCS.NPCInfo->desiredYaw + 40); // Spin around
      }
    }
  } else {
    if (TIMER_Done(NPCS.NPC, "roam")) {
...
void NPC_Droid_Pain(gentity_t *self, gentity_t *attacker, int damage) {
  ...
  if (self->client->NPC_class == CLASS_R5D2) {
    ...
    if (mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT ||
        Q_flrand(0.0f, 1.0f) <
            pain_chance) // Spin around in pain? Demp2 always does this
    {
...
static void R5D2_OffsetLook( float offset, vec3_t out )
{
        ...
        GetAnglesForDirection( NPC->r.currentOrigin, NPCInfo->investigateGoal,
angles ); angles[YAW] += offset; AngleVectors( angles, forward, NULL, NULL );
...
{
        vec3_t	lookPos;
        float	perc = (float) ( level.time - NPCInfo->pauseTime ) / (float)
