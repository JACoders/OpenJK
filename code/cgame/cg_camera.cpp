...
void CGCam_Enable(void) {
  ...
  if (&g_entities[0] && g_entities[0].client) {
    ...
    if (g_entities[0].client->ps.saberInFlight &&
        g_entities[0].client->ps.saber[0].Active()) { // saber is out
      gentity_t *saberent =
...
void CGCam_Disable(void) {
  ...
  // we just came out of camera, so update cg.refdef.vieworg out of the camera's
  // origin so the snapshot will know our new ori
...
void CGCam_ZoomAccel(float initialFOV, float fovVelocity, float fovAccel,
                     float duration) {
  if (!duration) {
    return;
  }
...
void CGCam_Update(void) {
  ...
  if (client_camera.info_state & CAMERA_ACCEL) {
    ...
    float t = (cg.time - client_camera.FOV_time) *
              0.001; // mult by 0.001 cuz otherwise t is too darned big
    ...
    if (client_camera.FOV_time + fovDuration < cg.time) {
      ...
      if (cg_roffdebug.integer) {
        Com_Printf("%d: fovaccel from %2.1f using vel = %2.4f, acc = %2.4f "
                   "(current fov calc = %5.6f)\n",
  ...
  if ((client_camera.info_state & CAMERA_ROFFING) &&
    ...
    if (client_camera.info_state & CAMERA_CUT) {
      ...
      // lerping business.
      for (i = 0; i < 3; i++) {
        cg.refdefViewAngles[i] = AngleNormalize360(
    ...
    if (client_camera.info_state & CAMERA_CUT) {
      ...
      // Note: does not actually change the camera's angles until the pan time
      // is done!
  ...
  if (client_camera.info_state & CAMERA_MOVING) {
    ...
    if (client_camera.move_time + client_camera.move_duration < cg.time) {
      ...
      if (client_camera.info_state & CAMERA_CUT) {
        // we're doing a cut, so just go to the new origin. none of this
        // fancypants lerping stuff.
        for (i = 0; i < 3; i++) {
...
void CGCam_NotetrackProcessFovAccel(const char *addlArg) {
  ...
  //
  // where 'begin fov' is initial position, 'fov delta' is velocity, and 'fov
  // delta2' is acceleration.
  char t[64];
  int d = 0, tsize = 64;
    ...
    if (addlArg[a]) {
      ...
      if (cg_roffdebug.integer) {
        Com_Printf("notetrack: 'fovaccel %2.2f %3.5f %3.5f %d' on frame %d\n",
                   beginFOV, fovDelta, fovDelta2, fovTime,
                   client_camera.roff_frame);
      }
...
static void CG_RoffNotetrackCallback(const char *notetrack) {
  ...
  */
  ...
  if (strcmp(type, "cut") == 0) {
    ...
    // this is just a really hacky way of getting a cut and a fov command on the
    // same frame
    if (addlArgs) {
    ...
    Com_Printf(
        "camera roff 'fovaccel' notetrack missing 'begin fov' argument\n",
        addlArg);
  }
}
  ...
  while (client_camera.next_roff_time <= cg.time) {
    ...
    if (roff->type == 2) {
      ...
      if (data->mStartNote != -1 || data->mNumNotes) {
        CG_RoffNotetrackCallback(
            roffs[roff_id - 1].mNoteTrackIndexes[data->mStartNote]);
      }
    } else {
      move_rotate_t *data =
    ...
    if (cg_developer.integer) {
      Com_Printf(S_COLOR_GREEN
                 "CamROFF: frame: %d o:<%.2f %.2f %.2f> a:<%.2f %.2f %.2f>\n",
