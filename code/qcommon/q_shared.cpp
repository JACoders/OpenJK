...
void COM_EndParseSession(void) {
  parseDataCount--;
#ifdef _DEBUG
  assert(parseDataCount >= -1 && "COM_EndParseSession: called without a "
                                 "starting COM_BeginParseSession.\n");
#endif
}

int COM_GetCurrentParseLine(int index) {
  if (parseDataCount < 0)
    Com_Error(ERR_FATAL, "COM_GetCurrentParseLine: parseDataCount < 0 (be sure "
                         "to call COM_BeginParseSession!)");
...
char *COM_ParseExt(const char **data_p, qboolean allowLineBreaks) {
  ...
  if (parseDataCount < 0)
    Com_Error(ERR_FATAL, "COM_ParseExt: parseDataCount < 0 (be sure to call "
...
char *QDECL va(const char *format, ...) {
  va_list argptr;
  static char string[MAX_VA_BUFFERS]
                    [MAX_VA_STRING]; // in case va is called by nested functions
  static int index = 0;
  char *buf;
...
qboolean Q_InBitflags(const uint32_t *bits, int index, uint32_t bitsPerByte) {
  return (bits[index / bitsPerByte] & (1 << (index % bitsPerByte))) ? qtrue
                                                                    : qfalse;
}
