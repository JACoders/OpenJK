...
void CG_TestG2Model_f(void) {
  ...
  cg.testModel = gi.G2API_InitGhoul2Model(
      *((CGhoul2Info_v *)cg.testModelEntity.ghoul2), cg.testModelName,
      cg.testModelEntity.hModel, NULL_HANDLE, NULL_HANDLE, 0, 0);
...
void CG_TestModelAnimate_f(void) {
  ...
  gi.G2API_SetBoneAnim(&ghoul2[cg.testModel], boneName, atoi(CG_Argv(2)),
                       atoi(CG_Argv(3)), BONE_ANIM_OVERRIDE_LOOP,
...
cg.refdef.vieworg
--at the start of the function holds the player actor's origin (center of player
model).
--it is set to the final view location of the camera at the end of the camera
code. cg.refdefViewAngles
...
static void CG_CalcIdealThirdPersonViewTarget(void) {
  ...
  qboolean usesViewEntity = (qboolean)(
      cg.snap->ps.viewEntity && cg.snap->ps.viewEntity < ENTITYNUM_WORLD);
  ...
  if (usesViewEntity) {
    ...
    if (gent->client && (gent->client->NPC_class == CLASS_GONK ||
                         ...
                         gent->client->NPC_class ==
                             CLASS_R5D2)) { // Droids use a generic offset
      cameraFocusLoc[2] += 4;
      VectorCopy(cameraFocusLoc, cameraIdealTarget);
      return;
    }

    if (gent->client->ps.pm_flags &
        PMF_DUCKED) { // sort of a nasty hack in order to get this to work.
                      // Don't tell Ensiform, or I'll have to kill him. --eez
  ...
  if (cg.snap && (cg.snap->ps.eFlags & EF_HELD_BY_SAND_CREATURE)) {
    ...
    // Add in a vertical offset from the viewpoint, which puts the actual target
    // above the head, regardless of angle.
    VectorCopy(cameraFocusLoc, cameraIdealTarget);
    cameraIdealTarget[2] += cg.overrides.thirdPersonVertOffset;
    // VectorMA(cameraFocusLoc, cg.overrides.thirdPersonVertOffset, cameraup,
    // cameraIdealTarget);
  } else {
    // Add in a vertical offset from the viewpoint, which puts the actual target
    // above the head, regardless of angle.
    ...
    // VectorMA(cameraFocusLoc, cg_thirdPersonVertOffset.value, cameraup,
    // cameraIdealTarget);
  }

  // Now, if the player is crouching, do a little special tweak.  The problem is
  // that the player's head is way out of his bbox.
  if (cg.predicted_player_state.pm_flags &
      PMF_DUCKED) { // Nudge to focus location up a tad.
    vec3_t nudgepos;
    trace_t trace;
    ...
    CG_Trace(&trace, cameraFocusLoc, cameramins, cameramaxs, nudgepos,
             (usesViewEntity) ? cg.snap->ps.viewEntity
...
static void CG_CalcIdealThirdPersonViewLocation(void) {
  if (cg.overrides.active & CG_OVERRIDE_3RD_PERSON_RNG) {
    VectorMA(cameraIdealTarget, -(cg.overrides.thirdPersonRange), camerafwd,
             cameraIdealLoc);
  } else if (cg.snap && (cg.snap->ps.eFlags & EF_HELD_BY_RANCOR) &&
             cg_entities[cg.snap->ps.clientNum].gent->activator) { // stay back
    VectorMA(
        cameraIdealTarget,
        -180.0f *
            cg_entities[cg.snap->ps.clientNum].gent->activator->s.modelScale[0],
        camerafwd, cameraIdealLoc);
  } else if (cg.snap && (cg.snap->ps.eFlags & EF_HELD_BY_WAMPA) &&
             cg_entities[cg.snap->ps.clientNum].gent->activator &&
             cg_entities[cg.snap->ps.clientNum]
                 .gent->activator->inuse) { // stay back
    VectorMA(
        cameraIdealTarget,
        -120.0f *
            cg_entities[cg.snap->ps.clientNum].gent->activator->s.modelScale[0],
        camerafwd, cameraIdealLoc);
             ...
             cg_entities[cg.snap->ps.clientNum].gent->activator) { // stay back
    VectorMA(cg_entities[cg_entities[cg.snap->ps.clientNum]
                             .gent->activator->s.number]
                 .lerpOrigin,
             -180.0f, camerafwd, cameraIdealLoc);
  } else {
    VectorMA(cameraIdealTarget, -(cg_thirdPersonRange.value), camerafwd,
             cameraIdealLoc);
  }
  ...
  if (cg.renderingThirdPerson &&
    ...
    if (timeLeft < 500) { // start going back
      VectorMA(cameraIdealLoc, (timeLeft) / 500 * amt, camerafwd,
               cameraIdealLoc);
    } else if (length - timeLeft < 1000) { // start zooming in
      VectorMA(cameraIdealLoc, (length - timeLeft) / 1000 * amt, camerafwd,
               cameraIdealLoc);
    } else {
...
static void CG_ResetThirdPersonViewDamp(void) {
  ...
  // First thing we do is trace from the first person viewpoint out to the new
  // target location.
  CG_Trace(&trace, cameraFocusLoc, cameramins, cameramaxs, cameraCurTarget,
           cg.predicted_player_state.clientNum, MASK_CAMERACLIP);
  if (trace.fraction <= 1.0) {
    VectorCopy(trace.endpos, cameraCurTarget);
  }

  // Now we trace from the new target location to the new view location, to make
  // sure there is nothing in the way.
  CG_Trace(&trace, cameraCurTarget, cameramins, cameramaxs, cameraCurLoc,
           cg.predicted_player_state.clientNum, MASK_CAMERACLIP);
  if (trace.fraction <= 1.0) {
...
static void CG_UpdateThirdPersonTargetDamp(void) {
  ...
  {                                                 // No damping.
    ...
    // Now we calculate how much of the difference we cover in the time
    // allotted. The equation is (Damp)^(time)
    dampfactor =
        1.0 -
        cg_thirdPersonTargetDamp.value; // We must exponent the amount LEFT
                                        // rather than the amount bled off
    dtime =
        (float)(cg.time - cameraLastFrame) * (1.0 / cg_timescale.value) *
        (1.0 / (float)CAMERA_DAMP_INTERVAL); // Our dampfactor is geared towards
                                             // a time interval equal to "1".

    // Note that since there are a finite number of "practical" delta
    // millisecond values possible, the ratio should be initialized into a chart
    // ultimately.
  ...
  if (cg.snap && (cg.snap->ps.eFlags & EF_HELD_BY_SAND_CREATURE) &&
      cg_entities[cg.snap->ps.clientNum]
          .gent->activator) { // if being held by a sand creature, trace from
                              // his actual origin, since we could be underground
                              // or otherwise in solid once he eats us
    CG_Trace(&trace,
             cg_entities[cg_entities[cg.snap->ps.clientNum]
                             .gent->activator->s.number]
                 .lerpOrigin,
             cameramins, cameramaxs, cameraCurTarget,
             cg.predicted_player_state.clientNum, MASK_CAMERACLIP);
  } else {
    CG_Trace(&trace, cameraFocusLoc, cameramins, cameramaxs, cameraCurTarget,
             cg.predicted_player_state.clientNum, MASK_CAMERACLIP);
  ...
  // Note that previously there was an upper limit to the number of physics
  // traces that are done through the world for the sake of camera collision,
  // since it wasn't calced per frame.  Now it is calculated every frame. This
  // has the benefit that the camera is a lot smoother now (before it lerped
  // between tested points), however two full volume traces each frame is a bit
...
static void CG_UpdateThirdPersonCameraDamp(void) {
  ...
  // First thing we do is calculate the appropriate damping factor for the
  // camera.
  dampfactor = 0.0f;
  if (CG_OnMovingPlat(&cg.snap->ps)) { // if moving on a plat, camera is *tight*
    dampfactor = 1.0f;
    ...
    if (cg.overrides.thirdPersonCameraDamp != 0.0f) {
      ...
      // The higher the pitch, the larger the factor, so as you look up, it
      // damps a lot less.
      pitch /= 115.0f;
    ...
    // The higher the pitch, the larger the factor, so as you look up, it damps
    // a lot less.
    pitch /= 115.0f;
    ...
    // Now we also multiply in the stiff factor, so that faster yaw changes are
    // stiffer.
    if (cameraStiffFactor >
        0.0f) { // The cameraStiffFactor is how much of the remaining damp below
                // 1 should be shaved off, i.e. approach 1 as stiffening
                // increases.
  ...
  {                      // No damping.
    ...
    // Now we calculate how much of the difference we cover in the time
    // allotted. The equation is (Damp)^(time)
    dampfactor = 1.0 - dampfactor; // We must exponent the amount LEFT rather
                                   // than the amount bled off
    dtime =
        (float)(cg.time - cameraLastFrame) * (1.0 / cg_timescale.value) *
        (1.0 / (float)CAMERA_DAMP_INTERVAL); // Our dampfactor is geared towards
                                             // a time interval equal to "1".
  ...
  // Now we trace from the first person viewpoint to the new view location, to
  // make sure there is nothing in the way between the user and the camera...
  //	CG_Trace(&trace, cameraFocusLoc, cameramins, cameramaxs, cameraCurLoc,
  //cg.predicted_player_state.clientNum, MASK_CAMERACLIP);
  // (OLD) Now we trace from the new target location to the new view location,
  // to make sure there is nothing in the way.
  if (cg.snap && (cg.snap->ps.eFlags & EF_HELD_BY_SAND_CREATURE) &&
      cg_entities[cg.snap->ps.clientNum]
          .gent->activator) { // if being held by a sand creature, trace from
                              // his actual origin, since we could be underground
    ...
    CG_Trace(&trace,
             cg_entities[cg_entities[cg.snap->ps.clientNum]
                             .gent->activator->s.number]
                 .lerpOrigin,
             cameramins, cameramaxs, cameraCurLoc,
             cg.predicted_player_state.clientNum, MASK_CAMERACLIP);
  } else {
    CG_Trace(&trace, cameraCurTarget, cameramins, cameramaxs, cameraCurLoc,
             cg.predicted_player_state.clientNum, MASK_CAMERACLIP);
  ...
  if (trace.fraction < 1.0f) {
    ...
    //		VectorMA(cameraFocusLoc, trace.fraction, locdiff,
    //cameraCurTarget);
    ...
    if ( trace.entityNum != ENTITYNUM_WORLD )
    {
            ...
            {
                    if ( cent->currentState.pos.trType == TR_LINEAR ||
    cent->currentState.pos.trType == TR_LINEAR_STOP )
                    {
                            vec3_t	diff;
                            VectorSubtract( cent->lerpOrigin,
    gent->currentOrigin, diff ); VectorAdd( cameraCurLoc, diff, cameraCurLoc );
                    }
            }
    }
    */
...
static void CG_OffsetThirdPersonView(void) {
  ...
  if (cg.snap && (cg.snap->ps.eFlags & EF_HELD_BY_RANCOR) &&
      cg_entities[cg.snap->ps.clientNum].gent->activator) {
    centity_t *monster = &cg_entities[cg_entities[cg.snap->ps.clientNum]
                                          .gent->activator->s.number];
    VectorSet(cameraFocusAngles, 0,
              AngleNormalize180(monster->lerpAngles[YAW] + 180), 0);
  } else if (cg.snap && (cg.snap->ps.eFlags & EF_HELD_BY_SAND_CREATURE)) {
    centity_t *monster = &cg_entities[cg_entities[cg.snap->ps.clientNum]
                                          .gent->activator->s.number];
    VectorSet(cameraFocusAngles, 0,
  ...
  } else if (cg.predicted_player_state.stats[STAT_HEALTH] <=
             0) { // if dead, look at killer
    if (MatrixMode) {
      if (cg.overrides.active & CG_OVERRIDE_3RD_PERSON_ANG) {
        cameraFocusAngles[YAW] += cg.overrides.thirdPersonAngle;
      } else {
  ...
  if (!cg.renderingThirdPerson &&
      (cg.snap->ps.weapon == WP_SABER ||
       cg.snap->ps.weapon == WP_MELEE)) { // First person saber
  ...
  // The next thing to do is to see if we need to calculate a new camera target
  // location.

  // If we went back in time for some reason, or if we just started, reset the
  // sample.
  if (cameraLastFrame == 0 || cameraLastFrame > cg.time) {
    ...
    if (deltayaw >
        180.0f) { // Normalize this angle so that it is between 0 and 180.
  ...
  // Now interestingly, the Quake method is to calculate a target focus point
  // above the player, and point the camera at it. We won't do that for now.
  ...
  if (dist < 1.0f) { // must be hitting something, need some value to calc
                     // angles, so use cam forward
  ...
  // ...and of course we should copy the new view location to the proper spot
  // too.
...
static void CG_OffsetThirdPersonView( void ) {
        ...
        if ( cg.predicted_player_state.stats[STAT_HEALTH] <= 0 ) {
                focusAngles[YAW] =
cg.predicted_player_state.stats[STAT_DEAD_YAW]; cg.refdefViewAngles[YAW] =
cg.predicted_player_state.stats[STAT_DEAD_YAW];
        }

        if ( focusAngles[PITCH] > 45 ) {
                focusAngles[PITCH] = 45;		// don't go too far
overhead
        }
        ...
        float tpAngle = cg.overrides.thirdPersonAngle ?
cg.overrides.thirdPersonAngle : cg_thirdPersonAngle.value; forwardScale = cos(
tpAngle / 180 * M_PI ); sideScale = sin( tpAngle / 180 * M_PI ); VectorMA( view,
-tpAngle * forwardScale, forward, view ); VectorMA( view, -tpAngle * sideScale,
right, view );
...
cg.predicted_player_state.clientNum, MASK_CAMERACLIP );
        ...
        if ( trace.fraction != 1.0 ) {
                ...
                // try another trace to this position, because a tunnel may have
the ceiling
                // close enogh that this is poking out

                CG_Trace( &trace, cg.refdef.vieworg, mins, maxs, view,
cg.predicted_player_state.clientNum, MASK_CAMERACLIP ); VectorCopy(
trace.endpos, view );
        }
        ...
        focusDist = sqrt( focusPoint[0] * focusPoint[0] + focusPoint[1] *
focusPoint[1] ); if ( focusDist < 1 ) { focusDist = 1;	// should never happen
        }
        cg.refdefViewAngles[PITCH] = -180 / M_PI * atan2( focusPoint[2],
focusDist ); cg.refdefViewAngles[YAW] -= tpAngle;
}
...
static void CG_OffsetThirdPersonOverheadView( void ) {
        ...
        // Move straight up from the player, making sure to always go at least
the min camera height,
        //	otherwise, the camera will clip into the head of the player.
        float tpRange = cg.overrides.thirdPersonRange ?
cg.overrides.thirdPersonRange : cg_thirdPersonRange.value; if ( tpRange <
MIN_CAMERA_HEIGHT )
        {
        ...
        // Now adjust the camera angles, but we shouldn't adjust the
viewAngles...only the viewAxis VectorCopy( cg.refdefViewAngles, angs );
        angs[PITCH] = cg.overrides.thirdPersonAngle ?
cg.overrides.thirdPersonAngle : cg_thirdPersonAngle.value;
        ...
        // Convert our new desired camera angles and store them where they will
get used by the engine
...
static void CG_OffsetFirstPersonView(qboolean firstPersonSaber) {
  ...
  if (g_entities[0].client && PM_InKnockDown(&g_entities[0].client->ps)) {
    float perc, animLen = (float)PM_AnimLength(
    ...
    if (PM_InGetUp(&g_entities[0].client->ps) ||
        PM_InForceGetUp(&g_entities[0].client->ps)) { // start righting the view
  ...
  if (cg.snap->ps.leanofs != 0) {
    ...
    VectorMA(cg.refdef.vieworg, (float)cg.snap->ps.leanofs, right,
             cg.refdef.vieworg);
  }
...
qboolean CG_CalcFOVFromX(float fov_x) {
  ...
  if (cg_fovAspectAdjust.integer) {
    ...
    fov_x = atan(tan(desiredFov * M_PI / 360.0f) * baseAspect * aspect) *
            360.0f / M_PI;
  }
  ...
  // there's a problem with this, it only takes the leafbrushes into account,
  // not the entity brushes,
  //	so if you give slime/water etc properties to a func_door area brush in
  //order to move the whole water 	level up/down this doesn't take into account
  //the door position, so warps the view the whole time 	whether the water is up
  //or not.
  // warp if underwater
  float phase;
  float v;
  ...
  // see if we are drugged by an interrogator.  We muck with the FOV here, a bit
  // later, after viewangles are calc'ed, I muck with those too.
  if (cg.wonkyTime > 0 && cg.wonkyTime > cg.time) {
    float perc =
        (float)(cg.wonkyTime - cg.time) / 10000.0f; // goes for 10 seconds
...
float CG_ForceSpeedFOV(void) {
  float fov;
  float timeLeft = player->client->ps.forcePowerDuration[FP_SPEED] - cg.time;
  float length = FORCE_SPEED_DURATION *
                 forceSpeedValue[player->client->ps.forcePowerLevel[FP_SPEED]];
...
static qboolean CG_CalcFov(void) {
  ...
  if (cg.predicted_player_state.pm_type == PM_INTERMISSION) {
    ...
    fov_x = 80;
  } else if (cg.snap && cg.snap->ps.viewEntity > 0 &&
             cg.snap->ps.viewEntity < ENTITYNUM_WORLD &&
             (!cg.renderingThirdPerson ||
              g_entities[cg.snap->ps.viewEntity].e_DieFunc ==
                  dieF_camera_die)) {
    // if in entity camera view, use a special FOV
    if (&g_entities[cg.snap->ps.viewEntity] &&
        g_entities[cg.snap->ps.viewEntity]
            .NPC) { // FIXME: looks bad when take over a jedi... but never
      ...
      if (cg.overrides.active & CG_OVERRIDE_FOV) {
        ...
        fov_x = 120; // FIXME: read from the NPC's fov stats?
             ...
             (cg.snap->ps.forcePowersActive & (1 << FP_SPEED)) &&
             player->client->ps
                 .forcePowerDuration[FP_SPEED]) // cg.renderingThirdPerson &&
  {
    ...
    if (cg.zoomMode &&
        cg.zoomMode < 3) //&& !cg.renderingThirdPerson ) // light amp goggles do
                         //none of the zoom silliness
    {
      if (!cg.zoomLocked) {
        if (cg.zoomMode == 1) {
        ...
        // Clamp zoomFov
        float actualFOV = (cg.overrides.active & CG_OVERRIDE_FOV)
                              ? cg.overrides.fov
                              : cg_fov.value;
        if (cg_zoomFov < MAX_ZOOM_FOV) {
          ...
          if (zoomSoundTime < cg.time) {
            ...
            cgi_S_StartSound(cg.refdef.vieworg, ENTITYNUM_WORLD, CHAN_LOCAL,
                             snd);
...
void CG_SaberClashFlare(void) {
  ...
  CG_Trace(&tr, cg.refdef.vieworg, NULL, NULL, g_saberFlashPos, -1,
           CONTENTS_SOLID);
...
static qboolean CG_CalcViewValues(void) {
  ...
  if (G_IsRidingVehicle(&g_entities[0])) {
    VectorCopy(ps->origin, cg.refdef.vieworg);
    VectorCopy(cg_entities[g_entities[0].owner->s.number].lerpAngles,
               cg.refdefViewAngles);
    if (!(ps->eFlags & EF_NODRAW)) { // riding it, not *inside* it
      // let us look up & down
      cg.refdefViewAngles[PITCH] =
          cg_entities[ps->clientNum].lerpAngles[PITCH] * 0.2f;
    }
  } else if (cg.snap->ps.viewEntity > 0 &&
             cg.snap->ps.viewEntity <
                 ENTITYNUM_WORLD) { // in an entity camera view
    /*
    ...
    */
    {
      VectorCopy(cg_entities[cg.snap->ps.viewEntity].lerpOrigin,
                 cg.refdef.vieworg);
    }
    VectorCopy(cg_entities[cg.snap->ps.viewEntity].lerpAngles,
               cg.refdefViewAngles);
    if (!Q_stricmp("misc_camera",
                   g_entities[cg.snap->ps.viewEntity].classname) ||
        g_entities[cg.snap->ps.viewEntity].s.weapon == WP_TURRET) {
      viewEntIsCam = qtrue;
    }
  } else if (cg.renderingThirdPerson && !cg.zoomMode &&
             (cg.overrides.active &
              CG_OVERRIDE_3RD_PERSON_ENT)) { // different center, same angle
    VectorCopy(cg_entities[cg.overrides.thirdPersonEntity].lerpOrigin,
               cg.refdef.vieworg);
    VectorCopy(ps->viewangles, cg.refdefViewAngles);
  ...
  if ((cg.renderingThirdPerson || cg.snap->ps.weapon == WP_SABER ||
    ...
    if (playerCent && playerCent->gent && playerCent->gent->client) {
      VectorCopy(cg.refdef.vieworg,
                 playerCent->gent->client->renderInfo.eyePoint);
      VectorCopy(cg.refdefViewAngles,
                 playerCent->gent->client->renderInfo.eyeAngles);
      if (cg.snap->ps.viewEntity > 0 &&
          cg.snap->ps.viewEntity <
              ENTITYNUM_WORLD) { // in an entity camera view
        if (cg_entities[cg.snap->ps.viewEntity]
                .gent->client) { // looking through a client's eyes
          VectorCopy(cg.refdef.vieworg, cg_entities[cg.snap->ps.viewEntity]
                                            .gent->client->renderInfo.eyePoint);
          VectorCopy(cg.refdefViewAngles,
                     cg_entities[cg.snap->ps.viewEntity]
                         .gent->client->renderInfo.eyeAngles);
        } else { // looking through a regular ent's eyes
          VectorCopy(cg.refdef.vieworg,
                     cg_entities[cg.snap->ps.viewEntity].lerpOrigin);
          VectorCopy(cg.refdefViewAngles,
                     cg_entities[cg.snap->ps.viewEntity].lerpAngles);
        }
      }
      VectorCopy(playerCent->gent->client->renderInfo.eyePoint,
                 playerCent->gent->client->renderInfo.headPoint);
      if (cg.snap->ps.viewEntity <= 0 ||
          cg.snap->ps.viewEntity >= ENTITYNUM_WORLD) { // not in entity cam
  ...
  if ( in_camera )
  {
          Com_Printf( "%s %s\n", vtos(client_camera.origin),
  vtos(cg.refdef.vieworg) );
  }
  */

  // see if we are drugged by an interrogator.  We muck with the angles here,
  // just a bit earlier, we mucked with the FOV
  if (cg.wonkyTime > 0 && cg.wonkyTime > cg.time) {
    float perc =
        (float)(cg.wonkyTime - cg.time) / 10000.0f; // goes for 10 seconds
    ...
    cg.refdefViewAngles[PITCH] +=
        (26.0f * perc + sin(cg.time * 0.0011f) * 3.0f * perc);
  }
...
static void CG_PowerupTimerSounds(void) {
  ...
  for (i = 0; i < MAX_POWERUPS; i++) {
    ...
    if (time > 0 && time < cg.time) {
      ...
      //				cgi_S_StartSound( NULL,
      //cg.snap->ps.clientNum, CHAN_ITEM, cgs.media.overchargeEndSound ); 				break;
      //			}
    }
  }
}
...
static void CG_DrawSkyBoxPortal(void) {
  ...
  token = COM_ParseExt(&cstr, qfalse);
  if (!token || !token[0]) {
    CG_Error("CG_DrawSkyBoxPortal: error parsing skybox configstring.  No fog "
             "state\n");
  } else {
    if (atoi(token)) {
      // this camera has fog
  ...
  COM_EndParseSession();
  /*
          static float lastfov = cg_zoomFov;	// for transitions back from
     zoomed in modes float fov_x; float fov_y; float x; float f = 0;

          if ( cg.predicted_player_state.pm_type == PM_INTERMISSION )
          {
                  // if in intermission, use a fixed value
                  fov_x = (cg.overrides.active&CG_OVERRIDE_FOV) ?
     cg.overrides.fov : cg_fov.value;
          ...
          else
          {
                  ...
                  {
                          fov_x = (cg.overrides.active&CG_OVERRIDE_FOV) ?
     cg.overrides.fov : cg_fov.value; if ( fov_x < 1 )
                          {
                                  fov_x = 1;
                          }
                          else if ( fov_x > 160 )
     ...
     cg_zoomFov);
          ...
          cg.refdef.fov_y = fov_y;
  */
  // inherit fov and axis from whatever the player is doing (regular, camera
  // overrides or zoomed, whatever)
  if (!cg.hyperspace) {
    CG_AddPacketEntities(
        qtrue); // rww - There was no proper way to put real entities inside the
                // portal view before. This will put specially flagged entities
                // in the render.
  ...
  cgi_CM_SnapPVS(
      cg.refdef.vieworg,
      cg.refdef.areamask); // fill in my areamask for this view origin
  // draw the skybox
...
void CG_RunEmplacedWeapon() {
  ...
  if (player && gun && !gun->bounceCount // not an eweb
    ...
    //		cg.refdefViewAngles[PITCH] += cg.overrides.thirdPersonPitchOffset?
    //cg.overrides.thirdPersonPitchOffset: cg_thirdPersonPitchOffset.value;
    //		cg.refdefViewAngles[YAW] += cg.overrides.thirdPersonAngle ?
    //cg.overrides.thirdPersonAngle : cg_thirdPersonAngle.value;;
...
static qboolean cg_rangedFogging =
    qfalse; // so we know if we should go back to normal fog
void CG_DrawActiveFrame(int serverTime, stereoFrame_t stereoView) {
  ...
  // FIXME: should really send forcePowersActive over network onto
  // cg.snap->ps...
  const int fpActive = cg_entities[0].gent->client->ps.forcePowersActive;
  const bool matrixMode = !!(fpActive & ((1 << FP_SPEED) | (1 << FP_RAGE)));
  float speed =
      cg.refdef.fov_y / 75.0 * (matrixMode ? 1.0f : cg_timescale.value);

  // FIXME: junk code, BUG:168

  static bool wasForceSpeed = false;
  bool isForceSpeed =
      cg_entities[0].gent->client->ps.forcePowersActive & (1 << FP_SPEED)
          ? true
          : false;
  ...
  if (cg.snap->ps.clientNum == 0 &&
      cg_scaleVehicleSensitivity.integer) { // pointless check, but..
  ...
  if (cg.zoomMode) { // zooming with binoculars or sniper, set the fog range
                     // based on the zoom level -rww
  ...
  if (!cg.hyperspace) {
    CG_AddPacketEntities(
        qfalse); // adter calcViewValues, so predicted player state is correct
    CG_AddMarks();
    CG_AddLocalEntities();
    CG_DrawMiscEnts();
  }
  ...
  if (0) {
    ...
    camTest[2] += 6;
    if (!(CG_PointContents(camTest, 0) & CONTENTS_SOLID) &&
        !gi.inPVS(cg.refdef.vieworg,
                  camTest)) { // crossed visible line into another room
      ...
      camTest[2] -= 6;
      if (!(CG_PointContents(camTest, 0) & CONTENTS_SOLID) &&
          !gi.inPVS(cg.refdef.vieworg, camTest)) {
        cg.refdef.vieworg[2] += 6;
        // cgi_CM_SnapPVS(cg.refdef.vieworg,cg.snap->areamask);
      } else // if ( inwater )
      { // extra-special hack... sometimes when crouched in water with first
        // person lightsaber, your PVS is wrong???
        /*
        if ( !cg.renderingThirdPerson && (cg.snap->ps.weapon ==
  ...
  // if ( !VectorCompare2( cg.refdef.vieworg, cg.snap->ps.serverViewOrg ) &&
  // !gi.inPVS( cg.refdef.vieworg, cg.snap->ps.serverViewOrg ) )
  { // actual view org and server's view org don't match and aren't same PVS,
    // rebuild the areamask
    // Com_Printf( S_COLOR_RED"%s != %s\n", vtos(cg.refdef.vieworg),
    // vtos(cg.snap->ps.serverViewOrg) );
    cgi_CM_SnapPVS(cg.refdef.vieworg, cg.snap->areamask);
  }

  // Don't draw the in-view weapon when in camera mode
  if (!in_camera && !cg_pano.integer && cg.snap->ps.weapon != WP_SABER &&
    ...
    if (g_entities[cg.snap->ps.viewEntity].client &&
      ...
      CG_AddViewWeapon(&g_entities[cg.snap->ps.viewEntity]
                            .client->ps); // HAX - because I wanted to --eez
    }
  }
  ...
  // NOTE: if we want to make you be able to hear far away sounds with
  // electrobinoculars, add the hacked-in positional offset here (base on fov)
  /*
  ...
  {//FIXME: if I can see through walls, why not actually move the
  listener_position through the wall too? if ( g_crosshairEntNum <
  ENTITYNUM_WORLD
          ...
          {//been looking at this guy for at least a full second
                  ...
                  {//a living client, let's listen in...?
                          //FIXME: lerp our listener_origin to their position
  over a second or two? VectorCopy( cg_entities[g_crosshairEntNum].lerpOrigin,
  listener_origin ); inwater = qtrue;//do cool reverby effect...?
                  }
          }
  }
  cgi_S_Respatialize( cg.snap->ps.clientNum, listener_origin,
  cg.refdef.viewaxis, inwater );
  */
  cgi_S_Respatialize(cg.snap->ps.clientNum, cg.refdef.vieworg,
                     cg.refdef.viewaxis, inwater);
  ...
  if (cg_pano.integer) {  // let's grab a panorama!
    ...
    cg.refdefViewAngles[YAW] =
        -360 * cg_pano.integer / cg_panoNumShots.integer; // choose angle
    AnglesToAxis(cg.refdefViewAngles, cg.refdef.viewaxis);
