...
static const save_field_t savefields_gEntity[] = {{strFOFS(client), F_GCLIENT},
...
static const save_field_t savefields_LevelLocals[] = {{strLLOFS(locationHead), F_GENTITY},
													  {strLLOFS(alertEvents), F_ALERTEVENT},
													  {strLLOFS(groups), F_AIGROUPS},
													  {strLLOFS(knownAnimFileSets), F_ANIMFILESETS},
													  {NULL, 0, F_IGNORE}};

static const save_field_t savefields_gVHIC[] = {{strVHICOFS(m_pPilot), F_GENTITY},
...
static char *GetStringPtr(int iStrlen, char *psOriginal /*may be NULL*/) {
	if (iStrlen != -1) {
		...
		if (psOriginal && gi.bIsFromZone(psOriginal, TAG_G_ALLOC)) {
			if (!strcmp(psOriginal, sString)) { // it's a legal ptr and they're the same so let's just reuse it instead of free/alloc
				return psOriginal;
			}
			gi.Free(psOriginal);
		}
	...
	case F_ANIMFILESETS: {
		...
		for (int i = 0; i < MAX_ANIM_FILES; i++) {
			for (int j = 0; j < MAX_ANIM_EVENTS; j++) {
				byteAlias_t *baTorso = (byteAlias_t *)&p[i].torsoAnimEvents[j].stringData, *baLegs = (byteAlias_t *)&p[i].legsAnimEvents[j].stringData;
...
template <typename T> static void EnumerateFields(const save_field_t *pFields, const T *src_instance, unsigned int ulChid) {
	strList.clear();
	...
	for (const auto &it : strList) {
		saved_game.write_chunk(INT_ID('S', 'T', 'R', 'G'), it.c_str(), static_cast<int>(it.length() + 1));
	}
}
...
static void copy_retail_gclient_to_current(const RetailGClient &src, gclient_t &dst) {
	...
	::memcpy(reinterpret_cast<char *>(&dst), reinterpret_cast<const char *>(&src), src_pre_size);

	for (int i = 0; i < MAX_SABERS; ++i) {
		src.ps.saber[i].sg_export(dst.ps.saber[i]);
	}

	::memcpy(reinterpret_cast<char *>(&dst) + src_post_offset, reinterpret_cast<const char *>(&src) + dst_post_offset, src_post_size);
}

template <typename T> static void EvaluateFields(const save_field_t *pFields, T *pbData, byte *pbOriginalRefData, unsigned int ulChid) {
	T &instance = *pbData;
	...
	if (ulChid != INT_ID('G', 'C', 'L', 'I')) {
		...
		if (!saved_game.try_read_chunk(ulChid, instance)) {
			...
			if (saved_game.try_read(retail_client)) {
				copy_retail_gclient_to_current(retail_client, *reinterpret_cast<gclient_t *>(pbData));
			} else {
...
qboolean GameAllowedToSaveHere(void) { return (qboolean)(!in_camera && !killPlayerTimer); }
