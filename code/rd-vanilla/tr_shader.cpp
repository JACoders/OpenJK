...
#define MAX_IMAGE_ANIMATIONS 32
			...
			while (1) {
				...
				if (num < MAX_IMAGE_ANIMATIONS) {
					images[num] =
						R_FindImageFile(token, (qboolean)!shader.noMipMaps, (qboolean)!shader.noPicMip, (qboolean)!shader.noTC, bClamp ? GL_CLAMP : GL_REPEAT);
					if (!images[num]) {
						ri.Printf(PRINT_WARNING, "WARNING: R_FindImageFile could not find '%s' in shader '%s'\n", token, shader.name);
						return qfalse;
					}
	...
	if (stage->rgbGen == CGEN_BAD) {
		if ( // blendSrcBits == 0 ||
			blendSrcBits == GLS_SRCBLEND_ONE || blendSrcBits == GLS_SRCBLEND_SRC_ALPHA) {
			stage->rgbGen = CGEN_IDENTITY_LIGHTING;
		} else {
			stage->rgbGen = CGEN_IDENTITY;
		}
...
static qboolean ParseShader(const char **text) {
	...
	// precalculated value.
	if (shaderHash == RETAIL_ARROW_W_SHADER_HASH && Q_stricmp(shader.name, "gfx/menus/radar/arrow_w") == 0) {
	...
	for (stage = 0; stage < MAX_SHADER_STAGES;) {
		...
		if ((pStage->stateBits & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) == (GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE)) {
			...
			pStage->mGLFogColorOverride = GLFOGOVERRIDE_WHITE;
				   ...
				   stages[stage + 1].bundle[0].isLightmap) { // multiple light map blending
			pStage->mGLFogColorOverride = GLFOGOVERRIDE_WHITE;
			...
			pStage->mGLFogColorOverride = GLFOGOVERRIDE_WHITE;
				   ...
				   (GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ZERO)) { // I don't know, it works. -rww
			pStage->mGLFogColorOverride = GLFOGOVERRIDE_BLACK;
		} else if ((pStage->stateBits & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) ==
				   (GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE_MINUS_SRC_COLOR)) { // I don't know, it works. -rww
			pStage->mGLFogColorOverride = GLFOGOVERRIDE_BLACK;
		} else {
...
shader_t *R_FindShader(const char *name, const int *lightmapIndex, const byte *styles, qboolean mipRawImage) {
	...
	*/
	...
	if (!image) {
		if (strncmp(name, "levelshots", 10) && strcmp(name, "*off")) { // hide these warnings
