...
#define ACCURATE_LIGHTGRID_SAMPLING 1
  ...
  for (i = 0; i < 8; i++) {
    ...
    if (r_debugLight->integer && ent->e.hModel == -1) {
      ...
      refEnt.radius =
          factor * 50 + 2.0f; // maybe always give it a minimum size?
      ...
      VectorMA(gridOrg, (factor * -255) - 2.0f, normal,
               refEnt.origin); // maybe always give it a minimum length
      refEnt.radius = 1.5f;
      RE_AddRefEntityToScene(&refEnt);
    }
#endif
