...
static void CL_Cvar_Get(const char *var_name, const char *value,
                        uint32_t flags) {
...
static void Key_KeynumToStringBuf(int keynum, char *buf, int buflen) {
  ...
  //
  const char *psKeyNameFriendly =
      SE_GetString(va("KEYNAMES_KEYNAME_%s", psKeyName));

  Q_strncpyz(buf,
             (psKeyNameFriendly && psKeyNameFriendly[0]) ? psKeyNameFriendly
                                                         : psKeyName,
             buflen);
}
...
static qboolean CL_G2API_AttachG2Model(void *ghoul2From, int modelIndexFrom,
                                       void *ghoul2To, int toBoltIndex,
                                       int toModel) {
  ...
  return re->G2API_AttachG2Model(*g2From, modelIndexFrom, *g2To, toBoltIndex,
                                 toModel);
}
...
intptr_t CL_UISystemCalls(intptr_t *args) {
  ...
  case UI_CM_LERPTAG:
    re->LerpTag((orientation_t *)VMA(1), args[2], args[3], args[4], VMF(5),
                (const char *)VMA(6));
    return 0;
...
void CL_BindUI(void) {
  ...
  if (uivm && !uivm->isLegacy) {
    ...
    uii.G2API_GetBoltMatrix_NoReconstruct =
