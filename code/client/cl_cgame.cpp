...
extern int S_AddLocalSet(const char *name, vec3_t listener_origin,
                         vec3_t origin, int entID, int time);
...
qboolean CL_InitCGameVM(void *gameLibrary) {
  ...
  DllEntryProc *dllEntry =
      (DllEntryProc *)Sys_LoadFunction(gameLibrary, "dllEntry");
  cgvm.entryPoint =
      (intptr_t(*)(int, ...))Sys_LoadFunction(gameLibrary, "vmMain");
  ...
  if (!cgvm.entryPoint || !dllEntry) {
    ...
    Com_Printf(
        "CL_InitCGameVM: client game entry point not found in %s" ARCH_STRING
            DLL_EXT ": %s\n",
...
*/
/*
qboolean	CL_GetParseEntityState( int parseEntityNumber, entityState_t
*state ) {
        ...
        // can't return anything that has been overwritten in the circular
buffer if ( parseEntityNumber <= cl.parseEntitiesNum - MAX_PARSE_ENTITIES ) {
                return qfalse;
        }
...
void CL_SetUserCmdAngles(float pitchOverride, float yawOverride,
                         float rollOverride) {
...
cgameImport_t CL_ConvertJK2SysCall(cgameJK2Import_t import) {
  // FIXME: This was a 5-minute slap-hack job in order to test if this really
  // works. CLEAN ME UP! --eez
  switch (import) {
  case CG_PRINT_JK2:
    return CG_PRINT;
    break;
...
intptr_t CL_CgameSystemCalls(intptr_t *args) {
  ...
  case CG_CM_TEMPBOXMODEL:
    return CM_TempBoxModel((const float *)VMA(1),
                           (const float *)VMA(2)); //, (int) VMA(3) );
  ...
  case CG_CM_TRANSFORMEDBOXTRACE:
    CM_TransformedBoxTrace((trace_t *)VMA(1), (const float *)VMA(2),
                           (const float *)VMA(3), (const float *)VMA(4),
                           (const float *)VMA(5), args[6], args[7],
                           (const float *)VMA(8), (const float *)VMA(9));
    return 0;
  case CG_CM_MARKFRAGMENTS:
    return re.MarkFragments(args[1], (float(*)[3])VMA(2), (const float *)VMA(3),
                            args[4], (float *)VMA(5), args[6],
  ...
  case CG_S_STARTSOUND:
    // stops an ERR_DROP internally if called illegally from game side, but note
    // that it also gets here
    //	legally during level start where normally the internal s_soundStarted
    //check would return. So ok to hit this.
    if (!cls.cgameStarted) {
      return 0;
    }
  ...
  case CG_S_ADDLOOPINGSOUND:
    ...
    S_AddLoopingSound(args[1], (const float *)VMA(2), (const float *)VMA(3),
                      args[4], (soundChannel_t)args[5]);
    return 0;
  case CG_S_UPDATEENTITYPOSITION:
    S_UpdateEntityPosition(args[1], (const float *)VMA(2));
    return 0;
  case CG_S_RESPATIALIZE:
    S_Respatialize(args[1], (const float *)VMA(2), (float(*)[3])VMA(3),
                   (qboolean)(args[4] != 0));
    return 0;
  ...
  case CG_R_FONTDRAWSTRING:
    re.Font_DrawString(args[1], args[2], (const char *)VMA(3), (float *)args[4],
                       args[5], args[6], VMF(7));
    return 0;
  ...
  case CG_ANYLANGUAGE_READFROMSTRING:
    return re.AnyLanguage_ReadCharFromString((char *)VMA(1), (int *)VMA(2),
                                             (qboolean *)VMA(3));
  case CG_ANYLANGUAGE_READFROMSTRING2:
    return re.AnyLanguage_ReadCharFromString2((char **)VMA(1),
                                              (qboolean *)VMA(3));
  ...
  case CG_R_GETLIGHTING:
    return re.GetLighting((const float *)VMA(1), (float *)VMA(2),
  ...
  case CG_R_DRAWSCREENSHOT:
    re.DrawStretchRaw(VMF(1), VMF(2), VMF(3), VMF(4), SG_SCR_WIDTH,
                      SG_SCR_HEIGHT, SCR_GetScreenshot(0), 0, qtrue);
    return 0;
  ...
  case CG_G2_SETMODELS:
    re.G2API_SetGhoul2ModelIndexes(*((CGhoul2Info_v *)VMA(1)),
                                   (qhandle_t *)VMA(2), (qhandle_t *)VMA(3));
    return 0;
  ...
  case CG_CIN_PLAYCINEMATIC:
    return CIN_PlayCinematic((const char *)VMA(1), args[2], args[3], args[4],
                             args[5], args[6], (const char *)VMA(7));
  ...
  case CG_UI_GETITEMTEXT:
    ...
    if (menu) {
      item =
          (itemDef_s *)Menu_FindItemByName((menuDef_t *)menu, (char *)VMA(2));
      if (item) {
  ...
  case CG_SP_REGISTER:
    return JK2SP_Register((const char *)VMA(1),
                          args[2] ? (SP_REGISTER_MENU | SP_REGISTER_REQUIRED)
                                  : SP_REGISTER_CLIENT);
#else
  case CG_SP_GETSTRINGTEXTSTRING:
    const char *text;
...
void CL_AdjustTimeDelta(void) {
  /*
          ...
          if ( cls.realtime + cl.serverTimeDelta < cl.frame.serverTime -
     RESET_TIME
                  || cls.realtime + cl.serverTimeDelta > cl.frame.serverTime +
     RESET_TIME  ) { cl.serverTimeDelta = cl.frame.serverTime - cls.realtime;
  ...
  */
  ...
  if (deltaDelta > RESET_TIME) {
    ...
    cl.oldServerTime =
        cl.frame.serverTime; // FIXME: is this a problem for cgame?
