...
void sentry_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  NPC_SetAnim(self, SETANIM_BOTH, BOTH_POWERUP1,
...
void NPC_Sentry_Pain(gentity_t *self, gentity_t *attacker, int damage) {
  ...
  if (mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT) {
    ...
    self->flags |= FL_SHIELDED;
    NPC_SetAnim(self, SETANIM_BOTH, BOTH_FLY_SHIELDED,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  ...
  //		self->flags &= ~FL_SHIELDED;
  //		NPC_SetAnim( self, SETANIM_BOTH, BOTH_POWERUP1,
  //SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD ); 		self->NPC->localState =
  //LSTATE_WAKEUP;
  //	}
}
...
void Sentry_Fire(void) {
  ...
  if (NPCS.NPCInfo->localState == LSTATE_POWERING_UP) {
    ...
    G_Sound(NPCS.NPC, CHAN_AUTO,
            G_SoundIndex("sound/chars/sentry/misc/sentry_shield_open"));
    NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_POWERUP1,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void Sentry_MaintainHeight(void) {
  ...
  if (NPCS.NPC->enemy) {
    ...
    dif = (NPCS.NPC->enemy->r.currentOrigin[2] + NPCS.NPC->enemy->r.maxs[2]) -
    ...
    if (fabs(dif) > 8) {
      ...
      NPCS.NPC->client->ps.velocity[2] =
          (NPCS.NPC->client->ps.velocity[2] + dif) / 2;
    }
  } else {
    gentity_t *goal = NULL;
...
void Sentry_Idle(void) {
  ...
  if (NPCS.NPCInfo->localState == LSTATE_WAKEUP) {
    ...
    NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_SLEEP1,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    NPCS.NPC->flags |= FL_SHIELDED;
...
void Sentry_Strafe(void) {
  ...
  trap->Trace(&tr, NPCS.NPC->r.currentOrigin, NULL, NULL, end,
              NPCS.NPC->s.number, MASK_SOLID, qfalse, 0, 0);

  // Close enough
  if (tr.fraction > 0.9f) {
    VectorMA(NPCS.NPC->client->ps.velocity, SENTRY_STRAFE_VEL * dir, right,
...
void Sentry_Hunt(qboolean visible, qboolean advance) {
  ...
  speed = SENTRY_FORWARD_BASE_SPEED +
          SENTRY_FORWARD_MULTIPLIER * g_npcspskill.integer;
  VectorMA(NPCS.NPC->client->ps.velocity, speed, forward,
           NPCS.NPC->client->ps.velocity);
}
...
void Sentry_RangedAttack(qboolean visible, qboolean advance) {
  if (TIMER_Done(NPCS.NPC, "attackDelay") &&
      NPCS.NPC->attackDebounceTime < level.time && visible) // Attack?
  {
    if (NPCS.NPCInfo->burstCount > 6) {
      if (!NPCS.NPC->fly_sound_debounce_time) { // delay closing down to give
                                                // the player an opening
        ...
        NPCS.NPC->flags |= FL_SHIELDED;
        NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_FLY_SHIELDED,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void Sentry_AttackDecision(void) {
  ...
  if (TIMER_Done(NPCS.NPC, "patrolNoise")) {
    if (TIMER_Done(NPCS.NPC, "angerNoise")) {
      G_SoundOnEnt(NPCS.NPC, CHAN_AUTO,
                   va("sound/chars/sentry/misc/talk%d", Q_irand(1, 3)));
