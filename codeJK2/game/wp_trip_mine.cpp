...
//---------------------------------------------------------
{
  ...
  if (ent->count == TRIPWIRE_STYLE) {
    ...
    gi.trace(&tr, ent->s.origin2, mins, maxs, ent->currentOrigin, ent->s.number,
             MASK_SHOT, G2_RETURNONHIT, 0);
...
// copied from flechette prox above...which will not be used if this gets
// approved
//---------------------------------------------------------
...
//---------------------------------------------------------
{
  ...
  if (ent->delay > level.time) {
    count = G_RadiusList(ent->currentOrigin, PROX_MINE_RADIUS_CHECK, ent, qtrue,
                         ent_list);
...
//---------------------------------------------------------
{
  ...
  gi.trace(&tr, ent->s.origin2, mins, maxs, end, ent->s.number, MASK_SHOT,
           G2_RETURNONHIT, 0);
  ...
  if (traceEnt->client || tr.startsolid) {
    ...
    AEL_DANGER ); AddSightEvent( ent->owner, ent->currentOrigin,
...
//---------------------------------------------------------
{
  if (!VALIDSTRING(laserTrap->classname)) {
    // since we may be coming from a map placed trip mine, we don't want to
    // override that class name....
    //	That would be bad because the player drop code tries to limit number of
    //placed items...so it would have removed map placed ones as well.
  ...
  laserTrap->clipmask =
      (CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_SHOTCLIP); // MASK_SHOT;
  ...
  gi.G2API_InitGhoul2Model(laserTrap->ghoul2,
                           weaponData[WP_TRIP_MINE].missileMdl,
...
//---------------------------------------------------------
{
  ...
  if (laserTrap) {
    ...
    WP_TraceSetStart(ent, start, vec3_origin,
                     vec3_origin); // make sure our start point isn't on the
