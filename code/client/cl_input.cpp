...
void CL_AdjustAngles(void) {
  ...
  if (!in_strafe.active) {
    if (cl_mYawOverride) {
      cl.viewangles[YAW] -= cl_mYawOverride * 5.0f * speed *
                            cl_yawspeed->value * CL_KeyState(&in_right);
      cl.viewangles[YAW] += cl_mYawOverride * 5.0f * speed *
                            cl_yawspeed->value * CL_KeyState(&in_left);
    } else {
  ...
  if (cl_mPitchOverride) {
    cl.viewangles[PITCH] -= cl_mPitchOverride * 5.0f * speed *
                            cl_pitchspeed->value * CL_KeyState(&in_lookup);
    cl.viewangles[PITCH] += cl_mPitchOverride * 5.0f * speed *
                            cl_pitchspeed->value * CL_KeyState(&in_lookdown);
  } else {
    cl.viewangles[PITCH] -=
...
void CL_JoystickMove(usercmd_t *cmd) {
  ...
  if (in_mlooking) {
    if (cl_mPitchOverride) {
      cl.viewangles[PITCH] +=
          5.0f * cl_mPitchOverride * cl.joystickAxis[AXIS_FORWARD];
    } else {
...
void CL_InitInput(void) {
  ...
  // can add up to button31 this just brings the number of available binds up to
  // par with MP
