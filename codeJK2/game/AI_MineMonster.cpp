...
void MineMonster_TryDamage(gentity_t *enemy, int damage) {
  ...
  gi.trace(&tr, NPC->currentOrigin, vec3_origin, vec3_origin, end,
           NPC->s.number, MASK_SHOT, G2_NOCOLLIDE, 0);

  if (tr.entityNum >= 0 && tr.entityNum < ENTITYNUM_NONE) {
    G_Damage(&g_entities[tr.entityNum], NPC, NPC, dir, tr.endpos, damage,
             DAMAGE_NO_KNOCKBACK, MOD_MELEE);
    G_SoundOnEnt(NPC, CHAN_VOICE_ATTEN,
                 va("sound/chars/mine/misc/bite%i.wav", Q_irand(1, 4)));
  } else {
    G_SoundOnEnt(NPC, CHAN_VOICE_ATTEN,
                 va("sound/chars/mine/misc/miss%i.wav", Q_irand(1, 4)));
  }
}
...
void MineMonster_Attack(void) {
  if (!TIMER_Exists(NPC, "attacking")) {
    // usually try and play a jump attack if the player somehow got above
    // them....or just really rarely
    if (NPC->enemy &&
        ((NPC->enemy->currentOrigin[2] - NPC->currentOrigin[2] > 10 &&
          Q_flrand(0.0f, 1.0f) > 0.1f) ||
         Q_flrand(0.0f, 1.0f) > 0.8f)) {
      // Going to do ATTACK4
      TIMER_Set(NPC, "attacking", 1750 + Q_flrand(0.0f, 1.0f) * 200);
      NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_ATTACK4,
    ...
    // Need to do delayed damage since the attack animations encapsulate
    // multiple mini-attacks
...
-------------------------
*/
void NPC_MineMonster_Pain(gentity_t *self, gentity_t *inflictor,
                          gentity_t *other, vec3_t point, int damage, int mod,
                          int hitLoc) {
