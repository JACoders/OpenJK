...
qboolean NPC_CheckEnemyStealth( gentity_t *target )
{
	...
	if ( clearLOS )
	{
		...
		contents = gi.PointContents( targ_org, target->s.number );
		if ( contents&CONTENTS_WATER )
		{
			int myContents = gi.PointContents( NPCS.NPC->client->renderInfo.eyePoint, NPCS.NPC->s.number );
	...
	*/
	...
	if ( level.alertEvents[eventID].level > AEL_MINOR && NPCS.NPCInfo->investigateCount > 1 && (NPCS.NPCInfo->scriptFlags&SCF_CHASE_ENEMIES) )
	{
		...
		{//we were able to move the investigateGoal to a point in which our bbox would fit
			...
			gi.Trace( &trace, NPCS.NPCInfo->investigateGoal, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, end, ENTITYNUM_NONE, ((NPCS.NPC->clipmask&~CONTENTS_BODY)|CONTENTS_BOTCLIP), qfalse, 0, 0 );
...
static void ST_CheckMoveState( void )
{
	...
	if ( ( NPCS.NPCInfo->goalEntity != NPCS.NPC->enemy ) && ( NPCS.NPCInfo->goalEntity != NULL ) )
	{
		...
		if ( NAV_HitNavGoal( NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, NPCS.NPCInfo->goalEntity->r.currentOrigin, 16, FlyingCreature( NPCS.NPC ) ) || 
			( !gi.ICARUS_TaskIDPending( (sharedEntity_t *)NPCS.NPC, TID_MOVE_NAV ) && NPCS.NPCInfo->squadState == SQUAD_SCOUT && enemyLOS && enemyDist <= 10000 ) )
...
void ST_TransferMoveGoal( gentity_t *self, gentity_t *other )
{
	if ( gi.ICARUS_TaskIDPending( (sharedEntity_t *)self, TID_MOVE_NAV ) )
...
void ST_Commander( void )
{
	...
	{//dissolve the group
		...
		for ( i = 0; i < group->numGroup; i++ )
		{
			...
			else if ( group->enemy->waypoint == WAYPOINT_NONE || (gi.Nav_GetPathCost( NPCS.NPC->waypoint, group->enemy->waypoint ) >= Q3_INFINITE) )
			{
	...
	*/
	...
	for ( i = curMemberNum; i < lastMemberNum; i++ )
	{
		...
		{//not already retreating
			...
			if ( TIMER_Done( NPCS.NPC, "roamTime" ) && TIMER_Done( NPCS.NPC, "hideTime" ) && NPCS.NPC->health > 10 && !gi.InPVS( group->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin ) )
			{//cant even see enemy
				//better go after him
			...
			{//not hit, see if there are other reasons we should run
				if ( gi.InPVS( NPCS.NPC->r.currentOrigin, group->enemy->r.currentOrigin ) )
