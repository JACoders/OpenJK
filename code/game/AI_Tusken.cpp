...
void NPC_Tusken_PlayConfusionSound(
...
void NPC_Tusken_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
...
void NPC_BSTusken_Patrol(void) { // FIXME: pick up on bodies of dead buddies?
...
void NPC_Tusken_Taunt(void) {
  NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_TUSKENTAUNT1,
...
void NPC_BSTusken_Attack(void) {
  ...
  // IN FLEE
  //---------
  if (TIMER_Done(NPC, "flee") && NPC_CheckForDanger(NPC_CheckAlertEvents(
                                     qtrue, qtrue, -1, qfalse, AEL_DANGER))) {
    NPC_UpdateAngles(qtrue, qtrue);
    return;
  }
  ...
  if (NPC->enemy->client && NPC->enemy->client->NPC_class == CLASS_JAWA) {
    ...
    if ((player) &&               // If There Is A Player Pointer
        ...
        (Distance(player->currentOrigin, NPC->currentOrigin) <
         130.0f) &&                      // The Player Is Close Enough
  ...
  if (TIMER_Done(NPC, "taunting")) {
    ...
    if (meleeRange || (!meleeWeapon && canSeeEnemy)) {
      if (!(NPCInfo->scriptFlags &
            SCF_FIRE_WEAPON) && // If This Flag Is On, It Calls Attack From
                                // Elsewhere
          !(NPCInfo->scriptFlags &
            SCF_DONT_FIRE) && // If This Flag Is On, Don't Fire At All
          (TIMER_Done(NPC, "attackDelay"))) {
        ucmd.buttons &= ~BUTTON_ALT_ATTACK;

        // If Not In Strike Range, Do Lunge, Or If We Don't Have The Staff, Just
        // Shoot Normally
        //--------------------------------------------------------------------------------------
      ...
      if (trace.fraction < 1.0f && trace.entityNum != lastHit) { // hit
        ...
        if (traceEnt->takedamage &&
          ...
          if (traceEnt->health > 0 &&
              ((traceEnt->client && traceEnt->client->NPC_class == CLASS_JAWA &&
                !Q_irand(0, 1)) ||
...
qboolean G_TuskenAttackAnimDamage(gentity_t *self) {
  if (self->client->ps.torsoAnim == BOTH_TUSKENATTACK1 ||
    ...
    if (!!gi.G2API_GetBoneAnimIndex(&self->ghoul2[self->playerModel],
                                    self->lowerLumbarBone, level.time, &current,
      ...
      case BOTH_TUSKENATTACK1:
        return (qboolean)(percentComplete > 0.3 && percentComplete < 0.7);
      case BOTH_TUSKENATTACK2:
        return (qboolean)(percentComplete > 0.3 && percentComplete < 0.7);
      case BOTH_TUSKENATTACK3:
        return (qboolean)(percentComplete > 0.1 && percentComplete < 0.5);
      case BOTH_TUSKENLUNGE1:
        return (qboolean)(percentComplete > 0.3 && percentComplete < 0.5);
      }
    }
  }
  return qfalse;
