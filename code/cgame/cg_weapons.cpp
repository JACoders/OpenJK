...
void CG_RegisterWeapon(int weaponNum) {
  ...
  weaponInfo->weaponModel =
      cgi_R_RegisterModel(weaponData[weaponNum].weaponMdl);
  { // in case the weaponmodel isn't _w, precache the _w.glm
    char weaponModel[64];

    Q_strncpyz(weaponModel, weaponData[weaponNum].weaponMdl,
               sizeof(weaponModel));
    if (char *spot = strstr(weaponModel, ".md3")) {
      *spot = 0;
      ...
      Q_strcat(weaponModel, sizeof(weaponModel), ".glm"); // and change to
                                                          // ghoul2
    }
  ...
  if (weaponData[weaponNum].weaponIcon[0]) {
    weaponInfo->weaponIcon =
        cgi_R_RegisterShaderNoMip(weaponData[weaponNum].weaponIcon);
    weaponInfo->weaponIconNoAmmo = cgi_R_RegisterShaderNoMip(
        va("%s_na", weaponData[weaponNum].weaponIcon));
  }

  for (ammo = bg_itemlist + 1; ammo->classname; ammo++) {
    if (ammo->giType == IT_AMMO &&
        ammo->giTag == weaponData[weaponNum].ammoIndex) {
      break;
    }
  }
  ...
  if (!weaponInfo->handsModel) {
    weaponInfo->handsModel = cgi_R_RegisterModel(
  ...
  if (weaponData[weaponNum].firingSnd[0]) {
    weaponInfo->firingSound =
        cgi_S_RegisterSound(weaponData[weaponNum].firingSnd);
  }
  if (weaponData[weaponNum].altFiringSnd[0]) {
    weaponInfo->altFiringSound =
        cgi_S_RegisterSound(weaponData[weaponNum].altFiringSnd);
  }
  ...
  if (weaponData[weaponNum].chargeSnd[0]) {
    weaponInfo->chargeSound =
        cgi_S_RegisterSound(weaponData[weaponNum].chargeSnd);
  }
  if (weaponData[weaponNum].altChargeSnd[0]) {
    weaponInfo->altChargeSound =
        cgi_S_RegisterSound(weaponData[weaponNum].altChargeSnd);
  }
  if (weaponData[weaponNum].selectSnd[0]) {
    weaponInfo->selectSound =
        cgi_S_RegisterSound(weaponData[weaponNum].selectSnd);
  }

  // give us missile models if we should
  if (weaponData[weaponNum].missileMdl[0]) {
    weaponInfo->missileModel =
        cgi_R_RegisterModel(weaponData[weaponNum].missileMdl);
  }
  if (weaponData[weaponNum].alt_missileMdl[0]) {
    weaponInfo->alt_missileModel =
        cgi_R_RegisterModel(weaponData[weaponNum].alt_missileMdl);
  }
  if (weaponData[weaponNum].missileSound[0]) {
    weaponInfo->missileSound =
        cgi_S_RegisterSound(weaponData[weaponNum].missileSound);
  }
  if (weaponData[weaponNum].alt_missileSound[0]) {
    weaponInfo->alt_missileSound =
        cgi_S_RegisterSound(weaponData[weaponNum].alt_missileSound);
  }
  if (weaponData[weaponNum].missileHitSound[0]) {
    weaponInfo->missileHitSound =
        cgi_S_RegisterSound(weaponData[weaponNum].missileHitSound);
  }
  if (weaponData[weaponNum].altmissileHitSound[0]) {
    weaponInfo->altmissileHitSound =
        cgi_S_RegisterSound(weaponData[weaponNum].altmissileHitSound);
  }
  if (weaponData[weaponNum].mMuzzleEffect[0]) {
    weaponData[weaponNum].mMuzzleEffectID =
        theFxScheduler.RegisterEffect(weaponData[weaponNum].mMuzzleEffect);
  }
  if (weaponData[weaponNum].mAltMuzzleEffect[0]) {
    weaponData[weaponNum].mAltMuzzleEffectID =
        theFxScheduler.RegisterEffect(weaponData[weaponNum].mAltMuzzleEffect);
  }
  ...
  if (weaponData[weaponNum].func) {
    weaponInfo->missileTrailFunc = (void (*)(
        struct centity_s *, const struct weaponInfo_s *))weaponData[weaponNum]
                                       .func;
  }
  if (weaponData[weaponNum].altfunc) {
    weaponInfo->alt_missileTrailFunc = (void (*)(
        struct centity_s *, const struct weaponInfo_s *))weaponData[weaponNum]
                                           .altfunc;
  }
  ...
  case WP_SABER:
    ...
    // cgs.effects.forceInvincibility	= theFxScheduler.RegisterEffect(
    // "force/invin" );
    cgs.effects.forceConfusion =
        theFxScheduler.RegisterEffect("force/confusion");
    cgs.effects.forceLightning =
        theFxScheduler.RegisterEffect("force/lightning");
    cgs.effects.forceLightningWide =
        theFxScheduler.RegisterEffect("force/lightningwide");
    ...
    */
    ...
    // cgs.media.forceSightBubble =
    // cgi_R_RegisterShader("gfx/misc/sightbubble"); cgs.media.forceShell =
    // cgi_R_RegisterShader("powerups/forceshell");
    ...
    cgs.media.saberBlurShader =
        cgi_R_RegisterShader("gfx/effects/sabers/saberBlur");
    cgs.media.swordTrailShader =
        cgi_R_RegisterShader("gfx/effects/sabers/swordTrail");
    cgs.media.yellowDroppedSaberShader =
        cgi_R_RegisterShader("gfx/effects/yellow_glow");
  ...
  case WP_JAWA:
    ...
    cgs.effects.bryarPowerupShotEffect =
        theFxScheduler.RegisterEffect("bryar/crackleShot");
    cgs.effects.bryarWallImpactEffect =
        theFxScheduler.RegisterEffect("bryar/wall_impact");
    cgs.effects.bryarWallImpactEffect2 =
        theFxScheduler.RegisterEffect("bryar/wall_impact2");
    cgs.effects.bryarWallImpactEffect3 =
        theFxScheduler.RegisterEffect("bryar/wall_impact3");
    cgs.effects.bryarFleshImpactEffect =
        theFxScheduler.RegisterEffect("bryar/flesh_impact");

    // Note....these are temp shared effects
    theFxScheduler.RegisterEffect("blaster/deflect");
    theFxScheduler.RegisterEffect(
        "blaster/smoke_bolton"); // note: this will be called game side
    break;

  case WP_BLASTER:
    cgs.effects.blasterShotEffect =
        theFxScheduler.RegisterEffect("blaster/shot");
    theFxScheduler.RegisterEffect("blaster/NPCshot");
    //		cgs.effects.blasterOverchargeEffect		=
    //theFxScheduler.RegisterEffect( "blaster/overcharge" );
    cgs.effects.blasterWallImpactEffect =
        theFxScheduler.RegisterEffect("blaster/wall_impact");
  ...
  case WP_DISRUPTOR:
    ...
    cgs.media.disruptorZoomLoop =
        cgi_S_RegisterSound("sound/weapons/disruptor/zoomloop.wav");
  ...
  case WP_BOWCASTER:
    cgs.effects.bowcasterShotEffect =
        theFxScheduler.RegisterEffect("bowcaster/shot");
    cgs.effects.bowcasterBounceEffect =
        theFxScheduler.RegisterEffect("bowcaster/bounce_wall");
    cgs.effects.bowcasterImpactEffect =
        theFxScheduler.RegisterEffect("bowcaster/explosion");
  ...
  case WP_FLECHETTE:
    cgs.effects.flechetteShotEffect =
        theFxScheduler.RegisterEffect("flechette/shot");
    cgs.effects.flechetteAltShotEffect =
        theFxScheduler.RegisterEffect("flechette/alt_shot");
    cgs.effects.flechetteShotDeathEffect =
        theFxScheduler.RegisterEffect("flechette/wall_impact"); // shot death
    cgs.effects.flechetteFleshImpactEffect =
        theFxScheduler.RegisterEffect("flechette/flesh_impact");
    cgs.effects.flechetteRicochetEffect =
        theFxScheduler.RegisterEffect("flechette/ricochet");
  ...
  case WP_THERMAL:
    cgs.media.grenadeBounce1 =
        cgi_S_RegisterSound("sound/weapons/thermal/bounce1.wav");
    cgs.media.grenadeBounce2 =
        cgi_S_RegisterSound("sound/weapons/thermal/bounce2.wav");
  ...
  case WP_TRIP_MINE:
    ...
    cgs.media.tripMineStickSound =
        cgi_S_RegisterSound("sound/weapons/laser_trap/stick.wav");
  ...
  case WP_DET_PACK:
    ...
    cgs.media.detPackStickSound =
        cgi_S_RegisterSound("sound/weapons/detpack/stick.wav");
  ...
  case WP_EMPLACED_GUN:
    ...
    cgs.media.emplacedHealthBarShader =
        cgi_R_RegisterShaderNoMip("gfx/hud/health_frame");
    cgs.media.turretComputerOverlayShader =
        cgi_R_RegisterShaderNoMip("gfx/hud/generic_target");
    cgs.media.turretCrossHairShader =
        cgi_R_RegisterShaderNoMip("gfx/2d/panel_crosshair");
    break;
...
void CG_RegisterItemVisuals(int itemNum) {
  ...
  if (item->giType == IT_HOLDABLE) {
    ...
    case INV_ELECTROBINOCULARS:
      ...
      cgs.media.binocularStatic =
          cgi_R_RegisterShader("gfx/2d/binocularWindow");
      cgs.media.binocularOverlay =
          cgi_R_RegisterShader("gfx/2d/binocularNumOverlay");
      break;

    case INV_LIGHTAMP_GOGGLES:
      // LA Goggles Shaders
      cgs.media.laGogglesStatic =
          cgi_R_RegisterShader("gfx/2d/lagogglesWindow");
...
int CG_MapTorsoToWeaponFrame(const clientInfo_t *ci, int frame, int animNum,
  ...
  // we should use the animNum to map a weapon frame instead of relying on the
  // torso frame
...
) { cent->pe.barrelTime = cg.time; cent->pe.barrelAngle = AngleNormalize360(
angle ); cent->pe.barrelSpinning = !!(cent->currentState.eFlags & EF_FIRING);
        }
...
static void CG_DoMuzzleFlash(centity_t *cent, vec3_t org, vec3_t dir,
                             weaponData_t *wData) {
  // Handle muzzle flashes, really this could just be a qboolean instead of a
  // time.......
  if (cent->muzzleFlashTime > 0) {
    ...
    if (cent->altFire) {
      // We're alt-firing, so see if we need to override with a custom alt-fire
      // effect
    ...
    if (/*( cent->currentState.eFlags & EF_FIRING || cent->currentState.eFlags &
           EF_ALT_FIRING ) &&*/
        effect) {
...
void CG_AddViewWeapon(playerState_t *ps) {
  ...
  if (cent->gent && cent->gent->client &&
    ...
    if (cent->gent->client->ps.forcePowerLevel[FP_LIGHTNING] >
      ...
      theFxScheduler.PlayEffect(cgs.effects.forceLightningWide, temp,
                                cg.refdef.viewaxis);
    } else { // line
      // AngleVectors( tAng, fxDir, NULL, NULL );
      theFxScheduler.PlayEffect(cgs.effects.forceLightning, temp,
  ...
  if (cent->gent && cent->gent->client &&
    ...
    if (cent->gent->client->ps.forcePowerLevel[FP_DRAIN] >
      ...
      // vec3_t	fxAxis[3];
      // AnglesToAxis( tAng, fxAxis );
      theFxScheduler.PlayEffect(cgs.effects.forceDrainWide, temp,
                                cg.refdef.viewaxis);
    } else { // line
      // AngleVectors( tAng, fxDir, NULL, NULL );
      theFxScheduler.PlayEffect(cgs.effects.forceDrain, temp,
                                cg.refdef.viewaxis[0]);
    }
  }
  ...
  if (!cg_drawGun.integer || cg.zoomMode) {
    ...
    // Doesn't look like we'll have lightning style guns.  Clean this crap up
    // when we are sure about this.
    ...
    // If we don't update this, the muzzle flash point won't even be updated
    // properly
  ...
  float actualFOV;
  if ((cg.snap->ps.forcePowersActive & (1 << FP_SPEED)) &&
      player->client->ps
          .forcePowerDuration[FP_SPEED]) // cg.renderingThirdPerson &&
  {
    ...
    if (cg.overrides.active & CG_OVERRIDE_FOV)
      ...
      actualFOV =
          cg_fovViewmodel.integer ? cg_fovViewmodel.value : cg_fov.value;
    }
  }
  ...
  if (ps->weapon == WP_TUSKEN_RIFLE || ps->weapon == WP_NOGHRI_STICK ||
      ps->weapon == WP_TUSKEN_STAFF) {
    extraOffset[0] = 2;
    extraOffset[1] = -3;
    extraOffset[2] = -6;
  }

  VectorMA(hand.origin, cg_gun_x.value + extraOffset[0], cg.refdef.viewaxis[0],
           hand.origin);
  VectorMA(hand.origin, (cg_gun_y.value + leanOffset + extraOffset[1]),
           cg.refdef.viewaxis[1], hand.origin);
  VectorMA(hand.origin, (cg_gun_z.value + fovOffset + extraOffset[2]),
  ...
  if (cg_fovViewmodel.integer) {
    ...
    float fracWeapFOV =
        (1.0f / fracDistFOV) * tanf(actualFOV * (M_PI / 180) * 0.5f);
...
#endif
  {
    ...
    if (cent->gent->lowerLumbarBone >= 0 &&
      ...
      hand.oldframe = CG_MapTorsoToWeaponFrame(
          ci, floor(currentFrame), torsoAnim, cent->currentState.weapon,
          (cent->currentState.eFlags & EF_FIRING));
      hand.frame = CG_MapTorsoToWeaponFrame(
          ci, ceil(currentFrame), torsoAnim, cent->currentState.weapon,
          (cent->currentState.eFlags & EF_FIRING));
      ...
      if (cg_debugAnim.integer == 1 && cent->currentState.clientNum == 0) {
        Com_Printf("Torso frame %d to %d makes Weapon frame %d to %d\n",
                   cent->pe.torso.oldFrame, cent->pe.torso.frame, hand.oldframe,
                   hand.frame);
      }
    } else {
  ...
  // add the weapon(s) - FIXME: allow for 2 weapons generically, not just 2
  // sabers?
  int numSabers = 1;
  ...
  for (int saberNum = 0; saberNum < numSabers; saberNum++) {
    ...
    //---------
    // OK, we are making an assumption here that if we have the phaser that it
    // is always on....
    ...
    if (cent->gent && cent->gent->client &&
      ...
      for (int bladeNum = 0;
        ...
        if (cent->gent->client->ps.saber[0].blade[0].active &&
          ...
          cent->gent->client->ps.saber[0].blade[0].length +=
              cg.frametime * 0.03;
          if (cent->gent->client->ps.saber[0].blade[0].length >
              cent->gent->client->ps.saber[0].blade[0].lengthMax) {
            cent->gent->client->ps.saber[0].blade[0].length =
                cent->gent->client->ps.saber[0].blade[0].lengthMax;
          }
        }
        //		FX_Saber( org_, axis_[0],
        //cent->gent->client->ps.saberLength, 2.0 + Q_flrand(-1.0f, 1.0f) *
        //0.2f, cent->gent->client->ps.saberColor );
        if (saberNum == 0 && bladeNum == 0) {
          ...
          VectorCopy(org_, cent->gent->client->ps.saber[saberNum]
                               .blade[bladeNum]
                               .muzzlePoint);
        }
        VectorCopy(
    ...
    //	CG_AddRefEntityWithPowerups( &gun, cent->currentState.powerups,
    //cent->gent );
    cgi_R_AddRefEntityToScene(&gun);

    /*	if ( ps->weapon == WP_STUN_BATON )
            {
                    gun.shaderRGBA[0] = gun.shaderRGBA[1] = gun.shaderRGBA[2] =
       25;

                    gun.customShader = cgi_R_RegisterShader(
       "gfx/effects/stunPass" ); gun.renderfx = RF_RGB_TINT | RF_FIRST_PERSON |
       RF_DEPTHHACK; cgi_R_AddRefEntityToScene( &gun );
            }
    */
    ...
    for (int i = 0; (i < wData->numBarrels); i++) {
      ...
      if (!i) {
        CG_PositionRotatedEntityOnTag(&barrel, &hand, weapon->handsModel,
                                      "tag_barrel", NULL);
      } else {
        CG_PositionRotatedEntityOnTag(&barrel, &hand, weapon->handsModel,
                                      va("tag_barrel%d", i + 1), NULL);
      }
    ...
    // Seems like we should always do this in case we have an animating muzzle
    // flash....that way we can always store the correct muzzle dir, etc.
    CG_PositionEntityOnTag(&flash, &gun, gun.hModel, "tag_flash");
  ...
  if ((ps->weaponstate == WEAPON_CHARGING_ALT &&
    ...
    FX_AddSprite(flash.origin, NULL, NULL, 3.0f * val * scale, 0.0f, 0.7f, 0.7f,
                 WHITE, WHITE, Q_flrand(0.0f, 1.0f) * 360, 0.0f, 1.0f, shader,
  ...
  if (ps->weapon == WP_REPEATER && ps->weaponstate == WEAPON_FIRING) {
    if (cent->gent && cent->gent->client &&
        cent->gent->client->ps.weaponstate != WEAPON_FIRING) {
      int ct = 0;
      ...
      for (int i = 0; i < ct; i++) {
        theFxScheduler.PlayEffect("repeater/muzzle_smoke",
...
int CG_WeaponCheck(int weaponIndex) {
  ...
  value = weaponData[weaponIndex].energyPerShot <
  ...
  // check how much energy(ammo) it takes to fire this weapon against how much
  // ammo we have
...
const char *weaponDesc[13] = {
    "SABER_DESC",         "NEW_BLASTER_PISTOL_DESC",
    "BLASTER_RIFLE_DESC", "DISRUPTOR_RIFLE_DESC",
    "BOWCASTER_DESC",     "HEAVYREPEATER_DESC",
    "DEMP2_DESC",         "FLECHETTE_DESC",
    "MERR_SONN_DESC",     "THERMAL_DETONATOR_DESC",
    "TRIP_MINE_DESC",     "DET_PACK_DESC",
    "CONCUSSION_DESC",
};
...
Allows user to cycle through the various weapons currently owned and view the
description
===================
*/
void CG_DrawDataPadWeaponSelect(void) {
  ...
  for (iconCnt = 1; iconCnt < (sideLeftIconCnt + 1); weaponSelectI--) {
    if (weaponSelectI == WP_CONCUSSION) {
      weaponSelectI--;
    } else if (weaponSelectI == WP_FLECHETTE && !drewConc &&
    ...
    if (weaponData[weaponSelectI].weaponIcon[0]) {
      ...
      if (!CG_WeaponCheck(weaponSelectI)) {
        CG_DrawPic(holdX, graphicYPos, smallIconSize, smallIconSize,
  ...
  if (weaponData[cg.DataPadWeaponSelect].weaponIcon[0]) {
    ...
    if (!CG_WeaponCheck(cg.DataPadWeaponSelect)) {
      CG_DrawPic(centerXPos - (bigIconSize / 2),
  ...
  for (iconCnt = 1; iconCnt < (sideRightIconCnt + 1); weaponSelectI++) {
    if (weaponSelectI == WP_CONCUSSION) {
      weaponSelectI++;
    } else if (weaponSelectI == WP_ROCKET_LAUNCHER && !drewConc &&
               cg.DataPadWeaponSelect != WP_CONCUSSION) {
      weaponSelectI = WP_CONCUSSION;
    }
    if (weaponSelectI > 13) {
  ...
  cgi_SP_GetStringTextString(
      va("SP_INGAME_%s", weaponDesc[cg.DataPadWeaponSelect - 1]), text,
      sizeof(text));
  ...
  if (text[0]) {
    ...
    CG_DisplayBoxedText(textboxXPos, textboxYPos, textboxWidth, textboxHeight,
                        text, 4, textScale, colorTable[CT_WHITE]);
  }
...
void CG_DrawDataPadIconBackground(const int backgroundType) {
  ...
  /*
          ...
          else if (backgroundType == ICON_WEAPONS)	// Display weapon
     background?
          {
          ...
          cgi_R_SetColor( colorTable[CT_WHITE] );	// Let the graphic set
     the color
...
void CG_DrawWeaponSelect(void) {
  ...
  if (item && item->classname && item->classname[0]) {
    ...
    if (cgi_SP_GetStringTextString(va("SP_INGAME_%s", item->classname), text,
                                   sizeof(text))) {
      int w = cgi_R_Font_StrLenPixels(text, cgs.media.qhFontSmall, 1.0f);
      int x = (SCREEN_WIDTH - w) / 2;
      cgi_R_Font_DrawString(x, (SCREEN_HEIGHT - 24) + yOffset, text, textColor,
...
qboolean CG_WeaponSelectable(int i, int original, qboolean dpMode) {
  ...
  if (cg.weaponSelectTime + 200 >
      cg.time) { // TEMP standard weapon cycle debounce for E3 because G2 can't
                 // keep up with fast weapon changes
    return qfalse;
  }

  // FIXME: this doesn't work below, can still cycle too fast!
  if (original == WP_SABER &&
      cg.weaponSelectTime + 500 >
          cg.time) { // when switch to lightsaber, have to stay there for at
                     // least half a second!
    return qfalse;
  }
  ...
  if (G_IsRidingVehicle(cg_entities[0].gent)) {
    if (G_IsRidingTurboVehicle(cg_entities[0].gent) ||
        (i != WP_NONE && i != WP_SABER && i != WP_BLASTER)) {
      return qfalse;
    }
  }

  if ((weaponData[i].ammoIndex != AMMO_NONE) &&
      !dpMode) { // weapon uses ammo, see if we have any
    usage_for_weap =
        weaponData[i].energyPerShot < weaponData[i].altEnergyPerShot
            ? weaponData[i].energyPerShot
            : weaponData[i].altEnergyPerShot;

    if (cg.snap->ps.ammo[weaponData[i].ammoIndex] - usage_for_weap < 0) {
      if (i != WP_DET_PACK) // detpack can be switched to...should possibly
                            // check if there are any stuck to a wall somewhere?
      {
        // This weapon doesn't have enough ammo to shoot either the main or the
        // alt-fire
        return qfalse;
      }
    }
...
void CG_PlayerLockedWeaponSpeech(int jumping) {
  ...
  if (!in_camera) {                     // not in a cinematic
    if (speechDebounceTime < cg.time) { // spoke more than 3 seconds ago
      if (!Q3_TaskIDPending(
              &g_entities[0],
              TID_CHAN_VOICE)) { // not waiting on a scripted sound to finish
        if (!jumping) {
          if (Q_flrand(0.0f, 1.0f) > 0.5) {
            G_SoundOnEnt(player, CHAN_VOICE,
                         va("sound/chars/kyle/09kyk015.wav"));
          } else {
            G_SoundOnEnt(player, CHAN_VOICE,
                         va("sound/chars/kyle/09kyk016.wav"));
          }
        } else {
...
void CG_ChangeWeapon( int num )
  ...
  Meant to be called from the normal game, so checks the game-side weapon
inventory data
*/
void CG_ChangeWeapon(int num) {
  ...
  if (player->client != NULL &&
      !(player->client->ps.stats[STAT_WEAPONS] & (1 << num))) {
    return; // don't have the weapon
  }

  // because we don't have an empty hand model for the thermal, don't allow
  // selecting that weapon if it has no ammo
  if (num == WP_THERMAL) {
...
void CG_Weapon_f(void) {
  ...
  */
  ...
  if (num == WP_SABER) { // lightsaber
    if (!(cg.snap->ps.stats[STAT_WEAPONS] &
          (1 << num))) { // don't have saber, try stun baton
      if ((cg.snap->ps.stats[STAT_WEAPONS] & (1 << WP_STUN_BATON))) {
        ...
        num = WP_MELEE;
               ...
               cg.snap->ps
                   .weapon) { // already have it up, let's try to toggle it
      if (!in_camera) { // player can't activate/deactivate saber when in a
                        // cinematic
        ...
        if (cg.predicted_player_state.stats[STAT_HEALTH] > 0 &&
            (!cg_entities[0].gent->client->ps.saberInFlight ||
             (&g_entities[cg_entities[0].gent->client->ps.saberEntityNum] !=
              ...
              g_entities[cg_entities[0].gent->client->ps.saberEntityNum]
                      .s.pos.trType ==
                  TR_LINEAR))) { // it's either in-hand or it's under
                                 // telekinetic control
          if (cg_entities[0].gent->client->ps.SaberActive()) { // a saber is on
            if (cg_entities[0].gent->client->ps.dualSabers &&
                cg_entities[0]
                    .gent->client->ps.saber[1]
                    .Active()) { // 2nd saber is on, turn it off, too
              cg_entities[0].gent->client->ps.saber[1].Deactivate();
            }
            cg_entities[0].gent->client->ps.saber[0].Deactivate();
            if (cg_entities[0]
                    .gent->client->ps.saberInFlight) { // play it on the saber
              cgi_S_UpdateEntityPosition(
                  cg_entities[0].gent->client->ps.saberEntityNum,
                  g_entities[cg_entities[0].gent->client->ps.saberEntityNum]
                      .currentOrigin);
              cgi_S_StartSound(
                  NULL, cg_entities[0].gent->client->ps.saberEntityNum,
                  CHAN_AUTO,
                  cgs.sound_precache
                      [cg_entities[0].gent->client->ps.saber[0].soundOff]);
            } else {
              cgi_S_StartSound(
...
void CG_OutOfAmmoChange(void) {
  ...
  if (cg_autoswitch.integer != 1) {
    ...
    for (i = WP_THERMAL; i <= WP_DET_PACK; i++) {
      ...
      if (original != i && CG_WeaponSelectable(i, original, qfalse)) {
        if (i == WP_DET_PACK &&
            cg.snap->ps.ammo[weaponData[i].ammoIndex] <= 0) {
  ...
  /*	if (( ent->powerups & ( 1<<PW_WEAPON_OVERCHARGE )))
          {
                  if ( alt_fire )
                  {
                          ...
                          case WP_BLASTER:
                                  cgi_S_StartSound( NULL, ent->number,
     CHAN_AUTO, cgs.media.overchargeFastSound ); break;

                          default:
                                  cgi_S_StartSound( NULL, ent->number,
     CHAN_AUTO, cgs.media.overchargeSlowSound ); break;
                          }
                  }
                  else
                  {
...
=================
*/
void CG_BounceEffect(centity_t *cent, int weapon, vec3_t origin,
                     vec3_t normal) {
  switch (weapon) {
  case WP_THERMAL:
    if (rand() & 1) {
      cgi_S_StartSound(origin, ENTITYNUM_WORLD, CHAN_AUTO,
                       cgs.media.grenadeBounce1);
    } else {
      cgi_S_StartSound(origin, ENTITYNUM_WORLD, CHAN_AUTO,
                       cgs.media.grenadeBounce2);
  ...
  case WP_BOWCASTER:
    theFxScheduler.PlayEffect(cgs.effects.bowcasterBounceEffect, origin,
                              normal);
    break;
...
qboolean CG_VehicleWeaponImpact(
  ...
  if (cent->currentState.otherEntityNum2 &&
    ...
    CG_PlayEffectID(
        ...
        cent->lerpOrigin, cent->gent->pos1);
    return qtrue;
  }
  return qfalse;
}
...
void CG_MissileHitWall(centity_t *cent, int weapon, vec3_t origin, vec3_t dir,
  ...
  case WP_EMPLACED_GUN:
    FX_EmplacedHitWall(origin, dir,
                       (qboolean)(cent->gent && cent->gent->alt_fire));
    break;
...
void CG_MissileHitPlayer(centity_t *cent, int weapon, vec3_t origin, vec3_t dir,
  ...
  if (cent->gent) {
    ...
    if (other->client) {
      ...
      // check for all droids, maybe check for certain monsters if they're
      ...
      if (npc_class == CLASS_SEEKER || npc_class == CLASS_PROBE ||
          ...
          npc_class == CLASS_ATST || npc_class == CLASS_SENTRY) {
        humanoid = qfalse;
      }
    }
  }
  ...
  case WP_EMPLACED_GUN:
    FX_EmplacedHitPlayer(origin, dir,
                         (qboolean)(cent->gent && cent->gent->alt_fire));
    break;
