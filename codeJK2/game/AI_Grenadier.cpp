...
void NPC_Grenadier_PlayConfusionSound(
...
void NPC_Grenadier_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
...
static qboolean Grenadier_Move(void) {
  ...
  if (moved == qfalse) { // couldn't get to enemy
    if ((NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) &&
        NPC->client->ps.weapon == WP_THERMAL && NPCInfo->goalEntity &&
...
void NPC_BSGrenadier_Idle( void )
{
        ...
        //Is there danger nearby?
        if ( NPC_CheckForDanger( NPC_CheckAlertEvents( qtrue, qtrue, -1, qfalse,
AEL_DANGER ) ) )
        {
                NPC_UpdateAngles( qtrue, qtrue );
                return;
...
static void Grenadier_CheckMoveState(void) {
  ...
  if (!(NPCInfo->scriptFlags &
        SCF_CHASE_ENEMIES)) // behaviorState == BS_STAND_AND_SHOOT )
  {
...
static void Grenadier_CheckFireState(void) {
  ...
  /*
  if ( !Q_irand( 0, 1 ) && NPCInfo->enemyLastSeenTime && level.time -
  NPCInfo->enemyLastSeenTime < 4000 )
  {
...
void NPC_BSGrenadier_Attack(void) {
  ...
  if (enemyDist < 16384 &&
      (!NPC->enemy->client || NPC->enemy->client->ps.weapon != WP_SABER ||
       !NPC->enemy->client->ps.saberActive)) // 128
  { // enemy is close and not using saber
    if (NPC->client->ps.weapon == WP_THERMAL) { // grenadier
      trace_t trace;
      gi.trace(&trace, NPC->currentOrigin, NPC->enemy->mins, NPC->enemy->maxs,
               NPC->enemy->currentOrigin, NPC->s.number, NPC->enemy->clipmask,
               G2_NOCOLLIDE, 0);
      if (!trace.allsolid && !trace.startsolid &&
          (trace.fraction == 1.0 ||
           trace.entityNum == NPC->enemy->s.number)) { // I can get right to him
        // reset fire-timing variables
        NPC_ChangeWeapon(WP_MELEE);
        if (!(NPCInfo->scriptFlags &
              SCF_CHASE_ENEMIES)) // NPCInfo->behaviorState ==
                                  // BS_STAND_AND_SHOOT )
        { // FIXME: should we be overriding scriptFlags?
          NPCInfo->scriptFlags |=
              SCF_CHASE_ENEMIES; // NPCInfo->behaviorState = BS_HUNT_AND_KILL;
        }
      }
    }
  } else if (enemyDist > 65536 ||
             (NPC->enemy->client && NPC->enemy->client->ps.weapon == WP_SABER &&
              NPC->enemy->client->ps.saberActive)) // 256
  { // enemy is far or using saber
    if (NPC->client->ps.weapon == WP_MELEE &&
        (NPC->client->ps.stats[STAT_WEAPONS] &
         (1 << WP_THERMAL))) { // fisticuffs, make switch to thermal if have it
      // reset fire-timing variables
  ...
  if (NPC_ClearLOS(NPC->enemy)) {
    ...
    if (NPC->client->ps.weapon == WP_MELEE) {
      if (enemyDist <= 4096 &&
          InFOV(NPC->enemy->currentOrigin, NPC->currentOrigin,
                NPC->client->ps.viewangles, 90, 45)) // within 64 & infront
      {
        VectorCopy(NPC->enemy->currentOrigin, NPCInfo->enemyLastSeenLocation);
        enemyCS = qtrue;
      }
    } else if (InFOV(NPC->enemy->currentOrigin, NPC->currentOrigin,
                     NPC->client->ps.viewangles, 45, 90)) { // in front of me
      ...
      if (hit == NPC->enemy->s.number ||
        ...
        VectorCopy(NPC->enemy->currentOrigin, NPCInfo->enemyLastSeenLocation);
        float enemyHorzDist = DistanceHorizontalSquared(
            NPC->enemy->currentOrigin, NPC->currentOrigin);
  ...
  if (enemyCS) {
    ...
    if (NPC->client->ps.weapon == WP_THERMAL) { // don't chase and throw
      AImove = qfalse;
               ...
               enemyDist < (NPC->maxs[0] + NPC->enemy->maxs[0] + 16) *
                               (NPC->maxs[0] + NPC->enemy->maxs[0] +
                                16)) { // close enough
      AImove = qfalse;
    }
