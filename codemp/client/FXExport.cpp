...
void FX_PlayBoltedEffectID(int id, vec3_t org, const int boltInfo,
                           ...
                           qboolean isRelative) {
  theFxScheduler.PlayEffect(id, org, 0, boltInfo, ghoul2, -1, -1, -1, qfalse,
                            iLooptime, !!isRelative);
}
...
void FX_SetRefDefFromCGame(refdef_t *refdef) { FX_SetRefDef(refdef); }

qboolean FX_FreeSystem(void) { return (qboolean)FX_Free(true); }

void FX_AdjustTime(int time) { theFxHelper.AdjustTime(time); }
