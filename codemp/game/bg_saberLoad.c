...
qboolean WP_UseFirstValidSaberStyle(saberInfo_t *saber1, saberInfo_t *saber2,
  ...
  if (!validStyles) {
    if (dualSabers)
      Com_Printf("WARNING: No valid saber styles for %s/%s", saber1->name,
                 saber2->name);
    else
      Com_Printf("WARNING: No valid saber styles for %s", saber1->name);
  }

  // using an invalid style and have at least one valid style to use, so switch
  // to it
...
void WP_SaberSetDefaults(saberInfo_t *saber) {
  ...
  saber->bounceSound[0] =
      0; // none - if set, plays one of these 3 sounds when saber/sword hits a
         // wall and bounces off (must set bounceOnWall to 1 to use these
         // sounds) - NOTE: must provide all 3!!!
  saber->bounceSound[1] =
      0; // none - if set, plays one of these 3 sounds when saber/sword hits a
         // wall and bounces off (must set bounceOnWall to 1 to use these
         // sounds) - NOTE: must provide all 3!!!
...
qboolean WP_SaberParseParm(const char *saberName, const char *parmname,
                           char *saberData) {
  const char *token;
  const char *value;
  const char *p;
...
qboolean WP_SaberValidForPlayerInMP(const char *saberName) {
  char allowed[8] = {0};
  if (!WP_SaberParseParm(saberName, "notInMP",
                         allowed)) { // not defined, default is yes
    return qtrue;
  }
...
void WP_RemoveSaber(saberInfo_t *sabers, int saberNum) {
  ...
  //	{
  //		trap->G2API_RemoveGhoul2Model( ent->ghoul2,
  //ent->weaponModel[saberNum] ); 		ent->weaponModel[saberNum] = -1;
  //	}
  //	if ( saberNum == 1 )
  //	{
  //		ent->client->ps.dualSabers = qfalse;
...
void WP_SetSaber(int entNum, saberInfo_t *sabers, int saberNum,
  ...
  if ((sabers[1].saberFlags &
    ...
    return;
             ...
             sabers[1].model[0]) { // you can't use a two-handed saber with a
                                   // second saber, so remove saber 2
    WP_RemoveSaber(sabers, 1);
    return;
  }
}
...
void WP_SaberLoadParms(void) {
  ...
  *marker = 0;
  ...
  fileCnt =
      trap->FS_GetFileList("ext_data/sabers", ".sab", saberExtensionListBuf,
...
#ifdef UI_BUILD
      Com_Error(ERR_FATAL,
                "WP_SaberLoadParms: Saber extensions (*.sab) are too "
...
void WP_SaberGetHiltInfo(const char *singleHilts[MAX_SABER_HILTS],
  ...
  while (p) {
    ...
    if (WP_IsSaberTwoHanded(saberName)) {
      if (numStaffHilts <
          MAX_SABER_HILTS - 1) //-1 because we have to NULL terminate the list
      {
...
//	[in]		bool bActive	Whether to activate it (default true),
//or deactivate it (false). 	[return]	void
...
void BG_SI_SetDesiredLength(saberInfo_t *saber, float len, int bladeNum) {
  ...
  if (bladeNum >= 0 &&
      bladeNum < saber->numBlades) { // doing this on a specific blade
    startBlade = bladeNum;
