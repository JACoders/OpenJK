...
static void DoLine2(const vec3_t start, const vec3_t end, const vec3_t up,
                    float spanWidth, float spanWidth2) {
  int vbase;
...
static void DoLine_Oriented(const vec3_t start, const vec3_t end,
                            const vec3_t up, float spanWidth) {
  float spanWidth2;
  int vbase;
  ...
  tess.texCoords[tess.numVertexes][0][0] = 0;
  tess.texCoords[tess.numVertexes][0][1] =
      backEnd.currentEntity->e.data.line.stscale;
  tess.vertexColors[tess.numVertexes][0] =
      backEnd.currentEntity->e.shaderRGBA[0];
  tess.vertexColors[tess.numVertexes][1] =
      backEnd.currentEntity->e.shaderRGBA[1];
...
static void RB_SurfaceCylinder(void) {
  static polyVert_t lower_points[NUM_CYLINDER_SEGMENTS],
      upper_points[NUM_CYLINDER_SEGMENTS], verts[4];
...
// these functions are pretty crappy in terms of returning a nice range of rnd
// numbers, but it's probably good enough?
...
//----------------------------------------------------------------------------
{
  ...
  for (i = 20; i <= dis; i += 20) {
    ...
    VectorMA(temp, Q_crandom(&e->frame) * 7.0f * e->axis[0][0], rt,
             temp); // move more in direction perpendicular to line, angles is
                    // really the chaos
    VectorMA(temp, Q_crandom(&e->frame) * 7.0f * e->axis[0][0], up,
    ...
    if ((e->renderfx & RF_FORKED) && f_count > 0 &&
        Q_random(&e->frame) > 0.94f && radius * (1.0f - perc) > 0.2f) {
      vec3_t newDest;
...
//------------------------------------------
{
  ...
  if (e->renderfx & RF_GROW) {
    perc = 1.0f - (e->axis[0][2] /*endTime*/ - tr.refdef.time) /
                      e->axis[0][1] /*duration*/;
...
void RB_SurfaceEntity(surfaceType_t *surfType) {
  ...
  case RT_ENT_CHAIN: {
    ...
    for (i = 0; i < count; i++) {
      memcpy(&backEnd.currentEntity->e, &backEnd.refdef.miniEntities[start + i],
             sizeof(backEnd.refdef.miniEntities[start + i]));
