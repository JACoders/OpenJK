...
static void R_MipMap2(byte *in, int inWidth, int inHeight) {
	...
	for (i = 0; i < outHeight; i++) {
		for (j = 0; j < outWidth; j++) {
			...
			for (k = 0; k < 4; k++) {
				total = 1 * (&in[4 * (((i * 2 - 1) & inHeightMask) * inWidth + ((j * 2 - 1) & inWidthMask))])[k] +
	...
	for (i = 0; i < outHeight; i++) {
		...
		for (j = 0; j < outWidth; j++) {
			for (k = 0; k < 3; k++) {
				...
				total = sRGBtoRGB((double)current);
...
static void R_MipMapLuminanceAlpha(const byte *in, byte *out, int width, int height) {
	...
	for (i = 0; i < height; i++, in += row) {
		for (j = 0; j < width; j++, out += 4, in += 8) {
			out[0] = out[1] = out[2] = (in[0] + in[4] + in[row] + in[row + 4]) >> 2;
...
===============
*/
static void RawImage_ScaleToPower2(byte **data, int *inout_width, int *inout_height, int *inout_scaled_width, int *inout_scaled_height, imgType_t type,
								   int flags, byte **resampledBuffer) {
...
static int CalcNumMipmapLevels(int width, int height) { return static_cast<int>(ceil(log2(Q_max(width, height))) + 1); }
...
static void RawImage_UploadTexture(byte *data, int x, int y, int width, int height, GLenum internalFormat, imgType_t type, int flags, qboolean subtexture) {
	...
	if ((flags & IMGFLAG_MIPMAP) && (!r_simpleMipMaps->integer) && (data != NULL || !ShouldUseImmutableTextures(flags, internalFormat))) {
...
static bool IsPowerOfTwo(int i) { return (i & (i - 1)) == 0; }
...
static void Upload32(byte *data, int width, int height, imgType_t type, int flags, qboolean lightMap, GLenum internalFormat, int *pUploadWidth,
					 int *pUploadHeight) {
...
static void EmptyTexture(int width, int height, imgType_t type, int flags, qboolean lightMap, GLenum internalFormat, int *pUploadWidth, int *pUploadHeight) {
...
image_t *R_CreateImage(const char *name, byte *pic, int width, int height, imgType_t type, int flags, int internalFormat) {
	...
	if (image->flags & IMGFLAG_CUBEMAP) {
		...
		if (pic) {
			Upload32(pic, image->width, image->height, image->type, image->flags, isLightmap, image->internalFormat, &image->uploadWidth, &image->uploadHeight);
		} else {
			EmptyTexture(image->width, image->height, image->type, image->flags, isLightmap, image->internalFormat, &image->uploadWidth, &image->uploadHeight);
		}
...
void R_LoadPackedMaterialImage(shaderStage_t *stage, const char *packedImageName, int flags) {
	...
	if (image != NULL) {
		...
		stage->specularScale[0] = stage->specularScale[2] = stage->specularScale[3] = 1.0f;
...
image_t *R_BuildSDRSpecGlossImage(shaderStage_t *stage, const char *specImageName, int flags) {
	...
	for (int i = 0; i < specWidth * specHeight * 4; i += 4) {
		...
		float ratio =
			(sRGBtoRGB(currentColor[0]) + sRGBtoRGB(currentColor[1]) + sRGBtoRGB(currentColor[1])) / (currentColor[0] + currentColor[1] + currentColor[2]);
...
static void R_CreateEnvBrdfLUT(void) {
	...
	tr.envBrdfImage =
		R_CreateImage("*envBrdfLUT", (byte *)data, LUT_WIDTH, LUT_HEIGHT, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_RGB16F);
}
...
static void R_CreateDefaultImage(void) {
	...
	tr.defaultImage = R_CreateImage("*default", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE, IMGTYPE_COLORALPHA, IMGFLAG_MIPMAP, GL_RGBA8);
}
...
void R_CreateBuiltinImages(void) {
	...
	tr.whiteImage = R_CreateImage("*white", (byte *)data, 8, 8, IMGTYPE_COLORALPHA, IMGFLAG_NONE, GL_RGBA8);

	if (r_dlightMode->integer >= 2) {
		tr.pointShadowArrayImage =
			R_Create2DImageArray(va("*pointshadowmapImage"), NULL, DSHADOW_MAP_SIZE, DSHADOW_MAP_SIZE, MAX_DLIGHTS * 6, IMGTYPE_COLORALPHA,
								 IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE | IMGLFAG_SHADOWCOMP | IMGFLAG_MUTABLE, GL_DEPTH_COMPONENT16);
	}
	...
	for (x = 0; x < 32; x++) {
		tr.scratchImage[x] =
			R_CreateImage("*scratch", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE, IMGTYPE_COLORALPHA, IMGFLAG_PICMIP | IMGFLAG_CLAMPTOEDGE | IMGFLAG_MUTABLE, 0);
	}
	...
	for (int i = 0; i < ARRAY_LEN(tr.glowImageScaled); i++) {
		tr.glowImageScaled[i] = R_CreateImage(va("*glowScaled%d", i), NULL, glowImageWidth, glowImageHeight, IMGTYPE_COLORALPHA,
	...
	tr.renderDepthImage =
		R_CreateImage("*renderdepth", NULL, width, height, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_DEPTH24_STENCIL8);
	tr.textureDepthImage = R_CreateImage("*texturedepth", NULL, PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE, IMGTYPE_COLORALPHA,
	...
	if (r_shadows->integer == 4) {
		tr.pshadowArrayImage = R_Create2DImageArray(va("*pshadowmapArray"), NULL, PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE, MAX_DRAWN_PSHADOWS, IMGTYPE_COLORALPHA,
													IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE | IMGLFAG_SHADOWCOMP | IMGFLAG_MUTABLE, GL_DEPTH_COMPONENT16);
	}

	if (r_sunlightMode->integer) {
		tr.sunShadowArrayImage =
			R_Create2DImageArray(va("*sunShadowmapArray"), NULL, r_shadowMapSize->integer, r_shadowMapSize->integer,
								 3, // number of cascades
								 IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE | IMGLFAG_SHADOWCOMP | IMGFLAG_MUTABLE, GL_DEPTH_COMPONENT16);

		tr.screenShadowImage = R_CreateImage("*screenShadow", NULL, width, height, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_R8);
	}

	if (r_cubeMapping->integer) {
		tr.renderCubeImage = R_CreateImage("*renderCube", NULL, CUBE_MAP_SIZE, CUBE_MAP_SIZE, IMGTYPE_COLORALPHA,
										   IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE | IMGFLAG_MIPMAP | IMGFLAG_CUBEMAP, hdrFormat);

		tr.renderCubeDepthImage = R_CreateImage("*renderdepth", NULL, CUBE_MAP_SIZE, CUBE_MAP_SIZE, IMGTYPE_COLORALPHA,
												IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_DEPTH24_STENCIL8);
	}

	tr.weatherDepthImage =
		R_CreateImage("*weatherDepth", nullptr, 1024, 1024, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_DEPTH_COMPONENT16);
}
