...
void NoghriGasCloudThink(gentity_t *self) {
  ...
  if (level.time - self->s.time <= 2500) {
    if (!Q_irand(0, 3 - g_spskill->integer)) {
      G_RadiusDamage(self->currentOrigin, self->owner, Q_irand(1, 4),
                     self->splashRadius, self->owner,
...
void G_SpawnNoghriGasCloud(
    gentity_t *ent) { // FIXME: force-pushable/dispersable?
...
void G_MissileImpacted(gentity_t *ent, gentity_t *other, vec3_t impactPos,
  ...
  if (other->takedamage) {
    ...
    if (ent->damage) {
      ...
      if (ent->s.weapon == WP_DEMP2) { // a hit with demp2 decloaks saboteurs
        if (other && other->client &&
            other->client->NPC_class ==
                CLASS_SABOTEUR) { // FIXME: make this disabled cloak hold for
                                  // some amount of time?
          Saboteur_Decloak(other, Q_irand(3000, 10000));
          if (ent->methodOfDeath ==
              MOD_DEMP2_ALT) { // direct hit with alt disabled cloak forever
            if (other->NPC) {  // permanently disable the saboteur's cloak
  ...
  {
    // Add the event
    AddSoundEvent(ent->owner, ent->currentOrigin, 256, AEL_SUSPICIOUS, qfalse,
                  qtrue);
...
static void G_MissileAddAlerts(gentity_t *ent) {
  // Add the event
  if (ent->s.weapon == WP_THERMAL &&
      ((ent->delay - level.time) < 2000 ||
       ent->s.pos.trType ==
           TR_INTERPOLATE)) { // a thermal about to explode or rolling
    if ((ent->delay - level.time) < 500) { // half a second before it explodes!
      AddSoundEvent(ent->owner, ent->currentOrigin, ent->splashRadius * 2,
                    AEL_DANGER_GREAT, qfalse, qtrue);
      AddSightEvent(ent->owner, ent->currentOrigin, ent->splashRadius * 2,
                    AEL_DANGER_GREAT, 20);
    } else { // 2 seconds until it explodes or it's rolling
      AddSoundEvent(ent->owner, ent->currentOrigin, ent->splashRadius * 2,
...
void G_MissileImpact(gentity_t *ent, trace_t *trace, int hitLoc = HL_NONE) {
  ...
  if (bounce) {
    ...
    G_MissileBounceEffect(ent, trace->endpos, trace->plane.normal,
                          (qboolean)(trace->entityNum == ENTITYNUM_WORLD));
  ...
  if (ent->s.eFlags & EF_MISSILE_STICK) {
    if (ent->owner) //&& ent->owner->s.number == 0 )
    {
      ...
      if (ent->s.weapon == WP_TRIP_MINE) {
        AddSoundEvent(ent->owner, ent->currentOrigin, ent->splashRadius / 2,
                      AEL_DISCOVERED /*AEL_DANGER*/, qfalse, qtrue);
        AddSightEvent(ent->owner, ent->currentOrigin, ent->splashRadius * 2,
                      AEL_DISCOVERED /*AEL_DANGER*/, 60);
        /*
        AddSoundEvent( ent->owner, ent->currentOrigin, ent->splashRadius*2,
        AEL_DANGER, qfalse, qtrue ); AddSightEvent( ent->owner,
        ent->currentOrigin, ent->splashRadius*2, AEL_DANGER, 60 );
        */
      } else {
        AddSoundEvent(ent->owner, ent->currentOrigin, 128, AEL_DISCOVERED,
                      qfalse, qtrue);
  ...
  if (other->contents & CONTENTS_LIGHTSABER) {
    ...
    if ((g_spskill->integer <= 0 // on easy, it reflects all shots
        ...
        (!ent->splashDamage ||
         !ent->splashRadius) // this would be cool, though, to "bat" the thermal
                             // det away...
        && ent->s.weapon != WP_NOGHRI_STICK) // gas bomb, don't reflect
    {
      ...
      if (!other->owner || !other->owner->client ||
          ...
          (InFront(ent->currentOrigin, other->owner->currentOrigin,
           ...
           !WP_DoingMoronicForcedAnimationForForcePowers(
               other))) // other->owner->s.number != 0 ||
      {                 // Jedi cannot block shots from behind!
        int blockChance = 0;
        switch (other->owner->client->ps.forcePowerLevel
                    [FP_SABER_DEFENSE]) { // level 1 reflects 50% of the time,
                                          // level 2 reflects 75% of the time
        case FORCE_LEVEL_3:
          blockChance = 10;
          break;
        case FORCE_LEVEL_2:
        ...
        if (blockChance && (other->owner->client->ps.forcePowersActive &
                            (1 << FP_SPEED))) { // in in force speed, better
                                                // chance of deflecting the shot
...
void G_ExplodeMissile(gentity_t *ent) {
  ...
  {
    // Add the event
    AddSoundEvent(ent->owner, ent->currentOrigin, 256, AEL_DISCOVERED, qfalse,
                  qtrue); // FIXME: are we on ground or not?
...
void G_RollMissile(gentity_t *ent) {
  ...
  if (objPML.groundPlane) { // FIXME: never happens!
    ...
    G_ClipVelocity(ent->s.pos.trDelta, objPML.groundTrace.plane.normal,
...
void G_RunMissile(gentity_t *ent) {
  ...
  if ((ent->s.eFlags & EF_HELD_BY_SAND_CREATURE)) { // in a sand creature's
                                                    // mouth
    if (ent->activator) {
      ...
      gi.G2API_GetBoltMatrix(
          ent->activator->ghoul2, ent->activator->playerModel,
          ent->activator->gutBolt, &boltMatrix, scAngles,
          ent->activator->currentOrigin, (cg.time ? cg.time : level.time), NULL,
  ...
  */
  ...
  if (tr.fraction == 1) {
    if (ent->s.weapon == WP_THERMAL &&
        ent->s.pos.trType ==
            TR_INTERPOLATE) { // a rolling thermal that didn't hit anything
      G_MissileAddAlerts(ent);
    }
    return;
  }
