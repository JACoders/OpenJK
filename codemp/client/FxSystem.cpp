...
//------------------------------------------------------
SFxHelper::SFxHelper()
    : mTime(0), mOldTime(0), mFrameTime(0), mTimeFrozen(false), refdef(0) {}
...
//------------------------------------------------------
qboolean SFxHelper::GetOriginAxisFromBolt(CGhoul2Info_v *pGhoul2, int mEntNum,
  ...
  CGVM_GetLerpData(); // this func will zero out pitch and roll for players, and
                      // ridable vehicles

  // Fixme: optimize these VM calls away by storing

  // go away and get me the bolt position for this frame please
  doesBoltExist = re->G2API_GetBoltMatrix(
      *pGhoul2, modelNum, boltNum, &boltMatrix, data->mAngles, data->mOrigin,
      theFxHelper.mOldTime, 0, data->mScale);

  if (doesBoltExist) { // set up the axis and origin we need for the actual
                       // effect spawning
