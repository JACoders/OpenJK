...
qboolean NPC_ClearPathToGoal(vec3_t dir, gentity_t *goal) {
  ...
  if (NAV_CheckAhead(NPC, goal->currentOrigin, trace,
  ...
  if (goal->svFlags & SVF_NAVGOAL) {
    ...
    if (NAV_HitNavGoal(trace.endpos, NPC->mins, NPC->maxs, goal->currentOrigin,
...
qboolean NAV_DirSafe(gentity_t *self, vec3_t dir, float dist) {
  ...
  gi.trace(&trace, self->currentOrigin, mins, self->maxs, end, self->s.number,
           CONTENTS_BOTCLIP, (EG2_Collision)0, 0);
...
qboolean NAV_MoveDirSafe(gentity_t *self, usercmd_t *cmd,
  ...
  if (VectorCompare(self->client->ps.moveDir,
                    vec3_origin)) {             // no movedir, build from cmd
    if (!cmd->forwardmove && !cmd->rightmove) { // not moving at all
      return qtrue;
    }
  ...
  return (
      NAV_DirSafe(self, moveDir, (self->client->ps.speed / 10.0f) * distScale));
}
