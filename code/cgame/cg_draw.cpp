  ...
  if (cg.missionInfoFlashTime > cg.time) {
    if (!((cg.time / 600) & 1)) {
      if (!cg.messageLitActive) {
        ...
        kef 4/16/03 --	as fun as this was, its time has passed. I will,
        however, hijack this cvar at James' recommendation and use it for
...
Draw the force power graphics (tics) and the force power numeric amount. Any
tics that are partial will be alphaed out.
================
*/
static void CG_DrawForcePower(const centity_t *cent, const int xPos,
                              const int yPos) {
  int i;
  ...
  for (i = MAX_HUD_TICS - 1; i >= 0; i--) {
    if (extra) { // supercharged
      ...
      percent =
          0.75f + (sin(cg.time * 0.005f) *
                   ((extra / cent->gent->client->ps.forcePowerMax) * 0.25f));
      calcColor[0] *= percent;
      calcColor[1] *= percent;
      calcColor[2] *= percent;
    ...
    CG_DrawPic(forceTics[i].xPos, forceTics[i].yPos, forceTics[i].width,
               forceTics[i].height, forceTics[i].background);
  ...
  CG_DrawNumField(
      otherHUDBits[OHB_FORCEAMOUNT].xPos, otherHUDBits[OHB_FORCEAMOUNT].yPos, 3,
      cent->gent->client->ps.forcePower, otherHUDBits[OHB_FORCEAMOUNT].width,
      otherHUDBits[OHB_FORCEAMOUNT].height, NUM_FONT_SMALL, qfalse);
}
...
================
*/
static void CG_DrawSaberStyle(const centity_t *cent, const int xPos,
                              const int yPos) {
  int index;
  ...
  if (!cg.saberAnimLevelPending &&
      cent->gent
          ->client) { // uninitialized after a loadgame, cheat across and get it
    cg.saberAnimLevelPending = cent->gent->client->ps.saberAnimLevel;
  }

  // don't need to draw ammo, but we will draw the current saber style in this
  // window
  ...
  CG_DrawPic(otherHUDBits[index].xPos, otherHUDBits[index].yPos,
             otherHUDBits[index].width, otherHUDBits[index].height,
...
Draw the ammo graphics (tics) and the ammo numeric amount. Any tics that are
partial will be alphaed out.
================
*/
static void CG_DrawAmmo(const centity_t *cent, const int xPos, const int yPos) {
  ...
  CG_DrawNumField(otherHUDBits[OHB_AMMOAMOUNT].xPos,
                  otherHUDBits[OHB_AMMOAMOUNT].yPos, 3,
                  ps->ammo[weaponData[cent->currentState.weapon].ammoIndex],
                  otherHUDBits[OHB_AMMOAMOUNT].width,
                  otherHUDBits[OHB_AMMOAMOUNT].height, NUM_FONT_SMALL, qfalse);

  inc = (float)ammoData[weaponData[cent->currentState.weapon].ammoIndex].max /
        MAX_HUD_TICS;
  currValue = ps->ammo[weaponData[cent->currentState.weapon].ammoIndex];
  memcpy(calcColor, colorTable[CT_WHITE], sizeof(vec4_t));
  ...
  for (i = MAX_HUD_TICS - 1; i >= 0; i--) {
    ...
    cgi_R_SetColor(calcColor);
    CG_DrawPic(ammoTics[i].xPos, ammoTics[i].yPos, ammoTics[i].width,
               ammoTics[i].height, ammoTics[i].background);
...
static void CG_DrawHealth(const int x, const int y, const int w, const int h) {
  ...
  for (i = (MAX_HUD_TICS - 1); i >= 0; i--) {
    ...
    CG_DrawPic(healthTics[i].xPos, healthTics[i].yPos, healthTics[i].width,
               healthTics[i].height, healthTics[i].background);
  ...
  CG_DrawNumField(
      otherHUDBits[OHB_HEALTHAMOUNT].xPos, otherHUDBits[OHB_HEALTHAMOUNT].yPos,
      3, ps->stats[STAT_HEALTH], otherHUDBits[OHB_HEALTHAMOUNT].width,
...
Draw the armor graphics (tics) and the armor numeric amount. Any tics that are
partial will be alphaed out.
================
*/
static void CG_DrawArmor(const int x, const int y, const int w, const int h) {
  ...
  // Look at the amount of armor left and show only as much of the graphic as
  // there is armor. Use alpha to fade out partial section of armor MAX_HEALTH
  // is the same thing as max armor
  float inc = (float)ps->stats[STAT_MAX_HEALTH] / MAX_HUD_TICS;
  ...
  for (i = (MAX_HUD_TICS - 1); i >= 0; i--) {
    ...
    if ((i == (MAX_HUD_TICS - 1)) && (currValue < inc)) {
      if (cg.HUDArmorFlag) {
        CG_DrawPic(armorTics[i].xPos, armorTics[i].yPos, armorTics[i].width,
                   armorTics[i].height, armorTics[i].background);
      }
    } else {
      CG_DrawPic(armorTics[i].xPos, armorTics[i].yPos, armorTics[i].width,
                 armorTics[i].height, armorTics[i].background);
  ...
  CG_DrawNumField(otherHUDBits[OHB_ARMORAMOUNT].xPos,
                  otherHUDBits[OHB_ARMORAMOUNT].yPos, 3, ps->stats[STAT_ARMOR],
...
static void CG_DrawVehicleSheild(const centity_t *cent, const Vehicle_t *pVeh) {
  ...
  if (pVeh->m_pVehicleInfo->type == VH_ANIMAL ||
      pVeh->m_pVehicleInfo->type == VH_FLIER) {
    maxHealth = 100.0f;
  ...
  if (cgi_UI_GetMenuItemInfo("swoopvehiclehud", "shieldbackground", &xPos,
                             &yPos, &width, &height, color, &background)) {
  ...
  for (i = 1; i <= MAX_VHUD_SHIELD_TICS; i++) {
    ...
    if (!cgi_UI_GetMenuItemInfo("swoopvehiclehud", itemName, &xPos, &yPos,
...
static void CG_DrawVehicleTurboRecharge(const centity_t *cent,
                                        const Vehicle_t *pVeh) {
  int xPos, yPos, width, height;
  qhandle_t background;
  vec4_t color;

  if (cgi_UI_GetMenuItemInfo("swoopvehiclehud", "turborecharge", &xPos, &yPos,
                             &width, &height, color, &background)) {
    float percent = 0.0f;
...
static void CG_DrawVehicleSpeed(const centity_t *cent, const Vehicle_t *pVeh,
                                const char *entHud) {
  int xPos, yPos, width, height;
  qhandle_t background;
  ...
  if (cgi_UI_GetMenuItemInfo(entHud, "speedbackground", &xPos, &yPos, &width,
  ...
  for (i = 1; i <= MAX_VHUD_SPEED_TICS; i++) {
    ...
    if (!cgi_UI_GetMenuItemInfo(entHud, itemName, &xPos, &yPos, &width, &height,
                                color, &background)) {
      continue;
    }
...
static void CG_DrawVehicleArmor(const centity_t *cent, const Vehicle_t *pVeh) {
  ...
  if (cgi_UI_GetMenuItemInfo("swoopvehiclehud", "armorbackground", &xPos, &yPos,
                             &width, &height, color, &background)) {
    cgi_R_SetColor(color);
    CG_DrawPic(xPos, yPos, width, height, background);
  }
...
static void CG_DrawVehicleAmmo(const centity_t *cent, const Vehicle_t *pVeh) {
  ...
  if (cgi_UI_GetMenuItemInfo("swoopvehiclehud", "ammobackground", &xPos, &yPos,
                             &width, &height, color, &background)) {
    cgi_R_SetColor(color);
    CG_DrawPic(xPos, yPos, width, height, background);
  }
...
static void CG_DrawVehicleAmmoUpper(const centity_t *cent,
                                    const Vehicle_t *pVeh) {
  int xPos, yPos, width, height, i;
  qhandle_t background;
  char itemName[64];
  ...
  if (cgi_UI_GetMenuItemInfo("swoopvehiclehud", "ammoupperbackground", &xPos,
                             &yPos, &width, &height, color, &background)) {
    cgi_R_SetColor(color);
    CG_DrawPic(xPos, yPos, width, height, background);
  }
...
static void CG_DrawVehicleAmmoLower(const centity_t *cent,
                                    const Vehicle_t *pVeh) {
  int xPos, yPos, width, height, i;
  qhandle_t background;
  char itemName[64];
  ...
  if (cgi_UI_GetMenuItemInfo("swoopvehiclehud", "ammolowerbackground", &xPos,
                             &yPos, &width, &height, color, &background)) {
    cgi_R_SetColor(color);
    CG_DrawPic(xPos, yPos, width, height, background);
  }
...
static void CG_DrawVehicleHud(const centity_t *cent, const Vehicle_t *pVeh) {
  ...
  // Draw frame
  if (cgi_UI_GetMenuItemInfo("swoopvehiclehud", "leftframe", &xPos, &yPos,
                             &width, &height, color, &background)) {
    cgi_R_SetColor(color);
    CG_DrawPic(xPos, yPos, width, height, background);
  }

  if (cgi_UI_GetMenuItemInfo("swoopvehiclehud", "rightframe", &xPos, &yPos,
                             &width, &height, color, &background)) {
    cgi_R_SetColor(color);
    CG_DrawPic(xPos, yPos, width, height, background);
  }
...
static void CG_DrawPanelTurretHud(void) {
  ...
  CG_DrawItemHealth(g_entities[cg.snap->ps.viewEntity].health,
...
static void CG_DrawATSTHud(centity_t *cent) {
  ...
  if (g_entities[cg.snap->ps.viewEntity]
          .activator) // ensure we can look back to the atst_drivable to get the
                      // max health
  {
    health = (g_entities[cg.snap->ps.viewEntity].health +
              g_entities[cg.snap->ps.viewEntity].client->ps.stats[STAT_ARMOR]);
  } else {
    health = (g_entities[cg.snap->ps.viewEntity].health +
              g_entities[cg.snap->ps.viewEntity].client->ps.stats[STAT_ARMOR]);
  }

  CG_DrawItemHealth(health,
                    g_entities[cg.snap->ps.viewEntity].activator->max_health);

  if (cgi_UI_GetMenuItemInfo("atsthud", "background", &xPos, &yPos, &width,
                             &height, color, &background)) {
    cgi_R_SetColor(color);
  ...
  if (cgi_UI_GetMenuItemInfo("atsthud", "outer_frame", &xPos, &yPos, &width,
                             &height, color, &background)) {
    cgi_R_SetColor(color);
    CG_DrawPic(xPos, yPos, width, height, background);
  }

  if (cgi_UI_GetMenuItemInfo("atsthud", "left_pic", &xPos, &yPos, &width,
                             &height, color, &background)) {
    cgi_R_SetColor(color);
...
static qboolean CG_DrawCustomHealthHud(centity_t *cent) {
  ...
  if ((cent->currentState.eFlags & EF_LOCKED_TO_WEAPON)) {
    ...
    /*		color[0] = color[1] = color[2] = 0.0f;
                    ...
                    if ( cent->gent && cent->gent->owner )
                    {
                            if (( cent->gent->owner->flags & FL_GODMODE ))
                            {
                                    // chair is in godmode, so render the health
       of the player instead health = cent->gent->health /
       (float)cent->gent->max_health;
                            }
                            else
                            {
                                    // render the chair health
                                    health = cent->gent->owner->health /
       (float)cent->gent->owner->max_health;
                            }
                    }
                    ...
                    cgi_R_SetColor( color );
                    CG_DrawPic( 18, 480 - 41, 87 * health, 19,
       cgs.media.whiteShader );

                    cgi_R_SetColor( colorTable[CT_WHITE] );
                    CG_DrawPic( 2, 480 - 64, 128, 64,
       cgs.media.emplacedHealthBarShader);
    */
  ...
  // In an ATST
    ...
    /*
                    ...
                    // we just calc the display value from the sum of health and
       armor if ( g_entities[cg.snap->ps.viewEntity].activator ) // ensure we
       can look back to the atst_drivable to get the max health
                    {
                            health = ( g_entities[cg.snap->ps.viewEntity].health
       + g_entities[cg.snap->ps.viewEntity].client->ps.stats[STAT_ARMOR] ) /
                                    (float)(g_entities[cg.snap->ps.viewEntity].max_health
       ...
       + g_entities[cg.snap->ps.viewEntity].client->ps.stats[STAT_ARMOR]) /
                                    (float)(g_entities[cg.snap->ps.viewEntity].max_health
       + 800 ); // hacked max armor since we don't have an activator...should
       never happen
                    }
  ...
  // Other?
  else if (cg.snap->ps.viewEntity &&
           (g_entities[cg.snap->ps.viewEntity].dflags & DAMAGE_CUSTOM_HUD)) {
...
#define SimpleHud_DrawString(x, y, str, color)                                 \
  cgi_R_Font_DrawString(x, y, str, color,                                      \
                        (int)0x80000000 | cgs.media.qhFontSmall, -1, 1.0f)
...
static void CG_DrawSimpleSaberStyle(const centity_t *cent) {
  ...
  if (!cg.saberAnimLevelPending && cent->gent &&
      cent->gent
          ->client) { // uninitialized after a loadgame, cheat across and get it
    cg.saberAnimLevelPending = cent->gent->client->ps.saberAnimLevel;
  }
  ...
  SimpleHud_DrawString(SCREEN_WIDTH - (weapX + 16 + 32),
                       (SCREEN_HEIGHT - 80) + 40, num, colorTable[calcColor]);
}
...
static void CG_DrawSimpleAmmo(const centity_t *cent) {
  ...
  if (currValue < 0 ||
    ...
    SimpleHud_DrawString(SCREEN_WIDTH - (16 + 32), (SCREEN_HEIGHT - 80) + 40,
  ...
  SimpleHud_DrawString(SCREEN_WIDTH - (16 + 32), (SCREEN_HEIGHT - 80) + 40, num,
...
static void CG_DrawSimpleForcePower(const centity_t *cent) {
  ...
  SimpleHud_DrawString(SCREEN_WIDTH - (16 + 32), (SCREEN_HEIGHT - 80) + 40 + 14,
                       num, colorTable[calcColor]);
}
...
static void CG_DrawHUD(centity_t *cent) {
  ...
  if (cg_hudFiles.integer) {
    ...
    SimpleHud_DrawString(x + 16, y + 40,
                         va("%i", cg.snap->ps.stats[STAT_HEALTH]),
                         colorTable[CT_HUD_RED]);

    SimpleHud_DrawString(x + 18 + 14, y + 40 + 14,
  ...
  if (cgi_UI_GetMenuInfo("lefthud", &sectionXPos, &sectionYPos, &sectionWidth,
                         &sectionHeight)) {
    ...
    if (cg_drawStatus.integer == 2) {
      CG_DrawSmallStringColor(sectionXPos + 5, sectionYPos - 60,
                              va("Armor:%d", cg.snap->ps.stats[STAT_ARMOR]),
                              colorTable[CT_HUD_GREEN]);
      CG_DrawSmallStringColor(sectionXPos + 5, sectionYPos - 40,
    ...
    CG_DrawPic(otherHUDBits[OHB_SCANLINE_LEFT].xPos,
    ...
    CG_DrawPic(
        otherHUDBits[OHB_FRAME_LEFT].xPos, otherHUDBits[OHB_FRAME_LEFT].yPos,
        otherHUDBits[OHB_FRAME_LEFT].width, otherHUDBits[OHB_FRAME_LEFT].height,
  ...
  if (cgi_UI_GetMenuInfo("righthud", &sectionXPos, &sectionYPos, &sectionWidth,
                         &sectionHeight)) {
    // Draw all the HUD elements --eez
    ...
    if (cg_drawStatus.integer == 2) {
      if (cent->currentState.weapon != WP_SABER &&
          cent->currentState.weapon != WP_STUN_BATON && cent->gent) {
        value =
            cg.snap->ps.ammo[weaponData[cent->currentState.weapon].ammoIndex];
        CG_DrawSmallStringColor(sectionXPos, sectionYPos - 60,
                                va("Ammo:%d", value), colorTable[CT_HUD_GREEN]);
      }
      CG_DrawSmallStringColor(sectionXPos, sectionYPos - 40,
    ...
    CG_DrawPic(otherHUDBits[OHB_SCANLINE_RIGHT].xPos,
    ...
    CG_DrawPic(otherHUDBits[OHB_FRAME_RIGHT].xPos,
...
void CG_DrawDataPadHUD(centity_t *cent) {
  ...
  if ((missionInfo_Updated) && ((cg_updatedDataPadForcePower1.integer) ||
                                (cg_updatedDataPadObjective.integer))) {
    // Stop flashing light
    cg.missionInfoFlashTime = 0;
    missionInfo_Updated = qfalse;

    // Set which force power to show.
    // cg_updatedDataPadForcePower is set from Q3_Interface, because force
    // powers would only be given from a script.
    if (cg_updatedDataPadForcePower1.integer) {
      cg.DataPadforcepowerSelect =
          cg_updatedDataPadForcePower1.integer - 1; // Not pretty, I know
...
static void CG_DrawBinocularNumbers(qboolean power) {
  ...
  if (power) {
    ...
    for (int i = -10; i < 30; i += 10) {
      ...
      // too far to the left it will poke outside the mask.
...
static void CG_DrawZoomMask(void) {
  ...
  float charge = cg.snap->ps.batteryCharge /
                 (float)MAX_BATTERIES; // convert charge to a percentage
  ...
  //--------------------------------
    ...
    max = cg_entities[0]
              .gent->client->ps.ammo[weaponData[WP_DISRUPTOR].ammoIndex] /
    ...
    for (float i = 18.5f; i <= 18.5f + max;
         i += 3) // going from 15 to 45 degrees, with 5 degree increments
    {
    ...
    // FIXME: doesn't know about ammo!! which is bad because it draws charge
    // beyond what ammo you may have..
    if (cg_entities[0].gent->client->ps.weaponstate == WEAPON_CHARGING_ALT) {
      ...
      max = (cg.time - cg_entities[0].gent->client->ps.weaponChargeTime) /
            (150.0f * 10.0f); // bad hardcodedness 150 is disruptor charge unit
                              // and 10 is max charge units allowed.

      if (max > 1.0f) {
        max = 1.0f;
      }

      CG_DrawPic2(257, 435, 134 * max, 34, 0, 0, max, 1,
  ...
  //--------------------------------
    ...
    if (power) {
      ...
      if (light <
          -81) // saber can really jack up local light levels....?magic number??
      {
        light = -81;
      }

      float pos1 = 220 + light;
      float pos2 = 220 + cos(cg.time * 0.0004f + light * 0.05f) * 40 +
                   sin(cg.time * 0.0013f + 1) * 20 + sin(cg.time * 0.0021f) * 5;
...
static void CG_DrawPickupItem(void) {
  ...
  if (value && cg_items[value].icon != -1) {
    ...
    if (fadeColor) {
      ...
      // CG_DrawBigString( ICON_SIZE + 16, 398, bg_itemlist[ value ].classname,
      // fadeColor[0] ); CG_DrawProportionalString( ICON_SIZE + 16, 398,
...
void CG_DrawHealthBar(centity_t *cent, float chX, float chY, float chW,
  ...
  CG_FillRect(x + (percent * chW), y + 1.0f, chW - (percent * chW) - 1.0f,
              chH - 1.0f, cColor);
}
...
void CG_AddHealthBarEnt(int entNum) {
  if (cg_numHealthBarEnts >=
      MAX_HEALTH_BAR_ENTS) { // FIXME: Debug error message?
    return;
  }

  if (DistanceSquared(cg_entities[entNum].lerpOrigin,
                      g_entities[0].client->renderInfo.eyePoint) <
      (HEALTHBARRANGE * HEALTHBARRANGE)) {
...
static void CG_DrawCrosshair(vec3_t worldPoint) {
  ...
  if (g_crosshairEntNum >= ENTITYNUM_WORLD) {
    ...
    if (crossEnt->client) {
      if (crossEnt->client->ps.powerups[PW_CLOAKED]) { // cloaked don't show up
        ...
        ecolor[2] = 1.0f;                              // B
                 ...
                 g_entities[0].client->playerTeam ==
                     TEAM_FREE) { // evil player: everyone is red
        // Enemies are red
        ecolor[0] = 1.0f; // R
        ecolor[1] = 0.1f; // G
        ecolor[2] = 0.1f; // B
        ...
        // NOTE: was yellow, but making it white unless they really decide they
        // want to see colors
        ecolor[0] = 1.0f; // R
        ecolor[1] = 1.0f; // G
        ecolor[2] = 1.0f; // B
    ...
    } else if (crossEnt->s.weapon == WP_TURRET &&
               (crossEnt->svFlags & SVF_NONNPC_ENEMY)) {
      // a turret
      ...
      ecolor[1] = 0.0;                                 // G
      ecolor[2] = 0.0;                                 // B
    } else if ((crossEnt->flags & FL_RED_CROSSHAIR)) { // special case flagged
                                                       // to turn crosshair red
      ecolor[0] = 1.0; // R
      ecolor[1] = 0.0; // G
      ecolor[2] = 0.0; // B
    } else {
      ...
      if (!ecolor[0] && !ecolor[1] && !ecolor[2]) {
        // We don't want a black crosshair, so use white since it will show up
        // better
        ecolor[0] = 1.0f; // R
        ecolor[1] = 1.0f; // G
        ecolor[2] = 1.0f; // B
  ...
  if (corona) // we are pointing at a crosshair item
  {
    ...
    if (cg.forceCrosshairEndTime) {
      // must have just gone over a force thing again...and we were in the
      // process of fading out.  Set fade in level to the level where the fade
      // left off
  ...
  {
    if (cg.forceCrosshairStartTime &&
        !cg.forceCrosshairEndTime) // were currently fading in
    {
      // must fade back out, but we will have to set the fadeout time to be
      // equal to the current level of faded-in-edness
      cg.forceCrosshairEndTime = cg.time - ecolor[3] * 500.0f;
    }
    if (cg.forceCrosshairEndTime &&
        cg.time - cg.forceCrosshairEndTime >
            500.0f) // not pointing at anything and fade out is totally done
    {
      // reset everything
  ...
  if (cg.forceCrosshairStartTime &&
      cg_crosshairForceHint.integer) // drawing extra bits
  {
    ...
    cgi_R_DrawStretchPic(x + cg.refdef.x + 0.5f * (640 - w),
                         y + cg.refdef.y + 0.5f * (480 - h), w, h, 0, 0, 1, 1,
...
// I'm keeping the rocket tracking code separate for now since I may want to do
// different logic...but it still uses trace info from scanCrosshairEnt
//-----------------------------------------
static void CG_ScanForRocketLock(void)
//-----------------------------------------
{
  gentity_t *traceEnt;
  static qboolean tempLock =
      qfalse; // this will break if anything else uses this locking code ( other
              // than the player )

  traceEnt = &g_entities[g_crosshairEntNum];

  if (!traceEnt || g_crosshairEntNum <= 0 ||
      g_crosshairEntNum >= ENTITYNUM_WORLD ||
      (!traceEnt->client && traceEnt->s.weapon != WP_TURRET) ||
      !traceEnt->health ||
      (traceEnt && traceEnt->client &&
    ...
    if (g_rocketSlackTime + 500 >= cg.time &&
      ...
      // were locked onto an ent, aren't right now.....but still within the slop
      // grace period
    ...
    if (!tempLock && g_rocketLockEntNum < ENTITYNUM_WORLD && dif >= 8) {
      ...
      if (g_rocketLockTime + 1200 < cg.time) {
        g_rocketLockTime = cg.time - 1200; // doh, hacking the time so the
                                           // targetting still gets drawn full
      }
    }

    // keep locking to this thing for one second after it gets out of view
    if (g_rocketLockTime + 2000.0f <
        cg.time) // since time was hacked above, I'm compensating so that 2000ms
                 // is really only 1000ms
    {
...
static void CG_ScanForCrosshairEntity(qboolean scanAll) {
  ...
  if (cg_entities[0].gent &&
      cg_entities[0].gent->client) // <-Mike said it should always do this //if
                                   // (cg_crosshairForceHint.integer &&
  { // try to check for force-affectable stuff first
    ...
    if ((pVeh = G_IsRidingVehicle(cg_entities[0].gent)) != NULL &&
      ...
      VectorCopy(cg_entities[pVeh->m_pParentEntity->s.number].lerpOrigin,
                 start);
      AngleVectors(cg_entities[pVeh->m_pParentEntity->s.number].lerpAngles, d_f,
                   d_rt, d_up);
    } else {
    ...
    // YES!  This is very very bad... but it works!  James made me do it.
    // Really, he did.  Blame James.
    gi.trace(
        &trace, start, vec3_origin, vec3_origin, end, ignoreEnt,
        MASK_OPAQUE | CONTENTS_SHOTCLIP | CONTENTS_BODY | CONTENTS_ITEM |
            CONTENTS_TERRAIN,
        G2_NOCOLLIDE,
        10); // ); took out CONTENTS_SOLID| so you can target people through
             // glass.... took out CONTENTS_CORPSE so disintegrated guys aren't
             // shown, could just remove their body earlier too...
      ...
      if (traceEnt) {
        ...
        if (traceEnt->client) { // is a client
          if (cg_entities[0].gent->client->ps.forcePowerLevel[FP_TELEPATHY] &&
              ...
              VALIDSTRING(
                  traceEnt
                      ->behaviorSet[BSET_MINDTRICK])) { // I have the ability to
                                                        // mind-trick and he is
                                                        // alive and he has a
                                                        // mind trick script
        ...
        // No?  Check for force-push/pullable doors and func_statics
          ...
          if (!Q_stricmp("func_door", traceEnt->classname)) { // it's a
                                                              // func_door
            if (traceEnt->spawnflags &
                2 /*MOVER_FORCE_ACTIVATE*/) { // it's force-usable
              if (cg_entities[0].gent->client->ps.forcePowerLevel[FP_PULL] ||
                  cg_entities[0]
                      .gent->client->ps
                      .forcePowerLevel[FP_PUSH]) { // player has push or pull
                float maxRange;
                if (cg_entities[0].gent->client->ps.forcePowerLevel[FP_PULL] >
                    cg_entities[0]
                        .gent->client->ps
                        .forcePowerLevel[FP_PUSH]) { // use the better range
                  maxRange = forcePushPullRadius[cg_entities[0]
                                                     .gent->client->ps
                                                     .forcePowerLevel[FP_PULL]];
                } else { // use the better range
                  maxRange = forcePushPullRadius[cg_entities[0]
                                                     .gent->client->ps
                                                     .forcePowerLevel[FP_PUSH]];
                }
                if (maxRange >=
                    trace.fraction * 2048) { // actually close enough to use one
                                             // of our force powers on it
                  cg_forceCrosshair = qtrue;
                }
              }
            }
          } else if (!Q_stricmp("func_static",
                                traceEnt->classname)) { // it's a func_static
            if ((traceEnt->spawnflags & 1 /*F_PUSH*/) &&
                (traceEnt->spawnflags & 2 /*F_PULL*/)) { // push or pullable
              float maxRange;
              if (cg_entities[0].gent->client->ps.forcePowerLevel[FP_PULL] >
                  cg_entities[0]
                      .gent->client->ps
                      .forcePowerLevel[FP_PUSH]) { // use the better range
                maxRange = forcePushPullRadius
                    [cg_entities[0].gent->client->ps.forcePowerLevel[FP_PULL]];
              } else { // use the better range
                maxRange = forcePushPullRadius
                    [cg_entities[0].gent->client->ps.forcePowerLevel[FP_PUSH]];
              }
              if (maxRange >=
                  trace.fraction * 2048) { // actually close enough to use one
                                           // of our force powers on it
              ...
              if (forcePushPullRadius[cg_entities[0]
                                          .gent->client->ps
                                          .forcePowerLevel[FP_PUSH]] >=
                  trace.fraction *
                      2048) { // actually close enough to use force push on it
                cg_forceCrosshair = qtrue;
              }
              ...
              if (forcePushPullRadius[cg_entities[0]
                                          ...
                                          .forcePowerLevel[FP_PULL]] >=
                  trace.fraction *
                      2048) { // actually close enough to use force pull on it
                cg_forceCrosshair = qtrue;
              }
            }
          }
  ...
  if (!cg_forceCrosshair) {
    if (cg_dynamicCrosshair.integer) { // 100% accurate
      ...
      if ((pVeh = G_IsRidingVehicle(cg_entities[0].gent)) != NULL &&
        ...
        VectorCopy(cg_entities[pVeh->m_pParentEntity->s.number].lerpOrigin,
                   start);
        AngleVectors(cg_entities[pVeh->m_pParentEntity->s.number].lerpAngles,
                     d_f, d_rt, d_up);
      } else if (cg.snap->ps.weapon == WP_NONE ||
                 cg.snap->ps.weapon == WP_SABER ||
                 cg.snap->ps.weapon == WP_STUN_BATON) {
        if (cg.snap->ps.viewEntity > 0 &&
            cg.snap->ps.viewEntity < ENTITYNUM_WORLD) { // in camera ent view
          ignoreEnt = cg.snap->ps.viewEntity;
          if (g_entities[cg.snap->ps.viewEntity].client) {
            VectorCopy(
                g_entities[cg.snap->ps.viewEntity].client->renderInfo.eyePoint,
                start);
          } else {
            VectorCopy(cg_entities[cg.snap->ps.viewEntity].lerpOrigin, start);
          }
          AngleVectors(cg_entities[cg.snap->ps.viewEntity].lerpAngles, d_f,
                       d_rt, d_up);
        } else {
          VectorCopy(g_entities[0].client->renderInfo.eyePoint, start);
          AngleVectors(cg_entities[0].lerpAngles, d_f, d_rt, d_up);
        ...
        extern void CalcMuzzlePoint(gentity_t *const ent, vec3_t forward,
    ...
    gi.trace(
        &trace, start, vec3_origin, vec3_origin, end, ignoreEnt,
        MASK_OPAQUE | CONTENTS_TERRAIN | CONTENTS_SHOTCLIP | CONTENTS_BODY |
            CONTENTS_ITEM,
        G2_NOCOLLIDE,
        10); // ); took out CONTENTS_SOLID| so you can target people through
             // glass.... took out CONTENTS_CORPSE so disintegrated guys aren't
    ...
    MASK_PLAYERSOLID|CONTENTS_CORPSE|CONTENTS_ITEM );
    */
    // FIXME: pick up corpses
    if (trace.startsolid || trace.allsolid) {
      // trace should not be allowed to pick up anything if it started solid.  I
      // tried actually moving the trace start back, which also worked,
      //	but the dynamic cursor drawing caused it to render around the
      //clip of the gun when I pushed the blaster all the way into a wall. 	It
      //looked quite horrible...but, if this is bad for some reason that I don't
      //know
  ...
  /*	if ( cg.crosshairClientNum && g_entities[cg.crosshairClientNum].health
     <= 0 )
          {
  ...
  */
  ...
  if (!traceEnt) {
    ...
    if (trace.entityNum != g_crosshairEntNum) { // new crosshair ent
      g_crosshairSameEntTime = 0;
    } else if (g_crosshairEntDist < 256) { // close enough to start counting how
                                           // long you've been looking
...
static void CG_DrawCrosshairNames(void) {
  ...
  // This is currently being called by the rocket tracking code, so we don't
  // necessarily want to do duplicate traces :)
  CG_ScanForCrosshairEntity(scanAll);
}
...
//--------------------------------------------------------------
{
  ...
  cgs.media.forcePowerIcons[forcePowerSorted[i]]); startx += (icon_size+2); //+2
  for spacing if ((startx+icon_size) >= SCREEN_WIDTH-80)
                  {
...
//--------------------------------------------------------------
{
  ...
  if (CG_WorldCoordToScreenCoord(org, &cx, &cy)) {
    ...
    // well now, take our current lock time and divide that by 8 wedge slices to
    // get the current lock amount
    int dif = (cg.time - g_rocketLockTime) / (1200.0f / 8.0f);
    ...
    if (oldDif != dif) {
      if (dif == 8) {
        cgi_S_StartSound(org, 0, CHAN_AUTO,
...
//------------------------------------
{
  ...
  // Only bother with this when the player is holding down the alt-fire button
...
static float CG_DrawSnapshot(float y) {
  ...
  cgi_R_Font_DrawString(635 - w, y + 2, s, colorTable[CT_LTGOLD1],
...
static float CG_DrawFPS(float y) {
  ...
  cgi_R_Font_DrawString(635 - xOffset - w, y + 2, s, colorTable[CT_LTGOLD1],
                        cgs.media.qhFontMedium, -1, 1.0f);
...
static void CG_DrawAmmoWarning(void) {
  ...
  if (weaponData[cg.snap->ps.weapon].ammoIndex ==
      AMMO_NONE) { // doesn't use ammo, so no warning
    return;
  }

  if (cg.lowAmmoWarning == 2) {
    cgi_SP_GetStringTextString("SP_INGAME_INSUFFICIENTENERGY", text,
                               sizeof(text));
  } else {
    return;
    // s = "LOW AMMO WARNING";
  ...
  cgi_R_Font_DrawString(320 - w / 2, 64, text, colorTable[CT_LTGOLD1],
...
static qboolean CG_RenderingFromMiscCamera() {
  ...
  if (cg.snap->ps.viewEntity > 0 &&
      cg.snap->ps.viewEntity <
          ENTITYNUM_WORLD) // cent && cent->gent && cent->gent->client &&
                           // cent->gent->client->ps.viewEntity)
  {
    ...
    if (!Q_stricmp("misc_camera",
      ...
      return qtrue;
    } else if (!Q_stricmp("misc_panel_turret",
                          g_entities[cg.snap->ps.viewEntity].classname)) {
      ...
      // FIXME: make sure that this assumption is correct...because I'm assuming
      // that I must be a droid.
...
static void CG_Draw2DScreenTints(void) {
  ...
  if ((cg.refdef.viewContents & CONTENTS_LAVA)) { // tint screen red
    ...
    CG_FillRect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, hcolor);
              ...
              CONTENTS_WATER)) { // tint screen light blue -- FIXME: check to
                                 // see if in fog?
    float phase = cg.time / 1000.0 * WAVE_FREQUENCY * M_PI * 2;
...
static void CG_Draw2D(void) {
  ...
  if (cg.snap->ps.forcePowersActive ||
      cg.snap->ps.forceRageRecoveryTime > cg.time) {
    CG_DrawActivePowers();
  }
  ...
  if (CG_RenderingFromMiscCamera()) {
    ...
    // allowing center print when in camera mode, probably just an alpha thing -
    // dmv
    CG_DrawCenterString();
    return;
  }

  if ((cg.snap->ps.forcePowersActive & (1 << FP_SEE))) { // force sight is on
  ...
  if (cg.snap->ps.stats[STAT_HEALTH] > 0) {
    ...
    // if ( !cg.renderingThirdPerson && !cg_dynamicCrosshair.integer ) //
    // disruptor draws it's own crosshair artwork; binocs draw nothing; third
    // person draws its own crosshair
    //{
  ...
  */
  if (missionInfo_Updated) {
    if (cg.predicted_player_state.pm_type != PM_DEAD) {
      ...
      /*			if ((cg_updatedDataPadForcePower.integer) ||
         (cg_updatedDataPadObjective.integer))
                              {
                                      // How long has the game been running? If
         within 15 seconds of starting, throw up the datapad. if
         (cg.dataPadLevelStartTime>cg.time)
                                      {
                                              // Make it pop up
                                              if (!in_camera)
                                              {
                                                      cgi_SendConsoleCommand(
         "datapad" ); cg.dataPadLevelStartTime=cg.time;	//and don't do it again
         this level!
                                              }
                                      }
                              }
      */
      ...
      cgi_SP_GetStringTextString("SP_INGAME_NEW_OBJECTIVE_INFO", text,
                                 sizeof(text));
      ...
      cgi_R_Font_DrawString(x_pos, y_pos, text, colorTable[CT_LTRED1],
                            cgs.media.qhFontMedium, -1, 1.0f);
    }
  }
  ...
  if (cg.weaponPickupTextTime > cg.time) {
    ...
    cgi_R_Font_DrawString(x_pos, y_pos, text, colorTable[CT_WHITE],
                          cgs.media.qhFontMedium, -1, 0.8f);
  }
}
...
Choose the proper background for the icons, scale it depending on if your
opening or closing the icon section of the HU
===================
*/
void CG_DrawIconBackground(void) {
  ...
  if (!cgi_UI_GetMenuInfo("iconbackground", &backgroundXPos, &backgroundYPos,
  ...
  if ((cg.iconSelectTime + WEAPON_SELECT_TIME) < cg.time) {
    ...
    if (background && cg.iconHUDActive) {
      cg.iconHUDPercent =
          (cg.time - (cg.iconSelectTime + WEAPON_SELECT_TIME)) / shutdownTime;
      ...
      CG_DrawPic(backgroundXPos, backgroundYPos, backgroundWidth,
                 -backgroundHeight, background); // Top half
      CG_DrawPic(backgroundXPos, backgroundYPos, backgroundWidth,
                 backgroundHeight, background); // Bottom half
    }
    return;
  }
...
void CG_DrawActive(stereoFrame_t stereoView) {
  ...
  if (cg.zoomMode == 3 &&
      cg.snap->ps.batteryCharge) // doing the Light amp goggles thing
  {
    cgi_R_LAGoggles();
  }
