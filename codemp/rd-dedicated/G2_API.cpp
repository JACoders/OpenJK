...
void G2API_AttachInstanceToEntNum(CGhoul2Info_v &ghoul2, int entityNum, qboolean server) { // Assign the pointers in the arrays
#ifdef _G2_LISTEN_SERVER_OPT
	if (server) {
...
qboolean G2API_OverrideServerWithClientData(CGhoul2Info_v &ghoul2, int modelIndex) {
	...
	if (!g2ClientAttachments[serverInstance->entityNum]) { // No clientside instance is attached to this entity
		return qfalse;
	}
	...
	if (clientInstance->mSkelFrameNum != frameNum) { // it has to be constructed already
		return qfalse;
	}
	...
	if (!serverInstance->mBoneCache) { // if this is the case.. I guess we can use the client one instead
...
qboolean G2_ShouldRegisterServer(void) {
	...
	if (currentVM && currentVM->slot == VM_GAME) {
		if (ri.Cvar_VariableIntegerValue("cl_running") && ri.Com_TheHunkMarkHasBeenMade() &&
			ShaderHashTableExists()) { // if the hunk has been marked then we are now loading client assets so don't load on server.
			return qfalse;
		}
...
int G2API_InitGhoul2Model(CGhoul2Info_v **ghoul2Ptr, const char *fileName, int modelIndex, qhandle_t customSkin, qhandle_t customShader, int modelFlags,
						  int lodBias) {
	int model;
	...
	if (model == ghoul2.size()) {  // init should not be used to create additional models, only the first one
...
qboolean G2API_SetSkin(CGhoul2Info_v &ghoul2, int modelIndex, qhandle_t customSkin, qhandle_t renderSkin) {
	...
	if (G2_SetupModelPointers(ghlInfo)) {
		...
		if (renderSkin) { // this is going to set the surfs on/off matching the skin file
...
qboolean G2API_RemoveGhoul2Models(CGhoul2Info_v **ghlRemove) { // remove 'em ALL!
	CGhoul2Info_v &ghlInfo = **ghlRemove;
	int modelIndex = 0;
	int newSize = 0;
	int i;
	...
	// sanity check
	if (!ghlInfo.size()) { // if we hit this then we are trying to delete a ghoul2 model on a ghoul2 instance that
		// one way or another is already gone.
		return qfalse;
	}
...
qboolean G2API_SetBoneAnimIndex(CGhoul2Info *ghlInfo, const int index, const int AstartFrame, const int AendFrame, const int flags, const float animSpeed,
								const int currentTime, const float AsetFrame, const int blendTime) {
...
qboolean G2API_SetBoneAnim(CGhoul2Info_v &ghoul2, const int modelIndex, const char *boneName, const int AstartFrame, const int AendFrame, const int flags,
						   const float animSpeed, const int currentTime, const float AsetFrame, const int blendTime) {
	int endFrame = AendFrame;
	int startFrame = AstartFrame;
	float setFrame = AsetFrame;
...
qboolean G2API_GetBoneAnim(CGhoul2Info_v &ghoul2, int modelIndex, const char *boneName, const int currentTime, float *currentFrame, int *startFrame,
	...
	if (G2_SetupModelPointers(ghlInfo)) {
		...
		qboolean ret = G2_Get_Bone_Anim(ghlInfo, ghlInfo->mBlist, boneName, aCurrentTime, currentFrame, startFrame, endFrame, flags, animSpeed, modelList,
										ghlInfo->mModelindex);
#ifdef _DEBUG
		/*
		assert(*endFrame>0);
...
qboolean G2API_SetBoneAnglesIndex(CGhoul2Info *ghlInfo, const int index, const vec3_t angles, const int flags, const Eorientations yaw,
								  const Eorientations pitch, const Eorientations roll, qhandle_t *modelList, int blendTime, int currentTime) {
	qboolean setPtrs = qfalse;
	qboolean res = qfalse;
...
qboolean G2API_SetBoneAngles(CGhoul2Info_v &ghoul2, const int modelIndex, const char *boneName, const vec3_t angles, const int flags, const Eorientations up,
							 const Eorientations left, const Eorientations forward, qhandle_t *modelList, int blendTime, int currentTime) {
	if (ghoul2.size() > modelIndex) {
		CGhoul2Info *ghlInfo = &ghoul2[modelIndex];
		qboolean setPtrs = qfalse;
		qboolean res = qfalse;
		...
		if (res) {
			// ensure we flush the cache
			ghlInfo->mSkelFrameNum = 0;
			return G2_Set_Bone_Angles(ghlInfo, ghlInfo->mBlist, boneName, angles, flags, up, left, forward, modelList, ghlInfo->mModelindex, blendTime,
									  currentTime);
		}
	}
	return qfalse;
...
qboolean G2API_SetBoneAnglesMatrixIndex(CGhoul2Info *ghlInfo, const int index, const mdxaBone_t &matrix, const int flags, qhandle_t *modelList, int blendTime,
										int currentTime) {
	if (G2_SetupModelPointers(ghlInfo)) {
		// ensure we flush the cache
		ghlInfo->mSkelFrameNum = 0;
...
qboolean G2API_SetBoneAnglesMatrix(CGhoul2Info *ghlInfo, const char *boneName, const mdxaBone_t &matrix, const int flags, qhandle_t *modelList, int blendTime,
								   int currentTime) {
	if (G2_SetupModelPointers(ghlInfo)) {
		// ensure we flush the cache
		ghlInfo->mSkelFrameNum = 0;
...
void G2API_SetRagDoll(CGhoul2Info_v &ghoul2, CRagDollParams *parms) { G2_SetRagDoll(ghoul2, parms); }
...
static inline boneInfo_t *G2_GetRagBoneConveniently(CGhoul2Info_v &ghoul2, const char *boneName) {
	...
	if (!(ghlInfo->mFlags & GHOUL2_RAG_STARTED)) { // can't do this if not in ragdoll
		return NULL;
	}
...
qboolean G2API_GetRagBonePos(CGhoul2Info_v &ghoul2, const char *boneName, vec3_t pos, vec3_t entAngles, vec3_t entPos, vec3_t entScale) { // do something?
	return qfalse;
}
...
qboolean G2API_IKMove(CGhoul2Info_v &ghoul2, int time, sharedIKMoveParams_t *params) { return G2_IKMove(ghoul2, time, params); }
...
qboolean G2API_GetBoltMatrix(CGhoul2Info_v &ghoul2, const int modelIndex, const int boltIndex, mdxaBone_t *matrix, const vec3_t angles, const vec3_t position,
	...
	if (G2_SetupModelPointers(ghoul2)) {
		if (matrix && modelIndex >= 0 && modelIndex < ghoul2.size()) {
			...
			if (boltIndex >= 0 && ghlInfo && (boltIndex < (int)ghlInfo->mBltlist.size())) {
				...
				if (!gG2_GBMUseSPMethod) { // this is horribly stupid and I hate it. But lots of game code is written to assume this 90 degree offset thing.
					float ftemp;
...
static inline bool G2_NeedRetransform(CGhoul2Info *g2, int frameNum) { // see if we need to do another transform
	size_t i = 0;
	...
	while (i < g2->mBlist.size()) {
		...
		if (newFrame < bone.endFrame || (bone.flags & BONE_ANIM_OVERRIDE_LOOP) ||
			(bone.flags & BONE_NEED_TRANSFORM)) { // ok, we're gonna have to do it. bone is apparently animating.
...
void G2API_CollisionDetectCache(CollisionRecord_t *collRecMap, CGhoul2Info_v &ghoul2, const vec3_t angles, const vec3_t position, int frameNumber, int entNum,
								vec3_t rayStart, vec3_t rayEnd, vec3_t scale, IHeapAllocator *G2VertSpace, int traceFlags, int useLod,
								float fRadius) { // this will store off the transformed verts for the next trace - this is slower, but for models that do not
	...
	if (G2_SetupModelPointers(ghoul2)) {
		...
		if (G2_NeedRetransform(&ghoul2[0], tframeNum) ||
			!ghoul2[0].mTransformedVertsArray) { // optimization, only create new transform space if we need to, otherwise
			// store it off!
			int i = 0;
			while (i < ghoul2.size()) {
				...
				*/
				if (!g2.mTransformedVertsArray || !(g2.mFlags & GHOUL2_ZONETRANSALLOC)) { // reworked so we only alloc once!
...
void G2API_CollisionDetect(CollisionRecord_t *collRecMap, CGhoul2Info_v &ghoul2, const vec3_t angles, const vec3_t position, int frameNumber, int entNum,
						   vec3_t rayStart, vec3_t rayEnd, vec3_t scale, IHeapAllocator *G2VertSpace, int traceFlags, int useLod, float fRadius) {
	/*
	if (1)
	{
...
void G2API_CopySpecificG2Model(CGhoul2Info_v &ghoul2From, int modelFrom, CGhoul2Info_v &ghoul2To, int modelTo) {
	...
	if (ghoul2From.size() > modelFrom) {
		...
		if (ghoul2To.IsValid() && ghoul2To.size() >= modelTo) { // remove the bonecache before we stomp over this instance.
...
void G2API_DuplicateGhoul2Instance(CGhoul2Info_v &g2From, CGhoul2Info_v **g2To) {
	...
	if (*g2To) { // This is bad.  We only want to do this if there is not yet a to declared.
		assert(0);
		return;
	}
...
qboolean G2API_SaveGhoul2Models(CGhoul2Info_v &ghoul2, char **buffer, int *size) { return G2_SaveGhoul2Models(ghoul2, buffer, size); }

void G2API_LoadGhoul2Models(CGhoul2Info_v &ghoul2, char *buffer) { G2_LoadGhoul2Model(ghoul2, buffer); }
...
qboolean G2_SetupModelPointers(CGhoul2Info *ghlInfo) // returns true if the model is properly set up
{
	...
	if (0) { // rww - Why are we bothering with all this? We can't change models like this anyway.
...
qboolean G2API_IsGhoul2InfovValid(CGhoul2Info_v &ghoul2) { return (qboolean)ghoul2.IsValid(); }

const char *G2API_GetModelName(CGhoul2Info_v &ghoul2, int modelIndex) { return ghoul2[modelIndex].mFileName; }
