...
void BG_G2SetBoneAngles(centity_t *cent, gentity_t *gent, int boneIndex, const vec3_t angles, const int flags, const Eorientations up, const Eorientations left,
...
qboolean PM_AdjustAngleForWallRun(gentity_t *ent, usercmd_t *ucmd, qboolean doMove) {
	if ((ent->client->ps.legsAnim == BOTH_WALL_RUN_RIGHT || ent->client->ps.legsAnim == BOTH_WALL_RUN_LEFT) &&
		...
		vec3_t rt, traceTo, mins = {ent->mins[0], ent->mins[1], 0}, maxs = {ent->maxs[0], ent->maxs[1], 24},
		...
		if (trace.fraction < 1.0f && trace.plane.normal[2] == 0.0f) { // still a vertical wall there
...
void PM_UpdateViewAngles(playerState_t *ps, usercmd_t *cmd, gentity_t *gent) {
	...
	if ((!cg.renderingThirdPerson || cg.zoomMode) && (cmd->buttons & BUTTON_USE) && cmd->rightmove != 0 && !cmd->forwardmove &&
