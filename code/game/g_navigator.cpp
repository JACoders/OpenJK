...
bool HFILEread(int &handle, void *data, int size) {
  return (gi.FS_Read(data, size, handle) != 0);
}
bool HFILEwrite(int &handle, const void *data, int size) {
  return (gi.FS_Write(data, size, handle) != 0);
}
...
extern qboolean G_CheckInSolidTeleport(const vec3_t &teleportPos,
                                       gentity_t *self);
...
public:
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  virtual bool InCircle(const CWayNode &A, const CWayNode &B,
                        const CWayNode &C) const {
...
class CWayEdge {
public:
  int mNodeA; // DO NOT REMOVE THIS: Handles are full ints because upper bits
              // are used
  int mNodeB; // DO NOT REMOVE THIS: Handles are full ints because upper bits
              // are used
  float mDistance; // DO NOT REMOVE THIS: It's a serious runtime optimization
                   // for A*

  unsigned short
      mOwnerNum; // Converted to short.  Largest entity number is 1024
  unsigned short
      mEntityNum; // Converted to short.  Largest entity number is 1024
  enum EWayEdgeFlags {
    WE_NONE = 0,
  ...
  inline bool BlockingBreakable() {
    return (mFlags.get_bit(WE_BLOCKING_BREAK));
  }
  ...
  inline int Size() const {
    return (mFlags.get_bit(WE_SIZE_MEDIUM) ? (WE_SIZE_MEDIUM)
                                           : (WE_SIZE_LARGE));
  }
  ...
  // GetPoint A
  ////////////////////////////////////////////////////////////////////////////////////
  const CVec3 &PointA() const { return GetNode(mNodeA).mPoint; }

  ////////////////////////////////////////////////////////////////////////////////////
  // GetPoint B
  ////////////////////////////////////////////////////////////////////////////////////
  const CVec3 &PointB() const { return GetNode(mNodeB).mPoint; }
};
...
// Defines
////////////////////////////////////////////////////////////////////////////////////////
typedef ragl::graph_vs<CWayNode, NAV::NUM_NODES, CWayEdge, NAV::NUM_EDGES,
                       NAV::NUM_EDGES_PER_NODE>
    TGraph;
typedef ragl::graph_region<CWayNode, NAV::NUM_NODES, CWayEdge, NAV::NUM_EDGES,
                           NAV::NUM_EDGES_PER_NODE, NAV::NUM_REGIONS,
                           NAV::NUM_REGIONS>
    TGraphRegion;
typedef TGraph::cells<NAV::NUM_NODES_PER_CELL, NAV::NUM_CELLS, NAV::NUM_CELLS>
    TGraphCells;
...
typedef ratl::vector_vs<NAV::TNodeHandle, NAV::MAX_NODES_PER_NAME>
    TNamedNodeList;
...
// Path Point
//
// This is actual vector and speed location (as well as node handle of that the
// agent has planned to go to.
////////////////////////////////////////////////////////////////////////////////////////
struct SPathPoint {
  CVec3 mPoint;
  float mSpeed;
...
// The Graph User
//
// Here we define our own user class, which can invalidate edges and generate
// unique costs for node traversal based on the nodes, and possibly the actor
// attempting to cross the nodes at any given time.
//
////////////////////////////////////////////////////////////////////////////////////////
...
public:
  ...
  virtual bool is_valid(CWayEdge &Edge, int EndPoint = 0) const {
    ...
    //-----------------------------------------------------------------
    if (mActor && Edge.mFlags.get_bit(CWayEdge::WE_FLYING) && mActor->NPC &&
        !(mActor->NPC->scriptFlags & SCF_NAV_CAN_FLY)) {
      return false;
    }

    // If The Actor Can't Fly, But This Is A Flying Edge, It's Invalid
    //-----------------------------------------------------------------
    if (mActor && Edge.mFlags.get_bit(CWayEdge::WE_JUMPING) && mActor->NPC &&
        !(mActor->NPC->scriptFlags & SCF_NAV_CAN_JUMP)) {
      return false;
    }
    ...
    if (Edge.mEntityNum != ENTITYNUM_NONE) {
      ...
      if (ent) {
        ...
        if (Edge.BlockingDoor()) {
          ...
          bool Closed = (StartOpen) ? (ent->moverState == MOVER_POS2)
          ...
          if (Closed) {
            ...
            if (owner) {
              // Check To See If The Owner Is Inactive Or Locked, Or Unavailable
              // To The NPC
              //----------------------------------------------------------------------------
              if ((owner->svFlags & SVF_INACTIVE) ||
                  (owner == ent && (owner->spawnflags &
                                    (MOVER_PLAYER_USE | MOVER_FORCE_ACTIVATE |
                                     MOVER_LOCKED))) ||
                  (owner != ent && (owner->spawnflags &
                                    (1 /*PLAYERONLY*/ | 4 /*USE_BOTTON*/)))) {
                return false;
              }
    ...
    } else if (Edge.BlockingBreakable()) { // we had a breakable in our way, now
                                           // it's gone, see if there is anything
                                           // else in the way
      if (NAV::TestEdge(Edge.mNodeA, Edge.mNodeB, qfalse)) { // clear it
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  // This is the cost estimate from any node to any other node (usually the
  // goal)
  ////////////////////////////////////////////////////////////////////////////////////
  ...
  virtual float cost(const CWayEdge &Edge, const CWayNode &B) const {
    ...
    if (mDangerSpotRadiusSq >
        mDangerSpot.DistToLine2(Edge.PointA(), Edge.PointB())) {
  ...
  //
  // This function is here because it evaluates the base cost from NodeA to
  // NodeB, which
  //
  ////////////////////////////////////////////////////////////////////////////////////
  virtual void setup_edge(CWayEdge &Edge, int A, int B, bool OnHull,
                          const CWayNode &NodeA, const CWayNode &NodeB,
                          bool CanBeInvalid = false) {
    Edge.mNodeA = A;
    Edge.mNodeB = B;
...
//
////////////////////////////////////////////////////////////////////////////////////////
const CWayNode &GetNode(int Handle) { return mGraph.get_node(Handle); }

////////////////////////////////////////////////////////////////////////////////////////
//
////////////////////////////////////////////////////////////////////////////////////////
const CWayEdge &GetEdge(int Handle) { return mGraph.get_edge(Handle); }

////////////////////////////////////////////////////////////////////////////////////////
//
////////////////////////////////////////////////////////////////////////////////////////
int GetAirRegion() { return mAirRegion; }
int GetIslandRegion() { return mIslandRegion; }
...
bool ViewTrace(const CVec3 &a, const CVec3 &b) {
  ...
  if ((mViewTrace.allsolid == qfalse) && (mViewTrace.startsolid == qfalse) &&
      (mViewTrace.fraction == 1.0f)) {
    return true;
  }
  return false;
}
...
////////////////////////////////////////////////////////////////////////////////////////
bool MoveTrace(const CVec3 &Start, const CVec3 &Stop, const CVec3 &Mins,
               const CVec3 &Maxs, int IgnoreEnt = 0,
               bool CheckForDoNotEnter = false,
               bool RetryIfStartInDoNotEnter = true, bool IgnoreAllEnts = false,
  ...
  // Did It Make It?
  //-----------------
  if ((mMoveTrace.allsolid == qfalse) && (mMoveTrace.startsolid == qfalse) &&
      (mMoveTrace.fraction == 1.0f)) {
    return true;
  }

  // If We Started In Solid, Try Removing The "Do Not Enter" Contents Type, And
  // Trace Again
  //----------------------------------------------------------------------------------------
  if (CheckForDoNotEnter && RetryIfStartInDoNotEnter &&
      ((mMoveTrace.allsolid == qtrue) || (mMoveTrace.startsolid == qtrue))) {
    contents &= ~CONTENTS_BOTCLIP;
...
////////////////////////////////////////////////////////////////////////////////////////
bool MoveTrace(gentity_t *actor, const CVec3 &goalPosition,
  ...
  return MoveTrace(actor->currentOrigin, goalPosition, Mins, Maxs,
                   actor->s.number, true, true,
...
// GoTo
//
// This Function serves as a master control for finding, updating, and following
// a path
////////////////////////////////////////////////////////////////////////////////////////
...
bool NAV::GoTo(gentity_t *actor, gentity_t *target, float MaxDangerLevel) {
  ...
  // If The Target Has No Nearest Nav Point, Go To His Last Valid Waypoint
  // Instead
  //-------------------------------------------------------------------------------
  if (targetNode == 0) {
...
////////////////////////////////////////////////////////////////////////////////////////
// This function exists as a wrapper so that the graph can write to the
// gi.Printf()
////////////////////////////////////////////////////////////////////////////////////////
void stupid_print(const char *data) { gi.Printf(data); }
...
bool NAV::TestEdge(TNodeHandle NodeA, TNodeHandle NodeB, qboolean IsDebugEdge) {
  ...
  float radius = (at.Size() == CWayEdge::WE_SIZE_LARGE) ? (SC_LARGE_RADIUS)
                                                        : (SC_MEDIUM_RADIUS);
  float height = (at.Size() == CWayEdge::WE_SIZE_LARGE) ? (SC_LARGE_HEIGHT)
                                                        : (SC_MEDIUM_HEIGHT);
  ...
  //-------------------------
  if (a.mFlags.get_bit(CWayNode::WN_FLOATING) ||
      b.mFlags.get_bit(CWayNode::WN_FLOATING)) {
  ...
  // Well, It' Can't Go, But Possibly If We Hit An Entity And Remove That
  // Entity, We Can Go?
  //-----------------------------------------------------------------------------------------
  if (!CanGo && !mMoveTrace.startsolid && EntHit != ENTITYNUM_WORLD &&
      EntHit != ENTITYNUM_NONE && (&g_entities[EntHit]) != 0) {
    ...
    if (at.BlockingDoor()) {
      ...
      while (slave) {
        VectorCopy((DoorIsStartOpen) ? (slave->pos1) : (slave->pos2),
                   slave->currentOrigin);
        gi.linkentity(slave);
      ...
      if (CanGo) {
        ...
        if (IsDebugEdge) {
          gi.Printf("Nav(%s)<->(%s): Unable Pass Through Door Even When Open, "
                    "Going A Size Down\n",
                    aName, bName);
        }
      }
      ...
      slave = master;
      while (slave) {
        VectorCopy((DoorIsStartOpen) ? (slave->pos2) : (slave->pos1),
                   slave->currentOrigin);
        gi.linkentity(slave);
        slave = slave->teamchain;
      }
    ...
    //-----------------------------------------------------
    else if (at.BlockingBreakable()) { // we'll do the trace again later if this
                                       // ent gets broken
      CanGo = true;
    }
    ...
    //----------------------------------------------------
      ...
      if (IsDebugEdge) {
        gi.Printf("Nav(%s)<->(%s): Unable Pass Through Even If Entity Was "
                  "Gone, Going A Size Down\n",
                  aName, bName);
      }
    }

    // If We Can Now Go, Ignoring The Entity, Remember That (But Don't Remember
    // Characters - They Won't Stay Anyway)
    //---------------------------------------------------------------------------------------------------------------
    if (CanGo && !HitCharacter) {
      ...
      if (at.BlockingDoor()) {
        ...
        // If There Is A Target Name, See If This Thing Is Controlled From A
        // Switch Or Something
        //---------------------------------------------------------------------------------------
        if (ent->targetname) {
          // Try Target
          ...
          if (owner && (!Q_stricmp("trigger_multiple", owner->classname) ||
      ...
      //-----------------------------------------------------
      else if (at.BlockingBreakable()) { // we'll do the trace again later if
                                         // this ent gets broken
...
bool NAV::LoadFromEntitiesAndSaveToFile(const char *filename, int checksum) {
  ...
  if (CHECK_START_OPEN) {
    for (int curEnt = 0; curEnt < MAX_GENTITIES; curEnt++) {
      ...
      //--------------------------------------
      if (ent->NPC ||
          (ent->NPC_type && Q_stricmp(ent->NPC_type, "atst") == 0)) {
        NPCs.set_bit(curEnt);
  ...
  */
  ...
  // PHASE II: SCAN THROUGH EXISTING NODES AND GENERATE EDGES TO OTHER NODES WAY
  // POINTS
  //====================================================================================
  CWayNode *at;
  int atHandle;
  ...
  //---------------------------------
  for (nodeIter = mGraph.nodes_begin(); nodeIter != mGraph.nodes_end();
       nodeIter++) {
    at = &(*nodeIter);
    atRoof = at->mPoint;
    atFloor = at->mPoint;
    ...
    if (at->mFlags.get_bit(CWayNode::WN_DROPTOFLOOR)) {
      ...
      at->mPoint[2] += 5.0f;
               ...
               (at->mType == PT_WAYNODE || at->mType == PT_GOALNODE)) {
  ...
  for (nodeIter = mGraph.nodes_begin(); nodeIter != mGraph.nodes_end();
    ...
    for (tgtNum = 0; tgtNum < NAV::NUM_TARGETS; tgtNum++) {
      ...
      // For Each One
      //--------------
      for (int tgtNameIndex = 0; tgtNameIndex < (*nameFinder).size();
           tgtNameIndex++) {
        ...
        if (atHandle != tgtHandle) {
          ...
          // Setup The Edge
          //----------------
          mUser.setup_edge(atToTgt, atHandle, tgtHandle, false,
                           mGraph.get_node(atHandle),
    ...
    // If It Is A Combat Or Goal Nav, Try To "Auto Connect" To A Few Nearby Way
    // Points
    //---------------------------------------------------------------------------------
    if (!at->mFlags.get_bit(CWayNode::WN_NOAUTOCONNECT) &&
      ...
      for (int cellNode = 0; cellNode < Cell.mNodes.size(); cellNode++) {
        ...
        //-------------------------------------------------------------
        if (tgtHandle == atHandle ||
            mGraph.get_edge_across(atHandle, tgtHandle)) {
          continue;
        }
        ...
        // Connecting To Another Combat Point Or Goal Node It Must Be Half The
        // Max Connect Distance
        //------------------------------------------------------------------------------------------
        if (node.mType == NAV::PT_COMBATNODE ||
            node.mType == NAV::PT_GOALNODE) {
          nonWPCount++;
        ...
        // If We Already Have Points, Ignore Anything Farther Than The Current
        // Farthest
        //------------------------------------------------------------------------------
        if (closestNbrs[highestCost].mHandle != 0 &&
            closestNbrs[highestCost].mCost < cost) {
          continue;
        }
      ...
      for (int i = 0; i < MIN_WAY_NEIGHBORS; i++) {
        if (closestNbrs[i].mHandle) {
          mUser.setup_edge(atToTgt, atHandle, closestNbrs[i].mHandle, false,
                           (*nodeIter), mGraph.get_node(closestNbrs[i].mHandle),
                           false);
  ...
  //==================================================================
  ratl::vector_vs<int, NAV::NUM_EDGES> *ToBeRemoved =
      new ratl::vector_vs<int, NAV::NUM_EDGES>;
  for (edgeIter = mGraph.edges_begin(); edgeIter != mGraph.edges_end();
       edgeIter++) {
    ...
    //--------------------------------------------------------------
    bool CanGo = false;
    bool IsDebugEdge = (g_nav1->string[0] && g_nav2->string[0] &&
                        (!Q_stricmp(*(a.mName), g_nav1->string) ||
                         !Q_stricmp(*(b.mName), g_nav1->string)) &&
                        (!Q_stricmp(*(a.mName), g_nav2->string) ||
                         !Q_stricmp(*(b.mName), g_nav2->string)));
    ...
    if (!CanGo) {
      ...
      if (IsDebugEdge) {
        ...
        if (mMoveTrace.entityNum != ENTITYNUM_WORLD) {
          ...
          gi.Printf(
              "Nav(%s)<->(%s):     on entity Type (%s), TargetName (%s)\n",
              aName, bName, ent->classname, ent->targetname);
        }
    ...
    if (!mGraph.node_has_neighbors(atHandle)) {
      ...
      if (at->mType == NAV::PT_COMBATNODE) {
        ...
        gi.Printf(S_COLOR_RED
                  "ERROR: Combat Point %s@%s Is Not Connected To Anything\n",
                  at->mName.c_str(), mLocStringA);
        delayedShutDown = level.time + 100;
#endif
      }
      if (at->mType == NAV::PT_GOALNODE) {
        // Try To Trace Down, If We Don't Hit Any Ground, Assume This Is An "Air
        // Point"
        //------------------------------------------------------------------------------
...
#ifndef FINAL_BUILD
          gi.Printf(S_COLOR_RED
                    "ERROR: Nav Goal %s@%s Is Not Connected To Anything\n",
                    at->mName.c_str(), mLocStringA);
          delayedShutDown = level.time + 100;
#endif
        }
...
void NAV::RegisterDangerSense(gentity_t *actor, int alertEventIndex) {
  ...
  //----------------------------------------------------------------
  TGraphCells::TCellNodes &cellEdges =
      mCells.get_cell(DangerPoint[0], DangerPoint[1]).mEdges;
  for (int cellEdgeIndex = 0; cellEdgeIndex < cellEdges.size();
       cellEdgeIndex++) {
    int edgeHandle = cellEdges[cellEdgeIndex];
    CWayEdge &edge = mGraph.get_edge(edgeHandle);
    float edgeDanger =
        ((ae.radius - DangerPoint.DistToLine(edge.PointA(), edge.PointB())) /
         (ae.radius));
    if (edgeDanger > 0.0f) {

      // Record The Square, So That Danger Drops Off Quadradically Rather Than
      // Linearly
      //--------------------------------------------------------------------------------
      ...
      for (int alIndex = 0; alIndex < TAlertList::CAPACITY; alIndex++) {
        ...
        //---------------------------------------------
        if (replaceIndex == -1 ||
            al[alIndex].mDanger < al[replaceIndex].mDanger) {
          replaceIndex = alIndex;
        }
      }
...
void NAV::SpawnedPoint(gentity_t *ent, NAV::EPointType type) {
  ...
  const char *pointName =
      (ent->targetname && ent->targetname[0]) ? (ent->targetname) : "?";
  ...
  // TO AVOID PROBLEMS WITH THE TRIANGULATION, WE MOVE THE POINTS AROUND JUST A
  // BIT
  //================================================================================
  // node.mPoint		+= CVec3(Q_flrand(-RANDOM_PERMUTE, RANDOM_PERMUTE),
  // Q_flrand(-RANDOM_PERMUTE, RANDOM_PERMUTE), 0.0f);
  //================================================================================
...
//
////////////////////////////////////////////////////////////////////////////////////////
NAV::TNodeHandle NAV::GetNearestNode(gentity_t *ent, bool forceRecalcNow,
                                     NAV::TNodeHandle goal) {
  if (!ent) {
    return 0;
  }

  if (ent->waypoint == WAYPOINT_NONE || forceRecalcNow ||
      (level.time > ent->noWaypointTime)) {
    if (ent->waypoint) {
      ent->lastWaypoint = ent->waypoint;
    }
    ent->waypoint =
        GetNearestNode(ent->currentOrigin, ent->waypoint, goal, ent->s.number,
                       (ent->client && ent->client->moveType == MT_FLYSWIM));
    ent->noWaypointTime =
        level.time + 1000; // Don't Erase This Result For 5 Seconds
  }
...
//
////////////////////////////////////////////////////////////////////////////////////////
NAV::TNodeHandle NAV::GetNearestNode(const vec3_t &position,
                                     NAV::TNodeHandle previous,
                                     NAV::TNodeHandle goal, int ignoreEnt,
                                     bool allowZOffset) {
  if (mGraph.size_edges() > 0) {
    ...
    if (Cell.mNodes.empty() && Cell.mEdges.empty()) {
      ...
      if (g_developer->value) {
        gi.Printf(
            "WARNING: Failure To Find A Node Here, Examine Cell Layout\n");
      }
#endif
    ...
    //===========================
    {
      ...
      for (int i = 0; i < Cell.mNodes.size() && !mNearestNavSort.full(); i++) {
        ...
        NodeSort.mInRadius =
            (NodeSort.mDistance < (node.mRadius * node.mRadius));
        ...
        if (previous && previous != NodeSort.mHandle &&
            !NAV::InSameRegion(previous, NodeSort.mHandle)) {
          NodeSort.mDistance += (100.0f * 100.0f);
        }
        if (previous > 0 && previous != NodeSort.mHandle &&
            !mGraph.get_edge_across(previous, NodeSort.mHandle)) {
          NodeSort.mDistance += (200.0f * 200.0f);
        }
        if (goal && goal != NodeSort.mHandle &&
            !NAV::InSameRegion(goal, NodeSort.mHandle)) {
      ...
      // Now , Run Through Each Of The Sorted Nodes, Starting With The Closest
      // One
      //---------------------------------------------------------------------------
      for (int j = 0; j < mNearestNavSort.size(); j++) {
        ...
        //-----------------------------------
        else if (ViewNavTrace(
                     Pos, mGraph.get_node(mNearestNavSort[j].mHandle).mPoint)) {
          return mNearestNavSort[j].mHandle;
        }
      }
    }
    ...
    //==========================
    {
      ...
      // Now , Run Through Each Of The Sorted Edges, Starting With The Closest
      // One
      //---------------------------------------------------------------------------
      for (int j = 0; j < mNearestNavSort.size(); j++) {
        ...
        PointOnEdgeRange =
            PointOnEdge.ProjectToLine(edge.PointA(), edge.PointB());
...
//
////////////////////////////////////////////////////////////////////////////////////////
NAV::TNodeHandle NAV::ChooseClosestNeighbor(NAV::TNodeHandle NodeHandle,
                                            const vec3_t &position) {
  if (NodeHandle != WAYPOINT_NONE && NodeHandle > 0) {
    CVec3 pos(position);
    TGraph::TNodeNeighbors &neighbors = mGraph.get_node_neighbors(NodeHandle);
...
//
////////////////////////////////////////////////////////////////////////////////////////
NAV::TNodeHandle NAV::ChooseFarthestNeighbor(NAV::TNodeHandle NodeHandle,
                                             const vec3_t &position) {
  if (NodeHandle != WAYPOINT_NONE && NodeHandle > 0) {
    CVec3 pos(position);
    TGraph::TNodeNeighbors &neighbors = mGraph.get_node_neighbors(NodeHandle);
...
//
////////////////////////////////////////////////////////////////////////////////////////
NAV::TNodeHandle NAV::ChooseFarthestNeighbor(gentity_t *actor,
                                             const vec3_t &target,
                                             float maxSafeDot) {
  ...
  //------------------------------------------------------------------------
  if (curDist > actorToTgtDist &&
      actorPos.Dist(curPos) > 300.0f) // curDot<maxSafeDot
  {
    best = cur;
    bestDist = curDist;
  }
...
//
////////////////////////////////////////////////////////////////////////////////////////
bool NAV::FindPath(gentity_t *actor, NAV::TNodeHandle target,
  ...
  //------------------------
  if (start < 0) {
    start = (Q_irand(0, 1) == 0) ? (mGraph.get_edge(abs(start)).mNodeA)
                                 : (mGraph.get_edge(abs(start)).mNodeB);
  }
  if (target < 0) {
    target = (Q_irand(0, 1) == 0) ? (mGraph.get_edge(abs(target)).mNodeA)
                                  : (mGraph.get_edge(abs(target)).mNodeB);
  }
  ...
  //------------------------
  if (mRegion.size() > 0 &&
      !mRegion.has_valid_edge(mSearch.mStart, mSearch.mEnd, mUser)) {
  ...
  if (actor->enemy && actor->enemy->client) {
    ...
    } else if (actor->enemy->client->NPC_class == CLASS_RANCOR ||
  ...
  AtSpeed *= 0.001f; // Convert units/sec to units/millisec for comparison
                     // against level.time
  AtSpeed *=
      0.25; // Cut the speed in half to account for accel & decel & some slop
  ...
  //---------------------------------------
  {
    SPathPoint PPoint = {};
    puser.mPath.clear();
    for (mSearch.path_begin(); !mSearch.path_end() && !puser.mPath.full();
         mSearch.path_inc()) {
      if (puser.mPath.full()) {
        // NAV_TODO: If the path is longer than the max length, we want to store
        // the first valid ones, instead of returning false
        assert("This Is A Test To See If We Hit This Condition Anymore...  It "
               "Should Be Handled Properly" == 0);
      ...
      PPoint.mReachedRadius =
          Max(radius * 3.0f, (mGraph.get_node(PPoint.mNode).mRadius * 0.40f));
      if (mGraph.get_node(PPoint.mNode).mFlags.get_bit(CWayNode::WN_FLOATING)) {
        PPoint.mReachedRadius = 20.0f;
      }
      PPoint.mReachedRadius *=
          PPoint.mReachedRadius; // squared, for faster checks later
      PPoint.mDist = 0.0f;
      PPoint.mETA = 0.0f;
    ...
    //-----------------------------------------------------
    puser.mPath[0].mSlowingRadius =
        Max(70.0f, (mGraph.get_node(PPoint.mNode).mRadius * 0.75f));
    puser.mPath[0].mReachedRadius =
        Max(radius, (mGraph.get_node(PPoint.mNode).mRadius * 0.15f));
    puser.mPath[0].mReachedRadius *=
        puser.mPath[0].mReachedRadius; // squared, for faster checks later
  }
  ...
  for (int edge = 0; edge < numEdges; edge++) {
    ...
    if (AtOnEdgeScale > 0.1f && AtOnEdgeScale < 0.9f) {
      if (AtDistToEdge < (radius) ||
          (AtDistToEdge < (radius * 20.0f) &&
           MoveTrace(At, AtOnEdge, actor->mins, actor->maxs, actor->s.number,
                     true, true, false))) {
        puser.mPath.resize(edge +
                           2); // +2 because every edge needs at least 2 points
  ...
  for (int i = puser.mPath.size() - 1; i > -1; i--) {
    SPathPoint &PPoint = puser.mPath[i]; // For Debugging And A Tad Speed
                                         // Improvement, Get A Ref Directly
    ...
    PPoint.mETA =
        (PPoint.mDist / PPoint.mSpeed); // Estimate Our Eta By Distance/Speed
    PPoint.mETA += AtTime;
    ...
    if (i != 0 && // is there a next point?
      ...
      if ((NextToBeyondDistance > 150.0f && PPoint.mDist > 150.0f &&
           NextToBeyondDot < 0.64f) ||
          (NextToBeyondDistance > 30.0f && NextToBeyondDot < 0.5f)) {
        PPoint.mSlowingRadius = Max(
            40.0f, mGraph.get_node(PPoint.mNode).mRadius); // Force A Stop Here
      }
    }
...
//
////////////////////////////////////////////////////////////////////////////////////////
bool NAV::SafePathExists(const CVec3 &startVec, const CVec3 &stopVec,
                         const CVec3 &danger, float dangerDistSq) {
  mUser.ClearActor();
...
float NAV::EstimateCostToGoal(TNodeHandle Start, TNodeHandle Goal) {
  mUser.ClearActor();
  if (Goal != 0 && Goal != WAYPOINT_NONE && Start != 0 &&
      Start != WAYPOINT_NONE) {
...
bool NAV::OnNeighboringPoints(gentity_t *actor, gentity_t *target) {
  if (OnNeighboringPoints(GetNearestNode(actor), GetNearestNode(target))) {
    if (Distance(actor->currentOrigin, target->currentOrigin) <
        NEIGHBORING_DIST) {
      //			if (ViewNavTrace(actor->currentOrigin,
...
////////////////////////////////////////////////////////////////////////////////////////
// Is The Position (at) within the safe radius of the atNode, targetNode, or the
// edge between?
////////////////////////////////////////////////////////////////////////////////////////
bool NAV::InSafeRadius(CVec3 at, TNodeHandle atNode, TNodeHandle targetNode) {
  ...
  //---------------------------------------
  if (Distance(at.v, GetNodePosition(atNode)) <
      mGraph.get_node(atNode).mRadius) {
    return true;
  }

  // If We Have A Target Node, We May Be On The Edge That Connects Between The
  // Two
  //-------------------------------------------------------------------------------
  if (targetNode > 0 && atNode != targetNode) {
    // If In The Radius Of Target Nav Point
    //---------------------------------------
    if (Distance(at.v, GetNodePosition(targetNode)) <
        mGraph.get_node(targetNode).mRadius) {
      return true;
    }
    ...
    if (atToTargetEdgeIndex != 0) {
      ...
      if (!atToTargetEdge.mFlags.get_bit(CWayEdge::WE_FLYING) &&
        ...
        float atDistToEdge =
            at.DistToLine(atToTargetEdge.PointA(), atToTargetEdge.PointB());
...
//
////////////////////////////////////////////////////////////////////////////////////////
bool NAV::FindPath(gentity_t *actor, const vec3_t &position,
                   float MaxDangerLevel) {
...
//
////////////////////////////////////////////////////////////////////////////////////////
bool NAV::NextPosition(gentity_t *actor, CVec3 &Position, float &SlowingRadius,
                       bool &Fly, bool &Jump) {
  assert(HasPath(actor));
  TPath &path = mPathUsers[mPathUserIndex[actor->s.number]].mPath;
  ...
  int edgeIndex = (curNodeIndex > 0)
                      ? (mGraph.get_edge_across(curNodeIndex, next.mNode))
                      : (abs(curNodeIndex));
...
////////////////////////////////////////////////////////////////////////////////////////
// This function completely removes any pathfinding information related to this
// agent and frees up this path user structure for someone else to use.
////////////////////////////////////////////////////////////////////////////////////////
...
// Update Path
//
// Removes points that have been reached, and frees the user if no points
// remain.
//
////////////////////////////////////////////////////////////////////////////////////////
bool NAV::UpdatePath(gentity_t *actor, TNodeHandle target,
                     float MaxDangerLevel) {
  int pathUserNum = mPathUserIndex[actor->s.number];
  if (pathUserNum == NULL_PATH_USER_INDEX) {
    return false;
  ...
  //----------------------------------------------------------------
  if (path[path.size() - 1].mETA < level.time ||
      (MaxDangerLevel != 1.0f && PathDangerLevel(NPC) > MaxDangerLevel)) {
    // Hmmm.  Should Have Reached This Point By Now, Or Too Dangerous.  Try To
    // Recompute The Path
    //--------------------------------------------------------------------------------------------
    NAV::TNodeHandle target =
        mPathUsers[pathUserNum].mEnd; // Remember The End As Our Target
    if (target == WAYPOINT_NONE) {
      ClearPath(actor);
      return false;
    }
  ...
  for (; curPathAt >= 0; curPathAt--) {
    ...
    if (prevNode < 0 || mGraph.get_edge_across(prevNode, PPoint.mNode)) {
      ...
      for (alIndex = 0; alIndex < TAlertList::CAPACITY; alIndex++) {
        if (al[alIndex].mHandle == curEdge &&
            al[alIndex].mDanger > DangerLevel) {
          DangerLevel = al[alIndex].mDanger;
        }
      }
    }

    // Check For Enemy Position Proximity To This Next Edge (using actual Edge
    // Locations)
    //------------------------------------------------------------------------------------
    if (enemySafeDist != 0.0f) {
      enemyDangerLevel =
          enemyPos.DistToLine(prevPoint, PPoint.mPoint) / enemySafeDist;
...
void NAV::ShowDebugInfo(const vec3_t &PlayerPosition, int PlayerWaypoint) {
  ...
  // Show Nodes
  //------------
  if (NAVDEBUG_showNodes || NAVDEBUG_showCombatPoints ||
      NAVDEBUG_showNavGoals) {
    for (TGraph::TNodes::iterator atIter = mGraph.nodes_begin();
         atIter != mGraph.nodes_end(); atIter++) {
      CWayNode &at = (*atIter);
      atEnd = at.mPoint;
      atEnd[2] += 30.0f;
      ...
      if (gi.inPVS(PlayerPosition, at.mPoint.v)) {
        if (at.mType == NAV::PT_WAYNODE && NAVDEBUG_showNodes) {
          ...
          if (NAVDEBUG_showRadius &&
              at.mPoint.Dist2(PlayerPosition) < (at.mRadius * at.mRadius)) {
            if (at.mFlags.get_bit(CWayNode::WN_FLOATING)) {
        ...
        } else if (at.mType == NAV::PT_COMBATNODE &&
  ...
  if (NAVDEBUG_showEdges) {
    for (TGraph::TEdges::iterator atIter = mGraph.edges_begin();
         atIter != mGraph.edges_end(); atIter++) {
      CWayEdge &at = (*atIter);
      CWayNode &a = mGraph.get_node(at.mNodeA);
      CWayNode &b = mGraph.get_node(at.mNodeB);
      ...
      if (AvePos.Dist2(PlayerPosition) < (500.0f * 500.0f) &&
          gi.inPVS(PlayerPosition, AvePos.v)) {
  ...
  if (NAVDEBUG_showNearest && player &&
      (player->waypoint != 0 || player->lastWaypoint != 0)) {
    PlayerWaypoint =
        (player->waypoint) ? (player->waypoint) : (player->lastWaypoint);
    CVec3 PPos(PlayerPosition);
    if (PlayerWaypoint > 0) {
      CWayNode &node = mGraph.get_node(PlayerWaypoint);

      CG_DrawEdge(PPos.v, node.mPoint.v,
                  (player->waypoint) ? (EDGE_NEARESTVALID)
                                     : (EDGE_NEARESTINVALID));
    } else {
      ...
      CG_DrawEdge(PPos.v, PosOnLine.v,
                  (player->waypoint) ? (EDGE_NEARESTVALID)
                                     : (EDGE_NEARESTINVALID));
    }
  }
...
void NAV::ShowStats() {
  ...
  mGraph.ProfilePrint("Path   : (%d)",
                      (sizeof(mPathUsers) + sizeof(mPathUserIndex)));
  mGraph.ProfilePrint("Steer  : (%d)",
                      (sizeof(mSteerUsers) + sizeof(mSteerUserIndex)));
  mGraph.ProfilePrint("Alerts : (%d)", (sizeof(mEntityAlertList)));
  float totalBytes =
      (sizeof(mCells) + sizeof(mGraph) + sizeof(mRegion) + sizeof(mPathUsers) +
       sizeof(mPathUserIndex) + sizeof(mSteerUsers) + sizeof(mSteerUserIndex) +
  ...
  mGraph.ProfilePrint("Connect Stats: Milliseconds(%d) Traces(%d)",
                      mConnectTime, mConnectTraceCount);
  mGraph.ProfilePrint("");
  mGraph.ProfilePrint("Move Trace: Count(%d) PerFrame(%f)", mMoveTraceCount,
                      (float)(mMoveTraceCount) / (float)(level.time));
  mGraph.ProfilePrint("View Trace: Count(%d) PerFrame(%f)", mViewTraceCount,
                      (float)(mViewTraceCount) / (float)(level.time));
...
void NAV::TeleportTo(gentity_t *actor, const char *pointName) {
  ...
  if (nameFinder != mNodeNames.end()) {
    if ((*nameFinder).size() > 1) {
      gi.Printf(
          "WARNING: More than one point named (%s).  Going to first one./n",
          pointName);
    }
    TeleportPlayer(actor, mGraph.get_node((*nameFinder)[0]).mPoint.v,
                   actor->currentAngles);
    return;
  }
...
void NAV::TeleportTo(gentity_t *actor, int pointNum) {
  assert(actor != 0);
  TeleportPlayer(actor, mGraph.get_node(pointNum).mPoint.v,
                 actor->currentAngles);
  return;
}
...
void STEER::Activate(gentity_t *actor) {
  assert(!Active(actor) && actor && actor->client &&
         actor->NPC); // Can't Activate If Already Active
  ...
  CVec3 Range(RangeSize, RangeSize,
              (actor->client->moveType == MT_FLYSWIM) ? (RangeSize)
                                                      : (suser.mRadius * 2.0f));
  ...
  for (int i = 0; i < numFound; i++) {
    ...
    if (neighbor->s.number == actor->s.number || neighbor == actor->enemy ||
        !neighbor->client || neighbor->health <= 0 || !neighbor->inuse) {
      continue;
    }
  ...
  // Clear Out Steering, So If No STEER Operations Are Called, Net Effect Is
  // Zero
  //------------------------------------------------------------------------------
  ...
  // PHASE III - Project The Current Velocity Forward, To The Side, And Onto The
  // Path
  //==================================================================================
...
void STEER::DeActivate(gentity_t *actor, usercmd_t *ucmd) {
  assert(Active(actor) && actor && actor->client &&
         actor->NPC); // Can't Deactivate If Never Activated
  ...
  if (!NPC_Jumping()) {
    ...
    VectorCopy(MoveDir.v, actor->client->ps.moveDir);
    actor->client->ps.speed = suser.mSpeed;

    VectorCopy(Angles.v, actor->NPC->lastPathAngles);
    ...
    if (suser.mSpeed > 0.0f) {
      ...
      AngleVectors(actor->currentAngles, forward, right,
                   up); // Use Current Angles

      float fDot =
          Com_Clamp(-127.0f, 127.0f, DotProduct(forward, MoveDir.v) * 127.0f);
      float rDot =
          Com_Clamp(-127.0f, 127.0f, DotProduct(right, MoveDir.v) * 127.0f);
    ...
    //========================================
    if (suser.mBlocked) {
      // If Not Previously Blocked, Record The Start Time And Any Entites
      // Involved
      //---------------------------------------------------------------------------
      ...
      // If Navigation Or Steering Had A Target Entity (Trying To Go To), Record
      // That Here
      //-----------------------------------------------------------------------------------
      ...
      VectorCopy(suser.mBlockedTgtPosition.v,
                 actor->NPC->blockedTargetPosition);
    } else {
    ...
    // If We've Been Blocked For More Than 2 Seconds, May Want To Take
    // Corrective Action
    //-----------------------------------------------------------------------------------
    if (STEER::HasBeenBlockedFor(actor, 2000)) {
      if (NAVDEBUG_showEnemyPath) {
        CG_DrawEdge(actor->currentOrigin, actor->NPC->blockedTargetPosition,
                    EDGE_PATHBLOCKED);
        if (actor->waypoint) {
      ...
      if ((actor->NPC->scriptFlags & SCF_NAV_CAN_FLY) ||
        ...
        // Ok, Well, How About Jumping To The Last Waypoint We Had That Was
        // Valid
        //------------------------------------------------------------------------
        if ((STEER::HasBeenBlockedFor(actor, 8000)) &&
            (!actor->waypoint || Distance(NAV::GetNodePosition(actor->waypoint),
                                          actor->currentOrigin) > 150.0f) &&
            (actor->lastWaypoint)) {
          if (player && (STEER::HasBeenBlockedFor(actor, 15000)) &&
              !gi.inPVS(player->currentOrigin,
                        NAV::GetNodePosition(actor->lastWaypoint)) &&
              !gi.inPVS(player->currentOrigin, actor->currentOrigin) &&
              !G_CheckInSolidTeleport(NAV::GetNodePosition(actor->lastWaypoint),
                                      actor)) {
...
////////////////////////////////////////////////////////////////////////////////////
// SafeToGoTo - returns true if it is safe for the actor to steer toward the
// target position
////////////////////////////////////////////////////////////////////////////////////
bool STEER::SafeToGoTo(gentity_t *actor, const vec3_t &targetPosition,
                       int targetNode) {
  ...
  //----------------------------------------
  if (actorToTargetDistance < 110.0f &&
      fabsf(targetPosition[2] - actor->currentOrigin[2]) < 50.0f) {
    return true;
  }
...
// Master Functions
////////////////////////////////////////////////////////////////////////////////////
bool STEER::GoTo(gentity_t *actor, gentity_t *target, float reachedRadius,
  ...
  //------------------------------------------------
  if (STEER::Reached(
          actor, target->currentOrigin, reachedRadius,
          (actor->client && actor->client->moveType == MT_FLYSWIM))) {
    NAV::ClearPath(actor);
    STEER::Stop(actor);
    return true;
  }
  ...
  if (
      ...
      //||
      !STEER::SafeToGoTo(actor, target->currentOrigin,
                         NAV::GetNearestNode(target))) {
    return false;
  }
...
////////////////////////////////////////////////////////////////////////////////////
bool STEER::GoTo(gentity_t *actor, const vec3_t &position, float reachedRadius,
                 bool avoidCollisions) {
  // If Within Reached Radius, Just Stop Right Here
  //------------------------------------------------
  if (STEER::Reached(
          actor, position, reachedRadius,
          (actor->client && actor->client->moveType == MT_FLYSWIM))) {
    NAV::ClearPath(actor);
    STEER::Stop(actor);
    return true;
...
float STEER::Stop(gentity_t *actor, float weight) {
  ...
  if (actor->NPC->aiFlags & NPCAI_FLY) {
    ...
    if (nearestNode > 0 &&
        !mGraph.get_node(nearestNode).mFlags.get_bit(CWayNode::WN_FLOATING)) {
...
//
////////////////////////////////////////////////////////////////////////////////////
float STEER::Seek(gentity_t *actor, const CVec3 &pos, float slowingDistance,
                  float weight, float desiredSpeed) {
  assert(Active(actor));
  SSteerUser &suser = mSteerUsers[mSteerUserIndex[actor->s.number]];
  ...
  if (suser.mDistance > 0.0f) {
    suser.mDesiredSpeed =
        (desiredSpeed != 0.0f) ? (desiredSpeed) : (suser.mMaxSpeed);
...
// position vectors instead of subtracting them to obtain a desired velocity
...
//
////////////////////////////////////////////////////////////////////////////////////
float STEER::Persue(gentity_t *actor, gentity_t *target,
...
float STEER::Persue(gentity_t *actor, gentity_t *target, float slowingDistance,
                    ...
                    bool relativeToTargetFacing) {
  assert(Active(actor) && target);
  SSteerUser &suser = mSteerUsers[mSteerUserIndex[actor->s.number]];

  CVec3 ProjectedTargetPosition(target->currentOrigin);

  // If Target Is A Client, Take His Velocity Into Account And Project His
  // Position
  //--------------------------------------------------------------------------------
  if (target->client) {
    float DistToTarget = ProjectedTargetPosition.Dist(suser.mPosition);
  ...
  if (relativeToTargetFacing) {
    AngleVectors(target->currentAngles, ProjectForward.v, ProjectRight.v,
                 ProjectUp.v);
    if (ProjectRight.Dot(DirectionToTarget) > 0.0f) {
      ProjectRight *= -1.0f; // If Going In Same Direction As Target Right,
                             // Project Toward Target Left
    }
  } else {
      ...
      if (suser.mNeighbors[i]->s.number > actor->s.number) {
        ...
        if (NbrToActDist > 1.0f) {
          ...
          if (NAVDEBUG_showCollision) {
            ...
            CG_DrawEdge(suser.mPosition.v, Prj.v,
                        EDGE_IMPACT_POSSIBLE); // Separation
          }
        }
      }
    }
...
// Alignment
////////////////////////////////////////////////////////////////////////////////////
float STEER::Alignment(gentity_t *actor, float Scale) { return 0.0f; }
...
gentity_t *STEER::SelectLeader(gentity_t *actor) {
  ...
  for (int i = 0; i < suser.mNeighbors.size(); i++) {
    if (suser.mNeighbors[i]->s.number > actor->s.number &&
        !Q_stricmp(suser.mNeighbors[i]->NPC_type, actor->NPC_type)) {
...
float STEER::FollowLeader(gentity_t *actor, gentity_t *leader, float dist) {
  ...
  if (TimeRemaining < 0 || (LeaderSpeed > 0.0f && TimeRemaining > 1000)) {
    ...
    if (mMoveTrace.fraction > 0.1) {
      ...
      leader->followPosWaypoint = NAV::GetNearestNode(
          leader->followPos, leader->waypoint, 0, leader->s.number);
    }
    ...
    leader->followPosRecalcTime =
        (level.time) + (Q_irand(50, 500)) + (SpeedScale * Q_irand(3000, 8000)) +
...
//
//////////////////////////////////////////////////////////////////////
bool TestCollision(gentity_t *actor, SSteerUser &suser,
                   const CVec3 &ProjectVelocity, float ProjectSpeed, ESide Side,
                   float weight = 1.0f) {
  // Test To See If The Projected Position Is Safe
  //-----------------------------------------------
  bool Safe = (Side == Side_None) ? (MoveTrace(actor, suser.mProjectFwd))
                                  : (MoveTrace(actor, suser.mProjectSide));
  if (mMoveTrace.entityNum != ENTITYNUM_NONE &&
      mMoveTrace.entityNum != ENTITYNUM_WORLD) {
    ...
    if (g_entities[mMoveTrace.entityNum].classname &&
        Q_stricmp(g_entities[mMoveTrace.entityNum].classname, "func_door") ==
            0) {
      Safe = true;
    }
  ...
  if (!Safe && Side == Side_None) {
    // Did We Hit A Client?
    //----------------------
    if (ContactNum != ENTITYNUM_WORLD && ContactNum != ENTITYNUM_NONE &&
        g_entities[ContactNum].client != 0) {
      gentity_t *Contact = &g_entities[ContactNum];
      // bool			ContactIsPlayer =
      // (Contact->client->ps.clientNum==0);
      ...
      // If He Is Moving, We Might Be Able To Just Slow Down Some And Stay
      // Behind Him
      //------------------------------------------------------------------------------
      if (ContactSpeed > 0.01f) {
        if (ContactSpeed < ProjectSpeed) {
          ...
          if (DirectionSimilarity > 0.5) {
            ...
            suser.mSteering += ((suser.mDesiredVelocity - ProjectVelocity) *
                                DirectionSimilarity);
            suser.mIgnoreEntity =
                ContactNum; // So The Side Trace Does Not Care About This Guy
#ifdef _DEBUG
            assert(suser.mSteering.IsFinite());
#endif
      ...
      //---------------------------------
        ...
        if (Next > AbsMin && Next < AbsMax) {
          ...
          Safe = true; // We say it is "Safe" because We Don't Want To Try And
                       // Steer Around
        }
      }
    }
    ...
    //-----------------------
    if (!Safe && ContactNormal[2] > 0.0f &&
        ContactNormal[2] < MIN_WALK_NORMAL) {
      Safe = true;
    }
    ...
    if (!Safe) {
      ...
      // Only Force A Particular Steering Side For A Max Of 2 Seconds, Then
      // Retest Dot Product
      //---------------------------------------------------------------------------------------
      if (actor->NPC->lastAvoidSteerSide != Side_None &&
          actor->NPC->lastAvoidSteerSideDebouncer < level.time) {
        actor->NPC->lastAvoidSteerSide = Side_None;
        actor->NPC->lastAvoidSteerSideDebouncer =
            level.time + Q_irand(500, STEER::SIDE_LOCKED_TIMER);
      }

      // Make Sure The Normal Is Going The Same Way As The Velocity
      //-----------------------------------------------------------
      if (((ESide)(actor->NPC->lastAvoidSteerSide) == Side_Right) ||
          ((ESide)(actor->NPC->lastAvoidSteerSide) == Side_None &&
           ContactNormal.Dot(ProjectDirection) < 0.0f)) {
        ContactNormal *= -1.0f;
  ...
  if (NAVDEBUG_showCollision) {
    ...
    if (Safe) {
      ...
      CG_DrawEdge(suser.mPosition.v, mMoveTrace.endpos,
                  EDGE_IMPACT_POSSIBLE); // RED LINE
      CG_DrawEdge(mMoveTrace.endpos, ContactPoint.v,
                  EDGE_IMPACT_POSSIBLE); // RED LINE
    }
  }
  //=================================================================
...
// Collision Avoidance
//
// Usually the last steering operation to call before finialization, this
// operation attempts to avoid collisions with nearby entities and architecture
// by thrusing away from them.
////////////////////////////////////////////////////////////////////////////////////
float STEER::AvoidCollisions(gentity_t *actor, gentity_t *leader) {
  ...
  if (ProjectedSpeed > 0.01f) {
    ...
    ProjectVelocitySide.Reposition(
        CVec3::mZero,
        (actor->NPC->avoidSide == Side_Left) ? (40.0f) : (-40.0f));
    ...
    //---------------------------------------------------
    bool HitFront = TestCollision(actor, suser, ProjectedVelocity,
                                  ProjectedSpeed, Side_None, 1.0f);
    bool HitSide =
        TestCollision(actor, suser, ProjectedVelocity, ProjectedSpeed,
                      (ESide)actor->NPC->avoidSide, 0.5f);
    if (!HitSide) {
      // If The Side Is Clear, Try The Other Side
      //------------------------------------------
      actor->NPC->avoidSide =
          (actor->NPC->avoidSide == Side_Left) ? (Side_Right) : (Side_Left);
    }
...
// Reached
////////////////////////////////////////////////////////////////////////////////////
bool STEER::Reached(gentity_t *actor, gentity_t *target, float targetRadius,
                    bool flying) {
  if (!actor || !target) {
    return false;
  }
  ...
  if (TargetPos.Dist2(ActorPos) < (targetRadius * targetRadius) ||
      (TargetPos > ActorMins && TargetPos < ActorMaxs)) {
    return true;
  }
  return false;
}
...
// Reached
////////////////////////////////////////////////////////////////////////////////////
bool STEER::Reached(gentity_t *actor, NAV::TNodeHandle target,
                    float targetRadius, bool flying) {
  if (!actor || !target) {
    return false;
  }
...
// Reached
////////////////////////////////////////////////////////////////////////////////////
bool STEER::Reached(gentity_t *actor, const vec3_t &target, float targetRadius,
                    bool flying) {
  if (!actor || !target) {
    return false;
  }
  ...
  //	if (target->client && target->client->ps.groundEntityNum ==
  //ENTITYNUM_NONE)
  //	{
  //		TargetPos -= ActorPos;
  ...
  //			TargetPos[2] = 0.0f;
  //			if
  //(TargetPos.Len2()<((targetRadius*2.0f)*(targetRadius*2.0f)))
  //			{
  //				return true;
  //			}
  //		}
...
// Clean up all of the krufty structures that only grow, never shrink,
// eventually causing asserts and subsequent memory trashing.
