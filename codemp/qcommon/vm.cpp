...
static const char *vmNames[MAX_VM] = {"jampgame", "cgame", "ui"};
...
//	CGVM_Init( foo, bar );			// internally may use VM_Call(
//cgvm, CGAME_INIT, foo, bar ) for legacy cgame modules 	cgvm = VM_Restart( cgvm
//);		// vmTable[VM_CGAME] is recreated, we update the cgvm pointer
//	VM_Free( cgvm );				// vmTable[VM_CGAME] is
//deallocated and set to NULL
//	cgvm = NULL;					// ...so we update the cgvm
//pointer
...
// This is likely due to C's inability to pass "..." parameters to a function in
// one clean chunk. On PowerPC Linux, these parameters are not necessarily
// passed on the stack, so while (&arg[0] == arg) is true,
//	(&arg[1] == 2nd function parameter) is not necessarily accurate, as
//arg's value might have been stored to the stack 	or other piece of scratch
//memory to give it a valid address, but the next parameter might still be
//sitting in a 	register.
// QVM's syscall system also assumes that the stack grows downward, and that any
// needed types can be squeezed, safely,
//	into a signed int.
// This hack below copies all needed values for an argument to an array in
// memory, so that QVM can get the correct values. This can also be used on
// systems where the stack grows upwards, as the presumably standard and safe
// stdargs.h macros
//	are used.
// The original code, while probably still inherently dangerous, seems to work
// well enough for the platforms it already
//	works on. Rather than add the performance hit for those platforms, the
//original code is still in use there.
// For speed, we just grab 15 arguments, and don't worry about exactly how many
// the syscall actually needs; the extra is
//	thrown away.
...
vm_t *VM_CreateLegacy(vmSlots_t vmSlot, intptr_t (*systemCalls)(intptr_t *)) {
  ...
  vm->dllHandle =
      Sys_LoadLegacyGameDll(vm->name, &vm->legacy.main, VM_DllSyscall);
...
intptr_t QDECL VM_Call(vm_t *vm, int callnum, intptr_t arg0, intptr_t arg1,
  ...
  return vm->legacy.main(callnum, arg0, arg1, arg2, arg3, arg4, arg5, arg6,
