...
static qboolean Grenadier_Move(void) {
  ...
  if (moved == qfalse) { // couldn't get to enemy
    if ((NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) &&
        NPCS.NPC->client->ps.weapon == WP_THERMAL && NPCS.NPCInfo->goalEntity &&
  ...
  */
  ...
  if ((NPCS.NPCInfo->goalEntity != NPCS.NPC->enemy) &&
    ...
    if (NAV_HitNavGoal(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins,
        ...
        (NPCS.NPCInfo->squadState == SQUAD_SCOUT && enemyLOS3 &&
         enemyDist3 <= 10000)) {
...
void NPC_BSGrenadier_Attack(void) {
  ...
  faceEnemy3 = qfalse;
  shoot3 = qfalse;
  enemyDist3 = DistanceSquared(NPCS.NPC->enemy->r.currentOrigin,
                               NPCS.NPC->r.currentOrigin);
  ...
  if (enemyDist3 < 16384 // 128
      && (!NPCS.NPC->enemy->client ||
          ...
          BG_SabersOff(&NPCS.NPC->enemy->client
                            ->ps))) { // enemy is close and not using saber
    if (NPCS.NPC->client->ps.weapon == WP_THERMAL) { // grenadier
      trace_t trace;
      trap->Trace(&trace, NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.mins,
                  NPCS.NPC->enemy->r.maxs, NPCS.NPC->enemy->r.currentOrigin,
                  NPCS.NPC->s.number, NPCS.NPC->enemy->clipmask, qfalse, 0, 0);
      if (!trace.allsolid && !trace.startsolid &&
          (trace.fraction == 1.0 ||
           trace.entityNum ==
               NPCS.NPC->enemy->s.number)) { // I can get right to him
        ...
        { // FIXME: should we be overriding scriptFlags?
          NPCS.NPCInfo->scriptFlags |=
             ...
             (NPCS.NPC->enemy->client &&
              NPCS.NPC->enemy->client->ps.weapon == WP_SABER &&
              !NPCS.NPC->enemy->client->ps.saberHolstered)) // 256
  { // enemy is far or using saber
    if (NPCS.NPC->client->ps.weapon == WP_STUN_BATON &&
        (NPCS.NPC->client->ps.stats[STAT_WEAPONS] &
    ...
    if (NPCS.NPC->client->ps.weapon == WP_STUN_BATON) {
      if (enemyDist3 <= 4096 &&
          InFOV3(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin,
                 NPCS.NPC->client->ps.viewangles, 90,
                 45)) // within 64 & infront
      {
        ...
        enemyCS3 = qtrue;
                   ...
                   NPCS.NPC->client->ps.viewangles, 45, 90)) { // in front of me
  ...
  */
  ...
  if (enemyLOS3) { // FIXME: no need to face enemy if we're moving to some other
  ...
  if (enemyCS3) {
    ...
    if (NPCS.NPC->client->ps.weapon == WP_THERMAL) { // don't chase and throw
      move3 = qfalse;
    } else if (NPCS.NPC->client->ps.weapon == WP_STUN_BATON &&
               enemyDist3 <
                   (NPCS.NPC->r.maxs[0] + NPCS.NPC->enemy->r.maxs[0] + 16) *
                       (NPCS.NPC->r.maxs[0] + NPCS.NPC->enemy->r.maxs[0] +
                        16)) { // close enough
      move3 = qfalse;
    }
