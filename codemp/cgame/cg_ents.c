...
void CG_PositionEntityOnTag(refEntity_t *entity, const refEntity_t *parent, qhandle_t parentModel, char *tagName) {
	...
	// lerp the tag
	trap->R_LerpTag(&lerped, parentModel, parent->oldframe, parent->frame, 1.0 - parent->backlerp, tagName);
...
void CG_S_UpdateLoopingSounds(int entityNum) {
	...
	if ((cent->currentState.eFlags & EF_SOUNDTRACKER) &&
		(!cg.snap || cent->currentState.trickedentindex !=
						 cg.snap->ps.clientNum)) { // keep sound for this entity updated in accordance with its attached entity at all times
		// entity out of range
...
static void CG_EntityEffects(centity_t *cent) {
	...
	if (cent->currentState.loopSound ||
		((cent->currentState.loopIsSoundset && cent->currentState.number >= MAX_CLIENTS) && cent->currentState.loopSound < MAX_SOUNDS)) {
...
localEntity_t *FX_AddOrientedLine(vec3_t start, vec3_t end, vec3_t normal, float stScale, float scale, float dscale, float startalpha, float endalpha,
...
static void CG_General(centity_t *cent) {
	...
	if (cent->currentState.boltToPlayer) { // Shove it into the player's left hand then.
		...
		if (CG_IsMindTricked(pl->currentState.trickedentindex, pl->currentState.trickedentindex2, pl->currentState.trickedentindex3,
		...
		if (!CG_RenderTimeEntBolt(cent)) { // If this function returns qfalse we shouldn't render this ent at all.
			if (cent->currentState.boltToPlayer > 0 && cent->currentState.boltToPlayer <= MAX_CLIENTS) {
				...
				if (cent->currentState.eFlags &
					EF_CLIENTSMOOTH) { // if it's set to smooth keep the smoothed lerp origin updated, as we don't want to smooth while bolted.
	...
	if (cent->currentState.modelGhoul2 >= G2_MODELPART_HEAD && cent->currentState.modelGhoul2 <= G2_MODELPART_RLEG &&
		...
		if (!cent->ghoul2) {
			...
			if (clEnt && clEnt->ghoul2) {
				...
				if (clEnt->localAnimIndex <= 0) { // humanoid
					...
					trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "pelvis", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels,
											  0, cg.time);
		...
		if (cent->ghoul2 && cent->bolt4 != -1 && cent->trailTime < cg.time) {
			if (cent->bolt4 != -1 && (cent->currentState.pos.trDelta[0] || cent->currentState.pos.trDelta[1] || cent->currentState.pos.trDelta[2])) {
				vec3_t boltOrg, boltAng;
...
static void CG_Item(centity_t *cent) {
	...
	*/
	...
	if ((item->giType == IT_WEAPON || item->giType == IT_POWERUP) && !(cent->currentState.eFlags & EF_DROPPEDWEAPON) && !cg_simpleItems.integer) {
		vec3_t uNorm;
		qboolean doGrey;
...
static void CG_Missile(centity_t *cent) {
	...
	if (s1->otherEntityNum2 && s1->weapon != WP_SABER) { // using an over-ridden trail effect!
		...
		if (weapon->altMissileDlight) {
			trap->R_AddLightToScene(cent->lerpOrigin, weapon->altMissileDlight, weapon->altMissileDlightColor[0], weapon->altMissileDlightColor[1],
		...
		if (weapon->missileDlight) {
			trap->R_AddLightToScene(cent->lerpOrigin, weapon->missileDlight, weapon->missileDlightColor[0], weapon->missileDlightColor[1],
...
void CG_CalcEntityLerpPositions(centity_t *cent) {
	...
	if (cg.predictedPlayerState.m_iVehicleNum && cg.predictedPlayerState.m_iVehicleNum == cent->currentState.number && cent->currentState.eType == ET_NPC &&
		cent->currentState.NPC_class == CLASS_VEHICLE) { // special case for vehicle we are riding
		centity_t *veh = &cg_entities[cg.predictedPlayerState.m_iVehicleNum];

		if (veh->currentState.owner == cg.predictedPlayerState.clientNum) { // only do this if the vehicle is pilotted by this client and predicting properly
