...
cvar_t *Cvar_Set2(const char *var_name, const char *value,
  ...
  if (!force) {
    if ((var->flags & (CVAR_SYSTEMINFO | CVAR_SERVER_CREATED)) &&
...
static const char *legacyCvars[] = {"bg_fighterAltControl",
                                    "g_dlURL",
...
void Cvar_Server_Set(const char *var_name, const char *value) {
  ...
  if (flags != CVAR_NONEXISTENT) {
    ...
    // But check for ones where the legacy gamecode might still need to be
    // allowed
    if (!(flags &
    ...
    if ((flags & CVAR_PROTECTED)) {
      if (value)
        Com_Error(ERR_DROP, "Server tried to set \"%s\" to \"%s\"", var_name,
                  value);
      else
...
void Cvar_VM_Set(const char *var_name, const char *value, vmSlots_t vmslot) {
  ...
  if (vmslot != VM_GAME && (flags & CVAR_SYSTEMINFO) &&
      CL_ConnectedToRemoteServer()) {
    Com_Printf("%s can only be set by server.\n", var_name);
    return;
  }
  ...
  if (flags != CVAR_NONEXISTENT && (flags & CVAR_PROTECTED)) {
    if (value)
      Com_Error(ERR_DROP, "%s tried to set \"%s\" to \"%s\"", vmStrs[vmslot],
                var_name, value);
    else
      Com_Error(ERR_DROP, "%s tried to modify \"%s\"", vmStrs[vmslot],
                var_name);
    return;
  }
...
============
*/
void Cvar_Reset(const char *var_name) { Cvar_SetSafe(var_name, NULL); }
...
============
*/
void Cvar_ForceReset(const char *var_name) { Cvar_Set(var_name, NULL); }
...
void Cvar_Math_f(void) {
  ...
  if (!Q_stricmp(cmd, "cvarAdd")) {
    ...
    Cvar_User_SetValue(Cmd_Argv(1), Cvar_VariableIntegerValue(Cmd_Argv(1)) %
                                        atoi(Cmd_Argv(2)));
  }
}
...
char *Cvar_InfoString(int bit) {
  ...
  for (var = cvar_vars; var; var = var->next) {
    if (!(var->flags & CVAR_INTERNAL) && var->name && (var->flags & bit)) {
...
void Cvar_Register(vmCvar_t *vmCvar, const char *varName,
  ...
  // user. In other words CVAR_ARCHIVE and
  //	CVAR_ROM are mutually exclusive flags. Unfortunately some historical
  //game code (including single player baseq3) 	sets both flags. We unset
  //CVAR_ROM for such cvars.
  if ((flags & (CVAR_ARCHIVE | CVAR_ROM)) == (CVAR_ARCHIVE | CVAR_ROM)) {
    Com_DPrintf(S_COLOR_YELLOW "WARNING: Unsetting CVAR_ROM cvar '%s', since "
                               "it is also CVAR_ARCHIVE\n",
                varName);
    flags &= ~CVAR_ROM;
  }
...
void Cvar_Init(void) {
  ...
  Cmd_AddCommand("unset_usercreated", Cvar_UnsetUserCreated_f,
                 "Unset all user generated cvars " S_COLOR_RED
                 "Use with caution!" S_COLOR_WHITE);
