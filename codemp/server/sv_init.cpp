...
void SV_SendMapChange(void) {
	...
	if (svs.clients) {
		for (i = 0; i < sv_maxclients->integer; i++) {
			if (svs.clients[i].state >= CS_CONNECTED) {
				if (svs.clients[i].netchan.remoteAddress.type != NA_BOT || svs.clients[i].demo.demorecording) {
...
void SV_Init(void) {
	...
	sv_floodProtectSlow =
		Cvar_Get("sv_floodProtectSlow", "1", CVAR_ARCHIVE | CVAR_SERVERINFO, "Use original method of delaying commands with flood protection");
	// systeminfo
