...
void MineMonster_TryDamage(gentity_t *enemy, int damage) {
  ...
  trap->Trace(&tr, NPCS.NPC->r.currentOrigin, vec3_origin, vec3_origin, end,
              NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0);

  if (tr.entityNum >= 0 && tr.entityNum < ENTITYNUM_NONE) {
    G_Damage(&g_entities[tr.entityNum], NPCS.NPC, NPCS.NPC, dir, tr.endpos,
             damage, DAMAGE_NO_KNOCKBACK, MOD_MELEE);
    G_Sound(
        NPCS.NPC, CHAN_AUTO,
        G_EffectIndex(va("sound/chars/mine/misc/bite%i.wav", Q_irand(1, 4))));
  } else {
    G_Sound(
        NPCS.NPC, CHAN_AUTO,
        G_EffectIndex(va("sound/chars/mine/misc/miss%i.wav", Q_irand(1, 4))));
  }
}
...
void MineMonster_Attack(void) {
  if (!TIMER_Exists(NPCS.NPC, "attacking")) {
    ...
    if (NPCS.NPC->enemy &&
        ((NPCS.NPC->enemy->r.currentOrigin[2] - NPCS.NPC->r.currentOrigin[2] >
              10 &&
          Q_flrand(0.0f, 1.0f) > 0.1f) ||
         Q_flrand(0.0f, 1.0f) > 0.8f)) {
      // Going to do ATTACK4
      TIMER_Set(NPCS.NPC, "attacking", 1750 + Q_flrand(0.0f, 1.0f) * 200);
      NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_ATTACK4,
      ...
      if (Q_flrand(0.0f, 1.0f) > 0.8f) {
        ...
        NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_ATTACK3,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      ...
      NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_ATTACK2,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void MineMonster_Combat(void) {
  ...
  if ((advance || NPCS.NPCInfo->localState == LSTATE_WAITING) &&
      TIMER_Done(NPCS.NPC, "attacking")) // waiting monsters can't attack
  {
...
void NPC_MineMonster_Pain(gentity_t *self, gentity_t *attacker, int damage) {
  G_AddEvent(
      self, EV_PAIN,
      floor((float)self->health / self->client->pers.maxHealth * 100.0f));
