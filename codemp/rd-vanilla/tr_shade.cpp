...
void R_BindAnimatedImage(textureBundle_t *bundle) {
  ...
  if ((r_fullbright->value /*|| tr.refdef.doFullbright */) &&
      bundle->isLightmap) {
  ...
  if (backEnd.currentEntity->e.renderfx & RF_SETANIMINDEX) {
    ...
    index =
        Q_ftol(tess.shaderTime * bundle->imageAnimationSpeed * FUNCTABLE_SIZE);
...
static void ComputeColors(shaderStage_t *pStage, int forceRGBGen) {
  ...
  alphaGen_t forceAlphaGen =
      pStage->alphaGen; // set this up so we can override below
  ...
  case AGEN_BLEND:
    if (forceRGBGen != CGEN_VERTEX) {
      for (i = 0; i < tess.numVertexes; i++) {
        colors[i][3] =
            tess.vertexAlphas[i][pStage->index]; // rwwRMG - added support
      }
    }
    break;
  default:
    ...
    for (tm = 0; tm < pStage->bundle[b].numTexMods; tm++) {
      ...
      case TMOD_SCROLL:
        RB_CalcScrollTexCoords(
            pStage->bundle[b].texMods[tm].translate, // scroll unioned
            (float *)tess.svars.texcoords[b]);
        break;
  ...
  for (stage = 0; stage < input->shader->numUnfoggedPasses; stage++) {
    ...
    if (pStage->bundle[1].image != 0) {
      ...
      if (tess.shader == tr.distortionShader &&
        ...
        if (backEnd.currentEntity->e.renderfx &
            RF_ALPHA_DEPTH) { // depth write, so faces through the model will be
                              // stomped over by nearer ones. this works because
          // we draw RF_FORCE_ENT_ALPHA stuff after everything else, including
