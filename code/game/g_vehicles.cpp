...
void Vehicle_SetAnim(gentity_t *ent, int setAnimParts, int anim,
                     int setAnimFlags, int iBlend) {
#ifdef _JK2MP
  assert(ent->client);
...
void G_VehicleTrace(trace_t *results, const vec3_t start, const vec3_t tMins,
                    const vec3_t tMaxs, const vec3_t end, int passEntityNum,
                    int contentmask) {
#ifdef _JK2MP
...
Vehicle_t *G_IsRidingVehicle(gentity_t *pEnt) {
  ...
  if (ent && ent->client && ent->client->NPC_class != CLASS_VEHICLE &&
      ent->s.m_iVehicleNum != 0) // ent->client && ( ent->client->ps.eFlags &
                                 // EF_IN_VEHICLE ) && ent->owner )
  {
...
bool G_IsRidingTurboVehicle(gentity_t *pEnt) {
  ...
  {
    return (level.time <
            g_entities[ent->s.m_iVehicleNum].m_pVehicle->m_iTurboTime);
  }
  return false;
}
...
float G_CanJumpToEnemyVeh(Vehicle_t *pVeh, const usercmd_t *pUcmd) {
  ...
  //------------------------------------------------------
  if (rider && rider->enemy && pUcmd->rightmove &&
    ...
    if (enemyVeh) {
      ...
      //------------------------------------------------
      VectorSubtract(rider->enemy->currentOrigin, rider->currentOrigin,
                     toEnemy);
      toEnemyDistance = VectorNormalize(toEnemy);
      if (toEnemyDistance < 70.0f &&
          pVeh->m_pParentEntity->resultspeed > 100.0f &&
          fabsf(pVeh->m_pParentEntity->resultspeed -
                enemyVeh->m_pParentEntity->resultspeed) < 100.0f) {
        ...
        if ((pUcmd->rightmove > 0 && riderRightDot > 0.2) ||
            (pUcmd->rightmove < 0 && riderRightDot < -0.2)) {
          ...
          if (DotProduct(enemyFwd, riderFwd) > 0.2f) {
            pVeh->m_safeJumpMountTime =
                level.time +
                Q_irand(3000, 4000); // Ok, now you get a 3 sec window
...
void G_AttachToVehicle(gentity_t *pEnt, usercmd_t **ucmd) {
  ...
  trap_G2API_GetBoltMatrix(vehEnt->ghoul2, 0, crotchBolt, &boltMatrix,
                           vehEnt->m_pVehicle->m_vOrientation,
                           vehEnt->currentOrigin, level.time, NULL,
                           vehEnt->modelScale);
  ...
  gi.G2API_GetBoltMatrix(
      vehEnt->ghoul2, vehEnt->playerModel, vehEnt->crotchBolt, &boltMatrix,
...
void G_KnockOffVehicle(gentity_t *pRider, gentity_t *self, qboolean bPull) {
  ...
  if (fDot >= 0.5f) { // I'm in front of them
    if (bPull) {      // pull them foward
      pVeh->m_EjectDir = VEH_EJECT_FRONT;
    } else { // push them back
      pVeh->m_EjectDir = VEH_EJECT_REAR;
    }
  } else if (fDot <= -0.5f) { // I'm behind them
    if (bPull) {              // pull them back
      pVeh->m_EjectDir = VEH_EJECT_REAR;
    } else { // push them forward
      pVeh->m_EjectDir = VEH_EJECT_FRONT;
    }
  } else { // to the side of them
    rDot = DotProduct(fDir, dir2Me);
    if (rDot >= 0.0f) { // to the right
      if (bPull) {      // pull them right
        pVeh->m_EjectDir = VEH_EJECT_RIGHT;
      } else { // push them left
        pVeh->m_EjectDir = VEH_EJECT_LEFT;
      }
    } else {       // to the left
      if (bPull) { // pull them left
        pVeh->m_EjectDir = VEH_EJECT_LEFT;
      } else { // push them right
        pVeh->m_EjectDir = VEH_EJECT_RIGHT;
      }
    }
  }
...
void G_DriveATST(gentity_t *pEnt, gentity_t *atst) {
  if (pEnt->NPC_type && pEnt->client &&
      (pEnt->client->NPC_class == CLASS_ATST)) { // already an atst, switch back
    // open hatch
    ...
    pEnt->client->ps.stats[STAT_WEAPONS] &=
    ...
    if (pEnt->client->ps.stats[STAT_WEAPONS] & (1 << WP_BLASTER)) {
      ...
      // camera
      if (cg_gunAutoFirst.integer) { // go back to first person
...
bool ValidateBoard(Vehicle_t *pVeh, bgEntity_t *pEnt) {
  // Determine where the entity is entering the vehicle from (left, right, or
  // back).
  vec3_t vVehToEnt;
  vec3_t vVehDir;
  ...
  if (ent->health <= 0) { // dead men can't ride vehicles
    return false;
  }

  if (pVeh->m_pPilot != NULL) { // already have a driver!
    if (pVeh->m_pVehicleInfo->type ==
        VH_FIGHTER) { // I know, I know, this should by in the fighters's
                      // validateboard()
      // can never steal a fighter from it's pilot
      return false;
    } else if (pVeh->m_pVehicleInfo->type ==
               VH_WALKER) { // I know, I know, this should by in the walker's
                            // validateboard()
      if (!ent->client ||
          ent->client->ps.groundEntityNum !=
              parent->s.number) { // can only steal an occupied AT-ST if you're
                                  // on top (by the hatch)
        return false;
      }
    } else if (pVeh->m_pVehicleInfo->type ==
               VH_SPEEDER) { // you can only steal the bike from the driver if
                             // you landed on the driver or bike
      return (pVeh->m_iBoarding == VEH_MOUNT_THROW_LEFT ||
              pVeh->m_iBoarding == VEH_MOUNT_THROW_RIGHT);
    }
  }
  // Yes, you shouldn't have put this here (you 'should' have made an
  // 'overriden' ValidateBoard func), but in this instance it's more than
  // adequate (which is why I do it too :-). Making a whole other function for
  // this is silly.
  ...
  // Find the angle between the vehicle right vector and the vehicle to entity
  // vector.
  ...
  // Maybe they're trying to board from the back...
    ...
    // Find the angle between the vehicle forward and the vehicle to entity
    // vector.
    //	fDot = DotProduct( vVehToEnt, vVehDir );
...
bool Board(Vehicle_t *pVeh, bgEntity_t *pEnt) {
  ...
  // If it's not a valid entity, OR if the vehicle is blowing up (it's dead), OR
  // it's not empty, OR we're already being boarded, OR the person trying to get
  // on us is already in a vehicle (that was a fun bug :-), leave!
  if (!ent ||
      parent->health <=
          0 /*|| !( parent->client->ps.eFlags & EF_EMPTY_VEHICLE )*/
      || (pVeh->m_iBoarding > 0) ||
#ifdef _JK2MP
  ...
  // Bucking so we can't do anything (NOTE: Should probably be a better name
  // since fighters don't buck...).
  ...
  // FIXME FIXME!!! Ask Mike monday where ent->client->ps.eFlags might be
  // getting changed!!! It is always 0 (when it should be 1024) so a person
  // riding a vehicle is able to ride another vehicle!!!!!!!!
  ...
  if (ent->s.number < MAX_CLIENTS) {
...
#ifdef QAGAME
    {
      ...
      if ((gParent->spawnflags & 2)) { // was being suspended
        gParent->spawnflags &=
            ~2; // SUSPENDED - clear this spawnflag, no longer docked, okay to
                // free-fall if not in space
        // gParent->client->ps.eFlags &= ~EF_RADAROBJECT;
        G_Sound(gParent, CHAN_AUTO,
                G_SoundIndex("sound/vehicles/common/release.wav"));
        if (gParent->fly_sound_debounce_time) { // we should drop like a rock
                                                // for a few seconds
    ...
    CG_CenterPrint("@SP_INGAME_EXIT_VIEW",
                   SCREEN_HEIGHT * 0.86); // tell them how to get out!
#endif
    ...
    // Set the looping sound only when there is a pilot (when the vehicle is
    // "on").
    if (pVeh->m_pVehicleInfo->soundLoop) {
#ifdef _JK2MP
      parent->client->ps.loopSound = parent->s.loopSound =
          pVeh->m_pVehicleInfo->soundLoop;
#else
  ...
  if (pVeh->m_pPilot == (bgEntity_t *)ent) {
    parent->client->ps.m_iVehicleNum =
        ent->s.number + 1; // always gonna be under MAX_CLIENTS so no worries
                           // about 1 byte overflow
  }
#else
  ent->s.m_iVehicleNum = parent->s.number;
  ent->owner = parent;
  ...
  if (pVeh->m_pVehicleInfo->numHands == 2) { // switch to vehicle weapon
    ...
    if (ent->s.number < MAX_CLIENTS) {       // Riding means you get WP_NONE
      ent->client->ps.stats[STAT_WEAPONS] |= (1 << WP_NONE);
    }
    if ((ent->client->ps.weapon != WP_SABER &&
         ent->client->ps.weapon != WP_BLASTER) ||
        !(pVeh->m_pVehicleInfo->type == VH_ANIMAL ||
          pVeh->m_pVehicleInfo->type == VH_SPEEDER)) { // switch to weapon none?
      if (ent->s.number < MAX_CLIENTS) {
        CG_ChangeWeapon(WP_NONE);
      }
...
#else
    // NOTE: Use this type so it's spatialized and updates play origin as bike
    // moves - MCG
...
bool VEH_TryEject(Vehicle_t *pVeh, gentity_t *parent, gentity_t *ent,
                  int ejectDir, vec3_t vExitPos) {
  float fBias;
  float fVehDiag;
  float fEntDiag;
  ...
  // bboxes will now intersect). This makes the entity leave the vehicle from
  // the right side.
  VectorSet(vVehAngles, 0, parent->currentAngles[YAW], 0);
  switch (ejectDir) {
  // Left.
  ...
  // Since (as of this time) the collidable geometry of the entity is just an
  // axis aligned box, we need to get the diagonal length of it in case we come
  // out on that side. Diagonal Length == squareroot( squared( Sidex / 2 ) +
  // squared( Sidey / 2 ) );
  ...
  fVehDiag = sqrtf((parent->maxs[0] * parent->maxs[0]) +
                   (parent->maxs[1] * parent->maxs[1]));
  VectorCopy(ent->maxs, vEntMaxs);
#ifdef _JK2MP
  if (ent->s.number <
      MAX_CLIENTS) { // for some reason, in MP, player client mins and maxs are
                     // never stored permanently, just set to these hardcoded
                     // numbers in PMove
    vEntMaxs[0] = 15;
    vEntMaxs[1] = 15;
  }
  ...
  VectorCopy(ent->maxs, vEntMaxs);
#endif
  G_VehicleTrace(&m_ExitTrace, ent->currentOrigin, vEntMins, vEntMaxs, vExitPos,
...
#ifdef _JK2MP
    if ((parent->clipmask &
         ent->r.contents)) // vehicle could actually get stuck on body
#else
    if ((parent->clipmask &
         ent->contents)) // vehicle could actually get stuck on body
#endif
...
//	pVeh->m_pDroidUnit->s.otherEntityNum2 = ENTITYNUM_NONE;
#ifdef QAGAME
  {
    ...
    if (kill) { // Kill them, too
      // FIXME: proper origin, MOD and attacker (for credit/death message)?  Get
      // from vehicle?
      G_MuteSound(droidEnt->s.number, CHAN_VOICE);
      G_Damage(droidEnt, NULL, NULL, NULL, droidEnt->s.origin, 10000, 0,
               MOD_SUICIDE); // FIXME: proper MOD?  Get from vehicle?
    }
  }
#endif
...
bool Eject(Vehicle_t *pVeh, bgEntity_t *pEnt, qboolean forceEject) {
  ...
  if (!forceEject) {
    if (!(pVeh->m_iBoarding == 0 || pVeh->m_iBoarding == -999 ||
          (pVeh->m_iBoarding < -3 && pVeh->m_iBoarding >= -9))) {
#ifdef _JK2MP // I don't care, if he's dead get him off even if he died while
              // boarding
      deadRider = qtrue;
      pVeh->m_iBoarding = 0;
...
#ifdef _JK2MP // in MP if someone disconnects on us, we still have to clear our
              // owner
getItOutOfMe:
#endif
  ...
  if ((gentity_t *)pVeh->m_pPilot == ent) {
...
#ifdef _JK2MP // if there are some passengers, promote the first passenger to
              // pilot
    while (j < pVeh->m_iNumPassengers) {
      if (pVeh->m_ppPassengers[j]) {
        ...
        parent->s.owner = parent->r.ownerNum; // for prediction
        parent->client->ps.m_iVehicleNum =
            pVeh->m_ppPassengers[j]->s.number + 1;
        ...
        while (k < pVeh->m_iNumPassengers) {
          if (!pVeh->m_ppPassengers[k - 1]) { // move down
            ...
            if (((gentity_t *)pVeh->m_ppPassengers[k - 1])->client) {
              ((gentity_t *)pVeh->m_ppPassengers[k - 1])->client->ps.generic1 =
                  k;
            }
#endif
          }
  ...
  // if was using vehicle weapon, remove it and switch to normal weapon when hop
  // out...
  if (ent->client->ps.weapon ==
      WP_NONE) { // FIXME: check against this vehicle's gun from the
                 // g_vehicleInfo table remove the vehicle's weapon from me
                 // ent->client->ps.stats[STAT_WEAPONS] &= ~( 1 <<
                 // WP_EMPLACED_GUN );
                 // ent->client->ps.ammo[weaponData[WP_EMPLACED_GUN].ammoIndex]
                 // = 0;//maybe store this ammo on the vehicle before clearing
                 // it? switch back to a normal weapon we're carrying

    // FIXME: store the weapon we were using when we got on and restore that
    // when hop off
    /*		if ( (ent->client->ps.stats[STAT_WEAPONS]&(1<<WP_SABER)) )
                    ...
                    {//go through all weapons and switch to highest held
                            for ( int checkWp = WP_NUM_WEAPONS-1; checkWp >
       WP_NONE; checkWp-- )
                            {
                                    if (
       (ent->client->ps.stats[STAT_WEAPONS]&(1<<checkWp)) )
                                    {
           ...
           // NOTE: this is because you're only allowed to use your first
           // saber's first blade on a vehicle
  }
...
bool EjectAll(Vehicle_t *pVeh) {
  ...
  if (pVeh->m_pPilot) {
    ...
    if (pVeh->m_pVehicleInfo->killRiderOnDeath && pilot) { // Kill them, too
      ...
      G_Damage(pilot, player, player, NULL, pilot->s.origin, 10000, 0,
               MOD_SUICIDE);
    }
#endif
  }
...
static void DeathUpdate(Vehicle_t *pVeh) {
  ...
  if (level.time >= pVeh->m_iDieTime) {
...
#ifdef _JK2MP
      if (pVeh->m_pVehicleInfo->Inhabited(
              pVeh)) { // if we've still got people in us, just kill the
                       // bastards
        if (pVeh->m_pPilot) {
          // FIXME: does this give proper credit to the enemy who shot you down?
          G_Damage((gentity_t *)pVeh->m_pPilot,
                   (gentity_t *)pVeh->m_pParentEntity,
                   (gentity_t *)pVeh->m_pParentEntity, NULL,
                   pVeh->m_pParentEntity->playerState->origin, 999,
                   DAMAGE_NO_PROTECTION, MOD_EXPLOSIVE);
        }
        if (pVeh->m_iNumPassengers) {
          ...
          for (i = 0; i < pVeh->m_pVehicleInfo->maxPassengers; i++) {
            if (pVeh->m_ppPassengers[i]) {
              // FIXME: does this give proper credit to the enemy who shot you
              // down?
              G_Damage((gentity_t *)pVeh->m_ppPassengers[i],
                       (gentity_t *)pVeh->m_pParentEntity,
                       (gentity_t *)pVeh->m_pParentEntity, NULL,
                       pVeh->m_pParentEntity->playerState->origin, 999,
                       DAMAGE_NO_PROTECTION, MOD_EXPLOSIVE);
    ...
    if (!pVeh->m_pVehicleInfo->Inhabited(
            pVeh)) { // explode now as long as we managed to kick everyone out
      ...
      if (pVeh->m_pVehicleInfo->iArmorLowFX) {
        G_StopEffect(pVeh->m_pVehicleInfo->iArmorLowFX, parent->playerModel,
                     parent->crotchBolt, parent->s.number);
      }
      if (pVeh->m_pVehicleInfo->iArmorGoneFX) {
        G_StopEffect(pVeh->m_pVehicleInfo->iArmorGoneFX, parent->playerModel,
                     parent->crotchBolt, parent->s.number);
      }
      //--------------------------------------
#endif
      if (pVeh->m_pVehicleInfo->iExplodeFX) {
        ...
        G_PlayEffect(pVeh->m_pVehicleInfo->iExplodeFX, parent->currentOrigin,
                     fxAng);

        // trace down and place mark
        VectorCopy(parent->currentOrigin, bottom);
        bottom[2] -= 80;
        G_VehicleTrace(&trace, parent->currentOrigin, vec3_origin, vec3_origin,
                       bottom, parent->s.number, CONTENTS_SOLID);
        if (trace.fraction < 1.0f) {
          VectorCopy(trace.endpos, bottom);
          bottom[2] += 2;
#ifdef _JK2MP
          VectorSet(fxAng, -90.0f, 0.0f, 0.0f);
          G_PlayEffectID(G_EffectIndex("ships/ship_explosion_mark"),
                         trace.endpos, fxAng);
#else
      ...
      if (pVeh->m_pVehicleInfo->explosionRadius > 0 &&
        ...
        G_VehicleTrace(&trace, parent->currentOrigin, lMins, lMaxs, bottom,
                       parent->s.number, CONTENTS_SOLID);
#ifdef _JK2MP
        G_RadiusDamage(
            trace.endpos, NULL, pVeh->m_pVehicleInfo->explosionDamage,
            pVeh->m_pVehicleInfo->explosionRadius, NULL, NULL,
            MOD_EXPLOSIVE); // FIXME: extern damage and radius or base on fuel
#else
        G_RadiusDamage(
            trace.endpos, player, pVeh->m_pVehicleInfo->explosionDamage,
            pVeh->m_pVehicleInfo->explosionRadius, NULL,
            MOD_EXPLOSIVE); // FIXME: extern damage and radius or base on fuel
#endif
      }
...
#ifndef _JK2MP
  else {                   // let everyone around me know I'm gonna blow!
...
bool Initialize(Vehicle_t *pVeh) {
  ...
  for (i = 0; i < MAX_VEHICLE_WEAPONS; i++) {
    parent->client->ps.ammo[i] = pVeh->weaponStatus[i].ammo =
        pVeh->m_pVehicleInfo->weapon[i].ammoMax;
  }
  for (i = 0; i < MAX_VEHICLE_TURRETS; i++) {
    pVeh->turretStatus[i].nextMuzzle =
        (pVeh->m_pVehicleInfo->turret[i].iMuzzle[i] - 1);
    parent->client->ps.ammo[MAX_VEHICLE_WEAPONS + i] =
        pVeh->turretStatus[i].ammo = pVeh->m_pVehicleInfo->turret[i].iAmmoMax;
    if (pVeh->m_pVehicleInfo->turret[i]
            .bAI) { // they're going to be finding enemies, init this to NONE
  ...
  */
  ...
  // initialize to blaster, just since it's a basic weapon and there's no
  // lightsaber crap...?
...
static bool Update(Vehicle_t *pVeh, const usercmd_t *pUmcd) {
  ...
  for (i = 0; i < MAX_VEHICLE_WEAPONS; i++) {
    if (pVeh->m_pVehicleInfo->weapon[i].ID >
            VEH_WEAPON_BASE // have a weapon in this slot
        && pVeh->m_pVehicleInfo->weapon[i]
               .ammoRechargeMS // its ammo is rechargable
        && pVeh->weaponStatus[i].ammo <
               pVeh->m_pVehicleInfo->weapon[i].ammoMax // its ammo is below max
        && pUmcd->serverTime - pVeh->weaponStatus[i].lastAmmoInc >=
               pVeh->m_pVehicleInfo->weapon[i]
                   .ammoRechargeMS) // enough time has passed
    {                               // add 1 to the ammo
  ...
  for (i = 0; i < MAX_VEHICLE_TURRETS; i++) {
    if (pVeh->m_pVehicleInfo->turret[i].iWeapon >
            VEH_WEAPON_BASE // have a weapon in this slot
        && pVeh->m_pVehicleInfo->turret[i]
               .iAmmoRechargeMS // its ammo is rechargable
        && pVeh->turretStatus[i].ammo <
               pVeh->m_pVehicleInfo->turret[i].iAmmoMax // its ammo is below max
        && pUmcd->serverTime - pVeh->turretStatus[i].lastAmmoInc >=
               pVeh->m_pVehicleInfo->turret[i]
                   .iAmmoRechargeMS) // enough time has passed
    {                                // add 1 to the ammo
  ...
  if (pVeh->m_pVehicleInfo->shieldRechargeMS &&
      ...
      && parentPS->stats[STAT_ARMOR] <
             pVeh->m_pVehicleInfo->shields // its below max
      && pUmcd->serverTime - pVeh->lastShieldInc >=
             pVeh->m_pVehicleInfo->shieldRechargeMS) // enough time has passed
  {
    parentPS->stats[STAT_ARMOR]++;
  ...
  // keep the PS value in sync. set it up here in case we return below at some
  // point.
  if (pVeh->m_iBoarding) {
  ...
  if (pVeh->m_iDieTime != 0
    ...
    // Need to copy orientation to our entity's viewangles so that it renders at
    // the proper angle and currentAngles is correct.
    SetClientViewAngle(parent, pVeh->m_vOrientation);
    if (pVeh->m_pPilot) {
    ...
    for ( i = 0; i < pVeh->m_pVehicleInfo->maxPassengers; i++ )
    {
            if ( pVeh->m_ppPassengers[i] )
            {
                    SetClientViewAngle( (gentity_t *)pVeh->m_ppPassengers[i],
    pVeh->m_vOrientation );
            }
    }
    */
    ...
    if (pVeh->m_pVehicleInfo->type == VH_FIGHTER) {
      AngleVectors(pVeh->m_vOrientation, parent->client->ps.moveDir, NULL,
                   NULL);
    } else {
  ...
  // Vehicle dead!
    ...
    if (pVeh->m_pVehicleInfo->type == VH_FIGHTER &&
      ...
      pVeh->m_pVehicleInfo->StartDeathDelay(pVeh,
                                            -1 /* -1 causes instant death */);
    } else {
  ...
  if (parent->spawnflags & 1) {
    if (pVeh->m_pPilot || !pVeh->m_bHasHadPilot) {
      ...
      if (!oldPilot->inuse || !oldPilot->client ||
          oldPilot->client->pers.connected !=
              CON_CONNECTED) { // no longer in the game?
        G_Damage(parent, parent, parent, NULL, parent->client->ps.origin, 99999,
                 DAMAGE_NO_PROTECTION, MOD_SUICIDE);
      } else {
        vec3_t v;
        VectorSubtract(parent->client->ps.origin, oldPilot->client->ps.origin,
                       v);

        if (VectorLength(v) <
            parent->speed) { // they are still within the minimum distance to
                             // their vehicle
          pVeh->m_iPilotTime = level.time + parent->damage;
        } else if (pVeh->m_iPilotTime < level.time) { // dying time
          G_Damage(parent, parent, parent, NULL, parent->client->ps.origin,
                   99999, DAMAGE_NO_PROTECTION, MOD_SUICIDE);
        }
      }
    }
  }
#endif
#else
  if (parent->spawnflags &
      1) { // NOTE: in SP, this actually just checks LOS to the Player
    if (pVeh->m_iPilotTime < level.time) { // do another check?
      if (!player ||
          G_ClearLineOfSight(pVeh->m_pParentEntity->currentOrigin,
    ...
    if (pVeh->m_iPilotTime &&
        pVeh->m_iPilotTime <
            level.time) { // die
                          // FIXME: does this give proper credit to the enemy
                          // who shot you down?
      G_Damage(parent, player, player, NULL, parent->client->ps.origin, 99999,
               DAMAGE_NO_PROTECTION, MOD_SUICIDE);
    }
  }
#endif
  ...
  // always knock guys around now...
  {
    ...
    if (force > 30.0f) {
      trace_t tr;
      G_VehicleTrace(&tr, parent->currentOrigin, parent->mins, parent->maxs,
                     projectedPosition, parent->s.number, CONTENTS_BODY);
      if (tr.fraction < 1.0f && !tr.allsolid && !tr.startsolid &&
          tr.entityNum != ENTITYNUM_NONE && tr.entityNum != ENTITYNUM_WORLD &&
        ...
        if (other && other->client && !other->s.m_iVehicleNum) {
          ...
          G_Damage(other, player, player, parent->client->ps.velocity,
                   parent->currentOrigin, force,
  ...
  pVeh->m_ucmd.buttons &=
      ~(BUTTON_TALK); //|BUTTON_GESTURE); //don't want some of these buttons
#else
  // Copy over the commands for local storage.
  ...
  pVeh->m_fTimeModifier = pVeh->m_ucmd.serverTime -
  parent->client->ps.commandTime;
  //sanity check
  ...
  */
  ...
  for (i = 0; i < MAX_VEHICLE_WEAPONS;
       i++) { // HMM... can't get a seperate command for each weapon, so do them
              // all...?
    ...
    { // pilot pressed the "weapon link" toggle button
      ...
      if (parent->s.owner != ENTITYNUM_NONE) {
        rider =
            PM_BGEntForNum(parent->s.owner); //&g_entities[parent->r.ownerNum];
      }
...
#endif
      if (!pVeh->linkWeaponToggleHeld) // so we don't hold it down and toggle it
                                       // back and forth
      {                                                      // okay to toggle
        if (pVeh->m_pVehicleInfo->weapon[i].linkable == 1) { // link-toggleable
          pVeh->weaponStatus[i].linked =
              (qboolean)!pVeh->weaponStatus[i].linked;
        }
      }
      linkHeld = qtrue;
    }
...
maintainSelfDuringBoarding:
  ...
  */
  ...
  if (pVeh->m_iTurboTime < curTime && pVeh->m_iSoundDebounceTimer < curTime &&
      ((nextSpeed > prevSpeed && nextSpeed > halfMaxSpeed &&
        prevSpeed < halfMaxSpeed) ||
       (nextSpeed > halfMaxSpeed && !Q_irand(0, 1000)))) {
  ...
  if (pVeh->m_pVehicleInfo->surfDestruction) {
    if (pVeh->m_iRemovedSurfaces) {
      ...
      if (parent->client->ps.otherKiller < ENTITYNUM_WORLD &&
        ...
        gentity_t *potentialKiller =
            &g_entities[parent->client->ps.otherKiller];
      ...
      // ?
      G_Damage(parent, killer, killer, NULL, parent->client->ps.origin,
               Q_irand(2, 5), DAMAGE_NO_PROTECTION | DAMAGE_NO_ARMOR,
               MOD_SUICIDE);
    }
...
#ifndef _JK2MP
  // Make sure the vehicle takes on the enemy of it's rider (for homing missles
  // for instance).
  if (pVeh->m_pPilot) {
...
static bool UpdateRider(Vehicle_t *pVeh, bgEntity_t *pRider, usercmd_t *pUmcd) {
  ...
  if (rider && rider->client && parent &&
      parent->client) { // so they know who we're locking onto with our rockets,
                        // if anyone
    ...
    if (pVeh->m_pVehicleInfo->type == VH_WALKER) { // just get the fuck out
      ...
      if ((parent->client->ps.speed <= 600) && pUmcd->rightmove != 0) {
        if (pVeh->m_pVehicleInfo->Eject(pVeh, pRider, qfalse)) {
          ...
          VectorScale(parent->client->ps.velocity, 0.25f,
                      rider->client->ps.velocity);
#if 1
          ...
          rider->client->ps.weaponTime =
              rider->client->ps.torsoAnimTimer -
        ...
        if (pVeh->m_iBoarding <= 1) {
          int iAnimLen;
          // NOTE: I know I shouldn't reuse pVeh->m_iBoarding so many times for
          // so many different purposes, but it's not used anywhere else right
          // here so why waste memory???
#ifdef _JK2MP
          iAnimLen = BG_AnimLength(rider->localAnimIndex, Anim);
#else
          iAnimLen =
              PM_AnimLength(pRider->client->clientInfo.animFileIndex, Anim);
#endif
          pVeh->m_iBoarding = level.time + iAnimLen;
          // Weird huh? Well I wanted to reuse flags and this should never be
          // set in an entity, so what the heck.
#ifdef _JK2MP
...
//===================================================================
      ...
      if (riderRightDot != 0.0f) {
        ...
        VectorScale(pVeh->m_pParentEntity->client->ps.velocity, 1.0f,
                    pVeh->m_pParentEntity->pos3);

        // Todo: Throw Old Vehicle Away From The New Vehicle Some
        //-------------------------------------------------------
        vec3_t toEnemy;
        VectorSubtract(pVeh->m_pParentEntity->currentOrigin,
                       rider->enemy->currentOrigin, toEnemy);
        ...
        //-----------------------------------
        Vehicle_t *enemyVeh = G_IsRidingVehicle(rider->enemy);
        enemyVeh->m_iBoarding = (riderRightDot > 0) ? (VEH_MOUNT_THROW_RIGHT)
      ...
      if (pVeh->m_pVehicleInfo->Eject(pVeh, pRider, qfalse)) {
        ...
        VectorScale(parent->client->ps.velocity, 0.5f,
                    rider->client->ps.velocity);
...
// Attachs all the riders of this vehicle to their appropriate tag (*driver,
// *pass1, *pass2, whatever...).
static void AttachRiders(Vehicle_t *pVeh) {
  ...
  if (pVeh->m_pOldPilot) {
    ...
    oldpilot->waypoint =
        parent->waypoint; // take the veh's waypoint as your own
  ...
  if (pVeh->m_pDroidUnit && pVeh->m_iDroidUnitTag != -1) {
    ...
    if (droid->client) {
      ...
      trap_G2API_GetBoltMatrix(
          parent->ghoul2, 0, pVeh->m_iDroidUnitTag, &boltMatrix, yawOnlyAngles,
          parent->currentOrigin, level.time, NULL, parent->modelScale);
      ...
      if (droid->NPC) {
        NPC_SetAnim(droid, SETANIM_BOTH, BOTH_STAND2,
                    (SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD));
  ...
  if (pVeh->m_pPilot) {
    ...
    // Get the driver tag.
    gi.G2API_GetBoltMatrix(
        parent->ghoul2, parent->playerModel, parent->crotchBolt, &boltMatrix,
...
void G_VehicleDamageBoxSizing(Vehicle_t *pVeh) {
  ...
  if (!parent->ghoul2 || !parent->m_pVehicle ||
      !parent->client) { // shouldn't have gotten in here then
    return;
  }
  ...
  // move the nose and back to opposite right/left, they will end up as our
  ...
  trap_Trace(&trace, parent->client->ps.origin, back, nose,
             parent->client->ps.origin, parent->s.number, parent->clipmask);
  if (!trace.allsolid && !trace.startsolid &&
      trace.fraction == 1.0f) { // all clear!
    VectorCopy(nose, parent->maxs);
    VectorCopy(back, parent->mins);
  } else { // oh well, DIE!
    // FIXME: does this give proper credit to the enemy who shot you down?
    G_Damage(parent, parent, parent, NULL, parent->client->ps.origin, 9999,
             DAMAGE_NO_PROTECTION, MOD_SUICIDE);
  }
}
...
int G_FlyVehicleImpactDir(gentity_t *veh, trace_t *trace) {
  ...
  trap_Trace(&localTrace, veh->client->ps.origin, testMins, testMaxs, fPos,
             veh->s.number, veh->clipmask);
  if (!localTrace.startsolid && !localTrace.allsolid &&
      localTrace.fraction == 1.0f) { // otherwise I guess it's not clear..
    noseClear = qtrue;
  }
  ...
  if (noseClear) { // if nose is clear check for tearing the wings off
    ...
    // can't get a real impact direction. First we'll trace forward and see if
    // the wings are colliding with anything, and if not, we'll fall back to
    // checking the trace plane normal.
    ...
    if (!(pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_E) ||
      ...
      trap_Trace(&localTrace, rWing, testMins, testMaxs, fPos, veh->s.number,
                 veh->clipmask);
      if (localTrace.startsolid || localTrace.allsolid ||
          localTrace.fraction != 1.0f) { // impact
    ...
    if (!(pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_C) ||
      ...
      trap_Trace(&localTrace, lWing, testMins, testMaxs, fPos, veh->s.number,
                 veh->clipmask);
      if (localTrace.startsolid || localTrace.allsolid ||
          localTrace.fraction != 1.0f) { // impact
...
extern void NPC_SetSurfaceOnOff(gentity_t *ent, const char *surfaceName,
                                int surfaceFlags); // NPC_utils.c
    ...
    if (shipSurf == SHIPSURF_BACK) { // destroy the droid if we have one
      if (veh->m_pVehicle && veh->m_pVehicle->m_pDroidUnit) { // we have one
        ...
        if (droidEnt &&
          ...
          droidEnt->flags &= ~FL_UNDYING;
          // blow it up
          G_Damage(droidEnt, veh->enemy, veh->enemy, NULL, NULL, 99999, 0,
                   MOD_UNKNOWN);
        }
      }
    }
...
void G_VehicleSetDamageLocFlags(gentity_t *veh, int impactDir, int deathPoint) {
  if (!veh->client) {
    ...
    if (veh->m_pVehicle && veh->m_pVehicle->m_pVehicleInfo &&
      ...
      float perc =
          ((float)veh->m_pVehicle->m_pVehicleInfo->malfunctionArmorLevel /
           (float)veh->m_pVehicle->m_pVehicleInfo->armor);
      if (perc > 0.99f) {
        perc = 0.99f;
      }
    ...
    } else if (veh->locationDamage[impactDir] <=
               heavyDamagePoint) { // heavy only
      G_SetVehDamageFlags(veh, impactDir, 2);
    } else if (veh->locationDamage[impactDir] <=
               lightDamagePoint) { // light only
...
qboolean G_FlyVehicleDestroySurface(gentity_t *veh, int surface) {
  ...
  case SHIPSURF_FRONT: // break the nose off
    ...
    numSurfs = 1;
    break;
  case SHIPSURF_BACK: // break both the bottom wings off for a backward impact I
                      // guess
    surfName[0] = "r_wing2";
    surfName[1] = "l_wing2";
    ...
    smashedBits = (SHIPSURF_BROKEN_A | SHIPSURF_BROKEN_B | SHIPSURF_BROKEN_D |
                   SHIPSURF_BROKEN_F);
  ...
  while (numSurfs > 0) { // use my silly system of automatically managing surf
                         // status on both client and server
    numSurfs--;
    NPC_SetSurfaceOnOff(veh, surfName[numSurfs], TURN_OFF);
  }

  if (!veh->m_pVehicle
           ->m_iRemovedSurfaces) {   // first time something got blown off
    if (veh->m_pVehicle->m_pPilot) { // make the pilot scream to his death
      G_EntitySound((gentity_t *)veh->m_pVehicle->m_pPilot, CHAN_VOICE,
...
void G_FlyVehicleSurfaceDestruction(gentity_t *veh, trace_t *trace,
                                    int magnitude, qboolean force) {
  int impactDir;
  int secondImpact;
  int deathPoint = -1;
  ...
  if (!veh->ghoul2 ||
      !veh->m_pVehicle) { // no g2 instance.. or no vehicle instance
    return;
  }
...
anotherImpact:
  ...
  if (deathPoint != -1) { // got a valid health value
    if (force && veh->locationDamage[impactDir] <
                     deathPoint) { // force that surf to be destroyed
      veh->locationDamage[impactDir] = deathPoint;
    }
    if (veh->locationDamage[impactDir] >= deathPoint) { // do it
      if (G_FlyVehicleDestroySurface(
              veh, impactDir)) { // actually took off a surface
  ...
  if (!alreadyRebroken) {
    ...
    if (impactDir !=
        secondImpact) { // can break off another piece in this same impact.. but
                        // only break off up to 2 at once
...
void G_VehUpdateShields(gentity_t *targ) {
  if (!targ || !targ->client || !targ->m_pVehicle ||
      !targ->m_pVehicle->m_pVehicleInfo) {
    return;
  }
  if (targ->m_pVehicle->m_pVehicleInfo->shields <=
      0) { // doesn't have shields, so don't have to send it
    return;
  }
  targ->client->ps.activeForcePass =
      floor(((float)targ->m_pVehicle->m_iShields /
             (float)targ->m_pVehicle->m_pVehicleInfo->shields) *
            10.0f);
}
#endif
...
void G_SetSharedVehicleFunctions(vehicleInfo_t *pVehInfo) {
  ...
  //	pVehInfo->ProcessMoveCommands			=
  //ProcessMoveCommands;
  //	pVehInfo->ProcessOrientCommands			=
