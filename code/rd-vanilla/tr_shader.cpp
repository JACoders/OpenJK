...
const byte stylesDefault[MAXLIGHTMAPS] = {LS_NORMAL, LS_NONE, LS_NONE, LS_NONE};
...
*timeOffset) { char		strippedName[MAX_QPATH]; int
hash; shader_t	*sh, *sh2; qhandle_t	h;

        sh = R_FindShaderByName( shaderName );
        if (sh == NULL || sh == tr.defaultShader) {
                h = RE_RegisterShaderLightMap(shaderName, lightmapsNone,
stylesDefault); sh = R_GetShaderByHandle(h);
        }
        if (sh == NULL || sh == tr.defaultShader) {
                ri.Printf( PRINT_WARNING, "WARNING: R_RemapShader: shader %s not
found\n", shaderName ); return;
        }

        sh2 = R_FindShaderByName( newShaderName );
        if (sh2 == NULL || sh2 == tr.defaultShader) {
                h = RE_RegisterShaderLightMap(newShaderName, lightmapsNone,
stylesDefault); sh2 = R_GetShaderByHandle(h);
        }

        if (sh2 == NULL || sh2 == tr.defaultShader) {
                ri.Printf( PRINT_WARNING, "WARNING: R_RemapShader: new shader %s
not found\n", newShaderName ); return;
        }
...
static void ParseSurfaceSpritesOptional(const char *param, const char *_text,
  ...
  if (!Q_stricmp(param, "ssFademax")) {
    ...
    if (value <= stage->ss->fadeDist) {
      ri.Printf(PRINT_WARNING,
                "WARNING: invalid surfacesprite fademax (%.2f <= "
  ...
  if (!Q_stricmp(param, "ssFXWeather")) {
    if (stage->ss->surfaceSpriteType != SURFSPRITE_EFFECT) {
      ri.Printf(PRINT_WARNING,
                "WARNING: weather applied to non-effect surfacesprite in "
                "shader '%s'\n",
                shader.name);
      return;
    }
    ...
    //
      ...
      if (!Q_stricmp(token, "$whiteimage")) {
        ...
        if (shader.lightmapIndex[0] < 0) {
          stage->bundle[0].image = tr.whiteImage;
#ifndef FINAL_BUILD
          // ri.Printf( PRINT_WARNING, "WARNING: $lightmap requested but none
          // available '%s'\n", shader.name );
#endif
        } else {
...
#define MAX_IMAGE_ANIMATIONS 32
      ...
      stage->bundle[0].image = (image_t *)R_Hunk_Alloc(
          stage->bundle[0].numImageAnimations * sizeof(image_t *), qfalse);
      ...
      stage->bundle[0].videoMapHandle = ri.CIN_PlayCinematic(
          token, 0, 0, 256, 256, (CIN_loop | CIN_silent | CIN_shader), NULL);
      ...
      if (!Q_stricmp(token, "wave")) {
        ...
        if (shader.lightmapIndex[0] == LIGHTMAP_NONE) {
          ri.Printf(PRINT_ERROR,
                    "ERROR: rgbGen vertex used on a model! in shader '%s'\n",
                    shader.name);
        }
        ...
        if (shader.lightmapIndex[0] != LIGHTMAP_NONE) {
          ri.Printf(PRINT_ERROR,
                    "ERROR: rgbGen lightingDiffuse used on a misc_model! in "
                    "shader '%s'\n",
                    shader.name);
        }
        ...
        if (shader.lightmapIndex[0] != LIGHTMAP_NONE) {
          ri.Printf(PRINT_ERROR,
                    "ERROR: rgbGen lightingDiffuseEntity used on a misc_model! "
                    "in shader '%s'\n",
                    shader.name);
        }
        ...
        ri.Printf(PRINT_ERROR,
                  "ERROR: unknown rgbGen parameter '%s' in shader '%s'\n",
                  token, shader.name);
        continue;
      }
    }
    ...
    //
      ...
      if (!Q_stricmp(token, "environment")) {
        ...
        stage->bundle[0].tcGenVectors =
            (vec3_t *)R_Hunk_Alloc(2 * sizeof(vec3_t), qfalse);
...
static void ParseDeform(const char **text) {
  ...
  shader.deforms[shader.numDeforms] =
      (deformStage_t *)R_Hunk_Alloc(sizeof(deformStage_t), qtrue);
...
static shader_t *GeneratePermanentShader(void) {
  ...
  if (tr.numShaders == MAX_SHADERS) {
    ...
    ri.Printf(PRINT_WARNING,
              "WARNING: GeneratePermanentShader - MAX_SHADERS (%d) hit "
              "(overflowed by %d)\n",
    ...
    for (b = 0; b < NUM_TEXTURE_BUNDLES; b++) {
      if (newShader->stages[i].bundle[b].numTexMods) {
        ...
        newShader->stages[i].bundle[b].texMods =
            (texModInfo_t *)R_Hunk_Alloc(size, qfalse);
...
static int VertexLightingCollapse(void) {
  ...
  if (shader.sort == SS_OPAQUE) {
    ...
    for (stage = 0; stage < MAX_SHADER_STAGES; stage++) {
      ...
      // SurfaceSprites are most certainly NOT desireable as the collapsed
      // surface texture.
  ...
  for (stage = 1, nextopenstage = 1; stage < MAX_SHADER_STAGES; stage++) {
    ...
    if (pStage->ss && pStage->ss->surfaceSpriteType) {
      // Copy this stage to the next open stage list (that is, we don't want any
      // inactive stages before this one)
...
static inline const int *R_FindLightmap(const int *lightmapIndex) {
  ...
  Com_sprintf(fileName, sizeof(fileName), "%s/" EXTERNAL_LIGHTMAP, tr.worldDir,
              *lightmapIndex);
  image = R_FindImageFile(fileName, qfalse, qfalse,
...
shader_t *R_FindShader(const char *name, const int *lightmapIndex,
  ...
  */
  ...
  for (sh = sh_hashTable[hash]; sh; sh = sh->next) {
    ...
    // every call to R_FindShader with that same strippedName a new default
    // shader is created.
    if (IsShader(sh, strippedName, lightmapIndex, styles)) { // match found
      return sh;
    }
  }
  ...
  //
  image = R_FindImageFile(name, mipRawImage, mipRawImage, qtrue,
                          mipRawImage ? GL_REPEAT : GL_CLAMP);
  if (!image) {
    if (strncmp(name, "levelshots", 10) &&
        strcmp(name, "*off")) { // hide these warnings
  ...
  if (shader.lightmapIndex[0] == LIGHTMAP_NONE) {
    ...
    stages[0].rgbGen = CGEN_IDENTITY; // lightmaps are scaled on creation
                                      // for identitylight
                                      // light map 0 should always be style 0,
                                      // which means that this will always be on
    stages[0].stateBits = GLS_DEFAULT;
...
#ifdef USE_STL_FOR_SHADER_LOOKUPS
// setup my STL shortcut list as to where all the shaders are, saves re-parsing
// every line for every .TGA request.
//
static void SetupShaderEntryPtrs(void) {
  ...
  while (1) {
    ...
    if (token[0] == '{') // '}'	// counterbrace for matching
    {
      ...
      Q_strlwr(token); // token is always a ptr to com_token here, not the
                       // original buffer.
                       //	(Not that it matters, except for reasons of
                       //speed by not strlwr'ing the whole buffer)

      // token = a string of this shader name, p = ptr within s_shadertext it's
      // found at, so store it...
      //
      ShaderEntryPtrs_Insert(token, p);
      SkipRestOfLine(
          &p); // now legally skip over this name and go get the next one
    }
  }

  COM_EndParseSession();

  // ri.Printf( PRINT_DEVELOPER, "SetupShaderEntryPtrs(): Stored %d shader
