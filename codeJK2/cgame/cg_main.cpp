...
const char *CG_DisplayBoxedText(int iBoxX, int iBoxY, int iBoxWidth,
                                int iBoxHeight, const char *psText,
...
const char *inv_names[] = {
    "ELECTROBINOCULARS", "BACTA CANISTER", "SEEKER",       "LIGHT AMP GOGGLES",
    "ASSAULT SENTRY",    "GOODIE KEY",     "GOODIE KEY",   "GOODIE KEY",
    "GOODIE KEY",        "GOODIE KEY",     "SECURITY KEY", "SECURITY KEY",
    "SECURITY KEY",      "SECURITY KEY",   "SECURITY KEY",
};
...
================
*/
extern "C" Q_EXPORT intptr_t vmMain(intptr_t command, intptr_t arg0,
...
vmCvar_t cg_VariantSoundCap; // 0 = no capping, else cap to (n) max (typically
                             // just 1, but allows more)
...
static cvarTable_t cvarTable[] = {
    ...
    {&cg_crosshairIdentifyTarget, "cg_crosshairIdentifyTarget", "1",
     CVAR_ARCHIVE},
    {&cg_crosshairForceHint, "cg_crosshairForceHint", "1",
...
void CG_RegisterCvars(void) {
  ...
  for (i = 0, cv = cvarTable; i < cvarTableSize; i++, cv++) {
    cgi_Cvar_Register(cv->vmCvar, cv->cvarName, cv->defaultString,
                      cv->cvarFlags);
  }
}
...
int CG_GetCameraPos(vec3_t camerapos) {
  if (in_camera) {
    ...
    return 1;
  } else if (cg_entities[0].gent && cg_entities[0].gent->client &&
             cg_entities[0].gent->client->ps.viewEntity > 0 &&
             cg_entities[0].gent->client->ps.viewEntity < ENTITYNUM_WORLD)
  // else if ( cg.snap && cg.snap->ps.viewEntity > 0 && cg.snap->ps.viewEntity <
  // ENTITYNUM_WORLD )
  { // in an entity camera view
    if (g_entities[cg_entities[0].gent->client->ps.viewEntity].client &&
        cg.renderingThirdPerson) {
      VectorCopy(g_entities[cg_entities[0].gent->client->ps.viewEntity]
                     .client->renderInfo.eyePoint,
                 camerapos);
    } else {
      VectorCopy(
          g_entities[cg_entities[0].gent->client->ps.viewEntity].currentOrigin,
          camerapos);
    }
    // VectorCopy(
    // cg_entities[cg_entities[0].gent->client->ps.viewEntity].lerpOrigin,
    // camerapos );
    /*
    if ( g_entities[cg.snap->ps.viewEntity].client && cg.renderingThirdPerson )
    {
            VectorCopy(
    g_entities[cg.snap->ps.viewEntity].client->renderInfo.eyePoint, camerapos );
    }
    else
    {//use the g_ent because it may not have gotten over to the client yet...
            VectorCopy( g_entities[cg.snap->ps.viewEntity].currentOrigin,
    camerapos );
    }
    */
    return 1;
  } else if (cg.renderingThirdPerson) { // in third person
    // FIXME: what about hacks that render in third person regardless of this
    // value?
    VectorCopy(cg.refdef.vieworg, camerapos);
    return 1;
  } else if (cg.snap && (cg.snap->ps.weapon == WP_SABER ||
                         cg.snap->ps.weapon ==
                             WP_MELEE)) // implied: !cg.renderingThirdPerson
...
static void CG_AS_Register(void) {
  ...
  // FIXME: Don't ask... I had to get around a really nasty MS error in the
  // templates with this...
...
static void CG_RegisterSounds(void) {
  ...
  //	cgs.media.useNothingSound = cgi_S_RegisterSound(
  //"sound/items/use_nothing.wav" );

  cgs.media.noAmmoSound = cgi_S_RegisterSound("sound/weapons/noammo.wav");
  //	cgs.media.talkSound = 	cgi_S_RegisterSound(
  //"sound/interface/communicator.wav" );
  ...
  cgs.media.overchargeFastSound =
      cgi_S_RegisterSound("sound/weapons/overchargeFast.wav");
  cgs.media.overchargeSlowSound =
      cgi_S_RegisterSound("sound/weapons/overchargeSlow.wav");
  cgs.media.overchargeLoopSound =
      cgi_S_RegisterSound("sound/weapons/overchargeLoop.wav");
  cgs.media.overchargeEndSound =
      cgi_S_RegisterSound("sound/weapons/overchargeEnd.wav");

  cgs.media.batteryChargeSound =
      cgi_S_RegisterSound("sound/interface/pickup_battery.wav");

  //	cgs.media.tedTextSound = cgi_S_RegisterSound(
  //"sound/interface/tedtext.wav" );
  cgs.media.messageLitSound = cgi_S_RegisterSound("sound/interface/update");
  cg.messageLitActive = qfalse;

  //	cgs.media.interfaceSnd1 = cgi_S_RegisterSound(
  //"sound/interface/button4.wav" ); 	cgs.media.interfaceSnd2 =
  //cgi_S_RegisterSound( "sound/interface/button2.wav" );
  //	cgs.media.interfaceSnd3 = cgi_S_RegisterSound(
  ...
  for (i = 0; i < 4; i++) {
    Com_sprintf(name, sizeof(name), "sound/player/footsteps/stone_step%i.wav",
                i + 1);
    cgs.media.footsteps[FOOTSTEP_NORMAL][i] = cgi_S_RegisterSound(name);

    Com_sprintf(name, sizeof(name), "sound/player/footsteps/metal_step%i.wav",
                i + 1);
    cgs.media.footsteps[FOOTSTEP_METAL][i] = cgi_S_RegisterSound(name);

    Com_sprintf(name, sizeof(name), "sound/player/footsteps/water_run%i.wav",
                i + 1);
    cgs.media.footsteps[FOOTSTEP_SPLASH][i] = cgi_S_RegisterSound(name);

    Com_sprintf(name, sizeof(name), "sound/player/footsteps/water_walk%i.wav",
                i + 1);
    cgs.media.footsteps[FOOTSTEP_WADE][i] = cgi_S_RegisterSound(name);

    Com_sprintf(name, sizeof(name), "sound/player/footsteps/water_wade_0%i.wav",
                i + 1);
  ...
  for (i = 1; i < bg_numItems; i++) {
    if (items[i] ==
        '1') // even with sound pooling, don't clutter it for low end machines
    {
...
qhandle_t CG_RegisterHeadSkin(const char *headModelName,
                              const char *headSkinName, qboolean *extensions) {
...
==========================
*/
qboolean CG_RegisterClientSkin(clientInfo_t *ci, const char *headModelName,
                               const char *headSkinName,
                               const char *torsoModelName,
                               const char *torsoSkinName,
                               const char *legsModelName,
                               const char *legsSkinName) {
  char hfilename[MAX_QPATH];
  ...
  if (!ci->legsSkin) {
    //		Com_Printf( "Failed to load skin file: %s : %s\n", legsModelName,
    //legsSkinName ); return qfalse;
  }

  if (torsoModelName && torsoSkinName && torsoModelName[0] &&
      torsoSkinName[0]) {
    Com_sprintf(tfilename, sizeof(tfilename), "models/players/%s/upper_%s.skin",
...
==========================
*/
qboolean CG_RegisterClientModelname(clientInfo_t *ci, const char *headModelName,
                                    const char *headSkinName,
                                    const char *torsoModelName,
                                    const char *torsoSkinName,
                                    const char *legsModelName,
...
*/
  ...
  if (!CG_RegisterClientSkin(ci, headModelName, headSkinName, torsoModelName,
                             torsoSkinName, legsModelName, legsSkinName)) {
  ...
  // FIXME: for now, uses the legs model dir for anim cfg, but should we set
  // this in some sort of NPCs.cfg?
  // load the animation file set
  if (!G_ParseAnimFileSet(legsModelName, legsModelName, &ci->animFileIndex)) {
    Com_Printf(S_COLOR_RED
               "Failed to load animation file set models/players/%s\n",
               legsModelName);
    return qfalse;
  }
#endif
...
void CG_RegisterClientRenderInfo(clientInfo_t *ci, renderInfo_t *ri) {
  ...
  if (!CG_RegisterClientModelname(ci, headModelName, headSkinName,
                                  torsoModelName, torsoSkinName, legsModelName,
                                  legsSkinName)) {
    if (!CG_RegisterClientModelname(ci, DEFAULT_HEADMODEL, "default",
                                    DEFAULT_TORSOMODEL, "default",
...
void CG_RegisterClientModels(int entityNum) {
  ...
  CG_RegisterClientRenderInfo(&ent->client->clientInfo,
...
static void CG_RegisterGraphics(void) {
  ...
  static char *sb_nums[11] = {
      "gfx/2d/numbers/zero",  "gfx/2d/numbers/one",   "gfx/2d/numbers/two",
      "gfx/2d/numbers/three", "gfx/2d/numbers/four",  "gfx/2d/numbers/five",
      "gfx/2d/numbers/six",   "gfx/2d/numbers/seven", "gfx/2d/numbers/eight",
      "gfx/2d/numbers/nine",  "gfx/2d/numbers/minus",
  };

  static char *sb_t_nums[11] = {
      "gfx/2d/numbers/t_zero",  "gfx/2d/numbers/t_one",
      "gfx/2d/numbers/t_two",   "gfx/2d/numbers/t_three",
      "gfx/2d/numbers/t_four",  "gfx/2d/numbers/t_five",
      "gfx/2d/numbers/t_six",   "gfx/2d/numbers/t_seven",
      "gfx/2d/numbers/t_eight", "gfx/2d/numbers/t_nine",
      "gfx/2d/numbers/t_minus",
  };

  static char *sb_c_nums[11] = {
      "gfx/2d/numbers/c_zero",  "gfx/2d/numbers/c_one",
      "gfx/2d/numbers/c_two",   "gfx/2d/numbers/c_three",
      "gfx/2d/numbers/c_four",  "gfx/2d/numbers/c_five",
      "gfx/2d/numbers/c_six",   "gfx/2d/numbers/c_seven",
      "gfx/2d/numbers/c_eight", "gfx/2d/numbers/c_nine",
      "gfx/2d/numbers/t_minus", //?????
  };
  ...
  // FIXME: conditionally do this??  Something must be wrong with inventory item
  // caching..?
  cgi_R_RegisterModel("models/items/remote.md3");

  cgs.media.explosionModel =
      cgi_R_RegisterModel("models/weaphits/explosion.md3");
  ...
  // on players
  cgs.media.personalShieldShader =
      cgi_R_RegisterShader("gfx/misc/personalshield");
  ...
  cgi_R_RegisterShader(
      "gfx/effects/irid_shield"); // for galak, but he doesn't have his own
                                  // weapon so I can't register the shader
                                  // there.

  // interface
  for (i = 0; i < NUM_CROSSHAIRS; i++) {
    cgs.media.crosshairShader[i] =
        cgi_R_RegisterShaderNoMip(va("gfx/2d/crosshair%c", 'a' + i));
  }
  ...
  cgs.media.weaponIconBackground =
      cgi_R_RegisterShaderNoMip("gfx/hud/background");
  ...
  cgs.media.forceIconBackground =
      cgi_R_RegisterShaderNoMip("gfx/hud/background_f");
  cgs.media.inventoryIconBackground =
      cgi_R_RegisterShaderNoMip("gfx/hud/background_i");
  ...
  // Chunk models
  // FIXME: jfm:? bother to conditionally load these if an ent has this material
  // type?
  for (i = 0; i < NUM_CHUNK_MODELS; i++) {
    cgs.media.chunkModels[CHUNK_METAL2][i] = cgi_R_RegisterModel(
        va("models/chunks/metal/metal1_%i.md3", i + 1)); //_ /switched\ _
    cgs.media.chunkModels[CHUNK_METAL1][i] = cgi_R_RegisterModel(
        va("models/chunks/metal/metal2_%i.md3", i + 1)); //  \switched/
    cgs.media.chunkModels[CHUNK_ROCK1][i] =
        cgi_R_RegisterModel(va("models/chunks/rock/rock1_%i.md3", i + 1));
    cgs.media.chunkModels[CHUNK_ROCK2][i] =
        cgi_R_RegisterModel(va("models/chunks/rock/rock2_%i.md3", i + 1));
    cgs.media.chunkModels[CHUNK_ROCK3][i] =
        cgi_R_RegisterModel(va("models/chunks/rock/rock3_%i.md3", i + 1));
    ...
    cgs.media.chunkModels[CHUNK_WHITE_METAL][i] =
        cgi_R_RegisterModel(va("models/chunks/metal/wmetal1_%i.md3", i + 1));
  }

  cgs.media.chunkSound =
      cgi_S_RegisterSound("sound/weapons/explosions/glasslcar");
  ...
  cgs.media.rockBounceSound[0] =
      cgi_S_RegisterSound("sound/effects/stone_bounce");
  cgs.media.rockBounceSound[1] =
      cgi_S_RegisterSound("sound/effects/stone_bounce2");
  cgs.media.metalBounceSound[0] =
      cgi_S_RegisterSound("sound/effects/metal_bounce");
  cgs.media.metalBounceSound[1] =
      cgi_S_RegisterSound("sound/effects/metal_bounce2");
  cgs.media.glassChunkSound =
      cgi_S_RegisterSound("sound/weapons/explosions/glassbreak1");
  cgs.media.crateBreakSound[0] =
      cgi_S_RegisterSound("sound/weapons/explosions/crateBust1");
  cgs.media.crateBreakSound[1] =
      cgi_S_RegisterSound("sound/weapons/explosions/crateBust2");

  cgs.media.weaponbox = cgi_R_RegisterShaderNoMip("gfx/interface/weapon_box");

  // Models & Shaders
  cgs.media.damageBlendBlobShader =
      cgi_R_RegisterShader("gfx/misc/borgeyeflare");

  cg.loadLCARSStage = 6;

  //	cgs.media.HUDLeftFrame= cgi_R_RegisterShaderNoMip(
  //"gfx/hud/hudleftframe" );
  cgs.media.HUDLeftFrame = cgi_R_RegisterShaderNoMip("gfx/hud/static_test");
  cgs.media.HUDInnerLeft =
      cgi_R_RegisterShaderNoMip("gfx/hud/hudleft_innerframe");
  ...
  cgs.media.HUDInnerRight =
      cgi_R_RegisterShaderNoMip("gfx/hud/hudright_innerframe");
  ...
  cgs.media.messageObjCircle =
      cgi_R_RegisterShaderNoMip("gfx/hud/objective_circle");
  ...
  for (i = 1; i < bg_numItems; i++) {
    ...
    if (bg_itemlist[i].giType == IT_HOLDABLE) {
      if (bg_itemlist[i].giTag < INV_MAX) {
        inv_icons[bg_itemlist[i].giTag] =
            cgi_R_RegisterShaderNoMip(bg_itemlist[i].icon);
      }
    }
  }
  ...
  //	cgs.media.bulletmarksShader				= cgi_R_RegisterShader(
  ...
  // with different bulge offsets and different texture scales, it has a much
  // more chaotic look
  cgs.media.electricBodyShader = cgi_R_RegisterShader("gfx/misc/electric");
  cgs.media.electricBody2Shader =
      cgi_R_RegisterShader("gfx/misc/fullbodyelectric2");
  ...
  for (i = 1; i < MAX_MODELS; i++) {
    ...
    //		OutputDebugString(va("### CG_RegisterGraphics(): cgs.model_draw[%d] =
    //\"%s\"\n",i,modelName));
  }
  ...
  for (i = 0; i < ENTITYNUM_WORLD; i++) {
    if (&g_entities[i]) {
      if (g_entities[i].client) {
        ...
        {
          CG_LoadingString(
              va("client %s", g_entities[i].client->clientInfo.name));
      ...
      } else if (g_entities[i].svFlags & SVF_NPC_PRECACHE &&
                 g_entities[i].NPC_type &&
                 g_entities[i].NPC_type[0]) { // Precache the NPC_type
...
static void CG_GameStateReceived(void) {
  ...
  iCGResetCount++;
  if (iCGResetCount == 1) // this will only equal 1 first time, after each
                          // vid_restart it just gets higher.
  { //	This non-clear is so the user can vid_restart during scrolling text
    //without losing it.
  ...
  */
  ...
  /*	cgs.charScale = cgs.glconfig.vidHeight * (1.0/480.0);
          if ( cgs.glconfig.vidWidth * 480 > cgs.glconfig.vidHeight * 640 ) {
                  // wide screen
                  cgs.bias = 0.5 * ( cgs.glconfig.vidWidth - (
...
void CG_WriteTheEvilCGHackStuff(void) {
  ...
  saved_game.write_chunk<int32_t>(INT_ID('F', 'P', 'S', 'L'),
                                  ::cg.forcepowerSelect);

  saved_game.write_chunk<int32_t>(INT_ID('I', 'V', 'S', 'L'),
                                  ::cg.inventorySelect);
}
...
void CG_ReadTheEvilCGHackStuff(void) {
  ...
  saved_game.read_chunk<int32_t>(INT_ID('F', 'P', 'S', 'L'),
                                 ::gi_cg_forcepowerSelect);

  saved_game.read_chunk<int32_t>(INT_ID('I', 'V', 'S', 'L'),
...
// in the active snap shots
void CG_Init_CG(void) { memset(&cg, 0, sizeof(cg)); }

// initialise the cg_entities structure - take into account the ghoul2 stl stuff
void CG_Init_CGents(void) { memset(cg_entities, 0, sizeof(cg_entities)); }
...
Called when DLL loads (after subsystem restart, but before gamestate is
received)
=================
*/
void CG_PreInit() {
...
void CG_Init(int serverCommandSequence) {
  ...
  static char *force_icon_files[NUM_FORCE_POWERS] = {
      "gfx/hud/f_icon_heal",         "gfx/hud/f_icon_levitation",
      "gfx/hud/f_icon_speed",        "gfx/hud/f_icon_push",
      "gfx/hud/f_icon_pull",         "gfx/hud/f_icon_telepathy",
      "gfx/hud/f_icon_grip",         "gfx/hud/f_icon_l1",
      "gfx/hud/f_icon_saber_throw",  "gfx/hud/f_icon_saber_defend",
...
void CG_DrawEdge(vec3_t start, vec3_t end, int type) {
  switch (type) {
  case EDGE_PATH:
    FX_AddLine(start, end, 4.0f, 4.0f, 0.0f, 1.0f, 1.0f, 51,
               cgi_R_RegisterShader("gfx/misc/nav_arrow"), 0);
    break;

  case EDGE_NORMAL:
    FX_AddLine(start, end, 8.0f, 4.0f, 0.0f, 0.5f, 0.5f, 51,
...
void CG_DrawCombatPoint(vec3_t origin, int type) {
  ...
  /*
          switch( type )
          {
          case 0:	//FIXME: To shut up the compiler warning (more will be
     added here later of course) default: FX_AddSprite( origin, NULL,
     NULL, 8.0f, 0.0f, 1.0f, 1.0f, color, color, 0.0f, 0.0f, 51,
     cgi_R_RegisterShader( "gfx/misc/nav_cpoint" ) ); break;
          }
  */
}
...
void CG_DrawAlert(vec3_t origin, float rating) {
  ...
  FX_AddSprite(drawPos, NULL, NULL, 16, 0.0f, 1.0f, 1.0f, startRGB, startRGB, 0,
               0, 50, cgs.media.whiteShader);
}
...
void CG_ParseMenu(const char *menuFile) {
  ...
  if (!result) {
    Com_Printf(
        "Unable to load hud menu file: %s. Using default ui/testhud.menu.\n",
        menuFile);
...
void CG_LoadMenus(const char *menuFile) {
  ...
  if (len >= MAX_MENUDEFFILE) {
    cgi_FS_FCloseFile(f);
    cgi_Error(va(S_COLOR_RED "menu file too large: %s is %i, max allowed is %i",
...
static void SetInventoryTime(void) {
  if (((cg.weaponSelectTime + WEAPON_SELECT_TIME) >
       cg.time) || // The Weapon HUD was currently active to just swap it out
                   // with Force HUD
      ((cg.forcepowerSelectTime + WEAPON_SELECT_TIME) >
       cg.time)) // The Force HUD was currently active to just swap it out with
                 // Force HUD
  {
...
void CG_DPPrevInventory_f(void) {
  ...
  for (i = 0; i < INV_MAX; i++) {
    ...
    if ((cg.DataPadInventorySelect < INV_ELECTROBINOCULARS) ||
        (cg.DataPadInventorySelect >= INV_MAX)) {
...
void CG_NextInventory_f(void) {
  ...
  // The first time it's been hit so just show inventory but don't advance in
  // inventory.
  ...
  for (i = 0; i < INV_MAX; i++) {
    ...
    if ((cg.inventorySelect < INV_ELECTROBINOCULARS) ||
        (cg.inventorySelect >= INV_MAX)) {
      cg.inventorySelect = INV_ELECTROBINOCULARS;
    }

    if (CG_InventorySelectable(cg.inventorySelect) &&
  ...
  for (i = 1; i < bg_numItems; i++) {
    if (bg_itemlist[i].giTag == tag &&
        bg_itemlist[i].giType ==
            IT_HOLDABLE) // I guess giTag's aren't unique amongst items..must
                         // also make sure it's a holdable
    {
...
void CG_DrawInventorySelect(void) {
  ...
  if ((cg.inventorySelectTime + WEAPON_SELECT_TIME) <
  ...
  if (!count) {
    ...
    x = (SCREEN_WIDTH - w) / 2;
    CG_DrawProportionalString(x, y2 + 22, text, CG_CENTER | CG_SMALLFONT,
  ...
  if (inv_icons[cg.inventorySelect]) {
    ...
    CG_DrawPic(x - (bigIconSize / 2),
               ...
               bigIconSize, inv_icons[cg.inventorySelect]);
    ...
    if (inv_names[cg.inventorySelect]) {
      // FIXME: This is ONLY a temp solution, the icon stuff, etc, should all
      // just use items.dat for everything
      gitem_t *item = FindInventoryItemTag(cg.inventorySelect);

      if (item && item->classname && item->classname[0]) {
        char itemName[256], data[1024]; // FIXME: do these really need to be
                                        // this large??  does it matter?
        ...
        if (cgi_SP_GetStringTextString(itemName, data, sizeof(data))) {
          ...
          cgi_R_Font_DrawString(x, (SCREEN_HEIGHT - 24), data, textColor,
                                cgs.media.qhFontSmall, -1, 1.0f);
        }
      }
      //			if (tag)
      //			{
      //				CG_DrawProportionalString(320, y + 53,
      //inv_names[cg.inventorySelect], CG_CENTER | CG_SMALLFONT,
      //colorTable[CT_ICON_BLUE]); 				CG_DrawProportionalString(320, y + 53,
      //bg_itemlist[i].pickup_name, CG_CENTER | CG_SMALLFONT,
      //colorTable[CT_ICON_BLUE]);
      //			}
    }
  }
...
char *inventoryDesc[15] = {
    "NEURO_SAAV_DESC",      "BACTA_DESC",           "INQUISITOR_DESC",
    "LA_GOGGLES_DESC",      "PORTABLE_SENTRY_DESC", "GOODIE_KEY_DESC",
...
void CG_DrawDataPadInventorySelect(void) {
  ...
  if (!count) {
    ...
    int w = cgi_R_Font_StrLenPixels(text, cgs.media.qhFontSmall, 1.0f);
    x = (SCREEN_WIDTH - w) / 2;
    CG_DrawProportionalString(x, 300 + 22, text, CG_CENTER | CG_SMALLFONT,
                              colorTable[CT_ICON_BLUE]);
    return;
  }
  ...
  if (inv_icons[cg.DataPadInventorySelect]) {
    ...
    CG_DrawPic(x - (bigIconSize / 2),
               (y - ((bigIconSize - smallIconSize) / 2)) + 10, bigIconSize,
               bigIconSize, inv_icons[cg.DataPadInventorySelect]);
    addX = (float)bigIconSize * .75;
    cgi_R_SetColor(colorTable[CT_ICON_BLUE]);
    CG_DrawNumField((x - (bigIconSize / 2)) + addX, y, 2,
  ...
  if ((cg.DataPadInventorySelect >= 0) && (cg.DataPadInventorySelect < 13)) {
    cgi_SP_GetStringTextString(
        va("INGAME_%s", inventoryDesc[cg.DataPadInventorySelect]), text,
        sizeof(text));

    if (text[0]) {
      CG_DisplayBoxedText(70, 50, 500, 300, text, cgs.media.qhFontSmall, 0.7f,
                          textColor);
    }
  }
}
...
void SetForcePowerTime(void) {
  if (((cg.weaponSelectTime + WEAPON_SELECT_TIME) >
       cg.time) || // The Weapon HUD was currently active to just swap it out
                   // with Force HUD
      ((cg.inventorySelectTime + WEAPON_SELECT_TIME) >
       cg.time)) // The Inventory HUD was currently active to just swap it out
                 // with Force HUD
  {
    cg.weaponSelectTime = 0;
...
int showPowers[MAX_SHOWPOWERS] = {FP_HEAL,      FP_SPEED, FP_PUSH,     FP_PULL,
                                  FP_TELEPATHY, FP_GRIP,  FP_LIGHTNING};
char *showPowersName[MAX_SHOWPOWERS] = {
    "HEAL2", "SPEED2", "PUSH2", "PULL2", "MINDTRICK2", "GRIP2", "LIGHTNING2",
};

int showDataPadPowers[MAX_DPSHOWPOWERS] = {
    FP_HEAL,       FP_LEVITATION,    FP_SPEED,         FP_PUSH,
    FP_PULL,       FP_TELEPATHY,     FP_GRIP,          FP_LIGHTNING,
    FP_SABERTHROW, FP_SABER_DEFENSE, FP_SABER_OFFENSE,
};
...
qboolean ForcePower_Valid(int index) {
  ...
  if (player->client->ps.forcePowersKnown & (1 << showPowers[index]) &&
      player->client->ps
          .forcePowerLevel[showPowers[index]]) // Does he have the force power?
  {
    return qtrue;
  }
...
void CG_DrawForceSelect(void) {
  ...
  if ((cg.forcepowerSelectTime + WEAPON_SELECT_TIME) <
      cg.time) // Time is up for the HUD to display
  {
    return;
  }
  ...
  */
  ...
  for (iconCnt = 1; iconCnt < (sideLeftIconCnt + 1); i--) {
    ...
    if (force_icons[showPowers[i]]) {
      CG_DrawPic(holdX, y, smallIconSize, smallIconSize,
  ...
  if (force_icons[showPowers[cg.forcepowerSelect]]) {
    CG_DrawPic(x - (bigIconSize / 2), (y - ((bigIconSize - smallIconSize) / 2)),
               bigIconSize, bigIconSize,
               force_icons[showPowers[cg.forcepowerSelect]]); // only cache the
                                                              // icon for display
  }
  ...
  for (iconCnt = 1; iconCnt < (sideRightIconCnt + 1); i++) {
    ...
    if (force_icons[showPowers[i]]) {
      CG_DrawPic(holdX, y, smallIconSize, smallIconSize,
                 force_icons[showPowers[i]]); // only cache the icon for display
      holdX += (smallIconSize + pad);
    }
  }
  ...
  if (cgi_SP_GetStringTextString(
          va("INGAME_%s", showPowersName[cg.forcepowerSelect]), text,
          sizeof(text))) {
    int w = cgi_R_Font_StrLenPixels(text, cgs.media.qhFontSmall, 1.0f);
    int x = (SCREEN_WIDTH - w) / 2;
    cgi_R_Font_DrawString(x, (SCREEN_HEIGHT - 24), text,
                          colorTable[CT_ICON_BLUE], cgs.media.qhFontSmall, -1,
                          1.0f);
  }
}
...
qboolean ForcePowerDataPad_Valid(int index) {
  ...
  if (player->client->ps.forcePowersKnown & (1 << showDataPadPowers[index]) &&
      player->client->ps
          .forcePowerLevel[showDataPadPowers[index]]) // Does he have the force
                                                      // power?
  {
    return qtrue;
  }
...
void CG_DPNextForcePower_f(void) {
  ...
  for (i = 0; i < MAX_DPSHOWPOWERS; i++) {
    ...
    if (ForcePowerDataPad_Valid(
            cg.DataPadforcepowerSelect)) // Does he have the force power?
    {
      return;
    }
  }
...
char *forcepowerDesc[NUM_FORCE_POWERS] = {
    "FORCE_HEAL_DESC",          "FORCE_JUMP_DESC",
    "FORCE_SPEED_DESC",         "FORCE_PUSH_DESC",
    "FORCE_PULL_DESC",          "FORCE_MIND_TRICK_DESC",
    "FORCE_GRIP_DESC",          "FORCE_LIGHTNING_DESC",
    "FORCE_SABER_THROW_DESC",   "FORCE_SABER_DEFENSE_DESC",
    "FORCE_SABER_OFFENSE_DESC",
};

char *forcepowerLvl1Desc[NUM_FORCE_POWERS] = {
    "FORCE_HEAL_LVL1_DESC",          "FORCE_JUMP_LVL1_DESC",
    "FORCE_SPEED_LVL1_DESC",         "FORCE_PUSH_LVL1_DESC",
    "FORCE_PULL_LVL1_DESC",          "FORCE_MIND_TRICK_LVL1_DESC",
    "FORCE_GRIP_LVL1_DESC",          "FORCE_LIGHTNING_LVL1_DESC",
    "FORCE_SABER_THROW_LVL1_DESC",   "FORCE_SABER_DEFENSE_LVL1_DESC",
    "FORCE_SABER_OFFENSE_LVL1_DESC",
};

char *forcepowerLvl2Desc[NUM_FORCE_POWERS] = {
    "FORCE_HEAL_LVL2_DESC",          "FORCE_JUMP_LVL2_DESC",
    "FORCE_SPEED_LVL2_DESC",         "FORCE_PUSH_LVL2_DESC",
    "FORCE_PULL_LVL2_DESC",          "FORCE_MIND_TRICK_LVL2_DESC",
    "FORCE_GRIP_LVL2_DESC",          "FORCE_LIGHTNING_LVL2_DESC",
...
char *forcepowerLvl3Desc[NUM_FORCE_POWERS] = {
    "FORCE_HEAL_LVL3_DESC",          "FORCE_JUMP_LVL3_DESC",
    "FORCE_SPEED_LVL3_DESC",         "FORCE_PUSH_LVL3_DESC",
    "FORCE_PULL_LVL3_DESC",          "FORCE_MIND_TRICK_LVL3_DESC",
    "FORCE_GRIP_LVL3_DESC",          "FORCE_LIGHTNING_LVL3_DESC",
    "FORCE_SABER_THROW_LVL3_DESC",   "FORCE_SABER_DEFENSE_LVL3_DESC",
...
void CG_DrawDataPadForceSelect(void) {
  ...
  for (iconCnt = 1; iconCnt < (sideLeftIconCnt + 1); i--) {
    ...
    if (force_icons[showDataPadPowers[i]]) {
      CG_DrawPic(holdX, y, smallIconSize, smallIconSize,
                 force_icons[showDataPadPowers[i]]);
    }
  ...
  if (force_icons[showDataPadPowers[cg.DataPadforcepowerSelect]]) {
    ...
    CG_DrawPic(
        x - (bigIconSize / 2), (y - ((bigIconSize - smallIconSize) / 2)),
        bigIconSize, bigIconSize,
        force_icons
            [showDataPadPowers[cg.DataPadforcepowerSelect]]); // only cache the
                                                              // icon for display

    // New force power
    if (((cg_updatedDataPadForcePower1.integer - 1) ==
         showDataPadPowers[cg.DataPadforcepowerSelect]) ||
        ((cg_updatedDataPadForcePower2.integer - 1) ==
         showDataPadPowers[cg.DataPadforcepowerSelect]) ||
        ((cg_updatedDataPadForcePower3.integer - 1) ==
         showDataPadPowers[cg.DataPadforcepowerSelect])) {
      CG_DrawPic(x - (bigIconSize / 2),
                 (y - ((bigIconSize - smallIconSize) / 2)), bigIconSize,
  ...
  for (iconCnt = 1; iconCnt < (sideRightIconCnt + 1); i++) {
    ...
    if (force_icons[showDataPadPowers[i]]) {
      CG_DrawPic(
          holdX, y, smallIconSize, smallIconSize,
          force_icons[showDataPadPowers[i]]); // only cache the icon for display
    }
    ...
    if (((cg_updatedDataPadForcePower1.integer - 1) == showDataPadPowers[i]) ||
      ...
      CG_DrawPic(
          holdX, y, smallIconSize, smallIconSize,
          cgs.media.DPForcePowerOverlay); // only cache the icon for display
    }
  ...
  cgi_SP_GetStringTextString(
      va("INGAME_%s", forcepowerDesc[cg.DataPadforcepowerSelect]), text,
      sizeof(text));

  if (player->client->ps.forcePowerLevel[cg.DataPadforcepowerSelect] == 1) {
    cgi_SP_GetStringTextString(
        va("INGAME_%s", forcepowerLvl1Desc[cg.DataPadforcepowerSelect]), text2,
        sizeof(text2));
  } else if (player->client->ps.forcePowerLevel[cg.DataPadforcepowerSelect] ==
             2) {
    cgi_SP_GetStringTextString(
        va("INGAME_%s", forcepowerLvl2Desc[cg.DataPadforcepowerSelect]), text2,
        sizeof(text2));
  } else {
    cgi_SP_GetStringTextString(
        va("INGAME_%s", forcepowerLvl3Desc[cg.DataPadforcepowerSelect]), text2,
        sizeof(text2));
  }
  ...
  if (text[0]) {
    ...
    CG_DisplayBoxedText(70, 50, 500, 300, va("%s%s", text, text2),
                        cgs.media.qhFontSmall, 0.7f, colorTable[CT_ICON_BLUE]);
  }
}

// actually, these are pretty pointless so far in CHC, since in TA codebase they
// were used only so init some HUD
//	function ptrs to allow cinematics in onscreen displays. So far, we don't
//use those, but here they are anyway...
//
/*	These stupid pragmas don't work, they still give the warning. Forget it,
REM the lot.

#pragma warning ( disable : 4505)		// unreferenced local function
has been removed static int CG_PlayCinematic(const char *name, float x, float y,
float w, float h) { return trap_CIN_PlayCinematic(name, x, y, w, h, CIN_loop);
}
