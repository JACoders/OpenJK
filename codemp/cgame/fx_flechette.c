...
void FX_FlechetteProjectileThink(centity_t *cent,
  ...
  trap->FX_PlayEffectID(cgs.effects.flechetteShotEffect, cent->lerpOrigin,
                        forward, -1, -1, qfalse);
}
...
void FX_FlechetteWeaponHitWall(vec3_t origin, vec3_t normal) {
  trap->FX_PlayEffectID(cgs.effects.flechetteWallImpactEffect, origin, normal,
                        -1, -1, qfalse);
}
...
void FX_FlechetteWeaponHitPlayer(vec3_t origin, vec3_t normal,
  ...
  //	if ( humanoid )
  //	{
  trap->FX_PlayEffectID(cgs.effects.flechetteFleshImpactEffect, origin, normal,
                        -1, -1, qfalse);
  //	}
  //	else
  //	{
...
void FX_FlechetteAltProjectileThink(centity_t *cent,
  ...
  trap->FX_PlayEffectID(cgs.effects.flechetteAltShotEffect, cent->lerpOrigin,
                        forward, -1, -1, qfalse);
}
