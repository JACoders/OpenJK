...
int adjustRespawnTime(float preRespawnTime, int itemType, int itemTag) {
  ...
  if (itemType == IT_WEAPON) {
    if (itemTag == WP_THERMAL || itemTag == WP_TRIP_MINE ||
        itemTag ==
            WP_DET_PACK) { // special case for these, use ammo respawn rate
  ...
  if (level.numPlayingClients > 4) {    // Start scaling the respawn times.
    if (level.numPlayingClients > 32) { // 1/4 time minimum.
      respawnTime *= 0.25;
    } else if (level.numPlayingClients >
               12) { // From 12-32, scale from 0.5 to 0.25;
      respawnTime *= 20.0 / (float)(level.numPlayingClients + 8);
    } else { // From 4-12, scale from 1.0 to 0.5;
      respawnTime *= 8.0 / (float)(level.numPlayingClients + 4);
    }
  }

  if (respawnTime < 1.0) { // No matter what, don't go lower than 1 second, or
                           // the pickups become very noisy!
    respawnTime = 1.0;
  }
...
void ShieldDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
               int damage, int mod) {
  // Play damaging sound...
...
void ShieldPain(gentity_t *self, gentity_t *attacker, int damage) {
  // Set the itemplaceholder flag to indicate the the shield drawing that the
  // shield pain should be drawn.
...
void ShieldGoSolid(gentity_t *self) {
  ...
  trap->Trace(&tr, self->r.currentOrigin, self->r.mins, self->r.maxs,
              self->r.currentOrigin, self->s.number, CONTENTS_BODY, qfalse, 0,
              0);
...
void ShieldGoNotSolid(gentity_t *self) {
  ...
  self->r.contents = 0;
  self->s.eFlags |= EF_NODRAW;
  // nextthink needs to have a large enough interval to avoid excess
  // accumulation of Activate messages
  self->nextthink = level.time + 200;
  self->think = ShieldGoSolid;
...
void CreateShield(gentity_t *ent) {
  ...
  start[2] += (height >> 1);
  trap->Trace(&tr, start, 0, 0, posTraceEnd, ent->s.number, MASK_SHOT, qfalse,
              0, 0);
  posWidth = MAX_SHIELD_HALFWIDTH * tr.fraction;
  // negative trace
  trap->Trace(&tr, start, 0, 0, negTraceEnd, ent->s.number, MASK_SHOT, qfalse,
              0, 0);
  ...
  trap->Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs,
              ent->r.currentOrigin, ent->s.number, CONTENTS_BODY, qfalse, 0, 0);
...
qboolean PlaceShield(gentity_t *playerent) {
  ...
  trap->Trace(&tr, playerent->client->ps.origin, mins, maxs, dest,
              playerent->s.number, MASK_SHOT, qfalse, 0, 0);
  if (tr.fraction > 0.9) { // room in front
    VectorCopy(tr.endpos, pos);
    // drop to floor
    VectorSet(dest, pos[0], pos[1], pos[2] - 4096);
    trap->Trace(&tr, pos, mins, maxs, dest, playerent->s.number, MASK_SOLID,
                qfalse, 0, 0);
    if (!tr.startsolid && !tr.allsolid) {
      ...
      shield->s.modelindex =
          HI_SHIELD; // this'll be used in CG_Useable() for rendering.
...
void ItemUse_Binoculars(gentity_t *ent) {
  ...
  if (ent->client->ps.weaponstate !=
      WEAPON_READY) { // So we can't fool it and reactivate while switching to
                      // the saber or something.
    return;
  }
  ...
  */
  ...
  if (ent->client->ps.zoomMode ==
      0) // not zoomed or currently zoomed with the disruptor
  {
...
//----------------------------------------------------------------
{
  ...
  WP_FireTurretMissile(&g_entities[ent->genericValue3], myOrg, fwd, qfalse, 10,
                       2300, MOD_SENTRY, ent);
...
//-----------------------------------------------------
{
  ...
  for (i = 0; i < count; i++) {
    ...
    if (target->s.eType == ET_NPC &&
        target->s.NPC_class ==
            CLASS_VEHICLE) { // don't get mad at vehicles, silly.
      continue;
    }
    ...
    trap->Trace(&tr, org2, NULL, NULL, org, self->s.number, MASK_SHOT, qfalse,
                0, 0);

    if (!tr.allsolid && !tr.startsolid &&
        (tr.fraction == 1.0 || tr.entityNum == target->s.number)) {
      // Only acquire if have a clear shot, Is it in range and closer than our
      // best?
      ...
      if (enemyDist < bestDist) // all things equal, keep current
      {
        if (self->attackDebounceTime + 100 < level.time) {
          ...
          self->attackDebounceTime =
              level.time + 900 + Q_flrand(0.0f, 1.0f) * 200;
        }
...
//---------------------------------
{
  ...
  if (ent->enemy->health <= 0) {
    ...
    trap->Trace(&tr, org2, NULL, NULL, org, ent->s.number, MASK_SHOT, qfalse, 0,
                0);

    if (tr.allsolid || tr.startsolid || tr.fraction < 0.9f ||
        tr.entityNum == ent->s.number) {
  ...
  } else if (ent->bounceCount < level.time &&
             ent->enemy) // don't ping pong on and off
  {
    ent->enemy = NULL;
    // shut-down sound
...
void turret_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                int damage, int mod);
...
//---------------------------------
{
  ...
  numListedEntities =
      trap->EntitiesInBox(testMins, testMaxs, iEntityList, MAX_GENTITIES);

  while (i < numListedEntities) {
    if (iEntityList[i] < MAX_CLIENTS) { // client stuck inside me. go nonsolid.
      int clNum = iEntityList[i];

      numListedEntities = trap->EntitiesInBox(g_entities[clNum].r.absmin,
                                              g_entities[clNum].r.absmax,
                                              iEntityList, MAX_GENTITIES);
  ...
  if (!g_entities[ent->genericValue3].inuse ||
      ...
      g_entities[ent->genericValue3].client->sess.sessionTeam !=
          ent->genericValue2) {
  ...
  if (ent->enemy) {
    // ...then we'll calculate what new aim adjustments we should attempt to
    // make this frame Aim at enemy
    if (ent->enemy->client) {
      VectorCopy(ent->enemy->client->ps.origin, org);
    } else {
      VectorCopy(ent->enemy->r.currentOrigin, org);
    ...
    // no enemy, so make us slowly sweep back and forth as if searching for a
    // new one
...
//------------------------------------------------------------------------------------------------------------
{
  ...
  if (!g_entities[self->genericValue3].inuse ||
      !g_entities[self->genericValue3].client) {
    G_FreeEntity(self);
    return;
  }
  ...
  G_RadiusDamage(self->s.pos.trBase, &g_entities[self->genericValue3], 30, 256,
                 self, self, MOD_UNKNOWN);
...
//---------------------------------
{
  ...
  base->s.bolt1 = 1; // This is a sort of hack to indicate that this model needs
                     // special turret things done to it
...
//------------------------------------------------------------------------
{
  ...
  sentry->genericValue2 =
      ent->client->sess
          .sessionTeam; // so we can remove ourself if our owner changes teams
...
extern gentity_t *NPC_SpawnType(gentity_t *ent, char *npc_type,
                                char *targetname, qboolean isVehicle);
void ItemUse_Seeker(gentity_t *ent) {
  if (level.gametype == GT_SIEGE &&
      d_siegeSeekerNPC.integer) { // actualy spawn a remote NPC
...
void Jetpack_On(gentity_t *ent) { // create effects?
  ...
  if (ent->client->ps.fd.forceGripBeingGripped >=
      level.time) { // can't turn on during grip interval
    return;
  }
...
void ItemUse_Jetpack(gentity_t *ent) {
  ...
  if (ent->health <= 0 || ent->client->ps.stats[STAT_HEALTH] <= 0 ||
      ...
      ent->client->ps.pm_type ==
          PM_DEAD) { // can't use it when dead under any circumstances.
    return;
  }
...
void ItemUse_UseDisp(gentity_t *ent, int type) {
  ...
  if (!ent->client || ent->client->tossableItemDebounce >
                          level.time) { // can't use it again yet
    return;
  }

  if (ent->client->ps.weaponTime > 0 ||
      ent->client->ps.forceHandExtend !=
          HANDEXTEND_NONE) { // busy doing something else
    return;
  }
  ...
  if (item) {
    ...
    //	G_SetAnim( ent, NULL, SETANIM_TORSO, BOTH_THERMAL_THROW,
...
void EWebDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
             int damage, int mod) {
  vec3_t fxDir;

  G_RadiusDamage(self->r.currentOrigin, self, EWEB_DEATH_DMG, EWEB_DEATH_RADIUS,
                 self, self, MOD_SUICIDE);
  ...
  if (self->r.ownerNum != ENTITYNUM_NONE) {
    ...
    if (owner->inuse && owner->client) {
      ...
      // make sure it resets next time we spawn one in case we someone obtain
      // one before death
      ...
      if (owner->client->ps.stats[STAT_HOLDABLE_ITEM] > 0 &&
          bg_itemlist[owner->client->ps.stats[STAT_HOLDABLE_ITEM]].giType ==
              IT_HOLDABLE &&
          bg_itemlist[owner->client->ps.stats[STAT_HOLDABLE_ITEM]].giTag ==
              HI_EWEB) { // he has it selected so deselect it and select the
...
void EWeb_SetBoneAngles(gentity_t *ent, char *bone, vec3_t angles) {
  ...
  while (thebone) {
    if (!*thebone &&
        !firstFree) { // if the value is 0 then this index is clear, we can use
                      // it if we don't find the bone we want already existing.
      firstFree = thebone;
  ...
  trap->G2API_SetBoneAngles(ent->ghoul2, 0, bone, angles, flags, up, right,
                            forward, NULL, 100, level.time);
}
...
void EWeb_SetBoneAnim(gentity_t *eweb, int startFrame, int endFrame) {
  // set info on the entity so it knows to start the anim on the client next
  // snapshot.
  eweb->s.eFlags |= EF_G2ANIMATING;

  if (eweb->s.torsoAnim == startFrame &&
      eweb->s.legsAnim ==
          endFrame) { // already playing this anim, let's flag it to restart
...
void EWebFire(gentity_t *owner, gentity_t *eweb) {
  ...
  trap->G2API_GetBoltMatrix(eweb->ghoul2, 0, eweb->genericValue10, &boltMatrix,
                            eweb->s.apos.trBase, eweb->r.currentOrigin,
                            level.time, NULL, eweb->modelScale);
  BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, p);
  BG_GiveMeVectorFromMatrix(&boltMatrix, NEGATIVE_Y, d);

  // Start the thing backwards into the bounding box so it can't start inside
  // other solid things
...
void EWebPositionUser(gentity_t *owner, gentity_t *eweb) {
  ...
  trap->G2API_GetBoltMatrix(eweb->ghoul2, 0, eweb->genericValue9, &boltMatrix,
                            eweb->s.apos.trBase, eweb->r.currentOrigin,
                            level.time, NULL, eweb->modelScale);
  BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, p);
  ...
  trap->Trace(&tr, owner->client->ps.origin, owner->r.mins, owner->r.maxs, p,
              owner->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);

  if (!tr.startsolid && !tr.allsolid &&
      tr.fraction == 1.0f) { // all clear, we can move there
    vec3_t pDown;

    VectorCopy(p, pDown);
    pDown[2] -= 7.0f;
    trap->Trace(&tr, p, owner->r.mins, owner->r.maxs, pDown, owner->s.number,
    ...
    if (!tr.startsolid && !tr.allsolid) {
      ...
      if (VectorLength(d) > 1.0f) { // we moved, do some animating
        ...
        if (dAng[YAW] > 0.0f) {
          if (owner->client->ps.legsAnim ==
              BOTH_STRAFE_RIGHT1) { // reset to change direction
            aFlags |= SETANIM_FLAG_OVERRIDE;
          }
          G_SetAnim(owner, &owner->client->pers.cmd, SETANIM_LEGS,
                    BOTH_STRAFE_LEFT1, aFlags, 0);
        } else {
          if (owner->client->ps.legsAnim ==
              BOTH_STRAFE_LEFT1) { // reset to change direction
            aFlags |= SETANIM_FLAG_OVERRIDE;
          }
          G_SetAnim(owner, &owner->client->pers.cmd, SETANIM_LEGS,
                    BOTH_STRAFE_RIGHT1, aFlags, 0);
        }
      } else if (owner->client->ps.legsAnim == BOTH_STRAFE_RIGHT1 ||
                 owner->client->ps.legsAnim ==
                     BOTH_STRAFE_LEFT1) { // don't keep animating in place
...
void EWebUpdateBoneAngles(gentity_t *owner, gentity_t *eweb) {
  ...
  VectorClear(yAng);
  yAng[2] = AngleSubtract(owner->client->ps.viewangles[PITCH],
                          eweb->s.angles[PITCH]) *
            0.8f;
...
// keep it updated
extern int BG_EmplacedView(vec3_t baseAngles, vec3_t angles, float *newYaw,
                           float constraint); // bg_misc.c
...
void EWebThink(gentity_t *self) {
  ...
  if (self->r.ownerNum == ENTITYNUM_NONE) {
    ...
    if (!owner->inuse || !owner->client ||
      ...
      killMe = qtrue;
    } else if (owner->client->ps.emplacedIndex !=
               self->s.number) { // just go back to the inventory then
      EWebDisattach(owner, self);
      return;
    }
    ...
    if (!killMe) {
      ...
      if (BG_EmplacedView(owner->client->ps.viewangles, self->s.angles, &yaw,
                          self->s.origin2[0])) {
      ...
      if (self->genericValue8 < level.time) { // make sure the anim timer is
                                              // done
        ...
        if (owner->client->pers.cmd.buttons & BUTTON_ATTACK) {
          if (self->genericValue5 < level.time) { // we can fire another shot
                                                  // off
            EWebFire(owner, self);
        ...
        } else if (self->genericValue5 < level.time &&
                   self->genericValue3) { // reset the anim back to non-firing
...
gentity_t *EWeb_Create(gentity_t *spawner) {
  ...
  trap->Trace(&tr, s, mins, maxs, pos, spawner->s.number, MASK_PLAYERSOLID,
              qfalse, 0, 0);

  if (tr.allsolid || tr.startsolid ||
      tr.fraction != 1.0f) { // can't spawn here, we are in solid
  ...
  // for the sake of being able to differentiate client-side between this and an
  // emplaced gun
  ent->s.weapon = WP_NONE;

  VectorCopy(pos, downPos);
  downPos[2] -= 18.0f;
  trap->Trace(&tr, pos, mins, maxs, downPos, spawner->s.number,
              MASK_PLAYERSOLID, qfalse, 0, 0);

  if (tr.startsolid || tr.allsolid || tr.fraction == 1.0f ||
      tr.entityNum < ENTITYNUM_WORLD) { // didn't hit ground.
    G_FreeEntity(ent);
    G_Sound(spawner, CHAN_AUTO, failSound);
    return NULL;
  }
  ...
  if (spawner->client->ewebHealth <=
      0) { // refresh the owner's e-web health if its last e-web did not exist
           // or was killed
  ...
  ent->genericValue10 =
      trap->G2API_AddBolt(ent->ghoul2, 0, "*cannonflash"); // muzzle bolt
  ent->genericValue9 = trap->G2API_AddBolt(
      ent->ghoul2, 0,
      "cannon_Yrot"); // for placing the owner relative to rotation

  // set the constraints for this guy as an emplaced weapon, and his constraint
  // angles
  ...
  // store off the owner's current weapons, we will be forcing him to use the
  // "emplaced" weapon
...
void ItemUse_UseEWeb(gentity_t *ent) {
  ...
  if (ent->client->ps.emplacedIndex &&
      !ent->client->ewebIndex) { // using an emplaced gun already that isn't our
                                 // own e-web
    return;
  }
...
int Pickup_Powerup(gentity_t *ent, gentity_t *other) {
  ...
  for (i = 0; i < level.maxclients; i++) {
    ...
    trap->Trace(&tr, client->ps.origin, NULL, NULL, ent->s.pos.trBase,
                ENTITYNUM_NONE, CONTENTS_SOLID, qfalse, 0, 0);
    if (tr.fraction != 1.0) {
      continue;
    }
...
int Pickup_Holdable(gentity_t *ent, gentity_t *other) {
  ...
  return adjustRespawnTime(RESPAWN_HOLDABLE, ent->item->giType,
                           ent->item->giTag);
}
...
void Add_Ammo(gentity_t *ent, int weapon, int count) {
  ...
  if (ent->client->ps.eFlags & EF_DOUBLE_AMMO) { // fix: double ammo for siege
    max *= 2;
  }
...
int Pickup_Ammo(gentity_t *ent, gentity_t *other) {
  ...
  if (ent->item->giTag == -1) { // an ammo_all, give them a bit of everything
    if (level.gametype ==
        GT_SIEGE) // complaints that siege tech's not giving enough ammo.  Does
                  // anything else use ammo all?
    {
...
int Pickup_Weapon(gentity_t *ent, gentity_t *other) {
  ...
  if (ent->count < 0) {
    ...
    if (!(ent->flags & FL_DROPPED_ITEM) && level.gametype != GT_TEAM) {
      ...
      // New method:  If the player has less than half the minimum, give them
      // the minimum, else add 1/2 the min.
      ...
      // Old method:  If the player has less than the minimum, give them the
      // minimum, else just add 1.
      /*
                              // drop the quantity if the already have over the
         minimum if ( other->client->ps.ammo[ ent->item->giTag ] < quantity ) {
                                      quantity = quantity -
         other->client->ps.ammo[ ent->item->giTag ]; } else { quantity = 1;
  ...
  if (level.gametype == GT_TEAM) {
    return adjustRespawnTime(RESPAWN_TEAM_WEAPON, ent->item->giType,
                             ent->item->giTag);
  }

  return adjustRespawnTime(g_weaponRespawn.integer, ent->item->giType,
                           ent->item->giTag);
}
...
int Pickup_Armor(gentity_t *ent, gentity_t *other) {
  ...
  if (other->client->ps.stats[STAT_ARMOR] >
      other->client->ps.stats[STAT_MAX_HEALTH] * ent->item->giTag) {
...
qboolean CheckItemCanBePickedUpByNPC(gentity_t *item, gentity_t *pickerupper) {
  if ((item->flags & FL_DROPPED_ITEM) && item->activator != &g_entities[0] &&
      pickerupper->s.number && pickerupper->s.weapon == WP_NONE &&
      pickerupper->enemy && pickerupper->painDebounceTime < level.time &&
      pickerupper->NPC &&
      pickerupper->NPC->surrenderTime < level.time // not surrendering
      && !(pickerupper->NPC->scriptFlags &
           SCF_FORCED_MARCH) // not being forced to march
      /*&& item->item->giTag != INV_SECURITY_KEY*/) { // non-player, in combat,
                                                      // picking up a dropped
                                                      // item that does NOT
                                                      // belong to the player and
                                                      // it *not* a security key
...
void Touch_Item(gentity_t *ent, gentity_t *other, trace_t *trace) {
  ...
  if (ent->genericValue10 > level.time && other &&
      other->s.number ==
          ent->genericValue11) { // this is the ent that we don't want to be
                                 // able to touch us for x seconds
    return;
  }
  ...
  if (ent->item->giType == IT_WEAPON && ent->s.powerups &&
  ...
  if (other->client->NPC_class == CLASS_ATST ||
      ...
      // other->client->NPC_class == CLASS_JAWA || //FIXME: in some cases it's
      // okay?
      other->client->NPC_class ==
          CLASS_UGNAUGHT || // FIXME: in some cases it's okay?
      other->client->NPC_class ==
          CLASS_SENTRY) { // FIXME: some flag would be better
    // droids can't pick up items/weapons!
    return;
  }

  if (CheckItemCanBePickedUpByNPC(ent, other)) {
    if (other->NPC && other->NPC->goalEntity &&
        other->NPC->goalEntity->enemy ==
            ent) { // they were running to pick me up, they did, so clear goal
    ...
    if (other->s.eType == ET_NPC) {                 // Not the player?
      ...
      if (ent->item->giType == IT_AMMO && ent->item->giTag == -1 &&
          other->s.NPC_class == CLASS_VEHICLE && other->m_pVehicle &&
          other->m_pVehicle->m_pVehicleInfo->type ==
              VH_WALKER) { // yeah, uh, atst gets healed by these things
  ...
  case IT_AMMO:
    ...
    if (ent->item->giTag == AMMO_THERMAL || ent->item->giTag == AMMO_TRIPMINE ||
      ...
      if (other && other->client &&
          other->client->ps.ammo[weaponData[weapForAmmo].ammoIndex] > 0) {
  ...
  if (predict) {
    if (other->client) {
      BG_AddPredictableEventToPlayerstate(EV_ITEM_PICKUP, ent->s.number,
                                          &other->client->ps);
    } else {
...
gentity_t *LaunchItem(gitem_t *item, vec3_t origin, vec3_t velocity) {
  ...
  if (item->giTag != WP_BOWCASTER && item->giTag != WP_DET_PACK &&
...
void FinishSpawningItem(gentity_t *ent) {
  ...
  if (level.gametype != GT_JEDIMASTER) {
    if (HasSetSaberOnly()) {
      ...
      if (ent->item->giType == IT_HOLDABLE) {
        if (ent->item->giTag == HI_SEEKER || ent->item->giTag == HI_SHIELD ||
  ...
  if (g_forcePowerDisable
          .integer) { // if force powers disabled, don't add force powerups
    if (ent->item->giType == IT_POWERUP) {
      if (ent->item->giTag == PW_FORCE_ENLIGHTENED_LIGHT ||
          ent->item->giTag == PW_FORCE_ENLIGHTENED_DARK ||
  ...
  /*	item = &bg_itemlist[ ent->s.modelindex ];
          if (!Q_stricmp(&item->world_model[0][strlen(item->world_model[0]) -
     4], ".glm"))
          {
                  trap->G2API_InitGhoul2Model(&ent->s, item->world_model[0],
     G_ModelIndex(item->world_model[0] ), 0, 0, 0, 0); ent->s.radius = 60;
          }
  */
  if (ent->spawnflags & ITMSF_SUSPEND) {
    ...
    // if it is directly even with the floor it will return startsolid, so raise
    // up by 0.1 and temporarily subtract 0.1 from the z maxs so that going up
    // doesn't push into the ceiling
    ent->s.origin[2] += 0.1f;
    ent->r.maxs[2] -= 0.1f;

    VectorSet(dest, ent->s.origin[0], ent->s.origin[1],
              ent->s.origin[2] - 4096);
    trap->Trace(&tr, ent->s.origin, ent->r.mins, ent->r.maxs, dest,
                ent->s.number, MASK_SOLID, qfalse, 0, 0);
    if (tr.startsolid) {
...
void ClearRegisteredItems(void) {
  ...
  if (level.gametype ==
      GT_SIEGE) { // kind of cheesy, maybe check if siege class with disp's is
                  // gonna be on this map too
...
void G_SpawnItem(gentity_t *ent, gitem_t *item) {
  ...
  if (item->giType == IT_WEAPON && wDisable &&
...
void G_BounceItem(gentity_t *ent, trace_t *trace) {
  ...
  if ((ent->s.weapon == WP_DET_PACK && ent->s.eType == ET_GENERAL &&
       ent->physicsObject)) { // detpacks only
    if (ent->touch) {
  ...
  if (ent->s.eType == ET_HOLOCRON ||
      (ent->s.shouldtarget && ent->s.eType == ET_GENERAL &&
       ent->physicsObject)) { // holocrons and sentry guns
    if (ent->touch) {
      ent->touch(ent, &g_entities[trace->entityNum], trace);
    }
  }
...
void G_RunItem(gentity_t *ent) {
  ...
  trap->Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin,
              ent->r.ownerNum, mask, qfalse, 0, 0);
