...
//
// This file is designed to be pasted into each game project that uses the
// StringEd package's files.
//  You can alter the way it does things by (eg) replacing STL with RATL, BUT
//  LEAVE THE OVERALL
//	FUNCTIONALITY THE SAME, or if I ever make any funadamental changes to
//the way the package works 	then you're going to be SOOL (shit out of luck
//;-)...
//
...
#ifdef _MSC_VER
#pragma warning(                                                               \
    disable : 4786) // disable the usual stupid and pointless STL warning
#endif
#include <set>
#include <string>
#include <vector>
...
typedef struct SE_Entry_s {
  ...
  std::string m_strDebug; // english and/or "#same", used for debugging only.
                          // Also prefixed by "SE:" to show which strings go
                          // through StringEd (ie aren't hardwired)
  int m_iFlags;

  SE_Entry_s() { m_iFlags = 0; }
...
public:
  CStringEdPackage() { Clear(SE_FALSE); }
...
void CStringEdPackage::Clear(SE_BOOL bChangingLanguages) {
  ...
  if (!bChangingLanguages) {
    // if we're changing languages, then I'm going to leave these alone. This is
    // to do with any (potentially) cached
    //	flag bitmasks on the game side. It shouldn't matter since all files are
    //written out at once using the build 	command in StringEd. But if ever
    //someone changed a file by hand, or added one, or whatever, and it had a
    //	different set of flags declared, or the strings were in a different
    //order, then the flags might also change 	the order I see them in, and hence
    //their indexes and masks. This should never happen unless people mess with
    //	the .STR files by hand and delete some, but this way makes sure it'll
    //all work just in case...
    //
    // ie. flags stay defined once they're defined, and only the destructor (at
    // app-end) kills them.
    //
  ...
  //
  // the other vars are cleared in SetupNewFileParse(), and are ok to not do
  // here.
  //
}
...
//
// (normally I'd call another function for this, but this is supposed to be
// engine-independant,
...
char *CStringEdPackage::Filename_WithoutExt(const char *psFilename) {
  ...
  // special check, make sure the first suffix we found from the end wasn't just
  // a directory suffix (eg on a path'd filename with no extension anyway)
  //
...
void CStringEdPackage::SetupNewFileParse(const char *psFileName,
                                         SE_BOOL bLoadDebug) {
  ...
  m_bLoadingEnglish_ParseOnly =
      (!Q_stricmp(m_strLoadingLanguage_ParseOnly.c_str(), "english"))
          ? SE_TRUE
          : SE_FALSE;
  m_bLoadDebug = bLoadDebug;
}

// returns SE_TRUE if supplied keyword found at line start (and advances
// supplied ptr past any whitespace to next arg (or line end if none),
//
//	else returns SE_FALSE...
//
SE_BOOL CStringEdPackage::CheckLineForKeyword(const char *psKeyword,
                                              const char *&psLine) {
...
// kill off any "//" onwards part in the line, but NOT if it's inside a quoted
// string...
//
void CStringEdPackage::REMKill(char *psBuffer) {
  ...
  // scan forwards in case there are more than one (and the first is inside
  // quotes)...
  //
  while ((p = strstr(psScanPos, "//")) != NULL) {
    // count the number of double quotes before this point, if odd number, then
    // we're inside quotes...
    //
      ...
      *p = '\0';
      ...
      if (psScanPos[0]) // any strlen? (else access violation with -1 below)
      {
        ...
        while (iWhiteSpaceScanPos >= 0 &&
               isspace(psScanPos[iWhiteSpaceScanPos])) {
...
// remove any outside quotes from this supplied line, plus any leading or
// trailing whitespace...
//
const char *CStringEdPackage::InsideQuotes(const char *psLine) {
  // I *could* replace this string object with a declared array, but wasn't sure
  // how big to leave it, and it'd have to
...
void CStringEdPackage::AddFlagReference(const char *psLocalReference,
  ...
  //
  mapStringEntries_t::iterator itEntry = m_StringEntries.find(
      va("%s_%s", m_strCurrentFileRef_ParseOnly.c_str(), psLocalReference));
...
// this copes with both foreigners using hi-char values (eg the french using
// 0x92 instead of 0x27
//  for a "'" char), as well as the fact that our buggy fontgen program writes
//  out zeroed glyph info for
//	some fonts anyway (though not all, just as a gotcha).
//
// New bit, instead of static buffer (since XBox guys are desperately short of
// mem) I return a malloc'd buffer now,
//	so remember to free it!
//
static char *CopeWithDumbStringData(const char *psSentence,
                                    const char *psThisLanguage) {
  const int iBufferSize =
      strlen(psSentence) *
      3; // *3 to allow for expansion of anything even stupid string consisting
         // entirely of elipsis chars
  char *psNewString = (char *)Z_Malloc(iBufferSize, TAG_TEMP_WORKSPACE, qfalse);
  Q_strncpyz(psNewString, psSentence, iBufferSize);

  // this is annoying, I have to just guess at which languages to do it for (ie
  // NOT ASIAN/MBCS!!!) since the
  //	string system was deliberately (and correctly) designed to not know or
  //care whether it was doing SBCS 	or MBCS languages, because it was never
  //envisioned that I'd have to clean up other people's mess.
  //
  // Ok, bollocks to it, this will have to do. Any other languages that come
  // later and have bugs in their text can
  ...
  if (!Q_stricmp(psThisLanguage, "ENGLISH") ||
    ...
    while ((p = strchr(psNewString, 0x92)) !=
           NULL) // "rich" (and illegal) apostrophe
    {
      *p = 0x27;
    }
    ...
    //	strXLS_Speech.Replace(va("%c",0x0B),".");			// full
    //stop
    while ((p = strchr(psNewString, 0x0B)) != NULL) {
      *p = '.';
    }

    //	strXLS_Speech.Replace(va("%c",0x85),"...");			// "..."-char ->
    //3-char "..."
    while ((p = strchr(psNewString, 0x85)) !=
           NULL) // "rich" (and illegal) apostrophe
    {
  ...
  if (psLine) {
    if (CheckLineForKeyword(sSE_KEYWORD_VERSION, psLine)) {
      ...
      if (psReference[0]) {
        ...
        char sFlags[1024] = {
            0}; // 1024 chars should be enough to store 8 flag names
        ...
        psErrorMessage =
            "Error parsing file: Unexpected \"" sSE_KEYWORD_FLAGS "\"\n";
      }
    } else if (CheckLineForKeyword(sSE_KEYWORD_ENDMARKER, psLine)) {
      // ENDMARKER
      //
      m_bEndMarkerFound_ParseOnly =
          SE_TRUE; // the only major error checking I bother to do (for file
                   // truncation)
    } else if (!Q_stricmpn(sSE_KEYWORD_LANG, psLine,
                           strlen(sSE_KEYWORD_LANG))) {
      // LANG_ENGLISH 	"GUARD:  Good to see you, sir.  Taylor is waiting for
      // you in the clean tent.  We need to get you suited up.  "
      //
      const char *psReference = GetCurrentReference_ParseOnly();
      if (psReference[0]) {
        ...
        strncpy(sThisLanguage, psLine,
                iCharsToCopy); // already declared as {0} so no need to zero-cap
                               // dest buffer

        psLine += strlen(sThisLanguage);
        const char *_psSentence = ConvertCRLiterals_Read(InsideQuotes(psLine));

        // Dammit, I hate having to do crap like this just because other people
        // mess up and put
        //	stupid data in their text, so I have to cope with it.
        //
        // note hackery with _psSentence and psSentence because of const-ness.
        // bleurgh. Just don't ask.
        //
        ...
        if (m_bLoadingEnglish_ParseOnly) {
          ...
          //
          SE_BOOL bSentenceIsEnglish =
              (!Q_stricmp(sThisLanguage, "english"))
                  ? SE_TRUE
          ...
          if (!bSentenceIsEnglish) {
            // basically this is just checking that an .STE file override is the
            // same language as the .STR...
            //
            if (Q_stricmp(m_strLoadingLanguage_ParseOnly.c_str(),
                          sThisLanguage)) {
              psErrorMessage =
                  va("Language \"%s\" found when expecting \"%s\"!\n",
        ...
        psErrorMessage =
            "Error parsing file: Unexpected \"" sSE_KEYWORD_LANG "\"\n";
      }
    } else {
...
void CStringEdPackage::AddEntry(const char *psLocalReference) {
  // the reason I don't just assign it anyway is because the optional .STE
  // override files don't contain flags,
  ...
  if (itEntry == m_StringEntries.end()) {
    SE_Entry_t SE_Entry;
    m_StringEntries[va("%s_%s", m_strCurrentFileRef_ParseOnly.c_str(),
                       psLocalReference)] = SE_Entry;
  }
...
const char *Leetify(const char *psString) {
  ...
  str = psString;
  if (sp_leet->integer ==
      42) // very specific test, so you won't hit it accidentally
  {
    static const char cReplace[] = {
        'o', '0', 'l', '1', 'e', '3', 'a', '4', 's', '5', 't',
        '7', 'i', '!', 'h', '#', 'O', '0', 'L', '1', 'E', '3',
        'A', '4', 'S', '5', 'T', '7', 'I', '!', 'H', '#' // laziness because of
                                                         // strchr()
    };
...
void CStringEdPackage::SetString(const char *psLocalReference,
  ...
  if (itEntry != m_StringEntries.end()) {
    ...
    if (bEnglishDebug || m_bLoadingEnglish_ParseOnly) {
      // then this is the leading english text of a foreign sentence pair (so
      // it's the debug-key text),
      ...
      if (m_bLoadDebug) {
        ...
        Entry.m_strDebug +=
            /* m_bLoadingEnglish_ParseOnly ? "" : */ psNewString;
        Entry.m_strDebug += sSE_DEBUGSTR_SUFFIX;
      }
      m_strCurrentEntryEnglish_ParseOnly =
          psNewString; // for possible "#same" resolving in foreign later
    } else {
      // then this is foreign text (so check for "#same" resolving)...
      //
      if (!Q_stricmp(psNewString, sSE_EXPORT_SAME)) {
        Entry.m_strString =
            m_strCurrentEntryEnglish_ParseOnly; // foreign "#same" is now
                                                // english
        if (m_bLoadDebug) {
          Entry.m_strDebug = sSE_DEBUGSTR_PREFIX;
...
// return is either NULL for good else error message to display...
//
static const char *SE_Load_Actual(const char *psFileName, SE_BOOL bLoadDebug,
                                  SE_BOOL bSpeculativeLoad) {
  const char *psErrorMessage = NULL;
  ...
  if (psLoadedData) {
    ...
    char sLineBuffer[16384]; // should be enough for one line of text (some of
                             // them can be BIG though)
    while (!psErrorMessage &&
           TheStringPackage.ReadLine((const char *&)psParsePos, sLineBuffer)) {
    ...
    if (!psErrorMessage && !TheStringPackage.EndMarkerFoundDuringParse()) {
      psErrorMessage = va("Truncated file, failed to find \"%s\" at file end!",
                          sSE_KEYWORD_ENDMARKER);
    }
  } else {
    if (bSpeculativeLoad) {
...
static const char *SE_GetFoundFile(std::string &strResult) {
  ...
  if (psSemiColon) {
    *psSemiColon = '\0';
    ...
    // no semicolon found, probably last entry? (though i think even those have
    // them on, oh well)
    //
    strResult.erase();
  }

  //	strlwr(sTemp);	// just for consistancy and set<> -> set<> erasure
  //checking etc
...
// return is either NULL for good else error message to display...
//
const char *SE_Load(const char *psFileName, SE_BOOL bLoadDebug = SE_TRUE,
                    SE_BOOL bFailIsCritical = SE_TRUE) {
  ////////////////////////////////////////////////////
  //
  // ingame here tends to pass in names without paths, but I expect them when
  // doing a language load, so...
  //
  char sTemp[1000] = {0};
  ...
  // check for any corresponding / overriding .STE files and load them
  // afterwards...
  //
  if (!psErrorMessage) {
  ...
  if (psErrorMessage) {
    if (bFailIsCritical) {
      //		TheStringPackage.Clear(TRUE);	// Will we want to do
      //this?  Any errors that arise should be fixed immediately
...
const char *SE_GetString(const char *psPackageReference,
  ...
  char sReference[256]; // will always be enough, I've never seen one more than
                        // about 30 chars long

  Com_sprintf(sReference, sizeof(sReference), "%s_%s", psPackageReference,
              psStringReference);
...
const char *SE_GetString(const char *psPackageAndStringReference) {
  ...
  mapStringEntries_t::iterator itEntry =
      TheStringPackage.m_StringEntries.find(sReference);
  ...
  // should never get here, but fall back anyway... (except we DO use this to
  // see if there's a debug-friendly key bind, which may not exist)
  //
  //	__ASSERT(0);
  return ""; // you may want to replace this with something based on _DEBUG or
             // not?
#endif
}
...
int SE_GetFlags(const char *psPackageAndStringReference) {
  mapStringEntries_t::iterator itEntry =
      TheStringPackage.m_StringEntries.find(psPackageAndStringReference);
  if (itEntry != TheStringPackage.m_StringEntries.end()) {
    SE_Entry_t &Entry = (*itEntry).second;
...
int SE_GetNumFlags(void) { return TheStringPackage.m_vstrFlagNames.size(); }
...
// I could cache the result of this since it won't change during app lifetime
// unless someone does a build-publish
//	while you're still ingame. Cacheing would make sense since it can take a
//while to scan, but I'll leave it and 	let whoever calls it cache the results
//instead. I'll make it known that it's a slow process to call this, but
//	whenever anyone calls someone else's code they should assign it to an
//int anyway, since you've no idea what's 	going on. Basically, don't  use this
//in a FOR loop as the end-condition. Duh.
//
// Groan, except for Bob. I mentioned that this was slow and only call it once,
// but he's calling it from
...
// these two functions aren't needed other than to make Quake-type games happy
// and/or stop memory managers
//	complaining about leaks if they report them before the global StringEd
//package object calls it's own dtor.
//
...
//	int iNumLanguages = SE_GetNumLanguages();
  ...
  se_language =
      Cvar_Get("se_language", "english", CVAR_ARCHIVE | CVAR_NORESTART);
  ...
  if (psErrorMessage) {
    Com_Error(ERR_DROP,
              "SE_Init() Unable to load language: \"%s\"!\nError: \"%s\"\n",
...
//
// Any errors that result from this should probably be treated as game-fatal,
// since an asset file is fuxored.
//
const char *SE_LoadLanguage(const char *psLanguage,
                            SE_BOOL bLoadDebug /* = SE_TRUE */) {
  const char *psErrorMessage = NULL;
...
// called in Com_Frame, so don't take up any time! (can also be called during
// dedicated)
//
// instead of re-loading just the files we've already loaded I'm going to load
// the whole language (simpler)
//
