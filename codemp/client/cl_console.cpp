...
void Con_DrawNotify(void) {
	...
	for (i = con.current - NUM_CON_TIMES + 1; i <= con.current; i++) {
		...
		if (re->Language_IsAsian()) {
			...
			// and print...
			//
			re->Font_DrawString(cl_conXOffset->integer + con.xadjust * (con.xadjust + con.charWidth), con.yadjust * v, sTemp, g_color_table[currentColor],
...
void Con_DrawSolidConsole(float frac) {
	...
	for (x = 0; x < i; x++) {
		SCR_DrawSmallChar(cls.glconfig.vidWidth - (i - x + 1) * con.charWidth, (lines - (con.charHeight + con.charHeight / 2)), JK_VERSION[x]);
	}
