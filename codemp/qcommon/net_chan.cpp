...
qboolean Netchan_Process(netchan_t *chan, msg_t *msg) {
  ...
  if (fragmented) {
    ...
    if (fragmentStart != chan->fragmentLength) {
      ...
      // rww - not clearing this will allow us to piece together fragments
      // belonging to other packets that happen to have the same sequence (or so
      // it seems). I am just going to clear it and force the packet to be
      // dropped.
    ...
    if (fragmentLength < 0 || msg->readcount + fragmentLength > msg->cursize ||
        chan->fragmentLength + fragmentLength >
            (int)sizeof(chan->fragmentBuffer)) {
      if (showdrop->integer || showpackets->integer) {
    ...
    *(int *)msg->data = LittleLong(sequence);
    ...
    // but I am a wuss -mw
    // chan->incomingSequence = sequence;   // lets not accept any more with
    // this sequence number -gil
    return qtrue;
  }
