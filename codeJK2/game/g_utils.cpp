...
//-----------------------------
void G_PlayEffect(const char *name, const int modelIndex, const int boltIndex,
                  const int entNum) {
...
extern void CG_TryPlayCustomSound(vec3_t origin, int entityNum,
                                  soundChannel_t channel, const char *soundName,
                                  int customSoundSet);
...
void G_SpeechEvent(gentity_t *self, int event) {
  ...
  case EV_CHASE3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*chase%i.wav", event - EV_CHASE1 + 1), CS_EXTRA);
    break;
  case EV_COVER1:
  case EV_COVER2:
  case EV_COVER3:
...
void G_FreeEntity(gentity_t *ed) {
  ...
  //			This is really fucking stupid and has caused us no end
  //of 			trouble!!!  Change it to set the s.number to the proper index 			(ed -
  //g_entities) *OR* ENTITYNUM_NONE!!!
