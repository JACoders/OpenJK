...
extern void G_ATSTCheckPain(gentity_t *self, gentity_t *other, vec3_t point,
                            int damage, int mod, int hitLoc);
...
static int G_CheckForLedge(gentity_t *self, vec3_t fallCheckDir,
                           float checkDist);
static int G_CheckSpecialDeathAnim(gentity_t *self, vec3_t point, int damage,
                                   int mod, int hitLoc);
static int G_PickDeathAnim(gentity_t *self, vec3_t point, int damage, int mod,
                           int hitLoc);
static void G_TrackWeaponUsage(gentity_t *self, gentity_t *inflictor, int add,
                               int mod);
  ...
  if (weapon == WP_SABER) {
    if (self->weaponModel < 0 || WP_SaberLose(self, NULL)) {
      self->s.weapon = WP_NONE;
             ...
             WP_BLASTER_PISTOL) { // FIXME: either drop the pistol and make the
                                  // pickup only give ammo or drop ammo
  } else if (weapon > WP_SABER &&
             weapon <= MAX_PLAYER_WEAPONS) //&& self->client->ps.ammo[
                                           //weaponData[weapon].ammoIndex ]
  {
    self->s.weapon = WP_NONE;

    if (weapon == WP_THERMAL &&
        self->client->ps.torsoAnim ==
            BOTH_ATTACK10) { // we were getting ready to throw the thermal, drop
                             // it!
      self->client->ps.weaponChargeTime =
          level.time - FRAMETIME; // so it just kind of drops it
      dropped = WP_DropThermal(self);
    } else { // find the item type for this weapon
    ...
    if (item && !dropped) {
      ...
      if (!self->s.number) { // player's dropped items never go away
        // dropped->e_ThinkFunc = thinkF_NULL;
        // dropped->nextthink = -1;
        dropped->count = 0; // no ammo
      } else { // FIXME: base this on the NPC's actual amount of ammo he's used
               // up...
        switch (weapon) {
      ...
      // well, dropped weapons are G2 models, so they have to be initialised if
      // they want to draw..give us a radius so we don't get prematurely culled
      if (weapon != WP_THERMAL && weapon != WP_TRIP_MINE &&
          weapon != WP_DET_PACK) {
        gi.G2API_InitGhoul2Model(dropped->ghoul2, item->world_model,
                                 G_ModelIndex(item->world_model), NULL_HANDLE,
                                 NULL_HANDLE, 0, 0);
        dropped->s.radius = 10;
      }
    }
  }
  //	else if (( self->client->NPC_class == CLASS_SENTRY ) || (
  //self->client->NPC_class == CLASS_PROBE )) // Looks dumb, Steve told us to
  //take it out.
  //	{
...
void G_DropKey(gentity_t *self) { // drop whatever security key I was holding
  gitem_t *item = NULL;
...
// void CG_SurfaceExplosion( vec3_t origin, vec3_t normal, float radius, float
// shake_speed, qboolean smoke );
void ExplodeDeath(gentity_t *self) {
  ...
  AngleVectors(self->s.angles, forward, NULL,
               NULL); // FIXME: letting effect always shoot up?  Might be ok.
  ...
  //	else
  //	{
  //		CG_SurfaceExplosion( self->currentOrigin, forward, 20.0f, 12.0f,
  //((self->spawnflags&4)==qfalse) );	//FIXME: This needs to be consistent to
  //all exploders! 		G_Sound(self, self->sounds );
  //	}
  ...
  if (self->splashDamage > 0 && self->splashRadius > 0) {
    ...
    G_RadiusDamage(self->currentOrigin, attacker, self->splashDamage,
                   self->splashRadius, attacker, MOD_UNKNOWN);
  }

  ObjectDie(self, self, self, 20, 0);
}

void ExplodeDeath_Wait(gentity_t *self, gentity_t *inflictor,
...
void ExplodeDeath(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                  int damage, int meansOfDeath, int dFlags, int hitLoc) {
  self->currentOrigin[2] +=
      16; // me bad for hacking this.  should either do it in the effect file or
          // make a custom explode death??
  ExplodeDeath(self);
}
...
void GoExplodeDeath(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  self->targetname = ""; // Make sure this entity cannot be told to explode
                         // again (recursive death fix)
...
void G_CheckVictoryScript(gentity_t *self) {
  if (!G_ActivateBehavior(self, BSET_VICTORY)) {
    if (self->NPC &&
        self->s.weapon == WP_SABER) { // Jedi taunt from within their AI
    ...
    // FIXME: any way to not say this *right away*?  Wait for victim's death
    // anim/scream to finish?
    if (self->NPC && self->NPC->group && self->NPC->group->commander &&
        self->NPC->group->commander->NPC &&
        self->NPC->group->commander->NPC->rank > self->NPC->rank &&
        !Q_irand(0, 2)) { // sometimes have the group commander speak instead
      self->NPC->group->commander->NPC->greetingDebounceTime =
          level.time + Q_irand(2000, 5000);
      // G_AddVoiceEvent( self->NPC->group->commander, Q_irand(EV_VICTORY1,
      // EV_VICTORY3), 2000 );
    } else if (self->NPC) {
...
qboolean OnSameTeam(gentity_t *ent1, gentity_t *ent2) {
  if (!ent1->client || !ent2->client) {
    if (ent1->noDamageTeam) {
      if (ent2->client && ent2->client->playerTeam == ent1->noDamageTeam) {
        ...
        if (ent1->splashDamage && ent2->splashDamage &&
            Q_stricmp("ambient_etherian_fliers", ent1->classname) !=
                0) { // Barrels, exploding breakables and mines will blow each
                     // other up
          return qfalse;
        } else {
          return qtrue;
  ...
  // shouldn't need this anymore, there were problems with certain droids, but
  // now they have been labeled TEAM_ENEMY so this isn't needed
  //	if ((( ent1->client->playerTeam == TEAM_IMPERIAL ) && (
  //ent1->client->playerTeam == TEAM_BOTS )) ||
  //		(( ent1->client->playerTeam == TEAM_BOTS ) && (
  //ent1->client->playerTeam == TEAM_IMPERIAL )))
  //	{
  //		return qtrue;
  //	}
...
void G_AlertTeam(gentity_t *victim, gentity_t *attacker, float radius,
                 float soundDist) {
  ...
  for (i = 0; i < numEnts; i++) {
    ...
    if (radiusEnts[i]->enemy ==
        NULL) { // only do this if they're not already mad at someone
      distSq =
          DistanceSquared(radiusEnts[i]->currentOrigin, victim->currentOrigin);
      if (distSq > 16384 /*128 squared*/ &&
          !gi.inPVS(victim->currentOrigin,
                    radiusEnts[i]->currentOrigin)) { // not even potentially
                                                     // visible/hearable
        continue;
      }
      // NOTE: this allows sound alerts to still go through doors/PVS if the
      // teammate is within 128 of the victim...
      if (soundDist <= 0 || distSq > sndDistSq) { // out of sound range
        if (!InFOV(victim, radiusEnts[i], radiusEnts[i]->NPC->stats.hfov,
                   radiusEnts[i]->NPC->stats.vfov) ||
            !NPC_ClearLOS(radiusEnts[i],
                          victim->currentOrigin)) { // out of FOV or no LOS
          continue;
        }
      }
...
void G_DeathAlert(gentity_t *victim,
                  gentity_t *attacker) { // FIXME: with all the other alert
                                         // stuff, do we really need this?
...
Not to be confused with NPC_RemoveBodyEffects (NPC.cpp), which only applies
effect when removing the body
----------------------------------------
*/
...
void DeathFX(gentity_t *ent) {
  ...
  */
  // team no longer indicates species/race.  NPC_class should be used to
  ...
  case CLASS_GONK:
    ...
    //);
    G_SoundOnEnt(ent, CHAN_AUTO,
                 va("sound/chars/gonk/misc/death%d.wav", Q_irand(1, 3)));
  ...
  case CLASS_INTERROGATOR:
    ...
    G_PlayEffect("droidexplosion1", effectPos);
    G_SoundOnEnt(ent, CHAN_AUTO,
                 "sound/chars/interrogator/misc/int_droid_explo");
    break;
...
void G_SetMissionStatusText(gentity_t *attacker, int mod) {
  ...
  if (mod == MOD_FALLING) { // fell to your death
    statusTextIndex = STAT_WATCHYOURSTEP;
  } else if (mod == MOD_CRUSH) { // crushed
    statusTextIndex = STAT_JUDGEMENTMUCHDESIRED;
  } else if (attacker && Q_stricmp("trigger_hurt", attacker->classname) ==
                             0) { // Killed by something that should have been
                                  // clearly dangerous
    //		statusTextIndex = Q_irand( IGT_JUDGEMENTDESIRED,
    //IGT_JUDGEMENTMUCHDESIRED );
    statusTextIndex = STAT_JUDGEMENTMUCHDESIRED;
  } else if (attacker && attacker->s.number != 0 && attacker->client &&
             attacker->client->playerTeam ==
                 TEAM_PLAYER) { // killed by a teammate
...
void G_StartMatrixEffect(
    gentity_t *ent, qboolean falling = qfalse,
    int length = 1000) { // FIXME: only do this if no other enemies around?
  if (g_timescale->value != 1.0) { // already in some slow-mo mode
    return;
  }
  ...
  if (matrix) {
    ...
    if (falling) { // no timescale or vert bob
      matrix->s.weapon = 1;
    }
  }
}
...
qboolean G_GetHitLocFromSurfName(gentity_t *ent, const char *surfName,
  ...
  *hitLoc = HL_NONE;
  ...
  if (ent->client &&
      (ent->client->NPC_class == CLASS_R2D2 ||
       ...
       ent->client->NPC_class ==
           CLASS_PROBE)) { // we don't care about per-surface hit-locations or
                           // dismemberment for these guys
    return qfalse;
  }
  ...
  if (ent->client && (ent->client->NPC_class == CLASS_ATST)) {
    ...
    if (!Q_stricmp("torso_antenna", surfName) ||
        !Q_stricmp("torso_antenna_base", surfName)) {
      *hitLoc = HL_GENERIC1;
      ...
      if (ent->footLBolt >= 0) {
        ...
        if (DistanceSquared(point, tagOrg) < 100) { // actually hit the foot
          *hitLoc = HL_FOOT_LT;
             ...
             !Q_strncmp("w_", surfName, 2)) { // right hand or weapon
    *hitLoc = HL_HAND_RT;
  ...
  if (g_saberRealisticCombat->integer) {
    dismember = qtrue;
  } else if (g_dismemberment->integer >= 11381138 ||
             !ent->client->dismembered) {
    if (ent->client && ent->client->NPC_class == CLASS_PROTOCOL) {
      dismember = qtrue;
    } else if (dir && (dir[0] || dir[1] || dir[2]) && bladeDir &&
               (bladeDir[0] || bladeDir[1] ||
                bladeDir[2])) { // we care about direction (presumably for
                                // dismemberment)
      if (g_dismemberProbabilities->value <= 0.0f ||
          G_Dismemberable(ent,
                          *hitLoc)) { // either we don't care about probabilties
                                      // or the probability let us continue
        char *tagName = NULL;
        float aoa = 0.5f;
        ...
        if (tagName) {
          int tagBolt =
              gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], tagName);
          if (tagBolt != -1) {
            mdxaBone_t boltMatrix;
            ...
            if (DistanceSquared(point, tagOrg) < 256) { // hit close
              ...
              if (dot < aoa && dot > -aoa) { // hit roughly perpendicular
                ...
                if (dot < aoa && dot > -aoa) { // blade was roughly
                                               // perpendicular
                  dismember = qtrue;
                }
              }
...
int G_GetHitLocation(gentity_t *target, vec3_t ppoint) {
  ...
  if (target->client) { // ignore player's pitch and roll
  ...
  */
  ...
  } else if (HitLoc <= 50) { // legs
    if (rdot > 0) {
      return HL_LEG_RT;
    } else {
      return HL_LEG_LT;
    }
  } else if (HitLoc == 56 || HitLoc == 60 || HitLoc == 61 || HitLoc == 65 ||
             HitLoc == 66 || HitLoc == 70) { // hands
    if (rdot > 0) {
      return HL_HAND_RT;
    } else {
      return HL_HAND_LT;
    }
  } else if (HitLoc == 83 || HitLoc == 87 || HitLoc == 88 || HitLoc == 92 ||
             HitLoc == 93 || HitLoc == 97) { // arms
    if (rdot > 0) {
      return HL_ARM_RT;
    } else {
      return HL_ARM_LT;
...
void LimbThink(gentity_t *ent) { // FIXME: just use object thinking?
  vec3_t origin;
  trace_t tr;

  ent->nextthink = level.time + FRAMETIME;

  if (ent->enemy) { // alert people that I am a piece of one of their friends
    AddSightEvent(ent->enemy, ent->currentOrigin, 384, AEL_DISCOVERED);
  }

  if (ent->s.pos.trType == TR_STATIONARY) { // stopped
    if (level.time > ent->s.apos.trTime + ent->s.apos.trDuration) {
      ...
      // FIXME: these keep drawing for a frame or so after being freed?!  See
      // them lerp to origin of world...
    } else {
  ...
  if (tr.fraction != 1) {
    ...
    if (ent->s.pos.trType == TR_STATIONARY) { // stopped, stop spinning
      // lay flat
      // pitch
      VectorCopy(ent->currentAngles, ent->s.apos.trBase);
      vec3_t flatAngles;
      if (ent->s.angles2[0] == -1) { // any pitch is okay
      ...
      // roll
      if (ent->s.angles2[2] == -1) { // any roll is okay
...
static qboolean G_Dismember(gentity_t *ent, vec3_t point, const char *limbBone,
                            const char *rotateBone, char *limbName,
                            char *limbCapName, char *stubCapName,
                            char *limbTagName, char *stubTagName, int limbAnim,
                            float limbRollBase, float limbPitchBase, int damage,
                            int hitLoc) {
  int newBolt;
  ...
  if (gi.G2API_GetSurfaceRenderStatus(&ent->ghoul2[ent->playerModel],
                                      limbName)) { // already lost this limb
    return qfalse;
  }

  // NOTE: only reason I have this next part is because
  // G2API_GetSurfaceRenderStatus is *not* working
  if (G_LimbLost(ent, hitLoc)) { // already lost this limb
    return qfalse;
  }

  // FIXME: when timescale is high, can sometimes cut off a surf that includes a
  // surf that was already cut off
  // 0) create a limb ent
  ...
  if (limbTagName) { // add smoke to cap tag
    newBolt = gi.G2API_AddBolt(&limb->ghoul2[limb->playerModel], limbTagName);
    if (newBolt != -1) {
      G_PlayEffect("blaster/smoke_bolton", limb->playerModel, newBolt,
                   limb->s.number);
    }
  }
  /*
  ...
  */
  ...
  animations[limbAnim].firstFrame - 1, animations[limbAnim].numFrames +
  animations[limbAnim].firstFrame - 1, BONE_ANIM_OVERRIDE_FREEZE, 1, cg.time);
  }
  */
  if (limbBone && hitLoc == HL_WAIST &&
      ent->client->NPC_class ==
          CLASS_PROTOCOL) { // play the dismember anim on the limb?
    gi.G2API_StopBoneAnim(&limb->ghoul2[limb->playerModel], "model_root");
    gi.G2API_StopBoneAnim(&limb->ghoul2[limb->playerModel], "motion");
    ...
    // FIXME: screws up origin
    animation_t *animations =
        level.knownAnimFileSets[ent->client->clientInfo.animFileIndex]
            .animations;
    // play the proper dismember anim on the limb
    gi.G2API_SetBoneAnim(
        &limb->ghoul2[limb->playerModel], 0, animations[limbAnim].firstFrame,
        animations[limbAnim].numFrames + animations[limbAnim].firstFrame,
        BONE_ANIM_OVERRIDE_FREEZE, 1, cg.time, -1, -1);
  }
  if (rotateBone) {
    gi.G2API_SetNewOrigin(&limb->ghoul2[0],
                          gi.G2API_AddBolt(&limb->ghoul2[0], rotateBone));
    ...
    if (newBolt != -1) {
      ...
      if ( newBolt != -1 )
      {
              gi.G2API_GetBoltMatrix( ent->ghoul2, ent->playerModel, newBolt,
                                              &boltMatrix, angles,
      ent->currentOrigin, actualTime, NULL, ent->s.modelScale );
  ...
  if (limbCapName) { // turn on caps
    gi.G2API_SetSurfaceOnOff(&limb->ghoul2[limb->playerModel], limbCapName, 0);
  }
  // 3) turn off w/descendants that surf in original model
  // NOTE: we actually change the ent's stuff on the cgame side so that there is
  // no 50ms lag 		also, if the limb was going to start in solid, we can delete it
  //and return
  if (stubTagName) { // add smoke to cap surf, spawn effect
    limb->target = G_NewString(stubTagName);
    /*
    newBolt = gi.G2API_AddBolt( &ent->ghoul2[ent->playerModel], stubTagName );
    if ( newBolt != -1 )
    {
            G_PlayEffect( "blaster/smoke_bolton", ent->playerModel, newBolt,
    ent->s.number);
    }
    */
  }
  if (limbName) {
    ...
    // gi.G2API_SetSurfaceOnOff( &ent->ghoul2[ent->playerModel], limbName,
    // 0x00000100 );//G2SURFACEFLAG_NODESCENDANTS
  }
  if (stubCapName) { // turn on caps
  ...
  if (ent->weaponModel >= 0 &&
      !ent->client->ps
           .saberInFlight) { // the corpse hasn't dropped their weapon
    if (limbAnim == BOTH_DISMEMBER_RARM ||
        limbAnim == BOTH_DISMEMBER_TORSO1) //&& ent->s.weapon == WP_SABER &&
                                           //ent->weaponModel != -1 )
    { // FIXME: is this first check needed with this lower one?
      if (!gi.G2API_GetSurfaceRenderStatus(
              &limb->ghoul2[0],
              "r_hand")) { // only copy the weapon over if the right hand is
                           // actually on this limb...
        // copy it to limb
        if (ent->s.weapon !=
            WP_NONE) { // only if they actually still have a weapon
          limb->s.weapon = ent->s.weapon;
          limb->weaponModel = ent->weaponModel;
        } // else - weaponModel is not -1 but don't have a weapon?  Oops,
          // somehow G2 model wasn't removed?
        // remove it on owner
        if (ent->weaponModel >= 0) {
        ...
        if (ent->client->ps.saberEntityNum != ENTITYNUM_NONE &&
            ent->client->ps.saberEntityNum >
                0) { // remove the owner ent's saber model and entity
  ...
  gi.trace(&trace, limb->s.pos.trBase, limb->mins, limb->maxs,
           limb->s.pos.trBase, limb->s.number, limb->clipmask, G2_NOCOLLIDE, 0);
  if (trace.startsolid) {
    limb->s.pos.trBase[2] -= limb->mins[2];
    gi.trace(&trace, limb->s.pos.trBase, limb->mins, limb->maxs,
             limb->s.pos.trBase, limb->s.number, limb->clipmask, G2_NOCOLLIDE,
             0);
    if (trace.startsolid) {
      limb->s.pos.trBase[2] += limb->mins[2];
      gi.trace(&trace, limb->s.pos.trBase, limb->mins, limb->maxs,
               limb->s.pos.trBase, limb->s.number, limb->clipmask, G2_NOCOLLIDE,
               0);
      if (trace.startsolid) { // stuck?  don't remove
        G_FreeEntity(limb);
        return qfalse;
      }
    }
  ...
  // no trDuration?
  // spin it
  // new way- try to preserve the exact angle and position of the limb as it was
  // when attached
  ...
  */
  ...
  if (hitLoc == HL_HAND_RT || hitLoc == HL_HAND_LT) { // hands fly farther
    ...
    limb->s.apos.trDelta[1] = Q_irand(-800, 800);
             ...
             limbAnim ==
                 BOTH_DISMEMBER_RARM) { // head and arms don't fly as far
  ...
  } else // if ( limbAnim == BOTH_DISMEMBER_TORSO1 || limbAnim ==
         // BOTH_DISMEMBER_LLEG || limbAnim == BOTH_DISMEMBER_RLEG )
  ...
  // limb->s.apos.trDelta[2] = Q_irand( -300, 300 );//FIXME: this scales it down
  // @ 80% and does weird stuff in timescale != 1.0 limb->s.apos.trDelta[2] =
  // limbRoll;
...
static qboolean G_Dismemberable(gentity_t *self, int hitLoc) {
  if (self->client->dismembered) { // cannot dismember me right now
    return qfalse;
  }
  if (g_dismemberment->integer < 11381138 && !g_saberRealisticCombat->integer) {
    if (g_dismemberProbabilities->value >
        0.0f) { // use the ent-specific dismemberProbabilities
      ...
      if (floor((Q_flrand(1, 100) * g_dismemberProbabilities->value)) >
          dismemberProb * 2.0f) // probabilities seemed really really low, had
                                // to crank them up
      {
        return qfalse;
      }
...
static qboolean G_Dismemberable2(gentity_t *self, int hitLoc) {
  ...
  if (g_dismemberment->integer < 11381138 && !g_saberRealisticCombat->integer) {
    if (g_dismemberProbabilities->value <=
        0.0f) { // add the passed-in damage to the locationDamage array, check
                // to see if it's taken enough damage to actually dismember
      if (self->locationDamage[hitLoc] <
          (self->client->ps.stats[STAT_MAX_HEALTH] *
           hitLocHealthPercentage[hitLoc])) { // this location has not taken
                                              // enough damage to dismember
        return qfalse;
      }
    }
  }
...
qboolean G_DoDismemberment(gentity_t *self, vec3_t point, int mod, int damage,
                           int hitLoc, qboolean force = qfalse) {
  extern cvar_t *g_iscensored;
  // dismemberment -- FIXME: should have a check for how long npc has been dead
  // so people can't continue to dismember a dead body long after it's been dead
  // NOTE that you can only cut one thing off unless the dismemberment is >=
  // 11381138
#ifdef GERMAN_CENSORED
  if (0) // germany == censorship
#else
  if (!g_iscensored->integer &&
      (g_dismemberment->integer || g_saberRealisticCombat->integer > 1) &&
  ...
  { // FIXME: don't do strcmps here
    if (G_StandardHumanoid(self->NPC_type) &&
        (force || g_dismemberProbabilities->value > 0.0f ||
         G_Dismemberable2(
             self,
             hitLoc))) { // either it's a forced dismemberment or we're using
                         // probabilities (which are checked before this) or
                         // we've done enough damage to this location
      // FIXME: check the hitLoc and hitDir against the cap tag for the place
      // where the split will be- if the hit dir is roughly perpendicular to
      // the direction of the cap, then the split is allowed, otherwise we
      // hit it at the wrong angle and should not dismember...
      char *limbBone = NULL, *rotateBone = NULL, *limbName = NULL,
           *limbCapName = NULL, *stubCapName = NULL, *limbTagName = NULL,
           *stubTagName = NULL;
      int anim = -1;
      ...
      case HL_BACK_LT:
        if (g_dismemberment->integer &&
            (!self->s.number || !self->message)) { // either the player or not
                                                   // carrying a key on my arm
          doDismemberment = qtrue;
          limbBone = "lhumerus";
          rotateBone = "lradius";
          limbName = "l_arm";
...
static int G_CheckSpecialDeathAnim(gentity_t *self, vec3_t point, int damage,
  ...
  if (PM_InRoll(&self->client->ps)) {
    ...
    float yawDiff = AngleNormalize180(
        AngleNormalize180(self->client->renderInfo.torsoAngles[YAW]) -
  ...
  } else if (PM_InKnockDown(
                 &self->client->ps)) { // since these happen a lot, let's handle
                                       // them case by case
    ...
    case BOTH_KNOCKDOWN1:
      if (animLength - self->client->ps.legsAnimTimer > 100) { // on our way
                                                               // down
        if (self->client->ps.legsAnimTimer > 600) { // still partially up
    ...
    case BOTH_KNOCKDOWN2:
      if (animLength - self->client->ps.legsAnimTimer > 700) { // on our way
                                                               // down
        if (self->client->ps.legsAnimTimer > 600) { // still partially up
          deathAnim = BOTH_DEATH_FALLING_UP;
        } else { // down
    ...
    case BOTH_KNOCKDOWN3:
      if (animLength - self->client->ps.legsAnimTimer > 100) { // on our way
                                                               // down
        if (self->client->ps.legsAnimTimer > 1300) { // still partially up
    ...
    case BOTH_KNOCKDOWN4:
      if (animLength - self->client->ps.legsAnimTimer > 300) { // on our way
                                                               // down
        if (self->client->ps.legsAnimTimer > 350) { // still partially up
          deathAnim = BOTH_DEATH_FALLING_UP;
        } else { // down
          deathAnim = BOTH_DEATH_LYING_UP;
        }
        ...
        if (thrown < -150) {
          deathAnim =
              BOTH_DEATHBACKWARD1; //# Death anim when crouched and thrown back
        } else {
    ...
    case BOTH_KNOCKDOWN5:
      if (self->client->ps.legsAnimTimer < 750) { // flat
        deathAnim = BOTH_DEATH_LYING_DN;
      }
      break;
    case BOTH_GETUP1:
      if (self->client->ps.legsAnimTimer < 350) {        // standing up
      } else if (self->client->ps.legsAnimTimer < 800) { // crouching
        vec3_t fwd;
        AngleVectors(self->currentAngles, fwd, NULL, NULL);
        float thrown = DotProduct(fwd, self->client->ps.velocity);
        if (thrown < -150) {
        ...
        if (animLength - self->client->ps.legsAnimTimer > 450) { // partially up
          deathAnim = BOTH_DEATH_FALLING_UP;
        } else { // down
          deathAnim = BOTH_DEATH_LYING_UP;
        }
      ...
      break;
    case BOTH_GETUP2:
      if (self->client->ps.legsAnimTimer < 150) {        // standing up
      } else if (self->client->ps.legsAnimTimer < 850) { // crouching
        vec3_t fwd;
        AngleVectors(self->currentAngles, fwd, NULL, NULL);
        float thrown = DotProduct(fwd, self->client->ps.velocity);
        if (thrown < -150) {
        ...
        if (animLength - self->client->ps.legsAnimTimer > 500) { // partially up
          deathAnim = BOTH_DEATH_FALLING_UP;
        } else { // down
          deathAnim = BOTH_DEATH_LYING_UP;
        }
      ...
      break;
    case BOTH_GETUP3:
      if (self->client->ps.legsAnimTimer < 250) {        // standing up
      } else if (self->client->ps.legsAnimTimer < 600) { // crouching
        vec3_t fwd;
        AngleVectors(self->currentAngles, fwd, NULL, NULL);
        float thrown = DotProduct(fwd, self->client->ps.velocity);
        if (thrown < -150) {
        ...
        if (animLength - self->client->ps.legsAnimTimer > 150) { // partially up
          deathAnim = BOTH_DEATH_FALLING_DN;
        } else { // down
          deathAnim = BOTH_DEATH_LYING_DN;
        }
    ...
    case BOTH_GETUP4:
      if (self->client->ps.legsAnimTimer < 250) {        // standing up
        ...
        if (animLength - self->client->ps.legsAnimTimer > 850) { // partially up
          deathAnim = BOTH_DEATH_FALLING_DN;
        } else { // down
          deathAnim = BOTH_DEATH_LYING_UP;
        }
      ...
      break;
    case BOTH_GETUP5:
      if (self->client->ps.legsAnimTimer > 850) {                 // lying down
        if (animLength - self->client->ps.legsAnimTimer > 1500) { // partially
                                                                  // up
          deathAnim = BOTH_DEATH_FALLING_DN;
        } else { // down
          deathAnim = BOTH_DEATH_LYING_DN;
    ...
    case BOTH_GETUP_CROUCH_B1:
      if (self->client->ps.legsAnimTimer < 800) { // crouching
        vec3_t fwd;
        AngleVectors(self->currentAngles, fwd, NULL, NULL);
        float thrown = DotProduct(fwd, self->client->ps.velocity);
        if (thrown < -150) {
        ...
        if (animLength - self->client->ps.legsAnimTimer > 400) { // partially up
          deathAnim = BOTH_DEATH_FALLING_UP;
        } else { // down
          deathAnim = BOTH_DEATH_LYING_UP;
        }
    ...
    case BOTH_FORCE_GETUP_B1:
      if (self->client->ps.legsAnimTimer < 325) {        // standing up
      } else if (self->client->ps.legsAnimTimer < 725) { // spinning up
        deathAnim = BOTH_DEATH_SPIN_180; //# Death anim when facing backwards
      } else if (self->client->ps.legsAnimTimer < 900) { // crouching
        vec3_t fwd;
        AngleVectors(self->currentAngles, fwd, NULL, NULL);
        float thrown = DotProduct(fwd, self->client->ps.velocity);
        if (thrown < -150) {
        ...
        if (animLength - self->client->ps.legsAnimTimer > 50) { // partially up
          deathAnim = BOTH_DEATH_FALLING_UP;
        } else { // down
          deathAnim = BOTH_DEATH_LYING_UP;
        }
    ...
    case BOTH_FORCE_GETUP_B2:
      if (self->client->ps.legsAnimTimer < 575) {        // standing up
      } else if (self->client->ps.legsAnimTimer < 875) { // spinning up
        deathAnim = BOTH_DEATH_SPIN_180; //# Death anim when facing backwards
      } else if (self->client->ps.legsAnimTimer < 900) { // crouching
        vec3_t fwd;
        AngleVectors(self->currentAngles, fwd, NULL, NULL);
    ...
    case BOTH_FORCE_GETUP_B3:
      if (self->client->ps.legsAnimTimer < 150) {        // standing up
      } else if (self->client->ps.legsAnimTimer < 775) { // flipping
    ...
    case BOTH_FORCE_GETUP_B5:
      if (self->client->ps.legsAnimTimer < 550) {               // standing up
      } else if (self->client->ps.legsAnimTimer < 1025) {       // kicking up
        deathAnim = BOTH_DEATHBACKWARD2;                        // backflip
      } else {                                                  // lying down
        if (animLength - self->client->ps.legsAnimTimer > 50) { // partially up
          deathAnim = BOTH_DEATH_FALLING_UP;
    ...
    case BOTH_FORCE_GETUP_B6:
      if (self->client->ps.legsAnimTimer < 225) {        // standing up
      } else if (self->client->ps.legsAnimTimer < 425) { // crouching up
        vec3_t fwd;
        AngleVectors(self->currentAngles, fwd, NULL, NULL);
        float thrown = DotProduct(fwd, self->client->ps.velocity);
        if (thrown < -150) {
      ...
      } else if (self->client->ps.legsAnimTimer < 825) {         // flipping up
        deathAnim = BOTH_DEATHFORWARD3;                          // backflip
      } else {                                                   // lying down
        if (animLength - self->client->ps.legsAnimTimer > 225) { // partially up
          deathAnim = BOTH_DEATH_FALLING_UP;
        } else { // down
          deathAnim = BOTH_DEATH_LYING_UP;
        }
    ...
    case BOTH_FORCE_GETUP_F1:
      if (self->client->ps.legsAnimTimer < 275) {        // standing up
      } else if (self->client->ps.legsAnimTimer < 750) { // flipping
        deathAnim = BOTH_DEATH14;
      } else {                                                   // lying down
        if (animLength - self->client->ps.legsAnimTimer > 100) { // partially up
          deathAnim = BOTH_DEATH_FALLING_DN;
        } else { // down
          deathAnim = BOTH_DEATH_LYING_DN;
        }
    ...
    case BOTH_FORCE_GETUP_F2:
      if (self->client->ps.legsAnimTimer < 1200) {               // standing
      } else {                                                   // lying down
        if (animLength - self->client->ps.legsAnimTimer > 225) { // partially up
          deathAnim = BOTH_DEATH_FALLING_DN;
        } else { // down
...
static int
G_PickDeathAnim(gentity_t *self, vec3_t point, int damage, int mod,
                int hitLoc) { // FIXME: play dead flop anims on body if in an
                              // appropriate _DEAD anim when this func is called
  int deathAnim = -1;
  if (hitLoc == HL_NONE) {
    hitLoc = G_GetHitLocation(self, point); // self->hitLoc
  }
  // dead flops...if you are already playing a death animation, I guess it can
...
case BOTH_DEADFLOP2:
  ...
  // Not currently playing a death animation, so try and get an appropriate one
  // now.
  if (deathAnim == -1) {
    deathAnim = G_CheckSpecialDeathAnim(self, point, damage, mod, hitLoc);

    if (deathAnim == -1) { // base on hitLoc
      vec3_t fwd;
      AngleVectors(self->currentAngles, fwd, NULL, NULL);
      float thrown = DotProduct(fwd, self->client->ps.velocity);
      // death anims
      ...
      case HL_ARM_RT:
      case HL_BACK_LT:
        if ((damage <= self->max_health * 0.25 && Q_irand(0, 1)) ||
            (fabs(thrown) < 200 && !Q_irand(0, 2)) || !Q_irand(0, 10)) {
          if (Q_irand(0, 1)) {
            deathAnim = BOTH_DEATH9; // chest right: snap, fall forward
          } else {
            deathAnim = BOTH_DEATH20; // chest right: snap, fall forward
          }
        } else if ((damage <= self->max_health * 0.5 && Q_irand(0, 1)) ||
                   !Q_irand(0, 10)) {
          deathAnim = BOTH_DEATH3; // chest right: back
        } else if ((damage <= self->max_health * 0.75 && Q_irand(0, 1)) ||
                   !Q_irand(0, 10)) {
...
int G_CheckLedgeDive(gentity_t *self, float checkDist, vec3_t checkVel,
                     qboolean tryOpposite, qboolean tryPerp) {
  //		Intelligent Ledge-Diving Deaths:
  //		If I'm an NPC, check for nearby ledges and fall off it if
  //possible 		How should/would/could this interact with knockback if we already
  //have some? 		Ideally - apply knockback if there are no ledges or a ledge in
  //that dir 		But if there is a ledge and it's not in the dir of my knockback,
  //fall off the ledge instead
  if (!self || !self->client) {
    return 0;
  }
  ...
  if (!cliff_fall && tryPerp) { // try sides
...
void player_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
  ...
  if (self->client->ps.pm_type == PM_DEAD &&
      (meansOfDeath != MOD_SNIPER ||
       (self->flags & FL_DISINTEGRATED))) { // do dismemberment/twitching
    if (self->client->NPC_class == CLASS_MARK1) {
      DeathFX(self);
      self->takedamage = qfalse;
      self->client->ps.eFlags |= EF_NODRAW;
      self->contents = 0;
      // G_FreeEntity( self ); // Is this safe?  I can't see why we'd mark it
      // nodraw and then just leave it around??
  ...
  if (self->NPC) {
    ...
    if (self->s.eFlags & EF_LOCKED_TO_WEAPON) {
      ...
      if (self->owner) {
        self->owner->s.frame = self->owner->startFrame = self->owner->endFrame =
            0;
  ...
  if (self->client->ps.saberEntityNum != ENTITYNUM_NONE &&
    ...
    if (self->client->ps.saberInFlight) { // just drop it
      self->client->ps.saberActive = qfalse;
    } else {
      if ((hitLoc != HL_HAND_RT || self->client->dismembered ||
           meansOfDeath !=
               MOD_SABER) // if might get hand cut off, leave saber in hand
          && holdingSaber &&
          (Q_irand(0, 1) || meansOfDeath == MOD_EXPLOSIVE ||
           meansOfDeath == MOD_REPEATER_ALT ||
           meansOfDeath == MOD_FLECHETTE_ALT || meansOfDeath == MOD_ROCKET ||
           meansOfDeath == MOD_ROCKET_ALT || meansOfDeath == MOD_THERMAL ||
           meansOfDeath == MOD_THERMAL_ALT || meansOfDeath == MOD_DETPACK ||
           meansOfDeath == MOD_LASERTRIP || meansOfDeath == MOD_LASERTRIP_ALT ||
           meansOfDeath == MOD_MELEE || meansOfDeath == MOD_FORCE_GRIP ||
           meansOfDeath == MOD_KNOCKOUT || meansOfDeath == MOD_CRUSH ||
           meansOfDeath == MOD_IMPACT || meansOfDeath == MOD_FALLING ||
           meansOfDeath == MOD_EXPLOSIVE_SPLASH)) { // drop it
  ...
  if (self->client->NPC_class == CLASS_SHADOWTROOPER) { // drop a force crystal
    gitem_t *item;
  ...
  if (attacker) {
    if (attacker->client && !attacker->s.number) {
      if (self->client) { // killed a client
        if (self->client->playerTeam == TEAM_ENEMY ||
            (self->NPC &&
             self->NPC->charmedTime > level.time)) { // killed an enemy
    ...
    if (d_slowmodeath->integer) {
      if (!self->s.number) { // what the hell, always do slow-mo when player
                             // dies
        // FIXME: don't do this when crushed to death?
        if (meansOfDeath == MOD_FALLING &&
            self->client->ps.groundEntityNum ==
                ENTITYNUM_NONE) { // falling to death, have not hit yet
          G_StartMatrixEffect(self, qtrue, 10000);
        } else if (meansOfDeath !=
                   MOD_CRUSH) { // for all deaths except being crushed
          G_StartMatrixEffect(self);
        }
      } else if (d_slowmodeath->integer < 4) { // any jedi killed by
                                               // player-saber
        if (d_slowmodeath->integer < 3) { // must be the last jedi in the room
        ...
        if (!attacker->s.number && holdingSaber && meansOfDeath == MOD_SABER &&
            attacker->client && attacker->client->ps.weapon == WP_SABER &&
            !attacker->client->ps.saberInFlight &&
            (d_slowmodeath->integer > 2 ||
             lastInGroup)) // either slow mo death level 3 (any jedi) or 2 and I
                           // was the last jedi in the room
        { // Matrix!
          G_StartMatrixEffect(self);
        }
      } else { // all player-saber kills
        if (!attacker->s.number && meansOfDeath == MOD_SABER &&
            attacker->client && attacker->client->ps.weapon == WP_SABER &&
            !attacker->client->ps.saberInFlight &&
            (d_slowmodeath->integer > 4 || lastInGroup ||
             holdingSaber)) // either slow mo death level 5 (any enemy) or 4 and
                            // I was the last in my group or I'm a saber user
        { // Matrix!
          G_StartMatrixEffect(self);
        }
      }
  ...
  if (self->client->playerTeam ==
      TEAM_PLAYER) { // FIXME: just HazTeam members in formation on away
                     // missions?
    // or more controlled- via deathscripts?
    // Don't count player
    if ((&g_entities[0] != NULL && g_entities[0].client) &&
        (self->s.number != 0)) { // add to the number of teammates lost
  ...
  if (self->s.number == 0 && attacker) {
    ...
    // TEST: If player killed, unmark all teammates from being undying so they
    // can buy it too NOTE: we want this to happen ONLY on our squad ONLY on
    // missions... in the tutorial or on voyager levels this could be really
    // weird.
  ...
  if (!holdingSaber
      //&& self->s.number != 0
      && !(contents & CONTENTS_NODROP) && meansOfDeath != MOD_SNIPER &&
      (!self->client || self->client->NPC_class != CLASS_GALAKMECH)) {
    TossClientItems(self);
  }

  if (meansOfDeath == MOD_SNIPER) { // I was disintegrated
    if (self->message) {            // I was holding a key
      // drop the key
      G_DropKey(self);
    }
  }

  if (holdingSaber) { // never drop a lightsaber!
    if (self->client->ps.saberActive) {
      ...
      if (self->client->playerTeam == TEAM_PLAYER) {
        ...
        G_SoundOnEnt(self, CHAN_AUTO,
                     "sound/weapons/saber/enemy_saber_off.wav");
      }
    }
  } else if (self->s.weapon !=
             WP_BLASTER_PISTOL) { // Sigh...borg shouldn't drop their weapon
                                  // attachments when they die..
    self->s.weapon = WP_NONE;
  ...
  self->s.powerups &=
      ~PW_REMOVE_AT_DEATH; // removes everything but electricity and force push

  // FIXME: do this on a callback?  So people can't walk through long death
  // anims? Maybe set on last frame?  Would be cool for big blocking corpses if
  // the never got set? self->contents = CONTENTS_CORPSE;//now done a second
  // after death
  /*
  ...
  */
  ...
  self->clipmask &= ~(CONTENTS_MONSTERCLIP |
                      CONTENTS_BOTCLIP); // so dead NPC can fly off ledges
  ...
  if (player && player->client &&
      player->client->ps.viewEntity ==
          self->s.number) { // I was the player's viewentity and I died, kick
                            // him back to his normal view
    G_ClearViewEntity(player);
  } else if (!self->s.number && self->client->ps.viewEntity > 0 &&
  ...
  if (self->client->NPC_class == CLASS_MARK1) {
    ...
    Interrogator_die(self, inflictor, attacker, damage, meansOfDeath, dflags,
                     hitLoc);
  } else if (self->client->NPC_class ==
             CLASS_GALAKMECH) { // FIXME: need keyframed explosions?
    NPC_SetAnim(self, SETANIM_BOTH, BOTH_DEATH1,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    G_AddEvent(self, Q_irand(EV_DEATH1, EV_DEATH3), self->health);
  } else if (self->client->NPC_class ==
             CLASS_ATST) { // FIXME: need keyframed explosions
    if (!self->s.number) {
      G_DrivableATSTDie(self);
    }
    anim =
        PM_PickAnim(self, BOTH_DEATH1, BOTH_DEATH25); // initialize to good data
    if (anim != -1) {
      NPC_SetAnim(self, SETANIM_BOTH, anim,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    }
  } else if (self->s.number && self->message &&
             meansOfDeath != MOD_SNIPER) { // imp with a key on his arm
    ...
    // FIXME: no good way to predict you're going to fall to your death... need
    // falling bushes/triggers?
    if (self->client->ps.groundEntityNum == ENTITYNUM_NONE // in the air
        && self->client->ps.velocity[2] < 0                // falling
        && self->client->ps.legsAnim !=
               BOTH_FALLDEATH1INAIR // not already in falling loop
        && self->client->ps.torsoAnim !=
               BOTH_FALLDEATH1INAIR) // not already in falling loop
    {
      ...
      if (PM_InOnGroundAnim(&self->client->ps)) {
        if (AngleNormalize180(self->client->renderInfo.torsoAngles[PITCH]) <
            0) {
          deathAnim = BOTH_DEATH_LYING_UP; //# Death anim when lying on back
        } else {
          deathAnim = BOTH_DEATH_LYING_DN; //# Death anim when lying on front
      ...
      NPC_SetAnim(self, SETANIM_BOTH, deathAnim,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      // HMM: check for nodrop?
      G_SoundOnEnt(self, CHAN_BODY, "sound/player/fallsplat.wav");
      if (gi.VoiceVolume[self->s.number] && self->NPC &&
          (self->NPC->aiFlags &
    ...
    anim =
        G_CheckSpecialDeathAnim(self, self->pos1, damage, meansOfDeath, hitLoc);
    if (anim == -1) {
      if (self->s.legsAnim == BOTH_RESTRAINED1 ||
          self->s.legsAnim ==
              BOTH_RESTRAINED1POINT) { // super special case, floating in air
                                       // lying down
        anim = BOTH_RESTRAINED1;
      } else if (PM_InOnGroundAnim(&self->client->ps) &&
                 PM_HasAnimation(self,
                                 BOTH_LYINGDEATH1)) { // on ground, need
                                                      // different death anim
        anim = BOTH_LYINGDEATH1;
      } else if (meansOfDeath == MOD_TRIGGER_HURT &&
                 (self->s.powerups & (1 << PW_SHOCKED))) { // electrocuted
        anim = BOTH_DEATH17;
      } else if (meansOfDeath == MOD_WATER) { // drowned
        anim = BOTH_DEATH17;
      } else if (meansOfDeath != MOD_SNIPER) {
        cliff_fall = G_CheckLedgeDive(self, 128, self->client->ps.velocity,
                                      qtrue, qfalse);
        if (cliff_fall == 2) {
          if (!FlyingCreature(self) && g_gravity->value > 0) {
            if (!self->NPC) {
              G_SoundOnEnt(self, CHAN_VOICE,
                           "*falling1.wav"); // CHAN_VOICE_ATTEN
            } else if (!(self->NPC->aiFlags & NPCAI_DIE_ON_IMPACT)) {
              G_SoundOnEnt(self, CHAN_VOICE,
                           "*falling1.wav"); // CHAN_VOICE_ATTEN
              self->NPC->aiFlags |= NPCAI_DIE_ON_IMPACT;
        ...
        if (self->client->ps.pm_time > 0 &&
          ...
          if (thrown > 100) {
            ...
            } else { // falling to one of the sides
              if (cliff_fall == 2 && PM_HasAnimation(self, BOTH_FALLDEATH1)) {
                anim = BOTH_FALLDEATH1;
                if (self->client->ps.velocity[2] > 0 &&
                    self->client->ps.velocity[2] < 100) {
    ...
    if (anim == -1) {
      if (meansOfDeath == MOD_ELECTROCUTE ||
          (meansOfDeath == MOD_CRUSH &&
           self->s.eFlags &
               EF_FORCE_GRIPPED)) { // electrocuted or choked to death
        anim = BOTH_DEATH17;
      } else {
        anim = G_PickDeathAnim(self, self->pos1, damage, meansOfDeath, hitLoc);
      }
    ...
    if (anim == -1) {
      anim = PM_PickAnim(self, BOTH_DEATH1,
                         BOTH_DEATH25); // initialize to good data
    ...
    if (meansOfDeath == MOD_KNOCKOUT) {
      ...
      if (self->NPC) { // stick around for a while
      ...
      if (self->NPC) {
        // need to pad deathtime some to stick around long enough for death
        // effect to play
        self->NPC->timeOfDeath = level.time + 2000;
      }
    } else {
      if (hitLoc == HL_HEAD && !(dflags & DAMAGE_RADIUS) &&
          meansOfDeath != MOD_REPEATER_ALT &&
          meansOfDeath != MOD_FLECHETTE_ALT && meansOfDeath != MOD_ROCKET &&
          meansOfDeath != MOD_ROCKET_ALT && meansOfDeath != MOD_THERMAL &&
          meansOfDeath != MOD_THERMAL_ALT && meansOfDeath != MOD_DETPACK &&
          ...
          meansOfDeath !=
              MOD_EXPLOSIVE_SPLASH) { // no sound when killed by headshot
                                      // (explosions don't count)
        G_AlertTeam(self, attacker, 512, 0);
        if (gi.VoiceVolume[self->s.number]) { // I was talking, so cut it off...
                                              // with a jump sound?
          G_SoundOnEnt(self, CHAN_VOICE, "*jump1.wav");
        }
      } else {
        if ((self->client->ps.eFlags &
             EF_FORCE_GRIPPED)) { // killed while gripped - no loud scream
          G_AlertTeam(self, attacker, 512, 32);
        ...
        } else { // screaming death is louder
          G_AlertTeam(self, attacker, 512, 1024);
        }
      }
    }

    if (attacker && attacker->s.number == 0) { // killed by player
      ...
      // FIXME: danger event so that others will run away from this area since
  ...
  if ((dflags & DAMAGE_DISMEMBER) &&
      G_DoDismemberment(self, self->pos1, meansOfDeath, damage, hitLoc) &&
      !specialAnim) { // we did dismemberment and our death anim is okay to
                      // override
    if (hitLoc == HL_HAND_RT && self->locationDamage[hitLoc] >= Q3_INFINITE &&
        cliff_fall != 2 &&
        self->client->ps.groundEntityNum !=
            ENTITYNUM_NONE) { // just lost our right hand and we're on the
                              // ground, use the special anim
  ...
  self->client->respawnTime =
      level.time + 2000; // self->client->ps.legsAnimTimer;
  ...
  // FIXME: This may screw up certain things that expect to float even while
  // dead <?>
  self->svFlags &= ~SVF_CUSTOM_GRAVITY;

  self->client->ps.pm_type = PM_DEAD;
  // need to update STAT_HEALTH here because ClientThink_real for self may
  // happen before STAT_HEALTH is updated from self->health and pmove will stomp
  // death anim with a move anim
  self->client->ps.stats[STAT_HEALTH] = self->health;

  if (self->NPC) { // If an NPC, make sure we start running our scripts again-
                   // this gets set to infinite while we fall to our deaths
  ...
  if (self->NPC && (self->NPC->scriptFlags & SCF_FFDEATH)) {
    if (G_ActivateBehavior(
            self, BSET_FFDEATH)) { // FIXME: should running this preclude
                                   // running the normal deathscript?
      deathScript = qtrue;
    }
  ...
  self->bounceCount = -1; // This is a cheap hack for optimizing the
                          // pointcontents check in deadthink
  if (self->NPC) {
    self->NPC->timeOfDeath =
        level.time; // this will change - used for debouncing post-death events
    self->s.time =
        level.time; // this will not chage- this is actual time of death
  }
...
qboolean G_CheckForStrongAttackMomentum(
    gentity_t *self) { // see if our saber attack has too much momentum to be
                       // interrupted
  if (PM_PowerLevelForSaberAnim(&self->client->ps) >
      FORCE_LEVEL_2) { // strong attacks can't be interrupted
    if (PM_InAnimForSaberMove(
            self->client->ps.torsoAnim,
            self->client->ps
                .saberMove)) { // our saberMove was not already interupted by
                               // some other anim (like pain)
      if (PM_SaberInStart(self->client->ps.saberMove)) {
        float animLength =
            PM_AnimLength(self->client->clientInfo.animFileIndex,
                          (animNumber_t)self->client->ps.torsoAnim);
        if (animLength - self->client->ps.torsoAnimTimer >
            750) { // start anim is already 3/4 of a second into it, can't
                   // interrupt it now
          return qtrue;
        }
      } else if (PM_SaberInReturn(self->client->ps.saberMove)) {
        if (self->client->ps.torsoAnimTimer >
            750) { // still have a good amount of time left in the return anim,
                   // can't interrupt it
          return qtrue;
        }
      } else { // cannot interrupt actual transitions and attacks
        return qtrue;
      }
    }
  }
...
void PlayerPain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
                vec3_t point, int damage, int mod, int hitLoc) {
  if (self->client->NPC_class ==
      CLASS_ATST) { // different kind of pain checking altogether
    G_ATSTCheckPain(self, other, point, damage, mod, hitLoc);
    int blasterTest = gi.G2API_GetSurfaceRenderStatus(
        &self->ghoul2[self->playerModel], "head_light_blaster_cann");
    int chargerTest = gi.G2API_GetSurfaceRenderStatus(
        &self->ghoul2[self->playerModel], "head_concussion_charger");
    if (blasterTest && chargerTest) { // lost both side guns
    ...
    if (level.time > self->painDebounceTime &&
        !(self->flags &
          FL_GODMODE)) { // first time hit this frame and not in godmode
      self->client->ps.damageEvent++;
      if (!Q3_TaskIDPending(self, TID_CHAN_VOICE)) {
        if (self->client->damage_blood) { // took damage myself, not just armor
          G_AddEvent(self, EV_PAIN, self->health);
        }
      }
    }
    if (damage != -1 &&
        (mod == MOD_MELEE || damage == 0 /*fake damage*/ ||
         (Q_irand(0, 10) <= damage &&
          self->client->damage_blood))) { //-1 == don't play pain anim
      if ((((mod == MOD_SABER || mod == MOD_MELEE) &&
            self->client->damage_blood) ||
           mod == MOD_CRUSH) &&
          (self->s.weapon == WP_SABER ||
           self->s.weapon ==
               WP_MELEE)) // FIXME: not only if using saber, but if in third
                          // person at all?  But then 1st/third person
      ...
      { // FIXME: only strong-level saber attacks should make me play pain anim?
        if (!G_CheckForStrongAttackMomentum(self) &&
            ...
            !PM_InKnockDown(
                &self->client
                     ->ps)) { // strong attacks and spins cannot be interrupted
                              // by pain, no pain when in knockdown
          ...
          if (self->client->ps.groundEntityNum != ENTITYNUM_NONE &&
              ...
              !PM_RunningAnim(
                  self->client->ps
                      .legsAnim)) { // if on a surface and not in a spin or
                                    // flip, play full body pain
            parts = SETANIM_BOTH;
          } else { // play pain just in torso
            parts = SETANIM_TORSO;
          }
          if (self->painDebounceTime < level.time) {
            // temp HACK: these are the only 2 pain anims that look good when
            // holding a saber
            NPC_SetAnim(self, parts, PM_PickAnim(self, BOTH_PAIN2, BOTH_PAIN3),
                        SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
            self->client->ps.saberMove =
                LS_READY; // don't finish whatever saber move you may have been
                          // in
            // WTF - insn't working
          ...
          if (parts == SETANIM_BOTH &&
              (damage > 30 ||
               (self->painDebounceTime > level.time &&
                damage > 10))) { // took a lot of damage in 1 hit //or took 2
                                 // hits in quick succession
            self->aimDebounceTime =
...
int CheckArmor(gentity_t *ent, int damage, int dflags) {
  ...
  if (client->NPC_class == CLASS_GALAKMECH) { // special case
    if (client->ps.stats[STAT_ARMOR] <= 0) {  // no shields
      client->ps.powerups[PW_GALAK_SHIELD] = 0;
      return 0;
    } else { // shields take all the damage
    ...
    if (count > (client->ps.stats[STAT_MAX_HEALTH] /
                 2)) // MAX_HEALTH is considered max armor. Or so I'm told.
    {
      save = damage;
    } else {
      if (!ent->s.number &&
          client->NPC_class ==
              CLASS_ATST) { // player in ATST... armor takes *all* the damage
        save = damage;
      } else {
...
void G_Knockdown(gentity_t *self, gentity_t *attacker, vec3_t pushDir,
                 float strength, qboolean breakSaberLock) {
  ...
  if (self->health > 0) {
    if (!self->s.number) {
      ...
      GEntity_PainFunc(self, attacker, attacker, self->currentOrigin, 0,
                       MOD_MELEE);
    }
    ...
    if (!PM_SpinningSaberAnim(self->client->ps.legsAnim) &&
      ...
      if (!self->s.number &&
          (!g_spskill->integer ||
           strength < 300)) { // player only knocked down if pushed *hard*
        return;
      } else if (PM_CrouchAnim(
                     self->client->ps.legsAnim)) { // crouched knockdown
        ...
        if (DotProduct(pLFwd, pushDir) > 0.2f) { // pushing him from behind
          knockAnim = BOTH_KNOCKDOWN3;
        } else { // pushing him from front
          knockAnim = BOTH_KNOCKDOWN1;
        }
      }
      if (knockAnim == BOTH_KNOCKDOWN1 && strength > 150) { // push *hard*
        knockAnim = BOTH_KNOCKDOWN2;
      }
      NPC_SetAnim(self, SETANIM_BOTH, knockAnim,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void G_CheckKnockdown(gentity_t *targ, gentity_t *attacker, vec3_t newDir,
                      int dflags, int mod) {
  if (!targ || !attacker) {
    return;
  }
  if (!(dflags & DAMAGE_RADIUS)) { // not inherently explosive damage, check mod
    if (mod != MOD_REPEATER_ALT && mod != MOD_FLECHETTE_ALT &&
        mod != MOD_ROCKET && mod != MOD_ROCKET_ALT && mod != MOD_THERMAL &&
        mod != MOD_THERMAL_ALT && mod != MOD_DETPACK && mod != MOD_LASERTRIP &&
        mod != MOD_LASERTRIP_ALT && mod != MOD_EXPLOSIVE &&
        mod != MOD_EXPLOSIVE_SPLASH) {
      return;
    }
  }

  if (!targ->client || targ->client->NPC_class == CLASS_PROTOCOL ||
      !G_StandardHumanoid(targ->NPC_type)) {
    return;
  }

  if (targ->client->ps.groundEntityNum == ENTITYNUM_NONE) { // already in air
    return;
  }

  if (!targ->s.number) {       // player less likely to be knocked down
    if (!g_spskill->integer) { // never in easy
      return;
    }
    if (!cg.renderingThirdPerson ||
        cg.zoomMode) { // never if not in chase camera view (so more likely with
                       // saber out)
      return;
    }
    if (g_spskill->integer == 1) { // 33% chance on medium
      if (Q_irand(0, 2)) {
        return;
      }
    } else { // 50% chance on hard
      if (Q_irand(0, 1)) {
        return;
      }
    }
...
void G_ApplyKnockback(gentity_t *targ, vec3_t newDir, float knockback) {
  ...
  if (g_gravity->value > 0) {
    VectorScale(newDir, g_knockback->value * (float)knockback / mass * 0.8,
                kvel);
    kvel[2] =
        newDir[2] * (g_knockback->value * (float)knockback) / (mass * 1.5) + 20;
  } else {
...
static int G_CheckForLedge(gentity_t *self, vec3_t fallCheckDir,
  ...
  // Should have clip burshes masked out by now and have bbox resized to death
  // size
  gi.trace(&tr, self->currentOrigin, self->mins, self->maxs, end,
           self->s.number, self->clipmask, G2_NOCOLLIDE, 0);
  ...
  gi.trace(&tr, start, self->mins, self->maxs, end, self->s.number,
           self->clipmask, G2_NOCOLLIDE, 0);
  if (tr.allsolid || tr.startsolid) {
    return 0;
  }
  if (tr.fraction >= 1.0) {
...
static void G_FriendlyFireReaction(gentity_t *self, gentity_t *other,
                                   int dflags) {
  if ((!player->client->ps.viewEntity ||
       other->s.number != player->client->ps.viewEntity)) { // hit by a teammate
    if (other != self->enemy &&
        self != other->enemy) { // we weren't already enemies
      if (self->enemy || other->enemy ||
          (other->s.number &&
           other->s.number !=
               player->client->ps
                   .viewEntity)) { // if one of us actually has an enemy
                                   // already, it's okay, just an accident OR
                                   // wasn't hit by player or someone controlled
                                   // by player OR player hit ally and didn't get
                                   // 25% chance of getting mad (FIXME:accumulate
                                   // anger+base on diff?)
        return;
...
void G_TrackWeaponUsage(gentity_t *self, gentity_t *inflictor, int add,
                        int mod) {
  if (!self || !self->client || self->s.number) { // player only
    return;
  }
  int weapon = WP_NONE;
  // FIXME: need to check the MOD to find out what weapon (if *any*) actually
  // did the killing
  if (inflictor && !inflictor->client && mod != MOD_SABER &&
      inflictor->lastEnemy &&
      inflictor->lastEnemy != self) { // a missile that was reflected, ie: not
                                      // owned by me originally
    if (inflictor->owner == self &&
        self->s.weapon == WP_SABER) { // we reflected it
      weapon = WP_SABER;
    }
  }
  if (weapon == WP_NONE) {
...
qboolean G_NonLocationSpecificDamage(int meansOfDeath) {
  if (meansOfDeath == MOD_EXPLOSIVE || meansOfDeath == MOD_REPEATER_ALT ||
      meansOfDeath == MOD_FLECHETTE_ALT || meansOfDeath == MOD_ROCKET ||
      meansOfDeath == MOD_ROCKET_ALT || meansOfDeath == MOD_THERMAL ||
      meansOfDeath == MOD_THERMAL_ALT || meansOfDeath == MOD_DETPACK ||
      meansOfDeath == MOD_LASERTRIP || meansOfDeath == MOD_LASERTRIP_ALT ||
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
  ...
  if (targ->health <= 0 && !targ->client) { // allow corpses to be disintegrated
    if (mod != MOD_SNIPER || (targ->flags & FL_DISINTEGRATED))
      return;
  }

  // if we are the player and we are locked to an emplaced gun, we have to
  // reroute damage to the gun....sigh.
  if (targ->s.eFlags & EF_LOCKED_TO_WEAPON && targ->s.number == 0 &&
      targ->owner && !(targ->owner->flags & FL_GODMODE)) {
    // swapping the gun into our place to absorb our damage
    targ = targ->owner;
  }

  if ((targ->flags & FL_SHIELDED) && mod != MOD_SABER &&
      !targ->client) { // magnetically protected, this thing can only be damaged
                       // by lightsabers
    return;
  }
  ...
  if ((targ->flags & FL_DMG_BY_SABER_ONLY) &&
      mod !=
          MOD_SABER) { // can only be damaged by lightsabers (but no shield...
                       // yeah, it's redundant, but whattayagonnado?)
    return;
  }

  if ((targ->flags & FL_DMG_BY_HEAVY_WEAP_ONLY) &&
      !(dflags & DAMAGE_HEAVY_WEAP_CLASS)) {
    // can only be damaged by an heavy type weapon...but impacting missile was
    // in the heavy weap class...so we just aren't taking damage from this
    // missile
    return;
  }
  ...
  if (mod == MOD_SABER) { // sabers do less damage to mark1's and atst's
    if (targ->client && (targ->client->NPC_class == CLASS_ATST ||
  ...
  //	if ( attacker->s.number != 0 && damage >= 2 && targ->s.number != 0 &&
  //attacker->client && attacker->client->playerTeam == TEAM_PLAYER )
  ...
  if (targ->s.number != 0) { // not the player
    if ((targ->flags & FL_GODMODE) ||
        (targ->flags &
         FL_UNDYING)) { // have god or undying on, so ignore no protection flag
  ...
  if (!attacker->s.number && targ->client && attacker->client &&
      targ->client->playerTeam ==
          attacker->client->playerTeam) { // player doesn't do knockback against
                                          // allies unless he kills them
    dflags |= DAMAGE_DEATH_KNOCKBACK;
  }

  if (client && client->NPC_class == CLASS_GALAKMECH) { // hit Galak
    if (client->ps.stats[STAT_ARMOR] > 0) {             // shields are up
      dflags &= ~DAMAGE_NO_ARMOR;                       // always affect armor
      if (mod == MOD_ELECTROCUTE || mod == MOD_DEMP2 ||
          mod == MOD_DEMP2_ALT) { // shield protects us from this
        damage = 0;
      }
    } else { // shields down
      if (mod == MOD_MELEE ||
          (mod == MOD_CRUSH && attacker &&
           attacker->client)) { // Galak takes no impact damage
        return;
      }
      if ((dflags & DAMAGE_RADIUS) || mod == MOD_REPEATER_ALT ||
          mod == MOD_FLECHETTE_ALT || mod == MOD_ROCKET ||
          mod == MOD_ROCKET_ALT || mod == MOD_THERMAL ||
          mod == MOD_THERMAL_ALT || mod == MOD_DETPACK ||
          mod == MOD_LASERTRIP || mod == MOD_LASERTRIP_ALT ||
          mod == MOD_EXPLOSIVE_SPLASH || mod == MOD_ENERGY_SPLASH ||
          mod == MOD_SABER) { // galak without shields takes quarter damage from
                              // explosives and lightsaber
  ...
  if (targ->flags & FL_GODMODE && !(dflags & DAMAGE_NO_PROTECTION)) {
    if (targ->client && attacker->client &&
        targ->client->playerTeam == attacker->client->playerTeam &&
        (!targ->NPC ||
         !targ->NPC->charmedTime)) { // complain, but don't turn on them
  ...
  /*
  if ( targ != attacker && !(dflags&DAMAGE_IGNORE_TEAM) && OnSameTeam (targ,
  attacker)  )
  {//on same team
          if ( !targ->client )
          {//a non-player object should never take damage from an ent on the
  same team return;
          }

          if ( attacker->client && attacker->client->playerTeam ==
  targ->noDamageTeam )
          {//NPC or player shot an object on his own team
                  return;
          }

          if ( attacker->s.number != 0 && targ->s.number != 0 &&//player not
  involved in any way in this exchange attacker->client && targ->client &&//two
  NPCs attacker->client->playerTeam == targ->client->playerTeam ) //on the same
  team
          ...
          if ( targ->s.number == 0 &&//player was hit
                  ...
                  attacker->client->playerTeam == TEAM_PLAYER ) //on the same
  team
          {
  ...
  */
  ...
  if (!(dflags & DAMAGE_NO_HIT_LOC) || !(dflags & DAMAGE_RADIUS)) {
    if (!G_NonLocationSpecificDamage(
            mod)) { // certain kinds of damage don't care about hitlocation
  ...
  if (client) {
    client->ps.persistant[PERS_ATTACKER] =
        attacker->s.number; // attack can be the world ent
  ...
  if (take || (dflags & DAMAGE_NO_DAMAGE)) {
    if (!targ->client || !attacker->client) {
      ...
      if (!alreadyDead &&
          (((targ->flags & FL_UNDYING) && !(dflags & DAMAGE_NO_PROTECTION)) ||
      ...
      if (targTeam != attackerTeam) { // on the same team
        targ->health = targ->health - take;

        // MCG - Falling should never kill player- only if a trigger_hurt does
        // so.
        ...
        } else if (targ->health < 1 &&
                   attacker->client) { // The player or NPC just killed an enemy
                                       // so increment the kills counter
          attacker->client->ps.persistant[PERS_ENEMIES_KILLED]++;
        }
      } else if (targTeam ==
                 TEAM_PLAYER) { // on the same team, and target is an ally
        qboolean takeDamage = qtrue;
        qboolean yellAtAttacker = qtrue;

        // 1) player doesn't take damage from teammates unless they're angry at
        // him
        if (targ->s.number == 0) { // the player
          if (attacker->enemy != targ &&
              attacker != targ) { // an NPC shot the player by accident
            takeDamage = qfalse;
          }
        }
        // 2) NPCs don't take any damage from player during combat
          ...
          if (((dflags & DAMAGE_RADIUS)) &&
              !(dflags &
                DAMAGE_IGNORE_TEAM)) { // An NPC got hit by player and this is
                                       // during combat or it was slash damage
            // NOTE: though it's not realistic to have teammates not take splash
            // damage, 		even when not in combat, it feels really bad to have them
            //able to 		actually be killed by the player's splash damage
            takeDamage = qfalse;
          }

          if ((dflags &
               DAMAGE_RADIUS)) { // you're fighting and it's just radius damage,
        ...
        if (takeDamage) {
          ...
          if (!alreadyDead &&
              (((targ->flags & FL_UNDYING) &&
                !(dflags & DAMAGE_NO_PROTECTION) && attacker->s.number != 0) ||
               (dflags & DAMAGE_NO_KILL))) { // guy is marked undying and we're
                                             // not the player or we're in combat
            if (targ->health < 1) {
              G_ActivateBehavior(targ, BSET_DEATH);
              ...
              targ->health = 1;
                     ...
                     (((targ->flags & FL_UNDYING) &&
                       !(dflags & DAMAGE_NO_PROTECTION) &&
                       !attacker->s.number && !targ->s.number) ||
                      (dflags & DAMAGE_NO_KILL))) { // player is undying and
    ...
    if (targ->s.number == 0) {
      ...
      {
        if (attacker->enemy == targ ||
            !OnSameTeam(targ, attacker)) { // don't set player's enemy to
                                           // teammates that hit him by accident
          targ->enemy = attacker;
        }
        NPC_SetLookTarget(targ, attacker->s.number, level.time + 1000);
               ...
               (mod == MOD_SABER || attacker->s.weapon != WP_SABER ||
                !attacker->enemy ||
                attacker->enemy->s.weapon !=
                    WP_SABER)) // keep enemy jedi over shooters
    { // this looks dumb when they're on the ground and you keep hitting them,
      // so only do this when first kill them
      if (!OnSameTeam(targ, attacker)) { // don't set player's enemy to
                                         // teammates that he hits by accident
        attacker->enemy = targ;
      }
    ...
    if (targ->health > 0 && targ->NPC &&
        targ->NPC->surrenderTime >
            level.time) { // he was surrendering, goes down with one hit
      targ->health = 0;
    }
    if (targ->health <= 0) {
      if (knockback && (dflags & DAMAGE_DEATH_KNOCKBACK)) //&& targ->client
      {                             // only do knockback on death
        if (mod == MOD_FLECHETTE) { // special case because this is shotgun-ish
                                    // damage, we need to multiply the knockback
      ...
      // If we are a breaking glass brush, store the damage point so we can do
      // cool things with it.
      ...
      if (!alreadyDead &&
          !targ->enemy) { // just killed and didn't have an enemy before
        targ->enemy = attacker;
      }
      ...
      if (targ->s.number == 0) { // player run painscript
...
qboolean CanDamage(gentity_t *targ, vec3_t origin) {
  ...
  gi.trace(&tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE,
           MASK_SOLID, G2_NOCOLLIDE, 0);
  if ((tr.fraction == 1.0 && !(targ->contents & MASK_SOLID)) ||
      tr.entityNum == targ->s.number) // if we also test the entitynum's we can
                                      // bust up bbrushes better!
    return qtrue;
...
============
*/
void G_RadiusDamage(vec3_t origin, gentity_t *attacker, float damage,
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (CanDamage(
            ent,
            origin)) { // FIXME: still do a little damage in in PVS and close?
