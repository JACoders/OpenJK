...
//=====================================================================================================================
// Surface List handling routines - so entities can determine what surfaces
// attached to a model are operational or not.

// find a particular surface in the surface override list
surfaceInfo_t *G2_FindOverrideSurface(int surfaceNum,
                                      surfaceInfo_v &surfaceList) {
 ...
 *
 ************************************************************************************************/
mdxmSurface_t *G2_FindSurface(CGhoul2Info *ghlInfo, surfaceInfo_v &slist,
  ...
  for (i = slist.size() - 1; i >= 0; i--) {
    if ((slist[i].surface != 10000) && (slist[i].surface != -1)) {
      mdxmSurface_t *surf =
          (mdxmSurface_t *)G2_FindSurface((void *)mod, slist[i].surface, 0);
...
qboolean G2_SetSurfaceOnOff(CGhoul2Info *ghlInfo, surfaceInfo_v &slist,
                            const char *surfaceName, const int offFlags) {
  int surfIndex = -1;
...
void G2_SetSurfaceOnOffFromSkin(CGhoul2Info *ghlInfo, qhandle_t renderSkin) {
  ...
  for (j = 0; j < skin->numSurfaces; j++) {
    ...
    if (!strcmp(((shader_t *)skin->surfaces[j]->shader)->name, "*off")) {
      G2_SetSurfaceOnOff(ghlInfo, ghlInfo->mSlist, skin->surfaces[j]->name,
                         G2SURFACEFLAG_OFF);
    } else {
      int flags;
      int surfaceNum = G2_IsSurfaceLegal((void *)ghlInfo->currentModel,
                                         skin->surfaces[j]->name, &flags);
      if ((surfaceNum != -1) &&
          (!(flags &
             G2SURFACEFLAG_OFF))) // only turn on if it's not an "_off" surface
      {
        G2_SetSurfaceOnOff(ghlInfo, ghlInfo->mSlist, skin->surfaces[j]->name,
                           0);
      }
...
// return a named surfaces off flags - should tell you if this surface is on or
// off.
int G2_IsSurfaceOff(CGhoul2Info *ghlInfo, surfaceInfo_v &slist,
                    const char *surfaceName) {
  model_t *mod = (model_t *)ghlInfo->currentModel;
  int surfIndex = -1;
  ...
  mdxmSurfHierarchy_t *surface =
      (mdxmSurfHierarchy_t *)((byte *)mod->mdxm + mod->mdxm->ofsSurfHierarchy);
  ...
  for (int i = 0; i < mod->mdxm->numSurfaces; i++) {
    ...
    // find the next surface
    surface =
        (mdxmSurfHierarchy_t *)((byte *)surface +
                                (intptr_t)(
                                    &((mdxmSurfHierarchy_t *)0)
...
void G2_FindRecursiveSurface(model_t *currentModel, int surfaceNum,
...
void G2_RemoveRedundantGeneratedSurfaces(surfaceInfo_v &slist,
                                         int *activeSurfaces) {
...
qboolean G2_SetRootSurface(CGhoul2Info_v &ghoul2, const int modelIndex,
  ...
  if (surf != -1) {
    ...
    activeSurfaces =
        (int *)Z_Malloc(mod_m->mdxm->numSurfaces * 4, TAG_GHOUL2, qtrue);
    ...
    G2_FindRecursiveSurface(mod_m, surf, ghoul2[modelIndex].mSlist,
                            activeSurfaces);

    // now generate the used bone list
    CConstructBoneList CBL(ghoul2[modelIndex].mSurfaceRoot, activeBones,
                           ghoul2[modelIndex].mSlist, mod_m,
    ...
    G2_RemoveRedundantGeneratedSurfaces(ghoul2[modelIndex].mSlist,
                                        activeSurfaces);
    ...
    // then remove all bolts that point at surfaces or bones that *arent*
    // active.
    G2_RemoveRedundantBolts(ghoul2[modelIndex].mBltlist,
                            ghoul2[modelIndex].mSlist, activeSurfaces,
                            activeBones);

    // then remove all models on this ghoul2 instance that use those bolts that
    // are being removed.
    for (int i = 0; i < ghoul2.size(); i++) {
      ...
      if (ghoul2[i].mModelBoltLink != -1) {
        ...
        // if either the bolt list is too small, or the bolt we are pointing at
        // references nothing, remove this model
  ...
  //g2r	if (entstate->ghoul2)
          {
                  ...
                  model_t				*mod_m =
  R_GetModelByHandle(RE_RegisterModel(ghoul2[modelIndex].mFileName)); model_t
  *mod_a = R_GetModelByHandle(mod_m->mdxm->animIndex); int
  surf; int					flags;
                  int					*activeSurfaces,
  *activeBones;
                  ...
                  if (surf != -1)
                  {
                          // first see if this ghoul2 model already has this as
  a root surface if (ghoul2[modelIndex].mSurfaceRoot == surf)
                          {
                                  return qtrue;
                          }
                          ...
                          // ok, now the tricky bits.
                          // firstly, generate a list of active / on surfaces
  below the root point

                          // gimme some space to put this list into
                          activeSurfaces = (int
  *)Z_Malloc(mod_m->mdxm->numSurfaces * 4, TAG_GHOUL2, qtrue);
                          memset(activeSurfaces, 0, (mod_m->mdxm->numSurfaces *
  4)); activeBones = (int *)Z_Malloc(mod_a->mdxa->numBones * 4, TAG_GHOUL2,
  qtrue); memset(activeBones, 0, (mod_a->mdxa->numBones * 4));

                          G2_FindRecursiveSurface(mod_m, surf,
  ghoul2[modelIndex].mSlist, activeSurfaces);

                          // now generate the used bone list
                          CConstructBoneList
  CBL(ghoul2[modelIndex].mSurfaceRoot, activeBones, ghoul2[modelIndex].mSlist,
                                                                  mod_m,
                                                                  ghoul2[modelIndex].mBlist);

                          G2_ConstructUsedBoneList(CBL);

                          // now remove all procedural or override surfaces that
  refer to surfaces that arent on this list
                          G2_RemoveRedundantGeneratedSurfaces(ghoul2[modelIndex].mSlist,
  activeSurfaces);

                          // now remove all bones that are pointing at bones
  that aren't active G2_RemoveRedundantBoneOverrides(ghoul2[modelIndex].mBlist,
  activeBones);

                          // then remove all bolts that point at surfaces or
  bones that *arent* active.
                          G2_RemoveRedundantBolts(ghoul2[modelIndex].mBltlist,
  ghoul2[modelIndex].mSlist, activeSurfaces, activeBones);

                          // then remove all models on this ghoul2 instance that
  use those bolts that are being removed. for (int i=0; i<ghoul2.size(); i++)
                          {
                                  // are we even bolted to anything?
                                  if (ghoul2[i].mModelBoltLink != -1)
                                  {
                                          int	boltMod =
  (ghoul2[i].mModelBoltLink >> MODEL_SHIFT) & MODEL_AND; int	boltNum =
  (ghoul2[i].mModelBoltLink >> BOLT_SHIFT) & BOLT_AND;
                                          // if either the bolt list is too
  small, or the bolt we are pointing at references nothing, remove this model if
  ((ghoul2[boltMod].mBltlist.size() <= boltNum) ||
                                                  ((ghoul2[boltMod].mBltlist[boltNum].boneNumber
  == -1) && (ghoul2[boltMod].mBltlist[boltNum].surfaceNumber == -1)))
                                          {
                                                  G2API_RemoveGhoul2Model(entstate,
  i);
                                          }
                                  }
                          }
...
int G2_AddSurface(CGhoul2Info *ghoul2, int surfaceNumber, int polyNumber,
  ...
  for (size_t i = 0; i < ghoul2->mSlist.size(); i++) {
    ...
    if (ghoul2->mSlist[i].surface == -1) {
      ...
      ghoul2->mSlist[i].surface =
          10000; // no model will ever have 10000 surfaces
  ...
  temp_slist_entry.genPolySurfaceIndex =
      ((polyNumber & 0xffff) << 16) | (surfaceNumber & 0xffff);
...
int G2_IsSurfaceRendered(CGhoul2Info *ghlInfo, const char *surfaceName,
  ...
  int surfNum =
      G2_IsSurfaceLegal((model_t *)ghlInfo->currentModel, surfaceName, &flags);
  if (surfNum != -1) { // must be legal
    ...
    while (surfNum != -1) {
      ...
      G2_IsSurfaceLegal((model_t *)ghlInfo->currentModel, parentSurfInfo->name,
                        &parentFlags);
