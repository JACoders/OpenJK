...
float forceSpeedLevels[4] = {1, // rank 0?
                             1.25, 1.5, 1.75};
...
int forcePowerNeeded[NUM_FORCE_POWER_LEVELS][NUM_FORCE_POWERS] = {
    ...
    {
        50, // FP_HEAL,//instant //You get 5 points of health.. for 50 force
            // points!
        10, // FP_LEVITATION,//hold/duration
        50, // FP_SPEED,//duration
        20, // FP_PUSH,//hold/duration
...
void PM_pitch_roll_for_slope(bgEntity_t *forwhom, vec3_t pass_slope,
                             vec3_t storeAngles) {
  vec3_t slope;
  ...
  if (!pass_slope || VectorCompare(vec3_origin, pass_slope)) {
    ...
    endspot[2] -= 300;
    pm->trace(&trace, pm->ps->origin, vec3_origin, vec3_origin, endspot,
              forwhom->s.number, MASK_SOLID);
  ...
  } else // if ( forwhom->client )
  {
    ...
    if (oldmins2 > pm->mins[2]) { // our mins is now lower, need to move up
      // FIXME: trace?
...
void PM_SetSpecialMoveValues(void) {
  ...
  if (pm->ps->clientNum < MAX_CLIENTS) { // we know that real players aren't
                                         // vehs
    pm_flying = FLY_NONE;
    return;
  }
  ...
  if (pEnt) {
    if ((pm->ps->eFlags2 &
         EF2_FLYING)) // pm->gent->client->moveType == MT_FLYSWIM )
    {
...
static void PM_SetVehicleAngles(vec3_t normal) {
  ...
  vehicleBankingSpeed =
      (pVeh->m_pVehicleInfo->bankingSpeed * 32.0f) * pml.frametime; // 0.25f
  ...
  if (pVeh->m_pVehicleInfo->type == VH_FIGHTER) {
    ...
    pitchBias =
        90.0f * pVeh->m_pVehicleInfo
                    ->centerOfGravity[0]; // if centerOfGravity is all the way
                                          // back (-1.0f), vehicle pitches up 90
                                          // degrees when in air
  }
  ...
  if (pm->waterlevel > 0) { // in water
    ...
    if ((pml.groundTrace.contents &
         (CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA))) { // on water
      // view pitch has some influence when on a fluid surface
      // FIXME: take center of gravity into account
      vAngles[PITCH] += (pm->ps->viewangles[PITCH] - vAngles[PITCH]) * 0.5f +
                        (pitchBias * 0.5f);
    }
...
void PM_HoverTrace(void) {
  ...
  // relativeWaterLevel = (pm->ps->waterheight -
  // (pm->ps->origin[2]+pm->mins[2]));
  ...
  if (pm->waterlevel && relativeWaterLevel >= 0) { // in water
...
#ifdef _GAME // yeah, this is kind of crappy and makes no use of prediction
             // whatsoever
          if (pVeh->m_pVehicleInfo->iWakeFX) {
            // G_PlayEffectID( pVeh->m_pVehicleInfo->iWakeFX, wakeOrg, fxAxis[0]
            // ); tempent use bad!
            G_AddEvent((gentity_t *)pEnt, EV_PLAY_EFFECT_ID,
    ...
    if (trace->plane.normal[0] > 0.5f || trace->plane.normal[0] < -0.5f ||
        trace->plane.normal[1] > 0.5f ||
        trace->plane.normal[1] < -0.5f) { // steep slanted hill, don't go up it.
      ...
      if (trace->fraction < 1.0f) { // push up off ground
        ...
        if (trace->fraction > 0.5f) {
          pm->ps->velocity[2] +=
              (1.0f - trace->fraction) * hoverForce * pVeh->m_fTimeModifier;
        } else {
          pm->ps->velocity[2] += (0.5f - (trace->fraction * trace->fraction)) *
                                 hoverForce * 2.0f * pVeh->m_fTimeModifier;
        }
        if ((trace->contents &
...
#ifdef _GAME
              if (pVeh->m_pVehicleInfo->iWakeFX) {
                G_PlayEffectID(pVeh->m_pVehicleInfo->iWakeFX, trace->endpos,
                               fxAxis[0]);
              }
#endif
            }
  ...
  if (pml.groundPlane) {
    ...
    // pVeh->m_vAngularVelocity *= 0.95f;		// Angular Velocity Decays Over
    // Time
    if (pVeh->m_vAngularVelocity > 0.0f) {
...
static void PM_Friction(void) {
  ...
  if (pm_flying != FLY_VEHICLE && pEnt && pEnt->s.NPC_class == CLASS_VEHICLE &&
      pEnt->m_pVehicle && pEnt->m_pVehicle->m_pVehicleInfo->type != VH_ANIMAL &&
...
static void PM_Accelerate(vec3_t wishdir, float wishspeed, float accel) {
  if (pm->gametype != GT_SIEGE || pm->ps->m_iVehicleNum ||
      ...
      pm->ps->pm_type !=
          PM_NORMAL) { // standard method, allows "bunnyhopping" and whatnot
    int i;
...
qboolean PM_ForceJumpingUp(void) {
  if (!(pm->ps->fd.forcePowersActive & (1 << FP_LEVITATION)) &&
      pm->ps->fd.forceJumpCharge) { // already jumped and let go
    return qfalse;
  }
  ...
  if (!BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime,
                      FP_LEVITATION)) {
    return qfalse;
  }
...
qboolean PM_AdjustAngleForWallRun(playerState_t *ps, usercmd_t *ucmd,
                                  qboolean doMove) {
  if (((ps->legsAnim) == BOTH_WALL_RUN_RIGHT ||
       (ps->legsAnim) == BOTH_WALL_RUN_LEFT) &&
      ps->legsTimer > 500) { // wall-running and not at end of anim
    ...
    pm->trace(&trace, ps->origin, mins, maxs, traceTo, ps->clientNum,
              MASK_PLAYERSOLID);
             ...
             0.4f)) //&& ent->client->ps.groundEntityNum == ENTITYNUM_NONE )
    {
      ...
      pm->trace(&trace2, pm->ps->origin, mins, maxs, traceTo2,
    ...
    if (trace.fraction < 1.0f &&
        (trace.plane.normal[2] >= 0.0f &&
         trace.plane.normal[2] <=
             0.4f /*MAX_WALL_RUN_Z_NORMAL*/)) { // still a wall there
      if ((ps->legsAnim) == BOTH_WALL_RUN_RIGHT) {
        ucmd->rightmove = 127;
      } else {
        ucmd->rightmove = -127;
      ...
      ucmd->angles[YAW] =
          ANGLE2SHORT(ps->viewangles[YAW]) - ps->delta_angles[YAW];
      if (doMove) {
        // push me forward
...
qboolean PM_AdjustAnglesForWallRunUpFlipAlt(usercmd_t *ucmd) {
  //	ucmd->angles[PITCH] = ANGLE2SHORT( pm->ps->viewangles[PITCH] ) -
  //pm->ps->delta_angles[PITCH]; 	ucmd->angles[YAW] = ANGLE2SHORT(
  //pm->ps->viewangles[YAW] ) - pm->ps->delta_angles[YAW];
  PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, ucmd);
  return qtrue;
}

qboolean PM_AdjustAngleForWallRunUp(playerState_t *ps, usercmd_t *ucmd,
                                    qboolean doMove) {
  if (ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START) { // wall-running up
    ...
    if (trace.fraction >
      ...
      bottom[2] -= 64.0f;
      pm->trace(&trace2, top, pm->mins, pm->maxs, bottom, ps->clientNum,
                MASK_PLAYERSOLID);
      ...
      { // cool, do the alt-flip and land on whetever it is we just scaled up
        ...
        PM_SetAnim(SETANIM_BOTH, BOTH_FORCEWALLRUNFLIP_ALT,
                   SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    ...
    if ( // ucmd->upmove <= 0 &&
        ...
        (trace.plane.normal[2] >= 0.0f &&
         trace.plane.normal[2] <=
             0.4f /*MAX_WALL_RUN_Z_NORMAL*/)) { // still a vertical wall there
      // make sure there's not a ceiling above us!
      trace_t trace2;
      VectorCopy(ps->origin, traceTo);
      traceTo[2] += 64;
      pm->trace(&trace2, ps->origin, mins, maxs, traceTo, ps->clientNum,
                MASK_PLAYERSOLID);
      if (trace2.fraction <
        ...
        PM_SetPMViewAngle(ps, ps->viewangles, ucmd);
        /*
        if ( ent->client->ps.viewEntity <= 0 || ent->client->ps.viewEntity >=
        ENTITYNUM_WORLD )
        ...
        */
        ...
        if (1) // aslkfhsakf
        {
          if (doMove) {
            ...
            VectorScale(trace.plane.normal, -dist * trace.fraction,
                        ps->velocity);
            // push me up
    ...
    if (doMove) { // stop it
      ...
      // SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD ); why?!?#?#@!%$R@$KR#F:Hdl;asfm
      PM_SetAnim(SETANIM_BOTH, BOTH_FORCEWALLRUNFLIP_END,
                 SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
qboolean PM_AdjustAngleForWallJump(playerState_t *ps, usercmd_t *ucmd,
                                   qboolean doMove) {
  if (((BG_InReboundJump(ps->legsAnim) || BG_InReboundHold(ps->legsAnim)) &&
       (BG_InReboundJump(ps->torsoAnim) || BG_InReboundHold(ps->torsoAnim))) ||
      (pm->ps->pm_flags &
       PMF_STUCK_TO_WALL)) { // hugging wall, getting ready to jump off
    // stick to wall, if there is one
    ...
    pm->trace(&trace, ps->origin, mins, maxs, traceTo, ps->clientNum,
              MASK_PLAYERSOLID);
    if ( // ucmd->upmove <= 0 &&
        ps->legsTimer > 100 && trace.fraction < 1.0f &&
        fabs(trace.plane.normal[2]) <=
            0.2f /*MAX_WALL_GRAB_SLOPE*/) { // still a vertical wall there
      // FIXME: don't pull around 90 turns
      /*
      if ( ent->s.number || !player_locked )
      {
...
// Set the height for when a force jump was started. If it's 0, nuge it up
// (slight hack to prevent holding jump over slopes)
...
static qboolean PM_CheckJump(void) {
  ...
  if (pm->ps->pm_type ==
      PM_JETPACK) { // there's no actual jumping while we jetpack
    return qfalse;
  }
  ...
  if (PM_InKnockDown(pm->ps) ||
      BG_InRoll(pm->ps, pm->ps->legsAnim)) { // in knockdown
    return qfalse;
  }
  ...
  if (pm->ps->fd.forcePowersActive &
      (1 << FP_LEVITATION)) { // Force jump is already active.. continue
                              // draining power appropriately until we land.
      ...
      if (PM_ForceJumpingUp()) { // holding jump in air
        ...
        {                               // can still go up
          if (curHeight > forceJumpHeight[0]) { // passed normal jump height *2?
                  ...
                  (1 << FP_LEVITATION))) // haven't started forcejump yet
            {
              ...
              if ((pm->cmd.forwardmove ||
                  ...
                  (pm->ps->legsAnim) != BOTH_FLIP_L && allowFlips) {
                int anim = BOTH_FORCEINAIR1;
                int parts = SETANIM_BOTH;
                ...
                PM_SetAnim(parts, anim,
                           SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
              } else if (pm->ps->fd.forcePowerLevel[FP_LEVITATION] >
                         FORCE_LEVEL_1) {
          ...
          pm->ps->velocity[2] =
              ...
              forceJumpHeight[pm->ps->fd.forcePowerLevel[FP_LEVITATION]] *
              forceJumpStrength
                  [pm->ps->fd.forcePowerLevel[FP_LEVITATION]]; // JUMP_VELOCITY;
          pm->ps->velocity[2] /= 10;
          pm->ps->velocity[2] += JUMP_VELOCITY;
          pm->ps->pm_flags |= PMF_JUMP_HELD;
        } else if (curHeight > forceJumpHeight[0] &&
                   curHeight <
                       forceJumpHeight[pm->ps->fd
                                           .forcePowerLevel[FP_LEVITATION]] -
  ...
  if (pm->ps->gravity <=
    ...
    pm->cmd.upmove = 0;
             ...
             BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime,
                            FP_LEVITATION)) {
    ...
    if (pm->ps->groundEntityNum != ENTITYNUM_NONE) { // on the ground
      ...
      int anim = -1;
      float vertPush = 0;
      if (pm->cmd.rightmove > 0 && pm->ps->fd.forcePowerLevel[FP_LEVITATION] >
        ...
        if (pm->cmd.forwardmove > 0) { // wall-run
          ...
          if (allowWallFlips) {
            ...
            anim = BOTH_WALL_FLIP_LEFT;
                 ...
                 !(pm->cmd.buttons & BUTTON_ATTACK)) { // backflip
        if (allowFlips) {
          vertPush = JUMP_VELOCITY;
          anim = BOTH_FLIP_BACK1; // BG_PickAnim( BOTH_FLIP_BACK1,
                                  // BOTH_FLIP_BACK3 );
        }
      }
            ...
            (trace.fraction < 1.0f &&
             (trace.entityNum < MAX_CLIENTS ||
              DotProduct(wallNormal, idealNormal) >
                  0.7))) { // there is a wall there.. or hit a client
          if ((anim != BOTH_WALL_RUN_LEFT && anim != BOTH_WALL_RUN_RIGHT &&
               anim != BOTH_FORCEWALLRUNFLIP_START) ||
              (wallNormal[2] >= 0.0f &&
               wallNormal[2] <=
                   0.4f /*MAX_WALL_RUN_Z_NORMAL*/)) { // wall-runs can only run
                                                      // on perfectly flat walls,
                                                      // sorry.
            int parts;
            // move me to side
            ...
            /*
            if ( doTrace && anim != BOTH_WALL_RUN_LEFT && anim !=
            BOTH_WALL_RUN_RIGHT )
            {
                    if (trace.entityNum < MAX_CLIENTS)
                    {
                            pm->ps->forceKickFlip = trace.entityNum+1; //let the
            server know that this person gets kicked by this client
                    }
            }
            */
            ...
            PM_SetForceJumpZStart(
                pm->ps->origin[2]); // so we don't take damage if we land at
                                    // same height
            pm->ps->pm_flags |= PMF_JUMP_HELD;
            pm->cmd.upmove = 0;
            pm->ps->fd.forceJumpSound = 1;
      ...
      // NEW JKA
        ...
        animLen = BG_AnimLength(pm_entSelf->localAnimIndex,
      ...
      /*
              ...
              && pm->ps->velocity[2] > 200
              && PM_GroundDistance() <= 80 //unfortunately we do not have a
      happy ground timer like SP (this would use up more bandwidth if we wanted
      ...
      {//run up wall, flip backwards
              ...
              pm->trace( &trace, pm->ps->origin, mins, maxs, traceto,
      pm->ps->clientNum, MASK_PLAYERSOLID );//FIXME: clip brushes too?
              VectorSubtract( pm->ps->origin, traceto, idealNormal );
              VectorNormalize( idealNormal );
              ...
              {//there is a wall there
                      ...
                      PM_SetAnim( parts, BOTH_WALL_FLIP_BACK1,
      SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );

                      pm->ps->legsTimer -= 600; //I force this anim to play to
      the end to prevent landing on your head and suddenly flipping over.
                                                                        //It is
      a bit too long at the end though, so I'll just shorten it.

                      PM_SetForceJumpZStart(pm->ps->origin[2]);//so we don't
      take damage if we land at same height pm->cmd.upmove = 0;
                      pm->ps->fd.forceJumpSound = 1;
                      BG_ForcePowerDrain( pm->ps, FP_LEVITATION, 5 );

                      if (trace.entityNum < MAX_CLIENTS)
                      {
                              pm->ps->forceKickFlip = trace.entityNum+1; //let
      the server know that this person gets kicked by this client
                      }
              }
      }
      */
      else if (pm->cmd.forwardmove > 0 // pushing forward
               && pm->ps->fd.forceRageRecoveryTime <
                      pm->cmd.serverTime // not in a force Rage recovery period
               && pm->ps->fd.forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1 &&
               PM_WalkableGroundDistance() <=
                   80 // unfortunately we do not have a happy ground timer like
                      // SP (this would use up more bandwidth if we wanted
                      // prediction workign right), so we'll just use the actual
                      // ground distance.
      ...
      {                              // run up wall, flip backwards
        if (allowWallRuns) {
          // FIXME: have to be moving... make sure it's opposite the wall... or
          // at least forward?
          ...
          if (1) // sure, we have it! Because I SAID SO.
          {
            ...
            pm->trace(&trace, pm->ps->origin, mins, maxs, traceto,
                      pm->ps->clientNum, contents); // FIXME: clip brushes too?
            VectorSubtract(pm->ps->origin, traceto, idealNormal);
            VectorNormalize(idealNormal);
            ...
            if (trace.fraction < 1.0f &&
              ...
              // animate me
              PM_SetAnim(parts, wallWalkAnim,
                         SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
              //						pm->ps->pm_flags
              //|= PMF_JUMPING|PMF_SLOW_MO_FALL; again with the flags!
              // G_SoundOnEnt( pm->gent, CHAN_BODY,
              // "sound/weapons/force/jump.wav" ); yucky!
              PM_SetForceJumpZStart(
                  pm->ps->origin[2]); // so we don't take damage if we land at
                                      // same height
              pm->cmd.upmove = 0;
              ...
              /*
              if ( kick && traceEnt && (traceEnt->s.eType == ET_PLAYER ||
              traceEnt->s.eType == ET_NPC) ) { //kick that thang!
              ...
              */
              pm->cmd.rightmove = pm->cmd.forwardmove = 0;
                  ...
                  || BG_InBackFlip(legsAnim))   // a backflip (needed so you can
                 ...
                 //&& pm->ps->forceRageRecoveryTime < pm->cmd.serverTime
                 ////not in a force Rage recovery period
                 && pm->ps->fd.forcePowerLevel[FP_LEVITATION] >
                        FORCE_LEVEL_2 // level 3 jump or better
                 //&& WP_ForcePowerAvailable( pm->gent, FP_LEVITATION, 10
                 //)//have enough force power to do another one
                 && BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime,
                                   FP_LEVITATION) &&
                 (pm->ps->origin[2] - pm->ps->fd.forceJumpZStart) <
                     (forceJumpHeightMax[FORCE_LEVEL_3] -
                      (BG_ForceWallJumpStrength() /
                       2.0f)) // can fit at least one more wall jump in (yes,
        ...
        if (allowWallGrabs) {
          ...
          if (anim != -1) { // trace in the dir we're pushing in and see if
            ...
            pm->trace(&trace, pm->ps->origin, mins, maxs, traceto,
                      pm->ps->clientNum,
                      CONTENTS_SOLID); // FIXME: clip brushes too?
            VectorSubtract(pm->ps->origin, traceto, idealNormal);
            VectorNormalize(idealNormal);
            traceEnt = PM_BGEntForNum(trace.entityNum);
            if (trace.fraction < 1.0f &&
  ...
  {//okay, we just jumped and we're in an attack
          ...
          {//not in an anim we shouldn't interrupt
                  ...
                  float animLength = PM_AnimLength( 0,
  (animNumber_t)pm->ps->torsoAnim ); if ( animLength - pm->ps->torsoTimer < 500
  )
                          ...
                          {//using medium attacks
                                  ...
                                  { //FLIP AND DOWNWARD ATTACK
                                          ...
                                          //if (PM_SomeoneInFront(&tr))
                                          {
                                                  ...
                                                  pml.walking = qfalse;
                                                  pm->ps->pm_flags |=
  PMF_JUMP_HELD; pm->ps->groundEntityNum = ENTITYNUM_NONE;
  ...
  pm->ps->torsoTimer;
                          ...
                          {//using strong attacks
                                  if ( pm->cmd.forwardmove > 0 && //going
  forward (pm->cmd.buttons & BUTTON_ATTACK) && //must be holding attack still
                                          PM_GroundDistance() < 32 &&
                                          !BG_InSpecialJump(pm->ps->legsAnim))
                                  {//strong attack: jump-hack
                                          PM_SetSaberMove(
  PM_SaberJumpAttackMove() ); pml.groundPlane = qfalse; pml.walking = qfalse;
                                          pm->ps->pm_flags |= PMF_JUMP_HELD;
                                          pm->ps->groundEntityNum =
  ENTITYNUM_NONE; VectorClear(pml.groundTrace.plane.normal);
...
static void PM_AirMove(void) {
  ...
  if (pVeh && pVeh->m_pVehicleInfo->hoverHeight >
...
#endif
  } else if (gPMDoSlowFall) { // no air-control
    VectorClear(wishvel);
  } else if (pm->ps->pm_type ==
             PM_JETPACK) { // reduced air control while not jetting
    for (i = 0; i < 2; i++) {
      wishvel[i] = pml.forward[i] * fmove + pml.right[i] * smove;
    }
    wishvel[2] = 0;
    ...
    } else { // if we are jetting then we have more control than usual
  ...
  if (pml.groundPlane) {
    if (!(pm->ps->pm_flags &
          PMF_STUCK_TO_WALL)) { // don't slide when stuck to a wall
  ...
  if ((pm->ps->pm_flags & PMF_STUCK_TO_WALL)) { // no grav when stuck to wall
    PM_StepSlideMove(qfalse);
  } else {
    PM_StepSlideMove(qtrue);
  }
...
static int PM_TryRoll(void) {
  ...
  if ((pm->ps->weapon != WP_SABER && pm->ps->weapon != WP_MELEE) ||
      PM_IsRocketTrooper() || BG_HasYsalamiri(pm->gametype, pm->ps) ||
      !BG_CanUseFPNow(pm->gametype, pm->ps, pm->cmd.serverTime,
                      FP_LEVITATION)) { // Not using saber, or can't use jump
    return 0;
  }
  ...
  if (anim != -1) { // We want to roll. Perform a trace to see if we can, and if
                    // so, send us into one.
...
static void PM_CrashLand(void) {
  ...
  // decide which landing animation to use
           ...
           !pm->ps->m_iVehicleNum) { // only play a land animation if we
                                     // transitioned into an in-air animation
  ...
  if (pm->ps->weapon != WP_SABER && pm->ps->weapon != WP_MELEE &&
      !PM_IsRocketTrooper()) { // saber handles its own anims
  ...
  if (!BG_InSpecialJump(pm->ps->legsAnim) || pm->ps->legsTimer < 1 ||
      (pm->ps->legsAnim) == BOTH_WALL_RUN_LEFT ||
      (pm->ps->legsAnim) ==
          BOTH_WALL_RUN_RIGHT) { // Only set the timer if we're in an anim that
                                 // can be interrupted (this would not be, say, a
                                 // flip)
    if (!BG_InRoll(pm->ps, pm->ps->legsAnim) && pm->ps->inAirAnim) {
      if (!BG_SaberInSpecial(pm->ps->saberMove) || pm->ps->weapon != WP_SABER) {
        if (pm->ps->legsAnim != BOTH_FORCELAND1 &&
            ...
            pm->ps->legsAnim !=
                BOTH_FORCELANDLEFT1) { // don't override if we have started a
                                       // force land
  ...
  if (pm->ps->pm_flags & PMF_DUCKED) {
    if (delta >= 2 && !PM_InOnGroundAnim(pm->ps->legsAnim) &&
        !PM_InKnockDown(pm->ps) && !BG_InRoll(pm->ps, pm->ps->legsAnim) &&
      ...
      if (anim) { // absorb some impact
        pm->ps->legsTimer = 0;
        delta /=
            3; // /= 2 just cancels out the above delta *= 2 when landing while
               // crouched, the roll itself should absorb a little damage
        pm->ps->legsAnim = 0;
  ...
  if (!(pml.groundTrace.surfaceFlags & SURF_NODAMAGE)) {
    if (delta > 7) {
      ...
      if (pm->ps->fd.forceJumpZStart) {
        if ((int)pm->ps->origin[2] >=
            (int)pm->ps->fd
                .forceJumpZStart) { // was force jumping, landed on higher or
                                    // same level as when force jump was started
          if (delta_send > 8) {
            delta_send = 8;
          }
        } else {
  ...
  // make sure velocity resets so we don't bounce back up again in case we miss
  // the clear elsewhere
  pm->ps->velocity[2] = 0;
...
static void PM_GroundTraceMissed(void) {
  ...
  // rww - don't want to do this when handextend_choke, because you can be
  // standing on the ground while still holding your throat.
  if (pm->ps->pm_type == PM_FLOAT) {
    // we're assuming this is because you're being choked
    int parts = SETANIM_LEGS;

    // rww - also don't use SETANIM_FLAG_HOLD, it will cause the legs to float
    // around a bit before going into a proper anim even when on the ground.
    PM_SetAnim(parts, BOTH_CHOKE3, SETANIM_FLAG_OVERRIDE);
  } else if (
      pm->ps->pm_type ==
      PM_JETPACK) { // jetpacking
                    // rww - also don't use SETANIM_FLAG_HOLD, it will cause the
                    // legs to float around a bit before going into a proper anim
                    // even when on the ground.
                    // PM_SetAnim(SETANIM_LEGS,BOTH_FORCEJUMP1,SETANIM_FLAG_OVERRIDE);
  }
  // If the anim is choke3, act like we just went into the air because we aren't
  // in a float
  ...
  if (PM_InRollComplete(
          pm->ps,
          pm->ps
              ->legsAnim)) { // Client won't catch an animation restart because
                             // it only checks frame against incoming frame, so
                             // if you roll when you land after rolling off of
                             // something it won't replay the roll anim unless we
                             // switch it off in the air. This fixes that.
    PM_SetAnim(SETANIM_BOTH, BOTH_INAIR1,
               SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    pm->ps->inAirAnim = qtrue;
  }
...
#ifdef _GAME
    if (pm->ps->clientNum < MAX_CLIENTS && !pm->ps->m_iVehicleNum &&
        trace.entityNum < ENTITYNUM_WORLD && trace.entityNum >= MAX_CLIENTS &&
        !pm->ps->zoomMode && pm_entSelf) { // check if we landed on a vehicle
      gentity_t *trEnt = &g_entities[trace.entityNum];
      if (trEnt->inuse && trEnt->client && trEnt->s.eType == ET_NPC &&
          trEnt->s.NPC_class == CLASS_VEHICLE &&
          !trEnt->client->ps.m_iVehicleNum && trEnt->m_pVehicle &&
          trEnt->m_pVehicle->m_pVehicleInfo->type != VH_WALKER &&
          trEnt->m_pVehicle->m_pVehicleInfo->type !=
              VH_FIGHTER) { // it's a vehicle alright, let's board it.. if it's
                            // not an atst or ship
        if (!BG_SaberInSpecial(pm->ps->saberMove) &&
          ...
          if (level.gametype < GT_TEAM || !trEnt->alliedTeam ||
              (trEnt->alliedTeam ==
               servEnt->client->sess.sessionTeam)) { // not belonging to a team,
                                                     // or client is on same team
            trEnt->m_pVehicle->m_pVehicleInfo->Board(trEnt->m_pVehicle,
                                                     pm_entSelf);
          }
        }
      }
...
qboolean PM_CheckDualForwardJumpDuck(void) {
  ...
  if (pm->ps->legsAnim == BOTH_JUMPATTACK6) {
    ...
    if ((pm->ps->legsTimer >= 1450 &&
        ...
        (pm->ps->legsTimer >= 400 &&
         PM_AnimLength(0, BOTH_JUMPATTACK6) - pm->ps->legsTimer >=
  ...
  {                                      // drop the mins back down
    ...
    if (!trace.allsolid && !trace.startsolid) { // should never start in solid
      if (trace.fraction >= 1.0f) {             // all clear
        ...
        pm->trace(&trace, pm->ps->origin, curMins, curMaxs, end,
                  pm->ps->clientNum, pm->tracemask);
        if (!trace.allsolid &&
            !trace.startsolid) {        // should never start in solid
          if (trace.fraction >= 1.0f) { // all clear
            // move me up
            ...
            if (pm->ps->legsAnim != BOTH_JUMPATTACK6 ||
                pm->ps->legsTimer <= 200) { // at the end of the anim, and we
                                            // can't leave ourselves like this
...
static qboolean PM_CanStand(void) {
  ...
  for (x = pm->mins[0] + 5.0f; canStand && x <= (pm->maxs[0] - 5.0f);
       x += 10.0f) {
    for (y = pm->mins[1] + 5.0f; y <= (pm->maxs[1] - 5.0f); y += 10.0f) {
      ...
      pm->trace(&trace, start, lineMins, lineMaxs, end, pm->ps->clientNum,
                pm->tracemask);
...
static void PM_CheckDuck(void) {
  ...
  if (pm->ps->m_iVehicleNum > 0 &&
      pm->ps->m_iVehicleNum <
          ENTITYNUM_NONE) { // riding a vehicle or are a vehicle
    ...
    if (pm_entVeh && pm_entVeh->m_pVehicle &&
      ...
      pm->trace(&solidTr, pm->ps->origin, pm->mins, pm->maxs, pm->ps->origin,
                pm->ps->m_iVehicleNum, pm->tracemask);
      if (solidTr.startsolid || solidTr.allsolid ||
          solidTr.fraction != 1.0f) { // whoops, can't fit here. Down to 0!
        VectorClear(pm->mins);
        VectorClear(pm->maxs);
#ifdef _GAME
        {
          gentity_t *me = &g_entities[pm->ps->clientNum];
          if (me->inuse && me->client) { // yeah, this is a really terrible
                                         // hack.
...
void PM_FootSlopeTrace(float *pDiff, float *pInterval) {
  ...
  trap->G2API_GetBoltMatrix(pm->ghoul2, 0, pm->g2Bolts_LFoot, &boltMatrix,
                            G2Angles, pm->ps->origin, pm->cmd.serverTime, NULL,
                            pm->modelScale);
  footLPoint[0] = boltMatrix.matrix[0][3];
  footLPoint[1] = boltMatrix.matrix[1][3];
  footLPoint[2] = boltMatrix.matrix[2][3];

  trap->G2API_GetBoltMatrix(pm->ghoul2, 0, pm->g2Bolts_RFoot, &boltMatrix,
                            G2Angles, pm->ps->origin, pm->cmd.serverTime, NULL,
                            pm->modelScale);
...
// rww - slowly back out of slope leg anims, to prevent skipping between slope
// anims and general jittering
...
static void PM_Footsteps(void) {
  ...
  if ((PM_InSaberAnim((pm->ps->legsAnim)) &&
      ...
      PM_PainAnim((pm->ps->legsAnim))) { // legs are in a saber anim, and not
  ...
  // if not trying to move
    ...
    if (pm->xyspeed < 5) {
      ...
      if (pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf &&
        ...
        } else if ((pm->ps->eFlags2 & EF2_ALERTED)) { // have an enemy or have
                                                      // had one since we spawned
        ...
        if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1) {
          ...
          // yeah.. the anim has a valid pose for the legs, it uses it (you
          // can't move while using disruptor)
  ...
  if (pm->ps->saberMove == LS_SPINATTACK) {
    ...
    if (((PM_RunningAnim(pm->ps->legsAnim) &&
          VectorLengthSquared(pm->ps->velocity) >= 40000 /*200*200*/) ||
         PM_CanRollFromSoulCal(pm->ps)) &&
        !BG_InRoll(pm->ps, pm->ps->legsAnim)) { // roll!
      rolled = PM_TryRoll();
    }
    if (!rolled) { // if the roll failed or didn't attempt, do standard
                   // crouching anim stuff.
      if (pm->ps->pm_flags & PMF_BACKWARDS_RUN) {
      ...
      pm->ps->legsTimer = 0;
      pm->ps->legsAnim = 0;
      PM_SetAnim(SETANIM_BOTH, rolled,
                 SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      PM_AddEventWithParm(EV_ROLL, 0);
      pm->maxs[2] = pm->ps->crouchheight; // CROUCH_MAXS_2;
      pm->ps->viewheight = DEFAULT_VIEWHEIGHT;
    ...
    if ((pm->ps->legsAnim == BOTH_FORCELAND1 ||
...
#ifndef BASE_COMPAT // FIXME: this doesn't break base compatibility at all,
                    // remove #ifndef
        if (pm->ps->weapon != WP_SABER) {
          desiredAnim = BOTH_RUN1;
        } else {
      ...
      if (pm->ps->pm_flags & PMF_BACKWARDS_RUN) {
#ifndef BASE_COMPAT // fixme, doesn't break base compat if enabled (I tested
                    // this to be sure)
        if (pm->ps->weapon != WP_SABER) {
          desiredAnim = BOTH_WALKBACK1;
        } else {
...
static void PM_WaterEvents(void) { // FIXME?
  ...
  if (impact_splash) {
    ...
    pm->trace(&tr, start, vec3_origin, vec3_origin, end, pm->ps->clientNum,
              MASK_WATER);
...
#ifdef _GAME
extern void WP_GetVehicleCamPos(gentity_t *ent, gentity_t *pilot,
                                vec3_t camPos);
#else
extern void CG_GetVehicleCamPos(vec3_t camPos);
#endif
#define MAX_XHAIR_DIST_ACCURACY 20000.0f
int BG_VehTraceFromCamPos(trace_t *camTrace, bgEntity_t *bgEnt,
                          const vec3_t entOrg, const vec3_t shotStart,
                          const vec3_t end, vec3_t newEnd, vec3_t shotDir,
                          float bestDist) {
  // NOTE: this MUST stay up to date with the method used in
  // CG_ScanForCrosshairEntity (where it checks the doExtraVehTraceFromViewPos
  // bool)
  vec3_t viewDir2End, extraEnd, camPos;
  float minAutoAimDist;

#ifdef _GAME
  WP_GetVehicleCamPos((gentity_t *)bgEnt,
                      (gentity_t *)bgEnt->m_pVehicle->m_pPilot, camPos);
#else
  CG_GetVehicleCamPos(camPos);
#endif

  minAutoAimDist = Distance(entOrg, camPos) +
                   (bgEnt->m_pVehicle->m_pVehicleInfo->length / 2.0f) + 200.0f;
  ...
  if (!camTrace->allsolid && !camTrace->startsolid &&
      camTrace->fraction < 1.0f &&
      (camTrace->fraction * MAX_XHAIR_DIST_ACCURACY) > minAutoAimDist &&
      ((camTrace->fraction * MAX_XHAIR_DIST_ACCURACY) -
       Distance(entOrg, camPos)) <
          bestDist) { // this trace hit *something* that's closer than the thing
                      // the main trace hit, so use this result instead
...
void PM_RocketLock(float lockDist, qboolean vehicleLock) {
  ...
  pm->trace(&tr, muzzlePoint, NULL, NULL, ang, pm->ps->clientNum,
            MASK_PLAYERSOLID);

  if (vehicleLock) { // vehicles also do a trace from the camera point if the
                     // main one misses
    if (tr.fraction >= 1.0f) {
      trace_t camTrace;
      vec3_t newEnd, shotDir;
      if (BG_VehTraceFromCamPos(&camTrace, PM_BGEntForNum(pm->ps->clientNum),
                                pm->ps->origin, muzzlePoint, tr.endpos, newEnd,
                                shotDir, (tr.fraction * lockDist))) {
    ...
    if (bgEnt && (bgEnt->s.powerups & PW_CLOAKED)) {
      ...
      if (pm->ps->rocketLockIndex == ENTITYNUM_NONE) {
        ...
        pm->ps->rocketLockTime = pm->cmd.serverTime;
                 ...
                 pm->ps->rocketTargetTime < pm->cmd.serverTime) {
        pm->ps->rocketLockIndex = tr.entityNum;
        pm->ps->rocketLockTime = pm->cmd.serverTime;
      } else if (pm->ps->rocketLockIndex == tr.entityNum) {
...
//---------------------------------------
{
  ...
  if (vehicleRocketLock) {
    if ((pm->cmd.buttons &
         (BUTTON_ATTACK | BUTTON_ALT_ATTACK))) { // actually charging
      if (veh && veh->m_pVehicle) { // just make sure we have this veh info
        if (((pm->cmd.buttons & BUTTON_ATTACK) &&
            ...
            ((pm->cmd.buttons & BUTTON_ALT_ATTACK) &&
             ...
             pm->ps->ammo[1] >=
                 g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[1].ID]
                     .iAmmoPerShot)) { // pressing the appropriate fire button
                                       // for the lock-on/charging weapon
  ...
  if (charging) {
    if (altFire) {
      if (pm->ps->weaponstate != WEAPON_CHARGING_ALT) {
        ...
        pm->ps->weaponChargeSubtractTime =
            pm->cmd.serverTime + weaponData[pm->ps->weapon].altChargeSubTime;

#ifdef _DEBUG
        //	Com_Printf("Starting charge\n");
#endif
        assert(pm->ps->weapon > WP_NONE);
        BG_AddPredictableEventToPlayerstate(EV_WEAPON_CHARGE_ALT,
                                            pm->ps->weapon, pm->ps);
      }
      ...
      } else if (pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] <
                 (weaponData[pm->ps->weapon].altChargeSub +
                  weaponData[pm->ps->weapon].altEnergyPerShot)) {
        pm->ps->weaponstate = WEAPON_CHARGING_ALT;

        goto rest;
      } else if ((pm->cmd.serverTime - pm->ps->weaponChargeTime) <
                 weaponData[pm->ps->weapon].altMaxCharge) {
      ...
      if (pm->ps->weaponstate != WEAPON_CHARGING) {
        ...
        pm->ps->weaponChargeTime = pm->cmd.serverTime;
        pm->ps->weaponChargeSubtractTime =
            pm->cmd.serverTime + weaponData[pm->ps->weapon].chargeSubTime;

#ifdef _DEBUG
        //	Com_Printf("Starting charge\n");
#endif
        BG_AddPredictableEventToPlayerstate(EV_WEAPON_CHARGE, pm->ps->weapon,
                                            pm->ps);
      }
      ...
      if (vehicleRocketLock) {
        if (veh &&
            pm->ps->ammo[0] <
                g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID]
                    .iAmmoPerShot) { // check vehicle ammo
          pm->ps->weaponstate = WEAPON_CHARGING;
          goto rest;
        }
      } else if (pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] <
                 (weaponData[pm->ps->weapon].chargeSub +
                  weaponData[pm->ps->weapon].energyPerShot)) {
        pm->ps->weaponstate = WEAPON_CHARGING;

        goto rest;
      } else if ((pm->cmd.serverTime - pm->ps->weaponChargeTime) <
                 weaponData[pm->ps->weapon].maxCharge) {
        if (pm->ps->weaponChargeSubtractTime < pm->cmd.serverTime) {
          pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] -=
              weaponData[pm->ps->weapon].chargeSub;
          pm->ps->weaponChargeSubtractTime =
...
int PM_ItemUsable(playerState_t *ps, int forcedUse) {
  ...
  if (ps->duelInProgress) { // not allowed to use holdables while in a private
                            // duel.
    return 0;
  }
  ...
  case HI_SENTRY_GUN:
    ...
    pm->trace(&tr, ps->origin, mins, maxs, trtest, ps->clientNum,
              MASK_PLAYERSOLID);
...
void PM_VehicleWeaponAnimate(void) {
  ...
  if (!veh || !veh->m_pVehicle || !veh->m_pVehicle->m_pPilot ||
      !veh->m_pVehicle->m_pPilot->playerState ||
      pm->ps->clientNum != veh->m_pVehicle->m_pPilot->playerState
                               ->clientNum) { // make sure the vehicle exists,
                                              // and its pilot is this player
    return;
  }
  ...
  if (pVeh->m_pVehicleInfo->type == VH_WALKER ||
      pVeh->m_pVehicleInfo->type ==
          VH_FIGHTER) { // slightly hacky I guess, but whatever.
    return;
  }
backAgain:
  ...
  if (pm->cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK)) {
    ...
    case WP_SABER:
      ...
      if (pm->ps->torsoTimer <=
          0) { // restart the anim if we are already in it (and finished)
    ...
    default:
      ...
      break;
             ...
             pVeh->m_pVehicleInfo->type ==
                 VH_ANIMAL) { // tauntaun is going backwards
    Anim = BOTH_VT_WALK_REV;
  } else if (veh->playerState && veh->playerState->speed < 0 &&
             pVeh->m_pVehicleInfo->type ==
                 VH_SPEEDER) { // speeder is going backwards
    Anim = BOTH_VS_REV;
  }
  // They're not firing so play the Idle for the weapon.
    ...
    case WP_SABER:
      ...
      // else if ( pVeh->m_ulFlags & VEH_CRASHING )
        ...
        pVeh->m_ulFlags &=
            ~VEH_CRASHING; // Remove the flag, we are doing the animation.
  ...
  if (Anim != -1) { // override it
    if (pVeh->m_pVehicleInfo->type ==
        VH_ANIMAL) { // agh.. remap anims for the tauntaun
      switch (Anim) {
      case BOTH_VS_IDLE:
        if (veh->playerState && veh->playerState->speed > 0) {
          if (veh->playerState->speed >
              pVeh->m_pVehicleInfo->speedMax) { // turbo
            Anim = BOTH_VT_TURBO;
          } else {
...
static void PM_Weapon(void) {
...
#ifdef _GAME
  if (pm->ps->clientNum >= MAX_CLIENTS && pm->ps->weapon == WP_NONE &&
      pm->cmd.weapon == WP_NONE && pm_entSelf) { // npc with no weapon
    ...
    if ((veh = pm_entVeh) &&
        (veh->m_pVehicle &&
         (veh->m_pVehicle->m_pVehicleInfo->type == VH_WALKER ||
          veh->m_pVehicle->m_pVehicleInfo->type ==
              VH_FIGHTER))) { // riding a walker/fighter
      // keep saber off, do no weapon stuff at all!
      pm->ps->saberHolstered = 2;
#ifdef _GAME
      pm->cmd.buttons &= ~(BUTTON_ATTACK | BUTTON_ALT_ATTACK);
#else
      if (g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID]
              .fHoming ||
          g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[1].ID]
              .fHoming) { // our vehicle uses a rocket launcher, so do the
                          // normal checks
  ...
  { // check for exceeding max charge time if not using disruptor or rocket
    // launcher or thermals
    if (pm->ps->weaponstate == WEAPON_CHARGING_ALT) {
  ...
  if (pm->ps->forceHandExtend == HANDEXTEND_WEAPONREADY &&
    ...
    pm->ps->forceHandExtend = HANDEXTEND_NONE;
  } else if (pm->ps->forceHandExtend !=
             HANDEXTEND_NONE) { // nothing else should be allowed to happen
                                // during this time, including weapon fire
    int desiredAnim = 0;
    ...
    case HANDEXTEND_FORCE_HOLD:
      ...
      } else if ((pm->ps->fd.forcePowersActive &
                  (1 << FP_LIGHTNING))) { // lightning
        if (pm->ps->weapon == WP_MELEE &&
    ...
    case HANDEXTEND_KNOCKDOWN:
      if (pm->ps->forceDodgeAnim) {
        if (pm->ps->forceDodgeAnim > 4) { // this means that we want to play a
                                          // sepereate anim on the torso
          int originalDAnim =
              pm->ps->forceDodgeAnim - 8; //-8 is the original legs anim
          if (originalDAnim == 2) {
    ...
    case HANDEXTEND_JEDITAUNT:
      ...
      // BOTH_MINDTRICK1 // wave (maybe for mind trick 2 & 3 - whole area, and
      // for force seeing) BOTH_MINDTRICK2 // tap (maybe for mind trick 1 - one
      // person) BOTH_FORCEGRIP_START //start grip BOTH_FORCEGRIP_HOLD //hold
      // grip BOTH_FORCEGRIP_RELEASE //release grip BOTH_FORCELIGHTNING //quick
      // lightning burst (level 1) BOTH_FORCELIGHTNING_START //start lightning
    ...
    if (!seperateOnTorso) { // of seperateOnTorso, handle it after setting the
                            // legs
      PM_SetAnim(SETANIM_TORSO, desiredAnim,
                 SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      pm->ps->torsoTimer = 1;
    }

    if (playFullBody) { // sorry if all these exceptions are getting confusing.
                        // This one just means play on both legs and torso.
      PM_SetAnim(SETANIM_BOTH, desiredAnim,
                 SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      pm->ps->legsTimer = pm->ps->torsoTimer = 1;
               ...
               (pm->ps->forceHandExtend == HANDEXTEND_CHOKE &&
                pm->ps->groundEntityNum ==
                    ENTITYNUM_NONE)) { // special case, play dodge anim on whole
                                       // body, choke anim too if off ground
      if (seperateOnTorso) {
        PM_SetAnim(SETANIM_LEGS, desiredAnim,
                   SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        pm->ps->legsTimer = 1;

        PM_SetAnim(SETANIM_TORSO, desiredOnTorso,
                   SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        pm->ps->torsoTimer = 1;
      } else {
        PM_SetAnim(SETANIM_LEGS, desiredAnim,
  ...
  if (pm->ps->weapon == WP_SABER && pm->ps->saberMove != LS_READY &&
      pm->ps->saberMove != LS_NONE) {
  ...
  if (pm->cmd.buttons & BUTTON_USE_HOLDABLE) {
    ...
    if (!(pm->ps->pm_flags & PMF_USE_ITEM_HELD)) {
      ...
      if (pm_entSelf->s.NPC_class != CLASS_VEHICLE &&
          pm->ps->m_iVehicleNum) { // riding a vehicle, can't use holdable
                                   // items, this button operates as the weapon
                                   // link/unlink toggle
        return;
      }
      ...
      if (!PM_ItemUsable(pm->ps, 0)) {
        ...
        if (pm->ps->stats[STAT_HOLDABLE_ITEMS] &
            (1 << bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag)) {
          if (bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag !=
              ...
              bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag !=
                  HI_CLOAK &&
              bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag !=
                  HI_EWEB) { // never use up the binoculars or jetpack or
                             // dispensers or cloak or ...
            pm->ps->stats[STAT_HOLDABLE_ITEMS] -=
                (1 << bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag);
          }
        } else {
  ...
  /*
  if (pm->ps->weapon == WP_SABER || pm->ps->weapon == WP_MELEE)
  { //we can't toggle zoom while using saber (for obvious reasons) so make sure
  it's always off pm->ps->zoomMode = 0; pm->ps->zoomFov = 0; pm->ps->zoomLocked
  = qfalse; pm->ps->zoomLockTime = 0;
  }
  */
  ...
  if (pm->ps->weapon != WP_NONE && pm->ps->weapon == pm->cmd.weapon &&
    ...
    if (pm->ps->clientNum < MAX_CLIENTS &&
      ...
      if (pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] <
              weaponData[pm->ps->weapon].energyPerShot &&
          pm->ps->ammo[weaponData[pm->ps->weapon].ammoIndex] <
              weaponData[pm->ps->weapon]
                  .altEnergyPerShot) { // the weapon is out of ammo essentially
                                       // because it cannot fire primary or
                                       // secondary, so do the switch regardless
                                       // of if the player is attacking or not
  ...
  if (PM_CanSetWeaponAnims() && !PM_IsRocketTrooper() &&
    ...
    if (pm->ps->weaponTime <= 0 && pm->ps->forceHandExtend == HANDEXTEND_NONE) {
      ...
      if (desTAnim == BOTH_STAND1 ||
          desTAnim == BOTH_STAND2) { // remap the standard standing anims for
                                     // melee stance
      ...
      if (!(pm->cmd.buttons &
            (BUTTON_ATTACK |
             BUTTON_ALT_ATTACK))) { // don't do this while holding attack
  ...
  if (veh && veh->m_pVehicle) {
    if (g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID]
            .fHoming ||
        g_vehWeaponInfo[veh->m_pVehicle->m_pVehicleInfo->weapon[1].ID]
            .fHoming) { // don't clear the rocket locking ever?
      vehicleRocketLock = qtrue;
    }
  }
  ...
  if (!vehicleRocketLock) {
    if (pm->ps->weapon != WP_ROCKET_LAUNCHER) {
      if (pm_entSelf->s.NPC_class != CLASS_VEHICLE &&
          pm->ps->m_iVehicleNum) { // riding a vehicle, the vehicle will tell me
                                   // my rocketlock stuff...
      } else {
        pm->ps->rocketLockIndex = ENTITYNUM_NONE;
        pm->ps->rocketLockTime = 0;
  ...
  if (pm->ps->weapon == WP_EMPLACED_GUN) {
    ...
    return;
             ...
             pm_entSelf->s.NPC_class ==
                 CLASS_VEHICLE) { // a vehicle NPC that has a pilot
    pm->ps->weaponstate = WEAPON_FIRING;
    pm->ps->weaponTime += 100;
#ifdef _GAME // hack, only do it game-side. vehicle weapons don't really need
             // predicting I suppose.
    if ((pm->cmd.buttons & BUTTON_ALT_ATTACK)) {
  ...
  if (pm->ps->weapon == WP_DISRUPTOR && (pm->cmd.buttons & BUTTON_ALT_ATTACK) &&
      pm->ps->zoomMode ==
          2) { // can't use disruptor secondary while zoomed binoculars
    return;
  }
  ...
  if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1) {
    PM_StartTorsoAnim(BOTH_ATTACK4);
             ...
             WP_MELEE) { // special anims for standard melee attacks
    // Alternate between punches and use the anim length as weapon time.
    if (!pm->ps->m_iVehicleNum) { // if riding a vehicle don't do this stuff at
                                  // all
      if (pm->debugMelee && (pm->cmd.buttons & BUTTON_ATTACK) &&
          (pm->cmd.buttons & BUTTON_ALT_ATTACK)) { // ok, grapple time
#if 0 // eh, I want to try turning the saber off, but can't do that reliably for
      // prediction..
        ...
        if (!BG_KickingAnim(pm->ps->torsoAnim) &&
          ...
          if (kickMove != -1) {
            if (pm->ps->groundEntityNum ==
              ...
              // let's only allow air kicks if a certain distance from the
              // ground it's silly to be able to do them right as you land. also
              // looks wrong to transition from a non-complete flip anim...
              if ((!BG_FlippingAnim(pm->ps->legsAnim) ||
                   pm->ps->legsTimer <= 0) &&
                  gDist > 64.0f && // strict minimum
                  gDist > (-pm->ps->velocity[2]) -
                              64.0f // make sure we are high to ground relative
                                    // to downward velocity as well
              ) {
                switch (kickMove) {
                case LS_KICK_F:
          ...
          if (kickMove != -1) {
            ...
            if (kickAnim != -1) {
              PM_SetAnim(SETANIM_BOTH, kickAnim,
                         SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  ...
  if (pm->cmd.buttons & BUTTON_ALT_ATTACK) {
    ...
    if (0) { // kind of a hack for now
      ...
      if (pm->ps->weapon != WP_MELEE ||
          !pm->ps->m_iVehicleNum) { // do not fire melee events at all when on
                                    // vehicle
        PM_AddEvent(EV_ALT_FIRE);
      }
    ...
    if (pm->gametype == GT_SIEGE &&
        pm->ps->weapon ==
            WP_DET_PACK) { // were far too spammy before?  So says Rick.
      addTime *= 2;
    }
  }
...
qboolean BG_UnrestrainedPitchRoll(playerState_t *ps, Vehicle_t *pVeh) {
  if (bg_fighterAltControl.integer && ps->clientNum < MAX_CLIENTS // real client
...
void PM_UpdateViewAngles(playerState_t *ps, const usercmd_t *cmd) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (pm_entVeh && pm_entVeh->m_pVehicle &&
        ...
        (cmd->serverTime - pm_entVeh->playerState->hyperSpaceTime) >=
            HYPERSPACE_TIME) { // in a vehicle and not hyperspacing
      if (i == PITCH) {
        int pitchClamp = ANGLE2SHORT(AngleNormalize180(
            pm_entVeh->m_pVehicle->m_vPrevRiderViewAngles[PITCH] + 10.0f));
      ...
      if (i == YAW) {
        int yawClamp = ANGLE2SHORT(AngleNormalize180(
            pm_entVeh->m_pVehicle->m_vPrevRiderViewAngles[YAW] + 10.0f));
    ...
    if (pm_entVeh &&
        BG_UnrestrainedPitchRoll(ps, pm_entVeh->m_pVehicle)) { // in a fighter
      /*
      if ( i == ROLL )
      {//get roll from vehicle
              ps->viewangles[ROLL] =
      pm_entVeh->playerState->viewangles[ROLL];//->m_pVehicle->m_vOrientation[ROLL];
              continue;
...
void PM_UpdateViewAngles( playerState_t *ps, const usercmd_t *cmd ) {
        short		temp;
        int		i;
        float	rootPitch = 0, pitchMin=-90, pitchMax=90, yawMin=0, yawMax=0,
lockedYawValue = 0;	//just to shut up warnings qboolean	lockedYaw =
qfalse, clamped = qfalse; bgEntity_t *vehEnt = NULL;
...
VH_FIGHTER )
                                        {
                                                rootPitch =
pVeh->m_vOrientation[PITCH];//gent->owner->client->ps.vehicleAngles[PITCH];//???
what if goes over 90 when add the min/max? if ( pVeh->m_pVehicleInfo->pitchLimit
== -1 )
                                                {
                                                        pitchMax = 180;
                                                }
                                                ...
                                                {
                                                        pitchMax =
pVeh->m_pVehicleInfo->pitchLimit;
                                                }
                                                pitchMin = -pitchMax;
                                        }
                                        else
                                        {
                                                lockedYawValue =
0;//gent->owner->client->ps.vehicleAngles[YAW]; lockedYaw = qtrue; yawMax =
pVeh->m_pVehicleInfo->lookYaw; yawMin = -yawMax; rootPitch =
0;//gent->owner->client->ps.vehicleAngles[PITCH];//???  what if goes over 90
when add the min/max? pitchMax = pVeh->m_pVehicleInfo->lookPitch; pitchMin =
-pitchMax;
                                        }
                                }
                        }
        ...
        if ( 1 )
        {
                ...
                for (i=0 ; i<3 ; i++)
                {
                        ...
                        if ( i == PITCH )
                        {
                                ...
                                // don't let the player look up or down more
than 90 degrees if ( temp > pitchClampMax )
                                {
                                        ps->delta_angles[i] = (pitchClampMax -
cmd->angles[i]) & 0xffff;	//& clamp to short temp = pitchClampMax; clamped
= qtrue;
                                }
                                else if ( temp < pitchClampMin )
                                {
                                        ps->delta_angles[i] = (pitchClampMin -
cmd->angles[i]) & 0xffff;	//& clamp to short temp = pitchClampMin; clamped
= qtrue;
                                }
                        }
                        if ( i == YAW && lockedYaw )
                        {
                                //FIXME get this limit from the NPCs stats?
                                // don't let the player look up or down more
than 90 degrees if ( temp > yawClampMax )
                                {
                                        ps->delta_angles[i] = (yawClampMax -
cmd->angles[i]) & 0xffff;	//& clamp to short temp = yawClampMax; clamped =
qtrue;
                                }
                                else if ( temp < yawClampMin )
                                {
                                        ps->delta_angles[i] = (yawClampMin -
cmd->angles[i]) & 0xffff;	//& clamp to short temp = yawClampMin; clamped =
qtrue;
                                }
                                ps->viewangles[i] = SHORT2ANGLE(temp);
                        }
        ...
        else
        {
                ...
                for (i=0 ; i<3 ; i++) {
                        ...
                        if ( i == PITCH ) {
                                // don't let the player look up or down more
than 90 degrees if ( temp > 16000 ) { ps->delta_angles[i] = 16000 -
cmd->angles[i]; temp = 16000; } else if ( temp < -16000 ) { ps->delta_angles[i]
= -16000 - cmd->angles[i]; temp = -16000;
                                }
                        }
                        ps->viewangles[i] = SHORT2ANGLE(temp);
                }
...
//-------------------------------------------
{
  ...
  if (pmove->ps->weapon == WP_DISRUPTOR &&
    ...
    if ( !(pmove->ps->eFlags & EF_ALT_FIRING) && (pmove->cmd.buttons & BUTTON_ALT_ATTACK) /*&&
      ...
      if (!pmove->ps->zoomMode && pmove->ps->pm_type != PM_DEAD) {
        ...
        PM_AddEvent(EV_DISRUPTOR_ZOOMSOUND);
                 ...
                 pmove->ps->zoomLockTime <
                     pmove->cmd
                         .serverTime) { // check for == 1 so we can't turn
                                        // binoculars off with disruptor alt fire
    ...
    } else if (!(pmove->cmd.buttons & BUTTON_ALT_ATTACK) &&
               pmove->ps->zoomLockTime < pmove->cmd.serverTime) {
      // Not pressing zoom any more
      if (pmove->ps->zoomMode) {
        if (pmove->ps->zoomMode == 1 &&
            !pmove->ps
                 ->zoomLocked) { // approximate what level the client should be
                                 // zoomed at based on how long zoom was held
          pmove->ps->zoomFov =
              ((pmove->cmd.serverTime + 50) - pmove->ps->zoomLockTime) * 0.035f;
    ...
    // This seemed like a good idea, but apparently it confuses people. So
    // disabled for now.
    /*
    else if (!(pmove->ps->eFlags & EF_ALT_FIRING) && (pmove->cmd.buttons &
    BUTTON_ALT_ATTACK) && (pmove->cmd.upmove > 0 || pmove->cmd.forwardmove ||
    pmove->cmd.rightmove)) { //if you try to zoom while moving, just convert it
    into a primary attack pmove->cmd.buttons &= ~BUTTON_ALT_ATTACK;
    ...
    */
    ...
    {
            if (pmove->ps->zoomMode == 1 && pmove->ps->zoomLockTime <
    pmove->cmd.serverTime) { //check for == 1 so we can't turn binoculars off
    with disruptor alt fire pmove->ps->zoomMode = 0; pmove->ps->zoomTime =
    pmove->ps->commandTime; pmove->ps->zoomLocked = qfalse;
                    PM_AddEvent(EV_DISRUPTOR_ZOOMSOUND);
            }
    }
    */
  ...
  /*
  else if (pmove->ps->weapon == WP_DISRUPTOR) //still perform certain checks,
  even if the weapon is not ready
  {
          if (pmove->cmd.upmove > 0 || pmove->cmd.forwardmove ||
  pmove->cmd.rightmove)
          {
                  if (pmove->ps->zoomMode == 1 && pmove->ps->zoomLockTime <
  pmove->cmd.serverTime) { //check for == 1 so we can't turn binoculars off with
  disruptor alt fire pmove->ps->zoomMode = 0; pmove->ps->zoomTime =
  pmove->ps->commandTime; pmove->ps->zoomLocked = qfalse;
                          PM_AddEvent(EV_DISRUPTOR_ZOOMSOUND);
                  }
          }
  ...
  */
  ...
  // set the firing flag for continuous beam weapons, saber will fire even if
  // out of ammo
  ...
  // zoomed
  if (pmove->ps->weapon == WP_DISRUPTOR) {
    if (pmove->cmd.buttons & BUTTON_ATTACK && pmove->ps->zoomMode == 1 &&
        pmove->ps->zoomLocked) {
      // converting the main fire to an alt-fire
      pmove->cmd.buttons |= BUTTON_ALT_ATTACK;
      pmove->ps->eFlags |= EF_ALT_FIRING;
    } else if (pmove->cmd.buttons & BUTTON_ALT_ATTACK &&
               pmove->ps->zoomMode == 1 && pmove->ps->zoomLocked) {
      pmove->cmd.buttons &= ~BUTTON_ALT_ATTACK;
      pmove->ps->eFlags &= ~EF_ALT_FIRING;
    }
  }
...
void BG_CmdForRoll(playerState_t *ps, int anim, usercmd_t *pCmd) {
  ...
  case BOTH_GETUP_FROLL_R:
    if (ps->legsTimer <= 250) { // end of anim
  ...
  case BOTH_GETUP_BROLL_B:
    if (ps->torsoTimer <= 250) { // end of anim
      pCmd->forwardmove = pCmd->rightmove = 0;
    } else if (PM_AnimLength(0, (animNumber_t)ps->legsAnim) - ps->torsoTimer <
  ...
  case BOTH_GETUP_FROLL_B:
    if (ps->torsoTimer <= 100) { // end of anim
      pCmd->forwardmove = pCmd->rightmove = 0;
    } else if (PM_AnimLength(0, (animNumber_t)ps->legsAnim) - ps->torsoTimer <
  ...
  case BOTH_GETUP_BROLL_F:
    if (ps->torsoTimer <= 550) { // end of anim
      pCmd->forwardmove = pCmd->rightmove = 0;
    } else if (PM_AnimLength(0, (animNumber_t)ps->legsAnim) - ps->torsoTimer <
...
void BG_AdjustClientSpeed(playerState_t *ps, usercmd_t *cmd, int svTime) {
  ...
  if (ps->clientNum >= MAX_CLIENTS) {
    ...
    if (bgEnt && bgEnt->s.NPC_class ==
                     CLASS_VEHICLE) { // vehicles manage their own speed
      return;
    }
  }

  // For prediction, always reset speed back to the last known server base speed
  // If we didn't do this, under lag we'd eventually dwindle speed down to 0
  // even though that would not be the correct predicted value.
  ...
  if (BG_SaberInAttack(ps->saberMove) &&
    ...
    if (ps->fd.saberAnimLevel == FORCE_LEVEL_3) {
      ...
      ps->speed *= 0.5f;
             ...
             BG_SaberInAttack(ps->saberMove)) { // if attacking with saber while
                                                // running, drop your speed
    switch (ps->fd.saberAnimLevel) {
    case FORCE_LEVEL_2:
    case SS_DUAL:
    case SS_STAFF:
    ...
    default:
      break;
             ...
             PM_SaberInTransition(
                 ps->saberMove)) { // Now, we want to even slow down in
                                   // transitions for level 3 (since it has
                                   // chains and stuff now)
  ...
  if (BG_InRoll(ps, ps->legsAnim) &&
      ps->speed > 50) { // can't roll unless you're able to move normally
    if ((ps->legsAnim) ==
...
void BG_IK_MoveArm(void *ghoul2, int lHandBolt, int time, entityState_t *ent,
  ...
  if (!*ikInProgress && !forceHalt) {
    ...
    ikP.startFrame = bgHumanoidAnimations[baseposeAnim].firstFrame +
                     bgHumanoidAnimations[baseposeAnim].numFrames;
    ikP.endFrame = bgHumanoidAnimations[baseposeAnim].firstFrame +
                   bgHumanoidAnimations[baseposeAnim].numFrames;

    ikP.forceAnimOnBone =
        qfalse; // let it use existing anim if it's the same as this one.
    ...
    if (trap->G2API_SetBoneIKState(ghoul2, time, "lhumerus", IKS_DYNAMIC,
                                   &ikP)) {
      // restrict the elbow joint
      VectorSet(ikP.pcjMins, -90.0f, -20.0f, -20.0f);
      VectorSet(ikP.pcjMaxs, 30.0f, 20.0f, -20.0f);

      if (trap->G2API_SetBoneIKState(ghoul2, time, "lradius", IKS_DYNAMIC,
  ...
  if (*ikInProgress && !forceHalt) { // actively update our ik state.
    ...
    trap->G2API_GetBoltMatrix(ghoul2, 0, lHandBolt, &lHandMatrix, tAngles,
                              origin, time, 0, scale);
    ...
    trap->G2API_SetBoneAngles(ghoul2, 0, "lhumerus", vec3_origin,
                              BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                              NEGATIVE_Z, NULL, 0, time);
    trap->G2API_SetBoneAngles(ghoul2, 0, "lradius", vec3_origin,
                              BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                              NEGATIVE_Z, NULL, 0, time);

    // Get the anim/frames that the pelvis is on exactly, and match the left arm
    // back up with them again.
    trap->G2API_GetBoneAnim(ghoul2, "pelvis", (const int)time, &cFrame, &sFrame,
                            &eFrame, &flags, &animSpeed, 0, 0);
    trap->G2API_SetBoneAnim(ghoul2, 0, "lhumerus", sFrame, eFrame, flags,
                            animSpeed, time, sFrame, 300);
    trap->G2API_SetBoneAnim(ghoul2, 0, "lradius", sFrame, eFrame, flags,
                            animSpeed, time, sFrame, 300);
...
void BG_UpdateLookAngles(int lookingDebounceTime, vec3_t lastHeadAngles,
                         int time, vec3_t lookAngles, float lookSpeed,
  ...
  if (lookingDebounceTime > time) {
    ...
    if (VectorLengthSquared(lookAnglesDiff)) {
      lookAngles[PITCH] =
          AngleNormalize180(oldLookAngles[PITCH] +
                            (lookAnglesDiff[PITCH] * fFrameInter * lookSpeed));
      lookAngles[YAW] = AngleNormalize180(
          oldLookAngles[YAW] + (lookAnglesDiff[YAW] * fFrameInter * lookSpeed));
      lookAngles[ROLL] =
          AngleNormalize180(oldLookAngles[ROLL] +
                            (lookAnglesDiff[ROLL] * fFrameInter * lookSpeed));
    }
  }
...
static void BG_G2ClientNeckAngles(void *ghoul2, int time,
                                  const vec3_t lookAngles, vec3_t headAngles,
                                  vec3_t neckAngles, vec3_t thoracicAngles,
  ...
  */
  ...
  trap->G2API_SetBoneAngles(ghoul2, 0, "cranium", headAngles,
                            BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                            NEGATIVE_Z, 0, 0, time);
  trap->G2API_SetBoneAngles(ghoul2, 0, "cervical", neckAngles,
                            BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                            NEGATIVE_Z, 0, 0, time);
  trap->G2API_SetBoneAngles(ghoul2, 0, "thoracic", thoracicAngles,
                            BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                            NEGATIVE_Z, 0, 0, time);
}
...
static void
BG_G2ClientSpineAngles(void *ghoul2, int motionBolt, vec3_t cent_lerpOrigin,
                       vec3_t cent_lerpAngles, entityState_t *cent, int time,
                       vec3_t viewAngles, int ciLegs, int ciTorso,
                       const vec3_t angles, vec3_t thoracicAngles,
                       vec3_t ulAngles, vec3_t llAngles, vec3_t modelScale,
                       float *tPitchAngle, float *tYawAngle, int *corrTime) {
  ...
  if (!BG_FlippingAnim(cent->legsAnim) &&
      ...
      !BG_InSpecialJump(cent->legsAnim) && !BG_InSpecialJump(cent->torsoAnim) &&
      !BG_InDeathAnim(cent->legsAnim) && !BG_InDeathAnim(cent->torsoAnim) &&
      !BG_InRollES(cent, cent->legsAnim) && !BG_InRollAnim(cent) &&
      !BG_SaberInSpecial(cent->saberMove) &&
      !BG_SaberInSpecialAttack(cent->torsoAnim) &&
      !BG_SaberInSpecialAttack(cent->legsAnim) &&

      !BG_InKnockDown(cent->torsoAnim) && !BG_InKnockDown(cent->legsAnim) &&
      !BG_InKnockDown(ciTorso) && !BG_InKnockDown(ciLegs) &&

      !BG_FlippingAnim(ciLegs) && !BG_SpinningSaberAnim(ciLegs) &&
      !BG_SpinningSaberAnim(ciTorso) && !BG_InSpecialJump(ciLegs) &&
      !BG_InSpecialJump(ciTorso) && !BG_InDeathAnim(ciLegs) &&
      !BG_InDeathAnim(ciTorso) && !BG_SaberInSpecialAttack(ciTorso) &&
      !BG_SaberInSpecialAttack(ciLegs) &&

      !(cent->eFlags & EF_DEAD) && (cent->legsAnim) != (cent->torsoAnim) &&
      (ciLegs) != (ciTorso) && !cent->m_iVehicleNum) {
    doCorr = qtrue;
  }
#else
  if (((!BG_FlippingAnim(cent->legsAnim) &&
        ...
        !BG_SpinningSaberAnim(cent->torsoAnim) &&
        (cent->legsAnim) !=
            (cent->torsoAnim)) // NOTE: presumes your legs & torso are on the
                               // same frame, though they *should* be because
                               // PM_SetAnimFinal tries to keep them in synch
       || (!BG_FlippingAnim(ciLegs) && !BG_SpinningSaberAnim(ciLegs) &&
           !BG_SpinningSaberAnim(ciTorso) && (ciLegs) != (ciTorso))) ||
      ciLegs != cent->legsAnim || ciTorso != cent->torsoAnim) {
    doCorr = qtrue;
    *corrTime = time + 1000; // continue correcting for a second after to smooth
                             // things out. SP doesn't need this for whatever
                             // reason but I can't find a way around it.
  } else if (*corrTime >= time) {
    if (!BG_FlippingAnim(cent->legsAnim) &&
        !BG_SpinningSaberAnim(cent->legsAnim) &&
        !BG_SpinningSaberAnim(cent->torsoAnim) && !BG_FlippingAnim(ciLegs) &&
  ...
  if (doCorr) { // FIXME: no need to do this if legs and torso on are same frame
    ...
    trap->G2API_GetBoltMatrix_NoRecNoRot(ghoul2, 0, motionBolt, &boltMatrix,
                                         vec3_origin, cent_lerpOrigin, time, 0,
                                         modelScale);
...
==================
*/
static float BG_SwingAngles(float destination, float swingTolerance,
                            float clampTolerance, float speed, float *angle,
                            qboolean *swinging, int frametime) {
  float swing;
  float move;
  float scale;
...
void BG_G2PlayerAngles(void *ghoul2, int motionBolt, entityState_t *cent,
  ...
  static int dir;
  static int i;
  //	static int			movementOffsets[8] = { 0, 22, 45, -22, 0, 22, -45,
  //-22 };
  ...
  static vec3_t ulAngles, llAngles, viewAngles, angles,
      thoracicAngles = {0, 0, 0};
  static vec3_t headClampMinAngles = {-25, -55, -10},
                headClampMaxAngles = {50, 50, 10};

  if (cent->m_iVehicleNum || cent->forceFrame ||
      BG_SaberLockBreakAnim(cent->legsAnim) ||
      BG_SaberLockBreakAnim(
          cent->torsoAnim)) { // a vehicle or riding a vehicle - in either case
    ...
    if (cent->number < MAX_CLIENTS) {
      trap->G2API_SetBoneAngles(ghoul2, 0, "lower_lumbar", vec3_origin,
                                BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                NEGATIVE_Z, 0, 0, time);
      trap->G2API_SetBoneAngles(ghoul2, 0, "upper_lumbar", vec3_origin,
                                BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                NEGATIVE_Z, 0, 0, time);
      trap->G2API_SetBoneAngles(ghoul2, 0, "cranium", vec3_origin,
                                BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                NEGATIVE_Z, 0, 0, time);
      trap->G2API_SetBoneAngles(ghoul2, 0, "thoracic", vec3_origin,
                                BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                NEGATIVE_Z, 0, 0, time);
      trap->G2API_SetBoneAngles(ghoul2, 0, "cervical", vec3_origin,
                                BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                NEGATIVE_Z, 0, 0, time);
    }
    return;
  ...
  *tYawAngle = torsoAngles[YAW];
  ...
  if (cent->groundEntityNum == ENTITYNUM_NONE || cent->forceFrame ||
      (cent->weapon == WP_EMPLACED_GUN &&
       emplaced)) { // off the ground, no direction-based leg angles (same if in
                    // saberlock)
  ...
  if (cent->m_iVehicleNum) { // swing instantly on vehicles
    *lYawAngle = legsAngles[YAW];
    ...
    BG_SwingAngles(legsAngles[YAW], /*40*/ 0, 90, 0.65f, lYawAngle, lYawing,
                   frametime);
  }
  ...
  */
  ...
  if (cent->heldByClient) { // keep the base angles clear when doing the IK
                            // stuff, it doesn't compensate for it.
                            // rwwFIXMEFIXME: Store leg angles off and add them
                            // to all the fed in angles for G2 functions?
    VectorClear(legsAngles);
  ...
  if (cent->weapon == WP_EMPLACED_GUN &&
      emplaced) { // if using an emplaced gun, then we want to make sure we're
                  // angled to "hold" it right
    vec3_t facingAngles;
    ...
    if (emplaced->weapon == WP_NONE) { // e-web
      ...
      float emplacedDif =
          AngleSubtract(cent_lerpAngles[YAW], facingAngles[YAW]);
      ...
      */
      ...
      if (cent->legsAnim == BOTH_STRAFE_LEFT1 ||
          cent->legsAnim ==
              BOTH_STRAFE_RIGHT1) { // try to adjust so it doesn't look wrong
        if (crazySmoothFactor) { // want to smooth a lot during this because it
                                 // chops around and looks like ass
          *crazySmoothFactor = time + 1000;
        }

        BG_G2ClientSpineAngles(
            ghoul2, motionBolt, cent_lerpOrigin, cent_lerpAngles, cent, time,
            viewAngles, ciLegs, ciTorso, angles, thoracicAngles, ulAngles,
            llAngles, modelScale, tPitchAngle, tYawAngle, corrTime);
        trap->G2API_SetBoneAngles(ghoul2, 0, "lower_lumbar", llAngles,
                                  BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                  NEGATIVE_Z, 0, 0, time);
        trap->G2API_SetBoneAngles(ghoul2, 0, "upper_lumbar", ulAngles,
                                  BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                  NEGATIVE_Z, 0, 0, time);
        trap->G2API_SetBoneAngles(ghoul2, 0, "cranium", vec3_origin,
                                  BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
        ...
        if (cent->legsAnim ==
            BOTH_STRAFE_LEFT1) { // this one needs some further correction
          facingAngles[YAW] -= 32.0f;
        }
      } else {
        //	trap->G2API_SetBoneAngles(ghoul2, 0, "lower_lumbar",
        //vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
        //0, 0, time); 	trap->G2API_SetBoneAngles(ghoul2, 0, "upper_lumbar",
        //vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
        //0, 0, time);
        trap->G2API_SetBoneAngles(ghoul2, 0, "cranium", vec3_origin,
                                  BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
      ...
      trap->G2API_SetBoneAngles(ghoul2, 0, "upper_lumbar", facingAngles,
                                BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                NEGATIVE_Z, 0, 0, time);
      VectorScale(facingAngles, 0.8f, facingAngles);
      trap->G2API_SetBoneAngles(ghoul2, 0, "thoracic", facingAngles,
                                BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                NEGATIVE_Z, 0, 0, time);
      ...
      VectorScale(facingAngles, 0.6f, facingAngles);
      trap->G2API_SetBoneAngles(ghoul2, 0, "cervical", facingAngles,
                                BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                NEGATIVE_Z, 0, 0, time);
  ...
  BG_G2ClientSpineAngles(ghoul2, motionBolt, cent_lerpOrigin, cent_lerpAngles,
                         cent, time, viewAngles, ciLegs, ciTorso, angles,
                         thoracicAngles, ulAngles, llAngles, modelScale,
  ...
  BG_UpdateLookAngles(lookTime, lastHeadAngles, time, lookAngles, lookSpeed,
                      -50.0f, 50.0f, -70.0f, 70.0f, -30.0f, 30.0f);

  BG_G2ClientNeckAngles(ghoul2, time, lookAngles, headAngles, neckAngles,
                        thoracicAngles, headClampMinAngles, headClampMaxAngles);
...
#ifdef BONE_BASED_LEG_ANGLES
  {
    ...
    strap_G2API_SetBoneAngles(ghoul2, 0, "model_root", bLAngles,
                              BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                              NEGATIVE_Z, 0, 0, time);
  ...
  trap->G2API_SetBoneAngles(ghoul2, 0, "thoracic", thoracicAngles,
                            ...
                            NEGATIVE_Z, 0, 0, time);
  //	trap->G2API_SetBoneAngles(ghoul2, 0, "cervical", vec3_origin,
  //BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
}

void BG_G2ATSTAngles(void *ghoul2, int time,
                     vec3_t cent_lerpAngles) { //																							up
                                               //right		fwd
  trap->G2API_SetBoneAngles(ghoul2, 0, "thoracic", cent_lerpAngles,
                            BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                            NEGATIVE_Z, 0, 0, time);
}

static qboolean PM_AdjustAnglesForDualJumpAttack(playerState_t *ps,
                                                 usercmd_t *ucmd) {
  // ucmd->angles[PITCH] = ANGLE2SHORT( ps->viewangles[PITCH] ) -
  // ps->delta_angles[PITCH]; ucmd->angles[YAW] = ANGLE2SHORT(
  // ps->viewangles[YAW] ) - ps->delta_angles[YAW];
  return qtrue;
}
...
static QINLINE void PM_CmdForSaberMoves(usercmd_t *ucmd) {
  ...
  if ((pm->ps->legsAnim == BOTH_JUMPATTACK6 &&
    ...
    if (pm->ps->legsAnim == BOTH_JUMPATTACK6) { // dual stance attack
      ...
      if (pm->ps->legsTimer >= lenMin &&
          pm->ps->legsTimer < lenMax) {                  // one of the two jumps
        if (pm->ps->groundEntityNum != ENTITYNUM_NONE) { // still on ground?
          // jump!
          if (pm->ps->legsAnim == BOTH_BUTTERFLY_LEFT) {
            pm->ps->velocity[2] = 350;
    ...
    if (pm->ps->groundEntityNum ==
        ENTITYNUM_NONE) { // can only turn when your feet hit the ground
...
void PM_VehicleViewAngles(playerState_t *ps, bgEntity_t *veh, usercmd_t *ucmd) {
  ...
  if (veh->m_pVehicle->m_pPilot &&
      veh->m_pVehicle->m_pPilot->s.number ==
          ps->clientNum) { // set the pilot's viewangles to the vehicle's
                           // viewangles
#ifdef VEH_CONTROL_SCHEME_4
    if (1)
#else  // VEH_CONTROL_SCHEME_4
    ...
    // NOTE: passengers can look around freely, UNLESS they're controlling a
    // turret!
    for (i = 0; i < MAX_VEHICLE_TURRETS; i++) {
      if (veh->m_pVehicle->m_pVehicleInfo->turret[i].passengerNum ==
          ps->generic1) { // this turret is my station
        setAngles = qtrue;
        clampMin[PITCH] =
            veh->m_pVehicle->m_pVehicleInfo->turret[i].pitchClampUp;
        clampMax[PITCH] =
            veh->m_pVehicle->m_pVehicleInfo->turret[i].pitchClampDown;
        clampMin[YAW] =
            veh->m_pVehicle->m_pVehicleInfo->turret[i].yawClampRight;
  ...
  if (setAngles) {
    for (i = 0; i < 3; i++) {                       // clamp viewangles
      if (clampMin[i] == -1 || clampMax[i] == -1) { // no clamp
      } else if (!clampMin[i] &&
                 !clampMax[i]) { // no allowance
                                 // ps->viewangles[i] =
                                 // veh->playerState->viewangles[i];
      } else { // allowance
...
void PM_VehicleViewAngles(playerState_t *ps, bgEntity_t *veh, usercmd_t *ucmd)
{
        ...
        {//clamp
                ...
                else if (ps->viewangles[PITCH] <
-pVeh->m_pVehicleInfo->lookPitch)
                {
                        ps->viewangles[PITCH] =
-pVeh->m_pVehicleInfo->lookPitch;
                }
...
void PM_VehForcedTurning(bgEntity_t *veh) {
...
#ifdef VEH_CONTROL_SCHEME_4
  veh->playerState->viewangles[YAW] =
      AngleSubtract(veh->playerState->viewangles[YAW], yawD);
  veh->playerState->viewangles[PITCH] =
      AngleSubtract(veh->playerState->viewangles[PITCH], pitchD);
  ...
  veh->m_pVehicle->m_vPrevRiderViewAngles[YAW] =
      AngleNormalize180(pm->ps->viewangles[YAW]);
...
void PM_VehFaceHyperspacePoint(bgEntity_t *veh) {
  ...
  if (!veh || !veh->m_pVehicle) {
    ...
    float timeFrac =
        ((float)(pm->cmd.serverTime - veh->playerState->hyperSpaceTime)) /
        HYPERSPACE_TIME;
    ...
    for (i = 0; i < 3; i++) {
      aDelta = AngleSubtract(veh->playerState->hyperSpaceAngles[i],
                             veh->m_pVehicle->m_vOrientation[i]);
      if (fabs(aDelta) < turnRate) { // all is good
        ...
        if (fabs(aDelta) < turnRate) {
          ...
          if (i == YAW) {
            veh->playerState->viewangles[i] =
                AngleNormalize360(veh->playerState->viewangles[i] + turnRate);
          } else {
            veh->playerState->viewangles[i] =
                AngleNormalize180(veh->playerState->viewangles[i] + turnRate);
          }
        } else {
          if (i == YAW) {
            veh->playerState->viewangles[i] =
    ...
    if (timeFrac < HYPERSPACE_TELEPORT_FRAC) { // haven't gone through yet
      ...
      } else if (!(veh->playerState->eFlags2 &
                   EF2_HYPERSPACE)) { // flag us as ready to hyperspace!
    ...
    for (i = 0; i < 3; i++) {
      ...
      if (fabs(aDelta) < turnRate) { // all is good
        ...
        if (fabs(aDelta) < turnRate) {
          ...
          if (i == YAW) {
            pm->ps->viewangles[i] =
                AngleNormalize360(pm->ps->viewangles[i] + turnRate);
          } else {
            pm->ps->viewangles[i] =
                AngleNormalize180(pm->ps->viewangles[i] + turnRate);
          }
        } else {
          if (i == YAW) {
            pm->ps->viewangles[i] =
...
void BG_VehicleAdjustBBoxForOrientation(
    Vehicle_t *veh, vec3_t origin, vec3_t mins, vec3_t maxs, int clientNum,
    int tracemask,
    void (*localTrace)(trace_t *results, const vec3_t start, const vec3_t mins,
                       const vec3_t maxs, const vec3_t end, int passEntityNum,
                       int contentMask)) {
  if (!veh || !veh->m_pVehicleInfo->length || !veh->m_pVehicleInfo->width ||
  ...
  //|| veh->m_LandTrace.fraction < 1.0f )
  {
    return;
             ...
             && veh->m_pVehicleInfo->type !=
                    VH_FLIER) { // only those types of vehicles have dynamic
                                // bboxes, the rest just use a static bbox
    VectorSet(maxs, veh->m_pVehicleInfo->width / 2.0f,
              veh->m_pVehicleInfo->width / 2.0f,
              veh->m_pVehicleInfo->height + DEFAULT_MINS_2);
    VectorSet(mins, veh->m_pVehicleInfo->width / -2.0f,
              veh->m_pVehicleInfo->width / -2.0f, DEFAULT_MINS_2);
    return;
  } else {
    matrix3_t axis;
    ...
    */
    ...
    if (localTrace) {
      localTrace(&trace, origin, newMins, newMaxs, origin, clientNum,
                 tracemask);
    ...
    // FIXME: make it as close as possible?  Or actually prevent the change in
    // m_vOrientation?  Or push away from anything we hit?
  }
}
/*
================
...
extern int BG_EmplacedView(vec3_t baseAngles, vec3_t angles, float *newYaw,
                           float constraint);
extern qboolean BG_FighterUpdate(
    Vehicle_t *pVeh, const usercmd_t *pUcmd, vec3_t trMins, vec3_t trMaxs,
    float gravity,
...
void PmoveSingle(pmove_t *pmove) {
  ...
  if (PM_IsRocketTrooper()) { // kind of nasty, don't let them crouch or
                              // anything if nonhumanoid (probably a
                              // rockettrooper)
    if (pm->cmd.upmove < 0) {
      pm->cmd.upmove = 0;
    }
  ...
  if (pm->ps->pm_type ==
      PM_FLOAT) { // You get no control over where you go in grip movement
    stiffenedUp = qtrue;
    ...
    stiffenedUp = qtrue;
             ...
             pm->ps->saberLockTime >=
                 pm->cmd.serverTime) { // can't move or turn
    stiffenedUp = qtrue;
    PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
    ...
    if (pm->ps->legsAnim == BOTH_JUMPFLIPSTABDOWN ||
        pm->ps->legsAnim ==
            BOTH_JUMPFLIPSLASHDOWN1) { // flipover medium stance attack
    ...
    stiffenedUp = qtrue;
  } else if (BG_KickMove(pm->ps->saberMove) ||
             BG_KickingAnim(pm->ps->legsAnim)) {
    stiffenedUp = qtrue;
    ...
    PM_SetPMViewAngle(pm->ps, pm->ps->viewangles, &pm->cmd);
             ...
             pm->ps->saberMove ==
                 LS_STABDOWN) { // FIXME: need to only move forward until we
    ...
    if ((pm->cmd.buttons & BUTTON_ATTACK) ||
      ...
      if (pm->ps->legsAnim == BOTH_MEDITATE &&
        ...
        PM_SetAnim(SETANIM_BOTH, BOTH_MEDITATE_END,
                   SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      } else {
        pm->ps->legsTimer = pm->ps->torsoTimer = 0;
      }
    ...
    pm->cmd.buttons = 0;
             ...
             pm->ps->legsAnim == BOTH_FORCELANDRIGHT1 ||
             pm->ps->legsAnim ==
                 BOTH_FORCELANDLEFT1) { // can't move while in a force land
    stiffenedUp = qtrue;
  }
  ...
  /*
  if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->weaponstate ==
  WEAPON_CHARGING_ALT) { //not allowed to move while charging the disruptor
          pm->cmd.forwardmove = 0;
          pm->cmd.rightmove = 0;
          if (pm->cmd.upmove > 0)
          {
  ...
  */
  ...
  if (pm->ps->weapon == WP_DISRUPTOR &&
      pm->ps->weaponstate == WEAPON_CHARGING_ALT) { // not allowed to move while
                                                    // charging the disruptor
    if (pm->cmd.forwardmove || pm->cmd.rightmove ||
        pm->cmd.upmove > 0) { // get out
      pm->ps->weaponstate = WEAPON_READY;
      pm->ps->weaponTime = 1000;
      PM_AddEventWithParm(EV_WEAPON_CHARGE,
                          WP_DISRUPTOR); // cut the weapon charge sound
      pm->cmd.upmove = 0;
    }
  } else if (pm->ps->weapon == WP_DISRUPTOR &&
             pm->ps->zoomMode == 1) { // can't jump
    if (pm->cmd.upmove > 0) {
      pm->cmd.upmove = 0;
    }
  }
  ...
  if (pm->ps->fd.forceGripCripple) { // don't let attack or alt attack if being
                                     // gripped I guess
    pm->cmd.buttons &= ~BUTTON_ATTACK;
    pm->cmd.buttons &= ~BUTTON_ALT_ATTACK;
  }
  ...
  if (BG_InRoll(
          pm->ps,
          pm->ps->legsAnim)) { // can't roll unless you're able to move normally
  ...
  if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1) {
    if ((pm->cmd.buttons & BUTTON_ALT_ATTACK) &&
        !(pm->cmd.buttons & BUTTON_ATTACK) && pm->ps->zoomLocked) {
...
pml.msec ); #else Com_Printf( S_COLOR_GREEN" CLIENT N%i msec %d\n",
pm->ps->clientNum, pml.msec ); #endif
  }
  */
  ...
  if (pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf &&
    ...
    if (veh && veh->playerState &&
        (pm->cmd.serverTime - veh->playerState->hyperSpaceTime) <
            HYPERSPACE_TIME) { // going into hyperspace, turn to face the right
                               // angles
      PM_VehFaceHyperspacePoint(veh);
    } else if (veh && veh->playerState &&
               veh->playerState->vehTurnaroundIndex &&
               veh->playerState->vehTurnaroundTime >
                   pm->cmd
                       .serverTime) { // riding this vehicle, turn my view too
  ...
  */
  ...
  if (pm->ps->pm_type == PM_JETPACK) {
    ...
    if (pm->ps->weapon == WP_SABER &&
        BG_SpinningSaberAnim(
            pm->ps->legsAnim)) { // make him stir around since he shouldn't have
                                 // any real control when spinning
    ...
    if (pm->cmd.upmove > 0 &&
        pm->ps->velocity[2] <
            256) { // cap upward velocity off at 256. Seems reasonable.
      float addIn = 12.0f;

      /*
                              //Add based on our distance to the ground if we're
         already travelling upward if (pm->ps->velocity[2] > 0)
                              {
                                      while (gDist > 64)
                                      { //subtract 1 for every 64 units off the
         ground we get addIn--;

                                              gDist -= 64;

                                              if (addIn <= 0)
                                              { //break out if we're not even
         going to add anything break;
                                              }
                                      }
                              }
      */
      ...
      if (pm->ps->velocity[2] < 256) {
        ...
        if (gDist <
            JETPACK_HOVER_HEIGHT) { // make sure we're always hovering off the
                                    // ground somewhat while jetpack is active
          pm->ps->velocity[2] += 2;
        }
      }
    }
  ...
  if (pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf &&
      pm_entSelf
          ->m_pVehicle) { // Now update our mins/maxs to match our
                          // m_vOrientation based on our length, width & height
    BG_VehicleAdjustBBoxForOrientation(pm_entSelf->m_pVehicle, pm->ps->origin,
                                       pm->mins, pm->maxs, pm->ps->clientNum,
                                       pm->tracemask, pm->trace);
  }
  ...
  if (pm->ps->pm_type == PM_DEAD) {
    if (pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf &&
        ...
        pm_entSelf->m_pVehicle->m_pVehicleInfo->type !=
            VH_ANIMAL) { // vehicles don't use deadmove
    } else {
      PM_DeadMove();
    }
  }
  ...
  if (pm_entSelf->s.NPC_class != CLASS_VEHICLE &&
    ...
    if (veh && veh->m_pVehicle &&
        (veh->m_pVehicle->m_pVehicleInfo->type == VH_WALKER ||
         veh->m_pVehicle->m_pVehicleInfo->type ==
             VH_FIGHTER)) { //*sigh*, until we get forced weapon-switching
                            //working?
      pm->cmd.buttons &= ~(BUTTON_ATTACK | BUTTON_ALT_ATTACK);
      pm->ps->eFlags &= ~(EF_FIRING | EF_ALT_FIRING);
  ...
  if (!pm->ps->m_iVehicleNum && pm_entSelf->s.NPC_class != CLASS_VEHICLE &&
      pm_entSelf->s.NPC_class != CLASS_RANCOR &&
      pm->ps->groundEntityNum < ENTITYNUM_WORLD &&
      pm->ps->groundEntityNum >=
          MAX_CLIENTS) { // I am a player client, not riding on a vehicle, and
                         // potentially standing on an NPC
    ...
    { // this is actually an NPC, let's try to bounce of its head to make sure
      // we can't just stand around on top of it.
      if (pm->ps->velocity[2] <
          270) { // try forcing velocity up and also force him to jump
             ...
             && pEnt->m_pVehicle)            // ent has a vehicle
    {
      ...
      { // it's a vehicle, see if we should get in it
        ...
        pEnt->m_pVehicle->m_pVehicleInfo->Board(pEnt->m_pVehicle,
  ...
  if (pm->ps->clientNum >= MAX_CLIENTS && pm_entSelf &&
    ...
    assert(veh && veh->playerState && veh->m_pVehicle &&
           veh->s.number >= MAX_CLIENTS);

    if (veh->m_pVehicle->m_pVehicleInfo->type !=
        VH_FIGHTER) { // kind of hacky, don't want to do this for flying
                      // vehicles
    ...
    if (!pm->ps->m_iVehicleNum) { // no one is driving, just update and get out
      ...
      if (self->playerState
              ->m_iVehicleNum) { // only do it if they still have a vehicle
                                 // (didn't get ejected this update or something)
        PM_VehicleViewAngles(self->playerState, veh, &veh->m_pVehicle->m_ucmd);
      }

#ifdef _GAME
      veh->m_pVehicle->m_pVehicleInfo->Update(veh->m_pVehicle,
                                              &veh->m_pVehicle->m_ucmd);
      veh->m_pVehicle->m_pVehicleInfo->Animate(veh->m_pVehicle);

      veh->m_pVehicle->m_pVehicleInfo->UpdateRider(veh->m_pVehicle, self,
                                                   &veh->m_pVehicle->m_ucmd);
      // update the passengers
      while (i < veh->m_pVehicle->m_iNumPassengers) {
        if (veh->m_pVehicle->m_ppPassengers[i]) {
          gentity_t *thePassenger =
              (gentity_t *)veh->m_pVehicle
                  ->m_ppPassengers[i]; // yes, this is, in fact, ass.
          if (thePassenger->inuse && thePassenger->client) {
            veh->m_pVehicle->m_pVehicleInfo->UpdateRider(
                veh->m_pVehicle, veh->m_pVehicle->m_ppPassengers[i],
...
#else
      if (!veh->playerState
               ->vehBoarding) //|| veh->m_pVehicle->m_pVehicleInfo->type ==
                              //VH_FIGHTER)
      {
        if (veh->m_pVehicle->m_pVehicleInfo->type ==
            VH_FIGHTER) { // client must explicitly call this for prediction
          BG_FighterUpdate(veh->m_pVehicle, &veh->m_pVehicle->m_ucmd, pm->mins,
                           pm->maxs, self->playerState->gravity, pm->trace);
        }
        ...
        if (veh->m_pVehicle->m_iBoarding == 0) {
          ...
          VectorCopy(veh->m_pVehicle->m_vOrientation,
                     veh->m_pVehicle->m_vPrevOrientation);

          veh->m_pVehicle->m_pVehicleInfo->ProcessOrientCommands(
              veh->m_pVehicle);
          PM_SetPMViewAngle(veh->playerState, veh->m_pVehicle->m_vOrientation,
                            &veh->m_pVehicle->m_ucmd);
          ...
          if (veh->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER) {
            AngleVectors(veh->m_pVehicle->m_vOrientation,
                         veh->playerState->moveDir, NULL, NULL);
          } else {
            vec3_t vVehAngles;
      ...
      else
      {
              ...
              PM_SetPMViewAngle(self->playerState, veh->playerState->viewangles,
      &veh->m_pVehicle->m_ucmd);
      }
      */
        ...
        veh->playerState->speed = 0.0f;
        if (veh->m_pVehicle) {
          PM_SetPMViewAngle(self->playerState, veh->m_pVehicle->m_vOrientation,
                            &pm->cmd);
          PM_SetPMViewAngle(veh->playerState, veh->m_pVehicle->m_vOrientation,
                            &pm->cmd);
        }
  ...
  if (pm_entSelf->s.NPC_class != CLASS_VEHICLE &&
      pm->ps->m_iVehicleNum) { // don't even run physics on a player if he's on
                               // a vehicle - he goes where the vehicle goes
  } else { // don't even run physics on a player if he's on a vehicle - he goes
  ...
  if (pm->cmd.invensel != (byte)-1 &&
      (pm->ps->stats[STAT_HOLDABLE_ITEMS] & (1 << pm->cmd.invensel))) {
    pm->ps->stats[STAT_HOLDABLE_ITEM] =
        BG_GetItemIndexByTag(pm->cmd.invensel, IT_HOLDABLE);
  }
  ...
  if (pm->ps->m_iVehicleNum
    ...
    if ((pm->ps->eFlags & EF_NODRAW)) {     // inside the vehicle, do nothing
               ...
               !PM_WeaponOkOnVehicle(
                   pm->ps->weapon)) { // this weapon is not legal for the
                                      // vehicle, force to our current one
  ...
  if (!pm->ps->m_iVehicleNum // not a vehicle and not riding one
      ...
      (!(pm->ps->eFlags & EF_NODRAW) &&
       PM_WeaponOkOnVehicle(
           pm->cmd
               .weapon))) // you're not inside the vehicle and the weapon you're
                          // holding can be used when riding this vehicle
  ...
  if (!pm->ps->m_iVehicleNum &&
      (pm->ps->clientNum < MAX_CLIENTS || !pm_entSelf ||
       pm_entSelf->s.NPC_class != CLASS_VEHICLE)) { // don't do this if we're on
                                                    // a vehicle, or we are one
  ...
  if ( // pm->ps->m_iVehicleNum &&
    ...
    // passenger pmove too. Maybe instead of AttachRiders we should have each
    // rider call attach for himself?
  ...
  if (pm_entSelf->s.NPC_class != CLASS_VEHICLE &&
      pm->ps->m_iVehicleNum) { // riding a vehicle, see if we should do some
                               // anim overrides
