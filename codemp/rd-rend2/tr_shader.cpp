...
const int lightmaps2d[MAXLIGHTMAPS] = {LIGHTMAP_2D, LIGHTMAP_2D, LIGHTMAP_2D, LIGHTMAP_2D};

const int lightmapsVertex[MAXLIGHTMAPS] = {LIGHTMAP_BY_VERTEX, LIGHTMAP_BY_VERTEX, LIGHTMAP_BY_VERTEX, LIGHTMAP_BY_VERTEX};

const int lightmapsFullBright[MAXLIGHTMAPS] = {LIGHTMAP_WHITEIMAGE, LIGHTMAP_WHITEIMAGE, LIGHTMAP_WHITEIMAGE, LIGHTMAP_WHITEIMAGE};
...
static animMapType_t AnimMapType(const char *token) {
	if (!Q_stricmp(token, "clampanimMap")) {
		return ANIMMAP_CLAMP;
	} else if (!Q_stricmp(token, "oneshotanimMap")) {
		return ANIMMAP_ONESHOT;
	} else {
		return ANIMMAP_NORMAL;
	}
}

static const char *animMapNames[] = {"animMap", "clapanimMap", "oneshotanimMap"};
...
static bool ParseSurfaceSprites(const char *buffer, shaderStage_t *stage) {
	...
	if (fadedist < 32.0f) {
		ri.Printf(PRINT_ALL, S_COLOR_YELLOW "WARNING: invalid fadedist (%.2f < 32) in shader '%s'\n", fadedist, shader.name);
		return false;
	}
...
// 		ssFXWeather
static bool ParseSurfaceSpritesOptional(const char *param, const char *buffer, shaderStage_t *stage) {
	const char *token;
	const char **text = &buffer;
	float value;
	...
	if (!Q_stricmp(param, "ssWind")) {
		...
		stage->ss->wind = value;
		if (stage->ss->windIdle <= 0) { // Also override the windidle, it usually is the same as wind
...
static qboolean ParseStage(shaderStage_t *stage, const char **text) {
	...
	while (1) {
		...
		//
			...
			stage->specularScale[0] = stage->specularScale[1] = stage->specularScale[2] = Com_Clamp(0.0f, 1.0f, atof(token));
		}
		//
	...
	if (stage->bundle[0].tcGen == TCGEN_LIGHTMAP && stage->bundle[0].isLightmap == qfalse) {
		...
		if (!Q_strncmp(filename, bufferBaseColorTextureName, sizeof(filename))) {
			if (shader.isHDRLit) {
				image_t *hdrImage = R_FindImageFile(bufferBaseColorTextureName, IMGTYPE_COLORALPHA,
													IMGFLAG_NOLIGHTSCALE | IMGFLAG_HDR_LIGHTMAP | IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE);
				if (hdrImage)
...
static qboolean ParseShader(const char **text) {
	...
	if (shaderHash == RETAIL_ROCKET_WEDGE_SHADER_HASH && Q_stricmp(shader.name, "gfx/2d/wedge") == 0) {
	...
	for (stage = 0; stage < MAX_SHADER_STAGES; stage++) {
		...
		for (i = 0; i < NUM_TEXTURE_BUNDLES; i++) {
			...
			case TCGEN_LIGHTMAP3:
				shader.vertexAttribs |= (ATTR_TEXCOORD1 | ATTR_TEXCOORD2 | ATTR_TEXCOORD3 | ATTR_TEXCOORD4);
				break;
...
static void CollapseStagesToLightall(shaderStage_t *stage, shaderStage_t *lightmap, qboolean useLightVector, qboolean useLightVertex, qboolean tcgen) {
	...
	if (r_normalMapping->integer) {
		image_t *diffuseImg;
		if (stage->bundle[TB_NORMALMAP].image[0]) {
			if (stage->bundle[TB_NORMALMAP].image[0]->type == IMGTYPE_NORMALHEIGHT && r_parallaxMapping->integer && defs & LIGHTDEF_LIGHTTYPE_MASK)
			...
			if (specularImg) {
				...
				if (!stage->bundle[TB_ORMSMAP].image[0]) {
					...
					if (!stage->bundle[TB_ORMSMAP].image[0]) {
						...
						stage->specularScale[2] = stage->specularScale[3] = 1.0f;
...
static qboolean CollapseStagesToGLSL(void) {
	...
	if (!skip) {
		...
		for (i = 0; i < MAX_SHADER_STAGES; i++) {
			...
			for (j = i + 1; j < MAX_SHADER_STAGES; j++) {
				...
				if (pStage2->bundle[0].tcGen >= TCGEN_LIGHTMAP && pStage2->bundle[0].tcGen <= TCGEN_LIGHTMAP3 && pStage2->rgbGen != CGEN_EXACT_VERTEX) {
			...
			if (diffuse->bundle[0].tcGen == TCGEN_ENVIRONMENT_MAPPED ||
				(diffuse->bundle[0].tcGen >= TCGEN_LIGHTMAP && diffuse->bundle[0].tcGen <= TCGEN_LIGHTMAP3) || diffuse->bundle[0].tcGen == TCGEN_VECTOR) {
				tcgen = qtrue;
			}

			diffuselit = qfalse;
			if (diffuse->rgbGen == CGEN_LIGHTING_DIFFUSE || diffuse->rgbGen == CGEN_LIGHTING_DIFFUSE_ENTITY) {
				diffuselit = qtrue;
			}

			vertexlit = qfalse;
			if (diffuse->rgbGen == CGEN_VERTEX_LIT || diffuse->rgbGen == CGEN_EXACT_VERTEX_LIT || diffuse->rgbGen == CGEN_VERTEX ||
				diffuse->rgbGen == CGEN_EXACT_VERTEX) {
				vertexlit = qtrue;
			}
			...
			if (lightmap != NULL) {
				for (j = i + 2; j < MAX_SHADER_STAGES; j++) {
					...
					if (blendBits != (GLS_DSTBLEND_ONE | GLS_SRCBLEND_ONE) ||
						(pStage2->bundle[0].tcGen < TCGEN_LIGHTMAP && pStage2->bundle[0].tcGen > TCGEN_LIGHTMAP3) || pStage2->rgbGen != CGEN_LIGHTMAPSTYLE)
						break;
		...
		for (i = 0; i < MAX_SHADER_STAGES; i++) {
			...
			if (pStage->bundle[0].tcGen >= TCGEN_LIGHTMAP && pStage->bundle[0].tcGen <= TCGEN_LIGHTMAP3 && lightmaps[i] == NULL) {
	...
	if (skip) {
		...
		for (i = 0; i < MAX_SHADER_STAGES; i++) {
			...
			pStage->specularScale[2] = pStage->specularScale[3] = 1.0f;
...
static shader_t *FinishShader(void) {
	...
	if (stages[0].active && stages[0].bundle[0].isLightmap && stages[1].active && shader.numDeforms != 1) {
	...
	if (shader.sort == SS_OPAQUE && shader.numDeforms == 0) {
		for (stage = 0; stage < MAX_SHADER_STAGES; stage++) {
			shaderStage_t *pStage = &stages[stage];
