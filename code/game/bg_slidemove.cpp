...
qboolean PM_SlideMove(float gravMod) {
	...
	if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) { // a non-player client, not an NPC under player control
	...
	for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
		...
		if ((trace.contents & CONTENTS_BOTCLIP) && (slideMoveContents & CONTENTS_BOTCLIP)) { // hit a do not enter brush
			...
			{ // crap, we're in a do not enter brush, take it out for the remainder of the traces and re-trace this one right now without it
				slideMoveContents &= ~CONTENTS_BOTCLIP;
				pm->trace(&trace, pm->ps->origin, pm->mins, pm->maxs, end, pm->ps->clientNum, slideMoveContents, (EG2_Collision)0, 0);
			} else if (trace.plane.normal[2] > 0.0f) { // on top of a do not enter brush, it, just redo this one trace without it
		...
		if (pm->gent->client && pm->gent->client->NPC_class == CLASS_VEHICLE && trace.plane.normal[2] < pm->gent->m_pVehicle->m_pVehicleInfo->maxSlope) {
...
void PM_StepSlideMove(float gravMod) {
	...
	if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_VEHICLE &&
		pm->gent->m_pVehicle->m_pVehicleInfo->hoverHeight > 0) { // Hovering vehicles don't do steps
	...
	if (fabs(stepUpMove[0]) < 0.1 && fabs(stepUpMove[1]) < 0.1 && VectorLengthSquared(slideMove) > VectorLengthSquared(stepUpMove)) {
		...
		if (g_stepSlideFix->integer) {
			if (pm->ps->clientNum < MAX_CLIENTS &&
				trace.plane.normal[2] < MIN_WALK_NORMAL) { // normal players cannot step up slopes that are too steep to walk on!
				vec3_t stepVec;
		...
		{
			if (pm->ps->clientNum && isGiant && g_entities[trace.entityNum].client && pm->gent && pm->gent->client &&
				pm->gent->client->NPC_class == CLASS_RANCOR) { // Rancor don't step on clients
				if (g_stepSlideFix->integer) {
					VectorCopy(down_o, pm->ps->origin);
					VectorCopy(down_v, pm->ps->velocity);
				} else {
			...
			} else if (pm->ps->clientNum && isGiant && g_entities[trace.entityNum].client &&
