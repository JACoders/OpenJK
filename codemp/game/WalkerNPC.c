...
static void ProcessMoveCommands(Vehicle_t *pVeh) {
  ...
  if (pVeh->m_ucmd.buttons & BUTTON_WALKING &&
  ...
  if (parentPS->stats[STAT_HEALTH] <=
      0) { // don't keep moving while you're dying!
    parentPS->speed = 0;
  }
...
void WalkerYawAdjust(Vehicle_t *pVeh, playerState_t *riderPS,
  ...
  if (parentPS && parentPS->speed) {
    ...
    float maxDif =
        pVeh->m_pVehicleInfo->turningSpeed * 1.5f; // magic number hackery
...
/*
void WalkerPitchAdjust(Vehicle_t *pVeh, playerState_t *riderPS, playerState_t
*parentPS)
{
        float angDif = AngleSubtract(pVeh->m_vOrientation[PITCH],
riderPS->viewangles[PITCH]);

        if (parentPS && parentPS->speed)
        {
                float s = parentPS->speed;
                float maxDif = pVeh->m_pVehicleInfo->turningSpeed*0.8f; //magic
number hackery
...
AngleNormalize360(pVeh->m_vOrientation[PITCH] -
angDif*(pVeh->m_fTimeModifier*0.2f));
        }
}
*/
...
static void ProcessOrientCommands(Vehicle_t *pVeh) {
  ...
  if (rider->s.number <
      MAX_CLIENTS) { // FIXME: use the vehicle's turning stat in this calc
    ...
    if (pVeh->m_pVehicleInfo->malfunctionArmorLevel &&
        pVeh->m_iArmor <=
            pVeh->m_pVehicleInfo->malfunctionArmorLevel) { // damaged badly
    }
  }
...
static void AnimateVehicle(Vehicle_t *pVeh) {
  ...
  if (fSpeedPercToMax > 0.0f) // fSpeedPercToMax >= 0.85f )
  {
    ...
    // NOTE: Mikes suggestion for fixing the stuttering walk (left/right) is to
    // maintain the current frame between animations. I have no clue how to do
    // this and have to work on other stuff so good luck to him :-p AReis
...
void G_SetWalkerVehicleFunctions(vehicleInfo_t *pVehInfo) {
  ...
  //	pVehInfo->AnimateRiders				=
  //AnimateRiders; 	pVehInfo->ValidateBoard				=
...
//	pVehInfo->Initialize				=
//Initialize; 	pVehInfo->Update					=
//Update; 	pVehInfo->UpdateRider				=
//UpdateRider;
#endif //_GAME
