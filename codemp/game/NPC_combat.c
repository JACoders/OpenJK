	...
	if (self->enemy == NULL) {
		...
		if (self->s.weapon == WP_BLASTER || self->s.weapon == WP_REPEATER ||
			...
			if (self->client->playerTeam == NPCTEAM_PLAYER) {
				...
				G_AimSet(self,
						 Q_irand(self->NPC->stats.aim - (maxErr * (3 - g_npcspskill.integer)), self->NPC->stats.aim - (minErr * (3 - g_npcspskill.integer))));
			}
		}
...
*/
void NPC_ApplyWeaponFireDelay(void) {
	if (NPCS.NPC->attackDebounceTime > level.time) { // Just fired, if attacking again, must be a burst fire, so don't add delay
...
qboolean HaveWeapon(int weapon) { return (NPCS.client->ps.stats[STAT_WEAPONS] & (1 << weapon)); }
...
float NPC_MaxDistSquaredForWeapon(void) {
	...
	case WP_DISRUPTOR: // disruptor
		...
		*/
	case WP_SABER:
		if (NPCS.NPC->client && NPCS.NPC->client->saber[0].blade[0].lengthMax) { // FIXME: account for whether enemy and I are heading towards each other!
			return (NPCS.NPC->client->saber[0].blade[0].lengthMax + NPCS.NPC->r.maxs[0] * 1.5) *
				   (NPCS.NPC->client->saber[0].blade[0].lengthMax + NPCS.NPC->r.maxs[0] * 1.5);
		} else {
			return 48 * 48;
		}
		break;
	...
	if (!(ent->flags & FL_NOTARGET)) {
		if (ent->health > 0) {
			if (!ent->client) {
				...
				if (entTeam == NPCTEAM_FREE || NPCS.NPC->client->enemyTeam == NPCTEAM_FREE || entTeam == NPCS.NPC->client->enemyTeam) {
...
gentity_t *NPC_PickEnemy(gentity_t *closestTo, int enemyTeam, qboolean checkVis, qboolean findPlayersFirst, qboolean findClosest) {
	...
	if (NPCS.NPCInfo->behaviorState == BS_STAND_AND_SHOOT ||
		NPCS.NPCInfo->behaviorState == BS_HUNT_AND_KILL) { // Formations guys don't require inFov to pick up a target
		...
		if (newenemy->client && !(newenemy->flags & FL_NOTARGET) && !(newenemy->s.eFlags & EF_NODRAW)) {
			if (newenemy->health > 0) {
				...
				{							  // FIXME:  check for range and FOV or vis?
					if (newenemy != NPCS.NPC->lastEnemy) { // Make sure we're not just going back and forth here
								...
								if (newenemy->client->hiddenDist > 0) {
									if (relDist > newenemy->client->hiddenDist * newenemy->client->hiddenDist) {
										...
										if (VectorLengthSquared(newenemy->client->hiddenDir)) { // They're only hidden from a certain direction, check
											...
											if (dot > 0.5) { // I'm not looking in the right dir toward them to see them
												failed = qtrue;
											} else {
	...
	*/
	...
	for (entNum = 0; entNum < level.num_entities; entNum++) {
		...
		if (newenemy != NPCS.NPC && (newenemy->client /*|| newenemy->svFlags & SVF_NONNPC_ENEMY*/) && !(newenemy->flags & FL_NOTARGET) &&
			...
			if (newenemy->health > 0) {
				if ((newenemy->client && NPC_ValidEnemy(newenemy)) ||
					...
					if (NPCS.NPC->client->playerTeam == NPCTEAM_PLAYER && enemyTeam == NPCTEAM_PLAYER) { // player allies turning on ourselves?  How?
		...
		if (ally->client) {
			if (ally->health > 0) {
				...
				{																	 // if on same team or if player is disguised as your team
					...
					if (movingOnly && ally->client && NPCS.NPC->client) { // They have to be moving relative to each other
...
gentity_t *NPC_CheckEnemy(qboolean findNew, qboolean tooFarOk, qboolean setEnemy) {
	...
	*/
	...
	if (NPCS.NPC->enemy) {
		if (NPC_EnemyTooFar(NPCS.NPC->enemy, 0, qfalse)) {
		...
		} else if (!trap->InPVS(NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin)) { // FIXME: should this be a line-of site check?
			...
			if (NPCS.NPC->enemy->client && NPCS.NPC->enemy->client->hiddenDist) { // He ducked into shadow while we weren't looking
	...
	if (NPCS.NPCInfo->defendEnt) {										 // Trying to protect someone
		if (NPCS.NPCInfo->defendEnt->health > 0) {						 // Still alive, We presume we're close to them, navigation should handle this?
			if (NPCS.NPCInfo->defendEnt->enemy) {						 // They were shot or acquired an enemy
				if (NPCS.NPC->enemy != NPCS.NPCInfo->defendEnt->enemy) { // They have a different enemy, take it!
					newEnemy = NPCS.NPCInfo->defendEnt->enemy;
					if (setEnemy) {
						G_SetEnemy(NPCS.NPC, NPCS.NPCInfo->defendEnt->enemy);
					}
				}
			} else if (NPCS.NPC->enemy == NULL) { // We don't have an enemy, so find closest to defendEnt
	...
	if (!NPCS.NPC->enemy || (NPCS.NPC->enemy && NPCS.NPC->enemy->health <= 0) ||
		forcefindNew) { // FIXME: NPCs that are moving after an enemy should ignore the can't hit enemy counter- that should only be for NPCs that are standing
						// still
...
int NPC_ShotEntity(gentity_t *ent, vec3_t impactPos) {
	...
	if (NPCS.NPC->s.weapon == WP_THERMAL) { // thermal aims from slightly above head
...
qboolean NPC_CheckCanAttack(float attack_scale, qboolean stationary) {
	...
	if (NPC_EnemyTooFar(NPCS.NPC->enemy, distanceToEnemy * distanceToEnemy, qtrue)) { // Too far away?  Do not attack
		return qfalse;
	}
	...
	if (NPCS.enemyVisibility >= VIS_FOV) { // He's in our FOV
		...
		if (NPCS.NPC->enemy->client) {
			if (NPCS.NPC->enemy->enemy == NPCS.NPC) {
				if (NPCS.NPC->enemy->client->buttons & BUTTON_ATTACK) { // FIXME: determine if enemy fire angles would hit me or get close
		...
		if (attack_ok) {
			...
			if (!dead_on) { // We're not going to hit him directly, try a suppressing fire
				...
				if (traceEnt && (traceEnt->health <= 30 || EntIsGlass(traceEnt))) { // easy to kill - go for it
					...
					{	   // going to explode, don't shoot if close to self
						...
						if (VectorLengthSquared(diff) < traceEnt->splashRadius * traceEnt->splashRadius) { // Too close to shoot!
							attack_ok = qfalse;
...
int NPC_FindCombatPoint(const vec3_t position, const vec3_t avoidPosition, vec3_t enemyPosition, const int flags, const float avoidDist,
	...
	for (j = 0; j < numPoints; j++) {
		...
		trap->Trace(&tr, level.combatPoints[i].origin, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, level.combatPoints[i].origin, NPCS.NPC->s.number, NPCS.NPC->clipmask,
					qfalse, 0, 0);
		...
		if (flags & CP_HAS_ROUTE) {
			...
			*/
			...
			if (waypoint == WAYPOINT_NONE || level.combatPoints[i].waypoint == WAYPOINT_NONE ||
				...
				if (!NAV_ClearPathToPoint(NPCS.NPC, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, level.combatPoints[i].origin, NPCS.NPC->clipmask,
				...
				if (dist < bestDist) {
					if (!trap->Nav_GetBestPathBetweenEnts((sharedEntity_t *)NPCS.NPC, (sharedEntity_t *)found, NF_CLEAR_PATH) ||
						trap->Nav_GetBestNodeAltRoute2(NPCS.NPC->waypoint, found->waypoint, NODE_NONE) ==
							WAYPOINT_NONE) { // can't possibly have a route to any OR can't possibly have a route to this one OR don't have a route to this one
						if (NAV_ClearPathToPoint(NPCS.NPC, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, found->r.currentOrigin, NPCS.NPC->clipmask,
...
void NPC_CheckGetNewWeapon(void) {
	if (NPCS.NPC->s.weapon == WP_NONE && NPCS.NPC->enemy) { // if running away because dropped weapon...
		if (NPCS.NPCInfo->goalEntity && NPCS.NPCInfo->goalEntity == NPCS.NPCInfo->tempGoal && NPCS.NPCInfo->goalEntity->enemy &&
			!NPCS.NPCInfo->goalEntity->enemy->inuse) { // maybe was running at a weapon that was picked up
			NPCS.NPCInfo->goalEntity = NULL;
		}
		if (TIMER_Done(NPCS.NPC, "panic") && NPCS.NPCInfo->goalEntity == NULL) { // need a weapon, any lying around?
...
void NPC_AimAdjust(int change) {
	...
	if (TIMER_Done(NPCS.NPC, "aimDebounce")) {
		...
		if (NPCS.NPCInfo->currentAim > NPCS.NPCInfo->stats.aim) { // can never be better than max aim
