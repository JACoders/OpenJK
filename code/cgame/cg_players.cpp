...
extern qboolean WP_SaberBladeUseSecondBladeStyle(saberInfo_t *saber,
                                                 int bladeNum);
extern void WP_SaberSwingSound(gentity_t *ent, int saberNum,
                               swingType_t swingType);
...
void CG_AddGhoul2Mark(int type, float size, vec3_t hitloc, vec3_t hitdirection,
                      ...
                      CGhoul2Info_v &ghoul2, vec3_t modelScale, int lifeTime,
                      int firstModel, vec3_t uaxis) {
  ...
  goreSkin.fadeRGB = true; //fade on RGB and alpha instead of just alpha (not
  needed for all shaders, but whatever)
  */
  ...
  if (uaxis) {
    ...
    goreSkin.TSize = 3;
    goreSkin.depthStart =
        -10; // arbitrary depths, just limiting marks to near hit loc
...
static void CG_PlayerFootsteps(centity_t *const cent,
                               footstepType_t footStepType);
static void CG_PlayerAnimEvents(int animFileIndex, qboolean torso, int oldFrame,
                                int frame, int entNum);
extern void BG_G2SetBoneAngles(centity_t *cent, gentity_t *gent, int boneIndex,
                               const vec3_t angles, const int flags,
                               const Eorientations up, const Eorientations left,
                               const Eorientations forward,
                               qhandle_t *modelList);
extern void FX_BorgDeathSparkParticles(vec3_t origin, vec3_t angles, vec3_t vel,
                                       vec3_t user);
...
const char *cg_customBasicSoundNames[MAX_CUSTOM_BASIC_SOUNDS] = {
    ...
    "*pain25.wav", "*pain50.wav",   "*pain75.wav", "*pain100.wav",
    "*gurp1.wav",  "*gurp2.wav",    "*drown.wav",  "*gasp.wav",
    "*land1.wav",  "*falling1.wav",
};
...
const char *cg_customCombatSoundNames[MAX_CUSTOM_COMBAT_SOUNDS] = {
    ...
    "*pushed2.wav",  "*pushed3.wav",  "*choke1.wav", "*choke2.wav",
    "*choke3.wav",   "*ffwarn.wav",   "*ffturn.wav",
};

// Used as a supplement to the basic set for stormtroopers
// (keep numbers in ascending order in order for variant-capping to work)
const char *cg_customExtraSoundNames[MAX_CUSTOM_EXTRA_SOUNDS] = {
    "*chase1.wav",      "*chase2.wav",      "*chase3.wav",
    "*cover1.wav",      "*cover2.wav",      "*cover3.wav",
    "*cover4.wav",      "*cover5.wav",      "*detected1.wav",
    "*detected2.wav",   "*detected3.wav",   "*detected4.wav",
    "*detected5.wav",   "*lost1.wav",       "*outflank1.wav",
    "*outflank2.wav",   "*escaping1.wav",   "*escaping2.wav",
    "*escaping3.wav",   "*giveup1.wav",     "*giveup2.wav",
    "*giveup3.wav",     "*giveup4.wav",     "*look1.wav",
    "*look2.wav",       "*sight1.wav",      "*sight2.wav",
    "*sight3.wav",      "*sound1.wav",      "*sound2.wav",
    "*sound3.wav",      "*suspicious1.wav", "*suspicious2.wav",
    "*suspicious3.wav", "*suspicious4.wav", "*suspicious5.wav",
};

// Used as a supplement to the basic set for jedi
// (keep numbers in ascending order in order for variant-capping to work)
const char *cg_customJediSoundNames[MAX_CUSTOM_JEDI_SOUNDS] = {
    "*combat1.wav",    "*combat2.wav",    "*combat3.wav", "*jdetected1.wav",
    "*jdetected2.wav", "*jdetected3.wav", "*taunt1.wav",  "*taunt2.wav",
    "*taunt3.wav",     "*jchase1.wav",    "*jchase2.wav", "*jchase3.wav",
    "*jlost1.wav",     "*jlost2.wav",     "*jlost3.wav",  "*deflect1.wav",
    "*deflect2.wav",   "*deflect3.wav",   "*gloat1.wav",  "*gloat2.wav",
    "*gloat3.wav",     "*pushfail.wav",
};

// done at registration time only...
//
// cuts down on sound-variant registration for low end machines,
//		eg *gloat1.wav (plus...2,...3) can be capped to all be just
//*gloat1.wav
//
static const char *GetCustomSound_VariantCapped(const char *ppsTable[],
                                                int iEntryNum,
                                                qboolean bForceVariant1) {
  ...
  if (iVariantCap || bForceVariant1) {
    ...
    if (p && p - 2 > ppsTable[iEntryNum] && isdigit(p[-1]) && !isdigit(p[-2])) {
      ...
      if (iThisVariant > iVariantCap || bForceVariant1) {
        // ok, let's not load this variant, so pick a random one below the cap
        // value...
        //
        for (int i = 0; i < 2;
             i++) // 1st pass, choose random, 2nd pass (if random not in list),
                  // choose xxx1, else fall through...
        {
          ...
          if (p) {
            *p = '\0';
            ...
            int iRandom =
                bForceVariant1 ? 1 : (!i ? Q_irand(1, iVariantCap) : 1);
            ...
            for (int iScanNum = 0; iScanNum < iEntryNum; iScanNum++) {
              if (!Q_stricmp(ppsTable[iScanNum], sName)) {
                // yeah, this entry is also present in the table, so ok to
                // return it
                //
        ...
        // didn't find an entry corresponding to either the random name, or the
        // xxxx1 version,
...
static void CG_RegisterCustomSounds(clientInfo_t *ci, int iSoundEntryBase,
                                    int iTableEntries, const char *ppsTable[],
                                    const char *psDir) {
  for (int i = 0; i < iTableEntries; i++) {
    ...
    if (g_sex->string[0] == 'f') {
      hSFX =
          cgi_S_RegisterSound(va("sound/chars/%s/misc/%s_f.wav", psDir, s + 1));
    }
    if (hSFX == 0 || com_buildScript->integer) {
      hSFX =
          cgi_S_RegisterSound(va("sound/chars/%s/misc/%s.wav", psDir, s + 1));
    }
    if (hSFX == 0) {
      // hmmm... variant in table was missing, so forcibly-retry with %1 version
      // (which we may have just tried, but wtf?)...
      //
      pS = GetCustomSound_VariantCapped(ppsTable, i, qtrue);
      COM_StripExtension(pS, s, sizeof(s));
      if (g_sex->string[0] == 'f') {
        hSFX = cgi_S_RegisterSound(
            va("sound/chars/%s/misc/%s_f.wav", psDir, s + 1));
      }
      if (hSFX == 0 || com_buildScript->integer) {
        hSFX =
            cgi_S_RegisterSound(va("sound/chars/%s/misc/%s.wav", psDir, s + 1));
...
CG_CustomSound
  ...
  NOTE: when you call this, check the value.  If zero, do not try to play the
sound. Either that or when a sound that doesn't exist is played, don't play the
null sound honk and don't display the error message
...
//		CG_Printf( "custom sound not on client: %s", soundName );
  ...
  // FIXME: if the sound you want to play could not be found, pick another from
  // the same general grouping?  ie: if you want ff_2c and there is none, try
  // ff_2b or ff_2a...
  switch (customSoundSet) {
  case CS_BASIC:
    // There should always be a clientInfo structure if there is a client, but
    // just make sure...
    if (ci) {
      for (i = 0; i < MAX_CUSTOM_BASIC_SOUNDS && cg_customBasicSoundNames[i];
           i++) {
  ...
  case CS_COMBAT:
    ...
    // just make sure...
    if (ci) {
      for (i = 0; i < MAX_CUSTOM_COMBAT_SOUNDS && cg_customCombatSoundNames[i];
           i++) {
  ...
  case CS_EXTRA:
    ...
    // just make sure...
    if (ci) {
      for (i = 0; i < MAX_CUSTOM_EXTRA_SOUNDS && cg_customExtraSoundNames[i];
           i++) {
        if (!Q_stricmp(soundName, cg_customExtraSoundNames[i])) {
          return ci
              ->sounds[i + MAX_CUSTOM_BASIC_SOUNDS + MAX_CUSTOM_COMBAT_SOUNDS];
        }
      }
    }
    break;
  case CS_JEDI:
    ...
    // just make sure...
    if (ci) {
      for (i = 0; i < MAX_CUSTOM_JEDI_SOUNDS && cg_customJediSoundNames[i];
           i++) {
        if (!Q_stricmp(soundName, cg_customJediSoundNames[i])) {
          return ci->sounds[i + MAX_CUSTOM_BASIC_SOUNDS +
                            MAX_CUSTOM_COMBAT_SOUNDS + MAX_CUSTOM_EXTRA_SOUNDS];
        }
      }
    }
    break;
...
qboolean CG_TryPlayCustomSound(vec3_t origin, int entityNum,
...
void CG_NewClientinfo(int clientNum) {
  ...
  // player uses only the basic custom and combat sound sets, not the extra or
  // jedi
...
void CG_RegisterNPCCustomSounds(clientInfo_t *ci) {
  ...
  if (ci->customExtraSoundDir && ci->customExtraSoundDir[0]) {
    CG_RegisterCustomSounds(
        ...
        MAX_CUSTOM_BASIC_SOUNDS +
            MAX_CUSTOM_COMBAT_SOUNDS, // int iSoundEntryBase,
  ...
  if (ci->customJediSoundDir && ci->customJediSoundDir[0]) {
    CG_RegisterCustomSounds(ci,
                            MAX_CUSTOM_BASIC_SOUNDS + MAX_CUSTOM_COMBAT_SOUNDS +
                                MAX_CUSTOM_EXTRA_SOUNDS, // int iSoundEntryBase,
...
void CG_RegisterNPCEffects( team_t team )
  ...
  This should register all the shaders, models and sounds used by a specific
type of NPC's spawn, death and other miscellaneous effects.  NOT WEAPON EFFECTS,
as those are taken care of in CG_RegisterWeapon
*/
/*
void CG_RegisterNPCEffects( team_t team )
{
...
===============
*/
static qboolean CG_RunLerpFrame(clientInfo_t *ci, lerpFrame_t *lf,
                                int newAnimation, float fpsMod, int entNum) {
  int f, animFrameTime;
  animation_t *anim;
  ...
  // FIXME: allow multiple-frame overlapped lerping between sequences? -
  // Possibly last 3 of last seq and first 3 of next seq?
  ...
  if (cg.time >= lf->frameTime) {
    ...
    /*if(fpsMod != 1.0)
    {//Note!  despite it's name, a higher fpsMod slows down the anim, a lower
    one speeds it up animFrameTime = ceil(lf->frameTime * fpsMod);
    }
    else*/
    {
...
static void CG_PlayerAnimation(centity_t *cent, int *legsOld, int *legs,
  ...
  // Changed this from cent->currentState.legsAnim to
  // cent->gent->client->ps.legsAnim because it was screwing up our timers when
  // we've just changed anims while turning
  ...
  if (legsTurnAnim != -1) {
    newLegsFrame = CG_RunLerpFrame(ci, &cent->pe.legs, legsTurnAnim,
                                   cent->gent->client->renderInfo.legsFpsMod,
                                   cent->gent->s.number);
    ...
    newLegsFrame = CG_RunLerpFrame(ci, &cent->pe.legs, legsAnim,
                                   cent->gent->client->renderInfo.legsFpsMod,
                                   cent->gent->s.number);
  }
  ...
  *legsBackLerp = cent->pe.legs.backlerp;
  ...
  if (newLegsFrame) {
    if (ValidAnimFileIndex(ci->animFileIndex)) {
      CG_PlayerAnimEvents(ci->animFileIndex, qfalse, cent->pe.legs.frame,
                          cent->pe.legs.frame, cent->currentState.number);
    }
  }

  newTorsoFrame = CG_RunLerpFrame(
      ci, &cent->pe.torso, cent->gent->client->ps.torsoAnim,
  ...
  *torsoBackLerp = cent->pe.torso.backlerp;
  ...
  if (newTorsoFrame) {
    if (ValidAnimFileIndex(ci->animFileIndex)) {
      CG_PlayerAnimEvents(ci->animFileIndex, qtrue, cent->pe.torso.frame,
                          cent->pe.torso.frame, cent->currentState.number);
    }
  }
}
...
static void CG_PlayerAnimEventDo(centity_t *cent, animevent_t *animEvent) {
  ...
  case AEV_SOUND:
    // are there variations on the sound?
    {
      const int holdSnd =
          animEvent->eventData
              [AED_SOUNDINDEX_START +
               Q_irand(0, animEvent->eventData[AED_SOUND_NUMRANDOMSNDS])];
      if (holdSnd > 0) {
        if (cgs.sound_precache[holdSnd]) {
          cgi_S_StartSound(NULL, cent->currentState.clientNum, channel,
                           cgs.sound_precache[holdSnd]);
        } else { // try a custom sound
          const char *s = CG_ConfigString(CS_SOUNDS + holdSnd);
          CG_TryPlayCustomSound(NULL, cent->currentState.clientNum, channel,
                                va("%s.wav", s), CS_TRY_ALL);
        }
      }
    }
    break;
  case AEV_SABER_SWING:
    if (cent->gent) { // cheat over to game side and play sound from there...
      WP_SaberSwingSound(
          cent->gent, animEvent->eventData[AED_SABER_SWING_SABERNUM],
  ...
  case AEV_SABER_SPIN:
    if (cent->gent && cent->gent->client) {
      saberInfo_t *saber =
          &cent->gent->client->ps
               .saber[animEvent->eventData[AED_SABER_SPIN_SABERNUM]];
      if (saber) {
        int spinSound = 0;
        if (saber->spinSound &&
            cgs.sound_precache[saber->spinSound]) { // use override
          ...
          default: // random saberspin1-3
            spinSound = cgi_S_RegisterSound(
                va("sound/weapons/saber/saberspin%d.wav", Q_irand(1, 3)));
            break;
          }
        }
        if (spinSound) {
          cgi_S_StartSound(NULL, cent->currentState.clientNum, CHAN_AUTO,
                           spinSound);
        }
      }
    }
  ...
  case AEV_EFFECT:
    if (animEvent->eventData[AED_EFFECTINDEX] == -1) { // invalid effect
      if (animEvent->stringData != NULL &&
          animEvent->stringData[0]) { // some sort of hard-coded effect
        if (Q_stricmp("push_l", animEvent->stringData) == 0) {
          ...
          cent->gent->client->ps.powerups[PW_FORCE_PUSH] =
              cg.time +
              animEvent
                  ->eventData[AED_EFFECT_PROBABILITY]; // AED_EFFECT_PROBABILITY
                                                       // in this case is the
                                                       // number of ms for the
                                                       // effect to last
          ...
          if (modelIndex > 0) { // we have a cinematic model
            int boltIndex =
                gi.G2API_AddBolt(&cent->gent->ghoul2[modelIndex], "*flash");
            if (boltIndex > -1) { // cinematic model has a flash bolt
              CG_PlayEffectBolted(
                  "scepter/beam.efx", modelIndex, boltIndex,
                  cent->currentState.clientNum, cent->lerpOrigin,
                  animEvent->eventData[AED_EFFECT_PROBABILITY],
                  qtrue); // AED_EFFECT_PROBABILITY in this case is the number
                          // of ms for the effect to last
            }
          }
        }
        // FIXME: add more
      ...
      if (animEvent->stringData != NULL && cent && cent->gent &&
          cent->gent->ghoul2.size()) { // have a bolt name we want to use
        ...
        if ((Q_stricmpn("*blade", animEvent->stringData, 6) == 0 ||
          ...
          animEvent->eventData[AED_BOLTINDEX] =
              gi.G2API_AddBolt(&cent->gent->ghoul2[cent->gent->weaponModel[0]],
                               animEvent->stringData);
          if (animEvent->eventData[AED_BOLTINDEX] != -1) { // found it!
            ...
            animEvent->eventData[AED_BOLTINDEX] =
                gi.G2API_AddBolt(&cent->gent->ghoul2[cent->gent->playerModel],
                                 animEvent->stringData);
          }
        } else {
          animEvent->eventData[AED_BOLTINDEX] =
      ...
      if (animEvent->eventData[AED_BOLTINDEX] !=
          -1) { // have a bolt we want to play the effect on
        ...
        CG_PlayEffectID(animEvent->eventData[AED_EFFECTINDEX], cent->lerpOrigin,
                        up);
        // G_PlayEffect( animEvent->eventData[AED_EFFECTINDEX],
        // cent->lerpOrigin, up ); theFxScheduler.PlayEffect(
        // animEvent->eventData[AED_EFFECTINDEX], cent->lerpOrigin, qfalse );
      }
    }
    break;
  case AEV_FIRE:
  ...
  case AEV_MOVE:
    ...
    if (cent && cent->gent && cent->gent->client) {
      if (cent->gent->client->ps.groundEntityNum !=
          ENTITYNUM_NONE) { // on something
        vec3_t fwd, rt, up,
            angles = {0, cent->gent->client->ps.viewangles[YAW], 0};
        ...
        VectorMA(cent->gent->client->ps.velocity,
                 animEvent->eventData[AED_MOVE_RT], rt,
                 cent->gent->client->ps.velocity);
        VectorMA(cent->gent->client->ps.velocity,
                 animEvent->eventData[AED_MOVE_UP], up,
                 cent->gent->client->ps.velocity);
        ...
        if (animEvent->eventData[AED_MOVE_UP] > 0) { // a jump
          ...
          // FIXME: if have force jump, do this?  or specify sound in the event
          // data? cent->gent->client->ps.forceJumpZStart =
          // cent->gent->client->ps.origin[2];//so we don't take damage if we
          // land at same height G_SoundOnEnt( cent->gent, CHAN_BODY,
...
static void CG_PlayerAnimEvents(int animFileIndex, qboolean torso, int oldFrame,
  ...
  hstring myModel =
      g_entities[entNum].NPC_type; // apparently NPC_type is always the same as
                                   // the model name???
  ...
  for (i = 0; i < MAX_ANIM_EVENTS; ++i) {
    ...
    match = qfalse;
    if (animEvents[i].modelOnly == 0 ||
        animEvents[i].modelOnly == myModel.handle()) {
      ...
      {                   // given a range, see if keyFrame falls in that range
        if (inSameAnim) { // if changed anims altogether, sorry, the sound is
                          // lost
          if (abs(oldFrame - animEvents[i].keyFrame) <= 3 ||
              abs(frame - animEvents[i].keyFrame) <=
      ...
      if (match) {
        ...
        case AEV_FOOTSTEP:
          // Determine probability of playing sound
          // Com_Printf( "Footstep event on frame %d, even should be on frame
...
static void CGG2_AnimEvents(centity_t *cent) {
  ...
  assert(cent->gent->playerModel >= 0 &&
         cent->gent->playerModel < cent->gent->ghoul2.size());
  if (ValidAnimFileIndex(cent->gent->client->clientInfo.animFileIndex)) {
    int junk, curFrame = 0;
    float currentFrame = 0, animSpeed;

    if (cent->gent->rootBone >= 0 &&
        gi.G2API_GetBoneAnimIndex(&cent->gent->ghoul2[cent->gent->playerModel],
                                  cent->gent->rootBone, cg.time, &currentFrame,
                                  &junk, &junk, &junk, &animSpeed,
                                  cgs.model_draw)) {
    ...
    if (cent->gent->lowerLumbarBone >= 0 &&
        gi.G2API_GetBoneAnimIndex(&cent->gent->ghoul2[cent->gent->playerModel],
                                  cent->gent->lowerLumbarBone, cg.time,
                                  &currentFrame, &junk, &junk, &junk,
...
==================
*/
static void CG_UpdateAngleClamp(float destAngle, float clampMin, float clampMax,
  ...
  if (swing >= 0) {
    ...
    if (move >=
        swing) { // our turnspeed is so fast, no need to swing, just match
      *curAngle = destAngle;
    } else {
...
CG_SwingAngles
  ...
  Parms:	desired angle,		(Our eventual goal angle
                        min swing tolerance,(Lower angle value threshold at
which to start turning) max swing tolerance,(Upper angle value threshold at
which to start turning) min clamp tolerance,(Lower angle value threshold to
clamp output angle to) max clamp tolerance,(Upper angle value threshold to clamp
output angle to) angle speed,		(How fast to turn) current angle,
(Current angle to modify) locked mode			(Don't turn unless you
exceed the swing/clamp tolerance)
==================
*/
static void CG_SwingAngles(float destAngle, float swingTolMin,
                           float swingTolMax, float clampMin, float clampMax,
                           float angleSpeed, float *curAngle,
                           qboolean *turning) {
  float swing;
  float move;
...
static void CG_BreathPuffs(centity_t *cent, vec3_t angles, vec3_t origin) {
  ...
  /*	cg_drawBreath.integer	== 0 - Don't draw at all.
                                                          == 1 - Draw both (but
     bubbles only when under water).
                                                          == 2 - Draw only cold
     breath.
                                                          == 3 - Draw only under
     water bubbles (when under water)	*/

  if (!client || cg_drawBreath.integer == 0 || !cg.renderingThirdPerson ||
      client->ps.pm_type == PM_DEAD || client->breathPuffTime > cg.time) {
    return;
  }

  // Get the head-front bolt/tag.
  int bolt = gi.G2API_AddBolt(&cent->gent->ghoul2[cent->gent->playerModel],
                              "*head_front");
  if (bolt == -1) {
    return;
  }
  ...
  gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel, bolt,
                         &boltMatrix, angles, origin, cg.time, cgs.model_draw,
  ...
  if (contents &
      (CONTENTS_SLIME |
       CONTENTS_LAVA)) // If they're submerged in something bad, leave.
  {
    return;
  }
  ...
  if ((contents & CONTENTS_WATER) &&
    ...
    CG_PlayEffectBolted("misc/waterbreath", cent->gent->playerModel, bolt,
                        cent->currentState.clientNum, vEffectOrigin);
  }
  // Draw cold breath effect.
  else if (cg_drawBreath.integer == 1 || cg_drawBreath.integer == 2) {
    CG_PlayEffectBolted("misc/breath", cent->gent->playerModel, bolt,
                        cent->currentState.clientNum, vEffectOrigin);
  }

  // TODO: It'd be nice if they breath faster when they're more damaged or when
  // running...
  if (gi.VoiceVolume[cent->currentState.number] >
      0) {                                  // make breath when talking
...
static qboolean CG_CheckLookTarget(centity_t *cent, vec3_t lookAngles,
                                   float *lookingSpeed) {
  if (!cent->gent->ghoul2.size()) {
    if (!cent->gent->client->clientInfo.torsoModel ||
        !cent->gent->client->clientInfo.headModel) {
      return qfalse;
    }
  }
  ...
  if (cent->gent->client->renderInfo.lookTarget >= 0 &&
      cent->gent->client->renderInfo.lookTarget < ENTITYNUM_WORLD) {
    vec3_t lookDir, lookOrg = {0.0f}, eyeOrg;
    if (cent->gent->client->renderInfo.lookMode == LM_ENT) {
      centity_t *lookCent =
          &cg_entities[cent->gent->client->renderInfo.lookTarget];
      if (lookCent && lookCent->gent) {
        if (lookCent->gent !=
            cent->gent->enemy) { // We turn heads faster than headbob speed, but
                                 // not as fast as if watching an enemy
          if (cent->gent->client->NPC_class ==
              CLASS_ROCKETTROOPER) { // they look around slowly and deliberately
        ...
        } else if (lookCent->gent->inuse &&
                   !VectorCompare(lookCent->gent->currentOrigin, vec3_origin)) {
    ...
    } else if (cent->gent->client->renderInfo.lookMode == LM_INTEREST &&
               cent->gent->client->renderInfo.lookTarget > -1 &&
               cent->gent->client->renderInfo.lookTarget <
                   MAX_INTEREST_POINTS) {
      VectorCopy(level.interestPoints[cent->gent->client->renderInfo.lookTarget]
                     .origin,
                 lookOrg);
    } else {
      return qfalse;
...
#else
    // FIXME: get the angle of the head tag and account for that when finding
    // the lookAngles- 		so if they're lying on their back we get an accurate
    //lookAngle...
    vec3_t headDirs[3];
    vec3_t finalDir;
    ...
    AnglesSubtract(lookAngles, cent->gent->client->renderInfo.eyeAngles,
                   lookAngles);
    return qtrue;
  }
...
static qboolean CG_AddHeadBob(centity_t *cent, vec3_t addTo) {
  ...
  if (!volume) {
    ...
    if (VectorLengthSquared(renderInfo->headBobAngles) < 1.0f) {
      // We are close enough to being back to our normal head position, so we
      // are done for now
      return qfalse;
    }
  } else if (volChange > 2) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      renderInfo->targetHeadBobAngles[i] +=
          Q_flrand(-1.0 * volChange, 1.0 * volChange);
  ...
  for (i = 0; i < 3; i++) {
    ...
    renderInfo->headBobAngles[i] +=
        (renderInfo->targetHeadBobAngles[i] - renderInfo->headBobAngles[i]) *
        (cg.frametime / 150.0f);
    if (addTo) {
      addTo[i] = AngleNormalize180(
          addTo[i] + AngleNormalize180(renderInfo->headBobAngles[i]));
    }
  }

  // We aren't back to our normal position yet, so we still have to apply
  // headBobAngles
  return qtrue;
}

extern float vectoyaw(const vec3_t vec);
static qboolean CG_PlayerLegsYawFromMovement(centity_t *cent,
                                             const vec3_t velocity, float *yaw,
                                             float fwdAngle, float swingTolMin,
                                             float swingTolMax,
                                             qboolean alwaysFace) {
  ...
  if (cent->gent && cent->gent->client &&
      cent->gent->client->ps.forcePowersActive &
          (1 << FP_SPEED)) { // using force speed
...
static void CG_ATSTLegsYaw(centity_t *cent, vec3_t trailingLegsAngles) {
  ...
  CG_PlayerLegsYawFromMovement(cent, cent->gent->client->ps.velocity,
                               &ATSTLegsYaw, cent->lerpAngles[YAW], -60, 60,
                               qtrue);

  float legAngleDiff = AngleNormalize180(ATSTLegsYaw) -
                       AngleNormalize180(cent->pe.legs.yawAngle);
  int legsAnim = cent->currentState.legsAnim;
  qboolean moving =
      (qboolean)!VectorCompare(cent->gent->client->ps.velocity, vec3_origin);
  if (moving || legsAnim == BOTH_TURN_LEFT1 || legsAnim == BOTH_TURN_RIGHT1 ||
      fabs(legAngleDiff) >
          45) { // moving or turning or beyond the turn allowance
    if (legsAnim == BOTH_STAND1 && !moving) { // standing
      if (legAngleDiff > 0) {
        NPC_SetAnim(cent->gent, SETANIM_LEGS, BOTH_TURN_LEFT1,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      } else {
        NPC_SetAnim(cent->gent, SETANIM_LEGS, BOTH_TURN_RIGHT1,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      }
      VectorSet(trailingLegsAngles, 0, cent->pe.legs.yawAngle, 0);
      cent->gent->client->renderInfo.legsYaw = trailingLegsAngles[YAW];
    } else if (legsAnim == BOTH_TURN_LEFT1 ||
               legsAnim == BOTH_TURN_RIGHT1) { // turning
      legAngleDiff =
          AngleSubtract(ATSTLegsYaw, cent->gent->client->renderInfo.legsYaw);
      float add = 0;
      if (legAngleDiff > 50) {
      ...
      float animLength = PM_AnimLength(
          cent->gent->client->clientInfo.animFileIndex, (animNumber_t)legsAnim);
      legAngleDiff *=
          (animLength - cent->gent->client->ps.legsAnimTimer) / animLength;
      VectorSet(trailingLegsAngles, 0,
                cent->pe.legs.yawAngle + legAngleDiff + add, 0);
      if (!cent->gent->client->ps
               .legsAnimTimer) { // FIXME: if start turning in the middle of
                                 // this, our legs pop back to the old
      ...
      VectorSet(trailingLegsAngles, 0,
                AngleNormalize180(cent->pe.legs.yawAngle + legAngleDiff), 0);
      cent->gent->client->renderInfo.legsYaw = trailingLegsAngles[YAW];
    }
    cent->gent->client->renderInfo.legsYaw = cent->pe.legs.yawAngle =
        trailingLegsAngles[YAW];
...
extern void G_BoneOrientationsForClass(int NPC_class, const char *boneName,
...
static void CG_G2ClientSpineAngles(centity_t *cent, vec3_t viewAngles,
                                   const vec3_t angles, vec3_t thoracicAngles,
  ...
  {//don't use lower bones
          ...
          BG_G2SetBoneAngles( cent, cent->gent, cent->gent->upperLumbarBone,
  vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, POSITIVE_Y, POSITIVE_Z,
  cgs.model_draw ); BG_G2SetBoneAngles( cent, cent->gent,
  cent->gent->lowerLumbarBone, vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X,
  POSITIVE_Y, POSITIVE_Z, cgs.model_draw ); return;
  }
  */

  if (cent->gent->client->NPC_class ==
      CLASS_SABER_DROID) { // don't use lower bones
    VectorClear(thoracicAngles);
    VectorClear(ulAngles);
    VectorClear(llAngles);
    return;
  ...
  if (cg_motionBoneComp.integer &&
      ...
      cent->currentState.legsAnim !=
          cent->currentState
              .torsoAnim // NOTE: presumes your legs & torso are on the same
                         // frame, though they *should* be because
                         // PM_SetAnimFinal tries to keep them in synch
      &&
      !G_ClassHasBadBones(
          cent->gent->client
              ->NPC_class)) // these guys' bones are so fucked up we shouldn't
                            // even bother with this motion bone comp...
  { // FIXME: no need to do this if legs and torso on are same frame
    mdxaBone_t boltMatrix;

    if (cg_motionBoneComp.integer > 2 && cent->gent->rootBone >= 0 &&
        cent->gent->lowerLumbarBone >= 0) { // expensive version
      ...
      if (!dummyGhoul2.size()) { // set it up
        int dummyHModel =
            cgi_R_RegisterModel("models/players/_humanoid/_humanoid.glm");
        gi.G2API_InitGhoul2Model(dummyGhoul2,
                                 "models/players/_humanoid/_humanoid.glm",
                                 dummyHModel, NULL_HANDLE, NULL_HANDLE, 0, 0);
        dummyRootBone =
            gi.G2API_GetBoneIndex(&dummyGhoul2[0], "model_root", qtrue);
        dummyHipsBolt = gi.G2API_AddBolt(&dummyGhoul2[0], "pelvis");
      }

      gi.G2API_GetBoneAnimIndex(&cent->gent->ghoul2[cent->gent->playerModel],
                                cent->gent->lowerLumbarBone, cg.time,
                                &upperFrame, &junk, &junk, &junk, &animSpeed,
                                cgs.model_draw);
      // set the dummyGhoul2 lower body to same frame as upper
      gi.G2API_SetBoneAnimIndex(&dummyGhoul2[0], dummyRootBone, upperFrame,
                                upperFrame, BONE_ANIM_OVERRIDE_FREEZE, 1,
                                cg.time, upperFrame, 0);
      // get the dummyGhoul2 lower_lumbar orientation
      gi.G2API_GetBoltMatrix(dummyGhoul2, 0, dummyHipsBolt, &boltMatrix,
      ...
      gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                             cent->gent->crotchBolt, &boltMatrix, vec3_origin,
                             vec3_origin, cg.time, cgs.model_draw,
                             cent->currentState.modelScale);
      gi.G2API_GiveMeVectorFromMatrix(boltMatrix, NEGATIVE_Z, llFwd);
      gi.G2API_GiveMeVectorFromMatrix(boltMatrix, NEGATIVE_Y, llRt);
      ...
      for (int ang = 0; ang < 3; ang++) {
        motionBoneCorrectAngles[ang] =
            AngleNormalize180(AngleDelta(AngleNormalize180(destPAngles[ang]),
                                         AngleNormalize180(curPAngles[ang])));
      }
#ifdef _DEBUG
      Com_Printf("motion bone correction:  %4.2f %4.2f %4.2f\n",
       ...
       for ( int ang = 0; ang < 3; ang++ )
       {
               viewAngles[ang] = AngleNormalize180( viewAngles[ang] -
       AngleNormalize180( destLLAngles[ang] ) );
       }
       */
    } else {
      ...
      gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                             cent->gent->motionBolt, &boltMatrix, vec3_origin,
      ...
      for (int ang = 0; ang < 3; ang++) {
        viewAngles[ang] = AngleNormalize180(
            viewAngles[ang] - AngleNormalize180(motionAngles[ang]));
      }
    }
  }
  ...
  if (cent->gent->client->NPC_class ==
      CLASS_HAZARD_TROOPER) { // only uses lower_lumbar and upper_lumbar to look
                              // around
    VectorClear(thoracicAngles);
    ulAngles[PITCH] = viewAngles[PITCH] * 0.50f;
    llAngles[PITCH] =
        viewAngles[PITCH] * 0.50f + motionBoneCorrectAngles[PITCH];
  ...
  } else if (cent->gent->client->NPC_class ==
             CLASS_ASSASSIN_DROID) { // each bone has only 1 axis of rotation!
    // upper lumbar does not pitch
    thoracicAngles[PITCH] = viewAngles[PITCH] * 0.40f;
    ulAngles[PITCH] = 0.0f;
    llAngles[PITCH] =
        viewAngles[PITCH] * 0.60f + motionBoneCorrectAngles[PITCH];
    ...
    llAngles[PITCH] =
        viewAngles[PITCH] * 0.40f + motionBoneCorrectAngles[PITCH];
  ...
  // thoracic is added modified again by neckAngle calculations, so don't set it
  // until then
  if (G_ClassHasBadBones(cent->gent->client->NPC_class)) {
    ...
    G_BoneOrientationsForClass(cent->gent->client->NPC_class, "upper_lumbar",
                               &oUp, &oRt, &oFwd);
    BG_G2SetBoneAngles(cent, cent->gent, cent->gent->upperLumbarBone, ulAngles,
                       BONE_ANGLES_POSTMULT, oUp, oRt, oFwd, cgs.model_draw);
    G_BoneOrientationsForClass(cent->gent->client->NPC_class, "lower_lumbar",
                               &oUp, &oRt, &oFwd);
    BG_G2SetBoneAngles(cent, cent->gent, cent->gent->lowerLumbarBone, llAngles,
                       BONE_ANGLES_POSTMULT, oUp, oRt, oFwd, cgs.model_draw);
  } else {
    BG_G2SetBoneAngles(cent, cent->gent, cent->gent->upperLumbarBone, ulAngles,
...
static void CG_G2ClientNeckAngles(centity_t *cent, const vec3_t lookAngles,
                                  vec3_t headAngles, vec3_t neckAngles,
                                  vec3_t thoracicAngles,
                                  vec3_t headClampMinAngles,
                                  vec3_t headClampMaxAngles) {
  /*
  if ( G_ClassHasBadBones( cent->gent->client->NPC_class ) )
  {//don't use lower bones
          VectorClear( thoracicAngles );
          ...
          BG_G2SetBoneAngles( cent, cent->gent, cent->gent->thoracicBone,
  vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, POSITIVE_Y, POSITIVE_Z,
  cgs.model_draw ); BG_G2SetBoneAngles( cent, cent->gent,
  cent->gent->cervicalBone, vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X,
  POSITIVE_Y, POSITIVE_Z, cgs.model_draw ); BG_G2SetBoneAngles( cent,
  cent->gent, cent->gent->craniumBone, vec3_origin, BONE_ANGLES_POSTMULT,
  POSITIVE_X, POSITIVE_Y, POSITIVE_Z, cgs.model_draw ); return;
  }
  */
  if (cent->gent->client->NPC_class ==
      CLASS_HAZARD_TROOPER) { // don't use upper bones
    return;
  }
  vec3_t lA;
  VectorCopy(lookAngles, lA);
  // clamp the headangles (which should now be relative to the cervical (neck)
  // angles
  ...
  if (cent->gent->client->NPC_class ==
    ...
    if (thoracicAngles[PITCH]) { // already been set above, blend them
      thoracicAngles[PITCH] =
          (thoracicAngles[PITCH] + (lA[PITCH] * 0.5f)) * 0.5f;
    } else {
    ...
    // no bones roll
             ...
             CLASS_SABER_DROID) { // each bone has only 1 axis of rotation!
    // no thoracic
    VectorClear(thoracicAngles);
    // cervical only yaws
    ...
    if (thoracicAngles[PITCH]) { // already been set above, blend them
      thoracicAngles[PITCH] =
          (thoracicAngles[PITCH] + (lA[PITCH] * 0.4f)) * 0.5f;
    } else {
  ...
  {                                  // aim torso forward too
    ...
    /* ABORTED ATTEMPT AT AIMING GUN WITH SHOULDER WHEN ON BIKE...  POSSIBLY
    RETURN TO THIS LATER if ( cent->gent && cent->gent->client &&
             cent->gent->enemy &&
             cent->gent->humerusRBone!=-1 &&
            (cent->gent->client->ps.torsoAnim==BOTH_VS_ATR_G ||
    cent->gent->client->ps.torsoAnim==BOTH_VS_ATF_G))
    {
            vec3_t		toEnemy;
            ...
            gi.G2API_GetBoltMatrix( actor->ghoul2, actor->playerModel,
    actor->humerusRBone, &boltMatrix, vec3_origin, cent->lerpOrigin, cg.time,
    ...
    actorAim ); vectoangles( actorAim, actorAngles ); vectoangles( toEnemy,
    toEnemyAngles );

                    boltAngles[0] = AngleDelta(actorAngles[0],
    toEnemyAngles[0]); boltAngles[1] = AngleDelta(actorAngles[1],
    toEnemyAngles[1]); boltAngles[2] = AngleDelta(actorAngles[2],
    toEnemyAngles[2]);

                    BG_G2SetBoneAngles( cent, actor, actor->humerusRBone,
    boltAngles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
    cgs.model_draw);
            }
    }*/
  }
  if (G_ClassHasBadBones(cent->gent->client->NPC_class)) {
    Eorientations oUp, oRt, oFwd;
    if (cent->gent->client->NPC_class !=
        CLASS_RANCOR) { // Rancor doesn't use cranium and cervical
      G_BoneOrientationsForClass(cent->gent->client->NPC_class, "cranium", &oUp,
                                 &oRt, &oFwd);
      BG_G2SetBoneAngles(cent, cent->gent, cent->gent->craniumBone, headAngles,
                         BONE_ANGLES_POSTMULT, oUp, oRt, oFwd, cgs.model_draw);
      G_BoneOrientationsForClass(cent->gent->client->NPC_class, "cervical",
                                 &oUp, &oRt, &oFwd);
      BG_G2SetBoneAngles(cent, cent->gent, cent->gent->cervicalBone, neckAngles,
                         BONE_ANGLES_POSTMULT, oUp, oRt, oFwd, cgs.model_draw);
    }
    if (cent->gent->client->NPC_class !=
        CLASS_SABER_DROID) { // saber droid doesn't use thoracic
      if (cent->gent->client->NPC_class == CLASS_RANCOR) {
        thoracicAngles[YAW] = thoracicAngles[ROLL] = 0.0f;
      }
      G_BoneOrientationsForClass(cent->gent->client->NPC_class, "thoracic",
                                 &oUp, &oRt, &oFwd);
      BG_G2SetBoneAngles(cent, cent->gent, cent->gent->thoracicBone,
                         thoracicAngles, BONE_ANGLES_POSTMULT, oUp, oRt, oFwd,
                         cgs.model_draw);
    }
  } else {
    BG_G2SetBoneAngles(cent, cent->gent, cent->gent->craniumBone, headAngles,
    ...
    BG_G2SetBoneAngles(cent, cent->gent, cent->gent->cervicalBone, neckAngles,
                       ...
                       cgs.model_draw);
    BG_G2SetBoneAngles(cent, cent->gent, cent->gent->thoracicBone,
                       thoracicAngles, BONE_ANGLES_POSTMULT, POSITIVE_X,
                       NEGATIVE_Y, NEGATIVE_Z, cgs.model_draw);
  }
}

static void CG_UpdateLookAngles(centity_t *cent, vec3_t lookAngles,
                                float lookSpeed, float minPitch, float maxPitch,
                                float minYaw, float maxYaw, float minRoll,
                                float maxRoll) {
  if (!cent || !cent->gent || !cent->gent->client) {
    return;
  }
  if (cent->gent->client->renderInfo.lookingDebounceTime > cg.time) {
    ...
    if (VectorLengthSquared(lookAnglesDiff)) {
      lookAngles[PITCH] = AngleNormalize180(
          oldLookAngles[PITCH] +
          (lookAnglesDiff[PITCH] * cg.frameInterpolation * lookSpeed));
      lookAngles[YAW] = AngleNormalize180(
          oldLookAngles[YAW] +
          (lookAnglesDiff[YAW] * cg.frameInterpolation * lookSpeed));
      lookAngles[ROLL] = AngleNormalize180(
          oldLookAngles[ROLL] +
          (lookAnglesDiff[ROLL] * cg.frameInterpolation * lookSpeed));
    }
  }
...
static void CG_G2PlayerAngles(centity_t *cent, vec3_t legs[3], vec3_t angles) {
  vec3_t headAngles, neckAngles, chestAngles,
      thoracicAngles = {0, 0, 0}; // legsAngles, torsoAngles,
  ...
  */
  ...
  if (cent->gent &&
      (cent->gent->flags & FL_NO_ANGLES)) { // flatten out all bone angles we
                                            // might have been overriding
    cent->lerpAngles[PITCH] = cent->lerpAngles[ROLL] = 0;
    VectorCopy(cent->lerpAngles, angles);

    BG_G2SetBoneAngles(cent, cent->gent, cent->gent->craniumBone, vec3_origin,
                       BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
                       cgs.model_draw);
    BG_G2SetBoneAngles(cent, cent->gent, cent->gent->cervicalBone, vec3_origin,
                       BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
                       cgs.model_draw);
    BG_G2SetBoneAngles(cent, cent->gent, cent->gent->thoracicBone, vec3_origin,
                       BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
                       cgs.model_draw);
    ...
    BG_G2SetBoneAngles(cent, cent->gent, cent->gent->upperLumbarBone,
                       ...
                       NEGATIVE_Y, NEGATIVE_Z, cgs.model_draw);
    BG_G2SetBoneAngles(cent, cent->gent, cent->gent->lowerLumbarBone,
                       vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X,
                       NEGATIVE_Y, NEGATIVE_Z, cgs.model_draw);
  ...
  if (cent->gent && cent->gent->health <= 0) {
    if (cent->gent->hipsBone != -1) {
      gi.G2API_StopBoneAnimIndex(&cent->gent->ghoul2[cent->gent->playerModel],
                                 cent->gent->hipsBone);
    }
  ...
  if (cent->gent && cent->gent->client &&
      ...
      (cent->gent->client->NPC_class != CLASS_ATST ||
       !cent->gent->s
            .number)) { // If we are rendering third person, we should just
                        // force the player body to always fully face
    //	whatever way they are looking, otherwise, you can end up with gun shots
    //coming off of the 	gun at angles that just look really wrong.

    // NOTENOTE: shots are coming out of the gun at ridiculous angles. The head
    // & torso should pitch *some* when looking up and down...
    VectorCopy(cent->lerpAngles, angles);
    angles[PITCH] = 0;

    if (cent->gent->client) {
      if (cent->gent->client->NPC_class != CLASS_ATST) {
        if (!PM_SpinningSaberAnim(
                cent->currentState.legsAnim)) { // don't turn legs if in a
                                                // spinning saber transition
          // FIXME: use actual swing/clamp tolerances?
          if (cent->gent->client->ps.groundEntityNum != ENTITYNUM_NONE &&
              !PM_InRoll(&cent->gent->client->ps)) { // on the ground
            CG_PlayerLegsYawFromMovement(cent, cent->gent->client->ps.velocity,
    ...
    if (cent->gent && cent->gent->client &&
        cent->gent->client->NPC_class ==
            CLASS_RANCOR) {    // rancor uses full pitch
      if (cent->gent->count) { // don't look up or down at enemy when he's in
                               // your hand...
        viewAngles[PITCH] = 0.0f;
      } else if (cent->gent->enemy) {
        if (cent->gent->enemy->s.solid ==
            SOLID_BMODEL) { // don't look up or down at architecture?
          viewAngles[PITCH] = 0.0f;
        } else if (cent->gent->client->ps.legsAnim ==
                   BOTH_MELEE1) { // don't look up or down when smashing the
                                  // ground
          viewAngles[PITCH] = 0.0f;
        } else {
          ...
          VectorSubtract(cg_entities[cent->gent->enemy->s.number].lerpOrigin,
                         lookFrom, eDir);
          vectoangles(eDir, eAngles);
          viewAngles[PITCH] = AngleNormalize180(eAngles[0]);
          if (cent->gent->client->ps.legsAnim ==
              BOTH_ATTACK2) { // swinging at something on the ground
            if (viewAngles[PITCH] > 0.0f) { // don't look down
              viewAngles[PITCH] = 0.0f;
            }
          } else if (cent->gent->client->ps.legsAnim ==
                     BOTH_ATTACK4) {        // in breath attack anim
            if (viewAngles[PITCH] > 0.0f) { // don't look down
              viewAngles[PITCH] = 0.0f;
    ...
    if (cent->gent && cent->gent->client &&
        cent->gent->client->NPC_class == CLASS_ATST) {
      lookAngles[YAW] = 0;
      BG_G2SetBoneAngles(cent, cent->gent, cent->gent->craniumBone, lookAngles,
      ...
      if (cg_turnAnims.integer && !in_camera && cent->gent->hipsBone >= 0) {
        ...
        // and clear it when we're not... does blend from and to parent actually
        // work?
        int startFrame, endFrame;
        const qboolean animatingHips = gi.G2API_GetAnimRangeIndex(
            &cent->gent->ghoul2[cent->gent->playerModel], cent->gent->hipsBone,
            &startFrame, &endFrame);

        // FIXME: make legs lag behind when turning in place, only play turn
        // anim when legs have to catch up
        if (angles[YAW] == cent->pe.legs.yawAngle) {
          gi.G2API_StopBoneAnimIndex(
              ...
              cent->gent->hipsBone);
                       ...
                       vec3_origin,
                       cent->gent->client->ps
                           .velocity)) { // FIXME: because of
                                         // LegsYawFromMovement, we play the
                                         // turnAnims when we stop running, which
                                         // looks really bad.
          ...
          if (turnAnim != -1 && cent->gent->health > 0) {
            animation_t *animations =
                level
                    .knownAnimFileSets[cent->gent->client->clientInfo
                                           .animFileIndex]
                    .animations;

            if (!animatingHips ||
                (animations[turnAnim].firstFrame !=
                 startFrame)) // only set the anim if we aren't going to do the
                              // same animation again
            {
              float animSpeed = 50.0f / animations[turnAnim].frameLerp *
                                PM_GetTimeScaleMod(cent->gent);

              gi.G2API_SetBoneAnimIndex(
                  &cent->gent->ghoul2[cent->gent->playerModel],
                  cent->gent->hipsBone, animations[turnAnim].firstFrame,
                  animations[turnAnim].firstFrame +
                      animations[turnAnim].numFrames,
                  BONE_ANIM_OVERRIDE_LOOP /*|BONE_ANIM_OVERRIDE_FREEZE|BONE_ANIM_BLEND*/
                  ,
                  animSpeed, cg.time, -1, 100);
            }
          } else {
            gi.G2API_StopBoneAnimIndex(
                &cent->gent->ghoul2[cent->gent->playerModel],
      ...
      CG_G2ClientSpineAngles(cent, viewAngles, angles, thoracicAngles, ulAngles,
                             llAngles);
    }
    ...
    */
    ...
    if (cent->gent && cent->gent->client &&
        cent->gent->client->NPC_class ==
            CLASS_VEHICLE) { // you are a vehicle, just use your lerpAngles
                             // which comes from m_vOrientation
      cent->pe.legs.yawing = qfalse;
    ...
    } else if (G_IsRidingVehicle(
                   cent->gent)) { // riding a vehicle, get the vehicle's
                                  // lerpAngles (which comes from m_vOrientation)
      cent->pe.legs.yawing = qfalse;
      cent->pe.legs.yawAngle =
          cg_entities[cent->gent->owner->s.number].lerpAngles[YAW];
      if (cent->gent->client) {
        cent->gent->client->renderInfo.legsYaw =
            cg_entities[cent->gent->owner->s.number].lerpAngles[YAW];
      }
      AnglesToAxis(cg_entities[cent->gent->owner->s.number].lerpAngles, legs);
    } else {

      // set the legs.yawing field so we play the turning anim when turning in
      // place
      if (angles[YAW] == cent->pe.legs.yawAngle) {
        cent->pe.legs.yawing = qfalse;
      } else {
      ...
      if (((cent->gent->client->ps.eFlags & EF_FORCE_GRIPPED) ||
        ...
        angles[PITCH] = AngleNormalize180(
            DotProduct(cent->gent->client->ps.velocity, centFwd) /
            (2 * divFactor));
        if (angles[PITCH] > 90) {
          angles[PITCH] = 90;
        ...
        angles[ROLL] = AngleNormalize180(
            DotProduct(cent->gent->client->ps.velocity, centRt) /
            (10 * divFactor));
        if (angles[ROLL] > 90) {
          angles[ROLL] = 90;
    ...
    if (cent->gent && cent->gent->client &&
      ...
      if (cent->pe.torso.yawing ||
          cent->pe.torso.pitching) { // If torso is turning, we want to turn
                                     // head just as fast
        if (cent->gent->NPC) {
          lookAngleSpeed =
              cent->gent->NPC->stats.yawSpeed / 150; // about 0.33 normally
        } else {
          lookAngleSpeed = CG_SWINGSPEED;
        }
      } else if (talking) { // Slow for head bobbing
        lookAngleSpeed = LOOK_TALKING_SPEED;
      } else if (looking) { // Not talking, set it up for looking at enemy,
                            // CheckLookTarget will scale it down if neccessary
        lookAngleSpeed = lookingSpeed;
      } else if (cent->gent->client->renderInfo.lookingDebounceTime >
                 cg.time) { // Not looking, not talking, head is returning from
                            // a talking head bob, use talking speed
        lookAngleSpeed = LOOK_TALKING_SPEED;
      }

      if (looking ||
          talking) { // want to keep doing this lerp behavior for a full second
                     // after stopped looking (so don't snap)
      ...
      if (cent->gent->client->NPC_class ==
          CLASS_RANCOR) { // always use the viewAngles we calced
        VectorCopy(viewAngles, lookAngles);
      }
      CG_UpdateLookAngles(cent, lookAngles, lookAngleSpeed, -50.0f, 50.0f,
    ...
    if (cent->gent && cent->gent->client &&
      ...
      BG_G2SetBoneAngles(cent, cent->gent, cent->gent->thoracicBone, lookAngles,
                         BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                         NEGATIVE_Z, cgs.model_draw);
    } else {
      vec3_t headClampMinAngles = {-25, -55, -10},
             headClampMaxAngles = {50, 50, 10};
      CG_G2ClientNeckAngles(cent, lookAngles, headAngles, neckAngles,
                            thoracicAngles, headClampMinAngles,
                            headClampMaxAngles);
    }
    return;
  }
  // All other entities
    ...
    if ((cent->gent->client->NPC_class == CLASS_PROBE) ||
      ...
      if (cent->gent->client->ps.groundEntityNum !=
        ...
        CG_PlayerLegsYawFromMovement(cent, cent->gent->client->ps.velocity,
                                     &angles[YAW], cent->lerpAngles[YAW], -60,
                                     60, qtrue);
      } else { // face legs to front
        CG_PlayerLegsYawFromMovement(cent, vec3_origin, &angles[YAW],
                                     cent->lerpAngles[YAW], -60, 60, qtrue);
      ...
      if (cent->gent->client->NPC_class == CLASS_ATST) { // body pitch
        BG_G2SetBoneAngles(cent, cent->gent, cent->gent->thoracicBone,
                           lookAngles, BONE_ANGLES_POSTMULT, POSITIVE_X,
                           NEGATIVE_Y, NEGATIVE_Z, cgs.model_draw);
      }
      ...
      /*
              ...
              && (cent->gent->client->NPC_class ==
      CLASS_WAMPA||cent->gent->client->NPC_class == CLASS_RANCOR) )
      {
              CG_ATSTLegsYaw( cent, trailingLegsAngles );
              AnglesToAxis( trailingLegsAngles, legs );
      }
      */
      ...
      if (cent->gent->client->renderInfo.lookingDebounceTime >
          cg.time) { // adjust for current body orientation
        ...
        // Remember last headAngles
        vec3_t oldLookAngles;
        VectorCopy(cent->gent->client->renderInfo.lastHeadAngles,
                   oldLookAngles);
        if (VectorCompare(oldLookAngles, lookAngles) == qfalse) {
          ...
          lookAngles[YAW] =
              oldLookAngles[YAW] + (lookAngles[YAW] - oldLookAngles[YAW]) *
                                       cg.frameInterpolation * 0.25;
        }
        // Remember current lookAngles next time
        VectorCopy(lookAngles, cent->gent->client->renderInfo.lastHeadAngles);
      ...
      if (cent->gent->client->NPC_class == CLASS_WAMPA) {
        Eorientations oUp, oRt, oFwd;
        G_BoneOrientationsForClass(cent->gent->client->NPC_class, "cranium",
                                   &oUp, &oRt, &oFwd);
        BG_G2SetBoneAngles(cent, cent->gent, cent->gent->craniumBone,
                           lookAngles, BONE_ANGLES_POSTMULT, oUp, oRt, oFwd,
                           cgs.model_draw);
      } else {
        BG_G2SetBoneAngles(cent, cent->gent, cent->gent->craniumBone,
                           lookAngles, BONE_ANGLES_POSTMULT, POSITIVE_X,
...
static void CG_PlayerAngles(centity_t *cent, vec3_t legs[3], vec3_t torso[3],
  ...
  if (cg.renderingThirdPerson && cent->gent && cent->gent->s.number == 0) {
    // If we are rendering third person, we should just force the player body to
    // always fully face
    //	whatever way they are looking, otherwise, you can end up with gun shots
    //coming off of the 	gun at angles that just look really wrong.
    ...
    // FIXME: this needs to properly set the legs.yawing field so we don't
    // erroneously play the turning anim, but we do play it when turning in place
    ...
    if (cent->gent->client->ps.velocity[0] ||
      ...
      lookAngles[YAW] =
          cent->lerpAngles[YAW] + AngleDelta(cent->lerpAngles[YAW], moveYaw);
    }
  ...
  if (cent->currentState.clientNum != 0) {
    ...
    // These next two are only used for a calc below- this clamp is done in
    // PM_UpdateViewAngles
    ...
    if (cent->gent && cent->gent->next_roff_time &&
        cent->gent->next_roff_time >= cg.time) { // Following a roff, body must
                                                 // keep up with head, yaw-wise
      headYawClampMin = headYawClampMax = torsoYawSwingTolMin =
          torsoYawSwingTolMax = torsoYawClampMin = torsoYawClampMax =
              legsYawSwingTolMin = legsYawSwingTolMax = 0;
    }
  ...
  if (cent->gent->client->renderInfo.renderFlags & RF_LOCKEDANGLE) {
    ...
    if (cent->gent && cent->gent->health > 0) {
      ...
      if (CG_PlayerLegsYawFromMovement(cent, cent->gent->client->ps.velocity,
                                       &legsAngles[YAW], headAngles[YAW],
                                       torsoYawClampMin, torsoYawClampMax,
                                       alwaysFace)) {
        ...
        CG_SwingAngles(legsAngles[YAW], legsYawSwingTolMin, legsYawSwingTolMax,
                       torsoYawClampMin, torsoYawClampMax, maxYawSpeed,
  ...
  */
  ...
  CG_SwingAngles(headAngles[YAW], torsoYawSwingTolMin, torsoYawSwingTolMax,
                 headYawClampMin, headYawClampMax, yawSpeed,
  ...
  CG_SwingAngles(dest, torsoPitchSwingTolMin, torsoPitchSwingTolMax,
                 torsoPitchClampMin, torsoPitchClampMax, 0.1f,
  ...
  // FIXME: to clamp the head angles, figure out tag_head's offset from
  // tag_torso and add 	that to whatever offset we're getting here... so turning
  //the head in an 	anim that also turns the head doesn't allow the head to turn
  //out of range.
  ...
  */
  ...
  if (cent->pe.torso.yawing ||
      cent->pe.torso
          .pitching) { // If torso is turning, we want to turn head just as fast
    ...
    lookAngleSpeed = lookingSpeed;
             ...
             cg.time) { // Not looking, not talking, head is returning from a
                        // talking head bob, use talking speed
    lookAngleSpeed = LOOK_TALKING_SPEED;
  }

  if (looking ||
      talking) { // Keep this type of looking for a second after stopped looking
    cent->gent->client->renderInfo.lookingDebounceTime = cg.time + 1000;
  }
  ...
  if (cent->gent->client->renderInfo.lookingDebounceTime > cg.time) {
    ...
    for (i = 0; i < 3; i++) {
      lookAngles[i] = AngleNormalize360(
          cent->gent->client->renderInfo.headBobAngles[i] + lookAngles[i]);
    }

    if (VectorCompare(headAngles, lookAngles) == qfalse) {
      // FIXME: This clamp goes off viewAngles,
      // but really should go off the tag_torso's axis[0] angles, no?
      CG_UpdateAngleClamp(lookAngles[PITCH], headPitchClampMin / 1.25,
                          headPitchClampMax / 1.25, lookAngleSpeed,
                          &headAngles[PITCH], viewAngles[PITCH]);
      CG_UpdateAngleClamp(lookAngles[YAW], headYawClampMin / 1.25,
                          headYawClampMax / 1.25, lookAngleSpeed,
                          &headAngles[YAW], viewAngles[YAW]);
      CG_UpdateAngleClamp(lookAngles[ROLL], -10, 10, lookAngleSpeed,
                          &headAngles[ROLL], viewAngles[ROLL]);
    }

    if (!cent->gent->enemy || cent->gent->enemy->s.number !=
                                  cent->gent->client->renderInfo.lookTarget) {
      ...
      scale = fabs(swing) /
              (torsoYawClampMax + 0.01); // NOTENOTE: Some ents have a clamp of
                                         // 0, which is bad for division
      ...
      scale = fabs(swing) /
              (torsoPitchClampMax + 0.01); // NOTENOTE: Some ents have a clamp
                                           // of 0, which is bad for division
...
static void CG_PlayerPowerups(centity_t *cent) {
  ...
  /*
          ...
          if ( cent->currentState.powerups & ( 1 << PW_QUAD ) ) {
                  cgi_R_AddLightToScene( cent->lerpOrigin, 200 + (rand()&31),
     0.2, 0.2, 1 );
          }
          ...
          if ( cent->currentState.powerups & ( 1 << PW_REDFLAG ) ) {
                  ...
                  cgi_R_AddLightToScene( cent->lerpOrigin, 200 + (rand()&31), 1,
     0.2, 0.2 );
          }
  ...
  */
  ...
  //	{
  //		cgi_R_AddLightToScene( cent->lerpOrigin, 200 + (rand()&31), 0.8f,
  //0.8f, 0.2f );
  //	}
...
static qboolean _PlayerShadow(const vec3_t origin, const float orientation,
                              float *const shadowPlane, const float radius,
  ...
  *shadowPlane = trace.endpos[2] + 1;
  ...
  if (cg_shadows.integer == 1 ||
      (in_camera && cg_shadows.integer ==
                        2)) // don't want stencil shadows during a cinematic
  {
    ...
    CG_ImpactMark(markShader, trace.endpos, trace.plane.normal, orientation, 1,
                  1, 1, alpha, qfalse, radius, qtrue);
  }
  return qtrue;
}
...
===============
*/
static qboolean CG_PlayerShadow(centity_t *const cent,
                                float *const shadowPlane) {
  *shadowPlane = 0;
  ...
  if (cent->gent->client->NPC_class ==
      CLASS_SAND_CREATURE) { // sand creatures have no shadow
    return qfalse;
  }
  ...
  { // i might be running out of my bounding box, so get my root origin
    mdxaBone_t boltMatrix;
    gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                           cent->gent->rootBone, &boltMatrix, tempAngles,
                           cent->lerpOrigin, cg.time, cgs.model_draw,
                           cent->currentState.modelScale);
  ...
  if (DistanceSquared(cg.refdef.vieworg, rootOrigin) >
      cg_shadowCullDistance.value * cg_shadowCullDistance.value) {
  ...
  if (cent->gent->client->NPC_class == CLASS_ATST) {
    ...
    gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                           cent->gent->footLBolt, &boltMatrix, tempAngles,
                           cent->lerpOrigin, cg.time, cgs.model_draw,
                           cent->currentState.modelScale);
    ...
    gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                           cent->gent->footRBolt, &boltMatrix, tempAngles,
                           cent->lerpOrigin, cg.time, cgs.model_draw,
                           cent->currentState.modelScale);
    gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, sideOrigin);
    sideOrigin[2] += 30; // fudge up a bit for coplaner
    bShadowed = (qboolean)(_PlayerShadow(sideOrigin, 0, shadowPlane, 28,
                                         cgs.media.shadowMarkShader) ||
                           bShadowed);

    bShadowed =
        (qboolean)(_PlayerShadow(rootOrigin, cent->pe.legs.yawAngle,
                                 shadowPlane, 64, cgs.media.shadowMarkShader) ||
                   bShadowed);
    return bShadowed;
  } else if (cent->gent->client->NPC_class == CLASS_RANCOR) {
    return _PlayerShadow(rootOrigin, cent->pe.legs.yawAngle, shadowPlane, 64,
                         cgs.media.shadowMarkShader);
  } else {
    return _PlayerShadow(rootOrigin, cent->pe.legs.yawAngle, shadowPlane, 16,
                         cgs.media.shadowMarkShader);
  }
}
...
static void _PlayerFootStep(const vec3_t origin, const vec3_t traceDir,
                            const float orientation, const float radius,
                            centity_t *const cent,
                            footstepType_t footStepType) {
  vec3_t end, mins = {-7, -7, 0}, maxs = {7, 7, 2};
  trace_t trace;
  ...
  // send a trace down from the player to the ground
  VectorCopy(origin, end);
  VectorMA(origin, FOOTSTEP_DISTANCE, traceDir,
           end); // was end[2] -= FOOTSTEP_DISTANCE;
  ...
  if (soundType < FOOTSTEP_TOTAL) {
    cgi_S_StartSound(NULL, cent->currentState.clientNum, CHAN_BODY,
  ...
  if (projNormal[2] > 0.5f) {
    // footsteps will not have the correct orientation for all surfaces, so punt
    // and set the projection to Z
    projNormal[0] = 0.0f;
    projNormal[1] = 0.0f;
    projNormal[2] = 1.0f;
  }
  CG_ImpactMark(footMarkShader, trace.endpos, projNormal, orientation, 1, 1, 1,
                1.0f, qfalse, radius, qfalse);
}
...
static void CG_PlayerFootsteps(centity_t *const cent,
  ...
  if (cent->gent->client->NPC_class != CLASS_ATST &&
    ...
    gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                           footBolt, &boltMatrix, tempAngles, cent->lerpOrigin,
                           cg.time, cgs.model_draw,
                           cent->currentState.modelScale);
    gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, sideOrigin);
    gi.G2API_GiveMeVectorFromMatrix(boltMatrix, NEGATIVE_Y, footDownDir);
    VectorMA(sideOrigin, -8.0f, footDownDir,
             sideOrigin); // was [2] += 15;	//fudge up a bit for coplanar
    _PlayerFootStep(sideOrigin, footDownDir, cent->pe.legs.yawAngle - 90, 6,
                    cent, footStepType);
  }
}

static void _PlayerSplash(const vec3_t origin, const vec3_t velocity,
                          const float radius, const int maxUp) {
  ...
  cgi_CM_BoxTrace(&trace, start, end, NULL, NULL, 0,
  ...
  FX_AddOrientedParticle(-1, end, trace.plane.normal, NULL, NULL, 6.0f,
                         radius + Q_flrand(0.0f, 1.0f) * 48.0f, 0, alpha, 0.0f,
                         0.0f, WHITE, WHITE, 0.0f, Q_flrand(0.0f, 1.0f) * 360,
                         Q_flrand(-1.0f, 1.0f) * 6.0f, NULL, NULL, 0.0f, 0, 0,
                         1200, cgs.media.wakeMarkShader,
...
static void CG_PlayerSplash(centity_t *cent) {
  ...
  if (cent->gent && cent->gent->client) {
    ...
    if (cent->gent->disconnectDebounceTime <
        cg.time) // can't do these expanding ripples all the time
    {
      ...
      cent->gent->disconnectDebounceTime =
          cg.time + 125 + Q_flrand(0.0f, 1.0f) * 50.0f;
    }
  }
}
...
static void CG_ForcePushRefraction(vec3_t org, centity_t *cent) {
  ...
  if (!cent->gent->client
           ->pushEffectFadeTime) { // the duration for the expansion and fade
  ...
  if ((REFRACT_EFFECT_DURATION - tDif) <
      200) { // stop following the hand after a little and stay in a fixed spot
  ...
  memset(&ent, 0, sizeof(ent));
  ent.shaderTime =
      (cent->gent->client->pushEffectFadeTime - REFRACT_EFFECT_DURATION) /
      1000.0f;
...
static void CG_ForcePushBodyBlur(centity_t *cent, const vec3_t origin,
                                 vec3_t tempAngles) {
  vec3_t fxOrg;
  mdxaBone_t boltMatrix;
  ...
  if (cent->gent->torsoBolt >= 0) {
    gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                           cent->gent->torsoBolt, &boltMatrix, tempAngles,
                           origin, cg.time, cgs.model_draw,
                           cent->currentState.modelScale);
  ...
  if (cent->gent->handRBolt >= 0) {
    ...
    gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                           cent->gent->handRBolt, &boltMatrix, tempAngles,
                           origin, cg.time, cgs.model_draw,
                           cent->currentState.modelScale);
    gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, fxOrg);
    CG_ForcePushBlur(fxOrg);
  ...
  if (cent->gent->handLBolt >= 0) {
    ...
    gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                           cent->gent->handLBolt, &boltMatrix, tempAngles,
                           origin, cg.time, cgs.model_draw,
                           cent->currentState.modelScale);
    gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, fxOrg);
    CG_ForcePushBlur(fxOrg);
  ...
  if (cent->gent->kneeLBolt >= 0) {
    gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                           cent->gent->kneeLBolt, &boltMatrix, tempAngles,
                           origin, cg.time, cgs.model_draw,
                           cent->currentState.modelScale);
    gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, fxOrg);
    CG_ForcePushBlur(fxOrg);
  ...
  if (cent->gent->kneeRBolt >= 0) {
    gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                           cent->gent->kneeRBolt, &boltMatrix, tempAngles,
                           origin, cg.time, cgs.model_draw,
                           cent->currentState.modelScale);
    gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, fxOrg);
    CG_ForcePushBlur(fxOrg);
  ...
  if (cent->gent->elbowLBolt >= 0) {
    // Do the elbows
    gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                           cent->gent->elbowLBolt, &boltMatrix, tempAngles,
                           origin, cg.time, cgs.model_draw,
                           cent->currentState.modelScale);
    gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, fxOrg);
    CG_ForcePushBlur(fxOrg);
  ...
  if (cent->gent->elbowRBolt >= 0) {
    gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                           cent->gent->elbowRBolt, &boltMatrix, tempAngles,
                           origin, cg.time, cgs.model_draw,
                           cent->currentState.modelScale);
    gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, fxOrg);
    CG_ForcePushBlur(fxOrg);
...
static void CG_ForceElectrocution(centity_t *cent, const vec3_t origin,
  ...
  if (bolt >= 0) {
    found =
        gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                               bolt, &boltMatrix, tempAngles, origin, cg.time,
                               cgs.model_draw, cent->currentState.modelScale);
  }
  ...
  if (tr.fraction < 1.0f || Q_flrand(0.0f, 1.0f) > 0.94f || alwaysDo) {
    FX_AddElectricity(
        -1, fxOrg, tr.endpos, 1.5f, 4.0f, 0.0f, 1.0f, 0.5f, 0.0f, rgb, rgb,
        0.0f, 5.5f, Q_flrand(0.0f, 1.0f) * 50 + 100, shader,
        FX_ALPHA_LINEAR | FX_SIZE_LINEAR | FX_BRANCH | FX_GROW | FX_TAPER, -1,
        -1);
  }
}

static void CG_BoltedEffects(centity_t *cent, const vec3_t origin,
                             vec3_t tempAngles) {
  if (cent->gent && cent->gent->client &&
      cent->gent->client->NPC_class == CLASS_VEHICLE) {
    Vehicle_t *pVeh = cent->gent->m_pVehicle;
    gentity_t *parent = cent->gent;
    if (pVeh->m_ulFlags & VEH_ARMORLOW && (pVeh->m_iLastFXTime <= cg.time) &&
        Q_irand(0, 1) == 0) {
      pVeh->m_iLastFXTime = cg.time + 50; // Q_irand(50, 100);
      CG_PlayEffectIDBolted(pVeh->m_pVehicleInfo->iArmorLowFX,
                            parent->playerModel, parent->crotchBolt,
...
*/
qboolean
CG_PlayerCanSeeCent(centity_t *cent) { // return true if this cent is in view
  ...
  if (g_entities[0].client->ps.forcePowerLevel[FP_SEE] < FORCE_LEVEL_2 &&
      cent->currentState.eType !=
          ET_PLAYER) { // TEST: level 1 only sees force hints and enemies
    return qfalse;
  }
  ...
  dot += (0.99f - dot) * centDist /
         range; // the farther away they are, the more in front they have to be
...
void CG_AddForceSightShell(refEntity_t *ent, centity_t *cent) {
  ...
  if ((cent->currentState.eFlags & EF_FORCE_VISIBLE) ||
      (cent->currentState.eType == ET_PLAYER && cent->gent &&
       cent->gent->message)) {
    ent->shaderRGBA[0] = 0;
    ent->shaderRGBA[1] = 0;
  ...
  if (g_entities[0].client->ps.forcePowerLevel[FP_SEE] >
      FORCE_LEVEL_2) { // TEST: level 3 also displays health
    if (cent->gent && cent->gent->health > 0 &&
        cent->gent->max_health > 0) { // draw a health bar over them
...
void CG_AddRefEntityWithPowerups(refEntity_t *ent, int powerups,
  ...
  //		cgi_S_AddLoopingSound( 0, cent->lerpOrigin, vec3_origin,
  ...
  if (gent->client->ps.powerups[PW_DISRUPTION]) {
    // I guess when something dies, it looks like pos1 gets set to the impact
    // point on death, we can do fun stuff with this
    vec3_t tempAng;
    VectorSubtract(gent->pos1, ent->origin, ent->oldorigin);
    // er, adjust this to get the proper position in model space... account for
    // yaw
    ...
    if (cg.time - ent->endTime < 1000 &&
        (cg_timescale.value * cg_timescale.value * Q_flrand(0.0f, 1.0f)) >
            0.05f) {
      vec3_t fxOrg;
      mdxaBone_t boltMatrix;

      gi.G2API_GetBoltMatrix(cent->gent->ghoul2, gent->playerModel,
                             gent->torsoBolt, &boltMatrix, gent->currentAngles,
                             ent->origin, cg.time, cgs.model_draw,
                             gent->s.modelScale);
      gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, fxOrg);
  ...
  if ((powerups & (1 << PW_UNCLOAKING))) { // in the middle of cloaking
    if ((cg.snap->ps.forcePowersActive & (1 << FP_SEE)) &&
      ...
      float perc =
          (float)(gent->client->ps.powerups[PW_UNCLOAKING] - cg.time) / 2000.0f;
    ...
    if ((cg.snap->ps.forcePowersActive & (1 << FP_SEE)) &&
      ...
      if (cg_renderToTextureFX.integer && cg_shadows.integer != 2 &&
          cgs.glconfig.stencilBits >= 4) {
        cgi_R_SetRefractProp(
            1.0f, 0.0f, qfalse,
  ...
  if ((powerups & (1 << PW_SHOCKED))) {
    ...
    if (dif > 0 && Q_flrand(0.0f, 1.0f) > 0.4f) {
      ...
      if (Q_flrand(0.0f, 1.0f) > 0.9f)
        cgi_S_StartSound(
            ent->origin, gent->s.number, CHAN_AUTO,
  ...
  if (cg_speedTrail.integer &&
      (gent->client->ps.forcePowersActive & (1 << FP_SPEED) // in force speed
       || cent->gent->client->ps.legsAnim ==
              BOTH_FORCELONGLEAP_START // or force long jump - FIXME: only 1st
                                       // half of that anim?
       || cent->gent->client->ps.legsAnim ==
              BOTH_FORCELONGLEAP_ATTACK) // or force long jump attack
      &&
      (gent->s.number || cg.renderingThirdPerson)) // looks dumb doing this with
                                                   // first peron mode on
  {
    // FIXME: debounce this
    localEntity_t *ex;
  ...
  if (powerups & (1 << PW_GALAK_SHIELD)) {
    /*		refEntity_t tent;
                    ...
                    // Setting up the 2d control points, these get swept around
       to make a 3D lathed model VectorSet2( tent.axis[0], 0.5, 0 );
       // start point of curve
                    VectorSet2( tent.axis[1], 50,	85 );		// control
       point 1 VectorSet2( tent.axis[2], 135, -100 );		// control point
       2 VectorSet2( tent.oldorigin, 0, -90 );		// end point of curve
       ...
       1000 > cg.time )
                    ...
                    gi.G2API_GetBoltMatrix( cent->gent->ghoul2,
       gent->playerModel, gent->genericBolt1, &boltMatrix, angles,
       cent->lerpOrigin, cg.time, cgs.model_draw, cent->currentState.modelScale
       ); gi.G2API_GiveMeVectorFromMatrix( boltMatrix, ORIGIN, tent.origin );//
       pass in the emitter origin here

                    tent.endTime = gent->fx_time + 1000;			// if you
       want the shell to build around the guy, pass in a time that is 1000ms
       after the start of the turn-on-effect tent.customShader =
  ...
  {
          theFxScheduler.PlayEffect( cgs.effects.forceInvincibility,
  cent->lerpOrigin );
  }
  */

  // Healing -- could use some work....maybe also make it NOT be framerate
  // dependant
  //------------------------------------------------------
  /*	if ( powerups & ( 1 << PW_HEALING ))
          {
                  ...
                  AngleVectors( cent->gent->client->renderInfo.eyeAngles,
     axis[0], axis[1], axis[2] );

                  theFxScheduler.PlayEffect( cgs.effects.forceHeal,
     cent->gent->client->renderInfo.eyePoint, axis );
          }
  */
  // Push Blur
  if (gent->forcePushTime > cg.time &&
      gi.G2API_HaveWeGhoul2Models(cent->gent->ghoul2)) {
  ...
  if ((cent->gent->client->ps.forcePowersActive & (1 << FP_PROTECT)) &&
      (cent->gent->client->ps.forcePowersActive &
       (1 << FP_ABSORB))) { // using both at once, save ourselves some rendering
    ...
    cgi_R_AddRefEntityToScene(ent);
  } else if (cent->gent->client->ps.forcePowersActive &
             (1 << FP_PROTECT)) { // protect is represented by green..
    ent->shaderRGBA[0] = 0;
    ent->shaderRGBA[1] = 255;
    ent->shaderRGBA[2] = 0;
    ent->shaderRGBA[3] = 254;
    ...
    cgi_R_AddRefEntityToScene(ent);
             ...
             (1 << FP_ABSORB)) { // absorb is represented by blue..
    ent->shaderRGBA[0] = 0;
    ent->shaderRGBA[1] = 0;
    ent->shaderRGBA[2] = 255;
    ent->shaderRGBA[3] = 254;
  ...
  if ((cg.snap->ps.forcePowersActive & (1 << FP_SEE)) &&
      ...
      (cent->currentState.eFlags & EF_FORCE_VISIBLE ||
       ((cent->gent->health > 0 || cent->gent->message) &&
        cent->currentState.eType ==
            ET_PLAYER // other things handle this in their own render funcs
        && CG_PlayerCanSeeCent(
               cent)))) { // force sight draws auras around living things
  ...
  if (((cent->gent->client->ps.eFlags & EF_FORCE_DRAINED) ||
      ...
      (cg.renderingThirdPerson ||
       cent->currentState.number !=
           cg.snap->ps.clientNum)) { // draining or being drained
    ent->renderfx |= RF_RGB_TINT;
    ent->shaderRGBA[0] = 255;
    ent->shaderRGBA[1] = ent->shaderRGBA[2] = 0;
    ent->shaderRGBA[3] = 255;
...
static void CG_G2SetHeadBlink(centity_t *cent, qboolean bStart) {
  ...
  // FIXME: need a version of this that *doesn't* need the mFileName in the
  // ghoul2
  ...
  gi.G2API_SetBoneAnglesIndex(&gent->ghoul2[gent->playerModel], hLeye,
                              desiredAngles, BONE_ANGLES_POSTMULT, POSITIVE_Y,
                              POSITIVE_Z, POSITIVE_X, NULL, blendTime, cg.time);
  ...
  if (!bWink)
    gi.G2API_SetBoneAnglesIndex(&gent->ghoul2[gent->playerModel], hReye,
                                desiredAngles, BONE_ANGLES_POSTMULT, POSITIVE_Y,
                                POSITIVE_Z, POSITIVE_X, NULL, blendTime,
                                cg.time);
}
...
static void CG_G2SetHeadAnim(centity_t *cent, int anim) {
  ...
  const float timeScaleMod =
      (cg_timescale.value) ? (1.0 / cg_timescale.value) : 1.0;
  ...
  //only set the anim if we aren't going to do the same animation again
  {
    gi.G2API_SetBoneAnimIndex(&gent->ghoul2[gent->playerModel],
                              cent->gent->faceBone, firstFrame, lastFrame,
...
static qboolean CG_G2PlayerHeadAnims(centity_t *cent) {
  ...
  if (cent->gent->health <=
      0) { // Dead people close their eyes and don't make faces!
    anim = FACE_DEAD;
  } else {
    ...
    if (cent->gent->client->facial_blink <
        0) { // yes, check if we are we done blinking ?
      if (-(cent->gent->client->facial_blink) <
          cg.time) { // yes, so reset blink timer
        cent->gent->client->facial_blink = cg.time + Q_flrand(4000.0, 8000.0);
    ...
    } else // no we aren't blinking
    {
      if (cent->gent->client->facial_blink < cg.time) // but should we start ?
      {
        ...
        if (cent->gent->client->facial_blink ==
          ...
          cent->gent->client->facial_blink =
              -(cg.time + 99999999.0f); // set blink timer
        } else {
          cent->gent->client->facial_blink =
              -(cg.time + 300.0f); // set blink timer
        }
      }
    }

    if (gi.VoiceVolume[cent->gent->s.clientNum] >
        0) // if we aren't talking, then it will be 0, -1 for talking but paused
    {
      ...
      if (cent->gent->client->breathPuffTime >
          cg.time + 300) { // when talking, do breath puff
        cent->gent->client->breathPuffTime = cg.time;
      }
    } else if (gi.VoiceVolume[cent->gent->s.clientNum] ==
               -1) { // talking but silent
      anim = FACE_TALK0;
      cent->gent->client->facial_timer = cg.time + Q_flrand(2000.0, 7000.0);
    } else if (gi.VoiceVolume[cent->gent->s.clientNum] ==
               0) // don't do aux if in a slient part of speech
    {             // not talking
      if (cent->gent->client->facial_timer < 0) // are we auxing ?
      {                                         // yes
        if (-(cent->gent->client->facial_timer) <
            cg.time) // are we done auxing ?
        {            // yes, reset aux timer
          cent->gent->client->facial_timer =
              cg.time + Q_flrand(7000.0, 10000.0);
      ...
      {      // but should we start ?
        if (cent->gent->client->facial_timer < cg.time) { // yes
          cent->gent->client->facial_anim =
              FACE_ALERT + Q_irand(0, 2); // alert, smile, frown
          // set aux timer
...
add it to the face skin
        ...
        {//FIXME: When talking, look at talkTarget, if any
                //ALSO: When talking, add a head bob/movement on syllables -
when gi.VoiceVolume[] changes drastically

                if ( cent->gent->health <= 0 )
                {//Dead people close their eyes and don't make faces!  They also
tell no tales...  BUM BUM BAHHHHHHH!
...
4+gi.VoiceVolume[cent->gent->s.clientNum];
                //reset the frown and blink timers
        }
        else
        // ok, we have facial extensions, but we aren't speaking. Lets decide if
we need to frown or blink if (ci->extensions)
        {
                int	add_in = 0;
                ...
                //Dead people close their eyes and don't make faces!  They also
tell no tales...  BUM BUM BAHHHHHHH! if ( cent->gent->health <= 0 )
                {
                        //Make them always blink and frown
                        head->customSkin = ci->headSkin + 3;
                        return qtrue;
                ...
                if (!cent->gent->client->facial_blink)
                {	// reset blink timer
                        cent->gent->client->facial_blink = cg.time +
Q_flrand(3000.0, 5000.0); cent->gent->client->facial_frown = cg.time +
Q_flrand(6000.0, 10000.0); cent->gent->client->facial_aux = cg.time +
Q_flrand(6000.0, 10000.0);
                }
                ...
                if (cent->gent->client->facial_aux < 0)
                {
                        ...
                        {
                                // reset frown timer
                                cent->gent->client->facial_aux = cg.time +
Q_flrand(6000.0, 10000.0);
                        }
                        else
                        {
...
3000.0);
                ...
                // now, if we aren't auxing - lets see if we should be blinking
or frowning if (!add_in)
                {
                        if( gi.VoiceVolume[cent->gent->s.clientNum] == -1 )
                        {//then we're talking and don't want to use blinking
normal frames, force open eyes. add_in = 0;
                                // reset blink timer
                                cent->gent->client->facial_blink = cg.time +
Q_flrand(3000.0, 5000.0);
                        }
                        // are we blinking ?
...
cg.time)
                                {
                                        ...
                                        // reset blink timer
                                        cent->gent->client->facial_blink =
cg.time + Q_flrand(3000.0, 5000.0);
                                }
                        }
...
-(cg.time + 200.0);
                        ...
                        if (cent->gent->client->facial_frown < 0)
                        {
                                ...
                                // are we done frowning ?
                                if (-(cent->gent->client->facial_frown) <
cg.time)
                                {
                                        add_in -= 2;
                                        // reset frown timer
                                        cent->gent->client->facial_frown =
cg.time + Q_flrand(6000.0, 10000.0);
                                }
                        }
                        // no we aren't frowning
                        else
...
static void CG_GetPlayerLightLevel(centity_t *cent) {
  ...
  if (cent->gent->client->ps.weapon == WP_SABER &&
    ...
    cent->gent->lightLevel += (cent->gent->client->ps.SaberLength() /
                               cent->gent->client->ps.SaberLengthMax()) *
                              200;
  }
}
...
static void CG_StopWeaponSounds(centity_t *cent) {
  ...
  if (cent->currentState.weapon == WP_SABER) {
    if (cent->gent && cent->gent->client) {
      if (!cent->gent->client->ps.SaberActive()) { // neither saber is on
        return;
      } else if (cent->gent->client->ps
                     .saberInFlight) // cent->gent->client->ps.saberInFlight )
      {                              // throwing saber
        if (!cent->gent->client->ps.dualSabers ||
            !cent->gent->client->ps.saber[1]
                 .Active()) { // don't have a second saber or it's not on
          return;
        }
      }
    }
  ...
  if (cent->currentState.weapon == WP_STUN_BATON ||
      cent->currentState.weapon == WP_CONCUSSION) { // idling sounds
  ...
  if (!(cent->currentState.eFlags & EF_FIRING)) {
    if (cent->pe.lightningFiring) {
      if (weapon->stopSound) {
        cgi_S_StartSound(cent->lerpOrigin, cent->currentState.number,
                         CHAN_WEAPON, weapon->stopSound);
      }
  ...
  if (cent->currentState.eFlags & EF_ALT_FIRING) {
    if (weapon->altFiringSound) {
      cgi_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                            vec3_origin, weapon->altFiringSound);
    }
...
void CG_SaberDoWeaponHitMarks(gclient_t *client, gentity_t *saberEnt,
                              ...
                              vec3_t hitPos, vec3_t hitDir, vec3_t uaxis,
                              vec3_t splashBackDir, float sizeTimeScale) {
  if (client && sizeTimeScale > 0.0f && hitEnt && hitEnt->client &&
      hitEnt->ghoul2.size()) { // burn mark with glow
    // FIXME: set the correct angle based on direction of swing
    // FIXME: keep a count of these on the ent and don't add too many
    int lifeTime = (1.01 - (float)(hitEnt->health) / hitEnt->max_health) *
                   (float)Q_irand(5000, 10000);
    float size = 0.0f;
    int weaponMarkShader = 0, markShader = cgs.media.bdecal_saberglowmark;

    // First: do mark decal on hitEnt
    if (WP_SaberBladeUseSecondBladeStyle(&client->ps.saber[saberNum],
                                         bladeNum)) {
      if (client->ps.saber[saberNum]
              .g2MarksShader2[0]) { // we have a shader to use instead of the
                                    // standard mark shader
        markShader =
            cgi_R_RegisterShader(client->ps.saber[saberNum].g2MarksShader2);
        lifeTime = Q_irand(20000, 30000); // last longer if overridden
      }
    } else {
      if (client->ps.saber[saberNum]
              .g2MarksShader[0]) { // we have a shader to use instead of the
                                   // standard mark shader
        markShader =
            cgi_R_RegisterShader(client->ps.saber[saberNum].g2MarksShader);
        lifeTime = Q_irand(20000, 30000); // last longer if overridden
      }
    }
    ...
    if (markShader) {
      ...
      CG_AddGhoul2Mark(markShader, size, hitPos, hitDir, hitEnt->s.number,
                       ...
                       hitEnt->client->renderInfo.legsYaw, hitEnt->ghoul2,
    ...
    if (WP_SaberBladeUseSecondBladeStyle(&client->ps.saber[saberNum],
                                         bladeNum)) {
      if (client->ps.saber[saberNum]
              .g2WeaponMarkShader2[0]) { // we have a shader to use instead of
                                         // the standard mark shader
        weaponMarkShader = cgi_R_RegisterShader(
            client->ps.saber[saberNum].g2WeaponMarkShader2);
        lifeTime = Q_irand(7000, 12000); // last longer if overridden
      }
    } else {
      if (client->ps.saber[saberNum]
              .g2WeaponMarkShader[0]) { // we have a shader to use instead of
                                        // the standard mark shader
        weaponMarkShader =
    ...
    if (weaponMarkShader) {
      centity_t *splatterOnCent = (saberEnt && client->ps.saberInFlight
      ...
      if (splatterOnCent->gent->ghoul2.size() > saberNum + 1) {
        CG_AddGhoul2Mark(weaponMarkShader, size, hitPos, backDir,
                         splatterOnCent->currentState.number,
                         splatterOnCent->lerpOrigin, yawAngle,
                         splatterOnCent->gent->ghoul2,
                         splatterOnCent->currentState.modelScale, lifeTime,
                         saberNum + 1, uaxis /*splashBackDir*/);
      }
    }
  }
}
...
static void CG_DoSaberLight(saberInfo_t *saber) {
  ...
  if (totallength) { // actually have something to do
    ...
    cgi_R_AddLightToScene(mid, diameter + (Q_flrand(0.0f, 1.0f) * 8.0f), rgb[0],
                          rgb[1], rgb[2]);
  }
}

static void CG_DoSaber(vec3_t origin, vec3_t dir, float length, float lengthMax,
                       float radius, saber_colors_t color, int rfx,
                       qboolean doLight) {
  vec3_t mid;
  ...
  // half!!  or something...
  if (doLight) { // FIXME: RGB combine all the colors of the sabers you're using
                 // into one averaged color!
    vec3_t rgb = {1, 1, 1};
    CG_RGBForSaberColor(color, rgb);
    cgi_R_AddLightToScene(mid, (length * 1.4f) + (Q_flrand(0.0f, 1.0f) * 3.0f),
                          rgb[0], rgb[1], rgb[2]);
  }
...
static void CG_CreateSaberMarks(vec3_t start, vec3_t end, vec3_t normal) {
  ...
  for (i = 0, mf = markFragments; i < numFragments; i++, mf++) {
    ...
    // by moving the start time back, we can hack it to fade out way before the
    // burn does
    mark = CG_AllocMark();
...
void CG_CheckSaberInWater(centity_t *cent, centity_t *scent, int saberNum,
  ...
  if (!scent || modelIndex == -1 || scent->gent->ghoul2.size() <= modelIndex ||
      scent->gent->ghoul2[modelIndex].mBltlist.size() <=
          0 || // using a camera puts away your saber so you have no bolts
      scent->gent->ghoul2[modelIndex].mModelindex == -1) {
    return;
  }
  if (cent && cent->gent && cent->gent->client &&
      (cent->gent->client->ps.saber[saberNum].saberFlags &
       SFL_ON_IN_WATER)) { // saber can stay on underwater
    return;
  }
  if (gi.totalMapContents() & (CONTENTS_WATER | CONTENTS_SLIME)) {
    ...
    gi.G2API_GetBoltMatrix(scent->gent->ghoul2, modelIndex, 0, &boltMatrix,
                           angles, origin, cg.time, cgs.model_draw,
                           scent->currentState.modelScale);
    // work the matrix axis stuff into the original axis and origins used.
    gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, saberOrg);

    const int contents =
        gi.pointcontents(saberOrg, cent->currentState.clientNum);
...
static void CG_AddSaberBladeGo(centity_t *cent, centity_t *scent,
                               refEntity_t *saber, int renderfx, int modelIndex,
                               vec3_t origin, vec3_t angles, int saberNum,
                               int bladeNum) {
  vec3_t org_, end, // org_future,
      axis_[3] = {
          {0, 0, 0}, {0, 0, 0}, {0, 0, 0}}; //, axis_future[3]={0,0,0, 0,0,0,
                                            //0,0,0};	// shut the compiler up
  trace_t trace;
  float length;
  int bolt;
  mdxaBone_t boltMatrix;
  ...
  */
  ...
  if (1) {
    ...
    {//hack - come off the forearm
            ...
            if ( saberNum == 0 )
            {
                    bolt = gi.G2API_AddBolt(
    &cent->gent->ghoul2[cent->gent->playerModel], "*r_hand_cap_r_arm" ); if (
    bolt == -1 )
                    {
                            bolt = cent->gent->handRBolt;
                            fwdAxis = NEGATIVE_Y;
            ...
            {
                    bolt = gi.G2API_AddBolt(
    &cent->gent->ghoul2[cent->gent->playerModel], "*l_hand_cap_l_arm" ); if (
    bolt == -1 )
                    {
                            bolt = cent->gent->handLBolt;
                            fwdAxis = NEGATIVE_Y;
            ...
            tagHack = qtrue;//use the hacked switch statement below to position
    and orient the blades
            // figure out where the actual model muzzle is
            gi.G2API_GetBoltMatrix( cent->gent->ghoul2, cent->gent->playerModel,
    bolt, &boltMatrix, angles, origin, cg.time, cgs.model_draw,
    cent->currentState.modelScale );
            // work the matrix axis stuff into the original axis and origins
    used. gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, org_);
            gi.G2API_GiveMeVectorFromMatrix(boltMatrix, (Eorientations)fwdAxis,
    axis_[0]); gi.G2API_GiveMeVectorFromMatrix(boltMatrix,
    (Eorientations)rtAxis, axis_[1]);//right
            gi.G2API_GiveMeVectorFromMatrix(boltMatrix, (Eorientations)upAxis,
    axis_[2]);//up
    }
    else
    */
    {
      ...
      if (bolt == -1) {
        tagHack = qtrue; // use the hacked switch statement below to position
                         // and orient the blades
        // hmm, just fall back to the most basic tag (this will also make it
        // work with pre-JKA saber models
      ...
      if (!WP_SaberBladeUseSecondBladeStyle(
              &cent->gent->client->ps.saber[saberNum], bladeNum) &&
          cent->gent->client->ps.saber[saberNum].bladeEffect) {
        CG_PlayEffectIDBolted(
            cent->gent->client->ps.saber[saberNum].bladeEffect, modelIndex,
            bolt, scent->currentState.clientNum, scent->lerpOrigin, -1, qfalse);
      } else if (WP_SaberBladeUseSecondBladeStyle(
                     &cent->gent->client->ps.saber[saberNum], bladeNum) &&
                 cent->gent->client->ps.saber[saberNum].bladeEffect2) {
        CG_PlayEffectIDBolted(
            cent->gent->client->ps.saber[saberNum].bladeEffect2, modelIndex,
            bolt, scent->currentState.clientNum, scent->lerpOrigin, -1, qfalse);
      }
      // get the boltMatrix
      gi.G2API_GetBoltMatrix(scent->gent->ghoul2, modelIndex, bolt, &boltMatrix,
                             angles, origin, cg.time, cgs.model_draw,
                             scent->currentState.modelScale);

      // work the matrix axis stuff into the original axis and origins used.
      gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, org_);
      gi.G2API_GiveMeVectorFromMatrix(
          boltMatrix, NEGATIVE_X,
          axis_[0]); // front (was NEGATIVE_Y, but the md3->glm exporter screws
                     // up this tag somethin' awful)
    ...
    /*
    {
            ...
            if ( cent->gent )
            {
                    VectorSubtract( cent->lerpOrigin, cent->gent->lastOrigin,
    orgDiff ); VectorSubtract( cent->lerpAngles, cent->gent->lastAngles, angDiff
    ); VectorAdd( futureOrigin, orgDiff, futureOrigin ); for ( int i = 0; i < 3;
    i++ )
                    {
                            futureAngles[i] = AngleNormalize360(
    futureAngles[i]+angDiff[i] );
                    }
            }

            // figure out where the actual model muzzle will be after next
    server frame. gi.G2API_GetBoltMatrix(scent->gent->ghoul2, modelIndex, 0,
    &boltMatrix, futureAngles, futureOrigin, futuretime, cgs.model_draw,
    scent->currentState.modelScale);
            // work the matrix axis stuff into the original axis and origins
    used. gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, org_future);
            gi.G2API_GiveMeVectorFromMatrix(boltMatrix, NEGATIVE_X,
    axis_future[0]);//was NEGATIVE_Y, but the md3->glm exporter screws up this
    tag somethin' awful
    }
    */
  } else {
  ...
  */
  ...
  // FIXME: temp hack until we have a tag_flash2 for the second (3rd? 4th?)
  // blade
  // FIXME: maybe fall back on this if the saber model has only 1 tag_flash?
  //			or, better yet, if the saber info doesn't list tagnames for the
  //blades?
  if (tagHack) {
    ...
    case SABER_STAR:
      ...
      else
      {
              ...
              // figure out where the actual model muzzle is
              gi.G2API_GetBoltMatrix( cent->gent->ghoul2,
      cent->gent->playerModel, bolt, &boltMatrix, angles, origin, cg.time,
      cgs.model_draw, cent->currentState.modelScale );
              // work the matrix axis stuff into the original axis and origins
      used. gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, org_);
      }
  ...
  VectorCopy(
      org_, cent->gent->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint);
  VectorCopy(axis_[0],
             cent->gent->client->ps.saber[saberNum].blade[bladeNum].muzzleDir);
  ...
  // length for purposes of rendering and marks trace will be longer than blade
  // so we don't damage past a wall's surface
  if (cent->gent->client->ps.saber[saberNum].blade[bladeNum].length <
      cent->gent->client->ps.saber[saberNum].blade[bladeNum].lengthMax) {
    if (cent->gent->client->ps.saber[saberNum].blade[bladeNum].length <
        cent->gent->client->ps.saber[saberNum].blade[bladeNum].lengthMax - 8) {
      length =
          cent->gent->client->ps.saber[saberNum].blade[bladeNum].length + 8;
    } else {
  ...
  // If the saber is in flight we shouldn't trace from the player to the muzzle
  // point
  if (cent->gent->client->ps.saberInFlight && saberNum == 0) {
    ...
    { // i might be running out of my bounding box, so get my root origin
      mdxaBone_t boltMatrix;
      gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                             cent->gent->rootBone, &boltMatrix, angles,
                             cent->lerpOrigin, cg.time, cgs.model_draw,
                             cent->currentState.modelScale);
      gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, rootOrigin);
    } else {
    ...
    gi.trace(&trace, rootOrigin, NULL, NULL,
             cent->gent->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint,
             cent->currentState.number, CONTENTS_SOLID, (EG2_Collision)0, 0);
  }
  ...
  if (trace.fraction < 1.0f) {
    ...
    if ((!WP_SaberBladeUseSecondBladeStyle(
             &cent->gent->client->ps.saber[saberNum], bladeNum) &&
         (cent->gent->client->ps.saber[saberNum].saberFlags2 &
          SFL2_NO_IDLE_EFFECT)) ||
        (WP_SaberBladeUseSecondBladeStyle(
             &cent->gent->client->ps.saber[saberNum], bladeNum) &&
         (cent->gent->client->ps.saber[saberNum].saberFlags2 &
          SFL2_NO_IDLE_EFFECT2))) { // do no effects when idle
      if (!cent->gent->client->ps.saberInFlight &&
          ...
          !PM_SaberInSpecialAttack(
              cent->gent->client->ps.torsoAnim)) { // idle, do no marks
        noMarks = qtrue;
      }
    }
              ...
              // hit a corner, blade is still 8 longer than hit
    {
      if (i) { // tracing from end to base
        gi.trace(&trace, end, NULL, NULL, org_, cent->currentState.clientNum,
                 traceMask, (EG2_Collision)0, 0);
      } else { // tracing from base to end
        gi.trace(&trace, org_, NULL, NULL, end, cent->currentState.clientNum,
                 traceMask | CONTENTS_WATER | CONTENTS_SLIME, (EG2_Collision)0,
                 0);
      }
      ...
      if (trace.fraction < 1.0f) {
        if ((trace.contents & CONTENTS_WATER) ||
          ...
          if (!noMarks) {
            if ((!WP_SaberBladeUseSecondBladeStyle(&client->ps.saber[saberNum],
                                                   bladeNum) &&
                 !(client->ps.saber[saberNum].saberFlags2 &
                   SFL2_NO_WALL_MARKS)) ||
                (WP_SaberBladeUseSecondBladeStyle(&client->ps.saber[saberNum],
                                                  bladeNum) &&
                 !(client->ps.saber[saberNum].saberFlags2 &
                   SFL2_NO_WALL_MARKS2))) {
              if (!(trace.surfaceFlags & SURF_NOIMPACT) // never spark on sky
                  && (trace.entityNum == ENTITYNUM_WORLD ||
                      cg_entities[trace.entityNum].currentState.solid ==
                          SOLID_BMODEL) &&
                  Q_irand(1, client->ps.saber[saberNum].numBlades) == 1) {
                // was "sparks/spark"
                theFxScheduler.PlayEffect("sparks/spark_nosnd", trace.endpos,
                                          trace.plane.normal);
              }
            }
            ...
            if (client->ps.saber[saberNum]
              ...
              if (trace.entityNum == ENTITYNUM_WORLD ||
                  ...
                  cg_entities[trace.entityNum].currentState.eType ==
                      ET_TERRAIN) { // only put marks on architecture
                if ((!WP_SaberBladeUseSecondBladeStyle(
                         &client->ps.saber[saberNum], bladeNum) &&
                     !(client->ps.saber[saberNum].saberFlags2 &
                       SFL2_NO_WALL_MARKS)) ||
                    (WP_SaberBladeUseSecondBladeStyle(
                         &client->ps.saber[saberNum], bladeNum) &&
                  ...
                  CG_CreateSaberMarks(client->ps.saber[saberNum]
                                          .blade[bladeNum]
                                          .trail.oldPos[i],
                  ...
                  if (Q_irand(1, client->ps.saber[saberNum].numBlades) == 1) {
                    // make a sound
                    if (cg.time - cent->gent->client->ps
                                      .saberHitWallSoundDebounceTime >=
                        100) { // ugh, need to have a real sound debouncer... or
                               // do this game-side
                      cent->gent->client->ps.saberHitWallSoundDebounceTime =
                          cg.time;
                      cgi_S_StartSound(
                          cent->lerpOrigin, cent->currentState.clientNum,
                          CHAN_ITEM,
                          cgi_S_RegisterSound(
        ...
        VectorCopy(
            trace.plane.normal,
            client->ps.saber[saberNum].blade[bladeNum].trail.oldNormal[i]);

        if (!i &&
            trace.contents &
                (CONTENTS_SOLID | CONTENTS_TERRAIN |
                 CONTENTS_SHOTCLIP)) { // Now that we don't let the blade go
                                       // through walls, we need to shorten the
                                       // blade when it hits one
          cent->gent->client->ps.saber[saberNum].blade[bladeNum].length =
              cent->gent->client->ps.saber[saberNum].blade[bladeNum].length *
              trace.fraction; // this will stop damage from going through walls
          if (cent->gent->client->ps.saber[saberNum].blade[bladeNum].length <=
              0.1f) { // SIGH... hack so it doesn't play the saber turn-on sound
                      // that plays when you first turn the saber on (assumed
                      // when saber is active but length is zero)
            cent->gent->client->ps.saber[saberNum].blade[bladeNum].length =
                0.1f; // FIXME: may go through walls still??
          }
          // FIXME: should probably re-extend instantly, not use the
          // "turning-on" growth rate
        }
      } else {
        cent->gent->client->ps.saberEventFlags &= ~SEF_INWATER;
        if (client->ps.saber[saberNum].blade[bladeNum].trail.haveOldPos[i]) {
          if (!noMarks) {
            if ((!WP_SaberBladeUseSecondBladeStyle(&client->ps.saber[saberNum],
              ...
              // Let's put the mark there, we should use an endcap mark to close
              // the line, but we
              //	can probably just get away with a round mark
              // CG_ImpactMark( cgs.media.rivetMarkShader,
              // client->ps.saber[saberNum].blade[bladeNum].trail.oldPos[i],
              // client->ps.saber[saberNum].blade[bladeNum].trail.oldNormal[i],
              //		0.0f, 1.0f, 1.0f, 1.0f, 1.0f, qfalse, 1.1f,
              //qfalse );
            }
          }
        }
  ...
  */
  if (!client->ps.saber[saberNum].blade[bladeNum].active &&
      client->ps.saber[saberNum].blade[bladeNum].length <= 0) {
    return;
  }
  ...
  if ((!WP_SaberBladeUseSecondBladeStyle(&client->ps.saber[saberNum],
      ...
      (WP_SaberBladeUseSecondBladeStyle(&client->ps.saber[saberNum],
                                        bladeNum) &&
       client->ps.saber[saberNum].trailStyle2 < 2)) { // okay to draw the trail
    saberTrail_t *saberTrail =
        &client->ps.saber[saberNum].blade[bladeNum].trail;

#define SABER_TRAIL_TIME 40.0f

    // if we happen to be timescaled or running in a high framerate situation,
    // we don't want to flood
    //	the system with very small trail slices...but perhaps doing it by
    //distance would yield better results?
    if (saberTrail->lastTime >
        cg.time) { // after a pause, cg.time jumps ahead in time for one frame
    ...
    {
      if (saberTrail->inAction &&
          cg.time <
              saberTrail->lastTime + 300) // if we have a stale segment, don't
      ...
      // we must always do this, even if we aren't active..otherwise we won't
  ...
  if ((!WP_SaberBladeUseSecondBladeStyle(&client->ps.saber[saberNum],
      ...
      (WP_SaberBladeUseSecondBladeStyle(&client->ps.saber[saberNum],
                                        bladeNum) &&
       (client->ps.saber[saberNum].saberFlags2 &
        SFL2_NO_BLADE2))) { // don't draw a blade
  ...
  CG_DoSaber(org_, axis_[0], length,
...
void CG_AddSaberBlade(centity_t *cent, centity_t *scent, refEntity_t *saber,
                      int renderfx, int modelIndex, vec3_t origin,
                      vec3_t angles) {
  // FIXME: if this is a dropped saber, it could be possible that it's the
  // second saber?
  if (cent->gent->client) {
    for (int i = 0; i < cent->gent->client->ps.saber[0].numBlades; i++) {
      CG_AddSaberBladeGo(cent, scent, saber, renderfx, modelIndex, origin,
                         angles, 0, i);
    }
...
/*
static void CG_AddSaberBlades( centity_t *cent, int renderfx, vec3_t origin,
vec3_t angles, int saberNum )
{
        if ( cent->gent->client )
        {
                for ( int i = 0; i <
cent->gent->client->ps.saber[saberNum].numBlades; i++ )
                {
                        CG_AddSaberBladeGo( cent, cent, NULL, renderfx,
cent->gent->weaponModel[saberNum], origin, angles, saberNum, i );
                }
        }
}
*/
...
CG_Player
  ...
  FIXME: Extend this to be a render func for all multiobject entities in the
game such that:

        You can have and stack multiple animated pieces (not just legs and
torso) You can attach "bolt-ons" that either animate or don't (weapons, heads,
borg pieces) You can attach any object to any tag on any object (weapon on the
head, etc.)
  ...
  ALSO:
        ...
        Implement alpha
        Implement tint
        Implement other effects (generic call effect at org and dir (or tag),
with parms?)

===============
*/
extern qboolean G_GetRootSurfNameWithVariant(gentity_t *ent,
...
void CG_Player(centity_t *cent) {
  ...
  if (cent->gent->s.number == 0 && cg.weaponSelect == WP_NONE &&
      cg.zoomMode == 1) {
    // HACK
    return;
  }
  ...
  if ((in_camera) && cent->currentState.clientNum ==
                         0) // If player in camera then no need for shadow
  {
    return;
  }
  ...
  */
  if (cent->gent->ghoul2.size()) // do we have ghoul models attached?
  {
    ...
    // FIXME: if at all possible, do all our sets before our gets to do only *1*
    // G2 skeleton transform per render frame
    ...
    // add the shadow
    // FIXME: funcs that modify our origin below will cause the shadow to be in
    // the wrong spot
    ...
    if (!cg.renderingThirdPerson ||
        cg.zoomMode) { // in first person or zoomed in
      if (cg.snap->ps.viewEntity <= 0 ||
          cg.snap->ps.viewEntity >= ENTITYNUM_WORLD) { // no viewentity
        if (cent->currentState.number ==
            cg.snap->ps.clientNum) { // I am the player
          if (cg.snap->ps.weapon != WP_SABER &&
              cg.snap->ps.weapon != WP_MELEE) { // not using saber or fists
            ent.renderfx = RF_THIRD_PERSON;     // only draw in mirrors
          }
        }
      } else if (cent->currentState.number ==
                 cg.snap->ps.viewEntity) { // I am the view entity
        if (cg.snap->ps.weapon != WP_SABER &&
            cg.snap->ps.weapon != WP_MELEE) { // not using first person saber
                                              // test or, if so, not using saber
          ent.renderfx = RF_THIRD_PERSON; // only draw in mirrors
        }
      }
    }
    ...
    if (cent->gent->client->ps.powerups[PW_DISINT_2] > cg.time) { // ghost!
      ent.renderfx = RF_THIRD_PERSON; // only draw in mirrors
               ...
               (ent.renderfx &
                RF_THIRD_PERSON)) { // show stencil shadow in first person now
                                    // because we can -rww
      ent.renderfx |= RF_SHADOW_ONLY;
    }

    if ((cg_shadows.integer == 2 && !in_camera) ||
    ...
    if (cent->currentState.eFlags & EF_LOCKED_TO_WEAPON && cent->gent &&
        cent->gent->health > 0 && cent->gent->owner) {
      ...
      if (chair && chair->gent) {
        ...
        if (chair->gent->bounceCount) { // EWeb
          ...
          chair->gent->pos1[YAW] = AngleSubtract(
              cent->lerpAngles[YAW],
              chair->gent->s.angles[YAW]); // remember which dir our turret is
                                           // facing for later
          cent->lerpAngles[ROLL] = 0;

          BG_G2SetBoneAngles(chair, chair->gent, chair->gent->lowerLumbarBone,
                             chair->gent->pos1, BONE_ANGLES_POSTMULT,
                             POSITIVE_Z, NEGATIVE_X, NEGATIVE_Y,
                             cgs.model_draw);
          BG_G2SetBoneAngles(chair, chair->gent, chair->gent->upperLumbarBone,
                             temp, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_X,
          ...
          cent->lerpAngles[ROLL] = 0;
          BG_G2SetBoneAngles(chair, chair->gent, chair->gent->lowerLumbarBone,
                             temp, BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z,
                             POSITIVE_X, cgs.model_draw);
        }
        // gi.G2API_SetBoneAngles( &chair->gent->ghoul2[0], "swivel_bone", temp,
        // BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X,
        // cgs.model_draw );
        ...
        gi.G2API_GetBoltMatrix(chair->gent->ghoul2, chair->gent->playerModel,
                               chair->gent->headBolt, &boltMatrix,
                               chair->gent->s.apos.trBase,
                               chair->gent->currentOrigin, cg.time,
        ...
        if (chair->gent->bounceCount) { // put behind it, not in chair
          ...
          gi.G2API_GiveMeVectorFromMatrix(boltMatrix, POSITIVE_Y,
                                          chair->gent->pos3);
          gi.G2API_GiveMeVectorFromMatrix(boltMatrix, NEGATIVE_Z,
                                          chair->gent->pos4);

          AnglesToAxis(cent->lerpAngles, ent.axis);
          VectorCopy(cent->lerpAngles,
                     tempAngles); // tempAngles is needed a lot below
        }

        VectorCopy(ent.origin, ent.oldorigin);
        VectorCopy(ent.origin, ent.lightingOrigin);

        // FIXME:  Mike claims that hacking the eyepoint will make them shoot at
        // me.......so,
        //	we move up from the seat bolt and store off that point.
        //			VectorMA( ent.origin, -20, chair->gent->pos3,
        //cent->gent->client->renderInfo.eyePoint ); 			VectorMA(
        //cent->gent->client->renderInfo.eyePoint, 40, chair->gent->pos4,
        //cent->gent->client->renderInfo.eyePoint );
      }
      ...
      // Get the driver tag.
      mdxaBone_t boltMatrix;
      gi.G2API_GetBoltMatrix(vehEnt->gent->ghoul2, vehEnt->gent->playerModel,
                             vehEnt->gent->crotchBolt, &boltMatrix,
                             vehEnt->lerpAngles, vehEnt->lerpOrigin,
                             (cg.time ? cg.time : level.time), NULL,
                             vehEnt->currentState.modelScale);
      gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, ent.origin);
      ...
      if (monster && monster->gent && monster->gent->inuse &&
          monster->gent->health > 0) {
        mdxaBone_t boltMatrix;
        ...
        gi.G2API_GetBoltMatrix(
            monster->gent->ghoul2, monster->gent->playerModel, boltIndex,
        ...
        if (cent->gent->client->isRagging) { // hack, ragdoll has you way at
        ...
        //	else
        //	{
        //		//cent->gent->client->ps.viewangles[YAW] =
        //cent->lerpAngles[YAW];
        //	}
      } else { // wtf happened to the guy holding me?  Better get out
        cent->gent->activator = NULL;
        cent->gent->client->ps.eFlags &=
            ~(EF_HELD_BY_WAMPA | EF_HELD_BY_RANCOR);
      }
      ...
      if (sand_creature && sand_creature->gent) {
        ...
        gi.G2API_GetBoltMatrix(
            sand_creature->gent->ghoul2, sand_creature->gent->playerModel,
            sand_creature->gent->gutBolt, &boltMatrix, scAngles,
            sand_creature->lerpOrigin, cg.time, cgs.model_draw,
            sand_creature->currentState.modelScale);
        // Storing ent position, bolt position, and bolt axis
        gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, ent.origin);
        gi.G2API_GiveMeVectorFromMatrix(boltMatrix, NEGATIVE_Y, ent.axis[0]);
      ...
      /*
      if ( cent->gent->client->ps.eFlags & EF_FORCE_DRAINED
              && !VectorCompare( cent->gent->client->ps.forceDrainOrg,
      vec3_origin ) )
    ...
    {//add it at my origin
            ...
            swoopEnt.hModel =
    cgs.model_draw[g_vehicleInfo[cent->currentState.vehicleIndex].modelIndex];
    ...
    */
    ...
    if (cent->currentState.number != 0 || cg.renderingThirdPerson ||
      ...
      // we don't override thes in pure 1st person because they will be set
      // before this func
    ...
    if (cent->gent->handRBolt != -1) {
      // Get handRPoint
      gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                             cent->gent->handRBolt, &boltMatrix, G2Angles,
                             ent.origin, cg.time, cgs.model_draw,
                             cent->currentState.modelScale);
      gi.G2API_GiveMeVectorFromMatrix(
          boltMatrix, ORIGIN, cent->gent->client->renderInfo.handRPoint);
    }
    if (cent->gent->handLBolt != -1) {
      // always get handLPoint too...?
      gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                             cent->gent->handLBolt, &boltMatrix, G2Angles,
                             ent.origin, cg.time, cgs.model_draw,
                             cent->currentState.modelScale);
      gi.G2API_GiveMeVectorFromMatrix(
          boltMatrix, ORIGIN, cent->gent->client->renderInfo.handLPoint);
    }
    if (cent->gent->footLBolt != -1) {
      // get the feet
      gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                             cent->gent->footLBolt, &boltMatrix, G2Angles,
                             ent.origin, cg.time, cgs.model_draw,
                             cent->currentState.modelScale);
      gi.G2API_GiveMeVectorFromMatrix(
          boltMatrix, ORIGIN, cent->gent->client->renderInfo.footLPoint);
    }

    if (cent->gent->footRBolt != -1) {
      gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                             cent->gent->footRBolt, &boltMatrix, G2Angles,
                             ent.origin, cg.time, cgs.model_draw,
                             cent->currentState.modelScale);
      gi.G2API_GiveMeVectorFromMatrix(
          boltMatrix, ORIGIN, cent->gent->client->renderInfo.footRPoint);
    }
    ...
    if (cent->gent && cent->gent->client &&
        ...
        cent->gent->client->NPC_class !=
            CLASS_ATST) { // FIXME: somehow saberactive is getting lost over the
                          // network
      ...
      for (int saberNum = 0; saberNum < numSabers; saberNum++) {
        ...
        for (int bladeNum = 0;
             bladeNum < cent->gent->client->ps.saber[saberNum].numBlades;
             bladeNum++) {
          if (!cent->gent->client->ps.saber[saberNum].blade[bladeNum].active ||
              cent->gent->client->ps.saber[saberNum].blade[bladeNum].length >
                  cent->gent->client->ps.saber[saberNum]
                      .blade[bladeNum]
                      .lengthMax) // hack around network lag for now
          {                       // saber blade is off
            if (cent->gent->client->ps.saber[saberNum].blade[bladeNum].length >
                0) {
              if (cent->gent->client->ps.stats[STAT_HEALTH] <=
                  0) { // dead, didn't actively turn it off
                cent->gent->client->ps.saber[saberNum].blade[bladeNum].length -=
                    cent->gent->client->ps.saber[saberNum]
                        .blade[bladeNum]
                        .lengthMax /
            ...
            if (cent->gent->client->ps.saber[saberNum].blade[bladeNum].length <
                cent->gent->client->ps.saber[saberNum]
                    .blade[bladeNum]
                    .lengthMax) {
              if (!cent->gent->client->ps.saber[saberNum]
                       .blade[bladeNum]
                       .length) {
                qhandle_t saberOnSound =
                    cgs.sound_precache[g_entities[cent->currentState.clientNum]
                                           .client->ps.saber[saberNum]
                                           .soundOn];
                ...
                if (cent->gent->client->ps.saberInFlight &&
                    saberNum == 0) { // play it on the saber
                  if (cg_saberOnSoundTime[cent->currentState.number] <
                      cg.time) {
                    cgi_S_UpdateEntityPosition(
                        cent->gent->client->ps.saberEntityNum,
                        g_entities[cent->gent->client->ps.saberEntityNum]
                            .currentOrigin);
                    cgi_S_StartSound(NULL,
                                     cent->gent->client->ps.saberEntityNum,
                                     CHAN_AUTO, saberOnSound);
                    cg_saberOnSoundTime[cent->currentState.number] =
                        cg.time; // so we don't play multiple on sounds at one
                                 // time
                  }
                } else {
                  if (cg_saberOnSoundTime[cent->currentState.number] <
              ...
              if (cg.frametime > 0) {
                if (PM_SuperBreakWinAnim(
                        cent->gent->client->ps
                            .torsoAnim)) { // just keep it full length!
                  // NOTE: does this mean it will go through walls now...?
                  cent->gent->client->ps.saber[saberNum]
                      .blade[bladeNum]
                      .length = cent->gent->client->ps.saber[saberNum]
                                    .blade[bladeNum]
                                    .lengthMax;
                } else {
                  cent->gent->client->ps.saber[saberNum]
          ...
          if (cent->gent->client->ps.saber[saberNum].blade[bladeNum].length >
              0) {
            if (!cent->gent->client->ps.saberInFlight ||
                saberNum != 0) //&& cent->gent->client->ps.saberActive)
            {                  // holding the saber in-hand
              //						CGhoul2Info
              //*currentModel = &cent->gent->ghoul2[1]; 						CGhoul2Info *nextModel =
              //&cent->gent->ghoul2[1];
              // FIXME: need a version of this that *doesn't* need the mFileName
              // in the ghoul2
              // FIXME: use an actual surfaceIndex?
              char handName[MAX_QPATH];
              if (saberNum == 0) {
                // this returns qfalse if it doesn't exist or isn't being
                // rendered
                if (G_GetRootSurfNameWithVariant(
                        cent->gent, "r_hand", handName,
                        sizeof(handName))) //! gi.G2API_GetSurfaceRenderStatus(
                                           //! &cent->gent->ghoul2[cent->gent->playerModel],
                                           //! "r_hand" ) )//surf is still on
                {
                  CG_AddSaberBladeGo(cent, cent, NULL, ent.renderfx,
                                     cent->gent->weaponModel[saberNum],
                                     ent.origin, tempAngles, saberNum,
                                     bladeNum);
                  // CG_AddSaberBlades( cent, ent.renderfx, ent.origin,
                  // tempAngles, saberNum );
            ...
            // if ( cent->gent->client->ps.saberEventFlags&SEF_INWATER )
            {
              CG_CheckSaberInWater(cent, cent, saberNum,
                                   cent->gent->weaponModel[saberNum],
                                   ent.origin, tempAngles);
            }
          }
          if (cent->currentState.weapon == WP_SABER &&
              (cent->gent->client->ps.saber[saberNum].blade[bladeNum].length >
      ...
      if (cent->gent->client->ps.dualSabers) {
        if (cent->gent->client->ps.saber[0].Length() > 0.0f &&
            !cent->gent->client->ps.saberInFlight) {
          if (cent->gent->client->ps.saber[0].numBlades >
              2) { // add blended light
            CG_DoSaberLight(&cent->gent->client->ps.saber[0]);
          }
        }
        if (cent->gent->client->ps.saber[1].Length() > 0.0f) {
          if (cent->gent->client->ps.saber[1].numBlades >
              2) { // add blended light
    ...
    if (cent->currentState.number != 0 || cg.renderingThirdPerson ||
      ...
      */
      if (cent->gent->headBolt == -1) { // no headBolt
        ...
        // FIXME: if head is missing, we should let the dismembered head set our
        // eyePoint...
        gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                               cent->gent->headBolt, &boltMatrix, tempAngles,
                               ent.origin, cg.time, cgs.model_draw,
                               cent->currentState.modelScale);
        gi.G2API_GiveMeVectorFromMatrix(
            boltMatrix, ORIGIN, cent->gent->client->renderInfo.eyePoint);
      ...
      if (cent->gent->chestBolt >= 0) {
        gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                               cent->gent->chestBolt, &boltMatrix, tempAngles,
                               ent.origin, cg.time, cgs.model_draw,
                               cent->currentState.modelScale);
        gi.G2API_GiveMeVectorFromMatrix(
            boltMatrix, ORIGIN, cent->gent->client->renderInfo.torsoPoint);
      ...
      if (cent->gent->crotchBolt >= 0) {
        gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                               cent->gent->crotchBolt, &boltMatrix, tempAngles,
                               ent.origin, cg.time, cgs.model_draw,
                               cent->currentState.modelScale);
        gi.G2API_GiveMeVectorFromMatrix(
            boltMatrix, ORIGIN, cent->gent->client->renderInfo.crotchPoint);
      } else {
        VectorCopy(ent.origin, cent->gent->client->renderInfo.crotchPoint);
      }
      // NOTE: these are used for any case where an NPC fires and the next shot
      // needs to come out 		of a new barrel/point.  That way the muzzleflash will
      //draw on the old barrel/point correctly NOTE: I'm only doing this for the
      // saboteur right now - AT-STs might need this... others?
      vec3_t oldMP = {0, 0, 0};
      vec3_t oldMD = {0, 0, 0};
      ...
      if (!calcedMp) {
        if (cent->gent && cent->gent->client &&
            cent->gent->client->NPC_class ==
                CLASS_ATST) { // FIXME: different for the three different weapon
                              // positions
          mdxaBone_t boltMatrix;
          int bolt;
          entityState_t *es;
          ...
          gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                                 bolt, &boltMatrix, tempAngles, ent.origin,
                                 cg.time, cgs.model_draw,
                                 cent->currentState.modelScale);

          // work the matrix axis stuff into the original axis and origins used.
          gi.G2API_GiveMeVectorFromMatrix(
              boltMatrix, ORIGIN, cent->gent->client->renderInfo.muzzlePoint);
          gi.G2API_GiveMeVectorFromMatrix(
              boltMatrix, NEGATIVE_Y, cent->gent->client->renderInfo.muzzleDir);
        } else if (cent->gent && cent->gent->client &&
          ...
          {
            if (cent->gent->alt_fire) { // fire from the lower barrel (not that
                                        // anyone will ever notice this, but...)
          ...
          if (bolt == -1) {
            VectorCopy(ent.origin, cent->gent->client->renderInfo.muzzlePoint);
            AngleVectors(tempAngles, cent->gent->client->renderInfo.muzzleDir,
                         NULL, NULL);
          } else {
            gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
                                   bolt, &boltMatrix, tempAngles, ent.origin,
        ...
        // Set the Vehicle Muzzle Point and Direction.
                 ...
                 cent->gent->client->NPC_class == CLASS_VEHICLE) {
          // Get the Position and Direction of the Tag and use that as our
          // Muzzles Properties.
          mdxaBone_t boltMatrix;
          vec3_t velocity;
          ...
          for (int i = 0; i < MAX_VEHICLE_MUZZLES; i++) {
            if (cent->gent->m_pVehicle->m_iMuzzleTag[i] != -1) {
              gi.G2API_GetBoltMatrix(
                  cent->gent->ghoul2, cent->gent->playerModel,
                  cent->gent->m_pVehicle->m_iMuzzleTag[i], &boltMatrix,
              ...
              VectorMA(cent->gent->m_pVehicle->m_Muzzles[i].m_vMuzzlePos,
                       0.075f, velocity,
                       cent->gent->m_pVehicle->m_Muzzles[i].m_vMuzzlePos);
            } else {
              break;
                   ...
                   */
                   && cent->gent->s.weapon ==
                          WP_BLASTER_PISTOL       // using blaster pistol
                   && cent->gent->weaponModel[1]) // one in each hand
        {
          ...
          if (cent->muzzleFlashTime > 0 && wData &&
              !(cent->currentState.eFlags &
                EF_LOCKED_TO_WEAPON)) { // we need to get both muzzles since
                                        // we're toggling and we fired recently
            getBoth = qtrue;
            oldOne = (cent->gent->count) ? 0 : 1;
          }
          if ((cent->gent->weaponModel[cent->gent->count] != -1) &&
              (cent->gent->ghoul2.size() >
               cent->gent->weaponModel[cent->gent->count]) &&
              (cent->gent->ghoul2[cent->gent->weaponModel[cent->gent->count]]
                   .mModelindex != -1)) { // get whichever one we're using now
            mdxaBone_t boltMatrix;
            // figure out where the actual model muzzle is
            gi.G2API_GetBoltMatrix(
                cent->gent->ghoul2, cent->gent->weaponModel[cent->gent->count],
                0, &boltMatrix, tempAngles, ent.origin, cg.time, cgs.model_draw,
                cent->currentState.modelScale);
            // work the matrix axis stuff into the original axis and origins
            // used.
            gi.G2API_GiveMeVectorFromMatrix(
          ...
          if (getBoth &&
              ...
              && (cent->gent->ghoul2.size() >
                  cent->gent
                      ->weaponModel[oldOne]) // have a valid ghoul model index
              && (cent->gent->ghoul2[cent->gent->weaponModel[oldOne]]
                      .mModelindex != -1)) // model exists and was loaded
          { // saboteur commando, toggle the muzzle point back and forth between
            // the two pistols each time he fires
            mdxaBone_t boltMatrix;
            // figure out where the actual model muzzle is
            gi.G2API_GetBoltMatrix(
                cent->gent->ghoul2, cent->gent->weaponModel[oldOne], 0,
                &boltMatrix, tempAngles, ent.origin, cg.time, cgs.model_draw,
                cent->currentState.modelScale);
            // work the matrix axis stuff into the original axis and origins
            // used.
            ...
            gi.G2API_GiveMeVectorFromMatrix(boltMatrix, NEGATIVE_Y, oldMD);
                   ...
                   (cent->gent->ghoul2[cent->gent->weaponModel[0]]
                        .mModelindex != -1)) {
          mdxaBone_t boltMatrix;
          // figure out where the actual model muzzle is
          gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->weaponModel[0],
                                 0, &boltMatrix, tempAngles, ent.origin,
                                 cg.time, cgs.model_draw,
                                 cent->currentState.modelScale);
          // work the matrix axis stuff into the original axis and origins used.
          gi.G2API_GiveMeVectorFromMatrix(
          ...
          VectorCopy(cent->gent->client->renderInfo.eyePoint,
                     cent->gent->client->renderInfo.muzzlePoint);
          AngleVectors(cent->gent->client->renderInfo.eyeAngles,
                       cent->gent->client->renderInfo.muzzleDir, NULL, NULL);
        }
        cent->gent->client->renderInfo.mPCalcTime = cg.time;
      }
      ...
      if (cent->gent && cent->gent->client &&
          cent->gent->client->NPC_class == CLASS_VEHICLE) {
        for (int i = 0; i < MAX_VEHICLE_MUZZLES; i++) {
          /*if ( cent->gent->m_pVehicle->m_pVehicleInfo->weap1ID ==
          cent->gent->m_pVehicle->m_pVehicleInfo->weapMuzzle[i] )
          {
                  iDelay = cent->gent->m_pVehicle->m_pVehicleInfo->weap1Delay;
          }
          else if ( cent->gent->m_pVehicle->m_pVehicleInfo->weap2ID ==
          cent->gent->m_pVehicle->m_pVehicleInfo->weapMuzzle[i] )
          {
                  iDelay = cent->gent->m_pVehicle->m_pVehicleInfo->weap2Delay;
          }

          if ( cent->gent->m_pVehicle->m_Muzzles[i].m_iMuzzleWait - cg.time > (
          iDelay - 500 ) )*/

          if (cent->gent->m_pVehicle->m_Muzzles[i].m_bFired) {
            const char *effect =
                &weaponData[cent->gent->m_pVehicle->m_pVehicleInfo
                                ->weapMuzzle[i]]
                     .mMuzzleEffect[0];
            if (effect) {
              theFxScheduler.PlayEffect(
                  effect, cent->gent->m_pVehicle->m_Muzzles[i].m_vMuzzlePos,
                  cent->gent->m_pVehicle->m_Muzzles[i].m_vMuzzleDir);
            }
      ...
      // Pick the right effect for the type of weapon we are, defaults to no
      // effect unless explicitly specified
      else if (cent->muzzleFlashTime > 0 && wData &&
        ...
        if (/*( cent->currentState.eFlags & EF_FIRING ||
          ...
          if (cent->gent && cent->gent->NPC) {
            if (!VectorCompare(oldMP, vec3_origin) &&
                !VectorCompare(
                    oldMD,
                    vec3_origin)) { // we have an old muzzlePoint we want to use
              theFxScheduler.PlayEffect(effect, oldMP, oldMD);
            } else { // use the current one
              theFxScheduler.PlayEffect(
                  effect, cent->gent->client->renderInfo.muzzlePoint,
                  cent->gent->client->renderInfo.muzzleDir);
            }
          } else {
      ...
      /*
      if ( cent->gent->NPC && ( cent->gent->NPC->confusionTime > cg.time ||
      cent->gent->NPC->charmedTime > cg.time || cent->gent->NPC->controlledTime
      > cg.time) )
      {// we are currently confused, so play an effect at the headBolt position
              if ( TIMER_Done( cent->gent, "confusionEffectDebounce" ) )
              {//ARGH!!!
                      theFxScheduler.PlayEffect( cgs.effects.forceConfusion,
      cent->gent->client->renderInfo.eyePoint ); TIMER_Set( cent->gent,
      "confusionEffectDebounce", 1000 );
              }
      }
      */

      if (cent->gent->client &&
          cent->gent->forcePushTime > cg.time) { // being pushed
      ...
      if ((cent->gent->client->ps.forcePowersActive &
           (1 << FP_GRIP))) { // doing the gripping
        // FIXME: effect?
      ...
      if (cent->gent->client && cent->gent->client->ps.powerups[PW_SHOCKED] >
                                    cg.time) { // being electrocuted
        CG_ForceElectrocution(cent, ent.origin, tempAngles,
                              cgs.media.boltShader);
      }

      if (cent->gent->client->ps.eFlags & EF_FORCE_DRAINED ||
          (cent->currentState.powerups & (1 << PW_DRAINED))) { // being drained
        // do red electricity lines off them and red drain shell on them
        CG_ForceElectrocution(cent, ent.origin, tempAngles,
                              cgs.media.drainShader, qtrue);
      }

      if (cent->gent->client->ps.forcePowersActive &
          (1 << FP_LIGHTNING)) { // doing the electrocuting
        // FIXME: if the target is absorbing or blocking lightning w/saber, draw
        // a beam from my hand to his (hand?chest?saber?)
        vec3_t tAng, fxDir;
        VectorCopy(cent->lerpAngles, tAng);
        if (cent->gent->client->ps.forcePowerLevel[FP_LIGHTNING] >
            FORCE_LEVEL_2) { // arc
          vec3_t fxAxis[3];
      ...
      if ((cent->gent->client->ps.eFlags & EF_POWERING_ROSH)) {
        ...
        AngleVectors(tAng, fxDir, NULL, NULL);
        theFxScheduler.PlayEffect(
            cgs.effects.forceDrain, cent->gent->client->renderInfo.handLPoint,
            fxDir); // theFxScheduler.RegisterEffect( "force/dr1" )
      }

      if (cent->gent->client->ps.forcePowersActive & (1 << FP_DRAIN) &&
          cent->gent->client->ps.forceDrainEntityNum >=
              ENTITYNUM_WORLD) { // doing the draining and not on a single
                                 // person
        vec3_t tAng, fxDir;
        VectorCopy(cent->lerpAngles, tAng);
        if (cent->gent->client->ps.forcePowerLevel[FP_DRAIN] >
            FORCE_LEVEL_2) { // arc
          vec3_t fxAxis[3];
          AnglesToAxis(tAng, fxAxis);
          theFxScheduler.PlayEffect(cgs.effects.forceDrainWide,
                                    cent->gent->client->renderInfo.handLPoint,
                                    fxAxis);
        } else { // line
          AngleVectors(tAng, fxDir, NULL, NULL);
          theFxScheduler.PlayEffect(cgs.effects.forceDrain,
                                    cent->gent->client->renderInfo.handLPoint,
                                    fxDir);
        }
      }
      ...
      if ((cent->currentState.eFlags &
           EF_SPOTLIGHT)) { // FIXME: player's view should glare/flare if look
                            // at this... maybe build into the effect?
        // hack for the spotlight
        vec3_t org, eyeFwd;

        AngleVectors(cent->gent->client->renderInfo.eyeAngles, eyeFwd, NULL,
                     NULL);
        theFxScheduler.PlayEffect("rockettrooper/light_cone",
                                  cent->gent->client->renderInfo.eyePoint,
                                  eyeFwd);
        // stay a bit back from the server-side's trace impact point...this may
        // not be enough?
        VectorMA(cent->gent->client->renderInfo.eyePoint, cent->gent->speed - 5,
                 eyeFwd, org);
    ...
    // CG_RunLerpFrame( ci, &cent->pe.torso, cent->gent->client->ps.torsoAnim,
    // cent->gent->client->renderInfo.torsoFpsMod, cent->gent->s.number );
    if (cent->gent && cent->gent->client &&
        cent->gent->client->ps.weapon == WP_SABER) {
      ...
      if (!PM_KickingAnim(cent->gent->client->ps.torsoAnim) ||
          cent->gent->client->ps.torsoAnim ==
              BOTH_A7_KICK_S) { // not kicking (unless it's the spinning kick)
        if (cg_timescale.value < 1.0f &&
          ...
          if (cent->gent->client->ps.saberDamageDebounceTime <= cg.time) {
            extern void WP_SabersDamageTrace(gentity_t * ent,
                                             qboolean noEffects);
            ...
            cent->gent->client->ps.saberDamageDebounceTime =
                cg.time + floor((float)wait * cg_timescale.value);
          }
        }
      }
    }
    ...
    */
    ...
    // FIXME: pass in the axis/angles offset between the tag_torso and the
    // tag_head?
    ...
    // NB: Also plays keyframed animSounds (Bob- hope you dont mind, I was here
    // late and at about 5:30 Am i needed to do something to keep me awake and i
    // figured you wouldn't mind- you might want to check it, though, to make
    // sure I wasn't smoking crack and missed something important, it is pretty
    // late and I'm getting pretty close to being up for 24 hours here, so i
    // wouldn't doubt if I might have messed something up, but i tested it and
    // it looked right.... noticed in old code base i was doing it wrong too,
    // whic            h explains why I was getting so many damn sounds all the
    // time!  I had to lower the probabilities because it seemed like i was
    // getting way too many sounds, and that was the problem!  Well, should be
    // fixed now I think...)
    ...
    if (cg.snap->ps.viewEntity > 0 &&
        ...
        cg.snap->ps.viewEntity ==
            cent->currentState
                .clientNum) { // player is in an entity camera view, ME
    ...
    // This seems to copy the scale to every piece attached, kinda cool, but
    // doesn't allow the body to be scaled up without scaling a bolt on or
    // whatnot... Only apply scale if it's not 100% scale...
    ...
    if (cent->currentState.modelScale[2] != 0.0f) {
      ...
      legs.nonNormalizedAxes = qtrue;
      if (!staticScale) {
        // FIXME:? need to know actual height of leg model bottom to origin, not
        // hardcoded
    ...
    if (torso.hModel) {
      ...
      vectoangles(tag_torso.axis[0],
                  cent->gent->client->renderInfo.torsoAngles);
      ...
      if (head.hModel) {
        ...
        vectoangles(tag_head.axis[0],
                    cent->gent->client->renderInfo.headAngles);
        ...
        if (!calcedMp) { // First person player's eyePoint and eyeAngles should
                         // be copies from cg.refdef...
          // Calc this client's eyepoint
          VectorCopy(head.origin, cent->gent->client->renderInfo.eyePoint);
          // race is gone, eyepoint should refer to the tag/bolt on the model...
          // if this breaks something let me know - dmv
          //	VectorMA( cent->gent->client->renderInfo.eyePoint,
          //CG_EyePointOfsForRace[cent->gent->client->race][1]*scaleFactor[2],
          //head.axis[2], cent->gent->client->renderInfo.eyePoint );//up
          //	VectorMA( cent->gent->client->renderInfo.eyePoint,
          //CG_EyePointOfsForRace[cent->gent->client->race][0]*scaleFactor[0],
          //head.axis[0], cent->gent->client->renderInfo.eyePoint );//forward
      ...
      if (gun.hModel) {
        ...
        // FIXME: allow it to be put anywhere and move this out of
        // if(torso.hModel) Will have to call CG_PositionRotatedEntityOnTag
        ...
        cent->gent->client->ps.saberInFlight ) )
                                {
                                        ...
                                        for ( int saberNum = 0; saberNum <
        numSabers; saberNum++ )
                                        {
                                                if (
        !cent->gent->client->ps.saber[saberNum].active
        ...
        cent->gent->client->ps.saber[saberNum].length > 0 )
                                                        {
                                                                if (
        cent->gent->client->ps.stats[STAT_HEALTH] <= 0 )
                                                                {//dead, didn't
        actively turn it off cent->gent->client->ps.saber[saberNum].length -=
        cent->gent->client->ps.saber[saberNum].lengthMax/10 * cg.frametime/100;
                                                                }
                                                                else
                                                                {//actively
        turned it off, shrink faster
                                                                        cent->gent->client->ps.saber[saberNum].length
        -= cent->gent->client->ps.saber[saberNum].lengthMax/3 *
        cg.frametime/100;
                                                                }
                                                        }
                                                        if (
        ...
        cent->gent->client->ps.saberInFlight )
                                                                {//play it on
        the saber cgi_S_UpdateEntityPosition(
        cent->gent->client->ps.saberEntityNum,
        g_entities[cent->gent->client->ps.saberEntityNum].currentOrigin );
                                                                        cgi_S_StartSound
        (NULL, cent->gent->client->ps.saberEntityNum, CHAN_AUTO,
        ...
        cgs.sound_precache[cent->gent->client->ps.saber[0].soundOn] ); #ifdef
        _IMMERSION cgi_FF_Start( cgi_FF_Register( "fffx/weapons/saber/saberon",
        FF_CHANNEL_WEAPON ), cent->currentState.number ); #endif // _IMMERSION
                                                                }
                                                        }
                                                        cent->gent->client->ps.saber[saberNum].length
        += cent->gent->client->ps.saber[saberNum].lengthMax/6 *
        cg.frametime/100;//= saber[saberNum].lengthMax; if (
        cent->gent->client->ps.saber[saberNum].length >
        cent->gent->client->ps.saber[saberNum].lengthMax )
                                                        {
                                                                cent->gent->client->ps.saber[saberNum].length
        ...
        !cent->gent->client->ps.saberInFlight )
                                                        {//holding the saber
        in-hand CG_AddSaberBlade( cent, cent, &gun, renderfx, 0, NULL, NULL );
                                                                calcedMp =
        qtrue;
                                                        }
                                                }
        ...
        */
        ...
        if (!calcedMp && !(cent->currentState.eFlags &
                           EF_LOCKED_TO_WEAPON)) { // Set the muzzle point
          orientation_t orientation;

          cgi_R_LerpTag(&orientation, weapon->weaponModel, gun.oldframe,
                        gun.frame, 1.0f - gun.backlerp, "tag_flash");
          ...
          //				VectorCopy( gun.axis[0],
          //cent->gent->client->renderInfo.muzzleDir ); 				VectorAdd( gun.axis[0],
          //orientation.axis[0], cent->gent->client->renderInfo.muzzleDir );
          //				VectorNormalize(
          //cent->gent->client->renderInfo.muzzleDir );

          cent->gent->client->renderInfo.mPCalcTime = cg.time;
          // Weapon wasn't firing anymore, so ditch any weapon associated
          // looping sounds.
  ...
  if (cent->currentState.number == 0 && cg.renderingThirdPerson) {
    ...
    if ((ps->weaponstate == WEAPON_CHARGING_ALT &&
      ...
      FX_AddSprite(cent->gent->client->renderInfo.muzzlePoint, NULL, NULL,
                   3.0f * val * scale, 0.0f, 0.7f, 0.7f, WHITE, WHITE,
                   Q_flrand(0.0f, 1.0f) * 360, 0.0f, 1.0f, shader,
                   FX_USE_ALPHA);
    }
  }
}
...
FIXME: We do not need to do this, we can remember the last anim and frame they
were on and coontinue from there.
===============
*/
void CG_ResetPlayerEntity(centity_t *cent) {
  ...
  if (cent->gent && cent->gent->ghoul2.size()) {
    if (cent->currentState.clientNum < MAX_CLIENTS) {
      ...
      CG_ClearLerpFrame(&cent->gent->client->clientInfo, &cent->pe.legs,
                        cent->currentState.legsAnim);
      CG_ClearLerpFrame(&cent->gent->client->clientInfo, &cent->pe.torso,
  ...
  // Removed by BTO (VV) - These values are crap anyway. Also changed below to
  // use lerp instead
