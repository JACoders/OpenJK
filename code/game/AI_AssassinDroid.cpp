...
void BubbleShield_TurnOn() {
  if (!BubbleShield_IsOn()) {
    ...
    gi.G2API_SetSurfaceOnOff(&NPC->ghoul2[NPC->playerModel], "force_shield",
                             TURN_ON);
  }
}
...
void BubbleShield_PushEnt(gentity_t *pushed, vec3_t smackDir) {
  G_Damage(pushed, NPC, NPC, smackDir, NPC->currentOrigin,
...
void BubbleShield_PushRadiusEnts() {
  ...
  for (int entIndex = 0; entIndex < numEnts; entIndex++) {
    ...
    if (NPC->enemy && NPCInfo->touchedByPlayer == NPC->enemy &&
        radiusEnts[entIndex] == NPC->enemy) {
      continue;
    }

    // Do The Vector Distance Test
    //-----------------------------
    VectorSubtract(radiusEnts[entIndex]->currentOrigin, NPC->currentOrigin,
                   smackDir);
...
void BubbleShield_Update() {
  ...
  if (NPC->client->ps.stats[STAT_ARMOR] > 100 &&
      TIMER_Done(NPC, "ShieldsDown")) {
    // Check On Timers To Raise And Lower Shields
    //--------------------------------------------
    if ((level.time - NPCInfo->enemyLastSeenTime) < 1000 &&
        TIMER_Done(NPC, "ShieldsUp")) {
      TIMER_Set(NPC, "ShieldsDown", 2000); // Drop Shields
      TIMER_Set(
          NPC, "ShieldsUp",
          Q_irand(4000, 5000)); // Then Bring Them Back Up For At Least 3 sec
    }
      ...
      NPC->client->renderInfo.customRGBA[0] =
          NPC->client->renderInfo.customRGBA[1] =
              NPC->client->renderInfo.customRGBA[2] =
                  NPC->client->renderInfo.customRGBA[3] =
