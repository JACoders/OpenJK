...
static QINLINE void SetSaberBoxSize(gentity_t *saberent) {
	...
	if ((level.time - owner->client->lastSaberStorageTime) > 200 ||
		(level.time - owner->client->saber[j].blade[k].storageTime) >
			100) { // it's been too long since we got a reliable point storage, so use the defaults and leave.
	...
	if (dualSabers || owner->client->saber[0].numBlades > 1) { // dual sabers or multi-blade saber
	...
	for (i = 0; i < 3; i++) {
		for (j = 0; j < MAX_SABERS; j++) {
			...
			if (dualSabers && owner->client->ps.saberHolstered == 1 && j == 1) { // this mother is holstered, get outta here.
				j++;
				continue;
			}
			for (k = 0; k < owner->client->saber[j].numBlades; k++) {
				...
				// VectorMA(owner->client->saber[j].blade[k].muzzlePoint, owner->client->saber[j].blade[k].lengthMax*0.5f,
				// owner->client->saber[j].blade[k].muzzleDir, saberOrg);
				VectorCopy(owner->client->saber[j].blade[k].muzzlePoint, saberOrg);
				VectorMA(owner->client->saber[j].blade[k].muzzlePoint, owner->client->saber[j].blade[k].lengthMax, owner->client->saber[j].blade[k].muzzleDir,
						 saberTip);
...
static QINLINE int G_GetAttackDamage(gentity_t *self, int minDmg, int maxDmg, float multPoint) {
	...
	float attackAnimLength = bgAllAnims[self->localAnimIndex].anims[self->client->ps.torsoAnim].numFrames *
							 fabs((float)(bgAllAnims[self->localAnimIndex].anims[self->client->ps.torsoAnim].frameLerp));
...
static QINLINE qboolean G_G2TraceCollide(trace_t *tr, vec3_t lastValidStart, vec3_t lastValidEnd, vec3_t traceMins,
										 vec3_t traceMaxs) { // Hit the ent with the normal trace, try the collision trace.
	G2Trace_t G2Trace;
	gentity_t *g2Hit;
	vec3_t angles;
	int tN = 0;
	...
	if (traceMins[0] || traceMins[1] || traceMins[2] || traceMaxs[0] || traceMaxs[1] || traceMaxs[2]) {
...
static QINLINE void G_BuildSaberFaces(vec3_t base, vec3_t tip, float radius, vec3_t fwd, vec3_t right, int *fNum, saberFace_t **fList) {
...
static QINLINE qboolean G_SaberFaceCollisionCheck(int fNum, saberFace_t *fList, vec3_t atkStart, vec3_t atkEnd, vec3_t atkMins, vec3_t atkMaxs,
												  vec3_t impactPoint) {
...
static QINLINE qboolean CheckSaberDamage(gentity_t *self, int rSaberNum, int rBladeNum, vec3_t saberStart, vec3_t saberEnd, qboolean doInterpolate, int trMask,
	...
	while (!saberTraceDone) {
		if (doInterpolate && !d_saberSPStyleDamage.integer) { // This didn't quite work out like I hoped. But it's better than nothing. Sort of.
	...
	if (self->client->ps.saberAttackWound < level.time &&
		...
		if (d_saberSPStyleDamage.integer) {
			...
			if (self->client->ps.saberInFlight) {
				...
				if (!saberEnt || !saberEnt->s.saberInFlight) { // does less damage on the way back
					fDmg = 1.0f;
				...
				if (g_saberRealisticCombat.integer) {
					...
					if (self->client->ps.torsoAnim == BOTH_SPINATTACK6 || self->client->ps.torsoAnim == BOTH_SPINATTACK7) { // too easy to do, lower damage
						fDmg = 2.5f;
					} else {
			...
			if (g_saberRealisticCombat.integer > 1) {	  // always do damage, and lots of it
				if (g_saberRealisticCombat.integer > 2) { // always do damage, and lots of it
					fDmg = 25.0f;
				} else if (fDmg > 0.1f) { // only do super damage if we would have done damage according to normal rules
					fDmg = 25.0f;
				}
			}
			/*
			...
			*/
			if (level.gametype != GT_DUEL && level.gametype != GT_POWERDUEL && level.gametype != GT_SIEGE) { // in faster-paced games, sabers do more damage
				fDmg *= 2.0f;
			}
			if (fDmg) { // the longer the trace, the more damage it does
				...
				} else { // fractional hit, the sooner you hit in the trace, the more damage you did
					dmg = ceil(fDmg * traceLength * (1.0f - tr.fraction) * 0.1f *
							   0.33f); //(1.0f-tr.fraction) isn't really accurate, but kind of simulates what we have in SP
				}
#ifdef DEBUG_SABER_BOX
			...
			*/
			...
			if (self->client->ps.fd.saberAnimLevel == SS_STAFF || self->client->ps.fd.saberAnimLevel == SS_DUAL) {
				if (saberInSpecial) {
					...
					if (self->client->ps.saberMove == LS_SPINATTACK ||
						self->client->ps.saberMove ==
							LS_SPINATTACK_DUAL) { // these attacks are long and have the potential to hit a lot so they will do less damage.
						dmg = 10;
					} else {
						if (BG_KickingAnim(self->client->ps.legsAnim) ||
							...
							dmg = 2;
						} else if (BG_SaberInKata(self->client->ps.saberMove)) { // special kata move
							if (self->client->ps.fd.saberAnimLevel == SS_DUAL) { // this is the nasty saber twirl, do big damage cause it makes you vulnerable
								dmg = 90;
							} else { // staff kata
				...
				} else { // otherwise we'll ramp up to 70 I guess, for both dual and staff
				...
				if (saberInSpecial && (self->client->ps.saberMove == LS_A_FLIP_STAB ||
									   self->client->ps.saberMove == LS_A_FLIP_SLASH)) { // a well-timed hit with this can do a full 85
			...
			attackStr = self->client->ps.fd.saberAnimLevel;
			   ...
			   self->client->ps.saberIdleWound <
				   level.time) { // just touching, do minimal damage and only check for it every 200ms (mainly to cut down on network traffic for hit events)
		if ((self->client->saber[0].saberFlags2 & SFL2_NO_IDLE_EFFECT)) { // no idle damage or effects
	...
	if (dmg > SABER_NONATTACK_DAMAGE && level.gametype == GT_SIEGE && self->client->siegeClass != -1 &&
		(bgSiegeClasses[self->client->siegeClass].classflags &
		 (1 << CFL_MORESABERDMG))) { // this class is flagged to do extra saber damage. I guess 2x will do for now.
		dmg *= 2;
	}
	...
	if (tr.entityNum == ENTITYNUM_WORLD || g_entities[tr.entityNum].s.eType == ET_TERRAIN) { // register this as a wall hit for jedi AI
	...
	if ((tr.fraction != 1 || tr.startsolid) && g_entities[tr.entityNum].takedamage &&
		...
		if (!d_saberSPStyleDamage.integer // let's trying making blocks have to be blocked by a saber
			...
			WP_SaberCanBlock(&g_entities[tr.entityNum], tr.endpos, 0, MOD_SABER, qfalse,
							 attackStr)) { // hit a client who blocked the attack (fake: didn't actually hit their saber)
			if (dmg <= SABER_NONATTACK_DAMAGE) {
				self->client->ps.saberIdleWound = level.time + g_saberDmgDelay_Idle.integer;
			}
			saberDoClashEffect = qtrue;
			...
			if (dmg > SABER_NONATTACK_DAMAGE) {
				...
				if ((g_entities[tr.entityNum].client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE] - self->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE]) > 1 &&
					Q_irand(1, 10) < lockFactor * 2) { // Just got blocked by someone with a decently higher attack level, so enter into a lock (where they have
			...
			*/
			...
			if (!d_saberSPStyleDamage.integer) {
				if (g_entities[tr.entityNum].client &&
					g_entities[tr.entityNum].client->ps.weapon ==
						WP_SABER) {					  // for jedi using the saber, half the damage (this comes with the increased default dmg debounce time)
			...
			if (self->s.eType == ET_NPC && g_entities[tr.entityNum].client &&
				self->client->playerTeam ==
					g_entities[tr.entityNum].client->playerTeam) { // Oops. Since he's an NPC, we'll be forgiving and cut the damage down.
				dmg *= 0.2f;
			}
			...
			if (!PM_SaberInParry(otherOwner->client->ps.saberMove) && !PM_SaberInBrokenParry(otherOwner->client->ps.saberMove) &&
				...
				if (crushTheParry && PM_SaberInParry(G_GetParryForBlock(
										 otherOwner->client->ps.saberBlocked))) { // This means that the attack actually hit our saber, and we went to block it.
																				  // But, one of the above cases says we actually can't. So we will be smashed
...
void G_SPSaberDamageTraceLerped(gentity_t *self, int saberNum, int bladeNum, vec3_t baseNew, vec3_t endNew, int clipmask) {
	...
	if (VectorCompare2(baseOld, baseNew) && VectorCompare2(endOld, endNew)) { // no diff
		...
		if (fabs(curDirFrac) < 1.0f - MAX_SABER_SWING_INC) { // the saber blade spun more than 33 degrees since the last damage trace
...
static QINLINE void saberCheckRadiusDamage(gentity_t *saberent,
										   int returning) { // we're going to cheat and damage players within the saber's radius, just for the sake of doing
															// things more "efficiently" (and because the saber entity has no server g2 instance)
	int i = 0;
	int dist = 0;
	gentity_t *ent;
...
void saberKnockDown(gentity_t *saberent, gentity_t *saberOwner, gentity_t *other) {
	...
	//(normally at the player's right hand) could result in the saber being stuck
	// in the the map and then freaking out.
	trap->Trace(&tr, saberent->r.currentOrigin, saberent->r.mins, saberent->r.maxs, saberent->r.currentOrigin, saberent->s.number, saberent->clipmask, qfalse,
				0, 0);
	if (tr.startsolid || tr.fraction != 1) { // bad position, try popping our origin up a bit
...
static void G_GrabSomeMofos(gentity_t *self) {
	...
	if (trace.fraction != 1.0f && trace.entityNum < ENTITYNUM_WORLD) {
		...
		if (grabbed->inuse && (grabbed->s.eType == ET_PLAYER || grabbed->s.eType == ET_NPC) && grabbed->client && grabbed->health > 0 &&
			G_CanBeEnemy(self, grabbed) && G_PrettyCloseIGuess(grabbed->client->ps.origin[2], self->client->ps.origin[2], 4.0f) &&
...
void WP_SaberPositionUpdate(
	...
	if (self && self->inuse && self->client && self->client->saberCycleQueue &&
		(self->client->ps.weaponTime <= 0 ||
		 self->health < 1)) { // we cycled attack levels while we were busy, so update now that we aren't (even if that means we're dead)
	...
	if (self->health < 1) { // we don't want to waste precious CPU time calculating saber positions for corpses. But we want to avoid the saber ent position
							// lagging on spawn, so..
	...
	if (!BG_SabersOff(&self->client->ps)) {
		...
		while (rSaberNum < MAX_SABERS) {
			...
			*/
			...
			if (rSaberNum > 0 && self->client->saber[1].model[0] && self->client->ps.saberHolstered == 1) { // don't to saber 2 if it's off
				break;
			}
			rBladeNum = 0;
