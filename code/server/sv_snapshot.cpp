...
static void SV_AddEntToSnapshot(svEntity_t *svEnt, gentity_t *gEnt, snapshotEntityNumbers_t *eNums) {
	...
	if (sv.snapshotCounter & 1 &&
		eNums->numSnapshotEntities ==
			svs.numSnapshotEntities - 1) { // we're full, and about to wrap around and stomp ents, so half the time send the first set without stomping.
		return;
	}
...
qboolean SV_PlayerCanSeeEnt(gentity_t *ent, int sightLevel) { // return true if this ent is in view
...
static void SV_AddEntitiesVisibleFromPoint(vec3_t origin, clientSnapshot_t *frame, snapshotEntityNumbers_t *eNums, qboolean portal) {
...
#ifndef JK2_MODE
	if (!portal) { // not if this if through a portal...???  James said to do this...
	...
	for (e = 0; e < ge->num_entities; e++) {
		...
		if (ent->s.eFlags & EF_PERMANENT) { // he's permanent, so don't send him down!
			continue;
		}
...
#ifndef JK2_MODE
		if (sightOn) {														  // force sight is on, sees through portals, so draw them always if in radius
			if (SV_PlayerCanSeeEnt(ent, frame->ps.forcePowerLevel[FP_SEE])) { // entity is visible
				SV_AddEntToSnapshot(svEnt, ent, eNums);
				continue;
			}
		}
