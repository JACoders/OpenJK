...
void ClampVec(vec3_t dat, byte *res) {
  ...
  // clamp all vec values, then multiply the normalized values by 255 to
  // maximize the result
  for (int i = 0; i < 3; i++) {
...
void CParticle::Die() {
  if (mFlags & FX_DEATH_RUNS_FX && !(mFlags & FX_KILL_ON_IMPACT)) {
    ...
    VectorSet(norm, Q_flrand(-1.0f, 1.0f), Q_flrand(-1.0f, 1.0f),
              Q_flrand(-1.0f, 1.0f));
...
bool CParticle::Update() {
  ...
  if (mFlags & FX_RELATIVE) {
    if (mClientID < 0 ||
        mClientID >= ENTITYNUM_WORLD) { // we are somehow not bolted even though
                                        // the flag is on?
      return false;
    }
    ...
    if (mModelNum >= 0 && mBoltNum >= 0) // bolt style
    {
      ...
      if (!theFxHelper.GetOriginAxisFromBolt(cent, mModelNum, mBoltNum, org,
...
bool CParticle::UpdateOrigin() {
  ...
  new_origin[0] =
      mOrigin1[0] + theFxHelper.mFloatFrameTime * mVel[0]; // + time2 * mVel[0];
  new_origin[1] =
      mOrigin1[1] + theFxHelper.mFloatFrameTime * mVel[1]; // + time2 * mVel[1];
  new_origin[2] =
      mOrigin1[2] + theFxHelper.mFloatFrameTime * mVel[2]; // + time2 * mVel[2];
  ...
  if ((mFlags & FX_APPLY_PHYSICS)) {
    ...
    if ((mFlags & FX_EXPENSIVE_PHYSICS) && fx_expensivePhysics.integer) {
      ...
      solid = !!(CG_PointContents(new_origin, ENTITYNUM_WORLD) &
    ...
    if (solid) {
      ...
      if (mFlags & FX_USE_BBOX) {
        if (mFlags & FX_GHOUL2_TRACE) {
          theFxHelper.G2Trace(&trace, mOrigin1, mMin, mMax, new_origin,
                              ENTITYNUM_NONE, (MASK_SHOT | CONTENTS_WATER));
        } else {
        ...
        if (mFlags & FX_GHOUL2_TRACE) {
          theFxHelper.G2Trace(&trace, mOrigin1, NULL, NULL, new_origin,
                              ENTITYNUM_NONE, (MASK_SHOT | CONTENTS_WATER));
        } else {
      ...
      if (trace.startsolid || trace.allsolid || trace.fraction == 1.0) {
        ...
        VectorMA(mVel, theFxHelper.mFloatFrameTime * trace.fraction, mAccel,
                 mVel);
...
void CParticle::UpdateRGB() {
  ...
  VectorMA(res, (1.0f - perc1), mRGBEnd,
           mRefEnt.angles); // angles is a temp storage, will get clamped to a
                            // byte in the UpdateAlpha section
}
...
bool CLine::Update() {
  ...
  if (mFlags & FX_RELATIVE) {
    ...
    if (mModelNum >= 0 && mBoltNum >= 0) // bolt style
    {
      ...
      if (!theFxHelper.GetOriginAxisFromBolt(
              cent, mModelNum, mBoltNum, mOrigin1, ax)) { // could not get bolt
        return false;
      }
    } else { // fixme change this to bolt style...
      // Get our current position and direction
    ...
    if (mFlags & FX_APPLY_PHYSICS) {
      ...
      theFxHelper.Trace(&trace, mOrigin1, NULL, NULL, end, mClientID,
                        MASK_SHOT);
...
bool CElectricity::Update() {
  ...
  if (mFlags &
      FX_RELATIVE) { // add mOrgOffset to bolt position and store in mOrigin1
    if (mClientID < 0 ||
        mClientID >= ENTITYNUM_WORLD) { // we are somehow not bolted even though
                                        // the flag is on?
      return false;
...
#define TRAIL_RATE 8 // we "think" at about a 60hz rate
    ...
    for (t = mOldTime; t <= theFxHelper.mTime; t += TRAIL_RATE) {
      ...
      if ((mFlags & FX_APPLY_PHYSICS)) {
        ...
        if ((mFlags & FX_EXPENSIVE_PHYSICS) && fx_expensivePhysics.integer) {
          solid =
              true; // by setting this to true, we force a real trace to happen
        } else {
          // if this returns solid, we need to do a trace
        ...
        if (solid) {
          ...
          if (mFlags & FX_USE_BBOX) {
            theFxHelper.Trace(&trace, mOldOrigin, mMin, mMax, org, -1,
                              MASK_SHOT);
          } else {
            theFxHelper.Trace(&trace, mOldOrigin, NULL, NULL, org, -1,
                              MASK_SHOT);
          }
...
bool CEmitter::Update() {
  ...
  */
  ...
  if ((mTimeStart < theFxHelper.mTime) &&
      UpdateOrigin() == false) { // we are marked for death
    return false;
  }
  ...
  UpdateAngles();
  UpdateSize();
  //	UpdateRGB();	// had wanted to do something slick whereby an emitted
  //effect could somehow inherit these
...
bool CBezier::Update(void) {
  ...
  */
  ...
  for (int i = 0; i < 3; i++) {
    mControl1[i] =
        mControl1[i] + ftime * mControl1Vel[i] + time2 * mControl1Vel[i];
    mControl2[i] =
        mControl2[i] + ftime * mControl2Vel[i] + time2 * mControl2Vel[i];
  }
...
//----------------------------
void CBezier::DrawSegment(vec3_t start, vec3_t end, float texcoord1,
                          float texcoord2) {
...
void CBezier::Draw(void) {
  ...
  // Calculate the texture coords so the texture can stretch along the whole
  // bezier
  //	if ( mFlags & FXF_WRAP )
  //	{
  //		tex = m_stScale / 1.0f;
...
void CFlash::Draw(void) {
  ...
  VectorMA(mRefEnt.origin, FLASH_DISTANCE_FROM_VIEWER, cg.refdef.viewaxis[0],
           mRefEnt.origin);

  // This is assuming that the screen is wider than it is tall.
  mRefEnt.radius =
      FLASH_DISTANCE_FROM_VIEWER * tan(DEG2RAD(cg.refdef.fov_x * 0.5f));
