...
typedef struct SE_Entry_s {
	...
	std::string
		m_strDebug; // english and/or "#same", used for debugging only. Also prefixed by "SE:" to show which strings go through StringEd (ie aren't hardwired)
	int m_iFlags;
...
class CStringEdPackage {
  ...
  public:
	...
	SE_BOOL EndMarkerFoundDuringParse(void) { return m_bEndMarkerFound_ParseOnly; }
...
static char *CopeWithDumbStringData(const char *psSentence, const char *psThisLanguage) {
	...
	//
	if (!Q_stricmp(psThisLanguage, "ENGLISH") || !Q_stricmp(psThisLanguage, "FRENCH") || !Q_stricmp(psThisLanguage, "GERMAN") ||
		!Q_stricmp(psThisLanguage, "ITALIAN") || !Q_stricmp(psThisLanguage, "SPANISH") || !Q_stricmp(psThisLanguage, "POLISH") ||
...
//
const char *CStringEdPackage::GetCurrentReference_ParseOnly(void) { return m_strCurrentEntryRef_ParseOnly.c_str(); }
...
// returns flag bitmask (eg 00000010b), else 0 for not found
//
int SE_GetFlagMask(const char *psFlagName) { return TheStringPackage.GetFlagMask(psFlagName); }
