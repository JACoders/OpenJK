...
extern void CG_LoadCISounds(clientInfo_t *ci,
                            qboolean modelloaded); // cg_players.c
...
void CG_PrecacheSiegeObjectiveAssetsForTeam(int myTeam) {
  ...
  if (BG_SiegeGetValueGroup(siege_info, teamstr, cgParseObjectives)) {
    int i = 1;
    while (i < 32) { // eh, just try 32 I guess
...
void CG_PrecachePlayersForSiegeTeam(int team) {
  ...
  if (!stm) { // invalid team/no theme for team?
    return;
  }
  ...
  while (i < stm->numClasses) {
    ...
    if (scl->forcedModel[0]) {
      ...
      trap->R_RegisterModel(
          va("models/players/%s/model.glm", scl->forcedModel));
      if (scl->forcedSkin[0]) {
...
void CG_InitSiegeMode(void) {
  ...
  if (BG_SiegeGetValueGroup(siege_info, "Teams", teams)) {
    ...
    if (team1[0] == '@') { // it's a damn stringed reference.
      char b[256];
    ...
    if (team2[0] == '@') { // it's a damn stringed reference.
      char b[256];
  ...
  while (i <= SIEGETEAM_TEAM2) {
    ...
    while (j < sTeam->numClasses) {
      ...
      if (cl->forcedModel[0]) { // This class has a forced model, so precache
                                // it.
        trap->R_RegisterModel(
            va("models/players/%s/model.glm", cl->forcedModel));

        if (cl->forcedSkin[0]) { // also has a forced skin, precache it.
          char *useSkinName;

          if (strchr(cl->forcedSkin, '|')) { // three part skin
            useSkinName =
                va("models/players/%s/|%s", cl->forcedModel, cl->forcedSkin);
          } else {
            useSkinName = va("models/players/%s/model_%s.skin", cl->forcedModel,
                             cl->forcedSkin);
          }
...
static char QINLINE *CG_SiegeObjectiveBuffer(int team, int objective) {
  ...
  if (BG_SiegeGetValueGroup(siege_info, teamstr,
                            cgParseObjectives)) { // found the team group
    if (BG_SiegeGetValueGroup(cgParseObjectives, va("Objective%i", objective),
                              buf)) { // found the objective group
      return buf;
    }
  }
...
void CG_ParseSiegeObjectiveStatus(const char *str) {
  ...
  while (str[i]) {
    if (str[i] == '|') { // switch over to team2, this is the next section
      ...
      if (str[i] == '1') { // it's completed
        trap->Cvar_Set(cvarName, "1");
      } else { // otherwise assume it is not
        trap->Cvar_Set(cvarName, "0");
      }

      s = CG_SiegeObjectiveBuffer(team, objectiveNum);
      if (s && s[0]) { // now set the description and graphic cvars to by read
                       // by the menu
        char buffer[8192];
  ...
  if (cg.predictedPlayerState.persistant[PERS_TEAM] !=
      TEAM_SPECTATOR) { // update menu cvars
...
void CG_SiegeRoundOver(centity_t *ent, int won) {
  ...
  if (cg.snap) { // this should always be true, if it isn't though use the
                 // predicted ps as a fallback
    ps = &cg.snap->ps;
  } else {
  ...
  if (BG_SiegeGetValueGroup(siege_info, teamstr, cgParseObjectives)) {
    if (won == myTeam) {
      success =
          BG_SiegeGetPairedValue(cgParseObjectives, "wonround", appstring);
    } else {
      success =
          BG_SiegeGetPairedValue(cgParseObjectives, "lostround", appstring);
    }
...
void CG_SiegeGetObjectiveDescription(int team, int objective, char *buffer) {
  ...
  buffer[0] = 0; // set to 0 ahead of time in case we fail to find the objective
                 // group/name
  ...
  if (BG_SiegeGetValueGroup(siege_info, teamstr,
                            cgParseObjectives)) { // found the team group
    if (BG_SiegeGetValueGroup(cgParseObjectives, va("Objective%i", objective),
                              objectiveStr)) { // found the objective group
...
void CG_SiegeBriefingDisplay(int team, int dontshow) {
  ...
  if (useTeam != SIEGETEAM_TEAM1 &&
      useTeam != SIEGETEAM_TEAM2) { // This shouldn't be happening. But just
                                    // fall back to team 2 anyway.
    useTeam = SIEGETEAM_TEAM2;
  }

  trap->Cvar_Set(va("siege_primobj_inuse"), "0");

  while (i < 16) { // do up to 16 objectives I suppose
    ...
    properValue[0] = 0;
    trap->Cvar_VariableStringBuffer(va("team%i_objective%i", useTeam, i),
                                    properValue, 1024);
    if (primary) {
    ...
    trap->Cvar_VariableStringBuffer(
        va("team%i_objective%i_longdesc", useTeam, i), properValue, 1024);
    if (primary) {
    ...
    properValue[0] = 0;
    trap->Cvar_VariableStringBuffer(va("team%i_objective%i_gfx", useTeam, i),
                                    properValue, 1024);
    if (primary) {
    ...
    properValue[0] = 0;
    trap->Cvar_VariableStringBuffer(
        va("team%i_objective%i_mapicon", useTeam, i), properValue, 1024);
    if (primary) {
    ...
    properValue[0] = 0;
    trap->Cvar_VariableStringBuffer(va("team%i_objective%i_mappos", useTeam, i),
                                    properValue, 1024);
    if (primary) {
    ...
    if (objectiveDesc[0]) { // found a valid objective description
      if (primary) {
    ...
    } else { // didn't find one, so set the "inuse" cvar to 0 for the objective
             // and mark it non-complete.
      trap->Cvar_Set(va("siege_objective%i_inuse", i), "0");
...
void CG_SiegeObjectiveCompleted(centity_t *ent, int won, int objectivenum) {
  ...
  if (BG_SiegeGetValueGroup(siege_info, teamstr, cgParseObjectives)) {
    ...
    if (BG_SiegeGetValueGroup(cgParseObjectives, objstr, foundobjective)) {
      if (myTeam == SIEGETEAM_TEAM1) {
        success =
            BG_SiegeGetPairedValue(foundobjective, "message_team1", appstring);
      } else {
        success =
            BG_SiegeGetPairedValue(foundobjective, "message_team2", appstring);
      }
      ...
      else
      {
              if (myTeam != won)
              {
                      Com_sprintf(soundstr, sizeof(soundstr),
      DEFAULT_LOSE_OBJECTIVE);
              }
              else
              {
                      Com_sprintf(soundstr, sizeof(soundstr),
      DEFAULT_WIN_OBJECTIVE);
              }
      }
      */

      if (soundstr[0]) {
        trap->S_StartLocalSound(trap->S_RegisterSound(soundstr),
                                CHAN_ANNOUNCER);
      }
    }
  }
...
void CG_ParseSiegeExtendedDataEntry(const char *conStr) {
  ...
  if (ammo >= 0 &&
      ammo <= maxAmmo) { // assure the weapon number is valid and not over max
    // keep the weapon so if it changes before our next ext data update we'll
    // know that the ammo is not applicable.
    cg_siegeExtendedData[clNum].weapon = cent->currentState.weapon;
  } else { // not valid? Oh well, just invalidate the weapon too then so we
           // don't display ammo
