...
qboolean PM_SlideMove(float gravMod) {
  ...
  if (gravMod) {
    ...
    if (pml.groundPlane) {
      if (PM_GroundSlideOkay(pml.groundTrace.plane
                                 .normal[2])) { // slide along the ground plane
  ...
  for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
    ...
    if (trace.allsolid) { // entity is completely trapped in another solid
      pm->ps->velocity[2] =
    ...
    if (trace.surfaceFlags & SURF_NODAMAGE) {
      damageSelf = qfalse;
               ...
               trace.plane.normal[2] >
                   0.5f) { // if we land on the ground, let falling damage do
                           // it's thing itself, otherwise do impact damage
      damageSelf = qfalse;
    } else {
      damageSelf = qtrue;
    }
    ...
    if (!PM_GroundSlideOkay(normal[2])) { // wall-running
...
void PM_StepSlideMove(float gravMod) {
  ...
  if (pm->gent && pm->gent->client &&
    ...
    isATST = qtrue;
    stepSize = 66; // hack for AT-ST stepping, slightly taller than a standing
                   // stormtrooper
  } else if (pm->maxs[2] <=
             0) { // short little guys can't go up steps... FIXME: just make
                  // this a flag for certain NPCs- especially ones that roll?
    stepSize = 4;
  }
  ...
  if (!pm->ps->velocity[0] &&
      !pm->ps->velocity[1]) { // All our velocity was cancelled sliding
    return;
  }
  ...
  if (fabs(stepUpMove[0]) < 0.1 && fabs(stepUpMove[1]) < 0.1 &&
    ...
    pm->trace(&trace, pm->ps->origin, pm->mins, pm->maxs, down,
              pm->ps->clientNum, pm->tracemask, G2_NOCOLLIDE, 0);
    if (!trace.allsolid) {
      if (pm->ps->clientNum && isATST && g_entities[trace.entityNum].client &&
          g_entities[trace.entityNum].client->playerTeam ==
              pm->gent->client->playerTeam) { // AT-ST's don't step up on allies
      } else {
        VectorCopy(trace.endpos, pm->ps->origin);
      }
    }
  ...
  /*
  if(cantStepUpFwd && pm->ps->origin[2] < start_o[2] + stepSize &&
  pm->ps->origin[2] >= start_o[2])
