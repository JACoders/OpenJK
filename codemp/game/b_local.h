...
extern void Debug_NPCPrintf(gentity_t *printNPC, vmCvar_t *cv, int debugLevel,
                            char *fmt, ...);

// MCG - Begin============================================================
// NPC_ai variables - shared by NPC.cpp and the following modules
// OJKFIXME: Should probably construct these at the NPC entry points and pass as
// arguments to any function that needs them
...
extern gentity_t *NPC_CheckEnemy(qboolean findNew, qboolean tooFarOk,
                                 qboolean setEnemy); // setEnemy = qtrue
...
extern int NPC_ShotEntity(gentity_t *ent,
                          vec3_t impactPos); // impactedPos = NULL
...
extern qboolean InFOV3(vec3_t spot, vec3_t from, vec3_t fromAngles, int hFOV,
                       int vFOV);
...
extern int
NPC_CheckAlertEvents(qboolean checkSight, qboolean checkSound, int ignoreAlert,
...
extern void G_AlertTeam(gentity_t *victim, gentity_t *attacker, float radius,
                        float soundDist);
...
extern qboolean NAV_HitNavGoal(vec3_t point, vec3_t mins, vec3_t maxs,
                               vec3_t dest, int radius, qboolean flying);

extern void NPC_SetMoveGoal(
    gentity_t *ent, vec3_t point, int radius, qboolean isNavGoal,
    int combatPoint,
    gentity_t
        *targetEnt); // isNavGoal = qfalse, combatPoint = -1, targetEnt = NULL

extern qboolean NAV_ClearPathToPoint(gentity_t *self, vec3_t pmins,
                                     vec3_t pmaxs, vec3_t point, int clipmask,
                                     int okToHitEnt);
extern void NPC_ApplyWeaponFireDelay(void);

// NPC_FaceXXX suite
extern qboolean NPC_FacePosition(vec3_t position,
                                 qboolean doPitch); // doPitch = qtrue
extern qboolean NPC_FaceEntity(gentity_t *ent,
                               qboolean doPitch); // doPitch = qtrue
