...
int Pickup_Weapon(gentity_t *ent, gentity_t *other) {
	...
	*/
	...
	if (other->s.number) {				  // NPC
		if (other->s.weapon == WP_NONE) { // NPC with no weapon picked up a weapon, change to this weapon
...
void Touch_Item(gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	if (other->client->NPC_class == CLASS_ATST || other->client->NPC_class == CLASS_GONK || other->client->NPC_class == CLASS_MARK1 ||
		...
		other->client->NPC_class == CLASS_SEEKER || other->client->NPC_class == CLASS_REMOTE ||
	...
	} else if (!(ent->spawnflags & ITMSF_TEAM) && !(ent->spawnflags & ITMSF_MONSTER)) { // Only player can pick it up
	...
	if (other->client) {
		if (other->client->ps.eFlags & EF_FORCE_GRIPPED) { // can't pick up anything while being gripped
			return;
		}
