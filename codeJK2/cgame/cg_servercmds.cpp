...
void CG_ParseServerinfo(void) {
  ...
  Q_strncpyz(cgs.stripLevelName[0], p ? p + 1 : mapname,
             sizeof(cgs.stripLevelName[0]));
  Q_strupr(cgs.stripLevelName[0]);
  for (int i = 1; i < STRIPED_LEVELNAME_VARIATIONS; i++) // clear retry-array
  {
    cgs.stripLevelName[i][0] = '\0';
  }
  // be careful with the []-numbers here. Currently I use 0,1,2 for replacements
  // or substitution, and [3] for "INGAME"
  //	I know, if I'd known there was going to be this much messing about I'd
  //have subroutinised it all and done it 	neater, but it kinda evolved...   Feel
  //free to bug me if you want to add to it... ?  -Ste.
  //
  if (!cgi_SP_Register(cgs.stripLevelName[0], qfalse)) {
    ...
    if (!Q_stricmp(cgs.stripLevelName[0], "YAVIN_FINAL") ||
      ...
      Q_strncpyz(cgs.stripLevelName[0], "YAVIN_CANYON",
                 sizeof(cgs.stripLevelName[0]));
      if (!cgi_SP_Register(cgs.stripLevelName[0], qfalse)) {
      ...
      Q_strncpyz(cgs.stripLevelName[0], "YAVIN_TEMPLE",
                 sizeof(cgs.stripLevelName[0]));
      if (!cgi_SP_Register(cgs.stripLevelName[0], qfalse)) {
        // failed again, give up for now...
        //
      ...
      Q_strncpyz(cgs.stripLevelName[0], "ARTUS_TOPSIDE",
                 sizeof(cgs.stripLevelName[0]));
      if (!cgi_SP_Register(cgs.stripLevelName[0], qfalse)) {
        // failed again, give up for now...
        //
    ...
    if (!Q_stricmp(cgs.stripLevelName[0], "NS_STARPAD")) {
      Q_strncpyz(cgs.stripLevelName[1], "ARTUS_TOPSIDE",
                 sizeof(cgs.stripLevelName[1])); // for dream sequence...
      if (!cgi_SP_Register(cgs.stripLevelName[1], qfalse)) {
        // failed again, give up for now...
        //
      }

      Q_strncpyz(cgs.stripLevelName[2], "BESPIN_UNDERCITY",
                 sizeof(cgs.stripLevelName[1])); // for dream sequence...
...
static void CG_ConfigStringModified(void) {
  ...
  if (num == CS_ITEMS) {
    ...
    //		OutputDebugString(va("### CG_ConfigStringModified():
    //cgs.model_draw[%d] = \"%s\"\n",num-CS_MODELS,str));
    // GHOUL2 Insert start
...
static void CG_CaptionText_f(void) {
  ...
  CG_CaptionText(CG_Argv(1), sound >= 0 && sound < MAX_SOUNDS
                                 ? cgs.sound_precache[sound]
                                 : NULL_SOUND);
}
