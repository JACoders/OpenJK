...
void CIcarus::ClearSignal(const char *identifier) { m_signals.erase(identifier); }
...
int CIcarus::Save() {
	...
	saved_game.write_chunk(INT_ID('I', 'S', 'E', 'Q'), m_byBuffer, static_cast<int>(m_ulBufferCurPos));
...
int CIcarus::Load() {
	...
	const unsigned char *sg_buffer_data = static_cast<const unsigned char *>(saved_game.get_buffer_data());
...
void CIcarus::BufferWrite(void *pSrcData, unsigned long ulNumBytesToWrite) {
	...
	if (MAX_BUFFER_SIZE - m_ulBufferCurPos < ulNumBytesToWrite) { // Write out the buffer with all our collected data so far...
		IGameInterface::GetGame()->DebugPrint(IGameInterface::WL_ERROR, "BufferWrite: Out of buffer space, Flushing.");

		ojk::SavedGameHelper saved_game(IGameInterface::GetGame()->get_saved_game_file());
...
void CIcarus::BufferRead(void *pDstBuff, unsigned long ulNumBytesToRead) {
	...
	if (m_ulBytesRead + ulNumBytesToRead > MAX_BUFFER_SIZE) { // We've tried to read past the buffer...
