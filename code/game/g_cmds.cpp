  ...
  if (give_all || !Q_stricmp(name, "force")) {
    if (argc == 3)
      ent->client->ps.forcePower =
          Com_Clampi(0, ent->client->ps.forcePowerMax, atoi(args));
    else
...
void Cmd_Kill_f(gentity_t *ent) {
  if ((level.time - ent->client->respawnTime) < 5000) {
    gi.SendServerCommand(ent - g_entities,
                         "cp @SP_INGAME_ONE_KILL_PER_5_SECONDS");
    return;
  }
  ent->flags &= ~FL_GODMODE;
...
qboolean PickSeekerSpawnPoint(vec3_t org, vec3_t fwd, vec3_t right, int skip,
  ...
  gi.trace(&tr, org, mins, maxs, end, skip, MASK_PLAYERSOLID, (EG2_Collision)0,
           0);
...
void Cmd_FlushCamFile_f(gentity_t *ent) { gi.FlushCamFile(); }

void G_Taunt(gentity_t *ent) {
  if (ent->client) {
    if (ent->client->ps.weapon == WP_SABER &&
        (ent->client->ps.saberAnimLevel ==
             SS_STAFF // ent->client->ps.saber[0].type == SABER_STAFF
...
void G_SetTauntAnim(gentity_t *ent, int taunt) {
  ...
  if (!ent->client->ps.torsoAnimTimer && !ent->client->ps.legsAnimTimer &&
    ...
    case TAUNT_TAUNT:
      if (ent->client->ps.weapon != WP_SABER) {
        ...
        case SS_FAST:
        case SS_TAVION:
          if (ent->client->ps.saber[1].Active()) { // turn off second saber
            G_Sound(ent, ent->client->ps.saber[1].soundOff);
          } else if (ent->client->ps.saber[0].Active()) { // turn off first
...
void Cmd_SaberDrop_f(gentity_t *ent, int saberNum) {
  ...
  if (G_DropSaberItem(ent->client->ps.saber[saberNum].name,
                      ...
                      (saberNum == 0 ? ent->client->renderInfo.handRPoint
                                     : ent->client->renderInfo.handLPoint),
                      ent->client->ps.velocity,
                      ent->currentAngles) != NULL) { // dropped it
    WP_RemoveSaber(ent, saberNum);
  }

  if (ent->weaponModel[0] <= 0 && ent->weaponModel[1] <= 0) { // no sabers left!
...
void ClientCommand(int clientNum) {
  ...
  if (Q_stricmp(cmd, "give") == 0)
    ...
    if (gi.argc() < 2) {
      ...
      gi.SendServerCommand(
          ent - g_entities,
          va("print \"Valid styles: SS_FAST, SS_MEDIUM, SS_STRONG, SS_DESANN, "
             "SS_TAVION, SS_DUAL and SS_STAFF\n\""));
      return;
    }
    ...
    if (gi.argc() < 2) {
      gi.SendServerCommand(
          ent - g_entities,
          va("print \"usage: setsaberstyle <saber style>\n\""));
      gi.SendServerCommand(
          ent - g_entities,
          va("print \"Valid styles: SS_FAST, SS_MEDIUM, SS_STRONG, SS_DESANN, "
             "SS_TAVION, SS_DUAL and SS_STAFF\n\""));
  ...
  else if (Q_stricmp (cmd, "drive") == 0)
  {
          ...
          if ( gi.argc() < 2 )
          {
                  gi.SendServerCommand( ent-g_entities, va("print \"usage: drive
  <vehicle name>\n\"")); gi.SendServerCommand( ent-g_entities, va("print
  \"Vehicles will be in vehicles.cfg, try using 'speeder' for now\n\""));
                  return;
          }
  ...
  */
    ...
    if (gi.argc() < 3) {
      gi.SendServerCommand(
          ent - g_entities,
          va("print \"usage: drive <NPC_targetname> <vehicle name>\n\""));
      gi.SendServerCommand(ent - g_entities,
                           va("print \"Vehicles will be in vehicles.cfg, try "
                              "using 'speeder' for now\n\""));
      return;
    }
    ...
    if (saberNum > 1) { // drop both
      Cmd_SaberDrop_f(ent, 1);
      Cmd_SaberDrop_f(ent, 0);
    } else { // drop either left or right
