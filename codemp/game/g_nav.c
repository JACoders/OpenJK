...
extern qboolean G_FindClosestPointOnLineSegment(const vec3_t start,
                                                ...
                                                vec3_t result);
// For debug graphics
// rwwFIXMEFIXME: Write these at some point for the sake of being able to debug
// visually
...
qboolean NAV_ClearPathToPoint(gentity_t *self, vec3_t pmins, vec3_t pmaxs,
  ...
  if (self->flags & FL_NAVGOAL) {
    ...
    if (trace.startsolid &&
      ...
      trap->Trace(&trace, point, mins, maxs, self->r.currentOrigin,
                  self->parent->s.number,
                  (clipmask | CONTENTS_MONSTERCLIP) & ~CONTENTS_BODY, qfalse, 0,
                  0);
    }
    ...
    if (NAV_HitNavGoal(self->r.currentOrigin, self->parent->r.mins,
                       self->parent->r.maxs, trace.endpos,
    ...
    trap->Trace(
        &trace, self->r.currentOrigin, mins, maxs, point, self->s.number,
    ...
    if (trace.startsolid &&
      ...
      trap->Trace(&trace, self->r.currentOrigin, mins, maxs, point,
                  self->s.number, clipmask | CONTENTS_MONSTERCLIP, qfalse, 0,
                  0);
    }
...
int NAV_FindClosestWaypointForEnt(gentity_t *ent, int targWp) {
  ...
  return trap->Nav_GetNearestNode((sharedEntity_t *)ent, ent->waypoint,
                                  NF_CLEAR_PATH, targWp);
}
...
int NAV_FindClosestWaypointForPoint(gentity_t *ent, vec3_t point) {
  ...
  bestWP = trap->Nav_GetNearestNode((sharedEntity_t *)marker, marker->waypoint,
...
qboolean NAV_CheckAhead(gentity_t *self, vec3_t end, trace_t *trace,
  ...
  trap->Trace(trace, self->r.currentOrigin, mins, self->r.maxs, end,
              self->s.number, clipmask, qfalse, 0, 0);
  ...
  radius =
      (self->r.maxs[0] > self->r.maxs[1]) ? self->r.maxs[0] : self->r.maxs[1];
  ...
  if (trace->entityNum < ENTITYNUM_WORLD) {
    ...
    if (VALIDSTRING(blocker->classname)) {
      ...
      // if ( Q_stricmp( blocker->classname, "func_door" ) == 0 )
      {
        ...
        if (DistanceSquared(self->r.currentOrigin, trace->endpos) <
...
qboolean NAV_StackedCanyon(gentity_t *self, gentity_t *blocker,
  ...
  trap->Trace(&tr, test, self->r.mins, self->r.maxs, test, self->s.number,
              self->clipmask | extraClip, qfalse, 0, 0);
  if (tr.startsolid &&
      (tr.contents &
       CONTENTS_BOTCLIP)) { // started inside do not enter, so ignore them
...
qboolean NAV_ResolveEntityCollision(gentity_t *self, gentity_t *blocker,
  ...
  // if ( Q_stricmp( blocker->classname, "func_door" ) == 0 )
  {
    if (DistanceSquared(self->r.currentOrigin, blocker->r.currentOrigin) >
        MIN_DOOR_BLOCK_DIST_SQR)
      return qtrue;
  }
...
qboolean NAV_TestForBlocked(gentity_t *self, gentity_t *goal,
  ...
  if (NAV_HitNavGoal(blocker->r.currentOrigin, blocker->r.mins, blocker->r.maxs,
                     goal->r.currentOrigin, 12, qfalse)) {
    *flags |= NIF_BLOCKED;
...
qboolean NAV_AvoidCollision(gentity_t *self, gentity_t *goal, navInfo_t *info) {
  ...
  if (self && self->NPC &&
      (self->NPC->aiFlags &
       NPCAI_NO_COLL_AVOID)) { // pretend there's no-one in the way
    return qtrue;
  }
...
int NAV_TestBestNode(gentity_t *self, int startID, int endID,
  ...
  if (self->s.weapon != WP_SABER &&
      fabs(self->r.currentOrigin[2] - end[2]) > 48) {
  } else {
    // This is a work around
    float radius =
        (self->r.maxs[0] > self->r.maxs[1]) ? self->r.maxs[0] : self->r.maxs[1];
    ...
    if
      VALIDSTRING(blocker->classname) { // special case: doors are architecture,
        ...
        { // it's unlocked, go for it
          ...
          if (self->s.weapon != WP_SABER &&
              fabs(self->r.currentOrigin[2] - end[2]) > 48) { // too far above
          } else {
            return endID;
          }
...
int NAV_GetNearestNode(gentity_t *self, int lastNode) {
  return trap->Nav_GetNearestNode((sharedEntity_t *)self, lastNode,
                                  NF_CLEAR_PATH, WAYPOINT_NONE);
}
...
int NAV_MoveToGoal(gentity_t *self, navInfo_t *info) {
  ...
  if (self->NPC->goalEntity->s.number >= 0 &&
      self->NPC->goalEntity->s.number < MAX_CLIENTS) {
  ...
  bestNode = trap->Nav_GetBestNode(self->waypoint,
                                   self->NPC->goalEntity->waypoint, NODE_NONE);
...
unsigned int waypoint_getRadius(gentity_t *ent) {
  ...
  for (i = 0; i < YAW_ITERATIONS; i++) {
    dist = waypoint_testDirection(ent->r.currentOrigin,
...
void SP_waypoint(gentity_t *ent) {
  if (navCalculatePaths) {
    ...
    ent->health =
        trap->Nav_AddRawPoint(ent->r.currentOrigin, ent->spawnflags, radius);
...
void SP_waypoint_small(gentity_t *ent) {
  if (navCalculatePaths) {
    ...
    ent->health =
        trap->Nav_AddRawPoint(ent->r.currentOrigin, ent->spawnflags, 2);
    NAV_StoreWaypoint(ent);
    G_FreeEntity(ent);
    return;
  }
...
void Svcmd_Nav_f(void) {
  ...
  if (Q_stricmp(cmd, "show") == 0) {
    ...
    if (Q_stricmp(cmd, "testgoal") == 0) {
      NAVDEBUG_curGoal = trap->Nav_GetNearestNode(
          (sharedEntity_t *)&g_entities[0], g_entities[0].waypoint,
          NF_CLEAR_PATH, WAYPOINT_NONE);
    }
  ...
  if (Distance(wp1->r.currentOrigin, wp2->r.currentOrigin) > 1024) {
    ...
    if (!wp1->targetname && !wp2->targetname) {
      Com_sprintf(temp, sizeof(temp),
                  S_COLOR_RED "Waypoint conn %s->%s > 1024\n",
...
static waypointData_t
    tempWaypointList[MAX_STORED_WAYPOINTS]; // rwwFIXMEFIXME: Need.. dynamic..
                                            // memory
...
void NAV_StoreWaypoint(gentity_t *ent) {
  ...
  if ( !tempWaypointList )
  {
          //tempWaypointList = (waypointData_t *)
  trap->Malloc(sizeof(waypointData_t)*MAX_STORED_WAYPOINTS, TAG_TEMP_WORKSPACE,
  qtrue);
  ...
  */
  ...
  if (numStoredWaypoints >= MAX_STORED_WAYPOINTS) {
    ...
    // numStoredWaypoints, MAX_STORED_WAYPOINTS ); rwwFIXMEFIXME: commented this
    // out so I can load some of the SP levels.
    return;
  }
  if (ent->targetname) {
...
#endif
      {
        trap->Nav_HardConnect(tempWaypointList[i].nodeID,
...
void NAV_ShowDebugInfo(void) {
  ...
  if (NAVDEBUG_showTestPath) {
    ...
    int nearestNode =
        trap->Nav_GetNearestNode((sharedEntity_t *)&g_entities[0],
                                 g_entities[0].waypoint, NF_ANY, WAYPOINT_NONE);
    int testNode =
        trap->Nav_GetBestNode(nearestNode, NAVDEBUG_curGoal, NODE_NONE);
    vec3_t dest, start;
...
void NAV_FindPlayerWaypoint(int clNum) {
  g_entities[clNum].waypoint = trap->Nav_GetNearestNode(
      (sharedEntity_t *)&g_entities[clNum], g_entities[clNum].lastWaypoint,
      NF_CLEAR_PATH, WAYPOINT_NONE);
}
