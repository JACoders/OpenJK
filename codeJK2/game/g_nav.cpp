...
qboolean NAV_ClearPathToPoint(gentity_t *self, vec3_t pmins, vec3_t pmaxs, vec3_t point, int clipmask, int okToHitEntNum) {
	...
	if (self->svFlags & SVF_NAVGOAL) {
		...
		if (trace.startsolid && (trace.contents & CONTENTS_BOTCLIP)) { // started inside do not enter, so ignore them
			...
			gi.trace(&trace, point, mins, maxs, self->currentOrigin, self->owner->s.number, (clipmask | CONTENTS_MONSTERCLIP) & ~CONTENTS_BODY, G2_NOCOLLIDE,
					 0);
		}
	...
	if (trace.entityNum < ENTITYNUM_WORLD) {
		...
		if VALIDSTRING (blocker->classname) { // special case: doors are architecture, but are dynamic, like entitites
			...
			{ // it's unlocked, go for it
				...
				if (self->s.weapon != WP_SABER && fabs(self->currentOrigin[2] - end[2]) > 48) { // too far above
				} else {
					return endID;
				}
				...
				if (failEdge) {
					navigator.AddFailedEdge(self->s.number, startID, endID);
					   ...
					   ((blocker->contents & CONTENTS_MONSTERCLIP) ||
						(blocker->contents & CONTENTS_BOTCLIP))) { // some other kind of do not enter entity brush that will probably be removed
