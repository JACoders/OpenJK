...
// Handy Functions
////////////////////////////////////////////////////////////////////////////////////////
// Returns a float min <= x < max (exclusive; will get max - 0.00001; but never
// max)
...
struct SFloatRange {
  ...
  inline void Pick(float &V) { V = WE_flrand(mMin, mMax); }
...
bool R_GetWindGusting() { return (mGlobalWindSpeed > 1000.0f); }
...
public:
  ...
  void AddWeatherZone(vec3_t mins, vec3_t maxs) {
    if (!mWeatherZones.full()) {
      ...
      Wz.mPointCache = (uint32_t *)Z_Malloc(arraySize * sizeof(uint32_t),
                                            TAG_POINTCACHE, qtrue);
    }
  }
            ...
            for (y = 0; y < wz.mHeight; y++) {
              ...
              if (contents & CONTENTS_INSIDE || contents & CONTENTS_OUTSIDE) {
                ...
                if (!mCacheInit) {
                  ...
                  assert(0);
                  Com_Error(ERR_DROP, "Weather Effect: Both Indoor and Outdoor "
                                      "brushs encountered in map.\n");
                  return;
                }

                // Mark The Point
                //----------------
                wz.mPointCache[((z * wz.mWidth * wz.mHeight) + (y * wz.mWidth) +
                                x)] |= bit;
              }
            } // for (y)
          }   // for (x)
...
bool R_IsShaking() {
  return (mOutside.mOutsideShake &&
          mOutside.PointOutside(backEnd.viewParms.ori.origin));
}
...
public:
  ...
  void Initialize(int count, const char *texturePath, int VertexCount = 4) {
    ...
    if (!mImage) {
      Com_Error(ERR_DROP, "CWeatherParticleCloud: Could not texture %s",
                texturePath);
    }
    ...
    for (particleNum = 0; particleNum < mParticleCount; particleNum++) {
      ...
      if (!partInRange && !partRendering) {
        ...
        if (UseSpawnPlane()) {
          ...
          part->mPosition +=
              (mSpawnPlaneRight * WE_flrand(-mSpawnPlaneSize, mSpawnPlaneSize));
          part->mPosition +=
              (mSpawnPlaneUp * WE_flrand(-mSpawnPlaneSize, mSpawnPlaneSize));
        }
  ...
  void Render() {
    ...
    // naughty, you are making the assumption that culling is on when you get
    // here. -rww
...
void RB_RenderWorldEffects(void) {
  if (!tr.world || (tr.refdef.rdflags & RDF_NOWORLDMODEL) ||
      ...
      !mParticleClouds
           .size()) { //  no world rendering or no world or no particle clouds
    return;
  }
...
qboolean WE_ParseVector(const char **text, int count, float *v) {
  ...
  if (strcmp(token, "(")) {
    ri.Printf(PRINT_WARNING,
              "WARNING: missing parenthesis in weather effect\n");
    return qfalse;
  }
  ...
  for (i = 0; i < count; i++) {
    ...
    if (!token[0]) {
      ri.Printf(PRINT_WARNING,
                "WARNING: missing vector element in weather effect\n");
      return qfalse;
    }
    v[i] = atof(token);
  }
...
bool R_IsPuffing() { // Eh? Don't want surfacesprites to know this?
  return false;
}
