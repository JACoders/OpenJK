...
void MC_Compress(const float mat[3][4], unsigned char *_comp) {
  ...
  // I added this because the line above actually ORs data into an int at the 22
  // byte (from 0), and therefore technically
  //	is writing beyond the 24th byte of the output array. This *should** be
  //harmless if the OR'd-in value doesn't change 	those bytes, but BoundsChecker
  //says that it's accessing undefined memory (which it does, sometimes). This
  //is probably 	bad, so...
