...
#define FX_MAX_EFFECT_COMPONENTS                                               \
  24 // how many primitives an effect can hold, this should be plenty
...
#define FX_ORG_ON_SPHERE                                                       \
  0x00001 // Pretty dang expensive, calculates a point on a sphere/ellipsoid
#define FX_AXIS_FROM_SPHERE                                                    \
  0x00002 // Can be used in conjunction with org_on_sphere to cause particles to
          // move out
          //	from the center of the sphere
#define FX_ORG_ON_CYLINDER 0x00004 // calculate point on cylinder/disk

#define FX_ORG2_FROM_TRACE 0x00010
#define FX_TRACE_IMPACT_FX                                                     \
  0x00020 // if trace impacts, we should play one of the specified impact fx
          // files
#define FX_ORG2_IS_OFFSET                                                      \
  0x00040 // template specified org2 should be the offset from a trace endpos or
          //	passed in org2. You might use this to lend a random flair to the
          //endpos. 	Note: this is done pre-trace, so you may have to specify
          //large numbers for this

#define FX_CHEAP_ORG_CALC                                                      \
  0x00100 // Origin is calculated relative to passed in axis unless this is on.
#define FX_CHEAP_ORG2_CALC                                                     \
  0x00200 // Origin2 is calculated relative to passed in axis unless this is on.
#define FX_VEL_IS_ABSOLUTE                                                     \
  0x00400 // Velocity isn't relative to passed in axis with this flag on.
#define FX_ACCEL_IS_ABSOLUTE                                                   \
  0x00800 // Acceleration isn't relative to passed in axis with this flag on.

#define FX_RAND_ROT_AROUND_FWD                                                 \
  0x01000 // Randomly rotates up and right around forward vector
#define FX_EVEN_DISTRIBUTION                                                   \
  0x02000 // When you have a delay, it normally picks a random time to play.
          // When this flag is on, it generates an even time distribution
#define FX_RGB_COMPONENT_INTERP                                                \
  0x04000 // Picks a color on the line defined by RGB min & max, default is to
          // pick color in cube defined by min & max

#define FX_AFFECTED_BY_WIND                                                    \
  0x10000 // we take into account our wind vector when we spawn in
...
public:
  ...
  int GetHandle() {
    if (mMediaList.size() == 0) {
      ...
      return mMediaList[irand(0, (int)mMediaList.size() - 1)];
    }
  }
...
public:
  ...
  bool mCopy;
  int mRefCount; // For a copy of a primitive...when we figure out how many
                 // items we want to spawn,
                 //	we'll store that here and then decrement us for each we
                 //actually spawn.  When we 	hit zero, we are no longer used and
                 //so we can just free ourselves
  ...
  int mSpawnFlags; // These are only used to control spawning, but never get
                   // passed to prims.
...
private:
  ...
  // Base key processing
  // Note that these all have their own parse functions in case it becomes
  // important to do certain kinds
  ...
  using StringViewIMap =
      std::map<gsl::cstring_view, Value, Q::CStringViewILess>;
  using ParseMethod = bool (CPrimitiveTemplate::*)(const gsl::cstring_view &);
  // Group type processing
  bool ParseGroup(const CGPGroup &grp,
                  const StringViewIMap<ParseMethod> &parseMethods,
                  gsl::czstring name);
...
template <typename T, int N> class PoolAllocator {
public:
  PoolAllocator()
      : pool(new T[N]), freeAndAllocated(new int[N]), numFree(N),
        highWatermark(0) {
    for (int i = 0; i < N; i++) {
  ...
  bool OwnsPtr(const T *ptr) const { return ptr >= pool && ptr < (pool + N); }
  ...
  void Free(T *ptr) {
    for (int i = numFree; i < N; i++) {
      ...
      if (p == ptr) {
        if (i > numFree) {
          std::rotate(freeAndAllocated + numFree, freeAndAllocated + i,
                      freeAndAllocated + i + 1);
        }
...
template <typename T, int N> class PagedPoolAllocator {
public:
  PagedPoolAllocator() : numPages(1), pages(new PoolAllocator<T, N>[1]()) {}
  ...
  ~PagedPoolAllocator() { delete[] pages; }
...
struct SLoopedEffect {
  int mId;       // effect id
  int mBoltInfo; // used to determine which bolt on the ghoul2 model we should
                 // be attaching this effect to
  int mNextTime; // time to render again
  int mLoopStopTime;  // time to die
  bool mPortalEffect; // rww - render this before skyportals, and not in the
                      // normal world view.
...
private:
  ...
  struct SScheduledEffect {
    ...
    int mStartTime;
    char mModelNum; // uset to determine which ghoul2 model we want to bolt this
                    // effect to
    char mBoltNum; // used to determine which bolt on the ghoul2 model we should
                   // be attaching this effect to
    short mEntNum; // used to determine which entity this ghoul model is
                   // attached to.
    short mClientID; // FIXME: redundant. this is used for muzzle bolts, merge
                     // into normal bolting
    bool mPortalEffect; // rww - render this before skyportals, and not in the
                        // normal world view.
    bool mIsRelative;   // bolt this puppy on keep it updated
  ...
  int ScheduleLoopedEffect(int id, int boltInfo, bool isPortal, int iLoopTime,
                           bool isRelative);
  ...
  TEffectID mEffectIDs; // if you only have the unique effect name, you'll have
                        // to use this to get the ID.
  ...
  void CreateEffect(CPrimitiveTemplate *fx, const vec3_t origin, vec3_t axis[3],
                    int lateTime, int clientID = -1, int modelNum = -1,
                    int boltNum = -1);
...
public:
  ...
  int RegisterEffect(const char *file,
                     bool bHasCorrectPath = false); // handles pre-caching

  // Nasty overloaded madness
  void PlayEffect(int id, vec3_t org,
                  bool isPortal = false); // uses a default up axis
  void
  PlayEffect(int id, vec3_t org, vec3_t fwd,
             bool isPortal = false); // builds arbitrary perp. right vector,
                                     // does a cross product to define up
  void PlayEffect(int id, vec3_t origin, vec3_t axis[3],
                  const int boltInfo = -1, const int entNum = -1,
                  bool isPortal = false, int iLoopTime = false,
                  bool isRelative = false);
  void PlayEffect(const char *file, vec3_t org,
                  bool isPortal = false); // uses a default up axis
  void
  PlayEffect(const char *file, vec3_t org, vec3_t fwd,
             bool isPortal = false); // builds arbitrary perp. right vector,
                                     // does a cross product to define up
  void PlayEffect(const char *file, vec3_t origin, vec3_t axis[3],
                  const int boltInfo, const int entNum, bool isPortal = false,
                  int iLoopTime = false, bool isRelative = false);

  // for muzzle
  void PlayEffect(const char *file, int clientID, bool isPortal = false);

  void StopEffect(const char *file, const int boltInfo,
                  bool isPortal = false); // find a scheduled Looping effect
                                          // with these parms and kill it

  void AddScheduledEffects(
      bool portal); // call once per CGame frame [rww ammendment - twice now
                    // actually, but first only renders portal effects]

  int NumScheduledFx() { return (int)mFxSchedule.size(); }
  void Clean(bool bRemoveTemplates = true,
             int idToPreserve = 0); // clean out the system
  ...
  CPrimitiveTemplate *GetPrimitiveCopy(SEffectTemplate *effectCopy,
