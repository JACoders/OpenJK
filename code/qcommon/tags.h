...
TAGDEF(ALL),
    TAGDEF(HUNKALLOC), // mem that was formerly from the hunk AFTER the SetMark
                       // (ie discarded during vid_reset)
    TAGDEF(HUNKMISCMODELS), // sub-hunk alloc to track misc models
    TAGDEF(FILESYS),        // general filesystem usage
    TAGDEF(EVENT), TAGDEF(CLIPBOARD),
    ...
    TAGDEF(SMALL), // used by S_Malloc, but probably more of a hint now. Will be
                   // dumped later
    TAGDEF(MODEL), // general model usage), includes header-struct-only stuff
                   // like 'model_t'
