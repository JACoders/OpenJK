...
CPrimitiveTemplate::CPrimitiveTemplate() {
  ...
  mDensity.SetRange(10.0f,
                    10.0f); // default this high so it doesn't do bad things
}

//-----------------------------------------------------------
void CPrimitiveTemplate::operator=(const CPrimitiveTemplate &that) {
  // I'm assuming that doing a memcpy wouldn't work here
  // If you are looking at this and know a better way to do this, please tell
  // me.
...
//------------------------------------------------------
bool CPrimitiveTemplate::ParseFloat(const gsl::cstring_view &val, float &min,
                                    float &max) {
  ...
  if (v == 0) { // nothing was there, failure
    return false;
  } else if (v == 1) { // only one field entered, this is ok, but we should copy
                       // min into max
    max = min;
  }
...
//	success of parse operation.
//------------------------------------------------------
bool CPrimitiveTemplate::ParseVector(const gsl::cstring_view &val, vec3_t min,
                                     vec3_t max) {
...
template <std::size_t remaining> struct ScanStrings {
  ...
  static int call(const gsl::cstring_view &val,
                  std::array<gsl::cstring_view, count> &arr, Args &... args) {
    return ScanStrings<remaining - 1>::call(val, arr, arr[remaining - 1],
                                            args...);
  }
};
...
static gsl::array_view<gsl::cstring_view>
scanStrings(const gsl::cstring_view &val,
...
//	success of parse operation.
//------------------------------------------------------
bool CPrimitiveTemplate::ParseGroupFlags(const gsl::cstring_view &val,
                                         int &flags) {
  ...
  // Clear out the flags field, then convert the flag string to an actual value
  // ( use generic flags )
  flags = 0;
...
bool CPrimitiveTemplate::ParseMin(const gsl::cstring_view &val) {
  ...
  if (ParseVector(val, min, min) == true) {
    ...
    // We assume that if a min is being set that we are using physics and a
    // bounding box
...
bool CPrimitiveTemplate::ParseMax(const gsl::cstring_view &val) {
  ...
  if (ParseVector(val, max, max) == true) {
    ...
    // We assume that if a max is being set that we are using physics and a
    // bounding box
    mFlags |= (FX_USE_BBOX | FX_APPLY_PHYSICS);
    return true;
  }
...
bool CPrimitiveTemplate::ParseElasticity(const gsl::cstring_view &val) {
  ...
  if (ParseFloat(val, min, max) == true) {
    ...
    // We assume that if elasticity is set that we are using physics, but don't
    // assume we are
...
bool CPrimitiveTemplate::ParseFlags(const gsl::cstring_view &val) {
  ...
  for (auto &cur : availableFlag) {
    static StringViewIMap<int> flagNames{
        ...
        // rww - begin g2 stuff
        {CSTRING_VIEW("ghoul2Collision"),
         (FX_GHOUL2_TRACE | FX_APPLY_PHYSICS | FX_EXPENSIVE_PHYSICS)},
    ...
    auto pos = flagNames.find(cur);
    if (pos == flagNames.end()) { // we have badness going on, but continue on
                                  // in case there are any valid fields in here
      ok = false;
    } else {
      mFlags |= pos->second;
    }
...
bool CPrimitiveTemplate::ParseShaders(const CGPProperty &grp) {
  ...
  if (!any) {
    // empty "list"
    theFxHelper.Print(
        "CPrimitiveTemplate::ParseShaders called with an empty list!\n");
    return false;
  }
  return true;
}
...
bool CPrimitiveTemplate::ParseSounds(const CGPProperty &grp) {
  ...
  if (!any) {
    // empty "list"
    theFxHelper.Print(
        "CPrimitiveTemplate::ParseSounds called with an empty list!\n");
    return false;
  }
  return true;
}
...
bool CPrimitiveTemplate::ParseModels(const CGPProperty &grp) {
  ...
  if (!any) {
    // empty "list"
    theFxHelper.Print(
        "CPrimitiveTemplate::ParseModels called with an empty list!\n");
    return false;
  }
...
static bool ParseFX(const CGPProperty &grp, CFxScheduler &scheduler,
                    CMediaHandles &handles, SFxHelper &helper, int &flags,
                    int successFlags, gsl::czstring loadError,
                    gsl::czstring emptyError) {
  bool any = false;
  for (auto &value : grp.GetValues()) {
    if (!value.empty()) {
      any = true;
      // TODO: string_view parameter
      int handle = scheduler.RegisterEffect(
...
bool CPrimitiveTemplate::ParseImpactFxStrings(const CGPProperty &grp) {
  return ParseFX(
      grp, theFxScheduler, mImpactFxHandles, theFxHelper, mFlags,
      FX_IMPACT_RUNS_FX | FX_APPLY_PHYSICS,
...
bool CPrimitiveTemplate::ParseDeathFxStrings(const CGPProperty &grp) {
  return ParseFX(
      grp, theFxScheduler, mDeathFxHandles, theFxHelper, mFlags,
      FX_DEATH_RUNS_FX, "FxTemplate: Death effect file not found.\n",
...
bool CPrimitiveTemplate::ParseEmitterFxStrings(const CGPProperty &grp) {
  return ParseFX(
      grp, theFxScheduler, mEmitterFxHandles, theFxHelper, mFlags, FX_EMIT_FX,
...
bool CPrimitiveTemplate::ParsePlayFxStrings(const CGPProperty &grp) {
  return ParseFX(
      grp, theFxScheduler, mPlayFxHandles, theFxHelper, mFlags, 0,
      "FxTemplate: Effect file not found.\n",
      "CPrimitiveTemplate::ParsePlayFxStrings called with an empty list!\n");
}

bool CPrimitiveTemplate::ParseGroup(
    const CGPGroup &grp, const StringViewIMap<ParseMethod> &parseMethods,
    gsl::czstring name) {
  ...
  for (auto &prop : grp.GetProperties()) {
    // Single Value Parsing
    {
      static StringViewIMap<ParseMethod> parseMethods{
          ...
          {CSTRING_VIEW("acceleration"),
           &CPrimitiveTemplate::ParseAcceleration},
          ...
          {CSTRING_VIEW("rotationDelta"),
           &CPrimitiveTemplate::ParseRotationDelta},
    ...
    // Property Parsing
    {
      using PropertyParseMethod =
          bool (CPrimitiveTemplate::*)(const CGPProperty &);
    ...
    // Special Cases
    if (Q::stricmp(prop.GetName(), CSTRING_VIEW("cullrange")) ==
        Q::Ordering::EQ) {
      mCullRange = Q::svtoi(prop.GetTopValue());
      mCullRange *= mCullRange; // Square
    } else if (Q::stricmp(prop.GetName(), CSTRING_VIEW("name")) ==
               Q::Ordering::EQ) {
      if (!prop.GetTopValue().empty()) {
        // just stash the descriptive name of the primitive
        std::size_t len = std::min<std::size_t>(prop.GetTopValue().size(),
                                                FX_MAX_PRIM_NAME - 1);
