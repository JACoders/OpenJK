...
extern saberMoveName_t PM_SaberAnimTransitionMove(saberMoveName_t curmove,
...
extern saberMoveName_t PM_SaberAttackForMovement(int forwardmove, int rightmove,
                                                 int curmove);
...
// const float	pm_frictionModifier	= 3.0f;	//Used for "careful" mode (when
// pressing use)
...
Vehicle_t *PM_RidingVehicle(void) { return (G_IsRidingVehicle(pm->gent)); }
...
qboolean BG_UnrestrainedPitchRoll(playerState_t *ps, Vehicle_t *pVeh) {
  /*
          if ( 0 && ps->clientNum < MAX_CLIENTS //real client
                  ...
                  //FIXME: specify per vehicle instead of assuming true for all
     fighters
...
qboolean PM_PredictJumpSafe(vec3_t jumpHorizDir, float jumpHorizSpeed,
                            float jumpVertSpeed, int predictTimeLength) {
  return qtrue;
}

void PM_GrabWallForJump(
    int anim) { // NOTE!!! assumes an appropriate anim is being passed in!!!
...
qboolean PM_CheckGrabWall(trace_t *trace) {
  ...
  if (pm->gent->health <= 0) { // must be alive
    return qfalse;
  }
  if (pm->gent->client->ps.groundEntityNum != ENTITYNUM_NONE) { // must be in
                                                                // air
    return qfalse;
  }
  if (trace->plane.normal[2] != 0) { // must be a flat wall
    return qfalse;
  }
  if (!trace->plane.normal[0] && !trace->plane.normal[1]) { // invalid normal
    return qfalse;
  }
  if ((trace->contents &
       (CONTENTS_PLAYERCLIP |
        CONTENTS_MONSTERCLIP))) { // can't jump off of clip brushes
    return qfalse;
  }
  if (pm->gent->client->ps.forcePowerLevel[FP_LEVITATION] <
      FORCE_LEVEL_1) { // must have at least FJ 1
    return qfalse;
  }
  if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
      pm->gent->client->ps.forcePowerLevel[FP_LEVITATION] <
          FORCE_LEVEL_3) { // player must have force jump 3
    return qfalse;
  }
  if ((pm->ps->saber[0].saberFlags & SFL_NO_WALL_GRAB)) {
    return qfalse;
  }
  if (pm->ps->dualSabers && (pm->ps->saber[1].saberFlags & SFL_NO_WALL_GRAB)) {
    return qfalse;
  }
  if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())) { // player
    ...
    if (DotProduct(moveDir, trace->plane.normal) >
        -0.65f) { // not enough of a direct impact, just slide off
      return qfalse;
    }
    ...
    if (PM_InReboundJump(pm->ps->legsAnim)) { // already in a rebound!
      return qfalse;
    }
    if ((pm->ps->eFlags & EF_FORCE_GRIPPED)) { // being gripped!
      return qfalse;
    }
    /*
    ...
    */
    if (pm->gent->NPC && (pm->gent->NPC->aiFlags &
                          NPCAI_DIE_ON_IMPACT)) { // faling to our death!
      return qfalse;
    }
    // FIXME: random chance, based on skill/rank?
    if (pm->ps->legsAnim != BOTH_FORCELONGLEAP_START &&
        pm->ps->legsAnim != BOTH_FORCELONGLEAP_ATTACK) { // not in a long-jump
      if (!pm->gent->enemy) {                            // no enemy
        return qfalse;
      } else { // see if the enemy is in the direction of the wall or above us
        ...
        { // enemy is way below us
          ...
          if (DotProduct(enemyDir, trace->plane.normal) <
              0.65f) { // jumping off this wall would not launch me in the
                       // general direction of my enemy
            return qfalse;
          }
        }
      }
    ...
    // Okay, now see if jumping off this thing would send us into a do not enter
    // brush
    if (!PM_PredictJumpSafe(trace->plane.normal, JUMP_OFF_WALL_SPEED,
                            G_ForceWallJumpStrength(),
                            1500)) { // we would hit a do not enter brush, so
                                     // don't grab the wall
      return qfalse;
    }
    ...
    if (fabs(fDot) >= 0.5f) { // hit a wall in front/behind
      if (fDot > 0.0f) {      // in front
        anim = BOTH_FORCEWALLREBOUND_FORWARD;
      } else { // behind
        anim = BOTH_FORCEWALLREBOUND_BACK;
      }
    } else if (DotProduct(rtDir, wallDir) > 0) { // hit a wall on the right
      anim = BOTH_FORCEWALLREBOUND_RIGHT;
    } else { // hit a wall on the left
...
qboolean PM_ClientImpact(trace_t *trace, qboolean damageSelf) {
  ...
  if (otherEntityNum == ENTITYNUM_WORLD ||
      (traceEnt->bmodel &&
       traceEnt->s.pos.trType ==
           TR_STATIONARY)) {       // hit world or a non-moving brush
    if (PM_CheckGrabWall(trace)) { // stopped on the wall
      return qtrue;
    }
  }
...
void PM_ClipVelocity(vec3_t in, vec3_t normal, vec3_t out, float overbounce) {
  ...
  if (g_stepSlideFix->integer) {
        ...
        && normal[2] < MIN_WALK_NORMAL) // sliding against a steep slope
    {
      ...
      { // if walking on the ground, don't slide up slopes that are too steep to
        // walk on
        out[2] = oldInZ;
      }
    }
...
static void PM_Friction(void) {
  ...
  if (pm->gent && pm->gent->client &&
    ...
    if (pm->gent->m_pVehicle &&
        pm->gent->m_pVehicle->m_pVehicleInfo->hoverHeight >
            0) { // in a hovering vehicle, have air control
      ...
      if ((pm->watertype & CONTENTS_LADDER) ||
        ...
        if (!(pm->ps->pm_flags & PMF_TIME_KNOCKBACK) &&
          ...
          if (pm->ps->legsAnim == BOTH_FORCELONGLEAP_START ||
              pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK ||
              pm->ps->legsAnim ==
                  BOTH_FORCELONGLEAP_LAND) { // super forward jump
            if (pm->ps->groundEntityNum != ENTITYNUM_NONE) { // not in air
              if (pm->cmd.forwardmove < 0) { // trying to hold back some
                friction *= 0.5f;            // 0.25f;
              } else {                       // free slide
                friction *= 0.2f;            // 0.1f;
              }
              ...
              if (pml.groundPlane &&
                ...
                // slide effect
                G_PlayEffect("env/slide_dust", pml.groundTrace.endpos,
          ...
          {//If the use key is pressed. slow the player more quickly
                  if ( pm->gent->client->NPC_class != CLASS_VEHICLE )	// if
          not in a vehicle...
          ...
          */
          ...
          drop += control * friction * pml.frametime;
             ...
             pm->gent->client->moveType == MT_FLYSWIM) { // player as Boba
...
qboolean PM_ForceJumpingUp(gentity_t *gent) {
  ...
  if (gent->client->ps.groundEntityNum == ENTITYNUM_NONE // in air
      && ((gent->client->ps.pm_flags & PMF_JUMPING) &&
          gent->client->ps.velocity[2] > 0) // jumped & going up or at water
                                            // surface///*(gent->client->ps.waterHeightLevel==WHL_SHOULDERS&&gent->client->usercmd.upmove>0)
                                            // ||*/
...
static qboolean PM_CheckJump(void) {
  ...
  if (PM_KickingAnim(pm->ps->legsAnim) &&
      !PM_InAirKickingAnim(
          pm->ps->legsAnim)) { // can't jump when in a kicking anim
    return qfalse;
  }
  /*
  ...
  */
  ...
  if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
      ...
      (pm->gent->client->NPC_class == CLASS_BOBAFETT ||
       pm->gent->client->NPC_class ==
           CLASS_ROCKETTROOPER)) { // player playing as boba fett
    if (pm->cmd.upmove > 0) {      // turn on/go up
      if (pm->ps->groundEntityNum == ENTITYNUM_NONE &&
          !(pm->ps->pm_flags &
            PMF_JUMP_HELD)) { // double-tap - must activate while in air
        if (!JET_Flying(pm->gent)) {
          JET_FlyStart(pm->gent);
        }
      }
    } else if (pm->cmd.upmove < 0) { // turn it off (or should we just go down)?
      /*
      if ( JET_Flying( pm->gent ) )
      {
              JET_FlyStop( pm->gent );
                ...
                //)
  {
    if (pm->ps->gravity > 0) { // can't do this in zero-G
      if (pm->ps->legsAnim == BOTH_FORCELONGLEAP_START ||
          pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK ||
          pm->ps->legsAnim ==
              BOTH_FORCELONGLEAP_LAND) { // in the middle of a force long-jump
        /*
        if ( pm->ps->groundEntityNum == ENTITYNUM_NONE
                && pm->cmd.upmove > 0
        ...
        */
        if ((pm->ps->legsAnim == BOTH_FORCELONGLEAP_START ||
             pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK) &&
            pm->ps->legsAnimTimer > 0) { // in the air
          // FIXME: need an actual set time so it doesn't matter when the attack
          // happens
          ...
          if (pm->ps->legsAnim == BOTH_FORCELONGLEAP_START ||
              pm->ps->legsAnim ==
                  BOTH_FORCELONGLEAP_ATTACK) { // still in start anim, but it's
                                               // run out
            pm->ps->forcePowersActive |= (1 << FP_LEVITATION);
            if (pm->ps->groundEntityNum ==
                ENTITYNUM_NONE) { // still in air?
                                  // hold it for another 50ms
                                  // PM_SetAnim( pm, SETANIM_BOTH,
                                  // BOTH_FORCELONGLEAP_START,
                                  // SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
            }
          } else { // in land-slide anim
                   // FIXME: force some forward movement?  Less if holding back?
          }
          if (pm->ps->groundEntityNum == ENTITYNUM_NONE // still in air
              && pm->ps->origin[2] <
                     pm->ps->jumpZStart) // dropped below original jump start
          {                              // slow down
            pm->ps->velocity[0] *= 0.75f;
            pm->ps->velocity[1] *= 0.75f;
            if ((pm->ps->velocity[0] + pm->ps->velocity[1]) * 0.5f <=
                10.0f) { // falling straight down
              PM_SetAnim(pm, SETANIM_BOTH, BOTH_FORCEINAIR1,
          ...
          return qfalse;
                  ...
                  PM_ControlledByPlayer()) // player-only for now
                 && pm->cmd.upmove > 0     // trying to jump
                 && pm->ps->forcePowerLevel[FP_LEVITATION] >=
                        FORCE_LEVEL_3 // force jump 3 or better
                 && pm->ps->forcePower >=
                        FORCE_LONGJUMP_POWER // this costs 20 force to do
                 && (pm->ps->forcePowersActive &
                     (1 << FP_SPEED))       // force-speed is on
                 && pm->cmd.forwardmove > 0 // pushing forward
                 && !pm->cmd.rightmove      // not strafing
                 && pm->ps->groundEntityNum != ENTITYNUM_NONE // not in mid-air
                 && !(pm->ps->pm_flags & PMF_JUMP_HELD)
                 //&& (float)(level.time-pm->ps->lastStationary) >=
                 //(3000.0f*g_timescale->value)//have to have a 3 second running
                 //start - relative to force speed slowdown
                 && (level.time - pm->ps->forcePowerDebounce[FP_SPEED]) <=
                        250 // have to have just started the force speed within
                            // the last half second
                 && pm->gent) { // start a force long-jump!
        vec3_t jFwdAngs, jFwdVec;
        // BOTH_FORCELONGLEAP_ATTACK if holding attack, too?
        PM_SetAnim(pm, SETANIM_BOTH, BOTH_FORCELONGLEAP_START,
                   SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        VectorSet(jFwdAngs, 0, pm->ps->viewangles[YAW], 0);
        AngleVectors(jFwdAngs, jFwdVec, NULL, NULL);
        VectorScale(jFwdVec, FORCE_LONG_LEAP_SPEED, pm->ps->velocity);
        ...
        WP_ForcePowerDrain(
            pm->gent, FP_LEVITATION,
            FORCE_LONGJUMP_POWER); // drain the required force power
        G_StartMatrixEffect(pm->gent, 0, pm->ps->legsAnimTimer + 500);
        return qtrue;
      } else if (PM_InCartwheel(pm->ps->legsAnim) ||
                 PM_InButterfly(
                     pm->ps->legsAnim)) { // can't keep jumping up in
                                          // cartwheels, ariels and butterflies
      }
      // FIXME: still able to pogo-jump...
      else if (PM_ForceJumpingUp(pm->gent) &&
               (pm->ps->pm_flags &
                  ...
                  (1 << FP_LEVITATION))) // haven't started forcejump yet
            {
              ...
              if (pm->gent) {
                ...
                if (pm->ps->clientNum == 0 && pm->gent->client) {
                  pm->gent->client->sess.missionStats
                      .forceUsed[(int)FP_LEVITATION]++;
                }
              }
              // play flip
              ...
              if (PM_InAirKickingAnim(pm->ps->legsAnim) &&
                  pm->ps->legsAnimTimer) { // still in kick
                         ...
                         && !(pm->ps->saber[0].saberFlags &
                              SFL_NO_FLIPS) // okay to do flips with this saber
                         &&
                         (!pm->ps->dualSabers ||
  ...
  if (pm->ps->gravity <=
    ...
    pm->trace(&trace, pm->ps->origin, pm->mins, pm->maxs, back,
              pm->ps->clientNum,
              pm->tracemask & ~(CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP),
              (EG2_Collision)0, 0);
    ...
    if (pm->ps->groundEntityNum ==
      ...
      return qtrue;
             ...
             && pm->ps->forcePowerLevel[FP_LEVITATION] >
                    FORCE_LEVEL_0 // have force jump ability
             && !(pm->ps->pm_flags &
                  PMF_JUMP_HELD) // not holding jump from a previous jump
             //&& !PM_InKnockDown( pm->ps )//not in a knockdown
             && pm->ps->forceRageRecoveryTime <
                    pm->cmd.serverTime // not in a force Rage recovery period
             && pm->gent &&
             WP_ForcePowerAvailable(pm->gent, FP_LEVITATION,
                                    0) // have enough force power to jump
             &&
                                            ...
                                            // make one
  {
    if (pm->gent->NPC && pm->gent->NPC->rank != RANK_CREWMAN &&
        pm->gent->NPC->rank <=
            RANK_LT_JG) { // reborn who are not acrobats can't do any of these
                          // acrobatics
      ...
      if ((pm->ps->weapon == WP_SABER &&
           G_TryingCartwheel(pm->gent,
                             &pm->cmd) /*(pm->cmd.buttons&BUTTON_FORCE_FOCUS)*/
           &&
           (pm->cmd.buttons &
            BUTTON_ATTACK)) // using saber and holding focus + attack
          //					  ||(pm->ps->weapon!=WP_SABER&&((pm->cmd.buttons&BUTTON_ATTACK)||(pm->cmd.buttons&BUTTON_ALT_ATTACK))
          //) )//using any other weapon and hitting either attack button
          &&
          (((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) &&
            pm->cmd.upmove > 0 &&
            pm->ps->velocity[2] >= 0) // jumping NPC, going up already
           || ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
               G_TryingCartwheel(
                   pm->gent,
      ...
      { // holding attack and jumping
        if (pm->cmd.rightmove > 0) {
          ...
          if (pm->ps->saberAnimLevel == SS_STAFF &&
            ...
            if ((pm->ps->clientNum >= MAX_CLIENTS &&
              ...
              forcePowerCostOverride =
                  G_CostForSpecialMove(SABER_ALT_ATTACK_POWER_LR);
            }
          } else if ((pm->ps->clientNum >= MAX_CLIENTS &&
                      !PM_ControlledByPlayer()) ||
            ...
            if (!(pm->ps->saber[0].saberFlags & SFL_NO_CARTWHEELS) &&
                (!pm->ps->dualSabers ||
                 !(pm->ps->saber[1].saberFlags &
                   SFL_NO_CARTWHEELS))) { // okay to do cartwheels with this
                                          // saber
              if (pm->ps->clientNum < MAX_CLIENTS ||
                  PM_ControlledByPlayer()) { // player: since we're on the
                                             // ground, always do a cartwheel
                /*
                anim = BOTH_CARTWHEEL_RIGHT;
                forcePowerCostOverride = G_CostForSpecialMove(
        ...
        if (pm->cmd.forwardmove > 0) { // wall-run
          if (!(pm->ps->saber[0].saberFlags & SFL_NO_WALL_RUNS) &&
              (!pm->ps->dualSabers ||
               !(pm->ps->saber[1].saberFlags &
                 SFL_NO_WALL_RUNS))) { // okay to do wall-runs with this saber
          ...
          if (!(pm->ps->saber[0].saberFlags & SFL_NO_WALL_FLIPS) &&
              (!pm->ps->dualSabers ||
               !(pm->ps->saber[1].saberFlags &
                 SFL_NO_WALL_FLIPS))) { // okay to do wall-flips with this saber
        ...
        if (pm->cmd.forwardmove > 0) { // wall-run
          ...
          if (!(pm->ps->saber[0].saberFlags & SFL_NO_WALL_FLIPS) &&
            ...
            anim = BOTH_WALL_FLIP_LEFT;
               ...
               &&*/ pm->cmd.forwardmove > 0 // pushing forward
                 && pm->ps->forcePowerLevel[FP_LEVITATION] >
                        FORCE_LEVEL_1) // have jump 2 or higher
      {                                // step off wall, flip backwards
        if (VectorLengthSquared(pm->ps->velocity) >
          ...
          if (pm->ps->forcePowerLevel[FP_LEVITATION] >
              FORCE_LEVEL_2) { // run all the way up wwall
            if (!(pm->ps->saber[0].saberFlags & SFL_NO_WALL_RUNS) &&
                (!pm->ps->dualSabers ||
                 !(pm->ps->saber[1].saberFlags &
                   SFL_NO_WALL_RUNS))) { // okay to do wall-runs with this saber
          ...
          } else { // run just a couple steps up
            if (!(pm->ps->saber[0].saberFlags & SFL_NO_WALL_FLIPS) &&
                (!pm->ps->dualSabers ||
                 !(pm->ps->saber[1].saberFlags &
                   SFL_NO_WALL_FLIPS))) { // okay to do wall-flips with this
                                          // saber
              vertPush = forceJumpStrength[FORCE_LEVEL_2] / 2.25f;
      ...
      { // back-jump does backflip... FIXME: always?!  What about just doing a
        // normal jump backwards?
        if (pm->ps->velocity[2] >= 0) { // must be going up already
          if (!(pm->ps->saber[0].saberFlags & SFL_NO_FLIPS) &&
              (!pm->ps->dualSabers ||
               !(pm->ps->saber[1].saberFlags &
                 SFL_NO_FLIPS))) { // okay to do backstabs with this saber
            vertPush = JUMP_VELOCITY;
            if (pm->gent->client &&
                pm->gent->client->NPC_class == CLASS_ALORA && !Q_irand(0, 2)) {
        ...
        if (pm->ps->weapon == WP_SABER && (pm->cmd.buttons & BUTTON_ATTACK)) {
          ...
          if (pm->ps->saber[0].jumpAtkUpMove != LS_INVALID) {
            if (pm->ps->saber[0].jumpAtkUpMove !=
                LS_NONE) { // actually overriding
              overrideJumpAttackUpMove =
                  (saberMoveName_t)pm->ps->saber[0].jumpAtkUpMove;
            } else if (pm->ps->dualSabers &&
                       pm->ps->saber[1].jumpAtkUpMove >
                           LS_NONE) { // would be cancelling it, but check the
                                      // second saber, too
              overrideJumpAttackUpMove =
                  (saberMoveName_t)pm->ps->saber[1].jumpAtkUpMove;
            } else { // nope, just cancel it
              overrideJumpAttackUpMove = LS_NONE;
            }
          } else if (pm->ps->dualSabers &&
                     pm->ps->saber[1].jumpAtkUpMove !=
                         LS_INVALID) { // first saber not overridden, check
                                       // second
            overrideJumpAttackUpMove =
                (saberMoveName_t)pm->ps->saber[0].jumpAtkUpMove;
          }
          if (overrideJumpAttackUpMove != LS_INVALID) { // do this move instead
            ...
            */
            if (pm->ps->clientNum >= MAX_CLIENTS &&
                !PM_ControlledByPlayer()) // NOTE: pretty much useless, so
                                          // player never does these
      ...
      if (anim != -1 && PM_HasAnimation(pm->gent, anim)) {
        ...
        if (doTrace) {
          ...
          pm->trace(&trace, pm->ps->origin, mins, maxs, traceto,
                    pm->ps->clientNum, contents, (EG2_Collision)0, 0);
          ...
          if (anim == BOTH_WALL_FLIP_LEFT) { // sigh.. check for bottomless pit
            ...
            if (anim == BOTH_WALL_FLIP_BACK1 ||
                anim ==
                    BOTH_FORCEWALLRUNFLIP_START) { // trace up and forward a
                                                   // little to make sure the
                                                   // wall it at least 64 tall
              ...
              { // no need to trace up, it's all good...
                ...
                pm->trace(&trace2, start, mins, maxs, traceto,
                          pm->ps->clientNum, contents, (EG2_Collision)0, 0);
                if (trace2.allsolid || trace2.startsolid ||
                    trace2.fraction >= 1.0f) { // no room above or no wall in
                                               // front at that height
                  trace.fraction = 1.0f; // way to stop if from doing the jump
                }
              }
            }
            if (trace.fraction < 1.0f) {          // still valid to jump
              if (anim == BOTH_WALL_FLIP_BACK1) { // sigh.. check for bottomless
                                                  // pit to the rear
                trace_t trace2;
                vec3_t start;
                ...
                if (!trace2.allsolid && !trace2.startsolid) {
                  ...
                  if (!trace2.allsolid && !trace2.startsolid &&
                      trace2.fraction >= 1.0f) { // bottomless pit!
                    trace.fraction =
                        1.0f; // way to stop it from doing the side-flip
                  }
                }
              }
            }
        ...
        if (!doTrace || (trace.fraction < 1.0f &&
                         ((trace.entityNum < ENTITYNUM_WORLD && traceEnt &&
                          ...
                          DotProduct(wallNormal, idealNormal) >
          ...
          if ((anim != BOTH_WALL_RUN_LEFT && anim != BOTH_WALL_RUN_RIGHT &&
              ...
              (wallNormal[2] >= 0.0f &&
               wallNormal[2] <=
                   MAX_WALL_RUN_Z_NORMAL)) { // wall-runs can only run on
                                             // relatively flat walls, sorry.
            if (anim == BOTH_ARIAL_LEFT || anim == BOTH_CARTWHEEL_LEFT) {
              pm->ps->velocity[0] = pm->ps->velocity[1] = 0;
              VectorMA(pm->ps->velocity, -185, right, pm->ps->velocity);
            } else if (anim == BOTH_ARIAL_RIGHT ||
                       anim == BOTH_CARTWHEEL_RIGHT) {
              pm->ps->velocity[0] = pm->ps->velocity[1] = 0;
              VectorMA(pm->ps->velocity, 185, right, pm->ps->velocity);
            ...
            // move me to side
              ...
              VectorMA(pm->ps->velocity, -150, right, pm->ps->velocity);
                       ...
                       anim == BOTH_FLIP_BACK3 || anim == BOTH_ALORA_FLIP_B ||
                       anim == BOTH_WALL_FLIP_BACK1) {
              pm->ps->velocity[0] = pm->ps->velocity[1] = 0;
            ...
            if (doTrace && anim != BOTH_WALL_RUN_LEFT &&
              ...
              if (pm->gent && trace.entityNum < ENTITYNUM_WORLD) {
                if (traceEnt && traceEnt->client && traceEnt->health > 0 &&
                    ...
                    !(traceEnt->flags &
                      FL_NO_KNOCKBACK)) { // push them away and do pain
                  vec3_t oppDir, fxDir;
                  ...
                  // va("sound/weapons/melee/punch%d", Q_irand(1, 4)) ) );
                  if (traceEnt->health > 0) { // didn't kill him
                    if ((traceEnt->s.number == 0 &&
                         !Q_irand(0, g_spskill->integer)) ||
                        (traceEnt->NPC != NULL &&
                         Q_irand(RANK_CIVILIAN, traceEnt->NPC->rank) +
                                 Q_irand(-2, 2) <
                             RANK_ENSIGN)) {
            ...
            } else { // not a proper saberMove, so do set all the details
                     // manually
      ...
      if (legsAnim == BOTH_WALL_RUN_LEFT ||
        ...
        if (pm->cmd.upmove != 0) { // jump failed, so don't try to do normal
          ...
          return qfalse;
                 ...
                 BOTH_FORCEWALLRUNFLIP_START) { // want to jump off wall
        vec3_t fwd, traceto, mins = {pm->mins[0], pm->mins[0], 0},
                             maxs = {pm->maxs[0], pm->maxs[0], 24},
                             fwdAngles = {0, pm->ps->viewangles[YAW], 0};
        trace_t trace;
        int anim = -1;
        ...
        if (anim != -1) {
          ...
          if (trace.fraction < 1.0f) { // flip off wall
            pm->ps->velocity[0] *= 0.5f;
            pm->ps->velocity[1] *= 0.5f;
            VectorMA(pm->ps->velocity, WALL_RUN_UP_BACKFLIP_SPEED, fwd,
                     pm->ps->velocity);
      ...
      */
      /*
              ...
              && pm->ps->forceRageRecoveryTime < pm->cmd.serverTime	//not in a
      force Rage recovery period
              && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1 //have
      force jump 2 or higher
              && (level.time - pm->ps->lastOnGround) <= 250//just jumped
              && (pm->ps->legsAnim == BOTH_JUMP1 || pm->ps->legsAnim ==
      BOTH_INAIR1 )//not in a flip or spin or anything
      {//run up wall, flip backwards
              //FIXME: have to be moving... make sure it's opposite the wall...
      or at least forward? int wallWalkAnim = BOTH_WALL_FLIP_BACK1; int parts =
      SETANIM_LEGS; int contents = CONTENTS_SOLID; qboolean kick = qtrue; if (
      pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_2 )
              {
      ...
      maxs = {pm->maxs[0],pm->maxs[1],24}, fwdAngles = {0,
      pm->ps->viewangles[YAW], 0}; trace_t	trace; vec3_t	idealNormal;

                      AngleVectors( fwdAngles, fwd, NULL, NULL );
                      VectorMA( pm->ps->origin, 32, fwd, traceto );

                      pm->trace( &trace, pm->ps->origin, mins, maxs, traceto,
      pm->ps->clientNum, contents );//FIXME: clip brushes too? VectorSubtract(
      pm->ps->origin, traceto, idealNormal ); VectorNormalize( idealNormal );
                      gentity_t *traceEnt = &g_entities[trace.entityNum];
                      ...
                      {//there is a wall there
                              ...
                              {
                                      pm->ps->velocity[2] =
      forceJumpStrength[FORCE_LEVEL_3]/2.0f;
                              }
                              else
                              {
                                      VectorMA( pm->ps->velocity, -150, fwd,
      pm->ps->velocity );
                              }
                              //animate me
                              PM_SetAnim( pm, parts, wallWalkAnim,
      SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0 ); pm->ps->forceJumpZStart =
      pm->ps->origin[2];//so we don't take damage if we land at same height
                              pm->ps->pm_flags |= PMF_JUMPING|PMF_SLOW_MO_FALL;
                              pm->cmd.upmove = 0;
                              G_SoundOnEnt( pm->gent, CHAN_BODY,
      "sound/weapons/force/jump.wav" ); WP_ForcePowerDrain( pm->gent,
      FP_LEVITATION, 0 );
                              //kick if jumping off an ent
                              if ( kick && pm->gent && trace.entityNum <
      ENTITYNUM_WORLD )
                              {
                                      if ( traceEnt
                                              && traceEnt->client
      ...
      !(traceEnt->flags&FL_NO_KNOCKBACK) )
                                      {//push them away and do pain
                                              vec3_t oppDir;
                                              float strength = VectorNormalize2(
      pm->ps->velocity, oppDir ); VectorScale( oppDir, -1, oppDir );
                                              //FIXME: need knockdown anim
                                              G_Damage( traceEnt, pm->gent,
      pm->gent, oppDir, traceEnt->currentOrigin, 10,
      DAMAGE_NO_ARMOR|DAMAGE_NO_HIT_LOC|DAMAGE_NO_KNOCKBACK, MOD_MELEE );
                                              G_Sound( traceEnt, G_SoundIndex(
      va("sound/weapons/melee/punch%d", Q_irand(1, 4)) ) ); if ( trace.fraction
      <= 0.5f )
                                              {//close to him
                                                      ...
                                                      {//didn't kill him
                                                              if (
      (traceEnt->s.number==0&&!Q_irand(0,g_spskill->integer))
                                                                      ||
      (traceEnt->NPC!=NULL&&Q_irand(RANK_CIVILIAN,traceEnt->NPC->rank)+Q_irand(-2,2)<RANK_ENSIGN)
      )
                                                              {
                                                                      NPC_SetAnim(
      traceEnt, SETANIM_BOTH, BOTH_KNOCKDOWN2,
      SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD ); G_Throw( traceEnt, oppDir,
      strength );
                                                              }
                                                      }
                                              }
      ...
      */
                ...
                || PM_InBackFlip(legsAnim))   // a backflip (needed so you can
                                            // jump off a wall behind you)
               //&& pm->ps->velocity[2] <= 0
               && pm->ps->velocity[2] > -1200 // not falling down very fast
               &&
               !(pm->ps->pm_flags &
                 PMF_JUMP_HELD) // have to have released jump since last press
               && (pm->cmd.forwardmove ||
                   pm->cmd.rightmove) // pushing in a direction
               //&& pm->ps->forceRageRecoveryTime < pm->cmd.serverTime //not in
               //a force Rage recovery period
               && pm->ps->forcePowerLevel[FP_LEVITATION] >
                      FORCE_LEVEL_2 // level 3 jump or better
               && pm->ps->forcePower >
                      10 // have enough force power to do another one
               &&
               (level.time - pm->ps->lastOnGround) >
                   250 // haven't been on the ground in the last 1/4 of a second
               &&
               (!(pm->ps->pm_flags & PMF_JUMPING) // not jumping
                || ((level.time - pm->ps->lastOnGround) >
                        250 // we are jumping, but have been in the air for at
                            // least half a second
                    && (g_debugMelee->integer // if you know kung fu, no height
                                              // cap on wall-grab-jumps
                        || ((pm->ps->origin[2] - pm->ps->forceJumpZStart) <
                            (forceJumpHeightMax[FORCE_LEVEL_3] -
                             (G_ForceWallJumpStrength() /
                              2.0f)))) // can fit at least one more wall jump in
                                       // (yes, using "magic numbers"... for now)
                    ))
               //&& (pm->ps->legsAnim == BOTH_JUMP1 || pm->ps->legsAnim ==
               //BOTH_INAIR1 ) )//not in a flip or spin or anything
        ...
        if (!(pm->ps->saber[0].saberFlags & SFL_NO_WALL_GRAB) &&
            (!pm->ps->dualSabers ||
             !(pm->ps->saber[1].saberFlags &
               SFL_NO_WALL_GRAB))) { // okay to do wall-grabs with this saber
          ...
          vec3_t checkDir, traceto, mins = {pm->mins[0], pm->mins[1], 0},
                                    maxs = {pm->maxs[0], pm->maxs[1], 24},
                                    fwdAngles = {0, pm->ps->viewangles[YAW], 0};
          trace_t trace;
          vec3_t idealNormal;
          ...
          if (anim != -1) { // trace in the dir we're pushing in and see if
                            // there's a vertical wall there
            VectorMA(pm->ps->origin, 16, checkDir, traceto); // was 8
            pm->trace(&trace, pm->ps->origin, mins, maxs, traceto,
                      pm->ps->clientNum, CONTENTS_SOLID, (EG2_Collision)0,
  ...
  if (pm->gent
    ...
    if (!PM_RollingAnim(pm->ps->legsAnim) && !PM_InKnockDown(pm->ps) &&
        ...
        !PM_SaberInSpecialAttack(
            pm->ps->torsoAnim)) { // HMM... do NPCs need this logic?
      ...
      { // not in an attack or finishing/starting/transitioning one
        ...
        {
                PM_SetSaberMove( PM_SaberDualJumpAttackMove() );//jump forward
        sideways flip attack
        }
        */
      }
      /*
      else if ( ( PM_SaberInTransitionAny( pm->ps->saberMove ) ||
      ...
      (animNumber_t)pm->ps->torsoAnim ); if ( animLength -
      pm->ps->torsoAnimTimer < 500 )
...
static void PM_FlyVehicleMove(void) {
  ...
  // We don't use these here because we pre-calculate the movedir in the vehicle
  // update anyways, and if you leave this, you get strange motion during
  // boarding (the player can move the vehicle).
  ...
  if (pm->ps->clientNum &&
      (USENEWNAVSYSTEM ||
       !VectorCompare(pm->ps->moveDir, vec3_origin))) { // NPC

    // If The UCmds Were Set, But Never Converted Into A MoveDir, Then Make The
    // WishDir From UCmds
    //--------------------------------------------------------------------------------------------
    if ((fmove != 0.0f || smove != 0.0f) &&
        VectorCompare(pm->ps->moveDir, vec3_origin)) {
...
static void PM_FlyMove(void) {
  ...
  if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
      ...
      (pm->gent->client->NPC_class == CLASS_BOBAFETT ||
       pm->gent->client->NPC_class == CLASS_ROCKETTROOPER) &&
      pm->gent->client->moveType == MT_FLYSWIM) { // jetpack accel
...
static void PM_AirMove(void) {
  ...
  if (pVeh && pVeh->m_pVehicleInfo->hoverHeight >
                  0) { // in a hovering vehicle, have air control
  ...
  } else if ((pm->ps->pm_flags & PMF_SLOW_MO_FALL)) { // no air-control
    VectorClear(wishvel);
  } else {
    for (i = 0; i < 2; i++) {
      wishvel[i] = pml.forward[i] * fmove + pml.right[i] * smove;
  ...
  if (pVeh && pVeh->m_pVehicleInfo->type ==
                  VH_SPEEDER) { // speeders have more control in air
    // in mid-air
    accelerate = pVeh->m_pVehicleInfo->acceleration;
    if (pml.groundPlane) { // on a slope of some kind, shouldn't have much
                           // control and should slide a lot
      accelerate *= 0.5f;
    }
  ...
  if (pVeh && pm->ps->pm_flags & PMF_BUMPED) {
    ...
    /*
                    ...
                    nAngles[0] =
       pVeh->m_pParentEntity->client->ps.viewangles[0]; nAngles[2] =
       pVeh->m_pParentEntity->client->ps.viewangles[2];
...
static void PM_WalkMove(void) {
  ...
  if (g_debugMelee->integer) {
    if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) // player
        ...
        && ((pm->cmd.buttons & BUTTON_USE) ||
            pm->ps->leanStopDebounceTime) // holding use or leaning
  ...
  /*	if ( ( pm->gent && pm->gent->client && pm->gent->client->NPC_class ==
     CLASS_VEHICLE ) && pm->gent->NPC )
          {//speeder control scheme
                  vec3_t	vfwd, vrt;
                  AngleVectors( ((CVehicleNPC *)pm->gent->NPC)->m_vOrientation,
     vfwd, vrt, NULL );
                  ...
                  {//going backwards
                          if ( speed < 0 )
                          {//speed is negative, but since our command is
     reverse, make speed positive speed = fabs( speed );
                          }
                          else if ( speed > 0 )
                          {//trying to move back but speed is still positive, so
     keep moving forward (we'll slow down eventually) speed *= -1;
                          }
                  }
...
static void PM_DeadMove(void) {
  ...
  // If this is a vehicle, tell him he's dead, but give him a little while to do
  // his things.
  /*	if ( pm->gent && pm->gent->client && pm->gent->client->NPC_class ==
     CLASS_VEHICLE && pm->gent->NPC && pm->gent->health != -99999 )
          {
                  pm->gent->health = 1;
...
static float PM_DamageForDelta(int delta) {
  ...
  if (pm->gent->NPC) {
    if (pm->ps->weapon == WP_SABER ||
        (pm->gent->client &&
         pm->gent->client->NPC_class ==
             CLASS_REBORN)) { // FIXME: for now Jedi take no falling damage, but
...
static qboolean PM_TryRoll(void) {
  ...
  if (pm->ps->dualSabers && (pm->ps->saber[1].saberFlags & SFL_NO_ROLLS)) {
    return qfalse;
  }
  if (pm->ps->clientNum && pm->gent->NPC) { // NPC
    if (pm->gent->NPC->scriptFlags &
        SCF_NO_ACROBATICS) { // scripted to never do acrobatics
      return qfalse;
    }

    if (pm->ps->weapon == WP_SABER) { // jedi/reborn
      if (pm->gent->NPC->rank != RANK_CREWMAN &&
          pm->gent->NPC->rank <
              RANK_LT_JG) { // reborn/jedi who are not acrobats or fencers can't
                            // do any of these acrobatics
        return qfalse;
      }
    } else {                         // non-jedi/reborn
      if (pm->ps->weapon != WP_NONE) // not empty-handed...who would that be???
      { // only jedi/reborn NPCs should be able to do rolls (with a few
        // exceptions)
        if (!pm->gent || !pm->gent->client ||
            (pm->gent->client->NPC_class !=
                 CLASS_BOBAFETT // boba can roll with it, baby
             && pm->gent->client->NPC_class !=
                    CLASS_REBORN // reborn using weapons other than saber can
                                 // still roll
             )) { // can't roll
          return qfalse;
        }
    ...
    if (!roll) {
      ...
      if (trace.fraction >= 1.0f) { // okay, clear, check for a bottomless drop
        ...
        pm->trace(&trace, top, mins, maxs, traceto, pm->ps->clientNum,
                  CONTENTS_SOLID, (EG2_Collision)0, 0);
...
static void PM_CrashLandEffect(void) {
  ...
  float delta = fabs(pml.previous_velocity[2]) /
                10; // VectorLength( pml.previous_velocity );?
  if (delta >= 30) {
    vec3_t bottom = {pm->ps->origin[0], pm->ps->origin[1],
                     pm->ps->origin[2] + pm->mins[2] + 1};
    CG_LandingEffect(bottom, pml.groundTrace.plane.normal,
                     (pml.groundTrace.surfaceFlags & MATERIAL_MASK));
  }
}
/*
=================
...
static void PM_CrashLand(void) {
  ...
  if (pm->gent && pm->gent->client &&
    ...
    if (pm->gent->m_pVehicle->m_pVehicleInfo->type != VH_ANIMAL) {
      ...
      if (dot < -100.0f) {
        ...
        if (pm->gent->m_pVehicle->m_pVehicleInfo->iImpactFX) { // make sparks
          if (!Q_irand(0, 3)) { // FIXME: debounce
            G_PlayEffect(pm->gent->m_pVehicle->m_pVehicleInfo->iImpactFX,
                         pm->ps->origin, pml.groundTrace.plane.normal);
          }
        }
    ...
    if (pm->gent && pm->gent->NPC &&
      ...
      if (JET_Flying(pm->gent)) {
        if (pm->gent->client->NPC_class == CLASS_BOBAFETT ||
            (pm->gent->client->NPC_class == CLASS_ROCKETTROOPER &&
             pm->gent->NPC && pm->gent->NPC->rank < RANK_LT)) {
          JET_FlyStop(pm->gent);
        } else {
      ...
      delta = 1;
               ...
               (pm->ps->forcePowerLevel[FP_LEVITATION] >= FORCE_LEVEL_1 ||
                (pm->ps->clientNum < MAX_CLIENTS ||
                 PM_ControlledByPlayer()))) {        // we were force-jumping
  ...
  // If he just entered the water (from a fall presumably), absorb some of the
  // impact.
  if (pm->waterlevel >= 2) {
    delta *= 0.4f;
  }
  ...
  if (!PM_InDeathAnim()) {
    if (PM_InAirKickingAnim(pm->ps->legsAnim) &&
      ...
      if (anim != -1) { // interrupting a kick clears everything
        PM_SetAnim(pm, SETANIM_BOTH, anim,
                   SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD,
                   100); // Only blend over 100ms
        ...
        if (!g_allowBunnyhopping->integer) {
          ...
          pm->ps->velocity[1] *= 0.5f;
               ...
               pm->gent->client->NPC_class ==
                   CLASS_ROCKETTROOPER) { // rockettroopers are simpler
      int anim = PM_GetLandingAnim();
      if (anim != -1) {
        if (pm->gent->NPC &&
            pm->gent->NPC->rank <
                RANK_LT) { // special case: ground-based rocket troopers
                           // *always* play land anim on whole body
          PM_SetAnim(pm, SETANIM_BOTH, anim,
                     SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD,
                     100); // Only blend over 100ms
        } else {
    ...
    } else if (pm->cmd.upmove >= 0 && !PM_InKnockDown(pm->ps) &&
        ...
        if (pm->gent && pm->gent->client &&
          ...
          if (anim != -1) {
            if (PM_FlippingAnim(pm->ps->torsoAnim) ||
                ...
                pm->ps->torsoAnim ==
                    BOTH_FLIP_LAND) { // interrupt these if we're going to play
                                      // a land
            ...
            if (anim == BOTH_FORCELONGLEAP_LAND) {
              ...
              PM_SetAnim(pm, SETANIM_BOTH, anim,
                       ...
                       pm->ps->torsoAnim ==
                           pm->ps->legsAnim) { // interrupting a kick clears
                                               // everything
              PM_SetAnim(pm, SETANIM_BOTH, anim,
                         SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD,
                         100); // Only blend over 100ms
              ...
              pm->ps->weaponTime = 0;
                       ...
                       pm->ps->torsoAnim ==
                           pm->ps
                               ->legsAnim) { // special case: if land during one
                                             // of these, set the torso, too, if
    ...
    if (pm->gent) {
      if ((!(pml.groundTrace.surfaceFlags & SURF_NODAMAGE)) &&
        ...
        if (pm->waterlevel <
          ...
          deadFallSound = qtrue;
          if (!(pm->ps->eFlags & EF_NODRAW)) { // no sound if no draw
            if (delta >= 75) {
              G_SoundOnEnt(pm->gent, CHAN_BODY, "sound/player/fallsplat.wav");
            } else {
              G_SoundOnEnt(
                  pm->gent, CHAN_BODY,
                  va("sound/player/bodyfall_human%d.wav", Q_irand(1, 3)));
            }
          }
        } else {
          G_SoundOnEnt(pm->gent, CHAN_BODY,
                       va("sound/player/bodyfall_water%d.wav", Q_irand(1, 3)));
        }
    ...
    if (pm->ps->legsAnim == BOTH_FALLDEATH1 ||
      ...
      AddSoundEvent(pm->gent, pm->ps->origin, 256, AEL_SUSPICIOUS, qfalse,
                    qtrue);
      return;
    }
  }
  ...
  if (delta >= 75) {
    ...
    if (pm->ps->stats[STAT_HEALTH] > 0) {
      if (!deadFallSound) {
        if (!(pm->ps->eFlags & EF_NODRAW)) { // no sound if no draw
          PM_AddEvent(
              EV_FALL_MEDIUM); // damage is dealt in g_active, ClientEvents
        }
      }
      if (pm->gent) {
        if (!(pml.groundTrace.surfaceFlags & SURF_NODAMAGE)) {
    ...
    if (!deadFallSound) {
      if (!(pm->ps->pm_flags & PMF_DUCKED) ||
          !Q_irand(0,
                   3)) { // only 25% chance of making landing alert when ducked
        AddSoundEvent(pm->gent, pm->ps->origin, 32, AEL_MINOR, qfalse, qtrue);
      }
      if (!(pm->ps->eFlags & EF_NODRAW)) { // no sound if no draw
...
qboolean PM_RocketeersAvoidDangerousFalls(void) {
  if (pm->gent->NPC && pm->gent->client &&
      (pm->gent->client->NPC_class == CLASS_BOBAFETT ||
       pm->gent->client->NPC_class ==
           CLASS_ROCKETTROOPER)) { // fixme:  fall through if jetpack broken?
    if (JET_Flying(pm->gent)) {
      if (pm->gent->client->NPC_class == CLASS_BOBAFETT) {
        pm->gent->client->jetPackTime = level.time + 2000;
        // Wait, what if the effect is already playing, how do we know???
        // G_PlayEffect( G_EffectIndex( "boba/jetSP" ), pm->gent->playerModel,
        // pm->gent->genericBolt1, pm->gent->s.number, pm->gent->currentOrigin,
        // pm->gent->client->jetPackTime-level.time );
      } else {
...
static void PM_FallToDeath(void) {
  ...
  if (pm->gent->client->NPC_class == CLASS_VEHICLE &&
      pm->gent->m_pVehicle->m_pVehicleInfo->type == VH_ANIMAL) {
...
static void PM_SetVehicleAngles(vec3_t normal) {
  ...
  if (vehicleBankingSpeed <= 0 ||
      (pVeh->m_pVehicleInfo->pitchLimit <= 0 &&
       pVeh->m_pVehicleInfo->rollLimit <=
           0)) { // don't bother, this vehicle doesn't bank
    return;
  }
  // FIXME: do 3 traces to define a plane and use that... smoothes it out some,
  // too... pitch_roll_for_slope( pm->gent, normal, vAngles );
  ...
  // float pitchBias = 90.0f*pVeh->m_pVehicleInfo->centerOfGravity[0];//if
  // centerOfGravity is all the way back (-1.0f), vehicle pitches up 90 degrees
  // when in air

  VectorClear(vAngles);

  if (pm->waterlevel > 0 ||
      (normal && (pml.groundTrace.contents & (CONTENTS_WATER | CONTENTS_SLIME |
                                              CONTENTS_LAVA)))) { // in water
    //	vAngles[PITCH] += (pm->ps->viewangles[PITCH]-vAngles[PITCH])*0.75f +
    //(pitchBias*0.5);
  ...
  if (pVeh->m_ulFlags & VEH_SPINNING) {
    ...
    if (speed > 0.01f) {
      ...
      // if (fabsf(dotp)>0.5f)
      {
        ...
        // Magic number fun!  Speed is used for banking, so modulate the speed
        // by a sine wave
  ...
  // do it
  for (int i = 0; i < 3; i++) {
    if (i == YAW) { // yawing done elsewhere
      continue;
    }

    if (i == ROLL &&
        pVeh->m_ulFlags &
            VEH_STRAFERAM) { // during strafe ram, roll is done elsewhere
      continue;
    }
    ...
    {
            curVehicleBankingSpeed =
    vehicleBankingSpeed*fabs(AngleNormalize180(AngleSubtract( vAngles[PITCH],
    pVeh->m_vOrientation[PITCH]
    )))/(g_vehicleInfo[pm->ps->vehicleIndex].pitchLimit/2.0f);
    }
    else if ( i == ROLL )
    {
            curVehicleBankingSpeed =
    vehicleBankingSpeed*fabs(AngleNormalize180(AngleSubtract( vAngles[ROLL],
    pVeh->m_vOrientation[ROLL]
    )))/(g_vehicleInfo[pm->ps->vehicleIndex].rollLimit/2.0f);
    }
    ...
    */
    {
      ...
      //	}
      //	else if ( pVeh->m_vOrientation[i] <= vAngles[i] -
      //vehicleBankingSpeed )
      //	{
      //		pVeh->m_vOrientation[i] += vehicleBankingSpeed;
      //	}
...
void BG_ExternThisSoICanRecompileInDebug(
    Vehicle_t *pVeh,
    playerState_t *
        riderPS) { /*
                          float pitchSubtract, pitchDelta, yawDelta;
                          //Com_Printf( S_COLOR_RED"PITCH: %4.2f, YAW: %4.2f,
                  ROLL: %4.2f\n",
                  riderPS->viewangles[0],riderPS->viewangles[1],riderPS->viewangles[2]);
                          yawDelta =
                  AngleSubtract(riderPS->viewangles[YAW],pVeh->m_vPrevRiderViewAngles[YAW]);
                  #ifndef QAGAME
                  ...
                  pm->cmd.serverTime, yawDelta );
                          ...
                          pitchDelta =
                  AngleSubtract(riderPS->viewangles[PITCH],pVeh->m_vPrevRiderViewAngles[PITCH]);
                          pitchDelta *= (2.0f*pVeh->m_fTimeModifier);
                          pitchSubtract = pitchDelta *
                  (fabs(pVeh->m_vOrientation[ROLL])/90.0f);
                          pVeh->m_vOrientation[PITCH] +=
                  pitchDelta-pitchSubtract; if ( pVeh->m_vOrientation[ROLL] > 0
                  )
                          {
                          ...
                          pVeh->m_vOrientation[PITCH] = AngleNormalize180(
                  pVeh->m_vOrientation[PITCH] ); pVeh->m_vOrientation[YAW] =
                  AngleNormalize360( pVeh->m_vOrientation[YAW] );
                          pVeh->m_vOrientation[ROLL] = AngleNormalize180(
                  pVeh->m_vOrientation[ROLL] );

                          VectorCopy( riderPS->viewangles,
                  pVeh->m_vPrevRiderViewAngles );*/
}

void BG_VehicleTurnRateForSpeed(Vehicle_t *pVeh, float speed,
...
static void PM_GroundTraceMissed(void) {
  ...
  if (Flying != FLY_HOVER) {
    if (!(pm->ps->eFlags & EF_FORCE_DRAINED)) {
      // FIXME: if in a contents_falldeath brush, play the falling death anim
      // and sound?
      if (pm->ps->clientNum != 0 && pm->gent && pm->gent->NPC &&
          pm->gent->client &&
          pm->gent->client->NPC_class !=
              CLASS_DESANN) // desann never falls to his death
      {
        if (pm->ps->groundEntityNum == ENTITYNUM_NONE) {
          if (pm->ps->stats[STAT_HEALTH] > 0 &&
              ...
              pm->gent->NPC->behaviorState !=
                  BS_JUMP) // not being scripted to jump
          {
            if ((level.time - pm->gent->client->respawnTime >
                ...
                pm->gent->e_ThinkFunc !=
                    thinkF_NPC_RemoveBody // Have to do this now because
                                          // timeOfDeath is used by
                                          // thinkF_NPC_RemoveBody to debounce
                                          // removal checks
                && !(pm->gent->client->ps.forcePowersActive &
                     (1 << FP_LEVITATION))) {
                ...
                if (!(pm->gent->flags & FL_UNDYING) &&
                  ...
                  if (!(pm->ps->eFlags & EF_FORCE_GRIPPED) &&
                    ...
                    if (!pm->ps->forceJumpZStart ||
                        pm->ps->forceJumpZStart >
                            pm->ps->origin
                                [2]) // && fabs(pm->ps->velocity[0])<10 &&
                                     // fabs(pm->ps->velocity[1])<10 &&
                                     // pm->ps->velocity[2]<0)//either not
                                     // force-jumping or force-jumped and now
                                     // fell below original jump start height
                    {
                      /*if ( pm->ps->legsAnim = BOTH_FALLDEATH1
                      ...
                      if ((trace.contents & CONTENTS_LAVA) &&
                          PM_RocketeersAvoidDangerousFalls()) { // got out of it
                                     ...
                                     128) //>=128 so we don't die on steps!
                      {
                        if (trace.fraction ==
                          ...
                          if (pm->ps->velocity[2] < 0 &&
                            ...
                            (!pm->gent || !pm->gent->client ||
                             (pm->gent->client->NPC_class != CLASS_BOBAFETT &&
                              ...
                              pm->gent->client->NPC_class !=
                                  CLASS_ROCKETTROOPER))) { // Jedi don't scream
                                                           // and die if they're
                                                           // heading for a hard
                                                           // impact
                          gentity_t *traceEnt = &g_entities[trace.entityNum];
                          if (trace.entityNum == ENTITYNUM_WORLD ||
                              (traceEnt &&
                               traceEnt->bmodel)) { // hit architecture, find
                                                    // impact force
                            float dmg;
                                ...
                                if (pm->gent->currentOrigin[2] >=
                                    pm->gent->client->ps
                                        .forceJumpZStart) { // we landed at same
                                                            ...
                                                            // than we landed
                                  dmg = 0;
                                } else { // FIXME: take off some of it, at
                                         // least?
                            ...
                            float maxDmg =
                                pm->ps->stats[STAT_HEALTH] > 20
                                    ...
                                    : 20; // a fall that would do less than 20
                                          // points of damage should never make
                                          // us scream to our deaths
                            if (pm->gent && pm->gent->client &&
                                pm->gent->client->NPC_class ==
                                    CLASS_HOWLER) { // howlers can survive long
                                                    // falls, despire their
                                                    // health
                              maxDmg *= 5;
                            }
                      ...
                      /*
                      vec3_t	start;
                      //okay, kind of expensive temp hack here, but let's check
                      to see if we should scream
                      //FIXME: we should either do a better check (predict using
                      actual velocity) or we should wait until they've been over
                      a bottomless pit for a certain amount of time...
                      VectorCopy( pm->ps->origin, start );
                      if ( pm->ps->forceJumpZStart < start[2] )
                      {//Jedi who are force-jumping should only do this from
                      landing point down? start[2] = pm->ps->forceJumpZStart;
                      }
                      VectorCopy( start, point );
                      point[2] -= 400;//320
                      pm->trace (&trace, pm->ps->origin, pm->mins, pm->maxs,
                      point, pm->ps->clientNum, pm->tracemask);
                      //FIXME: somehow, people can still get stuck on ledges and
                      not splat when hit...? if ( !trace.allsolid &&
      ...
      if (!cliff_fall) {
        if (pm->ps->legsAnim == BOTH_FORCELONGLEAP_START ||
            pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK ||
            pm->ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START ||
            pm->ps->legsAnim == BOTH_FLIP_LAND) { // let it stay on this anim
        } else if (PM_KnockDownAnimExtended(
                       pm->ps->legsAnim)) { // no in-air anims when in knockdown
                                            // anim
                   ...
                   !pm->ps->legsAnimTimer) { // if flip anim is done, okay to
                                             // use inair
          ...
          if (!pm->ps->legsAnimTimer) { // done?
            PM_SetAnim(pm, SETANIM_BOTH, BOTH_FLIP_HOLD7,
                       SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD,
                       350); // Only blend over 100ms
          }
        } else if (PM_InCartwheel(pm->ps->legsAnim)) {
          if (pm->ps->legsAnimTimer > 0) { // still playing on bottom
          } else {
          ...
          if (pm->ps->groundEntityNum != ENTITYNUM_NONE) {
            ...
            // if they aren't in a jumping animation and the ground is a ways
            // away, force into it if we didn't do the trace, the player would
            // be backflipping down staircases
            VectorCopy(pm->ps->origin, point);
            point[2] -= 64;

            pm->trace(&trace, pm->ps->origin, pm->mins, pm->maxs, point,
                      pm->ps->clientNum, pm->tracemask, (EG2_Collision)0, 0);
              ...
              if (pm->ps->velocity[2] <= 0 &&
                ...
                if (!PM_InDeathAnim()) {
                  ...
                  if (pm->gent && pm->gent->client &&
                    ...
                    if (pm->gent->m_pVehicle->m_pVehicleInfo->type ==
                        VH_ANIMAL) {
                ...
                if (pm->cmd.forwardmove >= 0) {
                  if (!PM_InDeathAnim()) {
                    PM_SetAnim(pm, SETANIM_LEGS, BOTH_JUMP1,
                               SETANIM_FLAG_OVERRIDE,
                               100); // Only blend over 100ms
                  }
                  pm->ps->pm_flags &= ~PMF_BACKWARDS_JUMP;
...
static void PM_GroundTrace(void) {
  ...
  if ((pm->ps->eFlags & EF_LOCKED_TO_WEAPON) ||
      ...
      (pm->ps->eFlags & EF_HELD_BY_SAND_CREATURE) || PM_RidingVehicle()) {
    ...
    */
    return;
             ...
             (pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT ||
              pm->ps->legsAnim == BOTH_WALL_RUN_LEFT ||
              pm->ps->legsAnim ==
                  BOTH_FORCEWALLRUNFLIP_START)) { // wall-running forces you to
                                                  // be in the air
    pml.groundPlane = qfalse;
    pml.walking = qfalse;
    pm->ps->groundEntityNum = ENTITYNUM_NONE;
  ...
  if (pm->gent && pm->gent->client &&
      pm->gent->client->NPC_class ==
          CLASS_VEHICLE) { // FIXME: extern this as part of vehicle data
  ...
  if (pm->gent && pm->gent->client &&
    ...
    if (((pm->ps->velocity[2] > 0 && (pm->ps->pm_flags & PMF_TIME_KNOCKBACK)) ||
      ...
      } else if (PM_KickingAnim(pm->ps->legsAnim)) {   // in kick
      } else if (pm->gent && pm->gent->client &&
                 (pm->gent->client->NPC_class == CLASS_RANCOR ||
                  pm->gent->client->NPC_class == CLASS_WAMPA)) {
      } else {
  ...
  */
  ...
  pm->ps->lastOnGround = level.time;
  if ((pm->ps->clientNum < MAX_CLIENTS ||
       PM_ControlledByPlayer())) { // if a player, clear the jumping "flag" so
                                   // can't double-jump
...
void PM_HoverTrace(void) {
  ...
  float relativeWaterLevel =
      (pm->ps->waterheight - (pm->ps->origin[2] + pm->mins[2]));
  if (pm->waterlevel && relativeWaterLevel >= 0) { // in water
    if (pVeh->m_pVehicleInfo->bouyancy <= 0.0f) {  // sink like a rock
    } else {                                       // rise up
      float floatHeight =
          (pVeh->m_pVehicleInfo->bouyancy *
           ((pm->maxs[2] - pm->mins[2]) * 0.5f)) -
          (hoverHeight *
           0.5f); // 1.0f should make you float half-in, half-out of water
      if (relativeWaterLevel > floatHeight) { // too low, should rise up
        pm->ps->velocity[2] +=
            (relativeWaterLevel - floatHeight) * pVeh->m_fTimeModifier;
      }
    }
    if (pm->ps->waterheight <
        pm->ps->origin[2] +
            pm->maxs[2]) { // part of us is sticking out of water
      if (fabs(pm->ps->velocity[0]) + fabs(pm->ps->velocity[1]) >
          100) {                                 // moving at a decent speed
    ...
    // FIXME: check for water, too?  If over water, go slower and make wave
    // effect 		If *in* water, go really slow and use bouyancy stat to determine
    //how far below surface to float

    // NOTE: if bouyancy is 2.0f or higher, you float over water like it's solid
    // ground. 		if it's 1.0f, you sink halfway into water.  If it's 0, you sink...
    if (pVeh->m_pVehicleInfo->bouyancy >= 2.0f) { // sit on water
      traceContents |= (CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA);
    }
    pm->trace(trace, pm->ps->origin, pm->mins, pm->maxs, point,
              pm->ps->clientNum, traceContents, (EG2_Collision)0, 0);
    if (trace->plane.normal[2] >=
        minNormal) {                // not a steep slope, so push us up
      if (trace->fraction < 0.3f) { // push up off ground
        float hoverForce = pVeh->m_pVehicleInfo->hoverStrength;
        pm->ps->velocity[2] +=
            (0.3f - trace->fraction) * hoverForce * pVeh->m_fTimeModifier;
        ...
        if ((trace->contents &
             (CONTENTS_WATER | CONTENTS_SLIME |
              CONTENTS_LAVA))) { // hovering on water, make a spash if moving
          if (fabs(pm->ps->velocity[0]) + fabs(pm->ps->velocity[1]) >
              100) {                                 // moving at a decent speed
            if (Q_irand(pml.frametime, 100) >= 50) { // splash
              vAng[PITCH] = vAng[ROLL] = 0;
              ...
              AngleVectors(vAng, fxAxis[2], fxAxis[1], fxAxis[0]);
              if (pVeh->m_pVehicleInfo->iWakeFX) {
                G_PlayEffect(pVeh->m_pVehicleInfo->iWakeFX, trace->endpos,
                             fxAxis);
              }
            }
          }
        ...
        if (pVeh->m_ulFlags & VEH_SLIDEBREAKING) {
          if (Q_irand(pml.frametime, 100) >= 50) { // dust
  ...
  if (pml.groundPlane) {
    ...
    if (pVeh->m_vAngularVelocity == 0.0f) {
      ...
      //=================================
      if (pm->gent && pm->gent->owner &&
          (pm->gent->owner->s.number < MAX_CLIENTS ||
           G_ControlledByPlayer(pm->gent->owner)) &&
          pVeh->m_pVehicleInfo->type == VH_SPEEDER &&
          level.time > (LastMatrixJumpTime + 5000) &&
          VectorLength(pm->ps->velocity) > 30.0f) {
        LastMatrixJumpTime = level.time;
        vec3_t predictedApx;
        vec3_t predictedFallVelocity;
        vec3_t predictedLandPosition;

        VectorScale(pm->ps->velocity, 2.0f,
                    predictedFallVelocity); // take friction into account
        predictedFallVelocity[2] =
            -(pm->ps->gravity * 1.1f); // take gravity into account

        VectorMA(pm->ps->origin, 0.25f, pm->ps->velocity, predictedApx);
        VectorMA(predictedApx, 0.25f, predictedFallVelocity,
                 predictedLandPosition);

        trace_t trace2;
        gi.trace(&trace2, predictedApx, pm->mins, pm->maxs,
                 predictedLandPosition, pm->ps->clientNum, traceContents,
                 (EG2_Collision)0, 0);
        if (!trace2.startsolid && !trace2.allsolid && trace2.fraction > 0.75 &&
          ...
          //					CG_DrawEdge(pm->ps->origin, predictedApx,
          //EDGE_WHITE_TWOSECOND); 					CG_DrawEdge(predictedApx,
          //predictedLandPosition,	EDGE_WHITE_TWOSECOND);
        }
        //				else
        //				{
        //					CG_DrawEdge(pm->ps->origin,
        //predictedApx,			EDGE_RED_TWOSECOND);
        //					CG_DrawEdge(predictedApx,
        //predictedLandPosition,	EDGE_RED_TWOSECOND);
        //				}
      }
      //=================================
    }
...
void PM_SetWaterHeight(void) {
  ...
  } else if (trace.contents & MASK_WATER) { // water is above me
    pm->ps->waterheight = top[2] + 4;
...
static void PM_CheckDuck(void) {
  ...
  if (PM_RidingVehicle() ||
      (pm->gent && pm->gent->client &&
       pm->gent->client->NPC_class ==
           CLASS_VEHICLE)) { // riding a vehicle or are a vehicle
...
qboolean PM_CrouchGetup(float crouchheight) {
  ...
  if (anim == -1) {              // WTF? stay down?
    ...
    if (PM_LockedAnim(
            pm->ps->torsoAnim)) { // need to be able to override this anim
      pm->ps->torsoAnimTimer = 0;
    }
    if (PM_LockedAnim(
            pm->ps->legsAnim)) { // need to be able to override this anim
...
qboolean PM_CheckRollGetup(void) {
  if (pm->ps->legsAnim == BOTH_KNOCKDOWN1 ||
      ...
      pm->ps->legsAnim == BOTH_RELEASED) { // lying on back or front
    if (((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
         (pm->cmd.rightmove ||
          (pm->cmd.forwardmove &&
           pm->ps->forcePowerLevel[FP_LEVITATION] >
        ...
        ((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) &&
         pm->gent->NPC // an NPC
         && pm->ps->forcePowerLevel[FP_LEVITATION] >
                FORCE_LEVEL_0                  // have at least force jump 1
         && pm->gent->enemy                    // I have an enemy
         && pm->gent->enemy->client            // a client
         && pm->gent->enemy->enemy == pm->gent // he's mad at me!
         && (PM_GoingToAttackDown(&pm->gent->enemy->client->ps) ||
             !Q_irand(0, 2)) // he's attacking downward! (or we just feel like
                             // doing it this time)
         && ((pm->gent->client && pm->gent->client->NPC_class == CLASS_ALORA) ||
             Q_irand(0, RANK_CAPTAIN) <
                 pm->gent->NPC->rank))) // higher rank I am, more likely I am to
                                        // roll away!
    { // roll away!
      int anim;
      ...
      if (pm->cmd.forwardmove > 0) {
        ...
        if (pm->ps->legsAnim == BOTH_KNOCKDOWN3 ||
            pm->ps->legsAnim == BOTH_KNOCKDOWN5 ||
            pm->ps->legsAnim == BOTH_LK_DL_ST_T_SB_1_L) { // on your front
      ...
      if ((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer())) {
        if (!G_CheckRollSafety(pm->gent, anim, 64)) { // oops, try other one
          if (pm->ps->legsAnim == BOTH_KNOCKDOWN3 ||
              pm->ps->legsAnim == BOTH_KNOCKDOWN5 ||
              pm->ps->legsAnim == BOTH_LK_DL_ST_T_SB_1_L) {
            ...
            if (!G_CheckRollSafety(pm->gent, anim,
                                   64)) { // neither side is clear, screw it
              return qfalse;
            }
          } else {
      ...
      PM_SetAnim(pm, SETANIM_BOTH, anim,
                 SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD |
                     SETANIM_FLAG_HOLDLESS);
      pm->ps->weaponTime = pm->ps->torsoAnimTimer -
                           300; // don't attack until near end of this anim
      pm->ps->saberMove = pm->ps->saberBounceMove =
          LS_READY; // don't finish whatever saber anim you may have been in
      pm->ps->saberBlocked = BLOCKED_NONE;
      if (forceGetUp) {
...
qboolean PM_GettingUpFromKnockDown(float standheight, float crouchheight) {
  ...
  if (legsAnim == BOTH_KNOCKDOWN1 || legsAnim == BOTH_KNOCKDOWN2 ||
      ...
      legsAnim == BOTH_KNOCKDOWN5 || legsAnim == BOTH_PLAYER_PA_3_FLY ||
      legsAnim == BOTH_LK_DL_ST_T_SB_1_L ||
      legsAnim == BOTH_RELEASED) { // in a knockdown
    int minTimeLeft = G_MinGetUpTime(pm->gent);
    if (pm->ps->legsAnimTimer <=
        minTimeLeft) { // if only a quarter of a second left, allow roll-aways
    ...
    if (TIMER_Exists(pm->gent, "noGetUpStraight")) {
      if (!TIMER_Done2(pm->gent, "noGetUpStraight",
                       qtrue)) { // not allowed to do straight get-ups for
                                 // another few seconds
        if (pm->ps->legsAnimTimer <= minTimeLeft) { // hold it for a bit
    ...
    if (!pm->ps->legsAnimTimer ||
        (pm->ps->legsAnimTimer <= minTimeLeft &&
         (pm->cmd.upmove > 0 ||
          (pm->gent && pm->gent->client &&
           pm->gent->client->NPC_class ==
               CLASS_ALORA)))) { // done with the knockdown - FIXME: somehow
          ...
          case BOTH_KNOCKDOWN1:
            if ((pm->ps->clientNum &&
                ...
                ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
                 pm->cmd.upmove > 0 &&
                 pm->ps->forcePowerLevel[FP_LEVITATION] >
                     FORCE_LEVEL_0)) // FORCE_LEVEL_1) )FORCE_LEVEL_1) )
            {
              anim =
...
void PM_CmdForRoll(playerState_t *ps, usercmd_t *pCmd) {
  ...
  case BOTH_GETUP_FROLL_R:
    if (ps->legsAnimTimer <= 250) { // end of anim
  ...
  case BOTH_GETUP_FROLL_B:
    if (ps->torsoAnimTimer <= 100) { // end of anim
      pCmd->forwardmove = pCmd->rightmove = 0;
    } else if (PM_AnimLength(
                   g_entities[ps->clientNum].client->clientInfo.animFileIndex,
                   (animNumber_t)ps->legsAnim) -
                   ps->torsoAnimTimer <
               200) { // beginning of anim
      pCmd->forwardmove = pCmd->rightmove = 0;
    } else {
      // FIXME: ramp down over length of anim
      pCmd->forwardmove = -64;
  ...
  case BOTH_GETUP_BROLL_F:
    if (ps->torsoAnimTimer <= 550) { // end of anim
      pCmd->forwardmove = pCmd->rightmove = 0;
    } else if (PM_AnimLength(
                   g_entities[ps->clientNum].client->clientInfo.animFileIndex,
                   (animNumber_t)ps->legsAnim) -
                   ps->torsoAnimTimer <
               150) { // beginning of anim
      pCmd->forwardmove = pCmd->rightmove = 0;
    } else {
      pCmd->forwardmove = 64;
      pCmd->rightmove = 0;
...
void PM_FootSlopeTrace(float *pDiff, float *pInterval) {
  ...
  */
  ...
  pm->trace(&trace, footLOrg, footMins, footMaxs, footLBot, pm->ps->clientNum,
            pm->tracemask, (EG2_Collision)0, 0);
...
qboolean PM_AdjustStandAnimForSlope(void) {
  ...
  if (pm->gent->client->NPC_class != CLASS_ATST &&
      (!pm->gent ||
       !G_StandardHumanoid(pm->gent))) { // only ATST and player does this
    return qfalse;
  }
  if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
      (!cg.renderingThirdPerson ||
  ...
  if ((legsAnim >= LEGS_LEFTUP1 && legsAnim <= LEGS_LEFTUP5) ||
      ...
      (legsAnim >= LEGS_S7_LUP1 &&
       legsAnim <= LEGS_S7_LUP5)) { // already in left-side up
    if (destAnim > legsAnim && pm->gent->client->slopeRecalcTime < level.time) {
      ...
      destAnim = legsAnim;
             ...
             (legsAnim >= LEGS_S7_RUP1 &&
              legsAnim <= LEGS_S7_RUP5)) { // already in right-side up
    if (destAnim > legsAnim && pm->gent->client->slopeRecalcTime < level.time) {
      legsAnim++;
      pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
    } else if (destAnim < legsAnim &&
    ...
    if (pm->gent->client->NPC_class == CLASS_ATST) {
      ...
      case BOTH_STAND5:
        ...
        break;
      case BOTH_SABERDUAL_STANCE:
        if (destAnim >= LEGS_S6_LUP1 &&
            destAnim <= LEGS_S6_LUP5) { // going into left side up
          destAnim = LEGS_S6_LUP1;
          pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
        } else if (destAnim >= LEGS_S6_RUP1 &&
                   destAnim <= LEGS_S6_RUP5) { // going into right side up
        ...
        break;
      case BOTH_SABERSTAFF_STANCE:
        if (destAnim >= LEGS_S7_LUP1 &&
            destAnim <= LEGS_S7_LUP5) { // going into left side up
          destAnim = LEGS_S7_LUP1;
          pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
        } else if (destAnim >= LEGS_S7_RUP1 &&
                   destAnim <= LEGS_S7_RUP5) { // going into right side up
...
void PM_JetPackAnim(void) {
  if (!PM_ForceJumpingAnim(pm->ps->legsAnim)) // haven't started forcejump yet
  {
    ...
    int parts = SETANIM_BOTH;
    if (pm->ps->weaponTime) { // FIXME: really only care if we're in a saber
                              // attack anim...
      parts = SETANIM_LEGS;
    }
  ...
  else
  {
          ...
          {//not in the middle of a legsAnim
                  ...
                  case BOTH_FORCEJUMPRIGHT1:
                          newAnim =
  BOTH_FORCELANDRIGHT1;//BOTH_FORCEINAIRRIGHT1; break;
                  }
                  if ( newAnim != -1 )
                  {
                          int parts = SETANIM_BOTH;
                          if ( pm->ps->weaponTime )
                          {//FIXME: really only care if we're in a saber attack
  anim... parts = SETANIM_LEGS;
                          }
...
static void PM_Footsteps(void) {
  ...
  if ((pm->ps->eFlags & EF_FORCE_DRAINED)) { // being drained
    // PM_SetAnim( pm, SETANIM_LEGS, BOTH_HUGGEE1,
    // SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
    return;
  }
  if ((pm->ps->forcePowersActive & (1 << FP_DRAIN)) &&
      pm->ps->forceDrainEntityNum < ENTITYNUM_WORLD) { // draining
    // PM_SetAnim( pm, SETANIM_LEGS, BOTH_HUGGER1,
    // SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
    return;
  } else if (pm->gent->NPC && pm->gent->NPC->aiFlags & NPCAI_KNEEL) { // kneeling
    return;
  }
    ...
    if (pm->waterlevel > 0) {
      if (pm->watertype & CONTENTS_LADDER) { // FIXME: check for watertype, save
        ...
        return;
                 ...
                 ((pm->ps->clientNum < MAX_CLIENTS ||
                   PM_ControlledByPlayer()) ||
                  pm->ps->weapon == WP_SABER || pm->ps->weapon == WP_NONE ||
                  pm->ps->weapon ==
        ...
        if (pm->ps->groundEntityNum == ENTITYNUM_NONE) { // unless in the air
          ...
          } else if (pm->ps->gravity <= 0) // FIXME: or just less than normal?
          {
            if (pm->gent && pm->gent->client &&
                (pm->gent->client->NPC_class == CLASS_BOBAFETT ||
                 pm->gent->client->NPC_class == CLASS_ROCKETTROOPER) &&
                pm->gent->client->moveType ==
                    MT_FLYSWIM) { // flying around with jetpack
              // do something else?
              PM_JetPackAnim();
            } else {
              PM_SwimFloatAnim();
    ...
    if (pm->gent && pm->gent->client &&
      ...
      if (pm->ps->legsAnimTimer &&
          (pm->ps->legsAnim == BOTH_THERMAL_READY ||
           ...
           pm->ps->legsAnim ==
               BOTH_ATTACK10)) { // still in a thermal anim, let it play
        return;
      }
        ...
        if (!PM_AdjustStandAnimForSlope()) {
          ...
          if ((pm->ps->torsoAnim == BOTH_SPINATTACK6 ||
              ...
              (pm->ps->groundEntityNum == ENTITYNUM_NONE // in air
               || (!PM_JumpingAnim(pm->ps->torsoAnim) &&
                   !PM_InAirKickingAnim(
                       pm->ps->torsoAnim)))) // OR: on ground and torso not in a
                                             // jump anim
          {
      ...
      { // Squadmates use BOTH_STAND3
        ...
        if (oldAnim != BOTH_GUARD_LOOKAROUND1 && oldAnim != BOTH_GUARD_IDLE1 &&
            oldAnim != BOTH_STAND2TO4 && oldAnim != BOTH_STAND4TO2 &&
        ...
        if (!PM_AdjustStandAnimForSlope()) {
          ...
          if (pm->gent && pm->gent->client &&
            ...
            if (pm->gent && pm->gent->client &&
              ...
              if (pm->gent->count) {
                PM_SetAnim(pm, SETANIM_LEGS, BOTH_STAND4, SETANIM_FLAG_NORMAL);
              } else if (pm->gent->enemy ||
                         pm->gent->wait) { // have an enemy or have had one
                                           // since we spawned
                ...
                PM_SetAnim(pm, SETANIM_LEGS, BOTH_STAND1, SETANIM_FLAG_NORMAL);
                       ...
                       pm->gent->client->NPC_class == CLASS_WAMPA) {
              if (pm->gent->count) { // holding a victim
                PM_SetAnim(pm, SETANIM_LEGS, BOTH_HOLD_IDLE /*BOTH_STAND2*/,
                           SETANIM_FLAG_NORMAL);
  ...
  if ((pm->ps->eFlags & EF_IN_ATST) ||
      (pm->gent && pm->gent->client &&
       pm->gent->client->NPC_class ==
           CLASS_RANCOR) // does this catch NPCs, too?
      || (pm->gent && pm->gent->client &&
          pm->gent->client->NPC_class ==
              CLASS_WAMPA)) // does this catch NPCs, too?
  { // atst, Rancor & Wampa, only override turn anims on legs (no torso)
    ...
    if ((PM_InSaberAnim(pm->ps->legsAnim) &&
        ...
        PM_SaberDrawPutawayAnim(pm->ps->legsAnim) ||
        pm->ps->legsAnim ==
            BOTH_SPINATTACK6 // not a full-body spin, just spinning the saber
        || pm->ps->legsAnim ==
               BOTH_SPINATTACK7 // not a full-body spin, just spinning the saber
        || pm->ps->legsAnim == BOTH_BUTTON_HOLD ||
        pm->ps->legsAnim == BOTH_BUTTON_RELEASE ||
        pm->ps->legsAnim == BOTH_THERMAL_READY ||
        pm->ps->legsAnim == BOTH_THERMAL_THROW ||
        pm->ps->legsAnim == BOTH_ATTACK10 || PM_LandingAnim(pm->ps->legsAnim) ||
  ...
  if (pm->ps->pm_flags & PMF_DUCKED) {
    ...
    if (!PM_InOnGroundAnim(pm->ps) // not on the ground
        && (!PM_InRollIgnoreTimer(pm->ps) ||
            (!pm->ps->legsAnimTimer &&
             pm->cmd.upmove < 0))) // not in a roll (or you just finished one
                                   // and you're still holding crouch)
    {
      qboolean rolled = qfalse;
      if (PM_RunningAnim(pm->ps->legsAnim) ||
          pm->ps->legsAnim == BOTH_FORCEHEAL_START ||
          PM_CanRollFromSoulCal(pm->ps)) { // roll!
        rolled = PM_TryRoll();
      }
      if (!rolled) {
        if (pm->ps->pm_flags & PMF_BACKWARDS_RUN) {
        ...
        if (!Q_irand(0, 19)) { // 5% chance of making an alert
          AddSoundEvent(pm->gent, pm->ps->origin, 16, AEL_MINOR, qtrue, qtrue);
        }
      } else { // rolling is a little noisy
    ...
    if (!(pm->cmd.buttons & BUTTON_WALKING)) {       // running backwards
      ...
      if (pm->ps->weapon == WP_SABER && pm->ps->SaberActive()) {
        ...
        */
        { PM_SetAnim(pm, SETANIM_LEGS, BOTH_RUNBACK2, setAnimFlags); }
                 ...
                 pm->gent->client->NPC_class == CLASS_RANCOR) { // no run anim
      ...
      if (!Q_irand(
              0,
              9)) { // 10% chance of a small alert, mainly for the sand_creature
        AddSoundEvent(pm->gent, pm->ps->origin, 16, AEL_MINOR, qtrue, qtrue);
      }
    }
  } else {
    if (pm->gent && pm->gent->client &&
      ...
      AddSoundEvent(pm->gent, pm->ps->origin, 128, AEL_SUSPICIOUS, qtrue,
                    qtrue);
    } else {
      if (!(pm->cmd.buttons & BUTTON_WALKING)) { // running
        bobmove = 0.4F;                          // faster speeds bob faster
        if (pm->ps->weapon == WP_SABER && pm->ps->SaberActive()) {
          if (pm->ps->saberAnimLevel == SS_DUAL) {
          ...
          if (pm->gent && pm->gent->client &&
            ...
            // if ( pm->gent->enemy && (pm->ps->weapon == WP_NONE ||
            ...
            */
            ...
            if (pm->ps->legsAnim != BOTH_RUN1) {
              ...
              PM_SetAnim(pm, SETANIM_LEGS, BOTH_RUN1, setAnimFlags);
                     ...
                     pm->gent->client->NPC_class == CLASS_WAMPA) {
            if (pm->gent->NPC && pm->gent->NPC->stats.runSpeed ==
                                     300) { // full on run, on all fours
        ...
        if (pm->ps->weapon == WP_SABER && pm->ps->SaberActive()) {
          if (pm->ps->saberAnimLevel == SS_DUAL) {
            ...
            PM_SetAnim(pm, SETANIM_LEGS, BOTH_WALK2, setAnimFlags);
                   ...
                   pm->gent->client->NPC_class == CLASS_WAMPA) {
          if (pm->gent->health <= 50) { // hurt walk
        ...
        if (pm->gent && pm->gent->NPC && pm->gent->client &&
          ...
          footstep = qtrue;
        } else if (!Q_irand(0, 9)) { // 10% chance of a small alert, mainly for
                                     // the sand_creature
          AddSoundEvent(pm->gent, pm->ps->origin, 16, AEL_MINOR, qtrue, qtrue);
        }
      }
  ...
  if (((old + 64) ^ (pm->ps->bobCycle + 64)) & 128) {
    if (pm->watertype & CONTENTS_LADDER) {
      ...
      if (pm->gent && pm->gent->s.number == 0) {
        ...
        //pm->gent->client->playerTeam != TEAM_DISGUISE )
        {
          AddSoundEvent(pm->gent, pm->ps->origin, 384, AEL_SUSPICIOUS, qfalse,
                        qtrue); // was bottom
      ...
      */
      ...
      if (pm->gent && pm->gent->s.number == 0) {
        ...
        //pm->gent->client->playerTeam != TEAM_DISGUISE )
        {
          AddSoundEvent(pm->gent, pm->ps->origin, 256, AEL_MINOR, qfalse,
                        qtrue);
...
static void PM_BeginWeaponChange(int weapon) {
  ...
  // eezstreet edit: also ignore if we change to WP_NONE..sorta hacky fix for
  // binoculars using WP_SABER
  ...
  if (pm->gent && pm->gent->client &&
    ...
    if (!(pm->ps->eFlags & EF_HELD_BY_WAMPA)) {
      if (pm->ps->weapon != WP_THERMAL && pm->ps->weapon != WP_TRIP_MINE &&
          pm->ps->weapon != WP_DET_PACK && !G_IsRidingVehicle(pm->gent)) {
...
qboolean PM_CanDoDualDoubleAttacks(void) {
  ...
  if (pm->gent && pm->gent->NPC &&
      pm->gent->NPC->rank >=
          Q_irand(RANK_LT_COMM, RANK_CAPTAIN + 2)) { // high-rank NPC
    return qtrue;
  }
  if (pm->gent && pm->gent->client &&
      pm->gent->client->NPC_class == CLASS_ALORA) { // Alora
    return qtrue;
  }
  return qfalse;
}
...
void PM_SetSaberMove(saberMoveName_t newMove) {
  ...
  if ((pm->ps->eFlags & EF_HELD_BY_WAMPA)) { // no anim
    return;
  }
  ...
  if (newMove == LS_READY) {
    if (pm->ps->saberBlockingTime > cg.time) {
      ...
      if (!pm->ps->SaberActive()) { // turn on all blades and sabers if none are
                                    // currently on
        pm->ps->SaberActivate();
      }
    ...
    } else if (pm->ps->saber[0].type ==
               SABER_ARC) { // FIXME: need it's own style?
      anim = BOTH_SABERFAST_STANCE;
    } else if ((pm->ps->dualSabers && pm->ps->saber[1].Active())) {
      anim = BOTH_SABERDUAL_STANCE;
    } else if ((pm->ps->SaberStaff() &&
                (!pm->ps->saber[0].singleBladeStyle ||
                 pm->ps->saber[0].blade[1].active)) // saber staff with more
                                                    // than first blade active
               || pm->ps->saber[0].type == SABER_ARC) {
      anim = BOTH_SABERSTAFF_STANCE;
    } else if (pm->ps->saber[0].type == SABER_LANCE ||
               pm->ps->saber[0].type ==
                   SABER_TRIDENT) { // FIXME: need some 2-handed
                                    // forward-pointing anim
      anim = BOTH_STAND1;
    } else {
    ...
    // FIXME: also dual
  } else if (pm->ps->saberAnimLevel == SS_STAFF && newMove >= LS_S_TL2BR &&
             newMove < LS_REFLECT_LL) { // staff has an entirely new set of
                                        // anims, besides special attacks
    // FIXME: include ready and draw/putaway?
    // FIXME: get hand-made bounces and deflections?
    if (newMove >= LS_V1_BR &&
        newMove <= LS_REFLECT_LL) { // there aren't 1-7, just 1, 6 and 7, so
                                    // just set it
      anim = BOTH_P7_S7_T_ +
             (anim - BOTH_P1_S1_T_); // shift it up to the proper set
    ...
    if (newMove >= LS_V1_BR &&
      ...
      anim = BOTH_P6_S6_T_ +
             (anim - BOTH_P1_S1_T_); // shift it up to the proper set
               ...
               G_CheckEnemyPresence(pm->gent, DIR_LEFT,
                                    150.0f)) { // enemy both on left and right
      newMove = LS_DUAL_LR;
      anim = saberMoveData[newMove].animToUse;
      ...
      pm->cmd.rightmove = 0;
               ...
               G_CheckEnemyPresence(pm->gent, DIR_BACK,
                                    150.0f)) { // enemy both in front and back
      newMove = LS_DUAL_FB;
      anim = saberMoveData[newMove].animToUse;
      // probably already moved, but...
      pm->cmd.forwardmove = 0;
  ...
  /*
  else if ( newMove == LS_DRAW && pm->ps->saberAnimLevel == SS_STAFF
  )//pm->ps->SaberStaff() )
  ...
  */
  else if (pm->ps->saberAnimLevel > FORCE_LEVEL_1 && !PM_SaberInIdle(newMove) &&
           !PM_SaberInParry(newMove) && !PM_SaberInKnockaway(newMove) &&
           !PM_SaberInBrokenParry(newMove) && !PM_SaberInReflect(newMove) &&
           !PM_SaberInSpecial(
               newMove)) { // readies, parries and reflections have only 1 level
    if (pm->ps->saber[0].type == SABER_LANCE ||
        pm->ps->saber[0].type ==
            SABER_TRIDENT) { // FIXME: hack for now - these use the fast anims,
                             // but slowed down.  Should have own style
    } else { // increment the anim to the next level of saber anims
      anim += (pm->ps->saberAnimLevel - FORCE_LEVEL_1) * SABER_ANIM_GROUP_SIZE;
    }
  } else if (newMove == LS_KICK_F_AIR || newMove == LS_KICK_B_AIR ||
  ...
  // restart...
  //	if ( saberMoveData[pm->ps->saberMove].animToUse == anim && newMove >
  //LS_PUTAWAY)
  ...
  if ((anim == BOTH_STAND1 &&
      ...
      anim == BOTH_SABERDUAL_STANCE || anim == BOTH_SABERFAST_STANCE ||
      anim == BOTH_SABERSLOW_STANCE) { // match torso anim to walk/run anim if
                                       // newMove is just LS_READY
  ...
  if (!PM_RidingVehicle()) {
    if (!manualBlocking) {
      if (newMove == LS_A_LUNGE || newMove == LS_A_JUMP_T__B_ ||
          ...
          newMove == LS_A_BACK_CR || newMove == LS_ROLL_STAB ||
          ...
          newMove == LS_BUTTERFLY_LEFT || newMove == LS_BUTTERFLY_RIGHT ||
          newMove == LS_A_BACKFLIP_ATK || newMove == LS_STABDOWN ||
          newMove == LS_STABDOWN_STAFF || newMove == LS_STABDOWN_DUAL ||
          newMove == LS_DUAL_SPIN_PROTECT || newMove == LS_STAFF_SOULCAL ||
          newMove == LS_A1_SPECIAL || newMove == LS_A2_SPECIAL ||
          newMove == LS_A3_SPECIAL || newMove == LS_UPSIDE_DOWN_ATTACK ||
          newMove == LS_PULL_ATTACK_STAB || newMove == LS_PULL_ATTACK_SWING ||
          PM_KickMove(newMove)) {
        parts = SETANIM_BOTH;
      } else if (PM_SpinningSaberAnim(
                     anim)) { // spins must be played on entire body
        parts = SETANIM_BOTH;
        ...
        if (!PM_FlippingAnim(pm->ps->legsAnim) && !PM_InRoll(pm->ps) &&
            ...
            // pm->cmd.upmove >= 0 &&
            !(pm->ps->pm_flags & PMF_DUCKED) && newMove != LS_PUTAWAY) {
          parts = SETANIM_BOTH;
        } else if (!(pm->ps->pm_flags & PMF_DUCKED) &&
  ...
  if (pm->ps->torsoAnim == anim) { // successfully changed anims
      ...
      if (PM_SaberInAttack(newMove) ||
        ...
        if (pm->ps->saberMove != newMove) { // wasn't playing that attack before
          if (PM_SaberInSpecialAttack(anim)) {
            ...
            if (!PM_InCartwheel(pm->ps->torsoAnim)) { // can still attack during
                                                      // a cartwheel/arial
              pm->ps->weaponTime =
                  pm->ps->torsoAnimTimer; // so we know our weapon is busy
            }
          } else {
            switch (pm->ps->saberAnimLevel) {
            case SS_DESANN:
            ...
            case SS_FAST:
              ...
              break;
                   ...
                   PM_SaberInSpecialAttack(
                       anim)) { // sigh, if restarted a special, then set the
                                // weaponTime *again*
          if (!PM_InCartwheel(pm->ps->torsoAnim)) { // can still attack during a
                                                    // cartwheel/arial
            pm->ps->weaponTime =
                pm->ps->torsoAnimTimer; // so we know our weapon is busy
          }
        ...
        { // don't damage on the first few frames of a start anim because it may
          // pop from one position to some drastically different one, killing the
          // enemy without hitting them.
          int damageDelay = 150;
    ...
    /*
    //wtf... getting stuck with weaponTime set even though we're not in an
    attack...? if ( PM_SaberInAttack( pm->ps->saberMove )
    ...
    */
    ...
    // Some special attacks can be started when sabers are off, make sure we
    // turn them on, first!
    switch (newMove) { // make sure the saber is on!
    case LS_A_LUNGE:
    case LS_ROLL_STAB:
      if (PM_InSecondaryStyle()) { // staff as medium or dual as fast
        if (pm->ps->dualSabers) {  // only force on the first saber
          pm->ps->saber[0].Activate();
        } else if (pm->ps->saber[0].numBlades >
                   1) { // only force on the first saber's first blade
          pm->ps->SaberBladeActivate(0, 0);
        }
      } else { // turn on all blades on all sabers
        pm->ps->SaberActivate();
      }
      break;
    ...
    if (pm->gent && pm->gent->client) {
      if (saberMoveData[newMove].trailLength > 0) {
        pm->gent->client->ps.SaberActivateTrail(
...
extern saberMoveName_t PM_AttackForEnemyPos(qboolean allowFB,
...
int PM_SaberLockWinAnim(saberLockResult_t result, int breakType) {
  ...
  if (winAnim != -1) {
    ...
    if (breakType == SABERLOCK_SUPERBREAK &&
        winAnim != BOTH_LK_ST_DL_T_SB_1_W) { // going to attack with saber, do a
                                             // saber trail
...
int PM_SaberLockResultAnim(gentity_t *duelist, int lockOrBreakOrSuperBreak,
                           int winOrLose) {
  ...
  NPC_SetAnim(duelist, SETANIM_BOTH, baseAnim,
              SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);

  if (lockOrBreakOrSuperBreak == SABERLOCK_SUPERBREAK &&
      winOrLose ==
          SABERLOCK_LOSE) { // if you lose a superbreak, you're defenseless
  ...
  if (lockOrBreakOrSuperBreak == SABERLOCK_SUPERBREAK &&
      ...
      baseAnim != BOTH_LK_ST_DL_T_SB_1_W) { // going to attack with saber, do a
                                            // saber trail
...
void PM_SaberLockBreak(gentity_t *gent, gentity_t *genemy,
  ...
  if (result == LOCK_VICTORY && Q_irand(0, 7) < victoryStrength) {
    if (genemy && genemy->NPC &&
        ...
        Q_irand(0,
                4)) { // less of a chance of getting a superbreak against a boss
  ...
  if (winAnim != -1) { // a single vs. single break
    if (genemy && genemy->client) {
      loseAnim = PM_SaberLockLoseAnim(genemy, result, breakType);
    }
  } else { // must be a saberlock that's not between single and single...
  ...
  if (result == LOCK_STALEMATE) { // no-one won
    ...
    if (result == LOCK_VICTORY) { // one person won
      if (Q_irand(FORCE_LEVEL_1, FORCE_LEVEL_2) <
        ...
        if (!singleVsSingle) { // all others have their own super breaks
          // so it doesn't try to set some other anim below
          winAnim = -1;
        } else if (winAnim == BOTH_LK_S_S_S_SB_1_W ||
                   winAnim ==
                       BOTH_LK_S_S_T_SB_1_W) { // doing a superbreak on
                                               // single-vs-single, don't do the
                                               // old superbreaks this time
          // so it doesn't try to set some other anim below
          winAnim = -1;
        } else { // JK2-style
          switch (winAnim) {
          ...
          if (winAnim != BOTH_CCWCIRCLEBREAK) {
            if ((!genemy->s.number &&
                 genemy->health <= 25) // player low on health
                ||
                (genemy->s.number && genemy->client->NPC_class != CLASS_KYLE &&
        ...
        if (!(genemy->client->ps.saber[0].saberFlags & SFL_NOT_DISARMABLE)) {
          ...
          if ((genemy->client->ps.saber[0].saberFlags & SFL_TWO_HANDED) ||
              (genemy->client->ps.dualSabers &&
               genemy->client->ps.saber[1]
                   .Active())) { // defender gets a bonus for using a 2-handed
                                 // saber or 2 sabers
            victoryStrength -= 2;
          }
          ...
          if (Q_irand(0, 10) < victoryStrength) {
            if (!(genemy->client->ps.saber[0].saberFlags & SFL_TWO_HANDED) ||
                !Q_irand(0, 1)) { // if it's a two-handed saber, it has a 50%
                                  // chance of resisting a disarming
...
int G_SaberLockStrength(gentity_t *gent) {
  ...
  if (gent->s.number >= MAX_CLIENTS) {
    ...
    strength += gent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] +
                Q_irand(0, g_spskill->integer) + Q_irand(0, 1);
  }
  return strength;
}
...
qboolean PM_SaberLocked(void) {
  ...
  if (pm->ps->saberLockEnemy == ENTITYNUM_NONE) {
    if (PM_InSaberLock(pm->ps->torsoAnim)) { // wtf?  Maybe enemy died?
  ...
  if (PM_InSaberLock(pm->ps->torsoAnim) &&
      PM_InSaberLock(genemy->client->ps.torsoAnim)) {
    if (pm->ps->saberLockTime <= level.time + 500 &&
      ...
      if (strength > 1 && eStrength > 1 &&
          !Q_irand(0, abs(strength - eStrength) +
      ...
      if (!(pm->ps->pm_flags & PMF_ATTACK_HELD)) { // tapping
        ...
        if (ValidAnimFileIndex(gent->client->clientInfo.animFileIndex)) {
          ...
          if (PM_InSaberLockOld(pm->ps->torsoAnim)) { // old locks
          ...
          */
          ...
          if (PM_InSaberLockOld(genemy->client->ps.torsoAnim)) {
            if (genemy->client->ps.torsoAnim == BOTH_CCWCIRCLELOCK ||
              ...
              PM_SetAnimFrame(genemy, anim->firstFrame + remaining, qtrue,
                              qtrue);
            }
          } else { // new locks
            //???
            if (G_CheckIncrementLockAnim(genemy->client->ps.torsoAnim,
                                         SABERLOCK_LOSE)) {
              PM_SetAnimFrame(genemy,
                              anim->firstFrame + anim->numFrames - remaining,
                              qtrue, qtrue);
...
qboolean G_EnemyInKickRange(gentity_t *self, gentity_t *enemy) {
  ...
  if (fabs(self->currentOrigin[2] - enemy->currentOrigin[2]) <
      32) { // generally at same height
    if (DistanceHorizontal(self->currentOrigin, enemy->currentOrigin) <=
        (STAFF_KICK_RANGE + 8.0f + (self->maxs[0] * 1.5f) +
         (enemy->maxs[0] * 1.5f))) { // within kicking range!
      return qtrue;
    }
  }
  return qfalse;
...
qboolean G_CanKickEntity(gentity_t *self, gentity_t *target) {
  if (target && target->client && !PM_InKnockDown(&target->client->ps) &&
...
float PM_GroundDistance(void) {
  ...
  pm->trace(&tr, pm->ps->origin, pm->mins, pm->maxs, down, pm->ps->clientNum,
            pm->tracemask, (EG2_Collision)0, 0);
...
float G_GroundDistance(gentity_t *self) {
  ...
  gi.trace(&tr, self->currentOrigin, self->mins, self->maxs, down,
...
saberMoveName_t G_PickAutoKick(gentity_t *self, gentity_t *enemy,
                               qboolean storeMove) {
  ...
  vec3_t v_fwd, v_rt, enemyDir,
      fwdAngs = {0, self->client->ps.viewangles[YAW], 0};
  ...
  if (fabs(rDot) > 0.5f && fabs(fDot) < 0.5f) { // generally to one side
    if (rDot > 0) {                             // kick right
      kickMove = LS_KICK_R;
    } else { // kick left
      kickMove = LS_KICK_L;
    }
  } else if (fabs(fDot) > 0.5f &&
             fabs(rDot) < 0.5f) { // generally in front or behind us
    if (fDot > 0) {               // kick fwd
      kickMove = LS_KICK_F;
    } else { // kick back
      kickMove = LS_KICK_B;
    }
  } else { // diagonal to us, kick would miss
  }
  if (kickMove != LS_NONE) { // have a valid one to do
    if (self->client->ps.groundEntityNum ==
        ENTITYNUM_NONE) { // if in air, convert kick to an in-air kick
      ...
      if ((!PM_FlippingAnim(self->client->ps.legsAnim) ||
           self->client->ps.legsAnimTimer <= 0) &&
          gDist > 64.0f && // strict minimum
          gDist > (-self->client->ps.velocity[2]) -
                      64.0f // make sure we are high to ground relative to
                            // downward velocity as well
      ) {
        switch (kickMove) {
        case LS_KICK_F:
        ...
        if (gDist > 128.0f || self->client->ps.velocity[2] >=
                                  0) { // off ground, but too close to ground
          kickMove = LS_NONE;
        }
      }
    }
...
saberMoveName_t G_PickAutoMultiKick(gentity_t *self, qboolean allowSingles,
                                    qboolean storeMove) {
  gentity_t *ent;
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (kickMove == LS_KICK_F_AIR && kickMove == LS_KICK_B_AIR &&
        kickMove == LS_KICK_R_AIR &&
        kickMove == LS_KICK_L_AIR) { // in air?  Can't do multikicks
    } else {
      switch (kickMove) {
      case LS_KICK_F:
        enemiesFront++;
  ...
  kickMove = LS_NONE;
  if (self->client->ps.groundEntityNum !=
      ENTITYNUM_NONE) { // can't do the multikicks in air
    if (enemiesFront && enemiesBack &&
        (enemiesFront + enemiesBack) - (enemiesRight + enemiesLeft) >
            1) { // more enemies in front/back than left/right
      kickMove = LS_KICK_BF;
    } else if (enemiesRight && enemiesLeft &&
               (enemiesRight + enemiesLeft) - (enemiesFront + enemiesBack) >
                   1) { // more enemies on left & right than front/back
      kickMove = LS_KICK_RL;
    } else if ((enemiesFront || enemiesBack) &&
               (enemiesRight ||
                enemiesLeft)) { // at least 2 enemies around us, not aligned
      kickMove = LS_KICK_S;
    } else if (enemiesSpin > 1) { // at least 2 enemies around us, not aligned
      kickMove = LS_KICK_S;
    }
  }
  if (kickMove == LS_NONE &&
      bestKick != LS_NONE) { // no good multi-kick move, but we do have a nice
                             // single-kick we found
    kickMove = bestKick;
...
qboolean PM_PickAutoMultiKick(qboolean allowSingles) {
  saberMoveName_t kickMove =
      G_PickAutoMultiKick(pm->gent, allowSingles, qfalse);
  if (kickMove != LS_NONE) {
...
qboolean PM_SaberThrowable(void) {
  ...
  if (!(pm->ps->saber[0].saberFlags &
        SFL_NOT_THROWABLE)) { // yes, this saber is always throwable
    return qtrue;
  }

  // saber is not normally throwable
  if ((pm->ps->saber[0].saberFlags &
       SFL_SINGLE_BLADE_THROWABLE)) { // it is throwable if only one blade is on
    if (pm->ps->saber[0].numBlades > 1) { // it has more than one blade
      ...
      if (numBladesActive == 1) { // only 1 blade is on
        return qtrue;
      }
    }
  }
...
qboolean PM_CheckAltKickAttack(void) {
  if ((pm->cmd.buttons & BUTTON_ALT_ATTACK) &&
      ...
      !(pm->ps->saber[0].saberFlags &
        SFL_NO_KICKS) // okay to do kicks with this saber
      && (!pm->ps->dualSabers ||
          !(pm->ps->saber[1].saberFlags &
            SFL_NO_KICKS)) // okay to do kicks with this saber
  ) {
...
qboolean PM_CheckUpsideDownAttack(void) {
  ...
  if ((pm->ps->clientNum >= MAX_CLIENTS &&
       !PM_ControlledByPlayer())) { // FIXME: check ranks?
    return qfalse;
  }
  // FIXME: enemy below
  // FIXME: more than 64 off ground
  if (!g_debugMelee->integer) { // hmm, can't get this to work quite the way we
                                // wanted... secret move, then!
    return qfalse;
  }
  ...
  // JKA
  case BOTH_FORCEWALLRUNFLIP_END: {
    float animLength = PM_AnimLength(pm->gent->client->clientInfo.animFileIndex,
                                     (animNumber_t)pm->ps->legsAnim);
    float elapsedTime = (float)(animLength - pm->ps->legsAnimTimer);
    float midPoint = animLength / 2.0f;
    if (elapsedTime < midPoint - 100.0f ||
        elapsedTime >
            midPoint + 100.0f) { // only a 200ms window (in middle of anim) of
                                 // opportunity to do this move in these anims
      return qfalse;
    }
  }
    // NOTE: falls through on purpose
...
qboolean PM_SaberMoveOkayForKata(void) {
  if (g_saberNewControlScheme->integer) {
    if (pm->ps->saberMove == LS_READY // not doing anything
        || PM_SaberInReflect(
               pm->ps->saberMove)) // interrupt a projectile blocking move
    {
      return qtrue;
    } else {
      return qfalse;
    }
  } else { // old control scheme, allow it to interrupt a start or ready
    if (pm->ps->saberMove == LS_READY ||
        PM_SaberInReflect(
            pm->ps->saberMove) // interrupt a projectile blocking move
...
qboolean PM_CanDoKata(void) {
  ...
  if (!pm->ps->saberInFlight // not throwing saber
      ...
      */
      ...
      && pm->cmd.forwardmove >=
             0              // not moving back (used to be !pm->cmd.forwardmove)
      ...
      && G_EnoughPowerForSpecialMove(
             ...
             qtrue) /*pm->ps->forcePower >= SABER_ALT_ATTACK_POWER*/) // SINGLE_SPECIAL_POWER
                                                                      // )// have
                                                                      // enough
                                                                      // power
...
void PM_SaberDroidWeapon(void) {
  ...
  if (pm->ps->saberBlocked) {
    ...
    case BLOCKED_LOWER_RIGHT_PROJ:
      PM_SetAnim(pm, SETANIM_BOTH, BOTH_P1_S1_TR,
                 SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    ...
    case BLOCKED_LOWER_LEFT_PROJ:
      PM_SetAnim(pm, SETANIM_BOTH, BOTH_P1_S1_TL,
                 SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      pm->ps->legsAnimTimer += Q_irand(200, 1000);
      pm->ps->weaponTime = pm->ps->legsAnimTimer;
      break;
    ...
    case BLOCKED_TOP_PROJ:
      PM_SetAnim(pm, SETANIM_BOTH, BOTH_P1_S1_T_,
                 SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      pm->ps->legsAnimTimer += Q_irand(200, 1000);
      pm->ps->weaponTime = pm->ps->legsAnimTimer;
      break;
...
void PM_TryGrab(void) {
  ...
  {
    PM_SetAnim(pm, SETANIM_BOTH, BOTH_KYLE_GRAB,
               SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void PM_TryAirKick(saberMoveName_t kickMove) {
  if (pm->ps->groundEntityNum < ENTITYNUM_NONE) { // just do it
    PM_SetSaberMove(kickMove);
  } else {
    ...
    if ((!PM_FlippingAnim(pm->ps->legsAnim) || pm->ps->legsAnimTimer <= 0) &&
      ...
      if (gDist > 128.0f ||
          pm->ps->velocity[2] >= 0) { // off ground, but too close to ground
      } else { // high close enough to ground to do a normal kick, convert it
        switch (kickMove) {
        case LS_KICK_F_AIR:
  ...
  {                                       // player kicks
    ...
    if (pm->cmd.rightmove) {       // kick to side
      ...
      if (pm->cmd.forwardmove > 0) {  // kick fwd
        ...
        {//FIXME: don't jump while doing this move and don't do this move if in
        air PM_SetSaberMove( LS_HILT_BASH );
        }
        */
        else {
          PM_SetSaberMove(LS_KICK_F);
      ...
      pm->cmd.forwardmove = 0;
    } else if (pm->gent && pm->gent->enemy &&
               G_CanKickEntity(pm->gent, pm->gent->enemy)) { // auto-pick?
      if (/*(pm->ps->pm_flags&PMF_ALT_ATTACK_HELD)
             && (pm->cmd.buttons&BUTTON_ATTACK)
             &&*/
          PM_PickAutoMultiKick(qfalse)) {      // kicked!
        if (pm->ps->saberMove == LS_KICK_RL) { // just pull back
          if (d_slowmodeath->integer > 3) {
            G_StartMatrixEffect(pm->gent, MEF_NO_SPIN,
                                pm->ps->legsAnimTimer + 500);
          }
        } else { // normal spin
          if (d_slowmodeath->integer > 3) {
        ...
        if (pm->ps->groundEntityNum == ENTITYNUM_NONE &&
            (pm->ps->saberMove == LS_KICK_S ||
             ...
             pm->ps->saberMove ==
                 LS_KICK_RL)) { // in the air and doing a jump-kick, which is a
                                // ground anim, so....
          // cut z velocity...?
          pm->ps->velocity[2] = 0;
        }
    ...
    if (pm->ps->saberBlocked >= BLOCKED_UPPER_RIGHT_PROJ &&
      ...
      if (pm->ps->forcePowerDebounce[FP_SABER_DEFENSE] <
        ...
        pm->ps->saberBlocked = BLOCKED_NONE;
               ...
               pm->ps->powerups[PW_SHOCKED] >
                   level.time) {               // probably blocking lightning
      if ((pm->cmd.buttons & BUTTON_ATTACK)) { // trying to attack
        // allow the attack
        pm->ps->weaponTime = 0;
        pm->ps->saberBlocked = BLOCKED_NONE;
      }
               ...
               PM_ControlledByPlayer()) // player
    {
      if (pm->ps->saberBlocked >= BLOCKED_UPPER_RIGHT_PROJ &&
        ...
        if ((pm->cmd.buttons & BUTTON_ATTACK)) {      // trying to attack
          if (pm->ps->saberMove == LS_READY ||
              PM_SaberInReflect(
                  pm->ps->saberMove)) { // not already busy or already in
                                        // projectile deflection
            // trying to attack during projectile blocking, so do the attack
            // instead
            pm->ps->saberBlocked = BLOCKED_NONE;
            pm->ps->saberBounceMove = LS_NONE;
            pm->ps->weaponstate = WEAPON_READY;
            if (PM_SaberInReflect(pm->ps->saberMove) &&
                pm->ps->weaponTime >
                    0) { // interrupt the current deflection move
              pm->ps->weaponTime = 0;
            }
            return qfalse;
          }
    ...
    case BLOCKED_ATK_BOUNCE:
      ...
      { // an actual bounce?  Other bounces before this are actually
        ...
        if (pm->ps->saberBounceMove != LS_NONE) {
          ...
          bounceMove =
              PM_SaberBounceForAttack((saberMoveName_t)pm->ps->saberMove);
        }
...
qboolean PM_NPCCheckAttackRoll(void) {
  if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer() // NPC
      ...
      DistanceHorizontalSquared(pm->gent->enemy->currentOrigin,
                                pm->ps->origin) < (128.0f * 128.0f) &&
      InFOV(pm->gent->enemy->currentOrigin, pm->ps->origin, pm->ps->viewangles,
            30, 90)) { // stab!
    return qtrue;
  }
  return qfalse;
...
void PM_WeaponLightsaber(void) {
  ...
  if (pm->gent && pm->gent->client &&
      pm->gent->client->NPC_class ==
          CLASS_SABER_DROID) { // Saber droid does it's own attack logic
    PM_SaberDroidWeapon();
    return;
  }
  ...
  if (pm->ps->stats[STAT_WEAPONS] & (1 << WP_SCEPTER) && !pm->ps->dualSabers &&
      pm->gent &&
      pm->gent
          ->weaponModel[1]) { // holding scepter in left hand, use dual style
    pm->ps->saberAnimLevel = SS_DUAL;
    animLevelOverridden = qtrue;
  } else if (pm->ps->saber[0].singleBladeStyle != SS_NONE // SaberStaff()
             && !pm->ps->dualSabers && pm->ps->saber[0].blade[0].active &&
             !pm->ps->saber[0]
                  .blade[1]
                  .active) { // using a staff, but only with first blade turned
                             // on, so use is as a normal saber...?
    ...
    animLevelOverridden = qtrue;
  } else if (pm->gent && cg.saberAnimLevelPending != pm->ps->saberAnimLevel &&
             WP_SaberStyleValidForSaber(
                 pm->gent,
                 cg.saberAnimLevelPending)) { // go ahead and use the
                                              // cg.saberAnimLevelPending below
    animLevelOverridden = qfalse;
             ...
             (WP_SaberStyleValidForSaber(pm->gent, pm->ps->saberAnimLevel) ||
              WP_UseFirstValidSaberStyle(
                  pm->gent,
                  &pm->ps
                       ->saberAnimLevel))) { // style we are using is not valid,
                                             // switched us to a valid one
    animLevelOverridden = qtrue;
  }
  /*
  ...
  */
    ...
    if (pm->ps->saber[1]
            .Active()) { // if second saber is on, must use dual style
      pm->ps->saberAnimLevel = SS_DUAL;
      animLevelOverridden = qtrue;
    } else if (pm->ps->saber[0]
                   .Active()) { // with only one saber on, use fast style
    ...
    if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
      ...
      if (!PM_SaberInStart(pm->ps->saberMove) &&
        ...
        pm->ps->saberAnimLevel = cg.saberAnimLevelPending;
              ...
              PM_ControlledByPlayer())) { // if overrid the player's
                                          // saberAnimLevel, let the cgame know
    cg.saberAnimLevelPending = pm->ps->saberAnimLevel;
  }
  /*
  ...
  */
  if (PM_InKnockDown(pm->ps) || PM_InRoll(pm->ps)) { // in knockdown
    if (pm->ps->legsAnim == BOTH_ROLL_F && pm->ps->legsAnimTimer <= 250) {
      if ((pm->cmd.buttons & BUTTON_ATTACK) || PM_NPCCheckAttackRoll()) {
        if (G_EnoughPowerForSpecialMove(pm->ps->forcePower,
                                        SABER_ALT_ATTACK_POWER_FB)) {
          if (!(pm->ps->saber[0].saberFlags & SFL_NO_ROLL_STAB) &&
              (!pm->ps->dualSabers ||
               !(pm->ps->saber[1].saberFlags &
                 SFL_NO_ROLL_STAB))) { // okay to do roll-stab
  ...
  if (pm->ps->torsoAnim == BOTH_FORCELONGLEAP_LAND ||
      (pm->ps->torsoAnim == BOTH_FORCELONGLEAP_START &&
       !(pm->cmd.buttons &
         BUTTON_ATTACK))) { // if you're in the long-jump and you're not
                            // attacking (or are landing), you're not doing
                            // anything
  ...
  if (pm->ps->legsAnim == BOTH_FLIP_HOLD7 &&
      !(pm->cmd.buttons &
        BUTTON_ATTACK)) { // if you're in the upside-down attack hold, don't do
                          // anything unless you're attacking
    return;
  }

  if (PM_KickingAnim(pm->ps->legsAnim)) {
    if (pm->ps->legsAnimTimer) { // you're kicking, no interruptions
      return;
    }
  ...
  if (pm->ps->saberMoveNext != LS_NONE &&
      (pm->ps->saberMove == LS_READY ||
       pm->ps->saberMove == LS_NONE)) // ready for another one
  ...
  if (!PM_SaberInBrokenParry(pm->ps->saberMove) &&
      ...
      pm->ps->weaponstate !=
          WEAPON_CHARGING) {     // we're not stuck in a broken parry
    if (pm->ps->saberInFlight) { // guiding saber
      if (pm->ps->saberEntityNum < ENTITYNUM_NONE &&
          pm->ps->saberEntityNum > 0) // player is 0
      {                               //
      ...
      if (pm->ps->weaponTime <= 0 || pm->ps->weaponstate != WEAPON_FIRING) {
        if (pm->ps->weapon != pm->cmd.weapon) {
          PM_BeginWeaponChange(pm->cmd.weapon);
                 ...
                 (!pm->ps->dualSabers ||
                  !pm->ps->saber[1].Active())) { // guiding saber
        if (saberInAir) {
  ...
  if (PM_LockedAnim(pm->ps->torsoAnim) &&
      pm->ps->torsoAnimTimer) { // can't interrupt these anims ever
    return;
  }
  if (PM_SuperBreakLoseAnim(pm->ps->torsoAnim) &&
      pm->ps->torsoAnimTimer) { // don't interrupt these anims
    return;
  }
  ...
  if (PM_SaberBlocking()) { // busy blocking, don't do attacks
    return;
  }
  ...
  if (pm->ps->weaponTime > 0) {
    ...
    if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) // player
        && PM_SaberInReturn(pm->ps->saberMove) // in a saber return move -
                                               // FIXME: what about transitions?
        //&& pm->ps->torsoAnimTimer<=250//towards the end of a saber return anim
        && pm->ps->saberBlocked ==
               BLOCKED_NONE // not interacting with any other saber
        && !(pm->cmd.buttons & BUTTON_ATTACK) // not trying to swing the saber
        && (pm->cmd.forwardmove ||
            pm->cmd.rightmove)) // trying to kick in a specific direction
    {
  ...
  if (PM_CanDoKata()) {
    ...
    if (overrideMove ==
        LS_INVALID) { // not overridden by first saber, check second
      if (pm->ps->dualSabers) {
    ...
    if (overrideMove == LS_INVALID) { // not overridden
      ...
      if (pm->gent) {
        G_DrainPowerForSpecialMove(pm->gent, FP_SABER_OFFENSE,
                                   ...
                                   qtrue); // FP_SPEED, SINGLE_SPECIAL_POWER );
        // G_StartMatrixEffect( pm->gent, MEF_REVERSE_SPIN,
        // pm->ps->torsoAnimTimer );
      }
  ...
  if (PM_CheckAltKickAttack()) { // trying to do a kick
    ...
    if (pm->ps->saberAnimLevel == SS_STAFF &&
        (pm->ps->clientNum >= MAX_CLIENTS ||
         PM_ControlledByPlayer())) { // NPCs spin the staff
      // NOTE: only NPCs can do it the easy way... they kick directly, not
      // through ucmds...
  ...
  if (!delayed_fire) {
    ...
    if (pm->ps->weapon == WP_SABER && pm->ps->saberInFlight && saberInAir &&
        (!pm->ps->dualSabers || !pm->ps->saber[1].Active())) { // guiding saber
      if (!PM_ForceAnim(pm->ps->torsoAnim) ||
      ...
      if (pm->ps->torsoAnim == BOTH_FORCELONGLEAP_ATTACK ||
          pm->ps->torsoAnim ==
              BOTH_FORCELONGLEAP_LAND) { // can't attack in these anims
        return;
      } else if (pm->ps->torsoAnim ==
                 BOTH_FORCELONGLEAP_START) { // only 1 attack you can do from
                                             // this anim
        if (pm->ps->torsoAnimTimer >=
            200) { // hit it early enough to do the attack
      ...
      if (anim == -1) {
          ...
          if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer() &&
              (Q_irand(0, pm->ps->forcePowerLevel[FP_SABER_OFFENSE] - 1) ||
               (pm->gent && pm->gent->enemy && pm->gent->enemy->client &&
                PM_InKnockDownOnGround(
                    &pm->gent->enemy->client
                         ->ps)) // enemy knocked down, use some logic
               || (pm->ps->saberAnimLevel == SS_FAST && pm->gent &&
                   pm->gent->NPC && pm->gent->NPC->rank >= RANK_LT_JG &&
                   Q_irand(0, 1)))) // minor change to make fast-attack users
                                    // use the special attacks more
        ...
        */
        if (newmove != LS_NONE) {
          if (!PM_InCartwheel(
                  pm->ps->legsAnim)) { // don't do transitions when cartwheeling
                                       // - could make you spin!
            // Now get the proper transition move
            newmove = PM_SaberAnimTransitionMove((saberMoveName_t)curmove,
                                                 (saberMoveName_t)newmove);
            if (PM_HasAnimation(pm->gent, saberMoveData[newmove].animToUse)) {
              anim = saberMoveData[newmove].animToUse;
            }
      ...
      if (anim == -1) { // not side-stepping, pick neutral anim
        if (!G_TryingSpecial(
                pm->gent,
                &pm->cmd) /*(pm->cmd.buttons&BUTTON_FORCE_FOCUS)*/) { // but
                                                                      // only if
                                                                      // not
                                                                      // trying
                                                                      // one of
          ...
          if (PM_InCartwheel(pm->ps->legsAnim) &&
              pm->ps->legsAnimTimer >
                  100) { // if in the middle of a cartwheel, the chain attack is
      ...
      if (!PM_InCartwheel(
              pm->ps->torsoAnim)) { // can still attack during a cartwheel/arial
        ...
        if (g_timescale->value < 1.0f) {
          if (!MatrixMode) { // Special test for Matrix Mode (tm)
            if (pm->ps->clientNum == 0 && !player_locked &&
                (pm->ps->forcePowersActive & (1 << FP_SPEED) ||
                 pm->ps->forcePowersActive &
                     (1 << FP_RAGE))) { // player always fires at normal speed
              addTime *= g_timescale->value;
            } else if (g_entities[pm->ps->clientNum].client &&
                       (pm->ps->forcePowersActive & (1 << FP_SPEED) ||
                        pm->ps->forcePowersActive & (1 << FP_RAGE))) {
              addTime *= g_timescale->value;
            }
          }
        }
...
void PM_WeaponWampa(void) {
  ...
  if (pm->ps->weapon == WP_SABER && (pm->cmd.buttons & BUTTON_ATTACK) &&
    ...
    PM_SetAnim(pm, SETANIM_BOTH, BOTH_HANG_ATTACK,
               SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    pm->ps->weaponstate = WEAPON_FIRING;
    pm->ps->saberBlocked = BLOCKED_NONE;
    pm->ps->saberMove = LS_READY;
...
static void PM_Weapon(void) {
  ...
  { // Separate logic for lightsaber, but not for player when zoomed
    PM_WeaponLightsaber();
    if (pm->gent && pm->gent->client && pm->ps->saber[0].Active() &&
  ...
  if (pm->gent && pm->gent->client) {
    if (pm->gent->client->fireDelay >
        0) { // FIXME: this is going to fire off one frame before you expect,
             // actually
      pm->gent->client->fireDelay -= pml.msec;
      if (pm->gent->client->fireDelay <= 0) { // just finished delay timer
        ...
        delayed_fire = qtrue;
        if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
            pm->ps->weapon == WP_THERMAL && pm->gent->alt_fire) {
          pm->cmd.buttons |= BUTTON_ALT_ATTACK;
        }
      } else {
        if (pm->ps->clientNum && pm->ps->weapon == WP_ROCKET_LAUNCHER &&
    ...
    if (pm->gent &&
      ...
      case WP_BLASTER_PISTOL:
        if (pm->gent && pm->gent->weaponModel[1] > 0) { // dual pistols
          // FIXME: should be a better way of detecting a dual-pistols user so
          // it's not hardcoded to the saboteurcommando...
          PM_SetAnim(pm, SETANIM_TORSO, BOTH_STAND1, SETANIM_FLAG_NORMAL);
        } else { // single pistol
  ...
  if (pm->gent) { // ready to throw thermal again, add it
    if (pm->ps->weapon == WP_THERMAL &&
        pm->gent->weaponModel[0] ==
            -1) { // add the thermal model back in our hand
      // remove anything if we have anything already
      G_RemoveWeaponModels(pm->gent);
      if (weaponData[pm->ps->weapon]
              .weaponMdl[0]) { // might be NONE, so check if it has a model
        G_CreateG2AttachedWeaponModel(pm->gent,
                                      weaponData[pm->ps->weapon].weaponMdl,
                                      pm->gent->handRBolt, 0);
        // make it sound like we took another one out from... uh.. somewhere...
        if (cg.time > 0) { // this way we don't get that annoying change weapon
                           // sound every time a map starts
          PM_AddEvent(EV_CHANGE_WEAPON);
        }
      }
    }
  ...
  if (!delayed_fire) { // didn't just finish a fire delay
    if (PM_DoChargedWeapons()) {
      // In some cases the charged weapon code may want us to short circuit the
      // rest of the firing code
      return;
    } else {
      if (!pm->gent->client->fireDelay // not already waiting to fire
          ...
          && (pm->cmd.buttons &
              (BUTTON_ATTACK | BUTTON_ALT_ATTACK))) // holding fire
  ...
  if (!delayed_fire) {
    if (pm->ps->weapon == WP_MELEE &&
        (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())) { // melee
      if ((pm->cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK)) !=
          (BUTTON_ATTACK | BUTTON_ALT_ATTACK)) { // not holding both buttons
        if ((pm->cmd.buttons & BUTTON_ATTACK) &&
            (pm->ps->pm_flags & PMF_ATTACK_HELD)) { // held button
          // clear it
          pm->cmd.buttons &= ~BUTTON_ATTACK;
        }
        if ((pm->cmd.buttons & BUTTON_ALT_ATTACK) &&
            (pm->ps->pm_flags & PMF_ALT_ATTACK_HELD)) { // held button
          // clear it
          pm->cmd.buttons &= ~BUTTON_ALT_ATTACK;
        }
      }
    ...
    if (!(pm->cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK))) {
      ...
      if (pm->ps->weapon == WP_MELEE &&
          (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
          PM_KickMove(pm->ps->saberMove)) { // melee, not attacking, clear move
        pm->ps->saberMove = LS_NONE;
      }
      return;
    }
    ...
    // start the animation even if out of ammo
      ...
      if (pm->gent->client->moveType == MT_FLYSWIM) {
        ...
        PM_SetAnim(pm, SETANIM_TORSO, BOTH_ATTACK1,
             ...
             pm->gent->client->NPC_class == CLASS_HAZARD_TROOPER) {
      // Kneel attack
      //--------------
      ...
      //-----------------
             ...
             pm->gent->client->NPC_class == CLASS_ASSASSIN_DROID) {
      // Crouched Attack
...
*/
      ...
      case WP_TUSKEN_RIFLE:
        if (pm->cmd.buttons & BUTTON_ALT_ATTACK) { // shoot
          // in alt-fire, sniper mode
          PM_SetAnim(pm, SETANIM_TORSO, BOTH_ATTACK4,
                     SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        } else { // melee
          int anim = PM_PickAnim(pm->gent, BOTH_TUSKENATTACK1,
                                 BOTH_TUSKENATTACK3); // Rifle
          if (VectorCompare(pm->ps->velocity, vec3_origin) &&
              pm->cmd.upmove >= 0) {
            PM_SetAnim(pm, SETANIM_BOTH, anim,
                       SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD |
        ...
        if (pm->gent && pm->gent->client) {
          ...
          if ((pm->ps->clientNum < MAX_CLIENTS ||
            ...
            if (pm->cmd.buttons & BUTTON_ALT_ATTACK) {
              ...
              anim =
                  PM_PickAnim(pm->gent, BOTH_TUSKENATTACK1, BOTH_TUSKENATTACK3);
            }
          }
          if (VectorCompare(pm->ps->velocity, vec3_origin) &&
              pm->cmd.upmove >= 0) {
      ...
      case WP_THERMAL:
        if ((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer())) {
          if (PM_StandingAnim(pm->ps->legsAnim)) {
            PM_SetAnim(pm, SETANIM_LEGS, BOTH_ATTACK10,
                       SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
          }
      ...
      case WP_EMPLACED_GUN:
        // Guess we don't play an attack animation?  Maybe we should have a
        // custom one??
        break;
  ...
  if (pm->ps->weapon == WP_EMPLACED_GUN) {
    if (pm->gent && pm->gent->owner &&
        pm->gent->owner->e_UseFunc ==
            useF_eweb_use) { // eweb always shoots alt-fire, for proper effects
                             // and sounds
      PM_AddEvent(EV_ALT_FIRE);
      addTime = weaponData[pm->ps->weapon].altFireTime;
    } else { // emplaced gun always shoots normal fire
      PM_AddEvent(EV_FIRE_WEAPON);
      addTime = weaponData[pm->ps->weapon].fireTime;
    }
  } else if ((pm->ps->weapon == WP_MELEE &&
              (pm->ps->clientNum >= MAX_CLIENTS || !g_debugMelee->integer)) ||
    ...
    PM_AddEvent(EV_ALT_FIRE);
    addTime = weaponData[pm->ps->weapon].altFireTime;
    if (pm->ps->weapon == WP_THERMAL) { // threw our thermal
      if (pm->gent) { // remove the thermal model if we had it.
        G_RemoveWeaponModels(pm->gent);
        if ((pm->ps->clientNum >= MAX_CLIENTS &&
             !PM_ControlledByPlayer())) { // NPCs need to know when to put the
                                          // thermal back in their hand
          pm->ps->weaponTime = pm->ps->torsoAnimTimer - 500;
        }
      }
    }
...
void PM_CheckForceUseButton(gentity_t *ent, usercmd_t *ucmd) {
  ...
  if (ucmd->buttons & BUTTON_USE_FORCE) {
    if (!(ent->client->ps.pm_flags & PMF_USEFORCE_HELD)) {
      ...
      case FP_RAGE: // duration - speed, invincibility and extra damage for
                    // short period, drains your health and leaves you weak and
                    // slow afterwards.
        ForceRage(ent);
        break;
      case FP_PROTECT: // duration - protect against physical/energy (level 1
                       // stops blaster/energy bolts, level 2 stops projectiles,
                       // level 3 protects against explosions)
        ForceProtect(ent);
        break;
      case FP_ABSORB: // duration - protect against dark force powers (grip,
                      // lightning, drain - maybe push/pull, too?)
        ForceAbsorb(ent);
        break;
    ...
    case FP_DRAIN:
      ...
      //showPowers[cg.forcepowerSelect]); 			break;
    }
  ...
  if (pm->gent) {
    if (pm->gent->client && pm->gent->client->moveType == MT_FLYSWIM) {
      ...
      if (pm->gent->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER) {
        Flying = FLY_VEHICLE;
      } else if (pm->gent->m_pVehicle->m_pVehicleInfo->hoverHeight >
                 0) { // FIXME: or just check for hoverHeight?
        Flying = FLY_HOVER;
      }
    }
  }
...
//-------------------------------------------
{
  ...
  if (!g_saberAutoBlocking->integer && !g_saberNewControlScheme->integer &&
      (pm->cmd.buttons & BUTTON_FORCE_FOCUS)) {
  ...
  if (pm->ps->weapon == WP_SABER &&
    ...
    if (pm->ps->saberInFlight) {
      ...
      if ((!pm->ps->dualSabers ||
           !pm->ps->saber[1].Active())) { // saber not in hand, can't swing it
        pm->cmd.buttons &= ~BUTTON_ATTACK;
      }
    }
    // saber staff alt-attack does a special attack anim, non-throwable sabers
    // do kicks
    if (pm->ps->saberAnimLevel != SS_STAFF &&
        !(pm->ps->saber[0].saberFlags &
          SFL_NOT_THROWABLE)) { // using a throwable saber, so remove the saber
                                // throw button
      if (!g_saberNewControlScheme->integer &&
          PM_CanDoKata()) { // old control scheme - alt-attack + attack does
                            // kata
      } else { // new control scheme - alt-attack doesn't have anything to do
               // with katas, safe to clear it here
        pm->cmd.buttons &= ~BUTTON_ALT_ATTACK;
      }
    }
  }
  ...
  // for the player?
  if (pm->ps->weapon == WP_DISRUPTOR && pm->gent &&
      (pm->gent->s.number < MAX_CLIENTS || G_ControlledByPlayer(pm->gent)) &&
      pm->ps->weaponstate != WEAPON_DROPPING) {
  ...
  if (((pm->cmd.buttons & BUTTON_ATTACK ||
    ...
    if (pm->cmd.buttons & BUTTON_ALT_ATTACK) {
      pm->ps->eFlags |= EF_ALT_FIRING;
      if (pm->ps->clientNum < MAX_CLIENTS && pm->gent &&
...
void PM_CheckInVehicleSaberAttackAnim(
    void) { // A bit of a hack, but makes the vehicle saber attacks act like any
            // other saber attack...
  // make weapon function
  if (pm->ps->weaponTime > 0) {
    pm->ps->weaponTime -= pml.msec;
  ...
  */
  ...
  if (saberMove != LS_INVALID) {
    if (pm->ps->saberMove == saberMove) { // already playing it
      if (!pm->ps->torsoAnimTimer) {      // anim was done, set it back to ready
        PM_SetSaberMove(LS_READY);
        pm->ps->saberMove = LS_READY;
        pm->ps->weaponstate = WEAPON_IDLE;
        if (pm->cmd.buttons & BUTTON_ATTACK) {
...
void Pmove(pmove_t *pmove) {
  ...
  if (pm->ps->clientNum >= MAX_CLIENTS && pm->gent && pm->gent->client &&
      pm->gent->client->NPC_class == CLASS_VEHICLE) { // we are a vehicle
    pVeh = pm->gent->m_pVehicle;
    assert(pVeh);
    if (pVeh) {
      pVeh->m_fTimeModifier =
          (pml.frametime * 60.0f); // at 16.67ms (60fps), should be 1.0f
    }
  } else if (pm->gent && PM_RidingVehicle()) {
    if (pm->ps->vehTurnaroundIndex &&
        pm->ps->vehTurnaroundTime >
            pm->cmd.serverTime) { // riding this vehicle, turn my view too
  ...
  if (pmove->waterlevel && pm->ps->clientNum) {
    if (pm->ps->forceJumpZStart // force jumping
        ...
        (pm->gent && pm->gent->NPC &&
         level.time <
             pm->gent->NPC->jumpTime)) // TIMER_Done(pm->gent,
                                       // "forceJumpChasing" )) )//force-jumping
    {
  ...
  // set groundentity
  PM_GroundTrace();
  if (Flying == FLY_HOVER) { // never stick to the ground
    PM_HoverTrace();
  }
  ...
  if (pm->ps &&
      ((pm->ps->eFlags & EF_LOCKED_TO_WEAPON) ||
       (pm->ps->eFlags & EF_HELD_BY_RANCOR) ||
       (pm->ps->eFlags & EF_HELD_BY_WAMPA) ||
       (pm->ps->eFlags & EF_HELD_BY_SAND_CREATURE))) { // in an emplaced gun
    PM_NoclipMove();
  ...
  // If we are riding a vehicle...
    ...
    if (pm->cmd.buttons &
        BUTTON_ALT_ATTACK) { // alt attack always does other stuff when riding a
                             // vehicle (turbo)
    } else if ((pm->ps->eFlags &
                EF_NODRAW)) { // inside a vehicle?  don't do any weapon stuff
    } else if (pm->ps->weapon == WP_BLASTER    // using blaster
               || pm->ps->weapon == WP_THERMAL // using thermal
               || pm->ps->weaponstate ==
                      WEAPON_DROPPING // changing weapon - dropping
               || pm->ps->weaponstate ==
                      WEAPON_RAISING // changing weapon - raising
               ||
               (pm->cmd.weapon != pm->ps->weapon &&
                PM_WeaponOkOnVehicle(
                    pm->cmd
                        .weapon))) // FIXME: make this a vehicle call to see if
                                   // this new weapon is valid for this vehicle
    { // either weilding a weapon we can fire with normal weapon logic, or
      // trying to change to a valid weapon
      // call normal weapons code... should we override the normal fire anims
      // with vehicle fire anims in here or in a subsequent call to
      // VehicleWeapons or something?
      // Maybe break PM_Weapon into PM_Weapon and PM_WeaponAnimate (then call
      // our own PM_VehicleWeaponAnimate)?
      PM_Weapon();
    }
    // BUT: now call Vehicle's weapon code, to handle lightsaber and (maybe)
    // overriding weapon ready/firing anims?
  }
  ...
  if (pm->gent) {
    pm->gent->resultspeed =
        ((Distance(pm->ps->origin, pm->gent->currentOrigin) / pml.msec) * 1000);
  ...
  if (pm->gent && pm->ps && pm->ps->eFlags & EF_LOCKED_TO_WEAPON) {
    ...
    } else { // stand (or could be overridden by strafe anims)
      PM_SetAnim(pm, SETANIM_LEGS, BOTH_STAND1, SETANIM_FLAG_NORMAL);
    }
  } else if (pm->gent && pm->ps && (pm->ps->eFlags & EF_HELD_BY_RANCOR)) {
    PM_SetAnim(pm, SETANIM_LEGS, BOTH_SWIM_IDLE1,
               SETANIM_FLAG_OVERRIDE |
                   SETANIM_FLAG_HOLD); // SETANIM_FLAG_NORMAL
  }
