...
void TurboLaser_SetBoneAnim(gentity_t *eweb, int startFrame, int endFrame) {
	...
	gi.G2API_SetBoneAnim(&eweb->ghoul2[0], "model_root", startFrame, endFrame, (BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND), 1.0f, level.time, -1, 100);
}
	...
	if (self->enemy && self->pushDebounceTime < level.time && self->attackDebounceTime < level.time) {
		...
		gi.G2API_GetBoltMatrix(self->ghoul2, 0,
							   (self->spawnflags & SPF_TURRETG2_TURBO)
								   ? ((self->alt_fire ? gi.G2API_AddBolt(&self->ghoul2[0], "*muzzle2") : gi.G2API_AddBolt(&self->ghoul2[0], "*muzzle1")))
								   : gi.G2API_AddBolt(&self->ghoul2[0], "*flash03"),
							   &boltMatrix, self->currentAngles, self->currentOrigin, level.time, NULL, self->modelScale);
		if ((self->spawnflags & SPF_TURRETG2_TURBO)) {
	...
	if (self->enemy) {
		...
		gi.G2API_GetBoltMatrix(self->ghoul2, 0,
							   (self->spawnflags & SPF_TURRETG2_TURBO)
								   ? ((self->alt_fire ? gi.G2API_AddBolt(&self->ghoul2[0], "*muzzle2") : gi.G2API_AddBolt(&self->ghoul2[0], "*muzzle1")))
								   : gi.G2API_AddBolt(&self->ghoul2[0], "*flash03"),
							   &boltMatrix, self->currentAngles, self->s.origin, level.time, NULL, self->modelScale);
	...
	if (dying) {
		...
		*/
		...
		if (!(self->spawnflags & SPF_TURRETG2_TURBO)) {
			...
			// set the new onw
			gi.G2API_InitGhoul2Model(self->ghoul2, name,
									 0, // base->s.modelindex,
									 // note, this is not the same kind of index - this one's referring to the actual
									 // index of the model in the g2 instance, whereas modelindex is the index of a
									 // configstring -rww
			...
			// set the new onw
			gi.G2API_InitGhoul2Model(self->ghoul2, name3,
									 0, // base->s.modelindex,
									 // note, this is not the same kind of index - this one's referring to the actual
									 // index of the model in the g2 instance, whereas modelindex is the index of a
									 // configstring -rww
