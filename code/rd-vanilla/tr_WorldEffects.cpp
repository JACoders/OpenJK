...
inline float FloatRand(void) { return ((float)rand() / (float)RAND_MAX); }
...
struct SVecRange {
  ...
  inline bool In(const CVec3 &V) { return (V > mMins && V < mMaxs); }
};
...
struct SFloatRange {
  ...
  inline void Pick(float &V) { V = Q_flrand(mMin, mMax); }
  inline bool In(const float &V) { return (V > mMin && V < mMax); }
};
...
struct SIntRange {
  ...
  inline bool In(const int &V) { return (V > mMin && V < mMax); }
};
...
public:
  ...
  CVec3 mPosition;
  CVec3 mVelocity;
  float mMass; // A higher number will more greatly resist force and result in
               // greater gravity
};
...
bool R_GetWindVector(vec3_t windVector, vec3_t atpoint) {
  ...
  if (atpoint && mLocalWindZones.size()) {
    for (int curLocalWindZone = 0; curLocalWindZone < mLocalWindZones.size();
         curLocalWindZone++) {
      if (mLocalWindZones[curLocalWindZone]->mRBounds.In(atpoint)) {
        VectorAdd(windVector,
                  mLocalWindZones[curLocalWindZone]->mCurrentVelocity.v,
                  windVector);
      }
    }
...
bool R_GetWindSpeed(float &windSpeed, vec3_t atpoint) {
  ...
  if (atpoint && mLocalWindZones.size()) {
    for (int curLocalWindZone = 0; curLocalWindZone < mLocalWindZones.size();
      ...
      if (mLocalWindZones[curLocalWindZone]->mRBounds.In(atpoint)) {
        windSpeed +=
            VectorLength(mLocalWindZones[curLocalWindZone]->mCurrentVelocity.v);
      }
    }
  }
  return true;
...
////////////////////////////////////////////////////////////////////////////////////////
class COutside {
#define COUTSIDE_STRUCT_VERSION                                                \
  1 // you MUST increase this any time you change any binary (fields) inside
    // this class, or cahced files will fuck up
public:
  ////////////////////////////////////////////////////////////////////////////////////
...
private:
  ...
  struct SWeatherZone {
    ...
    // Convert To Cell
    ////////////////////////////////////////////////////////////////////////////////////
    inline void ConvertToCell(const CVec3 &pos, int &x, int &y, int &z,
                              int &bit) {
    ...
    inline bool CellOutside(int x, int y, int z, int bit) {
      if ((x < 0 || x >= mWidth) || (y < 0 || y >= mHeight) ||
          (z < 0 || z >= mDepth) || (bit < 0 || bit >= 32)) {
        return !(mMarkedOutside);
      }
      return (mMarkedOutside ==
              (!!(mPointCache[((z * mWidth * mHeight) + (y * mWidth) + x)] &
                  (1 << bit))));
    }
  };
...
public:
  ...
  void Reset() {
    ...
    for (int wz = 0; wz < mWeatherZones.size(); wz++) {
      ...
      mWeatherZones[wz].miPointCacheByteSize =
          0; // not really necessary because of .clear() below, but keeps things
             // together in case stuff changes
    }
    mWeatherZones.clear();
  }
  ...
  bool Initialized() { return mCacheInit; }
  ...
  void AddWeatherZone(vec3_t mins, vec3_t maxs) {
    ...
    if (!mWeatherZones.full()) {
      ...
      Wz.miPointCacheByteSize =
          (Wz.mWidth * Wz.mHeight * Wz.mDepth) * sizeof(uint32_t);
      Wz.mPointCache =
          (uint32_t *)R_Malloc(Wz.miPointCacheByteSize, TAG_POINTCACHE, qtrue);
    } else {
  ...
  fileHandle_t WriteCachedWeatherFile(void) {
    ...
    if (f) {
      ...
      ri.Printf(PRINT_WARNING,
                "(Unable to open weather file \"%s\" for writing!)\n",
  ...
  // returns 0 for not-found or invalid file, else open handle to continue read
  // from (which you then close yourself)...
  //
  fileHandle_t ReadCachedWeatherFile(void) {
    ...
    if (f) {
      ...
      ri.FS_Read(&WeatherFileHeaderFromDisk, sizeof(WeatherFileHeaderFromDisk),
                 f);

      if (!memcmp(&WeatherFileHeaderForCompare, &WeatherFileHeaderFromDisk,
                  sizeof(WeatherFileHeaderFromDisk))) {
        // go for it...
        //
        return f;
      }
      ...
      ri.Printf(PRINT_WARNING,
                "( Cached weather file \"%s\" out of date, regenerating... )\n",
                GenCachedWeatherFilename());
      ri.FS_FCloseFile(f);
    } else {
      ri.Printf(PRINT_WARNING,
                "( No cached weather file found, generating... )\n");
    }
  ...
  void Cache() {
    ...
    // all this piece of code does really is fill in the bool
    // "SWeatherZone::mMarkedOutside", plus the mPointCache[] for each zone,
    ...
    if (f) {
      ...
      if (!mWeatherZones.size()) {
        ...
        AddWeatherZone(tr.world->bmodels[0].bounds[0],
              ...
              for (y = 0; y < wz.mHeight; y++) {
                ...
                if (contents & CONTENTS_INSIDE || contents & CONTENTS_OUTSIDE) {
                  ...
                  if (!mCacheInit) {
                    ...
                    assert(0);
                    Com_Error(ERR_DROP, "Weather Effect: Both Indoor and "
                                        "Outdoor brushs encountered in map.\n");
                    return;
                  }

                  // Mark The Point
                  //----------------
                  wz.mPointCache[((z * wz.mWidth * wz.mHeight) +
                                  (y * wz.mWidth) + x)] |= bit;
                }
              } // for (y)
            }   // for (x)
    ...
    if (!mCacheInit) {
      mCacheInit = true;
      SWeatherZone::mMarkedOutside =
          false; // Assume All Is Outside, Except Solid
    }
  }
...
bool R_IsOutside(vec3_t pos) { return mOutside.PointOutside(pos); }
...
bool R_SetTempGlobalFogColor(vec3_t color) {
  if (tr.world && tr.world->globalFog != -1) {
    ...
    if (color[0] || color[1] || color[2]) {
      ...
      tr.world->fogs[tr.world->globalFog].colorInt =
          ColorBytes4(color[0] * tr.identityLight, color[1] * tr.identityLight,
                      color[2] * tr.identityLight, 1.0);
    }

    // If Unable TO Parse The Command Color Vector, Restore The Previous Fog
    // Color
    //-----------------------------------------------------------------------------
    else if (mOutside.mFogColorTempActive) {
      mOutside.mFogColorTempActive = false;

      tr.world->fogs[tr.world->globalFog].parms.color[0] =
          mOutside.mFogColor[0];
      tr.world->fogs[tr.world->globalFog].parms.color[1] =
          mOutside.mFogColor[1];
      tr.world->fogs[tr.world->globalFog].parms.color[2] =
          mOutside.mFogColor[2];
...
public:
  ...
  SFloatRange
      mMass; // Determines how slowness to accelerate, higher number = slower
  float mFrictionInverse; // How much air friction does this particle
                          // have 1.0=none, 0.0=nomove
...
public:
  ...
  // Initialize - Will setup default values for all data
  ////////////////////////////////////////////////////////////////////////////////////
  ~CParticleCloud() { Reset(); }

  ////////////////////////////////////////////////////////////////////////////////////
  // UseSpawnPlane - Check To See If We Should Spawn On A Plane, Or Just Wrap
  // The Box
  ////////////////////////////////////////////////////////////////////////////////////
  inline bool UseSpawnPlane() { return (mGravity != 0.0f); }
  ...
  void Update() {
    ...
    //--------------
    {
      ...
      // If Using A Spawn Plane, Increase The Range Box A Bit To Account For
      // Rotation On The Spawn Plane
      //-------------------------------------------------------------------------------------------------
      if (UseSpawnPlane()) {
        ...
        MakeNormalVectors(mSpawnPlaneNorm.v, mSpawnPlaneRight.v,
                          mSpawnPlaneUp.v);
      }
      ...
      if (mVertexCount == 4) {
        ...
        mCameraLeftPlusUp =
            (mCameraDown +
             mCameraLeft); // should really be called mCamera Left + Down
      }
    }
    ...
    for (particleNum = 0; particleNum < mParticleCount; particleNum++) {
      ...
      if (numLocalWindZones) {
        for (curLocalWindZone = 0; curLocalWindZone < numLocalWindZones;
             curLocalWindZone++) {
      ...
      partInView = (partOutside && partInRange &&
                    (partToCamera.Dot(mCameraForward) > 0.0f));
      ...
      if (!partInRange && !partRendering) {
        ...
        if (UseSpawnPlane()) {
          ...
          part->mPosition +=
              (mSpawnPlaneRight * Q_flrand(-mSpawnPlaneSize, mSpawnPlaneSize));
          part->mPosition +=
              (mSpawnPlaneUp * Q_flrand(-mSpawnPlaneSize, mSpawnPlaneSize));
        }
      ...
      //--------------
      {
        ...
        //---------------------------------
                 ...
                 part->mFlags.get_bit(WFXParticle::FLAG_FADEOUT)) {
  ...
  void Render() {
    ...
    //------------------------------------
    GL_State((mBlendMode == 0) ? (GLS_ALPHA)
    ...
    qglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                     (mFilterMode == 0) ? (GL_LINEAR) : (GL_NEAREST));
    ...
    for (particleNum = 0; particleNum < mParticleCount; particleNum++) {
      ...
      // If Oriented With Velocity, We Want To Calculate Vertx Offsets
      ...
      //---------------------------------------
        ...
        qglColor4f(mColor[0] * part->mAlpha, mColor[1] * part->mAlpha,
                   mColor[2] * part->mAlpha, mColor[3] * part->mAlpha);
      }
      ...
      if (mVertexCount == 3) {
        ...
        qglVertex3f(part->mPosition[0], part->mPosition[1], part->mPosition[2]);
...
////////////////////////////////////////////////////////////////////////////////////////
// Init World Effects - Will Iterate Over All Particle Clouds, Clear Them Out,
// And Erase
////////////////////////////////////////////////////////////////////////////////////////
...
// And Erase
////////////////////////////////////////////////////////////////////////////////////////
void R_ShutdownWorldEffects(void) { R_InitWorldEffects(); }

////////////////////////////////////////////////////////////////////////////////////////
// RB_RenderWorldEffects - If any particle clouds exist, this will update and
// render them
////////////////////////////////////////////////////////////////////////////////////////
void RB_RenderWorldEffects(void) {
  if (!tr.world || (tr.refdef.rdflags & RDF_NOWORLDMODEL) ||
      (backEnd.refdef.rdflags & RDF_SKYBOXPORTAL) || !mParticleClouds.size() ||
      ri.CL_IsRunningInGameCinematic()) { //  no world rendering or no world or
                                          //  no particle clouds
    return;
  }
...
void R_WorldEffectCommand(const char *command) {
  ...
  //---------------
    ...
    vec3_t mins;
    vec3_t maxs;
    if (WE_ParseVector(&command, 3, mins) &&
        WE_ParseVector(&command, 3, maxs)) {
      mOutside.AddWeatherZone(mins, maxs);
    }
  }
...
bool R_IsRaining() { return !mParticleClouds.empty(); }

bool R_IsPuffing() { return false; }
