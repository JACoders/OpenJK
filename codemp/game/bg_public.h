...
#define ARMOR_REDUCTION_FACTOR                                                 \
  0.50 // Certain damage doesn't take off armor as efficiently
...
#define CS_CLIENT_DUELWINNER                                                   \
  29 // current duel round winner - needed for printing at top of scoreboard
#define CS_CLIENT_DUELISTS                                                     \
  30 // client numbers for both current duelists. Needed for a number of
     // client-side things.
#define CS_CLIENT_DUELHEALTHS                                                  \
  31 // nmckenzie: DUEL_HEALTH.  Hopefully adding this cs is safe and good?
...
#define GTB_NOTTEAM                                                            \
  0x03F // **SPECIAL: All of the above gametypes, i.e. not team-based
...
The pmove code takes a playerState_t and a usercmd_t and generates a new
playerState_t and some other output data.  Used for local prediction on the
...
#define AED_SABER_SPIN_TYPE                                                    \
  1 // 0 = saberspinoff, 1 = saberspin, 2-4 = saberspin1-saberspin3
#define AED_SABER_SPIN_PROBABILITY 2

typedef enum { // NOTENOTE:  Be sure to update animEventTypeTable and
               // ParseAnimationEvtBlock(...) if you change this enum list!
  AEV_NONE,
  AEV_SOUND,     //# animID AEV_SOUND framenum soundpath randomlow randomhi
                 //chancetoplay
  ...
  AEV_SOUNDCHAN, //# animID AEV_SOUNDCHAN framenum CHANNEL soundpath randomlow
                 //randomhi chancetoplay
  AEV_SABER_SWING, //# animID AEV_SABER_SWING framenum CHANNEL randomlow
                   //randomhi chancetoplay
...
typedef struct animevent_s {
  ...
  signed short eventData[AED_ARRAY_SIZE]; // Unique IDs, can be soundIndex of
                                          // sound file to play OR effect index
                                          // or footstep type, etc.
  char *stringData; // we allow storage of one string, temporarily (in case we
                    // have to look up an index later, then make sure to set
                    // stringData to NULL so we only do the look-up once)
} animevent_t;
...
// In SP this is shared in with the anim stuff, and humanoid anim sets can be
// loaded multiple times just for the sake of sounds being different. We probably
// wouldn't care normally but since we're working in VMs we have to do everything
// possible to cut memory cost. On the bright side this also means we're cutting
// a rather large size out of required game-side memory.
#ifndef _GAME
...
typedef enum {
  ...
  PM_FLOAT,  // float with no gravity in general direction of velocity (intended
             // for gripping)
...
#define PMF_FIX_MINS                                                           \
  128 // mins have been brought up, keep tracing down to fix them
#define PMF_TIME_WATERJUMP 256 // pm_time is waterjump
#define PMF_RESPAWNED 512      // clear after attack and jump buttons come up
#define PMF_USE_ITEM_HELD 1024
#define PMF_UPDATE_ANIM                                                        \
  2048 // The server updated the animation, the pmove should set the ghoul2 anim
       // to match.
...
typedef struct bgEntity_s {
  ...
  // Data type(s) must directly correspond to the head of the gentity and
  // centity structures
#if defined(__GNUC__) || defined(__GCC__) || defined(MINGW32) ||               \
    defined(MACOS_X)
} _bgEntity_t;
#else
} bgEntity_t;
...
typedef struct pmove_s {
  ...
  // hacky bool so we know if we're dealing with a nonhumanoid (which is
  // probably a rockettrooper)
  ...
  bgEntity_t
      *baseEnt; // base address of the entity array (g_entities or cg_entities)
  int entSize;  // size of the struct (gentity_t or centity_t) so things can be
               // dynamic
} pmove_t;
...
#define SETANIM_FLAG_RESTART                                                   \
  4 // Allow restarting the anim if playing the same one (weapon fires)
...
#define EF_G2ANIMATING                                                         \
  (1 << 0) // perform g2 bone anims based on torsoAnim and legsAnim, works for
           // ET_GENERAL -rww
#define EF_DEAD (1 << 1) // don't draw a foe marker over players with EF_DEAD
//#define	EF_BOUNCE_SHRAPNEL		(1<<2)		// special
//shrapnel flag do not use eflags for server-only things, it wastes bandwidth
// -rww
#define EF_RADAROBJECT (1 << 2) // display on team radar

#define EF_TELEPORT_BIT                                                        \
  (1 << 3) // toggled every time the origin abruptly changes

#define EF_SHADER_ANIM (1 << 4) // Animating shader (by s.frame)

#define EF_PLAYER_EVENT (1 << 5)
//#define	EF_BOUNCE				(1<<5)		// for
//missiles #define	EF_BOUNCE_HALF			(1<<6)		// for
//missiles these aren't even referenced in bg or client code and do not need to
// be eFlags, so I am using these flags for rag stuff -rww

#define EF_RAG (1 << 6) // ragdoll him even if he's alive

#define EF_PERMANENT                                                           \
  (1 << 7) // rww - I am claiming this. (for permanent entities)

#define EF_NODRAW (1 << 8) // may have an event, but no model (unspawned items)
#define EF_FIRING (1 << 9) // for lightning gun
#define EF_ALT_FIRING                                                          \
  (1 << 10) // for alt-fires, mostly for lightning guns though
...
#define EF_SOUNDTRACKER                                                        \
  (1 << 24) // sound position needs to be updated in relation to another entity
#define EF_DROPPEDWEAPON (1 << 25)  // it's a dropped weapon
#define EF_DISINTEGRATION (1 << 26) // being disintegrated by the disruptor
#define EF_INVULNERABLE                                                        \
  (1 << 27) // just spawned in or whatever, so is protected

#define EF_CLIENTSMOOTH                                                        \
  (1 << 28) // standard lerporigin smooth override on client
...
#define EF2_HELD_BY_MONSTER                                                    \
  (1 << 0) // Being held by something, like a Rancor or a Wampa
#define EF2_USE_ALT_ANIM                                                       \
  (1 << 1) // For certain special runs/stands for creatures like the Rancor and
           // Wampa whose runs/stands are conditional
#define EF2_ALERTED                                                            \
  (1 << 2) // For certain special anims, for Rancor: means you've had an enemy,
           // so use the more alert stand
#define EF2_GENERIC_NPC_FLAG (1 << 3) // So far, used for Rancor...
#define EF2_FLYING                                                             \
  (1 << 4) // Flying FIXME: only used on NPCs doesn't *really* have to be passed
           // over, does it?
#define EF2_HYPERSPACE                                                         \
  (1 << 5) // Used to both start the hyperspace effect on the predicted client
           // and to let the vehicle know it can now jump into hyperspace (after
           // turning to face the proper angle)
...
typedef enum {
  ...
  EV_GHOUL2_MARK, // create a projectile impact mark on something with a
                  // client-side g2 instance.
...
} entity_event_t; // There is a maximum of 256 events (8 bits transmission, 2
                  // high bits for uniqueness)
...
typedef enum {
  ...
  MOD_TRIGGER_HURT,
  MOD_TEAM_CHANGE,
  // AURELIO: when/if you put this back in, remember to make a case for it in
  // all the other places where mod's are checked. Also, it probably isn't the
  // most elegant solution for what you want - just add a frag back to the player
  // after you call the player_die (and keep a local of his pre-death score to
  // make sure he actually lost points, there may be cases where you don't lose
  // points on changing teams or suiciding, and so you would actually be giving
  // him a point) -Rich
  // I put it back in for now, if it becomes a problem we'll work around it
  // later (it shouldn't though)...
  MOD_MAX
} meansOfDeath_t;
...
#define SABER_BLOCK_DUR                                                        \
  150 // number of milliseconds a block animation should take.
...
#define MASK_OPAQUE                                                            \
  (CONTENTS_SOLID | CONTENTS_SLIME | CONTENTS_LAVA | CONTENTS_TERRAIN)
#define MASK_SHOT                                                              \
  (CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_CORPSE | CONTENTS_TERRAIN)
...
// rww - Moved all this to bg_public so that we can access the saberMoveData
// stuff on the cgame which is currently used for determining if a saber trail
// should be rendered in a given frame
#ifdef LS_NONE
#undef LS_NONE
#endif
...
typedef struct saberMoveData_s {
  ...
  int blendTime;
  int blocking;
  saberMoveName_t chain_idle;   // What move to call if the attack button is not
                                // pressed at the end of this anim
  saberMoveName_t chain_attack; // What move to call if the attack button (and
...
typedef struct saberTrail_s {
  ...
  vec3_t oldPos[2];
  vec3_t oldNormal[2]; // store this in case we don't have a connect-the-dots
                       // situation
                       //	..then we'll need the normal to project a mark
                       //blob onto the impact point
} saberTrail_t;
...
#define SFL_NOT_THROWABLE                                                      \
  (1 << 1) // can't be thrown - FIXME: maybe make this a max level of force
           // saber throw that can be used with this saber?
#define SFL_NOT_DISARMABLE (1 << 2) // can't be dropped
#define SFL_NOT_ACTIVE_BLOCKING                                                \
  (1 << 3) // don't to try to block incoming shots with this saber
#define SFL_TWO_HANDED (1 << 4) // uses both hands
#define SFL_SINGLE_BLADE_THROWABLE                                             \
  (1 << 5) // can throw this saber if only the first blade is on
#define SFL_RETURN_DAMAGE                                                      \
  (1 << 6) // when returning from a saber throw, it keeps spinning and doing
           // damage
// NEW FLAGS
#define SFL_ON_IN_WATER (1 << 7) // if set, weapon stays active even in water
#define SFL_BOUNCE_ON_WALLS                                                    \
  (1 << 8) // if set, the saber will bounce back when it hits solid architecture
           // (good for real-sword type mods)
#define SFL_BOLT_TO_WRIST                                                      \
  (1 << 9) // if set, saber model is bolted to wrist, not in hand... useful for
           // things like claws & shields, etc.
//#define SFL_STICK_ON_IMPACT		(1<<?)//if set, the saber will stick in
//the wall when thrown and hits solid architecture (good for sabers that are
//meant to be thrown).
//#define SFL_NO_ATTACK				(1<<?)//if set, you cannot attack
//with the saber (for sabers/weapons that are meant to be thrown only, not used
//as melee weapons). Move Restrictions
#define SFL_NO_PULL_ATTACK                                                     \
  (1 << 10) // if set, cannot do pull+attack move (move not available in MP
            // anyway)
#define SFL_NO_BACK_ATTACK (1 << 11) // if set, cannot do back-stab moves
#define SFL_NO_STABDOWN                                                        \
  (1 << 12) // if set, cannot do stabdown move (when enemy is on ground)
#define SFL_NO_WALL_RUNS                                                       \
  (1 << 13) // if set, cannot side-run or forward-run on walls
#define SFL_NO_WALL_FLIPS                                                      \
  (1 << 14) // if set, cannot do backflip off wall or side-flips off walls
...
#define SFL_NO_KICKS                                                           \
  (1 << 19) // if set, cannot do kicks (can't do kicks anyway if using a
            // throwable saber/sword)
#define SFL_NO_MIRROR_ATTACKS                                                  \
  (1 << 20) // if set, cannot do the simultaneous attack left/right moves (only
            // available in Dual Lightsaber Combat Style)
#define SFL_NO_ROLL_STAB                                                       \
  (1 << 21) // if set, cannot do roll-stab move at end of roll
// SABER FLAGS2
// Primary Blade Style
#define SFL2_NO_WALL_MARKS                                                     \
  (1 << 0) // if set, stops the saber from drawing marks on the world (good for
           // real-sword type mods)
#define SFL2_NO_DLIGHT                                                         \
  (1 << 1) // if set, stops the saber from drawing a dynamic light (good for
           // real-sword type mods)
#define SFL2_NO_BLADE                                                          \
  (1 << 2) // if set, stops the saber from drawing a blade (good for real-sword
           // type mods)
#define SFL2_NO_CLASH_FLARE                                                    \
  (1 << 3) // if set, the saber will not do the big, white clash flare with
           // other sabers
#define SFL2_NO_DISMEMBERMENT                                                  \
  (1 << 4) // if set, the saber never does dismemberment (good for pointed/blunt
           // melee weapons)
#define SFL2_NO_IDLE_EFFECT                                                    \
  (1 << 5) // if set, the saber will not do damage or any effects when it is
           // idle (not in an attack anim).  (good for real-sword type mods)
#define SFL2_ALWAYS_BLOCK                                                      \
  (1 << 6) // if set, the blades will always be blocking (good for things like
           // shields that should always block)
#define SFL2_NO_MANUAL_DEACTIVATE                                              \
  (1 << 7) // if set, the blades cannot manually be toggled on and off
#define SFL2_TRANSITION_DAMAGE                                                 \
  (1 << 8) // if set, the blade does damage in start, transition and return
           // anims (like strong style does)
// Secondary Blade Style
#define SFL2_NO_WALL_MARKS2                                                    \
  (1 << 9) // if set, stops the saber from drawing marks on the world (good for
           // real-sword type mods)
#define SFL2_NO_DLIGHT2                                                        \
  (1 << 10) // if set, stops the saber from drawing a dynamic light (good for
            // real-sword type mods)
#define SFL2_NO_BLADE2                                                         \
  (1 << 11) // if set, stops the saber from drawing a blade (good for real-sword
            // type mods)
#define SFL2_NO_CLASH_FLARE2                                                   \
  (1 << 12) // if set, the saber will not do the big, white clash flare with
            // other sabers
#define SFL2_NO_DISMEMBERMENT2                                                 \
  (1 << 13) // if set, the saber never does dismemberment (good for
            // pointed/blunt melee weapons)
#define SFL2_NO_IDLE_EFFECT2                                                   \
  (1 << 14) // if set, the saber will not do damage or any effects when it is
            // idle (not in an attack anim).  (good for real-sword type mods)
#define SFL2_ALWAYS_BLOCK2                                                     \
  (1 << 15) // if set, the blades will always be blocking (good for things like
            // shields that should always block)
#define SFL2_NO_MANUAL_DEACTIVATE2                                             \
  (1 << 16) // if set, the blades cannot manually be toggled on and off
#define SFL2_TRANSITION_DAMAGE2                                                \
  (1 << 17) // if set, the blade does damage in start, transition and return
            // anims (like strong style does)

#define SABER_NAME_LENGTH (64)
typedef struct saberInfo_s {
  char name[SABER_NAME_LENGTH];     // entry in sabers.cfg, if any
  char fullName[SABER_NAME_LENGTH]; // the "Proper Name" of the saber, shown in
                                    // UI
  ...
  int numBlades;
  bladeInfo_t
      blade[MAX_BLADES]; // blade info - like length, trail, origin, dir, etc.
  ...
  int maxChain; // how many moves can be chained in a row with this weapon (-1
                // is infinite, 0 is use default behavior)
  int forceRestrictions; // force powers that cannot be used while this saber is
                         // on (bitfield) - FIXME: maybe make this a limit on
                         // the max level, per force power, that can be used
                         // with this type?
  ...
  int disarmBonus, disarmBonus2; // added to disarm chance when win saberlock or
                                 // have a good parry (knockaway)
  saber_styles_t singleBladeStyle; // makes it so that you use a different style
                                   // if you only have the first blade active
  ...
  qhandle_t
      swingSound[3]; // none - if set, plays one of these 3 sounds when swung
                     // during an attack - NOTE: must provide all 3!!!
  ...
  int kataMove; // LS_INVALID - if set, player will execute this move when they
                // press both attack buttons at the same time
  int lungeAtkMove;   // LS_INVALID - if set, player will execute this move when
                      // they crouch+fwd+attack
  int jumpAtkUpMove;  // LS_INVALID - if set, player will execute this move when
                      // they jump+attack
  int jumpAtkFwdMove; // LS_INVALID - if set, player will execute this move when
                      // they jump+fwd+attack
  int jumpAtkBackMove;  // LS_INVALID - if set, player will execute this move
                        // when they jump+back+attack
  int jumpAtkRightMove; // LS_INVALID - if set, player will execute this move
                        // when they jump+rightattack
  int jumpAtkLeftMove;  // LS_INVALID - if set, player will execute this move
                        // when they jump+left+attack
  ...
  //***NOTE: you can only have a maximum of 2 "styles" of blades, so this next
  //value, "bladeStyle2Start" is the number of the first blade to use these
  //value on... all blades before this use the normal values above, all blades
  //at and after this number use the secondary values below***
  int bladeStyle2Start; // 0 - if set, blades from this number and higher use
                        // the following values (otherwise, they use the normal
                        // values already set)

  //***The following can be different for the extra blades - not setting them
  //individually defaults them to the value for the whole saber (and first
  //blade)***

  // done in cgame (client-side code)
  int trailStyle,
      trailStyle2; // 0 - default (0) is normal, 1 is a motion blur and 2 is no
                   // trail at all (good for real-sword type mods)
  int g2MarksShader, g2MarksShader2; // none - if set, the game will use this
                                     // shader for marks on enemies instead of
                                     // the default "gfx/damage/saberglowmark"
  int g2WeaponMarkShader,
      g2WeaponMarkShader2; // none - if set, the game will ry to project this
                           // shader onto the weapon when it damages a person
                           // (good for a blood splatter on the weapon)
  qhandle_t hitSound[3],
      hit2Sound[3]; // none - if set, plays one of these 3 sounds when saber
                    // hits a person - NOTE: must provide all 3!!!
  qhandle_t blockSound[3],
      block2Sound[3]; // none - if set, plays one of these 3 sounds when
                      // saber/sword hits another saber/sword - NOTE: must
                      // provide all 3!!!
  qhandle_t bounceSound[3],
      bounce2Sound[3]; // none - if set, plays one of these 3 sounds when
                       // saber/sword hits a wall and bounces off (must set
                       // bounceOnWall to 1 to use these sounds) - NOTE: must
                       // provide all 3!!!
  int blockEffect, blockEffect2; // none - if set, plays this effect when the
                                 // saber/sword hits another saber/sword
                                 // (instead of "saber/saber_block.efx")
  int hitPersonEffect,
      hitPersonEffect2; // none - if set, plays this effect when the saber/sword
                        // hits a person (instead of
                        // "saber/blood_sparks_mp.efx")
  int hitOtherEffect,
      hitOtherEffect2; // none - if set, plays this effect when the saber/sword
                       // hits something else damagable (instead of
                       // "saber/saber_cut.efx")
  int bladeEffect,
      bladeEffect2; // none - if set, plays this effect at the blade tag

  // done in game (server-side code)
  float knockbackScale,
      knockbackScale2; // 0 - if non-zero, uses damage done to calculate an
                       // appropriate amount of knockback
  float damageScale,
      damageScale2; // 1 - scale up or down the damage done by the saber
  float splashRadius, splashRadius2; // 0 - radius of splashDamage
  int splashDamage,
      splashDamage2; // 0 - amount of splashDamage, 100% at a distance of 0, 0%
                     // at a distance = splashRadius
  float splashKnockback,
      splashKnockback2; // 0 - amount of splashKnockback, 100% at a distance of
                        // 0, 0% at a distance = splashRadius
} saberInfo_t;
#define MAX_SABERS 2

bgEntity_t *PM_BGEntForNum(int num);
qboolean BG_KnockDownable(playerState_t *ps);
qboolean BG_LegalizedForcePowers(char *powerOut, size_t powerOutSize,
                                 int maxRank, qboolean freeSaber, int teamForce,
                                 int gametype, int fpDisabled);

// given a boltmatrix, return in vec a normalised vector for the axis requested
// in flags
void BG_GiveMeVectorFromMatrix(mdxaBone_t *boltMatrix, int flags, vec3_t vec);

void BG_IK_MoveArm(void *ghoul2, int lHandBolt, int time, entityState_t *ent,
                   int basePose, vec3_t desiredPos, qboolean *ikInProgress,
                   vec3_t origin, vec3_t angles, vec3_t scale, int blendTime,
                   qboolean forceHalt);

void BG_G2PlayerAngles(void *ghoul2, int motionBolt, entityState_t *cent,
                       int time, vec3_t cent_lerpOrigin, vec3_t cent_lerpAngles,
                       matrix3_t legs, vec3_t legsAngles, qboolean *tYawing,
                       qboolean *tPitching, qboolean *lYawing, float *tYawAngle,
                       float *tPitchAngle, float *lYawAngle, int frametime,
                       vec3_t turAngles, vec3_t modelScale, int ciLegs,
                       int ciTorso, int *corrTime, vec3_t lookAngles,
                       vec3_t lastHeadAngles, int lookTime,
...
void BG_ForcePowerDrain(playerState_t *ps, forcePowers_t forcePower,
                        int overrideAmt);
...
extern void BG_AttachToRancor(void *ghoul2, float rancYaw, vec3_t rancOrigin,
                              int time, qhandle_t *modelList, vec3_t modelScale,
                              qboolean inMouth, vec3_t out_origin,
                              vec3_t out_angles, matrix3_t out_axis);
