...
static QINLINE qboolean CG_Piloting(int vehNum) {
  ...
  if (veh->currentState.owner !=
      cg.predictedPlayerState
          .clientNum) { // the owner should be the current pilot
    return qfalse;
  }
...
void CG_BuildSolidList(void) {
  ...
  if (cg_numSolidEntities < MAX_ENTITIES_IN_SNAPSHOT) {
    ...
    cg_solidEntities[cg_numSolidEntities]->currentState.solid =
        (k << 16) | (j << 8) | i;
...
static QINLINE qboolean CG_VehicleClipCheck(centity_t *ignored,
                                            trace_t *trace) {
  if (!trace || trace->entityNum < 0 ||
      trace->entityNum >= ENTITYNUM_WORLD) { // it's alright then
    return qtrue;
  }

  if (ignored->currentState.eType != ET_PLAYER &&
      ignored->currentState.eType != ET_NPC) { // can't possibly be valid then
    return qtrue;
  }

  if (ignored->currentState
          .m_iVehicleNum) { // see if the ignore ent is a vehicle/rider - if so,
                            // see if the ent we supposedly hit is a
                            // vehicle/rider.
    ...
    if (otherguy->currentState.eType != ET_PLAYER &&
        otherguy->currentState.eType !=
            ET_NPC) { // can't possibly be valid then
      return qtrue;
    }

    if (otherguy->currentState
            .m_iVehicleNum) { // alright, both of these are either a vehicle or
                              // a player who is on a vehicle
      int index;

      if (ignored->currentState.eType == ET_PLAYER ||
          (ignored->currentState.eType == ET_NPC &&
           ignored->currentState.NPC_class !=
               CLASS_VEHICLE)) { // must be a player or NPC riding a vehicle
        index = ignored->currentState.m_iVehicleNum;
      } else { // a vehicle
        index = ignored->currentState.m_iVehicleNum - 1;
      }

      if (index == otherguy->currentState
                       .number) { // this means we're riding or being ridden by
                                  // this guy, so don't collide
        return qfalse;
      } else { // see if I'm hitting one of my own passengers
        if (otherguy->currentState.eType == ET_PLAYER ||
            (otherguy->currentState.eType == ET_NPC &&
             otherguy->currentState.NPC_class !=
                 CLASS_VEHICLE)) { // must be a player or NPC riding a vehicle
          if (otherguy->currentState.m_iVehicleNum ==
              ignored->currentState.number) { // this means we're other guy is
                                              // riding the ignored ent
            return qfalse;
          }
        }
      }
...
static void CG_ClipMoveToEntities(const vec3_t start, const vec3_t mins,
  ...
  for (i = 0; i < cg_numSolidEntities; i++) {
    ...
    if (ent->number > MAX_CLIENTS && (ent->genericenemyindex - MAX_GENTITIES ==
                                          cg.predictedPlayerState.clientNum ||
                                      ent->genericenemyindex - MAX_GENTITIES ==
                                          cg.predictedVehicleState.clientNum))
    //		if (ent->number > MAX_CLIENTS && cg.snap && ent->genericenemyindex &&
    //(ent->genericenemyindex-MAX_GENTITIES) == cg.snap->ps.clientNum)
    { // rww - method of keeping objects from colliding in client-prediction (in
      // case of ownership)
      continue;
    }
    ...
    if (ent->solid == SOLID_BMODEL) {
      ...
      if (ent->eType == ET_NPC && ent->NPC_class == CLASS_VEHICLE &&
          cent->m_pVehicle) { // try to dynamically adjust his bbox dynamically,
                              // if possible
        ...
        BG_VehicleAdjustBBoxForOrientation(
            cent->m_pVehicle, cent->lerpOrigin, bmins, bmaxs,
    ...
    trap->CM_TransformedTrace(&trace, start, end, mins, maxs, cmodel, mask,
                              origin, angles, 0);
    trace.entityNum = trace.fraction != 1.0 ? ent->number : ENTITYNUM_NONE;

    if (g2Check || (ignored && ignored->currentState.m_iVehicleNum)) {
      // keep these older variables around for a bit, incase we need to replace
      // them in the Ghoul2 Collision check or in the vehicle owner trace check
      oldTrace = *tr;
    }
    ...
    if (tr->allsolid) {
      if (ignored && ignored->currentState.m_iVehicleNum) {
        trace.entityNum = ent->number;
        if (CG_VehicleClipCheck(
                ignored,
                &trace)) { // this isn't our vehicle, we're really stuck
          return;
        } else { // it's alright, keep going
          trace = oldTrace;
          *tr = trace;
        }
      } else {
    ...
    if (g2Check) {
      if (trace.entityNum == ent->number && cent->ghoul2) {
        ...
        if (trace.entityNum ==
            ENTITYNUM_NONE) { // g2 trace failed, so put it back where it was.
          trace = oldTrace;
          *tr = trace;
        }
      }
    ...
    if (ignored && ignored->currentState
                       .m_iVehicleNum) { // see if this is the vehicle we hit
      centity_t *hit = &cg_entities[trace.entityNum];
      if (!CG_VehicleClipCheck(ignored, &trace)) { // looks like it
        trace = oldTrace;
        *tr = trace;
      } else if (hit->currentState.eType == ET_MISSILE &&
                 hit->currentState.owner ==
                     ignored->currentState
                         .number) { // hack, don't hit own missiles
        trace = oldTrace;
        *tr = trace;
      }
    }
...
int CG_PointContents(const vec3_t point, int passEntityNum) {
  ...
  for (i = 0; i < cg_numSolidEntities; i++) {
    ...
    contents |= trap->CM_TransformedPointContents(
...
static void CG_InterpolateVehiclePlayerState(qboolean grabAngles) {
  ...
  *out = cg.snap->vps;
  ...
  for (i = 0; i < 3; i++) {
    out->origin[i] =
        prev->vps.origin[i] + f * (next->vps.origin[i] - prev->vps.origin[i]);
    if (!grabAngles) {
      out->viewangles[i] =
          LerpAngle(prev->vps.viewangles[i], next->vps.viewangles[i], f);
    }
...
static void CG_TouchItem(centity_t *cent) {
  ...
  if (cent->currentState.brokenLimbs) { // dropped item
    return;
  }
  ...
  // Currently there is no reliable way of knowing if the client has touched a
  // certain item before another if they are next to each other, or rather if the
  // server has touched them in the same order. This results often in grabbing an
  // item in the prediction and the server giving you the other item. So for now
  // prediction of armor, health, and ammo is disabled.
  /*
          if (item->giType == IT_ARMOR)
          { //rww - this will be stomped next update, but it's set so that we
     don't try to pick up two shields in one prediction and have the server
     cancel one
          ...
          //	cg.predictedPlayerState.stats[STAT_HOLDABLE_ITEMS] |= (1 <<
     item->giTag);
          }
  */
  ...
  if (item->giType == IT_POWERUP &&
      (item->giTag == PW_FORCE_ENLIGHTENED_LIGHT ||
       item->giTag == PW_FORCE_ENLIGHTENED_DARK)) {
  ...
  // grab it
  BG_AddPredictableEventToPlayerstate(EV_ITEM_PICKUP, cent->currentState.number,
                                      &cg.predictedPlayerState);
...
static void CG_TouchTriggerPrediction(void) {
  ...
  for (i = 0; i < cg_numTriggerEntities; i++) {
    ...
    trap->CM_Trace(&trace, cg.predictedPlayerState.origin,
                   cg.predictedPlayerState.origin, cg_pmove.mins, cg_pmove.maxs,
                   cmodel, -1, 0);
...
qboolean CG_UsingEWeb(void) {
  if (cg.predictedPlayerState.weapon == WP_EMPLACED_GUN &&
      cg.predictedPlayerState.emplacedIndex &&
      cg_entities[cg.predictedPlayerState.emplacedIndex].currentState.weapon ==
          WP_NONE) {
    return qtrue;
  }
...
void CG_PredictPlayerState(void) {
  ...
  // rww - bgghoul2
  if (cg_pmove.ghoul2 !=
      pEnt->ghoul2) // only update it if the g2 instance has changed
  {
    if (cg.snap && pEnt->ghoul2 && !(cg.snap->ps.pm_flags & PMF_FOLLOW) &&
        cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR) {
      cg_pmove.ghoul2 = pEnt->ghoul2;
      cg_pmove.g2Bolts_LFoot =
          trap->G2API_AddBolt(pEnt->ghoul2, 0, "*l_leg_foot");
      cg_pmove.g2Bolts_RFoot =
          trap->G2API_AddBolt(pEnt->ghoul2, 0, "*r_leg_foot");
    } else {
      cg_pmove.ghoul2 = NULL;
    }
  }
  ...
  if (cg.nextSnap && !cg.nextFrameTeleport && !cg.thisFrameTeleport) {
    cg.nextSnap->ps.slopeRecalcTime =
        cg.predictedPlayerState
            .slopeRecalcTime; // this is the only value we want to maintain
                              // seperately on server/client
  ...
  for (i = 0; i < MAX_GENTITIES; i++) {
    ...
    if (cg_entities[i].currentState.eType == ET_PLAYER ||
      ...
      VectorCopy(cg_entities[i].currentState.pos.trDelta,
                 cgSendPS[i]->velocity);
  ...
  if (CG_Piloting(cg.predictedPlayerState.m_iVehicleNum)) {
    cg_entities[cg.predictedPlayerState.clientNum].playerState =
        &cg.predictedPlayerState;
    cg_entities[cg.predictedPlayerState.m_iVehicleNum].playerState =
        &cg.predictedVehicleState;

    // use the player command time, because we are running with the player cmds
    // (this is even the case on the server)
  ...
  for (cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++) {
    ...
    if (CG_Piloting(cg.predictedPlayerState.m_iVehicleNum) &&
        cg.predictedPlayerState.pm_type !=
            PM_INTERMISSION) { // we're riding a vehicle, let's predict it
      centity_t *veh = &cg_entities[cg.predictedPlayerState.m_iVehicleNum];
      int x, zd, zu;

      if (veh->m_pVehicle) { // make sure pointer is set up to go to our
                             // predicted state
        veh->m_pVehicle->m_vOrientation =
            &cg.predictedVehicleState.vehOrientation[0];

        // keep this updated based on what the playerstate says
        veh->m_pVehicle->m_iRemovedSurfaces =
            cg.predictedVehicleState.vehSurfaces;
        ...
        */
        // I think this was actually wrong.. just copy-pasted from id code. Oh
        // well.
        x = (veh->currentState.solid) & 255;
        ...
        if (!cg_vehPmoveSet) { // do all the one-time things
        ...
        // update boarding value sent from server. boarding is not predicted,
        // but no big deal
        ...
        AngleSubtract(veh->m_pVehicle->m_vOrientation[0],veh->m_pVehicle->m_vPrevOrientation[0])
        );
        }
        */
  ...
  if (cg.predictedPlayerState.m_iVehicleNum &&
      !CG_Piloting(
          cg.predictedPlayerState
              .m_iVehicleNum)) { // a passenger on this vehicle, bolt them in
    centity_t *veh = &cg_entities[cg.predictedPlayerState.m_iVehicleNum];
...
revertES:
  if (CG_Piloting(cg.predictedPlayerState.m_iVehicleNum)) {
    ...
    if (veh->m_pVehicle) {
      ...
      veh->m_pVehicle->m_vOrientation =
          &cgSendPS[veh->currentState.number]->vehOrientation[0];
    }

    cg_entities[cg.predictedPlayerState.clientNum].playerState =
        cgSendPS[cg.predictedPlayerState.clientNum];
    veh->playerState = cgSendPS[veh->currentState.number];
  }

  // copy some stuff back into the entstates to help actually "predict" them if
  // applicable
  for (i = 0; i < MAX_GENTITIES; i++) {
    if (cg_entities[i].currentState.eType == ET_PLAYER ||
        cg_entities[i].currentState.eType == ET_NPC) {
