...
extern qboolean G_BoundsOverlap(const vec3_t mins1, const vec3_t maxs1,
...
void G_PlayDoorSound(gentity_t *ent, int type) {
  ...
  if (ent->activator && ent->activator->client &&
      ent->activator->client->playerTeam == TEAM_PLAYER) {
...
gentity_t *G_TestEntityPosition(gentity_t *ent) {
  ...
  if ((ent->client && ent->health <= 0) ||
      !ent->clipmask) { // corpse or something with no clipmask
    mask = MASK_SOLID;
  } else {
    mask = ent->clipmask;
  }
  if (ent->client) {
    gi.trace(&tr, ent->client->ps.origin, ent->mins, ent->maxs,
             ent->client->ps.origin, ent->s.number, mask, G2_NOCOLLIDE, 0);
  } else {
    if (ent->s.eFlags &
        EF_MISSILE_STICK) // Arggh, this is dumb...but when it used the bbox, it
                          // was pretty much always in solid when it is riding
                          // something..which is wrong..so I changed it to
                          // basically be a point contents check
    {
      gi.trace(&tr, ent->s.pos.trBase, vec3_origin, vec3_origin,
               ent->s.pos.trBase, ent->s.number, mask, G2_NOCOLLIDE, 0);
    } else {
      gi.trace(&tr, ent->s.pos.trBase, ent->mins, ent->maxs, ent->s.pos.trBase,
               ent->s.number, mask, G2_NOCOLLIDE, 0);
    }
  }
...
qboolean G_TryPushingEntity(gentity_t *check, gentity_t *pusher, vec3_t move,
  ...
  */
  ...
  {//sigh... allow pushing corpses into walls... problem is, they'll be stuck in
  there... maybe just remove them? return qtrue;
  }
  */
  ...
  // blocked
  if (pusher->damage) { // Do damage
    G_Damage(check, pusher, pusher->activator, move, check->currentOrigin,
             pusher->damage, 0, MOD_CRUSH);
  }
...
qboolean G_MoverPush(gentity_t *pusher, vec3_t move, vec3_t amove,
  ...
  *obstacle = NULL;
  ...
  if (!pusher->bmodel) { // misc_model_breakable
  ...
  if (pusher->currentAngles[0] || pusher->currentAngles[1] ||
      pusher->currentAngles[2] || amove[0] || amove[1] || amove[2]) {
    float radius;
  ...
  listedEntities =
      gi.EntitiesInBox(totalMins, totalMaxs, entityList, MAX_GENTITIES);
  ...
  notMoving = (qboolean)(VectorCompare(vec3_origin, move) &&
  ...
  for (e = 0; e < listedEntities; e++) {
    ...
    if ((check->s.eFlags & EF_MISSILE_STICK) &&
        (notMoving || check->s.groundEntityNum < 0 ||
         check->s.groundEntityNum >= ENTITYNUM_NONE)) {
      // special case hack for sticky things, destroy it if we aren't attached
      // to the thing that is moving, but the moving thing is pushing us
    ...
    if (check->s.eType != ET_ITEM) {
      // FIXME: however it allows items to be pushed through stuff, do same for
      // corpses?
      ...
      */
        ...
        if (G_BoundsOverlap(
                checkMins, checkMaxs, pusherMins,
                pusherMaxs)) { // They're inside me already, no push - FIXME:
                               // we're testing a moves spot, aren't we, so we
                               // could have just moved inside them?
          continue;
        }
      }
    }
    ...
    if (check->maxs[0] - check->mins[0] <= 0 &&
        ...
        check->maxs[2] - check->mins[2] <= 0) { // no size, don't push
      continue;
    }
    ...
    if (((pusher->spawnflags & 2) &&
         !Q_stricmp("func_breakable", pusher->classname)) ||
        ((pusher->spawnflags & 16) &&
         !Q_stricmp(
             "func_static",
             pusher->classname))) { // ugh, avoid stricmp with a unique flag
      // Damage on impact
      if (pusher->damage) { // Do damage
        G_Damage(check, pusher, pusher->activator, move, check->currentOrigin,
                 pusher->damage, 0, MOD_CRUSH);
        if (pusher->health >= 0 && pusher->takedamage &&
            !(pusher->spawnflags & 1)) { // do some damage to me, too
          G_Damage(pusher, check, pusher->activator, move, pusher->s.pos.trBase,
                   floor(pusher->damage / 4.0f), 0, MOD_CRUSH);
        }
      }
    }
    // really need a flag like MOVER_TOUCH that calls the ent's touch function
    // here, instead of this stricmp crap
    else if ((pusher->spawnflags & 2) &&
             !Q_stricmp("func_rotating", pusher->classname)) {
    ...
    if (G_TryPushingEntity(check, pusher, move, amove)) {
      ...
      if (check->s.eFlags & EF_MISSILE_STICK) {
        if (!VectorCompare(oldOrg, check->s.pos.trBase)) {
          // and the rider was actually pushed, so interpolate position change
          // to smooth out the ride
          check->s.pos.trType = TR_INTERPOLATE;
          continue;
        }
        // else..the mover wasn't blocked & we are riding the mover & but the
        // rider did not move even though the mover itself did...so 	drop through
        //and let it blow up the rider up
      } else {
        continue;
      }
    }
...
void G_MoverTeam(gentity_t *ent) {
  ...
  if (part) {
    ...
    if (ent->e_BlockedFunc !=
        blockedF_NULL) { // this check no longer necessary, done internally
                         // below, but it's here for reference
...
void Use_BinaryMover_Go(gentity_t *ent) {
  ...
  if (ent->moverState == MOVER_2TO1) {
    ...
    if (ent->s.pos.trType == TR_NONLINEAR_STOP) {
      ...
      float fPartial =
          VectorLength(curDelta) / VectorLength(ent->s.pos.trDelta);
...
void Use_BinaryMover(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  ...
  if (ent->e_UseFunc == useF_NULL) { // I cannot be used anymore, must be a door
                                     // with a wait of -1 that's opened.
    return;
  }
  ...
  if (ent->spawnflags & MOVER_GOODIE) {
    if (ent->fly_sound_debounce_time > level.time) {
      ...
      if (key) { // activator has a goodie key, remove it
        activator->client->ps.inventory[key]--;
        G_Sound(activator, G_SoundIndex("sound/movers/goodie_pass.wav"));
        // once the goodie mover has been used, it no longer requires a goodie
        // key
        ent->spawnflags &= ~MOVER_GOODIE;
      } else { // don't have a goodie key
        ...
        // FIXME: temp message, only on certain difficulties?, graphic instead
        // of text?
...
void InitMover(gentity_t *ent) {
  ...
  if (ent->model2) {
    if (strstr(ent->model2, ".glm")) {
      ...
      ent->playerModel =
          gi.G2API_InitGhoul2Model(ent->ghoul2, ent->model2, ent->s.modelindex2,
  ...
  if (ent->spawnflags &
      MOVER_PLAYER_USE) { // Can be used by the player's BUTTON_USE
...
void Blocked_Door(gentity_t *ent, gentity_t *other) {
  ...
  if ((other->s.eType == ET_ITEM) && (other->item->giTag >= INV_GOODIE_KEY &&
                                      other->item->giTag <= INV_SECURITY_KEY)) {
  ...
  // if your not a client, or your a dead client remove yourself...
           ...
           (!other->client ||
            (other->client && other->health <= 0 &&
             other->contents == CONTENTS_CORPSE && !other->message))) {
...
void Touch_DoorTrigger(gentity_t *ent, gentity_t *other, trace_t *trace) {
  ...
  if (ent->owner->spawnflags &
      MOVER_LOCKED) { // don't even try to use the door if it's locked
    return;
  }

  if (ent->owner->moverState != MOVER_1TO2) { // Door is not already opening
    // if ( ent->owner->moverState == MOVER_POS1 || ent->owner->moverState ==
    // MOVER_2TO1 )
...
gentity_t *G_FindDoorTrigger(gentity_t *ent) {
  ...
  if (door->targetname) { // find out what is targeting it
    // FIXME: if ent->targetname, check what kind of trigger/ent is targetting
    // it?  If a normal trigger (active, etc), then it's okay?
  ...
  if (G_EntIsDoor(entityNum)) {
    ...
    if (ent->targetname) { // find out what is targetting it
      ...
      while ((owner = G_Find(owner, FOFS(target), ent->targetname)) != NULL) {
        if (!Q_stricmp("trigger_multiple",
                       owner->classname)) // FIXME: other triggers okay too?
        {
      ...
      if (owner && (owner->svFlags &
                    SVF_INACTIVE)) { // owning auto-created trigger is inactive
        return qfalse;
      }
    }
    if (!(ent->svFlags & SVF_INACTIVE) && // assumes that the reactivate trigger
                                          // isn't right next to the door!
        !ent->health && !(ent->spawnflags & MOVER_PLAYER_USE) &&
...
/*QUAKED func_door (0 .5 .8) ? START_OPEN FORCE_ACTIVATE CRUSHER TOGGLE LOCKED
GOODIE PLAYER_USE INACTIVE START_OPEN	the door to moves to its destination
when spawned, and operate in reverse.  It is used to temporarily or permanently
close off an area when triggered (not useful for touch or takedamage doors).
...
NOT work on Trek doors. LOCKED		Starts locked, with the shader animmap
at the first frame and inactive.  Once used, the animmap changes to the second
frame and the door operates normally.  Note that you cannot use the door again
after this.
GOODIE		Door will not work unless activator has a "goodie key" in his
inventory PLAYER_USE	Player can use it with the use button INACTIVE	must be
used by a target_activate before it can be used

"target"     Door fires this when it starts moving from it's closed position to
its open position. "opentarget" Door fires this after reaching its "open"
position "target2"    Door fires this when it starts moving from it's open
position to its closed position. "closetarget" Door fires this after reaching
its "closed" position "model2"	.md3 model to also draw "angle"
...
none "lip"		lip remaining at end of move (8 default) "dmg"
damage to inflict when blocked (2 default, set to negative for no damage)
...
(default is 0) 0 - no sound (default)
*/
void SP_func_door(gentity_t *ent) {
  ...
  if (ent->spawnflags &
      MOVER_LOCKED) { // a locked door, set up as locked until used directly
  ...
  if (!(ent->flags & FL_TEAMSLAVE)) {
    ...
    if (!(ent->spawnflags & MOVER_LOCKED) &&
        (ent->targetname || health || ent->spawnflags & MOVER_PLAYER_USE ||
         ent->spawnflags & MOVER_FORCE_ACTIVATE)) {
...
void SP_func_plat(gentity_t *ent) {
  ...
  //G_SoundIndex("sound/movers/plats/pt1_strt.wav"); 	ent->soundPos1 =
  //ent->soundPos2 = G_SoundIndex("sound/movers/plats/pt1_end.wav");
...
void Think_BeginMoving(gentity_t *ent) {
  ...
  if (ent->spawnflags & 2048) {
    // this tie fighter hack is done for doom_detention, where the shooting
    // gallery takes place. let them draw again when they start moving
...
void Reached_Train(gentity_t *ent) {
  ...
  /*
          ...
          {
                  ent->s.loopSound = CAS_GetBModelSound( next->soundSet, BMS_MID
     );//ent->soundLoop;
          }
  */
  G_PlayDoorLoopSound(ent);
  ...
  // This is for the tie fighter shooting gallery on doom detention, you could
  // see them waiting under the bay, but the architecture couldn't easily be
  // changed..
...
void Think_SetupTrainTargets(gentity_t *ent) {
  ...
  // FIXME: this can go into an infinite loop if last path_corner doesn't link
  // to first path_corner, like so:
  // t1---->t2---->t3
  //         ^      |
  //          \_____|
  start = NULL;
  for (path = ent->nextTrain; path != start; path = next) {
    ...
    if (!path->target) {
      //			gi.Printf( "Train corner at %s without a
      //target\n", 				vtos(path->s.origin) ); end of path
      break;
    }
...
/*QUAKED path_corner (.5 .3 0) (-8 -8 -8) (8 8 8) TURN_TRAIN INVISIBLE YAW_TRAIN
ROLL_TRAIN

TURN_TRAIN	func_train moving on this path will turn to face the next
path_corner within 2 seconds INVISIBLE - train will become invisible ( but still
solid ) when it reaches this path_corner. It will become visible again at the
next path_corner that does not have this option checked
...
void func_train_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
...
/*QUAKED func_train (0 .5 .8) ? START_ON TOGGLE BLOCK_STOPS x x LOOP PLAYER_USE
INACTIVE TIE

LOOP - if it's a ghoul2 model, it will just loop the animation
PLAYER_USE	Player can use it with the use button
INACTIVE	must be used by a target_activate before it can be used
TIE  flying Tie-fighter hack, should be made more flexible so other things can
use this if needed
...
"light"		constantLight radius
"linear" set to 1 and it will move linearly rather than with acceleration
(default is 0) "startframe" default 0...ghoul2 animation start frame "endframe"
default 0...ghoul2 animation end frame
*/
void SP_func_train(gentity_t *self) {
  ...
  if (self->playerModel >= 0 &&
      self->spawnflags &
          32) // loop...dunno, could support it on other things, but for now I
              // need it for the glider...so...kill the flag
  {
    self->spawnflags &= ~32; // once only

    gi.G2API_SetBoneAnim(&self->ghoul2[self->playerModel], "model_root",
                         self->startFrame, self->endFrame,
                         BONE_ANIM_OVERRIDE_LOOP,
                         1.0f + Q_flrand(-1.0f, 1.0f) * 0.1f, 0, -1, -1);
    self->endFrame = 0; // don't allow it to do anything with the animation
                        // function in G_main
  }
}
...
/*QUAKED func_static (0 .5 .8) ? F_PUSH F_PULL SWITCH_SHADER CRUSHER IMPACT x
PLAYER_USE INACTIVE BROADCAST F_PUSH		Will be used when you Force-Push
it F_PULL		Will be used when you Force-Pull it SWITCH_SHADER
Toggle the shader anim frame between 1 and 2 when used
...
"light"		constantLight radius
"dmg"		how much damage to do when it crushes (use with CRUSHER
spawnflag) "linear" set to 1 and it will move linearly rather than with
acceleration (default is 0)
*/
void SP_func_static(gentity_t *ent) {
  ...
  if (ent->spawnflags & 2048) {    // yes this is very very evil, but for now
                                   // (pre-alpha) it's a solution
    ent->svFlags |= SVF_BROADCAST; // I need to rotate something that is huge
                                   // and it's touching too many area portals...
  }
...
void func_rotating_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (self->s.apos.trType != TR_STATIONARY &&
      !(other->flags & FL_DISINTEGRATED)) // only damage if it's moving
  {
    //		VectorCopy( self->currentOrigin, spot );
    //		tent = G_TempEntity( spot, EV_DISINTEGRATION );
    //		tent->s.eventParm = PW_DISRUPTION;
    //		tent->owner = self;
    // let G_Damage call the fx instead, beside, this way you can disintegrate a
    // corpse this way
    G_Sound(other, G_SoundIndex("sound/effects/energy_crackle.wav"));
    G_Damage(other, self, self, NULL, NULL, 10000, DAMAGE_NO_KNOCKBACK,
             MOD_SNIPER);
  }
}

void func_rotating_use(gentity_t *self, gentity_t *other,
                       gentity_t *activator) {
  if (self->s.apos.trType == TR_LINEAR) {
    ...
    if (VALIDSTRING(self->soundSet) == true) {
      G_AddEvent(self, EV_BMODEL_SOUND,
                 CAS_GetBModelSound(self->soundSet, BMS_END));
    }
  } else {
    if (VALIDSTRING(self->soundSet) == true) {
      G_AddEvent(self, EV_BMODEL_SOUND,
                 CAS_GetBModelSound(self->soundSet, BMS_START));
...
/*QUAKED func_rotating (0 .5 .8) ? START_ON TOUCH_KILL X_AXIS Y_AXIS x x
PLAYER_USE INACTIVE PLAYER_USE	Player can use it with the use button TOUCH_KILL
Inflicts massive damage upon touching it, disitegrates bodies INACTIVE	must be
used by a target_activate before it can be used
...
void func_bobbing_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  if (self->s.pos.trType == TR_SINE) {
    ...
    // Store the current phase value so we know how to start up where we left
    // off.
    self->radius =
        (level.time - self->s.pos.trTime) / (float)self->s.pos.trDuration;
  } else {
...
/*QUAKED func_bobbing (0 .5 .8) ? X_AXIS Y_AXIS START_OFF x x x PLAYER_USE
INACTIVE PLAYER_USE	Player can use it with the use button INACTIVE	must be
used by a target_activate before it can be used
...
void security_panel_use(gentity_t *self, gentity_t *other,
                        gentity_t *activator) {
  if (!activator) {
    return;
  }
  if (INV_SecurityKeyCheck(activator, self->message)) { // congrats!
    ...
    if (activator->ghoul2.size()) {
      gi.G2API_SetSurfaceOnOff(&activator->ghoul2[activator->playerModel],
                               "l_arm_key", 0x00000002);
    }
  ...
  } else {                    // failure sound/display
    if (activator->message) { // have a key, just the wrong one
      gi.SendServerCommand(0, "cp @INGAME_INCORRECT_KEY");
    } else { // don't have a key at all
