...
int CFxScheduler::ScheduleLoopedEffect(int id, int boltInfo,
                                       CGhoul2Info_v *ghoul2, bool isPortal,
                                       int iLoopTime, bool isRelative) {
  int i;

  assert(id);
  assert(boltInfo != -1);

  for (i = 0; i < MAX_LOOPED_FX;
       i++) // see if it's already playing so we can just update it
  {
  ...
  mLoopedEffectArray[i].mNextTime =
      theFxHelper.mTime + mEffectTemplates[id].mRepeatDelay;
  mLoopedEffectArray[i].mLoopStopTime =
      (iLoopTime == 1) ? 0 : theFxHelper.mTime + iLoopTime;
  return i;
}
...
void CFxScheduler::AddLoopedEffects() {
  ...
  for (i = 0; i < MAX_LOOPED_FX; i++) {
    if (mLoopedEffectArray[i].mId &&
        mLoopedEffectArray[i].mNextTime < theFxHelper.mTime) {
      const int entNum =
          (mLoopedEffectArray[i].mBoltInfo >> ENTITY_SHIFT) & ENTITY_AND;
      ...
      PlayEffect(
          mLoopedEffectArray[i].mId, data->mPoint, 0,
          mLoopedEffectArray[i].mBoltInfo, mLoopedEffectArray[i].mGhoul2, -1,
          mLoopedEffectArray[i].mPortalEffect, false,
          mLoopedEffectArray[i].mIsRelative); // very important to send FALSE to
                                              // not recursively add me!
      mLoopedEffectArray[i].mNextTime =
          theFxHelper.mTime +
          mEffectTemplates[mLoopedEffectArray[i].mId].mRepeatDelay;
      if (mLoopedEffectArray[i].mLoopStopTime &&
          mLoopedEffectArray[i].mLoopStopTime < theFxHelper.mTime) // time's up
      { // kill this entry
        memset(&mLoopedEffectArray[i], 0, sizeof(mLoopedEffectArray[i]));
      }
    }
...
//
//------------------------------------------------------
void CFxScheduler::Clean(bool bRemoveTemplates /*= true*/,
                         int idToPreserve /*= 0*/) {
  int i, j;
...
//------------------------------------------------------
int CFxScheduler::RegisterEffect(const char *file,
                                 bool bHasCorrectPath /*= false*/) {
  // Dealing with file names:
  // File names can come from two places - the editor, in which case we should
  // use the given path as is, and the effect file, in which case we should add
  // the correct path and extension. In either case we create a stripped file
  // name to use for naming effects.
  //
  ...
  // see if the specified file is already registered.  If it is, just return the
  // id of that file
  ...
  // kef - grr. this angers me. every filename everywhere should start from the
  // base dir
  if (effectsSubstr.compare("effects") != 0) {
    // theFxHelper.Print("Hey!!! '%s' should be pathed from the base
    // directory!!!\n", finalFilename.c_str());
...
void CFxScheduler::AddPrimitiveToEffect(SEffectTemplate *fx,
  ...
  if (ct >= FX_MAX_EFFECT_COMPONENTS) {
    theFxHelper.Print(
        "FxScheduler:  Error--too many primitives in an effect\n");
  } else {
  ...
  for (int i = 1; i < FX_MAX_EFFECTS; i++) {
    ...
    if (!effect->mInUse) {
      *id = i;
      ...
      // If we are a copy, we really won't have a name that we care about saving
      // for later
      if (file) {
        mEffectIDs[file] = i;
...
//				you have to hold onto this if you intend to call it
//again
//
// Return:
...
SEffectTemplate *CFxScheduler::GetEffectCopy(int fxHandle, int *newHandle) {
  ...
  if (copy && *newHandle) {
    ...
    *copy = mEffectTemplates[fxHandle];
    ...
    // the user had better hold onto this handle if they ever hope to call this
    // effect.
    return copy;
  }
...
//------------------------------------------------------
CPrimitiveTemplate *CFxScheduler::GetPrimitiveCopy(SEffectTemplate *effectCopy,
...
void CFxScheduler::MaterialImpact(trace_t *tr, CEffect *effect) {
  /*	EMatImpactEffect matImpactEffect = effect->GetMatImpactFX();
          ...
          else if (matImpactEffect == MATIMPACTFX_SHELLSOUND)
          {
                  ...
                  {
                          theFxHelper.PlaySound( tr->endpos, ENTITYNUM_NONE,
     CHAN_AUTO, materials[material].GetShellSoundHandle(ammoName) );
                  }
          }*/
}
...
//	none
//------------------------------------------------------
void CFxScheduler::PlayEffect(int id, vec3_t origin, vec3_t forward, int vol,
                              int rad, bool isPortal) {
  matrix3_t axis;
...
//	none
//------------------------------------------------------
void CFxScheduler::PlayEffect(const char *file, vec3_t origin, matrix3_t axis,
                              const int boltInfo, CGhoul2Info_v *ghoul2,
                              int fxParm /*-1*/, int vol, int rad,
                              int iLoopTime, bool isRelative) {
  char sfile[MAX_QPATH];
  ...
  PlayEffect(mEffectIDs[sfile], origin, axis, boltInfo, ghoul2, fxParm, vol,
             rad, qfalse, iLoopTime, isRelative);
}

int totalPrimitives = 0;
int totalEffects = 0;

void GetRGB_Colors(CPrimitiveTemplate *fx, vec3_t outStartRGB,
                   vec3_t outEndRGB) {
  float percent;
  ...
  if (fx->mSpawnFlags & FX_RGB_COMPONENT_INTERP) {
    ...
    VectorSet(outStartRGB, fx->mRedStart.GetVal(percent),
              fx->mGreenStart.GetVal(percent), fx->mBlueStart.GetVal(percent));
    VectorSet(outEndRGB, fx->mRedEnd.GetVal(percent),
              fx->mGreenEnd.GetVal(percent), fx->mBlueEnd.GetVal(percent));
  } else {
    VectorSet(outStartRGB, fx->mRedStart.GetVal(), fx->mGreenStart.GetVal(),
              fx->mBlueStart.GetVal());
    VectorSet(outEndRGB, fx->mRedEnd.GetVal(), fx->mGreenEnd.GetVal(),
              fx->mBlueEnd.GetVal());
  }
}
...
//	Optional boltInfo (defaults to -1)
//  Optional entity number to be used by a cheap entity origin bolt (defaults to
//  -1)
//
// Return:
//	none
//------------------------------------------------------
void CFxScheduler::PlayEffect(int id, vec3_t origin, matrix3_t axis,
                              const int boltInfo, CGhoul2Info_v *ghoul2,
                              int fxParm /*-1*/, int vol, int rad,
                              bool isPortal /*false*/, int iLoopTime /*0*/,
                              bool isRelative) {
  SEffectTemplate *fx;
  CPrimitiveTemplate *prim;
  int i = 0;
  ...
  if (boltInfo > 0) {
    ...
    // We always force ghoul bolted objects to be scheduled so that they don't
    // play right away.
    forceScheduling = true;

    if (iLoopTime) // 0 = not looping, 1 for infinite, else duration
    {              // store off the id to reschedule every frame
      ScheduleLoopedEffect(id, boltInfo, ghoul2, !!isPortal, iLoopTime,
                           isRelative);
    }
  }
  ...
  for (i = 0; i < fx->mPrimitiveCount; i++) {
    ...
    if (prim->mCullRange) {
      if (DistanceSquared(origin, theFxHelper.refdef->vieworg) >
          prim->mCullRange) // cullrange gets squared on load
      {
        // is too far away
        continue;
      }
    ...
    if (prim->mCopy) {
      ...
      // that we
      //	can keep the primitive template around for the correct amount of
      //time.
      prim->mRefCount = count;
    }

    if (prim->mSpawnFlags & FX_EVEN_DISTRIBUTION) {
      factor = abs(prim->mSpawnDelay.GetMax() - prim->mSpawnDelay.GetMin()) /
               (float)count;
    }
    ...
    for (int t = 0; t < count; t++) {
      ...
      if (delay < 1 && !forceScheduling && !isPortal) {
        ...
        if (sfx == NULL) {
          Com_Error(ERR_DROP,
                    "ERROR: Failed to allocate EFX from memory pool.");
          return;
        }
        ...
        if (boltInfo == -1) {
          sfx->ghoul2 = NULL;
          if (entityNum == -1) {
            // we aren't bolting, so make sure the spawn system knows this by
            // putting -1's in these fields
            sfx->mBoltNum = -1;
            ...
            // we are doing bolting onto the origin of the entity, so use a
            // cheaper method
            sfx->mBoltNum = -1;
          ...
          // Also, the ghoul bolt may not be around yet, so delay the creation
          // one frame
          sfx->mStartTime++;
        }
...
//	none
//------------------------------------------------------
bool gEffectsInPortal =
    qfalse; // this is just because I don't want to have to add an mPortalEffect
            // field to every actual effect.
...
void CFxScheduler::AddScheduledEffects(bool portal) {
  ...
  for (itr = mFxSchedule.begin(); itr != mFxSchedule.end(); /* do nothing */) {
    ...
    if (portal == effect->mPortalEffect &&
        effect->mStartTime <=
            theFxHelper.mTime) { // only render portal fx on the skyportal pass
                                 // and vice versa
      if (effect->mBoltNum ==
          -1) { // ok, are we spawning a bolt on effect or a normal one?
        if (effect->mEntNum != ENTITYNUM_NONE) {
          ...
          CreateEffect(effect->mpTemplate, effect->mOrigin, effect->mAxis,
                       theFxHelper.mTime - effect->mStartTime);
        }
      } else { // bolted on effect
        // do we need to go and re-get the bolt matrix again? Since it takes
        // time lets try to do it only once
        if ((effect->mModelNum != oldModelNum) ||
          ...
          doesBoltExist = theFxHelper.GetOriginAxisFromBolt(
              effect->ghoul2, effect->mEntNum, effect->mModelNum,
              effect->mBoltNum, origin, axis);
        }

        // only do this if we found the bolt
        if (doesBoltExist) {
          if (effect->mIsRelative) {
            CreateEffect(effect->mpTemplate, origin, axis, 0, -1,
                         effect->ghoul2, effect->mEntNum, effect->mModelNum,
                         effect->mBoltNum);
          } else {
            CreateEffect(effect->mpTemplate, origin, axis,
                         theFxHelper.mTime - effect->mStartTime);
          }
        }
      }
...
bool CFxScheduler::Add2DEffect(float x, float y, float w, float h, vec4_t color,
...
void CFxScheduler::Draw2DEffects(float screenXScale, float screenYScale) {
  ...
  for (int i = 0; i < mNextFree2DEffect; i++) {
    ...
    re->DrawStretchPic(x - (w * 0.5f), y - (h * 0.5f), w, h, 0, 0, 1, 1,
                       /*m2DEffects[i].mColor,*/ m2DEffects[i].mShaderHandle);
  }
  // now that all 2D effects have been drawn we can consider the entire array to
  // be free
  mNextFree2DEffect = 0;
}
...
//	none
//------------------------------------------------------
void CFxScheduler::CreateEffect(CPrimitiveTemplate *fx, const vec3_t origin,
                                matrix3_t axis, int lateTime, int fxParm /*-1*/,
                                CGhoul2Info_v *ghoul2, int entNum, int modelNum,
                                int boltNum) {
  ...
  int flags = fx->mFlags;
  if (ghoul2 != NULL && modelNum >= 0 &&
      boltNum >= 0) { // since you passed in these values, mark as relative to
                      // use them if it is supported
    switch (fx->mType) {
    case Particle:
    case Line:
    case Tail:
  ...
  if (fx->mSpawnFlags & FX_CHEAP_ORG_CALC ||
      flags & FX_RELATIVE) { // let's take the easy way out
    VectorSet(org, fx->mOrigin1X.GetVal(), fx->mOrigin1Y.GetVal(),
              fx->mOrigin1Z.GetVal());
  ...
  // We always add our calculated offset to the passed in origin, unless
  // relative!
  if (!(flags & FX_RELATIVE)) {
    VectorAdd(org, origin, org);
  }
  // Now, we may need to calc a point on a sphere/ellipsoid/cylinder/disk and
  // add that to it
  //----------------------------------------------------------------
  if (fx->mSpawnFlags & FX_ORG_ON_SPHERE) {
    ...
    VectorSet(temp, sin(x) * width * sin(y), cos(x) * width * sin(y),
              cos(y) * height); // sinx * siny, cosx * siny, cosy
    ...
    // set up our point, then rotate around the current direction to.  Make
    // unrotated cylinder centered around 0,0,0
  ...
  if (fx->mType == OrientedParticle) { // bolted oriented particles use origin2
                                       // as an angular rotation offset...
    if (flags & FX_RELATIVE) {
      VectorSet(ax[0], fx->mOrigin2X.GetVal(), fx->mOrigin2Y.GetVal(),
                fx->mOrigin2Z.GetVal());
    }
  }

  // There are only a few types that really use velocity and acceleration, so do
  // extra work for those types
  //--------------------------------------------------------------------------------------------------------
  if (fx->mType == Particle || fx->mType == OrientedParticle ||
    ...
    if (fx->mSpawnFlags & FX_VEL_IS_ABSOLUTE || flags & FX_RELATIVE) {
      VectorSet(vel, fx->mVelX.GetVal(), fx->mVelY.GetVal(),
                fx->mVelZ.GetVal());
    ...
    if (fx->mSpawnFlags & FX_AFFECTED_BY_WIND) {
      /*rjr			vec3_t wind;
                              ...
                              // wind is affecting us, so modify our initial
         velocity.  ideally, we would update throughout our lives, but this is
         easier CL_GetWindVector( wind ); VectorMA( vel,
         fx->mWindModifier.GetVal() * 0.01f, wind, vel );
                              */
    }
    ...
    if (fx->mSpawnFlags & FX_ACCEL_IS_ABSOLUTE || flags & FX_RELATIVE) {
      VectorSet(accel, fx->mAccelX.GetVal(), fx->mAccelY.GetVal(),
                fx->mAccelZ.GetVal());
    } else {
  ...
  if (fx->mType == Line || fx->mType == Electricity) {
    ...
    if (fx->mSpawnFlags & FX_ORG2_FROM_TRACE) {
      ...
      if (fx->mSpawnFlags &
          FX_ORG2_IS_OFFSET) { // add a random flair to the endpoint...note:
                               // org2 will have to be pretty large to affect
                               // this much
        // we also do this pre-trace as opposed to post trace since we may have
        // to render an impact effect
        //	and we will want the normal at the exact endpos...
        if (fx->mSpawnFlags & FX_CHEAP_ORG2_CALC || flags & FX_RELATIVE) {
          VectorSet(org2, fx->mOrigin2X.GetVal(), fx->mOrigin2Y.GetVal(),
                    fx->mOrigin2Z.GetVal());
          VectorAdd(org2, temp, temp);
        } else { // I can only imagine a few cases where you might want to do
                 // this...
  ...
  case Particle:
    ...
    FX_AddParticle(org, vel, accel, fx->mSizeStart.GetVal(),
                   fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(),
                   fx->mAlphaStart.GetVal(), fx->mAlphaEnd.GetVal(),
                   fx->mAlphaParm.GetVal(), sRGB, eRGB, fx->mRGBParm.GetVal(),
                   fx->mRotation.GetVal(), fx->mRotationDelta.GetVal(),
                   fx->mMin, fx->mMax, fx->mElasticity.GetVal(),
                   fx->mDeathFxHandles.GetHandle(),
                   fx->mImpactFxHandles.GetHandle(), fx->mLife.GetVal(),
                   fx->mMediaHandles.GetHandle(), flags, fx->mMatImpactFX,
  ...
  case Line:
    ...
    FX_AddLine(org, org2, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(),
               fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(),
               fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB, eRGB,
               fx->mRGBParm.GetVal(), fx->mLife.GetVal(),
  ...
  case Tail:
    ...
    FX_AddTail(org, vel, accel, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(),
               fx->mSizeParm.GetVal(), fx->mLengthStart.GetVal(),
               fx->mLengthEnd.GetVal(), fx->mLengthParm.GetVal(),
               fx->mAlphaStart.GetVal(), fx->mAlphaEnd.GetVal(),
               fx->mAlphaParm.GetVal(), sRGB, eRGB, fx->mRGBParm.GetVal(),
               fx->mMin, fx->mMax, fx->mElasticity.GetVal(),
               fx->mDeathFxHandles.GetHandle(),
  ...
  case Electricity:
    ...
    FX_AddElectricity(org, org2, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(),
                      fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(),
                      fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB,
                      eRGB, fx->mRGBParm.GetVal(), fx->mElasticity.GetVal(),
                      fx->mLife.GetVal(), fx->mMediaHandles.GetHandle(), flags,
                      fx->mMatImpactFX, fxParm, ghoul2, entNum, modelNum,
                      boltNum);
    break;
  ...
  case Cylinder:
    ...
    FX_AddCylinder(org, ax[0], fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(),
                   fx->mSizeParm.GetVal(), fx->mSize2Start.GetVal(),
                   fx->mSize2End.GetVal(), fx->mSize2Parm.GetVal(),
                   fx->mLengthStart.GetVal(), fx->mLengthEnd.GetVal(),
                   fx->mLengthParm.GetVal(), fx->mAlphaStart.GetVal(),
                   fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB, eRGB,
                   fx->mRGBParm.GetVal(), fx->mLife.GetVal(),
                   fx->mMediaHandles.GetHandle(), flags, fx->mMatImpactFX,
                   fxParm, ghoul2, entNum, modelNum, boltNum,
  ...
  case Emitter:
    ...
    VectorSet(ang, fx->mAngle1.GetVal(), fx->mAngle2.GetVal(),
              fx->mAngle3.GetVal());

    vectoangles(ax[0], temp);
    VectorAdd(ang, temp, ang);

    VectorSet(angDelta, fx->mAngle1Delta.GetVal(), fx->mAngle2Delta.GetVal(),
              fx->mAngle3Delta.GetVal());

    emitterModel = fx->mMediaHandles.GetHandle();

    FX_AddEmitter(
        org, vel, accel, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(),
        fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(),
        fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB, eRGB,
        fx->mRGBParm.GetVal(), ang, angDelta, fx->mMin, fx->mMax,
        fx->mElasticity.GetVal(), fx->mDeathFxHandles.GetHandle(),
        fx->mImpactFxHandles.GetHandle(), fx->mEmitterFxHandles.GetHandle(),
        fx->mDensity.GetVal(), fx->mVariance.GetVal(), fx->mLife.GetVal(),
        emitterModel, flags, fx->mMatImpactFX, fxParm);
    break;
  ...
  case Decal:
    ...
    /* rjr
                    // This function is a somewhat higher-cost function for
       projecting a decal mark onto a surface.
                    // We shouldn't always need this, only for big hits or when
       a decal is very close-up.

                    // If the impact size is greater than 6, don't even try to
       use cheap sprites.  Big marks need real decals. if
       (fx->mSizeStart.GetVal()<6)
                    {
                            vec3_t dest, normal;
                            int findmarkret;
                            ...
                            {	// Legal to put down a mark.
                                    ...
                                    if ( findmarkret == FINDMARK_CHEAP ||
       // If we can't put down a decal OR if and distance > 200 DistanceSquared(
       dest, cg.refdef.vieworg ) > 40000) {	// Use cheap oriented particle
       decals. vec3_t zerovec={0,0,0};

                                            FX_AddOrientedParticle( effectCloud,
       org, ax[0], zerovec,	// velocity zerovec,	// acceleration
                                                            fx->mSizeStart.GetVal(),
       fx->mSizeStart.GetVal(), 0,	// size params fx->mAlphaStart.GetVal(),
       0, 0.75,						// alpha params (start fading at 75%
       life)
                                                            sRGB, sRGB, 0,
       // rgb params
                                                            fx->mRotation.GetVal(),
       0,								// rotation
       delta zerovec,	// min zerovec,	// max 0,			//
       bounce 0,			// deathID 0,			//
       impactID 20000,		// lifetime fx->mMediaHandles.GetHandle(),
       ...
       -1);
                                    ...
                                    {	// Use the expensive kind.
                                            ...
                                            color.rgba.a =
       fx->mAlphaStart.GetVal() * 0xff;

                                            CG_ImpactMark(
       fx->mMediaHandles.GetHandle(), org, ax[0], fx->mRotation.GetVal(),
       color.c, 12000, 8000, true, fx->mSizeStart.GetVal(), false );
                                    }
                            }
                    }
                    else
                    {	// Use the expensive kind.
                            ...
                            CG_ImpactMark( fx->mMediaHandles.GetHandle(), org,
       ax[0], fx->mRotation.GetVal(), color.c, 12000, 8000, true,
       fx->mSizeStart.GetVal(), false );
                    } */

    theFxHelper.AddDecalToScene(fx->mMediaHandles.GetHandle(), org, ax[0],
                                fx->mRotation.GetVal(), sRGB[0], sRGB[1],
                                sRGB[2], fx->mAlphaStart.GetVal(), qtrue,
                                fx->mSizeStart.GetVal(), qfalse);

    if (fx->mFlags & FX_GHOUL2_DECALS) {
      theFxHelper.AddGhoul2Decal(fx->mMediaHandles.GetHandle(), org, ax[0],
                                 fx->mSizeStart.GetVal());
    }
  ...
  case OrientedParticle:
    ...
    FX_AddOrientedParticle(
        org, ax[0], vel, accel, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(),
        fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(),
        fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB, eRGB,
        fx->mRGBParm.GetVal(), fx->mRotation.GetVal(),
        fx->mRotationDelta.GetVal(), fx->mMin, fx->mMax,
        fx->mElasticity.GetVal(), fx->mDeathFxHandles.GetHandle(),
        fx->mImpactFxHandles.GetHandle(), fx->mLife.GetVal(),
        fx->mMediaHandles.GetHandle(), flags, fx->mMatImpactFX, fxParm, ghoul2,
  ...
  case Sound:
    //---------
    if (gEffectsInPortal) { // could orient this anyway for panning, but eh.
                            // It's going to appear to the player in the sky the
                            // same place no matter what, so just make it a local
                            // sound.
      theFxHelper.PlayLocalSound(fx->mMediaHandles.GetHandle(), CHAN_AUTO);
    } else {
      theFxHelper.PlaySound(org, ENTITYNUM_NONE, CHAN_AUTO,
                            fx->mMediaHandles.GetHandle(), fx->mSoundVolume,
                            fx->mSoundRadius);
    }
    break;
  ...
  case Light:
    ...
    FX_AddLight(org, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(),
                fx->mSizeParm.GetVal(), sRGB, eRGB, fx->mRGBParm.GetVal(),
                fx->mLife.GetVal(), flags, fx->mMatImpactFX, fxParm, ghoul2,
                entNum, modelNum, boltNum);
    break;
  ...
  case CameraShake:
    //---------
    // It calculates how intense the shake should be based on how close you are
    // to the origin you pass in here
    //	elasticity is actually the intensity...radius is the distance in which
    //the shake will have some effect 	life is how long the effect lasts.
    theFxHelper.CameraShake(org, fx->mElasticity.GetVal(), fx->mRadius.GetVal(),
                            fx->mLife.GetVal());
    break;
  ...
  case ScreenFlash:
    ...
    FX_AddFlash(org, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(),
                fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(),
                fx->mAlphaEnd.GetVal(), fx->mAlphaParm.GetVal(), sRGB, eRGB,
                fx->mRGBParm.GetVal(), fx->mLife.GetVal(),
                fx->mMediaHandles.GetHandle(), flags, fx->mMatImpactFX, fxParm);
...
//
// Input:
//	template used to build the effect, and the scheduled effect we are based
//off of
//
// Return:
//	none
//------------------------------------------------------
void CFxScheduler::CreateEffect(CPrimitiveTemplate *fx,
                                SScheduledEffect *scheduledFx) {
  int boltInfo;

  // annoying bit....we have to pack the values back into an int before calling
  // playEffect since there isn't the ideal overload we can already use.
  ...
  PlayEffect(fx->mPlayFxHandles.GetHandle(), scheduledFx->mOrigin,
             scheduledFx->mAxis, boltInfo);
}
