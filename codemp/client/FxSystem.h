...
public:
  ...
  inline void CloseFile(fileHandle_t fh) { FS_FCloseFile(fh); }

  // Sound
  inline void PlaySound(vec3_t origin, int entityNum, int entchannel,
                        sfxHandle_t sfxHandle, int volume, int radius) {
    // S_StartSound( origin, ENTITYNUM_NONE, CHAN_AUTO, sfxHandle, volume,
    // radius );
  ...
  inline int RegisterSound(const char *sound) { return S_RegisterSound(sound); }

  // Physics/collision
  inline void Trace(trace_t &tr, vec3_t start, vec3_t min, vec3_t max,
  ...
  inline void G2Trace(trace_t &tr, vec3_t start, vec3_t min, vec3_t max,
                      vec3_t end, int skipEntNum, int flags) {
    TCGTrace *td = (TCGTrace *)cl.mSharedMemory;
  ...
  inline void AddLightToScene(vec3_t org, float radius, float red, float green,
                              float blue) {
  ...
  inline void AddDecalToScene(qhandle_t shader, const vec3_t origin,
                              const vec3_t dir, float orientation, float r,
                              float g, float b, float a, qboolean alphaFade,
                              float radius, qboolean temporary) {
    re->AddDecalToScene(shader, origin, dir, orientation, r, g, b, a, alphaFade,
                        radius, temporary);
  }

  void CameraShake(vec3_t origin, float intensity, int radius, int time);
  qboolean GetOriginAxisFromBolt(CGhoul2Info_v *pGhoul2, int mEntNum,
