...
// Handle Pool
// -----------
// The memory pool class is a simple routine for constant time allocation and
// deallocation from a fixed size pool of objects.  The class uses a simple
// array to hold the actual data, a queue for the free list, and a bit field to
// mark which spots in the array are allocated.
//
// In addition to the standard memory pool features, this Handle Pool provides a
// fast iterator, asserts on attempting to access unused data, and a unique ID
// "handle" for the external system to use.
//
//
//
// NOTES:
...
template <class T> class handle_pool_base : public pool_root<T> {
public:
...
public:
  ...
  // Constructor
  //
  // We need a routine to calculate the MASK used to convert a handle to an
  // index and the number of bits needed to shift by.
  //
  ////////////////////////////////////////////////////////////////////////////////////
  handle_pool_base() {
  ...
  void clear() {
    pool_root<T>::clear();
    // note that we do not refill the handles cause we want old handles to still
    // be stale
    for (int i = 0; i < CAPACITY; i++) {
      IncHandle(i);
    }
  ...
  const TTValue &operator[](int handle) const {
    assert(is_used(
        handle)); // typically this is a stale handle (already been freed)
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  void free(TTValue *me) { free_index(pointer_to_index(me)); }
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  // FInd the handle for a given index, this cannot check for stale handles, so
  // it is ill advised
  ////////////////////////////////////////////////////////////////////////////////////
  int index_to_handle(int index) const {
    assert(index >= 0 && index < CAPACITY &&
           pool_root<T>::is_used_index(
               index)); // disallowing this on stale handles
    return (mHandles[index]);
  }

  ////////////////////////////////////////////////////////////////////////////////////
  // converts a T pointer to a handle, generally not something you need, cannot
...
class handle_pool_vs
    : public handle_pool_base<storage::value_semantics<T, ARG_CAPACITY>> {
public:
...
template <class T, int ARG_CAPACITY>
class handle_pool_os
    : public handle_pool_base<storage::object_semantics<T, ARG_CAPACITY>> {
public:
...
class handle_pool_is
    : public handle_pool_base<
