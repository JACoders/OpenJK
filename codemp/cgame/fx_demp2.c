...
void FX_DEMP2_ProjectileThink(centity_t *cent,
  ...
  trap->FX_PlayEffectID(cgs.effects.demp2ProjectileEffect, cent->lerpOrigin,
                        forward, -1, -1, qfalse);
}
...
void FX_DEMP2_HitWall(vec3_t origin, vec3_t normal) {
  trap->FX_PlayEffectID(cgs.effects.demp2WallImpactEffect, origin, normal, -1,
                        -1, qfalse);
}
...
void FX_DEMP2_HitPlayer(vec3_t origin, vec3_t normal, qboolean humanoid) {
  trap->FX_PlayEffectID(cgs.effects.demp2FleshImpactEffect, origin, normal, -1,
                        -1, qfalse);
}
...
---------------------------
*/
void FX_DEMP2_AltBeam(vec3_t start, vec3_t end,
                      vec3_t normal, // qboolean spark,
  ...
  /*
     ...
     s3));
          ...
          FX_AddBezier( start, targ1,
                                                  ...
                                                  1.0f, 0.0f, 0.0f,
                                                  chaos, chaos, 0.0f,
                                                  1.0f, trap->R_RegisterShader(
     "gfx/misc/electric2" ), FX_ALPHA_LINEAR );
     ...
     "gfx/misc/electric2" ), FX_ALPHA_LINEAR );
          ...
          FX_AddSprite( c1, NULL, NULL, 12.0f + Q_flrand(-1.0f, 1.0f) * 4,
     0.0f, 1.0f, 1.0f, Q_flrand(0.0f, 1.0f) * 360, 0.0f, 1.0f,
                                                  trap->R_RegisterShader(
     "gfx/misc/lightningFlash" )); FX_AddSprite( c1, NULL, NULL, 6.0f +
     Q_flrand(-1.0f, 1.0f) * 2, 0.0f, 1.0f, 1.0f, Q_flrand(0.0f, 1.0f) * 360,
     0.0f, 1.0f, trap->R_RegisterShader( "gfx/misc/lightningFlash" ));

          FX_AddSprite( targ1, NULL, NULL, 4.0f + Q_flrand(-1.0f, 1.0f),
     0.0f, 1.0f, 0.0f, chaos, chaos, Q_flrand(0.0f, 1.0f) * 360, 0.0f, 10,
                                                  trap->R_RegisterShader(
     "gfx/misc/lightningFlash" )); FX_AddSprite( targ1, NULL, NULL, 8.0f +
     Q_flrand(-1.0f, 1.0f) * 2, 0.0f, 1.0f, 0.0f, chaos, chaos,
     Q_flrand(0.0f, 1.0f) * 360, 0.0f, 10, trap->R_RegisterShader(
     ...
     "gfx/misc/electric2" ), FX_ALPHA_LINEAR );
          ...
          FX_AddSprite( targ2, NULL, NULL, 4.0f + Q_flrand(-1.0f, 1.0f),
     0.0f, 1.0f, 0.0f, chaos, chaos, Q_flrand(0.0f, 1.0f) * 360, 0.0f, 10,
                                                  trap->R_RegisterShader(
     "gfx/misc/lightningFlash" )); FX_AddSprite( targ2, NULL, NULL, 8.0f +
     Q_flrand(-1.0f, 1.0f) * 2, 0.0f, 1.0f, 0.0f, chaos, chaos,
     Q_flrand(0.0f, 1.0f) * 360, 0.0f, 10, trap->R_RegisterShader(
     "gfx/misc/lightningFlash" ));
  */
