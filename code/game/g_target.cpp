...
void SP_target_give(gentity_t *ent) { ent->e_UseFunc = useF_Use_Target_Give; }
...
"sounds" va() min max, so if your sound string is borgtalk%d.wav, and you set a
"sounds" value of 4, it will randomly play borgtalk1.wav - borgtalk4.wav when
triggered to use this, you must store the wav name in "soundGroup", NOT "noise"
...
cost. "wait" : Seconds between triggerings, 0 = don't auto trigger "random"
...
void Use_Target_Speaker(gentity_t *ent, gentity_t *other,
  ...
  if (ent->sounds) {
    ent->noise_index =
        G_SoundIndex(va(ent->paintarget, Q_irand(1, ent->sounds)));
  }
...
void target_laser_think(gentity_t *self) {
  ...
  gi.trace(&tr, self->s.origin, NULL, NULL, end, self->s.number,
           CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_CORPSE, (EG2_Collision)0,
           0);
...
/*QUAKED target_relay (.5 .5 .5) (-8 -8 -8) (8 8 8) RED_ONLY BLUE_ONLY RANDOM x
x x x INACTIVE This doesn't perform any actions except fire its targets. The
activator can be forced to be from a certain team. if RANDOM is checked, only
one of the targets will be fired, not all of them

INACTIVE  Can't be used until activated

  "delay" - Will actually fire this many seconds after being used
  "wait" - Cannot be fired again until this many seconds after the last time it
was used
*/
void target_relay_use_go(gentity_t *self) {
  ...
  if (self->spawnflags & 4) {
    ...
    if (ent &&
        (ent->e_UseFunc != useF_NULL)) { // e_UseFunc check can be omitted
...
void target_relay_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  if ((self->spawnflags & 1) &&
      activator->client) { //&& activator->client->ps.persistant[PERS_TEAM] !=
                           //TEAM_RED ) {
    return;
  }
...
void target_kill_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  if (self->spawnflags & 1) { // falling death
...
void SP_target_kill(gentity_t *self) { self->e_UseFunc = useF_target_kill_use; }
...
bounceCount - number of times the counter should reset to it's full count when
it's done
*/

void target_counter_use(gentity_t *self, gentity_t *other,
                        gentity_t *activator) {
  if (self->count == 0) {
    return;
  }
  ...
  // activator->targetname, activator->s.number );
  self->count--;

  if (activator) {
    Quake3Game()->DebugPrint(
        IGameInterface::WL_VERBOSE, "target_counter %s used by %s (%d/%d)\n",
        self->targetname, activator->targetname,
        (self->max_health - self->count), self->max_health);
  }
...
void target_random_use(gentity_t *self, gentity_t *other,
                       gentity_t *activator) {
  int t_count = 0, pick;
  gentity_t *t = NULL;
...
void scriptrunner_run(gentity_t *self) {
  ...
  if (self->behaviorSet[BSET_USE])
  {
          ...
          sprintf((char *) &newname, "%s/%s", Q3_SCRIPT_DIR,
  ...
  */
  ...
  if (self->behaviorSet[BSET_USE]) {
    if (self->spawnflags & 1) {
      ...
      if (self->activator->m_iIcarusID ==
          IIcarusInterface::ICARUS_INVALID) { // Need to be initialized through
                                              // ICARUS
        if (!self->activator->script_targetname ||
            !self->activator->script_targetname[0]) {
          // We don't have a script_targetname, so create a new one
          self->activator->script_targetname =
              va("newICARUSEnt%d", numNewICARUSEnts++);
        }
        ...
        if (Quake3Game()->ValidEntity(self->activator)) {
          ...
          Quake3Game()->DebugPrint(IGameInterface::WL_ERROR,
                                   "target_scriptrunner tried to run on "
                                   "invalid ICARUS activator!\n");
          return;
        }
      }

      Quake3Game()->DebugPrint(
          IGameInterface::WL_VERBOSE,
          "target_scriptrunner running %s on activator %s\n",
          self->behaviorSet[BSET_USE], self->activator->targetname);

      Quake3Game()->RunScript(self->activator, self->behaviorSet[BSET_USE]);
    } else {
      if (self->activator) {
        Quake3Game()->DebugPrint(IGameInterface::WL_VERBOSE,
                                 "target_scriptrunner %s used by %s\n",
...
void target_scriptrunner_use(gentity_t *self, gentity_t *other,
                             gentity_t *activator) {
...
/*QUAKED target_scriptrunner (1 0 0) (-4 -4 -4) (4 4 4) runonactivator x x x x x
x INACTIVE
--- SPAWNFLAGS ---
runonactivator - Will run the script on the entity that used this or tripped the
trigger that used this INACTIVE - start off

----- KEYS ------
Usescript - Script to run when used
count - how many times to run, -1 = infinite.  Default is once
wait - can't be used again in this amount of seconds (Default is 1 second if
it's multiple-use) delay - how long to wait after use to run script
...
void SP_target_scriptrunner(gentity_t *self) {
  ...
  */
  // FIXME: this is a hack... because delay is read in as an int, so I'm
  // bypassing that because it's too late in the project to change it and I want
  // to be able to set less than a second delays no one should be setting a
  // radius on a scriptrunner, if they are this would be bad, take this out for
  // the next project
  self->radius = 0.0f;
...
void target_gravity_change_use(gentity_t *self, gentity_t *other,
                               gentity_t *activator) {
  G_ActivateBehavior(self, BSET_USE);
...
void target_friction_change_use(gentity_t *self, gentity_t *other,
                                gentity_t *activator) {
  G_ActivateBehavior(self, BSET_USE);

  if (self->spawnflags &
      1) { // FIXME - make a global?
...
void set_mission_stats_cvars(void) {
  ...
  gi.cvar_set(
      "ui_stats_enemieskilled",
      va("%d",
         client->sess.missionStats.enemiesKilled)); // pass this on to the menu
  ...
  if (wpn) {
    ...
    cgi_SP_GetStringTextString(va("SP_INGAME_%s", wItem->classname), text,
                               sizeof(text));
  ...
  gi.cvar_set(
      "ui_stats_shots",
      va("%d",
         client->sess.missionStats.shotsFired)); // pass this on to the menu

  gi.cvar_set(
      "ui_stats_hits",
      va("%d", client->sess.missionStats.hits)); // pass this on to the menu
  ...
  const float percent =
      cg_entities[0].gent->client->sess.missionStats.shotsFired
          ? 100.0f *
                ...
                cg_entities[0].gent->client->sess.missionStats.shotsFired
          : 0;
  gi.cvar_set("ui_stats_accuracy",
              va("%.2f%%", percent)); // pass this on to the menu

  gi.cvar_set(
      "ui_stats_thrown",
      va("%d",
         client->sess.missionStats.saberThrownCnt)); // pass this on to the menu

  gi.cvar_set("ui_stats_blocks",
              va("%d", client->sess.missionStats.saberBlocksCnt));
  gi.cvar_set("ui_stats_legattacks",
              va("%d", client->sess.missionStats.legAttacksCnt));
  gi.cvar_set("ui_stats_armattacks",
              va("%d", client->sess.missionStats.armAttacksCnt));
  gi.cvar_set("ui_stats_bodyattacks",
              va("%d", client->sess.missionStats.torsoAttacksCnt));

  gi.cvar_set("ui_stats_absorb",
              va("%d", client->sess.missionStats.forceUsed[FP_ABSORB]));
  gi.cvar_set("ui_stats_heal",
              va("%d", client->sess.missionStats.forceUsed[FP_HEAL]));
  gi.cvar_set("ui_stats_mindtrick",
              va("%d", client->sess.missionStats.forceUsed[FP_TELEPATHY]));
  gi.cvar_set("ui_stats_protect",
              va("%d", client->sess.missionStats.forceUsed[FP_PROTECT]));

  gi.cvar_set("ui_stats_jump",
              va("%d", client->sess.missionStats.forceUsed[FP_LEVITATION]));
  gi.cvar_set("ui_stats_pull",
              va("%d", client->sess.missionStats.forceUsed[FP_PULL]));
  gi.cvar_set("ui_stats_push",
              va("%d", client->sess.missionStats.forceUsed[FP_PUSH]));
  gi.cvar_set("ui_stats_sense",
              va("%d", client->sess.missionStats.forceUsed[FP_SEE]));
  gi.cvar_set("ui_stats_speed",
              va("%d", client->sess.missionStats.forceUsed[FP_SPEED]));
  gi.cvar_set("ui_stats_defense",
              va("%d", client->sess.missionStats.forceUsed[FP_SABER_DEFENSE]));
  gi.cvar_set("ui_stats_offense",
              va("%d", client->sess.missionStats.forceUsed[FP_SABER_OFFENSE]));
  gi.cvar_set("ui_stats_throw",
              va("%d", client->sess.missionStats.forceUsed[FP_SABERTHROW]));

  gi.cvar_set("ui_stats_drain",
              va("%d", client->sess.missionStats.forceUsed[FP_DRAIN]));
  gi.cvar_set("ui_stats_grip",
              va("%d", client->sess.missionStats.forceUsed[FP_GRIP]));
  gi.cvar_set("ui_stats_lightning",
              va("%d", client->sess.missionStats.forceUsed[FP_LIGHTNING]));
  gi.cvar_set("ui_stats_rage",
              va("%d", client->sess.missionStats.forceUsed[FP_RAGE]));
}

#include "../cgame/cg_media.h" //access to cgs
extern void G_ChangeMap(const char *mapname, const char *spawntarget,
                        qboolean hub); // g_utils
void target_level_change_use(gentity_t *self, gentity_t *other,
                             gentity_t *activator) {
  G_ActivateBehavior(self, BSET_USE);
  ...
  if (self->message && !Q_stricmp("disconnect", self->message)) {
    ...
    G_ChangeMap(self->message, self->target,
                (qboolean)((self->spawnflags & 1) != 0));
  }
  if (self->count >= 0) {
    gi.cvar_set("tier_storyinfo", va("%i", self->count));
    if (level.mapname[0] == 't' && level.mapname[2] == '_' &&
        (level.mapname[1] == '1' || level.mapname[1] == '2' ||
         level.mapname[1] == '3')) {
      char s[2048];
      gi.Cvar_VariableStringBuffer("tiers_complete", s,
                                   sizeof(s)); // get the current list
      if (*s) {
        gi.cvar_set(
            "tiers_complete",
            va("%s %s", s,
               level.mapname)); // strcat this level into the existing list
      } else {
        gi.cvar_set("tiers_complete",
                    level.mapname); // set this level into the list
      }
    }
    if (self->noise_index) {
      cgi_S_StopSounds();
      cgi_S_StartSound(NULL, 0, CHAN_VOICE,
                       cgs.sound_precache[self->noise_index]);
    }
  }
...
HUB - Will save the current map's status and load the next map with any saved
status it may have NO_STORYSOUND - will not play storyinfo wav file, even if you
'++' or set tier_storyinfo
...
void SP_target_level_change(gentity_t *self) {
  ...
  if (G_SpawnString("tier_storyinfo", "", &s)) {
    if (*s == '+') {
      self->noise_index =
          G_SoundIndex(va("sound/chars/tiervictory/%s.mp3", level.mapname));
      self->count = gi.Cvar_VariableIntegerValue("tier_storyinfo") + 1;
      G_SoundIndex(
          va("sound/chars/storyinfo/%d.mp3", self->count)); // cache for menu
    } else {
      self->count = atoi(s);
      if (!(self->spawnflags & 2)) {
        self->noise_index =
            G_SoundIndex(va("sound/chars/storyinfo/%d.mp3", self->count));
      }
    }

    if (G_SpawnString("storyhead", "", &s)) {  //[luke, kyle, or prot]
      gi.cvar_set("storyhead", s);             // pass this on to the menu
    } else {                                   // show head based on mapname
...
Copies any parms set on this ent to the entity that  fired the
trigger/button/whatever that uses this parm1 parm2 parm3 parm4 parm5 parm6 parm7
parm8
parm9
parm10
parm11
...
void target_change_parm_use(gentity_t *self, gentity_t *other,
                            gentity_t *activator) {
  if (!activator || !self) {
    return;
  }
...
void target_play_music_use(gentity_t *self, gentity_t *other,
                           gentity_t *activator) {
  G_ActivateBehavior(self, BSET_USE);
...
looping portion will start and loop indefinetly.  If no introfile is entered,
only the loopfile will play.
*/
void SP_target_play_music(gentity_t *self) {
  char *s;
  G_SetOrigin(self, self->s.origin);
  if (!G_SpawnString("music", "", &s)) {
    G_Error("target_play_music without a music key at %s",
            vtos(self->s.origin));
  }
...
void target_autosave_use(gentity_t *self, gentity_t *other,
                         gentity_t *activator) {
  G_ActivateBehavior(self, BSET_USE);
  // gi.SendServerCommand( NULL, "cp @SP_INGAME_CHECKPOINT" );
  CG_CenterPrint("@SP_INGAME_CHECKPOINT",
                 SCREEN_HEIGHT * 0.25); // jump the network
...
void target_secret_use(gentity_t *self, gentity_t *other,
                       gentity_t *activator) {
  // we'll assume that the activator is the player
  gclient_t *const client = &level.clients[0];
  ...
  if (client->sess.missionStats.secretsFound >
