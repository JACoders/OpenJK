...
// Graph
// -----
// A Graph object is one of the most generic data structures.  Theoretically, a
// graph could be used to make a tree, list, or any other standard structure.  A
// graph has two data types: NODE and EDGE, and maintains connection information
// how edges can link two nodes.
//
// One of the most intuitive uses of a graph class is a navigation bouy system.
// In such a use, the nodes would probably be vector based objects with
// positional information and the edges might contain portal information (door
// is open / closed / locked).
//
// Another example might be a web page, or decision tree, or any other problem
// space which requires object connection data.
//
//
//
//
// Implimentation
// --------------
// This template allocates a pool for NODES, a pool for EDGES, and a
// grid2_vs<int> to serve as an Adjacency Matrix (called Links).  The Adj.
// Matrix stores indicies to EDGE objects in the EDGE pool.
//
//
//
//
...
// -----------------------------------------
// It's fairly common to have a graph with no connection information other than
// the existance of the link.  For this case, you should be able to create a
// graph with a MAXEDGES of 1.  You will want to call the version of
// connect_node() which does not take an edge object, and uses 1 as the "index"
// in the Adj. Matrix.
//
//
//
...
//
// First, create a (graph_vs::search) object with the start and end points that
// you want to search for.  Then, call either bfs(), dfs(), or astar().  When
// you get the object back, it will have a vector of all the nodes that were
// visited and methods for iterating over that vector to get the path.
//
//	for (TestSearch.path_begin(); !TestSearch.path_end();
//TestSearch.path_inc())
//	{
...
// The Graph Class
////////////////////////////////////////////////////////////////////////////////////////
template <class TNODE, int MAXNODES, class TEDGE, int MAXEDGES,
          int MAXNODENEIGHBORS>
class graph_vs : public ratl::ratl_base {
public:
  ////////////////////////////////////////////////////////////////////////////////////
  // The Graph User Class
  //
  // When executing a search (in particular an A* search), you may want to
  // derive your own user class so that you can provide specific functionality
  // to the search.  For example, you might want characters on one team to bias
  // the cost of going to nodes which are occupied by the other team.  Or you
  // might want to allow specific characters to access some edges where others
  // cannot.  Perhaps one can fly or has a special key to a door...
  //
  ////////////////////////////////////////////////////////////////////////////////////
  class user {
  public:
    ...
    ////////////////////////////////////////////////////////////////////////////////////
    virtual void setup_edge(TEDGE &Edge, int A, int B, bool OnHull,
                            const TNODE &NodeA, const TNODE &NodeB,
                            bool CanBeInvalid = false) = 0;
  };
  ...
  typedef typename ratl::vector_vs<SNodeNeighbor, MAXNODENEIGHBORS>
      TNodeNeighbors;
  typedef typename ratl::array_vs<TNodeNeighbors, MAXNODES> TLinks;

  typedef typename ragl::graph_vs<TNODE, MAXNODES, TEDGE, MAXEDGES,
                                  MAXNODENEIGHBORS>
      TGraph;
  ...
  public:
    enum { SIZEX = CELLSX, SIZEY = CELLSY, SIZENODES = NODESPERCELL };

    struct SSortNode {
      float mCost;
      short mHandle;
      bool operator<(const SSortNode &t) const { return (mCost < t.mCost); }
    };
    ...
    //
    ////////////////////////////////////////////////////////////////////////////////
    SCell &get_cell(float x, float y) { return mCells.get(x, y); }
    SCell &get_cell(int x, int y) { return mCells.get(x, y); }
    ...
    void fill_cells_nodes(float range) {
      ...
      //----------------------------------------------------------------
      bool full = false;
      for (typename TNodes::iterator it = mGraph.nodes_begin();
           it != mGraph.nodes_end() && !full; it++) {
        TNODE &node = (*it);
      ...
      for (int x = 0; x < CELLSX; x++) {
        for (int y = 0; y < CELLSY; y++) {
          ...
          for (rcell = mCells.rangeBegin(iRange, x, y); !rcell.at_end();
               rcell++) {
            SCell &cell = (*rcell);
          ...
          // Copy The Sorted Nodes Vector Into The Sorted Cell (Of The Sorted
          // Cell Grid)
          //----------------------------------------------------------------------------
    ...
    void fill_cells_edges(float range) {
      ...
      //----------------------------------------------------------------
      bool full = false;
      for (typename TEdges::iterator eit = mGraph.edges_begin();
           eit != mGraph.edges_end() && !full; eit++) {
        TEDGE &edge = (*eit);
  ...
  // Number Of Nodes
  ////////////////////////////////////////////////////////////////////////////////////
  int size_nodes() { return (mNodes.size() - 1); }

  ////////////////////////////////////////////////////////////////////////////////////
  // Access A Node
  ////////////////////////////////////////////////////////////////////////////////////
  TNODE &get_node(int i) { return mNodes[i]; }
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  typename TNodes::iterator nodes_end() { return mNodes.end(); }

  ////////////////////////////////////////////////////////////////////////////////////
  // Number Of Edges
  ////////////////////////////////////////////////////////////////////////////////////
  int size_edges() { return (mEdges.size() - 1); }

  ////////////////////////////////////////////////////////////////////////////////////
  // Access An Edge
  ////////////////////////////////////////////////////////////////////////////////////
  TEDGE &get_edge(int i) { return mEdges[i]; }

  ////////////////////////////////////////////////////////////////////////////////////
  // Get The Edge That Connects nodeA to nodeB
  //
  // NOTE: This is now a linear scan witout the adjacency matrix (worst case 8
  // depth)
  ////////////////////////////////////////////////////////////////////////////////////
  ...
  // Access The End Of The Nodes List
  ////////////////////////////////////////////////////////////////////////////////////
  typename TEdges::iterator edges_end() { return mEdges.end(); }

  int edge_index(const TEDGE &edge) { return mEdges.pointer_to_index(&edge); }

  ////////////////////////////////////////////////////////////////////////////////////
  // Get All The Neighbors Of A Given Node
  ////////////////////////////////////////////////////////////////////////////////////
  TNodeNeighbors &get_node_neighbors(const int nodeA) { return mLinks[nodeA]; }
  bool node_has_neighbors(const int nodeA) { return !(mLinks[nodeA].empty()); }
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  int connect_node(const TEDGE &t, int nodeA, int nodeB,
                   bool reflexive = true) {
    if (nodeA == nodeB || !nodeA || !nodeB || !mNodes.is_used(nodeA) ||
        !mNodes.is_used(nodeB)) {
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  // Connect Node Without Allocating An Edge Object  (A->B)  if reflexive, also
  // (B->A)
  ////////////////////////////////////////////////////////////////////////////////////
  ...
  void remove_edge(int nodeA, int nodeB, bool reflexive = true) {
    ...
    for (linkNum = 0; linkNum < mLinks[nodeA].size(); linkNum++) {
      if (mLinks[nodeA][linkNum].mNode == nodeB) {
        if (mLinks[nodeA][linkNum].mEdge &&
            mEdges.is_used(mLinks[nodeA][linkNum].mEdge)) {
    ...
    for (linkNum = 0; linkNum < mLinks[nodeB].size(); linkNum++) {
      if (mLinks[nodeB][linkNum].mNode == nodeA) {
        if (mLinks[nodeB][linkNum].mEdge &&
            mEdges.is_used(mLinks[nodeB][linkNum].mEdge)) {
...
private:
  ...
  //
  // A handle heap is a specially designed heap class which remembers the handle
  // of any value that is inserted.  Should the value of an already inserted
  // object change, it is possible to reheapify around that value, no matter
  // where the value lies inside the heap because of the additional memory to
  // convert handle->index.
  //
  // The handle heap is used by A* to sort the open list by cost.
  //
  ////////////////////////////////////////////////////////////////////////////////////////
  template <class T> class handle_heap {
  public:
    ////////////////////////////////////////////////////////////////////////////////////
    // Constructor
    ////////////////////////////////////////////////////////////////////////////////////
    handle_heap(TNodes &Nodes) : mNodes(Nodes) { clear(); }

    ////////////////////////////////////////////////////////////////////////////////////
    // Get The Size (The Difference Between The Push And Pop "Pointers")
    ////////////////////////////////////////////////////////////////////////////////////
    int size() const { return (mPush); }
    ...
    ////////////////////////////////////////////////////////////////////////////////////
    bool full() const { return (size() == MAXNODES); }
    ...
    bool used(int handle) {
      return (mHandleToPos[handle] != -1 &&
              mData[mHandleToPos[handle]].handle() == handle);
    }
    ...
    T &top() {
      assert(mPush >
             0); // Don't Try To Look At This If There Is Nothing In Here
      return (mData[0]);
    }
    ...
    void push(const T &nValue) {
      ...
      // Get The Handle From The Value And Make Sure We Don't Already Have One
      // Stored There
      //------------------------------------------------------------------------------------
    ...
    ////////////////////////////////////////////////////////////////////////////////////
    // Call This Func If The Value At The Given Handle Has Changed & Needs
    // Adjustment
    ////////////////////////////////////////////////////////////////////////////////////
  ...
  private:
    ////////////////////////////////////////////////////////////////////////////////////
    // Returns The Location Of Node (i)'s Parent Node (The Parent Node Of Zero
    // Is Zero)
    ////////////////////////////////////////////////////////////////////////////////////
    int parent(int i) { return ((i - 1) / 2); }

    ////////////////////////////////////////////////////////////////////////////////////
    // Returns The Location Of Node (i)'s Left Child (The Child Of A Leaf Is The
    // Leaf)
    ////////////////////////////////////////////////////////////////////////////////////
    int left(int i) { return (2 * i) + 1; }

    ////////////////////////////////////////////////////////////////////////////////////
    // Returns The Location Of Node (i)'s Right Child (The Child Of A Leaf Is
    // The Leaf)
    ////////////////////////////////////////////////////////////////////////////////////
    ...
    int largest_child(int i) {
      if (left(i) < mPush) {
        ...
        return left(
            i); // Node i only has a left child, so by default it is the biggest
      }
    ...
    ////////////////////////////////////////////////////////////////////////////////////
    // Validate Will Run Through The Heap And Make Sure The Top Element Is
    // Smallest
    ////////////////////////////////////////////////////////////////////////////////////
#ifdef _DEBUG
    void validate() {
    ...
    ////////////////////////////////////////////////////////////////////////////////////
  private:
    ratl::array_vs<T, MAXNODES + 1>
        mData; // The Memory (Plus One For Temp Sort)
  ...
  public:
    ...
    search_node(int Node = -1, int Parent = -1)
        : mNode(Node), mParentVisit(Parent), mCostToGoal(-1),
          mCostFromStart(0) {}
    search_node(const search_node &t)
        : mNode(t.mNode), mParentVisit(t.mParentVisit),
          mCostToGoal(t.mCostToGoal), mCostFromStart(t.mCostFromStart) {}
    ...
    //
    ////////////////////////////////////////////////////////////////////////////////
    int handle() const { return mNode; }

    ////////////////////////////////////////////////////////////////////////////////
    //
    ////////////////////////////////////////////////////////////////////////////////
    float cost_estimate() const { return mCostFromStart + mCostToGoal; }
...
public:
  ...
  public:
    ...
    ////////////////////////////////////////////////////////////////////////////////
    void reset() { clear(false, false); }

    ////////////////////////////////////////////////////////////////////////////////
    // Mark This Node As Closed
    ////////////////////////////////////////////////////////////////////////////////
    void close(int node) { mClosed.set_bit(node); }
    ////////////////////////////////////////////////////////////////////////////////
    // Mark This Node As Closed
    ////////////////////////////////////////////////////////////////////////////////
    ...
    ////////////////////////////////////////////////////////////////////////////////
    bool path_end() { return (mPathVisit == NULL_VISIT_INDEX); }
    ...
    ////////////////////////////////////////////////////////////////////////////////
    int num_visited() { return mVisited.size(); }

    ////////////////////////////////////////////////////////////////////////////////
    // Get An Iterator To The Beginning Of The Visited Pool
    ////////////////////////////////////////////////////////////////////////////////
    typename TVisited::iterator visited_begin() { return mVisited.begin(); }

    ////////////////////////////////////////////////////////////////////////////////
    // Get An Iterator To The End Of The Visited Pool
    ////////////////////////////////////////////////////////////////////////////////
    typename TVisited::iterator visited_end() { return mVisited.end(); }
  ...
  private:
    ...
    void visit(search_node &t) {
      ...
      // Add It To The Visited List, And Mark The Location In The Node Index
      // Array
      //---------------------------------------------------------------------------
      ...
      // Setup Our Next Node To Know It Came From The Last Location In The
      // Visited Vector
      //----------------------------------------------------------------------------------
    ...
    ////////////////////////////////////////////////////////////////////////////////
    // This "Get Next" Function Is For A* and Sets Up THe Costs Of The Search
    // Node
    ////////////////////////////////////////////////////////////////////////////////
    search_node &get_next(const user &suser, TEDGE &edge_parent_to_next) {
      ...
      // NOTE: we do not do a "mClosed.set_bit(mNextIndex);" here because A*
      // only closes nodes that are visited

      mNext.mNode = mNextIndex;
      mNext.mCostToGoal =
          suser.cost((*mNodesPtr)[mNext.mNode], (*mNodesPtr)[mEnd]);
      mNext.mCostFromStart =
          suser.cost(edge_parent_to_next, (*mNodesPtr)[mNext.mNode]);
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  void setup_search(search &sdata) { sdata.setup(&mNodes); }
        ...
        if (curEdge == -1 || suser.is_valid(mEdges[curEdge], sdata.mEnd)) {
          ...
          if (open.used(snode.mNode)) {
            if (curCost < (open[snode.mNode]).cost_estimate()) {
              open[snode.mNode] =
                  snode; // Use This As The Node (With New Parent & Cost)
  ...
  void bfs(search &sdata) {
    ...
    while (open.size() > 0 && !sdata.success()) {
      ...
      // Search Through The Non Closed Nodes Edges
      //-------------------------------------------
      for (sdata.mNextIndex = 0; sdata.mNextIndex < MAXNODES;
           sdata.mNextIndex++) {
  ...
  void ProfileSpew() {
    ...
    ProfilePrint("");
    ProfilePrint(
        "GRAPH SIZE (Bytes): (%d)  (KiloBytes): (%5.3f)  MeggaBytes(%3.3f)",
        (sizeof(*this)), ((float)(sizeof(*this)) / 1024.0f),
        ((float)(sizeof(*this)) / 1048576.0f));
    ProfilePrint("GRAPH COUNT: (%d) Nodes  (%d) Edges", mNodes.size(),
                 mEdges.size());
    if (mNodes.size()) {
      ProfilePrint("GRAPH COUNT: (%f) Edges/Node",
                   (float)mEdges.size() / (float)mNodes.size());
    }
    ProfilePrint("");
    if (mSearchCount) {
      ProfilePrint("SEARCH: (%d) Searches  (%f) AveSize", mSearchCount,
                   ((float)mSearchMemorySize / (float)mSearchCount));
      if (mSearchSuccess) {
      ...
      if (mSearchFail) {
        ProfilePrint("SEARCH: (%d) Failures  (%f) AveVisited", mSearchFail,
                     ((float)mSearchFailVisited / (float)mSearchFail));
      }
    }
    ProfilePrint("");
  };
