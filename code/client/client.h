...
typedef struct {
  ...
  qboolean extrapolatedSnapshot; // set if any cgame frame has been forced to
                                 ...
                                 // looks at it
  qboolean
      newSnapshots; // set on parse, cleared when CL_AdjustTimeDelta looks at it
  ...
  int cinematictime; // cls.realtime for first cinematic frame (FIXME: NO LONGER
                     // USED!, but I wasn't sure if I could remove it because of
                     // struct sizes assumed elsewhere? -Ste)
...
const char *Key_KeynumToString(
    int keynum /*, qboolean bTranslate*/); // note: translate is only called for
                                           // menu display not configs
