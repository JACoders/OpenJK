...
qboolean PInUse(unsigned int entNum) {
  ...
  return (qboolean)((g_entityInUseBits[entNum / 32] &
                     (((unsigned int)1) << (entNum & 0x1f))) != 0);
}
...
/*qboolean PInUse2(gentity_t *ent)
{
        ...
        unsigned int entNum=ent-g_entities;
        return((g_entityInUseBits[entNum/32]&(((unsigned
int)1)<<(entNum&0x1f)))!=0);
}
*/
...
void WriteInUseBits() {
  ...
  saved_game.write_chunk<uint32_t>(INT_ID('I', 'N', 'U', 'S'),
                                   ::g_entityInUseBits);
}
...
void ReadInUseBits() {
  ...
  saved_game.read_chunk<uint32_t>(INT_ID('I', 'N', 'U', 'S'),
                                  ::g_entityInUseBits);
...
static void G_DynamicMusicUpdate(void) {
  ...
  if (!G_PlayerSpawned()) { // player hasn't spawned yet!
    return;
  }

  if (player->health <= 0 && player->max_health > 0) { // defeat music
  ...
  if (level.dmBeatTime > level.time) { // not on a beat
    return;
  }
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (!ent->client || !ent->NPC) {
      if (ent->classname &&
          (!Q_stricmp("PAS", ent->classname) ||
           !Q_stricmp("misc_turret", ent->classname))) { // a turret
    ...
    if (entTeam == player->client->playerTeam) { // ally
      continue;
    }

    if (entTeam == TEAM_NEUTRAL &&
        (!ent->enemy || !ent->enemy->client ||
         ent->enemy->client->playerTeam !=
             player->client
                 ->playerTeam)) { // a droid that is not mad at me or my allies
      continue;
    }

    if (!gi.inPVS(player->currentOrigin,
                  ent->currentOrigin)) { // not potentially visible
      continue;
    }

    if (ent->client &&
        ent->s.weapon == WP_NONE) { // they don't have a weapon... FIXME: only
                                    // do this for droids?
      continue;
    }
    ...
    if ((ent->enemy == player &&
        ...
        (!ent->client && ent->attackDebounceTime > level.time)) { // mad
      if (ent->health > 0) {                                      // alive
        // FIXME: do I really need this check?
        if (ent->s.weapon == WP_SABER && ent->client &&
            !ent->client->ps.SaberActive() &&
            ent->enemy != player) { // a Jedi who has not yet gotten made at me
          continue;
        }
        if (ent->NPC && ent->NPC->behaviorState ==
                            BS_CINEMATIC) { // they're not actually going to do
                                            // anything about being mad at me...
          continue;
        }
        // okay, they're in my PVS, but how close are they?  Are they actively
        // attacking me?
        if (!ent->client && ent->s.weapon == WP_TURRET &&
            ent->fly_sound_debounce_time &&
            ent->fly_sound_debounce_time - level.time <
                10000) { // a turret that shot at me less than ten seconds ago
        } else if (ent->client && ent->client->ps.lastShotTime &&
                   ent->client->ps.lastShotTime - level.time <
                       10000) { // an NPC that shot at me less than ten seconds
                                // ago
        } else { // not actively attacking me lately, see how far away they are
          distSq = DistanceSquared(ent->currentOrigin, player->currentOrigin);
          if (distSq > 4194304 /*2048*2048*/) { //> 2048 away
            continue;
          } else if (distSq > 1048576 /*1024*1024*/) { //> 1024 away
            clearLOS =
                G_ClearLOS(player, player->client->renderInfo.eyePoint, ent);
            LOScalced = qtrue;
            if (clearLOS == qfalse) { // No LOS
              continue;
            }
          }
        }
    ...
    if (level.dmState == DM_EXPLORE) { // only do these visibility checks if
                                       // you're still in exploration mode
      if (!InFront(ent->currentOrigin, player->currentOrigin,
                   player->client->ps.viewangles, 0.0f)) { // not in front
        continue;
      }
      ...
      if (!clearLOS) { // can't see them directly
        continue;
      }
    }
    ...
    if (ent->health <= 0) { // dead
      if (!ent->client ||
          level.time - ent->s.time >
              10000) { // corpse has been dead for more than 10 seconds
  ...
  if (!battle) { // no active enemies, but look for missiles, shot impacts,
                 // etc...
    int alert = G_CheckAlertEvents(player, qtrue, qtrue, 1024, 1024, -1, qfalse,
                                   AEL_SUSPICIOUS);
    if (alert != -1) { // FIXME: maybe tripwires and other FIXED things need
                       // their own sound, some kind of danger/caution theme
      if (G_CheckForDanger(player, alert)) { // found danger near by
        danger++;
        battle = 1;
      } else if (level.alertEvents[alert].owner &&
                 (level.alertEvents[alert].owner == player->enemy ||
                  (level.alertEvents[alert].owner->client &&
                   level.alertEvents[alert].owner->client->playerTeam ==
                       player->client->enemyTeam))) { // NPC on enemy team of
                                                      // player made some noise
  ...
  if (battle) { // battle - this can interrupt level.dmDebounceTime of lower
                // intensity levels
    // play battle
    ...
    if (battle > 5) {
      // level.dmDebounceTime = level.time + 8000;//don't change again for 5
      // seconds
    } else {
      // level.dmDebounceTime = level.time + 3000 + 1000*battle;
    }
  } else {
    if (level.dmDebounceTime > level.time) { // not ready to switch yet
      return;
    } else { // at least 1 second (for beats)
             // level.dmDebounceTime = level.time + 1000;//FIXME: define beat
             // time?
    }
    /*
    ...
    */
    {                                    // still nothing dangerous going on
      if (level.dmState != DM_EXPLORE) { // just went to explore, hold it for a
      ...
      // FIXME: look for interest points and play "mysterious" music instead of
      // exploration?
      // FIXME: suspicious and distraction sounds should play some cue or change
      // music in a subtle way? play exploration
    }
...
void G_InitCvars(void) {
  ...
  g_sex =
      gi.cvar("sex", "f",
              CVAR_USERINFO | CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
  g_spskill =
      gi.cvar("g_spskill", "0", CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
  g_knockback = gi.cvar("g_knockback", "1000", CVAR_CHEAT);
  g_dismemberment = gi.cvar("g_dismemberment", "3",
                            CVAR_ARCHIVE); // 0 = none, 1 = arms and hands, 2 =
                                           // legs, 3 = waist and head
  // for now I'm making default 10 seconds
  g_corpseRemovalTime =
      gi.cvar("g_corpseRemovalTime", "10",
              CVAR_ARCHIVE); // number of seconds bodies stick around for, at
                             // least... 0 = never go away
  ...
  g_saberAutoBlocking =
      gi.cvar("g_saberAutoBlocking", "1",
              CVAR_CHEAT); // must press +block button to do any blocking
  g_saberRealisticCombat =
      gi.cvar("g_saberMoreRealistic", "0",
              CVAR_ARCHIVE); // makes collision more precise, increases damage
  debug_subdivision = gi.cvar("debug_subdivision", "0",
                              CVAR_ARCHIVE); // debug for dismemberment
  g_dismemberProbabilities =
      gi.cvar("g_dismemberProbabilities", "1",
              CVAR_ARCHIVE); // 0 = ignore probabilities, 1 = use probabilities
  g_saberDamageCapping = gi.cvar(
      "g_saberDamageCapping", "1",
      CVAR_CHEAT); // caps damage of sabers vs players and NPC who use sabers
  g_saberMoveSpeed =
      gi.cvar("g_saberMoveSpeed", "1",
              CVAR_CHEAT); // how fast you run while attacking with a saber
  g_saberAnimSpeed = gi.cvar("g_saberAnimSpeed", "1",
                             CVAR_CHEAT); // how fast saber animations run
  g_saberAutoAim = gi.cvar("g_saberAutoAim", "1",
                           CVAR_CHEAT); // auto-aims at enemies when not moving
                                        // or when just running forward
  g_saberNewControlScheme = gi.cvar(
      "g_saberNewControlScheme", "0",
      CVAR_ARCHIVE); // use +forcefocus to pull off all the special moves
  g_debugSaberLock = gi.cvar("g_debugSaberLock", "0",
                             CVAR_CHEAT); // just for debugging/development,
                                          // makes saberlocks happen all the time
  g_saberLockRandomNess =
      gi.cvar("g_saberLockRandomNess", "2",
              CVAR_ARCHIVE); // just for debugging/development, controls
                             // frequency of saberlocks
  g_debugMelee = gi.cvar(
      "g_debugMelee", "0",
      CVAR_CHEAT); // just for debugging/development, test kicks and grabs
  g_saberRestrictForce =
      gi.cvar("g_saberRestrictForce", "0",
              CVAR_ARCHIVE); // restricts certain force powers when using a
                             // 2-handed saber or 2 sabers
  g_saberPickuppableDroppedSabers =
      gi.cvar("g_saberPickuppableDroppedSabers", "0",
              CVAR_CHEAT); // lets you pick up sabers that are dropped
  ...
  gi.cvar_set(
      "newTotalSecrets",
      "0"); // used to carry over the count from SP_target_secret to ClientBegin
  // g_iscensored = gi.cvar( "ui_iscensored", "0",
  // CVAR_ARCHIVE|CVAR_ROM|CVAR_INIT|CVAR_CHEAT|CVAR_NORESTART );

  g_speederControlScheme =
      gi.cvar("g_speederControlScheme", "2",
              CVAR_ARCHIVE); // 2 is default, 1 is alternate

  g_char_model = gi.cvar("g_char_model", "jedi_tf",
                         CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
  g_char_skin_head = gi.cvar("g_char_skin_head", "head_a1",
                             CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
  g_char_skin_torso = gi.cvar("g_char_skin_torso", "torso_a1",
                              CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
  g_char_skin_legs = gi.cvar("g_char_skin_legs", "lower_a1",
                             CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
  g_char_color_red = gi.cvar("g_char_color_red", "255",
                             CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
  g_char_color_green = gi.cvar("g_char_color_green", "255",
                               CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
  g_char_color_blue = gi.cvar("g_char_color_blue", "255",
                              CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
  g_saber = gi.cvar("g_saber", "single_1",
                    CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
  g_saber2 =
      gi.cvar("g_saber2", "", CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
  g_saber_color = gi.cvar("g_saber_color", "yellow",
                          CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
  g_saber2_color = gi.cvar("g_saber2_color", "yellow",
                           CVAR_ARCHIVE | CVAR_SAVEGAME | CVAR_NORESTART);
  g_saberDarkSideSaberColor =
      gi.cvar("g_saberDarkSideSaberColor", "0",
              CVAR_ARCHIVE); // when you turn evil, it turns your saber red!
  ...
  gi.cvar("ui_prisonerobj_currtotal", "0",
          CVAR_ROM | CVAR_SAVEGAME | CVAR_NORESTART);
  gi.cvar("ui_prisonerobj_maxtotal", "0",
          CVAR_ROM | CVAR_SAVEGAME | CVAR_NORESTART);
...
// I'm just declaring a global here which I need to get at in
// NAV_GenerateSquadPaths for deciding if pre-calc'd
//	data is valid, and this saves changing the proto of
//G_SpawnEntitiesFromString() to include a checksum param which 	may get changed
//anyway if a new nav system is ever used. This way saves messing with g_local.h
//each time -slc
int giMapChecksum;
SavedGameJustLoaded_e g_eSavedGameJustLoaded;
qboolean g_qbLoadTransition = qfalse;
void InitGame(const char *mapname, const char *spawntarget, int checkSum,
              const char *entities, int levelTime, int randomSeed,
              int globalTime, SavedGameJustLoaded_e eSavedGameJustLoaded,
              qboolean qbLoadTransition) {
  // rww - default this to 0, we will auto-set it to 1 if we run into a terrain
  // ent
  gi.cvar_set("RMG", "0");
  ...
  level.maxclients = 1;
  level.clients =
      (gclient_t *)G_Alloc(level.maxclients * sizeof(level.clients[0]));
...
static void G_Cvar_Create(const char *var_name, const char *var_value,
                          int flags) {
...
=================
*/
extern int PM_ValidateAnimRange(const int startFrame, const int endFrame,
...
static void G_CheckTasksCompleted(gentity_t *ent) {
  if (Q3_TaskIDPending(ent, TID_CHAN_VOICE)) {
    if (!gi.VoiceVolume[ent->s.number]) { // not playing a voice sound
  ...
  if (Q3_TaskIDPending(ent, TID_LOCATION)) {
    ...
    if (currentLoc && currentLoc[0] &&
        Q_stricmp(ent->message, currentLoc) ==
            0) { // we're in the desired location
...
static void G_CheckSpecialPersistentEvents(
    gentity_t *ent) { // special-case alerts that would be a pain in the ass to
                      // have the ent's think funcs generate
  if (ent == NULL) {
    return;
  }
  if (ent->s.eType == ET_MISSILE && ent->s.weapon == WP_THERMAL &&
      ent->s.pos.trType == TR_STATIONARY) {
    if (eventClearTime ==
        level.time +
            ALERT_CLEAR_TIME) { // events were just cleared out so add me again
      AddSoundEvent(ent->owner, ent->currentOrigin, ent->splashRadius * 2,
                    AEL_DANGER, qfalse, qtrue);
      AddSightEvent(ent->owner, ent->currentOrigin, ent->splashRadius * 2,
                    AEL_DANGER);
    }
  }
  if (ent->forcePushTime >= level.time) { // being pushed
    if (eventClearTime ==
        level.time +
            ALERT_CLEAR_TIME) { // events were just cleared out so add me again
      // NOTE: presumes the player did the pushing, this is not always true, but
      // shouldn't really matter?
      if (ent->item && ent->item->giTag == INV_SECURITY_KEY) {
        AddSightEvent(player, ent->currentOrigin, 128,
                      AEL_DISCOVERED); // security keys are more important
      } else {
        AddSightEvent(
            player, ent->currentOrigin, 128,
            AEL_SUSPICIOUS); // hmm... or should this always be discovered?
      }
    }
  }
  if (ent->contents == CONTENTS_LIGHTSABER &&
      !Q_stricmp("lightsaber", ent->classname)) { // lightsaber
    if (ent->owner && ent->owner->client) {
      if (ent->owner->client->ps.SaberLength() > 0) { // it's on
        // sight event
...
void G_RunThink(gentity_t *ent) {
  ...
  ent->nextthink = 0;
  if (ent->e_ThinkFunc == thinkF_NULL) // actually you don't need this since I
                                       // check for it in the next function -slc
  {
    goto runicarus;
  }
...
runicarus:
  if (ent->inuse) // GEntity_ThinkFunc( ent ) can have freed up this ent if it
                  // was a type flier_child (stasis1 crash)
  {
    if (ent->NPC == NULL) {
      if (ent->m_iIcarusID != IIcarusInterface::ICARUS_INVALID &&
          !stop_icarus) {
...
static void G_Animate(gentity_t *self) {
  ...
  if (self->s.frame == self->endFrame) {
    if (self->svFlags & SVF_ANIMATING) {
      // ghoul2 requires some extra checks to see if the animation is done since
      // it doesn't set the current frame directly
      if (self->ghoul2.size()) {
        float frame, junk2;
        int junk;

        // I guess query ghoul2 to find out what the current frame is and see if
        // we are done.
        gi.G2API_GetBoneAnimIndex(&self->ghoul2[self->playerModel],
                                  self->rootBone,
                                  (cg.time ? cg.time : level.time), &frame,
                                  &junk, &junk, &junk, &junk2, NULL);

        // It NEVER seems to get to what you'd think the last frame would be, so
        // I'm doing this to try and catch when the animation has stopped
  ...
  // With ghoul2, we'll just set the desired start and end frame and let it do
  // it's thing.
  if (self->ghoul2.size()) {
    ...
    gi.G2API_SetBoneAnimIndex(&self->ghoul2[self->playerModel], self->rootBone,
                              ...
                              BONE_ANIM_OVERRIDE_FREEZE, 1.0f, cg.time, -1, -1);
    return;
  }
...
void G_PlayerGuiltDeath(void) {
  if (player && player->client) { // simulate death
    player->client->ps.stats[STAT_HEALTH] = 0;
    // turn off saber
    if (player->client->ps.weapon == WP_SABER &&
        player->client->ps.SaberActive()) {
      G_SoundIndexOnEnt(player, CHAN_WEAPON,
                        player->client->ps.saber[0].soundOff);
      player->client->ps.SaberDeactivate();
    }
    // play the "what have I done?!" anim
    NPC_SetAnim(player, SETANIM_BOTH, BOTH_FORCEHEAL_START,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    player->client->ps.legsAnimTimer = player->client->ps.torsoAnimTimer = -1;
    // look at yourself
    player->client->ps.stats[STAT_DEAD_YAW] =
        player->client->ps.viewangles[YAW] + 180;
  }
}
extern void NPC_SetAnim(gentity_t *ent, int setAnimParts, int anim,
                        int setAnimFlags, int iBlend);
...
class CGameRagDollUpdateParams : public CRagDollUpdateParams {
  void EffectorCollision(const SRagDollEffectorCollision &data) {
    // Com_Printf("Effector Collision at (%f %f
    ...
    if (data.useTracePlane) {
      ...
      if (thisguy && thisguy->client) {
        VectorSubtract(thisguy->client->ps.origin, data.effectorPosition,
                       effectorPosDif);
      } else {
        return;
      }
  ...
  void RagDollBegin() { return; }
  virtual void RagDollSettled() { return; }
  void Collision() // we had a collision, please stop animating and (sometime
                   // soon) call SetRagDoll RP_DEATH_COLLISION
  {
    return;
  }
...
static void G_BodyDragUpdate(gentity_t *ent, gentity_t *dragger) {
  ...
  assert(ent && ent->inuse && ent->client && ent->ghoul2.size() && dragger &&
         dragger->inuse && dragger->client && dragger->ghoul2.size());

  VectorSubtract(dragger->client->renderInfo.handRPoint,
                 ent->client->renderInfo.torsoPoint, handVec);
...
// we want to see which way the pelvis is facing to get a relatively oriented
// base settling frame this is to avoid the arms stretching in opposite
// directions on the body trying to reach the base pose if the pelvis is flipped
// opposite of the base pose or something -rww
static int G_RagAnimForPositioning(gentity_t *ent) {
  ...
  gi.G2API_GetBoltMatrix(ent->ghoul2, ent->playerModel, ent->crotchBolt,
                         &matrix, G2Angles, ent->client->ps.origin,
                         (cg.time ? cg.time : level.time), NULL,
                         ent->s.modelScale);
  gi.G2API_GiveMeVectorFromMatrix(matrix, NEGATIVE_Z, dir);

  if (dir[2] > 0.1f) { // facing up
...
static inline qboolean G_RagWantsHumanoidsOnly(CGhoul2Info *ghlInfo) {
  ...
  if (!Q_stricmp(
          ...
          GLAName)) { // only _humanoid skeleton is expected to have these
    return qtrue;
  }
...
qboolean G_RagDoll(gentity_t *ent, vec3_t forcedAngles) {
  ...
  if (!ent || !ent->inuse || !ent->client || ent->health > 0 ||
      ent->client->noRagTime >= level.time || ent->client->noRagTime == -1 ||
      (ent->s.powerups & (1 << PW_DISRUPTION)) || !ent->e_DieFunc ||
      ent->playerModel < 0 || !ent->ghoul2.size() ||
  ...
  if (ent->client->ps.heldByClient <= ENTITYNUM_WORLD) {
    ...
    if (grabbedBy->inuse && grabbedBy->client && grabbedBy->ghoul2.size()) {
      G_BodyDragUpdate(ent, grabbedBy);
    }
  }

  //--FIXME: do not go into ragdoll if in a spinning death anim or something, it
  //really messes things up rww 12/17/02 - should be ok now actually with my new
  // stuff

  VectorCopy(ent->client->ps.origin, usedOrg);

  if (!ent->client->isRagging) { // If we're not in a ragdoll state, perform the
                                 // checks.
    if (ent->client->ps.heldByClient <=
        ENTITYNUM_WORLD) { // want to rag no matter what then
      inSomething = qtrue;
      ...
      if (VectorNormalize(cVel) >
          400) { // if he's flying through the air at a good enough speed,
                 // switch into ragdoll
        inSomething = qtrue;
      }
    }

    if (ragVar > 1) { // go into rag instantly upon death
      inSomething = qtrue;
    ...
    if (!inSomething) {
      ...
      if (ent->client->ps.legsAnimTimer <=
          0) { // Looks like the death anim is done playing
               // deathDone = qtrue;
      }

      // if (deathDone)
      if (1) { // only trace from the hands if the death anim is already done.
        boltChecks[0] =
            gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "rhand");
        boltChecks[1] =
            gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "lhand");
      } else { // otherwise start the trace loop at the cranium.
        i = 2;
      }
      boltChecks[2] =
          gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "cranium");
      boltChecks[3] =
          gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "rtalus");
      boltChecks[4] =
          gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "ltalus");

      // Do the head first, because the hands reference it anyway.
      gi.G2API_GetBoltMatrix(ent->ghoul2, ent->playerModel, boltChecks[2],
                             &boltMatrix, tAng, ent->client->ps.origin,
                             (cg.time ? cg.time : level.time), NULL,
                             ent->s.modelScale);
      gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN, boltPoints[2]);

      while (i < 5) {
        if (i < 2) { // when doing hands, trace to the head instead of origin
          ...
          if (i > 2) { // 2 is the head, which already has the bolt point.
            gi.G2API_GetBoltMatrix(ent->ghoul2, ent->playerModel, boltChecks[i],
                                   &boltMatrix, tAng, ent->client->ps.origin,
                                   (cg.time ? cg.time : level.time), NULL,
                                   ent->s.modelScale);
        ...
        // Now that we have all that sorted out, trace between the two points we
        // desire.
        gi.trace(&tr, trStart, NULL, NULL, trEnd, ent->s.number, MASK_SOLID,
                 (EG2_Collision)0, 0);

        if (tr.fraction != 1.0 || tr.startsolid ||
            tr.allsolid) { // Hit something or start in solid, so flag it and
                           // break. This is a slight hack, but if we aren't done
                           // with the death anim, we don't really want to go
                           // into ragdoll unless our body has a relatively
                           // "flat" pitch.
#if 0
					vec3_t vSub;
  ...
  if (ent->client->isRagging) { // We're in a ragdoll state, so make the call to
                                // keep our positions updated and whatnot.
    ...
    /*
    if (ent->ikStatus)
    { //ik must be reset before ragdoll is started, or you'll get some
    interesting results. trap_G2API_SetBoneIKState(cent->ghoul2, cg.time, NULL,
    IKS_NONE, NULL); cent->ikStatus = qfalse;
    }
    */
...
#if 1
    {
      ...
      if (gi.G2API_GetBoneAnim(&ent->ghoul2[0], "model_root",
                               ...
                               NULL)) { // lock the anim on the current frame.
        int blendTime = 500;

        gi.G2API_SetBoneAnim(&ent->ghoul2[0], "lower_lumbar", currentFrame,
                             currentFrame + 1, flags, animSpeed,
                             (cg.time ? cg.time : level.time), currentFrame,
                             blendTime);
        gi.G2API_SetBoneAnim(&ent->ghoul2[0], "model_root", currentFrame,
                             currentFrame + 1, flags, animSpeed,
                             (cg.time ? cg.time : level.time), currentFrame,
                             blendTime);
        gi.G2API_SetBoneAnim(&ent->ghoul2[0], "Motion", currentFrame,
                             currentFrame + 1, flags, animSpeed,
                             (cg.time ? cg.time : level.time), currentFrame,
                             blendTime);
      }
    ...
    gi.G2API_SetBoneAngles(&ent->ghoul2[ent->playerModel], "upper_lumbar",
                           vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X,
                           NEGATIVE_Y, NEGATIVE_Z, NULL, 100,
    ...
    gi.G2API_AnimateG2Models(ent->ghoul2, (cg.time ? cg.time : level.time),
                             &tuParms);
    ...
    if (ent->client->ps.heldByClient <= ENTITYNUM_WORLD) {
      ...
      if (grabEnt->client && grabEnt->ghoul2.size()) {
        ...
        // gi.G2API_GetBoltMatrix(grabEnt->ghoul2, 0,
        // grabEnt->gent->client->renderInfo.handRPoint, &matrix,
        // grabEnt->turAngles, grabEnt->lerpOrigin, 	cg.time, cgs.gameModels,
        //grabEnt->modelScale); BG_GiveMeVectorFromMatrix(&matrix, ORIGIN,
        // bOrg);
        VectorCopy(grabEnt->client->renderInfo.handRPoint, bOrg);

        // Get our hand's location
        // trap_G2API_GetBoltMatrix(cent->ghoul2, 0, 0, &matrix,
        // cent->turAngles, cent->lerpOrigin, 	cg.time, cgs.gameModels,
        //cent->modelScale); BG_GiveMeVectorFromMatrix(&matrix, ORIGIN,
        // thisHand);
        VectorCopy(ent->client->renderInfo.handRPoint, thisHand);

        // Get the position of the thoracic bone for hinting its velocity later
        // on thorBolt = trap_G2API_AddBolt(cent->ghoul2, 0, "thoracic");
        // trap_G2API_GetBoltMatrix(cent->ghoul2, 0, thorBolt, &matrix,
        // cent->turAngles, cent->lerpOrigin, 	cg.time, cgs.gameModels,
        //cent->modelScale); BG_GiveMeVectorFromMatrix(&matrix, ORIGIN,
        // thorPoint);
        ...
        VectorSubtract(ent->client->ragLastOrigin, ent->client->ps.origin,
                       pDif);
        ...
        if (ent->client->ragLastOriginTime >= level.time &&
            ent->client->ps.groundEntityNum !=
                ENTITYNUM_NONE) { // make sure it's reasonably updated
          difLen = VectorLength(pDif);
          if (difLen > 0.0f) { // if we're being dragged, then kick all the
                               // bones around a bit
            vec3_t dVel;
            vec3_t rVel;
            int i = 0;
            ...
            while (g_effectorStringTable[i]) {
              ...
              VectorSet(rVel, Q_flrand(-0.1f, 0.1f), Q_flrand(-0.1f, 0.1f),
                        Q_flrand(0.1f, 0.5));
              ...
              gi.G2API_RagEffectorKick(ent->ghoul2, g_effectorStringTable[i],
                                       dVel);
    ...
    } else if (ent->client->overridingBones) { // reset things to their normal
                                               // rag state
      vec3_t pcjMin, pcjMax;
      vec3_t dVel;
    ...
    if (tuParms.hasEffectorData) {
      ...
      */
      ...
      VectorAdd(ent->client->ps.velocity, tuParms.effectorTotal,
...
void G_RunFrame(int levelTime) {
  ...
  //	for ( i = 0, ent = &g_entities[0]; i < globals.num_entities ; i++,
  //ent++)
  for (i = 0; i < globals.num_entities; i++) {
    ...
    if (level.time - ent->eventTime > EVENT_VALID_MSEC) {
      ...
      /*	// This is never set to true anywhere. Killing the field (BTO -
      VV) else if ( ent->unlinkAfterEvent ) {
              // items that will respawn will hide themselves after their pickup
      event ent->unlinkAfterEvent = qfalse; gi.unlinkentity( ent );
      }
      */
    }
    ...
    if (!ent->client) {
      if (!(ent->svFlags &
            SVF_SELF_ANIMATING)) { // FIXME: make sure this is done only for
                                   // models with frames?
    ...
    if (ent->s.eType == ET_MOVER) {
      ...
      if (ent->model &&
          Q_stricmp("models/test/mikeg/tie_fighter.md3", ent->model) == 0) {
        TieFighterThink(ent);
      }
      G_RunMover(ent);
      continue;
    ...
    if (i == 0) {
      ...
      // dead
      if (ent->health <= 0) {
        if (ent->client->ps.groundEntityNum != ENTITYNUM_NONE) { // on the
                                                                 // ground
    ...
    G_RunThink(ent); // be aware that ent may be free after returning from here,
                     // at least one func frees them
    ClearNPCGlobals(); //	but these 2 funcs are ok
    // UpdateTeamCounters( ent );	//	   to call anyway on a freed
    // ent.
  }
  ...
  if (g_delayedShutdown->integer && delayedShutDown != 0 &&
...
void G_LoadSave_WriteMiscData() {
  ...
  saved_game.write_chunk<int32_t>(INT_ID('L', 'C', 'K', 'D'), ::player_locked);
}

void G_LoadSave_ReadMiscData() {
  ojk::SavedGameHelper saved_game(::gi.saved_game);

  saved_game.read_chunk<int32_t>(INT_ID('L', 'C', 'K', 'D'), ::player_locked);
}
