...
int SV_OrgVisibleBox(vec3_t org1, vec3_t mins, vec3_t maxs, vec3_t org2,
                     int ignore) {
  trace_t tr;
        ...
        if (gWPArray[c] && gWPArray[c]->inuse && i != c &&
          ...
          if ((nLDist < maxNeighborDist || forceJumpable) &&
              ((int)gWPArray[i]->origin[2] == (int)gWPArray[c]->origin[2] ||
               forceJumpable) &&
              (SV_OrgVisibleBox(gWPArray[i]->origin, mins, maxs,
...
==================
*/
void Bot_FreeMemoryGame(void *ptr) { Z_Free(ptr); }
...
// there's no such thing as this now, since the zone is unlimited, but I have to
// provide something
//	so it doesn't run out of control alloc-wise (since the bot code calls
//this in a while() loop to free 	up bot mem until zone has > 1MB available
//again. So, simulate a reasonable limit...
//
...
void SV_BotInitBotLib(void) {
  ...
  debugpolygons = (struct bot_debugpoly_s *)Z_Malloc(
      sizeof(bot_debugpoly_t) * bot_maxdebugpolys, TAG_BOTLIB, qtrue);
