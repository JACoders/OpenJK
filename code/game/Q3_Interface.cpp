...
stringID_table_t INVTable[] = {ENUM2STRING(INV_ELECTROBINOCULARS), ENUM2STRING(INV_BACTA_CANISTER), ENUM2STRING(INV_SEEKER),
							   ENUM2STRING(INV_LIGHTAMP_GOGGLES),  ENUM2STRING(INV_SENTRY),			{"", 0}};
...
stringID_table_t DMSTable[] = {{"NULL", -1},
...
void Q3_TaskIDComplete(gentity_t *ent, taskID_t taskType) {
	...
	if (ent->m_iIcarusID != IIcarusInterface::ICARUS_INVALID /*ent->taskManager*/ && Q3_TaskIDPending(ent, taskType)) { // Complete it
...
static void Q3_SetObjective(const char *ObjEnum, int status) {
	...
	if (objectiveID == LIGHTSIDE_OBJ &&
		status == SET_OBJ_FAILED) { // don't do this unless we just set that specific objective (just turned to the dark side) with this SET_OBJECTIVE command
...
=============
*/
static void Q3_SetCaptionTextColor(const char *color) { SetTextColor(textcolor_caption, color); }
...
=============
*/
static void Q3_SetCenterTextColor(const char *color) { SetTextColor(textcolor_center, color); }
...
=============
*/
static void Q3_SetScrollTextColor(const char *color) { SetTextColor(textcolor_scroll, color); }
...
void moverCallback(gentity_t *ent) {
	...
	if (!Q_stricmp("misc_model_breakable", ent->classname) && ent->physicsBounce) { // a gravity-affected model
...
stringID_table_t teamTable[] = {
	ENUM2STRING(TEAM_FREE), ENUM2STRING(TEAM_PLAYER), ENUM2STRING(TEAM_ENEMY), ENUM2STRING(TEAM_NEUTRAL), {"", TEAM_FREE},
};
...
static void Q3_SetHealth(int entID, int data) {
	...
	if (ent->s.number == 0) { // clamp health to max
		...
		if (data == 0) { // artificially "killing" the player", don't let him respawn right away
...
void G_SetWeapon(gentity_t *self, int wp) {
	...
	if (self->NPC) { // since a script sets a weapon, we presume we don't want to auto-match the player's weapon anymore
...
static void Q3_GiveSecurityKey(int entID, char *keyname) {
	...
	if (!keyname || !keyname[0] || !Q_stricmp("none", keyname) || !Q_stricmp("null", keyname)) { // remove the key
...
============
*/
static void Q3_CameraGroupZOfs(float camGZOfs) { client_camera.cameraGroupZOfs = camGZOfs; }
/*
============
Q3_CameraGroup
...
============
*/
static void Q3_CameraGroupTag(char *camGTag) { Q_strncpyz(client_camera.cameraGroupTag, camGTag, sizeof(client_camera.cameraGroupTag)); }
...
static void Q3_AddRHandModel(int entID, char *addModel) {
	...
	if (ent->cinematicModel != -1) {
		...
		gi.G2API_AttachG2Model(&ent->ghoul2[ent->cinematicModel], &ent->ghoul2[ent->playerModel], ent->handRBolt, ent->playerModel);
	}
}
...
static void Q3_AddLHandModel(int entID, char *addModel) {
	...
	if (ent->cinematicModel != -1) {
		// attach it to the hand
		gi.G2API_AttachG2Model(&ent->ghoul2[ent->cinematicModel], &ent->ghoul2[ent->playerModel], ent->handLBolt, ent->playerModel);
	}
}
...
void InflateOwner(gentity_t *self) {
	...
	gi.trace(&trace, self->currentOrigin, self->mins, self->maxs, self->currentOrigin, self->owner->s.number,
			 self->owner->clipmask & ~(CONTENTS_SOLID | CONTENTS_MONSTERCLIP), (EG2_Collision)0, 0);
...
static void Q3_SetSaberActive(int entID, qboolean active) {
	...
	if (ent->client->ps.weapon != WP_SABER) {
		if ((ent->client->ps.stats[STAT_WEAPONS] & (1 << WP_SABER))) { // change to it right now
			if (ent->NPC) {
...
============
*/
static void Q3_SetInterface(int entID, const char *data) { gi.cvar_set("cg_drawStatus", data); }
...
void RemoveOwner(gentity_t *self) {
	if (self->owner && self->owner->inuse) { // I have an owner and they heavn't been freed yet
...
void CQuake3GameInterface::VariableSaveFloats(varFloat_m &fmap) {
	...
	STL_ITERATE(vfi, fmap) {
		...
		saved_game.write_chunk(INT_ID('F', 'I', 'D', 'S'), ((*vfi).first).c_str(), idSize);
...
void CQuake3GameInterface::VariableSaveStrings(varString_m &smap) {
	...
	STL_ITERATE(vsi, smap) {
		...
		saved_game.write_chunk(INT_ID('S', 'I', 'D', 'S'), ((*vsi).first).c_str(), idSize);
		...
		saved_game.write_chunk(INT_ID('S', 'V', 'A', 'L'), ((*vsi).second).c_str(), idSize);
	}
}
...
int CQuake3GameInterface::PrecacheEntity(gentity_t *pEntity) {
	...
	for (i = 0; i < NUM_BSETS; i++) {
		...
		if (GetIDForString(BSTable, pEntity->behaviorSet[i]) == -1) { // not a behavior set
			char *pBuf = NULL;
			int iLength = 0;
...
int CQuake3GameInterface::PlayIcarusSound(int taskID, int entID, const char *name, const char *channel) {
	...
	//
	if (!in_camera || (!g_skippingcin || !g_skippingcin->integer)) // paranoia towards project end <g>
	{
		// Text on
		...
		if (g_subtitles->integer == 1 || (ent->NPC && (ent->NPC->scriptFlags & SCF_USE_SUBTITLES))) // Show all text
		{
			...
			} else // if (precacheWav[i].speaker==SP_NONE)	//  lower screen text
			{
				...
				//
				if (bBroadcast ||
					(DistanceSquared(ent->currentOrigin, ent2->currentOrigin) < ((voice_chan == CHAN_VOICE_ATTEN) ? (350 * 350) : (1200 * 1200)))) {
					gi.SendServerCommand(0, "ct \"%s\" %i", finalName, soundHandle);
				}
			}
		}
	...
	case SET_WINTER_GEAR: // Created: 03/26/03 by AReis.
	{
		...
		if (entID == 0 || (ent->NPC_type && Q_stricmp(ent->NPC_type, "player") == 0)) {
			...
			if (Q_stricmp("true", ((char *)data)) == 0) {
				...
				Com_sprintf(strSkin, sizeof(strSkin), "models/players/%s/|%s|%s|%s", g_char_model->string, g_char_skin_head->string, g_char_skin_torso->string,
...
void CQuake3GameInterface::Activate(int entID, const char *name) { Q3_SetInactive(entID, qtrue); }

void CQuake3GameInterface::Deactivate(int entID, const char *name) { Q3_SetInactive(entID, qfalse); }
...
void CQuake3GameInterface::Kill(int entID, const char *name) {
	...
	*/
	if (victim == ent) { // don't ICARUS_FreeEnt me, I'm in the middle of a script!  (FIXME: shouldn't ICARUS handle this internally?)
...
float CQuake3GameInterface::Random(float min, float max) { return ((rand() * (max - min)) / (float)RAND_MAX) + min; }
...
// Camera functions
void CQuake3GameInterface::CameraPan(vec3_t angles, vec3_t dir, float duration) { CGCam_Pan(angles, dir, duration); }

void CQuake3GameInterface::CameraMove(vec3_t origin, float duration) { CGCam_Move(origin, duration); }

void CQuake3GameInterface::CameraZoom(float fov, float duration) { CGCam_Zoom(fov, duration); }

void CQuake3GameInterface::CameraRoll(float angle, float duration) { CGCam_Roll(angle, duration); }

void CQuake3GameInterface::CameraFollow(const char *name, float speed, float initLerp) { CGCam_Follow(name, speed, initLerp); }

void CQuake3GameInterface::CameraTrack(const char *name, float speed, float initLerp) { CGCam_Track(name, speed, initLerp); }

void CQuake3GameInterface::CameraDistance(float dist, float initLerp) { CGCam_Distance(dist, initLerp); }
...
void CQuake3GameInterface::CameraPath(const char *name) { CGCam_StartRoff(G_NewString(name)); }

void CQuake3GameInterface::CameraEnable(void) { CGCam_Enable(); }

void CQuake3GameInterface::CameraDisable(void) { CGCam_Disable(); }

void CQuake3GameInterface::CameraShake(float intensity, int duration) { CGCam_Shake(intensity, duration); }
...
// Save / Load functions
ojk::ISavedGame *CQuake3GameInterface::get_saved_game_file() { return ::gi.saved_game; }
...
int CQuake3GameInterface::IsFrozen(int entID) { return (g_entities[entID].svFlags & SVF_ICARUS_FREEZE); }

void CQuake3GameInterface::Free(void *data) { gi.Free(data); }

void *CQuake3GameInterface::Malloc(int size) { return gi.Malloc(size, TAG_ICARUS, qtrue); }
...
void CQuake3GameInterface::PrecacheScript(const char *name) {
	...
	case SCRIPT_COULDNOTREGISTER:
		if (!Q_stricmp(newname, "NULL") || !Q_stricmp(newname, "default")) { // these are not real errors, suppress warning
			return;
		}
