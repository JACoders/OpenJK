...
int BotAllocMoveState(void) {
  ...
  for (i = 1; i <= MAX_CLIENTS; i++) {
    if (!botmovestates[i]) {
      botmovestates[i] =
          (struct bot_movestate_s *)GetClearedMemory(sizeof(bot_movestate_t));
      return i;
    } // end if
  }   // end for
  return 0;
...
int BotAvoidSpots(vec3_t origin, aas_reachability_t *reach,
  ...
  for (i = 0; i < numavoidspots; i++) {
    ...
    if (squareddist < squaredradius &&
      ...
      squareddist = DistanceFromLineSquared(avoidspots[i].origin, reach->start,
                                            reach->end);
      // if moving towards the avoid spot
      if (squareddist < squaredradius &&
...
int BotPredictVisiblePosition(vec3_t origin, int areanum, bot_goal_t *goal,
  ...
  for (i = 0; i < 20 && (areanum != goal->areanum); i++) {
    //
    reachnum = BotGetReachabilityToGoal(
        end, areanum, lastgoalareanum, lastareanum, avoidreach, avoidreachtimes,
...
bot_moveresult_t BotTravel_Grapple(bot_movestate_t *ms,
  ...
  if (ms->moveflags & MFL_ACTIVEGRAPPLE) {
    ...
    if (dist < 5 &&
      ...
      VectorAdd(ms->origin, ms->viewoffset, org);
      trace =
          AAS_Trace(org, NULL, NULL, reach->end, ms->entitynum, CONTENTS_SOLID);
    ...
    for (i = numareas - 1; i >= 0; i--) {
      if (AAS_AreaJumpPad(areas[i])) {
        ...
        lastreachnum = BotGetReachabilityToGoal(
            end, areas[i], ms->lastgoalareanum, ms->lastareanum, ms->avoidreach,
            ms->avoidreachtimes, ms->avoidreachtries, goal, TFL_JUMPPAD,
