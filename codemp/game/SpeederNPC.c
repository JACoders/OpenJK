...
static void ProcessMoveCommands(Vehicle_t *pVeh) {
        ...
        (pVeh->m_pPilot->playerState &&
         (pVeh->m_pPilot->playerState->weapon == WP_MELEE ||
          (pVeh->m_pPilot->playerState->weapon == WP_SABER &&
           BG_SabersOff(pVeh->m_pPilot->playerState))))) {
  ...
  if (parentPS->speed || parentPS->groundEntityNum == ENTITYNUM_NONE ||
    ...
    if (!pVeh->m_pVehicleInfo->strafePerc ||
        (0 && pVeh->m_pParentEntity->s.number <
                  MAX_CLIENTS)) { // if in a strafe-capable vehicle, clear
                                  // strafing unless using alternate control
                                  // scheme pVeh->m_ucmd.rightmove = 0;
    }
  }
