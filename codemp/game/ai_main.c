...
char *ctfStateNames[] = {"CTFSTATE_NONE",         "CTFSTATE_ATTACKER",
                         "CTFSTATE_DEFENDER",     "CTFSTATE_RETRIEVAL",
                         "CTFSTATE_GUARDCARRIER", "CTFSTATE_GETFLAGHOME",
                         "CTFSTATE_MAXCTFSTATES"};

char *ctfStateDescriptions[] = {"I'm not occupied",
...
char *siegeStateDescriptions[] = {
    "I'm not occupied", "I'm attempting to complete the current objective",
    "I'm preventing the enemy from completing their objective"};

char *teamplayStateDescriptions[] = {
    "I'm not occupied", "I'm following my squad commander",
...
void BotOrder(gentity_t *ent, int clientnum, int ordernum) {
  ...
  if (level.gametype != GT_CTF && level.gametype != GT_CTY &&
      level.gametype != GT_SIEGE && level.gametype != GT_TEAM) {
    return;
  }
...
void BotInputToUserCommand(bot_input_t *bi, usercmd_t *ucmd,
                           int delta_angles[3], int time, int useTime) {
  ...
  if (useTime < level.time &&
      Q_irand(1, 10) < 5) { // for now just hit use randomly in case there's
                            // something useable around
...
int BotAISetupClient(int client, struct bot_settings_s *settings,
  ...
  if (!botstates[client])
    botstates[client] = (bot_state_t *)B_Alloc(
        sizeof(bot_state_t)); // G_Alloc(sizeof(bot_state_t));
  ...
  bs->client =
      client; // need to know the client number before doing personality stuff
...
int WPOrgVisible(gentity_t *bot, vec3_t org1, vec3_t org2, int ignore) {
  ...
  if (tr.fraction == 1) {
    trap->Trace(&tr, org1, NULL, NULL, org2, ignore, MASK_PLAYERSOLID, qfalse,
                0, 0);

    if (tr.fraction != 1 && tr.entityNum != ENTITYNUM_NONE &&
        g_entities[tr.entityNum].s.eType == ET_SPECIAL) {
      if (g_entities[tr.entityNum].parent &&
          g_entities[tr.entityNum].parent->client) {
...
int GetNearestVisibleWP(vec3_t org, int ignore) {
  ...
  if (RMG.integer) {
    ...
    bestdist = 800; // 99999;
                    // don't trace over 800 units away to avoid GIANT HORRIBLE
                    // SPEED HITS ^_^
  }
  bestindex = -1;
  ...
  while (i < gWPNum) {
    if (gWPArray[i] && gWPArray[i]->inuse) {
      ...
      if (flLen < bestdist &&
          (RMG.integer || BotPVSCheck(org, gWPArray[i]->origin)) &&
...
int PassWayCheck(bot_state_t *bs, int windex) {
  ...
  if (RMG.integer) {
    if ((gWPArray[windex]->flags & WPFLAG_RED_FLAG) ||
        (gWPArray[windex]->flags &
         WPFLAG_BLUE_FLAG)) { // red or blue flag, we'd like to get here
      return 1;
    }
  }
  ...
  if (bs->wpDirection &&
      (gWPArray[windex]->flags &
       WPFLAG_ONEWAY_FWD)) { // we're not travelling in a direction on the trail
                             // that will allow us to pass this point
    return 0;
  } else if (!bs->wpDirection &&
             (gWPArray[windex]->flags &
              WPFLAG_ONEWAY_BACK)) { // we're not travelling in a direction on
                                     // the trail that will allow us to pass this
                                     // point
    return 0;
  }
  ...
  if (bs->wpCurrent && gWPArray[windex]->forceJumpTo &&
      ...
      bs->cur_ps.fd.forcePowerLevel[FP_LEVITATION] <
          gWPArray[windex]
              ->forceJumpTo) { // waypoint requires force jump level greater
                               // than our current one to pass
    return 0;
  }
...
float TotalTrailDistance(int start, int end, bot_state_t *bs) {
  ...
  while (beginat < endat) {
    if (beginat >= gWPNum || !gWPArray[beginat] ||
        !gWPArray[beginat]->inuse) { // invalid waypoint index
      return -1;
    }
    ...
    if (!RMG.integer) {
      if ((end > start && gWPArray[beginat]->flags & WPFLAG_ONEWAY_BACK) ||
          (start > end &&
           gWPArray[beginat]->flags &
               WPFLAG_ONEWAY_FWD)) { // a one-way point, this means this path
                                     // cannot be travelled to the final point
        return -1;
      }
    }
...
void CheckForShorterRoutes(bot_state_t *bs, int newwpindex) {
  ...
  while (i <
         gWPArray[newwpindex]
             ->neighbornum) { // now go through the neighbors and check the
                              // distance to the desired point from each neighbor
    checklen = TotalTrailDistance(gWPArray[newwpindex]->neighbors[i].num,
                                  bs->wpDestination->index, bs);

    if (checklen < bestlen - 64 ||
        bestlen ==
            -1) { // this path covers less distance, let's take it instead
      if (bs->cur_ps.fd.forcePowerLevel[FP_LEVITATION] >=
          gWPArray[newwpindex]->neighbors[i].forceJumpTo) {
        bestlen = checklen;
  ...
  if (bestindex != newwpindex &&
      bestindex != -1) { // we found a path we want to switch to, let's do it
...
void WPConstantRoutine(bot_state_t *bs) {
  ...
  if (bs->wpCurrent->flags &
      WPFLAG_DUCK) { // duck while travelling to this point
    bs->duckTime = level.time + 100;
  }

#ifndef FORCEJUMP_INSTANTMETHOD
  if (bs->wpCurrent->flags &
      WPFLAG_JUMP) { // jump while travelling to this point
    float heightDif = (bs->wpCurrent->origin[2] - bs->origin[2] + 16);

    if (bs->origin[2] + 16 >=
        bs->wpCurrent->origin[2]) { // don't need to jump, we're already higher
                                    // than this point
      heightDif = 0;
    }
    ...
    if (heightDif > 40 &&
        ...
        (bs->cur_ps.fd.forceJumpCharge <
             (forceJumpStrength[bs->cur_ps.fd.forcePowerLevel[FP_LEVITATION]] -
              100) ||
      ...
      if (bs->wpSeenTime < (level.time + 600)) {
        bs->wpSeenTime = level.time + 600;
               ...
               !(bs->cur_ps.fd.forcePowersKnown &
                 (1 << FP_LEVITATION))) { // this point needs force jump to
                                          // reach and we don't have it
...
void WPTouchRoutine(bot_state_t *bs) {
  ...
  if (bs->wpCurrent->flags &
      WPFLAG_NOMOVEFUNC) { // don't try to use any nearby map objects for a
                           // little while
  ...
  if ((bs->wpCurrent->flags & WPFLAG_JUMP) &&
      bs->wpCurrent->forceJumpTo) { // jump if we're flagged to but not if this
                                    // indicates a force jump point. Force
                                    // jumping is handled elsewhere.
    bs->jumpTime = level.time + 100;
  }
#else
  if ((bs->wpCurrent->flags & WPFLAG_JUMP) &&
  ...
  if (bs->isCamper && bot_camp.integer &&
      (BotIsAChickenWuss(bs) || BotCTFGuardDuty(bs) || bs->isCamper == 2) &&
      ((bs->wpCurrent->flags & WPFLAG_SNIPEORCAMP) ||
       (bs->wpCurrent->flags & WPFLAG_SNIPEORCAMPSTAND)) &&
      bs->cur_ps.weapon != WP_SABER && bs->cur_ps.weapon != WP_MELEE &&
      bs->cur_ps.weapon !=
          WP_STUN_BATON) { // if we're a camper and a chicken then camp
    if (bs->wpDirection) {
    ...
    if (gWPArray[lastNum] && gWPArray[lastNum]->inuse &&
        gWPArray[lastNum]->index && bs->isCamping < level.time) {
      ...
      if (bs->wpCurrent->flags & WPFLAG_SNIPEORCAMPSTAND) {
        ...
        bs->campStanding = qfalse;
             ...
             bs->isCamping > level.time) { // don't snipe/camp with a melee
                                           // weapon, that would be silly
    bs->isCamping = 0;
    bs->wpCampingTo = NULL;
    bs->wpCamping = NULL;
  }
  ...
  if (bs->wpDestination) {
    if (bs->wpCurrent->index == bs->wpDestination->index) {
      ...
      if (bs->runningLikeASissy) { // this obviously means we're scared and
                                   // running, so we'll want to keep our
                                   // navigational priorities less delayed
...
int BotTrace_Strafe(bot_state_t *bs, vec3_t traceto) {
  ...
  if (bs->cur_ps.groundEntityNum ==
      ENTITYNUM_NONE) { // don't do this in the air, it can be.. dangerous.
    return 0;
  }
  ...
  if (AngleDifference(bs->viewangles[YAW], dirAng[YAW]) > 60 ||
      AngleDifference(bs->viewangles[YAW], dirAng[YAW]) <
          -60) { // If we aren't facing the direction we're going here, then
                 // we've got enough excuse to be too stupid to strafe around
                 // anyway
    return 0;
  }
  ...
  trap->Trace(&tr, from, playerMins, playerMaxs, to, bs->client,
...
int BotTrace_Jump(bot_state_t *bs, vec3_t traceto) {
  ...
  trap->Trace(&tr, bs->origin, mins, maxs, traceto_mod, bs->client,
              MASK_PLAYERSOLID, qfalse, 0, 0);
  ...
  trap->Trace(&tr, tracefrom_mod, mins, maxs, traceto_mod, bs->client,
              MASK_PLAYERSOLID, qfalse, 0, 0);

  if (tr.fraction == 1) {
    if (orTr >= 0 && orTr < MAX_CLIENTS && botstates[orTr] &&
        botstates[orTr]->jumpTime > level.time) {
...
int PassStandardEnemyChecks(bot_state_t *bs, gentity_t *en) {
  ...
  if (bs->doingFallback &&
      (gLevelFlags &
       LEVELFLAG_IGNOREINFALLBACK)) { // we screwed up in our nav routines
                                      // somewhere and we've reverted to a
                                      // fallback state to
    // try to get back on the trail. If the level specifies to ignore enemies in
    // this state, then ignore them.
    return 0;
  }

  if (en->client->ps.pm_type == PM_INTERMISSION ||
      en->client->ps.pm_type == PM_SPECTATOR ||
      en->client->sess.sessionTeam ==
          TEAM_SPECTATOR) { // don't attack spectators
    return 0;
  }
  ...
  if (BotMindTricked(bs->client, en->s.number)) {
    if (bs->currentEnemy &&
        bs->currentEnemy->s.number ==
            en->s.number) { // if mindtricked by this enemy, then be less
                            // "aware" of them, even though
  ...
  if (en->client->ps.duelInProgress &&
      en->client->ps.duelIndex !=
          bs->client) { // don't attack duelists unless you're dueling them
    return 0;
  }

  if (bs->cur_ps.duelInProgress &&
      en->s.number != bs->cur_ps.duelIndex) { // ditto, the other way around
    return 0;
  }

  if (level.gametype == GT_JEDIMASTER && !en->client->ps.isJediMaster &&
      !bs->cur_ps.isJediMaster) { // rules for attacking non-JM in JM mode
    vec3_t vs;
    float vLen = 0;
...
void BotDamageNotification(gclient_t *bot, gentity_t *attacker) {
  ...
  if (attacker->s.number >=
      MAX_CLIENTS) { // if attacker is an npc also don't care I suppose.
    return;
  }
  ...
  if (bs->currentEnemy) { // we don't care about the guy attacking us if we have
                          // an enemy already
    return;
  }

  if (!PassStandardEnemyChecks(
          bs, attacker)) { // the person that hurt us is not a valid enemy
    return;
  }

  if (PassLovedOneCheck(
          bs, attacker)) { // the person that hurt us is the one we love!
...
int BotCanHear(bot_state_t *bs, gentity_t *en, float endist) {
  ...
  if (en && en->client &&
      en->client->ps.otherSoundTime >
          level.time) { // they made a noise in recent time
    minlen = en->client->ps.otherSoundLen;
    goto checkStep;
  }

  if (en && en->client &&
      en->client->ps.footstepTime > level.time) { // they made a footstep
    minlen = 256;
    goto checkStep;
  }

  if (gBotEventTracker[en->s.number].eventTime <
      level.time) { // no recent events to check
    return 0;
  }

  switch (
      gBotEventTracker[en->s.number]
          .events[gBotEventTracker[en->s.number].eventSequence &
                  (MAX_PS_EVENTS - 1)]) { // did the last event contain a sound?
...
checkStep:
  if (BotMindTricked(
          bs->client,
          en->s.number)) { // if mindtricked by this person, cut down on the
                           // minlen so they can't "hear" as well
    minlen /= 4;
  }
...
void UpdateEventTracker(void) {
  ...
  while (i < MAX_CLIENTS) {
    if (gBotEventTracker[i].eventSequence !=
        level.clients[i].ps.eventSequence) { // updated event
...
int PassLovedOneCheck(bot_state_t *bs, gentity_t *ent) {
  ...
  if (level.gametype == GT_DUEL ||
      level.gametype == GT_POWERDUEL) { // There is no love in 1-on-1
    return 1;
  }
  ...
  while (i < bs->lovednum) {
    if (strcmp(level.clients[loved->client].pers.netname, bs->loved[i].name) ==
        0) {
      if (!IsTeamplay() &&
          bs->loved[i].level < 2) { // if FFA and level of love is not greater
                                    // than 1, just don't care
        return 1;
      } else if (IsTeamplay() &&
                 !OnSameTeam(&g_entities[bs->client],
                             &g_entities[loved->client]) &&
                 bs->loved[i].level <
                     2) { // is teamplay, but not on same team and level < 2
        return 1;
      } else {
        return 0;
      }
...
int ScanForEnemies(bot_state_t *bs) {
  ...
  if (bs->currentEnemy) { // only switch to a new enemy if he's significantly
                          // closer
  ...
  if (level.gametype == GT_JEDIMASTER) {
    if (G_ThereIsAMaster() &&
        !bs->cur_ps.isJediMaster) { // if friendly fire is on in jedi master we
                                    // can attack people that bug us
      if (!g_friendlyFire.integer) {
        noAttackNonJM = qtrue;
      } else {
        closest = 128; // only get mad at people if they get close enough to you
                       // to anger you, or hurt you
      }
    }
  }
  ...
  while (i <= MAX_CLIENTS) {
    if (i != bs->client && g_entities[i].client &&
      ...
      if (g_entities[i]
              .client->ps.isJediMaster) { // make us think the Jedi Master is
                                          // close so we'll attack him above all
        distcheck = 1;
      }

      if (distcheck < closest &&
          ((InFieldOfVision(bs->viewangles, 90, a) &&
        ...
        if (BotMindTricked(bs->client, i)) {
          if (distcheck < 256 ||
              (level.time - g_entities[i].client->dangerTime) < 100) {
            if (!hasEnemyDist ||
                distcheck <
                    (hasEnemyDist -
                     128)) { // if we have an enemy, only switch to closer if he
                             // is 128+ closer to avoid flipping out
          ...
          if (!hasEnemyDist ||
              distcheck < (hasEnemyDist -
                           128)) { // if we have an enemy, only switch to closer
                                   // if he is 128+ closer to avoid flipping out
            if (!noAttackNonJM || g_entities[i].client->ps.isJediMaster) {
              closest = distcheck;
              bestindex = i;
            }
...
int WaitingForNow(
    bot_state_t *bs,
    vec3_t goalpos) { // checks if the bot is doing something along the lines of
  ...
  if (VectorLength(a) < 16 && bs->frame_Waypoint_Len > 100) {
    if (CheckForFunc(bs->origin, bs->client)) {
      return 1; // we're probably standing on an elevator and riding up/down. Or
                // at least we hope so.
    }
...
int BotIsAChickenWuss(bot_state_t *bs) {
  ...
  if (gLevelFlags &
      LEVELFLAG_IMUSTNTRUNAWAY) { // The level says we mustn't run away!
    return 0;
  }
  ...
  if (level.gametype == GT_JEDIMASTER &&
      !bs->cur_ps.isJediMaster) { // Then you may know no fear.
    // Well, unless he's strong.
    if (bs->currentEnemy && bs->currentEnemy->client &&
        bs->currentEnemy->client->ps.isJediMaster &&
        bs->currentEnemy->health > 40 &&
        bs->cur_ps.weapon <
            WP_ROCKET_LAUNCHER) { // explosive weapons are most effective
  ...
  if (level.gametype == GT_CTF && bs->currentEnemy &&
      bs->currentEnemy->client) {
    if (bs->currentEnemy->client->ps.powerups[PW_REDFLAG] ||
        bs->currentEnemy->client->ps
            .powerups[PW_BLUEFLAG]) { // don't be afraid of flag carriers, they
                                      // must die!
      return 0;
    }
  }
...
jmPass:
  ...
  if (bs->cur_ps.fd.forcePowersActive &
      (1 << FP_RAGE)) { // don't run while raging
    return 0;
  }

  if (level.gametype == GT_JEDIMASTER &&
      !bs->cur_ps.isJediMaster) { // be frightened of the jedi master? I guess
                                  // in this case.
    return 1;
  }
  ...
  if (g_entities[bs->client].health <
      BOT_RUN_HEALTH) { // we're low on health, let's get away
    return 1;
  }
  ...
  if (bWRange == BWEAPONRANGE_MELEE || bWRange == BWEAPONRANGE_SABER) {
    if (bWRange != BWEAPONRANGE_SABER ||
        !bs->saberSpecialist) { // run away if we're using melee, or if we're
                                // using a saber and not a "saber specialist"
      return 1;
    }
  }

  if (bs->cur_ps.weapon ==
      WP_BRYAR_PISTOL) { // the bryar is a weak weapon, so just try to find a
                         // new one if it's what you're having to use
    return 1;
  }

  if (bs->currentEnemy && bs->currentEnemy->client &&
      bs->currentEnemy->client->ps.weapon == WP_SABER &&
      bs->frame_Enemy_Len < 512 &&
      bs->cur_ps.weapon != WP_SABER) { // if close to an enemy with a saber and
                                       // not using a saber, then try to back off
    return 1;
  }

  if ((level.time - bs->cur_ps.electrifyTime) <
      16000) { // lightning is dangerous.
    return 1;
  }
...
gentity_t *GetNearestBadThing(bot_state_t *bs) {
  ...
  while (i < level.num_entities) {
    ...
    if ((ent && !ent->client && ent->inuse && ent->damage &&
        ...
        (ent && ent->genericValue5 == 1000 && ent->inuse && ent->health > 0 &&
         ent->genericValue3 != bs->client &&
         g_entities[ent->genericValue3].client &&
         !OnSameTeam(
             &g_entities[bs->client],
             &g_entities[ent->genericValue3]))) { // try to escape from anything
                                                  // with a non-0 s.weapon and
                                                  // non-0 damage. This hopefully
                                                  // only means dangerous
                                                  // projectiles. Or a sentry gun
                                                  // if bolt_Head == 1000. This
                                                  // is a terrible hack, yes.
      ...
      if (ent->s.weapon != WP_THERMAL && ent->s.weapon != WP_FLECHETTE &&
        ...
        if (ent->s.weapon && glen <= 256 &&
            bs->settings.skill > 2) { // it's a projectile so push it away
      ...
      if (ent->s.weapon == WP_ROCKET_LAUNCHER &&
          (ent->r.ownerNum == bs->client ||
           (ent->r.ownerNum > 0 && ent->r.ownerNum < MAX_CLIENTS &&
            ...
            OnSameTeam(
                &g_entities[bs->client],
                &g_entities[ent->r.ownerNum])))) { // don't be afraid of your
                                                   // own rockets or your
                                                   // teammates' rockets
        factor = 0;
      }
          ...
          (ent->r.ownerNum == bs->client ||
           (ent->r.ownerNum > 0 && ent->r.ownerNum < MAX_CLIENTS &&
            ...
            OnSameTeam(
                &g_entities[bs->client],
                &g_entities[ent->r.ownerNum])))) { // don't be afraid of your
                                                   // own trip mines or your
                                                   // teammates' trip mines
        factor = 0;
      }
          ...
          (ent->r.ownerNum == bs->client ||
           (ent->r.ownerNum > 0 && ent->r.ownerNum < MAX_CLIENTS &&
            ...
            OnSameTeam(
                &g_entities[bs->client],
                &g_entities[ent->r.ownerNum])))) { // don't be afraid of your
                                                   // own thermals or your
                                                   // teammates' thermals
        factor = 0;
      }

      if (glen < bestdist * factor &&
          BotPVSCheck(bs->origin, ent->s.pos.trBase)) {
        trap->Trace(&tr, bs->origin, NULL, NULL, ent->s.pos.trBase, bs->client,
    ...
    if (ent && !ent->client && ent->inuse && ent->damage && ent->s.weapon &&
        ent->r.ownerNum < MAX_CLIENTS &&
        ent->r.ownerNum >=
            0) { // if we're in danger of a projectile belonging to someone and
...
int BotGetFlagBack(bot_state_t *bs) {
  ...
  while (i < MAX_CLIENTS) {
    ...
    if (ent && ent->client && ent->client->ps.powerups[myFlag] &&
        !OnSameTeam(&g_entities[bs->client], ent)) {
      foundCarrier = 1;
      break;
    }
  ...
  if (bs->wpDestSwitchTime < level.time) {
    ...
    if (tempInt != -1 &&
        TotalTrailDistance(bs->wpCurrent->index, tempInt, bs) != -1) {
...
int BotGuardFlagCarrier(bot_state_t *bs) {
  ...
  while (i < MAX_CLIENTS) {
    ...
    if (ent && ent->client && ent->client->ps.powerups[enemyFlag] &&
        OnSameTeam(&g_entities[bs->client], ent)) {
      foundCarrier = 1;
      break;
    }
...
void GetNewFlagPoint(wpobject_t *wp, gentity_t *flagEnt,
                     int team) { // get the nearest possible waypoint to the
                                 // flag since it's not in its original position
  int i = 0;
  vec3_t a, mins, maxs;
  float bestdist;
  float testdist;
  ...
  if (bestdist <= WP_KEEP_FLAG_DIST) {
    trap->Trace(&tr, wp->origin, mins, maxs, flagEnt->s.pos.trBase,
                flagEnt->s.number, MASK_SOLID, qfalse, 0, 0);
  ...
  while (i < gWPNum) {
    ...
    if (testdist < bestdist) {
      trap->Trace(&tr, gWPArray[i]->origin, mins, maxs, flagEnt->s.pos.trBase,
                  flagEnt->s.number, MASK_SOLID, qfalse, 0, 0);
...
int CTFTakesPriority(bot_state_t *bs) {
  ...
  if (bs->cur_ps.weapon == WP_BRYAR_PISTOL &&
      (level.time - bs->lastDeadTime) <
          BOT_MAX_WEAPON_GATHER_TIME) { // get the nearest weapon laying around
                                        // base before heading off for battle
  ...
  if (!flagRed || !flagBlue || !flagRed->inuse || !flagBlue->inuse ||
      !eFlagRed || !eFlagBlue) {
    return 0;
  }
  ...
  while (i < MAX_CLIENTS) {
    ...
    if (ent && ent->client) {
      /*if (ent->client->ps.powerups[enemyFlag] &&
      OnSameTeam(&g_entities[bs->client], ent))
      {
              weHaveEnemyFlag = 1;
      }
      else */
      if (ent->client->ps.powerups[myFlag] &&
          !OnSameTeam(&g_entities[bs->client], ent)) {
        enemyHasOurFlag = 1;
      }
  ...
  if (bs->ctfState == CTFSTATE_RETRIEVAL) {
    if (BotGetFlagBack(bs)) {
      goto success;
    } else { // can't find anyone on another team being a carrier, so ignore
             // this priority
      bs->ctfState = 0;
    }
  }
  ...
  if (bs->ctfState == CTFSTATE_GUARDCARRIER) {
    if (BotGuardFlagCarrier(bs)) {
      goto success;
    } else { // can't find anyone on our team being a carrier, so ignore this
             // priority
      bs->ctfState = 0;
    }
  }
...
success:
  if (dosw) { // allow ctf code to run, but if after a particular item then keep
              // going after it
...
int EntityVisibleBox(vec3_t org1, vec3_t mins, vec3_t maxs, vec3_t org2,
...
int Siege_TargetClosestObjective(bot_state_t *bs, int flag) {
  ...
  if (bs->wpDestination && (bs->wpDestination->flags & flag) &&
      bs->wpDestination->associated_entity != ENTITYNUM_NONE &&
      &g_entities[bs->wpDestination->associated_entity] &&
  ...
  while (i < gWPNum) {
    if (gWPArray[i] && gWPArray[i]->inuse && (gWPArray[i]->flags & flag) &&
        ...
        &g_entities[gWPArray[i]->associated_entity] &&
...
hasPoint:
  ...
  // brush models can have tricky origins, so this is our hacky method of
  // getting the center point

  if (goalent->takedamage && testdistance < BOT_MIN_SIEGE_GOAL_SHOOT &&
      EntityVisibleBox(bs->origin, mins, maxs, dif, bs->client,
                       goalent->s.number)) {
...
void Siege_DefendFromAttackers(
    bot_state_t *bs) { // this may be a little cheap, but the best way to find
                       // our defending point is probably to just find the
                       // nearest person on the opposing team since they'll most
                       // likely be on offense in this situation
  int wpClose = -1;
  int i = 0;
...
int SiegeTakesPriority(bot_state_t *bs) {
  ...
  if (attacker) {
    ...
    if (defenders > teammates / 3 &&
        teammates >
            1) { // devote around 1/4 of our team to completing our own side
                 // goals even if we're a defender. If we have no side goals we
                 // will realize that later on and join the defenders
      bs->siegeState = SIEGESTATE_ATTACKER;
    }
  }
  ...
  if (bs->siegeState == SIEGESTATE_ATTACKER) {
    if (!Siege_TargetClosestObjective(
            bs, flagForAttackableObjective)) { // looks like we have no goals
                                               // other than to keep the other
      ...
      if (bs->shootGoal) {
        ...
        if (!BotPVSCheck(bs->origin, dif)) {
          ...
          trap->Trace(&tr, bs->origin, NULL, NULL, dif, bs->client, MASK_SOLID,
                      qfalse, 0, 0);
  ...
  if (dosw) { // allow siege objective code to run, but if after a particular
              // item then keep going after it
    bs->wpDestination = dest_sw;
  }
...
int JMTakesPriority(bot_state_t *bs) {
  ...
  // jmState becomes the index for the one who carries the saber. If jmState is
  // -1 then the saber is currently without an owner
  bs->jmState = -1;
  ...
  if (theImportantEntity && theImportantEntity->inuse &&
      bs->destinationGrabTime < level.time) {
    if (theImportantEntity->client) {
      wpClose = GetNearestVisibleWP(theImportantEntity->client->ps.origin,
                                    theImportantEntity->s.number);
    } else {
      wpClose = GetNearestVisibleWP(theImportantEntity->r.currentOrigin,
                                    theImportantEntity->s.number);
    }
...
int BotHasAssociated(bot_state_t *bs, wpobject_t *wp) {
  ...
  if (wp->associated_entity ==
      ENTITYNUM_NONE) { // make it think this is an item we have so we don't go
                        // after nothing
    return 1;
  }
...
void GetIdealDestination(bot_state_t *bs) {
  ...
  if (badthing && badthing->inuse && badthing->health > 0 &&
      badthing->takedamage) {
  ...
  if (!badthing && bs->dontGoBack > level.time) {
    ...
    return;
             ...
             bs->wpStoreDest) { // after we finish running away, switch back to
  ...
  if (badthing) {
    ...
    if (gWPArray[tempInt] && gWPArray[tempInt]->inuse &&
        bs->escapeDirTime < level.time) {
      ...
      if (plusLen < minusLen) {
        ...
        bs->escapeDirTime =
            level.time + Q_irand(500, 1000); // Q_irand(1000, 1400);
  ...
  if (bs->revengeEnemy && bs->revengeEnemy->health > 0 &&
      ...
      bs->revengeEnemy->client->pers.connected ==
          CON_CONNECTED) { // if we hate someone, always try to get to them
    if (bs->wpDestSwitchTime < level.time) {
      ...
      if (tempInt != -1 &&
        ...
        bs->wpDestSwitchTime = level.time + Q_irand(5000, 10000);
             ...
             bs->squadLeader->client->pers.connected == CON_CONNECTED) {
    if (bs->wpDestSwitchTime < level.time) {
    ...
    if (bs->frame_Enemy_Len < distChange || (bChicken && bChicken != 2)) {
      ...
      if (bs->frame_Enemy_Len >
          400) { // good distance away, start running toward a good place for an
                 // item or powerup or whatever
        idleWP = GetBestIdleGoal(bs);
...
void CommanderBotCTFAI(bot_state_t *bs) {
  ...
  while (i < MAX_CLIENTS) {
    ...
    if (ent && ent->client) {
      if (ent->client->ps.powerups[enemyFlag] &&
          OnSameTeam(&g_entities[bs->client], ent)) {
        weHaveEnemyFlag = 1;
      } else if (ent->client->ps.powerups[myFlag] &&
                 !OnSameTeam(&g_entities[bs->client], ent)) {
        enemyHasOurFlag = 1;
      }
  ...
  while (i < MAX_CLIENTS) {
    ...
    if (ent && ent->client && botstates[i] && botstates[i]->squadLeader &&
        botstates[i]->squadLeader->s.number == bs->client && i != bs->client) {
  ...
  if (enemyHasOurFlag &&
      !weHaveEnemyFlag) { // start off with an attacker instead of a retriever
                          // if we don't have the enemy flag yet so that they
                          // can't capture it first. after that we focus on
  ...
  while (i < squadmates) {
    if (squad[i] && squad[i]->client && botstates[squad[i]->s.number]) {
      if (botstates[squad[i]->s.number]->ctfState !=
          CTFSTATE_GETFLAGHOME) { // never tell a bot to stop trying to bring
                                  // the flag to the base
        if (defendAttackPriority) {
          ...
          defendAttackPriority = 1;
                 ...
                 enemyHasOurFlag) { // I'm the only one on my team who will
                                    // attack and the enemy has my flag, I have
                                    // to go after him
        botstates[squad[i]->s.number]->ctfState = CTFSTATE_RETRIEVAL;
      }
    }
...
void CommanderBotSiegeAI(bot_state_t *bs) {
  ...
  while (i < MAX_CLIENTS) {
    ...
    if (ent && ent->client && OnSameTeam(&g_entities[bs->client], ent) &&
      ...
      if (bst && !bst->isSquadLeader && !bst->state_Forced) {
        squad[squadmates] = ent;
        squadmates++;
      } else if (bst && !bst->isSquadLeader &&
                 bst->state_Forced) { // count them as commanded
        commanded++;
      }
    }
  ...
  // tell squad mates to do what I'm doing, up to half of team, let the other
  // half make their own decisions
  i = 0;
...
void BotDoTeamplayAI(bot_state_t *bs) {
  ...
  if (bs->teamplayState == TEAMPLAYSTATE_REGROUP) { // force to find a new
                                                    // leader
    bs->squadLeader = NULL;
    bs->isSquadLeader = 0;
  }
...
void CommanderBotTeamplayAI(bot_state_t *bs) {
  ...
  while (i < MAX_CLIENTS) {
    ...
    if (ent && ent->client && OnSameTeam(&g_entities[bs->client], ent) &&
      ...
      if (foundsquadleader && bst &&
          bst->isSquadLeader) { // never more than one squad leader
        bst->isSquadLeader = 0;
      }
  ...
  while (i < squadmates && squad[i]) {
    ...
    if (bst &&
        !bst->state_Forced) { // only order if this guy is not being ordered
                              // directly by the real player team leader
      if (teammate_indanger >= 0 &&
          !teammate_helped) { // send someone out to help whoever needs help
                              // most at the moment
        ...
        teammate_helped = 1;
                 ...
                 bst->teamplayState ==
                     TEAMPLAYSTATE_ASSISTING) { // no teammates need help badly,
                                                // but this guy is trying to help
                                                // them anyway, so stop
        bst->teamplayState = TEAMPLAYSTATE_FOLLOWING;
        bst->squadLeader = &g_entities[bs->client];
      }

      if (bs->squadRegroupInterval < level.time &&
          Q_irand(1, 10) < 5) { // every so often tell the squad to regroup for
...
void MeleeCombatHandling(bot_state_t *bs) {
  ...
  trap->Trace(&tr, usethisvec, mins, maxs, downvec, -1, MASK_SOLID, qfalse, 0,
              0);
  ...
  trap->Trace(&tr, bs->origin, mins, maxs, downvec, -1, MASK_SOLID, qfalse, 0,
              0);
...
void SaberCombatHandling(bot_state_t *bs) {
  ...
  if (me_down == en_down && en_down == mid_down) {
    if (usethisvec[2] > (bs->origin[2] + 32) && bs->currentEnemy->client &&
    ...
    if (bs->currentEnemy && bs->currentEnemy->client) {
      if (!BG_SaberInSpecial(bs->currentEnemy->client->ps.saberMove) &&
          bs->frame_Enemy_Len > 90 && bs->saberBFTime > level.time &&
          bs->saberBTime > level.time && bs->beStill < level.time &&
        ...
        bs->saberSTime = level.time + Q_irand(1200, 1800);
                 ...
                 ((Q_irand(1, 10) < 8 && bs->saberBFTime < level.time) ||
                  bs->saberBTime > level.time ||
                  BG_SaberInKata(bs->currentEnemy->client->ps.saberMove) ||
        ...
        if (BG_SaberInKata(bs->currentEnemy->client->ps.saberMove) ||
            bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK ||
...
void BotAimLeading(bot_state_t *bs, vec3_t headlevel, float leadAmount) {
  ...
  x = bs->frame_Enemy_Len *
      leadAmount; // hardly calculated with an exact science, but it works
  ...
  if (vtotal) {
    x = (bs->frame_Enemy_Len * 0.9) * leadAmount *
        (vtotal *
         0.0012); // hardly calculated with an exact science, but it works
  } else {
    x = (bs->frame_Enemy_Len * 0.9) *
        leadAmount; // hardly calculated with an exact science, but it works
  }
...
void BotAimOffsetGoalAngles(bot_state_t *bs) {
  ...
  if (bs->currentEnemy &&
      BotMindTricked(
          bs->client,
          bs->currentEnemy->s
              .number)) { // having to judge where they are by hearing them, so
                          // we should be quite inaccurate here
    accVal *= 7;
  ...
  if (bs->revengeEnemy && bs->revengeHateLevel &&
      bs->currentEnemy ==
          bs->revengeEnemy) { // bot becomes more skilled as anger level raises
    accVal = accVal / bs->revengeHateLevel;
  }

  if (bs->currentEnemy &&
      bs->frame_Enemy_Vis) { // assume our goal is aiming at the enemy, seeing
                             // as he's visible and all
    if (!bs->currentEnemy->s.pos.trDelta[0] &&
        !bs->currentEnemy->s.pos.trDelta[1] &&
        !bs->currentEnemy->s.pos.trDelta[2]) {
      accVal = 0; // he's not even moving, so he shouldn't really be hard to
                  // hit.
    } else {
    ...
    if (g_entities[bs->client].s.pos.trDelta[0] ||
      ...
      accVal +=
          accVal * 0.15; // make it somewhat harder to aim if we're moving also
    }
  }
...
int ShouldSecondaryFire(bot_state_t *bs) {
  ...
  if (bs->cur_ps.ammo[weaponData[weap].ammoIndex] <
  ...
  if (bs->cur_ps.weaponstate == WEAPON_CHARGING_ALT &&
    ...
    if (heldTime > 5000) { // just give up and release it if we can't manage a
                           // lock in 5 seconds
      return 2;
    }
    ...
    if (rTime > 0) {
      ...
      return 1;
             ...
             (level.time - bs->cur_ps.weaponChargeTime) > bs->altChargeTime) {
    return 2;
  } else if (bs->cur_ps.weaponstate == WEAPON_CHARGING_ALT) {
    return 1;
  }
  ...
  if (weap == WP_BRYAR_PISTOL && bs->frame_Enemy_Len < 300) {
    ...
    return 1;
  } else if (weap == WP_REPEATER && bs->frame_Enemy_Len < 600 &&
...
int CombatBotAI(bot_state_t *bs, float thinktime) {
  ...
  if (BotGetWeaponRange(bs) == BWEAPONRANGE_SABER) {
    ...
    if (bs->cur_ps.weapon == WP_THERMAL ||
        bs->cur_ps.weapon ==
            WP_ROCKET_LAUNCHER) { // be careful with the hurty weapons
      fovcheck = 40;

      if (bs->cur_ps.weaponstate == WEAPON_CHARGING_ALT &&
          bs->cur_ps.weapon ==
              WP_ROCKET_LAUNCHER) { // if we're charging the weapon up then we
                                    // can hold fire down within a normal fov
        fovcheck = 60;
      }
    } else {
      fovcheck = 60;
    ...
    if (InFieldOfVision(bs->viewangles, fovcheck, a)) {
      if (bs->cur_ps.weapon == WP_THERMAL) {
        if (((level.time - bs->cur_ps.weaponChargeTime) <
                 (bs->frame_Enemy_Len * 2) &&
        ...
        if (secFire == 1) {
          ...
          if (bs->cur_ps.weapon != WP_THERMAL) {
            if (bs->cur_ps.weaponstate != WEAPON_CHARGING ||
                bs->altChargeTime >
                    (level.time - bs->cur_ps.weaponChargeTime)) {
              bs->doAttack = 1;
            }
          } else {
            bs->doAttack = 1;
...
int BotFallbackNavigation(bot_state_t *bs) {
  ...
  trap->Trace(&tr, bs->origin, mins, maxs, trto, ENTITYNUM_NONE, MASK_SOLID,
              qfalse, 0, 0);
...
int BotTryAnotherWeapon(bot_state_t *bs) { // out of ammo, resort to the first
                                           // weapon we come across that has ammo
  int i;

  i = 1;

  while (i < WP_NUM_WEAPONS) {
    if (bs->cur_ps.ammo[weaponData[i].ammoIndex] >=
  ...
  if (bs->cur_ps.weapon != 1 &&
      bs->virtualWeapon != 1) { // should always have this.. shouldn't we?
    bs->virtualWeapon = 1;
...
qboolean BotWeaponSelectable(bot_state_t *bs, int weapon) {
  ...
  if (bs->cur_ps.ammo[weaponData[weapon].ammoIndex] >=
          weaponData[weapon].energyPerShot &&
...
int BotSelectIdealWeapon(bot_state_t *bs) {
  ...
  if (bs->currentEnemy && bs->frame_Enemy_Len < 300 &&
      (bestweapon == WP_BRYAR_PISTOL || bestweapon == WP_BLASTER ||
  ...
  if (bs->currentEnemy && bs->frame_Enemy_Len > 300 &&
      ...
      bs->currentEnemy->client->ps.weapon != WP_SABER &&
      (bestweapon ==
       WP_SABER)) { // if the enemy is far away, and we have our saber selected,
                    // see if we have any good distance weapons instead
  ...
  if (bestweight != -1 && bs->cur_ps.weapon != bestweapon &&
...
int BotSelectChoiceWeapon(
    bot_state_t *bs, int weapon,
    int doselection) { // if !doselection then bot will only check if he has the
  ...
  while (i < WP_NUM_WEAPONS) {
    if (bs->cur_ps.ammo[weaponData[i].ammoIndex] >
            weaponData[i].energyPerShot &&
        i == weapon && (bs->cur_ps.stats[STAT_WEAPONS] & (1 << i))) {
      hasit = 1;
      break;
    }
  ...
  if (hasit && bs->cur_ps.weapon != weapon && doselection &&
...
void BotLovedOneDied(bot_state_t *bs, bot_state_t *loved, int lovelevel) {
  ...
  if (!IsTeamplay()) {
    if (lovelevel < 2) {
      return;
                 ...
                 &g_entities[bs->client],
                 loved->lastHurt)) { // don't hate teammates no matter what
    return;
  }
  ...
  if (!PassLovedOneCheck(bs, loved->lastHurt)) { // a loved one killed a loved
                                                 // one.. you cannot hate them
  ...
  } else if (bs->revengeHateLevel <
             bs->loved_death_thresh -
                 1) { // only switch hatred if we don't hate the existing
                      // revenge-enemy too much
...
void BotDeathNotify(bot_state_t *bs) { // in case someone has an emotional
                                       // attachment to us, we'll notify them
  int i = 0;
  int ltest = 0;
  ...
  while (i < MAX_CLIENTS) {
    if (botstates[i] && botstates[i]->lovednum) {
      ...
      while (ltest < botstates[i]->lovednum) {
        if (strcmp(level.clients[bs->client].pers.netname,
                   botstates[i]->loved[ltest].name) == 0) {
...
void StrafeTracing(bot_state_t *bs) {
  ...
  trap->Trace(&tr, bs->origin, mins, maxs, rorg, bs->client, MASK_SOLID, qfalse,
              0, 0);
  ...
  trap->Trace(&tr, rorg, NULL, NULL, drorg, bs->client, MASK_SOLID, qfalse, 0,
              0);

  if (tr.fraction == 1) { // this may be a dangerous ledge, so don't strafe over
                          // it just in case
    bs->meleeStrafeDisable = level.time + Q_irand(500, 1500);
  }
}
...
// Try not to shoot our friends in the back. Or in the face. Or anywhere,
// really.
gentity_t *CheckForFriendInLOF(bot_state_t *bs) {
  ...
  trap->Trace(&tr, trfrom, mins, maxs, trto, bs->client, MASK_PLAYERSOLID,
              qfalse, 0, 0);
  ...
  if (tr.fraction != 1 && tr.entityNum <= MAX_CLIENTS) {
    ...
    if (trent && trent->client) {
      ...
      if (botstates[trent->s.number] &&
          GetLoveLevel(bs, botstates[trent->s.number]) > 1) {
        return trent;
      }
    }
  }
...
void BotScanForLeader(
    bot_state_t *bs) { // bots will only automatically obtain a leader if it's
                       // another bot using this method.
  int i = 0;
  gentity_t *ent;
  ...
  while (i < MAX_CLIENTS) {
    ...
    if (ent && ent->client && botstates[i] && botstates[i]->isSquadLeader &&
        bs->client != i) {
      if (OnSameTeam(&g_entities[bs->client], ent)) {
        bs->squadLeader = ent;
        break;
      }
      if (GetLoveLevel(bs, botstates[i]) > 1 &&
          !IsTeamplay()) { // ignore love status regarding squad leaders if
                           // we're in teamplay
        bs->squadLeader = ent;
        break;
      }
...
void CTFFlagMovement(bot_state_t *bs) {
  ...
  if (flagRed && flagBlue) {
    if (bs->wpDestination == flagRed || bs->wpDestination == flagBlue) {
      if (bs->wpDestination == flagRed && droppedRedFlag &&
          (droppedRedFlag->flags & FL_DROPPED_ITEM) &&
          droppedRedFlag->classname &&
          strcmp(droppedRedFlag->classname, "freed") != 0) {
        desiredDrop = droppedRedFlag;
        diddrop = 1;
      }
      if (bs->wpDestination == flagBlue && droppedBlueFlag &&
          (droppedBlueFlag->flags & FL_DROPPED_ITEM) &&
          droppedBlueFlag->classname &&
      ...
      if (diddrop && desiredDrop) {
        ...
        if (VectorLength(a) <= BOT_FLAG_GET_DISTANCE) {
          trap->Trace(&tr, bs->origin, mins, maxs, desiredDrop->s.pos.trBase,
                      bs->client, MASK_SOLID, qfalse, 0, 0);
...
void BotCheckDetPacks(bot_state_t *bs) {
  ...
  if (!bs->currentEnemy || !bs->currentEnemy->client ||
      !bs->frame_Enemy_Vis) { // require the enemy to be visilbe just to be
                              // fair..

    // unless..
    if (bs->currentEnemy && bs->currentEnemy->client &&
        (level.time - bs->plantContinue) <
            5000) { // it's a fresh plant (within 5 seconds) so we should be
                    // able to guess
      goto stillmadeit;
    }
    return;
...
stillmadeit:
  ...
  if (enLen < BOT_PLANT_BLOW_DISTANCE &&
      OrgVisible(
          bs->currentEnemy->client->ps.origin, myDet->s.pos.trBase,
          bs->currentEnemy->s
              .number)) { // we could just call the "blow all my detpacks"
                          // function here, but I guess that's cheating.
...
int BotUseInventoryItem(bot_state_t *bs) {
  ...
  if (bs->cur_ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_SHIELD)) {
    if (bs->currentEnemy && bs->frame_Enemy_Vis &&
        bs->runningToEscapeThreat) { // this will (hopefully) result in the bot
                                     // placing the shield down while facing the
...
wantuseitem:
  level.clients[bs->client].ps.stats[STAT_HOLDABLE_ITEM] =
      bs->cur_ps.stats[STAT_HOLDABLE_ITEM];
...
int BotSurfaceNear(bot_state_t *bs) {
  ...
  trap->Trace(&tr, bs->origin, NULL, NULL, fwd, bs->client, MASK_SOLID, qfalse,
              0, 0);
...
void StandardBotAI(bot_state_t *bs, float thinktime) {
...
#ifndef FINAL_BUILD
  if (bot_getinthecarrr.integer) { // stupid vehicle debug, I tire of having to
                                   // connect another client to test passengers.
    gentity_t *botEnt = &g_entities[bs->client];

    if (botEnt->inuse && botEnt->client &&
        botEnt->client->ps.m_iVehicleNum) { // in a vehicle, so...
      ...
      while (i < MAX_GENTITIES) {
        ...
        if (vehicle->inuse && vehicle->client && vehicle->s.eType == ET_NPC &&
            ...
            (vehicle->client->ps.m_iVehicleNum ||
             bot_getinthecarrr.integer ==
                 2)) { // ok, this is a vehicle, and it has a pilot/passengers
          break;
        }
        i++;
      }
      if (i != MAX_GENTITIES &&
          vehicle) { // broke before end so we must've found something
        vec3_t v;
  ...
  if (bot_forgimmick.integer) {
    ...
    if (bot_forgimmick.integer ==
        2) { // for debugging saber stuff, this is handy
      trap->EA_Attack(bs->client);
    }

    if (bot_forgimmick.integer ==
        3) { // for testing cpu usage moving around rmg terrain without AI
      vec3_t mdir;
  ...
  if (g_entities[bs->client].health < 1) {
    ...
    if (!bs->deathActivitiesDone && bs->lastHurt && bs->lastHurt->client &&
      ...
      if (PassLovedOneCheck(bs, bs->lastHurt)) {
        ...
        BotDoChat(bs, "Died", 0);
                 ...
                 PassLovedOneCheck(
                     botstates[bs->lastHurt->s.number],
                     &g_entities[bs->client])) { // killed by a bot that I love,
                                                 // but that does not love me
        bs->chatObject = bs->lastHurt;
        bs->chatAltObject = NULL;
...
#endif
  {
    ...
    if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_PUSH)) &&
        ...
        level.clients[bs->client].ps.fd.forcePower >
            forcePowerNeeded
                [level.clients[bs->client].ps.fd.forcePowerLevel[FP_PUSH]]
                [FP_PUSH] /*&& InFieldOfVision(bs->viewangles, 50, a_fo)*/) {
      level.clients[bs->client].ps.fd.forcePowerSelected = FP_PUSH;
      useTheForce = 1;
      forceHostile = 1;
    } else if (bs->cur_ps.fd.forceSide ==
               FORCE_DARKSIDE) { // try dark side powers
                                 // in order of priority top to bottom
      if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_GRIP)) &&
          (bs->cur_ps.fd.forcePowersActive & (1 << FP_GRIP)) &&
          InFieldOfVision(bs->viewangles, 50,
                          a_fo)) { // already gripping someone, so hold it
        ...
        forceHostile = 1;
                 ...
                 level.clients[bs->client].ps.fd.forcePower >
                     forcePowerNeeded[level.clients[bs->client]
                                          .ps.fd.forcePowerLevel[FP_GRIP]]
                                     [FP_GRIP] &&
                 InFieldOfVision(bs->viewangles, 50, a_fo)) {
        level.clients[bs->client].ps.fd.forcePowerSelected = FP_GRIP;
        useTheForce = 1;
        ...
        forceHostile = 1;
               ...
               FORCE_LIGHTSIDE) { // try light side powers
      if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_ABSORB)) &&
          bs->cur_ps.fd.forceGripCripple &&
          level.clients[bs->client].ps.fd.forcePower >
              forcePowerNeeded[level.clients[bs->client]
                                   .ps.fd.forcePowerLevel[FP_ABSORB]]
                              [FP_ABSORB]) { // absorb to get out
        ...
        forceHostile = 0;
                 ...
                 level.clients[bs->client].ps.fd.forcePower >
                     forcePowerNeeded[level.clients[bs->client]
                                          .ps.fd.forcePowerLevel[FP_TELEPATHY]]
                                     [FP_TELEPATHY] &&
                 InFieldOfVision(bs->viewangles, 50, a_fo) &&
        ...
        forceHostile = 0;
                 ...
                 level.clients[bs->client].ps.fd.forcePower >
                     forcePowerNeeded[level.clients[bs->client]
                                          .ps.fd.forcePowerLevel[FP_PROTECT]]
                                     [FP_PROTECT]) {
    ...
    if (!useTheForce) { // try neutral powers
      if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_PUSH)) &&
          ...
          level.clients[bs->client].ps.fd.forcePower >
              forcePowerNeeded[level.clients[bs->client]
                                   .ps.fd.forcePowerLevel[FP_PUSH]][FP_PUSH] &&
          InFieldOfVision(bs->viewangles, 50, a_fo)) {
        level.clients[bs->client].ps.fd.forcePowerSelected = FP_PUSH;
        useTheForce = 1;
        forceHostile = 1;
  ...
  if (!useTheForce) { // try powers that we don't care if we have an enemy for
    if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_HEAL)) &&
        ...
        level.clients[bs->client].ps.fd.forcePower >
            forcePowerNeeded[level.clients[bs->client]
                                 .ps.fd.forcePowerLevel[FP_HEAL]][FP_HEAL] &&
        bs->cur_ps.fd.forcePowerLevel[FP_HEAL] > FORCE_LEVEL_1) {
      level.clients[bs->client].ps.fd.forcePowerSelected = FP_HEAL;
      useTheForce = 1;
      forceHostile = 0;
    } else if ((bs->cur_ps.fd.forcePowersKnown & (1 << FP_HEAL)) &&
               g_entities[bs->client].health < 50 &&
               level.clients[bs->client].ps.fd.forcePower >
                   forcePowerNeeded[level.clients[bs->client]
                                        .ps.fd.forcePowerLevel[FP_HEAL]]
                                   [FP_HEAL] &&
               !bs->currentEnemy &&
               bs->isCamping >
                   level.time) { // only meditate and heal if we're camping
      level.clients[bs->client].ps.fd.forcePowerSelected = FP_HEAL;
      useTheForce = 1;
      forceHostile = 0;
    }
  ...
  if (useTheForce && forceHostile) {
    if (bs->currentEnemy && bs->currentEnemy->client &&
        !ForcePowerUsableOn(
            &g_entities[bs->client], bs->currentEnemy,
  ...
  if (bs->cur_ps.ammo[weaponData[bs->cur_ps.weapon].ammoIndex] <
      weaponData[bs->cur_ps.weapon].energyPerShot) {
    ...
    if (!bs->forceWeaponSelect && bs->cur_ps.hasDetPackPlanted &&
        bs->plantKillEmAll > level.time) {
  ...
  if (bs->cur_ps.weapon == WP_DET_PACK && bs->cur_ps.hasDetPackPlanted &&
      bs->plantKillEmAll > level.time) {
    bs->doAltAttack = 1;
  }
  ...
  if (bs->currentEnemy) {
    if (bs->enemySeenTime < level.time ||
      ...
      if (bs->revengeEnemy == bs->currentEnemy &&
          bs->currentEnemy->health < 1 && bs->lastAttacked &&
        ...
        bs->revengeHateLevel = 0;
                 ...
                 PassLovedOneCheck(bs, bs->currentEnemy) && bs->lastAttacked &&
                 bs->lastAttacked == bs->currentEnemy) {
        // CHAT: Killed
  ...
  if (bot_honorableduelacceptance.integer) {
    if (bs->currentEnemy && bs->currentEnemy->client &&
        bs->cur_ps.weapon == WP_SABER && g_privateDuel.integer &&
        bs->frame_Enemy_Vis && bs->frame_Enemy_Len < 400 &&
        bs->currentEnemy->client->ps.weapon == WP_SABER &&
      ...
      if (InFieldOfVision(
              bs->viewangles, 100,
              e_ang_vec)) { // Our enemy has his saber holstered and has
                            // challenged us to a duel, so challenge him back
  ...
  // Apparently this "allows you to cheese" when fighting against bots. I'm not
  // sure why you'd want to con bots into an easy kill, since they're bots and
  // all. But whatever.
  ...
  if (!bs->wpCurrent) {
    ...
    if (wp != -1) {
      ...
      bs->wpTravelTime =
          level.time +
          10000; // never take more than 10 seconds to travel to a waypoint
    }
  }

  if (bs->enemySeenTime < level.time || !bs->frame_Enemy_Vis || !bs->currentEnemy || (bs->currentEnemy /*&& bs->cur_ps.weapon == WP_SABER && bs->frame_Enemy_Len > 300*/)) {
  ...
  if (!bs->squadLeader &&
      bs->squadCannotLead < level.time) { // if still no leader after scanning,
                                          // then become a squad leader
    bs->isSquadLeader = 1;
  }

  if (bs->isSquadLeader &&
      bs->squadLeader) { // we don't follow anyone if we are a leader
    bs->squadLeader = NULL;
  }

  // ESTABLISH VISIBILITIES AND DISTANCES FOR THE WHOLE FRAME HERE
  if (bs->wpCurrent) {
    if (RMG.integer) { // this is somewhat hacky, but in RMG we don't really
                       // care about vertical placement because points are
                       // scattered across only the terrain.
      vec3_t vecB, vecC;
    ...
    visResult = WPOrgVisible(&g_entities[bs->client], bs->origin,
                             bs->wpCurrent->origin, bs->client);
  ...
  if (bs->wpCurrent) {
    ...
    if (!bs->wpCurrent) { // WPConstantRoutine has the ability to nullify the
                          // waypoint if it fails certain checks, so..
      return;
    }
    ...
    if (bs->frame_Waypoint_Vis || (bs->wpCurrent->flags & WPFLAG_NOVIS)) {
      if (RMG.integer) {
        bs->wpSeenTime =
            level.time + 5000; // if we lose sight of the point, we have 1.5
                               // seconds to regain it before we drop it
      } else {
        bs->wpSeenTime =
            level.time + 1500; // if we lose sight of the point, we have 1.5
                               // seconds to regain it before we drop it
      }
    }
    ...
    if (bs->wpCurrent && bs->wpDestination) {
      if (TotalTrailDistance(bs->wpCurrent->index, bs->wpDestination->index,
                             bs) == -1) {
        bs->wpDestination = NULL;
        bs->destinationGrabTime = level.time + 10000;
      }
    ...
    if (bs->frame_Waypoint_Len < wpTouchDist ||
      ...
      if (gWPArray[desiredIndex] && gWPArray[desiredIndex]->inuse &&
          desiredIndex < gWPNum && desiredIndex >= 0 &&
  ...
  if (RMG.integer) { // for RMG if the bot sticks around an area too long, jump
                     // around randomly some to spread to a new area (horrible
                     // hacky method)
    vec3_t vSubDif;
  ...
  if (bs->wpCurrent && RMG.integer) {
    ...
    if (bs->wpCurrent->origin[2] - 192 > bs->origin[2]) {
      doJ = qtrue;
    } else if ((bs->wpTravelTime - level.time) < 5000 &&
               bs->wpCurrent->origin[2] - 64 > bs->origin[2]) {
      doJ = qtrue;
    } else if ((bs->wpTravelTime - level.time) < 7000 &&
               (bs->wpCurrent->flags & WPFLAG_RED_FLAG)) {
      if ((level.time - bs->jumpTime) > 200) {
        ...
        bs->jDelay = 0;
               ...
               (bs->wpCurrent->flags & WPFLAG_BLUE_FLAG)) {
      if ((level.time - bs->jumpTime) > 200) {
        bs->jumpTime = level.time + 100;
        bs->jumpHoldTime = level.time + 100;
        bs->jDelay = 0;
  ...
  if (bs->timeToReact < level.time && bs->currentEnemy &&
      bs->enemySeenTime >
          level.time + (ENEMY_FORGET_MS - (ENEMY_FORGET_MS * 0.2))) {
    if (bs->frame_Enemy_Vis) {
      CombatBotAI(bs, thinktime);
    } else if (bs->cur_ps.weaponstate ==
               WEAPON_CHARGING_ALT) { // keep charging in case we see him again
                                      // before we lose track of him
      bs->doAltAttack = 1;
    } else if (bs->cur_ps.weaponstate ==
               WEAPON_CHARGING) { // keep charging in case we see him again
                                  // before we lose track of him
      bs->doAttack = 1;
    }
    ...
    if (bs->destinationGrabTime > level.time + 100) {
      bs->destinationGrabTime =
          level.time +
          100; // assures that we will continue staying within a general area of
               // where we want to be in a combat situation
    }
    ...
    if (!bs->frame_Enemy_Vis) {
      ...
      if (OrgVisible(bs->eye, bs->lastEnemySpotted, -1)) {
        ...
        if (bs->cur_ps.weapon == WP_FLECHETTE &&
            bs->cur_ps.weaponstate == WEAPON_READY && bs->currentEnemy &&
            bs->currentEnemy->client) {
  ...
  if (bs->currentEnemy) {
    if (BotGetWeaponRange(bs) == BWEAPONRANGE_SABER) {
      ...
      if (bs->saberPowerTime < level.time) { // Don't just use strong attacks
                                             // constantly, switch around a bit
        if (Q_irand(1, 10) <= 5) {
          bs->saberPower = qtrue;
        } else {
      ...
      if (g_entities[bs->client].client->ps.fd.saberAnimLevel != SS_STAFF &&
        ...
        if (bs->currentEnemy->health > 75 &&
          ...
          if (g_entities[bs->client].client->ps.fd.saberAnimLevel !=
                  SS_STRONG &&
              bs->saberPower) { // if we are up against someone with a lot of
                                // health and we have a strong attack available,
                                // then h4q them
            Cmd_SaberAttackCycle_f(&g_entities[bs->client]);
                   ...
                   g_entities[bs->client]
                       ...
                       1) {
          if (g_entities[bs->client].client->ps.fd.saberAnimLevel !=
              SS_MEDIUM) { // they're down on health a little, use level 2 if we
                           // can
            Cmd_SaberAttackCycle_f(&g_entities[bs->client]);
          }
        } else {
          if (g_entities[bs->client].client->ps.fd.saberAnimLevel !=
              SS_FAST) { // they've gone below 40 health, go at them with quick
                         // attacks
            Cmd_SaberAttackCycle_f(&g_entities[bs->client]);
          }
        }
  ...
  if (doingFallback &&
      bs->currentEnemy) // just stand and fire if we have no idea where we are
  {
    VectorCopy(bs->origin, bs->goalPosition);
  }
  ...
  if (bs->doChat && bs->chatTime > level.time &&
    ...
    bs->doChat = 0; // do we want to keep the bot waiting to chat until after
                    // the enemy is gone?
    bs->chatTeam = 0;
  ...
  if (/*bs->wpDestination &&*/ bs->shootGoal &&
    ...
    if (!bs->currentEnemy ||
        bs->frame_Enemy_Len >
            256) { // if someone is close then don't stop shooting them for this
      VectorSubtract(dif, bs->eye, a);
      vectoangles(a, a);
      VectorCopy(a, bs->goalAngles);

      if (InFieldOfVision(bs->viewangles, 30, a) &&
          EntityVisibleBox(bs->origin, NULL, NULL, dif, bs->client,
  ...
  if (bs->cur_ps.hasDetPackPlanted) { // check if our enemy gets near it and
                                      // detonate if he does
    BotCheckDetPacks(bs);
  } else if (bs->currentEnemy &&
             bs->lastVisibleEnemyIndex == bs->currentEnemy->s.number &&
             !bs->frame_Enemy_Vis && bs->plantTime < level.time &&
             !bs->doAttack && !bs->doAltAttack) {
    VectorSubtract(bs->origin, bs->hereWhenSpotted, a);

    if (bs->plantDecided > level.time ||
        (bs->frame_Enemy_Len < BOT_PLANT_DISTANCE * 2 &&
         VectorLength(a) < BOT_PLANT_DISTANCE)) {
      ...
      if (bs->plantDecided > level.time && bs->forceWeaponSelect &&
        ...
        if (BotSurfaceNear(bs)) {
          ...
          if (detSelect &&
              detSelect != 2) { // We have it and it is now our weapon
  ...
  if (level.gametype == GT_JEDIMASTER && !bs->cur_ps.isJediMaster &&
      bs->jmState == -1 && gJMSaberEnt && gJMSaberEnt->inuse) {
    vec3_t saberLen;
    float fSaberLen = 0;
  ...
  if (bs->beStill < level.time && !WaitingForNow(bs, bs->goalPosition) &&
    ...
    if (bs->meleeStrafeDir && meleestrafe &&
  ...
  if (bs->forceJumpChargeTime > level.time) {
    bs->jumpHoldTime =
        ((bs->forceJumpChargeTime - level.time) / 2) + level.time;
    bs->forceJumpChargeTime = 0;
  }
  ...
  if (bs->dangerousObject && bs->dangerousObject->inuse &&
      bs->dangerousObject->health > 0 && bs->dangerousObject->takedamage &&
    ...
    if (danLen > 256) {
      ...
      if (InFieldOfVision(bs->viewangles, 30, a) &&
          EntityVisibleBox(bs->origin, NULL, NULL,
                           bs->dangerousObject->r.currentOrigin, bs->client,
  ...
  if (PrimFiring(bs) || AltFiring(bs)) {
    ...
    if (friendInLOF) {
      ...
      if (!useTheForce &&
          friendInLOF
              ->client) { // we have a friend here and are not currently using
                          // force powers, see if we can help them out
        if (friendInLOF->health <= 50 &&
            level.clients[bs->client].ps.fd.forcePower >
                forcePowerNeeded[level.clients[bs->client]
                                     .ps.fd.forcePowerLevel[FP_TEAM_HEAL]]
                                [FP_TEAM_HEAL]) {
    ...
    if (!useTheForce && friendInLOF) {
      if (friendInLOF->health <= 50 &&
        ...
        forceHostile = 0;
                 ...
                 level.clients[bs->client].ps.fd.forcePower >
                     forcePowerNeeded[level.clients[bs->client]
                                          .ps.fd.forcePowerLevel[FP_TEAM_FORCE]]
                                     [FP_TEAM_FORCE]) {
        level.clients[bs->client].ps.fd.forcePowerSelected = FP_TEAM_FORCE;
        useTheForce = 1;
        forceHostile = 0;
  ...
  if (bs->doAttack && bs->cur_ps.weapon == WP_DET_PACK &&
      bs->cur_ps.hasDetPackPlanted) { // maybe a bit hackish, but bots only want
                                      // to plant one of these at any given time
                                      // to avoid complications
    bs->doAttack = 0;
  }

  if (bs->doAttack && bs->cur_ps.weapon == WP_SABER && bs->saberDefending &&
  ...
  if (bs->cur_ps.weapon == WP_SABER && bs->cur_ps.saberInFlight &&
      !bs->cur_ps
           .saberEntityNum) { // saber knocked away, keep trying to get it back
    bs->doAttack = 1;
    bs->doAltAttack = 0;
  }
...
int BotAISetup(int restart) {
  ...
  trap->Cvar_Register(&bot_honorableduelacceptance,
                      "bot_honorableduelacceptance", "0", CVAR_CHEAT);
