...
//---------------------------------------------------------
{
  ...
  int damage = altFire ? weaponData[WP_BLASTER].altDamage
                       : weaponData[WP_BLASTER].damage;

  if (ent && ent->client && ent->client->NPC_class == CLASS_VEHICLE) {
    damage *= 3;
    velocity = ATST_MAIN_VEL + ent->client->ps.speed;
  } else {
    // If an enemy is shooting at us, lower the velocity so you have a chance to
    // evade
  ...
  //	if ( ent->client )
  //	{
  //		if ( ent->client->ps.powerups[PW_WEAPON_OVERCHARGE] > 0 &&
...
//---------------------------------------------------------
{
  ...
  if (ent->client &&
      ent->client->NPC_class == CLASS_VEHICLE) { // no inherent aim screw up
