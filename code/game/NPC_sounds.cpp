...
void G_AddVoiceEvent(gentity_t *self, int event, int speakDebounceTime) {
  ...
  if (self->client && self->client->NPC_class == CLASS_SABOTEUR) {
    if (self->client->ps.powerups[PW_CLOAKED] ||
        self->client->ps.powerups[PW_UNCLOAKING] >
            level.time) { // I'm cloaked (or still decloaking), so don't talk
                          // and give away my position...
      // don't make any combat voice noises, but still make pain and death
      // sounds
