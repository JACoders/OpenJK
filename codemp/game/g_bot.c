...
void G_LoadArenasFromFile(char *filename) {
  ...
  level.arenas.num += G_ParseInfos(buf, MAX_ARENAS - level.arenas.num,
...
qboolean G_DoesMapSupportGametype(const char *mapname, int gametype) {
  ...
  if (typeBits & (1 << gametype)) { // the map in question supports the gametype
                                    // in question, so..
    return qtrue;
  }
...
// rww - auto-obtain nextmap. I could've sworn Q3 had something like this, but I
// guess not.
const char *G_RefreshNextMap(int gametype, qboolean forced) {
  ...
  n = thisLevel + 1;
  while (n != thisLevel) { // now cycle through the arena list and find the next
                           // map that matches the gametype we're in
    if (!level.arenas.infos[n] || n >= level.arenas.num) {
      if (loopingUp) { // this shouldn't happen, but if it does we have a null
                       // entry break in the arena file if this is the case just
                       // break out of the loop instead of sticking in an
                       // infinite loop
        break;
      }
      n = 0;
  ...
  if (desiredMap == thisLevel) { // If this is the only level for this game mode
                                 // or we just can't find a map for this game
                                 // mode, then nextmap will always restart.
...
//	trap->Print( "%i arenas parsed\n", level.arenas.num );
  ...
  // get all arenas from .arena files
  numFiles =
      trap->FS_GetFileList("scripts", ".arena", filelist, ARRAY_LEN(filelist));
...
int G_RemoveRandomBot(int team) {
  ...
  for (i = 0; i < sv_maxclients.integer; i++) {
    ...
    if (level.gametype == GT_SIEGE && team >= 0 &&
...
void G_CheckMinimumPlayers(void) {
  ...
  if (level.gametype >= GT_TEAM) {
          ...
          else if ((humanplayers+botplayers+humanplayers2+botplayers) >
  minplayers && botplayers)
          {
                  if ((humanplayers+botplayers) < (humanplayers2+botplayers2))
                  {
...
void G_CheckBotSpawn(void) {
  ...
  for (n = 0; n < BOT_SPAWN_QUEUE_DEPTH; n++) {
    ...
    if( level.gametype == GT_SINGLE_PLAYER ) {
            trap->GetUserinfo( botSpawnQueue[n].clientNum, userinfo,
    sizeof(userinfo) ); PlayerIntroSound( Info_ValueForKey (userinfo, "model")
    );
    }
    */
  }
...
qboolean G_BotConnect(int clientNum, qboolean restart) {
  ...
  Q_strncpyz(settings.personalityfile,
             Info_ValueForKey(userinfo, "personality"),
             sizeof(settings.personalityfile));
...
static void G_AddBot(const char *name, float skill, const char *team, int delay,
  ...
  char userinfo[MAX_INFO_STRING] = {0}, *botinfo = NULL, *key = NULL, *s = NULL,
       *botname = NULL, *model = NULL;

  // have the server allocate a client slot
  clientNum = trap->BotAllocateClient();
  if (clientNum == -1) {
    //		trap->Print( S_COLOR_RED "Unable to add bot.  All player slots are in
    //use.\n" ); 		trap->Print( S_COLOR_RED "Start server with more 'open'
    //slots.\n" );
...
void Svcmd_BotList_f(void) {
  ...
  for (i = 0; i < level.bots.num; i++) {
    Q_strncpyz(name, Info_ValueForKey(level.bots.infos[i], "name"),
               sizeof(name));
    if (!*name) {
      Q_strncpyz(name, "Padawan", sizeof(name));
    }
    Q_strncpyz(funname, Info_ValueForKey(level.bots.infos[i], "funname"),
               sizeof(funname));
    if (!*funname) {
      funname[0] = '\0';
    }
    Q_strncpyz(model, Info_ValueForKey(level.bots.infos[i], "model"),
               sizeof(model));
    if (!*model) {
      Q_strncpyz(model, DEFAULT_MODEL "/default", sizeof(model));
    }
    Q_strncpyz(personality,
               Info_ValueForKey(level.bots.infos[i], "personality"),
               sizeof(personality));
    if (!*personality) {
      Q_strncpyz(personality, "botfiles/kyle.jkb", sizeof(personality));
    }
    trap->Print("%-16s %-16s %-20s %-20s\n", name, model,
                COM_SkipPath(personality), funname);
  }
}
...
static void G_LoadBotsFromFile(char *filename) {
  ...
  level.bots.num += G_ParseInfos(buf, MAX_BOTS - level.bots.num,
