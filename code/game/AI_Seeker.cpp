...
//------------------------------------
void NPC_Seeker_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
                     const vec3_t point, int damage, int mod, int hitLoc) {
  if (!(self->svFlags & SVF_CUSTOM_GRAVITY)) {
    G_Damage(self, NULL, NULL, vec3_origin, (float *)vec3_origin, 999, 0,
             MOD_FALLING);
  }
...
void Seeker_Hunt(qboolean visible, qboolean advance) {
  ...
  speed = SEEKER_FORWARD_BASE_SPEED +
          SEEKER_FORWARD_MULTIPLIER * g_spskill->integer;
...
void Seeker_Fire(void) {
  ...
  // move a bit forward in the direction we shall shoot in so that the bolt
  // doesn't poke out the other side of the seeker
...
void Seeker_Ranged(qboolean visible, qboolean advance) {
  if (NPC->client->NPC_class != CLASS_BOBAFETT) {
    if (NPC->count > 0) {
      ...
      // out of ammo, so let it die...give it a push up so it can fall more and
      // blow up on impact
  ...
  for (int i = 0; i < numFound; i++) {
    ...
    if (ent->client->playerTeam == NPC->client->playerTeam ||
        ent->client->playerTeam ==
            TEAM_NEUTRAL) // don't attack same team or bots
    {
      continue;
    }
...
void Seeker_FollowPlayer(void) {
  ...
  float dis = DistanceHorizontalSquared(NPC->currentOrigin,
  ...
  if (dis < minDistSqr) {
    // generally circle the player closely till we take an enemy..this is our
    // target point
    if (NPC->client->NPC_class == CLASS_BOBAFETT) {
      pt[0] = g_entities[0].currentOrigin[0] +
              cos(level.time * 0.001f + NPC->random) * 250;
      pt[1] = g_entities[0].currentOrigin[1] +
              sin(level.time * 0.001f + NPC->random) * 250;
      ...
      pt[0] = g_entities[0].currentOrigin[0] +
              cos(level.time * 0.001f + NPC->random) * 56;
      pt[1] = g_entities[0].currentOrigin[1] +
              sin(level.time * 0.001f + NPC->random) * 56;
...
void NPC_BSSeeker_Default(void) {
  ...
  if (NPC->enemy && NPC->enemy->health && NPC->enemy->inuse) {
    if (NPC->client->NPC_class != CLASS_BOBAFETT &&
      ...
      // hacked to never take the player as an enemy, even if the player shoots
      // at it
      NPC->enemy = NULL;
    } else {
      Seeker_Attack();
