...
void NPC_BSSaberDroid_Patrol(void) { // FIXME: pick up on bodies of dead
                                     // buddies?
  ...
  } else if (!NPC->client->ps.weaponTime && TIMER_Done(NPC, "attackDelay") &&
             TIMER_Done(NPC, "inactiveDelay")) {
    if (NPC->client->ps.SaberActive()) {
      WP_DeactivateSaber(NPC, qfalse);
      NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_TURNOFF,
...
int SaberDroid_PowerLevelForSaberAnim(gentity_t *self) {
  ...
  case BOTH_A2_TR_BL:
    if (self->client->ps.torsoAnimTimer <= 200) { // end of anim
      return FORCE_LEVEL_0;
    } else if (PM_AnimLength(self->client->clientInfo.animFileIndex,
    ...
    break;
  case BOTH_A1_BL_TR:
    if (self->client->ps.torsoAnimTimer <= 300) { // end of anim
      return FORCE_LEVEL_0;
    } else if (PM_AnimLength(self->client->clientInfo.animFileIndex,
                             (animNumber_t)self->client->ps.legsAnim) -
                   self->client->ps.torsoAnimTimer <
    ...
    break;
  case BOTH_A1__L__R:
    if (self->client->ps.torsoAnimTimer <= 250) { // end of anim
      return FORCE_LEVEL_0;
    } else if (PM_AnimLength(self->client->clientInfo.animFileIndex,
                             (animNumber_t)self->client->ps.legsAnim) -
                   self->client->ps.torsoAnimTimer <
...
void NPC_SaberDroid_PickAttack(void) {
  ...
  NPC->client->ps.saberBlocking =
      saberMoveData[NPC->client->ps.saberMove].blocking;
  if (saberMoveData[NPC->client->ps.saberMove].trailLength > 0) {
    NPC->client->ps.SaberActivateTrail(
        saberMoveData[NPC->client->ps.saberMove]
            .trailLength); // saber trail lasts for 75ms...feel free to change
                           // this if you want it longer or shorter
  } else {
  ...
  NPC->client->ps.torsoAnim =
      NPC->client->ps.legsAnim; // need to do this because we have no anim split
                                // but saber code checks torsoAnim
...
void NPC_BSSaberDroid_Attack(void) {
  ...
  */
  ...
  if (!TIMER_Done(NPC, "taunting")) {
    ...
    shoot = qtrue;
    if (enemyDist <
        (NPC->maxs[0] + NPC->enemy->maxs[0] + 32) *
            (NPC->maxs[0] + NPC->enemy->maxs[0] + 32)) { // close enough
      doMove = qfalse;
    }
  ...
  if (doMove) { // doMove toward goal
    ...
    if (doMove) { // if we had to chase him, be sure to attack as soon as
                  // possible
                                                     ...
                                                     // need to do it again here
      {
        ...
        if (NPCInfo->rank > RANK_CREWMAN) {
          TIMER_Set(NPC, "attackDelay",
                    NPC->client->ps.weaponTime + Q_irand(0, 1000));
        } else {
          TIMER_Set(NPC, "attackDelay",
                    NPC->client->ps.weaponTime + Q_irand(0, 1000) +
                        (Q_irand(0, (3 - g_spskill->integer) * 2) * 500));
        }
      }
    }
  }
  ...
  {      // have an enemy
    if (!NPC->client->ps.SaberActive()) {
      ...
      if (NPC->client->ps.legsAnim == BOTH_TURNOFF ||
        ...
        NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_TURNON,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      }
    }
