...
void *B_Alloc(int size)
{
	...
	gi.Print("Allocations used: %i\nFree allocation slots: %i\n", used, free);

	i = 0;
#endif

	ptr = gi.BotGetMemoryGame(size);
...
#ifdef DEBUG
		gi.Print("WARNING: MAXIMUM B_ALLOC ALLOCATIONS EXCEEDED\n");
#endif
	}
...
#ifdef DEBUG
		gi.Print("WARNING: Freeing allocation which is not in the allocation structure\n");
#endif
	}

	gi.BotFreeMemoryGame(ptr);
#endif
}
...
void B_CleanupAlloc(void)
{
	...
	while (i < MAX_BALLOC)
	{
		if (BAllocList[i])
		{
			gi.BotFreeMemoryGame(BAllocList[i]);
			BAllocList[i] = NULL;
		}
...
int BotDoChat(bot_state_t *bs, char *section, int always)
{
	...
	if (gi.Cvar_VariableIntegerValue("se_language"))
...
int ReadChatGroups(bot_state_t *bs, char *buf)
{
	...
	{
		gi.Print(S_COLOR_RED "Error: Personality chat section exceeds max size\n");
		return 0;
	}
...
void BotUtilizePersonality(bot_state_t *bs)
{
	...
	len = gi.FS_Open(bs->settings.personalityfile, &f, FS_READ);

	failed = 0;

	if (!f)
	{
		gi.Print(S_COLOR_RED "Error: Specified personality not found\n");
	...
	if (len >= 131072)
	{
		gi.Print(S_COLOR_RED "Personality file exceeds maximum length\n");
		B_TempFree(131072); //buf
		return;
	}
	...
	{
		gi.Print(S_COLOR_RED "Personality file contains no GeneralBotInfo group\n");
