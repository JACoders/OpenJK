...
void R_Splash() {
  image_t *pImage =
      R_FindImageFile("menu/splash", qfalse, qfalse, qfalse, GL_CLAMP);
...
byte *RB_ReadPixels(int x, int y, int width, int height, size_t *offset,
  ...
  buffer = (byte *)R_Malloc(padwidth * height + *offset + packAlign - 1,
...
static void R_LevelShot(void) {
  ...
  buffer = (byte *)R_Malloc(LEVELSHOTSIZE * LEVELSHOTSIZE * 3 + 18,
                            TAG_TEMP_WORKSPACE, qfalse);
  Com_Memset(buffer, 0, 18);
  buffer[2] = 2; // uncompressed type
...
/************************************************************************************************
 * R_FogDistance_f * Console command to change the global fog opacity distance.
 *If you specify nothing on the * command line, it will display the current fog
 *opacity distance.  Specifying a float       * representing the world units
 *away the fog should be completely opaque will change the     * value. *
 *                                                                                              *
 * Input * none *
 *                                                                                              *
 * Output / Return * none *
...
void R_FogDistance_f(void) {
  ...
  if (ri.Cmd_Argc() != 2) {
    ri.Printf(PRINT_ALL,
              "R_FogDistance_f: Invalid number of arguments to set distance\n");
    return;
  }
...
/************************************************************************************************
 * R_FogColor_f * Console command to change the global fog color.  Specifying
 *nothing on the command will   * display the current global fog color.
 *Specifying a float R G B values between 0.0 and    * 1.0 will change the fog
 *color.                                                            *
 *                                                                                              *
 * Input * none *
 *                                                                                              *
...
void R_FogColor_f(void) {
  ...
  if (ri.Cmd_Argc() != 4) {
    ri.Printf(PRINT_ALL,
              "R_FogColor_f: Invalid number of arguments to set color\n");
    return;
  }
  ...
  tr.world->fogs[tr.world->globalFog].colorInt =
      ColorBytes4(atof(ri.Cmd_Argv(1)) * tr.identityLight,
...
void R_Register(void) {
  ...
  r_ext_compressed_textures =
      ri.Cvar_Get("r_ext_compress_textures", "1", CVAR_ARCHIVE_ND | CVAR_LATCH);
  ...
  r_ext_texture_env_add =
      ri.Cvar_Get("r_ext_texture_env_add", "1", CVAR_ARCHIVE_ND | CVAR_LATCH);
  r_ext_texture_filter_anisotropic =
      ri.Cvar_Get("r_ext_texture_filter_anisotropic", "16", CVAR_ARCHIVE_ND);
  ...
  r_DynamicGlowWidth =
      ri.Cvar_Get("r_DynamicGlowWidth", "320", CVAR_ARCHIVE_ND | CVAR_LATCH);
  r_DynamicGlowHeight =
      ri.Cvar_Get("r_DynamicGlowHeight", "240", CVAR_ARCHIVE_ND | CVAR_LATCH);
  ...
  r_znear = ri.Cvar_Get("r_znear", "4",
                        CVAR_ARCHIVE_ND); // if set any lower, you lose a lot of
                                          // precision in the distance
  ri.Cvar_CheckRange(r_znear, 0.001f, 10,
                     qfalse); // was qtrue in JA, is qfalse properly in ioq3
  ...
  */
  ...
  r_Ghoul2UnSqashAfterSmooth =
      ri.Cvar_Get("r_ghoul2unsquashaftersmooth", "1", 0);
...
#ifdef JK2_MODE
  sp_language = ri.Cvar_Get("sp_language", va("%d", SP_LANGUAGE_ENGLISH),
...
// need to do this hackery so ghoul2 doesn't crash the game because of ITS
// hackery...
//
...
float *get_tr_distortionAlpha(void) { return &tr_distortionAlpha; }

float *get_tr_distortionStretch(void) { return &tr_distortionStretch; }

qboolean *get_tr_distortionPrePost(void) { return &tr_distortionPrePost; }

qboolean *get_tr_distortionNegate(void) { return &tr_distortionNegate; }
...
void RE_SetRangedFog(float dist) {
  ...
  tr.rangedFog = dist;
  if (tr.rangedFog == 0.0f &&
      g_oldRangedFog) { // restore to previous state if applicable
...
@@@@@@@@@@@@@@@@@@@@@
*/
extern void R_LoadImage(const char *shortname, byte **pic, int *width,
                        int *height);
extern void R_WorldEffectCommand(const char *command);
extern qboolean R_inPVS(vec3_t p1, vec3_t p2);
extern void RE_GetModelBounds(refEntity_t *refEnt, vec3_t bounds1,
                              vec3_t bounds2);
extern void G2API_AnimateG2Models(CGhoul2Info_v &ghoul2, int AcurrentTime,
                                  CRagDollUpdateParams *params);
extern qboolean G2API_GetRagBonePos(CGhoul2Info_v &ghoul2, const char *boneName,
                                    vec3_t pos, vec3_t entAngles, vec3_t entPos,
                                    vec3_t entScale);
extern qboolean G2API_RagEffectorKick(CGhoul2Info_v &ghoul2,
                                      const char *boneName, vec3_t velocity);
extern qboolean G2API_RagForceSolve(CGhoul2Info_v &ghoul2, qboolean force);
extern qboolean G2API_SetBoneIKState(CGhoul2Info_v &ghoul2, int time,
                                     const char *boneName, int ikState,
                                     sharedSetBoneIKStateParams_t *params);
extern qboolean G2API_IKMove(CGhoul2Info_v &ghoul2, int time,
                             sharedIKMoveParams_t *params);
extern qboolean G2API_RagEffectorGoal(CGhoul2Info_v &ghoul2,
                                      const char *boneName, vec3_t pos);
extern qboolean G2API_RagPCJGradientSpeed(CGhoul2Info_v &ghoul2,
                                          const char *boneName,
                                          const float speed);
extern qboolean G2API_RagPCJConstraint(CGhoul2Info_v &ghoul2,
...
#ifdef JK2_MODE
unsigned int
AnyLanguage_ReadCharFromString_JK2(char **text,
