...
static void SV_EmitPacketEntities(clientSnapshot_t *from, clientSnapshot_t *to,
  ...
  newindex = 0;
  oldindex = 0;
  const int num2Send = to->num_entities >= svs.numSnapshotEntities
...
===============
*/
static void SV_AddEntToSnapshot(svEntity_t *svEnt, gentity_t *gEnt,
  ...
  if (sv.snapshotCounter & 1 &&
      eNums->numSnapshotEntities ==
          svs.numSnapshotEntities -
              1) { // we're full, and about to wrap around and stomp ents, so
                   // half the time send the first set without stomping.
    return;
  }
...
qboolean
SV_PlayerCanSeeEnt(gentity_t *ent,
                   int sightLevel) { // return true if this ent is in view
  ...
  if (VM_Call(CG_CAMERA_POS, viewOrg)) {
    if (VM_Call(CG_CAMERA_ANG, viewAngles)) {
      ...
      if ((ent->s.eFlags & EF_FORCE_VISIBLE)) { // no dist check on them?
      } else {
        if (entDist < 128.0f) { // can always see them if they're really close
          return qtrue;
        }

        if (entDist > range) { // too far away to see them
          return qfalse;
        }
      }
...
#ifndef JK2_MODE
  if (!portal) { // not if this if through a portal...???  James said to do
                 // this...
...
#ifndef JK2_MODE
    if (sightOn) { // force sight is on, sees through portals, so draw them
                   // always if in radius
      if (SV_PlayerCanSeeEnt(
              ent, frame->ps.forcePowerLevel[FP_SEE])) { // entity is visible
        SV_AddEntToSnapshot(svEnt, ent, eNums);
        continue;
      }
    }
...
static clientSnapshot_t *SV_BuildClientSnapshot(client_t *client) {
  ...
  // this stops the main client entity playerstate from being sent across, which
  // has the effect of breaking looping sounds for the main client. So I took it
  // out.
  /*	{
                  int clientNum; svEntity_t
     *svEnt; clientNum = frame->ps.clientNum; if ( clientNum < 0 || clientNum >=
     MAX_GENTITIES ) { Com_Error( ERR_DROP, "SV_SvEntityForGentity: bad gEnt" );
                  }
  ...
  */
  ...
  /*
  //was in here for debugging- print list of all entities in snapshot when you
  go over the limit if ( entityNumbers.numSnapshotEntities >= 256 )
  {
  ...
  {
          Com_Printf(S_COLOR_RED"%d snapshot entities!",
  entityNumbers.numSnapshotEntities );
  }
  else if ( entityNumbers.numSnapshotEntities >= 128 )
  {
          Com_Printf(S_COLOR_YELLOW"%d snapshot entities",
  entityNumbers.numSnapshotEntities );
  }
  */
