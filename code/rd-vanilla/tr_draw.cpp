...
void RE_StretchRaw(int x, int y, int w, int h, int cols, int rows,
                   const byte *data, int iClient, qboolean bDirty) {
  if (!tr.registered) {
    return;
  }
  ...
  int start, end;
  start = end =
      0; // only to stop compiler whining, don't need to be initialised
#endif
  ...
  // texture...
  //
  if (cols != tr.scratchImage[iClient]->width ||
      rows != tr.scratchImage[iClient]->height) {
    ...
    if (bDirty) // FIXME: some TA addition or other, not sure why, yet. Should
                // probably be true 99% of the time?
    {
...
extern byte *RB_ReadPixels(int x, int y, int width, int height, size_t *offset,
                           int *padlen);
void RE_GetScreenShot(byte *buffer, int w, int h) {
  ...
  source = RB_ReadPixels(0, 0, glConfig.vidWidth, glConfig.vidHeight, &offset,
                         &padlen);
  ...
  for (y = 0; y < h; y++) {
    for (x = 0; x < w; x++) {
      ...
      for (yy = 0; yy < 3; yy++) {
        for (xx = 0; xx < 4; xx++) {
          src = source + offset +
                3 * (glConfig.vidWidth * (int)((y * 3 + yy) * yScale) +
                     (int)((x * 4 + xx) * xScale));
          r += src[0];
          g += src[1];
          b += src[2];
...
// this is just a chunk of code from RE_TempRawImage_ReadFromFile() below,
// subroutinised so I can call it
    ...
    *piHeight = iLoadedHeight;
    ...
    for (int y = 0; y < *piHeight; y++) {
      for (int x = 0; x < *piWidth; x++) {
        ...
        for (float yy = (float)y * fYStep; yy < (float)(y + 1) * fYStep;
             yy += 1) {
          for (float xx = (float)x * fXStep; xx < (float)(x + 1) * fXStep;
               xx += 1) {
            byte *pbSrc =
                pbLoadedPic + 4 * (((int)yy * iLoadedWidth) + (int)xx);
...
// this is so the server (or anyone else) can get access to raw pixels if they
// really need to,
//	currently it's only used by the server so that savegames can embed a
//graphic in the auto-save files 	(which can't do a screenshot since they're
//saved out before the level is drawn).
//
// by default, the pic will be returned as the original dims, but if
// pbReSampleBuffer != NULL then it's assumed to
//	be a big enough buffer to hold the resampled image, which also means
//that the width and height params are read as 	inputs (as well as still being
//inherently outputs) and the pic is scaled to that size, and to that buffer.
//
// the return value is either NULL, or a pointer to the pixels to use (which may
// be either the pbReSampleBuffer param,
//	or the local ptr below).
//
// In either case, you MUST call the free-up function afterwards (
// RE_TempRawImage_CleanUp() ) to get rid of any temp
//	memory after you've finished with the pic.
//
// Note: ALWAYS use the return value if != NULL, even if you passed in a
// declared resample buffer. This is because the
//	resample will get skipped if the values you want are the same size as
//the pic that it loaded, so it'll return a 	different buffer.
//
// the vertflip param is used for those functions that expect things in OpenGL's
// upside-down pixel-read format (sigh)
//
// (not brilliantly fast, but it's only used for weird stuff anyway)
//
byte *pbLoadedPic = NULL;

byte *RE_TempRawImage_ReadFromFile(const char *psLocalFilename, int *piWidth,
  ...
  if (pbReturn && qbVertFlip) {
    ...
    unsigned int *pDstLine =
        (unsigned int *)pbReturn +
        (*piHeight * *piWidth); // *4 done by compiler (longs)
...
// leave the UV stuff in for now as comments in case I ever need to do some
// sneaky stuff, but for now...
//
static void RE_Blit(float fX0, float fY0, float fX1, float fY1, float fX2,
                    float fY2, float fX3, float fY3,
                    // float fU0, float fV0, float fU1, float fV1, float fU2,
                    // float fV2, float fU3, float fV3,
  ...
  qglBegin(GL_QUADS);
  {
    // TL...
    //
    //		qglTexCoord2f( fU0 / (float)pImage->width,  fV0 /
    //(float)pImage->height );
    qglTexCoord2f(0, 0);
    qglVertex2f(fX0, fY0);
    ...
    // TR...
    //
    //		qglTexCoord2f( fU1 / (float)pImage->width,  fV1 /
    //(float)pImage->height );
    qglTexCoord2f(1, 0);
    qglVertex2f(fX1, fY1);
    ...
    // BR...
    //
    //		qglTexCoord2f( fU2 / (float)pImage->width,  fV2 /
    //(float)pImage->height );
    qglTexCoord2f(1, 1);
    qglVertex2f(fX2, fY2);
    ...
    // BL...
    //
    //		qglTexCoord2f( fU3 / (float)pImage->width,  fV3 /
    //(float)pImage->height );
    qglTexCoord2f(0, 1);
    qglVertex2f(fX3, fY3);
  }
...
// Draw the dissolve pic to the screen, over the top of what's already been
// rendered.
//
// return = qtrue while still processing, for those interested...
//
#define iSAFETY_SPRITE_OVERLAP                                                 \
  2 // #pixels to overlap blit region by, in case some drivers leave onscreen
    // seams
qboolean RE_ProcessDissolve(void) {
  if (Dissolve.iStartTime) {
    if (Dissolve.bTouchNeeded) {
      ...
      //
      //	The problem is, that if I call RE_InitDissolve() then call
      //RestartMusic, then by the time the music 	has loaded in if it took longer
      //than one second the dissolve would think that it had finished, 	even if
      //it had never actually drawn up. However, if I called RE_InitDissolve()
      //AFTER the music had 	restarted, then the music would stutter on slow
      //video cards or CPUs while I did the binding/resampling.
      //
      // This way, I restart the millisecond counter the first time we actually
      // get as far as rendering, which
    ...
    int iDissolvePercentage =
        ((ri.Milliseconds() - Dissolve.iStartTime) * 100) /
        (1000.0f * fDISSOLVE_SECONDS);

    //		ri.Printf(PRINT_ALL,"iDissolvePercentage
    //%d\n",iDissolvePercentage);
    ...
    if (iDissolvePercentage <= 100) {
      ...
      case eDISSOLVE_RT_TO_LT: {
        float fXboundary =
            (float)Dissolve.iWidth -
            (((float)(Dissolve.iWidth + Dissolve.pDissolve->width) *
              (float)iDissolvePercentage) /
             100.0f);
        ...
        RE_Blit(x0, y0, x1, y1, x2, y2, x3, y3, Dissolve.pDissolve,
                GLS_DEPTHMASK_TRUE | GLS_SRCBLEND_ZERO | GLS_DSTBLEND_ONE |
                    GLS_ATEST_LT_80);

        // blit a blank thing over the area the old screen is to be displayed on
        // to enable screen-writing... (to the left of fXboundary)
        //
        x0 = 0.0f;
        y0 = 0.0f;
        ...
        x3 = x0;
        y3 = y2;
        RE_Blit(x0, y0, x1, y1, x2, y2, x3, y3, Dissolve.pBlack,
                GLS_DEPTHMASK_TRUE | GLS_SRCBLEND_ZERO | GLS_DSTBLEND_ONE);
      } break;

      case eDISSOLVE_LT_TO_RT: {
        float fXboundary =
            (((float)(Dissolve.iWidth + (2 * Dissolve.pDissolve->width)) *
              (float)iDissolvePercentage) /
             100.0f) -
            Dissolve.pDissolve->width;
        ...
        // blit a blank thing over the area the old screen is to be displayed on
        // to enable screen-writing... (to the right of fXboundary)
        //
        x0 = fXScaleFactor * ((fXboundary + Dissolve.pDissolve->width) -
                              iSAFETY_SPRITE_OVERLAP);
        y0 = 0.0f;
      ...
      case eDISSOLVE_TP_TO_BT: {
        float fYboundary =
            (((float)(Dissolve.iHeight + (2 * Dissolve.pDissolve->width)) *
              (float)iDissolvePercentage) /
             100.0f) -
            Dissolve.pDissolve->width;
        ...
        // blit a blank thing over the area the old screen is to be displayed on
        // to enable screen-writing... (underneath fYboundary)
        //
        x0 = 0.0f;
        y0 = fYScaleFactor * ((fYboundary + Dissolve.pDissolve->width) -
                              iSAFETY_SPRITE_OVERLAP);
      ...
      case eDISSOLVE_BT_TO_TP: {
        float fYboundary =
            Dissolve.iHeight -
            (((float)(Dissolve.iHeight + Dissolve.pDissolve->width) *
              (float)iDissolvePercentage) /
             100.0f);
        ...
        // blit a blank thing over the area the old screen is to be displayed on
        // to enable screen-writing... (above fYboundary)
        //
        x0 = 0.0f;
        y0 = 0.0f;
        x1 = fXScaleFactor * Dissolve.iWidth;
      ...
      case eDISSOLVE_CIRCULAR_IN: {
        float fDiagZoom =
            (((float)Dissolve.iWidth * 0.8) * (100 - iDissolvePercentage)) /
            100.0f;
      ...
      case eDISSOLVE_CIRCULAR_OUT: {
        float fDiagZoom =
            (((float)Dissolve.iWidth * 0.8) * iDissolvePercentage) / 100.0f;
        ...
        RE_Blit(x0, y0, x1, y1, x2, y2, x3, y3, Dissolve.pDissolve,
                GLS_DEPTHMASK_TRUE | GLS_SRCBLEND_ZERO | GLS_DSTBLEND_ONE |
                    GLS_ATEST_LT_80);
        // now blit the 4 black squares around it to mask off the rest of the
        // screen...
        //
        // LHS, top to bottom...
        //
        RE_Blit(0, 0,                           // x0,y0
                ...
                x0 + iSAFETY_SPRITE_OVERLAP,
                (fYScaleFactor * Dissolve.iHeight),    // x2,y2
        ...
        RE_Blit(x1 - iSAFETY_SPRITE_OVERLAP, 0,       // x0,y0
                ...
                (fXScaleFactor * Dissolve.iWidth),
                (fYScaleFactor * Dissolve.iHeight), // x2,y2
                x1 - iSAFETY_SPRITE_OVERLAP,
                (fYScaleFactor * Dissolve.iHeight), // x3,y3,
                Dissolve.pBlack,
                GLS_DEPTHMASK_TRUE | GLS_SRCBLEND_ZERO | GLS_DSTBLEND_ONE);
        ...
        //
        RE_Blit(
            x0 - iSAFETY_SPRITE_OVERLAP, 0,                           // x0,y0
        ...
        //
        RE_Blit(
            x0 - iSAFETY_SPRITE_OVERLAP, y3 - iSAFETY_SPRITE_OVERLAP, // x0,y0
      ...
      if (iDissolvePercentage <= 100) {
        ...
        RE_Blit(x0, y0, x1, y1, x2, y2, x3, y3, Dissolve.pImage,
                GLS_DEPTHFUNC_EQUAL);
      }
    }
...
qboolean RE_InitDissolve(qboolean bForceCircularExtroWipe) {
  ...
  if ( // Dissolve.iStartTime == 0	// no point in interruping an existing
       // one
       //&&
      tr.registered == qtrue // ... stops it crashing during first cinematic
                             // before the menus... :-)
  ) {
    ...
    if (pBuffer) {
      // read current screen image...  (GL_RGBA should work even on 3DFX in that
      // the RGB parts will be valid at least)
      //
      qglReadPixels(0, 0, glConfig.vidWidth, glConfig.vidHeight, GL_RGBA,
                    GL_UNSIGNED_BYTE, pBuffer);
      //
      // now expand the pic over the top of itself so that it has a stride value
      // of {PowerOf2(glConfig.vidWidth)}
      ...
      //
      // ok, now we've got the screen image in the top left of the power-of-2
      // texture square,
      //	but of course the damn thing's upside down (thanks, GL), so
      //invert it, but only within 	the picture pixels, NOT the upload texture as
      //a whole...
      //
      byte *pbSwapLineBuffer =
          (byte *)R_Malloc(iCopyBytes, TAG_TEMP_WORKSPACE, qfalse);
      pbSrc = &pBuffer[0];
      ...
      //
      // Now, in case of busted drivers, 3DFX cards, etc etc we stomp the alphas
      // to 255...
      //
      byte *pPix = pBuffer;
      ...
      // work out what res we're capable of storing/xfading this "screen
      // sprite"...
      //
      ...
      // alloc resample buffer...  (note slight optimisation to avoid spurious
      // alloc)
      //
      byte *pbReSampleBuffer =
          (iPow2VidWidth == Dissolve.iUploadWidth &&
           iPow2VidHeight == Dissolve.iUploadHeight)
              ? NULL
              : (byte *)R_Malloc(iPow2VidWidth * iPow2VidHeight * 4,
                                 TAG_TEMP_WORKSPACE, qfalse);

      // re-sample screen...
      //
      byte *pbScreenSprite =
          RE_ReSample(pBuffer,        // byte *pbLoadedPic
      ...
      // special tweak, although this code is normally called just before client
      // spawns into world (and
      //	is therefore pretty much immune to precache issues) I also need
      //to make sure that the inverse 	iris graphic is loaded so for the special
      //case of doing a circular wipe at the end of the last 	level doesn't stall
      //on loading the image. So I'll load it here anyway - to prime the image -
      //	then allow the random wiper to overwrite the ptr if needed. This
      //way the end of level call 	will be instant.  Downside: every level has
      //one extra 256x256 texture.
      // Trying to decipher these comments - looks like no problem taking this
      // out. I want the RAM.
      {
        Dissolve.pDissolve =
      ...
      //
      if (Dissolve
              .pDissolve) // test if image was found, if not, don't do dissolves
      {
        Dissolve.iStartTime = ri.Milliseconds(); // gets overwritten first time,
                                                 // but MUST be set to NZ
