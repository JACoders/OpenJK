...
void QDECL PrintMsg( gentity_t *ent, const char *fmt, ... ) {
        ...
        trap->SendServerCommand ( ( (ent == NULL) ? -1 : ent-g_entities ),
va("print \"%s\"", msg ));
}
*/
// Printing messages to players via this method is no longer done, StringEd
...
qboolean OnSameTeam(gentity_t *ent1, gentity_t *ent2) {
  ...
  if (ent1->client->sess.sessionTeam == TEAM_FREE &&
      ent2->client->sess.sessionTeam == TEAM_FREE && ent1->s.eType == ET_NPC &&
...
gentity_t *SelectRandomTeamSpawnPoint(int teamstate, team_t team,
  ...
  if (level.gametype == GT_SIEGE) {
    ...
    mustBeEnabled =
        qtrue; // siege spawn points need to be "enabled" to be used (because
               // multiple spawnpoint sets can be placed at once)
  } else {
  ...
  while ((spot = G_Find(spot, FOFS(classname), classname)) != NULL) {
    ...
    if (mustBeEnabled &&
        !spot->genericValue1) { // siege point that's not enabled, can't use it
      continue;
    }
  ...
  if (level.gametype == GT_SIEGE && siegeClass >= 0 &&
      bgSiegeClasses[siegeClass]
          .name[0]) { // out of the spots found, see if any have an idealclass
                      // to match our class name
    ...
    while (i < count) {
      if (spots[i] && spots[i]->idealclass && spots[i]->idealclass[0] &&
          !Q_stricmp(
              spots[i]->idealclass,
              bgSiegeClasses[siegeClass]
                  .name)) { // this spot's idealclass matches the class name
...
============
*/
gentity_t *SelectSiegeSpawnPoint(int siegeClass, team_t team, int teamstate,
                                 vec3_t origin, vec3_t angles, qboolean isbot) {
  gentity_t *spot;
...
void TeamplayInfoMessage(gentity_t *ent) {
  ...
  for (i = 0, cnt = 0; i < sv_maxclients.integer && cnt < TEAM_MAXOVERLAY;
       i++) {
  ...
  for (i = 0, cnt = 0; i < sv_maxclients.integer && cnt < TEAM_MAXOVERLAY;
    ...
    if (player->inuse && player->client->sess.sessionTeam == team) {
      ...
      if (player->client->tempSpectate >= level.time) {
        ...
        Com_sprintf(entry, sizeof(entry), " %i %i %i %i %i %i", i, 0, h, a, 0,
                    0);
      } else {
