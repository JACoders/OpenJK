...
void CG_PlayerAnimSounds(int animFileIndex, qboolean torso, int oldFrame,
...
static void CG_RegisterCustomSounds(clientInfo_t *ci, int iSoundEntryBase,
  ...
  for (int i = 0; i < iTableEntries; i++) {
    ...
    if (!s) {
      break; // fairly pointless code in original, since there are no NULL's in
             // the table, but wtf...
    }

    sfxHandle_t hSFX =
        cgi_S_RegisterSound(va("sound/chars/%s/misc/%s", psDir, s + 1));
    if (hSFX == 0) {
...
void CG_TryPlayCustomSound(vec3_t origin, int entityNum, soundChannel_t channel,
                           const char *soundName, int customSoundSet) {
...
void ParseAnimationSndBlock(const char *asb_filename, animsounds_t *animSounds,
  ...
  while (1) {
    if (*i >= MAX_ANIM_SOUNDS) {
      CG_Error(
          "ParseAnimationSndBlock:  animation number >= MAX_ANIM_SOUNDS(%i)",
          MAX_ANIM_SOUNDS);
    }
    ...
    if (animNum == -1) { // Unrecognized ANIM ENUM name, or we're skipping this
      ...
      Com_Printf(S_COLOR_YELLOW
                 "WARNING: Unknown token %s in animSound file %s\n",
                 token, asb_filename);
      continue;
    }

    if (animations[animNum].numFrames == 0) { // we don't use this anim
      // Com_Printf(S_COLOR_YELLOW"WARNING: %s animsounds.cfg: anim %s not used
      // by this model\n", filename, token);
    ...
    //		instead of strings, unfortunately, if these sounds were
    //previously 		registered, we cannot be guaranteed sequential indices.  Thus
    //an array
    if (lowestVal && highestVal) {
      for (n = lowestVal, num = 0;
           n <= highestVal && num < MAX_RANDOM_ANIMSOUNDS; n++, num++) {
        animSounds->soundIndex[num] =
            G_SoundIndex(va(soundString, n)); // cgi_S_RegisterSound
      }
      animSounds->numRandomAnimSounds = num - 1;
    } else {
      animSounds->soundIndex[0] =
          G_SoundIndex(va(soundString)); // cgi_S_RegisterSound
#ifndef FINAL_BUILD
      if (!animSounds->soundIndex[0]) { // couldn't register it - file not found
        Com_Printf(S_COLOR_RED "ParseAnimationSndBlock: sound %s does not "
                               "exist (animsound.cfg %s)!\n",
...
void CG_ParseAnimationSndFile(const char *as_filename, int animFileIndex) {
  ...
  if (level.knownAnimFileSets[animFileIndex]
          .soundsCached) { // already cached this one
    return;
  }
  ...
  Com_sprintf(sfilename, sizeof(sfilename), "models/players/%s/animsounds.cfg",
              as_filename);
  ...
  while (1) {
    ...
    {
      ParseAnimationSndBlock(as_filename, torsoAnimSnds, animations, &upper_i,
                             &text_p);
    }

    else if (!Q_stricmp(token, "LOWERSOUNDS")) // A batch of lower sounds
    {
      ParseAnimationSndBlock(as_filename, legsAnimSnds, animations, &lower_i,
                             &text_p);
    }
  }
  COM_EndParseSession();
...
===============
*/
void CG_SetLerpFrameAnimation(clientInfo_t *ci, lerpFrame_t *lf,
                              int newAnimation) {
  animation_t *anim;
...
===============
*/
void CG_PlayerAnimation(centity_t *cent, int *legsOld, int *legs,
  ...
  *legsBackLerp = cent->pe.legs.backlerp;
  ...
  if (newLegsFrame) {
    if (ValidAnimFileIndex(ci->animFileIndex)) {
      CG_PlayerAnimSounds(ci->animFileIndex, qfalse, cent->pe.legs.frame,
  ...
  *torsoBackLerp = cent->pe.torso.backlerp;
  ...
  if (newTorsoFrame) {
    if (ValidAnimFileIndex(ci->animFileIndex)) {
      CG_PlayerAnimSounds(ci->animFileIndex, qtrue, cent->pe.torso.frame,
...
/*
void CG_PlayerAnimSounds( int animFileIndex, qboolean torso, int oldFrame, int
frame, const vec3_t org, int entNum )
...
void CG_PlayerAnimSounds(int animFileIndex, qboolean torso, int oldFrame,
  ...
  qboolean playSound = qfalse, inSameAnim = qfalse, loopAnim = qfalse,
  ...
  if (fabs((double)oldFrame - frame) > 1 &&
      cg_reliableAnimSounds
          .integer) { // given a range, see if keyFrame falls in that range
    int oldAnim, anim;
    if (torso) {
      if (cg_reliableAnimSounds.integer > 1) { // more precise, slower
        oldAnim = PM_TorsoAnimForFrame(&g_entities[entNum], oldFrame);
        anim = PM_TorsoAnimForFrame(&g_entities[entNum], frame);
      } else { // less precise, but faster
  ...
  for (i = 0; i < MAX_ANIM_SOUNDS; ++i) {
    ...
    match = qfalse;
    if (animSounds[i].keyFrame == frame) { // exact match
      match = qtrue;
    } else if (fabs((double)oldFrame - frame) > 1 &&
               cg_reliableAnimSounds.integer) { // given a range, see if
                                                // keyFrame falls in that range
      if (inSameAnim) { // if changed anims altogether, sorry, the sound is lost
        if (fabs((double)oldFrame - animSounds[i].keyFrame) <= 3 ||
            fabs((double)frame - animSounds[i].keyFrame) <=
          ...
          if (animBackward) { // animation plays backwards
            if (oldFrame > animSounds[i].keyFrame &&
              ...
              match = qtrue;
                                   ...
                                   // see if we looped
              if (animSounds[i].keyFrame >= firstFrame &&
                  animSounds[i].keyFrame <
    ...
    if (match) {
      ...
      holdSnd = animSounds[i]
                    .soundIndex[Q_irand(0, animSounds[i].numRandomAnimSounds)];
...
void CGG2_AnimSounds(centity_t *cent) {
  ...
  if (ValidAnimFileIndex(cent->gent->client->clientInfo.animFileIndex)) {
    ...
    if (curFrame != cent->gent->client->renderInfo.legsFrame) {
      CG_PlayerAnimSounds(cent->gent->client->clientInfo.animFileIndex, qfalse,
    ...
    if (curFrame != cent->gent->client->renderInfo.torsoFrame) {
      CG_PlayerAnimSounds(cent->gent->client->clientInfo.animFileIndex, qtrue,
...
==================
*/
void CG_UpdateAngleClamp(float destAngle, float clampMin, float clampMax,
                         float angleSpeed, float *curAngle, float normalAngle) {
  float swing;
  float move;
  float scale;
  float actualSpeed;
...
==================
*/
void CG_SwingAngles(float destAngle, float swingTolMin, float swingTolMax,
                    float clampMin, float clampMax, float angleSpeed,
                    float *curAngle, qboolean *turning) {
  float swing;
  float move;
  float scale;
...
qboolean CG_PlayerLegsYawFromMovement(centity_t *cent, const vec3_t velocity,
                                      float *yaw, float fwdAngle,
...
void CG_G2ClientSpineAngles(centity_t *cent, vec3_t viewAngles,
  ...
  { // FIXME: no need to do this if legs and torso on are same frame
    ...
    for (int ang = 0; ang < 3; ang++) {
      viewAngles[ang] = AngleNormalize180(viewAngles[ang] -
                                          AngleNormalize180(motionAngles[ang]));
    }
  }
...
void CG_G2ClientNeckAngles(centity_t *cent, const vec3_t lookAngles,
                           vec3_t headAngles, vec3_t neckAngles,
                           vec3_t thoracicAngles, vec3_t headClampMinAngles,
...
void CG_UpdateLookAngles(centity_t *cent, vec3_t lookAngles, float lookSpeed,
                         float minPitch, float maxPitch, float minYaw,
    ...
    if (cent->gent->client && cent->gent->client->NPC_class == CLASS_ATST) {
      ...
      if (cent->gent->client->ps.eFlags & EF_FORCE_GRIPPED &&
        ...
        angles[PITCH] = AngleNormalize180(
            DotProduct(cent->gent->client->ps.velocity, centFwd) / 2);
        if (angles[PITCH] > 90) {
          angles[PITCH] = 90;
        ...
        angles[ROLL] = AngleNormalize180(
            DotProduct(cent->gent->client->ps.velocity, centRt) / 10);
        if (angles[ROLL] > 90) {
          angles[ROLL] = 90;
...
static qboolean CG_PlayerShadow(centity_t *const cent,
  ...
  *shadowPlane = 0;
  ...
  if (cent->gent->client->NPC_class == CLASS_ATST) {
    ...
    sideOrigin[2] += 30; // fudge up a bit for coplaner
    bShadowed =
        (qboolean)(_PlayerShadow(sideOrigin, 0, shadowPlane, 28) || bShadowed);

    bShadowed =
        (qboolean)(_PlayerShadow(cent->lerpOrigin, cent->pe.legs.yawAngle,
                                 shadowPlane, 64) ||
                   bShadowed);
    return bShadowed;
  } else {
    return _PlayerShadow(cent->lerpOrigin, cent->pe.legs.yawAngle, shadowPlane,
                         16);
  }
}

void _PlayerSplash(const vec3_t origin, const vec3_t velocity,
  ...
  FX_AddOrientedParticle(end, trace.plane.normal, NULL, NULL, 6.0f,
...
void CG_LightningBolt(centity_t *cent, vec3_t origin) {
  // FIXME:  This sound also plays when the weapon first fires which causes
  // little sputtering sounds..not exactly cool Must be currently firing
  if (!(cent->currentState.eFlags & EF_FIRING))
    return;

  // Must be a durational weapon
  //	if ( cent->currentState.weapon == WP_DEMP2 && cent->currentState.eFlags
  //& EF_ALT_FIRING ) 	{ /*nothing*/ } 	else
  { return; }
  ...
  /*	trace_t		trace;
          ...
          // for lightning weapons coming from the player, it had better hit the
  crosshairs or else.. if ( cent->gent->s.number )
          {
          ...
          VectorMA( org, weaponData[cent->currentState.weapon].range, forward,
  end );

          CG_Trace( &trace, org, vec3_origin, vec3_origin, end,
  cent->currentState.number, MASK_SHOT ); traceEnt = &g_entities[
  trace.entityNum ];

          // Make sparking be a bit less frame-rate dependent..also never add
  sparking when we hit a surface with a NOIMPACT flag if ( cent->gent->fx_time <
  cg.time && !(trace.surfaceFlags & SURF_NOIMPACT ))
          {
                  spark = qtrue;
                  cent->gent->fx_time = cg.time + Q_flrand(0.0f, 1.0f) * 100 +
  100;
          }

          // Don't draw certain kinds of impacts when it hits a player and
  such..or when we hit a surface with a NOIMPACT flag if ( (traceEnt->takedamage
  && traceEnt->client) || (trace.surfaceFlags & SURF_NOIMPACT) )
          {
                  impact = qfalse;
          }
  ...
  extern void FX_DEMP2_AltBeam( vec3_t start, vec3_t end, vec3_t normal,
  //qboolean spark, vec3_t targ1, vec3_t targ2 );

                  // Move the beam back a bit to help cover up the poly edges on
  the fire beam
  //		VectorMA( origin, -4, forward, org );
  //		FIXME:  Looks and works like ASS, so don't let people see it
  until it improves FX_DEMP2_AltBeam( origin, trace.endpos, trace.plane.normal,
  cent->gent->pos1, cent->gent->pos2 ); break;
...
static void CG_ForceElectrocution(centity_t *cent, const vec3_t origin,
  ...
  if (tr.fraction < 1.0f || Q_flrand(0.0f, 1.0f) > 0.94f) {
    FX_AddElectricity(
        fxOrg, tr.endpos, 1.5f, 4.0f, 0.0f, 1.0f, 0.5f, 0.0f, rgb, rgb, 0.0f,
        5.5f, Q_flrand(0.0f, 1.0f) * 50 + 100, cgs.media.boltShader,
...
void CG_AddRefEntityWithPowerups(refEntity_t *ent, int powerups,
  ...
  if (powerups & (1 << PW_GALAK_SHIELD)) {
    ...
    // Setting up the 2d control points, these get swept around to make a 3D
    // lathed model
    ...
    gi.G2API_GetBoltMatrix(cent->gent->ghoul2, gent->playerModel,
                           gent->genericBolt1, &boltMatrix, angles,
                           cent->lerpOrigin, cg.time, cgs.model_draw,
                           cent->currentState.modelScale);
    gi.G2API_GiveMeVectorFromMatrix(
        boltMatrix, ORIGIN, tent.origin); // pass in the emitter origin here

    tent.endTime =
        gent->fx_time +
        1000; // if you want the shell to build around the guy, pass in a time
              // that is 1000ms after the start of the turn-on-effect
...
qboolean CG_G2PlayerHeadAnims(centity_t *cent) {
  ...
  if (cent->gent->health <=
    ...
    {             // not talking
      ...
      {                                                 // but should we start ?
        if (cent->gent->client->facial_aux < cg.time) { // yes
          anim = FACE_ALERT;
          // set aux timer
...
int CG_PlayerHeadExtension(centity_t *cent, refEntity_t *head) {
  ...
  // if we have facial texture extensions, go get the sound override and add it
  // to the face skin if we aren't talking, then it will be 0
  if (ci->extensions &&
      (gi.VoiceVolume[cent->gent->s.clientNum] >
       0)) { // FIXME: When talking, look at talkTarget, if any
    // ALSO: When talking, add a head bob/movement on syllables - when
    // gi.VoiceVolume[] changes drastically

    if (cent->gent->health <=
        0) { // Dead people close their eyes and don't make faces!  They also
    ...
    head->customSkin =
        ci->headSkin + 4 + gi.VoiceVolume[cent->gent->s.clientNum];
    // reset the frown and blink timers
  } else
      // ok, we have facial extensions, but we aren't speaking. Lets decide if
      // we need to frown or blink
      if (ci->extensions) {
    int add_in = 0;

    // deal with blink first

    // Dead people close their eyes and don't make faces!  They also tell no
    // tales...  BUM BUM BAHHHHHHH!
    ...
    if (!cent->gent->client->facial_blink) { // reset blink timer
    ...
    if (!add_in) {
      if (gi.VoiceVolume[cent->gent->s.clientNum] ==
          -1) { // then we're talking and don't want to use blinking normal
                // frames, force open eyes.
        add_in = 0;
        // reset blink timer
        cent->gent->client->facial_blink = cg.time + Q_flrand(3000.0, 5000.0);
      }
  ...
  if (cent->currentState.weapon == WP_SABER) {
    if (cent->gent && cent->gent->client &&
        (cent->currentState.saberInFlight ||
         !cent->gent->client->ps
              .saberActive /*!cent->currentState.saberActive*/)) {
      return;
    }
...
void CG_DoSaber(vec3_t origin, vec3_t dir, float length, float lengthMax,
  ...
  // half!!  or something...
  cgi_R_AddLightToScene(mid, (length * 2.0f) + (Q_flrand(0.0f, 1.0f) * 8.0f),
                        rgb[0], rgb[1], rgb[2]);
...
void CG_CheckSaberInWater(centity_t *cent, centity_t *scent, int modelIndex,
  ...
  if ((contents & CONTENTS_WATER) ||
      (contents & CONTENTS_SLIME)) { // still in water
...
void CG_AddSaberBlade(centity_t *cent, centity_t *scent, refEntity_t *saber,
  ...
  vec3_t org_, end, // org_future,
      axis_[3] = {{0, 0, 0},
                  {0, 0, 0},
                  {0, 0, 0}}; //, axis_future[3]={{0,0,0}, {0,0,0}, {0,0,0}};
  ...
  */
  ...
  if (1) {
    ...
    gi.G2API_GiveMeVectorFromMatrix(
        ...
        axis_[0]); // was NEGATIVE_Y, but the md3->glm exporter screws up this
                   // tag somethin' awful
  ...
  */
  ...
  if (cent->gent->client->ps.saberLength <
      cent->gent->client->ps.saberLengthMax) {
    if (cent->gent->client->ps.saberLength <
        cent->gent->client->ps.saberLengthMax - 8) {
  ...
  if (cent->currentState.saberInFlight) {
    ...
    gi.trace(&trace, cent->lerpOrigin, NULL, NULL,
             cent->gent->client->renderInfo.muzzlePoint,
             cent->currentState.number, CONTENTS_SOLID, G2_NOCOLLIDE, 0);
  }
  ...
  if (trace.fraction < 1.0f) {
              ...
              // hit a corner, blade is still 8 longer than hit
    {
      if (i) { // tracing from end to base
        ...
        // cgi_CM_BoxTrace( &trace, end, org_, NULL, NULL, 0, MASK_SHOT );
        gi.trace(&trace, org_, NULL, NULL, end, ENTITYNUM_NONE,
                 MASK_SOLID | CONTENTS_WATER | CONTENTS_SLIME, G2_NOCOLLIDE, 0);
      }
      ...
      if (trace.fraction < 1.0f) {
        if ((trace.contents & CONTENTS_WATER) ||
          ...
          */
          ...
          if (client->saberTrail.haveOldPos[i]) {
            if (trace.entityNum ==
                ENTITYNUM_WORLD) { // only put marks on architecture
              // Let's do some cool burn/glowing mark bits!!!
              CG_CreateSaberMarks(client->saberTrail.oldPos[i], trace.endpos,
                                  trace.plane.normal);
              ...
              if (cg.time -
                ...
                cgi_S_StartSound(cent->lerpOrigin, cent->currentState.clientNum,
                                 CHAN_ITEM,
                                 cgi_S_RegisterSound(va(
                                     "sound/weapons/saber/saberhitwall%d.wav",
                                     Q_irand(1, 3))));
              }
            }
          } else {
        ...
        if (!i) {
          // Now that we don't let the blade go through walls, we need to
          // shorten the blade when it hits one
          cent->gent->client->ps.saberLength =
              cent->gent->client->ps.saberLength *
        ...
        if (client->saberTrail.haveOldPos[i]) {
          ...
          // client->saberTrail.oldPos[i], client->saberTrail.oldNormal[i],
...
#define SABER_TRAIL_TIME 40.0f
  ...
  CG_DoSaber(org_, axis_[0], length, client->ps.saberLengthMax,
             client->ps.saberColor, renderfx);
}
...
void CG_Player(centity_t *cent) {
  ...
  if ((in_camera) && !(cent->currentState.eFlags &
                       EF_NPC)) // If player in camera then no need for shadow
  {
    return;
  }
  ...
  if (cent->currentState.vehicleModel !=
      0) { // Using an alternate (md3 for now) model
    refEntity_t ent;
    ...
    ent.renderfx = 0;
    if (cent->currentState.number == cg.snap->ps.clientNum) { // player
      if (!cg.renderingThirdPerson) {
    ...
    */
    ...
    cent->gent->client->renderInfo.muzzleDir );
            cent->gent->client->renderInfo.mPCalcTime = cg.time;
            gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->playerModel,
    cent->gent->headBolt, &boltMatrix, tempAngles, ent.origin, cg.time,
    cgs.model_draw, cent->currentState.modelScale);
            // update where we think the head is, and where the eyes point
            gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN,
    cent->gent->client->renderInfo.eyePoint);
            gi.G2API_GiveMeVectorFromMatrix(boltMatrix,	NEGATIVE_Y,
    tempAxis);//was POSITIVE_X vectoangles( tempAxis,
    cent->gent->client->renderInfo.eyeAngles );
    }
    else
    */
    {
      ...
      AngleVectors(cent->lerpAngles, cent->gent->client->renderInfo.muzzleDir,
                   NULL, NULL);
      cent->gent->client->renderInfo.mPCalcTime = cg.time;
  ...
  */
  ...
  //	if (1)
  {
    ...
    if (!cg.renderingThirdPerson &&
        ...
        !cent->gent->s
             .number) { // Yeah um, this needs to not do this quite this way
      ent.customSkin = cgi_R_RegisterSkin(
          "models/players/kyle/model_fpls2.skin"); // precached in g_client.cpp
      // turn on arm caps
      gi.G2API_SetSurfaceOnOff(&cent->gent->ghoul2[cent->gent->playerModel],
                               "r_arm_cap_torso_off", 0);
      gi.G2API_SetSurfaceOnOff(&cent->gent->ghoul2[cent->gent->playerModel],
                               "l_arm_cap_torso_off", 0);
    } else {
      ent.customSkin = 0;
      // turn off arm caps
      gi.G2API_SetSurfaceOnOff(&cent->gent->ghoul2[cent->gent->playerModel],
                               "r_arm_cap_torso_off", 0x00000002);
      gi.G2API_SetSurfaceOnOff(&cent->gent->ghoul2[cent->gent->playerModel],
                               "l_arm_cap_torso_off", 0x00000002);
    }
    ...
    if (cent->gent && cent->gent->client &&
      ...
      if (/*!cent->currentState.saberActive ||*/ !cent->gent->client->ps
              .saberActive ||
          cent->gent->client->ps.saberLength >
              cent->gent->client->ps
                  .saberLengthMax) // hack around network lag for now
      {                            // saber is off
        if (cent->gent->client->ps.saberLength > 0) {
          if (cent->gent->client->ps.stats[STAT_HEALTH] <=
              0) { // dead, didn't actively turn it off
            cent->gent->client->ps.saberLength -=
                cent->gent->client->ps.saberLengthMax / 10 * cg.frametime / 100;
          } else { // actively turned it off, shrink faster
            cent->gent->client->ps.saberLength -=
                cent->gent->client->ps.saberLengthMax / 10 * cg.frametime / 100;
          }
        }
      } else if (cent->gent->client->ps.saberLength <
                 cent->gent->client->ps.saberLengthMax) { // saber is on
        if (!cent->gent->client->ps.saberLength) {
        ...
        if (cg.frametime > 0) {
          cent->gent->client->ps.saberLength +=
              cent->gent->client->ps.saberLengthMax / 10 * cg.frametime /
              100; //= saberLengthMax;
        }
        if (cent->gent->client->ps.saberLength >
            cent->gent->client->ps.saberLengthMax) {
      ...
      if (cent->gent->client->ps.saberLength > 0) {
        if (!cent->currentState
                 .saberInFlight) //&& cent->currentState.saberActive)
        {                        // holding the saber in-hand
          //						CGhoul2Info *currentModel =
          //&cent->gent->ghoul2[1]; 						CGhoul2Info *nextModel =
          //&cent->gent->ghoul2[1];
          // FIXME: need a version of this that *doesn't* need the mFileName in
          // the ghoul2
          ...
          {
            CG_AddSaberBlade(cent, cent, NULL, ent.renderfx,
                             cent->gent->weaponModel, ent.origin, tempAngles);
        ...
        // if ( cent->gent->client->ps.saberEventFlags&SEF_INWATER )
        {
          CG_CheckSaberInWater(cent, cent, cent->gent->weaponModel, ent.origin,
                               tempAngles);
        }
      }
      if (cent->currentState.weapon == WP_SABER &&
      ...
      */
          ...
          if (bolt == -1) {
            ...
            gi.G2API_GiveMeVectorFromMatrix(
                ...
                cent->gent->client->renderInfo.muzzleDir);
                   ...
                   (cent->gent->ghoul2[cent->gent->weaponModel].mModelindex !=
                    -1)) {
          mdxaBone_t boltMatrix;
          // figure out where the actual model muzzle is
          gi.G2API_GetBoltMatrix(cent->gent->ghoul2, cent->gent->weaponModel, 0,
                                 &boltMatrix, tempAngles, ent.origin, cg.time,
                                 cgs.model_draw, cent->currentState.modelScale);
          // work the matrix axis stuff into the original axis and origins used.
          gi.G2API_GiveMeVectorFromMatrix(
              boltMatrix, ORIGIN, cent->gent->client->renderInfo.muzzlePoint);
      ...
      if (cent->gent->NPC &&
          (cent->gent->NPC->confusionTime > cg.time ||
           ...
           cent->gent->NPC->controlledTime >
               cg.time)) { // we are currently confused, so play an effect at
                           // the headBolt position
      ...
      if (cent->gent->client->ps.powerups[PW_FORCE_PUSH] > cg.time ||
          (cent->gent->client->ps.forcePowersActive &
           (1 << FP_GRIP))) { // doing the pushing/gripping
      ...
      if (cent->gent->client->ps.forcePowersActive &
        ...
        if ( cent->currentState.number )
        {
                VectorSet( tAng, cent->pe.torso.pitchAngle,
        cent->pe.torso.yawAngle, 0 );
        }
        else
        {//FIXME: this dir looks right on players, but not on NPCs, NPCs pe
        angles are absolute, not relative? VectorSet( tAng,
        tempAngles[0]+cent->pe.torso.pitchAngle,
        tempAngles[1]+cent->pe.torso.yawAngle, 0 );
        }
        */
    ...
    if (torso.hModel) {
      ...
      if (gun.hModel) {
        ...
        if (cent->gent && cent->gent->client &&
          ...
          {                      // saber is off
            if (cent->gent->client->ps.saberLength > 0) {
              if (cent->gent->client->ps.stats[STAT_HEALTH] <=
                ...
                cent->gent->client->ps.saberLength -=
                    cent->gent->client->ps.saberLengthMax / 3 * cg.frametime /
                    100;
              }
            }
            if (cent->gent->client->ps.saberLength < 0) {
            ...
            cent->gent->client->ps.saberLength +=
                cent->gent->client->ps.saberLengthMax / 6 * cg.frametime /
                100; //= saberLengthMax;
            if (cent->gent->client->ps.saberLength >
                cent->gent->client->ps.saberLengthMax) {
              cent->gent->client->ps.saberLength =
          ...
          if (cent->currentState
                  .saberInFlight) { // not holding the saber in-hand
            drawGun = qfalse;
          }
          if (cent->gent->client->ps.saberLength > 0) {
            if (!cent->currentState.saberInFlight) { // holding the saber
                                                     // in-hand
