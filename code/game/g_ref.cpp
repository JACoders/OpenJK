...
void TAG_Init(void) {
  ...
  for (rtoi = refTagOwnerMap.begin(); rtoi != refTagOwnerMap.end(); ++rtoi) {
    ...
    for (rti = ((*rtoi).second)->tags.begin();
         rti != ((*rtoi).second)->tags.end(); ++rti) {
      if ((*rti) == NULL) {
...
reference_tag_t *TAG_Find(const char *owner, const char *name) {
  ...
  tagOwner = VALIDSTRING(owner) ? TAG_FindOwner(owner)
                                : TAG_FindOwner(TAG_GENERIC_NAME);
  ...
  if (rti == tagOwner->tagMap.end()) {
    ...
    Q_strlwr(
        (char *)tempName); // NOTENOTE: For case insensitive searches on a map
...
reference_tag_t *TAG_Add(const char *name, const char *owner, vec3_t origin,
  ...
  if (VALIDSTRING(name) == false) {
    // gi.Error("Nameless ref_tag found at (%i %i %i)", (int)origin[0],
...
void ref_link(gentity_t *ent) {
  ...
  // Add the tag
  /*tag = */ TAG_Add(ent->targetname, ent->ownername, ent->s.origin,
                     ent->s.angles, 16, 0);
