...
int G_FindConfigstringIndex(const char *name, int start, int max,
  ...
  if (i == max) {
    G_Error("G_FindConfigstringIndex: overflow adding %s to set %d-%d", name,
            start, max);
  }
...
void G_PlayEffect(const char *name, int clientNum) {
  ...
  tent =
      G_TempEntity(g_entities[clientNum].currentOrigin, EV_PLAY_MUZZLE_EFFECT);
...
//-----------------------------
void G_PlayEffect(int fxID, const int modelIndex, const int boltIndex,
                  const int entNum, const vec3_t origin, int iLoopTime,
                  qboolean isRelative) // iLoopTime 0 = not looping, 1 for
                                       // infinite, else duration
{
  gentity_t *tent;
  ...
  gi.G2API_AttachEnt(&tent->s.boltInfo, &g_entities[entNum].ghoul2[modelIndex],
...
void G_StopEffect(int fxID, const int modelIndex, const int boltIndex,
                  const int entNum) {
  gentity_t *tent;
...
void G_StopEffect(const char *name, const int modelIndex, const int boltIndex,
                  const int entNum) {
...
extern void cgi_S_StartSound(const vec3_t origin, int entityNum, int entchannel,
                             sfxHandle_t sfx);
#include "../cgame/cg_media.h" //access to cgs
extern qboolean CG_TryPlayCustomSound(vec3_t origin, int entityNum,
                                      soundChannel_t channel,
                                      const char *soundName,
                                      int customSoundSet);
extern cvar_t *g_timescale;
// NOTE: Do NOT Try to use this before the cgame DLL is valid, it will NOT work!
void G_SoundOnEnt(gentity_t *ent, soundChannel_t channel,
  ...
  if (g_timescale->integer > 50) { // Skip the sound!
    return;
  }
...
void G_SpeechEvent(gentity_t *self, int event) {
  if (in_camera && g_skippingcin &&
      g_skippingcin
          ->integer) { // Skipping a cinematic, so skip NPC voice sounds...
    return;
  }
  ...
  case EV_ANGER3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*anger%i.wav", event - EV_ANGER1 + 1), CS_COMBAT);
    break;
  case EV_VICTORY1: // Say when killed an enemy
  case EV_VICTORY2:
  case EV_VICTORY3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*victory%i.wav", event - EV_VICTORY1 + 1),
                          CS_COMBAT);
    break;
  ...
  case EV_CONFUSE3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*confuse%i.wav", event - EV_CONFUSE1 + 1),
                          CS_COMBAT);
    break;
  ...
  case EV_PUSHED3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*pushed%i.wav", event - EV_PUSHED1 + 1),
                          CS_COMBAT);
    break;
  ...
  case EV_CHOKE3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*choke%i.wav", event - EV_CHOKE1 + 1), CS_COMBAT);
    break;
  case EV_FFWARN: // Warn ally to stop shooting you
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE, "*ffwarn.wav",
                          CS_COMBAT);
    break;
  case EV_FFTURN: // Turn on ally after being shot by them
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE, "*ffturn.wav",
                          CS_COMBAT);
    break;
  // extra sounds for ST
  case EV_CHASE1:
  case EV_CHASE2:
  case EV_CHASE3:
    if (!CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                               va("*chase%i.wav", event - EV_CHASE1 + 1),
                               CS_EXTRA)) {
      CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                            va("*anger%i.wav", Q_irand(1, 3)), CS_COMBAT);
    }
    break;
  case EV_COVER1:
  case EV_COVER2:
  ...
  case EV_COVER5:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*cover%i.wav", event - EV_COVER1 + 1), CS_EXTRA);
    break;
  case EV_DETECTED1:
  case EV_DETECTED2:
  case EV_DETECTED3:
  ...
  case EV_DETECTED5:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*detected%i.wav", event - EV_DETECTED1 + 1),
                          CS_EXTRA);
    break;
  case EV_GIVEUP1:
  case EV_GIVEUP2:
  ...
  case EV_GIVEUP4:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*giveup%i.wav", event - EV_GIVEUP1 + 1),
                          CS_EXTRA);
    break;
  case EV_LOOK1:
  case EV_LOOK2:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*look%i.wav", event - EV_LOOK1 + 1), CS_EXTRA);
    break;
  case EV_LOST1:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE, "*lost1.wav",
                          CS_EXTRA);
    break;
  case EV_OUTFLANK1:
  case EV_OUTFLANK2:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*outflank%i.wav", event - EV_OUTFLANK1 + 1),
                          CS_EXTRA);
    break;
  case EV_ESCAPING1:
  case EV_ESCAPING2:
  case EV_ESCAPING3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*escaping%i.wav", event - EV_ESCAPING1 + 1),
                          CS_EXTRA);
    break;
  case EV_SIGHT1:
  case EV_SIGHT2:
  case EV_SIGHT3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*sight%i.wav", event - EV_SIGHT1 + 1), CS_EXTRA);
    break;
  case EV_SOUND1:
  case EV_SOUND2:
  case EV_SOUND3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*sound%i.wav", event - EV_SOUND1 + 1), CS_EXTRA);
    break;
  case EV_SUSPICIOUS1:
  case EV_SUSPICIOUS2:
  case EV_SUSPICIOUS3:
  ...
  case EV_SUSPICIOUS5:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*suspicious%i.wav", event - EV_SUSPICIOUS1 + 1),
                          CS_EXTRA);
    break;
  ...
  case EV_COMBAT3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*combat%i.wav", event - EV_COMBAT1 + 1), CS_JEDI);
    break;
  case EV_JDETECTED1:
  case EV_JDETECTED2:
  case EV_JDETECTED3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*jdetected%i.wav", event - EV_JDETECTED1 + 1),
                          CS_JEDI);
    break;
  case EV_TAUNT1:
  case EV_TAUNT2:
  case EV_TAUNT3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*taunt%i.wav", event - EV_TAUNT1 + 1), CS_JEDI);
    break;
  case EV_JCHASE1:
  case EV_JCHASE2:
  case EV_JCHASE3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*jchase%i.wav", event - EV_JCHASE1 + 1), CS_JEDI);
    break;
  case EV_JLOST1:
  case EV_JLOST2:
  case EV_JLOST3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*jlost%i.wav", event - EV_JLOST1 + 1), CS_JEDI);
    break;
  case EV_DEFLECT1:
  case EV_DEFLECT2:
  case EV_DEFLECT3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*deflect%i.wav", event - EV_DEFLECT1 + 1),
                          CS_JEDI);
    break;
  case EV_GLOAT1:
  case EV_GLOAT2:
  case EV_GLOAT3:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE,
                          va("*gloat%i.wav", event - EV_GLOAT1 + 1), CS_JEDI);
    break;
  case EV_PUSHFAIL:
    CG_TryPlayCustomSound(NULL, self->s.number, CHAN_VOICE, "*pushfail.wav",
                          CS_JEDI);
    break;
  }
}
...
/*
============
G_RadiusList - given an origin and a radius, return all entities that are in use
...
gentity_t *G_Spawn(void) {
  ...
  if (i == ENTITYNUM_MAX_NORMAL) {
    ...
    fclose(fp);
    /*
    //---------------Or use this to dump to the console -- beware though, the
    console will fill quickly and you probably won't see the full list for ( i =
    0 ; i<globals.num_entities ; i++, e++)
                    {
                            if ( e->classname )
                            {
                                    Com_Printf( "%d: %s\n", i, e->classname );
...
void G_FreeEntity(gentity_t *ed) {
  ...
  if (ed->NPC) {
    ...
    if (ed->client->clientInfo.customBasicSoundDir &&
        gi.bIsFromZone(ed->client->clientInfo.customBasicSoundDir,
                       TAG_G_ALLOC)) {
    ...
    if (ed->client->ps.saber[0].name &&
        gi.bIsFromZone(ed->client->ps.saber[0].name, TAG_G_ALLOC)) {
      gi.Free(ed->client->ps.saber[0].name);
    }
    if (ed->client->ps.saber[0].model &&
        gi.bIsFromZone(ed->client->ps.saber[0].model, TAG_G_ALLOC)) {
      gi.Free(ed->client->ps.saber[0].model);
    }
    if (ed->client->ps.saber[1].name &&
        gi.bIsFromZone(ed->client->ps.saber[1].name, TAG_G_ALLOC)) {
      gi.Free(ed->client->ps.saber[1].name);
    }
    if (ed->client->ps.saber[1].model &&
        gi.bIsFromZone(ed->client->ps.saber[1].model, TAG_G_ALLOC)) {
  ...
  // scripting
  if (ed->script_targetname &&
      gi.bIsFromZone(ed->script_targetname, TAG_G_ALLOC)) {
...
void G_KillBox(gentity_t *ent) {
  ...
  for (i = 0; i < num; i++) {
    ...
    if (ent->s.number && hit->client->ps.stats[STAT_HEALTH] <= 0) { // NPC
      continue;
    }
    if (ent->s.number) { // NPC
...
#endif
    if (eventParm > 255) {
      if (event == EV_PAIN) { // must have cheated, in undying?
        eventParm = 255;
      } else {
...
qboolean G_CheckInSolidTeleport(const vec3_t &teleportPos, gentity_t *self) {
  ...
  gi.trace(&trace, teleportPos, mins, self->maxs, end, self->s.number,
           self->clipmask, (EG2_Collision)0, 0);
...
qboolean G_CheckInSolid(gentity_t *self, qboolean fix) {
  ...
  gi.trace(&trace, self->currentOrigin, mins, self->maxs, end, self->s.number,
           self->clipmask, (EG2_Collision)0, 0);
  if (trace.allsolid || trace.startsolid) {
    return qtrue;
  }
...
void Svcmd_Use_f(void) {
  ...
  if (!cmd1 || !cmd1[0]) {
    ...
    gi.Printf(
        "'use' takes targetname of ent or 'list' (lists all usable ents)\n");
    return;
...
void G_SetActiveState(char *targetstring, qboolean actState) {
  ...
  while (NULL != (target = G_Find(target, FOFS(targetname), targetstring))) {
    target->svFlags = actState ? (target->svFlags & ~SVF_INACTIVE)
                               : (target->svFlags | SVF_INACTIVE);
  }
}
...
void target_deactivate_use(gentity_t *self, gentity_t *other,
                           gentity_t *activator) {
  G_ActivateBehavior(self, BSET_USE);
...
qboolean ValidUseTarget(gentity_t *ent) {
  ...
  if (ent->svFlags & SVF_INACTIVE) { // set by target_deactivate
    return qfalse;
  }

  if (!(ent->svFlags & SVF_PLAYER_USABLE)) { // Check for flag that denotes
...
static void DebugTraceForNPC(gentity_t *ent) {
  ...
  AngleVectors(ent->client->ps.viewangles, vf, NULL,
               NULL); // ent->client->renderInfo.eyeAngles was
  ...
  gi.trace(&trace, src, vec3_origin, vec3_origin, dest, ent->s.number,
           MASK_OPAQUE | CONTENTS_SOLID | CONTENTS_TERRAIN | CONTENTS_BODY |
               CONTENTS_ITEM | CONTENTS_CORPSE,
           (EG2_Collision)0, 0);
...
static qboolean G_IsTriggerUsable(gentity_t *self, gentity_t *other) {
  if (self->svFlags & SVF_INACTIVE) { // set by target_deactivate
    return qfalse;
  }
  ...
  if (self->spawnflags & 2) { // FACING
    ...
    if (DotProduct(self->movedir, forward) < 0.5) { // Not Within 45 degrees
      return qfalse;
    }
  }

  if ((!G_ValidActivateBehavior(self, BSET_USE) && !self->target) ||
      (self->target && (Q_stricmp(self->target, "n") == 0 ||
...
extern qboolean eweb_can_be_used(gentity_t *self, gentity_t *other,
                                 gentity_t *activator);
qboolean CanUseInfrontOf(gentity_t *ent) {
  ...
  if (ent->s.number &&
      ent->client->NPC_class ==
          CLASS_ATST) { // a player trying to get out of his ATST
  ...
  if (ent->client->ps.viewEntity != ent->s.number) {
    ...
    if (!Q_stricmp("misc_camera", ent->classname)) { // we are in a camera
      gentity_t *next = 0;
      if (ent->target2 != NULL) {
  ...
  if (ValidUseTarget(target)) {
    if (target->s.eType ==
        ET_ITEM) { // item, see if we could actually pick it up
      if ((target->spawnflags &
           128 /*ITMSF_USEPICKUP*/)) { // player has to be touching me and hit
                                       // use to pick it up, so don't allow this
        if (!G_BoundsOverlap(target->absmin, target->absmax, ent->absmin,
                             ent->absmax)) { // not touching
          return qfalse;
        }
      }
      if (!BG_CanItemBeGrabbed(
              &target->s,
              &ent->client->ps)) { // nope, so don't indicate that we can use it
        return qfalse;
      }
    } else if (target->e_UseFunc ==
               useF_misc_atst_use) { // drivable AT-ST from JK2
      if (ent->client->ps.groundEntityNum !=
          target->s.number) { // must be standing on it to use it
        return qfalse;
      }
  ...
  if (target->client && target->client->ps.pm_type < PM_DEAD &&
      target->NPC != NULL && target->client->playerTeam &&
      (target->client->playerTeam == ent->client->playerTeam ||
       target->client->playerTeam == TEAM_NEUTRAL) &&
...
void TryUse(gentity_t *ent) {
  ...
  // Check for a use command
  if (ValidUseTarget(target)) {
    NPC_SetAnim(ent, SETANIM_TORSO, BOTH_BUTTON_HOLD,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    /*
    if ( !VectorLengthSquared( ent->client->ps.velocity ) && !PM_CrouchAnim(
    ent->client->ps.legsAnim ) )
    {
            NPC_SetAnim( ent, SETANIM_LEGS, BOTH_BUTTON_HOLD,
    SETANIM_FLAG_NORMAL|SETANIM_FLAG_HOLD );
    }
    */
    // ent->client->ps.weaponTime = ent->client->ps.torsoAnimTimer;
    GEntity_UseFunc(target, ent, ent);
    return;
  } else if (target->client && target->client->ps.pm_type < PM_DEAD &&
             target->NPC != NULL && target->client->playerTeam &&
             (target->client->playerTeam == ent->client->playerTeam ||
              target->client->playerTeam == TEAM_NEUTRAL) &&
             !(target->NPC->scriptFlags & SCF_NO_RESPONSE)) {
...
void G_ChangeMap(const char *mapname, const char *spawntarget, qboolean hub) {
  ...
  if (g_entities[0].client->ps.pm_type == PM_DEAD)
    return;
  if (killPlayerTimer) { // can't go to next map if your allies have turned on
                         // you
    return;
  }
...
qboolean G_PointInBounds(const vec3_t point, const vec3_t mins,
                         const vec3_t maxs) {
  for (int i = 0; i < 3; i++) {
    if (point[i] < mins[i]) {
      return qfalse;
...
qboolean G_BoxInBounds(const vec3_t point, const vec3_t mins, const vec3_t maxs,
...
qboolean G_ClearTrace(const vec3_t start, const vec3_t mins, const vec3_t maxs,
                      const vec3_t end, int ignore, int clipmask) {
  static trace_t tr;
...
extern void CG_TestLine(vec3_t start, vec3_t end, int time, unsigned int color,
                        int radius);
void G_DebugLine(vec3_t A, vec3_t B, int duration, int color,
                 qboolean deleteornot) {
  /*
...
qboolean G_ExpandPointToBBox(vec3_t point, const vec3_t mins, const vec3_t maxs,
  ...
  gi.trace(&tr, start, mins, maxs, start, ignore, clipmask, (EG2_Collision)0,
           0);
  if (tr.allsolid || tr.startsolid) {
    return qfalse;
  }
...
void removeBoltSurface(gentity_t *ent) {
  ...
  if ((hitEnt->ghoul2.size() > ent->damage) &&
      ...
      (hitEnt->ghoul2[ent->damage].mSlist.size() >
       (unsigned int)ent->aimDebounceTime) &&
      (hitEnt->ghoul2[ent->damage].mSlist[ent->aimDebounceTime].surface !=
       -1) &&
      (hitEnt->ghoul2[ent->damage].mSlist[ent->aimDebounceTime].offFlags ==
       G2SURFACEFLAG_GENERATED)) {
    // remove the bolt
    ...
    if (ent->aimDebounceTime != -1) {
      gi.G2API_RemoveSurface(&hitEnt->ghoul2[ent->damage],
                             ent->aimDebounceTime);
    }
  }
...
void G_SetBoltSurfaceRemoval(const int entNum, const int modelIndex,
                             const int boltIndex, const int surfaceIndex,
                             float duration) {
  gentity_t *e;
