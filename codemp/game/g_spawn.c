...
qboolean G_SpawnBoolean(const char *key, const char *defaultString,
  ...
  if (!Q_stricmp(s, "qtrue") || !Q_stricmp(s, "true") || !Q_stricmp(s, "yes") ||
      !Q_stricmp(s, "1"))
    *out = qtrue;
  else if (!Q_stricmp(s, "qfalse") || !Q_stricmp(s, "false") ||
           !Q_stricmp(s, "no") || !Q_stricmp(s, "0"))
    *out = qfalse;
  else
    *out = qfalse;
...
field_t fields[] = {
    ...
    {"angerscript", FOFS(behaviorSet[BSET_ANGER]),
     F_STRING}, // name of script to run
    ...
    {"script_targetname", FOFS(script_targetname),
     F_STRING}, // scripts look for this when "affecting"
...
qboolean G_CallSpawn(gentity_t *ent) {
  ...
  s = (spawn_t *)Q_LinearSearch(ent->classname, spawns, ARRAY_LEN(spawns),
                                sizeof(spawn_t), spawncmp);
  if (s) { // found it
...
void G_ParseField(const char *key, const char *value, gentity_t *ent) {
  ...
  f = (field_t *)Q_LinearSearch(key, fields, ARRAY_LEN(fields), sizeof(field_t),
                                fieldcmp);
  if (f) { // found it
    b = (byte *)ent;
    ...
    case F_VECTOR:
      if (sscanf(value, "%f %f %f", &vec[0], &vec[1], &vec[2]) == 3) {
        ...
        trap->Print(
            "G_ParseField: Failed sscanf on F_VECTOR (key/value: %s/%s)\n", key,
            value);
        ((float *)(b + f->ofs))[0] = ((float *)(b + f->ofs))[1] =
...
void G_SpawnGEntityFromSpawnVars(qboolean inSubBSP) {
  ...
  if (trap->ICARUS_ValidEnt((sharedEntity_t *)ent)) {
    ...
    if (ent->classname && ent->classname[0]) {
      if (Q_strncmp("NPC_", ent->classname, 4) !=
          0) { // Not an NPC_spawner (rww - probably don't even care for MP, but
               // whatever)
...
static void HandleEntityAdjustment(void) {
  ...
  // damn VMs don't handle outputing a float that is compatible with sscanf in
  // all cases
  Com_sprintf(temp, sizeof(temp), "%0.0f %0.0f %0.0f", newOrigin[0],
  ...
  if (Q_stricmp(value, NOVALUE) != 0) {
    ...
    // damn VMs don't handle outputing a float that is compatible with sscanf in
    // all cases
    Com_sprintf(temp, sizeof(temp), "%0.0f %0.0f %0.0f", angles[0], angles[1],
                angles[2]);
...
qboolean G_ParseSpawnVars(qboolean inSubBSP) {
  ...
  if (com_token[0] != '{') {
    trap->Error(ERR_DROP, "G_ParseSpawnVars: found %s when expecting {",
                com_token);
  }
...
void SP_worldspawn(void) {
  ...
  for (i = 0; i < level.numSpawnVars; i++) {
    if (Q_stricmp("spawnscript", level.spawnVars[i][0]) ==
      ...
      G_ParseField(level.spawnVars[i][0], level.spawnVars[i][1],
                   &g_entities[ENTITYNUM_WORLD]);
    }
  }
  // The server will precache the standard model and animations, so that there
  // is no hit when the first client connnects.
  if (!BGPAFtextLoaded) {
    BG_ParseAnimationFile("models/players/_humanoid/animation.cfg",
                          bgHumanoidAnimations, qtrue);
  }

  if (!precachedKyle) {
    int defSkin;

    trap->G2API_InitGhoul2Model(&precachedKyle, "models/players/kyle/model.glm",
                                0, 0, -20, 0, 0);
  ...
  if (!g2SaberInstance) {
    trap->G2API_InitGhoul2Model(
        &g2SaberInstance, "models/weapons2/saber/saber_w.glm", 0, 0, -20, 0, 0);

    if (g2SaberInstance) {
      // indicate we will be bolted to model 0 (ie the player) on bolt 0 (always
      // the right hand) when we get copied
  ...
  if (g_restarted.integer) {
    ...
    level.warmupTime = 0;
             ...
             level.gametype != GT_SIEGE) { // Turn it on
    level.warmupTime = -1;
    trap->SetConfigstring(CS_WARMUP, va("%i", level.warmupTime));
    G_LogPrintf("Warmup:\n");
  }

  trap->SetConfigstring(CS_LIGHT_STYLES + (LS_STYLES_START * 3) + 0,
                        defaultStyles[0][0]);
  trap->SetConfigstring(CS_LIGHT_STYLES + (LS_STYLES_START * 3) + 1,
                        defaultStyles[0][1]);
  trap->SetConfigstring(CS_LIGHT_STYLES + (LS_STYLES_START * 3) + 2,
                        defaultStyles[0][2]);
