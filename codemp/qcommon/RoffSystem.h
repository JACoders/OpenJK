...
#define ROFF_AUTO_FIX_BAD_ANGLES // exporter can mess up angles,
                                 //	defining this attempts to detect and fix
                                 //these problems
...
private:
  ...
  //-------------------------------
  {
    ...
    float mCount; // I think this is a float because of a limitation of the roff
                  // exporter
  ...
  //-------------------------------
  {
    char mHeader[4]; // should match roff_string defined above
    long mVersion;   // version num, supported version defined above
    int mCount; // I think this is a float because of a limitation of the roff
                // exporter
    int mFrameRate; // Frame rate the roff should be played at
    int mNumNotes;  // number of notes (null terminated strings) after the roff
                    // data
  ...
  public:
    ...
    CROFF() { mUsedByClient = mUsedByServer = qfalse; }
  ...
  //--------------------------------------
  {
    ...
    qboolean mSignal; // TODO:  Need to implement some sort of signal to Icarus
                      // when roff is done.
    qboolean mTranslated; // should this roff be "rotated" to fit the entity's
                          // initial position?
    qboolean mIsClient;
  ...
  qboolean
  InitROFF(byte *file,
           CROFF *obj); // Handles stashing raw roff data into the roff object
  qboolean
  InitROFF2(byte *file,
            CROFF *obj); // Handles stashing raw roff data into the roff object
  ...
  qboolean
  ApplyROFF(SROFFEntity *roff_ent,
            CROFFSystem::CROFF *roff); // True = success; False = roff complete

  void ProcessNote(SROFFEntity *roff_ent, char *note);

  void SetLerp(trajectory_t *tr, trType_t, vec3_t origin, vec3_t delta,
               int time, int rate);

  qboolean ClearLerp(
      SROFFEntity *roff_ent); // Clears out the angular and position lerp fields
...
public:
  ...
  int Cache(const char *file, qboolean isClient); // roffs should be precached
                                                  // at the start of each level
  int GetID(const char *file); // find the roff id by filename
  qboolean
  Unload(int id); // when a roff is done, it can be removed to free up resources
  qboolean Clean(qboolean isClient); // should be called when level is done,
                                     // frees all roff resources
  void List(void); // dumps a list of all cached roff files to the console
  qboolean
  List(int id); // dumps the contents of the specified roff to the console

  qboolean
  Play(int entID, int roffID, qboolean doTranslation,
       qboolean isClient); // TODO: implement signal on playback completion.
  void ListEnts();         // List the entities that are currently roffing
  qboolean PurgeEnt(int entID, qboolean isClient); // Purge the specified entity
                                                   // from the entity list by id
  qboolean PurgeEnt(
      char *file); // Purge the specified entity from the entity list by name
  void
  UpdateEntities(qboolean isClient); // applys roff data to roffing entities.
