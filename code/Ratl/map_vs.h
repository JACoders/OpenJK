...
// Map
// ---
// This map is based on a red black tree, which guarentees balanced data, no
// mater what order elements are added.  The map uses a memory pool for storage
// of node data.
//
//
////////////////////////////////////////////////////////////////////////////////////////
...
public:
  enum {
    RED_BIT = 0x40000000, // to save space we are putting the red bool in a high
                          // bit this is in the parent only
  ...
  int parent() const { return mParent & (~RED_BIT); }
  bool red() const { return !!(mParent & RED_BIT); }
...
template <class T, int IS_MULTI> class tree_base {
public:
...
private:
  ...
  int insert_internal(const TTValue &key, int &at) {
    ...
    if (at == tree_node::NULL_NODE) {
      ...
      return tree_node::NULL_NODE; // There Are No Excess Red Children (No
                                   // Childeren At All, Actually)
    }

    int nxtChild; // The Child We Will Eventually Add Underneath
    // int		altChild;						// The "other"
    // Child
    bool nxtRotateLeft;
    int excessRedChild; // If The Insert Results In An Excess Red Child, This
                        // Will Be It
    ...
    if (key < mPool[at]) // The Key Classes Must Support A < Operator
    {
      ...
      if (tmp == tree_node::NULL_NODE) {
        ...
        link_left(at, mLastAdd); // T::node(mPool[at]).set_left(mLastAdd);
                                 // // If mLeft Of The Current Node Is NULL, We
                                 // Must Have Added DIRECTLY Below nAt
      }
      ...
      if (tmp == tree_node::NULL_NODE) {
        link_right(at, mLastAdd); // T::node(mPool[at]).set_right(mLastAdd);
                                  // // If mRight Of The Current Node Is NULL,
                                  // We Must Have Added DIRECTLY Below nAt
      }
    ...
    // If The Add Resulted In An Excess Red Child, We Need To Change Colors And
    // Rotate
    //---------------------------------------------------------------------------------
    if (excessRedChild != tree_node::NULL_NODE) {
      ...
      if (T::node(mPool[at]).right() != tree_node::NULL_NODE &&
        ...
        int excessRedChildCompare = (nxtRotateLeft)
                                        ? (T::node(mPool[nxtChild]).right())
                                        : (T::node(mPool[nxtChild]).left());
  ...
  bool erase_internal(const TTValue &key, int &at) {
    ...
    //-------------------------------------
    if (T::node(mPool[at]).left() == tree_node::NULL_NODE ||
        T::node(mPool[at]).right() == tree_node::NULL_NODE) {
    ...
    // We Need To Find An Open Leaf, Swap Data With That Leaf, and Then Go Find
    // It
    //==============================================================================
    ...
    if (at_parent != tree_node::NULL_NODE) {
      if (T::node(mPool[at_parent]).left() == at) {
        ...
        assert(T::node(mPool[at_parent]).right() ==
               at); // better be my parents right child then
  ...
  void rotate(int &at, bool left) {
    ...
    if (left) {
      ...
      link_right(
          at,
          T::node(mPool[t])
              .left()); // T::node(mPool[at]).set_right(T::node(mPool[t]).left());
      ...
      link_left(
          at,
          T::node(mPool[t])
              .right()); // T::node(mPool[at]).set_left(T::node(mPool[t]).right());
  ...
  bool rebalance(int &at, bool left) {
    ...
    //------------------------------------
    int w = (left) ? (T::node(mPool[at]).right())
                   : (T::node(mPool[at]).left()); // w is the child of at
    ...
    int x = (left) ? (wAt.left())
                   : (wAt.right()); // x and y are the grand children of at
...
protected:
  int front() const { return front(mRoot); }

  int back() const { return back(mRoot); }
  ...
  int next(int at) const {
    ...
    while (parent != tree_node::NULL_NODE &&
           (child == T::node(mPool[parent]).right())) {
      child = parent;
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  // This is the map internal recursive previous function - do not use
  // externally
  ////////////////////////////////////////////////////////////////////////////////////
  int previous(int at) const {
    ...
    while (parent != tree_node::NULL_NODE &&
           (child == T::node(mPool[parent]).left())) {
      child = parent;
      find_internal(mPool[parent], parent, mRoot, parent);
    }
    return parent;
...
public:
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  bool empty() const { return (mPool.empty()); }
  ...
  template <class CAST_TO> CAST_TO *verify_alloc_key(CAST_TO *p) const {
  ...
  void insert_alloced_key() {
    ...
    assert(!IS_MULTI ||
           find_index(mPool[mLastAdd]) !=
               tree_node::NULL_NODE); // fixme handle duplicates more sensibly?
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  int find_index(const TTValue &key) const { return find_internal(key, mRoot); }
...
public:
  ...
  // Allocs an item, when filled, call insert_alloced
  ////////////////////////////////////////////////////////////////////////////////////
  TTValue &alloc() { return tree_base<T, IS_MULTI>::alloc_key(); }

  ////////////////////////////////////////////////////////////////////////////////////
  // Allocs an item (raw), when constucted, call insert_alloced
  ////////////////////////////////////////////////////////////////////////////////////
  TRatlNew *alloc_raw() { return tree_base<T, IS_MULTI>::alloc_key_raw(); }
  template <class CAST_TO> CAST_TO *verify_alloc(CAST_TO *p) const {
    return verify_alloc_key(p);
  }

  void insert_alloced() { tree_base<T, IS_MULTI>::insert_alloced_key(); }
  ...
  public:
    iterator(set_base<TStorageTraits, IS_MULTI> *owner = 0,
             int loc = tree_node::NULL_NODE)
        : mOwner(owner), mLoc(loc) {}
    iterator(const iterator &o) : mOwner(o.mOwner), mLoc(o.mLoc) {}
    ...
    bool operator!=(const iterator p) const {
      return (mLoc != p.mLoc || mOwner != p.mOwner);
    }
    bool operator==(const iterator p) const {
      return (mLoc == p.mLoc && mOwner == p.mOwner);
    }
  ...
  public:
    const_iterator(const set_base<TStorageTraits, IS_MULTI> *owner = 0,
                   int loc = tree_node::NULL_NODE)
        : mOwner(owner), mLoc(loc) {}
    const_iterator(const const_iterator &o) : mOwner(o.mOwner), mLoc(o.mLoc) {}
    const_iterator(const iterator &o) : mOwner(o.mOwner), mLoc(o.mLoc) {}
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  iterator find(const TTValue &key) { return iterator(this, find_index(key)); }

  ////////////////////////////////////////////////////////////////////////////////////
  // Get An Iterator To The Smallest Element  - O(log n)
  ////////////////////////////////////////////////////////////////////////////////////
  iterator begin() { return iterator(this, tree_base<T, IS_MULTI>::front()); }

  ////////////////////////////////////////////////////////////////////////////////////
  // Get An Iterator To The Largest Element  - O(log n)
  ////////////////////////////////////////////////////////////////////////////////////
  iterator rbegin() { return iterator(this, tree_base<T, IS_MULTI>::back()); }
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  // The Invalid const_iterator, Use As A Stop Condition In Your For Loops  -
  // O(1)
  ////////////////////////////////////////////////////////////////////////////////////
...
class set_vs
    : public set_base<storage::value_semantics_node<T, ARG_CAPACITY, tree_node>,
                      0> {
public:
  typedef typename storage::value_semantics_node<T, ARG_CAPACITY, tree_node>
      TStorageTraits;
...
class set_os
    : public set_base<
          storage::object_semantics_node<T, ARG_CAPACITY, tree_node>, 0> {
public:
  typedef typename storage::object_semantics_node<T, ARG_CAPACITY, tree_node>
      TStorageTraits;
  typedef typename TStorageTraits::TValue TTValue;
  static const int CAPACITY = ARG_CAPACITY;
  set_os() {}
...
class set_is
    : public set_base<storage::virtual_semantics_node<
                          T, ARG_CAPACITY, ARG_MAX_CLASS_SIZE, tree_node>,
                      0> {
public:
  typedef
      typename storage::virtual_semantics_node<T, ARG_CAPACITY,
                                               ARG_MAX_CLASS_SIZE, tree_node>
          TStorageTraits;
  typedef typename TStorageTraits::TValue TTValue;
  static const int CAPACITY = ARG_CAPACITY;
...
public:
  map_base() { compile_assert<K::CAPACITY == V::CAPACITY>(); }
  ...
  void insert(const TKTValue &key, const TVTValue &value) {
    assert(!IS_MULTI ||
           (tree_base<K, IS_MULTI>::find_index(key) ==
            tree_node::NULL_NODE)); // fixme handle duplicates more sensibly?
  ...
  public:
    iterator(map_base<K, V, IS_MULTI> *owner = 0,
             int loc = tree_node::NULL_NODE)
        : mLoc(loc), mOwner(owner) {}
    iterator(const iterator &o) : mLoc(o.mLoc), mOwner(o.mOwner) {}
  ...
  public:
    const_iterator(const map_base<K, V, IS_MULTI> *owner = 0,
                   int loc = tree_node::NULL_NODE)
        : mOwner(owner), mLoc(loc) {}
    const_iterator(const const_iterator &o) : mOwner(o.mOwner), mLoc(o.mLoc) {}
    const_iterator(const iterator &o) : mOwner(o.mOwner), mLoc(o.mLoc) {}
  ...
  // Get An Iterator To The Smallest Element  - O(log n)
  ////////////////////////////////////////////////////////////////////////////////////
  iterator begin() { return iterator(this, tree_base<K, IS_MULTI>::front()); }

  ////////////////////////////////////////////////////////////////////////////////////
  // Get An Iterator To The Largest Element  - O(log n)
  ////////////////////////////////////////////////////////////////////////////////////
  iterator rbegin() { return iterator(this, tree_base<K, IS_MULTI>::back()); }
...
class map_vs
    : public map_base<storage::value_semantics_node<K, ARG_CAPACITY, tree_node>,
                      storage::value_semantics<V, ARG_CAPACITY>, 0> {
public:
...
class map_os
    : public map_base<storage::value_semantics_node<K, ARG_CAPACITY, tree_node>,
                      storage::object_semantics<V, ARG_CAPACITY>, 0> {
public:
...
class map_is
    : public map_base<
          ...
          storage::virtual_semantics<V, ARG_CAPACITY, ARG_MAX_CLASS_SIZE>, 0> {
public:
  typedef
      typename storage::virtual_semantics<V, ARG_CAPACITY, ARG_MAX_CLASS_SIZE>
          VStorageTraits;
  typedef typename VStorageTraits::TValue TTValue;
  static const int CAPACITY = ARG_CAPACITY;
  static const int MAX_CLASS_SIZE = ARG_MAX_CLASS_SIZE;
