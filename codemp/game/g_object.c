...
void G_BounceObject(gentity_t *ent, trace_t *trace) {
  ...
  //	bounceFactor = 60/ent->mass;		// NOTENOTE Mass is not yet
  //implemented
  bounceFactor = 1.0f;
    ...
    if (((trace->plane.normal[2] > 0.7 && g_gravity.value > 0) ||
        ...
        ((ent->s.pos.trDelta[2] < 40 && g_gravity.value > 0) ||
         (ent->s.pos.trDelta[2] > -40 &&
          g_gravity.value < 0))) // this can happen even on very slightly sloped
                                 // walls, so changed it from > 0 to > 0.7
    {
...
void G_RunObject(gentity_t *ent) {
  ...
  if (VectorCompare(ent->r.currentOrigin,
                    origin)) { // error - didn't move at all!
    return;
  }
  // trace a line from the previous position to the current position,
  // ignoring interactions with the missile owner
  trap->Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin,
              ent->parent ? ent->parent->s.number : ent->s.number,
  ...
  if (tr.fraction || (traceEnt && traceEnt->takedamage)) {
    if (!VectorCompare(ent->r.currentOrigin, oldOrg)) { // moved and impacted
      if ((traceEnt && traceEnt->takedamage)) {         // hurt someone
        ...
        //"sound/movers/objects/objectHurt.wav" ) );
      }
      //			G_Sound( ent, G_SoundIndex(
      //"sound/movers/objects/objectHit.wav" ) );
    }
