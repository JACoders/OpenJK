...
gotnewcl:
  ...
  *newcl = temp;
  ...
  denied = ge->ClientConnect(clientNum, qtrue,
                             eSavedGameJustLoaded); // firstTime = qtrue
  if (denied) {
    NET_OutOfBandPrint(NS_SERVER, from, "print\n%s\n", denied);
...
void SV_ClientEnterWorld(client_t *client, usercmd_t *cmd,
  ...
  // normally I check 'qbFromSavedGame' to avoid overwriting loaded client data,
  // but this stuff I want
  //	to be reset so that client packet delta-ing bgins afresh, rather than
  //based on your previous frame 	(which didn't in fact happen now if we've just
...
Pull specific info from a newly changed userinfo string into a more C friendly
form.
=================
*/
    ...
    if (sv_mapname->string[0] != '_') {
      ...
      if (eSavedGameJustLoaded == eNO) {
        ...
        if (Q_stricmpn(sv_mapname->string, "academy", 7) != 0) {
          Com_sprintf(savename, sizeof(savename), "auto_%s",
                      sv_mapname->string);
