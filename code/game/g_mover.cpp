...
gentity_t *G_TestEntityPosition(gentity_t *ent) {
  ...
  if (ent->client) {
    gi.trace(&tr, ent->client->ps.origin, ent->mins, ent->maxs,
             ent->client->ps.origin, ent->s.number, mask, (EG2_Collision)0, 0);
  } else {
    if (ent->s.eFlags &
                          ...
                          // basically be a point contents check
    {
      gi.trace(&tr, ent->s.pos.trBase, vec3_origin, vec3_origin,
               ent->s.pos.trBase, ent->s.number, mask, (EG2_Collision)0, 0);
    } else {
      gi.trace(&tr, ent->s.pos.trBase, ent->mins, ent->maxs, ent->s.pos.trBase,
               ent->s.number, mask, (EG2_Collision)0, 0);
    }
  }
...
qboolean G_TryPushingEntity(gentity_t *check, gentity_t *pusher, vec3_t move,
  ...
  */
  ...
  if (pusher->damage) {                      // Do damage
    ...
    { // crusher stuck on a non->player corpse that does not have a key and is
...
its "closed" position "model2"	.md3 model to also draw "modelAngles" md3
model's angles <pitch yaw roll> (in addition to any rotation on the part of the
brush entity itself) "angle"		determines the opening direction
...
"modelAngles" md3 model's angles <pitch yaw roll> (in addition to any rotation
on the part of the brush entity itself) "color"		constantLight color
"light"		constantLight radius
*/
...
"model2"	.md3 model to also draw
"modelAngles" md3 model's angles <pitch yaw roll> (in addition to any rotation
on the part of the brush entity itself) "angle"		determines the opening
direction "target"	all entities with a matching targetname will be used
...
void Think_SetupTrainTargets(gentity_t *ent) {
  ...
  for (path = ent->nextTrain; path != start; path = next) {
    if (!iterations--) {
      G_Error("Think_SetupTrainTargets:  last path_corner doesn't link back to "
              "first on func_train(%s)",
              vtos(ent->absmin));
    }
    if (!start) {
      start = path;
...
"model2"	.md3 model to also draw
"modelAngles" md3 model's angles <pitch yaw roll> (in addition to any rotation
on the part of the brush entity itself) "speed"		default 100 "dmg"
default	2 "health"	default 0
...
/*QUAKED func_static (0 .5 .8) ? F_PUSH F_PULL SWITCH_SHADER CRUSHER IMPACT
SOLITARY PLAYER_USE INACTIVE BROADCAST
...
SOLITARY	Can only be pushed when directly under crosshair, when pushed
you shall push nothing else BUT me. PLAYER_USE	Player can use it with the use
button INACTIVE	must be used by a target_activate before it can be used
BROADCAST   don't ever use this, it's evil

A bmodel that just sits there, doing nothing.  Can be used for conditional walls
and models. "model2"	.md3 model to also draw "modelAngles" md3 model's angles
<pitch yaw roll> (in addition to any rotation on the part of the brush entity
itself) "color"		constantLight color "light"		constantLight
radius
"dmg"		how much damage to do when it crushes (use with CRUSHER
spawnflag default is 2) "linear" set to 1 and it will move linearly rather than
with acceleration (default is 0) "NPC_targetname" if set up to be push/pullable,
only this NPC can push/pull it (for the player, use "player")
*/
void SP_func_static(gentity_t *ent) {
  ...
  if (ent->spawnflags & 8) { //!!! 8 is NOT the crusher spawnflag, 4 is!!!
    ent->spawnflags &= ~8;
...
"model2"	.md3 model to also draw
"modelAngles" md3 model's angles <pitch yaw roll> (in addition to any rotation
on the part of the brush entity itself) "speed"		determines how fast it
moves; default value is 100.
...
"model2"	.md3 model to also draw
"modelAngles" md3 model's angles <pitch yaw roll> (in addition to any rotation
on the part of the brush entity itself) "height"	amplitude of bob (32
default) "speed"		seconds to complete a bob cycle (4 default)
...
model to also draw "modelAngles" md3 model's angles <pitch yaw roll> (in
addition to any rotation on the part of the brush entity itself) "speed"
the number of degrees each way the pendulum swings, (30 default) "phase"
...
void use_wall(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  ...
  if (!ent->count) // off
  {
    ...
    ent->s.eFlags &= ~EF_NODRAW;
    ent->count = 1;
    // NOTE: reset the brush model because we need to get *all* the contents
    // back ent->contents |= CONTENTS_SOLID;
    gi.SetBrushModel(ent, ent->model);
