...
static qboolean NPC_Jump(vec3_t dest,
                         int goalEntNum) { // FIXME: if land on enemy, knock him
                                           // down & jump off again
  float targetDist, travelTime, impactDist,
      bestImpactDist = Q3_INFINITE; // fireSpeed,
  float originalShotSpeed, shotSpeed, speedStep = 50.0f, minShotSpeed = 30.0f,
                                      maxShotSpeed = 500.0f;
  ...
  trajectory_t tr;
  qboolean blocked;
  int elapsedTime, timeStep = 250, hitCount = 0, aboveTries = 0, belowTries = 0,
                   maxHits = 10;
  ...
  originalShotSpeed =
      targetDist; // DistanceHorizontal( dest, NPC->currentOrigin )/2.0f;
    ...
    {      // do a rough trace of the path
      ...
      for (elapsedTime = timeStep; elapsedTime < floor(travelTime) + timeStep;
        ...
        */
        ...
        if (trace.allsolid || trace.startsolid) { // started in solid
        ...
        if (trace.fraction < 1.0f) { // hit something
          ...
          if (trace.entityNum == goalEntNum) { // hit the enemy, that's bad!
            blocked = qtrue;
            /*
            if ( g_entities[goalEntNum].client &&
            g_entities[goalEntNum].client->ps.groundEntityNum == ENTITYNUM_NONE
            )
      ...
      if (blocked) { // hit something, adjust speed (which will change arc)
        hitCount++;
        // alternate back and forth between trying an arc slightly above or
        // below the ideal
        if ((hitCount % 2) && !belowBlocked) { // odd
          belowTries++;
          shotSpeed = originalShotSpeed - (belowTries * speedStep);
        } else if (!aboveBlocked) { // even
          aboveTries++;
          shotSpeed = originalShotSpeed + (aboveTries * speedStep);
        } else { // can't go any higher or lower
          hitCount = maxHits;
          break;
        }
...
qboolean NPC_CanTryJump() {
  if (!(NPCInfo->scriptFlags & SCF_NAV_CAN_JUMP) || // Can't Jump
      ...
      (level.time <
       NPCInfo->jumpBackupTime) || // If Backing Up, Don't Try The Jump Again
      (level.time <
       NPCInfo->jumpNextCheckTime) || // Don't Even Try To Jump Again For This
                                      // Amount Of Time
...
qboolean NPC_TryJump(const vec3_t &pos, float max_xy_dist, float max_z_diff) {
  if (NPC_CanTryJump()) {
    ...
    //-----------------------------------------
    {
      ...
      gi.trace(&mJumpTrace, NPCInfo->jumpDest, vec3_origin, vec3_origin,
               groundTest, NPC->s.number, NPC->clipmask, (EG2_Collision)0, 0);
      if (mJumpTrace.fraction >= 1.0f) {
        return qfalse; // no ground = no jump
      }
    }
    NPCInfo->jumpTarget = 0;
    NPCInfo->jumpMaxXYDist =
        (max_xy_dist)
            ? (max_xy_dist)
            : ((NPC->client->NPC_class == CLASS_ROCKETTROOPER) ? 1200 : 750);
    NPCInfo->jumpMazZDist =
        (max_z_diff)
            ? (max_z_diff)
...
qboolean NPC_TryJump() {
  ...
  // Test To See If There Is A Wall Directly In Front Of Actor, If So, Backup
  // Some
  //-------------------------------------------------------------------------------
  if (TIMER_Done(NPC, "jumpBackupDebounce")) {
    vec3_t actorProjectedTowardTarget;
    VectorMA(NPC->currentOrigin, NPC_JUMP_PREP_BACKUP_DIST, targetDirection,
             actorProjectedTowardTarget);
    gi.trace(&mJumpTrace, NPC->currentOrigin, vec3_origin, vec3_origin,
             actorProjectedTowardTarget, NPC->s.number, NPC->clipmask,
             (EG2_Collision)0, 0);
    if ((mJumpTrace.fraction < 1.0f) || (mJumpTrace.allsolid) ||
        (mJumpTrace.startsolid)) {
      if (NAVDEBUG_showCollision) {
        CG_DrawEdge(NPC->currentOrigin, actorProjectedTowardTarget,
                    EDGE_RED_TWOSECOND); // TryJump
      }
  ...
  //	bool	OnLowerLedge			= ((targetDistanceZ<-80.0f) &&
  //(targetDistanceZ>-200.0f)); 	bool	WithinNormalJumpRange	=
  //((targetDistanceZ<32.0f)  && (targetDistanceXY<200.0f));
  bool WithinForceJumpRange =
      ((fabsf(targetDistanceZ) > 0) || (targetDistanceXY > 128));
  ...
  */
  ...
  // If There Is Any Chance That This Jump Will Land On An Enemy, Try 8
  // Different Traces Around The Target
  //-------------------------------------------------------------------------------------------------------
  if (NPCInfo->jumpTarget) {
    float minSafeRadius =
        (NPC->maxs[0] * 1.5f) + (NPCInfo->jumpTarget->maxs[0] * 1.5f);
    float minSafeRadiusSq = (minSafeRadius * minSafeRadius);

    if (DistanceSquared(NPCInfo->jumpDest, NPCInfo->jumpTarget->currentOrigin) <
        minSafeRadiusSq) {
      vec3_t startPos;
      vec3_t floorPos;
      ...
      for (int sideTryCount = 0; sideTryCount < 8; sideTryCount++) {
        ...
        gi.trace(&mJumpTrace, NPCInfo->jumpDest, NPC->mins, NPC->maxs, floorPos,
                 (NPCInfo->jumpTarget) ? (NPCInfo->jumpTarget->s.number)
                                       : (NPC->s.number),
                 (NPC->clipmask | CONTENTS_BOTCLIP), (EG2_Collision)0, 0);
        if ((mJumpTrace.fraction < 1.0f) && (!mJumpTrace.allsolid) &&
            (!mJumpTrace.startsolid)) {
  ...
  //-----------------------------------------------
  if (NPC_Jump(NPCInfo->jumpDest, (NPCInfo->jumpTarget)
...
static void NPC_LadderMove(vec3_t dir) {
  ...
  if ((dir[2] > 0) ||
      (dir[2] < 0 && NPC->client->ps.groundEntityNum == ENTITYNUM_NONE)) {
...
inline qboolean NPC_GetMoveInformation(vec3_t dir, float *distance) {
  ...
  *distance = VectorNormalize(dir);
  ...
  VectorCopy(NPCInfo->goalEntity->currentOrigin,
             NPCInfo->blockedTargetPosition);
...
extern int navTime;
#endif //	AI_TIMERS
qboolean NPC_MoveToGoal(
    qboolean tryStraight) // FIXME: tryStraight not even used!  Stop passing it
{
#if AI_TIMERS
  ...
  if (PM_InKnockDown(&NPC->client->ps) ||
      ((NPC->client->ps.legsAnim >= BOTH_PAIN1) &&
       ...
       NPC->client->ps.legsAnimTimer >
           0)) { // If taking full body pain, don't move
    return qtrue;
  }

  if (NPC->s.eFlags &
      EF_LOCKED_TO_WEAPON) { // If in an emplaced gun, never try to navigate!
    return qtrue;
  }

  if (NPC->s.eFlags &
      EF_HELD_BY_RANCOR) { // If in a rancor's hand, never try to navigate!
    return qtrue;
  }
  if (NPC->s.eFlags &
      EF_HELD_BY_WAMPA) { // If in a wampa's hand, never try to navigate!
    return qtrue;
  }
  if (NPC->s.eFlags & EF_HELD_BY_SAND_CREATURE) { // If in a worm's mouth, never
                                                  // try to navigate!
    return qtrue;
  }

  if (NPC->watertype & CONTENTS_LADDER) { // Do we still want to do this?
    vec3_t dir;
    VectorSubtract(NPCInfo->goalEntity->currentOrigin, NPC->currentOrigin, dir);
    VectorNormalize(dir);
    NPC_LadderMove(dir);
