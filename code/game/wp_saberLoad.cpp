...
void Saber_SithSwordPrecache(
    void) { //*SIGH* special sounds used by the sith sword
  int i;
  for (i = 1; i < 5; i++) {
...
stringID_table_t SaberTable[] = {
    ENUM2STRING(SABER_NONE),       ENUM2STRING(SABER_SINGLE),
    ENUM2STRING(SABER_STAFF),      ENUM2STRING(SABER_BROAD),
    ENUM2STRING(SABER_PRONG),      ENUM2STRING(SABER_DAGGER),
    ENUM2STRING(SABER_ARC),        ENUM2STRING(SABER_SAI),
    ENUM2STRING(SABER_CLAW),       ENUM2STRING(SABER_LANCE),
    ENUM2STRING(SABER_STAR),       ENUM2STRING(SABER_TRIDENT),
    ENUM2STRING(SABER_SITH_SWORD), {"", -1}};
...
qboolean WP_SaberBladeUseSecondBladeStyle(saberInfo_t *saber, int bladeNum) {
  if (saber && saber->bladeStyle2Start > 0 &&
...
qboolean WP_UseFirstValidSaberStyle(gentity_t *ent, int *saberAnimLevel) {
  if (ent && ent->client) {
    ...
    if (styleInvalid &&
        validStyles) { // using an invalid style and have at least one valid
                       // style to use, so switch to it
      for (styleNum = SS_NONE + 1; styleNum < SS_NUM_SABER_STYLES; styleNum++) {
  ...
  if (ent && ent->client) {
    ...
    if (ent->client->ps.dualSabers) { // check second saber, too
      if (ent->client->ps.saber[1].Active()) {
        ...
        // now: if using dual sabers, only dual and tavion (if given with this
        // saber) are allowed
        if (saberAnimLevel != SS_DUAL) { // dual is okay
          if (saberAnimLevel !=
              SS_TAVION) { // tavion might be okay, all others are not
            return qfalse;
          } else { // see if "tavion" style is okay
            if (ent->client->ps.saber[0].Active() &&
                (ent->client->ps.saber[0].stylesLearned &
                 (1 << SS_TAVION))) { // okay to use tavion style, first saber
                                      // gave it to us
            } else if ((ent->client->ps.saber[1].stylesLearned &
                        (1 << SS_TAVION))) { // okay to use tavion style, second
                                             // saber gave it to us
            } else { // tavion style is not allowed because neither of the
                     // sabers we're using gave it to us (I know, doesn't quite
                     // make sense, but...)
              return qfalse;
            }
          }
                 ...
                 SS_DUAL) { // can't use dual style if not using dualSabers
        return qfalse;
      }
    } else if (saberAnimLevel ==
               SS_DUAL) { // can't use dual style if not using dualSabers
...
qboolean WP_SaberCanTurnOffSomeBlades(saberInfo_t *saber) {
  if (saber->bladeStyle2Start > 0 &&
    ...
    if ((saber->saberFlags2 & SFL2_NO_MANUAL_DEACTIVATE) &&
        (saber->saberFlags2 &
         SFL2_NO_MANUAL_DEACTIVATE2)) { // all blades are always on
      return qfalse;
    }
  } else {
    if ((saber->saberFlags2 &
         SFL2_NO_MANUAL_DEACTIVATE)) { // all blades are always on
      return qfalse;
    }
  }
...
void WP_SaberSetDefaults(saberInfo_t *saber, qboolean setColors = qtrue) {
  ...
  saber->singleBladeStyle =
      SS_NONE; // makes it so that you use a different style if you only have
               // the first blade active
  saber->brokenSaber1 = NULL; // if saber is actually hit by another saber, it
                              // can be cut in half/broken and will be replaced
                              // with this saber in your right hand
  saber->brokenSaber2 =
      NULL; // if saber is actually hit by another saber, it can be cut in
            // half/broken and will be replaced with this saber in your left hand
  //===NEW========================================================================================
  ...
  saber->spinSound =
      0; // none - if set, plays this sound as it spins when thrown
  saber->swingSound[0] =
      0; // none - if set, plays one of these 3 sounds when swung during an
         // attack - NOTE: must provide all 3!!!
  saber->swingSound[1] =
      0; // none - if set, plays one of these 3 sounds when swung during an
         // attack - NOTE: must provide all 3!!!
  saber->swingSound[2] =
      0; // none - if set, plays one of these 3 sounds when swung during an
         // attack - NOTE: must provide all 3!!!
  saber->fallSound[0] =
      0; // none - if set, plays one of these 3 sounds when weapon falls to the
         // ground - NOTE: must provide all 3!!!
  saber->fallSound[1] =
      0; // none - if set, plays one of these 3 sounds when weapon falls to the
         // ground - NOTE: must provide all 3!!!
  saber->fallSound[2] =
  ...
  saber->moveSpeedScale =
      1.0f; // 1.0 - you move faster/slower when using this saber
  saber->animSpeedScale =
      1.0f; // 1.0 - plays normal attack animations faster/slower

  saber->kataMove =
      LS_INVALID; // LS_INVALID - if set, player will execute this move when
                  // they press both attack buttons at the same time
  saber->lungeAtkMove = LS_INVALID; // LS_INVALID - if set, player will execute
                                    // this move when they crouch+fwd+attack
  saber->jumpAtkUpMove = LS_INVALID; // LS_INVALID - if set, player will execute
                                     // this move when they jump+attack
  saber->jumpAtkFwdMove =
      LS_INVALID; // LS_INVALID - if set, player will execute this move when
                  // they jump+fwd+attack
  saber->jumpAtkBackMove =
      LS_INVALID; // LS_INVALID - if set, player will execute this move when
                  // they jump+back+attack
  saber->jumpAtkRightMove =
      LS_INVALID; // LS_INVALID - if set, player will execute this move when
                  // they jump+rightattack
  saber->jumpAtkLeftMove =
      LS_INVALID; // LS_INVALID - if set, player will execute this move when
                  // they jump+left+attack
  ...
  saber->bladeStyle2Start =
      0; // 0 - if set, blades from this number and higher use the following
         // values (otherwise, they use the normal values already set)
  ...
  saber->trailStyle = 0; // 0 - default (0) is normal, 1 is a motion blur and 2
                         // is no trail at all (good for real-sword type mods)
  saber->g2MarksShader[0] =
      0; // none - if set, the game will use this shader for marks on enemies
         // instead of the default "gfx/damage/saberglowmark"
  saber->g2WeaponMarkShader[0] =
      0; // none - if set, the game will ry to project this shader onto the
         // weapon when it damages a person (good for a blood splatter on the
         // weapon)
  // saber->bladeShader = 0;				//none - if set, overrides the shader
  // used for the saber blade?
  // saber->trailShader = 0;				//none - if set, overrides the shader
  // used for the saber trail?
  saber->hitSound[0] = 0; // none - if set, plays one of these 3 sounds when
                          // saber hits a person - NOTE: must provide all 3!!!
  saber->hitSound[1] = 0; // none - if set, plays one of these 3 sounds when
                          // saber hits a person - NOTE: must provide all 3!!!
  saber->hitSound[2] = 0; // none - if set, plays one of these 3 sounds when
                          // saber hits a person - NOTE: must provide all 3!!!
  saber->blockSound[0] =
      0; // none - if set, plays one of these 3 sounds when saber/sword hits
         // another saber/sword - NOTE: must provide all 3!!!
  saber->blockSound[1] =
      0; // none - if set, plays one of these 3 sounds when saber/sword hits
         // another saber/sword - NOTE: must provide all 3!!!
  saber->blockSound[2] =
      0; // none - if set, plays one of these 3 sounds when saber/sword hits
         // another saber/sword - NOTE: must provide all 3!!!
  saber->bounceSound[0] =
      0; // none - if set, plays one of these 3 sounds when saber/sword hits a
         // wall and bounces off (must set bounceOnWall to 1 to use these sounds)
         // - NOTE: must provide all 3!!!
  saber->bounceSound[1] =
      0; // none - if set, plays one of these 3 sounds when saber/sword hits a
         // wall and bounces off (must set bounceOnWall to 1 to use these sounds)
         // - NOTE: must provide all 3!!!
  saber->bounceSound[2] =
      0; // none - if set, plays one of these 3 sounds when saber/sword hits a
         ...
         // - NOTE: must provide all 3!!!
  saber->blockEffect =
      0; // none - if set, plays this effect when the saber/sword hits another
         // saber/sword (instead of "saber/saber_block.efx")
  saber->hitPersonEffect =
      0; // none - if set, plays this effect when the saber/sword hits a person
         // (instead of "saber/blood_sparks_mp.efx")
  saber->hitOtherEffect =
      0; // none - if set, plays this effect when the saber/sword hits something
         // else damagable (instead of "saber/saber_cut.efx")
  saber->bladeEffect = 0; // none - if set, plays this effect at the blade tag

  // done in game (server-side code)
  saber->knockbackScale = 0; // 0 - if non-zero, uses damage done to calculate
                             // an appropriate amount of knockback
  saber->damageScale = 1.0f; // 1 - scale up or down the damage done by the
                             // saber
  saber->splashRadius = 0.0f; // 0 - radius of splashDamage
  saber->splashDamage = 0; // 0 - amount of splashDamage, 100% at a distance of
                           // 0, 0% at a distance = splashRadius
  saber->splashKnockback =
      0.0f; // 0 - amount of splashKnockback, 100% at a distance of 0, 0% at a
            // distance = splashRadius

  //===SECONDARY BLADES===================
  // done in cgame (client-side code)
  saber->trailStyle2 = 0; // 0 - default (0) is normal, 1 is a motion blur and 2
                          // is no trail at all (good for real-sword type mods)
  ...
  // saber->trailShader = 0;				//none - if set, overrides the shader
  // used for the saber trail?
  saber->hit2Sound[0] = 0; // none - if set, plays one of these 3 sounds when
                           // saber hits a person - NOTE: must provide all 3!!!
  saber->hit2Sound[1] = 0; // none - if set, plays one of these 3 sounds when
                           // saber hits a person - NOTE: must provide all 3!!!
  saber->hit2Sound[2] = 0; // none - if set, plays one of these 3 sounds when
                           // saber hits a person - NOTE: must provide all 3!!!
  saber->block2Sound[0] =
      0; // none - if set, plays one of these 3 sounds when saber/sword hits
         // another saber/sword - NOTE: must provide all 3!!!
  ...
  saber->block2Sound[2] =
      0; // none - if set, plays one of these 3 sounds when saber/sword hits
         // another saber/sword - NOTE: must provide all 3!!!
  saber->bounce2Sound[0] =
      0; // none - if set, plays one of these 3 sounds when saber/sword hits a
         // wall and bounces off (must set bounceOnWall to 1 to use these sounds)
         // - NOTE: must provide all 3!!!
  saber->bounce2Sound[1] =
      0; // none - if set, plays one of these 3 sounds when saber/sword hits a
         // wall and bounces off (must set bounceOnWall to 1 to use these sounds)
         // - NOTE: must provide all 3!!!
  saber->bounce2Sound[2] =
      0; // none - if set, plays one of these 3 sounds when saber/sword hits a
         // wall and bounces off (must set bounceOnWall to 1 to use these sounds)
         // - NOTE: must provide all 3!!!
  saber->blockEffect2 =
  ...
  // done in game (server-side code)
  saber->knockbackScale2 = 0; // 0 - if non-zero, uses damage done to calculate
                              // an appropriate amount of knockback
  saber->damageScale2 =
      1.0f; // 1 - scale up or down the damage done by the saber
  saber->splashRadius2 = 0.0f; // 0 - radius of splashDamage
  saber->splashDamage2 = 0; // 0 - amount of splashDamage, 100% at a distance of
                            // 0, 0% at a distance = splashRadius
  saber->splashKnockback2 =
      0.0f; // 0 - amount of splashKnockback, 100% at a distance of 0, 0% at a
            // distance = splashRadius
  //=========================================================================================================================================
}
...
static void Saber_ParseNumBlades(saberInfo_t *saber, const char **p) {
  ...
  if (n < 1 || n > MAX_BLADES) {
    Com_Error(ERR_DROP,
              "WP_SaberParseParms: saber %s has illegal number of blades (%d) "
              "max: %d",
              saber->name, n, MAX_BLADES);
    return;
  }
  saber->numBlades = n;
}
...
static void Saber_ParseSingleBladeThrowable(saberInfo_t *saber,
                                            const char **p) {
  int n;
  if (COM_ParseInt(p, &n)) {
    SkipRestOfLine(p);
...
static void Saber_ParseKataMove(saberInfo_t *saber, const char **p) {
  ...
  if (saberMove >= LS_INVALID && saberMove < LS_MOVE_MAX)
    saber->kataMove =
        saberMove; // LS_INVALID - if set, player will execute this move when
                   // they press both attack buttons at the same time
}
...
static void Saber_ParseG2WeaponMarkShader(saberInfo_t *saber, const char **p) {
  ...
  Q_strncpyz(saber->g2WeaponMarkShader, value,
             sizeof(saber->g2WeaponMarkShader));
  // NOTE: registers this on cgame side where it registers all client assets
}
...
static void Saber_ParseG2WeaponMarkShader2(saberInfo_t *saber, const char **p) {
  ...
  Q_strncpyz(saber->g2WeaponMarkShader2, value,
             sizeof(saber->g2WeaponMarkShader2));
  // NOTE: registers this on cgame side where it registers all client assets
}
...
static keywordHash_t *KeywordHash_Find(keywordHash_t *table[],
                                       const char *keyword) {
  keywordHash_t *key;
...
qboolean WP_SaberParseParms(const char *SaberName, saberInfo_t *saber,
  ...
  // check if we want to set the sabercolors or not (for if we're loading a
  // savegame)
  ...
  while (1) {
    ...
    if (!token[0]) {
      gi.Printf(
          S_COLOR_RED
          "ERROR: unexpected EOF while parsing '%s' (WP_SaberParseParms)\n",
          SaberName);
      return qfalse;
    }
  ...
  if (saber->type == SABER_SITH_SWORD) { // precache all the sith sword sounds
...
void WP_RemoveSaber(gentity_t *ent, int saberNum) {
  ...
  if (ent->client->ps.saberAnimLevel == SS_DUAL ||
      ent->client->ps.saberAnimLevel ==
          SS_STAFF) { // change to the style to the default
...
void WP_SetSaber(gentity_t *ent, int saberNum, const char *saberName) {
  ...
  if (Q_stricmp("none", saberName) == 0 ||
      Q_stricmp("remove", saberName) == 0) {
  ...
  WP_SaberParseParms(saberName,
                     &ent->client->ps.saber[saberNum]); // get saber info
  if (ent->client->ps.saber[saberNum].stylesLearned) {
    ent->client->ps.saberStylesKnown |=
        ent->client->ps.saber[saberNum].stylesLearned;
  }
  if (ent->client->ps.saber[saberNum].singleBladeStyle) {
    ent->client->ps.saberStylesKnown |=
        ent->client->ps.saber[saberNum].singleBladeStyle;
  }
  if (saberNum == 1 &&
      (ent->client->ps.saber[1].saberFlags &
       SFL_TWO_HANDED)) { // not allowed to use a 2-handed saber as second saber
    WP_RemoveSaber(ent, saberNum);
    return;
  }
  G_ModelIndex(ent->client->ps.saber[saberNum].model);
  WP_SaberInitBladeData(ent);
  if (saberNum == 1) { // now have 2 sabers
  ...
  */
  ...
  if (ent->client->ps.saber[saberNum]
          .stylesLearned) { // change to the style we're supposed to be using
    ent->client->ps.saberStylesKnown |=
        ent->client->ps.saber[saberNum].stylesLearned;
  }
  if (ent->client->ps.saber[saberNum].singleBladeStyle) {
...
void WP_SaberSetColor(gentity_t *ent, int saberNum, int bladeNum,
                      char *colorName) {
  if (!ent || !ent->client) {
    return;
  }
  ent->client->ps.saber[saberNum].blade[bladeNum].color =
      TranslateSaberColor(colorName);
}

extern void WP_SetSaberEntModelSkin(gentity_t *ent, gentity_t *saberent);
qboolean
WP_BreakSaber(gentity_t *ent, const char *surfName,
              saberType_t saberType) { // Make sure there *is* one specified and
                                       // not using dualSabers
  if (ent == NULL || ent->client == NULL) { // invalid ent or client
    return qfalse;
  }
  ...
  if (ent->health <= 0) { // not if they're dead
    return qfalse;
  }

  if (ent->client->ps.weapon != WP_SABER) { // not holding saber
    return qfalse;
  }

  if (ent->client->ps.dualSabers) { // FIXME: handle this?
    return qfalse;
  }

  if (!ent->client->ps.saber[0]
           .brokenSaber1) { // not breakable into another type of saber
    return qfalse;
  }
  ...
  if (Q_stricmpn("w_", surfName, 2) &&
      Q_stricmpn("saber", surfName,
                 5) // hack because using mod-community made saber
      && Q_stricmp("cylinder01",
                   surfName)) // hack because using mod-community made saber
  ...
  if (originalNumBlades <= 1) { // nothing to split off
                                // FIXME: handle this?
  } else {
    // remove saber[1], replace with replacementSaber2
    if (replacementSaber2) { // FIXME: 25% chance that it just breaks - just
                             // spawn the second saber piece and toss it away
                             // immediately, can't be picked up.
      // shouldn't be one in this hand, but just in case, remove it
      WP_RemoveSaber(ent, 1);
      WP_SetSaber(ent, 1, replacementSaber2);

      // put the remainder of the original saber's blade colors onto this
      // saber's blade(s)
      for (i = ent->client->ps.saber[0].numBlades; i < MAX_BLADES; i++) {
        ent->client->ps.saber[1]
            .blade[i - ent->client->ps.saber[0].numBlades]
            .color = colors[i];
      }
      broken = qtrue;
    }
...
void WP_SaberLoadParms(void) {
  ...
  fileCnt = gi.FS_GetFileList("ext_data/sabers", ".sab", saberExtensionListBuf,
    ...
    if (len == -1) {
      ...
      if (totallen + len >= MAX_SABER_DATA_SIZE) {
        G_Error("WP_SaberLoadParms: ran out of space before reading %s\n(you "
                "must make the .sab files smaller)",
                holdChar);
      }
