...
void ImperialProbe_MaintainHeight(void) {
  ...
  if (NPCS.NPC->enemy) {
    ...
    /*		VectorSet( endPos, NPC->r.currentOrigin[0], NPC->r.currentOrigin[1],
       NPC->r.currentOrigin[2] - 512 ); trap->Trace( &trace,
       NPC->r.currentOrigin, NULL, NULL, endPos, NPC->s.number, MASK_SOLID );
...
void ImperialProbe_Strafe(void) {
  ...
  // Close enough
  if (tr.fraction > 0.9f) {
    VectorMA(NPCS.NPC->client->ps.velocity, HUNTER_STRAFE_VEL * dir, right,
...
void ImperialProbe_Hunt(qboolean visible, qboolean advance) {
  ...
  speed = HUNTER_FORWARD_BASE_SPEED +
          HUNTER_FORWARD_MULTIPLIER * g_npcspskill.integer;
...
void ImperialProbe_FireBlaster(void) {
  ...
  trap->G2API_GetBoltMatrix(
      NPCS.NPC->ghoul2, 0, genBolt1, &boltMatrix, NPCS.NPC->r.currentAngles,
...
void NPC_Probe_Pain(gentity_t *self, gentity_t *attacker, int damage) {
      ...
      mod == MOD_DEMP2_ALT) // demp2 always messes them up real good
  {
    ...
    VectorSet(endPos, self->r.currentOrigin[0], self->r.currentOrigin[1],
              self->r.currentOrigin[2] - 128);
    trap->Trace(&trace, self->r.currentOrigin, NULL, NULL, endPos,
                self->s.number, MASK_SOLID, qfalse, 0, 0);
...
void ImperialProbe_Wait(void) {
  if (NPCS.NPCInfo->localState == LSTATE_DROP) {
    ...
    VectorSet(endPos, NPCS.NPC->r.currentOrigin[0],
              NPCS.NPC->r.currentOrigin[1], NPCS.NPC->r.currentOrigin[2] - 32);
    trap->Trace(&trace, NPCS.NPC->r.currentOrigin, NULL, NULL, endPos,
                NPCS.NPC->s.number, MASK_SOLID, qfalse, 0, 0);

    if (trace.fraction != 1.0f) {
      G_Damage(NPCS.NPC, NPCS.NPC->enemy, NPCS.NPC->enemy, NULL, NULL, 2000, 0,
               MOD_UNKNOWN);
    }
  }
