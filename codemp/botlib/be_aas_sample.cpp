...
// Changes Globals:		-
//===========================================================================
float AAS_BoxOriginDistanceFromPlane(vec3_t normal, vec3_t mins, vec3_t maxs,
                                     int side) {
  vec3_t v1, v2;
  int i;
...
aas_trace_t AAS_TraceClientBBox(vec3_t start, vec3_t end, int presencetype,
  ...
  while (1) {
    ...
    //		switch(plane->type)
    { /*FIXME: wtf doesn't this work? obviously the axial node planes aren't
         always facing positive!!!
...
int AAS_TraceAreas(vec3_t start, vec3_t end, int *areas, vec3_t *points,
  ...
  while (1) {
    ...
    //		switch(plane->type)
    { /*FIXME: wtf doesn't this work? obviously the node planes aren't always
         facing positive!!!
             //check for axial planes
             case PLANE_X:
             {
