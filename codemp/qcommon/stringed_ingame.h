...
#define sSE_DEBUGSTR_PREFIX                                                    \
  "[" // any string you want prefixing onto the debug versions of strings (to
      // spot hardwired english etc)
#define sSE_DEBUGSTR_SUFFIX "]" // ""

extern cvar_t *se_language;

// some needed text-equates, do not alter these under any circumstances !!!!
...
#define sSE_FILE_EXTENSION                                                     \
  ".st" // editor-only NEVER used ingame, but I wanted all extensions together
...
//
// general flag functions... (SEP_GetFlagMask() return should be used with
// SEP_GetFlags() return)
//
...
// note that so far the only place in the game that needs to know these is the
// font system so it can know how to
//	interpret char codes, for this reason I'm only exposing these simple
//bool queries...
//
inline SE_BOOL Language_IsRussian(void) {
  return (se_language && !Q_stricmp(se_language->string, "russian")) ? SE_TRUE
                                                                     : SE_FALSE;
}

inline SE_BOOL Language_IsPolish(void) {
  return (se_language && !Q_stricmp(se_language->string, "polish")) ? SE_TRUE
                                                                    : SE_FALSE;
}

inline SE_BOOL Language_IsKorean(void) {
  return (se_language && !Q_stricmp(se_language->string, "korean")) ? SE_TRUE
                                                                    : SE_FALSE;
}

inline SE_BOOL Language_IsTaiwanese(void) {
  return (se_language && !Q_stricmp(se_language->string, "taiwanese"))
             ? SE_TRUE
             : SE_FALSE;
}

inline SE_BOOL Language_IsJapanese(void) {
  return (se_language && !Q_stricmp(se_language->string, "japanese"))
             ? SE_TRUE
             : SE_FALSE;
}

inline SE_BOOL Language_IsChinese(void) {
  return (se_language && !Q_stricmp(se_language->string, "chinese")) ? SE_TRUE
                                                                     : SE_FALSE;
}

inline SE_BOOL Language_IsThai(void) {
  return (se_language && !Q_stricmp(se_language->string, "thai")) ? SE_TRUE
                                                                  : SE_FALSE;
}
