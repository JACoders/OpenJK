...
keywordArray_t CTokenizer::directiveKeywords[] = {
    "include",  DIR_INCLUDE,  "ifdef", DIR_IFDEF, "ifndef", DIR_IFNDEF,
    "endif",    DIR_ENDIF,    "else",  DIR_ELSE,  "define", DIR_DEFINE,
    "undefine", DIR_UNDEFINE, "",      TK_EOF,
};
...
LPCTSTR CDirectiveSymbol::GetValue() { return m_value; }
...
void CIntSymbol::Delete() { CSymbol::Delete(); }
...
int CIntSymbol::GetValue() { return m_value; }

//
// CSymbolTable
//

CSymbolTable::CSymbolTable() { Init(); }
...
void CSymbolTable::DiscardSymbols() {
  for (symbolmap_t::iterator isymbol = m_symbols.begin();
       isymbol != m_symbols.end(); ++isymbol) {
...
CParseStream *CParseStream::Create() { return NULL; }

void CParseStream::Delete() { delete this; }
...
bool CParseStream::NextChar(byte &theByte) { return false; }

long CParseStream::GetRemainingSize() { return 0; }

CParseStream *CParseStream::GetNext() { return m_next; }

void CParseStream::SetNext(CParseStream *next) { m_next = next; }

int CParseStream::GetCurLine() { return 0; }

void CParseStream::GetCurFilename(char **theBuff) { *theBuff = NULL; }

bool CParseStream::IsThisDefinition(void *theDefinition) { return false; }
...
CParsePutBack *CParsePutBack::Create(byte theByte, int curLine,
                                     LPCTSTR filename) {
...
int CParsePutBack::GetCurLine() { return m_curLine; }
...
bool CParseFile::Init(LPCTSTR filename, CTokenizer *tokenizer) {
  ...
  m_fileHandle = CreateFile(filename, dwAccess, dwShareMode, &sa, dwCreateFlag,
...
long CParseFile::GetRemainingSize() { return m_filesize - m_curByte; }
...
int CParseFile::GetCurLine() { return m_curLine; }
...
void CParseMemory::Delete() { delete this; }
...
long CParseMemory::GetRemainingSize() { return m_datasize - m_offset; }

int CParseMemory::GetCurLine() { return m_curLine; }

void CParseMemory::GetCurFilename(char **theBuff) { *theBuff = NULL; }
...
long CParseToken::GetRemainingSize() { return m_datasize - m_offset; }

int CParseToken::GetCurLine() { return m_curLine; }

void CParseToken::GetCurFilename(char **theBuff) { *theBuff = NULL; }
...
void CParseDefine::Delete() { CParseMemory::Delete(); }

void CParseDefine::Init(CDirectiveSymbol *definesymbol) {
  CParseMemory::Init((byte *)definesymbol->GetValue(),
                     strlen(definesymbol->GetValue()));
...
void CToken::SetNext(CToken *theToken) { m_next = theToken; }

CToken *CToken::GetNext() { return m_next; }

int CToken::GetType() { return TK_EOF; }
...
float CToken::GetFloatValue() { return 0.0; }
...
void CCharToken::Delete() { CToken::Delete(); }
...
int CCharToken::GetType() { return TK_CHAR; }
...
void CStringToken::Delete() { CToken::Delete(); }
...
int CStringToken::GetType() { return TK_STRING; }
...
void CIntToken::Delete() { CToken::Delete(); }
...
int CIntToken::GetType() { return TK_INT; }

int CIntToken::GetIntValue() { return m_value; }

float CIntToken::GetFloatValue() { return (float)m_value; }
...
void CFloatToken::Delete() { CToken::Delete(); }
...
int CFloatToken::GetType() { return TK_FLOAT; }

float CFloatToken::GetFloatValue() { return m_value; }
...
void CIdentifierToken::Delete() { CToken::Delete(); }
...
int CIdentifierToken::GetType() { return TK_IDENTIFIER; }
...
void CCommentToken::Delete() { CToken::Delete(); }
...
int CCommentToken::GetType() { return TK_COMMENT; }
...
void CUserToken::Delete() { CToken::Delete(); }
...
int CUserToken::GetType() { return m_value; }
...
void CUndefinedToken::Delete() { CToken::Delete(); }
...
void CTokenizerState::Delete() { delete this; }

CTokenizerState *CTokenizerState::GetNext() { return m_next; }
...
void CTokenizerState::SetNext(CTokenizerState *next) { m_next = next; }

bool CTokenizerState::Skipping() { return m_skip; }
...
void CTokenizerHolderState::Delete() { delete this; }
...
CKeywordTable::~CKeywordTable() { m_tokenizer->SetKeywords(m_holdKeywords); }
...
void CTokenizer::Error(int theError) {
  ...
  int i = 0;
  while ((errorMessages[i].m_tokenvalue != TKERR_USERERROR) &&
         (errorMessages[i].m_tokenvalue != theError)) {
    i++;
  }
  if ((errorMessages[i].m_tokenvalue == TKERR_USERERROR) &&
      (m_errors != NULL)) {
    i = 0;
    while ((m_errors[i].m_tokenvalue != TK_EOF) &&
           (m_errors[i].m_tokenvalue != theError)) {
      i++;
    }
...
void CTokenizer::PutBackToken(CToken *theToken, bool commented,
  ...
  if (commented) {
    ...
    if (addedChars != NULL) {
      ...
      CParseBlock *newBlock =
          CParseBlock::Create((byte *)addedChars, strlen(addedChars));
...
CToken *CTokenizer::GetToken(keywordArray_t *keywords, UINT onFlags,
                             UINT offFlags) {
...
CToken *CTokenizer::GetToEndOfLine(int tokenType) {
  // update, if you just want the whole line returned as a string, then allow a
  // much bigger size than
  ...
  while (true) {
    ...
    default:
      if (((theByte >= 'a') && (theByte <= 'z')) ||
          ((theByte >= 'A') && (theByte <= 'Z')) ||
          ((theByte == '_') &&
           ((m_flags & TKF_NOUNDERSCOREINIDENTIFIER) == 0))) {
        theToken = HandleIdentifier(theByte);
      } else if (((m_flags & TKF_NUMERICIDENTIFIERSTART) != 0) &&
                 (theByte >= '0') && (theByte <= '9')) {
        theToken = HandleIdentifier(theByte);
...
CToken *CTokenizer::TokenFromName(LPCTSTR name) {
  CDirectiveSymbol *defineSymbol =
      (CDirectiveSymbol *)m_defines.FindSymbol(name);
  ...
  for (int i = 1; i < MAX_IDENTIFIER_LENGTH; i++) {
    if (NextChar((byte &)theString[i])) {
      if (((theString[i] != '_') ||
           ((m_flags & TKF_NOUNDERSCOREINIDENTIFIER) == 0)) &&
          ((theString[i] != '-') ||
           ((m_flags & TKF_NODASHINIDENTIFIER) == 0))) {
...
void CTokenizer::SetFlags(UINT flags) { m_flags = flags; }

UINT CTokenizer::GetFlags() { return m_flags; }
...
CToken *CTokenizer::HandleDirective() {
  ...
  case DIR_IFDEF:
    ...
    state = CTokenizerState::Create(
        m_defines.FindSymbol(theToken->GetStringValue()) == NULL);
    theToken->Delete();
    state->SetNext(m_state);
    m_state = state;
    break;
...
CSymbolLookup *CSymbolLookup::GetNext() { return m_sibling; }

void CSymbolLookup::SetNext(CSymbolLookup *next) { m_sibling = next; }

void CSymbolLookup::SetParent(CSymbolLookup *parent) { m_parent = parent; }

void CSymbolLookup::SetValue(int value) { m_value = value; }

int CSymbolLookup::GetValue() { return m_value; }

byte CSymbolLookup::GetByte() { return m_byte; }

CSymbolLookup **CSymbolLookup::GetChildAddress() { return &m_child; }

CSymbolLookup *CSymbolLookup::GetChild() { return m_child; }
