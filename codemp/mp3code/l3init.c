...
static const float Ci[8] = {-0.6f, -0.535f, -0.33f, -0.185f, -0.095f, -0.041f, -0.0142f, -0.0037f};
				...
				for (sflen = 0; sflen < 6; sflen++) {
					for (sf = 0; sf < (n - 1); sf++, k++) {
						if (sf == 0) {
							...
							lr2[intensity_scale][ms_mode][k][0] = (float)(ms_factor[ms_mode] * pow(t, (sf + 1) / 2));
							lr2[intensity_scale][ms_mode][k][1] = ms_factor[ms_mode] * 1.0f;
						} else {
							lr2[intensity_scale][ms_mode][k][0] = ms_factor[ms_mode] * 1.0f;
							lr2[intensity_scale][ms_mode][k][1] = (float)(ms_factor[ms_mode] * pow(t, sf / 2));
						}
					}
