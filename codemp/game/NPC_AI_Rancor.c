...
extern void G_Dismember(gentity_t *ent, gentity_t *enemy, vec3_t point,
                        int limbType, float limbRollBase, float limbPitchBase,
                        int deathAnim, qboolean postDeath);
extern float NPC_EntRangeFromBolt(gentity_t *targEnt, int boltIndex);
extern int NPC_GetEntsNearBolt(int *radiusEnts, float radius, int boltIndex,
                               vec3_t boltOrg);
...
void Rancor_DropVictim(gentity_t *self) {
  ...
  if (self->activator) {
    if (self->activator->client) {
      ...
      SetClientViewAngle(self->activator,
                         self->activator->client->ps.viewangles);
      self->activator->r.currentAngles[PITCH] =
          self->activator->r.currentAngles[ROLL] = 0;
    ...
    if (self->activator->health <= 0) {
      ...
      {                         // never free player
        if (self->count == 1) { // in my hand, just drop them
          if (self->activator->client) {
            self->activator->client->ps.legsTimer =
                self->activator->client->ps.torsoTimer = 0;
            // FIXME: ragdoll?
          }
        } else {
          if (self->activator->client) {
            self->activator->client->ps.eFlags |=
                EF_NODRAW; // so his corpse doesn't drop out of me...
          }
...
void Rancor_Swing(qboolean tryGrab) {
  ...
  numEnts = NPC_GetEntsNearBolt(
      radiusEntNums, radius, NPCS.NPC->client->renderInfo.handRBolt, boltOrg);
  ...
  for (i = 0; i < numEnts; i++) {
    ...
    if (radiusEnt == NPCS.NPC) { // Skip the rancor ent
      continue;
    }
    ...
    if ((radiusEnt->client->ps.eFlags2 &
         EF2_HELD_BY_MONSTER)) { // can't be one already being held
      continue;
    }
    ...
    if (DistanceSquared(radiusEnt->r.currentOrigin, boltOrg) <= radiusSquared) {
      if (tryGrab &&
          NPCS.NPC->count !=
              1 // don't have one in hand or in mouth already - FIXME: allow one
                // in hand and any number in mouth!
        ...
        if (NPCS.NPC->count == 2) { // have one in my mouth, remove him
        ...
        G_Sound(radiusEnt, CHAN_AUTO,
                G_SoundIndex("sound/chars/rancor/swipehit.wav"));
        // actually push the enemy
        /*
        //VectorSubtract( radiusEnt->r.currentOrigin, boltOrg, pushDir );
        VectorSubtract( radiusEnt->r.currentOrigin, NPC->r.currentOrigin,
        ...
        */
        ...
        if (radiusEnt->client->NPC_class != CLASS_RANCOR &&
            radiusEnt->client->NPC_class != CLASS_ATST) {
          G_Damage(radiusEnt, NPCS.NPC, NPCS.NPC, vec3_origin,
                   radiusEnt->r.currentOrigin, Q_irand(25, 40),
...
void Rancor_Smash(void) {
  ...
  AddSoundEvent(NPCS.NPC, NPCS.NPC->r.currentOrigin, 512, AEL_DANGER,
                qfalse); //, qtrue );

  numEnts = NPC_GetEntsNearBolt(
      radiusEntNums, radius, NPCS.NPC->client->renderInfo.handLBolt, boltOrg);
  ...
  for (i = 0; i < numEnts; i++) {
    ...
    if (distSq <= radiusSquared) {
      ...
      if (distSq < halfRadSquared) { // close enough to do damage, too
        G_Damage(radiusEnt, NPCS.NPC, NPCS.NPC, vec3_origin,
                 radiusEnt->r.currentOrigin, Q_irand(10, 25),
                 DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK, MOD_MELEE);
      }
...
void Rancor_Bite(void) {
  ...
  numEnts = NPC_GetEntsNearBolt(radiusEntNums, radius,
                                NPCS.NPC->client->renderInfo.crotchBolt,
                                boltOrg); // was gutBolt?
  ...
  for (i = 0; i < numEnts; i++) {
    ...
    if (DistanceSquared(radiusEnt->r.currentOrigin, boltOrg) <= radiusSquared) {
      G_Damage(radiusEnt, NPCS.NPC, NPCS.NPC, vec3_origin,
               radiusEnt->r.currentOrigin, Q_irand(15, 30),
               DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK, MOD_MELEE);
      if (radiusEnt->health <= 0 &&
          radiusEnt->client) { // killed them, chance of dismembering
        if (!Q_irand(0, 1)) {  // bite something off
          int hitLoc = Q_irand(G2_MODELPART_HEAD, G2_MODELPART_RLEG);
          if (hitLoc == G2_MODELPART_HEAD) {
            NPC_SetAnim(radiusEnt, SETANIM_BOTH, BOTH_DEATH17,
                        SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
          ...
          G_Dismember(radiusEnt, NPCS.NPC, radiusEnt->r.currentOrigin, hitLoc,
                      90, 0, radiusEnt->client->ps.torsoAnim, qtrue);
          // G_DoDismemberment( radiusEnt, radiusEnt->r.currentOrigin,
...
void Rancor_Attack(float distance, qboolean doCharge) {
  ...
  if (!TIMER_Exists(NPCS.NPC, "attacking")) {
    if (NPCS.NPC->count == 2 && NPCS.NPC->activator) {
    } else if (NPCS.NPC->count == 1 && NPCS.NPC->activator) { // holding enemy
      if (NPCS.NPC->activator->health > 0 && Q_irand(0, 1)) { // quick bite
        ...
        if (NPCS.NPC->activator->health > 0 && NPCS.NPC->activator->client) {
          ...
          NPC_SetAnim(NPCS.NPC->activator, SETANIM_TORSO, BOTH_FALLDEATH1,
                      SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
          if (NPCS.NPC->activator->NPC) { // no more thinking for you
            TossClientItems(NPCS.NPC);
            NPCS.NPC->activator->NPC->nextBStateThink = Q3_INFINITE;
          }
        }
      }
    } else if (NPCS.NPC->enemy->health > 0 && doCharge) { // charge
      vec3_t fwd, yawAng;
      ...
      NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_MELEE2,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      TIMER_Set(NPCS.NPC, "attack_dmg", 1250);
    } else if (!Q_irand(0, 1)) { // smash
      NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_MELEE1,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    ...
    TIMER_Set(NPCS.NPC, "attacking",
              NPCS.NPC->client->ps.legsTimer + Q_flrand(0.0f, 1.0f) * 200);
  }
  ...
  if (TIMER_Done2(NPCS.NPC, "attack_dmg", qtrue)) {
    ...
    case BOTH_MELEE1:
      Rancor_Smash();
      G_GetBoltPosition(NPCS.NPC, NPCS.NPC->client->renderInfo.handLBolt,
                        shakePos, 0);
    ...
    case BOTH_ATTACK1:
      if (NPCS.NPC->count == 1 && NPCS.NPC->activator) {
        G_Damage(NPCS.NPC->activator, NPCS.NPC, NPCS.NPC, vec3_origin,
                 NPCS.NPC->activator->r.currentOrigin, Q_irand(25, 40),
                 DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK, MOD_MELEE);
        if (NPCS.NPC->activator->health <= 0) { // killed him
          // make it look like we bit his head off
          // NPC->activator->client->dismembered = qfalse;
          G_Dismember(NPCS.NPC->activator, NPCS.NPC,
                      NPCS.NPC->activator->r.currentOrigin, G2_MODELPART_HEAD,
                      90, 0, NPCS.NPC->activator->client->ps.torsoAnim, qtrue);
          // G_DoDismemberment( NPC->activator, NPC->activator->r.currentOrigin,
        ...
        G_Sound(NPCS.NPC->activator, CHAN_AUTO,
                G_SoundIndex("sound/chars/rancor/chomp.wav"));
      }
      break;
    case BOTH_ATTACK2:
    ...
    case BOTH_ATTACK3:
      if (NPCS.NPC->count == 1 && NPCS.NPC->activator) {
        ...
        if (NPCS.NPC->activator->client) {
          // NPC->activator->client->dismembered = qfalse;
          G_Dismember(NPCS.NPC->activator, NPCS.NPC,
                      NPCS.NPC->activator->r.currentOrigin, G2_MODELPART_WAIST,
                      90, 0, NPCS.NPC->activator->client->ps.torsoAnim, qtrue);
          // G_DoDismemberment( NPC->activator, NPC->enemy->r.currentOrigin,
          // MOD_SABER, 1000, HL_WAIST, qtrue );
        }
        // KILL
        G_Damage(NPCS.NPC->activator, NPCS.NPC, NPCS.NPC, vec3_origin,
                 NPCS.NPC->activator->r.currentOrigin,
    ...
    case BOTH_ATTACK2:
      break;
    case BOTH_ATTACK3:
      if (NPCS.NPC->count == 1 && NPCS.NPC->activator) { // swallow victim
        G_Sound(NPCS.NPC->activator, CHAN_AUTO,
                G_SoundIndex("sound/chars/rancor/chomp.wav"));
        ...
        if (NPCS.NPC->activator->health > 0) {
          ...
          G_Dismember(NPCS.NPC->activator, NPCS.NPC,
                      ...
                      90, 0, NPCS.NPC->activator->client->ps.torsoAnim, qtrue);
          // G_DoDismemberment( NPC->activator, NPC->enemy->r.currentOrigin,
          // MOD_SABER, 1000, HL_WAIST, qtrue ); KILL
          G_Damage(NPCS.NPC->activator, NPCS.NPC, NPCS.NPC, vec3_origin,
                   NPCS.NPC->activator->r.currentOrigin,
                   NPCS.NPC->enemy->health + 10,
        ...
        if (NPCS.NPC->activator
                ->client) { //*sigh*, can't get tags right, just remove them?
    ...
    if (NPCS.NPC->client->ps.legsTimer >= 1200 &&
        NPCS.NPC->client->ps.legsTimer <= 1350) {
      if (Q_irand(0, 2)) {
        Rancor_Swing(qfalse);
      } else {
        Rancor_Swing(qtrue);
      }
    } else if (NPCS.NPC->client->ps.legsTimer >= 1100 &&
               NPCS.NPC->client->ps.legsTimer <= 1550) {
      Rancor_Swing(qtrue);
    }
  }
...
void Rancor_Combat(void) {
  if (NPCS.NPC->count) { // holding my enemy
...

  {
    ...
    distance =
        Distance(NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin);
    advance = (qboolean)(
        distance > (NPCS.NPC->r.maxs[0] + MIN_DISTANCE) ? qtrue : qfalse);
    doCharge = qfalse;

    if (advance) { // have to get closer
      vec3_t yawOnlyAngles;
      VectorSet(yawOnlyAngles, 0, NPCS.NPC->r.currentAngles[YAW], 0);
      if (NPCS.NPC->enemy->health > 0 && fabs(distance - 250) <= 80 &&
...
void NPC_Rancor_Pain(gentity_t *self, gentity_t *attacker, int damage) {
  ...
  if (attacker && attacker->inuse && attacker != self->enemy &&
      !(attacker->flags & FL_NOTARGET)) {
    if (!self->count) {
      if ((!attacker->s.number && !Q_irand(0, 3)) || !self->enemy ||
          self->enemy->health == 0 ||
          (self->enemy->client &&
          ...
          (self->NPC && self->NPC->consecutiveBlockedMoves >= 10 &&
           DistanceSquared(attacker->r.currentOrigin, self->r.currentOrigin) <
               DistanceSquared(
                   ...
                   self->r.currentOrigin))) { // if my enemy is dead (or
                                              // attacked by player) and I'm not
                                              // still holding/eating someone,
    ...
    if (!Rancor_CheckRoar(self)) {
      if (self->client->ps.legsAnim != BOTH_MELEE1 &&
          ...
          self->client->ps.legsAnim !=
              BOTH_ATTACK2) { // cant interrupt one of the big attack anims
        /*
        if ( self->count != 1
        ...
        { // if going to bite our victim, only victim can interrupt that anim
          if (self->health > 100 || hitByRancor) {
            ...
            TIMER_Set(self, "takingPain",
                      self->client->ps.legsTimer + Q_irand(0, 500));
...
void Rancor_CheckDropVictim(void) {
  ...
  VectorSet(mins, NPCS.NPC->activator->r.mins[0] - 1,
            NPCS.NPC->activator->r.mins[1] - 1, 0);
  VectorSet(maxs, NPCS.NPC->activator->r.maxs[0] + 1,
            NPCS.NPC->activator->r.maxs[1] + 1, 1);
  VectorSet(start, NPCS.NPC->activator->r.currentOrigin[0],
            NPCS.NPC->activator->r.currentOrigin[1],
            NPCS.NPC->activator->r.absmin[2]);
  VectorSet(end, NPCS.NPC->activator->r.currentOrigin[0],
            NPCS.NPC->activator->r.currentOrigin[1],
            NPCS.NPC->activator->r.absmax[2] - 1);

  trap->Trace(&trace, start, mins, maxs, end, NPCS.NPC->activator->s.number,
              NPCS.NPC->activator->clipmask, qfalse, 0, 0);
...
void Rancor_Crush(void) {
  ...
  if (!NPCS.NPC || !NPCS.NPC->client ||
      NPCS.NPC->client->ps.groundEntityNum >=
          ENTITYNUM_WORLD) { // nothing to crush
    return;
  }

  crush = &g_entities[NPCS.NPC->client->ps.groundEntityNum];
  if (crush->inuse && crush->client &&
      !crush->localAnimIndex) { // a humanoid, smash them good.
    G_Damage(crush, NPCS.NPC, NPCS.NPC, NULL, NPCS.NPC->r.currentOrigin, 200, 0,
             MOD_CRUSH);
  }
}
...
void NPC_BSRancor_Default(void) {
  AddSightEvent(NPCS.NPC, NPCS.NPC->r.currentOrigin, 1024, AEL_DANGER_GREAT,
                50);

  Rancor_Crush();

  NPCS.NPC->client->ps.eFlags2 &= ~(EF2_USE_ALT_ANIM | EF2_GENERIC_NPC_FLAG);
  if (NPCS.NPC->count) { // holding someone
    NPCS.NPC->client->ps.eFlags2 |= EF2_USE_ALT_ANIM;
    if (NPCS.NPC->count == 2) { // in my mouth
  ...
  if (TIMER_Done2(NPCS.NPC, "clearGrabbed", qtrue)) {
    Rancor_DropVictim(NPCS.NPC);
             ...
             NPCS.NPC->count == 1 && NPCS.NPC->activator) {
    if (!Q_irand(0, 3)) {
  ...
  if (!TIMER_Done(
          NPCS.NPC,
          "rageTime")) { // do nothing but roar first time we see an enemy
    AddSoundEvent(NPCS.NPC, NPCS.NPC->r.currentOrigin, 1024, AEL_DANGER_GREAT,
                  qfalse); //, qfalse );
    NPC_FaceEnemy(qtrue);
    return;
  }
  if (NPCS.NPC->enemy) {
    ...
    NPC->enemy->client->NPC_class == CLASS_JAWA )//enemy is a lowly jawa or
    ugnaught
            && NPC->enemy->enemy != NPC//enemy's enemy is not me
            && (!NPC->enemy->enemy || !NPC->enemy->enemy->client ||
    NPC->enemy->enemy->client->NPC_class!=CLASS_RANCOR) )//enemy's enemy is not
    a client or is not a rancor (which is as scary as me anyway)
    ...
    */
    if (TIMER_Done(NPCS.NPC, "angrynoise")) {
      G_Sound(NPCS.NPC, CHAN_AUTO,
              G_SoundIndex(
                  va("sound/chars/rancor/misc/anger%d.wav", Q_irand(1, 3))));

      TIMER_Set(NPCS.NPC, "angrynoise", Q_irand(5000, 10000));
    } else {
      AddSoundEvent(NPCS.NPC, NPCS.NPC->r.currentOrigin, 512, AEL_DANGER_GREAT,
                    qfalse); //, qfalse );
    }
    if (NPCS.NPC->count == 2 &&
    ...
    if (NPCS.NPC->enemy->client &&
      ...
      if (ValidEnemy(NPCS.NPC->enemy) == qfalse) {
        TIMER_Remove(NPCS.NPC,
                     "lookForNewEnemy"); // make them look again right now
        if (!NPCS.NPC->enemy->inuse ||
            level.time - NPCS.NPC->enemy->s.time >
                Q_irand(10000,
      ...
      if (TIMER_Done(NPCS.NPC, "lookForNewEnemy")) {
        gentity_t *newEnemy,
            *sav_enemy = NPCS.NPC->enemy; // FIXME: what about NPC->lastEnemy?
        NPCS.NPC->enemy = NULL;
        newEnemy = NPC_CheckEnemy(NPCS.NPCInfo->confusionTime < level.time,
                                  qfalse, qfalse);
    ...
    if (TIMER_Done(NPCS.NPC, "idlenoise")) {
      G_Sound(
          NPCS.NPC, CHAN_AUTO,
          G_SoundIndex(va("sound/chars/rancor/snort_%d.wav", Q_irand(1, 2))));

      TIMER_Set(NPCS.NPC, "idlenoise", Q_irand(2000, 4000));
      AddSoundEvent(NPCS.NPC, NPCS.NPC->r.currentOrigin, 384, AEL_DANGER,
                    qfalse); //, qfalse );
    }
