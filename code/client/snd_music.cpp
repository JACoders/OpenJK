...
typedef struct {
  sstring_t sNextFile;
  sstring_t sNextMark; // blank if used for an explore piece, name of marker
                       // point to enter new file at
...
typedef std::map<sstring_t, MusicFile_t>
    MusicData_t; // string is "explore", "action", "boss" etc
MusicData_t *MusicData = NULL;
// there are now 2 of these, because of the new "uses" keyword...
//
sstring_t gsLevelNameForLoad; // eg "kejim_base", formed from literal BSP name,
                              // but also used as dir name for music paths
sstring_t
    gsLevelNameForCompare; // eg "kejim_base", formed from literal BSP name, but
                           // also used as dir name for music paths
sstring_t
    gsLevelNameForBossLoad; // eg "kejim_base', special case for enabling boss
                            // music to come from a different dir - sigh....
...
template <typename... Tail> static std::string build_string(Tail... tail) {
...
// only use during parse, not run-time use, and bear in mid that data is zapped
// after error message, so exit any loops immediately
//
static void Music_Parse_Error(gsl::czstring filename,
                              const std::string &error) {
  std::string message =
      build_string(S_COLOR_RED "Error parsing music data (in \"", filename,
                   "\"):\n", error, "\n");
...
// the 2nd param here is pretty kludgy (sigh), and only used for testing for the
// "boss" type. Unfortunately two of the places that calls this doesn't have
// much other access to the state other than
//	a string, not an enum, so for those cases they only pass in BOSS or
//EXPLORE, so don't rely on it totally.
//
static const char *Music_BuildFileName(const char *psFileNameBase,
...
const char *Music_BaseStateToString(MusicState_e eMusicState,
                                    qboolean bDebugPrintQuery /* = qfalse */) {
  switch (eMusicState) {
  ...
  case eBGRNDTRACK_SILENCE:
    return "silence"; // not used in this module, but snd_dma uses it now it's
                      // de-static'd
...
static qboolean Music_ParseMusic(gsl::czstring filename,
                                 const CGenericParser2 &Parser,
                                 MusicData_t *MusicData,
                                 const CGPGroup &pgMusicFiles,
                                 const gsl::cstring_view &psMusicName,
                                 const gsl::cstring_view &psMusicNameKey,
                                 MusicState_e eMusicState) {
  bool bReturn = false;
  ...
  if (pgMusicFile) {
    ...
    //
    // (read entry points first, so I can check exit points aren't too close in
    // time)
    //
    ...
    if (pEntryGroup) {
      ...
      for (auto &prop : pEntryGroup->GetProperties()) {
        // if( Q::substr( prop.GetName(), 0, sKEY_MARKER.size() ) == sKEY_MARKER
        // )	// for now, assume anything is a marker
        {
          MusicFile.MusicEntryTimes[prop.GetName()] =
              Q::svtoi(prop.GetTopValue());
          bEntryFound = true;
        }
      }
    }
    ...
    for (auto &group : pgMusicFile->GetSubGroups()) {
      ...
      if (groupName == sKEY_ENTRY) {
        ...
        int iThisExitPointIndex =
            MusicFile.MusicExitPoints
                .size(); // must eval this first, so unaffected by push_back etc
        //
        ...
        for (auto &prop : group.GetProperties()) {
          ...
          if (key == sKEY_NEXTFILE) {
            ...
            // new check, don't keep this this exit point if it's within 1.5
            // seconds either way of an entry point...
            //
            ...
            for (auto &item : MusicFile.MusicEntryTimes) {
              ...
              if (Q_fabs(fThisEntryTime - MusicExitTime.fTime) < 1.5f) {
                //								bTooCloseToEntryPoint
                //= true;	// not sure about this, ignore for now
                break;
              }
            }
        ...
        case eBGRNDTRACK_EXPLORE:
          if (iNumExitPoints > iMAX_EXPLORE_TRANSITIONS) {
            Music_Parse_Error(filename,
                              build_string("\"", psMusicName, "\" has > ",
                                           iMAX_EXPLORE_TRANSITIONS, " ",
                                           psMusicNameKey,
                                           " transitions defined!\n"));
            return qfalse;
          }
        ...
        case eBGRNDTRACK_DEATH:
          ...
          Music_Parse_Error(
              filename,
              build_string("\"", psMusicName, "\" has ", psMusicNameKey,
    ...
    // for now, assume everything was ok unless some obvious things are
    // missing...
    //
    bReturn = true;
    ...
    if (eMusicState != eBGRNDTRACK_BOSS && eMusicState != eBGRNDTRACK_DEATH) {
      if (!bEntryFound) {
        Music_Parse_Error(filename,
                          build_string("Unable to find subgroup \"", sKEY_ENTRY,
                                       "\" in group \"", psMusicName, "\"\n"));
        bReturn = false;
      }
      if (!bExitFound) {
        Music_Parse_Error(filename,
                          build_string("Unable to find subgroup \"", sKEY_EXIT,
                                       "\" in group \"", psMusicName, "\"\n"));
        bReturn = false;
      }
    }
    ...
    Music_Parse_Error(filename,
                      build_string("Unable to find musicfiles entry \"",
                                   psMusicName, "\"\n"));
  }
...
//
// This just initialises the Lucas music structs so the background music player
...
static qboolean Music_ParseLeveldata(gsl::czstring psLevelName) {
  ...
  //
  if (MusicData->size() &&
      !Q_stricmp(psLevelName, gsLevelNameForCompare.c_str())) {
    return qtrue;
  }

  MusicData->clear();

  // shorten level name to MAX_QPATH so sstring's assignment assertion is
  // satisfied.
  char sLevelName[MAX_QPATH];
  Q_strncpyz(sLevelName, psLevelName, sizeof(sLevelName));

  gsLevelNameForLoad =
      sLevelName; // harmless to init here even if we fail to parse dms.dat file
  gsLevelNameForCompare =
      sLevelName; // harmless to init here even if we fail to parse dms.dat file
  ...
  if (!Parser.Parse(filename)) {
    ...
    if (!pgMusicFiles) {
      Music_Parse_Error(filename, build_string("Unable to find subgroup \"",
                                               sKEY_MUSICFILES, "\"\n"));
    } else {
      ...
      if (!pgLevelMusic) {
        ...
        //
        int steps = 0;
        gsl::cstring_view searchName{&sLevelName[0],
                                     &sLevelName[strlen(&sLevelName[0])]};
        ...
        while (!searchName.empty() && steps < sanityLimit) {
          ...
          if (pgThisLevelMusic) {
            const CGPProperty *pValue =
                pgThisLevelMusic->FindProperty(sKEY_USES);
            if (pValue) {
        ...
        if (!pgThisLevelMusic || steps >= sanityLimit) {
          Music_Parse_Warning(build_string("Unable to find entry for \"",
                                           sLevelName, "\" in \"", filename,
                                           "\"\n"));
        } else {
          ...
          bReturn = qtrue; // defualt to ON now, so I can turn it off if
                           // "useboss" fails

          if (!psName_UseBoss.empty()) {
            const CGPGroup *pgLevelMusicOfBoss =
                pgLevelMusic->FindSubGroup(psName_UseBoss);
            if (!pgLevelMusicOfBoss) {
              Music_Parse_Error(
                  filename,
                  build_string("Unable to find 'useboss' entry \"",
                               psName_UseBoss, "\"\n", psName_UseBoss));
              bReturn = qfalse;
            } else {
              const CGPProperty *pValueBoss =
                  pgLevelMusicOfBoss->FindProperty(sKEY_BOSS);
              if (!pValueBoss) {
                Music_Parse_Error(filename,
                                  build_string("'useboss' \"", psName_UseBoss,
                                               "\" has no \"boss\" entry!\n"));
                bReturn = qfalse;
              } else {
          ...
          // done this way in case I want to conditionally pass any bools
          // depending on music type...
          //
          if (bReturn && psName_Explore) {
            bReturn = Music_ParseMusic(filename, Parser, MusicData,
                                       *pgMusicFiles, psName_Explore,
                                       sKEY_EXPLORE, eBGRNDTRACK_EXPLORE);
          }
          if (bReturn && psName_Action) {
            bReturn = Music_ParseMusic(filename, Parser, MusicData,
                                       *pgMusicFiles, psName_Action,
                                       sKEY_ACTION, eBGRNDTRACK_ACTION);
          }
          if (bReturn && psName_Boss) {
            bReturn =
                Music_ParseMusic(filename, Parser, MusicData, *pgMusicFiles,
                                 psName_Boss, sKEY_BOSS, eBGRNDTRACK_BOSS);
          }
          if (bReturn /*&& psName_Death*/) // LAST MINUTE HACK!!, always force
                                           // in some death music!!!!
          {
            // bReturn = Music_ParseMusic(Parser, MusicData, pgMusicFiles,
            // psName_Death,	sKEY_DEATH,   eBGRNDTRACK_DEATH);
  ...
  if (bReturn) {
    ...
    //
    for (MusicData_t::iterator itMusicData = MusicData->begin();
         itMusicData != MusicData->end(); ++itMusicData) {
      ...
      //
      MusicState_e eMusicState = !Q_stricmp(psMusicStateType, "boss")
                                     ? eBGRNDTRACK_BOSS
      ...
      // check music exists...
      //
      const char *psMusicFileName =
          Music_BuildFileName(MusicFile.sFileNameBase.c_str(), eMusicState);
      if (!S_FileExists(psMusicFileName)) {
        Music_Parse_Error(
            filename,
            build_string("Music file \"", psMusicFileName, "\" not found!\n"));
        return qfalse; // have to return, because music data destroyed now
      }

      // check all transition music pieces exist, and that entry points into new
      // pieces after transitions also exist...
      //
      for (size_t iExitPoint = 0; iExitPoint < MusicFile.MusicExitPoints.size();
           iExitPoint++) {
        MusicExitPoint_t &MusicExitPoint =
            MusicFile.MusicExitPoints[iExitPoint];

        const char *psTransitionFileName =
            Music_BuildFileName(MusicExitPoint.sNextFile.c_str(), eMusicState);
        if (!S_FileExists(psTransitionFileName)) {
          Music_Parse_Error(filename,
                            build_string("Transition file \"",
                                         psTransitionFileName, "\" (entry \"",
                                         MusicExitPoint.sNextFile.c_str(),
                                         "\" ) not found!\n"));
          return qfalse; // have to return, because music data destroyed now
        }
        ...
        if (strlen(psNextMark)) // always NZ ptr
        {
          ...
          //
          assert(!strcmp(psMusicStateType,
                         Music_BaseStateToString(eBGRNDTRACK_ACTION)
                             ? Music_BaseStateToString(eBGRNDTRACK_ACTION)
                             : ""));
          //
          // does this marker exist in the explore piece?
          //
          MusicData_t::iterator itExploreMusicData =
              MusicData->find(Music_BaseStateToString(eBGRNDTRACK_EXPLORE));
          if (itExploreMusicData != MusicData->end()) {
            ...
            if (!MusicFile_Explore.MusicEntryTimes.count(psNextMark)) {
              Music_Parse_Error(
                  filename,
                  build_string("Unable to find entry point \"", psNextMark,
                               "\" in description for \"",
...
//
static MusicFile_t *Music_GetBaseMusicFile(
    const char *psMusicState) // where psMusicState is (eg) "explore", "action"
                              // or "boss"
{
...
qboolean Music_DynamicDataAvailable(const char *psDynamicMusicLabel) {
  ...
  Q_strncpyz(sLevelName,
             COM_SkipPath(const_cast<char *>(
                 (psDynamicMusicLabel && psDynamicMusicLabel[0])
                     ? psDynamicMusicLabel
                     : gsLevelNameFromServer.c_str())),
             sizeof(sLevelName));
  Q_strlwr(sLevelName);

  if (strlen(sLevelName)) // avoid error messages when there's no music waiting
                          // to be played and we try and restart it...
  {
...
const char *Music_GetFileNameForState(MusicState_e eMusicState) {
  ...
  case eBGRNDTRACK_DEATH:
    ...
    if (pMusicFile) {
      return Music_BuildFileName(pMusicFile->sFileNameBase.c_str(),
                                 eMusicState);
    }
    break;
  ...
  case eBGRNDTRACK_ACTIONTRANS3:
    ...
    if (pMusicFile) {
      ...
      if (iTransNum < pMusicFile->MusicExitPoints.size()) {
        return Music_BuildFileName(
            pMusicFile->MusicExitPoints[iTransNum].sNextFile.c_str(),
            eMusicState);
      }
    }
    break;
...
qboolean Music_StateCanBeInterrupted(MusicState_e eMusicState,
                                     MusicState_e eProposedMusicState) {
  // death music can interrupt anything...
  //
  if (eProposedMusicState == eBGRNDTRACK_DEATH)
    return qtrue;
  //
  // ... and can't be interrupted once started...(though it will
  // internally-switch to silence at the end, rather than loop)
  //
  if (eMusicState == eBGRNDTRACK_DEATH) {
    return qfalse;
  }

  // boss music can interrupt anything (other than death, but that's already
  // handled above)...
  //
...
// returns qtrue if music is allowed to transition out of current state, based
// on current play position... (doesn't bother returning final state after
// transition (eg action->transition->explore) becuase it's fairly obvious)
//
// supply:
//
...
qboolean Music_AllowedToTransition(float fPlayingTimeElapsed,
  ...
  const float fTimeEpsilon =
      0.3f; // arb., how close we have to be to an exit point to take it.
            //		if set too high then music change is sloppy
            //		if set too low[/precise] then we might miss an exit if client
            //fps is poor
  ...
  if (pMusicFile && !pMusicFile->MusicExitTimes.empty()) {
    ...
    // since a MusicExitTimes_t item is a sorted array, we can use the
    // equal_range algorithm...
    //
    std::pair<MusicExitTimes_t::iterator, MusicExitTimes_t::iterator> itp =
        equal_range(pMusicFile->MusicExitTimes.begin(),
                    pMusicFile->MusicExitTimes.end(), T);
    if (itp.first != pMusicFile->MusicExitTimes.begin())
      itp.first--; // encompass the one before, in case we've just missed an
                   // exit point by < fTimeEpsilon
    if (itp.second != pMusicFile->MusicExitTimes.end())
      itp.second++; // increase range to one beyond, so we can do normal STL
                    // being/end looping below
    for (MusicExitTimes_t::iterator it = itp.first; it != itp.second; ++it) {
      ...
      if (Q_fabs(pExitTime->fTime - fPlayingTimeElapsed) <= fTimeEpsilon) {
        ...
        //
        MusicState_e eFeedBackTransition =
            eBGRNDTRACK_EXPLORETRANS0; // any old default
        ...
        if (iExitPoint < pMusicFile->MusicExitPoints.size()) {
          ...
          case eBGRNDTRACK_EXPLORE: {
            assert(
                iExitPoint <
                iMAX_EXPLORE_TRANSITIONS); // already been checked, but sanity
            assert(!ExitPoint.sNextMark
                        .c_str()[0]); // simple error checking, but harmless if
                                      // tripped. explore transitions go to
                                      // silence, hence no entry time for
                                      // [silence] state after transition

            eFeedBackTransition =
                (MusicState_e)(eBGRNDTRACK_EXPLORETRANS0 + iExitPoint);
          } break;

          case eBGRNDTRACK_ACTION: {
            assert(iExitPoint <
                   iMAX_ACTION_TRANSITIONS); // already been checked, but sanity
            ...
            if (ExitPoint.sNextMark.c_str()[0]) {
              ...
              if (itExploreMusicData != MusicData->end()) {
                MusicFile_t &MusicFile_Explore = (*itExploreMusicData).second;
                //
                // find the entry marker within the music and read the time
                // there...
                //
                MusicEntryTimes_t::iterator itEntryTime =
                    MusicFile_Explore.MusicEntryTimes.find(
                ...
                if (itEntryTime != MusicFile_Explore.MusicEntryTimes.end()) {
                  ...
                  eFeedBackTransition =
                      (MusicState_e)(eBGRNDTRACK_ACTIONTRANS0 + iExitPoint);
                } else {
#ifndef FINAL_BUILD
                  assert(0); // sanity, should have been caught elsewhere, but
                             // harmless to do this
                  Com_Printf(S_COLOR_RED
                             "Music_AllowedToTransition() unable to find entry "
                             "marker \"%s\" in \"%s\"",
                             ExitPoint.sNextMark.c_str(),
                             MusicFile_Explore.sFileNameBase.c_str());
#endif
                ...
                assert(0); // sanity, should have been caught elsewhere, but
                           // harmless to do this
                Com_Printf(S_COLOR_RED "Music_AllowedToTransition() unable to "
                                       "find %s version of \"%s\"\n",
                           Music_BaseStateToString(eBGRNDTRACK_EXPLORE),
                           pMusicFile->sFileNameBase.c_str());
#endif
                return qfalse;
              }
            } else {
...
#ifndef FINAL_BUILD
            assert(0);
            Com_Printf(S_COLOR_RED "Music_AllowedToTransition(): No code to "
                                   "transition from music type %d\n",
                       eMusicState);
#endif
            return qfalse;
          } break;
...
#ifndef FINAL_BUILD
          assert(0);
          Com_Printf(S_COLOR_RED "Music_AllowedToTransition(): Illegal exit "
                                 "point %d, max = %d (music: \"%s\")\n",
                     iExitPoint, pMusicFile->MusicExitPoints.size() - 1,
                     pMusicFile->sFileNameBase.c_str());
#endif
          return qfalse;
        }
...
// typically used to get a (predefined) random entry point for the action music,
// but will work on any defined type with entry points,
//	defaults safely to 0.0f if no info available...
//
float Music_GetRandomEntryTime(MusicState_e eMusicState) {
  MusicData_t::iterator itMusicData =
      MusicData->find(Music_BaseStateToString(eMusicState));
  if (itMusicData != MusicData->end()) {
    MusicFile_t &MusicFile = (*itMusicData).second;

    if (MusicFile.MusicEntryTimes
            .size()) // make sure at least one defined, else default to start
    {
      ...
      // );
      //
      // ... I'll do this (ensuring we don't get the same result on two
      // consecutive calls, but without while-loop)...
      //
      static int iPrevRandomNumber = -1;
      static int iCallCount = 0;
      iCallCount++;
      int iRandomEntryNum = (rand() + iCallCount) %
                            (MusicFile.MusicEntryTimes.size()); // legal range
      if (iRandomEntryNum == iPrevRandomNumber &&
          MusicFile.MusicEntryTimes.size() > 1) {
        iRandomEntryNum += 1;
        iRandomEntryNum %= (MusicFile.MusicEntryTimes.size());
      }
      iPrevRandomNumber = iRandomEntryNum;

      //			OutputDebugString(va("Music_GetRandomEntryTime():
      //Entry %d\n",iRandomEntryNum));

      for (MusicEntryTimes_t::iterator itEntryTime =
               MusicFile.MusicEntryTimes.begin();
           itEntryTime != MusicFile.MusicEntryTimes.end(); ++itEntryTime) {
        if (!iRandomEntryNum--) {
...
const char *Music_GetLevelSetName(void) {
  if (Q_stricmp(gsLevelNameForCompare.c_str(), gsLevelNameForLoad.c_str())) {
    ...
    //
    return va("%s -> %s", gsLevelNameForCompare.c_str(),
              gsLevelNameForLoad.c_str());
  }
