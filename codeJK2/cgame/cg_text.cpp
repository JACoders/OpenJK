...
void CG_CaptionText(const char *str, int sound) {
	...
	const int fontHeight = (int)((cgi_Language_IsAsian() ? 1.4f : 1.0f) *
								 (float)cgi_R_Font_HeightPixels(cgs.media.qhFontMedium, fFontScale)); // taiwanese & japanese need 1.5 fontheight spacing
	...
	if (in_camera) {
		...
		cg.captionTextY = (int)(0.88f * ((float)SCREEN_HEIGHT -
										 (float)fontHeight * 1.5f)); // do NOT move this, it has to fit in between the weapon HUD and the datapad update.
	}
