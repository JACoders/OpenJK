...
qhandle_t RE_RegisterIndividualSkin(const char *name, qhandle_t hSkin) {
  ...
  while (text_p && *text_p) {
    ...
    if ((int)(sizeof(skin->surfaces) / sizeof(skin->surfaces[0])) <=
        skin->numSurfaces) {
      assert((int)(sizeof(skin->surfaces) / sizeof(skin->surfaces[0])) >
             skin->numSurfaces);
    ...
    if (gServerSkinHack)
      surf->shader =
          R_FindServerShader(token, lightmapsNone, stylesDefault, qtrue);
    else
...
qhandle_t RE_RegisterSkin(const char *name) {
  ...
  if (strcmp(name + strlen(name) - 5, ".skin")) {
    /*		skin->numSurfaces = 1;
                    skin->surfaces[0] = (skinSurface_t *)Hunk_Alloc(
       sizeof(skin->surfaces[0]), h_low ); skin->surfaces[0]->shader =
...
qhandle_t RE_RegisterServerSkin(const char *name) {
  ...
  if (ri.Cvar_VariableIntegerValue("cl_running") &&
      ...
      ShaderHashTableExists()) { // If the client is running then we can go
                                 // straight into the normal registerskin func
...
void R_InitSkins(void) {
  ...
  skin->numSurfaces = 1;
  skin->surfaces[0] =
      (_skinSurface_t *)ri.Hunk_Alloc(sizeof(skinSurface_t), h_low);
