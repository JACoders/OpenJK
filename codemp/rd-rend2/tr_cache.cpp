...
CachedFile::CachedFile() : pDiskImage(nullptr), iLevelLastUsedOn(0), iPAKChecksum(-1), iAllocSize(0) {}

CModelCacheManager::FileCache::iterator CModelCacheManager::FindFile(const char *path) {
	return std::find_if(std::begin(files), std::end(files), [path](const CachedFile &file) { return strcmp(path, file.path) == 0; });
}
...
CModelCacheManager::AssetCache::iterator CModelCacheManager::FindAsset(const char *path) {
	return std::find_if(std::begin(assets), std::end(assets), [path](const Asset &asset) { return strcmp(path, asset.path) == 0; });
}
...
qboolean C_Models_LevelLoadEnd(qboolean deleteUnusedByLevel) { return CModelCache->LevelLoadEnd(deleteUnusedByLevel); }

qboolean C_Images_LevelLoadEnd() { return qfalse; }
