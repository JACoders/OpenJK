...
void SP_info_player_siegeteam1(gentity_t *ent) {
  ...
  if (level.gametype !=
      GT_SIEGE) { // turn into a DM spawn if not in siege game mode
    ent->classname = "info_player_deathmatch";
    SP_info_player_deathmatch(ent);
...
view direction. RED - In a Siege game, the intermission will happen here if the
Red (attacking) team wins BLUE - In a Siege game, the intermission will happen
here if the Blue (defending) team wins
*/
...
In a Siege game, the intermission will happen here if the Red (attacking) team
wins target - ent to look at target2 - ents to use when this intermission point
is chosen
*/
...
#define JMSABER_RESPAWN_TIME                                                   \
  20000 // in case it gets stuck somewhere no one can reach
...
void ThrowSaberToAttacker(gentity_t *self, gentity_t *attacker) {
  ...
  if (!ent ||
      ent->enemy !=
          self) { // something has gone very wrong (this should never happen)
  ...
  if (attacker && attacker->client &&
      self->client->ps
          .saberInFlight) { // someone killed us and we had the saber thrown, so
                            // actually move this saber to the saber location if
                            // we killed ourselves with saber thrown, however,
                            // same suicide rules of respawning at spawn spot
                            // still apply.
  ...
  self->client->ps.saberInFlight = qtrue; // say he threw it anyway in order to
                                          // properly remove from dead body

  WP_SaberAddG2Model(ent, self->client->saber[0].model,
...
void JMSaberTouch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (other->health < 200 &&
      other->health > 0) { // full health when you become the Jedi Master
...
gentity_t *SelectRandomFurthestSpawnPoint(vec3_t avoidPoint, vec3_t origin,
  ...
  if (level.gametype == GT_TEAM && team != TEAM_FREE &&
      team != TEAM_SPECTATOR) {
    char *classname = NULL;
    if (team == TEAM_RED) {
...
gentity_t *SelectDuelSpawnPoint(int team, vec3_t avoidPoint, vec3_t origin,
...
tryAgain:
  ...
  if (!numSpots) {
    if (Q_stricmp(
            spotName,
            "info_player_deathmatch")) { // try the loop again with
                                         // info_player_deathmatch as the target
                                         // if we couldn't find a duel spot
      spotName = "info_player_deathmatch";
      goto tryAgain;
    }

    // If we got here we found no free duel or DM spots, just try the first DM
    // spot
    spot = G_Find(NULL, FOFS(classname), "info_player_deathmatch");
    if (!spot)
      trap->Error(ERR_DROP, "Couldn't find a spawn point");
...
static qboolean CopyToBodyQue(gentity_t *ent) {
  ...
  if (ent->client && (ent->client->ps.eFlags &
                      EF_DISINTEGRATION)) { // for now, just don't spawn a body
                                            // if you got disint'd
    return qfalse;
  }
  ...
  body->s.angles[PITCH] = body->s.angles[ROLL] = body->s.apos.trBase[PITCH] =
  ...
  if (body->s.weapon == WP_SABER && ent->client->ps.saberInFlight) {
    body->s.weapon = WP_BLASTER; // lie to keep from putting a saber on the
                                 // corpse, because it was thrown at death
  }
  ...
  trap->SendServerCommand(-1, va("ircg %i %i %i %i", ent->s.number,
                                 body->s.number, body->s.weapon, islight));
...
void MaintainBodyQueue(
    gentity_t *ent) { // do whatever should be done taking ragdoll and
                      // dismemberment states into account.
  ...
  if (doRCG) { // bodyque func didn't manage to call ircg so call this to assure
               // our limbs and ragdoll states are proper on the client.
...
static void ClientCleanName(const char *in, char *out, int outSize) {
  ...
  for (; *in && outpos < outSize - 1; in++) {
    ...
    } else if ((byte)*in < 0x20 || (byte)*in == 0x81 || (byte)*in == 0x8D ||
               (byte)*in == 0x8F || (byte)*in == 0x90 || (byte)*in == 0x9D ||
...
qboolean G_SaberModelSetup(gentity_t *ent) {
  ...
  while (i < MAX_SABERS) {
    if (ent->client->saber[i].model[0]) {
      ...
      trap->G2API_InitGhoul2Model(&ent->client->weaponGhoul2[i],
                                  ent->client->saber[i].model, 0, 0, -20, 0, 0);
      ...
      if (ent->client->weaponGhoul2[i]) {
        ...
        if (ent->client->saber[i].skin) {
          trap->G2API_SetSkin(ent->client->weaponGhoul2[i], 0,
        ...
        while (j < ent->client->saber[i].numBlades) {
          ...
          tagBolt =
              trap->G2API_AddBolt(ent->client->weaponGhoul2[i], 0, tagName);

          if (tagBolt == -1) {
            if (j == 0) { // guess this is an 0ldsk3wl saber
              tagBolt = trap->G2API_AddBolt(ent->client->weaponGhoul2[i], 0,
                                            "*flash");
          ...
          fallbackForSaber =
              qfalse; // got at least one custom saber so don't need default
        }

        // Copy it into the main instance
        trap->G2API_CopySpecificGhoul2Model(ent->client->weaponGhoul2[i], 0,
                                            ent->ghoul2, i + 1);
      }
    } else {
      break;
...
There are two ghoul2 model instances per player (actually three).  One is on the
clientinfo (the base for the client side player, and copied for player spawns
and for corpses).  One is attached to the centity itself, which is the model
acutally animated and rendered by the system.  The final is the game ghoul2
model.  This is animated by pmove on the server, and is used for determining
where the lightsaber should be, and for per-poly collision tests.
===========
*/
void *g2SaberInstance = NULL;

qboolean BG_IsValidCharacterModel(const char *modelName, const char *skinName);
qboolean BG_ValidateSkinForTeam(const char *modelName, char *skinName, int team,
                                float *colors);
void BG_GetVehicleModelName(char *modelName, const char *vehicleName,
                            size_t len);
...
void SetupGameGhoul2Model(gentity_t *ent, char *modelname, char *skinName) {
  ...
  if (strlen(modelname) >= MAX_QPATH) {
    Com_Error(ERR_FATAL,
              "SetupGameGhoul2Model(%s): modelname exceeds MAX_QPATH.\n",
              modelname);
  }
  if (skinName && strlen(skinName) >= MAX_QPATH) {
    Com_Error(ERR_FATAL,
              "SetupGameGhoul2Model(%s): skinName exceeds MAX_QPATH.\n",
              skinName);
  }

  // First things first.  If this is a ghoul2 model, then let's make sure we
  // demolish this first.
  ...
  if (!precachedKyle) {
    ...
    handle =
        trap->G2API_InitGhoul2Model(&precachedKyle, afilename, 0, 0, -20, 0, 0);
  ...
  if (precachedKyle && trap->G2API_HaveWeGhoul2Models(precachedKyle)) {
    if (d_perPlayerGhoul2.integer || ent->s.number >= MAX_CLIENTS ||
        G_PlayerHasCustomSkeleton(
            ent)) { // rww - allow option for perplayer models on server for
                    // collision and bolt stuff.
      ...
      if (ent->client->NPC_class == CLASS_VEHICLE) {
        ...
        if (ent->m_pVehicle && ent->m_pVehicle->m_pVehicleInfo &&
          ...
          skinHandle = trap->R_RegisterSkin(
              va("models/players/%s/model_%s.skin", realModelName,
                 ent->m_pVehicle->m_pVehicleInfo->skin));
        } else {
          skinHandle = trap->R_RegisterSkin(
              va("models/players/%s/model_default.skin", realModelName));
        }
      } else {
        if (skinName && skinName[0]) {
          ...
          if (level.gametype >= GT_TEAM && level.gametype != GT_SIEGE &&
            ...
            BG_ValidateSkinForTeam(truncModelName, skin,
                                   ent->client->sess.sessionTeam,
                                   colorOverride);
            if (colorOverride[0] != 0.0f || colorOverride[1] != 0.0f ||
            ...
            // BG_ValidateSkinForTeam( truncModelName, skin,
            // ent->client->sess.sessionTeam, NULL );
          } else if (level.gametype ==
                     GT_SIEGE) { // force skin for class if appropriate
      ...
      if (skin[0]) {
        ...
        if (strchr(skin, '|')) { // three part skin
          useSkinName = va("models/players/%s/|%s", truncModelName, skin);
        } else {
          useSkinName =
              va("models/players/%s/model_%s.skin", truncModelName, skin);
        }
      ...
      handle = trap->G2API_InitGhoul2Model(&ent->ghoul2, modelFullPath, 0,
                                           skinHandle, -20, 0, 0);
      ...
      if (handle < 0) { // Huh. Guess we don't have this model. Use the default.
        ...
        if (!GLAName[0] || (!strstr(GLAName, "players/_humanoid/") &&
                            ...
                            !G_PlayerHasCustomSkeleton(ent))) { // a bad model
          trap->G2API_CleanGhoul2Models(&(ent->ghoul2));
          ent->ghoul2 = NULL;
          trap->G2API_DuplicateGhoul2Instance(precachedKyle, &ent->ghoul2);
        }
        ...
        if (ent->s.number >= MAX_CLIENTS) {
          ent->s.modelGhoul2 =
              1; // so we know to free it on the client when we're removed.
          ...
          if (ent->client->NPC_class ==
              CLASS_VEHICLE) { // vehicles are tricky and send over their
                               // vehicle names as the model (the model is then
    ...
    for (i = 0; i < MAX_VEHICLE_TURRET_MUZZLES; i++) {
      if (ent->m_pVehicle->m_pVehicleInfo->turret[i].gunnerViewTag) {
        ent->m_pVehicle->m_iGunnerViewTag[i] = trap->G2API_AddBolt(
            ent->ghoul2, 0,
            ent->m_pVehicle->m_pVehicleInfo->turret[i].gunnerViewTag);
      } else {
  ...
  if (ent->client->ps.weapon == WP_SABER ||
      ent->s.number < MAX_CLIENTS) { // a player or NPC saber user
    trap->G2API_AddBolt(ent->ghoul2, 0, "*r_hand");
    trap->G2API_AddBolt(ent->ghoul2, 0, "*l_hand");

    // rhand must always be first bolt. lhand always second. Whichever you want
    // the jetpack bolted to must always be third.
    ...
    trap->G2API_SetBoneAnim(ent->ghoul2, 0, "model_root", 0, 12,
                            BONE_ANIM_OVERRIDE_LOOP, 1.0f, level.time, -1, -1);
    trap->G2API_SetBoneAngles(ent->ghoul2, 0, "upper_lumbar", tempVec,
                              BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                              NEGATIVE_Z, NULL, 0, level.time);
    trap->G2API_SetBoneAngles(ent->ghoul2, 0, "cranium", tempVec,
                              BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y,
                              POSITIVE_X, NULL, 0, level.time);

    if (!g2SaberInstance) {
      trap->G2API_InitGhoul2Model(&g2SaberInstance,
                                  "models/weapons2/saber/saber_w.glm", 0, 0,
                                  -20, 0, 0);
  ...
  if (ent->s.number >= MAX_CLIENTS) { // some extra NPC stuff
    if (trap->G2API_AddBolt(ent->ghoul2, 0, "lower_lumbar") ==
        -1) { // check now to see if we have this bone for setting anims and
              // such
      ent->noLumbar = qtrue;
    }
  }
...
static userinfoValidate_t userinfoFields[] = {
    ...
    UIF(teamoverlay, 0,
        1), // only registered in cgame, not sent when connecting
};
...
void Svcmd_ToggleUserinfoValidation_f(void) {
  if (trap->Argc() == 1) {
    ...
    trap->Cvar_Set(
        ...
        va("%i",
           (1 << index) ^
               (g_userinfoValidate.integer &
                ((1 << (numUserinfoFields + USERINFO_VALIDATION_MAX)) - 1))));
    trap->Cvar_Update(&g_userinfoValidate);

    if (index < numUserinfoFields)
      Com_Printf("%s %s\n", userinfoFields[index].fieldClean,
                 ((g_userinfoValidate.integer & (1 << index)) ? "Validated"
                                                              : "Ignored"));
    else
...
char *G_ValidateUserinfo(const char *userinfo) {
  ...
  // size checks
  if (g_userinfoValidate.integer &
      (1 << (numUserinfoFields + USERINFO_VALIDATION_SIZE))) {
    if (length < 1)
  ...
  // slash checks
  if (g_userinfoValidate.integer &
      (1 << (numUserinfoFields + USERINFO_VALIDATION_SLASH))) {
  ...
  if (g_userinfoValidate.integer &
      (1 << (numUserinfoFields + USERINFO_VALIDATION_EXTASCII))) {
    for (i = 0, count = 0; i < length; i++) {
      if (userinfo[i] < 0)
        count++;
    }
  ...
  // disallow \n \r ; and \"
  if (g_userinfoValidate.integer &
      (1 << (numUserinfoFields + USERINFO_VALIDATION_CONTROLCHARS))) {
  ...
  for (i = 0, info = userinfoFields; i < numUserinfoFields; i++, info++) {
    if (g_userinfoValidate.integer & (1 << i)) {
      if (info->minCount && !fieldCount[i])
        ...
        return va("Too many %s fields (%i/%i)", info->fieldClean, fieldCount[i],
                  info->maxCount);
    }
  }
...
qboolean ClientUserinfoChanged(int clientNum) {
  ...
  const char *s = NULL;
  char *value = NULL, userinfo[MAX_INFO_STRING], buf[MAX_INFO_STRING],
       oldClientinfo[MAX_INFO_STRING], model[MAX_QPATH],
       forcePowers[DEFAULT_FORCEPOWERS_LEN], oldname[MAX_NETNAME],
       className[MAX_QPATH], color1[16], color2[16];
  ...
  if (s && *s) {
    G_SecurityLogPrintf(
        "Client %d (%s) failed userinfo validation: %s [IP: %s]\n", clientNum,
  ...
  Q_strncpyz(client->pers.netname_nocolor, client->pers.netname,
             sizeof(client->pers.netname_nocolor));
  ...
  if (client->sess.sessionTeam == TEAM_SPECTATOR &&
    ...
    Q_strncpyz(client->pers.netname_nocolor, "scoreboard",
               sizeof(client->pers.netname_nocolor));
  }
  ...
  if (client->pers.connected == CON_CONNECTED &&
    ...
    if (client->pers.netnameTime > level.time) {
      trap->SendServerCommand(
          clientNum, va("print \"%s\n\"",
                        G_GetStringEdString("MP_SVGAME", "NONAMECHANGE")));

      Info_SetValueForKey(userinfo, "name", oldname);
      trap->SetUserinfo(clientNum, userinfo);
      Q_strncpyz(client->pers.netname, oldname, sizeof(client->pers.netname));
      Q_strncpyz(client->pers.netname_nocolor, oldname,
                 sizeof(client->pers.netname_nocolor));
      Q_StripColor(client->pers.netname_nocolor);
    } else {
      trap->SendServerCommand(
          -1, va("print \"%s" S_COLOR_WHITE " %s %s\n\"", oldname,
                 G_GetStringEdString("MP_SVGAME", "PLRENAME"),
                 client->pers.netname));
  ...
  if (level.gametype >= GT_TEAM && level.gametype != GT_SIEGE &&
    ...
    BG_ValidateSkinForTeam(model, skin, client->sess.sessionTeam,
                           colorOverride);
    if (colorOverride[0] != 0.0f || colorOverride[1] != 0.0f ||
        colorOverride[2] != 0.0f)
  ...
  if (level.gametype >= GT_TEAM && g_entities[clientNum].r.svFlags & SVF_BOT) {
    ...
    if (!Q_stricmp(s, "red") || !Q_stricmp(s, "r"))
      ...
      team =
          PickTeam(clientNum); // pick the team with the least number of players
  } else
    team = client->sess.sessionTeam;

  // Testing to see if this fixes the problem with a bot's team getting set
  // incorrectly.
  team = client->sess.sessionTeam;
  ...
  if (level.gametype == GT_SIEGE) {
    ...
    // Now that the team is legal for sure, we'll go ahead and get an index for
    // it.
    ...
    if (client->siegeClass == -1) {
      ...
      Q_strncpyz(client->sess.siegeClass, className,
                 sizeof(client->sess.siegeClass));
      client->siegeClass = BG_SiegeFindClassIndexByName(className);
    } else {
    ...
    if (client->siegeClass != -1) {
      ...
      if (G_PlayerHasCustomSkeleton(
              ent)) { // force them to use their class model on the server, if
                      // the class dictates
  ...
  //	it will be read from userinfo on ClientSpawn and stored in
  //client->pers.saber1/2
  ...
  if (atoi(s) < sv_fps.integer)
    trap->SendServerCommand(clientNum,
                            va("print \"" S_COLOR_YELLOW
                               "Recommend setting /snaps %d or higher to match "
                               "this server's sv_fps\n\"",
                               sv_fps.integer));
  ...
  if (ent->r.svFlags & SVF_BOT)
    Q_strcat(buf, sizeof(buf),
             va("skill\\%s\\", Info_ValueForKey(userinfo, "skill")));
  ...
  trap->GetConfigstring(CS_PLAYERS + clientNum, oldClientinfo,
                        sizeof(oldClientinfo));
  ...
  if (modelChanged) {
    ...
    if (ent->ghoul2 && ent->client)
      ent->client->renderInfo.lastG2 =
          NULL; // update the renderinfo bolts next update.
...
char *ClientConnect(int clientNum, qboolean firstTime, qboolean isBot) {
  ...
  gclient_t *client;
  char userinfo[MAX_INFO_STRING] = {0}, tmpIP[NET_ADDRSTRMAXLEN] = {0},
       guid[33] = {0};
  ...
  if (!isBot && g_needpass.integer) {
    ...
    if (g_password.string[0] && Q_stricmp(g_password.string, "none") &&
      ...
      Q_strncpyz(sTemp,
                 G_GetStringEdString("MP_SVGAME", "INVALID_ESCAPE_TO_MAIN"),
                 sizeof(sTemp));
  ...
  if (!isBot && firstTime) {
    if (g_antiFakePlayer.integer) { // patched, check for > g_maxConnPerIP
  ...
  if (ent->inuse) { // if a player reconnects quickly after a disconnect, the
                    // client disconnect may never be called, thus flag can get
                    // lost in the ether
  ...
  if (level.gametype == GT_SIEGE &&
      (firstTime ||
       level.newSession)) { // if this is the first time then auto-assign a
                            // desired siege team and show briefing for that team
    client->sess.siegeDesiredTeam = 0; // PickTeam(ent->s.number);
    /*
    trap->SendServerCommand(ent->s.number, va("sb %i",
  ...
  if (level.gametype == GT_SIEGE &&
    ...
    if (firstTime || level.newSession) { // start as spec
  ...
  if (!isBot && firstTime) {
    if (!tmpIP[0]) { // No IP sent when connecting, probably an unban hack
                     // attempt
      client->pers.connected = CON_DISCONNECTED;
  ...
  //	if ( !client->areabits )
  //		client->areabits = G_Alloc( (trap->AAS_PointReachabilityAreaIndex(
  //NULL ) + 7) / 8 );
...
void G_BreakArm(gentity_t *ent, int arm) {
  ...
  if (ent->s.NPC_class == CLASS_VEHICLE ||
      ent->localAnimIndex >
          1) { // no broken limbs for vehicles and non-humanoids
    return;
  }
  ...
  if (ent->client->ps.fd.saberAnimLevel ==
      SS_STAFF) { // I'm too lazy to deal with this as well for now.
    return;
  }

  if (arm == BROKENLIMB_LARM) {
    if (ent->client->saber[1].model[0] && ent->client->ps.weapon == WP_SABER &&
        !ent->client->ps.saberHolstered &&
        ent->client->saber[1]
            .soundOff) { // the left arm shuts off its saber upon being broken
  ...
  // Do a pain anim based on the side. Since getting your arm broken does tend
  // to hurt.
  if (arm == BROKENLIMB_LARM) {
    anim = BOTH_PAIN2;
  ...
  G_SetAnim(ent, &ent->client->pers.cmd, SETANIM_BOTH, anim,
            SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);

  // This could be combined into a single event. But I guess limbs don't break
  ...
  G_Sound(ent, CHAN_AUTO,
          G_SoundIndex(va("sound/player/bodyfall_human%i.wav", Q_irand(1, 3))));
}
...
void G_UpdateClientAnims(gentity_t *self, float animSpeedScale) {
  ...
  if (self->client->ps.saberLockFrame) {
    trap->G2API_SetBoneAnim(self->ghoul2, 0, "model_root",
                            self->client->ps.saberLockFrame,
                            self->client->ps.saberLockFrame + 1,
                            BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
                            animSpeedScale, level.time, -1, 150);
    trap->G2API_SetBoneAnim(self->ghoul2, 0, "lower_lumbar",
                            self->client->ps.saberLockFrame,
                            self->client->ps.saberLockFrame + 1,
                            BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
                            animSpeedScale, level.time, -1, 150);
    trap->G2API_SetBoneAnim(self->ghoul2, 0, "Motion",
                            self->client->ps.saberLockFrame,
                            self->client->ps.saberLockFrame + 1,
                            BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND,
                            animSpeedScale, level.time, -1, 150);
  ...
  if (self->localAnimIndex > 1 &&
      ...
      bgAllAnims[self->localAnimIndex].anims[legsAnim].numFrames ==
          0) { // We'll allow this for non-humanoids.
    goto tryTorso;
  }

  if (self->client->legsAnimExecute != legsAnim ||
      self->client->legsLastFlip != self->client->ps.legsFlip) {
    animSpeed =
        50.0f / bgAllAnims[self->localAnimIndex].anims[legsAnim].frameLerp;
    ...
    aFlags |=
        BONE_ANIM_BLEND; // since client defaults to blend. Not sure if this
                         // will make much difference if any on server position,
                         // but it's here just for the sake of matching them.

    trap->G2API_SetBoneAnim(self->ghoul2, 0, "model_root", firstFrame,
                            lastFrame, aFlags, lAnimSpeedScale, level.time, -1,
                            150);
...
tryTorso:
  ...
  { // If this fails as well just return.
    return;
             ...
             self->s.NPC_class == CLASS_VEHICLE) { // we only want to set the
                                                   // root bone for vehicles
    return;
  }

  if ((self->client->torsoAnimExecute != torsoAnim ||
    ...
    BG_SaberStartTransAnim(self->s.number, self->client->ps.fd.saberAnimLevel,
                           self->client->ps.weapon, f, &animSpeedScale,
    ...
    trap->G2API_SetBoneAnim(self->ghoul2, 0, "lower_lumbar", firstFrame,
                            lastFrame, aFlags, lAnimSpeedScale, level.time,
  ...
  if (setTorso &&
    ...
    trap->G2API_SetBoneAnim(self->ghoul2, 0, "Motion", firstFrame, lastFrame,
                            aFlags, lAnimSpeedScale, level.time, -1, 150);
  }
...
void ClientSpawn(gentity_t *ent) {
  int i = 0, index = 0, saveSaberNum = ENTITYNUM_NONE, wDisable = 0,
      savedSiegeIndex = 0, maxHealth = 100;
  ...
  char userinfo[MAX_INFO_STRING] = {0}, *key = NULL, *value = NULL,
       *saber = NULL;
  ...
  // first we want the userinfo so we can see if we should update this client's
  // saber -rww
  ...
  for (i = 0; i < MAX_SABERS; i++) {
    ...
    if (saber && value &&
        (Q_stricmp(value, saber) || !saber[0] ||
         !ent->client->saber[0].model[0])) { // doesn't match up (or our saber
                                             // is BS), we want to try setting it
      if (G_SetSaber(ent, i, value, qfalse))
        changedSaber = qtrue;

      // Well, we still want to say they changed then (it means this is siege
      // and we have some overrides)
      else if (!saber[0] || !ent->client->saber[0].model[0])
        changedSaber = qtrue;
    }
  }

  if (changedSaber) { // make sure our new info is sent out to all the other
                      // clients, and give us a valid stance
    ...
    if (ent->client->saber[0].model[0] &&
        ent->client->saber[1].model[0]) { // dual
      ...
      ent->client->sess.saberLevel =
          Com_Clampi(SS_FAST, SS_STRONG, ent->client->sess.saberLevel);
      ent->client->ps.fd.saberAnimLevelBase =
          ent->client->ps.fd.saberAnimLevel =
              ent->client->ps.fd.saberDrawAnimLevel =
                  ent->client->sess.saberLevel;
    ...
    if (level.gametype !=
        GT_SIEGE) { // let's just make sure the styles we chose are cool
      if (!WP_SaberStyleValidForSaber(&ent->client->saber[0],
                                      &ent->client->saber[1],
                                      ent->client->ps.saberHolstered,
                                      ent->client->ps.fd.saberAnimLevel)) {
  ...
  if (client->sess.sessionTeam == TEAM_SPECTATOR) {
    ...
    spawnPoint =
        SelectSiegeSpawnPoint(client->siegeClass, client->sess.sessionTeam,
                              client->pers.teamState.state, spawn_origin,
                              spawn_angles, !!(ent->r.svFlags & SVF_BOT));
  } else {
    if (level.gametype == GT_POWERDUEL) {
      spawnPoint = SelectDuelSpawnPoint(
          client->sess.duelTeam, client->ps.origin, spawn_origin, spawn_angles,
          !!(ent->r.svFlags & SVF_BOT));
    } else if (level.gametype == GT_DUEL) { // duel
      spawnPoint =
          SelectDuelSpawnPoint(DUELTEAM_SINGLE, client->ps.origin, spawn_origin,
                               spawn_angles, !!(ent->r.svFlags & SVF_BOT));
    } else {
  ...
  if (level.gametype == GT_SIEGE && client->siegeClass != -1) {
    ...
    maxHealth =
        Com_Clampi(1, 100, atoi(Info_ValueForKey(userinfo, "handicap")));
  }
  ...
  if (level.gametype != GT_HOLOCRON && level.gametype != GT_JEDIMASTER &&
    ...
    if (level.gametype >= GT_TEAM &&
        (client->sess.sessionTeam == TEAM_BLUE ||
         client->sess.sessionTeam ==
             TEAM_RED)) { // In Team games, force one side to be merc and other
                          // to be jedi
      if (level.numPlayingClients > 0) { // already someone in the game
        ...
        for (i = 0; i < level.maxclients; i++) {
          ...
          if (level.clients[i].sess.sessionTeam == TEAM_BLUE ||
              level.clients[i].sess.sessionTeam == TEAM_RED) { // in-game
            if (WP_HasForcePowers(
                    &level.clients[i].ps)) { // this side is using force
        ...
        if (WP_HasForcePowers(&client->ps) &&
            client->sess.sessionTeam !=
                forceTeam) { // using force but not on right team, switch him
                             // over
  ...
  } else { // jediVmerc is incompatible with this gametype, turn it off!
    ...
    if (level.gametype == GT_HOLOCRON) {
      ...
      client->ps.stats[STAT_WEAPONS] |=
          (1 << WP_SABER); // these are precached in g_items,
      ...
      if (client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE]) {
        client->ps.stats[STAT_WEAPONS] |=
            (1 << WP_SABER); // these are precached in g_items,
                             // ClearRegisteredItems()
      } else { // if you don't have saber attack rank then you don't get a saber
        client->ps.stats[STAT_WEAPONS] |= (1 << WP_MELEE);
      }
    }
  ...
  client->ps.stats[STAT_HOLDABLE_ITEM] = BG_GetItemIndexByTag(HI_BINOCULARS,
  IT_HOLDABLE);
  */

  if (level.gametype == GT_SIEGE && client->siegeClass != -1 &&
      client->sess.sessionTeam !=
          TEAM_SPECTATOR) { // well then, we will use a custom weaponset for our
                            // class
    int m = 0;
    ...
    while (m < WP_NUM_WEAPONS) {
      if (client->ps.stats[STAT_WEAPONS] & (1 << m)) {
        if (client->ps.weapon !=
            WP_SABER) { // try to find the highest ranking weapon we have
        ...
        if (m >=
            WP_BRYAR_PISTOL) { // Max his ammo out for all the weapons he has.
          if (level.gametype == GT_SIEGE &&
              m == WP_ROCKET_LAUNCHER) { // don't give full ammo!
            // FIXME: extern this and check it when getting ammo from supplier,
            // pickups or ammo stations!
            if (client->siegeClass != -1 &&
                (bgSiegeClasses[client->siegeClass].classflags &
                 (1 << CFL_SINGLE_ROCKET))) {
            ...
            if (level.gametype == GT_SIEGE && client->siegeClass != -1 &&
                (bgSiegeClasses[client->siegeClass].classflags &
                 (1 << CFL_EXTRA_AMMO))) { // double ammo
              client->ps.ammo[weaponData[m].ammoIndex] =
                  ammoData[weaponData[m].ammoIndex].max * 2;
              client->ps.eFlags |= EF_DOUBLE_AMMO;
            } else {
              client->ps.ammo[weaponData[m].ammoIndex] =
                  ammoData[weaponData[m].ammoIndex].max;
            }
          }
        }
      }
  ...
  if (level.gametype == GT_SIEGE && client->siegeClass != -1 &&
      client->sess.sessionTeam !=
          TEAM_SPECTATOR) { // use class-specified inventory
    client->ps.stats[STAT_HOLDABLE_ITEMS] =
        bgSiegeClasses[client->siegeClass].invenItems;
  ...
  if (level.gametype == GT_SIEGE && client->siegeClass != -1 &&
      ...
      client->sess.sessionTeam !=
          TEAM_SPECTATOR) { // this class has some start powerups
    i = 0;
  ...
  // nmckenzie: DESERT_SIEGE... or well, siege generally.  This was over-writing
  // the max value, which was NOT good for siege.
  if (inSiegeWithClass == qfalse) {
    client->ps.ammo[AMMO_BLASTER] =
        100; // ammoData[AMMO_BLASTER].max; //100 seems fair.
  }
  //	client->ps.ammo[AMMO_POWERCELL] = ammoData[AMMO_POWERCELL].max;
  ...
  */
  ...
  if (level.gametype == GT_SIEGE && client->siegeClass != -1 &&
      bgSiegeClasses[client->siegeClass]
          .starthealth) { // class specifies a start health, so use it
    ent->health = client->ps.stats[STAT_HEALTH] =
        bgSiegeClasses[client->siegeClass].starthealth;
  } else if (level.gametype == GT_DUEL ||
             level.gametype ==
                 GT_POWERDUEL) { // only start with 100 health in Duel
    if (level.gametype == GT_POWERDUEL &&
  ...
  if (level.gametype == GT_SIEGE &&
    ...
    client->ps.stats[STAT_ARMOR] =
        bgSiegeClasses[client->siegeClass].startarmor;
             ...
             level.gametype == GT_POWERDUEL) { // no armor in duel
  ...
  if (level.gametype == GT_SIEGE) { // Imperial (team1) team is allied with
                                    // "enemy" NPCs in this mode
  ...
  */
  // Disabled. At least for now. Not sure if I'll want to do it or not
  // eventually.
  ...
  if (level.teamVoteTime[voteteam]) {
    if (ent->client->mGameFlags & PSG_TEAMVOTED) {
      if (ent->client->pers.teamvote == 1) {
        ...
        trap->SetConfigstring(CS_TEAMVOTE_YES,
                              va("%i", level.teamVoteYes[voteteam]));
      } else if (ent->client->pers.teamvote == 2) {
        level.teamVoteNo[voteteam]--;
        trap->SetConfigstring(CS_TEAMVOTE_NO,
                              va("%i", level.teamVoteNo[voteteam]));
      }
    }
...
void ClientDisconnect(int clientNum) {
  ...
  G_LogPrintf("ClientDisconnect: %i [%s] (%s) \"%s^7\"\n", clientNum,
              ent->client->sess.IP, ent->client->pers.guid,
              ent->client->pers.netname);

  // if we are playing in tourney mode, give a win to the other player and clear
  // his frags for this round
  if (level.gametype == GT_DUEL && !level.intermissiontime &&
      !level.warmupTime) {
  ...
  while (i < MAX_SABERS) {
    if (ent->client->weaponGhoul2[i] &&
        trap->G2API_HaveWeGhoul2Models(ent->client->weaponGhoul2[i])) {
      trap->G2API_CleanGhoul2Models(&ent->client->weaponGhoul2[i]);
    }
    i++;
  }
  ...
  if (ent->client->holdingObjectiveItem >
