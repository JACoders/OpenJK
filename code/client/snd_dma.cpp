...
const int iMP3MusicStream_DiskBufferSize =
    iMP3MusicStream_DiskBytesToRead * 2; //*10;
...
typedef struct {
  ...
  //
  sfx_t sfxMP3_Bgrnd;
  MP3STREAM streamMP3_Bgrnd; // this one is pointed at by the sfx_t's ptr, and
                             // is NOT the one the decoder uses every cycle
  ...
  //
  byte *pLoadedData; // Z_Malloc, Z_Free	// these two MUST be kept as
                     // valid/invalid together
  ...
  int iXFadeVolumeSeekTo; // when changing this, set the above timer to
                          // Sys_Milliseconds().
                          //	Note that this should be thought of more as an
                          //up/down bool rather than as a 	number now, in other
                          //words set it only to 0 or 255. I'll probably change
                          //this 	to actually be a bool later.
  int iXFadeVolume; // 0 = silent, 255 = max mixer vol, though still modulated
                    // via overall music_volume
  float fSmoothedOutVolume;
  qboolean bActive; // whether playing or not
  qboolean bExists; // whether was even loaded for this level (ie don't try and
                    // start playing it)
  //
  // new dynamic fields...
  //
  ...
  // Generic...
  //
  fileHandle_t s_backgroundFile; // valid handle, else -1 if an MP3 (so that NZ
                                 // compares still work)
...
static MusicState_e eMusic_StateActual =
    eBGRNDTRACK_EXPLORE; // actual state, can be any enum
static MusicState_e eMusic_StateRequest =
    eBGRNDTRACK_EXPLORE; // requested state, can only be explore, action, boss,
                         // or silence
static char sMusic_BackgroundLoop[MAX_QPATH] = {
    0}; // only valid for non-dynamic music
static char
    sInfoOnly_CurrentDynamicMusicSet[64]; // any old reasonable size, only has
                                          // to fit stuff like "kejim_post"
//
//////////////////////////
...
//#define DISPLAY_CLOSEST_ENVS		// Displays the closest env. zones
//(including the one the listener is in)
...
int s_UseOpenAL =
    0; // Determines if using Open AL or the default software mixer
...
LPENVTABLE s_lpEnvTable =
    NULL;                // Stores information about each environment zone
...
FXSLOTINFO s_FXSlotInfo[EAX_MAX_FXSLOTS]; // Stores information about the
                                          // EAX 4.0 FX Slots
...
const GUID EAX_NULL_GUID = {0x00000000,
                            0x0000,
                            0x0000,
...
// instead of clearing a whole channel_t struct, we're going to skip the
// MP3SlidingDecodeBuffer[] buffer in the middle...
//
#ifndef offsetof
#include <stddef.h>
#endif
static inline void Channel_Clear(channel_t *ch) {
  ...
  byte *const p = (byte *)ch + offsetof(channel_t, MP3SlidingDecodeBuffer) +
                  sizeof(ch->MP3SlidingDecodeBuffer);

  memset(p, 0,
         (sizeof(*ch) - offsetof(channel_t, MP3SlidingDecodeBuffer)) -
             sizeof(ch->MP3SlidingDecodeBuffer));
}
...
static void DynamicMusicInfoPrint(void) {
  if (bMusic_IsDynamic) {
    // horribly lazy... ;-)
    //
    const char *psRequestMusicState =
        Music_BaseStateToString(eMusic_StateRequest);
    const char *psActualMusicState =
        Music_BaseStateToString(eMusic_StateActual, qtrue);
    ...
    Com_Printf(
        "( Dynamic music ON, request state: '%s'(%d), actual: '%s' (%d) )\n",
...
void S_Init(void) {
  ...
  if (!cv->integer) {
    s_soundStarted = 0; // needed in case you set s_initsound to 0 midgame then
                        // snd_restart (div0 err otherwise later)
  ...
  if (s_UseOpenAL) {
    ...
    for (i = 0; i < MAX_CHANNELS; i++) {
      ...
      alSourcef(s_channels[i].alSource, AL_REFERENCE_DISTANCE,
      ...
      if (s_bEAX) {
        // Remove the RoomAuto flag from each Source (to remove Reverb Engine
        // Statistical model that is assuming units are in metres) Without this
        // call reverb sends from the sources will attenuate too quickly with
        // distance, especially for the non-primary reverb zones.

        unsigned long ulFlags = 0;

        if (s_eaxSet(&EAXPROPERTYID_EAX40_Source, EAXSOURCE_FLAGS,
                     s_channels[i].alSource, &ulFlags,
                     sizeof(ulFlags)) != AL_NO_ERROR) {
#ifdef _WIN32
    ...
    // Clamp sound volumes between 0.0f and 1.0f (just in case they aren't
    // already)
...
void S_ReloadAllUsedSounds(void) {
  if (s_soundStarted && !s_soundMuted) {
    ...
    //
    for (int i = 1; i < s_numSfx;
         i++) // start @ 1 to skip freeing default sound
    {
...
sfx_t *S_FindName(const char *name) {
  ...
  */
  i = s_numSfx; // we don't clear the soundName after failed loads any more, so
                // it'll always be the last entry

  if (s_numSfx == MAX_SFX) {
    // ok, no sfx's free, but are there any with defaultSound set? (which the
    // registering ent will never
    ...
    if (i == s_numSfx) {
      ...
      // if we ever reach this, let me know and I'll either boost the array or
      // put in a map-used-on
      //	reference to enable sfx_t recycling. TA codebase relies on being
      //able to have structs for every sound 	used anywhere, ever, all at once
      //(though audio bit-buffer gets recycled). SOF1 used about 1900 distinct
      //	events, so current MAX_SFX limit should do, or only need a small
      //boost...	-ste
      //
...
static void S_DefaultSound(sfx_t *sfx) {
  ...
  sfx->pSoundData =
      (short *)SND_malloc(512 * 2, sfx); // ... so *2 for alloc bytes
  sfx->bInMemory = true;
  ...
  if (s_bEALFileLoaded) {
    ...
    if ((s_bEAX) && (s_eaxSet)) {
      ...
      for (int i = 0; i < s_NumFXSlots; i++) {
        s_eaxSet(&s_FXSlotInfo[i].FXSlotGuid, EAXREVERB_ROOM, NULL, &lRoom,
                 sizeof(long));
      }
    }
  }
...
static qboolean S_CheckChannelStomp(int chan1, int chan2) {
  ...
  if ((chan1 == CHAN_VOICE || chan1 == CHAN_VOICE_ATTEN ||
...
channel_t *S_PickChannel(int entnum, int entchannel) {
  ...
  for (int pass = 0;
       (pass <
        ((entchannel == CHAN_AUTO || entchannel == CHAN_LESS_ATTEN) ? 1 : 2)) &&
       !foundChan;
       pass++) {
    for (ch_idx = 0, ch = &s_channels[0]; ch_idx < MAX_CHANNELS;
         ch_idx++, ch++) {
      if (entchannel == CHAN_AUTO || entchannel == CHAN_LESS_ATTEN ||
          pass >
              0) { // if we're on the second pass, just find the first open chan
      ...
      if (ch->entnum == listener_number && entnum != listener_number &&
          ch->thesfx) {
        continue;
      }
...
/*
        ...
        Allows more than one sound of the same type to emanate from the same
   entity - sounds much better on hardware this way esp. rapid fire modes of
   weapons!
*/
#ifdef USE_OPENAL
channel_t *S_OpenALPickChannel(int entnum, int entchannel) {
  ...
  for (ch_idx = 1, ch = s_channels + ch_idx; ch_idx < s_numChannels;
       ch_idx++, ch++) {
    if (ch->entnum == entnum &&
        S_CheckChannelStomp(ch->entchannel, entchannel)) {
      // always override sound from same entity
  ...
  if (!foundChan) {
    for (ch_idx = 1, ch = s_channels + ch_idx; ch_idx < s_numChannels;
      ...
      if ((ch->entnum == entnum) && (ch->entchannel == entchannel) &&
          (ch->entchannel != CHAN_AMBIENT) && (ch->entnum != listener_number)) {
  ...
  if (!foundChan) {
    ...
    if (ch->fixed_origin) {
      ...
      longestDist = ((listener_pos[0] - source_pos[0]) *
    ...
    for (ch_idx = 2, ch = s_channels + ch_idx; ch_idx < s_numChannels;
         ch_idx++, ch++) {
      if (ch->fixed_origin) {
        // Convert to Open AL co-ordinates
        source_pos[0] = ch->origin[0];
        ...
        dist = ((listener_pos[0] - source_pos[0]) *
                (listener_pos[0] - source_pos[0])) +
               ((listener_pos[1] - source_pos[1]) *
                (listener_pos[1] - source_pos[1])) +
               ((listener_pos[2] - source_pos[2]) *
...
=================
*/
static void S_SpatializeOrigin(const vec3_t origin, float master_vol,
  ...
  if (channel == CHAN_VOICE) {
    ...
    //		dist_mult = VOICE_ATTENUATE;	// tweak added (this fixes an NPC
    //dialogue "in your ears" bug, but we're not sure if it'll make a bunch of
    //others fade too early. Too close to shipping...)
  } else if (channel == CHAN_LESS_ATTEN) {
    dist -= SOUND_FULLVOLUME * 5.0f; // maybe is too large
  } else if (channel == CHAN_VOICE_ATTEN) {
    dist -= SOUND_FULLVOLUME *
            1.35f; // used to be 0.15f, dropped off too sharply - dmv
...
void S_StartAmbientSound(const vec3_t origin, int entityNum,
                         unsigned char volume, sfxHandle_t sfxHandle) {
  channel_t *ch;
  /*const*/ sfx_t *sfx;
  ...
  if (sfxHandle < 0 || sfxHandle >= s_numSfx)
    Com_Error(ERR_DROP, "S_StartAmbientSound: handle %i out of range",
              sfxHandle);
  ...
  if (sfx->pMP3StreamHeader) {
    memcpy(&ch->MP3StreamHeader, sfx->pMP3StreamHeader,
           sizeof(ch->MP3StreamHeader));
    // ch->iMP3SlidingDecodeWritePos = 0; // These will be zero from the memset
    // in S_PickChannel(), but keep them here for reference...
...
====================
*/
void S_StartSound(const vec3_t origin, int entityNum, soundChannel_t entchannel,
  ...
  if (s_UseOpenAL) {
    ...
    if (entchannel == CHAN_VOICE) {
      // Make howlers and sand_creature VOICE effects use the normal fall-off
      // (they will still be affected by the Voice Volume)
      if ((strstr(sfx->sSoundName, "sand_creature") != NULL) ||
          (strstr(sfx->sSoundName, "howler") != NULL)) {
    ...
    if (entchannel == CHAN_WEAPON) {
      ...
      ch = s_channels + 1;
      for (i = 1; i < s_numChannels; i++, ch++) {
        if ((ch->entnum == entityNum) && (ch->entchannel == CHAN_WEAPON) &&
            (ch->thesfx) &&
  ...
  if (entchannel < CHAN_AMBIENT &&
      entityNum ==
          listener_number) { // only do it for body sounds not local sounds
    ch->master_vol =
        SOUND_MAXVOL *
        SOUND_FMAXVOL; // this won't be attenuated so let it scale down
  }
  if (entchannel == CHAN_VOICE || entchannel == CHAN_VOICE_ATTEN ||
      entchannel == CHAN_VOICE_GLOBAL) {
    s_entityWavVol[ch->entnum] =
        -1; // we've started the sound but it's silent for now
  }
...
void S_StartLocalLoopingSound(sfxHandle_t sfxHandle) {
  ...
  if (sfxHandle < 0 || sfxHandle >= s_numSfx) {
    Com_Error(ERR_DROP, "S_StartLocalLoopingSound: handle %i out of range",
              sfxHandle);
  }
...
// returns length in milliseconds of supplied sound effect...  (else 0 for bad
// handle now)
//
float S_GetSampleLengthInMilliSeconds(sfxHandle_t sfxHandle) {
  sfx_t *sfx;

  if (!s_soundStarted) { // we have no sound, so let's just make a reasonable
                         // guess
    return 512 * 1000;
  }
...
==================
*/
void S_AddAmbientLoopingSound(const vec3_t origin, unsigned char volume,
                              sfxHandle_t sfxHandle) {
  /*const*/ sfx_t *sfx;
...
void S_AddLoopSounds(void) {
  ...
  for (i = 0; i < numLoopSounds; i++) {
    ...
    for (j = i; j < numLoopSounds; j++) {
      ...
      S_SpatializeOrigin(loop2->origin, loop2->volume, &left, &right,
                         loop2->entchan);
    ...
    // you cannot use MP3 files here because they offer only streaming access,
    // not random
    //
    if (loop->sfx->pMP3StreamHeader) {
      Com_Error(ERR_DROP,
                "S_AddLoopSounds(): Cannot use streamed MP3 files here for "
                "random access (%s)\n",
                loop->sfx->sSoundName);
    } else {
      memset(&ch->MP3StreamHeader, 0, sizeof(ch->MP3StreamHeader));
    ...
    for (int i = 1; i < s_numChannels; i++, ch++) {
      if ((s_channels[i].bPlaying) && (s_channels[i].entnum == entityNum) &&
        ...
        /*				pos[0] = origin[0];
                                        pos[1] = origin[2];
                                        pos[2] = -origin[1];
                                        alSourcefv(s_channels[i].alSource,
           AL_POSITION, pos);
...
// Given a current wav we are playing, and our position within it, lets figure
// out its volume...
//
...
static int S_CheckAmplitude(channel_t *ch, const int s_oldpaintedtime) {
  // now, is this a cycle - or have we just started a new sample - where we
  // should update the backup table, and write this value into the new table? or
  // should we just take the value FROM the back up table and feed it out.
  assert(ch->startSample != START_SAMPLE_IMMEDIATE);
  if (ch->startSample == s_oldpaintedtime ||
      (next_amplitude <
       s_soundtime)) //(ch->startSample ==
                     //START_SAMPLE_IMMEDIATE)//!s_entityWavVol_back[ch->entnum]
  {
    int sample;
    int sample_total = 0;
    int count = 0;
    ...
    for (int i = 0; i < 10; i++) {
      ...
      //			else
      {
        ...
        case ct_MP3: {
          const int iIndex = (i * 100) + ((offset * /*ch->thesfx->width*/ 2) -
                                          ch->iMP3SlidingDecodeWindowPos);
          const short *pwSamples =
              (short *)(ch->MP3SlidingDecodeBuffer + iIndex);
    ...
    if (sample_total < ch->thesfx->fVolRange * s_lip_threshold_1->value) {
      // tell the scripts that are relying on this that we are still going, but
      // actually silent right now.
      sample = -1;
    } else if (sample_total <
               ch->thesfx->fVolRange * s_lip_threshold_2->value) {
      sample = 1;
    } else if (sample_total <
               ch->thesfx->fVolRange * s_lip_threshold_3->value) {
      sample = 2;
    } else if (sample_total <
               ch->thesfx->fVolRange * s_lip_threshold_4->value) {
      sample = 3;
    } else {
      sample = 4;
    }
  ...
  if (s_UseOpenAL) {
    ...
    if (s_bEALFileLoaded) {
      ...
      if (inwater) {
        ...
        if (!s_bInWater) {
          ...
          // Load underwater reverb effect into FX Slot 0, and set this as the
          // Primary FX Slot
          unsigned int ulEnvironment = EAX_ENVIRONMENT_UNDERWATER;
          s_eaxSet(&EAXPROPERTYID_EAX40_FXSlot0, EAXREVERB_ENVIRONMENT, NULL,
                   &ulEnvironment, sizeof(unsigned int));
          s_EnvironmentID = 999;

          s_eaxSet(&EAXPROPERTYID_EAX40_Context, EAXCONTEXT_PRIMARYFXSLOTID,
                   NULL, (ALvoid *)&EAXPROPERTYID_EAX40_FXSlot0, sizeof(GUID));

          // Occlude all sounds into this environment, and mute all their sends
          // to other reverbs
          ...
          for (i = 1; i < s_numChannels; i++, ch++) {
            ...
            s_eaxSet(&EAXPROPERTYID_EAX40_Source, EAXSOURCE_ACTIVEFXSLOTID,
                     ch->alSource, &eaxActiveSlots, 2 * sizeof(GUID));
          }
        ...
        if (s_bInWater) {
          ...
          // Remove underwater Reverb effect, and reset Occlusion / Obstruction
          // amount on all Sources
          UpdateEAXListener();
    ...
    for (i = 0; i < MAX_CHANNELS; i++, ch++) {
      ...
      if (ch->entnum == listener_number ||
        ...
        S_SpatializeOrigin(*origin, (float)ch->master_vol, &ch->leftvol,
                           &ch->rightvol, ch->entchannel);
      }

      // NOTE: Made it so that voice sounds keep playing, even out of range
      //		so that tasks waiting for sound completion keep proper
      //timing
...
// this is now called AFTER the DMA painting, since it's only the painter calls
// that cause the MP3s to be unpacked,
...
void S_DoLipSynchs(const int s_oldpaintedtime) {
  ...
  for (i = 0; i < MAX_CHANNELS; i++, ch++) {
    ...
    // if we are playing a sample that should override the lip texture on its
    // owning model, lets figure out what the amplitude is, stick it in a table,
    // then return it
    if (ch->entchannel == CHAN_VOICE || ch->entchannel == CHAN_VOICE_ATTEN ||
      ...
      if (s_show->integer == 3) {
        Com_Printf("(%i)%i %s vol = %i\n", ch->entnum, i,
                   ch->thesfx->sSoundName, s_entityWavVol[ch->entnum]);
      }
    }
  }
  ...
  if (s_show->integer == 2) {
    ...
    for (i = 0; i < MAX_CHANNELS; i++, ch++) {
      if (ch->thesfx && (ch->leftvol || ch->rightvol)) {
        Com_Printf("(%i) %3i %3i %s\n", ch->entnum, ch->leftvol, ch->rightvol,
                   ch->thesfx->sSoundName);
        // total++;
    ...
    //s_paintedtime, totalMeg/1024.0f/1024.0f);
  }

  // The Open AL code, handles background music in the S_UpdateRawSamples
  // function
#ifdef USE_OPENAL
  if (!s_UseOpenAL)
#endif
...
void S_Update_(void) {
  ...
  if (s_UseOpenAL) {
    ...
    for (i = 1; i < MAX_CHANNELS; i++, ch++) {
      ...
      if (ch->entchannel == CHAN_VOICE_GLOBAL ||
        ...
        if (ch->fixed_origin) {
          ...
          if (ch->entnum == listener_number) {
            ...
            alSourcei(s_channels[source].alSource, AL_SOURCE_RELATIVE,
                      AL_FALSE);
          }
        }
        ...
        if (ch->entchannel == CHAN_VOICE) {
          // Reduced fall-off (Large Reference Distance), affected by Voice
          // Volume
          alSourcef(s_channels[source].alSource, AL_REFERENCE_DISTANCE,
      ...
      if (ch->thesfx->pMP3StreamHeader) {
        memcpy(&ch->MP3StreamHeader, ch->thesfx->pMP3StreamHeader,
               sizeof(ch->MP3StreamHeader));
        ...
        for (i = 0; i < NUM_STREAMING_BUFFERS; i++) {
          ...
          for (j = 0; j < (STREAMING_BUFFER_SIZE / 1152); j++) {
            nBytesDecoded =
                C_MP3Stream_Decode(&ch->MP3StreamHeader, 0); // added ,0 ?
            memcpy(ch->buffers[i].Data + nTotalBytesDecoded,
                   ch->MP3StreamHeader.bDecodeBuffer, nBytesDecoded);
            if (ch->entchannel == CHAN_VOICE ||
                ch->entchannel == CHAN_VOICE_ATTEN ||
                ch->entchannel == CHAN_VOICE_GLOBAL) {
              if (ch->thesfx->lipSyncData) {
                ch->thesfx->lipSyncData[(i * NUM_STREAMING_BUFFERS) + j] =
                    S_MP3PreProcessLipSync(
        ...
        for (i = 0; i < nBuffersToAdd; i++) {
          ...
          alBufferData(ch->buffers[i].BufferID, AL_FORMAT_MONO16,
                       ch->buffers[i].Data, STREAMING_BUFFER_SIZE, 22050);

          // Queue AL Buffer on Source
          alSourceQueueBuffers(s_channels[source].alSource, 1,
...
void UpdateSingleShotSounds() {
  ...
  // Firstly, check if any single-shot sounds have completed, or if they need
  // more data (for streaming Sources), and/or if any of the currently playing
  // (non-Ambient) looping sounds need to be stopped
  ch = s_channels + 1;
  for (i = 1; i < s_numChannels; i++, ch++) {
    ch->bProcessed = false;
      ...
      if (s_channels[i].bStreaming == false) {
        ...
        if (ch->thesfx->pMP3StreamHeader) {
          if (ch->MP3StreamHeader.iSourceBytesRemaining == 0) {
            // Finished decoding data - if the source has finished playing then
            // we're done
            alGetSourcei(ch->alSource, AL_SOURCE_STATE, &state);
            if (state == AL_STOPPED) {
              // Attach NULL buffer to Source to remove any buffers left in the
              // queue
              alSourcei(ch->alSource, AL_BUFFER, NULL);
              ch->thesfx = NULL;
              ch->bPlaying = false;
          ...
          for (j = 0; j < NUM_STREAMING_BUFFERS; j++) {
            if ((ch->buffers[j].Status == UNQUEUED) &&
              ...
              for (k = 0; k < (STREAMING_BUFFER_SIZE / 1152); k++) {
                ...
                if (nBytesDecoded > 0) {
                  memcpy(ch->buffers[j].Data + nTotalBytesDecoded,
                         ch->MP3StreamHeader.bDecodeBuffer, nBytesDecoded);
              ...
              if (nTotalBytesDecoded != STREAMING_BUFFER_SIZE) {
                ...
                // Move data to buffer
                alBufferData(ch->buffers[j].BufferID, AL_FORMAT_MONO16,
                             ch->buffers[j].Data, STREAMING_BUFFER_SIZE, 22050);

                // Queue Buffer on Source
                alSourceQueueBuffers(ch->alSource, 1,
                                     &(ch->buffers[j].BufferID));
          ...
          if (state != AL_PLAYING) {
            ...
            char szString[256];
            sprintf(szString,
                    "[%d] Restarting playback of single-shot streaming MP3 "
                    "sample - still have %d bytes to decode\n",
...
void UpdateLoopingSounds() {
  ...
  // First check to see if any of the looping sounds are already playing at the
  // correct positions
  ch = s_channels + 1;
  for (i = 1; i < s_numChannels; i++, ch++) {
    if (ch->bLooping && s_channels[i].bPlaying) {
      for (j = 0; j < numLoopSounds; j++) {
        ...
        // If this channel is playing the right sound effect at the right
        // position then mark this channel and looping sound as processed
        if ((loop->bProcessed == false) && (ch->thesfx == loop->sfx)) {
          ...
          } else if ((loop->bProcessed == false) && (ch->thesfx == loop->sfx) &&
                     (!memcmp(ch->origin, loop->origin, sizeof(ch->origin)))) {
            // Match !
            ch->bProcessed = true;
            loop->bProcessed = qtrue;
  ...
  // Next check if the correct looping sound is playing, but at the wrong
  // position
  ch = s_channels + 1;
  for (i = 1; i < s_numChannels; i++, ch++) {
  ...
  // If any non-procesed looping sounds are still playing on a channel, they can
  // be removed as they are no longer required
  ch = s_channels + 1;
  for (i = 1; i < s_numChannels; i++, ch++) {
  ...
  alGetError();
#endif
  // Finally if there are any non-processed sounds left, we need to try and play
  // them
  for (j = 0; j < numLoopSounds; j++) {
    loop = &loopSounds[j];
    ...
    if (!loop->bProcessed) {
      ...
      // Check if the Source is positioned at exactly the same location as the
      // listener
      if ((loop->origin[0] == listener_pos[0]) &&
          (loop->origin[1] == -listener_pos[2]) &&
          (loop->origin[2] == listener_pos[1])) {
      ...
      alSourcef(s_channels[source].alSource, AL_GAIN,
                ((float)(ch->master_vol) * s_volume->value) / 255.0f);
      alSourcei(s_channels[source].alSource, AL_SOURCE_RELATIVE,
                ch->fixed_origin ? AL_TRUE : AL_FALSE);
...
void AL_UpdateRawSamples() {
  ...
  while (processed) {
    // Unqueue each buffer, determine the length of the buffer, and then delete
    // it
  ...
  if (s_rawend > s_paintedtime) {
    ...
    if (size > (MAX_RAW_SAMPLES << 2)) {
      OutputDebugString(
          "UpdateRawSamples :- Raw Sample buffer has overflowed !!!\n");
    ...
    //		copy sg.rawdata + ((iterations * largestBufferSize)>>1) to
    //buffer 		size -= largestBufferSize 	else 		copy remainder 		size = 0 	queue the
    //buffer
    //  iterations++;

    int iterations = 0;
    int largestBufferSize =
        MAX_RAW_SAMPLES; // in bytes (== quarter of Raw Samples data)
    while (size) {
      ...
      if (size > largestBufferSize) {
        alBufferData(
            ...
            (char *)(s_rawdata + ((iterations * largestBufferSize) >> 1)),
            largestBufferSize, 22050);
        size -= largestBufferSize;
      } else {
        alBufferData(
            buffer, AL_FORMAT_STEREO16,
            (char *)(s_rawdata + ((iterations * largestBufferSize) >> 1)), size,
            22050);
        size = 0;
      }
    ...
    if (state != AL_PLAYING) {
      ...
      // Unqueue any buffers still on the Source (they will be PROCESSED), and
      // restart playback
      alGetSourcei(s_channels[0].alSource, AL_BUFFERS_PROCESSED, &processed);
      while (processed) {
        alSourceUnqueueBuffers(s_channels[0].alSource, 1, &buffer);
        ...
        // Update sg.soundtime (+= number of samples played (number of bytes /
        // 4))
        s_soundtime += (size >> 2);
      }
  ...
  for (i = 1; i < s_numChannels; i++, ch++) {
    ...
    if (ch->entchannel == CHAN_VOICE || ch->entchannel == CHAN_VOICE_ATTEN ||
      ...
      if (ch->thesfx->eSoundCompressionMethod == ct_16) {
        // There is a new computed lip-sync value every 1000 samples - so find
        // out how many samples have been played and lookup the value in the
        // lip-sync table
        ...
        if ((ch->thesfx->lipSyncData) &&
            (samples < (unsigned)ch->thesfx->iSoundLengthInSamples)) {
          s_entityWavVol[ch->entnum] = ch->thesfx->lipSyncData[samples / 1000];

          //					Com_Printf("%s,  total samples = %d, current
          //sample = %d, lip type = %d \n", ch->thesfx->sSoundName,
          //ch->thesfx->iSoundLengthInSamples, samples, s_entityWavVol[
          //ch->entnum ] );
          if (s_show->integer == 3) {
            Com_Printf("(%i)%i %s vol = %i\n", ch->entnum, i,
                       ch->thesfx->sSoundName, s_entityWavVol[ch->entnum]);
        ...
        // There is a new computed lip-sync value every 576 samples - so find
        // out how many samples have been played and lookup the value in the
        // lip-sync table
        samples = (timePlayed * 22050) / 1000;
        ...
        if ((ch->thesfx->lipSyncData) &&
            (samples < (unsigned)ch->thesfx->iSoundLengthInSamples)) {
          s_entityWavVol[ch->entnum] =
              ch->thesfx->lipSyncData[(samples / 576) % 16];
...
static void S_SetDynamicMusic_f(void) {
  ...
  if (c == 2) {
    if (bMusic_IsDynamic) {
      // don't need to check existance of 'explore' or 'action' music, since
      // music wouldn't
      //	be counted as dynamic if either were missing, but other types
      //are optional...
      //
...
// this table needs to be in-sync with the typedef'd enum
// "SoundCompressionMethod_t"...	-ste
//
...
void S_SoundList_f(void) {
  ...
  if (Cmd_Argc() == 2) {
    ...
    Com_Printf(
        ...
        "'ShouldBeMP3', '1'/'2'/'3' for %%d-variant capping )\n");
  }
  ...
  for (sfx = s_knownSfx, i = 0; i < s_numSfx; i++, sfx++) {
    ...
    qboolean bMP3DumpOverride =
        (qboolean)(bShouldBeMP3 && cv_MP3overhead && !sfx->bDefaultSound &&
                   !sfx->pMP3StreamHeader && sfx->pSoundData &&
                   (Z_Size(sfx->pSoundData) > cv_MP3overhead->integer));

    if (bMP3DumpOverride ||
        (!bShouldBeMP3 &&
         (!bWavOnly || sfx->eSoundCompressionMethod == ct_16))) {
      ...
      if (iVariantCap >= 1 && iVariantCap <= 3) {
        ...
        if (iStrLen > 2) // crash-safety, jic.
        {
          ...
          if (!isdigit(c2) // quick-avoid of stuff like "pain75"
            ...
            // need to see if this %d-variant should be omitted, in other words
            // if there's a %1 version then skip this...
            //
            ...
            for (sfx2 = s_knownSfx, i2 = 0; i2 < s_numSfx; i2++, sfx2++) {
              if (!Q_stricmp(sFindName, sfx2->sSoundName)) {
                bDumpThisOne = qfalse; // found a %1-variant of this, so use
                                       // variant capping and ignore this sfx_t
                break;
              }
            }
          }
      ...
      if (sfx->bDefaultSound) {
        ...
        if (bDumpThisOne) {
          iTotalBytes +=
              (sfx->bInMemory && sfx->pSoundData) ? Z_Size(sfx->pSoundData) : 0;
          iTotalBytes += (sfx->bInMemory && sfx->pMP3StreamHeader)
                             ? sizeof(*sfx->pMP3StreamHeader)
                             : 0;
          total += sfx->bInMemory ? size : 0;
        }
        Com_Printf("%5d %7i [%s] %s %2d %s", i, size,
                   sSoundCompressionMethodStrings[sfx->eSoundCompressionMethod],
                   sfx->bInMemory ? "y" : "n", sfx->iLastLevelUsedOn,
                   sfx->sSoundName);
  ...
  Com_Printf(
      "Total resident samples: %i %s ( not mem usage, see 'meminfo' ).\n",
      total, bWavOnly ? "(WAV only)" : "");
  Com_Printf("%d out of %d sfx_t slots used\n", s_numSfx, MAX_SFX);
  Com_Printf("%.2fMB bytes used when counting sfx_t->pSoundData + MP3 headers "
             "(if any)\n",
...
// fixme: need to move this into qcommon sometime?, but too much stuff altered
// by other people and I won't be able
//	to compile again for ages if I check that out...
//
// DO NOT replace this with a call to FS_FileExists, that's for checking about
// writing out, and doesn't work for this.
//
qboolean S_FileExists(const char *psFilename) {
  ...
  FS_FOpenFileRead(
      psFilename, &fhTemp,
      qtrue); // qtrue so I can fclose the handle without closing a PAK
  if (!fhTemp)
    return qfalse;
...
// some stuff for streaming MP3 files from disk (not pleasant, but nothing about
// MP3 is, other than compression ratios...)
//
...
//
static byte *MP3MusicStream_ReadFromDisk(MusicInfo_t *pMusicInfo,
                                         int iReadOffset,
                                         int iReadBytesNeeded) {
  if (iReadOffset < pMusicInfo->iMP3MusicStream_DiskWindowPos) {
    assert(0); // should never happen
    return pMusicInfo
        ->byMP3MusicStream_DiskBuffer; // ...but return something safe anyway
  }

  while (iReadOffset + iReadBytesNeeded >
         pMusicInfo->iMP3MusicStream_DiskReadPos) {
    int iBytesRead =
        FS_Read(pMusicInfo->byMP3MusicStream_DiskBuffer +
                    (pMusicInfo->iMP3MusicStream_DiskReadPos -
    ...
    if (iBytesRead !=
        iMP3MusicStream_DiskBytesToRead) // quietly ignore any requests to read
                                         // past file end
    {
      break; // we need to do this because the disk read code can't know how
             // much source data we need to
             //	read for a given number of requested output bytes, so we'll
             //always be asking for too many
    }
  }

  // if reached halfway point in buffer (approx 20k), backscroll it...
  //
  if (pMusicInfo->iMP3MusicStream_DiskReadPos -
          pMusicInfo->iMP3MusicStream_DiskWindowPos >
      iMP3MusicStream_DiskBufferSize / 2) {
    int iMoveSrcOffset =
        iReadOffset - pMusicInfo->iMP3MusicStream_DiskWindowPos;
    int iMoveCount = (pMusicInfo->iMP3MusicStream_DiskReadPos -
                      pMusicInfo->iMP3MusicStream_DiskWindowPos) -
                     iMoveSrcOffset;
    memmove(&pMusicInfo->byMP3MusicStream_DiskBuffer,
            &pMusicInfo->byMP3MusicStream_DiskBuffer[iMoveSrcOffset],
            iMoveCount);
    pMusicInfo->iMP3MusicStream_DiskWindowPos += iMoveSrcOffset;
  }

  return pMusicInfo->byMP3MusicStream_DiskBuffer +
         (iReadOffset - pMusicInfo->iMP3MusicStream_DiskWindowPos);
}
...
static void FreeMusic(MusicInfo_t *pMusicInfo) {
  if (pMusicInfo->pLoadedData) {
    ...
    pMusicInfo->pLoadedData =
        NULL; // these two MUST be kept as valid/invalid together
...
static qboolean S_StartBackgroundTrack_Actual(MusicInfo_t *pMusicInfo,
                                              qboolean qbDynamic,
                                              const char *intro,
                                              const char *loop) {
  int len;
  ...
  Q_strncpyz(name, intro,
             sizeof(name) -
                 4); // this seems to be so that if the filename hasn't got an
                     // extension
                     //	but doesn't have the room to append on either then
                     //you'll just 	get the "soft" fopen() error, rather than the
                     //ERR_DROP you'd get 	if COM_DefaultExtension didn't have
                     //room to add it on.
  COM_DefaultExtension(name, sizeof(name), ".mp3");

  // close the background track, but DON'T reset s_rawend (or remaining music
  // bits that haven't been output yet will be cut off)
  //
  ...
  // new bit, if file requested is not same any loaded one (if prev was in-mem),
  // ditch it...
  //
  ...
  if (!Q_stricmpn(name + (strlen(name) - 4), ".mp3", 4)) {
    ...
    int iInitialMP3ReadSize =
        8192; // fairly arbitrary, whatever size this is then the decoder is
              // allowed to scan up to halfway of it to find floating headers,
              // so don't make it too small. 8k works fine.
    qboolean bMusicSucceeded = qfalse;
    if (qbDynamic) {
      if (!pMusicInfo->pLoadedData) {
        pMusicInfo->pLoadedData = (byte *)Z_Malloc(
            pMusicInfo->iLoadedDataLen, TAG_SND_DYNAMICMUSIC, qfalse);

        S_ClearSoundBuffer();
        FS_Read(pMusicInfo->pLoadedData, pMusicInfo->iLoadedDataLen,
                pMusicInfo->s_backgroundFile);
        Q_strncpyz(pMusicInfo->sLoadedDataName, name,
                   sizeof(pMusicInfo->sLoadedDataName));
      }
      ...
      pbMP3DataSegment =
          MP3MusicStream_ReadFromDisk(pMusicInfo, 0, iInitialMP3ReadSize);
    }

    if (MP3_IsValid(name, pbMP3DataSegment, iInitialMP3ReadSize,
                    qtrue /*bStereoDesired*/)) {
      // init stream struct...
      //
      memset(&pMusicInfo->streamMP3_Bgrnd, 0,
             sizeof(pMusicInfo->streamMP3_Bgrnd));
      char *psError =
          C_MP3Stream_DecodeInit(&pMusicInfo->streamMP3_Bgrnd, pbMP3DataSegment,
                                 pMusicInfo->iLoadedDataLen, dma.speed,
                                 16,   // sfx->width * 8,
                                 qtrue // bStereoDesired
          );
      ...
      if (psError == NULL) {
        ...
        //			pMusicInfo->sfxMP3_Bgrnd.width
        //= 2;			// read by MP3_GetSamples()
        pMusicInfo->sfxMP3_Bgrnd.iSoundLengthInSamples =
            0x7FFFFFFF; // max possible +ve int, since music finishes when
                        // decoder stops
        pMusicInfo->sfxMP3_Bgrnd.pMP3StreamHeader =
            &pMusicInfo->streamMP3_Bgrnd;
        Q_strncpyz(pMusicInfo->sfxMP3_Bgrnd.sSoundName, name,
                   sizeof(pMusicInfo->sfxMP3_Bgrnd.sSoundName));

        if (qbDynamic) {
          MP3Stream_InitPlayingTimeFields(&pMusicInfo->streamMP3_Bgrnd, name,
                                          pbMP3DataSegment,
                                          pMusicInfo->iLoadedDataLen, qtrue);
        }

        pMusicInfo->s_backgroundInfo.format =
            WAV_FORMAT_MP3; // not actually used this way, but just ensures we
                            // don't match one of the legit formats
        pMusicInfo->s_backgroundInfo.channels =
            2; // always, for our MP3s when used for music (else 1 for FX)
        ...
        if (qbDynamic) {
          if (pMusicInfo->s_backgroundFile != -1) {
            FS_FCloseFile(pMusicInfo->s_backgroundFile);
            pMusicInfo->s_backgroundFile =
                -1; // special mp3 value for "valid, but not a real file"
          }
        }
      ...
      // MP3_IsValid() will already have printed any errors via Com_Printf at
      // this point...
      //
      if (pMusicInfo->s_backgroundFile != -1) {
        FS_FCloseFile(pMusicInfo->s_backgroundFile);
  ...
  } else // not an mp3 file
  {
    ...
    pMusicInfo->s_backgroundInfo.width =
        FGetLittleShort(pMusicInfo->s_backgroundFile) / 8;
    ...
    if (pMusicInfo->s_backgroundInfo.channels != 2 ||
        pMusicInfo->s_backgroundInfo.rate != 22050) {
    ...
    pMusicInfo->s_backgroundInfo.samples =
        len / (pMusicInfo->s_backgroundInfo.width *
               pMusicInfo->s_backgroundInfo.channels);
...
static void S_SwitchDynamicTracks(MusicState_e eOldState,
...
// called by both the config-string parser and the console-command
// state-changer...
//
// This either changes the music right now (copying track structures etc), or
// leaves the new state as pending
//	so it gets picked up by the general music player if in a transition that
//can't be overridden...
//
...
static void S_HandleDynamicMusicStateChange(void) {
  if (eMusic_StateRequest != eMusic_StateActual) {
    // check whether or not the new request can be honoured, given what's
    // currently playing...
    //
    if (Music_StateCanBeInterrupted(eMusic_StateActual, eMusic_StateRequest)) {
      LP_MP3STREAM pMP3StreamActual =
          &tMusic_Info[eMusic_StateActual].chMP3_Bgrnd.MP3StreamHeader;
      ...
      case eBGRNDTRACK_EXPLORE: // ... from action or silence
      {
        ...
        {
          // find the transition track to play, and the entry point for explore
          // when we get there,
          //	and also see if we're at a permitted exit point to switch at
          //all...
          //
          float fPlayingTimeElapsed =
              MP3Stream_GetPlayingTimeInSeconds(pMP3StreamActual) -
          ...
          if (Music_AllowedToTransition(fPlayingTimeElapsed, eBGRNDTRACK_ACTION,
                                        &eTransition, &fNewTrackEntryTime)) {
            S_SwitchDynamicTracks(eMusic_StateActual, eTransition,
                                  qfalse); // qboolean bNewTrackStartsFullVolume
        ...
        {
          S_SwitchDynamicTracks(eMusic_StateActual, eMusic_StateRequest,
                                qfalse); // qboolean bNewTrackStartsFullVolume

          //							float fEntryTime =
          //Music_GetRandomEntryTime( eMusic_StateRequest ); 							tMusic_Info[
          //eMusic_StateRequest ].SeekTo(fEntryTime);
          tMusic_Info[eMusic_StateRequest].Rewind();
        } break;

        default: // trying to transition from some state I wasn't aware you
                 // could transition from (shouldn't happen), so ignore
        {
          assert(0);
          S_SwitchDynamicTracks(eMusic_StateActual, eBGRNDTRACK_SILENCE,
                                qfalse); // qboolean bNewTrackStartsFullVolume
        } break;
        }
      } break;
      ...
      case eBGRNDTRACK_SILENCE: // from explore or action
      {
        ...
        case eBGRNDTRACK_EXPLORE: // explore->silence
        {
          ...
          MusicState_e eTransition;
          float fNewTrackEntryTime = 0.0f;
          if (Music_AllowedToTransition(fPlayingTimeElapsed, eMusic_StateActual,
                                        &eTransition, &fNewTrackEntryTime)) {
            S_SwitchDynamicTracks(eMusic_StateActual, eTransition,
                                  qfalse); // qboolean bNewTrackStartsFullVolume
            ...
            tMusic_Info[eTransition].bTrackSwitchPending = qtrue;
            tMusic_Info[eTransition].eTS_NewState = eMusic_StateRequest;
            tMusic_Info[eTransition].fTS_NewTime =
                0.0f; // fNewTrackEntryTime;  irrelevant when switching to
                      // silence
          }
        } break;

        default:     // some unhandled type switching to silence
          assert(0); // fall through since boss case just does silence->switch
                     // anyway
      ...
      case eBGRNDTRACK_ACTION: // anything->action
      {
        ...
        {
          S_SwitchDynamicTracks(eMusic_StateActual, eMusic_StateRequest,
                                qtrue); // qboolean bNewTrackStartsFullVolume
      ...
      case eBGRNDTRACK_BOSS: {
        S_SwitchDynamicTracks(eMusic_StateActual, eMusic_StateRequest,
                              qfalse); // qboolean bNewTrackStartsFullVolume
                                       //
        // ( no need to fast forward or rewind, boss track is only entered into
        // once, at start, and can't exit )
        //
      } break;
      ...
      case eBGRNDTRACK_DEATH: {
        S_SwitchDynamicTracks(eMusic_StateActual, eMusic_StateRequest,
                              qtrue); // qboolean bNewTrackStartsFullVolume
                                      //
        // ( no need to fast forward or rewind, death track is only entered into
        // once, at start, and can't exit or loop)
        //
      } break;
...
void S_RestartMusic(void) {
  if (s_soundStarted && !s_soundMuted) {
    // if (gsIntroMusic[0] || gsLoopMusic[0])	// dont test this anymore (but
    // still *use* them), they're blank for JK2 dynamic-music levels anyway
    {
      MusicState_e ePrevState = eMusic_StateRequest;
      S_StartBackgroundTrack(
          gsIntroMusic, gsLoopMusic,
...
// Basic logic here is to see if the intro file specified actually exists, and
// if so, then it's not dynamic music,
//	When called by the cgame start it loads up, then stops the playback
//(because of stutter issues), so that when the 	actual snapshot is received and
//the real play request is processed the data has already been loaded so will be
//quicker.
//
// to be honest, although the code still plays WAVs some of the file-check logic
// only works for MP3s, so if you ever want
//	to use WAV music you'll have to do some tweaking below (but I've got
//other things to do so it'll have to wait - Ste)
//
void S_StartBackgroundTrack(const char *intro, const char *loop,
                            qboolean bCalledByCGameStart) {
  ...
  // if dynamic music not allowed, then just stream the explore music instead of
  // playing dynamic...
  //
  if (!s_allowDynamicMusic->integer &&
      Music_DynamicDataAvailable(intro)) // "intro", NOT "sName" (i.e. don't use
                                         // version with ".mp3" extension)
  {
  ...
  // conceptually we always play the 'intro'[/sName] track, intro-to-loop
  // transition is handled in UpdateBackGroundTrack().
  //
  if ((strstr(sNameIntro, "/") &&
       S_FileExists(
           sNameIntro))) // strstr() check avoids extra file-exists check at
                         // runtime if reverting from streamed music to dynamic
                         // since literal files all need at least one slash in
                         // their name (eg "music/blah")
  {
    const char *psLoopName = S_FileExists(sNameLoop) ? sNameLoop : sNameIntro;
    Com_DPrintf("S_StartBackgroundTrack: Found/using non-dynamic music track "
                "'%s' (loop: '%s')\n",
                sNameIntro, psLoopName);
    S_StartBackgroundTrack_Actual(&tMusic_Info[eBGRNDTRACK_NONDYNAMIC],
                                  bMusic_IsDynamic, sNameIntro, psLoopName);
  } else {
    if (Music_DynamicDataAvailable(intro)) // "intro", NOT "sName" (i.e. don't
                                           // use version with ".mp3" extension)
    {
      ...
      for (int i = eBGRNDTRACK_DATABEGIN; i != eBGRNDTRACK_DATAEND; i++) {
        ...
        if (psMusicName &&
            (!Q_stricmp(tMusic_Info[i].sLoadedDataName, psMusicName) ||
             S_FileExists(psMusicName))) {
          bOk = S_StartBackgroundTrack_Actual(&tMusic_Info[i], qtrue,
                                              psMusicName, loop);
        }
      ...
      if (tMusic_Info[eBGRNDTRACK_EXPLORE].bExists &&
        ...
        //#ifdef _DEBUG
        //			float fRemaining =
        //MP3Stream_GetPlayingTimeInSeconds(
        //&pMusicInfo->chMP3_Bgrnd.MP3StreamHeader); #endif
      } else {
        Com_Printf(S_COLOR_RED "Dynamic music did not have both 'action' and "
                               "'explore' versions, inhibiting...\n");
        S_StopBackgroundTrack();
      }
    } else {
      if (sNameIntro[0] != '.') // blank name with ".mp3" or whatever attached -
                                // no error print out
      {
        Com_Printf(S_COLOR_RED "Unable to find music \"%s\" as explicit track "
                               "or dynamic music entry!\n",
                   sNameIntro);
        S_StopBackgroundTrack();
      }
    }
...
// qboolean return is true only if we're changing from a streamed intro to a
// dynamic loop...
//
static qboolean S_UpdateBackgroundTrack_Actual(MusicInfo_t *pMusicInfo,
                                               qboolean bFirstOrOnlyMusicTrack,
                                               float fDefaultVolume) {
  int bufferSamples;
  int fileSamples;
  byte raw[30000]; // just enough to fit in a mac stack frame  (note that MP3
                   // doesn't use full size of it)
  int fileBytes;
  int r;
  ...
  if (bMusic_IsDynamic) {
    ...
    if (pMusicInfo->iXFadeVolume != pMusicInfo->iXFadeVolumeSeekTo) {
      int iFadeMillisecondsElapsed =
          Sys_Milliseconds() - pMusicInfo->iXFadeVolumeSeekTime;

      if (iFadeMillisecondsElapsed > (fDYNAMIC_XFADE_SECONDS * 1000)) {
        pMusicInfo->iXFadeVolume = pMusicInfo->iXFadeVolumeSeekTo;
      } else {
        pMusicInfo->iXFadeVolume =
            (int)(255.0f * ((float)iFadeMillisecondsElapsed /
...
// this is to work around an obscure issue to do with sliding decoder windows
// and amounts being requested, since the
//	original MP3 stream-decoder wrapper was designed to work with
//audio-paintbuffer sized pieces... Basically 30000 	is far too big for the
//window decoder to handle in one request because of the time-travel issue
//associated with 	normal sfx buffer painting, and allowing sufficient sliding
//room, even though the music file never goes back in time.
//
...
#define RAWSIZE (pMusicInfo->bIsMP3 ? SIZEOF_RAW_BUFFER_FOR_MP3 : sizeof(raw))
  ...
  pMusicInfo->fSmoothedOutVolume =
      (pMusicInfo->fSmoothedOutVolume + fMasterVol) / 2.0f;
  ...
  while (s_rawend < s_soundtime + MAX_RAW_SAMPLES) {
    ...
    // our max buffer size
    fileBytes = fileSamples * (pMusicInfo->s_backgroundInfo.width *
                               pMusicInfo->s_backgroundInfo.channels);
    if ((unsigned)fileBytes > RAWSIZE) {
      fileBytes = RAWSIZE;
      fileSamples = fileBytes / (pMusicInfo->s_backgroundInfo.width *
                                 pMusicInfo->s_backgroundInfo.channels);
    }

    qboolean qbForceFinish = qfalse;
    if (pMusicInfo->bIsMP3) {
      int iStartingSampleNum =
          pMusicInfo->chMP3_Bgrnd.thesfx->iSoundLengthInSamples -
          pMusicInfo->s_backgroundSamples; // but this IS relevant
      // Com_Printf(S_COLOR_YELLOW "Requesting MP3 samples: sample
      // %d\n",iStartingSampleNum);
      ...
      if (pMusicInfo->s_backgroundFile == -1) {
        ...
        //
        qbForceFinish =
            (MP3Stream_GetSamples(&pMusicInfo->chMP3_Bgrnd, iStartingSampleNum,
                                  fileBytes / 2, (short *)raw, qtrue))
                ? qfalse
                : qtrue;

        // Com_Printf(S_COLOR_YELLOW "Music time remaining: %f seconds\n",
        // MP3Stream_GetRemainingTimeInSeconds(
        // &pMusicInfo->chMP3_Bgrnd.MP3StreamHeader ));
      } else {
        // streaming an MP3 file instead... (note that the 'fileBytes' request
        // size isn't that relevant for MP3s,
        //										since code
        //here can't know how much the MP3 needs to decompress)
        //
      ...
      // byte swap if needed (do NOT do for MP3 decoder, that has an internal
      // big/little endian handler)
      //
      S_ByteSwapRawSamples(fileSamples, pMusicInfo->s_backgroundInfo.width,
                           pMusicInfo->s_backgroundInfo.channels, raw);
    }
    ...
    if (!pMusicInfo->s_backgroundSamples || qbForceFinish) {
      ...
      if (bMusic_IsDynamic) // needs special logic for this, different call
      {
        ...
        // for non-dynamic music we need to check if "sMusic_BackgroundLoop" is
        // an actual filename,
        //	or if it's a dynamic music specifier (which can't literally
        //exist), in which case it should set 	a return flag then exit...
        //
        char sTestName[MAX_QPATH *
                       2]; // *2 so COM_DefaultExtension doesn't do an ERR_DROP
        ...
        if (S_FileExists(sTestName)) {
          S_StartBackgroundTrack_Actual(
              pMusicInfo, qfalse, sMusic_BackgroundLoop, sMusic_BackgroundLoop);
        } else {
          // proposed file doesn't exist, but this may be a dynamic track we're
          // wanting to loop,
...
// used to be just for dynamic, but now even non-dynamic music has to know
// whether it should be silent or not...
//
...
// (note that even if the state doesn't change it still gets here, so do a
// same-state check for applying)
//
...
static void S_UpdateBackgroundTrack(void) {
  if (bMusic_IsDynamic) {
    ...
    if (eMusic_StateActual != eBGRNDTRACK_SILENCE) {
      MusicInfo_t *pMusicInfoCurrent =
          &tMusic_Info[(eMusic_StateActual == eBGRNDTRACK_FADE)
                           ? eBGRNDTRACK_EXPLORE
                           : eMusic_StateActual];
      ...
      if (pMusicInfoCurrent->s_backgroundFile == -1) {
        ...
        /*			static int iPrevFrontVol = 0;
           ...
           pMusicInfoCurrent->iXFadeVolume)
                                {
                                        iPrevFrontVol  =
           pMusicInfoCurrent->iXFadeVolume; Com_Printf("front vol =
           %d\n",pMusicInfoCurrent->iXFadeVolume);
                                }
        */
        if (pMusicInfoFadeOut->bActive) {
          ...
          S_UpdateBackgroundTrack_Actual(
              pMusicInfoFadeOut, qfalse,
              s_musicVolume->value); // inactive-checked internally
                                     /*
                                        ...
                                        pMusicInfoFadeOut->iXFadeVolume)
                                                                     {
                                                                             iPrevFadeVol  =
                                        pMusicInfoFadeOut->iXFadeVolume;                            Com_Printf("fade vol =
                                        %d\n",pMusicInfoFadeOut->iXFadeVolume);
                                                                     }
                                     */
          //
        ...
        float fRemainingTimeInSeconds = MP3Stream_GetRemainingTimeInSeconds(
        ...
        if (fRemainingTimeInSeconds < fDYNAMIC_XFADE_SECONDS * 2) {
          // now either loop current track, switch if finishing a transition, or
          // stop if finished a death...
          //
          if (pMusicInfoCurrent->bTrackSwitchPending) {
            pMusicInfoCurrent->bTrackSwitchPending = qfalse; // ack
            S_SwitchDynamicTracks(eMusic_StateActual,
                                  pMusicInfoCurrent->eTS_NewState,
                                  qfalse); // qboolean bNewTrackStartsFullVolume
            if (tMusic_Info[pMusicInfoCurrent->eTS_NewState]
                    .bExists) // don't do this if switching to silence
            {
              tMusic_Info[pMusicInfoCurrent->eTS_NewState].SeekTo(
                  pMusicInfoCurrent->fTS_NewTime);
            }
          } else {
            // normal looping, so set rewind current track, set volume to 0 and
            // fade up to full (unless death track playing, then stays quiet)
            ...
            *pMusicInfoFadeOut = *pMusicInfoCurrent; // struct copy
            ...
            pMusicInfoCurrent->iXFadeVolumeSeekTo =
                (eMusic_StateActual == eBGRNDTRACK_DEATH) ? 0 : 255;
            pMusicInfoCurrent->iXFadeVolume = 0;
          }
        }
      }
    } else {
      // special case, when foreground music is shut off but fader still running
      // to fade off previous track...
      //
      MusicInfo_t *pMusicInfoFadeOut = &tMusic_Info[eBGRNDTRACK_FADE];
      if (pMusicInfoFadeOut->bActive) {
    ...
    const char *psCommand =
        S_Music_GetRequestedState(); // special check just for "silence" case...
    qboolean bShouldBeSilent =
        (qboolean)(psCommand && !Q_stricmp(psCommand, "silence"));
    float fDesiredVolume = bShouldBeSilent ? 0.0f : s_musicVolume->value;
    //
    // internal to this code is a volume-smoother...
    //
    qboolean bNewTrackDesired = S_UpdateBackgroundTrack_Actual(
        &tMusic_Info[eBGRNDTRACK_NONDYNAMIC], qtrue, fDesiredVolume);

    if (bNewTrackDesired) {
      S_StartBackgroundTrack(sMusic_BackgroundLoop, sMusic_BackgroundLoop,
                             qfalse);
    }
  }
}
...
// currently passing in sfx as a param in case I want to do something with it
// later.
//
byte *SND_malloc(int iSize, sfx_t *sfx) {
  byte *pData = (byte *)Z_Malloc(iSize, TAG_SND_RAWDATA,
                                 qfalse); // don't bother asking for zeroed mem
  ...
  if (s_soundpoolmegs && s_soundpoolmegs->integer < 0) {
    while ((Z_MemSize(TAG_SND_RAWDATA) + Z_MemSize(TAG_SND_MP3STREAMHDR)) >
           ((-s_soundpoolmegs->integer) * 1024 * 1024)) {
...
void S_DisplayFreeMemory() {
  int iSoundDataSize =
      Z_MemSize(TAG_SND_RAWDATA) + Z_MemSize(TAG_SND_MP3STREAMHDR);
...
//
// new param is so we can be usre of not freeing ourselves (without having to
// rely on possible uninitialised timers etc)
//
int SND_FreeOldestSound(sfx_t *pButNotThisOne /* = NULL */) {
  ...
  for (int i = 1; i < s_numSfx; i++) {
    ...
    if (sfx != pButNotThisOne) {
      if (!sfx->bDefaultSound && sfx->bInMemory &&
...
int SND_FreeOldestSound(void) {
  return SND_FreeOldestSound(NULL); // I had to add a void-arg version of this
                                    // because of link issues, sigh
}

// just before we drop into a level, ensure the audio pool is under whatever the
// maximum
//	pool size is (but not by dropping out sounds used by the current
//level)...
//
// returns qtrue if at least one sound was dropped out, so z_malloc-fail
// recovery code knows if anything changed
//
extern qboolean gbInsideLoadSound;
qboolean SND_RegisterAudio_LevelLoadEnd(
    qboolean bDeleteEverythingNotUsedThisLevel /* 99% qfalse */) {
  ...
  if (gbInsideLoadSound) {
    ...
    int iLoadedAudioBytes =
        Z_MemSize(TAG_SND_RAWDATA) + Z_MemSize(TAG_SND_MP3STREAMHDR);
    const int iMaxAudioBytes = s_soundpoolmegs->integer * 1024 * 1024;

    for (int i = 1; i < s_numSfx && (iLoadedAudioBytes > iMaxAudioBytes ||
                                     bDeleteEverythingNotUsedThisLevel);
         i++) // i=1 so we never page out default sound
    {
      ...
      if (sfx->bInMemory) {
        ...
        if (bDeleteEverythingNotUsedThisLevel) {
          bDeleteThis =
              (qboolean)(sfx->iLastLevelUsedOn != re.RegisterMedia_GetLevel());
        } else {
          bDeleteThis =
              (qboolean)(sfx->iLastLevelUsedOn < re.RegisterMedia_GetLevel());
        }
...
void InitEAXManager() {
  ...
  if (s_bEAX) {
    ...
    // Support for EAXUnified (automatic translation of EAX 4.0 calls into
    // EAX 3.0)
    if ((alIsExtensionPresent((ALubyte *)"EAX3.0")) &&
        (alIsExtensionPresent((ALubyte *)"EAX4.0Emulated"))) {
      s_bEAX = AL_TRUE;
  ...
  if (s_bEAX) {
    ...
    if (s_hEAXManInst) {
      lpEAXManagerCreateFn =
          (LPEAXMANAGERCREATE)GetProcAddress(s_hEAXManInst, "EaxManagerCreate");
      if (lpEAXManagerCreateFn) {
        if (lpEAXManagerCreateFn(&s_lpEAXManager) == EM_OK) {
          ...
          for (i = 0; i < EAX_MAX_FXSLOTS; i++) {
            if (s_eaxSet(&FXSlotGuids[i], EAXFXSLOT_ALLPARAMETERS, NULL,
                         &FXSlotProp,
              ...
              // If this slot already contains a reverb, then we will use it
              // anyway (Slot 0 will be in this category).  (It probably means
              // that Slot 0 is locked)
              if (s_eaxGet(&FXSlotGuids[i], EAXFXSLOT_LOADEFFECT, NULL, &Effect,
                           sizeof(GUID)) == AL_NO_ERROR) {
                if (Effect == EAX_REVERB_EFFECT) {
                  // We can use this slot
                  // Make sure the environment flag is on
                  s_eaxSet(&FXSlotGuids[i], EAXFXSLOT_FLAGS, NULL,
                           &FXSlotProp.ulFlags, sizeof(unsigned long));
                  s_FXSlotInfo[s_NumFXSlots].FXSlotGuid = FXSlotGuids[i];
                  s_NumFXSlots++;
                }
              }
  ...
  // If the EAXManager library was loaded (and there was a problem), then unload
  // it
  if (s_hEAXManInst) {
...
static bool LoadEALFile(char *szEALFilename) {
  ...
  if (bLoaded) {
    // For a valid eal file ... need to find 'Center' tag, record num of
    // instances,  and then find the right number of instances of 'Aperture0a'
    // and 'Aperture0b'.

    if (s_lpEAXManager->GetSourceID("Center", &lID) == EM_OK) {
      if (s_lpEAXManager->GetSourceNumInstances(lID, &s_lNumEnvironments) ==
          EM_OK) {
        if (s_lpEAXManager->GetSourceID("Aperture0a", &lID) == EM_OK) {
          if (s_lpEAXManager->GetSourceNumInstances(lID, &lNumInst) == EM_OK) {
            if (lNumInst == s_lNumEnvironments) {
              if (s_lpEAXManager->GetSourceID("Aperture0b", &lID) == EM_OK) {
                if (s_lpEAXManager->GetSourceNumInstances(lID, &lNumInst) ==
                    EM_OK) {
                  if (lNumInst == s_lNumEnvironments) {
                    ...
                    while (true) {
                      ...
                      if ((s_lpEAXManager->GetSourceID(szAperture, &lID) ==
                           EM_OK) &&
                          (s_lpEAXManager->GetSourceNumInstances(
                               lID, &lNumInstA) == EM_OK)) {
                        ...
                        if (lNumInstA != lNumInstB) {
                          Com_DPrintf(S_COLOR_YELLOW
                                      "Invalid EAL file - %d Aperture%da tags, "
                                      "and %d Aperture%db tags\n",
                    ...
                    if (bValid) {
                      s_lpEnvTable = (LPENVTABLE)Z_Malloc(s_lNumEnvironments *
                                                              sizeof(ENVTABLE),
                                                          TAG_NEWDEL, qtrue);
                    }
                  } else
                    Com_DPrintf(S_COLOR_YELLOW
                                "Invalid EAL File - expected %d instances of "
                                "Aperture0b, found %d\n",
                                s_lNumEnvironments, lNumInst);
                } else
                  Com_DPrintf(S_COLOR_YELLOW
                              "EAXManager- failed GetSourceNumInstances()\n");
              } else
                Com_DPrintf(S_COLOR_YELLOW "Invalid EAL File - no instances of "
                                           "'Aperture0b' source-tag\n");
            } else
              Com_DPrintf(
                  S_COLOR_YELLOW
                  "Invalid EAL File - found %d instances of the 'Center' tag, "
                  "but only %d instances of 'Aperture0a'\n",
                  s_lNumEnvironments, lNumInst);
          } else
            Com_DPrintf(S_COLOR_YELLOW
                        "EAXManager- failed GetSourceNumInstances()\n");
          ...
          Com_DPrintf(
              S_COLOR_YELLOW
              "Invalid EAL File - no instances of 'Aperture0a' source-tag\n");
      } else
        Com_DPrintf(S_COLOR_YELLOW
                    "EAXManager- failed GetSourceNumInstances()\n");
    } else
      Com_DPrintf(S_COLOR_YELLOW
                  "Invalid EAL File - no instances of 'Center' source-tag\n");
    ...
    if (s_lpEnvTable) {
      ...
      while (true) {
        ...
        if (s_lpEAXManager->GetSourceID(szAperture, &lID) == EM_OK) {
          if (s_lpEAXManager->GetSourceNumInstances(lID, &lNumInst) == EM_OK) {
            ...
            for (j = 0; j < lNumInst; j++) {
              if (s_lpEAXManager->GetSourceInstancePos(lID, j, &EMPoint) ==
                  EM_OK) {
                if (s_lpEAXManager->GetListenerDynamicAttributes(
                        0, &EMPoint, &lEnvID, 0) == EM_OK) {
                  if ((lEnvID >= 0) && (lEnvID < s_lNumEnvironments)) {
                    ...
                    if (!s_lpEnvTable[lEnvID].bUsed) {
                      ...
                      s_lpEnvTable[lEnvID]
                          .Aperture[s_lpEnvTable[lEnvID].ulNumApertures]
                          .vPos1[0] = EMPoint.fX;
                      s_lpEnvTable[lEnvID]
                          .Aperture[s_lpEnvTable[lEnvID].ulNumApertures]
                          .vPos1[1] = EMPoint.fY;
                      ...
                      Com_DPrintf(S_COLOR_YELLOW
                                  "Found more than one occurance of "
                                  "Aperture%da in %s sub-space\n",
                                  i, szString);
                      Com_DPrintf(
                          S_COLOR_YELLOW "One tag at %.3f,%.3f,%.3f, other at "
                                         "%.3f,%.3f,%.3f\n",
                    ...
                    if (lEnvID == -1)
                      Com_DPrintf(
                          S_COLOR_YELLOW "%s (%.3f,%.3f,%.3f) in Default "
                                         "Environment - please remove\n",
                          szAperture, EMPoint.fX, EMPoint.fY, EMPoint.fZ);
                    else
                      Com_DPrintf(S_COLOR_YELLOW
                                  "Detected more reverb presets than zones - "
                                  "please delete unused presets\n");
                    bValid = false;
                  }
                }
              }
                  ...
                  if (s_lpEAXManager->GetListenerDynamicAttributes(
                    ...
                    if ((lEnvID >= 0) && (lEnvID < s_lNumEnvironments)) {
                      if (!s_lpEnvTable[lEnvID].bUsed) {
                        ...
                        s_lpEAXManager->GetEnvironmentName(lEnvID, szString,
                                                           256);
                        Com_DPrintf(S_COLOR_YELLOW
                                    "Found more than one occurance of "
                                    "Aperture%db in %s sub-space\n",
                                    i, szString);
                        bValid = false;
                      }

                      // Calculate center position of aperture (average of 2
                      // points)
        ...
        if (!bValid) {
          // Found a problem
          Com_DPrintf(S_COLOR_YELLOW
                      "EAX legacy behaviour invoked (one reverb)\n");
...
#ifdef USE_OPENAL
/*
        Updates the current EAX Reverb setting, based on the location of the
   listener
*/
static void UpdateEAXListener() {
  ...
  if ((s_lLastEnvUpdate + ENV_UPDATE_RATE) < lCurTime) {
    ...
    // No panning information in .eal file, or we only have 1 FX Slot to use,
    // revert to legacy behaviour (i.e only one reverb)
    if ((!s_lpEnvTable) || (s_NumFXSlots == 1)) {
      ...
      if (SUCCEEDED(s_lpEAXManager->GetListenerDynamicAttributes(
              0, &ListPos, &lID, EMFLAG_LOCKPOSITION))) {
        if (lID != s_EnvironmentID) {
#ifdef DISPLAY_CLOSEST_ENVS
          if (SUCCEEDED(
                  s_lpEAXManager->GetEnvironmentName(lID, szEnvName, 256)))
            Com_Printf("Changing to '%s' zone !\n", szEnvName);
#endif
          // Get EAX Preset info.
          if (SUCCEEDED(
                  s_lpEAXManager->GetEnvironmentAttributes(lID, &s_eaxLPCur))) {
            // Override
    ...
    if (s_lpEAXManager->GetListenerDynamicAttributes(
            0, &ListPos, &lID, EMFLAG_LOCKPOSITION) == EM_OK) {
      if (lID == -1) {
        // Found default environment
        //				Com_Printf( S_COLOR_YELLOW "Listener in
        //default environment - ignoring zone !\n");
        return;
      }
      ...
      for (i = 0; i < s_lNumEnvironments; i++) {
        ...
        if (i != lID) {
          ...
          // Now have closest point for this Environment - see if this is closer
          // than any others
    ...
    for (i = 0; i < s_NumFXSlots; i++) {
      if ((s_FXSlotInfo[i].lEnvID != -1) &&
          (s_FXSlotInfo[i].lEnvID != ReverbData[0].lEnvID) &&
          (s_FXSlotInfo[i].lEnvID != ReverbData[1].lEnvID) &&
          (s_FXSlotInfo[i].lEnvID != ReverbData[2].lEnvID)) {
        // This environment is no longer needed

        // Mute it
        lVolume = -10000;
        if (s_eaxSet(&s_FXSlotInfo[i].FXSlotGuid, EAXFXSLOT_VOLUME, NULL,
                     &lVolume, sizeof(long)) != AL_NO_ERROR)
          OutputDebugString("Failed to Mute FX Slot\n");

        // If any source is sending to this Slot ID then we need to stop them
        // sending to the slot
        for (j = 1; j < s_numChannels; j++) {
          if (s_channels[j].lSlotID == i) {
            if (s_eaxSet(&EAXPROPERTYID_EAX40_Source, EAXSOURCE_ACTIVEFXSLOTID,
                         s_channels[j].alSource, (void *)&EAX_NULL_GUID,
                         sizeof(GUID)) != AL_NO_ERROR) {
              OutputDebugString(
        ...
        assert(s_FXSlotInfo[i].lEnvID < s_lNumEnvironments &&
               s_FXSlotInfo[i].lEnvID >= 0);
        if (s_FXSlotInfo[i].lEnvID < s_lNumEnvironments &&
            s_FXSlotInfo[i].lEnvID >= 0) {
    ...
    // Make sure all the reverbs we want are being rendered, if not, find an
    // empty slot and apply appropriate reverb settings
    for (j = 0; j < 3; j++) {
      bFound = false;
      ...
      if (!bFound) {
        ...
        for (i = 0; i < s_NumFXSlots; i++) {
          if (s_FXSlotInfo[i].lEnvID == -1) {
            ...
            if (s_lpEAXManager->GetEnvironmentAttributes(ReverbData[j].lEnvID,
                                                         &Reverb) == EM_OK) {
              // Override Air Absorption HF
              Reverb.flAirAbsorptionHF = 0.0f;

              s_eaxSet(&s_FXSlotInfo[i].FXSlotGuid, EAXREVERB_ALLPARAMETERS,
                       NULL, &Reverb, sizeof(EAXREVERBPROPERTIES));

              // See if any Sources are in this environment, if they are, enable
              // their sends
              ch = s_channels + 1;
              for (k = 1; k < s_numChannels; k++, ch++) {
                if (ch->fixed_origin) {
                  ...
                  if (ch->entnum == listener_number) {
                    ...
                    // Probably won't be any Occlusion / Obstruction effect --
                    // unless the listener is underwater Converting from Open AL
                    // -> DS3D (for EAGLE) ... invert Z
                    EMSourcePoint.fX = listener_pos[0];
                    EMSourcePoint.fY = listener_pos[1];
                    EMSourcePoint.fZ = -listener_pos[2];
                  } else {
                    // Get position of Entity
                    // Converting from Quake -> DS3D (for EAGLE) ... swap Y and
                    // Z
                ...
                if (s_lpEAXManager->GetListenerDynamicAttributes(
                        0, &EMSourcePoint, &lSourceID, 0) != EM_OK)
                  OutputDebugString(
                      "Failed to get environment zone for Source\n");

                if (lSourceID == i) {
                  if (s_eaxSet(&EAXPROPERTYID_EAX40_Source,
                               EAXSOURCE_ACTIVEFXSLOTID, ch->alSource,
                               (void *)&(s_FXSlotInfo[i].FXSlotGuid),
                               sizeof(GUID)) != AL_NO_ERROR) {
                    OutputDebugString("Failed to set Source ActiveFXSlotID to "
                                      "new environment\n");
                  }
              ...
              assert(ReverbData[j].lEnvID < s_lNumEnvironments &&
                     ReverbData[j].lEnvID >= 0);
              if (ReverbData[j].lEnvID < s_lNumEnvironments &&
                  ReverbData[j].lEnvID >= 0) {
    ...
    if (s_EnvironmentID != ReverbData[2].lEnvID) {
      s_eaxSet(&EAXPROPERTYID_EAX40_Context, EAXCONTEXT_PRIMARYFXSLOTID, NULL,
               &(s_FXSlotInfo[s_lpEnvTable[ReverbData[2].lEnvID].lFXSlotID]
                     .FXSlotGuid),
               sizeof(GUID));
    ...
    // We need to rotate the vector from the Listener to the reverb Aperture by
    // minus the listener orientation

    // Need dot product of Listener Orientation and the straight ahead vector
    // (0, 0, 1)

    // Since both vectors are already normalized, and two terms cancel out
    // (0's), the angle is the arc cosine of the z component of the Listener
    // Orientation

    float flTheta = (float)acos(ListOri.fZ);

    // If the Listener Orientation is to the left of straight ahead, then invert
    // the angle
    if (ListOri.fX < 0)
      flTheta = -flTheta;
    ...
    for (i = 0; i < Q_min(s_NumFXSlots, s_lNumEnvironments); i++) {
      if (s_FXSlotInfo[i].lEnvID == s_EnvironmentID) {
        ...
        LR.x = s_lpEnvTable[s_EnvironmentID].Aperture[lApertureNum].vCenter[0] -
               ListPos.fX;
        LR.y = s_lpEnvTable[s_EnvironmentID].Aperture[lApertureNum].vCenter[1] -
               ListPos.fY;
        LR.z = s_lpEnvTable[s_EnvironmentID].Aperture[lApertureNum].vCenter[2] -
               ListPos.fZ;
        ...
        // Magnitude is based on the angle subtended by the aperture, so compute
        // the angle between the vector from the Listener to Pos1 of the
        // aperture, and the vector from the Listener to Pos2 of the aperture.

        LP1.x = s_lpEnvTable[s_EnvironmentID].Aperture[lApertureNum].vPos1[0] -
                ListPos.fX;
        LP1.y = s_lpEnvTable[s_EnvironmentID].Aperture[lApertureNum].vPos1[1] -
                ListPos.fY;
        LP1.z = s_lpEnvTable[s_EnvironmentID].Aperture[lApertureNum].vPos1[2] -
                ListPos.fZ;

        LP2.x = s_lpEnvTable[s_EnvironmentID].Aperture[lApertureNum].vPos2[0] -
                ListPos.fX;
        LP2.y = s_lpEnvTable[s_EnvironmentID].Aperture[lApertureNum].vPos2[1] -
                ListPos.fY;
        LP2.z = s_lpEnvTable[s_EnvironmentID].Aperture[lApertureNum].vPos2[2] -
                ListPos.fZ;
        ...
        float flGamma =
            acos((LP1.x * LP2.x) + (LP1.y * LP2.y) + (LP1.z * LP2.z));

        // We want opposite magnitude (because we are 'in' this environment)
        float flMagnitude =
            1.0f - ((2.0f * (float)sin(flGamma / 2.0f)) / flGamma);
        ...
        if (s_eaxSet(&s_FXSlotInfo[i].FXSlotGuid, EAXREVERB_REVERBPAN, NULL,
                     &Pan, sizeof(EAXVECTOR)) != AL_NO_ERROR)
          OutputDebugString("Failed to set Listener Reverb Pan\n");

        if (s_eaxSet(&s_FXSlotInfo[i].FXSlotGuid, EAXREVERB_REFLECTIONSPAN,
                     NULL, &Pan, sizeof(EAXVECTOR)) != AL_NO_ERROR)
        ...
        LR.x = s_lpEnvTable[ReverbData[k].lEnvID]
                   .Aperture[ReverbData[k].lApertureNum]
                   .vCenter[0] -
               ListPos.fX;
        LR.y = s_lpEnvTable[ReverbData[k].lEnvID]
                   .Aperture[ReverbData[k].lApertureNum]
                   .vCenter[1] -
               ListPos.fY;
        LR.z = s_lpEnvTable[ReverbData[k].lEnvID]
                   .Aperture[ReverbData[k].lApertureNum]
                   .vCenter[2] -
               ListPos.fZ;
        ...
        LP1.x = s_lpEnvTable[ReverbData[k].lEnvID]
                    .Aperture[ReverbData[k].lApertureNum]
                    .vPos1[0] -
                ListPos.fX;
        LP1.y = s_lpEnvTable[ReverbData[k].lEnvID]
                    .Aperture[ReverbData[k].lApertureNum]
                    .vPos1[1] -
                ListPos.fY;
        LP1.z = s_lpEnvTable[ReverbData[k].lEnvID]
                    .Aperture[ReverbData[k].lApertureNum]
                    .vPos1[2] -
                ListPos.fZ;

        LP2.x = s_lpEnvTable[ReverbData[k].lEnvID]
                    .Aperture[ReverbData[k].lApertureNum]
                    .vPos2[0] -
                ListPos.fX;
        LP2.y = s_lpEnvTable[ReverbData[k].lEnvID]
                    .Aperture[ReverbData[k].lApertureNum]
                    .vPos2[1] -
                ListPos.fY;
        LP2.z = s_lpEnvTable[ReverbData[k].lEnvID]
                    .Aperture[ReverbData[k].lApertureNum]
                    .vPos2[2] -
                ListPos.fZ;
...
static void UpdateEAXBuffer(channel_t *ch) {
  ...
  // If EAX Manager is not initialized, or there is no EAX support, or the
  // listener is underwater, return
  if ((!s_lpEAXManager) || (!s_bEAX) || (s_bInWater))
    return;

  // Set Occlusion Direct Ratio to the default value (it won't get set by the
  // current version of EAX Manager)
  ...
  if (ch->fixed_origin) {
    ...
    if (ch->entnum == listener_number) {
      // Source at same position as listener
      // Probably won't be any Occlusion / Obstruction effect -- unless the
      // listener is underwater Converting from Open AL -> DS3D (for EAGLE) ...
      // invert Z
      EMSourcePoint.fX = listener_pos[0];
      EMSourcePoint.fY = listener_pos[1];
      EMSourcePoint.fZ = -listener_pos[2];
  ...
  if (s_lpEAXManager->GetListenerDynamicAttributes(0, &EMSourcePoint,
                                                   &lSourceID, 0) == EM_OK) {
    ...
    for (i = 0; i < s_NumFXSlots; i++) {
      if (s_FXSlotInfo[i].lEnvID == lSourceID) {
        ...
        if (ch->lSlotID != i) {
          // Set this
          if (s_eaxSet(&EAXPROPERTYID_EAX40_Source, EAXSOURCE_ACTIVEFXSLOTID,
                       ch->alSource, &s_FXSlotInfo[i].FXSlotGuid,
                       sizeof(GUID)) != AL_NO_ERROR)
            OutputDebugString(
    ...
    OutputDebugString(
        "UpdateEAXBuffer::Failed to get Source environment zone\n");
  }

  // Add some Exclusion to sounds that are not located in the Listener's
  // environment
  if (s_FXSlotInfo[ch->lSlotID].lEnvID == s_EnvironmentID) {
    lExclusion = 0;
    if (s_eaxSet(&EAXPROPERTYID_EAX40_Source, EAXSOURCE_EXCLUSION, ch->alSource,
                 &lExclusion, sizeof(long)) != AL_NO_ERROR)
  ...
  if ((ch->entchannel == CHAN_VOICE) || (ch->entchannel == CHAN_VOICE_ATTEN) ||
    ...
    hr = s_lpEAXManager->GetSourceDynamicAttributes(
        0, &EMSourcePoint, &eaxOBProp.lObstruction,
        &eaxOBProp.flObstructionLFRatio, &eaxOCProp.lOcclusion,
        &eaxOCProp.flOcclusionLFRatio, &eaxOCProp.flOcclusionRoomRatio,
