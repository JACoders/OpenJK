...
int G_ModelIndex(const char *name) {
  ...
  // debug to see if we are shoving data into configstrings for models that
  // don't exist, and if so, where we are doing it from -rww
  fileHandle_t fh;
...
// methods of creating/freeing "fake" dynamically allocated client entity
// structures. You ABSOLUTELY MUST free this client after creating it before game
// shutdown. If it is left around you will have a memory leak, because true
// dynamic memory is allocated by the exe.
void G_FreeFakeClient(
    gclient_t **cl) { // or not, the dynamic stuff is busted somehow at the
                      // moment. Yet it still works in the test. I think
                      // something is messed up in being able to cast the memory
                      // to stuff to modify it, while modifying it directly seems
                      // to work fine. trap->TrueFree((void **)cl);
}
...
void G_AllocateVehicleObject(Vehicle_t **pVeh) {
  ...
  while (i <
         MAX_VEHICLES_AT_A_TIME) { // iterate through and try to find a free one
...
void G_FreeVehicleObject(Vehicle_t *pVeh) {
  ...
  while (i < MAX_VEHICLES_AT_A_TIME) {
    if (g_vehiclePoolOccupied[i] &&
        &g_vehiclePool[i] == pVeh) { // guess this is it
...
// call this on game shutdown to run through and get rid of all the lingering
// client pointers.
void G_CleanAllFakeClients(void) {
  int i = MAX_CLIENTS; // start off here since all ents below have real client
                       // structs.
  gentity_t *ent;
...
void BG_SetAnim(playerState_t *ps, animation_t *animations, int setAnimParts,
                int anim, int setAnimFlags, int blendTime);
...
void G_SetAnim(gentity_t *ent, usercmd_t *ucmd, int setAnimParts, int anim,
  ...
  assert(ent->client);
  BG_SetAnim(&ent->client->ps, bgAllAnims[ent->localAnimIndex].anims,
             setAnimParts, anim, setAnimFlags, blendTime);
#endif
}
...
void G_InitGentity(gentity_t *e) {
  ...
  trap->ICARUS_FreeEnt(
      (sharedEntity_t *)e); // ICARUS information must be added after this point
}
...
gentity_t *G_Spawn(void) {
  ...
  trap->LocateGameData((sharedEntity_t *)level.gentities, level.num_entities,
...
void G_SendG2KillQueue(void) {
  ...
  gG2KillNum -= i;
  if (gG2KillNum < 0) { // hmm, should be impossible, but I'm paranoid as we're
                        // far past beta.
    assert(0);
    gG2KillNum = 0;
  }
...
void G_KillG2Queue(int entNum) {
  if (gG2KillNum >= MAX_G2_KILL_QUEUE) { // This would be considered a Bad
                                         // Thing.
#ifdef _DEBUG
    Com_Printf(
        "WARNING: Exceeded the MAX_G2_KILL_QUEUE count for this frame!\n");
#endif
...
void G_FreeEntity(gentity_t *ed) {
  ...
  trap->ICARUS_FreeEnt((
      sharedEntity_t *)ed); // ICARUS information must be added after this point
  ...
  // now-removed entity
  if (ed->s.modelGhoul2) { // force all clients to accept an event to destroy
  ...
  if (ed->s.eType == ET_NPC &&
      ed->m_pVehicle) { // tell the "vehicle pool" that this one is now free
    G_FreeVehicleObject(ed->m_pVehicle);
  }

  if (ed->s.eType == ET_NPC &&
      ed->client) { // this "client" structure is one of our dynamically
                    // allocated ones, so free the memory
    int saberEntNum = -1;
    int i = 0;
    ...
    while (i < MAX_SABERS) {
      if (ed->client->weaponGhoul2[i] &&
          trap->G2API_HaveWeGhoul2Models(ed->client->weaponGhoul2[i])) {
  ...
  if (ed->s.eFlags & EF_SOUNDTRACKER) {
    ...
    trap->SendServerCommand(
        -1, va("kls %i %i", ed->s.trickedentindex, ed->s.number));
  }
...
void G_ScaleNetHealth(gentity_t *self) {
  ...
  if (self->health > 0 &&
      self->s.health <=
          0) { // don't let it scale to 0 if the thing is still not "dead"
    self->s.health = 1;
  }
}
...
=============
*/
gentity_t *G_PlayEffectID(const int fxID, vec3_t org,
                          vec3_t ang) { // play an effect by the G_EffectIndex'd
                                        // ID instead of a predefined effect ID
  gentity_t *te;
  ...
  if (!te->s.angles[0] && !te->s.angles[1] &&
      !te->s.angles[2]) { // play off this dir by default then.
    te->s.angles[1] = 1;
  }
...
void G_Sound(gentity_t *ent, int channel, int soundIndex) {
  ...
  if (ent && ent->client &&
      channel > TRACK_CHANNEL_NONE) { // let the client remember the index of
                                      // the player entity so he can kill the
                                      // most recent sound on request
    if (g_entities[ent->client->ps.fd.killSoundEntIndex[channel - 50]].inuse &&
      ...
      G_MuteSound(ent->client->ps.fd.killSoundEntIndex[channel - 50],
                  CHAN_VOICE);
...
qboolean ValidUseTarget(gentity_t *ent) {
  ...
  if (!(ent->r.svFlags & SVF_PLAYER_USABLE)) { // Check for flag that denotes
...
void G_UseDispenserOn(gentity_t *ent, int dispType, gentity_t *target) {
  if (dispType == HI_HEALTHDISP) {
    ...
    if (ent->client->medSupplyDebounce < level.time) { // do the next increment
      ...
      target->client->ps
          .ammo[weaponData[target->client->ps.weapon].ammoIndex] +=
          weaponData[target->client->ps.weapon].energyPerShot;

      if (target->client->ps
              .ammo[weaponData[target->client->ps.weapon].ammoIndex] >
          ammoData[weaponData[target->client->ps.weapon].ammoIndex]
              .max) { // cap it off
        target->client->ps
            .ammo[weaponData[target->client->ps.weapon].ammoIndex] =
            ammoData[weaponData[target->client->ps.weapon].ammoIndex].max;
      }

      // base the next supply time on how long the weapon takes to fire. Seems
      // fair enough.
      ent->client->medSupplyDebounce =
          level.time + weaponData[target->client->ps.weapon].fireTime;
    }
...
int G_CanUseDispOn(gentity_t *ent, int dispType) {
  if (!ent->client || !ent->inuse || ent->health < 1 ||
      ent->client->ps.stats[STAT_HEALTH] < 1) { // dead or invalid
    return 0;
  }

  if (dispType == HI_HEALTHDISP) {
    if (ent->client->ps.stats[STAT_HEALTH] <
        ent->client->ps.stats[STAT_MAX_HEALTH]) { // he's hurt
      return 1;
    }
    ...
    if (ent->client->ps.weapon <= WP_NONE ||
        ent->client->ps.weapon >
            LAST_USEABLE_WEAPON) { // not a player-useable weapon
      return 0;
    }

    if (ent->client->ps.ammo[weaponData[ent->client->ps.weapon].ammoIndex] <
        ammoData[weaponData[ent->client->ps.weapon].ammoIndex]
            .max) { // needs more ammo for current weapon
      return 1;
    }
...
qboolean TryHeal(gentity_t *ent, gentity_t *target) {
  if (level.gametype == GT_SIEGE && ent->client->siegeClass != -1 && target &&
      target->inuse && target->maxHealth && target->healingclass &&
      target->healingclass[0] && target->health > 0 &&
      target->health < target->maxHealth) { // it's not dead yet...
    siegeClass_t *scl = &bgSiegeClasses[ent->client->siegeClass];

    if (!Q_stricmp(scl->name,
                   target->healingclass)) { // this thing can be healed by the
                                            // class this player is using
      if (target->healingDebounce < level.time) { // do the actual heal
        ...
        if (target->healingsound && target->healingsound[0]) { // play it
          if (target->s.solid ==
              SOLID_BMODEL) { // ok, well, just play it on the client then.
      ...
      // keep them in the healing anim even when the healing debounce is not yet
      // expired
      if (ent->client->ps.torsoAnim == BOTH_BUTTON_HOLD ||
          ent->client->ps.torsoAnim == BOTH_CONSOLE1) { // extend the time
        ent->client->ps.torsoTimer = 500;
      } else {
        G_SetAnim(ent, NULL, SETANIM_TORSO, BOTH_BUTTON_HOLD,
...
void TryUse(gentity_t *ent) {
  ...
  if (!ent || !ent->client ||
      ...
      ent->health < 1 || (ent->client->ps.pm_flags & PMF_FOLLOW) ||
  ...
  if (ent->client->ps.emplacedIndex) { // on an emplaced gun or using a vehicle,
                                       // don't do anything when hitting use key
    return;
  }
  ...
  if (ent->client->bodyGrabIndex !=
      ENTITYNUM_NONE) { // then hitting the use key just means let go
...
// Enable for corpse dragging
  ...
  if (target && target->m_pVehicle && target->client &&
      ...
      !ent->client->ps.zoomMode) { // if target is a vehicle then perform
                                   // appropriate checks
    Vehicle_t *pVeh = target->m_pVehicle;

    if (pVeh->m_pVehicleInfo) {
      if (ent->r.ownerNum ==
          target->s.number) { // user is already on this vehicle so eject him
        pVeh->m_pVehicleInfo->Eject(pVeh, (bgEntity_t *)ent, qfalse);
      } else { // Otherwise board this vehicle.
        if (level.gametype < GT_TEAM || !target->alliedTeam ||
            (target->alliedTeam ==
             ent->client->sess.sessionTeam)) { // not belonging to a team, or
                                               // client is on same team
  ...
  if (((ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_HEALTHDISP)) ||
       (ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_AMMODISP))) &&
      target && target->inuse && target->client && target->health > 0 &&
      OnSameTeam(ent, target) &&
      (G_CanUseDispOn(target, HI_HEALTHDISP) ||
       G_CanUseDispOn(
           target,
           HI_AMMODISP))) { // a live target that's on my team, we can use him
  ...
  if (ValidUseTarget(target) &&
      (level.gametype != GT_SIEGE || !target->alliedTeam ||
    ...
    ent->client->ps.weaponTime = ent->client->ps.torsoTimer;
    /*
    NPC_SetAnim( ent, SETANIM_TORSO, BOTH_FORCEPUSH,
    SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD ); if ( !VectorLengthSquared(
    ent->client->ps.velocity ) )
    {
            NPC_SetAnim( ent, SETANIM_LEGS, BOTH_FORCEPUSH,
...
tryJetPack:
  // if we got here, we didn't actually use anything else, so try to toggle
  // jetpack if we are in the air, or if it is already on
  ...
  if ( (ent->client->ps.stats[STAT_HOLDABLE_ITEMS] & (1 << HI_AMMODISP)) /*&&
    ...
    trap->Trace(&trToss, ent->client->ps.origin, playerMins, playerMaxs, fwd,
...
qboolean G_BoxInBounds(vec3_t point, vec3_t mins, vec3_t maxs,
...
qboolean G_ClearTrace(vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end,
...
qboolean G_CheckInSolid(gentity_t *self, qboolean fix) {
  ...
  trap->Trace(&trace, self->r.currentOrigin, mins, self->r.maxs, end,
...
void G_ROFF_NotetrackCallback(gentity_t *cent, const char *notetrack) {
  ...
  if (strcmp(type, "loop") == 0) {
    if (addlArg) // including an additional argument means reset to original
...
qboolean G_ExpandPointToBBox(vec3_t point, const vec3_t mins, const vec3_t maxs,
  ...
  for (i = 0; i < 3; i++) {
    ...
    trap->Trace(&tr, start, vec3_origin, vec3_origin, end, ignore, clipmask,
                qfalse, 0, 0);
...
float ShortestLineSegBewteen2LineSegs(vec3_t start1, vec3_t end1, vec3_t start2,
  ...
  if (fabs(denom) > 0.001f) {
    float s = -((v2v2 * DotProduct(v1, start_dif)) -
                (v1v2 * DotProduct(v2, start_dif))) /
              denom;
    float t = ((v1v1 * DotProduct(v2, start_dif)) -
               (v1v2 * DotProduct(v1, start_dif))) /
              denom;
    qboolean done = qtrue;
    ...
    // What we are gonna do is see if we can find a shorter distance than the
    // above involving the endpoints.
  } else {
  ...
  */
  ...
  if (new_dist <
      current_dist) { // then update close_pnt1 close_pnt2 and current_dist
