...
// Rail System
//
// The rail system is intended to provide a means for generating moving entities
// along tracks of varying speed and direction.  The entities are pulled from
// the map based upon their targets and recycled in random positions and order
//
////////////////////////////////////////////////////////////////////////////////////////
...
// The Rail Track
//
// Tracks are the central component to the rails system.  They provide the
// master repositry of all movers and maintain the list of available movers as
// well as
//
////////////////////////////////////////////////////////////////////////////////////////
class CRailTrack {
public:
  void Setup(gentity_t *ent) {
    ...
    mNegative = (ent->s.angles[1] == 180.0f ||
                 ent->s.angles[1] == 270.0f); // From Maxs To Mins
    ...
    //----------------------------
    mSpeedUnitsPerMillisecond =
        mSpeedGridCellsPerSecond * mGridCellSize / 1000.0f;
...
A rail track determines what location and direction rail_mover entities go.
Don't bother with any origin brushes.  Make sure to set:
...
"delay"		 How long the ent will wait from the start of the level before
placing movers
*/
////////////////////////////////////////////////////////////////////////////////////////
  ...
  void Initialize() {
    ...
    for (int i = 0; i < mRailTracks.size(); i++) {
      if (mRailTracks[i].mName == mNameTrack) {
        ...
        mMinCol =
            (int)((mMins[mTrack->mWAxis] - mTrack->mMins[mTrack->mWAxis]) /
                  mTrack->mGridCellSize);
        mMaxCol = (int)((mMaxs[mTrack->mWAxis] - mTrack->mMins[mTrack->mWAxis] -
                         (mTrack->mGridCellSize / 2.0f)) /
                        mTrack->mGridCellSize);
        ...
        //				mTrack->mCells.get_cell_coords(mMins[mTrack->mWAxis],
        //0, mMinCol, dummy);
        //				mTrack->mCells.get_cell_coords((mMaxs[mTrack->mWAxis]-10.0f),
        //0, mMaxCol, dummy);
        break;
      }
    }
...
Use rail lanes to split up tracks.  Just target it to a track that you want to
...
public:
  ...
  void Initialize() {
    ...
    if (mTrack) {
      ...
      mCols = (int)((mEnt->maxs[mTrack->mWAxis] - mEnt->mins[mTrack->mWAxis]) /
                    mTrack->mGridCellSize) +
              1;
      mRows = (int)((mEnt->maxs[mTrack->mHAxis] - mEnt->mins[mTrack->mHAxis]) /
                    mTrack->mGridCellSize) +
              1;
...
Rail Mover will go along the track and lane of your choice.  Just target it to
either a track or a lane.  Don't bother with any origin brushes.
...
void Rail_Update() {
  ...
  {
    for (int track = 0; track < mRailTracks.size(); track++) {
      if (level.time > mRailTracks[track].mNextUpdateTime &&
          !mRailTracks[track].mMovers.empty()) {
    ...
    if (player && gi.WE_IsOutside(player->currentOrigin)) {
      ...
      for (int moverIndex = 0; moverIndex < mRailMovers.size(); moverIndex++) {
        ...
        if (mover.Active() && !mover.mSoundPlayed) {
          VectorAdd(mover.mEnt->currentOrigin, mover.mOriginOffset.v,
                    moverOrigin);
          ...
          if (((mover.mLane || !mover.mCenter) &&           // Not Center Track
               ...
               (DotProduct(playerToMover, mover.mTrack->mDirection.v) >
                -0.45f))                          //  And On The Side
              ||                                  // OR
              ((!mover.mLane && mover.mCenter) && // Is Center Track
               (playerToMoverDistance <
                    WOOSH_SUPPORT_RANGE || //  And Close Enough for Support
                (playerToMoverDistance < WOOSH_TUNNEL_RANGE &&
                 mover.mRows > 10)) //   Or Close Enough For Tunnel
               )) {
            mover.mSoundPlayed = true;
            wooshSound = 0;

            // The Centered Entities Play Right On The Player's Head For Full
            // Volume
            //-----------------------------------------------------------------------
            ...
            //-------------------------------------------------------------
              ...
              //--------------------------------------------------
              playerToMoverDistanceFraction =
                  playerToMoverDistance / WOOSH_ALL_RANGE;
              ...
              VectorMA(player->currentOrigin, playerToMoverDistance,
                       playerToMover, wooshSoundPos);
            ...
            if (wooshSound) {
              ...
              if (WOOSH_DEBUG) {
                CG_DrawEdge(player->currentOrigin, wooshSoundPos,
...
void CRailTrack::Update() {
  ...
  for (attempt = 0; attempt < mNumMoversPerRow; attempt++) {
    ...
    //------------------------------------------
    if (level.time <
        ((mover->mLane) ? (mover->mLane->mStartTime) : (mStartTime))) {
      continue;
    }
    ...
    while (!mTestCols.empty()) {
      ...
      if (TestMoverInCells(mover, atCol)) {
        ...
        // Now Transport The Actual Mover Entity Into Position, Link It & Send
        // It Off
        //----------------------------------------------------------------------------
        CVec3 StartPos(mGridBottomLeftCorner);
        StartPos[mWAxis] +=
            ((atCol * mGridCellSize) + ((mover->mCols / 2.0f) * mGridCellSize));
        StartPos[mHAxis] += (((mover->mRows / 2.0f) * mGridCellSize) *
                             ((mNegative) ? (1) : (-1)));
        StartPos[2] = 0;

        // If Centered, Actually Put It At EXACTLY The Right Position On The
        // Width Axis
        //------------------------------------------------------------------------------
        if (mover->mCenter) {
          StartPos[mWAxis] = mGridCenter[mWAxis];
          float deltaOffset =
              mGridCenter[mWAxis] - mover->mOriginOffset[mWAxis];
        ...
        mover->mEnt->s.pos.trDuration =
            mTravelTimeMilliseconds + (mNextUpdateDelay * mover->mRows);
