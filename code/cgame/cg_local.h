...
#define MAX_CAPTIONTEXT                                                        \
  32 // we don't need 64 now since we don't use this for scroll text, and I
...
typedef struct {
  ...
  // For persistent beam weapons, so they don't play their start sound more than
  // once
...
struct centity_s {
  ...
  //	int				trailTime;		// so missile trails can handle
  //dropped initial packets
  int miscTime;

  playerEntity_t pe;

  //	int				errorTime;		// decay the error from this
  //time 	vec3_t			errorOrigin; 	vec3_t
  //errorAngles;

  //	qboolean		extrapolated;	// false if origin / angles is an
  //interpolation 	vec3_t			rawOrigin; 	vec3_t
  //rawAngles;
  ...
  vec3_t lerpAngles;
  vec3_t
      renderAngles; // for ET_PLAYERS, the actual angles it was rendered at-
                    // should be used by any getboltmatrix calls after CG_Player
...
typedef enum {
  ...
  LEF_NO_RANDOM_ROTATE = 0x0008 // MakeExplosion adds random rotate which could
                                // be bad in some cases
} leFlag_t;
...
typedef struct {
  ...
  float thirdPersonPitchOffset; // what offset pitch to apply the the camera
                                // view
  float thirdPersonCameraDamp; // how tightly to move the camera pos behind the
                               // player
  float thirdPersonAlpha; // how tightly to move the camera pos behind the
                          // player
...
typedef struct {
  ...
  char captionText[MAX_CAPTIONTEXT]
                  [256 /*128*/]; // bosted for taiwanese squealy radio static
                                 // speech in kejim post
  int captionTextY;
  ...
  int wonkyTime; // when interrogator gets you, wonky time controls "drugged"
                 // camera view.
  ...
  */
  int testModel;
  // had to be moved so we wouldn't wipe these out with the memset - these have
  // STL in them and shouldn't be cleared that way
  snapshot_t activeSnapshots[2];
  refEntity_t testModelEntity;
  /*
  Ghoul2 Insert End
  */
  overrides_t overrides; // for overriding certain third-person camera
                         // properties
...
void CG_DrawPic2(float x, float y, float width, float height, float s1,
                 float t1, float s2, float t2, qhandle_t hShader);
void CG_DrawRotatePic(float x, float y, float width, float height, float angle,
                      qhandle_t hShader);
void CG_DrawRotatePic2(float x, float y, float width, float height, float angle,
                       qhandle_t hShader);
...
void CG_DrawNumField(int x, int y, int width, int value, int charWidth,
                     int charHeight, int style, qboolean zeroFill);
void CG_DrawProportionalString(int x, int y, const char *str, int style,
                               vec4_t color);
...
void CG_AddGhoul2Mark(int type, float size, vec3_t hitloc, vec3_t hitdirection,
                      int entnum, vec3_t entposition, float entangle,
                      CGhoul2Info_v &ghoul2, vec3_t modelScale,
                      int lifeTime = 0, int firstModel = 0, vec3_t uaxis = 0);
void CG_Player(centity_t *cent);
void CG_ResetPlayerEntity(centity_t *cent);
void CG_AddRefEntityWithPowerups(refEntity_t *ent, int powerups,
                                 centity_t *cent);
void CG_GetTagWorldPosition(refEntity_t *model, char *tag, vec3_t pos,
                            vec3_t axis[3]);
...
void CG_Trace(trace_t *result, const vec3_t start, const vec3_t mins,
              const vec3_t maxs, const vec3_t end, const int skipNumber,
              const int mask, const EG2_Collision eG2TraceType = G2_NOCOLLIDE,
              const int useLod = 0);
...
/*localEntity_t *CG_MakeExplosion( vec3_t origin, vec3_t dir,
                                                                qhandle_t
hModel, int numframes, qhandle_t shader, int msec, qboolean isSprite, float
scale = 1.0f );// Overloaded

localEntity_t *CG_MakeExplosion( vec3_t origin, vec3_t dir,
                                                                qhandle_t
hModel, int numframes, qhandle_t shader, int msec, qboolean isSprite, float
scale, int flags );// Overloaded
*/
localEntity_t *CG_AddTempLight(vec3_t origin, float scale, vec3_t color,
                               int msec);

void CG_TestLine(vec3_t start, vec3_t end, int time, unsigned int color,
                 int radius);
...
void cgi_Cvar_Register(vmCvar_t *vmCvar, const char *varName,
...
clipHandle_t cgi_CM_TempBoxModel(const vec3_t mins,
                                 const vec3_t maxs); //, const int contents );
...
void cgi_CM_BoxTrace(trace_t *results, const vec3_t start, const vec3_t end,
                     ...
                     int brushmask);
void cgi_CM_TransformedBoxTrace(trace_t *results, const vec3_t start,
...
void cgi_S_StartSound(const vec3_t origin, int entityNum, int entchannel,
                      sfxHandle_t sfx);
...
void cgi_S_AddLoopingSound(int entityNum, const vec3_t origin,
...
void cgi_S_Respatialize(int entityNum, const vec3_t origin, vec3_t axis[3],
                        qboolean inwater);
sfxHandle_t
cgi_S_RegisterSound(const char *sample); // returns buzz if not found
void cgi_S_StartBackgroundTrack(const char *intro, const char *loop,
                                qboolean bForceStart); // empty name stops music
...
qhandle_t
cgi_R_RegisterModel(const char *name); // returns rgb axis if not found
qhandle_t cgi_R_RegisterSkin(const char *name);
qhandle_t
cgi_R_RegisterShader(const char *name); // returns default shader if not found
qhandle_t
cgi_R_RegisterShaderNoMip(const char *name); // returns all white if not found
qhandle_t cgi_R_RegisterFont(const char *name);
int cgi_R_Font_StrLenPixels(const char *text, const int iFontIndex,
                            const float scale = 1.0f);
int cgi_R_Font_StrLenChars(const char *text);
int cgi_R_Font_HeightPixels(const int iFontIndex, const float scale = 1.0f);
void cgi_R_Font_DrawString(int ox, int oy, const char *text, const float *rgba,
                           const int setIndex, int iMaxPixelWidth,
                           const float scale = 1.0f);
qboolean cgi_Language_IsAsian(void);
qboolean cgi_Language_UsesSpaces(void);
unsigned
cgi_AnyLanguage_ReadCharFromString(const char *psText, int *iAdvanceCount,
                                   qboolean *pbIsTrailingPunctuation = NULL);

void cgi_R_SetRefractProp(float alpha, float stretch, qboolean prepost,
                          qboolean negate);
...
void cgi_R_GetLighting(const vec3_t origin, vec3_t ambientLight,
                       vec3_t directedLight, vec3_t ligthDir);
...
void cgi_R_AddPolyToScene(qhandle_t hShader, int numVerts,
                          const polyVert_t *verts);
void cgi_R_AddLightToScene(const vec3_t org, float intensity, float r, float g,
                           float b);
void cgi_R_RenderScene(const refdef_t *fd);
void cgi_R_SetColor(const float *rgba); // NULL = 1,1,1,1
void cgi_R_DrawStretchPic(float x, float y, float w, float h, float s1,
                          float t1, float s2, float t2, qhandle_t hShader);

void cgi_R_ModelBounds(qhandle_t model, vec3_t mins, vec3_t maxs);
void cgi_R_LerpTag(orientation_t *tag, qhandle_t mod, int startFrame,
                   int endFrame, float frac, const char *tagName);
// Does weird, barely controllable rotation behaviour
void cgi_R_DrawRotatePic(float x, float y, float w, float h, float s1, float t1,
                         float s2, float t2, float a, qhandle_t hShader);
// rotates image around exact center point of passed in coords
void cgi_R_DrawRotatePic2(float x, float y, float w, float h, float s1,
...
void cgi_SetUserCmdAngles(float pitchOverride, float yawOverride,
                          float rollOverride);
...
int cgi_S_AddLocalSet(const char *name, vec3_t listener_origin, vec3_t origin,
                      int entID, int time);
...
// Weapon prototypes
void FX_Saber(vec3_t start, vec3_t normal, float height, float radius,
...
void FX_BlasterWeaponHitPlayer(gentity_t *hit, vec3_t origin, vec3_t normal,
                               qboolean humanoid);
...
void FX_ATSTMainProjectileThink(centity_t *cent,
                                const struct weaponInfo_s *weapon);

void FX_TuskenShotProjectileThink(centity_t *cent,
                                  const struct weaponInfo_s *weapon);
void FX_TuskenShotWeaponHitWall(vec3_t origin, vec3_t normal);
void FX_TuskenShotWeaponHitPlayer(gentity_t *hit, vec3_t origin, vec3_t normal,
                                  qboolean humanoid);

void FX_NoghriShotProjectileThink(centity_t *cent,
                                  const struct weaponInfo_s *weapon);
void FX_NoghriShotWeaponHitWall(vec3_t origin, vec3_t normal);
void FX_NoghriShotWeaponHitPlayer(gentity_t *hit, vec3_t origin, vec3_t normal,
                                  qboolean humanoid);
...
void CG_DrawTargetBeam(vec3_t start, vec3_t end, vec3_t norm,
                       const char *beamFx, const char *impactFx);
...
void trap_G2_SetGhoul2ModelIndexes(CGhoul2Info_v &ghoul2, qhandle_t *modelList,
                                   qhandle_t *skinList);
...
int cgi_UI_GetMenuItemInfo(const char *menuFile, const char *itemName, int *x,
                           int *y, int *w, int *h, vec4_t color,
...
void CG_PlayEffectBolted(const char *fxName, const int modelIndex,
                         const int boltIndex, const int entNum, vec3_t origin,
                         int iLoopTime = 0, const bool isRelative = false);
void CG_PlayEffectIDBolted(const int fxID, const int modelIndex,
                           const int boltIndex, const int entNum, vec3_t origin,
                           int iLoopTime = 0, const bool isRelative = false);
void CG_PlayEffectOnEnt(const char *fxName, const int clientNum, vec3_t origin,
                        const vec3_t fwd);
void CG_PlayEffectIDOnEnt(const int fxID, const int clientNum, vec3_t origin,
                          const vec3_t fwd);
