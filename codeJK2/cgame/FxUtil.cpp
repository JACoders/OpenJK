...
CParticle *FX_AddParticle(const vec3_t org, const vec3_t vel,
                          ...
                          float alphaParm, const vec3_t sRGB, const vec3_t eRGB,
...
CLine *FX_AddLine(int clientID, vec3_t start, float size1, float size2,
                  float sizeParm, float alpha1, float alpha2, float alphaParm,
                  vec3_t sRGB, vec3_t eRGB, float rgbParm, int killTime,
                  qhandle_t shader, int impactFX_id, int flags = 0) {
  if (theFxHelper.mFrameTime <
      1) { // disallow adding new effects when the system is paused
    return 0;
  }
...
CCylinder *FX_AddCylinder(vec3_t start, vec3_t normal, float size1s,
                          float size1e, float sizeParm, float size2s,
...
void FX_AddSprite(vec3_t origin, vec3_t vel, vec3_t accel, float scale,
  ...
  FX_AddParticle(origin, vel, accel, scale, scale, 0, sAlpha, eAlpha,
...
void FX_AddLine(vec3_t start, vec3_t end, float stScale, float width,
                ...
                qhandle_t shader, int flags) {
  FX_AddLine(start, end, width, width, 0, sAlpha, eAlpha, FX_ALPHA_LINEAR,
             WHITE, WHITE, 0, life, shader, 0);
}
...
void FX_AddLine(vec3_t start, vec3_t end, float stScale, float width,
  ...
  FX_AddLine(start, end, width, width, 0, sAlpha, eAlpha, FX_ALPHA_LINEAR, sRGB,
             eRGB, 0, life, shader, 0);
}
...
void FX_AddQuad(vec3_t origin, vec3_t normal, vec3_t vel, vec3_t accel,
                ...
                qhandle_t shader, int flags) {
  FX_AddOrientedParticle(origin, normal, vel, accel, sradius, eradius, 0.0f,
