...
void WP_InitForcePowers(gentity_t *ent) {
	...
	if (g_forceBasedTeams.integer) {
		if (ent->client->sess.sessionTeam == TEAM_RED)
			warnClient = !(BG_LegalizedForcePowers(forcePowers, sizeof(forcePowers), g_maxForceRank.integer, HasSetSaberOnly(), FORCE_DARKSIDE, level.gametype,
												   g_forcePowerDisable.integer));
		else if (ent->client->sess.sessionTeam == TEAM_BLUE)
			warnClient = !(BG_LegalizedForcePowers(forcePowers, sizeof(forcePowers), g_maxForceRank.integer, HasSetSaberOnly(), FORCE_LIGHTSIDE, level.gametype,
												   g_forcePowerDisable.integer));
		else
			warnClient = !(BG_LegalizedForcePowers(forcePowers, sizeof(forcePowers), g_maxForceRank.integer, HasSetSaberOnly(), 0, level.gametype,
												   g_forcePowerDisable.integer));
	} else
		warnClient = !(BG_LegalizedForcePowers(forcePowers, sizeof(forcePowers), g_maxForceRank.integer, HasSetSaberOnly(), 0, level.gametype,
											   g_forcePowerDisable.integer));
	...
	if (ent->s.eType == ET_NPC)
		...
		if (warnClientLimit)
			trap->SendServerCommand(ent - g_entities, va("print \"The server has one or more force powers that you have chosen disabled.\nYou will not be able "
														 "to use the disable force power(s) while playing on this server.\n\""));
	}
...
void WP_SpawnInitForcePowers(gentity_t *ent) {
	...
	while (i < NUM_FORCE_POWERS) {
		if ((ent->client->ps.fd.forcePowersKnown & (1 << i)) &&
			!ent->client->ps.fd.forcePowerLevel[i]) { // make sure all known powers are cleared if we have level 0 in them
			ent->client->ps.fd.forcePowersKnown &= ~(1 << i);
		}
	...
	if (level.gametype == GT_SIEGE && ent->client->siegeClass != -1) { // Then use the powers for this class.
		i = 0;
...
void ForceTeamHeal(gentity_t *self) {
	...
	while (i < numpl) {
		if (g_entities[pl[i]].client->ps.stats[STAT_HEALTH] > 0 && g_entities[pl[i]].health > 0) {
...
void ForceGrip(gentity_t *self) {
	...
	if (tr.fraction != 1.0 && tr.entityNum != ENTITYNUM_NONE && g_entities[tr.entityNum].client && !g_entities[tr.entityNum].client->ps.fd.forceGripCripple &&
...
int WP_GetVelocityForForceJump(gentity_t *self, vec3_t jumpVel, usercmd_t *ucmd) {
	...
	if (self->client->ps.velocity[2] < -30) { // so that we can get a good boost when force jumping in a fall
	...
	if (targetLive && traceEnt->NPC) { // hit an organic non-player
		...
		if (G_ActivateBehavior(traceEnt, BSET_MINDTRICK)) { // activated a script on him
			...
			WP_ForcePowerStart(self, FP_TELEPATHY, 0);
				   ...
				   (!self->NPC && traceEnt->client->playerTeam != (npcteam_t)self->client->sess.sessionTeam)) { // an enemy
			int override = 0;
...
void ForceTelepathy(gentity_t *self) {
	...
	if (self->client->ps.fd.forcePowerLevel[FP_TELEPATHY] == FORCE_LEVEL_1) {
		if (tr.fraction != 1.0 && tr.entityNum != ENTITYNUM_NONE && g_entities[tr.entityNum].inuse && g_entities[tr.entityNum].client &&
...
qboolean CanCounterThrow(gentity_t *self, gentity_t *thrower, qboolean pull) {
	...
	if (self->client->ps.weaponstate == WEAPON_CHARGING || self->client->ps.weaponstate == WEAPON_CHARGING_ALT) { // don't autodefend when charging a weapon
		return 0;
	}

	if (level.gametype == GT_SIEGE && pull && thrower &&
		thrower->client) { // in siege, pull will affect people if they are not facing you, so they can't run away so much
		vec3_t d;
		float a;
		...
		for (x = 0; x < ent_count; x++) {
			...
			if (push_list[x]->client) { // FIXME: make enemy jedi able to hunker down and resist this?
				...
				if (push_list[x]->client) {
					...
					if ((int)push_list[x]->client->ps.velocity[2] == 0) { // if not going anywhere vertically, boost them up a bit
						...
						push_list[x]->s.weapon != WP_THERMAL)) // rolling and stationary thermal detonators are dealt with below
			{
				...
				if (tr.entityNum != push_list[x]->s.number || tr.fraction == 1.0 || tr.allsolid || tr.startsolid) { // must be pointing right at it
					continue;
				}
...
static void WP_UpdateMindtrickEnts(gentity_t *self) {
	...
	if (!self->client->ps.fd.forceMindtrickTargetIndex && !self->client->ps.fd.forceMindtrickTargetIndex2 && !self->client->ps.fd.forceMindtrickTargetIndex3 &&
	...
	while (i < NUM_FORCE_POWERS) {
		if (self->client->ps.holocronsCarried[i]) { // carrying it, make sure we have the power
			...
			if (i == FP_LEVITATION) {
				...
				if (self->client->saber[0].model[0] && self->client->saber[1].model[0]) { // dual
					self->client->ps.fd.saberAnimLevelBase = self->client->ps.fd.saberAnimLevel = self->client->ps.fd.saberDrawAnimLevel = SS_DUAL;
				} else if ((self->client->saber[0].saberFlags & SFL_TWO_HANDED)) { // staff
...
void WP_ForcePowersUpdate(gentity_t *self, usercmd_t *ucmd) {
	...
	*/
	...
	if (self->client->ps.fd.forcePowerSelected < 0 || self->client->ps.fd.forcePowerSelected >= NUM_FORCE_POWERS) { // bad
	...
	if (self && self->client &&
		...
		while (i < NUM_FORCE_POWERS) {
			if ((self->client->ps.fd.forcePowersActive & (1 << i)) && i != FP_LEVITATION && !BG_CanUseFPNow(level.gametype, &self->client->ps, level.time, i)) {
				WP_ForcePowerStop(self, i);
			}
	...
	if (!(self->client->ps.fd.forcePowersActive & (1 << FP_TELEPATHY))) { // clear the mindtrick index values
		self->client->ps.fd.forceMindtrickTargetIndex = 0;
		self->client->ps.fd.forceMindtrickTargetIndex2 = 0;
		self->client->ps.fd.forceMindtrickTargetIndex3 = 0;
		self->client->ps.fd.forceMindtrickTargetIndex4 = 0;
	...
	if ((!self->client->ps.fd.forcePowersActive || self->client->ps.fd.forcePowersActive == (1 << FP_DRAIN)) && !self->client->ps.saberInFlight &&
		(self->client->ps.weapon != WP_SABER ||
		 !BG_SaberInSpecial(self->client->ps.saberMove))) { // when not using the force, regenerate at 1 point per half second
			...
			if (level.gametype == GT_SIEGE) {
				if (self->client->holdingObjectiveItem && g_entities[self->client->holdingObjectiveItem].inuse &&
					...
					self->client->ps.fd.forcePowerRegenDebounceTime +=
						Q_max(g_forceRegenTime.integer * 0.2,
							  1); // if this is siege and our player class has the fast force regen ability, then recharge with 1/5th the usual delay
				else
				...
				if (level.gametype == GT_POWERDUEL && self->client->sess.duelTeam == DUELTEAM_LONE) {
					if (duel_fraglimit.integer)
						self->client->ps.fd.forcePowerRegenDebounceTime +=
							Q_max(g_forceRegenTime.integer * (0.6 + (.3 * (float)self->client->sess.wins / (float)duel_fraglimit.integer)), 1);
					else
