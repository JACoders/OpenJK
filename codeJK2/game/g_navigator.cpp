...
extern int NAVNEW_ClearPathBetweenPoints(vec3_t start, vec3_t end, vec3_t mins,
...
int CNode::Save(int numNodes, fileHandle_t file) {
  ...
  edge_v::iterator ei;
  STL_ITERATE(ei, m_edges) { gi.FS_Write(&(*ei), sizeof(edge_t), file); }
...
void CNavigator::Init(void) { Free(); }
  ...
  STL_ITERATE(ni, m_nodes) {
    ...
    for (edgeNum = 0; edgeNum < (*ni)->GetNumEdges(); edgeNum++) {
      ...
      if ((flags & EFLAG_BLOCKED)) {
        ...
        gi.trace(&trace, p1, wpMins, wpMaxs, p2, ENTITYNUM_NONE,
        ...
        if (trace.entityNum < ENTITYNUM_WORLD &&
          ...
          if (G_EntIsDoor(trace.entityNum)) {            // door
            if (!G_EntIsUnlockedDoor(trace.entityNum)) { // locked door
              failed = qtrue;
            }
          } else {
            if (G_EntIsBreakable(trace.entityNum)) { // do same for breakable
...
void CNavigator::HardConnect(int first, int second) {
  ...
  gi.trace(&trace, p1, wpMins, wpMaxs, p2, ENTITYNUM_NONE,
           MASK_SOLID | CONTENTS_BOTCLIP | CONTENTS_MONSTERCLIP, G2_NOCOLLIDE,
           0);
...
int CNavigator::TestNodePath(gentity_t *ent, int okToHitEntNum, vec3_t position,
                             qboolean includeEnts) {
  ...
  // Check the path
  if (NAV_ClearPathToPoint(ent, ent->mins, ent->maxs, position, clipmask,
                           okToHitEntNum) == false)
    return false;
...
int CNavigator::TestBestFirst(gentity_t *ent, int lastID, int flags) {
  ...
  float bestDist = (bestNode == NODE_NONE)
                       ? Q3_INFINITE
                       : DistanceSquared(ent->currentOrigin, nodePos);
...
int CNavigator::GetBestPathBetweenEnts(gentity_t *ent, gentity_t *goal,
                                       int flags) {
  // Must have nodes
  if (m_nodes.size() == 0)
    return NODE_NONE;
...
#define MAX_Z_DELTA 18
  ...
  CollectNearestNodes(ent->currentOrigin, NODE_COLLECT_RADIUS, NODE_COLLECT_MAX,
                      nodeChain);
  CollectNearestNodes(goal->currentOrigin, NODE_COLLECT_RADIUS,
  ...
  STL_ITERATE(nci, nodeChain) {
    ...
    if (CheckedNode(nodeNum, ent->s.number) ==
      ...
      if ((signed)(*nci).distance >= (radius * radius) ||
          (fabs(position[2] - ent->currentOrigin[2]) >= MAX_Z_DELTA)) {
    ...
    STL_ITERATE(nci2, nodeChain2) {
      ...
      // from ents' positions
      cost = floor(Distance(ent->currentOrigin, position) +
                   Distance(goal->currentOrigin, position2));
      ...
      if (CheckedNode(nodeNum2, goal->s.number) ==
        ...
        // If we're not within the known clear radius of this node OR out of Z
        // height range...
        if ((signed)(*nci2).distance >= (radius * radius) ||
            (fabs(position2[2] - goal->currentOrigin[2]) >= MAX_Z_DELTA)) {
          // We're not *within* this node, so check clear path, etc.

          if (flags & NF_CLEAR_PATH) //|| flags & NF_CLEAR_LOS )
          {                          // need a clear path or LOS
            if (!gi.inPVS(goal->currentOrigin,
...
void CNavigator::CheckFailedNodes(gentity_t *ent) {
  ...
  if (ent->failedWaypointCheckTime &&
      ent->failedWaypointCheckTime < level.time) {
    int failed = 0;
    ...
    for (j = 0; j < MAX_FAILED_NODES; j++) {
      if (ent->failedWaypoints[j] != 0) {
        ...
        else if (NAV_ClearPathToPoint(
                     ent, ent->mins, ent->maxs, nodePos, CONTENTS_BODY,
    ...
    if (!failed) {
      ...
      ent->failedWaypointCheckTime =
          level.time + CHECK_FAILED_EDGE_INTERVAL + Q_irand(0, 1000);
    }
  }
}
...
void CNavigator::AddFailedEdge(int entID, int startID, int endID) {
  ...
  for (j = 0; j < MAX_FAILED_EDGES; j++) {
    if (failedEdges[j].startID == WAYPOINT_NONE) {
      ...
      10 seconds failedEdges[j].checkTime = level.time +
...
qboolean CNavigator::CheckFailedEdge(failedEdge_t *failedEdge) {
  ...
  if (failedEdge->checkTime < level.time) {
    if (failedEdge->startID != WAYPOINT_NONE) {
      ...
      int ignore, clipmask;
      gentity_t *ent = (failedEdge->entID < ENTITYNUM_WORLD)
                           ? &g_entities[failedEdge->entID]
                           : NULL;
      int hitEntNum;

      if (!ent || !ent->inuse || !ent->client || ent->health <= 0) {
        VectorSet(mins, DEFAULT_MINS_0, DEFAULT_MINS_1,
      ...
      gi.trace(&trace, start, mins, maxs, end, ignore,
               clipmask | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP, G2_NOCOLLIDE,
               0); // NOTE: should we really always include monsterclip
