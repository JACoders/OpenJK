...
void NPC_Sniper_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
                     vec3_t point, int damage, int mod) {
...
static qboolean Sniper_Move(void) {
  ...
  if (moved == qfalse) { // couldn't get to enemy
    if ((NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) && NPCInfo->goalEntity &&
      ...
      int cp = NPC_FindCombatPoint(NPC->currentOrigin, NPC->currentOrigin,
                                   NPC->currentOrigin, cpFlags, 32);
      if (cp == -1 && !(NPCInfo->scriptFlags &
                        SCF_USE_CP_NEAREST)) { // okay, try one by the enemy
        cp = NPC_FindCombatPoint(
            NPC->currentOrigin, NPC->currentOrigin, NPC->enemy->currentOrigin,
  ...
  if (NPCInfo->confusionTime < level.time) {
    ...
    if (!(NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
      ...
      if (NPC_CheckForDanger(alertEvent)) {
        ...
        if (alertEvent >=
            0) //&& level.alertEvents[alertEvent].ID != NPCInfo->lastAlertID )
        {
          ...
          if (level.alertEvents[alertEvent].level == AEL_DISCOVERED) {
            if (level.alertEvents[alertEvent].owner &&
              ...
              TIMER_Set(NPC, "attackDelay",
                        Q_irand((6 - NPCInfo->stats.aim) * 100,
      ...
      if (NPCInfo->investigateDebounceTime >
        ...
        VectorSubtract(NPCInfo->investigateGoal,
                       NPC->client->renderInfo.eyePoint, dir);
...
static void Sniper_CheckMoveState(void) {
  ...
  if (!TIMER_Done(NPC, "taunting")) { // no doMove while taunting
    doMove = qfalse;
    return;
  }
  ...
  if ((NPCInfo->goalEntity != NPC->enemy) && (NPCInfo->goalEntity != NULL)) {
    ...
    if (STEER::Reached(NPC, NPCInfo->goalEntity, 16, !!FlyingCreature(NPC)) ||
      ...
      TIMER_Set(NPC, "attackDelay",
                Q_irand((6 - NPCInfo->stats.aim) * 50,
                        (6 - NPCInfo->stats.aim) *
...
static void Sniper_CheckFireState(void) {
  ...
  if (!TIMER_Done(NPC, "taunting")) { // no shoot while taunting
    return;
  }

  // continue to fire on their last position
  if (!Q_irand(0, 1) && NPCInfo->enemyLastSeenTime &&
      level.time - NPCInfo->enemyLastSeenTime <
          ((5 - NPCInfo->stats.aim) * 1000)) // FIXME: incorporate skill too?
  {
...
void Sniper_FaceEnemy(void) {
  ...
  if (NPC->enemy) {
    ...
    if (enemyDist > 65536 &&
        NPCInfo->stats.aim < 5) // is 256 squared, was 16384 (128*128)
    {
      if (NPC->count < (5 - NPCInfo->stats.aim)) { // miss a few times first
        if (shoot && TIMER_Done(NPC, "attackDelay") &&
            level.time >= NPCInfo->shotTime) { // ready to fire again
          ...
          while (hit && tryMissCount < 10) {
            ...
            if (!Q_irand(0, 1)) {
              aimError = qtrue;
              if (!Q_irand(0, 1)) {
                VectorMA(target, NPC->enemy->maxs[2] * Q_flrand(1.5, 4), right,
                         target);
              } else {
                VectorMA(target, NPC->enemy->mins[2] * Q_flrand(1.5, 4), right,
                         target);
              }
            }
            if (!aimError || !Q_irand(0, 1)) {
              if (!Q_irand(0, 1)) {
                VectorMA(target, NPC->enemy->maxs[2] * Q_flrand(1.5, 4), up,
                         target);
              } else {
                VectorMA(target, NPC->enemy->mins[2] * Q_flrand(1.5, 4), up,
                         target);
              }
            }
...
void Sniper_UpdateEnemyPos(void) {
  int index;
  for (int i = MAX_ENEMY_POS_LAG - ENEMY_POS_LAG_INTERVAL; i >= 0;
       i -= ENEMY_POS_LAG_INTERVAL) {
    index = i / ENEMY_POS_LAG_INTERVAL;
    if (!index) {
      CalcEntitySpot(NPC->enemy, SPOT_HEAD_LEAN,
                     NPCInfo->enemyLaggedPos[index]);
      NPCInfo->enemyLaggedPos[index][2] -= Q_flrand(2, 16);
    } else {
      VectorCopy(NPCInfo->enemyLaggedPos[index - 1],
                 NPCInfo->enemyLaggedPos[index]);
    }
  }
}
...
void NPC_BSSniper_Attack(void) {
  ...
  {                      // too close, so switch to primary fire
    if (NPC->client->ps.weapon == WP_DISRUPTOR ||
        NPC->client->ps.weapon ==
            WP_TUSKEN_RIFLE) {                   // sniping... should be assumed
      if (NPCInfo->scriptFlags & SCF_ALT_FIRE) { // use primary fire
        trace_t trace;
        gi.trace(&trace, NPC->enemy->currentOrigin, NPC->enemy->mins,
                 NPC->enemy->maxs, NPC->currentOrigin, NPC->enemy->s.number,
                 NPC->enemy->clipmask, (EG2_Collision)0, 0);
        if (!trace.allsolid && !trace.startsolid &&
            (trace.fraction == 1.0 ||
             trace.entityNum == NPC->s.number)) { // he can get right to me
  ...
  if (NPC_ClearLOS(NPC->enemy)) //|| (NPCInfo->stats.aim >= 5 && gi.inPVS(
                                ...
                                //NPC->enemy->currentOrigin )) )
  {
    ...
    if (enemyDist < maxShootDist) {
      ...
      gi.trace(&tr, muzzle, NULL, NULL, end, NPC->s.number, MASK_SHOT,
               G2_RETURNONHIT, 0);
  ...
  */
  ...
  if (TIMER_Done(NPC, "duck") && TIMER_Done(NPC, "watch") &&
  ...
  if (!faceEnemy) { // we want to face in the dir we're running
    if (doMove) {   // don't run away and shoot
