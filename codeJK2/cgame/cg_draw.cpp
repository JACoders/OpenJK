...
================
*/
static void CG_Draw3DModel(float x, float y, float w, float h, qhandle_t model,
...
================
*/
void CG_DrawHead(float x, float y, float w, float h, int speaker_i,
                 vec3_t headAngles) {
  qhandle_t hm = 0;
  qhandle_t hs = 0;
  float len;
  ...
  if (!talking) { // no sound playing, don't display the head any more
...
static void CG_DrawForcePower(centity_t *cent, int x, int y) {
  ...
  for (i = MAX_TICS - 1; i >= 0; i--) {
    ...
    CG_DrawPic(x + forceTicPos[i].x, y + forceTicPos[i].y, forceTicPos[i].width,
               forceTicPos[i].height, forceTicPos[i].tic);
...
static void CG_DrawAmmo(centity_t *cent, int x, int y) {
  ...
  if (cent->currentState.weapon == WP_SABER && cent->gent) {
    ...
    if (!cg.saberAnimLevelPending &&
        cent->gent->client) { // uninitialized after a loadgame, cheat across
                              // and get it
  ...
  for (i = MAX_TICS - 1; i >= 0; i--) {
    ...
    cgi_R_SetColor(calcColor);
    CG_DrawPic(x + ammoTicPos[i].x, y + ammoTicPos[i].y, ammoTicPos[i].width,
               ammoTicPos[i].height, ammoTicPos[i].tic);
...
static void CG_DrawArmor(int x, int y) {
  ...
  if (armorPercent > 0) {
    ...
    armorPercent =
        (float)ps->stats[STAT_ARMOR] / (ps->stats[STAT_MAX_HEALTH] / 2);
  }
  memcpy(calcColor, colorTable[CT_HUD_GREEN], sizeof(vec4_t));
  calcColor[0] *= armorPercent;
  calcColor[1] *= armorPercent;
  ...
  CG_DrawPic(x, y, 80, 80, cgs.media.HUDArmor2); //	Inner Armor circular
                                                 /*
                                                         if (ps->stats[STAT_ARMOR])	// Is there armor? Draw the HUD Armor
                                                    TIC
                                                         {
                                                                 // Make tic flash if inner armor is at 50% (25% of full armor)
                                                                 if (armorPercent<.5)		// Do whatever the flash timer
                                                    says
                                                                 {
  ...
  if ((cent->currentState.eFlags & EF_LOCKED_TO_WEAPON)) {
    ...
    if (cent->gent && cent->gent->owner) {
      if ((cent->gent->owner->flags & FL_GODMODE)) {
        ...
        health =
            cent->gent->owner->health / (float)cent->gent->owner->max_health;
      }
    }
    ...
    if (g_entities[cg.snap->ps.viewEntity]
            .activator) // ensure we can look back to the atst_drivable to get
                        // the max health
    {
      health =
      ...
      health =
          (g_entities[cg.snap->ps.viewEntity].health +
           g_entities[cg.snap->ps.viewEntity].client->ps.stats[STAT_ARMOR]) /
          (float)(g_entities[cg.snap->ps.viewEntity].max_health +
                  800); // hacked max armor since we don't have an
                        // activator...should never happen
    }
...
static void CG_DrawHUD(centity_t *cent) {
  ...
  if (cgi_UI_GetMenuInfo("lefthud", &x, &y)) {
    ...
    if (cg_draw2D.integer == 2) {
      CG_DrawSmallStringColor(x + 5, y - 60,
                              va("Armor:%d", cg.snap->ps.stats[STAT_ARMOR]),
                              colorTable[CT_HUD_GREEN]);
      CG_DrawSmallStringColor(x + 5, y - 40,
  ...
  if (cgi_UI_GetMenuInfo("righthud", &x, &y)) {
    ...
    if (cg_draw2D.integer == 2) {
      if (cent->currentState.weapon != WP_SABER &&
        ...
        // Bob, just didn't want the ammo text drawing when the saber or the
        // stun baton is the current weapon...change it back if this is wrong.
        value =
            cg.snap->ps.ammo[weaponData[cent->currentState.weapon].ammoIndex];
        //				value =
        //cent->gent->client->ps.forcePower;
        CG_DrawSmallStringColor(x, y - 60, va("Ammo:%d", value),
                                colorTable[CT_HUD_GREEN]);
      } else {
        //				value =
        //cg.snap->ps.ammo[weaponData[cent->currentState.weapon].ammoIndex];
      }
      //			CG_DrawSmallStringColor(x, y -
      //60,va("Ammo:%d",value), colorTable[CT_HUD_GREEN] );
      CG_DrawSmallStringColor(x, y - 40,
  ...
  if (missionInfo_Updated) {
    if (cg.predicted_player_state.pm_type != PM_DEAD) {
      ...
      */
      ...
      if (cg_updatedDataPadForcePower1.integer) {
        y_pos += 25;
        cgi_SP_GetStringTextString("INGAME_NEW_FORCE_POWER_INFO", text,
                                   sizeof(text));
        if (cg_missionInfoCentered.integer) {
          w = cgi_R_Font_StrLenPixels(text, cgs.media.qhFontSmall, 1.0f);
          x_pos = (SCREEN_WIDTH / 2) - (w / 2);
      ...
      if (cg_updatedDataPadObjective.integer) {
        y_pos += 25;
        cgi_SP_GetStringTextString("INGAME_NEW_OBJECTIVE_INFO", text,
                                   sizeof(text));
        if (cg_missionInfoCentered.integer) {
          w = cgi_R_Font_StrLenPixels(text, cgs.media.qhFontSmall, 1.0f);
          x_pos = (SCREEN_WIDTH / 2) - (w / 2);
      ...
      //		{
      //				CG_DrawProportionalString((SCREEN_WIDTH/2),
      //(SCREEN_HEIGHT/2) + 20, ingame_text[IGT_MISSIONINFO_UPDATED2], 					CG_PULSE
      //| CG_CENTER| CG_SMALLFONT, colorTable[CT_LTRED1] );
      //		}
    }
  }
}
