...
extern void CG_AddSaberBlade(centity_t *cent, centity_t *scent,
                             refEntity_t *saber, int renderfx, int modelIndex,
                             vec3_t origin, vec3_t angles);
extern void CG_CheckSaberInWater(centity_t *cent, centity_t *scent,
                                 int saberNum, int modelIndex, vec3_t origin,
                                 vec3_t angles);
...
vec3_t *CG_SetEntitySoundPosition(centity_t *cent) {
  ...
  if (cent->currentState.solid == SOLID_BMODEL) {
    ...
    if (cent->currentState.eType == ET_PLAYER && cent->gent &&
        cent->gent->client && cent->gent->ghoul2.IsValid() &&
        cent->gent->ghoul2[0].animModelIndexOffset) // If it has an animOffset
                                                    // it's a cinematic anim
    { // I might be running out of my bounding box, so use my headPoint from the
      // last render frame...?
      // NOTE: if I'm not rendered, will this not update correctly?  Would
      // cent->lerpOrigin be any more updated?
...
static void CG_EntityEffects(centity_t *cent) {
  ...
  if (cent->currentState.loopSound) {
    ...
    sfxHandle_t sfx = (cent->currentState.eType == ET_MOVER)
    ...
    if (!(ent->s.eFlags & EF_NODRAW)) {
      cgi_S_AddLoopingSound(cent->currentState.number,
                            v3Origin /*cent->lerpOrigin*/, vec3_origin, sfx,
                            chan);
    }
  }
...
void CG_AddRefEntWithTransportEffect(centity_t *cent, refEntity_t *ent) {
  ...
  if (ent->renderfx & RF_PULSATE && cent->gent->owner &&
      cent->gent->owner->health && !cent->gent->owner->s.number &&
    ...
    // if we are the saber and we have been dropped, do a glow so it can be
    // spotted easier
    float wv;
    vec3_t org;
    ...
    for (int i = -4; i < 10; i += 1) {
      ...
      FX_AddSprite(org, NULL, NULL, 5.5f, 5.5f, wv, wv, 0.0f, 0.0f, 1.0f,
                   cgs.media.yellowDroppedSaberShader, 0x08000000);
    }
    if (cent->gent->owner->s.weapon == WP_SABER) { // he's still controlling me
      FX_AddSprite(cent->gent->owner->client->renderInfo.handRPoint, NULL, NULL,
                   8.0f, 8.0f, wv, wv, 0.0f, 0.0f, 1.0f,
                   cgs.media.yellowDroppedSaberShader, 0x08000000);
    }
  }
}
...
void G2_BoltToGhoul2Model(centity_t *cent, refEntity_t *ent) {
  ...
  gi.G2API_GetBoltMatrix(cent->gent->ghoul2, modelNum, boltNum, &boltMatrix,
                         cg_entities[entNum].currentState.angles,
                         cg_entities[entNum].currentState.origin, cg.time,
...
static void CG_General(centity_t *cent) {
  ...
  */
  ...
  if (cent->currentState.eFlags & EF_DISABLE_SHADER_ANIM) {
    // by setting the shader time to the current time, we can force an animating
    // shader to not animate
    ent.shaderTime = cg.time * 0.001f;
  }

  if (s1->eFlags & EF_SHADER_ANIM) {
    // Deliberately setting it up so that shader anim will completely override
    // any kind of model animation frame setting.
  ...
  */
  ...
  if (s1->boltInfo) {
    ...
    if (cent->gent->s.weapon == WP_EMPLACED_GUN ||
        (cent->gent->activator && cent->gent->activator->owner &&
      ...
      if (cent->gent->health <= 0 && cent->gent->e_ThinkFunc == thinkF_NULL) {
        ...
        CG_ImpactMark(cgs.media.scavMarkShader, cent->lerpOrigin, temp, 0, 1, 1,
                      1, 1.0f, qfalse, 92, qtrue);
        CG_ImpactMark(cgs.media.scavMarkShader, cent->lerpOrigin, temp, 90, 1,
                      1, 1, 1.0f, qfalse, 48, qtrue);
      } else {
        VectorSet(temp, 0, 0, 1);
        ...
        CG_ImpactMark(cgs.media.shadowMarkShader, cent->lerpOrigin, temp, 0, 1,
                      1, 1, 1.0f, qfalse, 32, qtrue);
      }
    }

    if (cent->gent->activator && cent->gent->activator->owner &&
        cent->gent->activator->s.eFlags & EF_LOCKED_TO_WEAPON &&
        cent->gent->activator->owner->s.number ==
            cent->currentState
                .number) // gun number must be same as current entities number
    {
      ...
      if (!(cc->currentState.eFlags & EF_FIRING) &&
          !(cc->currentState.eFlags & EF_ALT_FIRING)) {
        // not animating..pausing was leaving the barrels in a bad state
        gi.G2API_PauseBoneAnim(&cent->gent->ghoul2[cent->gent->playerModel],
                               "model_root", cg.time);

        //				gi.G2API_SetBoneAnimIndex(
        //&cent->gent->ghoul2[cent->gent->playerModel], cent->gent->rootBone, 						0,
        //0, BONE_ANIM_OVERRIDE, 1.0f, cg.time );
      }
      ...
      gi.G2API_GetBoltMatrix(cent->gent->ghoul2, 0, bolt, &boltMatrix,
                             cent->lerpAngles, cent->lerpOrigin, cg.time,
                             cgs.model_draw, cent->currentState.modelScale);

      // store the muzzle point and direction so that we can fire in the right
      // direction
      gi.G2API_GiveMeVectorFromMatrix(boltMatrix, ORIGIN,
                                      cc->gent->client->renderInfo.muzzlePoint);
      if (cent->gent->bounceCount) { // EWeb - *sigh* the muzzle tag on this is
                                     // not aligned like th eone on the emplaced
                                     // gun... consistency anyone...?
        gi.G2API_GiveMeVectorFromMatrix(boltMatrix, NEGATIVE_X,
                                        cc->gent->client->renderInfo.muzzleDir);
      } else { // Emplaced gun
        gi.G2API_GiveMeVectorFromMatrix(boltMatrix, POSITIVE_Y,
                                        cc->gent->client->renderInfo.muzzleDir);
      }
      ...
      if (cc->muzzleFlashTime > 0 && wData) {
        ...
        if (cc->currentState.eFlags & EF_ALT_FIRING) {
          // We're alt-firing, so see if we need to override with a custom
          // alt-fire effect
        ...
        if (cc->currentState.eFlags & EF_FIRING ||
          ...
          if (cent->gent->bounceCount) { // EWeb
            gi.G2API_SetBoneAnimIndex(
                ...
                cent->gent->rootBone, 2, 4, BONE_ANIM_OVERRIDE_FREEZE, 0.6f,
                cg.time, -1, -1);
          } else { // Emplaced Gun
            gi.G2API_SetBoneAnimIndex(
                &cent->gent->ghoul2[cent->gent->playerModel],
                cent->gent->rootBone, 0, 3, BONE_ANIM_OVERRIDE_FREEZE, 0.6f,
                cg.time, -1, -1);
          }

          if (effect) {
            // We got an effect and we're firing, so let 'er rip.
            theFxScheduler.PlayEffect(effect,
                                      cc->gent->client->renderInfo.muzzlePoint,
                                      cc->gent->client->renderInfo.muzzleDir);
          }
        }
  ...
  if (cent->gent->ghoul2.size()) {
    ...
    if (cent->gent->classname &&
        Q_stricmp("limb", cent->gent->classname) == 0) { // limb, copy RGB
    ...
    if (s1->weapon == WP_SABER && cent->gent && cent->gent->owner &&
        cent->gent->owner->inuse) { // flying lightsaber
      // FIXME: better way to tell what it is would be nice
      if (cent->gent->classname &&
          !Q_stricmp("limb", cent->gent->classname)) { // limb, just add blade
        if (cent->gent->owner->client) {
          if (cent->gent->owner->client->ps.saber[0].Length() > 0) {
            CG_AddSaberBlade(&cg_entities[cent->gent->owner->s.number],
                             &cg_entities[cent->gent->s.number], NULL,
                             ent.renderfx, cent->gent->weaponModel[0],
                             cent->lerpOrigin, cent->lerpAngles);
          } else if (cent->gent->owner->client->ps.saberEventFlags &
                     SEF_INWATER) {
            CG_CheckSaberInWater(&cg_entities[cent->gent->owner->s.number],
                                 &cg_entities[cent->gent->s.number], 0,
                                 cent->gent->weaponModel[0], cent->lerpOrigin,
                                 cent->lerpAngles);
          }
        }
      } else { // thrown saber
        ...
        if (cent->gent->owner->client &&
            ...
            g_entities[cent->currentState.otherEntityNum]
                .client->ps.saber[0]
                .Active()) { // saber is in-flight and active, play a sound on
                             // it
          if (cent->gent->owner->client->ps.saberEntityState == SES_RETURNING &&
            ...
            cgi_S_AddLoopingSound(
                ...
                cgs.sound_precache[g_entities[cent->currentState.clientNum]
                                       .client->ps.saber[0]
                                       .soundLoop]);
          } else {
            int spinSound;
            if (cent->gent->owner->client->ps.saber[0].spinSound &&
                cgs.sound_precache[cent->gent->owner->client->ps.saber[0]
                                       .spinSound]) {
              spinSound =
                  cgs.sound_precache[cent->gent->owner->client->ps.saber[0]
                                         .spinSound];
            } else if (cent->gent->owner->client->ps.saber[0].type ==
                       SABER_SITH_SWORD) {
              spinSound =
                  cgi_S_RegisterSound("sound/weapons/saber/saberspinoff.wav");
            } else {
              switch (cent->gent->owner->client->ps
                          .forcePowerLevel[FP_SABERTHROW]) {
              case FORCE_LEVEL_1:
                spinSound =
                    cgi_S_RegisterSound("sound/weapons/saber/saberspin3.wav");
                break;
              case FORCE_LEVEL_2:
                spinSound =
                    cgi_S_RegisterSound("sound/weapons/saber/saberspin2.wav");
                break;
              default:
              case FORCE_LEVEL_3:
                spinSound =
                    cgi_S_RegisterSound("sound/weapons/saber/saberspin1.wav");
                break;
              }
            }
            ...
            {
                    cgi_S_AddLoopingSound( cent->currentState.number,
            cent->lerpOrigin, vec3_origin, cg_weapons[WP_SABER].missileSound );
            }
            */
          }
        }
        if (cent->gent->owner->client) {
          if (cent->gent->owner->client->ps.saber[0].Length() >
              0) { // only add the blade if it's on
            CG_AddSaberBlade(&cg_entities[cent->gent->owner->s.number],
                             &cg_entities[cent->gent->s.number], NULL,
                             ent.renderfx, 0, cent->lerpOrigin,
                             cent->lerpAngles);
          } else if (cent->gent->owner->client->ps.saberEventFlags &
                     SEF_INWATER) {
            CG_CheckSaberInWater(&cg_entities[cent->gent->owner->s.number],
                                 &cg_entities[cent->gent->s.number], 0, 0,
                                 cent->lerpOrigin, cent->lerpAngles);
          }
        }
    ...
    if (s1->weapon == WP_SABER && cent->gent &&
        cent->gent->owner) { // flying lightsaber
      // light?  sound?
      if (cent->gent->owner->client &&
          cent->currentState.saberActive) { // saber is in-flight and active,
                                            // play a sound on it
        if (cent->gent->owner->client->ps.saberEntityState == SES_RETURNING &&
            cent->gent->owner->client->ps.saber[0].type != SABER_STAR) {
          if (cg_weapons[WP_SABER].firingSound) {
            cgi_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                                  vec3_origin,
                                  cg_weapons[WP_SABER].firingSound);
          }
        } else {
          int spinSound;
          if (cent->gent->owner->client->ps.saber[0].spinSound &&
            ...
            switch (
                cent->gent->owner->client->ps.forcePowerLevel[FP_SABERTHROW]) {
            case FORCE_LEVEL_1:
              spinSound =
                  cgi_S_RegisterSound("sound/weapons/saber/saberspin3.wav");
              break;
      ...
      CG_AddSaberBlade(&cg_entities[cent->gent->owner->s.number], NULL, &ent,
                       ent.renderfx, 0, NULL, NULL);
  ...
  */
  ...
  if (cent->gent && cent->gent->forcePushTime >
                        cg.time) { // FIXME: if I'm a rather large model, this
                                   // will look kind of stupid...
    CG_ForcePushBlur(cent->lerpOrigin);
  }
  CG_AddRefEntWithTransportEffect(cent, &ent);

  if (cent->gent->health <= 0 && cent->gent->s.weapon == WP_EMPLACED_GUN &&
      cent->gent->e_ThinkFunc) {
    // make the gun pulse red to warn about it exploding
    float val =
        (1.0f - (float)(cent->gent->nextthink - cg.time) / 3200.0f) * 0.3f;
  ...
  if (s1->eFlags & EF_FIRING && cent->gent->inuse) {
    ...
    if (temp) {
      ...
      if (prim) {
        ...
        CPrimitiveTemplate *prim =
            theFxScheduler.GetPrimitiveCopy(temp, "line2");
    ...
    theFxScheduler.PlayEffect("tripMine/laserImpactGlow", cent->gent->pos4,
                              ent.axis[0]);
  }
  ...
  if (s1->eFlags & EF_ALT_FIRING) {
    ...
    VectorMA(
        cent->lerpOrigin, cent->gent->radius - 5, dir,
        org); // stay a bit back from the impact point...this may not be enough?
  ...
  if (cent->gent->flags & (FL_DMG_BY_HEAVY_WEAP_ONLY | FL_SHIELDED)) {
    // Dumb assumption, but I guess we must be a shielded ion_cannon??  We
    // should probably verify if it's an ion_cannon that's Heavy Weapon only, we
    // don't want to make it shielded do we...?
    if ((!strcmp("misc_ion_cannon", cent->gent->classname)) &&
        (cent->gent->flags & FL_SHIELDED)) {
  ...
  if ((cg.snap->ps.forcePowersActive & (1 << FP_SEE)) &&
    ...
    if (s1->weapon == WP_THERMAL || s1->weapon == WP_DET_PACK ||
        s1->weapon == WP_TRIP_MINE ||
        (cent->gent &&
         cent->gent->e_UseFunc == useF_ammo_power_converter_use) ||
        (cent->gent &&
         cent->gent->e_UseFunc == useF_shield_power_converter_use) ||
        (s1->eFlags &
         EF_FORCE_VISIBLE)) { // really, we only need to do this for things like
                              // thermals, detpacks and tripmines, no?
...
static void CG_Speaker(centity_t *cent) {
  ...
  cgi_S_StartSound(NULL, cent->currentState.number, CHAN_ITEM,
                   cgs.sound_precache[cent->currentState.eventParm]);

  //	ent->s.frame = ent->wait * 10;
  //	ent->s.clientNum = ent->random * 10;
  cent->miscTime =
      (int)(cg.time + cent->currentState.frame * 100 +
            cent->currentState.clientNum * 100 * Q_flrand(-1.0f, 1.0f));
}
...
static void CG_Item(centity_t *cent) {
  ...
  */
  ...
  // if set to invisible, skip
  if ((!es->modelindex && !cent->gent->ghoul2.IsValid()) ||
      (es->eFlags & EF_NODRAW)) {
    return;
  }
  /*
  ...
  */
  if (cent->gent && !cent->gent->inuse) {
    // Yeah, I know....items were being freed on touch, but it could still get
    // here and draw incorrectly...
    return;
  }
  ...
  if (item->giType == IT_HOLOCRON) {
    ...
    cent->lerpOrigin[2] += (float)(4 + cos((cg.time + 1000) * scale) * 3) +
                           8; // just raised them up a bit
  }
  ...
  if (cent->gent->ghoul2.IsValid() &&
      cent->gent->ghoul2
          .size()) { // since modelindex is used by items as an index into
                     // items(not models), we need to ignore the hModel here to
                     // force it to use the ghoul2 model if we have one
  ...
  */
  ...
  // lovely...this is for weapons that should be oriented vertically.  For
  // weapons lockers and such.
  ...
  */
  ...
  if (item->giType == IT_WEAPON && item->giTag == WP_SABER &&
    ...
    for (int i = -4; i < 10; i += 1) {
      ...
      FX_AddSprite(org, NULL, NULL, 10.0f, 10.0f, wv * 0.5f, wv * 0.5f, 0.0f,
...
static void CG_Missile(centity_t *cent) {
  ...
  if (s1->otherEntityNum2 && (g_vehWeaponInfo[s1->otherEntityNum2].iShotFX ||
                              g_vehWeaponInfo[s1->otherEntityNum2].iModel)) {
    vec3_t forward;
    ...
    CG_PlayEffectID(g_vehWeaponInfo[s1->otherEntityNum2].iShotFX,
    ...
    if (g_vehWeaponInfo[s1->otherEntityNum2].iLoopSound) {
      ...
      if (cgs.sound_precache[g_vehWeaponInfo[s1->otherEntityNum2].iLoopSound] !=
          NULL_SFX) {
        cgi_S_AddLoopingSound(
            cent->currentState.number, cent->lerpOrigin, velocity,
            cgs.sound_precache[g_vehWeaponInfo[s1->otherEntityNum2]
                                   .iLoopSound]);
      }
    }
    // add custom model
    ...
    if (wData->alt_missileDlight)
      cgi_R_AddLightToScene(cent->lerpOrigin, wData->alt_missileDlight,
                            wData->alt_missileDlightColor[0],
                            wData->alt_missileDlightColor[1],
                            wData->alt_missileDlightColor[2]);

    // add missile sound
    if (weapon->alt_missileSound)
      cgi_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                            vec3_origin, weapon->alt_missileSound);
    ...
    if (wData->missileDlight)
      cgi_R_AddLightToScene(
          cent->lerpOrigin, wData->missileDlight, wData->missileDlightColor[0],
          wData->missileDlightColor[1], wData->missileDlightColor[2]);
  ...
  */
  ...
  if (s1->otherEntityNum2 && g_vehWeaponInfo[s1->otherEntityNum2].iModel &&
      cgs.model_draw[g_vehWeaponInfo[s1->otherEntityNum2].iModel] !=
          NULL_HANDLE)
    ...
    if (s1->pos.trType != TR_STATIONARY) {
      if (s1->eFlags & EF_MISSILE_STICK)
        RotateAroundDirection(
            ent.axis,
            cg.time * 0.5f); // Did this so regular missiles don't get broken
      else
...
static void CG_Mover(centity_t *cent) {
  ...
  */
  ...
  if (s1->solid == SOLID_BMODEL && s1->modelindex2) {
    ...
    /*		// Um, this does not interpolate nicely?  Not sure why it was
       here.... VectorAdd(cent->gent->absmin, cent->gent->absmax, org);
    ...
    */
    if (!VectorCompare(
            vec3_origin,
            cent->gent->modelAngles)) { // we have a rotational offset for the
                                        // model for this brush
      vec3_t modelAngles;
  ...
  // I changed it to always do it because nodraw seemed like it should actually
  // do what it says. Be aware that if you change this,
  ...
  if (s1->eFlags & EF_SHADER_ANIM) {
    ent.renderfx |= RF_SETANIMINDEX;
    ent.skinNum = s1->frame;
    // ent.shaderTime = cg.time*0.001f - s1->frame/s1->time;//NOTE: s1->time is
    // number of frames
  }
...
void CG_Cube(vec3_t mins, vec3_t maxs, vec3_t color, float alpha) {
  ...
  for (axis = 0, vec[0] = 0, vec[1] = 1, vec[2] = 2; axis < 3;
    ...
    //- face
    point[0][vec[0]] = point[1][vec[0]] = point[2][vec[0]] = point[3][vec[0]] =
        mins[vec[0]];

    FX_AddPoly(point, st, 4, NULL, NULL, alpha, alpha, 0.0f, color, color, 0.0f,
               rot, 0.0f, 0.0f, 100, cgs.media.solidWhiteShader, 0);
...
void CG_CubeOutline(vec3_t mins, vec3_t maxs, int time, unsigned int color,
  ...
  for (axis = 0, vec[0] = 0, vec[1] = 1, vec[2] = 2; axis < 3;
    ...
    //- face
    point1[vec[0]] = point2[vec[0]] = point3[vec[0]] = point4[vec[0]] =
        mins[vec[0]];
...
=========================
*/
void CG_AdjustPositionForMover(const vec3_t in, int moverNum, int atTime,
                               vec3_t out) {
  centity_t *cent;
  ...
  //	EvaluateTrajectory( &cent->currentState.apos, cg.snap->serverTime,
  //oldAngles );
...
void CG_CalcEntityLerpPositions(centity_t *cent) {
      ...
      cent->nextState) // cent->currentState.vehicleIndex != VEHICLE_NONE )
  {
    ...
    cent->currentState.vehicleAngles[0] =
        LerpAngle(cent->currentState.vehicleAngles[0],
                  cent->nextState->vehicleAngles[0], f);
    cent->currentState.vehicleAngles[1] =
        LerpAngle(cent->currentState.vehicleAngles[1],
                  cent->nextState->vehicleAngles[1], f);
    cent->currentState.vehicleAngles[2] =
        LerpAngle(cent->currentState.vehicleAngles[2],
  ...
  // FIXME: prediction on clients in timescale results in jerky positional
  // translation
  if (cent->interpolate) {
    // if the entity has a valid next state, interpolate a value between the
    // frames unless it is a mover with a known start and stop
    vec3_t current, next;
    float f;
    ...
    if (cent->currentState.apos.trType == TR_INTERPOLATE && cent->nextState) {
      ...
      !VectorCompare(current, next))
      {
              Com_Printf("%s last/next/lerp apos %s/%s/%s, f = %4.2f\n",
      cent->gent->script_targetname, vtos(current), vtos(next),
      vtos(cent->lerpAngles), f);
      }
      */
      /*
      Ghoul2 Insert Start
      */
      // now the nasty stuff - this will interpolate all ghoul2 models bone
      // angle overrides per model attached to this cent
      /*
      if (cent->gent->ghoul2.size())
      {
              LerpBoneAngleOverrides(cent);
    ...
    if (cent->currentState.pos.trType == TR_INTERPOLATE) {
      EvaluateTrajectory(&cent->currentState.pos, cg.snap->serverTime,
                         cent->lerpOrigin);
      /*
      if(cent->gent && cent->currentState.clientNum != 0 )
      {
  ...
  // FIXME: if it's blocked, it wigs out, draws it in a predicted spot, but
  // never makes it there - we need to predict it in the right place if this is
  // happens...
  ...
  trajectory_t *posData = &cent->currentState.pos;
  {
    ...
    if (ent && ent->inuse) {
      if (ent->s.eFlags & EF_BLOCKED_MOVER ||
          ent->s.pos.trType ==
              TR_STATIONARY) { // this mover has stopped moving and is going to
                               // wig out if we predict it
        // based on last frame's info- cut across the network and use the
        // currentOrigin
  ...
  CG_AdjustPositionForMover(cent->lerpOrigin,
                            cent->currentState.groundEntityNum, cg.time,
                            cent->lerpOrigin);
  /*
  Ghoul2 Insert Start
  */
  // now the nasty stuff - this will interpolate all ghoul2 models bone angle
  // overrides per model attached to this cent
  /*
  if (cent->gent->ghoul2.size())
  {
          LerpBoneAngleOverrides(cent);
...
void CG_CalcEntityLerpPositions(centity_t *cent) {
  if (cent->currentState.number == cg.snap->ps.clientNum) {
    ...
    OutputDebugString(va("b=(%6.2f,%6.2f,%6.2f)\n", cent->lerpOrigin[0],
                         cent->lerpOrigin[1], cent->lerpOrigin[2]));
    return;
  }

  if (cent->currentState.number != cg.snap->ps.clientNum && cent->interpolate &&
      cent->currentState.pos.trType == TR_INTERPOLATE) {
    if (cent->interpolate) {
      OutputDebugString(
          va("[%3d] interp %4.2f t=%6d  st = %6d  nst = %6d     b=%6.2f   "
             "nb=%6.2f\n",
             cent->currentState.number, cg.frameInterpolation, cg.time,
             cg.snap->serverTime, cg.nextSnap->serverTime,
             cent->currentState.pos.trBase[0], cent->nextState.pos.trBase[0]));
    } else {
      OutputDebugString(va("[%3d] nonext %4.2f t=%6d  st = %6d  nst = %6d     "
                           "b=%6.2f   nb=%6.2f\n",
                           cent->currentState.number, cg.frameInterpolation,
                           cg.time, cg.snap->serverTime, 0,
                           cent->currentState.pos.trBase[0], 0.0f));
    }
  }
...
static void CG_AddLocalSet(centity_t *cent) {
  cent->gent->setTime = cgi_S_AddLocalSet(
      cent->gent->soundSet, cg.refdef.vieworg, cent->lerpOrigin,
...
void CG_DLightThink(centity_t *cent) {
  if (cent->gent) {
    ...
    if (percentage >= 1.0f) { // We hit the end
      ...
      case 0:                             // Fading from start to final
        if (cent->gent->spawnflags & 8) { // PULSER
          if (tDelta - cent->gent->speed - cent->gent->wait >=
              0) { // Time to start fading down
      ...
      case 1: // Fading from final to start
        if (tDelta - cent->gent->speed - cent->gent->radius >=
            0) { // Time to start fading up
          cent->gent->painDebounceTime = cg.time;
    ...
    case 0: // Fading from start to final
      for (i = 0; i < 4; i++) {
        currentRGBA[i] =
            cent->gent->startRGBA[i] +
            ((cent->gent->finalRGBA[i] - cent->gent->startRGBA[i]) *
             percentage);
      }
      break;
    case 1: // Fading from final to start
      for (i = 0; i < 4; i++) {
        currentRGBA[i] =
            cent->gent->finalRGBA[i] +
            ((cent->gent->startRGBA[i] - cent->gent->finalRGBA[i]) *
             percentage);
      }
      break;
    ...
    case 3:                     // Fading from current intensity to 0
      ...
      currentRGBA[3] =
          cent->gent->startRGBA[3] - (cent->gent->startRGBA[3] * percentage);
      break;
    default:
      return;
      break;
    ...
    cgi_R_AddLightToScene(org, currentRGBA[3] * 10, currentRGBA[0],
                          currentRGBA[1], currentRGBA[2]);
  }
}

void CG_Limb(
    centity_t *cent) { // first time we're drawn, remove us from the owner ent
  if (cent->gent && cent->gent->owner && cent->gent->owner->ghoul2.size()) {
    gentity_t *owner = cent->gent->owner;
    if (cent->gent->aimDebounceTime) { // done with dismemberment, just waiting
                                       // to mark owner dismemberable again
      ...
      {                        // turn the limb off
        ...
        gi.G2API_SetSurfaceOnOff(&owner->ghoul2[owner->playerModel],
                                 cent->gent->target2,
      ...
      if (owner->weaponModel[0] > 0) { // the corpse hasn't dropped their weapon
        if (cent->gent->count == BOTH_DISMEMBER_RARM ||
            cent->gent->count ==
                BOTH_DISMEMBER_TORSO1) //&& ent->s.weapon == WP_SABER &&
                                       //ent->weaponModel[0] != -1 )
      ...
      if (owner->client->NPC_class == CLASS_PROTOCOL ||
          debug_subdivision->integer || g_saberRealisticCombat->integer) {
...
void CG_MatrixEffect(centity_t *cent) {
  ...
  if ((cent->currentState.boltInfo & MEF_MULTI_SPIN)) { // multiple spins
    ...
    if (cent->currentState.eventParm &&
        cent->currentState.eventParm !=
            MATRIX_EFFECT_TIME) { // not a falling multi-spin, so stretch out
                                  // the effect over the whole desired length
                                  // (or: condenses it, too)
  ...
  bool stopEffect = ((totalElapsedTime > cent->currentState.eventParm) ||
                     cg.missionStatusShow || in_camera);

  if (!stopEffect && (cent->currentState.boltInfo & MEF_HIT_GROUND_STOP) &&
      g_entities[cent->currentState.otherEntityNum].client) {
    if (g_entities[cent->currentState.otherEntityNum]
            .client->ps.groundEntityNum != ENTITYNUM_NONE) {
      stopEffect = true;
    } else if (g_entities[cent->currentState.otherEntityNum]
                   .client->NPC_class == CLASS_VEHICLE) {
      Vehicle_t *pVeh =
          g_entities[cent->currentState.otherEntityNum].m_pVehicle;
  ...
  if (stopEffect) { // time is up or this is a falling spin and they hit the
                    // ground or mission end screen is up
    cg.overrides.active &=
        ~(/*CG_OVERRIDE_3RD_PERSON_ENT|*/ CG_OVERRIDE_3RD_PERSON_RNG |
          CG_OVERRIDE_3RD_PERSON_ANG | CG_OVERRIDE_3RD_PERSON_POF);
  ...
  if (cent->currentState.boltInfo & MEF_LOOK_AT_ENEMY) {
    ...
    VectorCopy(cg_entities[g_entities[cent->currentState.otherEntityNum]
                               .lastEnemy->s.number]
                   .lerpOrigin,
               toEnemy);
    //		CG_DrawEdge(cg_entities[cent->currentState.otherEntityNum].lerpOrigin,
    //toEnemy, EDGE_NORMAL);
    VectorSubtract(cg_entities[cent->currentState.otherEntityNum].lerpOrigin,
                   toEnemy, toEnemy);
    ...
    cg.overrides.thirdPersonAngle =
        toEnemyAngles[1] -
        cg_entities[cent->currentState.otherEntityNum].lerpAngles[1] + 145.0f;
    cg.overrides.thirdPersonAngle =
        AngleNormalize180(cg.overrides.thirdPersonAngle);
  ...
  } else if (!(cent->currentState.boltInfo &
               MEF_NO_TIMESCALE)) { // ramp the timescale
    // slowdown
  ...
  } else { // FIXME: MEF_HIT_GROUND_STOP: if they're on the ground, stop
           // spinning and stop timescale
  ...
  if (!(cent->currentState.boltInfo & MEF_NO_VERTBOB)) { // bob the pitch
    ...
    if (elapsedTime < MATRIX_EFFECT_TIME * 0.33f) {
      cg.overrides.thirdPersonPitchOffset -=
          30.0f * elapsedTime / (MATRIX_EFFECT_TIME * 0.33);
    } else if (elapsedTime > MATRIX_EFFECT_TIME * 0.66f) {
      cg.overrides.thirdPersonPitchOffset -=
          30.0f * (MATRIX_EFFECT_TIME - elapsedTime) /
          (MATRIX_EFFECT_TIME * 0.33);
    } else {
      cg.overrides.thirdPersonPitchOffset -= 30.0f;
    }
  }

  if (!(cent->currentState.boltInfo &
        MEF_NO_RANGEVAR)) { // vary the camera range
    // pull back
...
static void CG_Clouds(centity_t *cent) {
  ...
  if (cent->gent->spawnflags &
      1) // TUBE type, the one with a hole in the middle
  {
...
static void CG_AddCEntity(centity_t *cent) {
  ...
  */
  ...
  if (cent->gent->ghoul2.IsValid()) {
    trap_G2_SetGhoul2ModelIndexes(cent->gent->ghoul2, cgs.model_draw,
                                  cgs.skins);
  }
...
void CG_AddPacketEntities(qboolean isPortal) {
  ...
  //	cent = &cg_entities[ ps->clientNum ];	// not needed now that player is
  //in the snap packet 	CG_AddCEntity( cent ); //
...
void CG_ROFF_NotetrackCallback(centity_t *cent, const char *notetrack) {
  ...
  if (notetrack[i] != ' ') { // didn't pass in a valid notetrack type, or forgot
                             // the argument for it
    return;
  }
  ...
  if (strcmp(type, "effect") == 0) {
    if (!addlArgs) {
      // sprintf(errMsg, "Offset position argument for 'effect' type is
      // invalid."); goto functionend;
    ...
    if (posoffsetGathered < 3) {
      Q_strncpyz(errMsg,
                 "Offset position argument for 'effect' type is invalid.",
                 sizeof(errMsg));
      goto functionend;
    }
  ...
  defaultoffsetposition:
    ...
    if (objectID) {
      if (addlArgs) { // if there is an additional argument for an effect it is
                      // expected to be XANGLE-YANGLE-ZANGLE
        i++;
    ...
    cgi_S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_BODY,
                     objectID);
