...
qboolean G2_SetRootSurface(CGhoul2Info_v &ghoul2, const int modelIndex, const char *surfaceName) {
	...
	if (surf != -1) {
		...
		CConstructBoneList CBL(ghoul2[modelIndex].mSurfaceRoot, activeBones, ghoul2[modelIndex].mSlist, mod_m, ghoul2[modelIndex].mBlist);
		...
		for (int i = 0; i < ghoul2.size(); i++) {
			...
			if (ghoul2[i].mModelBoltLink != -1) {
				...
				if (((int)ghoul2[boltMod].mBltlist.size() <= boltNum) ||
					((ghoul2[boltMod].mBltlist[boltNum].boneNumber == -1) && (ghoul2[boltMod].mBltlist[boltNum].surfaceNumber == -1))) {
