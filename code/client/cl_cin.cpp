...
static int CL_iPlaybackStartTime; // so I can stop users quitting playback <1
...
static void RoQShutdown(void) {
  ...
  if (!cinTable[currentHandle].buf) {
    if (cinTable[currentHandle].iFile) {
      //			assert( 0 && "ROQ handle leak-prevention WAS
      //needed!");
      FS_FCloseFile(cinTable[currentHandle].iFile);
...
int CIN_PlayCinematic(const char *arg, int x, int y, int w, int h,
                      int systemBits, const char *psAudioFile /* = NULL */) {
...
static void CIN_AddTextCrawl() {
  ...
  refdef.height = cls.glconfig.vidHeight *
                  2; // deliberately extend off the bottom of the screen
  ...
  VectorScaleM(verts[2].modulate, 0.1f,
               verts[2].modulate); // darken at the top??
  VectorScaleM(verts[3].modulate, 0.1f, verts[3].modulate);

#define TIMEOFFSET                                                             \
  +(cls.realtime - CL_iPlaybackStartTime - TC_DELAY) * 0.000015f - 1
  ...
  if (cls.realtime - CL_iPlaybackStartTime >= TC_STOPTIME) {
    ...
    RoQShutdown();
    SCR_StopCinematic(); // change ROQ from FMV_IDLE to FMV_EOF, and clear some
                         // other vars
  }
}
...
// external vars so I can check if the game is setup enough that I can play the
// intro video...
//
extern qboolean com_fullyInitialized;
extern qboolean s_soundStarted, s_soundMuted;
//
// ... and if the app isn't ready yet (which should only apply for the intro
// video), then I use these...
//
...
static qboolean CIN_HardwareReadyToPlayVideos(void) {
  if (com_fullyInitialized && cls.rendererStarted && cls.soundStarted &&
      cls.soundRegistered) {
    return qtrue;
  }
...
static void PlayCinematic(const char *arg, const char *s, qboolean qbInGame) {
  ...
  Cvar_Set("timescale", "1"); // jic we were skipping a scripted cinematic,
                              // return to normal after playing video
  ...
  if (S_FileExists(arg)) {
    SCR_StopCinematic();
    // command-line hack to avoid problems when playing intro video before app
    // is fully setup...
    //
    ...
    qboolean bIsForeign =
        (qboolean)(s_language && Q_stricmp(s_language->string, "english") &&
...
#ifdef JK2_MODE
      hCrawl =
          re.RegisterShaderNoMip(va("menu/video/tc_%d", sp_language->integer));
      if (!hCrawl) {
...
#else
      hCrawl =
          re.RegisterShaderNoMip(va("menu/video/tc_%s", se_language->string));
      if (!hCrawl) {
        hCrawl = re.RegisterShaderNoMip(
            "menu/video/tc_english"); // failed, so go back to english
      }
#endif
      bits |= CIN_hold;
    } else if (bIsForeign) {
    ...
    if (CL_handle >= 0) {
      ...
      if (qbInGame) {
        Cvar_SetValue("cl_paused", 1); // remove-menu call will have unpaused
                                       // us, so we sometimes need to re-pause
      }

      CL_iPlaybackStartTime =
          cls.realtime; // special use to avoid accidentally skipping ingame
                        // videos via fast-firing
    } else {
  ...
  if (bFailed) {
    ...
    SCR_StopCinematic(); // I know this seems pointless, but it clears a bunch
                         // of vars as well
  } else {
    // this doesn't work for now...
    //
    //		if (cls.state == CA_ACTIVE){
    //			re.InitDissolve(qfalse);	// so we get a dissolve between previous
    //screen image and cinematic
    //		}
  }
}
...
qboolean CL_CheckPendingCinematic(void) {
  if (gbPendingCinematic && CIN_HardwareReadyToPlayVideos()) {
    ...
    PlayCinematic(sPendingCinematic_Arg,
                  sPendingCinematic_s[0] ? sPendingCinematic_s : NULL, qfalse);
    return qtrue;
  }
  return qfalse;
}
...
// Externally-called only, and only if cls.state == CA_CINEMATIC (or
// CL_IsRunningInGameCinematic() == true now)
//
void SCR_DrawCinematic(void) {
  ...
  if (CL_handle >= 0 && CL_handle < MAX_VIDEO_HANDLES) {
    ...
    if (cinTable[CL_handle].hCRAWLTEXT &&
        (cls.realtime - CL_iPlaybackStartTime >= TC_DELAY)) {
      CIN_AddTextCrawl();
    }
  }
}
...
void SCR_RunCinematic(void) {
  ...
  if (CL_handle >= 0 && CL_handle < MAX_VIDEO_HANDLES) {
    ...
    if (CL_IsRunningInGameCinematic() && Status == FMV_IDLE &&
        !cinTable[CL_handle].holdAtEnd) {
      SCR_StopCinematic(); // change ROQ from FMV_IDLE to FMV_EOF, and clear
                           // some other vars
    }
  }
}
...
void SCR_StopCinematic(qboolean bAllowRefusal /* = qfalse */) {
  if (bAllowRefusal) {
    if ((CL_handle >= 0 && CL_handle < MAX_VIDEO_HANDLES) &&
        cls.realtime <
            CL_iPlaybackStartTime + 1200 // 1.2 seconds have to have elapsed
    ) {
      return;
    }
  }
