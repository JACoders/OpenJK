...
//---------------------------------------------------------
{
	...
	if ((ent->s.eFlags & EF_HELD_BY_SAND_CREATURE)) { // blow once creature is underground (done with anim)
		...
		if (!ent->activator || !ent->activator->client ||
			!ent->activator->client->ps.legsAnimTimer) { // either something happened to the sand creature or it's done with it's attack anim
			// blow!
...
//---------------------------------------------------------
{
	...
	if (ent->health > 0) {
		...
		if ((ent->NPC || (ent->s.number && thisIsAShooter)) && ent->enemy) { // NPC or misc_weapon_shooter
		...
		} else if (thisIsAShooter && ent->target && !VectorCompare(ent->pos1, vec3_origin)) { // misc_weapon_shooter firing at a position
