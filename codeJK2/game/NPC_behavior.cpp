...
void NPC_BSFollowLeader(void) {
  ...
  if (NPC->enemy &&
      NPC->client->ps.weapon) { // If have an enemy, face him and fire
  ...
  if (curAnim != BOTH_ATTACK1 && curAnim != BOTH_ATTACK2 &&
    ...
    if (ucmd.forwardmove || ucmd.rightmove ||
        VectorCompare(vec3_origin, NPC->client->ps.moveDir)) {
    ...
    if (NPCInfo->investigateDebounceTime > level.time) {
      ...
      if (NPCInfo->tempGoal->waypoint != WAYPOINT_NONE) {
        if (!Q_irand(0, 30)) {
          ...
          if (numEdges != WAYPOINT_NONE) {
            ...
            int nextWp =
                navigator.GetNodeEdge(NPCInfo->tempGoal->waypoint, branchNum);
            navigator.GetNodePosition(nextWp, branchPos);

            VectorSubtract(branchPos, NPCInfo->tempGoal->currentOrigin,
                           lookDir);
            NPCInfo->desiredYaw =
          ...
          // nextWp =
          // waypoints[NPCInfo->tempGoal->waypoint].nextWaypoint[branch][NPCInfo->stats.moveType];
          // vec3_t	lookDir;

          // VectorSubtract( waypoints[nextWp].origin,
          // NPCInfo->tempGoal->currentOrigin, lookDir ); Look in that direction
          // +- 45 degrees NPCInfo->desiredYaw = AngleNormalize360( vectoyaw(
          // lookDir ) + Q_flrand( -45, 45 ) );
        }
      }
      // gi.Printf(".");
      ...
      if (NPC->waypoint == NPCInfo->homeWp) {
        ...
        */
        ...
        navigator.GetNodePosition(NPCInfo->homeWp,
                                  NPCInfo->tempGoal->currentOrigin);
        NPCInfo->tempGoal->waypoint = NPCInfo->homeWp;
        /*
        VectorCopy( waypoints[NPCInfo->homeWp].origin,
        NPCInfo->tempGoal->currentOrigin ); NPCInfo->tempGoal->waypoint =
        NPCInfo->homeWp;
        //gi.Printf("\nHeading for wp %d...\n", NPCInfo->homeWp);
        */
      }
...
qboolean NPC_CheckSurrender(void) {
  if (!g_AIsurrender->integer) { // not enabled
    return qfalse;
  }
  if (!Q3_TaskIDPending(NPC, TID_MOVE_NAV) &&
    ...
    if (NPC->s.weapon != WP_ROCKET_LAUNCHER && NPC->s.weapon != WP_REPEATER &&
      ...
      if (NPCInfo->group &&
          NPCInfo->group->numGroup <=
              1) { // I'm alone but I was in a group//FIXME: surrender anyway if
                   // just melee or no weap?
      ...
      if (numEdges != WAYPOINT_NONE) {
        ...
        for (int branchNum = 0; branchNum < numEdges; branchNum++) {
          ...
          if (DotProduct(runDir, dangerDir) >
              Q_flrand(0, 0.5)) { // don't run toward danger
            continue;
          }
...
void NPC_StartFlee(gentity_t *enemy, vec3_t dangerPoint, int dangerLevel,
  ...
  if (dangerLevel > AEL_DANGER || NPC->s.weapon == WP_NONE ||
    ...
    cp = NPC_FindCombatPoint(
        NPC->currentOrigin, NPC->currentOrigin, dangerPoint,
        CP_COVER | CP_AVOID | CP_HAS_ROUTE | CP_NO_PVS, 128);
  }
  // FIXME: still happens too often...
  if (cp == -1) { // okay give up on the no PVS thing
    cp =
        NPC_FindCombatPoint(NPC->currentOrigin, NPC->currentOrigin, dangerPoint,
    ...
    if (cp == -1) { // okay give up on the avoid
      cp = NPC_FindCombatPoint(NPC->currentOrigin, NPC->currentOrigin,
                               dangerPoint, CP_COVER | CP_HAS_ROUTE, 128);
      if (cp == -1) { // okay give up on the cover
        cp = NPC_FindCombatPoint(NPC->currentOrigin, NPC->currentOrigin,
                                 dangerPoint, CP_HAS_ROUTE, 128);
      }
    }
  }
  ...
  if (cp != -1) { // found a combat point
    ...
    if (NPC->s.weapon != WP_NONE) {
      ...
      VectorCopy(dangerPoint,
                 NPCInfo->investigateGoal); // FIXME: make a new field for this?
    }
  }
