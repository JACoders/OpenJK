...
IIcarusInterface *IIcarusInterface::GetIcarus(int flavor,
...
void CIcarus::Signal(const char *identifier) { m_signals[identifier] = 1; }
...
int CIcarus::AllocateSequences(int numSequences, int *idTable) {
  ...
  for (int i = 0; i < numSequences; i++) {
    // If the GUID of this sequence is higher than the current, take this a the
    // "current" GUID
    if (idTable[i] > m_GUID)
      m_GUID = idTable[i];
...
int CIcarus::SaveSequenceIDTable() {
  ...
  STL_ITERATE(sqi, m_sequences) { idTable[itr++] = (*sqi)->GetID(); }

  // game->WriteSaveData( INT_ID('S','Q','T','B'), idTable, sizeof( int ) *
  // numSequences );
...
int CIcarus::SaveSequences() {
  ...
  sequence_l::iterator sqi;
  STL_ITERATE(sqi, m_sequences) { (*sqi)->Save(); }
...
int CIcarus::SaveSignals() {
  ...
  // game->WriteSaveData( INT_ID('I','S','I','G'), &numSignals, sizeof(
  // numSignals ) );
...
int CIcarus::GetFlavor() { return m_flavor; }
...
int CIcarus::Save() {
  ...
  ojk::SavedGameHelper saved_game(game->get_saved_game_file());

  // Save out a ICARUS save block header with the ICARUS version
  double version = ICARUS_VERSION;

  saved_game.write_chunk<double>(INT_ID('I', 'C', 'A', 'R'), version);
  ...
  saved_game.write_chunk(INT_ID('I', 'S', 'E', 'Q'), m_byBuffer,
...
int CIcarus::Load() {
  ...
  saved_game.read_chunk<double>(INT_ID('I', 'C', 'A', 'R'), version);
  ...
  if (version != ICARUS_VERSION) {
    ...
    game->DebugPrint(
        IGameInterface::WL_ERROR,
        "save game data contains outdated ICARUS version information!\n");
    return false;
  }

  // Read into the buffer all our data.
  saved_game.read_chunk(INT_ID('I', 'S', 'E', 'Q'));

  const unsigned char *sg_buffer_data =
      static_cast<const unsigned char *>(saved_game.get_buffer_data());

  int sg_buffer_size = saved_game.get_buffer_size();

  if (sg_buffer_size < 0 ||
      static_cast<size_t>(sg_buffer_size) > MAX_BUFFER_SIZE) {
    DestroyBuffer();
    game->DebugPrint(IGameInterface::WL_ERROR,
                     "invalid ISEQ length: %d bytes\n", sg_buffer_size);
    return false;
  }

  std::uninitialized_copy_n(sg_buffer_data, sg_buffer_size, m_byBuffer);

  // Load all signals
  if (LoadSignals() == false) {
    DestroyBuffer();
    game->DebugPrint(IGameInterface::WL_ERROR,
                     "failed to load signals from save game!\n");
    return false;
  }
  ...
  if (LoadSequences() == false) {
    DestroyBuffer();
    game->DebugPrint(IGameInterface::WL_ERROR,
                     "failed to load sequences from save game!\n");
    return false;
  }
  ...
  if (LoadSequencers() == false) {
    DestroyBuffer();
    game->DebugPrint(IGameInterface::WL_ERROR,
                     "failed to load sequencers from save game!\n");
    return false;
  }
...
void CIcarus::CreateBuffer() {
  DestroyBuffer();
  m_byBuffer =
      (unsigned char *)IGameInterface::GetGame()->Malloc(MAX_BUFFER_SIZE);
  m_ulBufferCurPos = 0;
}
...
void CIcarus::BufferWrite(void *pSrcData, unsigned long ulNumBytesToWrite) {
  ...
  if (MAX_BUFFER_SIZE - m_ulBufferCurPos <
      ulNumBytesToWrite) { // Write out the buffer with all our collected data
                           // so far...
    IGameInterface::GetGame()->DebugPrint(
        IGameInterface::WL_ERROR,
        "BufferWrite: Out of buffer space, Flushing.");
...
void CIcarus::BufferRead(void *pDstBuff, unsigned long ulNumBytesToRead) {
  ...
  if (m_ulBytesRead + ulNumBytesToRead >
      MAX_BUFFER_SIZE) { // We've tried to read past the buffer...
    IGameInterface::GetGame()->DebugPrint(
        IGameInterface::WL_ERROR,
