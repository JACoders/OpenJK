...
void SV_LinkEntity(gentity_t *gEnt) {
	...
	// set the abs box
	if (gEnt->bmodel && (angles[0] || angles[1] || angles[2])) { // expand for rotation
		float max;
		int i;
	...
	num_leafs = CM_BoxLeafnums(gEnt->absmin, gEnt->absmax, leafs, MAX_TOTAL_ENT_LEAFS, &lastLeaf);
		...
		if (area != -1) { // doors may legally straggle two areas,
			...
			if (ent->areanum != -1 && ent->areanum != area) {
				if (ent->areanum2 != -1 && ent->areanum2 != area && sv.state == SS_LOADING) {
					Com_DPrintf("Object %i touching 3 areas at %f %f %f\n", gEnt->s.number, gEnt->absmin[0], gEnt->absmin[1], gEnt->absmin[2]);
				}
				ent->areanum2 = area;
			} else {
				ent->areanum = area;
...
void SV_AreaEntities_r(worldSector_t *node, areaParms_t *ap) {
	...
	for (check = node->entities; check; check = next) {
		...
		if (gcheck->absmin[0] > ap->maxs[0] || gcheck->absmin[1] > ap->maxs[1] || gcheck->absmin[2] > ap->maxs[2] || gcheck->absmax[0] < ap->mins[0] ||
	...
	for (i = 0; i < num; i++) {
...
#endif
		{
			...
			CM_TransformedBoxTrace(&trace, (float *)clip->start, (float *)clip->end, (float *)clip->mins, (float *)clip->maxs, clipHandle, clip->contentmask,
								   origin, angles);
#else
			CM_TransformedBoxTrace(&trace, clip->start, clip->end, clip->mins, clip->maxs, clipHandle, clip->contentmask, origin, angles);
#endif
		...
		if (trace.allsolid) {
			if (!clip->trace.allsolid) { // We didn't come in here all solid, so set the clip->trace's entityNum
				clip->trace.entityNum = touch->s.number;
			}
			clip->trace.allsolid = qtrue;
			trace.entityNum = touch->s.number;
		} else if (trace.startsolid) {
			if (!clip->trace.startsolid) { // We didn't come in here starting solid, so set the clip->trace's entityNum
				clip->trace.entityNum = touch->s.number;
			}
		...
		if ((trace.entityNum == touch->s.number) && (clip->eG2TraceType != G2_NOCOLLIDE)) {
			...
			if (touch->ghoul2.size() && !(touch->contents & CONTENTS_LIGHTSABER)) {
				...
				if (touch->client) {
					...
					re.G2API_CollisionDetect(clip->trace.G2CollisionMap, touch->ghoul2, world_angles, touch->client->origin, sv.time, touch->s.number,
											 clip->start, clip->end, touch->s.modelScale, G2VertSpaceServer, clip->eG2TraceType, clip->useLod, radius);
				}
				// no, so use the normal entity state
				else {
					// use the correct origin and angles!  is this right now?
					re.G2API_CollisionDetect(clip->trace.G2CollisionMap, touch->ghoul2, touch->currentAngles, touch->currentOrigin, sv.time, touch->s.number,
											 clip->start, clip->end, touch->s.modelScale, G2VertSpaceServer, clip->eG2TraceType, clip->useLod, radius);
				}
...
Ghoul2 Insert Start
*/
void SV_Trace(trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, const int passEntityNum, const int contentmask,
