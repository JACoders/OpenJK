...
cvar_t *debugNPCFreeze; // set to disable bot ai and temporarily freeze them in
                        // place
cvar_t *debugNPCName;
...
void CorpsePhysics(gentity_t *self) {
  ...
  if (self->client->ps.groundEntityNum != ENTITYNUM_NONE &&
      !(self->flags & FL_DISINTEGRATED)) { // on the ground
...
qboolean G_OkayToRemoveCorpse(gentity_t *self) {
  ...
  if (self->client && self->client->NPC_class != CLASS_VEHICLE &&
    ...
    if (pVeh) {
      if (!pVeh->m_pVehicleInfo->Eject(
              pVeh, self,
              qtrue)) { // dammit, still can't get off the vehicle...
        return qfalse;
      }
    } else {
      assert(0);
#ifndef FINAL_BUILD
      Com_Printf(S_COLOR_RED "ERROR: Dead pilot's vehicle removed while corpse "
  ...
  if (IIcarusInterface::GetIcarus()->IsRunning(
          self->m_iIcarusID)) { // still running a script
    return qfalse;
  }
  ...
  if (self->activator && self->activator->client &&
      ((self->activator->client->ps.eFlags & EF_HELD_BY_RANCOR) ||
       ...
       (self->activator->client->ps.eFlags &
        EF_HELD_BY_WAMPA))) { // still holding a victim?
    return qfalse;
  }

  if (self->client && ((self->client->ps.eFlags & EF_HELD_BY_RANCOR) ||
                       (self->client->ps.eFlags & EF_HELD_BY_SAND_CREATURE) ||
                       (self->client->ps.eFlags &
                        EF_HELD_BY_WAMPA))) { // being held by a creature
    return qfalse;
  }

  if (self->client->ps.heldByClient < ENTITYNUM_WORLD) { // being dragged
    return qfalse;
  }
...
void NPC_RemoveBody(gentity_t *self) {
  ...
  if (self->NPC->nextBStateThink <= level.time) { // run logic at 10 fps
    ...
    self->maxs[2] =
        self->client->renderInfo.eyePoint[2] - self->currentOrigin[2] + 4;
    if (self->maxs[2] < -8) {
      self->maxs[2] = -8;
    }

    if ((self->NPC->aiFlags &
         NPCAI_HEAL_ROSH)) { // kothos twins' bodies are never removed
      return;
    }
    ...
    if (self->NPC && self->NPC->timeOfDeath <= level.time) {
      ...
      ///	if ( self->client->playerTeam == TEAM_SCAVENGERS ||
      ///self->client->playerTeam == TEAM_KLINGON
      //		|| self->client->playerTeam == TEAM_HIROGEN ||
      //self->client->playerTeam == TEAM_MALON )
      ...
      if (self->client->playerTeam == TEAM_ENEMY ||
        ...
        if (DistanceSquared(g_entities[0].currentOrigin, self->currentOrigin) <=
            REMOVE_DISTANCE_SQR) {
          return;
        }
      ...
      // want 			to remove the bodies... but in other cases it's just weird, like
      //			when they're right behind you in a closed room
      //and when they've been 			placed as dead NPCs by a designer... 			For now we
      //just assume that a corpse with no enemy was 			placed in the map as a
      //corpse
      if (self->enemy) {
...
void pitch_roll_for_slope(gentity_t *forwhom, vec3_t pass_slope,
                          vec3_t storeAngles, qboolean keepPitch) {
  vec3_t slope;
  ...
  if (!pass_slope || VectorCompare(vec3_origin, pass_slope)) {
    ...
    endspot[2] -= 300;
    gi.trace(&trace, forwhom->currentOrigin, vec3_origin, vec3_origin, endspot,
             forwhom->s.number, MASK_SOLID, (EG2_Collision)0, 0);
  ...
  float oldPitch = 0;
  if (forwhom->client && forwhom->client->NPC_class ==
                             CLASS_VEHICLE) { // special code for vehicles
  ...
  if (storeAngles) {
    ...
    if (oldmins2 > forwhom->mins[2]) { // our mins is now lower, need to move up
      // FIXME: trace?
...
void NPC_PostDeathThink( void )
{
        ...
        if( NPC->client->ps.groundEntityNum == ENTITYNUM_NONE || !VectorCompare(
vec3_origin, NPC->client->ps.velocity ) )
        {
                if ( NPC->client->ps.groundEntityNum != ENTITYNUM_NONE &&
NPC->client->ps.friction == 1.0 )//check avelocity?
                {
        ...
        cornerdist[FRONT] = cornerdist[BACK] = cornerdist[RIGHT] =
cornerdist[LEFT] = 0.0f;
        ...
        endpos[2] -= 128;
        gi.trace( &trace1, startpos, vec3_origin, vec3_origin, endpos,
NPC->s.number, MASK_SOLID, ); if( !trace1.allsolid && !trace1.startsolid )
        {
        ...
        VectorCopy( startpos, endpos );
        endpos[2] -= 128;
        gi.trace( &trace2, startpos, vec3_origin, vec3_origin, endpos,
NPC->s.number, MASK_SOLID ); if( !trace2.allsolid && !trace2.startsolid )
        {
        ...
        VectorCopy( startpos, endpos );
        endpos[2] -= 128;
        gi.trace( &trace3, startpos, vec3_origin, vec3_origin, endpos,
NPC->s.number, MASK_SOLID ); if ( !trace3.allsolid && !trace3.startsolid )
        {
        ...
        VectorCopy( startpos, endpos );
        endpos[2] -= 128;
        gi.trace( &trace4, startpos, vec3_origin, vec3_origin, endpos,
NPC->s.number, MASK_SOLID ); if ( !trace4.allsolid && !trace4.startsolid )
        {
        ...
        //OK!  Now if two opposite sides are hanging, use a third if any, else,
do nothing if ( cornerdist[FRONT] > MIN_DROP_DIST && cornerdist[BACK] >
MIN_DROP_DIST ) frontbackbothclear = true;

        if ( cornerdist[RIGHT] > MIN_DROP_DIST && cornerdist[LEFT] >
MIN_DROP_DIST ) rightleftbothclear = true;
        ...
        case 1:
                ...
                gi.trace( &movetrace, NPC->currentOrigin, NPC->mins, NPC->maxs,
endpos, NPC->s.number, MASK_MONSTERSOLID ); if ( movetrace.allsolid ||
movetrace.startsolid || movetrace.fraction < 1.0 ) if ( canmove( movetrace.ent )
) whichtrace = -1; else whichtrace = 0; break; case 2: VectorMA(
NPC->currentOrigin, -NPC->maxs[0], forward, endpos ); gi.trace( &movetrace,
NPC->currentOrigin, NPC->mins, NPC->maxs, endpos, NPC->s.number,
MASK_MONSTERSOLID ); if ( movetrace.allsolid || movetrace.startsolid ||
movetrace.fraction < 1.0 ) if ( canmove( movetrace.ent ) ) whichtrace = -1; else
                                whichtrace = 0;
                break;
        case 3:
...
endpos, NPC->s.number, MASK_MONSTERSOLID ); if ( movetrace.allsolid ||
movetrace.startsolid || movetrace.fraction < 1.0 ) if ( canmove( movetrace.ent )
) whichtrace = -1; else whichtrace = 0; break; case 4: VectorMA(
NPC->currentOrigin, -NPC->maxs[0], right, endpos ); gi.trace( &movetrace,
NPC->currentOrigin, NPC->mins, NPC->maxs, endpos, NPC->s.number,
MASK_MONSTERSOLID ); if (movetrace.allsolid || movetrace.startsolid ||
movetrace.fraction < 1.0 ) if ( canmove( movetrace.ent ) ) whichtrace = -1; else
                                whichtrace = 0;
        ...
        {
        case 1:
                VectorMA( NPC->client->ps.velocity, 200, forward,
NPC->client->ps.velocity ); if ( trace1.fraction >= 0.9 )
                {
...
//			NPC->avelocity[PITCH] = -300;
        ...
        case 2:
                VectorMA( NPC->client->ps.velocity, -200, forward,
NPC->client->ps.velocity ); if(trace2.fraction >= 0.9)
                {
//can't anymore, origin not in center of deathframe!
//			NPC->avelocity[PITCH] = 300;
                        NPC->client->ps.friction = 1.0;
        ...
        case 3:
                VectorMA( NPC->client->ps.velocity, 200, right,
NPC->client->ps.velocity ); if ( trace3.fraction >= 0.9 )
                {
//can't anymore, origin not in center of deathframe!
//			NPC->avelocity[ROLL] = -300;
        ...
        case 4:
                VectorMA( NPC->client->ps.velocity, -200, right,
NPC->client->ps.velocity ); if ( trace4.fraction >= 0.9 )
                {
//can't anymore, origin not in center of deathframe!
//			NPC->avelocity[ROLL] = 300;
                        NPC->client->ps.friction = 1.0;
...
static void DeadThink(void) {
  ...
  NPC->maxs[2] =
      NPC->client->renderInfo.eyePoint[2] - NPC->currentOrigin[2] + 4;
  if (NPC->maxs[2] < -8) {
    NPC->maxs[2] = -8;
  }
  if (NPC->maxs[2] >
      oldMaxs2) { // inflating maxs, make sure we're not inflating into solid
    gi.trace(&trace, NPC->currentOrigin, NPC->mins, NPC->maxs,
             NPC->currentOrigin, NPC->s.number, NPC->clipmask, (EG2_Collision)0,
             0);
    if (trace.allsolid) { // must be inflating
      NPC->maxs[2] = oldMaxs2;
    }
  }
  /*
  {
          ...
          {//not flying through the air
                  ...
                  {
                          NPC->mins[0] -= 1;
                          gi.trace (&trace, NPC->currentOrigin, NPC->mins,
  NPC->maxs, NPC->currentOrigin, NPC->s.number, NPC->clipmask ); if (
  trace.allsolid )
                          {
                                  NPC->mins[0] += 1;
                          }
...
void NPC_ApplyScriptFlags(void) {
  if (NPCInfo->scriptFlags & SCF_CROUCHED) {
    if (NPCInfo->charmedTime > level.time &&
        (ucmd.forwardmove || ucmd.rightmove)) { // ugh, if charmed and moving,
                                                // ignore the crouched command
    } else {
      ucmd.upmove = -127;
    }
  }
  ...
  */
  ...
  if ((NPCInfo->scriptFlags & SCF_ALT_FIRE) &&
      (ucmd.buttons & BUTTON_ATTACK)) { // Use altfire instead
...
void NPC_HandleAIFlags(void) {
  ...
  if (NPCInfo->scriptFlags & SCF_FLY_WITH_JET) {
    ...
    if (IsFlying) {
      ...
      if (IsInTheAir) {
        ...
        ground[2] -= 60.0f;
        gi.trace(&trace, NPC->currentOrigin, 0, 0, ground, NPC->s.number,
                 NPC->clipmask, (EG2_Collision)0, 0);

        IsInTheAir =
            (!trace.allsolid && !trace.startsolid && trace.fraction > 0.9f);
      }
  ...
  if (NPCInfo->aiFlags & NPCAI_LOST) { // Print that you need help!
    ...
    if ( showWaypoints )
    {
            Quake3Game()->DebugPrint(WL_WARNING, "%s can't navigate to target %s
    (my wp: %d, goal wp: %d)\n", NPC->targetname,
    NPCInfo->goalEntity->targetname, NPC->waypoint,
    NPCInfo->goalEntity->waypoint );
    }
    */

    if (NPCInfo->goalEntity &&
        NPCInfo->goalEntity == NPC->enemy) { // We can't nav to our enemy
  ...
  if (NPCInfo->greetingDebounceTime &&
      NPCInfo->greetingDebounceTime < level.time) {
    G_AddVoiceEvent(NPC, Q_irand(EV_VICTORY1, EV_VICTORY3),
                    Q_irand(2000, 4000));
...
void NPC_AvoidWallsAndCliffs(void) {
  /*
          ...
          if ( ucmd.upmove > 0 || NPC->client->ps.groundEntityNum ==
     ENTITYNUM_NONE )
          ...
          if ( fabs( AngleDelta( NPC->currentAngles[YAW], NPCInfo->desiredYaw )
     ) < 5.0 )//!ucmd.angles[YAW] )
          {//Not turning much, don't do this
                  //NOTE: Should this not happen only if you're not turning AT
     ALL?
                  //	You could be turning slowly but moving fast, so that
     would
                  //	still let you walk right off a cliff...
                  //NOTE: Or maybe it is a good idea to ALWAYS do this,
     regardless
                  //	of whether ot not we're turning?  But why would we be
     walking
                  //  straight into a wall or off	a cliff unless we really
     wanted to? return;
          }
          ...
          gi.trace( &trace, NPC->currentOrigin, mins, NPC->maxs, testPos,
     NPC->s.number, NPC->clipmask ); if ( trace.allsolid || trace.startsolid ||
     trace.fraction < 1.0 )
          ...
          gi.trace( &trace, trace.endpos, mins, NPC->maxs, testPos,
     NPC->s.number, NPC->clipmask ); if ( trace.allsolid || trace.startsolid ||
     trace.fraction < 1.0 )
          ...
          //going to fall at least 128, don't move, just turn... is this bad,
     though?  What if we want them to drop off? ucmd.forwardmove = 0;
          ucmd.rightmove = 0;
          return;
  */
}
...
void NPC_KeepCurrentFacing(void) {
  ...
  if (!ucmd.angles[PITCH]) {
    ucmd.angles[PITCH] = ANGLE2SHORT(client->ps.viewangles[PITCH]) -
...
qboolean
G_JediInNormalAI(gentity_t *ent) { // NOTE: should match above func's switch!
  // check our bState
...
void NPC_RunBehavior(int team, int bState) {
  ...
  if (bState == BS_CINEMATIC) {
    ...
    NPC_BSJedi_Default();
  } else if (NPC->client->NPC_class == CLASS_SABER_DROID) { // saber droid
    NPC_BSSD_Default();
  } else if (NPC->client->ps.weapon == WP_SABER) { // jedi
    NPC_BehaviorSet_Jedi(bState);
  } else if (NPC->client->NPC_class == CLASS_REBORN &&
             NPC->client->ps.weapon == WP_MELEE) { // force-only reborn
    NPC_BehaviorSet_Jedi(bState);
  ...
  } else if (NPC->client->NPC_class == CLASS_ROCKETTROOPER) { // bounty hunter
    ...
    G_CheckCharmed(NPC);
  } else if (NPCInfo->scriptFlags & SCF_FORCED_MARCH) { // being forced to march
    NPC_BSDefault();
    ...
    case TEAM_ENEMY:
      ...
      if (NPC->enemy && NPC->client->ps.weapon == WP_NONE &&
          bState != BS_HUNT_AND_KILL &&
          !Q3_TaskIDPending(
              NPC, TID_MOVE_NAV)) { // if in battle and have no weapon, run
                                    // away, fixme: when in BS_HUNT_AND_KILL,
                                    // they just stand there
        if (bState != BS_FLEE) {
          NPC_StartFlee(NPC->enemy, NPC->enemy->currentOrigin, AEL_DANGER_GREAT,
                        5000, 10000);
        } else {
          NPC_BSFlee();
        }
      ...
      if (NPC->client->ps.weapon == WP_SABER) { // special melee exception
        NPC_BehaviorSet_Default(bState);
        return;
      }
      if (NPC->client->ps.weapon == WP_DISRUPTOR &&
          (NPCInfo->scriptFlags & SCF_ALT_FIRE)) { // a sniper
        NPC_BehaviorSet_Sniper(bState);
        return;
      }
      if (NPC->client->ps.weapon == WP_THERMAL ||
          NPC->client->ps.weapon ==
              WP_MELEE) // FIXME: separate AI for melee fighters
      {                 // a grenadier
    ...
    case TEAM_NEUTRAL:
      ...
      if (NPC->client->NPC_class == CLASS_PROTOCOL) {
        NPC_BehaviorSet_Default(bState);
                 ...
                 NPC->client->NPC_class == CLASS_JAWA) { // others, too?
      ...
      // Add special vehicle behavior here.
        ...
        if (!pVehicle->m_pPilot && pVehicle->m_iBoarding == 0) {
          ...
          // TODO: The only other case were we want a vehicle to do something
          // specifically is perhaps in multiplayer where we want the shuttle to
          // be able to lift off when not occupied and in a landing zone.
        }
      } else {
...
static bState_t G_CurrentBState(gNPC_t *gNPC) {
  if (gNPC->tempBehavior !=
      BS_DEFAULT) { // Overrides normal behavior until cleared
...
void NPC_ExecuteBState(gentity_t *self) //, int msec )
{
  ...
  // Only process snapshot if independant and in combat mode- this would pick
  ...
  if (NPC->client->ps.saberLockTime &&
      NPC->client->ps.saberLockEnemy != ENTITYNUM_NONE) {
    ...
    if (NPC->enemy->flags & FL_DONT_SHOOT) {
      ...
      ucmd.buttons &= ~BUTTON_ALT_ATTACK;
               ...
               && (NPC->client->playerTeam != TEAM_FREE ||
                   (NPC->client->NPC_class == CLASS_TUSKEN &&
                    Q_irand(0, 4))) // not a rampaging creature or I'm a tusken
                                    // and I feel generous (temporarily)
               && NPC->enemy->NPC &&
               (NPC->enemy->NPC->surrenderTime > level.time ||
                (NPC->enemy->NPC->scriptFlags &
  ...
  if (!NPCInfo->attackHoldTime) {
    NPCInfo->last_ucmd.buttons &=
        ~(BUTTON_ATTACK | BUTTON_ALT_ATTACK |
          BUTTON_FORCE_FOCUS); // so we don't fire twice in one think
  }
  //============================================================================
  NPC_CheckAttackScript();
  NPC_KeepCurrentFacing();

  if (!NPC->next_roff_time ||
      NPC->next_roff_time < level.time) { // If we were following a roff, we
                                          // don't do normal pmoves.
  ...
  finishing it gi.Printf("LegsAnim: %s(%d) TorsoAnim: %s(%d)\n",
...
void NPC_CheckInSolid(void) {
  ...
  gi.trace(&trace, NPC->currentOrigin, NPC->mins, NPC->maxs, point,
  ...
  if (!trace.startsolid && !trace.allsolid) {
    ...
    if (VectorLengthSquared(NPCInfo->lastClearOrigin)) {
      //			gi.Printf("%s stuck in solid at %s:
...
void NPC_Think(gentity_t *self) //, int msec )
{
  ...
  if (self->health <= 0) {
    ...
    if (NPCInfo->nextBStateThink <= level.time) {
      if (self->m_iIcarusID != IIcarusInterface::ICARUS_INVALID &&
          !stop_icarus) {
        IIcarusInterface::GetIcarus()->Update(self->m_iIcarusID);
      }
    }
  ...
  // TODO! Tauntaun's (and other creature vehicles?) think, we'll need to make
  // an exception here to allow that.

  if (self->client && self->client->NPC_class == CLASS_VEHICLE &&
      self->NPC_type &&
      (!self->m_pVehicle->m_pVehicleInfo->Inhabited(
          self->m_pVehicle))) { // empty swoop logic
    if (self->owner) { // still have attached owner, check and see if can forget
                       // him (so he can use me later)
      vec3_t dir2owner;
      VectorSubtract(self->owner->currentOrigin, self->currentOrigin,
                     dir2owner);
      ...
      if (VectorLengthSquared(dir2owner) > 128 * 128 ||
          ...
          (DotProduct(self->client->ps.velocity,
                      oldOwner->client->ps.velocity) < -200.0f &&
           !G_BoundsOverlap(
               self->absmin, self->absmin, oldOwner->absmin,
               oldOwner->absmax))) { // all clear, become solid to our owner now
        gi.linkentity(self);
      } else { // blocked, retain owner
        self->owner = oldOwner;
      }
    }
  }
  if (player->client->ps.viewEntity ==
      self->s.number) { // being controlled by player
  ...
  if (NPCInfo->nextBStateThink <= level.time) {
...
#endif                               //	AI_TIMERS
    if (NPC->s.eType != ET_PLAYER) { // Something drastic happened in our script
      return;
    }

    if (NPC->s.weapon == WP_SABER && g_spskill->integer >= 2 &&
        NPCInfo->rank > RANK_LT_JG) { // Jedi think faster on hard difficulty,
                                      // except low-rank (reborn)
    ...
    if (addTime > 50) {
      gi.Printf(S_COLOR_RED "ERROR: NPC number %d, %s %s at %s, weaponnum: %d, "
                            "using %d of AI time!!!\n",
                NPC->s.number, NPC->NPC_type, NPC->targetname,
                vtos(NPC->currentOrigin), NPC->s.weapon, addTime);
    }
    AITime += addTime;
#endif //	AI_TIMERS
  } else {
    if (NPC->client && NPC->client->NPC_class == CLASS_ROCKETTROOPER &&
        ...
        NPC->client->ps.groundEntityNum == ENTITYNUM_NONE) { // reduce velocity
...
void NPC_InitAI(void) {
  ...
  d_slowmodeath =
      gi.cvar("d_slowmodeath", "3", CVAR_ARCHIVE); // save this setting
...
void NPC_InitAnimTable(void) {
  for (int i = 0; i < MAX_ANIM_FILES; i++) {
    for (int j = 0; j < MAX_ANIMATIONS; j++) {
      ...
      //			level.knownAnimFileSets[i].animations[j].initialLerp
      //= 100;
...
extern int G_ParseAnimFileSet(const char *skeletonName,
                              const char *modelName = 0);
void NPC_InitGame(void) {
  //	globals.NPCs = (gNPC_t *) gi.TagMalloc(game.maxclients *
  //sizeof(game.bots[0]), TAG_GAME);
...
void NPC_SetAnim(gentity_t *ent, int setAnimParts, int anim, int setAnimFlags,
                 int iBlend) { // FIXME : once torsoAnim and legsAnim are in the
                               // same structure for NCP and Players
  ...
  if (ent->health > 0) { // don't lock anims if the guy is dead
  ...
  if (ent->client) { // Players, NPCs
    ...
    PM_SetAnimFinal(&ent->client->ps.torsoAnim, &ent->client->ps.legsAnim,
                    setAnimParts, anim, setAnimFlags,
                    &ent->client->ps.torsoAnimTimer,
                    &ent->client->ps.legsAnimTimer, ent, iBlend);
  } else { // bodies, etc.
    if (setAnimFlags & SETANIM_FLAG_OVERRIDE) {
      if (setAnimParts & SETANIM_TORSO) {
    ...
    PM_SetAnimFinal(&ent->s.torsoAnim, &ent->s.legsAnim, setAnimParts, anim,
                    setAnimFlags, &ent->s.torsoAnimTimer, &ent->s.legsAnimTimer,
                    ent);
  }
}
