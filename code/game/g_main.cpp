...
static void ClearAllInUse(void) { memset(g_entityInUseBits, 0, sizeof(g_entityInUseBits)); }
...
void WriteInUseBits() {
	...
	saved_game.write_chunk<uint32_t>(INT_ID('I', 'N', 'U', 'S'), ::g_entityInUseBits);
}

void ReadInUseBits() {
	ojk::SavedGameHelper saved_game(::gi.saved_game);

	saved_game.read_chunk<uint32_t>(INT_ID('I', 'N', 'U', 'S'), ::g_entityInUseBits);
...
static void G_DynamicMusicUpdate(void) {
	...
	for (e = 0; e < numListedEntities; e++) {
		...
		if (!ent->client || !ent->NPC) {
			if (ent->classname && (!Q_stricmp("PAS", ent->classname) || !Q_stricmp("misc_turret", ent->classname))) { // a turret
		...
		if (entTeam == TEAM_NEUTRAL && (!ent->enemy || !ent->enemy->client ||
										ent->enemy->client->playerTeam != player->client->playerTeam)) { // a droid that is not mad at me or my allies
			continue;
		}

		if (!gi.inPVS(player->currentOrigin, ent->currentOrigin)) { // not potentially visible
			continue;
		}

		if (ent->client && ent->s.weapon == WP_NONE) { // they don't have a weapon... FIXME: only do this for droids?
			continue;
		}

		LOScalced = clearLOS = qfalse;
		if ((ent->enemy == player && (!ent->NPC || ent->NPC->confusionTime < level.time)) || (ent->client && ent->client->ps.weaponTime) ||
			(!ent->client && ent->attackDebounceTime > level.time)) { // mad
			if (ent->health > 0) {									  // alive
				// FIXME: do I really need this check?
				if (ent->s.weapon == WP_SABER && ent->client && !ent->client->ps.SaberActive() &&
					ent->enemy != player) { // a Jedi who has not yet gotten made at me
					continue;
				}
				if (ent->NPC && ent->NPC->behaviorState == BS_CINEMATIC) { // they're not actually going to do anything about being mad at me...
					continue;
				}
				// okay, they're in my PVS, but how close are they?  Are they actively attacking me?
				if (!ent->client && ent->s.weapon == WP_TURRET && ent->fly_sound_debounce_time &&
					ent->fly_sound_debounce_time - level.time < 10000) { // a turret that shot at me less than ten seconds ago
				} else if (ent->client && ent->client->ps.lastShotTime &&
						   ent->client->ps.lastShotTime - level.time < 10000) { // an NPC that shot at me less than ten seconds ago
		...
		if (level.dmState == DM_EXPLORE) { // only do these visibility checks if you're still in exploration mode
			if (!InFront(ent->currentOrigin, player->currentOrigin, player->client->ps.viewangles, 0.0f)) { // not in front
				continue;
			}
		...
		if (ent->health <= 0) {										// dead
			if (!ent->client || level.time - ent->s.time > 10000) { // corpse has been dead for more than 10 seconds
	...
	if (!battle) { // no active enemies, but look for missiles, shot impacts, etc...
		int alert = G_CheckAlertEvents(player, qtrue, qtrue, 1024, 1024, -1, qfalse, AEL_SUSPICIOUS);
		if (alert != -1) {						   // FIXME: maybe tripwires and other FIXED things need their own sound, some kind of danger/caution theme
				...
				battle = 1;
					   ...
					   (level.alertEvents[alert].owner == player->enemy ||
						(level.alertEvents[alert].owner->client &&
						 level.alertEvents[alert].owner->client->playerTeam == player->client->enemyTeam))) { // NPC on enemy team of player made some noise
	...
	if (battle) { // battle - this can interrupt level.dmDebounceTime of lower intensity levels
		// play battle
		...
		*/
		{									   // still nothing dangerous going on
			if (level.dmState != DM_EXPLORE) { // just went to explore, hold it for a couple seconds at least
...
void InitGame(const char *mapname, const char *spawntarget, int checkSum, const char *entities, int levelTime, int randomSeed, int globalTime,
			  SavedGameJustLoaded_e eSavedGameJustLoaded, qboolean qbLoadTransition) {
...
static void G_CheckTasksCompleted(gentity_t *ent) {
	...
	if (Q3_TaskIDPending(ent, TID_LOCATION)) {
		...
		if (currentLoc && currentLoc[0] && Q_stricmp(ent->message, currentLoc) == 0) { // we're in the desired location
...
static void G_CheckSpecialPersistentEvents(gentity_t *ent) { // special-case alerts that would be a pain in the ass to have the ent's think funcs generate
	if (ent == NULL) {
		return;
	}
	...
	if (ent->contents == CONTENTS_LIGHTSABER && !Q_stricmp("lightsaber", ent->classname)) { // lightsaber
...
static void G_Animate(gentity_t *self) {
	...
	if (self->s.frame == self->endFrame) {
		if (self->svFlags & SVF_ANIMATING) {
			...
			if (self->ghoul2.size()) {
				...
				gi.G2API_GetBoneAnimIndex(&self->ghoul2[self->playerModel], self->rootBone, (cg.time ? cg.time : level.time), &frame, &junk, &junk, &junk,
										  &junk2, NULL);
	...
	if (self->ghoul2.size()) {
		...
		gi.G2API_SetBoneAnimIndex(&self->ghoul2[self->playerModel], self->rootBone, self->startFrame, self->endFrame, BONE_ANIM_OVERRIDE_FREEZE, 1.0f, cg.time,
								  -1, -1);
		return;
	}
...
static void G_BodyDragUpdate(gentity_t *ent, gentity_t *dragger) {
	...
	assert(ent && ent->inuse && ent->client && ent->ghoul2.size() && dragger && dragger->inuse && dragger->client && dragger->ghoul2.size());
...
static int G_RagAnimForPositioning(gentity_t *ent) {
	...
	gi.G2API_GetBoltMatrix(ent->ghoul2, ent->playerModel, ent->crotchBolt, &matrix, G2Angles, ent->client->ps.origin, (cg.time ? cg.time : level.time), NULL,
						   ent->s.modelScale);
...
static inline qboolean G_RagWantsHumanoidsOnly(CGhoul2Info *ghlInfo) {
	...
	if (!Q_stricmp("models/players/_humanoid/_humanoid", GLAName)) { // only _humanoid skeleton is expected to have these
		return qtrue;
	}
...
qboolean G_RagDoll(gentity_t *ent, vec3_t forcedAngles) {
	...
	if (!ent || !ent->inuse || !ent->client || ent->health > 0 || ent->client->noRagTime >= level.time || ent->client->noRagTime == -1 ||
		(ent->s.powerups & (1 << PW_DISRUPTION)) || !ent->e_DieFunc || ent->playerModel < 0 || !ent->ghoul2.size() ||
	...
	if (!ent->client->isRagging) {							   // If we're not in a ragdoll state, perform the checks.
		if (ent->client->ps.heldByClient <= ENTITYNUM_WORLD) { // want to rag no matter what then
			inSomething = qtrue;
			...
			if (VectorNormalize(cVel) > 400) { // if he's flying through the air at a good enough speed, switch into ragdoll
				inSomething = qtrue;
			}
		}
		...
		if (!inSomething) {
			...
			if (ent->client->ps.legsAnimTimer <= 0) { // Looks like the death anim is done playing
													  // deathDone = qtrue;
			}
			...
			gi.G2API_GetBoltMatrix(ent->ghoul2, ent->playerModel, boltChecks[2], &boltMatrix, tAng, ent->client->ps.origin, (cg.time ? cg.time : level.time),
			...
			while (i < 5) {
				if (i < 2) { // when doing hands, trace to the head instead of origin
					gi.G2API_GetBoltMatrix(ent->ghoul2, ent->playerModel, boltChecks[i], &boltMatrix, tAng, ent->client->ps.origin,
				...
				if (tr.fraction != 1.0 || tr.startsolid || tr.allsolid) { // Hit something or start in solid, so flag it and break.
																		  // This is a slight hack, but if we aren't done with the death anim, we don't really
																		  // want to go into ragdoll unless our body has a relatively "flat" pitch.
#if 0
					vec3_t vSub;
	...
	if (ent->client->isRagging) { // We're in a ragdoll state, so make the call to keep our positions updated and whatnot.
		...
		*/
...
#if 1
		{
			...
			if (gi.G2API_GetBoneAnim(&ent->ghoul2[0], "model_root", (cg.time ? cg.time : level.time), &currentFrame, &startFrame, &endFrame, &flags, &animSpeed,
									 NULL)) { // lock the anim on the current frame.
				int blendTime = 500;

				gi.G2API_SetBoneAnim(&ent->ghoul2[0], "lower_lumbar", currentFrame, currentFrame + 1, flags, animSpeed, (cg.time ? cg.time : level.time),
									 currentFrame, blendTime);
				gi.G2API_SetBoneAnim(&ent->ghoul2[0], "model_root", currentFrame, currentFrame + 1, flags, animSpeed, (cg.time ? cg.time : level.time),
									 currentFrame, blendTime);
				gi.G2API_SetBoneAnim(&ent->ghoul2[0], "Motion", currentFrame, currentFrame + 1, flags, animSpeed, (cg.time ? cg.time : level.time),
									 currentFrame, blendTime);
			}
		}
#endif

		gi.G2API_SetBoneAngles(&ent->ghoul2[ent->playerModel], "upper_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, NULL, 100,
							   (cg.time ? cg.time : level.time));
		gi.G2API_SetBoneAngles(&ent->ghoul2[ent->playerModel], "lower_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, NULL, 100,
							   (cg.time ? cg.time : level.time));
		gi.G2API_SetBoneAngles(&ent->ghoul2[ent->playerModel], "thoracic", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, NULL, 100,
							   (cg.time ? cg.time : level.time));
		gi.G2API_SetBoneAngles(&ent->ghoul2[ent->playerModel], "cervical", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, NULL, 100,
							   (cg.time ? cg.time : level.time));
		...
		if (ent->client->ps.heldByClient <= ENTITYNUM_WORLD) {
			...
			if (grabEnt->client && grabEnt->ghoul2.size()) {
				...
				if (ent->client->ragLastOriginTime >= level.time && ent->client->ps.groundEntityNum != ENTITYNUM_NONE) { // make sure it's reasonably updated
					difLen = VectorLength(pDif);
					if (difLen > 0.0f) { // if we're being dragged, then kick all the bones around a bit
						vec3_t dVel;
						vec3_t rVel;
						int i = 0;
		...
		} else if (ent->client->overridingBones) { // reset things to their normal rag state
			vec3_t pcjMin, pcjMax;
			vec3_t dVel;
...
void G_RunFrame(int levelTime) {
	...
	for (i = 0; i < globals.num_entities; i++) {
		...
		if (!ent->client) {
			if (!(ent->svFlags & SVF_SELF_ANIMATING)) { // FIXME: make sure this is done only for models with frames?
		...
		if (i == 0) {
			...
			// dead
			if (ent->health <= 0) {
				if (ent->client->ps.groundEntityNum != ENTITYNUM_NONE) { // on the ground
