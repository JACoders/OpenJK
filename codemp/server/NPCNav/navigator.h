...
public:
  ...
  void GetPosition(vec3_t position) const {
    if (position)
      VectorCopy(m_position, position);
  }
...
public:
  ...
  int GetBestNodeAltRoute(int startID, int endID, int *pathCost,
                          int rejectID = NODE_NONE);
  int GetBestNodeAltRoute(int startID, int endID, int rejectID = NODE_NONE);
  int GetBestPathBetweenEnts(sharedEntity_t *ent, sharedEntity_t *goal,
                             int flags);
...
protected:
  int TestNodePath(sharedEntity_t *ent, int okToHitEntNum, vec3_t position,
...
#if __NEWCOLLECT
  int CollectNearestNodes(vec3_t origin, int radius, int maxCollect,
  ...
  void AddNodeEdges(CNode *node, int addDist, edge_l &edgeList,
                    bool *checkedNodes);

  void CalculatePath(CNode *node);

  // rww - made failedEdges private as it doesn't seem to need to be public.
  // And I'd rather shoot myself than have to devise a way of setting/accessing
  // this array via trap calls.
