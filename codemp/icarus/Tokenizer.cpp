...
keywordArray_t CTokenizer::directiveKeywords[] = {
	"include", DIR_INCLUDE, "ifdef",  DIR_IFDEF,  "ifndef",	  DIR_IFNDEF,	"endif", DIR_ENDIF,
	"else",	   DIR_ELSE,	"define", DIR_DEFINE, "undefine", DIR_UNDEFINE, "",		 TK_EOF,
};
...
void CSymbolTable::RemoveSymbol(LPCTSTR symbolName) { m_symbols.erase(symbolName); }
...
bool CParseDefine::IsThisDefinition(void *theDefinition) { return (CDirectiveSymbol *)theDefinition == m_defineSymbol; }
...
void CTokenizer::SetErrorProc(LPTokenizerErrorProc errorProc) { m_errorProc = errorProc; }

void CTokenizer::SetAdditionalErrors(keywordArray_t *theErrors) { m_errors = theErrors; }
		...
		if (NextChar((byte &)theString[i])) {
			if (((theString[i] != '_') || ((m_flags & TKF_NOUNDERSCOREINIDENTIFIER) == 0)) &&
				...
				if (((theString[i] >= 'A') && (theString[i] <= 'Z')) || ((theString[i] >= 'a') && (theString[i] <= 'z')) ||
					((theString[i] >= '0') && (theString[i] <= '9')) || (theString[i] == '_') || (theString[i] == '-')) {
					continue;
				}
			}
			PutBackChar(theString[i]);
	...
	if (lastLookup == NULL) {
		...
		if ((m_flags & TKF_WIDEUNDEFINEDSYMBOLS) != 0) {
			while (true) {
				...
				if (((theByte >= 'a') && (theByte <= 'z')) || ((theByte >= 'A') && (theByte <= 'Z')) || ((theByte >= '0') && (theByte <= '9'))) {
					PutBackChar(theByte);
					break;
				}
				if (theByte < ' ') {
