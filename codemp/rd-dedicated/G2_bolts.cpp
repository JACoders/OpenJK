...
//=====================================================================================================================
// Bolt List handling routines - so entities can attach themselves to any part
...
int G2_Find_Bolt_Surface_Num(boltInfo_v &bltlist, const int surfaceNum,
  ...
  for (size_t i = 0; i < bltlist.size(); i++) {
    ...
    if ((bltlist[i].surfaceNumber == surfaceNum) &&
        ((bltlist[i].surfaceType & flags) == flags)) {
      return i;
    }
  }
...
int G2_Add_Bolt_Surf_Num(CGhoul2Info *ghlInfo, boltInfo_v &bltlist,
  ...
  for (size_t i = 0; i < bltlist.size(); i++) {
    ...
    if (bltlist[i].boneNumber == -1 && bltlist[i].surfaceNumber == -1) {
      // if we found an entry that had a -1 for the bone / surface number, then
      // we hit a surface / bone slot that was empty
  ...
  // ok, we didn't find an existing surface of that name, or an empty slot. Lets
  // add an entry
...
// set the bolt list to all unused so the bone transformation routine ignores
// it.
void G2_Init_Bolt_List(boltInfo_v &bltlist) { bltlist.clear(); }

// remove any bolts that reference original surfaces, generated surfaces, or
// bones that aren't active anymore
    ...
    if ((bltlist[i].surfaceNumber != -1) || (bltlist[i].boneNumber != -1)) {
      ...
      if (bltlist[i].surfaceNumber != -1) {
        ...
        if (bltlist[i].surfaceType) {
          ...
          if (!G2_FindOverrideSurface(bltlist[i].surfaceNumber, slist)) {
            // no - we want to remove this bolt, regardless of how many people
            // are using it
            bltlist[i].boltUsed = 1;
      ...
      // no, must be looking at a bone then
        ...
        if (!activeBones[bltlist[i].boneNumber]) {
          // no - we want to remove this bolt, regardless of how many people are
          // using it
          bltlist[i].boltUsed = 1;
          G2_Remove_Bolt(bltlist, i);
        }
