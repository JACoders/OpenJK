...
#define RF_CAP_FRAMES                                                          \
  0x00400 // cap the model frames by the maxframes for one shot anims

#define RF_ALPHA_FADE                                                          \
  0x00800 // hacks blend mode and uses whatever the set alpha is.
#define RF_PULSATE                                                             \
  0x01000 // for things like a dropped saber, where we want to add an extra
          // visual clue
...
#define RF_SETANIMINDEX                                                        \
  0x20000 // use backEnd.currentEntity->e.skinNum for R_BindAnimatedImage

#define RF_DISINTEGRATE1 0x40000 // does a procedural hole-ripping thing.
#define RF_DISINTEGRATE2                                                       \
  0x80000 // does a procedural hole-ripping thing with scaling at the ripping
          // point

#define RF_G2MINLOD 0x100000 // force Lowest lod on g2

#define RF_SHADOW_ONLY                                                         \
  0x200000 // add surfs for shadowing but don't draw them normally -rww
...
typedef struct {
  ...
  // This doesn't have to be unioned, but it does make for more meaningful
  // variable names :)
  union {
    float rotation;
    float endTime;
  ...
  */
  ...
  CGhoul2Info_v *ghoul2; // has to be at the end of the ref-ent in order for it
...
*/
typedef enum { TC_NONE, TC_S3TC, TC_S3TC_DXT } textureCompression_t;
