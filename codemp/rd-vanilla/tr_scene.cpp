...
void RE_AddPolyToScene(qhandle_t hShader, int numVerts, const polyVert_t *verts,
  ...
  if (!hShader) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "WARNING: RE_AddPolyToScene: NULL poly shader\n");
    return;
  }
    ...
    if (r_numpolyverts + numVerts >= max_polyverts || r_numpolys >= max_polys) {
      ...
      */
      ri.Printf(PRINT_DEVELOPER,
                S_COLOR_YELLOW "WARNING: RE_AddPolyToScene: r_max_polys or "
...
void RE_AddRefEntityToScene(const refEntity_t *ent) {
  ...
  if (r_numentities >= MAX_REFENTITIES) {
    ri.Printf(PRINT_DEVELOPER, "RE_AddRefEntityToScene: Dropping refEntity, "
                               "reached MAX_REFENTITIES\n");
    return;
  }

  /*if ( Q_isnan(ent->origin[0]) || Q_isnan(ent->origin[1]) ||
  Q_isnan(ent->origin[2]) ) { static qboolean firstTime = qtrue; if (firstTime)
  { firstTime = qfalse; ri.Printf( PRINT_WARNING, "RE_AddRefEntityToScene passed
  a refEntity which has an origin with a NaN component\n");
          }
          return;
  }*/
  ...
  if (ent->reType == RT_ENT_CHAIN) { // minirefents must die.
    return;
  }
  ...
  if (ent->reType == RT_ENT_CHAIN)
  {
          ...
          backEndData->entities[r_numentities].e.uRefEnt.uMini.miniStart =
  r_numminientities - r_firstSceneMiniEntity;
...
/************************************************************************************************
 * RE_AddMiniRefEntityToScene * Adds a mini ref ent to the scene.  If the input
 *parameter is null, it signifies the end   * of the chain.  Otherwise, if there
 *is a valid chain parent, it will be added to that.     * If there is no
 *parent, it will be added as a regular ref ent.                             *
 *                                                                                              *
 * Input * ent: the mini ref ent to be added *
 *                                                                                              *
 * Output / Return * none *
 *                                                                                              *
 ************************************************************************************************/
void RE_AddMiniRefEntityToScene(const miniRefEntity_t *ent) {
  ...
  if (ent->reType < 0 || ent->reType >= RT_MAX_REF_ENTITY_TYPE) {
    Com_Error(ERR_DROP, "RE_AddMiniRefEntityToScene: bad reType %i",
              ent->reType);
  }

  if (!r_numentities || refEntParent == -1 ||
      r_numminientities >= MAX_MINI_ENTITIES) { // rww - add it as a refent also
                                                // if we run out of minis
    //		Com_Error( ERR_DROP, "RE_AddMiniRefEntityToScene: mini without parent
    //ref ent");
    refEntity_t tempEnt;

    memcpy(&tempEnt, ent, sizeof(*ent));
    memset(((char *)&tempEnt) + sizeof(*ent), 0,
           sizeof(tempEnt) - sizeof(*ent));
    RE_AddRefEntityToScene(&tempEnt);
    return;
  }
...
void RE_RenderScene(const refdef_t *fd) {
  ...
  tr.frontEndMsec +=
      ri.Milliseconds() * ri.Cvar_VariableValue("timescale") - startTime;
