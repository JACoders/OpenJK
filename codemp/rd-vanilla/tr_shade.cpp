	...
	for (l = 0; l < backEnd.refdef.num_dlights; l++) {
		...
		if (tess.shader && qglActiveTextureARB) {
			...
			while (i < tess.shader->numUnfoggedPasses) {
				const int blendBits = (GLS_SRCBLEND_BITS + GLS_DSTBLEND_BITS);
				if (((tess.shader->stages[i].bundle[0].image && !tess.shader->stages[i].bundle[0].isLightmap && !tess.shader->stages[i].bundle[0].numTexMods) ||
					 (tess.shader->stages[i].bundle[1].image && !tess.shader->stages[i].bundle[1].isLightmap &&
...
static void ComputeTexCoords(shaderStage_t *pStage) {
	...
	for (b = 0; b < NUM_TEXTURE_BUNDLES; b++) {
		...
		for (tm = 0; tm < pStage->bundle[b].numTexMods; tm++) {
			...
			case TMOD_SCALE:
				RB_CalcScaleTexCoords(pStage->bundle[b].texMods[tm].translate, (float *)tess.svars.texcoords[b]);
				break;
			...
			case TMOD_ROTATE:
				RB_CalcRotateTexCoords(pStage->bundle[b].texMods[tm].translate[0], (float *)tess.svars.texcoords[b]);
				break;
		...
		if (pStage->bundle[1].image != 0) {
			...
			if (tess.shader == tr.distortionShader && glConfig.stencilBits >= 4) { // draw it to the stencil buffer!
				...
				if (backEnd.currentEntity->e.renderfx &
					RF_ALPHA_DEPTH) { // depth write, so faces through the model will be stomped over by nearer ones. this works because
