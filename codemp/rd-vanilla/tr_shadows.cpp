...
void RB_DoShadowTessEnd(vec3_t lightPos) {
	...
	for (i = 0; i < numTris; i++) {
		...
		if (!lightPos) {
			...
			planeEq[3] = -(v1[0] * (v2[1] * v3[2] - v3[1] * v2[2]) + v2[0] * (v3[1] * v1[2] - v1[1] * v3[2]) + v3[0] * (v1[1] * v2[2] - v2[1] * v1[2]));

			d = planeEq[0] * lightPos[0] + planeEq[1] * lightPos[1] + planeEq[2] * lightPos[2] + planeEq[3];
		}
...
void RB_DistortionFill(void) {
	...
	if (tr_distortionAlpha == 1.0f && tr_distortionStretch == 0.0f) { // no overrides
