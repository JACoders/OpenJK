...
extern void SetMiscModelDefaults(gentity_t *ent, useFunc_t use_func,
                                 const char *material, int solid_mask,
                                 int animFlag, qboolean take_damage,
                                 qboolean damage_model);
...
q3map_onlyvertexlighting 1	=	brush only gets vertex lighting (reduces
bsp size!)
*/
...
LIGHT - If this info_null is only targeted by a non-switchable light (a light
without a targetname), it does NOT spawn in at all and doesn't count towards the
# of entities on the map, even at map spawn/load
*/
void SP_info_null(gentity_t *self) {
  if ((self->spawnflags & 1)) { // only used as a light target, so bugger off
    G_FreeEntity(self);
    return;
  }
  // FIXME: store targetname and vector (origin) in a list for further
  // reference... remove after 1st second of game?
...
point. "scale" multiplier for the light intensity - does not affect size
(default 1) greater than 1 is brighter, between 0 and 1 is dimmer. "color" sets
the light's color "targetname" to indicate a switchable light - NOTE that all
lights with the same targetname will be grouped together and act as one light
(ie: don't mix colors, styles or start_off flag) "style" to specify a specify
light style, even for switchable lights! "style_off" light style to use when
switched off (Only for switchable lights)
...
static void misc_lightstyle_set(gentity_t *ent) {
  ...
  if (!ent->misc_dlight_active) { // turn off
    if (mLightOffStyle) // i have a light style i'd like to use when off
    {
      char lightstyle[32];
      gi.GetConfigstring(CS_LIGHT_STYLES + (mLightOffStyle * 3) + 0, lightstyle,
                         32);
      gi.SetConfigstring(CS_LIGHT_STYLES + (mLightStyle * 3) + 0, lightstyle);

      gi.GetConfigstring(CS_LIGHT_STYLES + (mLightOffStyle * 3) + 1, lightstyle,
                         32);
      gi.SetConfigstring(CS_LIGHT_STYLES + (mLightStyle * 3) + 1, lightstyle);

      gi.GetConfigstring(CS_LIGHT_STYLES + (mLightOffStyle * 3) + 2, lightstyle,
                         32);
    ...
    {
      char lightstyle[32];
      gi.GetConfigstring(CS_LIGHT_STYLES + (mLightSwitchStyle * 3) + 0,
                         lightstyle, 32);
      gi.SetConfigstring(CS_LIGHT_STYLES + (mLightStyle * 3) + 0, lightstyle);

      gi.GetConfigstring(CS_LIGHT_STYLES + (mLightSwitchStyle * 3) + 1,
                         lightstyle, 32);
      gi.SetConfigstring(CS_LIGHT_STYLES + (mLightStyle * 3) + 1, lightstyle);

      gi.GetConfigstring(CS_LIGHT_STYLES + (mLightSwitchStyle * 3) + 2,
                         lightstyle, 32);
      gi.SetConfigstring(CS_LIGHT_STYLES + (mLightStyle * 3) + 2, lightstyle);
    } else {
...
void SP_light(gentity_t *self) {
  if (!self->targetname) { // if i don't have a light style switch, the i go
                           // away
    G_FreeEntity(self);
    return;
  }
...
void TeleportMover(gentity_t *mover, vec3_t origin, vec3_t diffAngles,
                   qboolean snapAngle) { // FIXME: need an effect
  ...
  if (snapAngle) { // not a diffAngle, actually an absolute angle
    vec3_t dir;
...
turns into BSP triangles - not solid by default (click SOLID or use _clipmodel
shader)
*/
...
*/
extern void CG_CreateMiscEntFromGent(gentity_t *ent, const vec3_t scale,
                                     float zOff); // cg_main.cpp
...
void cycleCamera(gentity_t *self) {
  ...
  if (self->owner == NULL) {
    ...
    self->owner = G_Find(self->owner, FOFS(targetname), self->target);
    if (self->owner == NULL) { // still didn't find one
...
void locateCamera(gentity_t *ent) { // FIXME: make this fadeout with distance
                                    // from misc_camera_portal
  ...
  if (!ent->targetname) { // not targetted, so auto-cycle
    if (G_Find(ent->owner, FOFS(targetname), ent->target) !=
        NULL) { // targeted at more than one thing
...
wait - makes it auto-cycle between all cameras it's pointed at at intevervals of
...
The target for a misc_portal_surface.  You can set either angles or target
another entity (NOT an info_null) to determine the direction of view. "roll" an
angle modifier to orient the camera around the target vector;
*/
...
void G_SubBSPSpawnEntitiesFromString(const char *entityString, vec3_t posOffset,
                                     vec3_t angOffset);
...
200) terxels - integer number of terxels on a patch side (default 4) (2 <= count
<= 8) seed - integer seed for random terrain generation (default 0) textureScale
- float scale of texture (default 0.005) heightmap - name of heightmap data
image to use, located in heightmaps/xxx.png. (must be PNG format) terrainDef -
defines how the game textures the terrain (file is base/ext_data/rmg/xxx.terrain
- default is grassyhills) instanceDef - defines which bsp instances appear
miscentDef - defines which client models spawn on the terrain (file is
base/ext_data/rmg/xxx.miscents) densityMap - how dense the client models are
packed

*/
void SP_terrain(gentity_t *ent) { G_FreeEntity(ent); }

// rww - Called by skyportal entities. This will check through entities and flag
// them as portal ents if they are in the same pvs as a skyportal entity and pass
// a direct point trace check between origins. I really wanted to use an eFlag
// for flagging portal entities, but too many entities like to reset their
// eFlags. Note that this was not part of the original wolf sky portal stuff.
void G_PortalifyEntities(gentity_t *ent) {
  ...
  while (i < MAX_GENTITIES) {
    ...
    if (scan && scan->inuse && scan->s.number != ent->s.number &&
        gi.inPVS(ent->s.origin, scan->currentOrigin)) {
      trace_t tr;

      gi.trace(&tr, ent->s.origin, vec3_origin, vec3_origin,
               scan->currentOrigin, ent->s.number, CONTENTS_SOLID, G2_NOCOLLIDE,
               0);

      if (tr.fraction == 1.0 ||
          (tr.entityNum == scan->s.number && tr.entityNum != ENTITYNUM_NONE &&
  ...
  ent->e_ThinkFunc =
      thinkF_G_FreeEntity; // the portal entity is no longer needed because its
                           // information is stored in a config string.
...
void SP_misc_skyportal(gentity_t *ent) {
  ...
  gi.SetConfigstring(CS_SKYBOXORG,
                     va("%.2f %.2f %.2f %i %.2f %.2f %.2f %i %i",
                        ent->s.origin[0], ent->s.origin[1], ent->s.origin[2],
                        isfog, fogv[0], fogv[1], fogv[2], fogn, fogf));

  ent->e_ThinkFunc = thinkF_G_PortalifyEntities;
  ent->nextthink =
      level.time +
      1050; // give it some time first so that all other entities are spawned.
}

extern qboolean G_ClearViewEntity(gentity_t *ent);
extern void G_SetViewEntity(gentity_t *self, gentity_t *viewEntity);
extern void SP_fx_runner(gentity_t *ent);
void camera_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
...
void camera_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  if (!activator || !activator->client ||
      activator->s.number) { // really only usable by the player
    return;
  }
  self->painDebounceTime =
      level.time +
      (self->wait *
       1000); // FRAMETIME*5;//don't check for player buttons for 500 ms

  // FIXME: I guess we are allowing them to switch to a dead camera.  Maybe we
  // should conditionally do this though?
  if (/*self->health <= 0 ||*/ (
      player && player->client &&
      player->client->ps.viewEntity ==
          self->s
              .number)) { // I'm already viewEntity, or I'm destroyed, find next
    gentity_t *next = NULL;
    if (self->target2 != NULL) {
      next = G_Find(NULL, FOFS(targetname), self->target2);
    }
    if (next) { // found another one
      if (!Q_stricmp("misc_camera",
                     next->classname)) { // make sure it's another camera
...
void camera_aim(gentity_t *self) {
  ...
  if (player && player->client &&
      player->client->ps.viewEntity == self->s.number) { // I am the viewEntity
    if (player->client->usercmd.forwardmove ||
        player->client->usercmd.rightmove ||
        player->client->usercmd.upmove) { // player wants to back out of camera
      G_UseTargets2(self, player, self->target4);
      G_ClearViewEntity(player);
      G_Sound(player, self->soundPos2);
      self->painDebounceTime =
          ...
          (self->wait *
           1000); // FRAMETIME*5;//don't check for player buttons for 500 ms
      if (player->client->usercmd.upmove >
          0) { // stop player from doing anything for a half second after
        player->aimDebounceTime = level.time + 500;
      }
    } else if (self->painDebounceTime < level.time) { // check for use button
      if ((player->client->usercmd.buttons &
           BUTTON_USE)) { // player pressed use button, wants to cycle to next
        camera_use(self, player, player);
      }
    } else { // don't draw me when being looked through
      self->s.eFlags |= EF_NODRAW;
      self->s.modelindex = 0;
    }
  } else if (self->health > 0) { // still alive, can draw me again
    self->s.eFlags &= ~EF_NODRAW;
  ...
  if (self->target) {
    ...
    if (targ) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        self->s.apos.trDelta[i] =
            AngleNormalize180((angles[i] - self->currentAngles[i]) * 10);
      }
      ...
      if (DistanceSquared(self->currentAngles, self->lastAngles) >
          0.01f) // if it moved at all, start a loop sound? not exactly the
                 // "bestest" solution
      {
...
A model in the world that can be used by the player to look through it's
viewpoint

There will be a video overlay instead of the regular HUD and the FOV will be
wider

VULNERABLE - allow camera to be destroyed

"target" - camera will remain pointed at this entity (if it's a train or some
other moving object, it will keep following it) "target2" - when player is in
this camera and hits the use button, it will cycle to this next camera (if no
target2, returns to normal view ) "target3" - thing to use when player enters
this camera view "target4" - thing to use when player leaves this camera view
...
void SP_misc_camera(gentity_t *self) {
  ...
  if (base) {
    base->s.modelindex =
        G_ModelIndex("models/map_objects/kejim/impcam_base.md3");
  ...
  self->s.modelindex3 = self->s.modelindex =
      G_ModelIndex("models/map_objects/kejim/impcam.md3");
...
*/
void SP_shooter_plasma(gentity_t *ent) { InitShooter(ent, WP_BRYAR_PISTOL); }
...
/*QUAKED object_cargo_barrel1 (1 0 0) (-16 -16 -16) (16 16 29) SMALLER KLINGON
NO_SMOKE POWDERKEG Cargo Barrel if given a targetname, using it makes it explode
...
void SP_object_cargo_barrel1(gentity_t *ent) {
  if (ent->spawnflags & 8) {
    ent->s.modelindex =
        G_ModelIndex("/models/mapobjects/cargo/barrel_wood2.md3");
    //		ent->sounds =
    //G_SoundIndex("sound/weapons/explosions/explode3.wav");
  } else if (ent->spawnflags & 2) {
    ent->s.modelindex =
        G_ModelIndex("/models/mapobjects/scavenger/k_barrel.md3");
    //		ent->sounds =
    //G_SoundIndex("sound/weapons/explosions/explode4.wav");
  } else {
    ent->s.modelindex = G_ModelIndex(
        va("/models/mapobjects/cargo/barrel%i.md3", Q_irand(0, 2)));
    //		ent->sounds =
    //G_SoundIndex("sound/weapons/explosions/explode1.wav");
  }
...
/*QUAKED misc_dlight (0.2 0.8 0.2) (-4 -4 -4) (4 4 4) STARTOFF FADEON FADEOFF
PULSE Dynamic light, toggles on and off when used

STARTOFF - Starts off
FADEON - Fades from 0 Radius to start Radius
FADEOFF - Fades from current Radius to 0 Radius before turning off
PULSE - This flag must be checked if you want it to fade/switch between start
and final RGBA, otherwise it will just sit at startRGBA

ownername - Will display the light at the origin of the entity with this
targetname

startRGBA - Red Green Blue Radius to start with - This MUST be set or your light
won't do anything

These next values are used only if you want to fade/switch between 2 values
(PULSE flag on) finalRGBA - Red Green Blue Radius to end with speed - how long
to take to fade from start to final and final to start.  Also how long to fade
on and off if appropriate flags are checked (seconds) finaltime - how long to
hold at final (seconds) starttime - how long to hold at start (seconds)
...
void SP_misc_dlight(gentity_t *ent) {
  ...
  if (!(ent->spawnflags & 1)) { // Turn myself on now
    GEntity_UseFunc(ent, ent, ent);
  }
}

void misc_dlight_use_old(gentity_t *ent, gentity_t *other,
                         gentity_t *activator) {
  G_ActivateBehavior(ent, BSET_USE);

  if (ent->misc_dlight_active) { // We're on, turn off
    if (ent->spawnflags & 4) {   // fade off
...
void station_pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
                  const vec3_t point, int damage, int mod, int hitLoc) {
  //	self->s.modelindex =
  //G_ModelIndex("/models/mapobjects/stasis/plugin2_in.md3"); 	self->s.eFlags &=
  //~ EF_ANIM_ALLFAST; 	self->s.eFlags |= EF_ANIM_ONCE; 	gi.linkentity (self);
...
void health_shutdown(gentity_t *self) {
  if (!(self->s.eFlags & EF_ANIM_ONCE)) {
    ...
    if (!Q_stricmp(self->model, "models/mapobjects/stasis/plugin2.md3")) {
      ...
      self->s.modelindex = self->s.modelindex2;
    } else if (!Q_stricmp(self->model,
                          "models/mapobjects/stasis/plugin2_floor.md3")) {
      self->s.modelindex = self->s.modelindex2;
      //			G_Sound(self,
...
void health_think(gentity_t *ent) {
  ...
  if (ent->count > 0) {
    ...
    } else // User has taken all health he can hold, see about giving it all to
           // armor
    {
...
void health_use(gentity_t *self, gentity_t *other,
                gentity_t *activator) { // FIXME: Heal entire team?  Or only
                                        // those that are undying...?
  int dif;
  int dif2;
  int hold;
  ...
  if (self->e_ThinkFunc != thinkF_NULL) {
    ...
    if (other->client) {
      ...
      if (other->client->ps.stats[STAT_HEALTH] < 1) {
        ...
        dif = other->client->ps.stats[STAT_MAX_HEALTH] -
              other->client->ps.stats[STAT_HEALTH];
        // Armor
        dif2 = other->client->ps.stats[STAT_MAX_HEALTH] -
               other->client->ps.stats[STAT_ARMOR];
        hold = (dif2 - dif);
        // For every 3 points of health, you get 1 point of armor
        // BUT!!! after health is filled up, you get the full energy going to
        // armor
        if (hold > 0) // Need more armor than health
        {
          ...
          hold = dif /
                 3; //	For every 3 points of health, you get 1 point of armor
          dif2 -= hold;
          dif2 += dif;
    ...
    } else { // Being triggered to be used up
      dif = 1;
      self->count = 0;
    }
...
void ammo_think(gentity_t *ent) {
  ...
  if (ent->count > 0 && ent->enemy) {
    dif =
        ammoData[AMMO_BLASTER].max - ent->enemy->client->ps.ammo[AMMO_BLASTER];
...
void mega_ammo_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  if (other->client->ps.ammo[AMMO_POWERCELL] >
      other->client->ps.ammo[AMMO_BLASTER])
    self->count =
        ammoData[AMMO_BLASTER].max - other->client->ps.ammo[AMMO_BLASTER];
  else
    self->count =
        ammoData[AMMO_POWERCELL].max - other->client->ps.ammo[AMMO_POWERCELL];
...
void mega_ammo_think(gentity_t *self) {
  ...
  // If the middle model is done animating, and we haven't switched to the last
  // model yet...
  //		chuck up the last model.

  if (!Q_stricmp(
          self->model,
          "models/mapobjects/forge/power_up_boss.md3")) // Because the normal
                                                        // forge_ammo model can
                                                        // use this too
  {
  ...
  if (self->enemy && self->count > 0) {
    ...
    if (self->enemy->client->ps.ammo[AMMO_POWERCELL] >
        ammoData[AMMO_POWERCELL].max)
      self->enemy->client->ps.ammo[AMMO_POWERCELL] =
          ammoData[AMMO_POWERCELL].max;

    // Decrement the count given counter
    self->count -= ammo_add;

    // If we've given all we should, prevent giving any more, even if they
    // player is no longer full
    if (self->count <= 0) {
      self->count = 0;
      self->e_ThinkFunc = thinkF_NULL;
      self->nextthink = -1;
...
//------------------------------------------------------------
void touch_ammo_crystal_tigger(gentity_t *self, gentity_t *other,
                               trace_t *trace) {
  if (!other->client)
    return;
...
void misc_replicator_item_remove(gentity_t *self, gentity_t *other,
                                 gentity_t *activator) {
  self->s.eFlags |= EF_NODRAW;
  // self->contents = 0;
  self->s.modelindex = 0;
  ...
  if (activator->client) {
    activator->health += 5;
    if (activator->health >
        activator->client->ps.stats[STAT_MAX_HEALTH]) // Past max health
    {
...
void misc_replicator_item_spawn(gentity_t *self, gentity_t *other,
                                gentity_t *activator) {
...
When used. this will "spawn" an entity with a random model from the ones
provided below...
...
NOTE: do not skip one of these model names, start with the lowest and fill in
each next highest one with a value.  A gap will cause the item to not work
correctly.

NOTE: if you use an invalid model, it will still try to use it and show the NULL
axis model (or nothing at all)
...
START_OFF Place in a map and point the angles at whatever surface you want it to
attach to.

START_ON - If you give it a targetname to make it toggle-able, but want it to
start on, set this flag BROADCAST - ONLY USE THIS IF YOU HAVE TO!  causes the
trip wire and loop sound to always happen, use this if the beam drops out in
certain situations START_OFF - If you give it a targetname, it will start
completely off (laser AND base unit)  until used.

The trip mine will attach to that surface and fire it's beam away from the
surface at an angle perpendicular to it.

targetname - starts off, when used, turns on (toggles)

FIXME: sometimes we want these to not be shootable... maybe just put them behind
a force field?
*/
extern void touchLaserTrap(gentity_t *ent, gentity_t *other, trace_t *trace);
extern void CreateLaserTrap(gentity_t *laserTrap, vec3_t start,
                            gentity_t *owner);

void misc_trip_mine_activate(gentity_t *self, gentity_t *other,
                             gentity_t *activator) {
  if (self->e_ThinkFunc == thinkF_laserTrapThink) {
    ...
    self->s.eFlags &= ~EF_NODRAW;
    self->contents = CONTENTS_SHOTCLIP; // CAN'T USE CONTENTS_SOLID because only
                                        // ARCHITECTURE is contents_solid!!!
    self->takedamage = qtrue;
  }
}
...
void SP_misc_trip_mine(gentity_t *self) {
  ...
  gi.trace(&trace, self->s.origin, vec3_origin, vec3_origin, end,
           self->s.number, MASK_SHOT, (EG2_Collision)0, 0);
  ...
  if (!self->targetname || (self->spawnflags & 1)) { // starts on
  ...
  if ((self->spawnflags &
       2)) // broadcast...should only be used in very rare cases.  could fix
           // networking, perhaps, but james suggested this because it's easier
  {
...
Place facing a door (using the angle, not a targetname) and it will lock that
door.  Can only be destroyed by lightsaber and will automatically unlock the
door it's attached to

NOTE: place these half-way in the door to make it flush with the door's surface.

"target"	thing to use when destoryed (not doors - it automatically
unlocks the door it was angled at) "health"	default is 10
*/
void maglock_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                 int damage, int mod, int dFlags, int hitLoc) {
...
void SP_misc_maglock(gentity_t *self) {
  ...
  //		or are targeted by a trigger, not doors fired off by scripts,
  //counters 		or other such things?
  self->s.modelindex =
  ...
  // FIXME: for some reason, when you re-load a level, these fail to find their
  // doors...?  Random?  Testing an additional 200ms after the
  // START_TIME_FIND_LINKS
  self->nextthink = level.time + START_TIME_FIND_LINKS +
                    200; // START_TIME_FIND_LINKS;//because we need to let the
                         // doors link up and spawn their triggers first!
}
void maglock_link(gentity_t *self) {
  ...
  gi.trace(&trace, start, vec3_origin, vec3_origin, end, self->s.number,
  ...
  if (trace.entityNum >= ENTITYNUM_WORLD || !traceEnt ||
      Q_stricmp("func_door", traceEnt->classname)) {
    self->e_ThinkFunc = thinkF_maglock_link;
    self->nextthink = level.time + 100;
  ...
  // check the traceEnt, make sure it's a door and give it a lockCount and
  // deactivate it find the trigger for the door
  ...
  // make it hittable
  // FIXME: if rotated/inclined this bbox may be off... but okay if we're a
  // ghoul model? self->s.modelindex = G_ModelIndex(
  // "models/map_objects/imp_detention/door_lock.md3" );
...
================
*/
void shield_power_converter_use(gentity_t *self, gentity_t *other,
                                gentity_t *activator) {
  int dif, add;
  ...
  if (self->setTime < level.time) {
    ...
    dif =
        100 -
        activator->client->ps.stats[STAT_ARMOR]; // FIXME: define for max armor?

    if (dif > 0 && self->count) // Already at full armor?..and do I even have
                                // anything to give
    {
    ...
    if (self->count <= 0) {
      ...
      self->setTime =
          level.time +
          1000; // extra debounce so that the sounds don't overlap too much
    ...
    } else if (activator->client->ps.stats[STAT_ARMOR] >=
               100) // FIXME: define for max
    {
      // play full sound
  ...
  if (self->s.loopSound) {
    // we will have to shut of the loop sound, so I guess try and do it
    // intelligently...NOTE: this could get completely stomped every time
    // through the loop
    //	this is fine, since it just controls shutting off the sound when there
    //are situations that could start the sound but not shut it off
...
/*QUAKED misc_model_shield_power_converter (1 0 0) (-16 -16 -16) (16 16 16) x x
x USETARGET model="models/items/psd_big.md3" Gives shield energy when used.

USETARGET - when used it fires off target

"health" - how much health the model has - default 60 (zero makes non-breakable)
"targetname" - dies and displays damagemodel when used, if any (if not, removes
itself) "target" - what to use when it dies "paintarget" - target to fire when
hit (but not destroyed) "count" - the amount of ammo given when used (default
100)
*/
//------------------------------------------------------------
void SP_misc_model_shield_power_converter(gentity_t *ent) {
  SetMiscModelDefaults(ent, useF_shield_power_converter_use, "4",
                       CONTENTS_SOLID, 0, qfalse, qfalse);
  ...
  ent->s.modelindex =
      G_ModelIndex("models/items/psd_big.md3"); // Precache model
  ent->s.modelindex2 =
      G_ModelIndex("models/items/psd_big.md3"); // Precache model
}
...
"forcevisible" - When you turn on force sight (any level), you can see these
draw through the entire level...
*/
//------------------------------------------------------------
void SP_misc_model_bomb_planted(gentity_t *ent) {
  ...
  SetMiscModelDefaults(ent, useF_bomb_planted_use, "4", CONTENTS_SOLID, 0,
                       qfalse, qfalse);
  ...
  ent->s.modelindex = G_ModelIndex(
      "models/map_objects/factory/bomb_new_deact.md3"); // Precache model
  ent->s.modelindex2 = G_ModelIndex(
      "models/map_objects/factory/bomb_new_deact.md3"); // Precache model
  ...
  // If we have a targetname, we're are invisible until we are spawned in by
  // being used.
  if (ent->targetname) {
  ...
  if (forceVisible) { // can see these through walls with force sight, so must
                      // be broadcast
    // ent->svFlags |= SVF_BROADCAST;
...
void SP_misc_model_beacon(gentity_t *ent) {
  ...
  SetMiscModelDefaults(ent, useF_beacon_use, "4", CONTENTS_SOLID, 0, qfalse,
                       qfalse);
  ...
  ent->s.modelindex =
      G_ModelIndex("models/map_objects/wedge/beacon.md3"); // Precache model
  ent->s.modelindex2 =
      G_ModelIndex("models/map_objects/wedge/beacon.md3"); // Precache model
  ent->noise_index = G_SoundIndex("sound/interface/ammocon_run");
...
void SP_misc_shield_floor_unit(gentity_t *ent) {
  ...
  ent->s.modelindex =
      G_ModelIndex("models/items/a_shield_converter.md3"); // Precache model
  ent->s.eFlags |= EF_SHADER_ANIM;
}
...
================
*/
void ammo_power_converter_use(gentity_t *self, gentity_t *other,
                              gentity_t *activator) {
  int add;
  ...
  if (!activator ||
      activator->s.number != 0) { // only the player gets to use these
    return;
  }
  ...
  if (self->setTime < level.time) {
    ...
    if (self->count &&
        (difBlaster > 0 || difPowerCell > 0 || difMetalBolts > 0)) {
...
/*QUAKED misc_model_ammo_power_converter (1 0 0) (-16 -16 -16) (16 16 16) x x x
USETARGET model="models/items/power_converter.md3" Gives ammo energy when used.
...
void SP_misc_model_ammo_power_converter(gentity_t *ent) {
  SetMiscModelDefaults(ent, useF_ammo_power_converter_use, "4", CONTENTS_SOLID,
                       0, qfalse, qfalse);
  ...
  ent->s.modelindex =
      G_ModelIndex("models/items/power_converter.md3"); // Precache model
  ent->s.modelindex2 =
      G_ModelIndex("models/items/power_converter.md3"); // Precache model
}
...
void SP_misc_ammo_floor_unit(gentity_t *ent) {
  ...
  ent->s.modelindex =
      G_ModelIndex("models/items/a_pwr_converter.md3"); // Precache model
  ent->s.eFlags |= EF_SHADER_ANIM;
}
...
void welder_think(gentity_t *self) {
  ...
  if (newBolt != -1) {
    ...
    //	G_PlayEffect( "blueWeldSparks", self->playerModel, newBolt,
    //self->s.number);
    // The welder gets rotated around a lot, and since the origin is offset by
    // 352 I have to make this super expensive call to position the hurt...
    gi.G2API_GetBoltMatrix(
        self->ghoul2, self->playerModel, newBolt, &boltMatrix,
        self->currentAngles, self->currentOrigin,
        (cg.time ? cg.time : level.time), NULL, self->s.modelScale);
...
void SP_misc_model_welder(gentity_t *ent) {
  ...
  SetMiscModelDefaults(ent, useF_welder_use, "4", CONTENTS_SOLID, 0, qfalse,
                       qfalse);
  ...
  ent->playerModel = gi.G2API_InitGhoul2Model(
      ent->ghoul2, "models/map_objects/cairn/welder.glm", ent->s.modelindex,
      NULL_HANDLE, NULL_HANDLE, 0, 0);
  ent->s.radius = 400.0f; // the origin of the welder is offset by approximately
                          // 352, so we need the big radius
...
void jabba_cam_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  if (self->spawnflags & 1) {
    ...
    gi.G2API_SetBoneAnimIndex(&self->ghoul2[self->playerModel], self->rootBone,
                              15, 0, BONE_ANIM_OVERRIDE_FREEZE, -1.5f,
                              (cg.time ? cg.time : level.time), -1, 0);

  } else {
    self->spawnflags |= 1;
    gi.G2API_SetBoneAnimIndex(&self->ghoul2[self->playerModel], self->rootBone,
                              0, 15, BONE_ANIM_OVERRIDE_FREEZE, 1.5f,
                              (cg.time ? cg.time : level.time), -1, 0);
  }
}
...
//-----------------------------------------------------
{
  ...
  ent->s.modelindex =
      G_ModelIndex("models/map_objects/nar_shaddar/jabacam/jabacam.glm");
  ent->playerModel = gi.G2API_InitGhoul2Model(
      ent->ghoul2, "models/map_objects/nar_shaddar/jabacam/jabacam.glm",
  ...
  // start extended..
  if (ent->spawnflags & 1) {
    gi.G2API_SetBoneAnimIndex(&ent->ghoul2[ent->playerModel], ent->rootBone, 0,
                              15, BONE_ANIM_OVERRIDE_FREEZE, 0.6f, cg.time, -1,
                              -1);
  }
...
void SP_misc_exploding_crate(gentity_t *ent) {
  ...
  ent->s.modelindex =
      G_ModelIndex("models/map_objects/nar_shaddar/crate_xplode.md3");
  ...
  ent->contents = CONTENTS_SOLID | CONTENTS_OPAQUE | CONTENTS_BODY |
                  CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP; // CONTENTS_SOLID;
  ent->takedamage = qtrue;
...
void gas_random_jet(gentity_t *self) {
  ...
  self->nextthink =
      level.time + Q_flrand(0.0f, 1.0f) * 16000 + 12000; // do this rarely
}

//------------------------------------------------------------
void GasBurst(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
              const vec3_t point, int damage, int mod, int hitLoc) {
  vec3_t pt;
  ...
  // do some damage to anything that may be standing on top of it when it bursts
  // into flame
  pt[2] += 32;
...
void SP_misc_gas_tank(gentity_t *ent) {
  ...
  ent->nextthink =
      level.time + Q_flrand(0.0f, 1.0f) * 12000 + 6000; // do this rarely
}
...
//------------------------------------------------------------
void CrystalCratePain(gentity_t *self, gentity_t *inflictor,
                      gentity_t *attacker, const vec3_t point, int damage,
                      int mod, int hitLoc) {
  vec3_t pt;
...
void SP_misc_crystal_crate(gentity_t *ent) {
  ...
  ent->s.modelindex =
      G_ModelIndex("models/map_objects/imp_mine/crate_open.md3");
  ...
  ent->contents = CONTENTS_SOLID | CONTENTS_OPAQUE | CONTENTS_BODY |
                  ...
                  CONTENTS_BOTCLIP; // Was CONTENTS_SOLID, but only architecture
                                    // should be this
...
Drivable ATST, when used by player, they become the ATST.  When the player hits
use again, they get out.
...
void misc_atst_setanim(gentity_t *self, int bone, int anim) {
  ...
  for (int i = 0; i < level.numKnownAnimFileSets; i++) {
    if (!Q_stricmp("atst", level.knownAnimFileSets[i].filename)) {
      ...
      lastFrame =
          firstFrame + level.knownAnimFileSets[i].animations[anim].numFrames;
      animSpeed = 50.0f / level.knownAnimFileSets[i].animations[anim].frameLerp;
      break;
    }
  }
  if (firstFrame != -1 && lastFrame != -1 && animSpeed != 0) {
    if (!gi.G2API_SetBoneAnimIndex(
            &self->ghoul2[self->playerModel], bone, firstFrame, lastFrame,
            BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, animSpeed,
            (cg.time ? cg.time : level.time), -1, 150)) {
      gi.G2API_SetBoneAnimIndex(&self->ghoul2[self->playerModel], bone,
                                firstFrame, lastFrame,
                                BONE_ANIM_OVERRIDE_FREEZE, animSpeed,
                                (cg.time ? cg.time : level.time), -1, 150);
    }
  }
}
void misc_atst_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                   int damage, int mod, int dFlags,
                   int hitLoc) { // ATST was destroyed while you weren't in it
  ...
  // set these to defaults that work in a worst-case scenario (according to
  // current animation.cfg)
  gi.G2API_StopBoneAnimIndex(&self->ghoul2[self->playerModel],
                             self->craniumBone);
...
void misc_atst_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  if (!activator || activator->s.number) { // only player can do this
    return;
  }
  ...
  if (activator->client->NPC_class != CLASS_ATST) { // get in the ATST
    if (activator->client->ps.groundEntityNum !=
        self->s.number) { // can only get in if on top of me...
  ...
  } else { // get out of ATST
    int legsAnim = activator->client->ps.legsAnim;
    if (legsAnim != BOTH_STAND1 && !PM_InSlopeAnim(legsAnim) &&
        legsAnim != BOTH_TURN_RIGHT1 &&
        legsAnim !=
            BOTH_TURN_LEFT1) { // can't get out of it while it's still moving
      return;
    }
    ...
    self->s.eFlags &= ~EF_NODRAW;
    self->svFlags &= ~SVF_NOCLIENT;
    self->contents = CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_MONSTERCLIP |
                     CONTENTS_BOTCLIP;
    self->takedamage = qtrue;
    // transfer armor
    tempHealth = self->health;
    ...
    vec3_t newOrg = {activator->currentOrigin[0], activator->currentOrigin[1],
                     activator->currentOrigin[2] +
                         (self->maxs[2] - self->mins[2]) + 1};
    G_SetOrigin(activator, newOrg);
    // open the hatch
    misc_atst_setanim(self, self->craniumBone, BOTH_STAND2);
    gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel],
                             "head_hatchcover", 0);
...
void SP_misc_atst_drivable(gentity_t *ent) {
  ...
  ent->playerModel = gi.G2API_InitGhoul2Model(
      ent->ghoul2, "models/players/atst/model.glm", ent->s.modelindex,
      NULL_HANDLE, NULL_HANDLE, 0, 0);
  ent->rootBone = gi.G2API_GetBoneIndex(&ent->ghoul2[ent->playerModel],
                                        "model_root", qtrue);
  ent->craniumBone = gi.G2API_GetBoneIndex(
      &ent->ghoul2[ent->playerModel], "cranium",
      qtrue); // FIXME: need to somehow set the anim/frame to the equivalent of
              // BOTH_STAND1...  use to be that BOTH_STAND1 was the first frame
              // of the glm, but not anymore
  ent->s.radius = 320;
  VectorSet(ent->s.modelScale, 1.0f, 1.0f, 1.0f);
  ...
  //	RegisterItem( FindItemForWeapon( WP_ROCKET_LAUNCHER ));	//precache the
  //weapon 	RegisterItem( FindItemForWeapon( WP_BOWCASTER ));	//precache the
  //weapon HACKHACKHACKTEMP - until ATST gets real weapons of it's own?
  ...
  gi.G2API_SetSurfaceOnOff(&ent->ghoul2[ent->playerModel], "head_hatchcover",
                           0);
  ...
  ent->contents =
      CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP;
...
Determines a region to check for weather contents - (optional, used to reduce
load times) Place surrounding your inside/outside brushes.  It will not check
for weather contents outside of these zones.
*/
void SP_misc_weather_zone(gentity_t *ent) {
  gi.SetBrushModel(ent, ent->model);

  char temp[256];
  sprintf(temp, "zone ( %f %f %f ) ( %f %f %f )", ent->mins[0], ent->mins[1],
          ent->mins[2], ent->maxs[0], ent->maxs[1], ent->maxs[2]);
...
void SP_misc_cubemap(gentity_t *ent) { G_FreeEntity(ent); }
