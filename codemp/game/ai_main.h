...
#define WPFLAG_SNIPEORCAMPSTAND                                                \
  0x00000800 // a good position to snipe or camp - stand
#define WPFLAG_WAITFORFUNC                                                     \
  0x00001000 // wait for a func brushent under this point before moving here
#define WPFLAG_SNIPEORCAMP                                                     \
  0x00002000 // a good position to snipe or camp - crouch
#define WPFLAG_ONEWAY_FWD                                                      \
  0x00004000 // can only go forward on the trial from here (e.g. went over a
             // ledge)
#define WPFLAG_ONEWAY_BACK                                                     \
  0x00008000 // can only go backward on the trail from here
#define WPFLAG_GOALPOINT                                                       \
  0x00010000 // make it a goal to get here.. goal points will be decided by
...
#define LEVELFLAG_NOPOINTPREDICTION                                            \
  1 // don't take waypoint beyond current into account when adjusting path view
    // angles
#define LEVELFLAG_IGNOREINFALLBACK                                             \
  2 // ignore enemies when in a fallback navigation routine
...
#define MAX_CHICKENWUSS_TIME                                                   \
  10000 // wait 10 secs between checking which run-away path to take

#define BOT_RUN_HEALTH 40
#define BOT_WPTOUCH_DISTANCE 32
#define ENEMY_FORGET_MS 10000
// if our enemy isn't visible within 10000ms (aprx 10sec) then "forget" about
// him and treat him like every other threat, but still look for more immediate
// threats while main enemy is not visible

#define BOT_PLANT_DISTANCE                                                     \
  256 // plant if within this radius from the last spotted enemy position
#define BOT_PLANT_INTERVAL 15000 // only plant once per 15 seconds at max
#define BOT_PLANT_BLOW_DISTANCE                                                \
  256 // blow det packs if enemy is within this radius and I am further away
      // than the enemy

#define BOT_MAX_WEAPON_GATHER_TIME                                             \
  1000 // spend a max of 1 second after spawn issuing orders to gather weapons
       // before attacking enemy base
#define BOT_MAX_WEAPON_CHASE_TIME                                              \
  15000 // time to spend gathering the weapon before persuing the enemy base (in
        // case it takes longer than expected)

#define BOT_MAX_WEAPON_CHASE_CTF                                               \
  5000 // time to spend gathering the weapon before persuing the enemy base (in
       // case it takes longer than expected) [ctf-only]

#define BOT_MIN_SIEGE_GOAL_SHOOT 1024
#define BOT_MIN_SIEGE_GOAL_TRAVEL 128

#define BASE_GUARD_DISTANCE 256 // guarding the flag
#define BASE_FLAGWAIT_DISTANCE                                                 \
  256 // has the enemy flag and is waiting in his own base for his flag to be
      // returned
...
int OrgVisibleBox(vec3_t org1, vec3_t mins, vec3_t maxs, vec3_t org2,
                  int ignore);
