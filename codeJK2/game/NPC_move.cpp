...
qboolean NPC_GetMoveDirection(vec3_t out, float *distance) {
  ...
  *distance = frameNavInfo.distance;
  ...
  if (NPC_ClearPathToGoal(frameNavInfo.direction, NPCInfo->goalEntity) ==
      qfalse) {
...
qboolean NPC_GetMoveDirectionAltRoute(vec3_t out, float *distance,
  ...
  *distance = frameNavInfo.distance;
  ...
  // Attempt a straight move to goal
  if (!tryStraight ||
      NPC_ClearPathToGoal(frameNavInfo.direction, NPCInfo->goalEntity) ==
          qfalse) { // blocked
    ...
    if (d_altRoutes->integer) { // try macro nav
      ...
      if (NAVNEW_AvoidCollision(NPC, NPCInfo->goalEntity, tempInfo, qtrue, 5) ==
          qfalse) { // revert to macro nav
      ...
      if (NAVNEW_AvoidCollision(NPC, NPCInfo->goalEntity, frameNavInfo, qtrue,
                                30) == qfalse) { // give up
        return qfalse;
      }
    }
...
qboolean NPC_MoveToGoal(qboolean tryStraight) {
  ...
  if (PM_InKnockDown(&NPC->client->ps) ||
      ((NPC->s.legsAnim >= BOTH_PAIN1) && (NPC->s.legsAnim <= BOTH_PAIN19))) {
    return qtrue;
  }
