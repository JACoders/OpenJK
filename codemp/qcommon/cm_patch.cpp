...
static void *Hunk_Alloc(int size, ha_pref preference) { return Hunk_Alloc(size); }
#endif
...
static inline int CM_PlaneEqual(patchPlane_t *p, float plane[4], int *flipped) {
	...
	if (Q_fabs(p->plane[0] - plane[0]) < NORMAL_EPSILON && Q_fabs(p->plane[1] - plane[1]) < NORMAL_EPSILON && Q_fabs(p->plane[2] - plane[2]) < NORMAL_EPSILON &&
...
===================
*/
static inline void CM_SetBorderInward(facet_t *facet, cGrid_t *grid, int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2], int i, int j, int which) {
	int k, l;
	float *points[4];
	int numPoints;
