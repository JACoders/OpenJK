...
void RB_BeginDrawingView(void) {
	...
	if (tr.refdef.rdflags & RDF_AUTOMAP || (!(backEnd.refdef.rdflags & RDF_NOWORLDMODEL) && r_DynamicGlow->integer && !g_bRenderGlowingObjects)) {
		if (tr.world &&
			tr.world->globalFog != -1) { // this is because of a bug in multiple scenes I think, it needs to clear for the second scene but it doesn't normally.
...
void RB_RenderDrawSurfList(drawSurf_t *drawSurfs, int numDrawSurfs) {
	...
	for (i = 0, drawSurf = drawSurfs; i < numDrawSurfs; i++, drawSurf++) {
		...
		if (entityNum != REFENTITYNUM_WORLD && g_numPostRenders < MAX_POST_RENDERS) {
			if ((backEnd.refdef.entities[entityNum].e.renderfx & RF_DISTORTION) || (backEnd.refdef.entities[entityNum].e.renderfx & RF_FORCEPOST) ||
...
const void *RB_RotatePic(const void *data) {
	...
	if (image) {
		...
		matrix3_t m = {{c, s, 0.0f}, {-s, c, 0.0f}, {cmd->x + cmd->w, cmd->y, 1.0f}};
