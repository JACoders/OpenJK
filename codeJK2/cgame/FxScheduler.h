...
private:
  ...
  struct SScheduledEffect {
    ...
    int mStartTime;
    int mModelNum; // uset to determine which ghoul2 model we want to bolt this
                   // effect to
    int mBoltNum;  // used to determine which bolt on the ghoul2 model we should
                   // be attaching this effect to
    int mEntNum; // used to determine which entity this ghoul model is attached
                 // to.
...
public:
  ...
  void PlayEffect(int id, vec3_t org,
                  vec3_t fwd); // builds arbitrary perp. right vector, does a
                               // cross product to define up
