...
void G2_DEBUG_ReportLeaks(void) {
  ...
  while (i < MAX_TRACKED_ALLOC) {
    if (g_G2AllocTrack[i]) {
      ri.Printf(PRINT_ALL, "Bad guy found in slot %i, attempting to access...",
                i);
...
void G2_DEBUG_ShovePtrInTracker(CGhoul2Info_v *g2) {
  ...
  if (g2v[0].currentModel && g2v[0].currentModel->name &&
      g2v[0].currentModel->name[0]) {
    ri.Printf(PRINT_ALL,
              "%s could not be fit into g2 debug instance tracker.\n",
...
void G2_DEBUG_RemovePtrFromTracker(CGhoul2Info_v *g2) {
  ...
  if (g2v[0].currentModel && g2v[0].currentModel->name &&
    ...
    ri.Printf(PRINT_ALL,
              "Crap g2 instance passed to instance tracker (out).\n");
  }
}
#endif
...
#if G2_DEBUG_TIME
  ri.Printf(PRINT_ALL, "Set Time: before c%6d  s%6d", G2TimeBases[1],
            G2TimeBases[0]);
#endif
...
int G2API_GetTime(
    int argTime) // this may or may not return arg depending on ghoul2_time cvar
{
...
void G2API_AttachInstanceToEntNum(
    CGhoul2Info_v &ghoul2, int entityNum,
    qboolean server) { // Assign the pointers in the arrays
#ifdef _G2_LISTEN_SERVER_OPT
  if (server) {
...
qboolean G2API_OverrideServerWithClientData(CGhoul2Info_v &ghoul2,
  ...
  if (ri.Cvar_VariableIntegerValue("dedicated")) { // No client to get from!
    return qfalse;
  }

  if (!g2ClientAttachments[serverInstance->entityNum]) { // No clientside
  ...
  if (clientInstance->mSkelFrameNum !=
      frameNum) { // it has to be constructed already
    return qfalse;
  }

  if (!clientInstance->mBoneCache) { // that just won't do
    return qfalse;
  }
  ...
  serverInstance->mTransformedVertsArray =
      clientInstance->mTransformedVertsArray;

  if (!serverInstance->mBoneCache) { // if this is the case.. I guess we can use
                                     // the client one instead
...
static size_t GetSizeOfGhoul2Info(const CGhoul2Info &g2Info) {
  ...
  // This is pretty ugly, but we don't want to save everything in the
  // CGhoul2Info object.
  size += offsetof(CGhoul2Info, mTransformedVertsArray) -
          offsetof(CGhoul2Info, mModelindex);
...
static size_t SerializeGhoul2Info(char *buffer, const CGhoul2Info &g2Info) {
  ...
  // Oh the ugliness...
  blockSize = offsetof(CGhoul2Info, mTransformedVertsArray) -
              offsetof(CGhoul2Info, mModelindex);
  ...
  *(int *)buffer = g2Info.mSlist.size();
  ...
  memcpy(buffer, g2Info.mSlist.data(),
         g2Info.mSlist.size() * sizeof(surfaceInfo_t));
  buffer += blockSize;
  ...
  *(int *)buffer = g2Info.mBlist.size();
  ...
  memcpy(buffer, g2Info.mBlist.data(),
         g2Info.mBlist.size() * sizeof(boneInfo_t));
  buffer += blockSize;
  ...
  *(int *)buffer = g2Info.mBltlist.size();
  ...
  memcpy(buffer, g2Info.mBltlist.data(),
         g2Info.mBltlist.size() * sizeof(boltInfo_t));
  buffer += blockSize;
...
public:
  ...
  bool IsValid(int handle) const {
    ...
    assert((handle & G2_INDEX_MASK) >= 0 &&
           (handle & G2_INDEX_MASK) < MAX_G2_MODELS); // junk handle
    if (mIds[handle & G2_INDEX_MASK] !=
        handle) // not a valid handle, could be old
    {
      return false;
    }
    return true;
  ...
  void Delete(int handle) {
    ...
    assert((handle & G2_INDEX_MASK) >= 0 &&
           (handle & G2_INDEX_MASK) < MAX_G2_MODELS); // junk handle
    assert(mIds[handle & G2_INDEX_MASK] ==
           handle); // not a valid handle, could be old or garbage
  ...
  std::vector<CGhoul2Info> &Get(int handle) {
    ...
    assert(mIds[handle & G2_INDEX_MASK] ==
           handle); // not a valid handle, could be old or garbage
    assert(!(handle <= 0 || (handle & G2_INDEX_MASK) < 0 ||
             (handle & G2_INDEX_MASK) >= MAX_G2_MODELS ||
             mIds[handle & G2_INDEX_MASK] != handle));
  ...
  vector<CGhoul2Info> &GetDebug(int handle) {
    ...
    if (handle <= 0 || (handle & G2_INDEX_MASK) < 0 ||
        (handle & G2_INDEX_MASK) >= MAX_G2_MODELS ||
        mIds[handle & G2_INDEX_MASK] != handle) {
...
void SaveGhoul2InfoArray() {
  ...
  if (!ri.PD_Store(PERSISTENT_G2DATA, data, size)) {
    Com_Printf(S_COLOR_RED
               "ERROR: Failed to store persistent renderer data.\n");
  }
}
...
qboolean G2_ShouldRegisterServer(void) {
  ...
  if (currentVM && currentVM->slot == VM_GAME) {
    if (ri.Cvar_VariableIntegerValue("cl_running") &&
        ...
        ShaderHashTableExists()) { // if the hunk has been marked then we are
                                   // now loading client assets so don't load on
                                   // server.
      return qfalse;
    }
...
int G2API_InitGhoul2Model(CGhoul2Info_v **ghoul2Ptr, const char *fileName,
  ...
  if (model == ghoul2.size()) { // init should not be used to create additional
                                // models, only the first one
...
qboolean G2API_SetSkin(CGhoul2Info_v &ghoul2, int modelIndex,
                       qhandle_t customSkin, qhandle_t renderSkin) {
  CGhoul2Info *ghlInfo = &ghoul2[modelIndex];

  if (G2_SetupModelPointers(ghlInfo)) {
    ghlInfo->mCustomSkin = customSkin;
    if (renderSkin) { // this is going to set the surfs on/off matching the skin
                      // file
...
qboolean G2API_SetSurfaceOnOff(CGhoul2Info_v &ghoul2, const char *surfaceName,
                               const int flags) {
...
qboolean G2API_SetRootSurface(CGhoul2Info_v &ghoul2, const int modelIndex,
...
int G2API_AddSurface(CGhoul2Info *ghlInfo, int surfaceNumber, int polyNumber,
  ...
  if (G2_SetupModelPointers(ghlInfo)) {
    // ensure we flush the cache
    ghlInfo->mMeshFrameNum = 0;
    return G2_AddSurface(ghlInfo, surfaceNumber, polyNumber, BarycentricI,
                         BarycentricJ, lod);
  }
  return -1;
}
...
int G2API_GetSurfaceRenderStatus(CGhoul2Info_v &ghoul2, int modelIndex,
                                 const char *surfaceName) {
  CGhoul2Info *ghlInfo = &ghoul2[modelIndex];
...
qboolean G2API_HasGhoul2ModelOnIndex(CGhoul2Info_v **ghlRemove,
                                     const int modelIndex) {
  CGhoul2Info_v &ghlInfo = **ghlRemove;

  if (!ghlInfo.size() || (ghlInfo.size() <= modelIndex) ||
...
qboolean G2API_RemoveGhoul2Model(CGhoul2Info_v **ghlRemove,
                                 const int modelIndex) {
  CGhoul2Info_v &ghlInfo = **ghlRemove;
  ...
  if (!ghlInfo.size() || (ghlInfo.size() <= modelIndex) ||
      (ghlInfo[modelIndex].mModelindex == -1)) {
    // if we hit this assert then we are trying to delete a ghoul2 model on a
    // ghoul2 instance that one way or another is already gone.
    assert(0);
    return qfalse;
  }
...
qboolean G2API_RemoveGhoul2Models(CGhoul2Info_v **ghlRemove) { // remove 'em
                                                               // ALL!
  CGhoul2Info_v &ghlInfo = **ghlRemove;
  int modelIndex = 0;
  int newSize = 0;
  ...
  // sanity check
  if (!ghlInfo.size()) { // if we hit this then we are trying to delete a ghoul2
                         // model on a ghoul2 instance that
  ...
  // now look through the list from the back and see if there is a block of -1's
  // we can resize off the end of the list
...
// check if a bone exists on skeleton without actually adding to the bone list
// -rww
qboolean G2API_DoesBoneExist(CGhoul2Info_v &ghoul2, int modelIndex,
                             const char *boneName) {
  CGhoul2Info *ghlInfo = &ghoul2[modelIndex];

  if (G2_SetupModelPointers(ghlInfo)) { // model is valid
    mdxaHeader_t *mdxa = ghlInfo->currentModel->mdxa;
    if (mdxa) { // get the skeleton data and iterate through the bones
      int i;
      mdxaSkel_t *skel;
      ...
      for (i = 0; i < mdxa->numBones; i++) {
        skel = (mdxaSkel_t *)((byte *)mdxa + sizeof(mdxaHeader_t) +
                              offsets->offsets[i]);
        if (!Q_stricmp(skel->name, boneName)) { // got it
          return qtrue;
        }
      }
    }
...
qboolean G2API_SetBoneAnimIndex(CGhoul2Info *ghlInfo, const int index,
                                const int AstartFrame, const int AendFrame,
                                const int flags, const float animSpeed,
                                const int currentTime, const float AsetFrame,
                                const int blendTime) {
  ...
  if (res) {
    ...
    return G2_Set_Bone_Anim_Index(ghlInfo->mBlist, index, startFrame, endFrame,
                                  flags, animSpeed, currentTime, setFrame,
...
qboolean G2API_SetBoneAnim(CGhoul2Info_v &ghoul2, const int modelIndex,
                           const char *boneName, const int AstartFrame,
                           const int AendFrame, const int flags,
  ...
  if (ghoul2.size() > modelIndex) {
    ...
    if (res) {
      // ensure we flush the cache
      ghlInfo->mSkelFrameNum = 0;
      return G2_Set_Bone_Anim(ghlInfo, ghlInfo->mBlist, boneName, startFrame,
                              endFrame, flags, animSpeed, currentTime, setFrame,
                              blendTime);
    }
  }
  return qfalse;
...
qboolean G2API_GetBoneAnim(CGhoul2Info_v &ghoul2, int modelIndex,
                           const char *boneName, const int currentTime,
                           float *currentFrame, int *startFrame, int *endFrame,
                           int *flags, float *animSpeed, int *modelList) {
  ...
  if (G2_SetupModelPointers(ghlInfo)) {
    ...
    qboolean ret =
        G2_Get_Bone_Anim(ghlInfo, ghlInfo->mBlist, boneName, aCurrentTime,
...
qboolean G2API_GetAnimRange(CGhoul2Info *ghlInfo, const char *boneName,
                            int *startFrame, int *endFrame) {
  assert(startFrame != endFrame); // this is bad
  if (G2_SetupModelPointers(ghlInfo)) {
    qboolean ret = G2_Get_Bone_Anim_Range(ghlInfo, ghlInfo->mBlist, boneName,
                                          startFrame, endFrame);
#ifdef _DEBUG
    assert(*endFrame > 0);
    assert(*endFrame < 100000);
...
qboolean G2API_PauseBoneAnim(CGhoul2Info *ghlInfo, const char *boneName,
...
qboolean G2API_SetBoneAnglesIndex(CGhoul2Info *ghlInfo, const int index,
  ...
  if (res) {
    ...
    return G2_Set_Bone_Angles_Index(
        ghlInfo->mBlist, index, angles, flags, yaw, pitch, roll, modelList,
        ghlInfo->mModelindex, blendTime, currentTime);
  }
  return qfalse;
}

qboolean G2API_SetBoneAngles(CGhoul2Info_v &ghoul2, const int modelIndex,
                             const char *boneName, const vec3_t angles,
  ...
  if (ghoul2.size() > modelIndex) {
    ...
    if (res) {
      // ensure we flush the cache
      ghlInfo->mSkelFrameNum = 0;
      return G2_Set_Bone_Angles(ghlInfo, ghlInfo->mBlist, boneName, angles,
                                flags, up, left, forward, modelList,
                                ghlInfo->mModelindex, blendTime, currentTime);
    }
  }
  return qfalse;
...
qboolean G2API_SetBoneAnglesMatrixIndex(CGhoul2Info *ghlInfo, const int index,
  ...
  if (G2_SetupModelPointers(ghlInfo)) {
    ...
    return G2_Set_Bone_Angles_Matrix_Index(
        ghlInfo->mBlist, index, matrix, flags, modelList, ghlInfo->mModelindex,
        blendTime, currentTime);
  }
  return qfalse;
}

qboolean G2API_SetBoneAnglesMatrix(CGhoul2Info *ghlInfo, const char *boneName,
  ...
  if (G2_SetupModelPointers(ghlInfo)) {
    ...
    return G2_Set_Bone_Angles_Matrix(
        ghlInfo->mFileName, ghlInfo->mBlist, boneName, matrix, flags, modelList,
        ghlInfo->mModelindex, blendTime, currentTime);
  }
  return qfalse;
}
...
void G2API_ResetRagDoll(CGhoul2Info_v &ghoul2) { G2_ResetRagDoll(ghoul2); }
// rww - RAGDOLL_END

qboolean G2API_RemoveBone(CGhoul2Info_v &ghoul2, int modelIndex,
                          const char *boneName) {
  CGhoul2Info *ghlInfo = &ghoul2[modelIndex];
...
void G2API_AnimateG2ModelsRag(CGhoul2Info_v &ghoul2, int AcurrentTime,
  ...
  if (ragTraceTime)
  {
          ri.Printf( PRINT_ALL, "Rag trace time: %i (%i STARTSOLID, %i
...
static inline boneInfo_t *G2_GetRagBoneConveniently(CGhoul2Info_v &ghoul2,
                                                    const char *boneName) {
  assert(ghoul2.size());
  CGhoul2Info *ghlInfo = &ghoul2[0];

  if (!(ghlInfo->mFlags &
        GHOUL2_RAG_STARTED)) { // can't do this if not in ragdoll
    return NULL;
  }

  int boneIndex = G2_Find_Bone_Rag(ghlInfo, ghlInfo->mBlist, boneName);

  if (boneIndex < 0) { // bad bone specification
    return NULL;
  }

  boneInfo_t *bone = &ghlInfo->mBlist[boneIndex];

  if (!(bone->flags & BONE_ANGLES_RAGDOLL)) { // only want to return rag bones
    return NULL;
  }
...
qboolean G2API_RagPCJConstraint(CGhoul2Info_v &ghoul2, const char *boneName,
  ...
  if (!(bone->RagFlags & RAG_PCJ)) { // this function is only for PCJ bones
    return qfalse;
  }
...
qboolean G2API_RagPCJGradientSpeed(CGhoul2Info_v &ghoul2, const char *boneName,
                                   const float speed) {
  boneInfo_t *bone = G2_GetRagBoneConveniently(ghoul2, boneName);
...
qboolean G2API_RagEffectorGoal(CGhoul2Info_v &ghoul2, const char *boneName,
                               vec3_t pos) {
  boneInfo_t *bone = G2_GetRagBoneConveniently(ghoul2, boneName);
  ...
  if (!(bone->RagFlags & RAG_EFFECTOR)) { // this function is only for effectors
    return qfalse;
  }

  if (!pos) { // go back to none in case we have one then
...
qboolean G2API_GetRagBonePos(CGhoul2Info_v &ghoul2, const char *boneName,
                             vec3_t pos, vec3_t entAngles, vec3_t entPos,
                             vec3_t entScale) { // do something?
  return qfalse;
}

qboolean G2API_RagEffectorKick(CGhoul2Info_v &ghoul2, const char *boneName,
                               vec3_t velocity) {
  boneInfo_t *bone = G2_GetRagBoneConveniently(ghoul2, boneName);
...
qboolean G2API_SetBoneIKState(CGhoul2Info_v &ghoul2, int time,
                              const char *boneName, int ikState,
                              sharedSetBoneIKStateParams_t *params) {
...
int G2API_AddBolt(CGhoul2Info_v &ghoul2, const int modelIndex,
                  const char *boneName) {
...
int G2API_AddBoltSurfNum(CGhoul2Info *ghlInfo, const int surfIndex) {
  if (G2_SetupModelPointers(ghlInfo)) {
    return G2_Add_Bolt_Surf_Num(ghlInfo, ghlInfo->mBltlist, ghlInfo->mSlist,
                                surfIndex);
  }
  return -1;
}

qboolean G2API_AttachG2Model(CGhoul2Info_v &ghoul2From, int modelFrom,
                             CGhoul2Info_v &ghoul2To, int toBoltIndex,
                             int toModel) {
  ...
  if (G2_SetupModelPointers(ghoul2From) && G2_SetupModelPointers(ghoul2To)) {
    // make sure we have a model to attach, a model to attach to, and a bolt on
    // that model
...
qboolean G2API_AttachEnt(int *boltInfo, CGhoul2Info_v &ghoul2, int modelIndex,
  ...
  if (boltInfo && G2_SetupModelPointers(ghlInfoTo)) {
    ...
    if (ghlInfoTo->mBltlist.size() &&
      ...
      toBoltIndex &= BOLT_AND;
      entNum &= ENTITY_AND;
      *boltInfo = (toBoltIndex << BOLT_SHIFT) | (toModelNum << MODEL_SHIFT) |
                  (entNum << ENTITY_SHIFT);
      return qtrue;
    }
  }
...
qboolean G2API_GetBoltMatrix_SPMethod(CGhoul2Info_v &ghoul2,
                                      const int modelIndex, const int boltIndex,
                                      mdxaBone_t *matrix, const vec3_t angles,
                                      const vec3_t position, const int frameNum,
                                      qhandle_t *modelList,
                                      const vec3_t scale) {
  assert(ghoul2.size() > modelIndex);
  ...
  if ((ghoul2.size() > modelIndex)) {
    ...
    if (ghlInfo && (boltIndex < (int)ghlInfo->mBltlist.size()) &&
        boltIndex >= 0) {
      ...
      if (scale[0] || scale[1] || scale[2]) {
        ...
        // this point its still in model space
        if (scale[0]) {
...
// qboolean G2API_GetBoltMatrix(CGhoul2Info_v &ghoul2, const int modelIndex,
// const int boltIndex, mdxaBone_t *matrix, const vec3_t angles, 							 const vec3_t
//position, const int AframeNum, qhandle_t *modelList, const vec3_t scale )
qboolean G2API_GetBoltMatrix(CGhoul2Info_v &ghoul2, const int modelIndex,
  ...
  if (G2_SetupModelPointers(ghoul2)) {
    if (matrix && modelIndex >= 0 && modelIndex < ghoul2.size()) {
      ...
      if (boltIndex >= 0 && ghlInfo &&
        ...
        if (!gG2_GBMUseSPMethod) { // this is horribly stupid and I hate it. But
                                   // lots of game code is written to assume this
                                   // 90 degree offset thing.
          float ftemp;
...
void G2API_SetGhoul2ModelIndexes(CGhoul2Info_v &ghoul2, qhandle_t *modelList,
                                 qhandle_t *skinList) {
  return;
#if 0
	int i;
...
static inline bool
G2_NeedRetransform(CGhoul2Info *g2,
                   int frameNum) { // see if we need to do another transform
  size_t i = 0;
  ...
  while (i < g2->mBlist.size()) {
    ...
    if (newFrame < bone.endFrame || (bone.flags & BONE_ANIM_OVERRIDE_LOOP) ||
        (bone.flags & BONE_NEED_TRANSFORM)) { // ok, we're gonna have to do it.
                                              // bone is apparently animating.
...
void G2API_CollisionDetectCache(
    CollisionRecord_t *collRecMap, CGhoul2Info_v &ghoul2, const vec3_t angles,
    ...
    int useLod,
    float fRadius) { // this will store off the transformed verts for the next
                     // trace - this is slower, but for models that do not
                     // animate frequently it is much much faster. -rww
#if 0 // UNUSED
  ...
  if (G2_SetupModelPointers(ghoul2)) {
    ...
    if (G2_NeedRetransform(&ghoul2[0], tframeNum) ||
        !ghoul2[0].mTransformedVertsArray) { // optimization, only create new
                                             // transform space if we need to,
                                             // otherwise
      // store it off!
      int i = 0;
      while (i < ghoul2.size()) {
        ...
        */
        if (!g2.mTransformedVertsArray ||
            !(g2.mFlags &
              GHOUL2_ZONETRANSALLOC)) { // reworked so we only alloc once!
          // if we have a pointer, but not a ghoul2_zonetransalloc flag, then
          // that means it is a miniheap pointer. Just stomp over it.
      ...
      // don't need to do this anymore now that I am using a flag for zone
      // alloc.
      /*
      i = 0;
      while (i < ghoul2.size())
    ...
    // now walk each model and check the ray against each poly - sigh, this is
    // SO expensive. I wish there was a better way to do this.
#ifdef _G2_GORE
    G2_TraceModels(ghoul2, transRayStart, transRayEnd, collRecMap, entNum,
                   traceFlags, useLod, fRadius, 0, 0, 0, 0, 0, qfalse);
#else
    G2_TraceModels(ghoul2, transRayStart, transRayEnd, collRecMap, entNum,
    ...
    // now sort the resulting array of collision records so they are distance
    // ordered
    qsort(collRecMap, i, sizeof(CollisionRecord_t), QsortDistance);
  }
}

void G2API_CollisionDetect(CollisionRecord_t *collRecMap, CGhoul2Info_v &ghoul2,
                           const vec3_t angles, const vec3_t position,
                           int frameNumber, int entNum, vec3_t rayStart,
                           vec3_t rayEnd, vec3_t scale,
                           IHeapAllocator *G2VertSpace, int traceFlags,
                           int useLod, float fRadius) {
  /*
  if (1)
  {
          G2API_CollisionDetectCache(collRecMap, ghoul2, angles, position,
  frameNumber, entNum, rayStart, rayEnd, scale, G2VertSpace, traceFlags, useLod,
  fRadius); return;
  }
  */
...
// in flags
void G2API_GiveMeVectorFromMatrix(mdxaBone_t *boltMatrix, Eorientations flags,
                                  vec3_t vec) {
  switch (flags) {
  case ORIGIN:
...
int G2API_CopyGhoul2Instance(CGhoul2Info_v &g2From, CGhoul2Info_v &g2To,
                             int modelIndex) {
  assert(
      modelIndex ==
      -1); // copy individual bolted parts is not used in jk2 and I didn't want
           // to deal with it if ya want it, we will add it back correctly
...
void G2API_CopySpecificG2Model(CGhoul2Info_v &ghoul2From, int modelFrom,
                               CGhoul2Info_v &ghoul2To, int modelTo) {
#if 0
  ...
  if (ghoul2From.size() > modelFrom) {
    ...
    if (ghoul2To.IsValid() &&
        ghoul2To.size() >= modelTo) { // remove the bonecache before we stomp
                                      // over this instance.
...
// This version will automatically copy everything about this model, and make a
// new one if necessary.
void G2API_DuplicateGhoul2Instance(CGhoul2Info_v &g2From,
                                   CGhoul2Info_v **g2To) {
  // int ignore;

  if (*g2To) { // This is bad.  We only want to do this if there is not yet a to
               // declared.
    assert(0);
    return;
  }
...
char *G2API_GetSurfaceName(CGhoul2Info_v &ghoul2, int modelIndex,
                           int surfNumber) {
  ...
  if (G2_SetupModelPointers(ghlInfo)) {
...
#ifndef FINAL_BUILD
    if (!mod || !mod->mdxm) {
      Com_Error(ERR_DROP, "G2API_GetSurfaceName: Bad model on instance %s.",
                ghlInfo->mFileName);
    }
#endif

    // ok, I guess it's semi-valid for the user to be passing in surface >
    // numSurfs because they don't know how many surfs a model may have.. but how
    // did they get that surf index to begin with? Oh well.
    if (surfNumber < 0 || surfNumber >= mod->mdxm->numSurfaces) {
      ri.Printf(PRINT_ALL,
                "G2API_GetSurfaceName: You passed in an invalid surface number "
                "(%i) for model %s.\n",
    ...
    if (surf) {
#ifndef FINAL_BUILD
      if (surf->thisSurfaceIndex < 0 ||
          surf->thisSurfaceIndex >= mod->mdxm->numSurfaces) {
        Com_Error(
            ERR_DROP,
            "G2API_GetSurfaceName: Bad surf num (%i) on surf for instance %s.",
            surf->thisSurfaceIndex, ghlInfo->mFileName);
      }
#endif
      mdxmHierarchyOffsets_t *surfIndexes =
          (mdxmHierarchyOffsets_t *)((byte *)mod->mdxm + sizeof(mdxmHeader_t));
      surfInfo =
...
char *G2API_GetGLAName(CGhoul2Info_v &ghoul2, int modelIndex) {
  if (G2_SetupModelPointers(ghoul2)) {
    if ((ghoul2.size() > modelIndex)) {
      // model_t	*mod =
      // R_GetModelByHandle(RE_RegisterModel(ghoul2[modelIndex].mFileName));
      // return mod->mdxm->animName;

      assert(ghoul2[modelIndex].currentModel &&
             ghoul2[modelIndex].currentModel->mdxm);
...
qboolean G2API_SetNewOrigin(CGhoul2Info_v &ghoul2, const int boltIndex) {
  ...
  if (G2_SetupModelPointers(ghlInfo)) {
    if (boltIndex < 0) {
      ...
      Com_Error(ERR_DROP,
                "Bad boltindex (%i) trying to SetNewOrigin (naughty "
                "naughty!)\nModel %s\n",
                boltIndex, modelName);
    }
...
qboolean G2API_SaveGhoul2Models(CGhoul2Info_v &ghoul2, char **buffer,
                                int *size) {
...
void G2API_FreeSaveBuffer(char *buffer) { Z_Free(buffer); }

// this is kinda sad, but I need to call the destructor in this module (exe),
// not the game.dll...
//
void G2API_LoadSaveCodeDestructGhoul2Info(CGhoul2Info_v &ghoul2) {
  ...
  G2API_ClearSkinGore(ghoul2);
#endif
  ghoul2.~CGhoul2Info_v(); // so I can load junk over it then memset to 0
                           // without orphaning
}
...
qboolean G2API_SkinlessModel(CGhoul2Info_v &ghoul2, int modelIndex) {
  ...
  if (G2_SetupModelPointers(g2)) {
    ...
    if (mod && mod->mdxm) {
      ...
      surf = (mdxmSurfHierarchy_t *)((byte *)mod->mdxm +
                                     mod->mdxm->ofsSurfHierarchy);

      for (i = 0; i < mod->mdxm->numSurfaces; i++) {
        if (surf->shader[0]) { // found a surface with a shader name, ok.
          return qfalse;
        }

        surf = (mdxmSurfHierarchy_t
                    *)((byte *)surf +
...
int G2API_Ghoul2Size(CGhoul2Info_v &ghoul2) { return ghoul2.size(); }
...
void G2API_AddSkinGore(CGhoul2Info_v &ghoul2, SSkinGoreData &gore) {
  ...
  // make sure we have transformed the whole skeletons for each model
  // G2_ConstructGhoulSkeleton(ghoul2, gore.currentTime, NULL, true,
  // gore.angles, gore.position, gore.scale, false);
  ...
  TransformAndTranslatePoint(gore.hitLocation, transHitLocation,
                             &worldMatrixInv);
  TransformPoint(gore.rayDirection, transRayDirection, &worldMatrixInv);

  int lod;
  ResetGoreTag();
  const int lodbias = Com_Clamp(
      0, 2,
      G2_DecideTraceLod(ghoul2[0], ri.Cvar_VariableIntegerValue("r_lodbias")));
  const int maxLod =
      Com_Clamp(0, ghoul2[0].currentModel->numLods,
                3); // limit to the number of lods the main model has
  for (lod = lodbias; lod < maxLod; lod++) {
    // now having done that, time to build the model
    ri.GetG2VertSpaceServer()->ResetHeap();

    G2_TransformModel(ghoul2, gore.currentTime, gore.scale,
                      ri.GetG2VertSpaceServer(), lod, true);

    // now walk each model and compute new texture coordinates
    G2_TraceModels(ghoul2, transHitLocation, transRayDirection, 0, gore.entNum,
                   0, lod, 0.0f, gore.SSize, gore.TSize, gore.theta,
                   gore.shader, &gore, qtrue);
  }
}
#endif

qboolean G2_TestModelPointers(
    CGhoul2Info *ghlInfo) // returns true if the model is properly set up
{
    ...
    if (ghlInfo->currentModel) {
      if (ghlInfo->currentModel->mdxm) {
        if (ghlInfo->currentModelSize) {
          if (ghlInfo->currentModelSize !=
              ghlInfo->currentModel->mdxm->ofsEnd) {
            Com_Error(ERR_DROP, "Ghoul2 model was reloaded and has changed, "
                                "map must be restarted.\n");
          }
        }
        ghlInfo->currentModelSize = ghlInfo->currentModel->mdxm->ofsEnd;
        ghlInfo->animModel =
            R_GetModelByHandle(ghlInfo->currentModel->mdxm->animIndex);
          ...
          if (ghlInfo->aHeader) {
            if (ghlInfo->currentAnimModelSize) {
              if (ghlInfo->currentAnimModelSize != ghlInfo->aHeader->ofsEnd) {
                Com_Error(ERR_DROP, "Ghoul2 model was reloaded and has "
                                    "changed, map must be restarted.\n");
              }
            }
...
qboolean G2_SetupModelPointers(
    CGhoul2Info *ghlInfo) // returns true if the model is properly set up
{
#ifdef G2_PERFORMANCE_ANALYSIS
  ...
  if (0) { // rww - Why are we bothering with all this? We can't change models
           // like this anyway. This function goes over 200k on the precision
           // timer (in debug, but still), so I'm cutting it off here because it
           // gets called constantly.
#ifdef G2_PERFORMANCE_ANALYSIS
    G2Time_G2_SetupModelPointers +=
        G2PerformanceTimer_G2_SetupModelPointers.End();
#endif
    return qtrue;
  }
  ...
  //	G2WARNING(ghlInfo->mModelindex != -1,"Setup request on non-used info
  //slot?");
  if (ghlInfo->mModelindex != -1) {
    ...
    G2ERROR(ghlInfo->currentModel,
            va("NULL Model (glm) %s", ghlInfo->mFileName));
    if (ghlInfo->currentModel) {
      G2ERROR(ghlInfo->currentModel->mdxm,
              va("Model has no mdxm (glm) %s", ghlInfo->mFileName));
      if (ghlInfo->currentModel->mdxm) {
        if (ghlInfo->currentModelSize) {
          if (ghlInfo->currentModelSize !=
              ghlInfo->currentModel->mdxm->ofsEnd) {
        ...
        ghlInfo->currentModelSize = ghlInfo->currentModel->mdxm->ofsEnd;
        G2ERROR(ghlInfo->currentModelSize,
                va("Zero sized Model? (glm) %s", ghlInfo->mFileName));

        ghlInfo->animModel =
            R_GetModelByHandle(ghlInfo->currentModel->mdxm->animIndex);
        G2ERROR(ghlInfo->animModel,
                va("NULL Model (gla) %s", ghlInfo->mFileName));
        if (ghlInfo->animModel) {
          ghlInfo->aHeader = ghlInfo->animModel->mdxa;
          G2ERROR(ghlInfo->aHeader,
                  va("Model has no mdxa (gla) %s", ghlInfo->mFileName));
          if (ghlInfo->aHeader) {
            if (ghlInfo->currentAnimModelSize) {
              if (ghlInfo->currentAnimModelSize != ghlInfo->aHeader->ofsEnd) {
                Com_Error(ERR_DROP, "Ghoul2 model was reloaded and has "
            ...
            ghlInfo->currentAnimModelSize = ghlInfo->aHeader->ofsEnd;
            G2ERROR(ghlInfo->currentAnimModelSize,
                    va("Zero sized Model? (gla) %s", ghlInfo->mFileName));
            ghlInfo->mValid = true;
          }
        }
      }
...
qboolean G2_SetupModelPointers(
    CGhoul2Info_v &ghoul2) // returns true if any model is properly set up
{
  bool ret = false;
  int i;
  for (i = 0; i < ghoul2.size(); i++) {
