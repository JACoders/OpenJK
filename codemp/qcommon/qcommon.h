...
void MSG_WriteDeltaPlayerstate(msg_t *msg, struct playerState_s *from,
                               struct playerState_s *to, int *bitComboDelta,
                               int *bitNumDelta, qboolean isVehiclePS = qfalse);
#else
void MSG_WriteDeltaPlayerstate(msg_t *msg, struct playerState_s *from,
...
#define MAX_MSGLEN                                                             \
  49152 // max length of a message, which may
        // be fragmented into multiple packets

// rww - 6/28/02 - Changed from 16384 to match sof2's. This does seem rather
// huge, but I guess it doesn't really hurt anything.
...
#define NUM_SERVER_PORTS                                                       \
  4 // broadcast scan this many ports after PORT_SERVER so a single machine can
...
typedef enum vmSlots_e { VM_GAME = 0, VM_CGAME, VM_UI, MAX_VM } vmSlots_t;

typedef struct vm_s {
  vmSlots_t slot; // VM_GAME, VM_CGAME, VM_UI
  char name[MAX_QPATH];
  void *dllHandle;
  qboolean isLegacy; // uses the legacy syscall/vm_call api, is set by
                     // VM_CreateLegacy
...
intptr_t QDECL VM_Call(vm_t *vm, int callNum, intptr_t arg0 = 0,
                       intptr_t arg1 = 0, intptr_t arg2 = 0, intptr_t arg3 = 0,
                       intptr_t arg4 = 0, intptr_t arg5 = 0, intptr_t arg6 = 0,
                       intptr_t arg7 = 0, intptr_t arg8 = 0, intptr_t arg9 = 0,
                       intptr_t arg10 = 0, intptr_t arg11 = 0);
...
char *Cmd_Cmd(void);
void Cmd_Args_Sanitize(size_t length = MAX_CVAR_VALUE_STRING,
                       const char *strip = "\n\r;", const char *repl = "   ");
...
cvar_t *Cvar_Set2(const char *var_name, const char *value,
                  uint32_t defaultFlags, qboolean force);
//
...
// same as Cvar_Set, but doesn't force setting the value (respects CVAR_ROM,
// etc)
...
#define RoundUp(N, M)                                                          \
  ((N) + ((unsigned int)(M)) - (((unsigned int)(N)) % ((unsigned int)(M))))
...
Z_TagMallocDebug(size, tag, #size, __FILE__, __LINE__) #define Z_Malloc(size)
Z_MallocDebug(size, #size, __FILE__, __LINE__) #define S_Malloc(size)
S_MallocDebug(size, #size, __FILE__, __LINE__) void *Z_TagMallocDebug( int size,
int tag, char *label, char *file, int line );	// NOT 0 filled memory void
*Z_MallocDebug( int size, char *label, char *file, int line );
// returns 0 filled memory void *S_MallocDebug( int size, char *label, char
*file, int line );			// returns 0 filled memory #else void
*Z_TagMalloc( int size, int tag );	// NOT 0 filled memory void *Z_Malloc(
int size );			// returns 0 filled memory void *S_Malloc( int
size );			// NOT 0 filled memory only for small allocations #endif
...
void *Z_Malloc(int iSize, memtag_t eTag, qboolean bZeroit = qfalse,
               int iAlign = 4); // return memory NOT zero-filled by default
...
inline int Round(float value) { return ((int)floorf(value + 0.5f)); }
