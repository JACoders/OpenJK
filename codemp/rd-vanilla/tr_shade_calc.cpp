...
#define WAVEVALUE(table, base, amplitude, phase, freq)                         \
  ((base) +                                                                    \
   table[Q_ftol((((phase) + tess.shaderTime * (freq)) * FUNCTABLE_SIZE)) &     \
         FUNCTABLE_MASK] *                                                     \
       (amplitude))
...
void RB_CalcBulgeVertexes(deformStage_t *ds) {
  ...
  for ( i = 0; i < tess.numVertexes; i++, xyz += 4, st += 2 * NUM_TEX_COORDS,
  normal += 4 ) { int		off; float scale;

          off = (float)( FUNCTABLE_SIZE / (M_PI*2) ) * ( st[0] * ds->bulgeWidth
  + now );
...
static void AutospriteDeform(void) {
  ...
  if (tess.numVertexes & 3) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "Autosprite shader %s had odd vertex count",
              tess.shader->name);
  }
  if (tess.numIndexes != (tess.numVertexes >> 2) * 6) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "Autosprite shader %s had odd index count",
              tess.shader->name);
  }
...
static void Autosprite2Deform(void) {
  ...
  if (tess.numVertexes & 3) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "Autosprite2 shader %s had odd vertex count",
              tess.shader->name);
  }
  if (tess.numIndexes != (tess.numVertexes >> 2) * 6) {
    ri.Printf(PRINT_ALL,
              S_COLOR_YELLOW "Autosprite2 shader %s had odd index count",
              tess.shader->name);
  }
  ...
  for (i = 0, indexes = 0; i < tess.numVertexes; i += 4, indexes += 6) {
    ...
    for (j = 0; j < 2; j++) {
      ...
      for (k = 0; k < 5; k++) {
        if (tess.indexes[indexes + k] ==
                (unsigned)(i + edgeVerts[nums[j]][0]) &&
            tess.indexes[indexes + k + 1] ==
                (unsigned)(i + edgeVerts[nums[j]][1])) {
          break;
        }
      }
