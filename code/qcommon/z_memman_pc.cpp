...
int Z_Validate(void) {
	...
	while (pMemory) {
		...
		if (pMemory->eTag != TAG_IMAGE_T && pMemory->eTag != TAG_MODEL_MD3 && pMemory->eTag != TAG_MODEL_GLM &&
...
const static StaticZeroMem_t gZeroMalloc = {{ZONE_MAGIC, TAG_STATIC, 0, NULL, NULL}, {ZONE_MAGIC}};
...
const static StaticMem_t gNumberString[] = {
	{DEF_STATIC('0')}, {DEF_STATIC('1')}, {DEF_STATIC('2')}, {DEF_STATIC('3')}, {DEF_STATIC('4')},
	{DEF_STATIC('5')}, {DEF_STATIC('6')}, {DEF_STATIC('7')}, {DEF_STATIC('8')}, {DEF_STATIC('9')},
};
...
void *openjk_minizip_malloc(int size) { return Z_Malloc(size, TAG_MINIZIP, qfalse); }
...
#ifdef DEBUG_ZONE_ALLOCS
void *_D_S_Malloc(int iSize, const char *psFile, int iLine) { return _D_Z_Malloc(iSize, TAG_SMALL, qfalse, psFile, iLine); }
#else
...
static void Z_TagDebug_f(void) {
	...
	while (pMemory) {
		if ((pMemory->eTag == eTag || eTag == TAG_ALL) &&
			(!bSnapShotTestActive ||
			 (pMemory->iSnapshotNumber == giZoneSnaphotNum && AllTagBlockLabels_Local[psTagStrings[pMemory->eTag]][pMemory->sOptionalLabel] < 0))) {
			...
			Com_Printf(" %9d (%2d.%02dMB) File: \"%s\", Line: %d\n", pMemory->iSize, iSize, iRemainder, pMemory->sSrcFileBaseName, pMemory->iSrcFileLineNum);
