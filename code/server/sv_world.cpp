...
void SV_LinkEntity(gentity_t *gEnt) {
  ...
  // set the abs box
  if (gEnt->bmodel &&
      (angles[0] || angles[1] || angles[2])) { // expand for rotation
    float max;
    int i;
  ...
  num_leafs = CM_BoxLeafnums(gEnt->absmin, gEnt->absmax, leafs,
  ...
  for (i = 0; i < num_leafs; i++) {
    ...
    if (area != -1) { // doors may legally straggle two areas,
      // but nothing should evern need more than that
      if (ent->areanum != -1 && ent->areanum != area) {
        if (ent->areanum2 != -1 && ent->areanum2 != area &&
            sv.state == SS_LOADING) {
          Com_DPrintf("Object %i touching 3 areas at %f %f %f\n",
                      gEnt->s.number, gEnt->absmin[0], gEnt->absmin[1],
                      gEnt->absmin[2]);
        }
        ent->areanum2 = area;
      } else {
...
void SV_AreaEntities_r(worldSector_t *node, areaParms_t *ap) {
  ...
  for (check = node->entities; check; check = next) {
    ...
    if (gcheck->absmin[0] > ap->maxs[0] || gcheck->absmin[1] > ap->maxs[1] ||
        gcheck->absmin[2] > ap->maxs[2] || gcheck->absmax[0] < ap->mins[0] ||
        gcheck->absmax[1] < ap->mins[1] || gcheck->absmax[2] < ap->mins[2]) {
      continue;
    }
...
================
*/
int SV_AreaEntities(const vec3_t mins, const vec3_t maxs, gentity_t **elist,
                    int maxcount) {
  areaParms_t ap;
...
void SV_AreaEntitiesTree(worldSector_t *node, areaParms_t *ap, int level) {
  ...
  entStats.insert(pair<int, pair<int, list<CBBox>>>(
      level, pair<int, list<CBBox>>(count, bblist)));
  if (node->axis == -1) {
    return; // terminal node
  }
  ...
  for (j = entStats.begin(); j != entStats.end(); j++) {
    ...
    for (k = (*j).second.second.begin(); k != (*j).second.second.end(); k++) {
      sprintf(mess, "mins=%f %f %f, maxs=%f %f %f\n", (*k).mMins[0],
              (*k).mMins[1], (*k).mMins[2], (*k).mMaxs[0], (*k).mMaxs[1],
              (*k).mMaxs[2]);
      OutputDebugString(mess);
    }
  }
...
typedef struct {
  ...
  */
  ...
  int useLod;
  trace_t
      trace; // make sure nothing goes under here for Ghoul2 collision purposes
  /*
  Ghoul2 Insert End
  */
} moveclip_t;
...
void SV_ClipMoveToEntities(moveclip_t *clip) {
  ...
  for (i = 0; i < num; i++) {
...
#endif
    {
      ...
      // ALSO: 2 players can be standing next to each other and this function
      // will think they're in each other!!!
    }
    oldTrace = clip->trace;

    if (trace.allsolid) {
      if (!clip->trace.allsolid) { // We didn't come in here all solid, so set
                                   // the clip->trace's entityNum
        clip->trace.entityNum = touch->s.number;
      }
      clip->trace.allsolid = qtrue;
      trace.entityNum = touch->s.number;
    } else if (trace.startsolid) {
      if (!clip->trace.startsolid) { // We didn't come in here starting solid,
                                     // so set the clip->trace's entityNum
        clip->trace.entityNum = touch->s.number;
      }
    ...
    */
    ...
    if ((trace.entityNum == touch->s.number) &&
      ...
      if (touch->ghoul2.size() && !(touch->contents & CONTENTS_LIGHTSABER)) {
        ...
        // we have to do this because sometimes you may hit a model's bounding
        // box, but not actually penetrate the Ghoul2 Models polygons this is,
        // needless to say, not good. So we must check to see if we did actually
        // hit the model, and if not, reset the trace stuff to what it was to
        // begin with
        ...
        // if we are looking at an entity then use the player state to get it's
        // angles and origin from
        float radius;
#if 0 // G2_SUPERSIZEDBBOX is not being used
				if (clip->eG2TraceType == G2_SUPERSIZEDBBOX)
				{
        ...
        if (touch->client) {
          ...
          re.G2API_CollisionDetect(clip->trace.G2CollisionMap, touch->ghoul2,
                                   world_angles, touch->client->origin, sv.time,
                                   touch->s.number, clip->start, clip->end,
                                   touch->s.modelScale, G2VertSpaceServer,
        ...
        // no, so use the normal entity state
          ...
          re.G2API_CollisionDetect(
              clip->trace.G2CollisionMap, touch->ghoul2, touch->currentAngles,
              touch->currentOrigin, sv.time, touch->s.number, clip->start,
              clip->end, touch->s.modelScale, G2VertSpaceServer,
              clip->eG2TraceType, clip->useLod, radius);
        }
        ...
        // did we actually touch this model? If not, lets reset this ent as
        // being hit..
        if (newTraceRecSize == oldTraceRecSize) {
          clip->trace = oldTrace;
        } else // this trace was valid, so copy the best collision into quake
               // trace place info
        {
          for (z = 0; z < MAX_G2_COLLISIONS; z++) {
            if (clip->trace.G2CollisionMap[z].mEntityNum == touch->s.number) {
              clip->trace.plane.normal[0] =
                  clip->trace.G2CollisionMap[z].mCollisionNormal[0];
              clip->trace.plane.normal[1] =
                  clip->trace.G2CollisionMap[z].mCollisionNormal[1];
              clip->trace.plane.normal[2] =
                  clip->trace.G2CollisionMap[z].mCollisionNormal[2];
              break;
            }
          }
...
*/
#ifdef _DEBUG
  assert(!Q_isnan(start[0]) && !Q_isnan(start[1]) && !Q_isnan(start[2]) &&
         !Q_isnan(end[0]) && !Q_isnan(end[1]) && !Q_isnan(end[2]));
#endif // _DEBUG
  ...
  */
  ...
  // clip to world
  // NOTE: this will stop not only on static architecture but also entity
  // brushes such as doors, etc.  This prevents us from being able to shorten the
  // trace so that we can ignore all ents past this endpoint... perhaps need to
  // check the entityNum in this BoxTrace or have it not clip against entity
  // brushes here.
  ...
  if (clip.trace.fraction == 0) { // blocked immediately by the world
    *results = clip.trace;
    //		goto addtime;
    return;
  }
