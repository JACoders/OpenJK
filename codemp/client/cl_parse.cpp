...
void CL_ParseSnapshot(msg_t *msg) {
	...
	if (old) {
		...
		if (newSnap.ps.m_iVehicleNum) { // this means we must have written our vehicle's ps too
...
void CL_ParseSetGame(msg_t *msg) {
	...
	while (i < MAX_QPATH) {
		...
		if (next) { // if next is 0 then we have finished reading to the end of the message
			newGameDir[i] = next;
		} else {
			break;
		}
...
int CL_GetValueForHidden(const char *s) { // string arg here just in case I want to add more sometime and make a lookup table
