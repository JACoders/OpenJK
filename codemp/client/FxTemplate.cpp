...
bool CPrimitiveTemplate::ParseVector(const char *val, vec3_t min, vec3_t max) {
  ...
  int v = sscanf(val, "%f %f %f   %f %f %f", &min[0], &min[1], &min[2], &max[0],
                 &max[1], &max[2]);
...
bool CPrimitiveTemplate::ParseGroupFlags(const char *val, int *flags) {
  ...
  *flags = 0;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    else { // we have badness going on, but continue on in case there are any
...
bool CPrimitiveTemplate::ParseFlags(const char *val) {
  ...
  int v = sscanf(val, "%s %s %s %s %s %s %s", flag[0], flag[1], flag[2],
  ...
  for (int i = 0; i < 7; i++) {
    ...
    // rww - end
      ...
      mFlags |=
          FX_PAPER_PHYSICS; // warning! shared flag.  You use this with a
                            // cylinder and you can expect evilness to ensue
    else if (!Q_stricmp(flag[i], "localizedFlash"))
      mFlags |=
          FX_LOCALIZED_FLASH; // warning! shared flag.  You use this with a
                              // cylinder and you can expect evilness to ensue
    else if (!Q_stricmp(flag[i], "playerView"))
      mFlags |= FX_PLAYER_VIEW; // warning! shared flag.  You use this with a
                                // cylinder and you can expect evilness to ensue
    else { // we have badness going on, but continue on in case there are any
           // valid fields in here
      ok = false;
...
bool CPrimitiveTemplate::ParseMaterialImpact(const char *val) {
  if (!Q_stricmp(val, "shellsound")) {
    ...
    mMatImpactFX = MATIMPACTFX_NONE;
    theFxHelper.Print("CPrimitiveTemplate::ParseMaterialImpact -- unknown "
                      "materialImpact type!\n");
    return false;
  }
  return true;
...
bool CPrimitiveTemplate::ParseImpactFxStrings(CGPValue *grp) {
  ...
  if (grp->IsList()) {
    ...
    if (val) {
      ...
      // empty "list"
      theFxHelper.Print("CPrimitiveTemplate::ParseImpactFxStrings called with "
                        "an empty list!\n");
      return false;
    }
  }
...
bool CPrimitiveTemplate::ParseDeathFxStrings(CGPValue *grp) {
  ...
  if (grp->IsList()) {
    ...
    if (val) {
      ...
      // empty "list"
      theFxHelper.Print("CPrimitiveTemplate::ParseDeathFxStrings called with "
                        "an empty list!\n");
      return false;
    }
  }
...
bool CPrimitiveTemplate::ParseEmitterFxStrings(CGPValue *grp) {
  ...
  if (grp->IsList()) {
    ...
    if (val) {
      ...
      // empty "list"
      theFxHelper.Print("CPrimitiveTemplate::ParseEmitterFxStrings called with "
                        "an empty list!\n");
      return false;
    }
  }
...
bool CPrimitiveTemplate::ParsePlayFxStrings(CGPValue *grp) {
  ...
  if (grp->IsList()) {
    ...
    if (val) {
      ...
      // empty "list"
      theFxHelper.Print("CPrimitiveTemplate::ParsePlayFxStrings called with an "
                        "empty list!\n");
      return false;
    }
  }
...
bool CPrimitiveTemplate::ParsePrimitive(CGPGroup *grp) {
  ...
  while (pairs) {
    ...
    if (!Q_stricmp(key, "count"))
      ...
      //			mCullRange = atoi( val );
      //			mCullRange *= mCullRange; // square it now so we
      //don't have to square every time we compare
    } else if (!Q_stricmp(key, "bounce") ||
               !Q_stricmp(key, "intensity")) // me==bad for reusing this...but
    ...
    // these need to get passed on to the primitive
      ...
      ParseFlags(val);
    // these are used to spawn things in cool ways, but don't ever get passed on
    // to prims.
