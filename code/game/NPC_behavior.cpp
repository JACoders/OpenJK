...
void NPC_BSFollowLeader_UpdateEnemy(void) {
	if (!NPC->enemy) {															 // no enemy, find one
		...
		if (NPC->enemy->health <= 0 || (NPC->enemy->flags & FL_NOTARGET)) {
			...
			NPC_CheckEnemy((qboolean)(NPCInfo->confusionTime < level.time || NPCInfo->tempBehavior != BS_FOLLOW_LEADER),
	...
	if (enemyVisibility > VIS_PVS) {											// face
		...
		if (enemyVisibility >= VIS_SHOOT) { // shoot
			...
			if (NPC_GetHFOVPercentage(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, NPCInfo->stats.hfov) > 0.6f &&
				NPC_GetHFOVPercentage(NPC->enemy->currentOrigin, NPC->currentOrigin, NPC->client->ps.viewangles, NPCInfo->stats.vfov) >
...
bool NPC_BSFollowLeader_InFullBodyAttack(void) {
	return (NPC->client->ps.legsAnim == BOTH_ATTACK1 || NPC->client->ps.legsAnim == BOTH_ATTACK2 || NPC->client->ps.legsAnim == BOTH_ATTACK3 ||
			NPC->client->ps.legsAnim == BOTH_MELEE1 || NPC->client->ps.legsAnim == BOTH_MELEE2);
}
...
void NPC_BSJump(void) {
	...
	case JS_JUMPING:
		...
		if (NPC->s.groundEntityNum != ENTITYNUM_NONE) { // Landed, start landing anim
			...
			// FIXME: landsound?
		} else if (NPC->client->ps.legsAnimTimer > 0) { // Still playing jumping anim
...
void NPC_BSSearch(void) {
	...
	if ((NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES) && NPC->client->enemyTeam != TEAM_NEUTRAL) { // look for enemies
		NPC_CheckEnemy(qtrue, qfalse);
...
void NPC_BSWander(void) { // FIXME: don't actually go all the way to the next waypoint, just move in fits and jerks...?
	...
	if ((NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES) && NPC->client->enemyTeam != TEAM_NEUTRAL) { // look for enemies
		NPC_CheckEnemy(qtrue, qfalse);
		if (NPC->enemy) {							  // found one
			if (NPCInfo->tempBehavior == BS_WANDER) { // if tempbehavior, set tempbehavior to default
				NPCInfo->tempBehavior = BS_DEFAULT;
			} else {								 // if bState, change to run and shoot
				NPCInfo->behaviorState = BS_DEFAULT; // BS_HUNT_AND_KILL;
				// NPC_BSRunAndShoot();
	...
	if (!HasPath) {
		...
		//------------------------------------------------------
		if (NPCInfo->investigateDebounceTime < level.time || ((NPCInfo->aiFlags & NPCAI_BLOCKED) && (level.time - NPCInfo->blockedDebounceTime) > 1000)) {
...
void NPC_Surrender(void) { // FIXME: say "don't shoot!" if we weren't already surrendering
	...
	if (NPC->s.weapon != WP_NONE && NPC->s.weapon != WP_MELEE && NPC->s.weapon != WP_SABER) {
	...
	//----------------------------------------------
		...
		if (NPC->client->NPC_class == CLASS_JAWA && NPC->client->ps.weapon == WP_NONE) { // an unarmed Jawa is very scared
			NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_COWER1, SETANIM_FLAG_HOLD | SETANIM_FLAG_OVERRIDE);
...
qboolean NPC_CheckSurrender(void) {
	if (!g_AIsurrender->integer && NPC->client->NPC_class != CLASS_UGNAUGHT && NPC->client->NPC_class != CLASS_JAWA) { // not enabled
		return qfalse;
	}
	if (!Q3_TaskIDPending(NPC, TID_MOVE_NAV)								// not scripted to go somewhere
		...
		(NPC->enemy->s.weapon != WP_MELEE ||
		 (NPC->enemy->client->NPC_class == CLASS_RANCOR || NPC->enemy->client->NPC_class == CLASS_WAMPA)) // enemy is using a weapon or is a Rancor or Wampa
		&& NPC->enemy->health > 20 && NPC->enemy->painDebounceTime < level.time - 3000 &&
		...
		if (NPC->s.weapon != WP_ROCKET_LAUNCHER && NPC->s.weapon != WP_CONCUSSION && NPC->s.weapon != WP_REPEATER && NPC->s.weapon != WP_FLECHETTE &&
			...
			if (NPC->s.weapon != WP_NONE) { // they have a weapon so they'd have to drop it to surrender
				...
				if (g_crosshairEntNum == NPC->s.number && NPC->painDebounceTime > level.time) { // if he just shot me, always give up
																								// fall through
				} else {							   // don't give up unless facing enemy and he's very close
					if (!InFOV(player, NPC, 60, 30)) { // I'm not looking at them
						return qfalse;
					} else if (DistanceSquared(NPC->currentOrigin, player->currentOrigin) < 65536 /*256*256*/) { // they're not close
						return qfalse;
					} else if (!gi.inPVS(NPC->currentOrigin, player->currentOrigin)) { // they're not in the same room
						return qfalse;
					}
				}
			}
			if (!NPCInfo->group ||
				(NPCInfo->group && NPCInfo->group->numGroup <= 1)) { // I'm alone but I was in a group//FIXME: surrender anyway if just melee or no weap?
				if (NPC->s.weapon == WP_NONE
					// NPC has a weapon
					|| NPC->enemy == player || (NPC->enemy->s.weapon == WP_SABER && NPC->enemy->client && NPC->enemy->client->ps.SaberActive()) ||
					(NPC->enemy->NPC && NPC->enemy->NPC->group &&
					 NPC->enemy->NPC->group->numGroup >
						 2)) {					// surrender only if have no weapon or fighting a player or jedi or if we are outnumbered at least 3 to 1
					if (NPC->enemy == player) { // player is the guy I'm running from
						if (g_crosshairEntNum == NPC->s.number) { // give up if player is aiming at me
							NPC_Surrender();
							NPC_UpdateAngles(qtrue, qtrue);
							return qtrue;
						} else if (player->s.weapon == WP_SABER) {													  // player is using saber
							if (InFOV(NPC, player, 60, 30)) {														  // they're looking at me
								if (DistanceSquared(NPC->currentOrigin, player->currentOrigin) < 16384 /*128*128*/) { // they're close
									if (gi.inPVS(NPC->currentOrigin, player->currentOrigin)) {						  // they're in the same room
										NPC_Surrender();
										NPC_UpdateAngles(qtrue, qtrue);
										return qtrue;
									}
						...
						if (InFOV(NPC, NPC->enemy, 30, 30)) { // they're looking at me
							...
							maxDist *= maxDist;
							if (DistanceSquared(NPC->currentOrigin, NPC->enemy->currentOrigin) < maxDist) { // they're close
								if (gi.inPVS(NPC->currentOrigin, NPC->enemy->currentOrigin)) {				// they're in the same room
...
void NPC_JawaFleeSound(void) {
	if (NPC && NPC->client && NPC->client->NPC_class == CLASS_JAWA && !Q_irand(0, 3) && NPCInfo->blockedSpeechDebounceTime < level.time &&
...
qboolean NPC_BSFlee(void) {
	...
	if (enemyRecentlySeen &&												 // Is There An Enemy Near?
		...
		TIMER_Done(NPC, "CheckForWeaponToPickup") && G_CanPickUpWeapons(NPC) // Allowed To Pick Up Dropped Weapons
	) {
	...
	//-------------------------------------------------------------------
	if (NPCInfo->goalEntity && !Q3_TaskIDPending(NPC, TID_MOVE_NAV) && NPC->enemy &&
	...
	STEER::Activate(NPC);
	{
		...
		//---------------------------------------------------------
		if (enemyRecentlySeen && Distance(NPC->enemy->currentOrigin, NPC->currentOrigin) < enemyTooCloseDist) {
	...
	if (enemyRecentlySeen) {
		...
		if ((!hasEscapePoint || reachedEscapePoint) && TIMER_Done(NPC, "FindNewEscapePointDebounce")) {
			...
			int escapePoint =
				NPC_FindCombatPoint(NPC->currentOrigin, NPC->enemy->currentOrigin, NPC->currentOrigin, CP_COVER | CP_AVOID_ENEMY | CP_HAS_ROUTE, 128);
			if (escapePoint != -1) {
				NPC_JawaFleeSound();
	...
	//---------------------------------------------------------------------
	if (NPCInfo->tempBehavior == BS_FLEE && TIMER_Done(NPC, "flee") && NPC->s.weapon != WP_NONE && NPC->s.weapon != WP_MELEE) {
		NPCInfo->tempBehavior = BS_DEFAULT;
	}
...
void NPC_StartFlee(gentity_t *enemy, vec3_t dangerPoint, int dangerLevel, int fleeTimeMin, int fleeTimeMax) {
	if (Q3_TaskIDPending(NPC, TID_MOVE_NAV)) { // running somewhere that a script requires us to go, don't interrupt that!
		return;
	}
