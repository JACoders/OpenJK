...
void CM_TraceThroughBrush(traceWork_t *tw, cbrush_t *brush) {
  ...
  if (!startout) { // original point was inside brush
    ...
    tw->trace.contents |=
        brush->contents; // note, we always want to know the contents of
                         // something we're inside of
    if (!getout) { // endpoint was inside brush
...
==================
*/
void CM_TraceThroughTree(traceWork_t *tw, clipMap_t *local, int num, float p1f,
...
void CM_TransformedBoxTrace(trace_t *results, const vec3_t start,
  ...
  CM_BoxTrace(&trace, start_l, end_l, symetricSize[0], symetricSize[1], model,
              brushmask);
...
bool CM_CullWorldBox(const cplane_t *frustum, const vec3pair_t bounds) {
  ...
  // rwwFIXMEFIXME: Was not ! before. But that seems the way it should be and it
  // works that way. Why?
