...
void AI_GroupMemberKilled(gentity_t *self) {
  /*	AIGroupInfo_t *group = self->NPC->group;
          ...
          //go through and drop aggression on my teammates (more cover, worse
     aim) for ( int i = 0; i < group->numGroup; i++ )
          {
                  member = &g_entities[group->member[i].number];
                  if ( member == self )
                  {
                  ...
                  else
                  {
                          ...
                          member->NPC->currentAim -= Q_irand( 0, 10 );//Q_irand(
     0, 2);//drop their aim accuracy
                  }
          }
          //okay, if I'm the group commander, make everyone else flee
          if ( group->commander != self )
          {//I'm not the commander... hmm, should maybe a couple flee... maybe
     those near me? return;
          }
          //now see if there is another of sufficient rank to keep them from
     fleeing if ( !noflee )
          {
                  ...
                  for ( int i = 0; i < group->numGroup; i++ )
                  {
                          ...
                          if ( member->NPC->rank < RANK_ENSIGN )
                          {//grunt
                                  if ( group->enemy && DistanceSquared(
     member->currentOrigin, group->enemy->currentOrigin ) < 65536 )//256*256
                                  {//those close to enemy run away!
                                          ST_StartFlee( member, group->enemy,
     member->currentOrigin, AEL_DANGER_GREAT, 3000, 5000 );
                                  }
                                  else if ( DistanceSquared(
     member->currentOrigin, self->currentOrigin ) < 65536 )
     ...
     member->NPC->rank )
                                          {//lower rank they are, higher rank I
     am, more likely they are to flee ST_StartFlee( member, group->enemy,
     member->currentOrigin, AEL_DANGER_GREAT, 3000, 5000 );
                                          }
                                          else
                                          {
     ...
     //Q_irand( 1, 3 );//drop their aim accuracy even more
                          }
                          member->NPC->currentAim -= Q_irand( 1, 15 );
     //Q_irand( 1, 3 );//drop their aim accuracy even more
                  }
