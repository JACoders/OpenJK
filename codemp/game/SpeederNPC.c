...
static void ProcessMoveCommands(Vehicle_t *pVeh) {
	...
	if (parentPS->speed || parentPS->groundEntityNum == ENTITYNUM_NONE || pVeh->m_ucmd.forwardmove || pVeh->m_ucmd.upmove > 0) {
		...
		if (!pVeh->m_pVehicleInfo->strafePerc ||
			(0 && pVeh->m_pParentEntity->s.number < MAX_CLIENTS)) { // if in a strafe-capable vehicle, clear strafing unless using alternate control scheme
...
void AnimateRiders(Vehicle_t *pVeh) {
	...
	if (pVeh->m_iBoarding != 0) {
		...
		if (pVeh->m_iBoarding < 0) {
			...
			BG_SetAnim(pVeh->m_pPilot->playerState, bgAllAnims[pVeh->m_pPilot->localAnimIndex].anims, SETANIM_BOTH, Anim, iFlags);
		}
	...
	BG_SetAnim(pVeh->m_pPilot->playerState, bgAllAnims[pVeh->m_pPilot->localAnimIndex].anims, SETANIM_BOTH, Anim, iFlags | SETANIM_FLAG_HOLD);
}
