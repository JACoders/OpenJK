...
#define CONTENTS_SOLID                                                         \
  (0x00000001u) // Default setting. An eye is never valid in a solid
...
#define CONTENTS_MONSTERCLIP                                                   \
  (0x00000020u) // NPCs cannot physically pass through
#define CONTENTS_BOTCLIP                                                       \
  (0x00000040u) // do not enter - NPCs try not to enter these
...
#define CONTENTS_CORPSE                                                        \
  (0x00000200u) // should never be on a brush, only in game
#define CONTENTS_TRIGGER (0x00000400u) //
#define CONTENTS_NODROP                                                        \
  (0x00000800u) // don't leave bodies or items (death fog, lava)
#define CONTENTS_TERRAIN (0x00001000u) // volume contains terrain data
#define CONTENTS_LADDER (0x00002000u)  //
#define CONTENTS_ABSEIL                                                        \
  (0x00004000u) // used like ladder to define where an NPC can abseil
#define CONTENTS_OPAQUE                                                        \
  (0x00008000u) // defaults to on, when off, solid can be seen through
#define CONTENTS_OUTSIDE                                                       \
  (0x00010000u) // volume is considered to be in the outside (i.e. not indoors)
...
#define CONTENTS_INSIDE                                                        \
  (0x10000000u) // volume is considered to be inside (i.e. indoors)
#define CONTENTS_UNUSED20000000 (0x20000000u) //
#define CONTENTS_UNUSED40000000 (0x40000000u) //
#define CONTENTS_TRANSLUCENT                                                   \
  (0x80000000u) // don't consume surface fragments inside
...
#define SURF_METALSTEPS                                                        \
  (0x00008000u) // CHC needs this since we use same tools (though this flag is
                // temp?)
#define SURF_FORCEFIELD                                                        \
  (0x00010000u)                           // CHC ""			(but not temp)
...
#define SURF_NODLIGHT                                                          \
  (0x00800000u) // don't dlight even if solid (solid lava, skies)
#define SURF_NOMISCENTS                                                        \
  (0x01000000u) // no client models allowed on this surface
...
#define MATERIALS                                                              \
  "none", "solidwood", "hollowwood", "solidmetal", "hollowmetal",              \
      "shortgrass", "longgrass", "dirt", "sand", "gravel", "glass",            \
      "concrete", "marble", "water", "snow", "ice", "flesh", "mud", "bpglass", \
      "dryleaves", "greenleaves", "fabric", "canvas", "rock", "rubber",        \
      "plastic", "tiles", "carpet", "plaster", "shatterglass", "armor",        \
      "computer" /* this was missing, see enums above, plus ShaderEd2 pulldown \
                    options */
