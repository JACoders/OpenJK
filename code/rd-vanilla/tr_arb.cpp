...
/***********************************************************************************************************/
// This vertex shader basically passes through most values and calculates no
// lighting. The only unusual thing it does is add the inputed texel offsets to
// all four texture units (this allows nearest neighbor pixel peeking).
const unsigned char g_strGlowVShaderARB[] = {"!!ARBvp1.0\
	\
	# Input.\n\
...
// This Pixel Shader loads four texture units and adds them all together (with a
// modifier multiplied to each in the process). The final output is r0 = t0 + t1
// + t2 + t3.
const unsigned char g_strGlowPShaderARB[] = {"!!ARBfp1.0\
	\
	# Input.\n\
...
void ARB_InitGlowShaders(void) {
  ...
  if (qglGenProgramsARB) {
    ...
    qglProgramStringARB(GL_VERTEX_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB,
                        (GLsizei)strlen((char *)g_strGlowVShaderARB),
                        g_strGlowVShaderARB);
  ...
  // NOTE: I make an assumption here. If you have (current) nvidia hardware, you
  // obviously support register combiners instead of fragment programs, so use
  // those. The problem with this is that nv30 WILL support fragment shaders,
  // breaking this logic. The good thing is that if you always ask for regcoms
  // before fragment shaders, you'll always just use regcoms (problem solved...
  // for now). - AReis

  // Load Pixel Shaders (either regcoms or fragprogs).
  if (qglCombinerParameteriNV) {
    // The purpose of this regcom is to blend all the pixels together from the 4
    // texture units, but with their texture coordinates offset by 1 (or more)
    // texels, effectively letting us blend adjoining pixels. The weight is used
    // to either strengthen or weaken the pixel intensity. The more it diffuses
    // (the higher the radius of the glow), the higher the intensity should be
    // for a noticable effect. Regcom result is: ( tex1 * fBlurWeight ) + ( tex2
    // * fBlurWeight ) + ( tex2 * fBlurWeight ) + ( tex2 * fBlurWeight )
    ...
    */
    ...
    qglCombinerInputNV(GL_COMBINER0_NV, GL_RGB, GL_VARIABLE_D_NV,
                       GL_CONSTANT_COLOR0_NV, GL_UNSIGNED_IDENTITY_NV, GL_RGB);
    qglCombinerOutputNV(GL_COMBINER0_NV, GL_RGB, GL_DISCARD_NV, GL_DISCARD_NV,
                        GL_SPARE0_NV, GL_NONE, GL_NONE, GL_FALSE, GL_FALSE,
                        GL_FALSE);
    ...
    qglCombinerInputNV(GL_COMBINER1_NV, GL_RGB, GL_VARIABLE_B_NV,
                       GL_CONSTANT_COLOR0_NV, GL_UNSIGNED_IDENTITY_NV, GL_RGB);
    qglCombinerInputNV(GL_COMBINER1_NV, GL_RGB, GL_VARIABLE_C_NV,
                       GL_TEXTURE3_ARB, GL_UNSIGNED_IDENTITY_NV, GL_RGB);
    qglCombinerInputNV(GL_COMBINER1_NV, GL_RGB, GL_VARIABLE_D_NV,
                       GL_CONSTANT_COLOR0_NV, GL_UNSIGNED_IDENTITY_NV, GL_RGB);
    qglCombinerOutputNV(GL_COMBINER1_NV, GL_RGB, GL_DISCARD_NV, GL_DISCARD_NV,
                        GL_SPARE1_NV, GL_NONE, GL_NONE, GL_FALSE, GL_FALSE,
                        GL_FALSE);

    // ( A * B ) + ( ( 1 - A ) * C ) + D = ( spare0 * 1 ) + ( ( 1 - spare0 ) * 0
    // ) + spare1 == spare0 + spare1.
    qglFinalCombinerInputNV(GL_VARIABLE_A_NV, GL_SPARE0_NV,
                            GL_UNSIGNED_IDENTITY_NV, GL_RGB);
    qglFinalCombinerInputNV(GL_VARIABLE_B_NV, GL_ZERO, GL_UNSIGNED_INVERT_NV,
                            GL_RGB);
    qglFinalCombinerInputNV(GL_VARIABLE_C_NV, GL_ZERO, GL_UNSIGNED_IDENTITY_NV,
                            GL_RGB);
    qglFinalCombinerInputNV(GL_VARIABLE_D_NV, GL_SPARE1_NV,
                            GL_UNSIGNED_IDENTITY_NV, GL_RGB);
    qglEndList();
    ...
    qglProgramStringARB(GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB,
                        (GLsizei)strlen((char *)g_strGlowPShaderARB),
                        g_strGlowPShaderARB);
