    ...
    if (self->client->ps.weapon == WP_NONE &&
      ...
      if (self->client->ps.stats[STAT_WEAPONS] & (1 << WP_BLASTER)) {
        ...
        G_CreateG2AttachedWeaponModel(self,
                                      weaponData[WP_BLASTER_PISTOL].weaponMdl);
      }
    }
    return;
  }
  ...
  if (changing && NPC->client->ps.weapon != WP_NONE) {
    if (NPC->client->ps.weapon == WP_SABER) {
      ...
      G_CreateG2AttachedWeaponModel(
          NPC, weaponData[NPC->client->ps.weapon].weaponMdl);
    }
  }
}
/*
...
float NPC_MaxDistSquaredForWeapon(void) {
  ...
  case WP_SABER:
    if (NPC->client &&
        NPC->client->ps.saberLength) { // FIXME: account for whether enemy and I
    ...
    if (newenemy->client && !(newenemy->flags & FL_NOTARGET) &&
      ...
      if (newenemy->health > 0) {
        if (enemyTeam == TEAM_PLAYER ||
            ...
            (enemyTeam ==
             TEAM_PLAYER)) { // FIXME:  check for range and FOV or vis?
...
void CP_FindCombatPointWaypoints(void) {
  for (int i = 0; i < level.numCombatPoints; i++) {
    level.combatPoints[i].waypoint =
        NAV_FindClosestWaypointForPoint(level.combatPoints[i].origin);
#ifndef FINAL_BUILD
...
int NPC_FindCombatPoint(const vec3_t position, const vec3_t avoidPosition,
                        vec3_t enemyPosition, const int flags, float avoidDist,
  ...
  STL_ITERATE(cpi, points) {
    ...
    if ((flags & CP_AVOID) &&
        (DistanceSquared(level.combatPoints[i].origin, position) <
         avoidDist)) // was using MIN_AVOID_DISTANCE_SQUARED, not passed in
                     // avoidDist
      continue;
      ...
      if (flags & CP_HORZ_DIST_COLL) {
        ...
        if ((*cpi).first <
            DistanceSquared(position,
                            enemyPosition)) { // it's closer, don't use it
          continue;
        }
      }
    }
    ...
    if (flags & CP_HAS_ROUTE) {
      ...
      */
      ...
      if (waypoint == WAYPOINT_NONE ||
          ...
          navigator.GetBestNodeAltRoute(waypoint,
                                        level.combatPoints[i].waypoint) ==
              WAYPOINT_NONE) { // can't possibly have a route to any OR can't
                               // possibly have a route to this one OR don't have
                               // a route to this one
        if (!NAV_ClearPathToPoint(NPC, NPC->mins, NPC->maxs,
                                  level.combatPoints[i].origin, NPC->clipmask,
        ...
        if (dist < bestDist) {
          if (!navigator.GetBestPathBetweenEnts(NPC, found, NF_CLEAR_PATH) ||
            ...
            if (NAV_ClearPathToPoint(
                    NPC, NPC->mins, NPC->maxs, found->currentOrigin,
                    NPC->clipmask,
    ...
    if (TIMER_Done(NPC, "panic") &&
      ...
      if (foundWeap) { // try to nav to it
        ...
        if ( !navigator.GetBestPathBetweenEnts( NPC, foundWeap, NF_CLEAR_PATH )
                || navigator.GetBestNodeAltRoute( NPC->waypoint,
        ...
        NPC, NPC->mins, NPC->maxs, foundWeap->currentOrigin, NPC->clipmask,
        ENTITYNUM_NONE ) )
