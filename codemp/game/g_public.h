...
#define SVF_BROADCASTCLIENTS                                                   \
  0x00000002                         // only broadcast to clients specified in
                                     // r.broadcastClients[clientNum/32]
...
#define SVF_OWNERNOTSHARED                                                     \
  0x00001000 // If it's owned by something and another thing owned by that
             // something hits it, it will still touch
...
#define G2TRFLAG_HITCORPSES                                                    \
  0x00000002 // will try g2 collision on the ent even if it's EF_DEAD
#define G2TRFLAG_GETSURFINDEX                                                  \
  0x00000004 // will replace surfaceFlags with the ghoul2 surface index that was
             // hit, if any.
#define G2TRFLAG_THICK                                                         \
  0x00000008 // assures that the trace radius will be significantly large
             // regardless of the trace box size.
...
typedef struct entityShared_s {
  ...
  // the first 32 clients are represented by the first array index and the
  // latter 32 clients are represented by the
  //	second array index.
...
typedef struct sharedEntity_s {
  ...
  playerState_t *playerState; // needs to be in the gentity for bg entity access
                              // if you want to actually see the contents I
                              // guess you will have to be sure to VMA it first.
#if (!defined(MACOS_X) && !defined(__GCC__) && !defined(__GNUC__))
...
// I suppose this could be in another in-engine header or something. But we
// never want to include an icarus file before sharedentity_t is declared.
...
typedef struct gameImport_s {
  ...
  int (*EntitiesInBox)(const vec3_t mins, const vec3_t maxs, int *list,
                       int maxcount);
  ...
  qboolean (*ICARUS_RegisterScript)(const char *name,
  ...
  void (*ICARUS_SetVar)(int taskID, int entID, const char *type_name,
                        const char *data);
  ...
  int (*Nav_GetNearestNode)(sharedEntity_t *ent, int lastID, int flags,
                            int targetID);
  ...
  qboolean (*Nav_RouteBlocked)(int startID, int testEdgeID, int endID,
                               int rejectRank);
  int (*Nav_GetBestNodeAltRoute)(int startID, int endID, int *pathCost,
                                 int rejectID);
  int (*Nav_GetBestNodeAltRoute2)(int startID, int endID, int rejectID);
  int (*Nav_GetBestPathBetweenEnts)(sharedEntity_t *ent, sharedEntity_t *goal,
                                    int flags);
  ...
  void (*BotMoveToGoal)(void *result, int movestate, void *goal,
                        int travelflags);
  int (*BotMoveInDirection)(int movestate, vec3_t dir, float speed, int type);
  void (*BotResetAvoidReach)(int movestate);
  void (*BotResetLastAvoidReach)(int movestate);
  int (*BotReachabilityArea)(vec3_t origin, int testground);
  int (*BotMovementViewTarget)(int movestate, void *goal, int travelflags,
  ...
  int (*BotPredictVisiblePosition)(vec3_t origin, int areanum, void *goal,
  ...
  int (*AAS_PredictClientMovement)(void *move, int entnum, vec3_t origin,
  ...
  int (*AAS_AlternativeRouteGoals)(vec3_t start, int startareanum, vec3_t goal,
                                   ...
                                   int type);
  int (*AAS_PredictRoute)(void *route, int areanum, vec3_t origin,
  ...
  void (*G2API_SetGhoul2ModelIndexes)(void *ghoul2, qhandle_t *modelList,
                                      qhandle_t *skinList);
...
typedef struct gameExport_s {
  ...
  qboolean (*NAV_ClearPathToPoint)(int entID, vec3_t pmins, vec3_t pmaxs,
...
typedef gameExport_t *(QDECL *GetGameAPI_t)(int apiVersion,
                                            gameImport_t *import);
