...
char *weaponNameFromIndex[WP_NUM_WEAPONS] = {
    "No Weapon", "Stun Baton", "Saber",           "Bryar Pistol",
    "Blaster",   "Disruptor",  "Bowcaster",       "Repeater",
    "Demp2",     "Flechette",  "Rocket Launcher", "Thermal",
...
void G_LogWeaponOutput(void) {
  ...
  for (i = 0; i < MAX_CLIENTS; i++) {
    if (G_WeaponLogClientTouch[i]) { // Ignore any entity/clients we don't care
                                     // about!
  ...
  for (j = 0; j < WP_NUM_WEAPONS; j++) {
    G_LogPrintf("%15s:  Pickups: %4d,  Time:  %5d,  Deaths: %5d\n",
                weaponNameFromIndex[j], totalpickups[j],
                (int)(totaltime[j] / 1000), totaldeaths[j]);
  }
  ...
  for (j = 0; j < MOD_MAX; j++) {
    G_LogPrintf("%25s:  Damage: %6d,  Kills: %5d\n", modNames[j],
                totaldamageMOD[j], totalkillsMOD[j]);
  }
  ...
  // Cycle through each player, give their name and the number of times they
  ...
  for (i = 0; i < MAX_CLIENTS; i++) {
    if (G_WeaponLogClientTouch[i]) { // Ignore any entity/clients we don't care
      ...
      // We must grab the totals from the damage types for the player and map
      // them to the weapons.
...
qboolean CalculateEfficiency(gentity_t *ent, int *efficiency) {
  ...
  for (i = 0; i < sv_maxclients.integer; i++) {
    ...
    nShotsFired =
        player->client
            ->accuracy_shots; // player->client->ps.persistant[PERS_ACCURACY_SHOTS];
    nShotsHit =
        player->client
            ->accuracy_hits; // player->client->ps.persistant[PERS_ACCURACY_HITS];
...
qboolean CalculateSharpshooter(gentity_t *ent, int *frags) {
  ...
  if (((float)(G_WeaponLogKills[ent - g_entities][MOD_DISRUPTOR_SNIPER])) /
          ((float)(playTime)) <
      1.0) {
    return qfalse;
  }
...
qboolean CalculateUntouchable(gentity_t *ent) {
  ...
  if (level.gametype == GT_JEDIMASTER &&
      ent->client->ps
          .isJediMaster) { // Jedi Master can only be killed once anyway
    return qfalse;
  }
  //------------------------------------------------------ MUST HAVE ACHIEVED 2
  //KILLS PER MINUTE
  if (((float)ent->client->ps.persistant[PERS_SCORE]) / ((float)(playTime)) <
          2.0 ||
      playTime == 0)
    return qfalse;
  //------------------------------------------------------ MUST HAVE ACHIEVED 2
...
qboolean CalculateTactician(gentity_t *ent, int *kills) {
  ...
  if (level.gametype == GT_JEDIMASTER &&
      ent->client->ps.isJediMaster) { // Jedi Master has only 1 weapon
    return qfalse;
  }
  //------------------------------------------------------ MUST HAVE ACHIEVED 2
  //KILLS PER MINUTE
  ...
  for (person = 0; person < sv_maxclients.integer; person++) {
    ...
    for (i = 0; i < MOD_MAX; i++) {
      ...
      killsWithWeapon[weapon] +=
          G_WeaponLogKills[person][i]; // Store Num Kills With Weapon
    }

    weapon = WP_STUN_BATON; // Start At Stun Baton
    //   keep looking through weapons if weapon is not on map, or if it is and
    //   we used it
    while (weapon < WP_NUM_WEAPONS &&
           (!wasPickedUpBySomeone[weapon] || killsWithWeapon[weapon] > 0)) {
      weapon++;
      nKills += killsWithWeapon[weapon]; //  Update the number of kills
    }
    //
    // At this point we have either successfully gone through every weapon on
    // the map and saw it had been used, or we found one that WAS on the map and
    // was NOT used
    //
    // so we look to see if the weapon==Max (i.e. we used every one) and then we
    // check to see if we got the most kills out of anyone else who did this.
    //
...
qboolean CalculateDemolitionist(gentity_t *ent, int *kills) {
  ...
  for (i = 0; i < sv_maxclients.integer; i++) {
    ...
    // if this guy didn't get two explosive kills per minute, reject him right
    // now
...
qboolean CalculateTeamInterceptor(gentity_t *ent) {
  ...
  /*
  if (CalculateTeamMVP(ent) || CalculateTeamDefender(ent) ||
  CalculateTeamWarrior(ent) || CalculateTeamCarrier(ent))
  {
          return qfalse;
  }
  */
...
qboolean CalculateTeamRedShirt(gentity_t *ent) {
  ...
  */
  for (i = 0; i < sv_maxclients.integer; i++) {
    ...
    nScore -= player->client->ps.fd
                  .suicides; // suicides don't count, you big cheater.
    if (nScore > nHighestScore) {
      nHighestScore = nScore;
      nBestPlayer = i;
    }
...
int CalculateTeamAward(gentity_t *ent) {
  ...
  if (!teamAwards &&
      CalculateTeamRedShirt(ent)) { // if you got nothing else and died a lot,
                                    // at least get bravery
