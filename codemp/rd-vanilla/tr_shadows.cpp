...
void R_RenderShadowEdges(void) {
  ...
  for (i = 0; i < tess.numVertexes; i++) {
    ...
    for (j = 0; j < c; j++) {
      ...
      // with this system we can still get edges shared by more than 2 tris
      // which produces artifacts including seeing the shadow through walls. So
      // for now we are going to render all edges even though it is a tiny bit
      // slower. -rww
#if 1
      i2 = edgeDefs[i][j].i2;
...
void RB_DoShadowTessEnd(vec3_t lightPos) {
...
#if 1 // controlled method - try to keep shadows in range so they don't show
      // through so much -rww
  vec3_t worldxyz;
  vec3_t entLight;
  float groundDist;
  ...
  for (i = 0; i < tess.numVertexes; i++) {
    // add or.origin to vert xyz to end up with world oriented coord, then
    // figure out the ground pos for the vert to project the shadow volume to
  ...
  if (lightPos) {
    for (i = 0; i < tess.numVertexes; i++) {
      shadowXyz[i][0] =
          tess.xyz[i][0] + ((tess.xyz[i][0] - lightPos[0]) * 128.0f);
      shadowXyz[i][1] =
          tess.xyz[i][1] + ((tess.xyz[i][1] - lightPos[1]) * 128.0f);
      shadowXyz[i][2] =
          tess.xyz[i][2] + ((tess.xyz[i][2] - lightPos[2]) * 128.0f);
    }
  } else {
  ...
  for (i = 0; i < numTris; i++) {
    ...
    if (!lightPos) {
      ...
      float planeEq[4];
      planeEq[0] = v1[1] * (v2[2] - v3[2]) + v2[1] * (v3[2] - v1[2]) +
                   v3[1] * (v1[2] - v2[2]);
      planeEq[1] = v1[2] * (v2[0] - v3[0]) + v2[2] * (v3[0] - v1[0]) +
                   v3[2] * (v1[0] - v2[0]);
      planeEq[2] = v1[0] * (v2[1] - v3[1]) + v2[0] * (v3[1] - v1[1]) +
                   v3[0] * (v1[1] - v2[1]);
      planeEq[3] = -(v1[0] * (v2[1] * v3[2] - v3[1] * v2[2]) +
                     v2[0] * (v3[1] * v1[2] - v1[1] * v3[2]) +
                     v3[0] * (v1[1] * v2[2] - v2[1] * v1[2]));

      d = planeEq[0] * lightPos[0] + planeEq[1] * lightPos[1] +
          planeEq[2] * lightPos[2] + planeEq[3];
    }
...
void RB_ShadowFinish(void) {
  ...
  //	GL_State( GLS_DEPTHMASK_TRUE | GLS_SRCBLEND_DST_COLOR |
  //GLS_DSTBLEND_ZERO );
  ...
  // GL_State( GLS_DEPTHMASK_TRUE | GLS_SRCBLEND_SRC_ALPHA |
...
void RB_CaptureScreenImage(void) {
  ...
  // using this method, we could pixel-filter the texture and all sorts of crazy
  // stuff. but, it is slow as hell.
  /*
  static byte *tmp = NULL;
  if (!tmp)
  {
          tmp = (byte
  *)Z_Malloc((sizeof(byte)*4)*(glConfig.vidWidth*glConfig.vidHeight),
  TAG_ICARUS, qtrue);
  }
  qglReadPixels(0, 0, glConfig.vidWidth, glConfig.vidHeight, GL_RGBA,
  GL_UNSIGNED_BYTE, tmp); qglTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 512, 512, 0,
  ...
  */
  ...
  if (cX + radX > glConfig.vidWidth) { // would it go off screen?
    cX = glConfig.vidWidth - radX;
  } else if (cX < 0) { // cap it off at 0
    cX = 0;
  }

  if (cY + radY > glConfig.vidHeight) { // would it go off screen?
...
void RB_DistortionFill(void) {
  ...
  if (tr_distortionStretch) { // override
    spost = tr_distortionStretch;
    spost2 = tr_distortionStretch;
  } else { // do slow stretchy effect
  ...
  } else { // be sure to reset the draw state
    GL_State(0);
  }
  ...
  if (tr_distortionAlpha == 1.0f &&
      tr_distortionStretch == 0.0f) { // no overrides
    if (tr_distortionNegate) { // probably the crazy alternate saber trail
      alpha = 0.8f;
