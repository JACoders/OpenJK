...
void NPC_GalakMech_Init(gentity_t *ent) {
  if (ent->NPC->behaviorState != BS_CINEMATIC) {
    ...
    // ent->client->ps.powerups[PW_GALAK_SHIELD] = Q3_INFINITE;//temp, for
    // effect ent->fx_time = level.time;
...
static void GM_CreateExplosion(gentity_t *self, const int boltID,
                               qboolean doSmall) // doSmall = qfalse
{
  if (boltID >= 0) {
    mdxaBone_t boltMatrix;
    vec3_t org, dir;

    trap->G2API_GetBoltMatrix(self->ghoul2, 0, boltID, &boltMatrix,
    ...
    if (TIMER_Done(self, "dyingExplosion")) {
      ...
      case 1:
        if (!trap->G2API_GetSurfaceRenderStatus(
                self->ghoul2, 0, "r_hand")) { // r_hand still there
          GM_CreateExplosion(
              self, trap->G2API_AddBolt(self->ghoul2, 0, "*flasha"), qtrue);
      ...
      case 2:
        // FIXME: do only once?
        if (!trap->G2API_GetSurfaceRenderStatus(
                self->ghoul2, 0, "l_hand")) { // l_hand still there
          GM_CreateExplosion(
              self, trap->G2API_AddBolt(self->ghoul2, 0, "*flashc"), qfalse);
...
void NPC_GM_Pain(gentity_t *self, gentity_t *attacker, int damage) {
  ...
  {      // shield is currently down
    ...
    /*
    if ( (hitLoc==HL_GENERIC1) && (self->locationDamage[HL_GENERIC1] >
    GENERATOR_HEALTH) )
    {
            int newBolt = trap->G2API_AddBolt( &self->ghoul2[self->playerModel],
    "*antenna_base" ); if ( newBolt != -1 )
            {
                    GM_CreateExplosion( self, newBolt, qfalse );
            }
    ...
    SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD ); TIMER_Set( self, "attackDelay",
    self->client->ps.torsoTimer ); G_AddEvent( self, Q_irand( EV_DEATH1,
    EV_DEATH3 ), self->health );
    }
    */
  ...
  if (!self->lockCount &&
      self->client->ps.torsoTimer <= 0) { // don't interrupt laser sweep attack
  ...
  if ((NPCS.NPCInfo->goalEntity != NPCS.NPC->enemy) &&
    ...
    if (NAV_HitNavGoal(NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins,
        ...
        (!trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC,
                                     TID_MOVE_NAV) &&
         enemyLOS4 &&
         enemyDist4 <= 10000)) { // either hit our navgoal or our navgoal was
                                 // not a crucial (scripted) one (maybe a combat
    ...
    if (level.time - NPCS.NPCInfo->enemyLastSeenTime < 10000) {
      if (!Q_irand(0, 10)) {
        ...
        if (dist < distThreshold) { // impact would be too close to me
          ...
          dist =
              DistanceSquared(impactPos4, NPCS.NPCInfo->enemyLastSeenLocation);
...
void NPC_GM_StartLaser(void) {
  if (!NPCS.NPC->lockCount) { // haven't already started a laser attack
    ...
    G_PlayEffectID(G_EffectIndex("galak/beam_warmup"),
                   NPCS.NPC->r.currentOrigin, vec3_origin);
...
void GM_StartGloat(void) {
  ...
  NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_STAND2TO1,
...
void NPC_BSGM_Attack(void) {
  ...
  enemyDist4 = DistanceSquared(NPCS.NPC->r.currentOrigin,
  ...
  if (0) {
    shoot4 = qfalse;
    if (TIMER_Done(NPCS.NPC, "smackTime") &&
        !NPCS.NPCInfo->blockedDebounceTime) { // time to smack
      ...
      if (enemyDist4 < MELEE_DIST_SQUARED &&
        ...
        VectorSubtract(NPCS.NPC->enemy->r.currentOrigin,
                       NPCS.NPC->r.currentOrigin, smackDir);
        smackDir[2] += 30;
        VectorNormalize(smackDir);
        // hurt them
        G_Sound(NPCS.NPC->enemy, CHAN_AUTO,
                G_SoundIndex("sound/weapons/galak/skewerhit.wav"));
        G_Damage(NPCS.NPC->enemy, NPCS.NPC, NPCS.NPC, smackDir,
                 NPCS.NPC->r.currentOrigin,
                 (g_npcspskill.integer + 1) * Q_irand(5, 10),
                 DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK, MOD_CRUSH);
        if (NPCS.NPC->client->ps.torsoAnim == BOTH_ATTACK4) { // smackdown
          int knockAnim = BOTH_KNOCKDOWN1;
          if (BG_CrouchAnim(NPCS.NPC->enemy->client->ps
                                .legsAnim)) { // knockdown from crouch
  ...
  { // sometimes use the laser beam attack, but only after he's taken down our
    // generator
    shoot4 = qfalse;
    if (NPCS.NPC->lockCount == 1) {            // charging up
      if (TIMER_Done(NPCS.NPC, "beamDelay")) { // time to start the beam
        int laserAnim;
        // if ( Q_irand( 0, 1 ) )
        if (1) {
        ...
        */
        NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, laserAnim,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        TIMER_Set(NPCS.NPC, "attackDelay",
                  NPCS.NPC->client->ps.torsoTimer + Q_irand(1000, 3000));
        // turn on beam effect
        NPCS.NPC->lockCount = 2;
        G_PlayEffectID(G_EffectIndex("galak/trace_beam"),
                       NPCS.NPC->r.currentOrigin, vec3_origin);
        NPCS.NPC->s.loopSound =
            G_SoundIndex("sound/weapons/galak/lasercutting.wav");
        if (!NPCS.NPCInfo
                 ->coverTarg) { // for moving looping sound at end of trace
          NPCS.NPCInfo->coverTarg = G_Spawn();
          if (NPCS.NPCInfo->coverTarg) {
            G_SetOrigin(NPCS.NPCInfo->coverTarg,
                        NPCS.NPC->client->renderInfo.muzzlePoint);
            NPCS.NPCInfo->coverTarg->r.svFlags |= SVF_BROADCAST;
            NPCS.NPCInfo->coverTarg->s.loopSound =
                G_SoundIndex("sound/weapons/galak/lasercutting.wav");
          }
        }
      }
    } else {                                      // in the actual attack now
      if (NPCS.NPC->client->ps.torsoTimer <= 0) { // attack done!
        ...
        VectorMA(NPCS.NPC->client->renderInfo.muzzlePoint, 1024,
                 NPCS.NPC->client->renderInfo.muzzleDir, end);
        trap->Trace(&trace, NPCS.NPC->client->renderInfo.muzzlePoint, mins,
                    maxs, end, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0);
        if (trace.allsolid || trace.startsolid) { // oops, in a wall
          ...
          if (trace.fraction < 1.0f) { // hit something
            ...
            if (traceEnt && traceEnt->takedamage) { // damage it
              G_SoundAtLoc(trace.endpos, CHAN_AUTO,
                           G_SoundIndex("sound/weapons/galak/laserdamage.wav"));
              G_Damage(traceEnt, NPCS.NPC, NPCS.NPC,
                       NPCS.NPC->client->renderInfo.muzzleDir, trace.endpos, 10,
                       0, MOD_UNKNOWN);
            }
          }
          if (NPCS.NPCInfo->coverTarg) {
    ...
    (Q_irand( 0, 1 )||enemyDist4 < MAX_LOB_DIST_SQUARED) )
    ...
    if (  // !NPC->client->ps.powerups[PW_GALAK_SHIELD]
        1 // rwwFIXMEFIXME: just act like the shield is down til the effects and
          // stuff are done
...
#endif
        // FIXME: swing sound
        NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, swingAnim,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        TIMER_Set(NPCS.NPC, "attackDelay",
                  NPCS.NPC->client->ps.torsoTimer + Q_irand(1000, 3000));
    ...
    } else if (!NPCS.NPC->lockCount &&
               NPCS.NPC->locationDamage[HL_GENERIC1] > GENERATOR_HEALTH &&
               TIMER_Done(NPCS.NPC, "attackDelay") &&
               InFront(NPCS.NPC->enemy->r.currentOrigin,
                       NPCS.NPC->r.currentOrigin,
                       NPCS.NPC->client->ps.viewangles, 0.3f) &&
               ((!Q_irand(0, 10 * (2 - g_npcspskill.integer)) &&
                 enemyDist4 > MIN_LOB_DIST_SQUARED &&
                 enemyDist4 < MAX_LOB_DIST_SQUARED) ||
                (!TIMER_Done(NPCS.NPC, "noLob") &&
                 !TIMER_Done(NPCS.NPC, "noRapid"))) &&
               NPCS.NPC->enemy->s.weapon !=
  ...
  if (NPC_ClearLOS4(NPCS.NPC->enemy)) {
    NPCS.NPCInfo->enemyLastSeenTime =
    ...
    if (NPCS.NPC->client->ps.weapon == WP_NONE) {
      ...
      if (((NPCS.NPC->client->ps.weapon == WP_REPEATER &&
            (NPCS.NPCInfo->scriptFlags & SCF_ALT_FIRE))) &&
          enemyDist4 < MIN_LOB_DIST_SQUARED) // 256
      {
        enemyCS4 = qfalse; // not true, but should stop us from firing
        hitAlly4 = qtrue;  // us!
  ...
  if (enemyLOS4) {
    ...
    if (NPCS.NPCInfo->goalEntity ==
        NPCS.NPC->enemy) { // for now, always chase the enemy
      move4 = qtrue;
    }
  }
  if (enemyCS4) {
  ...
  if (NPCS.NPC->client->ps.weapon == WP_REPEATER &&
      (NPCS.NPCInfo->scriptFlags & SCF_ALT_FIRE) && shoot4 &&
      TIMER_Done(NPCS.NPC, "attackDelay")) {
    vec3_t muzzle;
    vec3_t angles;
    vec3_t target;
    ...
    target[0] += flrand(-5, 5) +
                 (Q_flrand(-1.0f, 1.0f) * (6 - NPCS.NPCInfo->currentAim) * 2);
    target[1] += flrand(-5, 5) +
                 (Q_flrand(-1.0f, 1.0f) * (6 - NPCS.NPCInfo->currentAim) * 2);
    target[2] += flrand(-5, 5) +
                 (Q_flrand(-1.0f, 1.0f) * (6 - NPCS.NPCInfo->currentAim) * 2);
    ...
    clearshot = WP_LobFire(NPCS.NPC, muzzle, target, mins, maxs,
                           MASK_SHOT | CONTENTS_LIGHTSABER, velocity, qtrue,
                           NPCS.NPC->s.number, NPCS.NPC->enemy->s.number, 300,
                           1100, 1500, qtrue);
    if (VectorCompare(vec3_origin, velocity) ||
        (!clearshot && enemyLOS4 &&
         enemyCS4)) { // no clear lob shot and no lob shot that will hit
                      // something breakable
      if (enemyLOS4 && enemyCS4 &&
          TIMER_Done(
              NPCS.NPC,
      ...
      NPCS.NPC->client->hiddenDist =
          VectorNormalize(NPCS.NPC->client->hiddenDir);
    }
  ...
  if (move4 && !NPCS.NPC->lockCount) { // move toward goal
    if ( NPCS.NPCInfo->goalEntity
  ...
  if (!faceEnemy4) { // we want to face in the dir we're running
    if (!move4) {    // if we haven't moved, we should look in the direction we
                  // last looked?
  ...
  if (NPCS.NPC->enemy->s.weapon == WP_TURRET &&
      !Q_stricmp("PAS", NPCS.NPC->enemy->classname)) { // crush turrets
    if (G_BoundsOverlap(
            NPCS.NPC->r.absmin, NPCS.NPC->r.absmax, NPCS.NPC->enemy->r.absmin,
            NPCS.NPC->enemy->r.absmax)) { // have to do this test because placed
                                          // turrets are not solid to NPCs (so
                                          // they don't obstruct navigation)
      // if ( NPC->client->ps.powerups[PW_GALAK_SHIELD] > 0 )
      if (0) {
#ifdef BASE_COMPAT
        NPCS.NPC->client->ps.powerups[PW_BATTLESUIT] =
            level.time + ARMOR_EFFECT_TIME;
#endif
        G_Damage(NPCS.NPC->enemy, NPCS.NPC, NPCS.NPC, NULL,
                 NPCS.NPC->r.currentOrigin, 100, DAMAGE_NO_KNOCKBACK,
                 MOD_UNKNOWN);
      } else {
        G_Damage(NPCS.NPC->enemy, NPCS.NPC, NPCS.NPC, NULL,
                 NPCS.NPC->r.currentOrigin, 100, DAMAGE_NO_KNOCKBACK,
  ...
  } else if (NPCS.NPCInfo->touchedByPlayer != NULL &&
             NPCS.NPCInfo->touchedByPlayer ==
                 NPCS.NPC->enemy) { // touched enemy
    // if ( NPC->client->ps.powerups[PW_GALAK_SHIELD] > 0 )
    if (0) { // zap him!
      vec3_t smackDir;
...
// FIXME: some shield effect?
      ...
      G_Damage(NPCS.NPC->enemy, NPCS.NPC, NPCS.NPC, smackDir,
               NPCS.NPC->r.currentOrigin,
               (g_npcspskill.integer + 1) * Q_irand(5, 10), DAMAGE_NO_KNOCKBACK,
               MOD_UNKNOWN);
      // throw them
      G_Throw(NPCS.NPC->enemy, smackDir, 100);
...
void NPC_BSGM_Default(void) {
  ...
  if (NPCS.NPC->client->ps.stats[STAT_ARMOR] <= 0) { // armor gone
    ...
    if (0) { // start regenerating the armor
      ...
      if (NPCS.NPC->locationDamage[HL_GENERIC1] <
        ...
        NPCS.NPCInfo->investigateDebounceTime =
            level.time + (NPCS.NPCInfo->investigateCount * 5000);
      }
    } else if (NPCS.NPCInfo->investigateDebounceTime <
               level.time) { // armor regenerated, turn shield back on
      // do a trace and make sure we can turn this back on?
      trace_t tr;
      trap->Trace(&tr, NPCS.NPC->r.currentOrigin, shieldMins, shieldMaxs,
                  NPCS.NPC->r.currentOrigin, NPCS.NPC->s.number,
                  NPCS.NPC->clipmask, qfalse, 0, 0);
      if (!tr.startsolid) {
        VectorCopy(shieldMins, NPCS.NPC->r.mins);
        VectorCopy(shieldMaxs, NPCS.NPC->r.maxs);
        NPCS.NPC->client->ps.crouchheight = NPCS.NPC->client->ps.standheight =
            shieldMaxs[2];
  ...
  {//armor present
          NPC->client->ps.powerups[PW_GALAK_SHIELD] = Q3_INFINITE;//temp, for
  effect NPC_SetSurfaceOnOff( NPC, "torso_shield", TURN_ON );
  }
  else
  {
  ...
  */
  // rwwFIXMEFIXME: Allow this stuff, and again, going to have to let the client
  // know about it. Maybe a surface-off bitflag of some sort in the entity state?
