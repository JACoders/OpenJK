...
//
SBCSOverrideLanguages_t g_SBCSOverrideLanguages[] = {{"russian", eRussian}, {"polish", ePolish}, {NULL, eWestern}};
...
//
float RoundTenth(float fValue) { return (floorf((fValue * 10.0f) + 0.5f)) / 10.0f; }
...
int g_iNonScaledCharRange; // this is used with auto-scaling of asian fonts, anything below this number is preserved in scale, anything above is scaled down by
						   // 0.75f
...
static inline qboolean Korean_ValidKSC5601Hangul(byte _iHi, byte _iLo) {
	return (qboolean)(_iHi >= KSC5601_HANGUL_HIBYTE_START && _iHi <= KSC5601_HANGUL_HIBYTE_STOP && _iLo > KSC5601_HANGUL_LOBYTE_LOBOUND &&
					  _iLo < KSC5601_HANGUL_LOBYTE_HIBOUND);
}

static inline qboolean Korean_ValidKSC5601Hangul(unsigned int uiCode) { return Korean_ValidKSC5601Hangul(uiCode >> 8, uiCode & 0xFF); }
...
static qboolean Taiwanese_ValidBig5Code(unsigned int uiCode) {
	...
	if ((_iHi >= BIG5_HIBYTE_START0 && _iHi <= BIG5_HIBYTE_STOP0) || (_iHi >= BIG5_HIBYTE_START1 && _iHi <= BIG5_HIBYTE_STOP1)) {
		const byte _iLo = uiCode & 0xFF;

		if ((_iLo >= BIG5_LOBYTE_LOBOUND0 && _iLo <= BIG5_LOBYTE_HIBOUND0) || (_iLo >= BIG5_LOBYTE_LOBOUND1 && _iLo <= BIG5_LOBYTE_HIBOUND1)) {
			return qtrue;
		}
	}
...
static qboolean Taiwanese_IsTrailingPunctuation(unsigned int uiCode) {
	...
	//
	if (uiCode >= ((BIG5_HIBYTE_START0 << 8) | BIG5_LOBYTE_LOBOUND0) && uiCode < (((BIG5_HIBYTE_START0 << 8) | BIG5_LOBYTE_LOBOUND0) + 20)) {
		return qtrue;
	}
...
static qboolean Japanese_ValidShiftJISCode(byte _iHi, byte _iLo) {
	if ((_iHi >= SHIFTJIS_HIBYTE_START0 && _iHi <= SHIFTJIS_HIBYTE_STOP0) || (_iHi >= SHIFTJIS_HIBYTE_START1 && _iHi <= SHIFTJIS_HIBYTE_STOP1)) {
		if ((_iLo >= SHIFTJIS_LOBYTE_START0 && _iLo <= SHIFTJIS_LOBYTE_STOP0) || (_iLo >= SHIFTJIS_LOBYTE_START1 && _iLo <= SHIFTJIS_LOBYTE_STOP1)) {
			return qtrue;
		}
	}
...
static inline qboolean Japanese_ValidShiftJISCode(unsigned int uiCode) { return Japanese_ValidShiftJISCode(uiCode >> 8, uiCode & 0xFF); }
...
static qboolean Japanese_IsTrailingPunctuation(unsigned int uiCode) {
	...
	//
	if (uiCode >= ((SHIFTJIS_HIBYTE_START0 << 8) | SHIFTJIS_LOBYTE_START0) && uiCode < (((SHIFTJIS_HIBYTE_START0 << 8) | SHIFTJIS_LOBYTE_START0) + 18)) {
		return qtrue;
	}
...
static inline qboolean Chinese_ValidGBCode(byte _iHi, byte _iLo) {
	return (qboolean)(_iHi >= GB_HIBYTE_START && _iHi <= GB_HIBYTE_STOP && _iLo > GB_LOBYTE_LOBOUND && _iLo < GB_LOBYTE_HIBOUND);
}

static inline qboolean Chinese_ValidGBCode(unsigned int uiCode) { return Chinese_ValidGBCode(uiCode >> 8, uiCode & 0xFF); }
...
static qboolean Chinese_IsTrailingPunctuation(unsigned int uiCode) {
	...
	//
	if (uiCode > ((GB_HIBYTE_START << 8) | GB_LOBYTE_LOBOUND) && uiCode < (((GB_HIBYTE_START << 8) | GB_LOBYTE_LOBOUND) + 14)) {
		return qtrue;
	}
...
//
static inline qboolean Thai_IsTrailingPunctuation(unsigned int uiCode) { return (qboolean)(uiCode == '_'); }
...
//
// Note that I have to have this 3-param form instead of advancing a passed-in "const char **psText" because of VM-crap where you can only change ptr-contents,
// not ptrs themselves. Bleurgh. Ditto the qtrue:qfalse crap instead of just returning stuff straight through.
//
unsigned int AnyLanguage_ReadCharFromString(char *psText, int *piAdvanceCount, qboolean *pbIsTrailingPunctuation /* = NULL */) {
	...
	*piAdvanceCount = 1;
	...
	if (pbIsTrailingPunctuation) {
		*pbIsTrailingPunctuation = (qboolean)(uiLetter == '!' || uiLetter == '?' || uiLetter == ',' || uiLetter == '.' || uiLetter == ';' || uiLetter == ':');
	}
	...
	*piAdvanceCount = 1;
	...
	if (pbIsTrailingPunctuation) {
		*pbIsTrailingPunctuation =
			(uiLetter == '!' || uiLetter == '?' || uiLetter == ',' || uiLetter == '.' || uiLetter == ';' || uiLetter == ':') ? qtrue : qfalse;
	}
...
static const char *FontDatPath(const char *_fontName) {
	...
	sprintf(fontName, "fonts/%s.fontdat",
			COM_SkipPath(const_cast<char *>(_fontName))); // COM_SkipPath should take a const char *, but it's just possible people use it as a char * I guess,
...
int CFontInfo::GetHandle() { return m_handle; }

void CFontInfo::AddVariant(CFontInfo *replacer) { m_variants[m_numVariants++] = replacer; }

int CFontInfo::GetNumVariants() { return m_numVariants; }

CFontInfo *CFontInfo::GetVariant(int index) { return m_variants[index]; }
...
static CFontInfo *RE_Font_GetVariant(CFontInfo *font, float *scale) {
	...
	if (variants > 0) {
		CFontInfo *variant;
		int requestedSize = font->GetPointSize() * *scale * r_fontSharpness->value * (glConfig.vidHeight / SCREEN_HEIGHT);
	...
	if (!pFont->m_bIsFakeAlienLanguage) {
		if (GetLanguageEnum() == eLanguageSBCS) {
			if (pFont->m_iAltSBCSFont == -1) // no reg attempted yet?
			{
				...
				//
				int iAltFontIndex =
					RE_RegisterFont(va("%s/%s", COM_SkipPath(pFont->m_sFontName), psLanguageNameSBCS)); // ensure unique name (eg: "lcd/russian")
...
#ifndef JK2_MODE
		case '_':
			iCharCount += (GetLanguageEnum() == eThai && (((unsigned char *)psText)[0] >= TIS_GLYPHS_START)) ? 0 : 1;
...
void RE_Font_DrawString(int ox, int oy, const char *psText, const float *rgba, const int iFontHandleIn, int iMaxPixelWidth, const float fScaleIn) {
	...
	*/
	...
	fx = fox;
	foy += curfont->mbRoundCalcs ? Round((curfont->GetHeight() - (curfont->GetDescender() >> 1)) * fScale)
	...
	while (*psText && !bNextTextWouldOverflow) {
		...
		case 10: // linefeed
			...
			if (Language_IsAsian()) {
				foy += 4.0f; // this only comes into effect when playing in asian for "A long time ago in a galaxy" etc, all other text is line-broken in feeder
							 // functions
			}
			break;
		case 13: // Return
		...
		case 32: // Space
			...
			bNextTextWouldOverflow = (qboolean)(iMaxPixelWidth != -1 && ((fx - fox) > (float)iMaxPixelWidth));
			break;
		...
		default:
			...
			bNextTextWouldOverflow = (qboolean)(iMaxPixelWidth != -1 && (((fx + fAdvancePixels) - fox) > (float)iMaxPixelWidth));
	...
	//	{
	//		//psText = "Ӷ\B1\F8ս\B3\A1II  Լ\BA\B2?Ī\C1\D6˹  \C8\CE\CE\F1ʧ\B0\DC  \C4\E3Ҫ\CC\D7\D3û\AD\C3\E6\C9趨\B5ı\E4\B8\FC\C2\F0\A3\BF  Ԥ\C9\E8,S3 ѹ\CB\F5,DXT1
	//ѹ\CB\F5,DXT5 ѹ\CB\F5,16 Bit,32 Bit"; 		psText = "Ӷ\B1\F8ս\B3\A1II";
	//	}
	//	else
	//	if (GetLanguageEnum() == eThai)
	//	{
	//		//psText =
	//"\C1ҵðҹ\BC\C5Ե\C0ѳ\B1\EC\CDص\CA\D2ˡ\C3\C3\C1\C3\CB\D1\CA\CA\D3\CB\C3Ѻ\CDѡ\A2\C3\D0\E4\B7·\D5\E8\E3\AA\E9\A1Ѻ\A4\CD\C1\BE\D4\C7\E0\B5\CD\C3\EC"; 		psText =
	//"\C1ҵðҹ\BC\C5Ե"; 		psText = "\C3\CB\D1\CA\CA\D3\CB\C3Ѻ"; 		psText = "\C3\CB\D1\CA\CA\D3\CB\C3Ѻ   \CD\D2_\A1Թ_\A4\CD\C3\EC\B7_1415";
	//	}
	//	else
	...
	//	{
	////		//psText = "\CD\E0 \E2\E5\F0\F8\E8\ED\E5 \F5\EE\EB\EC\E0 \F1\F2\EE\E8\F2 \F1\F2\E0\F0\FB\E9 \E4\EE\EC \F1 \EF\F0\E8\E2\E8\E4\E5\ED\E8\FF\EC\E8 \E8
	///\E1\E0\F8\ED\FF \F1 \E2\EE\EB\F8\E5\E1\ED\FB\EC\E8 \F7\E0\F1\E0\EC\E8."
...
void R_FontList_f(void) {
	...
	for (it = g_mapFontIndexes.begin(); it != g_mapFontIndexes.end(); ++it) {
		...
		if (font) {
			Com_Printf("%3i:%s  ps:%hi h:%hi a:%hi d:%hi\n", (*it).second, font->m_sFontName, font->mPointSize, font->mHeight, font->mAscender,
					   font->mDescender);
		}
	}
	Com_Printf("------------------------------------\n");
...
void R_ReloadFonts_f(void) {
	...
	if (iFontToFind == g_iCurrentFontIndex) // found all of them?
	{
		...
		R_InitFonts();
		//
		// and re-register our fonts in the same order as before (note that some menu items etc cache the string lengths so really a vid_restart is better, but
