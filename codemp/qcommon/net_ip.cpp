...
void Sys_SendPacket(int length, const void *data, netadr_t to) {
  ...
  if (usingSocks && to.type == NA_IP) {
    ...
    ret = sendto(ip_socket, socksBuf, length + 10, 0,
                 (sockaddr *)&socksRelayAddr, sizeof(socksRelayAddr));
  } else {
    ret = sendto(ip_socket, (const char *)data, length, 0, (sockaddr *)&addr,
                 sizeof(addr));
  }
  if (ret == SOCKET_ERROR) {
    int err = socketError;
...
#define IFR_NEXT(ifr)                                                          \
  ((struct ifreq *)((char *)(ifr) + sizeof(*(ifr)) +                           \
                    Q_max(0, (int)(ifr)->ifr_addr.sa_len -                     \
...
static qboolean NET_GetCvars(void) {
  ...
  net_forcenonlocal =
      Cvar_Get("net_forcenonlocal", "0", CVAR_LATCH | CVAR_ARCHIVE_ND);
  ...
  net_socksEnabled =
      Cvar_Get("net_socksEnabled", "0", CVAR_LATCH | CVAR_ARCHIVE_ND);
  modified += net_socksEnabled->modified;
  net_socksEnabled->modified = qfalse;

  net_socksServer =
      Cvar_Get("net_socksServer", "", CVAR_LATCH | CVAR_ARCHIVE_ND);
  modified += net_socksServer->modified;
  net_socksServer->modified = qfalse;

  net_socksPort =
      Cvar_Get("net_socksPort", "1080", CVAR_LATCH | CVAR_ARCHIVE_ND);
  modified += net_socksPort->modified;
  net_socksPort->modified = qfalse;

  net_socksUsername =
      Cvar_Get("net_socksUsername", "", CVAR_LATCH | CVAR_ARCHIVE_ND);
  modified += net_socksUsername->modified;
  net_socksUsername->modified = qfalse;

  net_socksPassword =
      Cvar_Get("net_socksPassword", "", CVAR_LATCH | CVAR_ARCHIVE_ND);
