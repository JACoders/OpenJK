...
#define NPCAI_FORM_TELE_NAV                                                    \
  0x00010000 // Tells formation people to use nav info to get to
#define NPCAI_ENROUTE_TO_HOMEWP                                                \
  0x00020000 // Lets us know to run our lostenemyscript when we get to homeWp
#define NPCAI_MATCHPLAYERWEAPON                                                \
  0x00040000 // Match the player's weapon except when it changes during
             // cinematics
...
#define NPCAI_NAV_THROUGH_BREAKABLES                                           \
  0x00800000 // Navigation allows connections through breakable (func_glass,
             // func_breakable or misc_model_breakable)
#define NPCAI_KNEEL 0x01000000 // Kneel befor Zod
#define NPCAI_FLY 0x02000000   // Fly, My Pretty!
#define NPCAI_FLAMETHROW 0x04000000
#define NPCAI_ROSH                                                             \
  0x08000000 // I am Rosh, when I'm hurt, drop to one knee and wait for Vil or
             // Dasariah to heal me
#define NPCAI_HEAL_ROSH                                                        \
  0x10000000 // Constantly look for NPC with NPC_type of rosh_dark, follow him,
             // heal him if needbe
#define NPCAI_JUMP 0x20000000 // Jump Now
#define NPCAI_BOSS_CHARACTER                                                   \
  0x40000000 // Boss NPC flag for certain immunities/defenses
#define NPCAI_NO_JEDI_DELAY                                                    \
  0x80000000 // Reborn/Jedi don't taunt enemy before attacking
...
#define SCF_RUNNING 0x00000020    // Takes off walking button, overrides
                               // SCF_WALKING
#define SCF_ALT_FIRE 0x00000040 // Force to use alt-fire when firing
#define SCF_NO_RESPONSE                                                        \
  0x00000080 // NPC will not do generic responses to being used
#define SCF_FFDEATH                                                            \
  0x00000100 // Just tells player_die to run the friendly fire deathscript
#define SCF_NO_COMBAT_TALK                                                     \
  0x00000200 // NPC will not use their generic combat chatter stuff
#define SCF_CHASE_ENEMIES                                                      \
  0x00000400 // NPC chase enemies - FIXME: right now this is synonymous with
             // using combat points... should it be?
#define SCF_LOOK_FOR_ENEMIES 0x00000800 // NPC be on the lookout for enemies
#define SCF_FACE_MOVE_DIR                                                      \
  0x00001000 // NPC face direction it's moving - FIXME: not really implemented
             // right now
...
#define SCF_FORCED_MARCH                                                       \
  0x00010000 // NPC that the player must aim at to make him walk
#define SCF_NO_GROUPS 0x00020000 // NPC cannot alert groups or be part of a
                                 // group
...
#define SCF_USE_CP_NEAREST                                                     \
  0x00100000 // Will use combat point close to it, not next to player or try and
             // flank player
#define SCF_NO_FORCE 0x00200000 // Not succeptible to force powers
#define SCF_NO_FALLTODEATH                                                     \
  0x00400000 // NPC will not scream and tumble and fall to hit death over large
             // drops
#define SCF_NO_ACROBATICS 0x00800000 // Jedi won't jump, roll or cartwheel
#define SCF_USE_SUBTITLES                                                      \
  0x01000000 // Regardless of subtitle setting, this NPC will display subtitles
             // when it speaks lines
#define SCF_NO_ALERT_TALK                                                      \
  0x02000000 // Will not say alert sounds, but still can be woken up by alerts
...
#define SCF_FIRE_WEAPON_NO_ANIM                                                \
  0x40000000 // Fire weapon but don't play weapon firing anim
#define SCF_SAFE_REMOVE                                                        \
  0x80000000 // Remove NPC when it's safe (when player isn't looking)
...
#define AID_LOSTPATH                                                           \
  0x00000008 // Cannot make a valid movement due to lack of connections
...
class gNPCstats_t { // Stats, loaded in, and can be set by scripts
public:
  // AI
  int aggression;   //			"
  int aim;          //			"
  ...
  float
      shootDistance; // Maximum range- overrides range set for weapon if nonzero
...
public:
  ...
  int attackHold;
  int attackHoldTime;
  vec3_t shootAngles; // Angles to where bot is shooting - fixme: make he torso
                      // turn to reflect these
  ...
  gentity_t *
      tempGoal; // used for locational goals (player's last seen/heard position)
  ...
  vec3_t blockedTargetPosition;   // Where the actor was trying to get TO before
                                  // blocked
  gentity_t *blockedTargetEntity; // Where the actor was trying to get TO before
                                  // blocked

  // jump info
  vec3_t jumpDest;       // Where The Actor Is Trying To Jump TO
  gentity_t *jumpTarget; // What Entity The Actor Is Trying To Jump TO
  float jumpMaxXYDist;   // The Minimal Delta On The XY Plane Allowed To Jump To
                         // The Dest
  float jumpMazZDist;
  ...
  //
  int combatPoint; // NPCs in bState BS_COMBAT_POINT will find their closest
                   // empty combat_point
  int lastFailedCombatPoint; // NPCs in bState BS_COMBAT_POINT will find their
                             // closest empty combat_point
  ...
  */
  ...
  int confusionTime; // Doesn't respond to alerts or pick up enemies (unless
                     // shot) until this time is up
  ...
  int ffireCount; // sigh... you'd think I'd be able to find a way to do this
                  // without having to use 3 int fields, but...
  int ffireDebounce;
...
typedef struct {
        vec3_t		origin;
        vec2_t		absmin;		// region within this surface that is
the portal to the other surface vec2_t		absmax; int
surfaceNum; unsigned	flags;		// jump, prerequisite button, will take
falling damage, etc... float		cost; int dirIndex; vec3_t
endSpot; int			parm; } nedge_t;
*/
#endif
