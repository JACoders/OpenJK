			...
			if ((flags & EFLAG_BLOCKED)) {
				...
				if (trace.entityNum < ENTITYNUM_WORLD &&
					...
					if (G_EntIsDoor(trace.entityNum)) {												   // door
						...
						if (G_EntIsBreakable(trace.entityNum)) { // do same for breakable brushes/models/glass?
							failed = qtrue;
...
int CNavigator::TestNodeLOS(gentity_t *ent, vec3_t position) { return NPC_ClearLOS(ent, position); }
		...
		if (CheckedNode(nodeNum, ent->s.number) == CHECKED_PASSED) { // already checked this node against ent and it passed
			...
			if ((signed)(*nci).distance >= (radius * radius) || (fabs(position[2] - ent->currentOrigin[2]) >= MAX_Z_DELTA)) {
				...
				{												   // need a clear path or LOS
					if (!gi.inPVS(ent->currentOrigin, position)) { // not even potentially clear
						SetCheckedNode(nodeNum, ent->s.number, CHECKED_FAILED);
						continue;
					}
				}
		...
		STL_ITERATE(nci2, nodeChain2) {
			...
			if (CheckedNode(nodeNum2, goal->s.number) == CHECKED_PASSED) { // already checked this node against goal and it passed
				...
				if ((signed)(*nci2).distance >= (radius * radius) || (fabs(position2[2] - goal->currentOrigin[2]) >= MAX_Z_DELTA)) {
					...
					if (flags & NF_CLEAR_PATH)							 //|| flags & NF_CLEAR_LOS )
					{													 // need a clear path or LOS
						if (!gi.inPVS(goal->currentOrigin, position2)) { // not even potentially clear
							SetCheckedNode(nodeNum2, goal->s.number, CHECKED_FAILED);
							continue;
						}
					}
	...
	if (ent->failedWaypointCheckTime && ent->failedWaypointCheckTime < level.time) {
		...
		for (j = 0; j < MAX_FAILED_NODES; j++) {
			if (ent->failedWaypoints[j] != 0) {
				...
				if (!NAV_ClearPathToPoint(ent, ent->mins, ent->maxs, nodePos, (CONTENTS_SOLID | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP),
				...
				else if (NAV_ClearPathToPoint(ent, ent->mins, ent->maxs, nodePos, CONTENTS_BODY,
...
qboolean CNavigator::CheckFailedEdge(failedEdge_t *failedEdge) {
	...
	if (failedEdge->checkTime < level.time) {
		if (failedEdge->startID != WAYPOINT_NONE) {
			...
			gi.trace(&trace, start, mins, maxs, end, ignore, clipmask | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP, G2_NOCOLLIDE,
					 0); // NOTE: should we really always include monsterclip (physically blocks NPCs) and botclip (do not enter)?
