...
/*----------------*/
extern DEC_INFO
    decinfo; ////@@@@ this is ok, only written to during init, then chucked.
...
////@@@@static int pMP3Stream->band_limit21 = (576);	// limit for sf band 21
///// L3 only
////@@@@static int pMP3Stream->band_limit12 = (576);	// limit for sf band 12
///short	//L3 only

////@@@@int band_limit_nsb = 32;	/* global for hybrid */
////@@@@static int pMP3Stream->nsb_limit = 32;
////@@@@static int pMP3Stream->gain_adjust = 0;	/* adjust gain e.g. cvt to mono
///*/	// L3 only
////@@@@static int id; // L3 only
////@@@@static int pMP3Stream->ncbl_mixed;		/* number of long cb's in
///mixed block 8 or 6 */		// L3 only
...
static CB_INFO cb_info[2][2]; // L3 only	############ I think this is ok,
                              // only a scratchpad?
static IS_SF_INFO is_sf_info;
    /* MPEG-2 intensity stereo */ // L3 only	 ############## scratchpad?
...
static int nsamp[2][2];
    /* must start = 0, for nsamp[igr_prev] */ // ########## scratchpad?

/*- sample union of int/float  sample[ch][gr][576] */
/* static SAMPLE sample[2][2][576]; */
// @@@@FINDME
////@@@@extern SAMPLE sample[2][2][576];
///////////////????? suspicious, mainly used in decode loop, but zeroed init
///code
...
////@@@@typedef IN_OUT(*DECODE_FUNCTION) (unsigned char *bs, unsigned char
///*pcm);
////@@@@static DECODE_FUNCTION decode_function = L3audio_decode_MPEG1;
///<------------------ needs streaming, ditto above!!!

/*====================================================================*/
int hybrid(void *xin, void *xprev, float *y, int btype, int nlong, int ntot,
           int nprev);
int hybrid_sum(void *xin, void *xin_left, float *y, int btype, int nlong,
               int ntot);
...
static void Xform_mono(void *pcm, int igr) {
  ...
  nsamp[igr][0] =
      hybrid(pMP3Stream->sample[0][igr], pMP3Stream->sample[0][igr_prev], yout,
             side_info.gr[igr][0].block_type, n1, n2, nsamp[igr_prev][0]);
...
static void Xform_dual_right(void *pcm, int igr) {
  ...
  igr_prev = igr ^ 1;
  nsamp[igr][1] =
      hybrid(pMP3Stream->sample[1][igr], pMP3Stream->sample[1][igr_prev], yout,
             side_info.gr[igr][1].block_type, n1, n2, nsamp[igr_prev][1]);
...
static void Xform_dual(void *pcm, int igr) {
  ...
  for (ch = 0; ch < pMP3Stream->nchan; ch++) {
    ...
    if (n2 > pMP3Stream->band_limit)
      n2 = pMP3Stream->band_limit;
    nsamp[igr][ch] = hybrid(
        pMP3Stream->sample[ch][igr], pMP3Stream->sample[ch][igr_prev], yout,
        side_info.gr[igr][ch].block_type, n1, n2, nsamp[igr_prev][ch]);
...
static void Xform_dual_mono(void *pcm, int igr) {
  ...
  if ((side_info.gr[igr][0].block_type == side_info.gr[igr][1].block_type) &&
    ...
    n3 = nsamp[igr][0] = hybrid(
        pMP3Stream->sample[0][igr], pMP3Stream->sample[0][igr_prev], yout,
        side_info.gr[igr][0].block_type, n1, n2, nsamp[igr_prev][0]);
    ...
    nsamp[igr][1] =
        hybrid_sum(pMP3Stream->sample[1][igr], pMP3Stream->sample[0][igr], yout,
                   side_info.gr[igr][1].block_type, n1, n2);
    if (n3 < nsamp[igr][1])
      n1 = nsamp[igr][1];
  }
...
static int unpack_side_MPEG1() {
  ...
  if (br_index > 0) /* pMP3Stream->framebytes fixed for free format */
  {
    pMP3Stream->framebytes =
        2880 * mp_br_tableL3[pMP3Stream->id][br_index] /
        mp_sr20_table[pMP3Stream->id][pMP3Stream->sr_index];
  }
...
static int unpack_side_MPEG2(int igr) {
  ...
  if (br_index > 0) { /* pMP3Stream->framebytes fixed for free format */
    if (pMP3Stream->mpeg25_flag == 0) {
      pMP3Stream->framebytes =
          1440 * mp_br_tableL3[pMP3Stream->id][br_index] /
          mp_sr20_table[pMP3Stream->id][pMP3Stream->sr_index];
    } else {
      pMP3Stream->framebytes =
          2880 * mp_br_tableL3[pMP3Stream->id][br_index] /
...
static void unpack_main(unsigned char *pcm, int igr) {
  ...
  for (ch = 0; ch < pMP3Stream->nchan; ch++) {
    ...
    /*-- scale factors --*/
    if (pMP3Stream->id)
      unpack_sf_sub_MPEG1(&sf[igr][ch], &side_info.gr[igr][ch],
                          side_info.scfsi[ch], igr);
    else
      unpack_sf_sub_MPEG2(&sf[igr][ch], &side_info.gr[igr][ch],
                          pMP3Stream->is_mode & ch, &is_sf_info);
    /*--- huff data ---*/
    ...
    nn3 = n3 - n2;
    nn2 = n2 - n1;
    unpack_huff(pMP3Stream->sample[ch][igr], n1,
                side_info.gr[igr][ch].table_select[0]);
    unpack_huff(pMP3Stream->sample[ch][igr] + n1, nn2,
                side_info.gr[igr][ch].table_select[1]);
    unpack_huff(pMP3Stream->sample[ch][igr] + n2, nn3,
                side_info.gr[igr][ch].table_select[2]);
    qbits = side_info.gr[igr][ch].part2_3_length - (bitget_bits_used() - bit0);
    nn4 = unpack_huff_quad(pMP3Stream->sample[ch][igr] + n3,
                           pMP3Stream->band_limit - n3, qbits,
    ...
    if (bitdat.bs_ptr > bitdat.bs_ptr_end) { // bad data overrun
  ...
  for (ch = 0; ch < pMP3Stream->nchan; ch++) {
    dequant(pMP3Stream->sample[ch][igr],
            ...
            &sf[igr][ch], &side_info.gr[igr][ch], &cb_info[igr][ch],
            pMP3Stream->ncbl_mixed);
  }
  ...
  if (pMP3Stream->ms_mode) {
    if (pMP3Stream->is_mode == 0) {
      ...
      m0 = pMP3Stream
               ->sfBandIndex[cb_info[igr][1].cbtype][cb_info[igr][1].cbmax];
    }
  ...
  if (pMP3Stream->is_mode) {
    if (pMP3Stream->id)
      is_process_MPEG1(pMP3Stream->sample[0][igr], &sf[igr][1], cb_info[igr],
                       nsamp[igr][0], pMP3Stream->ms_mode);
    else
      is_process_MPEG2(pMP3Stream->sample[0][igr], &sf[igr][1], cb_info[igr],
                       &is_sf_info, nsamp[igr][0], pMP3Stream->ms_mode);
  }
...
IN_OUT L3audio_decode_MPEG1(unsigned char *bs, unsigned char *pcm) {
  ...
  if (bs + padframebytes > gpNextByteAfterData)
    return in_out; // error check if we're about to read off the end of the
                   // legal memory (caused by certain MP3 writers' goofy comment
                   // formats) -ste.
  ...
  w---------------------------------------------*/
  pMP3Stream->buf_ptr0 =
      pMP3Stream->buf_ptr1 - side_info.main_data_begin; /* decode start point */
  if (pMP3Stream->buf_ptr1 > BUF_TRIGGER) {             /* shift buffer */
    memmove(pMP3Stream->buf, pMP3Stream->buf + pMP3Stream->buf_ptr0,
  ...
  memmove(pMP3Stream->buf + pMP3Stream->buf_ptr1,
          bs + side_bytes + pMP3Stream->crcbytes, nbytes);
  ...
  if (pMP3Stream->buf_ptr0 >= 0) {
    ...
    memset(pcm, pMP3Stream->zero_level_pcm,
           pMP3Stream->outbytes); /* fill out skipped frames */
    in_out.out_bytes = pMP3Stream->outbytes;
...
/*--------------------------------------------------------------------*/
/*--------------------------------------------------------------------*/
static const int sr_table[8] = {22050, 24000, 16000, 1, 44100, 48000, 32000, 1};
...
static const SBT_FUNCTION sbt_table[2][3][2] = {
    {{(SBT_FUNCTION)sbt_mono_L3, (SBT_FUNCTION)sbt_dual_L3},
     {(SBT_FUNCTION)sbt16_mono_L3, (SBT_FUNCTION)sbt16_dual_L3},
     {(SBT_FUNCTION)sbt8_mono_L3, (SBT_FUNCTION)sbt8_dual_L3}},
    /*-- 8 bit output -*/
    {{(SBT_FUNCTION)sbtB_mono_L3, (SBT_FUNCTION)sbtB_dual_L3},
     {(SBT_FUNCTION)sbtB16_mono_L3, (SBT_FUNCTION)sbtB16_dual_L3},
     {(SBT_FUNCTION)sbtB8_mono_L3, (SBT_FUNCTION)sbtB8_dual_L3}}};
...
static XFORM_FUNCTION xform_table[5] = {
    Xform_mono,       Xform_dual, Xform_dual_mono, Xform_mono, /* left */
    Xform_dual_right,
};
int L3table_init();
void msis_init();
...
int L3audio_decode_init(MPEG_HEAD *h, int framebytes_arg, int reduction_code,
  ...
  if (k == 1) {
    ...
    pMP3Stream->band_limit = pMP3Stream->band_limit21 =
        sfBandIndexTable[k][h->sr_index].l[22];
  } else {
    pMP3Stream->band_limit12 = 3 * sfBandIndexTable[k][h->sr_index].s[12];
    pMP3Stream->band_limit = pMP3Stream->band_limit21 =
        sfBandIndexTable[k][h->sr_index].l[21];
  }
  ...
  pMP3Stream->band_limit_nsb =
      (pMP3Stream->band_limit + 17) / 18; /* limit nsb's rounded up */
  /*----------------------------------------------*/
  ...
  for (i = 0; i < 13; i++)
    pMP3Stream->sfBandIndex[1][i] =
        3 * sfBandIndexTable[k][h->sr_index].s[i + 1];
  for (i = 0; i < 22; i++)
    pMP3Stream->nBand[0][i] = sfBandIndexTable[k][h->sr_index].l[i + 1] -
                              sfBandIndexTable[k][h->sr_index].l[i];
  for (i = 0; i < 13; i++)
    pMP3Stream->nBand[1][i] = sfBandIndexTable[k][h->sr_index].s[i + 1] -
