...
extern void G_ATSTCheckPain(gentity_t *self, gentity_t *other,
                            const vec3_t point, int damage, int mod,
                            int hitLoc);
...
qboolean G_GetRootSurfNameWithVariant(gentity_t *ent, const char *rootSurfName,
...
gentity_t *TossClientItems(gentity_t *self) {
  ...
  if (weapon == WP_SABER) {
    if (self->weaponModel[0] < 0) { // don't have one in right hand
      self->s.weapon = WP_NONE;
    } else if (!(self->client->ps.saber[0].saberFlags & SFL_NOT_DISARMABLE) ||
               g_saberPickuppableDroppedSabers->integer) { // okay to drop it
      if (WP_SaberLose(self, NULL)) {
        self->s.weapon = WP_NONE;
      }
    }
    if (g_saberPickuppableDroppedSabers->integer) { // drop your left one, too
      if (self->weaponModel[1] >= 0) {              // have one in left
        if (!(self->client->ps.saber[0].saberFlags & SFL_NOT_DISARMABLE) ||
            g_saberPickuppableDroppedSabers->integer) { // okay to drop it
          // just drop an item
          if (self->client->ps.saber[1].name &&
              self->client->ps.saber[1]
                  .name[0]) { // have a valid string to use for saberType
            // turn it into a pick-uppable item!
            if (G_DropSaberItem(self->client->ps.saber[1].name,
                                self->client->ps.saber[1].blade[0].color,
                                self->client->renderInfo.handLPoint,
                                self->client->ps.velocity,
                                self->currentAngles) != NULL) { // dropped it
  ...
  } else if (weapon == WP_STUN_BATON || weapon == WP_MELEE) { // never drop
                                                              // these
...
void GoExplodeDeath(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  self->targetname = NULL; // Make sure this entity cannot be told to explode
...
qboolean OnSameTeam(gentity_t *ent1, gentity_t *ent2) {
  if (ent1->s.number < MAX_CLIENTS && ent1->client &&
      ent1->client->playerTeam == TEAM_FREE) { // evil player *has* no allies
    return qfalse;
  }
  if (ent2->s.number < MAX_CLIENTS && ent2->client &&
      ent2->client->playerTeam == TEAM_FREE) { // evil player *has* no allies
    return qfalse;
  }
...
void G_StartMatrixEffect(gentity_t *ent, int meFlags = 0, int length = 1000,
  ...
  if (matrix) {
    ...
    // now the cgame decides when to remove us... in case the framerate chugs so
    // severely that it never finishes the effect before it removes itself!
...
qboolean G_GetHitLocFromSurfName(gentity_t *ent, const char *surfName,
  ...
  *hitLoc = HL_NONE;
  ...
  if (!Q_stricmpn("hips", surfName, 4)) { // FIXME: test properly for legs
    *hitLoc = HL_WAIST;
      ...
      if (ent->footLBolt >= 0) {
        ...
        if (DistanceSquared(point, tagOrg) < 100) { // actually hit the foot
          *hitLoc = HL_FOOT_LT;
             ...
             WP_BreakSaber(ent, surfName, saberType)) { // saber hit and broken
    *hitLoc = HL_HAND_RT;
  } else if (!Q_stricmpn("r_hand", surfName, 6) ||
             !Q_stricmpn("w_", surfName, 2)) { // right hand or weapon
    // FIXME: if hit weapon, chance of breaking saber (if sabers.cfg entry shows
    // it as breakable) 			if breaks, remove saber and replace with the 2
    //replacement sabers (preserve color, length, etc.)
    *hitLoc = HL_HAND_RT;
  } else if (!Q_stricmpn("l_hand", surfName, 6)) {
    *hitLoc = HL_HAND_LT;
  } else if (ent->client && ent->client->ps.powerups[PW_GALAK_SHIELD] &&
             !Q_stricmp("force_shield", surfName)) {
    *hitLoc = HL_GENERIC2;
  ...
  if (g_saberRealisticCombat->integer > 1 || debug_subdivision->integer) {
    dismember = qtrue;
...
int G_PickPainAnim(gentity_t *self, const vec3_t point, int damage,
                   int hitLoc = HL_NONE) {
  if (hitLoc == HL_NONE) {
...
void G_RemoveWeaponsWithLimbs(gentity_t *ent, gentity_t *limb, int limbAnim) {
  ...
  for (weaponModelNum = 0; weaponModelNum < MAX_INHAND_WEAPONS;
       weaponModelNum++) {
    if (ent->weaponModel[weaponModelNum] >= 0) { // have a weapon in this hand
      if (weaponModelNum == 0 &&
          ent->client->ps
              .saberInFlight) { // this is the right-hand weapon and it's a
                                // saber in-flight (i.e.: not in-hand, so no need
                                // to worry about it)
        continue;
      }
      ...
      { // FIXME: is this first check needed with this lower one?
        ...
        } else { // the hand had already been removed
          if (ent->weaponModel[weaponModelNum] >
              0) { // still a weapon associated with it, remove it from the limb
            gi.G2API_RemoveGhoul2Model(limb->ghoul2,
                                       ent->weaponModel[weaponModelNum]);
            limb->weaponModel[weaponModelNum] = -1;
          }
        }
      } else { // this weapon isn't affected by this dismemberment
        if (ent->weaponModel[weaponModelNum] >
            0) { // but a weapon was copied over to the limb, so remove it
          gi.G2API_RemoveGhoul2Model(limb->ghoul2,
                                     ent->weaponModel[weaponModelNum]);
          limb->weaponModel[weaponModelNum] = -1;
        }
...
static qboolean G_Dismember(gentity_t *ent, vec3_t point, const char *limbBone,
                            const char *rotateBone, const char *limbName,
                            const char *limbCapName, const char *stubCapName,
                            const char *limbTagName, const char *stubTagName,
                            int limbAnim, float limbRollBase,
                            float limbPitchBase, int damage, int hitLoc) {
  // int newBolt;
  ...
  if (gi.G2API_GetSurfaceRenderStatus(&ent->ghoul2[ent->playerModel],
                                      limbName) ==
      0x00000100 /*G2SURFACEFLAG_NODESCENDANTS*/) { // already lost this limb
  ...
  if (limbTagName) { // add smoke to cap tag
    ...
    {
            G_PlayEffect( G_EffectIndex("saber/limb_bolton"), limb->playerModel,
    newBolt, limb->s.number, newPoint);
    }*/
  }
  /*
  ...
  */
  ...
  // no 50ms lag 		this is neccessary because the Ghoul2 info does not have to go
  //over the network, 		cgame looks at game's data.  The new limb ent will be
  //delayed so we will see 		that a limb is missing before the chopped off limb
  //appears. 		also, if the limb was going to start in solid, we can delete it and
  //return
  ...
  gi.trace(&trace, limb->s.pos.trBase, limb->mins, limb->maxs,
           limb->s.pos.trBase, limb->s.number, limb->clipmask, (EG2_Collision)0,
           0);
  if (trace.startsolid) {
    limb->s.pos.trBase[2] -= limb->mins[2];
    gi.trace(&trace, limb->s.pos.trBase, limb->mins, limb->maxs,
             limb->s.pos.trBase, limb->s.number, limb->clipmask,
             (EG2_Collision)0, 0);
    if (trace.startsolid) {
      limb->s.pos.trBase[2] += limb->mins[2];
      gi.trace(&trace, limb->s.pos.trBase, limb->mins, limb->maxs,
...
qboolean G_GetRootSurfNameWithVariant(gentity_t *ent, const char *rootSurfName,
                                      char *returnSurfName, int returnSize) {
  if (!gi.G2API_GetSurfaceRenderStatus(
          &ent->ghoul2[ent->playerModel],
          rootSurfName)) { // see if the basic name without variants is on
...
qboolean G_DoDismemberment(gentity_t *self, vec3_t point, int mod, int damage,
  ...
  // NOTE that you can only cut one thing off unless the debug_subdivisions is
  // on
  ...
  {                     // FIXME: don't do strcmps here
    if (G_StandardHumanoid(self) &&
      ...
      // the direction of the cap, then the split is allowed, otherwise we
      // hit it at the wrong angle and should not dismember...
      const char *limbBone = NULL, *rotateBone = NULL, *limbTagName = NULL,
                 *stubTagName = NULL;
      int anim = -1;
      ...
      case HL_LEG_RT:
        if (g_dismemberment->integer > 1) {
          ...
          limbBone = "rtibia";
          rotateBone = "rtalus";
          G_GetRootSurfNameWithVariant(self, "r_leg", limbName,
                                       sizeof(limbName));
          G_GetRootSurfNameWithVariant(self, "hips", stubName,
                                       sizeof(stubName));
          Com_sprintf(limbCapName, sizeof(limbCapName), "%s_cap_hips",
                      limbName);
          Com_sprintf(stubCapName, sizeof(stubCapName), "%s_cap_r_leg",
                      stubName);
      ...
      case HL_LEG_LT:
        if (g_dismemberment->integer > 1) {
          ...
          limbBone = "ltibia";
          rotateBone = "ltalus";
          G_GetRootSurfNameWithVariant(self, "l_leg", limbName,
                                       sizeof(limbName));
          G_GetRootSurfNameWithVariant(self, "hips", stubName,
                                       sizeof(stubName));
          Com_sprintf(limbCapName, sizeof(limbCapName), "%s_cap_hips",
                      limbName);
          Com_sprintf(stubCapName, sizeof(stubCapName), "%s_cap_l_leg",
                      stubName);
      ...
      case HL_BACK_RT:
        if (g_dismemberment->integer) {
          ...
          limbBone = "rhumerus";
          rotateBone = "rradius";
          G_GetRootSurfNameWithVariant(self, "r_arm", limbName,
                                       sizeof(limbName));
          G_GetRootSurfNameWithVariant(self, "torso", stubName,
                                       sizeof(stubName));
          Com_sprintf(limbCapName, sizeof(limbCapName), "%s_cap_torso",
                      limbName);
          Com_sprintf(stubCapName, sizeof(stubCapName), "%s_cap_r_arm",
                      stubName);
      ...
      case HL_BACK_LT:
        if (g_dismemberment->integer &&
          ...
          limbBone = "lhumerus";
          rotateBone = "lradius";
          G_GetRootSurfNameWithVariant(self, "l_arm", limbName,
                                       sizeof(limbName));
          G_GetRootSurfNameWithVariant(self, "torso", stubName,
                                       sizeof(stubName));
          Com_sprintf(limbCapName, sizeof(limbCapName), "%s_cap_torso",
                      limbName);
          Com_sprintf(stubCapName, sizeof(stubCapName), "%s_cap_l_arm",
                      stubName);
      ...
      case HL_HAND_RT:
        if (g_dismemberment->integer) {
          ...
          limbBone = "rradiusX";
          rotateBone = "rhand";
          G_GetRootSurfNameWithVariant(self, "r_hand", limbName,
                                       sizeof(limbName));
          G_GetRootSurfNameWithVariant(self, "r_arm", stubName,
                                       sizeof(stubName));
          Com_sprintf(limbCapName, sizeof(limbCapName), "%s_cap_r_arm",
                      limbName);
      ...
      case HL_HAND_LT:
        if (g_dismemberment->integer) {
          ...
          limbBone = "lradiusX";
          rotateBone = "lhand";
          G_GetRootSurfNameWithVariant(self, "l_hand", limbName,
                                       sizeof(limbName));
          G_GetRootSurfNameWithVariant(self, "l_arm", stubName,
                                       sizeof(stubName));
          Com_sprintf(limbCapName, sizeof(limbCapName), "%s_cap_l_arm",
                      limbName);
          Com_sprintf(stubCapName, sizeof(stubCapName), "%s_cap_l_hand",
                      stubName);
...
G_PickDeathAnim(gentity_t *self, vec3_t point, int damage, int mod,
  ...
  case BOTH_DEADBACKWARD2: //# Second thrown backward death finished pose
    // return -2;
    // break;
    if (PM_FinishedCurrentLegsAnim(self)) { // done with the anim
  ...
  if (deathAnim == -1 || !PM_HasAnimation(self, deathAnim)) {
    if (deathAnim == BOTH_DEADFLOP1 ||
        deathAnim ==
            BOTH_DEADFLOP2) { // if don't have deadflop, don't do anything
      deathAnim = -1;
    } else {
...
int G_CheckLedgeDive(gentity_t *self, float checkDist, const vec3_t checkVel,
...
void player_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
            ...
            if (CurSpeed < (pVeh->m_pVehicleInfo->speedMax * 0.9f)) {
              ...
              if (fabsf(parent->pos3[2]) < 0.3f) {
                VectorScale(parent->pos3,
                            (pVeh->m_pVehicleInfo->speedMax * 1.25f),
                            parent->pos3);
              } else {
  ...
  if (self->NPC) {
    ...
    if (self->client && self->client->NPC_class == CLASS_ASSASSIN_DROID &&
      ...
      gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel], "force_shield",
                               TURN_OFF);
    }

    if (self->client && self->client->NPC_class == CLASS_HOWLER) {
      G_StopEffect(G_EffectIndex("howler/sonic"), self->playerModel,
    ...
    if ((self->client->ps.stats[STAT_WEAPONS] & (1 << WP_SCEPTER))) {
      ...
      G_StopEffect(G_EffectIndex("scepter/beam.efx"), self->weaponModel[1],
                   self->genericBolt1, self->s.number);
  ...
  if (self->client->ps.saberEntityNum != ENTITYNUM_NONE &&
    ...
    if (self->client->ps.saberInFlight) { // just drop it
      ...
      if (g_saberPickuppableDroppedSabers->integer) { // always drop your sabers
        TossClientItems(self);
        self->client->ps.weapon = self->s.weapon = WP_NONE;
      } else if (((hitLoc != HL_HAND_RT && hitLoc != HL_CHEST_RT &&
                   hitLoc != HL_ARM_RT && hitLoc != HL_BACK_LT) ||
                  self->client->dismembered ||
                  meansOfDeath != MOD_SABER) // if might get hand cut off, leave
                                             // saber in hand
                 && holdingSaber &&
                 (Q_irand(0, 1) || meansOfDeath == MOD_EXPLOSIVE ||
                  ...
                  meansOfDeath == MOD_ROCKET_ALT || meansOfDeath == MOD_CONC ||
                  meansOfDeath == MOD_CONC_ALT || meansOfDeath == MOD_THERMAL ||
    ...
    if (d_slowmodeath->integer) {
      if (!self->s.number) { // what the hell, always do slow-mo when player
        ...
        if (meansOfDeath == MOD_FALLING &&
          ...
          G_StartMatrixEffect(
              self, (MEF_NO_VERTBOB | MEF_HIT_GROUND_STOP | MEF_MULTI_SPIN),
              10000, 0.25f);
        ...
        if (!attacker->s.number &&
            ...
            meansOfDeath == MOD_SABER && attacker->client &&
            attacker->client->ps.weapon == WP_SABER &&
            !attacker->client->ps
                 .saberInFlight // FIXME: if dualSabers, should still do slowmo
                                // if this killing blow was struck with the
                                // left-hand saber...
            && (d_slowmodeath->integer > 2 ||
                lastInGroup)) // either slow mo death level 3 (any jedi) or 2
                              // and I was the last jedi in the room
        { // Matrix!
          if (attacker->client->ps.torsoAnim ==
              BOTH_A6_SABERPROTECT) { // don't override the range and vertbob
        ...
        if (!attacker->s.number && meansOfDeath == MOD_SABER &&
            ...
            (d_slowmodeath->integer > 4 || lastInGroup || holdingSaber ||
             self->client->NPC_class ==
                 CLASS_WAMPA)) // either slow mo death level 5 (any enemy) or 4
                               // and I was the last in my group or I'm a saber
                               // user
        { // Matrix!
          if (attacker->client->ps.torsoAnim ==
              BOTH_A6_SABERPROTECT) { // don't override the range and vertbob
  ...
  if (holdingSaber) { // never drop a lightsaber!
    if (self->client->ps.SaberActive()) {
      ...
      G_SoundIndexOnEnt(self, CHAN_AUTO, self->client->ps.saber[0].soundOff);
             ...
             WP_BRYAR_PISTOL) { // since player can't pick up bryar pistols,
                                // never drop those
    self->s.weapon = WP_NONE;
  ...
  */
  ...
  if (self->NPC) {
    ...
    if (self->ghoul2.size()) {
      ...
      if (self->headBolt != -1) {
        G_StopEffect("force/confusion", self->playerModel, self->headBolt,
                     self->s.number);
      }
  ...
  if ((self->client->ps.eFlags & EF_HELD_BY_RANCOR) ||
      ...
      (self->client->ps.eFlags & EF_HELD_BY_WAMPA)) { // do nothing special here
  } else if (self->client->NPC_class == CLASS_MARK1) {
    ...
    if (anim == -1) {
      if (PM_InOnGroundAnim(&self->client->ps) &&
          PM_HasAnimation(
              self, BOTH_LYINGDEATH1)) { // on ground, need different death anim
        anim = BOTH_LYINGDEATH1;
      } else if (meansOfDeath == MOD_TRIGGER_HURT &&
                 (self->s.powerups & (1 << PW_SHOCKED))) { // electrocuted
        anim = BOTH_DEATH17;
      } else if (meansOfDeath == MOD_WATER || meansOfDeath == MOD_GAS ||
                 meansOfDeath == MOD_FORCE_DRAIN) { // drowned
        anim = BOTH_DEATH17;
    ...
    if (anim == -1) {
      if (meansOfDeath == MOD_ELECTROCUTE ||
          ...
          (meansOfDeath == MOD_FORCE_DRAIN &&
           self->s.eFlags &
               EF_FORCE_DRAINED)) { // electrocuted or choked to death
        anim = BOTH_DEATH17;
      } else {
        anim = G_PickDeathAnim(self, self->pos1, damage, meansOfDeath, hitLoc);
      }
    ...
    } else if (!PM_HasAnimation(self, anim)) { // crap, still missing an anim,
                                               // so pick one that we do have
      anim = PM_PickAnim(self, BOTH_DEATH1,
                         BOTH_DEATH25); // initialize to good data
    }
    ...
    if (meansOfDeath == MOD_KNOCKOUT) {
      ...
      if (hitLoc == HL_HEAD && !(dflags & DAMAGE_RADIUS) &&
          ...
          meansOfDeath != MOD_ROCKET_ALT && meansOfDeath != MOD_CONC &&
          meansOfDeath != MOD_THERMAL && meansOfDeath != MOD_THERMAL_ALT &&
          meansOfDeath != MOD_DETPACK && meansOfDeath != MOD_LASERTRIP &&
          meansOfDeath != MOD_LASERTRIP_ALT && meansOfDeath != MOD_EXPLOSIVE &&
          meansOfDeath !=
        ...
        if ((self->client->ps.eFlags & EF_FORCE_GRIPPED) ||
            (self->client->ps.eFlags &
             EF_FORCE_DRAINED)) { // killed while gripped - no loud scream
          G_AlertTeam(self, attacker, 512, 32);
  ...
  if (!deathScript && !(self->svFlags & SVF_KILLED_SELF)) {
    ...
    // WARNING!!! DO NOT DO THIS WHILE RUNNING A SCRIPT, ICARUS WILL HANDLE IT,
    // but it's bad
    ...
    if (level.time > self->painDebounceTime &&
      ...
      if (!Q3_TaskIDPending(self, TID_CHAN_VOICE)) {
        if (self->client->damage_blood) { // took damage myself, not just armor
          if (mod == MOD_GAS) {
            // SIGH... because our choke sounds are inappropriately long, I have
            // to debounce them in code!
    ...
    if (damage != -1 &&
      ...
      if ((((mod == MOD_SABER || mod == MOD_MELEE) &&
          ...
          (self->s.weapon == WP_SABER || self->s.weapon == WP_MELEE ||
           cg.renderingThirdPerson)) // FIXME: not only if using saber, but if
                                     // in third person at all?  But then
                                     // 1st/third person functionality is
                                     // different...
        ...
        if (!G_CheckForStrongAttackMomentum(self) &&
          ...
          if ((parts == SETANIM_BOTH && damage > 30) ||
              (self->painDebounceTime > level.time &&
               damage > 10)) { // took a lot of damage in 1 hit //or took 2 hits
                               // in quick succession
...
int CheckArmor(gentity_t *ent, int damage, int dflags, int mod) {
  ...
  if (client->NPC_class == CLASS_ASSASSIN_DROID) {
    ...
    //-----------------------------------------------------------
    if (mod == MOD_GAS || mod == MOD_IMPACT || mod == MOD_LAVA ||
        mod == MOD_SLIME || mod == MOD_WATER || mod == MOD_FORCE_GRIP ||
        mod == MOD_FORCE_DRAIN || mod == MOD_SEEKER || mod == MOD_MELEE ||
        mod == MOD_BOWCASTER || mod == MOD_BRYAR || mod == MOD_BRYAR_ALT ||
        mod == MOD_BLASTER || mod == MOD_BLASTER_ALT || mod == MOD_SNIPER ||
        mod == MOD_BOWCASTER || mod == MOD_BOWCASTER_ALT ||
        mod == MOD_REPEATER || mod == MOD_REPEATER_ALT) {
      return damage;
    }
    ...
    //----------------------------------------------------
    if (mod == MOD_SABER || mod == MOD_FLECHETTE || mod == MOD_FLECHETTE_ALT ||
        mod == MOD_DISRUPTOR) {
      return damage;
    }
...
extern qboolean Jedi_StopKnockdown(gentity_t *self, gentity_t *pusher,
                                   const vec3_t pushDir);
void G_Knockdown(gentity_t *self, gentity_t *attacker, const vec3_t pushDir,
  ...
  if (Boba_StopKnockdown(self, attacker, pushDir)) {
    return;
  } else if (Jedi_StopKnockdown(self, attacker,
                                pushDir)) { // They can sometimes backflip
                                            // instead of be knocked down
    return;
  ...
  if (self->health > 0) {
    ...
    if (!PM_SpinningSaberAnim(self->client->ps.legsAnim) &&
      ...
      NPC_SetAnim(self, SETANIM_BOTH, knockAnim,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      if (self->s.number >= MAX_CLIENTS) { // randomize getup times
...
void G_CheckKnockdown(gentity_t *targ, gentity_t *attacker, vec3_t newDir,
  ...
  if (!(dflags & DAMAGE_RADIUS)) { // not inherently explosive damage, check mod
    if (mod != MOD_REPEATER_ALT && mod != MOD_FLECHETTE_ALT &&
        mod != MOD_ROCKET && mod != MOD_ROCKET_ALT && mod != MOD_CONC &&
        mod != MOD_CONC_ALT && mod != MOD_THERMAL && mod != MOD_THERMAL_ALT &&
        mod != MOD_DETPACK && mod != MOD_LASERTRIP &&
  ...
  if (targ && targ->client &&
      (targ->client->NPC_class == CLASS_ATST ||
       ...
       targ->client->NPC_class ==
           CLASS_WAMPA)) { // much to large to *ever* throw
    return;
  }
...
qboolean G_NonLocationSpecificDamage(int meansOfDeath) {
  if (meansOfDeath == MOD_EXPLOSIVE || meansOfDeath == MOD_REPEATER_ALT ||
      ...
      meansOfDeath == MOD_THERMAL || meansOfDeath == MOD_THERMAL_ALT ||
      meansOfDeath == MOD_DETPACK || meansOfDeath == MOD_LASERTRIP ||
      meansOfDeath == MOD_LASERTRIP_ALT || meansOfDeath == MOD_MELEE ||
      meansOfDeath == MOD_FORCE_GRIP || meansOfDeath == MOD_KNOCKOUT ||
      meansOfDeath == MOD_CRUSH || meansOfDeath == MOD_EXPLOSIVE_SPLASH) {
    return qtrue;
  }
  return qfalse;
}
...
qboolean G_ImmuneToGas(gentity_t *ent) {
  if (!ent || !ent->client) { // only effects living clients
    return qtrue;
  }
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
  ...
  if (!targ->takedamage) {
    if (targ->client                                      // client
        ...
        && targ->activator ==
               inflictor) // inflictor of damage is the thing in our mouth
  ...
  if (targ->s.eFlags & EF_LOCKED_TO_WEAPON && targ->s.number == 0 &&
      targ->owner && !targ->owner->bounceCount // not an EWeb
  ...
  if ((targ->svFlags & SVF_BBRUSH) ||
      (!targ->client && Q_stricmp("misc_model_breakable", targ->classname) ==
                            0)) // FIXME: flag misc_model_breakables?
  {                             // breakable brush or misc_model_breakable
    if (targ->NPC_targetname) { // only a certain attacker can destroy this
      if (!attacker || !attacker->targetname ||
          Q_stricmp(targ->NPC_targetname, attacker->targetname) !=
              0) { // and it's not this one, do nothing
        return;
      }
    }
  }
  ...
  } else if (mod == MOD_SABER) { // sabers do less damage to mark1's and atst's,
                                 // and to hazard troopers and assassin droids
    if (targ->client) {
      ...
      /*
      //NOTE: a more controlled way to do the class-specific saber immunities,
      if we want else if ( targ->client->NPC_class == CLASS_ASSASSIN_DROID )
      {//takes 2 hits to kill on easy, 3 on medium, 4 on hard
              int maxDamage =
      ceil((float)targ->max_health/(2.0f+g_spskill->value)); if ( damage >
      maxDamage )
              {
                      damage = maxDamage;
              }
      ...
      {//takes 3 hits to kill on easy, 4 on medium, 5 on hard
              int maxDamage =
      ceil((float)targ->max_health/(3.0f+g_spskill->value)); if ( damage >
      maxDamage )
              {
                      damage = maxDamage;
              }
  ...
  if (mod == MOD_GAS) {        // certain NPCs are immune
    if (G_ImmuneToGas(targ)) { // immune
      return;
    }
  ...
  if ((client) && (mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT) &&
      (client->NPC_class == CLASS_SABER_DROID ||
       ...
       client->NPC_class == CLASS_GONK || client->NPC_class == CLASS_MOUSE ||
       client->NPC_class == CLASS_PROBE ||
       client->NPC_class == CLASS_PROTOCOL || client->NPC_class == CLASS_R2D2 ||
       client->NPC_class == CLASS_R5D2 || client->NPC_class == CLASS_SEEKER ||
  ...
  if (client && client->NPC_class == CLASS_GALAKMECH &&
      !(dflags & DAMAGE_NO_PROTECTION)) {   // hit Galak
    if (client->ps.stats[STAT_ARMOR] > 0) { // shields are up
      ...
      if ((dflags & DAMAGE_RADIUS) || mod == MOD_REPEATER_ALT ||
          ...
          mod == MOD_ROCKET_ALT || mod == MOD_CONC || mod == MOD_THERMAL ||
  ...
  if (targ && targ->client && !(dflags & DAMAGE_NO_PROTECTION) &&
      !(dflags & DAMAGE_DIE_ON_IMPACT)) // falling to you death ignores force
                                        // protect and force rage (but obeys
                                        // godmode and undying flags)
  { // force protections
    // rage
    if ((targ->client->ps.forcePowersActive & (1 << FP_RAGE))) {
      damage = floor((float)damage /
                     (float)(targ->client->ps.forcePowerLevel[FP_RAGE] * 2));
    }
    // protect
    if ((targ->client->ps.forcePowersActive & (1 << FP_PROTECT))) {
      ...
      case FORCE_LEVEL_3:
              ...
              case MOD_CRUSH:
                      ...
                      damage =
      floor((float)damage/(float)(targ->client->ps.forcePowerLevel[FP_PROTECT]-1));
                      break;
              }
      case FORCE_LEVEL_2:
              //NOTE: purposely falls through
              ...
              case MOD_ELECTROCUTE:
                      doSound = (Q_irand(0,4)==0);
                      damage =
      floor((float)damage/(float)(targ->client->ps.forcePowerLevel[FP_PROTECT]));
                      break;
              }
      case FORCE_LEVEL_1:
              switch ( mod )
      ...
      */
      ...
      if (mod == MOD_FALLING && targ->NPC &&
          (targ->NPC->aiFlags &
           NPCAI_DIE_ON_IMPACT)) { // if falling to your death, protect can't
                                   // save you
      } else {
        qboolean doSound = qfalse;
        switch (mod) {
        case MOD_CRUSH:
          if (attacker &&
              attacker->client) { // need to still be crushed by AT-ST
            break;
          }
        case MOD_REPEATER_ALT:
        case MOD_FLECHETTE_ALT:
    ...
    if ( (targ->client->ps.forcePowersActive & (1 << FP_ABSORB)) )
    {
            ...
            {
                    int absorbed =
    targ->client->ps.forcePowerLevel[FP_ABSORB]*5; damage -= absorbed; if (
    damage < 0 )
                    {
                            absorbed += damage;
                            damage = 0;
                    ...
                    //absorb the energy
                    //make absorb sound
                    G_SoundOnEnt( targ, CHAN_ITEM,
    "sound/weapons/force/absorbhit.wav" ); targ->client->ps.forcePower +=
    absorbed;
            }
    }
    */
  ...
  if (targ->client &&
      ...
      targ->client->ps.forcePowerLevel[FP_PROTECT] ==
          FORCE_LEVEL_3) { // pretend there was no damage?
    knockback = 0;
    ...
    knockback = 0;
             ...
             && targ->client->playerTeam ==
                    attacker->client->playerTeam) // on same team
  ...
  if (((targ->flags & FL_GODMODE) ||
       (targ->client &&
        targ->client->ps.powerups[PW_INVINCIBLE] > level.time)) &&
      !(dflags & DAMAGE_NO_PROTECTION)) {
    if (targ->client && attacker->client &&
        targ->client->playerTeam == attacker->client->playerTeam &&
        (!targ->NPC ||
  ...
  */
  if (client) {
    ...
    if (!asave) { // nothing was absorbed (or just ran out?)
    } else if (targ->client->NPC_class !=
               CLASS_VEHICLE) { // vehicles don't have personal shields
      targ->client->ps.powerups[PW_BATTLESUIT] = level.time + ARMOR_EFFECT_TIME;
      if (targ->client->ps.stats[STAT_ARMOR] <= 0) { // all out of armor
    ...
    if (mod == MOD_SLIME || mod == MOD_LAVA) {
      ...
      if (mod == MOD_SLIME) {
        ...
        // for (int i=0; i<numPuffs; i++)
        {
          ...
          gi.trace(&testTrace, testStartPos, NULL, NULL, testEndPos,
                   ENTITYNUM_NONE, MASK_SHOT, G2_COLLIDE, 0);

          if (!testTrace.startsolid && !testTrace.allsolid &&
              testTrace.entityNum == targ->s.number &&
              testTrace.G2CollisionMap[0].mEntityNum != -1) {
            G_PlayEffect("world/acid_fizz",
                         testTrace.G2CollisionMap[0].mCollisionPosition);
          }
          //					CG_DrawEdge(testStartPos,	testEndPos,
          //EDGE_IMPACT_POSSIBLE);
          float chanceOfFizz = gi.WE_GetChanceOfSaberFizz();
          TIMER_Set(targ, "AcidPainDebounce",
                    200 + (10000.0f * Q_flrand(0.0f, 1.0f) * chanceOfFizz));
          hitLoc = HL_CHEST;
        }
      }
    }
    ...
    if (targ->client->NPC_class == CLASS_VEHICLE) {
      ...
      if ((damage > 0) &&       // Actually took some damage
          ...
          (targ->m_pVehicle->m_pVehicleInfo->type ==
           VH_SPEEDER) &&       // and is a speeder
                                //	 (targ->client->ps.speed > 30.0f) &&						//
                                //and is moving
      ...
      if ((damage > 0) &&       // Actually took some damage
          ...
          (targ->m_pVehicle->m_pVehicleInfo->type ==
           VH_SPEEDER) && // and is a speeder
          !(targ->m_pVehicle->m_ulFlags &
            VEH_OUTOFCONTROL) &&              // and is not already spinning
          (targ->client->ps.speed > 30.0f) && // and is moving
          (attacker == inflictor ||
           Q_irand(0, 30) == 0) // and EITHER saber is held, or 1 in 30 chance
                                // of hitting when thrown
      ) {
        ...
        if (attacker == inflictor &&
            (!G_IsRidingVehicle(attacker) || Q_irand(0, 3) == 0)) {
          ...
          if (!G_IsRidingVehicle(attacker)) {
            G_StartRoll(attacker,
                        (Q_irand(0, 1) == 0) ? (BOTH_ROLL_L) : (BOTH_ROLL_R));
          }
        }

        if (targ->m_pVehicle->m_pPilot &&
            targ->m_pVehicle->m_pPilot->s.number >= MAX_CLIENTS) {
        ...
        // DISMEMBER THE FRONT PART OF THE MODEL
        {
          ...
          gi.G2API_SetSurfaceOnOff(&targ->ghoul2[targ->playerModel], "lfront",
                                   TURN_OFF);
          animation_t *animations =
              level.knownAnimFileSets[targ->client->clientInfo.animFileIndex]
                  .animations;

          // play the proper dismember anim on the limb
          gi.G2API_SetBoneAnim(&limb->ghoul2[limb->playerModel], 0,
                               animations[BOTH_A1_BL_TR].firstFrame,
                               animations[BOTH_A1_BL_TR].numFrames +
                                   animations[BOTH_A1_BL_TR].firstFrame,
                               BONE_ANIM_OVERRIDE_FREEZE, 1, level.time, -1,
                               -1);
      ...
      if ((targ->m_pVehicle->m_iArmor <= 0) &&
          targ->m_pVehicle->m_pVehicleInfo->type !=
              VH_ANIMAL) { // vehicle all out of armor
        Vehicle_t *pVeh = targ->m_pVehicle;
        if (dflags & DAMAGE_IMPACT_DIE) {
          // kill it now
          pVeh->m_pVehicleInfo->StartDeathDelay(
              pVeh, -1 /* -1 causes instant death */);
        } else {
          if (pVeh->m_iDieTime ==
              0) { // just start the flaming effect and explosion delay, if it's
                   // not going already...
  ...
  qboolean targUndying = (qboolean)(
      ...
      ((targ->flags & FL_UNDYING) || (dflags & DAMAGE_NO_KILL) ||
       ((targ->client) &&
        ...
        !(dflags & DAMAGE_NO_PROTECTION) && !(dflags & DAMAGE_DIE_ON_IMPACT))));

  if (targ->client && targ->client->NPC_class == CLASS_WAMPA && targ->count &&
      take >= targ->health) { // wampa holding someone, don't die unless you can
                              // release them!
    qboolean removeArm = qfalse;
    if (targ->activator && attacker == targ->activator && mod == MOD_SABER) {
      removeArm = qtrue;
    }
    if (Wampa_CheckDropVictim(targ, qtrue)) { // released our victim
      if (removeArm) {
        targ->client->dismembered = false;
        // FIXME: the limb should just disappear, cuz I ate it
        G_DoDismemberment(targ, targ->currentOrigin, MOD_SABER, 1000, HL_ARM_RT,
                          qtrue);
      }
    } else { // couldn't release him
      targUndying = qtrue;
    }
  }
  ...
  if (take || (dflags & DAMAGE_NO_DAMAGE)) {
    if (!targ->client || !attacker->client) {
      ...
      if (targTeam != attackerTeam ||
          (targ->s.number < MAX_CLIENTS &&
           targTeam == TEAM_FREE) // evil player hit
          || (attacker && attacker->s.number < MAX_CLIENTS &&
              attackerTeam == TEAM_FREE)) // evil player attacked
      {                                   // on opposite team
        targ->health = targ->health - take;
        ...
        if (targUndying) {
          if (targ->health < 1) {
            if (targ->NPC == NULL || !(targ->NPC->aiFlags & NPCAI_ROSH) ||
                !Rosh_TwinPresent(
                    targ)) { // NOTE: Rosh won't run his deathscript until he
                             // doesn't have the twins to heal him
              G_ActivateBehavior(targ, BSET_DEATH);
            }
            targ->health = 1;
          }
...
qboolean CanDamage(gentity_t *targ, const vec3_t origin) {
  ...
  if ((targ->contents & MASK_SOLID)) {   // can hit it
    if (targ->s.solid == SOLID_BMODEL) { // but only if it's a brushmodel
      cantHitEnt = qfalse;
    }
  }
...
extern void G_GetMassAndVelocityForEnt(gentity_t *ent, float *mass,
                                       vec3_t velocity);
/*
============
G_RadiusDamage
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (ent->client &&
      ...
      if (vehMoveSpeed > 300.0f) {
        ...
        explosionDirectionSimilarity =
            DotProduct(vehMoveDirection, explosionDirection);
