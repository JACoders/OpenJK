  ...
  for (i = 0, drawSurf = drawSurfs; i < numDrawSurfs; i++, drawSurf++) {
    ...
    if (entityNum != REFENTITYNUM_WORLD &&
      ...
      if ((backEnd.refdef.entities[entityNum].e.renderfx & RF_DISTORTION) ||
        ...
        oldSort = (unsigned int)-1; // invalidate this thing, cause we may want
                                    // to postrender more surfs of the same sort
  ...
  if (g_numPostRenders > 0) {
    ...
    while (g_numPostRenders > 0) {
      ...
      if ((backEnd.currentEntity->e.renderfx & RF_DISTORTION) &&
          lastPostEnt != pRender->entNum) { // do the capture now, we only need
                                            // to do it once per ent
        int x, y;
        ...
        if (R_WorldCoordToScreenCoord(backEnd.currentEntity->e.origin, &x,
                                      &y)) {
          int cX, cY;
  ...
  */
  ...
  if (!(backEnd.refdef.rdflags & RDF_NOWORLDMODEL) && g_bDynamicGlowSupported &&
    ...
    qglCopyTexSubImage2D(
        GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, backEnd.viewParms.viewportX,
...
const void *RB_DrawBuffer(const void *data) {
  ...
  } else if (!(backEnd.refdef.rdflags & RDF_NOWORLDMODEL) && tr.world &&
             tr.world->globalFog != -1 &&
             tr.sceneCount) // don't clear during menus, wait for real scene
  {
    const fog_t *fog = &tr.world->fogs[tr.world->globalFog];
...
const void *RB_SwapBuffers(const void *data) {
  ...
  if (r_measureOverdraw->integer) {
    ...
    stencilReadback = (unsigned char *)R_Malloc(
        glConfig.vidWidth * glConfig.vidHeight, TAG_TEMP_WORKSPACE, qfalse);
