...
#ifdef JK2_MODE
void LoadJPGFromBuffer(byte *inputBuffer, size_t len, unsigned char **pic,
  ...
  if (!cinfo.output_width || !cinfo.output_height ||
    ...
    ri.Printf(PRINT_ALL,
              "LoadJPG: invalid image format: %dx%d*4=%d, components: %d",
  ...
  *height = cinfo.output_height;
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
     ...
     */
    buf = ((out +
            (row_stride * (cinfo.output_height - cinfo.output_scanline - 1))));
    buffer = &buf;
    (void)jpeg_read_scanlines(&cinfo, buffer, 1);
  }
