...
stringID_table_t footstepTypeTable[] = {ENUM2STRING(FOOTSTEP_R),
...
stringID_table_t TeamTable[] = {
    {"free", TEAM_FREE},    // caution, some code checks a team_t via "if
                            // (!team_t_varname)" so I guess this should stay as
                            // entry 0, great or what? -slc
    ENUM2STRING(TEAM_FREE), // caution, some code checks a team_t via "if
                            // (!team_t_varname)" so I guess this should stay as
                            // entry 0, great or what? -slc
    {"player", TEAM_PLAYER},
    ...
    {"neutral", TEAM_NEUTRAL}, // most droids are team_neutral, there are some
                               // exceptions like Probe,Seeker,Interrogator
    ENUM2STRING(TEAM_NEUTRAL), // most droids are team_neutral, there are some
                               // exceptions like Probe,Seeker,Interrogator
    {"", -1}};

// this list was made using the model directories, this MUST be in the same
// order as the CLASS_ enum in teams.h
stringID_table_t ClassTable[] = {
    ENUM2STRING(CLASS_NONE), // hopefully this will never be used by an npc),
...
int CG_CheckAnimFrameForEventType(animevent_t *animEvents, int keyFrame,
  ...
  for (int i = 0; i < MAX_ANIM_EVENTS; i++) {
    if (animEvents[i].keyFrame ==
      ...
      if (animEvents[i].eventType == eventType) { // and it is of the same type
        if (animEvents[i].modelOnly ==
            modelIndex) { // and it is for the same model
          return i;
        }
      }
    }
...
static void ParseAnimationEvtBlock(int glaIndex, unsigned short modelIndex,
                                   const char *aeb_filename,
                                   animevent_t *animEvents,
                                   animation_t *animations,
                                   unsigned char &lastAnimEvent,
  ...
  while (1) {
    ...
    if (animNum == -1) { // Unrecognized ANIM ENUM name,
    ...
    if (eventType == AEV_NONE ||
        eventType == (animEventType_t)-1) { // Unrecognized ANIM EVENT TYPE
    ...
    keyFrame = atoi(token);
    if (bIsFrameSkipped &&
        (animations[animNum].numFrames >
         2) // important, else frame 1 gets divided down and becomes frame 0.
            // Carcass & Assimilate also work this way
    ) {
      keyFrame /= 2; // if we ever use any other value in frame-skipping we'll
                     // have to figure out some way of reading it, since it's
                     // not stored anywhere
    }
    ...
    // overwrite it
    curAnimEvent = CG_CheckAnimFrameForEventType(animEvents, keyFrame,
                                                 eventType, modelIndex);
    ...
    case AEV_SOUND: //# animID AEV_SOUND framenum soundpath randomlow randomhi
      ...
      // NOTE: If we can be assured sequential handles, we can store the first
      // sound index and count 		unfortunately, if these sounds were previously
    ...
    case AEV_EFFECT: //# animID AEV_EFFECT framenum effectpath boltName
      ...
      if (token[0] &&
          Q_stricmp("special", token) == 0) { // special hard-coded effects
      ...
      // as below: animEvent->eventData[AED_BOLTINDEX] = gi.G2API_AddBolt(
...
======================
*/
static void G_ParseAnimationEvtFile(int glaIndex, const char *eventsDirectory,
                                    int fileIndex, int iRealGLAIndex = -1,
  ...
  const char *psAnimFileInternalName =
      (iRealGLAIndex == -1
           ? NULL
           : gi.G2API_GetAnimFileInternalNameIndex(iRealGLAIndex));
  bool bIsFrameSkipped =
      (psAnimFileInternalName && strlen(psAnimFileInternalName) > 5 &&
       !Q_stricmp(&psAnimFileInternalName[strlen(psAnimFileInternalName) - 5],
                  "_skip"));

  // Open The File, Make Sure It Is Safe
  //-------------------------------------
  Com_sprintf(eventsPath, MAX_QPATH, "models/players/%s/animevents.cfg",
              eventsDirectory);
  ...
  while (1) {
    ...
    // these stomp anything set in the include file (if it's an event of the
    // same type on the same frame)!
    if (!Q_stricmp(token, "UPPEREVENTS")) // A batch of upper events
    {
      ParseAnimationEvtBlock(glaIndex, modelIndex, eventsPath, torsoAnimEvents,
                             animations, afileset.torsoAnimEventCount, &text_p,
                             bIsFrameSkipped);
    }

    else if (!Q_stricmp(token, "LOWEREVENTS")) // A batch of lower events
    {
      ParseAnimationEvtBlock(glaIndex, modelIndex, eventsPath, legsAnimEvents,
                             animations, afileset.legsAnimEventCount, &text_p,
                             bIsFrameSkipped);
    }
  }
  COM_EndParseSession();
...
======================
*/
qboolean G_ParseAnimationFile(int glaIndex, const char *skeletonName,
                              int fileIndex) {
  char text[80000];
  int len = 0;
  const char *token = 0;
  ...
  // Read In The File To The Text Buffer, Make Sure Everything Is Safe To
  // Continue
  //-------------------------------------------------------------------------------
  Com_sprintf(skeletonPath, MAX_QPATH, "models/players/%s/%s.cfg", skeletonName,
              skeletonName);
  len = gi.RE_GetAnimationCFG(skeletonPath, text, sizeof(text));
  if (len <= 0) {
    Com_sprintf(skeletonPath, MAX_QPATH, "models/players/%s/animation.cfg",
                skeletonName);
    len = gi.RE_GetAnimationCFG(skeletonPath, text, sizeof(text));
    if (len <= 0) {
      return qfalse;
...
int G_ParseAnimFileSet(const char *skeletonName, const char *modelName = 0) {
  ...
  for (fileIndex = 0; fileIndex < level.numKnownAnimFileSets; fileIndex++) {
    if (Q_stricmp(level.knownAnimFileSets[fileIndex].filename, skeletonName) ==
        0) {
      break;
    }
  }

  // Ok, We Don't Already Have One, So Time To Create A New One And Initialize
  // It
  //------------------------------------------------------------------------------
  if (fileIndex >= level.numKnownAnimFileSets) {
    ...
    for (i = 0; i < MAX_ANIM_EVENTS; i++) {
      ...
      torsoAnimEvents[i].keyFrame =
          (unsigned short)-1; // 65535 should never be a valid frame... :)
      legsAnimEvents[i].keyFrame = (unsigned short)-1; // Frame to play event on
      torsoAnimEvents[i].stringData =
          NULL; // we allow storage of one string, temporarily (in case we have
                // to look up an index later,
      legsAnimEvents[i].stringData =
          NULL; // then make sure to set stringData to NULL so we only do the
                // look-up once)
      ...
      for (j = 0; j < AED_ARRAY_SIZE;
           j++) // Unique IDs, can be soundIndex of sound file to play OR effect
                // index or footstep type, etc.
      {
    ...
    if (Q_stricmp(skeletonName, "_humanoid") == 0) {
      ...
      const int normalGLAIndex = gi.G2API_PrecacheGhoul2Model(
          "models/players/_humanoid/_humanoid.gla"); // double check this always
                                                     // comes first!

      // Make Sure To Precache The GLAs (both regular and cinematic), And
      // Remember Their Indicies
      //------------------------------------------------------------------------------------------
      G_ParseAnimationFile(0, skeletonName, fileIndex);
      G_ParseAnimationEvtFile(0, skeletonName, fileIndex, normalGLAIndex,
      ...
      if (cineGLAIndex) {
        ...
        if (cineGLAIndex != normalGLAIndex + 1) {
          Com_Error(ERR_DROP, "Cinematic GLA was not loaded after the normal "
                              "GLA.  Cannot continue safely.");
        }
        G_ParseAnimationFile(1, skeletonMapName, fileIndex);
        G_ParseAnimationEvtFile(1, skeletonMapName, fileIndex, cineGLAIndex,
                                false /*flag for model specific*/);
      }
    } else {
      // non-humanoid...
  ...
  if (modelName) {
    ...
    while (curModel < MAX_MODELS_PER_LEVEL &&
           !modelsAlreadyDone[curModel].empty()) {
    ...
    if (curModel == MAX_MODELS_PER_LEVEL) {
      Com_Error(ERR_DROP, "About to overflow modelsAlreadyDone, increase "
                          "MAX_MODELS_PER_LEVEL\n");
    }
    ...
    if (Q_stricmp(skeletonName, modelName) != 0) {
      const int iGLAIndexToCheckForSkip =
          (Q_stricmp(skeletonName, "_humanoid")
               ? -1
               : gi.G2API_PrecacheGhoul2Model(
                     "models/players/_humanoid/_humanoid.gla")); // ;-)

      G_ParseAnimationEvtFile(0, modelName, fileIndex, iGLAIndexToCheckForSkip,
                              true);
    }
  }
...
void G_LoadAnimFileSet(gentity_t *ent, const char *pModelName) {
  ...
  if (Q_stricmp("player", pModelName) ==
      0) { // model is actually stored on console
  ...
  // now load and parse the animation.cfg, animevents.cfg and set the
  // animFileIndex
  if (!GLAName) {
    Com_Printf(S_COLOR_RED
               "Failed find animation file name models/players/%s\n",
               modelName);
  ...
  ent->client->clientInfo.animFileIndex =
      G_ParseAnimFileSet(strippedName, modelName);
...
void NPC_PrecacheAnimationCFG(const char *NPC_type) {
  ...
  if (!Q_stricmp("random",
                 NPC_type)) { // sorry, can't precache a random just yet
    return;
  }
  ...
  while (1) {
    ...
    if (!Q_stricmp(token, "legsmodel")) {
      ...
      // must copy data out of this pointer into a different part of memory
      // because the funcs we're about to call will call COM_ParseExt
    ...
    if (!Q_stricmp(token, "playerModel")) {
      ...
      int handle = gi.G2API_PrecacheGhoul2Model(
          va("models/players/%s/model.glm", value));
...
extern int NPC_WeaponsForTeam(team_t team, int spawnflags,
                              const char *NPC_type);
  ...
  for (int curWeap = WP_SABER; curWeap < WP_NUM_WEAPONS; curWeap++) {
    if ((weapons & (1 << curWeap))) {
      ...
      if (char *spot = strstr(weaponModel, ".md3")) {
        *spot = 0;
        spot = strstr(weaponModel,
                      "_w"); // i'm using the in view weapon array instead of
                             // scanning the item list, so put the _w back on
        if (!spot) {
...
void CG_NPC_Precache(gentity_t *spawner) {
  ...
  if (!Q_stricmp(
          "random",
          spawner->NPC_type)) { // sorry, can't precache a random just yet
    return;
  }
  ...
  while (1) {
    ...
    if (!Q_stricmp(token, "snd")) {
      ...
      if (!(spawner->svFlags & SVF_NO_BASIC_SOUNDS)) {
        // FIXME: store this in some sound field or parse in the soundTable like
        // the animTable...
    ...
    if (!Q_stricmp(token, "weapon")) {
      ...
      if (weap >= WP_NONE && weap < WP_NUM_WEAPONS) {
        if (weap > WP_NONE) {
          RegisterItem(
              FindItemForWeapon((weapon_t)(weap))); // precache the weapon
        }
      }
      continue;
    }
...
qboolean NPC_ParseParms(const char *NPCName, gentity_t *NPC) {
  ...
  if (NPC->NPC) {
    ...
    */
    ...
    stats->visrange = 1024;
    if (g_entities[ENTITYNUM_WORLD].max_health &&
    ...
    stats->runSpeed = 300;
    stats->acceleration =
        15; // Increase/descrease speed this much per frame (20fps)
  } else {
    stats = NULL;
  }
  ...
  // Set defaults
  // FIXME: should probably put default torso and head models, but what about
  // enemies that don't have any- like Stasis? Q_strncpyz( ri->headModelName,
  // DEFAULT_HEADMODEL,  sizeof(ri->headModelName),	qtrue); Q_strncpyz(
  // ri->torsoModelName, DEFAULT_TORSOMODEL, sizeof(ri->torsoModelName),	qtrue);
  // Q_strncpyz( ri->legsModelName,	DEFAULT_LEGSMODEL,
  // sizeof(ri->legsModelName),	qtrue);
  ...
  ri->customRGBA[0] = ri->customRGBA[1] = ri->customRGBA[2] =
      ri->customRGBA[3] = 0xFFu;

  if (!Q_stricmp("random", NPCName)) { // Randomly assemble an NPC
    NPC_BuildRandom(NPC);
  } else {
    p = NPCParms;
    COM_BeginParseSession();
    ...
    while (1) {
      ...
      if (!Q_stricmp(token, "headmodel")) {
        ...
        if (!Q_stricmp("none", value)) {
          ...
          ri->headYawRangeLeft = ri->headYawRangeRight = ri->headPitchRangeUp =
      ...
      if (!Q_stricmp(token, "torsomodel")) {
        ...
        if (!Q_stricmp("none", value)) {
          ...
          ri->torsoYawRangeLeft = ri->torsoYawRangeRight =
              ri->torsoPitchRangeUp = ri->torsoPitchRangeDown = 0;
        } else {
          Q_strncpyz(ri->torsoModelName, value, sizeof(ri->torsoModelName));
        }
        continue;
      ...
      if (!Q_stricmp(token, "scale")) {
        ...
        if (n != 100) {
          NPC->s.modelScale[0] = NPC->s.modelScale[1] = NPC->s.modelScale[2] =
              n / 100.0f;
        }
        continue;
      }
      ...
      if (!Q_stricmp(token, "class")) {
        ...
        if (NPC->client->NPC_class == CLASS_VEHICLE) {
          if (!NPC->m_pVehicle) { // you didn't spawn this guy right!
            Com_Printf(S_COLOR_RED
                       "ERROR: Tried to spawn a vehicle NPC (%s) without using "
                       "NPC_Vehicle or 'NPC spawn vehicle <vehiclename>'!!!  "
      ...
      if (!Q_stricmp(token, "height")) {
        ...
        if (NPC->client->NPC_class == CLASS_VEHICLE && NPC->m_pVehicle &&
            NPC->m_pVehicle->m_pVehicleInfo &&
            NPC->m_pVehicle->m_pVehicleInfo->type ==
                VH_FIGHTER) { // a flying vehicle's origin must be centered in
                              // bbox
      ...
      if (!parsingPlayer) {
        ...
        if (!Q_stricmp(token, "sex")) {
          ...
          if (value[0] == 'm') { // male
            stats->sex = SEX_MALE;
          } else if (value[0] == 'n') { // neutral
            stats->sex = SEX_NEUTRAL;
          } else if (value[0] == 'a') { // asexual?
            stats->sex = SEX_NEUTRAL;
          } else if (value[0] == 'f') { // female
            stats->sex = SEX_FEMALE;
          } else if (value[0] == 's') { // shemale?
            stats->sex = SEX_SHEMALE;
          } else if (value[0] == 'h') { // hermaphrodite?
            stats->sex = SEX_SHEMALE;
          } else if (value[0] == 't') { // transsexual/transvestite?
            stats->sex = SEX_SHEMALE;
          }
          continue;
        }
      ...
      if (!Q_stricmp(token, "snd")) {
        ...
        if (!(NPC->svFlags & SVF_NO_BASIC_SOUNDS)) {
          // FIXME: store this in some sound field or parse in the soundTable
          // like the animTable...
          Q_strncpyz(sound, value, sizeof(sound));
          patch = strstr(sound, "/");
          if (patch) {
      ...
      if (!Q_stricmp(token, "weapon")) {
        ...
        if (weap >= WP_NONE && weap < WP_NUM_WEAPONS) {
          ...
          if (weap > WP_NONE) {
            RegisterItem(
                FindItemForWeapon((weapon_t)(weap))); // precache the weapon
            NPC->client->ps.ammo[weaponData[weap].ammoIndex] =
                ammoData[weaponData[weap].ammoIndex].max;
          }
        }
        continue;
      }
      ...
      // have a sabers.cfg and just name your saber in your NPCs.cfg/ICARUS
      // script saber name
      if (!Q_stricmp(token, "saber")) {
        if (COM_ParseString(&p, &value)) {
          continue;
        ...
        if (NPC->client->ps.saber[0].stylesLearned) {
          NPC->client->ps.saberStylesKnown |=
              NPC->client->ps.saber[0].stylesLearned;
        }
        if (NPC->client->ps.saber[0].singleBladeStyle) {
          NPC->client->ps.saberStylesKnown |=
              NPC->client->ps.saber[0].singleBladeStyle;
        }
        continue;
      }
      ...
      if (!Q_stricmp(token, "saber2")) {
        ...
        if (!(NPC->client->ps.saber[0].saberFlags &
          ...
          if (NPC->client->ps.saber[1].stylesLearned) {
            NPC->client->ps.saberStylesKnown |=
                NPC->client->ps.saber[1].stylesLearned;
          }
          if (NPC->client->ps.saber[1].singleBladeStyle) {
            NPC->client->ps.saberStylesKnown |=
                NPC->client->ps.saber[1].singleBladeStyle;
          }
          if ((NPC->client->ps.saber[1].saberFlags &
               SFL_TWO_HANDED)) { // tsk tsk, can't use a twoHanded saber as
                                  // second saber
      ...
      if (!Q_stricmpn(token, "saberColor", 10)) {
        ...
        if (strlen(token) == 10) {
          ...
          NPC->client->ps.saber[0].blade[index].color =
              TranslateSaberColor(value);
        } else {
          gi.Printf(S_COLOR_YELLOW "WARNING: bad saberColor '%s' in %s\n",
                    token, NPCName);
        }
      ...
      if (!Q_stricmpn(token, "saberRadius", 11)) {
        ...
        if (n == -1) { // NOTE: this fills in the rest of the blades with the
                       // same length by default
          for (n = 0; n < MAX_BLADES; n++) {
...
#pragma endregion
  ...
  */
  if (!md3Model) {
    NPC->weaponModel[0] = -1;
    if (Q_stricmp("player", playerModel) ==
        0) { // set the model from the console cvars
      G_InitPlayerFromCvars(NPC);
      // now set the weapon, etc.
      G_MatchPlayerWeapon(NPC);
      // NPC->NPC->aiFlags |= NPCAI_MATCHPLAYERWEAPON;//FIXME: may not always
      // want this
    } else { // do a normal model load

      // If this is a vehicle, set the model name from the vehicle type array.
      if (NPC->client->NPC_class == CLASS_VEHICLE) {
        int iVehIndex = BG_VehicleGetIndex(NPC->NPC_type);
        strcpy(customSkin, "default"); // Ignore any custom skin that may have
                                       // come from the NPC File
        Q_strncpyz(playerModel, g_vehicleInfo[iVehIndex].model,
                   sizeof(playerModel));
        if (g_vehicleInfo[iVehIndex].skin && g_vehicleInfo[iVehIndex].skin[0]) {
          ...
          for (ratl::string_vs<256>::tokenizer i = skins.begin("|");
               i != skins.end(); i++) {
            if (NPC->soundSet && NPC->soundSet[0] &&
                Q_stricmp(*i, NPC->soundSet) == 0) {
              forceSkin = true;
            }
          ...
          //--------------------------------
            ...
            if (NPC->soundSet && NPC->soundSet[0]) {
              gi.Printf(S_COLOR_RED "WARNING: Unable to use skin (%s)",
                        NPC->soundSet);
            }
            Q_strncpyz(customSkin, *skinarray[Q_irand(0, skinarray.size() - 1)],
                       sizeof(customSkin));
          }
  ...
  Ghoul2 Insert End
  */
  if (NPCsPrecached) { // Spawning in after initial precache, our models are
                       // precached, we just need to set our clientInfo
...
void NPC_LoadParms(void) {
  ...
  fileCnt = gi.FS_GetFileList("ext_data/npcs", ".npc", npcExtensionListBuf,
  ...
  for (i = 0; i < fileCnt; i++, holdChar += npcExtFNLen + 1) {
    ...
    if (len == -1) {
      ...
      if (totallen &&
          *(marker - 1) == '}') { // don't let previous file end on a } because
                                  // that must be a stand-alone token
        strcat(marker, " ");
        totallen++;
        marker++;
      }
