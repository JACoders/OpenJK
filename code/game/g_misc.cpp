...
extern void SetMiscModelDefaults(gentity_t *ent, useFunc_t use_func, const char *material, int solid_mask, int animFlag, qboolean take_damage,
...
LIGHT - If this info_null is only targeted by a non-switchable light (a light without a targetname), it does NOT spawn in at all and doesn't count towards the #
...
void TeleportMover(gentity_t *mover, vec3_t origin, vec3_t diffAngles, qboolean snapAngle) { // FIXME: need an effect
...
void misc_portal_use(gentity_t *self, gentity_t *other, gentity_t *activator) { cycleCamera(self); }

void locateCamera(gentity_t *ent) { // FIXME: make this fadeout with distance from misc_camera_portal
	...
	if (!ent->targetname) {												 // not targetted, so auto-cycle
		if (G_Find(ent->owner, FOFS(targetname), ent->target) != NULL) { // targeted at more than one thing
...
void SP_misc_skyportal(gentity_t *ent) {
	...
	gi.SetConfigstring(CS_SKYBOXORG, va("%.2f %.2f %.2f %i %.2f %.2f %.2f %i %i", ent->s.origin[0], ent->s.origin[1], ent->s.origin[2], isfog, fogv[0], fogv[1],
										fogv[2], fogn, fogf));
...
void camera_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
	if (!activator || !activator->client || activator->s.number) { // really only usable by the player
		return;
	}
	...
	if (/*self->health <= 0 ||*/ (player && player->client &&
								  player->client->ps.viewEntity == self->s.number)) { // I'm already viewEntity, or I'm destroyed, find next
		gentity_t *next = NULL;
		if (self->target2 != NULL) {
			next = G_Find(NULL, FOFS(targetname), self->target2);
		}
		if (next) {											  // found another one
			if (!Q_stricmp("misc_camera", next->classname)) { // make sure it's another camera
...
void camera_aim(gentity_t *self) {
	...
	if (player && player->client && player->client->ps.viewEntity == self->s.number) {									  // I am the viewEntity
		if (player->client->usercmd.forwardmove || player->client->usercmd.rightmove || player->client->usercmd.upmove) { // player wants to back out of camera
			G_UseTargets2(self, player, self->target4);
			G_ClearViewEntity(player);
			G_Sound(player, self->soundPos2);
			self->painDebounceTime = level.time + (self->wait * 1000); // FRAMETIME*5;//don't check for player buttons for 500 ms
			if (player->client->usercmd.upmove > 0) {				   // stop player from doing anything for a half second after
				player->aimDebounceTime = level.time + 500;
			}
		} else if (self->painDebounceTime < level.time) {		  // check for use button
			if ((player->client->usercmd.buttons & BUTTON_USE)) { // player pressed use button, wants to cycle to next
...
void Use_Shooter(gentity_t *ent, gentity_t *other, gentity_t *activator) { G_ActivateBehavior(ent, BSET_USE); }
...
void health_think(gentity_t *ent) {
	...
	if (ent->count > 0) {
		...
		{
			dif = ent->enemy->client->ps.stats[STAT_MAX_HEALTH] - ent->enemy->client->ps.stats[STAT_ARMOR];
...
void health_use(gentity_t *self, gentity_t *other, gentity_t *activator) { // FIXME: Heal entire team?  Or only those that are undying...?
	int dif;
	int dif2;
	int hold;
...
NOTE: do not skip one of these model names, start with the lowest and fill in each next highest one with a value.  A gap will cause the item to not work
correctly.
...
void SP_misc_trip_mine(gentity_t *self) {
	...
	if ((self->spawnflags &
		 2)) // broadcast...should only be used in very rare cases.  could fix networking, perhaps, but james suggested this because it's easier
	{
...
void SP_misc_model_bomb_planted(gentity_t *ent) {
	...
	if (forceVisible) { // can see these through walls with force sight, so must be broadcast
		// ent->svFlags |= SVF_BROADCAST;
...
void ammo_power_converter_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
	...
	if (!activator || activator->s.number != 0) { // only the player gets to use these
		return;
	}
	...
	if (self->setTime < level.time) {
		...
		} else if (ps->ammo[AMMO_BLASTER] >= ammoData[AMMO_BLASTER].max && ps->ammo[AMMO_POWERCELL] >= ammoData[AMMO_POWERCELL].max &&
				   ps->ammo[AMMO_METAL_BOLTS] >= ammoData[AMMO_METAL_BOLTS].max) {
			// play full sound
...
void welder_think(gentity_t *self) {
	...
	if (newBolt != -1) {
		...
		gi.G2API_GetBoltMatrix(self->ghoul2, self->playerModel, newBolt, &boltMatrix, self->currentAngles, self->currentOrigin,
...
void jabba_cam_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
	if (self->spawnflags & 1) {
		self->spawnflags &= ~1;
		gi.G2API_SetBoneAnimIndex(&self->ghoul2[self->playerModel], self->rootBone, 15, 0, BONE_ANIM_OVERRIDE_FREEZE, -1.5f, (cg.time ? cg.time : level.time),
								  -1, 0);

	} else {
		self->spawnflags |= 1;
		gi.G2API_SetBoneAnimIndex(&self->ghoul2[self->playerModel], self->rootBone, 0, 15, BONE_ANIM_OVERRIDE_FREEZE, 1.5f, (cg.time ? cg.time : level.time),
								  -1, 0);
	}
}
...
//-----------------------------------------------------
{
	...
	ent->playerModel =
		gi.G2API_InitGhoul2Model(ent->ghoul2, "models/map_objects/nar_shaddar/jabacam/jabacam.glm", ent->s.modelindex, NULL_HANDLE, NULL_HANDLE, 0, 0);
	ent->s.radius = 150.0f; //......
...
//------------------------------------------------------------------------
void misc_use(gentity_t *self, gentity_t *other, gentity_t *activator) { misc_model_breakable_die(self, other, activator, 100, MOD_UNKNOWN); }
...
void SP_misc_crystal_crate(gentity_t *ent) {
	...
	// Blocks movement
	ent->contents =
		CONTENTS_SOLID | CONTENTS_OPAQUE | CONTENTS_BODY | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP; // Was CONTENTS_SOLID, but only architecture should be this
...
void misc_atst_setanim(gentity_t *self, int bone, int anim) {
	...
	if (firstFrame != -1 && lastFrame != -1 && animSpeed != 0) {
		if (!gi.G2API_SetBoneAnimIndex(&self->ghoul2[self->playerModel], bone, firstFrame, lastFrame, BONE_ANIM_OVERRIDE_FREEZE | BONE_ANIM_BLEND, animSpeed,
									   (cg.time ? cg.time : level.time), -1, 150)) {
			gi.G2API_SetBoneAnimIndex(&self->ghoul2[self->playerModel], bone, firstFrame, lastFrame, BONE_ANIM_OVERRIDE_FREEZE, animSpeed,
									  (cg.time ? cg.time : level.time), -1, 150);
		}
	}
}
...
void misc_atst_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
	...
	if (activator->client->NPC_class != CLASS_ATST) {				   // get in the ATST
		if (activator->client->ps.groundEntityNum != self->s.number) { // can only get in if on top of me...
		...
		if (legsAnim != BOTH_STAND1 && !PM_InSlopeAnim(legsAnim) && legsAnim != BOTH_TURN_RIGHT1 &&
			legsAnim != BOTH_TURN_LEFT1) { // can't get out of it while it's still moving
			return;
		}
...
void SP_misc_atst_drivable(gentity_t *ent) {
	...
	ent->rootBone = gi.G2API_GetBoneIndex(&ent->ghoul2[ent->playerModel], "model_root", qtrue);
	ent->craniumBone = gi.G2API_GetBoneIndex(&ent->ghoul2[ent->playerModel], "cranium",
											 qtrue); // FIXME: need to somehow set the anim/frame to the equivalent of BOTH_STAND1...  use to be that
													 // BOTH_STAND1 was the first frame of the glm, but not anymore
	ent->s.radius = 320;
	VectorSet(ent->s.modelScale, 1.0f, 1.0f, 1.0f);
...
void SP_misc_weather_zone(gentity_t *ent) {
	...
	char temp[256];
	sprintf(temp, "zone ( %f %f %f ) ( %f %f %f )", ent->mins[0], ent->mins[1], ent->mins[2], ent->maxs[0], ent->maxs[1], ent->maxs[2]);
