...
void FX_BryarProjectileThink(centity_t *cent,
  ...
  if (cent->gent && cent->gent->owner && cent->gent->owner->s.number > 0) {
    ...
    theFxScheduler.PlayEffect(cgs.effects.bryarShotEffect, cent->lerpOrigin,
                              forward);
  }
}
...
void FX_BryarAltProjectileThink(centity_t *cent,
  ...
  for (int t = 1; t < cent->gent->count; t++) {
    ...
    theFxScheduler.PlayEffect(cgs.effects.bryarPowerupShotEffect,
...
void FX_BryarAltHitWall(vec3_t origin, vec3_t normal, int power) {
  ...
  case 4:
  case 5:
    theFxScheduler.PlayEffect(cgs.effects.bryarWallImpactEffect3, origin,
                              normal);
    break;

  case 2:
  case 3:
    theFxScheduler.PlayEffect(cgs.effects.bryarWallImpactEffect2, origin,
                              normal);
    break;

  default:
    theFxScheduler.PlayEffect(cgs.effects.bryarWallImpactEffect, origin,
                              normal);
    break;
  }
}
