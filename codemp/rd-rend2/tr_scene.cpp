...
void R_AddPolygonSurfaces(const trRefdef_t *refdef) {
	...
	for (i = 0, poly = refdef->polys; i < refdef->numPolys; i++, poly++) {
		...
		R_AddDrawSurf((surfaceType_t *)poly, REFENTITYNUM_WORLD, sh, poly->fogIndex & fogMask, qfalse, qfalse, 0 /* cubemapIndex */);
	}
}
...
void RE_BeginScene(const refdef_t *fd) {
	...
	if ((fd->rdflags & RDF_NOWORLDMODEL) || !(r_depthPrepass->value)) {
		...
		if (r_sunlightMode->integer == 1) {
			tr.refdef.sunCol[0] = tr.refdef.sunCol[1] = tr.refdef.sunCol[2] = 1.0f;
	...
	// ri.Printf(PRINT_ALL, "RE_BeginScene Frame: %i, skyportal: %i, entities: %i\n", backEndData->realFrameNumber, int(tr.world->skyboxportal &&
	// (tr.refdef.rdflags & RDF_SKYBOXPORTAL)), tr.refdef.num_entities);
