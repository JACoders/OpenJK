...
class CTraceSurface {
  public:
	...
	CTraceSurface(int initsurfaceNum, surfaceInfo_v &initrootSList, const model_t *initcurrentModel, int initlod, vec3_t initrayStart, vec3_t initrayEnd,
				  CCollisionRecord *initcollRecMap, int initentNum, int initmodelIndex, const skin_t *initskin, const shader_t *initcust_shader,
				  intptr_t *initTransformedVertsArray, const EG2_Collision einitG2TraceType,
#ifdef _G2_GORE
...
void G2_TransformSurfaces(int surfaceNum, surfaceInfo_v &rootSList, CBoneCache *boneCache, const model_t *currentModel, int lod, vec3_t scale,
						  CMiniHeap *G2VertSpace, intptr_t *TransformedVertArray, bool secondTimeAround) {
	int i;
	assert(currentModel);
...
static inline qboolean G2_SegmentTriangleTest(const vec3_t start, const vec3_t end, const vec3_t A, const vec3_t B, const vec3_t C, qboolean backFaces,
											  qboolean frontFaces, vec3_t returnedPoint, vec3_t returnedNormal, float *denom) {
...
static void G2_GorePolys(const mdxmSurface_t *surface, CTraceSurface &TS, const mdxmSurfHierarchy_t *surfInfo) {
	...
	if (!TS.gore->useTheta) {
		...
		if (DotProduct(basis1, basis1) < 0.005f) { // shot dir and slash dir are too close
			return;
		}
	}
...
#ifdef _G2_GORE
void G2_TraceModels(CGhoul2Info_v &ghoul2, vec3_t rayStart, vec3_t rayEnd, CCollisionRecord *collRecMap, int entNum, EG2_Collision eG2TraceType, int useLod,
...
#ifdef _G2_GORE
		CTraceSurface TS(g.mSurfaceRoot, g.mSlist, g.currentModel, lod, rayStart, rayEnd, collRecMap, entNum, i, skin, cust_shader, g.mTransformedVertsArray,
						 eG2TraceType, fRadius, ssize, tsize, theta, shader, &g, gore);
#else
		CTraceSurface TS(g.mSurfaceRoot, g.mSlist, g.currentModel, lod, rayStart, rayEnd, collRecMap, entNum, i, skin, cust_shader, g.mTransformedVertsArray,
						 eG2TraceType, fRadius);
#endif
...
void G2_SaveGhoul2Models(CGhoul2Info_v &ghoul2) {
	...
	if (!ghoul2.IsValid() || ghoul2.size() == 0) {
		...
		saved_game.write_chunk_and_size<int32_t>(INT_ID('G', 'L', '2', 'S'), INT_ID('G', 'H', 'L', '2'));
#else
