...
qboolean NPC_GetMoveDirectionAltRoute(vec3_t out, float *distance, qboolean tryStraight) {
	...
	*distance = frameNavInfo.distance;
	...
	// Attempt a straight move to goal
	if (!tryStraight || NPC_ClearPathToGoal(frameNavInfo.direction, NPCS.NPCInfo->goalEntity) == qfalse) { // blocked
		...
		if (d_altRoutes.integer) { // try macro nav
			...
			if (NAVNEW_AvoidCollision(NPCS.NPC, NPCS.NPCInfo->goalEntity, &tempInfo, qtrue, 5) == qfalse) { // revert to macro nav
			...
			if (NAVNEW_AvoidCollision(NPCS.NPC, NPCS.NPCInfo->goalEntity, &frameNavInfo, qtrue, 30) == qfalse) { // give up
				return qfalse;
			}
		}
	}
