...
public:
  ...
  CTraceSurface(int initsurfaceNum, surfaceInfo_v &initrootSList,
                ...
                vec3_t initrayStart, vec3_t initrayEnd,
                CCollisionRecord *initcollRecMap, int initentNum,
                int initmodelIndex, const skin_t *initskin,
      ...
      :
        ...
        eG2TraceType(einitG2TraceType), hitOne(false),
#ifdef _G2_GORE
...
void R_TransformEachSurface(const mdxmSurface_t *surface, vec3_t scale,
  ...
  if (!TransformedVerts) {
    ...
    Com_Error(ERR_DROP, "Ran out of transform space for Ghoul2 Models. Adjust "
                        "G2_MINIHEAP_SIZE in sv_init.cpp.\n");
  }
...
void G2_TransformSurfaces(int surfaceNum, surfaceInfo_v &rootSList,
                          ...
                          int lod, vec3_t scale, CMiniHeap *G2VertSpace,
                          intptr_t *TransformedVertArray,
  ...
  const mdxmSurface_t *surface =
      (mdxmSurface_t *)G2_FindSurface(currentModel, surfaceNum, lod);
...
void G2_TransformModel(CGhoul2Info_v &ghoul2, const int frameNum, vec3_t scale,
                       CMiniHeap *G2VertSpace, int useLod, bool ApplyGore,
                       SSkinGoreData *gore)
#else
void G2_TransformModel(CGhoul2Info_v &ghoul2, const int frameNum, vec3_t scale,
                       CMiniHeap *G2VertSpace, int useLod)
#endif
{
  int i, lod;
  vec3_t correctScale;
  ...
  for (i = 0; i < ghoul2.size(); i++) {
    ...
    g.mTransformedVertsArray = (intptr_t *)G2VertSpace->MiniHeapAlloc(
        g.currentModel->mdxm->numSurfaces * sizeof(intptr_t));
    if (!g.mTransformedVertsArray) {
      Com_Error(ERR_DROP, "Ran out of transform space for Ghoul2 Models. "
                          "Adjust G2_MINIHEAP_SIZE in sv_init.cpp.\n");
    }
...
G2_SegmentTriangleTest(const vec3_t start, const vec3_t end, const vec3_t A,
                       const vec3_t B, const vec3_t C, qboolean backFaces,
                       qboolean frontFaces, vec3_t returnedPoint,
...
static void G2_GorePolys(const mdxmSurface_t *surface, CTraceSurface &TS,
  ...
  if (!TS.gore->useTheta) {
    ...
    if (DotProduct(basis1, basis1) <
        0.005f) { // shot dir and slash dir are too close
      return;
    }
  }
  ...
  for (j = 0; j < numTris; j++) {
    ...
    if (G2_SegmentTriangleTest(
      ...
      if (i == MAX_G2_COLLISIONS) {
        assert(i != MAX_G2_COLLISIONS); // run out of collision record space -
                                        // will probalbly never happen
      ...
      for (; i < MAX_G2_COLLISIONS; i++) {
        if (TS.collRecMap[i].mEntityNum == -1) {
          ...
          if (fabsf(side2) < 1E-8f) {
            // i don't know the hitPoint, but let's just assume it's the first
            // vert for now...
            VectorSubtract(A, TS.rayStart, distVect);
            dist = VectorLength(distVect);
            VectorSubtract(TS.rayEnd, TS.rayStart, distVect);
            VectorMA(TS.rayStart, dist / VectorLength(distVect), distVect,
                     hitPoint);
          } else {
            dist = side / side2;
...
void G2_TraceModels(CGhoul2Info_v &ghoul2, vec3_t rayStart, vec3_t rayEnd,
                    CCollisionRecord *collRecMap, int entNum,
                    EG2_Collision eG2TraceType, int useLod, float fRadius,
                    float ssize, float tsize, float theta, int shader,
...
#ifdef _G2_GORE
    CTraceSurface TS(g.mSurfaceRoot, g.mSlist, g.currentModel, lod, rayStart,
                     rayEnd, collRecMap, entNum, i, skin, cust_shader,
                     g.mTransformedVertsArray, eG2TraceType, fRadius, ssize,
                     tsize, theta, shader, &g, gore);
#else
    CTraceSurface TS(g.mSurfaceRoot, g.mSlist, g.currentModel, lod, rayStart,
                     rayEnd, collRecMap, entNum, i, skin, cust_shader,
                     g.mTransformedVertsArray, eG2TraceType, fRadius);
#endif
...
#ifdef JK2_MODE
    saved_game.write<int32_t>(zero_size);

    saved_game.write_chunk_and_size<int32_t>(INT_ID('G', 'L', '2', 'S'),
                                             INT_ID('G', 'H', 'L', '2'));
#else
    saved_game.write_chunk<int32_t>(INT_ID('G', 'H', 'L', '2'),
