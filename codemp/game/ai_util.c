...
void *B_Alloc(int size) {
  ...
  if (i == MAX_BALLOC) {
    // If this happens we'll have to rely on this chunk being freed manually
    // with B_Free, which it hopefully will be
#ifdef DEBUG
...
void B_Free(void *ptr) {
  ...
  if (i == MAX_BALLOC) {
    // Likely because the limit was exceeded and we're now freeing the chunk
    // manually as we hoped would happen
#ifdef DEBUG
    trap->Print("WARNING: Freeing allocation which is not in the allocation "
                "structure\n");
#endif
  }
...
int GetValueGroup(char *buf, char *group, char *outbuf) {
  ...
  // we have found the proper group name if we made it here, so find the opening
  // brace and read into the outbuf until hitting the end brace
...
int GetPairedValue(char *buf, char *key, char *outbuf) {
  ...
  while (!found) {
    if (startletter == 0 || !buf[startletter] || buf[startletter] == '\0' ||
        buf[startletter] == 9 || buf[startletter] == ' ' ||
        buf[startletter] == '\n') {
      if (buf[startpoint] == '\0' || buf[startpoint] == 9 ||
          buf[startpoint] == ' ' || buf[startpoint] == '\n') {
        found = 1;
        break;
      }
    }
  ...
  while (buf[startpoint] == ' ' || buf[startpoint] == 9 ||
         buf[startpoint] == '\n') {
    startpoint++;
  }
  ...
  while (buf[startpoint] && buf[startpoint] != '\0' &&
         buf[startpoint] != '\n') {
    outbuf[i] = buf[startpoint];
    i++;
    startpoint++;
...
int BotDoChat(bot_state_t *bs, char *section, int always) {
  ...
  if (trap->Cvar_VariableIntegerValue(
          "se_language")) { // no chatting unless English.
    return 0;
  }
...
void ParseEmotionalAttachments(bot_state_t *bs, char *buf) {
  ...
  while (buf[i] && buf[i] != '}') {
    while (buf[i] == ' ' || buf[i] == '{' || buf[i] == 9 || buf[i] == 13 ||
           buf[i] == '\n') {
      i++;
    }
...
int ReadChatGroups(bot_state_t *bs, char *buf) {
  ...
  if (strlen(cgroupbegin) >= MAX_CHAT_BUFFER_SIZE) {
    trap->Print(S_COLOR_RED
                "Error: Personality chat section exceeds max size\n");
    return 0;
  }
...
void BotUtilizePersonality(bot_state_t *bs) {
  ...
  while (len <
         131072) { // kill all characters after the file length, since sometimes
                   // FS_Read doesn't do that entirely (or so it seems)
    buf[len] = '\0';
    len++;
  }
  ...
  if (!GetValueGroup(buf, "GeneralBotInfo", group)) {
    trap->Print(S_COLOR_RED
                "Personality file contains no GeneralBotInfo group\n");
  ...
  if (!failed && GetPairedValue(group, "forceinfo", readbuf)) {
    ...
    Com_sprintf(bs->forceinfo, sizeof(bs->forceinfo), "%s\0",
                DEFAULT_FORCEPOWERS);
  }
