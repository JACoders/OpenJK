...
static const char *gammaCorrectVtxShader = "!!ARBvp1.0\
MOV result.position, vertex.position;\
MOV result.texcoord[0], vertex.texcoord[0];\
END";

static const char *gammaCorrectPxShader = "!!ARBfp1.0\
TEMP r0;\
...
void ARB_InitGPUShaders(void) {
  ...
  qglProgramStringARB(GL_VERTEX_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB,
                      strlen(gammaCorrectVtxShader), gammaCorrectVtxShader);

  int errorChar;
  qglGetIntegerv(GL_PROGRAM_ERROR_POSITION_ARB, &errorChar);
  if (errorChar != -1) {
    Com_Printf(S_COLOR_RED "ERROR: Failed to compile gamma correction vertex "
                           "shader. Error at character %d\n",
               errorChar);
    ...
    qglProgramStringARB(GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB,
                        strlen(gammaCorrectPxShader), gammaCorrectPxShader);

    qglGetIntegerv(GL_PROGRAM_ERROR_POSITION_ARB, &errorChar);
    if (errorChar != -1) {
      Com_Printf(S_COLOR_RED "Failed to compile gamma correction pixel shader. "
