...
#define WAVEVALUE(table, base, amplitude, phase, freq)                         \
  ((base) + table[Q_ftol((((phase) + backEnd.refdef.floatTime * (freq)) *      \
                          FUNCTABLE_SIZE)) &                                   \
                  FUNCTABLE_MASK] *                                            \
                (amplitude))
...
static float EvalWaveForm(const waveForm_t *wf) {
  ...
  if (wf->func == GF_NOISE) {
    return (wf->base + R_NoiseGet4f(0, 0, 0,
                                    (backEnd.refdef.floatTime + wf->phase) *
                                        wf->frequency) *
                           wf->amplitude);
...
void RB_CalcBulgeVertexes(deformStage_t *ds) {
  ...
  if (ds->bulgeSpeed == 0.0f && ds->bulgeWidth == 0.0f) {
    ...
    // I guess do some extra dumb stuff..the fact that it uses ST seems bad
    // though because skin pages may be set up in certain ways that can cause
    ...
    for (i = 0; i < tess.numVertexes;
         i++, xyz += 4, st += 2 * NUM_TEX_COORDS, normal += 4) {
      off =
          (float)(FUNCTABLE_SIZE / (M_PI * 2)) * (st[0] * ds->bulgeWidth + now);
...
void DeformText(const char *text) {
  ...
  VectorClear(mid);
  bottom = WORLD_SIZE; // 999999;	// WORLD_SIZE instead of MAX_WORLD_COORD
                       // so guaranteed to be...
...
static void AutospriteDeform(void) {
  ...
  if (tess.numVertexes & 3) {
    Com_Error(ERR_DROP, "Autosprite shader %s had odd vertex count",
              tess.shader->name);
  }
  if (tess.numIndexes != (tess.numVertexes >> 2) * 6) {
    Com_Error(ERR_DROP, "Autosprite shader %s had odd index count",
              tess.shader->name);
  }
...
=====================
*/
static const glIndex_t edgeVerts[6][2] = {{0, 1}, {0, 2}, {0, 3},
                                          {1, 2}, {1, 3}, {2, 3}};
...
static void Autosprite2Deform(void) {
  ...
  for (i = 0, indexes = 0; i < tess.numVertexes; i += 4, indexes += 6) {
    ...
    nums[0] = nums[1] = 0;
    lengths[0] = lengths[1] =
        WORLD_SIZE; // 999999;	// ... instead of MAX_WORLD_COORD, so guaranteed
                    // to be outside legal range
...
void RB_CalcFogTexCoords(float *st) {
  ...
  if (fog->hasSurface) {
    fogDepthVector[0] = fog->surface[0] * backEnd.ori.axis[0][0] +
                        fog->surface[1] * backEnd.ori.axis[0][1] +
                        fog->surface[2] * backEnd.ori.axis[0][2];
    fogDepthVector[1] = fog->surface[0] * backEnd.ori.axis[1][0] +
                        fog->surface[1] * backEnd.ori.axis[1][1] +
                        fog->surface[2] * backEnd.ori.axis[1][2];
    fogDepthVector[2] = fog->surface[0] * backEnd.ori.axis[2][0] +
                        fog->surface[1] * backEnd.ori.axis[2][1] +
                        fog->surface[2] * backEnd.ori.axis[2][2];
    fogDepthVector[3] =
        -fog->surface[3] + DotProduct(backEnd.ori.origin, fog->surface);

    eyeT =
        DotProduct(backEnd.ori.viewOrigin, fogDepthVector) + fogDepthVector[3];
  } else {
...
void RB_CalcEnvironmentTexCoords(float *st) {
  ...
  if (backEnd.currentEntity &&
      backEnd.currentEntity->e.renderfx &
          RF_FIRST_PERSON) // this is a view model so we must use world lights
                           // instead of vieworg
  {
  ...
  for (i = 0; i < numVertexes; i++, v += 4, normal += 4, alphas += 4) {
    ...
    if (backEnd.currentEntity &&
        (backEnd.currentEntity->e.hModel ||
         backEnd.currentEntity->e.ghoul2)) // this is a model so we can use
                                           // world lights instead fake light
    {
...
void RB_CalcDiffuseEntityColor(unsigned char *colors) {
  ...
  if (!backEnd.currentEntity) { // error, use the normal lighting
...
void RB_CalcDisintegrateColors(unsigned char *colors, colorGen_t rgbGen) {
  ...
  // calculate the burn threshold at the given time, anything that passes the
  // threshold will get burnt
  threshold = (backEnd.refdef.time - ent->endTime) *
              0.045f; // endTime is really the start time, maybe I should just
                      // use a completely meaningless substitute?
    ...
    for (i = 0; i < numVertexes; i++, v += 4) {
      ...
      if (dis < threshold * threshold) {
        ...
        if (rgbGen == CGEN_LIGHTING_DIFFUSE_ENTITY) {
          colors[i * 4 + 0] =
              backEnd.currentEntity->e.shaderRGBA[0] * 0x6f / 255.0f;
          colors[i * 4 + 1] =
              backEnd.currentEntity->e.shaderRGBA[1] * 0x6f / 255.0f;
          colors[i * 4 + 2] =
              backEnd.currentEntity->e.shaderRGBA[2] * 0x6f / 255.0f;
        } else {
          colors[i * 4 + 0] = 0x6f;
          colors[i * 4 + 1] = 0x6f;
          colors[i * 4 + 2] = 0x6f;
        ...
        if (rgbGen == CGEN_LIGHTING_DIFFUSE_ENTITY) {
          colors[i * 4 + 0] =
              backEnd.currentEntity->e.shaderRGBA[0] * 0xaf / 255.0f;
          colors[i * 4 + 1] =
              backEnd.currentEntity->e.shaderRGBA[1] * 0xaf / 255.0f;
          colors[i * 4 + 2] =
              backEnd.currentEntity->e.shaderRGBA[2] * 0xaf / 255.0f;
        } else {
          colors[i * 4 + 0] = 0xaf;
          colors[i * 4 + 1] = 0xaf;
          colors[i * 4 + 2] = 0xaf;
...
void RB_CalcDisintegrateVertDeform(void) {
  ...
  if (backEnd.currentEntity->e.renderfx & RF_DISINTEGRATE2) {
    float threshold =
        (backEnd.refdef.time - backEnd.currentEntity->e.endTime) * 0.045f;
