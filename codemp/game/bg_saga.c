...
#define SIEGECHAR_TAB                                                          \
  9 // perhaps a bit hacky, but I don't think there's any define existing for
    // "tab"
...
stringID_table_t StanceTable[] = {
    ENUM2STRING(SS_NONE),   ENUM2STRING(SS_FAST),   ENUM2STRING(SS_MEDIUM),
    ENUM2STRING(SS_STRONG), ENUM2STRING(SS_DESANN), ENUM2STRING(SS_TAVION),
    ENUM2STRING(SS_DUAL),   ENUM2STRING(SS_STAFF),  {"", 0}};

// Weapon and force power tables are also used in NPC parsing code and some
// other places.
...
void BG_SiegeStripTabs(char *buf) {
  ...
  while (buf[i]) {
    if (buf[i] != SIEGECHAR_TAB) { // not a tab, just stick it in
      buf[i_r] = buf[i];
...
int BG_SiegeGetValueGroup(char *buf, char *group, char *outbuf) {
  ...
  while (buf[i]) {
    if (buf[i] != ' ' && buf[i] != '{' && buf[i] != '}' && buf[i] != '\n' &&
        buf[i] != '\r' &&
        buf[i] != SIEGECHAR_TAB) { // we're on a valid character
      if (buf[i] == '/' &&
      ...
      } else { // parse to the next space/endline/eos and check this value
               // against our group value.
        j = 0;

        while (buf[i] != ' ' && buf[i] != '\n' && buf[i] != '\r' &&
               buf[i] != SIEGECHAR_TAB && buf[i] != '{' && buf[i]) {
        ...
        if (!buf[i]) {
          Com_Error(ERR_DROP, "Unexpected EOF while looking for group '%s'",
                    group);
        }
        ...
        while (buf[i] &&
               (buf[i] == ' ' || buf[i] == SIEGECHAR_TAB || buf[i] == '\n' ||
                buf[i] == '\r')) { // parse to the next valid character
          i++;
        }

        if (buf[i] == '{') { // if the next valid character is an opening
                             // bracket, then this is indeed a group
          isGroup = qtrue;
        }

        // Is this the one we want?
        if (isGroup &&
            !Q_stricmp(checkGroup,
                       group)) { // guess so. Parse until we hit the {
                                 // indicating the beginning of the group.
          while (buf[i] != '{' && buf[i]) {
            i++;
          }

          if (buf[i]) { // We're at the start of the group now, so parse to the
                        // closing bracket.
            j = 0;
            ...
            while ((buf[i] != '}' || parseGroups) && buf[i]) {
              ...
              if (parseGroups < 0) { // Syntax error, I guess.
                Com_Error(ERR_DROP,
                          ...
                          "while looking for group '%s'",
                          group);
              }

              if ((buf[i] != '{' || parseGroups > 1) &&
                  (buf[i] != '}' ||
                   parseGroups > 0)) { // don't put the start and end brackets
                                       // for this group into the output buffer
                outbuf[j] = buf[i];
                j++;
              }

              if (buf[i] == '}' &&
                  !parseGroups) { // Alright, we can break out now.
                break;
              }
            ...
            if (buf[i] != '}') {
              Com_Error(ERR_DROP, "Group '%s' is missing a closing bracket",
                        group);
            }
            ...
            Com_Error(ERR_DROP,
                      "Error parsing group in file, unexpected EOF before "
                      "opening bracket while looking for group '%s'",
                      group);
          }
        } else if (!isGroup) { // if it wasn't a group, parse to the end of the
                               // line
          while (buf[i] && buf[i] != '\n' && buf[i] != '\r') {
            i++;
          }
        } else { // this was a group but we not the one we wanted to find, so
                 // parse by it.
          parseGroups = 0;
          ...
          if (buf[i] != '}') {
            Com_Error(ERR_DROP,
                      "Found an opening bracket without a matching closing "
                      "bracket while looking for group '%s'",
                      group);
          }
    ...
    } else if (buf[i] == '{') { // we're in a group that isn't the one we want,
                                // so parse to the end.
      parseGroups = 0;
      ...
      while (buf[i] && (buf[i] != '}' || parseGroups)) {
        ...
        if (parseGroups < 0) { // Syntax error, I guess.
          Com_Error(ERR_DROP,
                    "Found a closing bracket without an opening bracket while "
                    "looking for group '%s'",
                    group);
        }
      ...
      if (buf[i] != '}') {
        Com_Error(ERR_DROP,
                  "Found an opening bracket without a matching closing bracket "
                  "while looking for group '%s'",
                  group);
      }
    }
      ...
      if (buf[i] == '/' &&
        ...
        while (buf[i] && buf[i] != '\n' && buf[i] != '\r') {
          i++;
               ...
               // against our key value.
        j = 0;

        while (buf[i] != ' ' && buf[i] != '\n' && buf[i] != '\r' &&
               buf[i] != SIEGECHAR_TAB && buf[i]) {
          if (buf[i] == '/' && buf[i + 1] == '/') { // hit a comment, break out.
            break;
          }
        ...
        if (buf[k] == '{') { // this is not the start of a value but rather of a
                             // group. We don't want to look in subgroups so skip
          ...
          while (buf[i] && (buf[i] != '}' || openB)) {
            ...
            if (openB < 0) {
              Com_Error(ERR_DROP,
                        "Unexpected closing bracket (too many) while parsing "
                        "to end of group '%s'",
                        checkKey);
            }
          ...
          if (buf[i] != '/' ||
              buf[i + 1] !=
                  '/') { // make sure we didn't stop on a comment, if we did
                         // then this is considered an error in the file.
            if (!Q_stricmp(checkKey,
                           key)) { // guess so. Parse along to the next valid
                                   // character, then put that into the output
                                   // buffer and return 1.
              while ((buf[i] == ' ' || buf[i] == '\n' || buf[i] == '\r' ||
                      buf[i] == SIEGECHAR_TAB) &&
                     buf[i]) {
              ...
              if (buf[i]) { // We're at the start of the value now.
                ...
                if (buf[i] == '\"') { // if the value is in quotes, then stop at
                                      // the next quote instead of ' '
                  i++;
                  parseToQuote = qtrue;
                }
                ...
                while (((!parseToQuote && buf[i] != ' ' && buf[i] != '\n' &&
                  ...
                  if (buf[i] == '/' &&
                      buf[i + 1] ==
                          '/') { // hit a comment after the value? This isn't an
                                 // ideal way to be writing things, but we'll
                                 // support it anyway.
                    break;
                  }
                  outbuf[j] = buf[i];
                  ...
                  if (!buf[i]) {
                    if (parseToQuote) {
                      Com_Error(ERR_DROP,
                                "Unexpected EOF while looking for endquote, "
                                "error finding paired value for '%s'",
                                key);
                    } else {
                      Com_Error(ERR_DROP,
                                "Unexpected EOF while looking for space or "
                                "endline, error finding paired value for '%s'",
                                key);
                    }
                  }
                }
                ...
                Com_Error(ERR_DROP,
                          "Error parsing file, unexpected EOF while looking "
                          "for valud '%s'",
                          key);
              }
            } else { // if that wasn't the desired key, then make sure we parse
                     // to the end of the line, so we don't mistake a value for a
                     // key
              while (buf[i] && buf[i] != '\n') {
                i++;
              }
            ...
            Com_Error(
                ERR_DROP,
                "Error parsing file, found comment, expected value for '%s'",
                key);
          }
        }
      }
...
void BG_SiegeTranslateForcePowers(char *buf, siegeClass_t *siegeClass) {
  ...
  if (!Q_stricmp(buf, "FP_ALL")) { // this is a special case, just give us all
                                   // the powers on level 3
    allPowers = qtrue;
  }
  ...
  i = 0;
  while (buf[i]) { // parse through the list which is seperated by |, and add
                   // all the weapons into a bitflag
    if (buf[i] != ' ' && buf[i] != '|') {
      j = 0;
      ...
      if (checkPower[0]) { // Got the name, compare it against the weapon table
                           // strings.
        k = 0;
        ...
        while (FPTable[k].id != -1 && FPTable[k].name[0]) {
          if (!Q_stricmp(checkPower,
                         FPTable[k].name)) { // found it, add the weapon into
                                             // the weapons value
...
// Used for the majority of generic val parsing stuff. buf should be the value
// string, table should be the appropriate string/id table. If bitflag is qtrue
// then the values are accumulated into a bitflag. If bitflag is qfalse then the
// first value is returned as a directly corresponding id and no further parsing
// is done.
int BG_SiegeTranslateGenericTable(char *buf, stringID_table_t *table,
                                  qboolean bitflag) {
  int items = 0;
  char checkItem[1024];
  int i = 0;
  ...
  while (buf[i]) { // Using basically the same parsing method as we do for
                   // weapons and forcepowers.
    if (buf[i] != ' ' && buf[i] != '|') {
      j = 0;
      ...
      if (checkItem[0]) {
        ...
        while (table[k].name &&
               table[k].name[0]) { // go through the list and check the parsed
                                   // flag name against the hardcoded names
          if (!Q_stricmp(checkItem,
                         table[k].name)) { // Got it, so add the value into our
                                           // items value.
            if (bitflag) {
              items |= (1 << table[k].id);
...
void BG_SiegeParseClassFile(const char *filename,
                            siegeClassDesc_t *descBuffer) {
  fileHandle_t f;
  int len;
  int i;
  char classInfo[4096];
  ...
  if (descBuffer) {
    if (!BG_SiegeGetPairedValue(classInfo, "description", descBuffer->desc)) {
      Q_strncpyz(descBuffer->desc, "DESCRIPTION UNAVAILABLE",
                 sizeof(descBuffer->desc));
    }
  ...
  if (BG_SiegeGetPairedValue(classInfo, "name", parseBuf)) {
    Q_strncpyz(bgSiegeClasses[bgNumSiegeClasses].name, parseBuf,
               sizeof(bgSiegeClasses[0].name));
  } else {
  ...
  if (BG_SiegeGetPairedValue(classInfo, "model", parseBuf)) {
    Q_strncpyz(bgSiegeClasses[bgNumSiegeClasses].forcedModel, parseBuf,
               sizeof(bgSiegeClasses[0].forcedModel));
  ...
  if (BG_SiegeGetPairedValue(classInfo, "skin", parseBuf)) {
    Q_strncpyz(bgSiegeClasses[bgNumSiegeClasses].forcedSkin, parseBuf,
               sizeof(bgSiegeClasses[0].forcedSkin));
  } else { // It's ok if there isn't one, it's optional.
  ...
  if (BG_SiegeGetPairedValue(classInfo, "saber1", parseBuf)) {
    Q_strncpyz(bgSiegeClasses[bgNumSiegeClasses].saber1, parseBuf,
               sizeof(bgSiegeClasses[0].saber1));
  } else { // It's ok if there isn't one, it's optional.
  ...
  if (BG_SiegeGetPairedValue(classInfo, "saber2", parseBuf)) {
    Q_strncpyz(bgSiegeClasses[bgNumSiegeClasses].saber2, parseBuf,
               sizeof(bgSiegeClasses[0].saber2));
  } else { // It's ok if there isn't one, it's optional.
  ...
  if (BG_SiegeGetPairedValue(classInfo, "saberstyle", parseBuf)) {
    bgSiegeClasses[bgNumSiegeClasses].saberStance =
        BG_SiegeTranslateGenericTable(parseBuf, StanceTable, qtrue);
  } else { // It's ok if there isn't one, it's optional.
  ...
  if (BG_SiegeGetPairedValue(classInfo, "weapons", parseBuf)) {
    bgSiegeClasses[bgNumSiegeClasses].weapons =
        BG_SiegeTranslateGenericTable(parseBuf, WPTable, qtrue);
  } else {
    Com_Error(ERR_DROP, "Siege class without weapons entry");
  }

  if (!(bgSiegeClasses[bgNumSiegeClasses].weapons &
        (1 << WP_SABER))) { // make sure it has melee if there's no saber
  ...
  if (BG_SiegeGetPairedValue(classInfo, "classflags", parseBuf)) {
    bgSiegeClasses[bgNumSiegeClasses].classflags =
        BG_SiegeTranslateGenericTable(parseBuf, bgSiegeClassFlagNames, qtrue);
  ...
  if (BG_SiegeGetPairedValue(classInfo, "starthealth", parseBuf)) {
    ...
    bgSiegeClasses[bgNumSiegeClasses].starthealth =
        bgSiegeClasses[bgNumSiegeClasses].maxhealth;
  }
  ...
  if (BG_SiegeGetPairedValue(classInfo, "startarmor", parseBuf)) {
    ...
    if (!bgSiegeClasses[bgNumSiegeClasses]
             .maxarmor) { // if they didn't specify a damn max armor then use
                          // this.
      bgSiegeClasses[bgNumSiegeClasses].maxarmor =
          bgSiegeClasses[bgNumSiegeClasses].startarmor;
    }
  ...
  if (BG_SiegeGetPairedValue(classInfo, "uishader", parseBuf)) {
    ...
    memset(bgSiegeClasses[bgNumSiegeClasses].uiPortrait, 0,
    ...
    bgSiegeClasses[bgNumSiegeClasses].uiPortraitShader =
        trap->R_RegisterShaderNoMip(parseBuf);
    memcpy(bgSiegeClasses[bgNumSiegeClasses].uiPortrait, parseBuf,
           sizeof(bgSiegeClasses[bgNumSiegeClasses].uiPortrait));
#endif
  } else { // I guess this is an essential.. we don't want to render bad shaders
           // or anything.
  ...
  if (BG_SiegeGetPairedValue(classInfo, "class_shader", parseBuf)) {
...
#else // cgame, ui
#if defined(_CGAME)
    bgSiegeClasses[bgNumSiegeClasses].classShader =
        trap->R_RegisterShaderNoMip(parseBuf);
    ...
    if (!bgSiegeClasses[bgNumSiegeClasses].classShader) {
      // Com_Error( ERR_DROP, "ERROR: could not find class_shader %s for class
  ...
  if (BG_SiegeGetPairedValue(classInfo, "holdables", parseBuf)) {
    bgSiegeClasses[bgNumSiegeClasses].invenItems =
        BG_SiegeTranslateGenericTable(parseBuf, HoldableTable, qtrue);
  ...
  if (BG_SiegeGetPairedValue(classInfo, "powerups", parseBuf)) {
    bgSiegeClasses[bgNumSiegeClasses].powerups =
        BG_SiegeTranslateGenericTable(parseBuf, PowerupTable, qtrue);
  } else { // Just don't start out with any then.
...
// This is really getting ugly - looking to get the base class (within a class)
...
void BG_SiegeLoadClasses(siegeClassDesc_t *descBuffer) {
  ...
  numFiles = trap->FS_GetFileList("ext_data/Siege/Classes", ".scl", filelist,
                                  sizeof(filelist));
...
void BG_SiegeParseTeamFile(const char *filename) {
  ...
  if (BG_SiegeGetPairedValue(teamInfo, "name", parseBuf)) {
    Q_strncpyz(bgSiegeTeams[bgNumSiegeTeams].name, parseBuf,
               sizeof(bgSiegeTeams[0].name));
  } else {
...
// I don't entirely like doing things this way but it's the easiest way.
  ...
  if (BG_SiegeGetPairedValue(teamInfo, "FriendlyShader", parseBuf))
    bgSiegeTeams[bgNumSiegeTeams].friendlyShader =
        trap->R_RegisterShaderNoMip(parseBuf);
#else
  ...
  if (BG_SiegeGetValueGroup(teamInfo, "Classes", teamInfo)) {
    while (success &&
           i < MAX_SIEGE_CLASSES) { // keep checking for group values named
                                    // class# up to MAX_SIEGE_CLASSES until we
                                    // can't find one.
      ...
      bgSiegeTeams[bgNumSiegeTeams]
          .classes[bgSiegeTeams[bgNumSiegeTeams].numClasses] =
          BG_SiegeFindClassByName(parseBuf);

      if (!bgSiegeTeams[bgNumSiegeTeams]
               .classes[bgSiegeTeams[bgNumSiegeTeams].numClasses]) {
...
void BG_SiegeLoadTeams(void) {
  ...
  numFiles = trap->FS_GetFileList("ext_data/Siege/Teams", ".team", filelist,
                                  sizeof(filelist));
...
extern qboolean WP_SaberParseParms(const char *saberName,
                                   saberInfo_t *saber); // bg_saberLoad.cpp
extern int BG_ModelCache(const char *modelName,
                         const char *skinName); // bg_misc.c
...
qboolean BG_SiegeCheckClassLegality(int team, char *classname) {
  ...
  if (!teamPtr || !(*teamPtr)) { // Well, guess the class is ok, seeing as there
                                 // is no team theme to begin with.
    return qtrue;
  }

  // See if the class is listed on the team
  while (i < (*teamPtr)->numClasses) {
    if (!Q_stricmp(classname,
                   (*teamPtr)->classes[i]->name)) { // found it, so it's alright
      return qtrue;
    }
    i++;
  }
...
siegeTeam_t *BG_SiegeFindTeamForTheme(char *themeName) {
  ...
  while (i < bgNumSiegeTeams) {
    if (bgSiegeTeams[i].name[0] &&
        !Q_stricmp(bgSiegeTeams[i].name,
                   themeName)) { // this is what we're looking for
