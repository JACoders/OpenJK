...
void G_AngerAlert(gentity_t *self) {
  if (self && self->NPC &&
      (self->NPC->scriptFlags & SCF_NO_GROUPS)) { // I'm not a team playa...
    return;
  }
  if (!TIMER_Done(self,
                  "interrogating")) { // I'm interrogating, don't wake everyone
                                      // else up yet... FIXME: this may never
                                      // wake everyone else up, though!
    return;
  }
  // FIXME: hmm.... with all the other new alerts now, is this still neccesary
  // or even a good idea...?
...
qboolean G_TeamEnemy(
    gentity_t *self) { // FIXME: Probably a better way to do this, is a linked
                       // list of your teammates already available?
  int i;
  gentity_t *ent;
  ...
  for (i = 1; i < level.num_entities; i++) {
    ...
    if (ent->client->playerTeam !=
        self->client->playerTeam) { // ent is not on my team
      continue;
    }
    ...
    if (ent->enemy) { // they have an enemy
      if (!ent->enemy->client ||
          ent->enemy->client->playerTeam !=
              self->client
                  ->playerTeam) { // the ent's enemy is either a normal ent or
                                  // is a player/NPC that is not on my team
        return qtrue;
      }
    }
  }
...
void G_AttackDelay(gentity_t *self, gentity_t *enemy) {
  if (enemy && self->client && self->NPC) { // delay their attack based on how
                                            // far away they're facing from enemy
    vec3_t fwd, dir;
    int attDelay;

    VectorSubtract(self->client->renderInfo.eyePoint, enemy->r.currentOrigin,
    ...
    */
    ...
    case WP_ROCKET_LAUNCHER:
      attDelay += Q_irand(500, 1500);
      break;
      //		case WP_BLASTER_PISTOL:	// apparently some enemy only
      //version of the blaster 			attDelay -= Q_irand( 500, 1500 ); 			break;
    ...
    case WP_TURRET: // turret guns
      return;
      break;
      //		case WP_BOT_LASER:		// Probe droid - Laser
      //blast 			return; //rwwFIXMEFIXME: Have this weapon for NPCs?
      break;
      /*
      case WP_DEMP2:
              break;
...
void G_ForceSaberOn(gentity_t *ent) {
  if (ent->client->ps.saberInFlight) { // alright, can't turn it on now in any
                                       // case, so forget it.
    return;
  }
  ...
  if (ent->client->ps.weapon !=
      WP_SABER) { // This probably should never happen. But if it does we'll
...
void G_SetEnemy(gentity_t *self, gentity_t *enemy) {
  ...
  if (self->NPC->confusionTime >
      level.time) { // can't pick up enemies if confused
    return;
  }
  ...
  if (self->client && self->NPC && enemy->client &&
      enemy->client->playerTeam ==
          self->client->playerTeam) {          // Probably a damn script!
  ...
  if (self->NPC && self->client && self->client->ps.weapon == WP_SABER) {
    // when get new enemy, set a base aggression based on what that enemy is
    // using, how far they are, etc.
  ...
  if (self->enemy == NULL) {
    ...
    // Special case- if player is being hunted by his own people, set their
    // enemy team correctly
    ...
    if (self->s.weapon == WP_BLASTER || self->s.weapon == WP_REPEATER ||
        self->s.weapon ==
            WP_THERMAL /*|| self->s.weapon == WP_BLASTER_PISTOL */ // rwwFIXMEFIXME:
                                                                   // Blaster
                                                                   // pistol
                                                                   // useable by
                                                                   // npcs?
        || self->s.weapon ==
               WP_BOWCASTER) { // Hmm, how about sniper and bowcaster?
      // When first get mad, aim is bad
      // Hmm, base on game difficulty, too?  Rank?
      if (self->client->playerTeam == NPCTEAM_PLAYER) {
        G_AimSet(self,
                 Q_irand(self->NPC->stats.aim - (5 * (g_npcspskill.integer)),
        ...
        G_AimSet(self, Q_irand(self->NPC->stats.aim -
                                   (maxErr * (3 - g_npcspskill.integer)),
                               self->NPC->stats.aim -
                                   (minErr * (3 - g_npcspskill.integer))));
      }
    }

    // Alert anyone else in the area
    if (Q_stricmp("desperado", self->NPC_type) != 0 &&
        Q_stricmp("paladin", self->NPC_type) !=
            0) { // special holodeck enemies exception
      if (self->client->ps.fd.forceGripBeingGripped <
          level.time) { // gripped people can't call for help
        G_AngerAlert(self);
      }
    }
    ...
    start with their weapon holstered- need a better way if (
    self->client->ps.weapon == WP_NONE && !Q_strncmp( self->NPC_type, "imp", 3 )
    && !(self->NPC->scriptFlags & SCF_FORCED_MARCH)  )
    {
            if ( self->client->ps.stats[STAT_WEAPONS] & ( 1 << WP_BLASTER ) )
            {
                    ...
                    G_CreateG2AttachedWeaponModel( self,
    weaponData[WP_BLASTER].weaponMdl, self->handRBolt, 0 );
            }
            else if ( self->client->ps.stats[STAT_WEAPONS] & ( 1 <<
    WP_BLASTER_PISTOL ) )
            {
                    ...
                    self->client->ps.weaponstate = WEAPON_READY;
                    G_CreateG2AttachedWeaponModel( self,
    weaponData[WP_BLASTER_PISTOL].weaponMdl, self->handRBolt, 0 );
            }
    }
    */
    return;
...
int ChooseBestWeapon( void )
{
        ...
        // check weapons serially (mainly in case a weapon is not on the NPC's
list) for ( weapon = 1; weapon < WP_NUM_WEAPONS; weapon++ )
        {
                if ( !HaveWeapon( weapon ) )
                {
                        continue;
...
//	ent->NPC->burstSpacing = 600;//attackdebounce
  ...
  case WP_THERMAL:
    ...
    case WP_SABER:
            ...
            ent->NPC->attackHold = 1000;//Hold attack button for a 1-second
    burst break;
    */
...
//	ent->NPC->burstSpacing = 1000;//attackdebounce
  ...
  case WP_EMPLACED_GUN:
    ...
    if (ent->client && ent->client->NPC_class == CLASS_REELO) {
      ...
      if (ent->parent) // if we have an owner, it should be the chair at this
                       // point...so query the chair for its shot debounce
                       // times, etc.
      {
        if (g_npcspskill.integer == 0) {
...
void NPC_ChangeWeapon(int newWeapon) {
  ...
  if ( changing && NPC->client->ps.weapon != WP_NONE )
  {
          ...
          {
                  G_CreateG2AttachedWeaponModel( NPC,
  NPC->client->ps.saber[0].model, NPC->handRBolt, 0 ); if (
  NPC->client->ps.dualSabers )
                  {
                          G_CreateG2AttachedWeaponModel( NPC,
  NPC->client->ps.saber[1].model, NPC->handLBolt, 0 );
                  }
          }
          else
          {
                  G_CreateG2AttachedWeaponModel( NPC,
  weaponData[NPC->client->ps.weapon].weaponMdl, NPC->handRBolt, 0 );
          }
  }*/
  // rwwFIXMEFIXME: Change the same way as players, all this stuff is just
  // crazy.
}
/*
void NPC_ApplyWeaponFireDelay(void)
How long, if at all, in msec the actual fire should delay from the time the
attack was started
*/
void NPC_ApplyWeaponFireDelay(void) {
  if (NPCS.NPC->attackDebounceTime >
      level.time) { // Just fired, if attacking again, must be a burst fire, so
                    // don't add delay
    // NOTE: Borg AI uses attackDebounceTime "incorrectly", so this will always
    // return for them!
    return;
  }
...
case WP_BOT_LASER:
  ...
  case WP_THERMAL:
    if (NPCS.client->ps.clientNum) { // NPCs delay...
      // FIXME: player should, too, but would feel weird in 1st person, even
      // though it 			would look right in 3rd person.  Really should have a wind-up
      //anim 			for player as he holds down the fire button to throw, then play 			the
      //actual throw when he lets go...
...
void ShootThink(void) {
  ...
  */
  ...
  NPCS.NPCInfo->currentAmmo =
      NPCS.client->ps
          .ammo[weaponData[NPCS.client->ps.weapon].ammoIndex]; // checkme
  ...
  if (NPCS.NPCInfo->aiFlags & NPCAI_BURST_WEAPON) {
    if (!NPCS.NPCInfo->burstCount) {
      NPCS.NPCInfo->burstCount =
          Q_irand(NPCS.NPCInfo->burstMin, NPCS.NPCInfo->burstMax);
      /*
    ...
    if (!delay) {
      // HACK: dirty little emplaced bits, but is done because it would
      // otherwise require some sort of new variable...
      if (NPCS.client->ps.weapon == WP_EMPLACED_GUN) {
        if (NPCS.NPC->parent) // try and get the debounce values from the chair
                              // if we can
        {
          if (g_npcspskill.integer == 0) {
...
FIXME makes this so there's a delay from event that caused us to check to
actually doing it
...
void WeaponThink(qboolean inCombat) {
  ...
  if (NPCS.NPC->client->ps.ammo[weaponData[NPCS.client->ps.weapon].ammoIndex] <
      10) // checkme
...
qboolean EntIsGlass(gentity_t *check) {
  if (check->classname && !Q_stricmp("func_breakable", check->classname) &&
...
qboolean ShotThroughGlass(trace_t *tr, gentity_t *target, vec3_t spot,
                          int mask) {
...
Added: If can't shoot center, try head, if not, see if it's close enough to try
anyway.
*/
qboolean CanShoot(gentity_t *ent, gentity_t *shooter) {
  ...
  trap->Trace(&tr, muzzle, NULL, NULL, spot, shooter->s.number, MASK_SHOT,
              qfalse, 0, 0);
  ...
  // Actually, we should just check to fire in dir we're facing and if it's
  // close enough, and we didn't hit someone on our own team, shoot
  ...
  if (traceEnt->client &&
      (traceEnt->client->playerTeam == shooter->client->playerTeam)) {
    return qfalse;
  }
...
void NPC_CheckPossibleEnemy(gentity_t *other, visibility_t vis) {
  ...
  // we already have an enemy and this guy is in our FOV, see if this guy would
  // be better
  if (NPCS.NPC->enemy && vis == VIS_FOV) {
    ...
    if (NPCS.enemyVisibility == VIS_UNKNOWN) {
      NPCS.enemyVisibility =
          NPC_CheckVisibility(NPCS.NPC->enemy, CHECK_360 | CHECK_FOV);
    }
  ...
  if (!NPCS.NPC->enemy) { // only take an enemy if you don't have one yet
...
float NPC_MaxDistSquaredForWeapon(void) {
  if (NPCS.NPCInfo->stats.shootDistance > 0) { // overrides default weapon dist
    return NPCS.NPCInfo->stats.shootDistance *
           NPCS.NPCInfo->stats.shootDistance;
  }
...
case WP_BLASTER_PISTOL://prifle
  ...
  case WP_SABER:
    if (NPCS.NPC->client &&
        NPCS.NPC->client->saber[0]
            .blade[0]
            .lengthMax) { // FIXME: account for whether enemy and I are heading
                          // towards each other!
      return (NPCS.NPC->client->saber[0].blade[0].lengthMax +
              NPCS.NPC->r.maxs[0] * 1.5) *
...
qboolean ValidEnemy(gentity_t *ent) {
  ...
  if (!(ent->flags & FL_NOTARGET)) {
    if (ent->health > 0) {
      if (!ent->client) {
        return qtrue;
      } else if (ent->client->sess.sessionTeam ==
                 TEAM_SPECTATOR) { // don't go after spectators
        return qfalse;
      } else if (ent->client->tempSpectate >=
                 level.time) { // don't go after spectators
        return qfalse;
      } else {
...
qboolean NPC_EnemyTooFar(gentity_t *enemy, float dist, qboolean toShoot) {
  ...
  if (!toShoot) { // Not trying to actually press fire button with this check
    if (NPCS.NPC->client->ps.weapon == WP_SABER) { // Just have to get to him
      return qfalse;
    }
  }
...
You can mix and match any of those options (example: find closest visible
players first)

FIXME: this should go through the snapshot and find the closest enemy
*/
gentity_t *NPC_PickEnemy(gentity_t *closestTo, int enemyTeam, qboolean checkVis,
                         qboolean findPlayersFirst, qboolean findClosest) {
  int num_choices = 0;
  ...
  if (NPCS.NPCInfo->behaviorState == BS_STAND_AND_SHOOT ||
      NPCS.NPCInfo->behaviorState ==
          BS_HUNT_AND_KILL) { // Formations guys don't require inFov to pick up
                              // a target
  ...
  if (findPlayersFirst) { // try to find a player first
    ...
    if (newenemy->client && !(newenemy->flags & FL_NOTARGET) &&
      ...
      if (newenemy->health > 0) {
        if (NPC_ValidEnemy(
                newenemy)) // enemyTeam == TEAM_PLAYER ||
                           // newenemy->client->playerTeam == enemyTeam || (
                           // enemyTeam == TEAM_PLAYER ) )
        { // FIXME:  check for range and FOV or vis?
          if (newenemy != NPCS.NPC->lastEnemy) { // Make sure we're not just
                                                 // going back and forth here
            if (trap->InPVS(newenemy->r.currentOrigin,
                            NPCS.NPC->r.currentOrigin)) {
              if (NPCS.NPCInfo->behaviorState == BS_INVESTIGATE ||
                  NPCS.NPCInfo->behaviorState == BS_PATROL) {
                if (!NPCS.NPC->enemy) {
                  if (!InVisrange(newenemy)) {
                    failed = qtrue;
                  } else if (NPC_CheckVisibility(
                                 newenemy, CHECK_360 | CHECK_FOV |
              ...
              if (!failed) {
                ...
                if (newenemy->client->hiddenDist > 0) {
                  if (relDist > newenemy->client->hiddenDist *
                    ...
                    if (VectorLengthSquared(
                            newenemy->client
                                ->hiddenDir)) { // They're only hidden from a
                                                // certain direction, check
                      float dot;
                      VectorNormalize(diff);
                      dot = DotProduct(newenemy->client->hiddenDir, diff);
                      if (dot > 0.5) { // I'm not looking in the right dir
                                       // toward them to see them
                        failed = qtrue;
                      } else {
                        Debug_Printf(&d_npcai, DEBUG_LEVEL_INFO,
                                     "%s saw %s trying to hide - hiddenDir %s "
                                     "targetDir %s dot %f\n",
                ...
                if (!failed) {
                  if (findClosest) {
                    ...
                    if (checkVis) {
                      if (NPC_CheckVisibility(newenemy, CHECK_360 | CHECK_FOV |
                                                            CHECK_VISRANGE) ==
                          VIS_FOV) {
  ...
  /*
  //FIXME: used to have an option to look *only* for the player... now...? Still
  need it? if ( enemyTeam == TEAM_PLAYER )
  ...
  */
  ...
  for (entNum = 0; entNum < level.num_entities; entNum++) {
    ...
    if (newenemy != NPCS.NPC &&
        (newenemy->client /*|| newenemy->svFlags & SVF_NONNPC_ENEMY*/) &&
      ...
      if (newenemy->health > 0) {
        if ((newenemy->client && NPC_ValidEnemy(newenemy)) ||
          ...
          if (NPCS.NPC->client->playerTeam == NPCTEAM_PLAYER &&
              enemyTeam ==
                  NPCTEAM_PLAYER) { // player allies turning on ourselves?  How?
            if (newenemy->s.number >= MAX_CLIENTS) { // only turn on the player,
                                                     // not other player allies
              continue;
            }
          }
          ...
          if (newenemy != NPCS.NPC->lastEnemy) { // Make sure we're not just
            ...
            if (newenemy->client && newenemy->client->hiddenDist > 0) {
              if (relDist >
                ...
                if (VectorLengthSquared(
                  ...
                  VectorNormalize(diff);
                  dot = DotProduct(newenemy->client->hiddenDir, diff);
                  if (dot > 0.5) { // I'm not looking in the right dir toward
                                   // them to see them
                    continue;
                  } else {
                    Debug_Printf(&d_npcai, DEBUG_LEVEL_INFO,
            ...
            if (findClosest) {
              if (relDist < bestDist) {
                if (!NPC_EnemyTooFar(newenemy, relDist, qfalse)) {
                  if (checkVis) {
                    // FIXME: NPCs need to be able to pick up other NPCs behind
                    // them, but for now, commented out because it was picking up
                    // enemies it shouldn't if ( NPC_CheckVisibility ( newenemy,
                    // CHECK_360|CHECK_VISRANGE ) >= VIS_360 )
  ...
  if (findClosest) { // FIXME: you can pick up an enemy around a corner this
                     // way.
    return closestEnemy;
  }
...
gentity_t *NPC_PickAlly(qboolean facingEachOther, float range,
  ...
  for (entNum = 0; entNum < level.num_entities; entNum++) {
    ...
    if (ally->client) {
      if (ally->health > 0) {
        if (ally->client &&
            (ally->client->playerTeam == NPCS.NPC->client->playerTeam ||
             NPCS.NPC->client->playerTeam ==
                 NPCTEAM_ENEMY)) // && ally->client->playerTeam == TEAM_DISGUISE
                                 // ) ) )
        { // if on same team or if player is disguised as your team
          if (ignoreGroup) {
            ...
            if (ally->client && ally->client->leader &&
                ally->client->leader == NPCS.NPC) {
              // reject
              continue;
            }
          }
          ...
          if (movingOnly && ally->client &&
              NPCS.NPC
                  ->client) { // They have to be moving relative to each other
            if (!DistanceSquared(ally->client->ps.velocity,
                                 NPCS.NPC->client->ps.velocity)) {
              continue;
            }
          }

          VectorSubtract(NPCS.NPC->r.currentOrigin, ally->r.currentOrigin,
                         diff);
          ...
          if (relDist < bestDist) {
            ...
            if (NPC_CheckVisibility(ally, CHECK_360 | CHECK_VISRANGE) >=
                VIS_360) {
              bestDist = relDist;
              closestAlly = ally;
            }
...
gentity_t *NPC_CheckEnemy(qboolean findNew, qboolean tooFarOk,
                          qboolean setEnemy) {
  qboolean forcefindNew = qfalse;
  gentity_t *closestTo;
  gentity_t *newEnemy = NULL;
  // FIXME: have a "NPCInfo->persistance" we can set to determine how long to
  // try to shoot someone we can't hit?  Rather than hard-coded 10?
  ...
  if (NPCS.NPC->enemy) {
    if (!NPCS.NPC->enemy->inuse) //|| NPC->enemy == NPC )//wtf?  NPCs should
                                 //never get mad at themselves!
    {
      if (setEnemy) {
  ...
  {//keep this enemy until dead
          if ( NPC->enemy )
          {
                  if ( (!NPC->NPC && !(NPC->svFlags & SVF_NONNPC_ENEMY) ) ||
  NPC->enemy->health > 0 )
                  {//Enemy never had health (a train or info_not_null, etc) or
  did and is now dead (NPCs, turrets, etc) return NULL;
                  }
          }
  ...
  */
  ...
  if (NPCS.NPC->enemy) {
    if (NPC_EnemyTooFar(NPCS.NPC->enemy, 0, qfalse)) {
      if (findNew) { // See if there is a close one and take it if so, else keep
                     // this one
        forcefindNew = qtrue;
      } else if (!tooFarOk) // FIXME: don't need this extra bool any more
      {
        if (setEnemy) {
          G_ClearEnemy(NPCS.NPC);
                   ...
                   NPCS.NPC->r.currentOrigin,
                   NPCS.NPC->enemy->r.currentOrigin)) { // FIXME: should this be
                                                        // a line-of site check?
      ...
      if (NPCS.NPC->enemy->client &&
          NPCS.NPC->enemy->client
              ->hiddenDist) { // He ducked into shadow while we weren't looking
        // Drop enemy and see if we should search for him
        NPC_LostEnemyDecideChase();
      } else { // If we're not chasing him, we need to lose him
               // NOTE: since we no longer have bStates, really, this logic
               // doesn't work, so never give him up
        ...
        //FIXME: only do this if !(NPCInfo->scriptFlags&SCF_CHASE_ENEMY)
                //If he's not our goalEntity, we're running somewhere else, so
        lose him if ( NPC->enemy != NPCInfo->goalEntity )
                {
                        G_ClearEnemy( NPC );
                }
                break;
  ...
  closestTo = NPCS.NPC;
  // FIXME: check your defendEnt, if you have one, see if their enemy is
  // different than yours, or, if they don't have one, pick the closest enemy to
  // THEM?
  if (NPCS.NPCInfo->defendEnt) { // Trying to protect someone
    if (NPCS.NPCInfo->defendEnt->health >
        0) { // Still alive, We presume we're close to them, navigation should
             // handle this?
      if (NPCS.NPCInfo->defendEnt
              ->enemy) { // They were shot or acquired an enemy
        if (NPCS.NPC->enemy !=
            NPCS.NPCInfo->defendEnt
                ->enemy) { // They have a different enemy, take it!
          ...
          if (setEnemy) {
            G_SetEnemy(NPCS.NPC, NPCS.NPCInfo->defendEnt->enemy);
                 ...
                 NULL) { // We don't have an enemy, so find closest to defendEnt
        closestTo = NPCS.NPCInfo->defendEnt;
      }
    }
  }

  if (!NPCS.NPC->enemy || (NPCS.NPC->enemy && NPCS.NPC->enemy->health <= 0) ||
      forcefindNew) { // FIXME: NPCs that are moving after an enemy should
                      // ignore the can't hit enemy counter- that should only be
                      // for NPCs that are standing still
    ...
    //	try to find another enemy after 10 seconds (assuming the
    //cantHitEnemyCounter 	is allowed to increment in a chasing bState)
    ...
    if (NPCS.NPC->client->enemyTeam != NPCTEAM_NEUTRAL) {
      // NOTE:  this only checks vis if can't hit enemy for 10 tries, which I
      // suppose 			means they need to find one that in more than just PVS newenemy
      // = NPC_PickEnemy( closestTo, NPC->client->enemyTeam,
      // (NPC->cantHitEnemyCounter > 10), qfalse, qtrue );//3rd parm was
      // (NPC->enemyTeam == TEAM_STARFLEET) For now, made it so you ALWAYS have
      // to check VIS
      newEnemy = NPC_PickEnemy(
          closestTo, NPCS.NPC->client->enemyTeam, qtrue, qfalse,
    ...
    // FIXME: if we can't find any at all, go into independent NPC AI, pursue
    // and kill
  }

  if (NPCS.NPC->enemy && NPCS.NPC->enemy->client) {
    if (NPCS.NPC->enemy->client->playerTeam) {
      //			assert( NPC->client->playerTeam !=
      //NPC->enemy->client->playerTeam);
...
qboolean NPC_ClearShot(gentity_t *ent) {
  ...
  // add aim error
  // use weapon instead of specific npc types, although you could add certain
  // npc classes if you wanted
  //	if ( NPC->client->playerTeam == TEAM_SCAVENGERS )
  if (NPCS.NPC->s.weapon ==
      WP_BLASTER /*|| NPC->s.weapon == WP_BLASTER_PISTOL*/) // any other guns to
                                                            // check for?
  {
    vec3_t mins = {-2, -2, -2};
    vec3_t maxs = {2, 2, 2};

    trap->Trace(&tr, muzzle, mins, maxs, ent->r.currentOrigin,
                NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0);
  } else {
    trap->Trace(&tr, muzzle, NULL, NULL, ent->r.currentOrigin,
                NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0);
  }
...
int NPC_ShotEntity(gentity_t *ent, vec3_t impactPos) {
  ...
  if (NPCS.NPC->s.weapon ==
      WP_THERMAL) { // thermal aims from slightly above head
    ...
    trap->Trace(&tr, muzzle, vec3_origin, vec3_origin, end, NPCS.NPC->s.number,
                                                            ...
                                                            // check for?
  {
    ...
    trap->Trace(&tr, muzzle, mins, maxs, targ, NPCS.NPC->s.number, MASK_SHOT,
                qfalse, 0, 0);
  } else {
    trap->Trace(&tr, muzzle, NULL, NULL, targ, NPCS.NPC->s.number, MASK_SHOT,
                qfalse, 0, 0);
  }
  // FIXME: if using a bouncing weapon like the bowcaster, should we check the
  // reflection of the wall, too?
  if (impactPos) { // they want to know *where* the hit would be, too
    VectorCopy(tr.endpos, impactPos);
  }
  /* // NPCs should be able to shoot even if the muzzle would be inside their
     target if ( tr.startsolid || tr.allsolid )
          {
...
qboolean NPC_CheckCanAttack(float attack_scale, qboolean stationary) {
  ...
  if (NPC_EnemyTooFar(NPCS.NPC->enemy, distanceToEnemy * distanceToEnemy,
                      qtrue)) { // Too far away?  Do not attack
    return qfalse;
  }

  if (NPCS.client->ps.weaponTime > 0) { // already waiting for a shot to fire
  ...
  NPCS.enemyVisibility = NPC_CheckVisibility(
      NPCS.NPC->enemy, CHECK_360 | CHECK_FOV); ////CHECK_PVS|

  if (NPCS.enemyVisibility >= VIS_FOV) { // He's in our FOV
    ...
    if (NPCS.NPC->enemy->client) {
      if (NPCS.NPC->enemy->enemy == NPCS.NPC) {
        if (NPCS.NPC->enemy->client->buttons &
            BUTTON_ATTACK) { // FIXME: determine if enemy fire angles would hit
                             // me or get close
    ...
    if (attack_ok) {
      ...
      trap->Trace(&tr, muzzle, NULL, NULL, hitspot, NPCS.NPC->s.number,
                  MASK_SHOT, qfalse, 0, 0);
      ...
      */
      ...
      /*
      if( traceEnt != NPC->enemy &&//FIXME: if someone on our team is in the
      way, suggest that they duck if possible
      ...
      {//no, so shoot for somewhere between the head and torso
              ...
              enemy_org[0] += 0.3*Q_flrand(NPC->enemy->r.mins[0],
      NPC->enemy->r.maxs[0]); enemy_org[1] +=
      0.3*Q_flrand(NPC->enemy->r.mins[1], NPC->enemy->r.maxs[1]); enemy_org[2]
      -= NPC->enemy->r.maxs[2]*Q_flrand(0.0f, 1.0f);

              attack_scale *= 0.75;
              trap->Trace ( &tr, muzzle, NULL, NULL, enemy_org, NPC->s.number,
      MASK_SHOT ); ShotThroughGlass(&tr, NPC->enemy, enemy_org, MASK_SHOT);
              traceEnt = &g_entities[tr.entityNum];
      }
      */
      ...
      if (traceEnt == NPCS.NPC->enemy ||
          (traceEnt->client && NPCS.NPC->client->enemyTeam &&
           NPCS.NPC->client->enemyTeam == traceEnt->client->playerTeam)) {
        dead_on = qtrue;
      } else {
        attack_scale *= 0.5;
    ...
    if (attack_ok) {
      ...
      if (!dead_on) { // We're not going to hit him directly, try a suppressing
                      // fire
        // see if where we're going to shoot is too far from his origin
        if (traceEnt && (traceEnt->health <= 30 ||
                         EntIsGlass(traceEnt))) { // easy to kill - go for it
          ...
          {      // going to explode, don't shoot if close to self
            VectorSubtract(NPCS.NPC->r.currentOrigin, traceEnt->r.currentOrigin,
                           diff);
            if (VectorLengthSquared(diff) <
                traceEnt->splashRadius *
                    traceEnt->splashRadius) { // Too close to shoot!
              attack_ok = qfalse;
...
/*QUAKED point_combat (0.7 0 0.7) (-16 -16 -24) (16 16 32) DUCK FLEE INVESTIGATE
SQUAD LEAN SNIPE NPCs in bState BS_COMBAT_POINT will find their closest empty
combat_point
...
void SP_point_combat(gentity_t *self) {
  ...
  VectorCopy(self->r.currentOrigin,
             level.combatPoints[level.numCombatPoints].origin);
...
void CP_FindCombatPointWaypoints(void) {
  ...
  for (i = 0; i < level.numCombatPoints; i++) {
    level.combatPoints[i].waypoint =
        NAV_FindClosestWaypointForPoint2(level.combatPoints[i].origin);
#ifndef FINAL_BUILD
...
static int NPC_CollectCombatPoints(const vec3_t origin, const float radius,
                                   combatPt_t *points, const int flags) {
  ...
  for (i = 0; i < level.numCombatPoints; i++) {
    ...
    if (flags & CP_HORZ_DIST_COLL) {
      distance =
          DistanceHorizontalSquared(origin, level.combatPoints[i].origin);
    } else {
...
int NPC_FindCombatPoint(const vec3_t position, const vec3_t avoidPosition,
                        vec3_t enemyPosition, const int flags,
                        const float avoidDist, const int ignorePoint) {
  ...
  if ((flags & CP_HAS_ROUTE) ||
      (flags & CP_NEAREST)) { // going to be doing macro nav tests
  ...
  numPoints = NPC_CollectCombatPoints(enemyPosition, collRad, points,
                                      flags); // position
  ...
  for (j = 0; j < numPoints; j++) {
    ...
    // FIXME: able to mark certain ones as too dangerous to go to for now?  Like
    // a tripmine/thermal/detpack is near or something? If we need a cover point,
    // check this point
    if ((flags & CP_COVER) &&
        (NPC_ClearLOS(level.combatPoints[i].origin, enemyPosition) ==
         qtrue)) // Used to use NPC->enemy
      continue;

    // Need a clear LOS to our target... and be within shot range to enemy
    // position (FIXME: make this a separate CS_ flag? and pass in a range?)
    if (flags & CP_CLEAR) {
      if (NPC_ClearLOS3(level.combatPoints[i].origin, NPCS.NPC->enemy) ==
          qfalse) {
        continue;
      }
      if (NPCS.NPC->s.weapon == WP_THERMAL) { // horizontal
        dist = DistanceHorizontalSquared(level.combatPoints[i].origin,
                                         NPCS.NPC->enemy->r.currentOrigin);
      } else { // actual
        dist = DistanceSquared(level.combatPoints[i].origin,
                               NPCS.NPC->enemy->r.currentOrigin);
      }
      if (dist >
    ...
    // Avoid this position?
    if ((flags & CP_AVOID) &&
        (DistanceSquared(level.combatPoints[i].origin, position) <
         modifiedAvoidDist)) // was using MIN_AVOID_DISTANCE_SQUARED, not passed
                             // in modifiedAvoidDist
      continue;
    ...
    if (flags & CP_RETREAT) {
      if (flags & CP_HORZ_DIST_COLL) {
        if (pdist < DistanceHorizontalSquared(
                        position, enemyPosition)) { // it's closer, don't use it
          continue;
        }
      } else {
        if (pdist < DistanceSquared(
                        position, enemyPosition)) { // it's closer, don't use it
          continue;
        }
      }
    ...
    // FIXME: this needs to check for the waypoint you'll be taking to get to
    // that combat point
    if (flags & CP_AVOID_ENEMY) {
      ...
      if ( NPC->waypoint != WAYPOINT_NONE && NPC->waypoint !=
      level.combatPoints[i].waypoint )
      {
    ...
    trap->Trace(&tr, level.combatPoints[i].origin, NPCS.NPC->r.mins,
                NPCS.NPC->r.maxs, level.combatPoints[i].origin,
      ...
      */
      ...
      if (waypoint == WAYPOINT_NONE ||
          ...
          trap->Nav_GetBestNodeAltRoute2(
              waypoint, level.combatPoints[i].waypoint, NODE_NONE) ==
              WAYPOINT_NONE) { // can't possibly have a route to any OR can't
                               // possibly have a route to this one OR don't have
                               // a route to this one
        if (!NAV_ClearPathToPoint(NPCS.NPC, NPCS.NPC->r.mins, NPCS.NPC->r.maxs,
                                  level.combatPoints[i].origin,
                                  NPCS.NPC->clipmask,
                                  ENTITYNUM_NONE)) { // don't even have a clear
                                                     // straight path to this one
          continue;
        }
      }
    }
    ...
    // if ( flags & CP_CLOSEST )
    // they are sorted by this distance, so the first one to get this far is the
    // closest
    return i;
  }
...
int NPC_FindSquadPoint(vec3_t position) {
  ...
  // float			playerDist = DistanceSquared( g_entities[0].currentOrigin,
  // NPC->r.currentOrigin );
  ...
  for (i = 0; i < level.numCombatPoints; i++) {
    ...
    // if ( dist > ( playerDist * DotProduct( dirToPlayer, playerDir ) ) )
    // //FIXME: Retain this 	continue;
...
extern qboolean CheckItemCanBePickedUpByNPC(gentity_t *item,
...
gentity_t *NPC_SearchForWeapons(void) {
  ...
  //	for ( found = g_entities; found < &g_entities[globals.num_entities] ;
  //found++)
  for (i = 0; i < level.num_entities; i++) {
    ...
    if (CheckItemCanBePickedUpByNPC(found, NPCS.NPC)) {
      if (trap->InPVS(found->r.currentOrigin, NPCS.NPC->r.currentOrigin)) {
        dist =
            DistanceSquared(found->r.currentOrigin, NPCS.NPC->r.currentOrigin);
        if (dist < bestDist) {
          if (!trap->Nav_GetBestPathBetweenEnts((sharedEntity_t *)NPCS.NPC,
                                                (sharedEntity_t *)found,
                                                NF_CLEAR_PATH) ||
              trap->Nav_GetBestNodeAltRoute2(NPCS.NPC->waypoint,
                                             found->waypoint, NODE_NONE) ==
                  WAYPOINT_NONE) { // can't possibly have a route to any OR
                                   // can't possibly have a route to this one OR
                                   // don't have a route to this one
            if (NAV_ClearPathToPoint(
                    NPCS.NPC, NPCS.NPC->r.mins, NPCS.NPC->r.maxs,
...
void NPC_SetPickUpGoal(gentity_t *foundWeap) {
  ...
  org[2] += 24 - (foundWeap->r.mins[2] *
                  -1); // adjust the origin so that I am on the ground
  NPC_SetMoveGoal(NPCS.NPC, org, foundWeap->r.maxs[0] * 0.75, qfalse, -1,
                  foundWeap);
...
void NPC_CheckGetNewWeapon(void) {
  if (NPCS.NPC->s.weapon == WP_NONE &&
      NPCS.NPC->enemy) { // if running away because dropped weapon...
    if (NPCS.NPCInfo->goalEntity &&
        NPCS.NPCInfo->goalEntity == NPCS.NPCInfo->tempGoal &&
        NPCS.NPCInfo->goalEntity->enemy &&
        !NPCS.NPCInfo->goalEntity->enemy
             ->inuse) { // maybe was running at a weapon that was picked up
      NPCS.NPCInfo->goalEntity = NULL;
    }
    if (TIMER_Done(NPCS.NPC, "panic") &&
        NPCS.NPCInfo->goalEntity == NULL) { // need a weapon, any lying around?
      ...
      if (foundWeap) { // try to nav to it
        ...
        if ( !trap->Nav_GetBestPathBetweenEnts( NPC, foundWeap, NF_CLEAR_PATH )
                || trap->Nav_GetBestNodeAltRoute( NPC->waypoint,
        foundWeap->waypoint ) == WAYPOINT_NONE )
        {//can't possibly have a route to any OR can't possibly have a route to
        this one OR don't have a route to this one if ( !NAV_ClearPathToPoint(
        NPC, NPC->r.mins, NPC->r.maxs, foundWeap->r.currentOrigin,
        NPC->clipmask, ENTITYNUM_NONE ) )
...
void NPC_AimAdjust(int change) {
  ...
  if (TIMER_Done(NPCS.NPC, "aimDebounce")) {
    ...
    if (NPCS.NPCInfo->currentAim >
        NPCS.NPCInfo->stats.aim) { // can never be better than max aim
      NPCS.NPCInfo->currentAim = NPCS.NPCInfo->stats.aim;
    } else if (NPCS.NPCInfo->currentAim < -30) { // can never be worse than this
