...
extern void G_VehicleSetDamageLocFlags(gentity_t *veh, int impactDir,
                                       int deathPoint);
...
qboolean G_HeavyMelee(gentity_t *attacker) {
  if (level.gametype == GT_SIEGE && attacker && attacker->client &&
      attacker->client->siegeClass != -1 &&
      (bgSiegeClasses[attacker->client->siegeClass].classflags &
       (1 << CFL_HEAVYMELEE))) {
    return qtrue;
  }
  return qfalse;
...
int G_GetHitLocation(gentity_t *target, vec3_t ppoint) {
  ...
  //	tradius = (fabs(target->r.maxs[0]) + fabs(target->r.maxs[1]) +
  //fabs(target->r.mins[0]) + fabs(target->r.mins[1]))/4;
...
void ExplodeDeath(gentity_t *self) {
  ...
  {
    //		CG_SurfaceExplosion( self->r.currentOrigin, forward, 20.0f, 12.0f,
  ...
  if (self->splashDamage > 0 && self->splashRadius > 0) {
    ...
    G_RadiusDamage(self->r.currentOrigin, attacker, self->splashDamage,
                   self->splashRadius, attacker, NULL, MOD_UNKNOWN);
  }
...
void TossClientWeapon(gentity_t *self, vec3_t direction, float speed) {
  ...
  ammoSub = (self->client->ps.ammo[weaponData[weapon].ammoIndex] -
             bg_itemlist[BG_GetItemIndexByTag(weapon, IT_WEAPON)].quantity);
  ...
  if ((self->client->ps.ammo[weaponData[weapon].ammoIndex] < 1 &&
       weapon != WP_DET_PACK) ||
      (weapon != WP_THERMAL && weapon != WP_DET_PACK &&
       weapon != WP_TRIP_MINE)) {
    int i = 0;
    int weap = -1;
...
void TossClientItems(gentity_t *self) {
  ...
  if (weapon > WP_BRYAR_PISTOL && weapon != WP_EMPLACED_GUN &&
      weapon != WP_TURRET &&
      self->client->ps.ammo[weaponData[weapon].ammoIndex]) {
    gentity_t *te;
...
void body_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
  ...
  if (self->s.eType ==
      ET_NPC) { // well, just rem it then, so long as it's done with its death
                // anim and it's not a standard weapon.
    if (self->client && self->client->ps.torsoTimer <= 0 &&
        (meansOfDeath == MOD_UNKNOWN || meansOfDeath == MOD_WATER ||
         meansOfDeath == MOD_SLIME || meansOfDeath == MOD_LAVA ||
         meansOfDeath == MOD_CRUSH || meansOfDeath == MOD_TELEFRAG ||
...
char *modNames[MOD_MAX] = {"MOD_UNKNOWN",
                           "MOD_STUN_BATON",
                           "MOD_MELEE",
                           "MOD_SABER",
                           "MOD_BRYAR_PISTOL",
...
static int G_CheckSpecialDeathAnim(gentity_t *self, vec3_t point, int damage,
  ...
  } else if (G_InKnockDown(
                 &self->client->ps)) { // since these happen a lot, let's handle
                                       // them case by case
    int animLength =
        bgAllAnims[self->localAnimIndex]
            .anims[self->client->ps.legsAnim]
            .numFrames *
        fabs(
            (float)(bgHumanoidAnimations[self->client->ps.legsAnim].frameLerp));
...
int G_PickDeathAnim(
    gentity_t *self, vec3_t point, int damage, int mod,
    int hitLoc) { // FIXME: play dead flop anims on body if in an appropriate
                  // _DEAD anim when this func is called
  int deathAnim = -1;
  int max_health;
  int legAnim = 0;
  vec3_t objVelocity;
  ...
  if (self->client) {
    ...
    if (self->client->inSpaceIndex &&
        self->client->inSpaceIndex != ENTITYNUM_NONE) {
      return BOTH_CHOKE3;
    }
  } else {
    max_health = 60;
  ...
  for (i = 0; i < numEnts; i++) {
    ...
    if (check->enemy ==
      ...
      if (distSq > 16384 /*128 squared*/ &&
          !trap->InPVS(victim->r.currentOrigin,
                       check->r.currentOrigin)) { // not even potentially
                                                  // visible/hearable
        continue;
      }
      ...
      if (soundDist <= 0 || distSq > sndDistSq) { // out of sound range
        if (!InFOV(victim, check, check->NPC->stats.hfov,
                   check->NPC->stats.vfov) ||
            !NPC_ClearLOS2(check,
                           victim->r.currentOrigin)) { // out of FOV or no LOS
          continue;
        }
      }
...
void DeathFX(gentity_t *ent) {
  ...
  case CLASS_PROBE:
    VectorCopy(ent->r.currentOrigin, effectPos);
    effectPos[2] += 50;
    G_PlayEffectID(G_EffectIndex("explosions/probeexplosion1"), effectPos,
                   defaultDir);
    break;
  ...
  case CLASS_ATST:
    ...
    effectPos[2] += 180;
    G_PlayEffectID(G_EffectIndex("explosions/droidexplosion1"), effectPos,
                   defaultDir);
  ...
  case CLASS_SEEKER:
  case CLASS_REMOTE:
    G_PlayEffectID(G_EffectIndex("env/small_explode"), ent->r.currentOrigin,
                   defaultDir);
    break;
  ...
  case CLASS_GONK:
    ...
    G_Sound(
        ent, CHAN_AUTO,
        G_SoundIndex(va("sound/chars/gonk/misc/death%d.wav", Q_irand(1, 3))));
...
void G_AddPowerDuelScore(int team, int score) {
  ...
  while (i < MAX_CLIENTS) {
    ...
    if (check->inuse && check->client &&
        ...
        !check->client->iAmALoser && check->client->ps.stats[STAT_HEALTH] > 0 &&
        check->client->sess.sessionTeam != TEAM_SPECTATOR &&
        check->client->sess.duelTeam ==
            team) { // found a living client on the specified team
...
void player_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
  ...
  if (level.gametype == GT_POWERDUEL) { // don't want to wait til later in the
                                        // frame if this is the case
    CheckExitRules();
  ...
  if (self->s.eType == ET_NPC && self->s.NPC_class == CLASS_VEHICLE &&
      self->m_pVehicle && !self->m_pVehicle->m_pVehicleInfo->explosionDelay &&
      (self->m_pVehicle->m_pPilot || self->m_pVehicle->m_iNumPassengers > 0 ||
       self->m_pVehicle
           ->m_pDroidUnit)) { // kill everyone on board in the name of the
                              // attacker... if the vehicle has no death delay
    gentity_t *murderer = NULL;
    gentity_t *killEnt;

    if (self->client->ps.otherKillerTime >=
        level.time) { // use the last attacker
      murderer = &g_entities[self->client->ps.otherKiller];
      if (!murderer->inuse || !murderer->client) {
        murderer = NULL;
      } else {
        if (murderer->s.number >= MAX_CLIENTS && murderer->s.eType == ET_NPC &&
            murderer->s.NPC_class == CLASS_VEHICLE && murderer->m_pVehicle &&
            murderer->m_pVehicle->m_pPilot) {
          gentity_t *murderPilot =
              &g_entities[murderer->m_pVehicle->m_pPilot->s.number];
          if (murderPilot->inuse &&
              murderPilot->client) { // give the pilot of the offending vehicle
                                     // credit for the kill
      ...
      if (attacker->s.number >= MAX_CLIENTS && attacker->s.eType == ET_NPC &&
          attacker->s.NPC_class == CLASS_VEHICLE && attacker->m_pVehicle &&
          attacker->m_pVehicle
              ->m_pPilot) { // set vehicles pilot's killer as murderer
        murderer = &g_entities[attacker->m_pVehicle->m_pPilot->s.number];
        if (murderer->inuse && murderer->client &&
            murderer->client->ps.otherKillerTime >= level.time) {
    ...
    if (self->m_pVehicle->m_pVehicleInfo
            ->hideRider) { // pilot is *inside* me, so kill him, too
      killEnt = (gentity_t *)self->m_pVehicle->m_pPilot;
      if (killEnt && killEnt->inuse && killEnt->client) {
        G_Damage(killEnt, murderer, murderer, NULL, killEnt->client->ps.origin,
                 99999, DAMAGE_NO_PROTECTION, MOD_BLASTER);
      }
      if (self->m_pVehicle
              ->m_pVehicleInfo) { // FIXME: this wile got stuck in an endless
                                  // loop, that's BAD!!  This method SUCKS (not
                                  // initting "i", not incrementing it or using
                                  // it directly, all sorts of badness), so I'm
                                  // rewriting it
        // while (i < self->m_pVehicle->m_iNumPassengers)
        int numPass = self->m_pVehicle->m_iNumPassengers;
        for (i = 0; i < numPass && self->m_pVehicle->m_iNumPassengers;
             i++) { // go through and eject the last passenger
          killEnt =
              (gentity_t *)self->m_pVehicle
                  ->m_ppPassengers[self->m_pVehicle->m_iNumPassengers - 1];
          if (killEnt) {
            self->m_pVehicle->m_pVehicleInfo->Eject(
                self->m_pVehicle, (bgEntity_t *)killEnt, qtrue);
            if (killEnt->inuse && killEnt->client) {
              G_Damage(killEnt, murderer, murderer, NULL,
                       killEnt->client->ps.origin, 99999, DAMAGE_NO_PROTECTION,
                       MOD_BLASTER);
            }
          }
        }
  ...
  self->client->ps.saberEntityNum =
      self->client->saberStoredIndex; // in case we died while our saber was
                                      // knocked away.
  ...
  if (self->client->NPC_class != CLASS_VEHICLE &&
    ...
    if (veh->inuse && veh->client && veh->m_pVehicle) {
      veh->m_pVehicle->m_pVehicleInfo->Eject(veh->m_pVehicle,
                                             (bgEntity_t *)self, qtrue);

      if (veh->m_pVehicle->m_pVehicleInfo->type ==
          VH_FIGHTER) { // go into "die in ship" mode with flag
        self->client->ps.eFlags2 |= EF2_SHIP_DEATH;
...
extern void RunEmplacedWeapon( gentity_t *ent, usercmd_t **ucmd );
            ...
            if ( self->owner )
            {
                    self->owner->s.frame = self->owner->startFrame =
self->owner->endFrame = 0; self->owner->svFlags &= ~SVF_ANIMATING;
            }
  ...
  // Cheap method until/if I decide to put fancier stuff in (e.g. sabers falling
  // out of hand and slowly holstering on death like sp)
  ...
  if (g_slowmoDuelEnd.integer &&
      ...
      attacker && attacker->inuse && attacker->client) {
    if (!gDoSlowMoDuel) {
      gDoSlowMoDuel = qtrue;
  ...
  /*
  else if (self->NPC && attacker && attacker->client && attacker->s.number <
  MAX_CLIENTS && !gDoSlowMoDuel)
  {
          gDoSlowMoDuel = qtrue;
          gSlowMoDuelTime = level.time;
  }
  */
  ...
  if (inflictor && inflictor->activator && !inflictor->client &&
      !attacker->client && inflictor->activator->client &&
      inflictor->activator->inuse && inflictor->s.weapon == WP_TURRET) {
  ...
  Com_sprintf(buf, sizeof(buf), "Kill: %i %i %i: %s killed ", killer,
              self->s.number, meansOfDeath, killerName);
  ...
  if (g_austrian.integer && level.gametype == GT_DUEL &&
      level.numPlayingClients >= 2) {
    int spawnTime = (level.clients[level.sortedClients[0]].respawnTime >
  ...
  if (attacker && attacker->client) {
    ...
    if (attacker == self || OnSameTeam(self, attacker)) {
      if (level.gametype ==
          GT_DUEL) { // in duel, if you kill yourself, the person you are
                     // dueling against gets a kill for it
        int otherClNum = -1;
      ...
      if (level.gametype == GT_JEDIMASTER) {
        if (self->client &&
            self->client->ps
                .isJediMaster) { // killed ourself so return the saber to the
                                 // original position (to avoid people jumping
                                 //off ledges and making the saber unreachable
                                 // for 60 seconds)
    ...
    if (self->client &&
        self->client->ps.isJediMaster) { // killed ourself so return the saber
                                         // to the original position (to avoid
                                         //people jumping off ledges and making
                                         //the saber unreachable for 60 seconds)
      ThrowSaberToAttacker(self, NULL);
      self->client->ps.isJediMaster = qfalse;
    }

    if (level.gametype ==
        GT_DUEL) { // in duel, if you kill yourself, the person you are dueling
                   // against gets a kill for it
      int otherClNum = -1;
      if (level.sortedClients[0] == self->s.number) {
        otherClNum = level.sortedClients[1];
      } else if (level.sortedClients[1] == self->s.number) {
  ...
  // rww - 07/19/02 - I removed this because it isn't working and it's ugly (for
  ...
  */
  {
    ...
    if (anim >= 1) { // Some droids don't have death anims
      ...
      self->client->respawnTime =
          level.time + 1000; //((self->client->animations[anim].numFrames*40)/(50.0f
                             /// self->client->animations[anim].frameLerp))+300;

      sPMType = self->client->ps.pm_type;
      self->client->ps.pm_type =
          PM_NORMAL; // don't want pm type interfering with our setanim calls.

      if (self->inuse) { // not disconnecting
      ...
      if (meansOfDeath == MOD_SABER ||
          (meansOfDeath == MOD_MELEE &&
           G_HeavyMelee(attacker))) // saber or heavy melee (claws)
      { // update the anim on the actual skeleton (so bolt point will reflect
        // the correct position) and then check for dismem
        G_UpdateClientAnims(self, 1.0f);
        G_CheckForDismemberment(self, attacker, self->pos1, damage, anim,
                                qfalse);
      }
    } else if (self->NPC && self->client &&
               self->client->NPC_class != CLASS_MARK1 &&
               self->client->NPC_class !=
                   CLASS_VEHICLE) { // in this case if we're an NPC it's my
                                    // guess that we want to get removed straight
                                    // away.
      self->think = G_FreeEntity;
      self->nextthink = level.time;
    }
    ...
    //		self->client->ps.pm_flags |= PMF_UPDATE_ANIM;		// Make sure the
    //pmove sets up the GHOUL2 anims.
    ...
    if (self != attacker) { // don't make NPCs want to murder you on respawn for
                            // killing yourself!
      G_DeathAlert(self, attacker);
    }

    // the body can still be gibbed
    if (!self->NPC) { // don't remove NPCs like this!
      self->die = body_die;
    }
  ...
  if (self->NPC && (self->NPC->scriptFlags & SCF_FFDEATH)) {
    if (G_ActivateBehavior(
            self,
            BSET_FFDEATH)) { // FIXME: should running this preclude running the
                             // normal deathscript? deathScript = qtrue;
    }
  ...
  if ( !deathScript && !(self->svFlags&SVF_KILLED_SELF) )
  {
          ...
          //WARNING!!! DO NOT DO THIS WHILE RUNNING A SCRIPT, ICARUS WILL
  CRASH!!!
  ...
  */
  ...
  if (level.gametype == GT_POWERDUEL && !g_noPDuelCheck) { // powerduel checks
    if (self->client->sess.duelTeam ==
        DUELTEAM_LONE) { // automatically means a win as there is only one
      ...
      for (i = 0; i < MAX_CLIENTS; i++) {
        check = &g_entities[i];
        if (check->inuse && check->client &&
            check->s.number != self->s.number &&
            check->client->pers.connected == CON_CONNECTED &&
            !check->client->iAmALoser &&
            check->client->ps.stats[STAT_HEALTH] > 0 &&
            check->client->sess.sessionTeam != TEAM_SPECTATOR &&
            check->client->sess.duelTeam ==
                DUELTEAM_DOUBLE) { // still an active living paired duelist so
                                   // it's not over yet.
          heLives = qtrue;
          break;
        }
...
int CheckArmor(gentity_t *ent, int damage, int dflags) {
  ...
  if (client->NPC_class == CLASS_VEHICLE && ent->m_pVehicle &&
      ent->client->ps.electrifyTime >
          level.time) { // ion-cannon has disabled this ship's shields, take
                        // damage on hull!
    return 0;
  }
  // armor
  ...
  if (dflags & DAMAGE_HALF_ABSORB) { // Half the damage gets absorbed by the
                                     // shields, rather than 100%
    save = ceil(damage * ARMOR_PROTECTION);
  } else { // All the damage gets absorbed by the shields.
    save = damage;
  }

  // save is the most damage that the armor is elibigle to protect, of course,
  // but it's limited by the total armor.
  if (save >= count)
    save = count;
  ...
  if (dflags & DAMAGE_HALF_ARMOR_REDUCTION) // Armor isn't whittled so easily by
                                            // sniper shots.
  {
...
/*
===================================
rww - beginning of the majority of the dismemberment and location based damage
code.
===================================
*/
...
void G_GetDismemberLoc(gentity_t *self, vec3_t boltPoint,
                       int limbType) { // Just get the general area without
                                       // using server-side ghoul2
  vec3_t fwd, right, up;
...
void G_GetDismemberBolt(gentity_t *self, vec3_t boltPoint, int limbType) {
  ...
  trap->G2API_GetBoltMatrix(self->ghoul2, 0, useBolt, &boltMatrix, properAngles,
  ...
  if (self->client &&
      limbType == G2_MODELPART_RHAND) { // Make some saber hit sparks over the
                                        // severed wrist area
    vec3_t boltAngles;
    gentity_t *te;
...
void LimbThink(gentity_t *ent) {
  ...
  if (ent->genericValue5 <=
      level.time) { // this will be every frame by standard, but we want to
                    // compensate in case sv_fps is not 20.
...
extern qboolean BG_GetRootSurfNameWithVariant(void *ghoul2,
                                              const char *rootSurfName,
                                              char *returnSurfName,
                                              int returnSize);

void G_Dismember(gentity_t *ent, gentity_t *enemy, vec3_t point, int limbType,
                 float limbRollBase, float limbPitchBase, int deathAnim,
                 qboolean postDeath) {
  ...
  if (limbType == G2_MODELPART_HEAD) {
    ...
    BG_GetRootSurfNameWithVariant(ent->ghoul2, "l_arm", limbName,
                                  sizeof(limbName));
    BG_GetRootSurfNameWithVariant(ent->ghoul2, "torso", stubName,
                                  sizeof(stubName));
    Com_sprintf(stubCapName, sizeof(stubCapName), "%s_cap_l_arm", stubName);
  } else if (limbType == G2_MODELPART_RARM) {
    BG_GetRootSurfNameWithVariant(ent->ghoul2, "r_arm", limbName,
                                  sizeof(limbName));
    BG_GetRootSurfNameWithVariant(ent->ghoul2, "torso", stubName,
                                  sizeof(stubName));
    Com_sprintf(stubCapName, sizeof(stubCapName), "%s_cap_r_arm", stubName);
  } else if (limbType == G2_MODELPART_RHAND) {
    BG_GetRootSurfNameWithVariant(ent->ghoul2, "r_hand", limbName,
                                  sizeof(limbName));
    BG_GetRootSurfNameWithVariant(ent->ghoul2, "r_arm", stubName,
                                  sizeof(stubName));
    Com_sprintf(stubCapName, sizeof(stubCapName), "%s_cap_r_hand", stubName);
  } else if (limbType == G2_MODELPART_LLEG) {
    BG_GetRootSurfNameWithVariant(ent->ghoul2, "l_leg", limbName,
                                  sizeof(limbName));
    BG_GetRootSurfNameWithVariant(ent->ghoul2, "hips", stubName,
                                  sizeof(stubName));
    Com_sprintf(stubCapName, sizeof(stubCapName), "%s_cap_l_leg", stubName);
  } else if (limbType == G2_MODELPART_RLEG) {
    BG_GetRootSurfNameWithVariant(ent->ghoul2, "r_leg", limbName,
                                  sizeof(limbName));
    BG_GetRootSurfNameWithVariant(ent->ghoul2, "hips", stubName,
                                  sizeof(stubName));
  ...
  if (ent->ghoul2 && limbName[0] &&
      trap->G2API_GetSurfaceRenderStatus(
          ent->ghoul2, 0,
          limbName)) { // is it already off? If so there's no reason to be doing
                       // it again, so get out of here.
    return;
  }
  ...
  */
  ...
  if (enemy && enemy->client && ent && ent != enemy &&
      ...
      (level.time - enemy->client->lastSaberStorageTime) <
          200) { // The enemy has valid saber positions between this and last
                 // frame. Use them to factor in direction of the limb.
    vec3_t dif;
    float totalDistance;
    const float distScale = 1.2f;

    // scale down the initial velocity first, which is based on the speed of the
    // limb owner. ExPhys object velocity operates on a slightly different scale
    // than Q3-based physics velocity.
    ...
    if (ent->client &&
        (ent->client->ps.torsoTimer > 0 ||
         !BG_InDeathAnim(
             ent->client->ps
                 .torsoAnim))) { // if he's done with his death anim we don't
                                 // actually want the limbs going far
      vec3_t preVel;
  ...
  if (ent->s.eType == ET_NPC && ent->ghoul2 && limbName[0] &&
      stubCapName[0]) { // if it's an npc remove these surfs on the server too.
                        // For players we don't even care cause there's no
                        // further dismemberment after death.
    trap->G2API_SetSurfaceOnOff(ent->ghoul2, limbName, 0x00000100);
    trap->G2API_SetSurfaceOnOff(ent->ghoul2, stubCapName, 0);
  }

  if (level.gametype >= GT_TEAM && ent->s.eType != ET_NPC) { // Team game
...
qboolean G_GetHitLocFromSurfName(gentity_t *ent, const char *surfName,
  ...
  *hitLoc = HL_NONE;
  ...
  if (!Q_strncmp("hips", surfName, 4)) { // FIXME: test properly for legs
    *hitLoc = HL_WAIST;
    if (ent->client != NULL && ent->ghoul2) {
      ...
      if (kneeLBolt >= 0) {
        trap->G2API_GetBoltMatrix(ent->ghoul2, 0, kneeLBolt, &boltMatrix,
                                  angles, ent->r.currentOrigin, actualTime,
                                  NULL, ent->modelScale);
      ...
      if (*hitLoc == HL_WAIST) {
        if (kneeRBolt >= 0) {
          trap->G2API_GetBoltMatrix(ent->ghoul2, 0, kneeRBolt, &boltMatrix,
                                    angles, ent->r.currentOrigin, actualTime,
                                    NULL, ent->modelScale);
          BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, tagOrg);
          if (DistanceSquared(point, tagOrg) < 100) { // actually hit the knee
    ...
    *hitLoc = HL_ARM_RT;
    if (ent->client != NULL && ent->ghoul2) {
      ...
      VectorSet(angles, 0, ent->r.currentAngles[YAW], 0);
      if (handRBolt >= 0) {
        trap->G2API_GetBoltMatrix(ent->ghoul2, 0, handRBolt, &boltMatrix,
                                  angles, ent->r.currentOrigin, actualTime,
                                  NULL, ent->modelScale);
        BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, tagOrg);
    ...
    *hitLoc = HL_ARM_LT;
    if (ent->client != NULL && ent->ghoul2) {
      ...
      VectorSet(angles, 0, ent->r.currentAngles[YAW], 0);
      if (handLBolt >= 0) {
        trap->G2API_GetBoltMatrix(ent->ghoul2, 0, handLBolt, &boltMatrix,
                                  angles, ent->r.currentOrigin, actualTime,
                                  NULL, ent->modelScale);
        BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, tagOrg);
        if (DistanceSquared(point, tagOrg) < 256) { // actually hit the hand
    ...
    *hitLoc = HL_LEG_RT;
    if (ent->client != NULL && ent->ghoul2) {
      ...
      VectorSet(angles, 0, ent->r.currentAngles[YAW], 0);
      if (footRBolt >= 0) {
        trap->G2API_GetBoltMatrix(ent->ghoul2, 0, footRBolt, &boltMatrix,
                                  angles, ent->r.currentOrigin, actualTime,
                                  NULL, ent->modelScale);
        BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, tagOrg);
    ...
    *hitLoc = HL_LEG_LT;
    if (ent->client != NULL && ent->ghoul2) {
      ...
      VectorSet(angles, 0, ent->r.currentAngles[YAW], 0);
      if (footLBolt >= 0) {
        trap->G2API_GetBoltMatrix(ent->ghoul2, 0, footLBolt, &boltMatrix,
                                  angles, ent->r.currentOrigin, actualTime,
                                  NULL, ent->modelScale);
        BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, tagOrg);
        if (DistanceSquared(point, tagOrg) < 100) { // actually hit the foot
  ...
  {
          Com_Printf( "ERROR: surface %s does not belong to any
hitLocation!!!\n", surfName );
  }
#endif //_DEBUG
  */
      ...
      dismember = qtrue;
               ...
               (bladeDir[0] || bladeDir[1] ||
                bladeDir[2])) { // we care about direction (presumably for
                                // dismemberment)
      // if ( g_dismemberProbabilities->value<=0.0f||G_Dismemberable( ent,
      // *hitLoc ) )
      if (1) // Fix me?
      { // either we don't care about probabilties or the probability let us
        // continue
        char *tagName = NULL;
        float aoa = 0.5f;
        ...
        if (tagName) {
          ...
          if (tagBolt != -1) {
            ...
            VectorSet(angles, 0, ent->r.currentAngles[YAW], 0);
            trap->G2API_GetBoltMatrix(ent->ghoul2, 0, tagBolt, &boltMatrix,
                                      angles, ent->r.currentOrigin, actualTime,
                                      NULL, ent->modelScale);
            BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, tagOrg);
...
void G_CheckForDismemberment(gentity_t *ent, gentity_t *enemy, vec3_t point,
  ...
  if (ent->localAnimIndex > 1) {
    ...
    if (ent->client->NPC_class !=
        CLASS_PROTOCOL) { // this is the only non-humanoid allowed to do
                          // dismemberment.
      return;
    }
  }
  ...
  if (gGAvoidDismember != 2) { // this means do the dismemberment regardless of
  ...
  if (gGAvoidDismember == 2) {
    ...
    if (d_saberGhoul2Collision.integer && ent->client &&
      ...
      trap->G2API_GetSurfaceName(ent->ghoul2, ent->client->g2LastSurfaceHit, 0,
                                 hitSurface);

      if (hitSurface[0]) {
        G_GetHitLocFromSurfName(ent, hitSurface, &hitLoc, point, vec3_origin,
                                vec3_origin, MOD_UNKNOWN);
      }
    }
...
void G_LocationBasedDamageModifier(gentity_t *ent, vec3_t point, int mod,
                                   int dflags, int *damage) {
  int hitLoc = -1;
  ...
  if (ent->client &&
      ent->client->NPC_class ==
          CLASS_VEHICLE) { // no location-based damage on vehicles
    return;
  }

  if ((d_saberGhoul2Collision.integer && ent->client &&
       ent->client->g2LastSurfaceTime == level.time &&
       mod == MOD_SABER) || // using ghoul2 collision? Then if the mod is a
                            // saber we should have surface data from the last
                            // hit (unless thrown).
      (d_projectileGhoul2Collision.integer && ent->client &&
       ent->client->g2LastSurfaceTime ==
           level.time)) // It's safe to assume we died from the projectile that
                        // just set our surface index. So, go ahead and use that
                        // as the surf I guess.
  {
    char hitSurface[MAX_QPATH];
...
damage DAMAGE_NO_KNOCKBACK		do not affect velocity, just view angles
        ...
        DAMAGE_HALF_ARMOR_REDUCTION		Any damage that shields incur is
halved
============
*/
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
  ...
  if (targ && targ->damageRedirect) {
    G_Damage(&g_entities[targ->damageRedirectTo], inflictor, attacker, dir,
             point, damage, dflags, mod);
    return;
  }
    ...
    if (targ->client->ps.electrifyTime < level.time) { // electrocution effect
      if (targ->s.eType == ET_NPC && targ->s.NPC_class == CLASS_VEHICLE &&
          ...
          (targ->m_pVehicle->m_pVehicleInfo->type == VH_SPEEDER ||
           targ->m_pVehicle->m_pVehicleInfo->type ==
  ...
  if ((targ->flags & FL_DMG_BY_SABER_ONLY) &&
      mod != MOD_SABER) { // saber-only damage
    return;
  }
  ...
  if ((targ->flags & FL_DMG_BY_HEAVY_WEAP_ONLY)) { // only take damage from
                                                   // explosives and such
    if (mod != MOD_REPEATER_ALT && mod != MOD_ROCKET &&
        mod != MOD_FLECHETTE_ALT_SPLASH && mod != MOD_ROCKET_HOMING &&
        mod != MOD_THERMAL && mod != MOD_THERMAL_SPLASH &&
        mod != MOD_TRIP_MINE_SPLASH && mod != MOD_TIMED_MINE_SPLASH &&
        mod != MOD_DET_PACK_SPLASH && mod != MOD_VEHICLE && mod != MOD_CONC &&
        mod != MOD_CONC_ALT && mod != MOD_SABER && mod != MOD_TURBLAST &&
        mod != MOD_SUICIDE && mod != MOD_FALLING && mod != MOD_CRUSH &&
        mod != MOD_TELEFRAG && mod != MOD_TRIGGER_HURT) {
      if (mod != MOD_MELEE ||
          !G_HeavyMelee(attacker)) { // let classes with heavy melee ability
                                     // damage heavy wpn dmg doors with fists
        return;
      }
    }
  }
  ...
  if (targ->flags & FL_BBRUSH) {
    if (mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT || mod == MOD_BRYAR_PISTOL ||
        mod == MOD_BRYAR_PISTOL_ALT ||
        mod == MOD_MELEE) { // these don't damage bbrushes.. ever
      if (mod != MOD_MELEE ||
          !G_HeavyMelee(attacker)) { // let classes with heavy melee ability
                                     // damage breakable brushes with fists
        return;
      }
    }
  }
  ...
  if (!(dflags & DAMAGE_NO_PROTECTION)) { // rage overridden by no_protection
    if (targ && targ->client &&
        (targ->client->ps.fd.forcePowersActive & (1 << FP_RAGE))) {
      damage *= 0.5;
    }
  }
  ...
  // if genericValue4 == 1 then it's glass or a breakable and those do have
  // health
  ...
  if (!(dflags &
        DAMAGE_NO_HIT_LOC)) { // see if we should modify it by damage location
    if (targ->inuse && (targ->client || targ->s.eType == ET_NPC) &&
        attacker->inuse &&
        (attacker->client ||
         attacker->s.eType ==
             ET_NPC)) { // check for location based damage stuff.
  ...
  if (targ->client && targ->client->NPC_class == CLASS_RANCOR &&
    ...
    if (damage < 10) { // ignore piddly little damage
      damage = 0;
    } else if (damage >= 10) {
      damage = 10;
    }
  ...
  if (knockback && targ->client) {
    ...
    if (mod == MOD_SABER) {
      ...
      if ((dflags & DAMAGE_SABER_KNOCKBACK1) ||
          (dflags & DAMAGE_SABER_KNOCKBACK2)) { // saber does knockback, scale
                                                // it by the right number
      ...
      VectorScale(dir,
                  (g_knockback.value * (float)knockback / mass) *
                      saberKnockbackScale,
                  kvel);
    } else {
    ...
    if (attacker && attacker->client && attacker != targ) {
      float dur = 5000;
      float dur2 = 100;
      if (targ->client && targ->s.eType == ET_NPC &&
    ...
    if (!targ->client->ps.pm_time &&
        (g_saberDmgVelocityScale.integer || mod != MOD_SABER ||
  ...
  } else if (targ->client && targ->s.eType == ET_NPC &&
             targ->s.NPC_class == CLASS_VEHICLE && attacker != targ) {
    targ->client->ps.otherKiller = attacker->s.number;
  ...
  if ((g_jediVmerc.integer || level.gametype == GT_SIEGE) &&
      client) { // less explosive damage for jedi, more saber damage for
                // non-jedi
    if (client->ps.trueJedi ||
        (level.gametype == GT_SIEGE &&
         client->ps.weapon ==
             WP_SABER)) { // if the target is a trueJedi, reduce splash and
                          // explosive damage to 1/2
      switch (mod) {
    ...
    } else if ((client->ps.trueNonJedi || (level.gametype == GT_SIEGE &&
                                           client->ps.weapon != WP_SABER)) &&
               mod == MOD_SABER) { // if the target is a trueNonJedi, take more
                                   // saber damage... combined with the 1.5 in
                                   // the w_saber stuff, this is 6 times damage!
      if (damage < 100) {
        damage *= 4;
        if (damage > 100) {
          damage = 100;
  ...
  if (attacker->client && targ->client && level.gametype == GT_SIEGE &&
      ...
      (bgSiegeClasses[targ->client->siegeClass].classflags &
       (1 << CFL_STRONGAGAINSTPHYSICAL))) { // this class is flagged to take
                                            // less damage from physical attacks.
    // For now I'm just decreasing against any client-based attack, this can be
      ...
      if (OnSameTeam(targ, attacker)) {
        if (!g_friendlyFire.integer) {
          return;
                 ...
                 attacker->activator && targ != attacker->activator &&
                 attacker->activator->inuse &&
                 attacker->activator
                     ->client) { // emplaced guns don't hurt teammates of user
        if (OnSameTeam(targ, attacker->activator)) {
          if (!g_friendlyFire.integer) {
            return;
          }
        }
      } else if (targ->inuse && targ->client && level.gametype >= GT_TEAM &&
                 attacker->s.number >= MAX_CLIENTS && attacker->alliedTeam &&
                 targ->client->sess.sessionTeam == attacker->alliedTeam &&
                 !g_friendlyFire.integer) { // things allied with my team
                                            // should't hurt me.. I guess
        return;
      }
    }

    if (level.gametype == GT_JEDIMASTER && !g_friendlyFire.integer && targ &&
        targ->client && attacker && attacker->client && targ != attacker &&
        !targ->client->ps.isJediMaster && !attacker->client->ps.isJediMaster &&
        G_ThereIsAMaster()) {
      return;
    }

    if (targ->s.number >= MAX_CLIENTS && targ->client && targ->s.shouldtarget &&
        targ->s.teamowner && attacker && attacker->inuse && attacker->client &&
        targ->s.owner >= 0 && targ->s.owner < MAX_CLIENTS) {
      gentity_t *targown = &g_entities[targ->s.owner];

      if (targown && targown->inuse && targown->client &&
          OnSameTeam(targown, attacker)) {
        if (!g_friendlyFire.integer) {
          return;
        }
      }
  ...
  // check for teamnodmg
  // NOTE: non-client objects hitting clients (and clients hitting clients)
  // purposely doesn't obey this teamnodmg (for emplaced guns)
  ...
  if (attacker->client && targ != attacker && targ->health > 0 &&
      targ->s.eType != ET_MISSILE && targ->s.eType != ET_GENERAL && client) {
    if (OnSameTeam(targ, attacker)) {
  ...
  // always give half damage if hurting self... but not in siege.  Heavy weapons
  // need a counter. calculated after knockback, so rocket jumping works
  ...
  if (targ->client) { // update vehicle shields and armor, check for explode
    if (targ->client->NPC_class == CLASS_VEHICLE &&
        targ->m_pVehicle) { // FIXME: should be in its own function in
                            // g_vehicles.c now, too big to be here
      int surface = -1;
      ...
      if (targ->m_pVehicle->m_pVehicleInfo->type ==
          VH_FIGHTER) { // get the last surf that was hit
        if (targ->client && targ->client->g2LastSurfaceTime == level.time) {
          char hitSurface[MAX_QPATH];

          trap->G2API_GetSurfaceName(
              targ->ghoul2, targ->client->g2LastSurfaceHit, 0, hitSurface);
          ...
          if (hitSurface[0]) {
            ...
            if (take && surface > 0) { // hit a certain part of the ship
              ...
              if (deathPoint) {
                if (targ->locationDamage[surface] >=
                    deathPoint) { // this area of the ship is now dead
                  if (G_FlyVehicleDestroySurface(
                          targ, surface)) { // actually took off a surface
      ...
      if (targ->m_pVehicle->m_pVehicleInfo->type != VH_ANIMAL) {
        ...
        {//vehicle all out of armor
                ...
                {//just start the flaming effect and explosion delay, if it's
        not going already... pVeh->m_pVehicleInfo->StartDeathDelay( pVeh, 0 );
                }
        }
        else*/
        if (attacker
            ...
            targ->m_pVehicle->m_LandTrace.fraction >=
                1.0f) { // just took a hit, knock us around
  ...
  if (mod == MOD_DEMP2 ||
      mod == MOD_DEMP2_ALT) { // FIXME: screw with non-animal vehicles, too?
    if (client) {
      if (client->NPC_class == CLASS_VEHICLE && targ->m_pVehicle &&
          targ->m_pVehicle->m_pVehicleInfo &&
          targ->m_pVehicle->m_pVehicleInfo->type ==
              VH_FIGHTER) { // all damage goes into the disruption of shields
                            // and systems
        take = 0;
      } else {
  ...
  if (!(dflags & DAMAGE_NO_PROTECTION)) { // protect overridden by no_protection
    if (take && targ->client &&
      ...
      if (targ->client->ps.fd.forcePower) {
        ...
        } else if (targ->client->ps.fd.forcePowerLevel[FP_PROTECT] ==
                   FORCE_LEVEL_2) {
          famt = 0.5f;
          hamt = 0.60f;
    ...
    */
    {
      ...
      // Send off an event to show a shield shell on the player, pointing in the
      // right direction.
      // evEnt = G_TempEntity(vec3_origin, EV_SHIELD_HIT);
      // rww - er.. what the? This isn't broadcast, why is it being set on
      // vec3_origin?!
      ...
      /*
         ...
         + shieldAbsorbed))
                      {
                              targ->client->ps.powerups[PW_SHIELDHIT] =
         level.time + shieldAbsorbed;
                      }
                      //flicker for as many ms as damage was absorbed (*20)
                      //therefore 10 damage causes 1/5 of a seond of flickering,
         whereas
                      //a full 100 causes 2 seconds (but is reduced to 1.5
         seconds due to the max)
  ...
  if (take) {
    if (targ->client && targ->s.number < MAX_CLIENTS &&
        (mod == MOD_DEMP2 ||
         mod == MOD_DEMP2_ALT)) { // uh.. shock them or something. what the
                                  // hell, I don't know.
      if (targ->client->ps.weaponTime <=
          0) { // yeah, we were supposed to be beta a week ago, I don't feel
               // like
    ...
    if (!(dflags & DAMAGE_NO_PROTECTION)) { // rage overridden by no_protection
      if (targ->client &&
          (targ->client->ps.fd.forcePowersActive & (1 << FP_RAGE)) &&
          (inflictor->client || attacker->client)) {
        take /= (targ->client->ps.fd.forcePowerLevel[FP_RAGE] + 1);
      }
    }
    targ->health = targ->health - take;

    if ((targ->flags & FL_UNDYING)) { // take damage down to 1, but never die
      if (targ->health < 1) {
        targ->health = 1;
      }
    }
    ...
    // We want to go ahead and set gPainHitLoc regardless of if we have a pain
    // func, so we can adjust the location damage too.
    if (targ->client && targ->ghoul2 &&
        targ->client->g2LastSurfaceTime ==
            level.time) { // We updated the hit surface this frame, so it's
                          // valid.
      char hitSurface[MAX_QPATH];

      trap->G2API_GetSurfaceName(targ->ghoul2, targ->client->g2LastSurfaceHit,
                                 0, hitSurface);

      if (hitSurface[0]) {
        G_GetHitLocFromSurfName(targ, hitSurface, &gPainHitLoc, point, dir,
                                vec3_origin, mod);
      } else {
        gPainHitLoc = -1;
      }

      if (gPainHitLoc < HL_MAX && gPainHitLoc >= 0 &&
          targ->locationDamage[gPainHitLoc] < Q3_INFINITE &&
        ...
        if (g_armBreakage.integer && !targ->client->ps.brokenLimbs &&
          ...
          if (targ->locationDamage[HL_ARM_RT] +
                  targ->locationDamage[HL_HAND_RT] >=
              80) {
            G_BreakArm(targ, BROKENLIMB_RARM);
          } else if (targ->locationDamage[HL_ARM_LT] +
                         targ->locationDamage[HL_HAND_LT] >=
                     80) {
    ...
    if (targ->maxHealth) { // if this is non-zero this guy should be updated his
                           // s.health to send to the client
    ...
    if (targ->health <= 0) {
      ...
      } else if (targ->s.eType == ET_NPC) { // g2animent
        VectorCopy(point, targ->pos1);
      }
      ...
      if (targ->s.eType == ET_NPC && targ->client &&
          (targ->s.eFlags & EF_DEAD)) { // an NPC that's already dead. Maybe we
                                        // can cut some more limbs off!
        if ((mod == MOD_SABER ||
             (mod == MOD_MELEE &&
              G_HeavyMelee(attacker))) // saber or heavy melee (claws)
            && take > 2 && !(dflags & DAMAGE_NO_DISMEMBER)) {
          G_CheckForDismemberment(targ, attacker, targ->pos1, take,
                                  targ->client->ps.torsoAnim, qtrue);
        }
      }
      ...
      if (targ->pain) {
        if (targ->s.eType != ET_NPC || mod != MOD_SABER ||
            take > 1) { // don't even notify NPCs of pain if it's just idle
                        // saber damage
          gPainMOD = mod;
          if (point) {
...
qboolean CanDamage(gentity_t *targ, vec3_t origin) {
  ...
  trap->Trace(&tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE,
...
qboolean G_RadiusDamage(vec3_t origin, gentity_t *attacker, float damage,
  ...
  if (missile && !missile->client && missile->s.weapon > WP_NONE &&
          ...
          (missile->r.ownerNum < MAX_CLIENTS ||
  g_entities[missile->r.ownerNum].s.eType == ET_NPC)) { //sounds like it's a
  valid weapon projectile.. is it a valid explosive to create marks from?
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (CanDamage(ent, origin)) {
      ...
      if (attacker && attacker->inuse && attacker->client &&
          ...
          attacker->s.NPC_class == CLASS_VEHICLE && attacker->m_pVehicle &&
          attacker->m_pVehicle->m_pPilot) { // say my pilot did it.
        G_Damage(ent, NULL, (gentity_t *)attacker->m_pVehicle->m_pPilot, dir,
                 origin, (int)points, DAMAGE_RADIUS, mod);
      } else {
      ...
      if (ent && ent->client && roastPeople && missile &&
          !VectorCompare(
              ent->r.currentOrigin,
              missile->r.currentOrigin)) { // the thing calling this function
                                           // can create burn marks on people, so
                                           // create an event to do so
        gentity_t *evEnt = G_TempEntity(ent->r.currentOrigin, EV_GHOUL2_MARK);

        evEnt->s.otherEntityNum =
            ent->s.number; // the entity the mark should be placed on
        evEnt->s.weapon = WP_ROCKET_LAUNCHER; // always say it's rocket so we
                                              // make the right mark

        // Try to place the decal by going from the missile location to the
        // location of the person that was hit
        ...
        // the torso.
        if (missile->r.currentOrigin[2] <
            ent->r.currentOrigin[2]) { // move it up less so the decal is placed
                                       // lower on the model then
          evEnt->s.origin2[2] += 8;
        } else {
