...
static const field_t savefields_gEntity[] = {
    ...
    //	{strFOFS(timers),			F_BOOLPTR},	// handled
    //directly
...
ok	playerState_t	ps;				// communicated by
server to clients
...
using configstrings ok	clientInfo_t	clientInfo; ok	renderInfo_t
renderInfo;
};
*/
...
char *GetStringPtr(int iStrlen, char *psOriginal /*may be NULL*/) {
  if (iStrlen != -1) {
    ...
    // we can't do string recycling with the new g_alloc pool dumping, so just
    // always alloc here...
    //
...
void EnumerateField(const field_t *pField, byte *pbBase) {
  ...
  case F_GCLIENT: {
    // unfortunately, I now need to see if this is a 'real' client (and
    // therefore resolve to an enum), or
    //	whether it's one of Mike G's private clients that needs saving here
    //(thanks Mike...)
    //
    ...
    if (ent->NPC == NULL) {
      ...
      *(intptr_t *)pv = GetGClientNum(*(gclient_t **)pv);
      ...
      //
      *(intptr_t *)pv = -2; // yeuch, but distinguishes it from a valid 0 index,
                            // or -1 for client==NULL
    }
  } break;
...
template <typename T>
static void EnumerateFields(const field_t *pFields, T *src_instance,
                            unsigned int ulChid) {
  strList.clear();

  byte *pbData = reinterpret_cast<byte *>(src_instance);
...
static void EvaluateField(const field_t *pField, byte *pbBase,
  ...
  case F_STRING:
    *(char **)pv = GetStringPtr(
        *(intptr_t *)pv, pbOriginalRefData ? *(char **)pvOriginal : NULL);
    break;
  ...
  case F_BEHAVIORSET: {
    ...
    for (int i = 0; i < NUM_BSETS; i++, p++, pO++) {
      *p =
          GetStringPtr(*(intptr_t *)p, pbOriginalRefData ? *(char **)pO : NULL);
    }
  } break;
...
template <typename T>
static void EvaluateFields(const field_t *pFields, T *pbData,
                           T *pbOriginalRefData, unsigned int ulChid) {
  ojk::SavedGameHelper saved_game(::gi.saved_game);

  if (!saved_game.try_read_chunk(ulChid, *pbData)) {
    ::G_Error(
  ...
  for (i = 0; i < iCount; i++) {
    ...
    *pEntOriginal = *pEnt; // struct copy, so we can refer to original
    ...
    if (pEnt->NPC) // will be qtrue/qfalse
    {
      ...
      EvaluateFields(savefields_gNPC, &tempNPC, pEntOriginal->NPC,
                     INT_ID('G', 'N', 'P', 'C'));
    ...
    if (pEnt->client == (gclient_t *)-2) // one of Mike G's NPC clients?
    {
      ...
      EvaluateFields(savefields_gClient, &tempGClient, pEntOriginal->client,
                     INT_ID('G', 'C', 'L', 'I'));
...
#ifdef JK2_MODE
      // Skip GL2 data size
      saved_game.read_chunk(INT_ID('G', 'L', '2', 'S'));
#endif // JK2_MODE
...
void ReadLevel(qboolean qbAutosave, qboolean qbLoadTransition) {
  if (qbLoadTransition) {
    ...
    EvaluateFields(savefields_gClient, &junkClient, &level.clients[0],
                   INT_ID('G', 'C', 'L', 'I'));
    ...
    if (!qbAutosave) // always load the client unless it's an autosave
    {
      ...
      gclient_t GClient;
      EvaluateFields(savefields_gClient, &GClient, &level.clients[0],
                     INT_ID('G', 'C', 'L', 'I'));
