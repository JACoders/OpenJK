...
extern void G_StartMatrixEffect(gentity_t *ent, qboolean falling = qfalse,
                                int length = 1000);
...
void Jedi_CheckCloak(void) {
  if (NPC && NPC->client && NPC->client->NPC_class == CLASS_SHADOWTROOPER) {
    if (NPC->client->ps.saberActive || NPC->health <= 0 ||
        NPC->client->ps.saberInFlight ||
        (NPC->client->ps.eFlags & EF_FORCE_GRIPPED) ||
        NPC->painDebounceTime >
            level.time) { // can't be cloaked if saber is on, or dead or saber
                          // in flight or taking pain or being gripped
      Jedi_Decloak(NPC);
    } else if (NPC->health > 0 && !NPC->client->ps.saberInFlight &&
               !(NPC->client->ps.eFlags & EF_FORCE_GRIPPED) &&
               NPC->painDebounceTime <
                   level.time) { // still alive, have saber in hand, not taking
                                 // pain and not being gripped
      Jedi_Cloak(NPC);
    }
  }
}
...
static void Jedi_Aggression(gentity_t *self, int change) {
  ...
  if (self->client->playerTeam == TEAM_PLAYER) { // good guys are less
                                                 // aggressive
    upper_threshold = 7;
    lower_threshold = 1;
...
static void Jedi_AggressionErosion(int amt) {
  if (TIMER_Done(NPC, "roamTime")) { // the longer we're not alerted and have no
  ...
  if (NPCInfo->stats.aggression < 4 ||
      (NPCInfo->stats.aggression < 6 &&
       NPC->client->NPC_class == CLASS_DESANN)) { // turn off the saber
    if (NPC->client->ps.saberActive) {
      ...
      if (NPC->client->playerTeam == TEAM_PLAYER) {
        ...
        G_SoundOnEnt(NPC, CHAN_WEAPON,
                     "sound/weapons/saber/enemy_saber_off.wav");
      }
    }
  }
}
...
void NPC_Jedi_RateNewEnemy(gentity_t *self, gentity_t *enemy) {
  ...
  case WP_SABER:
    ...
    break;
  case WP_BLASTER:
    if (DistanceSquared(self->currentOrigin, enemy->currentOrigin) <
        65536) // 256 squared
    {
  ...
  int newAggression = ceil((healthAggression + weaponAggression +
...
static qboolean Jedi_BattleTaunt(void) {
  if (TIMER_Done(NPC, "chatter") && !Q_irand(0, 3) &&
      NPCInfo->blockedSpeechDebounceTime < level.time &&
      jediSpeechDebounceTime[NPC->client->playerTeam] < level.time) {
    int event = -1;
    if (NPC->client->playerTeam == TEAM_PLAYER && NPC->enemy &&
        NPC->enemy->client &&
        NPC->enemy->client->NPC_class ==
            CLASS_JEDI) { // a jedi fighting a jedi - training
      if (NPC->client->NPC_class == CLASS_JEDI &&
          NPCInfo->rank == RANK_COMMANDER) { // only trainer taunts
        event = EV_TAUNT1;
      }
    ...
    if (event != -1) {
      ...
      jediSpeechDebounceTime[NPC->client->playerTeam] =
          NPCInfo->blockedSpeechDebounceTime = level.time + 6000;
      TIMER_Set(NPC, "chatter", Q_irand(5000, 10000));

      if (NPC->enemy && NPC->enemy->NPC && NPC->enemy->s.weapon == WP_SABER &&
          NPC->enemy->client &&
          NPC->enemy->client->NPC_class ==
...
static qboolean Jedi_ClearPathToSpot(vec3_t dest, int impactEntNum) {
  ...
  gi.trace(&trace, NPC->currentOrigin, mins, NPC->maxs, dest, NPC->s.number,
           NPC->clipmask, G2_NOCOLLIDE, 0);
  ...
  if (dest[2] > NPC->currentOrigin[2]) { // going up, check for steps
    drop = STEPSIZE;
  } else { // going down or level, check for moderate drops
    drop = 64;
  }
  for (float i = NPC->maxs[0] * 2; i < dist;
       i += NPC->maxs[0] *
    ...
    gi.trace(&trace, start, mins, NPC->maxs, end, NPC->s.number, NPC->clipmask,
             G2_NOCOLLIDE, 0); // NPC->mins?
...
qboolean NPC_MoveDirClear(int forwardmove, int rightmove, qboolean reset) {
  ...
  if (ucmd.upmove > 0 || NPC->client->ps.forceJumpCharge) { // Going to jump
  ...
  /*
  if ( fabs( AngleDelta( NPC->currentAngles[YAW], NPCInfo->desiredYaw ) ) < 5.0
  )//!ucmd.angles[YAW] )
  ...
  */
  ...
  gi.trace(&trace, NPC->currentOrigin, mins, NPC->maxs, testPos, NPC->s.number,
  ...
  if (trace.fraction <
    ...
    if ((NPC->enemy && trace.entityNum == NPC->enemy->s.number) ||
        (NPCInfo->goalEntity &&
         trace.entityNum == NPCInfo->goalEntity->s
  ...
  if (NPCInfo->goalEntity) {
    if (NPCInfo->goalEntity->currentOrigin[2] <
        NPC->currentOrigin[2]) { // goal is below me, okay to step off at least
                                 // that far plus stepheight
      bottom_max +=
          NPCInfo->goalEntity->currentOrigin[2] - NPC->currentOrigin[2];
    }
  }
  VectorCopy(trace.endpos, testPos);
  testPos[2] += bottom_max;

  gi.trace(&trace, trace.endpos, mins, NPC->maxs, testPos, NPC->s.number,
           NPC->clipmask, G2_NOCOLLIDE, 0);
  ...
  if (trace.fraction < 1.0) { // Not going off a cliff
    ...
    // this doesn't account for slide-movement... gi.Printf( "%d walk off cliff
...
static void Jedi_AdjustSaberAnimLevel(gentity_t *self, int newLevel) {
  ...
  // circumstances
  if (newLevel > self->client->ps.forcePowerLevel[FP_SABER_OFFENSE]) { // cap it
    self->client->ps.saberAnimLevel =
        self->client->ps.forcePowerLevel[FP_SABER_OFFENSE];
...
static void Jedi_CheckDecreaseSaberAnimLevel(void) {
  if (!NPC->client->ps.weaponTime &&
      !(ucmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK))) { // not attacking
    if (TIMER_Done(NPC, "saberLevelDebounce") && !Q_irand(0, 10)) {
      ...
      Jedi_AdjustSaberAnimLevel(
          NPC, Q_irand(FORCE_LEVEL_1, FORCE_LEVEL_3)); // random
...
static void Jedi_CombatDistance(
  ...
  if (NPC->client->ps.forcePowersActive & (1 << FP_GRIP) &&
      NPC->client->ps.forcePowerLevel[FP_GRIP] >
  ...
  if (NPC->client->ps.saberInFlight &&
    ...
    if (NPC->client->ps.weapon == WP_SABER                 // using saber
        ...
        && NPC->client->ps.forcePowerLevel[FP_SABERTHROW] >
    ...
    if (enemy_dist <= 64) { // he's getting too close
      ...
      TIMER_Set(NPC, "taunting", -level.time);
               ...
               NPC->client->ps.torsoAnimTimer <
                   2000) { // we're almost done with our special taunt
      // FIXME: this doesn't always work, for some reason
      if (!NPC->client->ps.saberInFlight) {
        NPC->client->ps.saberActive = qtrue;
      }
    }
  } else if (NPC->client->ps.saberEventFlags &
    ...
    if (NPC->enemy->painDebounceTime + 2000 <
    ...
    if (enemy_dist <
        forcePushPullRadius[testlevel] - 16) { // close enough to push
      if (InFront(NPC->enemy->currentOrigin, NPC->client->renderInfo.eyePoint,
                  NPC->client->renderInfo.eyeAngles, 0.6f)) { // knock it down
        ...
        ForceThrow(NPC, qfalse);
             ...
             (NPCInfo->scriptFlags &
              SCF_DONT_FIRE)) { // can't use saber and they're in striking range
    if (!Q_irand(0, 5) && InFront(NPC->enemy->currentOrigin, NPC->currentOrigin,
    ...
    if (enemy_dist >
      ...
      if (!Q_irand(0, 10) && NPCInfo->blockedSpeechDebounceTime < level.time &&
          jediSpeechDebounceTime[NPC->client->playerTeam] < level.time) {
  ...
  /*
  else if ( enemy_dist < 96 && NPC->enemy && NPC->enemy->client &&
  ...
  */
  ...
  { // we're out of striking range and we are allowed to attack
    ...
    if (NPC->enemy && NPC->enemy->client &&
        (NPC->enemy->client->ps.eFlags &
         EF_FORCE_GRIPPED)) { // They're being gripped, rush them!
      if (NPC->enemy->client->ps.groundEntityNum !=
          ENTITYNUM_NONE) { // they're on the ground, so advance
        if (TIMER_Done(NPC, "parryTime") ||
            NPCInfo->rank > RANK_LT) { // not parrying
          if (enemy_dist > 200 ||
              !(NPCInfo->scriptFlags &
      ...
      if (NPCInfo->rank >= RANK_LT_JG && !Q_irand(0, 5) &&
          !(NPC->client->ps.forcePowersActive & (1 << FP_SPEED)) &&
          !(NPC->client->ps.saberEventFlags &
      ...
      {                   // throw saber
        ucmd.buttons |= BUTTON_ALT_ATTACK;
               ...
               NPC->enemy->client->ps.saberActive && // enemy throwing saber
               ...
               !Q_irand(0, 10) && // don't do it all the time, averages to 1
                                  // check a second
               Q_irand(0, 6) <
                   g_spskill->integer && // more likely on harder diff
               Q_irand(RANK_CIVILIAN, RANK_CAPTAIN) <
                   NPCInfo->rank) // more likely against harder enemies
    {                             // They're throwing their saber, grip them!
      ...
      if (NPC->enemy && NPC->enemy->client &&
          (NPC->enemy->client->ps.forcePowersActive &
      ...
      if (chanceScale &&
        ...
        if ((NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) &&
          ...
          } else if (WP_ForcePowerAvailable(NPC, FP_LIGHTNING, 0) &&
                     Q_irand(0, 1)) {
            ForceLightning(NPC);
      ...
      // see if we should advance now
        ...
        if (TIMER_Done(NPC, "parryTime") ||
            NPCInfo->rank > RANK_LT) { // not parrying
          if (!NPC->enemy->client ||
              NPC->enemy->client->ps.groundEntityNum !=
                  ENTITYNUM_NONE) { // they're on the ground, so advance
            if (enemy_dist > 200 ||
                !(NPCInfo->scriptFlags &
                  SCF_DONT_FIRE)) { // far away or allowed to use saber
...
qboolean Jedi_DodgeEvasion(gentity_t *self, gentity_t *shooter, trace_t *tr,
  ...
  if (self->client->ps.groundEntityNum ==
      ENTITYNUM_NONE) { // can't dodge in mid-air
    return qfalse;
  }

  if (self->client->ps.pm_time &&
      (self->client->ps.pm_flags &
       PMF_TIME_KNOCKBACK)) { // in some effect that stops me from moving on my
                              // own
    return qfalse;
  }

  if (self->enemy == shooter) { // FIXME: make it so that we are better able to
                                // dodge shots from my current enemy
  }
  if (self->s.number) { // if an NPC, check game skill setting
    /*
    if ( self->client->NPC_class != CLASS_DESANN &&
    ...
    {//those NPCs are "bosses" and always succeed
            ...
            //FIXME: check my overall skill (rank) to determine if I should be
    able to dodge it?
            //check force speed power level to determine if I should be able to
    dodge it if ( Q_irand( 0, 3 ) > self->client->ps.forcePowerLevel[FP_SPEED] )
            {//more likely to fail on lower force speed level, but NPCs are
    generally better at it than the player return qfalse;
            }
    }
    */
  } else { // the player
    if (!WP_ForcePowerUsable(
            self, FP_SPEED,
            0)) { // make sure we have it and have enough force power
      return qfalse;
    }
    // check force speed power level to determine if I should be able to dodge
    // it
    if (Q_irand(1, 10) >
        self->client->ps.forcePowerLevel[FP_SPEED]) { // more likely to fail on
                                                      // lower force speed level
      return qfalse;
    }
  }
    ...
    if (tr) {
      for (int z = 0; z < MAX_G2_COLLISIONS; z++) {
        if (tr->G2CollisionMap[z].mEntityNum ==
            -1) { // actually, completely break out of this for loop since
                  // nothing after this in the aray should ever be valid either
          continue; // break;//
        }

        CCollisionRecord &coll = tr->G2CollisionMap[z];
        G_GetHitLocFromSurfName(
            &g_entities[coll.mEntityNum],
            gi.G2API_GetSurfaceName(
                &g_entities[coll.mEntityNum].ghoul2[coll.mModelIndex],
                coll.mSurfaceIndex),
            &hitLoc, coll.mCollisionPosition, NULL, NULL, MOD_UNKNOWN);
        // only want the first
        break;
      }
    }
  ...
  case HL_LEG_LT:
  case HL_WAIST:
    if (!self->s.number) { // don't force the player to jump
      return qfalse;
    } else {
      if (!self->enemy) {
  ...
  if (dodgeAnim != -1) {
    ...
    */
    ...
    NPC_SetAnim(self, SETANIM_BOTH, dodgeAnim,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD); // type
...
evasionType_t Jedi_CheckFlipEvasions(gentity_t *self, float rightdot,
  ...
  if (self->client->ps.legsAnim == BOTH_WALL_RUN_LEFT ||
    ...
    float animLength = PM_AnimLength(self->client->clientInfo.animFileIndex,
    ...
    if (anim != -1) { // flip off the wall!
      ...
      return EVASION_OTHER;
             ...
             Q_irand(0, 1) && !PM_InRoll(&self->client->ps) &&
             !PM_InKnockDown(&self->client->ps) &&
             !PM_SaberInSpecialAttack(self->client->ps.torsoAnim)) {
    vec3_t fwd, right, traceto,
        mins = {self->mins[0], self->mins[1], 0},
        maxs = {self->maxs[0], self->maxs[1], 24},
        fwdAngles = {0, self->client->ps.viewangles[YAW], 0};
    trace_t trace;
    ...
    if (trace.fraction >= 1.0f) { // it's clear, let's do it
      ...
      self->client->ps.weaponTime =
          self->client->ps
              .legsAnimTimer; // don't attack again until this anim is done
      ...
      self->client->ps.forceJumpCharge =
          0; // so we don't play the force flip anim
      self->client->ps.velocity[2] = 200;
      self->client->ps.forceJumpZStart =
          self->currentOrigin[2]; // so we don't take damage if we land at same
                                  // height
      ...
      if ((trace.entityNum < ENTITYNUM_WORLD && traceEnt &&
        ...
        if (DotProduct(self->client->ps.velocity, fwd) <
          ...
          if ((trace.fraction * checkDist) <=
            ...
            VectorMA(self->currentOrigin, checkDist, right, traceto);
            // trace in the dir that we want to go
            gi.trace(&trace, self->currentOrigin, mins, maxs, traceto,
                     self->s.number,
                     CONTENTS_SOLID | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP,
                     G2_NOCOLLIDE, 0);
            if (trace.fraction >= 1.0f) { // it's clear, let's do it
              ...
              self->client->ps.forceJumpZStart =
                  self->currentOrigin[2]; // so we don't take damage if we land
                                          // at same height
              self->client->ps.pm_flags |= (PMF_JUMPING | PMF_SLOW_MO_FALL);
              G_SoundOnEnt(self, CHAN_BODY, "sound/weapons/force/jump.wav");
              return EVASION_OTHER;
        ...
        if (bestCheckDist) { // one of the walls was close enough to wall-run on
          ...
          NPC_SetAnim(self, parts, anim,
                      SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
          self->client->ps.forceJumpZStart =
              self->currentOrigin[2]; // so we don't take damage if we land at
                                      // same height
          self->client->ps.pm_flags |= (PMF_JUMPING | PMF_SLOW_MO_FALL);
          G_SoundOnEnt(self, CHAN_BODY, "sound/weapons/force/jump.wav");
          return EVASION_OTHER;
    ...
    if (!g_saberRealisticCombat->integer &&
      ...
      if (evasionType == EVASION_DODGE) {
        ...
        if (self->client->NPC_class == CLASS_TAVION) { // Tavion is faster
          ...
          if (Q_irand(0, 2)) { // medium speed parry
            baseTime = baseTime;
          } else { // with the occasional fast parry
            baseTime = ceil(baseTime / 2.0f);
          }
...
qboolean Jedi_QuickReactions(gentity_t *self) {
  if ((self->client->NPC_class == CLASS_JEDI &&
      ...
      self->client->NPC_class == CLASS_TAVION ||
      (self->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_1 &&
       g_spskill->integer > 1) ||
      (self->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_2 &&
       g_spskill->integer > 0)) {
    return qtrue;
  }
  return qfalse;
...
evasionType_t Jedi_SaberBlockGo(gentity_t *self, usercmd_t *cmd, vec3_t pHitloc,
  ...
  if ((dist > 16 && (Q_irand(0, 2) || saberBusy)) ||
      ...
      !self->client->ps
           .saberActive) { // either it will miss by a bit (and 25% chance) OR
  ...
  if (d_JediAI->integer) {
    gi.Printf("(%d) evading attack from height %4.2f, zdiff: %4.2f, rightdot: "
              "%4.2f\n",
  ...
  {                              // jump!
    if (self->client->ps.groundEntityNum ==
      ...
      if (self->NPC &&
        ...
        if (self->client->NPC_class == CLASS_TAVION && !incoming &&
          ...
          if (!PM_SaberInAttack(self->client->ps.saberMove) &&
            ...
            self->client->ps.forceJumpZStart =
                self->currentOrigin[2]; // so we don't take damage if we land at
                                        // same height
            self->client->ps.pm_flags |= PMF_JUMPING | PMF_SLOW_MO_FALL;
            self->client->saberTrail.inAction =
                qtrue; // FIXME: reset this when done!
            self->client->saberTrail.duration =
      ...
      if (((evasionType = Jedi_CheckFlipEvasions(self, rightdot, zdiff)) !=
           EVASION_NONE)) {
        if (d_slowmodeath->integer > 5 && self->enemy &&
            !self->enemy->s.number) {
...
static qboolean Jedi_SaberBlock(void) {
  ...
  */
  ...
  if (!TIMER_Done(NPC, "parryReCalcTime")) { // can't do our own re-think of
                                             // which parry to use yet
    return qfalse;
  }

  if (NPC->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] >
  ...
  {//lower rank reborn have a random chance of not doing it at all
          NPC->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] = level.time +
  300; return qfalse;
  }
  */

  if (NPC->enemy->health <= 0 ||
      !NPC->enemy->client) { // don't keep blocking him once he's dead (or if
                             // not a client)
    return qfalse;
  }
  /*
  ...
  top, saberPoint, axisPoint ); if ( 0 )//dist > NPC->maxs[0]*4 )//was *3
  ...
  trace_t	tr;
  gi.trace( &tr, saberPoint, vec3_origin, vec3_origin, axisPoint,
  ...
  */
  vec3_t pointDir, baseDir, tipDir, saberHitPoint, saberMins = {-4, -4, -4},
                                                   saberMaxs = {4, 4, 4};
  float pointDist, baseDirPerc;
  VectorMA(NPC->enemy->client->renderInfo.muzzlePointOld,
           NPC->enemy->client->ps.saberLength,
           NPC->enemy->client->renderInfo.muzzleDirOld, saberTipOld);
  VectorMA(NPC->enemy->client->renderInfo.muzzlePoint,
           NPC->enemy->client->ps.saberLength,
           NPC->enemy->client->renderInfo.muzzleDir, saberTip);
  VectorCopy(NPC->currentOrigin, top);
  top[2] = NPC->absmax[2];
  VectorCopy(NPC->currentOrigin, bottom);
  bottom[2] = NPC->absmin[2];

  float dist = ShortestLineSegBewteen2LineSegs(
      NPC->enemy->client->renderInfo.muzzlePoint, saberTip, bottom, top,
  ...
  { // FIXME: sometimes he reacts when you're too far away to actually hit him
    ...
    PM_SaberInAttack( NPC->enemy->client->ps.saberMove )) )//enemy is swinging
    at me
  ...
  VectorSubtract(saberPoint, NPC->enemy->client->renderInfo.muzzlePoint,
                 pointDir);
  ...
  VectorSubtract(NPC->enemy->client->renderInfo.muzzlePoint,
                 NPC->enemy->client->renderInfo.muzzlePointOld, baseDir);
  ...
  // get the actual point of impact
  trace_t tr;
  gi.trace(&tr, saberPoint, saberMins, saberMaxs, hitloc, NPC->enemy->s.number,
           CONTENTS_BODY, G2_NOCOLLIDE, 0); //, G2_RETURNONHIT, 10 );
  ...
  if (d_JediAI->integer) {
    G_DebugLine(saberPoint, hitloc, FRAMETIME,
                WPDEBUG_SaberColor(NPC->enemy->client->ps.saberColor), qtrue);
  }
  ...
  if ((evasionType = Jedi_SaberBlockGo(NPC, &ucmd, hitloc, dir, NULL, dist)) !=
...
static void Jedi_EvasionSaber(vec3_t enemy_movedir, float enemy_dist,
  ...
  if (!NPC->enemy->client) {
    return;
             ...
             NPC->enemy->client->ps.saberLockTime >
  ...
  if (NPC->enemy->client->ps.weaponTime &&
      NPC->enemy->client->ps.weaponstate ==
          WEAPON_FIRING) { // enemy is attacking
    enemy_attacking = qtrue;
    evasionChance = 90;
  }

  if ((NPC->enemy->client->ps.forcePowersActive &
  ...
  if (NPC->enemy->client->ps.saberInFlight &&
      ...
      NPC->enemy->client->ps.saberEntityState !=
  ...
  if (Q_irand(0, 100) < evasionChance) { // check to see if he's coming at me
    ...
    if (Q_flrand(0.25, 1) < facingAmt) { // coming at/facing me!
      int whichDefense = 0;
      if (NPC->client->ps.weaponTime ||
          NPC->client->ps
              .saberInFlight) { // I'm attacking or recovering from a parry, can
      ...
      if (whichDefense >= 4 && whichDefense <= 12) { // would try to block
        if (NPC->client->ps.saberInFlight) { // can't, saber in not in hand, so
                                             // fall back to strafe/jump
          whichDefense = 100;
        }
      }
        ...
        if (!Q_irand(0, 5) ||
          ...
          if (shooting_lightning || throwing_saber || enemy_dist < 80) {
            ...
            if (shooting_lightning ||
                (!Q_irand(0, 2) && NPCInfo->stats.aggression < 4 &&
...
gentity_t *Jedi_FindEnemyInCone(gentity_t *self, gentity_t *fallback,
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (!gi.inPVS(check->currentOrigin,
                  self->currentOrigin)) { // can't potentially see them
      continue;
    }
    ...
    gi.trace(&tr, self->currentOrigin, vec3_origin, vec3_origin,
             check->currentOrigin, self->s.number, MASK_SHOT, G2_NOCOLLIDE, 0);
    if (tr.fraction < 1.0f &&
...
static void Jedi_SetEnemyInfo(vec3_t enemy_dest, vec3_t enemy_dir,
  ...
  if (!NPC->enemy->client) {
    ...
    *enemy_movespeed = 0;
    ...
    enemy_dest[2] +=
        NPC->enemy->mins[2] + 24; // get it's origin to a height I can work with
...
static void Jedi_DebounceDirectionChanges(void) {
  ...
  if (ucmd.forwardmove > 0) {
    ...
    ucmd.forwardmove = 127;
    VectorClear(NPC->client->ps.moveDir);
  } else if (!TIMER_Done(NPC, "moveback")) { // NOTE: edge checking should stop
                                             // me if this is bad...
    ucmd.forwardmove = -127;
    VectorClear(NPC->client->ps.moveDir);
  }
  ...
  } else if (!TIMER_Done(NPC, "moveright")) { // NOTE: edge checking should stop
                                              // me if this is bad...
    ucmd.rightmove = 127;
    VectorClear(NPC->client->ps.moveDir);
  } else if (!TIMER_Done(NPC, "moveleft")) { // NOTE: edge checking should stop
                                             // me if this is bad...
    ucmd.rightmove = -127;
    VectorClear(NPC->client->ps.moveDir);
  }
...
static void Jedi_TimersApply(void) {
  ...
  if (!TIMER_Done(NPC, "gripping")) { // FIXME: what do we do if we ran out of
                                      // power?  NPC's can't?
...
static void Jedi_CombatTimersUpdate(int enemy_dist) {
  if (TIMER_Done(NPC, "roamTime")) {
    ...
    if (NPC->enemy && NPC->enemy->client) {
      ...
      case WP_SABER:
        ...
        if (!NPC->enemy->client->ps
                 .saberActive) { // fool!  Standing around unarmed, charge!
      ...
      case WP_ROCKET_LAUNCHER:
        ...
        if (NPC->enemy->attackDebounceTime <
  ...
  if (TIMER_Done(NPC, "noStrafe") && TIMER_Done(NPC, "strafeLeft") &&
      TIMER_Done(NPC, "strafeRight")) {
  ...
  if (NPC->client->ps.saberEventFlags) { // some kind of saber combat event is
                                         // still pending
    ...
    if (NPC->client->ps.saberEventFlags & SEF_PARRIED) { // parried
      ...
      */
      if (NPC->enemy &&
          (!NPC->enemy->client ||
           PM_SaberInKnockaway(NPC->enemy->client->ps.saberMove))) { // advance!
        Jedi_Aggression(NPC, 1); // get closer
        Jedi_AdjustSaberAnimLevel(
            NPC, (NPC->client->ps.saberAnimLevel - 1)); // use a faster attack
      } else {
      ...
      if (d_JediAI->integer) {
        gi.Printf("(%d) PARRY: agg %d, no parry until %d\n", level.time,
                  NPCInfo->stats.aggression, level.time + 100);
      }
      newFlags &= ~SEF_PARRIED;
    }
    if (!NPC->client->ps.weaponTime &&
        (NPC->client->ps.saberEventFlags & SEF_HITENEMY)) // hit enemy
    { // we hit our enemy last time we swung, drop our aggression
      if (!Q_irand(0, 1)) // FIXME: dependant on rank/diff?
      {
        ...
        if (!Q_irand(0, 3) && NPCInfo->blockedSpeechDebounceTime < level.time &&
            jediSpeechDebounceTime[NPC->client->playerTeam] < level.time &&
    ...
    if ((NPC->client->ps.saberEventFlags &
      ...
      if (PM_SaberInBrokenParry(NPC->client->ps.saberMove) ||
        ...
        Jedi_AdjustSaberAnimLevel(
            NPC, (NPC->client->ps.saberAnimLevel + 1)); // use a stronger attack
        if (d_JediAI->integer) {
...
static void Jedi_CombatIdle(int enemy_dist) {
  ...
  if (enemy_dist >= 64) { // FIXME: only do this if standing still?
    ...
    if (Q_irand(2, chance) < NPCInfo->stats.aggression) {
      if (TIMER_Done(NPC, "chatter")) { // FIXME: add more taunt behaviors
        // FIXME: sometimes he turns it off, then turns it right back on
        // again???
        if (enemy_dist > 200 && NPC->client->ps.saberActive &&
...
static qboolean Jedi_AttackDecide(int enemy_dist) {
  if (NPC->enemy->client && NPC->enemy->s.weapon == WP_SABER &&
      NPC->enemy->client->ps.saberLockTime > level.time &&
      NPC->client->ps.saberLockTime <
          level.time) { // enemy is in a saberLock and we are not
    return qfalse;
  ...
  if (NPC->client->ps.saberEventFlags &
    ...
    if (Q_irand(0, 30) < chance) { // based on skill with some randomness
      ...
      NPC->client->ps.weaponTime = NPCInfo->shotTime = NPC->attackDebounceTime =
          0;
      // NPC->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] = level.time +
      // 500;
      NPC->client->ps.saberBlocked = BLOCKED_NONE;
  ...
  if (NPC->client->NPC_class == CLASS_TAVION ||
    ...
    if ((PM_SaberInParry(NPC->client->ps.saberMove) ||
      ...
      NPC->client->ps.saberBlocked = BLOCKED_NONE;
      Jedi_AdjustSaberAnimLevel(
          NPC, FORCE_LEVEL_1); // try to follow-up with a quick attack
      WeaponThink(qtrue);
      return qtrue;
    }
  }
  ...
  if (!(ucmd.buttons & BUTTON_ATTACK) &&
      !(ucmd.buttons & BUTTON_ALT_ATTACK)) { // not already attacking
    // Try to attack
    WeaponThink(qtrue);
  }
    ...
    */
    ...
    if (!ucmd.rightmove) {  // not already strafing
      if (!Q_irand(0, 3)) { // 25% chance of doing this
        ...
        VectorSubtract(NPC->enemy->currentOrigin, NPC->currentAngles,
                       dir2enemy);
...
static qboolean Jedi_Jump(vec3_t dest,
  ...
  /*
  if ( dest[2] - NPC->currentOrigin[2] < 64 && DistanceHorizontal(
  NPC->currentOrigin, dest ) > 256 )
  ...
  */
  if (1) {
      ...
      {      // do a rough trace of the path
        ...
        for (elapsedTime = timeStep; elapsedTime < floor(travelTime) + timeStep;
          ...
          if (testPos[2] < lastPos[2]) { // going down, ignore botclip
            gi.trace(&trace, lastPos, NPC->mins, NPC->maxs, testPos,
                     NPC->s.number, NPC->clipmask, G2_NOCOLLIDE, 0);
          } else { // going up, check for botclip
            gi.trace(&trace, lastPos, NPC->mins, NPC->maxs, testPos,
                     NPC->s.number, NPC->clipmask | CONTENTS_BOTCLIP,
                     G2_NOCOLLIDE, 0);
          }
          ...
          if (elapsedTime == floor(travelTime)) { // reached end, all clear
            if (trace.fraction >=
              ...
              bottom[2] -= 128;
              gi.trace(&trace, trace.endpos, NPC->mins, NPC->maxs, bottom,
                       NPC->s.number, NPC->clipmask, G2_NOCOLLIDE, 0);
    ...
    } else if (NPC->currentOrigin[2] <
               dest[2]) // NPCInfo->goalEntity->currentOrigin
    {
    ...
    */
    ...
    //		gi.Printf("apex is %4.2f percent from p1: ",
    //(xy-z)*0.5/xy*100.0f);
    ...
    time = sqrt(height /
                (.5 * NPC->client->ps.gravity)); // was 0.5, but didn't work
  ...
  if (TIMER_Done(NPC, "jumpChaseDebounce")) {
    if ((!goal->client || goal->client->ps.groundEntityNum != ENTITYNUM_NONE)) {
      if (!PM_InKnockDown(&NPC->client->ps) &&
          !PM_InRoll(&NPC->client->ps)) { // enemy is on terra firma
        vec3_t goal_diff;
        float goal_z_diff;
            ...
            if (goal_z_diff > 0 || goal_xy_dist > 128) { // Fake a force-jump
              ...
              if (goal == NPC->enemy) {
                ...
                while (sideTry < 10) { // FIXME: make it so it doesn't try the
                  ...
                  bottom[2] -= 128;
                  gi.trace(&trace, dest, NPC->mins, NPC->maxs, bottom,
                           goal->s.number, NPC->clipmask, G2_NOCOLLIDE, 0);
                ...
                */
                { // FIXME: make this a function call
                  ...
                  if (NPCInfo->rank != RANK_CREWMAN &&
                      NPCInfo->rank <= RANK_LT_JG) { // can't do acrobatics
                    jumpAnim = BOTH_FORCEJUMP1;
                  } else {
                    jumpAnim = BOTH_FLIP_F;
                  }
                  NPC_SetAnim(NPC, SETANIM_BOTH, jumpAnim,
                              SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
                }
...
static qboolean Jedi_Jumping(gentity_t *goal) {
  if (!TIMER_Done(NPC, "forceJumpChasing") &&
    ...
    if (!(NPC->client->ps.pm_flags & PMF_JUMPING) // forceJumpZStart )
      ...
      //Cheat:
      if ( goal_dist < 128 && goal->currentOrigin[2] > NPC->currentOrigin[2] &&
      NPC->client->ps.velocity[2] <= 0 )
      {
              NPC->client->ps.velocity[2] += 320;
      }
...
static void Jedi_CheckEnemyMovement(float enemy_dist) {
  ...
  if (NPC->client->NPC_class != CLASS_TAVION &&
    ...
    if (NPC->enemy->enemy && NPC->enemy->enemy == NPC) { // enemy is mad at *me*
          ...
          if (enemy_dist < 256) { // close
            if (Q_irand(0, NPCInfo->rank) <
              ...
              */
              ...
              TIMER_Set(NPC, "strafeRight", -1);
              TIMER_Set(NPC, "noStrafe", Q_irand(500, 1000));
              TIMER_Set(NPC, "noturn",
                        Q_irand(250, 500) * (3 - g_spskill->integer));
      ...
      } else if (NPC->enemy->client->ps.legsAnim ==
                 BOTH_A2_STABBACK1) {              // he's stabbing backwards
        if (enemy_dist < 256 && enemy_dist > 64) { // close
          if (!InFront(NPC->currentOrigin, NPC->enemy->currentOrigin,
                       NPC->enemy->currentAngles, 0.0f)) { // behind him
            if (!Q_irand(0,
                         NPCInfo->rank)) { // be nice and stand still for him...
              // stop current movement
              ucmd.forwardmove = ucmd.rightmove = ucmd.upmove = 0;
              VectorClear(NPC->client->ps.moveDir);
              NPC->client->ps.forceJumpCharge = 0;
...
static void Jedi_CheckJumps(void) {
  ...
  VectorCopy(trace.endpos, bottom);
  if (bottom[2] > NPC->currentOrigin[2]) { // only care about dist down from
                                           // current height or lower
    bottom[2] = NPC->currentOrigin[2];
  } else if (NPC->currentOrigin[2] - bottom[2] >
  ...
  bottom[2] -= 128;
  gi.trace(&trace, trace.endpos, NPC->mins, NPC->maxs, bottom, NPC->s.number,
           NPC->clipmask, G2_NOCOLLIDE, 0);
  if (trace.allsolid || trace.startsolid ||
...
static void Jedi_Combat(void) {
  ...
  if (Jedi_Jumping(NPC->enemy)) { // I'm in the middle of a jump, so just see if
                                  // I should attack
  ...
  if (!(NPC->client->ps.forcePowersActive & (1 << FP_GRIP)) ||
    ...
    if (!Jedi_ClearPathToSpot(enemy_dest,
                              NPC->enemy->s.number)) { // hunt him down
                                ...
                                //NPCInfo->rank > RANK_LT_JG ) &&
      {
        ...
        gi.trace( &trace, NPC->currentOrigin, NPC->mins, NPC->maxs, end,
        NPC->s.number, NPC->clipmask|CONTENTS_BOTCLIP ); if ( !trace.allsolid &&
        ...
        */
        ...
        if (Jedi_TryJump(NPC->enemy)) { // FIXME: what about jumping to his
      ...
      if (Jedi_Hunt() &&
          !(NPCInfo->aiFlags & NPCAI_BLOCKED)) // FIXME: have to do this because
      ...
      */
             ...
             // there
        if (NPCInfo->aiFlags & NPCAI_BLOCKED) { // try to jump to the
                                                // blockedDest
...
void NPC_Jedi_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
                   vec3_t point, int damage, int mod, int hitLoc) {
  ...
  if (other->s.weapon == WP_SABER) { // back off
    ...
    if (self->client->NPC_class == CLASS_DESANN) { // less for Desann
      self->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] =
          level.time + (3 - g_spskill->integer) * 50;
    } else if (self->NPC->rank >= RANK_LT_JG) {
      self->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] =
          level.time + (3 - g_spskill->integer) * 100; // 300
    } else {
      self->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] =
          level.time + (3 - g_spskill->integer) * 200; // 500
    }
    if (!Q_irand(
            0,
    ...
    if (!level.alertEvents[alertEvent].owner ||
        ...
        (level.alertEvents[alertEvent].owner != NPC &&
         level.alertEvents[alertEvent].owner->client->playerTeam !=
             NPC->client->playerTeam)) { // no owner
      return qfalse;
    }
...
qboolean Jedi_CheckAmbushPlayer(void) {
  ...
  if (NPC->client->ps.powerups[PW_CLOAKED] ||
      g_crosshairEntNum !=
          NPC->s.number) { // if I'm not cloaked and the player's crosshair is
                           // on me, I will wake up, otherwise do this stuff down
                           // here...
    if (!gi.inPVS(player->currentOrigin,
                  NPC->currentOrigin)) { // must be in same room
      return qfalse;
    } else {
      if (!NPC->client->ps.powerups[PW_CLOAKED]) {
    ...
    if ((target_dist = DistanceHorizontalSquared(player->currentOrigin,
                                                 NPC->currentOrigin)) >
  ...
  {                                // look for enemies
    ...
    for (int i = 0; i < ENTITYNUM_WORLD; i++) {
      ...
      if (enemy && enemy->client && NPC_ValidEnemy(enemy) &&
        ...
        if (gi.inPVS(NPC->currentOrigin,
                     enemy->currentOrigin)) { // we could potentially see him
          enemy_dist =
              DistanceSquared(NPC->currentOrigin, enemy->currentOrigin);
          if (enemy->s.number == 0 || enemy_dist < best_enemy_dist) {
            ...
            if (enemy_dist < (220 * 220) || (NPCInfo->investigateCount >= 3 &&
              ...
              break;
                       ...
                       enemy->client->ps
                           .saberActive) { // threw his saber, see if it's
                                           // heading toward me and close enough
                                           // to consider a threat
              float saberDist;
              vec3_t saberDir2Me;
              vec3_t saberMoveDir;
              gentity_t *saber = &g_entities[enemy->client->ps.saberEntityNum];
              VectorSubtract(NPC->currentOrigin, saber->currentOrigin,
                             saberDir2Me);
              saberDist = VectorNormalize(saberDir2Me);
              VectorCopy(saber->s.pos.trDelta, saberMoveDir);
              VectorNormalize(saberMoveDir);
    ...
    if (!NPC->enemy) { // still not mad
      if (!best_enemy) {
            ...
            if (TIMER_Done(NPC,
              ...
              if (TIMER_Get(NPC, "watchTime") ==
                ...
                if (!NPCInfo->investigateCount) {
                  G_AddVoiceEvent(NPC, Q_irand(EV_JDETECTED1, EV_JDETECTED3),
                                  3000);
                }
        ...
        } else if (TIMER_Done(NPC, "watchTime")) { // haven't seen him in a bit,
...
void NPC_BSJedi_FollowLeader(void) {
  ...
  if (NPCInfo->goalEntity) {
    ...
    if (!NAV_CheckAhead(NPC, NPCInfo->goalEntity->currentOrigin, trace,
      ...
      if (NPC_ClearLOS(NPCInfo->goalEntity) &&
          NPC_FaceEntity(NPCInfo->goalEntity, qtrue)) { // no line of sight
...
static void Jedi_Attack(void) {
  ...
  if (NPC->client->ps.saberLockTime > level.time) {
    ...
    if (NPC->client->ps.forcePowerLevel[FP_PUSH] > FORCE_LEVEL_2 &&
        NPC->client->ps.saberLockTime < level.time + 5000 && !Q_irand(0, 10)) {
      ForceThrow(NPC, qfalse);
    }
  ...
  if (NPC->enemy) {
    if (NPC->enemy->health <= 0 && NPC->enemy->enemy == NPC &&
        NPC->client->playerTeam != TEAM_PLAYER) // good guys don't gloat
    {                                      // my enemy is dead and I killed him
      ...
      NPC->client->ps.saberBlocked = BLOCKED_NONE;
      if (NPC->client->ps.saberActive ||
          NPC->client->ps.saberInFlight) { // saber is still on (or we're trying
                                           // to pull it back), count down
                                           // erosion and keep facing the enemy
      ...
      if (NPC->client->ps.saberActive || NPC->client->ps.saberInFlight ||
          !TIMER_Done(NPC, "gloatTime")) { // keep walking
  ...
  if (!(NPCInfo->scriptFlags &
        SCF_CHASE_ENEMIES)) { // this is really stupid, but okay...
    ucmd.forwardmove = 0;
    ucmd.rightmove = 0;
    if (ucmd.upmove > 0) {
      ucmd.upmove = 0;
  ...
  // NOTE: for now, we clear ucmd.forwardmove & ucmd.rightmove while in air to
  // avoid jumps going awry...
  if (NPC->client->ps.groundEntityNum ==
  ...
  if (PM_SaberInBrokenParry(NPC->client->ps.saberMove) ||
      NPC->client->ps.saberBlocked ==
          BLOCKED_PARRY_BROKEN) { // just make sure they don't pull their saber
                                  // to them if they're being blocked
    ucmd.buttons &= ~BUTTON_ATTACK;
  }
       ...
       (NPC->client->NPC_class == CLASS_DESANN ||
        NPCInfo->rank >=
            Q_irand(RANK_CREWMAN,
                    RANK_CAPTAIN)))) { // Tavion will kick in force speed if the
                                       // player does...
    if (NPC->enemy && !NPC->enemy->s.number && NPC->enemy->client &&
...
void NPC_BSJedi_Default(void) {
  ...
  {                                // have an enemy
    if (Jedi_WaitingAmbush(NPC)) { // we were still waiting to drop down - must
                                   // have had enemy set on me outside my AI
      Jedi_Ambush(NPC);
    }
    Jedi_Attack();
    ...
    // switch if needbe...
    if (((!ucmd.buttons && !NPC->client->ps.forcePowersActive) ||
