...
const char *CG_GetGameStatusText(void) {
  ...
  if (cgs.gametype == GT_POWERDUEL) {
    ...
    if (cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR) {
      ...
      s = va("%s %s %i", CG_PlaceString(cg.snap->ps.persistant[PERS_RANK] + 1),
...
static void CG_Text_Paint_Limit(float *maxX, float x, float y, float scale,
  ...
  if (x + iPixelLen > *maxX) {
    ...
    *maxX = 0; // feedback
    CG_Text_Paint(x, y, scale, color, sTemp, adjust, limit,
                  ITEM_TEXTSTYLE_NORMAL, iMenuFont);
  } else {
    // whole text fits fine, so print it all...
    //
    *maxX = x + iPixelLen; // feedback the next position, as the caller expects
    CG_Text_Paint(x, y, scale, color, text, adjust, limit,
                  ITEM_TEXTSTYLE_NORMAL, iMenuFont);
  }
}
  ...
  if (cg.spectatorLen) {
    ...
    if (cg.spectatorPaintX2 >= 0) {
      ...
      CG_Text_Paint_Limit(&maxX2, cg.spectatorPaintX2, rect->y + rect->h - 3,
                          scale, color, cg.spectatorList, 0, cg.spectatorOffset,
                          FONT_MEDIUM);
    }
...
void CG_DrawMedal(int ownerDraw, rectDef_t *rect, float scale, vec4_t color,
  ...
  if (text) {
    ...
    CG_Text_Paint(rect->x + (rect->w - value) / 2, rect->y + rect->h + 10,
                  scale, color, text, 0, 0, 0, FONT_MEDIUM);
  }
  trap->R_SetColor(NULL);
}
...
void CG_OwnerDraw(float x, float y, float w, float h, float text_x,
  ...
  // Ignore all this, at least for now. May put some stat stuff back in menu
  // files later.
#if 0
	rectDef_t rect;
...
void CG_MouseEvent(int x, int y) {
  ...
  cg.predictedPlayerState.pm_type == PM_FLOAT || cg.predictedPlayerState.pm_type
  == PM_SPECTATOR) && cg.showScores == qfalse) { trap->Key_SetCatcher(0);
          return;
  }
  */
