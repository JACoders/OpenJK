...
#if DEFINEHASHING
  src.definehash =
      (struct define_s **)GetClearedMemory(DEFINEHASHSIZE * sizeof(define_t *));
#endif // DEFINEHASHING
  ...
  while (!error && firstoperator) {
    ...
    if (integer) {
      Log_Write("operator %s, value1 = %d",
                PunctuationFromNum(source->scriptstack, o->mOperator),
                v1->intvalue);
      if (v2)
...
int PC_ReadToken(source_t *source, token_t *token) {
  ...
  while (1) {
    ...
    if (token->type == TT_PUNCTUATION &&
        *token->string == '@') // It is a StringEd key
    {
...
#if DEFINEHASHING
  if (!globaldefines) {
    globaldefines = (struct define_s **)GetClearedMemory(DEFINEHASHSIZE *
                                                         sizeof(define_t *));
  }
#endif
