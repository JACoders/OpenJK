...
void OBJ_SetPendingObjectives(gentity_t *ent) {
	...
	for (i = 0; i < MAX_OBJECTIVES; ++i) {
		if ((ent->client->sess.mission_objectives[i].status == OBJECTIVE_STAT_PENDING) && (ent->client->sess.mission_objectives[i].display)) {
...
void OBJ_SaveMissionObjectives(gclient_t *client) {
	...
	saved_game.write_chunk(INT_ID('O', 'B', 'J', 'T'), client->sess.mission_objectives);
}
...
void OBJ_LoadMissionObjectives(gclient_t *client) {
	...
	saved_game.read_chunk(INT_ID('O', 'B', 'J', 'T'), client->sess.mission_objectives);
}
