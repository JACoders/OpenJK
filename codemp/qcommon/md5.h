...
void HMAC_MD5_Init(hmacMD5Context_t *ctx, unsigned char const *key,
                   unsigned int keylen);

// Update the HMAC message with len number of bytes from the given buffer.
void HMAC_MD5_Update(hmacMD5Context_t *ctx, unsigned char const *buf,
                     unsigned int len);

// Finalize the HMAC calculation and fill the given buffer with the digest
// bytes. 'digest' must point to a buffer that can hold MD5_DIGEST_SIZE bytes!
void HMAC_MD5_Final(hmacMD5Context_t *ctx, unsigned char *digest);

// Reset the context to begin working on a new message, using the same secret
// key as previously initialised.
