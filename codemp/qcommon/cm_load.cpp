...
static void CM_LoadMap_Actual(const char *name, qboolean clientload, int *checksum, clipMap_t &cm) { // rwwRMG - function needs heavy modification
	int *buf;
	dheader_t header;
...
cmodel_t *CM_ClipHandleToModel(clipHandle_t handle, clipMap_t **clipMap) {
	...
	if (handle < MAX_SUBMODELS) {
		Com_Error(ERR_DROP, "CM_ClipHandleToModel: bad handle (count: %i) < (handle: %i) < (max: %i)", cmg.numSubModels, handle, MAX_SUBMODELS);
	}
...
int CM_ModelContents_Actual(clipHandle_t model, clipMap_t *cm) {
	...
	for (i = 0; i < cmod->leaf.numLeafSurfaces; i++) {
		...
		if (cm->surfaces[surfaceNum] != NULL) { // HERNH?  How could we have a null surf within our cmod->leaf.numLeafSurfaces?
