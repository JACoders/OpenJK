...
bool CTail::Update() {
  ...
  if (mFlags & FX_RELATIVE) {
    if (mClientID < 0 || mClientID >= ENTITYNUM_WORLD) {
      // the thing we are bolted to is no longer valid, so we may as well just
      // die.
      return false;
    }
    ...
    // math-wise
    VectorMA(realVel, (theFxHelper.mTime - mTimeStart) * 0.001f, realAccel,
             realVel);
    ...
    VectorMA(org, ((theFxHelper.mTime - mTimeStart) - 3) * 0.001f, realVel,
             mOldOrigin);
  } else if ((mTimeStart < theFxHelper.mTime) && UpdateOrigin() == false) {
    // we are marked for death
    return false;
