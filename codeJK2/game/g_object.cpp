...
void G_BounceObject(gentity_t *ent, trace_t *trace) {
  ...
  VectorMA(velocity, -2 * dot * bounceFactor, trace->plane.normal,
           ent->s.pos.trDelta);
  ...
  if (ent->s.eFlags & EF_BOUNCE_HALF) {
    ...
    // check for stop
    if (((trace->plane.normal[2] > 0.7 && g_gravity->value > 0) ||
         (trace->plane.normal[2] < -0.7 && g_gravity->value < 0)) &&
        ((ent->s.pos.trDelta[2] < 40 && g_gravity->value > 0) ||
         (ent->s.pos.trDelta[2] > -40 &&
          g_gravity->value <
              0))) // this can happen even on very slightly sloped walls, so
                   // changed it from > 0 to > 0.7
    {
...
G_RunObject
  ...
  TODO:  When free-floating in air, apply some friction to your trDelta (based
on mass?)
================
*/
...
void G_RunObject(gentity_t *ent) {
  ...
  // FIXME: floaters need to stop floating up after a while, even if gravity
  // stays negative?
  ...
  if (VectorCompare(ent->currentOrigin, origin)) { // error - didn't move at
                                                   // all!
    return;
  }
  ...
  */
  ...
  if (tr.fraction || (traceEnt && traceEnt->takedamage)) {
    if (!VectorCompare(ent->currentOrigin, oldOrg)) { // moved and impacted
  ...
  if (!ent ||
      (ent->takedamage && ent->health <= 0)) { // been destroyed by impact
    // chunks?
  ...
  {                                    // FIXME: only do this if no trDelta
    if (g_gravity->value <= 0 || tr.plane.normal[2] < 0.7) {
      ...
      // FIXME: this means we can't fall if something is pulled out from under
      // us...
...
void G_StartObjectMoving(gentity_t *object, vec3_t dir, float speed,
                         trType_t trType) {
  VectorNormalize(dir);
  ...
  */
  ...
  // FIXME: make these objects go through G_RunObject automatically, like
  // missiles do
...
gentity_t *G_CreateObject(gentity_t *owner, vec3_t origin, vec3_t angles,
  ...
  object->e_ThinkFunc = thinkF_G_RunObject;
  object->s.eType = ET_GENERAL;
  object->s.eFlags |= EF_AUTO_SIZE; // CG_Ents will create the mins & max itself
                                    // based on model bounds
