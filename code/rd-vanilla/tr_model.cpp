...
void RE_LoadWorldMap_Actual(
    const char *name, world_t &worldData,
    int index); // should only be called for sub-bsp instances
...
void *RE_RegisterModels_Malloc(int iSize, void *pvDiskBufferIfJustLoaded,
  ...
  if (ModelBin.pModelDiskImage == NULL) {
    ...
    // fully...
    //
    // new, instead of doing a R_Malloc and assigning that we just morph the
    // disk buffer alloc
    ...
    *pqbAlreadyFound = qfalse;
    ...
    for (int i = 0; i < iEntries; i++) {
      ...
      const char *const psShaderName =
          &((char *)ModelBin.pModelDiskImage)[iShaderNameOffset];
      int *const piShaderPokePtr =
  ...
  if (gbInsideRegisterModel) {
    ...
    for (CachedModels_t::iterator itModel = CachedModels->begin();
      ...
      if (bDeleteEverythingNotUsedThisLevel) {
        bDeleteThis = (qboolean)(CachedModel.iLastLevelUsedOn !=
...
#ifdef _DEBUG
        //				LPCSTR psModelName =
        //(*itModel).first.c_str(); 				ri.Printf( PRINT_DEVELOPER, "Dumping
        //\"%s\"", psModelName); 				ri.Printf( PRINT_DEVELOPER, ", used on lvl
...
//
// param "bAllowScreenDissolve" is just a convenient way of getting hold of a
// bool which can be checked by the code that
...
void RE_InsertModelIntoHash(const char *name, model_t *mod) {
  ...
  mh->next = mhHashTable[hash]; // I have the breakpoint triggered here where
                                // mhHashTable[986] would be assigned
  mh->handle = mod->index;
  strcpy(mh->name, name);
  mhHashTable[hash] = mh;
}
...
static qhandle_t RE_RegisterModel_Actual(const char *name) {
  ...
  */
  //	if (!tr.registered) {
  //		ri.Printf( PRINT_WARNING, "RE_RegisterModel (%s) called before
  //ready!\n",name ); 		return 0;
  //	}
  //
  ...
  */
  ...
  if (name[0] == '#') {
    ...
    RE_LoadWorldMap_Actual(va("maps/%s.bsp", name + 1),
                           tr.bspModels[tr.numBSPModels - 1],
                           tr.numBSPModels); // this calls R_LoadSubmodels which
                                             // will put them into the Hash
#endif
  ...
  for (lod = iLODStart; lod >= 0; lod--) {
    ...
    if (!RE_RegisterModels_GetDiskFile(filename, (void **)&buf,
      ...
      {
        Com_Error(ERR_DROP,
                  "R_LoadMD3: %s has LOD %d but is missing LOD %d ('%s')!",
                  mod->name, lod + 1, lod, filename);
      }
      continue;
    }
...
qhandle_t RE_RegisterModel(const char *name) {
  ...
  if (Q_stricmp(&name[strlen(name) - 4], ".gla")) {
    gbInsideRegisterModel =
        qfalse; // GLA files recursively call this, so don't turn off half way.
                // A reference count would be nice, but if any ERR_DROP ever
                // occurs within the load then the refcount will be knackered
                // from then on
  }
...
static qboolean R_LoadMD3(model_t *mod, int lod, void *buffer,
  ...
  mod->md3[lod] = (md3Header_t *)RE_RegisterModels_Malloc(
      size, buffer, mod_name, &bAlreadyFound, TAG_MODEL_MD3);
...
void R_ModelInit(void) {
  static CachedModels_t
      singleton; // sorry vv, your dynamic allocation was a (false) memory leak
...
================
*/
void R_LerpTag(orientation_t *tag, qhandle_t handle, int startFrame,
  ...
  for (i = 0; i < 3; i++) {
    tag->origin[i] =
        start->origin[i] * backLerp + finish->origin[i] * frontLerp;
    tag->axis[0][i] =
        start->axis[0][i] * backLerp + finish->axis[0][i] * frontLerp;
    tag->axis[1][i] =
        start->axis[1][i] * backLerp + finish->axis[1][i] * frontLerp;
    tag->axis[2][i] =
        start->axis[2][i] * backLerp + finish->axis[2][i] * frontLerp;
  }
