...
public:
  ...
  int Interpret(CTokenizer *, CBlockStream *,
                char *filename = NULL); // Main interpretation function

  int Match(int); // Looks ahead to the next token to try and match it to the
                  // passed token, consumes token on success
  int LookAhead(int); // Looks ahead without consuming on success

  int FindSymbol(const char *,
                 keywordArray_t *); // Searches the symbol table for the given
                                    // name.  Returns the ID if found
  ...
  int GetString(
      CBlock *); // Attempts to match and retrieve the value of a string token
  int GetIdentifier(CBlock *get); // Attempts to match and retrieve the value of
                                  // an identifier token
  int GetInteger(
      CBlock *); // Attempts to match and retrieve the value of a int token
  int GetFloat(
      CBlock *); // Attempts to match and retrieve the value of a float token
  int GetVariable(int type);

  int GetID(char *); // Attempts to match and interpret an identifier

  keywordArray_t *GetSymbols(void) {
    return (keywordArray_t *)&m_symbolKeywords;
  } // Returns the interpreter's symbol table
  keywordArray_t *GetIDs(void) {
    return (keywordArray_t *)&m_IDKeywords;
  } // Returns the interpreter's ID table
  keywordArray_t *GetTypes(void) {
    return (keywordArray_t *)&m_typeKeywords;
  } // Returns the interpreter's type table
...
protected:
  ...
  std::string m_sCurrentFile; // full-pathed name of .TXT file (needed because
                              // of above, which affects parsestreams)
