...
//	case RACE_KLINGON:
                //G_Sound(NPC,
G_SoundIndex(va("sound/mgtest/klingon/talk%d.wav",	Q_irand(1, 4))));
//		break;
//	}
}
*/
...
void G_AddVoiceEvent(gentity_t *self, int event, int speakDebounceTime) {
  ...
  if ((self->NPC->scriptFlags & SCF_NO_COMBAT_TALK) &&
      ((event >= EV_ANGER1 && event <= EV_VICTORY3) ||
       (event >= EV_CHASE1 &&
        event <=
            EV_SUSPICIOUS5))) //(event < EV_FF_1A || event > EV_FF_3C) && (event
                              //< EV_RESPOND1 || event > EV_MISSION3) )
  {
    return;
  }

  if ((self->NPC->scriptFlags & SCF_NO_ALERT_TALK) &&
  ...
  // NOTE: was losing too many speech events, so we do it directly now, screw
  // networking! G_AddEvent( self, event, 0 );
  G_SpeechEvent(self, event);

  // won't speak again for 5 seconds (unless otherwise specified)
  self->NPC->blockedSpeechDebounceTime =
      level.time + ((speakDebounceTime == 0) ? 5000 : speakDebounceTime);
}
  ...
  if (self->health > 0) {
    if (self->enemy ||                           // was mad
      ...
      G_AddVoiceEvent(self, Q_irand(EV_CONFUSE2, EV_CONFUSE3), 2000);
               ...
               self->NPC->investigateDebounceTime + self->NPC->pauseTime >
                   level.time) // was checking something out
    {
      self->NPC->blockedSpeechDebounceTime = 0; // make sure we say this
