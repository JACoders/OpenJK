...
extern void BG_CalculateOffsetAngles(gentity_t *ent,
                                     usercmd_t *ucmd); // in bg_pangles.cpp
...
extern void FireSeeker(gentity_t *owner, gentity_t *target, vec3_t origin,
                       vec3_t dir);
...
extern qboolean PM_AdjustAnglesToPuller(gentity_t *ent, gentity_t *puller,
                                        usercmd_t *ucmd, qboolean faceAway);
extern qboolean PM_AdjustAngleForWallRun(gentity_t *ent, usercmd_t *ucmd,
                                         qboolean doMove);
extern qboolean PM_AdjustAngleForWallRunUp(gentity_t *ent, usercmd_t *ucmd,
                                           qboolean doMove);
extern qboolean PM_AdjustAnglesForSpinningFlip(gentity_t *ent, usercmd_t *ucmd,
                                               qboolean anglesOnly);
...
extern qboolean PM_AdjustAnglesForKnockdown(gentity_t *ent, usercmd_t *ucmd,
                                            qboolean angleClampOnly);
extern qboolean PM_AdjustAnglesForDualJumpAttack(gentity_t *ent,
                                                 usercmd_t *ucmd);
...
extern qboolean PM_AdjustAngleForWallJump(gentity_t *ent, usercmd_t *ucmd,
                                          qboolean doMove);
extern qboolean PM_AdjustAnglesForBFKick(gentity_t *ent, usercmd_t *ucmd,
                                         vec3_t fwdAngs, qboolean aimFront);
...
extern qboolean PM_AdjustAnglesForWallRunUpFlipAlt(gentity_t *ent,
                                                   usercmd_t *ucmd);
extern qboolean PM_AdjustAnglesForHeldByMonster(gentity_t *ent,
                                                gentity_t *monster,
                                                usercmd_t *ucmd);
...
extern qboolean G_PointInBounds(const vec3_t point, const vec3_t mins,
                                const vec3_t maxs);
extern void NPC_SetPainEvent(gentity_t *self);
extern qboolean G_HasKnockdownAnims(gentity_t *ent);
extern int G_GetEntsNearBolt(gentity_t *self, gentity_t **radiusEnts,
                             float radius, int boltIndex, vec3_t boltOrg);
extern qboolean PM_InOnGroundAnim(playerState_t *ps);
extern qboolean PM_LockedAnim(int anim);
extern qboolean WP_SabersCheckLock2(gentity_t *attacker, gentity_t *defender,
...
int G_FindLookItem(gentity_t *self) {
  ...
  // closest, most in front... did damage to... took damage from?  How do we
  // know who the player is focusing on?
  gentity_t *ent;
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if ((ent->spawnflags & 4 /*ITMSF_MONSTER*/)) { // NPCs only
      continue;
    }
    if (!BG_CanItemBeGrabbed(&ent->s, &self->client->ps)) { // don't need it
      continue;
    }
    if (!gi.inPVS(self->currentOrigin,
                  ent->currentOrigin)) { // not even potentially visible
      continue;
    }

    if (!G_ClearLOS(self, self->client->renderInfo.eyePoint,
                    ent)) { // can't see him
      continue;
    }
    if (ent->item->giType == IT_WEAPON &&
        ent->item->giTag == WP_SABER) { // a weapon_saber pickup
      if (self->client->ps.dualSabers   // using 2 sabers already
          || (self->client->ps.saber[0].saberFlags &
              SFL_TWO_HANDED)) // using a 2-handed saber
    ...
    if (ent->item->giType == IT_HOLDABLE &&
        ent->item->giTag ==
            INV_SECURITY_KEY) { // security keys are of the highest importance
      rating *= 2.0f;
    }
...
qboolean G_ClearViewEntity(gentity_t *ent) {
  ...
  if (ent->client->ps.viewEntity > 0 &&
      ent->client->ps.viewEntity < ENTITYNUM_NONE) {
    if (&g_entities[ent->client->ps.viewEntity]) {
      ...
      if (g_entities[ent->client->ps.viewEntity].NPC) {
        ...
        SetClientViewAngle(
            &g_entities[ent->client->ps.viewEntity],
        ...
        g_entities[ent->client->ps.viewEntity].NPC->desiredYaw =
            g_entities[ent->client->ps.viewEntity].currentAngles[YAW];
      }
    }
...
void G_SetViewEntity(gentity_t *self, gentity_t *viewEntity) {
  ...
  if (viewEntity->client) {
    ...
    {
            self->client->ps.delta_angles[i] =
    viewEntity->client->ps.delta_angles[i];
    }
    */
  }
  if (!self->s.number) {
...
qboolean G_ControlledByPlayer(gentity_t *self) {
  if (self && self->NPC &&
      self->NPC->controlledTime > level.time) { // being controlled
    gentity_t *controller = &g_entities[0];
    if (controller->client &&
        controller->client->ps.viewEntity ==
            self->s.number) { // we're the player's viewEntity
      return qtrue;
    }
  }
  return qfalse;
...
qboolean G_ValidateLookEnemy(gentity_t *self, gentity_t *enemy) {
  ...
  if (!enemy->client || !enemy->NPC) { // not valid
    if ((enemy->svFlags & SVF_NONNPC_ENEMY) && enemy->s.weapon == WP_TURRET &&
        enemy->noDamageTeam != self->client->playerTeam &&
        enemy->health > 0) { // a turret
                             // return qtrue;
    } else {
      return qfalse;
    }
    ...
    if (self->client->playerTeam != TEAM_FREE // evil player hates everybody
        &&
        enemy->client->playerTeam == self->client->playerTeam) { // on same team
      return qfalse;
    }
    ...
    if (enemy->health <= 0 &&
        ((level.time - enemy->s.time) > 3000 ||
         !InFront(enemy->currentOrigin, self->currentOrigin,
                  self->client->ps.viewangles, 0.2f) ||
         DistanceHorizontal(enemy->currentOrigin, self->currentOrigin) >
             16384)) //>128
  ...
  if ((!InFront(enemy->currentOrigin, self->currentOrigin,
...
void G_ChooseLookEnemy(gentity_t *self, usercmd_t *ucmd) {
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (!G_ValidateLookEnemy(self,
                             ent)) { // doesn't meet criteria of valid look
                                     // enemy (don't check current since we would
                                     // have done that before this func's call
      continue;
    }
    ...
    if (ent->health <= 0) {
      if ((ucmd->buttons & BUTTON_ATTACK) ||
          ...
          (ucmd->buttons &
           BUTTON_FORCE_FOCUS)) { // if attacking, don't consider dead enemies
        continue;
      }
      if (ent->message) { // keyholder
        rating *= 0.5f;
      } else {
        rating *= 0.1f;
      }
    ...
    if (ent->enemy == self) { // he's mad at me, he's more important
      rating *= 2.0f;
    } else if (ent->NPC &&
               ent->NPC->blockedSpeechDebounceTime >
                   level.time - 6000) { // he's detected me, he's more important
      if (ent->NPC->blockedSpeechDebounceTime > level.time + 4000) {
        rating *= 1.5f;
      } else { // from 1.0f to 1.5f
        rating += rating *
                  ((float)(ent->NPC->blockedSpeechDebounceTime - level.time) +
                   6000.0f) /
                  20000.0f;
      }
    }
...
extern void WP_ForcePowerStart(gentity_t *self, forcePowers_t forcePower,
                               int overrideAmt);
void P_WorldEffects(gentity_t *ent) {
  ...
  if (!in_camera) {
    if (gi.totalMapContents() & (CONTENTS_WATER | CONTENTS_SLIME)) {
      mouthContents =
          gi.pointcontents(ent->client->renderInfo.eyePoint, ent->s.number);
    }
  }
  //
  // check for drowning
  ...
  if ((mouthContents & (CONTENTS_WATER | CONTENTS_SLIME))) {
    ...
    if (ent->client->NPC_class == CLASS_SWAMPTROOPER) { // they have air tanks
      ent->client->airOutTime = level.time + 12000;     // don't need air
      ent->damage = 2;
    } else if (ent->client->airOutTime <
               level.time) { // if out of air, start drowning
      // drown!
  ...
  if (ent->waterlevel && (ent->watertype & (CONTENTS_LAVA | CONTENTS_SLIME))) {
    if (ent->health > 0 && ent->painDebounceTime < level.time) {
      ...
      if (ent->watertype & CONTENTS_LAVA) {
        G_Damage(ent, NULL, NULL, NULL, NULL, 15 * ent->waterlevel, 0,
                 MOD_LAVA);
      }
  ...
  if ((ent->client->poisonDamage) && (ent->client->poisonTime < level.time)) {
    ...
    G_Damage(ent, NULL, NULL, 0, 0, 2, DAMAGE_NO_KNOCKBACK | DAMAGE_NO_ARMOR,
             MOD_UNKNOWN); // FIXME: MOD_POISON?
  ...
  if (ent->client->inSpaceIndex &&
      ent->client->inSpaceIndex !=
          ENTITYNUM_NONE) { // we're in space, check for suffocating and for
                            // exiting
    gentity_t *spacetrigger = &g_entities[ent->client->inSpaceIndex];

    if (!spacetrigger->inuse ||
        !G_PointInBounds(
            ent->client->ps.origin, spacetrigger->absmin,
            spacetrigger->absmax)) { // no longer in space then I suppose
      ent->client->inSpaceIndex = 0;
    } else { // check for suffocation
      if (ent->client->inSpaceSuffocation < level.time) { // suffocate!
        if (ent->health > 0) { // if they're still alive..
          G_Damage(ent, spacetrigger, spacetrigger, NULL,
                   ent->client->ps.origin, Q_irand(20, 40), DAMAGE_NO_ARMOR,
                   MOD_SUICIDE);
          ...
          if (ent->health > 0) { // did that last one kill them?
            ...
            NPC_SetAnim(ent, SETANIM_BOTH, BOTH_CHOKE1,
...
void G_SetClientSound(gentity_t *ent) {
  //	if (ent->waterlevel && (ent->watertype&(CONTENTS_LAVA|CONTENTS_SLIME)) )
  //		ent->s.loopSound =
  //G_SoundIndex("sound/weapons/stasis/electricloop.wav");
...
//==============================================================
extern void G_Knockdown(gentity_t *self, gentity_t *attacker,
                        const vec3_t pushDir, float strength,
                        qboolean breakSaberLock);
extern void G_StartMatrixEffect(gentity_t *ent, int meFlags = 0,
                                int length = 1000, float timeScale = 0.0f,
                                int spinTime = 0);
...
void DoImpact(gentity_t *self, gentity_t *other, qboolean damageSelf,
  ...
  // See if either of these guys are vehicles, if so, keep a pointer to the
  // vehicle npc.
  ...
  /*	if ( self->client && self->client->NPC_class == CLASS_VEHICLE )
          {//we're in a vehicle
                  if ( other->client && other->client->ps.vehicleIndex !=
     VEHICLE_NONE )
                  {//they're in a vehicle
                          ...
                          {//were heading towards each other, this is going to
     be a STRONG impact... vec3_t velocityMod;

                                  //incorportate mass into each velocity to get
     directional force VectorScale( velocity, my_mass/50, velocityMod );
                                  VectorScale( o_velocity, o_mass/50, o_velocity
     );
                                  //figure out the overall magnitude of those 2
     directed forces impacting magnitude = (DotProduct( o_velocity, velocityMod
     ) * -1.0f)/500.0f;
                          }
                  }
          }*/
  ...
  //-----------------------------------------------
  if (pSelfVeh && pSelfVeh->m_pVehicleInfo->type != VH_ANIMAL && pOtherVeh &&
    ...
    if (attacker && victim) {
      //	float		maxMoveSpeed	=
      //pSelfVeh->m_pVehicleInfo->speedMax;
      ...
      VectorSubtract(victim->currentOrigin, attacker->currentOrigin,
                     victimTowardAttacker);
      ...
      if (fabsf(victimRightAccuracy) > 0.25 // Must Be Exactly Right Or Left
          //	&& victimTowardAttackerDistance<100.0f 					//
          //Must Be Close Enough
          //	&& attackerMoveSpeed>minLockingSpeed					// Must
          //be moving fast enough
          //	&& fabsf(attackerMoveSpeed - victimMoveSpeed)<100	 	//
          //Both must be going about the same speed
      ) {
        thrown = true;
        ...
        //				if (false)
        //				{
        //					VectorMA(victim->currentOrigin, 250.0f,
        //victimRight, victimRight); 					CG_DrawEdge(victim->currentOrigin,
        //victimRight, EDGE_IMPACT_POSSIBLE);
        //				}
        if (victimVeh->m_pVehicleInfo->iImpactFX) {
          G_PlayEffect(victimVeh->m_pVehicleInfo->iImpactFX,
                       victim->currentOrigin, trace->plane.normal);
        }
      }
    }
  }
  ...
  if (!self->client || self->client->ps.lastOnGround + 300 < level.time ||
    ...
    if (other->material == MAT_GLASS || other->material == MAT_GLASS_METAL ||
        ...
        ((other->svFlags & SVF_BBRUSH) &&
         (other->spawnflags & 8 /*THIN*/))) //(other->absmax[0]-other->absmin[0]<=32||other->absmax[1]-other->absmin[1]<=32||other->absmax[2]-other->absmin[2]<=32))
                                            //)
    { // glass and thin breakable brushes (axially aligned only, unfortunately)
      // take more impact damage
      magnitude *= 2;
    }
    ...
    if (pSelfVeh && pSelfVeh->m_pVehicleInfo->type != VH_ANIMAL) {
      if ((magnitude >= 80) && (self->painDebounceTime < level.time)) {
        ...
        bool vehPilotedByPlayer =
            (pSelfVeh->m_pPilot && pSelfVeh->m_pPilot->s.number < MAX_CLIENTS);
        ...
        if (pSelfVeh->m_ulFlags & VEH_OUTOFCONTROL) {
          vehToughnessAgainstOther *=
              0.01f; // If Out Of Control, No Damage Resistance
        } else {
          ...
          if (other && other->client) {
            vehToughnessAgainstOther *=
                15.0f; // Very Tough against other clients (NPCS, Player, etc)
          }
        }
        ...
        if (!other || !other->client) {
          ...
          //----------------------------------
          if (vehHitPercent > 0.9f && !pSelfVeh->m_pPilot &&
              vehSpeed > 1000.0f) {
          ...
          //------------------------------------------------------------
          if ((pSelfVeh->m_ulFlags & VEH_OUTOFCONTROL) &&
              (vehHitPercent > 0.5f || trace->plane.normal[2] < 0.5f ||
               velocity[2] < -50.0f)) {
            vehDFlags |= DAMAGE_IMPACT_DIE;
          }
          ...
          //-----------------------------------------------------
          if (vehHitPercent > 0.9f && (level.time - self->lastImpact) > 2000 &&
              vehSpeed > 300.0f) {
            ...
            //-----------------------------------------------
            if (vehPilotedByPlayer) {
              if ((vehHitPercent > 0.99f && vehSpeed > 1000.0f &&
                   !Q_irand(0, 30)) ||
                  (vehHitPercent > 0.999f && vehInTurbo)) {
                vehDFlags |= DAMAGE_IMPACT_DIE;
              }
            } else if (player && G_IsRidingVehicle(player) &&
                       (Distance(self->currentOrigin, player->currentOrigin) <
                        800.0f) &&
                       (vehInTurbo || !Q_irand(0, 1) ||
                        vehHitPercent > 0.999f)) {
              vehDFlags |= DAMAGE_IMPACT_DIE;
            }
          }
        ...
        if (magnitude > 10.0f) {
          ...
          if (pSelfVeh->m_pVehicleInfo->iImpactFX && vehSpeed > 100.0f) {
            G_PlayEffect(pSelfVeh->m_pVehicleInfo->iImpactFX,
                         self->currentOrigin, trace->plane.normal);
          }
        ...
        G_Damage(self, player, player, NULL, self->currentOrigin, magnitude,
                 vehDFlags, MOD_FALLING); // FIXME: MOD_IMPACT
      }

      if (self->owner == other ||
          self->activator == other) { // hit owner/activator
        if (self->m_pVehicle && !self->m_pVehicle->m_pVehicleInfo->Inhabited(
                                    self->m_pVehicle)) { // empty swoop
          if (self->client->respawnTime - level.time <
              1000) { // just spawned in a second ago
      ...
      if (pOtherVeh) {
        if (self->client->playerTeam == other->client->playerTeam) {
          magnitude /= 25;
               ...
               magnitude >= 120) { // FORCE-SMACKED into something
      if (TIMER_Done(self, "impactEffect")) {
        G_PlayEffect(G_EffectIndex("env/impact_dustonly"), trace->endpos,
                     trace->plane.normal);
        G_Sound(self,
    ...
    if (magnitude >= 100 && other->s.number < ENTITYNUM_WORLD) {
      ...
      if (VectorCompare(other->currentOrigin,
      ...
      if ((cont & CONTENTS_WATER)) { // water absorbs 2/3 velocity
        force *= 0.33333f;
      }

      if (self->NPC &&
          other->s.number == ENTITYNUM_WORLD) { // NPCs take less damage
        force *= 0.5f;
      }

      if (self->s.number >= MAX_CLIENTS && self->client &&
          (PM_InKnockDown(&self->client->ps) ||
           self->client->ps.eFlags &
               EF_FORCE_GRIPPED)) { // NPC: I was knocked down or being gripped,
                                    // impact should be harder
        // FIXME: what if I was just thrown - force pushed/pulled or thrown from
        // a grip?
        force *= 10;
      }

      // FIXME: certain NPCs/entities should be TOUGH - like Ion Cannons,
      // AT-STs, Mark1 droids, etc.
      if (pOtherVeh) { // if hit another vehicle, take their toughness into
                       // account, too
        force /= pOtherVeh->m_pVehicleInfo->toughness;
      }

      if (((force >= 1 || pSelfVeh) && other->s.number >= MAX_CLIENTS) ||
          force >= 10) {
        /*
                                dprint("Damage other (");
        ...
        */
        ...
        if (pSelfVeh) { // if in a vehicle when land on someone, always
                        // knockdown, throw, damage
          if (!vehicleHitOwner) { // didn't hit owner
        ...
        } else if (self->forcePushTime >
                       level.time - 1000 // was force pushed/pulled in the last
                                         // 1600 milliseconds
                   &&
                   self->forcePuller ==
                       other->s.number) // hit the person who pushed/pulled me
        {                               // ignore the impact
        } else if (other->takedamage) {
          if (!self->client || other->s.number < MAX_CLIENTS ||
              !other->client) { // aw, fuck it, clients no longer take impact
                                // damage from other clients, unless you're the
                                // player
            if (other->client   // he's a client
                && self->client // I'm a client
                && other->client->ps.forceGripEntityNum ==
                       self->s.number) // he's force-gripping me
            ...
            GEntity_PainFunc(other, self, self, self->currentOrigin, force,
                             MOD_IMPACT);
            // Hmm, maybe knockdown?
            if (!thrown) {
              if (!(other->flags & FL_NO_KNOCKBACK)) {
          ...
          if (other->health > 0) { // still alive?
            // TODO: if someone was thrown through the air (in a knockdown or
            // being gripped) 		and they hit me hard enough, knock me down
            if (other->client) {
              if (self->client) {
                ...
                G_Knockdown(other, &g_entities[self->forcePuller], dir2,
                            Q_irand(200, 400), qtrue);
              }
            }
          }
        } else {
    ...
    if (damageSelf && self->takedamage && !(self->flags & FL_NO_IMPACT_DMG)) {
      ...
      // vehicle
      if (pSelfVeh &&
          self->client->ps.forceJumpZStart) { // we were force-jumping
        if (self->currentOrigin[2] >=
            self->client->ps.forceJumpZStart) { // we landed at same height or
                                                // higher than we landed
          magnitude = 0;
        } else { // FIXME: take off some of it, at least?
          magnitude =
              (self->client->ps.forceJumpZStart - self->currentOrigin[2]) / 3;
        }
      }
      ...
      if ((magnitude >= 100 + self->health && self->s.number >= MAX_CLIENTS &&
          ...
          (magnitude >=
           700)) // health here is used to simulate structural integrity
      {
        if ((self->s.weapon == WP_SABER || self->s.number < MAX_CLIENTS ||
            ...
            self->client && self->client->ps.groundEntityNum < ENTITYNUM_NONE &&
        ...
        if (other->bmodel &&
            other->material != MAT_GLASS) { // take off only a little because we
                                            // broke architecture (not including
                                            // glass), that should hurt
          magnitude = magnitude - force / 8;
        } else { // take off half damage we did to it
          magnitude = magnitude - force / 2;
        }

        if (pSelfVeh) {
          // FIXME: if hit another vehicle, take their toughness into
          //			account, too?  Or should their toughness only
          //matter 			when they hit me?
          magnitude /= pSelfVeh->m_pVehicleInfo->toughness * 1000.0f;
          if (other->bmodel &&
              other->material !=
                  MAT_GLASS) { // broke through some architecture, take a good
                               // amount of damage
          } else if (pOtherVeh) { // they're tougher
                                  // magnitude /= 4.0f;//FIXME: get the
                                  // toughness of other from vehicles.cfg
          } else { // take some off because of give
        ...
        if (magnitude >= 1) {
          ...
          */
          if (self->NPC &&
              self->s.weapon ==
                  WP_SABER) { // FIXME: for now Jedi take no falling damage, but
                              // really they should if pushed off?
            magnitude = 0;
          }

          G_Damage(self, NULL, NULL, NULL, self->currentOrigin, magnitude / 2,
...
void ClientImpacts(gentity_t *ent, pmove_t *pm) {
  ...
  for (i = 0; i < pm->numtouch; i++) {
    ...
    if ((ent->NPC != NULL) &&
        (ent->e_TouchFunc != touchF_NULL)) { // last check unneccessary
      GEntity_TouchFunc(ent, other, &trace);
    }

    if (other->e_TouchFunc == touchF_NULL) { // not needed, but I'll leave it I
                                             // guess (cache-hit issues)
      continue;
    }
...
void G_TouchTriggersLerped(gentity_t *ent) {
  ...
  for (curDist = 0; !done && ent->maxs[1] > 0;
       curDist += (float)ent->maxs[1] / 2.0f) {
    if (curDist >= dist) {
    ...
    for (i = 0; i < num; i++) {
      ...
      if (ent->client->ps.stats[STAT_HEALTH] <= 0) {
        if (Q_stricmp("trigger_teleport", hit->classname) ||
            !(hit->spawnflags &
              16 /*TTSF_DEAD_OK*/)) { // dead clients can only touch
                                      // tiogger_teleports that are marked as
                                      // touchable
          continue;
        }
      }
...
void G_MatchPlayerWeapon(gentity_t *ent) {
  if (g_entities[0].inuse && g_entities[0].client) { // player is around
    int newWeap;
    ...
    if (newWeap != WP_NONE && ent->client->ps.weapon != newWeap) {
      ...
      if (newWeap == WP_SABER) {
        ...
        for (int saberNum = 0; saberNum < numSabers; saberNum++) {
          ...
          // ent->client->ps.saber[saberNum].model, ent->handRBolt, 0 );
          ent->client->ps.saber[saberNum].type =
              g_entities[0].client->ps.saber[saberNum].type;
          for (int bladeNum = 0;
               bladeNum < ent->client->ps.saber[saberNum].numBlades;
               bladeNum++) {
            ent->client->ps.saber[saberNum].blade[0].active =
                g_entities[0].client->ps.saber[saberNum].blade[bladeNum].active;
            ent->client->ps.saber[saberNum].blade[0].length =
                g_entities[0].client->ps.saber[saberNum].blade[bladeNum].length;
          }
        }
        ent->client->ps.saberAnimLevel =
            g_entities[0].client->ps.saberAnimLevel;
        ent->client->ps.saberStylesKnown =
            g_entities[0].client->ps.saberStylesKnown;
      } else {
...
void G_NPCMunroMatchPlayerWeapon(gentity_t *ent) {
  ...
  if (!in_camera) {
    if (ent && ent->client && ent->NPC &&
        (ent->NPC->aiFlags & NPCAI_MATCHPLAYERWEAPON)) { // we're a Player NPC
    ...
    /*		if ( ent->client->ps.powerups[PW_SEEKER] > level.time )
                    {
                            ...
                            gentity_t *enemy = SeekerAcquiresTarget( ent,
       seekerPos );

                            if ( enemy != NULL ) // set the client's enemy to a
       valid target
                            {
                                    FireSeeker( ent, enemy, seekerPos, dir );

                                    gentity_t *tent;
                                    tent = G_TempEntity( seekerPos,
       EV_POWERUP_SEEKER_FIRE ); VectorCopy( dir, tent->pos1 );
                                    tent->s.eventParm = ent->s.number;
                            }
                    }*/
    if ((ent->flags & FL_OVERCHARGED_HEALTH)) { // need to gradually reduce
                                                // health back to max
      if (ent->health > ent->client->ps.stats[STAT_MAX_HEALTH]) { // decrement
                                                                  // it
        ent->health--;
  ...
  if (client->ps.weapon == WP_SABER) {
    if (g_timescale->value >= 1.0f ||
      ...
      if (client->ps.saberDamageDebounceTime - level.time >
          wait) { // when you unpause the game with force speed on, the time
                  // gets *really* wiggy...
      ...
      if (client->ps.saberDamageDebounceTime <= level.time) {
        ...
        /*
        if (
        g_timescale->value&&client->ps.clientNum==0&&!player_locked&&!MatrixMode&&client->ps.forcePowersActive&(1<<FP_SPEED)
        )
        {
...
void G_ThrownDeathAnimForDeathAnim(gentity_t *hitEnt, vec3_t impactPoint) {
  ...
  case BOTH_DEATH7:  // knee collapse, twist & fall forward
  {
    ...
    float dot = DotProduct(
        facing,
        dir2Impact);  //-1 = hit in front, 0 = hit on side, 1 = hit in back
    if (dot > 0.5f) { // kicked in chest, fly backward
      switch (Q_irand(0, 4)) { // FIXME: don't start at beginning of anim?
      case 0:
    ...
    } else if (dot < -0.5f) {  // kicked in back, fly forward
      switch (Q_irand(0, 5)) { // FIXME: don't start at beginning of anim?
      case 0:
        anim = BOTH_DEATH14;
        break;
      case 1:
    ...
    } else {                   // hit on side, spin
      switch (Q_irand(0, 2)) { // FIXME: don't start at beginning of anim?
      case 0:
        anim = BOTH_DEATH12;
        break;
      case 1:
  ...
  if (anim != -1) {
    NPC_SetAnim(hitEnt, SETANIM_BOTH, anim,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  }
}

gentity_t *G_KickTrace(gentity_t *ent, vec3_t kickDir, float kickDist,
  ...
  if (kickEnd &&
    ...
    VectorSet(traceOrg, ent->currentOrigin[0], ent->currentOrigin[1],
              kickEnd[2]);
    VectorCopy(kickEnd, traceEnd);
  } else { // extrude
    VectorSet(traceOrg, ent->currentOrigin[0], ent->currentOrigin[1],
              ent->currentOrigin[2] + ent->maxs[2] * 0.5f);
    VectorMA(traceOrg, kickDist, kickDir, traceEnd);
  }

  gi.trace(&trace, traceOrg, kickMins, kickMaxs, traceEnd, ent->s.number,
           MASK_SHOT, (EG2_Collision)0, 0); // clipmask ok?
  if (trace.fraction < 1.0f && !trace.startsolid && !trace.allsolid &&
    ...
    if (hitEnt) { // we hit an entity
      if (hitEnt->client) {
        if (!(hitEnt->client->ps.pm_flags & PMF_TIME_KNOCKBACK) &&
            TIMER_Done(
                hitEnt,
                "kickedDebounce")) // not already flying through air?  Intended
                                   // to stop multiple hits, but...
        { // FIXME: this should not always work
          if (PM_InKnockDown(&hitEnt->client->ps) &&
              !PM_InGetUp(
                  &hitEnt->client->ps)) { // don't hit people who are knocked
                                          // down or being knocked down (okay to
                                          // hit people getting up, though)
            return NULL;
          }
          if (PM_InRoll(
                  &hitEnt->client->ps)) { // can't hit people who are rolling
            return NULL;
          }
          ...
          if (hitEnt->takedamage) { // hurt it
            G_Damage(hitEnt, ent, ent, kickDir, trace.endpos, kickDamage,
                     DAMAGE_NO_KNOCKBACK | DAMAGE_NO_KILL, MOD_MELEE);
          }
          ...
          if (ent->client->ps.torsoAnim == BOTH_A7_HILT) { // hit in head
            ...
            if (hitEnt->health > 0) { // knock down
              if (hitEnt->client->ps.groundEntityNum ==
                  ENTITYNUM_NONE) { // he's in the air?  Send him flying back
                if (!(hitEnt->flags & FL_NO_KNOCKBACK)) {
                  G_Throw(hitEnt, kickDir, kickPush);
                }
              } else {
          ...
          } else if (hitEnt->health <= 0) { // we kicked a dead guy
            // throw harder - FIXME: no matter how hard I push them, they don't
            // go anywhere... corpses use less physics???
            if (!(hitEnt->flags & FL_NO_KNOCKBACK)) {
      ...
      } else { // FIXME: don't do this in repeated frames... only allow 1 frame
               // in kick to hit wall?  The most extended one?  Pass in a bool on
               // that frame.
        if (doSoundOnWalls) { // do kick hit sound and impact effect
          if (TIMER_Done(ent, "kickSoundDebounce")) {
            if (ent->client->ps.torsoAnim == BOTH_A7_HILT) {
              G_Sound(ent, G_SoundIndex("sound/movers/objects/saber_slam"));
            } else {
              G_PlayEffect(G_EffectIndex("melee/kick_impact"), trace.endpos,
                           trace.plane.normal);
              // G_Sound( ent, G_SoundIndex( va( "sound/weapons/melee/punch%d",
              // Q_irand( 1, 4 ) ) ) );
            }
...
qboolean G_CheckRollSafety(gentity_t *self, int anim, float testDist) {
  ...
  if (PM_InAttackRoll(
          self->client->ps.legsAnim)) { // we don't care if people are in the
                                        // way, we'll knock them down!
  ...
  case BOTH_GETUP_FROLL_B:
    ...
    break;
  default
      : // FIXME: add normal rolls?  Make generic for any forced-movement anim?
    return qtrue;
    break;
  }

  gi.trace(&trace, self->currentOrigin, self->mins, self->maxs, testPos,
           self->s.number, contents, (EG2_Collision)0, 0);
  if (trace.fraction < 1.0f || trace.allsolid ||
      trace.startsolid) { // inside something or will hit something
    return qfalse;
  }
  return qtrue;
}
  ...
  if ((ent->s.number < MAX_CLIENTS || G_ControlledByPlayer(ent))) {
    float animLength =
        PM_AnimLength(ent->client->clientInfo.animFileIndex,
                      (useTorso ? (animNumber_t)ent->client->ps.torsoAnim
                                : (animNumber_t)ent->client->ps.legsAnim));
    float elapsedTime =
        (float)(animLength - (useTorso ? ent->client->ps.torsoAnimTimer
                                       : ent->client->ps.legsAnimTimer));
    float backDist = 0;
    if (elapsedTime < animLength / 2.0f) { // starting anim
...
qboolean G_GrabClient(gentity_t *ent, usercmd_t *ucmd) {
  ...
  for (i = 0; i < numEnts; i++) {
    ...
    if (radiusEnts[i] == ent) { // Skip the rancor ent
      continue;
    }

    if (!radiusEnts[i]->inuse || radiusEnts[i]->health <= 0) { // must be alive
      continue;
    }

    if (radiusEnts[i]->client == NULL) { // must be a client
      continue;
    }

    if ((radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_RANCOR) ||
        (radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_WAMPA) ||
        (radiusEnts[i]->client->ps.eFlags &
         EF_HELD_BY_SAND_CREATURE)) { // can't be one being held
      continue;
    }

    if (PM_LockedAnim(radiusEnts[i]->client->ps.torsoAnim) ||
        PM_LockedAnim(radiusEnts[i]->client->ps.legsAnim)) { // don't interrupt
      continue;
    }
    if (radiusEnts[i]->client->ps.groundEntityNum ==
        ENTITYNUM_NONE) { // must be on ground
      continue;
    }

    if (PM_InOnGroundAnim(&radiusEnts[i]->client->ps)) { // must be standing up
      continue;
    }

    if (fabs(radiusEnts[i]->currentOrigin[2] - ent->currentOrigin[2]) >
        8.0f) { // have to be close in Z
      continue;
    }

    if (!PM_HasAnimation(radiusEnts[i],
                         BOTH_PLAYER_PA_1)) { // doesn't have matching anims
      continue;
    }
...
qboolean G_PullAttack(gentity_t *ent, usercmd_t *ucmd) {
  ...
  if (ent->client->ps.torsoAnim == BOTH_PULL_IMPALE_STAB ||
      ent->client->ps.torsoAnim == BOTH_PULL_IMPALE_SWING) { // pulling
    if (ent->NPC) {
    ...
    ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
             ...
             ent->client->ps.torsoAnim == BOTH_PULLED_INAIR_F) { // being pulled
    gentity_t *puller = &g_entities[ent->client->ps.pullAttackEntNum];
    if (puller && puller->inuse && puller->client &&
      ...
      VectorMA( pullPos, puller->client->ps.saber[0].blade[0].length*0.5f,
      puller->client->ps.saber[0].blade[0].muzzleDir, pullPos );
      */
      vec3_t pullerFwd;
      AngleVectors(puller->client->ps.viewangles, pullerFwd, NULL, NULL);
      VectorMA(puller->currentOrigin, (puller->maxs[0] * 1.5f) + 16.0f,
               pullerFwd, pullPos);
      // pull me towards that pos
      VectorSubtract(pullPos, ent->currentOrigin, pullDir);
      float pullDist = VectorNormalize(pullDir);
      int sweetSpotTime =
          (puller->client->ps.torsoAnim == BOTH_PULL_IMPALE_STAB) ? 1250 : 1350;
      float pullLength =
          PM_AnimLength(puller->client->clientInfo.animFileIndex,
                        (animNumber_t)puller->client->ps.torsoAnim) -
          sweetSpotTime;
      if (pullLength <= 0.25f) {
        pullLength = 0.25f;
      }
      ...
      ent->forcePushTime =
          level.time + 100; // let the push effect last for 100 more ms
      // look at him
      PM_AdjustAnglesToPuller(
          ent, puller, ucmd,
          qboolean(ent->client->ps.legsAnim == BOTH_PULLED_INAIR_B));
      overridAngles = qtrue;
      // don't move
      if (ent->NPC) {
        VectorClear(ent->client->ps.moveDir);
...
void G_FixMins(gentity_t *ent) {
  ...
  if (!trace.allsolid && !trace.startsolid) {
    if (trace.fraction >= 1.0f) { // all clear
    ...
    } else { // move me up so the bottom of my bbox will be where the trace
             // ended, at least
      ...
      if (!trace.allsolid && !trace.startsolid) {
        ...
        } else { // crap, no room to expand!
          if (ent->client->ps.legsAnimTimer <=
              200) { // at the end of the anim, and we can't leave ourselves
                     // like this
...
qboolean G_CheckClampUcmd(gentity_t *ent, usercmd_t *ucmd) {
  ...
  if (ent->client->NPC_class == CLASS_PROTOCOL ||
      ...
      ent->client->NPC_class ==
          CLASS_MOUSE) { // these droids *cannot* strafe (looks bad)
    if (ucmd->rightmove) {
      // clear the strafe
      ucmd->rightmove = 0;
      // movedir is invalid now, but PM_WalkMove will rebuild it from the ucmds,
  ...
  if ((ent->s.number < MAX_CLIENTS || G_ControlledByPlayer(ent)) &&
      ...
      ent->client->ps.torsoAnim == BOTH_KYLE_GRAB) { // see if we grabbed enemy
    if (ent->client->ps.torsoAnimTimer <= 200) {     // close to end of anim
      if (G_GrabClient(ent, ucmd)) {                 // grabbed someone!
      } else {                                       // missed
        NPC_SetAnim(ent, SETANIM_BOTH, BOTH_KYLE_MISS,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  ...
  if ((!ent->s.number && ent->aimDebounceTime > level.time) ||
      ...
      ent->forcePushTime >
          level.time) { // being knocked back, can't do anything!
    ucmd->buttons = 0;
    ucmd->forwardmove = 0;
    ucmd->rightmove = 0;
    ucmd->upmove = 0;
  ...
  overridAngles =
      (PM_AdjustAnglesForKnockdown(ent, ucmd, qfalse) ? qtrue : overridAngles);
  if (PM_GetupAnimNoMove(ent->client->ps.legsAnim)) {
    ucmd->forwardmove = ucmd->rightmove = 0; // ucmd->upmove = ?
  }
  // check saberlock
  if (ent->client->ps.saberLockTime > level.time) { // in saberlock
    ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
    if (ent->client->ps.saberLockTime - level.time >
        SABER_LOCK_DELAYED_TIME) { // 2 second delay before either can push
    ...
    overridAngles =
        (PM_AdjustAnglesForSaberLock(ent, ucmd) ? qtrue : overridAngles);
    if (ent->NPC) {
      VectorClear(ent->client->ps.moveDir);
    }
  }
  // check force drain
  if ((ent->client->ps.forcePowersActive & (1 << FP_DRAIN))) { // draining
    ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
  ...
  if (ent->client->ps.saberMove == LS_A_LUNGE) { // can't move during lunge
  ...
  if (ent->client->ps.legsAnim == BOTH_FORCEWALLRUNFLIP_ALT &&
    ...
    ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
    overridAngles =
        (PM_AdjustAnglesForWallRunUpFlipAlt(ent, ucmd) ? qtrue : overridAngles);
  }

  if (ent->client->ps.saberMove == LS_A_JUMP_T__B_) { // can't move during leap
    if (ent->client->ps.groundEntityNum != ENTITYNUM_NONE ||
        (!ent->s.number && player_locked)) { // hit the ground
      ucmd->forwardmove = 0;
    }
    ucmd->rightmove = ucmd->upmove = 0;
    if (ent->NPC) { // invalid now
  ...
  if (ent->client->ps.saberMove == LS_A_BACKFLIP_ATK &&
      ent->client->ps.legsAnim == BOTH_JUMPATTACK7) { // backflip attack
    if (ent->client->ps.legsAnimTimer > 800           // not at end
        &&
        PM_AnimLength(ent->client->clientInfo.animFileIndex, BOTH_JUMPATTACK7) -
                ent->client->ps.legsAnimTimer >=
            400) // not in beginning
    {            // middle of anim
      if (ent->client->ps.groundEntityNum !=
          ENTITYNUM_NONE) { // still on ground?
        ...
        ent->client->ps.forceJumpZStart =
            ent->client->ps
                .origin[2]; // so we don't take damage if we land at same height
  ...
  if (ent->client->ps.legsAnim == BOTH_JUMPATTACK6 &&
    ...
    if (ent->client->ps.legsAnimTimer >= 100 // not at end
        &&
        PM_AnimLength(ent->client->clientInfo.animFileIndex, BOTH_JUMPATTACK6) -
                ent->client->ps.legsAnimTimer >=
    ...
    {                 // one of the two jumps
      if (ent->client->ps.groundEntityNum !=
        ...
        G_AddEvent(ent, EV_JUMP, 0);
        G_SoundOnEnt(ent, CHAN_BODY, "sound/weapons/force/jump.wav");
      } else { // FIXME: if this is the second jump, maybe we should just stop
               // the anim?
      }
    }
    // else
    { // disallow turning unless in the middle frame when you're on the ground
      // overridAngles = (PM_AdjustAnglesForDualJumpAttack( ent, ucmd
      // )?qtrue:overridAngles);
    }

    // dynamically reduce bounding box to let character sail over heads of
    // enemies
    if ((ent->client->ps.legsAnimTimer >= 1450 &&
        ...
        (ent->client->ps.legsAnimTimer >= 400 &&
         PM_AnimLength(ent->client->clientInfo.animFileIndex,
                       BOTH_JUMPATTACK6) -
                 ent->client->ps.legsAnimTimer >=
             1100)) { // in a part of the anim that we're pretty much sideways
                      // in, raise up the mins
      ent->mins[2] = 0;
      ent->client->ps.pm_flags |= PMF_FIX_MINS;
    } else if ((ent->client->ps.pm_flags &
                PMF_FIX_MINS)) { // drop the mins back down
      G_FixMins(ent);
    }
    if (ent->NPC) { // invalid now
      VectorClear(ent->client->ps.moveDir);
  ...
  if ((ent->client->ps.legsAnim == BOTH_BUTTERFLY_FL1 &&
      ...
      (ent->client->ps.legsAnim == BOTH_BUTTERFLY_LEFT &&
       ent->client->ps.saberMove ==
           LS_BUTTERFLY_LEFT)) { // forward flip/spin attack
    ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
            ...
            || ent->client->ps.legsAnim == BOTH_BUTTERFLY RIGHT )*/
    {
      ...
      if (ent->client->ps.legsAnimTimer >= 1700 &&
          ent->client->ps.legsAnimTimer < 1800) { // one of the two jumps
        if (ent->client->ps.groundEntityNum !=
            ENTITYNUM_NONE) { // still on ground?
          // jump!
          ent->client->ps.velocity[2] = 250;
          ent->client->ps.forceJumpZStart =
              ent->client->ps.origin[2]; // so we don't take damage if we land
                                         // at same height
          ent->client->ps.pm_flags |= PMF_JUMPING; //|PMF_SLOW_MO_FALL;
          // FIXME: NPCs yell?
          G_AddEvent(ent, EV_JUMP, 0);
  ...
  if (ent->client->ps.legsAnim == BOTH_A7_SOULCAL &&
      ent->client->ps.saberMove ==
          LS_STAFF_SOULCAL) { // forward spinning staff attack
    ucmd->upmove = 0;
    ...
    if (PM_CanRollFromSoulCal(&ent->client->ps)) {
      ...
      ucmd->rightmove = 0;
      // FIXME: don't slide off people/obstacles?
      if (ent->client->ps.legsAnimTimer >= 2750) { // not at end
        // push forward
        ucmd->forwardmove = 64;
      } else {
        ucmd->forwardmove = 0;
    ...
    if (ent->client->ps.legsAnimTimer >= 2650 &&
        ent->client->ps.legsAnimTimer < 2850) { // the jump
      if (ent->client->ps.groundEntityNum !=
          ENTITYNUM_NONE) { // still on ground?
        // jump!
        ent->client->ps.velocity[2] = 250;
  ...
  if (ent->client->ps.torsoAnim == BOTH_A6_FB ||
      ent->client->ps.torsoAnim ==
          BOTH_A6_LR) { // can't turn or move during dual attack
    if (ent->NPC) {
      VectorClear(ent->client->ps.moveDir);
    }
    overridAngles = (PM_LockAngles(ent, ucmd) ? qtrue : overridAngles);
  ...
  } else if (PM_SuperBreakLoseAnim(
                 ent->client->ps
                     .torsoAnim)) { // can't turn during Kyle's grappling
    if (ent->NPC) {
      VectorClear(ent->client->ps.moveDir);
    }
    overridAngles = (PM_LockAngles(ent, ucmd) ? qtrue : overridAngles);
    ...
    if (ent->client->ps.legsAnim == BOTH_LK_DL_ST_T_SB_1_L) {
      ...
      ent->client->ps.speed = 400;
             ...
             ent->client->ps.legsAnim ==
                 BOTH_FORCE_DRAIN_GRABBED) { // can't turn or move
    if (ent->s.number < MAX_CLIENTS || G_ControlledByPlayer(ent)) { // player
      float forceDrainAngle = 90.0f;
      if (ent->client->ps.torsoAnim ==
          BOTH_FORCE_DRAIN_GRAB_START) { // starting drain
        float animLength =
            PM_AnimLength(ent->client->clientInfo.animFileIndex,
                          (animNumber_t)ent->client->ps.torsoAnim);
        float elapsedTime = (float)(animLength - ent->client->ps.legsAnimTimer);
        float angle = (elapsedTime / animLength) * forceDrainAngle;
        cg.overrides.active |= CG_OVERRIDE_3RD_PERSON_ANG;
        cg.overrides.thirdPersonAngle = cg_thirdPersonAngle.value + angle;
      } else if (ent->client->ps.torsoAnim ==
                 BOTH_FORCE_DRAIN_GRAB_HOLD) { // draining
        cg.overrides.active |= CG_OVERRIDE_3RD_PERSON_ANG;
        cg.overrides.thirdPersonAngle =
            cg_thirdPersonAngle.value + forceDrainAngle;
      } else if (ent->client->ps.torsoAnim ==
                 BOTH_FORCE_DRAIN_GRAB_END) { // ending drain
        float animLength =
            PM_AnimLength(ent->client->clientInfo.animFileIndex,
                          (animNumber_t)ent->client->ps.torsoAnim);
        float elapsedTime = (float)(animLength - ent->client->ps.legsAnimTimer);
        float angle =
            forceDrainAngle - ((elapsedTime / animLength) * forceDrainAngle);
        cg.overrides.active |= CG_OVERRIDE_3RD_PERSON_ANG;
        cg.overrides.thirdPersonAngle = cg_thirdPersonAngle.value + angle;
      }
    }
    ...
    overridAngles = PM_LockAngles(ent, ucmd) ? qtrue : overridAngles;
             ...
             ent->client->ps.legsAnim ==
                 BOTH_KYLE_PA_3) { // can't turn during Kyle's grappling
    if (ent->NPC) {
      VectorClear(ent->client->ps.moveDir);
    }
    overridAngles =
        PM_AdjustAnglesForGrapple(ent, ucmd) ? qtrue : overridAngles;
      ...
      if (TIMER_Done(ent, "grappleDamageDebounce")) {
        ...
        case BOTH_PLAYER_PA_1:
          if (ent->client->ps.legsAnimTimer <= 3150 &&
            ...
            if (ent->s.number < MAX_CLIENTS) { // special case
              damage = Q_irand(1, 3);
            } else {
              damage = Q_irand(3, 5);
            }
        ...
        case BOTH_PLAYER_PA_3_FLY:
          ...
          {//special case
                  if ( ent->client->ps.legsAnimTimer >
          PLAYER_KNOCKDOWN_HOLD_EXTRA_TIME
                          && ent->client->ps.legsAnimTimer <=
          PLAYER_KNOCKDOWN_HOLD_EXTRA_TIME + 100 )
                  {
                          TIMER_Set( ent, "grappleDamageDebounce", 150 );
                          damage = Q_irand( 4, 8 );
                          dflags &= ~DAMAGE_NO_KILL;
    ...
    if (ent->s.number < MAX_CLIENTS &&
        ent->client->ps.legsAnim ==
            BOTH_PLAYER_PA_3_FLY) { // player holds extra long so you have more
                                    // time to decide to do the quick getup
    ...
    if (ent->client->ps.legsAnimTimer <=
        endOfAnimTime) { // pretty much done with the anim, so get up now
      if (ent->client->ps.legsAnim == BOTH_PLAYER_PA_3) {
        ...
        ent->client->ps.legsAnimTimer = ent->client->ps.torsoAnimTimer = 0;
        NPC_SetAnim(ent, SETANIM_BOTH, BOTH_PLAYER_PA_3_FLY,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        ent->client->ps.weaponTime = ent->client->ps.legsAnimTimer;
        /*
        if ( ent->s.number < MAX_CLIENTS )
        {//player holds extra long so you have more time to decide to do the
        quick getup ent->client->ps.legsAnimTimer +=
        PLAYER_KNOCKDOWN_HOLD_EXTRA_TIME; ent->client->ps.torsoAnimTimer +=
        PLAYER_KNOCKDOWN_HOLD_EXTRA_TIME;
        }
        */
        ...
        ent->client->ps.legsAnimTimer = ent->client->ps.torsoAnimTimer = 0;
        NPC_SetAnim(ent, SETANIM_BOTH, BOTH_KNOCKDOWN2,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        ent->client->ps.weaponTime = ent->client->ps.legsAnimTimer;
        if (ent->s.number <
            MAX_CLIENTS) { // player holds extra long so you have more time to
                           // decide to do the quick getup
      ...
      {
              ent->client->ps.legsAnimTimer = ent->client->ps.torsoAnimTimer =
      0; NPC_SetAnim( ent, SETANIM_BOTH, BOTH_GETUP1,
      SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD ); ent->client->ps.weaponTime =
      ent->client->ps.legsAnimTimer;
      }
      */
    }
    if (d_slowmodeath->integer <= 3 &&
        ent->s.number < MAX_CLIENTS) { // no matrix effect, so slide the camera
      ...
      G_CamCircleForLegsAnim(ent);
             ...
             ent->client->ps.legsAnim ==
                 BOTH_FORCELONGLEAP_LAND) { // can't turn during force leap
    if (ent->NPC) {
      VectorClear(ent->client->ps.moveDir);
    }
    overridAngles =
        PM_AdjustAnglesForLongJump(ent, ucmd) ? qtrue : overridAngles;
             ...
             ent->client->ps.legsAnim == BOTH_BUTTERFLY_LEFT ||
             ent->client->ps.legsAnim ==
                 BOTH_A7_SOULCAL) { // can't move, check for damage frame
    ...
    vec3_t kickDir = {0, 0, 0}, kickDir2 = {0, 0, 0}, kickEnd = {0, 0, 0},
           kickEnd2 = {0, 0, 0},
           fwdAngs = {0, ent->client->ps.viewangles[YAW], 0};
    float animLength = PM_AnimLength(ent->client->clientInfo.animFileIndex,
                                     (animNumber_t)ent->client->ps.legsAnim);
    float elapsedTime = (float)(animLength - ent->client->ps.legsAnimTimer);
    float remainingTime = (animLength - elapsedTime);
    float kickDist =
        (ent->maxs[0] * 1.5f) + STAFF_KICK_RANGE + 8.0f; // fudge factor of 8
    ...
    if (ent->client->ps.torsoAnim == BOTH_A7_HILT) {
      if (elapsedTime >= 250 && remainingTime >= 250) { // front
        doKick = qtrue;
        if (ent->handRBolt != -1) { // actually trace to a bolt
      ...
      case BOTH_A7_SOULCAL:
        ...
        if (elapsedTime >= 1400 && elapsedTime <= 1500) { // right leg
          doKick = qtrue;
          if (ent->footRBolt != -1) { // actually trace to a bolt
        ...
        break;
      case BOTH_ARIAL_F1:
        if (elapsedTime >= 550 && elapsedTime <= 1000) { // right leg
          doKick = qtrue;
          if (ent->footRBolt != -1) { // actually trace to a bolt
            G_GetBoltPosition(ent, ent->footRBolt, kickEnd);
            VectorSubtract(kickEnd, ent->currentOrigin, kickDir);
        ...
        if (elapsedTime >= 800 && elapsedTime <= 1200) { // left leg
          doKick2 = qtrue;
          if (ent->footLBolt != -1) { // actually trace to a bolt
      ...
      case BOTH_ARIAL_RIGHT:
        if (elapsedTime >= 200 && elapsedTime <= 600) { // lead leg
          int footBolt = (ent->client->ps.legsAnim == BOTH_ARIAL_LEFT)
                             ? ent->footRBolt
                             : ent->footLBolt; // mirrored anims
          doKick = qtrue;
          if (footBolt != -1) { // actually trace to a bolt
        ...
        if (elapsedTime >= 400 && elapsedTime <= 850) { // trailing leg
          int footBolt = (ent->client->ps.legsAnim == BOTH_ARIAL_LEFT)
                             ? ent->footLBolt
                             : ent->footRBolt; // mirrored anims
          doKick2 = qtrue;
          if (footBolt != -1) { // actually trace to a bolt
      ...
      case BOTH_CARTWHEEL_RIGHT:
        ...
        if (elapsedTime >= 350 && elapsedTime <= 650) { // trailing leg
          int footBolt = (ent->client->ps.legsAnim == BOTH_CARTWHEEL_LEFT)
                             ? ent->footLBolt
                             : ent->footRBolt; // mirrored anims
          doKick2 = qtrue;
      ...
      case BOTH_JUMPATTACK7:
        if (elapsedTime >= 300 && elapsedTime <= 900) { // right knee
          doKick = qtrue;
          if (ent->kneeRBolt != -1) { // actually trace to a bolt
        ...
        if (elapsedTime >= 600 && elapsedTime <= 900) { // left leg
          doKick2 = qtrue;
          if (ent->footLBolt != -1) { // actually trace to a bolt
            G_GetBoltPosition(ent, ent->footLBolt, kickEnd2);
            VectorSubtract(kickEnd2, ent->currentOrigin, kickDir2);
      ...
      case BOTH_BUTTERFLY_FR1:
        if (elapsedTime >= 950 && elapsedTime <= 1300) { // lead leg
          int footBolt = (ent->client->ps.legsAnim == BOTH_BUTTERFLY_FL1)
                             ? ent->footRBolt
                             : ent->footLBolt; // mirrored anims
          doKick = qtrue;
          if (footBolt != -1) { // actually trace to a bolt
        ...
        if (elapsedTime >= 1150 && elapsedTime <= 1600) { // trailing leg
          int footBolt = (ent->client->ps.legsAnim == BOTH_BUTTERFLY_FL1)
                             ? ent->footLBolt
                             : ent->footRBolt; // mirrored anims
          doKick2 = qtrue;
      ...
      case BOTH_BUTTERFLY_RIGHT:
        if ((elapsedTime >= 100 && elapsedTime <= 450) ||
            (elapsedTime >= 1100 && elapsedTime <= 1350)) { // lead leg
          int footBolt = (ent->client->ps.legsAnim == BOTH_BUTTERFLY_LEFT)
                             ? ent->footLBolt
                             : ent->footRBolt; // mirrored anims
          doKick = qtrue;
          if (footBolt != -1) { // actually trace to a bolt
        ...
        if (elapsedTime >= 900 && elapsedTime <= 1600) { // trailing leg
          int footBolt = (ent->client->ps.legsAnim == BOTH_BUTTERFLY_LEFT)
                             ? ent->footRBolt
                             : ent->footLBolt; // mirrored anims
          doKick2 = qtrue;
      ...
      case BOTH_GETUP_FROLL_F:
        ...
        if (ent->client->ps.legsAnim == BOTH_GETUP_BROLL_B ||
            ent->client->ps.legsAnim ==
                BOTH_GETUP_FROLL_B) { // rolling back, pull back the view
          G_CamPullBackForLegsAnim(ent);
        }
        break;
      case BOTH_A7_KICK_F_AIR:
        ...
        if (elapsedTime >= 100 && remainingTime >= 500) { // front
          doKick = qtrue;
          if (ent->footRBolt != -1) { // actually trace to a bolt
            G_GetBoltPosition(ent, ent->footRBolt, kickEnd);
            VectorSubtract(kickEnd, ent->currentOrigin, kickDir);
      ...
      case BOTH_A7_KICK_R_AIR:
        kickPush = Q_flrand(150.0f, 250.0f); // Q_flrand( 75.0f, 125.0f );
        kickSoundOnWalls = qtrue;
        if (elapsedTime >= 150 && remainingTime >= 300) { // left
          doKick = qtrue;
          if (ent->footLBolt != -1) { // actually trace to a bolt
      ...
      case BOTH_A7_KICK_BF:
        ...
        if (elapsedTime < 1500) {            // auto-aim!
          overridAngles = PM_AdjustAnglesForBFKick(ent, ucmd, fwdAngs,
                                                   qboolean(elapsedTime < 850))
                              ? qtrue
                              : overridAngles;
          // FIXME: if we haven't done the back kick yet and there's no-one
          // there to 			kick anymore, go into some anim that returns us to our base
          //stance
        }
        if (ent->footRBolt != -1) { // actually trace to a bolt
      ...
      case BOTH_A7_KICK_RL:
        ...
        // qboolean(elapsedTime<850) )?qtrue:overridAngles;
        if (elapsedTime >= 250 && elapsedTime < 350) { // right
          doKick = qtrue;
          if (ent->footRBolt != -1) { // actually trace to a bolt
            G_GetBoltPosition(ent, ent->footRBolt, kickEnd);
            VectorSubtract(kickEnd, ent->currentOrigin, kickDir);
        ...
        } else if (elapsedTime >= 350 && elapsedTime < 450) { // left
          doKick = qtrue;
          if (ent->footLBolt != -1) { // actually trace to a bolt
            G_GetBoltPosition(ent, ent->footLBolt, kickEnd);
            VectorSubtract(kickEnd, ent->currentOrigin, kickDir);
    ...
    if (doKick2) {
      G_KickTrace(ent, kickDir2, kickDist2, kickEnd2, kickDamage2, kickPush2,
                  kickSoundOnWalls);
    }
  } else if (ent->client->ps.saberMove == LS_DUAL_FB) {
    // pull back the view
    G_CamPullBackForLegsAnim(ent);
  } else if (ent->client->ps.saberMove == LS_A_BACK ||
             ent->client->ps.saberMove == LS_A_BACK_CR ||
             ent->client->ps.saberMove ==
                 LS_A_BACKSTAB) { // can't move or turn during back attacks
    ucmd->forwardmove = ucmd->rightmove = 0;
    if (ent->NPC) {
      VectorClear(ent->client->ps.moveDir);
    }
    if ((overridAngles = (PM_AdjustAnglesForBackAttack(ent, ucmd)
                              ? qtrue
                              : overridAngles)) == qtrue) {
      // pull back the view
      G_CamPullBackForLegsAnim(ent);
    ...
    if (!ent->s.number) {
      ...
      float backDist = 0;
      if (elapsedTime <= 300.0f) { // starting anim
        backDist = (elapsedTime / 300.0f) * 90.0f;
      } else if (ent->client->ps.torsoAnimTimer <= 300.0f) { // ending anim
        backDist = (ent->client->ps.torsoAnimTimer / 300.0f) * 90.0f;
      } else { // in middle of anim
        backDist = 90.0f;
      }
      // back off and look down
      cg.overrides.active |=
          (CG_OVERRIDE_3RD_PERSON_RNG | CG_OVERRIDE_3RD_PERSON_POF);
      cg.overrides.thirdPersonRange = cg_thirdPersonRange.value + backDist;
      cg.overrides.thirdPersonPitchOffset =
          cg_thirdPersonPitchOffset.value + (backDist / 2.0f);
    }
    overridAngles =
        (PM_AdjustAnglesForSpinProtect(ent, ucmd) ? qtrue : overridAngles);
  } else if (ent->client->ps.legsAnim == BOTH_A3_SPECIAL) { // push forward
    float animLength = PM_AnimLength(ent->client->clientInfo.animFileIndex,
                                     (animNumber_t)ent->client->ps.torsoAnim);
    float elapsedTime = (float)(animLength - ent->client->ps.torsoAnimTimer);
    ucmd->upmove = ucmd->rightmove = 0;
    ucmd->forwardmove = 0;
    if (elapsedTime >= 350 && elapsedTime < 1500) { // push forward
      ucmd->forwardmove = 64;
      ent->client->ps.speed = 200.0f;
    }
    // FIXME: pull back camera?
  } else if (ent->client->ps.legsAnim == BOTH_A2_SPECIAL) { // push forward
    ucmd->upmove = ucmd->rightmove = 0;
    ucmd->forwardmove = 0;
    if (ent->client->ps.legsAnimTimer > 200.0f) {
      ...
      float elapsedTime = (float)(animLength - ent->client->ps.torsoAnimTimer);
      if (elapsedTime < 750 ||
          (elapsedTime >= 1650 && elapsedTime < 2400)) { // push forward
        ucmd->forwardmove = 64;
        ent->client->ps.speed = 200.0f;
      }
    }
    // FIXME: pull back camera?
           ...
           (ucmd->forwardmove || ucmd->rightmove ||
            (VectorCompare(ent->client->ps.moveDir, vec3_origin) &&
             ent->client->ps.speed >
                 0))) { // moving during full-body fast special
    ent->client->ps.legsAnimTimer =
        0; // don't hold this legsAnim, allow them to run
    // FIXME: just add this to the list of overridable special moves in
    // PM_Footsteps?
  } else if (ent->client->ps.legsAnim ==
             BOTH_FLIP_LAND) { // moving during full-body fast special
    float animLength = PM_AnimLength(ent->client->clientInfo.animFileIndex,
                                     (animNumber_t)ent->client->ps.torsoAnim);
    float elapsedTime = (float)(animLength - ent->client->ps.torsoAnimTimer);
    ...
    if (elapsedTime > 600 && elapsedTime < 800 &&
        ent->client->ps.groundEntityNum !=
            ENTITYNUM_NONE) { // jump - FIXME: how do we stop double-jumps?
      ...
      G_AddEvent(ent, EV_JUMP, 0);
             ...
             ent->client->ps.torsoAnimTimer) { // can't move or turn
    ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
    if (ent->NPC) {
      VectorClear(ent->client->ps.moveDir);
      ent->client->ps.forceJumpCharge = 0;
    ...
    if ((ucmd->buttons & BUTTON_ATTACK) ||
        ...
        (ucmd->buttons & BUTTON_FORCE_DRAIN) || ucmd->upmove) { // stop the anim
      if (ent->client->ps.legsAnim == BOTH_MEDITATE &&
          ent->client->ps.torsoAnim == BOTH_MEDITATE) {
        NPC_SetAnim(ent, SETANIM_BOTH, BOTH_MEDITATE_END,
  ...
  if (PM_InRoll(&ent->client->ps)) {
    if (ent->s.number >= MAX_CLIENTS || !player_locked) {
      // FIXME: NPCs should try to face us during this roll, so they roll around
      // us...?
      PM_CmdForRoll(&ent->client->ps, ucmd);
      if (ent->s.number >=
          MAX_CLIENTS) { // make sure it doesn't roll me off a ledge
        if (!G_CheckRollSafety(
                ent, ent->client->ps.legsAnim,
                24)) { // crap!  I guess all we can do is stop...  UGH
  ...
  if (PM_InCartwheel(
          ent->client->ps.legsAnim)) { // can't keep moving in cartwheel
    if (ent->client->ps.legsAnimTimer >
        100) { // still have time left in the anim
      ucmd->forwardmove = ucmd->rightmove = ucmd->upmove = 0;
      if (ent->NPC) { // invalid now
        VectorClear(ent->client->ps.moveDir);
      }
  ...
  overridAngles =
      (PM_AdjustAnglesForStabDown(ent, ucmd) ? qtrue : overridAngles);
  overridAngles =
      (PM_AdjustAngleForWallJump(ent, ucmd, qtrue) ? qtrue : overridAngles);
  overridAngles =
      (PM_AdjustAngleForWallRunUp(ent, ucmd, qtrue) ? qtrue : overridAngles);
  overridAngles =
      (PM_AdjustAngleForWallRun(ent, ucmd, qtrue) ? qtrue : overridAngles);
...
static int NPC_GetWalkSpeed(gentity_t *ent) {
  ...
  case TEAM_PLAYER: // To shutup compiler, will add entries later (this is stub
                    // code)
  default:
...
void G_HeldByMonster(gentity_t *ent, usercmd_t **ucmd) {
  if (ent && ent->activator && ent->activator->inuse &&
      ent->activator->health > 0) {
    ...
    if (monster->count ==
        1) { // being held in hand rather than the mouth, so use *that* bolt
      boltIndex = monster->handRBolt;
    }
    vec3_t monAngles = {0};
    monAngles[YAW] =
        monster->currentAngles[YAW]; // only use YAW when passing angles to G2
    gi.G2API_GetBoltMatrix(monster->ghoul2, monster->playerModel, boltIndex,
                           &boltMatrix, monAngles, monster->currentOrigin,
                           (cg.time ? cg.time : level.time), NULL,
                           monster->s.modelScale);
    ...
    if (monster->client && monster->client->NPC_class ==
                               CLASS_WAMPA) { // can only hit attack button
      (*ucmd)->buttons &= ~((*ucmd)->buttons & ~BUTTON_ATTACK);
    }
  } else if (ent) { // doh, my captor died!
    ent->activator = NULL;
    if (ent->client) {
...
void G_CheckClientIdle(gentity_t *ent, usercmd_t *ucmd) {
  ...
  if (!VectorCompare(vec3_origin, ent->client->ps.velocity) || ucmd->buttons ||
      ucmd->forwardmove || ucmd->rightmove || ucmd->upmove ||
      !PM_StandingAnim(ent->client->ps.legsAnim) || ent->enemy ||
      ent->client->ps.legsAnimTimer ||
      ent->client->ps.torsoAnimTimer) { // FIXME: also check for turning?
    ...
    if (idleAnim != -1 && PM_HasAnimation(ent, idleAnim)) {
      NPC_SetAnim(ent, SETANIM_BOTH, idleAnim,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      // don't idle again after this anim for a while
      ent->client->idleTime =
          level.time +
          PM_AnimLength(ent->client->clientInfo.animFileIndex,
                        (animNumber_t)idleAnim) +
          Q_irand(0, 2000);
    }
  }
}
...
This function is called ONLY from ClientThinkReal, and is responsible for
setting client ps.speed
==============
*/
void ClientAlterSpeed(gentity_t *ent, usercmd_t *ucmd,
                      qboolean controlledByPlayer,
  ...
  if (ent->NPC != NULL && ent->client &&
      (ent->s.m_iVehicleNum !=
       0) /*&& ent->client->NPC_class == CLASS_VEHICLE*/) { // we don't actually
                                                            // scale the ucmd, we
                                                            // use actual speeds
    // FIXME: swoop should keep turning (and moving forward?) for a little bit?
    if (ent->NPC->combatMove == qfalse) {
      if (!(ucmd->buttons & BUTTON_USE)) { // Not leaning
        qboolean Flying =
            (qboolean)(ucmd->upmove && ent->client->moveType == MT_FLYSWIM);
        qboolean Climbing =
            (qboolean)(ucmd->upmove && ent->watertype & CONTENTS_LADDER);
        ...
        if (ent->NPC->currentSpeed <= 24 &&
          ...
          if (player && player->client &&
            ...
            // desiredYaw
            if (client->renderInfo.renderFlags &
                RF_LOCKEDANGLE) // yeah I know the RF_ flag is a pretty ugly
                                // hack...
            {
              turndelta = (180 - fabs(AngleDelta(ent->currentAngles[YAW],
  ...
  } else { // Client sets ucmds and such for speed alterations
    {
      ...
      if (ent->client->ps.heldClient < ENTITYNUM_WORLD) {
      ...
      } else if (PM_SaberInAttack(ent->client->ps.saberMove) &&
                 ucmd->forwardmove < 0) { // if running backwards while
                                          // attacking, don't run as fast.
      ...
      } else if (PM_LeapingSaberAnim(
                     client->ps.legsAnim)) { // no mod on speed when leaping
                                             // FIXME: maybe jump?
        ...
        if (g_saberMoveSpeed->value != 1.0f) {
          client->ps.speed *= g_saberMoveSpeed->value;
                 ...
                 (ucmd->buttons &
                  BUTTON_ATTACK)) { // if attacking with saber while running,
                                    // drop your speed
  ...
  if (client->NPC_class == CLASS_ATST &&
      client->ps.legsAnim == BOTH_RUN1START) { // HACK: when starting to move as
                                               // atst, ramp up speed
    // float animLength = PM_AnimLength( client->clientInfo.animFileIndex,
    // (animNumber_t)client->ps.legsAnim); client->ps.speed *= ( animLength -
    // client->ps.legsAnimTimer)/animLength;
  ...
  if (!pVeh) {
    if (ucmd->forwardmove < 0 && !(ucmd->buttons & BUTTON_WALKING) &&
        client->ps.groundEntityNum !=
            ENTITYNUM_NONE) { // running backwards is slower than running
                              // forwards
    ...
    if (client->ps.weapon == WP_SABER) {
      ...
      if (client->ps.dualSabers && client->ps.saber[1].moveSpeedScale != 1.0f) {
...
void ClientThink_real(gentity_t *ent, usercmd_t *ucmd) {
  ...
  if (ent->s.number == 0) {
    ...
    if (ent->client->renderInfo.lookTargetClearTime <
            level.time // NOTE: here this is used as a debounce, not an actual
                       // timer
        && ent->health > 0 // must be alive
        && (!ent->enemy ||
            ent->client->ps.saberMove !=
                LS_A_BACKSTAB)) // don't update if in backstab unless don't
                                // currently have an enemy
    { // NOTE: doesn't keep updating to nearest enemy once you're dead
      ...
      if (ent->enemy) { // target
        ...
        if (newLookTarget != ENTITYNUM_NONE) { // found something of interest
          ent->client->renderInfo.lookMode = LM_INTEREST;
        } else { // okay, no interesting things and no enemies, so look for
                 // items
          newLookTarget = G_FindLookItem(ent);
          ent->client->renderInfo.lookMode = LM_ENT;
        }
      }
      if (ent->client->renderInfo.lookTarget != newLookTarget) { // transitioning
        NPC_SetLookTarget(ent, newLookTarget, level.time + 1000);
      }
    }
    if (in_camera) {
      // watch the code here, you MUST "return" within this IF(), *unless*
      // you're stopping the cinematic skip.
      //
      if (ClientCinematicThink(ent->client)) {
        if (g_skippingcin->integer) // already doing cinematic skip?
        {                           // yes...   so stop skipping...
          G_StopCinematicSkip();
        } else { // no... so start skipping...
    ...
    // If he's riding the vehicle...
      ...
      if (g_skippingcin
              ->integer) { // We're skipping the cinematic and it's over now
        gi.cvar_set("timescale", "1");
        gi.cvar_set("skippingCinematic", "0");
      }
      if (ent->client->ps.pm_type == PM_DEAD &&
          cg.missionStatusDeadTime <
              level.time) { // mission status screen is up because player is
                            // dead, stop all scripts
        stop_icarus = qtrue;
      }
    }

    //		// Don't allow the player to adjust the pitch when they are in third
    //person overhead cam. extern vmCvar_t cg_thirdPerson; 		if (
    //cg_thirdPerson.integer == 2 )
    //		{
    //			ucmd->angles[PITCH] = 0;
    //		}

    if (cg.zoomMode == 2) {
      // Any kind of movement when the player is NOT ducked when the disruptor
      // gun is zoomed will cause us to auto-magically un-zoom
    ...
    if ((player_locked || (ent->client->ps.eFlags & EF_FORCE_GRIPPED) ||
    ...
    if (ent->client->ps.leanofs) { // no shooting while leaning
      ucmd->buttons &= ~BUTTON_ATTACK;
      if (ent->client->ps.weapon !=
          WP_DISRUPTOR) { // can still zoom around corners
        ucmd->buttons &= ~BUTTON_ALT_ATTACK;
      }
    }
  } else {
  ...
  if (pVeh &&
      (pVeh->m_pVehicleInfo->Inhabited(pVeh) || pVeh->m_iBoarding != 0 ||
       pVeh->m_pVehicleInfo->type != VH_ANIMAL)) {
    pVeh->m_pVehicleInfo->Update(pVeh, ucmd);
  } else if (ent->client) { // this is any client that is not a vehicle (OR: is
                            // a vehicle and it not being ridden, is not being
                            // boarded, or is a TaunTaun...!
    if (ent->client->NPC_class == CLASS_GONK ||
        ent->client->NPC_class == CLASS_MOUSE ||
        ent->client->NPC_class == CLASS_R2D2 ||
        ent->client->NPC_class ==
            CLASS_R5D2) { // no jumping or strafing in these guys
      ucmd->upmove = ucmd->rightmove = 0;
    } else if (ent->client->NPC_class == CLASS_ATST ||
               ent->client->NPC_class == CLASS_RANCOR) { // no jumping in atst
      ...
      if (ent->client->ps.groundEntityNum !=
        ...
        if (under && under->health && under->takedamage) {
          ...
          // real impact origin
          G_Damage(under, ent, ent, down, under->currentOrigin, 100, 0,
                   MOD_CRUSH);
        }
        ...
        if (ent->client->NPC_class != CLASS_RANCOR &&
            !Q_irand(0, 10)) { // not so often...
          AddSoundEvent(ent, ent->currentOrigin, ent->maxs[1] * 5, AEL_DANGER,
                        qfalse, qtrue);
          AddSightEvent(ent, ent->currentOrigin, ent->maxs[1] * 5, AEL_DANGER,
                        100);
        }
      }
    } else if (ent->client->ps.groundEntityNum < ENTITYNUM_WORLD &&
               !ent->client->ps.forceJumpCharge) { // standing on an entity and
                                                   // not currently force jumping
      ...
      if (groundEnt && groundEnt->client) {
        ...
        if (groundEnt->client &&
            groundEnt->client->NPC_class == CLASS_VEHICLE) {
          if (ent->client->NPC_class !=
              CLASS_VEHICLE) { // um... vehicles shouldn't ride other vehicles,
                               // mmkay?
            if ((groundEnt->m_pVehicle->m_pVehicleInfo->type == VH_ANIMAL &&
                 PM_HasAnimation(ent, BOTH_VT_IDLE)) ||
                (groundEnt->m_pVehicle->m_pVehicleInfo->type == VH_SPEEDER &&
                 PM_HasAnimation(ent, BOTH_VS_IDLE))) {
              // groundEnt->m_pVehicle->m_iBoarding = -3;	// Land From
              // Behind
              groundEnt->m_pVehicle->m_pVehicleInfo->Board(
                  groundEnt->m_pVehicle, ent);
            }
          }
        } else if (groundEnt->client &&
                   groundEnt->client->NPC_class == CLASS_SAND_CREATURE &&
                   G_HasKnockdownAnims(
                       ent)) { // step on a sand creature = *you* fall down
          G_Knockdown(ent, groundEnt, vec3_origin, 300, qtrue);
        } else if (groundEnt->client &&
                   groundEnt->client->NPC_class ==
                       CLASS_RANCOR) { // step on a Rancor, it bucks & throws
                                       // you off
          if (groundEnt->client->ps.legsAnim != BOTH_ATTACK3 &&
              groundEnt->client->ps.legsAnim != BOTH_ATTACK4 &&
              groundEnt->client->ps.legsAnim !=
                  BOTH_BUCK_RIDER) { // don't interrupt special anims
        ...
        { // landed on a live client who is on the ground, jump off them and
          // knock them down
          ...
          } else if (ent->s.number && ent->NPC &&
                     ent->client->ps.forcePowerLevel[FP_LEVITATION] >
                         FORCE_LEVEL_0) // ent->s.weapon == WP_SABER )
          {                             // force-jumper landed on someone
            // don't jump off, too many ledges, plus looks weird
            if (groundEnt->client->playerTeam !=
                ent->client->playerTeam) { // don't knock down own guys
              forceKnockdown =
                  (qboolean)(Q_irand(0, RANK_CAPTAIN + 4) < ent->NPC->rank);
            }
            // now what... push the groundEnt out of the way?
            if (!ent->client->ps.velocity[0] &&
                !ent->client->ps.velocity[1]) { // not moving, shove us a little
              vec3_t slideFwd;
            ...
            if (!PM_InRoll(&ent->client->ps) &&
              ...
              if (ent->s.number && ent->s.weapon == WP_SABER) {
                ent->client->ps.forceJumpCharge =
                    320;                  // FIXME: calc this intelligently?
              } else if (!ucmd->upmove) { // if not ducking (which should cause
                                          // a roll), then jump
                ucmd->upmove = 127;
              }
              if (!ucmd->forwardmove &&
                  !ucmd->rightmove) { //  If not moving, don't want to jump
                                      //  straight up
              ...
              if (!ent->s.number &&
                  ucmd->upmove < 0) { // player who should roll- force it
                ...
                NPC_SetAnim(ent, SETANIM_BOTH, rollAnim,
                            SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
          ...
          GEntity_PainFunc(groundEnt, ent, ent, groundEnt->currentOrigin, 0,
                           MOD_CRUSH);
          if (!forceKnockdown && groundEnt->client->NPC_class == CLASS_DESANN &&
              ent->client->NPC_class !=
                  CLASS_LUKE) { // can't knock down desann unless you're luke
                                // FIXME: should he smack you away like Galak
                                // Mech?
          } else if (forceKnockdown // forced
                     || ent->client->NPC_class ==
                            CLASS_DESANN // desann always knocks people down
                     ||
                     (((groundEnt->s.number &&
                        (groundEnt->s.weapon != WP_SABER || !groundEnt->NPC ||
                         groundEnt->NPC->rank <
                             Q_irand(RANK_CIVILIAN,
                                     RANK_CAPTAIN +
                                         1))) // an NPC who is either not a
                                              // saber user or passed the
                                              // rank-based probability test
                       ||
                       ((!ent->s.number || G_ControlledByPlayer(groundEnt)) &&
                        !Q_irand(0, 3) && cg.renderingThirdPerson &&
                        !cg.zoomMode)) // or a player in third person, 25% of
                                       // the time
                      && groundEnt->client->playerTeam !=
                             ent->client->playerTeam // and not on the same team
                      && ent->client->ps.legsAnim !=
                             BOTH_JUMPATTACK6)) // not in the sideways-spinning
                                                // jump attack
          {
            int knockAnim = BOTH_KNOCKDOWN1;
            if (PM_CrouchAnim(
                    groundEnt->client->ps.legsAnim)) { // knockdown from crouch
              knockAnim = BOTH_KNOCKDOWN4;
            } else {
              vec3_t gEFwd,
                  gEAngles = {0, groundEnt->client->ps.viewangles[YAW], 0};
              AngleVectors(gEAngles, gEFwd, NULL, NULL);
              if (DotProduct(ent->client->ps.velocity, gEFwd) >
                  50) { // pushing him forward
                knockAnim = BOTH_KNOCKDOWN3;
              }
            }
            NPC_SetAnim(groundEnt, SETANIM_BOTH, knockAnim,
                        SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
          }
        }
      }
  ...
  // FIXME: if global gravity changes this should update everyone's personal
  // gravity...
  if (!(ent->svFlags & SVF_CUSTOM_GRAVITY)) {
    if (ent->client->inSpaceIndex) { // in space, so no gravity...
  ...
  // NEED to do this every frame, since these overrides do not go into the
  // save/load data
  if (ent->client && ent->s.m_iVehicleNum != 0 && !ent->s.number &&
      !MatrixMode) { // FIXME: extern and read from g_vehicleInfo?
    Vehicle_t *pPlayerVeh = ent->owner->m_pVehicle;
    if (pPlayerVeh && pPlayerVeh->m_pVehicleInfo->cameraOverride) {
      // Vehicle Camera Overrides
      //--------------------------
      cg.overrides.active |=
          (CG_OVERRIDE_3RD_PERSON_RNG | CG_OVERRIDE_FOV |
           CG_OVERRIDE_3RD_PERSON_VOF | CG_OVERRIDE_3RD_PERSON_POF);
      ...
      cg.overrides.thirdPersonVertOffset =
          pPlayerVeh->m_pVehicleInfo->cameraVertOffset;
      cg.overrides.thirdPersonPitchOffset =
          pPlayerVeh->m_pVehicleInfo->cameraPitchOffset;
      ...
      //------------------------------------
      if ((level.time > pPlayerVeh->m_iTurboTime) &&
          pPlayerVeh->m_pVehicleInfo->type == VH_SPEEDER) {
        ...
        if ((pPlayerVeh->m_ucmd.rightmove != 0) &&
          ...
          cg.overrides.active |=
              CG_OVERRIDE_3RD_PERSON_ANG;      // Turn On Angle Offset
          cg.overrides.thirdPersonRange *= -2; // Camera In Front Of Player
          cg.overrides.thirdPersonAngle =
              (pPlayerVeh->m_ucmd.rightmove > 0) ? (20) : (-20);
        }
        ...
        //---------------------------------------
          ...
          cg.overrides.active &=
              ~CG_OVERRIDE_3RD_PERSON_ANG; // Turn Off Angle Offset
          if (ent->enemy) {
            vec3_t actorDirection;
            vec3_t enemyDirection;

            AngleVectors(ent->currentAngles, actorDirection, 0, 0);
            VectorSubtract(ent->enemy->currentOrigin, ent->currentOrigin,
                           enemyDirection);
            float enemyDistance = VectorNormalize(enemyDirection);

            if (enemyDistance > cg.overrides.thirdPersonRange &&
                enemyDistance < 400 &&
                DotProduct(actorDirection, enemyDirection) < -0.5f) {
    ...
    if (cg_thirdPersonAutoAlpha.integer) {
      if (ent->health > 0 && ent->client->ps.viewangles[PITCH] < 15 &&
        ...
        if (cg.overrides.thirdPersonAlpha > cg_thirdPersonAlpha.value) {
          cg.overrides.active &= ~CG_OVERRIDE_3RD_PERSON_APH;
        } else if (cg.overrides.thirdPersonAlpha <
                   cg_thirdPersonAlpha.value - 0.1f) {
    ...
    if (ent->client->ps.viewangles[PITCH] > 0) {
      cg.overrides.thirdPersonPitchOffset =
          ent->client->ps.viewangles[PITCH] * -0.75;
      cg.overrides.thirdPersonVertOffset =
          300 + ent->client->ps.viewangles[PITCH] * -10;
      ...
      cg.overrides.thirdPersonPitchOffset =
          ent->client->ps.viewangles[PITCH] * -0.75;
      cg.overrides.thirdPersonVertOffset =
          300 + ent->client->ps.viewangles[PITCH] * -5;
  ...
  if (ent->client->ps.eventSequence != oldEventSequence) {
    ent->eventTime = level.time;
    {
      ...
      ent->s.event = ent->client->ps.events[seq] |
                     ((ent->client->ps.eventSequence & 3) << 8);
  ...
  if (client->ps.stats[STAT_HEALTH] <= 0) {
    ...
    if (ent && !ent->s.number && ent->enemy && ent->enemy != ent &&
        ent->enemy->s.number < ENTITYNUM_WORLD && ent->enemy->inuse &&
        !(cg.overrides.active &
          CG_OVERRIDE_3RD_PERSON_ANG)) { // keep facing enemy
      vec3_t deadDir;
      float deadYaw;
...
void ClientThink(int clientNum, usercmd_t *ucmd) {
  ...
  if (ent->s.number < MAX_CLIENTS) {
    if (ent->client->ps.viewEntity > 0 &&
        ent->client->ps.viewEntity <
            ENTITYNUM_WORLD) { // you're controlling another NPC
      gentity_t *controlled = &g_entities[ent->client->ps.viewEntity];
      qboolean freed = qfalse;
      if (controlled->NPC && controlled->NPC->controlledTime &&
          ent->client->ps.forcePowerLevel[FP_TELEPATHY] >
              FORCE_LEVEL_3) { // An NPC I'm controlling with mind trick
        if (controlled->NPC->controlledTime < level.time) { // time's up!
          G_ClearViewEntity(ent);
          freed = qtrue;
        } else if (ucmd->upmove >
                   0) { // jumping gets you out of it FIXME: check some other
                        // button instead... like ESCAPE... so you could even
                        // have total control over an NPC?
          G_ClearViewEntity(ent);
          ...
          freed = qtrue;
                 ...
                 && controlled->client->moveType !=
                        MT_FLYSWIM) // and does not use upmove to fly
      {                             // these types use jump to get out
        if (ucmd->upmove > 0) { // jumping gets you out of it FIXME: check some
                                // other button instead... like ESCAPE... so you
                                // could even have total control over an NPC?
          G_ClearViewEntity(ent);
          ucmd->upmove = 0; // ucmd->buttons = 0;
          // stop player from doing anything for a half second after
          ent->aimDebounceTime = level.time + 500;
      ...
      if (!freed) { // still controlling, save off my ucmd and clear it for my
                    // actual run through pmove
        restore_ucmd = qtrue;
        memcpy(&sav_ucmd, ucmd, sizeof(usercmd_t));
        memset(ucmd, 0, sizeof(usercmd_t));
        // to keep pointing in same dir, need to set ucmd->angles
        ucmd->angles[PITCH] = ANGLE2SHORT(ent->client->ps.viewangles[PITCH]) -
                              ent->client->ps.delta_angles[PITCH];
        ucmd->angles[YAW] = ANGLE2SHORT(ent->client->ps.viewangles[YAW]) -
      ...
      if (ucmd->upmove > 0) { // get out of ATST
  ...
  // Rider logic.
  // NOTE: Maybe this should be extracted into a RiderUpdate() within the
  // vehicle.
  if ((pVeh = G_IsRidingVehicle(ent)) != 0) {
    ...
    if (pVeh->m_pVehicleInfo->UpdateRider(pVeh, ent, ucmd)) {
      ...
      // to keep pointing in same dir, need to set ucmd->angles
      // ucmd->angles[PITCH] = ANGLE2SHORT( ent->client->ps.viewangles[PITCH] )
      // - ent->client->ps.delta_angles[PITCH]; ucmd->angles[YAW] = ANGLE2SHORT(
      // ent->client->ps.viewangles[YAW] ) - ent->client->ps.delta_angles[YAW];
      // ucmd->angles[ROLL] = 0;
      ucmd->angles[PITCH] = sav_ucmd.angles[PITCH];
      ucmd->angles[YAW] = sav_ucmd.angles[YAW];
      ucmd->angles[ROLL] = sav_ucmd.angles[ROLL];
      // if ( sav_ucmd.weapon != ent->client->ps.weapon && PM_WeaponOkOnVehicle(
      // sav_ucmd.weapon ) )
      { // trying to change weapons to a valid weapon for this vehicle, to
        // preserve this weapon change command
      ...
      { // keep our current weapon
        ...
        { // not changing weapons and we are using one of our weapons, not using
          // vehicle weapon
  ...
  // If a vehicle, make sure to attach our driver and passengers here (after we
  // pmove, which is done in Think_Real))
  if (ent->client && ent->client->NPC_class == CLASS_VEHICLE) {
    pVeh = ent->m_pVehicle;
  ...
  if (restore_ucmd && ent->client) { // restore ucmd for later so NPC you're
                                     // controlling can refer to them
    memcpy(&ent->client->usercmd, &sav_ucmd, sizeof(usercmd_t));
  }

  if (ent->s.number) { // NPCs drown, burn from lava, etc, also
    P_WorldEffects(ent);
  }
}
