...
stringID_table_t VehicleTable[VH_NUM_VEHICLES + 1] = {ENUM2STRING(VH_WALKER),  // something you ride inside of, it walks like you, like an AT-ST
...
void G_VehicleSetDefaults(vehicleInfo_t *vehicle) {
	...
	/*
	   ...
	   is 0 vehicle->cameraPitchOffset = 0.0f;					//a modifier on the camera's pitch (up/down angle) to the vehicle - normal is 0
...
void G_VehicleClampData(vehicleInfo_t *vehicle) { // sanity check and clamp the vehicle's data
	int i;
...
static void G_VehicleStoreParms(const char *p) { // load up all into a table: g_vehicleInfo
	const char *token;
...
void G_VehicleLoadParms(void) { // HMM... only do this if there's a vehicle on the level?
