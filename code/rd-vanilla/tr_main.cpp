  ...
  for (i = 0; i < 3; i++) {
    ...
    for (j = 0; j < 3; j++) {
      destmat[temp + 3] -=
          ...
          sourcemat
              [j * 4 +
               3]; // dest->matrix[i][3]-=dest->matrix[i][j]*src->matrix[j][3];
    }
  }
}
...
void R_WorldNormalToEntity(const vec3_t worldvec, vec3_t entvec) {
  entvec[0] = -worldvec[0] * preTransEntMatrix[0] -
              worldvec[1] * preTransEntMatrix[4] +
              worldvec[2] * preTransEntMatrix[8];
  entvec[1] = -worldvec[0] * preTransEntMatrix[1] -
              worldvec[1] * preTransEntMatrix[5] +
              worldvec[2] * preTransEntMatrix[9];
  entvec[2] = -worldvec[0] * preTransEntMatrix[2] -
              worldvec[1] * preTransEntMatrix[6] +
              worldvec[2] * preTransEntMatrix[10];
}
...
{
        entvec[0] = worldvec[0] * preTransEntMatrix[0] + worldvec[1] *
preTransEntMatrix[4] + worldvec[2] * preTransEntMatrix[8]+preTransEntMatrix[12];
        entvec[1] = worldvec[0] * preTransEntMatrix[1] + worldvec[1] *
preTransEntMatrix[5] + worldvec[2] * preTransEntMatrix[9]+preTransEntMatrix[13];
        entvec[2] = worldvec[0] * preTransEntMatrix[2] + worldvec[1] *
preTransEntMatrix[6] + worldvec[2] *
preTransEntMatrix[10]+preTransEntMatrix[14];
}
*/
...
void R_RotateForEntity(const trRefEntity_t *ent, const viewParms_t *viewParms,
  ...
  myGlMultMatrix(preTransEntMatrix, viewParms->world.modelMatrix,
                 ori->modelMatrix);
...
static void SetFarClip(void) {
  ...
  tr.viewParms.zFar = Com_Clamp(2048.0f, tr.distanceCull * (1.732),
...
void R_SetupFrustum(void) {
  ...
  VectorMA(tr.viewParms.frustum[0].normal, xc, tr.viewParms.ori.axis[1],
           tr.viewParms.frustum[0].normal);

  VectorScale(tr.viewParms.ori.axis[0], xs, tr.viewParms.frustum[1].normal);
  VectorMA(tr.viewParms.frustum[1].normal, -xc, tr.viewParms.ori.axis[1],
  ...
  VectorMA(tr.viewParms.frustum[2].normal, xc, tr.viewParms.ori.axis[2],
           tr.viewParms.frustum[2].normal);

  VectorScale(tr.viewParms.ori.axis[0], xs, tr.viewParms.frustum[3].normal);
  VectorMA(tr.viewParms.frustum[3].normal, -xc, tr.viewParms.ori.axis[2],
  ...
  for (i = 0; i < 5; i++) {
    ...
    tr.viewParms.frustum[i].dist =
        DotProduct(tr.viewParms.ori.origin, tr.viewParms.frustum[i].normal);
    if (i == 4) {
      // far plane does not go through the view point, it goes alot farther..
      tr.viewParms.frustum[i].dist -=
          tr.distanceCull * 1.02f; // a little slack so we don't cull stuff
    }
...
===============
*/
static QINLINE void R_Radix(int byte, int size, drawSurf_t *source,
                            drawSurf_t *dest) {
  int count[256] = {0};
  int index[256];
  int i;
...
void R_AddDrawSurf(const surfaceType_t *surface, const shader_t *shader,
  ...
  if ((shader->surfaceFlags & SURF_FORCESIGHT) &&
      !(tr.refdef.rdflags &
        RDF_ForceSightOn)) { // if shader is only seen with ForceSight and we
                             // don't have ForceSight on, then don't draw
    return;
  }
...
qboolean R_FogParmsMatch(int fog1, int fog2) {
  for (int i = 0; i < 2; i++) {
    if (tr.world->fogs[fog1].parms.color[i] !=
        tr.world->fogs[fog2].parms.color[i]) {
      return qfalse;
    }
  }
  return qtrue;
...
void R_SetViewFogIndex(void) {
  if (tr.world->numfogs > 1) { // more than just the LA goggles
    fog_t *fog;
    int contents = ri.SV_PointContents(tr.refdef.vieworg, 0);
    if ((contents &
         CONTENTS_FOG)) { // only take a tr.refdef.fogIndex if the
                          // tr.refdef.vieworg is actually *in* that fog brush
                          // (assumption: checks pointcontents for any
                          // CONTENTS_FOG, not that particular brush...)
      for (tr.refdef.fogIndex = 1; tr.refdef.fogIndex < tr.world->numfogs;
           tr.refdef.fogIndex++) {
...
void R_RenderView(viewParms_t *parms) {
  ...
  if (!(tr.refdef.rdflags &
        RDF_NOWORLDMODEL)) { // Trying to do this with no world is not good.
    R_SetViewFogIndex();
  }
