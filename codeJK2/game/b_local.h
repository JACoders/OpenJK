...
extern cvar_t *debugNPCFreeze; // set to disable NPC ai and temporarily freeze
                               // them in place
...
extern void Debug_NPCPrintf(gentity_t *printNPC, cvar_t *cv, int debugLevel,
                            char *fmt, ...);
...
extern void NPC_BSFlee(void);
extern void NPC_StartFlee(gentity_t *enemy, vec3_t dangerPoint, int dangerLevel,
                          int fleeTimeMin, int fleeTimeMax);
extern void G_StartFlee(gentity_t *self, gentity_t *enemy, vec3_t dangerPoint,
...
extern gentity_t *NPC_PickEnemy(gentity_t *closestTo, int enemyTeam,
                                qboolean checkVis, qboolean findPlayersFirst,
                                qboolean findClosest);
extern gentity_t *NPC_CheckEnemy(qboolean findNew, qboolean tooFarOk,
                                 qboolean setEnemy = qtrue);
...
extern qboolean ShotThroughGlass(trace_t *tr, gentity_t *target, vec3_t spot,
                                 int mask);
extern qboolean ValidEnemy(gentity_t *ent);
extern void G_ClearEnemy(gentity_t *self);
extern void G_SetEnemy(gentity_t *self, gentity_t *enemy);
extern gentity_t *NPC_PickAlly(qboolean facingEachOther, float range,
                               qboolean ignoreGroup, qboolean movingOnly);
...
extern qboolean InFOV(vec3_t spot, vec3_t from, vec3_t fromAngles, int hFOV,
                      int vFOV);
...
extern void CalcEntitySpot(const gentity_t *ent, const spot_t spot,
                           vec3_t point);
extern qboolean NPC_UpdateAngles(qboolean doPitch, qboolean doYaw);
extern void NPC_UpdateShootAngles(vec3_t angles, qboolean doPitch,
                                  qboolean doYaw);
...
// other modules
extern void CalcMuzzlePoint(gentity_t *const ent, vec3_t forward, vec3_t right,
                            vec3_t up, vec3_t muzzlePoint, float lead_in);
...
inline qboolean NPC_ClearLOS(const vec3_t end) { return G_ClearLOS(NPC, end); }
inline qboolean NPC_ClearLOS(gentity_t *ent) { return G_ClearLOS(NPC, ent); }
...
extern int NPC_FindCombatPoint(const vec3_t position,
                               const vec3_t avoidPosition, vec3_t enemyPosition,
                               const int flags, const float avoidDist,
                               const int ignorePoint = -1);

extern qboolean NPC_ReserveCombatPoint(int combatPointID);
extern qboolean NPC_FreeCombatPoint(int combatPointID,
...
#define CP_INVESTIGATE                                                         \
  0x00000040 // A special point worth enemy investigation if searching
...
#define CP_CLOSEST                                                             \
  0x00000400 // Take the closest combatPoint to the enemy that's available
...
#define CP_SNIPE 0x00002000     // Pick a combatPoint that is marked as a sniper
                            // spot
#define CP_SAFE 0x00004000 // Pick a combatPoint that is not have dangerTime
#define CP_HORZ_DIST_COLL                                                      \
  0x00008000                  // Collect combat points within *horizontal* dist
...
int NPC_CheckAlertEvents(qboolean checkSight, qboolean checkSound,
                         int ignoreAlert = -1, qboolean mustHaveOwner = qfalse,
...
qboolean NAV_HitNavGoal(vec3_t point, vec3_t mins, vec3_t maxs, vec3_t dest,
                        int radius, qboolean flying);

void NPC_SetMoveGoal(gentity_t *ent, vec3_t point, int radius,
                     qboolean isNavGoal = qfalse, int combatPoint = -1,
...
extern qboolean NAV_AvoidCollision(gentity_t *self, gentity_t *goal,
                                   navInfo_t &info);
