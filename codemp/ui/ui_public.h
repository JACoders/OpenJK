...
typedef enum uiImportLegacy_e {
  ...
  UI_G2_PLAYANIM,
  UI_G2_GETBONEANIM,
  UI_G2_GETBONEFRAME, // trimmed down version of GBA, so I don't have to pass
                      // all those unused args across the VM-exe border
  UI_G2_GETGLANAME,
...
typedef struct uiImport_s {
  ...
  void (*Cvar_Create)(const char *var_name, const char *var_value,
                      uint32_t flags);
  void (*Cvar_InfoStringBuffer)(int bit, char *buffer, int bufsize);
  void (*Cvar_Register)(vmCvar_t *cvar, const char *var_name, const char *value,
                        uint32_t flags);
  ...
  int (*LAN_CompareServers)(int source, int sortKey, int sortDir, int s1,
                            int s2);
  ...
  int (*LAN_ServerStatus)(const char *serverAddress, char *serverStatus,
                          int maxLen);
  ...
  qboolean (*G2API_AttachG2Model)(void *ghoul2From, int modelIndexFrom,
