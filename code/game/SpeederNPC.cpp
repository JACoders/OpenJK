...
bool VEH_StartStrafeRam(Vehicle_t *pVeh, bool Right) {
  if (!(pVeh->m_ulFlags & VEH_STRAFERAM)) {
    ...
    if (speed > 400.0f) {
      ...
      VectorMA(pVeh->m_pParentEntity->client->ps.velocity,
               (Right) ? (speed) : (-speed), right,
               pVeh->m_pParentEntity->pos3);

      pVeh->m_ulFlags |= VEH_STRAFERAM;
      pVeh->m_fStrafeTime =
          (Right) ? (STRAFERAM_DURATION) : (-STRAFERAM_DURATION);
...
bool Update(Vehicle_t *pVeh, const usercmd_t *pUcmd) {
  ...
  if (pVeh->m_ulFlags & VEH_FLYING) {
    ...
    vec3_t vVehAngles;
    VectorSet(vVehAngles, pVeh->m_vOrientation[PITCH],
              pVeh->m_vOrientation[YAW], 0);
    AngleVectors(vVehAngles, parent->client->ps.moveDir, NULL, NULL);
  }
  ...
  if (!(pVeh->m_ulFlags & VEH_STRAFERAM) && !(pVeh->m_ulFlags & VEH_FLYING)) {
    ...
    if (pVeh->m_ucmd.rightmove && !pVeh->m_fStrafeTime) {
      pVeh->m_fStrafeTime =
          (pVeh->m_ucmd.rightmove > 0) ? (level.time) : (-1 * level.time);
    }
  ...
  if (pVeh->m_pVehicleInfo->iExhaustFX) {
    ...
    if (pVeh->m_ucmd.forwardmove && !(pVeh->m_ulFlags & VEH_ACCELERATORON)) {
      ...
      for (int i = 0;
           (i < MAX_VEHICLE_EXHAUSTS && pVeh->m_iExhaustTag[i] != -1); i++) {
        G_PlayEffect(pVeh->m_pVehicleInfo->iExhaustFX, parent->playerModel,
                     pVeh->m_iExhaustTag[i], parent->s.number,
    ...
    //------------------------------
    else if (!pVeh->m_ucmd.forwardmove &&
             (pVeh->m_ulFlags & VEH_ACCELERATORON)) {
      pVeh->m_ulFlags &= ~VEH_ACCELERATORON;
      for (int i = 0;
           (i < MAX_VEHICLE_EXHAUSTS && pVeh->m_iExhaustTag[i] != -1); i++) {
        G_StopEffect(pVeh->m_pVehicleInfo->iExhaustFX, parent->playerModel,
                     pVeh->m_iExhaustTag[i], parent->s.number);
      }
    }
  }
  ...
  if (!(pVeh->m_ulFlags & VEH_ARMORLOW) &&
      (pVeh->m_iArmor <= pVeh->m_pVehicleInfo->armor / 3)) {
  ...
  if (pVeh->m_pVehicleInfo->iArmorGoneFX) {
    if (!(pVeh->m_ulFlags & VEH_ARMORGONE) && (pVeh->m_iArmor <= 0)) {
      ...
      G_PlayEffect(pVeh->m_pVehicleInfo->iArmorGoneFX, parent->playerModel,
                   parent->crotchBolt, parent->s.number, parent->currentOrigin,
                   1, qtrue);
...
static void ProcessMoveCommands(Vehicle_t *pVeh) {
  ...
  float speedInc, speedIdleDec, speedIdle, /*speedIdleAccel, */ speedMin,
      speedMax;
  ...
  // and restore the pitch to origin (straight) position (at 5% increments).
  if (pVeh->m_ulFlags & VEH_FLYING) {
    speedInc =
        pVeh->m_pVehicleInfo->acceleration * pVeh->m_fTimeModifier * 0.4f;
  }
#ifdef _JK2MP
  ...
  if ((pVeh->m_pPilot /*&& (pilotPS->weapon == WP_NONE || pilotPS->weapon ==
                         WP_MELEE )*/
       && (pVeh->m_ucmd.buttons & BUTTON_ALT_ATTACK) &&
       pVeh->m_pVehicleInfo->turboSpeed)
#ifdef _JK2MP
      || (parentPS && parentPS->electrifyTime > curTime &&
          pVeh->m_pVehicleInfo->turboSpeed) // make them go!
#endif
  ) {
#ifdef _JK2MP
        ...
        if (pVeh->m_pVehicleInfo->iTurboStartFX) {
          ...
          for (i = 0;
            ...
            if (pVeh->m_pVehicleInfo->iTurboFX) {
              G_PlayEffect(pVeh->m_pVehicleInfo->iTurboFX,
                           pVeh->m_pParentEntity->playerModel,
                           pVeh->m_iExhaustTag[i],
                           pVeh->m_pParentEntity->s.number,
                           pVeh->m_pParentEntity->currentOrigin,
                           pVeh->m_pVehicleInfo->turboDuration, qtrue);
            }
          ...
          if (pVeh->m_pParentEntity && pVeh->m_pParentEntity->ghoul2 &&
              pVeh->m_pParentEntity
                  ->playerState) { // fine, I'll use a tempent for this, but
                                   // only because it's played only once at the
                                   // start of a turbo.
            vec3_t boltOrg, boltDir;
            mdxaBone_t boltMatrix;

            VectorSet(boltDir, 0.0f,
                      pVeh->m_pParentEntity->playerState->viewangles[YAW],
                      0.0f);

            trap_G2API_GetBoltMatrix(
                pVeh->m_pParentEntity->ghoul2, 0, pVeh->m_iExhaustTag[i],
                &boltMatrix, boltDir,
                pVeh->m_pParentEntity->playerState->origin, level.time, NULL,
            ...
            G_PlayEffectID(pVeh->m_pVehicleInfo->iTurboStartFX, boltOrg,
                           boltDir);
          }
#endif
#endif
...
#endif
        parentPS->speed =
            pVeh->m_pVehicleInfo->turboSpeed; // Instantly Jump To Turbo Speed
      }
#ifdef _JK2MP
    }
#endif
  ...
  if (pVeh->m_ulFlags & VEH_SLIDEBREAKING) {
    ...
    parentPS->speed = 0;
             ...
             !(pVeh->m_ulFlags & VEH_FLYING) && pVeh->m_ucmd.forwardmove < 0 &&
...
#endif
    { // if in a strafe-capable vehicle, clear strafing unless using alternate
      // control scheme pVeh->m_ucmd.rightmove = 0;
    }
  }
...
#ifndef _JK2MP
  // In SP, The AI Pilots Can Directly Control The Speed Of Their Bike In Order
  // To Match The Speed Of The Person They Are Trying To Chase
  //-------------------------------------------------------------------------------
...
void ProcessOrientCommands(Vehicle_t *pVeh) {
  ...
  if (parentPS && parentPS->speed) {
    ...
    float maxDif =
        pVeh->m_pVehicleInfo->turningSpeed * 4.0f; // magic number hackery
    if (s < 0.0f) {
      s = -s;
    }
    ...
    if (parentPS->electrifyTime > pm->cmd.serverTime) { // do some crazy stuff
      pVeh->m_vOrientation[YAW] +=
          (sin(pm->cmd.serverTime / 1000.0f) * 3.0f) * pVeh->m_fTimeModifier;
    }
  }
  ...
  } else if ((pVeh->m_ulFlags &
              VEH_SLIDEBREAKING) || // No Angles Control While Out Of Control
             (pVeh->m_ulFlags &
              VEH_OUTOFCONTROL) // No Angles Control While Out Of Control
  ) {
    // Any ability to change orientation?
  } else if ((pVeh->m_ulFlags &
              VEH_STRAFERAM) // No Angles Control While Strafe Ramming
  ) {
    if (pVeh->m_fStrafeTime > 0) {
      pVeh->m_fStrafeTime--;
      pVeh->m_vOrientation[ROLL] +=
          (pVeh->m_fStrafeTime < (STRAFERAM_DURATION / 2)) ? (-STRAFERAM_ANGLE)
                                                           : (STRAFERAM_ANGLE);
    } else if (pVeh->m_fStrafeTime < 0) {
      pVeh->m_fStrafeTime++;
      pVeh->m_vOrientation[ROLL] +=
          (pVeh->m_fStrafeTime > (-STRAFERAM_DURATION / 2))
              ? (STRAFERAM_ANGLE)
              : (-STRAFERAM_ANGLE);
    }
  } else {
...
// I want to keep this function BG too, because it's fairly generic already, and
// it would be nice to have proper prediction of animations. -rww
...
void AnimateRiders(Vehicle_t *pVeh) {
  ...
  if (pVeh->m_iBoarding != 0) {
    // We've just started moarding, set the amount of time it will take to
    // finish moarding.
    if (pVeh->m_iBoarding < 0) {
      int iAnimLen;
      ...
      // Set the delay time (which happens to be the time it takes for the
      // animation to complete). NOTE: Here I made it so the delay is actually
      // 40% (0.4f) of the animation time.
#ifdef _JK2MP
      iAnimLen = BG_AnimLength(pVeh->m_pPilot->localAnimIndex, Anim) * 0.4f;
      pVeh->m_iBoarding = BG_GetTime() + iAnimLen;
#else
      iAnimLen = PM_AnimLength(pVeh->m_pPilot->client->clientInfo.animFileIndex,
                               Anim); // * 0.4f;
      ...
      // TODO: But what if he's killed? Should the animation remain
      // persistant???
      iFlags = SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD;

#ifdef _JK2MP
      BG_SetAnim(pVeh->m_pPilot->playerState,
                 bgAllAnims[pVeh->m_pPilot->localAnimIndex].anims, SETANIM_BOTH,
                 Anim, iFlags, iBlend);
#else
      NPC_SetAnim(pVeh->m_pPilot, SETANIM_BOTH, Anim, iFlags, iBlend);
      if (pVeh->m_pOldPilot) {
        iAnimLen =
            PM_AnimLength(pVeh->m_pPilot->client->clientInfo.animFileIndex,
                          BOTH_VS_MOUNTTHROWEE);
        NPC_SetAnim(pVeh->m_pOldPilot, SETANIM_BOTH, BOTH_VS_MOUNTTHROWEE,
                    iFlags, iBlend);
      }
#endif
    }
...
#ifndef _JK2MP
    if (pVeh->m_pOldPilot &&
        pVeh->m_pOldPilot->client->ps.torsoAnimTimer <= 0) {
      ...
      pVeh->m_pVehicleInfo->Eject(pVeh, pVeh->m_pOldPilot,
                                  qtrue); // will set pointer to zero

      // Kill Him
      //----------
      oldPilot->client->noRagTime = -1; // no ragdoll for you
      G_Damage(oldPilot, pVeh->m_pPilot, pVeh->m_pPilot,
               pVeh->m_pPilot->currentAngles, pVeh->m_pPilot->currentOrigin,
               1000, 0, MOD_CRUSH);
      ...
      // Now Throw Him Out
      //-------------------
      G_Throw(oldPilot, throwDir,
              VectorLength(pVeh->m_pParentEntity->client->ps.velocity) / 10.0f);
      NPC_SetAnim(oldPilot, SETANIM_BOTH, BOTH_DEATHBACKWARD1,
                  SETANIM_FLAG_OVERRIDE, iBlend);
    }
#endif
  ...
  /*	// Going in reverse...
  #ifdef _JK2MP
          if ( pVeh->m_ucmd.forwardmove < 0 && !(pVeh->m_ulFlags &
  VEH_SLIDEBREAKING)) #else if ( fSpeedPercToMax < -0.018f && !(pVeh->m_ulFlags
  & VEH_SLIDEBREAKING)) #endif
          {
                  Anim = BOTH_VS_REV;
                  iBlend = 500;
                  bool		HasWeapon	= ((pilotPS->weapon != WP_NONE)
  && (pilotPS->weapon != WP_MELEE)); if (HasWeapon)
                  {
  ...
  */
  {
...
#ifndef _JK2MP
    if (HasWeapon &&
        (pVeh->m_pPilot->s.number >= MAX_CLIENTS ||
         (cg.weaponSelectTime + 500) < cg.time) &&
        (Turbo || (pilotPS->weapon == WP_SABER && !pilotPS->SaberActive()))) {
      if (pVeh->m_pPilot->s.number < MAX_CLIENTS) {
        pVeh->m_pPilot->client->ps.stats[STAT_WEAPONS] |=
            1; // Riding means you get WP_NONE
        CG_ChangeWeapon(WP_NONE);
      }
...
#else
    if (pilotPS->torsoAnim >= BOTH_VS_ATL_S &&
        pilotPS->torsoAnim <= BOTH_VS_ATF_G) {
      float bodyCurrent = 0.0f;
      int bodyEnd = 0;
      if (!!gi.G2API_GetBoneAnimIndex(
              &pVeh->m_pPilot->ghoul2[pVeh->m_pPilot->playerModel],
              pVeh->m_pPilot->rootBone, level.time, &bodyCurrent, NULL,
              &bodyEnd, NULL, NULL, NULL)) {
    ...
    if (pilotPS->weapon == WP_BLASTER) {
      ...
      if ((pVeh->m_ulFlags & VEH_SABERINLEFTHAND) &&
      ...
      WeaponPose = (pVeh->m_ulFlags & VEH_SABERINLEFTHAND) ? (WPOSE_SABERLEFT)
                                                           : (WPOSE_SABERRIGHT);
    }
    ...
    if (Attacking && WeaponPose) { // Attack!
      ...
      if (!Left && !Right) // Allow player strafe keys to override
      {
        ...
        if (pVeh->m_pPilot->enemy) {
          ...
          VectorSubtract(pVeh->m_pPilot->currentOrigin,
                         pVeh->m_pPilot->enemy->currentOrigin, toEnemy);
      ...
      if (Left) { // Attack Left
        ...
        default:
          assert(0);
               ...
               pVeh->m_ucmd.buttons & BUTTON_USE) { // Look To The Left Behind
      iBlend = 400;
      iFlags = SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD;
      switch (WeaponPose) {
      case WPOSE_SABERLEFT:
      ...
      default:
        Anim = BOTH_VS_LOOKLEFT;
               ...
               pVeh->m_ucmd.buttons & BUTTON_USE) { // Look To The Right Behind
      iBlend = 400;
      iFlags = SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD;
      switch (WeaponPose) {
      case WPOSE_SABERLEFT:
  ...
  BG_SetAnim(pVeh->m_pPilot->playerState,
             bgAllAnims[pVeh->m_pPilot->localAnimIndex].anims, SETANIM_BOTH,
             Anim, iFlags | SETANIM_FLAG_HOLD, iBlend);
#else
  NPC_SetAnim(pVeh->m_pPilot, SETANIM_BOTH, Anim, iFlags, iBlend);
#endif
}
...
void G_SetSpeederVehicleFunctions(vehicleInfo_t *pVehInfo) {
  ...
  //Animate; 	pVehInfo->Board						=
  //Board; 	pVehInfo->Eject						=
  //Eject; 	pVehInfo->EjectAll					=
