...
struct ThaiCodes_t {
  ...
  sstring_t m_strInitFailureReason; // so we don't have to keep retrying to work
                                    // this out
...
static inline bool Chinese_ValidGBCode(byte _iHi, byte _iLo) {
  return (_iHi >= GB_HIBYTE_START && _iHi <= GB_HIBYTE_STOP &&
...
//
unsigned int
AnyLanguage_ReadCharFromString(const char *psText, int *piAdvanceCount,
...
float RE_Font_StrLenPixelsNew(const char *psText, const int iFontHandle,
                              const float fScale) {
  ...
  while (*psText) {
    int iAdvanceCount;
    unsigned int uiLetter =
        AnyLanguage_ReadCharFromString(psText, &iAdvanceCount, NULL);
    ...
    if (uiLetter == '\n') {
      ...
      if (r_aspectCorrectFonts->integer == 1) {
        fValue *= ((float)(SCREEN_WIDTH * glConfig.vidHeight) /
                   (float)(SCREEN_HEIGHT * glConfig.vidWidth));
      } else if (r_aspectCorrectFonts->integer == 2) {
        fValue = ceilf(fValue * ((float)(SCREEN_WIDTH * glConfig.vidHeight) /
                                 (float)(SCREEN_HEIGHT * glConfig.vidWidth)));
      }
      thisLineWidth +=
          curfont->mbRoundCalcs
              ? roundf(fValue)
              : (r_aspectCorrectFonts->integer == 2) ? ceilf(fValue) : fValue;
...
void RE_Font_DrawString(int ox, int oy, const char *psText, const float *rgba,
  ...
  //	{
  //		//psText = "Ó¶±øÕ½³¡II  Ô¼º²?ÄªÁÖË¹  ÈÎÎñÊ§°Ü
  //ÄãÒªÌ×ÓÃ»­ÃæÉè¶¨µÄ±ä¸üÂð£¿  Ô¤Éè,S3 Ñ¹Ëõ,DXT1 Ñ¹Ëõ,DXT5 Ñ¹Ëõ,16 Bit,32 Bit";
  //		psText = "Ó¶±øÕ½³¡II";
  //	}
  //	else
  ...
  //	{
  //		//psText =
  //"ÁÒµÃ°Ò¹¼ÅÔµÀÑ³±ìÍØµÊÒË¡ÃÃÁÃËÑÊÊÓËÃÑºÍÑ¡¢ÃÐä·Â·Õèãªé¡Ñº¤ÍÁ¾ÔÇàµÍÃì"; 		psText
  //= "ÁÒµÃ°Ò¹¼ÅÔµ"; 		psText = "ÃËÑÊÊÓËÃÑº"; 		psText = "ÃËÑÊÊÓËÃÑº
  //ÍÒ_¡Ô¹_¤ÍÃì·_1415";
  //	}
  //	else
  //	if (GetLanguageEnum() == eKorean)
  //	{
  //		psText = "Wp:¼îÅ¸ÀÓÀÌ´Ù ¸Ö¸°. ±×µéÀÌ ¸»ÇÑ´ë·Î ³×°¡ ÀßÇÒÁö
  //±â´ëÇÏ°Ú´Ù.";
  //	}
  //	else
  ...
  //	{
  ////		//psText = "Íà âåðøèíå õîëìà ñòîèò ñòàðûé äîì ñ ïðèâèäåíèÿìè è áàøíÿ
  ///ñ âîëøåáíûìè ÷àñàìè."
  ...
  //	{
  //		psText = "za³o¿ony w 1364 roku, jest najstarsz¹ polsk¹ uczelni¹ i
  //nale¿y..."; 		psText = "za³o¿ony nale¿y";
  //	}
  ...
  while (*psText && !bNextTextWouldOverflow) {
    ...
    default:
      ...
      if (!bNextTextWouldOverflow) {
        ...
        if (r_aspectCorrectFonts->integer == 1) {
          fx += fAdvancePixels * ((float)(SCREEN_WIDTH * glConfig.vidHeight) /
                                  (float)(SCREEN_HEIGHT * glConfig.vidWidth));
        } else if (r_aspectCorrectFonts->integer == 2) {
          fx += ceilf(fAdvancePixels *
                      ((float)(SCREEN_WIDTH * glConfig.vidHeight) /
                       (float)(SCREEN_HEIGHT * glConfig.vidWidth)));
        } else {
          fx += fAdvancePixels;
        }
