...
//-----------------------------------------------------
{
	...
	if (self->enemy && self->pushDebounceTime < level.time && self->attackDebounceTime < level.time) {
		...
		gi.G2API_GetBoltMatrix(self->ghoul2, self->playerModel, self->torsoBolt, &boltMatrix, self->currentAngles, self->currentOrigin,
...
//-----------------------------------------------------
{
	...
	if (self->enemy) {
		...
		gi.G2API_GetBoltMatrix(self->ghoul2, self->playerModel, self->torsoBolt, &boltMatrix, self->currentAngles, self->s.origin,
							   (cg.time ? cg.time : level.time), NULL, self->s.modelScale);
	...
	if (diffPitch) {
		...
		gi.G2API_SetBoneAngles(&self->ghoul2[0], "Bone_body", desiredAngles, BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X, NULL, 100, cg.time);
	}
...
//-----------------------------------------------------
{
	...
	base->playerModel =
		gi.G2API_InitGhoul2Model(base->ghoul2, "models/map_objects/imp_mine/turret_canon.glm", base->s.modelindex, NULL_HANDLE, NULL_HANDLE, 0, 0);
	base->s.radius = 80.0f;
...
//-----------------------------------------------------
{
	...
	base->playerModel =
		gi.G2API_InitGhoul2Model(base->ghoul2, "models/map_objects/nar_shaddar/turret/turret.glm", base->s.modelindex, NULL_HANDLE, NULL_HANDLE, 0, 0);
	base->s.radius = 80.0f;
...
//----------------------------------------------------------------
{
	...
	// Getting the flash bolt here
	gi.G2API_GetBoltMatrix(ent->ghoul2, ent->playerModel, ent->torsoBolt, &boltMatrix, ent->currentAngles, ent->s.origin, (cg.time ? cg.time : level.time),
...
//-----------------------------------------------------
{
	...
	// Getting the "eye" here
	gi.G2API_GetBoltMatrix(self->ghoul2, self->playerModel, self->torsoBolt, &boltMatrix, self->currentAngles, self->s.origin, (cg.time ? cg.time : level.time),
...
//---------------------------------
{
	...
	gi.G2API_SetBoneAngles(&ent->ghoul2[ent->playerModel], "bone_barrel", frontAngles, BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, NEGATIVE_X, NULL, 100,
						   cg.time);
	gi.G2API_SetBoneAngles(&ent->ghoul2[ent->playerModel], "bone_gback", frontAngles, BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, NEGATIVE_X, NULL, 100,
						   cg.time);
	gi.G2API_SetBoneAngles(&ent->ghoul2[ent->playerModel], "bone_hinge", backAngles, BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X, NULL, 100,
						   cg.time);
...
//----------------------------------------
{
	...
	if (self->fxID) {
		...
		// Getting the flash bolt here
		gi.G2API_GetBoltMatrix(self->ghoul2, self->playerModel, self->torsoBolt, &boltMatrix, self->s.angles, self->s.origin, (cg.time ? cg.time : level.time),
							   NULL, self->s.modelScale);
...
//-----------------------------------------------------
{
	...
	base->playerModel =
		gi.G2API_InitGhoul2Model(base->ghoul2, "models/map_objects/imp_mine/ion_cannon.glm", base->s.modelindex, NULL_HANDLE, NULL_HANDLE, 0, 0);
...
void panel_turret_think(gentity_t *self) {
	...
	if (player && player->client && player->client->ps.viewEntity == self->s.number) {
		...
		if ((self->useDebounceTime < level.time) &&
			((ucmd->buttons & BUTTON_BLOCKING) || (ucmd->buttons & BUTTON_USE) || ucmd->forwardmove || ucmd->rightmove || ucmd->upmove)) {
			...
			cg.overrides.active &= ~CG_OVERRIDE_FOV;
			cg.overrides.fov = 0;
			if (ucmd->upmove > 0) { // stop player from doing anything for a half second after
