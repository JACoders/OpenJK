...
//---------------------------------------------------------
{
  ...
  for (int i = 0; i < FLECHETTE_SHOTS; i++) {
    ...
    if (i == 0 && ent->s.number == 0) {
      // do nothing on the first shot for the player, this one will hit the
      // crosshairs
    } else {
...
//---------------------------------------------------------
{
  ...
  if (ent->delay > level.time) {
    count = G_RadiusList(ent->currentOrigin, FLECHETTE_MINE_RADIUS_CHECK, ent,
                         qtrue, ent_list);
...
//---------------------------------------------------------
{
        gentity_t	*missile = CreateMissile( wpMuzzle, wpFwd,
FLECHETTE_MINE_VEL, 10000, ent, qtrue );
...
//----------------------------------------------
{
  EvaluateTrajectory(&ent->s.pos, level.time,
                     ent->currentOrigin); // Not sure if this is even necessary,
                                          // but correct origins are cool?

  G_RadiusDamage(ent->currentOrigin, ent->owner, ent->splashDamage,
                 ent->splashRadius, NULL, MOD_EXPLOSIVE_SPLASH);
...
{
  gentity_t *missile =
      CreateMissile(start, fwd, 950 + Q_flrand(0.0f, 1.0f) * 700,
