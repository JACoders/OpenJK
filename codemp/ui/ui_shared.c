...
// Some extern functions hoisted from the middle of this file to get all the
// non-cgame, non-namespace stuff together
...
void Window_Paint(windowDef_t *w, float fadeAmount, float fadeClamp,
                  float fadeCycle) {
  // float bordersize = 0;
  vec4_t color;
  rectDef_t fillRect;
...
int Menu_ItemsMatchingGroup(menuDef_t *menu, const char *name) {
  ...
  for (i = 0; i < menu->itemCount; i++) {
    if (!VALIDSTRING(menu->items[i]->window.name) &&
        !VALIDSTRING(menu->items[i]->window.group)) {
...
qboolean Script_SetItemRectCvar(itemDef_t *item, char **args) {
  ...
  //	Com_Printf(S_COLOR_YELLOW"WARNING: SetItemRectCvar: problems. Set cvar
  //to 0's\n" );
    ...
    if (item != NULL) {
      if (item->type == ITEM_TYPE_MODEL) {
        ...
        modelptr->g2maxsEffect[0] =
            fabs(modelptr->g2maxs2[0] - modelptr->g2maxs[0]) / amt;
        modelptr->g2maxsEffect[1] =
            fabs(modelptr->g2maxs2[1] - modelptr->g2maxs[1]) / amt;
        modelptr->g2maxsEffect[2] =
            fabs(modelptr->g2maxs2[2] - modelptr->g2maxs[2]) / amt;

        modelptr->g2minsEffect[0] =
            fabs(modelptr->g2mins2[0] - modelptr->g2mins[0]) / amt;
        modelptr->g2minsEffect[1] =
            fabs(modelptr->g2mins2[1] - modelptr->g2mins[1]) / amt;
        modelptr->g2minsEffect[2] =
            fabs(modelptr->g2mins2[2] - modelptr->g2mins[2]) / amt;
...
qboolean Script_Scale(itemDef_t *item, char **args) {
  ...
  if (String_Parse(args, &name)) {
    ...
    if (Float_Parse(args, &scale)) {
      for (j = 0; j < count; j++) {
        itemFound =
            Menu_GetMatchingItemByNumber((menuDef_t *)item->parent, j, name);
        if (itemFound != NULL) {
          rectTo.h = itemFound->window.rect.h * scale;
          rectTo.w = itemFound->window.rect.w * scale;

          rectTo.x = itemFound->window.rect.x +
                     ((itemFound->window.rect.h - rectTo.h) / 2);
          rectTo.y = itemFound->window.rect.y +
                     ((itemFound->window.rect.w - rectTo.w) / 2);
...
qboolean Script_Transition2(itemDef_t *item, char **args) {
  ...
  if (String_Parse(args, &name)) {
    if (ParseRect((const char **)args, &rectTo) && Int_Parse(args, &time) &&
        !COM_ParseFloat((const char **)args, &amt)) {
      Menu_TransitionItemByName((menuDef_t *)item->parent, name, 0, &rectTo,
                                time, amt);
    } else {
...
void Menu_SetItemText(const menuDef_t *menu, const char *itemName,
  ...
  for (j = 0; j < count; j++) {
    ...
    if (item != NULL) {
      if (text[0] == '*') {
        item->text = NULL; // Null this out because this would take presidence
                           // over cvar text.
        item->cvar = text + 1;
        switch (item->type) {
...
int Item_ListBox_OverLB(itemDef_t *item, float x, float y) {
  ...
  // Vertical Scroll
    ...
    // Multiple rows and columns (since it's more than twice as wide as an
    // element)
  ...
  if (force ||
    ...
    // Vertical scroll
      ...
      if (key == A_MWHEELUP) {
        int count = trap->Key_IsDown(A_CTRL) ? 5 : 1;
        listPtr->startPos -=
            ((int)item->special == FEEDER_Q3HEADS) ? viewmax : count;
        if (listPtr->startPos < 0) {
          listPtr->startPos = 0;
          Display_MouseMove(NULL, DC->cursorx, DC->cursory);
          return qfalse;
    ...
    if (Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory) &&
      ...
      if (key == A_MOUSE1 || key == A_ENTER || key == A_MOUSE2 ||
        ...
        if (key == A_MOUSE2 || key == A_CURSOR_LEFT ||
            key == A_MWHEELDOWN) // Xbox uses CURSOR_LEFT
        //	if (key == A_MOUSE2 || key == A_CURSOR_LEFT)	// Xbox uses
        //CURSOR_LEFT
        {
          current--;
          if (current < 0) {
...
qboolean Item_TextField_HandleKey(itemDef_t *item, int key) {
  ...
  if (item->cvar) {
    ...
    if (key & K_CHAR_FLAG) {
      ...
      if (key == A_HOME || key == A_KP_7) { // || ( tolower(key) == 'a' &&
                                            // trap->Key_IsDown( K_CTRL ) ) ) {
        item->cursorPos = 0;
        editPtr->paintOffset = 0;
        return qtrue;
      }

      if (key == A_END || key == A_KP_1) { // ( tolower(key) == 'e' &&
                                           // trap->Key_IsDown( K_CTRL ) ) ) {
        item->cursorPos = len;
  ...
  *height = item->textRect.h;
  ...
  if (*width == 0 ||
      ...
      || (item->text && item->text[0] == '@' &&
          item->asset !=
              se_language.modificationCount) // string package language changed
#endif
  ) {
    int originalWidth =
        DC->textWidth(textPtr, item->textscale, item->iMenuFont);
...
void Item_Text_AutoWrapped_Paint(itemDef_t *item) {
  ...
  // if (item->value == 0)
  //{
  //	item->value = (int)(0.5 + (float)DC->textWidth(textPtr, item->textscale,
  //item->font) / item->window.rect.w);
  //}
  height = DC->textHeight(textPtr, item->textscale, item->iMenuFont);
    ...
    if ((newLine && textWidth > item->window.rect.w) || *p == '\n' ||
      ...
      if (len) {
        ...
        DC->drawText(item->textRect.x, item->textRect.y, item->textscale, color,
                     buff, 0, 0, item->textStyle, item->iMenuFont);
      }
      if (*p == '\0') {
        break;
      }
...
void Item_TextField_Paint(itemDef_t *item) {
  ...
  if (item->window.flags & WINDOW_HASFOCUS && g_editingField) {
    ...
    DC->drawTextWithCursor(
        item->textRect.x + item->textRect.w + offset, item->textRect.y,
        item->textscale, newColor, buff + editPtr->paintOffset,
        item->cursorPos - editPtr->paintOffset, cursor, item->window.rect.w,
...
void Item_YesNo_Paint(itemDef_t *item) {
  ...
  if (item->text) {
    ...
    DC->drawText(item->textRect.x + item->textRect.w + 8, item->textRect.y,
                 item->textscale, color, yesnovalue, 0, 0, item->textStyle,
                 item->iMenuFont);
  } else {
    DC->drawText(item->textRect.x, item->textRect.y, item->textscale, color,
                 yesnovalue, 0, 0, item->textStyle, item->iMenuFont);
  }
  ...
  item->textRect.y, item->textscale, newColor, (value != 0) ? sYES : sNO, 0, 0,
  item->textStyle); DC->drawText(item->textRect.x + item->textRect.w + 8,
  item->textRect.y, item->textscale, newColor, (value != 0) ? sYES : sNO, 0, 0,
  item->textStyle,item->iMenuFont); } else {
  //		DC->drawText(item->textRect.x, item->textRect.y,
  item->textscale, newColor, (value != 0) ? sYES : sNO, 0, 0, item->textStyle);
                  DC->drawText(item->textRect.x, item->textRect.y,
  item->textscale, newColor, (value != 0) ? sYES : sNO, 0, 0,
  item->textStyle,item->iMenuFont);
          }
  */
}
...
void Item_Bind_Paint(itemDef_t *item) {
  ...
  if (item->text) {
    ...
    if (textScale != item->textscale) {
      ...
      yAdj =
          textHeight - DC->textHeight(g_nameBind, textScale, item->iMenuFont);
    }

    DC->drawText(startingXPos, item->textRect.y + yAdj, textScale, newColor,
                 g_nameBind, 0, maxChars, item->textStyle, item->iMenuFont);
  } else {
...
void Item_Model_Paint(itemDef_t *item) {
  ...
  if (item->ghoul2) { // ghoul2 models don't have bounds, so we have to parse
                      // them.
  ...
  // refdef.fov_y = atan2( refdef.height, refdef.width / tan( refdef.fov_x / 360
  // * M_PI ) ) * ( 360 / M_PI );
  ...
  if ((item->flags & ITF_ISANYSABER) &&
    ...
    if (modelPtr->rotationSpeed) {
      VectorSet(angles,
                modelPtr->angle + (float)refdef.time / modelPtr->rotationSpeed,
                0, 90);
    } else {
...
void Item_TextScroll_Paint(itemDef_t *item) {
  ...
  for (i = scrollPtr->startPos; i < count; i++) {
    ...
    DC->drawText(x + 4, y, item->textscale, item->window.foreColor, text, 0, 0,
                 item->textStyle, item->iMenuFont);
...
void Item_ListBox_Paint(itemDef_t *item) {
  ...
  if (item->cursorPos >
    ...
    // NOTE : might consider moving this to any spot in here we change the
    // cursor position
    DC->feederSelection(item->special, item->cursorPos, NULL);
  }
  ...
  if (item->window.flags & WINDOW_HORIZONTAL) {
    ...
    if (!listPtr->scrollhidden) {
      ...
      if (Item_ListBox_MaxScroll(item) > 0) {
        ...
        DC->drawHandlePic(x, y, sizeWidth + 1, SCROLLBAR_SIZE,
                          DC->Assets.scrollBar);
        x += sizeWidth - 1;
        DC->drawHandlePic(x, y, SCROLLBAR_SIZE, SCROLLBAR_SIZE,
    ...
    // Show pic name
    text =
        DC->feederItemText(item->special, item->cursorPos, 0, &optionalImage1,
                           &optionalImage2, &optionalImage3);
    if (text) {
      DC->drawText(item->window.rect.x,
                   item->window.rect.y + item->window.rect.h, item->textscale,
  ...
  // A vertical list box
    ...
    if (!listPtr->scrollhidden) {
      ...
      DC->drawHandlePic(x, y, SCROLLBAR_SIZE, sizeHeight + 1,
                        DC->Assets.scrollBar);
      y += sizeHeight - 1;
      DC->drawHandlePic(x, y, SCROLLBAR_SIZE, SCROLLBAR_SIZE,
                        DC->Assets.scrollBarArrowDown);
    ...
    if (listPtr->elementStyle == LISTBOX_IMAGE) {
      ...
      if (item->window.rect.w > (listPtr->elementWidth * 2)) {
        ...
        for (i2 = startPos; i2 < count; i2++) {
          ...
          // NOTE : Is endPos supposed to be valid or not? It was being used as
          // a valid entry but I changed those few spots that were causing bugs
          listPtr->endPos++;
      ...
      // JLF END
      {
        ...
        if (listPtr->numColumns > 0) {
          ...
          for (j = 0; j < listPtr->numColumns; j++) {
            ...
            text = DC->feederItemText(item->special, i, j, &optionalImage1,
                                      &optionalImage2, &optionalImage3);
            ...
            */
            if (text) {
              ...
              textyOffset = 0;
              //							DC->drawText(x
              //+ 4 + listPtr->columnInfo[j].pos, y + listPtr->elementHeight,
              //item->textscale, item->window.foreColor, text, 0,
              //listPtr->columnInfo[j].maxChars, item->textStyle); WAS LAST
              // DC->drawText(x + 4 + listPtr->columnInfo[j].pos, y,
              // item->textscale, item->window.foreColor, text, 0,
            ...
            DC->setColor(NULL);
            if (optionalImage3 >= 0) {
              DC->drawHandlePic(imageStartX - listPtr->elementHeight * 3,
                                y + listPtr->elementHeight + 2,
                                listPtr->elementHeight, listPtr->elementHeight,
                                optionalImage3);
            }
            if (optionalImage2 >= 0) {
              DC->drawHandlePic(imageStartX - listPtr->elementHeight * 2,
                                y + listPtr->elementHeight + 2,
                                listPtr->elementHeight, listPtr->elementHeight,
                                optionalImage2);
            }
            if (optionalImage1 >= 0) {
              DC->drawHandlePic(imageStartX - listPtr->elementHeight,
                                y + listPtr->elementHeight + 2,
                                listPtr->elementHeight, listPtr->elementHeight,
                                optionalImage1);
            }
          ...
          text = DC->feederItemText(item->special, i, 0, &optionalImage1,
                                    &optionalImage2, &optionalImage3);
          if (optionalImage1 >= 0 || optionalImage2 >= 0 ||
              optionalImage3 >= 0) {
            ...
            //item->textStyle);
            DC->drawText(x + 4, y + item->textaligny, item->textscale,
                         item->window.foreColor, text, 0, 0, item->textStyle,
                         item->iMenuFont);
          }
        }
        ...
        { // stupid C language
          ...
          while (1) {
            ...
            if (fDescScale != fDescScaleCopy) {
              int iOriginalTextHeight =
                  DC->textHeight(textPtr, fDescScaleCopy, FONT_MEDIUM);
              iYadj = iOriginalTextHeight -
                      DC->textHeight(textPtr, fDescScale, FONT_MEDIUM);
            }

            DC->drawText(xPos, parent->descY + iYadj, fDescScale,
                         parent->descColor, textPtr, 0, 0, item->textStyle,
                         FONT_SMALL2);
            break;
          }
        }
    ...
    for (i = 0; i < menu->itemCount; i++) {
      if (menu->items[i]->special == feeder) {
        Item_ListBox_HandleKey(
            menu->items[i], (down) ? A_CURSOR_DOWN : A_CURSOR_UP, qtrue, qtrue);
        return;
      }
    }
  }
...
void Item_ValidateTypeData(itemDef_t *item) {
  ...
  case ITEM_TYPE_TEXTSCROLL: {
    item->typeData.textscroll =
        (textScrollDef_t *)UI_Alloc(sizeof(textScrollDef_t));
...
void UI_InsertG2Pointer(void *ghoul2) {
  ...
  while ((*nextFree) &&
         (*nextFree)
             ->ghoul2) { // check if it has a ghoul2, if not we can reuse it.
  ...
  if (!(*nextFree)) { // if we aren't reusing a chain then allocate space for
                      // it.
...
void UI_ClearG2Pointer(void *ghoul2) {
  ...
  while (next) {
    if (next->ghoul2 ==
        ghoul2) { // found it, set it to null so we can reuse this link.
      next->ghoul2 = NULL;
      break;
    }
...
void UI_CleanupGhoul2(void) {
  ...
  while (next) {
    if (next->ghoul2 && trap->G2_HaveWeGhoul2Models(
                            next->ghoul2)) // found a g2 instance, clean it.
...
qboolean ItemParse_asset_model_go(itemDef_t *item, const char *name,
  ...
  *runTimeLength = 0.0f;
  ...
  if (!Q_stricmp(&name[strlen(name) - 4], ".glm")) { // it's a ghoul2 model then
    ...
    g2Model = trap->G2API_InitGhoul2Model(&item->ghoul2, name, 0,
                                          modelPtr->g2skin, 0, 0, 0);
    if (g2Model >= 0) {
      UI_InsertG2Pointer(item->ghoul2); // remember it so we can free it when
                                        // the ui shuts down.
      item->flags |= ITF_G2VALID;

      if (modelPtr->g2anim) { // does the menu request this model be playing an
                              // animation?
        //					DC->g2hilev_SetAnim(&item->ghoul2[0],
        //"model_root", modelPtr->g2anim);
        ...
        if (GLAName[0]) {
          ...
          if (slash) { // If this isn't true the gla path must be messed up
                       // somehow.
            strcpy(slash, "/animation.cfg");

            animIndex = UI_ParseAnimationFile(GLAName, NULL, qfalse);
            if (animIndex != -1) { // We parsed out the animation info for
                                   // whatever model this is
              animation_t *anim =
                  &bgAllAnims[animIndex].anims[modelPtr->g2anim];
              ...
              trap->G2API_SetBoneAnim(item->ghoul2, 0, "model_root", sFrame,
                                      eFrame, flags, animSpeed, time, -1,
                                      blendTime);
      ...
      if (modelPtr->g2skin) {
        ...
        //modelPtr->g2skin );//this is going to set the surfs on/off matching
        //the skin file trap->G2API_InitGhoul2Model(&item->ghoul2, name, 0,
        // modelPtr->g2skin, 0, 0, 0); ahh, what are you doing?!
        trap->G2API_SetSkin(item->ghoul2, 0, modelPtr->g2skin,
                            modelPtr->g2skin);
      }
    }
    /*
...
qboolean ItemParse_asset_model(itemDef_t *item, int handle) {
  ...
  if (!Q_stricmp(token.string, "ui_char_model")) {
    ...
    trap->Cvar_VariableStringBuffer("ui_char_model", ui_char_model,
                                    sizeof(ui_char_model));
    Com_sprintf(modelPath, sizeof(modelPath), "models/players/%s/model.glm",
                ui_char_model);
...
qboolean ItemParse_model_g2skin_go(itemDef_t *item, const char *skinName) {
  ...
  if (!skinName ||
      !skinName[0]) { // it was parsed correctly so still return true.
    modelPtr->g2skin = 0;
...
qboolean ItemParse_cvarStrList(itemDef_t *item, int handle) {
  ...
  if (!Q_stricmp(token.string, "feeder") &&
      item->special == FEEDER_PLAYER_SPECIES) {
#ifndef _CGAME
  ...
  if (!Q_stricmp(token.string, "feeder") && item->special == FEEDER_LANGUAGES) {
    ...
    for (; multiPtr->count < uiInfo.languageCount; multiPtr->count++) {
      ...
      trap->SE_GetLanguageName(
          ...
          (char *)currLanguage[multiPtr->count]); // eg "English"
      multiPtr->cvarList[multiPtr->count] = languageString;
      // The cvar value that goes into se_language
      trap->SE_GetLanguageName((const int)multiPtr->count,
...
static void Item_TextScroll_BuildLines(itemDef_t *item) {
  ...
  while (*psCurrentTextReadPos &&
    ...
    while (*psCurrentTextReadPos) {
      ...
      // read letter...
      //
      uiLetter = trap->R_AnyLanguage_ReadCharFromString(
      ...
      if (bIsTrailingPunctuation || uiLetter == ' ' ||
          (uiLetter > 255 && !trap->R_Language_UsesSpaces())) {
        psBestLineBreakSrcPos = psCurrentTextReadPos;
      }
    }
...
qboolean MenuParse_itemDef(itemDef_t *item, int handle) {
  ...
  if (menu->itemCount < MAX_MENUITEMS) {
    itemDef_t *newItem = menu->items[menu->itemCount] =
        (itemDef_t *)UI_Alloc(sizeof(itemDef_t));
    Item_Init(newItem);
...
void Menu_PaintAll() {
  ...
  if (debugMode) {
    ...
    DC->drawText(5, 45, .75, v, va("x: %d  y:%d", DC->cursorx, DC->cursory), 0,
                 0, 0, 0);
  }
}
