...
=============
*/
void R_DlightBmodel(bmodel_t *bmodel, bool NoLight) { // rwwRMG - modified args
  int i, j;
  dlight_t *dl;
  int mask;
  msurface_t *surf;
...
static void R_SetupEntityLightingGrid(trRefEntity_t *ent) {
  ...
  startGridPos =
      tr.world->lightGridArray +
      (pos[0] * gridStep[0] + pos[1] * gridStep[1] + pos[2] * gridStep[2]);
  ...
  for (i = 0; i < 8; i++) {
    ...
    if (gridPos >=
        tr.world->lightGridArray +
            tr.world
                ->numGridArrayElements) { // we've gone off the array somehow
      continue;
    }
    ...
    for (j = 0; j < MAXLIGHTMAPS; j++) {
      if (data->styles[j] != LS_LSNONE) {
        ...
        ent->ambientLight[0] +=
            factor * data->ambientLight[j][0] * styleColors[style][0] / 255.0f;
        ent->ambientLight[1] +=
            factor * data->ambientLight[j][1] * styleColors[style][1] / 255.0f;
        ent->ambientLight[2] +=
            factor * data->ambientLight[j][2] * styleColors[style][2] / 255.0f;

        ent->directedLight[0] +=
            factor * data->directLight[j][0] * styleColors[style][0] / 255.0f;
        ent->directedLight[1] +=
            factor * data->directLight[j][1] * styleColors[style][1] / 255.0f;
        ent->directedLight[2] +=
            factor * data->directLight[j][2] * styleColors[style][2] / 255.0f;
      } else {
        break;
      }
    }
...
void R_SetupEntityLighting(const trRefdef_t *refdef, trRefEntity_t *ent) {
  ...
  if (ent->e.renderfx & RF_MINLIGHT) { // the minlight flag is now for items
                                       // rotating on their holo thing
    if (ent->e.shaderRGBA[0] == 255 && ent->e.shaderRGBA[1] == 255 &&
