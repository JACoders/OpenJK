...
#define MUSIC_PARSE_ERROR(_string)                                             \
  Music_Parse_Error(_string) // only use during parse, not run-time use, and
                             // bear in mid that data is zapped after error
                             // message, so exit any loops immediately
...
static qboolean Music_ParseMusic(CGenericParser2 &Parser,
                                 MusicData_t *MusicData, CGPGroup *pgMusicFiles,
                                 const char *psMusicName,
                                 const char *psMusicNameKey,
                                 MusicState_e eMusicState) {
  ...
  if (pgMusicFile) {
    ...
    if (pEntryGroup) {
      // read entry points...
      //
      for (CGPValue *pValue = pEntryGroup->GetPairs(); pValue;
           pValue = pValue->GetNext()) {
        const char *psKey = pValue->GetName();
        const char *psValue = pValue->GetTopValue();

        // if (!Q_strncmp(psKey,sKEY_MARKER,strlen(sKEY_MARKER)))	// for
        // now, assume anything is a marker
        {
    ...
    for (CGPGroup *pGroup = pgMusicFile->GetSubGroups(); pGroup;
      ...
      if (!strcmp(psGroupName, sKEY_ENTRY)) {
        ...
        for (CGPValue *pValue = pGroup->GetPairs(); pValue;
             pValue = pValue->GetNext()) {
          const char *psKey = pValue->GetName();
          const char *psValue = pValue->GetTopValue();
    ...
    if (eMusicState != eBGRNDTRACK_BOSS &&
        eMusicState !=
            eBGRNDTRACK_DEATH) // boss & death pieces can omit entry/exit stuff
    {
      if (!bEntryFound) {
    ...
    MUSIC_PARSE_ERROR(
        va("Unable to find musicfiles entry \"%s\"\n", psMusicName));
  }
...
// I only need this because GP2 can't cope with trailing whitespace (for
// !@#$%^'s sake!!!!)...
//
...
static char *StripTrailingWhiteSpaceOnEveryLine(char *pText) {
  ...
  while (*pText) {
    ...
    while (*pThisLineEnd && *pThisLineEnd != '\r' &&
           ((unsigned)(pThisLineEnd - pText) < sizeof(sOneLine) - 1)) {
      pThisLineEnd++;
    }
...
static qboolean Music_ParseLeveldata(const char *psLevelName) {
  ...
  if (pText) {
    ...
    char *psDataPtr = psStrippedText; // because ptr gets advanced, so we supply
                                      // a clone that GP can alter
        ...
        if (pgMusicFiles) {
          ...
          if (pgLevelMusic) {
            ...
            while (sSearchName.c_str()[0] && iSanityLimit < 10) {
              ...
              pgThisLevelMusic =
                  pgLevelMusic->FindSubGroup(sSearchName.c_str());
            ...
            if (pgThisLevelMusic && iSanityLimit < 10) {
              ...
              for (CGPValue *pValue = pgThisLevelMusic->GetPairs(); pValue;
                   pValue = pValue->GetNext()) {
                const char *psKey = pValue->GetName();
                const char *psValue = pValue->GetTopValue();

                if (Q_stricmp(psValue,
                              sKEY_PLACEHOLDER)) // ignore "placeholder" items
                {
              ...
              if (psName_UseBoss) {
                CGPGroup *pgLevelMusicOfBoss =
                    pgLevelMusic->FindSubGroup(psName_UseBoss);
                if (pgLevelMusicOfBoss) {
                  CGPValue *pValueBoss =
                      pgLevelMusicOfBoss->FindPair(sKEY_BOSS);
                  if (pValueBoss) {
                  ...
                  MUSIC_PARSE_ERROR(
                      va("Unable to find 'useboss' entry \"%s\"\n",
                         psName_UseBoss));
                  bReturn = qfalse;
                }
              }
              ...
              if (bReturn && psName_Explore) {
                bReturn = Music_ParseMusic(Parser, MusicData, pgMusicFiles,
                                           psName_Explore, sKEY_EXPLORE,
                                           eBGRNDTRACK_EXPLORE);
              }
              if (bReturn && psName_Action) {
                bReturn = Music_ParseMusic(Parser, MusicData, pgMusicFiles,
                                           psName_Action, sKEY_ACTION,
                                           eBGRNDTRACK_ACTION);
              }
              if (bReturn && psName_Boss) {
                bReturn =
                    Music_ParseMusic(Parser, MusicData, pgMusicFiles,
                                     psName_Boss, sKEY_BOSS, eBGRNDTRACK_BOSS);
              }
              if (bReturn /*&& psName_Death*/) // LAST MINUTE HACK!!, always
                                               // force in some death music!!!!
              {
                // bReturn = Music_ParseMusic(Parser, MusicData, pgMusicFiles,
        ...
        if (strlen(psNextMark)) // always NZ ptr
        {
          ...
          if (itExploreMusicData != MusicData->end()) {
            ...
            if (!MusicFile_Explore.MusicEntryTimes.count(psNextMark)) {
              MUSIC_PARSE_ERROR(va("Unable to find entry point \"%s\" in "
                                   "description for \"%s\"\n",
                                   psNextMark,
...
/*
   ...
   itMusicData != MusicData->end(); ++itMusicData)
                {
                        const char *psMusicState		=
   (*itMusicData).first.c_str(); MusicFile_t &MusicFile	= (*itMusicData).second;

                        Com_OPrintf("Music State:  \"%s\",  File:
   \"%s\"\n",psMusicState, MusicFile.sFileNameBase.c_str());

                        // entry times...
                        //
                        for (MusicEntryTimes_t::iterator itEntryTimes =
   MusicFile.MusicEntryTimes.begin(); itEntryTimes !=
   MusicFile.MusicEntryTimes.end(); ++itEntryTimes)
                        {
                                const char *psMarkerName	=
   (*itEntryTimes).first.c_str(); float	fEntryTime		=
   (*itEntryTimes).second;
   ...
   psMarkerName, fEntryTime);
                        ...
                        {
                                MusicExitPoint_t &MusicExitPoint =
   MusicFile.MusicExitPoints[i];

                                Com_OPrintf("Exit point %d:	sNextFile:
   \"%s\", sNextMark:
   \"%s\"\n",i,MusicExitPoint.sNextFile.c_str(),MusicExitPoint.sNextMark.c_str());
                        ...
                        {
                                MusicExitTime_t &MusicExitTime =
   MusicFile.MusicExitTimes[i];

                                Com_OPrintf("Exit time %d:		fTime:
...
float Music_GetRandomEntryTime(MusicState_e eMusicState) {
  ...
  if (itMusicData != MusicData->end()) {
            ...
            .size()) // make sure at least one defined, else default to start
    {
      ...
      //			Com_OPrintf("Music_GetRandomEntryTime(): Entry
      //%d\n",iRandomEntryNum);
