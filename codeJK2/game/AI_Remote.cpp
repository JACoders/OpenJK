...
-------------------------
*/
void NPC_Remote_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
...
void Remote_MaintainHeight(void) {
  ...
  if (NPC->enemy) {
    if (TIMER_Done(NPC, "heightChange")) {
      ...
      dif =
          (NPC->enemy->currentOrigin[2] + Q_irand(0, NPC->enemy->maxs[2] + 8)) -
          NPC->currentOrigin[2];
...
void Remote_Strafe(void) {
  ...
  // Close enough
  if (tr.fraction > 0.9f) {
    VectorMA(NPC->client->ps.velocity, REMOTE_STRAFE_VEL * dir, right,
...
void Remote_Hunt(qboolean visible, qboolean advance, qboolean retreat) {
  ...
  speed = REMOTE_FORWARD_BASE_SPEED +
          REMOTE_FORWARD_MULTIPLIER * g_spskill->integer;
  if (retreat == qtrue) {
    speed *= -1;
  }
