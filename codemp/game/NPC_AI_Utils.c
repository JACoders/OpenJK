...
int AI_GetGroupSize2(gentity_t *ent, int radius) {
  ...
  return AI_GetGroupSize(ent->r.currentOrigin, radius,
                         (team_t)ent->client->playerTeam, ent);
}
...
void AI_SetClosestBuddy(AIGroupInfo_t *group) {
  ...
  for (i = 0; i < group->numGroup; i++) {
    ...
    for (j = 0; j < group->numGroup; j++) {
      dist =
          DistanceSquared(g_entities[group->member[i].number].r.currentOrigin,
...
qboolean AI_ValidateNoEnemyGroupMember(AIGroupInfo_t *group,
  ...
  if (!trap->InPVS(member->r.currentOrigin,
...
//		member->client->ps.weapon == WP_BOT_LASER ||		// Probe droid	- Laser blast
  ...
  if (member->enemy != group->enemy) {
    ...
    if (!trap->InPVS(member->r.currentOrigin,
                     group->enemy->r
                         .currentOrigin)) { // not within PVS of the group enemy
      return qfalse;
    }
...
void AI_GetGroup(gentity_t *self) {
  ...
  if (self->NPC->group->enemy) {
    ...
    VectorCopy(self->NPC->group->enemy->r.currentOrigin,
  ...
  for ( i = 0; i < numWaiters; i++ )
  {
          ...
          for ( j = 0; j < self->NPC->group->numGroup; j++ )
          {
                  ...
                  if ( trap->InPVS( waiter->r.currentOrigin,
  ...
  if (!noflee) {
    ...
    for (i = 0; i < group->numGroup; i++) {
      ...
      if (member->NPC->rank < RANK_ENSIGN) { // grunt
        if (group->enemy &&
            DistanceSquared(member->r.currentOrigin,
                            group->enemy->r.currentOrigin) <
                65536 /*256*256*/) { // those close to enemy run away!
          ST_StartFlee(member, group->enemy, member->r.currentOrigin,
                       AEL_DANGER_GREAT, 3000, 5000);
        } else if (DistanceSquared(member->r.currentOrigin,
...
qboolean AI_RefreshGroup(AIGroupInfo_t *group) {
  ...
  for (i = 0; i < group->numGroup; i++) {
    ...
    if ( j < group->numGroup )
    {//found a dupe!
            trap->Printf( S_COLOR_RED"ERROR: member %s(%d) a duplicate group
  ...
  */
  ...
  for (i = 0; i < group->numGroup; i++) {
    ...
    if (group->commander && d_npcai.integer) {
      // G_DebugLine( group->commander->r.currentOrigin,
      // member->r.currentOrigin, FRAMETIME, 0x00ff00ff, qtrue );
      G_TestLine(group->commander->r.currentOrigin, member->r.currentOrigin,
                 0x00000ff, FRAMETIME);
    }
  }
...
void AI_GetGroup( AIGroupInfo_t &group, gentity_t *ent, int radius )
{
        ...
        if ( ent->enemy )
        {
                VectorSubtract( ent->enemy->r.currentOrigin,
ent->r.currentOrigin, temp ); VectorNormalize( temp );	//FIXME: Needed?
        ...
        AI_GetGroup( group, ent->r.currentOrigin, ent->currentAngles,
...
gentity_t *AI_DistributeAttack(gentity_t *attacker, gentity_t *enemy,
  ...
  if (enemy != &g_entities[0]) {
    ...
    int aroundPlayer =
        AI_GetGroupSize(g_entities[0].r.currentOrigin, 48, team, attacker);
