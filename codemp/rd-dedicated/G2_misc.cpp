...
class CTraceSurface {
  public:
	...
	CTraceSurface(int initsurfaceNum, surfaceInfo_v &initrootSList, model_t *initcurrentModel, int initlod, vec3_t initrayStart, vec3_t initrayEnd,
				  CollisionRecord_t *initcollRecMap, int initentNum, int initmodelIndex, skin_t *initskin, shader_t *initcust_shader,
				  size_t *initTransformedVertsArray, int inittraceFlags,
#ifdef _G2_GORE
				  float fRadius, float initssize, float inittsize, float inittheta, int initgoreShader, CGhoul2Info *initghoul2info, SSkinGoreData *initgore)
		:
#else
				   float fRadius)
		:
		  ...
		  surfaceNum(initsurfaceNum), rootSList(initrootSList), currentModel(initcurrentModel), lod(initlod), collRecMap(initcollRecMap), entNum(initentNum),
		  modelIndex(initmodelIndex), skin(initskin), cust_shader(initcust_shader), TransformedVertsArray(initTransformedVertsArray),
		  traceFlags(inittraceFlags),
#ifdef _G2_GORE
		  m_fRadius(fRadius), ssize(initssize), tsize(inittsize), theta(inittheta), goreShader(initgoreShader), ghoul2info(initghoul2info), gore(initgore)
#else
		  m_fRadius(fRadius)
#endif
	{
...
void G2_TransformSurfaces(int surfaceNum, surfaceInfo_v &rootSList, CBoneCache *boneCache, const model_t *currentModel, int lod, vec3_t scale,
						  IHeapAllocator *G2VertSpace, size_t *TransformedVertArray, bool secondTimeAround) {
	int i;
	assert(currentModel);
...
#endif
{
	...
	for (i = 0; i < ghoul2.size(); i++) {
		...
		if (!(g.mFlags & GHOUL2_ZONETRANSALLOC)) { // do not stomp if we're using zone space
...
static void G2_BuildHitPointST(const vec3_t A, const float SA, const float TA, const vec3_t B, const float SB, const float TB, const vec3_t C, const float SC,
							   const float TC, const vec3_t P, float *s, float *t, float &bary_i, float &bary_j) {
...
qboolean G2_SegmentTriangleTest(const vec3_t start, const vec3_t end, const vec3_t A, const vec3_t B, const vec3_t C, qboolean backFaces, qboolean frontFaces,
...
void G2_GorePolys(const mdxmSurface_t *surface, CTraceSurface &TS, const mdxmSurfHierarchy_t *surfInfo) {
	...
	for (j = 0; j < numTris; j++) {
		...
		flags = 15 & GoreVerts[tris[j].indexes[0]].flags & GoreVerts[tris[j].indexes[1]].flags & GoreVerts[tris[j].indexes[2]].flags;
		if (flags) {
			continue;
		}
	...
	if (f == GoreTagsTemp.end()) // need to generate a record
	{
		...
		add.mGoreGrowFactor =
			(1.0f - TS.gore->goreScaleStartFraction) / (float)(TS.gore->growDuration); // curscale = (curtime-mGoreGrowStartTime)*mGoreGrowFactor;
	...
	if (gore) {
		...
		// data block format:
		unsigned int size = sizeof(int) +					  // num verts
							sizeof(int) +					  // num tris
		...
		if (G2_SegmentTriangleTest(TS.rayStart, TS.rayEnd, point1, point2, point3, qtrue, qtrue, hitPoint, normal,
			...
			for (i = 0; i < MAX_G2_COLLISIONS; i++) {
				if (TS.collRecMap[i].mEntityNum == -1) {
					...
					G2_BuildHitPointST(point1, point1[3], point1[4], point2, point2[3], point2[4], point3, point3[3], point3[4], hitPoint, &x_pos, &y_pos,
									   newCol.mBarycentricI, newCol.mBarycentricJ);

					/*
										const shader_t		*shader = 0;
										// now, we know what surface this hit belongs to, we need to go get the shader handle so we can get the correct hit
					   location and hit material info if ( cust_shader )
										{
											shader = cust_shader;
										}
										else if ( skin )
										...
										// do we even care to decide what the hit or location area's are? If we don't have them in the shader there is little
					   point if ((shader->hitLocation) || (shader->hitMaterial))
										{
											...
											if (shader->hitLocation)
											{
												newCol.mLocation = *(hitMatReg[shader->hitLocation].loc +
																	((int)(y_pos * hitMatReg[shader->hitLocation].height) *
					   hitMatReg[shader->hitLocation].width) +
											...
											if (shader->hitMaterial)
											{
												newCol.mMaterial = *(hitMatReg[shader->hitMaterial].loc +
																	((int)(y_pos * hitMatReg[shader->hitMaterial].height) *
					   hitMatReg[shader->hitMaterial].width) +
...
static bool G2_RadiusTracePolys(const mdxmSurface_t *surface, CTraceSurface &TS) {
	...
	for (j = 0; j < numTris; j++) {
		...
		assert(tris[j].indexes[1] >= 0 && tris[j].indexes[1] < numVerts);
		assert(tris[j].indexes[2] >= 0 && tris[j].indexes[2] < numVerts);
		flags = 63 & GoreVerts[tris[j].indexes[0]].flags & GoreVerts[tris[j].indexes[1]].flags & GoreVerts[tris[j].indexes[2]].flags;
		int i;
		if (flags) {
			continue;
		} else {
...
#ifdef _G2_GORE
void G2_TraceModels(CGhoul2Info_v &ghoul2, vec3_t rayStart, vec3_t rayEnd, CollisionRecord_t *collRecMap, int entNum, int eG2TraceType, int useLod,
					float fRadius, float ssize, float tsize, float theta, int shader, SSkinGoreData *gore, qboolean skipIfLODNotMatch)
#else
void G2_TraceModels(CGhoul2Info_v &ghoul2, vec3_t rayStart, vec3_t rayEnd, CollisionRecord_t *collRecMap, int entNum, int eG2TraceType, int useLod,
					float fRadius)
#endif
...
#ifdef _G2_GORE
		CTraceSurface TS(ghoul2[i].mSurfaceRoot, ghoul2[i].mSlist, (model_t *)ghoul2[i].currentModel, lod, rayStart, rayEnd, collRecMap, entNum, i, skin,
						 cust_shader, ghoul2[i].mTransformedVertsArray, eG2TraceType, fRadius, ssize, tsize, theta, shader, &ghoul2[i], gore);
#else
		CTraceSurface TS(ghoul2[i].mSurfaceRoot, ghoul2[i].mSlist, (model_t *)ghoul2[i].currentModel, lod, rayStart, rayEnd, collRecMap, entNum, i, skin,
						 cust_shader, ghoul2[i].mTransformedVertsArray, eG2TraceType, fRadius);
#endif
	...
	for (int i = 0; i < ghoul2.size(); i++) {
		if (ghoul2[i].mModelindex != -1) {
			...
			for (size_t x = 0; x < ghoul2[i].mBlist.size(); x++) {
				...
				if ((nextGhoul2.size() > i) && (nextGhoul2[i].mModelindex != -1) && (nextGhoul2[i].mBlist.size() > x) &&
