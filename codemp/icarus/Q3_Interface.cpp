...
void Q3_TaskIDComplete(sharedEntity_t *ent, taskID_t taskType) {
	...
	if (gTaskManagers[ent->s.number] && Q3_TaskIDPending(ent, taskType)) { // Complete it
...
int AppendToSaveGame(unsigned long chid, const void *data, int length) { return 1; }

// Changed by BTO (VV) - Visual C++ 7.1 doesn't allow default args on funcion pointers
int ReadFromSaveGame(unsigned long chid, void *pvAddress, int iLength /* , void **ppvAddressPtr = NULL */) { return 1; }
...
void CGCam_Pan(vec3_t dest, vec3_t panDirection, float duration) { CGCam_Anything(); }

void CGCam_Move(vec3_t dest, float duration) { CGCam_Anything(); }

void CGCam_Shake(float intensity, int duration) { CGCam_Anything(); }

void CGCam_Follow(const char *cameraGroup, float speed, float initLerp) { CGCam_Anything(); }

void CGCam_Track(const char *trackName, float speed, float initLerp) { CGCam_Anything(); }
