...
void Wampa_Move(qboolean visible) {
  if (NPCS.NPCInfo->localState != LSTATE_WAITING) {
    ...
    if (NPCS.NPC->enemy) { // pick correct movement speed and anim
      // run by default
      NPCS.ucmd.buttons &= ~BUTTON_WALKING;
      ...
      } else if (!TIMER_Done(NPCS.NPC, "walk")) { // keep walking for a bit
        NPCS.ucmd.buttons |= BUTTON_WALKING;
      } else if (visible && enemyDist > 384 &&
                 NPCS.NPCInfo->stats.runSpeed == 180) { // fast run, all fours
        NPCS.NPCInfo->stats.runSpeed = 300;
        TIMER_Set(NPCS.NPC, "runfar", Q_irand(2000, 4000));
      } else if (enemyDist > 256 &&
                 NPCS.NPCInfo->stats.runSpeed == 300) { // slow run, upright
    ...
    if (NPCS.NPCInfo->stats.runSpeed ==
        300) { // need to use the alternate run - hunched over on all fours
...
void Wampa_Slash(int boltIndex, qboolean backhand) {
  ...
  for (i = 0; i < numEnts; i++) {
    ...
    if (radiusEnt == NPCS.NPC) { // Skip the wampa ent
      continue;
    }
    ...
    if (DistanceSquared(radiusEnt->r.currentOrigin, boltOrg) <= radiusSquared) {
      ...
      G_Damage(radiusEnt, NPCS.NPC, NPCS.NPC, vec3_origin,
               radiusEnt->r.currentOrigin, damage,
               ((backhand) ? DAMAGE_NO_ARMOR
      ...
      if (backhand) {
        ...
        if (!Q_irand(0, 1)) {         // bite something off
          ...
          G_Dismember(radiusEnt, NPCS.NPC, radiusEnt->r.currentOrigin, hitLoc,
                 ...
                 radiusEnt->health > 0) { // one out of every 4 normal hits does
                                          // a knockdown, too
        vec3_t pushDir;
        vec3_t angs;
        VectorCopy(NPCS.NPC->client->ps.viewangles, angs);
...
void Wampa_Attack(float distance, qboolean doCharge) {
  ...
  if (NPCS.NPC->client->ps.legsAnim == BOTH_ATTACK1 &&
      distance > (NPCS.NPC->r.maxs[0] + MIN_DISTANCE)) { // okay to keep moving
    NPCS.ucmd.buttons |= BUTTON_WALKING;
    Wampa_Move(qtrue);
  }
}
...
void Wampa_Combat(void) {
  ...
  if (!NPC_ClearLOS(NPCS.NPC->r.currentOrigin,
    ...
    if (advance) { // have to get closer
      ...
      if (NPCS.NPC->enemy->health > 0   // enemy still alive
          ...
          && InFOV3(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin,
                    yawOnlyAngles, 20, 20)) // enemy generally in front
        ...
        TIMER_Done(NPCS.NPC, "attacking")) // waiting monsters can't attack
    {
      ...
      if (!Q_irand(0, 20)) { // FIXME: only do this if we just damaged them or
                             // vice-versa?
        if (Wampa_CheckRoar(NPCS.NPC)) {
          return;
        }
        ...
        (!Q_irand(0, 4) &&
         DistanceSquared(attacker->r.currentOrigin, self->r.currentOrigin) <
             DistanceSquared(
                 ...
                 self->r.currentOrigin))) { // if my enemy is dead (or attacked
...
void NPC_BSWampa_Default(void) {
  ...
  if (NPCS.NPC->enemy) {
    if (!TIMER_Done(NPCS.NPC, "attacking")) { // in middle of attack
      // face enemy
      NPC_FaceEnemy(qtrue);
      ...
      if (NPCS.NPC->enemy->client &&
        ...
        if (ValidEnemy(NPCS.NPC->enemy) == qfalse) {
          ...
          if (!NPCS.NPC->enemy->inuse ||
            ...
            // just lost my enemy
            if ((NPCS.NPC->spawnflags &
                 2)) { // search around me if I don't have an enemy
              NPC_BSSearchStart(NPCS.NPC->waypoint, BS_SEARCH);
              NPCS.NPCInfo->tempBehavior = BS_DEFAULT;
            } else if ((NPCS.NPC->spawnflags &
                        1)) { // wander if I don't have an enemy
    ...
    if ((NPCS.NPC->spawnflags &
         2)) { // search around me if I don't have an enemy
      if (NPCS.NPCInfo->homeWp ==
          WAYPOINT_NONE) { // no homewap, initialize the search behavior
        NPC_BSSearchStart(WAYPOINT_NONE, BS_SEARCH);
        NPCS.NPCInfo->tempBehavior = BS_DEFAULT;
      }
      NPCS.ucmd.buttons |= BUTTON_WALKING;
      NPC_BSSearch(); // this automatically looks for enemies
    } else if ((NPCS.NPC->spawnflags & 1)) { // wander if I don't have an enemy
      if (NPCS.NPCInfo->homeWp ==
          WAYPOINT_NONE) { // no homewap, initialize the wander behavior
