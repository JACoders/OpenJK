...
-------------------------
*/
void NPC_Sentry_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
...
void Sentry_Fire(void) {
  ...
  if (NPCInfo->localState == LSTATE_POWERING_UP) {
    ...
    NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_POWERUP1,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
void Sentry_Idle(void) {
  ...
  if (NPCInfo->localState == LSTATE_WAKEUP) {
    ...
    NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_SLEEP1,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    NPC->flags |= FL_SHIELDED;
...
void Sentry_Strafe(void) {
  ...
  // Close enough
  if (tr.fraction > 0.9f) {
    VectorMA(NPC->client->ps.velocity, SENTRY_STRAFE_VEL * dir, right,
...
void Sentry_Hunt(qboolean visible, qboolean advance) {
  ...
  speed = SENTRY_FORWARD_BASE_SPEED +
          SENTRY_FORWARD_MULTIPLIER * g_spskill->integer;
...
void Sentry_RangedAttack(qboolean visible, qboolean advance) {
  if (TIMER_Done(NPC, "attackDelay") && NPC->attackDebounceTime < level.time &&
      visible) // Attack?
  {
    if (NPCInfo->burstCount > 6) {
      if (!NPC->fly_sound_debounce_time) { // delay closing down to give the
                                           // player an opening
        ...
        NPC->flags |= FL_SHIELDED;
        NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_FLY_SHIELDED,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
