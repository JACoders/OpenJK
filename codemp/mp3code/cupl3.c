...
/*====================================================================*/
int hybrid(void *xin, void *xprev, float *y, int btype, int nlong, int ntot, int nprev);
int hybrid_sum(void *xin, void *xin_left, float *y, int btype, int nlong, int ntot);
...
/*------------- initialize bit getter -------------*/
static void bitget_init_end(unsigned char *buf_end) { bitdat.bs_ptr_end = buf_end; }
...
static void Xform_mono(void *pcm, int igr) {
	...
	nsamp[igr][0] = hybrid(pMP3Stream->sample[0][igr], pMP3Stream->sample[0][igr_prev], yout, side_info.gr[igr][0].block_type, n1, n2, nsamp[igr_prev][0]);
...
static void Xform_dual_right(void *pcm, int igr) {
	...
	if (n2 > pMP3Stream->band_limit)
		n2 = pMP3Stream->band_limit;
	igr_prev = igr ^ 1;
	nsamp[igr][1] = hybrid(pMP3Stream->sample[1][igr], pMP3Stream->sample[1][igr_prev], yout, side_info.gr[igr][1].block_type, n1, n2, nsamp[igr_prev][1]);
...
static void Xform_dual(void *pcm, int igr) {
	...
	for (ch = 0; ch < pMP3Stream->nchan; ch++) {
		...
		if (n2 > pMP3Stream->band_limit)
			n2 = pMP3Stream->band_limit;
		nsamp[igr][ch] =
			hybrid(pMP3Stream->sample[ch][igr], pMP3Stream->sample[ch][igr_prev], yout, side_info.gr[igr][ch].block_type, n1, n2, nsamp[igr_prev][ch]);
...
static void Xform_dual_mono(void *pcm, int igr) {
	...
	/*--- hybrid + sbt ---*/
	igr_prev = igr ^ 1;
	if ((side_info.gr[igr][0].block_type == side_info.gr[igr][1].block_type) && (side_info.gr[igr][0].mixed_block_flag == 0) &&
		...
		n3 = nsamp[igr][0] =
			hybrid(pMP3Stream->sample[0][igr], pMP3Stream->sample[0][igr_prev], yout, side_info.gr[igr][0].block_type, n1, n2, nsamp[igr_prev][0]);
		...
		nsamp[igr][1] = hybrid_sum(pMP3Stream->sample[1][igr], pMP3Stream->sample[0][igr], yout, side_info.gr[igr][1].block_type, n1, n2);
		if (n3 < nsamp[igr][1])
			n1 = nsamp[igr][1];
	}
...
static int unpack_side_MPEG1() {
	...
	{
		pMP3Stream->framebytes = 2880 * mp_br_tableL3[pMP3Stream->id][br_index] / mp_sr20_table[pMP3Stream->id][pMP3Stream->sr_index];
	}
...
static int unpack_side_MPEG2(int igr) {
	...
	if (br_index > 0) { /* pMP3Stream->framebytes fixed for free format */
		if (pMP3Stream->mpeg25_flag == 0) {
			pMP3Stream->framebytes = 1440 * mp_br_tableL3[pMP3Stream->id][br_index] / mp_sr20_table[pMP3Stream->id][pMP3Stream->sr_index];
		} else {
			pMP3Stream->framebytes = 2880 * mp_br_tableL3[pMP3Stream->id][br_index] / mp_sr20_table[pMP3Stream->id][pMP3Stream->sr_index];
...
static void unpack_main(unsigned char *pcm, int igr) {
	...
	for (ch = 0; ch < pMP3Stream->nchan; ch++) {
		...
		n2 = pMP3Stream->sfBandIndex[0][side_info.gr[igr][ch].region0_count + side_info.gr[igr][ch].region1_count + 1];
		...
		nn4 = unpack_huff_quad(pMP3Stream->sample[ch][igr] + n3, pMP3Stream->band_limit - n3, qbits, side_info.gr[igr][ch].count1table_select);
		n4 = n3 + nn4;
		nsamp[igr][ch] = n4;
	...
	/*--- dequant ---*/
	for (ch = 0; ch < pMP3Stream->nchan; ch++) {
		dequant(pMP3Stream->sample[ch][igr], &nsamp[igr][ch], /* nsamp updated for shorts */
				&sf[igr][ch], &side_info.gr[igr][ch], &cb_info[igr][ch], pMP3Stream->ncbl_mixed);
	}
	...
	if (pMP3Stream->is_mode) {
		if (pMP3Stream->id)
			is_process_MPEG1(pMP3Stream->sample[0][igr], &sf[igr][1], cb_info[igr], nsamp[igr][0], pMP3Stream->ms_mode);
		else
			is_process_MPEG2(pMP3Stream->sample[0][igr], &sf[igr][1], cb_info[igr], &is_sf_info, nsamp[igr][0], pMP3Stream->ms_mode);
	}
...
/*--------------------------------------------------------------------*/
/*-----------------------------------------------------------------*/
IN_OUT L3audio_decode(unsigned char *bs, unsigned char *pcm) { return pMP3Stream->decode_function(bs, pcm); }
...
static const SBT_FUNCTION sbt_table[2][3][2] = {{{(SBT_FUNCTION)sbt_mono_L3, (SBT_FUNCTION)sbt_dual_L3},
