...
void CGCam_Enable(void) {
	...
	if (g_entities[0].inuse && g_entities[0].client) {
		...
		if (g_entities[0].client->ps.saberInFlight && g_entities[0].client->ps.saberActive) { // saber is out
...
void CGCam_Disable(void) {
	...
	if (cg_skippingcin.integer) { // We're skipping the cinematic and it's over now
