...
// tell clients that this team won and print it on their scoreboard for
// intermission or whatever.
...
void InitSiegeMode(void) {
  ...
  if (g_siegeTeamSwitch.integer) {
    ...
    if (g_siegePersistant.beatingTime) {
      trap->SetConfigstring(CS_SIEGE_TIMEOVERRIDE,
                            va("%i", g_siegePersistant.lastTime));
    } else {
  ...
  if (BG_SiegeGetValueGroup(siege_info, "Teams", teams)) {
    if (g_siegeTeam1.string[0] &&
        Q_stricmp(g_siegeTeam1.string, "none")) { // check for override
      strcpy(team1, g_siegeTeam1.string);
    } else { // otherwise use level default
      BG_SiegeGetPairedValue(teams, "team1", team1);
    }

    if (g_siegeTeam2.string[0] &&
        Q_stricmp(g_siegeTeam2.string, "none")) { // check for override
  ...
  if (BG_SiegeGetValueGroup(siege_info, team2, gParseObjectives)) {
    ...
    if (BG_SiegeGetPairedValue(gParseObjectives, "RequiredObjectives",
                               goalreq)) {
  ...
  if (BG_SiegeGetValueGroup(siege_info, team1, gParseObjectives)) {
    ...
    if (BG_SiegeGetPairedValue(gParseObjectives, "Timed", goalreq)) {
      if (rebel_time_limit) {
        Com_Printf("Tried to set imperial time limit, but there's already a "
  ...
  */
  // Ok, I'm adding inventory item precaching now, so I'm finally going to
  // optimize this to only do weapons/items for the current teams used on the
  // level.
  ...
  // Get and set the team themes for each team. This will control which classes
  // can be used on each team.
  if (BG_SiegeGetValueGroup(siege_info, team1, gParseObjectives)) {
  ...
  while (objectiveNumTeam1 >
         0) { // mark them all as not completed since we just initialized
...
void G_SiegeSetObjectiveComplete(int team, int objective, qboolean failIt) {
  ...
  // Parse from the beginning of this team's objectives until we get to the
  // desired objective number.
  while (p && *p && *p != '|') {
    if (*p == '-') {
      onObjective++;
    }
...
void UseSiegeTarget(
    gentity_t *other, gentity_t *en,
    char *target) { // actually use the player which triggered the object which
                    // triggered the siege objective to trigger the target
  gentity_t *t;
  gentity_t *ent;

  if (!en || !en->client) { // looks like we don't have access to a player, so
                            // just use the activating entity
    ent = other;
  } else {
    ent = en;
  }
...
void BroadcastObjectiveCompletion(int team, int objective, int final,
                                  int client) {
  if (client != ENTITYNUM_NONE && g_entities[client].client &&
      g_entities[client].client->sess.sessionTeam ==
          team) { // guy who completed this objective gets points, providing
                  // he's on the opposing team
    AddScore(&g_entities[client], g_entities[client].client->ps.origin,
...
void AddSiegeWinningTeamPoints(int team, int winner) {
  ...
  while (i < MAX_CLIENTS) {
    ...
    if (ent && ent->client && ent->client->sess.sessionTeam == team) {
      if (i == winner) {
        AddScore(ent, ent->client->ps.origin,
                 SIEGE_POINTS_TEAMWONROUND +
                     SIEGE_POINTS_FINALOBJECTIVECOMPLETED);
      } else {
  ...
  while (i < MAX_CLIENTS) {
    ...
    if (ent->inuse && ent->client &&
        ent->client->pers.connected ==
            CON_CONNECTED) { // a connected client, switch his frickin teams
                             // around
...
void SiegeTeamSwitch(int winTeam, int winTime) {
  trap->SiegePersGet(&g_siegePersistant);
  if (g_siegePersistant
          .beatingTime) { // was already in "switched" mode, change back
...
void SiegeRoundComplete(int winningteam, int winningclient) {
  ...
  if (winningclient != ENTITYNUM_NONE && g_entities[winningclient].client &&
      g_entities[winningclient].client->sess.sessionTeam !=
          winningteam) { // this person just won the round for the other team..
  ...
  if (BG_SiegeGetValueGroup(siege_info, teamstr, gParseObjectives)) {
    if (!BG_SiegeGetPairedValue(
            gParseObjectives, "roundover_target",
            teamstr)) { // didn't find the name of the thing to target upon win,
                        // just logexit now then.
      LogExit("Objectives completed");
      return;
    }

    if (originalWinningClient ==
        ENTITYNUM_NONE) { // oh well, just find something active and use it
                          // then.
      int i = 0;
      gentity_t *ent;
    ...
    G_UseTargets2(&g_entities[originalWinningClient],
                  &g_entities[originalWinningClient], teamstr);
  }

  if (g_siegeTeamSwitch.integer &&
      (imperial_time_limit ||
       rebel_time_limit)) { // handle stupid team switching crap
    int time = 0;
...
void G_ValidateSiegeClassForTeam(gentity_t *ent, int team) {
  ...
  if (stm) {
    ...
    while (
        i <
        stm->numClasses) { // go through the team and see its valid classes, can
                           // we find one that matches our current player class?
      if (stm->classes[i]) {
        if (!Q_stricmp(scl->name,
                       stm->classes[i]->name)) { // the class we're using is
                                                 // already ok for this team.
          return;
        }
    ...
    if (newClassIndex != -1) { // ok, let's find it in the global class array
      ent->client->siegeClass =
          BG_SiegeFindClassIndexByName(stm->classes[newClassIndex]->name);
      Q_strncpyz(ent->client->sess.siegeClass,
                 stm->classes[newClassIndex]->name,
...
void SiegeBeginRound(
    int entNum) { // entNum is just used as something to fire targets from.
  char targname[1024];

  if (!g_preroundState) { // if players are not ingame on round start then
                          // respawn them now
    int i = 0;
    gentity_t *ent;
      ...
      if (ent->inuse && ent->client) {
        if (ent->client->sess.sessionTeam != TEAM_SPECTATOR &&
            !(ent->client->ps.pm_flags &
              PMF_FOLLOW)) { // not a spec, just respawn them
          spawnEnt = qtrue;
        } else if (ent->client->sess.sessionTeam == TEAM_SPECTATOR &&
                   (ent->client->sess.siegeDesiredTeam == TEAM_RED ||
                    ent->client->sess.siegeDesiredTeam ==
                        TEAM_BLUE)) { // spectator but has a desired team
          spawnEnt = qtrue;
        }
      }
  ...
  trap->SetConfigstring(CS_SIEGE_STATE,
                        va("0|%i", level.time)); // we're ready to g0g0g0
}
...
void SiegeCheckTimers(void) {
  ...
  if (!gSiegeRoundBegun) { // check if anyone is active on this team - if not,
                           // keep the timer set up.
    i = 0;
  ...
  if (!gSiegeRoundBegun) {
    if (!numTeam1 || !numTeam2) { // don't have people on both teams yet.
      ...
      trap->SetConfigstring(CS_SIEGE_STATE,
                            "1"); //"waiting for players on both teams"
      ...
      trap->SetConfigstring(
          CS_SIEGE_STATE,
          va("2|%i", gSiegeBeginTime -
                         SIEGE_ROUND_BEGIN_TIME)); // getting ready to begin
    }
  }
}
...
void siegeTriggerUse(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  ...
  if (!(ent->s.eFlags & EF_RADAROBJECT)) { // toggle radar on and exit if it is
                                           // not showing up already
    ent->s.eFlags |= EF_RADAROBJECT;
    return;
  }

  if (activator && activator->client) { // activator will hopefully be the
                                        // person who triggered this event
  ...
  if (BG_SiegeGetValueGroup(siege_info, teamstr, gParseObjectives)) {
    Com_sprintf(objectivestr, sizeof(objectivestr), "Objective%i",
                ent->objective);

    if (BG_SiegeGetValueGroup(gParseObjectives, objectivestr,
                              desiredobjective)) {
...
/*QUAKED info_siege_objective (1 0 1) (-16 -16 -24) (16 16 32) ? x x
STARTOFFRADAR STARTOFFRADAR - start not displaying on radar, don't display until
used.
...
void SiegeIconUse(gentity_t *ent, gentity_t *other,
                  gentity_t *activator) { // toggle it on and off
...
void decompTriggerUse(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  ...
  if (!G_SiegeGetCompletionStatus(
          ent->side, ent->objective)) { // if it's not complete then there's
                                        // nothing to do here
    return;
  }
...
"side" - set to 1 to specify an imperial (team1) goal, 2 to specify rebels
(team2)
*/
void SP_info_siege_decomplete(gentity_t *ent) {
  ...
  if (!ent->objective || !ent->side) { // j00 fux0red something up
    G_FreeEntity(ent);
    trap->Print("ERROR: info_siege_objective_decomplete without an objective "
                "or side value\n");
    return;
  }
}
...
void SiegeItemRemoveOwner(gentity_t *ent, gentity_t *carrier) {
  ...
  if (carrier) {
    carrier->client->holdingObjectiveItem =
        0; // The carrier is no longer carrying us
...
static void SiegeItemRespawnEffect(gentity_t *ent, vec3_t newOrg) {
  ...
  // Play it once on the current origin, and once on the origin we're respawning
  // to.
...
void SiegeItemThink(gentity_t *ent) {
  ...
  if (ent->genericValue8 !=
      ENTITYNUM_NONE) { // Just keep sticking it on top of the owner. We need it
                        // in the same PVS as him so it will render bolted onto
                        // him properly.
  ...
  } else if (ent->genericValue1) { // this means we want to run physics on the
                                   // object
  ...
  if (carrier) {
    // This checking can be a bit iffy on the death stuff, but in theory we
    // should always get a think in before the default minimum respawn time is
    // exceeded.
    if (!carrier->inuse || !carrier->client ||
        (carrier->client->sess.sessionTeam != SIEGETEAM_TEAM1 &&
         carrier->client->sess.sessionTeam != SIEGETEAM_TEAM2) ||
        (carrier->client->ps.pm_flags &
         PMF_FOLLOW)) { // respawn on the original spot
      SiegeItemRespawnOnOriginalSpot(ent, NULL);
    } else if (carrier->health < 1) { // The carrier died so pop out where he is
                                      // (unless in nodrop).
      ...
      if (trap->PointContents(carrier->client->ps.origin, carrier->s.number) &
          CONTENTS_NODROP) { // In nodrop land, go back to the original spot.
        SiegeItemRespawnOnOriginalSpot(ent, carrier);
      } else {
        // perform a startsolid check to make sure the seige item doesn't get
        // stuck in a wall or something
        trace_t tr;
        trap->Trace(&tr, carrier->client->ps.origin, ent->r.mins, ent->r.maxs,
                    carrier->client->ps.origin, ent->s.number, ent->clipmask,
                    qfalse, 0, 0);

        if (tr.startsolid) { // bad spawning area, try again with the trace up a
                             // bit.
          vec3_t TracePoint;
          VectorCopy(carrier->client->ps.origin, TracePoint);
          TracePoint[2] += 30;
          trap->Trace(&tr, TracePoint, ent->r.mins, ent->r.maxs, TracePoint,
                      ent->s.number, ent->clipmask, qfalse, 0, 0);

          if (tr.startsolid) { // hmm, well that didn't work. try one last time
                               // with the item back
        ...
        // We're in a nonstandard place, so if we go this long without being
        // touched, assume we may not be reachable and respawn on the original
        // spot.
  ...
  if (ent->genericValue9 &&
      ent->genericValue9 <
          level.time) { // time to respawn on the original spot then
    SiegeItemRespawnEffect(ent, ent->pos1);
    G_SetOrigin(ent, ent->pos1);
    ent->genericValue9 = 0;
...
void SiegeItemTouch(gentity_t *self, gentity_t *other, trace_t *trace) {
  if (!other || !other->inuse || !other->client || other->s.eType == ET_NPC) {
    if (trace && trace->startsolid) { // let me out! (ideally this should not
                                      // happen, but such is life)
      vec3_t escapePos;
  ...
  if (other->client
          ->holdingObjectiveItem) { // this guy's already carrying a siege item
    return;
  }

  if (other->client->ps.pm_type ==
      PM_SPECTATOR) { // spectators don't pick stuff up
    return;
  }
  ...
  if (self->genericValue6 ==
      other->client->sess
          .sessionTeam) { // Set to not be touchable by players on this team.
    return;
  }
  ...
  self->genericValue8 =
      other->s.number; // Keep the index so we know who is "carrying" us

  self->genericValue9 = 0; // So it doesn't think it has to respawn.

  if (self->target2 && self->target2[0] &&
      (!self->genericValue4 ||
       !self->genericValue5)) { // fire the target for pickup, if it's set to
                                // fire every time, or set to only fire the first
                                // time and the first time has not yet occured.
...
void SiegeItemDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
...
void SiegeItemUse(gentity_t *ent, gentity_t *other,
                  gentity_t *activator) {          // once used, become active
  ...
  if (ent->genericValue11 || !ent->takedamage) { // We want to be able to walk
                                                 // into it to pick it up then.
  ...
  if (ent->paintarget &&
      ent->paintarget[0]) { // want to be on this guy's origin now then
    ...
    if (targ && targ->inuse) {
      ...
      trap->Trace(&tr, targ->r.currentOrigin, ent->r.mins, ent->r.maxs,
                  targ->r.currentOrigin, targ->s.number, ent->clipmask, qfalse,
                  0, 0);
      ...
      if (tr.startsolid) { // bad spawning area, try again with the trace up a
        ...
        if (tr.startsolid) { // hmm, well that didn't work. try one last time
          ...
          if (tr.startsolid) { // crap, that's all we got.  just spawn at the
                               // defualt location.
            return;
          }
        }
...
"mins"				Actual mins of the object. Careful not to place
it into a solid, as these new mins will not be reflected visually in the editor.
                                        Default value is "-16 -16 -24".
"maxs"				Same as above for maxs. Default value is "16 16
32".
"targetname"		If it has a targetname, it will only spawn upon being
used.
"target2"			Target to fire upon pickup. If none, nothing
will happen. "pickuponlyonce"	If non-0, target2 will only be fired on the
first pickup. If the item is dropped and picked up again later, the target will
not be fired off on the sequential pickup. Default value is 1. "target3"
Target to fire upon delivery of the item to the goal point. If none, nothing
will happen. (but you should always want something to happen)
"health"			If > 0, object can be damaged and will die once
health reaches 0. Default is 0. "showhealth"		if health > 0, will show
a health meter for this item
"teamowner"			Which team owns this item, used only for
deciding what color to make health meter
"target4"			Target to fire upon death, if damageable.
Default is none. "deathfx"			Effect to play on death, if
damageable. Default is none.
"canpickup"			If non-0, item can be picked up. Otherwise it
will just be solid and sit on the ground. Default is 1. "pickupsound"
Sound to play on pickup, if any. "goaltarget"		Must be the targetname
of a trigger_multi/trigger_once. Once a player carrying this object is brought
inside the specified trigger, then that trigger will be allowed to fire. Ideally
it will target a siege objective or something like that. "usephysics"
If non-0, run standard physics on the object. Default is 1. "mass"
If usephysics, this will be the factored object mass. Default is 0.09. "gravity"
If usephysics, this will be the factored gravitational pull. Default is 3.0.
"bounce"			If usephysics, this will be the factored bounce
amount. Default is 1.3. "teamnotouch"		If 1 don't let team 1 pickup, if
2 don't let team 2. By default both teams can pick this object up and carry it
around until death. "teamnocomplete"	Same values as above, but controls if
this object can be taken into the objective area by said team. "respawnfx"
Plays this effect when respawning (e.g. it is left in an unknown area too long
                                        and goes back to the original spot). If
this is not specified there will be no effect. (expected format is .efx file)
"paintarget"		plop self on top of this guy's origin when we are used
(only applies if the siege item has a targetname)
"noradar"			if non-0 this thing will not show up on radar

"forcelimit"		if non-0, while carrying this item, the carrier's force
powers will be crippled. "target5"			target to fire when
respawning.
"target6"			target to fire when dropped by someone carrying
this item.

"icon"				icon that represents the gametype item on the
radar

health charge things only work with showhealth 1 on siege items that take
damage. "health_chargeamt"	if non-0 will recharge this much health every...
...
void SP_misc_siege_item(gentity_t *ent) {
  ...
  if (ent->genericValue1) { // if we're using physics we want lerporigin
                            // smoothing
  ...
  if (ent->model && !Q_stricmp(&ent->model[strlen(ent->model) - 4],
                               ".glm")) { // apparently so.
    ent->s.modelGhoul2 = 1;
  }
  ...
  VectorCopy(ent->s.origin,
             ent->pos1); // store off the initial origin for respawning
  ...
  if ((!ent->targetname || !ent->targetname[0]) ||
    ...
    if (canpickup || !ent->takedamage) { // We want to be able to walk into it
                                         // to pick it up then.
      ent->r.contents = CONTENTS_TRIGGER;
      ent->clipmask = CONTENTS_SOLID | CONTENTS_TERRAIN;
      if (canpickup) {
...
// assumed max 2 chars for cl num, 3 chars per ammo/health/maxhealth, even a
// single string full of info for all 32 clients should not get much past 450
// bytes, which is well within a reasonable range. We don't need to send anything
// about the max ammo or current weapon, because currentState.weapon can be
// checked for the ent in question on the client. -rww
void G_SiegeClientExData(gentity_t *msgTarg) {
  ...
  while (i < level.num_entities && count < MAX_EXDATA_ENTS_TO_SEND) {
    ...
    if (ent->inuse && ent->client && msgTarg->s.number != ent->s.number &&
        ...
        trap->InPVS(msgTarg->client->ps.origin,
                    ent->client->ps.origin)) { // another client in the same
                                               // pvs, send his jive
      if (count) { // append a separating space if we are not the first in the
                   // list
      ...
      // append the stats
      Com_sprintf(
          scratch, sizeof(scratch), "%i|%i|%i|%i", ent->s.number,
