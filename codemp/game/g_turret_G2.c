...
void turretG2_set_models(gentity_t *self, qboolean dying) {
  if (dying) {
    ...
    //base->s.modelindex,
                                                            //note, this is not
    the same kind of index - this one's referring to the actual
                                                            //index of the model
    in the g2 instance, whereas modelindex is the index of a
                                                            //configstring -rww
                                                            0,
                                                            0,
                                                            0,
    ...
    */
    ...
    if (!(self->spawnflags & SPF_TURRETG2_TURBO)) {
      ...
      trap->G2API_InitGhoul2Model(
          &self->ghoul2, name,
          0, // base->s.modelindex,
          // note, this is not the same kind of index - this one's referring to
          // the actual index of the model in the g2 instance, whereas modelindex
          // is the index of a configstring -rww
          0, 0, 0, 0);
    } else {
    ...
    if ((self->spawnflags &
         SPF_TURRETG2_TURBO)) { // different pitch bone and muzzle flash points
...
//------------------------------------------------------------------------------------------------------------
void turretG2_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                  int damage, int meansOfDeath)
//------------------------------------------------------------------------------------------------------------
{
  ...
  if (self->s.modelindex2) {
    ...
    if (self->spawnflags & SPF_TURRETG2_CANRESPAWN) { // respawn
      if (self->health < 1 &&
          !self->genericValue5) { // we are dead, set our respawn delay if we
                                  // have one
...
//----------------------------------------------------------------
{
  ...
  if ((ent->spawnflags & SPF_TURRETG2_TURBO)) {
    ...
    bolt->dflags = (DAMAGE_NO_KNOCKBACK |
                    DAMAGE_HEAVY_WEAP_CLASS); // Don't push them around, or else
                                              // we are constantly re-aiming
    ...
    // bolt->trigger_formation = qfalse;		// don't draw tail on first
    // frame
...
//-----------------------------------------------------
{
  ...
  if (self->enemy && self->setTime < level.time &&
...
//-----------------------------------------------------
{
  ...
  if (self->enemy) {
    ...
    if ((self->spawnflags &
         SPF_TURRETG2_LEAD_ENEMY)) { // we want to lead them a bit
    ...
    trap->G2API_GetBoltMatrix(
        self->ghoul2, 0,
        (self->alt_fire ? self->genericValue12 : self->genericValue11),
        &boltMatrix, self->r.currentAngles, self->s.origin, level.time, NULL,
        self->modelScale);
...
//-----------------------------------------------------
{
  ...
  for (i = 0; i < count; i++) {
    ...
    if (!target->client) {
      ...
      if (!(target->flags & FL_BBRUSH) // not a breakable brush
          ...
          || (target->NPC_targetname && self->targetname &&
              Q_stricmp(target->NPC_targetname, self->targetname) !=
                  0)) // not in invicible bbrush, but can only be broken by an
                      // NPC that is not me
      {
        continue;
      }
    ...
    if (!tr.allsolid && !tr.startsolid &&
      ...
      if (enemyDist < bestDist ||
          (target->client && !foundClient)) // all things equal, keep current
      {
  ...
  if (found) {
    ...
    if ( !self->enemy )
    {//just aquired one
            AddSoundEvent( bestTarget, self->r.currentOrigin, 256,
    AEL_DISCOVERED ); AddSightEvent( bestTarget, self->r.currentOrigin, 512,
    AEL_DISCOVERED, 20 );
    }
    */
...
//-----------------------------------------------------
{
  ...
  if (self->last_move_time <
      level.time) { // MISNOMER: used a enemy recalcing debouncer
    if (turretG2_find_enemies(self)) { // found one
      turnOff = qfalse;
      if (self->enemy &&
          self->enemy->client) { // hold on to clients for a min of 3 seconds
  ...
  if (self->enemy != NULL) {
    if (self->enemy->client &&
        self->enemy->client->sess.sessionTeam ==
            TEAM_SPECTATOR) { // don't keep going after spectators
      self->enemy = NULL;
    } else if (self->enemy->client &&
               self->enemy->client->tempSpectate >=
                   level.time) { // don't keep going after spectators
      self->enemy = NULL;
    } else { // FIXME: remain single-minded or look for a new enemy every now
             // and then?
      // enemy is alive
      VectorSubtract(self->enemy->r.currentOrigin, self->r.currentOrigin,
                     enemyDir);
      enemyDist = VectorLengthSquared(enemyDir);
      ...
      if (enemyDist < self->radius * self->radius) {
        ...
        if (trap->InPVS(self->r.currentOrigin, self->enemy->r.currentOrigin)) {
          ...
          trap->Trace(&tr, org2, NULL, NULL, org, self->s.number, MASK_SHOT,
                      qfalse, 0, 0);
...
/*QUAKED misc_turretG2 (1 0 0) (-8 -8 -22) (8 8 0) START_OFF UPSIDE_DOWN
CANRESPAWN TURBO LEAD SHOWRADAR
...
ceiling CANRESPAWN - will respawn after being killed (use count) TURBO -
Big-ass, Boxy Death Star Turbo Laser version LEAD - Turret will aim ahead of
moving targets ("lead" them) SHOWRADAR - show on radar
  ...
  count - if CANRESPAWN spawnflag, decides how long it is before gun respawns
(in ms) - defaults to 20000 (20 seconds)
  ...
  random - random error (in degrees) of projectile direction when it comes out
of the muzzle (default is 2)
...
it
  ...
  customscale - custom scaling size. 100 is normal size, 1024 is the max
scaling. this will change the bounding box size, so be careful of starting in
solid!
...
//-----------------------------------------------------
{
  ...
  if (base->s.iModelScale) {
    ...
    base->modelScale[0] = base->modelScale[1] = base->modelScale[2] =
        base->s.iModelScale / 100.0f;
  }
...
void finish_spawning_turretG2(gentity_t *base) {
  ...
  if ((base->spawnflags & SPF_TURRETG2_TURBO)) {
    ...
    if (level.gametype == GT_SIEGE) { // FIXME: designer-specified?
      // FIXME: put on other entities, too, particularly siege objectives and
      // bbrushes...
  ...
  // stash health off for respawn.  NOTE: cannot use maxhealth because that
  // might not be set if not showing the health bar
  base->genericValue6 = base->health;

  G_SpawnInt("showhealth", "0", &t);
  if (t) { // a non-0 maxhealth value will mean we want to show the health on
           // the hud
  ...
  base->r.contents = CONTENTS_BODY | CONTENTS_PLAYERCLIP |
                     CONTENTS_MONSTERCLIP | CONTENTS_SHOTCLIP;
