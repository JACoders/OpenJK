...
void CL_SetUserCmdValue(int userCmdValue, float sensitivityScale, float mPitchOverride, float mYawOverride, float mSensitivityOverride, int fpSel,
						int invenSel) {
...
void CL_ConfigstringModified(void) {
	...
	if (cl_autolodscale && cl_autolodscale->integer) {
		if (index >= CS_PLAYERS && index < CS_G2BONES) { // this means that a client was updated in some way. Go through and count the clients.
			int clientCount = 0;
			i = CS_PLAYERS;
...
void CL_CheckSVStringEdRef(char *buf, const char *str) { // I don't really like doing this. But it utilizes the system that was already in place.
	int i = 0;
	int b = 0;
	int strLen = 0;
...
qboolean CL_GetServerCommand(int serverCommandNumber) {
	...
	if (serverCommandNumber <= clc.serverCommandSequence - MAX_RELIABLE_COMMANDS) {
		...
		while (i < MAX_RELIABLE_COMMANDS) { // spew out the reliable command buffer
...
void CL_CGameRendering(stereoFrame_t stereo) {
	// rww - RAGDOLL_BEGIN
	if (!com_sv_running->integer) { // set the server time to match the client time, if we don't have a server going.
