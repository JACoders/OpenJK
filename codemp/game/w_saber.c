...
extern qboolean InFront(vec3_t spot, vec3_t from, vec3_t fromAngles,
                        float threshHold);
...
void WP_SaberAddG2Model(gentity_t *saberent, const char *saberModel,
                        qhandle_t saberSkin);
void WP_SaberRemoveG2Model(gentity_t *saberent);

//	g_randFix 0 == Same as basejka. Broken on Linux, fine on Windows
//	g_randFix 1 == Use proper behaviour of RAND_MAX. Fine on Linux, fine on
//Windows 	g_randFix 2 == Intentionally break RAND_MAX. Broken on Linux, broken
//on Windows.
...
static QINLINE int G_SaberAttackPower(gentity_t *ent, qboolean attacking) {
  ...
  if (attacking) { // the attacker gets a boost to help penetrate defense.
    ...
    if (ent->client->lastSaberStorageTime >= (level.time - 50) &&
      ...
      // We want different "tolerance" levels for adding in the distance of the
      // last swing to the base power level depending on which stance we are
      // using. Otherwise fast would have more advantage than it should since the
      // animations are all much faster.
      ...
      VectorSubtract(ent->client->lastSaberBase_Always,
                     ent->client->olderSaberBase, vSub);
      swingDist = (int)VectorLength(vSub);

      while (swingDist > 0) { // I would like to do something more clever. But I
                              // suppose this works, at least for now.
        baseLevel++;
  ...
  if ((ent->client->ps.brokenLimbs & (1 << BROKENLIMB_RARM)) ||
      (ent->client->ps.brokenLimbs &
       (1 << BROKENLIMB_LARM))) { // We're very weak when one of our arms is
                                  // broken
    baseLevel *= 0.3;
  }
  ...
  if (level.gametype == GT_POWERDUEL &&
    ...
    return baseLevel * 2;
             ...
             level.gametype ==
                 GT_SIEGE) { // in siege, saber battles should be quicker and
                             // more biased toward the attacker
    return baseLevel * 3;
  }
...
void WP_ActivateSaber(gentity_t *self) {
  ...
  if (self->NPC && self->client->ps.forceHandExtend == HANDEXTEND_JEDITAUNT &&
      (self->client->ps.forceHandExtendTime - level.time) >
          200) { // if we're an NPC and in the middle of a taunt then stop it
    self->client->ps.forceHandExtend = HANDEXTEND_NONE;
    self->client->ps.forceHandExtendTime = 0;
  } else if (self->client->ps.fd.forceGripCripple) { // can't activate saber
                                                     // while being gripped
    return;
  }
...
void SaberUpdateSelf(gentity_t *ent) {
  ...
  if (g_entities[ent->r.ownerNum].client->ps.saberInFlight &&
      g_entities[ent->r.ownerNum].health >
          0) { // let The Master take care of us now (we'll get treated like a
               // missile until we return)
    ent->nextthink = level.time;
  ...
  if (g_entities[ent->r.ownerNum].client->ps.weapon != WP_SABER ||
      ...
      (!g_entities[ent->r.ownerNum]
       ...
       g_entities[ent->r.ownerNum].s.eType !=
           ET_NPC)) { // owner is not using saber, spectating, dead, saber
                      // holstered, or has no attack level
    ent->r.contents = 0;
    ent->clipmask = 0;
    ...
    if (ent->r.contents != CONTENTS_LIGHTSABER) {
      if ((level.time -
           g_entities[ent->r.ownerNum].client->lastSaberStorageTime) <=
          200) { // Only go back to solid once we're sure our owner has updated
                 // recently
...
void SaberGotHit(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  // Do something here..? Was handling projectiles here, but instead they're now
...
static QINLINE void SetSaberBoxSize(gentity_t *saberent) {
  ...
  if (PM_SaberInBrokenParry(owner->client->ps.saberMove) ||
      BG_SuperBreakLoseAnim(
          owner->client->ps.torsoAnim)) { // let swings go right through when
                                          // we're in this state
    for (i = 0; i < MAX_SABERS; i++) {
      if (i > 0 &&
          !dualSabers) { // not using a second saber, set it to not blocking
        ...
        if (owner->client->saber[i].bladeStyle2Start > 0) {
          for (j = owner->client->saber[i].bladeStyle2Start;
               j < owner->client->saber[i].numBlades; j++) {
    ...
    if (!forceBlock) { // no sabers/blades to FORCE to be on, so turn off
                       // blocking altogether
  ...
  if ((level.time - owner->client->lastSaberStorageTime) > 200 ||
      (level.time - owner->client->saber[j].blade[k].storageTime) >
          100) { // it's been too long since we got a reliable point storage, so
                 // use the defaults and leave.
    VectorSet(saberent->r.mins, -SABER_BOX_SIZE, -SABER_BOX_SIZE,
              -SABER_BOX_SIZE);
    VectorSet(saberent->r.maxs, SABER_BOX_SIZE, SABER_BOX_SIZE, SABER_BOX_SIZE);
    return;
  }

  if (dualSabers || owner->client->saber[0].numBlades >
                        1) { // dual sabers or multi-blade saber
  ...
  // Start out at the saber origin, then go through all the blades and push out
  // the extents for each blade, then set the box relative to the origin.
  ...
  for (i = 0; i < 3; i++) {
    for (j = 0; j < MAX_SABERS; j++) {
      ...
      if (dualSabers && owner->client->ps.saberHolstered == 1 &&
          ...
          if (!dualSabers) {                             // using a single saber
            if (owner->client->saber[j].numBlades > 1) { // with multiple blades
              if (owner->client->ps.saberHolstered ==
                  1) { // all blades after the first one are off
                break;
              }
            }
          }
        }
        if (forceBlock) { // only do blocking with blades that are marked to
                          // block
          if (!alwaysBlock[j][k]) { // this blade shouldn't be blocking
            continue;
          }
        }
        // VectorMA(owner->client->saber[j].blade[k].muzzlePoint,
        // owner->client->saber[j].blade[k].lengthMax*0.5f,
        // owner->client->saber[j].blade[k].muzzleDir, saberOrg);
        VectorCopy(owner->client->saber[j].blade[k].muzzlePoint, saberOrg);
        VectorMA(owner->client->saber[j].blade[k].muzzlePoint,
                 owner->client->saber[j].blade[k].lengthMax,
                 owner->client->saber[j].blade[k].muzzleDir, saberTip);
...
void WP_SaberInitBladeData(gentity_t *ent) {
  ...
  while (i < level.num_entities) { // make sure there are no other saber
                                   // entities floating around that think they
                                   // belong to this client.
    checkEnt = &g_entities[i];

    if (checkEnt->inuse && checkEnt->neverFree &&
        checkEnt->r.ownerNum == ent->s.number && checkEnt->classname &&
  ...
  // We do not want the client to have any real knowledge of the entity
  // whatsoever. It will only ever be used on the server.
  if (!saberent) { // ok, make one then
    saberent = G_Spawn();
  }
  ent->client->ps.saberEntityNum = ent->client->saberStoredIndex =
      saberent->s.number;
  saberent->classname = "lightsaber";

  saberent->neverFree =
      qtrue; // the saber being removed would be a terrible thing.
  ...
  // we want to attempt to remove our g2 instance on the client in case we had
  // one.
...
static QINLINE qboolean G_CheckLookTarget(gentity_t *ent, vec3_t lookAngles,
                                          float *lookingSpeed) {
  // FIXME: also clamp the lookAngles based on the clamp + the existing
  // difference between 		headAngles and torsoAngles?  But often the tag_torso is
  //straight but the torso itself 		is deformed to not face straight... sigh...

  if (ent->s.eType == ET_NPC && ent->s.m_iVehicleNum &&
      ent->s.NPC_class != CLASS_VEHICLE) { // an NPC bolted to a vehicle should
  ...
  if (ent->client->renderInfo.lookTarget >= 0 &&
    ...
    if (ent->client->renderInfo.lookMode == LM_ENT) {
      ...
      if (lookCent) {
        if (lookCent !=
            ent->enemy) { // We turn heads faster than headbob speed, but not as
                          // fast as if watching an enemy
          *lookingSpeed = LOOK_DEFAULT_SPEED;
        }

        // FIXME: Ignore small deltas from current angles so we don't bob our
        // head in synch with theirs?

        /*
        if ( ent->client->renderInfo.lookTarget == 0 && !cg.renderingThirdPerson
        )//!cg_thirdPerson.integer )
        {//Special case- use cg.refdef.vieworg if looking at player and not in
        third person view VectorCopy( cg.refdef.vieworg, lookOrg );
        }
        */ //No no no!
        if (lookCent->client) {
          VectorCopy(lookCent->client->renderInfo.eyePoint, lookOrg);
        } else if (lookCent->inuse &&
                   !VectorCompare(lookCent->r.currentOrigin, vec3_origin)) {
      ...
      VectorCopy(
          level.interestPoints[ent->client->renderInfo.lookTarget].origin,
          lookOrg);
    } else {
      return qfalse;
    }
    ...
    for (i = 0; i < 3; i++) {
      ...
      ent->client->renderInfo.eyeAngles[i] =
          AngleNormalize180(ent->client->renderInfo.eyeAngles[i]);
    }
...
// bandwidth-intensive.
// This is primarily droid stuff I guess, I'm going to try to handle all
// humanoid NPC stuff in with the actual player stuff if possible.
void NPC_SetBoneAngles(gentity_t *ent, char *bone, vec3_t angles);
static QINLINE void G_G2NPCAngles(gentity_t *ent, matrix3_t legs,
                                  vec3_t angles) {
  ...
  if (ent->client) {
    if ((ent->client->NPC_class == CLASS_PROBE) ||
      ...
      if (ent->s.eType == ET_NPC && ent->s.m_iVehicleNum &&
          ent->s.NPC_class != CLASS_VEHICLE) { // an NPC bolted to a vehicle
                                               // should use the full angles
      ...
      {//on the ground
              CG_PlayerLegsYawFromMovement( cent, ent->client->ps.velocity,
      &angles[YAW], cent->lerpAngles[YAW], -60, 60, qtrue );
      }
      else
      {//face legs to front
              CG_PlayerLegsYawFromMovement( cent, vec3_origin, &angles[YAW],
      ...
      */
      ...
      if (ent->client->NPC_class == CLASS_ATST) { // body pitch
        ...
        // BG_G2SetBoneAngles( cent, ent, ent->thoracicBone, lookAngles,
      ...
      if (ent && ent->client && ent->client->NPC_class == CLASS_ATST) {
        ...
        // erroneously play the turning anim, but we do play it when turning in
        // place
        /*
      ...
      { // look at lookTarget!
        ...
        lookAngles[PITCH] = lookAngles[ROLL] =
            0;         // droids can't pitch or roll their heads
        if (looking) { // want to keep doing this lerp behavior for a full
                       // second after stopped looking (so don't snap)
          ent->client->renderInfo.lookingDebounceTime = level.time + 1000;
        }
      }
      if (ent->client->renderInfo.lookingDebounceTime >
          level.time) { // adjust for current body orientation
        vec3_t oldLookAngles;

        lookAngles[YAW] -=
            0; // ent->client->ps.viewangles[YAW];//cent->pe.torso.yawAngle;
        ...
        if (VectorCompare(oldLookAngles, lookAngles) == qfalse) {
          ...
          lookAngles[YAW] = oldLookAngles[YAW] +
                            (lookAngles[YAW] - oldLookAngles[YAW]) * 0.4f;
        }
      ...
      // BG_G2SetBoneAngles( cent, ent, ent->craniumBone, lookAngles,
      // BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
      // cgs.model_draw); return;
...
static QINLINE void G_G2PlayerAngles(gentity_t *ent, matrix3_t legs,
                                     vec3_t legsAngles) {
  qboolean tPitching = qfalse, tYawing = qfalse, lYawing = qfalse;
  float tYawAngle = ent->client->ps.viewangles[YAW], tPitchAngle = 0,
  ...
  if (ent->s.eType == ET_NPC && ent->client) { // sort of hacky, but it saves a
                                               // pretty big load off the server
    int i = 0;
    gentity_t *clEnt;
    ...
    while (i < MAX_CLIENTS) {
      ...
      if (clEnt && clEnt->inuse && clEnt->client &&
          trap->InPVS(clEnt->client->ps.origin,
                      ent->client->ps.origin)) { // this client can see him
        break;
      }
  ...
  if (ent->localAnimIndex <= 1) { // don't do these things on non-humanoids
    ...
    if (ent->client->ps.hasLookTarget) {
      VectorSubtract(g_entities[ent->client->ps.lookTarget].r.currentOrigin,
                     ent->client->ps.origin, lookAngles);
    ...
    BG_G2PlayerAngles(
        ent->ghoul2, ent->client->renderInfo.motionBolt, &ent->s, level.time,
        lerpOrg, lerpAng, legs, legsAngles, &tYawing, &tPitching, &lYawing,
        &tYawAngle, &tPitchAngle, &lYawAngle, FRAMETIME, turAngles,
        ent->modelScale, ciLegs, ciTorso, &ent->client->corrTime, lookAngles,
        ent->client->lastHeadAngles, ent->client->lookTime, emplaced, NULL);

    if (ent->client->ps.heldByClient &&
        ent->client->ps.heldByClient <=
            MAX_CLIENTS) { // then put our arm in this client's hand
      ...
      if (lHandBolt) {
        ...
        trap->G2API_GetBoltMatrix(other->ghoul2, 0, lHandBolt, &boltMatrix,
                                  tAngles, other->client->ps.origin, level.time,
                                  0, other->modelScale);
        ...
        BG_IK_MoveArm(ent->ghoul2, lHandBolt, level.time, &ent->s,
                      ent->client->ps.torsoAnim /*BOTH_DEAD1*/, boltOrg,
                      &ent->client->ikStatus, ent->client->ps.origin,
                      ent->client->ps.viewangles, ent->modelScale, 500, qfalse);
      }
    } else if (ent->client->ikStatus) { // make sure we aren't IKing if we don't
                                        // have anyone to hold onto us.
      int lHandBolt = 0;

      if (ent && ent->inuse && ent->client && ent->ghoul2) {
        lHandBolt = trap->G2API_AddBolt(ent->ghoul2, 0, "*l_hand");
      } else { // This shouldn't happen, but just in case it does, we'll have a
               // failsafe.
      ...
      if (lHandBolt) {
        BG_IK_MoveArm(ent->ghoul2, lHandBolt, level.time, &ent->s,
                      ...
                      &ent->client->ikStatus, ent->client->ps.origin,
                      ent->client->ps.viewangles, ent->modelScale, 500, qtrue);
      }
    }
  } else if (ent->m_pVehicle &&
  ...
  } else if (ent->NPC) { // an NPC not using a humanoid skeleton, do special
                         // angle stuff.
    if (ent->s.eType == ET_NPC && ent->s.NPC_class == CLASS_VEHICLE &&
        ent->m_pVehicle &&
        ent->m_pVehicle->m_pVehicleInfo->type ==
            VH_FIGHTER) { // fighters actually want to take pitch and roll into
                          // account for the axial angles
      VectorCopy(ent->client->ps.viewangles, legsAngles);
      AnglesToAxis(legsAngles, legs);
    } else {
...
int G_SaberLockAnim(int attackerSaberStyle, int defenderSaberStyle,
                    int topOrSide, int lockOrBreakOrSuperBreak, int winOrLose) {
  int baseAnim = -1;
  if (lockOrBreakOrSuperBreak ==
      SABERLOCK_LOCK) { // special case: if we're using the same style and
                        // locking
    if (attackerSaberStyle == defenderSaberStyle ||
        (attackerSaberStyle >= SS_FAST && attackerSaberStyle <= SS_TAVION &&
...
#define LOCK_IDEAL_DIST_JKA                                                    \
  46.0f // all of the new saberlocks are 46.08 from each other because Richard
        // Lico is da MAN

static QINLINE qboolean WP_SabersCheckLock2(gentity_t *attacker,
                                            gentity_t *defender,
  ...
  if (attacker->client->ps.fd.saberAnimLevel >= SS_FAST &&
      ...
      defender->client->ps.fd.saberAnimLevel <=
          SS_TAVION) { // 2 single sabers?  Just do it the old way...
    switch (lockMode) {
    case LOCK_TOP:
    ...
    idealDist = LOCK_IDEAL_DIST_JKA; // all of the new saberlocks are 46.08 from
                                     // each other because Richard Lico is da MAN
    if (lockMode == LOCK_TOP) { // top lock
      ...
      case LOCK_DIAG_TR:
        attAnim =
            G_SaberLockAnim(attacker->client->ps.fd.saberAnimLevel,
                            defender->client->ps.fd.saberAnimLevel,
                            SABERLOCK_SIDE, SABERLOCK_LOCK, SABERLOCK_WIN);
        defAnim =
            G_SaberLockAnim(defender->client->ps.fd.saberAnimLevel,
                            attacker->client->ps.fd.saberAnimLevel,
  ...
  G_SetAnim(attacker, NULL, SETANIM_BOTH, attAnim,
            SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
  attacker->client->ps.saberLockFrame =
      bgAllAnims[attacker->localAnimIndex].anims[attAnim].firstFrame +
      (bgAllAnims[attacker->localAnimIndex].anims[attAnim].numFrames *
       attStart);

  G_SetAnim(defender, NULL, SETANIM_BOTH, defAnim,
            SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
  defender->client->ps.saberLockFrame =
      bgAllAnims[defender->localAnimIndex].anims[defAnim].firstFrame +
      (bgAllAnims[defender->localAnimIndex].anims[defAnim].numFrames *
       defStart);
  ...
  attacker->client->ps.weaponTime = defender->client->ps.weaponTime =
      Q_irand(1000, 3000); // delay 1 to 3 seconds before pushing
  ...
  // MATCH POSITIONS
  diff = VectorNormalize(defDir) -
         idealDist; // diff will be the total error in dist
  // try to move attacker half the diff towards the defender
  VectorMA(attacker->r.currentOrigin, diff * 0.5f, defDir, newOrg);

  trap->Trace(&trace, attacker->r.currentOrigin, attacker->r.mins,
              attacker->r.maxs, newOrg, attacker->s.number, attacker->clipmask,
              qfalse, 0, 0);
  ...
  diff = VectorNormalize(attDir) -
         idealDist; // diff will be the total error in dist
  // try to move defender all of the remaining diff towards the attacker
  VectorMA(defender->r.currentOrigin, diff, attDir, newOrg);
  trap->Trace(&trace, defender->r.currentOrigin, defender->r.mins,
              defender->r.maxs, newOrg, defender->s.number, defender->clipmask,
              qfalse, 0, 0);
  if (!trace.startsolid && !trace.allsolid) {
    if (defender->client) {
...
qboolean WP_SabersCheckLock(gentity_t *ent1, gentity_t *ent2) {
  ...
  if (ent1->s.eType == ET_NPC ||
      ent2->s.eType == ET_NPC) { // if either ents is NPC, then never let an NPC
                                 // lock with someone on the same playerTeam
    if (ent1->client->playerTeam == ent2->client->playerTeam) {
      return qfalse;
    }
  }

  if (!ent1->client->ps.saberEntityNum || !ent2->client->ps.saberEntityNum ||
      ent1->client->ps.saberInFlight ||
      ent2->client->ps.saberInFlight) { // can't get in lock if one of them has
                                        // had the saber knocked out of his hand
    return qfalse;
  }

  if (ent1->s.eType != ET_NPC &&
      ent2->s.eType != ET_NPC) { // can always get into locks with NPCs
    if (!ent1->client->ps.duelInProgress || !ent2->client->ps.duelInProgress ||
        ent1->client->ps.duelIndex != ent2->s.number ||
        ent2->client->ps.duelIndex !=
            ent1->s.number) { // only allow saber locking if two players are
                              // dueling with each other directly
  ...
  if (ent1->client->saber[1].model[0] && !ent1->client->ps.saberHolstered &&
      (ent1->client->saber[1].saberFlags & SFL_NOT_LOCKABLE)) {
    return qfalse;
  }
  if (ent2->client->saber[1].model[0] && !ent2->client->ps.saberHolstered &&
      (ent2->client->saber[1].saberFlags & SFL_NOT_LOCKABLE)) {
    return qfalse;
  }

  if (!InFront(ent1->client->ps.origin, ent2->client->ps.origin,
               ent2->client->ps.viewangles, 0.4f)) {
    return qfalse;
  }
  if (!InFront(ent2->client->ps.origin, ent1->client->ps.origin,
               ent1->client->ps.viewangles, 0.4f)) {
    return qfalse;
  }
  ...
  if (ent1->client->ps.torsoAnim == BOTH_A1_T__B_ ||
      ...
      ent1->client->ps.torsoAnim ==
          BOTH_A7_T__B_) { // ent1 is attacking top-down
  ...
  if (ent2->client->ps.torsoAnim == BOTH_A1_T__B_ ||
      ...
      ent2->client->ps.torsoAnim ==
          BOTH_A7_T__B_) { // ent2 is attacking top-down
  ...
  if (ent1->client->ps.torsoAnim == BOTH_A1_TR_BL ||
      ...
      ent1->client->ps.torsoAnim ==
          BOTH_A7_TR_BL) {    // ent1 is attacking diagonally
    ...
    if (ent2->client->ps.torsoAnim == BOTH_A1_TR_BL ||
        ...
        ent2->client->ps.torsoAnim ==
            BOTH_P1_S1_TL) { // ent2 is attacking in the opposite diagonal
      return WP_SabersCheckLock2(ent1, ent2, LOCK_DIAG_TR);
    }
    if (ent2->client->ps.torsoAnim == BOTH_A1_BR_TL ||
        ...
        ent2->client->ps.torsoAnim ==
            BOTH_P1_S1_BL) { // ent2 is attacking in the opposite diagonal
  ...
  if (ent2->client->ps.torsoAnim == BOTH_A1_TR_BL ||
      ...
      ent2->client->ps.torsoAnim == BOTH_A6_TR_BL ||
      ent2->client->ps.torsoAnim ==
          BOTH_A7_TR_BL) {    // ent2 is attacking diagonally
    ...
    if (ent1->client->ps.torsoAnim == BOTH_A1_TR_BL ||
        ...
        ent1->client->ps.torsoAnim ==
            BOTH_P1_S1_TL) { // ent1 is attacking in the opposite diagonal
      return WP_SabersCheckLock2(ent2, ent1, LOCK_DIAG_TR);
    }
    if (ent1->client->ps.torsoAnim == BOTH_A1_BR_TL ||
        ...
        ent1->client->ps.torsoAnim ==
            BOTH_P1_S1_BL) { // ent1 is attacking in the opposite diagonal
  ...
  if (ent1->client->ps.torsoAnim == BOTH_A1_TL_BR ||
      ...
      ent1->client->ps.torsoAnim ==
          BOTH_A7_TL_BR) {    // ent1 is attacking diagonally
    if (ent2BlockingPlayer) { // player will block this anyway
    ...
    if (ent2->client->ps.torsoAnim == BOTH_A1_TL_BR ||
        ...
        ent2->client->ps.torsoAnim ==
            BOTH_P1_S1_TR) { // ent2 is attacking in the opposite diagonal
      return WP_SabersCheckLock2(ent1, ent2, LOCK_DIAG_TL);
    }
    if (ent2->client->ps.torsoAnim == BOTH_A1_BL_TR ||
        ...
        ent2->client->ps.torsoAnim ==
            BOTH_P1_S1_BR) { // ent2 is attacking in the opposite diagonal
  ...
  if (ent2->client->ps.torsoAnim == BOTH_A1_TL_BR ||
      ...
      ent2->client->ps.torsoAnim == BOTH_A6_TL_BR ||
      ent2->client->ps.torsoAnim ==
          BOTH_A7_TL_BR) {    // ent2 is attacking diagonally
    if (ent1BlockingPlayer) { // player will block this anyway
    ...
    if (ent1->client->ps.torsoAnim == BOTH_A1_TL_BR ||
        ...
        ent1->client->ps.torsoAnim ==
            BOTH_P1_S1_TR) { // ent1 is attacking in the opposite diagonal
      return WP_SabersCheckLock2(ent2, ent1, LOCK_DIAG_TL);
    }
    if (ent1->client->ps.torsoAnim == BOTH_A1_BL_TR ||
        ...
        ent1->client->ps.torsoAnim ==
            BOTH_P1_S1_BR) { // ent1 is attacking in the opposite diagonal
  ...
  if (ent1->client->ps.torsoAnim == BOTH_A1__L__R ||
    ...
    if (ent2->client->ps.torsoAnim == BOTH_A1_TL_BR ||
        ...
        ent2->client->ps.torsoAnim ==
            BOTH_P1_S1_BL) { // ent2 is attacking or blocking on the r
      return WP_SabersCheckLock2(ent1, ent2, LOCK_L);
    }
    return qfalse;
  }
  if (ent2->client->ps.torsoAnim == BOTH_A1__L__R ||
    ...
    if (ent1->client->ps.torsoAnim == BOTH_A1_TL_BR ||
        ...
        ent1->client->ps.torsoAnim ==
            BOTH_P1_S1_BL) { // ent1 is attacking or blocking on the r
      return WP_SabersCheckLock2(ent2, ent1, LOCK_L);
    }
    return qfalse;
  }
  ...
  if (ent1->client->ps.torsoAnim == BOTH_A1__R__L ||
    ...
    if (ent2->client->ps.torsoAnim == BOTH_A1_TR_BL ||
        ...
        ent2->client->ps.torsoAnim ==
            BOTH_P1_S1_BR) { // ent2 is attacking or blocking on the l
      return WP_SabersCheckLock2(ent1, ent2, LOCK_R);
    }
    return qfalse;
  }
  if (ent2->client->ps.torsoAnim == BOTH_A1__R__L ||
    ...
    if (ent1->client->ps.torsoAnim == BOTH_A1_TR_BL ||
        ...
        ent1->client->ps.torsoAnim ==
            BOTH_P1_S1_BR) { // ent1 is attacking or blocking on the l
      return WP_SabersCheckLock2(ent2, ent1, LOCK_R);
    }
    return qfalse;
  }
...
static QINLINE qboolean WP_GetSaberDeflectionAngle(gentity_t *attacker,
                                                   gentity_t *defender,
                                                   float saberHitFraction) {
  ...
  if ((level.time - attacker->client->lastSaberStorageTime) >
      500) { // last update was too long ago, something is happening to this
             // client to prevent his saber from updating
    return qfalse;
  }
  ...
  if (animBasedDeflection) {
    ...
    // reverse the left/right of the defQuad because of the mirrored nature of
    ...
    if (quadDiff > 4) { // wrap around so diff is never greater than 180 (4 *
                        // 45)
      quadDiff = 4 - (quadDiff - 4);
    }
    // have the quads, find a good anim to use
    if ((!quadDiff ||
         (quadDiff == 1 && Q_irand(0, 1))) // defender pretty much stopped the
                                           // attack at a 90 degree angle
        && (defSaberLevel == attSaberLevel ||
            Q_irand(0, defSaberLevel - attSaberLevel) >=
                0)) // and the defender's style is stronger
    {
      ...
      if (g_saberDebugPrint.integer) {
        Com_Printf(
            "attack %s vs. parry %s bounced to %s\n",
            animTable[saberMoveData[attMove].animToUse].name,
            animTable[saberMoveData[defender->client->ps.saberMove].animToUse]
                .name,
            animTable[saberMoveData[attacker->client->ps.saberMove].animToUse]
                .name);
      }
#endif
      attacker->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
      return qfalse;
    } else { // attack hit at an angle, figure out what angle it should bounce
             // off att
      int newQuad;
      quadDiff = defQuad - attQuadEnd;
      // add half the diff of between the defense and attack end to the attack
      // end
      if (quadDiff > 4) {
        quadDiff = 4 - (quadDiff - 4);
      } else if (quadDiff < -4) {
      ...
      if (newQuad ==
          attQuadStart) { // never come off at the same angle that we would have
                          // if the attack was not interrupted
        if (Q_irand(0, 1)) {
          newQuad--;
        } else {
          newQuad++;
      ...
      // else, pick a deflection
        ...
        if (g_saberDebugPrint.integer) {
          Com_Printf(
              "attack %s vs. parry %s deflected to %s\n",
              animTable[saberMoveData[attMove].animToUse].name,
              animTable[saberMoveData[defender->client->ps.saberMove].animToUse]
                  .name,
              animTable[saberMoveData[attacker->client->ps.saberMove].animToUse]
    ...
    if (hitDot < 0.25f &&
        hitDot > -0.25f) { // hit pretty much perpendicular, pop straight back
      attacker->client->ps.saberMove =
          PM_SaberBounceForAttack(attacker->client->ps.saberMove);
      attacker->client->ps.saberBlocked = BLOCKED_ATK_BOUNCE;
      return qfalse;
      ...
      // normalize the result to determine what direction our saber should
      // bounce back toward
      VectorNormalize(att_DeflectionDir);

      // need to know the direction of the deflectoin relative to the attacker's
      // facing
      VectorSet(temp, 0, attacker->client->ps.viewangles[YAW],
                0); // presumes no pitch!
      ...
      if (swingRDot > 0.25f) {   // deflect to right
        ...
        } else { // deflect horizontally?  Well, no such thing as straight back
                 // in my face, so use top
          if (swingRDot > 0) {
            attacker->client->ps.saberMove = LS_D1_TR;
          } else if (swingRDot < 0) {
            attacker->client->ps.saberMove = LS_D1_TL;
...
static QINLINE int G_GetAttackDamage(gentity_t *self, int minDmg, int maxDmg,
                                     float multPoint) {
  int speedDif = 0;
  int totalDamage = maxDmg;
  float peakPoint = 0;
  float attackAnimLength = bgAllAnims[self->localAnimIndex]
                               .anims[self->client->ps.torsoAnim]
                               .numFrames *
                           fabs((float)(bgAllAnims[self->localAnimIndex]
                                            .anims[self->client->ps.torsoAnim]
                                            .frameLerp));
  ...
  // Be sure to scale by the proper anim speed just as if we were going to play
  // the animation
  BG_SaberStartTransAnim(self->s.number, self->client->ps.fd.saberAnimLevel,
                         self->client->ps.weapon, self->client->ps.torsoAnim,
                         &animSpeedFactor, self->client->ps.brokenLimbs);
  ...
  // we treat torsoTimer as the point in the animation (closer it is to
  // attackAnimLength, closer it is to beginning)
...
// Get the point in the animation and return a percentage of the current point
// in the anim between 0 and the total anim length (0.0f - 1.0f)
...
static QINLINE qboolean G_G2TraceCollide(
    trace_t *tr, vec3_t lastValidStart, vec3_t lastValidEnd, vec3_t traceMins,
    vec3_t traceMaxs) { // Hit the ent with the normal trace, try the collision
                        // trace.
  G2Trace_t G2Trace;
  gentity_t *g2Hit;
  vec3_t angles;
  ...
  if (traceMins[0] || traceMins[1] || traceMins[2] || traceMaxs[0] ||
      traceMaxs[1] || traceMaxs[2]) {
  ...
  if (g2Hit && g2Hit->inuse && g2Hit->ghoul2) {
    ...
    if (com_optvehtrace.integer && g2Hit->s.eType == ET_NPC &&
        g2Hit->s.NPC_class == CLASS_VEHICLE && g2Hit->m_pVehicle) {
      trap->G2API_CollisionDetectCache(
          G2Trace, g2Hit->ghoul2, angles, g2HitOrigin, level.time,
          g2Hit->s.number, lastValidStart, lastValidEnd, g2Hit->modelScale, 0,
          g_g2TraceLod.integer, fRadius);
    } else {
      trap->G2API_CollisionDetect(G2Trace, g2Hit->ghoul2, angles, g2HitOrigin,
                                  level.time, g2Hit->s.number, lastValidStart,
                                  lastValidEnd, g2Hit->modelScale, 0,
                                  g_g2TraceLod.integer, fRadius);
    }
    ...
    if (G2Trace[0].mEntityNum != g2Hit->s.number) {
      ...
      tr->allsolid = 0;
      return qfalse;
    } else { // The ghoul2 trace result matches, so copy the collision position
             // into the trace endpos and send it back.
...
qboolean saberCheckKnockdown_Thrown(gentity_t *saberent, gentity_t *saberOwner,
                                    gentity_t *other);
qboolean saberCheckKnockdown_Smashed(gentity_t *saberent, gentity_t *saberOwner,
                                     gentity_t *other, int damage);
...
static QINLINE void G_BuildSaberFaces(vec3_t base, vec3_t tip, float radius,
                                      vec3_t fwd, vec3_t right, int *fNum,
                                      saberFace_t **fList) {
...
static QINLINE void G_SabCol_CalcPlaneEq(vec3_t x, vec3_t y, vec3_t z,
                                         float *planeEq) {
  planeEq[0] =
      x[1] * (y[2] - z[2]) + y[1] * (z[2] - x[2]) + z[1] * (x[2] - y[2]);
  planeEq[1] =
      x[2] * (y[0] - z[0]) + y[2] * (z[0] - x[0]) + z[2] * (x[0] - y[0]);
  planeEq[2] =
...
// collision utility function -rww
static QINLINE int G_SabCol_PointRelativeToPlane(vec3_t pos, float *side,
                                                 float *planeEq) {
  *side = planeEq[0] * pos[0] + planeEq[1] * pos[1] + planeEq[2] * pos[2] +
          planeEq[3];
...
static QINLINE qboolean G_SaberFaceCollisionCheck(int fNum, saberFace_t *fList,
                                                  vec3_t atkStart,
                                                  vec3_t atkEnd, vec3_t atkMins,
                                                  vec3_t atkMaxs,
                                                  vec3_t impactPoint) {
  ...
  if (VectorCompare(atkMins, vec3_origin) &&
      VectorCompare(atkMaxs, vec3_origin)) {
  ...
  while (i < fNum) {
    ...
    if (G_SabCol_PointRelativeToPlane(atkStart, &side, planeEq) !=
        G_SabCol_PointRelativeToPlane(
            atkEnd, &side2,
            planeEq)) { // start/end points intersect with the plane
      ...
      side2 = planeNormal[0] * dir[0] + planeNormal[1] * dir[1] +
              planeNormal[2] * dir[2];
      ...
      if (facing < 0) { // not intersecting.. let's try with the mins/maxs and
                        // see if they interesect on the edge plane
      ...
      if (facing >= 0) { // first edge is facing...
        ...
        if (facing >= 0) { // second edge is facing...
          ...
          if (facing < 0) { // not intersecting.. let's try with the mins/maxs
                            // and see if they interesect on the edge plane
            facing = G_SabCol_PointRelativeToPlane(minPoint, &side, planeEq);
            if (facing < 0) {
              facing = G_SabCol_PointRelativeToPlane(maxPoint, &side, planeEq);
            }
...
static QINLINE qboolean G_SaberCollide(gentity_t *atk, gentity_t *def,
                                       vec3_t atkStart, vec3_t atkEnd,
                                       vec3_t atkMins, vec3_t atkMaxs,
                                       vec3_t impactPoint) {
  static int i, j;
  ...
  if (!atk->inuse || !atk->client || !def->inuse ||
      !def->client) { // must have 2 clients and a valid saber entity
    return qfalse;
  }
    ...
    if (def->client->saber[i].model[0]) { // valid saber on the defender
      ...
      while (j < def->client->saber[i].numBlades) {
        ...
        if ((level.time - blade->storageTime) < 200) { // recently updated
          ...
          G_BuildSaberFaces(base, tip, blade->radius * 3.0f, fwd, right, &fNum,
                            &fList);
          if (fNum > 0) {
#if 0
						if (atk->s.number == 0)
            ...
            if (G_SaberFaceCollisionCheck(fNum, fList, atkStart, atkEnd,
                                          atkMins, atkMaxs,
                                          impactPoint)) { // collided
              return qtrue;
            }
          }
        }
...
Breaks the two saber paths into 2 tris each and tests each tri for the first
saber path against each of the other saber path's tris

FIXME: subdivide the arc into a consistant increment
FIXME: test the intersection to see if the sabers really did intersect (weren't
going in the same direction and/or passed through same point at different
times)?
*/
extern qboolean tri_tri_intersect(vec3_t V0, vec3_t V1, vec3_t V2, vec3_t U0,
                                  vec3_t U1, vec3_t U2);
#define SABER_EXTRAPOLATE_DIST 16.0f
qboolean WP_SabersIntersect(gentity_t *ent1, int ent1SaberNum, int ent1BladeNum,
                            gentity_t *ent2, qboolean checkDir) {
  ...
  for (ent2SaberNum = 0; ent2SaberNum < MAX_SABERS; ent2SaberNum++) {
    if (ent2->client->saber[ent2SaberNum].type != SABER_NONE) {
      for (ent2BladeNum = 0;
           ent2BladeNum < ent2->client->saber[ent2SaberNum].numBlades;
           ent2BladeNum++) {
        if (ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].lengthMax >
            0) { // valid saber and this blade is on
          // if ( ent1->client->saberInFlight )
          {
            VectorCopy(ent1->client->saber[ent1SaberNum]
                           .blade[ent1BladeNum]
                           .muzzlePointOld,
                       saberBase1);
            VectorCopy(ent1->client->saber[ent1SaberNum]
            ...
            VectorSubtract(ent1->client->saber[ent1SaberNum]
                               .blade[ent1BladeNum]
                               .muzzlePoint,
                           ent1->client->saber[ent1SaberNum]
                               .blade[ent1BladeNum]
                               .muzzlePointOld,
                           dir);
            VectorNormalize(dir);
            VectorMA(saberBaseNext1, SABER_EXTRAPOLATE_DIST, dir,
                     saberBaseNext1);
            ...
            VectorMA(
                ...
                ent1->client->saber[ent1SaberNum]
                        ...
                        .lengthMax +
                    SABER_EXTRAPOLATE_DIST,
                ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].muzzleDir,
                saberTipNext1);
          ...
          {
                  VectorCopy(
          ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].muzzlePoint,
          saberBase1 ); VectorCopy(
          ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].muzzlePointNext,
          saberBaseNext1 ); VectorMA( saberBase1,
          ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].lengthMax,
          ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].muzzleDir,
          saberTip1 ); VectorMA( saberBaseNext1,
          ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].lengthMax,
          ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].muzzleDirNext,
          saberTipNext1 );
          }
          */
          ...
          {
            VectorCopy(ent2->client->saber[ent2SaberNum]
                           .blade[ent2BladeNum]
                           .muzzlePointOld,
                       saberBase2);
            VectorCopy(ent2->client->saber[ent2SaberNum]
            ...
            VectorSubtract(ent2->client->saber[ent2SaberNum]
                               .blade[ent2BladeNum]
                               .muzzlePoint,
                           ent2->client->saber[ent2SaberNum]
                               .blade[ent2BladeNum]
                               .muzzlePointOld,
                           dir);
            VectorNormalize(dir);
            VectorMA(saberBaseNext2, SABER_EXTRAPOLATE_DIST, dir,
                     saberBaseNext2);
          ...
          {
                  VectorCopy(
          ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].muzzlePoint,
          saberBase2 ); VectorCopy(
          ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].muzzlePointNext,
          saberBaseNext2 ); VectorMA( saberBase2,
          ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].lengthMax,
          ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].muzzleDir,
          saberTip2 ); VectorMA( saberBaseNext2,
          ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].lengthMax,
          ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].muzzleDirNext,
          saberTipNext2 );
          }
          */
          if (checkDir) { // check the direction of the two swings to make sure
                          // the sabers are swinging towards each other
            ...
            if (DotProduct(saberDir1, saberDir2) >
                0.6f) { // sabers moving in same dir, probably didn't actually
                        // hit
              continue;
            }
            // now check orientation of sabers, make sure they're not parallel
            // or close to it
            dot = DotProduct(
                ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].muzzleDir,
                ent2->client->saber[ent2SaberNum]
                    ...
                    .muzzleDir);
            if (dot > 0.9f ||
                dot < -0.9f) { // too parallel to really block effectively?
              continue;
            }
          }
          ...
          if (g_saberDebugBox.integer == 2 || g_saberDebugBox.integer == 4) {
            G_TestLine(
                saberBase1, saberTip1,
                ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].color,
                500);
            G_TestLine(
                saberTip1, saberTipNext1,
                ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].color,
                500);
            G_TestLine(
                saberTipNext1, saberBase1,
                ent1->client->saber[ent1SaberNum].blade[ent1BladeNum].color,
                500);

            G_TestLine(
                saberBase2, saberTip2,
                ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].color,
                500);
            G_TestLine(
                saberTip2, saberTipNext2,
                ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].color,
                500);
            G_TestLine(
                saberTipNext2, saberBase2,
                ent2->client->saber[ent2SaberNum].blade[ent2BladeNum].color,
                500);
          }
#endif
          if (tri_tri_intersect(saberBase1, saberTip1, saberBaseNext1,
                                saberBase2, saberTip2, saberBaseNext2)) {
            return qtrue;
          }
          if (tri_tri_intersect(saberBase1, saberTip1, saberBaseNext1,
                                saberBase2, saberTip2, saberTipNext2)) {
            return qtrue;
          }
          if (tri_tri_intersect(saberBase1, saberTip1, saberTipNext1,
                                saberBase2, saberTip2, saberBaseNext2)) {
            return qtrue;
          }
          if (tri_tri_intersect(saberBase1, saberTip1, saberTipNext1,
...
static QINLINE int G_PowerLevelForSaberAnim(gentity_t *ent, int saberNum,
                                            qboolean mySaberHit) {
  if (!ent || !ent->client || saberNum >= MAX_SABERS) {
    ...
    int animTimeElapsed =
        BG_AnimLength(ent->localAnimIndex, (animNumber_t)anim) - animTimer;
    ...
    if (anim >= BOTH_P1_S1_T_ &&
        anim <= BOTH_H1_S1_BR) { // parries, knockaways and broken parries
      return FORCE_LEVEL_1;      // FIXME: saberAnimLevel?
    }
    switch (anim) {
    case BOTH_A2_STABBACK1:
      if (mySaberHit) { // someone else hit my saber, not asking for damage
                        // level, but defense strength
        return FORCE_LEVEL_1;
      }
    ...
    case BOTH_JUMPATTACK6:
      ...
      BOTH_JUMPATTACK6 ) - pm->ps->legsAnimTimer >= 400 )
                      ||(pm->ps->legsAnimTimer >= 400
                              && BG_AnimLength(
      g_entities[ps->clientNum].client->clientInfo.animFileIndex,
      BOTH_JUMPATTACK6 ) - pm->ps->legsAnimTimer >= 1100 ) )
    ...
    */
    ...
    case BOTH_LK_DL_S_S_SB_1_W:
      if (saberNum !=
          0) { // only right hand saber does damage in this suberbreak
        return FORCE_LEVEL_0;
      }
      if (animTimer < 900) { // end of anim
        return FORCE_LEVEL_0;
...
void WP_SaberDamageAdd(int trVictimEntityNum, vec3_t trDmgDir, vec3_t trDmgSpot,
                       int trDmg, qboolean doDismemberment,
                       int knockBackFlags) {
  ...
  if (trDmg) { // did some damage to something
    ...
    for (i = 0; i < numVictims; i++) {
      if (victimEntityNum[i] ==
          trVictimEntityNum) { // already hit this guy before
        curVictim = i;
        break;
      }
    }
    if (i == numVictims) { // haven't hit his guy before
      if (numVictims + 1 >=
          MAX_SABER_VICTIMS) { // can't add another victim at this time
        return;
      }
...
void WP_SaberApplyDamage(gentity_t *self) {
  ...
  for (i = 0; i < numVictims; i++) {
    ...
    G_Damage(victim, self, self, dmgDir[i], dmgSpot[i], totalDmg[i], dflags,
             MOD_SABER);
  }
}
  ...
  for (i = 0; i < numVictims; i++) {
    ...
    if (victim->client) {
      ...
      if (npc_class == CLASS_SEEKER || npc_class == CLASS_PROBE ||
          ...
          npc_class == CLASS_SENTRY) { // don't make "blood" sparks for droids.
        isDroid = qtrue;
      }
    }
    ...
    if (te) {
      ...
      if (!te->s.angles[0] && !te->s.angles[1] &&
          !te->s.angles[2]) { // don't let it play with no direction
        te->s.angles[1] = 1;
      }
      ...
      if (!isDroid && (victim->client || victim->s.eType == ET_NPC ||
        ...
        if (!WP_SaberBladeUseSecondBladeStyle(&self->client->saber[saberNum],
                                              bladeNum) &&
            (self->client->saber[saberNum].saberFlags2 &
             SFL2_NO_CLASH_FLARE)) { // don't do clash flare
        } else if (WP_SaberBladeUseSecondBladeStyle(
                       &self->client->saber[saberNum], bladeNum) &&
                   (self->client->saber[saberNum].saberFlags2 &
                    SFL2_NO_CLASH_FLARE2)) { // don't do clash flare
        } else {
          if (totalDmg[i] >
              SABER_NONATTACK_DAMAGE) { // I suppose I could tie this into the
                                        // saberblock event, but I'm tired of
                                        // adding flags to that thing.
...
void WP_SaberRadiusDamage(gentity_t *ent, vec3_t point, float radius,
  ...
  if (!ent || !ent->client) {
    ...
    for (i = 0; i < numEnts; i++) {
      ...
      if (radiusEnt->client == NULL) {               // must be a client
        if (G_EntIsBreakable(radiusEnt->s.number)) { // damage breakables within
                                                     // range, but not as much
          G_Damage(radiusEnt, ent, ent, vec3_origin, radiusEnt->r.currentOrigin,
                   10, 0, MOD_MELEE);
        }
        continue;
      }

      if ((radiusEnt->client->ps.eFlags2 &
           EF2_HELD_BY_MONSTER)) { // can't be one being held
        continue;
      }
      ...
      if (dist <= radius) { // in range
        if (damage > 0) {   // do damage
          ...
          G_Damage(radiusEnt, ent, ent, vec3_origin, radiusEnt->r.currentOrigin,
                   points, DAMAGE_NO_KNOCKBACK, MOD_MELEE);
        }
        if (knockBack > 0) { // do knockback
              ...
              !(radiusEnt->flags & FL_NO_KNOCKBACK)) // don't throw them back
          {
            ...
            if (radiusEnt->health > 0) { // still alive
              if (knockbackStr > 50) { // close enough and knockback high enough
                                       // to possibly knock down
                if (dist < (radius * 0.5f) ||
                    radiusEnt->client->ps.groundEntityNum !=
                        ENTITYNUM_NONE) { // within range of my fist or within
                                          // ground-shaking range and not in the
                                          // air
...
void WP_SaberBounceSound(gentity_t *ent, int saberNum, int bladeNum) {
  ...
  index = Q_irand(1, 9);
  if (!WP_SaberBladeUseSecondBladeStyle(&ent->client->saber[saberNum],
                                        bladeNum) &&
      ent->client->saber[saberNum].bounceSound[0]) {
    G_Sound(ent, CHAN_AUTO,
            ent->client->saber[saberNum].bounceSound[Q_irand(0, 2)]);
  } else if (WP_SaberBladeUseSecondBladeStyle(&ent->client->saber[saberNum],
                                              bladeNum) &&
             ent->client->saber[saberNum].bounce2Sound[0]) {
    G_Sound(ent, CHAN_AUTO,
            ent->client->saber[saberNum].bounce2Sound[Q_irand(0, 2)]);
  }
    ...
    G_Sound(ent, CHAN_AUTO,
            ent->client->saber[saberNum].blockSound[Q_irand(0, 2)]);
  } else if (WP_SaberBladeUseSecondBladeStyle(&ent->client->saber[saberNum],
                                              bladeNum) &&
             ent->client->saber[saberNum].block2Sound[0]) {
    G_Sound(ent, CHAN_AUTO,
            ent->client->saber[saberNum].block2Sound[Q_irand(0, 2)]);
  } else {
    G_Sound(ent, CHAN_AUTO,
            G_SoundIndex(va("sound/weapons/saber/saberblock%d.wav", index)));
  }
}
...
// rww - MP version of the saber damage function. This is where all the things
// like blocking, triggering a parry, triggering a broken parry, doing actual
// damage, etc. are done for the saber. It doesn't resemble the SP version very
// much, but functionality is (hopefully) about the same. This is a large
// function. I feel sort of bad inlining it. But it does get called tons of times
// per frame.
qboolean BG_SuperBreakWinAnim(int anim);

static QINLINE qboolean CheckSaberDamage(gentity_t *self, int rSaberNum,
                                         int rBladeNum, vec3_t saberStart,
                                         vec3_t saberEnd,
                                         qboolean doInterpolate, int trMask,
  ...
  saberBoxSize +=
      (self->client->saber[rSaberNum].blade[rBladeNum].radius * 0.5f);

  if (self->client->ps.weaponTime <=
      0) { // if not doing any attacks or anything, just use point traces.
    ...
    if (d_saberSPStyleDamage.integer) { // SP-size saber damage traces
      ...
      VectorSet(saberTrMins, -saberBoxSize * 3, -saberBoxSize * 3,
                -saberBoxSize * 3);
      VectorSet(saberTrMaxs, saberBoxSize * 3, saberBoxSize * 3,
                saberBoxSize * 3);
    }
  } else if (self->client->ps.fd.saberAnimLevel <
             FORCE_LEVEL_2) { // box trace for fast, because it doesn't get
                              // updated so often
    ...
    saberBoxSize =
        (self->client->saber[rSaberNum].blade[rBladeNum].radius * 0.4f);
  ...
  while (!saberTraceDone) {
    if (doInterpolate &&
        !d_saberSPStyleDamage
             .integer) { // This didn't quite work out like I hoped. But it's
      ...
      if ((level.time -
           self->client->saber[rSaberNum].blade[rBladeNum].trail.lastTime) >
          100) { // no valid last pos, use current
        VectorCopy(saberStart, oldSaberStart);
        VectorCopy(saberEnd, oldSaberEnd);
      } else { // trace from last pos
        VectorCopy(self->client->saber[rSaberNum].blade[rBladeNum].trail.base,
                   oldSaberStart);
        VectorCopy(self->client->saber[rSaberNum].blade[rBladeNum].trail.tip,
                   oldSaberEnd);
      }
      ...
      trap->Trace(&tr, saberEnd, saberTrMins, saberTrMaxs, saberStart,
                  self->s.number, trMask, qfalse, 0, 0);

      VectorCopy(saberEnd, lastValidStart);
      VectorCopy(saberStart, lastValidEnd);
      if (tr.entityNum < MAX_CLIENTS) {
        G_G2TraceCollide(&tr, lastValidStart, lastValidEnd, saberTrMins,
                         saberTrMaxs);
      } else if (tr.entityNum < ENTITYNUM_WORLD) {
        gentity_t *trHit = &g_entities[tr.entityNum];

        if (trHit->inuse &&
            trHit->ghoul2) { // hit a non-client entity with a g2 instance
          G_G2TraceCollide(&tr, lastValidStart, lastValidEnd, saberTrMins,
                           saberTrMaxs);
        }
      }
      ...
      while (tr.fraction == 1.0 && traceTests < 4 &&
             tr.entityNum >= ENTITYNUM_NONE) {
        if ((level.time -
             self->client->saber[rSaberNum].blade[rBladeNum].trail.lastTime) >
            100) { // no valid last pos, use current
      ...
      if (extrapolate) { // extrapolate 16
        ...
        VectorMA(saberStart, SABER_EXTRAPOLATE_DIST, diff,
                 saberEndExtrapolated);
      } else {
        VectorCopy(saberEnd, saberEndExtrapolated);
      }
      trap->Trace(&tr, saberStart, saberTrMins, saberTrMaxs,
                  saberEndExtrapolated, self->s.number, trMask, qfalse, 0, 0);
  ...
  if (self->client->ps.saberAttackWound < level.time &&
      (SaberAttacking(self) ||
       ...
       (d_saberSPStyleDamage.integer && self->client->ps.saberInFlight &&
        rSaberNum == 0) ||
       (WP_SaberBladeDoTransitionDamage(&self->client->saber[rSaberNum],
                                        rBladeNum) &&
        BG_SaberInTransitionAny(self->client->ps.saberMove)) ||
       (self->client->ps.m_iVehicleNum &&
        self->client->ps.saberMove >
            LS_READY))) { // this animation is that of the last attack movement,
                          // and so it should do full damage
    ...
    if (d_saberSPStyleDamage.integer) {
      ...
      if (self->client->ps.saberInFlight) {
        ...
        if (!saberEnt ||
            !saberEnt->s.saberInFlight) { // does less damage on the way back
          fDmg = 1.0f;
        ...
        if (g_saberRealisticCombat.integer) {
          ...
          if (self->client->ps.torsoAnim == BOTH_SPINATTACK6 ||
              self->client->ps.torsoAnim ==
                  BOTH_SPINATTACK7) { // too easy to do, lower damage
            fDmg = 2.5f;
          } else {
      ...
      if (g_saberRealisticCombat.integer >
          1) { // always do damage, and lots of it
        if (g_saberRealisticCombat.integer >
            2) { // always do damage, and lots of it
          fDmg = 25.0f;
        } else if (fDmg > 0.1f) { // only do super damage if we would have done
                                  // damage according to normal rules
          fDmg = 25.0f;
        }
      }
      /*
      ...
      */
      if (level.gametype != GT_DUEL && level.gametype != GT_POWERDUEL &&
          level.gametype !=
              GT_SIEGE) { // in faster-paced games, sabers do more damage
        fDmg *= 2.0f;
      }
      if (fDmg) { // the longer the trace, the more damage it does
        // FIXME: in SP, we only use the part of the trace that's actually
        // *inside* the hit ent...
        float traceLength = Distance(saberEnd, saberStart);
        if (tr.fraction >= 1.0f) { // allsolid?
          dmg = ceil(fDmg * traceLength * 0.1f * 0.33f);
        } else { // fractional hit, the sooner you hit in the trace, the more
                 // damage you did
          dmg = ceil(fDmg * traceLength * (1.0f - tr.fraction) * 0.1f *
                     0.33f); //(1.0f-tr.fraction) isn't really accurate, but
                             //kind of simulates what we have in SP
        }
#ifdef DEBUG_SABER_BOX
      ...
      self->s.number, dmg, animTable[self->client->ps.torsoAnim].name,
      ...
      */
      if (self->client->ps.torsoAnim == BOTH_A1_SPECIAL ||
          ...
          self->client->ps.torsoAnim ==
              BOTH_A3_SPECIAL) { // parry/block/break-parry bonus for
                                 // single-style kata moves
        attackStr++;
      }
      ...
      if (self->client->ps.fd.saberAnimLevel == SS_STAFF ||
        ...
        if (saberInSpecial) {
          ...
          if (self->client->ps.saberMove == LS_SPINATTACK ||
              self->client->ps.saberMove ==
                  LS_SPINATTACK_DUAL) { // these attacks are long and have the
                                        // potential to hit a lot so they will do
                                        // less damage.
            dmg = 10;
          } else {
            if (BG_KickingAnim(self->client->ps.legsAnim) ||
                BG_KickingAnim(
                    self->client->ps.torsoAnim)) { // saber shouldn't do more
                                                   // than min dmg during kicks
              dmg = 2;
            } else if (BG_SaberInKata(
                           self->client->ps.saberMove)) { // special kata move
              if (self->client->ps.fd.saberAnimLevel ==
                  SS_DUAL) { // this is the nasty saber twirl, do big damage
                             // cause it makes you vulnerable
                dmg = 90;
              } else { // staff kata
        ...
        } else { // otherwise we'll ramp up to 70 I guess, for both dual and
                 // staff
        ...
        if (!saberInSpecial && !inBackAttack) {
          ...
          dmg = G_GetAttackDamage(
              self, 2, 30,
              0.5f); // can hit multiple times (and almost always does), so..
        } else {
          dmg = 100;
        }
        ...
        if (saberInSpecial && (self->client->ps.saberMove == LS_A_FLIP_STAB ||
                               self->client->ps.saberMove ==
                                   LS_A_FLIP_SLASH)) { // a well-timed hit with
                                                       // this can do a full 85
      ...
      attackStr = self->client->ps.fd.saberAnimLevel;
             ...
             self->client->ps.saberIdleWound <
                 level.time) { // just touching, do minimal damage and only
                               // check for it every 200ms (mainly to cut down on
                               // network traffic for hit events)
    if ((self->client->saber[0].saberFlags2 &
         SFL2_NO_IDLE_EFFECT)) { // no idle damage or effects
      return qtrue; // true cause even though we didn't get a hit, we don't want
                    // to do those extra traces because the debounce time says
                    // not to.
    }
    trMask &= ~CONTENTS_LIGHTSABER;
    if (d_saberSPStyleDamage.integer) {
    ...
    return qtrue; // true cause even though we didn't get a hit, we don't want
                  // to do those extra traces because the debounce time says not
                  // to.
  }
  ...
  if (BG_SaberInSpecial(self->client->ps.saberMove)) {
    ...
    if (d_saberSPStyleDamage.integer) {
      ...
      if (self->client->ps.saberMove ==
          LS_A_JUMP_T__B_) { // do extra damage for special unblockables
        dmg += 5; // This is very tiny, because this move has a huge damage ramp
      } else if (self->client->ps.saberMove == LS_A_FLIP_STAB ||
                 self->client->ps.saberMove == LS_A_FLIP_SLASH) {
        dmg += 5; // ditto
        if (dmg <= 40 || G_GetAnimPoint(self) <=
                             0.4f) { // sort of a hack, don't want it doing big
                                     // damage in the off points of the anim
          dmg = 2;
        }
        ...
        if (G_GetAnimPoint(self) <= 0.4f) { // same as above
          dmg = 2;
                 ...
                 self->client->ps.saberMove ==
                     LS_SPINATTACK_DUAL) { // do a constant significant amount
                                           // of damage but ramp up a little to
                                           // the mid-point
        ...
        if (BG_KickingAnim(self->client->ps.legsAnim) ||
            BG_KickingAnim(
                self->client->ps.torsoAnim)) { // saber shouldn't do more than
                                               // min dmg during kicks
          dmg = 2;
        } else { // auto-ramp it I guess since it's a special we don't have a
                 // special case for.
  ...
  if (dmg > SABER_NONATTACK_DAMAGE) {
    ...
    if (!WP_SaberBladeUseSecondBladeStyle(&self->client->saber[rSaberNum],
                                          rBladeNum) &&
        self->client->saber[rSaberNum].damageScale != 1.0f) {
      dmg = ceil((float)dmg * self->client->saber[rSaberNum].damageScale);
    } else if (WP_SaberBladeUseSecondBladeStyle(&self->client->saber[rSaberNum],
                                                rBladeNum) &&
    ...
    if ((self->client->ps.brokenLimbs & (1 << BROKENLIMB_RARM)) ||
        (self->client->ps.brokenLimbs &
         (1 << BROKENLIMB_LARM))) { // weaken it if an arm is broken
      dmg *= 0.3;
      if (dmg <= SABER_NONATTACK_DAMAGE) {
  ...
  if (dmg > SABER_NONATTACK_DAMAGE &&
      self->client->ps
          .isJediMaster) { // give the Jedi Master more saber attack power
    dmg *= 2;
  }

  if (dmg > SABER_NONATTACK_DAMAGE && level.gametype == GT_SIEGE &&
      self->client->siegeClass != -1 &&
      (bgSiegeClasses[self->client->siegeClass].classflags &
       (1 << CFL_MORESABERDMG))) { // this class is flagged to do extra saber
                                   // damage. I guess 2x will do for now.
    dmg *= 2;
  }

  if (level.gametype == GT_POWERDUEL &&
      self->client->sess.duelTeam ==
          DUELTEAM_LONE) { // always x2 when we're powerdueling alone... er, so,
                           // we apparently no longer want this?  So they say.
    if (duel_fraglimit.integer) {
      // dmg *= 1.5 - (.4 * (float)self->client->sess.wins /
  ...
  if (tr.entityNum == ENTITYNUM_WORLD ||
      g_entities[tr.entityNum].s.eType ==
          ET_TERRAIN) { // register this as a wall hit for jedi AI
  ...
  if (saberHitWall &&
      ...
      (BG_SaberInAttackPure(
           self->client->ps.saberMove) // only in a normal attack anim
       || self->client->ps.saberMove ==
              LS_A_JUMP_T__B_) // or in the strong jump-fwd-attack "death from
                               // above" move
  ) { // then bounce off
    /*
    ...
    {
            if ((tr.plane.normal[2] >= 0.8f || VectorCompare(tr.plane.normal,
    vec3_origin)) && self->client->ps.viewangles[PITCH] <= 30.0f &&
    ...
    */
    {
      ...
      */
      ...
      self->client->ps.saberMove =
          BG_BrokenParryForAttack(self->client->ps.saberMove);
      self->client->ps.saberBlocked = BLOCKED_PARRY_BROKEN;
      if (self->client->ps.torsoAnim ==
          self->client->ps.legsAnim) { // set anim now on both parts
        int anim = saberMoveData[self->client->ps.saberMove].animToUse;
        G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, anim,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
      }
      ...
      te->s.otherEntityNum =
          ENTITYNUM_NONE; // we didn't hit anyone in particular
      te->s.otherEntityNum2 = self->s.number; // send this so it knows who we
                                              // are
      ...
      if (!WP_SaberBladeUseSecondBladeStyle(&self->client->saber[rSaberNum],
                                            rBladeNum)) {
        WP_SaberRadiusDamage(self, tr.endpos,
                             self->client->saber[rSaberNum].splashRadius,
                             self->client->saber[rSaberNum].splashDamage,
                             self->client->saber[rSaberNum].splashKnockback);
      } else {
        WP_SaberRadiusDamage(self, tr.endpos,
                             self->client->saber[rSaberNum].splashRadius2,
                             self->client->saber[rSaberNum].splashDamage2,
  ...
  // rww - I'm saying || tr.startsolid here, because otherwise your saber tends
  // to skip positions and go through people, and the compensation traces start
  // in their bbox too. Which results in the saber passing through people when
  // you visually cut right through them. Which sucks.

  if ((tr.fraction != 1 || tr.startsolid) &&
      g_entities[tr.entityNum].takedamage &&
      (g_entities[tr.entityNum].health > 0 ||
       !(g_entities[tr.entityNum].s.eFlags & EF_DISINTEGRATION)) &&
      tr.entityNum != self->s.number &&
      g_entities[tr.entityNum]
          .inuse) { // hit something that had health and takes damage
    if (idleDamage && g_entities[tr.entityNum].client &&
    ...
    if (BG_StabDownAnim(self->client->ps.torsoAnim) &&
        ...
        !BG_InKnockDownOnGround(
            &g_entities[tr.entityNum]
                 .client->ps)) { // stabdowns only damage people who are
    ...
    if (!d_saberSPStyleDamage.integer // let's trying making blocks have to be
                                      // blocked by a saber
        && g_entities[tr.entityNum].client && !unblockable &&
        WP_SaberCanBlock(&g_entities[tr.entityNum], tr.endpos, 0, MOD_SABER,
                         qfalse,
                         attackStr)) { // hit a client who blocked the attack
                                       // (fake: didn't actually hit their saber)
      if (dmg <= SABER_NONATTACK_DAMAGE) {
        self->client->ps.saberIdleWound =
            level.time + g_saberDmgDelay_Idle.integer;
      }
      saberDoClashEffect = qtrue;
      VectorCopy(tr.endpos, saberClashPos);
      VectorCopy(tr.plane.normal, saberClashNorm);
      ...
      if (dmg > SABER_NONATTACK_DAMAGE) {
        ...
        if ((g_entities[tr.entityNum]
            ...
            Q_irand(1, 10) <
                lockFactor *
                    2) { // Just got blocked by someone with a decently higher
                         // attack level, so enter into a lock (where they have
                         // the advantage due to a higher attack lev)
      ...
      if (g_entities[tr.entityNum]
              .client) { // not a "jedi", so make them suffer more
        if (dmg > SABER_NONATTACK_DAMAGE) { // don't bother increasing just for
                                            // idle touch damage
          dmg *= 1.5;
        }
      }
      /*
      if (g_entities[tr.entityNum].client
              && g_entities[tr.entityNum].client->ps.weapon != WP_SABER
      )//fd.forcePowerLevel[FP_SABER_OFFENSE]) { //not a "jedi", so make them
      suffer more if ( dmg > SABER_NONATTACK_DAMAGE ) { //don't bother
      increasing just for idle touch damage dmg *= 1.5;
              }
      }
      */
      ...
      if (!d_saberSPStyleDamage.integer) {
        if (g_entities[tr.entityNum].client &&
            g_entities[tr.entityNum].client->ps.weapon ==
                WP_SABER) { // for jedi using the saber, half the damage (this
                            // comes with the increased default dmg debounce
                            // time)
          if (level.gametype != GT_SIEGE) { // unless siege..
            if (dmg > SABER_NONATTACK_DAMAGE &&
                !unblockable) { // don't reduce damage if it's only 1, or if
                                // this is an unblockable attack
      ...
      if (self->s.eType == ET_NPC && g_entities[tr.entityNum].client &&
          self->client->playerTeam ==
              g_entities[tr.entityNum]
                  .client->playerTeam) { // Oops. Since he's an NPC, we'll be
                                         // forgiving and cut the damage down.
        dmg *= 0.2f;
      }
      ...
      if (!WP_SaberBladeUseSecondBladeStyle(&self->client->saber[rSaberNum],
                                            rBladeNum) &&
          !(self->client->saber[rSaberNum].saberFlags2 &
      ...
      WP_SaberDamageAdd(tr.entityNum, dir, tr.endpos, dmg, doDismemberment,
                        knockbackFlags);
    ...
    if (otherOwner && otherOwner->client &&
        otherOwner->client->ps.saberInFlight) { // don't do extra collision
                                                // checking vs sabers in air
      ...
      if (d_saberSPStyleDamage.integer) { // use SP-style blade-collision test
        if (!WP_SabersIntersect(self, rSaberNum, rBladeNum, otherOwner,
                                qfalse)) { // sabers did not actually intersect
          return qfalse;
        }
      } else { // MP-style
        if (!G_SaberCollide(
                self, otherOwner, lastValidStart, lastValidEnd, saberTrMins,
                saberTrMaxs,
    ...
    if ((self->s.eType == ET_NPC ||
         otherOwner->s.eType == ET_NPC) && // just make sure one of us is an npc
        self->client->playerTeam == otherOwner->client->playerTeam &&
        level.gametype != GT_SIEGE) { // don't hit your teammate's sabers if you
                                      // are an NPC. It can be rather annoying.
      return qfalse;
    }
    ...
    if (saberCheckKnockdown_Smashed(&g_entities[tr.entityNum], otherOwner, self,
                                    dmg)) { // smashed it out of the air
      return qfalse;
    }
    ...
    if (self->client->ps.saberEntityNum && self->client->ps.saberInFlight &&
        ...
        saberCheckKnockdown_Smashed(
            &g_entities[self->client->ps.saberEntityNum], self, otherOwner,
            dmg)) { // they smashed it out of the air
      return qfalse;
    }
  ...
  blockStuff:
    ...
    otherSaberLevel =
        G_SaberAttackPower(otherOwner, SaberAttacking(otherOwner));
    ...
    if (sabersClashed && dmg > SABER_NONATTACK_DAMAGE &&
      ...
      // if (Q_irand(1, 10) <= 6)
      if (1) // for now, just always try a deflect. (deflect func can cause
             // bounces too)
      {
        if (!WP_GetSaberDeflectionAngle(self, otherOwner, tr.fraction)) {
          tryDeflectAgain =
              qtrue; // Failed the deflect, try it again if we can if the guy
                     // we're smashing goes into a parry and we don't break it
        } else {
...
#ifndef FINAL_BUILD
        if (g_saberDebugPrint.integer) {
          Com_Printf("Client %i clashed into client %i's saber, did "
                     "BLOCKED_ATK_BOUNCE\n",
    ...
    if (((selfSaberLevel < FORCE_LEVEL_3 &&
          ((tryDeflectAgain && Q_irand(1, 10) <= 3) ||
           (!tryDeflectAgain && Q_irand(1, 10) <= 7))) ||
        ...
        && (otherSaberLevel > FORCE_LEVEL_2 ||
            (otherOwner->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] >= 3 &&
             Q_irand(0, otherSaberLevel))) &&
        !unblockable && !otherUnblockable && dmg > SABER_NONATTACK_DAMAGE &&
        !didOffense) // don't allow the person we're attacking to do this if
                     // we're making an unblockable attack
    { // knockaways can make fast-attacker go into a broken parry anim if the
      // ent is using fast or med. In MP, we also randomly decide this for level
      // 3 attacks.
      // Going to go ahead and let idle damage do simple knockaways. Looks sort
      // of good that way. turn the parry into a knockaway
      ...
      if (!PM_SaberInParry(otherOwner->client->ps.saberMove)) {
        ...
        otherOwner->client->ps.saberMove = G_KnockawayForParry(
            otherOwner->client->ps
                .saberMove); // BG_KnockawayForParry(
...
#ifndef FINAL_BUILD
      if (g_saberDebugPrint.integer) {
        Com_Printf("Client %i sent client %i into a reflected attack with a "
                   "knockaway\n",
      ...
      didDefense = qtrue;
                ...
                unblockable) && // if we're doing a special attack, we can send
                                // them into a broken parry too (MP only)
               (otherOwner->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] <
                    selfSaberLevel ||
                (otherOwner->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] ==
                     selfSaberLevel &&
                 (Q_irand(1, 10) >= otherSaberLevel * 1.5 || unblockable))) &&
               PM_SaberInParry(otherOwner->client->ps.saberMove) &&
               !PM_SaberInBrokenParry(otherOwner->client->ps.saberMove) &&
               !PM_SaberInParry(self->client->ps.saberMove) &&
               !PM_SaberInBrokenParry(self->client->ps.saberMove) &&
               !PM_SaberInBounce(self->client->ps.saberMove) &&
               dmg > SABER_NONATTACK_DAMAGE && !didOffense &&
               !otherUnblockable) { // they are in a parry, and we are slamming
                                    // down on them with a move of equal or
                                    // greater force than their defense, so send
                                    // them into a broken parry.. unless they are
                                    // already in one.
      if (otherOwner->client->ps
              .saberEntityNum) // make sure he has his saber still
      {
        saberCheckKnockdown_BrokenParry(
            &g_entities[otherOwner->client->ps.saberEntityNum], otherOwner,
            self);
      }
      ...
      didDefense = qtrue;
    } else if ((selfSaberLevel >
                FORCE_LEVEL_2) && // if we're doing a special attack, we can
                                  // send them into a broken parry too (MP only)
                                  //(
                                  //otherOwner->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE]
                                  //< selfSaberLevel ||
                                  //(otherOwner->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE]
                                  //== selfSaberLevel && (Q_irand(1, 10) >=
               ...
               dmg > SABER_NONATTACK_DAMAGE && !didOffense &&
               !unblockable) { // they are in a parry, and we are slamming down
                               // on them with a move of equal or greater force
                               // than their defense, so send them into a broken
                               // parry.. unless they are already in one.
#ifndef FINAL_BUILD
      if (g_saberDebugPrint.integer) {
      ...
      didOffense = qtrue;
    } else if (SaberAttacking(otherOwner) && dmg > SABER_NONATTACK_DAMAGE &&
               !BG_SaberInSpecial(otherOwner->client->ps.saberMove) &&
               !didOffense &&
               !otherUnblockable) { // they were attacking and our saber hit
                                    // their saber, make them bounce. But if
                                    // they're in a special attack, leave them.
      if (!PM_SaberInBounce(self->client->ps.saberMove) &&
          !PM_SaberInBounce(otherOwner->client->ps.saberMove) &&
          !PM_SaberInDeflect(self->client->ps.saberMove) &&
          !PM_SaberInDeflect(otherOwner->client->ps.saberMove) &&
        ...
        if (g_saberDebugPrint.integer) {
          Com_Printf(
              "Client %i and client %i bounced off of each other's sabers\n",
              self->s.number, otherOwner->s.number);
        }
#endif
        ...
        attackAdv =
            (attackStr + attackBonus +
             self->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE]) -
            (defendStr +
             otherOwner->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE]);

        if (attackAdv > 1) { // I won, he should knockaway
          otherOwner->client->ps.saberMove =
              BG_BrokenParryForAttack(otherOwner->client->ps.saberMove);
          otherOwner->client->ps.saberBlocked = BLOCKED_BOUNCE_MOVE;
        } else if (attackAdv > 0) { // I won, he should bounce, I should
                                    // continue
    ...
    if (d_saberGhoul2Collision.integer && !didDefense &&
        dmg <= SABER_NONATTACK_DAMAGE &&
        !otherUnblockable) // with perpoly, it looks pretty weird to have clash
                           // flares coming off the guy's face and whatnot
    {
      if (!PM_SaberInParry(otherOwner->client->ps.saberMove) &&
          !PM_SaberInBrokenParry(otherOwner->client->ps.saberMove) &&
          !BG_SaberInSpecial(otherOwner->client->ps.saberMove) &&
    ...
    } else if (!didDefense && dmg > SABER_NONATTACK_DAMAGE &&
               !otherUnblockable) // if not more than idle damage, don't even
                                  // bother blocking.
    { // block
      if (!PM_SaberInParry(otherOwner->client->ps.saberMove) &&
          !PM_SaberInBrokenParry(otherOwner->client->ps.saberMove) &&
        ...
        if (unblockable) { // It's unblockable. So send us into a broken parry
                           // immediately.
          crushTheParry = qtrue;
        }
        ...
        if (!SaberAttacking(otherOwner)) {
          ...
          if (attackStr +
            ...
            tryDeflectAgain = qtrue;
                   ...
                   (selfSaberLevel == otherSaberLevel &&
                    Q_irand(1, 10) <= 2)) { // they are attacking, and we
                                            // managed to make them break
...
#ifndef FINAL_BUILD
          if (g_saberDebugPrint.integer) {
            Com_Printf("Client %i forced client %i into a broken parry with a "
                       "stronger attack\n",
                       self->s.number, otherOwner->s.number);
          }
#endif
        } else { // They are attacking, so are we, and obviously they have an
                 // attack level higher than or equal to ours
          if (selfSaberLevel == otherSaberLevel) { // equal level, try to bounce
                                                   // off each other's sabers
            if (!didOffense && !PM_SaberInParry(self->client->ps.saberMove) &&
                !PM_SaberInBrokenParry(self->client->ps.saberMove) &&
                !BG_SaberInSpecial(self->client->ps.saberMove) &&
                !PM_SaberInBounce(self->client->ps.saberMove) &&
                !PM_SaberInDeflect(self->client->ps.saberMove) &&
...
#ifndef FINAL_BUILD
            if (g_saberDebugPrint.integer) {
              Com_Printf("Equal attack level bounce/deflection for clients %i "
                         "and %i\n",
                         self->s.number, otherOwner->s.number);
            }
#endif
          ...
          } else if ((level.time - otherOwner->client->lastSaberStorageTime) <
                         500 &&
                     !unblockable) // make sure the stored saber data is updated
          { // They are higher, this means they can actually smash us into a
            // broken parry
            ...
            {
              saberCheckKnockdown_BrokenParry(
                  &g_entities[self->client->ps.saberEntityNum], self,
                  otherOwner);
            }

#ifndef FINAL_BUILD
            if (g_saberDebugPrint.integer) {
              Com_Printf("Client %i hit client %i's stronger attack, was "
                         "forced into a broken parry\n",
                         self->s.number, otherOwner->s.number);
            }
#endif
        ...
        if (crushTheParry &&
            PM_SaberInParry(G_GetParryForBlock(
                otherOwner->client->ps
                    .saberBlocked))) { // This means that the attack actually
                                       // hit our saber, and we went to block it.
                                       // But, one of the above cases says we
                                       // actually can't. So we will be smashed
                                       // into a broken parry instead.
...
#ifndef FINAL_BUILD
          if (g_saberDebugPrint.integer) {
            Com_Printf("Client %i broke through %i's parry with a special or "
                       "stronger attack\n",
                       self->s.number, otherOwner->s.number);
          }
#endif
                       ...
                       otherOwner->client->ps.saberBlocked)) &&
                   !didOffense &&
                   tryDeflectAgain) { // We want to try deflecting again because
                                      // the other is in the parry and we haven't
                                      // made any new moves
    ...
    self->client->ps.saberAttackWound =
        level.time + g_saberDmgDelay_Wound.integer;
  }
...
void G_SPSaberDamageTraceLerped(gentity_t *self, int saberNum, int bladeNum,
                                vec3_t baseNew, vec3_t endNew, int clipmask) {
  vec3_t baseOld, endOld;
  vec3_t mp1, mp2;
  vec3_t md1, md2;

  if ((level.time -
       self->client->saber[saberNum].blade[bladeNum].trail.lastTime) >
      100) { // no valid last pos, use current
    VectorCopy(baseNew, baseOld);
    VectorCopy(endNew, endOld);
  } else { // trace from last pos
    VectorCopy(self->client->saber[saberNum].blade[bladeNum].trail.base,
               baseOld);
  ...
  if (VectorCompare2(baseOld, baseNew) &&
      VectorCompare2(endOld, endNew)) { // no diff
    CheckSaberDamage(self, saberNum, bladeNum, baseNew, endNew, qfalse,
                     clipmask, qfalse);
    ...
    CheckSaberDamage(self, saberNum, bladeNum, bladePointOld, bladePointNew,
                     qfalse, clipmask, qtrue);
    ...
    if (saberHitFraction < 1.0f) {
      ...
      VectorMA(baseNew, self->client->saber[saberNum].blade[bladeNum].lengthMax,
               md2, endNew);
    }

    // If the angle diff in the blade is high, need to do it in chunks of 33 to
    // avoid flattening of the arc
    if (BG_SaberInAttack(self->client->ps.saberMove) ||
    ...
    //|| (g_timescale->value<1.0f&&BG_SaberInTransitionAny(
    ...
    // NOTE: if saber spun at least 180 degrees since last damage trace, this is
    // not reliable...!
    if (fabs(curDirFrac) <
        1.0f - MAX_SABER_SWING_INC) { // the saber blade spun more than 33
                                      // degrees since the last damage trace
    ...
    while (1) {
      ...
      for (
          step = stepsize;
          step <=
          self->client->saber[saberNum]
              .blade[bladeNum]
              .lengthMax /*&& step <
                            self->client->saber[saberNum].blade[bladeNum].lengthOld*/
          ;
          step += stepsize) {
        ...
        if (step + stepsize >=
            self->client->saber[saberNum].blade[bladeNum].lengthMax) {
          extrapolate = qfalse;
        }
        // do the damage trace
        CheckSaberDamage(self, saberNum, bladeNum, bladePointOld, bladePointNew,
                         qfalse, clipmask, extrapolate);
        /*
        if ( WP_SaberDamageForTrace( ent->s.number, bladePointOld,
        bladePointNew, baseDamage, curMD2, qfalse, entPowerLevel,
    ...
    /*
    aveLength = (ent->client->ps.saber[saberNum].blade[bladeNum].lengthOld +
    ent->client->ps.saber[saberNum].blade[bladeNum].length)/2; if ( step >
    aveLength )
    ...
    if ( WP_SaberDamageForTrace( ent->s.number, endOld, endNew,
    tipDmgMod*baseDamage, md2, qfalse, entPowerLevel,
...
qboolean InFOV3(vec3_t spot, vec3_t from, vec3_t fromAngles, int hFOV,
                int vFOV);
...
evasionType_t Jedi_SaberBlockGo(gentity_t *self, usercmd_t *cmd, vec3_t pHitloc,
                                vec3_t phitDir, gentity_t *incoming,
                                float dist);
...
void WP_SaberStartMissileBlockCheck(gentity_t *self, usercmd_t *ucmd) {
  ...
  if (!(self->client->ps.eFlags2 &
        EF2_HELD_BY_MONSTER)) { // lookTarget is set by and to the monster
                                // that's holding you, no other operations can
                                // change that
  ...
  if (self->client->ps.weapon != WP_SABER &&
    ...
    doFullRoutine = qfalse;
             ...
             (1 << FP_LIGHTNING)) { // can't block while zapping
    doFullRoutine = qfalse;
  } else if (self->client->ps.fd.forcePowersActive &
             (1 << FP_DRAIN)) { // can't block while draining
    doFullRoutine = qfalse;
  } else if (self->client->ps.fd.forcePowersActive &
             (1 << FP_PUSH)) { // can't block while shoving
    doFullRoutine = qfalse;
  } else if (self->client->ps.fd.forcePowersActive &
             (1 << FP_GRIP)) { // can't block while gripping (FIXME: or should
                               // it break the grip?  Pain should break the grip,
                               // I think...)
    doFullRoutine = qfalse;
  }

  if (self->client->ps.weaponTime > 0) { // don't autoblock while busy with
                                         // stuff
    return;
  }

  if ((self->client->saber[0].saberFlags &
       SFL_NOT_ACTIVE_BLOCKING)) { // can't actively block with this saber type
    return;
  }

  if (self->health <= 0) { // dead don't try to block (NOTE: actual deflection
                           // happens in missile code)
    return;
  }
  ...
  if (self->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] >
      level.time) { // can't block while gripping (FIXME: or should it break the
                    // grip?  Pain should break the grip, I think...)
    doFullRoutine = qfalse;
  }
  ...
  numListedEntities =
      trap->EntitiesInBox(mins, maxs, entityList, MAX_GENTITIES);
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (self->s.eType == ET_PLAYER && ent->client &&
        ...
        (ent->s.eType != ET_NPC ||
         ent->s.NPC_class != CLASS_VEHICLE) && // don't look at vehicle NPCs
      ...
      if (lookTDist == -1 || vecLen < lookTDist) {
        ...
        trap->Trace(&tr, myEyes, NULL, NULL, ent->client->ps.origin,
                    self->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);

        if (tr.fraction == 1.0f ||
            tr.entityNum == ent->s.number) { // we have a clear line of sight to
    ...
    if (ent->s.eType != ET_MISSILE &&
      ...
      if (ent->r.ownerNum < 0 ||
          ent->r.ownerNum >=
              ENTITYNUM_WORLD) { // not going to be a client then.
        continue;
      }
      ...
      if (!pOwner->client->ps.saberEntityNum ||
          ...
          pOwner->client->ps.saberEntityNum !=
              ent->s.number) { // the saber is knocked away and/or not flying
                               // actively, or this ent is not the cl's saber ent
                               // at all
        continue;
      }
      ...
      if (ent->s.pos.trType == TR_STATIONARY &&
          self->s.eType == ET_PLAYER) { // nothing you can do with a stationary
                                        // missile if you're the player
        continue;
      }
    }
    ...
    if (ent->s.weapon == WP_THERMAL) { // thermal detonator!
      if (self->NPC && dist < ent->splashRadius) {
        if (dist < ent->splashRadius && ent->nextthink < level.time + 600 &&
            ent->count && self->client->ps.groundEntityNum != ENTITYNUM_NONE &&
            (ent->s.pos.trType == TR_STATIONARY ||
             ent->s.pos.trType == TR_INTERPOLATE ||
             (dot1 = DotProduct(dir, forward)) < SABER_REFLECT_MISSILE_CONE ||
             !WP_ForcePowerUsable(
                 self, FP_PUSH))) { // TD is close enough to hurt me, I'm on the
                                    // ground and the thing is at rest or behind
                                    // me and about to blow up, or I don't have
                                    // force-push so force-jump!
          // FIXME: sometimes this might make me just jump into it...?
          self->client->ps.fd.forceJumpCharge = 480;
        } else if (
            self->client->NPC_class !=
            CLASS_BOBAFETT) { // FIXME: check
                              // forcePushRadius[NPC->client->ps.forcePowerLevel[FP_PUSH]]
      ...
      //			maybe do a force-gesture that makes them
      //explode? 			But what if we're within it's splashradius?
      if (self->s.eType == ET_PLAYER) { // players don't auto-handle these at
                                        // all
        continue;
      } else {
        // if ( ent->s.pos.trType == TR_STATIONARY &&
        ...
        {      // a placed explosive like a tripmine or detpack
          if (InFOV3(ent->r.currentOrigin, self->client->renderInfo.eyePoint,
            ...
            if (G_ClearLOS4(self, ent)) {                    // can see it
              ...
              VectorMA(ent->r.currentOrigin, ent->r.maxs[0] + 4, throwDir,
                       ent->r.currentOrigin);
              ...
              ent->s.pos.trTime =
                  level.time; // move a bit on the very first frame
              VectorCopy(ent->r.currentOrigin, ent->s.pos.trBase);
              ...
              ent->nextthink = level.time + Q_irand(500, 3000);
                   ...
                   (DotProduct(dir, forward) < SABER_REFLECT_MISSILE_CONE ||
                    !WP_ForcePowerUsable(self,
                                         FP_PUSH))) { // NPCs try to evade it
          self->client->ps.fd.forceJumpCharge = 480;
        } else if (self->client->NPC_class !=
                   CLASS_BOBAFETT) { // else, try to force-throw it away
          // FIXME: check
          // forcePushRadius[NPC->client->ps.forcePowerLevel[FP_PUSH]]
          ForceThrow(self, qfalse);
        }
    ...
    if (ent->s.weapon != WP_SABER) { // only block shots coming from behind
      if ((dot1 = DotProduct(dir, forward)) < SABER_REFLECT_MISSILE_CONE)
        continue;
    } else if (self->s.eType ==
               ET_PLAYER) { // player never auto-blocks thrown sabers
      continue;
    ...
    if (dist < closestDist) {
      ...
      trap->Trace(&trace, ent->r.currentOrigin, ent->r.mins, ent->r.maxs,
                  traceTo, ent->s.number, ent->clipmask, qfalse, 0, 0);
      if (trace.allsolid || trace.startsolid ||
          (trace.fraction < 1.0f && trace.entityNum != self->s.number &&
           trace.entityNum !=
               self->client->ps.saberEntityNum)) { // okay, try one more check
        ...
        if (trace.allsolid || trace.startsolid ||
            (trace.fraction < 1.0f && trace.entityNum != self->s.number &&
             trace.entityNum !=
                 self->client->ps.saberEntityNum)) { // can't hit me, ignore it
          continue;
        }
      }
      if (self->s.eType == ET_NPC) { // An NPC
        if (self->NPC && !self->enemy && ent->r.ownerNum != ENTITYNUM_NONE) {
          ...
          if (owner->health >= 0 &&
              (!owner->client ||
               owner->client->playerTeam != self->client->playerTeam)) {
            G_SetEnemy(self, owner);
          }
        }
      }
      // FIXME: if NPC, predict the intersection between my current
      // velocity/path and the missile's, see if it intersects my bounding box
  ...
  if (self->s.eType == ET_NPC &&
      self->localAnimIndex <=
          1) { // humanoid NPCs don't set angles based on server angles for
               // looking, unlike other NPCs
  ...
  if (incoming) {
    if (self->NPC /*&& !G_ControlledByPlayer( self )*/) {
      ...
      if (self->client->NPC_class == CLASS_BOBAFETT &&
          ...
          && incoming->methodOfDeath !=
                 MOD_ROCKET_HOMING) { // a hovering Boba Fett, not a tracking
                                      // rocket
      ...
      } else if (Jedi_SaberBlockGo(self, &self->NPC->last_ucmd, NULL, NULL,
                                   incoming, 0.0f) !=
                 EVASION_NONE) { // make sure to turn on your saber if it's not
                                 // on
        if (self->client->NPC_class != CLASS_BOBAFETT) {
    ...
    } else // player
    {
      ...
      if (owner && owner->client &&
          (!self->enemy ||
           self->enemy->s.weapon != WP_SABER)) // keep enemy jedi over shooters
      {
        self->enemy = owner;
...
static QINLINE qboolean CheckThrownSaberDamaged(gentity_t *saberent,
                                                gentity_t *saberOwner,
                                                gentity_t *ent, int dist,
                                                int returning,
                                                qboolean noDCheck) {
  vec3_t vecsub;
  float veclen;
  ...
  if (ent && ent->client && ent->inuse &&
      ent->s.number != saberOwner->s.number && ent->health > 0 &&
      ent->takedamage &&
      trap->InPVS(ent->client->ps.origin, saberent->r.currentOrigin) &&
      ent->client->sess.sessionTeam != TEAM_SPECTATOR &&
      (ent->client->pers.connected || ent->s.eType == ET_NPC)) { // hit a client
    if (ent->inuse && ent->client && ent->client->ps.duelInProgress &&
        ent->client->ps.duelIndex != saberOwner->s.number) {
      return qfalse;
    }

    if (ent->inuse && ent->client && saberOwner->client->ps.duelInProgress &&
    ...
    if (veclen < dist) { // within range
      ...
      trap->Trace(&tr, saberent->r.currentOrigin, NULL, NULL,
                  ent->client->ps.origin, saberent->s.number, MASK_SHOT, qfalse,
                  0, 0);

      if (tr.fraction == 1 || tr.entityNum == ent->s.number) { // Slice them
        if (!saberOwner->client->ps.isJediMaster &&
            WP_SaberCanBlock(ent, tr.endpos, 0, MOD_SABER, qfalse,
                             999)) { // they blocked it
          ...
          if (saberCheckKnockdown_Thrown(
                  saberent, saberOwner,
                  &g_entities[tr.entityNum])) { // it was knocked out of the air
            return qfalse;
          }
          ...
          if ((saberOwner->client->saber[0].saberFlags2 &
               SFL2_NO_DISMEMBERMENT)) {
            dflags |= DAMAGE_NO_DISMEMBER;
          }
          ...
          if (saberOwner->client->ps
                  .isJediMaster) { // 2x damage for the Jedi Master
            G_Damage(ent, saberOwner, saberOwner, dir, tr.endpos,
                     saberent->damage * 2, dflags, MOD_SABER);
          } else {
            G_Damage(ent, saberOwner, saberOwner, dir, tr.endpos,
                     saberent->damage, dflags, MOD_SABER);
          }
  ...
  } else if (ent && !ent->client && ent->inuse && ent->takedamage &&
             ent->health > 0 && ent->s.number != saberOwner->s.number &&
             ent->s.number != saberent->s.number &&
             (noDCheck ||
              trap->InPVS(ent->r.currentOrigin,
                          saberent->r.currentOrigin))) { // hit a non-client
    ...
    if (veclen < dist) {
      ...
      trap->Trace(&tr, saberent->r.currentOrigin, NULL, NULL, entOrigin,
                  saberent->s.number, MASK_SHOT, qfalse, 0, 0);
      ...
      if (tr.fraction == 1 || tr.entityNum == ent->s.number) {
        ...
        if (ent->s.eType == ET_NPC) { // an animent
          G_Damage(ent, saberOwner, saberOwner, dir, tr.endpos, 40, dflags,
                   MOD_SABER);
        } else {
          G_Damage(ent, saberOwner, saberOwner, dir, tr.endpos, 5, dflags,
                   MOD_SABER);
        }
        ...
        // te->s.otherEntityNum = ent->s.number;
        te->s.otherEntityNum2 =
            saberOwner->s.number; // actually, do send this, though - for the
                                  // overridden per-saber hit effects/sounds
        te->s.weapon = 0;   // saberNum
        te->s.legsAnim = 0; // bladeNum
        VectorCopy(tr.endpos, te->s.origin);
        VectorCopy(tr.plane.normal, te->s.angles);
        ...
        if (ent->s.eType == ET_MOVER) {
          if (saberOwner && saberOwner->client &&
              (saberOwner->client->saber[0].saberFlags2 &
               SFL2_NO_CLASH_FLARE)) { // don't do clash flare - NOTE: assumes
                                       // same is true for both sabers if using
                                       // dual sabers!
            G_FreeEntity(te); // kind of a waste, but...
          } else {
            // I suppose I could tie this into the saberblock event, but I'm
            // tired of adding flags to that thing.
            gentity_t *teS = G_TempEntity(te->s.origin, EV_SABER_CLASHFLARE);
            VectorCopy(te->s.origin, teS->s.origin);
...
static QINLINE void saberCheckRadiusDamage(
    gentity_t *saberent,
    int returning) { // we're going to cheat and damage players within the
                     // saber's radius, just for the sake of doing things more
                     // "efficiently" (and because the saber entity has no server
                     // g2 instance)
  int i = 0;
  int dist = 0;
  gentity_t *ent;
...
static QINLINE void saberMoveBack(gentity_t *ent, qboolean goingBack) {
  ...
  if (!goingBack &&
      ent->s.pos.trType !=
          TR_GRAVITY) { // acts as a fallback in case touch code fails, keeps
                        // saber from going through things between predictions
    ...
    compensatedOrigin[0] =
        oldOrg[0] + calcComp[0] * (originalLength + iCompensationLength);
    compensatedOrigin[1] =
        oldOrg[1] + calcComp[1] * (originalLength + iCompensationLength);
    compensatedOrigin[2] =
        oldOrg[2] + calcComp[2] * (originalLength + iCompensationLength);

    trap->Trace(&tr, oldOrg, mins, maxs, compensatedOrigin, ent->r.ownerNum,
                MASK_PLAYERSOLID, qfalse, 0, 0);

    if ((tr.fraction != 1 || tr.startsolid || tr.allsolid) &&
        tr.entityNum != ent->r.ownerNum &&
        !(g_entities[tr.entityNum].r.contents & CONTENTS_LIGHTSABER)) {
      VectorClear(ent->s.pos.trDelta);

      // Unfortunately doing this would defeat the purpose of the compensation.
      // We will have to settle for a jerk on the client. VectorCopy( origin,
      // ent->r.currentOrigin );

      // we'll skip the dist check, since we don't really care about that (we
      // just hit it physically)
      CheckThrownSaberDamaged(ent, &g_entities[ent->r.ownerNum],
                              &g_entities[tr.entityNum], 256, 0, qtrue);

      if (ent->s.pos.trType ==
          TR_GRAVITY) { // got blocked and knocked away in the damage func
        return;
      }

      tr.startsolid = 0;
      if (tr.entityNum ==
          ENTITYNUM_NONE) { // eh, this is a filthy lie. (obviously it had to
                            // hit something or it wouldn't be in here, so we'll
                            // say it hit the world)
...
void MakeDeadSaber(gentity_t *ent) { // spawn a "dead" saber entity here so it
                                     // looks like the saber fell out of the air.
  ...
  if (level.gametype ==
      GT_JEDIMASTER) { // never spawn a dead saber in JM, because the only saber
                       // on the level is really a world object
  ...
  trap->Trace(&tr, startorg, saberent->r.mins, saberent->r.maxs, startorg,
              saberent->s.number, saberent->clipmask, qfalse, 0, 0);
  if (tr.startsolid ||
      tr.fraction != 1) { // bad position, try popping our origin up a bit
    startorg[2] += 20;
    trap->Trace(&tr, startorg, saberent->r.mins, saberent->r.maxs, startorg,
                saberent->s.number, saberent->clipmask, qfalse, 0, 0);
    if (tr.startsolid ||
        tr.fraction != 1) { // still no luck, try using our owner's origin
  ...
  if (ent->r.ownerNum >= 0 && ent->r.ownerNum < ENTITYNUM_WORLD) {
    ...
    if (owner->inuse && owner->client && owner->client->saber[0].model[0]) {
      WP_SaberAddG2Model(saberent, owner->client->saber[0].model,
...
void DownedSaberThink(gentity_t *saberent) {
  ...
  if (!saberOwn || !saberOwn->inuse || !saberOwn->client ||
  ...
  if (saberOwn->client->ps.saberEntityNum) {
    if (saberOwn->client->ps.saberEntityNum ==
        saberent->s.number) { // owner shouldn't have this set if we're thinking
                              // in here. Must've fallen off a cliff and
                              // instantly respawned or something.
      notDisowned = qtrue;
  ...
  if (notDisowned || saberOwn->health < 1 ||
      !saberOwn->client->ps.fd
           .forcePowerLevel[FP_SABER_OFFENSE]) { // He's dead, just go back to
                                                 // our normal saber status
    saberOwn->client->ps.saberEntityNum = saberOwn->client->saberStoredIndex;

    // MakeDeadSaber(saberent); //spawn a dead saber on top of where we are now.
    // The "bodyqueue" method. Actually this will get taken care of when the
    // thrown saber func sees we're dead.

#ifdef _DEBUG
    if (saberOwn->client->saberStoredIndex !=
        saberent->s.number) { // I'm paranoid.
    ...
    if (saberOwn->health >
        0) { // only set this if he's alive. If dead we want to reflect the lack
             // of saber on the corpse, as he died with his saber out.
      saberOwn->client->ps.saberInFlight = qfalse;
  ...
  if (saberOwn->client->saberKnockedTime < level.time &&
      (saberOwn->client->pers.cmd.buttons &
       BUTTON_ATTACK)) { // He wants us back
    pullBack = qtrue;
  } else if ((level.time - saberOwn->client->saberKnockedTime) >
             MAX_LEAVE_TIME) { // Been sitting around for too long, go back no
...
#define SABER_RETRIEVE_DELAY                                                   \
  3000 // 3 seconds for now. This will leave you nice and open if you lose your
       // saber.
...
void saberKnockDown(gentity_t *saberent, gentity_t *saberOwner,
  ...
  saberOwner->client->ps.saberEntityNum =
      0; // still stored in client->saberStoredIndex
  ...
  //(normally at the player's right hand) could result in the saber being stuck
  // in the the map and then freaking out.
  trap->Trace(&tr, saberent->r.currentOrigin, saberent->r.mins,
              saberent->r.maxs, saberent->r.currentOrigin, saberent->s.number,
              saberent->clipmask, qfalse, 0, 0);
  if (tr.startsolid ||
      tr.fraction != 1) { // bad position, try popping our origin up a bit
  ...
  WP_SaberAddG2Model(saberent, saberOwner->client->saber[0].model,
  ...
  saberent->r.svFlags &= ~(SVF_NOCLIENT); // make sure the client is getting
                                          // updates on where it is and such.
  ...
  if (saberOwner !=
      other) { // if someone knocked it out of the air and it wasn't turned off,
               // go in the direction they were facing.
...
// sort of a silly macro I guess. But if I change anything in here I'll probably
// want it to be everywhere.
#define SABERINVALID                                                           \
  (!saberent || !saberOwner || !other || !saberent->inuse ||                   \
   !saberOwner->inuse || !other->inuse || !saberOwner->client ||               \
   !other->client || !saberOwner->client->ps.saberEntityNum ||                 \
   saberOwner->client->ps.saberLockTime > (level.time - 100))
...
void WP_SaberAddG2Model(gentity_t *saberent, const char *saberModel,
  ...
  trap->G2API_InitGhoul2Model(&saberent->ghoul2, saberModel,
                              saberent->s.modelindex, saberSkin, 0, 0, 0);
}

// Make the saber go flying directly out of the owner's hand in the specified
// direction
qboolean saberKnockOutOfHand(gentity_t *saberent, gentity_t *saberOwner,
                             vec3_t velocity) {
  if (!saberent || !saberOwner || !saberent->inuse || !saberOwner->inuse ||
      !saberOwner->client) {
    return qfalse;
  }
  ...
  if ((level.time - saberOwner->client->lastSaberStorageTime) >
      50) { // must have a reasonably updated saber base pos
    return qfalse;
  }
  ...
  G_SetOrigin(
      saberent,
      saberOwner->client->lastSaberBase_Always); // use this as opposed to the
                                                 // right hand bolt,
  // because I don't want to risk reconstructing the skel again to get it here.
  // And it isn't worth storing.
  saberKnockDown(saberent, saberOwner, saberOwner);
  VectorCopy(velocity,
             saberent->s.pos
                 .trDelta); // override the velocity on the knocked away saber.

  return qtrue;
}

// Called at the result of a circle lock duel - the loser gets his saber tossed
// away and is put into a reflected attack anim
qboolean saberCheckKnockdown_DuelLoss(gentity_t *saberent,
  ...
  if (!other->client->olderIsValid ||
      (level.time - other->client->lastSaberStorageTime) >=
          200) { // see if the spots are valid
  ...
  if (validMomentum) {
    ...
    VectorSubtract(other->client->lastSaberBase_Always,
                   other->client->olderSaberBase, dif);
    totalDistance = VectorNormalize(dif);

    if (!totalDistance) { // fine, try our own
      if (!saberOwner->client->olderIsValid ||
          (level.time - saberOwner->client->lastSaberStorageTime) >= 200) {
        validMomentum = qfalse;
      }

      if (validMomentum) {
        VectorSubtract(saberOwner->client->lastSaberBase_Always,
                       saberOwner->client->olderSaberBase, dif);
        totalDistance = VectorNormalize(dif);
      }
    }
    ...
    if (validMomentum) {
      if (!totalDistance) { // try the difference between the two blades
        VectorSubtract(saberOwner->client->lastSaberBase_Always,
                       other->client->lastSaberBase_Always, dif);
        totalDistance = VectorNormalize(dif);
      }

      if (totalDistance) { // if we still have no difference somehow, just let
                           // it fall to the ground when the time comes.
        if (totalDistance < 20) {
          totalDistance = 20;
        }
  ...
  saberOwner->client->ps.saberMove =
      LS_V1_BL; // rwwFIXMEFIXME: Ideally check which lock it was exactly and
                // use the proper anim (same goes for the attacker)
...
// Called when we want to try knocking the saber out of the owner's hand upon
// them going into a broken parry. Also called on reflected attacks.
qboolean saberCheckKnockdown_BrokenParry(gentity_t *saberent,
                                         gentity_t *saberOwner,
                                         gentity_t *other) {
  int myAttack;
  ...
  if (!other->client->olderIsValid ||
      (level.time - other->client->lastSaberStorageTime) >=
          200) { // if we don't know which way to throw the saber based on
                 // momentum between saber positions, just don't throw it
    return qfalse;
  }

  // only knock the saber out of the hand if they're in a stronger stance I
  // suppose. Makes strong more advantageous.
  if (otherAttack > myAttack + 1 &&
      Q_irand(1, 10) <=
          7) { // This would be, say, strong stance against light stance.
    doKnock = qtrue;
  } else if (otherAttack > myAttack &&
             Q_irand(1, 10) <= 3) { // Strong vs. medium, medium vs. light
    doKnock = qtrue;
  }
...
qboolean saberCheckKnockdown_Smashed(gentity_t *saberent, gentity_t *saberOwner,
  ...
  if (!saberOwner->client->ps.saberInFlight) { // can only do this if the saber
                                               // is already actually in flight
    return qfalse;
  }
  ...
  if (other && other->inuse && other->client &&
      BG_InExtraDefenseSaberMove(
          other->client->ps
              .saberMove)) { // make sure the blow was strong enough
...
// Called upon blocking a thrown saber. If the throw level compared to the
// blocker's defense level is inferior, or equal and a random factor is met, then
// the saber will be tossed to the ground.
qboolean saberCheckKnockdown_Thrown(gentity_t *saberent, gentity_t *saberOwner,
                                    gentity_t *other) {
  int throwLevel = 0;
  int defenLevel = 0;
...
void saberBackToOwner(gentity_t *saberent) {
  ...
  if (!saberOwner->inuse || !saberOwner->client ||
  ...
  if (saberent->speed < level.time) {
    ...
    if (saberOwner->client->ps.fd.forcePowerLevel[FP_SABERTHROW] >=
        FORCE_LEVEL_3) { // allow players with high saber throw rank to control
                         // the return speed of the saber
      baseSpeed = 900;
    ...
    // Gradually slow down as it approaches, so it looks smoother coming into
    // the hand.
    if (ownerLen < 64) {
  ...
  */
  // I'm just doing this now. I don't really like the spin on the way back. And
  // it does weird stuff with the new saber-knocked-away code.
...
void thrownSaberTouch(gentity_t *saberent, gentity_t *other, trace_t *trace) {
  ...
  // we'll skip the dist check, since we don't really care about that (we just
  // hit it physically)
  CheckThrownSaberDamaged(saberent, &g_entities[saberent->r.ownerNum], hitEnt,
                          256, 0, qtrue);
...
void saberFirstThrown(gentity_t *saberent) {
  ...
  if ((level.time - saberOwn->client->ps.saberDidThrowTime) > 500) {
    if (!(saberOwn->client->buttons &
          BUTTON_ALT_ATTACK)) { // If owner releases altattack 500ms or later
                                // after throwing saber, it autoreturns
      thrownSaberTouch(saberent, saberent, NULL);
      goto runMin;
    } else if ((level.time - saberOwn->client->ps.saberDidThrowTime) >
               6000) { // if it's out longer than 6 seconds, return it
      thrownSaberTouch(saberent, saberent, NULL);
      goto runMin;
    }
  }
  ...
  if (!BG_CanUseFPNow(level.gametype, &saberOwn->client->ps, level.time,
                      FP_SABERTHROW)) {
    thrownSaberTouch(saberent, saberent, NULL);
    goto runMin;
  }
  ...
  if (vLen >= (SABER_MAX_THROW_DISTANCE *
               saberOwn->client->ps.fd.forcePowerLevel[FP_SABERTHROW])) {
    thrownSaberTouch(saberent, saberent, NULL);
    goto runMin;
  }

  if (saberOwn->client->ps.fd.forcePowerLevel[FP_SABERTHROW] >= FORCE_LEVEL_2 &&
      saberent->speed < level.time) { // if owner is rank 3 in saber throwing,
    ...
    if (saberOwn->client->ps.fd.forcePowerLevel[FP_SABERTHROW] >=
        FORCE_LEVEL_3) { // if highest saber throw rank, we can direct the saber
                         // toward players directly by looking at them
      trap->Trace(&tr, traceFrom, NULL, NULL, traceTo, saberOwn->s.number,
                  MASK_PLAYERSOLID, qfalse, 0, 0);
    } else {
      trap->Trace(&tr, traceFrom, NULL, NULL, traceTo, saberOwn->s.number,
    ...
    if (saberOwn->client->ps.fd.forcePowerLevel[FP_SABERTHROW] >=
        FORCE_LEVEL_3) { // we'll treat them to a quicker update rate if their
                         // throw rank is high enough
...
void UpdateClientRenderBolts(gentity_t *self, vec3_t renderOrigin,
                             vec3_t renderAngles) {
  mdxaBone_t boltMatrix;
  ...
  if (!self->ghoul2) {
    ...
    // head
    trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->headBolt, &boltMatrix,
                              renderAngles, renderOrigin, level.time, NULL,
                              self->modelScale);
    ...
    // right hand
    trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->handRBolt, &boltMatrix,
                              renderAngles, renderOrigin, level.time, NULL,
                              self->modelScale);
    ...
    // left hand
    trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->handLBolt, &boltMatrix,
                              renderAngles, renderOrigin, level.time, NULL,
                              self->modelScale);
    ...
    // chest
    trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->torsoBolt, &boltMatrix,
                              renderAngles, renderOrigin, level.time, NULL,
                              self->modelScale);
    ...
    // crotch
    trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->crotchBolt, &boltMatrix,
                              renderAngles, renderOrigin, level.time, NULL,
                              self->modelScale);
    ...
    // right foot
    trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->footRBolt, &boltMatrix,
                              renderAngles, renderOrigin, level.time, NULL,
                              self->modelScale);
    ...
    // left foot
    trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->footLBolt, &boltMatrix,
                              renderAngles, renderOrigin, level.time, NULL,
                              self->modelScale);
...
void UpdateClientRenderinfo(gentity_t *self, vec3_t renderOrigin,
                            vec3_t renderAngles) {
  renderInfo_t *ri = &self->client->renderInfo;
  if (ri->mPCalcTime < level.time) {
    ...
    if (self->ghoul2 &&
        self->ghoul2 !=
            ri->lastG2) { // the g2 instance changed, so update all the bolts.
    ...
    // we'll just say the legs/torso are whatever the first frame of our current
    // anim is.
    ri->torsoFrame = bgAllAnims[self->localAnimIndex]
                         .anims[self->client->ps.torsoAnim]
                         .firstFrame;
    ri->legsFrame = bgAllAnims[self->localAnimIndex]
                        .anims[self->client->ps.legsAnim]
                        .firstFrame;
    if (g_debugServerSkel.integer) { // Alright, I was doing this, but it's just
                                     // too slow to do every frame.
      // From now on if we want this data to be valid we're going to have to
      // make a verify call for it before accessing it. I'm only doing this now
      // if we want to debug the server skel by drawing lines from bolt positions
      // every frame.
      mdxaBone_t boltMatrix;
...
extern void G_GetBoltPosition(gentity_t *self, int boltIndex, vec3_t pos,
                              int modelIndex); // NPC_utils.c
...
static qboolean G_KickDownable(gentity_t *ent) {
  ...
  if (ent->client->ps.weaponTime <= 0 && ent->client->ps.weapon == WP_SABER &&
...
static void G_TossTheMofo(gentity_t *ent, vec3_t tossDir, float tossStr) {
  ...
  if (ent->s.eType == ET_NPC && ent->s.NPC_class == CLASS_VEHICLE) { // no,
                                                                     // silly
    return;
  }

  VectorMA(ent->client->ps.velocity, tossStr, tossDir,
  ...
  if (ent->health > 0 &&
    ...
    ent->client->ps.forceDodgeAnim = 0; // this toggles between 1 and 0, when
                                        // it's 1 we should play the get up anim
    // ent->client->ps.quickerGetup = qtrue;
  }
}

static gentity_t *G_KickTrace(gentity_t *ent, vec3_t kickDir, float kickDist,
                              vec3_t kickEnd, int kickDamage, float kickPush) {
  ...
  if (kickEnd &&
      !VectorCompare(kickEnd, vec3_origin)) { // they passed us the end point of
                                              // the trace, just use that
    ...
    VectorSet(traceOrg, ent->r.currentOrigin[0], ent->r.currentOrigin[1],
              ent->r.currentOrigin[2] + ent->r.maxs[2] * 0.5f);
    VectorMA(traceOrg, kickDist, kickDir, traceEnd);
  }

  if (d_saberKickTweak.integer) {
    trap->Trace(&trace, traceOrg, kickMins, kickMaxs, traceEnd, ent->s.number,
                MASK_SHOT, qfalse,
                G2TRFLAG_DOGHOULTRACE | G2TRFLAG_GETSURFINDEX | G2TRFLAG_THICK |
                    G2TRFLAG_HITCORPSES,
  ...
  if (trace.fraction < 1.0f && !trace.startsolid && !trace.allsolid) {
    if (ent->client->jediKickTime > level.time) {
      if (trace.entityNum ==
          ent->client
              ->jediKickIndex) { // we are hitting the same ent we last hit in
                                 // this same anim, don't hit it again
        return NULL;
      }
    }
    ...
    if (hitEnt->inuse) { // we hit an entity
      ...
      if (hitEnt->takedamage) { // hurt it
        ...
        if (d_saberKickTweak.integer) {
          G_Damage(hitEnt, ent, ent, kickDir, trace.endpos, kickDamage * 0.2f,
                   DAMAGE_NO_KNOCKBACK, MOD_MELEE);
        } else {
          G_Damage(hitEnt, ent, ent, kickDir, trace.endpos, kickDamage,
                   DAMAGE_NO_KNOCKBACK, MOD_MELEE);
        }
      }
      if (hitEnt->client &&
          !(hitEnt->client->ps.pm_flags &
            PMF_TIME_KNOCKBACK) // not already flying through air?  Intended to
                                // stop multiple hits, but...
          && G_CanBeEnemy(ent, hitEnt)) { // FIXME: this should not always work
        if (hitEnt->health <= 0) {        // we kicked a dead guy
          // throw harder - FIXME: no matter how hard I push them, they don't go
          // anywhere... corpses use less physics???
...
static void G_KickSomeMofos(gentity_t *ent) {
  ...
  float animLength = BG_AnimLength(ent->localAnimIndex,
                                   (animNumber_t)ent->client->ps.legsAnim);
  float elapsedTime = (float)(animLength - ent->client->ps.legsTimer);
  float remainingTime = (animLength - elapsedTime);
  float kickDist =
      (ent->r.maxs[0] * 1.5f) + STAFF_KICK_RANGE + 8.0f; // fudge factor of 8
  int kickDamage = Q_irand(
      10, 15); // Q_irand( 3, 8 ); //since it can only hit a guy once now
  ...
  // HMM... or maybe trace from origin to footRBolt/footLBolt?  Which one?  G2
  // trace?  Will do hitLoc, if so...
  if (ent->client->ps.torsoAnim == BOTH_A7_HILT) {
    ...
    case BOTH_A7_KICK_S:
      ...
      if (ri->footRBolt != -1) { // actually trace to a bolt
        if (elapsedTime >= 550 && elapsedTime <= 1050) {
          ...
          kickDir[2] = 0; // ah, flatten it, I guess...
          VectorNormalize(kickDir);
          // NOTE: have to fudge this a little because it's not getting enough
          // range with the anim as-is
    ...
    case BOTH_A7_KICK_BF:
      ...
      if (elapsedTime < 1500) { // auto-aim!
        //			overridAngles = PM_AdjustAnglesForBFKick( ent, ucmd,
        //fwdAngs, qboolean(elapsedTime<850) )?qtrue:overridAngles;
        // FIXME: if we haven't done the back kick yet and there's no-one there
        // to 			kick anymore, go into some anim that returns us to our base stance
      }
      if (ri->footRBolt != -1) { // actually trace to a bolt
        if ((elapsedTime >= 750 && elapsedTime < 850) ||
            (elapsedTime >= 1400 &&
             elapsedTime < 1500)) { // right, though either would do
          doKick = qtrue;
          G_GetBoltPosition(ent, ri->footRBolt, kickEnd, 0);
          VectorSubtract(kickEnd, ent->r.currentOrigin, kickDir);
          kickDir[2] = 0; // ah, flatten it, I guess...
    ...
    case BOTH_A7_KICK_RL:
      ...
      // ok, I'm tracing constantly on these things, they NEVER hit otherwise
      // (in MP at least)

      // FIXME: auto aim at enemies on the side of us?
      // overridAngles = PM_AdjustAnglesForRLKick( ent, ucmd, fwdAngs,
      // qboolean(elapsedTime<850) )?qtrue:overridAngles; if ( elapsedTime >= 250
      // && elapsedTime < 350 )
...
static void G_GrabSomeMofos(gentity_t *self) {
  ...
  trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->handRBolt, &boltMatrix,
                            flatAng, self->client->ps.origin, level.time, NULL,
                            self->modelScale);
  ...
  trap->Trace(&trace, self->client->ps.origin, grabMins, grabMaxs, pos,
              self->s.number, MASK_SHOT, qfalse,
              G2TRFLAG_DOGHOULTRACE | G2TRFLAG_GETSURFINDEX | G2TRFLAG_THICK |
                  G2TRFLAG_HITCORPSES,
              g_g2TraceLod.integer);
  ...
  if (trace.fraction != 1.0f && trace.entityNum < ENTITYNUM_WORLD) {
    ...
    if (grabbed->inuse &&
        ...
        grabbed->client && grabbed->health > 0 && G_CanBeEnemy(self, grabbed) &&
        G_PrettyCloseIGuess(grabbed->client->ps.origin[2],
                            self->client->ps.origin[2], 4.0f) &&
        (!BG_InGrappleMove(grabbed->client->ps.torsoAnim) ||
         grabbed->client->ps.torsoAnim == BOTH_KYLE_GRAB) &&
        (!BG_InGrappleMove(grabbed->client->ps.legsAnim) ||
         grabbed->client->ps.legsAnim ==
             BOTH_KYLE_GRAB)) { // grabbed an active player/npc
      int tortureAnim = -1;
      ...
      if (tortureAnim == -1 || correspondingAnim == -1) {
        if (self->client->ps.torsoTimer < 300 &&
            !self->client->grappleState) { // you failed to grab anyone, play
                                           // the "failed to grab" anim
          G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KYLE_MISS,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
          if (self->client->ps.torsoAnim ==
              BOTH_KYLE_MISS) { // providing the anim set succeeded..
      ...
      G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, tortureAnim,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
      if (self->client->ps.torsoAnim ==
          tortureAnim) { // providing the anim set succeeded..
        self->client->ps.weaponTime = self->client->ps.torsoTimer;
      }

      G_SetAnim(grabbed, &grabbed->client->pers.cmd, SETANIM_BOTH,
                correspondingAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD,
                0);
      if (grabbed->client->ps.torsoAnim ==
          correspondingAnim) { // providing the anim set succeeded..
        ...
        if (grabbed->client->ps.torsoTimer <
            self->client->ps.torsoTimer) { // make sure they stay in the anim at
                                           // least as long as the grabber
  ...
  if (self->client->ps.torsoTimer < 300 &&
      !self->client->grappleState) { // you failed to grab anyone, play the
                                     // "failed to grab" anim
    G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KYLE_MISS,
              SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
    if (self->client->ps.torsoAnim ==
...
void WP_SaberPositionUpdate(
    gentity_t *self,
    usercmd_t *ucmd) { // rww - keep the saber position as updated as possible
                       // on the server so that we can try to do
                       // realistic-looking contact stuff Note that this function
                       // also does the majority of working in maintaining the
                       // server g2 client instance (updating angles/anims/etc)
  ...
  if (self && self->inuse && self->client) {
    if (self->client->saberCycleQueue) {
      ...
      self->client->ps.fd.saberDrawAnimLevel =
  ...
  if (self && self->inuse && self->client && self->client->saberCycleQueue &&
      (self->client->ps.weaponTime <= 0 ||
       self->health <
           1)) { // we cycled attack levels while we were busy, so update now
                 // that we aren't (even if that means we're dead)
    self->client->ps.fd.saberAnimLevel = self->client->saberCycleQueue;
    self->client->saberCycleQueue = 0;
  }

  if (!self || !self->inuse || !self->client || !self->ghoul2 ||
      !g2SaberInstance) {
    return;
  }

  if (BG_KickingAnim(
          self->client->ps.legsAnim)) { // do some kick traces and stuff if
                                        // we're in the appropriate anim
    G_KickSomeMofos(self);
  } else if (self->client->ps.torsoAnim ==
             BOTH_KYLE_GRAB) { // try to grab someone
    G_GrabSomeMofos(self);
  } else if (self->client->grappleState) {
    gentity_t *grappler = &g_entities[self->client->grappleIndex];

    if (!grappler->inuse || !grappler->client ||
        grappler->client->grappleIndex != self->s.number ||
        !BG_InGrappleMove(grappler->client->ps.torsoAnim) ||
        !BG_InGrappleMove(grappler->client->ps.legsAnim) ||
        !BG_InGrappleMove(self->client->ps.torsoAnim) ||
        ...
        !G_PrettyCloseIGuess(self->client->ps.origin[2],
                             grappler->client->ps.origin[2], 4.0f)) {
      self->client->grappleState = 0;
      if ((BG_InGrappleMove(self->client->ps.torsoAnim) &&
           self->client->ps.torsoTimer > 100) ||
          (BG_InGrappleMove(self->client->ps.legsAnim) &&
           self->client->ps.legsTimer >
               100)) { // if they're pretty far from finishing the anim then
                       // shove them into another anim
        G_SetAnim(self, &self->client->pers.cmd, SETANIM_BOTH, BOTH_KYLE_MISS,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);
        if (self->client->ps.torsoAnim ==
            BOTH_KYLE_MISS) { // providing the anim set succeeded..
      ...
      VectorSubtract(grappler->client->ps.origin, self->client->ps.origin,
                     grapAng);
      ...
      if (VectorLength(grapAng) > 64.0f) { // too far away, break it off
        ...
        if (self->client->grappleState >= 20) { // grapplee
          ...
          trap->Trace(&trace, self->client->ps.origin, self->r.mins,
                      self->r.maxs, idealSpot, self->s.number, self->clipmask,
                      qfalse, 0, 0);
          if (!trace.startsolid && !trace.allsolid &&
              trace.fraction == 1.0f) { // go there
            G_SetOrigin(self, idealSpot);
            VectorCopy(idealSpot, self->client->ps.origin);
          }
        } else if (self->client->grappleState >= 1) { // grappler
          if (grappler->client->ps.weapon ==
              WP_SABER) { // make sure their saber is shut off
            if (!grappler->client->ps.saberHolstered) {
              grappler->client->ps.saberHolstered = 2;
              if (grappler->client->saber[0].soundOff) {
                G_Sound(grappler, CHAN_AUTO,
          ...
          if (self->client->ps.torsoAnim == BOTH_KYLE_PA_1) { // grab punch
            if (self->client->grappleState == 1) {            // smack
              if (self->client->ps.torsoTimer < 3400) {
                ...
                G_Damage(grappler, self, self, NULL, self->client->ps.origin,
                         10, 0, MOD_MELEE);
                // G_Sound( grappler, CHAN_AUTO, G_SoundIndex( va(
                // "sound/weapons/melee/punch%d", Q_irand( 1, 4 ) ) ) );

                // it might try to put them into a pain anim or something, so
                // override it back again
              ...
              if (self->client->ps.torsoTimer < 1300) {
                ...
                VectorSubtract(grappler->client->ps.origin,
                               self->client->ps.origin, tossDir);
                VectorNormalize(tossDir);
                VectorScale(tossDir, 500.0f, tossDir);
                tossDir[2] = 200.0f;

                VectorAdd(grappler->client->ps.velocity, tossDir,
                          grappler->client->ps.velocity);
            ...
            if (self->client->grappleState == 1) { // knee to the face
              if (self->client->ps.torsoTimer < 3200) {
                ...
                self->client->grappleState++;
                       ...
                       2) { // smashed on the ground
              if (self->client->ps.torsoTimer < 2000) {
                // G_Damage(grappler, self, self, NULL, self->client->ps.origin,
                // 10, 0, MOD_MELEE); don't do damage on this one, it would look
                // very freaky if they died
                G_EntitySound(grappler, CHAN_VOICE,
              ...
              if (self->client->ps.torsoTimer < 1000) {
                ...
                if (grappler->health > 0) {
                  ...
                  // G_SetAnim(grappler, &grappler->client->pers.cmd,
                  // SETANIM_BOTH, BOTH_GETUP3,
                  // SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD, 0);
  ...
  if (self->health <
      1) { // we don't want to waste precious CPU time calculating saber
           // positions for corpses. But we want to avoid the saber ent position
           // lagging on spawn, so..
    // I guess it's good to keep the position updated even when contents are 0
    if (mySaber &&
        ((mySaber->r.contents & CONTENTS_LIGHTSABER) ||
         mySaber->r.contents == 0) &&
        !self->client->ps
             .saberInFlight) { // Since we haven't got a bolt position, place it
                               // on top of the player origin.
      VectorCopy(self->client->ps.origin, mySaber->r.currentOrigin);
    }

    // I don't want to return now actually, I want to keep g2 instances for
    // corpses up to date because I'm doing better corpse hit detection/dismem
    // (particularly for the npc's) return;
  }
  ...
  if (self->client->ps.weapon != WP_SABER ||
      ...
      self->client->ps.weaponstate == WEAPON_DROPPING || self->health < 1) {
    if (!self->client->ps.saberInFlight) {
      returnAfterUpdate = 1;
    }
  }
  ...
  if (self->client->ps.saberThrowDelay < level.time) {
    if ((self->client->saber[0].saberFlags &
         SFL_NOT_THROWABLE)) { // cant throw it normally!
      if ((self->client->saber[0].saberFlags &
           SFL_SINGLE_BLADE_THROWABLE)) { // but can throw it if only have 1
                                          // blade on
        if (self->client->saber[0].numBlades > 1 &&
            self->client->ps.saberHolstered ==
                1) { // have multiple blades and only one blade on
...
nextStep:
  ...
  // try to predict the origin based on velocity so it's more like what the
  // client is seeing
  ...
  // Cap it off at reasonable values so the saber box doesn't go flying ahead of
  // us or something if we get a big speed boost from something.
  if (fVSpeed > 70) {
    fVSpeed = 70;
  }
  if (fVSpeed < -70) {
  ...
  if (!clientOverride) { // if we get the client instance we don't need to do
                         // this
  ...
  if (returnAfterUpdate &&
      saberNum) { // We don't even need to do GetBoltMatrix if we're only in
                  // here to keep the g2 server instance in sync
  ...
  // the constant ("_Always") values. Later we will handle going through each
  // blade.
  trap->G2API_GetBoltMatrix(self->ghoul2, 1, 0, &boltMatrix, properAngles,
  ...
  if (self->client->lastSaberStorageTime &&
      (level.time - self->client->lastSaberStorageTime) < 200) { // alright
    VectorCopy(self->client->lastSaberBase_Always,
               self->client->olderSaberBase);
  ...
  VectorMA(boltOrigin, self->client->saber[0].blade[0].lengthMax, boltAngles,
           end);
  ...
  if (self->client->ps.saberEntityNum) {
    ...
    if (mySaber &&
        ((mySaber->r.contents & CONTENTS_LIGHTSABER) ||
         mySaber->r.contents == 0) &&
        !self->client->ps
             .saberInFlight) { // place it roughly in the middle of the saber..
      VectorMA(boltOrigin, self->client->saber[0].blade[0].lengthMax,
               boltAngles, mySaber->r.currentOrigin);
    }
  }
  ...
  if (self->client->ps.saberInFlight) { // do the thrown-saber stuff
    ...
    if (saberent) {
      if (!self->client->ps.saberEntityState &&
        ...
        // startang[0] = 90;
        // Instead of this we'll sort of fake it and slowly tilt it down on the
        // client via a perframe method (which doesn't actually affect where or
        // how the saber hits)
        ...
        WP_SaberAddG2Model(saberent, self->client->saber[0].model,
                           self->client->saber[0].skin);
        ...
        trap->LinkEntity((sharedEntity_t *)saberent);
      } else if (self->client->ps.saberEntityNum) // only do this stuff if your
                                                  // saber is active and has not
                                                  // been knocked out of the air.
      {
        VectorCopy(boltOrigin, saberent->pos1);
        trap->LinkEntity((sharedEntity_t *)saberent);

        if (saberent->genericValue5 ==
            PROPER_THROWN_VALUE) { // return to the owner now, this is a bad
                                   // state to be in for here..
          saberent->genericValue5 = 0;
          saberent->think = SaberUpdateSelf;
          saberent->nextthink = level.time;
          WP_SaberRemoveG2Model(saberent);
  ...
  */
  ...
  if (!BG_SabersOff(&self->client->ps)) {
      ...
      while (rSaberNum < MAX_SABERS) {
        ...
        while (rBladeNum <
               self->client->saber[rSaberNum]
                   .numBlades) { // Don't bother updating the bolt for each
                                 // blade for this, it's just a very rough
                                 // fallback method for during saberlocks
          VectorCopy(
              boltOrigin,
    ...
    if (self->client->ps
            .saberInFlight) { // if saber is thrown then only do the standard
                              // stuff for the left hand saber
      if (!self->client->ps
               .saberEntityNum) { // however, if saber is not in flight but
                                  // rather knocked away, our left saber is off,
    ...
    // Now cycle through each saber and each blade on the saber and do damage
    // traces.
    while (rSaberNum < MAX_SABERS) {
      ...
      /*
      if (rSaberNum == 0 && (self->client->ps.brokenLimbs & (1 <<
      BROKENLIMB_RARM))) { //don't do saber 0 is the right arm is broken
              rSaberNum++;
              continue;
      }
      */
      // for now I'm keeping a broken right arm swingable, it will just look and
      // act damaged but still be useable

      if (rSaberNum == 1 && (self->client->ps.brokenLimbs &
                             (1 << BROKENLIMB_LARM))) { // don't to saber 1 if
                                                        // the left arm is broken
        break;
      }
      if (rSaberNum > 0 && self->client->saber[1].model[0] &&
          self->client->ps.saberHolstered ==
              1) { // don't to saber 2 if it's off
        break;
      }
      rBladeNum = 0;
      while (rBladeNum < self->client->saber[rSaberNum].numBlades) {
        ...
        VectorCopy(
            self->client->saber[rSaberNum].blade[rBladeNum].muzzlePoint,
            self->client->saber[rSaberNum].blade[rBladeNum].muzzlePointOld);
        VectorCopy(
            self->client->saber[rSaberNum].blade[rBladeNum].muzzleDir,
            self->client->saber[rSaberNum].blade[rBladeNum].muzzleDirOld);

        if (rBladeNum > 0                         // more than one blade
            && (!self->client->saber[1].model[0]) // not using dual blades
            && self->client->saber[rSaberNum].numBlades >
                   1 // using a multi-bladed saber
            && self->client->ps.saberHolstered == 1) //
        { // don't to extra blades if they're off
          break;
        }
        // get the new data
        // then update the bolt pos/dir. rBladeNum corresponds to the bolt index
        // because blade bolts are added in order.
        if (rSaberNum == 0 && self->client->ps.saberInFlight) {
          if (!self->client->ps.saberEntityNum) { // dropped it... shouldn't get
                                                  // here, but...
            // assert(0);
            ...
            if (saberent->s.saberInFlight) { // spinning
              BG_EvaluateTrajectory(&saberEnt->s.pos, level.time + 50,
                                    saberOrg);
              BG_EvaluateTrajectory(&saberEnt->s.apos, level.time + 50,
                                    saberAngles);
            ...
            trap->G2API_GetBoltMatrix(saberEnt->ghoul2, 0, rBladeNum,
                                      &boltMatrix, saberAngles, saberOrg,
                                      level.time, NULL, self->modelScale);
            BG_GiveMeVectorFromMatrix(
                &boltMatrix, ORIGIN,
                self->client->saber[rSaberNum].blade[rBladeNum].muzzlePoint);
            BG_GiveMeVectorFromMatrix(
                &boltMatrix, NEGATIVE_Y,
                self->client->saber[rSaberNum].blade[rBladeNum].muzzleDir);
            VectorCopy(
                self->client->saber[rSaberNum].blade[rBladeNum].muzzlePoint,
                boltOrigin);
            VectorMA(boltOrigin,
                     self->client->saber[rSaberNum].blade[rBladeNum].lengthMax,
                     self->client->saber[rSaberNum].blade[rBladeNum].muzzleDir,
                     end);
          }

        } else {
          trap->G2API_GetBoltMatrix(self->ghoul2, rSaberNum + 1, rBladeNum,
                                    &boltMatrix, properAngles, properOrigin,
                                    level.time, NULL, self->modelScale);
          BG_GiveMeVectorFromMatrix(
              &boltMatrix, ORIGIN,
              self->client->saber[rSaberNum].blade[rBladeNum].muzzlePoint);
        ...
        self->client->saber[rSaberNum].blade[rBladeNum].storageTime =
            level.time;

        if (self->client->hasCurrentPosition && d_saberInterpolate.integer) {
          if (self->client->ps.weaponTime <=
              0) { // rww - 07/17/02 - don't bother doing the extra stuff unless
                   // actually attacking. This is in attempt to save CPU.
            CheckSaberDamage(
                self, rSaberNum, rBladeNum, boltOrigin, end, qfalse,
                (MASK_PLAYERSOLID | CONTENTS_LIGHTSABER | MASK_SHOT), qfalse);
            ...
            if (!g_saberTraceSaberFirst.integer) {
              skipSaberTrace = qtrue;
                       ...
                       !self->client->ps
                            .duelInProgress) { // if value is >= 2, and not in a
                                               // duel, skip
              skipSaberTrace = qtrue;
            }

            if (skipSaberTrace) { // skip the saber-contents-only trace and get
                                  // right to the full trace
              trMask = (MASK_PLAYERSOLID | CONTENTS_LIGHTSABER | MASK_SHOT);
            } else {
              while (sN < MAX_CLIENTS) {
                if (g_entities[sN].inuse && g_entities[sN].client &&
                    g_entities[sN].r.linked && g_entities[sN].health > 0 &&
                    (g_entities[sN].r.contents &
                     CONTENTS_BODY)) { // Take this mask off before the saber
                                       // trace, because we want to hit the saber
                                       // first
            ...
            while (!gotHit) {
              if (!CheckSaberDamage(self, rSaberNum, rBladeNum, boltOrigin, end,
                                    qfalse, trMask, qfalse)) {
                if (!CheckSaberDamage(self, rSaberNum, rBladeNum, boltOrigin,
                                      end, qtrue, trMask, qfalse)) {
                  ...
                  if ((level.time - self->client->saber[rSaberNum]
                                        .blade[rBladeNum]
                                        .trail.lastTime) >
                      100) { // no valid last pos, use current
                    ...
                    VectorCopy(self->client->saber[rSaberNum]
                                   .blade[rBladeNum]
                                   .trail.base,
                               oldSaberStart);
                    VectorCopy(self->client->saber[rSaberNum]
                  ...
                  if ((deltaX != 0 || deltaY != 0 || deltaZ != 0) &&
                      deltaX < 180 && deltaY < 180 && deltaZ < 180 &&
                      (BG_SaberInAttack(self->client->ps.saberMove) ||
                       PM_SaberInTransition(
                           self->client->ps
                               .saberMove))) { // don't go beyond here if we
                                               // aren't attacking/transitioning
                                               // or the angle is too large.
                    ...
                    // Now that I have the angle, I'll just say the base for it
                    // is the difference between the two start points (even
                    // though that's quite possibly completely false)
                    ...
                    saberMidEnd[0] =
                        saberMidPoint[0] +
                        saberMidDir[0] * self->client->saber[rSaberNum]
                                             .blade[rBladeNum]
                                             .lengthMax;
                    saberMidEnd[1] =
                        saberMidPoint[1] +
                        saberMidDir[1] * self->client->saber[rSaberNum]
                                             .blade[rBladeNum]
                                             .lengthMax;
                    saberMidEnd[2] =
                        saberMidPoint[2] +
                        saberMidDir[2] * self->client->saber[rSaberNum]
                                             .blade[rBladeNum]
                                             .lengthMax;

                    // I'll just trace straight out and not even trace between
                    // positions to save speed.
                    if (CheckSaberDamage(self, rSaberNum, rBladeNum,
                                         saberMidPoint, saberMidEnd, qfalse,
                                         trMask, qfalse)) {
                      gotHit = qtrue;
                    }
                  }
          ...
          } else if (d_saberInterpolate.integer) // anything but 0 or 1, use the
                                                 // old plain method.
          {
            if (!CheckSaberDamage(
                    self, rSaberNum, rBladeNum, boltOrigin, end, qfalse,
                    ...
                    qfalse)) {
              CheckSaberDamage(
                  self, rSaberNum, rBladeNum, boltOrigin, end, qtrue,
                  (MASK_PLAYERSOLID | CONTENTS_LIGHTSABER | MASK_SHOT), qfalse);
            }
          }
        } else if (d_saberSPStyleDamage.integer) {
          G_SPSaberDamageTraceLerped(
              self, rSaberNum, rBladeNum, boltOrigin, end,
              (MASK_PLAYERSOLID | CONTENTS_LIGHTSABER | MASK_SHOT));
        } else {
          CheckSaberDamage(self, rSaberNum, rBladeNum, boltOrigin, end, qfalse,
                           (MASK_PLAYERSOLID | CONTENTS_LIGHTSABER | MASK_SHOT),
                           qfalse);
        }
    ...
    // NOTE: doing one call like this after the 2 loops above is a bit cheaper,
    // tempentity-wise... but won't use the correct saber and blade numbers...
...
finalUpdate:
  if (clientOverride) { // if we get the client instance we don't even need to
                        // bother setting anims and stuff
    return;
  }
...
void WP_SaberBlockNonRandom(gentity_t *self, vec3_t hitloc,
  ...
  fwdangles[1] = self->client->ps.viewangles[1];
  // Ultimately we might care if the shot was ahead or behind, but for now, just
  // quadrant is fine.
  ...
  {
    if (zdiff < -10) // 30 )
    { // hmm, pretty low, but not low enough to use the low block, so we need to
      // duck
    }
    if (rightdot > 0.1) {
      self->client->ps.saberBlocked = BLOCKED_UPPER_RIGHT;
...
void WP_SaberBlock(gentity_t *playerent, vec3_t hitloc, qboolean missileBlock) {
  ...
  } else { // The lower half is a bit iffy as far as block coverage.  Pick one
           // of the "low" ones at random.
    if (Q_irand(0, 1)) {
      playerent->client->ps.saberBlocked = BLOCKED_LOWER_RIGHT;
    } else {
      playerent->client->ps.saberBlocked = BLOCKED_LOWER_LEFT;
...
int WP_SaberCanBlock(gentity_t *self, vec3_t point, int dflags, int mod,
  ...
  if (PM_InSaberAnim(self->client->ps.torsoAnim) &&
    ...
    if (self->client->ps.saberMove < LS_PARRY_UP ||
        self->client->ps.saberMove > LS_REFLECT_LL) {
      return 0;
    }
  }
  ...
  // Removed this for now, the new broken parry stuff should handle it. This is
  // how blocks were decided before the 1.03 patch (as you can see, it was
  // STUPID.. for the most part)
  /*
  if (attackStr == FORCE_LEVEL_3)
  {
          if (self->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] >=
  FORCE_LEVEL_3)
          {
                  if (Q_irand(1, 10) < 3)
                  {
  ...
  FORCE_LEVEL_3)
          ...
          else if (self->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] >=
  FORCE_LEVEL_2)
          {
                  if (Q_irand(1, 10) < 5)
                  {
  ...
  if (attackStr == FORCE_LEVEL_1 &&
  !self->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] && Q_irand(1, 40) < 3)
  { //if I have no defense level at all then I might be unable to block a level
  1 attack (but very rarely) return 0;
  }
  */
  ...
  if (!InFront(point, self->client->ps.origin, self->client->ps.viewangles,
               blockFactor)) // orig 0.2f
  {
    return 0;
  }
