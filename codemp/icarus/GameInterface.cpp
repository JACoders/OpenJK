...
int ICARUS_RunScript(sharedEntity_t *ent, const char *name) {
  ...
  if
    S_FAILED(gSequencers[ent->s.number]->Run(buf, len))
  return false;
...
void ICARUS_FreeEnt(sharedEntity_t *ent) {
  ...
  // Remove them from the ICARUSE_EntList list so that when their g_entity index
  // is reused, ICARUS doesn't try to affect the new (incorrect) ent.
  if
    VALIDSTRING(ent->script_targetname) {
      char temp[1024];
...
bool ICARUS_ValidEnt(sharedEntity_t *ent) {
  ...
  for (i = 0; i < NUM_BSETS; i++) {
    if
      VALIDSTRING(ent->behaviorSet[i]) {
        // Com_Printf( "WARNING: Entity %d (%s) has behaviorSet but no
        // script_targetname -- using targetname\n", ent->s.number,
        // ent->targetname );

        // ent->script_targetname = ent->targetname;
        // rww - You CANNOT do things like this now. We're switching memory
        // around to be able to read this memory from vm land, and while this
        // allows us to read it on our "fake" entity here, we can't modify
        // pointers like this. We can however do something completely hackish
        // such as the following.
        assert(ent->s.number >= 0 && ent->s.number < MAX_GENTITIES);
        sharedEntity_t *trueEntity = SV_GentityNum(ent->s.number);
        // This works because we're modifying the actual shared game vm data and
        // turning one pointer into another. While these pointers both look like
        // garbage to us in here, they are not.
...
bool ICARUS_RegisterScript(const char *name,
                           qboolean bCalledDuringInterrogate /* = false */) {
  bufferlist_t::iterator ei;
  pscript_t *pscript;
  ...
  // note special return condition here, if doing interrogate and we already
  // have this file then we MUST return
  //	false (which stops the interrogator proceeding), this not only saves
  //some time, but stops a potential 	script recursion bug which could lock the
  //program in an infinite loop...   Return TRUE for normal though!
  //
  if (ei != ICARUS_BufferList.end())
  ...
  // small update here, if called during interrogate, don't let gi.FS_ReadFile()
  // complain because it can't
  //	find stuff like BS_RUN_AND_SHOOT as scriptname...   During FINALBUILD
  //the message won't appear anyway, hence 	the ifndef, this just cuts down on
  //internal error reports while testing release mode...
  //
  qboolean qbIgnoreFileRead = qfalse;
  //
  // NOTENOTE: For the moment I've taken this back out, to avoid doubling the
  // number of fopen()'s per file.
  ...
  if (length <= 0) {
    // File not found, but keep quiet during interrogate stage, because of stuff
    // like BS_RUN_AND_SHOOT as scriptname
    //
    if (!bCalledDuringInterrogate) {
  ...
  pscript->buffer =
      (char *)ICARUS_Malloc(length); // gi.Malloc(length, TAG_ICARUS, qfalse);
...
int ICARUS_GetIDForString(const char *string) {
  T_G_ICARUS_GETSETIDFORSTRING *sharedMem =
      (T_G_ICARUS_GETSETIDFORSTRING *)sv.mSharedMemory;
...
// at this point the filename should have had the "scripts" (Q3_SCRIPT_DIR)
// added to it (but not the IBI extension)
//
void ICARUS_InterrogateScript(const char *filename) {
  ...
  //////////////////////////////////
  //
  // ensure "scripts" (Q3_SCRIPT_DIR), which will be missing if this was called
  // recursively...
  //
  char sFilename[MAX_FILENAME_LENGTH]; // should really be MAX_QPATH (and 64
                                       // bytes instead of 1024), but this fits
                                       // the rest of the code

  if (!Q_stricmpn(filename, Q3_SCRIPT_DIR, strlen(Q3_SCRIPT_DIR))) {
    Q_strncpyz(sFilename, filename, sizeof(sFilename));
  } else {
    Q_strncpyz(sFilename, va("%s/%s", Q3_SCRIPT_DIR, filename),
               sizeof(sFilename));
  }
  //
  //////////////////////////////////
  ...
  if (ICARUS_RegisterScript(sFilename, qtrue) ==
      false) // true = bCalledDuringInterrogate
    return;
  ...
  while (stream.BlockAvailable()) {
    ...
    case ID_SOUND:
      // We can't just call over to S_RegisterSound or whatever because this is
      // on the server.
      sVal1 =
          (const char *)block.GetMemberData(1); // 0 is channel, 1 is filename
    ...
    case ID_SET:
      ...
      // NOTENOTE: This will not catch special case get() inlines! (There's not
...
stringID_table_t BSTable[] = {
    ...
    ENUM2STRING(BS_ADVANCE_FIGHT), //# Advance to captureGoal and shoot enemies
                                   //if you can
    ENUM2STRING(BS_SLEEP),         //# Play awake script when startled by sound
    ENUM2STRING(BS_FOLLOW_LEADER), //# Follow your leader and shoot any enemies
                                   //you come across
    ENUM2STRING(BS_JUMP),          //# Face navgoal and jump to it.
    ENUM2STRING(BS_SEARCH), //# Using current waypoint as a base), search the
                            //immediate branches of waypoints for enemies
    ...
    ENUM2STRING(BS_CINEMATIC), //# Does nothing but face it's angles and move to
                               //a goal if it has one
...
void ICARUS_PrecacheEnt(sharedEntity_t *ent) {
  ...
  for (i = 0; i < NUM_BSETS; i++) {
    ...
    if (GetIDForString(BSTable, ent->behaviorSet[i]) ==
        -1) { // not a behavior set
      Com_sprintf(newname, sizeof(newname), "%s/%s", Q3_SCRIPT_DIR,
                  ent->behaviorSet[i]);
...
Allocates a sequencer and task manager only if an entity is a potential script
user
==============
*/
...
int ICARUS_LinkEntity(int entID, CSequencer *sequencer,
...
void Svcmd_ICARUS_f(void) {
  ...
  if ( Q_stricmp( cmd, "log" ) == 0 )
  {
          ...
          {
                  sharedEntity_t	*ent = G_Find( NULL, FOFS(
  script_targetname ), gi.argv(2) );
                  ...
                  ICARUS_entFilter		= ( ent->s.number ==
  ICARUS_entFilter ) ? -1 : ent->s.number;
