...
void LoadJPG(const char *filename, unsigned char **pic, int *width, int *height) {
	...
	if (!cinfo.output_width || !cinfo.output_height || ((pixelcount * 4) / cinfo.output_width) / 4 != cinfo.output_height || pixelcount > 0x1FFFFFFF ||
		...
		ri.Printf(PRINT_ALL, "LoadJPG: %s has an invalid image format: %dx%d*4=%d, components: %d", filename, cinfo.output_width, cinfo.output_height,
...
void LoadJPGFromBuffer(byte *inputBuffer, size_t len, unsigned char **pic, int *width, int *height) {
	...
	if (!cinfo.output_width || !cinfo.output_height || ((pixelcount * 4) / cinfo.output_width) / 4 != cinfo.output_height || pixelcount > 0x1FFFFFFF ||
		...
		ri.Printf(PRINT_ALL, "LoadJPG: invalid image format: %dx%d*4=%d, components: %d", cinfo.output_width, cinfo.output_height, pixelcount * 4,
