...
//
char *C_MP3Stream_DecodeInit(LP_MP3STREAM pSFX_MP3Stream, void *pvSourceData, int iSourceBytesRemaining, int iGameAudioSampleRate, int iGameAudioSampleBits,
							 int bStereoDesired) {
	char *psReturn = NULL;
	...
	pMP3Stream->iSourceFrameBytes =
		head_info3((byte *)pvSourceData, iSourceBytesRemaining / 2, &head, &iBitRate, (unsigned int *)&pMP3Stream->iSourceReadIndex);
	...
	if (pMP3Stream->iSourceFrameBytes) {
		if (audio.decode_init(&head, pMP3Stream->iSourceFrameBytes, reduction_code, pMP3Stream->iSourceReadIndex,
			...
			if (audio.decode_init(&head, pMP3Stream->iSourceFrameBytes, pMP3Stream->iRewind_FinalReductionCode, pMP3Stream->iSourceReadIndex,
...
char *C_MP3Stream_Rewind(LP_MP3STREAM pSFX_MP3Stream) {
	...
	//
	if (pMP3Stream->iSourceFrameBytes ==
		head_info3(pMP3Stream->pbSourceData, pMP3Stream->iSourceBytesRemaining / 2, &head, &iBitRate, (unsigned int *)&iNULL)) {
		if (audio.decode_init(&head, pMP3Stream->iSourceFrameBytes, pMP3Stream->iRewind_FinalReductionCode, pMP3Stream->iSourceReadIndex,
							  pMP3Stream->iRewind_FinalConvertCode, freq_limit)) {
