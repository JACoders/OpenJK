	...
	if (!self->client || self->client->ps.lastOnGround + 300 < level.time || (self->client->ps.lastOnGround + 100 < level.time && easyBreakBrush)) {
		...
		if (damageSelf && self->takedamage) {
			...
			if (self->client && self->client->ps.fd.forceJumpZStart) {				   // we were force-jumping
				if (self->r.currentOrigin[2] >= self->client->ps.fd.forceJumpZStart) { // we landed at same height or higher than we landed
					magnitude = 0;
...
qboolean G_StandingAnim(int anim) { // NOTE: does not check idles or special (cinematic) stands
	switch (anim) {
	case BOTH_STAND1:
	case BOTH_STAND2:
	case BOTH_STAND3:
	...
	if (ent->client->ps.torsoTimer < 1 && ent->client->ps.forceHandExtend == HANDEXTEND_NONE && ent->client->ps.legsTimer < 1 &&
		...
		case TAUNT_TAUNT:
			if (ent->client->ps.weapon != WP_SABER) {
				...
				case SS_FAST:
				case SS_TAVION:
					if (ent->client->ps.saberHolstered == 1 && ent->client->saber[1].model[0]) { // turn off second saber
				...
				case SS_DESANN:
					...
					break;
				case SS_DUAL:
					if (ent->client->ps.saberHolstered == 1 && ent->client->saber[1].model[0]) { // turn on second saber
		...
		if (anim != -1) {
			...
			if (taunt != TAUNT_MEDITATE && taunt != TAUNT_BOW) { // no sound for meditate or bow
...
#define MIN_NPC_SPEED 16.0f
	...
	if (ent->client->ps.duelInProgress) {
		...
		if (ent->client->ps.duelTime < level.time) {
			...
			if (ent->client->ps.weapon == WP_SABER && ent->client->ps.saberHolstered && ent->client->ps.duelTime) {
				ent->client->ps.saberHolstered = 0;
			...
			if (duelAgainst && duelAgainst->client && duelAgainst->inuse && duelAgainst->client->ps.weapon == WP_SABER &&
	...
	if (ent->client->doingThrow > level.time) {
		...
		if (!throwee->inuse || !throwee->client || throwee->health < 1 || throwee->client->sess.sessionTeam == TEAM_SPECTATOR ||
			(throwee->client->ps.pm_flags & PMF_FOLLOW) || throwee->client->throwingIndex != ent->s.number) {
	...
	if (ent->client->beingThrown > level.time) {
		...
		if (!thrower->inuse || !thrower->client || thrower->health < 1 || thrower->client->sess.sessionTeam == TEAM_SPECTATOR ||
			(thrower->client->ps.pm_flags & PMF_FOLLOW) || thrower->client->throwingIndex != ent->s.number) {
		...
		} else if (thrower->inuse && thrower->client && thrower->ghoul2 && trap->G2API_HaveWeGhoul2Models(thrower->ghoul2)) {
#if 0
...
#endif
			{
				...
				if (VectorLength(vDif) > 32.0f &&
					(thrower->client->doingThrow - level.time) < 4500) { // the hand is too far away, and can no longer hold onto us, so escape.
					ent->client->ps.heldByClient = 0;
					ent->client->beingThrown = 0;
					thrower->client->doingThrow = 0;
				...
				} else if ((client->beingThrown - level.time) < 4000) { // step into the next part of the throw, and go flying back
					float vScale = 400.0f;
				...
				} else { // see if we can move to be next to the hand.. if it's not clear, break the throw.
					...
					} else { // if the guy can't be put here then it's time to break the throw off.
						ent->client->ps.heldByClient = 0;
						ent->client->beingThrown = 0;
						thrower->client->doingThrow = 0;
	...
	*/
	...
	if (pmove.checkDuelLoss) {
		if (pmove.checkDuelLoss > 0 &&
			(pmove.checkDuelLoss <= MAX_CLIENTS || (pmove.checkDuelLoss < (MAX_GENTITIES - 1) && g_entities[pmove.checkDuelLoss - 1].s.eType == ET_NPC))) {
	...
	if (pmove.cmd.generic_cmd && (pmove.cmd.generic_cmd != ent->client->lastGenCmd || ent->client->lastGenCmdTime < level.time)) {
		...
		if (pmove.cmd.generic_cmd != GENCMD_FORCE_THROW &&
			pmove.cmd.generic_cmd != GENCMD_FORCE_PULL) {	// these are the only two where you wouldn't care about a delay between
		...
		case GENCMD_ENGAGE_DUEL:
			if (level.gametype == GT_DUEL || level.gametype == GT_POWERDUEL) { // already in a duel, made it a taunt command
			} else {
			...
			if (faceKicked && faceKicked->client && faceKicked->health && faceKicked->takedamage) { // push them away and do pain
				...
				if (faceKicked->client->ps.weapon != WP_SABER || faceKicked->client->ps.fd.saberAnimLevel != FORCE_LEVEL_3 ||
					...
					if (faceKicked->health > 0 && faceKicked->client->ps.stats[STAT_HEALTH] > 0 &&
						...
						if (BG_KnockDownable(&faceKicked->client->ps) && Q_irand(1, 10) <= 3) { // only actually knock over sometimes, but always do velocity
																								// hit
							faceKicked->client->ps.forceHandExtend = HANDEXTEND_KNOCKDOWN;
