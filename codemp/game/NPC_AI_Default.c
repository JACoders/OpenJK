...
qboolean NPC_StandTrackAndShoot(gentity_t *NPC, qboolean canDuck) {
  ...
  if (canDuck && (duck_ok || (!attack_ok && NPCS.client->ps.weaponTime <= 0)) &&
      NPCS.ucmd.upmove !=
    ...
    if (duck_ok) { // duck and don't shoot
      ...
      NPCS.NPCInfo->duckDebounceTime =
          level.time + 1000; // duck for a full second
    }
  }
...
void NPC_BSIdle(void) {
  ...
  if ((NPCS.ucmd.forwardmove == 0) && (NPCS.ucmd.rightmove == 0) &&
      (NPCS.ucmd.upmove == 0)) {
    //		NPC_StandIdle();
  }
...
void NPC_BSStandGuard(void) {
  ...
  if (NPCS.NPC->enemy == NULL) { // Possible to pick one up by being shot
    if (Q_flrand(0.0f, 1.0f) < 0.5) {
      if (NPCS.NPC->client->enemyTeam) {
        gentity_t *newenemy = NPC_PickEnemy(
            NPCS.NPC, NPCS.NPC->client->enemyTeam,
...
void NPC_BSHuntAndKill(void) {
  ...
  NPC_CheckEnemy(NPCS.NPCInfo->tempBehavior != BS_HUNT_AND_KILL, qfalse,
                 qtrue); // don't find new enemy if this is tempbehav

  if (NPCS.NPC->enemy) {
    oEVis = NPCS.enemyVisibility = NPC_CheckVisibility(
    ...
    if (curAnim != BOTH_ATTACK1 && curAnim != BOTH_ATTACK2 &&
      ...
      if (enemyDist > 48 &&
          ((enemyDist * 1.5) * (enemyDist * 1.5) >=
           ...
           enemyDist > IdealDistance(NPCS.NPC) * 3)) { // We should close in?
        NPCS.NPCInfo->goalEntity = NPCS.NPC->enemy;

        NPC_MoveToGoal(qtrue);
      } else if (enemyDist < IdealDistance(NPCS.NPC)) { // We should back off?
...
void NPC_BSStandAndShoot(void) {
  ...
  if (NPCS.NPCInfo->duckDebounceTime > level.time &&
  ...
  if (NPCS.NPC->enemy) {
    if (!NPC_StandTrackAndShoot(NPCS.NPC,
...
void NPC_BSRunAndShoot(void) {
  ...
  if (NPCS.NPC->enemy) {
    ...
    if (!(NPCS.ucmd.buttons & BUTTON_ATTACK) && NPCS.ucmd.upmove >= 0 &&
        NPCS.NPC->cantHitEnemyCounter >
            monitor) { // not crouching and not firing
      vec3_t vec;
      ...
      if (VectorLength(vec) > 128 ||
          NPCS.NPC->cantHitEnemyCounter >= 10) { // run at enemy if too far away
        ...
        if (NPCS.NPC->cantHitEnemyCounter >=
            (NPCS.NPCInfo->stats.aggression + 1) * 10) {
...
void NPC_BSPointShoot(qboolean shoot) { // FIXME: doesn't check for clear
  ...
  if (!NPCS.NPC->enemy || !NPCS.NPC->enemy->inuse ||
      (NPCS.NPC->enemy->NPC &&
       NPCS.NPC->enemy->health <=
           0)) { // FIXME: should still keep shooting for a second or two after
                 // they actually die...
    trap->ICARUS_TaskIDComplete((sharedEntity_t *)NPCS.NPC, TID_BSTATE);
    goto finished;
  }
    ...
    */
    {
      ...
      float pitchMiss,
          pitchMissAllow =
              (NPCS.NPC->enemy->r.maxs[2] - NPCS.NPC->enemy->r.mins[2]) / 2;
      ...
      yawMiss = tan(DEG2RAD(AngleDelta(NPCS.NPC->client->ps.viewangles[YAW],
                                       NPCS.NPCInfo->desiredYaw))) *
                dist;
      pitchMiss = tan(DEG2RAD(AngleDelta(NPCS.NPC->client->ps.viewangles[PITCH],
...
void NPC_BSPatrol(void) {
  ...
  if (level.time > NPCS.NPCInfo->enemyCheckDebounceTime) {
    NPCS.NPCInfo->enemyCheckDebounceTime =
        level.time + (NPCS.NPCInfo->stats.vigilance * 1000);
    NPC_CheckEnemy(qtrue, qfalse, qtrue);
    if (NPCS.NPC->enemy) { // FIXME: do anger script
...
void NPC_BSDefault(void) {
  ...
  if (NPCS.NPCInfo->scriptFlags & SCF_FORCED_MARCH) { // being forced to walk
    if (NPCS.NPC->client->ps.torsoAnim != TORSO_SURRENDER_START) {
      NPC_SetAnim(NPCS.NPC, SETANIM_TORSO, TORSO_SURRENDER_START,
                  SETANIM_FLAG_HOLD);
    }
  }
  // look for a new enemy if don't have one and are allowed to look, validate
  // current enemy if have one
  NPC_CheckEnemy((NPCS.NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES), qfalse,
                 qtrue);
  if (!NPCS.NPC->enemy) { // still don't have an enemy
    if (!(NPCS.NPCInfo->scriptFlags &
          SCF_IGNORE_ALERTS)) { // check for alert events
      ...
      if (alertEvent >= 0 &&
          level.alertEvents[alertEvent].ID !=
              NPCS.NPCInfo->lastAlertID) { // heard/saw something
        if (level.alertEvents[alertEvent].level >= AEL_DISCOVERED &&
            (NPCS.NPCInfo->scriptFlags &
             SCF_LOOK_FOR_ENEMIES)) { // was a big event
  ...
  if (NPCS.NPC->enemy && !(NPCS.NPCInfo->scriptFlags & SCF_FORCED_MARCH)) {
    ...
    /*
       ...
       we really need to do the rest? VectorSubtract(
       NPC->enemy->r.currentOrigin, NPC->r.currentOrigin, enemyDir ); enemyDist
       = VectorNormalize( enemyDir ); enemyDist *= enemyDist; shootDist =
       NPC_MaxDistSquaredForWeapon();

                    enemyFOV = InFOV( NPC->enemy, NPC, NPCInfo->stats.hfov,
       NPCInfo->stats.vfov ); enemyShotFOV = InFOV( NPC->enemy, NPC, 20, 20 );
                    enemyPVS = trap->inPVS( NPC->enemy->r.currentOrigin,
       NPC->r.currentOrigin );
       ...
       NPC->enemy->maxs[2]*0.5f ); CalcEntitySpot( NPC, SPOT_WEAPON, muzzle );
                            ...
                            trap->trace ( &tr, muzzle, vec3_origin, vec3_origin,
  ...
  if (UpdateGoal()) { // have a goal
    if (!NPCS.NPC->enemy && NPCS.NPC->client->leader &&
        NPCS.NPCInfo->goalEntity == NPCS.NPC->client->leader &&
        !trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC, TID_MOVE_NAV)) {
      NPC_BSFollowLeader();
    } else {
      // set angles
      if ((NPCS.NPCInfo->scriptFlags & SCF_FACE_MOVE_DIR) ||
          NPCS.NPCInfo->goalEntity !=
              NPCS.NPC->enemy) { // face direction of movement, NOTE: default
      ...
      if (NPCS.NPCInfo->scriptFlags & SCF_FORCED_MARCH) { // being forced to
                                                          // walk
        // if ( g_crosshairEntNum != NPC->s.number )
        if (!NPC_SomeoneLookingAtMe(
                NPCS.NPC)) { // don't walk if player isn't aiming at me
          move = qfalse;
        }
      }
