...
pMP3Stream->framebytes      input, mpeg frame size (returned by call to
head_info) reduction_code  input, sample rate reduction code 0 = full rate 1 =
half rate 2 = quarter rate
...
DEC_INFO decinfo;
    /* global for Layer III */ // only written into by decode init funcs, then
                               // copied to stack struct higher up
...
////@@@@static int bit_skip;
static const int bat_bit_master[] = {0,  5,  7,  9,  10, 12, 15, 18, 21,
                                     24, 27, 30, 33, 36, 39, 42, 45, 48};

/*----------------*/
////@@@@static int nbat[4] = {3, 8, 12, 7};	// !!!!!!!!!!!!! not
///constant!!!!
////@@@@static int bat[4][16];	// built as constant, but built according to
///header type (sigh)
...
/*----------------*/
////@@@@FINDME - groan....  (I shoved a *2 in just in case it needed it for
///stereo. This whole thing is crap now
float sample[2304 * 2];
    /* global for use by Later 3 */     // !!!!!!!!!!!!!!!!!!!!!! // scratchpad?
...
////@@@@ FINDME - this stuff doesn't appear to be used by any of our samples
///(phew)
...
/*--------------------------------------------------------------*/
#define mac_load(n)                                                            \
  (bits -= n, bitval = bitbuf >> bits, bitbuf -= bitval << bits, bitval)
/*======================================================================*/
static void unpack_ba() {
  int i, j, k;
  static int nbit[4] = {4, 4, 3, 2};
  int nstereo;
  ...
  samp_dispatch[pMP3Stream->nsb_limit] =
      37;                /* terminate the dispatcher with skip */
...
dispatch:
  ...
  case 1: /* 2 factors 002 */
    mac_load_check(12);
    pMP3Stream->cs_factor[1][i] = pMP3Stream->cs_factor[0][i] =
        c_value[i] * sf_table[mac_load(6)];
    pMP3Stream->cs_factor[2][i] = c_value[i] * sf_table[mac_load(6)];
    goto dispatch;
  case 2: /* 1 factor 000 */
    mac_load_check(6);
    pMP3Stream->cs_factor[2][i] = pMP3Stream->cs_factor[1][i] =
        pMP3Stream->cs_factor[0][i] = c_value[i] * sf_table[mac_load(6)];
    goto dispatch;
...
/*-------------------------------------------------------------------------*/
#define UNPACK_N(n)                                                            \
  s[k] = pMP3Stream->cs_factor[i][k] * (load(n) - ((1 << (n - 1)) - 1));       \
...
#define UNPACK_N2(n)                                                           \
  ...
  s[k + 64] =                                                                  \
      pMP3Stream->cs_factor[i][k] * (mac_load(n) - ((1 << (n - 1)) - 1));      \
  s[k + 128] =                                                                 \
      pMP3Stream->cs_factor[i][k] * (mac_load(n) - ((1 << (n - 1)) - 1));      \
  goto dispatch;
#define UNPACK_N3(n)                                                           \
    ...
    dispatch:
      ...
      case 17:
        UNPACK_N(16) /* 65535 levels */
                     /* -- joint ---- */
      case 18 + 0:
        s[k + 128 + 1] = s[k + 128] = s[k + 64 + 1] = s[k + 64] = s[k + 1] =
            s[k] = 0.0F;
        k++; /* skip right chan dispatch */
        goto dispatch;
...
IN_OUT audio_decode(unsigned char *bs, signed short *pcm,
                    unsigned char *pNextByteAfterData) {
