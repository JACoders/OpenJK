...
void CG_AddViewWeapon(playerState_t *ps) {
	...
	if (cg_gun_frame.integer) {
		...
		if (cent->gent->lowerLumbarBone >= 0 && gi.G2API_GetBoneAnimIndex(&cent->gent->ghoul2[cent->gent->playerModel], cent->gent->lowerLumbarBone, cg.time,
																		  &currentFrame, &startFrame, &endFrame, &flags, &animSpeed, 0)) {
...
char *weaponDesc[13] = {
	"SABER_DESC",	  "BLASTER_PISTOL_DESC", "BLASTER_RIFLE_DESC",	   "DISRUPTOR_RIFLE_DESC", "BOWCASTER_DESC", "HEAVYREPEATER_DESC", "DEMP2_DESC",
	"FLECHETTE_DESC", "MERR_SONN_DESC",		 "THERMAL_DETONATOR_DESC", "TRIP_MINE_DESC",	   "DET_PACK_DESC",	 "STUN_BATON_DESC",
};
...
qboolean CG_WeaponSelectable(int i, int original, qboolean dpMode) {
	...
	if (original == WP_SABER && cg.weaponSelectTime + 500 > cg.time) { // when sqitch to lightsaber, have to stay there for at least half a second!
		return qfalse;
	}
	...
	if (num == WP_SABER) {									   // lightsaber
		if (!(cg.snap->ps.stats[STAT_WEAPONS] & (1 << num))) { // don't have saber, try stun baton
			...
			if (!in_camera) {					// player can't activate/deactivate saber when in a cinematic
				...
				if (cg.predicted_player_state.stats[STAT_HEALTH] > 0 &&
					(!cg_entities[0].currentState.saberInFlight || (&g_entities[cg_entities[0].gent->client->ps.saberEntityNum] != NULL &&
					...
					if (cg_entities[0].currentState.saberActive) {
						...
						if (cg_entities[0].currentState.saberInFlight) { // play it on the saber
