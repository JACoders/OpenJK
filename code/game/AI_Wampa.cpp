...
void Wampa_Move(qboolean visible) {
	if (NPCInfo->localState != LSTATE_WAITING) {
		...
		if (NPC->enemy) { // pick correct movement speed and anim
			...
			} else if (enemyDist > 200 && NPCInfo->stats.runSpeed == 300) { // slow run, upright
				// BOTH_RUN2
	...
	for (i = 0; i < numEnts; i++) {
		...
		if (DistanceSquared(radiusEnts[i]->currentOrigin, boltOrg) <= radiusSquared) {
			...
			if (backhand) {
				...
				if (radiusEnts[i]->client->NPC_class != CLASS_WAMPA && radiusEnts[i]->client->NPC_class != CLASS_RANCOR &&
					radiusEnts[i]->client->NPC_class != CLASS_ATST && !(radiusEnts[i]->flags & FL_NO_KNOCKBACK)) {
			...
			} else if (radiusEnts[i]->health <= 0 && radiusEnts[i]->client) { // killed them, chance of dismembering
			...
			} else if (!Q_irand(0, 3) && radiusEnts[i]->health > 0) { // one out of every 4 normal hits does a knockdown, too
				vec3_t pushDir;
				vec3_t angs;
				VectorCopy(NPC->client->ps.viewangles, angs);
				angs[YAW] += Q_flrand(25, 50);
...
void Wampa_Attack(float distance, qboolean doCharge) {
	...
	if (NPC->client->ps.legsAnim == BOTH_ATTACK1 && distance > (NPC->maxs[0] + MIN_DISTANCE)) { // okay to keep moving
		ucmd.buttons |= BUTTON_WALKING;
		Wampa_Move(qtrue);
	}
}
...
void Wampa_Combat(void) {
	...
	if ((advance || NPCInfo->localState == LSTATE_WAITING) && TIMER_Done(NPC, "attacking")) // waiting monsters can't attack
	{
		...
		if (!Q_irand(0, 15)) { // FIXME: only do this if we just damaged them or vice-versa?
			if (Wampa_CheckRoar(NPC)) {
				return;
			}
		}
...
void NPC_Wampa_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other, const vec3_t point, int damage, int mod, int hitLoc) {
	...
	if (other && other->inuse && other != self->enemy && !(other->flags & FL_NOTARGET)) {
		if ((!other->s.number && !Q_irand(0, 3)) || !self->enemy || self->enemy->health == 0 ||
			...
			(!Q_irand(0, 4) &&
			 DistanceSquared(other->currentOrigin, self->currentOrigin) <
				 DistanceSquared(
					 ...
					 self->currentOrigin))) { // if my enemy is dead (or attacked by player) and I'm not still holding/eating someone, turn on the attacker
			// FIXME: if can't nav to my enemy, take this guy if I can nav to him
			self->lastEnemy = other;
			G_SetEnemy(self, other);
			if (self->enemy != self->lastEnemy) { // clear this so that we only sniff the player the first time we pick them up
				self->useDebounceTime = 0;
			}
			TIMER_Set(self, "lookForNewEnemy", Q_irand(5000, 15000));
			if (hitByWampa) { // stay mad at this Wampa for 2-5 secs before looking for other enemies
...
qboolean Wampa_TryGrab(void) {
	...
	if (enemyDist <= radius && !NPC->count // don't have one in hand already
		&& NPC->enemy->client->NPC_class != CLASS_RANCOR && NPC->enemy->client->NPC_class != CLASS_GALAKMECH && NPC->enemy->client->NPC_class != CLASS_ATST &&
		NPC->enemy->client->NPC_class != CLASS_GONK && NPC->enemy->client->NPC_class != CLASS_R2D2 && NPC->enemy->client->NPC_class != CLASS_R5D2 &&
		NPC->enemy->client->NPC_class != CLASS_MARK1 && NPC->enemy->client->NPC_class != CLASS_MARK2 && NPC->enemy->client->NPC_class != CLASS_MOUSE &&
		NPC->enemy->client->NPC_class != CLASS_PROBE && NPC->enemy->client->NPC_class != CLASS_SEEKER && NPC->enemy->client->NPC_class != CLASS_REMOTE &&
		NPC->enemy->client->NPC_class != CLASS_SENTRY && NPC->enemy->client->NPC_class != CLASS_INTERROGATOR &&
...
void NPC_BSWampa_Default(void) {
	...
	if (NPC->count) {
		if (!NPC->activator || !NPC->activator->client) { // wtf?
			...
			if (NPC->client->ps.legsAnim == BOTH_HOLD_DROP) {
				if (NPC->client->ps.legsAnimTimer < PM_AnimLength(NPC->client->clientInfo.animFileIndex, (animNumber_t)NPC->client->ps.legsAnim) -
			...
			} else if (NPC->useDebounceTime >= level.time && NPC->activator) {								  // just sniffing the guy
				if (NPC->useDebounceTime <= level.time + 100 && NPC->client->ps.legsAnim != BOTH_HOLD_DROP) { // just about done, drop him
				...
				if (!NPC->useDebounceTime && NPC->activator && NPC->activator->s.number < MAX_CLIENTS) { // first time I pick the player, just sniff them
					...
					if (NPC->client->ps.legsAnim == BOTH_HOLD_ATTACK) {
						if (NPC->client->ps.legsAnimTimer) {
							if (TIMER_Done2(NPC, "grabAttackDamage", qtrue)) {
								...
								if (NPC->activator->health <= 0) { // killed them, chance of dismembering
									int hitLoc = HL_WAIST;
									if (g_dismemberment->integer < 4) {
										hitLoc = Q_irand(HL_BACK_RT, HL_HAND_LT);
									} else {
	...
	if (NPCInfo->localState == LSTATE_WAITING &&
		TIMER_Done2(NPC, "takingPain", qtrue)) { // was not doing anything because we were taking pain, but pain is done now, so clear it...
		NPCInfo->localState = LSTATE_CLEAR;
	}
	...
	if (NPC->enemy) {
		if (NPC->enemy->client																					// enemy is a client
			...
			&& (!NPC->enemy->enemy || !NPC->enemy->enemy->client ||
				NPC->enemy->enemy->client->NPC_class != CLASS_RANCOR)) // enemy's enemy is not a client or is not a rancor (which is scarier than me)
			...
			if (NPC->enemy->client && NPC->enemy->client->NPC_class == CLASS_WAMPA) { // got mad at another Wampa, look for a valid enemy
				...
				if (TIMER_Done(NPC, "lookForNewEnemy")) {
					...
					if (newEnemy && newEnemy != sav_enemy) { // picked up a new enemy!
						...
						if (NPC->enemy != NPC->lastEnemy) { // clear this so that we only sniff the player the first time we pick them up
