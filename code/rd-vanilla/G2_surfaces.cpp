...
const surfaceInfo_t *G2_FindOverrideSurface(int surfaceNum,
                                            const surfaceInfo_v &surfaceList) {
...
int G2_IsSurfaceLegal(const model_s *mod_m, const char *surfaceName,
                      uint32_t *flags) {
  assert(mod_m);
  assert(mod_m->mdxm);
 ...
 *
 * Input
 *    filename of model, surface list of model instance, name of surface, int to
 *be filled in with the index of this surface (defaults to NULL)
 *
 * Output
 *    pointer to surface if successful, false otherwise
 *
 ************************************************************************************************/
const mdxmSurface_t *G2_FindSurface(CGhoul2Info *ghlInfo, surfaceInfo_v &slist,
  ...
  for (i = slist.size() - 1; i >= 0; i--) {
    if ((slist[i].surface != 10000) && (slist[i].surface != -1)) {
      const mdxmSurface_t *surf = (mdxmSurface_t *)G2_FindSurface(
...
// set a named surface offFlags - if it doesn't find a surface with this name in
// the list then it will add one.
qboolean G2_SetSurfaceOnOff(CGhoul2Info *ghlInfo, const char *surfaceName,
                            const int offFlags) {
  int surfIndex = -1;
  ...
  const mdxmSurface_t *surf =
      G2_FindSurface(ghlInfo, ghlInfo->mSlist, surfaceName, &surfIndex);
  if (surf) {
    ...
    ghlInfo->mSlist[surfIndex].offFlags &=
        ~(G2SURFACEFLAG_OFF | G2SURFACEFLAG_NODESCENDANTS);
    ghlInfo->mSlist[surfIndex].offFlags |=
        offFlags & (G2SURFACEFLAG_OFF | G2SURFACEFLAG_NODESCENDANTS);
    return qtrue;
  } else {
    // ok, not in the list already - in that case, lets verify this surface
    // exists in the model mesh
    uint32_t flags;
    int surfaceNum =
        G2_IsSurfaceLegal(ghlInfo->currentModel, surfaceName, &flags);
    if (surfaceNum != -1) {
      ...
      if (newflags != flags) { // insert here then because it changed, no need
                               // to add an override otherwise
...
void G2_FindRecursiveSurface(const model_t *currentModel, int surfaceNum,
                             surfaceInfo_v &rootList, int *activeSurfaces) {
  assert(currentModel);
  assert(currentModel->mdxm);
  int i;
  const mdxmSurface_t *surface =
      (mdxmSurface_t *)G2_FindSurface(currentModel, surfaceNum, 0);
  const mdxmHierarchyOffsets_t *surfIndexes =
  ...
  const surfaceInfo_t *surfOverride =
      G2_FindOverrideSurface(surfaceNum, rootList);
...
qboolean G2_SetRootSurface(CGhoul2Info_v &ghoul2, const int modelIndex,
                           const char *surfaceName) {
  int surf;
  uint32_t flags;
  ...
  // first find if we already have this surface in the list
  surf =
      G2_IsSurfaceLegal(ghoul2[modelIndex].currentModel, surfaceName, &flags);
  if (surf != -1) {
...
int G2_AddSurface(CGhoul2Info *ghoul2, int surfaceNumber, int polyNumber,
  ...
  // first up, see if we have a free one already set up  - look only from the
  // end of the constant surfaces onwards
  size_t i;
...
int G2_GetParentSurface(CGhoul2Info *ghlInfo, const int index) {
  ...
  const mdxmSurface_t *surf =
      (mdxmSurface_t *)G2_FindSurface(ghlInfo->currentModel, index, 0);
  const mdxmSurfHierarchy_t *surfInfo =
      (mdxmSurfHierarchy_t *)((byte *)surfIndexes +
                              surfIndexes->offsets[surf->thisSurfaceIndex]);
...
int G2_IsSurfaceRendered(CGhoul2Info *ghlInfo, const char *surfaceName,
                         surfaceInfo_v &slist) {
  ...
  if (surfNum != -1) { // must be legal
    ...
    while (surfNum != -1) {
      ...
      parentSurfInfo = (mdxmSurfHierarchy_t *)((byte *)surfIndexes +
                                               surfIndexes->offsets[surfNum]);

      // find the original surface in the surface list
      // G2 was bug, above comment was accurate, but we don't want the original
      // flags, we want the parent flags
      G2_IsSurfaceLegal(ghlInfo->currentModel, parentSurfInfo->name,
                        &parentFlags);

      // now see if we already have overriden this surface in the slist
      parentSurf =
          G2_FindSurface(ghlInfo, slist, parentSurfInfo->name, &surfIndex);
      if (parentSurf) {
        // set descendants value
        parentFlags = slist[surfIndex].offFlags;
      }
      // now we have the parent flags, lets see if any have the 'no descendants'
      // flag set
  ...
  if (flags == 0) { // it's not being overridden by a parent
    // now see if we already have overriden this surface in the slist
    const mdxmSurface_t *surf =
        G2_FindSurface(ghlInfo, slist, surfaceName, &surfIndex);
    if (surf) {
      // set descendants value
      flags = slist[surfIndex].offFlags;
    }
    // ok, at this point in flags we have what this surface is set to, and the
    // index of the surface itself
  }
  return flags;
}
