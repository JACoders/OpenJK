...
void CG_ZoomDown_f( void )
 {
        ...
        if ( cg.zoomMode == 0 )
        {
                ...
                cg.zoomTime = cg.time;
                cgi_S_StartSound( cg.refdef.vieworg, ENTITYNUM_WORLD, CHAN_AUTO,
cgs.media.zoomStart ); return;
        }

        // Can only snap out of the zoom mode if it has already been locked
(CG_ZoomUp_f has been called) if ( cg.zoomLocked )
        {
...
qboolean CG_CalcFOVFromX(float fov_x) {
  ...
  //or not. Fortunately there's only one slime area in Trek that you can be
  //under, 	so lose it...
#if 1
  // warp if underwater
  float phase;
...
static qboolean CG_CalcViewValues(void) {
  ...
  if (cg.snap->ps.viewEntity > 0 &&
      cg.snap->ps.viewEntity < ENTITYNUM_WORLD) { // in an entity camera view
    /*
...
void CG_DrawActiveFrame(int serverTime, stereoFrame_t stereoView) {
  ...
  if (cg.snap->ps.clientNum == 0) { // pointless check, but..
