...
if "%APPVEYOR_FORCED_BUILD%"=="True" (
	...
	%CYGWIN_ROOT%\bin\bash -lc 'rsync -avz --progress -e "ssh -p 29022 -i %ROOT%/users/appveyor/.ssh/id_rsa" --protocol=29 %ROOT%/projects/openjk/openjk-windows*.zip "%DEPLOY_LOCATION%"'
)
