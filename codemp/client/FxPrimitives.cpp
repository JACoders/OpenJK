...
CEffect::CEffect(void)
    : mFlags(0), mMatImpactFX(MATIMPACTFX_NONE), mMatImpactParm(-1),
      mSoundRadius(-1), mSoundVolume(-1) {
...
void CParticle::Die(void) {
  if (mFlags & FX_DEATH_RUNS_FX && !(mFlags & FX_KILL_ON_IMPACT)) {
    ...
    VectorSet(norm, flrand(-1.0f, 1.0f), flrand(-1.0f, 1.0f),
              flrand(-1.0f, 1.0f));
...
bool CParticle::Cull(void) {
  ...
  if ((DotProduct(theFxHelper.refdef->viewaxis[0], dir)) <
      0) // cg.cosHalfFOV * (len - mRadius) )
  {
    return true;
  }
...
void CParticle::Draw(void) {
  ...
  if (mFlags & FX_PLAYER_VIEW) {
    ...
    // add this 2D effect to the proper list. it will get drawn after the
    // trap->RenderScene call
    theFxScheduler.Add2DEffect(mOrigin1[0], mOrigin1[1], mRefEnt.radius,
                               mRefEnt.radius, color, mRefEnt.customShader);
  } else {
...
bool CParticle::Update(void) {
  // Game pausing can cause dumb time things to happen, so kill the effect in
  // this instance
  ...
  if (mFlags & FX_RELATIVE) {
    if (!re->G2API_IsGhoul2InfovValid(
            *mGhoul2)) { // the thing we are bolted to is no longer valid, so we
    ...
    if (!theFxHelper.GetOriginAxisFromBolt(mGhoul2, mEntNum, mModelNum,
                                           mBoltNum, org,
                                           ax)) { // could not get bolt
      return false;
    }
    ...
    // Get our real velocity at the current time, taking into account the
    // effects of acceleartion.  NOTE: not sure if this is even 100% correct
    // math-wise
...
bool CParticle::UpdateOrigin(void) {
  ...
  new_origin[0] =
      mOrigin1[0] + (theFxHelper.mRealTime *
                     mVel[0]); // + (theFxHelper.mHalfRealTimeSq * mVel[0]);
  new_origin[1] =
      mOrigin1[1] + (theFxHelper.mRealTime *
                     mVel[1]); // + (theFxHelper.mHalfRealTimeSq * mVel[1]);
  new_origin[2] =
      mOrigin1[2] + (theFxHelper.mRealTime *
                     mVel[2]); // + (theFxHelper.mHalfRealTimeSq * mVel[2]);
    ...
    if (mFlags & FX_EXPENSIVE_PHYSICS) {
      ...
      if (mFlags & FX_USE_BBOX) {
        if (mFlags & FX_GHOUL2_TRACE) {
          theFxHelper.G2Trace(trace, mOrigin1, mMin, mMax, new_origin, -1,
                              MASK_SOLID);
        } else {
          theFxHelper.Trace(trace, mOrigin1, mMin, mMax, new_origin, -1,
                            MASK_SOLID);
        }
      } else {
        if (mFlags & FX_GHOUL2_TRACE) {
          theFxHelper.G2Trace(trace, mOrigin1, NULL, NULL, new_origin, -1,
                              MASK_PLAYERSOLID);
        } else {
          theFxHelper.Trace(trace, mOrigin1, NULL, NULL, new_origin, -1,
                            MASK_SOLID);
        }
      }
      ...
      if (trace.startsolid || trace.allsolid) {
        ...
        return true;
      } else if (trace.fraction <
                 1.0f) //&& !trace.startsolid && !trace.allsolid )
      {
        if (mFlags & FX_IMPACT_RUNS_FX &&
        ...
        // If the velocity is too low, make it stop moving, rotating, and turn
        // off physics to avoid
  ...
  if (mFlags & FX_PLAYER_VIEW) {
    if (mOrigin1[0] < 0 || mOrigin1[0] > 639 || mOrigin1[1] < 0 ||
        mOrigin1[1] > 479) {
      return false;
    }
  }
...
void CParticle::UpdateRotation(void) {
  ...
  mRotationDelta *=
      (1.0f - (theFxHelper.mFrameTime * 0.0007f)); // decay rotationDelta
}
...
bool COrientedParticle::Update(void) {
  ...
  if (!Cull()) { // Only update these if the thing is visible.
    UpdateSize();
    UpdateRGB();
    UpdateAlpha();
    UpdateRotation();
...
//
//----------------------------
CLine::CLine(void) { mRefEnt.reType = RT_LINE; }
...
bool CLine::Update(void) {
  ...
  if (mFlags & FX_RELATIVE) {
    ...
    VectorAdd(mOrigin1, mOrgOffset,
              mOrigin1); // add the offset to the bolt point
...
//
//----------------------------
CTail::CTail(void) { mRefEnt.reType = RT_LINE; }
...
bool CTail::Update(void) {
  ...
  if (mFlags & FX_RELATIVE) {
    ...
    // calc the real velocity and accel vectors
    // FIXME: if you want right and up movement in addition to the forward
    // movement, you'll have to convert dir into a set of perp. axes and do some
    // extra work
    VectorScale(ax[0], mVel[0], realVel);
    VectorMA(realVel, mVel[1], ax[1], realVel);
    VectorMA(realVel, mVel[2], ax[2], realVel);
    ...
    // Get our real velocity at the current time, taking into account the
    // effects of acceleration.  NOTE: not sure if this is even 100% correct
    // math-wise
    VectorMA(realVel, time, realAccel, realVel);
...
bool CCylinder::Cull(void) {
  if (mTraceEnd) { // eh, don't cull variable-length cylinders
    return false;
  }
...
void CEmitter::Draw(void) {
  ...
  if (mFlags & FX_ATTACHED_MODEL) {
    ...
    theFxHelper.AddFxToScene(
        (miniRefEntity_t *)0); // I hate having to do this, but this needs to
                               // get added as a regular refEntity
    theFxHelper.AddFxToScene(&mRefEnt);
  }

  // If we are emitting effects, we had better be careful because just calling
  // it every cgame frame could
  //	either choke up the effects system on a fast machine, or look really
  //nasty on a low end one.
...
#define TRAIL_RATE 12 // we "think" at about a 60hz rate
    ...
    for (t = mOldTime; t <= theFxHelper.mTime; t += TRAIL_RATE) {
      ...
      // ?Not sure if it's better to update this before or after updating the
      // origin
...
bool CEmitter::Update(void) {
  ...
  // If the thing is no longer moving, kill the angle delta, but don't do it too
  // quickly or it will
  //	look very artificial.  Don't do it too slowly or it will look like there
  //is no friction.
  ...
  if (mFlags & FX_PAPER_PHYSICS) {
    ...
    if (moving) {
      // scale the velocity down, basically inducing drag.  Acceleration (
      // gravity ) should keep it pulling down, which is what we want.
...
void CEmitter::UpdateAngles(void) {
  VectorMA(mAngles, theFxHelper.mFrameTime * 0.01f, mAngleDelta,
           mAngles); // was 0.001f, but then you really have to jack up the
...
void CLight::Draw(void) {
  theFxHelper.AddLightToScene(mOrigin1, mRefEnt.radius, mRefEnt.origin[0],
                              mRefEnt.origin[1], mRefEnt.origin[2]);
  drawnFx++;
}
...
void CTrail::Draw() {
  ...
  //	VectorScale( mVerts[NEW_MUZZLE].curRGB, mVerts[NEW_MUZZLE].curAlpha,
  //color );
  ...
  //	VectorScale( mVerts[OLD_MUZZLE].curRGB, mVerts[OLD_MUZZLE].curAlpha,
  //color );
...
bool CTrail::Update() {
  ...
  float perc =
      (float)(mTimeEnd - theFxHelper.mTime) / (float)(mTimeEnd - mTimeStart);

  for (int t = 0; t < 4; t++) {
    //		mVerts[t].curAlpha = mVerts[t].alpha * perc + mVerts[t].destAlpha *
    //( 1.0f - perc ); 		if ( mVerts[t].curAlpha < 0.0f )
    //		{
    //			mVerts[t].curAlpha = 0.0f;
    //		}

    //		VectorScale( mVerts[t].rgb, perc, mVerts[t].curRGB );
    //		VectorMA( mVerts[t].curRGB, ( 1.0f - perc ), mVerts[t].destrgb,
    //mVerts[t].curRGB );
    mVerts[t].curST[0] =
        mVerts[t].ST[0] * perc + mVerts[t].destST[0] * (1.0f - perc);
    if (mVerts[t].curST[0] > 1.0f) {
      mVerts[t].curST[0] = 1.0f;
    }
    mVerts[t].curST[1] =
        mVerts[t].ST[1] * perc + mVerts[t].destST[1] * (1.0f - perc);
  }
...
void CPoly::CalcRotateMatrix(void) {
  ...
  */
  ...
  mRot[2][2] = cosX;
  /*
  // ROLL is not supported unless anyone complains, if it needs to be added, use
  this format Roll
...
void CPoly::Rotate(void) {
  ...
  // Multiply our rotation matrix by each of the offset verts to get their new
  // position
  for (int i = 0; i < mCount; i++) {
...
bool CPoly::Update(void) {
  ...
  // If our timestamp hasn't exired yet, we won't even consider doing any kind
  // of motion
...
void CPoly::PolyInit(void) {
  ...
  // Now we process the passed in points and make it so that they aren't
  // actually the point...
...
bool CBezier::Update(void) {
  ...
  mControl1[0] =
      mControl1[0] + (ftime * mControl1Vel[0]) + (time2 * mControl1Vel[0]);
  mControl2[0] =
      mControl2[0] + (ftime * mControl2Vel[0]) + (time2 * mControl2Vel[0]);
  mControl1[1] =
      mControl1[1] + (ftime * mControl1Vel[1]) + (time2 * mControl1Vel[1]);
  mControl2[1] =
      mControl2[1] + (ftime * mControl2Vel[1]) + (time2 * mControl2Vel[1]);
  mControl1[2] =
      mControl1[2] + (ftime * mControl1Vel[2]) + (time2 * mControl1Vel[2]);
  mControl2[2] =
      mControl2[2] + (ftime * mControl2Vel[2]) + (time2 * mControl2Vel[2]);
...
//----------------------------
inline void CBezier::DrawSegment(vec3_t start, vec3_t end, float texcoord1,
                                 float texcoord2, float segPercent,
  ...
  // scaleBottom is the width of the bottom edge of the quad, scaleTop is the
  // width of the top edge
  scaleBottom =
      (mSizeStart + ((mSizeEnd - mSizeStart) * lastSegPercent)) * 0.5f;
...
void CBezier::Draw(void) {
  ...
  for (mu = incr; mu <= 1.0f; mu += incr) {
    ...
    for (i = 0; i < 3; i++) {
      pos[i] = mum13 * mOrigin1[i] + group1 * mControl1[i] +
               group2 * mControl2[i] + mu3 * mOrigin2[i];
    }
...
bool FX_WorldToScreen(vec3_t worldCoord, float *x, float *y) {
  ...
  // NOTE: did it this way because most draw functions expect virtual 640x480
  // coords 	and adjust them for current resolution
  xcenter =
      640 /
      2; // gives screen coords in virtual 640x480, to be adjusted when drawn
  ycenter =
      480 /
      2; // gives screen coords in virtual 640x480, to be adjusted when drawn
...
void CFlash::Init(void) {
  // 10/19/01 kef -- maybe we want to do something different here for localized
  // flashes, but right
  // now I want to be sure that whatever RGB changes occur to a non-localized
  // flash will also occur to a localized flash (so I'll have the same initial
  // RGBA values for both...I need them sync'd for an effect)
  ...
  if (mFlags & FX_LOCALIZED_FLASH) {
    ...
    // modify size of localized flash based on distance to effect (but not
    // orientation)
...
void CFlash::Draw(void) {
  ...
  if (mFlags & FX_LOCALIZED_FLASH) {
    ...
    // add this 2D effect to the proper list. it will get drawn after the
    // trap->RenderScene call
    theFxScheduler.Add2DEffect(mScreenX, mScreenY, mRefEnt.radius,
                               mRefEnt.radius, color, mRefEnt.customShader);
  } else {
    VectorCopy(theFxHelper.refdef->vieworg, mRefEnt.origin);
    VectorMA(mRefEnt.origin, FLASH_DISTANCE_FROM_VIEWER,
             theFxHelper.refdef->viewaxis[0], mRefEnt.origin);

    // This is assuming that the screen is wider than it is tall.
    mRefEnt.radius = FLASH_DISTANCE_FROM_VIEWER *
                     tan(DEG2RAD(theFxHelper.refdef->fov_x * 0.5f));
