...
// only need to use the CG_S_ system when you want a looping sound that isn't
// going to add itself via trap each frame. Such as ones generated by events.
/*
======================
...
void CG_S_AddLoopingSound(int entityNum, const vec3_t origin,
  ...
  if (alreadyPlaying && cSound) { // if this is the case, just update the
                                  // properties of the looping sound and return.
    VectorCopy(origin, cSound->origin);
    VectorCopy(velocity, cSound->velocity);
  } else if (cent->numLoopingSounds >=
             MAX_CG_LOOPSOUNDS) { // Just don't add it then I suppose.
    return;
  }
    ...
    while (i < cent->numLoopingSounds) {
      ...
      if (cSound->sfx == sfx) { // remove it then
        ...
        while (x < cent->numLoopingSounds) {
          memcpy(&cent->loopingSound[x - 1], &cent->loopingSound[x],
                 sizeof(cent->loopingSound[x]));
          x++;
        }
        cent->numLoopingSounds--;
      }
...
void CG_S_UpdateLoopingSounds(int entityNum) {
  ...
  if ((cent->currentState.eFlags & EF_SOUNDTRACKER) &&
      (!cg.snap ||
       cent->currentState.trickedentindex !=
           cg.snap->ps
               .clientNum)) { // keep sound for this entity updated in
                              // accordance with its attached entity at all times
    // entity out of range
    if (!cg_entities[cent->currentState.trickedentindex].currentValid)
      return;

    VectorCopy(cg_entities[cent->currentState.trickedentindex].lerpOrigin,
               lerpOrg);
  }
  ...
  while (i < cent->numLoopingSounds) {
    ...
    // trap->S_AddLoopingSound(entityNum, cSound->origin, cSound->velocity,
    // cSound->sfx); I guess just keep using lerpOrigin for now,
...
static void CG_EntityEffects(centity_t *cent) {
  ...
  if (cent->currentState.loopSound ||
      ((cent->currentState.loopIsSoundset &&
    ...
    if (cent->currentState.loopIsSoundset &&
        cent->currentState.number >=
            MAX_CLIENTS) { // If this is so, then first get our soundset from
                           // the index, and loopSound actually contains which
                           // part of the set to use rather than a sound index
                           // (BMS_START [0], BMS_MID [1], or BMS_END [2]).
                           // Typically loop sounds will be BMS_MID.
      const char *soundSet;

      soundSet =
          CG_ConfigString(CS_AMBIENT_SET + cent->currentState.soundSetIndex);

      if (soundSet && soundSet[0]) {
        realSoundIndex =
            trap->AS_GetBModelSound(soundSet, cent->currentState.loopSound);
      }
    } else {
      realSoundIndex = cgs.gameSounds[cent->currentState.loopSound];
    }

    // rww - doors and things with looping sounds have a crazy origin (being
    // brush models and all)
    if (realSoundIndex != -1) {
      if (cent->currentState.solid == SOLID_BMODEL) {
        vec3_t origin;
        float *v;
        ...
        trap->S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                                vec3_origin, realSoundIndex);
      } else {
        trap->S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                                vec3_origin, realSoundIndex);
        //	trap->S_AddRealLoopingSound( cent->currentState.number,
        //cent->lerpOrigin, vec3_origin, realSoundIndex );
      }
    }
  }
...
localEntity_t *FX_AddOrientedLine(vec3_t start, vec3_t end, vec3_t normal,
                                  float stScale, float scale, float dscale,
  ...
  RotateAroundDirection(le->refEntity.axis,
                        0); // le->refEntity.data.sprite.rotation );	This is
                            // roll in quad land
...
void FX_DrawPortableShield(centity_t *cent) {
  // rww - this code differs a bit from the draw code in EF, I don't know why I
  // had to do it this way yet it worked in EF the other way.

  int xaxis, height, posWidth, negWidth, team;
  vec3_t start, end, normal;
  qhandle_t shader;

  if (cl_paused.integer) { // rww - fix to keep from rendering repeatedly while
                           // HUD menu is up
    return;
  }
  ...
  FX_AddOrientedLine(start, end, normal, 1.0f, height, 0.0f, 1.0f, 1.0f, 50.0,
                     shader);
}
...
void CG_Special(centity_t *cent) {
  ...
  if (s1->modelindex == HI_SHIELD) { // The portable shield should go through a
...
void CG_CreateBBRefEnts(entityState_t *s1, vec3_t origin) {
  ...
  //g2r
          ...
          for (i=0; i<8; i++)
          {
                  ...
                  point[i].radius = 1;
                  point[i].customShader =
  trap->R_RegisterShader("textures/tests/circle"); point[i].shaderRGBA[0] = 255;
                  ...
                  // now, we need to put the correct origins into each origin
  from the mins and max's switch(i)
                  {
                  case 0:
                  ...
                  // add the original origin to each point and then stuff them
  out there VectorAdd(point[i].origin, origin, point[i].origin);
...
void G2_BoltToGhoul2Model(centity_t *cent, refEntity_t *ent) {
  ...
  // NOTENOTE I put this here because the cgs.gamemodels array no longer gets
  // initialized.
  assert(0);

  // go away and get me the bolt position for this frame please
  trap->G2API_GetBoltMatrix(cent->ghoul2, modelNum, boltNum, &boltMatrix,
...
void CG_Disintegration(centity_t *cent, refEntity_t *ent) {
  ...
  if (cg.time - ent->endTime < 1000 &&
    ...
    trap->G2API_GetBoltMatrix(cent->ghoul2, 0, torsoBolt, &boltMatrix,
    ...
    trap->FX_PlayEffectID(cgs.effects.mDisruptorDeathSmoke, fxOrg, fxDir, -1,
                          -1, qfalse);
...
static qboolean CG_RenderTimeEntBolt(centity_t *cent) {
  ...
  if (clientNum == cg.predictedPlayerState.clientNum &&
      !cg.renderingThirdPerson) { // If in first person and you have it then
                                  // render the thing spinning around on your
                                  // hud.
    cgSiegeEntityRender =
        cent->currentState.number; // set it to render at the end of the frame.
    return qfalse;
  }

  getBolt = trap->G2API_AddBolt(cl->ghoul2, 0, "lhand");

  trap->G2API_GetBoltMatrix(cl->ghoul2, 0, getBolt, &matrix, cl->turAngles,
                            cl->lerpOrigin, cg.time, cgs.gameModels,
                            cl->modelScale);
...
static void CG_SiegeEntRenderAboveHead(centity_t *cent)
{
        ...
        //Set the angles to the global auto rotating ones, and the origin to
slightly above the client VectorCopy(cg.autoAngles, renderAngles); AnglesToAxis(
renderAngles, ent.axis ); VectorCopy(cl->lerpOrigin, ent.origin); ent.origin[2]
+= 50;
...
static void CG_General(centity_t *cent) {
  ...
  if (cent->currentState.modelGhoul2 ==
      127) { // not ready to be drawn or initialized..
    return;
  }

  if (cent->ghoul2 && !cent->currentState.modelGhoul2 &&
      cent->currentState.eType != ET_BODY &&
  ...
  if (cent->currentState
          .boltToPlayer) { // Shove it into the player's left hand then.
    centity_t *pl = &cg_entities[cent->currentState.boltToPlayer - 1];
    if (CG_IsMindTricked(
            pl->currentState.trickedentindex, pl->currentState.trickedentindex2,
            pl->currentState.trickedentindex3,
            pl->currentState.trickedentindex4,
            cg.predictedPlayerState
                .clientNum)) { // don't show if this guy is mindtricking
      return;
    }
    if (!CG_RenderTimeEntBolt(cent)) { // If this function returns qfalse we
                                       // shouldn't render this ent at all.
      if (cent->currentState.boltToPlayer > 0 &&
        ...
        if (cent->currentState.eFlags &
            EF_CLIENTSMOOTH) { // if it's set to smooth keep the smoothed lerp
                               // origin updated, as we don't want to smooth
                               // while bolted.
    ...
    if (cent->currentState.eFlags &
        EF_CLIENTSMOOTH) { // if it's set to smooth keep the smoothed lerp
                           // origin updated, as we don't want to smooth while
                           // bolted.
      VectorCopy(cent->lerpOrigin, cent->turAngles);
    }

    /* disabled for now
                    if (pl->currentState.number !=
       cg.predictedPlayerState.clientNum) { //don't render thing above head to
       self CG_SiegeEntRenderAboveHead(cent);
                    }
    */
    ...
    } else { // if we're sitting on an entity like a moving plat then we don't
             // want to smooth either
      VectorCopy(cent->lerpOrigin, cent->turAngles);
    }
  }

  // rww - now do ragdoll stuff
  if (cent->ghoul2 && (cent->currentState.eType == ET_BODY ||
                       (cent->currentState.eFlags & EF_RAG))) {
    if (!(cent->currentState.eFlags & EF_NODRAW) &&
    ...
    if (cent->ghoul2 &&
        trap->G2_HaveWeGhoul2Models(
            cent->ghoul2)) { // May not be valid, in the case of a ragged entity
                             // being removed and a non-g2 ent filling its slot.
      trap->G2API_SetRagDoll(
          cent->ghoul2, NULL); // calling with null parms resets to no ragdoll.
    }
  }

  if (cent->currentState.boneOrient &&
      cent->ghoul2) { // server sent us some bone angles to use
    CG_G2ServerBoneAngles(cent);
  }

  if ((cent->currentState.eFlags & EF_G2ANIMATING) &&
      cent->ghoul2) { // mini-animation routine for general objects that want to
                      // play quick ghoul2 anims
    // obviously lacks much of the functionality contained in player/npc
    // animation. we actually use torsoAnim as the start frame and legsAnim as
    // the end frame and always play the anim on the root bone.
  ...
  if (cent->currentState.modelGhoul2 >= G2_MODELPART_HEAD &&
      ...
      cent->currentState.weapon ==
          G2_MODEL_PART) { // special case for client limbs
    centity_t *clEnt;
    ...
    if (!dismember_settings) { // This client does not wish to see
                               // dismemberment.
      return;
    }
    ...
    if (dismember_settings < 2 &&
        (cent->currentState.modelGhoul2 == G2_MODELPART_HEAD ||
         cent->currentState.modelGhoul2 ==
             G2_MODELPART_WAIST)) { // dismember settings are not high enough to
                                    // display decaps and torso slashes
      return;
    }
    ...
    if (!cent->ghoul2) {
      ...
      if (clEnt &&
          (clEnt->torsoBolt & limbBit)) { // already have this limb missing!
        return;
      }

      if (clEnt && !(clEnt->currentState.eFlags &
                     EF_DEAD)) { // death flag hasn't made it through yet for
                                 // the limb owner, we cannot create the limb
                                 // until he's flagged as dead
        return;
      }

      if (clEnt &&
          (!BG_InDeathAnim(clEnt->currentState.torsoAnim) ||
           !BG_InDeathAnim(
               clEnt->pe.torso
                   .animationNumber))) { // don't make it unless we're in an
                                         // actual death anim already
        if (clEnt->currentState.torsoAnim !=
            BOTH_RIGHTHANDCHOPPEDOFF) { // exception
          return;
        }
      }
      ...
      if (cent->currentState.modelGhoul2 == G2_MODELPART_HEAD) {
        ...
        rotateBone = "lradius";
        BG_GetRootSurfNameWithVariant(clEnt->ghoul2, "l_arm", limbName,
                                      sizeof(limbName));
        BG_GetRootSurfNameWithVariant(clEnt->ghoul2, "torso", stubName,
                                      sizeof(stubName));
        ...
        rotateBone = "rradius";
        BG_GetRootSurfNameWithVariant(clEnt->ghoul2, "r_arm", limbName,
                                      sizeof(limbName));
        BG_GetRootSurfNameWithVariant(clEnt->ghoul2, "torso", stubName,
                                      sizeof(stubName));
        Com_sprintf(limbCapName, sizeof(limbCapName), "%s_cap_torso", limbName);
        ...
        rotateBone = "rhand";
        BG_GetRootSurfNameWithVariant(clEnt->ghoul2, "r_hand", limbName,
                                      sizeof(limbName));
        BG_GetRootSurfNameWithVariant(clEnt->ghoul2, "r_arm", stubName,
                                      sizeof(stubName));
        Com_sprintf(limbCapName, sizeof(limbCapName), "%s_cap_r_arm", limbName);
        Com_sprintf(stubCapName, sizeof(stubCapName), "%s_cap_r_hand",
                    stubName);
        limbTagName = "*r_hand_cap_r_arm";
        stubTagName = "*r_arm_cap_r_hand";
      } else if (cent->currentState.modelGhoul2 == G2_MODELPART_LLEG) {
        rotateBone = "ltibia";
        BG_GetRootSurfNameWithVariant(clEnt->ghoul2, "l_leg", limbName,
                                      sizeof(limbName));
        BG_GetRootSurfNameWithVariant(clEnt->ghoul2, "hips", stubName,
                                      sizeof(stubName));
        ...
        rotateBone = "rtibia";
        BG_GetRootSurfNameWithVariant(clEnt->ghoul2, "r_leg", limbName,
                                      sizeof(limbName));
        BG_GetRootSurfNameWithVariant(clEnt->ghoul2, "hips", stubName,
                                      sizeof(stubName));
        Com_sprintf(limbCapName, sizeof(limbCapName), "%s_cap_hips", limbName);
      ...
      } else { // umm... just default to the right leg, I guess (same as on
               // server)
        rotateBone = "rtibia";
        BG_GetRootSurfNameWithVariant(clEnt->ghoul2, "r_leg", limbName,
                                      sizeof(limbName));
      ...
      if (clEnt && clEnt->ghoul2) {
        if (trap->G2API_HasGhoul2ModelOnIndex(
                &(clEnt->ghoul2),
                2)) { // don't want to bother dealing with a second saber on
                      // limbs and stuff, just remove the thing
          trap->G2API_RemoveGhoul2Model(&(clEnt->ghoul2), 2);
        }

        if (trap->G2API_HasGhoul2ModelOnIndex(
                &(clEnt->ghoul2), 3)) { // turn off jetpack also I suppose
          trap->G2API_RemoveGhoul2Model(&(clEnt->ghoul2), 3);
        }

        if (clEnt->localAnimIndex <= 0) { // humanoid
          trap->G2API_SetBoneAngles(
              clEnt->ghoul2, 0, "model_root", vec3_origin, BONE_ANGLES_POSTMULT,
              POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
          trap->G2API_SetBoneAngles(
              clEnt->ghoul2, 0, "pelvis", vec3_origin, BONE_ANGLES_POSTMULT,
              POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time);
          trap->G2API_SetBoneAngles(
              clEnt->ghoul2, 0, "thoracic", vec3_origin, BONE_ANGLES_POSTMULT,
              POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time);
          trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "upper_lumbar",
                                    vec3_origin, BONE_ANGLES_POSTMULT,
                                    POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
                                    cgs.gameModels, 100, cg.time);
          trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "lower_lumbar",
                                    vec3_origin, BONE_ANGLES_POSTMULT,
                                    POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z,
                                    cgs.gameModels, 100, cg.time);
          trap->G2API_SetBoneAngles(
              clEnt->ghoul2, 0, "cranium", vec3_origin, BONE_ANGLES_POSTMULT,
              POSITIVE_Z, NEGATIVE_Y, POSITIVE_X, cgs.gameModels, 100, cg.time);
        } else {
          trap->G2API_SetBoneAngles(
              clEnt->ghoul2, 0, "model_root", vec3_origin, BONE_ANGLES_POSTMULT,
              POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
      ...
      if (newBolt != -1) {
        ...
        trap->G2API_GetBoltMatrix(cent->ghoul2, 0, newBolt, &matrix,
                                  cent->lerpAngles, cent->lerpOrigin, cg.time,
                                  cgs.gameModels, cent->modelScale);
        ...
        trap->FX_PlayEffectID(cgs.effects.mBlasterSmoke, boltOrg, boltAng, -1,
                              -1, qfalse);
      }
      ...
      trap->G2API_SetNewOrigin(
          cent->ghoul2, trap->G2API_AddBolt(cent->ghoul2, 0, rotateBone));
      ...
      if (newBolt != -1) {
        ...
        trap->G2API_GetBoltMatrix(clEnt->ghoul2, 0, newBolt, &matrix,
                                  clEnt->lerpAngles, clEnt->lerpOrigin, cg.time,
                                  cgs.gameModels, clEnt->modelScale);
      ...
      if (cent->currentState.modelGhoul2 == G2_MODELPART_RARM ||
          cent->currentState.modelGhoul2 == G2_MODELPART_RHAND ||
          cent->currentState.modelGhoul2 ==
              G2_MODELPART_WAIST) { // Cut his weapon holding arm off, so remove
                                    // the weapon
      ...
      clEnt->torsoBolt |=
          limbBit; // reinit model after copying limbless one to queue
      // This causes issues after respawning.. just keep track of limbs cut/made
      // on server or something.
      /*
    ...
    if (cent->ghoul2 && cent->bolt4 != -1 && cent->trailTime < cg.time) {
      if (cent->bolt4 != -1 && (cent->currentState.pos.trDelta[0] ||
                                cent->currentState.pos.trDelta[1] ||
                                cent->currentState.pos.trDelta[2])) {
        vec3_t boltOrg, boltAng;

        trap->G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt4, &matrix,
                                  cent->lerpAngles, cent->lerpOrigin, cg.time,
                                  cgs.gameModels, cent->modelScale);
  ...
  if (cent->currentState.number >= MAX_CLIENTS &&
    ...
    trap->G2API_SetBoneAngles(cent->ghoul2, 0, "Bone02", empAngles,
                              BONE_ANGLES_REPLACE, NEGATIVE_Y, NEGATIVE_X,
  ...
  */
  ...
  if (cent->currentState
          .modelGhoul2) { // If the game says this guy uses a ghoul2 model and
                          // the g2 instance handle is null, then initialize it
    if (!cent->ghoul2 && !cent->currentState.bolt1) {
      char skinName[MAX_QPATH];
      const char *modelName =
          CG_ConfigString(CS_MODELS + cent->currentState.modelindex);
      int l;
      int skin = 0;

      trap->G2API_InitGhoul2Model(&cent->ghoul2, modelName, 0, 0, 0, 0, 0);
      if (cent->ghoul2 &&
          trap->G2API_SkinlessModel(cent->ghoul2,
                                    0)) { // well, you'd never want a skinless
                                          // model, so try to get his skin...
  ...
  if (s1->eType == ET_BODY) { // bodies should have a radius as well
    ...
    if (cent->ghoul2) { // all bodies should already have a ghoul2 instance. Use
                        // it to set the torso/head angles to 0.
      cent->lerpAngles[PITCH] = 0;
      cent->lerpAngles[ROLL] = 0;
      trap->G2API_SetBoneAngles(cent->ghoul2, 0, "pelvis", vec3_origin,
                                BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                NEGATIVE_Z, cgs.gameModels, 0, cg.time);
      trap->G2API_SetBoneAngles(cent->ghoul2, 0, "thoracic", vec3_origin,
                                BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                NEGATIVE_Z, cgs.gameModels, 0, cg.time);
      trap->G2API_SetBoneAngles(cent->ghoul2, 0, "upper_lumbar", vec3_origin,
                                BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                NEGATIVE_Z, cgs.gameModels, 100, cg.time);
      trap->G2API_SetBoneAngles(cent->ghoul2, 0, "lower_lumbar", vec3_origin,
                                BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
                                NEGATIVE_Z, cgs.gameModels, 100, cg.time);
      trap->G2API_SetBoneAngles(cent->ghoul2, 0, "cranium", vec3_origin,
                                BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y,
                                POSITIVE_X, cgs.gameModels, 100, cg.time);
    }
  }

  if (s1->eType == ET_HOLOCRON &&
      s1->modelindex < -100) { // special render, it's a holocron
    // Using actual models now:
    ent.hModel =
        trap->R_RegisterModel(forceHolocronModels[s1->modelindex + 128]);
  ...
  if (cent->currentState.iModelScale) { // if the server says we have a custom
                                        // scale then set it now.
    cent->modelScale[0] = cent->modelScale[1] = cent->modelScale[2] =
        cent->currentState.iModelScale / 100.0f;
    VectorCopy(cent->modelScale, ent.modelScale);
    ScaleModelAxis(&ent);
  } else {
  ...
  if (cent->currentState.time > cg.time &&
      cent->currentState.weapon == WP_EMPLACED_GUN) {
    ...
    trap->R_AddRefEntityToScene(&ent);
    ent.customShader = 0;
  } else if (cent->currentState.time == -1 &&
             cent->currentState.weapon == WP_EMPLACED_GUN) {
    ent.customShader = trap->R_RegisterShader(
        "models/map_objects/imp_mine/turret_chair_dmg.tga");
    // trap->R_AddRefEntityToScene( &ent );
  }
      ...
      */
      ...
      if (tMult > 200) { // begin the disintegration effect
        ...
        if (!cent->dustTrailTime) {
          ...
          if (lightSide) {
            trap->S_StartSound(
                NULL, cent->currentState.number, CHAN_AUTO,
  ...
  if (cent->bolt3 == 999) { // this is an in-flight saber being rendered
                            // manually
    vec3_t org;
    float wv;
    int i;
    ...
    for (i = -4; i < 10; i += 1) {
      ...
      // trap->FX_AddSprite( org, NULL, NULL, 5.5f, 5.5f, wv, wv, 0.0f,
      // 0.0f, 1.0f, cgs.media.yellowSaberGlowShader, 0x08000000 );
  ...
  if (cent->currentState.time == -1 &&
      cent->currentState.weapon == WP_TRIP_MINE &&
      (cent->currentState.eFlags &
       EF_FIRING)) { // if force sight is active, render the laser multiple
                     // times up to the force sight level to increase visibility
    if (cent->currentState.bolt2 == 1) {
      VectorMA(ent.origin, 6.6f, ent.axis[0], beamOrg); // forward
      beamID = cgs.effects.tripmineGlowFX;
      trap->FX_PlayEffectID(beamID, beamOrg, cent->currentState.pos.trDelta, -1,
                            -1, qfalse);
    } else {
      int i = 0;
      ...
      if (cg.snap->ps.fd.forcePowersActive & (1 << FP_SEE)) {
        ...
        while (i > 0) {
          trap->FX_PlayEffectID(beamID, beamOrg, cent->currentState.pos.trDelta,
                                -1, -1, qfalse);
          trap->FX_PlayEffectID(beamID, beamOrg, cent->currentState.pos.trDelta,
                                -1, -1, qfalse);
          i--;
...
static void CG_Item(centity_t *cent) {
  ...
  */
  ...
  if ((item->giType == IT_WEAPON || item->giType == IT_POWERUP) &&
    ...
    if (!doGrey) {
      trap->FX_PlayEffectID(cgs.effects.itemCone, ent.origin, uNorm, -1, -1,
                            qfalse);
    }
  }
  ...
  */
  ...
  if ((!(cent->currentState.eFlags & EF_DROPPEDWEAPON) ||
      ...
      (item->giType == IT_WEAPON ||
       item->giType == IT_POWERUP)) { // only weapons and powerups rotate now
  ...
  */
  ...
  if (item->giType != IT_TEAM && msec >= 0 && msec < ITEM_SCALEUP_TIME &&
      !(es->eFlags & EF_ITEMPLACEHOLDER) &&
      !(es->eFlags & EF_DROPPEDWEAPON)) { // if just respawned, fade in, but
                                          // don't do this for flags.
    float alpha;
    int a;
    ...
    if (item->giType != IT_POWERUP ||
        item->giTag != PW_FORCE_BOON) { // boon model uses a different blending
                                        // mode for the sprite inside and doesn't
                                        // look proper with this method
      ent.renderfx |= RF_FORCE_ENT_ALPHA;
    }
    trap->R_AddRefEntityToScene(&ent);
...
void CG_CreateDistortionTrailPart(centity_t *cent, float scale, vec3_t pos) {
  ...
  if (VectorNormalize(ent.axis[0]) <=
      0.1f) { // Entity is right on vieworg.  quit.
    return;
  }
  ...
  ent.hModel =
      trap->R_RegisterModel("models/weapons2/merr_sonn/trailmodel.md3");
  ent.customShader =
      cgs.media
          .itemRespawningRezOut; // cgs.media.cloakedShader;//cgs.media.halfShieldShader;
...
static void CG_Missile(centity_t *cent) {
  ...
  if (s1->weapon == WP_SABER) {
    if ((cent->currentState.modelindex != cent->serverSaberHitIndex ||
         !cent->ghoul2) &&
        !(s1->eFlags &
          EF_NODRAW)) { // no g2, or server changed the model we are using
      const char *saberModel =
          CG_ConfigString(CS_MODELS + cent->currentState.modelindex);

      cent->serverSaberHitIndex = cent->currentState.modelindex;

      if (cent->ghoul2) { // clean if we already have one (because server
                          // changed model string index)
        trap->G2API_CleanGhoul2Models(&(cent->ghoul2));
        cent->ghoul2 = 0;
      }
      ...
      if (saberModel && saberModel[0]) {
        ...
        trap->G2API_InitGhoul2Model(
            &cent->ghoul2, "models/weapons2/saber/saber_w.glm", 0, 0, 0, 0, 0);
      }
      return;
  ...
  if (s1->otherEntityNum2 &&
      s1->weapon != WP_SABER) { // using an over-ridden trail effect!
    vec3_t forward;
    ...
    if ((s1->eFlags &
        ...
        && (g_vehWeaponInfo[s1->otherEntityNum2].iShotFX ||
            g_vehWeaponInfo[s1->otherEntityNum2].iModel !=
                NULL_HANDLE)) { // a vehicle with an override for the weapon
                                // trail fx or model
      trap->FX_PlayEffectID(g_vehWeaponInfo[s1->otherEntityNum2].iShotFX,
                            cent->lerpOrigin, forward, -1, -1, qfalse);
      ...
      trap->FX_PlayEffectID(cgs.gameEffects[s1->otherEntityNum2],
                            cent->lerpOrigin, forward, -1, -1, qfalse);
      if (s1->loopSound) {
        vec3_t velocity;
        BG_EvaluateTrajectoryDelta(&cent->currentState.pos, cg.time, velocity);
        trap->S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                                velocity, s1->loopSound);
      }
      // FIXME: if has a custom model, too, then set it and do rest of code
      // below?
      return;
    }
    ...
    if (weapon->altMissileDlight) {
      trap->R_AddLightToScene(cent->lerpOrigin, weapon->altMissileDlight,
                              weapon->altMissileDlightColor[0],
                              weapon->altMissileDlightColor[1],
    ...
    if (weapon->altMissileSound) {
      ...
      trap->S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                              velocity, weapon->altMissileSound);
    }
    ...
    // Don't draw something without a model
    if (weapon->missileModel == NULL_HANDLE && s1->weapon != WP_SABER &&
        s1->weapon != G2_MODEL_PART) // saber uses ghoul2 model, doesn't matter
      return;
  }
  ...
  */
  ...
  if (s1->weapon == WP_SABER &&
      cgs.gametype == GT_JEDIMASTER) { // in jedimaster always make the saber
                                       // glow when on the ground
    vec3_t org;
    float wv;
    int i;
    addspriteArgStruct_t fxSArgs;
  ...
  if (s1->eFlags & EF_FIRING) { // special code for adding the beam to the
                                // attached tripwire mine
    vec3_t beamOrg;

    VectorMA(ent.origin, 8, ent.axis[0], beamOrg); // forward
    trap->FX_PlayEffectID(cgs.effects.mTripMineLaser, beamOrg, ent.axis[0], -1,
                          -1, qfalse);
  }
}
...
void CG_PlayDoorLoopSound(centity_t *cent) {
  ...
  if (cent->currentState.eType ==
      ET_MOVER) // shouldn't be in here otherwise, but just in case.
  {
    v = cgs.inlineModelMidpoints[cent->currentState.modelindex];
    VectorAdd(cent->lerpOrigin, v, origin);
  } else {
...
static void CG_Mover(centity_t *cent) {
  ...
  if ((cent->currentState.eFlags2 &
       EF2_HYPERSPACE)) { // I'm the hyperspace brush
    ...
    if (cg.predictedPlayerState.m_iVehicleNum &&
      ...
      if (cg.snap &&
          cg.snap->ps.pm_type == PM_INTERMISSION) { // in the intermission, stop
                                                    // drawing hyperspace ent
      } else if ((cg.predictedVehicleState.eFlags2 &
                  EF2_HYPERSPACE)) { // actually hyperspacing now
        float timeFrac =
            ((float)(cg.time - cg.predictedVehicleState.hyperSpaceTime -
                     1000)) /
            (HYPERSPACE_TIME - 1000);
        if (timeFrac < (HYPERSPACE_TELEPORT_FRAC +
                        0.1f)) { // still in hyperspace or just popped out
          const float alpha = timeFrac < 0.5f ? timeFrac / 0.5f : 1.0f;
          drawMe = qtrue;
          VectorMA(cg.refdef.vieworg, 1000.0f + ((1.0f - timeFrac) * 1000.0f),
                   cg.refdef.viewaxis[0], cent->lerpOrigin);
          VectorSet(cent->lerpAngles, cg.refdef.viewangles[PITCH],
                    cg.refdef.viewangles[YAW] - 90.0f,
                    0); // cos( ( cg.time + 1000 ) *  scale ) * 4 );
          ent.shaderRGBA[0] = ent.shaderRGBA[1] = ent.shaderRGBA[2] = 255;
...
void CG_CalcEntityLerpPositions(centity_t *cent) {
  ...
  if (cg.predictedPlayerState.m_iVehicleNum &&
      ...
      cent->currentState.eType == ET_NPC &&
      cent->currentState.NPC_class ==
          CLASS_VEHICLE) { // special case for vehicle we are riding
    centity_t *veh = &cg_entities[cg.predictedPlayerState.m_iVehicleNum];

    if (veh->currentState.owner ==
        cg.predictedPlayerState
            .clientNum) { // only do this if the vehicle is pilotted by this
                          // client and predicting properly
  ...
  if (cent->interpolate && cent->currentState.pos.trType == TR_LINEAR_STOP &&
    ...
    CG_InterpolateEntityPosition(cent);
    goAway = qtrue;
  } else if (cent->interpolate && cent->currentState.eType == ET_NPC &&
             cent->currentState.NPC_class == CLASS_VEHICLE) {
    CG_InterpolateEntityPosition(cent);
    goAway = qtrue;
  } else {
...
static void CG_AddCEntity(centity_t *cent) {
  ...
  if (cg.predictedPlayerState.pm_type ==
      PM_INTERMISSION) { // don't render anything then
    ...
    if (cent->currentState.eType == ET_NPC) { // NPC in intermission
      if (cent->currentState.NPC_class ==
          CLASS_VEHICLE) { // don't render vehicles in intermissions, allow
                           // other NPCs for scripts
        return;
      }
    }
  }

  // don't render when we are in spec, happens occasionally on map_restart and
  // such
  ...
  */
  ...
  if (cent->currentState.soundSetIndex &&
    ...
    if (soundSet && soundSet[0]) {
      trap->S_AddLocalSet(soundSet, cg.refdef.vieworg, cent->lerpOrigin,
  ...
  */
  ...
  case ET_SPEAKER:
    CG_Speaker(cent);
    break;
  case ET_NPC: // An entity that wants to be able to use ghoul2 humanoid (and
               // other) anims. Like a player, but not.
    CG_G2Animated(cent);
    break;
  case ET_TEAM:
    break;
...
void CG_AddPacketEntities(qboolean isPortal) {
  ...
  BG_PlayerStateToEntityState(
      ps, &cg_entities[cg.predictedPlayerState.clientNum].currentState, qfalse);

  if (cg.predictedPlayerState
          .m_iVehicleNum) { // add the vehicle I'm riding first
    // BG_PlayerStateToEntityState( &cg.predictedVehicleState,
    // &cg_entities[cg.predictedPlayerState.m_iVehicleNum].currentState, qfalse
    // ); cg_entities[cg.predictedPlayerState.m_iVehicleNum].currentState.eType =
    // ET_NPC;
    centity_t *veh = &cg_entities[cg.predictedPlayerState.m_iVehicleNum];

    if (veh->currentState.owner == cg.predictedPlayerState.clientNum) {
      BG_PlayerStateToEntityState(&cg.predictedVehicleState, &veh->currentState,
                                  qfalse);
      ...
      if (cent->currentState.eType == ET_PLAYER &&
        ...
        while (j < cg.snap->numEntities) {
          ...
          j++;
                 ...
                 cent->bodyHeight ==
                     cg.time) { // never add a vehicle with a pilot, his pilot
                                // entity will get him added first.
        // if we were to add the vehicle after the pilot, the pilot's bolt would
        // lag a frame behind.
        continue;
      }
      CG_AddCEntity(cent);
...
void CG_ROFF_NotetrackCallback(centity_t *cent, const char *notetrack) {
  ...
  defaultoffsetposition:
    ...
    trap->S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_BODY,
                       objectID);
...
void CG_Cube(vec3_t mins, vec3_t maxs, vec3_t color, float alpha) {
  ...
  for (axis = 0, vec[0] = 0, vec[1] = 1, vec[2] = 2; axis < 3;
    ...
    //- face
    apArgs.p[0][vec[0]] = apArgs.p[1][vec[0]] = apArgs.p[2][vec[0]] =
