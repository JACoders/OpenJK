...
checkprint:
	...
	if (gotbestindex && bestindex != gLastPrintedIndex) {
		...
		trap->Print(S_COLOR_YELLOW "Waypoint %i\nFlags - %i (%s) (w%f)\nOrigin - (%i %i %i)\n", (int)(gWPArray[bestindex]->index),
					(int)(gWPArray[bestindex]->flags), flagstr, gWPArray[bestindex]->weight, (int)(gWPArray[bestindex]->origin[0]),
					(int)(gWPArray[bestindex]->origin[1]), (int)(gWPArray[bestindex]->origin[2]));
...
int NodeHere(vec3_t spot) {
	...
	while (i < nodenum) {
		if ((int)nodetable[i].origin[0] == (int)spot[0] && (int)nodetable[i].origin[1] == (int)spot[1]) {
			if ((int)nodetable[i].origin[2] == (int)spot[2] || ((int)nodetable[i].origin[2] < (int)spot[2] && (int)nodetable[i].origin[2] + 5 > (int)spot[2]) ||
...
int ConnectTrail(int startindex, int endindex, qboolean behindTheScenes) {
	...
	if (RMG.integer) { // this might be temporary. Or not.
		if (!(gWPArray[startindex]->flags & WPFLAG_NEVERONEWAY) && !(gWPArray[endindex]->flags & WPFLAG_NEVERONEWAY)) {
		...
		if (gWPArray[i] && gWPArray[i]->inuse) {
			...
			while (c < gWPNum) {
				if (gWPArray[c] && gWPArray[c]->inuse && i != c && NotWithinRange(i, c)) {
					...
					if ((nLDist < maxNeighborDist || forceJumpable) && ((int)gWPArray[i]->origin[2] == (int)gWPArray[c]->origin[2] || forceJumpable) &&
...
int GetNearestVisibleWPToItem(vec3_t org, int ignore) {
	...
	while (i < gWPNum) {
		if (gWPArray[i] && gWPArray[i]->inuse && gWPArray[i]->origin[2] - 15 < org[2] && gWPArray[i]->origin[2] + 15 > org[2]) {
...
int SavePathData(const char *filename) {
	...
	Com_sprintf(fileString, 524288, "%i %i %f (%f %f %f) { ", gWPArray[i]->index, gWPArray[i]->flags, gWPArray[i]->weight, gWPArray[i]->origin[0],
	...
	while (i < gWPNum) {
		// sprintf(fileString, "%s%i %i %f (%f %f %f) { ", fileString, gWPArray[i]->index, gWPArray[i]->flags, gWPArray[i]->weight, gWPArray[i]->origin[0],
		// gWPArray[i]->origin[1], gWPArray[i]->origin[2]);
		Com_sprintf(storeString, 4096, "%i %i %f (%f %f %f) { ", gWPArray[i]->index, gWPArray[i]->flags, gWPArray[i]->weight, gWPArray[i]->origin[0],
					gWPArray[i]->origin[1], gWPArray[i]->origin[2]);
...
int G_NearestNodeToPoint(vec3_t point) { // gets the node on the entire grid which is nearest to the specified coordinates.
	vec3_t vSub;
	int bestIndex = -1;
	int i = 0;
	float bestDist = 0;
...
int G_NodeMatchingXY(float x, float y) { // just get the first unflagged node with the matching x,y coordinates.
	...
	while (i < nodenum) {
		if (nodetable[i].origin[0] == x && nodetable[i].origin[1] == y && !nodetable[i].flags) {
			return i;
		}
...
int G_NodeMatchingXY_BA(int x, int y, int final) { // return the node with the lowest weight that matches the specified x,y coordinates.
	...
	while (i < nodenum) {
		if ((int)nodetable[i].origin[0] == x && (int)nodetable[i].origin[1] == y && !nodetable[i].flags &&
...
void CreateAsciiTableRepresentation() { // Draw a text grid of the entire waypoint array (useful for debugging final waypoint placement)
	fileHandle_t f;
	int i = 0;
	int sP = 0;
	int placeX;
...
qboolean G_BackwardAttachment(int start, int finalDestination,
							  int insertAfter) { // After creating a node path between 2 points, this function links the 2 points with actual waypoint data.
	int indexDirections[4];						 // 0 == down, 1 == up, 2 == left, 3 == right
	int i = 0;
	...
	while (i < 4) {
		if (indexDirections[i] != -1) {
			...
			if (nodetable[indexDirections[i]].weight < lowestWeight && nodetable[indexDirections[i]].weight &&
				!nodetable[indexDirections[i]].flags /*&& (nodetable[indexDirections[i]].origin[2]-64 < nodetable[start].origin[2])*/) {
...
void G_RMGPathing(void) { // Generate waypoint information on-the-fly for the random mission.
	...
	while (placeY < terrain->r.absmax[1]) {
		...
		while (placeX < terrain->r.absmax[0]) {
			...
			if ((tr.entityNum >= ENTITYNUM_WORLD || g_entities[tr.entityNum].s.eType == ET_TERRAIN) &&
				tr.endpos[2] < terrain->r.absmin[2] + 750) { // only drop nodes on terrain directly
	...
	while (i < gSpawnPointNum - 1) {
		...
		if (G_RecursiveConnection(nearestIndex, nearestIndexForNext, 0, qtrue, terrain->r.absmin[2]) !=
			nearestIndexForNext) { // failed to branch to where we want. Oh well, try it without trace checks.
			G_NodeClearForNext();

			if (G_RecursiveConnection(nearestIndex, nearestIndexForNext, 0, qfalse, terrain->r.absmin[2]) !=
				nearestIndexForNext) { // still failed somehow. Just disregard this point.
				G_NodeClearForNext();
				i++;
				continue;
			}
		...
		if (G_BackwardAttachment(nearestIndexForNext, nearestIndex, gWPNum - 1)) { // successfully connected the trail from nearestIndex to nearestIndexForNext
			if (gSpawnPoints[i + 1]->inuse && gSpawnPoints[i + 1]->item && gSpawnPoints[i + 1]->item->giType == IT_TEAM) { // This point is actually a CTF flag.
				if (gSpawnPoints[i + 1]->item->giTag == PW_REDFLAG ||
					gSpawnPoints[i + 1]->item->giTag ==
						PW_BLUEFLAG) { // Place a waypoint on the flag next in the trail, so the nearest grid point will link to it.
