...
void NORETURN QDECL Com_Error(int code, const char *fmt, ...) {
  ...
  if (code == ERR_DISCONNECT || code == ERR_SERVERDISCONNECT ||
      code == ERR_DROP || code == ERR_NEED_CD) {
    throw code;
  } else {
    CL_Shutdown();
...
#else
    // OJKFIXME: Temporarily disabled dedicated server when not using the
    // dedicated server binary. 			Issue is the server not having a renderer when
    //not using ^^^^^ 				and crashing in SV_SpawnServer calling
    //re.RegisterMedia_LevelLoadBegin 			Until we fully remove the renderer from
    //the server, the client executable 				will not have dedicated support
    //capabilities. 			Use the dedicated server package.
    com_dedicated =
...
//	Cvar_CheckRange( com_dedicated, 0, 2, qtrue );
    ...
    com_sv_running =
        Cvar_Get("sv_running", "0", CVAR_ROM, "Is a server running?");
    com_cl_running =
        Cvar_Get("cl_running", "0", CVAR_ROM, "Is the client running?");
    ...
    com_bootlogo =
        Cvar_Get("com_bootlogo", "1", CVAR_ARCHIVE_ND, "Show intro movies");
...
void Com_Shutdown(void) {
  ...
  MSG_shutdownHuffman();
  /*
          // Only used for testing changes to huffman frequency table when
     tuning.
          {
