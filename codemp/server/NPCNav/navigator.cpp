...
// this was in the game before. But now it's all handled in the engine, and we
// make navigator. calls via traps.
...
CNode *CNode::Create(void) { return new CNode; }
...
void CNode::AddEdge(int ID, int cost, int flags) {
  if (m_numEdges) { // already have at least 1
    // see if it exists already
    edge_v::iterator ei;
    STL_ITERATE(ei, m_edges) {
      if ((*ei).ID == ID) { // found it
        (*ei).cost = cost;
        (*ei).flags = flags;
        return;
      }
...
void CNode::Draw(qboolean showRadius) { // rwwFIXMEFIXME: ...
  /*
...
int CNode::Save(int numNodes, fileHandle_t file) {
  ...
  edge_v::iterator ei;
  STL_ITERATE(ei, m_edges) { FS_Write(&(*ei), sizeof(edge_t), file); }
...
void CNavigator::FlagAllNodes(int newFlag) {
  ...
  STL_ITERATE(ni, m_nodes) { (*ni)->AddFlag(newFlag); }
}
...
void CNavigator::Free(void) {
  ...
  STL_ITERATE(ni, m_nodes) { delete (*ni); }
...
bool CNavigator::Save(const char *filename, int checksum) {
  ...
  STL_ITERATE(ni, m_nodes) { (*ni)->Save(numNodes, file); }
...
int CNavigator::GetEdgeCost(CNode *first, CNode *second) {
  ...
  SV_Trace(&trace, start, mins, maxs, end, ENTITYNUM_NONE, MASK_SOLID, qfalse,
           0, 10);
...
void CNavigator::SetEdgeCost(int ID1, int ID2, int cost) {
  if ((ID1 == -1) || (ID2 == -1)) { // not valid nodes, must have come from the
                                    // ClearAllFailedEdges initization-type calls
    return;
  }
  ...
  if (cost == -1) { // they want us to calc it
...
void CNavigator::AddNodeEdges(CNode *node, int addDist, edge_l &edgeList,
                              bool *checkedNodes) {
  // Add all edge
  for (int i = 0; i < node->GetNumEdges(); i++) {
    ...
    // Add it to the list
    STL_INSERT(edgeList, CEdge(nextNode->GetID(), node->GetID(),
                               addDist + (node->GetEdgeCost(i))));
  }
}
...
void CNavigator::CalculatePath(CNode *node) {
  ...
  for (i = 0; i < node->GetNumEdges(); i++) {
    ...
    pathList->Push(
        new CEdge(nextNode->GetID(), nextNode->GetID(), node->GetEdgeCost(i)));
  }
...
void CNavigator::ShowNodes(void) {
  ...
  STL_ITERATE(ni, m_nodes) {
    ...
    if (0) {
      ...
      radius = (*ni)->GetRadius();
      // if player within node radius or 256, draw radius (sometimes the radius
      // is really small, so we check for 256 to catch everything)
  ...
  STL_ITERATE(ni, m_nodes) {
    ...
    for (edgeNum = 0; edgeNum < (*ni)->GetNumEdges(); edgeNum++) {
      ...
      if ((flags & EFLAG_BLOCKED)) {
        ...
        // FIXME: can't we just store the trace.entityNum from the HardConnect
        // trace?  So we don't have to do another trace here...
        SV_Trace(&trace, p1, wpMins, wpMaxs, p2, ENTITYNUM_NONE,
                 MASK_SOLID | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP, qfalse,
                 0, 10);

        if (trace.entityNum < ENTITYNUM_WORLD &&
            (trace.fraction < 1.0f || trace.startsolid == qtrue ||
             trace.allsolid ==
                 qtrue)) { // could be assumed, since failed before
          if (GVM_NAV_EntIsDoor(trace.entityNum)) {            // door
            if (!GVM_NAV_EntIsUnlockedDoor(trace.entityNum)) { // locked door
              failed = qtrue;
            }
          } else {
            if (GVM_NAV_EntIsBreakable(
                    trace.entityNum)) { // do same for breakable
                                        // brushes/models/glass?
              failed = qtrue;
            } else if (GVM_NAV_EntIsRemovableUsable(trace.entityNum)) {
              failed = qtrue;
            } else if (trace.allsolid ||
                       trace.startsolid) { // FIXME: the entitynum would be none
                                           // here, so how do we know if this is
                                           // stuck inside an ent or the world?
            } else { // FIXME: what about func_plats and scripted movers?
            }
          }
        }

        if (failed) {
          // could add the EFLAG_FAILED to the two edges, but we stopped doing
          // that since it was pointless
...
void CNavigator::HardConnect(int first, int second) {
  ...
  SV_Trace(&trace, p1, wpMins, wpMaxs, p2, ENTITYNUM_NONE,
           MASK_SOLID | CONTENTS_BOTCLIP | CONTENTS_MONSTERCLIP, qfalse, 0, 10);

  int cost = Distance(p1, p2);

  if (trace.fraction != 1.0f || trace.startsolid == qtrue ||
...
int CNavigator::TestNodePath(
    sharedEntity_t *ent, int okToHitEntNum, vec3_t position,
    qboolean includeEnts) {  // rwwFIXMEFIXME: Share clipmask?
  ...
  // Check the path
  if (GVM_NAV_ClearPathToPoint(ent->s.number, ent->r.mins, ent->r.maxs,
                               position, clipmask, okToHitEntNum) == false)
    return false;
...
int CNavigator::TestBestFirst(sharedEntity_t *ent, int lastID, int flags) {
  ...
  // Setup our last node as our root, and search for a closer one according to
  // its edges
  int bestNode =
      (TestNodePath(ent, ENTITYNUM_NONE, nodePos, qtrue)) ? lastID : NODE_NONE;
  float bestDist = (bestNode == NODE_NONE)
                       ? Q3_INFINITE
                       : DistanceSquared(ent->r.currentOrigin, nodePos);
  ...
  for (int i = 0; i < numEdges; i++) {
    ...
    if (dist < bestDist) {
      ...
      if (CheckedNode(testNode->GetID(), ent->s.number) == CHECKED_PASSED ||
          TestNodePath(ent, ENTITYNUM_NONE, nodePos, qtrue)) {
        bestDist = dist;
...
int CNavigator::CollectNearestNodes(vec3_t origin, int radius, int maxCollect,
  ...
  STL_ITERATE(ni, m_nodes) {
    ...
    // Otherwise, always pad out the collection if possible so we don't miss
    // anything
...
int CNavigator::GetBestPathBetweenEnts(sharedEntity_t *ent,
...
#define MAX_Z_DELTA 18
  ...
  CollectNearestNodes(ent->r.currentOrigin, NODE_COLLECT_RADIUS,
                      NODE_COLLECT_MAX, nodeChain);
  CollectNearestNodes(goal->r.currentOrigin, NODE_COLLECT_RADIUS,
                      NODE_COLLECT_MAX, nodeChain2);
  ...
  STL_ITERATE(nci, nodeChain) {
    ...
    if (CheckedNode(nodeNum, ent->s.number) ==
        CHECKED_FAILED) { // already checked this node against ent and it failed
      continue;
    }
    if (CheckedNode(nodeNum, ent->s.number) ==
        CHECKED_PASSED) { // already checked this node against ent and it passed
    } else {              // haven't checked this node against ent yet
      if (NodeFailed(ent, nodeNum)) {
        SetCheckedNode(nodeNum, ent->s.number, CHECKED_FAILED);
        continue;
      }
      // okay, since we only have to do this once, let's check to see if this
      // node is even usable (could help us short-circuit a whole loop of the
      // dest nodes)
      radius = node->GetRadius();

      // If we're not within the known clear radius of this node OR out of Z
      // height range...
      if ((int)(*nci).distance >= (radius * radius) ||
          (fabs(position[2] - ent->r.currentOrigin[2]) >= MAX_Z_DELTA)) {
        // We're not *within* this node, so check clear path, etc.

        // FIXME: any way to call G_FindClosestPointOnLineSegment and see if I
        // can at least get to the waypoint's path
        if (flags & NF_CLEAR_PATH) //|| flags & NF_CLEAR_LOS )
        {                          // need a clear path or LOS
          if (!SV_inPVS(ent->r.currentOrigin,
                        position)) { // not even potentially clear
            SetCheckedNode(nodeNum, ent->s.number, CHECKED_FAILED);
            continue;
          }
        }
    ...
    STL_ITERATE(nci2, nodeChain2) {
      ...
      // Okay, first get the entire path cost, including distance to first node
      // from ents' positions
      cost = floor(Distance(ent->r.currentOrigin, position) +
                   Distance(goal->r.currentOrigin, position2));

      if (d_altRoutes->integer) {
        nextNode = GetBestNodeAltRoute((*nci).nodeID, (*nci2).nodeID, &pathCost,
                                       bestNode);
        cost += pathCost;
      } else {
      ...
      if (CheckedNode(nodeNum2, goal->s.number) ==
          CHECKED_FAILED) { // already checked this node against goal and it
                            // failed
        continue;
      }
      if (CheckedNode(nodeNum2, goal->s.number) ==
          CHECKED_PASSED) { // already checked this node against goal and it
                            // passed
      } else { // haven't checked this node against goal yet
        ...
        // If we're not within the known clear radius of this node OR out of Z
        // height range...
        if ((int)(*nci2).distance >= (radius * radius) ||
            (fabs(position2[2] - goal->r.currentOrigin[2]) >= MAX_Z_DELTA)) {
          // We're not *within* this node, so check clear path, etc.

          if (flags & NF_CLEAR_PATH) //|| flags & NF_CLEAR_LOS )
          {                          // need a clear path or LOS
            if (!SV_inPVS(goal->r.currentOrigin,
                          position2)) { // not even potentially clear
              SetCheckedNode(nodeNum2, goal->s.number, CHECKED_FAILED);
              continue;
            }
          }
          // Do we need a clear path?
          if (flags & NF_CLEAR_PATH) {
            if (TestNodePath(goal, ent->s.number, position2, qfalse) ==
                false) // qtrue?
            {
              SetCheckedNode(nodeNum2, goal->s.number, CHECKED_FAILED);
              continue;
  ...
  if (!d_altRoutes->integer) { // bestNode would not have been set by
                               // GetBestNodeAltRoute above, so get it here
    if (ent->waypoint != NODE_NONE &&
        goal->waypoint !=
            NODE_NONE) { // have 2 valid waypoints which means a valid path
      bestNode = GetBestNodeAltRoute(ent->waypoint, goal->waypoint, &bestCost,
                                     NODE_NONE);
    }
  }
  return bestNode;
...
int CNavigator::GetNearestNode(sharedEntity_t *ent, int lastID, int flags,
                               int targetID) {
...
#define MAX_Z_DELTA 18
  ...
  STL_ITERATE(nci, nodeChain) {
    ...
    if (CheckedNode((*nci).nodeID, ent->s.number) == CHECKED_FAILED) {
      ...
      // Do we need a clear path?
      if (flags & NF_CLEAR_PATH) {
        if (TestNodePath(ent, ENTITYNUM_NONE, position, qfalse) ==
            false) // qtrue?
        {
      ...
      if ( flags & NF_CLEAR_LOS )
      {
              ...
              {
                      nodeChecked[(*nci).nodeID][ent->s.number] =
      CHECKED_FAILED; continue;
              }
      }
      */
    ...
    if (targetID !=
        WAYPOINT_NONE) { // we want to find the one with the shortest route here
      dist = GetPathCost((*nci).nodeID, targetID);
      if (dist < bestDist) {
        bestDist = dist;
        bestNode = (*nci).nodeID;
      }
    } else { // first one we find is fine
      bestNode = (*nci).nodeID;
      break;
    }
  }
...
void CNavigator::ClearCheckedNodes(void) { CheckedNodes.clear(); }
...
byte CNavigator::CheckedNode(int wayPoint, int ent) {
  ...
  std::map<int, byte>::iterator f =
      CheckedNodes.find(wayPoint * MAX_GENTITIES + ent);
  ...
  if (ent->failedWaypointCheckTime && ent->failedWaypointCheckTime < svs.time) {
    ...
    for (j = 0; j < MAX_FAILED_NODES; j++) {
      if (ent->failedWaypoints[j] != 0) {
        failed++;
        //-1 because 0 is a valid node but also the default, so we add one when
        //we add one
        m_nodes[ent->failedWaypoints[j] - 1]->GetPosition(nodePos);
        if (!GVM_NAV_ClearPathToPoint(
                ent->s.number, ent->r.mins, ent->r.maxs, nodePos,
                (CONTENTS_SOLID | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP),
                ENTITYNUM_NONE)) { // no path clear of architecture, so clear
                                   // this since we can't check against entities
          ent->failedWaypoints[j] = 0;
          failed--;
        }
        // have clear architectural path, now check against ents only
        else if (GVM_NAV_ClearPathToPoint(
                     ent->s.number, ent->r.mins, ent->r.maxs, nodePos,
                     CONTENTS_BODY,
                     ENTITYNUM_NONE)) { // clear of ents, too, so all clear,
                                        // clear this one out
          ent->failedWaypoints[j] = 0;
          failed--;
        }
    ...
    if (!failed) {
      ...
      ent->failedWaypointCheckTime =
          svs.time + CHECK_FAILED_EDGE_INTERVAL + Q_irand(0, 1000);
    }
  }
}
...
void CNavigator::AddFailedNode(sharedEntity_t *ent, int nodeID) {
  ...
  for (j = 0; j < MAX_FAILED_NODES; j++) {
    if (ent->failedWaypoints[j] == 0) {
      ent->failedWaypoints[j] =
          nodeID + 1; //+1 because 0 is the default value and that's a valid
                      //node, so we take the +1 out when we check the node above
      if (!ent->failedWaypointCheckTime) {
        ent->failedWaypointCheckTime = svs.time + CHECK_FAILED_EDGE_INTITIAL;
      }
      return;
    }
    if (ent->failedWaypoints[j] ==
        nodeID + 1) { // already have this one marked as failed
      return;
    }
  }
...
qboolean
CNavigator::NodesAreNeighbors(int startID,
                              int endID) { // See if these 2 are neighbors
  if (startID == endID) {
    return qfalse;
  }
...
void CNavigator::AddFailedEdge(int entID, int startID, int endID) {
  ...
  for (j = 0; j < MAX_FAILED_EDGES; j++) {
    if (failedEdges[j].startID == WAYPOINT_NONE) {
      ...
      //			collision avoidance just wasn't at it's best but path is
      clear CNode *start = m_nodes[startID]; CNode *end = m_nodes[endID];
      ...
      for ( int i = 0; i < start->GetNumEdges(); i++ )
      {
              ...
              {//neighbor of or route to end
                      //There's an alternate route, so don't check this one for
      10 seconds failedEdges[j].checkTime = svs.time +
      CHECK_FAILED_EDGE_INTITIAL; break;
              }
      }
      */
  ...
  Com_Printf(S_COLOR_RED
             "NAV ERROR: too many blocked waypoint connections (%d)!!!\n",
             j);
#endif
}
...
qboolean CNavigator::CheckFailedEdge(failedEdge_t *failedEdge) {
  ...
  if (failedEdge->checkTime < svs.time) {
    if (failedEdge->startID != WAYPOINT_NONE) {
      ...
      int ignore, clipmask;
      sharedEntity_t *ent = SV_GentityNum(
          failedEdge
              ->entID); //(failedEdge->entID<ENTITYNUM_WORLD)?&g_entities[failedEdge->entID]:NULL;
      int hitEntNum;
      ...
      if (!ent || /*!ent->inuse || !ent->client || ent->health <= 0*/
        ...
        clipmask = MASK_SOLID; // ent->clipmask; //rwwFIXMEFIXME: share
                               // clipmask?
      }

      if (maxs[2] < mins[2]) { // don't invert bounding box
        maxs[2] = mins[2];
      }
      ...
      SV_Trace(&trace, start, mins, maxs, end, ignore,
               clipmask | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP, qfalse, 0,
               10); // NOTE: should we really always include monsterclip
                    // (physically blocks NPCs) and botclip (do not enter)?
      ...
      if (hitEntNum != ENTITYNUM_NONE && GVM_NAV_EntIsUnlockedDoor(hitEntNum)) {
        hitEntNum = ENTITYNUM_NONE;
                 ...
                 failedEdge->entID) { // don't hit the person who initially
                                      // marked the edge failed
        hitEntNum = ENTITYNUM_NONE;
      }
...
void CNavigator::CheckAllFailedEdges(void) {
  ...
  if (clearedAny) { // need to recalc the paths
    if (pathsCalculated) {
      // reconnect the nodes and mark every node's flag NF_RECALC
      // Com_Printf( S_COLOR_CYAN"%d marking all nodes for recalc\n", svs.time
      // );
...
qboolean CNavigator::RouteBlocked(int startID, int testEdgeID, int endID,
                                  int rejectRank) {
  ...
  if (testEdgeID == endID) { // Neighbors, checked out, all clear
    return qfalse;
  }
  ...
  while (1) {
    ...
    for (int i = 0; i < next->GetNumEdges(); i++) {
      ...
      if (edgeID == lastID) { // Don't backtrack
        continue;
      }

      if (edgeID == startID) { // Don't loop around
        continue;
      }
      ...
      if (edgeID == endID) { // We got there all clear!
        return qfalse;
      }
      ...
      if (testRank < 0) { // No route this way
        continue;
      }
...
int CNavigator::GetBestNodeAltRoute(int startID, int endID, int *pathCost,
                                    int rejectID) {
  // Must have nodes
  if (m_nodes.size() == 0)
  ...
  *pathCost = 0;
  ...
  if (rejectID != WAYPOINT_NONE) {
    for (int i = 0; i < start->GetNumEdges(); i++) {
      if (start->GetEdge(i) == rejectID) {
        rejectRank =
            GetPathCost(startID, endID); // end->GetRank( start->GetEdge(i) );
        break;
      }
    }
  }
  ...
  for (int i = 0; i < start->GetNumEdges(); i++) {
    ...
    if (edgeID == endID) {
      if (!d_altRoutes->integer ||
          !RouteBlocked(startID, edgeID, endID, rejectRank)) {
        *pathCost += start->GetEdgeCost(i);
        return edgeID;
      } else { // this is blocked, can't consider it
        continue;
      }
    }
    ...
    // Found a better one
    if (testRank < bestRank) {
      // FIXME: make sure all the edges down from startID through edgeID to
      // endID 		does NOT include a failedEdge...
      if (!d_altRoutes->integer ||
          !RouteBlocked(startID, edgeID, endID, rejectRank)) {
        bestNode = edgeID;
        bestRank = testRank;
...
unsigned int CNavigator::GetPathCost(int startID, int endID) {
  ...
  if (!startNode->GetNumEdges()) { // WTF?  Solitary waypoint!  Bad designer!
    return Q3_INFINITE;            // return 0;
  }
  ...
  while (moveNode != endNode) {
    ...
    if (dontScrewUp > 40000) { // ok, I think something probably screwed up.
      break;
    }
  }
...
CEdge *CPriorityQueue::Find(int npNum) {
  for (std::vector<CEdge *>::iterator HeapIter = mHeap.begin();
       HeapIter != mHeap.end(); ++HeapIter) {
...
void CPriorityQueue::Update(CEdge *edge) {
  for (std::vector<CEdge *>::iterator i = mHeap.begin(); i != mHeap.end();
       ++i) {
    if ((*i)->m_first ==
        edge->m_first) { // Found node - resort from this position in the mHeap
...
//////////////////////////////////////////////////////////////////
bool CPriorityQueue::Empty() { return (mHeap.empty()); };
