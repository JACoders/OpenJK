...
StaticZeroMem_t gZeroMalloc = {{ZONE_MAGIC, TAG_STATIC, 0, NULL, NULL},
                               {ZONE_MAGIC}};
...
#ifndef DEDICATED
      // ditch any image_t's (and associated GL memory) not used on this
      // level...
      //
...
void openjk_minizip_free(void *to_free) { Z_Free(to_free); }
...
void Z_Free(void *pvAddress) {
  if (pvAddress == NULL) // I've put this in as a safety measure because of some
                         // bits of #ifdef BSPC stuff	-Ste.
  {
...
void *S_Malloc(int iSize) { return Z_Malloc(iSize, TAG_SMALL); }
...
int Hunk_MemoryRemaining(void) {
  return (64 * 1024 * 1024) -
         (Z_MemSize(TAG_HUNK_MARK1) +
          Z_MemSize(TAG_HUNK_MARK2)); // Yeah. Whatever. We've got no size now.
}
...
===================
*/
void Hunk_SetMark(void) { hunk_tag = TAG_HUNK_MARK2; }
...
void Hunk_ClearToMark(void) {
  assert(hunk_tag ==
         TAG_HUNK_MARK2); // if this is not true then no mark has been made
  Z_TagFree(TAG_HUNK_MARK2);
}
...
// See if any ghoul2 stuff was leaked, at this point it should be all cleaned
// up.
#ifdef _FULL_G2_LEAK_CHECKING
  assert(g_Ghoul2Allocations == 0 && g_G2ClientAlloc == 0 &&
         g_G2ServerAlloc == 0);
...
=================
*/
void Hunk_ClearTempMemory(void) { Z_TagFree(TAG_TEMP_HUNKALLOC); }
