...
qboolean AI_ValidateNoEnemyGroupMember(AIGroupInfo_t *group, gentity_t *member) {
	...
	if (!gi.inPVS(member->currentOrigin, center)) { // not within PVS of the group enemy
		return qfalse;
	}
	return qtrue;
}
...
qboolean AI_ValidateGroupMember(AIGroupInfo_t *group, gentity_t *member) {
	...
	if (member->client->ps.weapon == WP_SABER || //!= self->s.weapon )
		member->client->ps.weapon == WP_THERMAL || member->client->ps.weapon == WP_DISRUPTOR || member->client->ps.weapon == WP_EMPLACED_GUN ||
		member->client->ps.weapon == WP_BOT_LASER ||									   // Probe droid	- Laser blast
		member->client->ps.weapon == WP_MELEE || member->client->ps.weapon == WP_TURRET || // turret guns
		member->client->ps.weapon == WP_ATST_MAIN || member->client->ps.weapon == WP_ATST_SIDE ||
	...
	if (member->enemy != group->enemy) {
		...
		if (!gi.inPVS(member->currentOrigin, group->enemy->currentOrigin)) { // not within PVS of the group enemy
			return qfalse;
		}
	...
	if (!noflee) {
		...
		for (int i = 0; i < group->numGroup; i++) {
			...
			if (member->NPC->rank < RANK_ENSIGN) {																			   // grunt
				if (group->enemy && DistanceSquared(member->currentOrigin, group->enemy->currentOrigin) < 65536 /*256*256*/) { // those close to enemy run away!
					ST_StartFlee(member, group->enemy, member->currentOrigin, AEL_DANGER_GREAT, 3000, 5000);
				} else if (DistanceSquared(member->currentOrigin, self->currentOrigin) < 65536 /*256*256*/) { // those close to me run away!
					ST_StartFlee(member, group->enemy, member->currentOrigin, AEL_DANGER_GREAT, 3000, 5000);
...
qboolean AI_RefreshGroup(AIGroupInfo_t *group) {
	...
	for (i = 0; i < group->numGroup; i++) {
		...
		); AI_DeleteGroupMember( group, i ); i--; continue;
		}
		*/
		member = &g_entities[group->member[i].number];
