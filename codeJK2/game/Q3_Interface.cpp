...
extern int ICARUS_LinkEntity(int entID, CSequencer *sequencer,
...
void Q3_TaskIDComplete(gentity_t *ent, taskID_t taskType) {
  ...
  if (ent->taskManager && Q3_TaskIDPending(ent, taskType)) { // Complete it
...
static char *Q3_GetAnimBoth(gentity_t *ent) {
  ...
  if (Q_stricmp(lowerName, upperName)) {
    ...
    Q3_DebugPrint(WL_WARNING, "Q3_GetAnimBoth: legs and torso animations did "
...
=============
*/
static unsigned int Q3_GetTime(void) { return level.time; }
...
//get the last slash before the wav if (start != NULL) { if (!strncmp( start,
...
slash before the wav if (start != NULL) { if (!strncmp( start, "/ms_", 4) ) {
...
static void Q3_Lerp2Start(int entID, int taskID, float duration) {
  ...
  ent->s.eType = ET_MOVER;
  ent->e_ReachedFunc =
      reachedF_moverCallback; // Callsback the the completion of the move
  if (ent->damage) {
...
static void Q3_Lerp2End(int entID, int taskID, float duration) {
  ...
  if (ent->moverState == MOVER_POS1) { // open the portal
    if (ent->svFlags & SVF_MOVER_ADJ_AREA_PORTALS) {
...
static void Q3_SetEnemy(int entID, const char *name) {
  ...
  if (!Q_stricmp("NONE", name) || !Q_stricmp("NULL", name)) {
    ...
    {
            //Q3_DebugPrint( WL_ERROR, "Q3_SetEnemy: ERROR - desired enemy has
    health %d\n", enemy->health ); return;
    }*/
    else {
      if (ent->NPC) {
...
static void Q3_SetDefendTarget( int entID, const char *name )
{
        ...
        if ( !ent )
        {
                Q3_DebugPrint( WL_WARNING, "Q3_SetDefendTarget: invalid entID
%d\n", entID); return;
        }

        if ( !ent->NPC )
        {
                Q3_DebugPrint( WL_WARNING, "Q3_SetDefendTarget: ent %d is NOT an
NPC!\n", entID); return;
        }
        ...
        {
                Q3_DebugPrint( WL_WARNING, "Q3_SetDefendTarget: %s NOT
found!\n", name);
        }
}
*/
...
static void Q3_SetLeader(int entID, const char *name) {
  ...
  if (!Q_stricmp("NONE", name) || !Q_stricmp("NULL", name)) {
    ...
    if (leader == NULL) {
      ...
      // Q3_DebugPrint( WL_ERROR,"Q3_SetEnemy: ERROR - desired enemy has health
      // %d\n", enemy->health );
      return;
    } else {
...
static void Q3_SetSquadName (int entID, const char *squadname)
{
        ...
        if ( !self )
        {
                Q3_DebugPrint( WL_WARNING, "Q3_SetSquadName: invalid entID
%d\n", entID); return;
        }

        if ( !self->client )
        {
                Q3_DebugPrint( WL_ERROR, "Q3_SetSquadName: '%s' is not an
...
void Q3_SetParm(int entID, int parmNum, const char *parmValue) {
  ...
  if ((val = Q3_CheckStringCounterIncrement(parmValue))) {
    ...
    // set the last charcter to null in case we had to truncate their passed
    // string
    if (ent->parms->parm[parmNum][sizeof(ent->parms->parm[0]) - 1] !=
...
static void Q3_AddRHandModel(int entID, char *addModel) {
  ...
  ent->cinematicModel =
      gi.G2API_InitGhoul2Model(ent->ghoul2, addModel, G_ModelIndex(addModel),
...
static void Q3_Face(int entID, int expression, float holdtime) {
  ...
  case SET_FACEAUX:
    ...
    ent->client->facial_blink =
        level.time + holdtime + Q_flrand(3000.0, 5000.0);
    ent->client->facial_frown =
        level.time + holdtime + Q_flrand(6000.0, 10000.0);
    break;
  case SET_FACEBLINK:
...
static void Q3_SetAnimFrame(int entID, int animFrame) {
  ...
  if (animFrame >= ent->endFrame) {
    ...
    Q3_DebugPrint(WL_WARNING, "Q3_SetAnimFrame: value must be valid number "
...
static void Q3_SetSaberActive(int entID, qboolean active) {
  ...
  // FIXME: need an externalized call for the sound and any other internal
  // changes?
...
static void Q3_Set(int taskID, int entID, const char *type_name,
  ...
  case SET_ORIGIN:
    ...
    if (strncmp("NPC_", ent->classname, 4) == 0) { // hack for moving spawners
...
static void Q3_Kill(int entID, const char *name) {
  ...
  if ( victim == ent )
  {
          Q3_DebugPrint( WL_ERROR, "Q3_Kill: entity %s trying to kill self - not
  allowed!\n", name); return;
  }
  */
  if (victim ==
...
static int Q3_GetFloat(int entID, int type, const char *name, float *value) {
  ...
  case SET_FORWARDMOVE: //## %d="0" # NPC move forward -127(back) to 127
    if (ent->client == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_FORWARDMOVE, %s not a client\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_SHOT_SPACING: //## %d="1000" # Time between shots for an NPC - reset
    ...
    if (ent->NPC == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_SHOT_SPACING, %s not an NPC\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_IGNOREALERTS: //## Do not get enemy set by allies in area(ambush)
    if (ent->NPC == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_IGNOREALERTS, %s not an NPC\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_CHASE_ENEMIES: //## %t="BOOL_TYPES" # NPC will chase after enemies
    if (ent->NPC == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_CHASE_ENEMIES, %s not an NPC\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_LOOK_FOR_ENEMIES: //## %t="BOOL_TYPES" # NPC will be on the lookout
    ...
    if (ent->NPC == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_LOOK_FOR_ENEMIES, %s not an NPC\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_FACE_MOVE_DIR: //## %t="BOOL_TYPES" # NPC will face in the direction
    ...
    if (ent->NPC == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_FACE_MOVE_DIR, %s not an NPC\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_FORCED_MARCH: //## %t="BOOL_TYPES" # Force NPC to move at runSpeed
    if (ent->NPC == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_FORCED_MARCH, %s not an NPC\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_GREET_ALLIES: //## %t="BOOL_TYPES" # Makes an NPC greet teammates
    if (ent->NPC == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_GREET_ALLIES, %s not an NPC\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_NO_COMBAT_TALK: //## %t="BOOL_TYPES" # NPCs will not do their combat
    ...
    if (ent->NPC == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_NO_COMBAT_TALK, %s not an NPC\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_NO_ALERT_TALK: //## %t="BOOL_TYPES" # NPCs will not do their combat
    ...
    if (ent->NPC == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_NO_ALERT_TALK, %s not an NPC\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_USE_CP_NEAREST: //## %t="BOOL_TYPES" # NPCs will use their closest
    ...
    if (ent->NPC == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_USE_CP_NEAREST, %s not an NPC\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_DISMEMBERABLE: //## %t="BOOL_TYPES" # NPC will not be affected by
    ...
    if (ent->client == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_DISMEMBERABLE, %s not a client\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_NO_ACROBATICS:
    if (ent->NPC == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_NO_ACROBATICS, %s not an NPC\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_USE_SUBTITLES:
    if (ent->NPC == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_USE_SUBTITLES, %s not an NPC\n",
                    ent->targetname);
      return false;
    }
  ...
  case SET_NO_FALLTODEATH: //## %t="BOOL_TYPES" # NPC will not be affected by
    ...
    if (ent->NPC == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetFloat: SET_NO_FALLTODEATH, %s not an NPC\n",
                    ent->targetname);
      return false;
    }
...
static int Q3_GetString(int entID, int type, const char *name, char **value) {
  ...
  case SET_LOOK_TARGET: //## %s="NULL" # object for NPC to look at
    if (ent->client == NULL) {
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetString: SET_LOOK_TARGET, %s not a client\n",
                    ent->targetname);
      return false;
    } else {
  ...
  case SET_WATCHTARGET: //## %s="NULL" # Set angles toward ent by targetname
    if (ent && ent->NPC && ent->NPC->watchTarget) {
      *value = ent->NPC->watchTarget->targetname;
      ...
      Q3_DebugPrint(WL_WARNING,
                    "Q3_GetString: SET_WATCHTARGET no watchTarget!\n");
      return false;
    }
    break;
  case SET_VIEWENTITY:
