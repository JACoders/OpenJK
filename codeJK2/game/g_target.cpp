...
void target_laser_think(gentity_t *self) {
  ...
  gi.trace(&tr, self->s.origin, NULL, NULL, end, self->s.number,
           CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_CORPSE, G2_NOCOLLIDE, 0);
  ...
  */
  ...
  if (self->behaviorSet[BSET_USE]) {
    if (self->spawnflags & 1) {
      ...
      if (!self->activator->sequencer ||
          !self->activator
               ->taskManager) { // Need to be initialized through ICARUS
        ...
        if (ICARUS_ValidEnt(self->activator)) {
          ...
          Q3_DebugPrint(WL_ERROR, "target_scriptrunner tried to run on invalid "
                                  "ICARUS activator!\n");
          return;
        }
      }

      Q3_DebugPrint(WL_VERBOSE,
                    "target_scriptrunner running %s on activator %s\n",
                    self->behaviorSet[BSET_USE], self->activator->targetname);
      ICARUS_RunScript(self->activator,
...
void target_autosave_use(gentity_t *self, gentity_t *other,
  ...
  CG_CenterPrint("@INGAME_CHECKPOINT", SCREEN_HEIGHT * 0.25); // jump the
                                                              // network
