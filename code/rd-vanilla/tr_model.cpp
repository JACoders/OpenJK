...
//
static int GetModelDataAllocSize(void) { return R_MemSize(TAG_MODEL_MD3) + R_MemSize(TAG_MODEL_GLM) + R_MemSize(TAG_MODEL_GLA); }
...
static qhandle_t RE_RegisterModel_Actual(const char *name) {
	...
	*/
	...
	if (name[0] == '#') {
		...
		tr.numBSPModels++;
#ifndef DEDICATED
		RE_LoadWorldMap_Actual(va("maps/%s.bsp", name + 1), tr.bspModels[tr.numBSPModels - 1],
							   tr.numBSPModels); // this calls R_LoadSubmodels which will put them into the Hash
#endif
...
qhandle_t RE_RegisterModel(const char *name) {
	...
	if (!COM_CompareExtension(name, ".gla")) {
		gbInsideRegisterModel = qfalse; // GLA files recursively call this, so don't turn off half way. A reference count would be nice, but if any ERR_DROP
										// ever occurs within the load then the refcount will be knackered from then on
	}
