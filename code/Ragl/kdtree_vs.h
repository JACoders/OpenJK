...
public:
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  bool empty() const { return (mRoot == NULL_NODE); }

  ////////////////////////////////////////////////////////////////////////////////////
  // Is This List Filled?
  ////////////////////////////////////////////////////////////////////////////////////
  bool full() const { return (mPool.full()); }
  ...
  void add(const T &data) {
    ...
    // Hey!  It Didn't Mark Any Targets, Which Means We Found An Exact match To
    // This Data
    //------------------------------------------------------------------------------------
    else {
      mPool.free(nNew);
  ...
  bool find(const T &data) {
    assert(mRoot != NULL_NODE); // If You Hit This Assert, You Are Asking For
                                // Data On An Empty Tree
...
private:
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  int find_index(const T &data, int curNode, int curDimension,
                 bool returnClosest, bool markTarget) {
    // Did We Just Go Off The End Of The Tree Or Find The Data We Were Looking
    // For?
    //------------------------------------------------------------------------------
    ...
    // Search Recursivly Down The Tree Either Left (For Data > Current Node), Or
    // Right
    //---------------------------------------------------------------------------------
    int findRecursive;
    bool goLeft = (data[curDimension] < mPool[curNode].mData[curDimension]);
    if (goLeft) {
      findRecursive = find_index(data, mPool[curNode].mLeft, nextDimension,
                                 returnClosest, markTarget);
    } else {
      findRecursive = find_index(data, mPool[curNode].mRight, nextDimension,
                                 returnClosest, markTarget);
    }
    ...
    // If We Want To Return The CLOSEST Node, And We Went Off The End, Then
    // Return This One
    //--------------------------------------------------------------------------------------
    if (returnClosest) {
      // If We Are Asked To Mark The Target, We Mark (TARG_NODE) At Either mLeft
      // or mRight, Depending On Where The Node Should Have Been
      //----------------------------------------------------------------------------------
      if (markTarget) {
        if (goLeft) {
      ...
      // Go Ahead And Return This Node, It's The One We Would Have Put As The
      // Child
      return curNode;
    }
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  // This function just sets up the range bounds and starts the recursive tree
  // search
  ////////////////////////////////////////////////////////////////////////////////////
  ...
  //
  ////////////////////////////////////////////////////////////////////////////////////
  void tree_search(range_query &query, int curNode, int curDimension,
                   range_bounds bounds) {
    assert(curNode < SIZE);
    ...
    if (curNode != NULL_NODE &&
        tree_search_node_in_range(query, mPool[curNode])) {
      query.mReported.push_back(mPool[curNode].mData);
    }

    // If This Is A Leaf Node, We're Done Here
    //-----------------------------------------
    if (curNode == NULL_NODE || (mPool[curNode].mLeft == NULL_NODE &&
                                 mPool[curNode].mRight == NULL_NODE)) {
      return;
    }
    ...
    // If The Bounds Are Contained Entirely Within The Query Range, We Report
    // The Sub Tree
    //-------------------------------------------------------------------------------------
    if (Side == Side_AllIn) {
    ...
    // Otherwise, If Our Bounds Intersect The Query Range, We Need To Look
    // Further
    //-----------------------------------------------------------------------------
      ...
      if (mPool[curNode].mLeft != NULL_NODE) {
        ...
        if (!bounds.mMins[curDimension] ||
            ((mPool[curNode].mData[curDimension]) <
             (mPool[bounds.mMins[curDimension]].mData[curDimension]))) {
          bounds.mMins[curDimension] = curNode;
        }
        tree_search(query, mPool[curNode].mLeft, nextDimension, bounds);
        bounds.mMaxs[curDimension] =
            OldMaxs; // Restore Old Maxs For The Right Child Search
      }

      // Test The Right Child
      //----------------------
      if (mPool[curNode].mRight != NULL_NODE) {
        if (!bounds.mMaxs[curDimension] ||
            ((mPool[bounds.mMaxs[curDimension]].mData[curDimension]) <
             (mPool[curNode].mData[curDimension]))) {
  ...
  ESide tree_search_bounds_in_range(range_query &query, range_bounds &bounds) {
    ESide S = Side_AllIn;
    for (int dim = 0; dim < DIMENSION; dim++) {
      // If Any Of Our Dimensions Are Undefined Right Now, Always Return
      // INTERSECT
      //---------------------------------------------------------------------------
