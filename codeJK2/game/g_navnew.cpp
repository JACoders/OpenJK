...
-------------------------
*/
void NAVNEW_PushBlocker(gentity_t *self, gentity_t *blocker, vec3_t right, qboolean setBlockedInfo) { // try pushing blocker to one side
...
-------------------------
*/
qboolean NAVNEW_DanceWithBlocker(gentity_t *self, gentity_t *blocker, vec3_t movedir, vec3_t right) { // sees if blocker has any lateral movement
...
-------------------------
*/
qboolean NAVNEW_SidestepBlocker(gentity_t *self, gentity_t *blocker, vec3_t blocked_dir, float blocked_dist, vec3_t movedir,
		...
		if (foundClearPath) { // clear!
			...
			if (inGoalWP) {												 // we headed for our goal and failed and our goal's WP and failed
				...
				if (d_altRoutes->integer) {
					...
					// if ( (bestNode = navigator.GetBestPathBetweenEnts( self, self->NPC->goalEntity, NF_CLEAR_PATH )) == NODE_NONE )//!NAVNEW_GetWaypoints(
					// self, qfalse ) )
