...
typedef struct {
  ...
  int iG2MarkShaderHandle;  // index of shader to use for G2 marks made on other
                           // models when hit by this projectile
  ...
  float fHoming; // 0.0 = not homing, 0.5 = half vel to targ, half cur vel, 1.0
                 // = all vel to targ
  float fHomingFOV;
  ...
  int iSplashDamage; // damage done to ents in splashRadius of end of traceline
                     // or projectile origin on impact
  float fSplashRadius; // radius that ent must be in to take splashDamage
                       // (linear fall-off)
  int iAmmoPerShot; // how much "ammo" each shot takes
  int iHealth; // if non-zero, projectile can be shot, takes this much damage
               // before being destroyed
  float
      fWidth; // width of traceline or bounding box of projecile (non-rotating!)
  float fHeight; // height of traceline or bounding box of projecile
                 // (non-rotating!)
  int iLifeTime;             // removes itself after this amount of time
  qboolean bExplodeOnExpire; // when iLifeTime is up, explodes rather than
                             // simply removing itself
} vehWeaponInfo_t;
...
typedef struct {
  ...
  int pitchAxis; // axis on pitchBone to which we should to apply the pitch
                 // angles
  ...
  int iMuzzle[MAX_VEHICLE_TURRET_MUZZLES]; // iMuzzle-1 = index of ship's muzzle
                                           // to fire this turret's 1st and 2nd
                                           // shots from
  char *gunnerViewTag; // Where to put the view origin of the gunner (name)
  float fTurnSpeed;    // how quickly the turret can turn
  qboolean bAI; // whether or not the turret auto-targets enemies when it's not
                // manned
  qboolean bAILead; // whether
  float fAIRange;   // how far away the AI will look for enemies
  int passengerNum; // which passenger, if any, has control of this turret
                    // (overrides AI)
} turretStats_t;
...
typedef struct {
  ...
  int delay;
  // Whether or not all the muzzles for each weapon can be linked together
  // (linked delay = weapon delay * number of muzzles linked!)
  int linkable;
  // Whether or not to auto-aim the projectiles/tracelines at the thing under
  // the crosshair when we fire
  qboolean aimCorrect;
  // maximum ammo
  int ammoMax;
  // ammo recharge rate - milliseconds per unit (minimum of 100, which is 10
  // ammo per second)
  int ammoRechargeMS;
  // sound to play when out of ammo (plays default "no ammo" sound if none
  // specified)
  int soundNoAmmo;
} vehWeaponStats_t;
...
typedef struct {
  ...
  int numHands; // if 2 hands, no weapons, if 1 hand, can use 1-handed weapons,
                // if 0 hands, can use 2-handed weapons
  float lookPitch; // How far you can look up and down off the forward of the
                   // vehicle
  float lookYaw; // How far you can look left and right off the forward of the
                 // vehicle
  float length; // how long it is - used for body length traces when
                // turning/moving?
  float width; // how wide it is - used for body length traces when
               // turning/moving?
  float height; // how tall it is - used for body length traces when
                // turning/moving?
  vec3_t centerOfGravity; // offset from origin: {forward, right, up} as a
                          // modifier on that dimension (-1.0f is all the way
                          // back, 1.0f is all the way forward)
  ...
  float throttleSticks; // if true, speed stays at whatever you accel/decel to,
                        // unless you turbo or brake
  float strafePerc; // multiplier on current speed for strafing.  If 1.0f, you
                    // can strafe at the same speed as you're going forward, 0.5
                    // is half, 0 is no strafing

  // handling stats
  float bankingSpeed; // how quickly it pitches and rolls (not under player
                      // control)
  ...
  float toughness; // modifies incoming damage, 1.0 is normal, 0.5 is half, etc.
                   // Simulates being made of tougher materials/construction
  int malfunctionArmorLevel; // when armor drops to or below this point, start
                             // malfunctioning
  ...
  // visuals & sounds
  char *model; // what model to use - if make it an NPC's primary model, don't
               // need this?
  char *skin; // what skin to use - if make it an NPC's primary model, don't
              // need this?
  ...
  int iTurboFX;   // turbo exhaust effect, played from "*exhaust" bolt(s) when
                // ship is in "turbo" mode
  int iTurboStartFX; // turbo begin effect, played from "*exhaust" bolts when
                     // "turbo" mode begins
  int iTrailFX;   // trail effect, played from "*trail" bolt(s)
  int iImpactFX;  // impact effect, for when it bumps into something
  int iExplodeFX; // explosion effect, for when it blows up (should have the
                  // sound built into explosion effect)
  ...
  // Which weapon a muzzle fires (has to match one of the weapons this vehicle
  // has). So 1 would be weapon 1, 2 would be weapon 2 and so on.
  ...
  float hoverStrength; // how hard it pushes off ground when less than hover
                       // height... causes "bounce", like shocks
  ...
  int maxPassengers;  // The max number of passengers this vehicle may have
                      // (Default = 0).
  qboolean hideRider; // rider (and passengers?) should not be drawn
  qboolean
      killRiderOnDeath; // if rider is on vehicle when it dies, they should die
  qboolean flammable; // whether or not the vehicle should catch on fire before
                      // it explodes
  int explosionDelay; // how long the vehicle should be on fire/dying before it
                      // explodes
  // camera stuff
  qboolean cameraOverride; // whether or not to use all of the following 3rd
                           // person camera override values
  float cameraRange;      // how far back the camera should be - normal is 80
  float cameraVertOffset; // how high over the vehicle origin the camera should
                          // be - normal is 16
  float cameraHorzOffset; // how far to left/right (negative/positive) of of the
                          // vehicle origin the camera should be - normal is 0
  float cameraPitchOffset; // a modifier on the camera's pitch (up/down angle)
                           // to the vehicle - normal is 0
  float cameraFOV;   // third person camera FOV, default is 80
  float cameraAlpha; // fade out the vehicle to this alpha (0.1-1.0f) if it's in
                     // the way of the crosshair
  qboolean cameraPitchDependantVertOffset; // use the hacky AT-ST pitch
                                           // dependant vertical offset

  // NOTE: some info on what vehicle weapon to use?  Like ATST or TIE bomber or
  ...
  int modelIndex; // set internally, not until this vehicle is spawned into the
                  // level

  // NOTE: Please note that most of this stuff has been converted from C++
  // classes to generic C. This part of the structure is used to simulate
  // inheritance for vehicles. The basic idea is that all vehicle use this
  // vehicle interface since they declare their own functions and assign the
  // function pointer to the corresponding function. Meanwhile, the base logic
  // can still call the appropriate functions. In C++ talk all of these
  // functions (pointers) are pure virtuals and this is an abstract base class
  // (although it cannot be inherited from, only contained and reimplemented
  // (through an object and a setup function respectively)). -AReis
  ...
  // Update the properties of a Rider (that may reflect what happens to the
  // vehicle).
  //
  //	[return]		bool			True if still in vehicle, false if
  //otherwise.
  ...
  // Attachs all the riders of this vehicle to their appropriate position/tag
  // (*driver, *pass1, *pass2, whatever...).
...
// Load the function pointers for a vehicle into this shared vehicle info
// structure.
...
#define MIN_LANDING_SPEED                                                      \
  200 // equal to or less than this and close to ground = auto-slow-down to land
...
struct Muzzle {
  // These are updated every frame and represent the current position and
  // direction for the specific muzzle.
  vec3_t m_vMuzzlePos;
  vec3_t m_vMuzzleDir;

  // This is how long to wait before being able to fire a specific muzzle again.
  // This is based on the firing rate so that a firing rate of 10 rounds/sec
  // would make this value initially 100 miliseconds.
  int m_iMuzzleWait;
...
// This is the implementation of the vehicle interface and any of the other
// variables needed. This is what actually represents a vehicle. -AReis.
// !!!!!!!!!!!!!!!!!! loadsave affecting structure !!!!!!!!!!!!!!!!!!!!!!!
struct Vehicle_t {
  // The entity who pilots/drives this vehicle.
  // NOTE: This is redundant (since m_pParentEntity->owner _should_ be the
  // pilot). This makes things clearer though.
  gentity_t *m_pPilot;

  int m_iPilotTime; // if spawnflag to die without pilot and this < level.time
                    // then die.
  ...
  // If not zero, how long to wait before we can do anything with the vehicle
  // (we're getting on still). -1 = board from left, -2 = board from right, -3 =
  // jump/quick board.  -4 & -5 = throw off existing pilot
  int m_iBoarding;
  ...
  // NOTE: Since each vehicle has their own model instance, these bolts must be
  // local to each vehicle as well.
  int m_iLeftWingBone;
  ...
  int m_iDroidUnitTag;
  int m_iGunnerViewTag[MAX_VEHICLE_TURRETS]; // Where to put the view origin of
                                             // the gunner (index)
  ...
  // NOTE: Vehicle Type ID, Orientation, and Armor MUST be transmitted over the
  // net.

  // Current angles of this vehicle.
  vec3_t m_vOrientation;

  // How long you have strafed left or right (increments every frame that you
  // strafe to right, decrements every frame you strafe left)
  int m_fStrafeTime;

  // Previous angles of this vehicle.
  vec3_t m_vPrevOrientation;

  // When control is lost on a speeder, current angular velocity is stored here
  // and applied until landing
  ...
  // This pointer is to a valid VehicleInfo (which could be an animal, speeder,
  // fighter, whatever). This contains the functions actually used to do things
  // to this specific kind of vehicle as well as shared information (max speed,
  // type, etc...).
