...
#define FX_SET_SHADER_TIME                                                     \
  0x00400000 // by having the effects system set the shader time, we can make
             // animating textures start at the correct time
...
public:
  ...
  virtual bool Update() { // Game pausing can cause dumb time things to happen,
                          // so kill the effect in this instance
...
protected:
  ...
  void Draw() {
    theFxHelper.AddLightToScene(
        mOrigin1, mRefEnt.radius, mRefEnt.lightingOrigin[0],
        mRefEnt.lightingOrigin[1], mRefEnt.lightingOrigin[2]);
  }
...
public:
  ...
  inline void DrawSegment(vec3_t start, vec3_t end, float texcoord1,
                          float texcoord2);
...
public:
  ...
  inline void SetNormalOffset(const vec3_t norm) {
    VectorCopy(norm, mNormalOffset);
  }
};
...
public:
  ...
  inline void SetNormal(const vec3_t norm) {
    VectorCopy(norm, mRefEnt.axis[0]);
  }
};
