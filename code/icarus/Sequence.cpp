...
void CSequence::Delete(CIcarus *icarus) {
  ...
  if (m_children.size() > 0) {
    /*for ( iterSeq = m_childrenMap.begin(); iterSeq != m_childrenMap.end();
    iterSeq++ )
    {
...
void CSequence::RemoveFlag(int flag, bool children) {
  ...
  if (children) {
    /*		sequenceID_m::iterator iterSeq = NULL;
                    for ( iterSeq = m_childrenMap.begin(); iterSeq !=
       m_childrenMap.end(); iterSeq++ )
                    {
...
CSequence *CSequence::GetChildByID(int id) {
  ...
  // NOTENOTE: Done for safety reasons, I don't know what this template will
  // return on underflow ( sigh... )
