...
static void CMod_LoadShaders(const lump_t *l, clipMap_t &cm) {
  ...
  cm.shaders =
      (CCMShader *)Hunk_Alloc((1 + count) * sizeof(*cm.shaders), h_high);
  cm.numShaders = count;
...
static void CMod_LoadSubmodels(const lump_t *l, clipMap_t &cm) {
  ...
  cm.cmodels =
      (struct cmodel_s *)Hunk_Alloc(count * sizeof(*cm.cmodels), h_high);
  ...
  for (i = 0; i < count; i++, in++, out++) {
    ...
    // Are they getting leaf data elsewhere? (the reason this needs to be done
    // is in sub bsp instances the first brush model isn't necessary a world
    // model and might be real architecture)
    if (i == 0 && &cm == &cmg) {
      out->firstNode = 0;
...
void CMod_LoadBrushes(const lump_t *l, clipMap_t &cm) {
  ...
  cm.brushes = (cbrush_t *)Hunk_Alloc(
      (BOX_BRUSHES + count) * sizeof(*cm.brushes), h_high);
  cm.numBrushes = count;
...
static void CMod_LoadLeafs(const lump_t *l, clipMap_t &cm) {
  ...
  cm.leafs =
      (cLeaf_t *)Hunk_Alloc((BOX_LEAFS + count) * sizeof(*cm.leafs), h_high);
  cm.numLeafs = count;
  ...
  cm.areaPortals = (int *)Hunk_Alloc(
      cm.numAreas * cm.numAreas * sizeof(*cm.areaPortals), h_high);
}
...
static void CMod_LoadPlanes(const lump_t *l, clipMap_t &cm) {
  ...
  cm.planes = (struct cplane_s *)Hunk_Alloc(
      (BOX_PLANES + count) * sizeof(*cm.planes), h_high);
  cm.numPlanes = count;
...
static void CMod_LoadBrushSides(const lump_t *l, clipMap_t &cm) {
  ...
  cm.brushsides = (cbrushside_t *)Hunk_Alloc(
      (BOX_SIDES + count) * sizeof(*cm.brushsides), h_high);
...
=================
*/
static void CMod_LoadEntityString(const lump_t *l, clipMap_t &cm,
                                  const char *name) {
  fileHandle_t h;
...
static void CMod_LoadPatches(const lump_t *surfs, const lump_t *verts,
  ...
  cm.surfaces =
      (cPatch_t **)Hunk_Alloc(cm.numSurfaces * sizeof(cm.surfaces[0]), h_high);
...
static void
CM_LoadMap_Actual(const char *name, qboolean clientload, int *checksum,
                  clipMap_t &cm) { // rwwRMG - function needs heavy modification
  int *buf;
  dheader_t header;
  ...
  // load the file
  //
  // rww - Doesn't this sort of defeat the purpose? We're clearing it even if
  // the map is the same as the last one! Not touching it though in case I'm just
  // overlooking something.
  if (gpvCachedMapDiskImage &&
      &cm == &cmg) // MP code: this'll only be NZ if we got an ERR_DROP during
                   // last map load,
...
#ifndef BSPC
  //
  // load the file into a buffer that we either discard as usual at the bottom,
  // or if we've got enough memory
  //	then keep it long enough to save the renderer re-loading it (if not
  //dedicated server), 	then discard it after that...
  //
  buf = NULL;
  fileHandle_t h;
  ...
  //
  // if we've got enough memory, and it's not a dedicated-server, then keep the
  // loaded map binary around
  //	for the renderer to chew on... (but not if this gets ported to a
  //big-endian machine, because some of the 	map data will have been
  //Little-Long'd, but some hasn't).
  //
  if (Sys_LowPhysicalMemory() || com_dedicated->integer
      //		|| we're on a big-endian machine
  ) {
    Z_Free(gpvCachedMapDiskImage);
    gpvCachedMapDiskImage = NULL;
  } else {
    // ... do nothing, and let the renderer free it after it's finished playing
    // with it...
    //
  }
#else
...
cmodel_t *CM_ClipHandleToModel(clipHandle_t handle, clipMap_t **clipMap) {
  ...
  if (handle < MAX_SUBMODELS) {
    Com_Error(ERR_DROP,
              "CM_ClipHandleToModel: bad handle (count: %i) < (handle: %i) < "
              "(max: %i)",
...
int CM_ModelContents_Actual(clipHandle_t model, clipMap_t *cm) {
  ...
  for (i = 0; i < cmod->leaf.numLeafSurfaces; i++) {
    ...
    if (cm->surfaces[surfaceNum] !=
        NULL) { // HERNH?  How could we have a null surf within our
