...
//---------------------------------------------------------
{
	...
	if (!(ent->client->ps.forcePowersActive & (1 << FP_SEE)) || ent->client->ps.forcePowerLevel[FP_SEE] < FORCE_LEVEL_2) { // force sight 2+ gives perfect aim
		...
		if (ent->NPC && ent->NPC->currentAim < 5) {
			...
			if (ent->client->NPC_class == CLASS_IMPWORKER) { //*sigh*, hack to make impworkers less accurate without affecteing imperial officer accuracy
