...
qboolean NPC_ClearLOS(const vec3_t start, const vec3_t end) { return G_ClearLOS(NPCS.NPC, start, end); }
...
qboolean NPC_ClearLOS3(const vec3_t start, gentity_t *ent) { return G_ClearLOS3(NPCS.NPC, start, ent); }
qboolean NPC_ClearLOS2(gentity_t *ent, const vec3_t end) { return G_ClearLOS2(NPCS.NPC, ent, end); }
		...
		|| (entTeam == NPCTEAM_FREE && ent->client->enemyTeam == NPCTEAM_FREE && ent->enemy && ent->enemy->client &&
			(ent->enemy->client->playerTeam == NPCS.NPC->client->playerTeam ||
			 (ent->enemy->client->playerTeam != NPCTEAM_ENEMY &&
			  NPCS.NPC->client->playerTeam == NPCTEAM_PLAYER))) // enemy is a rampaging non-aligned creature who is attacking someone on our team or a non-enemy
...
qboolean NPC_FacePosition(vec3_t position, qboolean doPitch) {
	...
	// Get the positions
	if (NPCS.NPC->client &&
		(NPCS.NPC->client->NPC_class == CLASS_RANCOR || NPCS.NPC->client->NPC_class == CLASS_WAMPA)) // || NPC->client->NPC_class == CLASS_SAND_CREATURE) )
	{
...
void NPC_CheckCharmed(void) {
	...
	if (NPCS.NPCInfo->charmedTime && NPCS.NPCInfo->charmedTime < level.time && NPCS.NPC->client) { // we were charmed, set us back!
...
void G_GetBoltPosition(gentity_t *self, int boltIndex, vec3_t pos, int modelIndex) {
	...
	if (self->client) { // clients don't actually even keep r.currentAngles maintained
...
float NPC_EnemyRangeFromBolt(int boltIndex) { return (NPC_EntRangeFromBolt(NPCS.NPC->enemy, boltIndex)); }
