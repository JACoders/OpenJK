...
gentity_t	*G_TestEntityPosition( gentity_t *ent ) {
	...
	if ( ent->client ) {
		...
		gi.Trace( &tr, ent->client->ps.origin, ent->r.mins, vMax, ent->client->ps.origin, ent->s.number, mask, qfalse, 0, 0 );
	} else {
		gi.Trace( &tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, ent->s.pos.trBase, ent->s.number, mask, qfalse, 0, 0 );
	}
...
qboolean	G_TryPushingEntity( gentity_t *check, gentity_t *pusher, vec3_t move, vec3_t amove ) {
	...
	*/
	...
	if (pushed_p > &pushed[MAX_GENTITIES]) {
		gi.Error( ERR_DROP, "pushed_p > &pushed[MAX_GENTITIES]" );
	}
	pushed_p->ent = check;
	...
	if (!block) {
		...
		gi.LinkEntity ((sharedEntity_t *)check);
		return qtrue;
	}
...
qboolean G_MoverPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
	...
	*obstacle = NULL;
	...
	gi.UnlinkEntity( (sharedEntity_t *)pusher );
	...
	gi.LinkEntity( (sharedEntity_t *)pusher );
	...
	for ( e = 0 ; e < listedEntities ; e++ ) {
		...
		*obstacle = check;
		...
		for ( p=pushed_p-1 ; p>=pushed ; p-- ) {
			...
			gi.LinkEntity ((sharedEntity_t *)p->ent);
		}
		return qfalse;
	}
...
void G_MoverTeam( gentity_t *ent ) {
	...
	if (part) {
		...
		for ( part = ent ; part ; part = part->teamchain ) {
			...
			gi.LinkEntity( (sharedEntity_t *)part );
		}
...
void Reached_BinaryMover( gentity_t *ent ) 
{
	...
	{
		gi.Error( ERR_DROP, "Reached_BinaryMover: bad moverState" );
	}
}
...
static void Touch_DoorTriggerSpectator( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	...
	gi.Trace(&tr, origin, pMins, pMaxs, origin, other->s.number, other->clipmask, qfalse, 0, 0);
	if (!tr.startsolid &&
		!tr.allsolid &&
		tr.fraction == 1.0f &&
...
void Think_SpawnNewDoorTrigger( gentity_t *ent ) 
{
	...
	gi.LinkEntity ((sharedEntity_t *)other);
...
void SpawnPlatTrigger( gentity_t *ent ) {
	...
	gi.LinkEntity ((sharedEntity_t *)trigger);
}
...
void SP_path_corner( gentity_t *self ) {
	if ( !self->targetname ) {
		gi.Print ("path_corner with no targetname at %s\n", vtos(self->s.origin));
		G_FreeEntity( self );
		return;
	}
...
void SP_func_train (gentity_t *self) {
	...
	if ( !self->target ) {
		gi.Print ("func_train without a target at %s\n", vtos(self->r.absmin));
		G_FreeEntity( self );
		return;
	}

	gi.SetBrushModel( (sharedEntity_t *)self, self->model );
	InitMover( self );
...
void funcBBrushDieGo (gentity_t *self)
{
	...
	gi.AdjustAreaPortalState( (sharedEntity_t *)self, qtrue );
...
void SP_func_breakable( gentity_t *self ) 
{
	...
	if ( self->team && self->team[0] )
	{
		...
		{
			gi.Error( ERR_DROP, "team name %s not recognized\n", self->team );
		}
	}
	*/
	...
	self->team = NULL;
	if (!self->model) {
		gi.Error( ERR_DROP, "func_breakable with NULL model\n" );
	}
	InitBBrush( self );
...
{
	//gi.Print("Mr. Glass says: PLZ NO IT HURTS\n");
	//Make "cracking" sound?
}
