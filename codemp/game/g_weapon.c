...
#define BRYAR_CHARGE_UNIT                                                      \
  200.0f // bryar charging gives us one more unit every 200ms--if you change
         // this, you'll have to do the same in bg_pmove
...
#define DISRUPTOR_ALT_TRACES                                                   \
  3 // can go through a max of 3 damageable(sp?) entities
#define DISRUPTOR_CHARGE_UNIT                                                  \
  50.0f // distruptor charging gives us one more unit every 50ms--if you change
        // this, you'll have to do the same in bg_pmove
...
#define DEMP2_ALT_DAMAGE                                                       \
  8 // 12		// does 12, 36, 84 at each of the 3 charge levels.
#define DEMP2_CHARGE_UNIT                                                      \
  700.0f // demp2 charging gives us one more unit every 700ms--if you change
         // this, you'll have to do the same in bg_weapons
...
#define ATST_SIDE_MAIN_SPLASH_DAMAGE                                           \
  10 // yeah, pretty small, either zero out or make it worth having?
#define ATST_SIDE_MAIN_SPLASH_RADIUS                                           \
  16 // yeah, pretty small, either zero out or make it worth having?
...
extern qboolean G_BoxInBounds(vec3_t point, vec3_t mins, vec3_t maxs,
...
void RocketDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
               int damage, int mod);

// We should really organize weapon data into tables or parse from the ext data
// so we have accurate info for this,
...
//-----------------------------------------------------------------------------
{
  ...
  trap->Trace(&tr, eyePoint, mins, maxs, start, ent->s.number,
...
//---------------------------------------------------------
{
  ...
  gentity_t *missile =
      CreateMissile(muzzle, forward, BRYAR_PISTOL_VEL, 10000, ent, altFire);
  ...
  if (altFire) {
    ...
    missile->s.generic1 =
        count; // The missile will then render according to the charge level.
...
//-----------------------------------------------------------------------------
{
  ...
  if (self->splashDamage > 0 && self->splashRadius > 0) {
    G_RadiusDamage(self->r.currentOrigin, attacker, self->splashDamage,
                   self->splashRadius, 0 /*don't ignore attacker*/, self,
                   MOD_UNKNOWN);
  }
...
// Currently only the seeker drone uses this, but it might be useful for other
// things as well.
...
//---------------------------------------------------------
{
  ...
  missile->nextthink =
      level.time + 5000; // at 20000 speed, that should be more than enough
}

//---------------------------------------------------------
void WP_FireEmplacedMissile(gentity_t *ent, vec3_t start, vec3_t dir,
                            qboolean altFire, gentity_t *ignore)
//---------------------------------------------------------
{
  int velocity = BLASTER_VELOCITY;
  int damage = BLASTER_DAMAGE;
...
//---------------------------------------------------------
{
  ...
  // FIXME: if temp_org does not have clear trace to inside the bbox, don't
  // shoot!
...
//---------------------------------------------------------
{
  ...
  traces = 0;
  while (traces <
         10) { // need to loop this in case we hit a Jedi who dodges the shot
    if (d_projectileGhoul2Collision.integer) {
      trap->Trace(&tr, start, NULL, NULL, end, ignore, MASK_SHOT, qfalse,
                  G2TRFLAG_DOGHOULTRACE | G2TRFLAG_GETSURFINDEX |
                      G2TRFLAG_THICK | G2TRFLAG_HITCORPSES,
    ...
    if (d_projectileGhoul2Collision.integer && traceEnt->inuse &&
        traceEnt->client) { // g2 collision checks -rww
      if (traceEnt->inuse && traceEnt->client &&
          traceEnt
              ->ghoul2) { // since we used G2TRFLAG_GETSURFINDEX,
                          // tr.surfaceFlags will actually contain the index of
      ...
      if (traceEnt->ghoul2) {
        tr.surfaceFlags = 0; // clear the surface flags after, since we actually
                             // care about them in here.
      }
    }
    ...
    if (Jedi_DodgeEvasion(
            traceEnt, ent, &tr,
            G_GetHitLocation(traceEnt,
                             tr.endpos))) { // act like we didn't even hit him
      VectorCopy(tr.endpos, start);
      ignore = tr.entityNum;
      traces++;
      continue;
    } else if (traceEnt && traceEnt->client &&
               traceEnt->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] >=
                   FORCE_LEVEL_3) {
      if (WP_SaberCanBlock(
              traceEnt, tr.endpos, 0, MOD_DISRUPTOR, qtrue,
              0)) { // broadcast and stop the shot because it was blocked
        gentity_t *te = NULL;
  ...
  // always render a shot beam, doing this the old way because I don't much feel
  // like overriding the effect.
  tent = G_TempEntity(tr.endpos, EV_DISRUPTOR_MAIN_SHOT);
  VectorCopy(muzzle, tent->s.origin2);
  tent->s.eventParm = ent->s.number;
  ...
  if (render_impact) {
    if (tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage) {
      ...
      G_Damage(traceEnt, ent, ent, forward, tr.endpos, damage, DAMAGE_NORMAL,
               MOD_DISRUPTOR);
...
qboolean G_CanDisruptify(gentity_t *ent) {
  if (!ent || !ent->inuse || !ent->client || ent->s.eType != ET_NPC ||
      ent->s.NPC_class != CLASS_VEHICLE || !ent->m_pVehicle) { // not vehicle
    return qtrue;
  }

  if (ent->m_pVehicle->m_pVehicleInfo->type ==
      VH_ANIMAL) { // animal is only type that can be disintigeiteigerated
    return qtrue;
  }
...
//---------------------------------------------------------
{
  ...
  if (ent->client) {
    ...
    count =
        (level.time - ent->client->ps.weaponChargeTime) / DISRUPTOR_CHARGE_UNIT;
    if (level.gametype == GT_SIEGE) { // maybe a full alt-charge should be a
                                      // *bit* more dangerous in Siege mode?
      // maxCount = ceil((200.0f-(float)damage)/2.0f);//cap at 200 damage total
      maxCount = 200; // the previous line ALWAYS evaluated to 135 - was that on
                      // purpose?
    }
  } else {
  ...
  for (i = 0; i < traces; i++) {
    ...
    if (tr.entityNum == ent->s.number) {
      // should never happen, but basically we don't want to consider a hit to
      // ourselves? Get ready for an attempt to trace through another person
      // VectorCopy( tr.endpos, muzzle2 );
      VectorCopy(tr.endpos, start);
      skip = tr.entityNum;
#ifdef _DEBUG
      trap->Print(
          "BAD! Disruptor gun shot somehow traced back and hit the owner!\n");
#endif
      continue;
    }
    ...
    if (Jedi_DodgeEvasion(traceEnt, ent, &tr,
      ...
      continue;
               ...
               traceEnt->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] >=
                   FORCE_LEVEL_3) {
      if (WP_SaberCanBlock(
              traceEnt, tr.endpos, 0, MOD_DISRUPTOR_SNIPER, qtrue,
              0)) { // broadcast and stop the shot because it was blocked
        gentity_t *te = NULL;
    ...
    // always render a shot beam, doing this the old way because I don't much
    // feel like overriding the effect.
    tent = G_TempEntity(tr.endpos, EV_DISRUPTOR_SNIPER_SHOT);
    VectorCopy(muzzle, tent->s.origin2);
    tent->s.shouldtarget = fullCharge;
    tent->s.eventParm = ent->s.number;

    // If the beam hits a skybox, etc. it would look foolish to add impact
    // effects
    if (render_impact) {
      ...
      if (traceEnt->takedamage) {
        ...
        if (traceEnt->client && preHealth > 0 && traceEnt->health <= 0 &&
            fullCharge &&
            G_CanDisruptify(traceEnt)) { // was killed by a fully charged sniper
                                         // shot, so disintegrate
...
//---------------------------------------------------------
{
  if (!ent || !ent->client ||
      ent->client->ps.zoomMode !=
          1) { // do not ever let it do the alt fire when not zoomed
    altFire = qfalse;
  }

  if (ent && ent->s.eType == ET_NPC &&
      !ent->client) { // special case for animents
...
static void WP_BowcasterAltFire(gentity_t *ent) {
  ...
  gentity_t *missile =
      CreateMissile(muzzle, forward, BOWCASTER_VELOCITY, 10000, ent, qfalse);
...
//---------------------------------------------------------
{
  ...
  gentity_t *missile =
      CreateMissile(muzzle, dir, REPEATER_VELOCITY, 10000, ent, qfalse);
...
//---------------------------------------------------------
{
  ...
  gentity_t *missile =
      CreateMissile(muzzle, forward, REPEATER_ALT_VELOCITY, 10000, ent, qtrue);

  missile->classname = "repeater_alt_proj";
  missile->s.weapon = WP_REPEATER;

  VectorSet(missile->r.maxs, REPEATER_ALT_SIZE, REPEATER_ALT_SIZE,
            REPEATER_ALT_SIZE);
  VectorScale(missile->r.maxs, -1, missile->r.mins);
  missile->s.pos.trType = TR_GRAVITY;
  missile->s.pos.trDelta[2] +=
      40.0f; // give a slight boost in the upward direction
  missile->damage = damage;
  missile->dflags = DAMAGE_DEATH_KNOCKBACK;
  ...
  if (level.gametype == GT_SIEGE) // we've been having problems with this being
                                  // too hyper-potent because of it's radius
  {
...
static void WP_DEMP2_MainFire(gentity_t *ent) {
  ...
  gentity_t *missile =
      CreateMissile(muzzle, forward, DEMP2_VELOCITY, 10000, ent, qfalse);
...
void DEMP2_AltRadiusDamage(gentity_t *ent) {
  float frac = (level.time - ent->genericValue5) /
               800.0f; // / 1600.0f; // synchronize with demp2 effect
  ...
  if (ent->r.ownerNum >= 0 &&
      ent->r.ownerNum <
          /*MAX_CLIENTS ... let npc's/shooters use it*/ MAX_GENTITIES) {
  ...
  frac *= frac * frac; // yes, this is completely ridiculous...but it causes the
                       // shell to grow slowly then "explode" at the end

  radius = frac * 200.0f; // 200 is max radius...the model is aprox. 100 units
                          // tall...the fx draw code mults. this by 2.
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (gent != myOwner) {
      G_Damage(gent, myOwner, myOwner, dir, ent->r.currentOrigin, ent->damage,
               DAMAGE_DEATH_KNOCKBACK, ent->splashMethodOfDeath);
      if (gent->takedamage && gent->client) {
        if (gent->client->ps.electrifyTime <
            level.time) { // electrocution effect
          if (gent->s.eType == ET_NPC && gent->s.NPC_class == CLASS_VEHICLE &&
              gent->m_pVehicle &&
              (gent->m_pVehicle->m_pVehicleInfo->type == VH_SPEEDER ||
               gent->m_pVehicle->m_pVehicleInfo->type ==
                   VH_WALKER)) { // do some extra stuff to speeders/walkers
            gent->client->ps.electrifyTime = level.time + Q_irand(3000, 4000);
          } else if (gent->s.NPC_class != CLASS_VEHICLE ||
                     (gent->m_pVehicle &&
                      gent->m_pVehicle->m_pVehicleInfo->type !=
                          VH_FIGHTER)) { // don't do this to fighters
            gent->client->ps.electrifyTime = level.time + Q_irand(300, 800);
          }
        }
        if (gent->client->ps.powerups[PW_CLOAKED]) { // disable cloak
                                                     // temporarily
          Jedi_Decloak(gent);
  ...
  // store the last fraction so that next time around we can test against those
  // things that fall between that last point and where the current shockwave
  // edge is
...
//---------------------------------------------------------
{
  ...
  if (!ent->pos1[0] && !ent->pos1[1] &&
      !ent->pos1[2]) { // don't play effect with a 0'd out directional vector
    ent->pos1[1] = 1;
  }
  // Let's just save ourself some bandwidth and play both the effect and sphere
  // spawn in 1 event
  efEnt =
      G_PlayEffect(EFFECT_EXPLOSION_DEMP2ALT, ent->r.currentOrigin, ent->pos1);
...
//---------------------------------------------------------
{
  ...
  trap->Trace(&tr, start, NULL, NULL, end, ent->s.number, MASK_SHOT, qfalse, 0,
              0);

  missile = G_Spawn();
  G_SetOrigin(missile, tr.endpos);
  // In SP the impact actually travels as a missile based on the trace fraction,
  // but we're just going to be instant. -rww
...
//---------------------------------------------------------
{
  ...
  if (ent->delay > level.time) {
    count = G_RadiusList(ent->r.currentOrigin, FLECHETTE_MINE_RADIUS_CHECK, ent,
                         qtrue, ent_list);
  ...
  if (blow) {
    ...
    // we probably don't need to do this thinking logic very often...maybe this
    // is fast enough?
...
//-----------------------------------------------------------------------------
static void WP_TraceSetStart(gentity_t *ent, vec3_t start, vec3_t mins,
                             vec3_t maxs)
//-----------------------------------------------------------------------------
{
  // make sure our start point isn't on the other side of a wall
  ...
  trap->Trace(&tr, ent->client->ps.origin, mins, maxs, start, ent->s.number,
              MASK_SOLID | CONTENTS_SHOTCLIP, qfalse, 0, 0);
...
void WP_ExplosiveDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                     int damage, int mod) {
...
//------------------------------------------------------------------------------
static void WP_CreateFlechetteBouncyThing(vec3_t start, vec3_t fwd,
                                          gentity_t *self)
//------------------------------------------------------------------------------
{
  gentity_t *missile =
      CreateMissile(start, fwd, 700 + Q_flrand(0.0f, 1.0f) * 700,
...
//---------------------------------------------------------
{
  ...
  WP_TraceSetStart(self, start, vec3_origin,
...
//---------------------------------------------------------
{
  vec3_t newdir, targetdir, rup = {0, 0, 1}, right;
  vec3_t org;
  float dot, dot2, dis;
  int i;
  ...
  if (ent->genericValue1 &&
      ent->genericValue1 < level.time) { // time's up, we're done, remove us
    if (ent->genericValue2) {            // explode when die
      RocketDie(ent, &g_entities[ent->r.ownerNum], &g_entities[ent->r.ownerNum],
                0, MOD_UNKNOWN);
  ...
  if (!ent->enemy || !ent->enemy->client || ent->enemy->health <= 0 ||
      ent->enemy->client->ps
          .powerups[PW_CLOAKED]) { // no enemy or enemy not a client or enemy
                                   // dead or enemy cloaked
  ...
  if ((ent->spawnflags & 1)) { // vehicle rocket
    if (ent->enemy->client && ent->enemy->client->NPC_class ==
                                  CLASS_VEHICLE) { // tracking another vehicle
  ...
  if (ent->enemy && ent->enemy->inuse) {
    ...
    // Now the rocket can't do a 180 in space, so we'll limit the turn to about
    // 45 degrees.
    dot = DotProduct(targetdir, ent->movedir);
    if ((ent->spawnflags & 1)) { // vehicle rocket
      if (ent->radius >
          -1.0f) { // can lose the lock if DotProduct drops below this number
    ...
    if (dot < 0.0f) {
      ...
      // Yeah we've adjusted horizontally, but let's split the difference
      // vertically, so we kinda try to move towards it.
      newdir[2] =
          ((targetdir[2] * newDirMult) + (ent->movedir[2] * oldDirMult)) * 0.5;
    ...
    if (ent->enemy->client &&
        ent->enemy->client->ps.groundEntityNum !=
            ENTITYNUM_NONE) { // tracking a client who's on the ground, aim at
                              // the floor...?
  ...
  ent->nextthink =
      level.time +
      ROCKET_ALT_THINK_TIME; // Nothing at all spectacular happened, continue.
  return;
}
...
//---------------------------------------------------------
{
  ...
  if (ent->client && ent->client->ps.rocketLockIndex != ENTITYNUM_NONE) {
    float lockTimeInterval =
        ((level.gametype == GT_SIEGE) ? 2400.0f : 1200.0f) / 16.0f;
    ...
    // have a sturdy lock first, and because there's a slight difference in time
    // between server and client
    if (dif >= 10 && rTime != -1) {
      missile->enemy = &g_entities[ent->client->ps.rocketLockIndex];

      if (missile->enemy && missile->enemy->client &&
          missile->enemy->health > 0 &&
          !OnSameTeam(ent,
                      missile->enemy)) { // if enemy became invalid, died, or is
                                         // on the same team, then don't seek it
        missile->angle = 0.5f;
  ...
  //===testing being able to shoot rockets out of the
  //air==================================
  missile->health = 10;
...
//---------------------------------------------------------
{
  if (!ent->count) {
    ...
    if (G_RadiusDamage(ent->r.currentOrigin, ent->parent, ent->splashDamage,
                       ent->splashRadius, ent, ent, ent->splashMethodOfDeath)) {
...
//---------------------------------------------------------
{
  ...
  W_TraceSetStart(
      ent, start, bolt->r.mins,
      bolt->r
          .maxs); // make sure our start point isn't on the other side of a wall
...
qboolean WP_LobFire(gentity_t *self, vec3_t start, vec3_t target, vec3_t mins,
                    vec3_t maxs, int clipmask, vec3_t velocity,
                    qboolean tracePath, int ignoreEntNum, int enemyNum,
                    float minSpeed, float maxSpeed, float idealSpeed,
                    qboolean mustHit)
//---------------------------------------------------------
{ // for the galak mech NPC
  float targetDist, shotSpeed, speedInc = 100, travelTime, impactDist,
  ...
  while (hitCount < maxHits) {
    ...
    if (!hitCount) { // save the first (ideal) one as the failCase (fallback
                     // value)
    ...
    if (tracePath) { // do a rough trace of the path
      ...
      for (elapsedTime = timeStep; elapsedTime < floor(travelTime) + timeStep;
        ...
        trap->Trace(&trace, lastPos, mins, maxs, testPos, ignoreEntNum,
                    clipmask, qfalse, 0, 0);
        ...
        if (trace.fraction < 1.0f) {         // hit something
          if (trace.entityNum == enemyNum) { // hit the enemy, that's perfect!
            break;
                     ...
                     DistanceSquared(trace.endpos, target) <
                         4096) // hit within 64 of desired location, should be
                               // okay
          { // close enough!
            break;
          } else { // FIXME: maybe find the extents of this brush and go above
                   // or below it on next try somehow?
            ...
            if (trace.entityNum < ENTITYNUM_WORLD) { // hit an ent
              ...
              if (traceEnt && traceEnt->takedamage &&
                  !OnSameTeam(
                      self,
                      traceEnt)) { // hit something breakable, so that's okay
      ...
      if (blocked) { // hit something, adjust speed (which will change arc)
        hitCount++;
        shotSpeed = idealSpeed + ((hitCount - skipNum) *
                                  speedInc); // from min to max (skipping ideal)
        if (hitCount >=
            skipNum) { // skip ideal since that was the first value we tested
          shotSpeed += speedInc;
        }
      } else { // made it!
        break;
  ...
  if (hitCount >=
      maxHits) { // NOTE: worst case scenario, use the one that impacted closest
                 // to the target (or just use the first try...?)
...
void laserTrapExplode(gentity_t *self) {
  ...
  if (self->activator) {
    G_RadiusDamage(self->r.currentOrigin, self->activator, self->splashDamage,
                   self->splashRadius, self, self,
...
void laserTrapDelayedExplode(gentity_t *self, gentity_t *inflictor,
                             gentity_t *attacker, int damage,
                             int meansOfDeath) {
  self->enemy = attacker;
...
void touchLaserTrap(gentity_t *ent, gentity_t *other, trace_t *trace) {
  if (other && other->s.number <
                   ENTITYNUM_WORLD) { // just explode if we hit any entity. This
                                      // way we don't have things happening like
                                      // tripmines floating in the air after
                                      // getting stuck to a moving door
...
void proxMineThink(gentity_t *ent) {
  ...
  if (ent->genericValue15 < level.time || !owner || !owner->inuse ||
      !owner->client ||
  ...
  while (i < MAX_CLIENTS) { // eh, just check for clients, don't care about
                            // anyone else...
    cl = &g_entities[i];

    if (cl->inuse && cl->client &&
        cl->client->pers.connected == CON_CONNECTED && owner != cl &&
        cl->client->sess.sessionTeam != TEAM_SPECTATOR &&
        cl->client->tempSpectate < level.time && cl->health > 0) {
      if (!OnSameTeam(owner, cl) ||
          g_friendlyFire
              .integer) { // not on the same team, or friendly fire is enabled
        vec3_t v;
...
void laserTrapThink(gentity_t *ent) {
  ...
  trap->Trace(&tr, ent->r.currentOrigin, NULL, NULL, end, ent->s.number,
...
void laserTrapStick(gentity_t *ent, vec3_t endpos, vec3_t normal) {
  ...
  // This does nothing, cg_missile makes assumptions about direction of travel
  // controlling angles
...
void CreateLaserTrap(gentity_t *laserTrap, vec3_t start,
                     gentity_t *owner) { // create a laser trap entity
...
void charge_stick(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (other && (other->flags & FL_BBRUSH) &&
    ...
    self->s.pos.trDelta[0] +=
        vNor[0] * (tN[0] * (((float)Q_irand(1, 10)) * 0.1));
    self->s.pos.trDelta[1] +=
        vNor[1] * (tN[1] * (((float)Q_irand(1, 10)) * 0.1));
    self->s.pos.trDelta[2] +=
        vNor[2] * (tN[2] * (((float)Q_irand(1, 10)) * 0.1));

    vectoangles(vNor, self->s.angles);
    vectoangles(vNor, self->s.apos.trBase);
    self->touch = charge_stick;
    return;
  } else if (other && other->s.number <
                          ENTITYNUM_WORLD) { // hit an entity that we just want
                                             // to explode on (probably another
    ...
    G_RadiusDamage(self->r.currentOrigin, self->parent, self->splashDamage,
                   self->splashRadius, self, self, MOD_DET_PACK_SPLASH);
  ...
  // When you suicide, all of the detpacks you have placed (either on a wall, or
  // still falling in the air) will have
  //	their ent->think() set to DetPackBlow and ent->nextthink will be between
  //100 <-> 300
  // If your detpacks land on a surface (i.e. charge_stick gets called) within
  // that 100<->300 ms then ent->think()
  //	will be overwritten (set to DetpackBlow) and ent->nextthink will be
  //30000
  // The end result is your detpacks won't explode, but will be stuck to the
  // wall for 30 seconds without being able to
  //	detonate them (or shoot them)
  // The fix Sil came up with is to check the think() function in charge_stick,
  // and only overwrite it if they haven't
...
void DetPackBlow(gentity_t *self) {
  ...
  if (self->target_ent) { // we were attached to something, do *direct* damage
                          // to it!
    G_Damage(self->target_ent, self, &g_entities[self->r.ownerNum], v,
             self->r.currentOrigin, self->damage, 0, MOD_DET_PACK_SPLASH);
  }
  G_RadiusDamage(self->r.currentOrigin, self->parent, self->splashDamage,
                 self->splashRadius, self, self, MOD_DET_PACK_SPLASH);
  v[0] = 0;
...
void DetPackDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                int damage, int mod) {
  self->think = DetPackBlow;
  self->nextthink = level.time + Q_irand(50, 100);
  self->takedamage = qfalse;
...
void drop_charge(gentity_t *self, vec3_t start, vec3_t dir) {
  ...
  bolt->s.modelindex =
      G_ModelIndex("models/weapons2/detpack/det_pack_proj.glm");
...
void BlowDetpacks(gentity_t *ent) {
  ...
  if (ent->client->ps.hasDetPackPlanted) {
    while ((found = G_Find(found, FOFS(classname), "detpack")) !=
           NULL) { // loop through all ents and blow the crap out of them!
      if (found->parent == ent) {
...
static void WP_FireConcussionAlt(gentity_t *ent) { // a rail-gun-like beam
  ...
  for (i = 0; i < traces; i++) {
    ...
    // NOTE: if you want to be able to hit guys in emplaced guns, use
    // "G2_COLLIDE, 10" instead of "G2_RETURNONHIT, 0" alternately, if you end up
    // hitting an emplaced_gun that has a sitter, just redo this one trace with
    // the "G2_COLLIDE, 10" to see if we it the sitter trap->trace( &tr, start,
    // NULL, NULL, end, skip, MASK_SHOT, G2_COLLIDE, 10 );//G2_RETURNONHIT, 0 );
    if (d_projectileGhoul2Collision.integer) {
      trap->Trace(
          &tr, start, shot_mins, shot_maxs, end, skip, MASK_SHOT, qfalse,
          G2TRFLAG_DOGHOULTRACE | G2TRFLAG_GETSURFINDEX | G2TRFLAG_HITCORPSES,
          g_g2TraceLod.integer);
    } else {
      trap->Trace(&tr, start, shot_mins, shot_maxs, end, skip, MASK_SHOT,
                  qfalse, 0, 0);
    }
...
#ifdef _DEBUG
      Com_Printf(
          "BAD! Concussion gun shot somehow traced back and hit the owner!\n");
#endif
      continue;
    }
    ...
    if (!hitDodged) {
      if (render_impact) {
        if ((tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage) ||
          ...
          // Create a simple impact type mark that doesn't last long in the
          // world
          // G_PlayEffectID( G_EffectIndex( "concussion/alt_hit" ), tr.endpos,
          // tr.plane.normal ); no no no

          if (traceEnt->client &&
              LogAccuracyHit(traceEnt,
                             ent)) { // NOTE: hitting multiple ents can still
                                     // get you over 100% accuracy
            ent->client->accuracy_hits++;
          }

          noKnockBack =
              (traceEnt->flags &
               FL_NO_KNOCKBACK); // will be set if they die, I want to know if
                                 // it was on *before* they died
          if (traceEnt && traceEnt->client &&
              traceEnt->client->NPC_class == CLASS_GALAKMECH) { // hehe
            G_Damage(traceEnt, ent, ent, forward, tr.endpos, 10,
                     DAMAGE_NO_KNOCKBACK | DAMAGE_NO_HIT_LOC, MOD_CONC_ALT);
            break;
          }
          G_Damage(traceEnt, ent, ent, forward, tr.endpos, damage,
                   DAMAGE_NO_KNOCKBACK | DAMAGE_NO_HIT_LOC, MOD_CONC_ALT);
          ...
          if (traceEnt->client) { // only if we hit a client
            ...
            */
            if (traceEnt->health > 0) { // alive
              ...
              if (!noKnockBack && !traceEnt->localAnimIndex &&
                  traceEnt->client->ps.forceHandExtend !=
                      HANDEXTEND_KNOCKDOWN &&
                  BG_KnockDownable(
                      &traceEnt->client->ps)) // just check for humanoids..
              {                               // knock-downable
                // G_Knockdown( traceEnt, ent, pushDir, 400, qtrue );
                vec3_t plPDif;
                float pStr;

                // cap it and stuff, base the strength and whether or not we can
                // knockdown on the distance from the shooter to the target
                VectorSubtract(traceEnt->client->ps.origin,
                               ent->client->ps.origin, plPDif);
                ...
                if (pStr > 200.0f) {
                  ...
                  traceEnt->client->ps.forceDodgeAnim =
                      0; // this toggles between 1 and 0, when it's 1 we should
                         // play the get up anim
                }
          ...
          break; // hit solid, but doesn't take damage, so stop the shot...we
                 // _could_ allow it to shoot through walls, might be cool?
        }
      } else // not rendering impact, must be a skybox or other similar thing?
      {
        break; // don't try anymore traces
...
#if 0 // yuck
      // FIXME: if shoot *really* close to someone, the alert could be way out
      // of their FOV
...
static void WP_FireConcussion(gentity_t *ent) { // a fast rocket-like projectile
  ...
  //		&& !cg.renderingThirdPerson )//gives an advantage to being in 3rd
  //person, but would look silly otherwise
  //	{//kick the view back
...
void WP_FireStunBaton(gentity_t *ent, qboolean alt_fire) {
  ...
  trap->Trace(&tr, muzzleStun, mins, maxs, end, ent->s.number, MASK_SHOT,
              qfalse, 0, 0);
  ...
  if (tr_ent && tr_ent->takedamage &&
      tr_ent->client) { // see if either party is involved in a duel
  ...
  if (tr_ent && tr_ent->takedamage) {
    ...
    G_Sound(tr_ent, CHAN_WEAPON,
            G_SoundIndex(va("sound/weapons/melee/punch%d", Q_irand(1, 4))));
    G_Damage(tr_ent, ent, ent, forward, tr.endpos, STUN_BATON_DAMAGE,
             (DAMAGE_NO_KNOCKBACK | DAMAGE_HALF_ABSORB), MOD_STUN_BATON);
    ...
    if (tr_ent->client) { // if it's a player then use the shock effect
      if (tr_ent->client->NPC_class == CLASS_VEHICLE) { // not on vehicles
        if (!tr_ent->m_pVehicle ||
            ...
            tr_ent->m_pVehicle->m_pVehicleInfo->type ==
                VH_FLIER) { // can zap animals
...
void WP_FireMelee(gentity_t *ent, qboolean alt_fire) {
  ...
  trap->Trace(&tr, muzzlePunch, mins, maxs, end, ent->s.number, MASK_SHOT,
              qfalse, 0, 0);
  ...
  if (tr.entityNum != ENTITYNUM_NONE) { // hit something
    ...
    if (tr_ent->takedamage) { // damage them, do more damage if we're in the
                              // second right hook
      int dmg = MELEE_SWING1_DAMAGE;

      if (ent->client &&
          ent->client->ps.torsoAnim ==
              BOTH_MELEE2) { // do a tad bit more damage on the second swing
      ...
      G_Damage(tr_ent, ent, ent, forward, tr.endpos, dmg, DAMAGE_NO_ARMOR,
               MOD_MELEE);
    }
  }
}
...
void CalcMuzzlePoint(gentity_t *ent, const vec3_t inForward,
  ...
  if (weapontype > WP_NONE &&
      weapontype <
          WP_NUM_WEAPONS) { // Use the table to generate the muzzlepoint;
...
void WP_CalcVehMuzzle(gentity_t *ent, int muzzleNum) {
  ...
  if (pVeh->m_iMuzzleTime[muzzleNum] ==
      level.time) { // already done for this frame, don't need to do it again
    return;
  }
  ...
  if (pVeh->m_pVehicleInfo && (pVeh->m_pVehicleInfo->type == VH_ANIMAL ||
  ...
  trap->G2API_GetBoltMatrix_NoRecNoRot(
      ent->ghoul2, 0, pVeh->m_iMuzzleTag[muzzleNum], &boltMatrix, vehAngles,
...
gentity_t *WP_FireVehicleWeapon(gentity_t *ent, vec3_t start, vec3_t dir,
                                vehWeaponInfo_t *vehWeapon, qboolean alt_fire,
                                qboolean isTurretWeap) {
  gentity_t *missile = NULL;

  // FIXME: add some randomness...?  Inherent inaccuracy stat of weapon?  Pilot
  // skill?
  if (!vehWeapon) { // invalid vehicle weapon
    ...
    VectorSet(maxs, vehWeapon->fWidth / 2.0f, vehWeapon->fWidth / 2.0f,
              vehWeapon->fHeight / 2.0f);
    VectorScale(maxs, -1, mins);
    ...
    */
    ...
    if (vehWeapon->fWidth ||
        vehWeapon->fHeight) { // we assume it's a rocket-like thing
    ...
    if (vehWeapon->bIonWeapon) { // so it disables ship shields and sends them
                                 // out of control
      missile->s.weapon = WP_DEMP2;
    }
    ...
    if (ent->m_pVehicle && ent->m_pVehicle->m_pPilot) { // owned by vehicle
                                                        // pilot
    ...
    if (isTurretWeap) { // look for the turret weapon info on cgame side, not
                        // vehicle weapon info
      missile->s.weapon = WP_TURRET;
    }
    ...
    if (vehWeapon->fHoming) { // homing missile
      if (ent->client && ent->client->ps.rocketLockIndex != ENTITYNUM_NONE) {
        ...
        // It's 10 even though it locks client-side at 8, because we want them
        // to have a sturdy lock first, and because there's a slight difference
        // in time between server and client
        if (dif >= 10 && rTime != -1) {
          missile->enemy = &g_entities[ent->client->ps.rocketLockIndex];
          ...
          if (missile->enemy && missile->enemy->client &&
              ...
              !OnSameTeam(
                  ent,
                  missile->enemy)) { // if enemy became invalid, died, or is on
                                     // the same team, then don't seek it
            missile->spawnflags |=
                1; // just to let it know it should be faster...
            ...
            if (missile->enemy->s.NPC_class ==
                CLASS_VEHICLE) { // let vehicle know we've locked on to them
...
void G_VehMuzzleFireFX(gentity_t *ent, gentity_t *broadcaster,
  ...
  // NOTE: just need MAX_VEHICLE_MUZZLES bits for this... should be cool since
  // it's currently 12 and we're sending it in 16 bits
...
void G_EstimateCamPos(vec3_t viewAngles, vec3_t cameraFocusLoc,
                      float viewheight, float thirdPersonRange,
                      float thirdPersonHorzOffset, float vertOffset,
                      float pitchOffset, int ignoreEntNum, vec3_t camPos) {
  ...
  VectorSubtract(cameraCurTarget, cameraCurLoc, diff);
  {
    ...
    if (!dist || (diff[0] == 0 ||
                  diff[1] == 0)) { // must be hitting something, need some value
                                   // to calc angles, so use cam forward
...
void WP_GetVehicleCamPos(gentity_t *ent, gentity_t *pilot, vec3_t camPos) {
  float thirdPersonHorzOffset =
      ent->m_pVehicle->m_pVehicleInfo->cameraHorzOffset;
  ...
  if (ent->client->ps.hackingTime) {
    thirdPersonHorzOffset +=
        (((float)ent->client->ps.hackingTime) / MAX_STRAFE_TIME) * -80.0f;
    thirdPersonRange +=
        fabs(((float)ent->client->ps.hackingTime) / MAX_STRAFE_TIME) * 100.0f;
  }
  ...
  G_EstimateCamPos(ent->client->ps.viewangles, pilot->client->ps.origin,
                   pilot->client->ps.viewheight, thirdPersonRange,
                   thirdPersonHorzOffset, vertOffset, pitchOffset,
                   pilot->s.number, camPos);
  /*
  //Control Scheme 2 Method:
  G_EstimateCamPos( ent->m_pVehicle->m_vOrientation, ent->r.currentOrigin,
  pilot->client->ps.viewheight, thirdPersonRange, thirdPersonHorzOffset,
  vertOffset, pitchOffset, pilot->s.number, camPos );
  */
}

void WP_VehLeadCrosshairVeh(gentity_t *camTraceEnt, vec3_t newEnd,
                            const vec3_t dir, const vec3_t shotStart,
                            vec3_t shotDir) {
  if (camTraceEnt && camTraceEnt->client &&
      camTraceEnt->client->NPC_class ==
          CLASS_VEHICLE) { // if the crosshair is on a vehicle, lead it
...
extern int BG_VehTraceFromCamPos(trace_t *camTrace, bgEntity_t *bgEnt,
                                 const vec3_t entOrg, const vec3_t shotStart,
                                 const vec3_t end, vec3_t newEnd,
                                 vec3_t shotDir, float bestDist);
qboolean WP_VehCheckTraceFromCamPos(gentity_t *ent, const vec3_t shotStart,
                                    vec3_t shotDir) {
  // FIXME: only if dynamicCrosshair and dynamicCrosshairPrecision is on!
  if (!ent || !ent->m_pVehicle || !ent->m_pVehicle->m_pVehicleInfo ||
      !ent->m_pVehicle->m_pPilot // not being driven
      || !((gentity_t *)ent->m_pVehicle->m_pPilot)
              ->client // not being driven by a client...?!!!
      || (ent->m_pVehicle->m_pPilot->s.number >=
          MAX_CLIENTS)) // being driven, but not by a real client, no need to
                        // worry about crosshair
  {
    return qfalse;
  }
  if ((ent->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER &&
       g_cullDistance > MAX_XHAIR_DIST_ACCURACY) ||
      ent->m_pVehicle->m_pVehicleInfo->type == VH_WALKER) {
    // FIRST: simulate the normal crosshair trace from the center of the veh
    // straight forward
    trace_t trace;
    vec3_t dir, start, end;
    if (ent->m_pVehicle->m_pVehicleInfo->type ==
        VH_WALKER) { // for some reason, the walker always draws the crosshair
                     // out from from the first muzzle point
    ...
    trap->Trace(&trace, start, vec3_origin, vec3_origin, end, ent->s.number,
                CONTENTS_SOLID | CONTENTS_BODY, qfalse, 0, 0);

    if (ent->m_pVehicle->m_pVehicleInfo->type ==
        VH_WALKER) { // just use the result of that one trace since walkers
                     // don't do the extra trace
      ...
      trace_t extraTrace;
      vec3_t newEnd;
      int camTraceEntNum = BG_VehTraceFromCamPos(
          &extraTrace, (bgEntity_t *)ent, ent->r.currentOrigin, shotStart, end,
          newEnd, shotDir, (trace.fraction * g_cullDistance));
      if (camTraceEntNum) {
        WP_VehLeadCrosshairVeh(&g_entities[camTraceEntNum - 1], newEnd, dir,
                               shotStart, shotDir);
        return qtrue;
      }
    }
...
//---------------------------------------------------------
{
  ...
  if (pVeh->m_pVehicleInfo->type == VH_WALKER &&
      ent->client->ps.electrifyTime >
          level.time) { // don't fire while being electrocuted
    return;
  }

  // TODO?: If possible (probably not enough time), it would be nice if
  // secondary fire was actually a mode switch/toggle so that, for instance, an
  // x-wing can have 4-gun fire, or individual muzzle fire. If you wanted a
  // different weapon, you would actually have to press the 2 key or something
  // like that (I doubt I'd get a graphic for it anyways though). -AReis

  // If this is not the alternate fire, fire a normal blaster shot...
  if (pVeh->m_pVehicleInfo &&
      (pVeh->m_pVehicleInfo->type != VH_FIGHTER ||
       (pVeh->m_ulFlags & VEH_WINGSOPEN))) // NOTE: Wings open also denotes that
  ...
  { // fighters can only fire when wings are open
    ...
    if (pVeh->weaponStatus[weaponNum].ammo <= 0) { // no ammo for this weapon
      if (pVeh->m_pPilot &&
          pVeh->m_pPilot->s.number <
              MAX_CLIENTS) { // let the client know he's out of ammo
        int i;
        // but only if one of the vehicle muzzles is actually ready to fire this
        // weapon
        for (i = 0; i < MAX_VEHICLE_MUZZLES; i++) {
          if (pVeh->m_pVehicleInfo->weapMuzzle[i] !=
              vehWeaponIndex) { // this muzzle doesn't match the weapon we're
                                // trying to use
            continue;
          }
          if (pVeh->m_iMuzzleTag[i] != -1 &&
              pVeh->m_iMuzzleWait[i] <
                  level.time) { // this one would have fired, send the no ammo
                                // message
    ...
    if (pVeh->m_pVehicleInfo->weapon[weaponNum].linkable == 2 // always linked
        || (pVeh->m_pVehicleInfo->weapon[weaponNum].linkable ==
                1                                     // optionally linkable
            && pVeh->weaponStatus[weaponNum].linked)) // linked
    { // we're linking the primary or alternate weapons, so we'll do *all* the
      // muzzles
      linkedFiring = qtrue;
    }

    if (vehWeaponIndex <= VEH_WEAPON_BASE ||
        vehWeaponIndex >= MAX_VEH_WEAPONS) { // invalid vehicle weapon
      return;
    } else {
      int i, numMuzzles = 0, numMuzzlesReady = 0, cumulativeDelay = 0,
             cumulativeAmmo = 0;
      qboolean sentAmmoWarning = qfalse;

      vehWeapon = &g_vehWeaponInfo[vehWeaponIndex];

      if (pVeh->m_pVehicleInfo->weapon[weaponNum].linkable ==
          2) { // always linked weapons don't accumulate delay, just use
               // specified delay
      ...
      for (i = 0; i < MAX_VEHICLE_MUZZLES; i++) {
        ...
        if (pVeh->m_pVehicleInfo
                ->weapMuzzle[pVeh->weaponStatus[weaponNum].nextMuzzle] !=
            vehWeaponIndex) { // Our designated next muzzle for this weapon
                              // isn't valid for this weapon (happens when ships
      ...
      if (linkedFiring) {                    // firing all muzzles at once
        if (numMuzzlesReady != numMuzzles) { // can't fire all linked muzzles
                                             // yet
          return;
        } else { // can fire all linked muzzles, check ammo
          if (pVeh->weaponStatus[weaponNum].ammo <
              cumulativeAmmo) { // can't fire, not enough ammo
            if (pVeh->m_pPilot &&
                pVeh->m_pPilot->s.number <
                    MAX_CLIENTS) { // let the client know he's out of ammo
              G_AddEvent((gentity_t *)pVeh->m_pPilot, EV_NOAMMO, weaponNum);
      ...
      for (i = 0; i < MAX_VEHICLE_MUZZLES; i++) {
        ...
        if (!linkedFiring && i != pVeh->weaponStatus[weaponNum]
                                      .nextMuzzle) { // we're only firing one
                                                     // muzzle and this isn't it
          continue;
        }
        ...
        if (pVeh->m_iMuzzleTag[i] != -1 &&
          ...
          if (pVeh->weaponStatus[weaponNum].ammo <
              vehWeapon->iAmmoPerShot) { // out of ammo!
            if (!sentAmmoWarning) {
            ...
            if (WP_VehCheckTraceFromCamPos(
                    ent, start,
                    dir)) { // auto-aim at whatever crosshair would be over from
                            // camera's point of view (if closer)
            } else if (aimCorrect) { // auto-aim the missile at the crosshair if
              ...
              trap->Trace(&trace, ent->r.currentOrigin, vec3_origin,
                          vec3_origin, end, ent->s.number, MASK_SHOT, qfalse, 0,
                          0);
              if (trace.fraction < 1.0f && !trace.allsolid &&
                  !trace.startsolid) {
                vec3_t newEnd;
                VectorCopy(trace.endpos, newEnd);
                WP_VehLeadCrosshairVeh(&g_entities[trace.entityNum], newEnd,
                                       fixedDir, start, dir);
              }
            }

            // play the weapon's muzzle effect if we have one
            // NOTE: just need MAX_VEHICLE_MUZZLES bits for this... should be
            // cool since it's currently 12 and we're sending it in 16 bits
            muzzlesFired |= (1 << i);

            missile = WP_FireVehicleWeapon(ent, start, dir, vehWeapon, alt_fire,
                                           qfalse);
            if (vehWeapon->fHoming) { // clear the rocket lock entity *after*
                                      // all muzzles have fired
              clearRocketLockEntity = qtrue;
            }
          }

          if (linkedFiring) { // we're linking the weapon, so continue on and
                              // fire all appropriate muzzles
            continue;
          }
          ...
          if (numMuzzles > 1) { // more than one, look for it
            ...
            while (1) {
              ...
              if (nextMuzzle == pVeh->weaponStatus[weaponNum]
                                    .nextMuzzle) { // WTF?  Wrapped without
                                                   // finding another valid one!
                break;
              }
              if (pVeh->m_pVehicleInfo->weapMuzzle[nextMuzzle] ==
                  vehWeaponIndex) { // this is the next muzzle for this weapon
          ...
          pVeh->m_iMuzzleWait[pVeh->weaponStatus[weaponNum].nextMuzzle] =
              level.time + delay;
          // take away the ammo
          pVeh->weaponStatus[weaponNum].ammo -= vehWeapon->iAmmoPerShot;
          // NOTE: in order to send the vehicle's ammo info to the client, we
          // copy the ammo into the first 2 ammo slots on the vehicle NPC's
          // client->ps.ammo array
          if (pVeh->m_pParentEntity &&
              ((gentity_t *)(pVeh->m_pParentEntity))->client) {
            ((gentity_t *)(pVeh->m_pParentEntity))->client->ps.ammo[weaponNum] =
      ...
      // we went through all the muzzles, so apply the cumulative delay and ammo
      // cost
      if (cumulativeAmmo) { // taking ammo one shot at a time
        // take the ammo
        pVeh->weaponStatus[weaponNum].ammo -= cumulativeAmmo;
        // NOTE: in order to send the vehicle's ammo info to the client, we copy
        // the ammo into the first 2 ammo slots on the vehicle NPC's
        // client->ps.ammo array
        if (pVeh->m_pParentEntity &&
            ((gentity_t *)(pVeh->m_pParentEntity))->client) {
          ((gentity_t *)(pVeh->m_pParentEntity))->client->ps.ammo[weaponNum] =
      ...
      if (cumulativeDelay) { // we linked muzzles so we need to apply the
                             // cumulative delay now, to each of the linked
                             // muzzles
        for (i = 0; i < MAX_VEHICLE_MUZZLES; i++) {
          if (pVeh->m_pVehicleInfo->weapMuzzle[i] !=
              vehWeaponIndex) { // this muzzle doesn't match the weapon we're
...
tryFire:
  if (clearRocketLockEntity) { // hmm, should probably clear that anytime any
                               // weapon fires?
    ent->client->ps.rocketLockIndex = ENTITYNUM_NONE;
    ent->client->ps.rocketLockTime = 0;
    ent->client->ps.rocketTargetTime = 0;
...
===============
*/
int BG_EmplacedView(vec3_t baseAngles, vec3_t angles, float *newYaw,
                    float constraint);
...
void FireWeapon(gentity_t *ent, qboolean altFire) {
  ...
  if (ent && ent->client && ent->client->NPC_class == CLASS_VEHICLE) {
    ...
    if (ent->s.weapon == WP_EMPLACED_GUN &&
        ent->client->ps.emplacedIndex) { // if using emplaced then base muzzle
                                         // point off of gun position/angles
      ...
      if (emp->inuse) {
        ...
        override = BG_EmplacedView(ent->client->ps.viewangles, emp->s.angles,
                                   &yaw, emp->s.origin2[0]);
    ...
    } else if (ent->s.number < MAX_CLIENTS && ent->client->ps.m_iVehicleNum &&
               ent->s.weapon ==
                   WP_BLASTER) { // riding a vehicle...with blaster selected
    ...
    case WP_EMPLACED_GUN:
      if (ent->client &&
          ent->client->ewebIndex) { // specially handled by the e-web itself
        break;
      }
...
//---------------------------------------------------------
{
  ...
  if (!ent->client->ps.emplacedIndex) { // shouldn't be using WP_EMPLACED_GUN if
                                        // we aren't on an emplaced weapon
    return;
  }
  ...
  if (!gun->inuse ||
      gun->health <=
          0) { // gun was removed or killed, although we should never hit this
               // check because we should have been forced off it already
    return;
  }
...
/*QUAKED emplaced_gun (0 0 1) (-30 -20 8) (30 20 60) CANRESPAWN
 ...
 ms) constraint - number of degrees gun is constrained from base angles on each
 side (default 60.0)

 showhealth - set to 1 to show health bar on this entity when crosshair is over
 it
...
void emplaced_gun_use(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (activator->client->ps.emplacedTime >
      level.time) { // last use attempt still too recent
    return;
  }

  if (activator->client->ps.forceHandExtend !=
      HANDEXTEND_NONE) { // don't use if busy doing something else
    return;
  }

  if (activator->client->ps.origin[2] >
      self->s.origin[2] + zoffset - 8) { // can't use it from the top
    return;
  }
  ...
  // otherwise we don't allow to use it.
  if (dot < -0.2f) {
    goto tryHeal;
  }
  ...
  VectorSubtract(self->r.currentOrigin, activator->client->ps.origin,
                 anglesToOwner);
...
void emplaced_gun_realuse(gentity_t *self, gentity_t *other,
                          gentity_t *activator) {
...
void emplaced_gun_update(gentity_t *self) {
  ...
  if (self->health < 1 && !self->genericValue5) { // we are dead, set our
                                                  // respawn delay if we have one
    if (self->spawnflags & EMPLACED_CANRESPAWN) {
      self->genericValue5 = level.time + 4000 + self->count;
    }
  } else if (self->health < 1 &&
             self->genericValue5 <
                 level.time) { // we are dead, see if it's time to respawn
    self->s.time = 0;
  ...
  if (self->genericValue4 && self->genericValue4 < 2 &&
      self->s.time < level.time) { // we have finished our warning (red
                                   // flashing) effect, it's time to finish dying
    vec3_t explOrg;
    ...
    G_RadiusDamage(self->r.currentOrigin, self, self->splashDamage,
                   self->splashRadius, self, NULL, MOD_UNKNOWN);
  ...
  if (self->genericValue3 >
      level.time) { // see if we are freshly dead and should be smoking
    if (self->genericValue2 <
        level.time) { // is it time yet to spawn another smoke puff?
      VectorSet(puffAngle, 0, 0, 1);
  ...
  if (self->activator && self->activator->client &&
      self->activator->inuse) { // handle updating current user
    vec3_t vLen;
    VectorSubtract(self->s.origin, self->activator->client->ps.origin, vLen);
    ownLen = VectorLength(vLen);

    if (!(self->activator->client->pers.cmd.buttons & BUTTON_USE) &&
        self->genericValue1) {
  ...
  if ((self->activator && self->activator->client) &&
      (!self->activator->inuse ||
       self->activator->client->ps.emplacedIndex != self->s.number ||
       self->genericValue4 || ownLen > 64)) { // get the user off of me then
    ...
    self->s.activeForcePass = 0;
             ...
             self->activator->client) { // make sure the user is using the
                                        // emplaced gun weapon
...
//----------------------------------------------------------
void emplaced_gun_die(gentity_t *self, gentity_t *inflictor,
                      gentity_t *attacker, int damage,
                      int mod) { // set us up to flash and then explode
...
void SP_emplaced_gun(gentity_t *ent) {
  ...
  trap->Trace(&tr, ent->s.origin, ent->r.mins, ent->r.maxs, down, ent->s.number,
  ...
  if (ent->spawnflags & EMPLACED_CANRESPAWN) { // make it somewhat easier to
                                               // kill if it can respawn
    ent->health *= 0.4;
  }
