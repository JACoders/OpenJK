...
void SCR_DrawBigChar(int x, int y, int ch) {
  ...
  */
  ...
  re.DrawStretchPic(ax, ay, aw, ah, fcol, frow, fcol + size, frow + size2,
                    cls.charSetShader);
}
...
void SCR_DrawSmallChar(int x, int y, int ch) {
  ...
  */
  ...
  re.DrawStretchPic(x * con.xadjust, y * con.yadjust,
                    SMALLCHAR_WIDTH * con.xadjust,
                    SMALLCHAR_HEIGHT * con.yadjust, fcol, frow, fcol + size,
                    frow + size2, cls.charSetShader);
}
...
==================
*/
void SCR_DrawBigStringExt(int x, int y, const char *string, float *setColor,
...
void SCR_PrecacheScreenshot() {
  ...
  //
  //	char *psInfo = cl.gameState.stringData + cl.gameState.stringOffsets[
  //CS_SERVERINFO ]; 	int iMaxClients = atoi(Info_ValueForKey( psInfo,
  //"sv_maxclients" ));

  // (no need to check single-player status in voyager, this code base is all
  // singleplayer)
  ...
  if (!Key_GetCatcher()) {
    ...
    //		pbScreenData = (byte *)Z_Malloc(SG_SCR_WIDTH * SG_SCR_HEIGHT *
    //4);
    S_ClearSoundBuffer(); // clear DMA etc because the following glReadPixels()
                          // call can take ages
...
// called from save-game code to set the lo-res loading screen to be the one
// from the save file...
//
...
#ifdef JK2_MODE
// This is just a client-side wrapper for the function
// RE_TempRawImage_ReadFromFile() in the renderer code...
//

byte *SCR_TempRawImage_ReadFromFile(const char *psLocalFilename, int *piWidth,
                                    int *piHeight, byte *pbReSampleBuffer,
                                    qboolean qbVertFlip) {
  return re.TempRawImage_ReadFromFile(psLocalFilename, piWidth, piHeight,
                                      pbReSampleBuffer, qbVertFlip);
}
//
// ditto (sort of)...
//
