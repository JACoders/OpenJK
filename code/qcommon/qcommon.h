...
//
//#define DEBUG_ZONE_ALLOCS			// adds __FILE__ and __LINE__ info
//to zone blocks, to see who's leaking #define DETAILED_ZONE_DEBUG_CODE	// this
//slows things down a LOT, and is only for tracking nasty double-freeing
//Z_Malloc bugs
#endif
#endif
...
#define PACKET_BACKUP                                                          \
  16 // number of old messages that must be kept on client and
...
fileHandle_t FS_FOpenFileAppend(
    const char *filename); // this was present already, but no public proto
...
// These 2 are generally only used by the save games, filenames are local (eg
// "saves/blah.sav")
//
...
qboolean Z_IsFromZone(const void *pvAddress,
                      memtag_t eTag); // returns size if true
...
#define Z_Malloc(iSize, eTag, bZeroit)                                         \
  _D_Z_Malloc((iSize), (eTag), (bZeroit), __FILE__, __LINE__)
#define S_Malloc(iSize)                                                        \
  _D_S_Malloc((iSize), __FILE__,                                               \
              __LINE__) // NOT 0 filled memory only for small allocations
...
// note the opposite default for 'bZeroIt' in Hunk_Alloc to Z_Malloc, since
// Hunk_Alloc always used to memset(0)...
//
