...
==================
*/
void CM_Trace(trace_t *trace, const vec3_t start, const vec3_t end, const vec3_t mins, const vec3_t maxs, clipHandle_t model, const vec3_t origin,
	...
	assert(trace->allsolid || trace->fraction == 1.0 || VectorLengthSquared(trace->plane.normal) > 0.9999);
}
...
==================
*/
void CM_TransformedBoxTrace(trace_t *trace, const vec3_t start, const vec3_t end, const vec3_t mins, const vec3_t maxs, clipHandle_t model, int brushmask,
...
bool CM_CullBox(const cplane_t *frustum, const vec3_t transformed[8]) {
	...
	for (i = 0, frust = frustum; i < 4; i++, frust++) {
		for (j = 0; j < 8; j++) {
			if (DotProduct(transformed[j], frust->normal) > frust->dist) { // a point is in front
				break;
			}
		}
