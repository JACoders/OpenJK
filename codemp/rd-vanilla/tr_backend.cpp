...
void GL_Cull(int cullType) {
  ...
  if (backEnd
          .projection2D) { // don't care, we're in 2d when it's always disabled
    return;
  }
...
void RB_BeginDrawingView(void) {
  ...
  if (tr.refdef.rdflags & RDF_AUTOMAP ||
      (!(backEnd.refdef.rdflags & RDF_NOWORLDMODEL) && r_DynamicGlow->integer &&
       !g_bRenderGlowingObjects)) {
    if (tr.world &&
        tr.world->globalFog !=
            -1) { // this is because of a bug in multiple scenes I think, it
                  // needs to clear for the second scene but it doesn't normally.
      const fog_t *fog = &tr.world->fogs[tr.world->globalFog];

      clearBits |= GL_COLOR_BUFFER_BIT;
      qglClearColor(fog->parms.color[0], fog->parms.color[1],
                    fog->parms.color[2], 1.0f);
    }
  }

  // If this pass is to just render the glowing objects, don't clear the depth
  // buffer since we're sharing it with the main scene (since the main scene has
  // already been rendered). -AReis
...
static inline bool R_WorldCoordToScreenCoordFloat(vec3_t worldCoord, float *x,
                                                  float *y) {
  int xcenter, ycenter;
...
// used by RF_DISTORTION
static inline bool R_WorldCoordToScreenCoord(vec3_t worldCoord, int *x,
                                             int *y) {
  float xF, yF;
...
// note that postrenders lack much of the optimization that the standard
// sort-render crap does, so it's slower.
...
void RB_RenderDrawSurfList(drawSurf_t *drawSurfs, int numDrawSurfs) {
  ...
  for (i = 0, drawSurf = drawSurfs; i < numDrawSurfs; i++, drawSurf++) {
    ...
    if (entityNum != REFENTITYNUM_WORLD &&
      ...
      if ((backEnd.refdef.entities[entityNum].e.renderfx & RF_DISTORTION) ||
          ...
          (backEnd.refdef.entities[entityNum].e.renderfx &
           RF_FORCE_ENT_ALPHA)) { // must render last
        ...
        */
        ...
        oldSort = -20; // invalidate this thing, cause we may want to postrender
                       // more surfs of the same sort
    ...
    { //not an ent, just a surface that needs this effect
            ...
            oldSort = -20; //invalidate this thing, cause we may want to
    postrender more surfs of the same sort
  ...
  if (g_numPostRenders > 0) {
    ...
    while (g_numPostRenders > 0) {
      ...
      backEnd.refdef.dlights, &backEnd.ori );
      }
      else
      */
      { // ent
      ...
      /*
      if (!pRender->eValid)
      { //special full-screen "distortion" (or refraction or whatever the heck
you want to call it) if (!tr_stencilled) { //only need to do this once every
frame (that a surface using it is around) int radX = 2048; int radY = 2048; int
x = glConfig.vidWidth/2; int y = glConfig.vidHeight/2; int cX, cY;

                      GL_Bind( tr.screenImage );
                      //using this method, we could pixel-filter the texture and
all sorts of crazy stuff.
                      ...
                      {
                              tmp = (byte
*)Z_Malloc((sizeof(byte)*4)*(glConfig.vidWidth*glConfig.vidHeight), TAG_ICARUS,
qtrue);
                      }
                      qglReadPixels(0, 0, glConfig.vidWidth, glConfig.vidHeight,
GL_RGBA, GL_UNSIGNED_BYTE, tmp); qglTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 512,
512, 0, GL_RGBA, GL_UNSIGNED_BYTE, tmp); #endif
                      ...
                      qglCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16, cX, cY,
radX, radY, 0);
              }
      ...
      */
      if (!pRender->eValid) {
      } else if ((backEnd.refdef.entities[pRender->entNum].e.renderfx &
                  RF_DISTORTION) &&
                 lastPostEnt !=
                     pRender->entNum) { // do the capture now, we only need to
                                        // do it once per ent
        int x, y;
        int rad;
        bool r;
...
#endif
  if (tr_stencilled &&
      !tr_distortionPrePost) { // draw in the stencil buffer's cutout
    RB_DistortionFill();
  }
  if (!didShadowPass) {
...
void RE_StretchRaw(int x, int y, int w, int h, int cols, int rows,
  ...
  if (cols != tr.scratchImage[client]->width ||
    ...
    qglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                     glConfig.clampToEdgeAvailable ? GL_CLAMP_TO_EDGE
                                                   : GL_CLAMP);
...
void RE_UploadCinematic(int cols, int rows, const byte *data, int client,
                        qboolean dirty) {
...
const void *RB_RotatePic(const void *data) {
  ...
  if (image) {
    ...
    matrix3_t m = {
        {c, s, 0.0f}, {-s, c, 0.0f}, {cmd->x + cmd->w, cmd->y, 1.0f}};
    ...
    tess.xyz[numVerts + 3][0] =
        m[0][0] * (-cmd->w) + m[1][0] * (cmd->h) + m[2][0];
    tess.xyz[numVerts + 3][1] =
        m[0][1] * (-cmd->w) + m[1][1] * (cmd->h) + m[2][1];
    tess.xyz[numVerts + 3][2] = 0;
...
const void *RB_RotatePic2(const void *data) {
  ...
  if (shader->numUnfoggedPasses) {
    ...
    if (image) {
      ...
      matrix3_t m = {{c, s, 0.0f}, {-s, c, 0.0f}, {cmd->x, cmd->y, 1.0f}};
      ...
      tess.xyz[numVerts][0] =
          m[0][0] * (-cmd->w * 0.5f) + m[1][0] * (-cmd->h * 0.5f) + m[2][0];
      tess.xyz[numVerts][1] =
          m[0][1] * (-cmd->w * 0.5f) + m[1][1] * (-cmd->h * 0.5f) + m[2][1];
      tess.xyz[numVerts][2] = 0;

      tess.texCoords[numVerts][0][0] = cmd->s1;
      tess.texCoords[numVerts][0][1] = cmd->t1;

      tess.xyz[numVerts + 1][0] =
          m[0][0] * (cmd->w * 0.5f) + m[1][0] * (-cmd->h * 0.5f) + m[2][0];
...
const void *RB_DrawSurfs(const void *data) {
  ...
  // Dynamic Glow/Flares:
  /*
          The basic idea is to render the glowing parts of the scene to an
     offscreen buffer, then take that buffer and blur it. After it is
     sufficiently blurred, re-apply that image back to the normal screen using a
     additive blending. To blur the scene I use a vertex program to supply four
     texture coordinate offsets that allow 'peeking' into adjacent pixels. In
     the register combiner (pixel shader), I combine the adjacent pixels using a
     weighting factor. - Aurelio
  */

  // Render dynamic glowing/flaring objects.
  if (!(backEnd.refdef.rdflags & RDF_NOWORLDMODEL) && g_bDynamicGlowSupported &&
      r_DynamicGlow->integer) {
    // Copy the normal scene to texture.
    qglDisable(GL_TEXTURE_2D);
    qglEnable(GL_TEXTURE_RECTANGLE_ARB);
    qglBindTexture(GL_TEXTURE_RECTANGLE_ARB, tr.sceneImage);
    qglCopyTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, 0, 0,
                         glConfig.vidWidth, glConfig.vidHeight);
    qglDisable(GL_TEXTURE_RECTANGLE_ARB);
    qglEnable(GL_TEXTURE_2D);

    // Just clear colors, but leave the depth buffer intact so we can 'share'
    // it.
...
const void *RB_DrawBuffer(const void *data) {
  ...
  if (tr.world && tr.world->globalFog != -1) {
    ...
    qglClearColor(fog->parms.color[0], fog->parms.color[1], fog->parms.color[2],
                  1.0f);
...
static void RB_GammaCorrectRender() {
  ...
  GL_SelectTexture(0);
  qglBindTexture(GL_TEXTURE_RECTANGLE_ARB, tr.sceneImage);
  qglCopyTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA, 0, 0,
...
const void *RB_SwapBuffers(const void *data) {
  ...
  if (r_measureOverdraw->integer) {
    ...
    stencilReadback = (unsigned char *)Hunk_AllocateTempMemory(
        glConfig.vidWidth * glConfig.vidHeight);
...
static inline void RB_BlurGlowTexture() {
  ...
  qglPushMatrix();
  qglLoadIdentity();
  qglOrtho(0, backEnd.viewParms.viewportWidth, backEnd.viewParms.viewportHeight,
           0, -1, 1);
  qglMatrixMode(GL_MODELVIEW);
  qglPushMatrix();
  qglLoadIdentity();
  ...
  // NOTE: The 0.25 is because we're blending 4 textures (so = 1.0) and we want
  // a relatively normalized pixel intensity distribution, but this won't happen
  // anyways if intensity is higher than 1.0.
  float fBlurDistribution = r_DynamicGlowIntensity->value * 0.25f;
  float fBlurWeight[4] = {fBlurDistribution, fBlurDistribution,
                          fBlurDistribution, 1.0f};
  ...
  if (qglCombinerParameterfvNV) {
    ...
    qglProgramEnvParameter4fARB(GL_FRAGMENT_PROGRAM_ARB, 0, fBlurWeight[0],
                                fBlurWeight[1], fBlurWeight[2], fBlurWeight[3]);
  }
  ...
  // int iTexWidth = backEnd.viewParms.viewportWidth, iTexHeight =
  // backEnd.viewParms.viewportHeight;
  int iTexWidth = glConfig.vidWidth, iTexHeight = glConfig.vidHeight;

  for (int iNumBlurPasses = 0; iNumBlurPasses < r_DynamicGlowPasses->integer;
       iNumBlurPasses++) {
    // Load the Texel Offsets into the Vertex Program.
    qglProgramEnvParameter4fARB(GL_VERTEX_PROGRAM_ARB, 0, -fTexelWidthOffset,
                                -fTexelWidthOffset, 0.0f, 0.0f);
    qglProgramEnvParameter4fARB(GL_VERTEX_PROGRAM_ARB, 1, -fTexelWidthOffset,
                                fTexelWidthOffset, 0.0f, 0.0f);
    qglProgramEnvParameter4fARB(GL_VERTEX_PROGRAM_ARB, 2, fTexelWidthOffset,
                                -fTexelWidthOffset, 0.0f, 0.0f);
    qglProgramEnvParameter4fARB(GL_VERTEX_PROGRAM_ARB, 3, fTexelWidthOffset,
                                fTexelWidthOffset, 0.0f, 0.0f);

    // After first pass put the tex coords to the viewport size.
    if (iNumBlurPasses == 1) {
      // OK, very weird, but dependent on which texture rectangle extension
      // we're using, the texture either needs to be always texure correct or
      // view correct...
    ...
    qglVertex2f(backEnd.viewParms.viewportWidth,
                backEnd.viewParms.viewportHeight);
    ...
    // NOTE: This is possibly the most important input to the effect. Even by
    // using an exponential function I've been able to make it look better (at a
    // much higher cost of course). This is cheap though and still looks pretty
    // great. In the future I might want to use an actual gaussian equation to
    // correctly calculate the pixel coefficients and attenuates, texel offsets,
    // gaussian amplitude and radius...
...
static inline void RB_DrawGlowOverlay() {
  ...
  // One and Inverse Src Color give a very soft addition, while one one is a bit
  // stronger. With one one we can use additive blending through multitexture
  // though.
  ...
  // NOTE: Multi-texture wasn't that much faster (we're obviously not
  // bottlenecked by transform pipeline), and besides, soft glow looks better
  // anyways.
  /*	else
          {
                  int iTexWidth = glConfig.vidWidth, iTexHeight =
     glConfig.vidHeight; if ( GL_TEXTURE_RECTANGLE_ARB ==
     GL_TEXTURE_RECTANGLE_NV )
                  ...
                  qglBegin(GL_QUADS);
                          ...
                          qglMultiTexCoord2fARB( GL_TEXTURE1_ARB, iTexWidth,
     iTexHeight ); qglMultiTexCoord2fARB( GL_TEXTURE0_ARB, glConfig.vidWidth,
     glConfig.vidHeight ); qglVertex2f( glConfig.vidWidth, 0 ); qglEnd();
                  ...
                  qglActiveTextureARB(GL_TEXTURE0_ARB );
                  qglTexEnvf( GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE
     ); qglDisable( GL_TEXTURE_RECTANGLE_ARB ); qglEnable( GL_TEXTURE_2D );
          }*/
