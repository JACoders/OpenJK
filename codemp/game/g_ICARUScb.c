...
//====================================================================================
//
// rww - ICARUS callback file, all that can be handled within vm's is handled in
// here.
//
//====================================================================================
...
// This is a hack I guess. It's because we can't include the file this enum is
// in unless we're using cpp. But we need it for the interpreter stuff. In any
// case, DO NOT modify this enum.
...
static char *Q3_GetAnimBoth(gentity_t *ent) {
  ...
  if (Q_stricmp(lowerName, upperName)) {
    ...
    G_DebugPrint(WL_WARNING, "Q3_GetAnimBoth: legs and torso animations did "
                             "not match : returning legs\n");
#endif
  }
...
int Q3_PlaySound(int taskID, int entID, const char *name, const char *channel) {
  ...
  soundChannel_t voice_chan =
      CHAN_VOICE; // set a default so the compiler doesn't bitch
  ...
  if ((Q_stricmp(channel, "CHAN_ANNOUNCER") == 0) ||
      (ent->classname &&
       Q_stricmp("target_scriptrunner", ent->classname) == 0)) {
    bBroadcast = qtrue;
  }

  // moved here from further down so I can easily check channel-type without
  // code dup...
  //
  if (Q_stricmp(channel, "CHAN_VOICE") == 0) {
    ...
    type_voice = qtrue;
  } else if (Q_stricmp(channel, "CHAN_VOICE_GLOBAL") ==
             0) // this should broadcast to everyone, put only casue animation
                // on G_SoundOnEnt...
  {
  ...
  // if we're in-camera, check for skipping cinematic and ifso, no subtitle
  // print (since screen is not being
  //	updated anyway during skipping). This stops leftover subtitles being
  //left onscreen after unskipping.
  //
  /*
  if (!in_camera ||
  ...
  {
          // Text on
          // certain NPC's we always want to use subtitles regardless of
  subtitle setting if (g_subtitles->integer == 1 || (ent->NPC &&
  (ent->NPC->scriptFlags & SCF_USE_SUBTITLES) ) ) // Show all text
          {
                  if ( in_camera)	// Cinematic
                  {
                          trap->SendServerCommand( -1, va("ct \"%s\" %i",
  finalName, soundHandle) );
                  }
                  else //if (precacheWav[i].speaker==SP_NONE)	//  lower screen
  text
                  {
                          sharedEntity_t		*ent2 =
  SV_GentityNum(0);
                          // the numbers in here were either the original ones
  Bob entered (350), or one arrived at from checking the distance Chell stands
  at in stasis2 by the computer core that was submitted as a bug report...
                          //
                          if (bBroadcast || (DistanceSquared(ent->currentOrigin,
  ent2->currentOrigin) < ((voice_chan == CHAN_VOICE_ATTEN)?(350 * 350):(1200 *
  1200)) ) )
                          {
                                  trap->SendServerCommand( -1, va("ct \"%s\"
  %i", finalName, soundHandle) );
                          }
                  }
          }
          // Cinematic only
          else if (g_subtitles->integer == 2) // Show only talking head text and
  CINEMATIC
          {
...
void Q3_Play(int taskID, int entID, const char *type, const char *name) {
  ...
  if (!Q_stricmp(type, "PLAY_ROFF")) {
    ...
    if (ent->roffid) {
      ...
      // rww - Maybe use pos1 and pos2? I don't think we need to care if these
      // values are sent across the net.
...
void anglerCallback(gentity_t *ent) {
  ...
  VectorMA(ent->s.apos.trBase, (ent->s.apos.trDuration * 0.001f),
           ent->s.apos.trDelta, ent->r.currentAngles);
...
void Blocked_Mover(gentity_t *ent, gentity_t *other) {
  ...
  if (other->s.eType == ET_ITEM) {
    // should we be doing anything special if a key blocks it... move it
    // somehow..?
  }
  // if your not a client, or your a dead client remove yourself...
  else if (other->s.number &&
           (!other->client ||
            (other->client && other->health <= 0 &&
             other->r.contents == CONTENTS_CORPSE && !other->message))) {
...
=============
*/
void Q3_Lerp2Pos(int taskID, int entID, vec3_t origin, vec3_t angles,
                 float duration) {
  gentity_t *ent = &g_entities[entID];
  vec3_t ang;
  int i;
...
void Q3_Kill(int entID, const char *name) {
  ...
  // rww - I guess this would only apply to NPCs anyway. I'm not going to
  // bother. if ( victim == ent )
  //{//don't ICARUS_FreeEnt me, I'm in the middle of a script!  (FIXME:
  //shouldn't ICARUS handle this internally?) 	victim->svFlags |=
  //SVF_KILLED_SELF;
  //}
...
void Q3_RemoveEnt(gentity_t *victim) {
  if (victim->client) {
    if (victim->s.eType != ET_NPC) {
      ...
      if (victim->client->NPC_class ==
          CLASS_VEHICLE) { // eject everyone out of a vehicle that's about to
                           // remove itself
    ...
    if ( victim->client->ps.saberEntityNum != ENTITYNUM_NONE &&
    victim->client->ps.saberEntityNum > 0 )
    {
...
int Q3_GetFloat(int entID, int type, const char *name, float *value) {
  ...
  toGet = GetIDForString(setTable,
                         name); // FIXME: May want to make a "getTable" as well
  ...
  case SET_PARM16:
    if (ent->parms == NULL) {
      ...
      return 0; // would prefer qfalse, but I'm fitting in with what's here
                // <sigh>
    }
  ...
  case SET_TIMESCALE: //## %f="0.0" # Speed-up slow down game (0 - 1.0)
    return 0;
    break;
  case SET_CAMERA_GROUP_Z_OFS: //## %s="NULL" # all ents with this cameraGroup
                               //will be focused on
    return 0;
    break;
  ...
  case SET_FACEAUX:   //## %f="0.0" # Set face to Aux expression for number of
                      //seconds
  case SET_FACEBLINK: //## %f="0.0" # Set face to Blink expression for number of
                      //seconds
  case SET_FACEBLINKFROWN: //## %f="0.0" # Set face to Blinkfrown expression for
                           //number of seconds
  case SET_FACEFROWN: //## %f="0.0" # Set face to Frown expression for number of
                      //seconds
  case SET_FACENORMAL: //## %f="0.0" # Set face to Normal expression for number
                       //of seconds
  ...
  case SET_WAIT: //## %f="0.0" # Change an entity's wait field
    *value = ent->wait;
    break;
  case SET_FOLLOWDIST: //## %f="0.0" # How far away to stay from leader in
                       //BS_FOLLOW_LEADER
    return 0;
    break;
  //# #sep ints
  case SET_ANIM_HOLDTIME_LOWER: //## %d="0" # Hold lower anim for number of
                                //milliseconds
    if (ent->client == NULL) {
      G_DebugPrint(WL_WARNING,
                   "Q3_GetFloat: SET_ANIM_HOLDTIME_LOWER, %s not a client\n",
                   ent->targetname);
      return 0;
    }
    *value = ent->client->ps.legsTimer;
    break;
  case SET_ANIM_HOLDTIME_UPPER: //## %d="0" # Hold upper anim for number of
                                //milliseconds
    if (ent->client == NULL) {
      G_DebugPrint(WL_WARNING,
                   "Q3_GetFloat: SET_ANIM_HOLDTIME_UPPER, %s not a client\n",
                   ent->targetname);
      return 0;
    }
    *value = ent->client->ps.torsoTimer;
    break;
  case SET_ANIM_HOLDTIME_BOTH: //## %d="0" # Hold lower and upper anims for
  ...
  case SET_VFOV: //## %d="0" # Vertical field of view
    return 0;
    break;
  case SET_DELAYSCRIPTTIME: //## %d="0" # How many seconds to wait before
                            //running delayscript
    return 0;
    break;
  ...
  case SET_SHOT_SPACING: //## %d="1000" # Time between shots for an NPC - reset
                         //to defaults when changes weapon
    return 0;
    break;
  case SET_MISSIONSTATUSTIME: //## %d="0" # Amount of time until Mission Status
  ...
  case SET_CHASE_ENEMIES: //## %t="BOOL_TYPES" # NPC will chase after enemies
    return 0;
    break;
  case SET_LOOK_FOR_ENEMIES: //## %t="BOOL_TYPES" # NPC will be on the lookout
                             //for enemies
    return 0;
    break;
  case SET_FACE_MOVE_DIR: //## %t="BOOL_TYPES" # NPC will face in the direction
                          //it's moving
    return 0;
    break;
  ...
  case SET_UNDYING: //## %t="BOOL_TYPES" # Can take damage down to 1 but not die
    return 0;
    break;
  case SET_NOAVOID: //## %t="BOOL_TYPES" # Will not avoid other NPCs or
                    //architecture
    return 0;
    break;
  ...
  case SET_SOLID: //## %t="BOOL_TYPES" # Make yourself notsolid or solid
    *value = ent->r.contents;
    break;
  case SET_PLAYER_USABLE: //## %t="BOOL_TYPES" # Can be activateby the player's
                          //"use" button
    *value = (ent->r.svFlags & SVF_PLAYER_USABLE);
    break;
  case SET_LOOP_ANIM: //## %t="BOOL_TYPES" # For non-NPCs: loop your animation
                      //sequence
    return 0;
    break;
  case SET_INTERFACE: //## %t="BOOL_TYPES" # Player interface on/off
    ...
    return 0;
    break;
  case SET_SHIELDS: //## %t="BOOL_TYPES" # NPC has no shields (Borg do not
                    //adapt)
    return 0;
    break;
  case SET_INVISIBLE: //## %t="BOOL_TYPES" # Makes an NPC not solid and not
                      //visible
    *value = (ent->s.eFlags & EF_NODRAW);
    break;
  case SET_VAMPIRE: //## %t="BOOL_TYPES" # Makes an NPC not solid and not
                    //visible
    return 0;
    break;
  case SET_FORCE_INVINCIBLE: //## %t="BOOL_TYPES" # Makes an NPC not solid and
                             //not visible
    return 0;
    break;
  ...
  case SET_PLAYER_LOCKED: //## %t="BOOL_TYPES" # Makes it so player cannot move
    return 0;
    break;
  case SET_LOCK_PLAYER_WEAPONS: //## %t="BOOL_TYPES" # Makes it so player cannot
                                //switch weapons
    return 0;
    break;
  case SET_NO_IMPACT_DAMAGE: //## %t="BOOL_TYPES" # Makes it so player cannot
                             //switch weapons
    return 0;
    break;
  case SET_NO_KNOCKBACK: //## %t="BOOL_TYPES" # Stops this ent from taking
                         //knockback from weapons
    *value = (ent->flags & FL_NO_KNOCKBACK);
    break;
  case SET_ALT_FIRE: //## %t="BOOL_TYPES" # Force NPC to use altfire when
                     //shooting
    return 0;
    break;
  case SET_NO_RESPONSE: //## %t="BOOL_TYPES" # NPCs will do generic responses
                        //when this is on (usescripts override generic responses
                        //as well)
    return 0;
    break;
  ...
  case SET_MISSIONSTATUSACTIVE: //# Turns on Mission Status Screen
    return 0;
    break;
  case SET_NO_COMBAT_TALK: //## %t="BOOL_TYPES" # NPCs will not do their combat
                           //talking noises when this is on
    return 0;
    break;
  case SET_NO_ALERT_TALK: //## %t="BOOL_TYPES" # NPCs will not do their combat
                          //talking noises when this is on
    return 0;
    break;
  case SET_USE_CP_NEAREST: //## %t="BOOL_TYPES" # NPCs will use their closest
                           //combat points, not try and find ones next to the
                           //player, or flank player
    return 0;
    break;
  case SET_DISMEMBERABLE: //## %t="BOOL_TYPES" # NPC will not be affected by
                          //force powers
    return 0;
    break;
  case SET_NO_FORCE:
  ...
  case SET_USE_SUBTITLES:
    return 0;
    break;
  case SET_NO_FALLTODEATH: //## %t="BOOL_TYPES" # NPC will not be affected by
                           //force powers
    return 0;
    break;
  case SET_MORELIGHT: //## %t="BOOL_TYPES" # NPCs will use their closest combat
                      //points, not try and find ones next to the player, or
                      //flank player
    return 0;
    break;
  case SET_TREASONED: //## %t="BOOL_TYPES" # Player has turned on his own-
                      //scripts will stop: NPCs will turn on him and level
                      //changes load the brig
    return 0;
    break;
  case SET_DISABLE_SHADER_ANIM: //## %t="BOOL_TYPES" # Shaders won't animate
    return 0;
    break;
  case SET_SHADER_ANIM: //## %t="BOOL_TYPES" # Shader will be under frame
                        //control
    return 0;
    break;
...
int Q3_GetVector(int entID, int type, const char *name, vec3_t value) {
  ...
  case SET_PARM15:
  case SET_PARM16:
    if (sscanf(ent->parms->parm[toGet - SET_PARM1], "%f %f %f", &value[0],
               &value[1], &value[2]) != 3) {
      G_DebugPrint(WL_WARNING,
                   "Q3_GetVector: failed sscanf on SET_PARM%d (%s)\n", toGet,
                   name);
      VectorClear(value);
    }
    break;
  ...
  case SET_TELEPORT_DEST: //## %v="0.0 0.0 0.0" # Set origin here as soon as the
                          //area is clear
    G_DebugPrint(WL_WARNING,
...
int Q3_GetString(int entID, int type, const char *name, char **value) {
  ...
  case SET_SPAWNSCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" #
                        //Script to run when spawned //0 - do not change these,
                        //these are equal to BSET_SPAWN, etc
    *value = ent->behaviorSet[BSET_SPAWN];
    break;
  case SET_USESCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" # Script
                      //to run when used
    *value = ent->behaviorSet[BSET_USE];
    break;
  case SET_AWAKESCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" #
                        //Script to run when startled
    *value = ent->behaviorSet[BSET_AWAKE];
    break;
  case SET_ANGERSCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" #
                        //Script run when find an enemy for the first time
    *value = ent->behaviorSet[BSET_ANGER];
    break;
  case SET_ATTACKSCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" #
                         //Script to run when you shoot
    *value = ent->behaviorSet[BSET_ATTACK];
    break;
  case SET_VICTORYSCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" #
                          //Script to run when killed someone
    *value = ent->behaviorSet[BSET_VICTORY];
    break;
  case SET_LOSTENEMYSCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" #
                            //Script to run when you can't find your enemy
    *value = ent->behaviorSet[BSET_LOSTENEMY];
    break;
  case SET_PAINSCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" # Script
                       //to run when hit
    *value = ent->behaviorSet[BSET_PAIN];
    break;
  case SET_FLEESCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" # Script
                       //to run when hit and low health
    *value = ent->behaviorSet[BSET_FLEE];
    break;
  case SET_DEATHSCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" #
                        //Script to run when killed
    *value = ent->behaviorSet[BSET_DEATH];
    break;
  case SET_DELAYEDSCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" #
                          //Script to run after a delay
    *value = ent->behaviorSet[BSET_DELAYED];
    break;
  case SET_BLOCKEDSCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" #
                          //Script to run when blocked by teammate
    *value = ent->behaviorSet[BSET_BLOCKED];
    break;
  case SET_FFIRESCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" #
                        //Script to run when player has shot own team repeatedly
    *value = ent->behaviorSet[BSET_FFIRE];
    break;
  case SET_FFDEATHSCRIPT: //## %s="NULL" !!"W:\game\base\scripts\!!#*.txt" #
  ...
  case SET_PAINTARGET: //## %s="NULL" # Set/change what to use when hit
    return 0;
    break;
  case SET_CAMERA_GROUP: //## %s="NULL" # all ents with this cameraGroup will be
                         //focused on
    return 0;
    break;
  case SET_CAMERA_GROUP_TAG: //## %s="NULL" # all ents with this cameraGroup
                             //will be focused on
    return 0;
    break;
  case SET_LOOK_TARGET: //## %s="NULL" # object for NPC to look at
    ...
    break;
  case SET_TARGET2: //## %s="NULL" # Set/change your target2: on NPC's: this
                    //fires when they're knocked out by the red hypo
    return 0;
    break;

  case SET_REMOVE_TARGET: //## %s="NULL" # Target that is fired when someone
  ...
  case SET_SCROLLTEXTCOLOR: //## %s=""  # Color of text RED:WHITE:BLUE: YELLOW
    ...
    return 0;
    break;
  case SET_COPY_ORIGIN: //## %s="targetname"  # Copy the origin of the ent with
                        //targetname to your origin
    G_DebugPrint(WL_WARNING, "Q3_GetString: SET_COPY_ORIGIN not implemented\n");
    return 0;
    break;
  case SET_DEFEND_TARGET: //## %s="targetname"  # This NPC will attack the
                          //target NPC's enemies
    G_DebugPrint(WL_WARNING, "Q3_GetString: SET_COPY_ORIGIN not implemented\n");
    return 0;
    break;
  case SET_VIDEO_PLAY: //## %s="filename" !!"W:\game\base\video\!!#*.roq" # Play
                       //a Video (inGame)
    G_DebugPrint(WL_WARNING, "Q3_GetString: SET_VIDEO_PLAY not implemented\n");
    return 0;
    break;
  case SET_LOADGAME: //## %s="exitholodeck" # Load the savegame that was
                     //auto-saved when you started the holodeck
    G_DebugPrint(WL_WARNING, "Q3_GetString: SET_LOADGAME not implemented\n");
    return 0;
    break;
  case SET_LOCKYAW: //## %s="off"  # Lock legs to a certain yaw angle (or "off"
                    //or "auto" uses current)
...
Copies passed origin to ent running script once there is nothing there blocking
the spot
=============
*/
...
void Q3_Lerp2Origin(int taskID, int entID, vec3_t origin, float duration) {
  ...
  InitMoverTrData(ent); // FIXME: This will probably break normal things that
                        // are being moved...
...
static void Q3_SetEnemy(int entID, const char *name) {
  ...
  if (!Q_stricmp("NONE", name) || !Q_stricmp("NULL", name)) {
    ...
    {
            //G_DebugPrint( WL_ERROR, "Q3_SetEnemy: ERROR - desired enemy has
    health %d\n", enemy->health ); return;
    }*/
    else {
      if (ent->NPC) {
...
static void Q3_SetLeader(int entID, const char *name) {
  ...
  if (!Q_stricmp("NONE", name) || !Q_stricmp("NULL", name)) {
    ...
    if (leader == NULL) {
      ...
      // G_DebugPrint( WL_ERROR,"Q3_SetEnemy: ERROR - desired enemy has health
      // %d\n", enemy->health );
      return;
    } else {
...
static qboolean Q3_SetNavGoal(int entID, const char *name) {
  ...
  if (!ent->NPC->tempGoal) {
    G_DebugPrint(WL_ERROR,
                 "Q3_SetNavGoal: tried to set a navgoal (\"%s\") on a dead "
                 "NPC: \"%s\"\n",
                 name, ent->script_targetname);
    return qfalse;
  }
  if (!ent->NPC->tempGoal->inuse) {
    G_DebugPrint(WL_ERROR,
                 "Q3_SetNavGoal: NPC's (\"%s\") navgoal is freed: \"%s\"\n",
                 name, ent->script_targetname);
    return qfalse;
  }
  if (Q_stricmp("null", name) == 0 || Q_stricmp("NULL", name) == 0) {
    ...
    if (TAG_GetOrigin2(NULL, name, goalPos) == qfalse) {
      ...
      if (!targ) {
        ...
        ent->NPC->goalRadius = sqrt(ent->r.maxs[0] + ent->r.maxs[0]) +
                               sqrt(targ->r.maxs[0] + targ->r.maxs[0]);
...
static void SetLowerAnim(int entID, int animID) {
  ...
  G_SetAnim(ent, NULL, SETANIM_LEGS, animID,
...
static void SetUpperAnim(int entID, int animID) {
  ...
  G_SetAnim(ent, NULL, SETANIM_TORSO, animID,
            SETANIM_FLAG_RESTART | SETANIM_FLAG_HOLD | SETANIM_FLAG_OVERRIDE,
            0);
}
...
static qboolean Q3_SetAnimUpper(int entID, const char *anim_name) {
  ...
  if (animID == -1) {
    G_DebugPrint(WL_WARNING,
                 "Q3_SetAnimUpper: unknown animation sequence '%s'\n",
                 anim_name);
    return qfalse;
  }
...
static qboolean Q3_SetAnimLower(int entID, const char *anim_name) {
  ...
  if (animID == -1) {
    G_DebugPrint(WL_WARNING,
                 "Q3_SetAnimLower: unknown animation sequence '%s'\n",
                 anim_name);
    return qfalse;
  }
...
static void Q3_SetAnimHoldTime(int entID, int int_data, qboolean lower) {
  ...
  if ( !ent->client )
  {
          G_DebugPrint( WL_ERROR, "Q3_SetAnimHoldTime: ent %d is NOT a player or
  NPC!\n", entID); return;
  }
...
static void Q3_SetHealth(int entID, int data) {
  ...
  if (ent->client->ps.stats[STAT_HEALTH] >
  ...
  if (data == 0) {
    ent->health = 1;
    if (ent->client->sess.sessionTeam == TEAM_SPECTATOR) { // this would be
                                                           // silly
      return;
    }
...
static qboolean Q3_SetBState(int entID, const char *bs_name) {
  ...
  if (bSID != (bState_t)-1) {
    if (bSID == BS_SEARCH || bSID == BS_WANDER) {
      ...
      if (ent->waypoint != WAYPOINT_NONE) {
        ...
        else if( ent->lastValidWaypoint >=0 && ent->lastValidWaypoint <
        num_waypoints )
        {
    ...
    if (ent->NPC->behaviorState == BS_NOCLIP &&
        bSID !=
            BS_NOCLIP) { // need to rise up out of the floor after noclipping
  ...
  /*
          if ( bSID == BS_FACE || bSID == BS_POINT_AND_SHOOT || bSID ==
     BS_FACE_ENEMY )
          {
...
static void Q3_SetDYaw(int entID, float data) {
  ...
  if (!ent->enemy) { // don't mess with this if they're aiming at someone
...
static void Q3_SetViewTarget(int entID, const char *name) {
  ...
  // FIXME: should we set behavior to BS_FACE and keep facing this ent as it
  // moves around for a script-specified length of time...?
...
static void Q3_SetWatchTarget(int entID, const char *name) {
  ...
  if (Q_stricmp("NULL", name) == 0 || Q_stricmp("NONE", name) == 0 ||
      (self->targetname &&
       (Q_stricmp(self->targetname, name) == 0))) { // clearing watchTarget
  ...
  if (watchTarget == NULL) {
    G_DebugPrint(WL_WARNING,
                 "Q3_SetWatchTarget: can't find WatchTarget: '%s'\n", name);
    return;
  }
...
static float Q3_GameSideCheckStringCounterIncrement(const char *string) {
  ...
  if (string[0] == '+') { // We want to increment whatever the value is by
...
static void Q3_SetForcePowerLevel(int entID, int forcePower, int forceLevel) {
  ...
  if (!self->client) {
    G_DebugPrint(WL_ERROR,
                 "Q3_SetForcePowerLevel: ent %s is not a player or NPC\n",
                 self->targetname);
    return;
  }
...
void Q3_SetParm(int entID, int parmNum, const char *parmValue) {
  ...
  if ((val = Q3_GameSideCheckStringCounterIncrement(parmValue))) {
    ...
    Com_sprintf(ent->parms->parm[parmNum], sizeof(ent->parms->parm[parmNum]),
                "%f", val);
  } else { // Just copy the string
    // copy only 16 characters
    strncpy(ent->parms->parm[parmNum], parmValue,
            sizeof(ent->parms->parm[parmNum]));
    // set the last character to null in case we had to truncate their passed
    // string
    if (ent->parms->parm[parmNum][sizeof(ent->parms->parm[parmNum]) - 1] !=
        0) { // Tried to set a string that is too long
...
static void Q3_SetCaptureGoal(int entID, const char *name) {
  ...
  if (goal == NULL) {
    G_DebugPrint(WL_ERROR,
                 "Q3_SetCaptureGoal: can't find CaptureGoal target: '%s'\n",
                 name);
    return;
  }
...
=============
*/
static void Q3_SetEvent(int entID,
                        const char *event_name) { // rwwFIXMEFIXME: Use in MP?
  G_DebugPrint(
      WL_WARNING,
...
static void Q3_SetFuncUsableVisible(int entID, qboolean visible) {
  ...
  // Yeah, I know that this doesn't even do half of what the func_usable use
  // code does, but if I've got two things on top of each other...and only
  //	one is visible at a time....and neither can ever be used......and
  //finally, the shader on it has the shader_anim stuff going on....It doesn't
  //seem 	like I can easily use the other version without nasty side effects.
  if (visible) {
...
static qboolean Q3_SetSolid(int entID, qboolean solid) {
  ...
  if (solid) { // FIXME: Presumption
    ...
    if (ent->s.eFlags &
        EF_NODRAW) { // We're invisible too, so set contents to none
      ent->r.contents = 0;
    } else {
...
static void Q3_SetLockAngle(int entID, const char *lockAngle) {
  ...
  G_DebugPrint(WL_WARNING, "Q3_SetLockAngle is not currently available. Ask if "
                           "you really need it.\n");
  /*
...
static void Q3_LookTarget(int entID, char *targetName) {
  ...
  if (Q_stricmp("none", targetName) == 0 ||
      Q_stricmp("NULL", targetName) == 0) { // clearing look target
...
============
*/
static qboolean Q3_SetBehaviorSet(int entID, int toSet,
  ...
  if (!Q_stricmp("NULL", scriptname)) {
    ...
    if (scriptname) {
      ...
      ent->behaviorSet[bSet] =
          G_NewString((char *)scriptname); // FIXME: This really isn't good...
    }
...
static void Q3_SetSaberActive(int entID, qboolean active) {
  ...
  // fixme: Take into account player being in state where saber won't toggle?
  // For now we simply won't care.
...
qboolean Q3_Set(int taskID, int entID, const char *type_name,
                const char *data) {
  gentity_t *ent = &g_entities[entID];
  float float_data;
  int int_data, toSet;
  ...
  // TODO: Throw in a showscript command that will list each command and what
  // they're doing... 		maybe as simple as printing that line of the script to the
  //console preceeded by the person's name? 		showscript can take any number of
  //targetnames or "all"?  Groupname?
  switch (toSet) {
  case SET_ORIGIN:
    if (sscanf(data, "%f %f %f", &vector_data[0], &vector_data[1],
               &vector_data[2]) != 3) {
  ...
  case SET_TELEPORT_DEST:
    if (sscanf(data, "%f %f %f", &vector_data[0], &vector_data[1],
               &vector_data[2]) != 3) {
      G_DebugPrint(WL_WARNING,
                   "Q3_Set: failed sscanf on SET_TELEPORT_DEST (%s)\n",
                   type_name);
      VectorClear(vector_data);
    }
  ...
  case SET_ANIM_UPPER:
    if (Q3_SetAnimUpper(entID, (char *)data)) {
      Q3_TaskIDClear(
          &ent->taskID[TID_ANIM_BOTH]); // We only want to wait for the top
  ...
  case SET_ANIM_LOWER:
    if (Q3_SetAnimLower(entID, (char *)data)) {
      Q3_TaskIDClear(
          &ent->taskID[TID_ANIM_BOTH]); // We only want to wait for the bottom
  ...
  case SET_PLAYER_TEAM:
    G_DebugPrint(WL_WARNING, "Q3_SetPlayerTeam: Not in MP ATM, let a "
                             "programmer (ideally Rich) know if you need it\n");
    break;
  ...
  case SET_DPITCH: // FIXME: make these set tempBehavior to BS_FACE and await
                   // completion?  Or set lockedDesiredPitch/Yaw and aimTime?
    float_data = atof((char *)data);
  ...
  case SET_ICARUS_UNFREEZE:
    Q3_SetICARUSFreeze(entID, (char *)data,
                       (qboolean)(toSet == SET_ICARUS_FREEZE));
    break;
  ...
  case SET_VIDEO_PLAY:
    // don't do this check now, James doesn't want a scripted cinematic to also
    // skip any Video cinematics as well,
    //	the "timescale" and "skippingCinematic" cvars will be set back to normal
    //in the Video code, so doing a 	skip will now only skip one section of a
    //multiple-part story (eg VOY1 bridge sequence)
    //
