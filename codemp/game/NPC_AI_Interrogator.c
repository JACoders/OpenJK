...
void Interrogator_PartsMove(void) {
  ...
  if (TIMER_Done(NPCS.NPC, "syringeDelay")) {
    ...
    //NPC->genericBone1, NPC->pos1, BONE_ANGLES_POSTMULT, POSITIVE_X,
    //NEGATIVE_Y, NEGATIVE_Z, NULL );
  ...
  if (TIMER_Done(NPCS.NPC, "scalpelDelay")) {
    ...
    //	trap->G2API_SetBoneAnglesIndex( &NPC->ghoul2[NPC->playerModel],
    //NPC->genericBone2, NPC->pos2, BONE_ANGLES_POSTMULT, POSITIVE_X,
    //NEGATIVE_Y, NEGATIVE_Z, NULL );
  ...
  // trap->G2API_SetBoneAnglesIndex( &NPC->ghoul2[NPC->playerModel],
  // NPC->genericBone3, NPC->pos3, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y,
  // NEGATIVE_Z, NULL );
...
void Interrogator_Strafe(void) {
  ...
  if (tr.fraction > 0.9f) {
    ...
    if (NPCS.NPC->enemy) {
      // Find the height difference
      dif = (NPCS.NPC->enemy->r.currentOrigin[2] + 32) -
            NPCS.NPC->r.currentOrigin[2];
...
void Interrogator_Hunt(qboolean visible, qboolean advance) {
  ...
  if (NPCS.NPCInfo->standTime < level.time) {
    ...
    if (visible) {
      Interrogator_Strafe();
      if (NPCS.NPCInfo->standTime > level.time) { // successfully strafed
        return;
      }
    }
  }
...
void Interrogator_Melee(qboolean visible, qboolean advance) {
  if (TIMER_Done(NPCS.NPC, "attackDelay")) // Attack?
  {
    ...
    if (NPCS.NPC->r.currentOrigin[2] >=
            NPCS.NPC->enemy->r.currentOrigin[2] + NPCS.NPC->enemy->r.mins[2] &&
        NPCS.NPC->r.currentOrigin[2] + NPCS.NPC->r.mins[2] + 8 <
            NPCS.NPC->enemy->r.currentOrigin[2] + NPCS.NPC->enemy->r.maxs[2]) {
      // gentity_t *tent;

      TIMER_Set(NPCS.NPC, "attackDelay", Q_irand(500, 3000));
      G_Damage(NPCS.NPC->enemy, NPCS.NPC, NPCS.NPC, 0, 0, 2,
      ...
      //			tent = G_TempEntity(
      //NPC->enemy->r.currentOrigin, EV_DRUGGED ); 			tent->owner = NPC->enemy;
