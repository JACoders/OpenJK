...
void R_BindAnimatedImage(const textureBundle_t *bundle) {
  ...
  if ((r_fullbright->integer || tr.refdef.doLAGoggles ||
       (tr.refdef.rdflags & RDF_doFullbright)) &&
      bundle->isLightmap) {
...
//--EF_old dlight code...reverting back to Quake III dlight to see if people
//like that better
// Lifted the whole function because someone hacked the heck out of this and it
// doesn't seem to
...
static void ProjectDlightTexture( void ) {
        ...
        for ( l = 0 ; l < backEnd.refdef.num_dlights ; l++ ) {
                ...
                if ( !( tess.dlightBits & ( 1 << l ) ) ) {
                        continue;	// this surface definately doesn't have
any of this light
                }
...
+= 4 ) { vec3_t	distVec; int		clip; float	tempColor; float
modulate, dist;

//			if ( 0 ) {
//				clipBits[i] = 255;	// definately not
dlighted
//				continue;
//			}
//
                        ...
                        texCoords[0] = 0.5 + distVec[0] * scale;	//xy
projection texCoords[1] = 0.5 + distVec[1] * scale;
...
add light
                ...
                GL_State( GLS_SRCBLEND_SRC_ALPHA | GLS_DSTBLEND_SRC_COLOR |
GLS_DEPTHFUNC_EQUAL);//our way
//		GL_State( GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ONE |
GLS_DEPTHFUNC_EQUAL );	//Id way R_DrawElements( numIndexes, hitIndexes );
...
static void ProjectDlightTexture2(void) {
  ...
  for (l = 0; l < backEnd.refdef.num_dlights; l++) {
    ...
    for (i = 0; i < tess.numIndexes; i += 3) {
      ...
      // rjr - removed for hacking 			if (
      // (!r_dlightBacks->integer &&
      // DotProduct(normal,origin)-DotProduct(normal,posa) <= 0.0f) || //
      // backface
      if (DotProduct(normal, origin) - DotProduct(normal, posa) <=
              0.0f ||                         // backface
      ...
      if ((texCoordsArray[numIndexes][0] < 0.0f &&
           texCoordsArray[numIndexes + 1][0] < 0.0f &&
           texCoordsArray[numIndexes + 2][0] < 0.0f) ||
          (texCoordsArray[numIndexes][0] > 1.0f &&
           texCoordsArray[numIndexes + 1][0] > 1.0f &&
           texCoordsArray[numIndexes + 2][0] > 1.0f) ||
          (texCoordsArray[numIndexes][1] < 0.0f &&
           texCoordsArray[numIndexes + 1][1] < 0.0f &&
           texCoordsArray[numIndexes + 2][1] < 0.0f) ||
          (texCoordsArray[numIndexes][1] > 1.0f &&
           texCoordsArray[numIndexes + 1][1] > 1.0f &&
           texCoordsArray[numIndexes + 2][1] > 1.0f)) {
...
#ifndef JK2_MODE
    // don't have fog enabled when we redraw with alpha test, or it will double
    // over and screw the tri up -rww
    ...
    if (tess.shader && qglActiveTextureARB) {
      ...
      while (i < tess.shader->numUnfoggedPasses) {
        ...
        if (((tess.shader->stages[i].bundle[0].image &&
              ...
              tess.shader->stages[i].bundle[0].tcGen !=
             ...
             (tess.shader->stages[i].bundle[1].image &&
              ...
              tess.shader->stages[i].bundle[1].tcGen !=
                  TCGEN_ENVIRONMENT_MAPPED &&
              tess.shader->stages[i].bundle[1].tcGen != TCGEN_FOG)) &&
            (tess.shader->stages[i].stateBits & blendBits) ==
                0) { // only use non-lightmap opaque stages
    ...
    if (dStage) {
      ...
      if (dStage->bundle[0].image && !dStage->bundle[0].isLightmap &&
      ...
      GL_State(GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE |
               GLS_DEPTHFUNC_EQUAL); // | GLS_ATEST_GT_0);
...
static void ProjectDlightTexture(void) {
  ...
  for (l = 0; l < backEnd.refdef.num_dlights; l++) {
    ...
    for (i = 0; i < tess.numVertexes; i++, texCoords += 2, colors += 4) {
      ...
      if (VectorCompare(tess.normal[i], vec3_origin)) { // damn you terrain!
        bestIndex = 2;
      } else {
        while (l < 3) {
      ...
      if (bestIndex == 2) {
        ...
        if (VectorCompare(tess.normal[i], vec3_origin) ||
            ...
            tess.normal[i][1] <
                -lightScaleTolerance) { // if not perfectly flat, we must use a
                                        // constant dist
          scale = 1.0f / radius;
        } else {
...
===============
*/
static void ComputeColors(shaderStage_t *pStage, alphaGen_t forceAlphaGen,
                          colorGen_t forceRGBGen) {
  int i;

  if (tess.shader != tr.projectionShadowShader &&
      tess.shader != tr.shadowShader &&
    ...
    // We've done some custom alpha and color stuff, so we can skip the rest.
    // Let it do fog though
  ...
  if (backEnd.currentEntity->e.renderfx &
      RF_VOLUMETRIC) // does not work for rotated models, technically, this
                     // should also be a CGEN type, but that would entail adding
                     // new shader commands....which is too much work for one
                     // thing
  {
    int i;
    float *normal, dot;
    ...
    for (i = 0; i < numVertexes; i++, normal += 4, color += 4) {
      ...
      color[0] = color[1] = color[2] = color[3] =
          Q_ftol(backEnd.currentEntity->e.shaderRGBA[0] * (1 - dot));
    }
  ...
  case CGEN_LIGHTING_DIFFUSE_ENTITY:
    ...
    if (forceAlphaGen == AGEN_IDENTITY &&
      ...
      forceAlphaGen = AGEN_SKIP; // already got it in this set since it does all
                                 // 4 components
    }
    break;
  ...
  case CGEN_LIGHTMAPSTYLE:
    for (i = 0; i < tess.numVertexes; i++) {
      byteAlias_t *baDest = (byteAlias_t *)&tess.svars.colors[i],
                  *baSource =
                      (byteAlias_t *)&styleColors[pStage->lightmapStyle];
  ...
  case AGEN_LIGHTING_SPECULAR:
    ...
    break;
  case AGEN_ENTITY:
    if (forceRGBGen != CGEN_ENTITY) { // already got it in the CGEN_entity since
                                      // it does all 4 components
...
static void ComputeTexCoords(shaderStage_t *pStage) {
  ...
  for (b = 0; b < NUM_TEXTURE_BUNDLES; b++) {
    ...
    for (tm = 0; tm < pStage->bundle[b].numTexMods; tm++) {
      ...
      case TMOD_SCROLL:
        RB_CalcScrollTexCoords(pStage->bundle[b]
                                   .texMods[tm]
                                   .translate, // union scroll into translate
                               (float *)tess.svars.texcoords[b]);
        break;

      case TMOD_SCALE:
        RB_CalcScaleTexCoords(pStage->bundle[b]
                                  .texMods[tm]
                                  .translate, // union scroll into translate
                              (float *)tess.svars.texcoords[b]);
        break;
      ...
      case TMOD_ROTATE:
        RB_CalcRotateTexCoords(
            pStage->bundle[b]
                .texMods[tm]
                .translate[0], // union rotateSpeed into translate[0]
            (float *)tess.svars.texcoords[b]);
        break;
...
static void RB_IterateStagesGeneric(shaderCommands_t *input) {
  ...
  if (tess.fogNum && tess.shader->fogPass &&
      (tess.fogNum == tr.world->globalFog ||
       tess.fogNum == tr.world->numfogs) &&
      r_drawfog->value == 2) { // only gl fog global fog and the "special fog"
    fog = tr.world->fogs + tess.fogNum;
    ...
    if (tr.rangedFog) { // ranged fog, used for sniper scope
      ...
      if (tr.rangedFog < 0.0f) { // special designer override
        fStart = -tr.rangedFog;
        ...
        // the greater tr.rangedFog is, the more fog we will get between the
        // view point and cull distance
        if ((tr.distanceCull - fStart) <
            tr.rangedFog) { // assure a minimum range between fog beginning and
                            // cutoff distance
  ...
  for (stage = 0; stage < input->shader->numUnfoggedPasses; stage++) {
    ...
    if (stage && r_lightmap->integer) {
      if (!(pStage->bundle[0].isLightmap || pStage->bundle[1].isLightmap ||
        ...
        stateBits = (GLS_DSTBLEND_ZERO |
                     GLS_SRCBLEND_ONE); // we want to replace the prior stages
                                        // with this LM, not blend
      }
    }
    ...
    if (backEnd.currentEntity) {
      if (backEnd.currentEntity->e.renderfx & RF_DISINTEGRATE1) {
        ...
        // and by doing the depth-testing it avoids some kinds of artefacts, but
        // will probably introduce others?
      ...
      if (backEnd.currentEntity->e.renderfx &
          RF_RGB_TINT) { // want to use RGBGen from ent
    ...
    if (!input->fading) { // this means ignore this, while we do a fade-out
    ...
    if (pStage->bundle[1].image != 0) {
      ...
      if ((tess.shader == tr.distortionShader) ||
          (backEnd.currentEntity &&
           (backEnd.currentEntity->e.renderfx &
            RF_DISTORTION))) { // special distortion effect -rww
        // tr.screenImage should have been set for this specific entity before
        // we got in here.
        GL_Bind(tr.screenImage);
        GL_Cull(CT_TWO_SIDED);
      } else if (pStage->bundle[0].vertexLightmap && (r_vertexLight->integer) &&
                 r_lightmap->integer) {
        GL_Bind(tr.whiteImage);
      } else
        R_BindAnimatedImage(&pStage->bundle[0]);
      ...
      if (tess.shader == tr.distortionShader &&
          glConfig.stencilBits >= 4) { // draw it to the stencil buffer!
        tr_stencilled = true;
        lStencilled = true;
      ...
      if (lStencilled) { // re-enable the color buffer, disable stencil test
        lStencilled = false;
  ...
  if (r_surfaceSprites->integer) {
    for (stage = 1; stage < tess.shader->numUnfoggedPasses; stage++) {
      if (tess.xstages[stage].ss &&
          tess.xstages[stage].ss->surfaceSpriteType) { // Draw the surfacesprite
