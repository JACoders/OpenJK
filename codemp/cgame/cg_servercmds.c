...
static void CG_ParseTeamInfo( void ) {
	...
	{
		cgi.Error( ERR_DROP, "CG_ParseTeamInfo: numSortedTeamPlayers out of range (%d)", numSortedTeamPlayers );
		return;
	}
	...
	for ( i = 0 ; i < numSortedTeamPlayers ; i++ ) {
		...
		{
			cgi.Error( ERR_DROP, "CG_ParseTeamInfo: bad client number: %d", client );
			return;
		}
...
static void CG_BodyQueueCopy(centity_t *cent, int clientNum, int knownWeapon)
{
	...
	{
		cgi.G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, knownWeapon), 0, cent->ghoul2, 1);
	}
...
static void CG_ServerCommand( void ) {
	...
	if ( !strcmp( cmd, "spd" ) ) 
	{
		...
		cgi.SP_Print(holdInt, (byte *)string);
		return;
	}
#endif
	...
	if ( !strcmp( cmd, "lchat" ) ) {
		if ( !cg_teamChatsOnly.integer ) {
			...
			if (loc[0] == '@')
			{ //get localized text
				cgi.SP_GetStringTextString(loc+1, loc, sizeof( loc ) );
			}
	...
	cgi.Print( "Unknown client game command: %s\n", cmd );
}
