...
qboolean NPC_CheckSurrender(void) {
	...
	if (!Q3_TaskIDPending(NPC, TID_MOVE_NAV) && NPC->client->ps.groundEntityNum != ENTITYNUM_NONE && !NPC->client->ps.weaponTime &&
		!PM_InKnockDown(&NPC->client->ps) && NPC->enemy && NPC->enemy->client && NPC->enemy->enemy == NPC && NPC->enemy->s.weapon != WP_NONE &&
		NPC->enemy->s.weapon != WP_MELEE && NPC->enemy->health > 20 && NPC->enemy->painDebounceTime < level.time - 3000 &&
		...
		if (NPC->s.weapon != WP_ROCKET_LAUNCHER && NPC->s.weapon != WP_REPEATER && NPC->s.weapon != WP_FLECHETTE &&
			...
			if (NPCInfo->group && NPCInfo->group->numGroup <= 1) { // I'm alone but I was in a group//FIXME: surrender anyway if just melee or no weap?
				if (NPC->s.weapon == WP_NONE
					...
					if (NPC->enemy == player) { // player is the guy I'm running from
						...
						if (InFOV(NPC, NPC->enemy, 30, 30)) {											 // they're looking at me
							if (DistanceSquared(NPC->currentOrigin, NPC->enemy->currentOrigin) < 4096) { // they're close
		...
		if (NPC->waypoint != WAYPOINT_NONE) {
			...
			if (numEdges != WAYPOINT_NONE) {
				...
				for (int branchNum = 0; branchNum < numEdges; branchNum++) {
					...
					if (DotProduct(runDir, dangerDir) > Q_flrand(0, 0.5)) { // don't run toward danger
						continue;
					}
