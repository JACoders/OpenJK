...
void SP_info_player_deathmatch(gentity_t *ent) {
  ...
  if (ent->spawnflags & 32) // STUN_BATON
  {
    ...
    G_SkinIndex(
        "models/players/kyle/model_fpls2.skin"); // preache the skin used in
                                                 // cg_players.cpp
  }
}

/*QUAKED info_player_start (1 0 0) (-16 -16 -24) (16 16 32) KEEP_PREV
DROPTOFLOOR x x x STUN_BATON NOWEAPON x KEEP_PREV - keep previous
health/ammo/etc DROPTOFLOOR - Player will start on the first solid structure
under it STUN_BATON - Gives player the stun baton and bryar pistol, but not the
saber, plus any weapons they may have carried over from previous levels.
...
void SP_info_player_start(gentity_t *ent) {
  ...
  ent->spawnflags |=
      1; // James suggests force-ORing the KEEP_PREV flag in for now
...
gentity_t *SelectSpawnPoint(vec3_t avoidPoint, team_t team, vec3_t origin,
  ...
  if (spot->spawnflags & 2) {
    ...
    gi.trace(&tr, spot->s.origin, playerMins, playerMaxs, origin,
             ENTITYNUM_NONE, MASK_PLAYERSOLID, G2_NOCOLLIDE, 0);
  ...
  if (client) // though I can't see it not being true...
  {
                   ...
                   // sscanf() works, but this is clearer
    {
      sscanf(s, "%i %i %i %i %i %i %i %f %f %f %i %i %i %i %i %i",
             ...
             &client->ps.forcePowersKnown, &client->ps.forcePower, &saberActive,
             &client->ps.saberAnimLevel, &client->ps.saberLockEnemy,
...
void G_SetSkin(gentity_t *ent, const char *modelName, const char *customSkin) {
  ...
  // FIXME: is have an alternate skin (in modelName, after '/'), replace
  // "default" with that skin name
  if (!customSkin) {
...
qboolean G_StandardHumanoid(const char *modelName) {
  ...
  if (!Q_stricmp("kyle", modelName) || !Q_strncmp("st", modelName, 2) ||
      !Q_strncmp("imp", modelName, 3) || !Q_strncmp("gran", modelName, 4) ||
      ...
      !Q_strncmp("prisoner", modelName, 8) || !Q_stricmp("tavion", modelName) ||
      !Q_stricmp("desann", modelName) || !Q_stricmp("trandoshan", modelName) ||
      !Q_stricmp("jan", modelName) || !Q_stricmp("luke", modelName) ||
      !Q_stricmp("lando", modelName) || !Q_stricmp("reelo", modelName) ||
      !Q_stricmp("bartender", modelName) ||
      !Q_stricmp("monmothma", modelName) || !Q_stricmp("chiss", modelName) ||
...
qboolean G_SetG2PlayerModelInfo(gentity_t *ent, const char *modelName,
  ...
  if (ent->playerModel != -1) { // we found the model ok
    ...
    // on.
    if (G_StandardHumanoid(modelName)) { // temp hack because only the gran are
                                         // set up right so far
      ent->headBolt =
      ...
      if (!Q_stricmp("gonk", modelName) || !Q_stricmp("seeker", modelName) ||
          !Q_stricmp("remote", modelName) || !Q_strncmp("r2d2", modelName, 4) ||
          !Q_strncmp("r5d2", modelName,
      ...
      */
                     ...
                     modelName)) //&& (ent->client->playerTeam != TEAM_PLAYER))
      {
        ...
        ent->genericBolt1 =
            gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "*antenna_effect");
        ent->headBolt =
            gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "*head_eyes");
        ent->torsoBolt =
            gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "torso");
        ent->crotchBolt =
            gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "hips");
        ent->handRBolt =
            gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "*flasha");
        ent->genericBolt3 =
            gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "*flashb");
        ent->handLBolt =
            gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel], "*flashc");
  ...
  // set the weaponmodel to -1 so we don't try and remove it in Pmove before we
  // have it built
  ent->weaponModel = -1;
...
void G_SetG2PlayerModel(gentity_t *const ent, const char *modelName,
  ...
  // NOTE: for some reason, it still loads the default skin's tga's?  Because
  // they're referenced in the .glm?
  ...
  if (!Q_stricmp("kyle", modelName)) {
    // Try to get the skin we'll use when we switch to the first person light
    // saber.
    //	We use a new skin to disable certain surfaces so they are not drawn but
    //we can still collide against them
...
void G_ActivatePersonalShield(gentity_t *ent) {
  ...
  ent->client->ps.powerups[PW_BATTLESUIT] =
      Q3_INFINITE; // Doesn't go away until armor does
}
...
void G_PilotXWing(gentity_t *ent) {
  ...
  if (ent->client->ps.vehicleModel != 0) {
    ...
    ent->client->ps.vehicleModel =
        G_ModelIndex("models/map_objects/ships/x_wing.md3");

    ent->client->ps.stats[STAT_WEAPONS] |= (1 << WP_ATST_SIDE);
    ent->client->ps.ammo[weaponData[WP_ATST_SIDE].ammoIndex] =
        ammoData[weaponData[WP_ATST_SIDE].ammoIndex].max;
    gitem_t *item = FindItemForWeapon(WP_ATST_SIDE);
    RegisterItem(
        item); // make sure the weapon is cached in case this runs at startup
...
void G_DriveATST(gentity_t *ent, gentity_t *atst) {
  if (ent->NPC_type && ent->client &&
      (ent->client->NPC_class == CLASS_ATST)) { // already an atst, switch back
    // open hatch
    ...
    { gi.cvar_set("cg_thirdperson", "0"); }
    cg.overrides.active &=
        ~(CG_OVERRIDE_3RD_PERSON_RNG | CG_OVERRIDE_3RD_PERSON_VOF |
          CG_OVERRIDE_3RD_PERSON_POF | CG_OVERRIDE_3RD_PERSON_APH);
    cg.overrides.thirdPersonRange = cg.overrides.thirdPersonVertOffset =
    ...
    ent->client->ps.stats[STAT_WEAPONS] |=
        (1 << WP_ATST_MAIN) | (1 << WP_ATST_SIDE);
    ent->client->ps.ammo[weaponData[WP_ATST_MAIN].ammoIndex] =
    ...
    CG_RegisterItemVisuals((item - bg_itemlist));
    // HACKHACKHACKTEMP
    // FIXME: these get lost in load/save!  Must use variables that are set
    // every frame or saved/loaded camera
    gi.cvar_set("cg_thirdperson", "1");
    cg.overrides.active |= CG_OVERRIDE_3RD_PERSON_RNG;
    cg.overrides.thirdPersonRange = 240;
...
qboolean ClientSpawn(gentity_t *ent,
      ...
      g_qbLoadTransition == qfalse) // qbFromSavedGame)
  {
    ...
    client->squadname = ent->targetname = ent->script_targetname =
        ent->NPC_type = "kyle";
    ...
    client->ps.stats[STAT_WEAPONS] |=
        (1 << WP_BRYAR_PISTOL); // these are precached in g_items,
                                // ClearRegisteredItems()
    client->ps.inventory[INV_ELECTROBINOCULARS] = 1;

    // always give the bryar pistol, but we have to give EITHER the saber or the
    // stun baton..never both
    if (spawnPoint->spawnflags & 32) // STUN_BATON
    {
      client->ps.stats[STAT_WEAPONS] |= (1 << WP_STUN_BATON);
    } else { // give the saber AND the blaster because most test maps will not
             // have the STUN BATON flag set
      client->ps.stats[STAT_WEAPONS] |=
    ...
    // restore some player data if this is a spawn point with KEEP_REV
    // (spawnflags&1) set...
    //
    ...
    */
    ...
    if (spawnPoint->spawnflags &
        64) { // player starts with absolutely no weapons
