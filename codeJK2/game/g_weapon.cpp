...
//-----------------------------------------------------------------------------
{
  ...
  gi.trace(&tr, newstart, entMins, entMaxs, start, ent->s.number,
           MASK_SOLID | CONTENTS_SHOTCLIP, G2_NOCOLLIDE, 0);
...
//-----------------------------------------------------------------------------
{
  ...
  if (self->splashDamage > 0 && self->splashRadius > 0) {
    G_RadiusDamage(self->currentOrigin, attacker, self->splashDamage,
                   self->splashRadius, attacker, MOD_EXPLOSIVE_SPLASH);
  }
...
//---------------------------------------------------------
void CalcMuzzlePoint(gentity_t *const ent, vec3_t wpFwd, vec3_t right,
                     vec3_t wpUp, vec3_t muzzlePoint, float lead_in)
//---------------------------------------------------------
{
  vec3_t org;
  ...
  case WP_BLASTER:
    ...
    muzzlePoint[2] -= 1;
    if (ent->s.number == 0)
      VectorMA(muzzlePoint, 12, wpFwd,
               muzzlePoint); // player, don't set this any lower otherwise the
                             // projectile will impact immediately when your
                             // back is to a wall
    else
      VectorMA(muzzlePoint, 2, wpFwd,
               muzzlePoint); // NPC, don't set too far wpFwd otherwise the
...
/*QUAKED emplaced_gun (0 0 1) (-24 -24 0) (24 24 64) INACTIVE FACING VULNERABLE
 ...
 order to use it VULNERABLE - allow the gun to take damage
  ...
  if (self->delay + 500 < level.time) {
    ...
    if (activator->NPC) {
      if (activator->weaponModel >= 0) {
        ...
        // Doesn't work?
        //				activator->maxs[2] += 35; // make it so you
        //can potentially shoot their head 				activator->s.radius += 10; //
        //increase ghoul radius so we can collide with the enemy more accurately
...
void SP_emplaced_gun(gentity_t *ent) {
  ...
  ent->lowerLumbarBone =
      gi.G2API_GetBoneIndex(&ent->ghoul2[0], "swivel_bone", qtrue);
  gi.G2API_SetBoneAngles(&ent->ghoul2[0], "swivel_bone", vec3_origin,
