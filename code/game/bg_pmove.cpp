...
qboolean PM_PredictJumpSafe(vec3_t jumpHorizDir, float jumpHorizSpeed, float jumpVertSpeed, int predictTimeLength) { return qtrue; }
...
qboolean PM_CheckGrabWall(trace_t *trace) {
	...
	if (pm->gent->client->ps.groundEntityNum != ENTITYNUM_NONE) { // must be in air
		return qfalse;
	}
	...
	if ((trace->contents & (CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP))) { // can't jump off of clip brushes
		return qfalse;
	}
	if (pm->gent->client->ps.forcePowerLevel[FP_LEVITATION] < FORCE_LEVEL_1) { // must have at least FJ 1
		return qfalse;
	}
	if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
		pm->gent->client->ps.forcePowerLevel[FP_LEVITATION] < FORCE_LEVEL_3) { // player must have force jump 3
		return qfalse;
	}
	...
	if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())) { // player
		...
		if (DotProduct(moveDir, trace->plane.normal) > -0.65f) { // not enough of a direct impact, just slide off
			return qfalse;
		}
		...
		*/
		...
		if (pm->ps->legsAnim != BOTH_FORCELONGLEAP_START && pm->ps->legsAnim != BOTH_FORCELONGLEAP_ATTACK) { // not in a long-jump
			if (!pm->gent->enemy) {																			 // no enemy
				...
				{ // enemy is way below us
					...
					if (DotProduct(enemyDir, trace->plane.normal) < 0.65f) { // jumping off this wall would not launch me in the general direction of my enemy
						return qfalse;
					}
				}
			}
		...
		if (!PM_PredictJumpSafe(trace->plane.normal, JUMP_OFF_WALL_SPEED, G_ForceWallJumpStrength(),
								1500)) { // we would hit a do not enter brush, so don't grab the wall
			return qfalse;
		}
...
qboolean PM_ClientImpact(trace_t *trace, qboolean damageSelf) {
	...
	if (otherEntityNum == ENTITYNUM_WORLD || (traceEnt->bmodel && traceEnt->s.pos.trType == TR_STATIONARY)) { // hit world or a non-moving brush
...
static void PM_Friction(void) {
	...
	if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_VEHICLE && pm->gent->m_pVehicle &&
		...
		if (pm->gent->m_pVehicle && pm->gent->m_pVehicle->m_pVehicleInfo->hoverHeight > 0) { // in a hovering vehicle, have air control
		...
		if ((pm->watertype & CONTENTS_LADDER) || pm->waterlevel <= 1) {
			if ((pm->watertype & CONTENTS_LADDER) || (pml.walking && !(pml.groundTrace.surfaceFlags & SURF_SLICK))) {
				...
				if (!(pm->ps->pm_flags & PMF_TIME_KNOCKBACK) && !(pm->ps->pm_flags & PMF_TIME_NOFRICTION)) {
					if (pm->ps->legsAnim == BOTH_FORCELONGLEAP_START || pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK ||
						pm->ps->legsAnim == BOTH_FORCELONGLEAP_LAND) {	 // super forward jump
	...
	} else if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && pm->gent && pm->gent->client &&
...
qboolean PM_ForceJumpingUp(gentity_t *gent) {
	...
	if ((gent->s.number < MAX_CLIENTS || G_ControlledByPlayer(gent)) && in_camera) { // player can't use force powers in cinematic
		return qfalse;
	}
	if (gent->client->ps.groundEntityNum == ENTITYNUM_NONE // in air
		&& ((gent->client->ps.pm_flags & PMF_JUMPING) &&
			gent->client->ps.velocity[2] >
				0) // jumped & going up or at water surface///*(gent->client->ps.waterHeightLevel==WHL_SHOULDERS&&gent->client->usercmd.upmove>0) ||*/
...
static qboolean PM_CheckJump(void) {
	...
	if (PM_KickingAnim(pm->ps->legsAnim) && !PM_InAirKickingAnim(pm->ps->legsAnim)) { // can't jump when in a kicking anim
		return qfalse;
	}
	/*
	...
	*/
	...
	if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && pm->gent && pm->gent->client &&
		(pm->gent->client->NPC_class == CLASS_BOBAFETT || pm->gent->client->NPC_class == CLASS_ROCKETTROOPER)) { // player playing as boba fett
		if (pm->cmd.upmove > 0) {																				 // turn on/go up
			if (pm->ps->groundEntityNum == ENTITYNUM_NONE && !(pm->ps->pm_flags & PMF_JUMP_HELD)) {				 // double-tap - must activate while in air
	...
	} else if (pm->waterlevel < 3) //|| (pm->ps->waterHeightLevel==WHL_SHOULDERS&&pm->cmd.upmove>0) )
	{
		if (pm->ps->gravity > 0) { // can't do this in zero-G
			if (pm->ps->legsAnim == BOTH_FORCELONGLEAP_START || pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK ||
				pm->ps->legsAnim == BOTH_FORCELONGLEAP_LAND) { // in the middle of a force long-jump
				/*
				if ( pm->ps->groundEntityNum == ENTITYNUM_NONE
					&& pm->cmd.upmove > 0
					&& pm->cmd.forwardmove > 0 )
				*/
				if ((pm->ps->legsAnim == BOTH_FORCELONGLEAP_START || pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK) && pm->ps->legsAnimTimer > 0) { // in the
																																					// air
					...
					if (pm->ps->legsAnim == BOTH_FORCELONGLEAP_START || pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK) { // still in start anim, but it's run
																														 // out
						pm->ps->forcePowersActive |= (1 << FP_LEVITATION);
					...
					{											   // slow down
						...
						if ((pm->ps->velocity[0] + pm->ps->velocity[1]) * 0.5f <= 10.0f) { // falling straight down
					...
					return qfalse;
					   ...
					   && !(pm->ps->pm_flags & PMF_JUMP_HELD)
					   //&& (float)(level.time-pm->ps->lastStationary) >= (3000.0f*g_timescale->value)//have to have a 3 second running start - relative to
					   //force speed slowdown
				...
				return qtrue;
			} else if (PM_InCartwheel(pm->ps->legsAnim) || PM_InButterfly(pm->ps->legsAnim)) { // can't keep jumping up in cartwheels, ariels and butterflies
			}
				...
				{																		 // can still go up
					...
					if (curHeight > forceJumpHeight[0]) {						 // passed normal jump height  *2?
						if (!(pm->ps->forcePowersActive & (1 << FP_LEVITATION))) // haven't started forcejump yet
						{
							...
							if (PM_InAirKickingAnim(pm->ps->legsAnim) && pm->ps->legsAnimTimer) { // still in kick
									   ...
									   pm->ps->legsAnim != BOTH_FLIP_L && pm->ps->legsAnim != BOTH_ALORA_FLIP_1 && pm->ps->legsAnim != BOTH_ALORA_FLIP_2 &&
					  ...
					  FL_LOCK_PLAYER_WEAPONS)))) // yes this locked weapons check also includes force powers, if we need a separate check later I'll make one
	{
		if (pm->gent->NPC && pm->gent->NPC->rank != RANK_CREWMAN &&
			...
			if ((pm->ps->weapon == WP_SABER && G_TryingCartwheel(pm->gent, &pm->cmd) /*(pm->cmd.buttons&BUTTON_FORCE_FOCUS)*/ &&
				 (pm->cmd.buttons & BUTTON_ATTACK)) // using saber and holding focus + attack
				//					  ||(pm->ps->weapon!=WP_SABER&&((pm->cmd.buttons&BUTTON_ATTACK)||(pm->cmd.buttons&BUTTON_ALT_ATTACK)) ) )//using any other weapon
				//and hitting either attack button
				&& (((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) && pm->cmd.upmove > 0 &&
				...
				&& G_EnoughPowerForSpecialMove(pm->ps->forcePower,
											   SABER_ALT_ATTACK_POWER_LR) /*pm->ps->forcePower >= SABER_ALT_ATTACK_POWER_LR*/) // have enough power
				...
				if (pm->cmd.rightmove > 0) {
					...
					if (pm->ps->saberAnimLevel == SS_STAFF && pm->ps->weapon == WP_SABER) {
						...
						if (!(pm->ps->saber[0].saberFlags & SFL_NO_CARTWHEELS) &&
							...
							if (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) { // player: since we're on the ground, always do a cartwheel
								/*
			...
			} else if (pm->cmd.rightmove > 0 && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_1) { // strafing right
				if (pm->cmd.forwardmove > 0) {															  // wall-run
					if (!(pm->ps->saber[0].saberFlags & SFL_NO_WALL_RUNS) &&
						(!pm->ps->dualSabers || !(pm->ps->saber[1].saberFlags & SFL_NO_WALL_RUNS))) { // okay to do wall-runs with this saber
					...
					if (!(pm->ps->saber[0].saberFlags & SFL_NO_WALL_FLIPS) &&
						(!pm->ps->dualSabers || !(pm->ps->saber[1].saberFlags & SFL_NO_WALL_FLIPS))) { // okay to do wall-flips with this saber
			...
			{																	  // step off wall, flip backwards
				if (VectorLengthSquared(pm->ps->velocity) >
					...
					if (pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_2) { // run all the way up wwall
						if (!(pm->ps->saber[0].saberFlags & SFL_NO_WALL_RUNS) &&
							(!pm->ps->dualSabers || !(pm->ps->saber[1].saberFlags & SFL_NO_WALL_RUNS))) { // okay to do wall-runs with this saber
							vertPush = forceJumpStrength[FORCE_LEVEL_2] / 2.0f;
			...
			{												  // back-jump does backflip... FIXME: always?!  What about just doing a normal jump backwards?
				if (pm->ps->velocity[2] >= 0) {				  // must be going up already
					if (!(pm->ps->saber[0].saberFlags & SFL_NO_FLIPS) &&
						(!pm->ps->dualSabers || !(pm->ps->saber[1].saberFlags & SFL_NO_FLIPS))) { // okay to do backstabs with this saber
						vertPush = JUMP_VELOCITY;
				...
				if (pm->ps->weapon == WP_SABER && (pm->cmd.buttons & BUTTON_ATTACK)) {
					...
					if (pm->ps->saber[0].jumpAtkUpMove != LS_INVALID) {
						if (pm->ps->saber[0].jumpAtkUpMove != LS_NONE) { // actually overriding
							overrideJumpAttackUpMove = (saberMoveName_t)pm->ps->saber[0].jumpAtkUpMove;
						} else if (pm->ps->dualSabers && pm->ps->saber[1].jumpAtkUpMove > LS_NONE) { // would be cancelling it, but check the second saber, too
					...
					} else if (pm->ps->dualSabers && pm->ps->saber[1].jumpAtkUpMove != LS_INVALID) { // first saber not overridden, check second
						overrideJumpAttackUpMove = (saberMoveName_t)pm->ps->saber[0].jumpAtkUpMove;
					}
			...
			if (anim != -1 && PM_HasAnimation(pm->gent, anim)) {
				...
				if (doTrace) {
					...
					if (anim == BOTH_WALL_FLIP_LEFT) { // sigh.. check for bottomless pit to the right
						...
						if (anim == BOTH_WALL_FLIP_BACK1 ||
							anim == BOTH_FORCEWALLRUNFLIP_START) {	   // trace up and forward a little to make sure the wall it at least 64 tall
							...
							{										   // no need to trace up, it's all good...
								...
								} else if (anim == BOTH_FORCEWALLRUNFLIP_START) {				// instead of wall-running up, do the backflip
									anim = BOTH_WALL_FLIP_BACK1;
									vertPush = forceJumpStrength[FORCE_LEVEL_2] / 2.25f;
								}
								...
								pm->trace(&trace2, start, mins, maxs, traceto, pm->ps->clientNum, contents, (EG2_Collision)0, 0);
								if (trace2.allsolid || trace2.startsolid || trace2.fraction >= 1.0f) { // no room above or no wall in front at that height
									trace.fraction = 1.0f;											   // way to stop if from doing the jump
								}
							}
						}
						if (trace.fraction < 1.0f) {			// still valid to jump
							if (anim == BOTH_WALL_FLIP_BACK1) { // sigh.. check for bottomless pit to the rear
								trace_t trace2;
								vec3_t start;
				...
				if (!doTrace || (trace.fraction < 1.0f && ((trace.entityNum < ENTITYNUM_WORLD && traceEnt && traceEnt->s.solid != SOLID_BMODEL) ||
														   DotProduct(wallNormal, idealNormal) > 0.7))) { // there is a wall there

					if ((anim != BOTH_WALL_RUN_LEFT && anim != BOTH_WALL_RUN_RIGHT && anim != BOTH_FORCEWALLRUNFLIP_START) ||
						(wallNormal[2] >= 0.0f && wallNormal[2] <= MAX_WALL_RUN_Z_NORMAL)) { // wall-runs can only run on relatively flat walls, sorry.
						...
						// move me to side
							...
							pm->ps->velocity[0] = pm->ps->velocity[1] = 0;
							VectorMA(pm->ps->velocity, -150, right, pm->ps->velocity);
						} else if (anim == BOTH_FLIP_BACK1 || anim == BOTH_FLIP_BACK2 || anim == BOTH_FLIP_BACK3 || anim == BOTH_ALORA_FLIP_B ||
								   anim == BOTH_WALL_FLIP_BACK1) {
							pm->ps->velocity[0] = pm->ps->velocity[1] = 0;
						...
						if (doTrace && anim != BOTH_WALL_RUN_LEFT && anim != BOTH_WALL_RUN_RIGHT && anim != BOTH_FORCEWALLRUNFLIP_START) {
							if (pm->gent && trace.entityNum < ENTITYNUM_WORLD) {
								if (traceEnt && traceEnt->client && traceEnt->health > 0 && traceEnt->takedamage &&
									traceEnt->client->NPC_class != CLASS_GALAKMECH && traceEnt->client->NPC_class != CLASS_DESANN &&
									!(traceEnt->flags & FL_NO_KNOCKBACK)) { // push them away and do pain
									vec3_t oppDir, fxDir;
						...
						if (anim == BOTH_BUTTERFLY_RIGHT) {
						...
						} else { // not a proper saberMove, so do set all the details manually
							...
							if (/*anim == BOTH_BUTTERFLY_LEFT
							   || anim == BOTH_BUTTERFLY_RIGHT
							   ||*/ anim == BOTH_FJSS_TR_BL || anim == BOTH_FJSS_TL_BR || anim == BOTH_FORCEWALLRUNFLIP_START) {
			...
			} else if (pm->ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START) { // want to jump off wall
					...
					{//there is a wall there
						...
						if ( kick && pm->gent && trace.entityNum < ENTITYNUM_WORLD )
						{
							...
							{//push them away and do pain
								...
								//FIXME: need knockdown anim
								G_Damage( traceEnt, pm->gent, pm->gent, oppDir, traceEnt->currentOrigin, 10,
			DAMAGE_NO_ARMOR|DAMAGE_NO_HIT_LOC|DAMAGE_NO_KNOCKBACK, MOD_MELEE ); G_Sound( traceEnt, G_SoundIndex( va("sound/weapons/melee/punch%d", Q_irand(1,
			4)) ) ); if ( trace.fraction <= 0.5f )
								{//close to him
							 ...
							 && (g_debugMelee->integer						   // if you know kung fu, no height cap on wall-grab-jumps
								 || ((pm->ps->origin[2] - pm->ps->forceJumpZStart) <
									 (forceJumpHeightMax[FORCE_LEVEL_3] -
									  (G_ForceWallJumpStrength() / 2.0f)))) // can fit at least one more wall jump in (yes, using "magic numbers"... for now)
							 ))
					 //&& (pm->ps->legsAnim == BOTH_JUMP1 || pm->ps->legsAnim == BOTH_INAIR1 ) )//not in a flip or spin or anything
			) { // see if we're pushing at a wall and jump off it if so
				if (!(pm->ps->saber[0].saberFlags & SFL_NO_WALL_GRAB) &&
					(!pm->ps->dualSabers || !(pm->ps->saber[1].saberFlags & SFL_NO_WALL_GRAB))) { // okay to do wall-grabs with this saber
					...
					vec3_t checkDir, traceto, mins = {pm->mins[0], pm->mins[1], 0}, maxs = {pm->maxs[0], pm->maxs[1], 24},
											  fwdAngles = {0, pm->ps->viewangles[YAW], 0};
					trace_t trace;
					vec3_t idealNormal;
					int anim = -1;
					...
					if (anim != -1) {									 // trace in the dir we're pushing in and see if there's a vertical wall there
						...
						pm->trace(&trace, pm->ps->origin, mins, maxs, traceto, pm->ps->clientNum, CONTENTS_SOLID, (EG2_Collision)0,
								  0); // FIXME: clip brushes too?
						VectorSubtract(pm->ps->origin, traceto, idealNormal);
						VectorNormalize(idealNormal);
						gentity_t *traceEnt = &g_entities[trace.entityNum];
						if (trace.fraction < 1.0f && fabs(trace.plane.normal[2]) <= MAX_WALL_GRAB_SLOPE &&
	...
	if (pm->gent
		//&& pm->cmd.upmove > 0
		&& pm->ps->forceRageRecoveryTime < pm->cmd.serverTime // not in a force Rage recovery period
		&& pm->ps->weapon == WP_SABER && (pm->ps->weaponTime > 0 || (pm->cmd.buttons & BUTTON_ATTACK)) &&
		((pm->ps->clientNum && !PM_ControlledByPlayer()) || ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && cg.renderingThirdPerson &&
		...
		if (!PM_RollingAnim(pm->ps->legsAnim) && !PM_InKnockDown(pm->ps) && !PM_InDeathAnim() && !PM_PainAnim(pm->ps->torsoAnim) &&
			!PM_FlippingAnim(pm->ps->legsAnim) && !PM_SpinningAnim(pm->ps->legsAnim) &&
			!PM_SaberInSpecialAttack(pm->ps->torsoAnim)) {	// HMM... do NPCs need this logic?
	...
	if (zNormal > 0) {
		if (pm->ps->velocity[2] > 0) {
			if (pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT || pm->ps->legsAnim == BOTH_WALL_RUN_LEFT || pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT_STOP ||
				pm->ps->legsAnim == BOTH_WALL_RUN_LEFT_STOP || pm->ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START ||
				pm->ps->legsAnim == BOTH_FORCELONGLEAP_START || pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK || pm->ps->legsAnim == BOTH_FORCELONGLEAP_LAND ||
...
static void PM_AirMove(void) {
	...
	if (pVeh && pVeh->m_pVehicleInfo->hoverHeight > 0) { // in a hovering vehicle, have air control
	...
	if (pVeh && pVeh->m_pVehicleInfo->type == VH_SPEEDER) { // speeders have more control in air
		// in mid-air
		accelerate = pVeh->m_pVehicleInfo->acceleration;
		if (pml.groundPlane) { // on a slope of some kind, shouldn't have much control and should slide a lot
			accelerate *= 0.5f;
		}
	...
	if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0 && pm->ps->forceJumpZStart &&
...
static float PM_DamageForDelta(int delta) {
	...
	if (pm->gent->NPC) {
		if (pm->ps->weapon == WP_SABER ||
			(pm->gent->client &&
			 pm->gent->client->NPC_class == CLASS_REBORN)) { // FIXME: for now Jedi take no falling damage, but really they should if pushed off?
			damage = 0;
		}
...
static qboolean PM_TryRoll(void) {
	...
	if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && (!cg.renderingThirdPerson || cg.zoomMode)) { // player can't do this in 1st person
		return qfalse;
	}
	if (!pm->gent) {
		return qfalse;
	...
	if (pm->ps->clientNum && pm->gent->NPC) {				  // NPC
		if (pm->gent->NPC->scriptFlags & SCF_NO_ACROBATICS) { // scripted to never do acrobatics
			return qfalse;
		}

		if (pm->ps->weapon == WP_SABER) { // jedi/reborn
			if (pm->gent->NPC->rank != RANK_CREWMAN &&
				pm->gent->NPC->rank < RANK_LT_JG) { // reborn/jedi who are not acrobats or fencers can't do any of these acrobatics
				return qfalse;
			}
...
static void PM_CrashLand(void) {
	...
	if ((pm->ps->pm_flags & PMF_TRIGGER_PUSHED)) {
		...
		if (pm->gent && pm->gent->NPC &&
			...
			delta = 1;
		} else if (pm->ps->jumpZStart && (pm->ps->forcePowerLevel[FP_LEVITATION] >= FORCE_LEVEL_1 ||
										  (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()))) { // we were force-jumping
	...
	if (!PM_InDeathAnim()) {
		if (PM_InAirKickingAnim(pm->ps->legsAnim) && pm->ps->torsoAnim == pm->ps->legsAnim) {
		...
		} else if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_ROCKETTROOPER) { // rockettroopers are simpler
			int anim = PM_GetLandingAnim();
			if (anim != -1) {
				if (pm->gent->NPC && pm->gent->NPC->rank < RANK_LT) { // special case: ground-based rocket troopers *always* play land anim on whole body
					PM_SetAnim(pm, SETANIM_BOTH, anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100); // Only blend over 100ms
				} else {
					PM_SetAnim(pm, SETANIM_LEGS, anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100); // Only blend over 100ms
				}
			}
		} else if (pm->cmd.upmove >= 0 && !PM_InKnockDown(pm->ps) && !PM_InRoll(pm->ps)) { // not crouching
			...
			{				  // decide which landing animation to use
				if (pm->gent && pm->gent->client && (pm->gent->client->NPC_class == CLASS_RANCOR || pm->gent->client->NPC_class == CLASS_WAMPA)) {
					...
					int anim = PM_GetLandingAnim();
					if (anim != -1) {
						if (PM_FlippingAnim(pm->ps->torsoAnim) || PM_SpinningAnim(pm->ps->torsoAnim) ||
							pm->ps->torsoAnim == BOTH_FLIP_LAND) { // interrupt these if we're going to play a land
						...
						if (anim == BOTH_FORCELONGLEAP_LAND) {
							...
							PM_SetAnim(pm, SETANIM_BOTH, anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);		 // Only blend over 100ms
						} else if (PM_InAirKickingAnim(pm->ps->legsAnim) && pm->ps->torsoAnim == pm->ps->legsAnim) { // interrupting a kick clears everything
							PM_SetAnim(pm, SETANIM_BOTH, anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100);		 // Only blend over 100ms
							pm->ps->saberMove = LS_READY;
							pm->ps->weaponTime = 0;
						} else if (PM_ForceJumpingAnim(pm->ps->legsAnim) &&
								   pm->ps->torsoAnim ==
									   pm->ps->legsAnim) { // special case: if land during one of these, set the torso, too, if it's not doing something else
							PM_SetAnim(pm, SETANIM_BOTH, anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100); // Only blend over 100ms
						} else {
							PM_SetAnim(pm, SETANIM_LEGS, anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 100); // Only blend over 100ms
						}
	...
	if (delta >= 75) {
		...
		if (!deadFallSound) {
			if (!(pm->ps->pm_flags & PMF_DUCKED) || !Q_irand(0, 3)) { // only 25% chance of making landing alert when ducked
				AddSoundEvent(pm->gent, pm->ps->origin, 32, AEL_MINOR, qfalse, qtrue);
			}
			if (!(pm->ps->eFlags & EF_NODRAW)) { // no sound if no draw
...
qboolean PM_RocketeersAvoidDangerousFalls(void) {
	if (pm->gent->NPC && pm->gent->client &&
		(pm->gent->client->NPC_class == CLASS_BOBAFETT || pm->gent->client->NPC_class == CLASS_ROCKETTROOPER)) { // fixme:  fall through if jetpack broken?
		if (JET_Flying(pm->gent)) {
			if (pm->gent->client->NPC_class == CLASS_BOBAFETT) {
				pm->gent->client->jetPackTime = level.time + 2000;
				// Wait, what if the effect is already playing, how do we know???
				// G_PlayEffect( G_EffectIndex( "boba/jetSP" ), pm->gent->playerModel, pm->gent->genericBolt1, pm->gent->s.number, pm->gent->currentOrigin,
...
static void PM_SetVehicleAngles(vec3_t normal) {
	...
	if (vehicleBankingSpeed <= 0 || (pVeh->m_pVehicleInfo->pitchLimit <= 0 && pVeh->m_pVehicleInfo->rollLimit <= 0)) { // don't bother, this vehicle doesn't
																													   // bank
		return;
	}
	...
	// float pitchBias = 90.0f*pVeh->m_pVehicleInfo->centerOfGravity[0];//if centerOfGravity is all the way back (-1.0f), vehicle pitches up 90 degrees when in
	// air

	VectorClear(vAngles);

	if (pm->waterlevel > 0 || (normal && (pml.groundTrace.contents & (CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA)))) { // in water
	...
	for (int i = 0; i < 3; i++) {
		...
		if (i == ROLL && pVeh->m_ulFlags & VEH_STRAFERAM) { // during strafe ram, roll is done elsewhere
			continue;
		}
		...
		{
			curVehicleBankingSpeed = vehicleBankingSpeed*fabs(AngleNormalize180(AngleSubtract( vAngles[PITCH], pVeh->m_vOrientation[PITCH]
		)))/(g_vehicleInfo[pm->ps->vehicleIndex].pitchLimit/2.0f);
		}
		else if ( i == ROLL )
		{
			curVehicleBankingSpeed = vehicleBankingSpeed*fabs(AngleNormalize180(AngleSubtract( vAngles[ROLL], pVeh->m_vOrientation[ROLL]
...
void BG_ExternThisSoICanRecompileInDebug(
	...
	playerState_t *riderPS) { /*
							 ...
							 riderPS->viewangles[0],riderPS->viewangles[1],riderPS->viewangles[2]); yawDelta =
							 AngleSubtract(riderPS->viewangles[YAW],pVeh->m_vPrevRiderViewAngles[YAW]); #ifndef QAGAME if ( !cg_paused.integer )
								 {
...
static void PM_GroundTraceMissed(void) {
	...
	if (Flying != FLY_HOVER) {
		if (!(pm->ps->eFlags & EF_FORCE_DRAINED)) {
			...
			{
				if (pm->ps->groundEntityNum == ENTITYNUM_NONE) {
					if (pm->ps->stats[STAT_HEALTH] > 0 && !(pm->gent->NPC->aiFlags & NPCAI_DIE_ON_IMPACT) &&
								...
								if (!(pm->gent->flags & FL_UNDYING) && !(pm->gent->flags & FL_GODMODE)) {
									if (!(pm->ps->eFlags & EF_FORCE_GRIPPED) && !(pm->ps->eFlags & EF_FORCE_DRAINED) &&
										...
										if (!pm->ps->forceJumpZStart ||
											pm->ps->forceJumpZStart > pm->ps->origin[2]) // && fabs(pm->ps->velocity[0])<10 && fabs(pm->ps->velocity[1])<10 &&
																						 // pm->ps->velocity[2]<0)//either not force-jumping or force-jumped and
																						 // now fell below original jump start height
										{
											...
											if ((trace.contents & CONTENTS_LAVA) && PM_RocketeersAvoidDangerousFalls()) { // got out of it
											} else if (!trace.allsolid && !trace.startsolid &&
													   (pm->ps->origin[2] - trace.endpos[2]) >= 128) //>=128 so we don't die on steps!
											{
												if (trace.fraction == 1.0) { // didn't hit, we're probably going to die
													if (pm->ps->velocity[2] < 0 &&
														pm->ps->origin[2] - point[2] > 256) { // going down, into a bottomless pit, apparently
														PM_FallToDeath();
														cliff_fall = qtrue;
													}
												} else if (trace.entityNum < ENTITYNUM_NONE && pm->ps->weapon != WP_SABER &&
														   (!pm->gent || !pm->gent->client ||
															(pm->gent->client->NPC_class != CLASS_BOBAFETT && pm->gent->client->NPC_class != CLASS_REBORN &&
															 pm->gent->client->NPC_class !=
																 CLASS_ROCKETTROOPER))) { // Jedi don't scream and die if they're heading for a hard impact
													gentity_t *traceEnt = &g_entities[trace.entityNum];
													if (trace.entityNum == ENTITYNUM_WORLD ||
														(traceEnt && traceEnt->bmodel)) { // hit architecture, find impact force
														float dmg;
														...
														float maxDmg = pm->ps->stats[STAT_HEALTH] > 20 ? pm->ps->stats[STAT_HEALTH]
																									   : 20; // a fall that would do less than 20 points of
																											 // damage should never make us scream to our deaths
														if (pm->gent && pm->gent->client &&
															pm->gent->client->NPC_class ==
																CLASS_HOWLER) { // howlers can survive long falls, despire their health
															maxDmg *= 5;
														}
											...
											//FIXME: we should either do a better check (predict using actual velocity) or we should wait until they've been
											over a bottomless pit for a certain amount of time... VectorCopy( pm->ps->origin, start ); if (
											pm->ps->forceJumpZStart < start[2] )
			...
			if (!cliff_fall) {
				if (pm->ps->legsAnim == BOTH_FORCELONGLEAP_START || pm->ps->legsAnim == BOTH_FORCELONGLEAP_ATTACK ||
					pm->ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START || pm->ps->legsAnim == BOTH_FLIP_LAND) { // let it stay on this anim
				} else if (PM_KnockDownAnimExtended(pm->ps->legsAnim)) {									 // no in-air anims when in knockdown anim
				} else if ((pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT || pm->ps->legsAnim == BOTH_WALL_RUN_LEFT || pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT_STOP ||
							pm->ps->legsAnim == BOTH_WALL_RUN_LEFT_STOP || pm->ps->legsAnim == BOTH_WALL_RUN_RIGHT_FLIP ||
							pm->ps->legsAnim == BOTH_WALL_RUN_LEFT_FLIP || pm->ps->legsAnim == BOTH_WALL_FLIP_RIGHT ||
							pm->ps->legsAnim == BOTH_WALL_FLIP_LEFT || pm->ps->legsAnim == BOTH_FORCEWALLREBOUND_FORWARD ||
							pm->ps->legsAnim == BOTH_FORCEWALLREBOUND_LEFT || pm->ps->legsAnim == BOTH_FORCEWALLREBOUND_BACK ||
							pm->ps->legsAnim == BOTH_FORCEWALLREBOUND_RIGHT || pm->ps->legsAnim == BOTH_CEILING_DROP) &&
				...
				} else if (!PM_InRoll(pm->ps) && !PM_SpinningAnim(pm->ps->legsAnim) && !PM_FlippingAnim(pm->ps->legsAnim) &&
...
static void PM_GroundTrace(void) {
	...
	if ((pm->ps->eFlags & EF_LOCKED_TO_WEAPON) || (pm->ps->eFlags & EF_HELD_BY_RANCOR) || (pm->ps->eFlags & EF_HELD_BY_WAMPA) ||
		...
		*/
		return;
											   ...
											   pm->ps->legsAnim == BOTH_FORCEWALLRUNFLIP_START)) { // wall-running forces you to be in the air
		pml.groundPlane = qfalse;
		pml.walking = qfalse;
		pm->ps->groundEntityNum = ENTITYNUM_NONE;
		return;
	...
	if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_VEHICLE) { // FIXME: extern this as part of vehicle data
	...
	if (pm->gent && pm->gent->client && pm->gent->client->NPC_class != CLASS_SAND_CREATURE &&
		...
		if (((pm->ps->velocity[2] > 0 && (pm->ps->pm_flags & PMF_TIME_KNOCKBACK)) || pm->ps->velocity[2] > 100) &&
			DotProduct(pm->ps->velocity, trace.plane.normal) >
				10) { // either thrown off ground (PMF_TIME_KNOCKBACK) or going off the ground at a large velocity
			if (pm->debugLevel) {
	...
	*/
	...
	pm->ps->lastOnGround = level.time;
	if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())) { // if a player, clear the jumping "flag" so can't double-jump
...
void PM_HoverTrace(void) {
	...
	if (pm->waterlevel && relativeWaterLevel >= 0) {  // in water
		if (pVeh->m_pVehicleInfo->bouyancy <= 0.0f) { // sink like a rock
			...
			float floatHeight = (pVeh->m_pVehicleInfo->bouyancy * ((pm->maxs[2] - pm->mins[2]) * 0.5f)) -
		...
		if (pm->ps->waterheight < pm->ps->origin[2] + pm->maxs[2]) {		   // part of us is sticking out of water
			if (fabs(pm->ps->velocity[0]) + fabs(pm->ps->velocity[1]) > 100) { // moving at a decent speed
		...
		if (trace->plane.normal[2] >= minNormal) { // not a steep slope, so push us up
	...
	if (pml.groundPlane) {
		...
		if (pVeh->m_vAngularVelocity == 0.0f) {
			...
			//=================================
			if (pm->gent && pm->gent->owner && (pm->gent->owner->s.number < MAX_CLIENTS || G_ControlledByPlayer(pm->gent->owner)) &&
				pVeh->m_pVehicleInfo->type == VH_SPEEDER && level.time > (LastMatrixJumpTime + 5000) && VectorLength(pm->ps->velocity) > 30.0f) {
...
static void PM_CheckDuck(void) {
	...
	if (pm->gent && pm->gent->client) {
		...
		if (pm->ps->clientNum < MAX_CLIENTS && (pm->gent->client->NPC_class == CLASS_ATST || pm->gent->client->NPC_class == CLASS_RANCOR) &&
	...
	if (PM_RidingVehicle() || (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_VEHICLE)) { // riding a vehicle or are a vehicle
...
qboolean PM_CrouchGetup(float crouchheight) {
	...
	if (anim == -1) {				 // WTF? stay down?
		...
		if (PM_LockedAnim(pm->ps->torsoAnim)) { // need to be able to override this anim
			pm->ps->torsoAnimTimer = 0;
		}
		if (PM_LockedAnim(pm->ps->legsAnim)) { // need to be able to override this anim
...
qboolean PM_CheckRollGetup(void) {
	if (pm->ps->legsAnim == BOTH_KNOCKDOWN1 || pm->ps->legsAnim == BOTH_KNOCKDOWN2 || pm->ps->legsAnim == BOTH_KNOCKDOWN3 ||
		pm->ps->legsAnim == BOTH_KNOCKDOWN4 || pm->ps->legsAnim == BOTH_KNOCKDOWN5 || pm->ps->legsAnim == BOTH_LK_DL_ST_T_SB_1_L ||
		pm->ps->legsAnim == BOTH_PLAYER_PA_3_FLY || pm->ps->legsAnim == BOTH_RELEASED) { // lying on back or front
		if (((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) &&
			...
			|| ((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) && pm->gent->NPC						 // an NPC
				...
				&& ((pm->gent->client && pm->gent->client->NPC_class == CLASS_ALORA) ||
					Q_irand(0, RANK_CAPTAIN) < pm->gent->NPC->rank))) // higher rank I am, more likely I am to roll away!
		{															  // roll away!
			int anim;
			...
			if (pm->cmd.forwardmove > 0) {
				...
				if (pm->ps->legsAnim == BOTH_KNOCKDOWN3 || pm->ps->legsAnim == BOTH_KNOCKDOWN5 || pm->ps->legsAnim == BOTH_LK_DL_ST_T_SB_1_L) { // on your front
			...
			if ((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer())) {
				if (!G_CheckRollSafety(pm->gent, anim, 64)) { // oops, try other one
					if (pm->ps->legsAnim == BOTH_KNOCKDOWN3 || pm->ps->legsAnim == BOTH_KNOCKDOWN5 || pm->ps->legsAnim == BOTH_LK_DL_ST_T_SB_1_L) {
						...
						if (!G_CheckRollSafety(pm->gent, anim, 64)) { // neither side is clear, screw it
							return qfalse;
						}
					} else {
...
qboolean PM_GettingUpFromKnockDown(float standheight, float crouchheight) {
	...
	if (legsAnim == BOTH_KNOCKDOWN1 || legsAnim == BOTH_KNOCKDOWN2 || legsAnim == BOTH_KNOCKDOWN3 || legsAnim == BOTH_KNOCKDOWN4 ||
		legsAnim == BOTH_KNOCKDOWN5 || legsAnim == BOTH_PLAYER_PA_3_FLY || legsAnim == BOTH_LK_DL_ST_T_SB_1_L || legsAnim == BOTH_RELEASED) { // in a knockdown
		int minTimeLeft = G_MinGetUpTime(pm->gent);
		if (pm->ps->legsAnimTimer <= minTimeLeft) { // if only a quarter of a second left, allow roll-aways
		...
		if (TIMER_Exists(pm->gent, "noGetUpStraight")) {
			if (!TIMER_Done2(pm->gent, "noGetUpStraight", qtrue)) { // not allowed to do straight get-ups for another few seconds
		...
		if (!pm->ps->legsAnimTimer ||
			(pm->ps->legsAnimTimer <= minTimeLeft &&
			 (pm->cmd.upmove > 0 ||
			  (pm->gent && pm->gent->client &&
			   pm->gent->client->NPC_class == CLASS_ALORA)))) { // done with the knockdown - FIXME: somehow this is allowing an *instant* getup...???
			...
			if (pm->cmd.upmove < 0) {
				...
				if (!trace.allsolid) { // stand up
					...
					case BOTH_KNOCKDOWN3:
						if ((pm->ps->clientNum && pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0) ||
							((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && pm->cmd.upmove > 0 &&
							 pm->ps->forcePowerLevel[FP_LEVITATION] > FORCE_LEVEL_0)) // FORCE_LEVEL_1) )
						{
...
void PM_CmdForRoll(playerState_t *ps, usercmd_t *pCmd) {
	...
	case BOTH_GETUP_BROLL_B:
		if (ps->torsoAnimTimer <= 250) { // end of anim
			pCmd->forwardmove = pCmd->rightmove = 0;
		} else if (PM_AnimLength(g_entities[ps->clientNum].client->clientInfo.animFileIndex, (animNumber_t)ps->legsAnim) - ps->torsoAnimTimer <
...
qboolean PM_AdjustStandAnimForSlope(void) {
	...
	if (pm->gent->client->NPC_class != CLASS_ATST && (!pm->gent || !G_StandardHumanoid(pm->gent))) { // only ATST and player does this
		return qfalse;
	}
	if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && (!cg.renderingThirdPerson || cg.zoomMode)) { // first person doesn't do this
		return qfalse;
	}
	...
	if ((legsAnim >= LEGS_LEFTUP1 && legsAnim <= LEGS_LEFTUP5) || (legsAnim >= LEGS_S1_LUP1 && legsAnim <= LEGS_S1_LUP5) ||
		...
		(legsAnim >= LEGS_S5_LUP1 && legsAnim <= LEGS_S5_LUP5) || (legsAnim >= LEGS_S6_LUP1 && legsAnim <= LEGS_S6_LUP5) ||
		(legsAnim >= LEGS_S7_LUP1 && legsAnim <= LEGS_S7_LUP5)) { // already in left-side up
		if (destAnim > legsAnim && pm->gent->client->slopeRecalcTime < level.time) {
			...
			destAnim = legsAnim;
			   ...
			   (legsAnim >= LEGS_S5_RUP1 && legsAnim <= LEGS_S5_RUP5) || (legsAnim >= LEGS_S6_RUP1 && legsAnim <= LEGS_S6_RUP5) ||
			   (legsAnim >= LEGS_S7_RUP1 && legsAnim <= LEGS_S7_RUP5)) { // already in right-side up
		if (destAnim > legsAnim && pm->gent->client->slopeRecalcTime < level.time) {
			legsAnim++;
			pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
		} else if (destAnim < legsAnim && pm->gent->client->slopeRecalcTime < level.time) {
		...
		if (pm->gent->client->NPC_class == CLASS_ATST) {
			...
			case BOTH_STAND5:
				...
				break;
			case BOTH_SABERDUAL_STANCE:
				if (destAnim >= LEGS_S6_LUP1 && destAnim <= LEGS_S6_LUP5) { // going into left side up
					destAnim = LEGS_S6_LUP1;
					pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
				} else if (destAnim >= LEGS_S6_RUP1 && destAnim <= LEGS_S6_RUP5) { // going into right side up
				...
				break;
			case BOTH_SABERSTAFF_STANCE:
				if (destAnim >= LEGS_S7_LUP1 && destAnim <= LEGS_S7_LUP5) { // going into left side up
					destAnim = LEGS_S7_LUP1;
					pm->gent->client->slopeRecalcTime = level.time + SLOPE_RECALC_INT;
				} else if (destAnim >= LEGS_S7_RUP1 && destAnim <= LEGS_S7_RUP5) { // going into right side up
...
static void PM_Footsteps(void) {
	...
	if (pm->ps->legsAnim == BOTH_FLIP_F || pm->ps->legsAnim == BOTH_FLIP_B || pm->ps->legsAnim == BOTH_FLIP_L || pm->ps->legsAnim == BOTH_FLIP_R ||
		pm->ps->legsAnim == BOTH_ALORA_FLIP_1 || pm->ps->legsAnim == BOTH_ALORA_FLIP_2 || pm->ps->legsAnim == BOTH_ALORA_FLIP_3) {
		flipping = qtrue;
	}
		...
		if (pm->waterlevel > 0) {
			if (pm->watertype & CONTENTS_LADDER) { // FIXME: check for watertype, save waterlevel for whether to play
				...
				return;
					   ...
					   ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) || pm->ps->weapon == WP_SABER || pm->ps->weapon == WP_NONE ||
						pm->ps->weapon == WP_MELEE)) // pm->waterlevel > 1 )	//in deep water
			{
				...
				if (pm->ps->groundEntityNum == ENTITYNUM_NONE) { // unless in the air
					...
					} else if (pm->ps->gravity <= 0) // FIXME: or just less than normal?
					{
						if (pm->gent && pm->gent->client &&
							(pm->gent->client->NPC_class == CLASS_BOBAFETT || pm->gent->client->NPC_class == CLASS_ROCKETTROOPER) &&
							pm->gent->client->moveType == MT_FLYSWIM) { // flying around with jetpack
							// do something else?
							PM_JetPackAnim();
						} else {
							PM_SwimFloatAnim();
			...
			{
				if (pm->gent && pm->gent->client && (pm->gent->client->NPC_class == CLASS_BOBAFETT || pm->gent->client->NPC_class == CLASS_ROCKETTROOPER) &&
					pm->gent->client->moveType == MT_FLYSWIM) { // flying around with jetpack
					// do something else?
					PM_JetPackAnim();
				} else {
	...
	if (!pm->cmd.forwardmove && !pm->cmd.rightmove) {
		if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_ATST) {
			...
			if (pm->ps->legsAnimTimer && (pm->ps->legsAnim == BOTH_THERMAL_READY || pm->ps->legsAnim == BOTH_THERMAL_THROW ||
										  pm->ps->legsAnim == BOTH_ATTACK10)) { // still in a thermal anim, let it play
				return;
			}
			...
			if (pm->gent && pm->gent->client &&
				...
				if (!PM_AdjustStandAnimForSlope()) {
					int legsAnim;
					if ((pm->ps->torsoAnim == BOTH_SPINATTACK6 || pm->ps->torsoAnim == BOTH_SPINATTACK7 || PM_SaberInAttack(pm->ps->saberMove) ||
			...
			{																					// Squadmates use BOTH_STAND3
				...
				if (oldAnim != BOTH_GUARD_LOOKAROUND1 && oldAnim != BOTH_GUARD_IDLE1 && oldAnim != BOTH_STAND2TO4 && oldAnim != BOTH_STAND4TO2 &&
				...
				if (!PM_AdjustStandAnimForSlope()) {
					...
					if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_IMPERIAL) {
						...
						if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_RANCOR) {
							if (pm->gent->count) {
								PM_SetAnim(pm, SETANIM_LEGS, BOTH_STAND4, SETANIM_FLAG_NORMAL);
							} else if (pm->gent->enemy || pm->gent->wait) { // have an enemy or have had one since we spawned
	...
	{																					   // atst, Rancor & Wampa, only override turn anims on legs (no torso)
		...
		if ((PM_InSaberAnim(pm->ps->legsAnim) && !PM_SpinningSaberAnim(pm->ps->legsAnim)) || PM_SaberStanceAnim(pm->ps->legsAnim) ||
			PM_SaberDrawPutawayAnim(pm->ps->legsAnim) || pm->ps->legsAnim == BOTH_SPINATTACK6 // not a full-body spin, just spinning the saber
			|| pm->ps->legsAnim == BOTH_SPINATTACK7											  // not a full-body spin, just spinning the saber
			|| pm->ps->legsAnim == BOTH_BUTTON_HOLD || pm->ps->legsAnim == BOTH_BUTTON_RELEASE || pm->ps->legsAnim == BOTH_THERMAL_READY ||
			pm->ps->legsAnim == BOTH_THERMAL_THROW || pm->ps->legsAnim == BOTH_ATTACK10 || PM_LandingAnim(pm->ps->legsAnim) || PM_PainAnim(pm->ps->legsAnim) ||
	...
	if (pm->ps->pm_flags & PMF_DUCKED) {
		...
		if (!PM_InOnGroundAnim(pm->ps) // not on the ground
			&& (!PM_InRollIgnoreTimer(pm->ps) ||
				(!pm->ps->legsAnimTimer && pm->cmd.upmove < 0))) // not in a roll (or you just finished one and you're still holding crouch)
		{
			qboolean rolled = qfalse;
			if (PM_RunningAnim(pm->ps->legsAnim) || pm->ps->legsAnim == BOTH_FORCEHEAL_START || PM_CanRollFromSoulCal(pm->ps)) { // roll!
				rolled = PM_TryRoll();
			}
			if (!rolled) {
				if (pm->ps->pm_flags & PMF_BACKWARDS_RUN) {
		...
		if (!(pm->cmd.buttons & BUTTON_WALKING)) {	   // running backwards
			...
			} else if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_RANCOR) { // no run anim
			...
			if (!Q_irand(0, 9)) { // 10% chance of a small alert, mainly for the sand_creature
				AddSoundEvent(pm->gent, pm->ps->origin, 16, AEL_MINOR, qtrue, qtrue);
			}
		}
	} else {
		if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_GALAKMECH) {
			...
			if (!(pm->cmd.buttons & BUTTON_WALKING)) { // running
					...
					if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_JAWA) {
						...
						*/
						...
						if (pm->ps->legsAnim != BOTH_RUN1) {
							if (pm->ps->legsAnim !=
								BOTH_RUN1START) { // Hmm, he should really start slow and have to accelerate... also need to do this for stopping
						...
						if (pm->gent->NPC && pm->gent->NPC->stats.runSpeed == 300) { // full on run, on all fours
				...
				if (pm->gent && pm->gent->NPC && pm->gent->client && pm->gent->client->playerTeam != TEAM_PLAYER) {
					footstep = qtrue;
				} else if (!Q_irand(0, 9)) { // 10% chance of a small alert, mainly for the sand_creature
					AddSoundEvent(pm->gent, pm->ps->origin, 16, AEL_MINOR, qtrue, qtrue);
				}
			}
		}
...
static void PM_FinishWeaponChange(void) {
	...
	if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_ATST) { // do nothing
		...
		if (!pm->ps->saberInFlight || pm->ps->dualSabers) { // if it's not in flight or lying around, turn it on!
			// FIXME: AddSound/Sight Event
			// FIXME: don't do this if just loaded a game
			if (trueSwitch) {			  // actually did switch weapons, turn it on
				if (PM_RidingVehicle()) { // only turn on the first saber's first blade...?
...
qboolean PM_CanDoDualDoubleAttacks(void) {
	...
	if (pm->ps->dualSabers && (pm->ps->saber[1].saberFlags & SFL_NO_MIRROR_ATTACKS)) {
		return qfalse;
	}
	...
	if (pm->gent && pm->gent->NPC && pm->gent->NPC->rank >= Q_irand(RANK_LT_COMM, RANK_CAPTAIN + 2)) { // high-rank NPC
		return qtrue;
	}
	if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_ALORA) { // Alora
		return qtrue;
	}
	return qfalse;
}
...
void PM_SetSaberMove(saberMoveName_t newMove) {
	...
	if (newMove == LS_READY) {
		if (pm->ps->saberBlockingTime > cg.time) {
			...
			if (!pm->ps->SaberActive()) { // turn on all blades and sabers if none are currently on
				pm->ps->SaberActivate();
			}
		...
		} else if (pm->ps->saber[0].type == SABER_ARC) { // FIXME: need it's own style?
			anim = BOTH_SABERFAST_STANCE;
		} else if ((pm->ps->dualSabers && pm->ps->saber[1].Active())) {
			anim = BOTH_SABERDUAL_STANCE;
		} else if ((pm->ps->SaberStaff() &&
					(!pm->ps->saber[0].singleBladeStyle || pm->ps->saber[0].blade[1].active)) // saber staff with more than first blade active
				   || pm->ps->saber[0].type == SABER_ARC) {
			anim = BOTH_SABERSTAFF_STANCE;
		} else if (pm->ps->saber[0].type == SABER_LANCE || pm->ps->saber[0].type == SABER_TRIDENT) { // FIXME: need some 2-handed forward-pointing anim
			anim = BOTH_STAND1;
		} else {
		...
		if (pm->ps->saber[0].drawAnim != -1) {
			...
			anim = BOTH_S1_S7;
		} else if (pm->ps->dualSabers && !(pm->ps->saber[0].stylesForbidden & (1 << SS_DUAL)) && !(pm->ps->saber[1].stylesForbidden & (1 << SS_DUAL))) {
			anim = BOTH_S1_S6;
		}
	...
	} else if ((pm->ps->saberAnimLevel == SS_DUAL || (pm->ps->dualSabers && pm->ps->saber[1].Active())) && newMove >= LS_S_TL2BR &&
			   newMove < LS_REFLECT_LL) { // staff has an entirely new set of anims, besides special attacks
		// FIXME: include ready and draw/putaway?
		// FIXME: get hand-made bounces and deflections?
		...
		if (newMove >= LS_V1_BR && newMove <= LS_REFLECT_LL) { // there aren't 1-7, just 1, 6 and 7, so just set it
		...
		} else if ((newMove == LS_A_R2L || newMove == LS_S_R2L || newMove == LS_A_L2R || newMove == LS_S_L2R) && PM_CanDoDualDoubleAttacks() &&
				   G_CheckEnemyPresence(pm->gent, DIR_RIGHT, 150.0f) && G_CheckEnemyPresence(pm->gent, DIR_LEFT, 150.0f)) { // enemy both on left and right
			newMove = LS_DUAL_LR;
			anim = saberMoveData[newMove].animToUse;
			...
			pm->cmd.rightmove = 0;
				   ...
				   G_CheckEnemyPresence(pm->gent, DIR_FRONT, 150.0f) && G_CheckEnemyPresence(pm->gent, DIR_BACK, 150.0f)) { // enemy both in front and back
			newMove = LS_DUAL_FB;
			anim = saberMoveData[newMove].animToUse;
			// probably already moved, but...
			pm->cmd.forwardmove = 0;
	...
	*/
		...
		if (pm->ps->saber[0].type == SABER_LANCE ||
			pm->ps->saber[0].type == SABER_TRIDENT) { // FIXME: hack for now - these use the fast anims, but slowed down.  Should have own style
	...
	if ((anim == BOTH_STAND1 && (pm->ps->saber[0].type == SABER_ARC || (pm->ps->dualSabers && pm->ps->saber[1].Active()))) ||
		...
		|| (0 && anim == BOTH_SABERSTAFF_STANCE) || anim == BOTH_SABERDUAL_STANCE || anim == BOTH_SABERFAST_STANCE ||
		anim == BOTH_SABERSLOW_STANCE) { // match torso anim to walk/run anim if newMove is just LS_READY
	...
	if (!PM_RidingVehicle()) {
		if (!manualBlocking) {
			if (newMove == LS_A_LUNGE || newMove == LS_A_JUMP_T__B_ || newMove == LS_A_BACKSTAB || newMove == LS_A_BACK || newMove == LS_A_BACK_CR ||
				...
				newMove == LS_BUTTERFLY_LEFT || newMove == LS_BUTTERFLY_RIGHT || newMove == LS_A_BACKFLIP_ATK || newMove == LS_STABDOWN ||
				newMove == LS_STABDOWN_STAFF || newMove == LS_STABDOWN_DUAL || newMove == LS_DUAL_SPIN_PROTECT || newMove == LS_STAFF_SOULCAL ||
				newMove == LS_A1_SPECIAL || newMove == LS_A2_SPECIAL || newMove == LS_A3_SPECIAL || newMove == LS_UPSIDE_DOWN_ATTACK ||
				newMove == LS_PULL_ATTACK_STAB || newMove == LS_PULL_ATTACK_SWING || PM_KickMove(newMove)) {
				parts = SETANIM_BOTH;
			...
			if (PM_SaberInAttack(newMove) || PM_SaberInSpecialAttack(anim)) { // playing an attack
				if (pm->ps->saberMove != newMove) {							  // wasn't playing that attack before
					if (PM_SaberInSpecialAttack(anim)) {
						...
						if (!PM_InCartwheel(pm->ps->torsoAnim)) {		 // can still attack during a cartwheel/arial
				...
				} else if ((setflags & SETANIM_FLAG_RESTART) && PM_SaberInSpecialAttack(anim)) { // sigh, if restarted a special, then set the weaponTime
																								 // *again*
					if (!PM_InCartwheel(pm->ps->torsoAnim)) {		 // can still attack during a cartwheel/arial
						pm->ps->weaponTime = pm->ps->torsoAnimTimer; // so we know our weapon is busy
					}
				...
				{ // don't damage on the first few frames of a start anim because it may pop from one position to some drastically different one, killing the
		...
		*/
		...
		case LS_ROLL_STAB:
			if (PM_InSecondaryStyle()) {  // staff as medium or dual as fast
				...
				} else if (pm->ps->saber[0].numBlades > 1) { // only force on the first saber's first blade
					pm->ps->SaberBladeActivate(0, 0);
				}
...
int PM_SaberLockWinAnim(saberLockResult_t result, int breakType) {
	...
	if (winAnim != -1) {
		...
		if (breakType == SABERLOCK_SUPERBREAK && winAnim != BOTH_LK_ST_DL_T_SB_1_W) { // going to attack with saber, do a saber trail
...
int PM_SaberLockResultAnim(gentity_t *duelist, int lockOrBreakOrSuperBreak, int winOrLose) {
	...
	if (lockOrBreakOrSuperBreak == SABERLOCK_SUPERBREAK && winOrLose == SABERLOCK_LOSE) { // if you lose a superbreak, you're defenseless
	...
	if (lockOrBreakOrSuperBreak == SABERLOCK_SUPERBREAK && winOrLose == SABERLOCK_WIN &&
		baseAnim != BOTH_LK_ST_DL_T_SB_1_W) { // going to attack with saber, do a saber trail
...
void PM_SaberLockBreak(gentity_t *gent, gentity_t *genemy, saberLockResult_t result, int victoryStrength) {
	...
	if (result == LOCK_VICTORY && Q_irand(0, 7) < victoryStrength) {
		if (genemy && genemy->NPC &&
			((genemy->NPC->aiFlags & NPCAI_BOSS_CHARACTER) || (genemy->NPC->aiFlags & NPCAI_SUBBOSS_CHARACTER) ||
			 (genemy->client && genemy->client->NPC_class == CLASS_SHADOWTROOPER)) &&
			Q_irand(0, 4)) { // less of a chance of getting a superbreak against a boss
			...
			if (Q_irand(FORCE_LEVEL_1, FORCE_LEVEL_2) < pm->ps->forcePowerLevel[FP_SABER_OFFENSE]) {
				...
				if (!singleVsSingle) { // all others have their own super breaks
					...
					winAnim = -1;
						   ...
						   winAnim == BOTH_LK_S_S_T_SB_1_W) { // doing a superbreak on single-vs-single, don't do the old superbreaks this time
					// so it doesn't try to set some other anim below
					winAnim = -1;
				} else { // JK2-style
					switch (winAnim) {
					...
					if (winAnim != BOTH_CCWCIRCLEBREAK) {
						if ((!genemy->s.number && genemy->health <= 25) // player low on health
							|| (genemy->s.number && genemy->client->NPC_class != CLASS_KYLE && genemy->client->NPC_class != CLASS_LUKE &&
								genemy->client->NPC_class != CLASS_TAVION && genemy->client->NPC_class != CLASS_ALORA &&
				...
				if (!(genemy->client->ps.saber[0].saberFlags & SFL_NOT_DISARMABLE)) {
					...
					if ((genemy->client->ps.saber[0].saberFlags & SFL_TWO_HANDED) ||
						(genemy->client->ps.dualSabers &&
						 genemy->client->ps.saber[1].Active())) { // defender gets a bonus for using a 2-handed saber or 2 sabers
						victoryStrength -= 2;
					}
					...
					if (Q_irand(0, 10) < victoryStrength) {
						if (!(genemy->client->ps.saber[0].saberFlags & SFL_TWO_HANDED) ||
							!Q_irand(0, 1)) { // if it's a two-handed saber, it has a 50% chance of resisting a disarming
...
qboolean PM_SaberLocked(void) {
	...
	if (PM_InSaberLock(pm->ps->torsoAnim) && PM_InSaberLock(genemy->client->ps.torsoAnim)) {
		if (pm->ps->saberLockTime <= level.time + 500 && pm->ps->saberLockEnemy != ENTITYNUM_NONE) { // lock just ended
			...
			if (strength > 1 && eStrength > 1 && !Q_irand(0, abs(strength - eStrength) + 1)) { // both knock each other down!
				...
				if (ValidAnimFileIndex(genemy->client->clientInfo.animFileIndex)) {
					...
					*/
					...
					if (PM_InSaberLockOld(genemy->client->ps.torsoAnim)) {
						if (genemy->client->ps.torsoAnim == BOTH_CCWCIRCLELOCK || genemy->client->ps.torsoAnim == BOTH_BF2LOCK) {
...
qboolean G_EnemyInKickRange(gentity_t *self, gentity_t *enemy) {
	...
	if (fabs(self->currentOrigin[2] - enemy->currentOrigin[2]) < 32) { // generally at same height
		if (DistanceHorizontal(self->currentOrigin, enemy->currentOrigin) <=
			(STAFF_KICK_RANGE + 8.0f + (self->maxs[0] * 1.5f) + (enemy->maxs[0] * 1.5f))) { // within kicking range!
			return qtrue;
		}
	}
	return qfalse;
...
saberMoveName_t G_PickAutoKick(gentity_t *self, gentity_t *enemy, qboolean storeMove) {
	...
	} else if (fabs(fDot) > 0.5f && fabs(rDot) < 0.5f) { // generally in front or behind us
		if (fDot > 0) {									 // kick fwd
			kickMove = LS_KICK_F;
		} else { // kick back
			kickMove = LS_KICK_B;
	...
	if (kickMove != LS_NONE) {									  // have a valid one to do
		if (self->client->ps.groundEntityNum == ENTITYNUM_NONE) { // if in air, convert kick to an in-air kick
			...
			if ((!PM_FlippingAnim(self->client->ps.legsAnim) || self->client->ps.legsAnimTimer <= 0) && gDist > 64.0f && // strict minimum
				...
				if (gDist > 128.0f || self->client->ps.velocity[2] >= 0) { // off ground, but too close to ground
					kickMove = LS_NONE;
				}
			}
		}
...
saberMoveName_t PM_PickAutoKick(gentity_t *enemy) { return G_PickAutoKick(pm->gent, enemy, qfalse); }
...
saberMoveName_t G_PickAutoMultiKick(gentity_t *self, qboolean allowSingles, qboolean storeMove) {
	...
	for (e = 0; e < numListedEntities; e++) {
		...
		if (kickMove == LS_KICK_F_AIR && kickMove == LS_KICK_B_AIR && kickMove == LS_KICK_R_AIR && kickMove == LS_KICK_L_AIR) { // in air?  Can't do multikicks
		} else {
			switch (kickMove) {
			case LS_KICK_F:
				enemiesFront++;
	...
	kickMove = LS_NONE;
	if (self->client->ps.groundEntityNum != ENTITYNUM_NONE) {												  // can't do the multikicks in air
		if (enemiesFront && enemiesBack && (enemiesFront + enemiesBack) - (enemiesRight + enemiesLeft) > 1) { // more enemies in front/back than left/right
			kickMove = LS_KICK_BF;
		} else if (enemiesRight && enemiesLeft &&
				   (enemiesRight + enemiesLeft) - (enemiesFront + enemiesBack) > 1) { // more enemies on left & right than front/back
			kickMove = LS_KICK_RL;
		} else if ((enemiesFront || enemiesBack) && (enemiesRight || enemiesLeft)) { // at least 2 enemies around us, not aligned
			kickMove = LS_KICK_S;
		} else if (enemiesSpin > 1) { // at least 2 enemies around us, not aligned
			kickMove = LS_KICK_S;
		}
	}
	if (kickMove == LS_NONE && bestKick != LS_NONE) { // no good multi-kick move, but we do have a nice single-kick we found
		kickMove = bestKick;
		// get mad at him so he knows he's being targetted
		if ((self->s.number < MAX_CLIENTS || G_ControlledByPlayer(self)) && bestEnt != NULL) { // player
...
qboolean PM_SaberThrowable(void) {
	...
	if (!(pm->ps->saber[0].saberFlags & SFL_NOT_THROWABLE)) { // yes, this saber is always throwable
		return qtrue;
	}

	// saber is not normally throwable
	if ((pm->ps->saber[0].saberFlags & SFL_SINGLE_BLADE_THROWABLE)) { // it is throwable if only one blade is on
...
qboolean PM_CheckAltKickAttack(void) {
	if ((pm->cmd.buttons & BUTTON_ALT_ATTACK) && (!(pm->ps->pm_flags & PMF_ALT_ATTACK_HELD) || PM_SaberInReturn(pm->ps->saberMove)) &&
		(!PM_FlippingAnim(pm->ps->legsAnim) || pm->ps->legsAnimTimer <= 250) && (!PM_SaberThrowable()) && pm->ps->SaberActive() &&
...
qboolean PM_CheckUpsideDownAttack(void) {
	...
	if ((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer())) { // FIXME: check ranks?
		return qfalse;
	}
	// FIXME: enemy below
	// FIXME: more than 64 off ground
	if (!g_debugMelee->integer) { // hmm, can't get this to work quite the way we wanted... secret move, then!
		return qfalse;
	}
	...
	case BOTH_FORCEWALLRUNFLIP_END: {
		...
		if (elapsedTime < midPoint - 100.0f ||
			elapsedTime > midPoint + 100.0f) { // only a 200ms window (in middle of anim) of opportunity to do this move in these anims
			return qfalse;
		}
	}
		// NOTE: falls through on purpose
	...
	if (!pm->ps->saberInFlight // not throwing saber
		...
		*/
		...
		&& G_EnoughPowerForSpecialMove(pm->ps->forcePower, SABER_ALT_ATTACK_POWER,
									   qtrue) /*pm->ps->forcePower >= SABER_ALT_ATTACK_POWER*/) // SINGLE_SPECIAL_POWER )// have enough power
...
void PM_TryAirKick(saberMoveName_t kickMove) {
	if (pm->ps->groundEntityNum < ENTITYNUM_NONE) { // just do it
		...
		// it's silly to be able to do them right as you land.
		// also looks wrong to transition from a non-complete flip anim...
		if ((!PM_FlippingAnim(pm->ps->legsAnim) || pm->ps->legsAnimTimer <= 0) && gDist > 64.0f && // strict minimum
			gDist > (-pm->ps->velocity[2]) - 64.0f // make sure we are high to ground relative to downward velocity as well
		) {
			PM_SetSaberMove(kickMove);
		} else {											  // leave it as a normal kick unless we're too high up
			if (gDist > 128.0f || pm->ps->velocity[2] >= 0) { // off ground, but too close to ground
...
void PM_CheckKick(void) {
	...
	{										// player kicks
		...
		if (pm->cmd.rightmove) {		 // kick to side
			...
			pm->cmd.forwardmove = 0;
		} else if (pm->gent && pm->gent->enemy && G_CanKickEntity(pm->gent, pm->gent->enemy)) { // auto-pick?
			if (																				/*(pm->ps->pm_flags&PMF_ALT_ATTACK_HELD)
				...
				if (pm->ps->groundEntityNum == ENTITYNUM_NONE &&
					(pm->ps->saberMove == LS_KICK_S || pm->ps->saberMove == LS_KICK_BF ||
					 pm->ps->saberMove == LS_KICK_RL)) { // in the air and doing a jump-kick, which is a ground anim, so....
					// cut z velocity...?
					pm->ps->velocity[2] = 0;
				}
				pm->cmd.upmove = 0;
...
void PM_CheckClearSaberBlock(void) {
	if (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) {										// player
		...
		} else if (pm->ps->saberBlocked == BLOCKED_UPPER_LEFT && pm->ps->powerups[PW_SHOCKED] > level.time) { // probably blocking lightning
...
qboolean PM_SaberBlocking(void) {
	...
	if (pm->ps->saberBlocked) {
		...
		} else if (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) // player
		{
			if (pm->ps->saberBlocked >= BLOCKED_UPPER_RIGHT_PROJ && pm->ps->saberBlocked <= BLOCKED_TOP_PROJ) { // blocking a projectile
				if ((pm->cmd.buttons & BUTTON_ATTACK)) {														// trying to attack
					if (pm->ps->saberMove == LS_READY || PM_SaberInReflect(pm->ps->saberMove)) { // not already busy or already in projectile deflection
						// trying to attack during projectile blocking, so do the attack instead
						pm->ps->saberBlocked = BLOCKED_NONE;
						pm->ps->saberBounceMove = LS_NONE;
						pm->ps->weaponstate = WEAPON_READY;
						if (PM_SaberInReflect(pm->ps->saberMove) && pm->ps->weaponTime > 0) { // interrupt the current deflection move
							pm->ps->weaponTime = 0;
						}
						return qfalse;
					}
...
qboolean PM_NPCCheckAttackRoll(void) {
	if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer() // NPC
		...
		&& (pm->gent->NPC->rank > RANK_CREWMAN && !Q_irand(0, 3 - g_spskill->integer)) && pm->gent->enemy &&
		...
		InFOV(pm->gent->enemy->currentOrigin, pm->ps->origin, pm->ps->viewangles, 30, 90)) { // stab!
		return qtrue;
	}
	return qfalse;
}
...
void PM_WeaponLightsaber(void) {
	...
	if (pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_SABER_DROID) { // Saber droid does it's own attack logic
		PM_SaberDroidWeapon();
		return;
	}
	...
	if (pm->ps->stats[STAT_WEAPONS] & (1 << WP_SCEPTER) && !pm->ps->dualSabers && pm->gent &&
		pm->gent->weaponModel[1]) { // holding scepter in left hand, use dual style
		...
		animLevelOverridden = qtrue;
			   ...
			   !pm->ps->saber[0].blade[1].active) { // using a staff, but only with first blade turned on, so use is as a normal saber...?
		...
		animLevelOverridden = qtrue;
			   ...
			   WP_SaberStyleValidForSaber(pm->gent, cg.saberAnimLevelPending)) { // go ahead and use the cg.saberAnimLevelPending below
		animLevelOverridden = qfalse;
	} else if (pm->gent && (WP_SaberStyleValidForSaber(pm->gent, pm->ps->saberAnimLevel) ||
							WP_UseFirstValidSaberStyle(pm->gent, &pm->ps->saberAnimLevel))) { // style we are using is not valid, switched us to a valid one
		animLevelOverridden = qtrue;
	}
	/*
	...
	*/
		...
		if (pm->ps->saber[1].Active()) { // if second saber is on, must use dual style
			pm->ps->saberAnimLevel = SS_DUAL;
			animLevelOverridden = qtrue;
		} else if (pm->ps->saber[0].Active()) { // with only one saber on, use fast style
			pm->ps->saberAnimLevel = SS_FAST;
			animLevelOverridden = qtrue;
		}
	}
	if (!animLevelOverridden) {
		if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && cg.saberAnimLevelPending > SS_NONE &&
	...
	} else if ((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())) { // if overrid the player's saberAnimLevel, let the cgame know
		cg.saberAnimLevelPending = pm->ps->saberAnimLevel;
	}
	/*
	...
	*/
			...
			if ((pm->cmd.buttons & BUTTON_ATTACK) || PM_NPCCheckAttackRoll()) {
				if (G_EnoughPowerForSpecialMove(pm->ps->forcePower, SABER_ALT_ATTACK_POWER_FB)) {
					if (!(pm->ps->saber[0].saberFlags & SFL_NO_ROLL_STAB) &&
						(!pm->ps->dualSabers || !(pm->ps->saber[1].saberFlags & SFL_NO_ROLL_STAB))) { // okay to do roll-stab
	...
	if (pm->ps->torsoAnim == BOTH_FORCELONGLEAP_LAND ||
		(pm->ps->torsoAnim == BOTH_FORCELONGLEAP_START &&
		 !(pm->cmd.buttons & BUTTON_ATTACK))) { // if you're in the long-jump and you're not attacking (or are landing), you're not doing anything
	...
	if (pm->ps->legsAnim == BOTH_FLIP_HOLD7 &&
		!(pm->cmd.buttons & BUTTON_ATTACK)) { // if you're in the upside-down attack hold, don't do anything unless you're attacking
		return;
	}
	...
	if (!PM_SaberInBrokenParry(pm->ps->saberMove) && pm->ps->saberBlocked != BLOCKED_PARRY_BROKEN && !PM_DodgeAnim(pm->ps->torsoAnim) &&
		pm->ps->weaponstate != WEAPON_CHARGING_ALT && pm->ps->weaponstate != WEAPON_CHARGING) { // we're not stuck in a broken parry
		if (pm->ps->saberInFlight) {															// guiding saber
			if (pm->ps->saberEntityNum < ENTITYNUM_NONE && pm->ps->saberEntityNum > 0)			// player is 0
			{																					//
				if (&g_entities[pm->ps->saberEntityNum] != NULL &&
			...
			} else if (pm->ps->weapon == WP_SABER && (!pm->ps->dualSabers || !pm->ps->saber[1].Active())) { // guiding saber
				if (saberInAir) {
	...
	if (PM_LockedAnim(pm->ps->torsoAnim) && pm->ps->torsoAnimTimer) { // can't interrupt these anims ever
		return;
	}
	if (PM_SuperBreakLoseAnim(pm->ps->torsoAnim) && pm->ps->torsoAnimTimer) { // don't interrupt these anims
		return;
	}
	if (PM_SuperBreakWinAnim(pm->ps->torsoAnim) && pm->ps->torsoAnimTimer) { // don't interrupt these anims
		return;
	}
	...
	if (pm->ps->weaponstate == WEAPON_RAISING) { // Just selected the weapon
		...
		if (pm->gent && (pm->gent->s.number < MAX_CLIENTS || G_ControlledByPlayer(pm->gent))) {
			...
			if (pm->ps->weapon == WP_SABER && pm->ps->saberInFlight && saberInAir && (!pm->ps->dualSabers || !pm->ps->saber[1].Active())) { // guiding saber
				if (!PM_ForceAnim(pm->ps->torsoAnim) || pm->ps->torsoAnimTimer < 300) { // don't interrupt a force power anim
					if (pm->ps->torsoAnim != BOTH_LOSE_SABER || !pm->ps->torsoAnimTimer) {
						PM_SetAnim(pm, SETANIM_TORSO, BOTH_SABERPULL, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
					}
	...
	if (PM_CheckAltKickAttack()) { // trying to do a kick
		// FIXME: in-air kicks?
		if (pm->ps->saberAnimLevel == SS_STAFF && (pm->ps->clientNum >= MAX_CLIENTS || PM_ControlledByPlayer())) { // NPCs spin the staff
	...
	if (!delayed_fire) {
		...
		if (pm->ps->weapon == WP_SABER && pm->ps->saberInFlight && saberInAir && (!pm->ps->dualSabers || !pm->ps->saber[1].Active())) { // guiding saber
			...
			if (pm->ps->torsoAnim == BOTH_FORCELONGLEAP_ATTACK || pm->ps->torsoAnim == BOTH_FORCELONGLEAP_LAND) { // can't attack in these anims
				...
				if (pm->ps->torsoAnimTimer >= 200) {					// hit it early enough to do the attack
			...
			if (curmove >= LS_PARRY_UP && curmove <= LS_REFLECT_LL) {				// from a parry or reflection, can go directly into an attack
				if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) { // NPCs
			...
			if (anim == -1) {
				...
				{	   // get attack move from movement command
					...
					if (pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer() &&
						(Q_irand(0, pm->ps->forcePowerLevel[FP_SABER_OFFENSE] - 1) ||
						 ...
						 || (pm->ps->saberAnimLevel == SS_FAST && pm->gent && pm->gent->NPC && pm->gent->NPC->rank >= RANK_LT_JG &&
				...
				*/
				if (newmove != LS_NONE) {
					if (!PM_InCartwheel(pm->ps->legsAnim)) { // don't do transitions when cartwheeling - could make you spin!
			...
			if (anim == -1) {																		 // not side-stepping, pick neutral anim
				if (!G_TryingSpecial(pm->gent, &pm->cmd) /*(pm->cmd.buttons&BUTTON_FORCE_FOCUS)*/) { // but only if not trying one of the special attacks!
					if (PM_InCartwheel(pm->ps->legsAnim) &&
						pm->ps->legsAnimTimer > 100) { // if in the middle of a cartwheel, the chain attack is just a normal attack
				...
				if (g_timescale->value < 1.0f) {
					if (!MatrixMode) { // Special test for Matrix Mode (tm)
						if (pm->ps->clientNum == 0 && !player_locked &&
							(pm->ps->forcePowersActive & (1 << FP_SPEED) ||
							 pm->ps->forcePowersActive & (1 << FP_RAGE))) { // player always fires at normal speed
							addTime *= g_timescale->value;
...
static void PM_Weapon(void) {
	...
	{																						   // Separate logic for lightsaber, but not for player when zoomed
		PM_WeaponLightsaber();
		if (pm->gent && pm->gent->client && pm->ps->saber[0].Active() && pm->ps->saberInFlight) { // FIXME: put saberTrail in playerState
	...
	if (pm->gent && pm->gent->client) {
		if (pm->gent->client->fireDelay > 0) { // FIXME: this is going to fire off one frame before you expect, actually
			pm->gent->client->fireDelay -= pml.msec;
			if (pm->gent->client->fireDelay <= 0) { // just finished delay timer
	...
	if (pm->gent) {															  // ready to throw thermal again, add it
		if (pm->ps->weapon == WP_THERMAL && pm->gent->weaponModel[0] == -1) { // add the thermal model back in our hand
	...
	if (!delayed_fire) {
		if (pm->ps->weapon == WP_MELEE && (pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer())) {		  // melee
			if ((pm->cmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK)) != (BUTTON_ATTACK | BUTTON_ALT_ATTACK)) { // not holding both buttons
				if ((pm->cmd.buttons & BUTTON_ATTACK) && (pm->ps->pm_flags & PMF_ATTACK_HELD)) {				  // held button
					// clear it
					pm->cmd.buttons &= ~BUTTON_ATTACK;
				}
				if ((pm->cmd.buttons & BUTTON_ALT_ATTACK) && (pm->ps->pm_flags & PMF_ALT_ATTACK_HELD)) { // held button
					// clear it
					pm->cmd.buttons &= ~BUTTON_ALT_ATTACK;
				}
			}
	...
	*/
			...
			case WP_DISRUPTOR:
				if (((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer()) && pm->gent && pm->gent->NPC &&
					...
					((pm->ps->clientNum < MAX_CLIENTS || PM_ControlledByPlayer()) && cg.zoomMode == 2)) { // NPC or player in alt-fire, sniper mode
					PM_SetAnim(pm, SETANIM_TORSO, BOTH_ATTACK4, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
	...
	if (pm->ps->weapon == WP_EMPLACED_GUN) {
		if (pm->gent && pm->gent->owner && pm->gent->owner->e_UseFunc == useF_eweb_use) { // eweb always shoots alt-fire, for proper effects and sounds
			PM_AddEvent(EV_ALT_FIRE);
			addTime = weaponData[pm->ps->weapon].altFireTime;
	...
	} else if ((pm->ps->weapon == WP_MELEE && (pm->ps->clientNum >= MAX_CLIENTS || !g_debugMelee->integer)) || pm->ps->weapon == WP_TUSKEN_STAFF ||
		...
		if (pm->ps->weapon == WP_THERMAL) { // threw our thermal
			if (pm->gent) {					// remove the thermal model if we had it.
				G_RemoveWeaponModels(pm->gent);
				if ((pm->ps->clientNum >= MAX_CLIENTS && !PM_ControlledByPlayer())) { // NPCs need to know when to put the thermal back in their hand
					pm->ps->weaponTime = pm->ps->torsoAnimTimer - 500;
				}
			}
		}
	...
	if (g_timescale != NULL) {
		if (g_timescale->value < 1.0f) {
			if (!MatrixMode) { // Special test for Matrix Mode (tm)
				if (pm->ps->clientNum == 0 && !player_locked &&
					(pm->ps->forcePowersActive & (1 << FP_SPEED) || pm->ps->forcePowersActive & (1 << FP_RAGE))) { // player always fires at normal speed
					addTime *= g_timescale->value;
				} else if (g_entities[pm->ps->clientNum].client &&
						   (pm->ps->forcePowersActive & (1 << FP_SPEED) || pm->ps->forcePowersActive & (1 << FP_RAGE))) {
					addTime *= g_timescale->value;
	...
	if (ucmd->buttons & BUTTON_USE_FORCE) {
		if (!(ent->client->ps.pm_flags & PMF_USEFORCE_HELD)) {
			...
			case FP_RAGE: // duration - speed, invincibility and extra damage for short period, drains your health and leaves you weak and slow afterwards.
				ForceRage(ent);
				break;
			case FP_PROTECT: // duration - protect against physical/energy (level 1 stops blaster/energy bolts, level 2 stops projectiles, level 3 protects
							 // against explosions)
				ForceProtect(ent);
				break;
	...
	if (pm->gent) {
		if (pm->gent->client && pm->gent->client->moveType == MT_FLYSWIM) {
			...
			if (pm->gent->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER) {
				Flying = FLY_VEHICLE;
			} else if (pm->gent->m_pVehicle->m_pVehicleInfo->hoverHeight > 0) { // FIXME: or just check for hoverHeight?
				Flying = FLY_HOVER;
			}
		}
	}
...
//-------------------------------------------
{
	...
	if (!g_saberAutoBlocking->integer && !g_saberNewControlScheme->integer && (pm->cmd.buttons & BUTTON_FORCE_FOCUS)) {
	...
	if (pm->ps->weapon == WP_SABER && (!cg.zoomMode || pm->ps->clientNum)) { // don't let the alt-attack be interpreted as an actual attack command
		if (pm->ps->saberInFlight) {
			...
			if ((!pm->ps->dualSabers || !pm->ps->saber[1].Active())) { // saber not in hand, can't swing it
				pm->cmd.buttons &= ~BUTTON_ATTACK;
			}
		}
		// saber staff alt-attack does a special attack anim, non-throwable sabers do kicks
		if (pm->ps->saberAnimLevel != SS_STAFF &&
			!(pm->ps->saber[0].saberFlags & SFL_NOT_THROWABLE)) {	   // using a throwable saber, so remove the saber throw button
			if (!g_saberNewControlScheme->integer && PM_CanDoKata()) { // old control scheme - alt-attack + attack does kata
			} else { // new control scheme - alt-attack doesn't have anything to do with katas, safe to clear it here
				pm->cmd.buttons &= ~BUTTON_ALT_ATTACK;
			}
		}
	}
	...
	if (((pm->cmd.buttons & BUTTON_ATTACK || pm->cmd.buttons & BUTTON_ALT_ATTACK) && (amount >= 0 || pm->ps->weapon == WP_SABER))) {
		if (pm->cmd.buttons & BUTTON_ALT_ATTACK) {
			pm->ps->eFlags |= EF_ALT_FIRING;
			if (pm->ps->clientNum < MAX_CLIENTS && pm->gent && (pm->ps->eFlags & EF_IN_ATST)) { // switch ATST barrels
...
void PM_CheckInVehicleSaberAttackAnim(void) { // A bit of a hack, but makes the vehicle saber attacks act like any other saber attack...
	// make weapon function
	if (pm->ps->weaponTime > 0) {
		pm->ps->weaponTime -= pml.msec;
		if (pm->ps->weaponTime <= 0) {
...
void Pmove(pmove_t *pmove) {
	...
	if (pm->ps->clientNum >= MAX_CLIENTS && pm->gent && pm->gent->client && pm->gent->client->NPC_class == CLASS_VEHICLE) { // we are a vehicle
		pVeh = pm->gent->m_pVehicle;
		assert(pVeh);
		if (pVeh) {
		...
		if (pm->ps->vehTurnaroundIndex && pm->ps->vehTurnaroundTime > pm->cmd.serverTime) { // riding this vehicle, turn my view too
	...
	if (pm->ps && ((pm->ps->eFlags & EF_LOCKED_TO_WEAPON) || (pm->ps->eFlags & EF_HELD_BY_RANCOR) || (pm->ps->eFlags & EF_HELD_BY_WAMPA) ||
	...
	// If we are riding a vehicle...
		...
		if (pm->cmd.buttons & BUTTON_ALT_ATTACK) {			 // alt attack always does other stuff when riding a vehicle (turbo)
		} else if ((pm->ps->eFlags & EF_NODRAW)) {			 // inside a vehicle?  don't do any weapon stuff
				   ...
				   || (pm->cmd.weapon != pm->ps->weapon &&
					   PM_WeaponOkOnVehicle(pm->cmd.weapon))) // FIXME: make this a vehicle call to see if this new weapon is valid for this vehicle
		{ // either weilding a weapon we can fire with normal weapon logic, or trying to change to a valid weapon
			// call normal weapons code... should we override the normal fire anims with vehicle fire anims in here or in a subsequent call to VehicleWeapons or
			// something?
