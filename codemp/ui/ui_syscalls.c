...
void trap_Cvar_Create(const char *var_name, const char *var_value,
                      uint32_t flags) {
...
int trap_R_Font_StrLenPixels(const char *text, const int iFontIndex,
  ...
  float width = (float)Q_syscall(UI_R_FONT_STRLENPIXELS, text, iFontIndex,
                                 PASSFLOAT(1.0f));
  return width * scale;
}
...
int trap_R_Font_HeightPixels(const int iFontIndex, const float scale) {
  float height =
      (float)Q_syscall(UI_R_FONT_STRHEIGHTPIXELS, iFontIndex, PASSFLOAT(1.0f));
  return height * scale;
}
void trap_R_Font_DrawString(int ox, int oy, const char *text, const float *rgba,
                            const int setIndex, int iCharLimit,
                            const float scale) {
  Q_syscall(UI_R_FONT_DRAWSTRING, ox, oy, text, rgba, setIndex, iCharLimit,
            PASSFLOAT(scale));
}
...
unsigned int
trap_AnyLanguage_ReadCharFromString(const char *psText, int *piAdvanceCount,
                                    qboolean *pbIsTrailingPunctuation) {
  return Q_syscall(UI_ANYLANGUAGE_READCHARFROMSTRING, psText, piAdvanceCount,
...
int trap_CM_LerpTag(orientation_t *tag, clipHandle_t mod, int startFrame,
  ...
  return Q_syscall(UI_CM_LERPTAG, tag, mod, startFrame, endFrame,
...
qboolean trap_SP_GetStringTextString(const char *text, char *buffer,
                                     int bufferLength) {
...
qboolean trap_G2API_GetBoltMatrix(void *ghoul2, const int modelIndex,
                                  ...
                                  vec3_t scale) {
  return (qboolean)(Q_syscall(UI_G2_GETBOLT, ghoul2, modelIndex, boltIndex,
...
qboolean trap_G2API_GetBoltMatrix_NoReconstruct(
  ...
  return (qboolean)(Q_syscall(UI_G2_GETBOLT_NOREC, ghoul2, modelIndex,
...
qboolean trap_G2API_GetBoltMatrix_NoRecNoRot(
    ...
    const vec3_t angles, const vec3_t position, const int frameNum,
    qhandle_t *modelList, vec3_t scale) {
  return (qboolean)(Q_syscall(UI_G2_GETBOLT_NOREC_NOROT, ghoul2, modelIndex,
                              boltIndex, matrix, angles, position, frameNum,
                              modelList, scale));
}
int trap_G2API_InitGhoul2Model(void **ghoul2Ptr, const char *fileName,
                               ...
                               int lodBias) {
  return Q_syscall(UI_G2_INITGHOUL2MODEL, ghoul2Ptr, fileName, modelIndex,
...
void trap_G2API_CollisionDetect(CollisionRecord_t *collRecMap, void *ghoul2,
                                ...
                                float fRadius) {
  Q_syscall(UI_G2_COLLISIONDETECT, collRecMap, ghoul2, angles, position,
...
void trap_G2API_CollisionDetectCache(CollisionRecord_t *collRecMap,
                                     ...
                                     float fRadius) {
  Q_syscall(UI_G2_COLLISIONDETECTCACHE, collRecMap, ghoul2, angles, position,
            frameNumber, entNum, rayStart, rayEnd, scale, traceFlags, useLod,
            PASSFLOAT(fRadius));
}
...
qboolean trap_G2API_SetBoneAngles(void *ghoul2, int modelIndex,
                                  ...
                                  int currentTime) {
  return (Q_syscall(UI_G2_ANGLEOVERRIDE, ghoul2, modelIndex, boneName, angles,
...
qboolean trap_G2API_SetBoneAnim(void *ghoul2, const int modelIndex,
  ...
  return Q_syscall(UI_G2_PLAYANIM, ghoul2, modelIndex, boneName, startFrame,
...
qboolean trap_G2API_GetBoneAnim(void *ghoul2, const char *boneName,
  ...
  return Q_syscall(UI_G2_GETBONEANIM, ghoul2, boneName, currentTime,
...
qboolean trap_G2API_GetBoneFrame(void *ghoul2, const char *boneName,
  ...
  return Q_syscall(UI_G2_GETBONEFRAME, ghoul2, boneName, currentTime,
...
qboolean trap_G2API_SetBoneIKState(void *ghoul2, int time, const char *boneName,
  ...
  return Q_syscall(UI_G2_SETBONEIKSTATE, ghoul2, time, boneName, ikState,
                   params);
}
...
qboolean trap_G2API_AttachG2Model(void *ghoul2From, int modelIndexFrom,
                                  void *ghoul2To, int toBoltIndex,
                                  int toModel) {
  return Q_syscall(UI_G2_ATTACHG2MODEL, ghoul2From, modelIndexFrom, ghoul2To,
                   toBoltIndex, toModel);
}
...
void UISyscall_R_AddPolysToScene(qhandle_t hShader, int numVerts,
...
void UISyscall_AddCommand(const char *cmd_name) {
  ...
  Com_Printf(S_COLOR_YELLOW "WARNING: trap->ext.AddCommand() is only supported "
                            "with OpenJK mod API!\n");
}

void UISyscall_RemoveCommand(const char *cmd_name) {
  // TODO warn developer only
  Com_Printf(S_COLOR_YELLOW "WARNING: trap->ext.RemoveCommand() is only "
                            "supported with OpenJK mod API!\n");
}
