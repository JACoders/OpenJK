...
static void CM_LoadMap_Actual(const char *name, qboolean clientload, int *checksum, clipMap_t &cm) {
	...
	if (&cm == &cmg) {
		...
		//
		if (gpvCachedMapDiskImage && (gsCachedMapDiskImage[0] == '\0' || strcmp(gsCachedMapDiskImage, name))) {
			Z_Free(gpvCachedMapDiskImage);
			gpvCachedMapDiskImage = NULL;
			gsCachedMapDiskImage[0] = '\0';
	...
	if (name[0] && !strcmp(cm.name, name) && !clientload && &cm == &cmg) {
		...
		if (&cm == &cmg) {
			if (gpvCachedMapDiskImage && gsCachedMapDiskImage[0]) { // didn't get cleared elsewhere so free it before we allocate the pointer again
																	// Maps with terrain will allow this to happen because they want everything to be cleared
...
cmodel_t *CM_ClipHandleToModel(clipHandle_t handle, clipMap_t **clipMap) {
	...
	if (handle < MAX_SUBMODELS) {
		Com_Error(ERR_DROP, "CM_ClipHandleToModel: bad handle %i < %i < %i", cmg.numSubModels, handle, MAX_SUBMODELS);
	}
...
void CM_WritePortalState() {
	...
	saved_game.write_chunk<int32_t>(INT_ID('P', 'R', 'T', 'S'), ::cmg.areaPortals, ::cmg.numAreas * ::cmg.numAreas);
}
...
void CM_ReadPortalState() {
	...
	saved_game.read_chunk<int32_t>(INT_ID('P', 'R', 'T', 'S'), ::cmg.areaPortals, ::cmg.numAreas * ::cmg.numAreas);
