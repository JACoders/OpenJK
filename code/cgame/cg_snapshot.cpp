...
void CG_TransitionSnapshot(void) {
  ...
  for (i = 0; i < cg.snap->numEntities; i++) {
    if (1) // cg.snap->entities[ i ].number != 0 ) // I guess the player adds
           // his/her events elsewhere, so doing this also gives us double events
           // for the player!
    {
      cent = &cg_entities[cg.snap->entities[i].number];
...
void CG_SetEntityNextState(centity_t *cent, entityState_t *state) {
  ...
  // since we can't interpolate ghoul2 stuff from one frame to another, I'm just
  // going to copy the ghoul2 info directly into the current state now
  ...
  if (!cent->currentValid ||
      ((cent->currentState.eFlags ^ state->eFlags) & EF_TELEPORT_BIT)) {
    cent->interpolate = qfalse;
  } else {
