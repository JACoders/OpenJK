...
extern void Vehicle_SetAnim(gentity_t *ent, int setAnimParts, int anim,
    ...
    /*		else
                    {
                            ...
                            {
                                    G_PlayEffect(
       pVeh->m_pVehicleInfo->explodeFX, parent->currentOrigin );
                                    //trace down and place mark
                                    VectorCopy( parent->currentOrigin, bottom );
                                    bottom[2] -= 80;
                                    gi.trace( &trace, parent->currentOrigin,
       vec3_origin, vec3_origin, bottom, parent->s.number, CONTENTS_SOLID ); if
       ( trace.fraction < 1.0f )
                                    {
                                            VectorCopy( trace.endpos, bottom );
                                            bottom[2] += 2;
                                            G_PlayEffect(
       "ships/ship_explosion_mark", trace.endpos );
                                    }
                            }

                            parent->takedamage = qfalse;//so we don't
       recursively damage ourselves if ( pVeh->m_pVehicleInfo->explosionRadius >
       0 && pVeh->m_pVehicleInfo->explosionDamage > 0 )
                            {
                                    VectorCopy( parent->mins, mins );
                                    mins[2] = -4;//to keep it off the ground a
       *little* VectorCopy( parent->maxs, maxs ); VectorCopy(
       parent->currentOrigin, bottom ); bottom[2] += parent->mins[2] - 32;
                                    gi.trace( &trace, parent->currentOrigin,
       mins, maxs, bottom, parent->s.number, CONTENTS_SOLID ); G_RadiusDamage(
       trace.endpos, NULL, pVeh->m_pVehicleInfo->explosionDamage,
       pVeh->m_pVehicleInfo->explosionRadius, NULL, MOD_EXPLOSIVE );//FIXME:
       extern damage and radius or base on fuel
                            }
...
static void ProcessMoveCommands(Vehicle_t *pVeh) {
  ...
  if (pVeh->m_ulFlags & VEH_BUCKING || pVeh->m_ulFlags & VEH_FLYING ||
  ...
  if (curTime > pVeh->m_iTurboTime && (pVeh->m_ucmd.buttons & BUTTON_WALKING) &&
...
static void ProcessOrientCommands(Vehicle_t *pVeh) {
  ...
  parentPS->speed/200.0f*0.05f;
                  ...
                  if ( pVeh->m_pVehicleInfo->malfunctionArmorLevel &&
...
/*
static void AnimalTailSwipe(Vehicle_t* pVeh, gentity_t *parent, gentity_t
*pilot)
{
        trace_t	trace;
        vec3_t angles;
        ...
        trap_G2API_GetBoltMatrix( parent->ghoul2, 0, iRootBone,
                                &boltMatrix, angles, parent->currentOrigin,
level.time, NULL, parent->modelScale ); BG_GiveMeVectorFromMatrix( &boltMatrix,
ORIGIN, vRoot );

        trap_G2API_GetBoltMatrix( parent->ghoul2, 0, iRootTail,
                                &boltMatrix, angles, parent->currentOrigin,
level.time, NULL, parent->modelScale ); BG_GiveMeVectorFromMatrix( &boltMatrix,
ORIGIN, vTail ); #else iRootBone = gi.G2API_GetBoneIndex(
&parent->ghoul2[parent->playerModel], "tail_01", qtrue ); iRootTail =
gi.G2API_GetBoneIndex( &parent->ghoul2[parent->playerModel], "tail_04", qtrue );
...
(cg.time?cg.time:level.time), NULL, parent->s.modelScale );
        ...
        G_VehicleTrace( &trace, vRoot, lMins, lMaxs, vTail, parent->s.number,
MASK_NPCSOLID ); if ( trace.fraction < 1.0f )
        {
                if ( ENTITYNUM_NONE != trace.entityNum &&
g_entities[trace.entityNum].client && #ifndef _JK2MP //no rancor in jk2mp (at
least not currently) g_entities[trace.entityNum].client->NPC_class !=
CLASS_RANCOR && #else //and in mp want to check inuse
                        g_entities[trace.entityNum].inuse &&
#endif
                        g_entities[trace.entityNum].client->NPC_class !=
CLASS_VEHICLE )
                {
                        vec3_t pushDir;
                        vec3_t angs;
                        ...
                        // Smack this ho down.
#ifdef _JK2MP
                        G_Sound( &g_entities[trace.entityNum], CHAN_AUTO,
G_SoundIndex( "sound/chars/rancor/swipehit.wav" ) ); #else G_Sound(
&g_entities[trace.entityNum], G_SoundIndex( "sound/chars/rancor/swipehit.wav" )
); #endif G_Throw( &g_entities[trace.entityNum], pushDir, 50 );
                        ...
                        if ( g_entities[trace.entityNum].health > 0 )
                        {
                                ...
                                {
                                        hit->client->ps.forceHandExtend =
HANDEXTEND_KNOCKDOWN; hit->client->ps.forceHandExtendTime = pm->cmd.serverTime +
1100; hit->client->ps.forceDodgeAnim = 0; //this toggles between 1 and 0, when
it's 1 we should play the get up anim

                                        hit->client->ps.otherKiller =
pilot->s.number; hit->client->ps.otherKillerTime = level.time + 5000;
                                        hit->client->ps.otherKillerDebounceTime
= level.time + 100;

                                        hit->client->ps.velocity[0] =
pushDir[0]*80; hit->client->ps.velocity[1] = pushDir[1]*80;
                                        hit->client->ps.velocity[2] = 100;
                                }
#else
                                G_Knockdown( hit, parent, pushDir, 300, qtrue );
#endif
                                G_Damage( hit, parent, parent, NULL, NULL,
iDamage, DAMAGE_NO_KNOCKBACK | DAMAGE_IGNORE_TEAM, MOD_MELEE );
                                //G_PlayEffect( pVeh->m_pVehicleInfo->explodeFX,
...
static void AnimateVehicle(Vehicle_t *pVeh) {
  ...
  // We're dead (boarding is reused here so I don't have to make another
  // variable :-).
  if (parent->health <= 0) {
  ...
  if (pVeh->m_iBoarding != 0) {
    // We've just started boarding, set the amount of time it will take to
    // finish boarding.
    if (pVeh->m_iBoarding < 0) {
      int iAnimLen;
      ...
      // 70% (0.7f) of the animation time.
#ifdef _JK2MP
      iAnimLen = BG_AnimLength(parent->localAnimIndex, Anim) * 0.7f;
#else
      iAnimLen =
          PM_AnimLength(parent->client->clientInfo.animFileIndex, Anim) * 0.7f;
#endif
  ...
  if (fSpeedPercToMax < -0.01f) {
    ...
    bool Walking =
        (fSpeedPercToMax > 0.0f && ((pVeh->m_ucmd.buttons & BUTTON_WALKING) ||
                                    fSpeedPercToMax <= 0.275f));
    ...
    if (Turbo) { // Kicked In Turbo
      ...
      Anim = (Walking) ? (BOTH_VT_WALK_FWD)
                       : ((Running) ? (BOTH_VT_RUN_FWD) : (BOTH_VT_IDLE1));
    }
  }
  Vehicle_SetAnim(parent, SETANIM_LEGS, Anim, iFlags, iBlend);
}

// rwwFIXMEFIXME: This is all going to have to be predicted I think, or it will
// feel awful and lagged
...
#else
    if (pilotPS->torsoAnim >= BOTH_VT_ATL_S &&
        pilotPS->torsoAnim <= BOTH_VT_ATF_G) {
    ...
    if (Attacking && WeaponPose) { // Attack!
      ...
      if (WeaponPose == WPOSE_NONE) {
        if (Walking) {
          ...
          Anim =
              BOTH_VT_IDLE1; //(Q_irand(0,1)==0)?(BOTH_VT_IDLE):(BOTH_VT_IDLE1);
        }
      } else {
        switch (WeaponPose) {
        case WPOSE_BLASTER:
