...
void RT_FireDecide(void) {
	...
	if (NPC->client->ps.groundEntityNum == ENTITYNUM_NONE && NPC->client->ps.forceJumpZStart && !PM_FlippingAnim(NPC->client->ps.legsAnim) &&
	...
	if (TIMER_Done(NPC, "nextAttackDelay") && TIMER_Done(NPC, "flameTime")) {
		if (NPC_ClearLOS(NPC->enemy)) {
			...
			if (NPC->client->ps.weapon == WP_NONE) {
				...
				if ((NPC->client->ps.weapon == WP_ROCKET_LAUNCHER || (NPC->client->ps.weapon == WP_CONCUSSION && !(NPCInfo->scriptFlags & SCF_ALT_FIRE)) ||
		...
		if (NPC->client->fireDelay) {
			...
			if (TIMER_Done(NPC, "nextAttackDelay")) {
				...
				if (NPC->s.weapon == WP_ROCKET_LAUNCHER) {
					if ((ucmd.buttons & BUTTON_ATTACK) && !Q_irand(0, altChance)) { // every now and then, shoot a homing rocket
						ucmd.buttons &= ~BUTTON_ATTACK;
						ucmd.buttons |= BUTTON_ALT_ATTACK;
						NPC->client->fireDelay = Q_irand(1000, 3000); // FIXME: base on g_spskill
					}
				} else if (NPC->s.weapon == WP_CONCUSSION) {
					if ((ucmd.buttons & BUTTON_ATTACK) && Q_irand(0, altChance * 5)) { // fire the beam shot
						ucmd.buttons &= ~BUTTON_ATTACK;
						ucmd.buttons |= BUTTON_ALT_ATTACK;
...
// FLYING behavior
//=====================================================================================
qboolean RT_Flying(gentity_t *self) { return ((qboolean)(self->client->moveType == MT_FLYSWIM)); }
...
void RT_Flying_MaintainHeight(void) {
	...
	if (NPC->forcePushTime > level.time) { // if being pushed, we don't have control over our movement
		return;
	}

	if ((NPC->client->ps.pm_flags & PMF_TIME_KNOCKBACK)) { // don't slow down for a bit
	...
	if (NPC->enemy && (!Q3_TaskIDPending(NPC, TID_MOVE_NAV) || !NPCInfo->goalEntity)) {
		if (TIMER_Done(NPC, "heightChange")) {
			...
			if (NPC->enemy->client && NPC->enemy->client->ps.groundEntityNum == ENTITYNUM_NONE &&
				(NPC->enemy->client->ps.forcePowersActive & (1 << FP_LEVITATION))) { // so we don't go up when they force jump up at us
		...
		} else if (VectorCompare(NPC->pos1, vec3_origin)) { // have a starting position as a reference point
...
void RT_Flying_Hunt(qboolean visible, qboolean advance) {
	...
	if (advance && distance < Q_flrand(256, 3096)) {
		...
		if (NPC->health <= 50) { // always back off
			NPC->delay = 0;
		} else if (!TIMER_Done(NPC, "backoffTime")) { // still backing off from end of last delay
			NPC->delay = 0;
...
void RT_Flying_Think(void) {
	if (Q3_TaskIDPending(NPC, TID_MOVE_NAV) && UpdateGoal()) { // being scripted to go to a certain spot, don't maintain height
