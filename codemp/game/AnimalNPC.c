  ...
  if (level.time >= pVeh->m_iDieTime) {
    ...
    /*		else
       ...
       pVeh->m_pVehicleInfo->explodeFX, parent->currentOrigin );
                                    ...
                                    bottom[2] -= 80;
                                    trap->trace( &trace, parent->currentOrigin,
...
static void ProcessMoveCommands(Vehicle_t *pVeh) {
  ...
  //	speedIdleAccel = pVeh->m_pVehicleInfo->accelIdle *
...
static void AnimateVehicle(Vehicle_t *pVeh) {
  ...
  if (parent->health <= 0) {
    ...
    if ( pVeh->m_iBoarding != -999 )	// Animate the death just once!
    {
            ...
            //Vehicle_SetAnim( parent, SETANIM_LEGS, BOTH_VT_DEATH1, iFlags,
    iBlend );
    }
    */
    return;
  ...
  if (fSpeedPercToMax < -0.01f) {
    ...
    qboolean Turbo =
        (fSpeedPercToMax > 0.0f && level.time < pVeh->m_iTurboTime);
    qboolean Walking =
...
static void AnimateRiders(Vehicle_t *pVeh) {
  ...
  if (0) {
    ...
    qboolean Attacking =
        (HasWeapon && !!(pVeh->m_ucmd.buttons & BUTTON_ATTACK));
