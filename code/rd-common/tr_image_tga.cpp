...
typedef struct TGAHeader_s {
  ...
  byte byImageType;          // 1 = colour mapped (palette), uncompressed, 2 =
                             // truecolour, uncompressed, else bad
  ...
  byte byImagePlanes;        // bits per pixel	(8 for paletted, else 24 for
                             // true-colour)
  byte byScanLineOrder;      // Image descriptor bytes
                             // bits 0-3 = # attr bits (alpha chan)
                             // bits 4-5 = pixel order/dir
                             // bits 6-7 scan line interleave (00b=none,01b=2way
                             // interleave,10b=4way)
} TGAHeader_t;
#pragma pack(pop)
...
//
//  returns false if found but had a format error, else true for either OK or
//  not-found (there's a reason for this)
//
...
void LoadTGA(const char *name, byte **pic, int *width, int *height) {
  ...
  // these don't need to be declared or initialised until later, but the
  // compiler whines that 'goto' skips them.
  //
  byte *pRGBA = NULL;
  byte *pOut = NULL;
  byte *pIn = NULL;
...
#define TGA_FORMAT_ERROR(blah)                                                 \
  ...
  if (pHeader->byImageType != 2 && pHeader->byImageType != 3 &&
      pHeader->byImageType != 10) {
    TGA_FORMAT_ERROR("LoadTGA: Only type 2 (RGB), 3 (gray), and 10 (RLE-RGB) "
                     "images supported\n");
  }
  ...
  if (pHeader->byColourMapEntrySize != 0 &&
      pHeader->byColourMapEntrySize != 24) {
    TGA_FORMAT_ERROR("LoadTGA: ColourMapEntrySize must be either 0 or 24\n");
  }

  if ((pHeader->byImagePlanes != 24 && pHeader->byImagePlanes != 32) &&
      (pHeader->byImagePlanes != 8 && pHeader->byImageType != 3)) {
    TGA_FORMAT_ERROR("LoadTGA: Only type 2 (RGB), 3 (gray), and 10 (RGB) TGA "
                     "images supported\n");
  }
  ...
  // these last checks are so i can use ID's RLE-code. I don't dare fiddle with
  // it or it'll probably break...
  //
  if (pHeader->byImageType == 10) {
    if ((pHeader->byScanLineOrder & 0x30) != 0x00) {
      TGA_FORMAT_ERROR("LoadTGA: RLE-RGB Images (type 10) must be in "
                       "bottom-to-top format\n");
    }
    if (pHeader->byImagePlanes != 24 &&
        pHeader->byImagePlanes !=
            32) // probably won't happen, but avoids compressed greyscales?
    {
      TGA_FORMAT_ERROR(
          "LoadTGA: RLE-RGB Images (type 10) must be 24 or 32 bit\n");
    }
  }
  ...
  default:   // default case stops the compiler complaining about using
             // uninitialised vars
  ...
  pRGBA = (byte *)R_Malloc(pHeader->wImageWidth * pHeader->wImageHeight * 4,
                           TAG_TEMP_WORKSPACE, qfalse);
  *pic = pRGBA;
  pOut = pRGBA;
  pIn = pTempLoadedBuffer + sizeof(*pHeader);

  // I don't know if this ID-thing here is right, since comments that I've seen
  ...
  if (pHeader->byImageType == 2 ||
      pHeader->byImageType == 3) // RGB or greyscale
  {
    for (int y = iYStart, iYCount = 0; iYCount < pHeader->wImageHeight;
         y += iYStep, iYCount++) {
      pOut = pRGBA + y * pHeader->wImageWidth * 4;
      for (int x = iXStart, iXCount = 0; iXCount < pHeader->wImageWidth;
           x += iXStep, iXCount++) {
        ...
        default:
          assert(0); // if we ever hit this, someone deleted a header check
                     // higher up
          TGA_FORMAT_ERROR("LoadTGA: Image can only have 8, 24 or 32 planes "
                           "for RGB/greyscale\n");
          break;
        }
      }
  ...
  {
    // I've no idea if this stuff works, I normally reject RLE targas, but this
    // is from ID's code
