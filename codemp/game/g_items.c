...
void ShieldGoSolid(gentity_t *self)
{
	...
	gi.Trace (&tr, self->r.currentOrigin, self->r.mins, self->r.maxs, self->r.currentOrigin, self->s.number, CONTENTS_BODY, qfalse, 0, 0 );
	if(tr.startsolid)
...
//	static int	shieldID;
	...
	gi.Trace (&tr, ent->r.currentOrigin, NULL, NULL, end, ent->s.number, MASK_SHOT, qfalse, 0, 0 );
	...
	start[2] += (height>>1);
	gi.Trace (&tr, start, 0, 0, posTraceEnd, ent->s.number, MASK_SHOT, qfalse, 0, 0 );
	posWidth = MAX_SHIELD_HALFWIDTH * tr.fraction;
	// negative trace
	gi.Trace (&tr, start, 0, 0, negTraceEnd, ent->s.number, MASK_SHOT, qfalse, 0, 0 );
...
//  negWidth - 0 - 255 8 bits
	...
	// see if we're valid
	gi.Trace (&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, ent->r.currentOrigin, ent->s.number, CONTENTS_BODY, qfalse, 0, 0 ); 
...
qboolean PlaceShield(gentity_t *playerent)
{
	...
	gi.Trace (&tr, playerent->client->ps.origin, mins, maxs, dest, playerent->s.number, MASK_SHOT, qfalse, 0, 0 );
	if (tr.fraction > 0.9)
	{//room in front
		VectorCopy(tr.endpos, pos);
		// drop to floor
		VectorSet( dest, pos[0], pos[1], pos[2] - 4096 );
		gi.Trace( &tr, pos, mins, maxs, dest, playerent->s.number, MASK_SOLID, qfalse, 0, 0 );
		if ( !tr.startsolid && !tr.allsolid )
		{
			...
			gi.LinkEntity ((sharedEntity_t *)shield);
...
//---------------------------------
{
	...
	else
	{
		...
		gi.Trace( &tr, org2, NULL, NULL, org, ent->s.number, MASK_SHOT, qfalse, 0, 0 );
...
//---------------------------------
{
	...
	numListedEntities = gi.EntitiesInBox( testMins, testMaxs, iEntityList, MAX_GENTITIES );
	...
	while (i < numListedEntities)
	{
		...
		{ //client stuck inside me. go nonsolid.
			...
			numListedEntities = gi.EntitiesInBox( g_entities[clNum].r.absmin, g_entities[clNum].r.absmax, iEntityList, MAX_GENTITIES );
...
//------------------------------------------------------------------------
{
	...
	gi.LinkEntity((sharedEntity_t *)sentry);
...
void ItemUse_UseDisp(gentity_t *ent, int type)
{
	...
	if (item)
	{
		...
		gi.LinkEntity((sharedEntity_t *)eItem);
...
void EWebPositionUser(gentity_t *owner, gentity_t *eweb)
{
	...
	gi.Trace(&tr, owner->client->ps.origin, owner->r.mins, owner->r.maxs, p, owner->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	{ //all clear, we can move there
		...
		VectorCopy(p, pDown);
		pDown[2] -= 7.0f;
		gi.Trace(&tr, p, owner->r.mins, owner->r.maxs, pDown, owner->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
...
gentity_t *EWeb_Create(gentity_t *spawner)
{
	...
	gi.Trace(&tr, s, mins, maxs, pos, spawner->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	downPos[2] -= 18.0f;
	gi.Trace(&tr, pos, mins, maxs, downPos, spawner->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
...
int Pickup_Powerup( gentity_t *ent, gentity_t *other ) {
    ...
    // cannot use OnSameTeam as it expects to g_entities, not clients
      ...
      continue;
		...
		gi.Trace( &tr, client->ps.origin, NULL, NULL, ent->s.pos.trBase, ENTITYNUM_NONE, CONTENTS_SOLID, qfalse, 0, 0 );
		if ( tr.fraction != 1.0 ) {
			continue;
		}
...
void RespawnItem( gentity_t *ent ) {
	...
	if (ent->team) {
		...
		if ( !ent->teammaster ) {
			gi.Error( ERR_DROP, "RespawnItem: bad teammaster");
		}
...
gentity_t *LaunchItem( gitem_t *item, vec3_t origin, vec3_t velocity ) {
	...
	gi.LinkEntity ((sharedEntity_t *)dropped);
...
/*	item = &bg_itemlist[ ent->s.modelindex ];
	...
	{
		gi.G2API_InitGhoul2Model(&ent->s, item->world_model[0], G_ModelIndex(item->world_model[0] ), 0, 0, 0, 0);
		ent->s.radius = 60;
	}
*/
	if ( ent->spawnflags & ITMSF_SUSPEND ) {
		...
		if ( tr.startsolid ) {
			gi.Print ("FinishSpawningItem: %s startsolid at %s\n", ent->classname, vtos(ent->s.origin));
			G_FreeEntity( ent );
			return;
		}
...
void G_CheckTeamItems( void ) {
	...
	if( level.gametype == GT_CTF || level.gametype == GT_CTY ) {
		...
		if ( !item || !itemRegistered[ item - bg_itemlist ] ) {
			gi.Print( S_COLOR_YELLOW "WARNING: No team_CTF_redflag in map\n" );
		}
		item = BG_FindItem( "team_CTF_blueflag" );
		if ( !item || !itemRegistered[ item - bg_itemlist ] ) {
			gi.Print( S_COLOR_YELLOW "WARNING: No team_CTF_blueflag in map\n" );
		}
	}
}
...
void RegisterItem( gitem_t *item ) {
	if ( !item ) {
		gi.Error( ERR_DROP, "RegisterItem: NULL" );
	}
...
void G_RunItem( gentity_t *ent ) {
	...
	gi.Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, ent->r.ownerNum, mask, qfalse, 0, 0 );
	...
	gi.LinkEntity( (sharedEntity_t *)ent );	// FIXME: avoid this for stationary?
	...
	contents = gi.PointContents( ent->r.currentOrigin, -1 );
