...
qboolean NPC_StandTrackAndShoot(gentity_t *NPC, qboolean canDuck) {
	...
	if (canDuck && (duck_ok || (!attack_ok && client->fireDelay == 0)) && ucmd.upmove != -127) { // if we didn't attack check to duck if we're not already
		if (!duck_ok) {
			if (NPC->enemy->client) {
	...
	if (NPC->enemy == NULL) { // Possible to pick one up by being shot
		if (Q_flrand(0.0f, 1.0f) < 0.5) {
			if (NPC->client->enemyTeam) {
				gentity_t *newenemy = NPC_PickEnemy(NPC, NPC->client->enemyTeam, (qboolean)(NPC->cantHitEnemyCounter < 10),
...
void NPC_BSHuntAndKill(void) {
	...
	if (NPC->enemy) {
		...
		if (curAnim != BOTH_ATTACK1 && curAnim != BOTH_ATTACK2 && curAnim != BOTH_ATTACK3 && curAnim != BOTH_MELEE1 &&
			...
			if (enemyDist > 48 && ((enemyDist * 1.5) * (enemyDist * 1.5) >= NPC_MaxDistSquaredForWeapon() || oEVis != VIS_SHOOT ||
...
void NPC_BSPointShoot(qboolean shoot) { // FIXME: doesn't check for clear shot...
	...
	if (NPC_UpdateAngles(qtrue, qtrue)) { // FIXME: if angles clamped, this may never work!
		...
		if (!shoot || !(NPC->svFlags & SVF_LOCKEDENEMY)) { // If locked_enemy is on, dont complete until it is destroyed...
			Q3_TaskIDComplete(NPC, TID_BSTATE);
			goto finished;
		}
	} else if (shoot && (NPC->svFlags & SVF_LOCKEDENEMY)) { // shooting them till their dead, not aiming right at them yet...
		/*
...
void NPC_BSDefault(void) {
	...
	if (UpdateGoal()) { // have a goal
		if (!NPC->enemy && NPC->client->leader && NPCInfo->goalEntity == NPC->client->leader && !Q3_TaskIDPending(NPC, TID_MOVE_NAV)) {
			...
			if (NPCInfo->scriptFlags & SCF_FORCED_MARCH) { // being forced to walk
				if (g_crosshairEntNum != NPC->s.number) {  // don't walk if player isn't aiming at me
					move = qfalse;
				}
			}
