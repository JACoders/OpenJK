...
//	G_VehicleAttachDroidUnit( ent );
	...
	if (ent->client->pers.send_event_timer < level.time)
	{
		if (!(ent->client->pers.player_statuses & (1 << 2)))
		...
		else if (!(ent->client->pers.player_statuses & (1 << 3)))
		{
		...
		else if (!(ent->client->pers.player_statuses & (1 << 4)))
		{
			// zyk: event to set the stealth attacker upgrade
			if (client->sess.amrpgmode == 2 && ent->client->pers.rpg_class == 5 && ent->client->pers.secrets_found & (1 << 7))
		...
		ent->client->pers.send_event_timer = level.time + 300;
	}
