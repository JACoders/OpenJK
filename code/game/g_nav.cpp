...
void NPC_SetMoveGoal(gentity_t *ent, vec3_t point, int radius,
  ...
  // FIXME: TESTING let's try making sure the tempGoal isn't stuck in the
  // ground?
  if (0) {
    trace_t trace;
    vec3_t bottom = {ent->NPC->tempGoal->currentOrigin[0],
                     ent->NPC->tempGoal->currentOrigin[1],
                     ent->NPC->tempGoal->currentOrigin[2] +
                         ent->NPC->tempGoal->mins[2]};
    gi.trace(&trace, ent->NPC->tempGoal->currentOrigin, vec3_origin,
             vec3_origin, bottom, ent->s.number, ent->clipmask,
             (EG2_Collision)0, 0);
    if (trace.fraction < 1.0f) { // in the ground, raise it up
      ent->NPC->tempGoal->currentOrigin[2] -=
          ent->NPC->tempGoal->mins[2] * (1.0f - trace.fraction) - 0.125f;
    }
  }
...
position) DROP_TO_FLOOR - will cause the point to auto drop to the floor

radius is automatically calculated in-world.
"targetJump" is a special edge that only guys who can jump will cross (so
basically Jedi)
*/
...
void SP_waypoint(gentity_t *ent) {
  ...
  if (!(ent->spawnflags & 1) &&
    ...
    if (G_CheckInSolid(ent, qtrue)) {
      ...
      //				if
      //(!g_entities[ENTITYNUM_WORLD].s.radius){	//not a region
      //					G_Error("Waypoint %s at %s in
      //solid!\n", ent->targetname, vtos(ent->currentOrigin));
      //				}
...
/*QUAKED waypoint_navgoal (0.3 1 0.3) (-20 -20 -24) (20 20 40) SOLID_OK
DROP_TO_FLOOR NO_AUTO_CONNECT A waypoint for script navgoals Not included in
navigation data

DROP_TO_FLOOR - will cause the point to auto drop to the floor
NO_AUTO_CONNECT - will not automatically connect to any other points, you must
then connect it by hand
...
void Svcmd_Nav_f(void) {
  ...
  if (Q_stricmp(cmd, "show") == 0) {
    ...
    if (Q_stricmp(cmd, "all") == 0) {
      ...
      NAVDEBUG_showCollision = NAVDEBUG_showNavGoals =
          NAVDEBUG_showCombatPoints = NAVDEBUG_showEnemyPath =
              NAVDEBUG_showEdges = NAVDEBUG_showNearest = NAVDEBUG_showRadius =
                  NAVDEBUG_showNodes;
    ...
    if (Q_stricmp(cmd, "testgoal") == 0) {
      //	NAVDEBUG_curGoal = navigator.GetNearestNode( &g_entities[0],
      //g_entities[0].waypoint, NF_CLEAR_PATH, WAYPOINT_NONE );
    }
