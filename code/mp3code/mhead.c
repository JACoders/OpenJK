...
static const int mp_br_table[2][16] = {{0, 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160, 0},
									   {0, 32, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 384, 0}};
static const int mp_sr20_table[2][4] = {{441, 480, 320, -999}, {882, 960, 640, -999}};

static const int mp_br_tableL1[2][16] = {{0, 32, 48, 56, 64, 80, 96, 112, 128, 144, 160, 176, 192, 224, 256, 0}, /* mpeg2 */
										 {0, 32, 64, 96, 128, 160, 192, 224, 256, 288, 320, 352, 384, 416, 448, 0}};

static const int mp_br_tableL3[2][16] = {{0, 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160, 0}, /* mpeg 2 */
...
int head_info3(unsigned char *buf, unsigned int n, MPEG_HEAD *h, int *br, unsigned int *searchForward) {
	...
	// jdw insertion...
	while ((pBuf < n) && !((buf[pBuf] == 0xFF) && ((buf[pBuf + 1] & 0xF0) == 0xF0 || (buf[pBuf + 1] & 0xF0) == 0xE0))) {
		pBuf++;
	}
