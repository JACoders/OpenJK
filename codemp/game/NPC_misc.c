...
Debug_NPCPrintf
*/
void Debug_NPCPrintf(gentity_t *printNPC, vmCvar_t *cv, int debugLevel,
                     char *fmt, ...) {
  int color;
  va_list argptr;
  char msg[1024];
  ...
  //	if ( debugNPCName.string[0] && Q_stricmp( debugNPCName.string,
  //printNPC->targetname) != 0 )
  //	{
  //		return;
  //	}
  ...
  Com_Printf("%c%c%5i (%s) %s", Q_COLOR_ESCAPE, color, level.time,
             printNPC->targetname, msg);
}
