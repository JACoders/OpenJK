...
static void Sniper_CheckMoveState(void) {
	...
	if ((NPCInfo->goalEntity != NPC->enemy) && (NPCInfo->goalEntity != NULL)) {
		// Did we make it?
		if (STEER::Reached(NPC, NPCInfo->goalEntity, 16, !!FlyingCreature(NPC)) || (NPCInfo->squadState == SQUAD_SCOUT && enemyLOS && enemyDist <= 10000)) {
	...
	if (TIMER_Done(NPC, "duck")) {		   // we're not ducking
		if (TIMER_Done(NPC, "roamTime")) { // not roaming
			...
			if ((NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) && (!NPCInfo->goalEntity || NPCInfo->goalEntity == NPC->enemy)) { // we were running after enemy
				...
				if (cp == -1 && !(NPCInfo->scriptFlags & SCF_USE_CP_NEAREST)) { // okay, try one by the enemy
					cp =
						NPC_FindCombatPoint(NPC->currentOrigin, NPC->currentOrigin, NPC->enemy->currentOrigin, CP_CLEAR | CP_HAS_ROUTE | CP_HORZ_DIST_COLL, 32);
				}
				// NOTE: there may be a perfectly valid one, just not one within CP_COLLECT_RADIUS of either me or him...
				if (cp != -1) { // found a combat point that has a clear shot to enemy
					NPC_SetCombatPoint(cp);
...
qboolean Sniper_EvaluateShot(int hit) {
	...
	if (hit == NPC->enemy->s.number || (hitEnt && hitEnt->client && hitEnt->client->playerTeam == NPC->client->enemyTeam) ||
		...
		(hitEnt && (hitEnt->svFlags & SVF_GLASS_BRUSH))) { // can hit enemy or will hit glass, so shoot anyway
		return qtrue;
	}
	return qfalse;
}
	...
	if (NPC->enemy) {
		...
		if (enemyDist > 65536 && NPCInfo->stats.aim < 5) // is 256 squared, was 16384 (128*128)
		{
			if (NPC->count < (5 - NPCInfo->stats.aim)) {										  // miss a few times first
				if (shoot && TIMER_Done(NPC, "attackDelay") && level.time >= NPCInfo->shotTime) { // ready to fire again
...
void NPC_BSSniper_Attack(void) {
	...
	{																							   // too close, so switch to primary fire
		if (NPC->client->ps.weapon == WP_DISRUPTOR || NPC->client->ps.weapon == WP_TUSKEN_RIFLE) { // sniping... should be assumed
			if (NPCInfo->scriptFlags & SCF_ALT_FIRE) {											   // use primary fire
				...
				gi.trace(&trace, NPC->enemy->currentOrigin, NPC->enemy->mins, NPC->enemy->maxs, NPC->currentOrigin, NPC->enemy->s.number, NPC->enemy->clipmask,
						 (EG2_Collision)0, 0);
				if (!trace.allsolid && !trace.startsolid && (trace.fraction == 1.0 || trace.entityNum == NPC->s.number)) { // he can get right to me
	...
	*/
	...
	} else if (NPC->client->ps.weapon == WP_TUSKEN_RIFLE && !Q_irand(0, 100)) { // start a taunt
		NPC_Tusken_Taunt();
		TIMER_Set(NPC, "duck", -1);
		doMove = qfalse;
	}
