  ...
  if (!cgi_Language_IsAsian()) {
    ...
    while (*p) {
      while (*p && CountsAsWhiteSpaceForCaps(*p))
        p++; //(isspace(*p) || *p == '-' || *p == '.')) p++;	// also copes with
             //hyphenated names (awkward gits)
      if (*p) {
        *p = toupper(*p);
        while (*p && !CountsAsWhiteSpaceForCaps(*p))
          p++; // cope with hyphenated names and initials (awkward gits)
      }
    }
  }
  ...
  while (*psTextParse != '\0') {
    ...
    while (1) {
      ...
      unsigned int uiLetter = cgi_AnyLanguage_ReadCharFromString(
          &psTextParse, &bIsTrailingPunctuation);
