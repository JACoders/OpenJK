...
cvar_t *sv_compress_saved_games; // compress the saved games on the way out
                                 // (only affect saver, loader can read both)
...
// If a packet has not been received from a client for timeout->integer seconds,
// drop the conneciton. Server time is used instead of realtime to avoid
// dropping the local client while debugging. When a client is normally dropped,
// the client_t goes into a zombie state for a few seconds to make sure any
// final
...
/*
This wonderful hack is needed to avoid rendering frames until several camera
related things have wended their way through the network. The problem is
basically that the server asks the client where the camera is to decide what
entities down to the client. However right after certain transitions the client
tends to give a wrong answer. CGCam_Disable is one such time/ When this happens
we want to dump all rendered frame until these things have happened, in order:
...
4) The client has recieved a packet (This state also means the game is running
normally).
...
void SV_Frame(int msec, float fractionMsec) {
  ...
  SE_CheckForLanguageUpdates(); // will fast-return else load different language
                                // if menu changed it
  ...
  if (sv.time > 0x70000000) {
    ...
    Com_Printf("You win.  if you can play this long and not die, you deserve "
               "to win.\n");
    return;
  }
  ...
  SG_TestSave(); // returns immediately if not active, used for
                 // fake-save-every-cycle to test (mainly) Icarus disk code
