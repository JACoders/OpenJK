...
void Con_Init(void) {
  ...
  con_opacity = Cvar_Get("con_opacity", "1.0", CVAR_ARCHIVE_ND,
...
void Con_DrawNotify(void) {
  ...
  for (i = con.current - NUM_CON_TIMES + 1; i <= con.current; i++) {
    ...
    if (re->Language_IsAsian()) {
      ...
      //
      re->Font_DrawString(
          cl_conXOffset->integer +
              con.xadjust *
                  (con.xadjust + (1 * SMALLCHAR_WIDTH /*aesthetics*/)),
      ...
      for (x = 0; x < con.linewidth; x++) {
        ...
        SCR_DrawSmallChar((int)(cl_conXOffset->integer + con.xadjust +
                                (x + 1) * SMALLCHAR_WIDTH),
                          v, text[x] & 0xff);
      }
...
void Con_DrawSolidConsole(float frac) {
  ...
  if (re->Language_IsAsian()) {
    ...
    iPixelHeightToAdvance =
        ...
        re->Font_HeightPixels(
            iFontIndexForAsian,
            fFontScaleForAsian); // for asian spacing, since we don't want
                                 // glyphs to touch.
  }
