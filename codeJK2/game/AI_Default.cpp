...
void NPC_LostEnemyDecideChase(void)
  ...
  We lost our enemy and want to drop him but see if we should chase him if we
are in the proper bState
*/
...
void NPC_LostEnemyDecideChase(void) {
  ...
  case BS_HUNT_AND_KILL:
    ...
    if (NPC->enemy == NPCInfo->goalEntity &&
        NPC->enemy->lastWaypoint !=
            WAYPOINT_NONE) { // Remember his last valid Wp, then check it out
      // FIXME: Should we only do this if there's no other enemies or we've got
      // LOCKED_ENEMY on?
...
void NPC_StandIdle(void) {
  /*
          ...
          //FIXME: Account for STAND1 or STAND2 here and set the base anim
     accordingly int	baseSeq = ( anim == BOTH_STAND1 ) ? BOTH_STAND1_RANDOM1
     : BOTH_STAND2_RANDOM1;

          //Must have at least one random idle animation
          //NOTENOTE: This relies on proper ordering of animations, which SHOULD
     be okay if ( PM_HasAnimation( NPC, baseSeq ) == false ) return;

          int	newIdle = Q_irand( 0, MAX_IDLE_ANIMS-1 );

          //FIXME: Technically this could never complete.. but that's not really
     too likely while( 1 )
          {
          ...
          NPC_SetAnim( NPC, SETANIM_BOTH, baseSeq + newIdle,
     SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );

          int newTime = PM_AnimLength( NPC->client->clientInfo.animFileIndex,
     (animNumber_t) (baseSeq + newIdle) );
...
qboolean NPC_StandTrackAndShoot(gentity_t *NPC, qboolean canDuck) {
  ...
  // Maybe is can be shot even when ducked, we should run away to the nearest
  // cover?
  if (canDuck) {
    if (NPC->health < 20) {
  ...
  if (canDuck && (duck_ok || (!attack_ok && client->fireDelay == 0)) &&
      ucmd.upmove !=
          -127) { // if we didn't attack check to duck if we're not already
    if (!duck_ok) {
      if (NPC->enemy->client) {
        if (NPC->enemy->enemy == NPC) {
          if (NPC->enemy->client->buttons &
              BUTTON_ATTACK) { // FIXME: determine if enemy fire angles would
                               // hit me or get close
    ...
    if (Q_flrand(0.0f, 1.0f) < 0.5) {
      if (NPC->client->enemyTeam) {
        gentity_t *newenemy = NPC_PickEnemy(
            ...
            (qboolean)(NPC->client->enemyTeam == TEAM_PLAYER), qtrue);
...
void NPC_BSHuntAndKill(void) {
  ...
  NPC_CheckEnemy((qboolean)(NPCInfo->tempBehavior != BS_HUNT_AND_KILL),
                 qfalse); // don't find new enemy if this is tempbehav

  if (NPC->enemy) {
    oEVis = enemyVisibility = NPC_CheckVisibility(
        NPC->enemy, CHECK_FOV | CHECK_SHOOT); // CHECK_360|//CHECK_PVS|
    if (enemyVisibility > VIS_PVS) {
      if (!NPC_EnemyTooFar(
              NPC->enemy, 0,
              qtrue)) { // Enemy is close enough to shoot - FIXME: this next
                        // func does this also, but need to know here for info on
                        // whether ot not to turn later
    ...
    if (curAnim != BOTH_ATTACK1 && curAnim != BOTH_ATTACK2 &&
        curAnim != BOTH_ATTACK3 && curAnim != BOTH_MELEE1 &&
        curAnim != BOTH_MELEE2) { // Don't move toward enemy if we're in a
                                  // full-body attack anim
...
void NPC_BSStandAndShoot(void) {
  ...
  if (NPC->client->playerTeam && NPC->client->enemyTeam) {
    // FIXME: don't realize this right away- or else enemies show up and we're
    // standing around
    /*
    ...
    //Also: our team count includes medics!
            ...
            {//Can we rush the enemy?
                    if(teamNumbers[NPC->enemyTeam] == 1 ||
                            teamNumbers[NPC->playerTeam] >=
    teamNumbers[NPC->enemyTeam]*3)
                    {//Only one of them or we outnumber 3 to 1
                            if(teamStrength[NPC->playerTeam] >= 75 ||
                                    (teamStrength[NPC->playerTeam] >= 50 &&
    teamStrength[NPC->playerTeam] > teamStrength[NPC->enemyTeam]))
                            {//Our team is strong enough to rush
                                    teamCounter[NPC->playerTeam]++;
                                    if(teamNumbers[NPC->playerTeam] * 17 <=
    teamCounter[NPC->playerTeam])
                                    {//ok, we waited 1.7 think cycles on average
    and everyone is go, let's do it!
                                            //FIXME: Should we do this to
    everyone on our team? NPCInfo->behaviorState = BS_HUNT_AND_KILL;
                                            //FIXME: if the tide changes, we
    should retreat!
                                            //FIXME: when do we reset the
    counter? NPC_BSHuntAndKill (); return;
                                    }
                            }
                            else//Oops!  Something's wrong, reset the counter to
    rush teamCounter[NPC->playerTeam] = 0;
                    }
  ...
  if (NPC->enemy) {
    if (!NPC_StandTrackAndShoot(NPC,
                                qtrue)) { // That func didn't update our angles
...
void NPC_BSRunAndShoot(void) {
  ...
  {
          //FIXME: don't realize this right away- or else enemies show up and
  we're standing around if( teamNumbers[NPC->enemyTeam] == 0 )
          {//ok, stand guard until we find another enemy
                  //reset our rush counter
                  teamCounter[NPC->playerTeam] = 0;
                  NPCInfo->tempBehavior = BS_STAND_GUARD;
  ...
  if (NPC->enemy) {
    ...
    if (!(ucmd.buttons & BUTTON_ATTACK) && ucmd.upmove >= 0 &&
        NPC->cantHitEnemyCounter > monitor) { // not crouching and not firing
      vec3_t vec;

      VectorSubtract(NPC->enemy->currentOrigin, NPC->currentOrigin, vec);
      vec[2] = 0;
      if (VectorLength(vec) > 128 ||
          NPC->cantHitEnemyCounter >= 10) { // run at enemy if too far away
...
void NPC_BSFace(void) {
  // FIXME: once you stop sending turning info, they reset to whatever their
  // delta_angles was last???? Once this is over, it snaps back to what it was
  // facing before- WHY???
...
void NPC_BSPointShoot(qboolean shoot) { // FIXME: doesn't check for clear
                                        // shot...
  vec3_t muzzle, dir, angles, org;

  if (!NPC->enemy || !NPC->enemy->inuse ||
      (NPC->enemy->NPC &&
       NPC->enemy->health <= 0)) { // FIXME: should still keep shooting for a
                                   // second or two after they actually die...
    Q3_TaskIDComplete(NPC, TID_BSTATE);
    goto finished;
    return;
  }
  ...
  if (NPC_UpdateAngles(
          qtrue, qtrue)) { // FIXME: if angles clamped, this may never work!
    // NPCInfo->shotTime = NPC->attackDebounceTime = 0;

    if (shoot) { // FIXME: needs to hold this down if using a weapon that
                 // requires it, like phaser...
      ucmd.buttons |= BUTTON_ATTACK;
    }

    if (!shoot ||
        !(NPC->svFlags & SVF_LOCKEDENEMY)) { // If locked_enemy is on, dont
                                             // complete until it is destroyed...
      Q3_TaskIDComplete(NPC, TID_BSTATE);
      goto finished;
    }
  } else if (shoot && (NPC->svFlags &
                       SVF_LOCKEDENEMY)) { // shooting them till their dead, not
                                           // aiming right at them yet...
    /*
    ...
    */
    {
      ...
      float pitchMiss,
          pitchMissAllow = (NPC->enemy->maxs[2] - NPC->enemy->mins[2]) / 2;
      ...
      yawMiss = tan(DEG2RAD(AngleDelta(NPC->client->ps.viewangles[YAW],
                                       NPCInfo->desiredYaw))) *
                dist;
      pitchMiss = tan(DEG2RAD(AngleDelta(NPC->client->ps.viewangles[PITCH],
...
void NPC_BSPatrol( void )
  ...
  using your angles, HFOV, VFOV and visrange, and listen for sounds within
earshot...
*/
void NPC_BSPatrol(void) {
  // int	alertEventNum;
  ...
  if (level.time > NPCInfo->enemyCheckDebounceTime) {
    NPCInfo->enemyCheckDebounceTime =
        level.time + (NPCInfo->stats.vigilance * 1000);
    NPC_CheckEnemy(qtrue, qfalse);
...
void NPC_BSDefault(void) {
  ...
  // look for a new enemy if don't have one and are allowed to look, validate
  // current enemy if have one
  NPC_CheckEnemy((qboolean)(NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES),
                 qfalse);
  if (!NPC->enemy) { // still don't have an enemy
    if (!(NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) { // check for alert events
      // FIXME: Check Alert events, see if we should investigate or just look at
      // it
      int alertEvent =
          NPC_CheckAlertEvents(qtrue, qtrue, -1, qtrue, AEL_DISCOVERED);

      // There is an event to look at
      if (alertEvent >= 0 && level.alertEvents[alertEvent].ID !=
                                 NPCInfo->lastAlertID) { // heard/saw something
        if (level.alertEvents[alertEvent].level >= AEL_DISCOVERED &&
            (NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES)) { // was a big event
          if (level.alertEvents[alertEvent].owner &&
              level.alertEvents[alertEvent].owner->client &&
              level.alertEvents[alertEvent].owner->health >= 0 &&
              level.alertEvents[alertEvent].owner->client->playerTeam ==
                  NPC->client->enemyTeam) { // an enemy
  ...
  if (NPC->enemy && !(NPCInfo->scriptFlags & SCF_FORCED_MARCH)) {
    ...
    if (NPC->client->leader && NPCInfo->goalEntity == NPC->client->leader &&
    ...
    /*
                    //have an enemy
                    //FIXME: if one of these fails, meaning we can't shoot, do
       we really need to do the rest? VectorSubtract( NPC->enemy->currentOrigin,
       NPC->currentOrigin, enemyDir ); enemyDist = VectorNormalize( enemyDir );
                    enemyDist *= enemyDist;
                    shootDist = NPC_MaxDistSquaredForWeapon();

                    enemyFOV = InFOV( NPC->enemy, NPC, NPCInfo->stats.hfov,
       NPCInfo->stats.vfov ); enemyShotFOV = InFOV( NPC->enemy, NPC, 20, 20 );
                    enemyPVS = gi.inPVS( NPC->enemy->currentOrigin,
       NPC->currentOrigin );
                    ...
                    {//in the pvs
                            ...
                            enemyHead[2] -= Q_flrand( 0.0f,
       NPC->enemy->maxs[2]*0.5f ); CalcEntitySpot( NPC, SPOT_WEAPON, muzzle );
                            enemyLOS = NPC_ClearLOS( muzzle, enemyHead );

                            gi.trace ( &tr, muzzle, vec3_origin, vec3_origin,
       enemyHead, NPC->s.number, MASK_SHOT ); enemyCS = NPC_EvaluateShot(
       tr.entityNum, qtrue );
                    }
                    else
                    ...
                    {//can shoot
                            ...
                            {//my goal is my enemy and I have a clear shot, no
       need to chase right now move = qfalse;
                            }
                    }
                    else
       ...
       SCF_FIRE_WEAPON) ) // we've already fired, no need to do it again here
                    ...
                    {//go after him
                            NPCInfo->goalEntity = NPC->enemy;
                            //FIXME: don't need to chase when have a clear shot
       and in range? if ( !enemyCS && NPC->cantHitEnemyCounter > 60 )
                            {//haven't been able to shoot enemy for about 6
       seconds, need to do something
                            ...
                            //FIXME: in normal behavior, should we use combat
       Points?  Do we care?  Is anyone actually going to ever use this AI?
                    }
                    ...
                    {//have a clear LOS to him//, but not looking at him
                            ...
                            NPCInfo->desiredYaw		= AngleNormalize180(
       angles[YAW] );
                            NPCInfo->desiredPitch	= AngleNormalize180(
       angles[PITCH] );
                    }
                    */
  }
  ...
  if (UpdateGoal()) { // have a goal
    if (!NPC->enemy && NPC->client->leader &&
      ...
      if ((NPCInfo->scriptFlags & SCF_FACE_MOVE_DIR) ||
          NPCInfo->goalEntity !=
              NPC->enemy) { // face direction of movement, NOTE: default
                            // behavior when not chasing enemy
        NPCInfo->combatMove = qfalse;
      } else { // face goal.. FIXME: what if have a navgoal but want to face
               // enemy while moving?  Will this do that?
        vec3_t dir, angles;

        NPCInfo->combatMove = qfalse;

        VectorSubtract(NPCInfo->goalEntity->currentOrigin, NPC->currentOrigin,
                       dir);
        vectoangles(dir, angles);
      ...
      if (NPCInfo->scriptFlags & SCF_FORCED_MARCH) { // being forced to walk
        if (g_crosshairEntNum !=
            NPC->s.number) { // don't walk if player isn't aiming at me
          move = qfalse;
        }
      }
