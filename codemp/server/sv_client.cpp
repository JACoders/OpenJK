  ...
  if (!cl->download) {
    ...
    if (!sv_allowDownload->integer || idPack || unreferenced ||
      ...
      if (unreferenced) {
        ...
        // filesystem scheme?
        //   if the pk3 is referenced, it must have been found somewhere in the
        //   filesystem
...
void SV_UserinfoChanged(client_t *cl) {
  ...
  if (Sys_IsLANAddress(cl->netchan.remoteAddress) &&
    ...
    if (sv_ratePolicy->integer == 1) {
      ...
      i = atoi(val);
      if (!i) {
        i = sv_maxRate->integer; // FIXME old code was 3000 here, should
                                 // increase to 5000 instead or maxRate?
      }
  ...
  int minSnaps = Com_Clampi(
      1, sv_snapsMax->integer,
      sv_snapsMin->integer); // between 1 and sv_snapsMax ( 1 <-> 40 )
  int maxSnaps = Q_min(
      sv_fps->integer,
      sv_snapsMax->integer); // can't produce more than sv_fps snapshots/sec,
