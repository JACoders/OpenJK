...
void FX_DisruptorMainShot(vec3_t start, vec3_t end) {
  ...
  trap->FX_AddLine(start, end, 0.1f, 6.0f, 0.0f, 1.0f, 0.0f, 0.0f, WHITE, WHITE,
                   0.0f, 150, trap->R_RegisterShader("gfx/effects/redLine"),
  ...
  //								5.0f, 5.0f,
  //0.0f, 								len, len, 0.0f, 								1.0f, 1.0f, 0.0f, 								WHITE, WHITE, 0.0f, 								400,
  //cgi_R_RegisterShader( "gfx/effects/spiral" ), 0 );
}
...
void FX_DisruptorAltShot(vec3_t start, vec3_t end, qboolean fullCharge) {
  ...
  if (fullCharge) {
    ...
    // add some beef
    trap->FX_AddLine(start, end, 0.1f, 7.0f, 0.0f, 1.0f, 0.0f, 0.0f, YELLER,
                     YELLER, 0.0f, 150,
...
void FX_DisruptorAltMiss(vec3_t origin, vec3_t normal) {
  ...
  0.0f, 0.2f, 0.5f, WHITE, WHITE, 0.0f, 4000, trap->R_RegisterShader(
  ...
  */
  ...
  trap->FX_PlayEffectID(cgs.effects.disruptorAltMissEffect, origin, normal, -1,
                        -1, qfalse);
}
...
void FX_DisruptorAltHit(vec3_t origin, vec3_t normal) {
  trap->FX_PlayEffectID(cgs.effects.disruptorAltHitEffect, origin, normal, -1,
                        -1, qfalse);
}
...
void FX_DisruptorHitWall(vec3_t origin, vec3_t normal) {
  trap->FX_PlayEffectID(cgs.effects.disruptorWallImpactEffect, origin, normal,
                        -1, -1, qfalse);
}
...
void FX_DisruptorHitPlayer(vec3_t origin, vec3_t normal, qboolean humanoid) {
  trap->FX_PlayEffectID(cgs.effects.disruptorFleshImpactEffect, origin, normal,
                        -1, -1, qfalse);
}
