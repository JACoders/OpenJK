...
//------------------------------------------------------
CFxScheduler::CFxScheduler() { memset(&mEffectTemplates, 0, sizeof(mEffectTemplates)); }
...
void CFxScheduler::CreateEffect(CPrimitiveTemplate *fx, int clientID, int delay) {
	...
	case Particle:
		...
		FX_AddParticle(clientID, org, vel, accel, fx->mGravity.GetVal(), fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(),
					   ...
					   fx->mRotationDelta.GetVal(), fx->mLife.GetVal(), fx->mMediaHandles.GetHandle(), flags);
		break;
	...
	case Line:
		...
		FX_AddLine(clientID, org, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(), fx->mAlphaEnd.GetVal(),
				   fx->mAlphaParm.GetVal(), sRGB, eRGB, fx->mRGBParm.GetVal(), fx->mLife.GetVal(), fx->mMediaHandles.GetHandle(),
...
void CFxScheduler::AddScheduledEffects(void) {
	...
	for (itr = mFxSchedule.begin(); itr != mFxSchedule.end(); /* do nothing */) {
		...
		if (effect->mStartTime <= theFxHelper.mTime) {
			...
			} else if (effect->mBoltNum == -1) { // ok, are we spawning a bolt on effect or a normal one?
				if (effect->mEntNum != -1) {
					// Find out where the entity currently is
					CreateEffect(effect->mpTemplate, cg_entities[effect->mEntNum].lerpOrigin, effect->mAxis, theFxHelper.mTime - effect->mStartTime);
				} else {
					...
					if (cg_entities[effect->mEntNum].gent->ghoul2.IsValid()) {
						if (effect->mModelNum >= 0 && effect->mModelNum < cg_entities[effect->mEntNum].gent->ghoul2.size()) {
							if (cg_entities[effect->mEntNum].gent->ghoul2[effect->mModelNum].mModelindex >= 0) {
								...
								doesBoltExist =
									gi.G2API_GetBoltMatrix(cg_entities[effect->mEntNum].gent->ghoul2, effect->mModelNum, effect->mBoltNum, &boltMatrix,
														   cg_entities[effect->mEntNum].lerpAngles, cg_entities[effect->mEntNum].lerpOrigin, cg.time,
														   cgs.model_draw, cg_entities[effect->mEntNum].currentState.modelScale);
...
void CFxScheduler::CreateEffect(CPrimitiveTemplate *fx, const vec3_t origin, vec3_t axis[3], int lateTime) {
	...
	case Line:
		...
		FX_AddLine(org, org2, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(), fx->mAlphaEnd.GetVal(),
				   fx->mAlphaParm.GetVal(), sRGB, eRGB, fx->mRGBParm.GetVal(), fx->mLife.GetVal(), fx->mMediaHandles.GetHandle(), fx->mFlags);
		break;
	...
	case Emitter:
		...
		FX_AddEmitter(org, vel, accel, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(), fx->mAlphaEnd.GetVal(),
					  ...
					  fx->mVariance.GetVal(), fx->mLife.GetVal(), emitterModel, fx->mFlags);
		break;
	...
	case OrientedParticle:
		...
		FX_AddOrientedParticle(org, ax[0], vel, accel, fx->mSizeStart.GetVal(), fx->mSizeEnd.GetVal(), fx->mSizeParm.GetVal(), fx->mAlphaStart.GetVal(),
							   ...
							   fx->mImpactFxHandles.GetHandle(), fx->mLife.GetVal(), fx->mMediaHandles.GetHandle(), fx->mFlags);
		break;
