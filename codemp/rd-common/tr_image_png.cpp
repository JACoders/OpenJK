...
int RE_SavePNG(const char *filename, byte *buf, size_t width, size_t height,
               int byteDepth) {
  fileHandle_t fp;
  ...
  */
  ...
  for (y = 0; y < height; ++y) {
    png_byte *row =
        (png_byte *)png_malloc(png_ptr, sizeof(uint8_t) * width * byteDepth);
...
struct PNGFileReader {
  PNGFileReader(char *buf)
      : buf(buf), offset(0), png_ptr(NULL), info_ptr(NULL) {}
  ~PNGFileReader() {
    ri.FS_FreeFile(buf);
  ...
  int Read(byte **data, int *width, int *height) {
    ...
    *height = 0;
    ...
    png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL,
                                     png_print_error, png_print_warning);
    if (png_ptr == NULL) {
      ri.Printf(PRINT_ERROR,
                "Could not allocate enough memory to load the image.");
      return 0;
    }
    ...
    // This generic "ignore all, except required chunks" requires 1.6.0 or
    // newer"
    ...
    png_get_IHDR(png_ptr, info_ptr, &width_, &height_, &depth, &colortype, NULL,
                 NULL, NULL);

    // While modern OpenGL can handle non-PoT textures, it's faster to handle
    // only PoT so that the graphics driver doesn't have to fiddle about with
    // the texture when uploading.
    if (!IsPowerOfTwo(width_) || !IsPowerOfTwo(height_)) {
      ri.Printf(PRINT_ERROR, "Width or height is not a power-of-two.\n");
      return 0;
    }

    // This function is equivalent to using what used to be LoadPNG32. LoadPNG8
    // also existed, but this only seemed to be used by the RMG system which
    // does not work in JKA. If this does need to be re-implemented, then
    // colortype should be PNG_COLOR_TYPE_PALETTE or PNG_COLOR_TYPE_GRAY.
    ...
    // We always assume there are 4 channels. RGB channels are expanded to RGBA
    // when read.
    byte *tempData =
        (byte *)ri.Z_Malloc(width_ * height_ * 4, TAG_TEMP_PNG, qfalse, 4);
    if (!tempData) {
      ri.Printf(PRINT_ERROR,
                "Could not allocate enough memory to load the image.");
      return 0;
    ...
    // Dynamic array of row pointers, with 'height' elements, initialized to
    // NULL.
    byte **row_pointers =
        (byte **)ri.Hunk_AllocateTempMemory(sizeof(byte *) * height_);
    if (!row_pointers) {
      ri.Printf(PRINT_ERROR,
                "Could not allocate enough memory to load the image.");
