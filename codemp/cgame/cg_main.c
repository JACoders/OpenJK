...
static void C_G2Mark(void) {
	...
	if (tr.entityNum < ENTITYNUM_WORLD && cg_entities[tr.entityNum].ghoul2) {
		...
		CG_AddGhoul2Mark(td->shader, td->size, tr.endpos, end, tr.entityNum, cent->lerpOrigin, cent->lerpAngles[YAW], cent->ghoul2, cent->modelScale,
						 Q_irand(2000, 4000));
...
static void C_ImpactMark(void) {
	...
	CG_ImpactMark(data->mHandle, data->mPoint, data->mAngle, data->mRotation, data->mRed, data->mGreen, data->mBlue, data->mAlphaStart, qtrue, data->mSizeStart,
				  qfalse);
}
...
static void CG_AS_Register(void) {
	...
	if (soundName && soundName[0] && Q_stricmp(soundName, "default")) { // global soundset
		trap->AS_AddPrecacheEntry(soundName);
	}
#endif
...
static const char *CG_FeederItemText(float feederID, int index, int column, qhandle_t *handle1, qhandle_t *handle2, qhandle_t *handle3) {
	gitem_t *item;
	int scoreIndex = 0;
...
static void CG_GetOrigin(int entID, vec3_t out) { VectorCopy(cg_entities[entID].currentState.pos.trBase, out); }

static void CG_GetAngles(int entID, vec3_t out) { VectorCopy(cg_entities[entID].currentState.apos.trBase, out); }
