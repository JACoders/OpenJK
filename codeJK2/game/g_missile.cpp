			...
			{										 // Jedi cannot block shots from behind!
				if ((other->owner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_1 && Q_irand(0, 3)) ||
					(other->owner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_0 &&
					 Q_irand(0, 1))) { // level 1 reflects 50% of the time, level 2 reflects 75% of the time
...
void G_RunMissile(gentity_t *ent) {
	...
	if (ent->s.pos.trType == TR_INTERPOLATE) { // rolling missile?
		...
		*/
		...
		{
			if ( ent->s.pos.trType == TR_GRAVITY && !(ent->s.eFlags&EF_BOUNCE) && !(ent->s.eFlags&EF_BOUNCE_HALF) && ent->s.weapon == WP_THERMAL )//FIXME:
		EF_ROLLING
			{
		...
		for (int i = 0; i < MAX_G2_COLLISIONS; i++) {
			...
			*/
			...
			if ((coll.mFlags & G2_FRONTFACE) /* && !(hitModel)*/ && hitEnt->health) {
				...
				//				int newSurface = gi.G2API_AddSurface(&hitEnt->ghoul2[coll.mModelIndex], coll.mSurfaceIndex, coll.mPolyIndex, coll.mBarycentricI,
				//coll.mBarycentricJ, 10); 				surfaceInfo_t	*newSuf = &hitEnt->ghoul2[coll.mModelIndex].mSlist[newSurface];
