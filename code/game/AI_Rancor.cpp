...
qboolean Rancor_CheckAhead(vec3_t end) {
  ...
  gi.trace(&trace, end, vec3_origin, vec3_origin, bottom, NPC->s.number,
           NPC->clipmask, (EG2_Collision)0, 0);
  if (trace.fraction < 1.0f) { // in the ground, raise it up
    end[2] -= NPC->mins[2] * (1.0f - trace.fraction) - 0.125f;
  }

  gi.trace(&trace, NPC->currentOrigin, NPC->mins, NPC->maxs, end, NPC->s.number,
           clipmask, (EG2_Collision)0, 0);

  if (trace.startsolid &&
      (trace.contents &
       CONTENTS_BOTCLIP)) { // started inside do not enter, so ignore them
    clipmask &= ~CONTENTS_BOTCLIP;
    gi.trace(&trace, NPC->currentOrigin, NPC->mins, NPC->maxs, end,
             NPC->s.number, clipmask, (EG2_Collision)0, 0);
  }
  // Do a simple check
  ...
  if (trace.entityNum < ENTITYNUM_WORLD &&
      G_EntIsBreakable(trace.entityNum,
                       NPC)) { // breakable brush in our way, break it
...
qboolean Rancor_CheckRoar(gentity_t *self) {
  if (!self->wait) { // haven't ever gotten mad yet
    self->wait = 1;  // do this only once
    NPC_SetAnim(self, SETANIM_BOTH, BOTH_STAND1TO2,
...
void Rancor_Move(qboolean visible) {
  if (NPCInfo->localState != LSTATE_WAITING) {
    ...
    NPCInfo->goalRadius =
        NPC->maxs[0] +
        (MIN_DISTANCE *
         NPC->s.modelScale[0]); // just get us within combat range
    // FIXME: for some reason, if NPC_MoveToGoal fails, it sets my angles to my
    // lastPathAngles, which I don't want
    float savYaw = NPCInfo->desiredYaw;
    bool savWalking = !!(ucmd.buttons & BUTTON_WALKING);
    if (!NPC_MoveToGoal(qtrue)) { // can't macro-nav, just head right for him
      // FIXME: if something in the way that's not the world, set blocked ent
      vec3_t dest;
      VectorCopy(NPCInfo->goalEntity->currentOrigin, dest);
      if (Rancor_CheckAhead(dest)) { // use our temp move straight to goal check
        if (!savWalking) {
        ...
        if ( (ucmd.buttons&BUTTON_WALKING) && NPC->client->ps.speed >
        NPCInfo->stats.walkSpeed )
        {
        ...
        */
      } else { // all else fails, look at him
        //	gi.Printf("Fail\n");
        ...
        /*	if ( !NPCInfo->blockedEntity )
           ...
           NPC->enemy->client->ps.groundEntityNum < ENTITYNUM_WORLD )
                        {//hmm, maybe he's on a breakable brush?
                                if ( G_EntIsBreakable(
           NPC->enemy->client->ps.groundEntityNum, NPC ) )
                                {//break it!
                                        gentity_t *breakable =
           &g_entities[NPC->enemy->client->ps.groundEntityNum]; int
           sanityCheck = 0;
                                        //FiXME: and if he's on a stack of 3
           breakables?
                                        //FIXME: See if the breakable has a
           targetname, if so see if the thing targeting it is a breakable, if
           so, etc... while ( sanityCheck < 20 && breakable &&
           breakable->targetname )
                                        {
                                                gentity_t *breakableNext = NULL;
                                                while ( sanityCheck < 20 &&
           (breakableNext = G_Find( breakableNext, FOFS(target),
           breakable->targetname )) != NULL )
                                                {
                                                        if ( breakableNext &&
           G_EntIsBreakable( breakableNext->s.number, NPC ) )
                                                        {
                                                                breakable =
           breakableNext; break;
                                                        }
                                                ...
                                                if ( !breakableNext )
                                                {//not targetted by another
           breakable that we can break break;
                                                }
                                                else
                                                {
                                                        sanityCheck++;
        ...
        if (!NPCInfo->blockedEntity && NPC->enemy &&
            gi.inPVS(NPC->currentOrigin,
                     NPC->enemy->currentOrigin)) { // nothing to destroy?  just
                                                   // go straight at goal dest
          qboolean horzClose = qfalse;
          if (!savWalking) {
            ucmd.buttons &= ~BUTTON_WALKING; // Unset from MoveToGoal()
          }

          if (DistanceHorizontal(NPC->enemy->currentOrigin,
                                 NPC->currentOrigin) <
              (NPC->maxs[0] +
               (MIN_DISTANCE *
                NPC->s.modelScale[0]))) { // close, just look at him
            horzClose = qtrue;
            NPC_FaceEnemy(qtrue);
          } else { // try to move  towards him
            STEER::Activate(NPC);
            STEER::Seek(NPC, dest);
            STEER::AvoidCollisions(NPC);
            STEER::DeActivate(NPC, &ucmd);
...
void Rancor_DropVictim(gentity_t *self) {
  ...
  if (self->activator) {
    ...
    if (self->activator->health <= 0) {
      if (self->activator->s.number) { // never free player
        if (self->count == 1) {        // in my hand, just drop them
          if (self->activator->client) {
            self->activator->client->ps.legsAnimTimer =
                self->activator->client->ps.torsoAnimTimer = 0;
            // FIXME: ragdoll?
          }
        } else {
    ...
    if (self->activator->s.number ==
        0) { // don't attack the player again for a bit
...
void Rancor_Swing(int boltIndex, qboolean tryGrab) {
  ...
  // if ( NPCInfo->blockedEntity && G_EntIsBreakable(
  // NPCInfo->blockedEntity->s.number, NPC ) )
  { // attacking a breakable brush
    // HMM... maybe always do this?
    // if boltOrg inside a breakable brush, damage it
    trace_t trace;
    gi.trace(&trace, NPC->pos3, vec3_origin, vec3_origin, boltOrg,
             NPC->s.number, CONTENTS_SOLID | CONTENTS_BODY, (EG2_Collision)0,
             0);
#ifndef FINAL_BUILD
...
#endif
    // remember pos3 for the trace from last hand pos to current hand pos next
    // time
    VectorCopy(boltOrg, NPC->pos3);
    // FIXME: also do a trace TO the bolt from where we are...?
    if (G_EntIsBreakable(trace.entityNum, NPC)) {
      G_Damage(&g_entities[trace.entityNum], NPC, NPC, vec3_origin, boltOrg,
               100, 0, MOD_MELEE);
    } else { // fuck, do an actual line trace, I guess...
      gi.trace(&trace, originUp, vec3_origin, vec3_origin, boltOrg,
               NPC->s.number, CONTENTS_SOLID | CONTENTS_BODY, (EG2_Collision)0,
               0);
#ifndef FINAL_BUILD
      if (g_bobaDebug->integer > 0) {
  ...
  for (i = 0; i < numEnts; i++) {
    ...
    if (radiusEnts[i] == NPC) { // Skip the rancor ent
      continue;
    }
    ...
    if ((radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_RANCOR) ||
        (radiusEnts[i]->client->ps.eFlags &
         EF_HELD_BY_WAMPA)) { // can't be one already being held
      continue;
    }

    if ((radiusEnts[i]->s.eFlags & EF_NODRAW)) { // not if invisible
      continue;
    }
    /*
    ...
    */
    ...
    if (DistanceSquared(radiusEnts[i]->currentOrigin, boltOrg) <=
        radiusSquared) {
      if (!gi.inPVS(radiusEnts[i]->currentOrigin,
                    NPC->currentOrigin)) { // don't grab anything that's in
                                           // another PVS
        continue;
      }
      /*
      ...
      */
      if (tryGrab
          //&& !skipGrab
          && NPC->count != 1 // don't have one in hand or in mouth already -
                             // FIXME: allow one in hand and any number in mouth!
          ...
          radiusEnts[i]->client->NPC_class != CLASS_VEHICLE) { // grab
        ...
        radiusEnts[i]->activator = NPC; // kind of dumb, but when we are locked
                                        // to the Rancor, we are owned by it.
        ...
        if (radiusEnts[i]->health > 0) { // do pain on enemy
          GEntity_PainFunc(radiusEnts[i], NPC, NPC,
                           radiusEnts[i]->currentOrigin, 0, MOD_CRUSH);
        ...
        VectorSubtract( radiusEnts[i]->currentOrigin, NPC->currentOrigin,
        pushDir ); pushDir[2] = Q_flrand( 100, 200 ); VectorNormalize( pushDir
        );
        */
        if ((NPC->spawnflags & SPF_RANCOR_FASTKILL) &&
            radiusEnts[i]->s.number >= MAX_CLIENTS) {
          G_Damage(radiusEnts[i], NPC, NPC, vec3_origin, boltOrg,
                   radiusEnts[i]->health + 1000,
...
void Rancor_Smash(void) {
  ...
  { // attacking a breakable brush
    ...
    // if boltOrg inside a breakable brush, damage it
    trace_t trace;
    gi.trace(&trace, boltOrg, vec3_origin, vec3_origin, NPC->pos3,
             NPC->s.number, CONTENTS_SOLID | CONTENTS_BODY, (EG2_Collision)0,
             0);
#ifndef FINAL_BUILD
    if (g_bobaDebug->integer > 0) {
    ...
    if (G_EntIsBreakable(trace.entityNum, NPC)) {
      ...
      gi.trace(&trace, NPC->currentOrigin, vec3_origin, vec3_origin, boltOrg,
               NPC->s.number, CONTENTS_SOLID | CONTENTS_BODY, (EG2_Collision)0,
               0);
#ifndef FINAL_BUILD
      if (g_bobaDebug->integer > 0) {
      ...
      if (G_EntIsBreakable(
        ...
        if (!Q_irand(0, 1)) {
          G_Damage(radiusEnts[i], NPC, NPC, vec3_origin,
                   radiusEnts[i]->currentOrigin, 100, 0, MOD_MELEE);
        }
      }
      continue;
    }

    if ((radiusEnts[i]->client->ps.eFlags &
         EF_HELD_BY_RANCOR)) { // can't be one being held
      continue;
    }
      ...
      if (distSq < halfRadSquared) { // close enough to do damage, too
        ...
        if ((NPC->spawnflags & SPF_RANCOR_FASTKILL) &&
          ...
          G_Damage(radiusEnts[i], NPC, NPC, vec3_origin, boltOrg,
                   radiusEnts[i]->health + 1000,
                   (DAMAGE_NO_KNOCKBACK | DAMAGE_NO_PROTECTION), MOD_MELEE);
        } else if ((NPC->spawnflags &
                    SPF_RANCOR_MUTANT)) // FIXME: a flag or something would be
                                        // better
        { // more damage
          G_Damage(radiusEnts[i], NPC, NPC, vec3_origin,
                   radiusEnts[i]->currentOrigin, Q_irand(40, 55),
                   DAMAGE_NO_KNOCKBACK, MOD_MELEE);
        } else {
          G_Damage(radiusEnts[i], NPC, NPC, vec3_origin,
                   radiusEnts[i]->currentOrigin, Q_irand(10, 25),
                   DAMAGE_NO_KNOCKBACK, MOD_MELEE);
        }
      }
      if (radiusEnts[i]->health > 0 && radiusEnts[i]->client &&
        ...
        if (distSq < halfRadSquared ||
            radiusEnts[i]->client->ps.groundEntityNum !=
                ENTITYNUM_NONE) { // within range of my fist or withing
          ...
          if ((NPC->spawnflags & SPF_RANCOR_MUTANT)) {
            ...
            G_Knockdown(radiusEnts[i], NPC, vec3_origin, Q_irand(200, 350),
                        qtrue);
          }
        }
      }
...
void Rancor_Bite(void) {
  ...
  for (i = 0; i < numEnts; i++) {
    ...
    if ((radiusEnts[i]->client->ps.eFlags &
         EF_HELD_BY_RANCOR)) { // can't be one already being held
      continue;
    }
    ...
    if (DistanceSquared(radiusEnts[i]->currentOrigin, boltOrg) <=
      ...
      if ((NPC->spawnflags & SPF_RANCOR_FASTKILL) &&
          radiusEnts[i]->s.number >= MAX_CLIENTS) {
        G_Damage(radiusEnts[i], NPC, NPC, vec3_origin,
                 radiusEnts[i]->currentOrigin, radiusEnts[i]->health + 1000,
                 (DAMAGE_NO_KNOCKBACK | DAMAGE_NO_PROTECTION), MOD_MELEE);
      } else if ((NPC->spawnflags & SPF_RANCOR_MUTANT)) { // more damage
        G_Damage(radiusEnts[i], NPC, NPC, vec3_origin,
                 radiusEnts[i]->currentOrigin, Q_irand(35, 50),
                 DAMAGE_NO_KNOCKBACK, MOD_MELEE);
      } else {
        G_Damage(radiusEnts[i], NPC, NPC, vec3_origin,
                 radiusEnts[i]->currentOrigin, Q_irand(15, 30),
                 DAMAGE_NO_KNOCKBACK, MOD_MELEE);
      }
  ...
  if (!TIMER_Exists(NPC, "attacking") && TIMER_Done(NPC, "attackDebounce")) {
    if (NPC->count == 2 && NPC->activator) {
      ...
      if ((!(NPC->spawnflags & SPF_RANCOR_FASTKILL) ||
          ...
          NPC->activator->health > 0 && Q_irand(0, 1)) { // quick bite
      ...
      if (!Q_irand(0, 3)) {
        NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_ATTACK5,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        TIMER_Set(NPC, "attack_dmg", 1250);
        if (NPC->enemy && NPC->enemy->s.number ==
                              0) { // don't attack the player again for a bit
          TIMER_Set(
              NPC, "attackDebounce",
      ...
      } else if ((NPC->spawnflags & SPF_RANCOR_MUTANT)) { // breath attack
        ...
        if (NPC->enemy && NPC->enemy->inuse) {
          ...
          if (VectorCompare(NPCInfo->blockedEntity->s.origin,
                            vec3_origin)) { // no origin brush, calc center
            VectorAdd(NPCInfo->blockedEntity->mins,
                      NPCInfo->blockedEntity->maxs, center);
            VectorScale(center, 0.5f, center);
          } else { // use origin brush as center
        ...
        NPC_SetAnim(NPC, SETANIM_BOTH, breathAnim,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        // start effect here
        G_PlayEffect(G_EffectIndex("mrancor/breath"), NPC->playerModel,
                     NPC->gutBolt, NPC->s.number, NPC->currentOrigin,
        ...
        NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_MELEE2,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        TIMER_Set(NPC, "attack_dmg", 1250);
        ...
        if (NPC->enemy && NPC->enemy->s.number ==
          ...
          TIMER_Set(
              ...
              NPC->client->ps.legsAnimTimer +
               ...
               /*&& (NPC->spawnflags&SPF_RANCOR_MUTANT)*/) { // mutant rancor
                                                             // can smash
      NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_MELEE1,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      TIMER_Set(NPC, "attack_dmg", 900);
      ...
      VectorCopy(NPC->currentOrigin, NPC->pos3);
               ...
               distance >= NPC->maxs[0] +
                               (MIN_DISTANCE * NPC->s.modelScale[0]) -
                               64.0f) { // try to grab
      int grabAnim = BOTH_ATTACK2;
      gentity_t *checkEnt = NULL;
      vec3_t center;
      if ((!aimAtBlockedEntity || !NPCInfo->blockedEntity) && NPC->enemy &&
          NPC->enemy->inuse) {
        checkEnt = NPC->enemy;
        VectorCopy(NPC->enemy->currentOrigin, center);
      } else if (NPCInfo->blockedEntity && NPCInfo->blockedEntity->inuse) {
        ...
        if (VectorCompare(NPCInfo->blockedEntity->s.origin,
                          vec3_origin)) { // no origin brush, calc center
          VectorAdd(NPCInfo->blockedEntity->mins, NPCInfo->blockedEntity->maxs,
                    center);
          VectorScale(center, 0.5f, center);
        } else { // use origin brush as center
          VectorCopy(NPCInfo->blockedEntity->s.origin, center);
      ...
      NPC_SetAnim(NPC, SETANIM_BOTH, grabAnim,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      ...
      if (NPC->count == 1 && NPC->activator) {
        if ((NPC->spawnflags & SPF_RANCOR_FASTKILL) &&
          ...
          G_Damage(NPC->activator, NPC, NPC, vec3_origin,
                   NPC->activator->currentOrigin, NPC->activator->health + 1000,
                   (DAMAGE_NO_KNOCKBACK | DAMAGE_NO_PROTECTION), MOD_MELEE);
        } else if ((NPC->spawnflags &
                    SPF_RANCOR_MUTANT)) // FIXME: a flag or something would be
                                        // better
        { // more damage
          G_Damage(NPC->activator, NPC, NPC, vec3_origin,
                   NPC->activator->currentOrigin, Q_irand(55, 70),
                   DAMAGE_NO_KNOCKBACK, MOD_MELEE);
        } else {
          G_Damage(NPC->activator, NPC, NPC, vec3_origin,
        ...
        if (NPC->activator->health <= 0) { // killed him
          if (g_dismemberment->integer >=
              3) { // make it look like we bit his head off
            NPC->activator->client->dismembered = false;
            G_DoDismemberment(NPC->activator, NPC->activator->currentOrigin,
                              MOD_SABER, 1000, HL_HEAD, qtrue);
          }
    ...
    case BOTH_ATTACK3:
      if (NPC->count == 1 && NPC->activator) {
        ...
        if (NPC->activator->client) {
          NPC->activator->client->dismembered = false;
          G_DoDismemberment(NPC->activator, NPC->enemy->currentOrigin,
                            MOD_SABER, 1000, HL_WAIST, qtrue);
        }
        // KILL
        G_Damage(NPC->activator, NPC, NPC, vec3_origin,
                 NPC->activator->currentOrigin, NPC->enemy->health + 1000,
...
void Rancor_Combat(void) {
  if (NPC->count) { // holding my enemy
    ...
    if (TIMER_Done2(NPC, "takingPain", qtrue)) {
      NPCInfo->localState = LSTATE_CLEAR;
    } else if ((NPC->spawnflags & SPF_RANCOR_FASTKILL) && NPC->activator &&
               NPC->activator->s.number >= MAX_CLIENTS) {
      ...
      if (NPC->useDebounceTime <= level.time + 100 &&
        ...
        TIMER_Set(NPC, "attacking",
                  NPC->client->ps.legsAnimTimer +
                      (Q_irand(500, 1000) * (3 - g_spskill->integer)));
      }
    } else {
  ...
  NPCInfo->goalRadius =
      NPC->maxs[0] +
      (MAX_DISTANCE * NPC->s.modelScale[0]); // just get us within combat range
  ...
  qboolean advance = (qboolean)(
      distance > (NPC->maxs[0] + (MIN_DISTANCE * NPC->s.modelScale[0]))
          ? qtrue
          : qfalse);
  ...
  if (advance) { // have to get closer
    if ((NPC->spawnflags & SPF_RANCOR_MUTANT) &&
        (!NPC->enemy ||
         !NPC->enemy->client)) { // don't do breath attack vs. bbrushes
    } else {
      vec3_t yawOnlyAngles = {0, NPC->currentAngles[YAW], 0};
      if (NPC->enemy->health > 0 &&
          fabs(distance - (250.0f * NPC->s.modelScale[0])) <=
              (80.0f * NPC->s.modelScale[0]) &&
          InFOV(NPC->enemy->currentOrigin, NPC->currentOrigin, yawOnlyAngles,
                30, 30)) {
        int chance = 9;
        if ((NPC->spawnflags &
             SPF_RANCOR_MUTANT)) { // higher chance of doing breath attack
          chance = 5 - g_spskill->integer;
        }
        if (!Q_irand(0, chance)) { // go for the charge
          doCharge = qtrue;
          advance = qfalse;
        }
      }
...
void NPC_Rancor_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
  ...
  if (!TIMER_Done(self, "breathAttack")) { // nothing interrupts breath attack
    return;
  }
          ...
          (!Q_irand(0, 4) &&
           DistanceSquared(other->currentOrigin, self->currentOrigin) <
               DistanceSquared(
                   ...
                   self->currentOrigin))) { // if my enemy is dead (or attacked
                                            // by player) and I'm not still
                                            // holding/eating someone, turn on
                                            // the attacker
        // FIXME: if can't nav to my enemy, take this guy if I can nav to him
        self->lastEnemy = self->enemy;
        G_SetEnemy(self, other);
        if (self->enemy !=
            self->lastEnemy) { // clear this so that we only sniff the player
                               // the first time we pick them up
          self->useDebounceTime = 0;
        }
        TIMER_Set(self, "lookForNewEnemy", Q_irand(5000, 15000));
        if (hitByRancor) { // stay mad at this Rancor for 2-5 secs before
                           // looking for other enemies
  ...
  if ((hitByRancor || (self->count == 1 && self->activator && !Q_irand(0, 4)) ||
       Q_irand(0, 200) < damage) // hit by rancor, hit while holding live
                                 // victim, or took a lot of damage
    ...
    if (!Rancor_CheckRoar(self)) {
      if (self->client->ps.legsAnim != BOTH_MELEE1 &&
          ...
          self->client->ps.legsAnim !=
              BOTH_ATTACK11) { // cant interrupt one of the big attack anims
        /*
        if ( self->count != 1
                || other == self->activator
                || (self->client->ps.legsAnim !=
        BOTH_ATTACK1&&self->client->ps.legsAnim != BOTH_ATTACK3) )
        */
...
void Rancor_CheckDropVictim(void) {
  ...
  vec3_t start = {NPC->activator->currentOrigin[0],
                  NPC->activator->currentOrigin[1], NPC->activator->absmin[2]};
  vec3_t end = {NPC->activator->currentOrigin[0],
                NPC->activator->currentOrigin[1],
                NPC->activator->absmax[2] - 1};
  trace_t trace;
  gi.trace(&trace, start, mins, maxs, end, NPC->activator->s.number,
           NPC->activator->clipmask, (EG2_Collision)0, 0);
...
qboolean Rancor_AttackBBrush(void) {
  ...
  float brushSize =
      ((NPCInfo->blockedEntity->maxs[0] - NPCInfo->blockedEntity->mins[0]) *
           0.5f +
       (NPCInfo->blockedEntity->maxs[1] - NPCInfo->blockedEntity->mins[1]) *
           0.5f) *
      0.5f;
  float dist2Brush = VectorNormalize(dir2Brush) - (NPC->maxs[0]) - brushSize;
  if (dist2Brush <
      (MIN_DISTANCE * NPC->s.modelScale[0])) { // close enough to just hit it
    trace.fraction = 0.0f;
  ...
  if (trace.fraction >= 1.0f // too far away
      || trace.entityNum !=
             NPCInfo->blockedEntity->s.number) // OR blocked by something else
  ...
  } else if (trace.entityNum ==
             NPCInfo->blockedEntity->s.number) { // close enough, smash it!
    Rancor_Attack((trace.fraction * checkDist), qfalse,
                  qtrue); // FIXME: maybe charge at it, smash through?
    TIMER_Remove(NPC, "attackDebounce"); // don't wait on these
    NPCInfo->enemyLastSeenTime = level.time;
  } else {
    // Com_Printf( S_COLOR_RED"RANCOR cannot reach intended breakable %s,
    // blocked by %s\n", NPC->blockedEntity->targetname,
    // g_entities[trace.entityNum].classname );
    if (G_EntIsBreakable(trace.entityNum, NPC)) { // oh, well, smash that, then
...
void Rancor_FireBreathAttack(void) {
  ...
  gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->playerModel, NPC->gutBolt,
                         &boltMatrix, rancAngles, NPC->currentOrigin,
  ...
  if (tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage &&
      traceEnt->client) { // breath attack only does damage to living things
    G_Damage(traceEnt, NPC, NPC, dir, tr.endpos, damage * 2,
             DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK | DAMAGE_NO_HIT_LOC |
                 DAMAGE_IGNORE_TEAM,
             MOD_LAVA, HL_NONE);
  }
  if (tr.fraction < 1.0f) { // hit something, do radius damage
...
void Rancor_CheckAnimDamage(void) {
  if (NPC->client->ps.legsAnim == BOTH_ATTACK2 ||
    ...
    if (NPC->client->ps.legsAnimTimer >= 1200 &&
        NPC->client->ps.legsAnimTimer <= 1350) {
      if (Q_irand(0, 2)) {
        Rancor_Swing(NPC->handRBolt, qfalse);
      } else {
        Rancor_Swing(NPC->handRBolt, qtrue);
      }
    } else if (NPC->client->ps.legsAnimTimer >= 1100 &&
               NPC->client->ps.legsAnimTimer <= 1550) {
      Rancor_Swing(NPC->handRBolt, qtrue);
    }
  } else if (NPC->client->ps.legsAnim == BOTH_ATTACK5) {
    if (NPC->client->ps.legsAnimTimer >= 750 &&
        NPC->client->ps.legsAnimTimer <= 1300) {
      Rancor_Swing(NPC->handLBolt, qfalse);
    } else if (NPC->client->ps.legsAnimTimer >= 1700 &&
               NPC->client->ps.legsAnimTimer <= 2300) {
      Rancor_Swing(NPC->handRBolt, qfalse);
    }
  }
}
...
void NPC_BSRancor_Default(void) {
  ...
  if (!TIMER_Done(NPC, "breathAttack")) { // doing breath attack, just do damage
    ...
    G_StopEffect(G_EffectIndex("mrancor/breath"), NPC->playerModel,
                 NPC->gutBolt, NPC->s.number);
    NPC->s.loopSound = 0;
  }
  ...
  if (TIMER_Done2(NPC, "clearGrabbed", qtrue)) {
    Rancor_DropVictim(NPC);
             ...
             NPC->count == 1 && NPC->activator) {
    Rancor_CheckDropVictim();
  }
  if (!TIMER_Done(
          NPC, "rageTime")) { // do nothing but roar first time we see an enemy
    AddSoundEvent(NPC, NPC->currentOrigin, 1024, AEL_DANGER_GREAT, qfalse,
                  qfalse);
    NPC_FaceEnemy(qtrue);
    return;
  }
  ...
  if (NPC->enemy) {
    ...
    if (TIMER_Done(NPC, "angrynoise")) {
      G_SoundOnEnt(NPC, CHAN_AUTO,
                   va("sound/chars/rancor/anger%d.wav", Q_irand(1, 3)));

      TIMER_Set(NPC, "angrynoise", Q_irand(5000, 10000));
    } else {
      AddSoundEvent(NPC, NPC->currentOrigin, 512, AEL_DANGER_GREAT, qfalse,
                    qfalse);
    }
    if (NPC->count == 2 &&
        NPC->client->ps.legsAnim ==
            BOTH_ATTACK3) { // we're still chewing our enemy up
      NPC_UpdateAngles(qtrue, qtrue);
      return;
    }
    ...
    if (NPC->enemy->client &&
        NPC->enemy->client->NPC_class ==
            CLASS_RANCOR) { // got mad at another Rancor, look for a valid enemy
      if (TIMER_Done(NPC, "rancorInfight")) {
        NPC_CheckEnemyExt(qtrue);
      }
    } else if (!NPC->count) {
      if (NPCInfo->blockedEntity) {           // something in our way
        if (!NPCInfo->blockedEntity->inuse) { // was destroyed
          NPCInfo->blockedEntity = NULL;
        } else {
          // a breakable?
          if (G_EntIsBreakable(NPCInfo->blockedEntity->s.number,
                               NPC)) {    // breakable brush
            if (!Rancor_AttackBBrush()) { // didn't move inside that func, so
                                          // call move here...?
              Rancor_Move(qtrue);
            }
            NPC_UpdateAngles(qtrue, qtrue);
            return;
          } else { // if it's a client and in our way, get mad at it!
            if (NPCInfo->blockedEntity != NPC->enemy &&
                NPCInfo->blockedEntity->client &&
                NPC_ValidEnemy(NPCInfo->blockedEntity) && !Q_irand(0, 9)) {
      ...
      if (NPC_ValidEnemy(NPC->enemy) == qfalse) {
        ...
        if (!NPC->enemy->inuse ||
            ...
            (NPC->spawnflags &
             SPF_RANCOR_FASTKILL)) // don't linger on dead bodies
        { // it's been a while since the enemy died, or enemy is completely
          // gone, get bored with him
          if ((NPC->spawnflags & SPF_RANCOR_MUTANT) && player &&
              player->health >=
                  0) { // all else failing, always go after the player
    ...
    Rancor_Combat();
    if (TIMER_Done(NPC, "attacking") && TIMER_Done(NPC, "takingpain") &&
        TIMER_Done(NPC, "confusionDebounce") &&
        NPCInfo->localState == LSTATE_CLEAR && !NPC->count) { // not busy
      if (!ucmd.forwardmove && !ucmd.rightmove &&
          VectorCompare(NPC->client->ps.moveDir, vec3_origin)) { // not moving
        if (level.time - NPCInfo->enemyLastSeenTime >
            5000) { // haven't seen an enemy in a while
          if (!Q_irand(0, 20)) {
            if (Q_irand(0, 1)) {
              NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_GUARD_IDLE1,
                          SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
            } else {
            ...
            TIMER_Set(NPC, "confusionDebounce",
    ...
    if (TIMER_Done(NPC, "idlenoise")) {
      G_SoundOnEnt(NPC, CHAN_AUTO,
                   va("sound/chars/rancor/snort_%d.wav", Q_irand(1, 4)));
    ...
    if (NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES) {
      Rancor_Patrol();
      if (!NPC->enemy &&
          NPC->wait) { // we've been mad before and can't find an enemy
        if ((NPC->spawnflags & SPF_RANCOR_MUTANT) && player &&
            player->health >=
                0) { // all else failing, always go after the player
          NPC->lastEnemy = NPC->enemy;
