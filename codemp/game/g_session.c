...
void G_WriteClientSessionData(gclient_t *client) {
  char s[MAX_CVAR_VALUE_STRING] = {0}, siegeClass[64] = {0},
...
void G_ReadSessionData(gclient_t *client) {
  ...
  sscanf(
      ...
      &client->sess.setForce, &client->sess.saberLevel,
      &client->sess.selectedFP, &client->sess.duelTeam,
      &client->sess.siegeDesiredTeam, client->sess.siegeClass, client->sess.IP);
  ...
  // convert back to spaces from unused chars, as session data is written that
  // way.
...
void G_InitSessionData(gclient_t *client, char *userinfo, qboolean isBot) {
  ...
  if (level.gametype >= GT_TEAM) {
    if (g_teamAutoJoin.integer &&
      ...
      client->ps.fd.forceDoInit = 1; // every time we change teams make sure our
                                     // force powers are set right
    } else {
      ...
      if (!isBot) {
        ...
        client->ps.fd.forceDoInit = 1; // every time we change teams make sure
                                       // our force powers are set right
      }
    }
  } else {
    value = Info_ValueForKey(userinfo, "team");
