...
static const char *keywordNames[NUM_AS_KEYWORDS] = {
    "timeBetweenWaves", "subWaves", "loopedWave", "volRange", "radius", "type",
...
static void AS_SkipLine(void) {
  if (parsePos > parseSize) // needed to avoid a crash because of some OOR
                            // access that shouldn't be done
    return;
...
#ifndef FINAL_BUILD
    Com_Printf(S_COLOR_YELLOW "WARNING: Corrected swapped start / end times in "
                              "a \"timeBetweenWaves\" keyword\n");
#endif
...
static void AS_GetSubWaves(ambientSet_t &set) {
  ...
  while (parsePos <= parseSize) {
    ...
    if (set.numSubWaves >= MAX_WAVES_PER_GROUP) {
      ...
      Com_sprintf(waveName, sizeof(waveName), "sound/%s/%s.wav", dirBuffer,
                  waveBuffer);

      // Place this onto the sound directory name

      // Precache the file at this point and store off the ID instead of the
      // name
...
static void AS_GetVolumeRange(ambientSet_t &set) {
  ...
  if (min > max) {
#ifndef FINAL_BUILD
    Com_Printf(S_COLOR_YELLOW "WARNING: Corrected swapped min / max range in a "
                              "\"volRange\" keyword\n");
#endif
...
#ifndef FINAL_BUILD
        Com_Printf(S_COLOR_YELLOW
                   "WARNING: Unknown ambient set keyword \"%s\"\n",
                   tempBuffer);
#endif
      }
...
void AS_ParseSets(void) {
  ...
  if (AS_ParseFile(AMBIENT_SET_FILENAME, aSets) == qfalse) {
    Com_Error(ERR_FATAL,
              S_COLOR_RED "ERROR: Couldn't load ambient sound sets from %s",
  ...
  for (namePrecache_m::iterator it = pMap->begin(); it != pMap->end(); ++it) {
    ...
    if (!aSet) {
      // I print these red instead of yellow because they're going to cause an
      // ERR_DROP if they occur
      Com_Printf(
          S_COLOR_RED
          "ERROR: AS_ParseSets: Unable to find ambient soundset \"%s\"!\n",
          str);
      iErrorsOccured++;
    }
  }
...
static void AS_UpdateCurrentSet(int id) {
  ...
  if (id != currentSet) {
    ...
    old = aSets->GetSet(oldSet);
    current = aSets->GetSet(currentSet);
    // Ste, I just put this null check in for now, not sure if there's a more
    // graceful way to exit this function - dmv
    if (!current) {
      return;
    }
    if (old) {
...
static void AS_PlayLocalSet(vec3_t listener_origin, vec3_t origin,
                            ambientSet_t *set, int entID, int *lastTime) {
  ...
  // Determine the volume based on distance (NOTE: This sits on top of what
  // SpatializeOrigin does)
  distScale = (dist < (set->radius * 0.5f))
                  ? 1
                  : (set->radius - dist) / (set->radius * 0.5f);
  volume = (distScale > 1.0f || distScale < 0.0f)
               ? 0
               : (unsigned char)(set->masterVolume * distScale);
  ...
  *lastTime = time;
  ...
  volume = (unsigned char)Q_irand((int)(volScale * set->volRange_start),
                                  (int)(volScale * set->volRange_end));

  // Add the random subwave
  if (set->numSubWaves)
    S_StartAmbientSound(origin, entID, volume,
                        set->subWaves[Q_irand(0, set->numSubWaves - 1)]);
}
...
static void AS_PlayAmbientSet(vec3_t origin, ambientSet_t *set, int *lastTime) {
  ...
  // Add the looping sound
  if (set->loopedWave)
    S_AddAmbientLoopingSound(origin, (unsigned char)set->masterVolume,
                             set->loopedWave);
  ...
  *lastTime = time;
  ...
  volume = Q_irand((int)(volScale * set->volRange_start),
                   (int)(volScale * set->volRange_end));
  ...
  // Add the random subwave
  if (set->numSubWaves)
    S_StartAmbientSound(origin, 0, volume,
                        set->subWaves[Q_irand(0, set->numSubWaves - 1)]);
}
