...
void NPC_BSAdvanceFight(void) { // FIXME: IMPLEMENT
  ...
  if (NPCS.NPCInfo->captureGoal) { // FIXME: if no captureGoal, what do we do?
    // VectorCopy( NPCInfo->captureGoal->r.currentOrigin,
    // NPCInfo->tempGoal->r.currentOrigin ); NPCInfo->goalEntity =
    // NPCInfo->tempGoal;

    NPC_SetMoveGoal(NPCS.NPC, NPCS.NPCInfo->captureGoal->r.currentOrigin, 16,
                    qtrue, -1, NULL);
  ...
  if (NPCS.NPC->enemy) { // See if we can shoot him
    ...
    // Yaw to enemy
    VectorMA(NPCS.NPC->enemy->r.absmin, 0.5, NPCS.NPC->enemy->r.maxs,
             enemy_org);
    ...
    if (attack_ok) {
      ...
      NPCS.enemyVisibility = NPC_CheckVisibility(
          NPCS.NPC->enemy, CHECK_FOV); // CHECK_360|//CHECK_PVS|
      ...
      if (NPCS.enemyVisibility == VIS_FOV) { // He's in our FOV
        ...
        if (attack_ok) {
          ...
          trap->Trace(&tr, muzzle, NULL, NULL, enemy_org, NPCS.NPC->s.number,
                      MASK_SHOT, qfalse, 0, 0);
          traceEnt = &g_entities[tr.entityNum];
          if (traceEnt != NPCS.NPC->enemy &&
              (!traceEnt || !traceEnt->client || !NPCS.NPC->client->enemyTeam ||
               NPCS.NPC->client->enemyTeam !=
                   traceEnt->client->playerTeam)) { // no, so shoot for the head
            attack_scale *= 0.75;
            trap->Trace(&tr, muzzle, NULL, NULL, enemy_head, NPCS.NPC->s.number,
                        MASK_SHOT, qfalse, 0, 0);
            traceEnt = &g_entities[tr.entityNum];
          }
  ...
  if (!NPCS.ucmd.forwardmove &&
      !NPCS.ucmd.rightmove) { // We reached our captureGoal
...
void NPC_BSCinematic(void) {
  ...
  if (NPCS.NPCInfo->watchTarget) { // have an entity which we want to keep
                                   // facing
...
void NPC_BSInvestigate(void) {
  ...
  *saveGoal;
  ...
  SETANIM_FLAG_NORMAL );
          ...
          {
                  VectorCopy(NPCInfo->eventOwner->r.currentOrigin,
  NPCInfo->investigateGoal);
          ...
          if(	level.time > NPCInfo->walkDebounceTime )
          {
                  ...
                  VectorSubtract(NPCInfo->investigateGoal, NPC->r.currentOrigin,
...
qboolean NPC_CheckInvestigate(int alertEventNum) {
  ...
  if (owner->s.eType != ET_PLAYER && owner->s.eType != ET_NPC &&
      owner == NPCS.NPCInfo->goalEntity) {
    return qfalse;
  }
  ...
  // if(!trap->InPVSIgnorePortals(ent->r.currentOrigin,
  // NPC->r.currentOrigin))//should we be able to hear through areaportals?
  if (!trap->InPVS(soundPos,
                   NPCS.NPC->r.currentOrigin)) { // can hear through doors?
    return qfalse;
  }

  if (owner->client && owner->client->playerTeam &&
      NPCS.NPC->client->playerTeam &&
      owner->client->playerTeam != NPCS.NPC->client->playerTeam) {
    if ((float)NPCS.NPCInfo->investigateCount >=
            (NPCS.NPCInfo->stats.vigilance * 200) &&
    ...
    */
    ...
    if (NPCS.NPCInfo->investigateCount > 20) {
      ...
      NPCS.NPCInfo->investigateDebounceTime =
          level.time + (NPCS.NPCInfo->investigateCount * 500);
    }
...
void NPC_BSFollowLeader(void) {
  ...
  if (!NPCS.NPC->client->leader) { // ok, stand guard until we find an enemy
  ...
  if (!NPCS.NPC->enemy) { // no enemy, find one
    NPC_CheckEnemy(NPCS.NPCInfo->confusionTime < level.time, qfalse,
                   qtrue); // don't find new enemy if this is tempbehav
    if (NPCS.NPC->enemy) { // just found one
      ...
      if (!(NPCS.NPCInfo->scriptFlags & SCF_IGNORE_ALERTS)) {
        ...
        if (level.alertEvents[eventID].level >= AEL_SUSPICIOUS &&
    ...
    if (!NPCS.NPC->enemy) {
      if (NPCS.NPC->client->leader && NPCS.NPC->client->leader->enemy &&
          NPCS.NPC->client->leader->enemy != NPCS.NPC &&
          ((NPCS.NPC->client->leader->enemy->client &&
            NPCS.NPC->client->leader->enemy->client->playerTeam ==
                NPCS.NPC->client->enemyTeam) ||
           (/*NPC->client->leader->enemy->r.svFlags&SVF_NONNPC_ENEMY*/ 0 &&
            NPCS.NPC->client->leader->enemy->alliedTeam ==
                NPCS.NPC->client->enemyTeam)) &&
          NPCS.NPC->client->leader->enemy->health >
              0) { // rwwFIXMEFIXME: use SVF_NONNPC_ENEMY?
    ...
    if (NPCS.NPC->enemy->health <= 0 ||
        (NPCS.NPC->enemy->flags & FL_NOTARGET)) {
    ...
    } else if (NPCS.NPC->client->ps.weapon &&
               NPCS.NPCInfo->enemyCheckDebounceTime < level.time) {
      NPC_CheckEnemy((NPCS.NPCInfo->confusionTime < level.time ||
                      NPCS.NPCInfo->tempBehavior != BS_FOLLOW_LEADER),
                     qfalse,
                     qtrue); // don't find new enemy if this is tempbehav
    }
  }
  ...
  if (NPCS.NPC->enemy &&
      NPCS.NPC->client->ps.weapon) { // If have an enemy, face him and fire
    ...
    NPCS.enemyVisibility = NPC_CheckVisibility(
        NPCS.NPC->enemy, CHECK_FOV | CHECK_SHOOT); // CHECK_360|CHECK_PVS|
    if (NPCS.enemyVisibility > VIS_PVS) {          // face
      ...
      if (NPCS.enemyVisibility >= VIS_SHOOT) { // shoot
        NPC_AimAdjust(2);
        if (NPC_GetHFOVPercentage(NPCS.NPC->enemy->r.currentOrigin,
                                  NPCS.NPC->r.currentOrigin,
                                  NPCS.NPC->client->ps.viewangles,
                                  NPCS.NPCInfo->stats.hfov) > 0.6f &&
            NPC_GetHFOVPercentage(
                NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin,
                NPCS.NPC->client->ps.viewangles, NPCS.NPCInfo->stats.vfov) >
  ...
  // leader visible?
  leaderVis = NPC_CheckVisibility(
      NPCS.NPC->client->leader,
      CHECK_PVS | CHECK_360 |
          CHECK_SHOOT); //			ent->e_UseFunc = useF_NULL;

  // Follow leader, stay within visibility and a certain distance, maintain a
  // distance from.
  curAnim = NPCS.NPC->client->ps.legsAnim;
  if (curAnim != BOTH_ATTACK1 && curAnim != BOTH_ATTACK2 &&
      curAnim != BOTH_ATTACK3 && curAnim != BOTH_MELEE1 &&
      curAnim != BOTH_MELEE2) { // Don't move toward leader if we're in a
    ...
    VectorSubtract(NPCS.NPC->client->leader->r.currentOrigin,
                   NPCS.NPC->r.currentOrigin, vec);
    ...
    if (leaderHDist > backupdist &&
        (leaderVis != VIS_SHOOT ||
         leaderDist > walkdist)) { // We should close in?
      ...
      VectorScale(NPCS.NPC->client->ps.moveDir, -1,
                  NPCS.NPC->client->ps.moveDir);
    } // otherwise, stay where we are
    // check for do not enter and stop if there's one there...
    if (NPCS.ucmd.forwardmove || NPCS.ucmd.rightmove ||
        VectorCompare(vec3_origin, NPCS.NPC->client->ps.moveDir)) {
...
void NPC_BSJump(void) {
  ...
  if (!NPCS.NPCInfo->goalEntity) { // Should have task completed the navgoal
    return;
  }

  if (NPCS.NPCInfo->jumpState != JS_JUMPING &&
      NPCS.NPCInfo->jumpState != JS_LANDING) {
    // Face navgoal
    VectorSubtract(NPCS.NPCInfo->goalEntity->r.currentOrigin,
                   NPCS.NPC->r.currentOrigin, dir);
    vectoangles(dir, angles);
    NPCS.NPCInfo->desiredPitch = NPCS.NPCInfo->lockedDesiredPitch =
        AngleNormalize360(angles[PITCH]);
    NPCS.NPCInfo->desiredYaw = NPCS.NPCInfo->lockedDesiredYaw =
        AngleNormalize360(angles[YAW]);
  }

  NPC_UpdateAngles(qtrue, qtrue);
  yawError = AngleDelta(NPCS.NPC->client->ps.viewangles[YAW],
  ...
  case JS_FACING:
    if (yawError < MIN_ANGLE_ERROR) { // Facing it, Start crouching
      NPC_SetAnim(NPCS.NPC, SETANIM_LEGS, BOTH_CROUCH1,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      NPCS.NPCInfo->jumpState = JS_CROUCHING;
    }
    break;
  case JS_CROUCHING:
    if (NPCS.NPC->client->ps.legsTimer > 0) { // Still playing crouching anim
      return;
    }
    ...
    if (NPCS.NPC->r.currentOrigin[2] >
        NPCS.NPCInfo->goalEntity->r.currentOrigin[2]) {
  ...
  case JS_JUMPING:
    ...
    if (NPCS.NPC->s.groundEntityNum !=
        ENTITYNUM_NONE) { // Landed, start landing anim
      // FIXME: if the
      VectorClear(NPCS.NPC->client->ps.velocity);
      NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_LAND1,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      NPCS.NPCInfo->jumpState = JS_LANDING;
      // FIXME: landsound?
    } else if (NPCS.NPC->client->ps.legsTimer >
               0) { // Still playing jumping anim
    ...
    break;
  case JS_LANDING:
    if (NPCS.NPC->client->ps.legsTimer > 0) { // Still playing landing anim
      return;
    } else {
...
void NPC_BSRemove(void) {
  ...
  if (!trap->InPVS(NPCS.NPC->r.currentOrigin,
                   g_entities[0].r.currentOrigin)) // FIXME: use cg.vieworg?
...
void NPC_BSSearch(void) {
  ...
  if (!NPCS.NPCInfo->investigateDebounceTime) { // On our way to a tempGoal
    ...
    VectorSubtract(NPCS.NPCInfo->tempGoal->r.currentOrigin,
                   NPCS.NPC->r.currentOrigin, vec);
    if (vec[2] < 24) {
      vec[2] = 0;
    }
    ...
    if (VectorLengthSquared(vec) < minGoalReachedDistSquared) {
      ...
      NPCS.NPC->waypoint =
          NAV_FindClosestWaypointForEnt(NPCS.NPC, WAYPOINT_NONE);
      ...
      if (!Q_irand(0, 1)) {
        ...
        NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_GUARD_IDLE1,
                    SETANIM_FLAG_NORMAL);
      }
    ...
    if (NPCS.NPCInfo->investigateDebounceTime > level.time) {
      ...
      if (NPCS.NPCInfo->tempGoal->waypoint != WAYPOINT_NONE) {
        if (!Q_irand(0, 30)) {
          int numEdges =
              trap->Nav_GetNodeNumEdges(NPCS.NPCInfo->tempGoal->waypoint);
          ...
          if (numEdges != WAYPOINT_NONE) {
            ...
            int nextWp = trap->Nav_GetNodeEdge(NPCS.NPCInfo->tempGoal->waypoint,
                                               branchNum);
            trap->Nav_GetNodePosition(nextWp, branchPos);

            VectorSubtract(branchPos, NPCS.NPCInfo->tempGoal->r.currentOrigin,
                           lookDir);
            NPCS.NPCInfo->desiredYaw =
                AngleNormalize360(vectoyaw(lookDir) + flrand(-45, 45));
          }

          // pick an angle +-45 degrees off of the dir of a random branch
          // from NPCInfo->tempGoal->waypoint
          // int branch = Q_irand( 0,
          // (waypoints[NPCInfo->tempGoal->waypoint].numNeighbors - 1) ); int
          // nextWp =
          // waypoints[NPCInfo->tempGoal->waypoint].nextWaypoint[branch][NPC->client->moveType];
          // vec3_t	lookDir;

          // VectorSubtract( waypoints[nextWp].origin,
          // NPCInfo->tempGoal->r.currentOrigin, lookDir ); Look in that
          // direction +- 45 degrees NPCInfo->desiredYaw = AngleNormalize360(
          // vectoyaw( lookDir ) + Q_flrand( -45, 45 ) );
        }
      }
      // Com_Printf(".");
      ...
      if (NPCS.NPC->waypoint == NPCS.NPCInfo->homeWp) {
        ...
        if (numEdges != WAYPOINT_NONE) {
          ...
          trap->Nav_GetNodePosition(nextWp,
        ...
        */
        ...
        trap->Nav_GetNodePosition(NPCS.NPCInfo->homeWp,
                                  NPCS.NPCInfo->tempGoal->r.currentOrigin);
        NPCS.NPCInfo->tempGoal->waypoint = NPCS.NPCInfo->homeWp;
        /*
        VectorCopy( waypoints[NPCInfo->homeWp].origin,
        NPCInfo->tempGoal->r.currentOrigin ); NPCInfo->tempGoal->waypoint =
        NPCInfo->homeWp;
        //Com_Printf("\nHeading for wp %d...\n", NPCInfo->homeWp);
        */
      }
...
void NPC_BSWander(void) { // FIXME: don't actually go all the way to the next
  ...
  if (!NPCS.NPCInfo->investigateDebounceTime) { // Starting out
...
void NPC_Surrender(
  ...
  if (NPCS.NPC->s.weapon != WP_NONE && NPCS.NPC->s.weapon != WP_STUN_BATON &&
      NPCS.NPC->s.weapon != WP_SABER) {
    // WP_DropWeapon( NPC, NULL ); //rwwFIXMEFIXME: Do this (gonna need a system
    // for notifying client of removal)
  }
  ...
  //SETANIM_FLAG_HOLD|SETANIM_FLAG_OVERRIDE ); 	NPC->client->ps.torsoTimer =
  //1000;
  NPCS.NPCInfo->surrenderTime =
      level.time + 1000; // stay surrendered for at least 1 second
...
qboolean NPC_CheckSurrender(void) {
  if (!trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC, TID_MOVE_NAV) &&
      ...
      !PM_InKnockDown(&NPCS.NPC->client->ps) && NPCS.NPC->enemy &&
      NPCS.NPC->enemy->client && NPCS.NPC->enemy->enemy == NPCS.NPC &&
      NPCS.NPC->enemy->s.weapon != WP_NONE &&
      NPCS.NPC->enemy->s.weapon != WP_STUN_BATON &&
      NPCS.NPC->enemy->health > 20 &&
      NPCS.NPC->enemy->painDebounceTime < level.time - 3000 &&
      NPCS.NPC->enemy->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] <
          level.time -
    ...
    if (NPCS.NPC->s.weapon != WP_ROCKET_LAUNCHER &&
      ...
      if (NPCS.NPC->s.weapon != WP_NONE) { // they have a weapon so they'd have
                                           // to drop it to surrender
        // don't give up unless low on health
        if (NPCS.NPC->health >
            25 /*|| NPC->health >= NPC->max_health*/) { // rwwFIXMEFIXME: Keep
                                                        // max health not a ps
                                                        // state?
          return qfalse;
        }
        // if ( g_crosshairEntNum == NPC->s.number && NPC->painDebounceTime >
        // level.time )
        if (NPC_SomeoneLookingAtMe(NPCS.NPC) &&
            NPCS.NPC->painDebounceTime >
                level.time) { // if he just shot me, always give up
                              // fall through
        } else { // don't give up unless facing enemy and he's very close
          if (!InFOV(NPCS.NPC->enemy, NPCS.NPC, 60,
                     30)) { // I'm not looking at them
            return qfalse;
          } else if (DistanceSquared(NPCS.NPC->r.currentOrigin,
            ...
            return qfalse;
                         ...
                         NPCS.NPC->r.currentOrigin,
                         NPCS.NPC->enemy->r
                             .currentOrigin)) { // they're not in the same room
            return qfalse;
          }
        }
      }
      ...
      {//I'm alone but I was in a group//FIXME: surrender anyway if just melee
      or no weap? if ( NPC->s.weapon == WP_NONE
                      //NPC has a weapon
                      || (NPC->enemy && NPC->enemy->s.number < MAX_CLIENTS)
                      || (NPC->enemy->s.weapon ==
      WP_SABER&&NPC->enemy->client&&!NPC->enemy->client->ps.saberHolstered)
                      || (NPC->enemy->NPC && NPC->enemy->NPC->group &&
      NPC->enemy->NPC->group->numGroup > 2) )
              {//surrender only if have no weapon or fighting a player or jedi
      or if we are outnumbered at least 3 to 1 if ( (NPC->enemy &&
                      ...
                      {//player is the guy I'm running from
                              ...
                              {//player is using saber
                                      ...
                                      {//they're looking at me
                                              if ( DistanceSquared(
      NPC->r.currentOrigin, NPC->enemy->r.currentOrigin ) < 16384 )
                                              {//they're close
                                                      if ( trap->InPVS(
      NPC->r.currentOrigin, NPC->enemy->r.currentOrigin ) )
                                                      {//they're in the same
      room NPC_Surrender(); NPC_UpdateAngles( qtrue, qtrue ); return qtrue;
                                                      }
                                              }
                                      }
                              }
                      ...
                      {//???
                              ...
                              {//they're looking at me
                                      if ( DistanceSquared(
      NPC->r.currentOrigin, NPC->enemy->r.currentOrigin ) < 4096 )
                                      {//they're close
                                              if ( trap->InPVS(
      NPC->r.currentOrigin, NPC->enemy->r.currentOrigin ) )
                                              {//they're in the same room
                                                      //FIXME: should
      player-team NPCs not fire on surrendered NPCs? NPC_Surrender();
                                                      NPC_UpdateAngles( qtrue,
      qtrue ); return qtrue;
                                              }
                                      }
                              }
  ...
  if (goal) {
    ...
    if (NPCS.NPC->waypoint != WAYPOINT_NONE) {
      ...
      if (numEdges != WAYPOINT_NONE) {
        ...
        VectorSubtract(NPCS.NPCInfo->investigateGoal, NPCS.NPC->r.currentOrigin,
                       dangerDir);
        ...
        for (branchNum = 0; branchNum < numEdges; branchNum++) {
          ...
          if (DotProduct(runDir, dangerDir) >
              flrand(0, 0.5)) { // don't run toward danger
            continue;
          }
    ...
    moved = NPC_MoveToGoal(
        qfalse); // qtrue? (do try to move straight to (away from) goal)

    if (NPCS.NPC->s.weapon == WP_NONE &&
        (moved == qfalse || reverseCourse)) { // No weapon and no escape
                                              // route... Just cower?  Need anim.
      NPC_Surrender();
      NPC_UpdateAngles(qtrue, qtrue);
      return;
    }
...
void NPC_StartFlee(gentity_t *enemy, vec3_t dangerPoint, int dangerLevel,
  ...
  if (trap->ICARUS_TaskIDPending(
          ...
          TID_MOVE_NAV)) { // running somewhere that a script requires us to go,
  ...
  // for it?
  if (dangerLevel > AEL_DANGER || NPCS.NPC->s.weapon == WP_NONE ||
      ((!NPCS.NPCInfo->group || NPCS.NPCInfo->group->numGroup <= 1) &&
       NPCS.NPC->health <=
    ...
    cp = NPC_FindCombatPoint(
        NPCS.NPC->r.currentOrigin, NPCS.NPC->r.currentOrigin, dangerPoint,
  ...
  if (cp == -1) { // okay give up on the no PVS thing
    cp = NPC_FindCombatPoint(NPCS.NPC->r.currentOrigin,
                             NPCS.NPC->r.currentOrigin, dangerPoint,
  ...
  if (cp != -1) { // found a combat point
    ...
    if (NPCS.NPC->s.weapon != WP_NONE) {
      ...
      // Run straight away from here... FIXME: really want to find farthest
      // waypoint/navgoal from this pos... maybe based on alert event radius?
      NPC_SetMoveGoal(NPCS.NPC, dangerPoint, 0, qtrue, -1, NULL);
      // store the danger point
      VectorCopy(
          dangerPoint,
          NPCS.NPCInfo->investigateGoal); // FIXME: make a new field for this?
    }
  }
...
void NPC_BSEmplaced(void) {
  ...
  /*
          else if ( trap->InPVS( NPC->enemy->r.currentOrigin,
     NPC->r.currentOrigin ) )
          {
