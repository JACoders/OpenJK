...
void EWebPositionUser(gentity_t *owner, gentity_t *eweb) {
  ...
  if (owner->s.number < MAX_CLIENTS) { // extra checks
    gi.trace(&tr, owner->currentOrigin, owner->mins, owner->maxs,
             owner->currentOrigin, owner->s.number, owner->clipmask,
             (EG2_Collision)0, 0);
    if (tr.startsolid || tr.allsolid) { // crap, they're already in solid
                                        // somehow, don't bother tracing over
      traceOver = qfalse;
    }
  }
  if (traceOver) { // trace up
    VectorCopy(owner->currentOrigin, p2);
    p2[2] += STEPSIZE;
    gi.trace(&tr, owner->currentOrigin, owner->mins, owner->maxs, p2,
             owner->s.number, owner->clipmask, (EG2_Collision)0, 0);
  ...
  if (!traceOver) {
    ...
    if (owner->s.number < MAX_CLIENTS) { // extra checks
      ...
      gi.trace(&tr, p, owner->mins, owner->maxs, p, owner->s.number,
               owner->clipmask, (EG2_Collision)0, 0);
      if (tr.startsolid || tr.allsolid) { // would be in solid there, so just
                                          // trace over, I guess?
        gi.trace(&tr, p2, owner->mins, owner->maxs, p, owner->s.number,
                 owner->clipmask, (EG2_Collision)0, 0);
      }
    } else { // trace over
      gi.trace(&tr, p2, owner->mins, owner->maxs, p, owner->s.number,
  ...
  if (!tr.startsolid && !tr.allsolid) {
    ...
    VectorCopy(p, p2);
    p2[2] -= STEPSIZE;
    gi.trace(&tr, p, owner->mins, owner->maxs, p2, owner->s.number,
             owner->clipmask, (EG2_Collision)0, 0);
    ...
    {                                   // all clear, we can move there
      ...
      if (moveDist > 4.0f) { // moved past the threshold from last position
        vec3_t oRight;
        int strafeAnim;
        ...
        if (DotProduct(moveDir, oRight) >
            0) { // moved to his right, play right strafe
          strafeAnim = BOTH_STRAFE_RIGHT1;
        } else { // moved left, play left strafe
          strafeAnim = BOTH_STRAFE_LEFT1;
        }
        NPC_SetAnim(owner, SETANIM_LEGS, strafeAnim,
...
/*QUAKED emplaced_eweb (0 0 1) (-12 -12 -24) (12 12 24) INACTIVE FACING
 INVULNERABLE PLAYERUSE

 INACTIVE cannot be used until used by a target_activate
 FACING - player must be facing relatively in the same direction as the gun in
 order to use it VULNERABLE - allow the gun to take damage PLAYERUSE - only the
 player makes it run its usescript
 ...
 ) wait - ONLY AFFECTS NPCs - time between bursts ( default 800 on hardest
 setting ) splashdamage - how much damage a blowing up gun deals ( default 80 )
...
*/
//----------------------------------------------------------
void eweb_pain(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
...
//----------------------------------------------------------
void eweb_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
  ...
  self->health = 0;
  //	self->s.weapon = WP_EMPLACED_GUN; // we need to be able to switch back
  //to the old weapon
  ...
  if (self->activator && self->activator->client) {
    if (self->activator->NPC) {
      ...
      // radius damage seems to throw them, but add an extra bit to throw them
      // away from the weapon
      AngleVectors(self->currentAngles, NULL, right, NULL);
      VectorMA(self->activator->client->ps.velocity, 140, right,
               self->activator->client->ps.velocity);
  ...
  G_RadiusDamage(self->currentOrigin, self, self->splashDamage,
                 self->splashRadius, self, MOD_UNKNOWN);
...
#define TURN_OFF 0x00000100 // G2SURFACEFLAG_NODESCENDANTS
  gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel], "eweb_damage",
                           TURN_OFF);
...
qboolean eweb_can_be_used(gentity_t *self, gentity_t *other,
                          gentity_t *activator) {
  if (self->health <= 0) {
  ...
  if (other && other->client &&
      G_IsRidingVehicle(other)) { // can't use eweb when on a vehicle
    return qfalse;
  }

  if (activator && activator->client &&
      G_IsRidingVehicle(activator)) { // can't use eweb when on a vehicle
    return qfalse;
  }

  if (activator && activator->client &&
      (activator->client->ps.pm_flags &
       PMF_DUCKED)) { // stand up, ya cowardly varmint!
    return qfalse;
  }

  if (activator &&
      activator->health <= 0) { // dead men ain't got no more use fer guns...
    return qfalse;
  }
  ...
  if (activator->s.number < MAX_CLIENTS) { // player must be facing general
                                           // direction of the turret head
    ...
    // Must be reasonably facing the way the gun points ( 90 degrees or so ),
...
void eweb_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  // swap the users weapon with the emplaced gun and add the ammo the gun has to
  // the player
  ...
  activator->owner = self; // kind of dumb, but when we are locked to the
                           // weapon, we are owned by it.
  ...
  VectorCopy(activator->currentOrigin,
             self->pos4); // keep this around so we know when to make them play
                          // the strafe anim
  ...
  // Ugh, so much AI code seems to assume enemies are clients, maybe this
  // shouldn't be on, but it's too late in the game to change it now without
  // knowing what side-effects this will have
  ...
  if (!(self->spawnflags & EMPLACED_PLAYERUSE) ||
      activator->s.number == 0) { // player-only usescript or any usescript
    // Run use script
...
void SP_emplaced_eweb(gentity_t *ent) {
  ...
  G_SpawnFloat("delay", "200",
               &ent->random); // NOTE: spawning into a different field!!
  ...
  ent->playerModel = gi.G2API_InitGhoul2Model(
      ent->ghoul2, name, ent->s.modelindex, NULL_HANDLE, NULL_HANDLE, 0, 0);

  // Activate our tags and bones
  ent->handLBolt = gi.G2API_AddBolt(&ent->ghoul2[ent->playerModel],
                                    "*cannonflash"); // muzzle bolt
  ent->headBolt = gi.G2API_AddBolt(
      &ent->ghoul2[ent->playerModel],
      "cannon_Xrot"); // for placing the owner relative to rotation
  ent->rootBone = gi.G2API_GetBoneIndex(&ent->ghoul2[ent->playerModel],
                                        "model_root", qtrue);
  ent->lowerLumbarBone = gi.G2API_GetBoneIndex(&ent->ghoul2[ent->playerModel],
                                               "cannon_Yrot", qtrue);
  ent->upperLumbarBone = gi.G2API_GetBoneIndex(&ent->ghoul2[ent->playerModel],
                                               "cannon_Xrot", qtrue);
  gi.G2API_SetBoneAnglesIndex(
      &ent->ghoul2[ent->playerModel], ent->lowerLumbarBone, vec3_origin,
      BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_X, NEGATIVE_Y, NULL, 0, 0);
  gi.G2API_SetBoneAnglesIndex(
      &ent->ghoul2[ent->playerModel], ent->upperLumbarBone, vec3_origin,
      BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_X, NEGATIVE_Y, NULL, 0, 0);
  // gi.G2API_SetBoneAngles( &ent->ghoul2[0], "cannon_Yrot", vec3_origin,
  // BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X, NULL); set the
  // constraints for this guy as an emplaced weapon, and his constraint angles
...
/*QUAKED emplaced_gun (0 0 1) (-24 -24 0) (24 24 64) INACTIVE x VULNERABLE
 PLAYERUSE
...
void emplaced_gun_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  // We'll just let the designers duke this one out....I mean, as to whether
  // they even want to limit such a thing.
  ...
  if (self->delay + 500 < level.time) {
    ...
    // swap the users weapon with the emplaced gun and add the ammo the gun has
    // to the player
    activator->client->ps.weapon = self->s.weapon;
    Add_Ammo(activator, WP_EMPLACED_GUN, self->count);
    activator->client->ps.stats[STAT_WEAPONS] |= (1 << WP_EMPLACED_GUN);
    ...
    // Since we move the activator inside of the gun, we reserve a solid spot
    // where they were standing in order to be able to get back out without
    // being in solid
    if (self->nextTrain) { // you never know
      G_FreeEntity(self->nextTrain);
    }
    self->nextTrain = G_Spawn();
    // self->nextTrain->classname = "emp_placeholder";
    self->nextTrain->contents =
        CONTENTS_MONSTERCLIP |
        CONTENTS_PLAYERCLIP; // hmm... playerclip too now that we're doing it
                             // for NPCs?
    ...
    // need to inflate the activator's mins/maxs since the gunsit anim puts them
    // outside of their bbox
    VectorSet(activator->mins, -24, -24, -24);
    VectorSet(activator->maxs, 24, 24, 40);

    // Move the activator into the center of the gun.  For NPC's the only way
    // the can get out of the gun is to die.
    VectorCopy(self->s.origin, activator->client->ps.origin);
    activator->client->ps.origin[2] +=
        30; // move them up so they aren't standing in the floor
    ...
    self->activator = activator;
    self->delay =
        level.time; // can't disconnect from the thing for half a second
...
//----------------------------------------------------------
void emplaced_gun_pain(gentity_t *self, gentity_t *inflictor,
...
void emplaced_gun_die(gentity_t *self, gentity_t *inflictor,
  ...
  gi.G2API_SetBoneAnglesIndex(&self->ghoul2[self->playerModel],
                              self->lowerLumbarBone, ugly, BONE_ANGLES_POSTMULT,
                              POSITIVE_Y, POSITIVE_Z, POSITIVE_X, NULL, 0, 0);
...
void SP_emplaced_gun(gentity_t *ent) {
  ...
  ent->svFlags |= SVF_PLAYER_USABLE;
  ent->contents =
      CONTENTS_BODY; // CONTENTS_SHOTCLIP|CONTENTS_PLAYERCLIP|CONTENTS_MONSTERCLIP;//CONTENTS_SOLID;
  ...
  ent->rootBone =
      gi.G2API_GetBoneIndex(&ent->ghoul2[ent->playerModel], "base_bone", qtrue);
  ent->lowerLumbarBone = gi.G2API_GetBoneIndex(&ent->ghoul2[ent->playerModel],
                                               "swivel_bone", qtrue);
  gi.G2API_SetBoneAnglesIndex(
      &ent->ghoul2[ent->playerModel], ent->lowerLumbarBone, vec3_origin,
      BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X, NULL, 0, 0);
...
void G_UpdateEmplacedWeaponData(gentity_t *ent) {
  if (ent && ent->owner && ent->health > 0) {
    ...
    if (chair->e_UseFunc ==
        useF_emplaced_gun_use) // yeah, crappy way to check this, but...
    {                          // one that you sit in
      ...
      gi.linkentity(ent);
    } else if (chair->e_UseFunc ==
               useF_eweb_use) // yeah, crappy way to check this, but...
...
void ExitEmplacedWeapon(gentity_t *ent) {
  ...
  if (ent->client) {
    ...
    if (ent->health >
        0) { // he's still alive, and we have a placeholder, so put him back
      if (ent->owner->nextTrain) {
        // reset the players position
        VectorCopy(ent->owner->nextTrain->currentOrigin,
                   ent->client->ps.origin);
                 ...
                 useF_eweb_use) // yeah, crappy way to check this, but...
      {
        ...
        for (curRadius = minRadius; curRadius <= maxRadius; curRadius += 4.0f) {
          ...
          gi.trace(&trace, start, ent->mins, ent->maxs, end, ent->s.number,
        ...
        ent->owner = eweb;
        if (!safeExit) { // don't try again for a second
    ...
    // don't let them move towards me for a couple frames so they don't step
    // back into me while I'm becoming solid to them
    ...
    if (!ent->owner->bounceCount) { // not an EWeb - the overridden bone angles
                                    // will remember the angle we left it at
  ...
  if (ent->health <=
      0) { // when die, don't set weapon back on when ejected from emplaced/eweb
    // empty hands
    ...
    if (ent->NPC) { // BTW, if a saber-using NPC ever gets off of an emplaced
                    // gun/eweb, this will not work, look at NPC_ChangeWeapon for
                    // the proper way
...
void RunEmplacedWeapon(gentity_t *ent, usercmd_t **ucmd) {
  if (((*ucmd)->buttons & BUTTON_USE || (*ucmd)->forwardmove < 0 ||
    ...
    if ((*ucmd)->upmove > 0) { // don't actually jump
      (*ucmd)->upmove = 0;
    }
  } else {
