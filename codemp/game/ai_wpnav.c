...
void BotWaypointRender(void) {
    ...
    if (gWPArray[i] && gWPArray[i]->inuse) {
      ...
      while (n < gWPArray[i]->neighbornum) {
        if (gWPArray[i]->neighbors[n].forceJumpTo &&
            gWPArray[gWPArray[i]->neighbors[n].num]) {
          G_TestLine(gWPArray[i]->origin,
                     gWPArray[gWPArray[i]->neighbors[n].num]->origin, 0x0000ff,
                     5000);
        }
        n++;
      }
  ...
  if (i >= gWPNum) {
    gWPRenderTime =
        level.time + 1500; // wait a bit after we finish doing the whole trail
    gWPRenderedFrame = 0;
  }
...
checkprint:
  ...
  if (gotbestindex && bestindex != gLastPrintedIndex) {
    ...
    trap->Print(S_COLOR_YELLOW
                "Waypoint %i\nFlags - %i (%s) (w%f)\nOrigin - (%i %i %i)\n",
                (int)(gWPArray[bestindex]->index),
                (int)(gWPArray[bestindex]->flags), flagstr,
                gWPArray[bestindex]->weight,
                (int)(gWPArray[bestindex]->origin[0]),
                (int)(gWPArray[bestindex]->origin[1]),
                (int)(gWPArray[bestindex]->origin[2]));
    // GetFlagStr allocates 128 bytes for this, if it's changed then obviously
...
void TransferWPData(int from, int to) {
  ...
  if (!gWPArray[to]) {
    trap->Print(S_COLOR_RED
                "FATAL ERROR: Could not allocated memory for waypoint\n");
  }
...
void RemoveWP_InTrail(int afterindex) {
  ...
  if (!foundanindex) {
    trap->Print(S_COLOR_YELLOW
                "Waypoint index %i should exist, but does not (?)\n",
                afterindex);
    return;
  }
...
void WPFlagsModify(int wpnum, int flags) {
  if (wpnum < 0 || wpnum >= gWPNum || !gWPArray[wpnum] ||
      !gWPArray[wpnum]->inuse) {
  ...
  while (i < nodenum) {
    if ((int)nodetable[i].origin[0] == (int)spot[0] &&
      ...
      if ((int)nodetable[i].origin[2] == (int)spot[2] ||
          ((int)nodetable[i].origin[2] < (int)spot[2] &&
           (int)nodetable[i].origin[2] + 5 > (int)spot[2]) ||
          ((int)nodetable[i].origin[2] > (int)spot[2] &&
           (int)nodetable[i].origin[2] - 5 < (int)spot[2])) {
        return 1;
      }
...
int CanGetToVector(vec3_t org1, vec3_t org2, vec3_t mins, vec3_t maxs) {
  ...
  trap->Trace(&tr, org1, mins, maxs, org2, ENTITYNUM_NONE, MASK_SOLID, qfalse,
              0, 0);
...
// int ExampleAnimEntMove(gentity_t *self, vec3_t moveTo, float stepSize)
{
  ...
  stepSize = VectorLength(stepSub); // make the step size the length of the
                                    // original positions without Z
  ...
  while (!initialDone || didMove) {
    ...
    trap->Trace(&tr, workingOrg, mins, maxs, stepGoal, ENTITYNUM_NONE,
                traceMask, qfalse, 0, 0);
    ...
    if (didMove != 1) { // stair check
      ...
      if (VectorLength(vecMeasure) > 1) {
        trap->Trace(&tr, trFrom, mins, maxs, trTo, ENTITYNUM_NONE, traceMask,
                    qfalse, 0, 0);

        if (!tr.startsolid && !tr.allsolid &&
            tr.fraction == 1) { // clear trace here, probably up a step
          vec3_t trDown;
          vec3_t trUp;
          ...
          if (!tr.startsolid &&
              !tr.allsolid) { // plop us down on the step after moving up
    ...
    if (!measureLength) { // no progress, break out. If last movement was a
                          // sucess didMove will equal 1.
      break;
    }

    stepSize -= measureLength; // subtract the progress distance from the step
                               // size so we don't overshoot the mark.
    if (stepSize <= 0) {
      break;
    }
...
int ConnectTrail(int startindex, int endindex, qboolean behindTheScenes) {
  ...
  int insertindex;
  int prenodestart;
  static byte extendednodes[MAX_NODETABLE_SIZE]; // for storing checked nodes
                                                 // and not trying to extend them
                                                 // each a bazillion times
  float fvecmeas;
  float baseheight;
  ...
  if (!RMG.integer) {
    ...
    branchDistance = 512; // be less precise here, terrain is fairly broad, and
                          // we don't want to take an hour precalculating
  }
  ...
  if (!behindTheScenes) {
    trap->Print(S_COLOR_YELLOW
                "Point %i is not connected to %i - Repairing...\n",
                startindex, endindex);
  }
  ...
  trap->Trace(&tr, startplace, NULL, NULL, starttrace, ENTITYNUM_NONE,
  ...
  while (nodenum < MAX_NODETABLE_SIZE && !foundit && cancontinue) {
    if (RMG.integer) { // adjust the branch distance dynamically depending on
      ...
      VectorSubtract(nodetable[nodenum - 1].origin,
                     gWPArray[startindex]->origin, startDist);
      VectorSubtract(nodetable[nodenum - 1].origin, gWPArray[endindex]->origin,
                     endDist);
    ...
    while (i < prenodestart) {
      if (extendednodes[i] != 1) {
        ...
        if (fvecmeas < 128 && CanGetToVector(gWPArray[endindex]->origin,
                                             nodetable[i].origin, mins, maxs)) {
          foundit = 1;
          successnodeindex = i;
          break;
        }
        ...
        trap->Trace(&tr, testspot, NULL, NULL, starttrace, ENTITYNUM_NONE,
                    MASK_SOLID, qfalse, 0, 0);

        testspot[2] = tr.endpos[2] + baseheight;

        if (!NodeHere(testspot) && !tr.startsolid && !tr.allsolid &&
            CanGetToVector(nodetable[i].origin, testspot, mins, maxs)) {
          VectorCopy(testspot, nodetable[nodenum].origin);
          nodetable[nodenum].inuse = 1;
          //					nodetable[nodenum].index =
          //nodenum;
          nodetable[nodenum].weight = nodetable[i].weight + 1;
          nodetable[nodenum].neighbornum = i;
          if ((nodetable[i].origin[2] - nodetable[nodenum].origin[2]) >
              50) { // if there's a big drop, make sure we know we can't just
                    // magically fly back up
  ...
  if (!foundit) {
    ...
    {
      trap->Print(S_COLOR_RED
                  "Could not link %i to %i, unreachable by node branching.\n",
                  startindex, endindex);
    }
    gWPArray[startindex]->flags |= WPFLAG_ONEWAY_FWD;
    gWPArray[endindex]->flags |= WPFLAG_ONEWAY_BACK;
    if (!behindTheScenes) {
      trap->Print(
          S_COLOR_YELLOW
          "Since points cannot be connected, point %i has been flagged as "
          "only-forward and point %i has been flagged as only-backward.\n",
          startindex, endindex);
    }

    /*while (nodenum >= 0)
    {
            if (nodetable[nodenum].origin[0] || nodetable[nodenum].origin[1] ||
    nodetable[nodenum].origin[2])
            {
                    CreateNewWP(nodetable[nodenum].origin,
    nodetable[nodenum].flags);
            }

            nodenum--;
    }*/
    // The above code transfers nodes into the "rendered" waypoint array.
    // Strictly for debugging.

    if (!behindTheScenes) { // just use what we have if we're auto-pathing the
                            // level
      return 0;
    } else {
      vec3_t endDist;
      ...
      while (nCount < nodenum) {
        VectorSubtract(nodetable[nCount].origin, gWPArray[endindex]->origin,
                       endDist);
  ...
  while (failsafe < MAX_NODETABLE_SIZE && i < MAX_NODETABLE_SIZE && i >= 0) {
    ...
    if (!nodetable[nodetable[i].neighbornum].inuse ||
        !CanGetToVectorTravel(
            validspotpos,
            /*nodetable[nodetable[i].neighbornum].origin*/ nodetable[i].origin,
            mins, maxs) ||
        VectorLength(a) > maxDistFactor ||
        (!CanGetToVectorTravel(validspotpos, gWPArray[endindex]->origin, mins,
                               maxs) &&
         CanGetToVectorTravel(nodetable[i].origin, gWPArray[endindex]->origin,
                              mins, maxs))) {
      nodetable[i].flags |= WPFLAG_CALCULATED;
      if (!CreateNewWP_InTrail(nodetable[i].origin, nodetable[i].flags,
                               insertindex)) {
        if (!behindTheScenes) {
          trap->Print(S_COLOR_RED
                      "Could not link %i to %i, waypoint limit hit.\n",
                      startindex, endindex);
        }
        return 0;
      }
...
int OpposingEnds(int start, int end) {
  if (!gWPArray[start] || !gWPArray[start]->inuse || !gWPArray[end] ||
      !gWPArray[end]->inuse) {
    return 0;
  }
...
int DoorBlockingSection(
    int start,
    int end) { // if a door blocks the trail, we'll just have to assume the
               // points on each side are in visibility when it's open
  trace_t tr;
  gentity_t *testdoor;
  ...
  trap->Trace(&tr, gWPArray[start]->origin, NULL, NULL, gWPArray[end]->origin,
              ENTITYNUM_NONE, MASK_SOLID, qfalse, 0, 0);
  ...
  trap->Trace(&tr, gWPArray[end]->origin, NULL, NULL, gWPArray[start]->origin,
              ENTITYNUM_NONE, MASK_SOLID, qfalse, 0, 0);
...
int RepairPaths(qboolean behindTheScenes) {
  ...
  while (i < gWPNum) {
    if (gWPArray[i] && gWPArray[i]->inuse && gWPArray[i + 1] &&
        gWPArray[i + 1]->inuse) {
      VectorSubtract(gWPArray[i]->origin, gWPArray[i + 1]->origin, a);

      if (!(gWPArray[i + 1]->flags & WPFLAG_NOVIS) &&
          !(gWPArray[i + 1]->flags &
            WPFLAG_JUMP) && // don't calculate on jump points because they might
                            // not always want to be visible (in cases of force
                            // jumping)
          !(gWPArray[i]->flags &
            WPFLAG_CALCULATED) && // don't calculate it again
          !OpposingEnds(i, i + 1) &&
          ((bot_wp_distconnect.value && VectorLength(a) > maxDistFactor) ||
           (!OrgVisible(gWPArray[i]->origin, gWPArray[i + 1]->origin,
                        ENTITYNUM_NONE) &&
            bot_wp_visconnect.value)) &&
...
int OrgVisibleCurve(vec3_t org1, vec3_t mins, vec3_t maxs, vec3_t org2,
                    int ignore) {
  trace_t tr;
  vec3_t evenorg1;
  ...
  trap->Trace(&tr, evenorg1, mins, maxs, org2, ignore, MASK_SOLID, qfalse, 0,
              0);

  if (tr.fraction == 1 && !tr.startsolid && !tr.allsolid) {
    trap->Trace(&tr, evenorg1, mins, maxs, org1, ignore, MASK_SOLID, qfalse, 0,
                0);
...
int CanForceJumpTo(int baseindex, int testingindex, float distance) {
  ...
  if (!OrgVisibleCurve(wpBase->origin, mins, maxs, wpTest->origin,
                       ENTITYNUM_NONE)) {
    return 0;
  }
      ...
      while (c < gWPNum) {
        if (gWPArray[c] && gWPArray[c]->inuse && i != c &&
          ...
          if ((nLDist < maxNeighborDist || forceJumpable) &&
              ((int)gWPArray[i]->origin[2] == (int)gWPArray[c]->origin[2] ||
               forceJumpable) &&
              (OrgVisibleBox(gWPArray[i]->origin, mins, maxs,
                             gWPArray[c]->origin, ENTITYNUM_NONE) ||
               forceJumpable)) {
            gWPArray[i]->neighbors[gWPArray[i]->neighbornum].num = c;
            if (forceJumpable &&
                ((int)gWPArray[i]->origin[2] != (int)gWPArray[c]->origin[2] ||
                 nLDist < maxNeighborDist)) {
              gWPArray[i]->neighbors[gWPArray[i]->neighbornum].forceJumpTo =
                  999; // forceJumpable; //FJSR
            } else {
              gWPArray[i]->neighbors[gWPArray[i]->neighbornum].forceJumpTo = 0;
            }
            gWPArray[i]->neighbornum++;
...
void CalculateSiegeGoals(void) {
  ...
  while (i < level.num_entities) {
    ...
    if (ent && ent->classname &&
        strcmp(ent->classname, "info_siege_objective") == 0) {
      tent = ent;
      t2ent = GetObjectThatTargets(tent);
      looptracker = 0;

      while (t2ent &&
             looptracker < 2048) { // looptracker keeps us from getting stuck in
                                   // case something is set up weird on this map
        tent = t2ent;
        t2ent = GetObjectThatTargets(tent);
        looptracker++;
      }
    ...
    if (tent && ent && tent != ent) { // tent should now be the object attached
      ...
      if (wpindex != -1 && gWPArray[wpindex] &&
          gWPArray[wpindex]->inuse) { // found the waypoint nearest the center
                                      // of this objective-related object
...
float botGlobalNavWeaponWeights[WP_NUM_WEAPONS] = {
    ...
    0, // WP_STUN_BATON,
    0, // WP_MELEE
    0, // WP_SABER,				 // NOTE: lots of code assumes this
       // is the first weapon (... which is crap) so be careful -Ste.
    0, // WP_BRYAR_PISTOL,
    3, // WP_BLASTER,
    5, // WP_DISRUPTOR,
    4, // WP_BOWCASTER,
...
int GetNearestVisibleWPToItem(vec3_t org, int ignore) {
  ...
  i = 0;
  bestdist =
      64; // has to be less than 64 units to the item or it isn't safe enough
  bestindex = -1;
  ...
  while (i < gWPNum) {
    if (gWPArray[i] && gWPArray[i]->inuse &&
      ...
      if (flLen < bestdist && trap->InPVS(org, gWPArray[i]->origin) &&
          OrgVisibleBox(org, mins, maxs, gWPArray[i]->origin, ignore)) {
        bestdist = flLen;
        bestindex = i;
      }
    }
...
void CalculateWeightGoals(
    void) { // set waypoint weights depending on weapon and item placement
  int i = 0;
  int wpindex = 0;
  gentity_t *ent;
  float weight;
  ...
  if (bot_wp_clearweight.integer) { // if set then flush out all weight/goal
                                    // values before calculating them again
    while (i < gWPNum) {
      if (gWPArray[i] && gWPArray[i]->inuse) {
        gWPArray[i]->weight = 0;
  ...
  while (i < level.num_entities) {
    ...
    if (ent && weight) {
      ...
      if (wpindex != -1 && gWPArray[wpindex] &&
          gWPArray[wpindex]
              ->inuse) { // found the waypoint nearest the center of this object
  ...
  while (i < len) {
    ...
    while (fileString[i] != '}') {
      ...
      if (fileString[i] == '-') {
        ...
        thiswp.neighbors[thiswp.neighbornum].forceJumpTo =
            999; // atoi(currentVar); //FJSR
      } else {
...
void FlagObjects(void) {
  ...
  while (i < gWPNum) {
    if (gWPArray[i] && gWPArray[i]->inuse) {
      ...
      if (tlen < bestdist) {
        trap->Trace(&tr, flag_red->s.pos.trBase, mins, maxs,
                    gWPArray[i]->origin, flag_red->s.number, MASK_SOLID, qfalse,
                    0, 0);
  ...
  while (i < gWPNum) {
    if (gWPArray[i] && gWPArray[i]->inuse) {
      ...
      if (tlen < bestdist) {
        trap->Trace(&tr, flag_blue->s.pos.trBase, mins, maxs,
                    gWPArray[i]->origin, flag_blue->s.number, MASK_SOLID,
                    qfalse, 0, 0);
...
int SavePathData(const char *filename) {
  ...
  if (!RepairPaths(qfalse)) // check if we can see all waypoints from the last.
                            // If not, try to branch over.
  {
    trap->FS_Close(f);
    return 0;
  }
  ...
  Com_sprintf(fileString, 524288, "%i %i %f (%f %f %f) { ", gWPArray[i]->index,
              gWPArray[i]->flags, gWPArray[i]->weight, gWPArray[i]->origin[0],
  ...
  while (n < gWPArray[i]->neighbornum) {
    if (gWPArray[i]->neighbors[n].forceJumpTo) {
      Com_sprintf(storeString, 4096, "%s%i-%i ", storeString,
                  gWPArray[i]->neighbors[n].num,
                  gWPArray[i]->neighbors[n].forceJumpTo);
    } else {
      Com_sprintf(storeString, 4096, "%s%i ", storeString,
                  gWPArray[i]->neighbors[n].num);
    }
    n++;
  }
  ...
  while (i < gWPNum) {
    // sprintf(fileString, "%s%i %i %f (%f %f %f) { ", fileString,
    // gWPArray[i]->index, gWPArray[i]->flags, gWPArray[i]->weight,
    // gWPArray[i]->origin[0], gWPArray[i]->origin[1], gWPArray[i]->origin[2]);
    Com_sprintf(storeString, 4096, "%i %i %f (%f %f %f) { ", gWPArray[i]->index,
                gWPArray[i]->flags, gWPArray[i]->weight, gWPArray[i]->origin[0],
                gWPArray[i]->origin[1], gWPArray[i]->origin[2]);
  ...
  trap->Print("Path data has been saved and updated. You may need to restart "
              "the level for some things to be properly calculated.\n");
...
int G_NearestNodeToPoint(
    vec3_t point) { // gets the node on the entire grid which is nearest to the
...
int G_NodeMatchingXY(float x, float y) { // just get the first unflagged node
                                         // with the matching x,y coordinates.
  int i = 0;

  while (i < nodenum) {
    if (nodetable[i].origin[0] == x && nodetable[i].origin[1] == y &&
        !nodetable[i].flags) {
      return i;
    }
...
int G_NodeMatchingXY_BA(
    int x, int y, int final) { // return the node with the lowest weight that
                               // matches the specified x,y coordinates.
  int i = 0;
  int bestindex = -1;
  float bestWeight = 9999;

  while (i < nodenum) {
    if ((int)nodetable[i].origin[0] == x && (int)nodetable[i].origin[1] == y &&
        !nodetable[i].flags &&
...
int G_RecursiveConnection(int start, int end, int weight, qboolean traceCheck,
                          float baseHeight) {
  ...
  i = 0;
  while (i < 4) {
    if (indexDirections[i] ==
        end) { // we've connected all the way to the destination.
      return indexDirections[i];
    }

    if (indexDirections[i] != -1 &&
        nodetable[indexDirections[i]]
            .flags) { // this point is already used, so it's not valid.
      indexDirections[i] = -1;
    ...
    if (indexDirections[i] != -1 &&
        traceCheck) { // if we care about trace visibility between nodes,
                      // perform the check and mark as not valid if the trace
                      // isn't clear.
      trap->Trace(&tr, nodetable[start].origin, NULL, NULL,
                  nodetable[indexDirections[i]].origin, ENTITYNUM_NONE,
    ...
    if (indexDirections[i] !=
        -1) { // it's still valid, so keep connecting via this point.
      recursiveIndex = G_RecursiveConnection(
          indexDirections[i], end, passWeight, traceCheck, baseHeight);
    }

    if (recursiveIndex !=
        -1) { // the result of the recursive check was valid, so return it.
...
void CreateAsciiTableRepresentation() { // Draw a text grid of the entire
                                        // waypoint array (useful for debugging
                                        // final waypoint placement)
  fileHandle_t f;
  int i = 0;
  int sP = 0;
  int placeX;
  ...
  while (placeY < terrain->r.absmax[1]) {
    while (placeX < terrain->r.absmax[0]) {
      ...
      i = 0;
      while (i < gWPNum) {
        if (((int)gWPArray[i]->origin[0] <= placeX &&
             (int)gWPArray[i]->origin[0] > oldX) &&
            ((int)gWPArray[i]->origin[1] <= placeY &&
             (int)gWPArray[i]->origin[1] > oldY)) {
          gotit = qtrue;
          break;
        }
        i++;
...
void CreateAsciiNodeTableRepresentation(
    int start,
    int end) { // draw a text grid of a single node path, from point A to Z.
  fileHandle_t f;
  int i = 0;
  int sP = 0;
  int placeX;
  ...
  while (placeY < terrain->r.absmax[1]) {
    while (placeX < terrain->r.absmax[0]) {
      ...
      i = 0;
      while (i < nodenum) {
        if (((int)nodetable[i].origin[0] <= placeX &&
             (int)nodetable[i].origin[0] > oldX) &&
            ((int)nodetable[i].origin[1] <= placeY &&
             (int)nodetable[i].origin[1] > oldY)) {
          gotit = qtrue;
          break;
        }
        i++;
...
qboolean G_BackwardAttachment(
    int start, int finalDestination,
    int insertAfter) { // After creating a node path between 2 points, this
                       // function links the 2 points with actual waypoint data.
  int indexDirections[4]; // 0 == down, 1 == up, 2 == left, 3 == right
  int i = 0;
  ...
  givenXY[0] -= DEFAULT_GRID_SPACING;
  indexDirections[0] =
      G_NodeMatchingXY_BA(givenXY[0], givenXY[1], finalDestination);
  ...
  while (i < 4) {
    if (indexDirections[i] != -1) {
      if (indexDirections[i] ==
          finalDestination) { // hooray, we've found the original point and
                              // linked all the way back to it.
        CreateNewWP_InsertUnder(nodetable[start].origin, 0, insertAfter);
        CreateNewWP_InsertUnder(nodetable[indexDirections[i]].origin, 0,
                                insertAfter);
        return qtrue;
      }

      if (nodetable[indexDirections[i]].weight < lowestWeight &&
          nodetable[indexDirections[i]].weight && !nodetable[indexDirections[i]].flags /*&& (nodetable[indexDirections[i]].origin[2]-64 < nodetable[start].origin[2])*/) {
  ...
  if (desiredIndex !=
      -1) { // Create a waypoint here, and then recursively call this function
            // for the next neighbor with the lowest weight.
    if (gWPNum < 3900) {
      CreateNewWP_InsertUnder(nodetable[start].origin, 0, insertAfter);
    } else {
      return qfalse;
...
void G_RMGPathing(
    void) { // Generate waypoint information on-the-fly for the random mission.
  ...
  while (placeY < terrain->r.absmax[1]) {
    ...
    while (placeX < terrain->r.absmax[0]) {
      ...
      downVec[2] -= 3000;
      trap->Trace(&tr, nodetable[nodenum].origin, trMins, trMaxs, downVec,
                  ENTITYNUM_NONE, MASK_SOLID, qfalse, 0, 0);

      if ((tr.entityNum >= ENTITYNUM_WORLD ||
           g_entities[tr.entityNum].s.eType == ET_TERRAIN) &&
          tr.endpos[2] < terrain->r.absmin[2] +
                             750) { // only drop nodes on terrain directly
  ...
  // The grid has been placed down, now use it to connect the points in the
  // level.
  while (i < gSpawnPointNum - 1) {
    if (!gSpawnPoints[i] || !gSpawnPoints[i]->inuse || !gSpawnPoints[i + 1] ||
        !gSpawnPoints[i + 1]->inuse) {
      i++;
      continue;
    }
    ...
    if (nearestIndex == -1 ||
        nearestIndexForNext ==
            -1) { // Looks like there is no grid data near one of the points.
    ...
    if (nearestIndex ==
        nearestIndexForNext) { // Two spawn points on top of each other? We
                               // don't need to do both points, keep going until
                               // the next differs.
      i++;
      continue;
    }

    // So, nearestIndex is now the node for the spawn point we're on, and
    // nearestIndexForNext is the node we want to get to from here.

    // For now I am going to branch out mindlessly, but I will probably want to
    // use some sort of A* algorithm here to lessen the time taken.
    if (G_RecursiveConnection(nearestIndex, nearestIndexForNext, 0, qtrue,
                              terrain->r.absmin[2]) !=
        nearestIndexForNext) { // failed to branch to where we want. Oh well,
                               // try it without trace checks.
      G_NodeClearForNext();

      if (G_RecursiveConnection(nearestIndex, nearestIndexForNext, 0, qfalse,
                                terrain->r.absmin[2]) !=
          nearestIndexForNext) { // still failed somehow. Just disregard this
                                 // point.
        G_NodeClearForNext();
        i++;
        continue;
    ...
    // Now our node array is set up so that highest reasonable weight is the
    // destination node, and 2 is next to the original index, so trace back to
    // that point.
    G_NodeClearFlags();
...
#endif
#endif
    if (G_BackwardAttachment(nearestIndexForNext, nearestIndex,
                             gWPNum -
                                 1)) { // successfully connected the trail from
                                       // nearestIndex to nearestIndexForNext
      if (gSpawnPoints[i + 1]->inuse && gSpawnPoints[i + 1]->item &&
          gSpawnPoints[i + 1]->item->giType ==
              IT_TEAM) { // This point is actually a CTF flag.
        if (gSpawnPoints[i + 1]->item->giTag == PW_REDFLAG ||
            gSpawnPoints[i + 1]->item->giTag ==
                PW_BLUEFLAG) { // Place a waypoint on the flag next in the
                               // trail, so the nearest grid point will link to
                               // it.
          CreateNewWP_InsertUnder(gSpawnPoints[i + 1]->s.origin,
                                  WPFLAG_NEVERONEWAY, gWPNum - 1);
        }
      }
  ...
  RepairPaths(qtrue); // this has different behaviour for RMG and will just flag
                      // all points one way that don't trace to each other.
...
void BeginAutoPathRoutine(void) { // Called for RMG levels.
  ...
  while (i < level.num_entities) {
    ...
    if (ent && ent->inuse && ent->classname && ent->classname[0] &&
      ...
      if (ent->s.origin[2] < 1280) { // h4x
        gSpawnPoints[gSpawnPointNum] = ent;
        gSpawnPointNum++;
      }
    } else if (ent && ent->inuse && ent->item && ent->item->giType == IT_TEAM &&
               (ent->item->giTag == PW_REDFLAG ||
                ent->item->giTag ==
                    PW_BLUEFLAG)) { // also make it path to flags in CTF.
      gSpawnPoints[gSpawnPointNum] = ent;
      gSpawnPointNum++;
    }
  ...
  // rww - Using a faster in-engine version because we're having to wait for
  // this stuff to get done as opposed to just saving it once.
  ...
  while (i <
         gWPNum - 1) { // disttonext is normally set on save, and when a file is
                       // loaded. For RMG we must do it after calc'ing.
...
gentity_t *GetClosestSpawn(gentity_t *ent) {
  ...
  while (i < level.num_entities) {
    ...
    if (spawn && spawn->inuse &&
        (!Q_stricmp(spawn->classname, "info_player_start") ||
         !Q_stricmp(spawn->classname, "info_player_deathmatch"))) {
      float checkDist;
      vec3_t vSub;
...
int AcceptBotCommand(char *cmd, gentity_t *pl) {
  ...
  // once bot_wp_save is issued and the trail is recalculated, bots will
  // activate again.
  ...
  if (Q_stricmp(cmd, "bot_wp_cmdlist") ==
      0) // lists all the bot waypoint commands.
  {
    trap->Print(S_COLOR_YELLOW
                "bot_wp_add" S_COLOR_WHITE
                " - Add a waypoint (optional int parameter will insert the "
                "point after the specified waypoint index in a trail)\n\n");
    trap->Print(S_COLOR_YELLOW
                "bot_wp_rem" S_COLOR_WHITE
                " - Remove a waypoint (removes last unless waypoint index is "
                "specified as a parameter)\n\n");
    trap->Print(S_COLOR_YELLOW "bot_wp_addflagged" S_COLOR_WHITE
                               " - Same as wp_add, but adds a flagged point "
                               "(type bot_wp_addflagged for help)\n\n");
    trap->Print(S_COLOR_YELLOW "bot_wp_switchflags" S_COLOR_WHITE
                               " - Switches flags on an existing waypoint "
                               "(type bot_wp_switchflags for help)\n\n");
    trap->Print(S_COLOR_YELLOW
                "bot_wp_tele" S_COLOR_WHITE
                " - Teleport yourself to the specified waypoint's location\n");
    trap->Print(S_COLOR_YELLOW "bot_wp_killoneways" S_COLOR_WHITE
                               " - Removes oneway (backward and forward) flags "
                               "on all waypoints in the level\n\n");
    trap->Print(S_COLOR_YELLOW
                "bot_wp_save" S_COLOR_WHITE
                " - Saves all waypoint data into a file for later use\n");
  ...
  if (Q_stricmp(cmd, "bot_wp_addflagged") == 0) {
    ...
    if (!RequiredSArgument || !RequiredSArgument[0]) {
      trap->Print(
          ...
          "forward\ny - Oneway, back\ng - Mission goal\nn - No "
          "visibility\nExample (for a point the bot would jump at, and reverse "
          "on when traveling a trail backwards):\nbot_wp_addflagged jx\n");
      return 1;
    }
    ...
    if (OptionalSArgument && OptionalSArgument[0]) {
      CreateNewWP_InTrail(pl->client->ps.origin, FlagsFromArgument,
                          OptionalArgument);
    } else {
  ...
  if (Q_stricmp(cmd, "bot_wp_switchflags") == 0) {
    ...
    if (!RequiredSArgument || !RequiredSArgument[0]) {
      trap->Print(
          S_COLOR_YELLOW
          "Flag string needed for bot_wp_switchflags\nType bot_wp_addflagged "
          "for a list of flags and their corresponding characters, or use 0 "
