...
extern void pitch_roll_for_slope(gentity_t *forwhom, vec3_t pass_slope = NULL,
                                 vec3_t storeAngles = NULL,
...
qboolean NPC_TryJump(gentity_t *goal, float max_xy_dist = 0.0f,
                     float max_z_diff = 0.0f);
qboolean NPC_TryJump(const vec3_t &pos, float max_xy_dist = 0.0f,
                     float max_z_diff = 0.0f);
...
extern void NPC_SetAnim(gentity_t *ent, int setAnimParts, int anim,
                        int setAnimFlags, int iBlend = SETANIM_BLEND_DEFAULT);
...
extern int NPC_FindCombatPointRetry(const vec3_t position,
                                    ...
                                    vec3_t enemyPosition, int *cpFlags,
...
#define CP_SHORTEST_PATH                                                       \
  0x0004000 // Shortest path from me to combat point to enemy
...
extern int NPC_CheckAlertEvents(qboolean checkSight, qboolean checkSound,
                                int ignoreAlert = -1,
...
extern void NPC_SetMoveGoal(gentity_t *ent, vec3_t point, int radius,
