...
// Boba Fett
// ---------
// Ah yes, this file is pretty messy.  I've tried to move everything in here,
// but in fact a lot of his AI occurs in the seeker and jedi AI files.  Some of
// these functions
//
//
//
...
qboolean Boba_StopKnockdown(gentity_t *self, gentity_t *pusher,
...
extern void WP_ResistForcePush(gentity_t *self, gentity_t *pusher,
                               qboolean noPenalty);
...
void Boba_DustFallNear(const vec3_t origin, int dustcount) {
  ...
  for (int i = 0; i < dustcount; i++) {
    ...
    gi.trace(&testTrace, origin, NULL, NULL, testEndPos,
             (player && player->inuse) ? (0) : (ENTITYNUM_NONE), MASK_SHOT,
             (EG2_Collision)0, 0);

    if (!testTrace.startsolid && !testTrace.allsolid &&
        testTrace.fraction > 0.1f && testTrace.fraction < 0.9f) {
...
////////////////////////////////////////////////////////////////////////////////////////
qboolean Boba_StopKnockdown(gentity_t *self, gentity_t *pusher,
                            const vec3_t pushDir, qboolean forceKnockdown) {
...
void Boba_Pain(gentity_t *self, gentity_t *inflictor, int damage, int mod) {
  if (mod == MOD_SABER && !(NPCInfo->aiFlags & NPCAI_FLAMETHROW)) {
    TIMER_Set(self, "Boba_TacticsSelect",
              0); // Hurt By The Saber, Time To Try Something New
  }
...
void Boba_FlyStart(gentity_t *self) { // switch to seeker AI for a while
  if (TIMER_Done(self, "jetRecharge") && !Boba_Flying(self)) {
    ...
    if (self->genericBolt1 != -1) {
      G_PlayEffect(G_EffectIndex("boba/jetSP"), self->playerModel,
...
void Boba_FlyStop(gentity_t *self) {
  ...
  if (self->genericBolt1 != -1) {
    G_StopEffect("boba/jetSP", self->playerModel, self->genericBolt1,
                 self->s.number);
  }
  if (self->genericBolt2 != -1) {
    G_StopEffect("boba/jetSP", self->playerModel, self->genericBolt2,
                 self->s.number);
  }
...
void Boba_FireFlameThrower(gentity_t *self) {
  ...
  if (tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage) {
    ...
    if (traceEnt->health > 0) {
      //			G_Knockdown( traceEnt, self, dir, Q_irand(200,
      //330), qfalse);
...
void Boba_StopFlameThrower(gentity_t *self) {
  if (self->s.number < MAX_CLIENTS) {
    ...
    G_StopEffect(G_EffectIndex("boba/fthrw"), self->playerModel,
                 self->genericBolt3, self->s.number);
    return;
  }
...
void Boba_StartFlameThrower(gentity_t *self) {
  if (!(NPCInfo->aiFlags & NPCAI_FLAMETHROW)) {
    ...
    G_PlayEffect(G_EffectIndex("boba/fthrw"), self->playerModel,
                 self->genericBolt3, self->s.number, self->s.origin, 1);
...
void Boba_DoSniper(gentity_t *self) {
  if (TIMER_Done(NPC, "PickNewSniperPoint")) {
    ...
    int SniperPoint = NPC_FindCombatPoint(
        NPC->currentOrigin, 0, NPC->currentOrigin,
        CP_SNIPE | CP_CLEAR | CP_HAS_ROUTE | CP_TRYFAR | CP_HORZ_DIST_COLL, 0,
        -1);
    if (SniperPoint != -1) {
      NPC_SetCombatPoint(SniperPoint);
      NPC_SetMoveGoal(NPC, level.combatPoints[SniperPoint].origin, 20, qtrue,
                      SniperPoint);
    }
  }

  if (Distance(NPC->currentOrigin,
               level.combatPoints[NPCInfo->combatPoint].origin) < 50.0f) {
    Boba_FireDecide();
  }
  ...
  //--------------------------
  if (NPCInfo->aiFlags & NPCAI_BLOCKED && NPC->client->moveType != MT_FLYSWIM &&
...
//	BTS_RIFLE,			// Uses Jedi / Seeker Movement
//	BTS_MISSILE,		// Uses Jedi / Seeker Movement
//	BTS_SNIPER,			// Uses Special Movement Internal To This
//File 	BTS_FLAMETHROW,		// Locked In Place
//	BTS_AMBUSHWAIT,		// Goto CP & Wait
//
//
...
void Boba_TacticsSelect() {
  ...
  bool enemyInRocketRange = (enemyDistance > BOBA_ROCKETRANGEMIN &&
                             enemyDistance < BOBA_ROCKETRANGEMAX);
  ...
  if (!enemyAlive) {
    ...
    // If It's Been Long Enough Since Our Last Flame Blast, Try To Torch The
    // Enemy
    //-----------------------------------------------------------------------------
  ...
  //---------------------------------------------------------
    ...
    nextState = (!enemyInRocketRange || Q_irand(0, NPC->count) < 1)
                    ? (BTS_RIFLE)
                    : (BTS_MISSILE);
  }

  // Hmmm...  Havn't Seen The Player In A While, We Might Want To Try Something
  // Sneaky
  //-----------------------------------------------------------------------------------
  else {
    ...
    if (SnipePointsNear && TIMER_Done(NPC, "Boba_NoSniperTime")) {
      TIMER_Set(NPC, "Boba_NoSniperTime",
                120000); // Don't snipe again for a while
      TIMER_Set(NPC, "Boba_TacticsSelect",
                Q_irand(35000, 45000)); // More patience here
      nextState = BTS_SNIPER;
    } else if (AmbushPointNear) {
      TIMER_Set(NPC, "Boba_TacticsSelect",
                Q_irand(15000, 25000)); // More patience here
...
bool Boba_Tactics() {
  ...
  //----------------------------------------------
  if (!NPCInfo->localState || NPCInfo->localState == BTS_RIFLE ||
...
bool Boba_Respawn() {
  ...
  if (AverageEnemyDirectionSamples && NPC->behaviorSet[BSET_DEATH] == 0) {
    vec3_t endPos;
    VectorMA(NPC->enemy->currentOrigin,
             1000.0f / (float)AverageEnemyDirectionSamples,
             AverageEnemyDirection, endPos);
    cp = NPC_FindCombatPoint(endPos, 0, endPos,
  ...
  //-------------------------------------------------
  if (cp == -1) {
    cp = NPC_FindCombatPoint(NPC->enemy->currentOrigin, 0,
...
void Boba_Update() {
  ...
  if (player && player->inuse && !NPC->enemy) {
    ...
    NPC->svFlags |=
        SVF_LOCKEDENEMY; // Don't forget about the enemy once you've found him
  }
  ...
  if (NPC->enemy) {
    if (!(NPC->svFlags & SVF_NOCLIENT)) {
      ...
      gi.trace(&testTrace, eyes, NULL, NULL, NPC->enemy->currentOrigin,
    ...
    if (!NPCInfo->surrenderTime) {
      if ((level.time - NPCInfo->enemyLastSeenTime) > 20000 &&
          TIMER_Done(NPC, "TooLongGoneRespawn")) {
        TIMER_Set(
            NPC, "TooLongGoneRespawn",
            30000); // Give him some time to get to you before trying again
  ...
  // Make Sure He Always Appears In The Last Area With Full Health When His
  // Death Script Is Turned On
  //--------------------------------------------------------------------------------------------------
  ...
  // Don't Forget To Turn Off That Flame Thrower, Mr. Fett - You're Waisting
  // Precious Natural Gases
  //------------------------------------------------------------------------------------------------
  ...
  if (NPC->client->ps.groundEntityNum == ENTITYNUM_NONE &&
      NPC->client->ps.forceJumpZStart && !Q_irand(0, 10)) { // take off
    Boba_FlyStart(NPC);
  }
  ...
  if (!NPCInfo->surrenderTime && (NPC->health < NPC->max_health / 10)) {
    ...
    if (cp != -1) {
      ...
      if (NPC->count < 6) {
        NPCInfo->surrenderTime =
            level.time + Q_irand(5000, 10000) + 1000 * (6 - NPC->count);
      } else {
...
bool Boba_Flee() {
  ...
  bool ReachedEscapePoint =
      (Distance(level.combatPoints[NPCInfo->combatPoint].origin,
                NPC->currentOrigin) < 50.0f);
  bool HasBeenGoneEnough = (level.time > NPCInfo->surrenderTime ||
                            (level.time - NPCInfo->enemyLastSeenTime) > 400000);
    ...
    if (HasBeenGoneEnough) {
      ...
      if (Boba_Respawn()) {
        return true;
               ...
               (NPCInfo->surrenderTime - level.time) > 3000) {
      if (TIMER_Done(NPC, "SpookPlayerTimer")) {
        ...
        case 1:
          ...
          testDirection[2] = 1.0f;
          VectorMA(NPC->enemy->currentOrigin, 400.0f, testDirection,
                   BobaFootStepLoc);
      ...
      if (BobaFootStepCount && TIMER_Done(NPC, "BobaFootStepFakeTimer")) {
        ...
        G_SoundAtSpot(
            BobaFootStepLoc,
            G_SoundIndex(va("sound/player/footsteps/boot%d", Q_irand(1, 4))),
            qtrue);
      }

      if (TIMER_Done(NPC, "ResampleEnemyDirection") &&
          NPC->enemy->resultspeed > 10.0f) {
      ...
      if (g_bobaDebug->integer && AverageEnemyDirectionSamples) {
        vec3_t endPos;
        VectorMA(NPC->enemy->currentOrigin,
                 500.0f / (float)AverageEnemyDirectionSamples,
                 AverageEnemyDirection, endPos);
  ...
  bool IsOnAPath = !!NPC_MoveToGoal(qtrue);
  if (!ReachedEscapePoint && NPCInfo->aiFlags & NPCAI_BLOCKED &&
      NPC->client->moveType != MT_FLYSWIM &&
      ((level.time - NPCInfo->blockedDebounceTime) > 1000)) {
    if (!Boba_CanSeeEnemy(NPC) &&
        Distance(NPC->currentOrigin,
                 level.combatPoints[NPCInfo->combatPoint].origin) < 200) {
