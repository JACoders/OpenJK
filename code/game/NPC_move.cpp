...
static qboolean NPC_Jump(vec3_t dest, int goalEntNum) {						// FIXME: if land on enemy, knock him down & jump off again
...
void NPC_JumpAnimation() {
	...
	if (NPC->client->NPC_class == CLASS_BOBAFETT || (NPC->client->NPC_class == CLASS_REBORN && NPC->s.weapon != WP_SABER) ||
		NPC->client->NPC_class == CLASS_ROCKETTROOPER || (NPCInfo->rank != RANK_CREWMAN && NPCInfo->rank <= RANK_LT_JG)) { // can't do acrobatics
...
qboolean NPC_TryJump() {
	...
	if (TIMER_Done(NPC, "jumpBackupDebounce")) {
		...
		if ((mJumpTrace.fraction < 1.0f) || (mJumpTrace.allsolid) || (mJumpTrace.startsolid)) {
			if (NAVDEBUG_showCollision) {
		...
		if (DistanceSquared(NPCInfo->jumpDest, NPCInfo->jumpTarget->currentOrigin) < minSafeRadiusSq) {
			...
			for (int sideTryCount = 0; sideTryCount < 8; sideTryCount++) {
				...
				gi.trace(&mJumpTrace, NPCInfo->jumpDest, NPC->mins, NPC->maxs, floorPos,
						 (NPCInfo->jumpTarget) ? (NPCInfo->jumpTarget->s.number) : (NPC->s.number), (NPC->clipmask | CONTENTS_BOTCLIP), (EG2_Collision)0, 0);
				if ((mJumpTrace.fraction < 1.0f) && (!mJumpTrace.allsolid) && (!mJumpTrace.startsolid)) {
					break;
				}
...
qboolean NPC_MoveToGoal(qboolean tryStraight) // FIXME: tryStraight not even used!  Stop passing it
{
	...
	if (PM_InKnockDown(&NPC->client->ps) || ((NPC->client->ps.legsAnim >= BOTH_PAIN1) && (NPC->client->ps.legsAnim <= BOTH_PAIN18) &&
											 NPC->client->ps.legsAnimTimer > 0)) { // If taking full body pain, don't move
		return qtrue;
	}

	if (NPC->s.eFlags & EF_LOCKED_TO_WEAPON) { // If in an emplaced gun, never try to navigate!
		return qtrue;
	}

	if (NPC->s.eFlags & EF_HELD_BY_RANCOR) { // If in a rancor's hand, never try to navigate!
		return qtrue;
	}
	if (NPC->s.eFlags & EF_HELD_BY_WAMPA) { // If in a wampa's hand, never try to navigate!
		return qtrue;
	}
	if (NPC->s.eFlags & EF_HELD_BY_SAND_CREATURE) { // If in a worm's mouth, never try to navigate!
		return qtrue;
	}
