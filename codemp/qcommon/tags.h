...
TAGDEF(ALL), TAGDEF(BOTLIB),
    ...
    TAGDEF(SND_MP3STREAMHDR), // specific MP3 struct for decoding (about 18..22K
                              // each?), not the actual MP3 binary
    TAGDEF(SND_DYNAMICMUSIC), // in-mem MP3 files
    TAGDEF(BSP_DISKIMAGE),    // temp during loading, to save both server and
                              // renderer fread()ing the same file. Only used if
                              // not low physical memory (currently 96MB)
    ...
    TAGDEF(AMBIENTSET),
    TAGDEF(STATIC), // special usage for 1-byte allocations from 0..9 to avoid
                    // CopyString() slowdowns during cvar value copies
    TAGDEF(SMALL), // used by S_Malloc, but probably more of a hint now. Will be
                   // dumped later
    TAGDEF(MODEL_MD3), // specific model types' disk images
    TAGDEF(MODEL_GLM), //	   "
    TAGDEF(MODEL_GLA), //	   "
    TAGDEF(ICARUS),    // Memory used internally by the Icarus scripting system
    // sorry, I don't want to have to keep adding these and recompiling, so
    // there may be more than I need
    ...
    TAGDEF(TEMP_WORKSPACE), // anything like file loading or image workspace
                            // that's only temporary
    TAGDEF(TEMP_PNG),       // image workspace that's only temporary
    TAGDEF(TEXTPOOL),       // for some special text-pool class thingy
    TAGDEF(IMAGE_T),  // an image_t struct (no longer on the hunk because of
                      // cached texture stuff)
    TAGDEF(INFLATE),  // Temp memory used by zlib32
    TAGDEF(DEFLATE),  // Temp memory used by zlib32//	TAGDEF(SOUNDPOOL),
                      // // pool of mem for the sound system
    TAGDEF(BSP),      // guess.
    TAGDEF(GRIDMESH), // some specific temp workspace that only seems to be in
                      // the MP codebase
    ...
    TAGDEF(TEMP_HUNKALLOC), TAGDEF(AVI), TAGDEF(MINIZIP),
    TAGDEF(COUNT)
