...
static gsl::cstring_span removeTrailingWhitespace(const gsl::cstring_span &text) {
	return {text.begin(), std::find_if_not(std::reverse_iterator<const char *>(text.end()), std::reverse_iterator<const char *>(text.begin()),
...
static gsl::cstring_span GetToken(gsl::cstring_span &text, bool allowLineBreaks, bool readToEOL = false) {
	...
	if (text[0] == '"') {
		...
		for (auto &pattern : commentPatterns) {
			tokenEnd = std::min(tokenEnd, std::search(text.begin(), tokenEnd, pattern.begin(), pattern.end()));
		}
...
CGPProperty::CGPProperty(gsl::cstring_span initKey, gsl::cstring_span initValue) : mKey(initKey) {
	if (!initValue.empty()) {
...
CGPGroup::CGPGroup(const gsl::cstring_span &initName) : mName(initName) {}
