...
void SetGoal(gentity_t *goal, float rating) {
  ...
  if (goal) {
    //		Debug_NPCPrintf( NPC, debugNPCAI, DEBUG_LEVEL_INFO, "NPC_SetGoal: %s @
    //%s (%f)\n", goal->classname, vtos( goal->currentOrigin), rating );
  } else {
    //		Debug_NPCPrintf( NPC, debugNPCAI, DEBUG_LEVEL_INFO, "NPC_SetGoal:
    //NONE\n" );
  }
}
...
void NPC_SetGoal(gentity_t *goal, float rating) {
  ...
  if (!goal) {
    //		Debug_NPCPrintf( NPC, debugNPCAI, DEBUG_LEVEL_ERROR, "NPC_SetGoal:
    //NULL goal\n" );
    return;
  }
  ...
  if (NPCInfo->goalEntity) {
    //		Debug_NPCPrintf( NPC, debugNPCAI, DEBUG_LEVEL_INFO, "NPC_SetGoal: push
...
void NPC_ClearGoal(void) {
  ...
  if (goal->inuse && !(goal->s.eFlags & EF_NODRAW)) {
    //		Debug_NPCPrintf( NPC, debugNPCAI, DEBUG_LEVEL_INFO, "NPC_ClearGoal:
    //pop %s\n", goal->classname );
...
void NPC_ReachedGoal(void) {
  //	Debug_NPCPrintf( NPC, debugNPCAI, DEBUG_LEVEL_INFO, "UpdateGoal: reached
  //goal entity\n" );
  NPC_ClearGoal();
  NPCInfo->goalTime = level.time;
...
qboolean ReachedGoal(gentity_t *goal) {
  ...
  return (qboolean)STEER::Reached(NPC, goal, NPCInfo->goalRadius,
...
Id removed a lot of shit here... doesn't seem to handle waypoints independantly
of goalentity
...
gentity_t *UpdateGoal(void) {
  ...
  //		this func doesn't seem to be working correctly for the sand
  //creature
