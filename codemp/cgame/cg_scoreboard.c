...
static void CG_DrawClientScore(int y, score_t *score, float *color, float fade,
  ...
  if (ci->powerups & (1 << PW_NEUTRALFLAG)) {
    if (largeFormat)
      CG_DrawFlagModel(iconx, y - (32 - BIGCHAR_HEIGHT) / 2, iconSize, iconSize,
                       TEAM_FREE, qfalse);
    else
    ...
    CG_DrawPic(iconx, y, iconSize, iconSize,
               trap->R_RegisterShaderNoMip((ci->duelTeam == DUELTEAM_LONE)
                                               ? "gfx/mp/pduel_icon_lone"
                                               : "gfx/mp/pduel_icon_double"));
  }

  else if (cgs.gametype == GT_SIEGE) { // try to draw the shader for this class
                                       // on the scoreboard
    if (ci->siegeIndex != -1) {
      siegeClass_t *scl = &bgSiegeClasses[ci->siegeIndex];

      if (scl->classShader) {
        CG_DrawPic(iconx, y, largeFormat ? 24 : 12, largeFormat ? 24 : 12,
                   scl->classShader);
      }
    }
  }
  ...
  CG_Text_Paint(SB_NAME_X, y, 0.9f * scale, colorWhite, ci->name, 0, 0,
                ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);

  if (score->ping != -1) {
    if (ci->team != TEAM_SPECTATOR || cgs.gametype == GT_DUEL ||
        cgs.gametype == GT_POWERDUEL) {
      if (cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL) {
        CG_Text_Paint(SB_SCORE_X, y, 1.0f * scale, colorWhite,
                      va("%i/%i", ci->wins, ci->losses), 0, 0,
                      ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
      } else {
        CG_Text_Paint(SB_SCORE_X, y, 1.0f * scale, colorWhite,
                      va("%i", score->score), 0, 0, ITEM_TEXTSTYLE_OUTLINED,
                      FONT_SMALL);
      }
    }

    if (cg_scoreboardBots.integer && ci->botSkill != -1)
      CG_Text_Paint(SB_PING_X, y, 1.0f * scale, colorWhite, "BOT", 0, 0,
                    ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
    else
      CG_Text_Paint(SB_PING_X, y, 1.0f * scale, colorWhite,
                    va("%i", score->ping), 0, 0, ITEM_TEXTSTYLE_OUTLINED,
                    FONT_SMALL);
    CG_Text_Paint(SB_TIME_X, y, 1.0f * scale, colorWhite, va("%i", score->time),
                  0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
  } else {
    CG_Text_Paint(SB_SCORE_X, y, 1.0f * scale, colorWhite, "-", 0, 0,
                  ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
    CG_Text_Paint(SB_PING_X, y, 1.0f * scale, colorWhite, "-", 0, 0,
                  ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
    CG_Text_Paint(SB_TIME_X, y, 1.0f * scale, colorWhite, "-", 0, 0,
                  ITEM_TEXTSTYLE_OUTLINED, FONT_SMALL);
  }

  // add the "ready" marker for intermission exiting
  if (cg.snap->ps.stats[STAT_CLIENTS_READY] & (1 << score->client)) {
    CG_Text_Paint(SB_NAME_X - 64, y + 2, 0.7f * scale, colorWhite,
                  CG_GetStringEdString("MP_INGAME", "READY"), 0, 0,
                  ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
  }
}
...
static int CG_TeamScoreboard(int y, team_t team, float fade, int maxClients,
                             int lineHeight, qboolean countOnly) {
  int i;
  score_t *score;
  float color[4];
  int count;
...
int CG_GetTeamNonScoreCount(team_t team) {
  ...
  for (i = 0; i < cgs.maxclients; i++) {
    ...
    if ((!ci->infoValid) ||
        (team != ci->team && team != ci->siegeDesiredTeam)) {
      continue;
    }
...
qboolean CG_DrawOldScoreboard(void) {
  ...
  if ((cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL) &&
      cgs.duelWinner != -1 &&
      cg.predictedPlayerState.pm_type == PM_INTERMISSION) {
    s = va("%s^7 %s", cgs.clientinfo[cgs.duelWinner].name,
    ...
    */
    x = (SCREEN_WIDTH) / 2;
    y = 40;
    CG_Text_Paint(x - CG_Text_Width(s, 1.0f, FONT_MEDIUM) / 2, y, 1.0f,
                  colorWhite, s, 0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
    ...
    if (cgs.gametype == GT_POWERDUEL && cgs.duelist3 != -1) {
      ...
      s = va("%s^7 %s %s", cgs.clientinfo[cgs.duelist1].name,
             CG_GetStringEdString("MP_INGAME", "SPECHUD_VERSUS"),
             cgs.clientinfo[cgs.duelist2].name);
    }
    /*w = CG_DrawStrlen( s ) * BIGCHAR_WIDTH;
    ...
    */
    ...
    s = va("%s %s", CG_GetStringEdString("MP_INGAME", "KILLEDBY"),
           cg.killerName);
    /*w = CG_DrawStrlen( s ) * BIGCHAR_WIDTH;
    x = ( SCREEN_WIDTH - w ) / 2;
    y = 40;
  ...
  // current rank
  if (cgs.gametype == GT_POWERDUEL) { // do nothing?
    ...
    if (cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR) {
      ...
      s = va("%s %s (%s %i) %s %i",
             CG_PlaceString(cg.snap->ps.persistant[PERS_RANK] + 1), sPlace, sOf,
             cg.numScores, sWith, cg.snap->ps.persistant[PERS_SCORE]);
      //	w = CG_DrawStrlen( s ) * BIGCHAR_WIDTH;
      x = (SCREEN_WIDTH) / 2;
      y = 60;
      // CG_DrawBigString( x, y, s, fade );
      CG_DrawProportionalString(x, y, s, UI_CENTER | UI_DROPSHADOW,
                                colorTable[CT_WHITE]);
    }
  } else if (cgs.gametype != GT_SIEGE) {
    if (cg.teamScores[0] == cg.teamScores[1]) {
      s = va("%s %i", CG_GetStringEdString("MP_INGAME", "TIEDAT"),
             cg.teamScores[0]);
    } else if (cg.teamScores[0] >= cg.teamScores[1]) {
      s = va("%s, %i / %i", CG_GetStringEdString("MP_INGAME", "RED_LEADS"),
    ...
    CG_Text_Paint(x - CG_Text_Width(s, 1.0f, FONT_MEDIUM) / 2, y, 1.0f,
             ...
             (cg_siegeWinTeam == 1 || cg_siegeWinTeam == 2)) {
  ...
  CG_Text_Paint(SB_NAME_X, y, 1.0f, colorWhite,
                CG_GetStringEdString("MP_INGAME", "NAME"), 0, 0,
                ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
  if (cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL) {
    char sWL[100];
    trap->SE_GetStringTextString("MP_INGAME_W_L", sWL, sizeof(sWL));

    CG_Text_Paint(SB_SCORE_X, y, 1.0f, colorWhite, sWL, 0, 0,
                  ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
  } else {
    CG_Text_Paint(SB_SCORE_X, y, 1.0f, colorWhite,
                  CG_GetStringEdString("MP_INGAME", "SCORE"), 0, 0,
                  ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
  }
  CG_Text_Paint(SB_PING_X, y, 1.0f, colorWhite,
                CG_GetStringEdString("MP_INGAME", "PING"), 0, 0,
                ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
  CG_Text_Paint(SB_TIME_X, y, 1.0f, colorWhite,
                CG_GetStringEdString("MP_INGAME", "TIME"), 0, 0,
                ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM);
  ...
  // I guess this can be accomplished simply by printing the first teams score
  // with a maxClients value passed in related to how many players are on both
  // teams.
  if (cgs.gametype >= GT_TEAM) {
    ...
    if (cg.teamScores[0] >= cg.teamScores[1]) {
      ...
      team2MaxCl =
          (maxClients - team1MaxCl); // team2 can display however many is left
                                     // over after team1's display

      n1 = CG_TeamScoreboard(y, TEAM_RED, fade, team1MaxCl, lineHeight, qtrue);
      CG_DrawTeamBackground(
          SB_SCORELINE_X - 5, y - topBorderSize, 640 - SB_SCORELINE_X * 2 + 10,
