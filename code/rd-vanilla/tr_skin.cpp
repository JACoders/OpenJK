...
// have problems
//	with our use of cached models but uncached animation.cfg files (so frame
//sequences are out of sync 	if someone rebuild the model while you're ingame and
//you change levels)...
//
// Usage:  call with psDest == NULL for a size enquire (for malloc),
//				then with NZ ptr for it to copy to your supplied
//buffer...
//
...
void RE_AnimationCFGs_DeleteAll(void) {
  for (AnimationCFGs_t::iterator it = AnimationCFGs.begin();
       it != AnimationCFGs.end(); ++it) {
...
*/
bool RE_SplitSkins(
    const char *INname, char *skinhead, char *skintorso,
    char *skinlower) { // INname=
...
qhandle_t RE_RegisterIndividualSkin(const char *name, qhandle_t hSkin) {
  ...
  if (!text) {
    ri.Printf(PRINT_WARNING,
              "WARNING: RE_RegisterSkin( '%s' ) failed to load!\n", name);
    return 0;
  }

  assert(
      tr.skins[hSkin]); // should already be setup, but might be an 3part append
  ...
  while (text_p && *text_p) {
    ...
    if (!strncmp(token, "tag_", 4)) { // these aren't in there, but just in case
                                      // you load an id style one...
      continue;
    }
    ...
    if ((unsigned)skin->numSurfaces >= ARRAY_LEN(skin->surfaces)) {
      ...
      ri.Printf(PRINT_WARNING,
                "WARNING: RE_RegisterSkin( '%s' ) more than %u surfaces!\n",
...
qhandle_t RE_RegisterSkin(const char *name) {
  ...
  // return 1;	// cope with Ghoul2's
  // calling-the-renderer-before-its-even-started hackery, must be any NZ amount
  // here to trigger configstring setting
  //	}
  ...
  Q_strncpyz(
      skin->name, name,
      sizeof(skin->name)); // always make one so it won't search for it again
  ...
  if (strcmp(name + strlen(name) - 5, ".skin")) {
    ...
    skin->surfaces[0] =
        (skinSurface_t *)R_Hunk_Alloc(sizeof(skin->surfaces[0]), qtrue);
    skin->surfaces[0]->shader =
        R_FindShader(name, lightmapsNone, stylesDefault, qtrue);
    return hSkin;
#endif
    /*		skin->numSurfaces = 1;
                    skin->surfaces[0] = (skinSurface_t *) R_Hunk_Alloc(
       sizeof(skin->surfaces[0]), qtrue ); skin->surfaces[0]->shader =
       R_FindShader( name, lightmapsNone, stylesDefault, qtrue ); return hSkin;
    */
  }
  ...
  if (RE_SplitSkins(name, (char *)&skinhead, (char *)&skintorso,
                    (char *)&skinlower)) { // three part
