...
//
// this file is my struct to gather all loose MP3 global vars into one struct so
// we can do multiple-stream decompression
//
...
typedef struct {
  union {
    ...
    struct {
      ...
      SAMPLE sample[2][2][576]; // if this isn't kept per stream then the decode
                                // breaks up

// the 4k version of these 2 seems to work for everything, but I'm reverting to
// original 8k for safety jic.
//
#define NBUF (8 * 1024)
#define BUF_TRIGGER (NBUF - 1500)
      //		#define NBUF (4096)	// 2048 works for all except
      //133+ kbps VBR files, 4096 copes with these 		#define BUF_TRIGGER
      //((NBUF/4)*3)
  ...
  //
  byte *pbSourceData; // a useful dup ptr only, this whole struct will be owned
                      // by an sfx_t struct that has the actual data ptr field
  ...
  int iBytesDecodedThisPacket; // not sure how useful this will be, it's only
                               // per-frame, so will always be full frame size
                               // (eg 2304 or below for mono) except possibly
                               // for the last frame?
  ...
  int iCopyOffset; // used for painting to DMA-feeder, since 2304 won't match
                   // the size it wants

  // some new stuff added for dynamic music, to allow "how many seconds left to
  // play" queries...
  //
  // ( m_lengthInSeconds = ((iUnpackedDataLength / iRate) / iChannels) / iWidth;
  // )
  //
  // Note that these fields are only valid/initialised if
  // MP3Stream_InitPlayingTimeFields() was called.
