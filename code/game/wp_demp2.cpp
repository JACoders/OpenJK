...
//---------------------------------------------------------
{
  ...
  gentity_t *missile =
      CreateMissile(start, forwardVec, DEMP2_VELOCITY, 10000, ent);
...
// NOTE: this is 100% for the demp2 alt-fire effect, so changes to the visual
// effect will affect game side demp2 code
//--------------------------------------------------
void DEMP2_AltRadiusDamage(gentity_t *ent) {
  float frac =
      (level.time - ent->fx_time) / 1300.0f; // synchronize with demp2 effect
  float dist, radius;
  gentity_t *gent;
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    G_Damage(gent, ent, ent->owner, dir, ent->currentOrigin,
             weaponData[WP_DEMP2].altDamage, DAMAGE_DEATH_KNOCKBACK,
...
//---------------------------------------------------------
{
  ...
  // start the effects, unfortunately, I wanted to do some custom things that I
  // couldn't easily do with the fx system, so part of it uses an event and
  // localEntities
...
//---------------------------------------------------------
{
  ...
  damage *= (1 + (count * (count - 1))); // yields damage of 12,36,84...gives a
                                         // higher bonus for longer charge

  // the shot can travel a whopping 4096 units in 1 second. Note that the shot
  // will auto-detonate at 4096 units...we'll see if this looks cool or not
  WP_MissileTargetHint(ent, start, forwardVec);
  gentity_t *missile =
      CreateMissile(start, forwardVec, DEMP2_ALT_RANGE, 1000, ent, qtrue);
