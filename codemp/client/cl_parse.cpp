...
char *svc_strings[256] = {
    ...
    "svc_snapshot", "svc_setgame",       "svc_mapchange",
};
...
void CL_ParseSnapshot(msg_t *msg) {
  ...
  if (newSnap.deltaNum <= 0) {
    ...
    if (!old->valid) {
      ...
      while ((newSnap.deltaNum & PACKET_MASK) !=
                 (newSnap.messageNum & PACKET_MASK) &&
             !old->valid) {
        newSnap.deltaNum++;
        old = &cl.snapshots[newSnap.deltaNum & PACKET_MASK];
      }
  ...
  if (old) {
    ...
    if (newSnap.ps.m_iVehicleNum) { // this means we must have written our
                                    // vehicle's ps too
...
void CL_ParseSetGame(msg_t *msg) {
  ...
  while (i < MAX_QPATH) {
    ...
    if (next) { // if next is 0 then we have finished reading to the end of the
                // message
      newGameDir[i] = next;
    } else {
      break;
...
int CL_GetValueForHidden(
    const char *s) { // string arg here just in case I want to add more sometime
  ...
  { //yes.. we seem to have an incoming server info.
          ...
          if (f)
          {
                  ...
                  { //got it
                          ...
                          //don't worry about backing over beginning of string I
  guess,
                          //we already know we successfully strstr'd the initial
  string
                          //which exceeds this length.
                          //MSG_ReadString appears to just return a static
  buffer so I
