...
// ??
// static	facet_t			facets[MAX_FACETS];	// Switched to
// MAX_FACETS = VV_FIXME, allocate these only during use
...
static int CM_GridPlane(int gridPlanes[CM_MAX_GRID_SIZE][CM_MAX_GRID_SIZE][2],
                        int i, int j, int tri) {
  int p;
...
CM_EdgePlaneNum(cGrid_t *grid,
                int gridPlanes[CM_MAX_GRID_SIZE][CM_MAX_GRID_SIZE][2], int i,
                int j, int k) {
  float *p1, *p2;
  vec3_t up;
  int p;
...
static void CM_PatchCollideFromGrid(cGrid_t *grid, patchCollide_t *pf) {
  ...
  for (i = 0; i < grid->width - 1; i++) {
    for (j = 0; j < grid->height - 1; j++) {
      ...
      noAdjust[EN_BOTTOM] =
          (qboolean)(borders[EN_BOTTOM] == gridPlanes[i][j][1]);
  ...
  if (numFacets) {
    pf->facets =
        (facet_t *)Z_Malloc(numFacets * sizeof(*pf->facets), TAG_BSP, qfalse);
    memcpy(pf->facets, facets, numFacets * sizeof(*pf->facets));
  } else {
    pf->facets = 0;
  }
  pf->planes = (patchPlane_t *)Z_Malloc(numPlanes * sizeof(*pf->planes),
                                        TAG_BSP, qfalse);
...
void CM_PreparePatchCollide(int num) {
  pfScratch =
      (patchCollide_t *)Z_Malloc(sizeof(*pfScratch) * num, TAG_BSP, qfalse);
}
...
void CM_TracePointThroughPatchCollide(traceWork_t *tw,
  ...
  if (!pc->numFacets) { // not gonna do anything anyhow?
    return;
  }
