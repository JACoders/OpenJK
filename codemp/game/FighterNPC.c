...
static qboolean Update(Vehicle_t *pVeh, const usercmd_t *pUcmd) {
  ...
  if (!BG_FighterUpdate(pVeh, pUcmd,
                        ((gentity_t *)pVeh->m_pParentEntity)->r.mins,
                        ((gentity_t *)pVeh->m_pParentEntity)->r.maxs,
                        g_gravity.value, G_VehicleTrace)) {
    return qfalse;
  }
...
#ifdef _GAME // only do this check on game side, because if it's cgame, it's
...
static void ProcessMoveCommands(Vehicle_t *pVeh) {
  ...
  // this function should only be called from pmove.. if it gets called
  // elsehwere, obviously this will explode.
  ...
  if (parentPS->hyperSpaceTime && curTime - parentPS->hyperSpaceTime <
    ...
    if (timeFrac < HYPERSPACE_TELEPORT_FRAC) { // for first half, instantly jump
      ...
      if (!(parentPS->eFlags2 & EF2_HYPERSPACE)) { // waiting to face the right
        ...
        if (parentPS->speed <
            HYPERSPACE_SPEED) { // just started hyperspace
...
// suggest using a predicted event or only doing it game-side. -rich
#ifdef _GAME
                                // G_EntitySound( ((gentity_t
                                // *)(pVeh->m_pParentEntity)), CHAN_LOCAL,
                                // pVeh->m_pVehicleInfo->soundHyper );
#elif _CGAME
          trap->S_StartSound(NULL, pm->ps->clientNum, CHAN_LOCAL,
                             pVeh->m_pVehicleInfo->soundHyper);
#endif
        }
  ...
  isLandingOrLaunching =
      (FighterIsLanding(pVeh, parentPS) || FighterIsLaunching(pVeh, parentPS));
...
#ifdef _GAME // MP GAME-side
             // NOTE: turbo sound can't be part of effect if effect is played on
             // every muzzle!
  ...
  if (curTime < pVeh->m_iTurboTime) { // going turbo speed
    ...
    // speedInc *= 2.0f;
    // no no no! this would el breako el predictiono! we want the following...
    // -rww
    speedInc =
        (pVeh->m_pVehicleInfo->acceleration * 2.0f) * pVeh->m_fTimeModifier;
...
#ifdef _GAME // well, the thing is always going to be inhabited if it's being
             // predicted!
...
static void FighterDamageRoutine(Vehicle_t *pVeh, bgEntity_t *parent,
  ...
  if (pVeh->m_LandTrace.fraction < 1.0f) { // if you land at all when pieces of
    ...
    G_Damage(vparent, killer, killer, vec3_origin, vparent->client->ps.origin,
...
void FighterRollAdjust(Vehicle_t *pVeh, playerState_t *riderPS,
                       playerState_t *parentPS) {
  /*
          float angDif = AngleSubtract(pVeh->m_vOrientation[YAW],
     riderPS->viewangles[YAW]);
  */
  float angDif =
      AngleSubtract(pVeh->m_vPrevRiderViewAngles[YAW],
                    riderPS->viewangles[YAW]); /// 2.0f;//AngleSubtract(pVeh->m_vPrevRiderViewAngles[YAW],
                                               /// riderPS->viewangles[YAW]);
  /*
  ...
  */
  ...
  if (parentPS && parentPS->speed) {
    float maxDif =
        pVeh->m_pVehicleInfo->turningSpeed * FIGHTER_TURNING_MULTIPLIER;
    ...
    pVeh->m_vOrientation[ROLL] = AngleNormalize180(
        pVeh->m_vOrientation[ROLL] + angDif * (pVeh->m_fTimeModifier * 0.2f));
  }
}
...
void FighterPitchAdjust(Vehicle_t *pVeh, playerState_t *riderPS,
                        playerState_t *parentPS) {
  float angDif = AngleSubtract(
      0, riderPS->viewangles[PITCH]); // AngleSubtract(pVeh->m_vPrevRiderViewAngles[PITCH],
  ...
  if (parentPS && parentPS->speed) {
    ...
    pVeh->m_vOrientation[PITCH] =
        AngleNormalize180(pVeh->m_vOrientation[PITCH] -
...
void FighterPitchClamp(Vehicle_t *pVeh, playerState_t *riderPS,
                       playerState_t *parentPS, int curTime) {
  if (!BG_UnrestrainedPitchRoll(riderPS, pVeh)) { // cap pitch reasonably
    if (pVeh->m_pVehicleInfo->pitchLimit != -1 && !pVeh->m_iRemovedSurfaces &&
      ...
      } else if (pVeh->m_vOrientation[PITCH] <
...
static void ProcessOrientCommands(Vehicle_t *pVeh) {
  ...
  if (parentPS->hyperSpaceTime && (curTime - parentPS->hyperSpaceTime) <
    ...
    return;
             ...
             parentPS->vehTurnaroundTime > curTime) { // in turn-around brush
    // VectorCopy( riderPS->viewangles, pVeh->m_vOrientation );
    // VectorCopy( riderPS->viewangles, parentPS->viewangles );
    VectorCopy(parentPS->viewangles, pVeh->m_vOrientation);
    VectorClear(pVeh->m_vPrevRiderViewAngles);
  ...
  isLandingOrLanded =
      (FighterIsLanding(pVeh, parentPS) || FighterIsLanded(pVeh, parentPS));
  ...
  */
  ...
  // comes tumbling down.
#ifdef _GAME // never gonna happen on client anyway, we can't be getting
  ...
  if (!parentPS->hackingTime) { // use that roll
    ...
    float strafeRoll =
        (parentPS->hackingTime / MAX_STRAFE_TIME) *
