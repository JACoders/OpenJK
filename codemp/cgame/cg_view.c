...
static void CG_CalcIdealThirdPersonViewTarget(void) {
  ...
  //	VectorMA(cameraFocusLoc, thirdPersonVertOffset, cameraup,
  //cameraIdealTarget);
    ...
    if (cg.snap && cg.snap->ps.m_iVehicleNum) {
      ...
      if (veh->m_pVehicle &&
          veh->m_pVehicle->m_pVehicleInfo
              ->cameraOverride) { // override the range with what the vehicle
                                  // wants it to be
      ...
      } else if (veh->m_pVehicle && veh->m_pVehicle->m_pVehicleInfo &&
  ...
  if (cg.snap && cg.snap->ps.m_iVehicleNum) {
    ...
    if (veh->m_pVehicle &&
      ...
      if (veh->playerState->hackingTime) {
        thirdPersonRange +=
            fabs(((float)veh->playerState->hackingTime) / MAX_STRAFE_TIME) *
            100.0f;
      }
    }
  }

  if (cg.snap && (cg.snap->ps.eFlags2 & EF2_HELD_BY_MONSTER) &&
      cg.snap->ps.hasLookTarget &&
      cg_entities[cg.snap->ps.lookTarget].currentState.NPC_class ==
          CLASS_RANCOR) // only possibility for now, may add Wampa and sand
                        // creature later
  { // stay back
...
static void CG_ResetThirdPersonViewDamp(void) {
  ...
  CG_Trace(&trace, cameraFocusLoc, cameramins, cameramaxs, cameraCurTarget,
           cg.snap->ps.clientNum, MASK_CAMERACLIP);
...
static void CG_UpdateThirdPersonTargetDamp(void) {
  ...
  if (cg.predictedVehicleState.hyperSpaceTime &&
      (cg.time - cg.predictedVehicleState.hyperSpaceTime) <
          HYPERSPACE_TIME) { // hyperspacing, no damp
    VectorCopy(cameraIdealTarget, cameraCurTarget);
  } else if (cg_thirdPersonTargetDamp.value >= 1.0 || cg.thisFrameTeleport ||
             cg.predictedPlayerState.m_iVehicleNum) { // No damping.
    VectorCopy(cameraIdealTarget, cameraCurTarget);
...
static void CG_UpdateThirdPersonCameraDamp(void) {
  ...
  if (cg.predictedVehicleState.hyperSpaceTime &&
      (cg.time - cg.predictedVehicleState.hyperSpaceTime) <
          HYPERSPACE_TIME) { // hyperspacing - don't damp camera
    dampfactor = 1.0f;
  ...
  if (dampfactor >= 1.0 || cg.thisFrameTeleport) { // No damping.
    VectorCopy(cameraIdealLoc, cameraCurLoc);
  ...
  if (trace.fraction < 1.0) {
    if (trace.entityNum < ENTITYNUM_WORLD &&
        ...
        cg_entities[trace.entityNum].currentState.eType ==
            ET_MOVER) { // get a different position for movers -rww
      centity_t *mover = &cg_entities[trace.entityNum];

      // this is absolutely hackiful, since we calc view values before we add
      // packet ents and lerp, if we hit a mover we want to update its lerp pos
      // and force it when we do the trace against it.
      if (mover->currentState.pos.trType != TR_STATIONARY &&
        ...
        // force the calc'd lerp to be the base and say we are stationary so we
        // don't try to extrapolate out further.
      ...
      if (trace.fraction <
          1.0f) { // still hit it, so take the proper trace endpos and use that.
        VectorCopy(trace.endpos, cameraCurLoc);
      }
    } else {
      VectorCopy(trace.endpos, cameraCurLoc);
...
static void CG_OffsetThirdPersonView(void) {
    ...
    if (veh->m_pVehicle &&
      ...
      if (veh->playerState->hackingTime) {
        thirdPersonHorzOffset +=
            (((float)veh->playerState->hackingTime) / MAX_STRAFE_TIME) * -80.0f;
      }
    }
  }
  ...
  { // being held
    ...
    */
    ...
    cameraFocusAngles[YAW] += cg_thirdPersonAngle.value;
    {
      ...
      cg_entities[cg.predictedPlayerState.m_iVehicleNum].m_pVehicle ) )//can
      roll/pitch without restriction
      {
      ...
      pitchOffset-(pitchOffset*pitchPerc);
              }
              else
              {
                      cameraFocusAngles[YAW] +=
    ...
    if (cg.predictedPlayerState.m_iVehicleNum // in a vehicle
        && BG_UnrestrainedPitchRoll(
               ...
               cg_entities[cg.predictedPlayerState.m_iVehicleNum]
                   .m_pVehicle)) // can roll/pitch without restriction
    {                            // no clamp on pitch
      // FIXME: when pitch >= 90 or <= -90, camera rotates oddly... need to
      // CrossProduct not just vectoangles
    } else {
  ...
  if (thirdPersonHorzOffset != 0.0f) {
    ...
    VectorMA(cameraCurLoc, thirdPersonHorzOffset, cg.refdef.viewaxis[1],
             cameraCurLoc);
  }
...
static void CG_OffsetFighterView(void) {
  ...
  if (veh->m_pVehicle &&
      veh->m_pVehicle->m_pVehicleInfo
          ->cameraOverride) { // override the horizontal offset with what the
                              // vehicle wants it to be
    ...
    if (veh->playerState->hackingTime) {
      horzOffset +=
          (((float)veh->playerState->hackingTime) / MAX_STRAFE_TIME) * -80.0f;
      range += fabs(((float)veh->playerState->hackingTime) / MAX_STRAFE_TIME) *
               100.0f;
    }
  }
  ...
  // trace to that pos
  CG_Trace(&trace, cg.refdef.vieworg, cameramins, cameramaxs, camOrg,
           cg.snap->ps.clientNum, MASK_CAMERACLIP);
  if (trace.fraction < 1.0) {
  ...
  // trace to that pos
  CG_Trace(&trace, camOrg, cameramins, cameramaxs, camBackOrg,
           cg.snap->ps.clientNum, MASK_CAMERACLIP);
  VectorCopy(trace.endpos, camOrg);
...
static int CG_CalcFov(void) {
  ...
  if (cg.predictedPlayerState.pm_type == PM_INTERMISSION) {
    ...
    if (cg.predictedPlayerState.zoomMode == 2) { // binoculars
      ...
      if (!cg.predictedPlayerState.zoomLocked) {
        ...
        if (zoomFov < MAX_ZOOM_FOV) {
          ...
          if (zoomSoundTime < cg.time || zoomSoundTime > cg.time + 10000) {
            trap->S_StartSound(cg.refdef.vieworg, ENTITYNUM_WORLD, CHAN_LOCAL,
      ...
      if (f <= 1.0) {
        fov_x = cg.predictedPlayerState.zoomFov +
                f * (fov_x - cg.predictedPlayerState.zoomFov);
      }
    }
  }
...
static void CG_DamageBlendBlob(void) {
  ...
  if (cg.snap->ps.damageType == 0) { // pure health
  ...
  } else if (cg.snap->ps.damageType == 1) { // pure shields
...
static qboolean CG_ThirdPersonActionCam(void) {
  ...
  if (!cent->ghoul2) { // if we don't have a g2 instance this frame for whatever
                       // reason then do nothing
    return qfalse;
  }

  if (cent->currentState.weapon !=
      WP_SABER) { // just being safe, should not ever happen
    return qfalse;
  }

  if ((cg.time - ci->saber[0].blade[0].trail.lastTime) >
      300) { // too long since we last got the blade position
    return qfalse;
  }

  // get direction from base to ent origin
  VectorSubtract(ci->saber[0].blade[0].trail.base, cent->lerpOrigin,
                 positionDir);
  VectorNormalize(positionDir);

  // position the cam based on the direction and saber position
  VectorMA(cent->lerpOrigin, cg_thirdPersonRange.value * 2, positionDir,
           desiredPos);

  // trace to the desired pos to see how far that way we can actually go before
  // we hit something the endpos will be valid for our desiredpos no matter what
  CG_Trace(&tr, cent->lerpOrigin, NULL, NULL, desiredPos,
  ...
  if (VectorLength(v) >
      64.0f) { // don't bother moving yet if not far from the last pos
    for (i = 0; i < 3; i++) {
        ...
        if (vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum]
          ...
          if (vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum]
            ...
            if (vehCent->m_pVehicle->m_iGunnerViewTag[turretNum] != -1) {
              ...
              if (vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum]
                      .yawBone) {
                boltIndex = trap->G2API_AddBolt(
                    vehCent->ghoul2, 0,
                    vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum]
                        .yawBone);
              } else if (vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum]
                             .pitchBone) {
                boltIndex = trap->G2API_AddBolt(
                    vehCent->ghoul2, 0,
                    vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum]
                        .pitchBone);
              } else { // well, no way of knowing, so screw it
                return qfalse;
              }
            }
            if (boltIndex != -1) {
              ...
              trap->G2API_GetBoltMatrix_NoRecNoRot(
                  vehCent->ghoul2, 0, boltIndex, &boltMatrix,
                  vehCent->lerpAngles, vehCent->lerpOrigin, cg.time, NULL,
                  vehCent->modelScale);
              BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, cg.refdef.vieworg);
              if (hackPosAndAngle) {
                // FIXME: these are assumptions, externalize?  BETTER YET: give
                // me a controller view bolt/tag for each turret
              ...
              }
              {
                ...
                VectorMA(cg_lastTurretViewAngles,
                         0.5f * (float)cg.frametime / 100.0f, deltaAngles,
...
static int CG_CalcViewValues(void) {
  ...
  */
  ...
  if (!manningTurret) { // not manning a turret on a vehicle
    VectorCopy(ps->origin, cg.refdef.vieworg);
    ...
    if (cg.predictedPlayerState.m_iVehicleNum) // in a vehicle
    {
      ...
      if (BG_UnrestrainedPitchRoll(&cg.predictedPlayerState,
                                   pVeh)) // can roll/pitch without restriction
      ...
      {
        VectorCopy(cg.predictedVehicleState.viewangles, cg.refdef.viewangles);
        cg.refdef.viewangles[PITCH] =
            AngleNormalize180(cg.refdef.viewangles[PITCH]);
      } else {
        VectorCopy(ps->viewangles, cg.refdef.viewangles);
      }
    }
  ...
  if (cg.snap->ps.weapon == WP_EMPLACED_GUN &&
      cg.snap->ps
          .emplacedIndex) { // constrain the view properly for emplaced guns
  ...
  if (!manningTurret) {
    ...
    {                            // use the vehicle's viewangles to render view!
      ...
      if (cg_thirdPersonSpecialCam.integer &&
          BG_SaberInSpecial(cg.snap->ps.saberMove)) { // the action cam
        if (!CG_ThirdPersonActionCam()) { // couldn't do it for whatever reason,
                                          // resort back to third person then
          CG_OffsetThirdPersonView();
        }
      } else {
        CG_OffsetThirdPersonView();
...
static void CG_PowerupTimerSounds(void) {
  ...
  for (i = 0; i < MAX_POWERUPS; i++) {
    ...
    if ((t - cg.time) / POWERUP_BLINK_TIME !=
      ...
      // trap->S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_ITEM,
...
void CG_DrawSkyBoxPortal(const char *cstr) {
  ...
  token = COM_ParseExt(&cstr, qfalse);
  if (!token || !token[0]) {
    trap->Error(ERR_DROP, "CG_DrawSkyBoxPortal: error parsing skybox "
  ...
  if (!cg.hyperspace) { // rww - also had to add this to add effects being
                        // rendered in portal sky areas properly.
    trap->FX_AddScheduledEffects(qtrue);
  }

  CG_AddPacketEntities(
      qtrue); // rww - There was no proper way to put real entities inside the
              // portal view before. This will put specially flagged entities in
              // the render.

  if (cg_skyOri) { // ok, we want to orient the sky refdef vieworg based on the
                   // normal vieworg's relation to the ori pos
    vec3_t dif;
  ...
  if (cg_noFogOutsidePortal) { // make sure no fog flag is stripped first, and
                               // make sure it is set on the normal refdef
...
static void CG_PlayBufferedSounds(void) {
  if (cg.soundTime < cg.time) {
    if (cg.soundBufferOut != cg.soundBufferIn &&
      ...
      trap->S_StartLocalSound(cg.soundBuffer[cg.soundBufferOut],
                              CHAN_ANNOUNCER);
...
void CG_UpdateSoundTrackers() {
  ...
  for (num = 0; num < ENTITYNUM_NONE; num++) {
    ...
    if (cent && (cent->currentState.eFlags & EF_SOUNDTRACKER) &&
        cent->currentState.number == num)
    // make sure the thing is valid at least.
    { // keep sound for this entity updated in accordance with its attached
      // entity at all times
      if (cg.snap &&
          cent->currentState.trickedentindex ==
              cg.snap->ps.clientNum) { // this is actually the player, so center
                                       // the sound origin right on top of us
        VectorCopy(cg.refdef.vieworg, cent->lerpOrigin);
        trap->S_UpdateEntityPosition(cent->currentState.number,
                                     cent->lerpOrigin);
      } else {
        trap->S_UpdateEntityPosition(
...
void CG_SE_UpdateShake(vec3_t origin, vec3_t angles) {
  ...
  if (cg.time >
      (cgScreenEffects.shake_start + cgScreenEffects.shake_duration)) {
  ...
  intensity_scale =
      1.0f -
      ((float)(cg.time - cgScreenEffects.shake_start) /
       (float)cgScreenEffects.shake_duration) *
...
void CG_SE_UpdateMusic(void) {
  ...
  if (cgScreenEffects.music_volume_time < cg.time) {
    if (cgScreenEffects.music_volume_multiplier != 1.0f ||
      ...
      Com_sprintf(musMultStr, sizeof(musMultStr), "%f",
                  cgScreenEffects.music_volume_multiplier);
  ...
  if (!cgScreenEffects
           .music_volume_set) { // if the volume_time is >= cg.time, we should
                                // have a volume multiplier set
    char musMultStr[512];
...
void CG_DoCameraShake(vec3_t origin, float intensity, int radius, int time) {
  // FIXME: When exactly is the vieworg calculated in relation to the rest of
  // the frame?s
...
void CG_EmplacedView(vec3_t angles) {
  ...
  override = BG_EmplacedView(
      cg.refdef.viewangles, angles, &yaw,
  ...
  override = BG_EmplacedView(
      cg.predictedPlayerState.viewangles, angles, &yaw,
      cg_entities[cg.snap->ps.emplacedIndex].currentState.origin2[0]);
  if (override) {
...
static void CG_AddRefentForAutoMap(centity_t *cent) {
  ...
  if (cent->ghoul2 &&
      (cent->currentState.eType == ET_PLAYER ||
       ...
       cent->currentState.modelGhoul2)) { // using a ghoul2 model
...
void CG_DrawAutoMap(void) {
  ...
  if (!r_autoMap.integer) { // don't do anything then
    return;
  }

  if (cg.snap->ps.stats[STAT_HEALTH] <= 0) { // don't show when dead
    return;
  }

  if ((cg.predictedPlayerState.pm_flags & PMF_FOLLOW) ||
      cg.predictedPlayerState.persistant[PERS_TEAM] ==
          TEAM_SPECTATOR) { // don't show when spec
    return;
  }

  local = &cgs.clientinfo[cg.predictedPlayerState.clientNum];
  if (!local->infoValid) { // don't show if bad ci
    return;
  }

  if (cgs.gametype < GT_TEAM) { // don't show in non-team gametypes
    return;
  }
  ...
  CG_DrawPic(x - SIDEFRAME_WIDTH, y, SIDEFRAME_WIDTH, h,
             cgs.media.wireframeAutomapFrame_left);
  CG_DrawPic(x + w, y, SIDEFRAME_WIDTH, h,
             cgs.media.wireframeAutomapFrame_right);
  CG_DrawPic(x - SIDEFRAME_WIDTH, y - SIDEFRAME_HEIGHT,
             w + (SIDEFRAME_WIDTH * 2), SIDEFRAME_HEIGHT,
             cgs.media.wireframeAutomapFrame_top);
  CG_DrawPic(x - SIDEFRAME_WIDTH, y + h, w + (SIDEFRAME_WIDTH * 2),
             SIDEFRAME_HEIGHT, cgs.media.wireframeAutomapFrame_bottom);
  ...
  if (cg.predictedPlayerState.m_iVehicleNum &&
      cg_entities[cg.predictedPlayerState.m_iVehicleNum].currentState.eType ==
          ET_NPC &&
      cg_entities[cg.predictedPlayerState.m_iVehicleNum]
              .currentState.NPC_class == CLASS_VEHICLE &&
      cg_entities[cg.predictedPlayerState.m_iVehicleNum].m_pVehicle &&
      cg_entities[cg.predictedPlayerState.m_iVehicleNum]
              .m_pVehicle->m_pVehicleInfo->type ==
          VH_FIGHTER) { // constantly adjust to current height
    trap->R_AutomapElevationAdjustment(cg.predictedPlayerState.origin[2]);
  } else {
    // Trace down and set the ground elevation as the main automap elevation
    // point
    ...
    fwd[2] -= 4096.0f;
    CG_Trace(&tr, cg.predictedPlayerState.origin, playerMins, playerMaxs, fwd,
...
float cg_linearFogOverride =
    0.0f; // designer-specified override for linear fogging style
...
void CG_DrawActiveFrame(int serverTime, stereoFrame_t stereoView,
  ...
  if (cgQueueLoad) { // do this before you start messing around with adding
                     // ghoul2 refents and crap
  ...
  if (cgs.gametype == GT_SIEGE && cg.snap &&
    ...
    if (cg_siegeClassIndex == -1) {
      ...
      trap->Cvar_Set("ui_mySiegeClass",
                     bgSiegeClasses[cg_siegeClassIndex].name);
    }
  }
  ...
  if (cg.snap && cg.snap->ps.saberLockTime > cg.time) {
    mSensitivity = 0.01f;
  } else if (cg.predictedPlayerState.weapon ==
             WP_EMPLACED_GUN) { // lower sens for emplaced guns and vehicles
    mSensitivity = 0.2f;
  }
#ifdef VEH_CONTROL_SCHEME_4
           ...
           && !cg.predictedPlayerState.generic1) // not as a passenger
  {
    ...
    if (cent->m_pVehicle && cent->m_pVehicle->m_pVehicleInfo &&
      ...
      BG_VehicleTurnRateForSpeed(cent->m_pVehicle, cent->currentState.speed,
                                 &mPitchOverride, &mYawOverride);
      // mSensitivityOverride = 5.0f;//old default value
      mSensitivityOverride = 0.0f;
      bUseFighterPitch = qtrue;
      trap->SetUserCmdValue(cg.weaponSelect, mSensitivity, mPitchOverride,
                            mYawOverride, mSensitivityOverride, cg.forceSelect,
                            cg.itemSelect, bUseFighterPitch);
      isFighter = qtrue;
    }
  }
...
#endif // VEH_CONTROL_SCHEME_4
  {
    ...
    if (veh && veh->currentState.eType == ET_NPC &&
        veh->currentState.NPC_class == CLASS_VEHICLE && veh->m_pVehicle &&
        veh->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER &&
        bg_fighterAltControl.integer) {
      trap->SetUserCmdValue(cg.weaponSelect, mSensitivity, mPitchOverride,
                            mYawOverride, 0.0f, cg.forceSelect, cg.itemSelect,
                            qtrue);
      veh = NULL; // this is done because I don't want an extra assign each
                  // frame because I am so perfect and super efficient.
    } else {
      trap->SetUserCmdValue(cg.weaponSelect, mSensitivity, mPitchOverride,
                            mYawOverride, 0.0f, cg.forceSelect, cg.itemSelect,
                            qfalse);
  ...
  if (cg.snap->ps.stats[STAT_HEALTH] > 0) {
    if (cg.predictedPlayerState.weapon == WP_EMPLACED_GUN && cg.predictedPlayerState.emplacedIndex /*&&
      ...
      cg.renderingThirdPerson = 1;
               ...
               cg.predictedPlayerState.forceHandExtend ==
  ...
  if (cg.predictedPlayerState.pm_type ==
      PM_SPECTATOR) { // always first person for spec
    cg.renderingThirdPerson = 0;
  }
  ...
  } else if (cg.predictedPlayerState
                 .zoomMode) { // zooming with binoculars or sniper, set the fog
                              // range based on the zoom level -rww
  ...
  // if there are any entities flagged as sound trackers and attached to other
  // entities, update their sound pos
  ...
  trap->S_Respatialize(cg.snap->ps.clientNum, cg.refdef.vieworg,
