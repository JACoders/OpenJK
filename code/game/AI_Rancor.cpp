...
qboolean Rancor_CheckAhead(vec3_t end) {
	...
	if (trace.entityNum < ENTITYNUM_WORLD && G_EntIsBreakable(trace.entityNum, NPC)) { // breakable brush in our way, break it
			...
			if (Rancor_CheckAhead(dest)) { // use our temp move straight to goal check
				...
				*/
				...
				if (!NPCInfo->blockedEntity && NPC->enemy &&
					gi.inPVS(NPC->currentOrigin, NPC->enemy->currentOrigin)) { // nothing to destroy?  just go straight at goal dest
					...
					if (DistanceHorizontal(NPC->enemy->currentOrigin, NPC->currentOrigin) <
						(NPC->maxs[0] + (MIN_DISTANCE * NPC->s.modelScale[0]))) { // close, just look at him
						horzClose = qtrue;
						NPC_FaceEnemy(qtrue);
...
void Rancor_DropVictim(gentity_t *self) {
	...
	if (self->activator) {
		...
		if (self->activator->s.number == 0) { // don't attack the player again for a bit
...
void Rancor_Swing(int boltIndex, qboolean tryGrab) {
	...
	for (i = 0; i < numEnts; i++) {
		...
		if ((radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_RANCOR) || (radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_WAMPA)) { // can't be one already being
																															   // held
			continue;
		}
		...
		*/
		...
		if (DistanceSquared(radiusEnts[i]->currentOrigin, boltOrg) <= radiusSquared) {
			if (!gi.inPVS(radiusEnts[i]->currentOrigin, NPC->currentOrigin)) { // don't grab anything that's in another PVS
				continue;
			}
			/*
			...
			*/
			if (tryGrab
				...
				&& radiusEnts[i]->client->NPC_class != CLASS_RANCOR && radiusEnts[i]->client->NPC_class != CLASS_GALAKMECH &&
				radiusEnts[i]->client->NPC_class != CLASS_ATST && radiusEnts[i]->client->NPC_class != CLASS_GONK &&
				radiusEnts[i]->client->NPC_class != CLASS_R2D2 && radiusEnts[i]->client->NPC_class != CLASS_R5D2 &&
				radiusEnts[i]->client->NPC_class != CLASS_MARK1 && radiusEnts[i]->client->NPC_class != CLASS_MARK2 &&
				radiusEnts[i]->client->NPC_class != CLASS_MOUSE && radiusEnts[i]->client->NPC_class != CLASS_PROBE &&
				radiusEnts[i]->client->NPC_class != CLASS_SEEKER && radiusEnts[i]->client->NPC_class != CLASS_REMOTE &&
				radiusEnts[i]->client->NPC_class != CLASS_SENTRY && radiusEnts[i]->client->NPC_class != CLASS_INTERROGATOR &&
				...
				*/
				if ((NPC->spawnflags & SPF_RANCOR_FASTKILL) && radiusEnts[i]->s.number >= MAX_CLIENTS) {
					G_Damage(radiusEnts[i], NPC, NPC, vec3_origin, boltOrg, radiusEnts[i]->health + 1000, (DAMAGE_NO_KNOCKBACK | DAMAGE_NO_PROTECTION),
							 MOD_MELEE);
				}
				vec3_t angs;
				...
				if (radiusEnts[i]->client->NPC_class != CLASS_RANCOR && radiusEnts[i]->client->NPC_class != CLASS_ATST &&
...
void Rancor_Smash(void) {
	...
	for (i = 0; i < numEnts; i++) {
		...
		if ((radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_RANCOR)) { // can't be one being held
			continue;
		}
		...
		if (distSq <= radiusSquared) {
			...
			if (radiusEnts[i]->health > 0 && radiusEnts[i]->client && radiusEnts[i]->client->NPC_class != CLASS_RANCOR &&
				radiusEnts[i]->client->NPC_class != CLASS_ATST) {
				if (distSq < halfRadSquared ||
					radiusEnts[i]->client->ps.groundEntityNum != ENTITYNUM_NONE) { // within range of my fist or withing ground-shaking range and not in the air
...
void Rancor_Bite(void) {
	...
	for (i = 0; i < numEnts; i++) {
		...
		if ((radiusEnts[i]->client->ps.eFlags & EF_HELD_BY_RANCOR)) { // can't be one already being held
			continue;
		}
		...
		if (DistanceSquared(radiusEnts[i]->currentOrigin, boltOrg) <= radiusSquared) {
			if ((NPC->spawnflags & SPF_RANCOR_FASTKILL) && radiusEnts[i]->s.number >= MAX_CLIENTS) {
				G_Damage(radiusEnts[i], NPC, NPC, vec3_origin, radiusEnts[i]->currentOrigin, radiusEnts[i]->health + 1000,
			...
			if (radiusEnts[i]->health <= 0 && radiusEnts[i]->client) { // killed them, chance of dismembering
...
void Rancor_Attack(float distance, qboolean doCharge, qboolean aimAtBlockedEntity) {
	if (!TIMER_Exists(NPC, "attacking") && TIMER_Done(NPC, "attackDebounce")) {
		if (NPC->count == 2 && NPC->activator) {
			...
			if ((!(NPC->spawnflags & SPF_RANCOR_FASTKILL) || NPC->activator->s.number < MAX_CLIENTS) && NPC->activator->health > 0 &&
				Q_irand(0, 1)) { // quick bite
			...
			if (!Q_irand(0, 3)) {
				...
				if (NPC->enemy && NPC->enemy->s.number == 0) { // don't attack the player again for a bit
				...
				if (NPC->enemy && NPC->enemy->inuse) {
					...
					if (VectorCompare(NPCInfo->blockedEntity->s.origin, vec3_origin)) { // no origin brush, calc center
				...
				// start effect here
				G_PlayEffect(G_EffectIndex("mrancor/breath"), NPC->playerModel, NPC->gutBolt, NPC->s.number, NPC->currentOrigin,
				...
				if (NPC->enemy && NPC->enemy->s.number == 0) { // don't attack the player again for a bit
				   ...
				   /*&& (NPC->spawnflags&SPF_RANCOR_MUTANT)*/) { // mutant rancor can smash
		...
		} else if ((NPC->spawnflags & SPF_RANCOR_MUTANT) || distance >= NPC->maxs[0] + (MIN_DISTANCE * NPC->s.modelScale[0]) - 64.0f) { // try to grab
	...
	if (TIMER_Done2(NPC, "attack_dmg", qtrue)) {
		...
		case BOTH_ATTACK1:
			if (NPC->count == 1 && NPC->activator) {
				if ((NPC->spawnflags & SPF_RANCOR_FASTKILL) && NPC->activator->s.number >= MAX_CLIENTS) {
					G_Damage(NPC->activator, NPC, NPC, vec3_origin, NPC->activator->currentOrigin, NPC->activator->health + 1000,
							 (DAMAGE_NO_KNOCKBACK | DAMAGE_NO_PROTECTION), MOD_MELEE);
				} else if ((NPC->spawnflags & SPF_RANCOR_MUTANT)) // FIXME: a flag or something would be better
				{												  // more damage
				...
				if (NPC->activator->health <= 0) {		 // killed him
					if (g_dismemberment->integer >= 3) { // make it look like we bit his head off
		...
		case BOTH_ATTACK3:
			if (NPC->count == 1 && NPC->activator) {
				...
				// KILL
				G_Damage(NPC->activator, NPC, NPC, vec3_origin, NPC->activator->currentOrigin, NPC->enemy->health + 1000,
...
void Rancor_Combat(void) {
	...
	if (advance) {																			 // have to get closer
		if ((NPC->spawnflags & SPF_RANCOR_MUTANT) && (!NPC->enemy || !NPC->enemy->client)) { // don't do breath attack vs. bbrushes
		} else {
			vec3_t yawOnlyAngles = {0, NPC->currentAngles[YAW], 0};
			if (NPC->enemy->health > 0 && fabs(distance - (250.0f * NPC->s.modelScale[0])) <= (80.0f * NPC->s.modelScale[0]) &&
				InFOV(NPC->enemy->currentOrigin, NPC->currentOrigin, yawOnlyAngles, 30, 30)) {
				int chance = 9;
				if ((NPC->spawnflags & SPF_RANCOR_MUTANT)) { // higher chance of doing breath attack
...
void NPC_Rancor_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other, const vec3_t point, int damage, int mod, int hitLoc) {
	...
	if (other && other->inuse && other != self->enemy && !(other->flags & FL_NOTARGET)) {
		if (!self->count) {
			if ((!other->s.number && !Q_irand(0, 3)) || !self->enemy || self->enemy->health == 0 ||
				...
				if (hitByRancor) { // stay mad at this Rancor for 2-5 secs before looking for other enemies
	...
	if ((hitByRancor || (self->count == 1 && self->activator && !Q_irand(0, 4)) ||
		...
		if (!Rancor_CheckRoar(self)) {
			if (self->client->ps.legsAnim != BOTH_MELEE1 && self->client->ps.legsAnim != BOTH_MELEE2 && self->client->ps.legsAnim != BOTH_ATTACK2 &&
				self->client->ps.legsAnim != BOTH_ATTACK10 && self->client->ps.legsAnim != BOTH_ATTACK11) { // cant interrupt one of the big attack anims
				/*
				if ( self->count != 1
...
qboolean Rancor_AttackBBrush(void) {
	...
	float brushSize = ((NPCInfo->blockedEntity->maxs[0] - NPCInfo->blockedEntity->mins[0]) * 0.5f +
					   (NPCInfo->blockedEntity->maxs[1] - NPCInfo->blockedEntity->mins[1]) * 0.5f) *
					  0.5f;
	float dist2Brush = VectorNormalize(dir2Brush) - (NPC->maxs[0]) - brushSize;
	if (dist2Brush < (MIN_DISTANCE * NPC->s.modelScale[0])) { // close enough to just hit it
		trace.fraction = 0.0f;
	...
	} else if (trace.entityNum == NPCInfo->blockedEntity->s.number) { // close enough, smash it!
...
void Rancor_FireBreathAttack(void) {
	...
	gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->playerModel, NPC->gutBolt, &boltMatrix, rancAngles, NPC->currentOrigin, (cg.time ? cg.time : level.time), NULL,
						   NPC->s.modelScale);
	...
	if (tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage && traceEnt->client) { // breath attack only does damage to living things
		G_Damage(traceEnt, NPC, NPC, dir, tr.endpos, damage * 2, DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK | DAMAGE_NO_HIT_LOC | DAMAGE_IGNORE_TEAM, MOD_LAVA,
				 HL_NONE);
	}
	...
	if (NPC->enemy) {
		if (NPC->enemy->client																					// enemy is a client
			...
			&& (!NPC->enemy->enemy || !NPC->enemy->enemy->client ||
				NPC->enemy->enemy->client->NPC_class != CLASS_RANCOR)) // enemy's enemy is not a client or is not a rancor (which is as scary as me anyway)
		...
		if (NPC->enemy->client && NPC->enemy->client->NPC_class == CLASS_RANCOR) { // got mad at another Rancor, look for a valid enemy
			...
			if (NPCInfo->blockedEntity) {			  // something in our way
				if (!NPCInfo->blockedEntity->inuse) { // was destroyed
					...
					// a breakable?
					if (G_EntIsBreakable(NPCInfo->blockedEntity->s.number, NPC)) { // breakable brush
						if (!Rancor_AttackBBrush()) {							   // didn't move inside that func, so call move here...?
							Rancor_Move(qtrue);
						}
						NPC_UpdateAngles(qtrue, qtrue);
						return;
					} else { // if it's a client and in our way, get mad at it!
						if (NPCInfo->blockedEntity != NPC->enemy && NPCInfo->blockedEntity->client && NPC_ValidEnemy(NPCInfo->blockedEntity) &&
							!Q_irand(0, 9)) {
			...
			if (NPC_ValidEnemy(NPC->enemy) == qfalse) {
				...
				{											 // it's been a while since the enemy died, or enemy is completely gone, get bored with him
					if ((NPC->spawnflags & SPF_RANCOR_MUTANT) && player && player->health >= 0) { // all else failing, always go after the player
		...
		Rancor_Combat();
		if (TIMER_Done(NPC, "attacking") && TIMER_Done(NPC, "takingpain") && TIMER_Done(NPC, "confusionDebounce") && NPCInfo->localState == LSTATE_CLEAR &&
			!NPC->count) {																					   // not busy
			if (!ucmd.forwardmove && !ucmd.rightmove && VectorCompare(NPC->client->ps.moveDir, vec3_origin)) { // not moving
				if (level.time - NPCInfo->enemyLastSeenTime > 5000) {										   // haven't seen an enemy in a while
					if (!Q_irand(0, 20)) {
						if (Q_irand(0, 1)) {
		...
		if (NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES) {
			Rancor_Patrol();
			if (!NPC->enemy && NPC->wait) {													  // we've been mad before and can't find an enemy
				if ((NPC->spawnflags & SPF_RANCOR_MUTANT) && player && player->health >= 0) { // all else failing, always go after the player
					NPC->lastEnemy = NPC->enemy;
					G_SetEnemy(NPC, player);
					if (NPC->enemy != NPC->lastEnemy) { // clear this so that we only sniff the player the first time we pick them up
