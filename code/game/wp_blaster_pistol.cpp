...
{
  vec3_t start;
  int damage = !alt_fire ? weaponData[WP_BRYAR_PISTOL].damage
  ...
  gentity_t *missile =
      CreateMissile(start, forwardVec, BRYAR_PISTOL_VEL, 10000, ent, alt_fire);

  missile->classname = "bryar_proj";
  if (ent->s.weapon == WP_BLASTER_PISTOL ||
      ent->s.weapon == WP_JAWA) { //*SIGH*... I hate our weapon system...
  ...
  if (alt_fire) {
    int count =
        (level.time - ent->client->ps.weaponChargeTime) / BRYAR_CHARGE_UNIT;
    ...
    damage *= count;
    missile->count = count; // this will get used in the projectile rendering
                            // code to make a beefier effect
  }
  ...
  if (ent->weaponModel[1] >
      0) { // dual pistols, toggle the muzzle point back and forth between the
           // two pistols each time he fires
