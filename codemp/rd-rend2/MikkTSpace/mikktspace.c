...
static tbool GenerateTSpaces(STSpace psTspace[], const STriInfo pTriInfos[], const SGroup pGroups[], const int iNrActiveGroups, const int piTriListIn[],
...
static void DegenEpilogue(STSpace psTspace[], STriInfo pTriInfos[], int piTriListIn[], const SMikkTSpaceContext *pContext, const int iNrTrianglesIn,
						  const int iTotTris);
	...
	if (fSep >= fvMax[channel] || fSep <= fvMin[channel]) {
		...
		for (l = iL_in; l <= iR_in; l++) {
			...
			while (l2 < l && bNotFound) {
				...
				if (vP.x == vP2.x && vP.y == vP2.y && vP.z == vP2.z && vN.x == vN2.x && vN.y == vN2.y && vN.z == vN2.z && vT.x == vT2.x && vT.y == vT2.y &&
					vT.z == vT2.z)
					bNotFound = TFALSE;
				else
					++l2;
