...
// extern void PM_SetAnimFinal(int *torsoAnim,int *legsAnim,int type,int
// anim,int priority,int *torsoAnimTimer,int *legsAnimTimer,gentity_t *gent);
...
void pitch_roll_for_slope(gentity_t *forwhom, vec3_t pass_slope) {
  ...
  if (!pass_slope || VectorCompare(vec3_origin, pass_slope)) {
    ...
    gi.trace(&trace, forwhom->currentOrigin, vec3_origin, vec3_origin, endspot,
             forwhom->s.number, MASK_SOLID, G2_NOCOLLIDE, 0);
...
void NPC_HandleAIFlags(void) {
  ...
  */
  ...
  if (d_patched->integer) { // use patch-style navigation
    if (NPCInfo->consecutiveBlockedMoves >
...
void NPC_CheckAttackHold(void) {
  ...
  /*	if ( ( NPC->client->ps.weapon == WP_BORG_ASSIMILATOR ) || (
     ...
     hold their attack anim. if ( !(NPC->NPC->aiFlags & NPCAI_SHIELDS) )
                  ...
                  VectorSubtract(NPC->enemy->currentOrigin, NPC->currentOrigin,
     vec); if( VectorLengthSquared(vec) > NPC_MaxDistSquaredForWeapon() )
                  {
                          NPCInfo->attackHoldTime = 0;
  ...
  { // everyone else...?  FIXME: need to tie this into AI somehow?
    ...
    if (VectorLengthSquared(vec) > NPC_MaxDistSquaredForWeapon()) {
      NPCInfo->attackHoldTime = 0;
    } else if (NPCInfo->attackHoldTime &&
               NPCInfo->attackHoldTime > level.time) {
      ucmd.buttons |= BUTTON_ATTACK;
...
void NPC_RunBehavior(int team, int bState) {
  ...
  if (bState == BS_CINEMATIC) {
    ...
    case TEAM_ENEMY:
      ...
      if (NPC->enemy && NPC->s.weapon == WP_NONE &&
...
void NPC_ExecuteBState(gentity_t *self) //, int msec )
{
  ...
  if (NPC->enemy) {
    if (NPC->enemy->flags & FL_DONT_SHOOT) {
      ucmd.buttons &= ~BUTTON_ATTACK;
      ucmd.buttons &= ~BUTTON_ALT_ATTACK;
    } else if (NPC->client->playerTeam != TEAM_ENEMY && NPC->enemy->NPC &&
...
void NPC_SetAnim(
  ...
  if (ent->client) { // Players, NPCs
    ...
    PM_SetAnimFinal(&ent->client->ps.torsoAnim, &ent->client->ps.legsAnim,
                    ...
                    &ent->client->ps.legsAnimTimer, ent);
  } else { // bodies, etc.
    if (setAnimFlags & SETANIM_FLAG_OVERRIDE) {
      if (setAnimParts & SETANIM_TORSO) {
