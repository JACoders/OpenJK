...
void G_RunMissile(gentity_t *ent) {
	...
	if (d_projectileGhoul2Collision.integer) {
		...
		if (tr.fraction != 1.0 && tr.entityNum < ENTITYNUM_WORLD) {
			...
			if (g2Hit->inuse && g2Hit->client && g2Hit->ghoul2) { // since we used G2TRFLAG_GETSURFINDEX, tr.surfaceFlags will actually contain the index of the
																  // surface on the ghoul2 model we collided with.
