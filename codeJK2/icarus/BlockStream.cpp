...
void CBlockMember::SetData(const char *data) { WriteDataPointer(data, strlen(data) + 1); }
...
int CBlockMember::ReadMember(char **stream, int *streamPos) {
	...
	*streamPos += sizeof(int);
	...
	if (m_id == ID_RANDOM) { // special case, need to initialize this member's data to Q3_INFINITE so we can randomize the number only the first time random is
							 // checked when inside a wait
		m_size = sizeof(float);
		*streamPos += sizeof(int);
