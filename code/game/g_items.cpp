...
int Add_Ammo2(gentity_t *ent, int ammoType, int count) {
  ...
  if (ammoType != AMMO_FORCE) {
    ...
    if (ent->client->ps.forcePower >=
        ammoData[ammoType]
            .max) { // if have full force, just get 25 extra per crystal
      ent->client->ps.forcePower += 25;
    } else { // else if don't have full charge, give full amount, up to max + 25
      ent->client->ps.forcePower += count;
      if (ent->client->ps.forcePower >=
          ammoData[ammoType].max + 25) { // cap at max + 25
        ent->client->ps.forcePower = ammoData[ammoType].max + 25;
      }
    }

    if (ent->client->ps.forcePower >=
        ammoData[ammoType].max * 2) { // always cap at twice a full charge
...
int Pickup_Battery(gentity_t *ent, gentity_t *other) {
  ...
  // There may be some left over in quantity if the player is close to full, but
  // with pickup items, this amount will just be lost
...
gentity_t *
G_DropSaberItem(const char *saberType, saber_colors_t saberColor,
                vec3_t saberPos, vec3_t saberVel, vec3_t saberAngles,
                gentity_t *copySaber) { // turn it into a pick-uppable item!
  gentity_t *newItem = NULL;
  if (saberType && saberType[0]) { // have a valid string to use for saberType
    newItem = G_Spawn();
    if (newItem) {
      ...
      newItem->delay =
          level.time + 500; // so you can't pick it back up right away
    }
  }
  return newItem;
}
...
qboolean Pickup_Saber(gentity_t *self, qboolean hadSaber,
                      gentity_t *pickUpSaber) {
  // NOTE: loopAnim = saberSolo, alt_fire = saberLeftHand, NPC_type = saberType,
  ...
  if (Q_stricmp("player", pickUpSaber->NPC_type) ==
      0) { //"player" means use cvar info
    ...
    if (self->client->ps.weapon == WP_SABER &&
        self->client->ps.weaponTime >
            0) { // can't pick up a new saber while the old one is busy (also
                 // helps to work as a debouncer so you don't swap out sabers
                 // rapidly when touching more than one at a time)
      return qfalse;
    }

    if (pickUpSaber->count == 1 && g_saberPickuppableDroppedSabers->integer) {
      swapSabers = qtrue;
    }

    if (WP_SaberParseParms(
            pickUpSaber->NPC_type,
            &newSaber)) { // successfully found a saber .sab entry to use
      int saberNum = 0;
      qboolean removeLeftSaber = qfalse;
      if (pickUpSaber->alt_fire) { // always go in the left hand
        if (!hadSaber) {           // can't have a saber only in your left hand!
          return qfalse;
        }
        saberNum = 1;
        // just in case...
        removeLeftSaber = qtrue;
      } else if (!hadSaber) { // don't have a saber at all yet, put it in our
                              // right hand
        saberNum = 0;
        // just in case...
        removeLeftSaber = qtrue;
      } else if (pickUpSaber->loopAnim // only supposed to use this one saber
                                       // when grab this pickup
                 || (newSaber.saberFlags &
                     SFL_TWO_HANDED) // new saber is two-handed
                 || (hadSaber && (self->client->ps.saber[0].saberFlags &
                                  SFL_TWO_HANDED))) // old saber is two-handed
      { // replace the old right-hand saber and remove the left hand one
        saberNum = 0;
        removeLeftSaber = qtrue;
      } else { // have, at least, a saber in our right hand and the new one
               // could go in either left or right hand
        if (self->client->ps.dualSabers) { // I already have 2 sabers
          vec3_t dir2Saber, rightDir;
          // to determine which one to replace, see which side of me it's on
          VectorSubtract(pickUpSaber->currentOrigin, self->currentOrigin,
                         dir2Saber);
          dir2Saber[2] = 0;
        ...
        } else { // just add it as a second saber
          saberNum = 1;
          // just in case...
          removeLeftSaber = qtrue;
        }
      }
      if (saberNum == 0) { // want to reach out with right hand
        if (self->client->ps.torsoAnim ==
            BOTH_BUTTON_HOLD) { // but only if already playing the pickup with
                                // left hand anim...
          NPC_SetAnim(self, SETANIM_TORSO, BOTH_SABERPULL,
                      SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
        }
        if (swapSabers) { // drop first one where the one we're picking up is
          G_DropSaberItem(self->client->ps.saber[saberNum].name,
                          self->client->ps.saber[saberNum].blade[0].color,
                          pickUpSaber->currentOrigin, (float *)vec3_origin,
                          pickUpSaber->currentAngles, pickUpSaber);
          if (removeLeftSaber) { // drop other one at my origin
            G_DropSaberItem(self->client->ps.saber[1].name,
                            self->client->ps.saber[1].blade[0].color,
                            self->currentOrigin, (float *)vec3_origin,
                            self->currentAngles, pickUpSaber);
          }
        }
      } else {
        if (swapSabers) {
      ...
      if (self->client->ps.saber[saberNum].stylesLearned) {
        self->client->ps.saberStylesKnown |=
            self->client->ps.saber[saberNum].stylesLearned;
      }
      if (self->client->ps.saber[saberNum].singleBladeStyle) {
        self->client->ps.saberStylesKnown |=
            self->client->ps.saber[saberNum].singleBladeStyle;
      }
      if (pickUpSaber->NPC_targetname != NULL) { // NPC_targetname = saberColor
        saber_colors_t saber_color =
      ...
      if (self->client->ps.torsoAnim == BOTH_BUTTON_HOLD ||
          self->client->ps.torsoAnim ==
              BOTH_SABERPULL) { // don't let them attack right away, force them
                                // to finish the anim
...
int Pickup_Weapon(gentity_t *ent, gentity_t *other) {
  ...
  */
  ...
  if (ent->item->giTag == WP_SABER &&
      (!hadWeapon ||
       ent->NPC_type != NULL)) { // didn't have a saber or it is specifying a
                                 // certain kind of saber to use
    if (!Pickup_Saber(other, hadWeapon, ent)) {
      return 0;
    }
  }

  if (other->s.number) { // NPC
    if (other->s.weapon == WP_NONE ||
        ent->item->giTag == WP_SABER) { // NPC with no weapon picked up a
                                        // weapon, change to this weapon
      // FIXME: clear/set the alt-fire flag based on the picked up weapon and my
      // class?
      ...
      if (ent->item->giTag == WP_SABER) {
        ...
        G_CreateG2AttachedWeaponModel(
            other, weaponData[ent->item->giTag].weaponMdl, other->handRBolt, 0);
      }
    }
  }
  if (ent->item->giTag == WP_SABER) {  // picked up a saber
    if (other->s.weapon != WP_SABER) { // player picking up saber
      other->client->ps.weapon = WP_SABER;
      other->client->ps.weaponstate = WEAPON_RAISING;
      if (other->s.number < MAX_CLIENTS) { // make sure the cgame-side knows
                                           // this
        CG_ChangeWeapon(WP_SABER);
      } else { // make sure the cgame-side knows this
        ChangeWeapon(other, WP_SABER);
      }
    }
    if (!other->client->ps.SaberActive()) { // turn it/them on!
      other->client->ps.SaberActivate();
    }
  }
...
int Pickup_Holocron(gentity_t *ent, gentity_t *other) {
  ...
  gi.cvar_set("cg_updatedDataPadForcePower2",
              "0"); // The +1 is offset in the print routine.
  cg_updatedDataPadForcePower2.integer = 0;
  gi.cvar_set("cg_updatedDataPadForcePower3",
              "0"); // The +1 is offset in the print routine.
...
qboolean CheckItemCanBePickedUpByNPC(gentity_t *item, gentity_t *pickerupper) {
  ...
  { // non-player, in combat, picking up a dropped item that does NOT belong to
    // the player and it *not* a security key
...
void Touch_Item(gentity_t *ent, gentity_t *other, trace_t *trace) {
  ...
  if (other->client->ps.pm_time > 0) { // cant pick up when out of control
    return;
  }
  ...
  if (ent->noDamageTeam != TEAM_FREE &&
      other->client->playerTeam !=
          ent->noDamageTeam) { // only one team can pick it up
    return;
  }

  if (!G_CanPickUpWeapons(other)) { // FIXME: some flag would be better
    // droids can't pick up items/weapons!
    return;
  }

  // FIXME: need to make them run toward a dropped weapon when fleeing without
  // one?
  // FIXME: need to make them come out of flee mode when pick up their old
  // weapon?
  if (CheckItemCanBePickedUpByNPC(ent, other)) {
    if (other->NPC && other->NPC->goalEntity &&
        other->NPC->goalEntity ==
  ...
  if (other->client) {
    if ((other->client->ps.eFlags & EF_FORCE_GRIPPED) ||
        (other->client->ps.eFlags &
         EF_FORCE_DRAINED)) { // can't pick up anything while being gripped
      return;
    }
    if (PM_InKnockDown(&other->client->ps) &&
        !PM_InGetUp(&other->client->ps)) { // can't pick up while in a knockdown
      return;
    }
  }
  ...
  if (ent->item->giType == IT_WEAPON &&
      ent->item->giTag == WP_SABER) { // a saber
    if (ent->delay >
        level.time) { // just picked it up, don't pick up again right away
      return;
    }
  }
  ...
  if (other->s.number < MAX_CLIENTS &&
      (ent->spawnflags &
       ITMSF_USEPICKUP)) { // only if player is holing use button
    if (!(other->client->usercmd.buttons & BUTTON_USE)) { // not holding use?
      return;
    }
  }
  ...
  case IT_WEAPON:
    if (other->NPC &&
        other->s.weapon ==
            WP_NONE) { // Make them duck and sit here for a few seconds
  ...
  if (!other->s.number &&
      g_timescale->value <
          1.0f) { // SIGH... with timescale on, you lose events left and right
    extern void CG_ItemPickup(int itemNum, qboolean bHadItem);
    // but we're SP so we'll cheat
    cgi_S_StartSound(NULL, other->s.number, CHAN_AUTO,
  ...
  if (ent->item->giType == IT_WEAPON &&
      ent->item->giTag == WP_SABER) { // a saber that was picked up
    if (ent->count < 0) {             // infinite supply
      ent->delay = level.time + 500;
      return;
    }
    ent->count--;
    if (ent->count > 0) { // still have more to pick up
      ent->delay = level.time + 500;
      return;
    }
  }
...
gentity_t *LaunchItem(gitem_t *item, const vec3_t origin, const vec3_t velocity,
  ...
  dropped->classname =
      G_NewString(item->classname); // copy it so it can be freed safely
  dropped->item = item;
  ...
  dropped->contents = CONTENTS_TRIGGER |
                      CONTENTS_ITEM; // CONTENTS_TRIGGER;//not CONTENTS_BODY for
                                     // dropped items, don't need to ID them
  ...
  if (item->giType == IT_WEAPON) {
    // give weapon items zero pitch, a random yaw, and rolled onto their
    // sides...but would be bad to do this for a bowcaster
    if (item->giTag != WP_BOWCASTER && item->giTag != WP_THERMAL &&
        item->giTag != WP_TRIP_MINE && item->giTag != WP_DET_PACK) {
...
gentity_t *Drop_Item(gentity_t *ent, gitem_t *item, float angle,
  ...
  dropped->activator =
      ent; // so we know who we belonged to so they can pick it back up later
  dropped->s.time = level.time; // mark this time so we aren't picked up
                                // instantly by the guy who dropped us
  return dropped;
}
...
void Use_Item(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  if ((ent->svFlags & SVF_PLAYER_USABLE) && other &&
      !other->s.number) { // used directly by the player, pick me up
    if ((ent->spawnflags &
         ITMSF_USEPICKUP)) { // player has to be touching me and hit use to pick
                             // it up, so don't allow this
      if (!G_BoundsOverlap(ent->absmin, ent->absmax, other->absmin,
                           other->absmax)) { // not touching
        return;
      }
    }
...
void FinishSpawningItem(gentity_t *ent) {
  ...
  if ((!ent->mins[0] && !ent->mins[1] && !ent->mins[2]) &&
    ...
    VectorSet(ent->mins, -ITEM_RADIUS, -ITEM_RADIUS,
              -2); // to match the comments in the items.dat file!
  ...
  if (ent->item->giType == IT_WEAPON && ent->item->giTag == WP_SABER &&
    ...
    if (Q_stricmp("player", ent->NPC_type) == 0 && g_saber->string &&
        g_saber->string[0] && Q_stricmp("none", g_saber->string) &&
        Q_stricmp("NULL", g_saber->string)) { // player's saber
      WP_SaberParseParms(g_saber->string, &itemSaber);
    } else { // specific saber
      WP_SaberParseParms(ent->NPC_type, &itemSaber);
    }
    // NOTE:  should I keep this string around for any reason?  Will I ever need
    // it later? ent->??? = G_NewString( itemSaber.model );
    gi.G2API_InitGhoul2Model(ent->ghoul2, itemSaber.model,
                             G_ModelIndex(itemSaber.model), NULL_HANDLE,
                             NULL_HANDLE, 0, 0);
    WP_SaberFreeStrings(itemSaber);
  } else {
    gi.G2API_InitGhoul2Model(ent->ghoul2, ent->item->world_model,
                             G_ModelIndex(ent->item->world_model), NULL_HANDLE,
                             NULL_HANDLE, 0, 0);
  }
  ...
  */
  ...
  ent->contents =
      CONTENTS_TRIGGER | CONTENTS_ITEM; // CONTENTS_BODY;//CONTENTS_TRIGGER|
  ...
  // Hang in air?
  ent->s.origin[2] +=
      1; // just to get it off the damn ground because coplanar = insolid
  if ((ent->spawnflags & ITMSF_SUSPEND) || (ent->flags & FL_DROPPED_ITEM)) {
    // suspended
    G_SetOrigin(ent, ent->s.origin);
  } else {
    // drop to floor
    VectorSet(dest, ent->s.origin[0], ent->s.origin[1], MIN_WORLD_COORD);
    gi.trace(&tr, ent->s.origin, ent->mins, ent->maxs, dest, ent->s.number,
             MASK_SOLID | CONTENTS_PLAYERCLIP, (EG2_Collision)0, 0);
    if (tr.startsolid) {
  ...
  */
  ...
  if ((ent->spawnflags & ITMSF_STATIONARY)) { // can't be pushed around
    ent->flags |= FL_NO_KNOCKBACK;
  }

  if ((ent->flags & FL_DROPPED_ITEM)) { // go away after 30 seconds
...
void ClearRegisteredItems(void) {
  ...
  // these are given in g_client, ClientSpawn(), but MUST be registered HERE,
  // BEFORE cgame starts. RegisterItem( FindItemForWeapon( WP_NONE ) ); //has no
  // item
...
void RegisterItem(gitem_t *item) {
  ...
  gi.SetConfigstring(
      CS_ITEMS, itemRegistered); // Write the needed items to a config string
}
...
//-----------------------------------------------------------------------------
{
  ...
  // I could be fancy and add a count or something like that to be able to spawn
  // the item numerous times...
...
void G_SpawnItem(gentity_t *ent, gitem_t *item) {
  ...
  } else { // some movers spawn on the second frame, so delay item
  ...
  // NOTE: if we want to do cool cross-hair colors for items, we can just modify
  // this, but for now, don't do it
  ...
  if (ent->item && ent->item->giType == IT_WEAPON &&
      ent->item->giTag == WP_SABER) { // weapon_saber item
    if (!ent->count) {                // can only pick up once
      ent->count = 1;
    }
  }
  ent->team = NULL;
...
void G_BounceItem(gentity_t *ent, trace_t *trace) {
  ...
  if (ent->item && ent->item->giType == IT_WEAPON &&
      ent->item->giTag == WP_SABER && (ent->flags & FL_DROPPED_ITEM)) {
    droppedSaber = qtrue;
  }
  ...
  if (droppedSaber) { // a dropped saber item
    // FIXME: use NPC_type (as saberType) to get proper bounce sound?
    WP_SaberFallSound(NULL, ent);
  }

  // check for stop
  if (trace->plane.normal[2] > 0 && ent->s.pos.trDelta[2] < 40) { // stop
    ...
    if (droppedSaber) { // a dropped saber item
      ...
      if (ent->NPC_type && ent->NPC_type[0]) { // we have a valid saber for this
        saberInfo_t saber;
  ...
  if (droppedSaber) { // a dropped saber item
    ...
    VectorSet(ent->s.apos.trDelta, Q_irand(-300, 300), Q_irand(-300, 300),
              Q_irand(-300, 300));
  }
...
void G_RunItem(gentity_t *ent) {
  ...
  if (ent->s.pos.trType == TR_STATIONARY) {
    ...
    if (!g_gravity->value) {
      ...
      ent->s.pos.trTime = level.time;
      ent->s.pos.trDelta[0] +=
          Q_flrand(-1.0f, 1.0f) * 40.0f; // I dunno, just do this??
      ent->s.pos.trDelta[1] += Q_flrand(-1.0f, 1.0f) * 40.0f;
      ent->s.pos.trDelta[2] += Q_flrand(0.0f, 1.0f) * 20.0f;
    } else if ((ent->flags & FL_DROPPED_ITEM) && ent->item &&
               ent->item->giType == IT_WEAPON &&
               ent->item->giTag == WP_SABER) { // a dropped saber item, check
                                               // below, just in case
      int ignore = ENTITYNUM_NONE;
      if (ent->clipmask) {
        mask = ent->clipmask;
      } else {
        mask = MASK_SOLID | CONTENTS_PLAYERCLIP; // shouldn't be able to get
                                                 // anywhere player can't
      }
      if (ent->owner) {
      ...
      VectorSet(origin, ent->currentOrigin[0], ent->currentOrigin[1],
                ent->currentOrigin[2] - 1);
      gi.trace(&tr, ent->currentOrigin, ent->mins, ent->maxs, origin, ignore,
               mask, (EG2_Collision)0, 0);
      if (!tr.allsolid && !tr.startsolid &&
          tr.fraction > 0.001f) { // wha?  fall....
        ent->s.pos.trType = TR_GRAVITY;
        ent->s.pos.trTime = level.time;
      }
    }
  ...
  if (ent->clipmask) {
    ...
    mask =
        MASK_SOLID |
        CONTENTS_PLAYERCLIP; // shouldn't be able to get anywhere player can't
  }
...
void ItemUse_Bacta(gentity_t *ent) {
  ...
  G_SoundOnEnt(
      ent, CHAN_VOICE,
      va("sound/weapons/force/heal%d_%c.mp3", Q_irand(1, 4), g_sex->string[0]));
}
