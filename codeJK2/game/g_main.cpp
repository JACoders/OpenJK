...
qboolean PInUse(unsigned int entNum) {
  ...
  return (qboolean)(
      (g_entityInUseBits[entNum / 32] & (1u << (entNum & 0x1f))) != 0);
}
...
void G_InitCvars(void) {
  ...
  g_dismemberment =
      gi.cvar("g_dismemberment", "3",
              CVAR_ARCHIVE); // 0 = none, 1 = arms and hands, 2 = legs, 3 =
                             // waist and head, 4 = mega dismemberment
  ...
  g_saberAutoBlocking = gi.cvar(
      "g_saberAutoBlocking", "1",
      CVAR_ARCHIVE | CVAR_CHEAT); // must press +block button to do any blocking
  g_saberRealisticCombat =
      gi.cvar("g_saberRealisticCombat", "0",
  ...
  g_saberAnimSpeed =
      gi.cvar("g_saberAnimSpeed", "1",
              CVAR_ARCHIVE | CVAR_CHEAT); // how fast saber animations run
  g_saberAutoAim =
      gi.cvar("g_saberAutoAim", "1",
              CVAR_ARCHIVE | CVAR_CHEAT); // auto-aims at enemies when not
                                          // moving or when just running forward
  ...
  g_iscensored = gi.cvar("ui_iscensored", "0",
                         CVAR_ARCHIVE | CVAR_ROM | CVAR_INIT | CVAR_CHEAT |
                             CVAR_NORESTART);
}
...
void InitGame(const char *mapname, const char *spawntarget, int checkSum,
  ...
  // FIXME: if this is from a loadgame, it needs to be sure to write this out
  // whenever you do a savegame since the edges and routes are dynamic...
  ...
  if (navCalculatePaths) { // not loaded - need to calc paths
    ...
    // nav-load/calc-paths
    navigator.pathsCalculated =
        qtrue; // just to be safe?  Does this get saved out?  No... assumed
    // need to do this, because combatpoint waypoints aren't saved out...?
    CP_FindCombatPointWaypoints();
    navCalcPathTime = 0;

    if (g_eSavedGameJustLoaded ==
        eNO) { // clear all the failed edges unless we just loaded the game
               // (which would include failed edges)
...
void G_RunThink(gentity_t *ent) {
  ...
  */
  ...
  ent->nextthink = 0;
  if (ent->e_ThinkFunc == thinkF_NULL) // actually you don't need this if I
