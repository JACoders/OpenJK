...
qboolean AI_ValidateGroupMember(AIGroupInfo_t *group, gentity_t *member) {
	...
	if (member->client->NPC_class == CLASS_ATST || member->client->NPC_class == CLASS_PROBE || member->client->NPC_class == CLASS_SEEKER ||
		...
		member->client->NPC_class == CLASS_MINEMONSTER || member->client->NPC_class == CLASS_HOWLER || member->client->NPC_class == CLASS_RANCOR ||
		member->client->NPC_class == CLASS_MARK1 || member->client->NPC_class == CLASS_MARK2) { // these kinds of enemies don't actually use this group AI
		return qfalse;
	}
