...
// these vars I moved here out of the level_locals_t struct simply because it's
// pointless to try saving them,
//	and the level_locals_t struct is included in the save process... -slc
//
qboolean spawning = qfalse; // the G_Spawn*() functions are valid  (only turned
                            // on during one function)
int numSpawnVars;
...
qboolean G_SpawnVector4(const char *key, const char *defaultString,
                        float *out) {
  char *s;
  qboolean present;
...
qboolean G_SpawnFlag(const char *key, int flag, int *out) {
  ...
  for (int i = 0; i < numSpawnVars; i++) {
    if (!strcmp(key, spawnVars[i][0])) {
      // found the key
      if (atoi(spawnVars[i][1]) != 0) { // if it's non-zero, and in the flag
        *out |= flag;
      } else { // if it's zero, or out the flag
        *out &= ~flag;
      }
      return qtrue;
    }
...
qboolean G_SpawnAngleHack(const char *key, const char *defaultString,
                          float *out) {
  char *s;
  qboolean present;
  float temp = 0;
...
field_t fields[] = {
    ...
    {"noBasicSounds", FOFS(classname),
     F_IGNORE}, // will be looked at separately
    {"noCombatSounds", FOFS(classname),
     F_IGNORE}, // will be looked at separately
    {"noExtraSounds", FOFS(classname),
     F_IGNORE}, // will be looked at separately
    ...
    // Script parms - will this handle clamping to 16 or whatever length of
    // parm[0] is?
    {"parm1", 0, F_PARM1},
    {"parm2", 0, F_PARM2},
    {"parm3", 0, F_PARM3},
    ...
    {"backwardstarget", FOFS(paintarget),
     F_LSTRING}, // for trigger_bidirectional
    {"splashDamage", FOFS(splashDamage), F_INT},
    {"splashRadius", FOFS(splashRadius), F_INT},
    // Script stuff
    {"spawnscript", FOFS(behaviorSet[BSET_SPAWN]),
     F_LSTRING}, // name of script to run
    {"usescript", FOFS(behaviorSet[BSET_USE]),
     F_LSTRING}, // name of script to run
    {"awakescript", FOFS(behaviorSet[BSET_AWAKE]),
     F_LSTRING}, // name of script to run
    {"angerscript", FOFS(behaviorSet[BSET_ANGER]),
     F_LSTRING}, // name of script to run
    {"attackscript", FOFS(behaviorSet[BSET_ATTACK]),
     F_LSTRING}, // name of script to run
    {"victoryscript", FOFS(behaviorSet[BSET_VICTORY]),
     F_LSTRING}, // name of script to run
    {"lostenemyscript", FOFS(behaviorSet[BSET_LOSTENEMY]),
     F_LSTRING}, // name of script to run
    {"painscript", FOFS(behaviorSet[BSET_PAIN]),
     F_LSTRING}, // name of script to run
    {"fleescript", FOFS(behaviorSet[BSET_FLEE]),
     F_LSTRING}, // name of script to run
    {"deathscript", FOFS(behaviorSet[BSET_DEATH]),
     F_LSTRING}, // name of script to run
    {"delayscript", FOFS(behaviorSet[BSET_DELAYED]),
     F_LSTRING},                                       // name of script to run
    {"delayscripttime", FOFS(delayScriptTime), F_INT}, // name of script to run
    {"blockedscript", FOFS(behaviorSet[BSET_BLOCKED]),
     F_LSTRING}, // name of script to run
    {"ffirescript", FOFS(behaviorSet[BSET_FFIRE]),
     F_LSTRING}, // name of script to run
    {"ffdeathscript", FOFS(behaviorSet[BSET_FFDEATH]),
     F_LSTRING}, // name of script to run
    {"mindtrickscript", FOFS(behaviorSet[BSET_MINDTRICK]),
     F_LSTRING}, // name of script to run
    {"script_targetname", FOFS(script_targetname),
     F_LSTRING}, // scripts look for this when "affecting"
    // For NPCs
    ...
    {"mass", FOFS(mass),
     F_FLOAT}, // really only used for pushable misc_model_breakables
...
spawn_t spawns[] = {
    ...
    // rwwFIXMEFIXME: Faked for testing NPCs (another other things) in RMG with
    // sof2 assets
  ...
  for (f = fields; f->name; f++) {
    if (!Q_stricmp(f->name, key)) {
      ...
      case F_VECTOR: {
        ...
        if (_iFieldsRead != 3) {
          gi.Printf(S_COLOR_YELLOW "G_ParseField: VEC3 sscanf() failed to read "
                                   "3 floats ('angle' key bug?)\n");
          delayedShutDown = level.time + 100;
        }
      ...
      case F_VECTOR4: {
        int _iFieldsRead = sscanf(value, "%f %f %f %f", &vec4[0], &vec4[1],
                                  &vec4[2], &vec4[3]);
      ...
      case F_PARM16:
        ...
        break;
      case F_FLAG: { // try to find the proper flag for that key:
...
static qboolean SpawnForCurrentDifficultySetting(gentity_t *ent) {
  ...
  if (ent->spawnflags &
      (1 << (8 + g_spskill->integer))) { // easy -256	medium -512		hard
                                         // -1024
    return qfalse;
  } else {
    return qtrue;
...
void G_SpawnGEntityFromSpawnVars(void) {
  ...
  if (Quake3Game()->ValidEntity(ent)) {
    ...
    if (ent->classname && ent->classname[0]) {
      if (Q_strncmp("NPC_", ent->classname, 4) != 0) { // Not an NPC_spawner
...
static const char *defaultStyles[LS_NUM_STYLES][3] = {
    ...
    {// 1 FLICKER (first variety)
     "mmnmmommommnonmmonqnmmo", "mmnmmommommnonmmonqnmmo",
    ...
    {// 3 CANDLE (first variety)
     "mmmmmaaaaammmmmaaaaaabcdefgabcdefg", "mmmmmaaaaammmmmaaaaaabcdefgabcdefg",
     "mmmmmaaaaammmmmaaaaaabcdefgabcdefg"},
    {// 4 FAST STROBE
     "mamamamamama", "mamamamamama", "mamamamamama"},
    {// 5 GENTLE PULSE 1
     "jklmnopqrstuvwxyzyxwvutsrqponmlkj", "jklmnopqrstuvwxyzyxwvutsrqponmlkj",
     "jklmnopqrstuvwxyzyxwvutsrqponmlkj"},
    {// 6 FLICKER (second variety)
     "nmonqnmomnmomomno", "nmonqnmomnmomomno", "nmonqnmomnmomomno"},
    {// 7 CANDLE (second variety)
     "mmmaaaabcdefgmmmmaaaammmaamm", "mmmaaaabcdefgmmmmaaaammmaamm",
    ...
    {// 10 FLUORESCENT FLICKER
     "mmamammmmammamamaaamammma", "mmamammmmammamamaaamammma",
    ...
    {// 13 Test Blending
     "abcdefghijklmqrstuvwxyz", "zyxwvutsrqmlkjihgfedcba",
...
"_minlight"   minimum lighting on a surf.  overrides _mingridlight and
_minvertexlight

Game Options
"fog"          shader name of the global fog texture - must include the full
path, such as "textures/rj/fog1" "ls_Xr"	override lightstyle X with this
pattern for Red. "ls_Xg"	green (valid patterns are "a-z") "ls_Xb"
blue (a is OFF, z is ON) "breath"		Whether the entity's have breath
puffs or not (0 = No, 1 = All, 2 = Just cold breath, 3 = Just under water
bubbles ). "clearstats" default 1, if 0 loading this map will not clear the
stats for player "tier_storyinfo" sets 'tier_storyinfo' cvar
*/
void SP_worldspawn(void) {
  ...
  for (i = 0; i < numSpawnVars; i++) {
    if (Q_stricmp("spawnscript", spawnVars[i][0]) ==
        0) { // ONly let them set spawnscript, we don't want them setting an
             // angle or something on the world.
      G_ParseField(spawnVars[i][0], spawnVars[i][1],
    ...
    if (Q_stricmp("distancecull", spawnVars[i][0]) == 0) {
      g_entities[ENTITYNUM_WORLD].max_health =
          (int)((float)(atoi(spawnVars[i][1])) * 0.7f);
    }
  }
  ...
  // Lightstyles
  gi.SetConfigstring(CS_LIGHT_STYLES + (LS_STYLES_START * 3) + 0,
                     defaultStyles[0][0]);
  gi.SetConfigstring(CS_LIGHT_STYLES + (LS_STYLES_START * 3) + 1,
                     defaultStyles[0][1]);
  gi.SetConfigstring(CS_LIGHT_STYLES + (LS_STYLES_START * 3) + 2,
                     defaultStyles[0][2]);
...
void G_SpawnEntitiesFromString(const char *entityString) {
  ...
  if (g_entities[ENTITYNUM_WORLD].behaviorSet[BSET_SPAWN] &&
      g_entities[ENTITYNUM_WORLD]
          .behaviorSet[BSET_SPAWN]
                      [0]) { // World has a spawn script, but we don't want the
                             // world in ICARUS and running scripts,
    // so make a scriptrunner and start it going.
    gentity_t *script_runner = G_Spawn();
    if (script_runner) {
      script_runner->behaviorSet[BSET_USE] =
          g_entities[ENTITYNUM_WORLD].behaviorSet[BSET_SPAWN];
      ...
      if (Quake3Game()->ValidEntity(script_runner)) {
        Quake3Game()->InitEntity(
            script_runner); // ICARUS_InitEnt( script_runner );
      }
    }
  }
