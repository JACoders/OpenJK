...
void CG_InitSiegeMode(void) {
	...
	while (i <= SIEGETEAM_TEAM2) {
		...
		while (j < sTeam->numClasses) {
			...
			if (cl->forcedModel[0]) { // This class has a forced model, so precache it.
...
static char QINLINE *CG_SiegeObjectiveBuffer(int team, int objective) {
	...
	if (BG_SiegeGetValueGroup(siege_info, teamstr, cgParseObjectives)) {				   // found the team group
		if (BG_SiegeGetValueGroup(cgParseObjectives, va("Objective%i", objective), buf)) { // found the objective group
			return buf;
		}
	}
...
void CG_ParseSiegeObjectiveStatus(const char *str) {
	...
	while (str[i]) {
		if (str[i] == '|') { // switch over to team2, this is the next section
			...
			if (s && s[0]) { // now set the description and graphic cvars to by read by the menu
				char buffer[8192];
	...
	if (cg.predictedPlayerState.persistant[PERS_TEAM] != TEAM_SPECTATOR) { // update menu cvars
...
void CG_SiegeRoundOver(centity_t *ent, int won) {
	...
	if (cg.snap) { // this should always be true, if it isn't though use the predicted ps as a fallback
		ps = &cg.snap->ps;
	} else {
...
void CG_SiegeGetObjectiveDescription(int team, int objective, char *buffer) {
	...
	if (BG_SiegeGetValueGroup(siege_info, teamstr, cgParseObjectives)) {							// found the team group
		if (BG_SiegeGetValueGroup(cgParseObjectives, va("Objective%i", objective), objectiveStr)) { // found the objective group
...
void CG_SiegeBriefingDisplay(int team, int dontshow) {
	...
	if (useTeam != SIEGETEAM_TEAM1 && useTeam != SIEGETEAM_TEAM2) { // This shouldn't be happening. But just fall back to team 2 anyway.
	...
	while (i < 16) { // do up to 16 objectives I suppose
		...
		} else { // didn't find one, so set the "inuse" cvar to 0 for the objective and mark it non-complete.
			trap->Cvar_Set(va("siege_objective%i_inuse", i), "0");
...
void CG_ParseSiegeExtendedDataEntry(const char *conStr) {
	...
	if (ammo >= 0 && ammo <= maxAmmo) { // assure the weapon number is valid and not over max
	...
	} else { // not valid? Oh well, just invalidate the weapon too then so we don't display ammo
