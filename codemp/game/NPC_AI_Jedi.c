...
qboolean Jedi_CultistDestroyer(gentity_t *self) {
	...
	if (self->client->NPC_class == CLASS_REBORN && self->s.weapon == WP_MELEE && !Q_stricmp("cultist_destroyer", self->NPC_type)) {
		return qtrue;
	}
	return qfalse;
}
...
void Boba_FireFlameThrower(gentity_t *self) {
	...
	trap->G2API_GetBoltMatrix(self->ghoul2, 0, self->client->renderInfo.handLBolt, &boltMatrix, self->r.currentAngles, self->r.currentOrigin, level.time, NULL,
							  self->modelScale);
...
void Boba_StartFlameThrower(gentity_t *self) {
	...
	*/
	...
	trap->G2API_GetBoltMatrix(NPCS.NPC->ghoul2, 0, NPCS.NPC->client->renderInfo.handRBolt, &boltMatrix, NPCS.NPC->r.currentAngles, NPCS.NPC->r.currentOrigin,
							  level.time, NULL, NPCS.NPC->modelScale);
...
void Boba_FireDecide(void) {
	...
	if (NPCS.NPC->client->ps.groundEntityNum == ENTITYNUM_NONE && NPCS.NPC->client->ps.fd.forceJumpZStart && !BG_FlippingAnim(NPCS.NPC->client->ps.legsAnim) &&
	...
	if ((enemyDist < (128 * 128) && enemyInFOV) || !TIMER_Done(NPCS.NPC, "flameTime")) { // flamethrower
	...
	{												// enemy within 128
		if ((NPCS.NPC->client->ps.weapon == WP_FLECHETTE || NPCS.NPC->client->ps.weapon == WP_REPEATER) &&
			(NPCS.NPCInfo->scriptFlags & SCF_ALT_FIRE)) { // shooting an explosive, but enemy too close, switch to primary fire
			NPCS.NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;
	...
	{
		if (NPCS.NPC->client->ps.weapon == WP_DISRUPTOR) {	   // sniping... should be assumed
	...
	if (TIMER_Done(NPCS.NPC, "nextAttackDelay") && TIMER_Done(NPCS.NPC, "flameTime")) {
		if (NPC_ClearLOS4(NPCS.NPC->enemy)) {
			...
			if (NPCS.NPC->client->ps.weapon == WP_NONE) {
					...
					enemyDist < MIN_ROCKET_DIST_SQUARED) // 128*128
				{
					...
					if (hit == NPCS.NPC->enemy->s.number || (hitEnt && hitEnt->client && hitEnt->client->playerTeam == NPCS.NPC->client->enemyTeam) ||
						(hitEnt && hitEnt->takedamage &&
						 ((hitEnt->r.svFlags & SVF_GLASS_BRUSH) || hitEnt->health < 40 ||
						  NPCS.NPC->s.weapon == WP_EMPLACED_GUN))) { // can hit enemy or enemy ally or will hit glass or other minor breakable (or in emplaced
																	 // gun), so shoot anyway
						enemyCS = qtrue;
						...
						if (hitEnt && hitEnt->client && hitEnt->client->playerTeam == NPCS.NPC->client->playerTeam) { // would hit an ally, don't fire!!!
							hitAlly = qtrue;
					...
					if (!Q_irand(0, 10)) {
						...
						if (dist < distThreshold) { // impact would be too close to me
							tooClose = qtrue;
								   ...
								   (NPCS.NPCInfo->group &&
									level.time - NPCS.NPCInfo->group->lastSeenEnemyTime > 5000)) { // we've haven't seen them in the last 5 seconds
		...
		if (NPCS.NPC->client->ps.weaponTime > 0) {
			...
			if (TIMER_Done(NPCS.NPC, "nextAttackDelay")) {
				...
				// NASTY
				if (NPCS.NPC->s.weapon == WP_ROCKET_LAUNCHER && (NPCS.ucmd.buttons & BUTTON_ATTACK) &&
...
void Jedi_CheckCloak(void) {
	if (NPCS.NPC && NPCS.NPC->client && NPCS.NPC->client->NPC_class == CLASS_SHADOWTROOPER) {
		if (!NPCS.NPC->client->ps.saberHolstered || NPCS.NPC->health <= 0 || NPCS.NPC->client->ps.saberInFlight ||
			//	(NPC->client->ps.eFlags&EF_FORCE_GRIPPED) ||
			//	(NPC->client->ps.eFlags&EF_FORCE_DRAINED) ||
			NPCS.NPC->painDebounceTime > level.time) { // can't be cloaked if saber is on, or dead or saber in flight or taking pain or being gripped
			Jedi_Decloak(NPCS.NPC);
		} else if (NPCS.NPC->health > 0 &&
				   !NPCS.NPC->client->ps.saberInFlight
				   //	&& !(NPC->client->ps.eFlags&EF_FORCE_GRIPPED)
				   //	&& !(NPC->client->ps.eFlags&EF_FORCE_DRAINED)
				   && NPCS.NPC->painDebounceTime < level.time) { // still alive, have saber in hand, not taking pain and not being gripped
			Jedi_Cloak(NPCS.NPC);
		}
	}
}
...
static void Jedi_AggressionErosion(int amt) {
	if (TIMER_Done(NPCS.NPC, "roamTime")) { // the longer we're not alerted and have no enemy, the more our aggression goes down
		TIMER_Set(NPCS.NPC, "roamTime", Q_irand(2000, 5000));
		Jedi_Aggression(NPCS.NPC, amt);
	}

	if (NPCS.NPCInfo->stats.aggression < 4 || (NPCS.NPCInfo->stats.aggression < 6 && NPCS.NPC->client->NPC_class == CLASS_DESANN)) { // turn off the saber
...
static qboolean Jedi_BattleTaunt(void) {
	if (TIMER_Done(NPCS.NPC, "chatter") && !Q_irand(0, 3) && NPCS.NPCInfo->blockedSpeechDebounceTime < level.time &&
		jediSpeechDebounceTime[NPCS.NPC->client->playerTeam] < level.time) {
		int event = -1;
		if (NPCS.NPC->client->playerTeam == NPCTEAM_PLAYER && NPCS.NPC->enemy && NPCS.NPC->enemy->client &&
			NPCS.NPC->enemy->client->NPC_class == CLASS_JEDI) {										 // a jedi fighting a jedi - training
			if (NPCS.NPC->client->NPC_class == CLASS_JEDI && NPCS.NPCInfo->rank == RANK_COMMANDER) { // only trainer taunts
				event = EV_TAUNT1;
			}
		...
		if (event != -1) {
			...
			if (NPCS.NPC->enemy && NPCS.NPC->enemy->NPC && NPCS.NPC->enemy->s.weapon == WP_SABER && NPCS.NPC->enemy->client &&
				NPCS.NPC->enemy->client->NPC_class == CLASS_JEDI) { // Have the enemy jedi say something in response when I'm done?
			}
			return qtrue;
		}
	}
...
static qboolean Jedi_ClearPathToSpot(vec3_t dest, int impactEntNum) {
	...
	for (i = NPCS.NPC->r.maxs[0] * 2; i < dist;
		 i += NPCS.NPC->r.maxs[0] * 2) { // FIXME: does this check the last spot, too?  We're assuming that should be okay since the enemy is there?
...
qboolean NPC_MoveDirClear(int forwardmove, int rightmove, qboolean reset) {
	...
	if (NPCS.ucmd.upmove > 0 || NPCS.NPC->client->ps.fd.forceJumpCharge) { // Going to jump
	...
	*/
	...
	if (trace.fraction < 0.6) { // Going to bump into something very close, don't move, just turn
		if ((NPCS.NPC->enemy && trace.entityNum == NPCS.NPC->enemy->s.number) ||
			(NPCS.NPCInfo->goalEntity && trace.entityNum == NPCS.NPCInfo->goalEntity->s.number)) { // okay to bump into enemy or goal
	...
	if (NPCS.NPCInfo->goalEntity) {
		if (NPCS.NPCInfo->goalEntity->r.currentOrigin[2] <
			NPCS.NPC->r.currentOrigin[2]) { // goal is below me, okay to step off at least that far plus stepheight
...
static void Jedi_CheckDecreaseSaberAnimLevel(void) {
	if (!NPCS.NPC->client->ps.weaponTime && !(NPCS.ucmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK))) { // not attacking
...
static void Jedi_CombatDistance(int enemy_dist) { // FIXME: for many of these checks, what we really want is horizontal distance to enemy
	if (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_GRIP) &&
		NPCS.NPC->client->ps.fd.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1) { // when gripping, don't move
		return;
	} else if (!TIMER_Done(NPCS.NPC, "gripping")) { // stopped gripping, clear timers just in case
	...
	if (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_DRAIN) &&
		NPCS.NPC->client->ps.fd.forcePowerLevel[FP_DRAIN] > FORCE_LEVEL_1) { // when draining, don't move
		return;
	} else if (!TIMER_Done(NPCS.NPC, "draining")) { // stopped draining, clear timers just in case
	...
	if (NPCS.NPC->client->NPC_class == CLASS_BOBAFETT) {
	...
	} else if (NPCS.NPC->client->ps.saberInFlight && !PM_SaberInBrokenParry(NPCS.NPC->client->ps.saberMove) &&
			   NPCS.NPC->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN) { // maintain distance
		...
		if (NPCS.NPC->client->ps.weapon == WP_SABER									  // using saber
			...
			&& !(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED)) && !(NPCS.NPC->client->ps.saberEventFlags & SEF_INWATER)) // saber not in water
		{																																// hold it out there
			NPCS.ucmd.buttons |= BUTTON_ALT_ATTACK;
			// FIXME: time limit?
		}
		...
		// else if ( NPC->client->ps.torsoAnim == BOTH_GESTURE1 && NPC->client->ps.torsoTimer < 2000 )
				 ...
				 (NPCS.NPC->client->ps.forceHandExtendTime - level.time) < 200) { // we're almost done with our special taunt
			// FIXME: this doesn't always work, for some reason
			if (!NPCS.NPC->client->ps.saberInFlight) {
				WP_ActivateSaber(NPCS.NPC);
			}
		}
	} else if (NPCS.NPC->client->ps.saberEventFlags & SEF_LOCK_WON) { // we won a saber lock, press the advantage
		...
		if (NPCS.NPC->enemy->painDebounceTime + 2000 < level.time) { // the window of opportunity is gone
			NPCS.NPC->client->ps.saberEventFlags &= ~SEF_LOCK_WON;
		}
		// don't strafe?
		TIMER_Set(NPCS.NPC, "strafeLeft", -1);
		TIMER_Set(NPCS.NPC, "strafeRight", -1);
	} else if (NPCS.NPC->enemy->client && NPCS.NPC->enemy->s.weapon == WP_SABER && NPCS.NPC->enemy->client->ps.saberLockTime > level.time &&
			   NPCS.NPC->client->ps.saberLockTime < level.time) { // enemy is in a saberLock and we are not
	...
	// rwwFIXMEFIXME: Give them the ability to do this again (turret needs to be fixed up to allow it)
	else if (enemy_dist <= 64 && ((NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE) ||
								  (!Q_stricmp("Yoda", NPCS.NPC->NPC_type) && !Q_irand(0, 10)))) { // can't use saber and they're in striking range
		if (!Q_irand(0, 5) && InFront(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles, 0.2f)) {
			if (((NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE) ||
				 NPCS.NPC->client->pers.maxHealth - NPCS.NPC->health > NPCS.NPC->client->pers.maxHealth * 0.25f) // lost over 1/4 of our health or not firing
		...
		Jedi_Retreat();
	} else if (enemy_dist <= 64 && NPCS.NPC->client->pers.maxHealth - NPCS.NPC->health > NPCS.NPC->client->pers.maxHealth * 0.25f // lost over 1/4 of our health
			   && NPCS.NPC->client->ps.fd.forcePowersKnown & (1 << FP_DRAIN)													  // know how to drain
			   && WP_ForcePowerAvailable(NPCS.NPC, FP_DRAIN, 20)																  // have enough power
			   && !Q_irand(0, 10) && InFront(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles, 0.2f)) {
		TIMER_Set(NPCS.NPC, "draining", 3000);
		TIMER_Set(NPCS.NPC, "attackDelay", 3000);
		Jedi_Advance();
		return;
		...
		if (NPCS.NPCInfo->stats.aggression < Q_irand(0, 20) && NPCS.NPC->health < NPCS.NPC->client->pers.maxHealth * 0.75f && !Q_irand(0, 2)) {
			if ((NPCS.NPC->client->ps.fd.forcePowersKnown & (1 << FP_HEAL)) != 0 && (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_HEAL)) == 0 &&
				Q_irand(0, 1)) {
				ForceHeal(NPCS.NPC);
				usedForce = qtrue;
				// FIXME: check level of heal and know not to move or attack when healing
					   ...
					   (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_PROTECT)) == 0 && Q_irand(0, 1)) {
				ForceProtect(NPCS.NPC);
				usedForce = qtrue;
			} else if ((NPCS.NPC->client->ps.fd.forcePowersKnown & (1 << FP_ABSORB)) != 0 &&
					   (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_ABSORB)) == 0 && Q_irand(0, 1)) {
				ForceAbsorb(NPCS.NPC);
				usedForce = qtrue;
			} else if ((NPCS.NPC->client->ps.fd.forcePowersKnown & (1 << FP_RAGE)) != 0 && (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_RAGE)) == 0 &&
					   Q_irand(0, 1)) {
				Jedi_Rage();
				usedForce = qtrue;
			}
	...
	{						  // we're out of striking range and we are allowed to attack
		...
		if (NPCS.NPC->enemy && NPCS.NPC->enemy->client &&
			(NPCS.NPC->enemy->client->ps.fd.forceGripBeingGripped > level.time)) {			// They're being gripped, rush them!
			if (NPCS.NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE) {			// they're on the ground, so advance
				if (TIMER_Done(NPCS.NPC, "parryTime") || NPCS.NPCInfo->rank > RANK_LT) {	// not parrying
					if (enemy_dist > 200 || !(NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE)) { // far away or allowed to use saber
						Jedi_Advance();
					}
				}
			}
			if (NPCS.NPCInfo->rank >= RANK_LT_JG && !Q_irand(0, 5) && !(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED)) &&
			...
			{														   // throw saber
				NPCS.ucmd.buttons |= BUTTON_ALT_ATTACK;
				   ...
				   /*NPC->enemy->client->ps.saber[0].Active()*/ NPCS.NPC->enemy->client->ps.saberEntityNum && // enemy throwing saber
		...
		{																	  // They're throwing their saber, grip them!
			...
			if (chanceScale &&
				...
				enemy_dist < 500 &&
				(Q_irand(0, chanceScale * 10) < 5 || (NPCS.NPC->enemy->client && NPCS.NPC->enemy->client->ps.weapon != WP_SABER &&
				...
				if (((NPCS.NPCInfo->rank == RANK_ENSIGN || NPCS.NPCInfo->rank > RANK_LT_JG) && !Q_irand(0, 1)) || NPCS.NPC->s.weapon != WP_SABER) {
					if (WP_ForcePowerUsable(NPCS.NPC, FP_PULL) && !Q_irand(0, 2)) {
						...
						if (Q_irand(0, 1)) {
							NPCS.ucmd.buttons |= BUTTON_ATTACK;
								...
								((NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp("cultist_lightning", NPCS.NPC->NPC_type))) ||
							   Q_irand(0, 1)) {
					...
					// rwwFIXMEFIXME: After new drain stuff from SP is in re-enable this.
							  ...
							  ((NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE) && Q_stricmp("cultist_drain", NPCS.NPC->NPC_type))) ||
							 Q_irand(0, 1)) {
						ForceDrain(NPCS.NPC);
						NPCS.NPC->client->ps.weaponTime = Q_irand(1000, 3000 + (g_npcspskill.integer * 500));
						TIMER_Set(NPCS.NPC, "draining", NPCS.NPC->client->ps.weaponTime);
						TIMER_Set(NPCS.NPC, "attackDelay", NPCS.NPC->client->ps.weaponTime);
					} else if (WP_ForcePowerUsable(NPCS.NPC, FP_GRIP) && NPCS.NPC->enemy && InFOV(NPCS.NPC->enemy, NPCS.NPC, 20, 30)) {
						// taunt
						if (TIMER_Done(NPCS.NPC, "chatter") && jediSpeechDebounceTime[NPCS.NPC->client->playerTeam] < level.time &&
							NPCS.NPCInfo->blockedSpeechDebounceTime < level.time) {
							G_AddVoiceEvent(NPCS.NPC, Q_irand(EV_TAUNT1, EV_TAUNT3), 3000);
						...
						if (WP_ForcePowerUsable(NPCS.NPC, FP_SABERTHROW) && !(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED)) &&
							!(NPCS.NPC->client->ps.saberEventFlags & SEF_INWATER)) // saber not in water
						{														   // throw saber
							NPCS.ucmd.buttons |= BUTTON_ALT_ATTACK;
						}
					...
					if (NPCS.NPCInfo->rank >= RANK_LT_JG && !(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED)) &&
						!(NPCS.NPC->client->ps.saberEventFlags & SEF_INWATER)) // saber not in water
					{														   // throw saber
						NPCS.ucmd.buttons |= BUTTON_ALT_ATTACK;
					}
			...
			// see if we should advance now
				...
				if (TIMER_Done(NPCS.NPC, "parryTime") || NPCS.NPCInfo->rank > RANK_LT) {							 // not parrying
					if (!NPCS.NPC->enemy->client || NPCS.NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE) { // they're on the ground, so advance
						if (enemy_dist > 200 || !(NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE)) {						 // far away or allowed to use saber
							Jedi_Advance();
						}
					}
		...
		if (NPCS.NPCInfo->stats.aggression < 4) { // back off and defend
			...
			if (enemy_dist > 0 && !(NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE)) {		 // we're allowed to use our lightsaber, get closer
				if (TIMER_Done(NPCS.NPC, "parryTime") || NPCS.NPCInfo->rank > RANK_LT) { // not parrying
					if (!NPCS.NPC->enemy->client || NPCS.NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE) { // they're on the ground, so advance
						Jedi_Advance();
					}
	...
	if (NPCS.NPCInfo->stats.aggression > Q_irand(5, 15) && NPCS.NPC->health < NPCS.NPC->client->pers.maxHealth * 0.75f && !Q_irand(0, 2)) {
		if ((NPCS.NPC->client->ps.fd.forcePowersKnown & (1 << FP_RAGE)) != 0 && (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_RAGE)) == 0) {
			Jedi_Rage();
		}
	}
}
...
static qboolean Jedi_Strafe(int strafeTimeMin, int strafeTimeMax, int nextStrafeTimeMin, int nextStrafeTimeMax, qboolean walking) {
	...
	if (NPCS.NPC->client->ps.saberEventFlags & SEF_LOCK_WON && NPCS.NPC->enemy &&
		NPCS.NPC->enemy->painDebounceTime > level.time) { // don't strafe if pressing the advantage of winning a saberLock
		return qfalse;
	}
...
int Jedi_ReCalcParryTime(gentity_t *self, evasionType_t evasionType) {
	...
	if (self->s.number >= 0 && self->s.number < MAX_CLIENTS) { // player
		...
		if (!g_saberRealisticCombat.integer && (g_npcspskill.integer == 2 || (g_npcspskill.integer == 1 && self->client->NPC_class == CLASS_TAVION))) {
...
qboolean Jedi_QuickReactions(gentity_t *self) {
	if ((self->client->NPC_class == CLASS_JEDI && NPCS.NPCInfo->rank == RANK_COMMANDER) || self->client->NPC_class == CLASS_TAVION ||
	...
	if (!incoming) {
		...
		if (self->client->ps.saberInFlight) { // DOH!  do non-saber evasion!
			saberBusy = qtrue;
					   ...
					   self)) { // jedi trainer and tavion are must faster at parrying and can do it whenever they like
...
static qboolean Jedi_SaberBlock(int saberNum, int bladeNum) // saberNum = 0, bladeNum = 0
{
	...
	*/
	...
	if (!TIMER_Done(NPCS.NPC, "parryReCalcTime")) { // can't do our own re-think of which parry to use yet
		return qfalse;
	}

	if (NPCS.NPC->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] > level.time) { // can't move the saber to another position yet
		return qfalse;
	}
	...
	*/
	...
	if (NPCS.NPC->enemy->health <= 0 || !NPCS.NPC->enemy->client) { // don't keep blocking him once he's dead (or if not a client)
		return qfalse;
	}
	/*
	...
	*/
	...
	VectorMA(NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzlePointOld, NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].length,
			 NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzleDirOld, saberTipOld);
	VectorMA(NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzlePoint, NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].length,
	...
	if ((evasionType = Jedi_SaberBlockGo(NPCS.NPC, &NPCS.ucmd, hitloc, dir, NULL, dist)) != EVASION_DODGE) { // we did block (not dodge)
		int parryReCalcTime;
		...
		if (TIMER_Done(NPCS.NPC, "parryTime")) {
			...
			} else if (NPCS.NPCInfo->rank >= RANK_LT_JG) { // fencers and higher hold a parry less
		...
		if (NPCS.NPCInfo->rank > RANK_LT_COMM && NPCS.NPC->client->NPC_class != CLASS_DESANN) { // higher-level guys can dodge faster
			dodgeTime -= 200;
		}
...
static void Jedi_EvasionSaber(vec3_t enemy_movedir, float enemy_dist, vec3_t enemy_dir) {
	...
	if (!NPCS.NPC->enemy->client) {
		return;
			   ...
			   NPCS.NPC->enemy->client->ps.saberLockTime > level.time) { // don't try to block/evade an enemy who is in a saberLock
		return;
	} else if (NPCS.NPC->client->ps.saberEventFlags & SEF_LOCK_WON &&
			   NPCS.NPC->enemy->painDebounceTime > level.time) { // pressing the advantage of winning a saber lock
		return;
	}

	if (NPCS.NPC->enemy->client->ps.saberInFlight && !TIMER_Done(NPCS.NPC, "taunting")) { // if he's throwing his saber, stop taunting
		TIMER_Set(NPCS.NPC, "taunting", -level.time);
		if (!NPCS.NPC->client->ps.saberInFlight) {
			WP_ActivateSaber(NPCS.NPC);
		}
	...
	if (TIMER_Done(NPCS.NPC, "parryTime")) {
		if (NPCS.NPC->client->ps.saberBlocked != BLOCKED_ATK_BOUNCE && NPCS.NPC->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN) { // wasn't blocked myself
	...
	if (NPCS.NPC->enemy->client->ps.weaponTime && NPCS.NPC->enemy->client->ps.weaponstate == WEAPON_FIRING) { // enemy is attacking
		enemy_attacking = qtrue;
		evasionChance = 90;
	}

	if ((NPCS.NPC->enemy->client->ps.fd.forcePowersActive & (1 << FP_LIGHTNING))) { // enemy is shooting lightning
		enemy_attacking = qtrue;
		shooting_lightning = qtrue;
		evasionChance = 50;
	}

	if (NPCS.NPC->enemy->client->ps.saberInFlight && NPCS.NPC->enemy->client->ps.saberEntityNum != ENTITYNUM_NONE &&
		NPCS.NPC->enemy->client->ps.saberEntityState != SES_RETURNING) { // enemy is shooting lightning
		enemy_attacking = qtrue;
	...
	if (Q_irand(0, 100) < evasionChance) { // check to see if he's coming at me
		...
		if (flrand(0.25, 1) < facingAmt) { // coming at/facing me!
			int whichDefense = 0;
			if (NPCS.NPC->client->ps.weaponTime || NPCS.NPC->client->ps.saberInFlight ||
				NPCS.NPC->client->NPC_class == CLASS_BOBAFETT) { // I'm attacking or recovering from a parry, can only try to strafe/jump right now
				...
				if (shooting_lightning) { // check for lightning attack
					...
					if (DotProduct(saberMoveDir, saberDir2Me) > 0.5) { // it's heading towards me
				...
				if (whichDefense) {									 // already chose one
					...
					if (DotProduct(enemy_dir, fwd) < 0.5) { // I'm not really facing him, best option is to strafe
			...
			if (whichDefense >= 4 && whichDefense <= 12) { // would try to block
				if (NPCS.NPC->client->ps.saberInFlight) {  // can't, saber in not in hand, so fall back to strafe/jump
					whichDefense = 100;
				}
			}
			...
			case 3:
				...
				if ((NPCS.NPCInfo->rank == RANK_ENSIGN || NPCS.NPCInfo->rank > RANK_LT_JG) &&
					TIMER_Done(NPCS.NPC, "parryTime")) { // FIXME: check forcePushRadius[NPC->client->ps.fd.forcePowerLevel[FP_PUSH]]
					ForceThrow(NPCS.NPC, qfalse);
				}
				break;
			case 4:
				...
				if (!Q_irand(0, 5) || !Jedi_Strafe(300, 1000, 0, 1000, qfalse)) { // certain chance they will pick an alternative evasion
					...
					if (shooting_lightning || throwing_saber || enemy_dist < 80) {
						...
						if (shooting_lightning || (!Q_irand(0, 2) && NPCS.NPCInfo->stats.aggression < 4 && TIMER_Done(NPCS.NPC, "parryTime"))) {
							if ((NPCS.NPCInfo->rank == RANK_ENSIGN || NPCS.NPCInfo->rank > RANK_LT_JG) && !shooting_lightning &&
								...
								ForceThrow(NPCS.NPC, qfalse);
									   ...
									   !(NPCS.NPCInfo->scriptFlags & SCF_NO_ACROBATICS) && NPCS.NPC->client->ps.fd.forceRageRecoveryTime < level.time &&
									   !(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_RAGE)) &&
									   !PM_InKnockDown(&NPCS.NPC->client->ps)) { // FIXME: make this a function call?
					...
					if (!(NPCS.NPCInfo->scriptFlags & SCF_NO_ACROBATICS) && NPCS.NPC->client->ps.fd.forceRageRecoveryTime < level.time &&
						!(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_RAGE)) &&
						(NPCS.NPCInfo->rank == RANK_CREWMAN || NPCS.NPCInfo->rank > RANK_LT_JG) && !PM_InKnockDown(&NPCS.NPC->client->ps) &&
...
static void Jedi_SetEnemyInfo(vec3_t enemy_dest, vec3_t enemy_dir, float *enemy_dist, vec3_t enemy_movedir, float *enemy_movespeed, int prediction) {
	...
	if (!NPCS.NPC->enemy->client) {
		...
		*enemy_movespeed = VectorNormalize(enemy_movedir);
		...
		// FIXME: enemy_dist calc needs to include all blade lengths, and include distance from hand to start of blade....
		*enemy_dist =
			VectorNormalize(enemy_dir) - (NPCS.NPC->client->saber[0].blade[0].lengthMax + NPCS.NPC->r.maxs[0] * 1.5 + 16); // just use the blade 0 len I guess
...
static void Jedi_FaceEnemy(qboolean doPitch) {
	...
	if (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_GRIP) && NPCS.NPC->client->ps.fd.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1) { // don't update?
	...
	if (NPCS.NPC->client->NPC_class == CLASS_BOBAFETT && TIMER_Done(NPCS.NPC, "flameTime") && NPCS.NPC->s.weapon != WP_NONE &&
		NPCS.NPC->s.weapon != WP_DISRUPTOR && (NPCS.NPC->s.weapon != WP_ROCKET_LAUNCHER || !(NPCS.NPCInfo->scriptFlags & SCF_ALT_FIRE)) &&
		NPCS.NPC->s.weapon != WP_THERMAL && NPCS.NPC->s.weapon != WP_TRIP_MINE && NPCS.NPC->s.weapon != WP_DET_PACK && NPCS.NPC->s.weapon != WP_STUN_BATON
	...
	// Find the desired angles
	if (!NPCS.NPC->client->ps.saberInFlight && (NPCS.NPC->client->ps.legsAnim == BOTH_A2_STABBACK1 || NPCS.NPC->client->ps.legsAnim == BOTH_CROUCHATTACKBACK1 ||
...
static void Jedi_DebounceDirectionChanges(void) {
	...
	if (NPCS.ucmd.forwardmove > 0) {
		...
		} else if (TIMER_Done(NPCS.NPC, "moveforward")) { // FIXME: should be if it's zero?
		...
		} else if (TIMER_Done(NPCS.NPC, "moveback")) { // FIXME: should be if it's zero?
			TIMER_Set(NPCS.NPC, "moveback", Q_irand(250, 1000));
		}
	} else if (!TIMER_Done(NPCS.NPC, "moveforward")) { // NOTE: edge checking should stop me if this is bad... but what if it sends us colliding into the enemy?
		NPCS.ucmd.forwardmove = 127;
		VectorClear(NPCS.NPC->client->ps.moveDir);
	} else if (!TIMER_Done(NPCS.NPC, "moveback")) { // NOTE: edge checking should stop me if this is bad...
		NPCS.ucmd.forwardmove = -127;
		VectorClear(NPCS.NPC->client->ps.moveDir);
	}
	...
	if (NPCS.ucmd.rightmove > 0) {
		...
		} else if (TIMER_Done(NPCS.NPC, "moveright")) { // FIXME: should be if it's zero?
		...
		} else if (TIMER_Done(NPCS.NPC, "moveleft")) { // FIXME: should be if it's zero?
			TIMER_Set(NPCS.NPC, "moveleft", Q_irand(250, 1500));
		}
	} else if (!TIMER_Done(NPCS.NPC, "moveright")) { // NOTE: edge checking should stop me if this is bad...
		NPCS.ucmd.rightmove = 127;
		VectorClear(NPCS.NPC->client->ps.moveDir);
	} else if (!TIMER_Done(NPCS.NPC, "moveleft")) { // NOTE: edge checking should stop me if this is bad...
		NPCS.ucmd.rightmove = -127;
		VectorClear(NPCS.NPC->client->ps.moveDir);
	}
}
...
static void Jedi_TimersApply(void) {
	if (!NPCS.ucmd.rightmove) { // only if not already strafing
		...
		if (!TIMER_Done(NPCS.NPC, "strafeLeft")) {
			if (NPCS.NPCInfo->desiredYaw > NPCS.NPC->client->ps.viewangles[YAW] + 60) { // we want to turn left, don't apply the strafing
			} else {																	// go ahead and strafe left
				NPCS.ucmd.rightmove = -127;
				VectorClear(NPCS.NPC->client->ps.moveDir);
			}
		} else if (!TIMER_Done(NPCS.NPC, "strafeRight")) {
			if (NPCS.NPCInfo->desiredYaw < NPCS.NPC->client->ps.viewangles[YAW] - 60) { // we want to turn right, don't apply the strafing
			} else {																	// go ahead and strafe left
				NPCS.ucmd.rightmove = 127;
				VectorClear(NPCS.NPC->client->ps.moveDir);
			}
	...
	if (!TIMER_Done(NPCS.NPC, "gripping")) { // FIXME: what do we do if we ran out of power?  NPC's can't?
		// FIXME: don't keep turning to face enemy or we'll end up spinning around
		NPCS.ucmd.buttons |= BUTTON_FORCEGRIP;
	}

	if (!TIMER_Done(NPCS.NPC, "draining")) { // FIXME: what do we do if we ran out of power?  NPC's can't?
		// FIXME: don't keep turning to face enemy or we'll end up spinning around
...
static void Jedi_CombatTimersUpdate(int enemy_dist) {
	...
	if (TIMER_Done(NPCS.NPC, "roamTime")) {
		...
		} else if (NPCS.NPC->client->ps.fd.forceRageRecoveryTime > level.time) { // recovering
		...
		if (NPCS.NPC->enemy && NPCS.NPC->enemy->client) {
			...
			case WP_SABER:
				...
				if (BG_SabersOff(&NPCS.NPC->enemy->client->ps)) { // fool!  Standing around unarmed, charge!
			...
			case WP_ROCKET_LAUNCHER:
				...
				if (NPCS.NPC->enemy->attackDebounceTime < level.time) { // does this apply to players?
	...
	if (NPCS.NPC->client->ps.saberEventFlags) { // some kind of saber combat event is still pending
		...
		if (NPCS.NPC->client->ps.saberEventFlags & SEF_PARRIED) { // parried
			...
			*/
			if (NPCS.NPC->enemy && PM_SaberInKnockaway(NPCS.NPC->enemy->client->ps.saberMove)) {   // advance!
		...
		{						// we hit our enemy last time we swung, drop our aggression
			if (!Q_irand(0, 1)) // FIXME: dependant on rank/diff?
			{
				...
				if (!Q_irand(0, 3) && NPCS.NPCInfo->blockedSpeechDebounceTime < level.time &&
					jediSpeechDebounceTime[NPCS.NPC->client->playerTeam] < level.time && NPCS.NPC->painDebounceTime < level.time - 1000) {
		...
		if ((NPCS.NPC->client->ps.saberEventFlags & SEF_BLOCKED)) { // was blocked whilst attacking
		...
		if (NPCS.NPC->client->ps.saberEventFlags & SEF_DEFLECTED) { // deflected a shot
		...
		if (NPCS.NPC->client->ps.saberEventFlags & SEF_HITOBJECT) { // hit some other damagable object
			if (!Q_irand(0, 3)) {
				Jedi_AdjustSaberAnimLevel(NPCS.NPC, (NPCS.NPC->client->ps.fd.saberAnimLevel - 1));
			}
...
static void Jedi_CombatIdle(int enemy_dist) {
	...
	if (NPCS.NPC->client->ps.saberInFlight) { // don't do this idle stuff if throwing saber
		return;
	}
	if (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_RAGE) ||
		NPCS.NPC->client->ps.fd.forceRageRecoveryTime > level.time) { // never taunt while raging or recovering from rage
		return;
	}
	...
	if (enemy_dist >= 64) { // FIXME: only do this if standing still?
		...
		if (Q_irand(2, chance) < NPCS.NPCInfo->stats.aggression) {
			if (TIMER_Done(NPCS.NPC, "chatter") && NPCS.NPC->client->ps.forceHandExtend == HANDEXTEND_NONE) { // FIXME: add more taunt behaviors
				// FIXME: sometimes he turns it off, then turns it right back on again???
				if (enemy_dist > 200 && NPCS.NPC->client->NPC_class != CLASS_BOBAFETT && !NPCS.NPC->client->ps.saberHolstered &&
...
static qboolean Jedi_AttackDecide(int enemy_dist) {
	...
	if (NPCS.NPC->enemy->client && NPCS.NPC->enemy->s.weapon == WP_SABER && NPCS.NPC->enemy->client->ps.saberLockTime > level.time &&
		NPCS.NPC->client->ps.saberLockTime < level.time) { // enemy is in a saberLock and we are not
		return qfalse;
	}

	if (NPCS.NPC->client->ps.saberEventFlags & SEF_LOCK_WON) { // we won a saber lock, press the advantage with an attack!
		int chance = 0;
		if (NPCS.NPC->client->NPC_class == CLASS_DESANN || NPCS.NPC->client->NPC_class == CLASS_LUKE ||
			!Q_stricmp("Yoda", NPCS.NPC->NPC_type)) { // desann and luke
			chance = 20;
		} else if (NPCS.NPC->client->NPC_class == CLASS_TAVION) { // tavion
			chance = 10;
		} else if (NPCS.NPC->client->NPC_class == CLASS_REBORN && NPCS.NPCInfo->rank == RANK_LT_JG) { // fencer
			chance = 5;
		} else {
	...
	if (NPCS.NPC->client->NPC_class == CLASS_TAVION || (NPCS.NPC->client->NPC_class == CLASS_REBORN && NPCS.NPCInfo->rank == RANK_LT_JG) ||
		(NPCS.NPC->client->NPC_class == CLASS_JEDI &&
		 NPCS.NPCInfo->rank == RANK_COMMANDER)) { // tavion, fencers, jedi trainer are all good at following up a parry with an attack
		if ((PM_SaberInParry(NPCS.NPC->client->ps.saberMove) || PM_SaberInKnockaway(NPCS.NPC->client->ps.saberMove)) &&
			NPCS.NPC->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN) { // try to attack straight from a parry
			NPCS.NPC->client->ps.weaponTime = NPCS.NPCInfo->shotTime = NPCS.NPC->attackDebounceTime = 0;
			// NPC->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] = level.time + 500;
			NPCS.NPC->client->ps.saberBlocked = BLOCKED_NONE;
	...
	if (!(NPCS.ucmd.buttons & BUTTON_ATTACK) && !(NPCS.ucmd.buttons & BUTTON_ALT_ATTACK)) { // not already attacking
		// Try to attack
		WeaponThink(qtrue);
	}
...
static qboolean Jedi_Jump(vec3_t dest, int goalEntNum) { // FIXME: if land on enemy, knock him down & jump off again
	/*
	...
	if (1) {
		...
		while (hitCount < maxHits) {
			...
			{	   // do a rough trace of the path
				...
				// This may be kind of wasteful, especially on long throws... use larger steps?  Divide the travelTime into a certain hard number of slices?
				...
				for (elapsedTime = timeStep; elapsedTime < floor(travelTime) + timeStep; elapsedTime += timeStep) {
					...
					if (testPos[2] < lastPos[2]) { // going down, ignore botclip
						...
						trap->Trace(&trace, lastPos, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, testPos, NPCS.NPC->s.number, NPCS.NPC->clipmask | CONTENTS_BOTCLIP,
									qfalse, 0, 0);
					}
...
static qboolean Jedi_TryJump(gentity_t *goal) { // FIXME: never does a simple short, regular jump...
	...
	if (TIMER_Done(NPCS.NPC, "jumpChaseDebounce")) {
		if ((!goal->client || goal->client->ps.groundEntityNum != ENTITYNUM_NONE)) {
			if (!PM_InKnockDown(&NPCS.NPC->client->ps) && !BG_InRoll(&NPCS.NPC->client->ps, NPCS.NPC->client->ps.legsAnim)) { // enemy is on terra firma
				vec3_t goal_diff;
				float goal_z_diff;
				float goal_xy_dist;
																		   ...
																		   // && (NPC->health >= 100 || goal_z_diff > -128 ))//closer than @512
				{
					...
					if (NPCS.NPC->health < 150 &&
						((NPCS.NPC->health < 30 && goal_z_diff < 0) || goal_z_diff < -128)) { // don't jump, just walk off... doesn't help with ledges, though
						debounce = qtrue;
							...
							if (Jedi_Jump(dest, goal->s.number)) {
								...
								*/
								{ // FIXME: make this a function call
									...
									if (NPCS.NPC->client->NPC_class == CLASS_BOBAFETT ||
										(NPCS.NPCInfo->rank != RANK_CREWMAN && NPCS.NPCInfo->rank <= RANK_LT_JG)) { // can't do acrobatics
...
static qboolean Jedi_Jumping(gentity_t *goal) {
	if (!TIMER_Done(NPCS.NPC, "forceJumpChasing") && goal) { // force-jumping at the enemy
...
static void Jedi_CheckEnemyMovement(float enemy_dist) {
	...
	if (NPCS.NPC->client->NPC_class != CLASS_TAVION && NPCS.NPC->client->NPC_class != CLASS_DESANN && NPCS.NPC->client->NPC_class != CLASS_LUKE &&
		Q_stricmp("Yoda", NPCS.NPC->NPC_type)) {
		if (NPCS.NPC->enemy->enemy && NPCS.NPC->enemy->enemy == NPCS.NPC) { // enemy is mad at *me*
			if (NPCS.NPC->enemy->client->ps.legsAnim == BOTH_JUMPFLIPSLASHDOWN1 ||
				NPCS.NPC->enemy->client->ps.legsAnim == BOTH_JUMPFLIPSTABDOWN) { // enemy is flipping over me
				if (Q_irand(0, NPCS.NPCInfo->rank) < RANK_LT) {					 // be nice and stand still for him...
			...
			} else if (NPCS.NPC->enemy->client->ps.legsAnim == BOTH_WALL_FLIP_BACK1 || NPCS.NPC->enemy->client->ps.legsAnim == BOTH_WALL_FLIP_RIGHT ||
					   NPCS.NPC->enemy->client->ps.legsAnim == BOTH_WALL_FLIP_LEFT || NPCS.NPC->enemy->client->ps.legsAnim == BOTH_WALL_RUN_LEFT_FLIP ||
					   NPCS.NPC->enemy->client->ps.legsAnim == BOTH_WALL_RUN_RIGHT_FLIP) { // he's flipping off a wall
				if (NPCS.NPC->enemy->client->ps.groundEntityNum == ENTITYNUM_NONE) {	   // still in air
			...
			} else if (NPCS.NPC->enemy->client->ps.legsAnim == BOTH_A2_STABBACK1) { // he's stabbing backwards
				if (enemy_dist < 256 && enemy_dist > 64) {							// close
					if (!InFront(NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->enemy->r.currentAngles, 0.0f)) { // behind him
						if (!Q_irand(0, NPCS.NPCInfo->rank)) { // be nice and stand still for him...
							vec3_t enemyFwd, dest, dir;
...
static void Jedi_CheckJumps(void) {
	...
	for (elapsedTime = 500; elapsedTime <= 4000; elapsedTime += 500) {
		...
		if (testPos[2] < lastPos[2]) { // going down, don't check for BOTCLIP
			trap->Trace(&trace, lastPos, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, testPos, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0,
	...
	VectorCopy(trace.endpos, bottom);
	if (bottom[2] > NPCS.NPC->r.currentOrigin[2]) { // only care about dist down from current height or lower
		bottom[2] = NPCS.NPC->r.currentOrigin[2];
	} else if (NPCS.NPC->r.currentOrigin[2] - bottom[2] > 400) { // whoa, long drop, don't do it!
...
static void Jedi_Combat(void) {
	...
	if (Jedi_Jumping(NPCS.NPC->enemy)) { // I'm in the middle of a jump, so just see if I should attack
		Jedi_AttackDecide(enemy_dist);
		return;
	}

	if (!(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_GRIP)) || NPCS.NPC->client->ps.fd.forcePowerLevel[FP_GRIP] < FORCE_LEVEL_2) { // not gripping
		// If we can't get straight at him
		if (!Jedi_ClearPathToSpot(enemy_dest, NPCS.NPC->enemy->s.number)) { // hunt him down
				...
				NPC_FaceEnemy(qtrue)) //( NPCInfo->rank == RANK_CREWMAN || NPCInfo->rank > RANK_LT_JG ) &&
			{
				...
				*/
				...
				if (Jedi_TryJump(NPCS.NPC->enemy)) { // FIXME: what about jumping to his enemyLastSeenLocation?
					return;
				}
			}
			...
			if (TIMER_Done(NPCS.NPC, "parryTime")) { // finished parrying
				if (NPCS.NPC->client->ps.saberBlocked != BLOCKED_ATK_BOUNCE &&
					NPCS.NPC->client->ps.saberBlocked != BLOCKED_PARRY_BROKEN) { // wasn't blocked myself
					NPCS.NPC->client->ps.saberBlocked = BLOCKED_NONE;
				}
			}
			...
			*/
				...
				if (NPCS.NPCInfo->aiFlags & NPCAI_BLOCKED) { // try to jump to the blockedDest
	...
	if (!enemy_lost) {
		...
		if (!NPCS.NPC->enemy->client ||
			(NPCS.NPC->enemy->client->ps.groundEntityNum != ENTITYNUM_NONE && NPCS.NPC->client->ps.groundEntityNum != ENTITYNUM_NONE)) {
			VectorCopy(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPCInfo->enemyLastSeenLocation);
		}
		NPCS.NPCInfo->enemyLastSeenTime = level.time;
	}
	...
	if (!NPCS.NPC->client->ps.saberInFlight && (!(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_GRIP)) ||
												NPCS.NPC->client->ps.fd.forcePowerLevel[FP_GRIP] < FORCE_LEVEL_2)) { // not throwing saber or using force grip
	...
	if (!NPC_MoveDirClear(NPCS.ucmd.forwardmove, NPCS.ucmd.rightmove, qtrue)) { // uh-oh, we are going to fall or hit something
		navInfo_t info;
		// Get the move info
		NAV_GetLastMove(&info);
	...
	if (level.alertEvents[alertEvent].level >= AEL_DANGER) { // run away!
		if (!level.alertEvents[alertEvent].owner || !level.alertEvents[alertEvent].owner->client ||
			(level.alertEvents[alertEvent].owner != NPCS.NPC &&
			 level.alertEvents[alertEvent].owner->client->playerTeam != NPCS.NPC->client->playerTeam)) { // no owner
			return qfalse;
		}
...
qboolean Jedi_CheckAmbushPlayer(void) {
	...
	for (i = 0; i < MAX_CLIENTS; i++) {
		...
		if (NPCS.NPC->client->ps.powerups[PW_CLOAKED] ||
			!NPC_SomeoneLookingAtMe(NPCS.NPC)) // rwwFIXMEFIXME: Need to pay attention to who is under crosshair for each player or something.
		{									   // if I'm not cloaked and the player's crosshair is on me, I will wake up, otherwise do this stuff down here...
			if (!trap->InPVS(player->r.currentOrigin, NPCS.NPC->r.currentOrigin)) { // must be in same room
				continue;
			} else {
				if (!NPCS.NPC->client->ps.powerups[PW_CLOAKED]) {
			...
			if ((target_dist = DistanceHorizontalSquared(player->r.currentOrigin, NPCS.NPC->r.currentOrigin)) > 4096) { // closer than 64 - always ambush
	...
	{															 // look for enemies
		...
		for (i = 0; i < ENTITYNUM_WORLD; i++) {
			...
			if (enemy && enemy->client && NPC_ValidEnemy(enemy) && enemy->client->playerTeam == NPCS.NPC->client->enemyTeam) {
				if (trap->InPVS(NPCS.NPC->r.currentOrigin, enemy->r.currentOrigin)) { // we could potentially see him
			...
			if (!best_enemy) {
				...
				if (NPC_ClearLOS4(best_enemy)) { // we have a clear (of architecture) LOS to him
					if (best_enemy->s.number) {	 // just attack
						...
						NPCS.NPCInfo->stats.aggression = 3;
					} else if (NPCS.NPC->client->NPC_class != CLASS_BOBAFETT) { // the player, toy with him
						// get progressively more interested over time
						if (TIMER_Done(NPCS.NPC, "watchTime")) {		  // we want to pick him up in stages
							if (TIMER_Get(NPCS.NPC, "watchTime") == -1) { // this is the first time, we'll ignore him for a couple seconds
						...
						if (best_enemy_dist < (440 * 440) || NPCS.NPCInfo->investigateCount >= 2) { // stage three: keep facing him
						...
						} else if (best_enemy_dist < (550 * 550) || NPCS.NPCInfo->investigateCount == 1) { // stage two: stop and face him every now and then
				...
				} else if (TIMER_Done(NPCS.NPC, "watchTime")) { // haven't seen him in a bit, clear the lookTarget
...
void NPC_BSJedi_FollowLeader(void) {
	...
	if (NPCS.NPC->client->ps.saberInFlight) {																 // saber is not in hand
		...
		{																									 //
			if (g_entities[NPCS.NPC->client->ps.saberEntityNum].s.pos.trType == TR_STATIONARY) {			 // fell to the ground, try to pick it up...
				if (Jedi_CanPullBackSaber(NPCS.NPC)) {
					...
					NPCS.ucmd.buttons |= BUTTON_ATTACK;
					if (NPCS.NPC->enemy && NPCS.NPC->enemy->health > 0) { // get our saber back NOW!
	...
	if (NPCS.NPCInfo->goalEntity) {
		...
		if (!NAV_CheckAhead(NPCS.NPC, NPCS.NPCInfo->goalEntity->r.currentOrigin, &trace,
							(NPCS.NPC->clipmask & ~CONTENTS_BODY) | CONTENTS_BOTCLIP)) {					  // can't get straight to him
			if (NPC_ClearLOS4(NPCS.NPCInfo->goalEntity) && NPC_FaceEntity(NPCS.NPCInfo->goalEntity, qtrue)) { // no line of sight
...
static void Jedi_Attack(void) {
	...
	if (NPCS.NPC->client->ps.saberLockTime > level.time) {
		...
		if (NPCS.NPC->client->ps.fd.forcePowerLevel[FP_PUSH] > FORCE_LEVEL_2 && NPCS.NPC->client->ps.saberLockTime < level.time + 5000 && !Q_irand(0, 10)) {
			ForceThrow(NPCS.NPC, qfalse);
		}
			...
			{	   // fell to the ground, try to pick it up
				...
				if (1) // no matter
				{
					...
					if (NPCS.NPC->enemy && NPCS.NPC->enemy->health > 0) { // get our saber back NOW!
						...
						NPC_UpdateAngles(qtrue, qtrue);
						if (NPCS.NPC->enemy->s.weapon == WP_SABER) { // be sure to continue evasion
							vec3_t enemy_dir, enemy_movedir, enemy_dest;
							float enemy_dist, enemy_movespeed;
							Jedi_SetEnemyInfo(enemy_dest, enemy_dir, &enemy_dist, enemy_movedir, &enemy_movespeed, 300);
							Jedi_EvasionSaber(enemy_movedir, enemy_dist, enemy_dir);
		...
		{											  // my enemy is dead and I killed him
			...
			if (NPCS.NPC->client->NPC_class == CLASS_BOBAFETT) {
				...
				if (!TIMER_Done(NPCS.NPC, "gloatTime")) {
					if (DistanceHorizontalSquared(NPCS.NPC->client->renderInfo.eyePoint, NPCS.NPC->enemy->r.currentOrigin) > 4096 &&
				...
				if (!NPCS.NPC->client->ps.saberHolstered &&
					NPCS.NPC->client->ps.saberInFlight) { // saber is still on (or we're trying to pull it back), count down erosion and keep facing the enemy
					...
					if (BG_SabersOff(&NPCS.NPC->client->ps) && !NPCS.NPC->client->ps.saberInFlight) { // turned off saber (in hand), gloat
						G_AddVoiceEvent(NPCS.NPC, Q_irand(EV_VICTORY1, EV_VICTORY3), 3000);
						jediSpeechDebounceTime[NPCS.NPC->client->playerTeam] = level.time + 3000;
						NPCS.NPCInfo->desiredPitch = 0;
						NPCS.NPCInfo->goalEntity = NULL;
				...
				if (!NPCS.NPC->client->ps.saberHolstered || NPCS.NPC->client->ps.saberInFlight || !TIMER_Done(NPCS.NPC, "gloatTime")) { // keep walking
					if (DistanceHorizontalSquared(NPCS.NPC->client->renderInfo.eyePoint, NPCS.NPC->enemy->r.currentOrigin) > 4096 &&
						(NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) // 64 squared
					{
						NPCS.NPCInfo->goalEntity = NPCS.NPC->enemy;
						...
						if (NPCS.NPC->health < NPCS.NPC->client->pers.maxHealth && (NPCS.NPC->client->ps.fd.forcePowersKnown & (1 << FP_HEAL)) != 0 &&
	...
	if (!NPCS.NPC->enemy) {
		NPCS.NPC->client->ps.saberBlocked = BLOCKED_NONE;
		if (NPCS.NPCInfo->tempBehavior == BS_HUNT_AND_KILL) { // lost him, go back to what we were doing before
	...
	if (!(NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) ||
		((NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_HEAL)) &&
		 NPCS.NPC->client->ps.fd.forcePowerLevel[FP_HEAL] < FORCE_LEVEL_2)) { // this is really stupid, but okay...
		NPCS.ucmd.forwardmove = 0;
		NPCS.ucmd.rightmove = 0;
	...
	// NOTE: for now, we clear ucmd.forwardmove & ucmd.rightmove while in air to avoid jumps going awry...
	if (NPCS.NPC->client->ps.groundEntityNum == ENTITYNUM_NONE) { // don't push while in air, throws off jumps!
	...
	if (NPCS.NPC->client->NPC_class != CLASS_BOBAFETT) {
		if (PM_SaberInBrokenParry(NPCS.NPC->client->ps.saberMove) ||
			NPCS.NPC->client->ps.saberBlocked == BLOCKED_PARRY_BROKEN) { // just make sure they don't pull their saber to them if they're being blocked
			NPCS.ucmd.buttons &= ~BUTTON_ATTACK;
		}
	}
	...
	if (NPCS.ucmd.buttons & BUTTON_ATTACK && NPCS.NPC->client->playerTeam == NPCTEAM_ENEMY) {
		if (Q_irand(0, NPCS.NPC->client->ps.fd.saberAnimLevel) > 0 && Q_irand(0, NPCS.NPC->client->pers.maxHealth + 10) > NPCS.NPC->health &&
	...
	if (NPCS.NPC->client->NPC_class != CLASS_BOBAFETT) {
		if (NPCS.NPC->client->NPC_class == CLASS_TAVION ||
			(g_npcspskill.integer && (NPCS.NPC->client->NPC_class == CLASS_DESANN ||
									  NPCS.NPCInfo->rank >= Q_irand(RANK_CREWMAN, RANK_CAPTAIN)))) { // Tavion will kick in force speed if the player does...
			if (NPCS.NPC->enemy && NPCS.NPC->enemy->s.number >= 0 && NPCS.NPC->enemy->s.number < MAX_CLIENTS && NPCS.NPC->enemy->client &&
				(NPCS.NPC->enemy->client->ps.fd.forcePowersActive & (1 << FP_SPEED)) && !(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED))) {
				int chance = 0;
				switch (g_npcspskill.integer) {
				case 0:
					chance = 9;
...
qboolean Jedi_InSpecialMove(void) {
	if (NPCS.NPC->client->ps.torsoAnim == BOTH_KYLE_PA_1 || NPCS.NPC->client->ps.torsoAnim == BOTH_KYLE_PA_2 ||
		NPCS.NPC->client->ps.torsoAnim == BOTH_KYLE_PA_3 || NPCS.NPC->client->ps.torsoAnim == BOTH_PLAYER_PA_1 ||
		NPCS.NPC->client->ps.torsoAnim == BOTH_PLAYER_PA_2 || NPCS.NPC->client->ps.torsoAnim == BOTH_PLAYER_PA_3 ||
		NPCS.NPC->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_END || NPCS.NPC->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRABBED) {
		NPC_UpdateAngles(qtrue, qtrue);
		return qtrue;
	}
	...
	if (NPCS.NPC->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_START || NPCS.NPC->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_HOLD) {
		if (!TIMER_Done(NPCS.NPC, "draining")) { // FIXME: what do we do if we ran out of power?  NPC's can't?
			// FIXME: don't keep turning to face enemy or we'll end up spinning around
			NPCS.ucmd.buttons |= BUTTON_FORCE_DRAIN;
		}
...
void NPC_BSJedi_Default(void) {
	...
	{										// have an enemy
		if (Jedi_WaitingAmbush(NPCS.NPC)) { // we were still waiting to drop down - must have had enemy set on me outside my AI
			Jedi_Ambush(NPCS.NPC);
		}

		if (Jedi_CultistDestroyer(NPCS.NPC) && !NPCS.NPCInfo->charmedTime) { // destroyer
			// permanent effect
		...
		// if needbe...
		if (((!NPCS.ucmd.buttons && !NPCS.NPC->client->ps.fd.forcePowersActive) || (NPCS.NPC->enemy && NPCS.NPC->enemy->health <= 0)) &&
			NPCS.NPCInfo->enemyCheckDebounceTime < level.time) { // not doing anything (or walking toward a vanquished enemy - fixme: always taunt the player?),
																 // not using force powers and it's time to look again
