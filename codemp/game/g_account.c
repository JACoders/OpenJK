...
void G_SpawnBlocksFromCfg(void) //loda fixme
{
	...
	Q_strncpyz(filename, Info_ValueForKey(info, "mapname"), sizeof(filename));
	Q_strlwr(filename);//dat linux
	Q_strcat(filename, sizeof(filename), "_blocks.cfg");
	...
	if (!f) {
		Com_Printf ("Couldn't load block locations from %s\n", filename);
		return;
	}
	if (fLen >= MAX_FILESIZE) {
		trap->FS_Close(f);
		Com_Printf ("Couldn't load block locations from %s, file is too large\n", filename);
		return;
	}
	...
	pch = strtok (buf," \n\t");  //loda fixme why is this broken
	while (pch != NULL && row < MAX_NUM_BLOCKS)
	{
		if ((args % 2) == 1)
	...
	Com_Printf ("Loaded block locations from %s\n", filename);
}
