...
void NPC_BSAdvanceFight(void) { // FIXME: IMPLEMENT
		...
		if (attack_ok) {
			...
			if (NPCS.enemyVisibility == VIS_FOV) { // He's in our FOV
				...
				if (attack_ok) {
					...
					if (traceEnt != NPCS.NPC->enemy && (!traceEnt || !traceEnt->client || !NPCS.NPC->client->enemyTeam ||
														NPCS.NPC->client->enemyTeam != traceEnt->client->playerTeam)) { // no, so shoot for the head
						attack_scale *= 0.75;
					...
					if (traceEnt == NPCS.NPC->enemy ||
						(traceEnt->client && NPCS.NPC->client->enemyTeam && NPCS.NPC->client->enemyTeam == traceEnt->client->playerTeam)) {
						dead_on = qtrue;
					} else {
						attack_scale *= 0.5;
						if (NPCS.NPC->client->playerTeam) {
							if (traceEnt && traceEnt->client && traceEnt->client->playerTeam) {
								if (NPCS.NPC->client->playerTeam == traceEnt->client->playerTeam) { // Don't shoot our own team
									attack_ok = qfalse;
								}
							}
						}
	...
	if (!NPCS.ucmd.forwardmove && !NPCS.ucmd.rightmove) { // We reached our captureGoal
...
void NPC_BSCinematic(void) {
	...
	if (NPCS.NPCInfo->watchTarget) { // have an entity which we want to keep facing
...
qboolean NPC_CheckInvestigate(int alertEventNum) {
	...
	if (!trap->InPVS(soundPos, NPCS.NPC->r.currentOrigin)) { // can hear through doors?
		return qfalse;
	}

	if (owner->client && owner->client->playerTeam && NPCS.NPC->client->playerTeam && owner->client->playerTeam != NPCS.NPC->client->playerTeam) {
		if ((float)NPCS.NPCInfo->investigateCount >= (NPCS.NPCInfo->stats.vigilance * 200) && owner) { // If investigateCount == 10, just take it as enemy and
																									   // go
...
void NPC_BSFollowLeader(void) {
	...
	if (!NPCS.NPC->enemy) {														 // no enemy, find one
		...
		if (!NPCS.NPC->enemy) {
			if (NPCS.NPC->client->leader && NPCS.NPC->client->leader->enemy && NPCS.NPC->client->leader->enemy != NPCS.NPC &&
				((NPCS.NPC->client->leader->enemy->client && NPCS.NPC->client->leader->enemy->client->playerTeam == NPCS.NPC->client->enemyTeam) ||
				 (/*NPC->client->leader->enemy->r.svFlags&SVF_NONNPC_ENEMY*/ 0 &&
				  NPCS.NPC->client->leader->enemy->alliedTeam == NPCS.NPC->client->enemyTeam)) &&
				NPCS.NPC->client->leader->enemy->health > 0) { // rwwFIXMEFIXME: use SVF_NONNPC_ENEMY?
		...
		if (NPCS.NPC->enemy->health <= 0 || (NPCS.NPC->enemy->flags & FL_NOTARGET)) {
			...
			NPC_CheckEnemy((NPCS.NPCInfo->confusionTime < level.time || NPCS.NPCInfo->tempBehavior != BS_FOLLOW_LEADER), qfalse,
						   qtrue); // don't find new enemy if this is tempbehav
		}
	}

	if (NPCS.NPC->enemy && NPCS.NPC->client->ps.weapon) {		  // If have an enemy, face him and fire
		if (NPCS.NPC->client->ps.weapon == WP_SABER)			  //|| NPCInfo->confusionTime>level.time )
		{														  // lightsaber user or charmed enemy
			if (NPCS.NPCInfo->tempBehavior != BS_FOLLOW_LEADER) { // not already in a temp bState
				// go after the guy
		...
		if (NPCS.enemyVisibility > VIS_PVS) {												  // face
			...
			if (NPCS.enemyVisibility >= VIS_SHOOT) { // shoot
				NPC_AimAdjust(2);
				if (NPC_GetHFOVPercentage(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles,
										  NPCS.NPCInfo->stats.hfov) > 0.6f &&
					NPC_GetHFOVPercentage(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles,
										  NPCS.NPCInfo->stats.vfov) > 0.5f) { // actually withing our front cone
					WeaponThink(qtrue);
				}
			} else {
				NPC_AimAdjust(1);
...
void NPC_BSJump(void) {
	...
	case JS_JUMPING:
		...
		if (NPCS.NPC->s.groundEntityNum != ENTITYNUM_NONE) { // Landed, start landing anim
			// FIXME: if the
			...
			// FIXME: landsound?
		} else if (NPCS.NPC->client->ps.legsTimer > 0) { // Still playing jumping anim
...
void NPC_BSSearch(void) {
	...
	if (NPCS.NPC->enemy) {
		if (NPCS.NPCInfo->tempBehavior == BS_SEARCH) { // if tempbehavior, set tempbehavior to default
			NPCS.NPCInfo->tempBehavior = BS_DEFAULT;
...
void NPC_Surrender(void) { // FIXME: say "don't shoot!" if we weren't already surrendering
	...
	if (NPCS.NPC->s.weapon != WP_NONE && NPCS.NPC->s.weapon != WP_STUN_BATON && NPCS.NPC->s.weapon != WP_SABER) {
		// WP_DropWeapon( NPC, NULL ); //rwwFIXMEFIXME: Do this (gonna need a system for notifying client of removal)
	}
	if (NPCS.NPCInfo->surrenderTime < level.time - 5000) { // haven't surrendered for at least 6 seconds, tell them what you're doing
...
qboolean NPC_CheckSurrender(void) {
	if (!trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC, TID_MOVE_NAV) && NPCS.NPC->client->ps.groundEntityNum != ENTITYNUM_NONE &&
		!NPCS.NPC->client->ps.weaponTime && !PM_InKnockDown(&NPCS.NPC->client->ps) && NPCS.NPC->enemy && NPCS.NPC->enemy->client &&
		NPCS.NPC->enemy->enemy == NPCS.NPC && NPCS.NPC->enemy->s.weapon != WP_NONE && NPCS.NPC->enemy->s.weapon != WP_STUN_BATON &&
		NPCS.NPC->enemy->health > 20 && NPCS.NPC->enemy->painDebounceTime < level.time - 3000 &&
		...
		if (NPCS.NPC->s.weapon != WP_ROCKET_LAUNCHER && NPCS.NPC->s.weapon != WP_REPEATER && NPCS.NPC->s.weapon != WP_FLECHETTE &&
			NPCS.NPC->s.weapon != WP_SABER) { // jedi and heavy weapons guys never surrender
			// FIXME: rework all this logic into some orderly fashion!!!
			if (NPCS.NPC->s.weapon != WP_NONE) { // they have a weapon so they'd have to drop it to surrender
				// don't give up unless low on health
				if (NPCS.NPC->health > 25 /*|| NPC->health >= NPC->max_health*/) { // rwwFIXMEFIXME: Keep max health not a ps state?
					return qfalse;
				}
				// if ( g_crosshairEntNum == NPC->s.number && NPC->painDebounceTime > level.time )
				if (NPC_SomeoneLookingAtMe(NPCS.NPC) && NPCS.NPC->painDebounceTime > level.time) { // if he just shot me, always give up
																								   // fall through
				} else {											 // don't give up unless facing enemy and he's very close
					if (!InFOV(NPCS.NPC->enemy, NPCS.NPC, 60, 30)) { // I'm not looking at them
						return qfalse;
					} else if (DistanceSquared(NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin) < 65536 /*256*256*/) { // they're not close
						return qfalse;
					} else if (!trap->InPVS(NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin)) { // they're not in the same room
						return qfalse;
					}
				}
			}
...
void NPC_BSFlee(void) { // FIXME: keep checking for danger
	...
	if (goal) {
		...
		if (NPCS.NPC->waypoint != WAYPOINT_NONE) {
			...
			if (numEdges != WAYPOINT_NONE) {
				...
				for (branchNum = 0; branchNum < numEdges; branchNum++) {
					...
					if (DotProduct(runDir, dangerDir) > flrand(0, 0.5)) { // don't run toward danger
						continue;
					}
		...
		if (NPCS.NPC->s.weapon == WP_NONE && (moved == qfalse || reverseCourse)) { // No weapon and no escape route... Just cower?  Need anim.
			NPC_Surrender();
			NPC_UpdateAngles(qtrue, qtrue);
			return;
		}
...
void NPC_StartFlee(gentity_t *enemy, vec3_t dangerPoint, int dangerLevel, int fleeTimeMin, int fleeTimeMax) {
	...
	if (trap->ICARUS_TaskIDPending((sharedEntity_t *)NPCS.NPC, TID_MOVE_NAV)) { // running somewhere that a script requires us to go, don't interrupt that!
		return;
	}
	...
	if (dangerLevel > AEL_DANGER || NPCS.NPC->s.weapon == WP_NONE ||
		((!NPCS.NPCInfo->group || NPCS.NPCInfo->group->numGroup <= 1) &&
		 NPCS.NPC->health <= 10)) { // IF either great danger OR I have no weapon OR I'm alone and low on health, THEN try to find a combat point out of PVS
