...
void CG_SetInitialSnapshot(snapshot_t *snap) {
  ...
  if ((cg_entities[snap->ps.clientNum].ghoul2 == NULL) &&
    ...
    if (trap->G2API_AddBolt(cg_entities[snap->ps.clientNum].ghoul2, 0,
                            "face") == -1) { // check now to see if we have this
                                             // bone for setting anims and such
...
static void CG_SetNextSnap(snapshot_t *snap) {
  ...
  // No longer want to do this, as the cg_entities[clnum] and
  // cg.predictedPlayerEntity are one in the same.
...
static snapshot_t *CG_ReadNextSnapshot(void) {
  ...
  while (cgs.processedSnapshotNum < cg.latestSnapshotNum) {
    ...
    // FIXME: why would trap->GetSnapshot return a snapshot with the same server
    // time
    if (cg.snap && r && dest->serverTime == cg.snap->serverTime) {
      //[BugFix30]
      // According to dumbledore, this situation occurs when you're playing back
      // a demo that was record when the game was running in local mode.  As
      // such, we need to skip those snaps or the demo looks laggy.
      if (cg.demoPlayback) {
        continue;
      }
      //[/BugFix30]
