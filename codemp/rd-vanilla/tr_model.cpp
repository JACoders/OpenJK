...
static const byte FakeGLAFile[] = {
	0x32, 0x4C, 0x47, 0x41, 0x06, 0x00, 0x00, 0x00, 0x2A, 0x64, 0x65, 0x66, 0x61, 0x75, 0x6C, 0x74, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	...
	};
...
//
static int GetModelDataAllocSize(void) { return Z_MemSize(TAG_MODEL_MD3) + Z_MemSize(TAG_MODEL_GLM) + Z_MemSize(TAG_MODEL_GLA); }
...
qboolean RE_RegisterModels_LevelLoadEnd(qboolean bDeleteEverythingNotUsedThisLevel /* = qfalse */) {
	...
	if (gbInsideRegisterModel) {
		...
		for (CachedModels_t::iterator itModel = CachedModels->begin();
			 itModel != CachedModels->end() && (bDeleteEverythingNotUsedThisLevel || iLoadedModelBytes > iMaxModelBytes);) {
...
qhandle_t RE_RegisterServerModel(const char *name) {
	...
	*/
	...
	if (!r_noServerGhoul2) { // keep it from choking when it gets to these checks in the g2 code. Registering all r_ cvars for the server would be a Bad Thing
							 // though.
	...
	if (numLoaded) {
		...
		*/
		...
		if (r_noPrecacheGLA && r_noPrecacheGLA->integer &&
			ident == MDXA_IDENT) { // I expect this will cause leaks, but I don't care because it's a debugging utility.
			return mod->index;
		}
#endif
...
static qboolean R_LoadMD3(model_t *mod, int lod, void *buffer, const char *mod_name, qboolean &bAlreadyCached) {
	...
	for (i = 0; i < mod->md3[lod]->numSurfaces; i++) {
		...
		if (surf->numVerts >= SHADER_MAX_VERTEXES) {
			Com_Error(ERR_DROP, "R_LoadMD3: %s has more than %i verts on %s (%i)", mod_name, SHADER_MAX_VERTEXES - 1, surf->name[0] ? surf->name : "a surface",
					  surf->numVerts);
		}
		if (surf->numTriangles * 3 >= SHADER_MAX_INDEXES) {
			Com_Error(ERR_DROP, "R_LoadMD3: %s has more than %i triangles on %s (%i)", mod_name, (SHADER_MAX_INDEXES / 3) - 1,
...
void RE_HunkClearCrap(void) { // get your dirty sticky assets off me, you damn dirty hunk!
