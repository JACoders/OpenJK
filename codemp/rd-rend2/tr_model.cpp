...
static qboolean R_LoadMD3(model_t *mod, int lod, void *buffer, const char *modName) {
	...
	for (i = 0; i < md3Model->numSurfaces; i++) {
		...
		if (md3Surf->numVerts >= SHADER_MAX_VERTEXES) {
			ri.Printf(PRINT_WARNING, "R_LoadMD3: %s has more than %i verts on %s (%i).\n", modName, SHADER_MAX_VERTEXES - 1,
					  md3Surf->name[0] ? md3Surf->name : "a surface", md3Surf->numVerts);
			return qfalse;
		}
		if (md3Surf->numTriangles * 3 >= SHADER_MAX_INDEXES) {
			ri.Printf(PRINT_WARNING, "R_LoadMD3: %s has more than %i triangles on %s (%i).\n", modName, (SHADER_MAX_INDEXES / 3) - 1,
					  md3Surf->name[0] ? md3Surf->name : "a surface", md3Surf->numTriangles);
			return qfalse;
		}
...

	{
		...
		for (i = 0; i < mdvModel->numSurfaces; i++, surf++) {
			...
			R_CalcMikkTSpaceMD3Surface(surf->numIndexes / 3, surf->verts, tangentsf, surf->st, surf->indexes);
...
int R_LerpTag(orientation_t *tag, qhandle_t handle, int startFrame, int endFrame, float frac, const char *tagName) {
	...
	if (!model->data.mdv[0]) {
		if (model->type == MOD_MDR) {
			...
			return R_IQMLerpTag(tag, (iqmData_t *)model->data.iqm, startFrame, endFrame, frac, tagName);
		} else {
