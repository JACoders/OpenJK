	...
	*/
	...
	{														   // FIXME: no need to do this if legs and torso on are same frame
		...
		if (cg_motionBoneComp.integer > 2 && cent->gent->rootBone >= 0 && cent->gent->lowerLumbarBone >= 0) { // expensive version
			...
			gi.G2API_GetBoneAnimIndex(&cent->gent->ghoul2[cent->gent->playerModel], cent->gent->lowerLumbarBone, cg.time, &upperFrame, &junk, &junk, &junk,
		...
		*/
		{
			...
			if (!WP_SaberBladeUseSecondBladeStyle(&cent->gent->client->ps.saber[saberNum], bladeNum) && cent->gent->client->ps.saber[saberNum].bladeEffect) {
		...
		*/
						...
						{															// holding the saber in-hand
							...
							if (saberNum == 0) {
								...
								if (G_GetRootSurfNameWithVariant(
										...
										sizeof(handName))) //! gi.G2API_GetSurfaceRenderStatus( &cent->gent->ghoul2[cent->gent->playerModel], "r_hand" ) )//surf
														   //! is still on
								{
								...
								if (G_GetRootSurfNameWithVariant(
										...
										sizeof(handName))) //! gi.G2API_GetSurfaceRenderStatus( &cent->gent->ghoul2[cent->gent->playerModel], "l_hand" ) )//surf
														   //! is still on
								{
									CG_AddSaberBladeGo(cent, cent, NULL, ent.renderfx, cent->gent->weaponModel[saberNum], ent.origin, tempAngles, saberNum,
													   bladeNum);
		...
		if (cent->currentState.number != 0 || cg.renderingThirdPerson || cg.snap->ps.stats[STAT_HEALTH] <= 0 ||
			...
			*/
			...
			if (cent->gent->client->ps.eFlags & EF_FORCE_DRAINED || (cent->currentState.powerups & (1 << PW_DRAINED))) { // being drained
