...
void turret_SetBoneAngles(gentity_t *ent, const char *bone,
                          const vec3_t angles);
...
//------------------------------------------------------------------------------------------------------------
{
  ...
  if (self->spawnflags & SPF_TURRETG2_TURBO) {
    G_PlayEffect(G_EffectIndex("explosions/fighter_explosion2"),
...
void TurboLaser_SetBoneAnim(gentity_t *eweb, int startFrame, int endFrame) {
  ...
  // snapshot. eweb->s.eFlags |= EF_G2ANIMATING;
...
//----------------------------------------------------------------
{
  ...
  if ((ent->spawnflags & SPF_TURRETG2_TURBO)) {
    ...
    G_SoundOnEnt(ent, CHAN_LESS_ATTEN,
                 "sound/vehicles/weapons/turbolaser/fire1");
...
//-----------------------------------------------------
{
  ...
  if (self->enemy && self->pushDebounceTime < level.time &&
    ...
    gi.G2API_GetBoltMatrix(
        ...
        &boltMatrix, self->currentAngles, self->currentOrigin, level.time, NULL,
        self->modelScale);
    if ((self->spawnflags & SPF_TURRETG2_TURBO)) {
    ...
    gi.G2API_GetBoltMatrix(
        ...
        (self->spawnflags & SPF_TURRETG2_TURBO)
            ? ((self->alt_fire
                    ...
                    : gi.G2API_AddBolt(&self->ghoul2[0], "*muzzle1")))
            : gi.G2API_AddBolt(&self->ghoul2[0], "*flash03"),
        &boltMatrix, self->currentAngles, self->s.origin, level.time, NULL,
        self->modelScale);
...
void turret_SetBoneAngles(gentity_t *ent, const char *bone,
  ...
  while (thebone)
  {
          ...
          { //if the value is 0 then this index is clear, we can use it if we
  don't find the bone we want already existing. firstFree = thebone; freeBoneVec
  = boneVector;
          }
          else if (*thebone)
          {
...
*/
  ...
  gi.G2API_SetBoneAngles(&ent->ghoul2[0], bone, angles, flags, up, right,
...
ceiling TURBO - Big-ass, Boxy Death Star Turbo Laser version
...
//-----------------------------------------------------
{
  /*base->s.modelindex = G_ModelIndex(
  "models/map_objects/imp_mine/turret_canon.glm" ); base->s.modelindex2 =
  G_ModelIndex( "models/map_objects/imp_mine/turret_damage.md3" );
  base->playerModel = gi.G2API_InitGhoul2Model( base->ghoul2,
...
void finish_spawning_turret(gentity_t *base) {
  ...
  if ((base->spawnflags & SPF_TURRETG2_TURBO)) {
    ...
    base->contents = CONTENTS_BODY | CONTENTS_PLAYERCLIP |
...
/*QUAKED misc_ion_cannon (1 0 0) (-280 -280 0) (280 280 640) START_OFF BURSTS
...
void spotlight_think(gentity_t *ent) {
  ...
  gi.trace(&tr, ent->currentOrigin, vec3_origin, vec3_origin, end,
           ent->s.number, CONTENTS_SOLID, (EG2_Collision)0, 0);
...
void panel_turret_think(gentity_t *self) {
  ...
  if (player && player->client &&
    ...
    if ((self->useDebounceTime < level.time) &&
        ((ucmd->buttons & BUTTON_USE) || ucmd->forwardmove || ucmd->rightmove ||
         ucmd->upmove)) {
