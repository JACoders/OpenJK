...
// seems to be a compiler bug, it doesn't clean out the #ifdefs between
// dif-compiles or something, so the headers spew errors on these defs from the
// previous compile. this fixes that. -rww
#ifdef _JK2MP
...
#ifdef _JK2MP
// this is really horrible, but it works! just be sure not to use any locals or
// anything with these names (exluding bool, false, true). -rww
...
extern void PM_SetAnim(pmove_t *pm, int setAnimParts, int anim,
                       int setAnimFlags, int blendTime);
extern int PM_AnimLength(int index, animNumber_t anim);
extern void G_VehicleTrace(trace_t *results, const vec3_t start,
                           const vec3_t tMins, const vec3_t tMaxs,
...
extern void BG_SetAnim(playerState_t *ps, animation_t *animations,
...
#include "b_local.h"
extern void BG_ExternThisSoICanRecompileInDebug(Vehicle_t *pVeh,
                                                playerState_t *riderPS);

// this stuff has got to be predicted, so..
bool BG_FighterUpdate(Vehicle_t *pVeh, const usercmd_t *pUcmd, vec3_t trMins,
                      vec3_t trMaxs, float gravity,
                      void (*traceFunc)(trace_t *results, const vec3_t start,
                                        const vec3_t lmins, const vec3_t lmaxs,
                                        const vec3_t end, int passEntityNum,
                                        int contentMask)) {
  vec3_t bottom;
  ...
  if (!parentPS) {
    Com_Error(ERR_DROP, "NULL PS in BG_FighterUpdate (%s)",
              pVeh->m_pVehicleInfo->name);
    return false;
  }
  ...
  {
  */
  // argh, no, I need to have a way to see when they impact the ground while
  // damaged. -rww

  // Check to see if the fighter has taken off yet (if it's a certain height
  // above ground).
  VectorCopy(parentPS->origin, bottom);
  bottom[2] -= pVeh->m_pVehicleInfo->landingHeight;

  traceFunc(&pVeh->m_LandTrace, parentPS->origin, trMins, trMaxs, bottom,
            pVeh->m_pParentEntity->s.number, (MASK_NPCSOLID & ~CONTENTS_BODY));
  //}
...
// Board this Vehicle (get on). The first entity to board an empty vehicle
// becomes the Pilot.
static bool Board(Vehicle_t *pVeh, bgEntity_t *pEnt) {
  if (!g_vehicleInfo[VEHICLE_BASE].Board(pVeh, pEnt))
    return false;

  // Set the board wait time (they won't be able to do anything, including
  // getting off, for this amount of time).
...
// method of decrementing the given angle based on the given taking variable
// frame times into account
static float PredictedAngularDecrement(float scale, float timeMod,
                                       float originalAngle) {
  ...
  if (fixedBaseDec < 0.1f) { // don't increment in incredibly small fractions,
                             // it would eat up unnecessary bandwidth.
    fixedBaseDec = 0.1f;
  }
...
#ifdef QAGAME // only do this check on GAME side, because if it's CGAME, it's
              // being predicted, and it's only predicted if the local client is
              // the driver
qboolean FighterIsInSpace(gentity_t *gParent) {
  if (gParent && gParent->client && gParent->client->inSpaceIndex &&
...
qboolean FighterIsLanding(Vehicle_t *pVeh, playerState_t *parentPS) {
  ...
  if (FighterOverValidLandingSurface(pVeh)
              ...
              // being predicted, and it's only predicted if the local client is
              // the driver
      && pVeh->m_pVehicleInfo->Inhabited(
             pVeh) // has to have a driver in order to be capable of landing
#endif
      && (pVeh->m_ucmd.forwardmove < 0 ||
          pVeh->m_ucmd.upmove < 0) // decelerating or holding crouch button
      && parentPS->speed <=
             MIN_LANDING_SPEED) // going slow enough to start landing - was
                                // using pVeh->m_pVehicleInfo->speedIdle, but
...
qboolean FighterIsLaunching(Vehicle_t *pVeh, playerState_t *parentPS) {
  ...
  if (FighterOverValidLandingSurface(pVeh)
      ...
      &&
      parentPS->speed <=
          200.0f) // going slow enough to start landing - was using
                  // pVeh->m_pVehicleInfo->speedIdle, but that's still too fast
  {
    return qtrue;
  }
  return qfalse;
...
qboolean FighterSuspended(Vehicle_t *pVeh, playerState_t *parentPS) {
  ...
  if (!pVeh->m_pPilot     // empty
      ...
      && pVeh->m_ucmd.forwardmove <=
             0 // not trying to go forward for whatever reason
      && pVeh->m_pParentEntity != NULL &&
      (((gentity_t *)pVeh->m_pParentEntity)->spawnflags &
       2)) // SUSPENDED spawnflag is on
  {
    return qtrue;
  }
  return qfalse;
...
#ifdef CGAME
extern void trap_S_StartSound(vec3_t origin, int entityNum, int entchannel,
                              sfxHandle_t sfx);              // cg_syscalls.c
...
static void ProcessMoveCommands(Vehicle_t *pVeh) {
  /************************************************************************************/
  /*	BEGIN	Here is where we move the vehicle (forward or back or
   * whatever). BEGIN	*/
  /************************************************************************************/
...
#ifdef _JK2MP
  if (parentPS->hyperSpaceTime && curTime - parentPS->hyperSpaceTime <
                                      HYPERSPACE_TIME) { // Going to Hyperspace
    // totally override movement
    float timeFrac =
        ((float)(curTime - parentPS->hyperSpaceTime)) / HYPERSPACE_TIME;
    if (timeFrac < HYPERSPACE_TELEPORT_FRAC) { // for first half, instantly jump
                                               // to top speed!
      if (!(parentPS->eFlags2 & EF2_HYPERSPACE)) { // waiting to face the right
                                                   // direction, do nothing
        parentPS->speed = 0.0f;
      } else {
        if (parentPS->speed < HYPERSPACE_SPEED) { // just started hyperspace
// MIKE: This is going to play the sound twice for the predicting client, I
// suggest using a predicted event or only doing it game-side. -rich
#ifdef QAGAME // MP GAME-side
              // G_EntitySound( ((gentity_t *)(pVeh->m_pParentEntity)),
              // CHAN_LOCAL, pVeh->m_pVehicleInfo->soundHyper );
#elif defined CGAME // MP CGAME-side
          trap_S_StartSound(NULL, pm->ps->clientNum, CHAN_LOCAL,
                            pVeh->m_pVehicleInfo->soundHyper);
#endif
        }
      ...
      parentPS->speed =
          200.0f + ((1.0f - timeFrac) * (1.0f / HYPERSPACE_TELEPORT_FRAC) *
                    (HYPERSPACE_SPEED - 200.0f));
  ...
  isLandingOrLaunching = (qboolean)(FighterIsLanding(pVeh, parentPS) ||
  ...
  if (isLandingOrLaunching // going slow enough to start landing
      ...
      (pVeh->m_ucmd.forwardmove <= 0 ||
       pVeh->m_LandTrace.fraction <=
           FIGHTER_MIN_TAKEOFF_FRACTION)) // not trying to accelerate away
                                          // already (or: you are trying to, but
                                          // not high enough off the ground yet)
  { // FIXME: if start to move forward and fly over something low while still
    // going relatively slow, you may try to land even though you don't mean
    // to...
    ...
    if (pVeh->m_ucmd.upmove > 0) {
...
#ifdef QAGAME                                   // MP GAME-side
        G_EntitySound(((gentity_t *)(pVeh->m_pParentEntity)), CHAN_AUTO,
                      pVeh->m_pVehicleInfo->soundTakeOff);
#endif
      }
#endif
      parentPS->velocity[2] +=
          pVeh->m_pVehicleInfo->acceleration *
          pVeh->m_fTimeModifier; // * ( /*fInvFrac **/ 1.5f );
    } else if (pVeh->m_ucmd.upmove < 0) {
      parentPS->velocity[2] -=
          pVeh->m_pVehicleInfo->acceleration *
          pVeh->m_fTimeModifier; // * ( /*fInvFrac **/ 1.8f );
      ...
      if (pVeh->m_LandTrace.fraction <= FIGHTER_MIN_TAKEOFF_FRACTION) {
        // pVeh->m_pParentEntity->client->ps.velocity[0] *=
        // pVeh->m_LandTrace.fraction;
        // pVeh->m_pParentEntity->client->ps.velocity[1] *=
        // pVeh->m_LandTrace.fraction;

        // remember to always base this stuff on the time modifier! otherwise,
        // you create framerate-dependancy issues and break prediction in MP -rww
        // parentPS->velocity[2] *= pVeh->m_LandTrace.fraction;
        // it's not an angle, but hey
        parentPS->velocity[2] = PredictedAngularDecrement(
            pVeh->m_LandTrace.fraction, pVeh->m_fTimeModifier * 5.0f,
            parentPS->velocity[2]);
    ...
    pVeh->m_vOrientation[PITCH] = PredictedAngularDecrement(
        0.7f, pVeh->m_fTimeModifier * 10.0f, pVeh->m_vOrientation[PITCH]);
  ...
  if ((pVeh->m_ucmd.upmove > 0) && pVeh->m_pVehicleInfo->turboSpeed) {
    if ((curTime - pVeh->m_iTurboTime) > pVeh->m_pVehicleInfo->turboRecharge) {
...
#ifndef _JK2MP // SP
          G_PlayEffect(pVeh->m_pVehicleInfo->iTurboStartFX,
                       pVeh->m_pParentEntity->playerModel,
      ...
      // NOTE: turbo sound can't be part of effect if effect is played on every
      // muzzle!
      if (pVeh->m_pVehicleInfo->soundTurbo) {
#ifndef _JK2MP // SP
        G_SoundIndexOnEnt(pVeh->m_pParentEntity, CHAN_AUTO,
                          pVeh->m_pVehicleInfo->soundTurbo);
#elif defined QAGAME // MP GAME-side
        G_EntitySound(((gentity_t *)(pVeh->m_pParentEntity)), CHAN_AUTO,
                      pVeh->m_pVehicleInfo->soundTurbo);
#elif defined CGAME  // MP CGAME-side
                    // trap_S_StartSound( NULL, pVeh->m_pParentEntity->s.number,
                    // CHAN_AUTO, pVeh->m_pVehicleInfo->soundTurbo );
#endif
      }
    }
  }
  ...
  /*
  //FIXME: if turbotime is up and we're waiting for it to recharge, should our
  max speed drop while we recharge? else if ( (curTime -
  pVeh->m_iTurboTime)<3000 )
  ...
  */
...
#ifdef _JK2MP // SP can cheat and just check m_iTurboTime directly... :)
    if ((parentPS->eFlags & EF_JETPACK_ACTIVE)) { // stop cgame from playing the
                                                  // turbo exhaust effect
      parentPS->eFlags &= ~EF_JETPACK_ACTIVE;
    }
#endif
  }
  ...
  if ((parentPS->brokenLimbs &
       (1 << SHIPSURF_DAMAGE_BACK_HEAVY))) { // engine has taken heavy damage
    speedMax *= 0.8f;                        // at 80% speed
  } else if ((parentPS->brokenLimbs &
              (1 << SHIPSURF_DAMAGE_BACK_LIGHT))) { // engine has taken light
                                                    // damage
  ...
  if (pVeh->m_iRemovedSurfaces ||
...
#ifdef QAGAME // well, the thing is always going to be inhabited if it's being
              // predicted!
    ...
    pVeh->m_ucmd.forwardmove = 0;
             ...
             parentPS->speed >
                 0) { // pilot jumped out while we were moving forward (not
                      // landing or landed) so just keep the throttle locked
    // Why set forwardmove?  PMove code doesn't use it... does it?
    pVeh->m_ucmd.forwardmove = 127;
  }
#endif
    ...
    if (pVeh->m_ucmd.forwardmove > 0 && speedInc) {
      ...
      if (pVeh->m_ucmd.upmove <
          0) { // braking (trying to land?), slow down faster
      ...
      if (parentPS->speed > speedIdle) {
        ...
        if (FighterOverValidLandingSurface(
                pVeh)) { // there's ground below us and we're trying to slow
                         // down, slow down faster
          parentPS->speed -= speedInc;
        } else {
          parentPS->speed -= speedIdleDec;
          if (parentPS->speed <
              MIN_LANDING_SPEED) { // unless you can land, don't drop below the
                                   // landing speed!!!  This way you can't come
                                   // to a dead stop in mid-air
    ...
    // else not accel, decel or braking
    else if (pVeh->m_pVehicleInfo
                 ->throttleSticks) { // we're using a throttle that sticks at
                                     // current speed
      if (parentPS->speed <=
          MIN_LANDING_SPEED) { // going less than landing speed
        if (FighterOverValidLandingSurface(
                pVeh)) { // close to ground and not going very fast
          ...
          // speed up to idle speed if not over a valid landing surf and not
          // accel/decel/braking
          if (parentPS->speed < speedIdle) {
      ...
      if ((pVeh->m_LandTrace.fraction >= 1.0f // no ground
           || pVeh->m_LandTrace.plane.normal[2] <
                  MIN_LANDING_SLOPE) // or can't land on ground below us
      ...
      {      // slow to a stop if no idle speed or within landing height and not
        // accel/decel/braking
        if (parentPS->speed > 0) { // slow down
          parentPS->speed -= speedIdleDec;
        } else if (parentPS->speed < 0) { // going backwards, slow down
    ...
    if (!pVeh->m_pVehicleInfo->strafePerc ||
        (!g_speederControlScheme->value &&
         !pVeh->m_pParentEntity->s
              .number)) { // if in a strafe-capable vehicle, clear strafing
                          // unless using alternate control scheme
  ...
  if (pVeh->m_pVehicleInfo->strafePerc
    ...
    if (pVeh->m_ucmd.rightmove > 0) { // strafe right
      ...
      //		go faster than max speed if you keep turning and
      //strafing...
      if (pVeh->m_fStrafeTime >
          -MAX_STRAFE_TIME) { // can strafe right for 2 seconds
        ...
        if (strafeSpeed > 0) {
          VectorMA(parentPS->velocity, strafeSpeed * pVeh->m_fTimeModifier,
                   vRight, parentPS->velocity);
        }
        pVeh->m_fStrafeTime -= 50 * pVeh->m_fTimeModifier;
      }
    } else { // strafe left
      if (pVeh->m_fStrafeTime <
          MAX_STRAFE_TIME) { // can strafe left for 2 seconds
        float curStrafeSpeed = DotProduct(parentPS->velocity, vRight);
    ...
    // strafing takes away from forward speed?  If so, strafePerc above should
    // use speedMax parentPS->speed *= (1.0f-pVeh->m_pVehicleInfo->strafePerc);
...
// STRAFING==============================================================================
  ...
  if ((pVeh->m_vOrientation[PITCH] * 0.1f) >
      10.0f) { // pitched downward, increase speed more and more based on our
               // tilt
    if (FighterIsInSpace(
            (gentity_t *)
                parent)) { // in space, do nothing with speed base on pitch...
    } else {
      ...
      parentPS->speed = PredictedAngularDecrement(
          mult, pVeh->m_fTimeModifier * 10.0f, parentPS->speed);
    }
  }
  ...
  if (pVeh->m_iRemovedSurfaces ||
    ...
    if (FighterIsInSpace(
            (gentity_t *)parent)) { // we're in a valid trigger_space brush
      // simulate randomness
    ...
    parentPS->gravity = 0;
  } else if ((!parentPS->speed || parentPS->speed < speedIdle) &&
             pVeh->m_ucmd.upmove <=
                 0) { // slowing down or stopped and not trying to take off
    if (FighterIsInSpace(
            (gentity_t *)parent)) { // we're in space, stopping doesn't make us
                                    // drift downward
      if (FighterOverValidLandingSurface(
              pVeh)) { // well, there's something below us to land on, so go
                       // ahead and lower us down to it
...
extern void BG_VehicleTurnRateForSpeed(Vehicle_t *pVeh, float speed,
                                       float *mPitchOverride,
                                       float *mYawOverride);
static void FighterWingMalfunctionCheck(Vehicle_t *pVeh,
                                        playerState_t *parentPS) {
  float mPitchOverride = 1.0f;
  float mYawOverride = 1.0f;
  BG_VehicleTurnRateForSpeed(pVeh, parentPS->speed, &mPitchOverride,
                             &mYawOverride);
  // check right wing damage
  if ((parentPS->brokenLimbs &
       (1
        << SHIPSURF_DAMAGE_RIGHT_HEAVY))) { // right wing has taken heavy damage
    pVeh->m_vOrientation[ROLL] +=
        (sin(pVeh->m_ucmd.serverTime * 0.001) + 1.0f) * pVeh->m_fTimeModifier *
        mYawOverride * 50.0f;
  } else if ((parentPS->brokenLimbs &
              (1 << SHIPSURF_DAMAGE_RIGHT_LIGHT))) { // right wing has taken
                                                     // light damage
    pVeh->m_vOrientation[ROLL] +=
        (sin(pVeh->m_ucmd.serverTime * 0.001) + 1.0f) * pVeh->m_fTimeModifier *
        mYawOverride * 12.5f;
  ...
  if ((parentPS->brokenLimbs &
       (1 << SHIPSURF_DAMAGE_LEFT_HEAVY))) { // left wing has taken heavy damage
    pVeh->m_vOrientation[ROLL] -=
        (sin(pVeh->m_ucmd.serverTime * 0.001) + 1.0f) * pVeh->m_fTimeModifier *
        mYawOverride * 50.0f;
  } else if ((parentPS->brokenLimbs &
              (1 << SHIPSURF_DAMAGE_LEFT_LIGHT))) { // left wing has taken light
                                                    // damage
    pVeh->m_vOrientation[ROLL] -=
        (sin(pVeh->m_ucmd.serverTime * 0.001) + 1.0f) * pVeh->m_fTimeModifier *
        mYawOverride * 12.5f;
...
static void FighterNoseMalfunctionCheck(Vehicle_t *pVeh,
                                        playerState_t *parentPS) {
  float mPitchOverride = 1.0f;
  float mYawOverride = 1.0f;
  BG_VehicleTurnRateForSpeed(pVeh, parentPS->speed, &mPitchOverride,
  ...
  // check nose damage
  if ((parentPS->brokenLimbs &
       (1 << SHIPSURF_DAMAGE_FRONT_HEAVY))) { // nose has taken heavy damage
    // pitch up and down over time
    pVeh->m_vOrientation[PITCH] += sin(pVeh->m_ucmd.serverTime * 0.001) *
                                   pVeh->m_fTimeModifier * mPitchOverride *
                                   50.0f;
  } else if ((parentPS->brokenLimbs &
              (1 << SHIPSURF_DAMAGE_FRONT_LIGHT))) { // nose has taken heavy
                                                     // damage
    // pitch up and down over time
    pVeh->m_vOrientation[PITCH] += sin(pVeh->m_ucmd.serverTime * 0.001) *
                                   pVeh->m_fTimeModifier * mPitchOverride *
...
static void FighterDamageRoutine(Vehicle_t *pVeh, bgEntity_t *parent,
                                 playerState_t *parentPS,
                                 playerState_t *riderPS, qboolean isDead) {
  if (!pVeh->m_iRemovedSurfaces) {         // still in one piece
    if (pVeh->m_pParentEntity && isDead) { // death spiral
      ...
      */
      if (!(pVeh->m_pParentEntity->s.number %
            3)) { // NOT everyone should do this
...
#ifdef QAGAME
  if (pVeh->m_LandTrace.fraction < 1.0f) { // if you land at all when pieces of
                                           // your ship are missing, then die
    ...
    if (parent->client->ps.otherKiller < ENTITYNUM_WORLD &&
      ...
      if (potentialKiller->inuse &&
          potentialKiller->client) { // he's valid I guess
        killer = potentialKiller;
      }
    }
#endif
    G_Damage(parent, killer, killer, vec3_origin, parent->client->ps.origin,
             99999, DAMAGE_NO_ARMOR, MOD_SUICIDE);
  }
#endif
  ...
  if (((pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_C) ||
      ...
      ((pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_E) ||
       (pVeh->m_iRemovedSurfaces &
        SHIPSURF_BROKEN_F))) { // wings on both side broken
    float factor = 2.0f;
    ...
    if (!(pVeh->m_pParentEntity->s.number % 4) ||
        !(pVeh->m_pParentEntity->s.number %
          5)) { // won't yaw, so increase roll factor
      factor *= 4.0f;
    }

    pVeh->m_vOrientation[ROLL] +=
        (pVeh->m_fTimeModifier * factor); // do some spiralling
  } else if ((pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_C) ||
             (pVeh->m_iRemovedSurfaces &
              SHIPSURF_BROKEN_D)) { // left wing broken
    float factor = 2.0f;
    if ((pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_C) &&
        (pVeh->m_iRemovedSurfaces &
         SHIPSURF_BROKEN_D)) { // if both are broken..
      factor *= 2.0f;
    }
    ...
    pVeh->m_vOrientation[ROLL] += factor * pVeh->m_fTimeModifier;
             ...
             (pVeh->m_iRemovedSurfaces &
              SHIPSURF_BROKEN_F)) { // right wing broken
    float factor = 2.0f;
    if ((pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_E) &&
        (pVeh->m_iRemovedSurfaces &
         SHIPSURF_BROKEN_F)) { // if both are broken..
      factor *= 2.0f;
    }
...
#ifdef _JK2MP
void FighterYawAdjust(Vehicle_t *pVeh, playerState_t *riderPS,
                      playerState_t *parentPS) {
  float angDif =
      AngleSubtract(pVeh->m_vOrientation[YAW], riderPS->viewangles[YAW]);

  if (parentPS && parentPS->speed) {
    float s = parentPS->speed;
    float maxDif =
        pVeh->m_pVehicleInfo->turningSpeed * 0.8f; // magic number hackery
...
void FighterPitchAdjust(Vehicle_t *pVeh, playerState_t *riderPS,
                        playerState_t *parentPS) {
  float angDif =
      AngleSubtract(pVeh->m_vOrientation[PITCH], riderPS->viewangles[PITCH]);
...
static void ProcessOrientCommands(Vehicle_t *pVeh) {
  /********************************************************************************/
  /*	BEGIN	Here is where make sure the vehicle is properly oriented.
   * BEGIN	*/
  /********************************************************************************/
  ...
  if (pVeh->m_iDropTime >= curTime) { // you can only YAW during this
    parentPS->viewangles[YAW] = pVeh->m_vOrientation[YAW] =
  ...
  if (isDead || parentPS->electrifyTime >= curTime ||
      (pVeh->m_pVehicleInfo->surfDestruction && pVeh->m_iRemovedSurfaces &&
       ...
       (pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_E) &&
       (pVeh->m_iRemovedSurfaces &
        SHIPSURF_BROKEN_F))) { // do some special stuff for when all the wings
                               // are torn off
  ...
  if (!BG_UnrestrainedPitchRoll(riderPS, pVeh)) {
    pVeh->m_vOrientation[ROLL] = PredictedAngularDecrement(
        0.95f, angleTimeMod * 2.0f, pVeh->m_vOrientation[ROLL]);
  }

  isLandingOrLanded = (qboolean)(FighterIsLanding(pVeh, parentPS) ||
                                 FighterIsLanded(pVeh, parentPS));

  if (!isLandingOrLanded) { // don't do this stuff while landed.. I guess. I
                            // don't want ships spinning in place, looks silly.
    int m = 0;
    float aVelDif;
    float dForVel;
    ...
    while (m < 3) {
      ...
      if (aVelDif != 0.0f) {
        ...
        if (dForVel > 1.0f || dForVel < -1.0f) {
          ...
          if (m == PITCH) { // don't pitch downward into ground even more.
            if (pVeh->m_vOrientation[m] > 90.0f &&
                (pVeh->m_vOrientation[m] - dForVel) < 90.0f) {
      ...
      parentPS->electrifyTime < curTime) // not spiraling out of control
  {
    if (parentPS->speed > 0.0f) { // Uh... what?  Why?
      if (pVeh->m_LandTrace.fraction < 0.3f) {
        ...
        pVeh->m_vOrientation[PITCH] = PredictedAngularDecrement(
            0.83f, angleTimeMod * 10.0f, pVeh->m_vOrientation[PITCH]);
      }
    }
    if (pVeh->m_LandTrace.fraction > 0.1f ||
        pVeh->m_LandTrace.plane.normal[2] <
            MIN_LANDING_SLOPE) { // off the ground, at least (or not on a valid
                                 // landing surf)
                                 // Dampen the turn rate based on the current
                                 // height.
#ifdef _JK2MP
      FighterYawAdjust(pVeh, riderPS, parentPS);
#else
      pVeh->m_vOrientation[YAW] =
          riderPS->viewangles[YAW]; //*pVeh->m_LandTrace.fraction;
#endif
    }
  } else if ((pVeh->m_iRemovedSurfaces ||
              parentPS->electrifyTime >= curTime) // spiralling out of control
             && (!(pVeh->m_pParentEntity->s.number % 4) ||
                 !(pVeh->m_pParentEntity->s.number % 5))) { // no yaw control
  } else if (pVeh->m_pPilot && pVeh->m_pPilot->s.number < MAX_CLIENTS &&
             parentPS->speed > 0.0f) //&& !( pVeh->m_ucmd.forwardmove > 0 &&
                                     //pVeh->m_LandTrace.fraction != 1.0f ) )
  {
    if (BG_UnrestrainedPitchRoll(riderPS, pVeh)) {
      VectorCopy(riderPS->viewangles, pVeh->m_vOrientation);
      ...
      */
      ...
      // If we are not hitting the ground, allow the fighter to pitch up and
      // down.
      if (!FighterOverValidLandingSurface(pVeh) ||
          parentPS->speed > MIN_LANDING_SPEED)
      // if ( ( pVeh->m_LandTrace.fraction >= 1.0f || pVeh->m_ucmd.forwardmove
      // != 0 ) && pVeh->m_LandTrace.fraction >= 0.0f )
      {
        float fYawDelta;
        ...
        fYawDelta = AngleSubtract(
            pVeh->m_vOrientation[YAW],
            pVeh->m_vPrevOrientation[YAW]); // pVeh->m_vOrientation[YAW] -
        ...
        curRoll =
            PredictedAngularDecrement(0.93f, angleTimeMod * 2.0f, curRoll);
        // cap it reasonably
  ...
  if (isLandingOrLanded) { // only if capable of landing
    if (!isDead && parentPS->electrifyTime < curTime &&
        (!pVeh->m_pVehicleInfo->surfDestruction ||
         !pVeh->m_iRemovedSurfaces)) { // not crashing or spiralling out of
                                       // control...
      if (pVeh->m_vOrientation[PITCH] > 0) {
        pVeh->m_vOrientation[PITCH] = PredictedAngularDecrement(
            0.2f, angleTimeMod * 10.0f, pVeh->m_vOrientation[PITCH]);
      } else {
        pVeh->m_vOrientation[PITCH] = PredictedAngularDecrement(
            0.75f, angleTimeMod * 10.0f, pVeh->m_vOrientation[PITCH]);
      }
    }
  }

  /*
  //NOTE: all this is redundant now since we have the FighterDamageRoutine
  func... #ifdef _JK2MP //...yeah. Need to send armor across net for prediction
  to work. if ( isDead ) #else if ( pVeh->m_iArmor <= 0 ) #endif
          {//going to explode
                  //FIXME: maybe make it erratically jerk or spin or start and
  stop? #ifndef _JK2MP if ( g_speederControlScheme->value > 0 || !rider ||
  rider->s.number ) #else if (1) #endif
                  {
  ...
  */
  // If no one is in this vehicle and it's up in the sky, pitch it forward as it
  // comes tumbling down.
#ifdef QAGAME // never gonna happen on client anyway, we can't be getting
              // predicted unless the predicting client is boarded
  ...
  if (!pVeh->m_fStrafeTime) { // use that roll
    pVeh->m_vOrientation[ROLL] = curRoll;
    // NOTE: this seems really backwards...
    if (pVeh->m_vOrientation[ROLL]) { // continually adjust the yaw based on the
                                      // roll..
      if ((pVeh->m_iRemovedSurfaces ||
           parentPS->electrifyTime >= curTime) // spiralling out of control
          && (!(pVeh->m_pParentEntity->s.number % 4) ||
              !(pVeh->m_pParentEntity->s.number % 5))) { // leave YAW alone
      } else {
        if (!BG_UnrestrainedPitchRoll(riderPS, pVeh)) {
          pVeh->m_vOrientation[YAW] -=
              ((pVeh->m_vOrientation[ROLL]) * 0.05f) * pVeh->m_fTimeModifier;
        }
      }
    }
  } else { // add in strafing roll
    float strafeRoll =
        (pVeh->m_fStrafeTime / MAX_STRAFE_TIME) *
    ...
    if (!BG_UnrestrainedPitchRoll(riderPS, pVeh)) { // cap it reasonably
      if (pVeh->m_pVehicleInfo->rollLimit != -1 && !pVeh->m_iRemovedSurfaces &&
        ...
        } else if (pVeh->m_vOrientation[ROLL] <
                   -pVeh->m_pVehicleInfo->rollLimit) {
          pVeh->m_vOrientation[ROLL] = -pVeh->m_pVehicleInfo->rollLimit;
        }
      }
    }
...
static void AnimateVehicle(Vehicle_t *pVeh) {
...
#endif
  {
    ...
    // otherwise we're below launch height and still taking off.
      ...
      if ((pVeh->m_ucmd.forwardmove < 0 || pVeh->m_ucmd.upmove < 0 ||
           isLanded) &&
          pVeh->m_LandTrace.fraction <= 0.4f &&
          pVeh->m_LandTrace.plane.normal[2] >=
              MIN_LANDING_SLOPE) { // already landed or trying to land and close
                                   // to ground
        // Open gears.
...
#elif defined CGAME // MP CGAME-side
                    // trap_S_StartSound( NULL, pVeh->m_pParentEntity->s.number,
                    // CHAN_AUTO, pVeh->m_pVehicleInfo->soundLand );
#endif
          }
#endif
          pVeh->m_ulFlags |= VEH_GEARSOPEN;
...
// This function makes sure that the rider's in this vehicle are properly
// animated.
...
void G_SetFighterVehicleFunctions(vehicleInfo_t *pVehInfo) {
  ...
  //	pVehInfo->ValidateBoard				=
  //ValidateBoard; 	pVehInfo->SetParent					=
  //SetParent; 	pVehInfo->SetPilot					=
  //SetPilot; 	pVehInfo->AddPassenger				=
  //AddPassenger; 	pVehInfo->Animate					=
  //Animate;
  pVehInfo->Board = Board;
  pVehInfo->Eject = Eject;
  //	pVehInfo->EjectAll					=
  //EjectAll; 	pVehInfo->StartDeathDelay			=
  //StartDeathDelay; 	pVehInfo->DeathUpdate				=
  //DeathUpdate; 	pVehInfo->RegisterAssets			=
  //RegisterAssets; 	pVehInfo->Initialize				=
  //Initialize;
  pVehInfo->Update = Update;
//	pVehInfo->UpdateRider				=
//UpdateRider;
#endif // game-only
...
#endif
  //	pVehInfo->AttachRiders				=
  //AttachRiders;
  //	pVehInfo->Ghost						=
  //Ghost; 	pVehInfo->UnGhost					=
  //UnGhost; 	pVehInfo->Inhabited					=
  //Inhabited;
}
...
#ifdef _JK2MP
#ifdef QAGAME
  // these will remain on entities on the client once allocated because the
  // pointer is never stomped. on the server, however, when an ent is freed, the
  // entity struct is memset to 0, so this memory would be lost..
