...
typedef enum {
  ...
  VF_SHADER_NOMIP,  // (cgame only) take the string, call
...
vehField_t vehWeaponFields[] = {
    ...
    {"ionWeapon", VWFOFS(bIonWeapon),
     VF_BOOL}, // disables ship shields and sends them out of control
    {"saberBlockable", VWFOFS(bSaberBlockable),
     VF_BOOL}, // lightsabers can deflect this projectile
    {"muzzleFX", VWFOFS(iMuzzleFX), VF_EFFECT_CLIENT}, // index of Muzzle Effect
    {"model", VWFOFS(iModel),
     VF_MODEL_CLIENT}, // handle to the model used by this projectile
    ...
    {"g2MarkShader", VWFOFS(iG2MarkShaderHandle),
     VF_SHADER}, // index of shader to use for G2 marks made on other models
                 // when hit by this projectile
    {"g2MarkSize", VWFOFS(fG2MarkSize),
     VF_FLOAT}, // size (diameter) of the ghoul2 mark
    {"loopSound", VWFOFS(iLoopSound), VF_SOUND_CLIENT}, // index of loopSound
    {"speed", VWFOFS(fSpeed),
     VF_FLOAT}, // speed of projectile/range of traceline
    {"homing", VWFOFS(fHoming),
     VF_FLOAT}, // 0.0 = not homing, 0.5 = half vel to targ, half cur vel, 1.0 =
                // all vel to targ
    {"homingFOV", VWFOFS(fHomingFOV),
     VF_FLOAT}, // missile will lose lock on if DotProduct of missile direction
                // and direction to target ever drops below this (-1 to 1, -1 =
                // never lose target, 0 = lose if ship gets behind missile, 1 =
                // pretty much will lose it's target right away)
    {"lockOnTime", VWFOFS(iLockOnTime),
     VF_INT}, // 0 = no lock time needed, else # of ms needed to lock on
    {"damage", VWFOFS(iDamage),
     VF_INT}, // damage done when traceline or projectile directly hits target
    {"splashDamage", VWFOFS(iSplashDamage),
     VF_INT}, // damage done to ents in splashRadius of end of traceline or
              // projectile origin on impact
    {"splashRadius", VWFOFS(fSplashRadius),
     VF_FLOAT}, // radius that ent must be in to take splashDamage (linear
                // fall-off)
    {"ammoPerShot", VWFOFS(iAmmoPerShot),
     VF_INT}, // how much "ammo" each shot takes
    {"health", VWFOFS(iHealth),
     VF_INT}, // if non-zero, projectile can be shot, takes this much damage
              // before being destroyed
    {"width", VWFOFS(fWidth), VF_FLOAT}, // width of traceline or bounding box
                                         // of projecile (non-rotating!)
    {"height", VWFOFS(fHeight), VF_FLOAT}, // height of traceline or bounding
                                           // box of projecile (non-rotating!)
    {"lifetime", VWFOFS(iLifeTime),
     VF_INT}, // removes itself after this amount of time
    {"explodeOnExpire", VWFOFS(bExplodeOnExpire),
     VF_BOOL}, // when iLifeTime is up, explodes rather than simply removing
               // itself
};
...
static vehField_t *FindVehWeaponParm(const char *parmName) {
  ...
  for (i = 0; i < numVehWeaponFields; i++) {
    if (vehWeaponFields[i].name &&
        !Q_stricmp(vehWeaponFields[i].name, parmName))
      return &vehWeaponFields[i];
  }
  return NULL;
}

static qboolean BG_ParseVehWeaponParm(vehWeaponInfo_t *vehWeapon,
                                      const char *parmName, char *pValue) {
  ...
  case VF_STRING: // string on disk, pointer in memory
    if (!*(char **)(b +
                    vehWeaponField->ofs)) { // just use 1024 bytes in case we
                                            // want to write over the string
#ifdef _JK2MP
      *(char **)(b + vehWeaponField->ofs) =
          (char *)BG_Alloc(1024); //(char *)BG_Alloc(strlen(value));
  ...
  case VF_VECTOR:
    ...
    if (_iFieldsRead != 3) {
      Com_Printf(S_COLOR_YELLOW "BG_ParseVehWeaponParm: VEC3 sscanf() failed "
                                "to read 3 floats ('angle' key bug?)\n");
      VectorClear(vec);
    }
  ...
  case VF_EFFECT_CLIENT: // (MP cgame only) take the string, get the index
    ...
    *(int *)(b + vehWeaponField->ofs) = G_EffectIndex(value);
#elif defined QAGAME
                         //*(int *)(b+vehWeaponField->ofs) = G_EffectIndex(
                         //value );
#elif defined CGAME
    *(int *)(b + vehWeaponField->ofs) = trap_FX_RegisterEffect(value);
#endif
...
#endif
    break;
  case VF_SHADER_NOMIP: // (cgame only) take the string, call
                        // trap_R_RegisterShaderNoMip
#ifndef QAGAME
    *(int *)(b + vehWeaponField->ofs) = trap_R_RegisterShaderNoMip(value);
#endif
...
int VEH_LoadVehWeapon(
    const char *vehWeaponName) { // load up specified vehWeapon and save in
                                 // array: g_vehWeaponInfo
  const char *token;
  ...
  // BG_VehWeaponSetDefaults( &g_vehWeaponInfo[0] );//set the first vehicle to
  // default data
  ...
  while (1) {
    ...
    if (!token[0]) {
      Com_Printf(S_COLOR_RED
                 "ERROR: unexpected EOF while parsing Vehicle Weapon '%s'\n",
                 vehWeaponName);
      COM_EndParseSession();
      return VEH_WEAPON_NONE;
    }
    ...
    if (!value || !value[0]) {
      ...
      if (!BG_ParseVehWeaponParm(vehWeapon, parmName, value)) {
        Com_Printf(S_COLOR_RED
                   "ERROR: Unknown Vehicle Weapon key/value pair '%s','%s'!\n",
                   parmName, value);
      }
    }
  }
  if (vehWeapon
          ->fHoming) { // all lock-on weapons use these 2 sounds
#ifdef QAGAME
...
int VEH_VehWeaponIndexForName(const char *vehWeaponName) {
  ...
  for (vw = VEH_WEAPON_BASE; vw < numVehicleWeapons; vw++) {
    if (g_vehWeaponInfo[vw].name &&
        Q_stricmp(g_vehWeaponInfo[vw].name, vehWeaponName) ==
            0) { // already loaded this one
      return vw;
    }
  }
  ...
  if (vw >= MAX_VEH_WEAPONS) { // no more room!
    Com_Printf(
        S_COLOR_RED
        "ERROR: Too many Vehicle Weapons (max 16), aborting load on %s!\n",
        vehWeaponName);
    return VEH_WEAPON_NONE;
  }
...
vehField_t vehicleFields[] = {
    ...
    {"numHands", VFOFS(numHands),
     VF_INT}, // if 2 hands, no weapons, if 1 hand, can use 1-handed weapons, if
              // 0 hands, can use 2-handed weapons
    {"lookPitch", VFOFS(lookPitch),
     VF_FLOAT}, // How far you can look up and down off the forward of the
                // vehicle
    {"lookYaw", VFOFS(lookYaw),
     VF_FLOAT}, // How far you can look left and right off the forward of the
                // vehicle
    {"length", VFOFS(length), VF_FLOAT}, // how long it is - used for body
                                         // length traces when turning/moving?
    {"width", VFOFS(width), VF_FLOAT}, // how wide it is - used for body length
                                       // traces when turning/moving?
    {"height", VFOFS(height), VF_FLOAT}, // how tall it is - used for body
                                         // length traces when turning/moving?
    {"centerOfGravity", VFOFS(centerOfGravity),
     VF_VECTOR}, // offset from origin: {forward, right, up} as a modifier on
                 // that dimension (-1.0f is all the way back, 1.0f is all the
                 // way forward)
    ...
    {"speedIdle", VFOFS(speedIdle),
     VF_FLOAT}, // what speed it drifts to when no accel/decel input is given
    {"accelIdle", VFOFS(accelIdle),
     VF_FLOAT}, // if speedIdle > 0, how quickly it goes up to that speed
    {"acceleration", VFOFS(acceleration),
     VF_FLOAT}, // when pressing on accelerator
    {"decelIdle", VFOFS(decelIdle),
     VF_FLOAT}, // when giving no input, how quickly it drops to speedIdle
    {"throttleSticks", VFOFS(throttleSticks),
     VF_BOOL}, // if true, speed stays at whatever you accel/decel to, unless
               // you turbo or brake
    {"strafePerc", VFOFS(strafePerc),
     VF_FLOAT}, // multiplier on current speed for strafing.  If 1.0f, you can
                // strafe at the same speed as you're going forward, 0.5 is half,
                // 0 is no strafing

    // handling stats
    {"bankingSpeed", VFOFS(bankingSpeed),
     VF_FLOAT}, // how quickly it pitches and rolls (not under player control)
    {"pitchLimit", VFOFS(pitchLimit),
     VF_FLOAT}, // how far it can roll forward or backward
    {"rollLimit", VFOFS(rollLimit),
     VF_FLOAT},                            // how far it can roll to either side
    ...
    {"turnWhenStopped", VFOFS(turnWhenStopped),
     VF_BOOL}, // whether or not you can turn when not moving
    {"traction", VFOFS(traction),
     VF_FLOAT}, // how much your command input affects velocity
    {"friction", VFOFS(friction),
     VF_FLOAT}, // how much velocity is cut on its own
    {"maxSlope", VFOFS(maxSlope),
     VF_FLOAT}, // the max slope that it can go up with control
    {"speedDependantTurning", VFOFS(speedDependantTurning),
     VF_BOOL}, // vehicle turns faster the faster it's going

    // durability stats
    {"mass", VFOFS(mass),
     VF_INT}, // for momentum and impact force (player mass is 10)
    {"armor", VFOFS(armor), VF_INT},     // total points of damage it can take
    {"shields", VFOFS(shields), VF_INT}, // energy shield damage points
    {"shieldRechargeMS", VFOFS(shieldRechargeMS),
     VF_INT}, // energy shield milliseconds per point recharged
    {"toughness", VFOFS(toughness),
     VF_FLOAT}, // modifies incoming damage, 1.0 is normal, 0.5 is half, etc.
                // Simulates being made of tougher materials/construction
    {"malfunctionArmorLevel", VFOFS(malfunctionArmorLevel),
     VF_INT}, // when armor drops to or below this point, start malfunctioning
    {"surfDestruction", VFOFS(surfDestruction), VF_INT},

    // visuals & sounds
    {"model", VFOFS(model), VF_STRING}, // what model to use - if make it an
                                        // NPC's primary model, don't need this?
    {"skin", VFOFS(skin), VF_STRING}, // what skin to use - if make it an NPC's
                                      // primary model, don't need this?
    {"g2radius", VFOFS(g2radius),
     VF_INT}, // render radius (really diameter, but...) for the ghoul2 model
    {"riderAnim", VFOFS(riderAnim), VF_ANIM}, // what animation the rider uses
    {"droidNPC", VFOFS(droidNPC),
     VF_STRING}, // NPC to attach to *droidunit tag (if it exists in the model)

#ifdef _JK2MP
    {"radarIcon", VFOFS(radarIconHandle),
     VF_SHADER_NOMIP}, // what icon to show on radar in MP
    {"dmgIndicFrame", VFOFS(dmgIndicFrameHandle),
     VF_SHADER_NOMIP}, // what image to use for the frame of the damage
                       // indicator
    {"dmgIndicShield", VFOFS(dmgIndicShieldHandle),
     VF_SHADER_NOMIP}, // what image to use for the shield of the damage
                       // indicator
    {"dmgIndicBackground", VFOFS(dmgIndicBackgroundHandle),
     VF_SHADER_NOMIP}, // what image to use for the background of the damage
                       // indicator
    {"icon_front", VFOFS(iconFrontHandle),
     VF_SHADER_NOMIP}, // what image to use for the front of the ship on the
                       // damage indicator
    {"icon_back", VFOFS(iconBackHandle),
     VF_SHADER_NOMIP}, // what image to use for the back of the ship on the
                       // damage indicator
    {"icon_right", VFOFS(iconRightHandle),
     VF_SHADER_NOMIP}, // what image to use for the right of the ship on the
                       // damage indicator
    {"icon_left", VFOFS(iconLeftHandle),
     VF_SHADER_NOMIP}, // what image to use for the left of the ship on the
                       // damage indicator
    {"crosshairShader", VFOFS(crosshairShaderHandle),
     VF_SHADER_NOMIP}, // what image to use as the crosshair
    {"shieldShader", VFOFS(shieldShaderHandle),
     VF_SHADER}, // What shader to use when drawing the shield shell
    ...
    {"soundTakeOff", VFOFS(soundTakeOff),
     VF_SOUND}, // sound to play when ship takes off
    {"soundEngineStart", VFOFS(soundEngineStart),
     VF_SOUND_CLIENT}, // sound to play when ship's thrusters first activate
    {"soundSpin", VFOFS(soundSpin),
     VF_SOUND}, // sound to loop while spiraling out of control
    {"soundTurbo", VFOFS(soundTurbo),
     VF_SOUND}, // sound to play when turbo/afterburner kicks in
    {"soundHyper", VFOFS(soundHyper),
     VF_SOUND_CLIENT}, // sound to play when hits hyperspace
    {"soundLand", VFOFS(soundLand), VF_SOUND}, // sound to play when ship lands
    {"soundFlyBy", VFOFS(soundFlyBy),
     VF_SOUND_CLIENT}, // sound to play when they buzz you
    {"soundFlyBy2", VFOFS(soundFlyBy2),
     VF_SOUND_CLIENT}, // alternate sound to play when they buzz you
    {"soundShift1", VFOFS(soundShift1),
     VF_SOUND}, // sound to play when changing speeds
    {"soundShift2", VFOFS(soundShift2),
     VF_SOUND}, // sound to play when changing speeds
    {"soundShift3", VFOFS(soundShift3),
     VF_SOUND}, // sound to play when changing speeds
    {"soundShift4", VFOFS(soundShift4),
     VF_SOUND}, // sound to play when changing speeds

    {"exhaustFX", VFOFS(iExhaustFX),
     VF_EFFECT_CLIENT}, // exhaust effect, played from "*exhaust" bolt(s)
    {"turboFX", VFOFS(iTurboFX),
     VF_EFFECT_CLIENT}, // turbo exhaust effect, played from "*exhaust" bolt(s)
                        // when ship is in "turbo" mode
    {"turboStartFX", VFOFS(iTurboStartFX),
     VF_EFFECT}, // turbo start effect, played from "*exhaust" bolt(s) when ship
                 // is in "turbo" mode
    {"trailFX", VFOFS(iTrailFX),
     VF_EFFECT_CLIENT}, // trail effect, played from "*trail" bolt(s)
    {"impactFX", VFOFS(iImpactFX),
     VF_EFFECT_CLIENT}, // impact effect, for when it bumps into something
    {"explodeFX", VFOFS(iExplodeFX),
     VF_EFFECT}, // explosion effect, for when it blows up (should have the
                 // sound built into explosion effect)
    {"wakeFX", VFOFS(iWakeFX),
     VF_EFFECT_CLIENT}, // effect it makes when going across water
    {"dmgFX", VFOFS(iDmgFX),
     VF_EFFECT_CLIENT}, // effect to play on damage from a weapon or something
#ifdef _JK2MP
    {"injureFX", VFOFS(iInjureFX),
     VF_EFFECT_CLIENT}, // effect to play on partially damaged ship surface
    {"noseFX", VFOFS(iNoseFX),
     VF_EFFECT_CLIENT}, // effect for nose piece flying away when blown off
    {"lwingFX", VFOFS(iLWingFX),
     VF_EFFECT_CLIENT}, // effect for left wing piece flying away when blown off
    {"rwingFX", VFOFS(iRWingFX),
     VF_EFFECT_CLIENT}, // effect for right wing piece flying away when blown
                        // off
#else
    {"armorLowFX", VFOFS(iArmorLowFX),
     VF_EFFECT_CLIENT}, // effect to play on damage from a weapon or something
    {"armorGoneFX", VFOFS(iArmorGoneFX),
     VF_EFFECT_CLIENT}, // effect to play on damage from a weapon or something
#endif

    // Weapon stuff:
    {"weap1", VFOFS(weapon[0].ID), VF_WEAPON}, // weapon used when press fire
    {"weap2", VFOFS(weapon[1].ID),
     VF_WEAPON}, // weapon used when press alt-fire
    ...
    // Whether or not to auto-aim the projectiles at the thing under the
    // crosshair when we fire
    ...
    // specified)
    {"weap1SoundNoAmmo", VFOFS(weapon[0].soundNoAmmo),
     VF_SOUND_CLIENT}, // sound to play when try to fire weapon 1 with no ammo
    {"weap2SoundNoAmmo", VFOFS(weapon[1].soundNoAmmo),
     VF_SOUND_CLIENT}, // sound to play when try to fire weapon 2 with no ammo
    ...
    {"hoverHeight", VFOFS(hoverHeight),
     VF_FLOAT}, // if 0, it's a ground vehicle
    {"hoverStrength", VFOFS(hoverStrength),
     VF_FLOAT}, // how hard it pushes off ground when less than hover height...
                // causes "bounce", like shocks
    {"waterProof", VFOFS(waterProof),
     VF_BOOL}, // can drive underwater if it has to
    {"bouyancy", VFOFS(bouyancy),
     VF_FLOAT}, // when in water, how high it floats (1 is neutral bouyancy)
    ...
    {"turboRecharge", VFOFS(turboRecharge),
     VF_INT}, // how long turbo takes to recharge
    ...
    // new stuff
    {"maxPassengers", VFOFS(maxPassengers),
     VF_INT}, // The max number of passengers this vehicle may have (Default =
              // 0).
    {"hideRider", VFOFS(hideRider),
     VF_BOOL}, // rider (and passengers?) should not be drawn
    {"killRiderOnDeath", VFOFS(killRiderOnDeath),
     VF_BOOL}, // if rider is on vehicle when it dies, they should die
    {"flammable", VFOFS(flammable),
     VF_BOOL}, // whether or not the vehicle should catch on fire before it
               // explodes
    {"explosionDelay", VFOFS(explosionDelay),
     VF_INT}, // how long the vehicle should be on fire/dying before it explodes
    // camera stuff
    {"cameraOverride", VFOFS(cameraOverride),
     VF_BOOL}, // override the third person camera with the below values -
               // normal is 0 (off)
    {"cameraRange", VFOFS(cameraRange),
     VF_FLOAT}, // how far back the camera should be - normal is 80
    {"cameraVertOffset", VFOFS(cameraVertOffset),
     VF_FLOAT}, // how high over the vehicle origin the camera should be -
                // normal is 16
    {"cameraHorzOffset", VFOFS(cameraHorzOffset),
     VF_FLOAT}, // how far to left/right (negative/positive) of of the vehicle
                // origin the camera should be - normal is 0
    {"cameraPitchOffset", VFOFS(cameraPitchOffset),
     VF_FLOAT}, // a modifier on the camera's pitch (up/down angle) to the
                // vehicle - normal is 0
    {"cameraFOV", VFOFS(cameraFOV),
     VF_FLOAT}, // third person camera FOV, default is 80
    {"cameraAlpha", VFOFS(cameraAlpha),
     VF_FLOAT}, // fade out the vehicle to this alpha (0.1-1.0f) if it's in the
                // way of the crosshair
    {"cameraPitchDependantVertOffset", VFOFS(cameraPitchDependantVertOffset),
     VF_BOOL}, // use the hacky AT-ST pitch dependant vertical offset
    //===TURRETS===========================================================================
    // Turret 1
    ...
    {"turret1ClampYawL", VFOFS(turret[0].yawClampLeft),
     VF_FLOAT}, // how far the turret is allowed to turn left
    {"turret1ClampYawR", VFOFS(turret[0].yawClampRight),
     VF_FLOAT}, // how far the turret is allowed to turn right
    {"turret1ClampPitchU", VFOFS(turret[0].pitchClampUp),
     VF_FLOAT}, // how far the turret is allowed to title up
    {"turret1ClampPitchD", VFOFS(turret[0].pitchClampDown),
     VF_FLOAT}, // how far the turret is allowed to tilt down
    ...
    {"turret1PassengerNum", VFOFS(turret[0].passengerNum),
     VF_INT}, // which number passenger can control this turret
    ...
    {"turret2ClampYawL", VFOFS(turret[1].yawClampLeft),
     VF_FLOAT}, // how far the turret is allowed to turn left
    {"turret2ClampYawR", VFOFS(turret[1].yawClampRight),
     VF_FLOAT}, // how far the turret is allowed to turn right
    {"turret2ClampPitchU", VFOFS(turret[1].pitchClampUp),
     VF_FLOAT}, // how far the turret is allowed to title up
    {"turret2ClampPitchD", VFOFS(turret[1].pitchClampDown),
     VF_FLOAT}, // how far the turret is allowed to tilt down
    ...
    {"turret2PassengerNum", VFOFS(turret[1].passengerNum),
     VF_INT}, // which number passenger can control this turret
...
stringID_table_t VehicleTable[VH_NUM_VEHICLES + 1] = {
    ENUM2STRING(VH_NONE),
    ENUM2STRING(VH_WALKER), // something you ride inside of, it walks like you,
                            // like an AT-ST
    ENUM2STRING(VH_FIGHTER), // something you fly inside of, like an X-Wing or
                             // TIE fighter
    ENUM2STRING(VH_SPEEDER), // something you ride on that hovers, like a
                             // speeder or swoop
    ENUM2STRING(
...
void BG_VehicleSetDefaults(vehicleInfo_t *vehicle) {
  ...
  /*
          ...
          //general data
          vehicle->type = VH_SPEEDER;				//what kind of
  vehicle
          //FIXME: no saber or weapons if numHands = 2, should switch to speeder
  weapon, no attack anim on player vehicle->numHands = 0;
  //if 2 hands, no weapons, if 1 hand, can use 1-handed weapons, if 0 hands, can
  use 2-handed weapons
          vehicle->lookPitch = 0;				//How far you can look
  up and down off the forward of the vehicle vehicle->lookYaw = 5;
  //How far you can look left and right off the forward of the vehicle
          vehicle->length = 0;					//how long it is -
  used for body length traces when turning/moving? vehicle->width = 0;
  //how wide it is - used for body length traces when turning/moving?
          vehicle->height = 0;					//how tall it is -
  used for body length traces when turning/moving? VectorClear(
  vehicle->centerOfGravity );//offset from origin: {forward, right, up} as a
  modifier on that dimension (-1.0f is all the way back, 1.0f is all the way
  forward)

          //speed stats - note: these are DESIRED speed, not actual current
  speed/velocity vehicle->speedMax = VEH_DEFAULT_SPEED_MAX;	//top speed
          vehicle->turboSpeed = 0; //turboBoost vehicle->speedMin = 0;
  //if < 0, can go in reverse
          vehicle->speedIdle = 0;						//what
  speed it drifts to when no accel/decel input is given vehicle->accelIdle = 0;
  //if speedIdle > 0, how quickly it goes up to that speed vehicle->acceleration
  = VEH_DEFAULT_ACCEL;	//when pressing on accelerator (1/2 this when going in
  reverse) vehicle->decelIdle = VEH_DEFAULT_DECEL;		//when giving no
  input, how quickly it desired speed drops to speedIdle vehicle->strafePerc =
  VEH_DEFAULT_STRAFE_PERC;//multiplier on current speed for strafing.  If 1.0f,
  you can strafe at the same speed as you're going forward, 0.5 is half, 0 is no
  strafing

          //handling stats
          vehicle->bankingSpeed = VEH_DEFAULT_BANKING_SPEED;	//how quickly it
  pitches and rolls (not under player control) vehicle->rollLimit =
  VEH_DEFAULT_ROLL_LIMIT;		//how far it can roll to either side
          vehicle->pitchLimit = VEH_DEFAULT_PITCH_LIMIT;		//how far it
  can pitch forward or backward vehicle->braking = VEH_DEFAULT_BRAKING;
  //when pressing on decelerator (backwards) vehicle->turningSpeed =
  VEH_DEFAULT_TURNING_SPEED;	//how quickly you can turn
          vehicle->turnWhenStopped = qfalse;
  //whether or not you can turn when not moving vehicle->traction =
  VEH_DEFAULT_TRACTION;			//how much your command input affects
  velocity
          vehicle->friction = VEH_DEFAULT_FRICTION;			//how much
  velocity is cut on its own vehicle->maxSlope = VEH_DEFAULT_MAX_SLOPE;
  //the max slope that it can go up with control

          //durability stats
          vehicle->mass = VEH_DEFAULT_MASS;			//for momentum and
  impact force (player mass is 10) vehicle->armor = VEH_DEFAULT_MAX_ARMOR;
  //total points of damage it can take vehicle->toughness =
  VEH_DEFAULT_TOUGHNESS;	//modifies incoming damage, 1.0 is normal, 0.5 is
  half, etc.  Simulates being made of tougher materials/construction
          vehicle->malfunctionArmorLevel = 0;			//when armor
  drops to or below this point, start malfunctioning

          //visuals & sounds
          //vehicle->model = "models/map_objects/ships/swoop.md3";	//what
  model to use - if make it an NPC's primary model, don't need this? if
  (!vehicle->model)
          {
  ...
  //what skin to use - if make it an NPC's primary model, don't need this?
          vehicle->riderAnim = BOTH_GUNSIT1;					//what
  animation the rider uses

          vehicle->soundOn = NULL;
  //sound to play when get on it vehicle->soundLoop = NULL;
  //sound to loop while riding it vehicle->soundOff = NULL;
  //sound to play when get off
          vehicle->exhaustFX = NULL;
  //exhaust effect, played from "*exhaust" bolt(s) vehicle->trailFX = NULL;
  //trail effect, played from "*trail" bolt(s) vehicle->impactFX = NULL;
  //explosion effect, for when it blows up (should have the sound built into
  explosion effect) vehicle->explodeFX = NULL;
  //explosion effect, for when it blows up (should have the sound built into
  explosion effect) vehicle->wakeFX = NULL;
  //effect itmakes when going across water

          //other misc stats
          vehicle->gravity = VEH_DEFAULT_GRAVITY;
  //normal is 800 vehicle->hoverHeight = 0;//VEH_DEFAULT_HOVER_HEIGHT;	//if 0,
  it's a ground vehicle vehicle->hoverStrength =
  0;//VEH_DEFAULT_HOVER_STRENGTH;//how hard it pushes off ground when less than
  hover height... causes "bounce", like shocks vehicle->waterProof = qtrue;
  //can drive underwater if it has to vehicle->bouyancy = 1.0f;
  //when in water, how high it floats (1 is neutral bouyancy) vehicle->fuelMax =
  1000;							//how much fuel it can hold
  (capacity)
          vehicle->fuelRate = 1;
  //how quickly is uses up fuel vehicle->visibility = VEH_DEFAULT_VISIBILITY;
  //radius for sight alerts vehicle->loudness = VEH_DEFAULT_LOUDNESS;
  //radius for sound alerts vehicle->explosionRadius = VEH_DEFAULT_EXP_RAD;
          vehicle->explosionDamage = VEH_DEFAULT_EXP_DMG;
          vehicle->maxPassengers = 0;

          //new stuff
          vehicle->hideRider = qfalse;						//
  rider (and passengers?) should not be drawn vehicle->killRiderOnDeath =
  qfalse;					//if rider is on vehicle when it dies,
  they should die vehicle->flammable = qfalse;
  //whether or not the vehicle should catch on fire before it explodes
          vehicle->explosionDelay = 0;						//how
  long the vehicle should be on fire/dying before it explodes
          //camera stuff
          vehicle->cameraOverride = qfalse;
  //whether or not to use all of the following 3rd person camera override values
          vehicle->cameraRange = 0.0f;						//how
  far back the camera should be - normal is 80 vehicle->cameraVertOffset = 0.0f;
  //how high over the vehicle origin the camera should be - normal is 16
          vehicle->cameraHorzOffset = 0.0f;					//how
  far to left/right (negative/positive) of of the vehicle origin the camera
  should be - normal is 0 vehicle->cameraPitchOffset = 0.0f;
  //a modifier on the camera's pitch (up/down angle) to the vehicle - normal is
  0
          vehicle->cameraFOV = 0.0f;
  //third person camera FOV, default is 80 vehicle->cameraAlpha = qfalse;
  //fade out the vehicle if it's in the way of the crosshair
  */
}

void BG_VehicleClampData(
    vehicleInfo_t *vehicle) { // sanity check and clamp the vehicle's data
  int i;
...
static qboolean BG_ParseVehicleParm(vehicleInfo_t *vehicle,
                                    const char *parmName, char *pValue) {
  vehField_t *vehField;
  vec3_t vec;
  byte *b = (byte *)vehicle;
  ...
  case VF_FLOAT:
    *(float *)(b + vehField->ofs) = atof(value);
    break;
  case VF_STRING:                         // string on disk, pointer in memory
    if (!*(char **)(b + vehField->ofs)) { // just use 128 bytes in case we want
                                          // to write over the string
#ifdef _JK2MP
      *(char **)(b + vehField->ofs) =
          (char *)BG_Alloc(128); //(char *)BG_Alloc(strlen(value));
  ...
  case VF_VECTOR:
    ...
    // assert(_iFieldsRead==3 );
    if (_iFieldsRead != 3) {
      Com_Printf(S_COLOR_YELLOW "BG_ParseVehicleParm: VEC3 sscanf() failed to "
                                "read 3 floats ('angle' key bug?)\n");
      VectorClear(vec);
    }
...
#endif
    break;
  case VF_EFFECT_CLIENT: // (MP cgame only) take the string, get the
                         // G_EffectIndex
#ifndef _JK2MP
    *(int *)(b + vehField->ofs) = G_EffectIndex(value);
#elif defined QAGAME
...
int VEH_LoadVehicle(const char *vehicleName) { // load up specified vehicle and
                                               // save in array: g_vehicleInfo
  const char *token;
  ...
  while (1) {
    ...
    if (!token[0]) {
      Com_Printf(S_COLOR_RED
                 "ERROR: unexpected EOF while parsing Vehicle '%s'\n",
                 vehicleName);
      COM_EndParseSession();
      return VEHICLE_NONE;
    }
    ...
    if (!value || !value[0]) {
      Com_Printf(S_COLOR_RED "ERROR: Vehicle token '%s' has no value!\n",
               ...
               0) { // hmm, store this off because we don't want to call another
                    // one of these text parsing routines while we're in the
                    // middle of one...
...
#ifndef FINAL_BUILD
        Com_Printf(S_COLOR_RED
                   "ERROR: Unknown Vehicle key/value pair '%s', '%s'!\n",
                   parmName, value);
#endif
      }
    }
...
#ifndef FINAL_BUILD
      Com_Printf(S_COLOR_RED
                 "ERROR: Unknown Vehicle key/value pair 'weap1', '%s'!\n",
                 weap1);
#endif
    }
  }
...
#ifndef FINAL_BUILD
      Com_Printf(S_COLOR_RED
                 "ERROR: Unknown Vehicle key/value pair 'weap2', '%s'!\n",
                 weap2);
#endif
    }
  }
...
#ifndef FINAL_BUILD
      Com_Printf(S_COLOR_RED
                 "ERROR: Unknown Vehicle key/value pair 'weapMuzzle1', '%s'!\n",
                 weapMuzzle1);
#endif
    }
  }
...
#ifndef FINAL_BUILD
      Com_Printf(S_COLOR_RED
                 "ERROR: Unknown Vehicle key/value pair 'weapMuzzle2', '%s'!\n",
                 weapMuzzle2);
#endif
    }
  }
...
#ifndef FINAL_BUILD
      Com_Printf(S_COLOR_RED
                 "ERROR: Unknown Vehicle key/value pair 'weapMuzzle3', '%s'!\n",
                 weapMuzzle3);
#endif
    }
  }
...
#ifndef FINAL_BUILD
      Com_Printf(S_COLOR_RED
                 "ERROR: Unknown Vehicle key/value pair 'weapMuzzle4', '%s'!\n",
                 weapMuzzle4);
#endif
    }
  }
...
#ifndef FINAL_BUILD
      Com_Printf(S_COLOR_RED
                 "ERROR: Unknown Vehicle key/value pair 'weapMuzzle5', '%s'!\n",
                 weapMuzzle5);
#endif
    }
  }
...
#ifndef FINAL_BUILD
      Com_Printf(S_COLOR_RED
                 "ERROR: Unknown Vehicle key/value pair 'weapMuzzle6', '%s'!\n",
                 weapMuzzle6);
#endif
    }
  }
...
#ifndef FINAL_BUILD
      Com_Printf(S_COLOR_RED
                 "ERROR: Unknown Vehicle key/value pair 'weapMuzzle7', '%s'!\n",
                 weapMuzzle7);
#endif
    }
  }
...
#ifndef FINAL_BUILD
      Com_Printf(S_COLOR_RED
                 "ERROR: Unknown Vehicle key/value pair 'weapMuzzle8', '%s'!\n",
                 weapMuzzle8);
#endif
    }
  }
...
#ifndef FINAL_BUILD
      Com_Printf(S_COLOR_RED
                 "ERROR: Unknown Vehicle key/value pair 'weapMuzzle9', '%s'!\n",
                 weapMuzzle9);
#endif
    }
  }
  if (weapMuzzle10[0]) {
    if (!BG_ParseVehicleParm(vehicle, "weapMuzzle10", weapMuzzle10)) {
      ...
      Com_Printf(
          S_COLOR_RED
          "ERROR: Unknown Vehicle key/value pair 'weapMuzzle10', '%s'!\n",
          weapMuzzle10);
#endif
    }
  }
...
#ifdef QAGAME
    vehicle->modelIndex =
        G_ModelIndex(va("models/players/%s/model.glm", vehicle->model));
#else
    vehicle->modelIndex =
        trap_R_RegisterModel(va("models/players/%s/model.glm", vehicle->model));
#endif
  }
  ...
  if (vehicle->skin && vehicle->skin[0]) {
    ...
    for (ratl::string_vs<256>::tokenizer i = skins.begin("|"); i != skins.end();
         i++) {
      // this will just turn off surfs if there is a *off shader on a surf, the
      // skin will actually get thrown away when cgame starts
      gi.RE_RegisterSkin(
          va("models/players/%s/model_%s.skin", vehicle->model, *i));
      // this is for the server-side call, it will propgate down to cgame with
      // configstrings and register it at the same time as all the other skins
      // for ghoul2 models
    ...
    // skin will actually get thrown away when cgame starts
    gi.RE_RegisterSkin(
        va("models/players/%s/model_default.skin", vehicle->model));
    // this is for the server-side call, it will propgate down to cgame with
    // configstrings and register it at the same time as all the other skins for
    // ghoul2 models
  ...
  if (vehicle->skin && vehicle->skin[0]) {
    trap_R_RegisterSkin(
        va("models/players/%s/model_%s.skin", vehicle->model, vehicle->skin));
  }
#endif
#endif
...
int VEH_VehicleIndexForName(const char *vehicleName) {
  ...
  for (v = VEHICLE_BASE; v < numVehicles; v++) {
    if (g_vehicleInfo[v].name &&
        Q_stricmp(g_vehicleInfo[v].name, vehicleName) ==
            0) { // already loaded this one
      return v;
    }
  }
  ...
  if (v >= MAX_VEHICLES) { // no more room!
    Com_Printf(S_COLOR_RED
               "ERROR: Too many Vehicles (max 64), aborting load on %s!\n",
               vehicleName);
    return VEHICLE_NONE;
  }
  // we have room for another one, load it up and return the index
...
#ifdef _JK2MP
  fileCnt = trap_FS_GetFileList("ext_data/vehicles/weapons", ".vwp",
                                vehWeaponExtensionListBuf,
                                sizeof(vehWeaponExtensionListBuf));
#else
  fileCnt = gi.FS_GetFileList("ext_data/vehicles/weapons", ".vwp",
                              vehWeaponExtensionListBuf,
                              sizeof(vehWeaponExtensionListBuf));
#endif
...
#else
  tempReadBuffer =
      (char *)gi.Malloc(MAX_VEH_WEAPON_DATA_SIZE, TAG_G_ALLOC, qtrue);
#endif
...
#ifdef _JK2MP
    len = trap_FS_FOpenFile(va("ext_data/vehicles/weapons/%s", holdChar), &f,
                            FS_READ);
#else
    //		len = gi.FS_ReadFile( va( "ext_data/vehicles/weapons/%s", holdChar),
    //(void **) &buffer );
    len = gi.FS_FOpenFile(va("ext_data/vehicles/weapons/%s", holdChar), &f,
                          FS_READ);
#endif
...
void BG_VehicleLoadParms(
    void) { // HMM... only do this if there's a vehicle on the level?
  int len, totallen, vehExtFNLen, mainBlockLen, fileCnt, i;
...
#ifdef _JK2MP
  fileCnt =
      trap_FS_GetFileList("ext_data/vehicles", ".veh", vehExtensionListBuf,
                          sizeof(vehExtensionListBuf));
#else
  fileCnt = gi.FS_GetFileList("ext_data/vehicles", ".veh", vehExtensionListBuf,
                              sizeof(vehExtensionListBuf));
#endif
...
#else
    //		len = gi.FS_ReadFile( va( "ext_data/vehicles/%s", holdChar), (void **)
    //&buffer );
...
void BG_GetVehicleModelName(char *modelname) {
  ...
  if (vIndex == VEHICLE_NONE) {
    Com_Error(ERR_DROP, "BG_GetVehicleModelName:  couldn't find vehicle %s",
              vehName);
  }
...
void BG_GetVehicleSkinName(char *skinname) {
  ...
  if (vIndex == VEHICLE_NONE) {
    Com_Error(ERR_DROP, "BG_GetVehicleSkinName:  couldn't find vehicle %s",
              vehName);
  }
...
#ifdef _JK2MP
#ifndef WE_ARE_IN_THE_UI
// so cgame can assign the function pointer for the vehicle attachment without
// having to bother with all the other funcs that don't really exist cgame-side.
extern int BG_GetTime(void);
extern int trap_G2API_AddBolt(void *ghoul2, int modelIndex,
                              const char *boneName);
extern qboolean trap_G2API_GetBoltMatrix(
