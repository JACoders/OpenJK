...
static void UI_AddWeaponSelection(const int weaponIndex, const int ammoIndex,
...
static void
UI_AddThrowWeaponSelection(const int weaponIndex, const int ammoIndex,
                           const int ammoAmount, const char *iconItemName,
                           const char *litIconItemName,
                           const char *hexBackground, const char *soundfile);
...
static void UI_OwnerDraw(float x, float y, float w, float h, float text_x,
                         ...
                         qhandle_t shader, int textStyle, int iFontIndex);
...
static datpadmovedata_t datapadMoveData[MD_MOVE_TITLE_MAX][MAX_MOVES] = {
    {
        ...
        {"@MENUS_WALL_RUN", "@MENUS_WALL_RUN_DESC", "BOTH_WALL_RUN_RIGHT",
         MDS_FORCE_JUMP},
        {"@MENUS_LONG_JUMP", "@MENUS_LONG_JUMP_DESC",
         "BOTH_FORCELONGLEAP_START", MDS_FORCE_JUMP},
...

    {
        ...
        {"@MENUS_LUNGE_ATTACK", "@MENUS_LUNGE_ATTACK_DESC", "BOTH_LUNGE2_B__T_",
         MDS_SABER},
        {"@MENUS_FORCE_PULL_IMPALE", "@MENUS_FORCE_PULL_IMPALE_DESC",
         "BOTH_PULL_IMPALE_STAB", MDS_SABER},
...

    {
        ...
        {"@MENUS_FORCE_PULL_SLASH", "@MENUS_FORCE_PULL_SLASH_DESC",
         "BOTH_PULL_IMPALE_SWING", MDS_SABER},
...

    {
        ...
        {"@MENUS_JUMP_KICK", "@MENUS_JUMP_KICK_DESC", "BOTH_A7_KICK_F_AIR",
         MDS_FORCE_JUMP},
        {"@MENUS_SPLIT_KICK", "@MENUS_SPLIT_KICK_DESC", "BOTH_A7_KICK_RL",
         MDS_FORCE_JUMP},
        {"@MENUS_SPIN_KICK", "@MENUS_SPIN_KICK_DESC", "BOTH_A7_KICK_S",
         MDS_FORCE_JUMP},
        {"@MENUS_FLIP_KICK", "@MENUS_FLIP_KICK_DESC", "BOTH_A7_KICK_BF",
         MDS_FORCE_JUMP},
...
static void UI_UpdateScreenshot(void) {
  ...
  if (ui_screenshotType.string[0] && isalpha(ui_screenshotType.string[0])) {
    ...
    if (ssf == -1) {
      ui.Printf(
          "UI Screenshot Format Type '%s' unrecognised, defaulting to JPEG\n",
          ui_screenshotType.string);
      uiInfo.uiDC.screenshotFormat = SSF_JPEG;
      changed = qtrue;
    } else
      uiInfo.uiDC.screenshotFormat = ssf;
  } else if (ui_screenshotType.integer < SSF_JPEG ||
             ui_screenshotType.integer > SSF_PNG) {
  ...
  if (changed) {
    Cvar_Set("ui_screenshotType",
             UI_GetScreenshotFormatString(uiInfo.uiDC.screenshotFormat));
...
static cvarTable_t cvarTable[] = {
    ...
    {&ui_char_model, "ui_char_model", "", NULL,
     0}, // these are filled in by the "g_*" versions on load
    {&ui_char_skin_head, "ui_char_skin_head", "", NULL,
     0}, // the "g_*" versions are initialized in UI_Init, ui_atoms.cpp
    ...
    {&ui_screenshotType, "ui_screenshotType", "jpg", UI_UpdateScreenshot,
     CVAR_ARCHIVE}};
...
void _UI_Refresh(int realtime) {
  ...
  if (Menus_AnyFullScreenVisible()) { // if not in full screen, don't mess with
                                      // ghoul2
    // rww - ghoul2 needs to know what time it is even if the client/server are
    // not running
...
================
*/
extern "C" Q_EXPORT intptr_t QDECL vmMain(int command, int arg0, int arg1,
...
*/
/*
static void Text_PaintChar(float x, float y, float width, float height, float
scale, float s, float t, float s2, float t2, qhandle_t hShader)
{
        float w, h;

        w = width * scale;
        h = height * scale;
        ui.R_DrawStretchPic((int)x, (int)y, w, h, s, t, s2, t2, hShader );
//make the coords (int) or else the chars bleed
}
*/
...
================
*/
// iMaxPixelWidth is 0 here for no limit (but gets converted to -1), else max
// printable pixel width relative to start pos
//
void Text_Paint(float x, float y, float scale, vec4_t color, const char *text,
                int iMaxPixelWidth, int style, int iFontIndex) {
  if (iFontIndex == 0) {
  ...
  int iStyleOR = 0;
  switch (style) {
    //	case  ITEM_TEXTSTYLE_NORMAL:			iStyleOR = 0;break;					// JK2
    //normal text
    //	case  ITEM_TEXTSTYLE_BLINK:				iStyleOR = STYLE_BLINK;break;		//
    //JK2 fast blinking
...
void Text_PaintWithCursor(float x, float y, float scale, vec4_t color,
  ...
  //
  char sTemp[1024];
  int iCopyCount = iMaxPixelWidth > 0 ? Q_min((int)strlen(text), iMaxPixelWidth)
                                      : (int)strlen(text);
  ...
  Text_Paint(x + iNextXpos, y, scale, color, va("%c", cursor), iMaxPixelWidth,
             style | ITEM_TEXTSTYLE_BLINK, iFontIndex);
}
  ...
  *handle = -1;
  ...
  if (feederID == FEEDER_SAVEGAMES) {
    ...
    if (index >= 0 &&
      ...
      *handle = ui.R_RegisterShaderNoMip(
          va("models/players/%s/icon_%s.jpg",
    ...
    if (index >= 0 &&
      ...
      *handle = ui.R_RegisterShaderNoMip(va(
          "models/players/%s/icon_%s.jpg",
          uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name,
...
qhandle_t UI_FeederItemImage(float feederID, int index) {
  if (feederID == FEEDER_PLAYER_SKIN_HEAD) {
    if (index >= 0 &&
      ...
      return ui.R_RegisterShaderNoMip(
          va("models/players/%s/icon_%s.jpg",
             uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name,
             uiInfo.playerSpecies[uiInfo.playerSpeciesIndex]
                 .SkinHead[index]
    ...
    if (index >= 0 &&
      ...
      return ui.R_RegisterShaderNoMip(va(
          "models/players/%s/icon_%s.jpg",
          uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name,
          uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinLeg[index].name));
    }
...
static qboolean UI_RunMenuScript(const char **args) {
  ...
  if (String_Parse(args, &name)) {
    if (Q_stricmp(name, "resetdefaults") == 0) {
      ...
      if (s_savedata[s_savegame.currentLine]
              .currentSaveFileName) // && (*s_file_desc_field.field.buffer))
      {
        Menus_CloseAll();
        ui.Cmd_ExecuteText(
            EXEC_APPEND,
      ...
      // after loading a game, the list box (and it's highlight) get's reset
      // back to 0, but currentLine sticks around, so set it to 0 here
      s_savegame.currentLine = 0;

    } else if (Q_stricmp(name, "deletegame") == 0) {
      if (s_savedata[s_savegame.currentLine]
              .currentSaveFileName) // A line was chosen
      {
#ifndef FINAL_BUILD
        ...
        if ((s_savegame.currentLine > 0) &&
            ((s_savegame.currentLine + 1) == s_savegame.saveFileCnt)) {
      ...
      //			if (
      //!s_savedata[s_savegame.currentLine].currentSaveFileName)	// No
      //line was chosen
      {
        CreateNextSaveName(fileName); // Get a name to save to
      }
      //			else	// Overwrite a current save game? Ask
      //first.
      {
        //				s_savegame.yes.generic.flags	=
        //QMF_HIGHLIGHT_IF_FOCUS; 				s_savegame.no.generic.flags		=
        //QMF_HIGHLIGHT_IF_FOCUS;

        //				strcpy(fileName,s_savedata[s_savegame.currentLine].currentSaveFileName);
        //				s_savegame.awaitingSave = qtrue;
        //				s_savegame.deletegame.generic.flags	=
        //QMF_GRAYED;	// Turn off delete button 				break;
      }

      // Save description line
      ui.Cvar_VariableStringBuffer("ui_gameDesc", description,
                                   sizeof(description));
      ...
      ui.Cmd_ExecuteText(EXEC_APPEND, "dpinvprev\n");
               ...
               0) // Warn user data has changed before leaving screen?
    {
    ...
    //		}
      ...
      if (menu) {
        ...
        if (item) {
          ...
          if (modelPtr) {
            ...
            DC->g2hilev_SetAnim(&item->ghoul2[0], "model_root",
      ...
      // NOTE : ONLY do this if saber menu is set properly (ie. first time we
      // enter this menu)
      if (!Cvar_VariableIntegerValue("saber_menu")) {
        UI_ResetSaberCvars();
      }
    ...
    // initialize weapon selection screen
      ...
      UI_AddWeaponSelection(atoi(weaponIndex), atoi(ammoIndex),
                            atoi(ammoAmount), itemName, litItemName,
      ...
      UI_AddThrowWeaponSelection(atoi(weaponIndex), atoi(ammoIndex),
                                 atoi(ammoAmount), itemName, litItemName,
                                 backgroundName, soundfile);
...
static void UI_CalcForceStatus(void) {
  ...
  lightSide =
      pState->forcePowerLevel[FP_HEAL] + pState->forcePowerLevel[FP_TELEPATHY] +
      pState->forcePowerLevel[FP_PROTECT] + pState->forcePowerLevel[FP_ABSORB];

  darkSide =
      pState->forcePowerLevel[FP_GRIP] + pState->forcePowerLevel[FP_LIGHTNING] +
      pState->forcePowerLevel[FP_RAGE] + pState->forcePowerLevel[FP_DRAIN];
  ...
  if (lukeFlag) {
    DC->startLocalSound(DC->registerSound(lukeForceStatusSounds[index], qfalse),
                        CHAN_VOICE);
  } else {
    DC->startLocalSound(DC->registerSound(kyleForceStatusSounds[index], qfalse),
                        CHAN_VOICE);
  }
}
#endif // !JK2_MODE
...
static void UI_StopCinematic(int handle) {
  if (handle >= 0) {
    ...
    if (handle == UI_MAPCINEMATIC) {
      ...
      //			if
      //(uiInfo.mapList[ui_currentMap.integer].cinematic >= 0)
      //			{
      ...
      // FIXME - BOB do we need this?
      //			int i =
      //UI_TeamIndexFromName(UI_Cvar_VariableString("ui_teamName")); 			if (i >= 0
      //&& i < uiInfo.teamCount)
      //			{
...
static void UI_HandleLoadSelection() {
  Cvar_Set("ui_SelectionOK",
           va("%d", (s_savegame.currentLine < s_savegame.saveFileCnt)));
  if (s_savegame.currentLine >= s_savegame.saveFileCnt)
    return;
#ifdef JK2_MODE
  Cvar_Set("ui_gameDesc", s_savedata[s_savegame.currentLine]
                              .currentSaveFileComments); // set comment

  if (!ui.SG_GetSaveImage(
          s_savedata[s_savegame.currentLine].currentSaveFileName,
          &screenShotBuf)) {
...
// unfortunately we cannot rely on any game/cgame module code to do our
// animation stuff, because the ui can be loaded while the game/cgame are not
// loaded. So we're going to recreate what we need here.
...
qboolean UI_ParseAnimFileSet(
    const char *animCFG,
    int *animFileIndex) { // Not going to bother parsing the sound config here.
  ...
  // if this anims file was loaded before, don't parse it again, just point to
  // the correct table of info
  ...
  if (ui_numKnownAnimFileSets == MAX_ANIM_FILES) { // TOO MANY!
    for (i = 0; i < MAX_ANIM_FILES; i++) {
      Com_Printf("animfile[%d]: %s\n", i, ui_knownAnimFileSets[i].filename);
    }
    Com_Error(ERR_FATAL, "UI_ParseAnimFileSet: %d == MAX_ANIM_FILES == %d",
              ui_numKnownAnimFileSets, MAX_ANIM_FILES);
  }

  // Okay, time to parse in a new one
  Q_strncpyz(ui_knownAnimFileSets[ui_numKnownAnimFileSets].filename,
             strippedName,
...
int UI_G2SetAnim(CGhoul2Info *ghlInfo, const char *boneName, int animNum,
                 const qboolean freeze) {
  ...
  if (animIndex != -1) {
    ...
    re.G2API_SetBoneAnim(ghlInfo, boneName, sFrame, eFrame, flags, animSpeed,
                         time, -1, blendTime);
...
=================
*/
static qboolean IsImageFile(const char *dirptr, const char *skinname,
                            qboolean building) {
  ...
  if (f) {
    ui.FS_FCloseFile(f);
    if (building)
      ui.R_RegisterShaderNoMip(fpath);
    return qtrue;
  }
...
static void UI_BuildPlayerModel_List(qboolean inGameLoad) {
  ...
  for (i = 0; i < numdirs; i++, dirptr += dirlen + 1) {
    ...
    if (f) {
      ...
      numfiles = ui.FS_GetFileList(va("models/players/%s", dirptr), ".skin",
                                   filelist, sizeof(filelist));
      fileptr = filelist;
      for (j = 0; j < numfiles; j++, fileptr += filelen + 1) {
        if (building) {
          ui.FS_FOpenFile(va("models/players/%s/%s", dirptr, fileptr), &f,
                          FS_READ);
          if (f)
            ui.FS_FCloseFile(f);
          ui.FS_FOpenFile(va("models/players/%s/sounds.cfg", dirptr), &f,
                          FS_READ);
          if (f)
            ui.FS_FCloseFile(f);
          ui.FS_FOpenFile(va("models/players/%s/animevents.cfg", dirptr), &f,
                          FS_READ);
          if (f)
            ui.FS_FCloseFile(f);
        }
        ...
        if (IsImageFile(dirptr, skinname,
                        (qboolean)(building != 0))) { // if it exists
...
=================
*/
void UI_Shutdown(void) { UI_FreeAllSpecies(); }
...
void _UI_Init(qboolean inGameLoad) {
  ...
  uiInfo.languageCount =
      SE_GetNumLanguages(); // this does a dir scan, so use carefully
#else
  // sod it, parse every menu strip file until we find a gap in the sequence...
  //
  for (int i = 0; i < 10; i++) {
    if (!ui.SP_Register(va("menus%d", i),
                        /*SP_REGISTER_REQUIRED|*/ SP_REGISTER_MENU))
      break;
  }
#endif
...
qboolean Asset_Parse(char **buffer) {
  ...
  while (1) {
    ...
    // fonts
    if (Q_stricmp(token, "smallFont") ==
        0) // legacy, really it only matters which order they are registered
    {
      ...
      if (PC_ParseInt(&pointSize)) {
        //				PC_ParseWarning("Bad 2nd parameter for keyword
        //'smallFont'");
      }
    ...
    if (Q_stricmp(token, "stripedFile") == 0) {
      ...
      if (!ui.SP_Register(sTemp, /*SP_REGISTER_REQUIRED|*/ SP_REGISTER_MENU)) {
        ...
        // return qfalse;	// hmmm... dunno about this, don't want to break
        // scripts for just missing subtitles
      } else {
    ...
    // Chose a force power from the ingame force allocation screen (the one
    // where you get to allocate a force power point)
    if (Q_stricmp(token, "forceChosenSound") == 0) {
      ...
      uiInfo.uiDC.Assets.forceChosenSound =
          trap_S_RegisterSound(tempStr, qfalse);
      continue;
    }

    // Unchose a force power from the ingame force allocation screen (the one
    // where you get to allocate a force power point)
    if (Q_stricmp(token, "forceUnchosenSound") == 0) {
      ...
      uiInfo.uiDC.Assets.forceUnchosenSound =
          trap_S_RegisterSound(tempStr, qfalse);
      continue;
    }
    ...
    if (Q_stricmp(token, "datapadmoveRollSound") == 0) {
      ...
      uiInfo.uiDC.Assets.datapadmoveRollSound =
          trap_S_RegisterSound(tempStr, qfalse);
      continue;
    }
    ...
    if (Q_stricmp(token, "datapadmoveJumpSound") == 0) {
      ...
      uiInfo.uiDC.Assets.datapadmoveJumpSound =
          trap_S_RegisterSound(tempStr, qfalse);
      continue;
    }
    ...
    if (Q_stricmp(token, "datapadmoveSaberSound1") == 0) {
      if (PC_ParseString(&tempStr)) {
        PC_ParseWarning(
            "Bad 1st parameter for keyword 'datapadmoveSaberSound1'");
        return qfalse;
      }
    ...
    if (Q_stricmp(token, "datapadmoveSaberSound2") == 0) {
      if (PC_ParseString(&tempStr)) {
        PC_ParseWarning(
            "Bad 1st parameter for keyword 'datapadmoveSaberSound2'");
        return qfalse;
      }
    ...
    if (Q_stricmp(token, "datapadmoveSaberSound3") == 0) {
      if (PC_ParseString(&tempStr)) {
        PC_ParseWarning(
            "Bad 1st parameter for keyword 'datapadmoveSaberSound3'");
        return qfalse;
      }
    ...
    if (Q_stricmp(token, "datapadmoveSaberSound4") == 0) {
      if (PC_ParseString(&tempStr)) {
        PC_ParseWarning(
            "Bad 1st parameter for keyword 'datapadmoveSaberSound4'");
        return qfalse;
      }
    ...
    if (Q_stricmp(token, "datapadmoveSaberSound5") == 0) {
      if (PC_ParseString(&tempStr)) {
        PC_ParseWarning(
            "Bad 1st parameter for keyword 'datapadmoveSaberSound5'");
        return qfalse;
      }
    ...
    if (Q_stricmp(token, "datapadmoveSaberSound6") == 0) {
      if (PC_ParseString(&tempStr)) {
        PC_ParseWarning(
            "Bad 1st parameter for keyword 'datapadmoveSaberSound6'");
        return qfalse;
      }
...
void AssetCache(void) {
  ...
  uiInfo.uiDC.Assets.scrollBarArrowDown =
      ui.R_RegisterShaderNoMip(ASSET_SCROLLBAR_ARROWDOWN);
  uiInfo.uiDC.Assets.scrollBarArrowUp =
      ui.R_RegisterShaderNoMip(ASSET_SCROLLBAR_ARROWUP);
  uiInfo.uiDC.Assets.scrollBarArrowLeft =
      ui.R_RegisterShaderNoMip(ASSET_SCROLLBAR_ARROWLEFT);
  uiInfo.uiDC.Assets.scrollBarArrowRight =
      ui.R_RegisterShaderNoMip(ASSET_SCROLLBAR_ARROWRIGHT);
  uiInfo.uiDC.Assets.scrollBarThumb =
      ui.R_RegisterShaderNoMip(ASSET_SCROLL_THUMB);

  uiInfo.uiDC.Assets.sliderBar = ui.R_RegisterShaderNoMip("menu/new/slider");
  uiInfo.uiDC.Assets.sliderThumb =
      ui.R_RegisterShaderNoMip("menu/new/sliderthumb");

  /*
  for( n = 0; n < NUM_CROSSHAIRS; n++ )
  {
          uiInfo.uiDC.Assets.crosshairShader[n] = ui.R_RegisterShaderNoMip(
  va("gfx/2d/crosshair%c", 'a' + n ) );
  }
  */
}
...
static void UI_DrawEffects(rectDef_t *rect, float scale, vec4_t color) {
  UI_DrawHandlePic(rect->x, rect->y - 14, 128, 8,
                   0 /*uiInfo.uiDC.Assets.fxBasePic*/);
...
static void UI_Version(rectDef_t *rect, float scale, vec4_t color,
  ...
  DC->drawText(rect->x - width, rect->y, scale, color, Q3_VERSION, 0,
               ITEM_TEXTSTYLE_SHADOWED, iFontIndex);
}
...
static void UI_DrawKeyBindStatus(rectDef_t *rect, float scale, vec4_t color,
  ...
  if (Display_KeyBindPending()) {
    ...
    Text_Paint(rect->x, rect->y, scale, color,
               ui.SP_GetStringTextString("MENUS_WAITINGFORKEY"), 0, textStyle,
               iFontIndex);
#else
    Text_Paint(rect->x, rect->y, scale, color,
    ...
    //		Text_Paint(rect->x, rect->y, scale, color,
    //ui.SP_GetStringTextString("MENUS_ENTERTOCHANGE"), 0, textStyle,
    //iFontIndex);
  }
}
...
#define MAX_LINES 64
  ...
  Text_Paint(rect->x, y, scale, color,
             va("GL_VENDOR: %s", uiInfo.uiDC.glconfig.vendor_string), rect->w,
             textStyle, iFontIndex);
  y += 15;
  Text_Paint(rect->x, y, scale, color,
  ...
  y += 15;
  Text_Paint(rect->x, y, scale, color, "GL_PIXELFORMAT:", rect->w, textStyle,
             iFontIndex);
  y += 15;
  Text_Paint(rect->x, y, scale, color,
  ...
  numLines--;
  while (i < numLines) {
    Text_Paint(rect->x, y, scale, color, lines[i++], rect->w, textStyle,
               iFontIndex);
    y += 16;
  }
}
...
*/
/*
static void UI_DataPad_Inventory(rectDef_t *rect, float scale, vec4_t color, int
iFontIndex)
{
        Text_Paint(rect->x, rect->y, scale, color, "INVENTORY", 0, 1,
iFontIndex);
}
*/
/*
...
*/
/*
static void UI_DataPad_ForcePowers(rectDef_t *rect, float scale, vec4_t color,
int iFontIndex)
{
        Text_Paint(rect->x, rect->y, scale, color, "FORCE POWERS", 0, 1,
iFontIndex);
}
*/
...
static void UI_OwnerDraw(float x, float y, float w, float h, float text_x,
  ...
  case UI_ALLMAPS_SELECTION: // saved game thumbnail
    ...
    int levelshot;
    levelshot = ui.R_RegisterShaderNoMip(
        va("levelshots/%s",
           s_savedata[s_savegame.currentLine].currentSaveFileMap));
#ifdef JK2_MODE
    if (screenShotBuf[0]) {
      ui.DrawStretchRaw(x, y, w, h, SG_SCR_WIDTH, SG_SCR_HEIGHT, screenShotBuf,
                        0, qtrue);
    } else
#endif
        if (levelshot) {
    ...
    ui.R_Font_DrawString(x,     // int ox
                         y + h, // int oy
                         s_savedata[s_savegame.currentLine]
                             .currentSaveFileMap, // const char *text
                         color,                   // paletteRGBA_c c
...
int UI_OwnerDrawWidth(int ownerDraw, float scale) {
  ...
  case UI_KEYBINDSTATUS:
    if (Display_KeyBindPending()) {
      ...
      //			s =
      //ui.SP_GetStringTextString("MENUS_ENTERTOCHANGE");
    }
    break;

    // FIXME BOB
    //	case UI_SERVERREFRESHDATE:
    //		s = UI_Cvar_VariableString(va("ui_lastServerRefresh_%i",
    //ui_netSource.integer)); 		break;
  default:
    break;
  }
...
void UI_MainMenu(void) {
  char buf[256];
  ui.Cvar_Set("sv_killserver",
              "1"); // let the demo server know it should shut down
...
void UI_UpdateVideoSetup(void) {
  ...
  Cvar_Set("r_ext_compress_textures",
           Cvar_VariableString("ui_r_ext_compress_textures"));
...
void UI_GetVideoSetup(void) {
  ...
  Cvar_Set("ui_r_ext_compress_textures",
           Cvar_VariableString("r_ext_compress_textures"));
...
static void UI_SetSexandSoundForModel(const char *char_model) {
  ...
  i = ui.FS_FOpenFile(va("models/players/%s/sounds.cfg", char_model), &f,
                      FS_READ);
  if (!f) { // no?  oh bother.
    Cvar_Reset("snd");
    Cvar_Reset("sex");
    ...
    if (cl && cl->gentity && cl->gentity->client) {
      ...
      if (pState->saberStylesKnown & (1 << SS_FAST)) {
        ...
        if (pState->saberStylesKnown & (1 << SS_MEDIUM)) {
          Cvar_Set("ui_fightingstylesallowed",
                   "6"); // Has FAST and MEDIUM, so can only choose STRONG
          Cvar_Set("ui_newfightingstyle", "2"); // STRONG
        } else {
          Cvar_Set("ui_fightingstylesallowed",
                   "1"); // Has FAST, so can choose from MEDIUM and STRONG
      ...
      // Knows Medium style?
        ...
        if (pState->saberStylesKnown & (1 << SS_STRONG)) {
          Cvar_Set("ui_fightingstylesallowed",
                   "4"); // Has MEDIUM and STRONG, so can only choose FAST
          Cvar_Set("ui_newfightingstyle", "0"); // FAST
        } else {
          Cvar_Set("ui_fightingstylesallowed",
                   "2"); // Has MEDIUM, so can choose from FAST and STRONG
          Cvar_Set("ui_newfightingstyle", "0"); // FAST
        }
      }
      // Knows Strong style?
        ...
        if (pState->saberStylesKnown & (1 << SS_FAST)) {
          Cvar_Set("ui_fightingstylesallowed",
                   "5"); // Has STRONG and FAST, so can only take MEDIUM
          Cvar_Set("ui_newfightingstyle", "1"); // MEDIUM
        } else {
          Cvar_Set("ui_fightingstylesallowed",
                   "3"); // Has STRONG, so can choose from FAST and MEDIUM
          Cvar_Set("ui_newfightingstyle", "1"); // MEDIUM
        }
      } else // They have nothing, which should not happen
      {
        ...
        Cvar_Set("ui_newfightingstyle", "0");     // FAST??
        Cvar_Set("ui_fightingstylesallowed",
                 "0"); // Default to no new styles allowed
      }
...
static qboolean UI_GetForcePowerIndex(const char *forceName,
                                      short *forcePowerI) {
  int i;
...
// Set the fields for the allocation of force powers (Used by Force Power
// Allocation screen)
static void UI_InitAllocForcePowers(const char *forceName) {
  menuDef_t *menu;
  itemDef_t *item;
  ...
  // NOTE: this UIScript can be called outside the running game now, so handle
  // that case by getting info frim UIInfo instead of PlayerState
  if (cl) {
    playerState_t *pState = cl->gentity->client;
  ...
  char itemName[128];
  Com_sprintf(itemName, sizeof(itemName), "%s_hexpic",
...
// Flip flop between being able to see the text showing the Force Point has or
// hasn't been allocated (Used by Force Power Allocation screen)
...
//. Find weapons button and make active/inactive  (Used by Force Power
//Allocation screen)
static void UI_ForcePowerWeaponsButton(qboolean activeFlag) {
  menuDef_t *menu;
  menu = Menu_GetFocused(); // Get current menu
...
static void UI_SetHexPicLevel(const menuDef_t *menu, const int forcePowerI,
                              const int powerLevel, const qboolean goldFlag) {
  char itemName[128];
  itemDef_t *item;
  ...
  if (item) {
    char itemGraphic[128];
    if (goldFlag) {
      Com_sprintf(itemGraphic, sizeof(itemGraphic),
                  "gfx/menus/hex_pattern_%d_gold",
    ...
    if (item) {
      if (goldFlag) {
        // Change description text to tell player they can decrement the force
        // point
        item->descText = "@MENUS_REMOVEFP";
      } else {
        // Change description text to tell player they can increment the force
        // point
...
// if this is the first time into the force power allocation screen, show the
// INSTRUCTION screen
static void UI_ForceHelpActive(void) {
...
static void UI_DemoSetForceLevels(void) {
  ...
  if (Q_stricmp(buffer, "t1_sour") ==
      0) { // NOTE : always set the uiInfo powers
  ...
  if (pState) { // i am carrying over from a previous level, so get the
                // increased power! (non-core only)
    uiInfo.forcePowerLevel[FP_HEAL] = Q_max(pState->forcePowerLevel[FP_HEAL],
                                            uiInfo.forcePowerLevel[FP_HEAL]);
    uiInfo.forcePowerLevel[FP_TELEPATHY] =
        Q_max(pState->forcePowerLevel[FP_TELEPATHY],
              uiInfo.forcePowerLevel[FP_TELEPATHY]);
    uiInfo.forcePowerLevel[FP_GRIP] = Q_max(pState->forcePowerLevel[FP_GRIP],
                                            uiInfo.forcePowerLevel[FP_GRIP]);
    uiInfo.forcePowerLevel[FP_LIGHTNING] =
        Q_max(pState->forcePowerLevel[FP_LIGHTNING],
              uiInfo.forcePowerLevel[FP_LIGHTNING]);
    uiInfo.forcePowerLevel[FP_PROTECT] =
        Q_max(pState->forcePowerLevel[FP_PROTECT],
              uiInfo.forcePowerLevel[FP_PROTECT]);

    uiInfo.forcePowerLevel[FP_ABSORB] = Q_max(
        pState->forcePowerLevel[FP_ABSORB], uiInfo.forcePowerLevel[FP_ABSORB]);
    uiInfo.forcePowerLevel[FP_DRAIN] = Q_max(pState->forcePowerLevel[FP_DRAIN],
                                             uiInfo.forcePowerLevel[FP_DRAIN]);
    uiInfo.forcePowerLevel[FP_RAGE] = Q_max(pState->forcePowerLevel[FP_RAGE],
                                            uiInfo.forcePowerLevel[FP_RAGE]);
  }
}
#endif // !JK2_MODE

// record the force levels into a cvar so when restoring player from map
// transition the force levels are set up correctly
static void UI_RecordForceLevels(void) {
  menuDef_t *menu;
...
static void UI_ShutdownForceHelp(void) {
  ...
  // Not in upgrade mode so turn on all the force buttons, the big forceicon and
  // description text
  ...
  // In upgrade mode so just turn the deallocate button on
    ...
    item = (itemDef_s *)Menu_FindItemByName(
        menu, va("%s_fbutton", powerEnums[uiInfo.forcePowerUpdated].title));
    if (item) {
      item->window.flags |= WINDOW_HASFOCUS;
    }
    ...
    Com_sprintf(
        itemName, sizeof(itemName), "%s_level%ddesc",
        powerEnums[uiInfo.forcePowerUpdated].title,
        pState
            ->forcePowerLevel[powerEnums[uiInfo.forcePowerUpdated].powerEnum]);
  ...
  if (uiInfo.forcePowerUpdated > FP_UPDATED_NONE) {
    ...
    Com_sprintf(itemhexName, sizeof(itemhexName), "%s_hexpic",
                powerEnums[uiInfo.forcePowerUpdated].title);
    Com_sprintf(itemiconName, sizeof(itemiconName), "%s_iconpic",
                powerEnums[uiInfo.forcePowerUpdated].title);
...
static void UI_DecrementCurrentForcePower(void) {
  ...
  if (cl) {
    ...
    forcelevel =
        uiInfo.forcePowerLevel[powerEnums[uiInfo.forcePowerUpdated].powerEnum];
  }
  ...
  if (forcelevel > 0) {
    if (pState) {
      pState->forcePowerLevel[powerEnums[uiInfo.forcePowerUpdated]
                                  .powerEnum]--; // Decrement it
      forcelevel =
          pState
              ->forcePowerLevel[powerEnums[uiInfo.forcePowerUpdated].powerEnum];
      // Turn off power if level is 0
      if (pState->forcePowerLevel[powerEnums[uiInfo.forcePowerUpdated]
                                      .powerEnum] < 1) {
        pState->forcePowersKnown &=
            ~(1 << powerEnums[uiInfo.forcePowerUpdated].powerEnum);
      }
    } else {
      uiInfo.forcePowerLevel[powerEnums[uiInfo.forcePowerUpdated]
                                 .powerEnum]--; // Decrement it
      forcelevel =
          uiInfo
              .forcePowerLevel[powerEnums[uiInfo.forcePowerUpdated].powerEnum];
...
static void UI_AffectForcePowerLevel(const char *forceName) {
  ...
  if (forcelevel > 2) { // Too big, can't be incremented
    return;
  }
  ...
  if (pState) {
    pState
        ->forcePowerLevel[powerEnums[forcePowerI].powerEnum]++; // Increment it
...
// Show force level description that matches current player level (Used by Force
...
static void UI_ShowForceLevelDesc(const char *forceName) {
  ...
  // Update level description
  Com_sprintf(itemName, sizeof(itemName), "%s_level%ddesc",
              powerEnums[forcePowerI].title,
...
// Reset force level powers screen to what it was before player upgraded them
...
static void UI_UpdateFightingStyle(void) {
  ...
  } else // Must be at the beginning of the game so the client hasn't been
         // created, shove data in a cvar
  {
...
// If we have both weapons and the throwable weapon, turn on the begin mission
// button, otherwise, turn it off
...
// if this is the first time into the weapon allocation screen, show the
// INSTRUCTION screen
static void UI_WeaponHelpActive(void) {
  int tier_storyinfo = Cvar_VariableIntegerValue("tier_storyinfo");
  menuDef_t *menu;
...
static void UI_AddWeaponSelection(const int weaponIndex, const int ammoIndex,
  ...
  UI_WeaponsSelectionsComplete(); // Test to see if the mission begin button
...
static void UI_RemoveWeaponSelection(const int weaponSelectionIndex) {
  ...
  if (cl) // No client, get out
  {
    ...
    if (cl->gentity && cl->gentity->client) {
      ...
      // Remove ammo too
      if ((ammoIndex > 0) &&
          (ammoIndex < AMMO_MAX)) { // But don't take it away if the other
                                    // weapon is using that ammo
        if (uiInfo.selectedWeapon1AmmoIndex !=
            uiInfo.selectedWeapon2AmmoIndex) {
  ...
  if (weaponSelectionIndex == 1) {
    uiInfo.selectedWeapon1 = NOWEAPON;
    memset(uiInfo.selectedWeapon1ItemName, 0,
           sizeof(uiInfo.selectedWeapon1ItemName));
    uiInfo.selectedWeapon1AmmoIndex = 0;
  } else if (weaponSelectionIndex == 2) {
    uiInfo.selectedWeapon2 = NOWEAPON;
    memset(uiInfo.selectedWeapon2ItemName, 0,
           sizeof(uiInfo.selectedWeapon2ItemName));
  ...
  // FIXME hack to prevent error in jk2 by disabling
  DC->startLocalSound(
      DC->registerSound("sound/interface/weapon_deselect.mp3", qfalse),
      CHAN_LOCAL);
#endif
...
// Update the player throwable weapons (okay it's a bad description) with the
// chosen weapon
static void
UI_AddThrowWeaponSelection(const int weaponIndex, const int ammoIndex,
                           const int ammoAmount, const char *iconItemName,
  ...
  if (uiInfo.selectedThrowWeapon != NOWEAPON) {
    ...
    if (uiInfo.selectedThrowWeapon == weaponIndex) { // Deselect it
      UI_RemoveThrowWeaponSelection();
    }
    return;
  }
  ...
  memcpy(uiInfo.selectedThrowWeaponItemName, hexBackground,
         sizeof(uiInfo.selectedWeapon1ItemName));
...
static void UI_RemoveThrowWeaponSelection(void) {
  ...
  if (cl) // No client, get out
  {
    ...
    if (cl->gentity && cl->gentity->client) {
      ...
      if ((uiInfo.selectedThrowWeapon > 0) &&
          (uiInfo.selectedThrowWeapon < WP_NUM_WEAPONS)) {
        pState->stats[STAT_WEAPONS] &= ~(1 << uiInfo.selectedThrowWeapon);
      }

      // Remove ammo too
      if ((uiInfo.selectedThrowWeaponAmmoIndex > 0) &&
          (uiInfo.selectedThrowWeaponAmmoIndex < AMMO_MAX)) {
  ...
  // Now do a little clean up
  uiInfo.selectedThrowWeapon = NOWEAPON;
  memset(uiInfo.selectedThrowWeaponItemName, 0,
...
Cvar_VariableString("g_saber_color")));
}
*/
char GoToMenu[1024];
...
void UI_CheckVid1Data(const char *menuTo, const char *warningMenuName) {
  ...
  if (!menu) {
    Com_Printf(
        S_COLOR_YELLOW
        "WARNING: No videoMenu was found. Video data could not be checked\n");
    return;
  }
  ...
  if ((applyChanges->window.flags &
       WINDOW_VISIBLE)) // Is the APPLY CHANGES button active?
  {
    //		Menus_SaveGoToMenu(menuTo);							// Save menu you're going
    //to 		Menus_HideItems(menu->window.name);
    //// HIDE videMenu in case you have to come back
...
void ReadSaveDirectory(void) {
...
#ifdef JK2_MODE
  memset(screenShotBuf, 0,
         (SG_SCR_WIDTH * SG_SCR_HEIGHT * 4)); // blank out sshot
#endif
  ...
  for (i = 0; i < fileCnt; i++) {
    ...
    if (Q_stricmp("current", holdChar) != 0) {
      ...
      } else { // Is this a valid file??? & Get comment of file
  ...
  qsort(s_savedata, s_savegame.saveFileCnt, sizeof(savedata_t),
        UI_SortSaveGames);
}
