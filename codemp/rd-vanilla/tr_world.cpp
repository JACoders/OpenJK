...
static qboolean R_CullSurface(surfaceType_t *surface, shader_t *shader) {
  ...
  if (r_cullRoofFaces->integer) { // Very slow, but this is only intended for
                                  // taking shots for automap images.
    if (sface->plane.normal[2] > 0.0f &&
        sface->numPoints > 0) { // it's facing up I guess
      static int i;
      static trace_t tr;
      ...
      // The fact that this point is in the middle of the array has no relation
      // to the orientation in the surface outline.
      ...
      ri.CM_BoxTrace(&tr, basePoint, endPoint, NULL, NULL, 0,
                     (CONTENTS_SOLID | CONTENTS_TERRAIN), qfalse);

      if (!tr.startsolid && !tr.allsolid &&
          (tr.fraction == 1.0f ||
           (tr.surfaceFlags &
            SURF_NOIMPACT))) { // either hit nothing or sky, so this surface is
                               // near the top of the level I guess. Or the floor
                               // of a really tall room, but if that's the case
                               // we're just screwed.
        VectorSubtract(basePoint, tr.endpos, v);
        if (tr.fraction == 1.0f ||
            VectorLength(v) <
                r_roofCullCeilDist
                    ->value) { // ignore it if it's not close to the top, unless
                               // it just hit nothing
          // Let's try to dig back into the brush based on the negative
          // direction of the plane, and if we pop out on the other side we'll
          // see if it's ground or not.
          i = 4;
          ...
          while (i < 4096) {
            ...
            ri.CM_BoxTrace(&tr, endPoint, endPoint, NULL, NULL, 0,
                           (CONTENTS_SOLID | CONTENTS_TERRAIN), qfalse);
            if (!tr.startsolid && !tr.allsolid &&
                tr.fraction == 1.0f) { // in the clear
              break;
            }
            i++;
          }
          if (i < 4096) { // Make sure we got into clearance
            ...
            // trace a second time from the clear point in the inverse normal
            // direction of the surface. If we hit something within a set amount
            // of units, we will assume it's a bridge type object and leave it to
            // be drawn. Otherwise we will assume it is a roof or other
            // obstruction and cull it out.
            ri.CM_BoxTrace(&tr, basePoint, endPoint, NULL, NULL, 0,
                           (CONTENTS_SOLID | CONTENTS_TERRAIN), qfalse);
            ...
            if (!tr.startsolid && !tr.allsolid &&
                (tr.fraction != 1.0f &&
                 !(tr.surfaceFlags &
                   SURF_NOIMPACT))) { // if we hit nothing or a noimpact going
                                      // down then this is probably "ground".
              VectorSubtract(basePoint, tr.endpos, endPoint);
              if (VectorLength(endPoint) >
                  r_roofCullCeilDist
                      ->value) { // 128 (by default) is our maximum tolerance,
...
static void R_AddWorldSurface(msurface_t *surf, int dlightBits,
  ...
  */
  ...
  if (tr_drawingAutoMap) {
    ...
    //		!g_lastHeightValid)
    if (*surf->data == SF_FACE) { // only do this if we need to
      ...
      while (i < face->numIndices) {
        ...
        // base the color on the elevation... for now, just check the first
        // point height
        ...
        if (alpha != 1.0f) { // this point is not entirely alpha'd out, so still
                             // draw the surface
...
void R_AddBrushModelSurfaces(trRefEntity_t *ent) {
  ...
  if (pModel->bspInstance) { // rwwRMG - added
    R_SetupEntityLighting(&tr.refdef, ent);
  }

  // rww - Take this into account later?
  //	if ( !ri.Cvar_VariableIntegerValue( "com_RMG" ) )
  //	{	// don't dlight bmodels on rmg, as multiple copies of the same
  //instance will light up
...
void RE_GetBModelVerts(int bmodelIndex, vec3_t *verts, vec3_t normal) {
  ...
  for (i = 0; i < bmodel->numSurfaces; i++) {
    ...
    // It seems that the safest way to handle this is by finding the area of the
    // faces
    dist = GetQuadArea(face->points[0], face->points[1], face->points[2],
                       face->points[3]);
  ...
  // Hopefully we've found two best case candidates.  Now we should see which of
  // these faces the viewer
  ...
  } else { // Possibly only have one face, so may as well use the first face,
           // which also should be the best one
    // i = rand() & 1; // ugh, we don't know which to use.  I'd hope this would
    // never happen
    i = maxIndx[0]; // use the first face
  }
...
static inline wireframeMapSurf_t *R_GetNewWireframeMapSurf(void) {
  ...
  if (g_autoMapNextFree) { // save us the time of going through the entire
                           // linked list from root
    next = g_autoMapNextFree;
  }

  while (*next) { // iterate through until we find the next unused one
    next = &(*next)->next;
  }

  // allocate memory for it and pass it back
  (*next) = (wireframeMapSurf_t *)Z_Malloc(sizeof(wireframeMapSurf_t), TAG_ALL,
                                           qtrue);
...
static inline void R_EvaluateWireframeSurf(msurface_t *surf) {
  if (*surf->data == SF_FACE) {
    ...
    if (points && numPoints > 0) { // we can add it
      int i = 0;
      ...
      nextSurf->points = (wireframeSurfPoint_t *)Z_Malloc(
          sizeof(wireframeSurfPoint_t) * face->numIndices, TAG_ALL, qtrue);
...
static inline void R_RecursiveWireframeSurf(mnode_t *node) {
  ...
  while (1) {
    if (!node || node->visframe !=
                     tr.visCount) { // not valid, stop this chain of recursion
      return;
    }
...
#if 0 // doesn't work, I take it surfs on nodes are not related to surfs on
      // brushes
...
void R_DestroyWireframeMap(void) {
  ...
  if (!g_autoMapValid) { // not valid to begin with
    return;
  }
...
qboolean R_WriteWireframeMapToFile(void) {
  ...
  // let's go through and see how much space we're going to need to stuff all
  // this data into
  while (surf) {
  ...
  if (requiredSize <= 0) { // nothing to do..?
    return qfalse;
  }
  ...
  while (surf) {
    memcpy(out, surf,
           (sizeof(wireframeSurfPoint_t) * surf->numPoints) + sizeof(int));
...
qboolean R_GetWireframeMapFromFile(void) {
  ...
  while (i < len) {
    ...
    newSurf->points = (wireframeSurfPoint_t *)Z_Malloc(
        sizeof(wireframeSurfPoint_t) * surfs->numPoints, TAG_ALL, qtrue);

    // copy the surf data into the new surf
    // note - the surfs->points pointer is NOT pointing to valid memory, a
    // pointer to that pointer is actually what we want to use as the location of
    // the point offsets.
    memcpy(newSurf->points, &surfs->points,
           sizeof(wireframeSurfPoint_t) * surfs->numPoints);
...
qboolean R_InitializeWireframeAutomap(void) { // yoink
  return qtrue;
}
#endif
...
const void *R_DrawWireframeAutomap(const void *data) {
...
#ifndef _ALT_AUTOMAP_METHOD
  if (!g_autoMapValid) { // data is not valid, don't draw
    return (const void *)(cmd + 1);
  }
#endif
...
#if 0 // this does no good sadly, because of the issue of having to clear with a
      // second scene in order for global fog clearing to work.
  ...
  if (r_autoMap->integer == 2) { // line mode
    GL_State(GLS_POLYMODE_LINE | GLS_SRCBLEND_SRC_ALPHA |
             GLS_DSTBLEND_SRC_COLOR | GLS_DEPTHMASK_TRUE);
  } else { // fill mode
  ...
  while (s) {
    ...
    // if the alpha ends up being completely transparent for every point, we
    // don't even need to draw it
    if (g_playerHeight != g_lastHeight ||
        !g_lastHeightValid) { // only do this if we need to
      i = 0;
      ...
      while (i < s->numPoints) {
        ...
        if (r_autoMap->integer != 2) { // fill mode
        ...
        if (s->points[i].alpha != 1.0f) { // this point is not entirely alpha'd
                                          // out, so still draw the surface
    ...
    while (i < s->numPoints) {
      if (r_autoMap->integer == 2 ||
          s->numPoints < 3) { // line mode or not enough verts on surface
        qglColor4f(s->points[i].color[0], s->points[i].color[1],
                   s->points[i].color[2], s->points[i].alpha);
      } else { // fill mode
        vec3_t planeNormal;
        float fAlpha = s->points[i].alpha;
        planeNormal[0] =
            s->points[0].xyz[1] * (s->points[1].xyz[2] - s->points[2].xyz[2]) +
            s->points[1].xyz[1] * (s->points[2].xyz[2] - s->points[0].xyz[2]) +
            s->points[2].xyz[1] * (s->points[0].xyz[2] - s->points[1].xyz[2]);
        planeNormal[1] =
            s->points[0].xyz[2] * (s->points[1].xyz[0] - s->points[2].xyz[0]) +
            s->points[1].xyz[2] * (s->points[2].xyz[0] - s->points[0].xyz[0]) +
            s->points[2].xyz[2] * (s->points[0].xyz[0] - s->points[1].xyz[0]);
        planeNormal[2] =
            s->points[0].xyz[0] * (s->points[1].xyz[1] - s->points[2].xyz[1]) +
            s->points[1].xyz[0] * (s->points[2].xyz[1] - s->points[0].xyz[1]) +
            s->points[2].xyz[0] * (s->points[0].xyz[1] - s->points[1].xyz[1]);
        ...
        */
        ...
        qglColor4f(s->points[i].color[0], s->points[i].color[1],
                   1.0f - planeNormal[2], fAlpha);
      }
      qglVertex3f(s->points[i].xyz[0], s->points[i].xyz[1],
                  s->points[i].xyz[2]);
      i++;
    }
    qglEnd();
      ...
      if (dlightBits) {
        ...
        for (i = 0; i < tr.refdef.num_dlights; i++) {
          ...
          if (dlightBits & (1 << i)) {
            dl = &tr.refdef.dlights[i];
            dist =
                DotProduct(dl->origin, node->plane->normal) - node->plane->dist;
