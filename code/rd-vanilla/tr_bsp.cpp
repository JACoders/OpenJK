...
static void R_LoadLightmaps(lump_t *l, const char *psMapName, world_t &worldData) {
	...
	for (i = 0; i < count; i++) {
		...
		tr.lightmaps[worldData.startLightMapIndex + i] =
			R_CreateImage(va("$%s/lightmap%d", sMapName, worldData.startLightMapIndex + i), image, LIGHTMAP_SIZE, LIGHTMAP_SIZE, GL_RGBA, qfalse, qfalse,
						  (qboolean)(r_ext_compressed_lightmaps->integer != 0), GL_CLAMP);
	}
...
static void R_LoadNodesAndLeafs(lump_t *nodeLump, lump_t *leafLump, world_t &worldData) {
	...
	for (i = 0; i < numLeafs; i++, inLeaf++, out++) {
		...
		out->firstmarksurface = worldData.marksurfaces + LittleLong(inLeaf->firstLeafSurface);
...
static void R_LoadFogs(lump_t *l, lump_t *brushesLump, lump_t *sidesLump, world_t &worldData, int index) {
	...
	for (i = 0; i < count; i++, fogs++) {
		...
		out->colorInt = ColorBytes4(out->parms.color[0], out->parms.color[1], out->parms.color[2], 1.0);
