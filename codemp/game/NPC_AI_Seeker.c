...
void NPC_Seeker_Pain(gentity_t *self, gentity_t *attacker, int damage) {
  if (!(self->NPC->aiFlags & NPCAI_CUSTOM_GRAVITY)) {
    G_Damage(self, NULL, NULL, (float *)vec3_origin, (float *)vec3_origin, 999,
             0, MOD_FALLING);
  }
...
void Seeker_MaintainHeight(void) {
  ...
  if (NPCS.NPC->enemy) {
    if (TIMER_Done(NPCS.NPC, "heightChange")) {
      ...
      dif = (NPCS.NPC->enemy->r.currentOrigin[2] +
             flrand(NPCS.NPC->enemy->r.maxs[2] / 2,
                    NPCS.NPC->enemy->r.maxs[2] + 8)) -
...
void Seeker_Strafe(void) {
  ...
  if (Q_flrand(0.0f, 1.0f) > 0.7f || !NPCS.NPC->enemy ||
    ...
    if (tr.fraction > 0.9f) {
      ...
      VectorMA(NPCS.NPC->client->ps.velocity, vel * side, right,
    ...
    AngleVectors(NPCS.NPC->enemy->client->renderInfo.eyeAngles, dir, right,
                 NULL);
    ...
    if (tr.fraction > 0.9f) {
      ...
      VectorMA(NPCS.NPC->client->ps.velocity, dis, dir,
               NPCS.NPC->client->ps.velocity);
...
void Seeker_Hunt(qboolean visible, qboolean advance) {
  ...
  speed = SEEKER_FORWARD_BASE_SPEED +
          SEEKER_FORWARD_MULTIPLIER * g_npcspskill.integer;
...
void Seeker_Fire(void) {
  ...
  G_PlayEffectID(G_EffectIndex("blaster/muzzle_flash"),
...
void Seeker_FindEnemy(void) {
  ...
  for (i = 0; i < numFound; i++) {
    ...
    if (ent->s.number == NPCS.NPC->s.number || !ent->client //&& || !ent->NPC
    ...
    if (ent->client->playerTeam == NPCS.NPC->client->playerTeam ||
        ent->client->playerTeam ==
            NPCTEAM_NEUTRAL) // don't attack same team or bots
    {
      continue;
    }
  ...
  if (dis < minDistSqr) {
    ...
    if (NPCS.NPC->client->NPC_class == CLASS_BOBAFETT) {
      pt[0] = owner->r.currentOrigin[0] +
              cos(level.time * 0.001f + NPCS.NPC->random) * 250;
      pt[1] = owner->r.currentOrigin[1] +
              sin(level.time * 0.001f + NPCS.NPC->random) * 250;
      ...
      pt[0] = owner->r.currentOrigin[0] +
              cos(level.time * 0.001f + NPCS.NPC->random) * 56;
      pt[1] = owner->r.currentOrigin[1] +
              sin(level.time * 0.001f + NPCS.NPC->random) * 56;
      pt[2] = owner->r.currentOrigin[2] + 40;
    }

    VectorSubtract(pt, NPCS.NPC->r.currentOrigin, dir);
    VectorMA(NPCS.NPC->client->ps.velocity, 0.8f, dir,
             NPCS.NPC->client->ps.velocity);
  } else {
    if (NPCS.NPC->client->NPC_class != CLASS_BOBAFETT) {
...
void NPC_BSSeeker_Default(void) {
  ...
  */
  ...
  if (NPCS.NPC->r.ownerNum < ENTITYNUM_NONE) {
    ...
    if (owner->health <= 0 ||
        (owner->client && owner->client->pers.connected ==
                              CON_DISCONNECTED)) { // owner is dead or gone
      // remove me
      G_Damage(NPCS.NPC, NULL, NULL, NULL, NULL, 10000, DAMAGE_NO_PROTECTION,
               MOD_TELEFRAG);
      return;
    }
  }
  ...
  if (NPCS.NPC->enemy && NPCS.NPC->enemy->health && NPCS.NPC->enemy->inuse) {
    // OJKFIXME: clientnum 0
    if (NPCS.NPC->client->NPC_class != CLASS_BOBAFETT &&
        (NPCS.NPC->enemy->s.number == 0 ||
         (NPCS.NPC->enemy->client &&
          NPCS.NPC->enemy->client->NPC_class == CLASS_SEEKER))) {
