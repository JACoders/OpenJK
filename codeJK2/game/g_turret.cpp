...
//------------------------------------------------------------------------------------------------------------
void TurretPain(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
...
//----------------------------------------------------------------
{
  ...
  bolt->dflags = DAMAGE_NO_KNOCKBACK |
                 DAMAGE_HEAVY_WEAP_CLASS; // Don't push them around, or else we
                                          // are constantly re-aiming
  bolt->splashDamage = 0;
  bolt->splashRadius = 0;
...
//-----------------------------------------------------
{
  // if it's time to fire and we have an enemy, then gun 'em down!  pushDebounce
  // time controls next fire time
  if (self->enemy && self->pushDebounceTime < level.time &&
    ...
    gi.G2API_GetBoltMatrix(
        self->ghoul2, self->playerModel, self->torsoBolt, &boltMatrix,
...
//-----------------------------------------------------
{
  ...
  if (self->enemy) {
    ...
    gi.G2API_GetBoltMatrix(self->ghoul2, self->playerModel, self->torsoBolt,
                           &boltMatrix, self->currentAngles, self->s.origin,
    ...
    // new one
    //		diffYaw = sin( level.time * 0.0001f + self->count ) * 5.0f;	// don't
    //do this for now since it can make it go into walls.
  }
  ...
  if (diffPitch) {
    ...
    // Note that this is NOT interpolated, so it will be less smooth...On the
    // other hand, it does use Ghoul2 to blend, so it may smooth it out a bit?
    if (self->spawnflags & 2) {
    ...
    gi.G2API_SetBoneAngles(&self->ghoul2[0], "Bone_body", desiredAngles,
                           BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z,
                           POSITIVE_X, NULL, 100, cg.time);
  }
...
//-----------------------------------------------------
{
  ...
  if (found) {
    if (!self->enemy) { // just aquired one
...
//-----------------------------------------------------
{
    ...
    if (self->enemy->health > 0) {
      ...
      if (enemyDist < self->radius * self->radius) {
        ...
        if (gi.inPVS(self->currentOrigin, self->enemy->currentOrigin)) {
          ...
          if (!tr.allsolid && !tr.startsolid &&
              tr.entityNum == self->enemy->s.number) {
  ...
  if (turnOff) {
    if (self->bounceCount < level.time) // bounceCount is used to keep the thing
                                        // from ping-ponging from on to off
    {
      turret_turnoff(self);
    }
  } else {
...
Turret that hangs from the ceiling, will aim and shoot at enemies
  ...
  UPSIDE_DOWN - make it rest on a surface/floor instead of hanging from the
ceiling
...
{
  base->s.modelindex =
      G_ModelIndex("models/map_objects/imp_mine/turret_canon.glm");
  base->s.modelindex2 =
      G_ModelIndex("models/map_objects/imp_mine/turret_damage.md3");
  base->playerModel = gi.G2API_InitGhoul2Model(
      base->ghoul2, "models/map_objects/imp_mine/turret_canon.glm",
      base->s.modelindex, NULL_HANDLE, NULL_HANDLE, 0, 0);
  base->s.radius = 80.0f;

  gi.G2API_SetBoneAngles(&base->ghoul2[base->playerModel], "Bone_body",
                         vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Y,
                         POSITIVE_Z, POSITIVE_X, NULL, 0, 0);
  base->torsoBolt =
      gi.G2API_AddBolt(&base->ghoul2[base->playerModel], "*flash03");
...
void finish_spawning_turret(gentity_t *base) {
  ...
  if (base->spawnflags & 2) { // upside-down, invert mins and maxe
  ...
  base->contents = CONTENTS_BODY | CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP |
                   CONTENTS_SHOTCLIP;
...
{
  base->s.modelindex =
      G_ModelIndex("models/map_objects/nar_shaddar/turret/turret.glm");
  base->s.modelindex2 =
      G_ModelIndex("models/map_objects/imp_mine/turret_damage.md3"); // FIXME!
  base->playerModel = gi.G2API_InitGhoul2Model(
      base->ghoul2, "models/map_objects/nar_shaddar/turret/turret.glm",
      base->s.modelindex, NULL_HANDLE, NULL_HANDLE, 0, 0);
  base->s.radius = 80.0f;
  ...
  gi.G2API_SetBoneAngles(&base->ghoul2[base->playerModel], "Bone_body",
                         ...
                         POSITIVE_Z, POSITIVE_X, NULL, 0, 0);
  base->torsoBolt =
      gi.G2API_AddBolt(&base->ghoul2[base->playerModel], "*flash02");
...
void laser_arm_fire(gentity_t *ent) {
  ...
  gi.trace(&trace, start, NULL, NULL, end, ENTITYNUM_NONE, MASK_SHOT,
           G2_NOCOLLIDE, 0);                 // ignore
  ...
  if (trace.fraction < 1.0 && ent->alt_fire) {
    if (trace.entityNum < ENTITYNUM_WORLD) {
      ...
      if (hapless_victim && hapless_victim->takedamage && ent->damage) {
        G_Damage(hapless_victim, ent, ent->nextTrain->activator, fwd,
                 trace.endpos, ent->damage, DAMAGE_IGNORE_TEAM, MOD_UNKNOWN);
      }
    }
  }

  if (ent->alt_fire) {
    //		CG_FireLaser( start, trace.endpos, trace.plane.normal,
    //ent->nextTrain->startRGBA, qfalse );
  } else {
...
void laser_arm_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  default:
    ...
    self->lastEnemy->lastEnemy->attackDebounceTime =
        level.time + self->lastEnemy->lastEnemy->wait;
    G_Sound(self->lastEnemy->lastEnemy,
  ...
  case 1:
    ...
    //		bolt_head_to_arm( self->lastEnemy, self->lastEnemy->lastEnemy,
    //LARM_FOFS, LARM_ROFS, LARM_UOFS );
...
What it does when used depends on it's "count" (can be set by a script)
  ...
  dmg	- How much damage the laser does 10 times a second (default 5 = 50
points per second) wait  - How long the beam lasts, in seconds (default is 3)

  targetname - to use it
  target - What thing for it to be pointing at to start with

  "startRGBA" - laser color, Red Green Blue Alpha, range 0 to 1 (default  1.0
0.85 0.15 0.75 = Yellow-Orange)
*/
void laser_arm_start(gentity_t *base) {
  ...
  if (base->target && base->target[0]) { // Start out pointing at something
    gentity_t *targ = G_Find(NULL, FOFS(targetname), base->target);
    if (!targ) { // couldn't find it!
  ...
  // anglespeed - how fast it can track the player, entered in degrees per
  // second, so we divide by FRAMETIME/1000
  if (!base->speed) {
    base->speed = 3.0f;
  } else {
  ...
  // The head should always think, since it will be either firing a damage laser
  // or just a target laser
...
//----------------------------------------------------------------
{
  ...
  // Getting the flash bolt here
  gi.G2API_GetBoltMatrix(ent->ghoul2, ent->playerModel, ent->torsoBolt,
                         &boltMatrix, ent->currentAngles, ent->s.origin,
                         (cg.time ? cg.time : level.time), NULL,
                         ent->s.modelScale);
...
//---------------------------------
{
  ...
  } else // if ( Q_flrand(0.0f, 1.0f) > 0.5f )
  {
    ...
    trace_t tr;
    gi.trace(&tr, org2, NULL, NULL, org, ent->s.number, MASK_SHOT, G2_NOCOLLIDE,
             0);
...
{
  if (!ent->damage) {
    // let us do our animation, then we are good to go in terms of pounding the
    // crap out of enemies.
    ent->damage = 1;
    gi.G2API_SetBoneAnimIndex(&ent->ghoul2[ent->playerModel], ent->rootBone, 0,
                              11, BONE_ANIM_OVERRIDE_FREEZE, 0.8f, cg.time, -1,
                              -1);
  ...
  gi.G2API_SetBoneAngles(&ent->ghoul2[ent->playerModel], "bone_barrel",
                         frontAngles, BONE_ANGLES_POSTMULT, POSITIVE_Y,
                         POSITIVE_Z, NEGATIVE_X, NULL, 100, cg.time);
  gi.G2API_SetBoneAngles(&ent->ghoul2[ent->playerModel], "bone_gback",
                         frontAngles, BONE_ANGLES_POSTMULT, POSITIVE_Y,
                         POSITIVE_Z, NEGATIVE_X, NULL, 100, cg.time);
  gi.G2API_SetBoneAngles(&ent->ghoul2[ent->playerModel], "bone_hinge",
                         backAngles, BONE_ANGLES_POSTMULT, POSITIVE_Y,
...
//---------------------------------
{
  ...
  base->playerModel = gi.G2API_InitGhoul2Model(
      base->ghoul2, "models/items/psgun.glm", base->s.modelindex, NULL_HANDLE,
      NULL_HANDLE, 0, 0);
  base->s.radius = 30.0f;
  VectorSet(base->s.modelScale, 1.0f, 1.0f, 1.0f);

  base->rootBone = gi.G2API_GetBoneIndex(&base->ghoul2[base->playerModel],
                                         "model_root", qtrue);
  gi.G2API_SetBoneAngles(&base->ghoul2[base->playerModel], "bone_hinge",
                         vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Y,
                         POSITIVE_Z, POSITIVE_X, NULL, 0, 0);
  gi.G2API_SetBoneAngles(&base->ghoul2[base->playerModel], "bone_gback",
                         vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Y,
                         POSITIVE_Z, POSITIVE_X, NULL, 0, 0);
  gi.G2API_SetBoneAngles(&base->ghoul2[base->playerModel], "bone_barrel",
                         vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Y,
                         POSITIVE_Z, POSITIVE_X, NULL, 0, 0);
  ...
  if (!(base->spawnflags & 1)) // START_OFF
  {
    base->nextthink =
        level.time +
        1000; // we aren't starting off, so start working right away
  ...
  // hack this flag on so that when it calls the turret die code, it will orient
  // the effect up HACK
  //--------------------------------------
  base->spawnflags |= 2;
...
//------------------------------------------------------------------------
qboolean place_portable_assault_sentry(gentity_t *self, vec3_t origin,
                                       vec3_t angs)
//------------------------------------------------------------------------
{
  vec3_t fwd, pos;
  ...
  // and move a consistent distance away from us so we don't have the dumb thing
  // spawning inside of us.
  ...
  // check for a decent surface, meaning mostly flat...should probably also
  // check surface parms so we don't set us down on lava or something.
  if (!tr.startsolid && !tr.allsolid && tr.fraction < 1.0f &&
      tr.plane.normal[2] > 0.9f && tr.entityNum >= ENTITYNUM_WORLD) {
    ...
    if (pas) {
      ...
      pas->contents |= CONTENTS_PLAYERCLIP; // player placed ones can block
                                            // players but not npcs
...
//----------------------------------------
{
  if (self->spawnflags & 2) {
    if (self->count) {
      ...
      self->nextthink =
          level.time + (self->delay + Q_flrand(-1.0f, 1.0f) * self->random);
  ...
  if (self->fxID) {
    ...
    // Getting the flash bolt here
    gi.G2API_GetBoltMatrix(self->ghoul2, self->playerModel, self->torsoBolt,
                           &boltMatrix, self->s.angles, self->s.origin,
                           (cg.time ? cg.time : level.time), NULL,
                           self->s.modelScale);
  ...
  gi.G2API_SetBoneAnimIndex(&self->ghoul2[self->playerModel], self->rootBone, 0,
                            8, BONE_ANIM_OVERRIDE_FREEZE, 0.6f, cg.time, -1,
                            -1);
  self->nextthink =
      level.time + self->wait + Q_flrand(-1.0f, 1.0f) * self->random;
}

//----------------------------------------------------------------------------------------------
void ion_cannon_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
...
/*QUAKED misc_ion_cannon (1 0 0) (-140 -140 0) (140 140 320) START_OFF BURSTS
SHIELDED Huge ion cannon, like the ones at the rebel base on Hoth.

  START_OFF - Starts off
  BURSTS - adds more variation, shots come out in bursts
  SHIELDED - cannon is shielded, any kind of shot bounces off.

  wait	- How fast it shoots (default 1500 ms between shots, can't be less than
500 ms) random - milliseconds wait variation (default 400 ms...up to plus or
minus .4 seconds) delay	- Number of milliseconds between bursts (default 6000
ms, can't be less than 1000 ms, only works when BURSTS checked)

  health - default 2000
  splashDamage - how much damage to do when it dies, must be greater than 0 to
actually work splashRadius - damage radius, must be greater than 0 to actually
work
...
//-----------------------------------------------------
{
  ...
  base->s.modelindex =
      G_ModelIndex("models/map_objects/imp_mine/ion_cannon.glm");
  base->playerModel = gi.G2API_InitGhoul2Model(
      base->ghoul2, "models/map_objects/imp_mine/ion_cannon.glm",
      base->s.modelindex, NULL_HANDLE, NULL_HANDLE, 0, 0);
  ...
  base->s.modelindex2 =
      G_ModelIndex("models/map_objects/imp_mine/ion_cannon_damage.md3");
  ...
  {
    base->flags |= FL_SHIELDED; // technically, this would only take damage from
                                // a lightsaber, but the other flag just above
                                // would cancel that out too.
  }
  ...
  if (base->spawnflags & 1) {
    ...
    base->nextthink =
        level.time + base->wait + Q_flrand(-1.0f, 1.0f) * base->random;
  }
...
void spotlight_think(gentity_t *ent) {
  ...
  VectorMA(ent->currentOrigin, 2048, dir,
           end); // just pick some max trace distance
  gi.trace(&tr, ent->currentOrigin, vec3_origin, vec3_origin, end,
           ent->s.number, CONTENTS_SOLID, G2_NOCOLLIDE, 0);
...
//-----------------------------------------------------
{
  ...
  base->s.modelindex =
      G_ModelIndex("models/map_objects/imp_mine/spotlight.md3");

  G_SpawnInt("health", "300", &base->health);

  // Set up our lightcone effect, though we will have it draw cgame side so it
  // looks better
...
Creates a turret that, when the player uses a panel, takes control of this
turret and adopts the turret view
...
"target3" - thing to use when it dies.
  ...
  radius - the max yaw range in degrees, (default 90) which means you can move
90 degrees on either side of the start angles. random - the max pitch range in
degrees, (default 60) which means you can move 60 degrees above or below the
start angles. delay - time between shots, in milliseconds (default 200). damage
- amount of damage shots do, (default 50). speed - missile speed, (default 3000)

  heatlh - how much heatlh the thing has, (default 200) only works if HEALTH is
checked, otherwise it can't be destroyed.
*/

extern gentity_t *player;
extern qboolean G_ClearViewEntity(gentity_t *ent);
extern void G_SetViewEntity(gentity_t *self, gentity_t *viewEntity);
extern gentity_t *CreateMissile(vec3_t org, vec3_t dir, float vel, int life,
                                gentity_t *owner, qboolean altFire = qfalse);
...
//-----------------------------------------
void misc_panel_turret_die(gentity_t *self, gentity_t *inflictor,
                           gentity_t *attacker, int damage, int mod, int dFlags,
                           int hitLoc) {
  if (self->target3) {
...
void panel_turret_think(gentity_t *self) {
  ...
  if (player && player->client &&
    ...
    // We are the viewEnt so update our new viewangles based on the sum of our
    // start angles and the ucmd angles
    for (int i = 0; i < 3; i++) {
      // convert our base angle to a short, add with the usercmd.angle ( a short
      // ), then switch use back to a real angle
      self->s.apos.trBase[i] = AngleNormalize180(SHORT2ANGLE(
          ucmd->angles[i] + ANGLE2SHORT(self->s.angles[i]) + self->pos3[i]));
    }
    ...
    // Angle clamping -- PITCH
    {
      ...
      {
        self->pos3[PITCH] += ANGLE2SHORT(
            AngleNormalize180(self->random - self->s.apos.trBase[PITCH]));
    ...
    if (self->radius != 0.0f) {
      float yawDif =
          AngleSubtract(self->s.apos.trBase[YAW], self->s.angles[YAW]);
      ...
      if (yawDif > self->radius) // radius is YAW
      {
        ...
        self->s.apos.trBase[YAW] =
            AngleNormalize180(self->s.angles[YAW] + self->radius);
    ...
    self->s.pos.trType = TR_INTERPOLATE; // not really moving, but this fixes an
                                         // interpolation bug in cg_ents.

    // Check for backing out of turret
    if ((self->useDebounceTime < level.time) &&
        ((ucmd->buttons & BUTTON_BLOCKING) || (ucmd->buttons & BUTTON_USE) ||
...
void panel_turret_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  // Compensating for the difference between the players view at the time of use
  // and the start angles that the gun object has
...
void SP_misc_panel_turret(gentity_t *self) {
  ...
  self->s.modelindex =
      G_ModelIndex("models/map_objects/imp_mine/ladyluck_gun.md3");
