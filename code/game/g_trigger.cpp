...
void multi_trigger_run(gentity_t *ent) {
	...
	if (ent->activator && ent->activator->s.number == 0) { // mark the trigger as being touched by the player
		ent->aimDebounceTime = level.time;
	}
}

void multi_trigger(gentity_t *ent, gentity_t *activator) {
	if (ent->e_ThinkFunc == thinkF_multi_trigger_run) { // already triggered, just waiting to run
		return;
	}
...
void Touch_Multi(gentity_t *self, gentity_t *other, trace_t *trace) {
	...
	if (self->spawnflags & 4) { // USE_BUTTON
		...
		if (!(other->client->usercmd.buttons & BUTTON_USE)) { // not pressing use button
			return;
		}
	}
	...
	if (self->spawnflags & 8) { // FIRE_BUTTON
		...
		if (!(other->client->ps.eFlags & EF_FIRING /*usercmd.buttons & BUTTON_ATTACK*/) &&
			!(other->client->ps.eFlags & EF_ALT_FIRING /*usercmd.buttons & BUTTON_ALT_ATTACK*/)) { // not pressing fire button or altfire button
			return;
		}

		// FIXME: do we care about the sniper rifle or not?

		if (other->s.number == 0 && (other->client->ps.weapon > MAX_PLAYER_WEAPONS ||
									 other->client->ps.weapon <= WP_NONE)) { // don't care about non-player weapons if this is the player
			return;
		}
	}
	...
	if (other->client && self->radius) {
		...
		if (G_PointInBounds(eyeSpot, self->absmin, self->absmax)) {
			if (!(other->client->ps.eFlags & EF_FIRING) && !(other->client->ps.eFlags & EF_ALT_FIRING)) { // not attacking, so hiding bonus
				...
				if (VectorLength(self->movedir)) { // They can only be hidden from enemies looking in this direction
	...
	if (self->e_ThinkFunc == thinkF_trigger_cleared_fire) { // We're waiting to fire our target2 first
		self->nextthink = level.time + self->speed;
		return;
	}
...
void trigger_push_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
	...
	if (self->spawnflags & PUSH_CONVEYOR) { // only push player if he's on the ground
	...
	if (other && !other->s.number) { // mark that the player has activated this trigger this frame
...
void AimAtTarget(gentity_t *self) {
	...
	if (self->classname && !Q_stricmp("trigger_push", self->classname)) {
		if ((self->spawnflags & 2)) { // check once a second to see if we should activate or deactivate ourselves
...
CHECKCLEAR - Every 1 second, it will check to see if it can trace to the target_position, if it can, the trigger is touchable, if it can't, the trigger is not
touchable NPCONLY - only NPCs are affected RELATIVE - instead of pushing you in a direction that is always from the center of the trigger to the
target_position, it pushes *you* toward the target position, relative to your current location (can use with "speed"... if don't set a speed, it will use the
distance from you to the target_position) CONVEYOR - acts like a conveyor belt, will only push if player is on the ground ( should probably use RELATIVE also,
if you want a true conveyor belt ) INACTIVE - not active until targeted by a target_activate MULTIPLE - multiple entities can touch this trigger in a single
...
void trigger_teleporter_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
	...
	else if (!(self->svFlags & SVF_NO_TELEPORT) && !(self->spawnflags & NO_MISSILES) &&
			 VectorLengthSquared(other->s.pos.trDelta)) { // It's a mover of some sort and is currently moving
...
void hurt_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
	...
	if (self->NPC_targetname && self->NPC_targetname[0]) {						  // I am for you, Kirk
		if (other->script_targetname && other->script_targetname[0]) {			  // must have a name
			if (Q_stricmp(self->NPC_targetname, other->script_targetname) != 0) { // not the right guy to fire me off
				return;
			}
	...
	if (self->delay) {								  // Increase dmg over time
		if (self->attackDebounceTime < self->delay) { // FIXME: this is for the entire trigger, not per person, so if someone else jumped in after you were in
													  // it for 5 seconds, they'd get damaged faster
	...
	if (actualDmg) {
		...
		if (self->spawnflags & 32) { // falling death
			if (other->NPC && other->client &&
				(other->client->NPC_class == CLASS_BOBAFETT || other->client->NPC_class == CLASS_ROCKETTROOPER)) { // boba never falls to his death!
...
void space_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
	...
	if (other->s.m_iVehicleNum && other->s.m_iVehicleNum <= MAX_CLIENTS) { // a player client inside a vehicle
	...
	if (!G_PointInBounds(other->client->ps.origin, self->absmin, self->absmax)) { // his origin must be inside the trigger
		return;
	}
...
/*QUAKED trigger_visible (.1 .5 .1) (-8 -8 -8) (8 8 8) NOTRACE FORCESIGHT x x x x x INACTIVE
  ...
  FOV - how far off to the side of the player's field of view this can be, max, and still be considered "seen".  Player FOV is 80, so the default for this value
  is 30.
