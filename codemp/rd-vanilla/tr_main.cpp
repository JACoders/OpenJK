...
static void SetFarClip(void) {
  ...
  if (tr.refdef.rdflags & RDF_NOWORLDMODEL) {
    if (tr.refdef.rdflags & RDF_AUTOMAP) { // override the zfar then
  ...
  { //volume caps need an "infinite" far clipping plane. So I'm using this
  semi-arbitrary massive number. tr.viewParms.zFar = 524288.0f;
  }
  */
}
...
qboolean R_MirrorViewBySurface(drawSurf_t *drawSurf, int entityNum) {
  ...
  if (tr.viewParms.isPortal) {
    ri.Printf(PRINT_DEVELOPER,
              S_COLOR_RED "WARNING: recursive mirror/portal found\n");
    return qfalse;
  }
