...
int G2API_GetTime(int argTime); // this may or may not return arg depending on
                                // ghoul2_time cvar
...
struct surfaceInfo_t {
  ...
  int surface;  // index into array held inside the model definition of pointers
               // to the actual surface data loaded in - used by both client and
               // game
  float genBarycentricJ; // point 0 barycentric coors
  float genBarycentricI; // point 1 barycentric coors - point 2 is 1 - point0 -
                         // point1
  int genPolySurfaceIndex; // used to point back to the original surface and
                           // poly if this is a generated surface
  int genLod; // used to determine original lod of original surface and poly hit
              // location

  surfaceInfo_t()
      : offFlags(0), surface(0), genBarycentricJ(0), genBarycentricI(0),
        genPolySurfaceIndex(0), genLod(0) {}
...
/*#define MDXABONEDEF				// used in the mdxformat.h file to
stop redefinitions of the bone struct.
...
struct boneInfo_t {
  ...
  mdxaBone_t matrix; // details of bone angle overrides - some are pre-done on
                     // the server, some in ghoul2
  ...
  float animSpeed;    // speed at which this anim runs. 1.0f means full speed of
                      // animation incoming - ie if anim is 20hrtz, we run at
                      // 20hrts. If 5hrts, we run at 5 hrts
  float blendFrame;   // frame PLUS LERP value to blend from
  int blendLerpFrame; // frame to lerp the blend frame with.
  int blendTime;  // Duration time for blending - used to calc amount each frame
                  // of new anim is blended with last frame of the last anim
  int blendStart; // Time when blending starts - not necessarily the same as
                  // startTime since we might start half way through an anim
  int boneBlendTime;    // time for duration of bone angle blend with normal
                        // animation
  int boneBlendStart;   // time bone angle blend with normal animation began
  mdxaBone_t newMatrix; // This is the lerped matrix that Ghoul2 uses on the
                        // client side - does not go across the network
  ...
  int ragIndex;
  vec3_t velocityRoot; // I am really tired of recomiling the whole game to add
                       // a param here
  int ragStartTime;
  int firstTime;
  ...
  mdxaBone_t
      parentOriginalTrueBoneMatrix; // figure I will need this sooner or later
  ...
  vec3_t extraVec1; // I am really tired of recomiling the whole game to add a
                    // param here
  float extraFloat1;
  int extraInt1;
  ...
  vec3_t epVelocity; // velocity factor, can be set, and is also maintained by
                     // physics based on gravity, mass, etc.
  float epGravFactor; // gravity factor maintained by bone physics
  int solidCount; // incremented every time we try to move and are in solid - if
                  // we get out of solid, it is reset to 0
  bool physicsSettled; // true when the bone is on ground and finished bouncing,
                       // etc. but may still be pushed into solid by other bones
  ...
  mdxaBone_t
      animFrameMatrix; // matrix for the bone in the desired settling pose -rww
  ...
  boneInfo_t()
      : boneNumber(-1), flags(0), startFrame(0), endFrame(0), startTime(0),
        pauseTime(0), animSpeed(0), blendFrame(0), blendLerpFrame(0),
        blendTime(0), blendStart(0), boneBlendTime(0), boneBlendStart(0) {
    matrix.matrix[0][0] = matrix.matrix[0][1] = matrix.matrix[0][2] =
        matrix.matrix[0][3] = matrix.matrix[1][0] = matrix.matrix[1][1] =
            matrix.matrix[1][2] = matrix.matrix[1][3] = matrix.matrix[2][0] =
                matrix.matrix[2][1] = matrix.matrix[2][2] =
...
struct boltInfo_t {
  ...
  int surfaceType;   // if we attach to a surface, this tells us if it is an
                     // original surface or a generated one - doesn't go across
                     // the network
  int boltUsed;      // nor does this
  boltInfo_t()
      : boneNumber(-1), surfaceNumber(-1), surfaceType(0), boltUsed(0) {}
...
// NOTE order in here matters. We save out from mModelindex to mFlags, but not
// the STL vectors that are at the top or the bottom.
class CBoneCache;
struct model_s;
// struct mdxaHeader_t;
...
public:
  int ident; // ident of this surface - required so the materials renderer knows
             // what sort of surface this refers to
  CBoneCache *boneCache; // pointer to transformed bone list for this surf
  mdxmSurface_t
      *surfaceData; // pointer to surface data loaded into file - only used by
                    // client renderer DO NOT USE IN GAME SIDE - if there is a
                    // vid restart this will be out of wack on the game

  CRenderableSurface()
      : ident(8), // SF_MDX
        boneCache(0), surfaceData(0) {}

  CRenderableSurface(const CRenderableSurface &rs)
      : ident(rs.ident), boneCache(rs.boneCache), surfaceData(rs.surfaceData) {}
};
#endif
...
public:
  ...
  boltInfo_v mBltlist;
  boneInfo_v mBlist;
// save from here (do not put any ptrs etc within this save block unless you
// adds special handlers to G2_SaveGhoul2Models /
// G2_LoadGhoul2Models!!!!!!!!!!!!
#define BSAVE_START_FIELD                                                      \
  ...
  int mSurfaceRoot;
  int mLodBias;
  int mNewOrigin; // this contains the bolt index of the new origin for this
                  // model
  ...
  qhandle_t mModel; // this and the next entries do NOT go across the network.
                    // They are for gameside access ONLY
  char mFileName[MAX_QPATH];
  int mAnimFrameDefault;
  int mSkelFrameNum;
  int mMeshFrameNum;
  int mFlags; // used for determining whether to do full collision detection
              // against this object
// to here
#define BSAVE_END_FIELD                                                        \
  mTransformedVertsArray // this is the end point for loadsave, keep it up to
                         // date it you change anything
  intptr_t *mTransformedVertsArray; // used to create an array of pointers to
                                    // transformed verts per surface for
                                    // collision detection
  CBoneCache *mBoneCache;
  int mSkin;

  // these occasionally are not valid (like after a vid_restart)
  // call the questionably efficient G2_SetupModelPointers(this) to insure
  // validity
  ...
  CGhoul2Info()
      : mModelindex(-1), animModelIndexOffset(0), mCustomShader(0),
        mCustomSkin(0), mModelBoltLink(0), mSurfaceRoot(0), mLodBias(0),
        mNewOrigin(-1),
#ifdef _G2_GORE
        mGoreSetTag(0),
#endif
        mModel(0), mAnimFrameDefault(0), mSkelFrameNum(-1), mMeshFrameNum(-1),
        mFlags(0), mTransformedVertsArray(0), mBoneCache(0), mSkin(0),
        mValid(false), currentModel(0), currentModelSize(0), animModel(0),
        currentAnimModelSize(0), aHeader(0) {
    mFileName[0] = 0;
  }
...
public:
  ...
  bool IsValid() const { return InfoArray().IsValid(mItem); }
  void kill() {
    // this scary method zeros the infovector handle without actually freeing it
    // it is used for some places where a copy is made, but we don't want to go
    // through the trouble of making a deep copy
    mItem = 0;
  }
...
public:
  ...
  CCollisionRecord() : mDistance(100000), mEntityNum(-1) {}
...
enum EG2_Collision { G2_NOCOLLIDE, G2_COLLIDE, G2_RETURNONHIT };
