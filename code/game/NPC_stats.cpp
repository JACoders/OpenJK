...
stringID_table_t animEventTypeTable[] = {ENUM2STRING(AEV_SOUND),	   //# animID AEV_SOUND framenum soundpath randomlow randomhi chancetoplay
...
stringID_table_t ClassTable[] = {ENUM2STRING(CLASS_NONE), // hopefully this will never be used by an npc), just covering all bases
	...
	for (int i = 0; i < MAX_ANIM_EVENTS; i++) {
		if (animEvents[i].keyFrame == keyFrame) {			 // there is an animevent on this frame already
			if (animEvents[i].eventType == eventType) {		 // and it is of the same type
				if (animEvents[i].modelOnly == modelIndex) { // and it is for the same model
					return i;
				}
			}
		}
...
======================
*/
static void ParseAnimationEvtBlock(int glaIndex, unsigned short modelIndex, const char *aeb_filename, animevent_t *animEvents, animation_t *animations,
	...
	while (1) {
		...
		if (eventType == AEV_NONE || eventType == (animEventType_t)-1) { // Unrecognized ANIM EVENT TYPE
		...
		case AEV_EFFECT: //# animID AEV_EFFECT framenum effectpath boltName
			...
			if (token[0] && Q_stricmp("special", token) == 0) { // special hard-coded effects
...
static void G_ParseAnimationEvtFile(int glaIndex, const char *eventsDirectory, int fileIndex, int iRealGLAIndex = -1, bool modelSpecific = false) {
	...
	bool bIsFrameSkipped =
		(psAnimFileInternalName && strlen(psAnimFileInternalName) > 5 && !Q_stricmp(&psAnimFileInternalName[strlen(psAnimFileInternalName) - 5], "_skip"));
	...
	if (modelName) {
		...
		if (Q_stricmp(skeletonName, modelName) != 0) {
			const int iGLAIndexToCheckForSkip =
				(Q_stricmp(skeletonName, "_humanoid") ? -1 : gi.G2API_PrecacheGhoul2Model("models/players/_humanoid/_humanoid.gla")); // ;-)
...
void G_LoadAnimFileSet(gentity_t *ent, const char *pModelName) {
	...
	if (Q_stricmp("player", pModelName) == 0) { // model is actually stored on console
...
void NPC_PrecacheAnimationCFG(const char *NPC_type) {
	...
	if (!Q_stricmp("random", NPC_type)) { // sorry, can't precache a random just yet
		return;
	}
		...
		while (1) {
			...
			if (!Q_stricmp(token, "height")) {
				...
				if (NPC->client->NPC_class == CLASS_VEHICLE && NPC->m_pVehicle && NPC->m_pVehicle->m_pVehicleInfo &&
					NPC->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER) { // a flying vehicle's origin must be centered in bbox
			...
			if (!Q_stricmp(token, "saber2")) {
				...
				if (!(NPC->client->ps.saber[0].saberFlags & SFL_TWO_HANDED)) { // can't use a second saber if first one is a two-handed saber...?
					char *saberName = G_NewString(value);
					...
					if ((NPC->client->ps.saber[1].saberFlags & SFL_TWO_HANDED)) { // tsk tsk, can't use a twoHanded saber as second saber
			...
			if (!Q_stricmpn(token, "saberRadius", 11)) {
				...
				if (n == -1) { // NOTE: this fills in the rest of the blades with the same length by default
					for (n = 0; n < MAX_BLADES; n++) {
...
#pragma endregion
	...
	*/
	if (!md3Model) {
		NPC->weaponModel[0] = -1;
		if (Q_stricmp("player", playerModel) == 0) { // set the model from the console cvars
	...
	Ghoul2 Insert End
	*/
	if (NPCsPrecached) { // Spawning in after initial precache, our models are precached, we just need to set our clientInfo
...
void NPC_LoadParms(void) {
	...
	for (i = 0; i < fileCnt; i++, holdChar += npcExtFNLen + 1) {
		...
		if (len == -1) {
			...
			if (totallen && *(marker - 1) == '}') { // don't let previous file end on a } because that must be a stand-alone token
				strcat(marker, " ");
				totallen++;
				marker++;
			}
