...
//#define _USINGNEWHUFFTABLE_		// Build a new frequency table to cut
//and paste.
  ...
  if (bits != 32) {
    if (bits > 0) {
      if (value > ((1 << bits) - 1) || value < 0) {
        overflows++;
#ifndef FINAL_BUILD
//				Com_Printf ("MSG_WriteBits: overflow writing %d in %d
...
void MSG_WriteString(msg_t *sb, const char *s) {
  if (!s) {
    ...
    // eurofix: eliminating this means you can chat in european languages. WTF
    // are "old clients" anyway?	-ste
    //
...
void MSG_WriteBigString(msg_t *sb, const char *s) {
  if (!s) {
    ...
    /*
                    // get rid of 0xff chars, because old clients don't like
       them for ( int i = 0 ; i < l ; i++ ) { if ( ((byte *)string)[i] > 127 ) {
                                    string[i] = '.';
                            }
                    }
    */
...
char *MSG_ReadString(msg_t *msg) {
  ...
  // some bonus protection, shouldn't occur cause server doesn't write such
  // things
...
void MSG_WriteDeltaUsercmdKey(msg_t *msg, int key, usercmd_t *from,
  ...
  if (from->angles[0] == to->angles[0] && from->angles[1] == to->angles[1] &&
      ...
      from->forcesel == to->forcesel && from->invensel == to->invensel &&
...
// rww - Remember to update ext_data/MP/netf_overrides.txt if you change any of
// this! (for the sake of being consistent)
...
netField_t entityStateFields[] = {
    ...
    {NETF(legsAnim), 16}, // Maximum number of animation sequences is 2048.  Top
                          // bit is reserved for the togglebit
    // suspicious
    {NETF(torsoAnim), 16}, // Maximum number of animation sequences is 2048. Top
                           // bit is reserved for the togglebit
    // large use beyond GENTITYNUM_BITS - should use generic1 insead
    {NETF(genericenemyindex),
     32}, // Do not change to GENTITYNUM_BITS, used as a time offset for seeker
    {NETF(eFlags), 32},
    ...
    {NETF(fireflag), 2},
    {NETF(event), 10}, // There is a maximum of 256 events (8 bits transmission,
                       // 2 high bits for uniqueness)
    ...
    {NETF(clientNum),
     GENTITYNUM_BITS}, // with npc's clientnum can be > MAX_CLIENTS so use
                       // entnum bits now instead.
    ...
    {NETF(emplacedOwner),
     32}, // As above, also used as a time value (for electricity render time)
    {NETF(apos.trDelta[0]), 0},
    {NETF(apos.trDelta[2]), 0},
    // shouldn't these be better off as flags?  otherwise, they may consume more
    // bits this way
    {NETF(torsoFlip), 1},
    {NETF(angles2[1]), 0},
    ...
    // used for both force pass and an emplaced gun - gun is just a flag
    // indicator
    {NETF(activeForcePass), 6},
    // used to indicate health
    {NETF(health), 10}, // if something's health exceeds 1024, then.. too bad!
    // appears to have multiple means, could be eliminated by indicating a sound
    // set differently
    ...
    {NETF(iModelScale), 10}, // 0-1024 (guess it's gotta be increased if we want
                             // larger allowable scale.. but 1024% is pretty big)
    // full bits used
    {NETF(powerups), MAX_POWERUPS},
    // can this be reduced?
    {NETF(soundSetIndex), 8}, // rww - if MAX_AMBIENT_SETS is changed from 256,
                              // REMEMBER TO CHANGE THIS
    // looks like this can be reduced to 4? (ship parts = 4, people parts = 2)
    {NETF(brokenLimbs),
     8}, // up to 8 limbs at once (not that that many are used)
    {NETF(csSounds_Std),
     8}, // soundindex must be 8 unless max sounds is changed
    // used extensively
    {NETF(saberInFlight), 1},
    {NETF(angles2[0]), 0},
    {NETF(frame), 16},
    {NETF(angles2[2]), 0},
    // why not use torsoAnim and set a flag to do the same thing as forceFrame
    // (saberLockFrame)
    {NETF(forceFrame),
     16}, // if you have over 65536 frames, then this will explode. Of course if
          // you have that many things then lots of things will probably explode.
    {NETF(generic1), 8},
    // do we really need 4 indexes?
    {NETF(boneIndex1),
     6}, // up to 64 bones can be accessed by this indexing method
    // only 54 classes, could cut down 2 bits
    {NETF(NPC_class), 8},
    {NETF(apos.trDuration), 32},
    // there appears to be only 2 different version of parms passed - a flag
    // would better be suited
    {NETF(boneOrient), 9}, // 3 bits per orientation dir
    // this looks to be a single bit flag
    {NETF(bolt1), 8},
    {NETF(trickedentindex3), 16},
    // in use for vehicles
    {NETF(m_iVehicleNum), GENTITYNUM_BITS}, // 10 bits fits all possible entity
                                            // nums (2^10 = 1024). - AReis
    ...
    {NETF(heldByClient), 6},
    // this does not appear to be used in any production or non-cheat fashion -
    // REMOVE
...
void MSG_ReadDeltaEntity(msg_t *msg, entityState_t *from, entityState_t *to,
  ...
  if (lc > numFields || lc < 0)
    Com_Error(ERR_DROP,
              "invalid entityState field count (got: %i, expecting: %i)", lc,
              numFields);
  ...
  if (cl_shownet && (cl_shownet->integer >= 2 || cl_shownet->integer == -1)) {
    print = 1;
    if (sv.state) {
      Com_Printf("%3i: #%-3i (%s) ", msg->readcount, number,
                 SV_GentityNum(number)->classname);
    } else {
...
// rww - Remember to update ext_data/MP/psf_overrides.txt if you change any of
// this! (for the sake of being consistent)
...
netField_t playerStateFields[] = {
    ...
    {PSF(legsAnim), 16}, // Maximum number of animation sequences is 2048.  Top
                         // bit is reserved for the togglebit
    {PSF(delta_angles[0]), 16},
    {PSF(torsoAnim), 16}, // Maximum number of animation sequences is 2048.  Top
                          // bit is reserved for the togglebit
    ...
    {PSF(genericEnemyIndex),
     32}, // NOTE: This isn't just an index all the time, it's often used as a
          // time value, and thus needs 32 bits
    {PSF(events[0]), 10},    // There is a maximum of 256 events (8 bits
                             // transmission, 2 high bits for uniqueness)
    {PSF(events[1]), 10},    // There is a maximum of 256 events (8 bits
                             // transmission, 2 high bits for uniqueness)
    {PSF(customRGBA[0]), 8}, // 0-255
    {PSF(movementDir), 4},
    {PSF(saberEntityNum),
     GENTITYNUM_BITS}, // Also used for channel tracker storage, but should
                       // never exceed entity number
    {PSF(customRGBA[3]), 8}, // 0-255
    {PSF(weaponstate), 4},
    {PSF(saberMove), 32}, // This value sometimes exceeds the max LS_ value and
                          // gets set to a crazy amount, so it needs 32 bits
    {PSF(standheight), 10},
    {PSF(crouchheight), 10},
    {PSF(basespeed), -16},
    {PSF(pm_flags), 16},
    ...
    {PSF(fd.forcePowerLevel[FP_LEVITATION]),
     2}, // unfortunately we need this for fall damage calculation (client needs
         // to know the distance for the fall noise)
    ...
    {PSF(damageYaw), 8},
    {PSF(damageCount), 8},
    {PSF(inAirAnim), 1}, // just transmit it for the sake of knowing right when
                         // on the client to play a land anim, it's only 1 bit
    {PSF(eventParms[1]), 8},
    {PSF(fd.forceSide), 2}, // so we know if we should apply greyed out shaders
                            // to dark/light force enlightenment
    ...
    {PSF(loopSound),
     16}, // rwwFIXMEFIXME: max sounds is 256, doesn't this only need to be 8?
    {PSF(hasLookTarget), 1},
    {PSF(saberBlocked), 8},
    {PSF(damageType), 2},
    ...
    {PSF(damagePitch), 8},
    {PSF(m_iVehicleNum), GENTITYNUM_BITS}, // 10 bits fits all possible entity
                                           // nums (2^10 = 1024). - AReis
    ...
    {PSF(fd.forceMindtrickTargetIndex),
     16}, // NOTE: Not just an index, used as a (1 << val) bitflag for up to 16
          // clients
    {PSF(fd.forceMindtrickTargetIndex2),
     16}, // NOTE: Not just an index, used as a (1 << val) bitflag for up to 16
          // clients
    //{ PSF(vehWeaponsLinked), 1 },//only used by vehicle?
    {PSF(lastHitLoc[2]), 0},
    //{ PSF(hyperSpaceTime), 32 },//only used by vehicle?
    {PSF(fd.forceMindtrickTargetIndex3),
     16}, // NOTE: Not just an index, used as a (1 << val) bitflag for up to 16
          // clients
    {PSF(lastHitLoc[0]), 0},
    {PSF(eFlags2), 10},
    {PSF(fd.forceMindtrickTargetIndex4),
     16}, // NOTE: Not just an index, used as a (1 << val) bitflag for up to 16
          // clients
    //{ PSF(hyperSpaceAngles[1]), 0 },//only used by vehicle?
    {PSF(lastHitLoc[1]),
     0}, // currently only used so client knows to orient disruptor
         // disintegration.. seems a bit much for just that though.
    //{ PSF(vehBoarding), 1 }, //only used by vehicle? not like the normal
    //boarding value, this is a simple "1 or 0" value
    ...
    //{ PSF(vehSurfaces), 16 }, //only used by vehicle? allow up to 16 surfaces
    //in the flag I guess
    {PSF(fd.forcePowerLevel[FP_SEE]),
     2}, // needed for knowing when to display players through walls
    ...
    {PSF(iModelScale), 10}, // 0-1024 (guess it's gotta be increased if we want
                            // larger allowable scale.. but 1024% is pretty big)
    {PSF(hackingBaseTime),
     16}, // up to 65536ms, over 10 seconds would just be silly anyway
...
netField_t pilotPlayerStateFields[] = {
    ...
    //===THE REST OF THESE SHOULD NOT BE RELEVANT, BUT, FOR SAFETY, INCLUDE THEM
    //ANYWAY, JUST AT THE
    //BOTTOM===============================================================
    {PSF(velocity[0]), 0},
    {PSF(velocity[1]), 0},
    ...
    //{ PSF(vehTurnaroundIndex), GENTITYNUM_BITS },//only used by vehicle?
    //{ PSF(vehSurfaces), 16 }, //only used by vehicle? allow up to 16 surfaces
    //in the flag I guess { PSF(vehBoarding), 1 }, //only used by vehicle? not
    //like the normal boarding value, this is a simple "1 or 0" value {
    //PSF(hyperSpaceAngles[1]), 0 },//only used by vehicle? {
    //PSF(hyperSpaceAngles[0]), 0 },//only used by vehicle? {
    //PSF(hyperSpaceAngles[2]), 0 },//only used by vehicle?
...
netField_t vehPlayerStateFields[] = {
    ...
    {PSF(legsTimer), 16},
    {PSF(rocketLockIndex), GENTITYNUM_BITS},
    //{ PSF(genericEnemyIndex), 32 }, //NOTE: This isn't just an index all the
    //time, it's often used as a time value, and thus needs 32 bits
    {PSF(events[0]), 10}, // There is a maximum of 256 events (8 bits
                          // transmission, 2 high bits for uniqueness)
    {PSF(events[1]), 10}, // There is a maximum of 256 events (8 bits
                          // transmission, 2 high bits for uniqueness)
    ...
    {PSF(moveDir[2]), 0},
    {PSF(rocketTargetTime), 32},
    //{ PSF(activeForcePass), 6 },//actually, you only need to know this for
    //other vehicles, not your own
    {PSF(electrifyTime), 32},
    //{ PSF(fd.forceJumpZStart), 0 },//set on rider by vehicle, but not used by
    //vehicle
    {PSF(loopSound),
     16}, // rwwFIXMEFIXME: max sounds is 256, doesn't this only need to be 8?
    {PSF(rocketLockTime), 32},
    ...
    //{ PSF(generic1), 8 },//used by passengers of vehicles, but not vehicles
    //themselves
    {PSF(hackingTime), 32},
    {PSF(brokenLimbs),
     8}, // up to 8 limbs at once (not that that many are used)
    ...
    {PSF(vehBoarding),
     1}, // not like the normal boarding value, this is a simple "1 or 0" value
...
// The unoptimized way, throw *all* the vehicle stuff into the playerState along
...
void MSG_CheckNETFPSFOverrides(qboolean psfOverrides) {
  ...
  if (psfOverrides) { // do PSF overrides instead of NETF
  ...
  if (*bitStorage) { // if we have saved off the defaults before we want to
                     // stuff them all back in now
  ...
  if (!f || len < 0) { // silently exit since this file is not needed to
                       // proceed.
    return;
  }
  ...
  if (!*bitStorage) {
    ...
    while (i < numFields) {
      ...
      *bitStorage =
          (bitStorage_t *)Z_Malloc(sizeof(bitStorage_t), TAG_GENERAL, qtrue);
  ...
  while (overrideFile[i]) {
    if (overrideFile[i] == ';') { // parse to end of the line
      while (overrideFile[i] != '\n') {
        i++;
      }
    }

    if (overrideFile[i] != ';' && overrideFile[i] != '\n' &&
        overrideFile[i] != '\r') { // on a valid char I guess, parse it
      j = 0;
      ...
      if (!overrideFile[i]) { // just give up, this shouldn't happen
        Com_Printf("WARNING: Parsing error for %s\n", fileName);
        return;
      }

      while (overrideFile[i] == ',' ||
             overrideFile[i] == ' ') { // parse to the start of the value
        i++;
      }

      j = 0;
      while (overrideFile[i] != '\n' &&
             overrideFile[i] != '\r') { // now read the value in
      ...
      if (bits[0]) {
        if (!strcmp(bits, "GENTITYNUM_BITS")) { // special case
        ...
        while (j < numFields) {
          if (psfOverrides) { // check psf fields
            if (!strcmp(playerStateFields[j].name,
                        entryName)) { // found it, set the bits
              playerStateFields[j].bits = ibits;
              break;
            }
          } else { // otherwise check netf fields
            if (!strcmp(entityStateFields[j].name,
                        entryName)) { // found it, set the bits
        ...
        if (j == numFields) { // failed to find the value
          Com_Printf("WARNING: Value '%s' from %s is not valid\n", entryName,
                     fileName);
        }
      } else { // also should not happen
        Com_Printf("WARNING: Parsing error for %s\n", fileName);
        return;
      }
    }
...
void MSG_WriteDeltaPlayerstate(msg_t *msg, struct playerState_s *from,
                               ...
                               int *bitNumDelta, qboolean isVehiclePS) {
#else
void MSG_WriteDeltaPlayerstate(msg_t *msg, struct playerState_s *from,
                               struct playerState_s *to, qboolean isVehiclePS) {
#endif
  int i;
  playerState_t dummy;
  int statsbits;
...
#ifdef _ONEBIT_COMBO
    if (numBitsInMask < 32 && field->bits == 1) {
    ...
    for (i = 0; i < MAX_STATS; i++) {
      if (statsbits & (1 << i)) {
        if (i == STAT_WEAPONS) { // ugly.. but we're gonna need it anyway -rww
          //(just send this one in MAX_WEAPONS bits, so that we can add up to
          //MAX_WEAPONS weaps without hassle)
...
#ifdef _ONEBIT_COMBO
sendBitMask:
  if (numBitsInMask) { // don't need to send at all if we didn't pass any 1bit
                       // values
    if (!bitComboDelta || bitComboMask != *bitComboDelta ||
        numBitsInMask != *bitNumDelta) { // send the mask, it changed
      MSG_WriteBits(msg, 1, 1);
    ...
    } else { // send 1 bit 0 to indicate no change
      MSG_WriteBits(msg, 0, 1);
    }
  }
#endif
...
void MSG_ReadDeltaPlayerstate(msg_t *msg, playerState_t *from,
                              playerState_t *to, qboolean isVehiclePS) {
  int i, lc;
  int bits;
  netField_t *field;
  netField_t *PSFields = playerStateFields;
...
//=====_OPTIMIZED_VEHICLE_NETWORKING=======================================================================
  ...
  if (lc > numFields || lc < 0)
    Com_Error(ERR_DROP,
              "invalid playerState field count (got: %i, expecting: %i)", lc,
              numFields);
...
/*
// New data gathered to tune Q3 to JK2MP. Takes longer to crunch and gain was
minimal. int msg_hData[256] =
{
        3163878,		// 0
        473992,			// 1
        564019,			// 2
