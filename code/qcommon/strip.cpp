...
public:
  cStringPackageID(const char *in_name, byte in_reg) {
    name = in_name;
    reg = in_reg;
  }
...
public:
  cStringPackage(const char *in, unsigned char initID = 0,
                 char *initDescription = NULL, char *initReference = NULL);
  virtual ~cStringPackage(void);

  void Register(unsigned char newRegistration) {
    Registration |= newRegistration;
  }
  bool UnRegister(unsigned char oldRegistration) {
    Registration &= ~oldRegistration;
    return (Registration == 0);
  }
  bool RegisteredOnServer(void) const {
    return (!!(Registration & SP_REGISTER_SERVER));
  }
...
public:
  cStringPackageSingle(const char *in, unsigned char initID = 0,
                       char *initReference = NULL);
...
const char *Tokens[TK_END] = {"TEXT_LANGUAGE1",
                              "TEXT_LANGUAGE2",
                              "TEXT_LANGUAGE3",
                              "TEXT_LANGUAGE4",
                              "TEXT_LANGUAGE5",
...
sFlagPair FlagPairs[] = {
    {TK_SP_FLAG1, SP_FLAG1}, {TK_SP_FLAG2, SP_FLAG2},
    {TK_SP_FLAG3, SP_FLAG3}, {TK_SP_FLAG4, SP_FLAG4},
    {TK_SP_FLAG5, SP_FLAG5}, {TK_SP_FLAG6, SP_FLAG6},
    {TK_SP_FLAG7, SP_FLAG7}, {TK_SP_FLAG8, SP_FLAG8},
    {TK_SP_FLAG9, SP_FLAG9}, {TK_SP_FLAG_ORIGINAL, SP_FLAG_ORIGINAL},
    {TK_INVALID, 0}};

sFlagPair LanguagePairs[] = {{TK_TEXT_LANGUAGE1, SP_LANGUAGE_ENGLISH},
...
int FindToken(char *token, bool whole) {
  ...
  for (token_value = 0; token_value != TK_END; token_value++) {
    if (whole) {
      ...
      if (Q_stricmpn(token, Tokens[token_value], strlen(Tokens[token_value])) ==
          0) {
...
void GetLine(char *&Data, int &Size, int &token, char *&data) {
  ...
  pos = temp_data;
  while ((*pos) && strchr(" \n\r", *pos)) { // remove white space
    pos++;
  }
  test_token = pos;

  while ((*pos) && !strchr(" \n\r", *pos)) { // scan until end of white space
    pos++;
  }
 ...
 *
 ************************************************************************************************/
void cStrings::SetFlags(unsigned int newFlags) { Flags = newFlags; }
...
cStringsSingle::~cStringsSingle() { Clear(); }
...
// fix problems caused by fucking morons entering clever "rich" chars in to new
// text files *after* the auto-stripper
//	removed them all in the first place...
//
// ONLY DO THIS FOR EUROPEAN LANGUAGES, OR IT BREAKS ASIAN
// STRINGS!!!!!!!!!!!!!!!!!!!!!
//
...
bool cStringsSingle::UnderstandToken(int token, char *data) {
  ...
  //	{
  //		default:
  for (LanguagePair = LanguagePairs; LanguagePair->Name != TK_INVALID;
       LanguagePair++) {
    if (LanguagePair->Name == TK_TEXT_LANGUAGE1 && token == TK_TEXT_LANGUAGE1 &&
        !Text) { // default to english in case there is no foreign
      ...
      SetText(data);
      return true;
    } else if (LanguagePair->Name == token &&
               (signed)LanguagePair->Value == sp_language->integer) {
      if (LanguagePair->Name == TK_TEXT_LANGUAGE1 ||
          LanguagePair->Name == TK_TEXT_LANGUAGE2 ||
          LanguagePair->Name == TK_TEXT_LANGUAGE3 ||
          LanguagePair->Name == TK_TEXT_LANGUAGE8) {
...
cStringPackage::cStringPackage(const char *in, unsigned char initID,
                               char *initDescription, char *initReference) {
  ID = initID;
  Registration = 0;
  name = in;
  Reference = NULL;
...
bool cStringPackage::UnderstandToken(char *&Data, int &Size, int token,
                                     char *data) {
  switch (token) {
  case TK_ID:
...
cStringPackageSingle::cStringPackageSingle(const char *in, unsigned char initID,
                                           char *initReference)
    : cStringPackage(in, initID, initReference) {}

cStringPackageSingle::~cStringPackageSingle(void) { ReferenceTable.clear(); }
...
bool cStringPackageSingle::UnderstandToken(char *&Data, int &Size, int token,
                                           char *data) {
  int count, i, pos;
...
qboolean JK2SP_Register(const char *inPackage, unsigned char Registration) {
  ...
  if (i != JK2SP_ListByName.end()) {
    ...
    if (Registration & SP_REGISTER_CLIENT) {
      Com_DPrintf(S_COLOR_YELLOW "JK2SP_Register: Registered client string "
                                 "package '%s' with ID %02x\n",
                  Package, (int)new_sp->GetID());
    } else {
      Com_DPrintf(
          S_COLOR_YELLOW
          "JK2SP_Register: Registered string package '%s' with ID %02x\n",
          Package, (int)new_sp->GetID());
    }
  ...
  for (i = JK2SP_ListByName.begin(); i != JK2SP_ListByName.end(); i = next) {
    ...
    if ((*i).second->UnRegister(Registration)) {
      Com_DPrintf(S_COLOR_YELLOW
                  "JK2SP_UnRegister: Package '%s' with ID %02x\n",
...
//
const char *JK2SP_GetReferenceText(unsigned short ID,
                                   const char *&psPackageName,
                                   const char *&psPackageReference,
                                   const char *&psText) {
  cStringPackageSingle *sp;
  std::map<unsigned char, cStringPackageSingle *>::iterator i;
...
static void JK2SP_UpdateLanguage(void) {
  ...
  for (it = JK2SP_ListByID.begin(); it != JK2SP_ListByID.end(); ++it) {
    sps.push_back(cStringPackageID((*it).second->GetName(),
                                   (*it).second->GetRegistration()));
  }
  // Clear out all pointers
  JK2SP_Unload(SP_REGISTER_CLIENT | SP_REGISTER_SERVER | SP_REGISTER_MENU |
               SP_REGISTER_REQUIRED);
...
void JK2SP_Init(void) {
  sp_language = Cvar_Get("sp_language", va("%d", SP_LANGUAGE_ENGLISH),
                         CVAR_ARCHIVE | CVAR_NORESTART);
  sp_show_strip =
      Cvar_Get("sp_show_strip", "0",
               0); // don't switch this on!!!!!!, test only (apparently)
  sp_leet = Cvar_Get("sp_leet", "0",
                     CVAR_ROM); // do NOT leave this on in final product!!!!
                                // (only works when == 42 anyway ;-)

  //	Cvar_Set("sp_language", va("%d", SP_LANGUAGE_JAPANESE));	// stetest,
  //do NOT leave in
