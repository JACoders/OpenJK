...
static void CL_UpdateGUID(const char *prefix, int prefix_len) {
	if (cl_enableGuid->integer) {
		...
		if (len != QKEY_SIZE) {
			...
			Cvar_Set("ja_guid", Com_MD5File(QKEY_FILE, QKEY_SIZE, prefix, prefix_len));
		}
	} else {
...
#endif
static void CL_CheckSVStringEdRef(char *buf, const char *str) { // I don't really like doing this. But it utilizes the system that was already in place.
	int i = 0;
	int b = 0;
	int strLen = 0;
...
void CL_Frame(int msec) {
	...
	if (cls.state == CA_DISCONNECTED && !(Key_GetCatcher() & KEYCATCH_UI) && !com_sv_running->integer && cls.uiStarted) {
