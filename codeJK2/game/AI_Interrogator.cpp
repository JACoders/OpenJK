...
-------------------------
*/
void Interrogator_die(gentity_t *self, gentity_t *inflictor,
...
void Interrogator_PartsMove(void) {
  ...
  if (TIMER_Done(NPC, "scalpelDelay")) {
    ...
    gi.G2API_SetBoneAnglesIndex(
        &NPC->ghoul2[NPC->playerModel], NPC->genericBone2, NPC->pos2,
        BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, NULL, 0, 0);
  }
  ...
  gi.G2API_SetBoneAnglesIndex(&NPC->ghoul2[NPC->playerModel], NPC->genericBone3,
                              NPC->pos3, BONE_ANGLES_POSTMULT, POSITIVE_X,
...
void Interrogator_Melee(qboolean visible, qboolean advance) {
  ...
  {
    // Make sure that we are within the height range before we allow any damage
    // to happen
    if (NPC->currentOrigin[2] >=
            NPC->enemy->currentOrigin[2] + NPC->enemy->mins[2] &&
        NPC->currentOrigin[2] + NPC->mins[2] + 8 <
      ...
      G_Sound(NPC,
              G_SoundIndex(
                  "sound/chars/interrogator/misc/torture_droid_inject.mp3"));
    }
  }
