...
extern saberMoveName_t G_PickAutoKick(gentity_t *self, gentity_t *enemy,
                                      qboolean storeMove);
extern saberMoveName_t
G_PickAutoMultiKick(gentity_t *self, qboolean allowSingles, qboolean storeMove);
extern qboolean NAV_DirSafe(gentity_t *self, vec3_t dir, float dist);
extern qboolean NAV_MoveDirSafe(gentity_t *self, usercmd_t *cmd,
...
extern qboolean G_ClearLineOfSight(const vec3_t point1, const vec3_t point2,
...
void NPC_Jedi_PlayConfusionSound(gentity_t *self) {
  if (self->health > 0) {
    if (self->client && (self->client->NPC_class == CLASS_ALORA ||
...
qboolean Jedi_StopKnockdown(gentity_t *self, gentity_t *pusher,
                            const vec3_t pushDir) {
  if (self->s.number < MAX_CLIENTS || !self->NPC) { // only NPCs
    return qfalse;
  }

  if (self->client->ps.forcePowerLevel[FP_LEVITATION] <
      FORCE_LEVEL_1) { // only force-users
    return qfalse;
  }

  if (self->client->moveType ==
      MT_FLYSWIM) { // can't knock me down when I'm flying
    return qtrue;
  }

  if (self->NPC &&
      (self->NPC->aiFlags &
       NPCAI_BOSS_CHARACTER)) { // bosses always get out of a knockdown
  } else if (Q_irand(0, RANK_CAPTAIN + 5) >
             self->NPC->rank) { // lower their rank, the more likely they are
                                // fall down
    return qfalse;
  }
  ...
  if (NPC->genericBolt1 != -1) {
    ...
    for (int time = curTime - 25; time <= curTime + 25 && !hit; time += 25) {
      ...
      gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->weaponModel[1],
                             NPC->genericBolt1, &boltMatrix, angles,
                             NPC->currentOrigin, time, NULL, NPC->s.modelScale);
...
#endif
      gi.trace(&trace, base, vec3_origin, vec3_origin, tip, NPC->s.number,
               MASK_SHOT, G2_RETURNONHIT, 10);
      if (trace.fraction < 1.0f) { // hit something
        ...
        // if ( time == curTime )
        { // UGH
          G_PlayEffect(G_EffectIndex("scepter/impact.efx"), trace.endpos,
                       trace.plane.normal);
        }

        if (traceEnt->takedamage && trace.entityNum != lastHit &&
            (!traceEnt->client || traceEnt == NPC->enemy ||
             traceEnt->client->NPC_class != NPC->client->NPC_class)) { // smack
          int dmg = Q_irand(10, 20) * (g_spskill->integer + 1); // NOTE: was
                                                                // 6-12
          // FIXME: debounce?
          // FIXME: do dismemberment
          G_Damage(traceEnt, NPC, NPC, vec3_origin, trace.endpos, dmg,
                   DAMAGE_NO_KNOCKBACK, MOD_SABER); // MOD_MELEE );
          if (traceEnt->client) {
            if (!Q_irand(0, 2)) {
...
void Tavion_ScepterSlam(void) {
  ...
  int boltIndex =
      gi.G2API_AddBolt(&NPC->ghoul2[NPC->weaponModel[1]], "*weapon");
  if (boltIndex != -1) {
    mdxaBone_t boltMatrix;
    ...
    gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->weaponModel[1], boltIndex,
                           &boltMatrix, angles, NPC->currentOrigin,
    ...
    gi.trace(&trace, handle, vec3_origin, vec3_origin, bottom, NPC->s.number,
             MASK_SHOT, G2_RETURNONHIT, 10);
    G_PlayEffect(G_EffectIndex("scepter/slam.efx"), trace.endpos,
                 trace.plane.normal);
    ...
    for (i = 0; i < numEnts; i++) {
      ...
      if (radiusEnts[i]->client == NULL) { // must be a client
        if (G_EntIsBreakable(
                ...
                NPC)) { // damage breakables within range, but not as much
          G_Damage(radiusEnts[i], NPC, NPC, vec3_origin,
                   radiusEnts[i]->currentOrigin, 100, 0, MOD_EXPLOSIVE_SPLASH);
        }
        continue;
      }
      ...
      if (dist <= radius) {
        if (dist < halfRad) { // close enough to do damage, too
          G_Damage(radiusEnts[i], NPC, NPC, vec3_origin,
                   radiusEnts[i]->currentOrigin, Q_irand(20, 30),
            ...
            if (dist < halfRad ||
                radiusEnts[i]->client->ps.groundEntityNum !=
                    ENTITYNUM_NONE) { // within range of my fist or within
                                      // ground-shaking range and not in the air
...
void Tavion_StartScepterBeam(void) {
  G_PlayEffect(G_EffectIndex("scepter/beam_warmup.efx"), NPC->weaponModel[1],
               NPC->genericBolt1, NPC->s.number, NPC->currentOrigin, 0, qtrue);
  ...
  NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_SCEPTER_START,
...
void Tavion_StartScepterSlam(void) {
  G_PlayEffect(G_EffectIndex("scepter/slam_warmup.efx"), NPC->weaponModel[1],
               NPC->genericBolt1, NPC->s.number, NPC->currentOrigin, 0, qtrue);
  G_SoundOnEnt(NPC, CHAN_ITEM, "sound/weapons/scepter/slam_warmup.wav");
  NPC->client->ps.legsAnimTimer = NPC->client->ps.torsoAnimTimer = 0;
  NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_TAVION_SCEPTERGROUND,
              SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  NPC->painDebounceTime = level.time + NPC->client->ps.torsoAnimTimer;
  NPC->client->ps.pm_time = NPC->client->ps.torsoAnimTimer;
  NPC->client->ps.pm_flags |= PMF_TIME_KNOCKBACK;
...
void Tavion_SithSwordRecharge(void) {
  if (NPC->client->ps.torsoAnim != BOTH_TAVION_SWORDPOWER && NPC->count &&
      TIMER_Done(NPC, "rechargeDebounce") && NPC->weaponModel[0] != -1) {
    NPC->s.loopSound = G_SoundIndex("sound/weapons/scepter/recharge.wav");
    int boltIndex =
        gi.G2API_AddBolt(&NPC->ghoul2[NPC->weaponModel[0]], "*weapon");
    NPC->client->ps.legsAnimTimer = NPC->client->ps.torsoAnimTimer = 0;
    NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_TAVION_SWORDPOWER,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    G_PlayEffect(G_EffectIndex("scepter/recharge.efx"), NPC->weaponModel[0],
                 boltIndex, NPC->s.number, NPC->currentOrigin,
                 NPC->client->ps.torsoAnimTimer, qtrue);
    NPC->painDebounceTime = level.time + NPC->client->ps.torsoAnimTimer;
    NPC->client->ps.pm_time = NPC->client->ps.torsoAnimTimer;
    NPC->client->ps.pm_flags |= PMF_TIME_KNOCKBACK;
    VectorClear(NPC->client->ps.velocity);
    VectorClear(NPC->client->ps.moveDir);
    NPC->client->ps.powerups[PW_INVINCIBLE] =
        level.time + NPC->client->ps.torsoAnimTimer + 10000;
    G_PlayEffect(G_EffectIndex("scepter/invincibility.efx"), NPC->playerModel,
                 0, NPC->s.number, NPC->currentOrigin,
                 NPC->client->ps.torsoAnimTimer + 10000, qfalse);
    TIMER_Set(NPC, "rechargeDebounce",
              NPC->client->ps.torsoAnimTimer + 10000 + Q_irand(10000, 20000));
    NPC->count--;
    ...
    if (NPC->enemy && NPC->enemy->client &&
        (NPC->enemy->client->NPC_class == CLASS_RANCOR ||
         ...
         NPC->enemy->client->NPC_class ==
             CLASS_SAND_CREATURE)) { // never taunt these mindless creatures
                                     // NOTE: howlers?  tusken?  etc?  Only
                                     // reborn?
    } else {
...
static qboolean Jedi_ClearPathToSpot(vec3_t dest, int impactEntNum) {
  ...
  gi.trace(&trace, NPC->currentOrigin, mins, NPC->maxs, dest, NPC->s.number,
           NPC->clipmask, (EG2_Collision)0, 0);
...
qboolean NPC_MoveDirClear(int forwardmove, int rightmove, qboolean reset) {
  ...
  */
  ...
  gi.trace(&trace, NPC->currentOrigin, mins, NPC->maxs, testPos, NPC->s.number,
           NPC->clipmask | CONTENTS_BOTCLIP, (EG2_Collision)0, 0);
  if (trace.allsolid ||
...
static qboolean Jedi_Move(gentity_t *goal, qboolean retreat) {
  ...
  if (retreat) { // FIXME: should we trace and make sure we can go this way?  Or
    ...
    ucmd.forwardmove *= -1;
    ucmd.rightmove *= -1;
    // we clear moveDir here so the Jedi's ucmd-driven movement does do not
    // enter checks
    VectorClear(NPC->client->ps.moveDir);
    // VectorScale( NPC->client->ps.moveDir, -1, NPC->client->ps.moveDir );
  }
...
static qboolean Jedi_Hunt(void) {
  ...
  if (NPCInfo->stats.aggression > 1) { // approach enemy
    ...
    if (!(NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) {
      ...
      //			NPC_SetMoveGoal(NPC,
      //NPC->enemy->currentOrigin, 40.0f, false, 0, NPC->enemy);
      NPCInfo->goalEntity = NPC->enemy;
...
static void Jedi_AdjustSaberAnimLevel(gentity_t *self, int newLevel) {
  ...
  if (!(self->client->ps.saberStylesKnown &
        (1 << newLevel))) { // don't know that style, sorry
    return;
...
static void Jedi_CheckDecreaseSaberAnimLevel(void) {
  if (!NPC->client->ps.weaponTime &&
      !(ucmd.buttons & (BUTTON_ATTACK | BUTTON_ALT_ATTACK |
                        BUTTON_FORCE_FOCUS))) { // not attacking
...
static qboolean Jedi_DecideKick(void) {
  ...
  if (!NPC->enemy ||
      (NPC->enemy->s.number < MAX_CLIENTS &&
       NPC->enemy->health <= 0)) { // have no enemy or enemy is a dead player
    return qfalse;
  }
  // FIXME: check FP_SABER_OFFENSE?
  // FIXME: check for saber staff style only?
  // FIXME: g_spskill?
  if (Q_irand(0, RANK_CAPTAIN + 5) >
      NPCInfo->rank) { // low chance, based on rank
    return qfalse;
  }
...
void Kyle_GrabEnemy(void) {
  WP_SabersCheckLock2(NPC, NPC->enemy,
                      (sabersLockMode_t)Q_irand(
                          LOCK_KYLE_GRAB1, LOCK_KYLE_GRAB2)); // LOCK_KYLE_GRAB3
  TIMER_Set(NPC, "grabEnemyDebounce",
            NPC->client->ps.torsoAnimTimer + Q_irand(4000, 20000));
}

void Kyle_TryGrab(void) {
  NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_KYLE_GRAB,
              SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  NPC->client->ps.torsoAnimTimer += 200;
      ...
      if (TIMER_Done(NPC, "grabEnemyDebounce")) { // okay to grab again
        if (NPC->client->ps.groundEntityNum != ENTITYNUM_NONE &&
            NPC->enemy->client->ps.groundEntityNum !=
                ENTITYNUM_NONE) { // me and enemy are on ground
          if (!PM_InOnGroundAnim(&NPC->enemy->client->ps)) {
            if ((NPC->client->ps.weaponTime <= 200 ||
                 NPC->client->ps.torsoAnim == BOTH_KYLE_GRAB) &&
                !NPC->client->ps.saberInFlight) {
              if (fabs(NPC->enemy->currentOrigin[2] - NPC->currentOrigin[2]) <=
                  8.0f) { // close to same level of ground
                if (DistanceSquared(NPC->enemy->currentOrigin,
                                    NPC->currentOrigin) <= 10000.0f) {
                  return qtrue;
                }
              }
            }
...
static void Jedi_CombatDistance(
  ...
  if (enemy_dist < 128 && NPC->enemy && NPC->enemy->client &&
      (NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK6 ||
       NPC->enemy->client->ps.torsoAnim ==
           BOTH_SPINATTACK7)) { // whoa, back off!!!
  ...
  if (NPC->client->ps.forcePowersActive & (1 << FP_DRAIN) &&
      NPC->client->ps.forcePowerLevel[FP_DRAIN] >
  ...
  if (NPC->client->NPC_class == CLASS_BOBAFETT) {
  ...
  } else if (NPC->client->ps.legsAnim ==
             BOTH_ALORA_SPIN_THROW) { // don't move at all
                                      // FIXME: sabers need trails
  } else if (NPC->client->ps.torsoAnim ==
             BOTH_KYLE_GRAB) { // see if we grabbed enemy
    if (NPC->client->ps.torsoAnimTimer <= 200) {
      if (Kyle_CanDoGrab() &&
          NPC_EnemyRangeFromBolt(NPC->handRBolt) <= 72.0f) { // grab him!
        Kyle_GrabEnemy();
        return;
      } else {
        ...
        ForceThrow(NPC, qfalse);
             ...
             ((NPCInfo->scriptFlags & SCF_DONT_FIRE) ||
              (!Q_stricmp("Yoda", NPC->NPC_type) &&
               !Q_irand(
                   0, 10)))) { // can't use saber and they're in striking range
    if (!Q_irand(0, 5) && InFront(NPC->enemy->currentOrigin, NPC->currentOrigin,
      ...
      if (((NPCInfo->scriptFlags & SCF_DONT_FIRE) ||
          ...
          && WP_ForcePowerUsable(NPC, FP_DRAIN,
                                 20) // know how to drain and have enough power
          && !Q_irand(0, 2)) {       // drain
        ...
        if (Jedi_DecideKick()) { // let's try a kick
          if (G_PickAutoMultiKick(NPC, qfalse, qtrue) != LS_NONE ||
              (G_CanKickEntity(NPC, NPC->enemy) &&
               G_PickAutoKick(NPC, NPC->enemy, qtrue) != LS_NONE)) { // kicked!
    ...
    Jedi_Retreat();
             ...
             NPC->max_health - NPC->health >
                 NPC->max_health * 0.25f // lost over 1/4 of our health
             && NPC->client->ps.forcePowersKnown &
    ...
    if (NPCInfo->stats.aggression < Q_irand(0, 20) &&
        NPC->health < NPC->max_health * 0.75f && !Q_irand(0, 2)) {
      if (NPC->enemy && NPC->enemy->s.number < MAX_CLIENTS &&
          ...
          Q_irand(0, 3 - g_spskill->integer)) { // hmm, bosses should do this
                                                // less against the player
    ...
    if (NPC->enemy && NPC->enemy->client &&
      ...
      {                   // throw saber
        ucmd.buttons |= BUTTON_ALT_ATTACK;
               ...
               NPC->enemy->client->ps.saber[0]
                   .Active() &&               // enemy throwing saber
               ...
               && InFOV(NPC->enemy->currentOrigin, NPC->currentOrigin,
                        NPC->client->ps.viewangles, 20,
                        30)) { // They're throwing their saber, grip them!
      // taunt
      ...
      if (NPC->client->NPC_class == CLASS_KYLE && (NPC->spawnflags & 1) &&
          ...
          chanceScale = 4;
                   ...
                   ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) ||
                    NPC->client->NPC_class ==
                        CLASS_SHADOWTROOPER)) { // hmm, bosses do this less
                                                // against player
          chanceScale = 8 - g_spskill->integer * 2;
        } else if (NPC->client->NPC_class == CLASS_DESANN ||
                   !Q_stricmp("Yoda", NPC->NPC_type))
        //|| (NPC->client->NPC_class == CLASS_CULTIST && NPC->client->ps.weapon
        //== WP_NONE) )//force-only cultists use force a lot
        {
          chanceScale = 1;
        ...
        if (chanceScale &&
          ...
          //			the only thing they might be able to do is throw their
          //saber
          if ((NPCInfo->rank == RANK_ENSIGN  // old reborn crap
            ...
            if (WP_ForcePowerUsable(NPC, FP_PULL, 0) && !Q_irand(0, 2)) {
              ...
              if (Q_irand(0, 1)) {
                ucmd.buttons |= BUTTON_ATTACK;
                       ...
                       (((NPCInfo->scriptFlags & SCF_DONT_FIRE) &&
                        ...
                        Q_irand(0, 1))) {
              ForceLightning(NPC);
              if (NPC->client->ps.forcePowerLevel[FP_LIGHTNING] >
                  FORCE_LEVEL_1) {
              ...
              TIMER_Set(NPC, "attackDelay", NPC->client->ps.weaponTime);
                       ...
                       WP_ForcePowerUsable(NPC, FP_DRAIN, 0) &&
                       (((NPCInfo->scriptFlags & SCF_DONT_FIRE) &&
                         Q_stricmp("cultist_drain", NPC->NPC_type)) ||
                        Q_irand(0, 1))) {
              ForceDrain2(NPC);
              NPC->client->ps.weaponTime =
                  Q_irand(1000, 3000 + (g_spskill->integer * 500));
  ...
  if (self->s.number) { // if an NPC, check game skill setting
    ...
    */
    ...
    if (!(self->client->ps.forcePowersActive &
          (1 << FP_SPEED))) { // not already in speed
      ...
      if (self->client &&
          (self->client->ps.forceRageRecoveryTime > level.time ||
           (self->client->ps.forcePowersActive &
            (1 << FP_RAGE)))) { // no fancy dodges when raging or recovering
        return qfalse;
      }
    ...
    if (self->s.number < MAX_CLIENTS) {                             // player
      if ((self->client->ps.forcePowersActive & (1 << FP_SPEED))) { // in speed
        if (PM_DodgeAnim(self->client->ps.torsoAnim) &&
            !PM_DodgeHoldAnim(
                self->client->ps.torsoAnim)) { // already in a dodge
...
evasionType_t Jedi_CheckFlipEvasions(gentity_t *self, float rightdot,
  ...
  if (self->client->ps.legsAnim == BOTH_WALL_RUN_LEFT ||
    ...
    vec3_t fwd, right, traceto,
        mins = {self->mins[0], self->mins[1], self->mins[2] + STEPSIZE},
            ...
            if (trace.fraction >= 1.0f) { // it's clear, let's do it
              ...
              if (self->client->ps.weapon == WP_SABER) {
                if ((self->client->ps.saber[0].saberFlags &
                  ...
                  allowWallFlips = qfalse;
                           ...
                           (self->client->ps.saber[1].saberFlags &
                            SFL_NO_WALL_FLIPS)) {
                  allowWallFlips = qfalse;
                }
              }
              if (allowWallFlips) { // okay to do wall-flips with this saber
                ...
                NPC_SetAnim(self, parts, anim,
                            SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
                self->client->ps.forceJumpZStart =
                    self->currentOrigin[2]; // so we don't take damage if we
                                            // land at same height
                self->client->ps.pm_flags |= (PMF_JUMPING | PMF_SLOW_MO_FALL);
                if (self->client->NPC_class == CLASS_BOBAFETT ||
                    (self->client->NPC_class == CLASS_REBORN &&
...
int Jedi_ReCalcParryTime(gentity_t *self, evasionType_t evasionType) {
  ...
  if (!self->s.number) { // player
    ...
    if ( !g_saberRealisticCombat->integer
            && ( g_spskill->integer == 2 || (g_spskill->integer == 1 &&
    (self->client->NPC_class == CLASS_TAVION||self->client->NPC_class ==
    CLASS_ALORA) ) ) )
    {
            if ( (self->client->NPC_class ==
    CLASS_TAVION||self->client->NPC_class == CLASS_ALORA) )
            {
                    return 0;
            }
            else
        ...
        */
        ...
        if (self->client->NPC_class == CLASS_ALORA ||
            ...
            self->client->NPC_class ==
                CLASS_TAVION) { // Tavion & Alora are faster
...
qboolean Jedi_InNoAIAnim(gentity_t *self) {
  ...
  if (NPCInfo->rank >=
      RANK_COMMANDER) { // boss-level guys can multitask, the rest need to chill
                        // out during special moves
    return qfalse;
  }
...
void Jedi_CheckJumpEvasionSafety(gentity_t *self, usercmd_t *cmd,
                                 evasionType_t evasionType) {
  if (evasionType !=
          EVASION_OTHER // not a FlipEvasion, which does it's own safety checks
      && NPC->client->ps.groundEntityNum !=
             ENTITYNUM_NONE) { // on terra firma right now
    if (NPC->client->ps.velocity[2] > 0 || NPC->client->ps.forceJumpCharge ||
        cmd->upmove > 0) { // going to jump
      if (!NAV_MoveDirSafe(
              NPC, cmd,
              NPC->client->ps.speed *
                  10.0f)) { // we can't jump in the dir we're pushing in
        // cancel the evasion
        ...
        if (d_JediAI->integer) {
          Com_Printf(S_COLOR_RED "jump not safe, cancelling!");
                 ...
                 NPC->client->ps.velocity[1]) { // sliding
        vec3_t jumpDir;
        float jumpDist = VectorNormalize2(NPC->client->ps.velocity, jumpDir);
        if (!NAV_DirSafe(
                NPC, jumpDir,
                jumpDist)) { // this jump combined with our momentum would send
                             // us into a do not enter brush, so cancel it
          // cancel the evasion
          NPC->client->ps.velocity[2] = NPC->client->ps.forceJumpCharge = 0;
          cmd->upmove = 0;
          if (d_JediAI->integer) {
...
evasionType_t Jedi_SaberBlockGo(gentity_t *self, usercmd_t *cmd, vec3_t pHitloc,
  ...
  if (PM_LockedAnim(self->client->ps.torsoAnim) &&
      self->client->ps.torsoAnimTimer) { // Never interrupt these...
    return EVASION_NONE;
  }
  ...
  if (!incoming) {
    ...
    {//jedi trainer and tavion are must faster at parrying and can do it
    whenever they like
            //Also, on medium, all level 3 people can parry any time and on
    hard, all level 2 or 3 people can parry any time
    }
    */
    else {
  ...
  if (self->client->NPC_class == CLASS_BOBAFETT) {
    ...
    if ((dist > 16 && (Q_irand(0, 2) || saberBusy)) ||
        self->client->ps.saberInFlight || !self->client->ps.SaberActive() ||
        (self->client->NPC_class == CLASS_REBORN &&
         self->s.weapon !=
             WP_SABER)) { // either it will miss by a bit (and 25% chance) OR
      ...
      if (self->NPC &&
        ...
        if (self->client->ps.groundEntityNum !=
            ...
            cmd->upmove >= 0 && TIMER_Done(self, "duck") // not ducking
  ...
  if ((self->client->NPC_class == CLASS_BOBAFETT // boba fett
       || (self->client->NPC_class == CLASS_REBORN &&
           self->s.weapon != WP_SABER) // non-saber reborn (cultist)
       ) &&
      !Q_irand(0, 2)) {
    doRoll = qtrue;
  }
  ...
  else if (zdiff > -22) // was-15 )
  {
    ...
    if (saberBusy ||
        (zdiff < -36 &&
         (zdiff < -44 || !Q_irand(0, 2)))) // was -30 and -40//2nd one was -46
    {                                      // jump!
      if (self->client->ps.groundEntityNum ==
              ...
              if (!PM_SaberInAttack(self->client->ps.saberMove) &&
                ...
                self->client->ps.forceJumpZStart =
                    self->currentOrigin[2]; // so we don't take damage if we
                                            // land at same height
                self->client->ps.pm_flags |= PMF_JUMPING | PMF_SLOW_MO_FALL;
                self->client->ps.SaberActivateTrail(
...
static evasionType_t Jedi_CheckEvadeSpecialAttacks(void) {
  ...
  if (NPC->enemy->client->ps.torsoAnim == BOTH_SPINATTACK6 ||
      NPC->enemy->client->ps.torsoAnim ==
          BOTH_SPINATTACK7) { // back away from these
    if ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) ||
      ...
      if (InFront(NPC->currentOrigin, NPC->enemy->currentOrigin,
                  NPC->enemy->currentAngles)) { // facing me
        float minSafeDistSq =
            (NPC->maxs[0] * 1.5f + NPC->enemy->maxs[0] * 1.5f +
             NPC->enemy->client->ps.SaberLength() + 24.0f);
        minSafeDistSq *= minSafeDistSq;
        if (DistanceSquared(NPC->currentOrigin, NPC->enemy->currentOrigin) <
            minSafeDistSq) { // back off!
          Jedi_StartBackOff();
          return EVASION_OTHER;
        }
      }
    ...
    if (NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_STAB ||
        (NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_F &&
         ((NPC->enemy->client->pers.lastCommand.buttons & BUTTON_ATTACK) ||
          (NPC->enemy->client->ps.pm_flags &
           PMF_ATTACK_HELD)))) { // either already in a roll-stab or may go into
                                 // one
      if ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER) ||
          NPC->client->NPC_class == CLASS_SHADOWTROOPER ||
          NPC->client->NPC_class == CLASS_ALORA ||
        ...
        if (InFront(NPC->currentOrigin, NPC->enemy->currentOrigin,
          ...
          if (distSq < minSafeDistSq) { // evade!
            qboolean doJump =
                (qboolean)(NPC->enemy->client->ps.torsoAnim == BOTH_ROLL_STAB ||
                           distSq < 3000.0f); // not much time left, just jump!
            if ((NPCInfo->scriptFlags & SCF_NO_ACROBATICS) || !doJump) { // roll?
              ...
              VectorSubtract(NPC->currentOrigin, NPC->enemy->currentOrigin,
                             dir2Me);
              ...
              if (dot > 0) {                              // I'm to his right
                if (!NPC_MoveDirClear(0, -127, qfalse)) { // fuck, jump instead
                  ...
                  if (NPC->client->ps.groundEntityNum !=
                      ENTITYNUM_NONE) { // fuck it, just force it
            ...
            if (NPC->s.weapon != WP_SABER ||
                ...
                NPC->client->NPC_class == CLASS_SHADOWTROOPER ||
                NPC->client->NPC_class == CLASS_ALORA ||
                Q_irand(-3, NPCInfo->rank) > RANK_CREWMAN) { // superjump
...
static qboolean Jedi_SaberBlock(void) {
  ...
  */
  ...
  for (saberNum = 0; saberNum < MAX_SABERS; saberNum++) {
    for (bladeNum = 0;
         bladeNum < NPC->enemy->client->ps.saber[saberNum].numBlades;
         bladeNum++) {
      if (NPC->enemy->client->ps.saber[saberNum].type != SABER_NONE &&
        ...
        VectorMA(
            NPC->enemy->client->ps.saber[saberNum]
                .blade[bladeNum]
                .muzzlePointOld,
            NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].length,
            NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].muzzleDirOld,
            saberTipOld);
        VectorMA(
            NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].muzzlePoint,
            NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].length,
            NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].muzzleDir,
            saberTip);
  ...
  VectorMA(NPC->enemy->client->ps.saber[closestSaberNum]
               .blade[closestBladeNum]
               .muzzlePointOld,
           NPC->enemy->client->ps.saber[closestSaberNum]
               .blade[closestBladeNum]
               .length,
           NPC->enemy->client->ps.saber[closestSaberNum]
               .blade[closestBladeNum]
  ...
  if (NPC->enemy->client->ps.saber[closestSaberNum]
          .blade[closestBladeNum]
          .length <= 0) {
    baseDirPerc = 0.5f;
  } else {
    baseDirPerc = pointDist / NPC->enemy->client->ps.saber[closestSaberNum]
                                  .blade[closestBladeNum]
                                  .length;
  }
  VectorSubtract(NPC->enemy->client->ps.saber[closestSaberNum]
                     .blade[closestBladeNum]
                     .muzzlePoint,
                 NPC->enemy->client->ps.saber[closestSaberNum]
                     .blade[closestBladeNum]
  ...
  gi.trace(&tr, saberPoint, saberMins, saberMaxs, hitloc, NPC->enemy->s.number,
           CONTENTS_BODY, (EG2_Collision)0, 0); //, G2_RETURNONHIT, 10 );
  ...
  if (d_JediAI->integer) {
    G_DebugLine(saberPoint, hitloc, FRAMETIME,
                WPDEBUG_SaberColor(NPC->enemy->client->ps.saber[closestSaberNum]
                                       .blade[closestBladeNum]
                                       .color),
                qtrue);
  }
  ...
  if ((evasionType = Jedi_SaberBlockGo(NPC, &ucmd, hitloc, dir, NULL, dist)) !=
      EVASION_NONE) {                       // did some sort of evasion
  ...
  if (evasionType != EVASION_DUCK_PARRY && evasionType != EVASION_JUMP_PARRY &&
      evasionType != EVASION_JUMP && evasionType != EVASION_DUCK &&
      evasionType != EVASION_FJUMP) {
...
static void Jedi_EvasionSaber(vec3_t enemy_movedir, float enemy_dist,
  ...
  if (NPC->enemy->client->ps.weaponTime &&
    ...
    if ((!NPC->client->ps.saberInFlight ||
         (NPC->client->ps.dualSabers && NPC->client->ps.saber[1].Active())) &&
        Jedi_SaberBlock()) {
      return;
    }
    ...
    if (Q_flrand(0.25, 1) < facingAmt) { // coming at/facing me!
      ...
      /*if ( NPC->client->NPC_class == CLASS_SABOTEUR )
      {
              ...
              else
              {
                      ...
                      {//50% chance of trying to dodge/roll/jump using jedi
      missile evasion logic Jedi_SaberBlock();
                      }
                      return;
              }
      }
      ...
      if (NPC->client->ps.weaponTime || NPC->client->ps.saberInFlight ||
          ...
          NPC->client->NPC_class ==
              CLASS_ROCKETTROOPER) { // I'm attacking or recovering from a
                                     // parry, can only try to strafe/jump right
                                     // now
      ...
      case 3:
        ...
        if (Jedi_DecideKick() // let's try a kick
          ...
          TIMER_Set(NPC, "kickDebounce", Q_irand(3000, 10000));
            ...
            (NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) &&
            TIMER_Done(
                NPC,
      ...
      default:
        ...
        if (!Q_irand(0, 5) ||
          ...
          if (Jedi_DecideKick() && G_CanKickEntity(NPC, NPC->enemy) &&
              G_PickAutoKick(NPC, NPC->enemy, qtrue) != LS_NONE) { // kicked!
            TIMER_Set(NPC, "kickDebounce", Q_irand(3000, 10000));
            ...
            if (shooting_lightning ||
              ...
              if ((NPCInfo->rank == RANK_ENSIGN ||
                ...
                ForceThrow(NPC, qfalse);
                         ...
                         !(NPC->client->ps.forcePowersActive &
                           (1 << FP_RAGE)) &&
                         !PM_InKnockDown(
                             &NPC->client
...
gentity_t *Jedi_FindEnemyInCone(gentity_t *self, gentity_t *fallback,
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    gi.trace(&tr, self->currentOrigin, vec3_origin, vec3_origin,
             check->currentOrigin, self->s.number, MASK_SHOT, (EG2_Collision)0,
             0);
    if (tr.fraction < 1.0f &&
...
static void Jedi_SetEnemyInfo(vec3_t enemy_dest, vec3_t enemy_dir,
  ...
  if (*enemy_dist <= 0.0f) {
  ...
  } else { // if he's too far away, see if he's at least facing us or coming
           // towards us
    if (*enemy_dist <= 32.0f) { // has to be facing us
      vec3_t eAngles = {0, NPC->currentAngles[YAW], 0};
      if (InFOV(NPC->currentOrigin, NPC->enemy->currentOrigin, eAngles, 30,
                90)) { // in striking range
        enemy_in_striking_range = qtrue;
      }
    }
    if (*enemy_dist >= 64.0f) { // we have to be approaching each other
      ...
      if (!VectorCompare(
              ...
              vec3_origin)) { // I am moving, see if I'm moving toward the enemy
        vec3_t eDir;
        ...
        vDot = DotProduct(eDir, NPC->client->ps.velocity);
                 ...
                 !VectorCompare(
                     ...
                     vec3_origin)) { // I'm not moving, but the enemy is, see if
...
void NPC_EvasionSaber(void) {
  if (ucmd.upmove <= 0                      // not jumping
      && (!ucmd.upmove || !ucmd.rightmove)) // either just ducking or just
                                            // strafing (i.e.: not rolling
...
static void Jedi_FaceEnemy(qboolean doPitch) {
  ...
  if (NPC->client->NPC_class == CLASS_BOBAFETT &&
      TIMER_Done(NPC, "flameTime") && NPC->s.weapon != WP_NONE &&
      NPC->s.weapon != WP_DISRUPTOR &&
      (NPC->s.weapon != WP_ROCKET_LAUNCHER ||
       !(NPCInfo->scriptFlags & SCF_ALT_FIRE)) &&
      NPC->s.weapon != WP_THERMAL && NPC->s.weapon != WP_TRIP_MINE &&
    ...
    if (NPC->health < NPC->max_health * 0.5f) { // lead
      float missileSpeed = WP_SpeedOfMissileForWeapon(
          NPC->s.weapon, ((qboolean)(NPCInfo->scriptFlags & SCF_ALT_FIRE)));
      if (missileSpeed) {
        float eDist = Distance(eyes, enemy_eyes);
        eDist /=
            missileSpeed; // How many seconds it will take to get to the enemy
        VectorMA(enemy_eyes, eDist * Q_flrand(0.95f, 1.25f),
                 NPC->enemy->client->ps.velocity, enemy_eyes);
      }
    }
  }
  ...
  if (!NPC->client->ps.saberInFlight &&
      (NPC->client->ps.legsAnim == BOTH_A2_STABBACK1 ||
       ...
       NPC->client->ps.legsAnim == BOTH_A7_KICK_B)) { // point *away*
    GetAnglesForDirection(enemy_eyes, eyes, angles);
  } else if (NPC->client->ps.legsAnim == BOTH_A7_KICK_R) { // keep enemy to
                                                           // right
  ...
  if (ucmd.forwardmove > 0) {
    if (!TIMER_Done(NPC, "moveback") || !TIMER_Done(NPC, "movenone")) {
      ...
      if (TIMER_Done(NPC, "lastmoveforward")) {
        ...
        // so we don't keep doing this over and over again - new nav stuff makes
        // them coast to a stop, so they could be just slowing down from the last
        // "moveback" timer's ending...
        TIMER_Set(NPC, "lastmoveforward", holdDirTime + Q_irand(1000, 2000));
      }
    } else { // NOTE: edge checking should stop me if this is bad... but what if
             // it sends us colliding into the enemy?
...
static void Jedi_TimersApply(void) {
  ...
  if (!TIMER_Done(NPC, "draining")) { // FIXME: what do we do if we ran out of
                                      // power?  NPC's can't?
    // FIXME: don't keep turning to face enemy or we'll end up spinning around
...
static void Jedi_CombatIdle(int enemy_dist) {
  ...
  if (NPC->client->ps.forcePowersActive & (1 << FP_RAGE) ||
      NPC->client->ps.forceRageRecoveryTime >
          level.time) { // never taunt while raging or recovering from rage
    return;
  }
  if (NPC->client->ps.stats[STAT_WEAPONS] &
      (1 << WP_SCEPTER)) { // never taunt when holding scepter
    return;
  }
  if (NPC->client->ps.saber[0].type ==
      SABER_SITH_SWORD) { // never taunt when holding sith sword
    return;
  }
  ...
  if (enemy_dist >= 64) { // FIXME: only do this if standing still?
    ...
    if (Q_irand(2, chance) < NPCInfo->stats.aggression) {
      if (TIMER_Done(NPC, "chatter")) { // FIXME: add more taunt behaviors
        ...
        // again???
        if (enemy_dist > 200 && NPC->client->NPC_class != CLASS_BOBAFETT &&
            (NPC->client->NPC_class != CLASS_REBORN ||
             NPC->s.weapon == WP_SABER) &&
            NPC->client->NPC_class != CLASS_ROCKETTROOPER &&
          ...
          if (NPC->client->ps.saberAnimLevel != SS_STAFF &&
              NPC->client->ps.saberAnimLevel !=
                  SS_DUAL) { // those taunts leave saber on
            WP_DeactivateSaber(NPC);
          }
...
static qboolean Jedi_AttackDecide(int enemy_dist) {
  ...
  if (Jedi_CultistDestroyer(NPC)) { // destroyer
    if (enemy_dist <= 32) {         // go boom!
      ...
      NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_FORCE_RAGE,
                  SETANIM_FLAG_HOLD | SETANIM_FLAG_OVERRIDE);
      NPC->client->ps.forcePowersActive |= (1 << FP_RAGE);
      NPC->painDebounceTime = NPC->useDebounceTime =
          level.time + NPC->client->ps.torsoAnimTimer;
      return qtrue;
    }
    return qfalse;
  }
  ...
  if (NPC->client->ps.saberEventFlags &
    ...
    if ((NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER)) { // desann and luke
      chance = 20;
               ...
               NPC->client->NPC_class == CLASS_ALORA) { // tavion
      chance = 10;
  ...
  if (NPC->client->NPC_class == CLASS_TAVION ||
    ...
    if ((PM_SaberInParry(NPC->client->ps.saberMove) ||
      ...
      NPC->client->ps.saberBlocked = BLOCKED_NONE;
      Jedi_AdjustSaberAnimLevel(
          NPC, SS_FAST); // try to follow-up with a quick attack
      WeaponThink(qtrue);
      return qtrue;
    }
  }
  ...
  if (!(ucmd.buttons & BUTTON_ATTACK) && !(ucmd.buttons & BUTTON_ALT_ATTACK) &&
      !(ucmd.buttons & BUTTON_FORCE_FOCUS)) { // not already attacking
    // Try to attack
    WeaponThink(qtrue);
  }
  ...
  if (!(NPCInfo->aiFlags & NPCAI_BOSS_CHARACTER)) {
    if (PM_KickingAnim(NPC->enemy->client->ps.legsAnim) &&
        ...
        && (NPC->enemy->client->ps.legsAnim == BOTH_A7_KICK_RL ||
            ...
            (NPC->enemy->enemy &&
             NPC->enemy->enemy == NPC))) { // run into the kick!
      ucmd.forwardmove = ucmd.rightmove = ucmd.upmove = 0;
      VectorClear(NPC->client->ps.moveDir);
      Jedi_Advance();
    } else if (NPC->enemy->client->ps.torsoAnim == BOTH_A7_HILT &&
               NPC->client->ps.groundEntityNum !=
                   ENTITYNUM_NONE) { // run into the hilt bash
      ...
      Jedi_Advance();
               ...
               NPC->client->ps.groundEntityNum !=
                   ENTITYNUM_NONE) { // run into the attack
      // FIXME : only if on R/L or F/B?
      ucmd.forwardmove = ucmd.rightmove = ucmd.upmove = 0;
      VectorClear(NPC->client->ps.moveDir);
      Jedi_Advance();
    } else if (NPC->enemy->enemy &&
               NPC->enemy->enemy == NPC) { // enemy is mad at *me*
      if (NPC->enemy->client->ps.legsAnim == BOTH_JUMPFLIPSLASHDOWN1 ||
          ...
          NPC->enemy->client->ps.legsAnim ==
              BOTH_FLIP_ATTACK7) { // enemy is flipping over me
...
void RT_CheckJump(void) {
  ...
  if (!NPCInfo->goalEntity) {
    if (NPC->enemy) {
      // FIXME: debounce this?
      if (TIMER_Done(NPC, "roamTime") &&
          Q_irand(0, 9)) { // okay to try to find another spot to be
        int cpFlags =
            (CP_CLEAR | CP_HAS_ROUTE); // must have a clear shot at enemy
        float enemyDistSq = DistanceHorizontalSquared(
            NPC->currentOrigin, NPC->enemy->currentOrigin);
        ...
        int cp = NPC_FindCombatPointRetry(
            NPC->currentOrigin, NPC->currentOrigin, NPC->currentOrigin,
            &sendFlags, 256, NPCInfo->lastFailedCombatPoint);
        if (cp ==
            -1) { // try again, no route needed since we can rocket-jump to it!
          cpFlags &= ~CP_HAS_ROUTE;
          cp = NPC_FindCombatPointRetry(NPC->currentOrigin, NPC->currentOrigin,
                                        NPC->currentOrigin, &cpFlags, 256,
  ...
  if (fabs(vec2Goal[2]) < 32) { // not a big height diff, see how far it is
    vec2Goal[2] = 0;
    if (VectorLengthSquared(vec2Goal) <
        (256 * 256)) { // too close!  Don't rocket-jump to it...
      return;
    }
  }
  ...
  if (!Jedi_ClearPathToSpot(jumpPos, jumpEntNum)) { // hunt him down
    if ((NPC_ClearLOS(NPC->enemy) ||
        ...
        InFOV(jumpPos, NPC->currentOrigin, NPC->client->ps.viewangles, 20,
              60)) {
    ...
    { // can macro-navigate to him
      return;
             ...
             // there
      if (STEER::HasBeenBlockedFor(
              NPC, 2000)) { // try to jump to the blockedTargetPosition
...
static void Jedi_Combat(void) {
  ...
  if (NPC_Jumping()) { // I'm in the middle of a jump, so just see if I should
                       // attack
      ...
      if (!Jedi_ClearPathToSpot(enemy_dest,
                                  ...
                                  //NPCInfo->rank > RANK_LT_JG ) &&
        {
          ...
          && !trace.startsolid && trace.fraction >= 1.0 )
          {
                  ...
                  gi.trace( &trace, NPC->currentOrigin, NPC->mins, NPC->maxs,
          end, NPC->s.number, NPC->clipmask|CONTENTS_BOTCLIP ); if (
          !trace.allsolid && !trace.startsolid )
                  {
                          if ( trace.fraction >= 1.0 || trace.plane.normal[2] >
          0 )
                          {
                                  ucmd.upmove = 127;
                                  ucmd.forwardmove = 127;
          ...
          */
          ...
          /*	else if ( NPC_TryJump( NPC->enemy ) )	// Jedi, can see enemy,
  ...
  if (TIMER_Done(NPC, "allyJediDelay")) {
    if ((!NPC->client->ps.saberInFlight ||
      ...
      if (!Jedi_AttackDecide(
        ...
        // FIXME: lower aggression when actually strike offensively?  Or just
        // when do damage?
  ...
  if (VectorCompare(NPC->client->ps.moveDir,
                    vec3_origin) // stomped the NAV system's moveDir
      && !NPC_MoveDirClear(ucmd.forwardmove, ucmd.rightmove,
                           qtrue)) // check ucmd-driven movement
...
void NPC_Jedi_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
  ...
  if (other && other->client && !OnSameTeam(self, other)) { // hit by a client
    ...
    if (mod == MOD_FORCE_GRIP || mod == MOD_FORCE_LIGHTNING ||
        mod == MOD_FORCE_DRAIN) { // see if we should turn on absorb
      if ((self->client->ps.forcePowersKnown & (1 << FP_ABSORB)) != 0 &&
          (self->client->ps.forcePowersActive & (1 << FP_ABSORB)) ==
              0) { // know absorb and not already using it
      ...
      if ((self->client->ps.forcePowersKnown & (1 << FP_PROTECT)) != 0 &&
          (self->client->ps.forcePowersActive & (1 << FP_PROTECT)) ==
              0) { // know protect and not already using it
        if (other->s.number >= MAX_CLIENTS         // enemy is an NPC
            || Q_irand(0, g_spskill->integer + 1)) // enemy is player
        {
          if (Q_irand(0, self->NPC->rank) > RANK_ENSIGN) {
      ...
      if (enemy && enemy->client && NPC_ValidEnemy(enemy)) {
        if (gi.inPVS(NPC->currentOrigin,
          ...
          if (enemy->s.number == 0 || enemy_dist < best_enemy_dist) {
            ...
            // FIXME: use jediSpeechDebounceTime[NPC->client->playerTeam] <
            // level.time ) check for anger sound
    ...
    if (!NPC->enemy) { // still not mad
      if (!best_enemy) {
        ...
        if (NPC_ClearLOS(
          ...
          if ((NPCInfo->aiFlags &
               NPCAI_NO_JEDI_DELAY)) { // just get mad right away
            if (DistanceHorizontalSquared(NPC->currentOrigin,
                                          best_enemy->currentOrigin) <
                (1024 * 1024)) {
...
void NPC_BSJedi_FollowLeader(void) {
  ...
  if (!NPC->enemy && NPC->health < NPC->max_health &&
    ...
    if ((ucmd.buttons & BUTTON_ATTACK)) { // attacking
      if ((g_saberNewControlScheme->integer &&
          ...
          (!g_saberNewControlScheme->integer &&
           !(ucmd.buttons & BUTTON_ALT_ATTACK))) { // not already going to do a
                                                   // kata move somehow
        if (NPC->client->ps.groundEntityNum != ENTITYNUM_NONE) { // on the
                                                                 // ground
          if (ucmd.upmove <= 0 && NPC->client->ps.forceJumpCharge <=
                                      0) { // not going to try to jump
            /*
            if ( (NPCInfo->scriptFlags&SCF_NO_ACROBATICS) )
            ...
            */
            ...
            if (Q_irand(0,
                        g_spskill->integer +
                            1) // 50% chance on easy, 66% on medium, 75% on hard
...
static void Jedi_Attack(void) {
  ...
  if (NPC->painDebounceTime > level.time) {
    ...
    NPC_UpdateAngles(qtrue, qtrue);
    if (NPC->client->ps.torsoAnim == BOTH_KYLE_GRAB) { // see if we grabbed
                                                       // enemy
      if (NPC->client->ps.torsoAnimTimer <= 200) {
        if (Kyle_CanDoGrab() &&
            NPC_EnemyRangeFromBolt(NPC->handRBolt) < 88.0f) { // grab him!
          Kyle_GrabEnemy();
          return;
        } else {
          NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_KYLE_MISS,
  ...
  if (NPC->client->ps.saberLockTime > level.time) {
    ...
    // maybe if I'm losing I should try to force-push out of it?  Very rarely,
    // though...
    ...
    if (NPC->enemy->health <= 0 && NPC->enemy->enemy == NPC &&
        (NPC->client->playerTeam != TEAM_PLAYER ||
         (NPC->client->NPC_class == CLASS_KYLE && (NPC->spawnflags & 1) &&
          NPC->enemy == player))) // good guys don't gloat (unless it's Kyle
                                  // having just killed his student
    {                                      // my enemy is dead and I killed him
      ...
      if (NPC->client->NPC_class == CLASS_BOBAFETT ||
          (NPC->client->NPC_class == CLASS_REBORN &&
           NPC->s.weapon != WP_SABER) ||
          NPC->client->NPC_class == CLASS_ROCKETTROOPER) {
        if (NPCInfo->walkDebounceTime < level.time &&
        ...
        if (!TIMER_Done(NPC, "parryTime")) {
          TIMER_Set(NPC, "parryTime", -1);
          NPC->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] =
              level.time + 500;
        }
        NPC->client->ps.saberBlocked = BLOCKED_NONE;
    ...
    if (NPC->enemy->count <= 0) { // it's out of ammo
      if (NPC->enemy->activator && NPC_ValidEnemy(NPC->enemy->activator)) {
        ...
        G_ClearEnemy(NPC);
             ...
             NPC->enemy->NPC->charmedTime >
                 level.time) {         // my enemy was charmed
    if (OnSameTeam(NPC, NPC->enemy)) { // has been charmed to be on my team
      G_ClearEnemy(NPC);
    }
  }
  if (NPC->client->playerTeam == TEAM_ENEMY &&
      NPC->client->enemyTeam == TEAM_PLAYER && NPC->enemy &&
      NPC->enemy->client &&
      NPC->enemy->client->playerTeam != NPC->client->enemyTeam &&
      OnSameTeam(NPC, NPC->enemy) &&
      !(NPC->svFlags &
        SVF_LOCKEDENEMY)) { // an evil jedi somehow got another evil NPC as an
                            // enemy, they were probably charmed and it's run out
                            // now
  ...
  if (!(NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) ||
      ((NPC->client->ps.forcePowersActive & (1 << FP_HEAL)) &&
       NPC->client->ps.forcePowerLevel[FP_HEAL] <
  ...
  if (Jedi_CheckKataAttack()) { // doing a kata attack
    ...
    if (NPC->client->NPC_class == CLASS_ALORA) {
      if ((ucmd.buttons &
           BUTTON_ALT_ATTACK)) { // chance of doing a special dual saber throw
        ...
        if (Distance(NPC->enemy->currentOrigin, NPC->currentOrigin) > 80) {
          if (NPC->client->ps.legsAnim == BOTH_FLIP_F ||
            ...
            if (NPC->client->ps.legsAnimTimer <= 200 &&
                Q_irand(0, 2)) { // go ahead and start anotther
  ...
  if (VectorCompare(NPC->client->ps.moveDir, vec3_origin) &&
      (ucmd.forwardmove ||
       ucmd.rightmove)) { // using ucmds to move this turn, not NAV
    if ((ucmd.buttons &
         BUTTON_WALKING)) { // FIXME: NAV system screws with speed directly, so
                            // now I have to re-set it myself!
...
qboolean Rosh_BeingHealed(gentity_t *self) {
  if (self && self->NPC && self->client && (self->NPC->aiFlags & NPCAI_ROSH) &&
...
qboolean Rosh_TwinNearBy(gentity_t *self) {
  ...
  if (self->client && foundTwin->client) {
    if (Distance(self->currentOrigin, foundTwin->currentOrigin) <= 512.0f &&
        G_ClearLineOfSight(self->client->renderInfo.eyePoint,
                           foundTwin->client->renderInfo.eyePoint,
...
qboolean Kothos_HealRosh(void) {
  if (NPC->client && NPC->client->leader && NPC->client->leader->client) {
    if (DistanceSquared(NPC->client->leader->currentOrigin,
                        NPC->currentOrigin) <= (256 * 256) &&
        G_ClearLineOfSight(NPC->client->leader->client->renderInfo.eyePoint,
                           NPC->client->renderInfo.eyePoint, NPC->s.number,
                           MASK_OPAQUE)) {
      ...
      if (NPC->ghoul2.size()) {
        ...
        gi.G2API_GetBoltMatrix(
            ...
            (Q_irand(0, 1) ? NPC->handLBolt : NPC->handRBolt), &boltMatrix,
            angles, NPC->currentOrigin, (cg.time ? cg.time : level.time), NULL,
            NPC->s.modelScale);
      ...
      NPC->client->leader->health +=
          Q_irand(1 + g_spskill->integer * 2,
                  4 + g_spskill->integer * 3); // from 1-5 to 4-10
      if (NPC->client->leader->client) {
        if (NPC->client->leader->client->ps.legsAnim == BOTH_FORCEHEAL_START &&
            NPC->client->leader->health >=
                NPC->client->leader->max_health) { // let him get up now
          NPC_SetAnim(NPC->client->leader, SETANIM_BOTH, BOTH_FORCEHEAL_STOP,
                      SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
          // FIXME: temp effect
          G_PlayEffect(G_EffectIndex("force/kothos_recharge.efx"),
                       NPC->client->leader->playerModel, 0,
          ...
          NPC->client->leader->client->ps.powerups[PW_INVINCIBLE] =
              level.time + NPC->client->leader->client->ps.torsoAnimTimer;
      ...
      if (g_spskill->integer) { // not on easy
        G_PlayEffect(G_EffectIndex("force/kothos_recharge.efx"),
                     NPC->playerModel, 0, NPC->s.number, NPC->currentOrigin,
                     500, qfalse);
...
void Kothos_PowerRosh(void) {
  if (NPC->client && NPC->client->leader) {
    if (Distance(NPC->client->leader->currentOrigin, NPC->currentOrigin) <=
            512.0f &&
        G_ClearLineOfSight(NPC->client->leader->client->renderInfo.eyePoint,
                           NPC->client->renderInfo.eyePoint, NPC->s.number,
                           MASK_OPAQUE)) {
      NPC_FaceEntity(NPC->client->leader, qtrue);
      NPC_SetAnim(NPC, SETANIM_TORSO, BOTH_FORCELIGHTNING_HOLD,
                  SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
      NPC->client->ps.torsoAnimTimer = 500;
      // FIXME: unique effect and sound
      // NPC->client->ps.eFlags |= EF_POWERING_ROSH;
      G_PlayEffect(G_EffectIndex("force/kothos_beam.efx"), NPC->playerModel,
                   NPC->handLBolt, NPC->s.number, NPC->currentOrigin, 500,
                   qfalse);
...
qboolean Jedi_InSpecialMove(void) {
  ...
  if (Jedi_InNoAIAnim(NPC)) { // in special anims, don't do force powers or
                              // attacks, just face the enemy
    if (NPC->enemy) {
      NPC_FaceEnemy(qtrue);
    } else {
      NPC_UpdateAngles(qtrue, qtrue);
  ...
  */
  ...
  if (NPC->client->ps.torsoAnim == BOTH_SCEPTER_START) {
    if (NPC->client->ps.torsoAnimTimer <= 100) { // go into the hold
      ...
      G_PlayEffect(G_EffectIndex("scepter/beam.efx"), NPC->weaponModel[1],
                   NPC->genericBolt1, NPC->s.number, NPC->currentOrigin, 10000,
                   qtrue);
      NPC->client->ps.legsAnimTimer = NPC->client->ps.torsoAnimTimer = 0;
    ...
    if (NPC->client->ps.torsoAnimTimer <= 100) {
      NPC->s.loopSound = 0;
      G_StopEffect(G_EffectIndex("scepter/beam.efx"), NPC->weaponModel[1],
                   NPC->genericBolt1, NPC->s.number);
      NPC->client->ps.legsAnimTimer = NPC->client->ps.torsoAnimTimer = 0;
    ...
    if (!NPC->takedamage) { // ready to explode
      if (NPC->useDebounceTime <= level.time) {
        ...
        NPC->client->playerTeam =
            TEAM_FREE; // FIXME: will this destroy wampas, tusken & rancors?
        WP_Explode(NPC);
        return qtrue;
      }
      if (NPC->enemy) {
  ...
  if (NPC->client->NPC_class == CLASS_REBORN) {
    if ((NPCInfo->aiFlags & NPCAI_HEAL_ROSH)) {
      ...
      if (NPC->client->leader) {
        ...
        if (NPC->client->leader->client) {
          NPC->client->leader->client->ps.forcePowersKnown |=
        ...
        */
        ...
        if (helpingRosh) {
          ...
          return qtrue;
                   ...
                   DistanceSquared(NPC->enemy->currentOrigin,
                                   NPC->currentOrigin) < Twins_DangerDist()) {
          if (NPC->enemy && Kothos_Retreat()) {
            ...
            if (TIMER_Done(NPC, "attackDelay")) {
              if (NPC->painDebounceTime > level.time ||
                ...
                NPC->flags &= ~FL_LOCK_PLAYER_WEAPONS;
                switch (
                    Q_irand(0, 7 + g_spskill->integer)) // on easy: no lightning
                {
                case 0:
                case 1:
                case 2:
            ...
            NPC->flags &= ~FL_LOCK_PLAYER_WEAPONS;
                   ...
                   DistanceSquared(NPC->currentOrigin,
                                   NPC->client->leader->currentOrigin) >
                       (512 * 512)) { // can't see Rosh or too far away, catch
                                      // up with him
          if (!TIMER_Done(NPC, "attackDelay")) {
            NPC->flags &= ~FL_LOCK_PLAYER_WEAPONS;
          }
      ...
      NPC_UpdateAngles(qtrue, qtrue);
      // NPC->client->ps.eFlags &= ~EF_POWERING_ROSH;
      // G_StopEffect( G_EffectIndex( "force/kothos_beam.efx" ),
      // NPC->playerModel, NPC->handLBolt, NPC->s.number );
    } else if ((NPCInfo->aiFlags & NPCAI_ROSH)) {
      if ((NPC->flags &
           FL_UNDYING)) {    // Vil and/or Dasariah still around to heal me
        if (NPC->health == 1 // need healing
            || NPC->client->ps.powerups[PW_INVINCIBLE] >
                   level.time) // being healed
        {                      // FIXME: custom anims
          if (Rosh_TwinPresent(NPC)) {
            if (!NPC->client->ps.weaponTime) { // not attacking
              if (NPC->client->ps.legsAnim != BOTH_FORCEHEAL_START &&
                  NPC->client->ps.legsAnim !=
                      BOTH_FORCEHEAL_STOP) { // get down and wait for Vil or
                                             // Dasariah to help us
                // FIXME: sound?
                NPC->client->ps.legsAnimTimer = NPC->client->ps.torsoAnimTimer =
                    0;
                NPC_SetAnim(NPC, SETANIM_BOTH, BOTH_FORCEHEAL_START,
                            SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
                NPC->client->ps.torsoAnimTimer = NPC->client->ps.legsAnimTimer =
                    -1;
                NPC->client->ps.SaberDeactivate();
