...
qboolean NAV_CheckAhead(gentity_t *self, vec3_t end, trace_t *trace,
                        int clipmask);
qboolean NAV_TestForBlocked(gentity_t *self, gentity_t *goal,
                            gentity_t *blocker, float distance, int *flags);

void G_Line(vec3_t start, vec3_t end, vec3_t color, float alpha);
void G_Cube(vec3_t mins, vec3_t maxs, vec3_t color, float alpha);
void G_CubeOutline(vec3_t mins, vec3_t maxs, int time, unsigned int color,
                   float alpha);
...
qboolean NAV_CheckNodeFailedForEnt(gentity_t *ent, int nodeNum) {
  ...
  for (j = 0; j < MAX_FAILED_NODES; j++) {
    if (ent->failedWaypoints[j] ==
        nodeNum + 1) //+1 because 0 is a valid nodeNum, but also the default
...
-------------------------
*/
int NAVNEW_ClearPathBetweenPoints(vec3_t start, vec3_t end, vec3_t mins,
  ...
  // FIXME: if startsolid or allsolid, then the path isn't clear... but
  // returning ENTITYNUM_NONE indicates to CheckFailedEdge that is is clear...?
...
-------------------------
*/
void NAVNEW_PushBlocker(
    gentity_t *self, gentity_t *blocker, vec3_t right,
    qboolean setBlockedInfo) { // try pushing blocker to one side
  trace_t tr;
  vec3_t mins, end;
  ...
  if (blocker->s.number >= 0 &&
      blocker->s.number < MAX_CLIENTS) { // never push the player
    return;
  }

  if (!blocker->client ||
      !VectorCompare(blocker->client->pushVec,
                     vec3_origin)) { // someone else is pushing him, wait until
                                     // they give up?
    return;
  }
  ...
  trap->Trace(&tr, blocker->r.currentOrigin, mins, blocker->r.maxs, end,
              blocker->s.number, blocker->clipmask | CONTENTS_BOTCLIP, qfalse,
              0, 0);
...
qboolean NAVNEW_DanceWithBlocker(
    gentity_t *self, gentity_t *blocker, vec3_t movedir,
    vec3_t right) { // sees if blocker has any lateral movement
  if (blocker->client &&
      !VectorCompare(blocker->client->ps.velocity, vec3_origin)) {
    ...
    if (dot >
        50.0f) { // he's moving to the right of me at a relatively good speed
      // go to my left
      VectorMA(movedir, -1, right, movedir);
      VectorNormalize(movedir);
      return qtrue;
    } else if (dot > -50.0f) { // he's moving to the left of me at a relatively
                               // good speed
      // go to my right
    ...
    if ( NAVNEW_CheckAhead( self, block_pos, tr, ( self->clipmask &
...
qboolean NAVNEW_SidestepBlocker(
    gentity_t *self, gentity_t *blocker, vec3_t blocked_dir, float blocked_dist,
    vec3_t movedir,
    vec3_t right) { // trace to sides of blocker and see if either is clear
  trace_t tr;
  vec3_t avoidAngles;
  ...
  avoidRadius = sqrt((blocker->r.maxs[0] * blocker->r.maxs[0]) +
                     (blocker->r.maxs[1] * blocker->r.maxs[1])) +
                sqrt((self->r.maxs[0] * self->r.maxs[0]) +
                     (self->r.maxs[1] * self->r.maxs[1]));

  // See if we're inside our avoidance radius
  arcAngle =
      (blocked_dist <= avoidRadius) ? 135 : ((avoidRadius / blocked_dist) * 90);
  ...
  */
  ...
  // need to stop it from ping-ponging, so we have a bit of a debounce time on
  // which side you try
  if (self->NPC->sideStepHoldTime > level.time) {
    ...
    trap->Trace(&tr, self->r.currentOrigin, mins, self->r.maxs, block_pos,
                self->s.number, self->clipmask | CONTENTS_BOTCLIP, qfalse, 0,
                0);
  ...
  trap->Trace(&tr, self->r.currentOrigin, mins, self->r.maxs, block_pos,
              self->s.number, self->clipmask | CONTENTS_BOTCLIP, qfalse, 0, 0);

  if (!tr.allsolid && !tr.startsolid) {
    if (tr.fraction >=
        1.0f) { // all clear, go for it (favor the right if both are equal)
  ...
  if (!tr.allsolid && !tr.startsolid) {
    if (tr.fraction >= 1.0f) { // all clear, go for it (right side would have
                               // already succeeded if as good as this)
  ...
  if (rightSucc * blocked_dist >= avoidRadius ||
      leftSucc * blocked_dist >= avoidRadius) { // the traces hit something, but
                                                // got a relatively good distance
...
-------------------------
*/
qboolean NAVNEW_Bypass(gentity_t *self, gentity_t *blocker, vec3_t blocked_dir,
                       float blocked_dist, vec3_t movedir,
  ...
  // Check to see what dir the other guy is moving in (if any) and pick the
  // opposite dir
  ...
  if (NAVNEW_SidestepBlocker(self, blocker, blocked_dir, blocked_dist, movedir,
                             right)) {
    return qtrue;
  }
...
-------------------------
*/
qboolean NAVNEW_CheckDoubleBlock(gentity_t *self, gentity_t *blocker,
                                 vec3_t blocked_dir) {
  // Stop double waiting
...
qboolean NAVNEW_ResolveEntityCollision(gentity_t *self, gentity_t *blocker,
                                       vec3_t movedir, vec3_t pathDir,
                                       qboolean setBlockedInfo) {
  vec3_t blocked_dir;
  float blocked_dist;
  ...
  if (Q_stricmp(blocker->classname, "func_door") == 0) {
    ...
    if (DistanceSquared(self->r.currentOrigin, center) >
  ...
  //	if ( NAVNEW_PredictCollision( self, blocker, movedir, blocked_dir ) ==
  //qfalse ) 		return qtrue;

  // First, attempt to walk around the blocker or shove him out of the way
  if (NAVNEW_Bypass(self, blocker, blocked_dir, blocked_dist, movedir,
                    setBlockedInfo))
    return qtrue;

  // Can't get around him... see if I'm blocking him too... if so, I need to
  // just keep moving?
...
-------------------------
*/
qboolean NAVNEW_AvoidCollision(gentity_t *self, gentity_t *goal,
  ...
  if (NAV_CheckAhead(self, movepos, &info->trace, CONTENTS_BODY) == qfalse) {
    ...
    // if ( NAVNEW_TrueCollision( self, info->blocker, movedir, info->direction
    // ) == qfalse ) 	return qtrue;

    // Test for blocking by standing on goal
    if (NAV_TestForBlocked(self, goal, info->blocker, info->distance,
                           &info->flags) == qtrue)
      return qfalse;
    ...
    */
    ...
    if (NAVNEW_ResolveEntityCollision(self, info->blocker, movedir,
                                      info->pathDirection,
...
qboolean NAVNEW_TestNodeConnectionBlocked(
    int wp1, int wp2, gentity_t *ignoreEnt, int goalEntNum, qboolean checkWorld,
    qboolean checkEnts) { // see if the direct path between 2 nodes is blocked
                          // by architecture or an ent
  ...
  trap->Trace(&trace, pos1, mins, maxs, pos2, ignoreEntNum, clipmask, qfalse, 0,
              0);
  if (trace.fraction >= 1.0f ||
      trace.entityNum == goalEntNum) { // clear or hit goal
    return qfalse;
  }
...
int NAVNEW_MoveToGoal(gentity_t *self, navInfo_t *info) {
  ...
  if (self->waypoint == WAYPOINT_NONE &&
      self->noWaypointTime >
          level.time) { // didn't have a valid one in about the past second,
                        // don't look again just yet
    return WAYPOINT_NONE;
  }
  if (self->NPC->goalEntity->waypoint == WAYPOINT_NONE &&
      self->NPC->goalEntity->noWaypointTime >
          level.time) { // didn't have a valid one in about the past second,
                        // don't look again just yet
    return WAYPOINT_NONE;
  }
  if (self->noWaypointTime > level.time &&
      self->NPC->goalEntity->noWaypointTime >
          level.time) { // just use current waypoints
    bestNode = trap->Nav_GetBestNodeAltRoute2(
        self->waypoint, self->NPC->goalEntity->waypoint, bestNode);
  }
  // FIXME!!!!: this is making them wiggle back and forth between waypoints
  else if ((bestNode = trap->Nav_GetBestPathBetweenEnts(
                (sharedEntity_t *)self, (sharedEntity_t *)self->NPC->goalEntity,
                NF_CLEAR_PATH)) ==
           NODE_NONE) //! NAVNEW_GetWaypoints( self, qtrue ) )
  {                   // one of us didn't have a valid waypoint!
    if (self->waypoint ==
        NODE_NONE) { // don't even try to find one again for a bit
      self->noWaypointTime = level.time + Q_irand(500, 1500);
    }
    if (self->NPC->goalEntity->waypoint ==
        NODE_NONE) { // don't even try to find one again for a bit
  ...
  while (!foundClearPath) {
    ...
    /*
    bestNode = trap->Nav_GetBestNodeAltRoute( self->waypoint,
    self->NPC->goalEntity->waypoint, bestNode );
    */
    ...
    // see if we can get directly to the next node off bestNode en route to
    // goal's node... NOTE: shouldn't be necc. now
    /*
    int oldBestNode = bestNode;
    bestNode = NAV_TestBestNode( self, self->waypoint, bestNode, qtrue );//,
    self->NPC->goalEntity->waypoint );//
    ...
    */
    ...
    {//we haven't already tried to go straight to goal or goal's wp
            ...
            {//our bestNode is the goal's wp
                    if ( NAV_HitNavGoal( self->r.currentOrigin, self->r.mins,
    self->r.maxs, origin, trap->Nav_GetNodeRadius( bestNode ), FlyingCreature(
    self ) ) )
                    {//we're in the goal's wp
                            inGoalWP = qtrue;
                            //we're in the goalEntity's waypoint already
                            //so head for the goalEntity since we know it's
    clear of architecture
                            //FIXME: this is pretty stupid because the NPCs try
    to go straight
                            //		towards their goal before then even try
    macro_nav... VectorCopy( self->NPC->goalEntity->r.currentOrigin, origin );
                    }
            }
    }
    */
    if (!inGoalWP) {                    // not heading straight for goal
      if (bestNode == self->waypoint) { // we know it's clear or architecture
        ...
        /*
        if ( NAV_HitNavGoal( self->r.currentOrigin, self->r.mins, self->r.maxs,
        origin, trap->Nav_GetNodeRadius( bestNode ), FlyingCreature( self ) ) )
        {//we're in the wp we're heading for already
                inBestWP = qtrue;
        }
        */
      } else { // heading to an edge off our confirmed clear waypoint... make
               // sure it's clear
        // it it's not, bestNode will fall back to our waypoint
        int oldBestNode = bestNode;
        bestNode = NAV_TestBestNode(self, self->waypoint, bestNode, qtrue);
        if (bestNode ==
            self->waypoint) { // we fell back to our waypoint, reset the origin
          self->NPC->aiFlags |= NPCAI_BLOCKED;
    ...
    foundClearPath = NAVNEW_AvoidCollision(self, self->NPC->goalEntity,
                                           &tempInfo, setBlockedInfo, 5);

    if (!foundClearPath) { // blocked by an ent
      if (inGoalWP) { // we were heading straight for the goal, head for the
                      // goal's wp instead
        trap->Nav_GetNodePosition(bestNode, origin);
        foundClearPath = NAVNEW_AvoidCollision(self, self->NPC->goalEntity,
                                               &tempInfo, setBlockedInfo, 5);
    ...
    if (foundClearPath) { // clear!
      ...
      if (inGoalWP) { // we headed for our goal and failed and our goal's WP and
                      // failed
        if (self->waypoint ==
            self->NPC->goalEntity->waypoint) { // our waypoint is our goal's
                                               // waypoint, nothing we can do
          // remember that this node is blocked
          trap->Nav_AddFailedNode((sharedEntity_t *)self, self->waypoint);
          goto failed;
        } else { // try going for our waypoint this time
          inGoalWP = qfalse;
        }
      } else if (bestNode !=
                 self->waypoint) { // we headed toward our next waypoint
                                   // (instead of our waypoint) and failed
        if (d_altRoutes.integer) { // mark this edge failed and try our waypoint
          ...
          //			between my waypoint and the bestNode... I could be
          //off 			that path because of collision avoidance...
          if (d_patched.integer && // use patch-style navigation
              (!trap->Nav_NodesAreNeighbors(self->waypoint, bestNode) ||
               NAVNEW_TestNodeConnectionBlocked(
                   self->waypoint, bestNode, self,
                   self->NPC->goalEntity->s.number, qfalse,
                   qtrue))) { // the direct path between these 2 nodes is
                              // blocked by an ent
        ...
        if (d_altRoutes.integer) {
          ...
          // FIXME: cache the trace-data for subsequent calls as only the route
          // info would have changed if ( (bestNode =
          // trap->Nav_GetBestPathBetweenEnts( self, self->NPC->goalEntity,
          // NF_CLEAR_PATH )) == NODE_NONE )//!NAVNEW_GetWaypoints( self, qfalse
          // ) )
...
failed:
  ...
  //		closest clearpath waypoints, ranked.  If the first set fails, try
  //the rest 		until there are no alternatives.
  ...
  if ( DistanceSquared( origin, self->r.currentOrigin ) < 16 )//woo, magic
  number
