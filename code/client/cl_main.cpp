...
// RAZFIXME: BAD BAD, maybe? had to move it out of ghoul2_shared.h ->
// CGhoul2Info_v at the least..
...
void CL_FlushMemory(void) {
  ...
  // clear sounds (moved higher up within this func to avoid the odd sound
  // stutter)
  S_DisableSounds();
  ...
  // rwwFIXMEFIXME: The game server appears to continue running, so clearing
  // common bsp data causes crashing and other bad things
  /*
  CM_ClearMap();
  */
...
void CL_Disconnect_f(void) {
  ...
  // FIXME:
  // TA codebase added additional CA_CINEMATIC check below, presumably so they
  // could play cinematics
  //	in the menus when disconnected, although having the SCR_StopCinematic()
  //call above is weird.
  // Either there's a bug, or the new version of that function
  // conditionally-doesn't stop cinematics...
  //
...
void CL_Vid_Restart_f(void) {
  ...
  // rww - sof2mp does this here, but it seems to cause problems in this
  // codebase.
  //	CM_ClearMap();
...
void CL_ConnectionlessPacket(netadr_t from, msg_t *msg) {
  ...
  if (!strcmp(c, "connectResponse")) {
    ...
    Netchan_Setup(NS_CLIENT, &clc.netchan, from,
                  Cvar_VariableIntegerValue("net_qport"));
...
void CL_CheckTimeout(void) {
  ...
  if ((!CL_CheckPaused() || !sv_paused->integer)
      //		&& cls.state >= CA_CONNECTED && cls.state !=
      //CA_CINEMATIC
      && cls.state >= CA_CONNECTED &&
...
void CL_Frame(int msec, float fractionMsec) {
  ...
  if (cls.state == CA_DISCONNECTED && !(Key_GetCatcher() & KEYCATCH_UI) &&
    ...
    if (!CL_CheckPendingCinematic()) // this avoid having the menu flash for one
                                     // frame before pending cinematics
    {
  ...
  if (cl_pano->integer && cls.state == CA_ACTIVE) { // grab some panoramic shots
    ...
    for (; i <= cl_panoNumShots->integer; i++) {
      ...
      Cbuf_ExecuteText(
          EXEC_NOW, va("screenshot %dpano%02d\n", pref, i)); // grab this screen
    }
...
void CL_StartHunkUsers(void) {
  ...
  // we require the ui to be loaded here or else it crashes trying to access the
  // ui on command line map loads
  if (!cls.uiStarted) {
    cls.uiStarted = qtrue;
    CL_InitUI();
  }

  //	if ( !cls.cgameStarted && cls.state > CA_CONNECTED && cls.state !=
  //CA_CINEMATIC ) {
  if (!cls.cgameStarted && cls.state > CA_CONNECTED &&
      (cls.state != CA_CINEMATIC && !CL_IsRunningInGameCinematic())) {
...
extern cvar_t *Cvar_Set2(const char *var_name, const char *value,
                         qboolean force);
extern CMiniHeap *G2VertSpaceServer;
static CMiniHeap *GetG2VertSpaceServer(void) { return G2VertSpaceServer; }

// NOTENOTE: If you change the output name of rd-vanilla, change this define
// too!
#ifdef JK2_MODE
...
void CL_Init(void) {
  ...
  Cvar_Get("snd", "jaden_fmle",
           CVAR_USERINFO | CVAR_ARCHIVE | CVAR_SAVEGAME |
               CVAR_NORESTART); // UI_SetSexandSoundForModel changes to match
                                // sounds.cfg for model
