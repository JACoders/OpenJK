...
=================
*/
void R_AttachFBOTextureDepth(int texId) { qglFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, texId, 0); }
...
=================
*/
void R_AttachFBOTexturePackedDepthStencil(int texId) { qglFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, texId, 0); }
...
void FBO_Init(void) {
	...
	if (tr.pshadowArrayImage != NULL) {
		for (i = 0; i < MAX_DRAWN_PSHADOWS; i++) {
			tr.pshadowFbos[i] = FBO_Create(va("_shadowmap%i", i), tr.pshadowArrayImage->width, tr.pshadowArrayImage->height);
	...
	if (tr.sunShadowArrayImage != NULL) {
		for (i = 0; i < 3; i++) {
			tr.sunShadowFbo[i] = FBO_Create(va("_sunshadowmap%i", i), tr.sunShadowArrayImage->width, tr.sunShadowArrayImage->height);
	...
	if (tr.renderCubeImage != NULL) {
		for (i = 0; i < 6; i++) {
			tr.renderCubeFbo[i] = FBO_Create("_renderCubeFbo", tr.renderCubeImage->width, tr.renderCubeImage->height);
		...
		tr.filterCubeFbo = FBO_Create("_filterCubeFbo", tr.renderCubeImage->width, tr.renderCubeImage->height);
	...
	if (tr.weatherDepthImage != nullptr) {
		tr.weatherDepthFbo = FBO_Create("_weatherDepthFbo", tr.weatherDepthImage->width, tr.weatherDepthImage->height);
...
void FBO_BlitFromTexture(struct image_s *src, vec4i_t inSrcBox, vec2_t inSrcTexScale, FBO_t *dst, vec4i_t inDstBox, struct shaderProgram_s *shaderProgram,
						 vec4_t inColor, int blend) {
...
void FBO_FastBlit(FBO_t *src, vec4i_t srcBox, FBO_t *dst, vec4i_t dstBox, int buffers, int filter) {
	...
	qglBlitFramebuffer(srcBoxFinal[0], srcBoxFinal[1], srcBoxFinal[2], srcBoxFinal[3], dstBoxFinal[0], dstBoxFinal[1], dstBoxFinal[2], dstBoxFinal[3], buffers,
					   filter);
