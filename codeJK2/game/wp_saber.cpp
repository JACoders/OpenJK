...
int parryDebounce[NUM_FORCE_POWER_LEVELS] = {1000000, // if don't even have defense, can't use defense!
											 300, 150, 50};
...
void WP_SaberUpdateOldBladeData(gentity_t *ent) {
	if (ent->client) {
		...
		if (ent->client->ps.saberLengthOld <= 0 && ent->client->ps.saberLength > 0) { // just turned on
			// do sound event
			vec3_t saberOrg;
	...
	for (z = 0; z < MAX_G2_COLLISIONS; z++) {
		...
		*/
		...
		if (i == numHitEnts) {					   // first time we hit this ent
			...
			if ((hitLoc[numHitEnts] == HL_NONE && trHitLoc == HL_NONE) || (hitDismemberLoc[numHitEnts] == HL_NONE && trDismemberLoc == HL_NONE) ||
				(!hitDismember[numHitEnts] && !trDismember)) { // no hit loc set for this ent this damage cycle yet
				...
				trDismember = G_GetHitLocFromSurfName(&g_entities[coll.mEntityNum],
													  gi.G2API_GetSurfaceName(&g_entities[coll.mEntityNum].ghoul2[coll.mModelIndex], coll.mSurfaceIndex),
...
qboolean WP_SaberDamageForTrace(int ignore, vec3_t start, vec3_t end, float dmg, vec3_t bladeDir, qboolean noGhoul, int attackStrength,
	...
	if (!noGhoul) {
		if (!attacker->s.number || (attacker->client && (attacker->client->playerTeam == TEAM_PLAYER || attacker->client->NPC_class == CLASS_SHADOWTROOPER ||
														 attacker->client->NPC_class == CLASS_TAVION ||
														 attacker->client->NPC_class == CLASS_DESANN))) //&&attackStrength==FORCE_LEVEL_3)
	...
	if (&g_entities[tr.entityNum]) {
		...
		if (attacker && attacker->client && attacker->client->ps.saberInFlight) { // thrown saber hit something
			if ((hitEnt && hitEnt->client && hitEnt->health > 0 &&
				 (hitEnt->client->NPC_class == CLASS_DESANN || hitEnt->client->NPC_class == CLASS_LUKE ||
				  (hitEnt->client->NPC_class == CLASS_GALAKMECH && hitEnt->client->ps.powerups[PW_GALAK_SHIELD] > 0))) ||
				(owner && owner->client && owner->health > 0 &&
				 (owner->client->NPC_class == CLASS_DESANN || owner->client->NPC_class == CLASS_LUKE ||
		...
		if (hitEnt->takedamage) {
			// no team damage: if ( !hitEnt->client || attacker == NULL || !attacker->client || (hitEnt->client->playerTeam != attacker->client->playerTeam) )
			{
				...
				if (noGhoul || !hitEnt->ghoul2.size()) { // we weren't doing a ghoul trace
					...
					if (!WP_SaberDamageEffects(&tr, start, len, dmg, dir, bladeDir, attacker->client->enemyTeam)) { // didn't hit a ghoul ent
						/*
...
qboolean WP_SabersCheckLock(gentity_t *ent1, gentity_t *ent2) {
	...
	if (ent1->client->ps.torsoAnim == BOTH_A1_BR_TL || ent1->client->ps.torsoAnim == BOTH_A2_BR_TL || ent1->client->ps.torsoAnim == BOTH_A3_BR_TL ||
		ent1->client->ps.torsoAnim == BOTH_A4_BR_TL || ent1->client->ps.torsoAnim == BOTH_A5_BR_TL) { // ent1 is attacking in the opposite diagonal
	...
	if (ent2->client->ps.torsoAnim == BOTH_A1_BR_TL || ent2->client->ps.torsoAnim == BOTH_A2_BR_TL || ent2->client->ps.torsoAnim == BOTH_A3_BR_TL ||
		ent2->client->ps.torsoAnim == BOTH_A4_BR_TL || ent2->client->ps.torsoAnim == BOTH_A5_BR_TL) { // ent2 is attacking in the opposite diagonal
	...
	if (ent1->client->ps.torsoAnim == BOTH_A1_BL_TR || ent1->client->ps.torsoAnim == BOTH_A2_BL_TR || ent1->client->ps.torsoAnim == BOTH_A3_BL_TR ||
		ent1->client->ps.torsoAnim == BOTH_A4_BL_TR || ent1->client->ps.torsoAnim == BOTH_A5_BL_TR) { // ent1 is attacking in the opposite diagonal
	...
	if (ent2->client->ps.torsoAnim == BOTH_A1_BL_TR || ent2->client->ps.torsoAnim == BOTH_A2_BL_TR || ent2->client->ps.torsoAnim == BOTH_A3_BL_TR ||
		ent2->client->ps.torsoAnim == BOTH_A4_BL_TR || ent2->client->ps.torsoAnim == BOTH_A5_BL_TR) { // ent2 is attacking in the opposite diagonal
	...
	if (ent1->client->ps.torsoAnim == BOTH_A1__L__R || ent1->client->ps.torsoAnim == BOTH_A2__L__R || ent1->client->ps.torsoAnim == BOTH_A3__L__R ||
		ent1->client->ps.torsoAnim == BOTH_A4__L__R || ent1->client->ps.torsoAnim == BOTH_A5__L__R) { // ent1 is attacking l to r
	...
	if (ent2->client->ps.torsoAnim == BOTH_A1__L__R || ent2->client->ps.torsoAnim == BOTH_A2__L__R || ent2->client->ps.torsoAnim == BOTH_A3__L__R ||
		ent2->client->ps.torsoAnim == BOTH_A4__L__R || ent2->client->ps.torsoAnim == BOTH_A5__L__R) { // ent2 is attacking l to r
	...
	if (ent1->client->ps.torsoAnim == BOTH_A1__R__L || ent1->client->ps.torsoAnim == BOTH_A2__R__L || ent1->client->ps.torsoAnim == BOTH_A3__R__L ||
		ent1->client->ps.torsoAnim == BOTH_A4__R__L || ent1->client->ps.torsoAnim == BOTH_A5__R__L) { // ent1 is attacking r to l
	...
	if (ent2->client->ps.torsoAnim == BOTH_A1__R__L || ent2->client->ps.torsoAnim == BOTH_A2__R__L || ent2->client->ps.torsoAnim == BOTH_A3__R__L ||
		ent2->client->ps.torsoAnim == BOTH_A4__R__L || ent2->client->ps.torsoAnim == BOTH_A5__R__L) { // ent2 is attacking r to l
	...
	if (ent1->client->ps.torsoAnim == BOTH_A1_TR_BL || ent1->client->ps.torsoAnim == BOTH_A2_TR_BL || ent1->client->ps.torsoAnim == BOTH_A3_TR_BL ||
		ent1->client->ps.torsoAnim == BOTH_A4_TR_BL || ent1->client->ps.torsoAnim == BOTH_A5_TR_BL) { // ent1 is attacking diagonally
	...
	if (ent2->client->ps.torsoAnim == BOTH_A1_TR_BL || ent2->client->ps.torsoAnim == BOTH_A2_TR_BL || ent2->client->ps.torsoAnim == BOTH_A3_TR_BL ||
		ent2->client->ps.torsoAnim == BOTH_A4_TR_BL || ent2->client->ps.torsoAnim == BOTH_A5_TR_BL) { // ent2 is attacking diagonally
	...
	if (ent1->client->ps.torsoAnim == BOTH_A1_TL_BR || ent1->client->ps.torsoAnim == BOTH_A2_TL_BR || ent1->client->ps.torsoAnim == BOTH_A3_TL_BR ||
		ent1->client->ps.torsoAnim == BOTH_A4_TL_BR || ent1->client->ps.torsoAnim == BOTH_A5_TL_BR) { // ent1 is attacking diagonally
	...
	if (ent2->client->ps.torsoAnim == BOTH_A1_TL_BR || ent2->client->ps.torsoAnim == BOTH_A2_TL_BR || ent2->client->ps.torsoAnim == BOTH_A3_TL_BR ||
		ent2->client->ps.torsoAnim == BOTH_A4_TL_BR || ent2->client->ps.torsoAnim == BOTH_A5_TL_BR) { // ent2 is attacking diagonally
	...
	if (ent1->client->ps.torsoAnim == BOTH_A1_T__B_ || ent1->client->ps.torsoAnim == BOTH_A2_T__B_ || ent1->client->ps.torsoAnim == BOTH_A3_T__B_ ||
		ent1->client->ps.torsoAnim == BOTH_A4_T__B_ || ent1->client->ps.torsoAnim == BOTH_A5_T__B_) { // ent1 is attacking top-down
		/*
	...
	if (ent2->client->ps.torsoAnim == BOTH_A1_T__B_ || ent2->client->ps.torsoAnim == BOTH_A2_T__B_ || ent2->client->ps.torsoAnim == BOTH_A3_T__B_ ||
		ent2->client->ps.torsoAnim == BOTH_A4_T__B_ || ent2->client->ps.torsoAnim == BOTH_A5_T__B_) { // ent2 is attacking top-down
		/*
...
void WP_SaberDamageTrace(gentity_t *ent) {
	...
	if (ent->client->ps.saberInFlight) { // flying sabers are much more deadly
		// unless you're dead
		if (ent->health <= 0 && !g_saberRealisticCombat->integer) { // so enemies don't keep trying to block it
			// FIXME: still do damage, just not to humanoid clients who should try to avoid it
			// baseDamage = 0.0f;
			return;
		}
		// or unless returning
		else if (ent->client->ps.saberEntityState == SES_RETURNING) { // special case, since we're returning, chances are if we hit something
			...
			if (!PM_SaberInSpecialAttack(ent->client->ps.torsoAnim) &&
				...
				baseDamage = 0;
					   ...
					   (PM_SaberInAttack(ent->client->ps.saberMove) || PM_SaberInSpecialAttack(ent->client->ps.torsoAnim) ||
						...
						entPowerLevel > FORCE_LEVEL_2)) { // normal attack swing swinging/spinning (or if using strong set), do normal damage
		...
		if (ent->health > 0 && !ent->client->ps.saberLockTime) {
			trace_t trace;
			gi.trace(&trace, ent->currentOrigin, vec3_origin, vec3_origin, mp1, ent->s.number, (MASK_SHOT & ~(CONTENTS_CORPSE | CONTENTS_ITEM)), G2_NOCOLLIDE,
					 0);
			...
			{																	 // a valid ent
				...
				if (traceEnt && traceEnt->client && traceEnt->health > 0 && traceEnt->client->playerTeam != ent->client->playerTeam &&
					...
					if (!traceEnt->client->ps.saberLockTime && !traceEnt->message) { // don't push people in saberlock or with security keys
						vec3_t hitDir;
	...
	if ((saberHitFraction < 1.0f || (sabersCrossed >= 0 && sabersCrossed <= 32.0f)) &&
		(ent->client->ps.weaponstate == WEAPON_FIRING || ent->client->ps.saberInFlight)) { // The saber (in-hand) hit another saber, mano.
		...
		if (hitOwner && hitOwner->client) {
			...
			if (entPowerLevel == FORCE_LEVEL_3 && PM_SaberInSpecialAttack(ent->client->ps.torsoAnim)) { // a special "unblockable" attack
				if (hitOwner->client->NPC_class == CLASS_DESANN || hitOwner->client->NPC_class == CLASS_TAVION ||
					hitOwner->client->NPC_class == CLASS_LUKE) { // these masters can even block unblockables (stops cheap kills)
		...
		if (ent->client->ps.saberInFlight && ent->client->ps.saberActive && ent->client->ps.saberEntityNum != ENTITYNUM_NONE &&
		...
		if (hitOwner && hitOwner->health > 0 && ent->health > 0																	  // both are alive
			...
			if (baseDamage) { // there is damage involved, not just effects
				...
				if (entAttacking && hitOwnerAttacking &&
					(entPowerLevel == hitOwnerPowerLevel || (entPowerLevel > FORCE_LEVEL_2 && hitOwnerPowerLevel > FORCE_LEVEL_2) ||
					 (entPowerLevel < FORCE_LEVEL_3 && hitOwnerPowerLevel < FORCE_LEVEL_3 &&
					  hitOwner->client->ps.forcePowerLevel[FP_SABER_OFFENSE] > FORCE_LEVEL_2 && Q_irand(0, 2)) ||
					 (entPowerLevel < FORCE_LEVEL_2 && hitOwnerPowerLevel < FORCE_LEVEL_3 &&
					  hitOwner->client->ps.forcePowerLevel[FP_SABER_OFFENSE] > FORCE_LEVEL_1 && Q_irand(0, 1)) ||
					...
					if (!Q_irand(0, 2) && activeDefense &&
						...
						if (entPowerLevel < FORCE_LEVEL_3
							...
							&& hitOwner->client->ps.saberAnimLevel != FORCE_LEVEL_5 && activeDefense &&
							(hitOwnerPowerLevel > FORCE_LEVEL_2 ||
							 (hitOwner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_2 &&
							  Q_irand(0, hitOwner->client->ps.saberAnimLevel)))) { // knockaways can make fast-attacker go into a broken parry anim if the ent
																				   // is using fast or med (but not Tavion)
							// make me parry
							...
							if (d_saberCombat->integer) {
								gi.Printf(S_COLOR_RED "%s knockaway %s's attack, new move = %s, anim = %s\n", hitOwner->NPC_type, ent->NPC_type,
								   ...
								   (!deflected &&
									Q_irand(0,
											PM_PowerLevelForSaberAnim(&ent->client->ps) -
							...
							if (hitOwner->client->ps.forcePowerLevel[FP_SABER_DEFENSE] < FORCE_LEVEL_2 && (ent->s.number || g_saberRealisticCombat->integer) &&
								...
								if ((ent->client->ps.saberAnimLevel == FORCE_LEVEL_3 && !Q_irand(0, 3)) ||
									(ent->client->ps.saberAnimLevel == FORCE_LEVEL_4 && !Q_irand(0, 1))) { // a strong attack
								...
								if ((ent->client->ps.saberAnimLevel == FORCE_LEVEL_3 && !Q_irand(0, 5)) ||
									(ent->client->ps.saberAnimLevel == FORCE_LEVEL_4 && !Q_irand(0, 3))) { // a strong attack
									if (WP_BrokenParryKnockDown(hitOwner)) {
										hitOwner->client->ps.saberBlocked = BLOCKED_NONE;
										hitOwner->client->ps.saberBounceMove = LS_NONE;
									}
							...
							if (PM_SaberInBounce(ent->client->ps.saberMove) // FIXME: saberMove not set until pmove!
								&& activeDefense && hitOwner->client->ps.saberAnimLevel != FORCE_LEVEL_1 &&
								...
								hitOwner->client->ps.saberBounceMove = PM_KnockawayForParry(hitOwner->client->ps.saberBlocked);
									   ...
									   (ent->client->ps.saberAnimLevel == FORCE_LEVEL_4 && !Q_irand(0, 3))) { // a strong attack can sometimes do a knockdown
...
ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE] ) )
		...
		if (saberHitFraction < 1.0f) {
			if (!collisionResolved && baseDamage) { // some other kind of in-hand saber collision
				...
				if (hitOwner && hitOwner->health > 0 && hitOwner->client && !hitOwner->client->ps.saberInFlight &&
					...
					if (PM_SaberInAttack(hitOwner->client->ps.saberMove) || PM_SaberInSpecialAttack(hitOwner->client->ps.torsoAnim) ||
						(hitOwner->client->ps.saberAnimLevel > FORCE_LEVEL_2 && !PM_SaberInIdle(hitOwner->client->ps.saberMove) &&
						...
						*/
						...
						if (!PM_SaberInBrokenParry(hitOwner->client->ps.saberMove)) { // not currently in a broken parry
							if (!WP_SaberParry(hitOwner, ent)) { // FIXME: hitOwner can't parry, do some time-consuming saber-knocked-aside broken parry anim?
																 // hitOwner->client->ps.saberBlocked = BLOCKED_PARRY_BROKEN;
							}
						}
					}
			...
			if (inFlightSaberBlocked) { // we threw a saber and it was blocked, do any effects, etc.
				...
				if (hitEnt && hitOwner && hitOwner->client &&
					(PM_SaberInAttack(hitOwner->client->ps.saberMove) || PM_SaberInSpecialAttack(hitOwner->client->ps.torsoAnim) ||
					 PM_SpinningSaberAnim(hitOwner->client->ps.torsoAnim))) { // if hit someone who was in an attack or spin anim, more likely to have in-flight
																			  // saber knocked away
					if (hitOwnerPowerLevel > FORCE_LEVEL_2) { // string attacks almost always knock it aside!
						knockAway = 1;
					} else { // 33% chance
						knockAway = 2;
					}
				...
				if (!Q_irand(0, knockAway) || // random
					(hitOwner && hitOwner->client &&
					 (hitOwner->client->NPC_class == CLASS_DESANN || hitOwner->client->NPC_class == CLASS_TAVION ||
					  hitOwner->client->NPC_class == CLASS_LUKE)) // or if blocked by a Boss character FIXME: or base on defense level?
	...
	if (ent->client->ps.saberLockTime > level.time && ent->s.number < ent->client->ps.saberLockEnemy && !Q_irand(0, 3)) { // need to make some kind of effect
	...
	if (WP_SaberApplyDamage(ent, baseDamage, baseDFlags, brokenParry)) { // actually did damage to something
#ifndef FINAL_BUILD
		if (d_saberCombat->integer) {
	...
	if (!other->takedamage && (saber->s.eFlags & (EF_BOUNCE | EF_BOUNCE_HALF))) {
		...
		// see if we stopped
			...
			saber->aimDebounceTime = level.time;
			   ...
			   (other->client->NPC_class == CLASS_DESANN || other->client->NPC_class == CLASS_TAVION || other->client->NPC_class == CLASS_LUKE ||
		...
		if (ent_count) {
			for (int x = 0; x < ent_count; x++) {
				if (missile_list[x]->s.weapon == WP_SABER) { // just send it back
					if (missile_list[x]->owner && missile_list[x]->owner->client && missile_list[x]->owner->client->ps.saberActive &&
...
void WP_RunSaber(gentity_t *self, gentity_t *saber) {
	...
	gi.trace(&tr, saber->currentOrigin, saber->mins, saber->maxs, origin, saber->owner ? saber->owner->s.number : ENTITYNUM_NONE, clipmask, G2_NOCOLLIDE, 0);
...
qboolean WP_SaberLaunch(gentity_t *self, gentity_t *saber, qboolean thrown) { // FIXME: probably need a debounce time
	...
	gi.trace(&trace, self->client->renderInfo.handRPoint, saberMins, saberMaxs, self->client->renderInfo.handRPoint, saber->s.number, MASK_SOLID, G2_NOCOLLIDE,
			 0);
	if (trace.startsolid || trace.allsolid) {
		return qfalse;
	}
...
void WP_SaberThrow(gentity_t *self, usercmd_t *ucmd) {
	...
	if (!self->client->ps.saberInFlight) {		   // saber is not in flight right now
		...
		if (saberent->s.pos.trType != TR_STATIONARY) { // saber is in flight, lerp it
			...
			if (self->health <= 0 && level.time > saberent->s.time + 5000) { // make us free ourselves after a time
	...
	if (self->client->ps.weapon != WP_SABER) { // switched away
		...
		WP_SaberDrop(self, saberent);
		return;
	} else if (!self->client->ps.saberActive && self->client->ps.saberEntityState != SES_RETURNING) { // we turned it off, drop it
		WP_SaberDrop(self, saberent);
		return;
	}
...
void WP_SaberBlock(gentity_t *saber, vec3_t hitloc, qboolean missileBlock) {
	...
	if (saber && saber->owner) {
		...
		if (playerent->client->ps.weaponstate == WEAPON_DROPPING || playerent->client->ps.weaponstate == WEAPON_RAISING) { // don't block while changing weapons
			return;
		}
...
void WP_SaberStartMissileBlockCheck(gentity_t *self, usercmd_t *ucmd) {
	...
	for (e = 0; e < numListedEntities; e++) {
		...
		if (ent->s.eType != ET_MISSILE && !(ent->s.eFlags & EF_MISSILE_STICK)) { // not a normal projectile
			...
			if (ent->owner->health <= 0 && !g_saberRealisticCombat->integer) { // it's not doing damage, so ignore it
				continue;
			}
		} else {
		...
		if (ent->s.weapon == WP_THERMAL) { // thermal detonator!
			...
			if (!self->s.number) { // players don't auto-handle these at all
				...
				if (ent->s.pos.trType == TR_STATIONARY && (ent->s.eFlags & EF_MISSILE_STICK)) { // a placed explosive like a tripmine or detpack
	...
	if (incoming) {
		if (self->NPC && !G_ControlledByPlayer(self)) {
			...
			if (Jedi_SaberBlockGo(self, &self->NPC->last_ucmd, NULL, NULL, incoming) != EVASION_NONE) { // make sure to turn on your saber if it's not on
				self->client->ps.saberActive = qtrue;
			}
		} else // player
		{
...
void WP_SaberUpdate(gentity_t *self, usercmd_t *ucmd) {
	...
	if (!self->client->ps.saberInFlight) {										  // It isn't, which means we can update its position as we will.
		if (!self->client->ps.saberActive || PM_InKnockDown(&self->client->ps)) { // can't block if saber isn't on
			...
			if (((self->s.number && !Jedi_SaberBusy(self) && !g_saberRealisticCombat->integer) ||
				...
				self->client->ps.weaponTime <= 0) { // full-size blocking for non-attacking player with g_saberAutoBlocking on
...
void ForceThrow(gentity_t *self, qboolean pull) { // FIXME: pass in a target ent so we (an NPC) can push/pull just one targeted ent.
	...
	if (cone >= 1.0f) { // must be pointing right at them
		...
		gi.trace(&tr, self->client->renderInfo.eyePoint, vec3_origin, vec3_origin, end, self->s.number,
				 MASK_OPAQUE | CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_ITEM | CONTENTS_CORPSE, G2_NOCOLLIDE,
	...
	for (e = 0; e < numListedEntities; e++) {
		...
		if (ent->s.eType != ET_MISSILE) {
			...
			if (ent->s.eType != ET_ITEM && ent->e_ThinkFunc != thinkF_G_RunObject) //|| !(ent->flags&FL_DROPPED_ITEM) )//was only dropped items
			{
				...
				} else if (ent->client->NPC_class == CLASS_GALAKMECH || ent->client->NPC_class == CLASS_ATST) { // can't push ATST or Galak
					continue;
			...
			} else if (ent->s.eType == ET_ITEM && ent->item && ent->item->giType == IT_HOLDABLE && ent->item->giTag == INV_SECURITY_KEY)
		...
		if (ent != forwardEnt) { // don't need to trace against forwardEnt again
			...
			gi.trace(&tr, self->client->renderInfo.eyePoint, vec3_origin, vec3_origin, ent_org, self->s.number, MASK_OPAQUE | CONTENTS_SOLID, G2_NOCOLLIDE,
	...
	if (ent_count) {
		for (int x = 0; x < ent_count; x++) {
			if (push_list[x]->client) {
				...
				if (!push_list[x]->s.number) {										   // player
					...
					{																									 // trying to push or pull the player!
						if (push_list[x]->client->ps.powerups[PW_FORCE_PUSH] > level.time								 // player was pushing/pulling too
							|| (pull && Q_irand(0, (push_list[x]->client->ps.forcePowerLevel[FP_PULL] - self->client->ps.forcePowerLevel[FP_PULL]) * 2 + 1) >
											0) // player's pull is high enough
							|| (!pull && Q_irand(0, (push_list[x]->client->ps.forcePowerLevel[FP_PUSH] - self->client->ps.forcePowerLevel[FP_PUSH]) * 2 + 1) >
											 0)) // player's push is high enough
						{						 // player's force push/pull is high enough to try to stop me
							if (InFront(self->currentOrigin, push_list[x]->client->renderInfo.eyePoint, push_list[x]->client->ps.viewangles,
				...
				if (((self->s.number == 0 && Q_irand(0, 2)) || Q_irand(0, 2)) && push_list[x]->client && push_list[x]->health > 0 // a living client
					...
					if (pull) {
						...
						if (self->client->ps.forcePowerLevel[FP_PULL] > FORCE_LEVEL_1 && push_list[x]->s.weapon != WP_SABER &&
							push_list[x]->s.weapon != WP_MELEE &&
							push_list[x]->s.weapon != WP_THERMAL) { // yank the weapon - NOTE: level 1 just knocks them down, not take weapon
			...
			} else if (push_list[x]->s.weapon == WP_SABER && (push_list[x]->contents & CONTENTS_LIGHTSABER)) { // a thrown saber, just send it back
				/*
				if ( pull )
				{//steal it?
				}
		...
		if (pull) {
			if (self->client->ps.forcePowerLevel[FP_PUSH] > FORCE_LEVEL_2) { // at level 3, can pull multiple, so it costs more
			...
			} else if (self->client->ps.forcePowerLevel[FP_PUSH] > FORCE_LEVEL_1) { // at level 3, can push multiple, so costs more
	...
	if (targetLive && traceEnt->NPC) {						// hit an organic non-player
		if (G_ActivateBehavior(traceEnt, BSET_MINDTRICK)) { // activated a script on him
			...
			if ((traceEnt->NPC->scriptFlags & SCF_NO_MIND_TRICK)) {
				...
				if (self->client->ps.forcePowerLevel[FP_TELEPATHY] > FORCE_LEVEL_2) { // turn them to our side
...
void ForceGrip(gentity_t *self) { // FIXME: make enemy Jedi able to use this
	...
	if (self->enemy &&
		(self->s.number || InFront(self->enemy->currentOrigin, self->client->renderInfo.eyePoint, self->client->ps.viewangles,
		...
		if (!traceEnt->client || traceEnt->client->playerTeam != self->client->playerTeam || self->enemy == traceEnt ||
			...
			if (traceEnt->client && traceEnt->health > 0 &&
				(traceEnt->client->NPC_class == CLASS_DESANN ||
				 traceEnt->client->NPC_class == CLASS_LUKE)) { // Luke and Desann can shield themselves from the attack
...
void ForceJump(gentity_t *self, usercmd_t *ucmd) {
	...
	case FJ_FORWARD:
		if (self->NPC && self->NPC->rank != RANK_CREWMAN && self->NPC->rank <= RANK_LT_JG) { // can't do acrobatics
...
static void WP_ForcePowerRun(gentity_t *self, forcePowers_t forcePower, usercmd_t *cmd) {
	...
	switch ((int)forcePower) {
	case FP_HEAL:
		if (self->client->ps.forceHealCount >= MAX_FORCE_HEAL || self->health >= self->client->ps.stats[STAT_MAX_HEALTH]) { // fully healed or used up all 25
	...
	case FP_GRIP:
		if (!WP_ForcePowerAvailable(self, FP_GRIP, 0) ||
			...
			{															   // either invalid ent, or dead ent
				WP_ForcePowerStop(self, FP_GRIP);
				return;
			} else if (self->client->ps.forcePowerLevel[FP_GRIP] == FORCE_LEVEL_1 && gripEnt->client && gripEnt->client->ps.groundEntityNum == ENTITYNUM_NONE) {
				WP_ForcePowerStop(self, FP_GRIP);
				return;
			} else if (gripEnt->s.weapon == WP_SABER && gripEnt->NPC && gripEnt->client && gripEnt->client->ps.forcePowersKnown & (1 << FP_PUSH) &&
					   !Q_irand(0, 100 - (gripEnt->NPC->stats.evasion * 10) -
									   (g_spskill->integer * 10))) { // a jedi who broke free FIXME: maybe have some minimum grip length- a reaction time?
				...
				if (gripEnt->client) {
					if (self->client->ps.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1) { // level 1 just holds them
						...
						if (self->client->ps.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_2) { // level 2 just lifts them
				...
				if (self->client->ps.forcePowerDebounce[FP_GRIP] < level.time) {
					...
					G_Damage(gripEnt, self, self, dir, gripOrg, forceGripDamage[self->client->ps.forcePowerLevel[FP_GRIP]], DAMAGE_NO_ARMOR,
							 MOD_CRUSH); // MOD_???
					if (gripEnt->s.number) {
...
void WP_InitForcePowers(gentity_t *ent) {
	...
	if (ent->client->NPC_class == CLASS_TAVION || ent->client->NPC_class == CLASS_REBORN || ent->client->NPC_class == CLASS_DESANN ||
		ent->client->NPC_class == CLASS_SHADOWTROOPER || ent->client->NPC_class == CLASS_JEDI || ent->client->NPC_class == CLASS_LUKE) { // an NPC jedi
		...
		if (ent->client->NPC_class == CLASS_DESANN) {
			ent->client->ps.forcePowersKnown = (1 << FP_LEVITATION) | (1 << FP_PUSH) | (1 << FP_PULL) | (1 << FP_GRIP) | (1 << FP_LIGHTNING) |
			...
			ent->client->ps.forcePowersKnown = (1 << FP_LEVITATION) | (1 << FP_PUSH) | (1 << FP_PULL) | (1 << FP_SABERTHROW) | (1 << FP_SPEED) |
			...
			ent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] = FORCE_LEVEL_3;
				   ...
				   (ent->client->NPC_class == CLASS_JEDI && ent->NPC->rank == RANK_COMMANDER)) { // Tavia or trainer Jedi
			ent->client->ps.forcePowersKnown = (1 << FP_LEVITATION) | (1 << FP_PUSH) | (1 << FP_PULL) | (1 << FP_SABERTHROW) | (1 << FP_SPEED) |
											   (1 << FP_SABER_DEFENSE) | (1 << FP_SABER_OFFENSE);
			ent->client->ps.forcePowerLevel[FP_LEVITATION] = FORCE_LEVEL_3;
			ent->client->ps.forcePowerLevel[FP_PUSH] = FORCE_LEVEL_3;
			...
			ent->client->ps.forcePowersKnown = (1 << FP_LEVITATION) | (1 << FP_PUSH) | (1 << FP_PULL) | (1 << FP_SABERTHROW) | (1 << FP_GRIP) |
											   (1 << FP_LIGHTNING) | (1 << FP_SPEED) | (1 << FP_SABER_DEFENSE) | (1 << FP_SABER_OFFENSE);
			...
			ent->client->ps.forcePowerLevel[FP_SABER_DEFENSE] = FORCE_LEVEL_3;
			ent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] = FORCE_LEVEL_3;
		} else if (ent->NPC->rank == RANK_LT || ent->client->NPC_class == CLASS_JEDI) { // Reborn Boss or ally Jedi
			ent->client->ps.forcePowersKnown = (1 << FP_LEVITATION) | (1 << FP_PUSH) | (1 << FP_PULL) | (1 << FP_SABERTHROW) | (1 << FP_GRIP) |
			...
			ent->client->ps.forcePowersKnown =
				(1 << FP_LEVITATION) | (1 << FP_PUSH) | (1 << FP_PULL) | (1 << FP_SPEED) | (1 << FP_SABER_DEFENSE) | (1 << FP_SABER_OFFENSE);
			ent->client->ps.forcePowerLevel[FP_PUSH] = FORCE_LEVEL_2;
			ent->client->ps.forcePowerLevel[FP_PULL] = FORCE_LEVEL_1;
			ent->client->ps.forcePowerLevel[FP_SPEED] = FORCE_LEVEL_1;
			...
			ent->client->ps.forcePowerLevel[FP_SABER_OFFENSE] = FORCE_LEVEL_1;
			ent->client->ps.forcePowerLevel[FP_LEVITATION] = FORCE_LEVEL_2;
		} else if (ent->NPC->rank == RANK_CIVILIAN) { // Grunt (NOTE: grunt turns slower and has less health)
