	...
	while ( 1 )
	{
		...
		else if ( !Q_stricmp( token, "map" ) )
		{
			...
			else if ( !Q_stricmp( token, "$lightmap" ) )
			{
				...
				if ( shader.lightmapIndex[0] < 0 ) {
					stage->bundle[0].image = tr.whiteImage;
#ifndef FINAL_BUILD
					//CL_RefPrintf( PRINT_WARNING, "WARNING: $lightmap requested but none available '%s'\n", shader.name );
#endif
				} else {
		...
		else if ( !Q_stricmp( token, "rgbGen" ) )
		{
			...
			else if ( !Q_stricmp( token, "vertex" ) )
			{
				...
				{
					CL_RefPrintf( PRINT_ERROR, "ERROR: rgbGen vertex used on a model! in shader '%s'\n", shader.name );
				}
			...
			else if ( !Q_stricmp( token, "lightingDiffuse" ) )
			{
				if (shader.lightmapIndex[0] != LIGHTMAP_NONE)
				{
					CL_RefPrintf( PRINT_ERROR, "ERROR: rgbGen lightingDiffuse used on a misc_model! in shader '%s'\n", shader.name );
				}
			...
			else if ( !Q_stricmp( token, "lightingDiffuseEntity" ) )
			{
				if (shader.lightmapIndex[0] != LIGHTMAP_NONE)
				{
					CL_RefPrintf( PRINT_ERROR, "ERROR: rgbGen lightingDiffuseEntity used on a misc_model! in shader '%s'\n", shader.name );
				}
			...
			{
				CL_RefPrintf( PRINT_ERROR, "ERROR: unknown rgbGen parameter '%s' in shader '%s'\n", token, shader.name );
				continue;
			}
		}
		//
...
static void ParseSkyParms( const char **text ) {
	...
	token = COM_ParseExt( text, qfalse );
	if ( strcmp( token, "-" ) ) {
		CL_RefPrintf( PRINT_WARNING, "WARNING: in shader '%s' 'skyParms', innerbox is not supported!", shader.name);
	}
}
...
static shader_t *GeneratePermanentShader( void ) {
	...
	if ( tr.numShaders == MAX_SHADERS ) {
		...
		CL_RefPrintf( PRINT_WARNING, "WARNING: GeneratePermanentShader - MAX_SHADERS (%d) hit (overflowed by %d)\n", MAX_SHADERS, tr.iNumDeniedShaders);
		return tr.defaultShader;
	}
...
*/
	...
	if ( !image ) {
		...
		{	//hide these warnings
			CL_RefPrintf( PRINT_WARNING, "WARNING: Couldn't find image for shader %s\n", name );
		}
		shader.defaultShader = true;
		return FinishShader();
	}
...
static void SetupShaderEntryPtrs(void)
{
	...
	//CL_RefPrintf( PRINT_DEVELOPER, "SetupShaderEntryPtrs(): Stored %d shader ptrs\n",ShaderEntryPtrs_Size() );
}
#endif
...
static void ScanAndLoadShaderFiles( void )
{
	...
	{
		Com_Error( ERR_FATAL, "WARNING: no shader files found\n" );
		return;
	}
