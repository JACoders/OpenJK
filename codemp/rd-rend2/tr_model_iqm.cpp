...
static qboolean IQM_CheckRange(iqmHeader_t *header, int offset, int count, int size) {
	...
	return (qboolean)(count <= 0 || offset < 0 || offset > header->filesize || offset + count * size < 0 || offset + count * size > header->filesize);
}
...
void RB_IQMSurfaceAnim(surfaceType_t *surface) {
	...
	for (i = 0; i < surf->num_vertexes; i++, outXYZ++, outNormal++, outTexCoord++, outColor++) {
		...
		(*outXYZ)[0] =
			vtxMat[0] * data->positions[3 * vtx + 0] + vtxMat[1] * data->positions[3 * vtx + 1] + vtxMat[2] * data->positions[3 * vtx + 2] + vtxMat[3];
		(*outXYZ)[1] =
			vtxMat[4] * data->positions[3 * vtx + 0] + vtxMat[5] * data->positions[3 * vtx + 1] + vtxMat[6] * data->positions[3 * vtx + 2] + vtxMat[7];
		(*outXYZ)[2] =
			vtxMat[8] * data->positions[3 * vtx + 0] + vtxMat[9] * data->positions[3 * vtx + 1] + vtxMat[10] * data->positions[3 * vtx + 2] + vtxMat[11];
		(*outXYZ)[3] = 1.0f;
