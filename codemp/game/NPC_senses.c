...
qboolean G_ClearLineOfSight(const vec3_t point1, const vec3_t point2,
  ...
  if (EntIsGlass(hit)) {
    ...
    trap->Trace(&tr, newpoint1, NULL, NULL, point2, hit->s.number, clipmask,
                qfalse, 0, 0);
...
qboolean CanSee(gentity_t *ent) {
  ...
  trap->Trace(&tr, eyes, NULL, NULL, spot, NPCS.NPC->s.number, MASK_OPAQUE,
              qfalse, 0, 0);
...
qboolean InFOV(gentity_t *ent, gentity_t *from, int hFOV, int vFOV) {
  ...
  if (from->client) {
    if (!VectorCompare(from->client->renderInfo.eyeAngles,
...
static int G_CheckSoundEvents(gentity_t *self, float maxHearDist,
  ...
  for (i = 0; i < level.numAlertEvents; i++) {
    ...
    if (level.alertEvents[i]
      ...
      if (G_ClearLOS5(self, level.alertEvents[i].position) ==
...
float G_GetLightLevel(vec3_t pos, vec3_t fromDir) {
  ...
  lightLevel = VectorLength( ambient ) + (VectorLength( directed )*DotProduct(
  lightDir, fromDir ));
  */
  float lightLevel;
  // rwwFIXMEFIXME: ...this is evil. We can possibly read from the server BSP
  // data, or load the lightmap along with collision data and whatnot, but is it
  // worth it?
  lightLevel = 255;
...
int G_CheckAlertEvents(gentity_t *self, qboolean checkSight,
  ...
  // get sound event
  bestSoundEvent = G_CheckSoundEvents(self, maxHearDist, ignoreAlert,
  ...
  if (self->NPC) {
    ...
    bestSightEvent = G_CheckSightEvents(
        ...
        minAlertLevel); // FIXME: look at cg_view to get more accurate numbers?
  }
...
int NPC_CheckAlertEvents(qboolean checkSight, qboolean checkSound,
  ...
  return G_CheckAlertEvents(
      NPCS.NPC, checkSight, checkSound, NPCS.NPCInfo->stats.visrange,
  ...
  while (tr.fraction < 1.0 &&
    ...
    if (tr.entityNum < ENTITYNUM_WORLD) {
      if (&g_entities[tr.entityNum] != NULL &&
          (g_entities[tr.entityNum].r.svFlags &
           SVF_GLASS_BRUSH)) { // can see through glass, trace again, ignoring
                               // me
        trap->Trace(&tr, tr.endpos, NULL, NULL, end, tr.entityNum, MASK_OPAQUE,
                    qfalse, 0, 0);
        traceCount++;
        continue;
      }
