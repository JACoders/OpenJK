...
void NPC_Mark1_Part_Explode(gentity_t *self, int bolt) {
  if (bolt >= 0) {
    ...
    gi.G2API_GetBoltMatrix(self->ghoul2, self->playerModel, bolt, &boltMatrix,
...
void Mark1Dead_FireRocket(void) {
  ...
  gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->playerModel, NPC->genericBolt5,
                         &boltMatrix, NPC->currentAngles, NPC->currentOrigin,
                         (cg.time ? cg.time : level.time), NULL,
                         NPC->s.modelScale);
  ...
  gentity_t *missile =
      CreateMissile(muzzle1, muzzle_dir, BOWCASTER_VELOCITY, 10000, NPC);
...
void Mark1Dead_FireBlaster(void) {
  ...
  gi.G2API_GetBoltMatrix(NPC->ghoul2, NPC->playerModel, bolt, &boltMatrix,
                         NPC->currentAngles, NPC->currentOrigin,
                         (cg.time ? cg.time : level.time), NULL,
                         NPC->s.modelScale);
...
-------------------------
*/
void Mark1_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
  ...
  if (((anim==BOTH_DEATH1) || (anim==BOTH_DEATH2)) &&
  (self->client->ps.torsoAnimTimer==0)) {	// This is because self->health
  keeps getting zeroed out. HL_NONE acts as health in this case.
          ...
          if (self->locationDamage[HL_NONE] > 50)
          {
                  ...
                  // G_FreeEntity( self ); // Is this safe?  I can't see why
  we'd mark it nodraw and then just leave it around?? self->e_ThinkFunc =
  thinkF_G_FreeEntity; self->nextthink = level.time + FRAMETIME;
          }
          return;
  }
  */
  ...
  G_Sound(self, G_SoundIndex(
                    va("sound/chars/mark1/misc/death%d.wav", Q_irand(1, 2))));

  // Choose a death anim
  if (Q_irand(1, 10) > 5) {
    NPC_SetAnim(self, SETANIM_BOTH, BOTH_DEATH2,
...
void Mark1_dying(gentity_t *self) {
  ...
  if (self->client->ps.torsoAnimTimer > 0) {
    ...
    //); 		VectorMA( self->client->ps.velocity, -80, right,
    ...
    if (!gi.G2API_GetSurfaceRenderStatus(
            ...
            "l_arm")) // Is the blaster still on the model?
    {
      if (Q_irand(1, 5) == 1) {
        SaveNPCGlobals();
        SetNPCGlobals(self);
    ...
    if (!gi.G2API_GetSurfaceRenderStatus(
            &self->ghoul2[self->playerModel],
            "r_arm")) // Is the rocket still on the model?
    {
      if (Q_irand(1, 10) == 1) {
        SaveNPCGlobals();
        SetNPCGlobals(self);
...
-------------------------
*/
void NPC_Mark1_Pain(gentity_t *self, gentity_t *inflictor, gentity_t *other,
  ...
  // Hit in the left arm?
           ...
           (self->locationDamage[HL_ARM_LT] > LEFT_ARM_HEALTH)) {
    if (self->locationDamage[hitLoc] >= LEFT_ARM_HEALTH) // Blow it up?
    {
      ...
      gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel], "l_arm",
                               TURN_OFF);
    }
  }
  // Hit in the right arm?
  else if ((hitLoc == HL_ARM_RT) &&
           (self->locationDamage[HL_ARM_RT] > RIGHT_ARM_HEALTH)) // Blow it up?
  {
    if (self->locationDamage[hitLoc] >= RIGHT_ARM_HEALTH) {
      newBolt = gi.G2API_AddBolt(&self->ghoul2[self->playerModel], "*flash4");
      if (newBolt != -1) {
        //				G_PlayEffect( "small_chunks",
        //self->playerModel, self->genericBolt2, self->s.number);
        NPC_Mark1_Part_Explode(self, newBolt);
      }

      gi.G2API_SetSurfaceOnOff(&self->ghoul2[self->playerModel], "r_arm",
                               TURN_OFF);
    }
  }
  // Check ammo pods
    ...
    for (i = 0; i < 6; i++) {
      if ((hitLoc == HL_GENERIC1 + i) &&
          (self->locationDamage[HL_GENERIC1 + i] >
           AMMO_POD_HEALTH)) // Blow it up?
      {
  ...
  if ((gi.G2API_GetSurfaceRenderStatus(&self->ghoul2[self->playerModel],
                                       "l_arm")) &&
      (gi.G2API_GetSurfaceRenderStatus(&self->ghoul2[self->playerModel],
                                       "r_arm"))) {
...
void Mark1_FireBlaster(void) {
  ...
  if ((NPCInfo->localState <= LSTATE_FIRED0) ||
  ...
  if (TIMER_Done(NPC, "attackDelay")) // Attack?
  {
    ...
    if (chance == 1) {
      ...
      NPC->client->ps.torsoAnimTimer =
          0; // Just in case the firing anim is running.
    } else {
...
void Mark1_FireRocket(void) {
  ...
  gentity_t *missile =
      CreateMissile(muzzle1, forward, BOWCASTER_VELOCITY, 10000, NPC);
...
void Mark1_RocketAttack(qboolean advance) {
  ...
  {
    TIMER_Set(NPC, "attackDelay", Q_irand(1000, 3000));
    NPC_SetAnim(NPC, SETANIM_TORSO, BOTH_ATTACK2,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    Mark1_FireRocket();
  } else if (advance) {
    Mark1_Hunt();
...
void Mark1_AttackDecision(void) {
  ...
  if (TIMER_Done(NPC, "patrolNoise")) {
    if (TIMER_Done(NPC, "angerNoise")) {
      //			G_Sound( NPC,
      //G_SoundIndex(va("sound/chars/mark1/misc/talk%d.wav",	Q_irand(1,
      //4))));
  ...
  float distance = (int)DistanceHorizontalSquared(NPC->currentOrigin,
                                                  NPC->enemy->currentOrigin);
  distance_e distRate =
      (distance > MIN_MELEE_RANGE_SQR) ? DIST_LONG : DIST_MELEE;
  ...
  blasterTest =
      gi.G2API_GetSurfaceRenderStatus(&NPC->ghoul2[NPC->playerModel], "l_arm");
  rocketTest =
      gi.G2API_GetSurfaceRenderStatus(&NPC->ghoul2[NPC->playerModel], "r_arm");
...
void Mark1_Patrol(void) {
  ...
  if (!NPC->enemy) {
    ...
    //		if (TIMER_Done(NPC,"patrolNoise"))
    //		{
    //			G_Sound( NPC, G_SoundIndex(va("sound/chars/mark1/misc/talk%d.wav",
    //Q_irand(1, 4))));
    //
