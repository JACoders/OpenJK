...
static void R_AddWorldSurface(msurface_t *surf, const trRefEntity_t *entity, int entityNum, int dlightBits, int pshadowBits) {
	...
	R_AddDrawSurf(surf->data, entityNum, surf->shader, surf->fogIndex, dlightBits, isPostRenderEntity, surf->cubemapIndex);

	for (int i = 0, numSprites = surf->numSurfaceSprites; i < numSprites; ++i) {
		srfSprites_t *sprites = surf->surfaceSprites + i;
		R_AddDrawSurf((surfaceType_t *)sprites, entityNum, sprites->shader, surf->fogIndex, dlightBits, isPostRenderEntity, 0);
	}
}
...
void RE_SetRangedFog(float range) { tr.rangedFog = range; }
...
void R_AddWorldSurfaces(viewParms_t *viewParms, trRefdef_t *refdef) {
	...
	for (int i = 0; i < tr.world->numWorldSurfaces; i++) {
		...
		R_AddWorldSurface(tr.world->surfaces + i, nullptr, REFENTITYNUM_WORLD, tr.world->surfacesDlightBits[i], tr.world->surfacesPshadowBits[i]);
	}
	...
	for (int i = 0; i < tr.world->numMergedSurfaces; i++) {
		...
		R_AddWorldSurface(tr.world->mergedSurfaces + i, nullptr, REFENTITYNUM_WORLD, tr.world->mergedSurfacesDlightBits[i],
