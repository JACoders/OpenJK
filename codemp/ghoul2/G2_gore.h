...
public:
  ...
  // at some point I'll want to make VM callbacks in here. For now I am just
  // doing nothing.
...
#ifdef _DEBUG
  virtual void DebugLine(const vec3_t p1, const vec3_t p2, bool bbox) {
    assert(0);
  }
#endif
};
...
public:
  enum ERagPhase {
    ...
    RP_DEATH_COLLISION,
    RP_CORPSE_SHOT,
    RP_GET_PELVIS_OFFSET, // this actually does nothing but set the
                          // pelvisAnglesOffset, and pelvisPositionOffset
    RP_SET_PELVIS_OFFSET, // this actually does nothing but set the
                          // pelvisAnglesOffset, and pelvisPositionOffset
    RP_DISABLE_EFFECTORS  // this removes effectors given by the
                          // effectorsToTurnOff member
  };
  vec3_t angles;
  vec3_t position;
  vec3_t scale;
  vec3_t pelvisAnglesOffset;   // always set on return, an argument for
                               // RP_SET_PELVIS_OFFSET
  vec3_t pelvisPositionOffset; // always set on return, an argument for
                               // RP_SET_PELVIS_OFFSET

  float fImpactStrength; // should be applicable when RagPhase is
                         // RP_DEATH_COLLISION
  float fShotStrength; // should be applicable for setting velocity of corpse on
                       // shot (probably only on RP_CORPSE_SHOT)
  // CServerEntity *me;
  int me;

  // rww - we have convenient animation/frame access in the game, so just send
  // this info over from there.
  int startFrame;
  int endFrame;

  int collisionType; // 1 = from a fall, 0 from effectors, this will be going
                     // away soon, hence no enum

  qboolean CallRagDollBegin; // a return value, means that we are now begininng
                             // ragdoll and the NPC stuff needs to happen
  ...
  ERagEffector effectorsToTurnOff; // set this to an | of the above flags for a
