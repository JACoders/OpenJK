...
//---------------------------------------------------------
{
  if (!ent->count) {
    ...
    VectorSet(pos, ent->currentOrigin[0], ent->currentOrigin[1],
              ent->currentOrigin[2] + 8);

    ent->takedamage = qfalse; // don't allow double deaths!

    G_RadiusDamage(
        ent->currentOrigin, ent->owner, weaponData[WP_THERMAL].splashDamage,
        weaponData[WP_THERMAL].splashRadius, NULL, MOD_EXPLOSIVE_SPLASH);
...
//-------------------------------------------------------------------------------------------------------------
void thermal_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
...
//---------------------------------------------------------
{
  ...
  // Thermal detonators for the player do occasional radius checks and blow up
  // if there are entities in the blast radius
  //	This is done so that the main fire is actually useful as an attack.  We
  //explode anyway after delay expires.
  if (ent->delay > level.time) {
    //	Finally, we force it to bounce at least once before doing the special
    //checks, otherwise it's just too easy for the player?
    if (ent->has_bounced) {
      count =
          G_RadiusList(ent->currentOrigin, TD_TEST_RAD, ent, qtrue, ent_list);

      for (int i = 0; i < count; i++) {
        if (ent_list[i]->s.number == 0) {
          // avoid deliberately blowing up next to the player, no matter how
          // close any enemy is..
          //	...if the delay time expires though, there is no saving the
          //player...muwhaaa haa ha
          blow = qfalse;
          break;
        } else if (ent_list[i]->client && ent_list[i]->health > 0) {
          // sometimes the ent_list order changes, so we should make sure that
          // the player isn't anywhere in this list
          blow = qtrue;
        }
      }
    }
...
//---------------------------------------------------------
{
  ...
  if (ent->s.number != 0) {
    // If not the player, cut the damage a bit so we don't get pounded on so
    // much
  ...
  WP_TraceSetStart(ent, start, bolt->mins,
                   bolt->maxs); // make sure our start point isn't on the other
                                // side of a wall
  ...
  if (ent->health > 0) {
    ...
    if (ent->NPC && ent->enemy) { // FIXME: we're assuming he's actually facing
                                  // this direction...
      vec3_t target;
      ...
      target[0] += Q_flrand(-5, 5) +
                   (Q_flrand(-1.0f, 1.0f) * (6 - ent->NPC->currentAim) * 2);
      target[1] += Q_flrand(-5, 5) +
                   (Q_flrand(-1.0f, 1.0f) * (6 - ent->NPC->currentAim) * 2);
      target[2] += Q_flrand(-5, 5) +
                   (Q_flrand(-1.0f, 1.0f) * (6 - ent->NPC->currentAim) * 2);

      WP_LobFire(ent, start, target, bolt->mins, bolt->maxs, bolt->clipmask,
                 bolt->s.pos.trDelta, qtrue, ent->s.number,
                 ent->enemy->s.number);
    }
  }
