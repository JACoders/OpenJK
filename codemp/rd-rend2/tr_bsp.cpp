...
static void R_LoadLightmaps(world_t *worldData, lump_t *l, lump_t *surfs) {
	...
	if (numLightmaps <= 1) {
		...
		for (i = 0, surf = (dsurface_t *)(fileBase + surfs->fileofs); tr.worldDeluxeMapping && i < surfs->filelen / sizeof(dsurface_t); i++, surf++) {
			for (int j = 0; j < MAXLIGHTMAPS; j++) {
	...
	if (tr.worldInternalLightmapping) {
		for (i = 0; i < tr.numLightmaps; i++) {
			tr.lightmaps[i] = R_CreateImage(va("_lightmapatlas%d", i), NULL, tr.lightmapAtlasSize[0], tr.lightmapAtlasSize[1], IMGTYPE_COLORALPHA,
											IMGFLAG_NOLIGHTSCALE | IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, textureInternalFormat);

			if (tr.worldDeluxeMapping) {
				tr.deluxemaps[i] = R_CreateImage(va("_fatdeluxemap%d", i), NULL, tr.lightmapAtlasSize[0], tr.lightmapAtlasSize[1], IMGTYPE_DELUXE,
	...
	for (i = 0; i < numLightmaps; i++) {
		...
		// if (tr.worldLightmapping)
		{
			...
			if (foundLightmap) {
				...
				if (tr.worldInternalLightmapping)
					R_UpdateSubImage(tr.lightmaps[lightmapnum], image, xoff, yoff, lightmapWidth, lightmapHeight);
				else
					tr.lightmaps[i] = R_CreateImage(va("*lightmap%d", i), image, lightmapWidth, lightmapHeight, IMGTYPE_COLORALPHA,
													IMGFLAG_NOLIGHTSCALE | IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, textureInternalFormat);
			}
		...
		if (tr.worldDeluxeMapping && buf) {
			...
			if (tr.worldInternalLightmapping) {
				...
				tr.deluxemaps[i] = R_CreateImage(va("*deluxemap%d", i), image, tr.lightmapSize, tr.lightmapSize, IMGTYPE_DELUXE,
												 IMGFLAG_NOLIGHTSCALE | IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, 0);
			}
			...
			if (!tr.deluxemaps) {
				tr.deluxemaps = (image_t **)ri.Hunk_Alloc(tr.numLightmaps * sizeof(image_t *), h_low);
				if (tr.worldInternalLightmapping) {
					tr.deluxemaps[lightmapnum] = R_CreateImage(va("_fatdeluxemap%d", i), NULL, tr.lightmapAtlasSize[0], tr.lightmapAtlasSize[1], IMGTYPE_DELUXE,
															   IMGFLAG_NOLIGHTSCALE | IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, 0);
				}
			}

			if (tr.worldInternalLightmapping) {
				R_UpdateSubImage(tr.deluxemaps[lightmapnum], image, xoff, yoff, lightmapWidth, lightmapHeight);
			} else {
				tr.deluxemaps[i] = R_CreateImage(va("*deluxemap%d", i), image, lightmapWidth, lightmapHeight, IMGTYPE_DELUXE,
												 IMGFLAG_NOLIGHTSCALE | IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, 0);
			}
...
===============
*/
static void ParseFace(const world_t *worldData, dsurface_t *ds, drawVert_t *verts, packedTangentSpace_t *tangentSpace, float *hdrVertColors, msurface_t *surf,
					  int *indexes) {
	int i, j;
	srfBspSurface_t *cv;
	glIndex_t *tri;
...
===============
*/
static void ParseTriSurf(const world_t *worldData, dsurface_t *ds, drawVert_t *verts, packedTangentSpace_t *tangentSpace, float *hdrVertColors,
...
static void R_CreateWorldVBOs(world_t *worldData) {
	...
	for (firstSurf = lastSurf = surfacesSorted; firstSurf < &surfacesSorted[numSortedSurfaces]; firstSurf = lastSurf) {
		...
		while (currVboSize < maxVboSize && currIboSize < maxIboSize && lastSurf < &surfacesSorted[numSortedSurfaces]) {
			...
			if ((currVboSize != 0 && addVboSize + currVboSize > maxVboSize) || (currIboSize != 0 && addIboSize + currIboSize > maxIboSize))
				break;
...
static void R_LoadFogs(world_t *worldData, lump_t *l, lump_t *brushesLump, lump_t *sidesLump) {
	...
	for (i = 0; i < count; i++, fogs++) {
		...
		VectorSet4(out->color, shader->fogParms.color[0] * tr.identityLight, shader->fogParms.color[1] * tr.identityLight,
...
void R_LoadLightGrid(world_t *worldData, lump_t *l) {
	...
	for (i = 0; i < numGridDataElements; i++) {
		for (int j = 0; j < MAXLIGHTMAPS; j++) {
			R_ColorShiftLightingBytes(worldData->lightGridData[i].ambientLight[j], worldData->lightGridData[i].ambientLight[j]);
			R_ColorShiftLightingBytes(worldData->lightGridData[i].directLight[j], worldData->lightGridData[i].directLight[j]);
		}
	}
		...
		if (hdrLightGrid) {
			if (size != sizeof(float) * 6 * worldData->lightGridBounds[0] * worldData->lightGridBounds[1] * worldData->lightGridBounds[2]) {
				ri.Error(ERR_DROP, "Bad size for %s (%i, expected %i)!", filename, size,
						 (int)(sizeof(float)) * 6 * worldData->lightGridBounds[0] * worldData->lightGridBounds[1] * worldData->lightGridBounds[2]);
			}
...
static void R_CalcVertexLightDirs(world_t *worldData) {
	...
	for (k = 0, surface = &worldData->surfaces[0]; k < worldData->numsurfaces /* worldData->numWorldSurfaces */; k++, surface++) {
		...
		case SF_TRIANGLES:
			for (i = 0; i < bspSurf->numVerts; i++)
				R_LightDirForPoint(bspSurf->verts[i].xyz, bspSurf->verts[i].lightdir, bspSurf->verts[i].normal, worldData);
...
static uint32_t R_CountSurfaceSprites(const srfBspSurface_t *bspSurf, float density, const shaderStage_t *stage) {
...
static int R_CreateSurfaceSpritesVertexData(const srfBspSurface_t *bspSurf, float density, const shaderStage_t *stage, std::vector<sprite_t> *sprites) {
	const srfVert_t *verts = bspSurf->verts;
	const glIndex_t *indexes = bspSurf->indexes;
	...
	bool vertexLit =
		(stage->rgbGen == CGEN_VERTEX || stage->rgbGen == CGEN_EXACT_VERTEX || stage->rgbGen == CGEN_VERTEX_LIT || stage->rgbGen == CGEN_EXACT_VERTEX_LIT);
...
static void R_GenerateSurfaceSprites(const srfBspSurface_t *bspSurf, const shader_t *shader, const shaderStage_t *stage, const int fogIndex, srfSprites_t *out,
...
static void R_GenerateSurfaceSprites(const world_t *world, int worldIndex) {
	...
	qglBufferData(GL_UNIFORM_BUFFER, numSpriteStages * spriteAlignedBlockSize, nullptr, GL_STATIC_DRAW);
	...
	for (int i = 0; i < tr.numShaders; i++) {
		...
		for (int j = 0, numStages = shader->numUnfoggedPasses; j < numStages; ++j) {
			...
			qglBufferSubData(GL_UNIFORM_BUFFER, ss->spriteUboOffset, sizeof(SurfaceSpriteBlock), &surfaceSpriteBlock);
...
world_t *R_LoadBSP(const char *name, int *bspIndex) {
	...
	if (bspIndex == nullptr)
		R_LoadWeatherZones(worldData, &header->lumps[LUMP_BRUSHES], &header->lumps[LUMP_BRUSHSIDES]);
	...
	if (r_cubeMapping->integer && bspIndex == nullptr) {
		...
		const char *cubemapEntities[numCubemapEntities] = {
			"misc_cubemap", "info_player_deathmatch", "info_player_start", "info_player_duel", "info_player_intermission",
		};
