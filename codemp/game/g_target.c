...
void Use_Target_Delay(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  if (ent->nextthink > level.time &&
      (ent->spawnflags & 1)) { // Leave me alone, I am thinking.
    return;
  }
...
ago If "private", only the activator gets the message.  If no checks, all
...
void Use_Target_Print(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  ...
  if (ent->genericValue15 > level.time) {
    ...
    Com_Error(ERR_DROP, "target_print used in quick succession, fix it! See "
  ...
  if ((ent->spawnflags & 4)) { // private, to one client only
    ...
    if (activator &&
        activator
            ->client) { // make sure there's a valid client ent to send it to
      if (ent->message[0] == '@' && ent->message[1] != '@') {
        trap->SendServerCommand(activator - g_entities,
    ...
    // NOTE: change in functionality - if there *is* no valid client ent, it
    // won't send it to anyone at all
    return;
  }
...
void SP_target_speaker(gentity_t *ent) {
  ...
  if (!G_SpawnString("noise", "NOSOUND", &s)) {
    trap->Error(ERR_DROP, "target_speaker without a noise key at %s",
                vtos(ent->s.origin));
  }
...
void target_laser_think(gentity_t *self) {
  ...
  trap->Trace(&tr, self->s.origin, NULL, NULL, end, self->s.number,
              CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_CORPSE, qfalse, 0, 0);
...
void SP_target_location(gentity_t *self) {
  if (self->targetname && self->targetname[0]) {
    ...
    if (level.locations.num >= MAX_LOCATIONS) {
      if (!didwarn) {
        trap->Print("Maximum target_locations hit (%d)! Remaining locations "
                    "will be removed.\n",
                    MAX_LOCATIONS);
        didwarn = qtrue;
      }
    ...
    VectorCopy(self->s.origin,
               level.locations.data[level.locations.num].origin);
    ...
    level.locations.data[level.locations.num].count =
        Com_Clampi(0, 7, self->count);
...
void target_random_use(gentity_t *self, gentity_t *other,
  ...
  while ((t = G_Find(t, FOFS(targetname), self->target)) != NULL) {
    ...
    if (t == self) {
      //				trap->Printf ("WARNING: Entity used
      //itself.\n");
  ...
  if (self->behaviorSet[BSET_USE]) {
    if (self->spawnflags & 1) {
      ...
      if (!trap->ICARUS_IsInitialized(
              self->s.number)) { // Need to be initialized through ICARUS
        ...
        if (trap->ICARUS_ValidEnt((sharedEntity_t *)self->activator)) {
          ...
          if (developer.integer) {
            Com_Printf("target_scriptrunner tried to run on invalid ICARUS "
                       "activator!\n");
          }
          return;
        }
...
void G_SetActiveState(char *targetstring, qboolean actState) {
  ...
  while (NULL != (target = G_Find(target, FOFS(targetname), targetstring))) {
    target->flags = actState ? (target->flags & ~FL_INACTIVE)
...
void SP_target_play_music(gentity_t *self) {
  ...
  if (!G_SpawnString("music", "", &s)) {
    trap->Error(ERR_DROP, "target_play_music without a music key at %s",
                vtos(self->s.origin));
  }
