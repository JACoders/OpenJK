...
void Jedi_CheckCloak(void) {
	if (NPC && NPC->client && NPC->client->NPC_class == CLASS_SHADOWTROOPER) {
		if (NPC->client->ps.saberActive || NPC->health <= 0 || NPC->client->ps.saberInFlight || (NPC->client->ps.eFlags & EF_FORCE_GRIPPED) ||
			...
			Jedi_Decloak(NPC);
		} else if (NPC->health > 0 && !NPC->client->ps.saberInFlight && !(NPC->client->ps.eFlags & EF_FORCE_GRIPPED) &&
...
static void Jedi_Aggression(gentity_t *self, int change) {
	...
	if (self->client->playerTeam == TEAM_PLAYER) { // good guys are less aggressive
		upper_threshold = 7;
		lower_threshold = 1;
...
static qboolean Jedi_BattleTaunt(void) {
	if (TIMER_Done(NPC, "chatter") && !Q_irand(0, 3) && NPCInfo->blockedSpeechDebounceTime < level.time &&
		...
		int event = -1;
		if (NPC->client->playerTeam == TEAM_PLAYER && NPC->enemy && NPC->enemy->client &&
...
static qboolean Jedi_ClearPathToSpot(vec3_t dest, int impactEntNum) {
	...
	for (float i = NPC->maxs[0] * 2; i < dist;
		 i += NPC->maxs[0] * 2) { // FIXME: does this check the last spot, too?  We're assuming that should be okay since the enemy is there?
...
qboolean NPC_MoveDirClear(int forwardmove, int rightmove, qboolean reset) {
	...
	*/
	...
	if (NPCInfo->goalEntity) {
		if (NPCInfo->goalEntity->currentOrigin[2] < NPC->currentOrigin[2]) { // goal is below me, okay to step off at least that far plus stepheight
...
static void Jedi_CombatDistance(int enemy_dist) { // FIXME: for many of these checks, what we really want is horizontal distance to enemy
	if (NPC->client->ps.forcePowersActive & (1 << FP_GRIP) && NPC->client->ps.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1) { // when gripping, don't move
		return;
	...
	if (NPC->client->ps.saberInFlight && !PM_SaberInBrokenParry(NPC->client->ps.saberMove) &&
		...
		} else if (NPC->client->ps.torsoAnim == BOTH_GESTURE1 && NPC->client->ps.torsoAnimTimer < 2000) { // we're almost done with our special taunt
		...
		if (enemy_dist < forcePushPullRadius[testlevel] - 16) {																	 // close enough to push
			if (InFront(NPC->enemy->currentOrigin, NPC->client->renderInfo.eyePoint, NPC->client->renderInfo.eyeAngles, 0.6f)) { // knock it down
				WP_KnockdownTurret(NPC, NPC->enemy);
				// do the forcethrow call just for effect
				ForceThrow(NPC, qfalse);
			}
		}
	} else if (enemy_dist <= 64 && (NPCInfo->scriptFlags & SCF_DONT_FIRE)) { // can't use saber and they're in striking range
	...
	*/
	...
	{						  // we're out of striking range and we are allowed to attack
		...
		if (NPC->enemy && NPC->enemy->client && (NPC->enemy->client->ps.eFlags & EF_FORCE_GRIPPED)) { // They're being gripped, rush them!
			...
			if (NPCInfo->rank >= RANK_LT_JG && !Q_irand(0, 5) && !(NPC->client->ps.forcePowersActive & (1 << FP_SPEED)) &&
		...
		{																						 // They're throwing their saber, grip them!
			...
			if (chanceScale && (enemy_dist > Q_irand(100, 200) || (NPCInfo->scriptFlags & SCF_DONT_FIRE)) && enemy_dist < 500 &&
				...
				if ((NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) && !Q_irand(0, 1)) {
					if (WP_ForcePowerAvailable(NPC, FP_PULL, 0) && !Q_irand(0, 2)) {
						...
						TIMER_Set(NPC, "gripping", 3000);
						TIMER_Set(NPC, "attackDelay", 3000);
					} else if (WP_ForcePowerAvailable(NPC, FP_SABERTHROW, 0) && !(NPC->client->ps.forcePowersActive & (1 << FP_SPEED)) &&
							   !(NPC->client->ps.saberEventFlags & SEF_INWATER)) // saber not in water
					{															 // throw saber
						ucmd.buttons |= BUTTON_ALT_ATTACK;
					}
				} else if (NPCInfo->rank >= RANK_LT_JG && !(NPC->client->ps.forcePowersActive & (1 << FP_SPEED)) &&
						   !(NPC->client->ps.saberEventFlags & SEF_INWATER)) // saber not in water
				{															 // throw saber
					ucmd.buttons |= BUTTON_ALT_ATTACK;
				}
...
qboolean Jedi_DodgeEvasion(gentity_t *self, gentity_t *shooter, trace_t *tr, int hitLoc) {
	...
	if (self->client->ps.pm_time && (self->client->ps.pm_flags & PMF_TIME_KNOCKBACK)) { // in some effect that stops me from moving on my own
		return qfalse;
	}

	if (self->enemy == shooter) { // FIXME: make it so that we are better able to dodge shots from my current enemy
	}
	if (self->s.number) { // if an NPC, check game skill setting
		...
		*/
		...
		if (!WP_ForcePowerUsable(self, FP_SPEED, 0)) { // make sure we have it and have enough force power
			return qfalse;
		}
		// check force speed power level to determine if I should be able to dodge it
		if (Q_irand(1, 10) > self->client->ps.forcePowerLevel[FP_SPEED]) { // more likely to fail on lower force speed level
			return qfalse;
		}
	}
		...
		if (tr) {
			for (int z = 0; z < MAX_G2_COLLISIONS; z++) {
				...
				G_GetHitLocFromSurfName(&g_entities[coll.mEntityNum],
										gi.G2API_GetSurfaceName(&g_entities[coll.mEntityNum].ghoul2[coll.mModelIndex], coll.mSurfaceIndex), &hitLoc,
										coll.mCollisionPosition, NULL, NULL, MOD_UNKNOWN);
				// only want the first
				break;
			}
		}
...
evasionType_t Jedi_CheckFlipEvasions(gentity_t *self, float rightdot, float zdiff) {
	...
	if (self->client->ps.legsAnim == BOTH_WALL_RUN_LEFT || self->client->ps.legsAnim == BOTH_WALL_RUN_RIGHT) { // already running on a wall
		...
		if (self->client->ps.legsAnim == BOTH_WALL_RUN_LEFT && rightdot < 0) { // I'm running on a wall to my left and the attack is on the left
			if (animLength - self->client->ps.legsAnimTimer > 400 && self->client->ps.legsAnimTimer > 400) { // not at the beginning or end of the anim
		...
		if (anim != -1) { // flip off the wall!
			...
			return EVASION_OTHER;
			   ...
			   && (self->NPC->rank == RANK_CREWMAN || self->NPC->rank >= RANK_LT) && Q_irand(0, 1) && !PM_InRoll(&self->client->ps) &&
			   !PM_InKnockDown(&self->client->ps) && !PM_SaberInSpecialAttack(self->client->ps.torsoAnim)) {
		vec3_t fwd, right, traceto, mins = {self->mins[0], self->mins[1], 0}, maxs = {self->maxs[0], self->maxs[1], 24},
...
qboolean Jedi_SaberBusy(gentity_t *self) {
	if (self->client->ps.torsoAnimTimer > 300 &&
		((PM_SaberInAttack(self->client->ps.saberMove) && self->client->ps.saberAnimLevel == FORCE_LEVEL_3) ||
		 ...
		 || PM_FlippingAnim(self->client->ps.torsoAnim) || PM_RollingAnim(self->client->ps.torsoAnim))) { // my saber is not in a parrying position
		return qtrue;
	}
	return qfalse;
}
...
evasionType_t Jedi_SaberBlockGo(gentity_t *self, usercmd_t *cmd, vec3_t pHitloc, vec3_t phitDir, gentity_t *incoming, float dist = 0.0f) {
	...
	if ((dist > 16 && (Q_irand(0, 2) || saberBusy)) || self->client->ps.saberInFlight ||
		!self->client->ps.saberActive) { // either it will miss by a bit (and 25% chance) OR our saber is not in-hand OR saber is off
		if (self->NPC && (self->NPC->rank == RANK_CREWMAN || self->NPC->rank >= RANK_LT_JG)) {					// acrobat or fencer or above
			if (self->client->ps.groundEntityNum != ENTITYNUM_NONE &&											// on the ground
				...
				&& (self->client->ps.saberInFlight || (!PM_SaberInAttack(self->client->ps.saberMove)			// not attacking
		...
		if (self->client->ps.groundEntityNum == ENTITYNUM_NONE) {			  // already in air, duck to pull up legs
			...
			if (self->NPC && (self->NPC->rank == RANK_CREWMAN || self->NPC->rank > RANK_LT_JG) &&
				...
				if (self->client->NPC_class == CLASS_TAVION && !incoming && self->client->ps.groundEntityNum < ENTITYNUM_NONE && !Q_irand(0, 2)) {
					if (!PM_SaberInAttack(self->client->ps.saberMove) && !PM_SaberInStart(self->client->ps.saberMove) && !PM_InRoll(&self->client->ps) &&
						!PM_InKnockDown(&self->client->ps) && !PM_SaberInSpecialAttack(self->client->ps.torsoAnim)) { // do the butterfly!
						int butterflyAnim;
						if (Q_irand(0, 1)) {
...
static qboolean Jedi_SaberBlock(void) {
	...
	*/
	...
	if (NPC->client->ps.forcePowerDebounce[FP_SABER_DEFENSE] > level.time) { // can't move the saber to another position yet
		return qfalse;
	}
...
static void Jedi_EvasionSaber(vec3_t enemy_movedir, float enemy_dist, vec3_t enemy_dir) {
	...
	if ((NPC->enemy->client->ps.forcePowersActive & (1 << FP_LIGHTNING))) { // enemy is shooting lightning
		enemy_attacking = qtrue;
	...
	if (Q_irand(0, 100) < evasionChance) { // check to see if he's coming at me
		...
		if (Q_flrand(0.25, 1) < facingAmt) { // coming at/facing me!
			int whichDefense = 0;
			if (NPC->client->ps.weaponTime || NPC->client->ps.saberInFlight) { // I'm attacking or recovering from a parry, can only try to strafe/jump right
																			   // now
			...
			case 3:
				...
				if ((NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) &&
					TIMER_Done(NPC, "parryTime")) { // FIXME: check forcePushRadius[NPC->client->ps.forcePowerLevel[FP_PUSH]]
					ForceThrow(NPC, qfalse);
				}
				break;
			case 4:
			...
			default:
				...
				if (!Q_irand(0, 5) || !Jedi_Strafe(300, 1000, 0, 1000, qfalse)) { // certain chance they will pick an alternative evasion
					...
					if (shooting_lightning || throwing_saber || enemy_dist < 80) {
						...
						if (shooting_lightning || (!Q_irand(0, 2) && NPCInfo->stats.aggression < 4 && TIMER_Done(NPC, "parryTime"))) {
							if ((NPCInfo->rank == RANK_ENSIGN || NPCInfo->rank > RANK_LT_JG) && !shooting_lightning &&
								Q_irand(0, 2)) { // FIXME: check forcePushRadius[NPC->client->ps.forcePowerLevel[FP_PUSH]]
								ForceThrow(NPC, qfalse);
					...
					if (!(NPCInfo->scriptFlags & SCF_NO_ACROBATICS) && (NPCInfo->rank == RANK_CREWMAN || NPCInfo->rank > RANK_LT_JG) &&
...
gentity_t *Jedi_FindEnemyInCone(gentity_t *self, gentity_t *fallback, float minDot) {
	...
	for (e = 0; e < numListedEntities; e++) {
		...
		if (!gi.inPVS(check->currentOrigin, self->currentOrigin)) { // can't potentially see them
			continue;
		}
...
static void Jedi_FaceEnemy(qboolean doPitch) {
	...
	if (NPC->client->ps.forcePowersActive & (1 << FP_GRIP) && NPC->client->ps.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1) { // don't update?
...
static void Jedi_CombatTimersUpdate(int enemy_dist) {
	if (TIMER_Done(NPC, "roamTime")) {
		...
		if (NPC->enemy && NPC->enemy->client) {
			...
			case WP_SABER:
				...
				if (!NPC->enemy->client->ps.saberActive) { // fool!  Standing around unarmed, charge!
	...
	if (NPC->client->ps.saberEventFlags) { // some kind of saber combat event is still pending
		...
		if (NPC->client->ps.saberEventFlags & SEF_PARRIED) { // parried
			...
			*/
			if (NPC->enemy && (!NPC->enemy->client || PM_SaberInKnockaway(NPC->enemy->client->ps.saberMove))) { // advance!
	...
	if (enemy_dist >= 64) { // FIXME: only do this if standing still?
		...
		if (Q_irand(2, chance) < NPCInfo->stats.aggression) {
			if (TIMER_Done(NPC, "chatter")) { // FIXME: add more taunt behaviors
				...
				if (enemy_dist > 200 && NPC->client->ps.saberActive && !Q_irand(0, 5)) { // taunt even more, turn off the saber
...
static qboolean Jedi_AttackDecide(int enemy_dist) {
	...
	if (NPC->client->ps.saberEventFlags & SEF_LOCK_WON) { // we won a saber lock, press the advantage with an attack!
		int chance = 0;
		if (NPC->client->NPC_class == CLASS_DESANN || NPC->client->NPC_class == CLASS_LUKE) { // desann and luke
			chance = 20;
...
static qboolean Jedi_TryJump(gentity_t *goal) { // FIXME: never does a simple short, regular jump...
	...
	if (TIMER_Done(NPC, "jumpChaseDebounce")) {
		if ((!goal->client || goal->client->ps.groundEntityNum != ENTITYNUM_NONE)) {
			if (!PM_InKnockDown(&NPC->client->ps) && !PM_InRoll(&NPC->client->ps)) { // enemy is on terra firma
				vec3_t goal_diff;
				float goal_z_diff;
							...
							if (Jedi_Jump(dest, goal->s.number)) {
								...
								*/
								{ // FIXME: make this a function call
									...
									if (NPCInfo->rank != RANK_CREWMAN && NPCInfo->rank <= RANK_LT_JG) { // can't do acrobatics
		...
		if (NPC->enemy->enemy && NPC->enemy->enemy == NPC) { // enemy is mad at *me*
			if (NPC->enemy->client->ps.legsAnim == BOTH_JUMPFLIPSLASHDOWN1 ||
				...
				if (enemy_dist < 256 && enemy_dist > 64) {															// close
					if (!InFront(NPC->currentOrigin, NPC->enemy->currentOrigin, NPC->enemy->currentAngles, 0.0f)) { // behind him
...
static void Jedi_CheckJumps(void) {
	...
	VectorCopy(trace.endpos, bottom);
	if (bottom[2] > NPC->currentOrigin[2]) { // only care about dist down from current height or lower
		bottom[2] = NPC->currentOrigin[2];
	} else if (NPC->currentOrigin[2] - bottom[2] > 400) { // whoa, long drop, don't do it!
	...
	if (trace.allsolid || trace.startsolid || trace.fraction < 1.0f) { // hit ground!
		if (trace.entityNum < ENTITYNUM_WORLD) {					   // landed on an ent
			...
			if (groundEnt->svFlags & SVF_GLASS_BRUSH) { // don't land on breakable glass!
				goto jump_unsafe;
				return;
			}
		}
...
static void Jedi_Combat(void) {
	...
	if (!(NPC->client->ps.forcePowersActive & (1 << FP_GRIP)) || NPC->client->ps.forcePowerLevel[FP_GRIP] < FORCE_LEVEL_2) { // not gripping
	...
	if (!NPC->client->ps.saberInFlight && (!(NPC->client->ps.forcePowersActive & (1 << FP_GRIP)) ||
										   NPC->client->ps.forcePowerLevel[FP_GRIP] < FORCE_LEVEL_2)) { // not throwing saber or using force grip
...
qboolean Jedi_CheckAmbushPlayer(void) {
	...
	if (NPC->client->ps.powerups[PW_CLOAKED] ||
		g_crosshairEntNum != NPC->s.number) { // if I'm not cloaked and the player's crosshair is on me, I will wake up, otherwise do this stuff down here...
		if (!gi.inPVS(player->currentOrigin, NPC->currentOrigin)) { // must be in same room
			return qfalse;
		} else {
			if (!NPC->client->ps.powerups[PW_CLOAKED]) {
		...
		if ((target_dist = DistanceHorizontalSquared(player->currentOrigin, NPC->currentOrigin)) > 4096) { // closer than 64 - always ambush
	...
	{														// look for enemies
		...
		for (int i = 0; i < ENTITYNUM_WORLD; i++) {
			...
			if (enemy && enemy->client && NPC_ValidEnemy(enemy) && enemy->client->playerTeam == NPC->client->enemyTeam) {
				if (gi.inPVS(NPC->currentOrigin, enemy->currentOrigin)) { // we could potentially see him
					...
					if (enemy->s.number == 0 || enemy_dist < best_enemy_dist) {
						...
						if (enemy_dist < (220 * 220) || (NPCInfo->investigateCount >= 3 && NPC->client->ps.saberActive)) {
							...
							break;
								   ...
								   enemy->client->ps.saberActive) { // threw his saber, see if it's heading toward me and close enough to consider a threat
							float saberDist;
							vec3_t saberDir2Me;
							vec3_t saberMoveDir;
		...
		if (!NPC->enemy) { // still not mad
			if (!best_enemy) {
				...
				if (NPC_ClearLOS(best_enemy)) { // we have a clear (of architecture) LOS to him
...
void NPC_BSJedi_FollowLeader(void) {
	...
	if (NPCInfo->goalEntity) {
		...
		if (!NAV_CheckAhead(NPC, NPCInfo->goalEntity->currentOrigin, trace, (NPC->clipmask & ~CONTENTS_BODY) | CONTENTS_BOTCLIP)) { // can't get straight to him
			if (NPC_ClearLOS(NPCInfo->goalEntity) && NPC_FaceEntity(NPCInfo->goalEntity, qtrue)) {									// no line of sight
...
static void Jedi_Attack(void) {
	...
	if (NPC->enemy) {
		...
		{																								   // my enemy is dead and I killed him
			...
			if (NPC->client->ps.saberActive ||
				...
				if (!NPC->client->ps.saberActive && !NPC->client->ps.saberInFlight) { // turned off saber (in hand), gloat
			...
			if (NPC->client->ps.saberActive || NPC->client->ps.saberInFlight || !TIMER_Done(NPC, "gloatTime")) { // keep walking
	...
	if (!(NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) { // this is really stupid, but okay...
		ucmd.forwardmove = 0;
		ucmd.rightmove = 0;
		if (ucmd.upmove > 0) {
			ucmd.upmove = 0;
	...
	if (ucmd.buttons & BUTTON_ATTACK && NPC->client->playerTeam == TEAM_ENEMY) {
		if (Q_irand(0, NPC->client->ps.saberAnimLevel) > 0 && Q_irand(0, NPC->max_health + 10) > NPC->health &&
	...
	if (NPC->client->NPC_class == CLASS_TAVION ||
		...
		if (NPC->enemy && !NPC->enemy->s.number && NPC->enemy->client && (NPC->enemy->client->ps.forcePowersActive & (1 << FP_SPEED)) &&
