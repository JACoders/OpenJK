...
const int forcePowerMinRank[NUM_FORCE_POWER_LEVELS]
                           [NUM_FORCE_POWERS] = // 0 == neutral
    {{
         999, // FP_HEAL,//instant
...
void WP_InitForcePowers(gentity_t *ent) {
  ...
  char userinfo[MAX_INFO_STRING], forcePowers[DEFAULT_FORCEPOWERS_LEN + 1],
      readBuf[DEFAULT_FORCEPOWERS_LEN + 1];

  // if server has no max rank, default to max (50)
  if (g_maxForceRank.integer <= 0 ||
      g_maxForceRank.integer >= NUM_FORCE_MASTERY_LEVELS) {
  ...
  // so that the client configstring is already modified with this when we need
  // it
  if (!speedLoopSound)
  ...
  // if in siege, then use the powers for this class, and skip all this
  // nonsense.
  if (level.gametype == GT_SIEGE && ent->client->siegeClass != -1) {
    for (i = 0; i < NUM_FORCE_POWERS; i++) {
      ent->client->ps.fd.forcePowerLevel[i] =
          bgSiegeClasses[ent->client->siegeClass].forcePowerLevels[i];
  ...
  if (ent->s.eType == ET_NPC && ent->s.number >= MAX_CLIENTS)
    Q_strncpyz(userinfo, "forcepowers\\7-1-333003000313003120",
               sizeof(userinfo));
  else
    trap->GetUserinfo(ent->s.number, userinfo, sizeof(userinfo));

  Q_strncpyz(forcePowers, Info_ValueForKey(userinfo, "forcepowers"),
             sizeof(forcePowers));
  ...
  // rww - parse through the string manually and eat out all the appropriate
  // data
  i = 0;
  i_r = 0;
  ...
  if (level.gametype != GT_SIEGE && (ent->r.svFlags & SVF_BOT) &&
      botstates[ent->s.number]) {
  ...
  if (ent->s.eType == ET_NPC)
    ...
    if (warnClient || !ent->client->sess.setForce) {
      // the client's rank is too high for the server and has been autocapped,
      // so tell them
      if (level.gametype != GT_HOLOCRON && level.gametype != GT_JEDIMASTER) {
        ...
        if (!(ent->r.svFlags & SVF_BOT) && ent->s.eType != ET_NPC) {
          if (!g_teamAutoJoin.integer) {
            // make them a spectator so they can set their powerups up without
            // being bothered.
            ...
            trap->SendServerCommand(ent - g_entities,
                                    "spc"); // Fire up the profile menu
          }
        }

        // event isn't very reliable, I made it a string. This way I can send it
        // to just one client also,
        //	as opposed to making a broadcast event.
        trap->SendServerCommand(ent->s.number,
                                va("nfr %i %i %i", g_maxForceRank.integer, 1,
                                   ent->client->sess.sessionTeam));
        // arg1 is new max rank, arg2 is non-0 if force menu should be shown,
        // arg3 is the current team
      }
      ent->client->sess.setForce = qtrue;
    }
    ...
    if (!didEvent)
      trap->SendServerCommand(ent->s.number,
                              va("nfr %i %i %i", g_maxForceRank.integer, 0,
                                 ent->client->sess.sessionTeam));

    // the server has one or more force powers disabled and the client is using
    // them in his config
    if (warnClientLimit)
      trap->SendServerCommand(
          ent - g_entities,
          va("print \"The server has one or more force powers that you have "
             "chosen disabled.\nYou will not be able to use the disable force "
             "power(s) while playing on this server.\n\""));
  }
  ...
  for (i = 0; i < NUM_FORCE_POWERS; i++) {
    if ((ent->client->ps.fd.forcePowersKnown & (1 << i)) &&
        !ent->client->ps.fd.forcePowerLevel[i])
      ent->client->ps.fd.forcePowersKnown &= ~(1 << i);
    else if (i != FP_LEVITATION && i != FP_SABER_OFFENSE &&
             i != FP_SABER_DEFENSE && i != FP_SABERTHROW)
      lastFPKnown = i;
  }
  ...
  if (!(ent->client->ps.fd.forcePowersKnown &
        (1 << ent->client->ps.fd.forcePowerSelected))) {
    if (lastFPKnown != -1)
  ...
  for (/*i=0*/; i < NUM_FORCE_POWERS; i++)
    ent->client->ps.fd.forcePowerBaseLevel[i] =
        ent->client->ps.fd.forcePowerLevel[i];
...
void WP_SpawnInitForcePowers(gentity_t *ent) {
  ...
  if (level.gametype == GT_HOLOCRON) {
    ...
    if (HasSetSaberOnly()) {
      ...
      if (ent->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] <
          FORCE_LEVEL_1) {
  ...
  while (i < NUM_FORCE_POWERS) {
    if ((ent->client->ps.fd.forcePowersKnown & (1 << i)) &&
        !ent->client->ps.fd
             .forcePowerLevel[i]) { // make sure all known powers are cleared if
                                    // we have level 0 in them
      ent->client->ps.fd.forcePowersKnown &= ~(1 << i);
    }
  ...
  if (level.gametype == GT_SIEGE &&
      ent->client->siegeClass != -1) { // Then use the powers for this class.
    i = 0;
...
int ForcePowerUsableOn(gentity_t *attacker, gentity_t *other,
  ...
  if (other && other->client &&
      BG_HasYsalamiri(level.gametype, &other->client->ps)) {
    return 0;
  }

  if (attacker && attacker->client &&
      !BG_CanUseFPNow(level.gametype, &attacker->client->ps, level.time,
                      forcePower)) {
    return 0;
  }

  // Dueling fighters cannot use force powers on others, with the exception of
  // force push when locked with each other
  ...
  if (forcePower == FP_GRIP) {
    if (other && other->client &&
        (other->client->ps.fd.forcePowersActive &
         (1 << FP_ABSORB))) { // don't allow gripping to begin with if they are
                              // absorbing
      // play sound indicating that attack was absorbed
      if (other->client->forcePowerSoundDebounce < level.time) {
        gentity_t *abSound =
            G_PreDefSound(other->client->ps.origin, PDSOUND_ABSORBHIT);
      ...
      return 0;
               ...
               BG_SaberInSpecial(
                   other->client->ps
                       .saberMove)) { // don't grip person while they are in a
                                      // special or some really bad things can
                                      // happen.
      return 0;
    }
  }
  ...
  if (other && other->client && other->s.eType == ET_NPC &&
      other->s.NPC_class ==
          CLASS_VEHICLE) { // can't use the force on vehicles.. except lightning
  ...
  if (other && other->client && other->s.eType == ET_NPC &&
      level.gametype ==
          GT_SIEGE) { // can't use powers at all on npc's normally in siege...
    return 0;
  }
...
qboolean WP_ForcePowerAvailable(gentity_t *self, forcePowers_t forcePower,
  ...
  int drain =
      overrideAmt
          ? overrideAmt
          : forcePowerNeeded[self->client->ps.fd.forcePowerLevel[forcePower]]
                            [forcePower];

  if (self->client->ps.fd.forcePowersActive &
      (1 << forcePower)) { // we're probably going to deactivate it..
    return qtrue;
  }
  ...
  if ((forcePower == FP_DRAIN || forcePower == FP_LIGHTNING) &&
      self->client->ps.fd.forcePower >=
          25) { // it's ok then, drain/lightning are actually duration
    return qtrue;
  }
...
qboolean WP_ForcePowerInUse(gentity_t *self, forcePowers_t forcePower) {
  if ((self->client->ps.fd.forcePowersActive &
       (1 << forcePower))) { // already using this power
    return qtrue;
  }
...
qboolean WP_ForcePowerUsable(gentity_t *self, forcePowers_t forcePower) {
  ...
  if (self->client->ps.pm_flags &
      PMF_FOLLOW) { // specs can't use powers through people
    return qfalse;
  }
  ...
  if (!BG_CanUseFPNow(level.gametype, &self->client->ps, level.time,
                      forcePower)) {
    return qfalse;
  }

  if (!(self->client->ps.fd.forcePowersKnown &
        (1 << forcePower))) { // don't know this power
    return qfalse;
  }
  ...
  if (g_debugMelee.integer) {
    if ((self->client->ps.pm_flags &
         PMF_STUCK_TO_WALL)) { // no offensive force powers when stuck to wall
      switch (forcePower) {
      case FP_GRIP:
      case FP_LIGHTNING:
      case FP_DRAIN:
  ...
  if (!self->client->ps.saberHolstered) {
    ...
    if ((self->client->saber[0].saberFlags & SFL_TWO_HANDED) ||
        self->client->saber[0]
            .model[0]) { // this saber requires the use of two hands OR our
                         // other hand is using an active saber too
      if ((self->client->saber[0].forceRestrictions &
           (1 << forcePower))) { // this power is verboten when using this saber
        return qfalse;
      }
    }
    ...
    if (self->client->saber[0].model[0]) { // both sabers on
      ...
      if ((self->client->saber[1].forceRestrictions &
           (1 << forcePower))) { // this power is verboten when using this saber
        return qfalse;
      }
    }
...
int WP_AbsorbConversion(gentity_t *attacked, int atdAbsLevel,
                        gentity_t *attacker, int atPower, int atPowerLevel,
                        int atForceSpent) {
  int getLevel = 0;
  int addTot = 0;
  gentity_t *abSound;
  ...
  if (!(attacked->client->ps.fd.forcePowersActive &
        (1 << FP_ABSORB))) { // absorb is not active
    return -1;
  }
  ...
  // let the attacker absorb an amount of force used in this attack based on his
  // level of absorb
  addTot =
      (atForceSpent / 3) * attacked->client->ps.fd.forcePowerLevel[FP_ABSORB];

  if (addTot < 1 && atForceSpent >= 1) {
    addTot = 1;
  }
  attacked->client->ps.fd.forcePower += addTot;
  if (attacked->client->ps.fd.forcePower >
      attacked->client->ps.fd.forcePowerMax) {
...
void WP_ForcePowerRegenerate(gentity_t *self,
                             int overrideAmt) { // called on a regular interval
                                                // to regenerate force power.
  if (!self->client) {
    return;
  }
  ...
  if (self->client->ps.fd.forcePower >
      self->client->ps.fd
          .forcePowerMax) { // cap it off at the max (default 100)
    self->client->ps.fd.forcePower = self->client->ps.fd.forcePowerMax;
  }
}

void WP_ForcePowerStart(gentity_t *self, forcePowers_t forcePower,
                        int overrideAmt) { // activate the given force power
  int duration = 0;
  ...
  // hearable and hearDist are merely for the benefit of bots, and not related
  // to if a sound is actually played. If duration is set, the force power will
  // assume to be timer-based.
  ...
  case FP_TELEPATHY:
    ...
    if (self->client->ps.fd.forcePowerLevel[FP_TELEPATHY] == FORCE_LEVEL_1) {
      duration = 20000;
    } else if (self->client->ps.fd.forcePowerLevel[FP_TELEPATHY] ==
               FORCE_LEVEL_2) {
      duration = 25000;
    } else if (self->client->ps.fd.forcePowerLevel[FP_TELEPATHY] ==
               FORCE_LEVEL_3) {
  ...
  case FP_LIGHTNING:
    ...
    overrideAmt = 0;
    self->client->ps.fd.forcePowersActive |= (1 << forcePower);
    self->client->ps.activeForcePass =
        self->client->ps.fd.forcePowerLevel[FP_LIGHTNING];
    break;
  case FP_RAGE:
    hearable = qtrue;
    hearDist = 256;
  ...
  case FP_DRAIN:
    ...
    self->client->ps.fd.forcePowersActive |= (1 << forcePower);
    // self->client->ps.activeForcePass =
    // self->client->ps.fd.forcePowerLevel[FP_DRAIN];
    break;
  case FP_SEE:
    hearable = qtrue;
    hearDist = 256;
  ...
  } else if ((int)forcePower != FP_GRIP &&
             (int)forcePower !=
                 FP_DRAIN) { // grip and drain drain as damage is done
...
void ForceTeamHeal(gentity_t *self) {
  ...
  while (i < numpl) {
    if (g_entities[pl[i]].client->ps.stats[STAT_HEALTH] > 0 &&
      ...
      g_entities[pl[i]].health =
          g_entities[pl[i]].client->ps.stats[STAT_HEALTH];

      // At this point we know we got one, so add him into the collective event
      // client bitflag
      if (!te) {
        te = G_TempEntity(self->client->ps.origin, EV_TEAM_POWER);
        te->s.eventParm = 1; // eventParm 1 is heal, eventParm 2 is force regen

        // since we had an extra check above, do the drain now because we got at
        // least one guy
        BG_ForcePowerDrain(
            &self->client->ps, FP_TEAM_HEAL,
            forcePowerNeeded[self->client->ps.fd.forcePowerLevel[FP_TEAM_HEAL]]
                            [FP_TEAM_HEAL]);
      }

      WP_AddToClientBitflags(te, pl[i]);
      // Now cramming it all into one event.. doing this many g_sound events at
...
void ForceTeamForceReplenish(gentity_t *self) {
  ...
  BG_ForcePowerDrain(
      &self->client->ps, FP_TEAM_FORCE,
      forcePowerNeeded[self->client->ps.fd.forcePowerLevel[FP_TEAM_FORCE]]
                      [FP_TEAM_FORCE]);
  ...
  while (i < numpl) {
    ...
    if (g_entities[pl[i]].client->ps.fd.forcePower >
        g_entities[pl[i]].client->ps.fd.forcePowerMax) {
      g_entities[pl[i]].client->ps.fd.forcePower =
          g_entities[pl[i]].client->ps.fd.forcePowerMax;
    }
...
void ForceGrip(gentity_t *self) {
  ...
  trap->Trace(&tr, tfrom, NULL, NULL, tto, self->s.number, MASK_PLAYERSOLID,
              qfalse, 0, 0);

  if (tr.fraction != 1.0 && tr.entityNum != ENTITYNUM_NONE &&
      g_entities[tr.entityNum].client &&
      !g_entities[tr.entityNum].client->ps.fd.forceGripCripple &&
      g_entities[tr.entityNum].client->ps.fd.forceGripBeingGripped <
          level.time &&
      ForcePowerUsableOn(self, &g_entities[tr.entityNum], FP_GRIP) &&
      (g_friendlyFire.integer ||
       !OnSameTeam(self, &g_entities[tr.entityNum]))) // don't grip someone
                                                      // who's still crippled
  {
    if (g_entities[tr.entityNum].s.number < MAX_CLIENTS &&
        g_entities[tr.entityNum]
            .client->ps.m_iVehicleNum) { // a player on a vehicle
      gentity_t *vehEnt =
          &g_entities[g_entities[tr.entityNum].client->ps.m_iVehicleNum];
      if (vehEnt->inuse && vehEnt->client && vehEnt->m_pVehicle) {
        if (vehEnt->m_pVehicle->m_pVehicleInfo->type == VH_SPEEDER ||
            vehEnt->m_pVehicle->m_pVehicleInfo->type ==
                VH_ANIMAL) { // push the guy off
          vehEnt->m_pVehicle->m_pVehicleInfo->Eject(
              vehEnt->m_pVehicle, (bgEntity_t *)&g_entities[tr.entityNum],
              qfalse);
        }
      }
    }
...
void ForceSpeed(gentity_t *self, int forceDuration) {
  ...
  if (self->client->holdingObjectiveItem >= MAX_CLIENTS &&
      self->client->holdingObjectiveItem <
          ENTITYNUM_WORLD) { // holding Siege item
    if (g_entities[self->client->holdingObjectiveItem]
            .genericValue15) { // disables force powers
      return;
    }
  }
...
void ForceLightning(gentity_t *self) {
  ...
  if (self->client->ps.fd.forcePowerDebounce[FP_LIGHTNING] >
      level.time) { // stops it while using it and also after using it, up to 3
                    // second delay
    return;
  }
...
void ForceLightningDamage(gentity_t *self, gentity_t *traceEnt, vec3_t dir,
                          vec3_t impactPoint) {
  ...
  if (traceEnt && traceEnt->takedamage) {
    ...
    if (traceEnt->client) { // an enemy or object
      if (traceEnt->client->noLightningTime >=
          level.time) { // give them power and don't hurt them.
        traceEnt->client->ps.fd.forcePower++;
        if (traceEnt->client->ps.fd.forcePower >
            traceEnt->client->ps.fd.forcePowerMax) {
          traceEnt->client->ps.fd.forcePower =
              traceEnt->client->ps.fd.forcePowerMax;
        }
        return;
      ...
      if (ForcePowerUsableOn(self, traceEnt, FP_LIGHTNING)) {
        ...
        if (traceEnt->client) {
          modPowerLevel = WP_AbsorbConversion(
              traceEnt, traceEnt->client->ps.fd.forcePowerLevel[FP_ABSORB],
              self, FP_LIGHTNING,
        ...
        if (self->client->ps.weapon == WP_MELEE &&
            self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] >
                FORCE_LEVEL_2) { // 2-handed lightning
        ...
        if (dmg) {
          ...
          G_Damage(traceEnt, self, self, dir, impactPoint, dmg, 0,
                   MOD_FORCE_DARK);
        }
        if (traceEnt->client) {
          if (!Q_irand(0, 2)) {
            G_Sound(traceEnt, CHAN_BODY,
                    G_SoundIndex(va("sound/weapons/force/lightninghit%i",
                                    Q_irand(1, 3))));
          }

          if (traceEnt->client->ps.electrifyTime <
              (level.time +
               400)) { // only update every 400ms to reduce bandwidth usage (as
                       // it is passing a 32-bit time value)
            traceEnt->client->ps.electrifyTime = level.time + 800;
          }
          if (traceEnt->client->ps
                  .powerups[PW_CLOAKED]) { // disable cloak temporarily
            Jedi_Decloak(traceEnt);
            traceEnt->client->cloakToggleTime =
...
void ForceShootLightning(gentity_t *self) {
  ...
  if (self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] > FORCE_LEVEL_2) { // arc
    ...
    numListedEntities =
        trap->EntitiesInBox(mins, maxs, iEntityList, MAX_GENTITIES);
    ...
    for (e = 0; e < numListedEntities; e++) {
      ...
      if (traceEnt == self)
        continue;
      if (traceEnt->r.ownerNum == self->s.number &&
      ...
      if (!g_friendlyFire.integer && OnSameTeam(self, traceEnt))
        continue;
      // this is all to see if we need to start a saber attack, if it's in
      // flight, this doesn't matter
      ...
      // in PVS?
      if (!traceEnt->r.bmodel &&
          !trap->InPVS(ent_org, self->client->ps.origin)) { // must be in PVS
        continue;
      }

      // Now check and see if we can actually hit it
      trap->Trace(&tr, self->client->ps.origin, vec3_origin, vec3_origin,
                  ent_org, self->s.number, MASK_SHOT, qfalse, 0, 0);
      if (tr.fraction < 1.0f &&
          tr.entityNum != traceEnt->s.number) { // must have clear LOS
        continue;
      }
    ...
    trap->Trace(&tr, self->client->ps.origin, vec3_origin, vec3_origin, end,
...
void ForceDrain(gentity_t *self) {
  ...
  if (self->client->ps.fd.forcePower < 25 ||
      !WP_ForcePowerUsable(self, FP_DRAIN)) {
    return;
  }
  if (self->client->ps.fd.forcePowerDebounce[FP_DRAIN] >
      level.time) { // stops it while using it and also after using it, up to 3
                    // second delay
    return;
  }
...
void ForceDrainDamage(gentity_t *self, gentity_t *traceEnt, vec3_t dir,
                      vec3_t impactPoint) {
  gentity_t *tent;
  ...
  if (traceEnt && traceEnt->takedamage) {
    if (traceEnt->client &&
        (!OnSameTeam(self, traceEnt) || g_friendlyFire.integer) &&
      ...
      if (ForcePowerUsableOn(self, traceEnt, FP_DRAIN)) {
        ...
        } else if (self->client->ps.fd.forcePowerLevel[FP_DRAIN] ==
                   FORCE_LEVEL_2) {
          dmg = 3;
        } else if (self->client->ps.fd.forcePowerLevel[FP_DRAIN] ==
                   FORCE_LEVEL_3) {
        ...
        if (traceEnt->client) {
          modPowerLevel = WP_AbsorbConversion(
              traceEnt, traceEnt->client->ps.fd.forcePowerLevel[FP_ABSORB],
              self, FP_DRAIN, self->client->ps.fd.forcePowerLevel[FP_DRAIN], 1);
        }
        ...
        if (self->client->ps.stats[STAT_HEALTH] <
        ...
        traceEnt->client->ps.fd.forcePowerRegenDebounceTime =
            level.time + 800; // don't let the client being drained get force
                              // power back right away
        ...
        {
                BG_ForcePowerDrain( &self->client->ps, FP_DRAIN,
        forcePowerNeeded[self->client->ps.fd.forcePowerLevel[FP_DRAIN]][FP_DRAIN]/5
        );
        }
        ...
        if ( traceEnt->client )
        {
                if ( !Q_irand( 0, 2 ) )
                {
                        //G_Sound( traceEnt, CHAN_BODY, G_SoundIndex(
        "sound/weapons/force/lightninghit.wav" ) );
                }
...
int ForceShootDrain(gentity_t *self) {
  ...
  if (self->client->ps.fd.forcePowerLevel[FP_DRAIN] > FORCE_LEVEL_2) { // arc
    ...
    trap->Trace(&tr, self->client->ps.origin, vec3_origin, vec3_origin, end,
                self->s.number, MASK_SHOT, qfalse, 0, 0);
    if (tr.entityNum == ENTITYNUM_NONE || tr.fraction == 1.0 || tr.allsolid ||
        tr.startsolid || !g_entities[tr.entityNum].client ||
  ...
  self->client->ps.activeForcePass =
      self->client->ps.fd.forcePowerLevel[FP_DRAIN] + FORCE_LEVEL_3;

  BG_ForcePowerDrain(
      &self->client->ps, FP_DRAIN,
      5); // used to be 1, but this did, too, anger the God of Balance.
...
void ForceJumpCharge(gentity_t *self,
                     usercmd_t *ucmd) { // I guess this is unused now. Was used
                                        // for the "charge" jump type.
  float forceJumpChargeInterval =
      forceJumpStrength[0] / (FORCE_JUMP_CHARGE_TIME / FRAMETIME);
  ...
  if (self->client->ps.fd.forcePower <
      forcePowerNeeded[self->client->ps.fd.forcePowerLevel[FP_LEVITATION]]
                      [FP_LEVITATION]) {
    G_MuteSound(self->client->ps.fd.killSoundEntIndex[TRACK_CHANNEL_1 - 50],
                CHAN_VOICE);
    return;
  }
  ...
  if (self->client->ps.fd.forceJumpCharge >
      forceJumpStrength[self->client->ps.fd.forcePowerLevel[FP_LEVITATION]]) {
  ...
  if (self->client->ps.fd.forceJumpCharge / forceJumpChargeInterval /
    ...
    G_MuteSound(self->client->ps.fd.killSoundEntIndex[TRACK_CHANNEL_1 - 50],
                CHAN_VOICE);
    self->client->ps.fd.forceJumpCharge = self->client->ps.fd.forcePower *
...
int WP_GetVelocityForForceJump(gentity_t *self, vec3_t jumpVel,
                               usercmd_t *ucmd) {
  ...
  if (self->client->ps.fd.forceJumpCharge <
      JUMP_VELOCITY + 40) { // give him at least a tiny boost from just a tap
    self->client->ps.fd.forceJumpCharge = JUMP_VELOCITY + 400;
  }

  if (self->client->ps.velocity[2] <
      -30) { // so that we can get a good boost when force jumping in a fall
...
void ForceJump(gentity_t *self, usercmd_t *ucmd) {
  ...
  self->client->ps.fd.forceJumpZStart =
      self->client->ps.origin[2]; // remember this for when we land
  ...
  WP_ForcePowerStart(
      self, FP_LEVITATION,
      self->client->ps.fd.forceJumpCharge / forceJumpChargeInterval /
          (FORCE_JUMP_CHARGE_TIME / FRAMETIME) *
          forcePowerNeeded[self->client->ps.fd.forcePowerLevel[FP_LEVITATION]]
                          [FP_LEVITATION]);
  // self->client->ps.fd.forcePowerDuration[FP_LEVITATION] = level.time +
  // self->client->ps.weaponTime;
  self->client->ps.fd.forceJumpCharge = 0;
  self->client->ps.forceJumpFlip = qtrue;

  // We've just jumped, we're not gonna be on the ground in the following
  // frames. This makes sure npc's wont trigger the ForceJump function multiple
  // times before detecting that they have left the ground.
  self->client->ps.groundEntityNum = ENTITYNUM_NONE;
}
...
qboolean ForceTelepathyCheckDirectNPCTarget(gentity_t *self, trace_t *tr,
                                            qboolean *tookPower) {
  gentity_t *traceEnt;
  ...
  if (tr->entityNum == ENTITYNUM_NONE || tr->fraction == 1.0f || tr->allsolid ||
      tr->startsolid) {
    return qfalse;
  }
  ...
  if (targetLive && traceEnt->NPC) { // hit an organic non-player
    vec3_t eyeDir;
    if (G_ActivateBehavior(traceEnt,
                           BSET_MINDTRICK)) { // activated a script on him
      ...
      WP_ForcePowerStart(self, FP_TELEPATHY, 0);
               ...
               (!self->NPC &&
                traceEnt->client->playerTeam !=
                    (npcteam_t)self->client->sess.sessionTeam)) { // an enemy
      int override = 0;
      if ((traceEnt->NPC->scriptFlags & SCF_NO_MIND_TRICK)) {
      } else if (traceEnt->s.weapon != WP_SABER &&
                 traceEnt->client->NPC_class !=
                     CLASS_REBORN) { // haha!  Jedi aren't easily confused!
        if (self->client->ps.fd.forcePowerLevel[FP_TELEPATHY] >
            FORCE_LEVEL_2) { // turn them to our side
          // if mind trick 3 and aiming at an enemy need more force power
          if (traceEnt->s.weapon !=
              WP_NONE) { // don't charm people who aren't capable of fighting...
                         // like ugnaughts and droids
            ...
            if (self->NPC) { // NPC
              ...
              } else if (self->client->sess.sessionTeam ==
                         TEAM_RED) { // imperial
            ...
            // FIXME: need a *charmed* timer on this...?  Or do TEAM_PLAYERS
            // assume that "confusion" means they should switch to team_enemy
            // when done?
            traceEnt->NPC->charmedTime =
                level.time + mindTrickTime[self->client->ps.fd
                                               .forcePowerLevel[FP_TELEPATHY]];
          }
        } else { // just confuse them
          // somehow confuse them?  Set don't fire to true for a while?  Drop
          // their aggression?  Maybe just take their enemy away and don't let
          // them pick one up for a while unless shot?
          traceEnt->NPC->confusionTime =
              level.time +
              mindTrickTime[self->client->ps.fd
                                .forcePowerLevel[FP_TELEPATHY]]; // confused for
                                                                 // about 10
                                                                 // seconds
    ...
    } else if (traceEnt->client->playerTeam ==
               self->client->playerTeam) { // an ally
    ...
    G_PlayEffectID(G_EffectIndex("force/force_touch"),
                   traceEnt->client->renderInfo.eyePoint, eyeDir);

    // make sure this plays and that you cannot press fire for about 1 second
    // after this
    ...
    if (self->client->ps.fd.forcePowerLevel[FP_TELEPATHY] > FORCE_LEVEL_1 &&
        tr->fraction * 2048 > 64) { // don't create a diversion less than 64
                                    // from you of if at power level 1
      // use distraction anim instead
      G_PlayEffectID(G_EffectIndex("force/force_touch"), tr->endpos,
                     tr->plane.normal);
  ...
  // self->client->ps.saberMove = self->client->ps.saberBounceMove =
  ...
  {
          self->client->ps.weaponTime = floor( self->client->ps.weaponTime *
  g_timescale->value );
  }
  */
  return qtrue;
...
void ForceTelepathy(gentity_t *self) {
  ...
  if (self->client->ps.powerups[PW_REDFLAG] ||
      self->client->ps
          .powerups[PW_BLUEFLAG]) { // can't mindtrick while carrying the flag
    return;
  }
  ...
  if (ForceTelepathyCheckDirectNPCTarget(self, &tr,
                                         &tookPower)) { // hit an NPC directly
    self->client->ps.forceAllowDeactivateTime = level.time + 1500;
  ...
  if (self->client->ps.fd.forcePowerLevel[FP_TELEPATHY] == FORCE_LEVEL_1) {
    if (tr.fraction != 1.0 && tr.entityNum != ENTITYNUM_NONE &&
        g_entities[tr.entityNum].inuse && g_entities[tr.entityNum].client &&
  ...
  } else // level 2 & 3
  {
    ...
    while (e < numListedEntities) {
      ...
      if (ent) { // not in the arc, don't consider it
        ...
        } else if (!InFieldOfVision(self->client->ps.viewangles, visionArc,
                                    a)) { // only bother with arc rules if the
                                          // victim is a client
          entityList[e] = ENTITYNUM_NONE;
...
qboolean CanCounterThrow(gentity_t *self, gentity_t *thrower, qboolean pull) {
  ...
  if (self->client->ps.weaponstate == WEAPON_CHARGING ||
      self->client->ps.weaponstate ==
          WEAPON_CHARGING_ALT) { // don't autodefend when charging a weapon
    return 0;
  }

  if (level.gametype == GT_SIEGE && pull && thrower &&
      thrower->client) { // in siege, pull will affect people if they are not
                         // facing you, so they can't run away so much
    vec3_t d;
    float a;
    ...
    if (a > 60.0f ||
        a < -60.0f) { // if facing more than 60 degrees away they cannot defend
      return 0;
    }
  }
  ...
  if (self->client->ps.groundEntityNum ==
      ENTITYNUM_NONE) { // you cannot counter a push/pull if you're in the air
    return 0;
  }
...
float forcePushPullRadius[NUM_FORCE_POWER_LEVELS] = {0,   // none
                                                     384, // 256,
                                                     448, // 384,
                                                     512};
// rwwFIXMEFIXME: incorporate this into the below function? Currently it's only
// being used by jedi AI
...
void ForceThrow(gentity_t *self, qboolean pull) {
  ...
  if (!pull && self->client->ps.saberLockTime > level.time &&
      self->client->ps.saberLockFrame) {
    G_Sound(self, CHAN_BODY, G_SoundIndex("sound/weapons/force/push.wav"));
    self->client->ps.powerups[PW_DISINT_4] = level.time + 1500;

    self->client->ps.saberLockHits +=
        self->client->ps.fd.forcePowerLevel[FP_PUSH] * 2;
  ...
  if (pull) {
    ...
    if (self->client->ps.forceHandExtend == HANDEXTEND_NONE) {
      ...
      if (level.gametype == GT_SIEGE &&
          self->client->ps.weapon ==
              WP_SABER) { // hold less so can attack right after a pull
    ...
    self->client->ps.powerups[PW_DISINT_4] =
        self->client->ps.forceHandExtendTime + 200;
    ...
    if (self->client->ps.forceHandExtend == HANDEXTEND_NONE) {
      ...
      self->client->ps.forceDodgeAnim +=
          8; // special case, play push on upper torso, but keep playing current
             // knockdown anim on legs
    }
  ...
  if (powerLevel ==
      FORCE_LEVEL_1) { // can only push/pull targeted things at level 1
    VectorCopy(self->client->ps.origin, tfrom);
    tfrom[2] += self->client->ps.viewheight;
    AngleVectors(self->client->ps.viewangles, fwd, NULL, NULL);
    tto[0] = tfrom[0] + fwd[0] * radius / 2;
    ...
    if (tr.fraction != 1.0 && tr.entityNum != ENTITYNUM_NONE) {
      if (!g_entities[tr.entityNum].client &&
          g_entities[tr.entityNum].s.eType == ET_NPC) { // g2animent
    ...
    while (e < numListedEntities) {
      ...
      if (ent) { // not in the arc, don't consider it
        ...
        if (ent->client &&
            !InFieldOfVision(self->client->ps.viewangles, visionArc, a) &&
            ForcePowerUsableOn(self, ent,
                               powerUse)) { // only bother with arc rules if the
                                            // victim is a client
          entityList[e] = ENTITYNUM_NONE;
        } else if (ent->client) {
          if (pull) {
  ...
  for (e = 0; e < numListedEntities; e++) {
    if (entityList[e] != ENTITYNUM_NONE && entityList[e] >= 0 &&
    ...
    if (ent->s.eType != ET_MISSILE) {
      if (ent->s.eType != ET_ITEM) {
        ...
        if (Q_stricmp("func_button", ent->classname) == 0) { // we might push it
          if (pull ||
              !(ent->spawnflags &
                SPF_BUTTON_FPUSHABLE)) { // not force-pushable, never pullable
            continue;
          }
        } else {
          ...
          if (!ent->client) {
            if (Q_stricmp("lightsaber", ent->classname) !=
                0) { // not a lightsaber
              if (Q_stricmp("func_door", ent->classname) != 0 ||
                  !(ent->spawnflags &
                ...
                continue;
                     ...
                     ent->client->NPC_class ==
                         CLASS_RANCOR) { // can't push ATST or Galak or Rancor
            continue;
          }
        }
      }
      ...
      if (ent->s.pos.trType == TR_STATIONARY &&
          (ent->s.eFlags & EF_MISSILE_STICK)) { // can't force-push/pull stuck
                                                // missiles (detpacks, tripmines)
        continue;
      }
      if (ent->s.pos.trType == TR_STATIONARY &&
          ent->s.weapon != WP_THERMAL) { // only thermal detonators can be
                                         // pushed once stopped
        continue;
      }
    }

    // this is all to see if we need to start a saber attack, if it's in flight,
    // this doesn't matter
    // find the distance from the edge of the bounding box
    for (i = 0; i < 3; i++) {
    ...
    trap->Trace(&tr, self->client->ps.origin, vec3_origin, vec3_origin, ent_org,
                self->s.number, MASK_SHOT, qfalse, 0, 0);
    if (tr.fraction < 1.0f &&
        tr.entityNum != ent->s.number) { // must have clear LOS
      // try from eyes too before you give up
      vec3_t eyePoint;
      VectorCopy(self->client->ps.origin, eyePoint);
      eyePoint[2] += self->client->ps.viewheight;
      trap->Trace(&tr, eyePoint, vec3_origin, vec3_origin, ent_org,
                  self->s.number, MASK_SHOT, qfalse, 0, 0);
  ...
  if (ent_count) {
    ...
    for (x = 0; x < ent_count; x++) {
      ...
      if (push_list[x]->client) {
        modPowerLevel = WP_AbsorbConversion(
            push_list[x],
            push_list[x]->client->ps.fd.forcePowerLevel[FP_ABSORB], self,
            powerUse, powerLevel,
            forcePowerNeeded[self->client->ps.fd.forcePowerLevel[powerUse]]
                            [powerUse]);
        if (modPowerLevel == -1) {
      ...
      if (push_list[x]->client) { // FIXME: make enemy jedi able to hunker down
                                  // and resist this?
        int otherPushPower =
            push_list[x]->client->ps.fd.forcePowerLevel[powerUse];
        qboolean canPullWeapon = qtrue;
        float dirLen = 0;

        if (g_debugMelee.integer) {
          if ((push_list[x]->client->ps.pm_flags &
               PMF_STUCK_TO_WALL)) { // no resistance if stuck to wall
        ...
        if (push_list[x]->client->pers.cmd.forwardmove ||
            push_list[x]
                ->client->pers.cmd.rightmove) { // if you are moving, you get
                                                // one less level of defense
          otherPushPower--;
        ...
        if (otherPushPower && CanCounterThrow(push_list[x], self, pull)) {
          ...
          push_list[x]->client->ps.powerups[PW_DISINT_4] =
              push_list[x]->client->ps.forceHandExtendTime + 200;

          if (pull) {
            push_list[x]->client->ps.powerups[PW_PULL] =
                push_list[x]->client->ps.powerups[PW_DISINT_4];
          } else {
        ...
        if (pull) {
          ...
          if (push_list[x]->client && VectorLength(pushDir) <= 256) {
            ...
            if (!OnSameTeam(self, push_list[x]) && Q_irand(1, 10) <= randfact &&
                canPullWeapon) {
              vec3_t uorg, vecnorm;
        ...
        if ((modPowerLevel > otherPushPower ||
          ...
          if (modPowerLevel == FORCE_LEVEL_3 &&
            ...
            if (BG_KnockDownable(&push_list[x]->client->ps) &&
              ...
              gentity_t *vehEnt =
                  &g_entities[push_list[x]->client->ps.m_iVehicleNum];
              if (vehEnt->inuse && vehEnt->client && vehEnt->m_pVehicle) {
                if (vehEnt->m_pVehicle->m_pVehicleInfo->type == VH_SPEEDER ||
                    vehEnt->m_pVehicle->m_pVehicleInfo->type ==
                        VH_ANIMAL) { // push the guy off
        ...
        if (push_list[x]->client) {
          ...
          if (self->client->ps.fd.forceGripBeingGripped >
              level.time) { // force the enemy to stop gripping me if I managed
                            // to push him
            if (push_list[x]->client->ps.fd.forceGripEntityNum ==
                self->s.number) {
              if (modPowerLevel >=
                  push_list[x]->client->ps.fd.forcePowerLevel
                      [FP_GRIP]) { // only break the grip if our push/pull level
                                   // is >= their grip level
                WP_ForcePowerStop(push_list[x], FP_GRIP);
                self->client->ps.fd.forceGripBeingGripped = 0;
                push_list[x]->client->ps.fd.forceGripUseTime =
                    level.time + 1000; // since we just broke out of it..
              }
            }
          }
          ...
          if ((int)push_list[x]->client->ps.velocity[2] ==
              0) { // if not going anywhere vertically, boost them up a bit
            ...
            push_list[x]->client->ps.velocity[2] = pushDir[2] * pushPowerMod;
                 ...
                 (push_list[x]->s.pos.trType != TR_INTERPOLATE ||
                  push_list[x]->s.weapon !=
                      WP_THERMAL)) // rolling and stationary thermal detonators
                                   // are dealt with below
      {
        ...
        if (!pull && (push_list[x]->spawnflags & 1 /*F_PUSH*/)) {
          ...
          GEntity_UseFunc(push_list[x], self, self);
                 ...
                 (push_list[x]->spawnflags & 2)) { // push/pull the door
        vec3_t pos1, pos2;
        vec3_t trFrom;
        ...
        trap->Trace(&tr, trFrom, vec3_origin, vec3_origin, end, self->s.number,
                    MASK_SHOT, qfalse, 0, 0);
        if (tr.entityNum != push_list[x]->s.number || tr.fraction == 1.0 ||
            tr.allsolid || tr.startsolid) { // must be pointing right at it
          continue;
        }

        if (VectorCompare(
                vec3_origin,
                push_list[x]->s.origin)) { // does not have an origin brush, so
                                           // pos1 & pos2 are relative to world
                                           // origin, need to calc center
          ...
          if ((push_list[x]->spawnflags & 1) &&
              push_list[x]->moverState ==
                  MOVER_POS1) { // if at pos1 and started open, make sure we get
                                // the center where it *started* because we're
                                // going to add back in the relative values pos1
                                // and pos2
            VectorSubtract(center, push_list[x]->pos1, center);
          } else if (!(push_list[x]->spawnflags & 1) &&
                     push_list[x]->moverState ==
                         MOVER_POS2) { // if at pos2, make sure we get the
                                       // center where it *started* because we're
                                       // going to add back in the relative
                                       // values pos1 and pos2
        ...
        if (Distance(pos1, trFrom) < Distance(pos2, trFrom)) { // pos1 is closer
          if (push_list[x]->moverState == MOVER_POS1) { // at the closest pos
            if (pull) { // trying to pull, but already at closest point, so
                        // screw it
              continue;
            }
          } else if (push_list[x]->moverState == MOVER_POS2) { // at farthest
                                                               // pos
            if (!pull) { // trying to push, but already at farthest point, so
                         // screw it
              continue;
            }
          }
        ...
        GEntity_UseFunc(push_list[x], self, self);
      } else if (Q_stricmp("func_button", push_list[x]->classname) ==
                 0) { // pretend you pushed it
  ...
  // if we're still in a current grip that wasn't broken by the push, it will
  // still remain
  self->client->dangerTime = level.time;
  self->client->ps.eFlags &= ~EF_INVULNERABLE;
  self->client->invulnerableTimer = 0;
...
void WP_ForcePowerStop(gentity_t *self, forcePowers_t forcePower) {
  ...
  case FP_SPEED:
    if (wasActive & (1 << FP_SPEED)) {
      G_MuteSound(self->client->ps.fd.killSoundEntIndex[TRACK_CHANNEL_2 - 50],
                  CHAN_VOICE);
    }
    break;
  case FP_PUSH:
  ...
  case FP_SEE:
    if (wasActive & (1 << FP_SEE)) {
      G_MuteSound(self->client->ps.fd.killSoundEntIndex[TRACK_CHANNEL_5 - 50],
                  CHAN_VOICE);
    }
    break;
  case FP_GRIP:
    ...
    if (self->client->ps.fd.forcePowerLevel[FP_GRIP] > FORCE_LEVEL_1 &&
        ...
        (level.time - g_entities[self->client->ps.fd.forceGripEntityNum]
                          .client->ps.fd.forceGripStarted) >
            500) { // if we had our throat crushed in for more than half a
                   // second, gasp for air when we're let go
      if (wasActive & (1 << FP_GRIP)) {
        G_EntitySound(&g_entities[self->client->ps.fd.forceGripEntityNum],
                      CHAN_VOICE, G_SoundIndex("*gasp.wav"));
      }
    }
    ...
    if (g_entities[self->client->ps.fd.forceGripEntityNum].client &&
      ...
      g_entities[self->client->ps.fd.forceGripEntityNum]
          .client->ps.forceGripChangeMovetype = PM_NORMAL;
    }
  ...
  case FP_LIGHTNING:
    if (self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] <
        FORCE_LEVEL_2) { // don't do it again for 3 seconds, minimum... FIXME:
                         // this should be automatic once regeneration is slower
                         // (normal)
  ...
  case FP_RAGE:
    ...
    if (wasActive & (1 << FP_RAGE)) {
      G_MuteSound(self->client->ps.fd.killSoundEntIndex[TRACK_CHANNEL_3 - 50],
                  CHAN_VOICE);
    }
    break;
  case FP_ABSORB:
  ...
  case FP_PROTECT:
    ...
    break;
  case FP_DRAIN:
    if (self->client->ps.fd.forcePowerLevel[FP_DRAIN] <
        FORCE_LEVEL_2) { // don't do it again for 3 seconds, minimum...
...
void DoGripAction(gentity_t *self, forcePowers_t forcePower) {
  ...
  trap->Trace(&tr, self->client->ps.origin, NULL, NULL,
              gripEnt->client->ps.origin, self->s.number, MASK_PLAYERSOLID,
              qfalse, 0, 0);

  gripLevel = WP_AbsorbConversion(
      gripEnt, gripEnt->client->ps.fd.forcePowerLevel[FP_ABSORB], self, FP_GRIP,
  ...
  if (!InFront(gripEnt->client->ps.origin, self->client->ps.origin,
               self->client->ps.viewangles, 0.9f) &&
  ...
  if (self->client->ps.fd.forcePowerDebounce[FP_GRIP] <
      level.time) { // 2 damage per second while choking, resulting in 10 damage
                    // total (not including The Squeeze<tm>)
    self->client->ps.fd.forcePowerDebounce[FP_GRIP] = level.time + 1000;
    G_Damage(gripEnt, self, self, NULL, NULL, 2, DAMAGE_NO_ARMOR,
             MOD_FORCE_DARK);
  }
  ...
  if (gripLevel == FORCE_LEVEL_2) {
    ...
    if (gripEnt->client->ps.forceGripMoveInterval < level.time) {
      ...
      gripEnt->client->ps.forceGripMoveInterval =
          level.time + 300; // only update velocity every 300ms, so as to avoid
    ...
    if ((level.time - gripEnt->client->ps.fd.forceGripStarted) > 3000 &&
        !self->client->ps.fd
             .forceGripDamageDebounceTime) { // if we managed to lift him into
                                             // the air for 2 seconds, give him a
                                             // crack
      self->client->ps.fd.forceGripDamageDebounceTime = 1;
      G_Damage(gripEnt, self, self, NULL, NULL, 20, DAMAGE_NO_ARMOR,
               MOD_FORCE_DARK);

      // Must play custom sounds on the actual entity. Don't use G_Sound (it
      // creates a temp entity for the sound)
      G_EntitySound(gripEnt, CHAN_VOICE,
                    G_SoundIndex(va("*choke%d.wav", Q_irand(1, 3))));

      gripEnt->client->ps.forceHandExtend = HANDEXTEND_CHOKE;
      gripEnt->client->ps.forceHandExtendTime = level.time + 2000;

      if (gripEnt->client->ps.fd.forcePowersActive &
          (1 << FP_GRIP)) { // choking, so don't let him keep gripping himself
    ...
    if ((level.time - gripEnt->client->ps.fd.forceGripStarted) > 3000 &&
        !self->client->ps.fd
             .forceGripDamageDebounceTime) { // if we managed to lift him into
                                             ...
                                             // crack
      self->client->ps.fd.forceGripDamageDebounceTime = 1;
      G_Damage(gripEnt, self, self, NULL, NULL, 40, DAMAGE_NO_ARMOR,
               MOD_FORCE_DARK);
...
static void WP_UpdateMindtrickEnts(gentity_t *self) {
  ...
  while (i < MAX_CLIENTS) {
    if (G_IsMindTricked(&self->client->ps.fd, i)) {
      ...
      } else if ((level.time - self->client->dangerTime) <
                 g_TimeSinceLastFrame *
                     4) { // Untrick this entity if the tricker (self) fires
                          // while in his fov
        if (trap->InPVS(ent->client->ps.origin, self->client->ps.origin) &&
            OrgVisible(ent->client->ps.origin, self->client->ps.origin,
                       ent->s.number)) {
          RemoveTrickedEnt(&self->client->ps.fd, i);
        }
  ...
  if (!self->client->ps.fd.forceMindtrickTargetIndex &&
      ...
      !self->client->ps.fd
           .forceMindtrickTargetIndex4) { // everyone who we had tricked is no
                                          // longer tricked, so stop the power
    WP_ForcePowerStop(self, FP_TELEPATHY);
...
#define FORCE_DEBOUNCE_TIME                                                    \
  50 // sv_fps 20 = 50msec frametime, basejka balance/timing

static void WP_ForcePowerRun(gentity_t *self, forcePowers_t forcePower,
                             usercmd_t *cmd) {
  ...
  case FP_HEAL:
    if (self->client->ps.fd.forcePowerLevel[FP_HEAL] == FORCE_LEVEL_1) {
      if (self->client->ps.velocity[0] || self->client->ps.velocity[1] ||
    ...
    if (self->health >
        self->client->ps
            .stats[STAT_MAX_HEALTH]) { // rww - we might start out over
                                       // max_health and we don't want force heal
                                       // taking us down to 100 or whatever
                                       // max_health is
      WP_ForcePowerStop(self, forcePower);
      break;
    }
    ...
    if (self->health >
        self->client->ps.stats[STAT_MAX_HEALTH]) // Past max health
    {
      self->health = self->client->ps.stats[STAT_MAX_HEALTH];
      WP_ForcePowerStop(self, forcePower);
    }
    ...
    if ((self->client->ps.fd.forcePowerLevel[FP_HEAL] == FORCE_LEVEL_1 &&
         self->client->ps.fd.forceHealAmount >= 25) ||
        (self->client->ps.fd.forcePowerLevel[FP_HEAL] == FORCE_LEVEL_2 &&
         self->client->ps.fd.forceHealAmount >= 33)) {
      WP_ForcePowerStop(self, forcePower);
    }
    break;
  case FP_SPEED:
  ...
  case FP_GRIP:
    ...
    if (self->client->ps.fd.forcePowerDebounce[FP_PULL] <
        level.time) { // This is sort of not ideal. Using the debounce value
                      // reserved for pull for this because pull doesn't need it.
  ...
  case FP_LEVITATION:
    if (self->client->ps.groundEntityNum != ENTITYNUM_NONE &&
        !self->client->ps.fd.forceJumpZStart) { // done with jump
      WP_ForcePowerStop(self, forcePower);
    }
    break;
  case FP_RAGE:
    ...
    if (self->client->ps.forceRageDrainTime < level.time) {
      ...
      if (self->client->ps.fd.forcePowerLevel[FP_RAGE] == FORCE_LEVEL_1) {
        addTime = 150;
      } else if (self->client->ps.fd.forcePowerLevel[FP_RAGE] ==
                 FORCE_LEVEL_2) {
        addTime = 300;
      } else if (self->client->ps.fd.forcePowerLevel[FP_RAGE] ==
                 FORCE_LEVEL_3) {
    ...
    if (self->client->ps.fd.forcePowerLevel[FP_DRAIN] >
      ...
      if ((cmd->buttons & BUTTON_FORCE_DRAIN) ||
          ((cmd->buttons & BUTTON_FORCEPOWER) &&
           self->client->ps.fd.forcePowerSelected ==
               FP_DRAIN)) { // holding it keeps it going
        self->client->ps.fd.forcePowerDuration[FP_DRAIN] = level.time + 500;
      }
    }
    // OVERRIDEFIXME
    if (!WP_ForcePowerAvailable(self, forcePower, 0) ||
        self->client->ps.fd.forcePowerDuration[FP_DRAIN] < level.time ||
  ...
  case FP_LIGHTNING:
    if (self->client->ps.forceHandExtend !=
        HANDEXTEND_FORCE_HOLD) { // Animation for hand extend doesn't end with
                                 // hand out, so we have to limit lightning
                                 // intervals by animation intervals (once hand
                                 // starts to go in in animation, lightning
                                 // should stop)
      WP_ForcePowerStop(self, forcePower);
      break;
    }
    ...
    if (self->client->ps.fd.forcePowerLevel[FP_LIGHTNING] >
      ...
      if ((cmd->buttons & BUTTON_FORCE_LIGHTNING) ||
          ((cmd->buttons & BUTTON_FORCEPOWER) &&
           self->client->ps.fd.forcePowerSelected ==
               FP_LIGHTNING)) { // holding it keeps it going
  ...
  case FP_TELEPATHY:
    if (self->client->holdingObjectiveItem >= MAX_CLIENTS &&
        self->client->holdingObjectiveItem < ENTITYNUM_WORLD &&
        g_entities[self->client->holdingObjectiveItem]
            .genericValue15) { // if force hindered can't mindtrick whilst
                               // carrying a siege item
      WP_ForcePowerStop(self, FP_TELEPATHY);
    } else {
...
int WP_DoSpecificPower(gentity_t *self, usercmd_t *ucmd,
                       forcePowers_t forcepower) {
  int powerSucceeded;
  ...
  case FP_HEAL:
    ...
    if (self->client->ps.fd
            .forceButtonNeedRelease) { // need to release before we can use
                                       // nonhold powers again
      break;
    }
    ForceHeal(self);
    ...
    break;
  case FP_LEVITATION:
    // if leave the ground by some other means, cancel the force jump so we
    // don't suddenly jump when we land.

    if (self->client->ps.groundEntityNum == ENTITYNUM_NONE) {
      self->client->ps.fd.forceJumpCharge = 0;
      G_MuteSound(self->client->ps.fd.killSoundEntIndex[TRACK_CHANNEL_1 - 50],
                  CHAN_VOICE);
      // This only happens if the groundEntityNum == ENTITYNUM_NONE when the
  ...
  case FP_PUSH:
    ...
    if (self->client->ps.fd.forceButtonNeedRelease &&
        !(self->r.svFlags &
          SVF_BOT)) { // need to release before we can use nonhold powers again
      break;
    }
...
void FindGenericEnemyIndex(
    gentity_t *self) { // Find another client that would be considered a threat.
  int i = 0;
  float tlen;
  gentity_t *ent;
  ...
  while (i < MAX_CLIENTS) {
    ...
    if (ent && ent->client && ent->s.number != self->s.number &&
        ent->health > 0 && !OnSameTeam(self, ent) &&
      ...
      if (tlen < blen &&
          InFront(ent->client->ps.origin, self->client->ps.origin,
                  self->client->ps.viewangles, 0.8f) &&
          OrgVisible(self->client->ps.origin, ent->client->ps.origin,
                     self->s.number)) {
        blen = tlen;
        besten = ent;
      }
...
void SeekerDroneUpdate(gentity_t *self) {
  ...
  if (self->health < 1) {
    ...
    angle = ((level.time / 12) & 255) * (M_PI * 2) /
            255; // magical numbers make magic happen
    dir[0] = cos(angle) * 20;
    dir[1] = sin(angle) * 20;
    dir[2] = cos(angle) * 5;
  ...
  if (self->client->ps.genericEnemyIndex != ENTITYNUM_NONE &&
      self->client->ps.genericEnemyIndex != -1) {
    ...
    if (!en || !en->client) {
      ...
      if (!InFront(en->client->ps.origin, self->client->ps.origin,
                   self->client->ps.viewangles, 0.8f)) {
        self->client->ps.genericEnemyIndex = ENTITYNUM_NONE;
      } else if (!OrgVisible(self->client->ps.origin, en->client->ps.origin,
                             self->s.number)) {
        self->client->ps.genericEnemyIndex = ENTITYNUM_NONE;
      }
    }
  ...
  if (self->client->ps.genericEnemyIndex != ENTITYNUM_NONE &&
    ...
    // org is now where the thing should be client-side because it uses the same
    // time-based offset
    if (self->client->ps.droneFireTime < level.time) {
      trap->Trace(&tr, org, NULL, NULL, en->client->ps.origin, -1, MASK_SOLID,
                  qfalse, 0, 0);
      ...
      if (tr.fraction == 1 && !tr.startsolid && !tr.allsolid) {
        ...
        WP_FireGenericBlasterMissile(self, org, endir, 0, 15, 2000,
                                     MOD_BLASTER);
        G_SoundAtLoc(org, CHAN_WEAPON,
...
void HolocronUpdate(
    gentity_t *self) { // keep holocron status updated in holocron mode
  int i = 0;
  int noHRank = 0;
  ...
  while (i < NUM_FORCE_POWERS) {
    if (self->client->ps
            .holocronsCarried[i]) { // carrying it, make sure we have the power
      ...
      if ((self->client->ps.fd.forcePowersKnown & (1 << i)) &&
          i != FP_LEVITATION && i != FP_SABER_OFFENSE) {
        self->client->ps.fd.forcePowersKnown -= (1 << i);
      }
      ...
      if (i == FP_LEVITATION) {
        ...
        // make sure that the player's saber stance is reset so they can't
        // continue to use that stance when they don't have the skill for it
        // anymore.
        if (self->client->saber[0].model[0] &&
            self->client->saber[1].model[0]) { // dual
  ...
  if (HasSetSaberOnly()) { // if saberonly, we get these powers no matter what
                           // (still need the holocrons for level 3)
...
void JediMasterUpdate(
    gentity_t *self) { // keep jedi master status updated for JM gametype
  int i = 0;
  ...
  while (i < NUM_FORCE_POWERS) {
    if (self->client->ps.isJediMaster) {
      ...
      if (i == FP_TEAM_HEAL || i == FP_TEAM_FORCE || i == FP_DRAIN ||
          i ==
              FP_ABSORB) { // team powers are useless in JM, absorb is too
                           // because no one else has powers to absorb. Drain is
                           // just relatively useless in comparison, because its
                           // main intent is not to heal, but rather to cripple
                           // others by draining their force at the same time.
                           // And no one needs force in JM except the JM himself.
        self->client->ps.fd.forcePowersKnown &= ~(1 << i);
        self->client->ps.fd.forcePowerLevel[i] = 0;
      }

      if (i == FP_TELEPATHY) { // this decision was made because level 3
                               // mindtrick allows the JM to just hide too much,
                               // and no one else has force sight to counteract
                               // it. Since the JM himself is the focus of
                               // gameplay in this mode, having him hidden for
                               // large durations is indeed a bad thing.
...
qboolean G_SpecialRollGetup(
    gentity_t
        *self) { // fixme: currently no knockdown will actually land you on your
                 // front... so froll's are pretty useless at the moment.
...
void WP_ForcePowersUpdate(gentity_t *self, usercmd_t *ucmd) {
  ...
  if (self->client->ps.pm_flags &
      PMF_FOLLOW) { // not a "real" game client, it's a spectator following
                    // someone
    return;
  }
  if (self->client->sess.sessionTeam == TEAM_SPECTATOR) {
  ...
  */
  // The stance in relation to power level is no longer applicable with the
  // crazy new akimbo/staff stances.
  if (!self->client->ps.fd.saberAnimLevel) {
    self->client->ps.fd.saberAnimLevel = FORCE_LEVEL_1;
  }
  ...
  if (self->client->ps.fd.forcePowerSelected < 0 ||
      self->client->ps.fd.forcePowerSelected >= NUM_FORCE_POWERS) { // bad
    self->client->ps.fd.forcePowerSelected = 0;
  }

  if (((self->client->sess.selectedFP !=
        self->client->ps.fd.forcePowerSelected) ||
       (self->client->sess.saberLevel != self->client->ps.fd.saberAnimLevel)) &&
      !(self->r.svFlags & SVF_BOT)) {
    if (self->client->sess.updateUITime <
        level.time) { // a bit hackish, but we don't want the client to flood
                      // with userinfo updates if they rapidly cycle through
                      // their force powers or saber attack levels
  ...
  if (self->client->ps.forceHandExtend == HANDEXTEND_KNOCKDOWN &&
    ...
    if (self->client->ps.forceHandExtend == HANDEXTEND_KNOCKDOWN &&
      ...
      if (self->health < 1 || (self->client->ps.eFlags & EF_DEAD)) {
        ...
        self->client->ps.forceHandExtend = HANDEXTEND_NONE;
      } else { // hmm.. ok.. no more getting up on your own, you've gotta push
               // something, unless..
        if ((level.time - self->client->ps.forceHandExtendTime) >
            4000) { // 4 seconds elapsed, I guess they're too dumb to push
                    // something to get up!
          if (self->client->pers.cmd.upmove &&
              self->client->ps.fd.forcePowerLevel[FP_LEVITATION] >
                  FORCE_LEVEL_1) { // force getup
            G_PreDefSound(self->client->ps.origin, PDSOUND_FORCEJUMP);
            self->client->ps.forceDodgeAnim = 2;
            self->client->ps.forceHandExtendTime = level.time + 500;
  ...
  if (self && self->client &&
      (BG_HasYsalamiri(level.gametype, &self->client->ps) ||
       self->client->ps.fd.forceDeactivateAll ||
       self->client->tempSpectate >=
           level.time)) { // has ysalamiri.. or we want to forcefully stop all
                          // his active powers
    i = 0;
  ...
  } else { // otherwise just do a check through them all to see if they need to
  ...
  if (self->client->ps.powerups[PW_FORCE_ENLIGHTENED_LIGHT] ||
      self->client->ps.powerups[PW_FORCE_ENLIGHTENED_DARK]) { // enlightenment
    if (!self->client->ps.fd.forceUsingAdded) {
      i = 0;

      while (i < NUM_FORCE_POWERS) {
        self->client->ps.fd.forcePowerBaseLevel[i] =
      ...
      self->client->ps.fd.forceUsingAdded = 1;
                 ...
                 .forceUsingAdded) { // we don't have enlightenment but we're
                                     // still using enlightened powers, so clear
                                     // them back to how they should be.
    i = 0;
  ...
  if (!(self->client->ps.fd.forcePowersActive &
        (1 << FP_TELEPATHY))) { // clear the mindtrick index values
    self->client->ps.fd.forceMindtrickTargetIndex = 0;
    self->client->ps.fd.forceMindtrickTargetIndex2 = 0;
    self->client->ps.fd.forceMindtrickTargetIndex3 = 0;
    self->client->ps.fd.forceMindtrickTargetIndex4 = 0;
  ...
  if (self->client->ps.fd.forceJumpCharge &&
      self->client->ps.groundEntityNum == ENTITYNUM_NONE &&
      self->client
          ->fjDidJump) { // this was for the "charge" jump method... I guess
    if (ucmd->upmove < 10 &&
...
#ifndef METROID_JUMP
  else if ((ucmd->upmove > 10) && (self->client->ps.pm_flags & PMF_JUMP_HELD) &&
           self->client->ps.groundTime &&
           (level.time - self->client->ps.groundTime) > 150 &&
           !BG_HasYsalamiri(level.gametype, &self->client->ps) &&
           BG_CanUseFPNow(level.gametype, &self->client->ps, level.time,
                          FP_LEVITATION)) { // just charging up
  ...
  if (ucmd->buttons &
      BUTTON_FORCEGRIP) { // grip is one of the powers with its own button.. if
                          // it's held, call the specific grip power function.
  ...
  for (i = 0; i < NUM_FORCE_POWERS; i++) {
    if (self->client->ps.fd.forcePowerDuration[i]) {
      if (self->client->ps.fd.forcePowerDuration[i] < level.time) {
        if ((self->client->ps.fd.forcePowersActive & (1 << i))) { // turn it off
          WP_ForcePowerStop(self, (forcePowers_t)i);
        }
        self->client->ps.fd.forcePowerDuration[i] = 0;
      }
  ...
  if ((!self->client->ps.fd.forcePowersActive ||
      ...
      (self->client->ps.weapon != WP_SABER ||
       !BG_SaberInSpecial(
           self->client->ps
               .saberMove))) { // when not using the force, regenerate at 1
                               // point per half second
    while (self->client->ps.fd.forcePowerRegenDebounceTime < level.time) {
      if (level.gametype != GT_HOLOCRON || g_maxHolocronCarry.value) {
        ...
        else if (self->client->ps.isJediMaster &&
                 level.gametype == GT_JEDIMASTER)
          WP_ForcePowerRegenerate(self,
                                  4); // jedi master regenerates 4 times as fast
        else
      ...
      if (level.gametype == GT_SIEGE) {
        if (self->client->holdingObjectiveItem &&
          ...
          self->client->ps.fd.forcePowerRegenDebounceTime +=
              7000; // 1 point per 7 seconds.. super slow
          ...
          self->client->ps.fd.forcePowerRegenDebounceTime += Q_max(
              g_forceRegenTime.integer * 0.2,
              1); // if this is siege and our player class has the fast force
                  // regen ability, then recharge with 1/5th the usual delay
        else
          self->client->ps.fd.forcePowerRegenDebounceTime +=
              Q_max(g_forceRegenTime.integer, 1);
      } else {
        if (level.gametype == GT_POWERDUEL &&
            self->client->sess.duelTeam == DUELTEAM_LONE) {
          if (duel_fraglimit.integer)
            self->client->ps.fd.forcePowerRegenDebounceTime +=
                Q_max(g_forceRegenTime.integer *
                          (0.6 + (.3 * (float)self->client->sess.wins /
                                  (float)duel_fraglimit.integer)),
                      1);
          else
            self->client->ps.fd.forcePowerRegenDebounceTime +=
                Q_max(g_forceRegenTime.integer * 0.7, 1);
        } else
          self->client->ps.fd.forcePowerRegenDebounceTime +=
              Q_max(g_forceRegenTime.integer, 1);
      }
...
qboolean Jedi_DodgeEvasion(gentity_t *self, gentity_t *shooter, trace_t *tr,
  ...
  if (self->client->ps.weaponTime > 0 ||
      self->client->ps.forceHandExtend !=
          HANDEXTEND_NONE) { // in some effect that stops me from moving on my
                             // own
    return qfalse;
  }
  ...
  if (g_forceDodge.integer == 2) {
    if (self->client->ps.fd
            .forcePowersActive) { // for now just don't let us dodge if we're
                                  // using a force power at all
      return qfalse;
    }
  }
  ...
  if (g_forceDodge.integer == 2) {
    if (!WP_ForcePowerUsable(
            self,
            FP_SPEED)) { // make sure we have it and have enough force power
      return qfalse;
    }
  }
  ...
  if (g_forceDodge.integer == 2) {
    if (Q_irand(1, 7) >
        self->client->ps.fd
            .forcePowerLevel[FP_SPEED]) { // more likely to fail on lower force
                                          // speed level
      return qfalse;
    }
  } else {
    // We now dodge all the time, but only on level 3
    if (self->client->ps.fd.forcePowerLevel[FP_SEE] <
        FORCE_LEVEL_3) { // more likely to fail on lower force sight level
      return qfalse;
    }
  }
