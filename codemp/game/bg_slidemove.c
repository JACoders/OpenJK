...
#ifdef _GAME
extern void G_FlyVehicleSurfaceDestruction(gentity_t *veh, trace_t *trace,
                                           int magnitude,
                                           qboolean force);      // g_vehicle.c
...
void PM_VehicleImpact(bgEntity_t *pEnt, trace_t *trace) {
  ...
  float magnitude =
      VectorLength(pm->ps->velocity) * pSelfVeh->m_pVehicleInfo->mass / 50.0f;
  qboolean forceSurfDestruction = qfalse;
#ifdef _GAME
  gentity_t *hitEnt = trace != NULL ? &g_entities[trace->entityNum] : NULL;

  if (!hitEnt || (pSelfVeh && pSelfVeh->m_pPilot && hitEnt &&
                  hitEnt->s.eType == ET_MISSILE && hitEnt->inuse &&
  ...
  { // spiralling to our deaths, explode on any solid impact
    if (hitEnt->s.NPC_class == CLASS_VEHICLE) { // hit another vehicle, explode!
      ...
      // FIXME: damage hitEnt, some, too?  Our explosion should hurt them some,
      // but...
      G_Damage((gentity_t *)pEnt, killer, killer, NULL, pm->ps->origin, 999999,
               DAMAGE_NO_ARMOR, MOD_FALLING); // FIXME: MOD_IMPACT
      return;
    } else if (!VectorCompare(trace->plane.normal, vec3_origin) &&
               (trace->entityNum == ENTITYNUM_WORLD ||
                hitEnt->r.bmodel)) { // have a valid hit plane and we hit a
                                     // solid brush
      vec3_t moveDir;
      float impactDot;
      ...
      {                       // Just DIE now
        ...
        G_Damage((gentity_t *)pEnt, killer, killer, NULL, pm->ps->origin,
                 999999, DAMAGE_NO_ARMOR, MOD_FALLING); // FIXME: MOD_IMPACT
        return;
      }
    }
  }

  if (trace->entityNum < ENTITYNUM_WORLD && hitEnt->s.eType == ET_MOVER &&
      hitEnt->s.apos.trType != TR_STATIONARY // rotating
      && (hitEnt->spawnflags & 16)           // IMPACT
      && Q_stricmp("func_rotating", hitEnt->classname) ==
             0) { // hit a func_rotating that is supposed to destroy anything it
                  // touches!
...
if ( (pSelfVeh->m_ulFlags&VEH_GEARSOPEN)
  ...
  { // we're landing, we're cool
    ...
    if ( pSelfVeh->m_pVehicleInfo->iImpactFX )
    {
            vec3_t up = {0,0,1};
#ifdef _GAME
            G_PlayEffectID( pSelfVeh->m_pVehicleInfo->iImpactFX, pm->ps->origin,
up ); #else trap->FX_PlayEffectID( pSelfVeh->m_pVehicleInfo->iImpactFX,
pm->ps->origin, up, -1, -1, qfalse ); #endif
    }
    */
    // this was annoying me -rww
    // FIXME: this shouldn't even be getting called when the vehicle is at rest!
#ifdef _GAME
    if (hitEnt && (hitEnt->s.eType == ET_PLAYER || hitEnt->s.eType == ET_NPC) &&
        pSelfVeh->m_pVehicleInfo->type == VH_FIGHTER) { // always smack players
    } else
#endif
    {
      return;
  ...
  if (pSelfVeh &&
      (pSelfVeh->m_pVehicleInfo->type == VH_SPEEDER ||
       pSelfVeh->m_pVehicleInfo->type ==
           VH_FIGHTER) && // this is kind of weird on tauntauns and atst's..
      (magnitude >= 100 || forceSurfDestruction)) {
    if (pEnt->m_pVehicle->m_iHitDebounce < pm->cmd.serverTime ||
        forceSurfDestruction) { // a bit of a hack, may conflict with getting
                                // shot, but...
      ...
      if (trace && !pSelfVeh->m_iRemovedSurfaces && !forceSurfDestruction) {
        ...
        if ((trace->entityNum == ENTITYNUM_WORLD ||
            ...
            !VectorCompare(trace->plane.normal,
                           vec3_origin)) // have a valid plane to bounce off of
        { // bounce off in the opposite direction of the impact
          if (pSelfVeh->m_pVehicleInfo->type == VH_SPEEDER) {
            ...
            VectorCopy(trace->plane.normal, bounceDir);
                         ...
                         MIN_LANDING_SLOPE // flat enough to land on
                     && pSelfVeh->m_LandTrace.fraction < 1.0f // ground present
                     &&
                     pm->ps->speed <=
                         MIN_LANDING_SPEED) { // could land here, don't bounce
                                              // off, in fact, return altogether!
            return;
          } else {
        ...
        } else if (pSelfVeh->m_pVehicleInfo->type ==
                   VH_FIGHTER) { // check for impact with another fighter
#ifdef _CGAME
          bgEntity_t *hitEnt = PM_BGEntForNum(trace->entityNum);
#endif
          if (hitEnt->s.NPC_class == CLASS_VEHICLE && hitEnt->m_pVehicle &&
              hitEnt->m_pVehicle->m_pVehicleInfo &&
              hitEnt->m_pVehicle->m_pVehicleInfo->type ==
                  VH_FIGHTER) { // two vehicles hit each other, turn away from
                                // the impact
            turnFromImpact = qtrue;
            turnHitEnt = qtrue;
#ifdef _GAME
        ...
        if (turnFromImpact) { // bounce off impact surf and turn away
          vec3_t pushDir = {0}, turnAwayAngles, turnDelta;
          float turnStrength, pitchTurnStrength, yawTurnStrength;
          vec3_t moveDir;
          float bounceDot, turnDivider;
          // bounce
          if (!turnHitEnt) { // hit wall
            VectorScale(
                bounceDir,
                (pm->ps->speed * 0.25f / pSelfVeh->m_pVehicleInfo->mass),
                pushDir);
            ...
            if (hitEnt->client) {
              ...
              VectorScale(bounceDir, (pm->ps->speed + hitEnt->s.speed) * 0.5f,
                          pushDir);
            }
#else
            VectorScale(bounceDir, (pm->ps->speed + hitEnt->s.speed) * 0.5f,
          ...
          // now do yaw
          if (!bounceDir[0] && !bounceDir[1]) { // shouldn't be any yaw
          } else {
          ...
          /*
          PM_SetPMViewAngle(pm->ps, pSelfVeh->m_vOrientation,
          &pSelfVeh->m_ucmd); if ( pm_entVeh )
          {//I'm a vehicle, so pm_entVeh is actually my pilot
                  ...
                  {
                          //set the rider's viewangles to the vehicle's
          viewangles PM_SetPMViewAngle(pilot->playerState,
          pSelfVeh->m_vOrientation, &pSelfVeh->m_ucmd);
                  }
          }
          */
          ...
          if (turnHitEnt // make the other guy turn and get pushed
              ...
              && !FighterIsLanded(hitEnt->m_pVehicle,
                                  &hitEnt->client->ps) // but not if landed
              && !(hitEnt->spawnflags & 2))            // and not if suspended
          {
            ...
            VectorScale(pushDir, bounceDot, pushDir);
            VectorAdd(hitEnt->client->ps.velocity, pushDir,
            ...
            vectoangles(bounceDir, turnAwayAngles);
            // get the delta from our current angles to those new angles
            AnglesSubtract(turnAwayAngles, hitEnt->m_pVehicle->m_vOrientation,
                           turnDelta);
            // now do pitch
            if (!bounceDir[2]) { // shouldn't be any pitch
            } else {
              ...
              hitEnt->m_pVehicle->m_vFullAngleVelocity[PITCH] =
                  AngleNormalize180(hitEnt->m_pVehicle->m_vOrientation[PITCH] +
            ...
            if (!bounceDir[0] && !bounceDir[1]) { // shouldn't be any yaw
              ...
              hitEnt->m_pVehicle->m_vFullAngleVelocity[ROLL] =
                  AngleNormalize180(hitEnt->m_pVehicle->m_vOrientation[ROLL] -
                                    yawTurnStrength / turnDivider *
                                        pSelfVeh->m_fTimeModifier);
            }
            // NOTE: will these angle changes stick or will they be stomped
            //		when the vehicle goes through its own update and
            //re-grabs 		its angles from its pilot...?  Should we do a
            ...
            {
                    SetClientViewAngle( (gentity_t
            *)hitEnt->m_pVehicle->m_pPilot, hitEnt->m_pVehicle->m_vOrientation
            );
            }
            */
          }
      ...
      if (hitEnt &&
          (hitEnt->s.eType != ET_TERRAIN || !(hitEnt->spawnflags & 1) ||
           pSelfVeh->m_pVehicleInfo->type ==
               VH_FIGHTER)) { // don't damage the vehicle from terrain that
                              // doesn't want to damage vehicles
        if (pSelfVeh->m_pVehicleInfo->type ==
            VH_FIGHTER) { // increase the damage...
          float mult = (pSelfVeh->m_vOrientation[PITCH] * 0.1f);
          if (mult < 1.0f) {
            mult = 1.0f;
          }
          if (hitEnt->inuse &&
              hitEnt->takedamage) { // if the other guy takes damage, don't hurt
                                    // us a lot for ramming him
            // unless it's a vehicle, then we get 1.5 times damage
            if (hitEnt->s.eType == ET_NPC &&
                hitEnt->s.NPC_class == CLASS_VEHICLE && hitEnt->m_pVehicle) {
              mult = 1.5f;
            } else {
              mult = 0.5f;
            }
        ...
        G_Damage((gentity_t *)pEnt, NULL, NULL, NULL, pm->ps->origin,
                 magnitude * 5, DAMAGE_NO_ARMOR,
                 MOD_FALLING); // FIXME: MOD_IMPACT

        if (pSelfVeh->m_pVehicleInfo->surfDestruction) {
          G_FlyVehicleSurfaceDestruction((gentity_t *)pEnt, trace, magnitude,
      ...
      if (hitEnt && hitEnt->inuse &&
        ...
        if ((hitEnt->s.eType == ET_PLAYER && hitEnt->s.number < MAX_CLIENTS) ||
            (hitEnt->s.eType == ET_NPC &&
             hitEnt->s.NPC_class !=
                 CLASS_VEHICLE)) { // probably a humanoid, or something
          if (pSelfVeh->m_pVehicleInfo->type ==
              VH_FIGHTER) { // player die good.. if me fighter
            pmult = 2000.0f;
          } else {
            pmult = 40.0f;
          }

          if (hitEnt->client && BG_KnockDownable(&hitEnt->client->ps) &&
            ...
            if (hitEnt->client->ps.forceHandExtend != HANDEXTEND_KNOCKDOWN) {
              ...
              hitEnt->client->ps.forceHandExtendTime =
                  pm->cmd.serverTime + 1100;
              hitEnt->client->ps.forceDodgeAnim =
            ...
            hitEnt->client->ps.otherKillerDebounceTime =
                pm->cmd.serverTime + 100;

            // add my velocity into his to force him along in the correct
            // direction from impact
            VectorAdd(hitEnt->client->ps.velocity, pm->ps->velocity,
                      hitEnt->client->ps.velocity);
            // upward thrust
        ...
        G_Damage(hitEnt, attackEnt, attackEnt, NULL, pm->ps->origin, finalD, 0,
                 MOD_MELEE); // FIXME: MOD_IMPACT
      }
#else // this is gonna result in "double effects" for the client doing the
      // prediction. it doesn't look bad though. could just use predicted events,
      // but I'm too lazy.
      hitEnt = PM_BGEntForNum(trace->entityNum);

      if (!hitEnt ||
          hitEnt->s.owner != pEnt->s.number) { // don't hit your own missiles!
        AngleVectors(pSelfVeh->m_vOrientation, NULL, NULL, vehUp);
        pEnt->m_pVehicle->m_iHitDebounce = pm->cmd.serverTime + 200;
        trap->FX_PlayEffectID(pSelfVeh->m_pVehicleInfo->iImpactFX,
...
qboolean PM_ClientImpact(trace_t *trace) {
  ...
  {
    Client_CheckImpactBBrush((gentity_t *)(pm_entSelf),
                             &g_entities[otherEntityNum]);
  }

  if (!traceEnt || !(traceEnt->r.contents &
                     pm->tracemask)) { // it's dead or not in my way anymore,
                                       // don't clip against it
    return qtrue;
  }
  ...
  for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
    ...
    if (pm->ps->clientNum >= MAX_CLIENTS) {
      ...
      if (pEnt && pEnt->s.eType == ET_NPC &&
          ...
          pEnt->m_pVehicle) { // do vehicle impact stuff then
    ...
    //
    if (!(pm->ps->pm_flags &
          PMF_STUCK_TO_WALL)) { // no sliding if stuck to wall!
      for (i = 0; i < numplanes; i++) {
        if (VectorCompare(
                normal,
                planes[i])) { // DotProduct( normal, planes[i] ) > 0.99 ) {
...
void PM_StepSlideMove(qboolean gravity) {
  ...
  if (pm->ps->clientNum >= MAX_CLIENTS) {
    ...
    if (pEnt && (pEnt->s.NPC_class == CLASS_ATST ||
                 (pEnt->s.NPC_class == CLASS_VEHICLE && pEnt->m_pVehicle &&
                  pEnt->m_pVehicle->m_pVehicleInfo->type ==
                      VH_WALKER))) { // AT-STs can step high
      up[2] += 66.0f;
      isGiant = qtrue;
    } else if (pEnt &&
               pEnt->s.NPC_class == CLASS_RANCOR) { // also can step up high
      up[2] += 64.0f;
      isGiant = qtrue;
    } else {
      up[2] += STEPSIZE;
  ...
  if (pm->stepSlideFix) {
    if (pm->ps->clientNum < MAX_CLIENTS &&
        trace.plane.normal[2] <
            MIN_WALK_NORMAL) { // normal players cannot step up slopes that are
      ...
      //  (B)\__
      //        \_____(A)
      // Then it might still be okay, so we figure out the slope of the entire
      // move from (A) to (B) and if that slope is walk-upabble, then it's okay
  ...
  if (!trace.allsolid && !skipStep) // normal players cannot step up slopes that
                                    // are too steep to walk on!
  {
    if (pm->ps->clientNum >= MAX_CLIENTS // NPC
        && isGiant && trace.entityNum < MAX_CLIENTS && pEnt &&
        pEnt->s.NPC_class == CLASS_RANCOR) { // Rancor don't step on clients
      if (pm->stepSlideFix) {
