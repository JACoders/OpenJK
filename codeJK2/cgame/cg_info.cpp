...
const char *showLoadPowersName[] = {
    "INGAME_HEAL2",          "INGAME_JUMP2",          "INGAME_SPEED2",
    "INGAME_PUSH2",          "INGAME_PULL2",          "INGAME_MINDTRICK2",
    "INGAME_GRIP2",          "INGAME_LIGHTNING2",     "INGAME_SABER_THROW2",
    "INGAME_SABER_OFFENSE2", "INGAME_SABER_DEFENSE2", NULL,
};
...
====================
*/
static void MissionPrint_Line(const int color, const int objectIndex,
                              int &missionYcnt) {
  char *str, *strBegin;
  ...
  int iYPixelsPerLine = cgi_R_Font_HeightPixels(cgs.media.qhFontMedium, 1.0f) *
                        (cgi_Language_IsAsian() ? 1.2f : 1.0f);

  cgi_SP_GetStringText(PACKAGE_OBJECTIVES << 8 | objectIndex, finalText,
                       sizeof(finalText));
  ...
  */
  if (pixelLen < 500) // One shot - small enough to print entirely on one line
  {
    ...
    cgi_R_Font_DrawString(108, y, str, colorTable[color],
  ...
  // Text is too long, break into lines.
    ...
    while (*str) {
      ...
      if (pixelLen > 500) { // Reached max length of this line
        ...
        CG_DrawProportionalString(108, y, holdText, CG_SMALLFONT,
                                  colorTable[color]);
        ++missionYcnt;
      } else if (*(str + 1) == '\0') {
        ++charLen;
...
void MissionInformation_Draw(centity_t *cent) {
  ...
  cgi_R_Font_DrawString(96, missionYpos - 23, text, colorTable[CT_WHITE],
                        cgs.media.qhFontMedium, -1, 1.0f);

  int missionYcnt = 0;

  obj_graphics[0] = obj_graphics[1] = obj_graphics[2] = obj_graphics[3] =
      qfalse;
  ...
  for (i = 0; i < MAX_OBJECTIVES; ++i) {
    if (cent->gent->client->sess.mission_objectives[i].display) {
      totalY = missionYpos + (iYPixelsPerLine * (missionYcnt)) +
               (iYPixelsPerLine / 2);
      if (obj_graphics[0]) {
        totalY += 32 + 4;
      }
  ...
  if (!missionYcnt) {
    ...
    //		CG_DrawProportionalString(108, missionYpos, text, CG_SMALLFONT,
    //colorTable[CT_LTBLUE1] );
    cgi_R_Font_DrawString(108, missionYpos, text, colorTable[CT_LTBLUE1],
                          cgs.media.qhFontMedium, -1, 1.0f);
  }
}
...
static void CG_LoadScreen_PersonalInfo(void)
{
        ...
        if (hasForcePowers)
        {
                cgi_SP_GetStringTextString( "INGAME_CURRENTFORCEPOWERS", text,
sizeof(text) ); CG_DrawProportionalString( 200, 65, text, CG_CENTER |
...
"INGAME_ALONGTIME", text, sizeof(text) ); int w =
...
void CG_DrawInformation(void) {
  ...
  if (!levelshot || g_eSavedGameJustLoaded == eFULL) {
    // keep whatever's in the screen buffer so far (either the last ingame
    // rendered-image (eg for maptransition)
      ...
      !strcmp(s, "kejim_post")) // special case for first map!
  {
    ...
    cgi_R_Font_DrawString((320) - (w / 2), 140, text, colorTable[CT_ICON_BLUE],
                          cgs.media.qhFontMedium, -1, 1.0f);
  } else if (cg_missionstatusscreen.integer) {
...
#ifndef NDEBUG
    cgi_R_Font_DrawString(48, 398, va("LOADING ... %s", cg.infoScreenText),
