...
void G_ReflectMissile(gentity_t *ent, gentity_t *missile, vec3_t forward) {
  ...
  if (missile->r.ownerNum ==
      ent->s.number) { // the original owner is bouncing the missile, so don't
                       // try to bounce it back at him
    isowner = 1;
  }
  ...
  if (&g_entities[missile->r.ownerNum] && missile->s.weapon != WP_SABER &&
    ...
    VectorSubtract(g_entities[missile->r.ownerNum].r.currentOrigin,
                   missile->r.currentOrigin, bounce_dir);
    VectorNormalize(bounce_dir);
  } else if (isowner) { // in this case, actually push the missile away from me,
                        // and since we're giving boost to our own missile by
                        // pushing it, up the velocity
    vec3_t missile_dir;
  ...
  if (missile->s.weapon != WP_SABER &&
      missile->s.weapon != G2_MODEL_PART) { // you are mine, now!
...
void G_DeflectMissile(gentity_t *ent, gentity_t *missile, vec3_t forward) {
  ...
  if (ent->client) {
    // VectorSubtract( ent->r.currentOrigin, missile->r.currentOrigin,
    // missile_dir );
...
void G_BounceMissile(gentity_t *ent, trace_t *trace) {
  ...
  if (ent->s.weapon == WP_THERMAL) { // slight hack for hit sound
    G_Sound(
        ent, CHAN_BODY,
        G_SoundIndex(va("sound/weapons/thermal/bounce%i.wav", Q_irand(1, 2))));
  } else if (ent->s.weapon == WP_SABER) {
    G_Sound(
        ent, CHAN_BODY,
        G_SoundIndex(va("sound/weapons/saber/bounce%i.wav", Q_irand(1, 3))));
...
//-----------------------------------------------------------------------------
{
  ...
  missile->s.pos.trTime =
      level.time; // - MISSILE_PRESTEP_TIME;	// NOTENOTE This is a Quake 3
                  // addition over JK2
...
void G_MissileBounceEffect(gentity_t *ent, vec3_t org, vec3_t dir) {
  ...
  switch (ent->s.weapon) {
  case WP_BOWCASTER:
    G_PlayEffectID(G_EffectIndex("bowcaster/deflect"), ent->r.currentOrigin,
                   dir);
    break;
  case WP_BLASTER:
...
void G_MissileImpact(gentity_t *ent, trace_t *trace) {
  ...
  // check for bounce
  if (!other->takedamage && (ent->bounceCount > 0 || ent->bounceCount == -5) &&
      (ent->flags & (FL_BOUNCE | FL_BOUNCE_HALF))) {
    G_BounceMissile(ent, trace);
    G_AddEvent(ent, EV_GRENADE_BOUNCE, 0);
    return;
  } else if (ent->neverFree && ent->s.weapon == WP_SABER &&
             (ent->flags & FL_BOUNCE_HALF)) { // this is a knocked-away saber
  ...
  // I would glom onto the FL_BOUNCE code section above, but don't feel like
  ...
  */
  ...
  if ((other->r.contents & CONTENTS_LIGHTSABER) &&
      !isKnockedSaber) { // hit this person's saber, so..
  ...
  if (other->flags & FL_DMG_BY_HEAVY_WEAP_ONLY) {
    if (ent->methodOfDeath != MOD_REPEATER_ALT &&
        ...
        ent->methodOfDeath != MOD_VEHICLE && ent->methodOfDeath != MOD_CONC &&
        ent->methodOfDeath != MOD_CONC_ALT && ent->methodOfDeath != MOD_SABER &&
  ...
  if ((other->flags & FL_SHIELDED) && ent->s.weapon != WP_ROCKET_LAUNCHER &&
      ent->s.weapon != WP_THERMAL && ent->s.weapon != WP_TRIP_MINE &&
      ent->s.weapon != WP_DET_PACK && ent->s.weapon != WP_DEMP2 &&
      ent->s.weapon != WP_EMPLACED_GUN &&
      ent->methodOfDeath != MOD_REPEATER_ALT &&
      ent->methodOfDeath != MOD_FLECHETTE_ALT_SPLASH &&
      ent->methodOfDeath != MOD_TURBLAST && ent->methodOfDeath != MOD_VEHICLE &&
  ...
  if (other->takedamage && other->client &&
      ent->s.weapon != WP_ROCKET_LAUNCHER && ent->s.weapon != WP_THERMAL &&
      ent->s.weapon != WP_TRIP_MINE && ent->s.weapon != WP_DET_PACK &&
      ent->s.weapon != WP_DEMP2 && ent->methodOfDeath != MOD_REPEATER_ALT &&
      ent->methodOfDeath != MOD_FLECHETTE_ALT_SPLASH &&
      ent->methodOfDeath != MOD_CONC && ent->methodOfDeath != MOD_CONC_ALT &&
      other->client->ps.saberBlockTime < level.time && !isKnockedSaber &&
      WP_SaberCanBlock(other, ent->r.currentOrigin, 0, 0, qtrue,
                       0)) { // only block one projectile per 200ms (to prevent
                             // giant swarms of projectiles being blocked)
    vec3_t fwd;
    gentity_t *te;
    ...
    if (other->client->ps.velocity[2] > 0 ||
        other->client->pers.cmd.forwardmove <
            0) // now we only do it if jumping or running backward. Should be
               // able to full-on charge.
    {
      otherDefLevel -= 1;
      if (otherDefLevel < 0) {
        otherDefLevel = 0;
    ...
    if (otherDefLevel == FORCE_LEVEL_1) {
      // if def is only level 1, instead of deflecting the shot it should just
      // die here
    ...
    other->client->ps.saberBlockTime =
        level.time + (350 - (otherDefLevel * 100)); // 200;
    ...
    if (otherOwner->takedamage && otherOwner->client &&
      ...
      // in this case, deflect it even if we can't actually block it because it
      // hit our saber WP_SaberCanBlock(otherOwner, ent->r.currentOrigin, 0, 0,
      // qtrue, 0);
  ...
  if (other->takedamage && !isKnockedSaber) {
    ...
    if (ent->damage) {
      ...
      if (ent->s.weapon == WP_BOWCASTER || ent->s.weapon == WP_FLECHETTE ||
        ...
        if (ent->s.weapon == WP_FLECHETTE && (ent->s.eFlags & EF_ALT_FIRING)) {
          ...
          function for explosion event. This, then, would set
          ent->freeAfterEvent to qtrue, so event later, when reusing this entity
          by using G_InitEntity(), it would have this freeAfterEvent set AND
          this would in case of dropped item erase it from game immeadiately.
          THIS for example caused very rare flag dissappearing bug.	 */
        ...
        G_Damage(other, ent, &g_entities[ent->r.ownerNum], velocity,
                 /*ent->s.origin*/ ent->r.currentOrigin, ent->damage, 0,
                 ent->methodOfDeath);
        didDmg = qtrue;
      }

      if (didDmg && other &&
          other->client) { // What I'm wondering is why this isn't in the NPC
                           // pain funcs. But this is what SP does, so whatever.
    ...
    if (ent->s.weapon ==
        WP_DEMP2) { // a hit with demp2 decloaks people, disables ships
      if (other && other->client &&
          other->client->NPC_class == CLASS_VEHICLE) { // hit a vehicle
        if (other->m_pVehicle                          // valid vehicle ent
            && other->m_pVehicle->m_pVehicleInfo       // valid stats
            && (other->m_pVehicle->m_pVehicleInfo->type ==
                    VH_SPEEDER // always affect speeders
                || (other->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER &&
                    ent->classname &&
                    Q_stricmp("vehicle_proj", ent->classname) ==
                        0)) // only vehicle ion weapons affect a fighter in this
                            // manner
            && !FighterIsLanded(other->m_pVehicle,
                                &other->client->ps) // not landed
        ...
        if (ent->methodOfDeath ==
            MOD_DEMP2_ALT) { // direct hit with alt disables cloak forever
...
void G_RunMissile(gentity_t *ent) {
  ...
  if (ent->neverFree && ent->s.weapon == WP_SABER &&
  ...
  if (ent->target_ent) {
    ...
    if ((ent->r.svFlags & SVF_OWNERNOTSHARED) &&
        (ent->s.eFlags & EF_JETPACK_ACTIVE)) { // A vehicle missile that should
                                               // be solid to its owner
  ...
  if (d_projectileGhoul2Collision.integer) {
    ...
    if (tr.fraction != 1.0 && tr.entityNum < ENTITYNUM_WORLD) {
      ...
      if (g2Hit->inuse && g2Hit->client &&
          g2Hit->ghoul2) { // since we used G2TRFLAG_GETSURFINDEX,
    ...
    trap->Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin,
                passent, ent->clipmask, qfalse, 0, 0);
  }

  if (tr.startsolid || tr.allsolid) {
    // make sure the tr.entityNum is set to the entity we're stuck in
    trap->Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs,
                ent->r.currentOrigin, passent, ent->clipmask, qfalse, 0, 0);
    tr.fraction = 0;
  } else {
  ...
  if (ent->s.weapon == G2_MODEL_PART && !ent->bounceCount) {
    ...
    lowerOrg[2] -= 1;
    trap->Trace(&trG, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, lowerOrg,
                passent, ent->clipmask, qfalse, 0, 0);
  ...
  if (tr.fraction != 1) {
    ...
    if (ent->s.weapon > WP_NONE && ent->s.weapon < WP_NUM_WEAPONS &&
        (tr.entityNum < MAX_CLIENTS ||
         g_entities[tr.entityNum].s.eType ==
             ET_NPC)) { // player or NPC, try making a mark on him
      // copy current pos to s.origin, and current projected traj to origin2
      VectorCopy(ent->r.currentOrigin, ent->s.origin);
      BG_EvaluateTrajectory(&ent->s.pos, level.time, ent->s.origin2);
    ...
    if (tr.entityNum ==
        ent->s.otherEntityNum) { // if the impact event other and the trace ent
                                 // match then it's ok to do the g2 mark
