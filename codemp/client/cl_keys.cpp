...
int historyLine;     // the line being displayed from history buffer will be <=
                     // nextHistoryLine
...
// do NOT blithely change any of the key names (3rd field) here, since they have
// to match the key binds
//	in the CFG files, they're also prepended with "KEYNAME_" when looking up
//StringEd references
//
keyname_t keynames[MAX_KEYS] = {
    ...
    {L'\u00A3', L'\u00A3', NULL, A_POUND,
     false}, // pound (as in currency) symbol
    ...
    {L'\u00AE', L'\u00AE', NULL, A_REGISTERED,
     false}, // registered trademark symbol
    ...
    {L'\u00BF', L'\u00BF', NULL, A_QUESTION_DOWN,
     false}, // upside-down question mark
...
void Console_Key(int key) {
  ...
  if (key == A_ENTER || key == A_KP_ENTER) {
    ...
    if (cls.cgameStarted && cl.mSharedMemory) {
      TCGIncomingConsoleCommand *icc =
          (TCGIncomingConsoleCommand *)cl.mSharedMemory;

      Q_strncpyz(icc->conCommand, g_consoleField.buffer,
                 sizeof(icc->conCommand));
  ...
  if (key == A_CURSOR_UP || key == A_KP_8 ||
      ...
      (kg.keys[A_CTRL].down &&
       keynames[key].lower ==
           'p')) { // scroll up: arrow-up, numpad-up, shift + mwheelup, ctrl + p
  ...
  if (key == A_CURSOR_DOWN || key == A_KP_2 ||
      ...
      (kg.keys[A_CTRL].down &&
       keynames[key].lower == 'n')) { // scroll down: arrow-down, numpad-down,
                                      // shift + mwheeldown, ctrl + n
    historyLine++;
...
qboolean Key_GetOverstrikeMode(void) { return kg.key_overstrikeMode; }

void Key_SetOverstrikeMode(qboolean state) { kg.key_overstrikeMode = state; }
...
void Key_Bind_f(void) {
  ...
  if (c == 2) {
    if (kg.keys[b].binding && kg.keys[b].binding[0])
      Com_Printf(S_COLOR_GREY "Bind " S_COLOR_WHITE "%s = " S_COLOR_GREY
...
void Key_Bindlist_f(void) {
  for (size_t i = 0; i < MAX_KEYS; i++) {
    if (kg.keys[i].binding && kg.keys[i].binding[0])
      Com_Printf(S_COLOR_GREY "Key " S_COLOR_WHITE "%s (%s) = " S_COLOR_GREY
                              "\"" S_COLOR_WHITE "%s" S_COLOR_GREY
                              "\"" S_COLOR_WHITE "\n",
                 Key_KeynumToAscii(i), Key_KeynumToString(i),
                 kg.keys[i].binding);
  }
}
...
void CL_ParseBinding(int key, qboolean down, unsigned time) {
  ...
  if (cls.state == CA_DISCONNECTED && Key_GetCatcher() == 0)
    return;
  if (!kg.keys[keynames[key].upper].binding ||
      !kg.keys[keynames[key].upper].binding[0])
    return;
    ...
    if (*p == '+') {
      ...
      if (allCommands || CL_BindUICommand(p)) {
        ...
        if (cls.cgameStarted && cl.mSharedMemory) {
          ...
          if (CGVM_IncomingConsoleCommand()) {
            // rww - let mod authors filter client console messages so they can
            // cut them off if they want.
            Cbuf_AddText(p);
            Cbuf_AddText("\n");
          } else if (icc->conCommand[0]) {
...
void CL_CharEvent(int key) {
  ...
  if (Key_GetCatcher() & KEYCATCH_CONSOLE)
    ...
    UIVM_KeyEvent(key | K_CHAR_FLAG, qtrue);
  else if (Key_GetCatcher() & KEYCATCH_CGAME)
    CGVM_KeyEvent(key | K_CHAR_FLAG, qtrue);
  else if (Key_GetCatcher() & KEYCATCH_MESSAGE)
