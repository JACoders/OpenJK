...
gentity_t *G_TestEntityPosition(gentity_t *ent) {
  ...
  if (ent->client) {
    ...
    trap->Trace(&tr, ent->client->ps.origin, ent->r.mins, vMax,
                ent->client->ps.origin, ent->s.number, mask, qfalse, 0, 0);
  } else {
    trap->Trace(&tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs,
                ent->s.pos.trBase, ent->s.number, mask, qfalse, 0, 0);
  }
  ...
  */
  if (pusher->s.apos.trType != TR_STATIONARY // rotating
      ...
      && Q_stricmp("func_rotating", pusher->classname) ==
             0) { // just blow the fuck out of them
    G_Damage(check, pusher, pusher, NULL, NULL, pusher->damage,
  ...
  if (check->takedamage && !check->client && check->s.weapon &&
      check->r.ownerNum < MAX_CLIENTS && check->health < 500) {
    if (check->health > 0) {
      G_Damage(check, pusher, pusher, vec3_origin, check->r.currentOrigin, 999,
               0, MOD_UNKNOWN);
    }
  }
...
qboolean G_MoverPush(gentity_t *pusher, vec3_t move, vec3_t amove,
  ...
  *obstacle = NULL;
  ...
  listedEntities =
      trap->EntitiesInBox(totalMins, totalMaxs, entityList, MAX_GENTITIES);
  ...
  for (e = 0; e < listedEntities; e++) {
    ...
    if (/*check->s.eType != ET_ITEM &&*/ check->s.eType != ET_PLAYER &&
        check->s.eType != ET_NPC && !check->physicsObject) {
      continue;
    }
    ...
    if (check->s.eType == ET_BODY ||
        (check->s.eType == ET_PLAYER &&
         check->health < 1)) { // whatever, just crush it
    ...
    if ((check->r.contents & CONTENTS_TRIGGER) &&
        check->s.weapon ==
            G2_MODEL_PART) { // keep limbs from blocking elevators.  Kill the
                             // limb and keep moving.
      G_FreeEntity(check);
      continue;
    }

    if (check->s.eFlags &
        EF_DROPPEDWEAPON) { // keep dropped weapons from blocking elevators.
                            // Kill the weapon and keep moving.
      G_FreeEntity(check);
      continue;
    }
    ...
    if (check->s.eType == ET_NPC // an NPC
        ...
        && !(check->flags &
             FL_NOTARGET)) // NPC isn't still spawned or in no target mode.
...
void Use_BinaryMover(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  if (!ent->use) { // I cannot be used anymore, must be a door with a wait of -1
                   // that's opened.
    return;
  }
...
static void Touch_DoorTriggerSpectator(gentity_t *ent, gentity_t *other,
  ...
  trap->Trace(&tr, origin, pMins, pMaxs, origin, other->s.number,
              other->clipmask, qfalse, 0, 0);
...
void Touch_DoorTrigger(gentity_t *ent, gentity_t *other, trace_t *trace) {
  ...
  if (!ent->genericValue14 && (!ent->parent || !ent->parent->genericValue14)) {
    ...
    if (other->client && other->s.number < MAX_CLIENTS &&
        other->client->ps
            .m_iVehicleNum) { // can't open a door while on a vehicle
      return;
    }
  }
  ...
  if (ent->parent->spawnflags &
    ...
    if (!ent->parent->alliedTeam // we don't have a "teamallow" team
        ...
        other->client->sess.sessionTeam !=
            ent->parent
                ->alliedTeam) // it is a client, but it's not on the right team
    {
      return;
    } else { // temporarily unlock us while we call Use_BinaryMover (so it
  ...
  if (ent->parent->moverState != MOVER_1TO2) { // Door is not already opening
    // if ( ent->parent->moverState == MOVER_POS1 || ent->parent->moverState ==
    // MOVER_2TO1 )
  ...
  if (G_EntIsDoor(entityNum)) {
    ...
    if (ent->targetname) { // find out what is targetting it
      ...
      if (owner && (owner->flags &
                    FL_INACTIVE)) { // owning auto-created trigger is inactive
        return qfalse;
      }
    }
    if (!(ent->flags & FL_INACTIVE) && // assumes that the reactivate trigger
...
/*QUAKED func_door (0 .5 .8) ? START_OPEN FORCE_ACTIVATE CRUSHER TOGGLE LOCKED x
PLAYER_USE INACTIVE START_OPEN	the door to moves to its destination when
...
after this. PLAYER_USE	Player can use it with the use button INACTIVE	must be
...
(default is 0) "teamallow" even if locked, this team can always open and close
it just by walking up to it 0 - none (locked to everyone) 1 - red 2 - blue
...
void SP_func_door(gentity_t *ent) {
  ...
  if (!(ent->flags & FL_TEAMSLAVE)) {
    ...
    if (!(ent->spawnflags & MOVER_LOCKED) &&
      ...
      if (ent->spawnflags &
          MOVER_FORCE_ACTIVATE) { // so we know it's push/pullable on the client
        ent->s.bolt1 = 1;
      }
...
void Think_SetupTrainTargets(gentity_t *ent) {
  ...
  for (path = ent->nextTrain; path != start; path = next) {
    ...
    if (!path->target) {
      //			trap->Printf( "Train corner at %s without a
...
PLAYER_USE INACTIVE A train is a mover that moves between path_corner target
points. Trains MUST HAVE AN ORIGIN BRUSH. The train spawns at the first target
it is pointing at. CRUSH_THROUGH spawnflag combined with a dmg value will make
the train pass through entities and damage them on contact as well. "model2"
.md3 model to also draw "speed"		default 100 "dmg"		default
2 "target"	next path corner "color"		constantLight color
"light"		constantLight radius
*/
...
and models. "model2"	.md3 model to also draw "model2scale"	percent of
normal scale (on all x y and z axii) to scale the model2 if there is one. 100 is
normal scale, min is 1 (100 times smaller than normal), max is 1000 (ten times
normal). "color"		constantLight color
"light"		constantLight radius
...
void SP_func_static(gentity_t *ent) {
  ...
  if (ent->spawnflags & 2048) { // yes this is very very evil, but for now
    ...
    ent->r.svFlags |=
        SVF_BROADCAST; // I need to rotate something that is huge and it's
  ...
  if ((ent->spawnflags & 1) ||
      (ent->spawnflags & 2)) { // so we know it's push/pullable on the client
    ent->s.bolt1 = 1;
  }
  ...
  if (level.mBSPInstanceDepth) { // this means that this guy will never be
                                 // updated, moved, changed, etc.
...
/*QUAKED func_rotating (0 .5 .8) ? START_ON RADAR X_AXIS Y_AXIS IMPACT x
PLAYER_USE INACTIVE RADAR - shows up on Radar in Siege mode (good for asteroids
when you're in a ship) IMPACT - Kills anything on impact when rotating (10000
points of damage, unless specified otherwise)
...
"color"			constantLight color
"light"			constantLight radius
"spinangles"	instead of using "speed", you can set use this to set rotation
on all 3 axes (pitch yaw and roll)
...
"showhealth" if non-0, will display the health bar on the hud when the crosshair
is over this ent (in siege) "teamowner" in siege this will specify which team
this thing is "owned" by. To that team the crosshair will highlight green, to
the other team it will highlight red.
...
void SP_func_rotating(gentity_t *ent) {
  ...
  { // show up on Radar at close range and play impact sound when close...?
    // Range based on my size
...
void G_MiscModelExplosion(vec3_t mins, vec3_t maxs, int size,
...
void G_Chunks(int owner, vec3_t origin, const vec3_t normal, const vec3_t mins,
  ...
  // Now it's time to cram everything horribly into the entitystate of an event
  // entity.
  te->s.owner = owner;
...
void funcBBrushDieGo(gentity_t *self) {
  ...
  G_Chunks(self->s.number, org, dir, self->r.absmin, self->r.absmax, 300,
           numChunks, chunkType, 0, (scale * self->mass));
...
void funcBBrushPain(gentity_t *self, gentity_t *attacker, int damage) {
  ...
  if (self->material == MAT_DRK_STONE || self->material == MAT_LT_STONE ||
      self->material == MAT_GREY_STONE || self->material == MAT_SNOWY_ROCK) {
    vec3_t org, dir;
    float scale;
    int numChunks;
    VectorSubtract(self->r.absmax, self->r.absmin, org); // size
    ...
    if (self->radius > 0.0f) {
      // designer wants to scale number of chunks, helpful because the above
      // scale code is far from perfect
      //	I do this after the scale calculation because it seems that the
      //chunk size generally seems to be very close, it's just the number of
      //chunks is a bit weak
      numChunks = ceil(numChunks * self->radius);
    }
    G_Chunks(self->s.number, org, dir, self->r.absmin, self->r.absmax, 300,
             numChunks, self->material, 0, (scale * self->mass));
  }
...
===============================================================================
*/
void GlassDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
              int damage, int mod) {
  gentity_t *te;
  vec3_t dif;
...
void GlassDie_Old(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                  int damage, int mod) {
  gentity_t *te;
  vec3_t dif;
...
fire this target every time it is toggled OFF "model2"	.md3 model to also draw
"color"		constantLight color
"light"		constantLight radius
"usescript" script to run when turned on
"deathscript"  script to run when turned off
"wait"		amount of time before the object is usable again (only valid
with ALWAYS_ON flag) "health"	if it has health, it will be used whenever shot
at/killed - if you want it to only be used once this way, set health to 1
"endframe"	Will make it animate to next shader frame when used, not turn
...
void SP_func_usable(gentity_t *self) {
  ...
  {//FIXME: need to be able to do change to something when it's done?  Or not be
  usable until it's done? self->s.eFlags |= EF_ANIM_ONCE;
  }
  */
