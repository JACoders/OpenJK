...
static const char
    *psTagStrings[TAG_COUNT + 1] = // +1 because TAG_COUNT will itself become a
                                   // string here. Oh well.
    {
#include "tags.h"
};
...
// if you change ANYTHING in this structure, be sure to update the tables below
// using DEF_STATIC...
//
...
// Scans through the linked list of mallocs and makes sure no data has been
// overwritten
...
int Z_Validate(void) {
  ...
  while (pMemory) {
    ...
    if (pMemory->eTag != TAG_IMAGE_T && pMemory->eTag != TAG_MODEL_MD3 &&
        pMemory->eTag != TAG_MODEL_GLM &&
        pMemory->eTag !=
            TAG_MODEL_GLA) // don't bother with disk caches as they've already
                           // been hit or will be thrown out next
    {
...
#define DEF_STATIC(_char)                                                      \
  {ZONE_MAGIC, TAG_STATIC, 2, NULL, NULL, "<static>", 0, "", 0},               \
      {_char, '\0'}, {                                                         \
    ZONE_MAGIC                                                                 \
  }
#else
...
const static StaticMem_t gEmptyString = {DEF_STATIC('\0')};

const static StaticMem_t gNumberString[] = {
    {DEF_STATIC('0')}, {DEF_STATIC('1')}, {DEF_STATIC('2')}, {DEF_STATIC('3')},
    {DEF_STATIC('4')}, {DEF_STATIC('5')}, {DEF_STATIC('6')}, {DEF_STATIC('7')},
    {DEF_STATIC('8')}, {DEF_STATIC('9')},
};
...
#ifdef DEBUG_ZONE_ALLOCS
void *_D_Z_Malloc(int iSize, memtag_t eTag, qboolean bZeroit,
                  const char *psFile, int iLine)
#else
...
#endif
{
  ...
  // Add in tracking info and round to a longword...  (ignore longword aligning
  // now we're not using contiguous blocks)
  //
  //	int iRealSize = (iSize + sizeof(zoneHeader_t) + sizeof(zoneTail_t) + 3)
  //& 0xfffffffc;
  ...
  while (pMemory == NULL) {
    if (gbMemFreeupOccured) {
      Sys_Sleep(1000); // sleep for a second, so Windows has a chance to shuffle
                       // mem to de-swiss-cheese it
    }
    ...
    if (!pMemory) {
      // new bit, if we fail to malloc memory, try dumping some of the cached
      // stuff that's non-vital and try again...
      //
      ...
      if (CM_DeleteCachedMap(qfalse)) {
        ...
        continue; // we've just ditched a whole load of memory, so try again
                  // with the malloc
      }

      // ditch any sounds not used on this level...
      //
      extern qboolean SND_RegisterAudio_LevelLoadEnd(
          qboolean bDeleteEverythingNotUsedThisLevel);
      if (SND_RegisterAudio_LevelLoadEnd(qtrue)) {
        gbMemFreeupOccured = qtrue;
        continue; // we've dropped at least one sound, so try again with the
                  // malloc
      }

      // ditch any image_t's (and associated GL texture mem) not used on this
      // level...
      //
      if (re.RegisterImages_LevelLoadEnd()) {
        gbMemFreeupOccured = qtrue;
        continue; // we've dropped at least one image, so try again with the
                  // malloc
      }
      ...
      // as a last panic measure, dump all the audio memory, but not if we're in
      // the audio loader
      //	(which is annoying, but I'm not sure how to ensure we're not
      //dumping any memory needed by the sound 	currently being loaded if that
      //was the case)...
      //
      // note that this keeps querying until it's freed up as many bytes as the
      // requested size, but freeing
      //	several small blocks might not mean that one larger one is
      //satisfiable after freeup, however that'll 	just make it go round again
      //and try for freeing up another bunch of blocks until the total is
      //satisfied 	again (though this will have freed twice the requested amount
      //in that case), so it'll either work 	eventually or not free up enough and
      //drop through to the final ERR_DROP. No worries...
      //
      extern qboolean gbInsideLoadSound;
      extern int SND_FreeOldestSound(
          void); // I had to add a void-arg version of this because of link
                 // issues, sigh
      ...
      // sigh, dunno what else to try, I guess we'll have to give up and report
      // this as an out-of-mem error...
      //
      // findlabel:  "recovermem"

      Com_Printf(S_COLOR_RED
                 "Z_Malloc(): Failed to alloc %d bytes (TAG_%s) !!!!!\n",
                 iSize, psTagStrings[eTag]);
      Z_Details_f();
      Com_Error(
          ERR_FATAL,
          "(Repeat): Z_Malloc(): Failed to alloc %d bytes (TAG_%s) !!!!!\n",
          iSize, psTagStrings[eTag]);
      return NULL;
    }
  }
...
#ifdef DEBUG_ZONE_ALLOCS
  Q_strncpyz(pMemory->sSrcFileBaseName, _D_Z_Filename_WithoutPath(psFile),
             sizeof(pMemory->sSrcFileBaseName));
...
int openjk_minizip_free(void *to_free) { return Z_Free(to_free); }

// used during model cacheing to save an extra malloc, lets us morph the
// disk-load buffer then
...
static int Zone_FreeBlock(zoneHeader_t *pMemory) {
  ...
  if (pMemory->eTag !=
      TAG_STATIC) // belt and braces, should never hit this though
  {
    // Update stats...
    //
    TheZone.Stats.iCount--;
...
int Z_Free(void *pvAddress) {
  ...
  if (pMemory->iMagic == INT_ID('F', 'R', 'E', 'E')) {
    Com_Error(
        ERR_FATAL,
        "Z_Free(%s): Block already-freed, or not allocated through Z_Malloc!",
        pvAddress);
    return -1;
  }
#endif
  ...
  if (iAllocCount <= 0) {
    Com_Error(
        ERR_FATAL,
        "Z_Free(): Block already-freed, or not allocated through Z_Malloc!");
    return -1;
  }
#endif
...
int Z_MemSize(memtag_t eTag) { return TheZone.Stats.iSizesPerTag[eTag]; }
...
#else
void *S_Malloc(int iSize) { return Z_Malloc(iSize, TAG_SMALL, qfalse); }
#endif
...
static void Z_MemRecoverTest_f(void) {
  ...
  while (1) {
    ...
    Z_Malloc(iThisMalloc, TAG_SPECIAL_MEM_TEST,
             qfalse); // and lose, just to consume memory
...
static void Z_Details_f(void) {
  ...
  for (int i = 0; i < TAG_COUNT; i++) {
    ...
    if (iThisCount) {
      // can you believe that using %2.2f as a format specifier doesn't bloody
      // work?
      //	It ignores the left-hand specifier. Sigh, now I've got to do
      //shit like this...
      //
      ...
      Com_Printf("%20s %9d (%2d.%02dMB) in %6d blocks (%9d Bytes/block)\n",
                 psTagStrings[i], iThisSize, iSize, iRemainder, iThisCount,
...
#ifdef DEBUG_ZONE_ALLOCS
typedef std::map<sDebugString_t, int>
    LabelRefCount_t; // yet another place where Gil's string class works and
                     // MS's doesn't
...
static void Z_TagDebug_f(void) {
  ...
  if (psTAGName[0]) {
    ...
    Com_Printf("Usage: 'zone_tagdebug [#snap] <tag>', e.g. TAG_GHOUL2, TAG_ALL "
               "(careful!)\n");
    return;
  }

  Com_Printf("Dumping debug data for tag \"%s\"...%s\n\n", psTagStrings[eTag],
             bSnapShotTestActive ? "( since snapshot only )" : "");

  Com_Printf("%8s", " "); // to compensate for code further down:
  ...
  if (bSnapShotTestActive) {
    // dec ref counts in last snapshot for all current blocks (which will make
    // new stuff go negative)
    //
    zoneHeader_t *pMemory = TheZone.Header.pNext;
    while (pMemory) {
      if (pMemory->eTag == eTag || eTag == TAG_ALL) {
        AllTagBlockLabels_Local[psTagStrings[pMemory->eTag]]
  ...
  while (pMemory) {
    if ((pMemory->eTag == eTag || eTag == TAG_ALL) &&
      ...
      Com_Printf(" %9d (%2d.%02dMB) File: \"%s\", Line: %d\n", pMemory->iSize,
                 iSize, iRemainder, pMemory->sSrcFileBaseName,
      ...
      if (bSnapShotTestActive) {
        // bump ref count so we only 1 warning per new string, not for every one
        // sharing that label...
        //
        AllTagBlockLabels_Local[psTagStrings[pMemory->eTag]]
                               [pMemory->sOptionalLabel]++;
  ...
  Com_Printf("( %d blocks listed, %d bytes (%.2fMB) total )\n", iBlocksListed,
             iTotalSize, (float)iTotalSize / 1024.0f / 1024.0f);
}
#endif
...
void Com_ShutdownZoneMemory(void) {
  ...
  if (TheZone.Stats.iCount) {
    ...
    // assert(!TheZone.Stats.iCount);	// These aren't really problematic per se,
    // it's just warning us that we're freeing extra
    // assert(!TheZone.Stats.iCurrent);  // memory that is in the zone manager
    // (but not actively tracked..) so if anything, zone_*
    // commands will just simply be wrong in displaying bytes, but in my tests,
    // it's only off by like 10 bytes / 1 block, which isn't a real problem
    // --eez
    if (TheZone.Stats.iCount < 0) {
      Com_Printf(S_COLOR_YELLOW "WARNING: Freeing %d extra blocks (%d bytes) "
                                "not tracked by the zone manager\n",
