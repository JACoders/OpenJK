...
extern void NPC_TempLookTarget(gentity_t *self, int lookEntNum, int minLookTime,
                               int maxLookTime);
extern qboolean G_ExpandPointToBBox(vec3_t point, const vec3_t mins,
                                    const vec3_t maxs, int ignore,
                                    int clipmask);
extern qboolean NPC_CheckEnemyStealth(void);
extern void G_SoundOnEnt(gentity_t *ent, soundChannel_t channel,
...
extern qboolean G_GetHitLocFromSurfName(gentity_t *ent, const char *surfName,
                                        int *hitLoc, vec3_t point, vec3_t dir,
                                        vec3_t bladeDir, int mod);
extern qboolean WP_ForcePowerUsable(gentity_t *self, forcePowers_t forcePower);
extern qboolean WP_ForcePowerAvailable(gentity_t *self,
                                       forcePowers_t forcePower,
                                       int overrideAmt);
extern void WP_ForcePowerStop(gentity_t *self, forcePowers_t forcePower);
extern void WP_DeactivateSaber(gentity_t *self,
                               qboolean clearLength); // clearLength = qfalse
...
extern int WP_GetVelocityForForceJump(gentity_t *self, vec3_t jumpVel,
                                      usercmd_t *ucmd);
...
static int jediSpeechDebounceTime[TEAM_NUM_TEAMS]; // used to stop several jedi
                                                   // from speaking all at once
// Local state enums
enum {
  LSTATE_NONE = 0,
  LSTATE_UNDERFIRE,
...
void Jedi_PlayBlockedPushSound(gentity_t *self) {
  ...
  } else if (self->health > 0 && self->NPC &&
...
void NPC_Jedi_PlayConfusionSound(gentity_t *self) {
  if (self->health > 0) {
    if (self->client && (self->client->NPC_class == CLASS_TAVION ||
...
qboolean Jedi_CultistDestroyer(gentity_t *self) {
  ...
  if (self->client->NPC_class == CLASS_REBORN && self->s.weapon == WP_MELEE &&
...
extern void G_CreateG2AttachedWeaponModel(gentity_t *ent,
                                          const char *weaponModel, int boltNum,
                                          int weaponNum);
...
void Boba_ChangeWeapon(int wp) {
  ...
  NPC_ChangeWeapon(wp);
  G_AddEvent(NPCS.NPC, EV_GENERAL_SOUND,
...
void WP_ResistForcePush(gentity_t *self, gentity_t *pusher,
  ...
  if (!self || self->health <= 0 || !self->client || !pusher ||
      !pusher->client) {
    return;
  }
  if (((self->s.number >= 0 && self->s.number < MAX_CLIENTS) ||
  ...
  if (!runningResist && self->client->ps.groundEntityNum != ENTITYNUM_NONE &&
      !BG_SpinningSaberAnim(self->client->ps.legsAnim) &&
      !BG_FlippingAnim(self->client->ps.legsAnim) &&
      !PM_RollingAnim(self->client->ps.legsAnim) &&
      !PM_InKnockDown(&self->client->ps) &&
      !BG_CrouchAnim(
          self->client->ps.legsAnim)) { // if on a surface and not in a spin or
                                        // flip, play full body resist
    parts = SETANIM_BOTH;
  } else { // play resist just in torso
    parts = SETANIM_TORSO;
  }
  NPC_SetAnim(self, parts, BOTH_RESISTPUSH,
    ...
    if (!runningResist) {
      ...
      if (self->client->ps.fd.forcePowersActive & (1 << FP_SPEED)) {
        self->client->ps.weaponTime =
            floor(self->client->ps.weaponTime * tFVal);
      }
      ...
      // self->forcePushTime = level.time + 600; // let the push effect last for
      // 600 ms rwwFIXMEFIXME: Do this?
    } else {
  ...
  self->client->ps.powerups[PW_DISINT_4] =
      level.time + self->client->ps.torsoTimer + 500;
...
qboolean Boba_StopKnockdown(gentity_t *self, gentity_t *pusher, vec3_t pushDir,
                            qboolean forceKnockdown) // forceKnockdown = qfalse
{
  ...
  if (Q_irand(0, 2)) { // flip or roll with it
    ...
    if (!Q_irand(0, 1)) { // flip
      self->client->ps.fd.forceJumpCharge =
          280; // FIXME: calc this intelligently?
...
qboolean Boba_Flying(gentity_t *self) {
  return ((qboolean)(self->client->ps.eFlags2 &
                     EF2_FLYING)); // moveType==MT_FLYSWIM));
}
...
void Boba_FireFlameThrower(gentity_t *self) {
  ...
  trap->G2API_GetBoltMatrix(self->ghoul2, 0, self->client->renderInfo.handLBolt,
                            &boltMatrix, self->r.currentAngles,
                            self->r.currentOrigin, level.time, NULL,
                            self->modelScale);
  ...
  trap->Trace(&tr, start, traceMins, traceMaxs, end, self->s.number, MASK_SHOT,
              qfalse, 0, 0);
  ...
  if (tr.entityNum < ENTITYNUM_WORLD && traceEnt->takedamage) {
    G_Damage(traceEnt, self, self, dir, tr.endpos, damage,
             DAMAGE_NO_ARMOR | DAMAGE_NO_KNOCKBACK |
                 /*DAMAGE_NO_HIT_LOC|*/ DAMAGE_IGNORE_TEAM,
             MOD_LAVA);
...
void Boba_StartFlameThrower(gentity_t *self) {
  ...
  if ( fire != NULL )
  {
          mdxaBone_t	boltMatrix;
          vec3_t		org, dir, ang;
          trap->G2API_GetBoltMatrix( NPC->ghoul2, NPC->playerModel,
  NPC->handRBolt, &boltMatrix, NPC->r.currentAngles, NPC->r.currentOrigin,
  (cg.time?cg.time:level.time), NULL, NPC->s.modelScale );
  ...
  );//"env/small_fire" fx_explosion_trail_link( fire ); fx_explosion_trail_use(
  fire, NPC, NPC );
  ...
  */
  ...
  trap->G2API_GetBoltMatrix(
      NPCS.NPC->ghoul2, 0, NPCS.NPC->client->renderInfo.handRBolt, &boltMatrix,
      NPCS.NPC->r.currentAngles, NPCS.NPC->r.currentOrigin, level.time, NULL,
      NPCS.NPC->modelScale);
...
void Boba_FireDecide(void) {
  ...
  enemyDist = DistanceSquared(NPCS.NPC->r.currentOrigin,
                              NPCS.NPC->enemy->r.currentOrigin);

  VectorSubtract(NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin,
                 enemyDir);
  VectorNormalize(enemyDir);
  AngleVectors(NPCS.NPC->client->ps.viewangles, shootDir, NULL, NULL);
  dot = DotProduct(enemyDir, shootDir);
  if (dot > 0.5f || (enemyDist * (1.0f - dot)) <
                        10000) { // enemy is in front of me or they're very
                                 // close and not behind me
    enemyInFOV = qtrue;
  }

  if ((enemyDist < (128 * 128) && enemyInFOV) ||
      !TIMER_Done(NPCS.NPC, "flameTime")) { // flamethrower
  ...
  {                                               // enemy within 128
    if ((NPCS.NPC->client->ps.weapon == WP_FLECHETTE ||
         NPCS.NPC->client->ps.weapon == WP_REPEATER) &&
        (NPCS.NPCInfo->scriptFlags &
         SCF_ALT_FIRE)) { // shooting an explosive, but enemy too close, switch
                          // to primary fire
      NPCS.NPCInfo->scriptFlags &= ~SCF_ALT_FIRE;
      // FIXME: we can never go back to alt-fire this way since, after this, we
      // don't know if we were initially supposed to use alt-fire or not...
    }
  } else if (enemyDist > 65536) // 256 squared
  {
    if (NPCS.NPC->client->ps.weapon ==
        WP_DISRUPTOR) { // sniping... should be assumed
      if (!(NPCS.NPCInfo->scriptFlags & SCF_ALT_FIRE)) { // use primary fire
        NPCS.NPCInfo->scriptFlags |= SCF_ALT_FIRE;
  ...
  if (TIMER_Done(NPCS.NPC, "nextAttackDelay") &&
    ...
    if (NPC_ClearLOS4(NPCS.NPC->enemy)) {
      ...
      if (NPCS.NPC->client->ps.weapon == WP_NONE) {
        ...
        if ((NPCS.NPC->client->ps.weapon == WP_ROCKET_LAUNCHER ||
             (NPCS.NPC->client->ps.weapon == WP_FLECHETTE &&
              (NPCS.NPCInfo->scriptFlags & SCF_ALT_FIRE))) &&
        ...
        } else if (enemyInFOV) { // if enemy is FOV, go ahead and check for
                                 // shooting
          int hit = NPC_ShotEntity(NPCS.NPC->enemy, impactPos);
          gentity_t *hitEnt = &g_entities[hit];

          if (hit == NPCS.NPC->enemy->s.number ||
              (hitEnt && hitEnt->client &&
               hitEnt->client->playerTeam == NPCS.NPC->client->enemyTeam) ||
              (hitEnt && hitEnt->takedamage &&
               ((hitEnt->r.svFlags & SVF_GLASS_BRUSH) || hitEnt->health < 40 ||
                NPCS.NPC->s.weapon ==
                    WP_EMPLACED_GUN))) { // can hit enemy or enemy ally or will
                                         // hit glass or other minor breakable
                                         // (or in emplaced gun), so shoot anyway
            enemyCS = qtrue;
            // NPC_AimAdjust( 2 );//adjust aim better longer we have clear shot
            // at enemy
            VectorCopy(NPCS.NPC->enemy->r.currentOrigin,
                       NPCS.NPCInfo->enemyLastSeenLocation);
          } else { // Hmm, have to get around this bastard
            // NPC_AimAdjust( 1 );//adjust aim better longer we can see enemy
            if (hitEnt && hitEnt->client &&
                hitEnt->client->playerTeam ==
                    NPCS.NPC->client
                        ->playerTeam) { // would hit an ally, don't fire!!!
              hitAlly = qtrue;
            } else { // Check and see where our shot *would* hit... if it's not
                     // close to the enemy (within 256?), then don't fire
            }
          }
        } else {
          enemyCS = qfalse; // not true, but should stop us from firing
    ...
    if (!enemyCS) { // if have a clear shot, always try
      ...
      if (!hitAlly      // we're not going to hit an ally
          && enemyInFOV // enemy is in our FOV //FIXME: or we don't have a clear
                        // LOS?
          && NPCS.NPCInfo->enemyLastSeenTime > 0) // we've seen the enemy
      {
        if (level.time - NPCS.NPCInfo->enemyLastSeenTime <
            10000) // we have seem the enemy in the last 10 seconds
        {
          if (!Q_irand(0, 10)) {
            ...
            if (VectorCompare(impactPos,
                              vec3_origin)) { // never checked ShotEntity this
                                              // frame, so must do a trace...
              trace_t tr;
              // vec3_t	mins = {-2,-2,-2}, maxs = {2,2,2};
              vec3_t forward, end;
              AngleVectors(NPCS.NPC->client->ps.viewangles, forward, NULL,
                           NULL);
              VectorMA(muzzle, 8192, forward, end);
              trap->Trace(&tr, muzzle, vec3_origin, vec3_origin, end,
                          NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0);
            ...
            if (dist < distThreshold) { // impact would be too close to me
              tooClose = qtrue;
                       ...
                       (NPCS.NPCInfo->group &&
                        level.time - NPCS.NPCInfo->group->lastSeenEnemyTime >
                            5000)) { // we've haven't seen them in the last 5
                                     // seconds
              ...
              dist = DistanceSquared(impactPos,
                                     NPCS.NPCInfo->enemyLastSeenLocation);
    ...
    if (NPCS.NPC->client->ps.weaponTime > 0) {
      ...
      if (TIMER_Done(NPCS.NPC, "nextAttackDelay")) {
        if (!(NPCS.NPCInfo->scriptFlags &
              SCF_FIRE_WEAPON)) // we've already fired, no need to do it again
                                // here
        {
          WeaponThink(qtrue);
        }
...
void Jedi_CheckCloak(void) {
  if (NPCS.NPC && NPCS.NPC->client &&
      NPCS.NPC->client->NPC_class == CLASS_SHADOWTROOPER) {
    if (!NPCS.NPC->client->ps.saberHolstered || NPCS.NPC->health <= 0 ||
        NPCS.NPC->client->ps.saberInFlight ||
        //	(NPC->client->ps.eFlags&EF_FORCE_GRIPPED) ||
        //	(NPC->client->ps.eFlags&EF_FORCE_DRAINED) ||
        NPCS.NPC->painDebounceTime >
            level.time) { // can't be cloaked if saber is on, or dead or saber
                          // in flight or taking pain or being gripped
      Jedi_Decloak(NPCS.NPC);
               ...
               //	&& !(NPC->client->ps.eFlags&EF_FORCE_DRAINED)
               && NPCS.NPC->painDebounceTime <
                      level.time) { // still alive, have saber in hand, not
                                    // taking pain and not being gripped
      Jedi_Cloak(NPCS.NPC);
    }
  }
}
...
static void Jedi_Aggression(gentity_t *self, int change) {
  ...
  if (self->client->playerTeam ==
      NPCTEAM_PLAYER) { // good guys are less aggressive
    upper_threshold = 7;
    lower_threshold = 1;
  ...
  // Com_Printf( "(%d) %s agg %d change: %d\n", level.time, self->NPC_type,
  // self->NPC->stats.aggression, change );
}

static void Jedi_AggressionErosion(int amt) {
  if (TIMER_Done(NPCS.NPC,
                 "roamTime")) { // the longer we're not alerted and have no
                                // enemy, the more our aggression goes down
    TIMER_Set(NPCS.NPC, "roamTime", Q_irand(2000, 5000));
    Jedi_Aggression(NPCS.NPC, amt);
  }

  if (NPCS.NPCInfo->stats.aggression < 4 ||
      (NPCS.NPCInfo->stats.aggression < 6 &&
       NPCS.NPC->client->NPC_class == CLASS_DESANN)) { // turn off the saber
...
void NPC_Jedi_RateNewEnemy(gentity_t *self, gentity_t *enemy) {
  ...
  case WP_SABER:
    ...
    break;
  case WP_BLASTER:
    if (DistanceSquared(self->r.currentOrigin, enemy->r.currentOrigin) <
        65536) // 256 squared
    {
  ...
  newAggression = ceil((healthAggression + weaponAggression +
                        (float)self->NPC->stats.aggression) /
                       3.0f);
...
static void Jedi_Rage(void) {
  Jedi_Aggression(NPCS.NPC,
                  10 - NPCS.NPCInfo->stats.aggression + Q_irand(-2, 2));
...
static qboolean Jedi_BattleTaunt(void) {
  if (TIMER_Done(NPCS.NPC, "chatter") && !Q_irand(0, 3) &&
      NPCS.NPCInfo->blockedSpeechDebounceTime < level.time &&
      jediSpeechDebounceTime[NPCS.NPC->client->playerTeam] < level.time) {
    int event = -1;
    if (NPCS.NPC->client->playerTeam == NPCTEAM_PLAYER && NPCS.NPC->enemy &&
        NPCS.NPC->enemy->client &&
        NPCS.NPC->enemy->client->NPC_class ==
            CLASS_JEDI) { // a jedi fighting a jedi - training
      if (NPCS.NPC->client->NPC_class == CLASS_JEDI &&
          NPCS.NPCInfo->rank == RANK_COMMANDER) { // only trainer taunts
        event = EV_TAUNT1;
      }
    ...
    if (event != -1) {
      ...
      jediSpeechDebounceTime[NPCS.NPC->client->playerTeam] =
          NPCS.NPCInfo->blockedSpeechDebounceTime = level.time + 6000;
      TIMER_Set(NPCS.NPC, "chatter", Q_irand(5000, 10000));

      if (NPCS.NPC->enemy && NPCS.NPC->enemy->NPC &&
          NPCS.NPC->enemy->s.weapon == WP_SABER && NPCS.NPC->enemy->client &&
          NPCS.NPC->enemy->client->NPC_class ==
              CLASS_JEDI) { // Have the enemy jedi say something in response
                            // when I'm done?
      }
      return qtrue;
    }
...
static qboolean Jedi_ClearPathToSpot(vec3_t dest, int impactEntNum) {
  ...
  VectorSet(mins, NPCS.NPC->r.mins[0], NPCS.NPC->r.mins[1],
            NPCS.NPC->r.mins[2] + STEPSIZE);

  trap->Trace(&trace, NPCS.NPC->r.currentOrigin, mins, NPCS.NPC->r.maxs, dest,
  ...
  if (trace.fraction < 1.0f) { // hit something
    if (impactEntNum != ENTITYNUM_NONE &&
        trace.entityNum == impactEntNum) { // hit what we're going after
      return qtrue;
    } else {
      return qfalse;
    }
  ...
  // Now at intervals of my size, go along the trace and trace down STEPSIZE to
  // make sure there is a solid floor.
  VectorSubtract(dest, NPCS.NPC->r.currentOrigin, dir);
  dist = VectorNormalize(dir);
  if (dest[2] > NPCS.NPC->r.currentOrigin[2]) { // going up, check for steps
    drop = STEPSIZE;
  } else { // going down or level, check for moderate drops
    drop = 64;
  }
  for (i = NPCS.NPC->r.maxs[0] * 2; i < dist;
       i += NPCS.NPC->r.maxs[0] *
            2) { // FIXME: does this check the last spot, too?  We're assuming
                 // that should be okay since the enemy is there?
    VectorMA(NPCS.NPC->r.currentOrigin, i, dir, start);
    VectorCopy(start, end);
    end[2] -= drop;
    trap->Trace(&trace, start, mins, NPCS.NPC->r.maxs, end, NPCS.NPC->s.number,
                NPCS.NPC->clipmask, qfalse, 0, 0); // NPC->r.mins?
    if (trace.fraction < 1.0f || trace.allsolid ||
        trace.startsolid) { // good to go
      continue;
    }
...
qboolean NPC_MoveDirClear(int forwardmove, int rightmove, qboolean reset) {
  ...
  if (NPCS.ucmd.upmove > 0 ||
      NPCS.NPC->client->ps.fd.forceJumpCharge) { // Going to jump
    // Com_Printf( "%d skipping walk-cliff check (going to jump)\n", level.time
    // );
    return qtrue;
  }

  if (NPCS.NPC->client->ps.groundEntityNum == ENTITYNUM_NONE) { // in the air
    // Com_Printf( "%d skipping walk-cliff check (in air)\n", level.time );
    return qtrue;
  }
  /*
  if ( fabs( AngleDelta( NPC->r.currentAngles[YAW], NPCInfo->desiredYaw ) )
  < 5.0 )//!ucmd.angles[YAW] )
  {//Not turning much, don't do this
          ...
          //  straight into a wall or off	a cliff unless we really wanted
  to? return;
  }
  */

  // FIXME: to really do this right, we'd have to actually do a pmove to predict
  // where we're going to be... maybe this should be a flag and pmove handles it
  // and sets a flag so AI knows NEXT frame?  Or just incorporate current
  // velocity, runspeed and possibly friction?
  ...
  trap->Trace(&trace, NPCS.NPC->r.currentOrigin, mins, NPCS.NPC->r.maxs,
              testPos, NPCS.NPC->s.number,
              NPCS.NPC->clipmask | CONTENTS_BOTCLIP, qfalse, 0, 0);
  if (trace.allsolid ||
      trace.startsolid) { // hmm, trace started inside this brush... how do we
                          // decide if we should continue?
    // FIXME: what do we do if we start INSIDE a CONTENTS_BOTCLIP? Try the trace
    // again without that in the clipmask?
    if (reset) {
      trace.fraction = 1.0f;
    }
  ...
  if (trace.fraction <
      0.6) { // Going to bump into something very close, don't move, just turn
    if ((NPCS.NPC->enemy && trace.entityNum == NPCS.NPC->enemy->s.number) ||
        (NPCS.NPCInfo->goalEntity &&
         trace.entityNum == NPCS.NPCInfo->goalEntity->s
                                .number)) { // okay to bump into enemy or goal
  ...
  if (NPCS.NPCInfo->goalEntity) {
    if (NPCS.NPCInfo->goalEntity->r.currentOrigin[2] <
        NPCS.NPC->r.currentOrigin[2]) { // goal is below me, okay to step off at
                                        // least that far plus stepheight
      bottom_max += NPCS.NPCInfo->goalEntity->r.currentOrigin[2] -
                    NPCS.NPC->r.currentOrigin[2];
    }
  }
  VectorCopy(trace.endpos, testPos);
  testPos[2] += bottom_max;

  trap->Trace(&trace, trace.endpos, mins, NPCS.NPC->r.maxs, testPos,
              NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0, 0);

  // FIXME:Should we try to see if we can still get to our goal using the
  // waypoint network from this trace.endpos? OR: just put NPC clip brushes on
  // these edges (still fall through when die)
  ...
  if (trace.fraction < 1.0) { // Not going off a cliff
    // FIXME: what if plane.normal is sloped?  We'll slide off, not land... plus
    // this doesn't account for slide-movement... Com_Printf( "%d walk off cliff
    // okay will hit entnum %d at dropdist of %4.2f\n", level.time,
    // trace.entityNum, (trace.fraction*bottom_max) );
    return qtrue;
  }

  // going to fall at least bottom_max, don't move, just turn... is this bad,
  // though?  What if we want them to drop off?
...
static void Jedi_Move(gentity_t *goal, qboolean retreat) {
  ...
  // FIXME: temp retreat behavior- should really make this toward a safe spot or
  // maybe to outflank enemy
  if (retreat) { // FIXME: should we trace and make sure we can go this way?  Or
                 // somehow let NPC_MoveToGoal know we want to retreat and have
                 // it handle it?
...
static qboolean Jedi_Hunt(void) {
  ...
  if (NPCS.NPCInfo->stats.aggression > 1) { // approach enemy
    NPCS.NPCInfo->combatMove = qtrue;
    if (!(NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) {
      NPC_UpdateAngles(qtrue, qtrue);
      return qtrue;
    } else {
      if (NPCS.NPCInfo->goalEntity == NULL) { // hunt
...
static void Jedi_Retreat(void) {
  if (!TIMER_Done(NPCS.NPC, "noRetreat")) { // don't actually move
    return;
  }
...
static void Jedi_AdjustSaberAnimLevel(gentity_t *self, int newLevel) {
  ...
  // FIXME: each NPC shold have a unique pattern of behavior for the order in
  // which they
  ...
  if (self->client->playerTeam == NPCTEAM_ENEMY) {
    if (self->NPC->rank == RANK_CIVILIAN ||
        self->NPC->rank ==
            RANK_LT_JG) { // grunt and fencer always uses quick attacks
  ...
  // use the different attacks, how often they switch and under what
  // circumstances
  if (newLevel >
      self->client->ps.fd.forcePowerLevel[FP_SABER_OFFENSE]) { // cap it
...
static void Jedi_CheckDecreaseSaberAnimLevel(void) {
  if (!NPCS.NPC->client->ps.weaponTime &&
      !(NPCS.ucmd.buttons &
        (BUTTON_ATTACK | BUTTON_ALT_ATTACK))) { // not attacking
    if (TIMER_Done(NPCS.NPC, "saberLevelDebounce") && !Q_irand(0, 10)) {
      ...
      Jedi_AdjustSaberAnimLevel(
          NPCS.NPC, Q_irand(FORCE_LEVEL_1, FORCE_LEVEL_3)); // random
...
static void Jedi_CombatDistance(
    int enemy_dist) { // FIXME: for many of these checks, what we really want is
                      // horizontal distance to enemy
  if (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_GRIP) &&
      NPCS.NPC->client->ps.fd.forcePowerLevel[FP_GRIP] >
          FORCE_LEVEL_1) { // when gripping, don't move
    return;
  } else if (!TIMER_Done(
                 NPCS.NPC,
                 "gripping")) { // stopped gripping, clear timers just in case
  ...
  if (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_DRAIN) &&
      NPCS.NPC->client->ps.fd.forcePowerLevel[FP_DRAIN] >
          FORCE_LEVEL_1) { // when draining, don't move
    return;
  } else if (!TIMER_Done(
                 NPCS.NPC,
                 "draining")) { // stopped draining, clear timers just in case
  ...
  if (NPCS.NPC->client->NPC_class == CLASS_BOBAFETT) {
    if (!TIMER_Done(NPCS.NPC, "flameTime")) {
      ...
      Jedi_Advance();
             ...
             NPCS.NPC->client->ps.saberBlocked !=
                 BLOCKED_PARRY_BROKEN) { // maintain distance
    if (enemy_dist < NPCS.NPC->client->ps.saberEntityDist) {
      Jedi_Retreat();
    } else if (enemy_dist > NPCS.NPC->client->ps.saberEntityDist &&
               enemy_dist > 100) {
      Jedi_Advance();
    }
    if (NPCS.NPC->client->ps.weapon == WP_SABER // using saber
        && NPCS.NPC->client->ps.saberEntityState ==
               SES_LEAVING // not returning yet
        && NPCS.NPC->client->ps.fd.forcePowerLevel[FP_SABERTHROW] >
               FORCE_LEVEL_1 // 2nd or 3rd level lightsaber
        && !(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED)) &&
        !(NPCS.NPC->client->ps.saberEventFlags &
          SEF_INWATER)) // saber not in water
    {                   // hold it out there
      NPCS.ucmd.buttons |= BUTTON_ALT_ATTACK;
      // FIXME: time limit?
    }
    ...
    // NPC->client->ps.torsoTimer < 2000 )
    else if (NPCS.NPC->client->ps.forceHandExtend == HANDEXTEND_JEDITAUNT &&
             (NPCS.NPC->client->ps.forceHandExtendTime - level.time) <
                 200) { // we're almost done with our special taunt
      // FIXME: this doesn't always work, for some reason
      if (!NPCS.NPC->client->ps.saberInFlight) {
        WP_ActivateSaber(NPCS.NPC);
      }
    }
  } else if (NPCS.NPC->client->ps.saberEventFlags &
             SEF_LOCK_WON) { // we won a saber lock, press the advantage
    ...
    if (NPCS.NPC->enemy->painDebounceTime + 2000 <
        level.time) { // the window of opportunity is gone
      NPCS.NPC->client->ps.saberEventFlags &= ~SEF_LOCK_WON;
    }
    // don't strafe?
    TIMER_Set(NPCS.NPC, "strafeLeft", -1);
    TIMER_Set(NPCS.NPC, "strafeRight", -1);
  } else if (NPCS.NPC->enemy->client && NPCS.NPC->enemy->s.weapon == WP_SABER &&
             NPCS.NPC->enemy->client->ps.saberLockTime > level.time &&
             NPCS.NPC->client->ps.saberLockTime <
                 level.time) { // enemy is in a saberLock and we are not
    if (enemy_dist < 64) {     // FIXME: maybe just pick another enemy?
      Jedi_Retreat();
    }
  }
  // rwwFIXMEFIXME: Give them the ability to do this again (turret needs to be
  // fixed up to allow it)
  else if (enemy_dist <= 64 &&
           ((NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE) ||
            (!Q_stricmp("Yoda", NPCS.NPC->NPC_type) &&
             !Q_irand(0,
                      10)))) { // can't use saber and they're in striking range
    if (!Q_irand(0, 5) &&
      ...
      if (((NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE) ||
           NPCS.NPC->client->pers.maxHealth - NPCS.NPC->health >
               NPCS.NPC->client->pers.maxHealth *
                   0.25f) // lost over 1/4 of our health or not firing
          && NPCS.NPC->client->ps.fd.forcePowersKnown &
                 (1 << FP_DRAIN)                            // know how to drain
    ...
    Jedi_Retreat();
             ...
             NPCS.NPC->client->pers.maxHealth - NPCS.NPC->health >
                 NPCS.NPC->client->pers.maxHealth *
                     0.25f // lost over 1/4 of our health
             && NPCS.NPC->client->ps.fd.forcePowersKnown &
                    (1 << FP_DRAIN) // know how to drain
             &&
             WP_ForcePowerAvailable(NPCS.NPC, FP_DRAIN, 20) // have enough power
             && !Q_irand(0, 10) &&
             InFront(NPCS.NPC->enemy->r.currentOrigin,
                     NPCS.NPC->r.currentOrigin, NPCS.NPC->client->ps.viewangles,
                     0.2f)) {
    TIMER_Set(NPCS.NPC, "draining", 3000);
    TIMER_Set(NPCS.NPC, "attackDelay", 3000);
    Jedi_Advance();
    ...
    if (NPCS.NPCInfo->stats.aggression < 4) { // back off and defend
      Jedi_Retreat();
    }
    ...
    if (NPCS.NPCInfo->stats.aggression < Q_irand(0, 20) &&
      ...
      if ((NPCS.NPC->client->ps.fd.forcePowersKnown & (1 << FP_HEAL)) != 0 &&
        ...
        ForceHeal(NPCS.NPC);
        usedForce = qtrue;
        // FIXME: check level of heal and know not to move or attack when
        // healing
      } else if ((NPCS.NPC->client->ps.fd.forcePowersKnown &
                  (1 << FP_PROTECT)) != 0 &&
                 (NPCS.NPC->client->ps.fd.forcePowersActive &
                  (1 << FP_PROTECT)) == 0 &&
                 Q_irand(0, 1)) {
        ...
        usedForce = qtrue;
      } else if ((NPCS.NPC->client->ps.fd.forcePowersKnown & (1 << FP_RAGE)) !=
                     0 &&
    ...
    if (enemy_dist >
        384) { // FIXME: check for enemy facing away and/or moving away
      if (!Q_irand(0, 10) &&
          NPCS.NPCInfo->blockedSpeechDebounceTime < level.time &&
          jediSpeechDebounceTime[NPCS.NPC->client->playerTeam] < level.time) {
        if (NPC_ClearLOS4(NPCS.NPC->enemy)) {
    ...
    if (NPCS.NPCInfo->stats.aggression > 0) { // approach enemy
      if (!usedForce) {
        Jedi_Advance();
      }
    }
  }
  // FIXME: enemy_dist calc needs to include all blade lengths, and include
  // distance from hand to start of blade....
  else if (enemy_dist > 50) // FIXME: not hardcoded- base on our reach
                            // (modelScale?) and saberLengthMax
  { // we're out of striking range and we are allowed to attack
    ...
    if (NPCS.NPC->enemy && NPCS.NPC->enemy->client &&
        (NPCS.NPC->enemy->client->ps.fd.forceGripBeingGripped >
         level.time)) { // They're being gripped, rush them!
      if (NPCS.NPC->enemy->client->ps.groundEntityNum !=
          ENTITYNUM_NONE) { // they're on the ground, so advance
        if (TIMER_Done(NPCS.NPC, "parryTime") ||
            NPCS.NPCInfo->rank > RANK_LT) { // not parrying
          if (enemy_dist > 200 ||
              !(NPCS.NPCInfo->scriptFlags &
                SCF_DONT_FIRE)) { // far away or allowed to use saber
            Jedi_Advance();
          }
        }
      }
      if (NPCS.NPCInfo->rank >= RANK_LT_JG && !Q_irand(0, 5) &&
          !(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED)) &&
          !(NPCS.NPC->client->ps.saberEventFlags &
            SEF_INWATER)) // saber not in water
      {                   // throw saber
        NPCS.ucmd.buttons |= BUTTON_ALT_ATTACK;
               ...
               NPCS.NPC->enemy->client->ps.saberInFlight &&
               /*NPC->enemy->client->ps.saber[0].Active()*/
               NPCS.NPC->enemy->client->ps
                   .saberEntityNum &&                  // enemy throwing saber
               NPCS.NPC->client->ps.weaponTime <= 0 && // I'm not busy
               WP_ForcePowerAvailable(NPCS.NPC, FP_GRIP,
                                      0) && // I can use the power
               !Q_irand(0, 10) && // don't do it all the time, averages to 1
                                  // check a second
               Q_irand(0, 6) <
                   g_npcspskill.integer && // more likely on harder diff
               Q_irand(RANK_CIVILIAN, RANK_CAPTAIN) <
                   NPCS.NPCInfo->rank) // more likely against harder enemies
    { // They're throwing their saber, grip them!
      ...
      if (NPCS.NPC->enemy && NPCS.NPC->enemy->client &&
          (NPCS.NPC->enemy->client->ps.fd.forcePowersActive &
           (1 << FP_GRIP))) { // They're choking someone, probably an ally, run
                              // at them and do some sort of attack
        if (NPCS.NPC->enemy->client->ps.groundEntityNum !=
            ENTITYNUM_NONE) { // they're on the ground, so advance
          if (TIMER_Done(NPCS.NPC, "parryTime") ||
              NPCS.NPCInfo->rank > RANK_LT) { // not parrying
      ...
      if (chanceScale &&
          (enemy_dist > Q_irand(100, 200) ||
           (NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE) ||
           (!Q_stricmp("Yoda", NPCS.NPC->NPC_type) && !Q_irand(0, 3))) &&
          enemy_dist < 500 &&
          (Q_irand(0, chanceScale * 10) < 5 ||
           (NPCS.NPC->enemy->client &&
            NPCS.NPC->enemy->client->ps.weapon != WP_SABER &&
            !Q_irand(0, chanceScale)))) { // else, randomly try some kind of
        ...
        if (((NPCS.NPCInfo->rank == RANK_ENSIGN ||
          ...
          if (WP_ForcePowerUsable(NPCS.NPC, FP_PULL) && !Q_irand(0, 2)) {
            ...
            // FIXME: check
            // forcePushRadius[NPC->client->ps.fd.forcePowerLevel[FP_PUSH]]
            ...
            if (Q_irand(0, 1)) {
              NPCS.ucmd.buttons |= BUTTON_ATTACK;
                      ...
                      ((NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE) &&
                       Q_stricmp("cultist_lightning", NPCS.NPC->NPC_type))) ||
                     Q_irand(0, 1)) {
            ForceLightning(NPCS.NPC);
            if (NPCS.NPC->client->ps.fd.forcePowerLevel[FP_LIGHTNING] >
                FORCE_LEVEL_1) {
              NPCS.NPC->client->ps.weaponTime =
                  Q_irand(1000, 3000 + (g_npcspskill.integer * 500));
              TIMER_Set(NPCS.NPC, "holdLightning",
                        NPCS.NPC->client->ps.weaponTime);
            }
          ...
          // rwwFIXMEFIXME: After new drain stuff from SP is in re-enable this.
          else if ((NPCS.NPC->health <
                        NPCS.NPC->client->ps.stats[STAT_MAX_HEALTH] * 0.75f &&
                    Q_irand(FORCE_LEVEL_0,
                            NPCS.NPC->client->ps.fd.forcePowerLevel[FP_DRAIN]) >
                        FORCE_LEVEL_1 &&
                    WP_ForcePowerUsable(NPCS.NPC, FP_DRAIN) &&
                    ((NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE) &&
                     Q_stricmp("cultist_drain", NPCS.NPC->NPC_type))) ||
                   Q_irand(0, 1)) {
            ForceDrain(NPCS.NPC);
            NPCS.NPC->client->ps.weaponTime =
                Q_irand(1000, 3000 + (g_npcspskill.integer * 500));
            ...
            TIMER_Set(NPCS.NPC, "attackDelay", NPCS.NPC->client->ps.weaponTime);
                     ...
                     NPCS.NPC->enemy &&
                     InFOV(NPCS.NPC->enemy, NPCS.NPC, 20, 30)) {
            // taunt
            if (TIMER_Done(NPCS.NPC, "chatter") &&
                jediSpeechDebounceTime[NPCS.NPC->client->playerTeam] <
                    level.time &&
      ...
      else if (NPCS.NPCInfo->stats.aggression > 5) { // approach enemy
        if (TIMER_Done(NPCS.NPC, "parryTime") ||
            NPCS.NPCInfo->rank > RANK_LT) { // not parrying
          if (!NPCS.NPC->enemy->client ||
              NPCS.NPC->enemy->client->ps.groundEntityNum !=
                  ENTITYNUM_NONE) { // they're on the ground, so advance
            if (enemy_dist > 200 ||
                !(NPCS.NPCInfo->scriptFlags &
                  SCF_DONT_FIRE)) { // far away or allowed to use saber
  ...
  } else { // we're not close enough to attack, but not far enough away to be
           // safe
    if (NPCS.NPCInfo->stats.aggression < 4) { // back off and defend
      Jedi_Retreat();
    } else if (NPCS.NPCInfo->stats.aggression > 5) { // try to get closer
      if (enemy_dist > 0 &&
          !(NPCS.NPCInfo->scriptFlags &
            SCF_DONT_FIRE)) { // we're allowed to use our lightsaber, get closer
        if (TIMER_Done(NPCS.NPC, "parryTime") ||
            NPCS.NPCInfo->rank > RANK_LT) { // not parrying
          if (!NPCS.NPC->enemy->client ||
              NPCS.NPC->enemy->client->ps.groundEntityNum !=
...
static qboolean Jedi_Strafe(int strafeTimeMin, int strafeTimeMax,
                            int nextStrafeTimeMin, int nextStrafeTimeMax,
                            qboolean walking) {
  if (Jedi_CultistDestroyer(NPCS.NPC)) {
    return qfalse;
  }
  if (NPCS.NPC->client->ps.saberEventFlags & SEF_LOCK_WON && NPCS.NPC->enemy &&
      NPCS.NPC->enemy->painDebounceTime >
          level.time) { // don't strafe if pressing the advantage of winning a
                        // saberLock
    return qfalse;
  }
  if (TIMER_Done(NPCS.NPC, "strafeLeft") &&
    ...
    //		Maybe try to strafe toward designer-placed "safe spots" or
    //"goals"?
...
*/
// rwwFIXMEFIXME: Going to use qboolean Jedi_DodgeEvasion( gentity_t *self,
// gentity_t *shooter, trace_t *tr, int hitLoc ) from w_saber.c.. maybe use
// seperate one for NPCs or add cases to that one?

evasionType_t Jedi_CheckFlipEvasions(gentity_t *self, float rightdot,
                                     float zdiff) {
  if (self->NPC && (self->NPC->scriptFlags & SCF_NO_ACROBATICS)) {
    return EVASION_NONE;
  }
  if (self->client && (self->client->ps.fd.forceRageRecoveryTime > level.time ||
                       (self->client->ps.fd.forcePowersActive &
                        (1 << FP_RAGE)))) { // no fancy dodges when raging
    return EVASION_NONE;
  }
  // Check for:
  // ARIALS/CARTWHEELS
  ...
  if (self->client->ps.legsAnim == BOTH_WALL_RUN_LEFT ||
      self->client->ps.legsAnim ==
          BOTH_WALL_RUN_RIGHT) { // already running on a wall
    ...
    animLength = BG_AnimLength(self->localAnimIndex,
                               (animNumber_t)self->client->ps.legsAnim);
    if (self->client->ps.legsAnim == BOTH_WALL_RUN_LEFT &&
        rightdot < 0) { // I'm running on a wall to my left and the attack is on
                        // the left
      if (animLength - self->client->ps.legsTimer > 400 &&
          self->client->ps.legsTimer >
              400) { // not at the beginning or end of the anim
        anim = BOTH_WALL_RUN_LEFT_FLIP;
      }
    } else if (self->client->ps.legsAnim == BOTH_WALL_RUN_RIGHT &&
               rightdot > 0) { // I'm running on a wall to my right and the
                               // attack is on the right
      if (animLength - self->client->ps.legsTimer > 400 &&
          self->client->ps.legsTimer >
              400) { // not at the beginning or end of the anim
    ...
    if (anim != -1) { // flip off the wall!
      int parts;
      // FIXME: check the direction we will flip towards for
      // do-not-enter/walls/drops? NOTE: we presume there is still a wall there!
      if (anim == BOTH_WALL_RUN_LEFT_FLIP) {
        self->client->ps.velocity[0] *= 0.5f;
        self->client->ps.velocity[1] *= 0.5f;
        VectorMA(self->client->ps.velocity, 150, right,
  ...
  } else if (self->client->NPC_class != CLASS_DESANN // desann doesn't do these
                                                     // kind of frilly acrobatics
             &&
    ...
    trap->Trace(
        &trace, self->r.currentOrigin, mins, maxs, traceto, self->s.number,
    ...
    if (trace.fraction >= 1.0f && allowCartWheels) { // it's clear, let's do it
      ...
      self->client->ps.weaponTime =
          self->client->ps
              .legsTimer; // don't attack again until this anim is done
      ...
      self->client->ps.fd.forceJumpCharge =
          0; // so we don't play the force flip anim
      self->client->ps.velocity[2] = 200;
      self->client->ps.fd.forceJumpZStart =
          self->r.currentOrigin[2]; // so we don't take damage if we land at
                                    // same height
    ...
    } else if (!(trace.contents &
                 CONTENTS_BOTCLIP)) { // hit a wall, not a do-not-enter brush
      // FIXME: before we check any of these jump-type evasions, we should check
      // for headroom, right? Okay, see if we can flip *off* the wall and go the
      // other way
      vec3_t idealNormal;
      gentity_t *traceEnt;
      ...
      if ((trace.entityNum < ENTITYNUM_WORLD && traceEnt &&
           traceEnt->s.solid != SOLID_BMODEL) ||
          DotProduct(trace.plane.normal, idealNormal) >
              0.7f) { // it's a ent of some sort or it's a wall roughly facing
                      // us
        float bestCheckDist = 0;
        // hmm, see if we're moving forward
        if (DotProduct(self->client->ps.velocity, fwd) <
            200) { // not running forward very fast
          // check to see if it's okay to move the other way
          if ((trace.fraction * checkDist) <=
              32) { // wall on that side is close enough to wall-flip off of or
                    // wall-run on
            bestCheckDist = checkDist;
            checkDist *= -1.0f;
            VectorMA(self->r.currentOrigin, checkDist, right, traceto);
            // trace in the dir that we want to go
            trap->Trace(&trace, self->r.currentOrigin, mins, maxs, traceto,
                        self->s.number,
            ...
            if (trace.fraction >= 1.0f) { // it's clear, let's do it
              if (allowWallFlips) { // okay to do wall-flips with this saber
                ...
                if (rightdot > 0) {
                  ...
                  self->client->ps.velocity[0] = self->client->ps.velocity[1] =
                      0;
                  VectorMA(self->client->ps.velocity, 150, right,
                           self->client->ps.velocity);
                } else {
                ...
                self->client->ps.velocity[2] =
                    forceJumpStrength[FORCE_LEVEL_2] / 2.25f;
                // animate me
                parts = SETANIM_LEGS;
                if (!self->client->ps.weaponTime) {
                  parts = SETANIM_BOTH;
                ...
                NPC_SetAnim(self, parts, anim,
                            SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
                self->client->ps.fd.forceJumpZStart =
                    self->r.currentOrigin[2]; // so we don't take damage if we
                                              // land at same height
                // self->client->ps.pm_flags |= (PMF_JUMPING|PMF_SLOW_MO_FALL);
                if (self->client->NPC_class == CLASS_BOBAFETT) {
                  G_AddEvent(self, EV_JUMP, 0);
          ...
          } else { // too far from that wall to flip or run off it, check other
                   // side
            checkDist *= -1.0f;
            VectorMA(self->r.currentOrigin, checkDist, right, traceto);
            // trace in the dir that we want to go
            trap->Trace(&trace, self->r.currentOrigin, mins, maxs, traceto,
                        ...
                        qfalse, 0, 0);
            if ((trace.fraction * checkDist) <=
                32) { // wall on this side is close enough
              bestCheckDist = checkDist;
        ...
        // Try wall run?
        if (bestCheckDist) { // one of the walls was close enough to wall-run on
          ...
          if (allowWallRuns) { // okay to do wallruns with this saber
            ...
            NPC_SetAnim(self, parts, anim,
                        SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
            self->client->ps.fd.forceJumpZStart =
                self->r.currentOrigin[2]; // so we don't take damage if we land
                                          // at same height
            // self->client->ps.pm_flags |= (PMF_JUMPING|PMF_SLOW_MO_FALL);
            if (self->client->NPC_class == CLASS_BOBAFETT) {
              G_AddEvent(self, EV_JUMP, 0);
  ...
  if (self->s.number >= 0 && self->s.number < MAX_CLIENTS) { // player
    ...
    if (!g_saberRealisticCombat.integer &&
      ...
      if (evasionType == EVASION_DODGE) {
        ...
        } else if (self->NPC->rank >=
                   RANK_LT_JG) { // fencers, bosses, shadowtroopers, luke,
                                 // desann, et al use the norm
          if (!Q_irand(0, 2)) { // with the occasional fast parry
            baseTime = ceil(baseTime / 2.0f);
          }
        ...
        if (evasionType == EVASION_DUCK || evasionType == EVASION_DUCK_PARRY) {
          baseTime += 100;
        } else if (evasionType == EVASION_JUMP ||
...
qboolean Jedi_QuickReactions(gentity_t *self) {
  if ((self->client->NPC_class == CLASS_JEDI &&
       NPCS.NPCInfo->rank == RANK_COMMANDER) ||
      self->client->NPC_class == CLASS_TAVION ||
      (self->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_1 &&
       g_npcspskill.integer > 1) ||
      (self->client->ps.fd.forcePowerLevel[FP_SABER_DEFENSE] > FORCE_LEVEL_2 &&
       g_npcspskill.integer > 0)) {
    return qtrue;
  }
  return qfalse;
}
...
qboolean Jedi_SaberBusy(gentity_t *self) {
  if (self->client->ps.torsoTimer > 300 &&
      ((BG_SaberInAttack(self->client->ps.saberMove) &&
       ...
       PM_RollingAnim(self->client->ps.torsoAnim))) { // my saber is not in a
                                                      // parrying position
    return qtrue;
  }
  return qfalse;
...
FIXME: Based on difficulty level/enemy saber combat skill, make this
decision-making more/less effective

NOTE: always blocking projectiles in this func!

-------------------------
*/
extern qboolean G_FindClosestPointOnLineSegment(const vec3_t start,
                                                const vec3_t end,
                                                const vec3_t from,
                                                vec3_t result);
evasionType_t Jedi_SaberBlockGo(gentity_t *self, usercmd_t *cmd, vec3_t pHitloc,
                                vec3_t phitDir, gentity_t *incoming,
                                float dist) // dist = 0.0f
{
  ...
  // FIXME: if we don't have our saber in hand, pick the force throw option or a
  // jump or strafe!
  ...
  if (!incoming) {
    ...
    if (self->client->ps.saberInFlight) { // DOH!  do non-saber evasion!
      saberBusy = qtrue;
    } else if (Jedi_QuickReactions(
                   self)) { // jedi trainer and tavion are must faster at
                            // parrying and can do it whenever they like Also, on
                            // medium, all level 3 people can parry any time and
                            // on hard, all level 2 or 3 people can parry any
                            // time
    } else {
      saberBusy = Jedi_SaberBusy(self);
    }
  } else {
    if (incoming->s.weapon ==
        WP_SABER) { // flying lightsaber, face it!
                    // FIXME: for this to actually work, we'd need to call
                    // update angles too? Jedi_FaceEntity( self, incoming, qtrue
                    // );
    }
  ...
  if ((dist > 16 && (Q_irand(0, 2) || saberBusy)) ||
      self->client->ps.saberInFlight || BG_SabersOff(&self->client->ps) ||
      self->client->NPC_class ==
          CLASS_BOBAFETT) { // either it will miss by a bit (and 25% chance) OR
                            // our saber is not in-hand OR saber is off
    if (self->NPC &&
        (self->NPC->rank == RANK_CREWMAN ||
         self->NPC->rank >= RANK_LT_JG)) { // acrobat or fencer or above
      if (self->client->ps.groundEntityNum != ENTITYNUM_NONE && // on the ground
          !(self->client->ps.pm_flags & PMF_DUCKED) && cmd->upmove >= 0 &&
          TIMER_Done(self, "duck") // not ducking
          && !BG_InRoll(&self->client->ps,
                        self->client->ps.legsAnim) // not rolling
          ...
          && (self->client->ps.saberInFlight ||
              ...
              (!BG_SaberInAttack(self->client->ps.saberMove) // not attacking
               && !PM_SaberInStart(
                      self->client->ps.saberMove) // not starting an attack
               && !BG_SpinningSaberAnim(
                      self->client->ps.torsoAnim) // not in a saber spin
               && !BG_SaberInSpecialAttack(
                      self->client->ps.torsoAnim)) // not in a special attack
              )) { // need to check all these because it overrides both torso
                   // and legs with the dodge
        doDodge = qtrue;
      }
    }
  }
  ...
  if (d_JediAI.integer) {
    Com_Printf("(%d) evading attack from height %4.2f, zdiff: %4.2f, rightdot: "
               "%4.2f\n",
      ...
      if (rightdot > 8 || (rightdot > 3 && zdiff < -11)) // was normalized, 0.2
      {
        ...
        if (d_JediAI.integer) {
          Com_Printf("mid-UR block\n");
                 ...
                 (rightdot < -3 && zdiff < -11)) // was normalized, -0.2
      {
        if (doDodge) {
          if (self->client->NPC_class == CLASS_BOBAFETT &&
              !Q_irand(0, 2)) { // roll!
        ...
        if (d_JediAI.integer) {
          Com_Printf("mid-TOP block\n");
             ...
             (zdiff < -36 &&
              (zdiff < -44 ||
               !Q_irand(0, 2)))) // was -30 and -40//2nd one was -46
  {                              // jump!
    if (self->client->ps.groundEntityNum ==
        ENTITYNUM_NONE) { // already in air, duck to pull up legs
      TIMER_Start(self, "duck", Q_irand(500, 1500));
      evasionType = EVASION_DUCK;
      if (d_JediAI.integer) {
        Com_Printf("legs up\n");
      ...
      if (self->NPC &&
          ...
          (!Q_irand(0, 10) ||
           (!Q_irand(0, 2) &&
            (cmd->forwardmove || cmd->rightmove)))) { // superjump
        ...
        if (self->NPC && !(self->NPC->scriptFlags & SCF_NO_ACROBATICS) &&
            ...
            !PM_InKnockDown(&self->client->ps)) {
          self->client->ps.fd.forceJumpCharge =
              320; // FIXME: calc this intelligently
        ...
        if (self->client->NPC_class == CLASS_TAVION) {
          if (!incoming && self->client->ps.groundEntityNum < ENTITYNUM_NONE &&
              !Q_irand(0, 2)) {
            if (!BG_SaberInAttack(self->client->ps.saberMove) &&
                !PM_SaberInStart(self->client->ps.saberMove) &&
                !BG_InRoll(&self->client->ps, self->client->ps.legsAnim) &&
                !PM_InKnockDown(&self->client->ps) &&
                !BG_SaberInSpecialAttack(
                    self->client->ps.torsoAnim)) { // do the butterfly!
              int butterflyAnim;
              if (Q_irand(0, 1)) {
              ...
              self->client->ps.fd.forceJumpZStart =
                  self->r.currentOrigin[2]; // so we don't take damage if we
                                            // land at same height
              //	self->client->ps.pm_flags |=
              //PMF_JUMPING|PMF_SLOW_MO_FALL;
              //	self->client->ps.SaberActivateTrail( 300 );//FIXME:
              //reset this when done! Ah well. No hacking from the server for
              // now.
              if (self->client->NPC_class == CLASS_BOBAFETT) {
                G_AddEvent(self, EV_JUMP, 0);
              } else {
                G_Sound(self, CHAN_BODY,
      ...
      if (((evasionType = Jedi_CheckFlipEvasions(self, rightdot, zdiff)) !=
           EVASION_NONE)) {
        saberBusy = qtrue;
  ...
  if (dodgeAnim != -1) { // dodged
    ...
    NPC_SetAnim(self, SETANIM_BOTH, dodgeAnim,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
...
extern float ShortestLineSegBewteen2LineSegs(vec3_t start1, vec3_t end1,
                                             vec3_t start2, vec3_t end2,
                                             vec3_t close_pnt1,
                                             vec3_t close_pnt2);
extern int WPDEBUG_SaberColor(saber_colors_t saberColor);
static qboolean Jedi_SaberBlock(int saberNum,
                                int bladeNum) // saberNum = 0, bladeNum = 0
{
  vec3_t hitloc, saberTipOld, saberTip, top, bottom, axisPoint, saberPoint,
      dir; // saberBase,
  ...
  */
  ...
  if (!TIMER_Done(NPCS.NPC, "parryReCalcTime")) { // can't do our own re-think
                                                  // of which parry to use yet
    return qfalse;
  }

  if (NPCS.NPC->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] >
      level.time) { // can't move the saber to another position yet
    return qfalse;
  }
  ...
  {//lower rank reborn have a random chance of not doing it at all
          NPC->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] = level.time +
  300; return qfalse;
  }
  */

  if (NPCS.NPC->enemy->health <= 0 ||
      !NPCS.NPC->enemy->client) { // don't keep blocking him once he's dead (or
                                  // if not a client)
    return qfalse;
  }
  /*
  //VectorMA( NPC->enemy->client->renderInfo.muzzlePoint,
  NPC->enemy->client->ps.saberLength, NPC->enemy->client->renderInfo.muzzleDir,
  saberTip );
  //VectorMA( NPC->enemy->client->renderInfo.muzzlePointNext,
  NPC->enemy->client->ps.saberLength,
  NPC->enemy->client->renderInfo.muzzleDirNext, saberTipNext ); VectorMA(
  NPC->enemy->client->renderInfo.muzzlePointOld,
  NPC->enemy->client->ps.saberLength,
  NPC->enemy->client->renderInfo.muzzleDirOld, saberTipOld ); VectorMA(
  NPC->enemy->client->renderInfo.muzzlePoint,
  NPC->enemy->client->ps.saberLength, NPC->enemy->client->renderInfo.muzzleDir,
  saberTip );

  VectorSubtract( NPC->enemy->client->renderInfo.muzzlePoint,
  NPC->enemy->client->renderInfo.muzzlePointOld, dir );//get the dir VectorAdd(
  dir, NPC->enemy->client->renderInfo.muzzlePoint, saberBase );//extrapolate
  ...
  float dist = ShortestLineSegBewteen2LineSegs( saberBase, saberTipOld, bottom,
  top, saberPoint, axisPoint ); if ( 0 )//dist > NPC->r.maxs[0]*4 )//was *3
  ...
  trace_t	tr;
  trap->Trace( &tr, saberPoint, vec3_origin, vec3_origin, axisPoint,
  NPC->enemy->s.number, MASK_SHOT, G2_RETURNONHIT, 10 ); if ( tr.allsolid ||
  tr.startsolid )
  {//estimate
  ...
  */
  ...
  VectorMA(
      NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzlePointOld,
      NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].length,
      NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzleDirOld,
      saberTipOld);
  VectorMA(NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzlePoint,
           NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].length,
           NPCS.NPC->enemy->client->saber[saberNum].blade[bladeNum].muzzleDir,
           saberTip);
  //	VectorCopy(NPC->enemy->client->lastSaberBase_Always, muzzlePoint);
  //	VectorMA(muzzlePoint, GAME_SABER_LENGTH,
  //NPC->enemy->client->lastSaberDir_Always, saberTip); 	VectorCopy(saberTip,
  //saberTipOld);
  ...
  dist = ShortestLineSegBewteen2LineSegs(
      NPCS.NPC->enemy->client->renderInfo.muzzlePoint, saberTip, bottom, top,
  ...
  { // FIXME: sometimes he reacts when you're too far away to actually hit him
    ...
    /*
    if ( dist < 300 //close
            && !Jedi_QuickReactions( NPC )//quick reaction people can interrupt
    themselves
            && (PM_SaberInStart( NPC->enemy->client->ps.saberMove ) ||
    BG_SaberInAttack( NPC->enemy->client->ps.saberMove )) )//enemy is swinging
    at me
    {//he's swinging at me and close enough to be a threat, don't start an
    attack right now TIMER_Set( NPC, "parryTime", 100 );
    }
    else
    */
  ...
  VectorSubtract(saberPoint, NPCS.NPC->enemy->client->renderInfo.muzzlePoint,
                 pointDir);
  ...
  VectorSubtract(NPCS.NPC->enemy->client->renderInfo.muzzlePoint,
                 NPCS.NPC->enemy->client->renderInfo.muzzlePointOld, baseDir);
  ...
  // get the actual point of impact
  trap->Trace(&tr, saberPoint, saberMins, saberMaxs, hitloc,
              NPCS.NPC->enemy->s.number, CONTENTS_BODY, qfalse, 0,
              0); //, G2_RETURNONHIT, 10 );
  if (tr.allsolid || tr.startsolid || tr.fraction >= 1.0f) { // estimate
    ...
    if (DotProduct(dir, dir2Me) <
        0.2f) { // saber is not swinging in my direction
      /*
      if ( dist < 300 //close
              && !Jedi_QuickReactions( NPC )//quick reaction people can
    ...
    ShortestLineSegBewteen2LineSegs(saberPoint, hitloc, bottom, top,
                                    saberHitPoint, hitloc);
    /*
    VectorSubtract( saberPoint, axisPoint, dir );
    VectorNormalize( dir );
    VectorMA( axisPoint, NPC->r.maxs[0]*1.22, dir, hitloc );
  ...
  if (d_JediAI.integer) {
    // G_DebugLine( saberPoint, hitloc, FRAMETIME, WPDEBUG_SaberColor(
    // NPC->enemy->client->ps.saber[saberNum].blade[bladeNum].color ), qtrue );
    G_TestLine(saberPoint, hitloc, 0x0000ff, FRAMETIME);
  }

  // FIXME: if saber is off and/or we have force speed and want to be really
  // cocky, 		and the swing misses by some amount, we can use the dodges here... :)
  if ((evasionType =
           Jedi_SaberBlockGo(NPCS.NPC, &NPCS.ucmd, hitloc, dir, NULL, dist)) !=
      EVASION_DODGE) { // we did block (not dodge)
    int parryReCalcTime;

    if (!NPCS.NPC->client->ps.saberInFlight) { // make sure saber is on
      WP_ActivateSaber(NPCS.NPC);
    }
    ...
    if (TIMER_Done(NPCS.NPC, "parryTime")) {
      if (NPCS.NPC->client->NPC_class == CLASS_TAVION) {
        TIMER_Set(NPCS.NPC, "parryTime",
                  Q_irand(parryReCalcTime / 2, parryReCalcTime * 1.5));
      } else if (NPCS.NPCInfo->rank >=
                 RANK_LT_JG) { // fencers and higher hold a parry less
    ...
    if (NPCS.NPCInfo->rank > RANK_LT_COMM &&
        NPCS.NPC->client->NPC_class !=
            CLASS_DESANN) { // higher-level guys can dodge faster
      dodgeTime -= 200;
    }
...
-------------------------
*/
static void Jedi_EvasionSaber(vec3_t enemy_movedir, float enemy_dist,
                              vec3_t enemy_dir) {
  vec3_t dirEnemy2Me;
  int evasionChance =
      30; // only step aside 30% if he's moving at me but not attacking
  ...
  if (!NPCS.NPC->enemy->client) {
    return;
             ...
             NPCS.NPC->enemy->client->ps.saberLockTime >
                 level.time) { // don't try to block/evade an enemy who is in a
                               // saberLock
    return;
  } else if (NPCS.NPC->client->ps.saberEventFlags & SEF_LOCK_WON &&
             NPCS.NPC->enemy->painDebounceTime >
                 level.time) { // pressing the advantage of winning a saber lock
    return;
  }

  if (NPCS.NPC->enemy->client->ps.saberInFlight &&
      !TIMER_Done(NPCS.NPC,
                  "taunting")) { // if he's throwing his saber, stop taunting
    TIMER_Set(NPCS.NPC, "taunting", -level.time);
    if (!NPCS.NPC->client->ps.saberInFlight) {
      WP_ActivateSaber(NPCS.NPC);
    }
  ...
  if (TIMER_Done(NPCS.NPC, "parryTime")) {
    if (NPCS.NPC->client->ps.saberBlocked != BLOCKED_ATK_BOUNCE &&
        NPCS.NPC->client->ps.saberBlocked !=
            BLOCKED_PARRY_BROKEN) { // wasn't blocked myself
      NPCS.NPC->client->ps.saberBlocked = BLOCKED_NONE;
    }
  }

  if (NPCS.NPC->enemy->client->ps.weaponTime &&
      NPCS.NPC->enemy->client->ps.weaponstate == WEAPON_FIRING) {
    if (!NPCS.NPC->client->ps.saberInFlight && Jedi_SaberBlock(0, 0)) {
      return;
    }
  }

  VectorSubtract(NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin,
                 dirEnemy2Me);
  VectorNormalize(dirEnemy2Me);

  if (NPCS.NPC->enemy->client->ps.weaponTime &&
      NPCS.NPC->enemy->client->ps.weaponstate ==
          WEAPON_FIRING) { // enemy is attacking
    enemy_attacking = qtrue;
    evasionChance = 90;
  }

  if ((NPCS.NPC->enemy->client->ps.fd.forcePowersActive &
       (1 << FP_LIGHTNING))) { // enemy is shooting lightning
    enemy_attacking = qtrue;
  ...
  if (NPCS.NPC->enemy->client->ps.saberInFlight &&
      ...
      NPCS.NPC->enemy->client->ps.saberEntityState !=
          SES_RETURNING) { // enemy is shooting lightning
    enemy_attacking = qtrue;
    throwing_saber = qtrue;
  }

  // FIXME: this needs to take skill and rank(reborn type) into account much
  // more
  if (Q_irand(0, 100) < evasionChance) { // check to see if he's coming at me
    ...
    if (VectorCompare(enemy_movedir, vec3_origin) || shooting_lightning ||
        throwing_saber) { // he's not moving (or he's using a ranged attack),
                          // see if he's facing me
      vec3_t enemy_fwd;
      AngleVectors(NPCS.NPC->enemy->client->ps.viewangles, enemy_fwd, NULL,
                   NULL);
    ...
    if (flrand(0.25, 1) < facingAmt) { // coming at/facing me!
      int whichDefense = 0;
      if (NPCS.NPC->client->ps.weaponTime ||
          NPCS.NPC->client->ps.saberInFlight ||
          NPCS.NPC->client->NPC_class ==
              CLASS_BOBAFETT) { // I'm attacking or recovering from a parry, can
                                // only try to strafe/jump right now
        ...
        if (shooting_lightning) { // check for lightning attack
          ...
          whichDefense = 100;
        } else if (throwing_saber) { // he's thrown his saber!  See if it's
                                     // coming at me
          float saberDist;
          vec3_t saberDir2Me;
          vec3_t saberMoveDir;
          gentity_t *saber =
              &g_entities[NPCS.NPC->enemy->client->ps.saberEntityNum];
          VectorSubtract(NPCS.NPC->r.currentOrigin, saber->r.currentOrigin,
                         saberDir2Me);
          ...
          if (DotProduct(saberMoveDir, saberDir2Me) >
              0.5) {               // it's heading towards me
        ...
        if (whichDefense) { // already chose one
                   ...
                   !enemy_attacking) { // he's pretty far, or not swinging, just
                                       // strafe
          if (VectorCompare(
                  enemy_movedir,
                  vec3_origin)) { // if he's not moving, not swinging and far
                                  // enough away, no evasion necc.
            return;
          }
          if (Q_irand(0, 10) < NPCS.NPCInfo->stats.aggression) {
            return;
          ...
          if (DotProduct(enemy_dir, fwd) <
              0.5) { // I'm not really facing him, best option is to strafe
            whichDefense = Q_irand(5, 16);
          } else if (enemy_dist < 56) { // he's very close, maybe we should be
                                        // more inclined to block or throw
      ...
      if (whichDefense >= 4 && whichDefense <= 12) { // would try to block
        if (NPCS.NPC->client->ps
                .saberInFlight) { // can't, saber in not in hand, so fall back
                                  // to strafe/jump
          whichDefense = 100;
        }
      }
      ...
      default:
        ...
        if (!Q_irand(0, 5) ||
            !Jedi_Strafe(300, 1000, 0, 1000,
                         qfalse)) { // certain chance they will pick an
                                    // alternative evasion
          ...
          if (shooting_lightning || throwing_saber || enemy_dist < 80) {
            ...
            if (shooting_lightning ||
                (!Q_irand(0, 2) && NPCS.NPCInfo->stats.aggression < 4 &&
                 TIMER_Done(NPCS.NPC, "parryTime"))) {
              if ((NPCS.NPCInfo->rank == RANK_ENSIGN ||
                   NPCS.NPCInfo->rank > RANK_LT_JG) &&
                  !shooting_lightning &&
                  Q_irand(
                      ...
                      2)) { // FIXME: check
                            // forcePushRadius[NPC->client->ps.fd.forcePowerLevel[FP_PUSH]]
                ForceThrow(NPCS.NPC, qfalse);
              } else if ((NPCS.NPCInfo->rank == RANK_CREWMAN ||
                          NPCS.NPCInfo->rank > RANK_LT_JG) &&
                         !(NPCS.NPCInfo->scriptFlags & SCF_NO_ACROBATICS) &&
                         NPCS.NPC->client->ps.fd.forceRageRecoveryTime <
                             level.time &&
                         !(NPCS.NPC->client->ps.fd.forcePowersActive &
                           (1 << FP_RAGE)) &&
                         !PM_InKnockDown(
                             &NPCS.NPC->client
                                  ->ps)) { // FIXME: make this a function call?
                ...
                // FIXME: if this jump is cleared, we can't block... so pick a
                // random lower block?
...
==========================================================================================
*/
gentity_t *Jedi_FindEnemyInCone(gentity_t *self, gentity_t *fallback,
                                float minDot) {
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (check->client->playerTeam !=
        self->client->enemyTeam) { // not an enemy - FIXME: what about turrets?
      continue;
    }
    ...
    if (!trap->InPVS(check->r.currentOrigin,
                     self->r.currentOrigin)) { // can't potentially see them
      continue;
    }
    ...
    trap->Trace(&tr, self->r.currentOrigin, vec3_origin, vec3_origin,
                check->r.currentOrigin, self->s.number, MASK_SHOT, qfalse, 0,
                0);
    if (tr.fraction < 1.0f &&
        tr.entityNum != check->s.number) { // must have clear shot
      continue;
    }
...
static void Jedi_SetEnemyInfo(vec3_t enemy_dest, vec3_t enemy_dir,
                              float *enemy_dist, vec3_t enemy_movedir,
                              float *enemy_movespeed, int prediction) {
  if (!NPCS.NPC || !NPCS.NPC->enemy) { // no valid enemy
    return;
  }
  if (!NPCS.NPC->enemy->client) {
    ...
    *enemy_movespeed = 0;
    ...
    enemy_dest[2] += NPCS.NPC->enemy->r.mins[2] +
                     24; // get it's origin to a height I can work with
    VectorSubtract(enemy_dest, NPCS.NPC->r.currentOrigin, enemy_dir);
    // FIXME: enemy_dist calc needs to include all blade lengths, and include
    // distance from hand to start of blade....
    *enemy_dist =
        VectorNormalize(enemy_dir); // - (NPC->client->ps.saberLengthMax +
                                    // NPC->r.maxs[0]*1.5 + 16);
    ...
    *enemy_movespeed = VectorNormalize(enemy_movedir);
    ...
    VectorMA(NPCS.NPC->enemy->r.currentOrigin,
             *enemy_movespeed * 0.001 * prediction, enemy_movedir, enemy_dest);
    // figure out what dir the enemy's estimated position is from me and how far
    // from the tip of my saber he is
    VectorSubtract(enemy_dest, NPCS.NPC->r.currentOrigin,
                   enemy_dir); // NPC->client->renderInfo.muzzlePoint
    // FIXME: enemy_dist calc needs to include all blade lengths, and include
    // distance from hand to start of blade....
    *enemy_dist =
        VectorNormalize(enemy_dir) -
        (NPCS.NPC->client->saber[0].blade[0].lengthMax +
         NPCS.NPC->r.maxs[0] * 1.5 + 16); // just use the blade 0 len I guess
    // FIXME: keep a group of enemies around me and use that info to make
    // decisions... 		For instance, if there are multiple enemies, evade more, push
    //them away 		and use medium attacks.  If enemies are using blasters, switch
    //to fast. 		If one jedi enemy, use strong attacks.  Use grip when fighting
    //one or 		two enemies, use lightning spread when fighting multiple enemies,
    //etc. 		Also, when kill one, check rest of group instead of walking up to
    //victim.
  }
}
...
static void Jedi_FaceEnemy(qboolean doPitch) {
  ...
  if (NPCS.NPC->client->NPC_class == CLASS_BOBAFETT &&
      TIMER_Done(NPCS.NPC, "flameTime") && NPCS.NPC->s.weapon != WP_NONE &&
      NPCS.NPC->s.weapon != WP_DISRUPTOR &&
      (NPCS.NPC->s.weapon != WP_ROCKET_LAUNCHER ||
       !(NPCS.NPCInfo->scriptFlags & SCF_ALT_FIRE)) &&
      NPCS.NPC->s.weapon != WP_THERMAL && NPCS.NPC->s.weapon != WP_TRIP_MINE &&
      NPCS.NPC->s.weapon != WP_DET_PACK && NPCS.NPC->s.weapon != WP_STUN_BATON
      /*&& NPC->s.weapon != WP_MELEE*/) { // boba leads his enemy
    if (NPCS.NPC->health < NPCS.NPC->client->pers.maxHealth * 0.5f) { // lead
      float missileSpeed = WP_SpeedOfMissileForWeapon(
          NPCS.NPC->s.weapon,
          ((qboolean)(NPCS.NPCInfo->scriptFlags & SCF_ALT_FIRE)));
      if (missileSpeed) {
        float eDist = Distance(eyes, enemy_eyes);
        eDist /=
            missileSpeed; // How many seconds it will take to get to the enemy
        VectorMA(enemy_eyes, eDist * flrand(0.95f, 1.25f),
                 NPCS.NPC->enemy->client->ps.velocity, enemy_eyes);
      }
    }
  }
  ...
  if (!NPCS.NPC->client->ps.saberInFlight &&
      (NPCS.NPC->client->ps.legsAnim == BOTH_A2_STABBACK1 ||
       ...
       NPCS.NPC->client->ps.legsAnim == BOTH_ATTACK_BACK)) { // point *away*
  ...
  */
  ...
  if (doPitch) {
    ...
    if (NPCS.NPC->client->ps.saberInFlight) { // tilt down a little
...
static void Jedi_DebounceDirectionChanges(void) {
  ...
  if (NPCS.ucmd.forwardmove > 0) {
    if (!TIMER_Done(NPCS.NPC, "moveback") ||
        !TIMER_Done(NPCS.NPC, "movenone")) {
      NPCS.ucmd.forwardmove = 0;
    ...
    } else if (TIMER_Done(NPCS.NPC,
                          "moveforward")) { // FIXME: should be if it's zero?
      TIMER_Set(NPCS.NPC, "moveforward", Q_irand(500, 2000));
    }
  } else if (NPCS.ucmd.forwardmove < 0) {
    if (!TIMER_Done(NPCS.NPC, "moveforward") ||
        !TIMER_Done(NPCS.NPC, "movenone")) {
      NPCS.ucmd.forwardmove = 0;
      // now we have to normalize the total movement again
      if (NPCS.ucmd.rightmove > 0) {
      ...
      if (TIMER_Done(NPCS.NPC, "movenone")) {
        TIMER_Set(NPCS.NPC, "movenone", Q_irand(1000, 2000));
                          ...
                          "moveback")) { // FIXME: should be if it's zero?
      TIMER_Set(NPCS.NPC, "moveback", Q_irand(250, 1000));
    }
  } else if (!TIMER_Done(NPCS.NPC,
                         "moveforward")) { // NOTE: edge checking should stop me
                                           // if this is bad... but what if it
                                           // sends us colliding into the enemy?
    NPCS.ucmd.forwardmove = 127;
    VectorClear(NPCS.NPC->client->ps.moveDir);
  } else if (!TIMER_Done(NPCS.NPC, "moveback")) { // NOTE: edge checking should
                                                  // stop me if this is bad...
    NPCS.ucmd.forwardmove = -127;
    VectorClear(NPCS.NPC->client->ps.moveDir);
  }
  // Time-debounce changes in right/left dir
  if (NPCS.ucmd.rightmove > 0) {
    if (!TIMER_Done(NPCS.NPC, "moveleft") ||
        !TIMER_Done(NPCS.NPC, "movecenter")) {
      NPCS.ucmd.rightmove = 0;
      // now we have to normalize the total movement again
      if (NPCS.ucmd.forwardmove > 0) {
        NPCS.ucmd.forwardmove = 127;
      ...
      if (TIMER_Done(NPCS.NPC, "movecenter")) {
        TIMER_Set(NPCS.NPC, "movecenter", Q_irand(1000, 2000));
                          ...
                          "moveright")) { // FIXME: should be if it's zero?
      TIMER_Set(NPCS.NPC, "moveright", Q_irand(250, 1500));
    }
  } else if (NPCS.ucmd.rightmove < 0) {
    if (!TIMER_Done(NPCS.NPC, "moveright") ||
        !TIMER_Done(NPCS.NPC, "movecenter")) {
      NPCS.ucmd.rightmove = 0;
      // now we have to normalize the total movement again
      if (NPCS.ucmd.forwardmove > 0) {
      ...
      if (TIMER_Done(NPCS.NPC, "movecenter")) {
        TIMER_Set(NPCS.NPC, "movecenter", Q_irand(1000, 2000));
                          ...
                          "moveleft")) { // FIXME: should be if it's zero?
      TIMER_Set(NPCS.NPC, "moveleft", Q_irand(250, 1500));
    }
  } else if (!TIMER_Done(NPCS.NPC, "moveright")) { // NOTE: edge checking should
                                                   // stop me if this is bad...
    NPCS.ucmd.rightmove = 127;
    VectorClear(NPCS.NPC->client->ps.moveDir);
  } else if (!TIMER_Done(NPCS.NPC, "moveleft")) { // NOTE: edge checking should
                                                  // stop me if this is bad...
    NPCS.ucmd.rightmove = -127;
    VectorClear(NPCS.NPC->client->ps.moveDir);
  }
...
static void Jedi_TimersApply(void) {
  if (!NPCS.ucmd.rightmove) { // only if not already strafing
    // FIXME: if enemy behind me and turning to face enemy, don't strafe in that
    // direction, too
    if (!TIMER_Done(NPCS.NPC, "strafeLeft")) {
      if (NPCS.NPCInfo->desiredYaw >
          NPCS.NPC->client->ps.viewangles[YAW] +
              60) { // we want to turn left, don't apply the strafing
      ...
      if (NPCS.NPCInfo->desiredYaw <
          NPCS.NPC->client->ps.viewangles[YAW] -
              60) { // we want to turn right, don't apply the strafing
      } else {      // go ahead and strafe left
        NPCS.ucmd.rightmove = 127;
        VectorClear(NPCS.NPC->client->ps.moveDir);
      }
  ...
  if (!TIMER_Done(NPCS.NPC, "gripping")) { // FIXME: what do we do if we ran out
                                           // of power?  NPC's can't?
    // FIXME: don't keep turning to face enemy or we'll end up spinning around
    NPCS.ucmd.buttons |= BUTTON_FORCEGRIP;
  }

  if (!TIMER_Done(NPCS.NPC, "draining")) { // FIXME: what do we do if we ran out
                                           // of power?  NPC's can't?
    // FIXME: don't keep turning to face enemy or we'll end up spinning around
...
static void Jedi_CombatTimersUpdate(int enemy_dist) {
  ...
  if (TIMER_Done(NPCS.NPC, "roamTime")) {
    ...
    if (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_RAGE)) { // raging
      Jedi_Aggression(NPCS.NPC, Q_irand(0, 3));
    } else if (NPCS.NPC->client->ps.fd.forceRageRecoveryTime >
               level.time) { // recovering
    ...
    if (NPCS.NPC->enemy && NPCS.NPC->enemy->client) {
      ...
      case WP_SABER:
        ...
        if (BG_SabersOff(
                &NPCS.NPC->enemy->client
                     ->ps)) { // fool!  Standing around unarmed, charge!
      ...
      case WP_ROCKET_LAUNCHER:
        ...
        if (NPCS.NPC->enemy->attackDebounceTime <
            level.time) { // does this apply to players?
  ...
  if (TIMER_Done(NPCS.NPC, "noStrafe") && TIMER_Done(NPCS.NPC, "strafeLeft") &&
      TIMER_Done(NPCS.NPC, "strafeRight")) {
    // FIXME: Maybe more likely to do this if aggression higher?  Or some other
    // stat?
  ...
  if (NPCS.NPC->client->ps.saberEventFlags) { // some kind of saber combat event
                                              // is still pending
    int newFlags = NPCS.NPC->client->ps.saberEventFlags;
    if (NPCS.NPC->client->ps.saberEventFlags & SEF_PARRIED) { // parried
      TIMER_Set(NPCS.NPC, "parryTime", -1);
      /*
      if ( NPCInfo->rank >= RANK_LT_JG )
      {
              NPC->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] =
      level.time + 100;
      }
      else
      {
      ...
      */
      if (NPCS.NPC->enemy &&
          PM_SaberInKnockaway(
              NPCS.NPC->enemy->client->ps.saberMove)) { // advance!
        Jedi_Aggression(NPCS.NPC, 1);                   // get closer
        Jedi_AdjustSaberAnimLevel(NPCS.NPC,
                                  (NPCS.NPC->client->ps.fd.saberAnimLevel -
                                   1)); // use a faster attack
      } else {
        ...
        if (!Q_irand(0, 1)) {
          Jedi_AdjustSaberAnimLevel(
              NPCS.NPC, (NPCS.NPC->client->ps.fd.saberAnimLevel - 1));
        }
      }
      if (d_JediAI.integer) {
        Com_Printf("(%d) PARRY: agg %d, no parry until %d\n", level.time,
                   NPCS.NPCInfo->stats.aggression, level.time + 100);
      }
      newFlags &= ~SEF_PARRIED;
    }
    if (!NPCS.NPC->client->ps.weaponTime &&
        (NPCS.NPC->client->ps.saberEventFlags & SEF_HITENEMY)) // hit enemy
    { // we hit our enemy last time we swung, drop our aggression
      ...
      if (!Q_irand(0, 2)) {
        Jedi_AdjustSaberAnimLevel(NPCS.NPC,
                                  (NPCS.NPC->client->ps.fd.saberAnimLevel + 1));
      }
      newFlags &= ~SEF_HITENEMY;
    }
    if ((NPCS.NPC->client->ps.saberEventFlags &
         SEF_BLOCKED)) { // was blocked whilst attacking
      if (PM_SaberInBrokenParry(NPCS.NPC->client->ps.saberMove) ||
        ...
        if (NPCS.NPC->client->ps.saberInFlight) { // lost our saber, too!!!
          Jedi_Aggression(NPCS.NPC,
                          -5); // really really really should back off!!!
        } else {
          Jedi_Aggression(NPCS.NPC, -2); // really should back off!
        }
        Jedi_AdjustSaberAnimLevel(NPCS.NPC,
                                  (NPCS.NPC->client->ps.fd.saberAnimLevel +
                                   1)); // use a stronger attack
        if (d_JediAI.integer) {
      ...
      //		right now that's generic in the saber animation code,
      //auto-picks 		a next anim for me, but really should be AI-controlled.
    }
    if (NPCS.NPC->client->ps.saberEventFlags &
        SEF_DEFLECTED) { // deflected a shot
    ...
    if (NPCS.NPC->client->ps.saberEventFlags & SEF_HITWALL) { // hit a wall
      newFlags &= ~SEF_HITWALL;
    }
    if (NPCS.NPC->client->ps.saberEventFlags &
        SEF_HITOBJECT) { // hit some other damagable object
      if (!Q_irand(0, 3)) {
        Jedi_AdjustSaberAnimLevel(NPCS.NPC,
                                  (NPCS.NPC->client->ps.fd.saberAnimLevel - 1));
      }
...
static void Jedi_CombatIdle(int enemy_dist) {
  ...
  if (NPCS.NPC->client->ps
          .saberInFlight) { // don't do this idle stuff if throwing saber
    return;
  }
  if (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_RAGE) ||
      NPCS.NPC->client->ps.fd.forceRageRecoveryTime >
          level.time) { // never taunt while raging or recovering from rage
    return;
  }
  ...
  if (enemy_dist >= 64) { // FIXME: only do this if standing still?
    ...
    if (Q_irand(2, chance) < NPCS.NPCInfo->stats.aggression) {
      if (TIMER_Done(NPCS.NPC, "chatter") &&
          NPCS.NPC->client->ps.forceHandExtend ==
              HANDEXTEND_NONE) { // FIXME: add more taunt behaviors
        // FIXME: sometimes he turns it off, then turns it right back on
        // again???
        if (enemy_dist > 200 && NPCS.NPC->client->NPC_class != CLASS_BOBAFETT &&
...
static qboolean Jedi_AttackDecide(int enemy_dist) {
  ...
  if (Jedi_CultistDestroyer(NPCS.NPC)) { // destroyer
    if (enemy_dist <= 32) {              // go boom!
      // float?
      ...
      NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_FORCE_RAGE,
                  SETANIM_FLAG_HOLD | SETANIM_FLAG_OVERRIDE);
      NPCS.NPC->client->ps.fd.forcePowersActive |= (1 << FP_RAGE);
      NPCS.NPC->painDebounceTime = NPCS.NPC->useDebounceTime =
          level.time + NPCS.NPC->client->ps.torsoTimer;
      return qtrue;
    }
    return qfalse;
  }

  if (NPCS.NPC->enemy->client && NPCS.NPC->enemy->s.weapon == WP_SABER &&
      NPCS.NPC->enemy->client->ps.saberLockTime > level.time &&
      NPCS.NPC->client->ps.saberLockTime <
          level.time) { // enemy is in a saberLock and we are not
    return qfalse;
  ...
  if (NPCS.NPC->client->ps.saberEventFlags &
      SEF_LOCK_WON) { // we won a saber lock, press the advantage with an
                      // attack!
    int chance = 0;
    if (NPCS.NPC->client->NPC_class == CLASS_DESANN ||
        NPCS.NPC->client->NPC_class == CLASS_LUKE ||
        !Q_stricmp("Yoda", NPCS.NPC->NPC_type)) { // desann and luke
      chance = 20;
    } else if (NPCS.NPC->client->NPC_class == CLASS_TAVION) { // tavion
      chance = 10;
    } else if (NPCS.NPC->client->NPC_class == CLASS_REBORN &&
               NPCS.NPCInfo->rank == RANK_LT_JG) { // fencer
      chance = 5;
    } else {
    ...
    if (Q_irand(0, 30) < chance) { // based on skill with some randomness
      NPCS.NPC->client->ps.saberEventFlags &=
          ~SEF_LOCK_WON; // clear this now that we are using the opportunity
      TIMER_Set(NPCS.NPC, "noRetreat", Q_irand(500, 2000));
      // FIXME: check enemy_dist?
      NPCS.NPC->client->ps.weaponTime = NPCS.NPCInfo->shotTime =
          NPCS.NPC->attackDebounceTime = 0;
      // NPC->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] = level.time +
      // 500;
      NPCS.NPC->client->ps.saberBlocked = BLOCKED_NONE;
      WeaponThink(qtrue);
  ...
  if (NPCS.NPC->client->NPC_class == CLASS_TAVION ||
      ...
      (NPCS.NPC->client->NPC_class == CLASS_JEDI &&
       NPCS.NPCInfo->rank ==
           RANK_COMMANDER)) { // tavion, fencers, jedi trainer are all good at
                              // following up a parry with an attack
    if ((PM_SaberInParry(NPCS.NPC->client->ps.saberMove) ||
         PM_SaberInKnockaway(NPCS.NPC->client->ps.saberMove)) &&
        NPCS.NPC->client->ps.saberBlocked !=
            BLOCKED_PARRY_BROKEN) { // try to attack straight from a parry
      NPCS.NPC->client->ps.weaponTime = NPCS.NPCInfo->shotTime =
          NPCS.NPC->attackDebounceTime = 0;
      // NPC->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] = level.time +
      // 500;
      NPCS.NPC->client->ps.saberBlocked = BLOCKED_NONE;
      Jedi_AdjustSaberAnimLevel(
          NPCS.NPC, FORCE_LEVEL_1); // try to follow-up with a quick attack
      WeaponThink(qtrue);
      return qtrue;
    }
  }
  ...
  if (!(NPCS.ucmd.buttons & BUTTON_ATTACK) &&
      !(NPCS.ucmd.buttons & BUTTON_ALT_ATTACK)) { // not already attacking
    // Try to attack
    WeaponThink(qtrue);
  }
  ...
  if (NPCS.ucmd.buttons & BUTTON_ATTACK) { // attacking
    /*
    ...
    */
    ...
    if (!NPCS.ucmd.rightmove) { // not already strafing
      if (!Q_irand(0, 3)) {     // 25% chance of doing this
        ...
        VectorSubtract(NPCS.NPC->enemy->r.currentOrigin,
                       NPCS.NPC->r.currentAngles, dir2enemy);
...
static qboolean Jedi_Jump(vec3_t dest,
                          int goalEntNum) { // FIXME: if land on enemy, knock
                                            // him down & jump off again
  /*
  if ( dest[2] - NPC->r.currentOrigin[2] < 64 && DistanceHorizontal(
  NPC->r.currentOrigin, dest ) > 256 )
  ...
  */
  if (1) {
    float targetDist, shotSpeed = 300, travelTime, impactDist,
                      bestImpactDist = Q3_INFINITE; // fireSpeed,
    ...
    while (hitCount < maxHits) {
      ...
      {      // do a rough trace of the path
        ...
        // This may be kind of wasteful, especially on long throws... use larger
        // steps?  Divide the travelTime into a certain hard number of slices?
        // Trace just to apex and down?
        for (elapsedTime = timeStep; elapsedTime < floor(travelTime) + timeStep;
             elapsedTime += timeStep) {
          if ((float)elapsedTime > travelTime) { // cap it
            elapsedTime = floor(travelTime);
          }
          BG_EvaluateTrajectory(&tr, level.time + elapsedTime, testPos);
          if (testPos[2] < lastPos[2]) { // going down, ignore botclip
            trap->Trace(&trace, lastPos, NPCS.NPC->r.mins, NPCS.NPC->r.maxs,
                        testPos, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse,
                        0, 0);
          ...
          if (trace.fraction < 1.0f) { // hit something
            if (trace.entityNum ==
                goalEntNum) { // hit the enemy, that's perfect!
              // Hmm, don't want to land on him, though...
              break;
            } else {
              if (trace.contents &
                  CONTENTS_BOTCLIP) { // hit a do-not-enter brush
                blocked = qtrue;
                break;
              }
              if (trace.plane.normal[2] > 0.7 &&
                  DistanceSquared(trace.endpos, dest) <
                      4096) // hit within 64 of desired location, should be okay
              {             // close enough!
                break;
              } else { // FIXME: maybe find the extents of this brush and go
                       // above or below it on next try somehow?
          ...
          if (elapsedTime == floor(travelTime)) { // reached end, all clear
            if (trace.fraction >=
                1.0f) { // hmm, make sure we'll land on the ground...
              // FIXME: do we care how far below ourselves or our dest we'll
              // land?
              VectorCopy(trace.endpos, bottom);
              bottom[2] -= 128;
              trap->Trace(&trace, trace.endpos, NPCS.NPC->r.mins,
                          NPCS.NPC->r.maxs, bottom, NPCS.NPC->s.number,
        ...
        if (blocked) { // hit something, adjust speed (which will change arc)
          hitCount++;
          shotSpeed =
              300 + ((hitCount - 2) * 100); // from 100 to 900 (skipping 300)
          if (hitCount >=
              2) { // skip 300 since that was the first value we tested
            shotSpeed += 100;
          }
        } else { // made it!
          break;
    ...
    if (hitCount >=
        maxHits) { // NOTE: worst case scenario, use the one that impacted
                   // closest to the target (or just use the first try...?)
    ...
    if (NPCS.NPC->r.currentOrigin[2] >
        dest[2]) // NPCInfo->goalEntity->r.currentOrigin
    {
      VectorCopy(NPCS.NPC->r.currentOrigin, p1);
      VectorCopy(dest, p2); // NPCInfo->goalEntity->r.currentOrigin
    } else if (NPCS.NPC->r.currentOrigin[2] <
               dest[2]) // NPCInfo->goalEntity->r.currentOrigin
    {
    ...
    // FIXME: length of xy will change curve of parabola, need to account for
    // this somewhere... PARA_WIDTH
    /*
    ...
    */
    ...
    //		Com_Printf("apex is %4.2f percent from p1: ",
    //(xy-z)*0.5/xy*100.0f);
    ...
    time =
        sqrt(height /
             (.5 * NPCS.NPC->client->ps.gravity)); // was 0.5, but didn't work
                                                   // well for very long jumps
    if (!time) {
      // Com_Printf( S_COLOR_RED"ERROR: no time in jump\n" );
      return qfalse;
    }

    VectorSubtract(apex, NPCS.NPC->r.currentOrigin,
                   NPCS.NPC->client->ps.velocity);
    ...
    VectorScale(NPCS.NPC->client->ps.velocity, forward,
                NPCS.NPC->client->ps.velocity);

    // FIXME:  Uh.... should we trace/EvaluateTrajectory this to make sure we
    // have clearance and we land where we want?
    NPCS.NPC->client->ps.velocity[2] = time * NPCS.NPC->client->ps.gravity;

    // Com_Printf("Jump Velocity: %4.2f, %4.2f, %4.2f\n",
    // NPC->client->ps.velocity[0], NPC->client->ps.velocity[1],
    // NPC->client->ps.velocity[2] );
  }
  return qtrue;
}

static qboolean Jedi_TryJump(
    gentity_t *goal) { // FIXME: never does a simple short, regular jump...
  ...
  if (TIMER_Done(NPCS.NPC, "jumpChaseDebounce")) {
    if ((!goal->client || goal->client->ps.groundEntityNum != ENTITYNUM_NONE)) {
      if (!PM_InKnockDown(&NPCS.NPC->client->ps) &&
          !BG_InRoll(
              &NPCS.NPC->client->ps,
              NPCS.NPC->client->ps.legsAnim)) { // enemy is on terra firma
        vec3_t goal_diff;
        float goal_z_diff;
        float goal_xy_dist;
        VectorSubtract(goal->r.currentOrigin, NPCS.NPC->r.currentOrigin,
                       goal_diff);
        ...
        if (goal_xy_dist < 550 &&
            goal_z_diff >
                -400 /*was -256*/) // for now, jedi don't take falling damage &&
                                   // (NPC->health > 20 || goal_z_diff > 0 ) &&
                                   // (NPC->health >= 100 || goal_z_diff > -128
                                   // ))//closer than @512
        {
          qboolean debounce = qfalse;
          if (NPCS.NPC->health < 150 &&
              ((NPCS.NPC->health < 30 && goal_z_diff < 0) ||
               goal_z_diff < -128)) { // don't jump, just walk off... doesn't
                                      // help with ledges, though
            debounce = qtrue;
          } else if (goal_z_diff < 32 &&
                     goal_xy_dist < 200) { // what is their ideal jump height?
            ...
            */
            if (goal_z_diff > 0 || goal_xy_dist > 128) { // Fake a force-jump
              ...
              if (goal == NPCS.NPC->enemy) {
                int sideTry = 0;
                while (sideTry < 10) { // FIXME: make it so it doesn't try the
                                       // same spot again?
                  trace_t trace;
                  vec3_t bottom;
                  ...
                  bottom[2] -= 128;
                  trap->Trace(&trace, dest, NPCS.NPC->r.mins, NPCS.NPC->r.maxs,
                              bottom, goal->s.number, NPCS.NPC->clipmask,
                              qfalse, 0, 0);
              ...
              if (Jedi_Jump(dest, goal->s.number)) {
                ...
                // FIXME: store the dir we;re going in in case something gets in
                // the way of the jump? ? = vectoyaw( NPC->client->ps.velocity );
                /*
                ...
                */
                { // FIXME: make this a function call
                  int jumpAnim;
                  // FIXME: this should be more intelligent, like the normal
                  // force jump anim logic
                  if (NPCS.NPC->client->NPC_class == CLASS_BOBAFETT ||
                      (NPCS.NPCInfo->rank != RANK_CREWMAN &&
                       NPCS.NPCInfo->rank <=
                           RANK_LT_JG)) { // can't do acrobatics
                ...
                NPCS.NPC->client->ps.weaponTime =
                    NPCS.NPC->client->ps.torsoTimer;
                NPCS.NPC->client->ps.fd.forcePowersActive |=
                    (1 << FP_LEVITATION);
                if (NPCS.NPC->client->NPC_class == CLASS_BOBAFETT) {
                  G_SoundOnEnt(NPCS.NPC, CHAN_ITEM, "sound/boba/jeton.wav");
                  NPCS.NPC->client->jetPackTime =
                      level.time + Q_irand(1000, 3000);
                } else {
                  G_SoundOnEnt(NPCS.NPC, CHAN_BODY,
...
static qboolean Jedi_Jumping(gentity_t *goal) {
  if (!TIMER_Done(NPCS.NPC, "forceJumpChasing") &&
      goal) { // force-jumping at the enemy
    //		if ( !(NPC->client->ps.pm_flags & PMF_JUMPING )//forceJumpZStart
    //)
    //			&& !(NPC->client->ps.pm_flags&PMF_TRIGGER_PUSHED))
    if (NPCS.NPC->client->ps.groundEntityNum !=
        ENTITYNUM_NONE) // rwwFIXMEFIXME: Not sure if this is gonna work, use
                        // the PM flags ideally.
    { // landed
      TIMER_Set(NPCS.NPC, "forceJumpChasing", 0);
    } else {
      NPC_FaceEntity(goal, qtrue);
      // FIXME: push me torward where I was heading
      // FIXME: if hit a ledge as soon as we jumped, we need to push toward our
      // goal... must remember original jump dir and/or original jump dest
      /*
      ...
      //Cheat:
      if ( goal_dist < 128 && goal->r.currentOrigin[2] > NPC->r.currentOrigin[2]
...
static void Jedi_CheckEnemyMovement(float enemy_dist) {
  ...
  if (NPCS.NPC->client->NPC_class != CLASS_TAVION &&
      ...
      NPCS.NPC->client->NPC_class != CLASS_LUKE &&
      Q_stricmp("Yoda", NPCS.NPC->NPC_type)) {
    if (NPCS.NPC->enemy->enemy &&
        NPCS.NPC->enemy->enemy == NPCS.NPC) { // enemy is mad at *me*
      if (NPCS.NPC->enemy->client->ps.legsAnim == BOTH_JUMPFLIPSLASHDOWN1 ||
          NPCS.NPC->enemy->client->ps.legsAnim ==
              BOTH_JUMPFLIPSTABDOWN) { // enemy is flipping over me
        if (Q_irand(0, NPCS.NPCInfo->rank) <
            RANK_LT) { // be nice and stand still for him...
          ...
          TIMER_Set(NPCS.NPC, "movecenter", Q_irand(500, 1000));
                 ...
                 NPCS.NPC->enemy->client->ps.legsAnim ==
                     BOTH_WALL_RUN_RIGHT_FLIP) { // he's flipping off a wall
        if (NPCS.NPC->enemy->client->ps.groundEntityNum ==
            ENTITYNUM_NONE) {     // still in air
          if (enemy_dist < 256) { // close
            if (Q_irand(0, NPCS.NPCInfo->rank) <
              ...
              */
              ...
              NPCS.ucmd.forwardmove = NPCS.ucmd.rightmove = NPCS.ucmd.upmove =
                  0;
              VectorClear(NPCS.NPC->client->ps.moveDir);
              NPCS.NPC->client->ps.fd.forceJumpCharge = 0;
              TIMER_Set(NPCS.NPC, "strafeLeft", -1);
              TIMER_Set(NPCS.NPC, "strafeRight", -1);
              TIMER_Set(NPCS.NPC, "noStrafe", Q_irand(500, 1000));
              TIMER_Set(NPCS.NPC, "noturn",
                        Q_irand(250, 500) * (3 - g_npcspskill.integer));
      ...
      } else if (NPCS.NPC->enemy->client->ps.legsAnim ==
                 BOTH_A2_STABBACK1) {              // he's stabbing backwards
        if (enemy_dist < 256 && enemy_dist > 64) { // close
          if (!InFront(NPCS.NPC->r.currentOrigin,
                       NPCS.NPC->enemy->r.currentOrigin,
                       NPCS.NPC->enemy->r.currentAngles, 0.0f)) { // behind him
            if (!Q_irand(
                    0,
                    NPCS.NPCInfo->rank)) { // be nice and stand still for him...
              vec3_t enemyFwd, dest, dir;
              ...
              AngleVectors(NPCS.NPC->enemy->r.currentAngles, enemyFwd, NULL,
                           NULL);
  ...
  //		If enemy doing back-attack and we're behind him keep running forward
  //toward his back, don't strafe
}
...
static void Jedi_CheckJumps(void) {
  ...
  // FIXME: should probably check this before AI decides that best move is to
  // jump?  Otherwise, they may end up just standing there and looking dumb
  // FIXME: all this work and he still jumps off ledges... *sigh*... need
  // CONTENTS_BOTCLIP do-not-enter brushes...?
  VectorClear(jumpVel);
  ...
  // NOTE: for now, we clear ucmd.forwardmove & ucmd.rightmove while in air to
  // avoid jumps going awry...
  if (!jumpVel[0] &&
      !jumpVel[1]) // FIXME: && !ucmd.forwardmove && !ucmd.rightmove?
  {                // we assume a jump straight up is safe
    // Com_Printf( "(%d) jump straight up is safe\n", level.time );
    return;
  }
  // Now predict where this is going
  // in steps, keep evaluating the trajectory until the new z pos is <= than
  // current z pos, trace down from there
  ...
  // This may be kind of wasteful, especially on long throws... use larger
  // steps?  Divide the travelTime into a certain hard number of slices?  Trace
  // just to apex and down?
  for (elapsedTime = 500; elapsedTime <= 4000; elapsedTime += 500) {
    BG_EvaluateTrajectory(&tr, level.time + elapsedTime, testPos);
    // FIXME: account for PM_AirMove if ucmd.forwardmove and/or ucmd.rightmove
    // is non-zero...
    if (testPos[2] < lastPos[2]) { // going down, don't check for BOTCLIP
      trap->Trace(&trace, lastPos, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, testPos,
                  NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0,
                  0); // FIXME: include CONTENTS_BOTCLIP?
    } else {          // going up, check for BOTCLIP
      trap->Trace(&trace, lastPos, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, testPos,
                  NPCS.NPC->s.number, NPCS.NPC->clipmask | CONTENTS_BOTCLIP,
                  qfalse, 0, 0);
    }
    if (trace.allsolid || trace.startsolid) { // WTF?
      // FIXME: what do we do when we start INSIDE the CONTENTS_BOTCLIP?  Do the
      // trace again without that clipmask?
      goto jump_unsafe;
    }
    if (trace.fraction < 1.0f) {               // hit something
  ...
  VectorCopy(trace.endpos, bottom);
  if (bottom[2] > NPCS.NPC->r.currentOrigin[2]) { // only care about dist down
                                                  // from current height or lower
    bottom[2] = NPCS.NPC->r.currentOrigin[2];
  } else if (NPCS.NPC->r.currentOrigin[2] - bottom[2] >
             400) { // whoa, long drop, don't do it!
  ...
  trap->Trace(&trace, trace.endpos, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, bottom,
              NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0, 0);
  if (trace.allsolid || trace.startsolid ||
      trace.fraction < 1.0f) {               // hit ground!
    if (trace.entityNum < ENTITYNUM_WORLD) { // landed on an ent
      gentity_t *groundEnt = &g_entities[trace.entityNum];
      if (groundEnt->r.svFlags &
          SVF_GLASS_BRUSH) { // don't land on breakable glass!
        goto jump_unsafe;
      }
    }
...
static void Jedi_Combat(void) {
  ...
  Jedi_SetEnemyInfo(enemy_dest, enemy_dir, &enemy_dist, enemy_movedir,
                    &enemy_movespeed, 300);

  if (Jedi_Jumping(NPCS.NPC->enemy)) { // I'm in the middle of a jump, so just
                                       // see if I should attack
  ...
  if (!(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_GRIP)) ||
      NPCS.NPC->client->ps.fd.forcePowerLevel[FP_GRIP] <
          FORCE_LEVEL_2) { // not gripping
    // If we can't get straight at him
    if (!Jedi_ClearPathToSpot(enemy_dest,
                              NPCS.NPC->enemy->s.number)) { // hunt him down
      // Com_Printf( "No Clear Path\n" );
      if ((NPC_ClearLOS4(NPCS.NPC->enemy) ||
           NPCS.NPCInfo->enemyLastSeenTime > level.time - 500) &&
          NPC_FaceEnemy(qtrue)) //( NPCInfo->rank == RANK_CREWMAN ||
                                //NPCInfo->rank > RANK_LT_JG ) &&
      {
        ...
        trap->Trace( &trace, NPC->r.currentOrigin, NPC->r.mins, NPC->r.maxs,
        end, NPC->s.number, NPC->clipmask|CONTENTS_BOTCLIP ); if (
        !trace.allsolid && !trace.startsolid && trace.fraction >= 1.0 )
        {
                ...
                trap->Trace( &trace, NPC->r.currentOrigin, NPC->r.mins,
        NPC->r.maxs, end, NPC->s.number, NPC->clipmask|CONTENTS_BOTCLIP ); if (
        ...
        */
        ...
        if (Jedi_TryJump(NPCS.NPC->enemy)) { // FIXME: what about jumping to his
                                             // enemyLastSeenLocation?
          return;
        }
      }

      // Check for evasion
      if (TIMER_Done(NPCS.NPC, "parryTime")) { // finished parrying
        if (NPCS.NPC->client->ps.saberBlocked != BLOCKED_ATK_BOUNCE &&
            NPCS.NPC->client->ps.saberBlocked !=
                BLOCKED_PARRY_BROKEN) { // wasn't blocked myself
          NPCS.NPC->client->ps.saberBlocked = BLOCKED_NONE;
      ...
      if (Jedi_Hunt() && !(NPCS.NPCInfo->aiFlags &
                           NPCAI_BLOCKED)) // FIXME: have to do this because
                                           // they can ping-pong forever
      { // can macro-navigate to him
        if (enemy_dist < 384 && !Q_irand(0, 10) &&
            NPCS.NPCInfo->blockedSpeechDebounceTime < level.time &&
            jediSpeechDebounceTime[NPCS.NPC->client->playerTeam] < level.time &&
      ...
      */
      else { // FIXME: try to find a waypoint that can see enemy, jump from
             // there
        if (NPCS.NPCInfo->aiFlags &
            NPCAI_BLOCKED) {               // try to jump to the blockedDest
  ...
  // We call this even if lost enemy to keep him moving and to update the
  // taunting behavior maintain a distance from enemy appropriate for our
  // aggression level
  ...
  if (!NPCS.NPC->client->ps.saberInFlight &&
      (!(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_GRIP)) ||
       NPCS.NPC->client->ps.fd.forcePowerLevel[FP_GRIP] <
           FORCE_LEVEL_2)) { // not throwing saber or using force grip
    // see if we can attack
    if (!Jedi_AttackDecide(
            enemy_dist)) { // we're not attacking, decide what else to do
      Jedi_CombatIdle(enemy_dist);
      // FIXME: lower aggression when actually strike offensively?  Or just when
      // do damage?
  ...
  if (!NPC_MoveDirClear(
          ...
          qtrue)) { // uh-oh, we are going to fall or hit something
    navInfo_t info;
    // Get the move info
    NAV_GetLastMove(&info);
    if (!(info.flags & NIF_MACRO_NAV)) { // micro-navigation told us to step off
                                         // a ledge, try using macronav for now
...
void NPC_Jedi_Pain(gentity_t *self, gentity_t *attacker, int damage) {
  ...
  if (other->s.weapon == WP_SABER) { // back off
    ...
    if (self->client->NPC_class == CLASS_DESANN ||
        !Q_stricmp("Yoda", self->NPC_type)) { // less for Desann
      self->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] =
          level.time + (3 - g_npcspskill.integer) * 50;
    } else if (self->NPC->rank >= RANK_LT_JG) {
      self->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] =
          level.time + (3 - g_npcspskill.integer) * 100; // 300
    } else {
      self->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] =
          level.time + (3 - g_npcspskill.integer) * 200; // 500
    }
    if (!Q_irand(
            0,
            3)) { // ouch... maybe switch up which saber power level we're using
    ...
    if (d_JediAI.integer) {
      Com_Printf("(%d) PAIN: agg %d, no parry until %d\n", level.time,
                 self->NPC->stats.aggression, level.time + 500);
    }
    // for testing only
  ...
  if (self->client->ps.legsAnim == BOTH_CEILING_CLING) {
    NPC_SetAnim(self, SETANIM_LEGS, BOTH_CEILING_DROP,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  }
  if (self->client->ps.torsoAnim == BOTH_CEILING_CLING) {
    NPC_SetAnim(self, SETANIM_TORSO, BOTH_CEILING_DROP,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  }
}
...
qboolean Jedi_CheckDanger(void) {
  ...
  if (level.alertEvents[alertEvent].level >= AEL_DANGER) { // run away!
    if (!level.alertEvents[alertEvent].owner ||
        ...
        (level.alertEvents[alertEvent].owner != NPCS.NPC &&
         level.alertEvents[alertEvent].owner->client->playerTeam !=
             NPCS.NPC->client->playerTeam)) { // no owner
      return qfalse;
    }
...
qboolean Jedi_CheckAmbushPlayer(void) {
  ...
  for (i = 0; i < MAX_CLIENTS; i++) {
    ...
    //		if ( NPC->client->ps.powerups[PW_CLOAKED] || g_crosshairEntNum !=
    //NPC->s.number )
    if (NPCS.NPC->client->ps.powerups[PW_CLOAKED] ||
        !NPC_SomeoneLookingAtMe(
            NPCS.NPC)) // rwwFIXMEFIXME: Need to pay attention to who is under
                       // crosshair for each player or something.
    { // if I'm not cloaked and the player's crosshair is on me, I will wake up,
      // otherwise do this stuff down here...
      if (!trap->InPVS(player->r.currentOrigin,
                       NPCS.NPC->r.currentOrigin)) { // must be in same room
        continue;
      } else {
        if (!NPCS.NPC->client->ps.powerups[PW_CLOAKED]) {
      ...
      if (zDiff <= 0 ||
          zDiff > 512) { // never ambush if they're above me or way way below me
        continue;
      }
      ...
      if ((target_dist = DistanceHorizontalSquared(player->r.currentOrigin,
                                                   NPCS.NPC->r.currentOrigin)) >
          4096) {                   // closer than 64 - always ambush
...
void Jedi_Ambush(gentity_t *self) {
  self->client->noclip = qfalse;
  //	self->client->ps.pm_flags |= PMF_JUMPING|PMF_SLOW_MO_FALL;
  NPC_SetAnim(self, SETANIM_BOTH, BOTH_CEILING_DROP,
              SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
  self->client->ps.weaponTime =
      self->client->ps.torsoTimer; // NPC->client->ps.torsoTimer; //what the?
  if (self->client->NPC_class != CLASS_BOBAFETT) {
...
static void Jedi_Patrol(void) {
  ...
  if (Jedi_WaitingAmbush(NPCS.NPC)) { // hiding on the ceiling
    NPC_SetAnim(NPCS.NPC, SETANIM_BOTH, BOTH_CEILING_CLING,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
    if (NPCS.NPCInfo->scriptFlags & SCF_LOOK_FOR_ENEMIES) { // look for enemies
  ...
  } else if (NPCS.NPCInfo->scriptFlags &
             SCF_LOOK_FOR_ENEMIES) // NPCInfo->scriptFlags & SCF_CHASE_ENEMIES )
  {                                // look for enemies
    ...
    for (i = 0; i < ENTITYNUM_WORLD; i++) {
      ...
      if (enemy && enemy->client && NPC_ValidEnemy(enemy) &&
          enemy->client->playerTeam == NPCS.NPC->client->enemyTeam) {
        if (trap->InPVS(
                NPCS.NPC->r.currentOrigin,
                enemy->r.currentOrigin)) { // we could potentially see him
          enemy_dist = DistanceSquared(NPCS.NPC->r.currentOrigin,
                                       enemy->r.currentOrigin);
          if (enemy->s.eType == ET_PLAYER || enemy_dist < best_enemy_dist) {
            // if the enemy is close enough, or threw his saber, take him as the
            // enemy
            // FIXME: what if he throws a thermal detonator?
            if (enemy_dist < (220 * 220) ||
                (NPCS.NPCInfo->investigateCount >= 3 &&
                 !NPCS.NPC->client->ps.saberHolstered)) {
              ...
              break;
                       ...
                       !enemy->client->ps
                            .saberHolstered) { // threw his saber, see if it's
                                               // heading toward me and close
                                               // enough to consider a threat
              float saberDist;
              vec3_t saberDir2Me;
              vec3_t saberMoveDir;
              ...
              if (DotProduct(saberMoveDir, saberDir2Me) >
                ...
                if (saberDist < 200) { // incoming!
                  G_SetEnemy(NPCS.NPC, enemy);
                  // NPCInfo->behaviorState = BS_HUNT_AND_KILL;//should be auto
                  // now
                  NPCS.NPCInfo->stats.aggression = 3;
                  break;
                }
    ...
    if (!NPCS.NPC->enemy) { // still not mad
      if (!best_enemy) {
        ...
        if (NPC_ClearLOS4(
                best_enemy)) { // we have a clear (of architecture) LOS to him
          if (best_enemy->s.number) { // just attack
            G_SetEnemy(NPCS.NPC, best_enemy);
            NPCS.NPCInfo->stats.aggression = 3;
          } else if (NPCS.NPC->client->NPC_class !=
                     CLASS_BOBAFETT) { // the player, toy with him
            // get progressively more interested over time
            if (TIMER_Done(NPCS.NPC,
                           "watchTime")) { // we want to pick him up in stages
              if (TIMER_Get(NPCS.NPC, "watchTime") ==
                  -1) { // this is the first time, we'll ignore him for a couple
                        // seconds
                ...
                if (!NPCS.NPCInfo->investigateCount) {
                  G_AddVoiceEvent(NPCS.NPC,
                                  Q_irand(EV_JDETECTED1, EV_JDETECTED3), 3000);
                }
            ...
            if (best_enemy_dist < (440 * 440) ||
                NPCS.NPCInfo->investigateCount >=
                    2) { // stage three: keep facing him
              NPC_FaceEntity(best_enemy, qtrue);
              if (best_enemy_dist <
                  (330 * 330)) { // stage four: turn on the saber
                if (!NPCS.NPC->client->ps.saberInFlight) {
                  WP_ActivateSaber(NPCS.NPC);
                }
              }
            } else if (best_enemy_dist < (550 * 550) ||
                       NPCS.NPCInfo->investigateCount ==
                           1) { // stage two: stop and face him every now and
                                // then
              ...
              NPC_SetLookTarget(NPCS.NPC, best_enemy->s.number, 0);
                              ...
                              "watchTime")) { // haven't seen him in a bit,
                                              // clear the lookTarget
...
finish:
  ...
  if (NPCS.NPC->enemy) { // just picked one up
    NPCS.NPCInfo->enemyCheckDebounceTime = level.time + Q_irand(3000, 10000);
  }
}

qboolean Jedi_CanPullBackSaber(gentity_t *self) {
  if (self->client->ps.saberBlocked == BLOCKED_PARRY_BROKEN &&
      !TIMER_Done(self, "parryTime")) {
    return qfalse;
  }
  ...
  if (self->painDebounceTime >
      level.time) //|| self->client->ps.weaponTime > 0 )
  {
    return qfalse;
  }
...
void NPC_BSJedi_FollowLeader(void) {
  ...
  if (NPCS.NPC->client->ps.saberInFlight) { // saber is not in hand
    if (NPCS.NPC->client->ps.saberEntityNum < ENTITYNUM_NONE &&
        NPCS.NPC->client->ps.saberEntityNum > 0) // player is 0
    {                                            //
      if (g_entities[NPCS.NPC->client->ps.saberEntityNum].s.pos.trType ==
          TR_STATIONARY) { // fell to the ground, try to pick it up...
        if (Jedi_CanPullBackSaber(NPCS.NPC)) {
          // FIXME: if it's on the ground and we just pulled it back to us,
          // should we 		stand still for a bit to make sure it gets to us...?
          //		otherwise we could end up running away from it while it's on
          //its 		way back to us and we could lose it again.
          NPCS.NPC->client->ps.saberBlocked = BLOCKED_NONE;
          NPCS.NPCInfo->goalEntity =
              &g_entities[NPCS.NPC->client->ps.saberEntityNum];
          NPCS.ucmd.buttons |= BUTTON_ATTACK;
          if (NPCS.NPC->enemy &&
              NPCS.NPC->enemy->health > 0) { // get our saber back NOW!
            if (!NPC_MoveToGoal(
                    qtrue)) // Jedi_Move( NPCInfo->goalEntity, qfalse );
  ...
  if (NPCS.NPCInfo->goalEntity) {
    ...
    if (Jedi_Jumping(NPCS.NPCInfo->goalEntity)) { // in mid-jump
      return;
    }

    if (!NAV_CheckAhead(NPCS.NPC, NPCS.NPCInfo->goalEntity->r.currentOrigin,
                        &trace,
                        (NPCS.NPC->clipmask & ~CONTENTS_BODY) |
                            CONTENTS_BOTCLIP)) { // can't get straight to him
      if (NPC_ClearLOS4(NPCS.NPCInfo->goalEntity) &&
          NPC_FaceEntity(NPCS.NPCInfo->goalEntity, qtrue)) { // no line of sight
        if (Jedi_TryJump(NPCS.NPCInfo->goalEntity)) {        // started a jump
          return;
        }
      }
    }
    if (NPCS.NPCInfo->aiFlags &
        NPCAI_BLOCKED) { // try to jump to the blockedDest
      if (fabs(NPCS.NPCInfo->blockedDest[2] - NPCS.NPC->r.currentOrigin[2]) >
          64) {
        gentity_t *tempGoal = G_Spawn(); // ugh, this is NOT good...?
        G_SetOrigin(tempGoal, NPCS.NPCInfo->blockedDest);
        trap->LinkEntity((sharedEntity_t *)tempGoal);
...
static void Jedi_Attack(void) {
  ...
  if (NPCS.NPC->client->ps.saberLockTime > level.time) {
    // FIXME: maybe if I'm losing I should try to force-push out of it?  Very
    // rarely, though...
    ...
    // based on my skill, hit attack button every other to every several frames
    // in order to push enemy back
    else {
      float chance;
      ...
      } else { // the escalation in difficulty is nice, here, but cap it so it
               // doesn't get *impossible* on hard
  ...
  if (NPCS.NPC->client->ps.saberInFlight) { // saber is not in hand
    ...
    if (!NPCS.NPC->client->ps.saberEntityNum &&
        NPCS.NPC->client->saberStoredIndex) // this is valid, it's 0 when our
                                            // saber is gone -rww (mp-specific)
    { //
      // if ( g_entities[NPC->client->ps.saberEntityNum].s.pos.trType ==
      // TR_STATIONARY )
      if (1) // no matter
      {      // fell to the ground, try to pick it up
        ...
        if (1) // no matter
        {
          NPCS.NPC->client->ps.saberBlocked = BLOCKED_NONE;
          NPCS.NPCInfo->goalEntity =
              &g_entities[NPCS.NPC->client->saberStoredIndex];
          NPCS.ucmd.buttons |= BUTTON_ATTACK;
          if (NPCS.NPC->enemy &&
              NPCS.NPC->enemy->health > 0) { // get our saber back NOW!
            Jedi_Move(NPCS.NPCInfo->goalEntity, qfalse);
            NPC_UpdateAngles(qtrue, qtrue);
            if (NPCS.NPC->enemy->s.weapon ==
                WP_SABER) { // be sure to continue evasion
              vec3_t enemy_dir, enemy_movedir, enemy_dest;
              float enemy_dist, enemy_movespeed;
              Jedi_SetEnemyInfo(enemy_dest, enemy_dir, &enemy_dist,
                                enemy_movedir, &enemy_movespeed, 300);
              Jedi_EvasionSaber(enemy_movedir, enemy_dist, enemy_dir);
            }
            return;
          }
  ...
  // see if our enemy was killed by us, gloat and turn off saber after cool
  // down.
  // FIXME: don't do this if we have other enemies to fight...?
  if (NPCS.NPC->enemy) {
    if (NPCS.NPC->enemy->health <= 0 && NPCS.NPC->enemy->enemy == NPCS.NPC &&
        NPCS.NPC->client->playerTeam != NPCTEAM_PLAYER) // good guys don't gloat
    { // my enemy is dead and I killed him
      NPCS.NPCInfo->enemyCheckDebounceTime = 0; // keep looking for others

      if (NPCS.NPC->client->NPC_class == CLASS_BOBAFETT) {
        if (NPCS.NPCInfo->walkDebounceTime < level.time &&
            NPCS.NPCInfo->walkDebounceTime >= 0) {
        ...
        if (!TIMER_Done(NPCS.NPC, "gloatTime")) {
          if (DistanceHorizontalSquared(NPCS.NPC->client->renderInfo.eyePoint,
                                        NPCS.NPC->enemy->r.currentOrigin) >
                  4096 &&
              (NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) // 64 squared
          {
            NPCS.NPCInfo->goalEntity = NPCS.NPC->enemy;
            Jedi_Move(NPCS.NPC->enemy, qfalse);
            NPCS.ucmd.buttons |= BUTTON_WALKING;
        ...
        if (!TIMER_Done(NPCS.NPC, "parryTime")) {
          TIMER_Set(NPCS.NPC, "parryTime", -1);
          NPCS.NPC->client->ps.fd.forcePowerDebounce[FP_SABER_DEFENSE] =
              level.time + 500;
        }
        NPCS.NPC->client->ps.saberBlocked = BLOCKED_NONE;
        if (!NPCS.NPC->client->ps.saberHolstered &&
            NPCS.NPC->client->ps
                .saberInFlight) { // saber is still on (or we're trying to pull
                                  // it back), count down erosion and keep facing
                                  // the enemy
          // FIXME: need to stop this from happening over and over again when
          // they're blocking their victim's saber
          // FIXME: turn off saber sooner so we get cool walk anim?
          // Com_Printf( "(%d) drop agg - enemy dead\n", level.time );
          Jedi_AggressionErosion(-3);
          if (BG_SabersOff(&NPCS.NPC->client->ps) &&
              !NPCS.NPC->client->ps
                   .saberInFlight) { // turned off saber (in hand), gloat
            G_AddVoiceEvent(NPCS.NPC, Q_irand(EV_VICTORY1, EV_VICTORY3), 3000);
            jediSpeechDebounceTime[NPCS.NPC->client->playerTeam] =
                level.time + 3000;
            NPCS.NPCInfo->desiredPitch = 0;
        ...
        if (!NPCS.NPC->client->ps.saberHolstered ||
            NPCS.NPC->client->ps.saberInFlight ||
            !TIMER_Done(NPCS.NPC, "gloatTime")) { // keep walking
          if (DistanceHorizontalSquared(NPCS.NPC->client->renderInfo.eyePoint,
                                        NPCS.NPC->enemy->r.currentOrigin) >
                  4096 &&
              (NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES)) // 64 squared
          {
            ...
            if (NPCS.NPC->health < NPCS.NPC->client->pers.maxHealth &&
                (NPCS.NPC->client->ps.fd.forcePowersKnown & (1 << FP_HEAL)) !=
                    0 &&
                (NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_HEAL)) ==
                    0) {
              ForceHeal(NPCS.NPC);
            }
          }
  ...
  if (NPCS.NPC->enemy->s.weapon == WP_TURRET &&
      !Q_stricmp("PAS", NPCS.NPC->enemy->classname)) {
    if (NPCS.NPC->enemy->count <= 0) { // it's out of ammo
      if (NPCS.NPC->enemy->activator &&
          NPC_ValidEnemy(NPCS.NPC->enemy->activator)) {
  ...
  if (!NPCS.NPC->enemy) {
    ...
    if (NPCS.NPCInfo->tempBehavior ==
        BS_HUNT_AND_KILL) { // lost him, go back to what we were doing before
  ...
  if (!(NPCS.NPCInfo->scriptFlags & SCF_CHASE_ENEMIES) ||
      ((NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_HEAL)) &&
       NPCS.NPC->client->ps.fd.forcePowerLevel[FP_HEAL] <
           FORCE_LEVEL_2)) { // this is really stupid, but okay...
    NPCS.ucmd.forwardmove = 0;
    NPCS.ucmd.rightmove = 0;
    if (NPCS.ucmd.upmove > 0) {
      NPCS.ucmd.upmove = 0;
  ...
  // avoid jumps going awry...
  if (NPCS.NPC->client->ps.groundEntityNum ==
      ENTITYNUM_NONE) { // don't push while in air, throws off jumps!
    // FIXME: if we are in the air over a drop near a ledge, should we try to
    // push back towards the ledge?
    NPCS.ucmd.forwardmove = 0;
    NPCS.ucmd.rightmove = 0;
    VectorClear(NPCS.NPC->client->ps.moveDir);
  }
  ...
  if (NPCS.NPC->client->NPC_class != CLASS_BOBAFETT) {
    if (PM_SaberInBrokenParry(NPCS.NPC->client->ps.saberMove) ||
        NPCS.NPC->client->ps.saberBlocked ==
            BLOCKED_PARRY_BROKEN) { // just make sure they don't pull their
                                    // saber to them if they're being blocked
      NPCS.ucmd.buttons &= ~BUTTON_ATTACK;
    }
  }
  ...
  if ((NPCS.NPCInfo->scriptFlags & SCF_DONT_FIRE) // not allowed to attack
      || ((NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_HEAL)) &&
          NPCS.NPC->client->ps.fd.forcePowerLevel[FP_HEAL] < FORCE_LEVEL_3) ||
      ((NPCS.NPC->client->ps.saberEventFlags & SEF_INWATER) &&
       !NPCS.NPC->client->ps.saberInFlight)) // saber in water
  {
    NPCS.ucmd.buttons &= ~(BUTTON_ATTACK | BUTTON_ALT_ATTACK);
  }
  ...
  if (NPCS.ucmd.buttons & BUTTON_ATTACK &&
    ...
    if (Q_irand(0, NPCS.NPC->client->ps.fd.saberAnimLevel) > 0 &&
        ...
        !Q_irand(0, 3)) { // the more we're hurt and the stronger the attack
                          // we're using, the more likely we are to make a anger
                          // noise when we swing
  ...
  if (NPCS.NPC->client->NPC_class != CLASS_BOBAFETT) {
    if (NPCS.NPC->client->NPC_class == CLASS_TAVION ||
        (g_npcspskill.integer &&
         (NPCS.NPC->client->NPC_class == CLASS_DESANN ||
          NPCS.NPCInfo->rank >=
              Q_irand(RANK_CREWMAN,
                      RANK_CAPTAIN)))) { // Tavion will kick in force speed if
                                         // the player does...
      if (NPCS.NPC->enemy && NPCS.NPC->enemy->s.number >= 0 &&
          NPCS.NPC->enemy->s.number < MAX_CLIENTS && NPCS.NPC->enemy->client &&
          (NPCS.NPC->enemy->client->ps.fd.forcePowersActive &
           (1 << FP_SPEED)) &&
          !(NPCS.NPC->client->ps.fd.forcePowersActive & (1 << FP_SPEED))) {
        int chance = 0;
...
qboolean Jedi_InSpecialMove(void) {
  ...
  if (NPCS.NPC->client->ps.torsoAnim == BOTH_FORCE_DRAIN_GRAB_START ||
    ...
    if (!TIMER_Done(NPCS.NPC, "draining")) { // FIXME: what do we do if we ran
                                             // out of power?  NPC's can't?
      // FIXME: don't keep turning to face enemy or we'll end up spinning around
      NPCS.ucmd.buttons |= BUTTON_FORCE_DRAIN;
    }
    NPC_UpdateAngles(qtrue, qtrue);
  ...
  NPC->weaponModel[1], NPC->genericBolt1, NPC->s.number );
                  NPC->client->ps.legsTimer = NPC->client->ps.torsoTimer = 0;
                  NPC_SetAnim( NPC, SETANIM_BOTH, BOTH_SCEPTER_STOP,
  SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD ); NPC->painDebounceTime = level.time
  + NPC->client->ps.torsoTimer; NPC->client->ps.pm_time =
  NPC->client->ps.torsoTimer; NPC->client->ps.pm_flags |= PMF_TIME_KNOCKBACK;
                  VectorClear( NPC->client->ps.velocity );
                  VectorClear( NPC->client->ps.moveDir );
          }
          else
  ...
  if (Jedi_CultistDestroyer(NPCS.NPC)) {
    if (!NPCS.NPC->takedamage) { // ready to explode
      if (NPCS.NPC->useDebounceTime <= level.time) {
        ...
        NPCS.NPC->client->playerTeam =
            NPCTEAM_FREE; // FIXME: will this destroy wampas, tusken & rancors?
...
void NPC_BSJedi_Default(void) {
  ...
  if (!NPCS.NPC->enemy) { // don't have an enemy, look for one
    if (NPCS.NPC->client->NPC_class == CLASS_BOBAFETT) {
      NPC_BSST_Patrol();
    } else {
      Jedi_Patrol();
  ...
  {      // have an enemy
    if (Jedi_WaitingAmbush(
            NPCS.NPC)) { // we were still waiting to drop down - must have had
                         // enemy set on me outside my AI
      Jedi_Ambush(NPCS.NPC);
    }

    if (Jedi_CultistDestroyer(NPCS.NPC) &&
        !NPCS.NPCInfo->charmedTime) { // destroyer
      // permanent effect
      ...
      // NPC->client->ps.eFlags |= EF_FORCE_DRAINED;
      // FIXME: precache me!
      NPCS.NPC->s.loopSound = G_SoundIndex(
          "sound/movers/objects/green_beam_lp2.wav"); // test/charm.wav" );
    }

    if (NPCS.NPC->client->NPC_class == CLASS_BOBAFETT) {
      if (NPCS.NPC->enemy->enemy != NPCS.NPC &&
          NPCS.NPC->health == NPCS.NPC->client->pers.maxHealth &&
          DistanceSquared(NPCS.NPC->r.currentOrigin,
                          NPCS.NPC->enemy->r.currentOrigin) > (800 * 800)) {
        NPCS.NPCInfo->scriptFlags |= SCF_ALT_FIRE;
        Boba_ChangeWeapon(WP_DISRUPTOR);
    ...
    Jedi_Attack();
    // if we have multiple-jedi combat, probably need to keep checking (at
    // certain debounce intervals) for a better (closer, more active) enemy and
    // switch if needbe...
    if (((!NPCS.ucmd.buttons && !NPCS.NPC->client->ps.fd.forcePowersActive) ||
        ...
        NPCS.NPCInfo->enemyCheckDebounceTime <
            level.time) { // not doing anything (or walking toward a vanquished
                          // enemy - fixme: always taunt the player?), not using
                          // force powers and it's time to look again
      // FIXME: build a list of all local enemies (since we have to find best
      // anyway) for other AI factors- like when to use group attacks, determine
      // when to change tactics, when surrounded, when blocked by another in the
      // enemy group, etc.  Should we build this group list or let the enemies
      // maintain their own list and we just access it?
      gentity_t *sav_enemy =
          NPCS.NPC->enemy; // FIXME: what about NPC->lastEnemy?
      gentity_t *newEnemy;

      NPCS.NPC->enemy = NULL;
      newEnemy = NPC_CheckEnemy(
          (qboolean)(NPCS.NPCInfo->confusionTime < level.time), qfalse, qfalse);
